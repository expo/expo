...
function validate(data: unknown): ReviewPayload {
  ...
  if (!["APPROVE", "REQUEST_CHANGES", "COMMENT"].includes(obj.verdict as string)) {
    throw new Error(
      `Invalid verdict: ${obj.verdict}. Must be APPROVE, REQUEST_CHANGES, or COMMENT`
    );
  }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "submit": {
      ...
      const event = (args[3] as "APPROVE" | "REQUEST_CHANGES" | "COMMENT") ?? review.verdict;
      if (!["APPROVE", "REQUEST_CHANGES", "COMMENT"].includes(event)) {
        console.error(`Invalid event: ${event}. Must be APPROVE, REQUEST_CHANGES, or COMMENT`);
        process.exit(1);
      }
    ...
    default:
      console.error(`Unknown command: ${command}. Use 'preview', 'post', or 'submit'.`);
      process.exit(1);
  }
}
