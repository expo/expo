...
**REJECT verdict:** Use for AI-generated slop, spam, or PRs that are fundamentally unfit (wrong repo, completely unrelated changes, etc.). For REJECT PRs, after showing the verdict table, offer to close the PR with a short comment explaining why: `gh pr comment <PR_URL> --body "<reason>"` then `gh pr close <PR_URL>`. Always confirm with the user before closing.
...
**Review length must be proportional to the change.** A 4-line docs fix does not need a 4-paragraph review with numbered findings. For small/simple PRs: one short summary paragraph, inline comments only where needed. Save deep analysis for PRs that warrant it (large diffs, architectural changes, tricky logic). When in doubt, be shorter.
