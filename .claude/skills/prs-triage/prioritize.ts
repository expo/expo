...
function printResults(
  ...
  for (const pr of quickWins.slice(0, 25)) {
    ...
    const tags = [pr.is_reviewed && "*REVIEWED*"].filter(Boolean).join(" ");
  ...
  console.log("\n=== Aging (unreviewed, 14+ days old) ===\n");

  if (aging.length === 0) {
    console.log("  No unreviewed PRs older than 14 days.");
  } else {
    for (const pr of aging.slice(0, 25)) {
      const diff = `+${pr.additions}/-${pr.deletions}`;
      const weeks = Math.floor(pr.age_days / 7);
      const ageStr = weeks >= 4
        ? `${Math.floor(pr.age_days / 30)}mo`
        : `${weeks}w`;
      console.log(
        `  ${pr.url}` +
          `  T${pr.tier}` +
          `  ${ageStr.padStart(3)} old` +
          `  Comp:${String(pr.composite_score).padStart(2)}` +
          `  ${diff.padEnd(12)}` +
          `${String(pr.changed_files).padStart(3)} files`,
      );
...
function main(): void {
  ...
  const aging = [...prioritized]
    .filter((p) => !p.is_reviewed && p.age_days >= 14)
  ...
  printResults(byQuickWin, byImpact, aging);
}
