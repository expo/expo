...
- **Design fit** - Respects module boundaries? Follows existing patterns and AGENTS.md file for the modified package (if present)? Appropriate abstraction level?
...
Resolve the output directory by running `bun run .Codex/skills/deep-code-review/review-dir.ts` — it prints the path and creates it if needed. Write findings to `<output_dir>/code-review-{pr_number}.json` (one file per PR). The `summary` field **must** start with `_🤖 This is an automated review. Addressing it doesn't guarantee a merge._` — this is required so readers know the review is AI-generated.
...
3. Optionally, verify line placement (ask user if they want it): `bun run .Codex/skills/deep-code-review/post-review.ts local-preview <output_dir>/code-review-{pr_number}.json`
4. **STOP and ask the user** if they want to post the PENDING review to GitHub.
5. Only if the user approves, stage the review as PENDING: `bun run .Codex/skills/deep-code-review/post-review.ts post-pending <output_dir>/code-review-{pr_number}.json`
...
7. Only if the user says to submit: `bun run .Codex/skills/deep-code-review/post-review.ts submit <output_dir>/code-review-{pr_number}.json <review_id> [APPROVE|REQUEST_CHANGES|COMMENT]`
