...
function appendCustomFontCodeToMainApp(config, fonts) {
  ...
  return (0, config_plugins_1.withDangerousMod)(config, [
    'android',
    async (config) => {
      const fontsDir = path_1.default.join(
        ...
        Object.values(fontsByFamily).map((fonts) => {
          return config_plugins_1.XML.writeXMLAsync({
            path: path_1.default.join(
              fontsDir,
              `${(0, utils_1.toAndroidResourceString)(fonts[0].family)}.xml`
            ),
            xml: {
              'font-family': {
                $: {
...
function addFontXmlToMainApplication(config, families) {
  return (0, config_plugins_1.withMainApplication)(config, async (config) => {
    config.modResults.contents = (0, codeMod_1.addImports)(
      config.modResults.contents,
      ['com.facebook.react.common.assets.ReactFontManager'],
      config.modResults.language === 'java'
    );
    config.modResults.contents = (0, codeMod_1.appendContentsInsideDeclarationBlock)(
      config.modResults.contents,
      'onCreate',
...
function addFontsInDir(config, fonts) {
  return (0, config_plugins_1.withDangerousMod)(config, [
    ...
    async (config) => {
      ...
      const resolvedFonts = await (0, utils_1.resolveFontPaths)(
        paths,
