...
describe('build:android command', () => {
  ...
  describe('without prebuild', () => {
    ...
    it('should automatically run prebuild in a non-interactive shell', async () => {
      ...
      // Prebuild ran, but this temp project cannot generate the brownfield
      // library (pre-existing behavior on this branch) — the command proceeds
      // past the prompt and fails at library discovery instead of hanging.
