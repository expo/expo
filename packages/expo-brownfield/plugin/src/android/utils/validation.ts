...
const PATTERNS: Record<GradleFieldKind, { pattern: RegExp; shape: string }> = {
  groovyIdentifier: {
    pattern: GROOVY_IDENTIFIER,
    shape:
      'a Groovy identifier (letters, digits, and underscores, starting with a letter or underscore)',
  },
  gradleProjectName: {
    pattern: GRADLE_PROJECT_NAME,
    shape:
      'a Gradle project name (letters, digits, underscores, and hyphens, starting with a letter or underscore)',
  },
  javaPackage: {
  ...
  envVarName: {
    ...
    shape:
      'an environment variable name (letters, digits, and underscores, starting with a letter or underscore)',
  },
};

export const validateGradleField = (
  kind: GradleFieldKind,
  value: unknown,
  fieldName: string
): string => {
...
export const escapeGroovyDoubleQuotedString = (value: string): string => {
