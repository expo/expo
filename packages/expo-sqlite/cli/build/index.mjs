import { runCliExtension, createCliDevToolsPluginClient, sendDevToolsRequestAsync, } from 'expo/devtools.mjs';
        ...
        switch (command) {
            case 'list_databases': {
                ...
                if (response.method === 'listDatabases') {
                    console.info(`${blue(app.title)}: ${response.databases.map((d) => d.name).join(', ') || 'No databases registered.'}`);
                }
            ...
            case 'execute_query': {
                ...
                if (response.method === 'executeQuery') {
                    ...
                    if (rows.length > 0) {
                        ...
                        console.info(`${blue(app.title)}: ${changes} row(s) affected, last insert rowid: ${lastInsertRowId}`);
                    }
                    else {
