...
/**
 * Expo addition, not part of upstream wa-sqlite. Removes one OPFS entry by absolute path,
 * tolerating a path that no longer exists.
 *
 * @param {string} path
 * @returns {Promise<void>}
 */
async function removeOpfsEntry(path) {
...
export class OPFSCoopSyncVFS extends FacadeVFS {
  ...
  /**
   * Expo addition, not part of upstream wa-sqlite.
   *
   * `jDelete` is synchronous, so it can only empty a file whose sync access handle is currently
   * held. Closing a main database releases its handles, so deleting a closed database through
   * `jDelete` silently fails. This VFS is filesystem transparent, so remove the entries through
   * the asynchronous OPFS API instead and drop the cached state for them.
   *
   * @param {string} zName
   * @returns {Promise<void>}
   */
  async deleteFileAsync(zName) {
    ...
    for (const suffix of DB_RELATED_FILE_SUFFIXES) {
      ...
      if (persistentFile?.accessHandle) {
        // The file is still open, so the synchronous path keeps SQLite's view consistent.
      ...
      if (persistentFile) {
        persistentFile.handleRequestChannel?.close();
        this.persistentFiles.delete(path);
      }
