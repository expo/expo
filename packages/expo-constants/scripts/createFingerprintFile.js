 ...
 * The file is the build-time baseline that `npx @expo/agent-cli needs-rebuild` compares against.
 *
 ...
 * on either side makes `npx @expo/agent-cli needs-rebuild` report a permanent mismatch.
 *
...
function warnFingerprintEmbedFailed(error) {
  // The fingerprint is optional metadata for `npx @expo/agent-cli needs-rebuild` — never fail the build.
  console.warn(
...
// before computing keeps the hash identical to the check side (`npx @expo/agent-cli needs-rebuild`).
if (require.main === module) {
  ...
  Promise.resolve()
    .then(() => {
      ...
      // Match the check side (`npx @expo/agent-cli needs-rebuild` and the dev server), which evaluates the
