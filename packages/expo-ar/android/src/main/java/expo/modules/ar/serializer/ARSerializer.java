...
class ARSerializer {
  ...
  private static Bundle serializeImage(AugmentedImage image, Bundle output) {
    ...
    output.putFloatArray("transform", ARSerializer.serializePose(image.getCenterPose()));
  ...
  private static Bundle serializeAnchor(Anchor anchor) {
    ...
    output.putFloatArray("transform", ARSerializer.serializePose(pose)); //TODO:Bacon: Prolly not right
    output.putString("cloudId", anchor.getCloudAnchorId());
    output.putString("cloudState", serializeCloudAnchorState(state));
    output.putString("state", ARSerializer.serializeTrackingState(trackingState));
    return output;
  }
