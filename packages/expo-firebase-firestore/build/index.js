...
export const statics = {
    ...
    setLogLevel(logLevel) {
        invariant(LogLevels.includes(logLevel), 'Argument `logLevel` must be one of: `debug`, `error`, `silent`');
...
export default class Firestore extends ModuleBase {
    constructor(app) {
        ...
        this.getAppEventName(NATIVE_EVENTS.firestoreCollectionSyncEvent), this._onCollectionSyncEvent.bind(this));
        SharedEventEmitter.addListener(
        // sub to internal native event - this fans out to
        // public event name: onDocumentSnapshot
        this.getAppEventName(NATIVE_EVENTS.firestoreDocumentSyncEvent), this._onDocumentSyncEvent.bind(this));
    }
    /**
     * -------------
     *  PUBLIC API
    ...
    settings(settings) {
        ...
        else if (hop(settings, 'timestampsInSnapshots') &&
    ...
    _onCollectionSyncEvent(event) {
        if (event.error) {
            SharedEventEmitter.emit(this.getAppEventName(`onQuerySnapshotError:${event.listenerId}`), event);
        }
        else {
            SharedEventEmitter.emit(this.getAppEventName(`onQuerySnapshot:${event.listenerId}`), event.querySnapshot);
        }
    }
    /**
    ...
    _onDocumentSyncEvent(event) {
        if (event.error) {
            SharedEventEmitter.emit(this.getAppEventName(`onDocumentSnapshotError:${event.listenerId}`), event);
        }
        else {
            SharedEventEmitter.emit(this.getAppEventName(`onDocumentSnapshot:${event.listenerId}`), event.documentSnapshot);
        }
    }
}
Firestore.moduleName = MODULE_NAME;
Firestore.namespace = NAMESPACE;
Firestore.statics = statics;
