...
internal enum AgeRangeDeclaration: String, Enumerable {
  ...
  public init(_ range: AgeRangeService.AgeRangeDeclaration) {
    ...
    @unknown default:
      ...
      log.error(
        "Unhandled `AgeRangeService.AgeRangeDeclaration` value: \(range), reporting `selfDeclared` as fallback. "
          + "Either the value was added in an iOS SDK newer than the one you build against (build with a newer "
          + "Xcode), or expo-age-range needs to map it, in which case report it at github.com/expo/expo/issues."
      )
      self = .selfDeclared
    }
  }
