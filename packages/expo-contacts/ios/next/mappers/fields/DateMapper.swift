...
private extension ContactDateNext {
  init(from nsDateComponents: NSDateComponents) {
    ...
    self.month = Int(month) ?? 0 >= 10 ? String(nsDateComponents.month) : "0\(nsDateComponents.month)"
    self.day = Int(day) ?? 0 >= 10 ? String(nsDateComponents.month) : "0\(nsDateComponents.month)"
  }
