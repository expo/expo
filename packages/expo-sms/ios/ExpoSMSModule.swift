...
public class ExpoSMSModule: Module, SMSResultHandler {
    private var smsContext: ExpoSMSContext? = nil
    ...
    private func sendSMSAsync(addresses: [String], message: String, options: SmsOptions?, promise: Promise) {
        ...
        if let options {
            for attachment in options.attachments {
                let utiRef = UTTypeCreatePreferredIdentifierForTag(kUTTagClassMIMEType, attachment.mimeType as CFString, nil)
                
                if utiRef == nil {
                    context.promise.reject("E_SMS_ATTACHMENT", "Failed to find UTI for mimeType: \(attachment.mimeType)")
                    return
                }
                ...
                do {
                    ...
                    let attached = messageComposeViewController.addAttachmentData(data, typeIdentifier: attachment.mimeType, filename: attachment.filename)
                    if !attached {
