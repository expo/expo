...
extension UIImage {
  ...
  static func delayForImageAtIndex(index: Int, source: CGImageSource) -> TimeInterval {
    ...
    let gifProperties: CFDictionary? = CFDictionaryGetValue(cfProperties, Unmanaged.passUnretained(kCGImagePropertyGIFDictionary).toOpaque())?
    .assumingMemoryBound(to: CFDictionary.self).pointee
