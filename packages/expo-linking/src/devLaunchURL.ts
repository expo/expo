...
const URL_PARAM = `${RESERVED_PREFIX}url`;
 ...
 * unwrapDevLaunchURL('myapp://login?__expo_disable_fab=1');
 * // 'myapp://login'
 * ```
 */
export function unwrapDevLaunchURL(url: string): string {
  ...
  const target =
    parsed.searchParams.get(URL_PARAM) ?? (isLegacyHost ? parsed.searchParams.get('url') : null);
  if (target != null) {
    return target;
  }
  if (isLegacyHost) {
