...
it('can continue to push with same path but different params', async () => }
  ...
  act(() => router.push('/test/three'));
  expect(screen).toHavePathname('/test/three');
  expect(screen.getByTestId('three')).toBeOnTheScreen();

  act(() => router.push('/test/four'));
  expect(screen).toHavePathname('/test/four');
