...
function ModalStackRouteDrawer({ routeKey, options, renderScreen, onDismiss, themeColors, }) {
    ...
    const sheetProps = isSheet
        ...
        : {};
    return (<Drawer.Root key={`${routeKey}-${isSheet ? 'sheet' : 'modal'}`} open={open} dismissible={options.gestureEnabled ?? true} onAnimationEnd={handleOpenChange} shouldScaleBackground autoFocus onOpenChange={setOpen} {...sheetProps}>
      <Drawer.Portal>
