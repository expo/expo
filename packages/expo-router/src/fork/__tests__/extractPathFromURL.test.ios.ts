...
describe(extractExpoPathFromURL, () => {
  ...
  it(`ignores reserved launch params on a legacy dev client URL`, () => {
    ...
    expect(
      extractExpoPathFromURL(
        [],
        'scheme://expo-development-client/?url=http%3A%2F%2Flocalhost%3A8081%2Fexample%2Fpath&__expo_disable_auto_launch=1'
      )
  ...
  it(`unwraps a dev client URL that uses __expo_url`, () => {
    ...
    expect(
      extractExpoPathFromURL(
        [],
        'scheme://?__expo_url=http%3A%2F%2Flocalhost%3A8081%2Fexample%2Fpath&__expo_disable_fab=1'
      )
    ).toEqual('example/path');
  });
  ...
  it(`ignores reserved launch params in Expo Go`, () => {
    ...
    expect(
      extractExpoPathFromURL(
        [],
        'exp://127.0.0.1:8081/--/test/path?__expo_disable_auto_launch=1&query=param'
      )
  ...
  it(`ignores a reserved launch param on an app deep link`, () => {
    delete expo.modules.ExpoGo;
    expect(extractExpoPathFromURL([], 'myapp://login?__expo_disable_fab=1&x=1')).toEqual(
      'login?x=1'
    );
  });
