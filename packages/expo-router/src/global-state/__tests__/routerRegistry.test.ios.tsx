...
describe(RouterRegistryProvider, () => {
  ...
  it('replaces the snapshot instead of mutating it', () => {
    ...
    expect(second.get(state.key)).toBe(secondEntry);
  });

  it('keeps the snapshot identity when registration does not change', () => {
    const stores: RouterRegistryStore[] = [];
  ...
  it('reports a change only when the snapshot changes', () => {
