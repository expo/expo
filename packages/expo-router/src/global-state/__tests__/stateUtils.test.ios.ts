...
describe('findDivergentState', () => {
  it('stops at the mounted parent when the child state is partial', () => {
    const actionState: ResultState = {
      routes: [{ name: 'root', state: { routes: [{ name: 'next' }] } }],
    };
    ...
    expect(result.navigationState).toBe(navState);
    expect(result.actionState).toBe(actionState);
