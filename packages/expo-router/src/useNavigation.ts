...
>(parent?: string | Href): T {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (!navigation) {
      ...
      if (!ids.length) {
        throw new Error(
          `No child navigation object named "${parent}" found. This can happen if you are calling useNavigation() or a named Screen (<Stack.Screen name="${parent}"/>) in a route instead of a layout file.`
        );
      }
