...
describe(usePathname, () => {
  it(`return pathname of deeply nested routes`, async () => {
    const { result } = await renderHook(() => usePathname(), ['[fruit]/[shape]/[...veg?]'], {
    ...
    await act(() => router.push('/banana/circle/carrot/beetroot/beans'));
    expect(result.current).toEqual('/banana/circle/carrot/beetroot/beans');

    await act(() => router.push('/banana/circle/carrot/beetroot?foo=bar'));
    expect(result.current).toEqual('/banana/circle/carrot/beetroot');
  });
});
