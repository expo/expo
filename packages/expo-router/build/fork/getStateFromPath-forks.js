...
function createConfig(screen, pattern, route, config = {}) {
    const parts = [];
    let isDynamic = false;
...
function appendIsInitial(initialRoutes) {
    ...
    return function (config) {
        ...
        config.isInitial = resolvedInitialPatterns.includes(config.route.map(({ name }) => name).join('/'));
        return config;
    };
}
...
function getInitialRouteNamePriority({ expandedRoute }) {
