...
import { store, storeRef, getSplashScreenAnimationFrame, setSplashScreenAnimationFrame, } from './store';
...
export function useStore(context, linkingConfigOptions, serverUrl) {
    const navigationRef = useNavigationContainerRef();
    const config = Constants.expoConfig?.extra?.router;
    let linking;
    let rootComponent = Fragment;
    let initialState;
    ...
    if (routeNode) {
        ...
        if (typeof initialURL === 'string') {
            ...
            setCachedRouteInfo(initialState, initialRouteInfo);
        }
    }
    else {
