...
import { getInternalExpoRouterParams, INTERNAL_EXPO_ROUTER_IS_PREVIEW_NAVIGATION_PARAM_NAME, INTERNAL_EXPO_ROUTER_ZOOM_TRANSITION_SCREEN_ID_PARAM_NAME, INTERNAL_EXPO_ROUTER_ZOOM_TRANSITION_SOURCE_ID_PARAM_NAME, } from '../../navigationParams';
...
export function ZoomTransitionEnabler({ route }) {
    ...
    if (shouldEnableZoomTransition) {
        ...
        return (<LinkZoomTransitionEnabler zoomTransitionSourceIdentifier={zoomTransitionId} dismissalBoundsRect={effectiveDismissalBoundsRect}/>);
    }
    return null;
}
/**
 * @internal
 */
export function useShouldEnableZoomTransition(route) {
    ...
    if (isZoomTransitionEnabled() &&
        ...
        const zoomTransitionId = internalParams[INTERNAL_EXPO_ROUTER_ZOOM_TRANSITION_SOURCE_ID_PARAM_NAME];
        const zoomTransitionScreenId = internalParams[INTERNAL_EXPO_ROUTER_ZOOM_TRANSITION_SCREEN_ID_PARAM_NAME];
        ...
        if (hasZoomTransition && typeof zoomTransitionId === 'string') {
            ...
            // even when the user hasn't called usePreventZoomTransitionDismissal().
            const descriptorsMap = use(DescriptorsContext);
            const isLinkPreviewNavigation = !!internalParams[INTERNAL_EXPO_ROUTER_IS_PREVIEW_NAVIGATION_PARAM_NAME];
