...
import { ZoomTransitionSourceContext, ZoomTransitionTargetContext, } from './zoom-transition-context';
import { getInternalExpoRouterParams, INTERNAL_EXPO_ROUTER_ZOOM_TRANSITION_SCREEN_ID_PARAM_NAME, INTERNAL_EXPO_ROUTER_ZOOM_TRANSITION_SOURCE_ID_PARAM_NAME, } from '../../navigationParams';
import { shouldLinkExternally } from '../../utils/url';
import { useIsPreview } from '../preview/PreviewRouteContext';
export function ZoomTransitionSourceContextProvider({ children, linkProps, }) {
    const { href, asChild } = linkProps;
    const isExternalHref = typeof href === 'string' ? shouldLinkExternally(href) : shouldLinkExternally(href.pathname);
...
export function ZoomTransitionTargetContextProvider({ route, children, }) {
    const [dismissalBoundsRect, setDismissalBoundsRect] = useState(null);
    ...
    if (isZoomTransitionEnabled() &&
        ...
        if (hasZoomTransition && typeof zoomTransitionId === 'string') {
            return (<ZoomTransitionTargetContext value={{
                    identifier: zoomTransitionId,
                    dismissalBoundsRect,
