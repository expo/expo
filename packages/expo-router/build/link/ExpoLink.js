...
export function ExpoLink(props) {
    return (<ZoomTransitionSourceContextProvider linkProps={props}>
      <ExpoLinkImpl {...props}/>
...
function ExpoLinkImpl(props) {
    ...
    if (React.Children.count(props.children) > 1) {
        const arrayChildren = React.Children.toArray(props.children).filter((child) => !isValidElement(child) || (child.type !== LinkPreview && child.type !== LinkMenu));
...
function isLinkWithPreview(props) {
    ...
    return Children.toArray(props.children).some((child) => isValidElement(child) &&
