...
import { createScrollEdgeAppearanceFromOptions, createStandardAppearanceFromOptions, } from './appearance';
import { Color } from '../color';
import { NativeTabsBottomAccessory } from './common/elements';
import { SUPPORTED_TAB_BAR_ITEM_LABEL_VISIBILITY_MODES, SUPPORTED_TAB_BAR_MINIMIZE_BEHAVIORS, } from './types';
import { useTheme } from '../react-navigation/native';
import { useBottomAccessoryFunctionFromBottomAccessories } from './utils/bottomAccessory';
import { convertOptionsIconToRNScreensPropsIcon, convertOptionsIconToIOSPropsIcon, useAwaitedScreensIcon, } from './utils/icon';
...
export function NativeTabsView(props) {
    ...
    const bottomAccessory = useMemo(() => getFirstChildOfType(nonTriggerChildren, NativeTabsBottomAccessory), [nonTriggerChildren]);
...
function Screen(props) {
    ...
    return (<Tabs.Screen {...options} overrideScrollViewContentInsetAdjustmentBehavior={!options.disableAutomaticContentInsets} tabBarItemBadgeBackgroundColor={standardAppearance.stacked?.normal?.tabBarItemBadgeBackgroundColor} tabBarItemBadgeTextColor={badgeTextColor} standardAppearance={standardAppearance} scrollEdgeAppearance={scrollEdgeAppearance} icon={convertOptionsIconToRNScreensPropsIcon(icon, standardAppearance?.stacked?.normal?.tabBarItemIconColor)} selectedIcon={convertOptionsIconToIOSPropsIcon(selectedIcon, standardAppearance?.stacked?.selected?.tabBarItemIconColor)} title={title} freezeContents={false} systemItem={options.role} {...options.nativeProps} tabKey={routeKey} isFocused={isFocused}>
      {wrappedContent}
    </Tabs.Screen>);
}
...
function TabsHostWrapper(props) {
    ...
    return (<Tabs.Host tabBarItemLabelVisibilityMode={tabBarItemLabelVisibilityMode} tabBarMinimizeBehavior={tabBarMinimizeBehavior} {...rest}/>);
}
