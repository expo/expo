...
export function NativeTabsNavigator({ children, backBehavior = defaultBackBehavior, labelStyle, iconColor, blurEffect, backgroundColor, badgeBackgroundColor, indicatorColor, badgeTextColor, shadowColor, screenListeners, ...rest }) {
    ...
    const selectedLabelStyle = processedLabelStyle.selected
        ...
        : rest.tintColor
            ...
            : undefined;
    const { state, descriptors, navigation, NavigationContent } = useNavigationBuilder(NativeBottomTabsRouter, {
        children,
        backBehavior,
        screenListeners,
        screenOptions: {
    ...
    const { routes } = state;
    const visibleTabs = useMemo(() => routes
    ...
    const visibleFocusedTabIndex = useMemo(() => visibleTabs.findIndex((tab) => tab.routeKey === routes[state.index].key), [visibleTabs, routes, state.index]);
    const visibleTabsKeys = useMemo(() => visibleTabs.map((tab) => tab.routeKey).join(';'), [visibleTabs]);
    ...
    return (<NavigationContent>
      <NativeTabsContext value>
        <NativeTabsView {...rest} key={visibleTabsKeys} focusedIndex={focusedIndex} tabs={visibleTabs} onTabChange={onTabChange}/>
      </NativeTabsContext>
    </NavigationContent>);
}
const createNativeTabNavigator = createNavigatorFactory(NativeTabsNavigator);
const NativeTabsNavigatorWithContext = withLayoutContext(createNativeTabNavigator().Navigator, undefined, true);
export function NativeTabsNavigatorWrapper(props) {
    const triggerChildren = useMemo(() => getAllChildrenOfType(props.children, NativeTabTrigger), [props.children]);
    const nonTriggerChildren = useMemo(() => getAllChildrenNotOfType(props.children, NativeTabTrigger), [props.children]);
