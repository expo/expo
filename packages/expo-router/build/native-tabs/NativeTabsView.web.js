...
export function NativeTabsView(props) {
    ...
    const defaultTab = useMemo(() => currentTab, 
    ...
    const children = tabs.map((tab) => {
        return (<TabsContent key={tab.routeKey} value={tab.routeKey} className={nativeTabsStyles.tabContent} forceMount>
        {tab.contentRenderer()}
      </TabsContent>);
    });
    return (<Tabs className={nativeTabsStyles.nativeTabsContainer} defaultValue={defaultTab.routeKey} value={value} onValueChange={(value) => {
...
function TabItem(props) {
    ...
    return (<TabsTrigger value={routeKey} className={nativeTabsStyles.navigationMenuTrigger}>
