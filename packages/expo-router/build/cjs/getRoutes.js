...
function getFileMeta(key, options) {
  ...
  let route = (0, _matchers().removeSupportedExtensions)(key);
  const filename = parts[parts.length - 1];
  const [filenameWithoutExtensions, platformExtension] = (0, _matchers().removeSupportedExtensions)(filename).split('.');
...
function extrapolateGroups(key, keys = new Set()) {
  const match = (0, _matchers().matchArrayGroupName)(key);
  if (!match) {
    keys.add(key);
    return keys;
  }
...
function generateDynamic(path) {
  const dynamic = path.split('/').map(part => {
    ...
    const deepDynamicName = (0, _matchers().matchDeepDynamicRouteName)(part);
    const dynamicName = deepDynamicName ?? (0, _matchers().matchDynamicName)(part);
...
function getLayoutNode(node, options) {
  ...
  const groupName = (0, _matchers().matchGroupName)(node.route);
