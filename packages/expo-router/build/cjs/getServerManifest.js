...
function getServerManifest(route) {
  function getFlatNodes(route) {
    ...
    let key;
    if (route.type === 'api') {
      key = (0, _matchers().getContextKey)(route.contextKey).replace(/\/index$/, '') ?? '/';
    } else {
      key = (0, _matchers().getContextKey)(route.route).replace(/\/index$/, '') ?? '/';
    }
    return [[key, route]];
  }

  // Remove duplicates from the runtime manifest which expands array syntax.
  const flat = getFlatNodes(route).sort(([, a], [, b]) => (0, _sortRoutes().sortRoutes)(b, a)).reverse();
...
function getMatchableManifestForPaths(paths) {
  return paths.map(normalizedRoutePath => {
    const matcher = getNamedRouteRegex(normalizedRoutePath[0], (0, _matchers().getContextKey)(normalizedRoutePath[1].route), normalizedRoutePath[1].contextKey);
  ...
  return {
    namedParameterizedRoute: segments.map((segment, index) => {
      ...
      if (/^\[.*\]$/.test(segment)) {
        ...
        const groupName = (0, _matchers().matchGroupName)(segment).split(',').map(group => group.trim()).filter(Boolean);
