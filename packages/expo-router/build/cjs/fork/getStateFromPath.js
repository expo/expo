...
function _findFocusedRoute() {
  const data = require("./findFocusedRoute");
  _findFocusedRoute = function () {
    return data;
  };
  return data;
}
function _validatePathConfig() {
  const data = _interopRequireDefault(require("./validatePathConfig"));
  _validatePathConfig = function () {
    return data;
  };
  return data;
}
...
function getMatchableRouteConfigs(options) {
  ...
  const converted = Object.keys(screens).map(key => createNormalizedConfigs(key, screens, [], initialRoutes)).flat();
...
function sortConfigs(a, b) {
  ...
  .filter(part => (0, _matchers().matchGroupName)(part) == null);
  if (a.screen === 'index') {
    aParts.push('index');
  }
  const bParts = b.pattern.split('/').filter(part => (0, _matchers().matchGroupName)(part) == null);
  if (b.screen === 'index') {
    bParts.push('index');
  }
...
function getStateFromEmptyPathWithConfigs(path, configs, initialRoutes) {
  ...
  const leafNodes = configs.filter(config => !config.hasChildren).map(value => {
    return {
      ...
      path: (0, _matchers().stripGroupSegmentsFromPath)(value.path)
    };
  });
...
function getStateFromPathWithConfigs(path, configs, initialRoutes, baseUrl = process.env.EXPO_BASE_URL) {
  ...
  let cleanPath = stripBaseUrl((0, _matchers().stripGroupSegmentsFromPath)(formattedPaths.url.pathname), baseUrl) + formattedPaths.url.search;
...
function matchAgainstConfigs(remaining, configs) {
  ...
  for (const config of configs) {
    ...
    const matchedParams = config.pattern?.split('/').filter(p => p.match(/^[:*]/)).reduce((acc, p, i) => {
      if (p.match(/^\*/)) {
        return {
          ...acc,
...
function formatRegexPattern(it) {
  ...
  if ((0, _matchers().matchGroupName)(it) != null) {
    // Groups are optional segments
    // this enables us to match `/bar` and `/(foo)/bar` for the same route
    // NOTE(EvanBacon): Ignore this match in the regex to avoid capturing the group
    return `(?:${(0, _escapeStringRegexp().default)(it)}\\/)?`;
  }
  return (0, _escapeStringRegexp().default)(it) + `\\/`;
}
const createConfigItem = (screen, routeNames, pattern, path, hasChildren, parse, _route) => {
  ...
  const regex = pattern ? new RegExp(`^(${pattern.split('/').map(formatRegexPattern).join('')})$`) : undefined;
  return {
    screen,
    regex,
    pattern,
...
function getBaseUrlRegex(baseUrl) {
  ...
  const regex = new RegExp(`^\\/?${(0, _escapeStringRegexp().default)(baseUrl)}`, 'g');
