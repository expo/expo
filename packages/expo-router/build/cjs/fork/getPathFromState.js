...
function segmentMatchesConvention(segment) {
  return segment === 'index' || (0, _matchers().matchDynamicName)(segment) != null || (0, _matchers().matchGroupName)(segment) != null || (0, _matchers().matchDeepDynamicRouteName)(segment) != null;
}
...
function getPathDataFromState(state, _options = {
  ...
  (0, _core().validatePathConfig)(options);
...
function getPathWithConventionsCollapsed({
  ...
  return segments.map((p, i) => {
    ...
    if (!preserveGroups && (0, _matchers().matchGroupName)(p) != null) {
...
function getParamsWithConventionsCollapsed({
  ...
  if (segments.some(segment => segment.startsWith('*'))) {
    ...
    const name = (0, _matchers().testNotFound)(routeName) ? 'not-found' : (0, _matchers().matchDeepDynamicRouteName)(routeName) ?? routeName;
    delete processedParams[name];
  }
