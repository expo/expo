...
function _fastDeepEqual() {
  const data = _interopRequireDefault(require("fast-deep-equal"));
  _fastDeepEqual = function () {
    return data;
  };
  return data;
}
...
function _createMemoryHistory() {
  const data = _interopRequireDefault(require("@react-navigation/native/lib/commonjs/createMemoryHistory"));
  _createMemoryHistory = function () {
    return data;
  };
  return data;
}
...
function _getPathFromState() {
  const data = require("./getPathFromState");
      ...
      if (route?.path) {
        ...
        if (stateForPath) {
          ...
          if (focusedRoute && focusedRoute.name === route.name && (0, _fastDeepEqual().default)(focusedRoute.params, route.params)) {
            ...
            return (0, _getPathFromState().appendBaseUrl)(route.path);
            /* End of fork */
          }
        }
      }
