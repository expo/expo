...
const defaultOverlay = ({ style }) => style ? <Animated.View pointerEvents="none" style={[styles.overlay, style]}/> : null;
function Card({ shadowEnabled = false, gestureEnabled = true, gestureVelocityImpact = GESTURE_VELOCITY_IMPACT, overlay = defaultOverlay, animated, interpolationIndex, opening, closing, next, current, gesture, layout, insets, direction, pageOverflowEnabled, gestureDirection, onOpen, onClose, onTransition, onGestureBegin, onGestureCanceled, onGestureEnd, children, overlayEnabled, gestureResponseDistance, transitionSpec, preloaded, styleInterpolator, containerStyle: customContainerStyle, contentStyle, }) {
    ...
    const animate = useLatestCallback(({ closing: isClosingParam, velocity }) => {
    ...
    const onGestureStateChange = useLatestCallback(({ nativeEvent }) => {
    ...
    return (<CardAnimationContext.Provider value={interpolationProps}>
      {Platform.OS !== 'web' ? (<Animated.View style={{
      ...
      {overlayEnabled ? (<View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
          {overlay({ style: overlayStyle })}
        </View>) : null}
      <Animated.View pointerEvents="box-none" style={[styles.container, containerStyle, customContainerStyle]}>
        <PanGestureHandler enabled={layout.width !== 0 && gestureEnabled} onGestureEvent={onGestureEvent} onHandlerStateChange={onGestureStateChange} {...gestureActivationCriteria({
        layout,
        direction,
        gestureDirection,
        gestureResponseDistance,
    })}>
          <Animated.View pointerEvents="box-none" needsOffscreenAlphaCompositing={hasOpacityStyle(cardStyle)} style={[styles.container, cardStyle]}>
            {shadowEnabled && shadowStyle && !isTransparent ? (<Animated.View pointerEvents="none" style={[
                styles.shadow,
