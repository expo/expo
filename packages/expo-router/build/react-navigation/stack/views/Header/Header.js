...
export const Header = React.memo(function Header({ back, layout, progress, options, route, navigation, styleInterpolator, }) {
    ...
    const statusBarHeight = options.headerStatusBarHeight !== undefined
        ...
        : isModal || isParentHeaderShown
            ? 0
            : insets.top;
    return (<HeaderSegment {...options} title={getHeaderTitle(options, route.name)} progress={progress} layout={layout} modal={isModal} headerBackTitle={options.headerBackTitle !== undefined ? options.headerBackTitle : previousTitle} headerStatusBarHeight={statusBarHeight} onGoBack={back ? goBack : undefined} backHref={back ? back.href : undefined} styleInterpolator={styleInterpolator}/>);
});
