...
- Make `history` optional in `TabNavigationState` and `DrawerNavigationState`
- Remove the static `default` field from `DrawerNavigationState` and deprecate `getDrawerStatusFromState`.
- Compute deep navigation state at dispatch time and stop interpreting `screen`, `params`, `initial`, and `state` as nested navigation params. Remove `getActionFromState`, `useLinkTo`, and action building from `useLinkBuilder`.
