...
- Make every JS-initiated navigation a React transition. `router.push`/`navigate`/`replace`/`back`/…, `Link` presses, and deep links now commit inside `React.startTransition`, and navigators read the navigation tree from React state instead of an external store. A navigation whose destination suspends (a lazy bundle-split screen, a `use(promise)`/suspending loader) keeps the current screen mounted and interactive instead of flashing the Suspense fallback. Native-induced navigation (swipe-back, native tab press, hardware/header back) stays synchronous. Three consequences to note: (1) a slow destination now shows nothing until it is ready — wire a pending indicator rather than relying on an immediate push; (2) `store.state`/`getRootState()` and the imperative "would this do anything" queries (`router.canGoBack()`/`canDismiss()`) answer for the last _committed_ tree, so `router.push(); router.canGoBack()` in the same tick now reflects the pre-push state (the imperative store no longer leads render); and (3) a navigation dispatched from a mount effect that targets an already-registered navigator now applies, where before it could be silently dropped in a pre-initialization window (the root reducer is React state seeded at mount, so it is always registered). ([#00000](https://github.com/expo/expo/pull/00000) by [@Ubax](https://github.com/Ubax))
- Force `freezeOnBlur: false` on the screens Expo Router renders (native stack, JS stack, bottom tabs, drawer, and the headless `ui` `TabSlot`), so a Suspense-frozen background subtree can no longer starve an in-flight React transition. The per-screen `freezeOnBlur` option and `enableFreeze()` no longer re-enable freeze on router-rendered screens (this reverses the direction of [#38837](https://github.com/expo/expo/pull/38837), which removed the global `enableFreeze(false)`). On the native stack only, `unstable_nativeProps={{ freezeOnBlur: true }}` can still re-enable it as an advanced, at-your-own-risk escape hatch — doing so reintroduces the transition starvation this change prevents. ([#00000](https://github.com/expo/expo/pull/00000) by [@Ubax](https://github.com/Ubax))
- Remove the unhandled-action reporting machinery and dispatch telemetry. The `onUnhandledAction` prop is gone from the navigation container (`expo-router/react-navigation`), unhandled actions no longer log a development console error (or throw in tests), the `UNSTABLE_routeNamesChangeBehavior: 'lastUnhandled'` route-restore behavior is removed, and the `__unsafe_action__` container event and the public `actionDispatched` navigation event are no longer emitted. Dispatch-time action telemetry (consumed by EAS Observe) is planned to return, redesigned on the global-state reducer model. ([#00000](https://github.com/expo/expo/pull/00000) by [@Ubax](https://github.com/Ubax))
