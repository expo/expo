...
- Remove `NavigationIndependentTree` and `useNavigationIndependentTree` from `expo-router/react-navigation`. Nesting a navigation container inside another now always throws. To embed an isolated navigation tree in a screen, install `@react-navigation/native` and render that package's `NavigationContainer` there.
