...
const { promiseOrCallback, isFunction, isObject, isString, tryJSONParse, tryJSONStringify, generatePushID, } = utils;
...
(function (ReferenceEventTypes) {
    ReferenceEventTypes["value"] = "value";
    ReferenceEventTypes["child_added"] = "child_added";
    ReferenceEventTypes["child_removed"] = "child_removed";
    ReferenceEventTypes["child_changed"] = "child_changed";
    ReferenceEventTypes["child_moved"] = "child_moved";
})(ReferenceEventTypes || (ReferenceEventTypes = {}));
/**
...
export default class Reference extends ReferenceBase {
     ...
     */
    keepSynced(bool) {
        return this._database.nativeModule.keepSynced(this._getRefKey(), this.path, this._query.getModifiers(), bool);
    }
    /**
    ...
    async set(value, onComplete) {
        return promiseOrCallback(this._database.nativeModule.set(this.path, this._serializeAnyType(value)), onComplete);
    }
    /**
    ...
    async setPriority(priority, onComplete) {
        ...
        return promiseOrCallback(this._database.nativeModule.setPriority(this.path, _priority), onComplete);
    }
    /**
    ...
    async setWithPriority(value, priority, onComplete) {
        ...
        const _priority = this._serializeAnyType(priority);
        return promiseOrCallback(this._database.nativeModule.setWithPriority(this.path, _value, _priority), onComplete);
    }
    /**
    ...
    once(eventName = 'value', successCallback, cancelOrContext, context) {
        return this._database.nativeModule
            ...
            .catch(error => {
            if (isFunction(cancelOrContext))
                return cancelOrContext(error);
            throw error;
        });
    }
    /**
    ...
    push(value, onComplete) {
        if (value === null || value === undefined) {
            return new ThenableReference(this._database, `${this.path}/${generatePushID(this._database._serverTimeOffset)}`);
        }
        const newRef = new ThenableReference(this._database, `${this.path}/${generatePushID(this._database._serverTimeOffset)}`);
    ...
    on(eventType, callback, cancelCallbackOrContext, context) {
        ...
        const _context = cancelCallbackOrContext && !isFunction(cancelCallbackOrContext)
