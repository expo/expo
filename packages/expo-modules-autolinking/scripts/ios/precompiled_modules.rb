    ...
    class << self
      ...
      def try_link_with_prebuilt_xcframework(spec)
        ...
        if framework_exists
          ...
          # Create or update symlink to default build flavor
          ...
          # Create symlink pointing to the default build flavor
          File.symlink(File.join('..', build_flavor, "#{product_name}.xcframework"), current_xcframework_path)
...
"$NODE_BINARY" "#{script_path}" \\
  ...
  -x "#{xcframeworks_dir}"
        ...
        # :always_out_of_date ensures the script runs before every compile without warning
        # Available in CocoaPods 1.13.0 and later
