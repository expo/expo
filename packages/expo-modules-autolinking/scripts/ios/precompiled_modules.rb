  ...
  module PrecompiledModules
    ...
    class << self
      ...
      def _resolve_prebuilt_status_uncached(pod_name, visiting)
        ...
        return { available: false, reason: :core_abi_mismatch } if core_abi_incompatible?
      ...
      # prebuilt ExpoModulesCore artifact does not export. Disabling the whole install
      # rather than the offending pod also avoids pairing a source-built module with a
      # prebuilt core.
      #
      ...
      def warn_core_abi_incompatible(mismatches)
        ...
        pods = mismatches.map { |mismatch| mismatch[:pod_name] }
        symbols = mismatches.flat_map { |mismatch| mismatch[:missing] }.uniq.sort
        listed = symbols.first(ABI_REPORT_LIMIT).map { |symbol| "    #{demangle_symbol(symbol)}" }
        listed << "    ...and #{symbols.size - ABI_REPORT_LIMIT} more" if symbols.size > ABI_REPORT_LIMIT

        Pod::UI.warn <<~WARNING
          [Expo-precompiled] Precompiled Expo modules are disabled for this install.

          #{pods.size} prebuilt #{pods.size == 1 ? 'module needs' : 'modules need'} #{symbols.size} #{symbols.size == 1 ? 'symbol' : 'symbols'} that #{core_label} does not provide:

          #{listed.join("\n")}

          Affected: #{summarize_pod_names(pods)}

          These prebuilt binaries were compiled against a different expo-modules-core than
          the one this project installs. Linking them would build with no error, then crash
          at launch with dyld `Symbol not found`, and App Store Connect would reject the
          upload as ITMS-90863. Every Expo module will be built from source instead. That
          build is correct, only slower.

          To restore precompiled modules, align your Expo package versions:
      ...
      def summarize_pod_names(pods)
        return pods.join(', ') if pods.size <= ABI_REPORT_LIMIT

        "#{pods.first(ABI_REPORT_LIMIT).join(', ')} and #{pods.size - ABI_REPORT_LIMIT} more"
      end
