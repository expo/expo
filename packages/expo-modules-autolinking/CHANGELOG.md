...
- [Android] Declare the `EXPO_PUBLIC_*` environment variables and the project's `.env` files as inputs of the `createBundle*JsAndAssets` tasks, so a release build that only changes such a value re-bundles instead of reporting `UP-TO-DATE` and embedding the previous value.
