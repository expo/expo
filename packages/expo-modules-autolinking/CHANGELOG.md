...
- [iOS] Fix `'React/RCTBridge.h' file not found` when building with `ios.useFrameworks: "dynamic"` and the prebuilt React Native XCFramework. Under `use_frameworks! :linkage => :dynamic` every pod target is already a dynamic framework, so the guard added in [#47500](https://github.com/expo/expo/pull/47500) matched every pod and skipped the downgrade of pods that depend on `React-Core`. ([#48781](https://github.com/expo/expo/issues/48781))
