...
- [iOS] Build a precompiled module from source instead of linking a prebuilt XCFramework that was compiled against a different React Native than the one installed. The prebuilds bundled inside npm packages sit at `prebuilds/output/<flavor>/`, a path with no React Native version in it, so any installed version was accepted — an ABI mismatch that compiles, links and runs the JS bundle fine, then corrupts memory at runtime. The prebuild pipeline now records the React Native and Hermes versions next to the tarballs; artifacts published without that record keep the previous behavior. ([#PRNUM](https://github.com/expo/expo/pull/PRNUM) by [@himself65](https://github.com/himself65))
