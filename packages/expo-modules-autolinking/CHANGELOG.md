...
- [iOS] Pull a 3rd-party pod's prebuilt XCFramework to source when a 3rd-party pod that depends on it builds from source. A prebuilt pod's headers only exist inside its XCFramework, so the source-built dependent failed to compile with e.g. `'worklets/Compat/StableApi.h' file not found` when `react-native-reanimated` had no prebuilt artifact for the installed version while `react-native-worklets` did. ([#49147](https://github.com/expo/expo/pull/49147) by [@chrfalch](https://github.com/chrfalch))
