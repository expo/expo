...
internal class ExpoLaunchUrlPreferencesTest {
  @Test
  fun `disable fab hides the fab only`() {
    val preferences = mockk<DevMenuPreferences>(relaxed = true)

    ExpoLaunchUrl(Uri.parse("exp://h:8081?__expo_disable_fab=1")).applyDevMenuPreferences(preferences)
  ...
  fun `disable auto launch turns off the launch menu and finishes onboarding`() {
    ...
    ExpoLaunchUrl(Uri.parse("exp://h:8081?__expo_disable_auto_launch=1")).applyDevMenuPreferences(preferences)
  ...
  fun `disable onboarding finishes onboarding only`() {
    ...
    ExpoLaunchUrl(Uri.parse("exp://h:8081?__expo_disable_onboarding=1")).applyDevMenuPreferences(preferences)
  ...
  fun `legacy names on the legacy host write the same preferences`() {
    ...
    ExpoLaunchUrl(Uri.parse("scheme://expo-development-client/?url=http%3A%2F%2Fh%3A8081&disableFab=1&disableAutoLaunch=1"))
  ...
  fun `a plain url writes nothing`() {
    ...
    ExpoLaunchUrl(Uri.parse("exp://h:8081?x=1")).applyDevMenuPreferences(preferences)
