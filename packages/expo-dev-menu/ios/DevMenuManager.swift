...
open class DevMenuManager: NSObject, DevMenuManagerProtocol {
  ...
  func readAutoLaunchState() {
    let userDefaultsValue = UserDefaults.standard.bool(forKey: "EXKernelDisableNuxDefaultsKey")
    if (userDefaultsValue) {
      self.shouldAutoLaunch = false
      UserDefaults.standard.removeObject(forKey: "EXKernelDisableNuxDefaultsKey")
    }
  }
