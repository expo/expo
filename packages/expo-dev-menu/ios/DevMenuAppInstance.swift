...
class DevMenuAppInstance: DevMenuRCTAppDelegate {
	 ...
	 */
	func sendCloseEvent() {
		self.reactNativeFactory?.rootViewFactory.bridge?.enqueueJSCall(
			"RCTDeviceEventEmitter.emit", args: [DevMenuAppInstance.CloseEventName])
	}

	func sendOpenEvent() {
		self.reactNativeFactory?.rootViewFactory.bridge?.enqueueJSCall(
			"RCTDeviceEventEmitter.emit", args: [DevMenuAppInstance.OpenEventName])
	}
	...
	override func bridge(_ bridge: RCTBridge, didNotFindModule moduleName: String)
	-> Bool
	{
...
#endif
		guard
			let url = DevMenuUtils.resourcesBundle()?.url(
				forResource: "EXDevMenuApp.ios", withExtension: "js")
		else {
	...
	private func jsPackagerHost() -> String? {
		...
		guard
			let packagerHostPath = DevMenuUtils.resourcesBundle()?.path(
				forResource: "dev-menu-packager-host", ofType: nil)
		else {
			return nil
		}
		...
		guard
			let content = try? String(
				contentsOfFile: packagerHostPath, encoding: .utf8
