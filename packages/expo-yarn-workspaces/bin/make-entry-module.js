...
function makeEntryModule(projectPath) {
  ...
  const mainModule = packageJson.main || 'index.js';
  ...
  const mainModulePath = path.join(projectPath, mainModule);
  ...
  const mainModuleDirectory = path.dirname(mainModulePath);
  ...
  const template = fs.readFileSync(path.join(__dirname, 'AppEntry.template.js'), 'utf8');
