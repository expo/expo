...
+ (NSString *)uid {
  return [[[EXDeviceUID alloc] initWithKey:@"deviceUID"] uid];
}
 ...
 */
- (void)save {
  if (![EXDeviceUID valueForUserDefaultsKey:_uidKey]) {
    [EXDeviceUID setValue:_uid forUserDefaultsKey:_uidKey];
  }
  if (![EXDeviceUID valueForKeychainKey:_uidKey service:_uidKey]) {
    [EXDeviceUID setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
  }
}
