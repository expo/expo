...
function maybeWrapWithReturnStatement(
  ...
  if (type.kind === TypeKind.BASIC && type.type === BasicType.NEVER) {
    return [
      ts.factory.createWhileStatement(ts.factory.createTrue(), ts.factory.createBlock([], false)),
    ];
  }
...
function getBasicTypeMockLiteral(type: BasicType): ts.Expression | undefined {
  switch (type) {
...
function createObjectLiteralExpression(
  values: [name: string, value: ts.Expression][]
): ts.Expression {
  return ts.factory.createObjectLiteralExpression(
...
const CONVERTIBLE_TYPE_MOCK: Record<ConvertibleType, () => ts.Expression> = {
  ...
  [ConvertibleType.COLOR]: () => ts.factory.createStringLiteral('red'),
  [ConvertibleType.JS_FUNCTION]: () =>
...
function getMockedValueForType(
  ...
  switch (type.kind) {
    ...
    case TypeKind.CONVERTIBLE:
      return getConvertibleTypeMockLiteral(type.type as ConvertibleType);
