...
exports.getIdentifierAnyDeclaration = getIdentifierAnyDeclaration;
...
function getIdentifierAnyDeclaration(identifier, typeParametersCount) {
    const paramCount = typeParametersCount.get(identifier);
    return typescript_1.default.factory.createTypeAliasDeclaration(undefined, identifier, paramCount === undefined ? undefined : getNParameters(paramCount), typescript_1.default.factory.createKeywordTypeNode(typescript_1.default.SyntaxKind.AnyKeyword));
}
...
function getModuleTypesDeclarationsForModule(moduleClassDeclaration, fileTypeInformation, recordTypes, enumTypes, undeclaredTypeIdentifiers) {
    return [].concat(getPrefix(), newlineIdentifier, getOneNamedImport('NativeModule', 'expo'), newlineIdentifier, [...undeclaredTypeIdentifiers].map((identifier) => getIdentifierAnyDeclaration(identifier, fileTypeInformation.typeParametersCount)), newlineIdentifier, recordTypes.flatMap(getRecordDeclaration), newlineIdentifier, enumTypes.flatMap(getEnumDeclaration), newlineIdentifier, moduleClassDeclaration.classes.map((c) => getExportedClassDeclaration(c, fileTypeInformation)), newlineIdentifier, getExportedModuleDeclaration(moduleClassDeclaration), newlineIdentifier, getModuleDefaultValueExport(moduleClassDeclaration.name));
}
