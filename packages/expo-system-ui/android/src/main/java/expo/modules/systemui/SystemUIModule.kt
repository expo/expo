...
class SystemUIModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    Function("DynamicColor") { name: String, scheme: String ->
      dynamicColor(name, scheme)
    }

    Function("ContentBasedDynamicColor") { name:String, base64Image: String ->
      val decodedBytes = Base64.decode(base64Image, Base64.DEFAULT);
      val bitmap = BitmapFactory.decodeByteArray(decodedBytes, 0, decodedBytes.size);

      val wrappedContext = DynamicColors.wrapContextIfAvailable(
        context,
        DynamicColorsOptions.Builder()
          .setContentBasedSource(bitmap)
          .build()
      )
      dynamicColor(name, wrappedContext)
    }
  ...
  private fun dynamicColor(name: String, scheme: String): String? {
    ...
    return dynamicColor(name, wrappedContext)
  }

  private fun dynamicColor(name: String, wrappedContext: Context): String? {
    val attrMap = mapOf(
