 ...
 * the hybrid encryptor does) you can use the encyption and decryption methods of this class.
 */
class AESEncryptor : KeyBasedEncryptor<KeyStore.SecretKeyEntry> {
  ...
  /***
   * We create two key stores for every `keychainService` passed from the JS side. This is because
   * it's not possible to store unauthenticated data in authenticated key stores.
   */
  ...
  override suspend fun createEncryptedItem(
    ...
    val authenticatedCipher = authenticationHelper.authenticateCipher(cipher, options.requireAuthentication, options.authenticationPrompt)

    return createEncryptedItemWithCipher(plaintextValue, options, authenticatedCipher, gcmSpec)
  }
  ...
  internal fun createEncryptedItemWithCipher(
    ...
    return JSONObject()
      .put(AuthenticationHelper.REQUIRE_AUTHENTICATION_PROPERTY, options.requireAuthentication)
