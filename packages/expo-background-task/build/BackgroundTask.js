...
import { BackgroundTaskInfoStatus, BackgroundTaskType, } from './BackgroundTask.types';
 ...
 */
export const createTask = (taskIdentifier, taskExecutor) => {
 ...
 * @param options
 */
export const scheduleTaskAsync = async (taskIdentifier, options) => {
    ...
    if (!taskIdentifier || typeof taskIdentifier !== 'string') {
        console.warn(`BackgroundTask.scheduleTaskAsync: 'taskIdentifier' argument must be a non-empty string.`);
        return;
    }
 ...
 */
export const cancelScheduledTaskAsync = async (taskIdentifier) => {
    console.log('BackgroundTask.cancelTaskAsync', { taskIdentifier });
    if (!taskIdentifier || typeof taskIdentifier !== 'string') {
        console.warn(`BackgroundTask.cancelTaskAsync: 'taskIdentifier' argument must be a non-empty string.`);
        return;
    }
    // Get the task info
 ...
 */
export const isTaskRegisteredAsync = async (taskIdentifier) => {
 ...
 */
export const isTaskScheduled = async (taskIdentifier) => {
    if (!isTaskRegisteredAsync(taskIdentifier)) {
        console.warn(`Task with identifier ${taskIdentifier} is not registered. Use createTask to register the task.`);
    }
 ...
 * @returns Task log
 */
export const getTaskLogItems = async (taskIdentifier) => {
    if (!taskIdentifier || typeof taskIdentifier !== 'string') {
        console.warn(`BackgroundTask.getTaskInfoLog: 'taskIdentifier' argument must be a non-empty string.`);
        return;
    }
    return (await BackgroundTaskRepositiory.getLogItems()).filter((t) => t.identifier === taskIdentifier);
};
/**
 * Returns task log from the task repository for a given task
 * @returns Task log
 ...
 */
const deleteTaskAsync = async (taskIdentifier) => {
    ...
    if (!taskIdentifier || typeof taskIdentifier !== 'string') {
        console.warn(`BackgroundTask.stopTaskAsync: 'taskIdentifier' argument must be a non-empty string.`);
        return;
    }
    ...
    if ((await BackgroundTaskRepositiory.getScheduledTaskInfos()).length === 0 &&
 ...
 */
export const addOnWorkListener = (cb) => {
    if (ExpoBackgroundTaskModule.EVENT_WORK_DONE !== 'onWorkDone') {
        throw new Error("Expected 'onWorkDone' got '" +
...
if (ExpoBackgroundTaskModule) {
    console.log('BackgroundTask: Setting up event listener for', ExpoBackgroundTaskModule.EVENT_PERFORM_WORK);
    // Ensure all events have the correct names from our types
    if (ExpoBackgroundTaskModule.EVENT_PERFORM_WORK !== 'onPerformWork') {
        throw new Error("Expected 'onPerformWork' got '" +
