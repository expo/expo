...
ruleTester.run("no-dynamic-env-var", rule, {
  ...
  invalid: [
    {
      code: 'const myVar = process.env["MY_VAR"]',
      errors: [{ message: "Unexpected dynamic access. Cannot dynamically access MY_VAR from process.env", type: "VariableDeclarator" }],
    },
    {
      code: 'const dynamicVar = "MY_VAR"; const myVar = process.env[dynamicVar];',
      errors: [{ message: "Unexpected dynamic access. Cannot dynamically access dynamicVar from process.env", type: "VariableDeclarator" }],
    },
  ],
});
