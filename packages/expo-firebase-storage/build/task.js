...
export default class StorageTask {
    ...
    _subscribe(nextOrObserver, error, complete) {
        ...
        if (_next) {
            this.storage._addListener(this.path, `Expo.Firebase.${StorageStatics.TaskEvent.STATE_CHANGED}`, _next);
        }
        if (_error) {
        ...
        return () => {
            if (_next)
                this.storage._removeListener(this.path, `Expo.Firebase.${StorageStatics.TaskEvent.STATE_CHANGED}`, _next);
            if (_error)
    ...
    on(event = StorageStatics.TaskEvent.STATE_CHANGED, nextOrObserver, error, complete) {
        ...
        invariant(event === StorageStatics.TaskEvent.STATE_CHANGED, `StorageTask.on event argument must be a string with a value of '${StorageStatics.TaskEvent.STATE_CHANGED}'`);
