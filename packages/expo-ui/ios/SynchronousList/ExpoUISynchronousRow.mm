...
// Never stop/unmount a surface inside SwiftUI teardown or a parent Fabric mount.
    ...
    if (presenter.scheduler && surface.surfaceHandler.getStatus() == SurfaceHandler::Status::Running) {
      ...
      if (scheduler) {
        scheduler->executeNowOnTheSameThread([&](jsi::Runtime &runtime) {
          auto unmount = runtime.global().getProperty(runtime, "__ExpoUIUnmountSyncRow");
...
@implementation ExpoUISynchronousRootPool
...
- (void)didMountComponentsWithRootTag:(ReactTag)rootTag
{
  ...
  // other updates directly to their owner, not every retained SwiftUI wrapper.
...
- (void)recordRenderDuration:(double)duration wait:(double)wait
{
  ...
  __weak ExpoUISynchronousRootPool *weakSelf = self;
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC), dispatch_get_main_queue(), ^{
    ExpoUISynchronousRootPool *pool = weakSelf;
    if (!pool) { return; }
    NSLog(@"[SwiftUISyncList] requests=%lu cached=%lu avgWait=%.2fms avgWork=%.2fms max=%.2fms created=%lu reused=%lu live=%lu idle=%lu",
...
@implementation ExpoUISynchronousRow {
  ExpoUISynchronousRootPool *_pool;
...
- (CGSize)renderWithWidth:(CGFloat)width
{
  ...
  if (!isfinite(width) || width <= 0) {
    return CGSizeZero; // An unspecified SwiftUI proposal must not create a root.
  }
  // SwiftUI proposals and UIKit bounds can differ by subpixel rounding.
  ...
  scheduler->executeNowOnTheSameThread([&](jsi::Runtime &runtime) {
    ...
    auto render = runtime.global().getPropertyAsFunction(runtime, "__ExpoUIRenderSyncRow");
...
- (void)didMountComponentsWithRootTag:(ReactTag)rootTag
{
  ...
  // Later React state changes may alter height. Notify SwiftUI after mounting,
  // without recursively rendering or changing layout in the mount callback.
  __weak ExpoUISynchronousRow *weakSelf = self;
  dispatch_async(dispatch_get_main_queue(), ^{
    ExpoUISynchronousRow *row = weakSelf;
...
- (void)didMoveToWindow
{
  ...
  if (!self.window) {
    // SwiftUI may retain an offscreen representable. Release its lease even if
    // dismantleUIView hasn't run. Avoid releasing during transient reparenting.
    __weak ExpoUISynchronousRow *weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
      ExpoUISynchronousRow *row = weakSelf;
...
- (void)dealloc
{
  if (_surface) {
    ...
    // SwiftUI can destroy a wrapper before dismantleUIView releases its lease.
    ...
    ExpoUISynchronousRootPool *pool = _pool;
