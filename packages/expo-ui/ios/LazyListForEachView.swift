...
struct LazyListForEachView: ExpoSwiftUI.View {
  ...
  var body: some View {
    ForEach(identify(props.items ?? [])) { item in
      DescriptorView(descriptor: item.descriptor)
    }
  }
}

// MARK: - Descriptor rendering

private struct IdentifiedDescriptor: Identifiable {
  let id: String
  let descriptor: [String: Any]
}

private func identify(_ items: [[String: Any]]) -> [IdentifiedDescriptor] {
...
private struct ChildrenView: View {
  ...
  var body: some View {
    ForEach(identify(children)) { item in
      DescriptorView(descriptor: item.descriptor)
    }
  }
}
...
//
// Kept inline to avoid pulling the full ModifierArray pipeline into the lazy path.
// The lazy DSL is intentionally a closed set of fields, not arbitrary modifiers.
