...
- [iOS] Documented the `matchContents` sizing contract on `Host` and `RNHostView`: the content's measured size must not derive from the granted size, or the layout renegotiates every pass and system chrome such as the Button Shapes accessibility setting turns it into an unbounded loop. ([#48058](https://github.com/expo/expo/issues/48058), [#48309](https://github.com/expo/expo/pull/48309) by [@intergalacticspacehighway](https://github.com/intergalacticspacehighway))
