...
- [iOS] Fix `tvOS` build failure in the SwiftUI `menuOrder` modifier — `MenuOrder.priority` is unavailable on tvOS, so it now falls back to `.automatic` there. ([#XXXXX](https://github.com/expo/expo/pull/XXXXX) by [@nishan](https://github.com/intergalacticspacehighway))
