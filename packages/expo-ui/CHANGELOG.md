...
- [iOS] Fixed a crash when a `TextField` or `SecureField` is unmounted while focused inside a list row that is being removed, for example closing a modal with the keyboard up on a field nested in `SwipeActions`. UIKit asserted with "The first responder contained inside of a deleted section or item refused to resign" because SwiftUI restored focus from its `@FocusState` during the delete pass. Hosted text inputs now blur before React removes their native views. ([#49348](https://github.com/expo/expo/issues/49348) by [@nishan](https://github.com/intergalacticspacehighway))
