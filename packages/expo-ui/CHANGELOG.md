...
- [android] Added `HorizontalPager` component wrapping Compose's `HorizontalPager`. State is driven through `usePagerNativeState`, which exposes `currentPage` (an `ObservableState<number>`) for reactive/worklet reads and `animateScrollToPage`/`scrollToPage` methods for programmatic navigation. ([#45163](https://github.com/expo/expo/pull/45163) by [@vonovak](https://github.com/vonovak))
