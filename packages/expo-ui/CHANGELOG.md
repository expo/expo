...
- [iOS] Fix the keyboard covering a focused `TextInput` in `community/bottom-sheet`. The sheet presents its content in its own view controller, so React Native lays that content out from the sheet's own origin and `KeyboardAvoidingView` computes too small an inset. `BottomSheetScrollView`, `BottomSheetFlatList` and `BottomSheetSectionList` now set `automaticallyAdjustKeyboardInsets`, which the native scroll view applies from its own window frame. ([#49327](https://github.com/expo/expo/pull/49327) by [@expo-bot](https://github.com/expo-bot))
