...
- [Android] Fix the `community/datetime-picker` (and Jetpack Compose `DatePickerDialog`) calendar rendering squeezed with overlapping rows when switching from text input to calendar mode while the keyboard is visible — the window manager kept the dialog window clamped to the space above the soft keyboard left over from input mode. ([#47273](https://github.com/expo/expo/issues/47273) by [@TomCorvus](https://github.com/TomCorvus))
