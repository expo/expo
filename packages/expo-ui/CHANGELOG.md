...
- [iOS] `Host` now defaults `ignoreSafeArea` to `'all'` when `matchContents` is set on either axis. A content-sized host is exactly as large as its content, so SwiftUI's safe-area and keyboard insets pushed the content outside the frame React Native laid out, leaving it visible but untappable at screen edges, shifted under the keyboard, and stuck once scrolled. Such hosts now lay out like any React Native view: the content starts at the frame origin and the app handles insets and keyboard avoidance. Pass `ignoreSafeArea` explicitly to keep the previous behavior. Hosts without `matchContents` are unchanged. Views hosted in `RNHostView` now report their position from the host's own frame, so `measure()` is correct when SwiftUI insets the content. ([#TBD](https://github.com/expo/expo/pull/TBD) by [@nishan](https://github.com/intergalacticspacehighway))
