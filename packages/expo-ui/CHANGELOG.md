...
- [Android] Fix hosted React Native content sliding off the top of a `BottomSheet` when the keyboard opens. Compose centers a child that is bigger than the space its parent gives it, so a `matchContents` host lost half of the overflow off the top once `imePadding` shrank the sheet. ([#49399](https://github.com/expo/expo/issues/49399) by [@starsky-nev](https://github.com/starsky-nev)) ([#PR](https://github.com/expo/expo/pull/PR) by [@intergalacticspacehighway](https://github.com/intergalacticspacehighway))
