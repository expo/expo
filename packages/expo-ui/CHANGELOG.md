...
- Added universal `Icon` component (`@expo/ui`) wrapping SF Symbols on iOS and `@expo/material-symbols` XML drawables on Android. Includes `Icon.select({ ios, android })` helper for cross-platform definitions, paired with a Babel plugin (`@expo/ui/babel-plugin`) that rewrites the call into a `Platform.OS` ternary and converts `import('*.xml')` into `require('*.xml')` so Metro DCE can strip the unused side per platform. The Android slot accepts both `require()` and `import()`; the `import()` form gives TypeScript validation of the literal path. The plugin is auto-loaded by `babel-preset-expo` when `@expo/ui` is installed; users on a custom Babel config must add `'@expo/ui/babel-plugin'` manually. `@expo/material-symbols` is now an optional peer dependency — install it directly to use Material Symbols icons on Android.
