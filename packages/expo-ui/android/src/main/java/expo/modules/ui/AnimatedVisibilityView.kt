...
private fun List<EnterTransitionRecord>.toComposedEnterTransition(): EnterTransition =
  map { it.toComposeTransition() }.reduceOrNull { acc, transition -> acc + transition } ?: (fadeIn() + expandIn())

private fun List<ExitTransitionRecord>.toComposedExitTransition(): ExitTransition =
  map { it.toComposeTransition() }.reduceOrNull { acc, transition -> acc + transition } ?: (fadeOut() + shrinkOut())
...
fun FunctionalComposableScope.AnimatedVisibilityContent(props: AnimatedVisibilityProps) {
  val enter = props.enterTransition?.takeIf { it.isNotEmpty() }?.toComposedEnterTransition() ?: (fadeIn() + expandIn())
  val exit = props.exitTransition?.takeIf { it.isNotEmpty() }?.toComposedExitTransition() ?: (fadeOut() + shrinkOut())
