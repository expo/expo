...
fun FunctionalComposableScope.SwitchContent(
  ...
  if (state != null) {
    StatefulSwitch(props, state)
  } else {
    DefaultSwitch(props, onCheckedChange)
  }
}

@Composable
private fun FunctionalComposableScope.StatefulSwitch(
  props: SwitchProps,
  state: ToggleState
) {
  Switch(
    checked = state.isOn,
    onCheckedChange = { state.isOn = it },
    modifier = ModifierRegistry.applyModifiers(props.modifiers, appContext, composableScope, globalEventDispatcher),
    enabled = props.enabled,
    thumbContent = resolveThumbContent(),
    colors = resolveSwitchColors(props)
  )
}

@Composable
private fun FunctionalComposableScope.DefaultSwitch(
  props: SwitchProps,
  onCheckedChange: (Boolean) -> Unit
) {
  Switch(
...
@Composable
private fun FunctionalComposableScope.resolveThumbContent(): (@Composable () -> Unit)? {
  val thumbContentSlotView = findChildSlotView(view, "thumbContent")
  return thumbContentSlotView?.let {
    {
...
@Composable
private fun resolveSwitchColors(props: SwitchProps) = SwitchDefaults.colors(
