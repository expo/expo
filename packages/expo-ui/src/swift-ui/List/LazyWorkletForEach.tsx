...
type NativeLazyWorkletForEachProps = CommonViewModifierProps & {
  data: number | null;
  idKey: string;
...
const LazyWorkletForEachNativeView: React.ComponentType<NativeLazyWorkletForEachProps> =
  requireNativeView<NativeLazyWorkletForEachProps>('ExpoUI', 'LazyListWorkletForEachView');
 ...
 */
export function LazyWorkletForEach<T>({
  data,
  idKey,
  renderItem,
  animated = true,
  ...props
}: LazyWorkletForEachProps<T>) {
  const workletCallback = useWorkletProp(renderItem as (...args: any[]) => any, 'renderItem');
  return (
    <LazyWorkletForEachNativeView
      {...props}
      data={getStateId(data)}
      idKey={(idKey as string) ?? 'id'}
      renderItem={getStateId(workletCallback)}
      animated={animated}
    />
  );
}
