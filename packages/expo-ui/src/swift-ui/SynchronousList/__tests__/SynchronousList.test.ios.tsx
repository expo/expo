...
const globals = globalThis as typeof globalThis & {
  __ExpoUIRenderSyncRow: (id: string, root: number, index: number) => boolean;
  __ExpoUIUnmountSyncRow: (root: number) => void;
};
...
it('does no React work before native requests an index and keeps list registrations isolated', () => {
  ...
  const a = registerSynchronousCellRenderer(first);
  const b = registerSynchronousCellRenderer(second);
  expect(first).not.toHaveBeenCalled();
  expect(second).not.toHaveBeenCalled();
  expect(globals.__ExpoUIRenderSyncRow(b.rendererId, 11, 9999)).toBe(true);
  expect(first).not.toHaveBeenCalled();
...
it('reuses the root and subtree key across row indices', () => {
  const registration = registerSynchronousCellRenderer(() => <React.Fragment />);
  globals.__ExpoUIRenderSyncRow(registration.rendererId, 11, 0);
  globals.__ExpoUIRenderSyncRow(registration.rendererId, 11, 1);
...
it('reports an unfinished commit and rejects requests after disposal', () => {
  ...
  const registration = registerSynchronousCellRenderer(() => <React.Fragment />);
  expect(globals.__ExpoUIRenderSyncRow(registration.rendererId, 11, 0)).toBe(false);
  registration.dispose();
  expect(() => globals.__ExpoUIRenderSyncRow(registration.rendererId, 11, 0)).toThrow(
    'is no longer registered'
  );
  // Native teardown still works after the renderer registration is removed.
  globals.__ExpoUIUnmountSyncRow(11);
...
it('updates recycled row components without remounting', () => {
  ...
  const registration = registerSynchronousCellRenderer((index) => <Row index={index} />);
  globals.__ExpoUIRenderSyncRow(registration.rendererId, 11, 0);
...
it('owns registration, renders data lazily, refreshes rows, and disposes on unmount', () => {
  ...
  const screen = render(<SynchronousList data={data} renderItem={renderItem} />);
  ...
  globals.__ExpoUIRenderSyncRow(id, 11, 0);
  ...
  screen.rerender(<SynchronousList data={updated} renderItem={replacement} extraData={1} />);
  globals.__ExpoUIRenderSyncRow(id, 11, 0);
  expect(replacement).toHaveBeenCalledTimes(1);
  const beforeExtraData = mockNativeProps.revision;
  screen.rerender(<SynchronousList data={updated} renderItem={replacement} extraData={2} />);
  ...
  screen.unmount();
  expect(() => globals.__ExpoUIRenderSyncRow(id, 11, 0)).toThrow('no longer registered');
});

it('handles empty data and reuses the subtree when populated', () => {
  const { SynchronousList } = require('..');
  const renderItem = jest.fn(() => <React.Fragment />);
  const screen = render(<SynchronousList data={[]} renderItem={renderItem} />);
  expect(mockNativeProps.itemCount).toBe(0);
  expect(renderItem).not.toHaveBeenCalled();
  const id = mockNativeProps.rendererId;
  screen.rerender(<SynchronousList data={[1, 2]} renderItem={renderItem} />);
  globals.__ExpoUIRenderSyncRow(id, 11, 0);
  globals.__ExpoUIRenderSyncRow(id, 11, 1);
...
it('rejects a missing item before calling renderItem', () => {
  const { SynchronousList } = require('..');
  const renderItem = jest.fn(() => <React.Fragment />);
  const screen = render(<SynchronousList data={['row']} renderItem={renderItem} />);
  expect(() => globals.__ExpoUIRenderSyncRow(mockNativeProps.rendererId, 11, 1)).toThrow(
