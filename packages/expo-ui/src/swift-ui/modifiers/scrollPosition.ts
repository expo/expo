 ...
 *
 * @param value - String identifier matched by `scrollPosition`'s observable state.
 *
 * @see Official [SwiftUI documentation](https://developer.apple.com/documentation/swiftui/view/id(_:)).
 */
export const id = (value: string) => createModifier('id', { id: value });
...
export const scrollPosition = (
  state: ObservableState<string | null>,
  options?: {
  ...
  if (onChange) {
    return createModifierWithEventListener(
      'scrollPosition',
      (event: { id?: string | null }) => onChange(event?.id ?? null),
      params
    );
  }
  return createModifier('scrollPosition', params);
};
