...
describe('Button', () => {
  it('maps the variant prop to a buttonStyle modifier', async () => {
    await render(<Button label="Tap" variant="outlined" />);
  ...
  it('does not inject the variant buttonStyle when the user supplies a buttonStyle modifier', async () => {
    await render(<Button label="Tap" modifiers={[buttonStyle('glassProminent')]} />);
    expect(nativeButtonModifiers()).toEqual([{ $type: 'buttonStyle', style: 'glassProminent' }]);
  });

  it('prefers a user buttonStyle modifier over the variant prop', async () => {
    await render(<Button label="Tap" variant="text" modifiers={[buttonStyle('automatic')]} />);
    expect(nativeButtonModifiers()).toEqual([{ $type: 'buttonStyle', style: 'automatic' }]);
  });

  it('keeps the variant buttonStyle when the user supplies unrelated modifiers', async () => {
    await render(<Button label="Tap" variant="text" modifiers={[controlSize('large')]} />);
