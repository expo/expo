...
type SyncRowGlobal = typeof globalThis & {
  __ExpoUISyncCollectionRowRegistry?: Map<string, Renderer>;
  __ExpoUISyncCollectionRowNextId?: number;
  __ExpoUIRenderSyncCollectionRow?: (rendererId: string, rootTag: number, index: number) => boolean;
  __ExpoUIUnmountSyncCollectionRow?: (rootTag: number) => void;
};

/**
 * Internal registration owned by SynchronousCollectionList. Uses RNTester's legacy Fabric root
 ...
 */
export function registerSynchronousCollectionRenderer(render: Renderer): {
  rendererId: string;
  dispose: () => void;
  update: (render: Renderer) => void;
} {
  const globals = globalThis as SyncRowGlobal;
  const registry = (globals.__ExpoUISyncCollectionRowRegistry ??= new Map());
  // Keep the counter and registry through Fast Refresh so IDs cannot collide.
  const rendererId = `sync-collection-row-${(globals.__ExpoUISyncCollectionRowNextId =
    (globals.__ExpoUISyncCollectionRowNextId ?? 0) + 1)}`;
  ...
  globals.__ExpoUIRenderSyncCollectionRow = (id, rootTag, index) => {
  ...
  globals.__ExpoUIUnmountSyncCollectionRow = (rootTag) => fabric.unmountComponentAtNode(rootTag);
