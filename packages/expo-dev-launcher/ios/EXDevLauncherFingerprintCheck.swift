...
internal struct FingerprintCheckRequest: Equatable {
  ...
  internal static func parse(_ url: URL) -> FingerprintCheckRequest? {
    // Matched on a reserved query parameter, not on a host: the host of a deep link belongs to the
    // app, and a launcher command must not take a name out of it.
...
internal func isPrivateAddress(_ host: String) -> Bool {
  ...
  if host.withCString({ inet_pton(AF_INET, $0, &ipv4Addr) }) == 1 {
    // `inet_pton` writes the four octets into `s_addr` in network order, and copying that value
    // back out lays the same bytes down again, so the octets read in order on either endianness.
 ...
 * device, so it opens this responder with a URL and the app posts its embedded fingerprint back.
 *
