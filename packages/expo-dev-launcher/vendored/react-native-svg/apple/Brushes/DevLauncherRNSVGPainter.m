...
@implementation DevLauncherRNSVGPainter
{
    NSArray<DevLauncherRNSVGLength *> *_points;
    NSArray<NSNumber *> *_colors;
    DevLauncherRNSVGBrushType _type;
...
- (instancetype)initWithPointsArray:(NSArray<DevLauncherRNSVGLength *> *)pointsArray
{
    if ((self = [super init])) {
        _points = pointsArray;
    }
...
{
    _useObjectBoundingBox = unit == kDevLauncherRNSVGUnitsObjectBoundingBox;
}

- (void)setContentUnits:(DevLauncherRNSVGUnits)unit
{
    _useContentObjectBoundingBox = unit == kDevLauncherRNSVGUnitsObjectBoundingBox;
}
...
- (void)setPattern:(DevLauncherRNSVGPattern *)pattern
{
    if (_type != kDevLauncherRNSVGUndefinedType) {
        // todo: throw error
        return;
    }

    _type = kDevLauncherRNSVGPattern;
    _pattern = pattern;
}
...
- (void)setLinearGradientColors:(NSArray<NSNumber *> *)colors
{
    ...
    _type = kDevLauncherRNSVGLinearGradient;
    _colors = colors;
}
...
- (void)setRadialGradientColors:(NSArray<NSNumber *> *)colors
{
    ...
    _type = kDevLauncherRNSVGRadialGradient;
    _colors = colors;
}

- (void)paint:(CGContextRef)context bounds:(CGRect)bounds
{
    if (_type == kDevLauncherRNSVGLinearGradient) {
        [self paintLinearGradient:context bounds:bounds];
    } else if (_type == kDevLauncherRNSVGRadialGradient) {
        [self paintRadialGradient:context bounds:bounds];
    } else if (_type == kDevLauncherRNSVGPattern) {
...
void __PatternFunction__(void* info, CGContextRef context)
{
    ...
    if (vbWidth > 0 && vbHeight > 0) {
        ...
        CGAffineTransform _viewBoxTransform = [DevLauncherRNSVGViewBox
                                               getTransform:vbRect
                                               eRect:rect
...
- (CGFloat)getVal:(DevLauncherRNSVGLength*)length relative:(CGFloat)relative
{
...
- (void)paintLinearGradient:(CGContextRef)context bounds:(CGRect)bounds
{
    ...
    CGGradientRef gradient = CGGradientRetain([RCTConvert DevLauncherRNSVGCGGradient:_colors]);
