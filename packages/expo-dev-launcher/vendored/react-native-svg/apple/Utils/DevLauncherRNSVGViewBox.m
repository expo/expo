...
@implementation DevLauncherRNSVGViewBox
...
+ (CGAffineTransform)getTransform:(CGRect)vbRect eRect:(CGRect)eRect align:(NSString *)align meetOrSlice:(DevLauncherRNSVGVBMOS)meetOrSlice
{
    ...
    // If align is 'none'
    if (meetOrSlice == kDevLauncherRNSVGVBMOSNone) {
        ...
        if (![align isEqualToString: @"none"] && meetOrSlice == kDevLauncherRNSVGVBMOSMeet) {
            scaleX = scaleY = fmin(scaleX, scaleY);
        } else if (![align isEqualToString: @"none"] && meetOrSlice == kDevLauncherRNSVGVBMOSSlice) {
