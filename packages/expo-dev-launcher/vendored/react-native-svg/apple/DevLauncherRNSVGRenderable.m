...
#import "DevLauncherRNSVGViewBox.h"
...
@implementation DevLauncherRNSVGRenderable
{
    NSMutableDictionary *_originProperties;
    NSArray<NSString *> *_lastMergedList;
    NSArray<NSString *> *_attributeList;
    NSArray<DevLauncherRNSVGLength *> *_sourceStrokeDashArray;
    CGFloat *_strokeDashArrayData;
    CGPathRef _srcHitPath;
}

static DevLauncherRNSVGRenderable * _contextElement;
+ (DevLauncherRNSVGRenderable *)contextElement { return _contextElement; }
+ (void)setContextElement:(DevLauncherRNSVGRenderable *)contextElement { _contextElement = contextElement; }
...
- (id)init
{
    if (self = [super init]) {
        _fillOpacity = 1;
        _strokeOpacity = 1;
        _strokeWidth = [DevLauncherRNSVGLength lengthWithNumber:1];
        _fillRule = kDevLauncherRNSVGCGFCRuleNonzero;
    }
    return self;
}
...
- (void)setFill:(DevLauncherRNSVGBrush *)fill
{
    if (fill == _fill) {
        return;
    }
...
- (void)setFillRule:(DevLauncherRNSVGCGFCRule)fillRule
{
...
- (void)setStroke:(DevLauncherRNSVGBrush *)stroke
{
    if (stroke == _stroke) {
        return;
    }
...
- (void)setStrokeWidth:(DevLauncherRNSVGLength*)strokeWidth
{
...
- (void)setStrokeDasharray:(NSArray<DevLauncherRNSVGLength *> *)strokeDasharray
{
...
- (void)setVectorEffect:(DevLauncherRNSVGVectorEffect)vectorEffect
{
...
- (void)renderTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (self.mask) {
        ...
        DevLauncherRNSVGMask *_maskNode = (DevLauncherRNSVGMask*)[self.svgView getDefinedMask:self.mask];
...
- (void)prepareStrokeDash:(NSUInteger)count strokeDasharray:(NSArray<DevLauncherRNSVGLength *> *)strokeDasharray {
...
- (void)renderMarkers:(CGContextRef)context path:(CGPathRef)path rect:(const CGRect *)rect {
    DevLauncherRNSVGMarker *markerStart = (DevLauncherRNSVGMarker*)[self.svgView getDefinedMarker:self.markerStart];
    DevLauncherRNSVGMarker *markerMid = (DevLauncherRNSVGMarker*)[self.svgView getDefinedMarker:self.markerMid];
    DevLauncherRNSVGMarker *markerEnd = (DevLauncherRNSVGMarker*)[self.svgView getDefinedMarker:self.markerEnd];
    if (markerStart || markerMid || markerEnd) {
        _contextElement = self;
        NSArray<DevLauncherRNSVGMarkerPosition*>* positions = [DevLauncherRNSVGMarkerPosition fromCGPath:path];
        ...
        for (DevLauncherRNSVGMarkerPosition* position in positions) {
            DevLauncherRNSVGMarkerType type = [position type];
            DevLauncherRNSVGMarker *marker;
            switch (type) {
                case kStartMarker:
                    marker = markerStart;
                    break;
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (_vectorEffect == kDevLauncherRNSVGVectorEffectNonScalingStroke) {
    ...
    BOOL evenodd = self.fillRule == kDevLauncherRNSVGCGFCRuleEvenodd;

    if (self.fill) {
        if (self.fill.class == DevLauncherRNSVGBrush.class) {
    ...
    if (self.stroke) {
        ...
        NSArray<DevLauncherRNSVGLength *>* strokeDasharray = self.strokeDasharray;
        ...
        if (self.stroke.class == DevLauncherRNSVGBrush.class) {
...
- (DevLauncherRNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (self.clipPath) {
        DevLauncherRNSVGClipPath *clipNode = (DevLauncherRNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if ([clipNode isSimpleClipPath]) {
            CGPathRef clipPath = [self getClipPath];
            if (clipPath && !CGPathContainsPoint(clipPath, nil, transformed, clipNode.clipRule == kDevLauncherRNSVGCGFCRuleEvenodd)) {
                return nil;
            }
        } else {
            DevLauncherRNSVGRenderable *clipGroup = (DevLauncherRNSVGRenderable*)clipNode;
