...
static NSCharacterSet *DevLauncherRNSVGTSpan_separators = nil;
static CGFloat DevLauncherRNSVGTSpan_radToDeg = 180 / (CGFloat)M_PI;

@implementation DevLauncherRNSVGTSpan
{
    CGFloat startOffset;
    DevLauncherRNSVGTextPath *textPath;
    ...
    CGFloat firstX;
    CGFloat firstY;
    DevLauncherRNSVGPathMeasure *measure;
}
...
- (id)init
{
    ...
    if (DevLauncherRNSVGTSpan_separators == nil) {
        DevLauncherRNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
    }

    emoji = [NSMutableArray arrayWithCapacity:0];
    emojiTransform = [NSMutableArray arrayWithCapacity:0];
    measure = [[DevLauncherRNSVGPathMeasure alloc]init];
        ...
        if (self.inlineSize != nil && self.inlineSize.value != 0) {
            ...
            if (self.path) {
                ...
                for (NSUInteger i = 0; i < count; i++) {
                    DevLauncherRNSVGPlatformView *label = [emoji objectAtIndex:i];
...
- (NSMutableDictionary *)getAttributes:(DevLauncherRNSVGFontData *)fontdata {
    ...
    bool allowOptionalLigatures = letterSpacing == 0 && fontdata->fontVariantLigatures == DevLauncherRNSVGFontVariantLigaturesNormal;
...
- (void)drawWrappedText:(CGContextRef)context gc:(DevLauncherRNSVGGlyphContext *)gc rect:(CGRect)rect color:(CGColorRef)color {
    ...
    DevLauncherRNSVGFontData* fontdata = [gc getFont];
    ...
    enum DevLauncherRNSVGTextAnchor textAnchor = fontdata->textAnchor;
    NSTextAlignment align;
    switch (textAnchor) {
        case DevLauncherRNSVGTextAnchorStart:
            align = NSTextAlignmentLeft;
            break;

        case DevLauncherRNSVGTextAnchorMiddle:
            align = NSTextAlignmentCenter;
            break;

        case DevLauncherRNSVGTextAnchorEnd:
  ...
  __label___.textColor = [DevLauncherRNSVGColor colorWithCGColor:color];
    ...
    CGFloat width = [DevLauncherRNSVGPropHelper fromRelative:self.inlineSize
...
- (CGPathRef)getLinePath:(NSString *)str context:(CGContextRef)context
{
    ...
    DevLauncherRNSVGGlyphContext *gc = [self.textRoot getGlyphContext];
    DevLauncherRNSVGFontData *font = [gc getFont];
     ...
     */
    bool allowOptionalLigatures = letterSpacing == 0 && font->fontVariantLigatures == DevLauncherRNSVGFontVariantLigaturesNormal;
     ...
     */
    enum DevLauncherRNSVGTextAnchor textAnchor = font->textAnchor;
    DevLauncherRNSVGText *anchorRoot = [self getTextAnchorRoot];
    CGFloat textMeasure = [anchorRoot getSubtreeTextChunksTotalAdvance];
    CGFloat offset = [DevLauncherRNSVGTSpan getTextAnchorOffset:textAnchor width:textMeasure];
    ...
    if (hasTextPath) {
        //sharpMidLine = DevLauncherRNSVGTextPathMidLineFromString([textPath midLine]) == DevLauncherRNSVGTextPathMidLineSharp;
        /*
         Name
         side
         Value
         ...
         */
        side = DevLauncherRNSVGTextPathSideFromString([textPath side]) == DevLauncherRNSVGTextPathSideRight ? -1 : 1;
        /*
         Name
         startOffset
         Value
         ...
         the path is reached.
         */
        CGFloat absoluteStartOffset = [DevLauncherRNSVGPropHelper fromRelative:textPath.startOffset
                                                          relative:measure.pathLength
                                                          fontSize:fontSize];
        offset += absoluteStartOffset;
        if (measure.isClosed) {
            CGFloat halfPathDistance = measure.pathLength / 2;
            startOfRendering = absoluteStartOffset + (textAnchor == DevLauncherRNSVGTextAnchorMiddle ? -halfPathDistance : 0);
            endOfRendering = startOfRendering + measure.pathLength;
        }
        /*
         DevLauncherRNSVGTextPathSpacing spacing = textPath.getSpacing();
         if (spacing == DevLauncherRNSVGTextPathSpacing.auto) {
         // Hmm, what to do here?
    ...
    DevLauncherRNSVGLength *mTextLength = [self textLength];
    enum DevLauncherRNSVGTextLengthAdjust mLengthAdjust = DevLauncherRNSVGTextLengthAdjustFromString([self lengthAdjust]);
    if (mTextLength != nil) {
        CGFloat author = [DevLauncherRNSVGPropHelper fromRelative:mTextLength
                                             relative:[gc getWidth]
                                             fontSize:fontSize];
        if (author < 0) {
        ...
        switch (mLengthAdjust) {
            default:
            case DevLauncherRNSVGTextLengthAdjustSpacing:
                // TODO account for ligatures
                letterSpacing += (author - textMeasure) / (n - 1);
                break;
            case DevLauncherRNSVGTextLengthAdjustSpacingAndGlyphs:
    ...
    */
    ...
    enum DevLauncherRNSVGAlignmentBaseline baseline = DevLauncherRNSVGAlignmentBaselineFromString(self.alignmentBaseline);
    if (baseline != DevLauncherRNSVGAlignmentBaselineBaseline) {
        ...
        switch (baseline) {
            ...
            default:
            case DevLauncherRNSVGAlignmentBaselineBaseline:
                // Use the dominant baseline choice of the parent.
                // Match the box’s corresponding baseline to that of its parent.
                baselineShift = 0;
                break;

            case DevLauncherRNSVGAlignmentBaselineTextBottom:
            case DevLauncherRNSVGAlignmentBaselineAfterEdge:
            case DevLauncherRNSVGAlignmentBaselineTextAfterEdge:
            ...
            case DevLauncherRNSVGAlignmentBaselineCentral:
            ...
            case DevLauncherRNSVGAlignmentBaselineMathematical:
            ...
            case DevLauncherRNSVGAlignmentBaselineHanging:
                baselineShift = (CGFloat)0.8 * ascenderHeight;
                break;

            case DevLauncherRNSVGAlignmentBaselineTextTop:
            case DevLauncherRNSVGAlignmentBaselineBeforeEdge:
            case DevLauncherRNSVGAlignmentBaselineTextBeforeEdge:
            ...
            case DevLauncherRNSVGAlignmentBaselineBottom:
                // Align the top of the aligned subtree with the top of the line box.
                baselineShift = bottom;
                break;

            case DevLauncherRNSVGAlignmentBaselineCenter:
                // Align the center of the aligned subtree with the center of the line box.
                baselineShift = totalHeight / 2;
                break;

            case DevLauncherRNSVGAlignmentBaselineTop:
        ...
        switch (baseline) {
            ...
            default:
                if (fontData != nil && [baselineShiftString isEqualToString:@"sub"]) {
                    ...
                    baselineShift -= [DevLauncherRNSVGPropHelper fromRelativeWithNSString:baselineShiftString
                                                                   relative:fontSize
                                                                   fontSize:fontSize];
                }
                break;
    ...
    for (CFIndex ri = 0; ri < runEnd; ri++) {
        ...
        for(CFIndex g = 0; g < runGlyphCount; g++) {
            ...
            bool isWordSeparator = [DevLauncherRNSVGTSpan_separators characterIsMember:currentChar];
            ...
            CGFloat r = [gc nextRotation] / DevLauncherRNSVGTSpan_radToDeg;
            ...
            if (width == 0) { // Render unicode emoji
                DevLauncherRNSVGTextView *label = [[DevLauncherRNSVGTextView alloc] init];
...
+ (CGFloat)getTextAnchorOffset:(DevLauncherRNSVGTextAnchor)textAnchor width:(CGFloat) width
{
    switch (textAnchor) {
        case DevLauncherRNSVGTextAnchorStart:
            return 0;
...
{
    textPath = nil;
    DevLauncherRNSVGText *parent = (DevLauncherRNSVGText*)[self superview];
    CGPathRef path = nil;
    while (parent) {
        if ([parent class] == [DevLauncherRNSVGTextPath class]) {
            textPath = (DevLauncherRNSVGTextPath*) parent;
            DevLauncherRNSVGNode *template = [self.svgView getDefinedTemplate:textPath.href];
            path = [template getPath:context];
            [measure extractPathData:path];
            break;
        } else if (![parent isKindOfClass:[DevLauncherRNSVGText class]]) {
            break;
        }
        parent = (DevLauncherRNSVGText*)[parent superview];
    }
    if (!path) {
        [measure reset];
    }
