...
@interface DevLauncherRNSVGGlyphContext () {
@public
    ...
    NSMutableArray<NSArray<DevLauncherRNSVGLength*>*> *mXsContext_;
    NSMutableArray<NSArray<DevLauncherRNSVGLength*>*> *mYsContext_;
    NSMutableArray<NSArray<DevLauncherRNSVGLength*>*> *mDXsContext_;
    NSMutableArray<NSArray<DevLauncherRNSVGLength*>*> *mDYsContext_;
    NSMutableArray<NSArray<DevLauncherRNSVGLength*>*> *mRsContext_;
    ...
    CGFloat mFontSize_;
    DevLauncherRNSVGFontData *topFont_;
    ...
    NSArray<DevLauncherRNSVGLength*> *mXs_;

    // https://www.w3.org/TR/SVG/text.html#TSpanElementYAttribute
    NSArray<DevLauncherRNSVGLength*> *mYs_;
    ...
    NSArray<DevLauncherRNSVGLength*> *mDXs_;

    // https://www.w3.org/TR/SVG/text.html#TSpanElementDYAttribute
    NSArray<DevLauncherRNSVGLength*> *mDYs_;
    ...
    NSArray<DevLauncherRNSVGLength*> *mRs_;
...
@implementation DevLauncherRNSVGGlyphContext
...
- (CTFontRef)getGlyphFont
{
    ...
    NSString *fontStyle = DevLauncherRNSVGFontStyleStrings[topFont_->fontStyle];
    NSString *fontWeight = DevLauncherRNSVGFontWeightStrings[topFont_->fontWeight];
...
- (instancetype)initWithWidth:(CGFloat)width
    ...
    self->mFontSize_ = DevLauncherRNSVGFontData_DEFAULT_FONT_SIZE;
    self->topFont_ = [DevLauncherRNSVGFontData Defaults];

    self->mXs_ = [[NSArray alloc]init];
    self->mYs_ = [[NSArray alloc]init];
    self->mDXs_ = [[NSArray alloc]init];
    self->mDYs_ = [[NSArray alloc]init];
    self->mRs_ = [[NSArray alloc]initWithObjects:[DevLauncherRNSVGLength lengthWithNumber:0], nil];
...
- (DevLauncherRNSVGFontData *)getTopOrParentFont:(DevLauncherRNSVGGroup *)child
{
    if (self->mTop_ > 0) {
        return self->topFont_;
    } else {
        DevLauncherRNSVGGroup *parentRoot = [child getParentTextRoot];
        DevLauncherRNSVGFontData *Defaults = [DevLauncherRNSVGFontData Defaults];
        while (parentRoot != nil) {
            DevLauncherRNSVGFontData *map = [[parentRoot getGlyphContext] getFont];
            if (map != Defaults) {
                return map;
            }
...
- (void)pushNode:(DevLauncherRNSVGGroup *)node andFont:(NSDictionary *)font
{
    DevLauncherRNSVGFontData *parent = [self getTopOrParentFont:node];
    self->mTop_++;
    if (font == nil) {
        [self->mFontContext_ addObject:parent];
        return;
    }
    DevLauncherRNSVGFontData *data = [DevLauncherRNSVGFontData initWithNSDictionary:font
                                                       parent:parent];
...
- (void)pushContext:(DevLauncherRNSVGText*)node
             ...
             deltaY:(NSArray<DevLauncherRNSVGLength*>*)deltaY
             rotate:(NSArray<DevLauncherRNSVGLength*>*)rotate {
    [self pushNode:(DevLauncherRNSVGGroup*)node andFont:font];
    if (x != nil && [x count] != 0) {
        mXsIndex_++;
        mXIndex_ = -1;
...
- (CGFloat)nextXWithDouble:(CGFloat)advance {
    [DevLauncherRNSVGGlyphContext incrementIndices:mXIndices_ topIndex:mXsIndex_];
    long nextIndex = mXIndex_ + 1;
    if (nextIndex < [mXs_ count]) {
        mDX_ = 0;
        mXIndex_ = nextIndex;
        DevLauncherRNSVGLength *length = [mXs_ objectAtIndex:nextIndex];
        mX_ = [DevLauncherRNSVGPropHelper fromRelative:length
                                   relative:mWidth_
                                   fontSize:mFontSize_];
    }
    mX_ += advance;
...
- (CGFloat)nextY {
    [DevLauncherRNSVGGlyphContext incrementIndices:mYIndices_ topIndex:mYsIndex_];
    long nextIndex = mYIndex_ + 1;
    if (nextIndex < [mYs_ count]) {
        mDY_ = 0;
        mYIndex_ = nextIndex;
        DevLauncherRNSVGLength *length = [mYs_ objectAtIndex:nextIndex];
        mY_ = [DevLauncherRNSVGPropHelper fromRelative:length
                                   relative:mHeight_
                                   fontSize:mFontSize_];
    }
    return mY_;
...
- (CGFloat)nextDeltaX {
    [DevLauncherRNSVGGlyphContext incrementIndices:mDXIndices_ topIndex:mDXsIndex_];
    long nextIndex = mDXIndex_ + 1;
    if (nextIndex < [mDXs_ count]) {
        mDXIndex_ = nextIndex;
        DevLauncherRNSVGLength *length = [mDXs_ objectAtIndex:nextIndex];
        CGFloat val = [DevLauncherRNSVGPropHelper fromRelative:length
                                          relative:mWidth_
                                          fontSize:mFontSize_];
        mDX_ += val;
    }
...
- (CGFloat)nextDeltaY {
    [DevLauncherRNSVGGlyphContext incrementIndices:mDYIndices_ topIndex:mDYsIndex_];
    long nextIndex = mDYIndex_ + 1;
    if (nextIndex < [mDYs_ count]) {
        mDYIndex_ = nextIndex;
        DevLauncherRNSVGLength *length = [mDYs_ objectAtIndex:nextIndex];
        CGFloat val = [DevLauncherRNSVGPropHelper fromRelative:length
                                          relative:mHeight_
                                          fontSize:mFontSize_];
        mDY_ += val;
...
- (CGFloat)nextRotation {
    [DevLauncherRNSVGGlyphContext incrementIndices:mRIndices_ topIndex:mRsIndex_];
