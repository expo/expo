...
@implementation DevLauncherRNSVGGroup
{
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[DevLauncherRNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(DevLauncherRNSVGNode *node) {
        if ([node isKindOfClass:[DevLauncherRNSVGNode class]] && ![node isKindOfClass:[DevLauncherRNSVGMask class]]) {
...
- (DevLauncherRNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [DevLauncherRNSVGNode class]];
    ...
    for (DevLauncherRNSVGView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[DevLauncherRNSVGNode class]]) {
            if ([node isKindOfClass:[DevLauncherRNSVGMask class]]) {
                continue;
            }
            DevLauncherRNSVGNode* svgNode = (DevLauncherRNSVGNode*)node;
            if (event) {
            ...
            DevLauncherRNSVGPlatformView *hitChild = [svgNode hitTest:transformed withEvent:event];
            if (hitChild) {
                svgNode.active = YES;
            ...
            DevLauncherRNSVGSvgView* svgView = (DevLauncherRNSVGSvgView*)node;
            DevLauncherRNSVGPlatformView *hitChild = [svgView hitTest:transformed withEvent:event];
            if (hitChild) {
                return hitChild;
            }
        }
    ...
    DevLauncherRNSVGPlatformView *hitSelf = [super hitTest:transformed withEvent:event];
    if (hitSelf) {
        return hitSelf;
    }
...
{
    [self traverseSubviews:^(__kindof DevLauncherRNSVGNode *node) {
        if ([node isKindOfClass:[DevLauncherRNSVGRenderable class]]) {
            [(DevLauncherRNSVGRenderable*)node resetProperties];
        }
        return YES;
