...
internal class DevLauncherManifestTest {
  ...
  fun `checks if fromJson can parse manifest`() {
    ...
    val manifestReader = StringReader(manifest)
    val manifestObj = DevLauncherManifest.fromJson(manifestReader)
    manifestReader.close()

    Truth.assertThat(manifestObj).isNotNull()
    Truth.assertThat(manifestObj.slug).isEqualTo("native-component-list")
    Truth.assertThat(manifestObj.hostUri).isEqualTo("127.0.0.1:19000")
    Truth.assertThat(manifestObj.primaryColor).isEqualTo("#cccccc")
    Truth.assertThat(manifestObj.bundleUrl).isEqualTo("http://127.0.0.1:19000/__generated__/AppEntry.bundle?platform=ios&dev=true&hot=false&minify=false")
    Truth.assertThat(manifestObj.orientation).isEqualTo(DevLauncherOrientation.DEFAULT)
  }
  ...
  fun `checks if fromJson can parse android subsection`() {
    ...
    Truth.assertThat(manifestObj).isNotNull()
    Truth.assertThat(manifestObj.userInterfaceStyle).isEqualTo(DevLauncherUserInterface.DARK)
    Truth.assertThat(manifestObj.backgroundColor).isEqualTo("#ff0000")
  }
  ...
  fun `checks if fromJson throws on invalid json`() {
    ...
    val manifestReader = StringReader(manifest)
    val failure = assertThrows(JsonSyntaxException::class.java) {
      DevLauncherManifest.fromJson(manifestReader)
    }
    manifestReader.close()
