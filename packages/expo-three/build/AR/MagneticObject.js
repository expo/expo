...
class MagneticObject extends THREE.Object3D {
    constructor() {
        ...
        this.maintainScale = true;
        this.maintainRotation = true;
        this.updateForAnchor = (position, planeAnchor, camera) => {
            if (planeAnchor != null) {
        ...
        this.update = async (camera, screenPosition) => {
            const { worldPosition, planeAnchor } = await worldPositionFromScreenPosition(camera, screenPosition, this.position);
            if (worldPosition) {
        ...
        this.scaleBasedOnDistance = camera => {
            if (camera) {
                ...
                let newScale = distanceFromCamera < 0.7
...
//# sourceMappingURL=MagneticObject.js.map