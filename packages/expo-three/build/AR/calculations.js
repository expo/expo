...
export async function hitTestWithFeatures(camera, point, coneOpeningAngleInDegrees, minDistance = 0, maxDistance = 99999999999999, maxResults = 1, rawFeaturePoints) {
    let results = [];
    ...
    for (let feature of featurePoints) {
        ...
        const hitTestResultDistance = hitTestResult
            ...
            .sub(ray.origin)
            .length();
        if (hitTestResultDistance < minDistance ||
    ...
    results = results.sort((first, second) => first.distanceToRayOrigin < second.distanceToRayOrigin ? -1 : +1);
...
//-> [FeatureHitTestResult]
export async function hitTestWithPoint(camera, point) {
    var results = [];
    const ray = hitTestRayFromScreenPos(camera, point);
...
async function getRawFeaturePoints(rawFeaturePoints) {
    if (rawFeaturePoints) {
...
//-> FeatureHitTestResult?
export async function hitTestFromOrigin(origin, direction, rawFeaturePoints) {
...
export async function worldPositionFromScreenPosition(camera, position, objectPos, infinitePlane = false, dragOnInfinitePlanesEnabled = false, rawFeaturePoints) {
    // -------------------------------------------------------------------------------
    ...
    const highQualityfeatureHitTestResults = await hitTestWithFeatures(camera, position, 18, 0.2, 2.0, 1, rawFeaturePoints);
...
export function positionFromAnchor({ transformWorld }) {
    const transform = convertTransformArray(transformWorld);
    const position = positionFromTransform(transform);
    return position;
}
export async function improviseHitTest(point, camera) {
    const hitTestResults = await AR.performHitTestAsync(point, [AR.HitTestType.HorizontalPlane]);
    if (hitTestResults.length > 0) {
