...
async function parseBinAsync({ json, assetProvider }) {
    ...
    if (!buffers) {
        console.error(`ExpoTHREE.parseAsync: Invalid json! The json file must contain a "buffers" key.`);
        return;
    }
    const bin = await assetProvider(buffers);
    if (bin === undefined) {
        console.error(`ExpoTHREE.parseAsync: Cannot parse undefined .bin. Using the assetProvider you must pass a valid reference for: ${buffers}.`);
        return;
    }
