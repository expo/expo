...
static JSValueRef AppendDataCallback(
  ...
  AppendDataCallbackState *state = reinterpret_cast<AppendDataCallbackState*>(JSObjectGetPrivate(thisObject));
...
static void FinalizeAppendDataCallbackState(JSObjectRef object) {
  AppendDataCallbackState *state = reinterpret_cast<AppendDataCallbackState*>(JSObjectGetPrivate(object));
  free(state);
}
...
static JSObjectRef CreateAppendDataCallbackState(JSContextRef ctx) {
  ...
  AppendDataCallbackState *state = reinterpret_cast<AppendDataCallbackState*>(malloc(sizeof(AppendDataCallbackState)));
...
void JSContextPrepareTypedArrayAPI(JSContextRef ctx) {
  ...
  for (int type = kJSTypedArrayTypeInt8Array; type <= kJSTypedArrayTypeArrayBuffer; type++) {
    JSObjectRef jsConstructor = GetConstructor(ctx, static_cast<JSTypedArrayType>(type));
...
JSTypedArrayType JSObjectGetTypedArrayType(JSContextRef ctx, JSObjectRef object) {
  ...
  return jsType ? static_cast<JSTypedArrayType>(GetInt32(ctx, jsType)) : kJSTypedArrayTypeNone;
}
...
void *JSObjectGetTypedArrayDataMalloc(JSContextRef ctx, JSObjectRef object, size_t *plength) {
  ...
  if (int32Count) {
    ...
    AppendDataCallbackState *state = reinterpret_cast<AppendDataCallbackState*>(JSObjectGetPrivate(jsState));
    state->currentDataPtr = reinterpret_cast<int32_t*>(data);
  ...
  if (uint8Count) {
    uint8_t *values8 = reinterpret_cast<uint8_t*>(data);
...
void JSObjectSetTypedArrayData(JSContextRef ctx, JSObjectRef object, void *data, size_t length) {
  ...
  if (int32Count) {
    ...
    JSValueRef *jsValues = reinterpret_cast<JSValueRef*>(malloc(int32Count  * sizeof(JSValueRef)));
    int32_t *values32 = reinterpret_cast<int32_t*>(data);
