...
export class AudioPlayerWeb extends globalThis.expo.SharedObject {
    ...
    _connectMeter() {
        ...
        if (!this.isAudioSamplingSupported || !this.workletNode)
            return;
        try {
            if (this.panner)
                this.panner.connect(this.workletNode);
    ...
    _disconnectMeter() {
        ...
        try {
            if (this.panner)
                this.panner.disconnect(this.workletNode);
        }
        catch { }
        try {
            if (this.gainNode)
                this.gainNode.disconnect(this.workletNode);
        }
        catch { }
        try {
