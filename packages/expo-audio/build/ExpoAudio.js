...
AudioModule.AudioPlayer.prototype.setActiveForLockScreen = function (active, metadata, options) {
    if (active && metadata && 'artworkUrl' in metadata) {
        ...
        const isUnrecognizedProtocol = isString &&
            !url.startsWith('http://') &&
        ...
        if (!isString || isUnrecognizedProtocol || isDevServerUrl) {
            ...
            console.warn(`[expo-audio] Invalid or unsupported data provided for 'artworkUrl': ${valueStr} (type: ${typeStr}).\n` +
