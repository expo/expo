...
- [Android] Don't abandon audio focus during buffering and track transitions, which let other apps play on top of ongoing playback. Focus is now held across non-playing states only while a permanent `AUDIOFOCUS_GAIN` is held, and released when the player goes idle — the same condition as media3's `AudioFocusManager.shouldHandleAudioFocus`, so a ducking request is still abandoned as soon as playback stops. `doNotMix` requests `AUDIOFOCUS_GAIN` instead of `AUDIOFOCUS_GAIN_TRANSIENT`, the focus request declares `USAGE_MEDIA`, and focus is abandoned once the last player is removed. ([#48683](https://github.com/expo/expo/pull/48683) by [@sang88ho](https://github.com/sang88ho))
