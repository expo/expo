...
- [web] Improved `static` image source selection on web to be based on the rendered layout size by leading the generated `sizes` with `auto`, and default `static` images to `loading="lazy"` (opt out with `loading="eager"`). ([#XXXXX](https://github.com/expo/expo/pull/XXXXX) by [@sebholl](https://github.com/sebholl))

### 🐛 Bug fixes

### 💡 Others

- [web] Deprecated `webMaxViewportWidth`. With layout-based selection it is now only used to emit fallback `sizes` breakpoints for browsers that don't yet support `sizes="auto"`. ([#XXXXX](https://github.com/expo/expo/pull/XXXXX) by [@sebholl](https://github.com/sebholl))
