...
class BaseObservabilityManager(
  ...
  suspend fun dispatchUnsentSpans(): Unit = spansDispatchMutex.withLock {
    ...
    while (currentCoroutineContext().isActive) {
      ...
      // One batched read rather than a query per session, matching the metrics and logs paths
      // above and the iOS side. A backlog accumulated while offline can span many sessions, and
      // each halving retry would otherwise re-run every lookup.
