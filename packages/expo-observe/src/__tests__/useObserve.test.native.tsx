...
describe(useObserve, () => {
  it('returns the router-scoped markInteractive when useObserveForRouter resolves to a function', async () => {
    const routerScoped = jest.fn();
    mockUseObserveForRouter.mockReturnValue(routerScoped);

    const { result } = await renderHook(() => useObserve());

    expect(result.current.markInteractive).toBe(routerScoped);
  });

  it('returns the react-navigation-scoped markInteractive when only that integration is active', async () => {
  ...
  it('prefers router over react-navigation when both happen to resolve', async () => {
    const routerScoped = jest.fn();
    const reactNavigationScoped = jest.fn();
    mockUseObserveForRouter.mockReturnValue(routerScoped);
    mockUseObserveForReactNavigation.mockReturnValue(reactNavigationScoped);
  ...
  it('falls back to AppMetrics.markInteractive when neither integration is active', async () => {
    mockUseObserveForRouter.mockReturnValue(null);
    mockUseObserveForReactNavigation.mockReturnValue(null);
  ...
  it('falls back to react-navigation when router is not initialized', async () => {
  ...
  it('falls back to AppMetrics.markInteractive when react-navigation and expo router are not initialized', async () => {
    isExpoRouterInitializedMock.mockReturnValue(false);
  ...
  it('returns an object with exactly one own key (markInteractive)', async () => {
    mockUseObserveForRouter.mockReturnValue(null);
    const { result } = await renderHook(() => useObserve());
