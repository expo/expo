...
/**
 * Normalizes the `networkTraces` sugar (`true` / `false` / `{ filter }`) into the config shape
 * the native producer persists. `configure` is a full replacement, so an absent `networkTraces`
 * resets to the default: enabled, no filter.
 */
function networkTracesConfigFromOption(networkTraces: ObserveConfig['networkTraces']) {
  const option = networkTraces ?? true;
  if (typeof option === 'boolean') {
    return { enabled: option };
  }
  return option.filter != null ? { enabled: true, filter: option.filter } : { enabled: true };
}
