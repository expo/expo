...
| [`<link />`][html-link]         |         None         |                Eject the `index.html` with `expo customize` and link resources directly with `<link />`                |
| [`<noscript />`][html-noscript] |         None         |                  Eject the `index.html` with `expo customize` and use `<noscript />` directly as HTML                  |
