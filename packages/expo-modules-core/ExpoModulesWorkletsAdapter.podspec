...
# Defined (possibly) by ExpoModulesWorklets.podspec — redefine defensively
# if this podspec is evaluated first so both stay in sync.
...
Pod::Spec.new do |s|
  ...
  s.summary        = 'Runtime adapter that bridges ExpoModulesWorklets to react-native-worklets. Only contributes sources when react-native-worklets is installed.'
  ...
  if shouldEnableWorkletsIntegration
    ...
    s.pod_target_xcconfig = {
      ...
      'GCC_PREPROCESSOR_DEFINITIONS' => "$(inherited) REACT_NATIVE_MINOR_VERSION=#{reactNativeTargetVersion}",
    }
    ...
    s.source_files = 'ios/WorkletsAdapter/**/*.{h,m,mm,cpp}'
    s.private_header_files = 'ios/WorkletsAdapter/**/*.h'
  else
    # CocoaPods refuses to validate an empty pod, so contribute a
    # single no-op compilation unit when react-native-worklets is
    # absent. All of the real `worklets::*`-touching sources stay
    # excluded in this branch.
    s.source_files = 'ios/WorkletsAdapter/Placeholder.m'
  end
end
