...
+ (BOOL)installExpoModulesHostObject:
    (nonnull id<EXAppContextProtocol>)appContext {
  EXRuntime *runtime = [appContext _runtime];

  // The runtime may be unavailable, e.g. remote debugger is enabled or it
  // hasn't been set yet.
  if (!runtime) {
    return false;
  }
  ...
  EXJavaScriptValue *coreProperty =
      [global getProperty:EXGlobalCoreObjectPropertyName];
  NSAssert([coreProperty isObject],
           @"The global core property should be an object");
  ...
  std::shared_ptr<expo::ExpoModulesHostObject> modulesHostObjectPtr =
      std::make_shared<expo::ExpoModulesHostObject>(appContextObj);
  EXJavaScriptObject *modulesHostObject =
      [runtime createHostObject:modulesHostObjectPtr];

  // Define the `global.expo.modules` object as a non-configurable, read-only
  // and enumerable property.
...
+ (void)installSharedObjectClass:(nonnull EXRuntime *)runtime
                        releaser:(void (^)(long))releaser {
  expo::SharedObject::installBaseClass(
      *[runtime get], [releaser](expo::SharedObject::ObjectId objectId) {
        releaser(objectId);
      });
}
