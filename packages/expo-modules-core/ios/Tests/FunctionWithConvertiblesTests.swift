...
struct FunctionWithConvertiblesTests {
  ...
  @Test
  func `converts arguments to CoreGraphics types`() async {
    let x = 18.3
    let y = -4.1
    let width = 734.6
    let height = 592.1
    ...
    await withCheckedContinuation { continuation in
      mockModuleHolder(appContext) {
        AsyncFunction(functionName) { (point: CGPoint, size: CGSize, vector: CGVector, rect: CGRect) in
          ...
          #expect(Double(vector.dx) == x)
          #expect(Double(vector.dy) == y)
  ...
  @Test
  func `converts arguments to CGColor`() async {
  ...
  struct RecordTests {
    ...
    @Test
    func `converts convertibles`() throws {
