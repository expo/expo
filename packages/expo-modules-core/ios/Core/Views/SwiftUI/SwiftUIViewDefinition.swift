   ...
   */
    public func UnwrappedChildren<T: View>(  // swiftlint:disable:this identifier_name
        ...
        return ForEach(0..<childrenArray.count, id: \.self) { index in
            ...
            AnyView(
                Group {
                    if let hostingView = child.view
                        as? (any ExpoSwiftUI.AnyHostingView) {
                        ...
                        transform(
                            AnyView(
                                content
                                    ...
                                    .environmentObject(
                                        ExpoSwiftUI.ShadowNodeProxy
                                            .SHADOW_NODE_MOCK_PROXY)
                            ),
                            true
                        )
                    } else {
...
extension ExpoSwiftUI {
   ...
   */
    public final class ViewDefinition<Props: ViewProps, ViewType: View<Props>>:
        ExpoModulesCore.ViewDefinition<HostingView<Props, ViewType>>
    {
        ...
        init(_ viewType: ViewType.Type) {
            ...
            let nameDefinitionElement = ViewNameDefinition(
                name: String(describing: viewType))
            super.init(
                HostingView<Props, ViewType>.self,
                elements: [nameDefinitionElement])
        }
        ...
        public override func createView(appContext: AppContext) -> UIView? {
                ...
                let view = HostingView(
                    viewType: ViewType.self, props: props,
                ...
                return UnimplementedExpoView(
                    appContext: appContext,
                    text:
                        "Rendering SwiftUI views is possible only with the New Architecture enabled"
                )
            #endif
        }

        public override func getSupportedPropNames() -> [String] {
            return dummyPropsMirror.children.compactMap {
