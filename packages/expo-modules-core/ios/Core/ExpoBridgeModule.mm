...
- (instancetype)init {
  if (self = [super init]) {
    ...
    // load time
    _appContext =
        (EXAppContext *)[EXAppContextFactoryRegistry createAppContext];
  }
  return self;
}
...
- (void)maybeSetupAppContext {
  ...
  if (runtime && ![[runtime global] hasProperty:@"expo"]) {
    NSLog(@"Expo is being initialized from the deprecated ExpoBridgeModule, "
          @"make sure to migrate to ExpoReactNativeFactory in your project");
