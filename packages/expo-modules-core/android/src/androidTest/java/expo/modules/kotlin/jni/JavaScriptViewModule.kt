...
class JavaScriptViewModule {
  ...
  @Test
  fun should_export_view_prototype_legacy() = withSingleModule({
    View(android.view.View::class) {
      AsyncFunction("viewFunction") { _: Int -> }
      AsyncFunction("anotherViewFunction") { 20 }
    }
  }) {
    val viewFunctions = evaluateScript("Object.getOwnPropertyNames($moduleRef.ViewPrototypes.TestModule)")
      .getArray()
      .map { it.getString() }
  ...
  @Test
  fun view_functions_should_be_callable_legacy() = withSingleModule({
    View(android.view.View::class) {
      AsyncFunction("viewFunction") { 123 }
    }
  }) {
    val result = callViewAsync("ViewPrototypes.TestModule", "viewFunction").getInt()
    Truth.assertThat(result).isEqualTo(123)
  }

  @Test
  fun view_functions_should_be_able_to_receives_view_as_argument_legacy() = withSingleModule({
    View(android.view.View::class) {
      AsyncFunction("viewFunction") { view: android.view.View ->
        Truth.assertThat(view).isNotNull()
        123
    ...
    val result = waitForAsyncFunction(
      """
      const nativeView = { nativeTag: 1 };
      Object.assign(nativeView.__proto__, $moduleRef.ViewPrototypes.TestModule);
      nativeView.viewFunction()
      """.trimIndent()
    ).getInt()
    Truth.assertThat(result).isEqualTo(123)
