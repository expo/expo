...
class JSISuspendableFunctionsTest {
  ...
  fun primitive_arguments_should_be_convertible() = withSingleModule({
    AsyncFunction("stringF") Coroutine { a: String -> a }
    AsyncFunction("intF") Coroutine { a: Int -> a }
    AsyncFunction("doubleF") Coroutine { a: Double -> a }
    AsyncFunction("floatF") Coroutine { a: Float -> a }
    AsyncFunction("boolF") Coroutine { a: Boolean -> a }
    AsyncFunction("longF") Coroutine { a: Long -> a }
  }) {
  ...
  fun simple_list_should_be_convertible() = withSingleModule({
    AsyncFunction("listF") Coroutine { a: List<String> -> a }
  }) {
  ...
  fun complex_list_should_be_convertible() = withSingleModule({
    AsyncFunction("listF") Coroutine { a: List<List<Int>> -> a }
  }) {
  ...
  fun map_should_be_convertible() = withSingleModule({
    AsyncFunction("mapF") Coroutine { a: Map<String, String> -> a }
  }) {
  ...
  fun set_should_be_convertible() = withSingleModule({
    AsyncFunction("setF") Coroutine { a: Set<String> -> a }
  }) {
  ...
  fun enums_should_be_obtainable_as_function_argument() {
    ...
    withSingleModule({
      AsyncFunction("f1") Coroutine { enum: SimpleEnumClass ->
        f1WasCalled = true
        Truth.assertThat(enum).isEqualTo(SimpleEnumClass.V2)
      }

      AsyncFunction("f2") Coroutine { enum: StringEnumClass ->
        f2WasCalled = true
        Truth.assertThat(enum).isEqualTo(StringEnumClass.K2)
      }

      AsyncFunction("f3") Coroutine { enum: IntEnumClass ->
        f3WasCalled = true
  ...
  fun coded_error_should_be_converted() = withSingleModule({
    AsyncFunction("f").Coroutine<Unit> {
  ...
  fun int_array_should_be_convertible() = withSingleModule({
    AsyncFunction("intArray") Coroutine { a: IntArray -> a }
  }) {
  ...
  fun string_array_should_be_convertible() = withSingleModule({
    AsyncFunction("stringArray") Coroutine { a: Array<String> -> a }
  }) {
  ...
  fun int_array_array_should_be_convertible() = withSingleModule({
    AsyncFunction("array") Coroutine { a: Array<IntArray> -> a }
  }) {
  ...
  fun long_array_should_be_convertible() = withSingleModule({
    AsyncFunction("longArray") Coroutine { a: LongArray -> a }
  }) {
