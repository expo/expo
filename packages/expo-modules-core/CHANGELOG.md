...
- Fixed `useReleasingSharedObject` returning a released object after React's `<Activity>` hides and shows the tree again, which left `useVideoPlayer` and `useAudioPlayer` permanently unusable.
