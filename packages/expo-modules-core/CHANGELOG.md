...
- [Android] Register a fallback `android.graphics.Color` type converter on Android below API 26, where the class-based `Color` API is unavailable, so `Color?` arguments and view props resolve to `null` (degrading to default colors) instead of throwing `MissingTypeConverter` — which made `@expo/ui` unusable on Android 7. ([#47546](https://github.com/expo/expo/issues/47546) by [@anasvemmully](https://github.com/anasvemmully)) ([#47575](https://github.com/expo/expo/pull/47575) by [@intergalacticspacehighway](https://github.com/intergalacticspacehighway))
