...
- On platforms with no native `ExpoModulesCore` runtime (e.g. react-native-windows), fall back to the same pure-JS `globalThis.expo` polyfill `web` already uses, instead of leaving it `undefined`. Individual native modules still degrade via the existing `requireNativeModule`/`requireOptionalNativeModule` null/throw path. ([#47705](https://github.com/expo/expo/pull/47705) by [@FaithfulAudio](https://github.com/FaithfulAudio))
