...
- [Android] Fixed a crash on native tab switch when `@expo/ui` Hosts are mounted. The hosting `ComposeView` took its id from `View.generateViewId()`, which can collide with the ids `react-native-screens` resolves with `findViewById`, so a tab fragment was attached to the `ComposeView`. Hosting view ids now come from a window (`0x01000000` and up) that no other allocator uses. ([#49977](https://github.com/expo/expo/pull/49977) by [@expo-bot](https://github.com/expo-bot))
