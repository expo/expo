...
- [iOS] Gate `ExpoModulesWorkletsAdapter` via a conditional `podspecPath` entry (`autolinkWhen`) in `expo-module.config.json` instead of a `spm.config.json` companion-pod gate, fixing the adapter not linking when `react-native-worklets` is installed only transitively or through an optional peer dependency. ([#46569](https://github.com/expo/expo/pull/46569) by [@chrfalch](https://github.com/chrfalch))
