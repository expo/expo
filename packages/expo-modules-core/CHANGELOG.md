...
- [Android] Fixed a `SIGABRT` crash when a long-lived native `SharedObject` (e.g. a native singleton behind a synchronous getter) is converted back to JavaScript after its previous JS peer was garbage collected. `convertSharedObject` dereferenced the registry lookup without a null check, but the lookup resolves through a weak reference that fails between the end of Hermes marking and the pairing's deletion on the background sweep. The stale pairing is now deleted and the native object is paired with a fresh JS object instead. ([#47787](https://github.com/expo/expo/issues/47787), [#47805](https://github.com/expo/expo/pull/47805) by [@setoy](https://github.com/setoy))
