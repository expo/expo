...
#import <EXUpdates/EXSyncDatabase+Tests.h>
#import <EXUpdates/EXSyncDatabaseInitialization.h>
#import <EXUpdates/EXSyncDatabaseUtils.h>

#import <sqlite3.h>

NS_ASSUME_NONNULL_BEGIN

@interface EXSyncDatabase ()

@property (nonatomic, assign) sqlite3 *db;

@end

static NSString * const EXSyncDatabaseManifestFiltersKey = @"manifestFilters";
static NSString * const EXSyncDatabaseServerDefinedHeadersKey = @"serverDefinedHeaders";

@implementation EXSyncDatabase
...
- (BOOL)openDatabaseInDirectory:(NSURL *)directory withError:(NSError ** _Nullable)error
{
  ...
  sqlite3 *db;
  if (![EXSyncDatabaseInitialization initializeDatabaseWithLatestSchemaInDirectory:directory database:&db error:error]) {
    return NO;
  }
...
- (void)addNewAssets:(NSArray<EXSyncAsset *> *)assets toUpdateWithId:(NSUUID *)updateId error:(NSError ** _Nullable)error
{
  ...
  for (EXSyncAsset *asset in assets) {
...
{
  if (update.status != EXSyncManifestStatusDevelopment) {
    update.status = EXSyncManifestStatusReady;
  }
...
- (nullable NSArray<EXSyncManifest *> *)allUpdatesWithConfig:(EXSyncConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<EXSyncManifest *> *launchableUpdates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<EXSyncManifest *> *)launchableUpdatesWithConfig:(EXSyncConfig *)config error:(NSError ** _Nullable)error
{
  ...
  FROM updates\
  WHERE scope_key = ?1\
  AND status IN (%li, %li, %li);", (long)EXSyncManifestStatusReady, (long)EXSyncManifestStatusEmbedded, (long)EXSyncManifestStatusDevelopment];
...
- (nullable NSArray<EXSyncAsset *> *)assetsWithUpdateId:(NSUUID *)updateId error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<EXSyncAsset *> *assets = [NSMutableArray arrayWithCapacity:rows.count];
...
{
  return [self _jsonDataWithKey:EXSyncDatabaseServerDefinedHeadersKey scopeKey:scopeKey error:error];
}

- (nullable NSDictionary *)manifestFiltersWithScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  return [self _jsonDataWithKey:EXSyncDatabaseManifestFiltersKey scopeKey:scopeKey error:error];
}
...
{
  [self _setJsonData:serverDefinedHeaders withKey:EXSyncDatabaseServerDefinedHeadersKey scopeKey:scopeKey isInTransaction:NO error:error];
}

- (void)setManifestFilters:(NSDictionary *)manifestFilters withScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  [self _setJsonData:manifestFilters withKey:EXSyncDatabaseManifestFiltersKey scopeKey:scopeKey isInTransaction:NO error:error];
}
...
- (void)setMetadataWithManifest:(EXSyncManifest *)updateManifest error:(NSError ** _Nullable)error
{
  ...
  if (updateManifest.serverDefinedHeaders) {
    if (![self _setJsonData:updateManifest.serverDefinedHeaders
                   withKey:EXSyncDatabaseServerDefinedHeadersKey
  ...
  if (updateManifest.manifestFilters) {
    if (![self _setJsonData:updateManifest.manifestFilters
                   withKey:EXSyncDatabaseManifestFiltersKey
                  scopeKey:updateManifest.scopeKey
           isInTransaction:YES
                     error:error]) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
...
- (nullable NSArray<NSDictionary *> *)_executeSql:(NSString *)sql withArgs:(nullable NSArray *)args error:(NSError ** _Nullable)error
{
  ...
  dispatch_assert_queue(_databaseQueue);
  return [EXSyncDatabaseUtils executeSql:sql withArgs:args onDatabase:_db error:error];
}

- (NSError *)_errorFromSqlite:(struct sqlite3 *)db
{
  return [EXSyncDatabaseUtils errorFromSqlite:db];
}

- (EXSyncManifest *)_updateWithRow:(NSDictionary *)row config:(EXSyncConfig *)config
{
  NSError *error;
  id metadata = nil;
  ...
  EXSyncManifest *update = [EXSyncManifest updateWithId:row[@"id"]
                                                 scopeKey:row[@"scope_key"]
                                               commitTime:[NSDate dateWithTimeIntervalSince1970:[(NSNumber *)row[@"commit_time"] doubleValue] / 1000]
                                           runtimeVersion:row[@"runtime_version"]
                                                 metadata:metadata
                                                   status:(EXSyncManifestStatus)[(NSNumber *)row[@"status"] integerValue]
...
- (EXSyncAsset *)_assetWithRow:(NSDictionary *)row
{
  NSError *error;
  id metadata = nil;
  id rowMetadata = row[@"metadata"];
  ...
  EXSyncAsset *asset = [[EXSyncAsset alloc] initWithKey:key type:row[@"type"]];
  asset.url = url;
