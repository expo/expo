...
#import <EXUpdates/EXSyncDatabaseInitialization+Tests.h>
...
static NSString * const EXSyncDatabaseInitializationErrorDomain = @"EXSyncDatabaseInitialization";
static NSString * const EXSyncDatabaseLatestFilename = @"expo-v5.db";

static NSString * const EXSyncDatabaseInitializationLatestSchema = @"\
...
@implementation EXSyncDatabaseInitialization
...
{
  return [[self class] initializeDatabaseWithSchema:EXSyncDatabaseInitializationLatestSchema
                                           filename:EXSyncDatabaseLatestFilename
                               ...
                               error:(NSError ** _Nullable)error
{
  ...
  if (!success) {
    ...
    if ([NSFileManager.defaultManager fileExistsAtPath:dbUrl.path] &&
      ...
      if (error != nil) {
        ...
        *error = [NSError errorWithDomain:EXSyncDatabaseInitializationErrorDomain
                                     code:1022
  ...
  if (resultCode != SQLITE_OK) {
    NSLog(@"Error opening SQLite db: %@", [EXSyncDatabaseUtils errorFromSqlite:db].localizedDescription);
    sqlite3_close(db);
    ...
    if (resultCode == SQLITE_CORRUPT || resultCode == SQLITE_NOTADB) {
      ...
      if ([[NSFileManager defaultManager] moveItemAtURL:dbUrl toURL:destinationUrl error:&err]) {
        ...
        if (sqlite3_open([[dbUrl absoluteString] UTF8String], &db) != SQLITE_OK) {
          if (error != nil) {
            *error = [EXSyncDatabaseUtils errorFromSqlite:db];
          }
          return NO;
        }
        shouldInitializeDatabaseSchema = YES;
  ...
  if (![EXSyncDatabaseUtils executeSql:@"PRAGMA foreign_keys=ON;" withArgs:nil onDatabase:db error:&pragmaForeignKeysError]) {
...
{
  NSURL *latestURL = [directory URLByAppendingPathComponent:EXSyncDatabaseLatestFilename];
  ...
  NSArray<id<EXSyncDatabaseMigration>> *migrations = [EXSyncDatabaseMigrationRegistry migrations];
  __block NSURL *existingURL;
  __block NSUInteger startingMigrationIndex;
  [migrations enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id<EXSyncDatabaseMigration> migration, NSUInteger idx, BOOL *stop) {
  ...
  if (existingURL) {
    ...
    if (sqlite3_open(latestURL.absoluteString.UTF8String, &db) != SQLITE_OK) {
      NSLog(@"Error opening migrated SQLite db: %@", [EXSyncDatabaseUtils errorFromSqlite:db].localizedDescription);
      sqlite3_close(db);
      return NO;
    }
    ...
    for (NSUInteger i = startingMigrationIndex; i < migrations.count; i++) {
      ...
      id<EXSyncDatabaseMigration> migration = migrations[i];
      if (![migration runMigrationOnDatabase:db error:&migrationError]) {
        NSLog(@"Error migrating SQLite db: %@", [EXSyncDatabaseUtils errorFromSqlite:db].localizedDescription);
        sqlite3_close(db);
        return NO;
      }
    }
