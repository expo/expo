    ...
    if let expectedSize = asset.expectedSize {
      do {
        ...
        if let actualSize = attributes[.size] as? NSNumber {
          if actualSize.int64Value != expectedSize.int64Value {
            logger.warn(
              message: "Asset \(asset.key ?? asset.filename) size mismatch: expected=\(expectedSize.int64Value) actual=\(actualSize.int64Value). File is corrupted.",
              code: UpdatesErrorCode.assetsFailedToLoad
            )
            return false
          }
        ...
        logger.warn(
          message: "Failed to get file size for asset \(asset.key ?? asset.filename): \(error.localizedDescription)",
          code: UpdatesErrorCode.assetsFailedToLoad
        )
        return false
      }
      ...
      do {
        ...
        if let contentHash = asset.contentHash, actualHash != contentHash {
          logger.warn(
            message: "Asset \(asset.key ?? asset.filename) hash mismatch. File is corrupted.",
            code: UpdatesErrorCode.assetsFailedToLoad
          )
          return false
        }
        ...
        logger.warn(
          message: "Failed to validate asset \(asset.key ?? asset.filename): \(error.localizedDescription)",
          code: UpdatesErrorCode.assetsFailedToLoad
        )
        return false
      }
