...
static NSString * const EXSyncExpoAssetBaseUrl = @"https://d1wp6m56sqw74a.cloudfront.net/~assets/";
static NSString * const EXSyncExpoIoDomain = @"expo.io";
static NSString * const EXSyncExpHostDomain = @"exp.host";
static NSString * const EXSyncExpoTestDomain = @"expo.test";

@implementation EXSyncLegacyManifest
                                     ...
                                     database:(EXSyncDatabase *)database
{
  ...
  if ([[self class] isDevelopmentModeManifest:manifest]) {
    ...
    update.status = EXSyncManifestStatusDevelopment;
  } else {
...
+ (NSURL *)bundledAssetBaseUrlWithManifest:(NSDictionary *)manifest config:(EXSyncConfig *)config
{
  ...
  if (!host ||
      [host containsString:EXSyncExpoIoDomain] ||
      [host containsString:EXSyncExpHostDomain] ||
      [host containsString:EXSyncExpoTestDomain]) {
    return [NSURL URLWithString:EXSyncExpoAssetBaseUrl];
  } else {
