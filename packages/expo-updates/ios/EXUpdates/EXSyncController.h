...
#import <EXUpdates/EXSyncLoaderTask.h>
...
@class EXSyncController;

@protocol EXSyncControllerDelegate <NSObject>

- (void)appController:(EXSyncController *)appController didStartWithSuccess:(BOOL)success;

@end

@interface EXSyncController : NSObject <EXSyncLoaderTaskDelegate>

/**
 Delegate which will be notified when EXUpdates has an update ready to launch and
 `launchAssetUrl` is nonnull.
 */
@property (nonatomic, weak) id<EXSyncControllerDelegate> delegate;
...
/**
 ...
 Will be null until the EXSyncController delegate method is called.
...
/**
 ...
 shared instance of EXSyncController.
 */
