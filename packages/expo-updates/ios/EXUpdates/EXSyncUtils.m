...
static NSString * const EXSyncEventName = @"Expo.nativeUpdatesEvent";
static NSString * const EXSyncUtilsErrorDomain = @"EXSyncUtils";

@implementation EXSyncUtils
...
+ (nullable NSURL *)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  ...
  if (exists) {
    if (!isDir) {
      *error = [NSError errorWithDomain:EXSyncUtilsErrorDomain code:1005 userInfo:@{NSLocalizedDescriptionKey: @"Failed to create the Updates Directory; a file already exists with the required directory name"}];
      return nil;
    }
  } else {
    NSError *err;
...
+ (void)sendEventToBridge:(nullable RCTBridge *)bridge withType:(NSString *)eventType body:(NSDictionary *)body
{
  if (bridge) {
    ...
    [bridge enqueueJSCall:@"RCTDeviceEventEmitter.emit" args:@[EXSyncEventName, mutableBody]];
  } else {
...
+ (BOOL)shouldCheckForUpdateWithConfig:(EXSyncConfig *)config
{
  switch (config.checkOnLaunch) {
    case EXSyncCheckAutomaticallyConfigNever:
      return NO;
    case EXSyncCheckAutomaticallyConfigWifiOnly: {
    ...
    case EXSyncCheckAutomaticallyConfigAlways:
    default:
      return YES;
  }
}
