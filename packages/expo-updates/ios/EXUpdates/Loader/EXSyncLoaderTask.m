...
static NSString * const EXSyncLoaderTaskErrorDomain = @"EXSyncLoaderTask";

@interface EXSyncLoaderTask ()

@property (nonatomic, strong) EXSyncConfig *config;
@property (nonatomic, strong) EXSyncDatabase *database;
@property (nonatomic, strong) NSURL *directory;
@property (nonatomic, strong) id<EXSyncSelectionPolicy> selectionPolicy;
@property (nonatomic, strong) dispatch_queue_t delegateQueue;

@property (nonatomic, strong) id<EXSyncLauncher> candidateLauncher;
@property (nonatomic, strong) id<EXSyncLauncher> finalizedLauncher;
@property (nonatomic, strong) EXSyncEmbeddedLoader *embeddedAppLoader;
@property (nonatomic, strong) EXSyncRemoteLoader *remoteAppLoader;
...
@implementation EXSyncLoaderTask
...
- (void)start
{
  if (!_config.isEnabled) {
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:EXSyncLoaderTaskErrorDomain code:1030 userInfo:@{
                    NSLocalizedDescriptionKey: @"EXSyncLoaderTask was passed a configuration object with updates disabled. You should load updates from an embedded source rather than calling EXSyncLoaderTask, or enable updates in the configuration."
                  }]];
    });
    return;
  }
  ...
  if (!_config.updateUrl) {
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:EXSyncLoaderTaskErrorDomain code:1030 userInfo:@{
                    NSLocalizedDescriptionKey: @"EXSyncLoaderTask was passed a configuration object with a null URL. You must pass a nonnull URL in order to use EXSyncLoaderTask to load updates."
                  }]];
    });
    return;
  }
  ...
  if (!_directory) {
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:EXSyncLoaderTaskErrorDomain code:1030 userInfo:@{
                    NSLocalizedDescriptionKey: @"EXSyncLoaderTask directory must be nonnull."
                  }]];
    });
    return;
  }

  __block BOOL shouldCheckForUpdate = [EXSyncUtils shouldCheckForUpdateWithConfig:_config];
  ...
  [self _loadEmbeddedUpdateWithCompletion:^{
    [self _launchWithCompletion:^(NSError * _Nullable error, BOOL success) {
      ...
      if (shouldCheckForUpdate) {
        [self _loadRemoteUpdateWithCompletion:^(NSError * _Nullable error, EXSyncManifest * _Nullable update) {
...
- (void)_finishWithError:(nullable NSError *)error
{
  ...
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
      if (self->_isReadyToLaunch && (self->_finalizedLauncher.launchAssetUrl || self->_finalizedLauncher.launchedUpdate.status == EXSyncManifestStatusDevelopment)) {
        [self->_delegate appLoaderTask:self didFinishWithLauncher:self->_finalizedLauncher isUpToDate:self->_isUpToDate];
      } else {
        [self->_delegate appLoaderTask:self didFinishWithError:error ?: [NSError errorWithDomain:EXSyncLoaderTaskErrorDomain code:1031 userInfo:@{
          NSLocalizedDescriptionKey: @"EXSyncLoaderTask encountered an unexpected error and could not launch an update."
        }]];
      }
    });
  }
...
{
  [EXSyncLauncherWithDatabase launchableUpdateWithConfig:_config database:_database selectionPolicy:_selectionPolicy completion:^(NSError * _Nullable error, EXSyncManifest * _Nullable launchableUpdate) {
    dispatch_async(self->_database.databaseQueue, ^{
      ...
      dispatch_async(self->_loaderTaskQueue, ^{
        ...
        if (self->_config.hasEmbeddedUpdate &&
            [self->_selectionPolicy shouldLoadNewUpdate:[EXSyncEmbeddedLoader embeddedManifestWithConfig:self->_config database:self->_database]
                                     withLaunchedUpdate:launchableUpdate
                                                filters:manifestFilters]) {
          self->_embeddedAppLoader = [[EXSyncEmbeddedLoader alloc] initWithConfig:self->_config database:self->_database directory:self->_directory completionQueue:self->_loaderTaskQueue];
          [self->_embeddedAppLoader loadUpdateFromEmbeddedManifestWithCallback:^BOOL(EXSyncManifest * _Nonnull update) {
...
{
  EXSyncLauncherWithDatabase *launcher = [[EXSyncLauncherWithDatabase alloc] initWithConfig:_config database:_database directory:_directory completionQueue:_loaderTaskQueue];
  _candidateLauncher = launcher;
  [launcher launchUpdateWithSelectionPolicy:_selectionPolicy completion:completion];
}

- (void)_loadRemoteUpdateWithCompletion:(void (^)(NSError * _Nullable error, EXSyncManifest * _Nullable update))completion
{
  _remoteAppLoader = [[EXSyncRemoteLoader alloc] initWithConfig:_config database:_database directory:_directory completionQueue:_loaderTaskQueue];
  [_remoteAppLoader loadUpdateFromUrl:_config.updateUrl onManifest:^BOOL(EXSyncManifest * _Nonnull update) {
...
- (void)_handleRemoteUpdateLoaded:(nullable EXSyncManifest *)update error:(nullable NSError *)error
{
  ...
  dispatch_async(_loaderTaskQueue, ^{
    ...
    if (update) {
      if (!self->_hasLaunched) {
        EXSyncLauncherWithDatabase *newLauncher = [[EXSyncLauncherWithDatabase alloc] initWithConfig:self->_config database:self->_database directory:self->_directory completionQueue:self->_loaderTaskQueue];
        ...
        [self _didFinishBackgroundUpdateWithStatus:EXSyncBackgroundUpdateStatusManifestAvailable manifest:update error:nil];
        [self _runReaper];
      }
    } else {
      ...
      [self _finishWithError:error];
      if (error) {
        [self _didFinishBackgroundUpdateWithStatus:EXSyncBackgroundManifestStatusError manifest:nil error:error];
      } else {
        [self _didFinishBackgroundUpdateWithStatus:EXSyncBackgroundUpdateStatusNoManifestAvailable manifest:nil error:nil];
      }
      [self _runReaper];
    }
  });
...
- (void)_didFinishBackgroundUpdateWithStatus:(EXSyncBackgroundManifestStatus)status manifest:(nullable EXSyncManifest *)manifest error:(nullable NSError *)error
{
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
