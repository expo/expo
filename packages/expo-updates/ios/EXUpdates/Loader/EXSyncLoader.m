...
@property (nonatomic, strong) NSMutableArray<EXSyncAsset *> *assetsToLoad;
@property (nonatomic, strong) NSMutableArray<EXSyncAsset *> *erroredAssets;
@property (nonatomic, strong) NSMutableArray<EXSyncAsset *> *finishedAssets;
@property (nonatomic, strong) NSMutableArray<EXSyncAsset *> *existingAssets;
...
static NSString * const EXSyncLoaderErrorDomain = @"EXSyncLoader";

@implementation EXSyncLoader
...
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call EXSyncLoader#loadUpdate -- use a subclass instead" userInfo:nil];
}
...
- (void)startLoadingFromManifest:(EXSyncManifest *)updateManifest
{
  ...
  if (updateManifest.isDevelopmentMode) {
    dispatch_async(_database.databaseQueue, ^{
      ...
      EXSyncLoaderSuccessBlock successBlock;
  ...
  dispatch_async(_database.databaseQueue, ^{
    ...
    EXSyncManifest *existingUpdate = [self->_database updateWithId:updateManifest.updateId config:self->_config error:&existingUpdateError];
    ...
    if (existingUpdate && ![existingUpdate.scopeKey isEqualToString:updateManifest.scopeKey]) {
      ...
      NSLog(@"EXSyncLoader: Loaded an update with the same ID but a different scopeKey than one we already have on disk. This is a server error. Overwriting the scopeKey and loading the existing update.");
    }

    if (existingUpdate && existingUpdate.status == EXSyncManifestStatusReady) {
      if (self->_successBlock) {
        dispatch_async(self->_completionQueue, ^{
    ...
    if (self->_updateManifest.assets && self->_updateManifest.assets.count > 0) {
      ...
      for (EXSyncAsset *asset in self->_updateManifest.assets) {
        // before downloading, check to see if we already have this asset in the database
        NSError *matchingAssetError;
        EXSyncAsset *matchingDbEntry = [self->_database assetWithKey:asset.key error:&matchingAssetError];
...
- (BOOL)_shouldStartLoadingUpdate:(EXSyncManifest *)updateManifest
{
...
- (void)_finish
{
  dispatch_async(_database.databaseQueue, ^{
    ...
    for (EXSyncAsset *existingAsset in self->_existingAssets) {
      NSError *error;
      ...
      if (!existingAssetFound) {
        ...
        existingAsset.contentHash = [EXSyncUtils sha256WithData:contents];
    ...
    EXSyncLoaderSuccessBlock successBlock;
    EXSyncLoaderErrorBlock errorBlock;
    ...
    dispatch_async(self->_completionQueue, ^{
      if (errorBlock) {
        errorBlock([NSError errorWithDomain:EXSyncLoaderErrorDomain
                                       code:1012
