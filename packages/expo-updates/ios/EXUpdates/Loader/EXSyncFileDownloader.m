...
NSString * const EXSyncFileDownloaderErrorDomain = @"EXSyncFileDownloader";
NSTimeInterval const EXSyncDefaultTimeoutInterval = 60;

@interface EXSyncFileDownloader () <NSURLSessionDataDelegate>
...
@implementation EXSyncFileDownloader
                 ...
                 errorBlock:(EXSyncFileDownloaderErrorBlock)errorBlock
{
  [self downloadDataFromURL:url successBlock:^(NSData *data, NSURLResponse *response) {
    ...
    if ([data writeToFile:destinationPath options:NSDataWritingAtomic error:&error]) {
      ...
      errorBlock([NSError errorWithDomain:EXSyncFileDownloaderErrorDomain
                                     code:1002
                                 userInfo:@{
...
- (NSURLRequest *)createManifestRequestWithURL:(NSURL *)url extraHeaders:(nullable NSDictionary *)extraHeaders
{
  ...
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:cachePolicy timeoutInterval:EXSyncDefaultTimeoutInterval];
                     ...
                     errorBlock:(EXSyncFileDownloaderErrorBlock)errorBlock
{
  ...
  [self _downloadDataWithRequest:request successBlock:^(NSData *data, NSURLResponse *response) {
    ...
    if (signature != nil && !isUnsignedFromXDL) {
      ...
      [EXSyncCrypto verifySignatureWithData:(NSString *)manifestString
                                  ...
                                  successBlock:^(BOOL isValid) {
                                                  if (isValid) {
                                                    ...
                                                    NSError *error = [NSError errorWithDomain:EXSyncFileDownloaderErrorDomain code:1003 userInfo:@{NSLocalizedDescriptionKey: @"Manifest verification failed"}];
                       ...
                       errorBlock:(EXSyncFileDownloaderErrorBlock)errorBlock
{
  ...
  EXSyncManifest *update = [EXSyncManifest updateWithManifest:mutableManifest.copy
                                                       response:response
                                                         config:_config
                                                       database:database];
  if (![EXSyncSelectionPolicyFilterAware doesUpdate:update matchFilters:update.manifestFilters]) {
    NSError *error = [NSError errorWithDomain:EXSyncFileDownloaderErrorDomain
                                         code:1021
...
- (nullable NSDictionary *)_extractUpdateResponseDictionary:(id)parsedJson error:(NSError **)error
{
  ...
  if (error) {
    *error = [NSError errorWithDomain:EXSyncFileDownloaderErrorDomain code:1009 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"No compatible update found at %@. Only %@ are supported.", _config.updateUrl.absoluteString, _config.sdkVersion]}];
  }
  return nil;
}
...
- (void)_setManifestHTTPHeaderFields:(NSMutableURLRequest *)request withExtraHeaders:(nullable NSDictionary *)extraHeaders
{
  ...
  NSString *previousFatalError = [EXSyncLauncherNoDatabase consumeError];
...
- (NSError *)_errorFromResponse:(NSHTTPURLResponse *)response body:(NSString *)body
{
  ...
  return [NSError errorWithDomain:EXSyncFileDownloaderErrorDomain code:response.statusCode userInfo:userInfo];
}
