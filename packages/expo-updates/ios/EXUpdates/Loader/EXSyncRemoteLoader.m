...
#import <EXUpdates/EXSyncCrypto.h>
#import <EXUpdates/EXSyncFileDownloader.h>
#import <UMCore/UMUtilities.h>

NS_ASSUME_NONNULL_BEGIN

@interface EXSyncRemoteLoader ()

@property (nonatomic, strong) EXSyncFileDownloader *downloader;
@property (nonatomic, strong) EXSyncManifest *remoteUpdate;

@property (nonatomic, strong) dispatch_queue_t completionQueue;

@end
static NSString * const EXSyncRemoteLoaderErrorDomain = @"EXSyncRemoteLoader";

@implementation EXSyncRemoteLoader
               ...
               completionQueue:(dispatch_queue_t)completionQueue
{
  if (self = [super initWithConfig:config database:database directory:directory completionQueue:completionQueue]) {
    _downloader = [[EXSyncFileDownloader alloc] initWithUpdatesConfig:self.config];
                    ...
                    error:(EXSyncLoaderErrorBlock)error
{
  ...
  UM_WEAKIFY(self)
  self.successBlock = ^(EXSyncManifest * _Nullable update) {
    UM_STRONGIFY(self)
  ...
  dispatch_async(self.database.databaseQueue, ^{
    ...
    [self->_downloader downloadManifestFromURL:url withDatabase:self.database extraHeaders:extraHeaders successBlock:^(EXSyncManifest *update) {
...
- (void)downloadAsset:(EXSyncAsset *)asset
{
  ...
  dispatch_async([EXSyncFileDownloader assetFilesQueue], ^{
    if ([[NSFileManager defaultManager] fileExistsAtPath:[urlOnDisk path]]) {
      ...
      if (!asset.url) {
        [self handleAssetDownloadWithError:[NSError errorWithDomain:EXSyncRemoteLoaderErrorDomain code:1006 userInfo:@{NSLocalizedDescriptionKey: @"Failed to download asset with no URL provided"}] asset:asset];
        return;
      }
