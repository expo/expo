...
static NSString * const EXSyncCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const EXSyncCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const EXSyncCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation EXSyncCrypto
...
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"EXSyncCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }
  ...
  EXSyncFileDownloader *fileDownloader = [[EXSyncFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
  [fileDownloader downloadDataFromURL:[NSURL URLWithString:EXSyncCryptoPublicKeyUrl]
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(EXSyncVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[EXSyncCryptoPublicKeyTag UTF8String] length:[EXSyncCryptoPublicKeyTag length]];
