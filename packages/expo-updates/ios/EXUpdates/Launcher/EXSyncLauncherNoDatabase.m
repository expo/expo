...
static NSString * const EXSyncErrorLogFile = @"expo-error.log";

@interface EXSyncLauncherNoDatabase ()
...
@implementation EXSyncLauncherNoDatabase

- (void)launchUpdateWithConfig:(EXSyncConfig *)config
{
  _launchedUpdate = [EXSyncEmbeddedLoader embeddedManifestWithConfig:config database:nil];
  if (_launchedUpdate) {
    if (_launchedUpdate.status == EXSyncManifestStatusEmbedded) {
      ...
      _launchAssetUrl = [[NSBundle mainBundle] URLForResource:EXSyncEmbeddedBundleFilename withExtension:EXSyncEmbeddedBundleFileType];
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:EXSyncErrorLogFile] path];
}
