...
@objc
public enum EXUpdatesRemoteLoadStatus: Int {
  case Idle
  case Loading
...
public final class EXUpdatesErrorRecovery: NSObject {
  ...
  public weak var delegate: EXUpdatesErrorRecoveryDelegate?
  ...
  public convenience override init() {
    self.init(
      ...
      diskWriteQueue: nil,
      remoteLoadTimeout: EXUpdatesErrorRecovery.RemoteLoadTimeoutMs
    )
  }
  ...
  public func notify(newRemoteLoadStatus newStatus: EXUpdatesRemoteLoadStatus) {
  ...
  public func writeErrorOrExceptionToLog(_ errorOrException: Any) {
    ...
    queue.async {
      ...
      if let errorOrException = errorOrException as? NSError {
        serializedError = "Fatal error: \(EXUpdatesErrorRecovery.serialize(error: errorOrException))"
      } else if let errorOrException = errorOrException as? NSException {
        serializedError = "Fatal exception: \(EXUpdatesErrorRecovery.serialize(exception: errorOrException))"
      } else {
        return
      }
      ...
      let errorLogFile = EXUpdatesErrorRecovery.errorLogFile()
