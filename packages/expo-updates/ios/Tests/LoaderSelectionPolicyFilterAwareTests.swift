...
@Suite("LoaderSelectionPolicyFilterAware update overrides")
struct LoaderSelectionPolicyFilterAwareTests {
  ...
  @Test
  func `should load new update when it matches override URL and headers`() {
  ...
  @Test
  func `should not load same update even it matches override URL and headers`() {
    let loaderPolicy = LoaderSelectionPolicyFilterAware(config: configWithOverride)
  ...
  @Test
  func `should not load new update when URL doesn't match override`() {
    let loaderPolicy = LoaderSelectionPolicyFilterAware(config: configWithOverride)
  ...
  @Test
  func `should not load new update when headers don't match override`() {
    let loaderPolicy = LoaderSelectionPolicyFilterAware(config: configWithOverride)
  ...
  @Test
  func `should load new update when launched update doesn't match filters`() {
    let loaderPolicy = LoaderSelectionPolicyFilterAware(config: configWithOverride)
    let result = loaderPolicy.shouldLoadNewUpdate(updateWithOverrideUrl, withLaunchedUpdate: updateDefault1, filters: manifestFilters)
    #expect(result == true)
  }
  ...
  @Test
  func `should not load new update that doesn't match filters`() {
