...
class EXUpdatesLoggerTests: XCTestCase {
  func test_BasicLoggingWorks() {
    ...
    XCTAssertTrue(UInt((logEntry?.timestamp)!/1000) == UInt(epoch.timeIntervalSince1970))
    ...
    XCTAssertTrue(UInt((logEntry2?.timestamp)!/1000) == UInt(epoch.timeIntervalSince1970))
