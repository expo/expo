...
// Swift Testing gives each test its own instance and runs them in parallel, so `init` and
// `deinit` would delete the database directory from under a sibling test. `.serialized` keeps
// them in sequence, and the directory name is unique to this suite so it can't collide with
// `UpdatesDatabaseTests` or `FileDownloaderTests`.
@Suite("UpdatesDatabaseIntegrityCheck", .serialized)
final class UpdatesDatabaseIntegrityCheckTests {
  ...
  @Test
  func `filter embedded updates works`() {
