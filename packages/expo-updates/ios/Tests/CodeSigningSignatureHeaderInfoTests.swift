...
class CodeSigningSignatureHeaderInfoTests : XCTestCase {
  func test_parseSignatureHeader_ParsesCodeSigningInfo() throws {
    let codeSigningInfo = try CodeSigningSignatureHeaderInfo.parseSignatureHeader(signatureHeader: "sig=\"12345\", keyid=\"test\", alg=\"rsa-v1_5-sha256\"")
    ...
    func test_parseSignatureHeader_DefaultsKeyIdAndAlg() throws {
      let codeSigningInfo = try CodeSigningSignatureHeaderInfo.parseSignatureHeader(signatureHeader: "sig=\"12345\"")
      XCTAssertEqual(codeSigningInfo.signature, "12345")
    ...
    func test_parseSignatureHeader_ThrowsForInvalidAlg() {      
      XCTAssertThrowsError(try CodeSigningSignatureHeaderInfo.parseSignatureHeader(signatureHeader: "fake=\"12345\"")) { error in
        XCTAssertEqual(error as? CodeSigningError, CodeSigningError.SignatureHeaderSigMissing)
      }
      
      XCTAssertThrowsError(try CodeSigningSignatureHeaderInfo.parseSignatureHeader(signatureHeader: "s=1")) { error in
        XCTAssertEqual(error as? CodeSigningError, CodeSigningError.SignatureHeaderStructuredFieldParseError)
      }
      
      XCTAssertThrowsError(try CodeSigningSignatureHeaderInfo.parseSignatureHeader(signatureHeader: "sig=\"12345\", alg=\"blah\"")) { error in
