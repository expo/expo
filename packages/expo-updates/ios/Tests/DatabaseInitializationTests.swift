...
struct DatabaseInitializationTests {
  ...
  init() throws {
    ...
    testDatabaseDir = applicationSupportDir!.appendingPathComponent("DatabaseInitializationTests")
  ...
  struct DatabasePersistenceTests {
    ...
    init() throws {
      let applicationSupportDir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).last
      testDatabaseDir = applicationSupportDir!.appendingPathComponent("DatabasePersistenceTests")
  ...
  struct MigrationsTests {
    ...
    init() throws {
      let applicationSupportDir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).last
      testDatabaseDir = applicationSupportDir!.appendingPathComponent("MigrationsTests")
    ...
    @Test
    func `migrates 4 to latest`() throws {
    ...
    func `migrates 4 to 5`() throws {
      ...
      #expect {
        ...
        guard let dbError = error as? UpdatesDatabaseUtilsError else {
          return false
        }
        return dbError.info?.extendedCode == 787
      }
    ...
    @Test
    func `migrates 10 to 11`() throws {
      let db = try UpdatesDatabaseInitialization.initializeDatabase(
        withSchema: UpdatesDatabaseV10Schema,
        filename: "expo-v10.db",
        inDirectory: testDatabaseDir,
        shouldMigrate: false,
        migrations: [],
        logger: UpdatesLogger()
      ...
      let migratedDb = try UpdatesDatabaseInitialization.initializeDatabaseWithLatestSchema(
        inDirectory: testDatabaseDir,
        migrations: [UpdatesDatabaseMigration10To11()],
        logger: UpdatesLogger()
      )
