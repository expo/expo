...
class NewUpdateManifest private constructor(
  ...
  private val assetHeaders: Map<String, JSONObject> by lazy {
    ...
    if (assetRequestHeadersJSON !== null) {
      assetRequestHeadersJSON.let { it.keys().asSequence().associateWith { key -> it.require(key) } }
    } else {
      mapOf()
    }
  }
  ...
  companion object {
    ...
    fun fromNewManifest(
      ...
      val serverDefinedHeaders = headers["expo-server-defined-headers"]
      val manifestFilters = headers["expo-manifest-filters"]
