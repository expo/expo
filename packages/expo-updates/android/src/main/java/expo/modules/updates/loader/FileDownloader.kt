...
class FileDownloader(
      ...
      response.use { resp ->
        ...
        val hash = if (allowPatch && isPatch) {
          ...
          if (!allowPatch && isPatch) {
            responseBody.use {
              throw IOException("Received patch response even though patch application is disabled")
            }
          }
  ...
  private fun prepareAssetForDiff(
    ...
    if (!asset.isLaunchAsset) {
      failWithClosedBody(responseBody, "Received patch for non-launch asset ${asset.key}")
    }
  ...
  fun createRequestForAsset(
    ...
    val hasAcceptOverride = assetEntity.extraRequestHeaders?.has("Accept") == true ||
      extraHeaders.has("Accept") ||
