...
open class FileDownloader(context: Context) {
  ...
  internal suspend fun parseManifestResponse(response: Response, configuration: UpdatesConfiguration): UpdateManifest {
    ...
    return if (isMultipart) {
      val boundaryParameter = ParameterParser().parse(contentType, ';')["boundary"] ?: throw IOException("Missing boundary in multipart manifest content-type")
      parseMultipartManifestResponse(response, boundaryParameter, configuration)
    } else {
      ...
      parseManifest(response.body()!!.string(), manifestHeaderData, null, configuration)
    }
  }
  ...
  private suspend fun parseManifest(manifestBody: String, manifestHeaderData: ManifestHeaderData, extensions: JSONObject?, configuration: UpdatesConfiguration): UpdateManifest {
    try {
      ...
      if (signature != null && !isUnsignedFromXDL) {
        val isValid = Crypto.verifyPublicRSASignature(
          manifestString,
          signature,
          this@FileDownloader,
        )
        if (isValid) {
          try {
            return createManifest(preManifest, manifestHeaderData, extensions, true, configuration)
          } catch (e: Exception) {
        ...
        return createManifest(preManifest, manifestHeaderData, extensions, false, configuration)
      }
    } catch (e: Exception) {
      throw Error(
