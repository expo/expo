...
class UpdatesDatabaseMigrationTest {
 ...
 (X'594100ea066e4804b5c7c907c773f980',4)"""
  ...
  fun testMigrate13To14() {
    ...
    // db has schema version 13. insert some data using SQL queries.
    // cannot use DAO classes because they expect the latest schema.
    db.execSQL(
      """INSERT INTO "assets" ("id","url","key","headers","type","metadata","download_time","relative_path","hash","hash_type","marked_for_deletion") VALUES (2,'https://url.to/b56cf690e0afa93bd4dc7756d01edd3e','b56cf690e0afa93bd4dc7756d01edd3e.png',NULL,'image/png',NULL,1614137309295,'b56cf690e0afa93bd4dc7756d01edd3e.png',NULL,0,0),
 (3,'https://url.to/bundle-1614137308871','bundle-1614137308871',NULL,'application/javascript',NULL,1614137309513,'bundle-1614137308871',NULL,0,0),
 (4,NULL,NULL,NULL,'js',NULL,1614137406588,'bundle-1614137401950',NULL,0,0)"""
    ...
    // Re-open the database with version 14 and provide
    // MIGRATION_13_14 as the migration process.
    db = helper.runMigrationsAndValidate(TEST_DB, 14, true, UpdatesDatabase.MIGRATION_13_14)
    db.execSQL("PRAGMA foreign_keys=ON")

    // schema changes automatically verified, we just need to verify data integrity
    // Verify all existing assets have expected_size as NULL (since it's a new nullable column)
    val cursorAssets1 =
      db.query("SELECT * FROM `assets` WHERE `id` = 2 AND `url` = 'https://url.to/b56cf690e0afa93bd4dc7756d01edd3e' AND `key` = 'b56cf690e0afa93bd4dc7756d01edd3e.png' AND `headers` IS NULL AND `type` = 'image/png' AND `metadata` IS NULL AND `download_time` = 1614137309295 AND `relative_path` = 'b56cf690e0afa93bd4dc7756d01edd3e.png' AND `hash` IS NULL AND `hash_type` = 0 AND `marked_for_deletion` = 0 AND `expected_size` IS NULL")
    Assert.assertEquals(1, cursorAssets1.count.toLong())
    val cursorAssets2 =
      db.query("SELECT * FROM `assets` WHERE `id` = 3 AND `url` = 'https://url.to/bundle-1614137308871' AND `key` = 'bundle-1614137308871' AND `headers` IS NULL AND `type` = 'application/javascript' AND `metadata` IS NULL AND `download_time` = 1614137309513 AND `relative_path` = 'bundle-1614137308871' AND `hash` IS NULL AND `hash_type` = 0 AND `marked_for_deletion` = 0 AND `expected_size` IS NULL")
    Assert.assertEquals(1, cursorAssets2.count.toLong())
    val cursorAssets3 =
      db.query("SELECT * FROM `assets` WHERE `id` = 4 AND `url` IS NULL AND `key` IS NULL AND `headers` IS NULL AND `type` = 'js' AND `metadata` IS NULL AND `download_time` = 1614137406588 AND `relative_path` = 'bundle-1614137401950' AND `hash` IS NULL AND `hash_type` = 0 AND `marked_for_deletion` = 0 AND `expected_size` IS NULL")
    Assert.assertEquals(1, cursorAssets3.count.toLong())

    // Verify we can insert new assets with expected_size
    db.execSQL(
      """INSERT INTO "assets" ("id","url","key","headers","type","metadata","download_time","relative_path","hash","hash_type","marked_for_deletion","expected_size") VALUES (5,'https://url.to/new-bundle','new-bundle',NULL,'js',NULL,1614137406589,'new-bundle',NULL,0,0,72728472)"""
    )
    val cursorNewAsset =
      db.query("SELECT * FROM `assets` WHERE `id` = 5 AND `expected_size` = 72728472")
    Assert.assertEquals(1, cursorNewAsset.count.toLong())
  }
