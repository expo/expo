...
class UpdatesConfigurationInstrumentationTest {
  ...
  @Test
  fun shouldRejectInvalidRetentionOverridesWithoutCoercion() {
    val invalidValues = listOf(-1, 0, 1, 2.5, Double.NaN, Double.POSITIVE_INFINITY, Long.MAX_VALUE, "3", true)
    for (value in invalidValues) {
      Assert.assertThrows("value: $value", AssertionError::class.java) {
        UpdatesConfiguration(
          null,
          mapOf(
            UpdatesConfiguration.UPDATES_CONFIGURATION_UPDATE_URL_KEY to Uri.parse("https://example.com"),
            UpdatesConfiguration.UPDATES_CONFIGURATION_MAX_UPDATES_TO_KEEP_KEY to value
          )
        )
      }
    }
  ...
  @Test
  fun shouldRejectMalformedRetentionMetadata() {
    val metadataKey = "expo.modules.updates.EXPO_UPDATES_MAX_UPDATES_TO_KEEP"
    val invalidMetadata = listOf(
      Bundle().apply { putString(metadataKey, "3") },
      Bundle().apply { putBoolean(metadataKey, true) },
      Bundle().apply { putFloat(metadataKey, 2.5f) },
      Bundle().apply { putLong(metadataKey, Long.MAX_VALUE) },
      Bundle().apply { putInt(metadataKey, 1) }
    )
    for (metadata in invalidMetadata) {
      metadata.putString("expo.modules.updates.EXPO_UPDATE_URL", "https://example.com")
      val context = mockk<Context> {
        every { packageName } returns "test"
        every { packageManager } returns mockk {
          every { getApplicationInfo("test", PackageManager.GET_META_DATA) } returns mockk {
            metaData = metadata
          }
        }
      }
      Assert.assertThrows(AssertionError::class.java) { UpdatesConfiguration(context, null) }
    }
  }

  @Test
  fun shouldAcceptRetentionIntegerBoundaries() {
    for (value in listOf(2, 3, Int.MAX_VALUE)) {
      ...
      Assert.assertEquals(value, config.maxUpdatesToKeep)
    }
  }
      ...
      every { packageManager } returns mockk {
        every { getApplicationInfo(testPackageName, PackageManager.GET_META_DATA) } returns mockk {
          metaData = Bundle().apply {
            ...
            putInt("expo.modules.updates.EXPO_UPDATES_MAX_UPDATES_TO_KEEP", 9)
