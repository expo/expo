...
class ManifestMetadataTest {
    ...
    fun setupManifest() {
        val manifestString = "{\"runtimeVersion\":\"1\",\"id\":\"0eef8214-4833-4089-9dff-b4138a14f196\",\"createdAt\":\"2020-11-11T00:17:54.797Z\",\"launchAsset\":{\"url\":\"https://url.to/bundle.js\",\"contentType\":\"application/javascript\"}}"
        manifestJson = JSONObject(manifestString)
        config = createConfig()
    }
    ...
    fun testManifestFilters_OverwriteAllFields() {
        ...
        Mockito.`when`(response1.header("expo-manifest-filters")).thenReturn("branch-name=\"rollout-1\",test=\"value\"")
        val manifest1: Manifest = NewManifest.fromManifestJson(manifestJson!!, response1, config!!)
        ManifestMetadata.saveMetadata(manifest1, db, config)
        val response2 = Mockito.mock(ManifestResponse::class.java)
        Mockito.`when`(response2.header("expo-manifest-filters")).thenReturn("branch-name=\"rollout-2\"")
        val manifest2: Manifest = NewManifest.fromManifestJson(manifestJson!!, response2, config!!)
    ...
    fun testManifestFilters_OverwriteEmpty() {
        val response1 = Mockito.mock(ManifestResponse::class.java)
        Mockito.`when`(response1.header("expo-manifest-filters")).thenReturn("branch-name=\"rollout-1\"")
        val manifest1: Manifest = NewManifest.fromManifestJson(manifestJson!!, response1, config!!)
        ManifestMetadata.saveMetadata(manifest1, db, config)
        val response2 = Mockito.mock(ManifestResponse::class.java)
