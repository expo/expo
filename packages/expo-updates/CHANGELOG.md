...
- [iOS] Fix an `EXC_BREAKPOINT` at launch when a native consumer reads `launchedUpdateId`, `launchAssetPath` or `launchAssetUrl()` after `initializeWithoutStarting()` publishes the controller but before `start()` assigns `startupProcedure` — these now return `nil` instead of trapping on the implicitly-unwrapped optional. ([#48898](https://github.com/expo/expo/pull/48898) by [@spsaucier](https://github.com/spsaucier))
- [iOS] Fix an `EXC_BAD_ACCESS` in `UpdatesStateMachine.processEvent` caused by an unsynchronized `stateChangeListeners` dictionary being mutated by native subscribers while the state machine iterated it — access is now locked and listeners are notified off a snapshot. ([#48898](https://github.com/expo/expo/pull/48898) by [@spsaucier](https://github.com/spsaucier))
