...
- Add `EXPO_UPDATES_COMMIT_TIME_SOURCE=git` and `EXPO_UPDATES_COMMIT_TIME_OVERRIDE`, letting a build date its embedded bundle by the source commit rather than by when the build ran, so a build that outlasts the gap between two commits no longer outranks an update published from the newer one. Opt-in; the default is unchanged. ([#49582](https://github.com/expo/expo/pull/49582) by [@marcodejongh](https://github.com/marcodejongh))
