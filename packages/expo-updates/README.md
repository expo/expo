...
If present, enforce code and asset signing. Each downloaded item should have a `expo-signed-hash` response header. For each downloaded item compute its sha256 hash and verify that the signature included in the header is valid for the computed hash given the certificate stored in the configuration value.
