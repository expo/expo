...
beforeEach(() => {
  ...
  delete process.env.EXPO_UPDATES_COMMIT_TIME_SOURCE;
  cwd = process.cwd();
...
// running; dating it to its commit lets an update published later from an OLDER
// commit outrank the binary. Both are real, so the git source is opt-in and the
// default stays the build time.
describe('embedded commitTime', () => {
  const committedAt = new Date('2026-01-02T03:04:05.000Z');

  it('uses the build time by default, even inside a clean checkout', async () => {
  ...
  it('uses the build time for an unrecognised source, and says so', async () => {
    const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
    process.env.EXPO_UPDATES_COMMIT_TIME_SOURCE = 'commit';
    const before = Date.now();
    await commitProjectAsync(committedAt);
    const manifest = await createManifestAsync('ios', destinationDir);
    expect(manifest.commitTime).toBeGreaterThanOrEqual(before);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('EXPO_UPDATES_COMMIT_TIME_SOURCE'));
    warn.mockRestore();
  });

  describe('with EXPO_UPDATES_COMMIT_TIME_SOURCE=git', () => {
    beforeEach(() => {
      process.env.EXPO_UPDATES_COMMIT_TIME_SOURCE = 'git';
    });
    ...
    it('falls back to the build time when the working tree is dirty', async () => {
      ...
      const manifest = await createManifestAsync('ios', destinationDir);
      expect(manifest.commitTime).toBeGreaterThanOrEqual(before);
      expect(warn).toHaveBeenCalledWith(expect.stringContaining('dirty'));
      warn.mockRestore();
    });
    ...
    // ordered by their commits, which is the property this source exists to give.
    it('orders two builds by commit rather than by build order', async () => {
      ...
      const earlierCommitBuiltSecond = (await createManifestAsync('ios', destinationDir))
        .commitTime;
  ...
  it('prefers EXPO_UPDATES_COMMIT_TIME_OVERRIDE over every other source', async () => {
    process.env.EXPO_UPDATES_COMMIT_TIME_SOURCE = 'git';
