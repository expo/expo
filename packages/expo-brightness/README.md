...
`expo-brightness` module

## Installation

Install the package from `npm` registry:

`yarn add expo-brightness` or `npm install expo-brightness`
...
2.  Insert the following lines inside the dependencies block in `android/app/build.gradle`:
    ```gradle
    compile project(':expo-brightness')
    ```
3.  Add `new ExpoBrightnessPackage()` to your module registry provider in `MainApplication.java`.
...
export default class App extends Component {
  ...
  doGreatJob = async () => {
    await ExpoBrightness.someGreatMethod();
  }

  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.paragraph}>'expo-brightness' example</Text>
      </View>
    );
  }
}
...
### `ExpoBrightness.someGreatMethod(options)`
