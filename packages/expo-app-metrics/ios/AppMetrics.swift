...
public struct AppMetrics {
  ...
  /// Deletes span rows with `id <= upToId`. Spans are readable per session (`getSpans(forSessionId:)`),
  /// so callers should delete only rows they know no reader still needs.
  @AppMetricsActor
