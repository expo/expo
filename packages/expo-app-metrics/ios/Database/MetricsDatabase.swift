...
final class MetricsDatabase: Sendable {
  ...
  /// the session-association read: spans are retained after dispatch so a session's trace
  /// activity can be inspected alongside its metrics and logs.
  @AppMetricsActor
  ...
  /// dropped) a batch, and by the insert-time row cap. Spans are also readable per session
  /// (`getSpans(forSessionId:)`), so callers delete only rows no reader still needs.
  @AppMetricsActor
