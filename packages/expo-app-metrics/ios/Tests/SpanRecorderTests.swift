...
struct SpanRecorderTests {
  ...
  @Test
  func `stores the status message with an error status`() throws {
    let recorder = makeRecorder()
    let row = try #require(
  ...
  @Test
  func `drops a status message accompanying an OK status`() throws {
    // The Trace API allows a description only with the Error status code.
    let recorder = makeRecorder()
    let row = try #require(recorder.end(statusCode: SpanRow.statusOk, statusMessage: "all good", endTimestampMs: endMs))
    #expect(row.statusCode == SpanRow.statusOk)
    #expect(row.statusMessage == nil)
  }
