...
/// database until then, because the `spans` table stores only completed spans.
///
...
public final class SpanRecorder: Sendable {
  ...
  init(
    ...
    self.state = Mutex(State(attributes: sanitizeLogEventAttributes(attributes, source: "startSpan").attributes ?? [:]))
  }
  ...
  func setAttributes(_ attributes: [String: Any]) {
    guard let sanitized = sanitizeLogEventAttributes(attributes, source: "setAttributes").attributes else {
      return
    }
  ...
  /// anyway), as is anything past `maxEventCount`. No-op after `end`.
  func addEvent(name: String, attributes: [String: Any]?, timeMs: Int64) {
    ...
    guard !trimmedName.hasPrefix(reservedEventNamePrefix) else {
    ...
    guard trimmedName.count <= maxEventNameLength else {
      logger.warn(
        "[AppMetrics] Dropping a span event whose name is \(trimmedName.count) characters long, past the \(maxEventNameLength)-character limit"
      )
      return
    }
    let sanitized = sanitizeLogEventAttributes(attributes, source: "addEvent").attributes
    state.withLock { state in
      if state.ended {
        return
      }
  ...
  /// returns `nil`. `statusMessage` is only stored alongside an ERROR status code: the Trace
  /// API allows a description with that status only, so a message with UNSET or OK is dropped.
  func end(statusCode: Int?, statusMessage: String?, endTimestampMs: Int64) -> SpanRow? {
    ...
    return SpanRow(
      ...
      statusMessage: statusCode == SpanRow.statusError ? validateEventBody(statusMessage) : nil,
