...
interface SpanDao {
  ...
  /** Spans with `id > cursor`, in ascending rowid order. Mirrors the iOS cursor read. */
  @Query("SELECT * FROM spans WHERE id > :cursor ORDER BY id ASC")
  suspend fun getSpans(cursor: Long): List<Span>

  /**
   * The spans attributed to one session, in ascending rowid (insertion) order. This is the
   * session-association read: spans stay readable per session alongside its metrics and logs.
   */
   ...
   * a batch, and by the insert-time row cap. Spans are also readable per session
   * (`getSpansForSession`), so callers delete only rows no reader still needs.
   */
