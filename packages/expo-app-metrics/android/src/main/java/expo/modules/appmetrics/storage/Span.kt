...
interface SpanDao {
  ...
  /**
   * At most `limit` spans with `id > cursor`, in ascending rowid order. Mirrors the iOS
   * `getSpans(afterId:limit:)` read so consumers can drain the table in chunks.
   */
  @Query("SELECT * FROM spans WHERE id > :cursor ORDER BY id ASC LIMIT :limit")
  suspend fun getSpans(cursor: Long, limit: Int): List<Span>
