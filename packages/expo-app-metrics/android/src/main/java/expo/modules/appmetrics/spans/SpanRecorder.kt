 ...
 * database until then, because the `spans` table stores only completed spans. Mirrors the iOS
 * `SpanRecorder`.
 *
...
class SpanRecorder(
  ...
  init {
    ...
    sanitizeLogEventAttributes(attributes, source = "startSpan").attributes?.let {
  ...
  fun setAttributes(attributes: Map<String, Any?>) {
    val sanitized = sanitizeLogEventAttributes(attributes, source = "setAttributes").attributes ?: return
    synchronized(lock) {
      if (ended) {
        return
      }
  ...
  /**
   * Appends a point-in-time event. Events whose name trims to empty, uses the reserved `expo.`
   * prefix, or runs past the name limit are dropped with a warning (the server drops them
   * anyway), as is anything past `MAX_EVENT_COUNT`. No-op after `end`.
   */
  fun addEvent(name: String, attributes: Map<String, Any?>?, timeMs: Long) {
    val trimmedName = name.trim()
    if (trimmedName.isEmpty()) {
      Log.w(TAG, "[AppMetrics] Dropping a span event without a name")
      return
    }
    if (trimmedName.startsWith(RESERVED_EVENT_NAME_PREFIX)) {
      // The SDK's own network producer writes `expo.http.redirect`, and the backend reads that
      // name rather than just displaying it, so a caller can't be allowed to mint one.
      Log.w(
        TAG,
        "[AppMetrics] Dropping span event `$trimmedName`: the `expo.` prefix is reserved for SDK-recorded events"
      )
      return
    }
    if (trimmedName.length > MAX_EVENT_NAME_LENGTH) {
      Log.w(
        TAG,
        "[AppMetrics] Dropping a span event whose name is ${trimmedName.length} characters long, past the $MAX_EVENT_NAME_LENGTH-character limit"
      )
      return
    }
    val sanitized = sanitizeLogEventAttributes(attributes, source = "addEvent").attributes
    synchronized(lock) {
      if (ended) {
        return
      }
      if (events.size >= MAX_EVENT_COUNT) {
        Log.w(TAG, "[AppMetrics] Dropping a span event past the $MAX_EVENT_COUNT-event limit")
        return
      }
   ...
   * `statusMessage` is only stored alongside an ERROR status code: the Trace API allows a
   * description with that status only, so a message with UNSET or OK is dropped.
   */
  fun end(statusCode: Int?, statusMessage: String?, endTimestampMs: Long): Span? {
    ...
    return Span(
      ...
      statusMessage = if (statusCode == Span.STATUS_ERROR) validateEventBody(statusMessage) else null,
