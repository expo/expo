...
class SpanRecorder(
  ...
  /**
   * Appends a point-in-time event. Events whose name trims to empty are dropped with a warning
   * (the server drops them anyway), as is anything past `MAX_EVENT_COUNT`. No-op after `end`.
   */
  ...
  fun end(statusCode: Int?, statusMessage: String?, endTimestampMs: Long): Span? {
    ...
    return Span(
      ...
      statusMessage = if (statusCode == Span.STATUS_ERROR) statusMessage else null,
