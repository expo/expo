...
class MetricsDatabaseMigrationTest {
  @Test
  fun `migration 17 to 18 creates the spans table and preserves existing data`() = runTest {
    ...
    // schema version back to 17. Reopening then runs MIGRATION_17_18, and Room validates the
    // migrated table against the entity at open, so wrong DDL fails this test loudly.
    ...
    try {
      ...
      assertEquals(1, migrated.spanDao().getSpans(cursor = -1, limit = 10).size)
    } finally {
      migrated.close()
      context.deleteDatabase(name)
    }
