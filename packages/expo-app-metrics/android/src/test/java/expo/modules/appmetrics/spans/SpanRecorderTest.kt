...
class SpanRecorderTest {
  ...
  @Test
  fun `stores the status message with an error status`() {
  ...
  @Test
  fun `drops a status message accompanying an OK status`() {
    // The Trace API allows a description only with the Error status code.
    val row = endedRow(makeRecorder(), statusCode = Span.STATUS_OK, statusMessage = "all good")
    assertEquals(Span.STATUS_OK, row.statusCode)
    assertNull(row.statusMessage)
  }
...
class SpanRecorderDatabaseTest {
  ...
  fun `a recorded span round-trips through the spans table`() = runTest {
    ...
    try {
      ...
      database.spanDao().insert(row)
      val stored = database.spanDao().getSpans(afterId = -1, limit = 10).single()
