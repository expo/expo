...
export default class PhoneAuthListener {
    ...
    constructor(auth, phoneNumber, timeout, forceResend) {
        ...
        if (isAndroid) {
            this._auth.nativeModule.verifyPhoneNumber(phoneNumber, this._phoneAuthRequestKey, this._timeout, this._forceResending);
        }
        if (isIOS) {
    ...
    on(event, observer, errorCb, successCb) {
        ...
        invariant(event === 'state_changed', INTERNALS.STRINGS.ERROR_ARG_INVALID_VALUE('event', 'state_changed', event));
        invariant(isFunction(observer), INTERNALS.STRINGS.ERROR_MISSING_ARG_NAMED('observer', 'function', 'on'));
