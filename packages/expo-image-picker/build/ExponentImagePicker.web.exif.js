...
function readTagValue(view, entryOffset, tiffOffset, tiffEnd, littleEndian) {
    ...
    switch (type) {
        ...
        case 3:
            return readNumberArray(view, valueOffset, count, 2, littleEndian, (offset) => view.getUint16(offset, littleEndian));
        case 4:
            return readNumberArray(view, valueOffset, count, 4, littleEndian, (offset) => view.getUint32(offset, littleEndian));
        case 5:
            return readRationalArray(view, valueOffset, count, littleEndian, false);
        case 6:
            return readNumberArray(view, valueOffset, count, 1, littleEndian, (offset) => view.getInt8(offset));
        case 7:
            return new Uint8Array(view.buffer.slice(valueOffset, valueOffset + count));
        case 8:
            return readNumberArray(view, valueOffset, count, 2, littleEndian, (offset) => view.getInt16(offset, littleEndian));
        case 9:
            return readNumberArray(view, valueOffset, count, 4, littleEndian, (offset) => view.getInt32(offset, littleEndian));
        case 10:
            return readRationalArray(view, valueOffset, count, littleEndian, true);
        case 11:
            return readNumberArray(view, valueOffset, count, 4, littleEndian, (offset) => view.getFloat32(offset, littleEndian));
        case 12:
            return readNumberArray(view, valueOffset, count, 8, littleEndian, (offset) => view.getFloat64(offset, littleEndian));
        default:
            return null;
    }
}
function getUnitSize(type) {
    switch (type) {
        case 1:
        case 2:
        case 6:
...
function readUnsignedBytes(view, offset, count) {
...
function readNumberArray(view, offset, count, step, _littleEndian, reader) {
    const values = Array.from({ length: count }, (_, index) => reader(offset + index * step));
    const first = values[0];
    return values.length === 1 && first !== undefined ? first : values;
}
function readRationalArray(view, offset, count, littleEndian, signed) {
...
function normalizeGpsCoordinate(rawValue, directionRef) {
...
function normalizeGenericValue(rawValue) {
...
//# sourceMappingURL=ExponentImagePicker.web.exif.js.map