...
type ResolvedVideoSource = {
  uri: string;
  release: () => void;
  shouldUseAnonymousCors: boolean;
};

type ThumbnailSizeLimit = {
  maxWidth: number;
  maxHeight: number;
};

type RequestedThumbnail = {
...
type ThumbnailRenderer = {
  width: number;
  height: number;
  render(requestedTime: number, actualTime: number, uri: string): VideoThumbnailWeb;
  renderUri(): string;
};
...
export async function generateVideoThumbnailsAsync(
  source: ThumbnailSource,
  times: number | number[],
  options: VideoThumbnailOptions = {}
): Promise<VideoThumbnailWeb[]> {
  ...
  try {
    ...
    const thumbnails = new Array<VideoThumbnailWeb>(requestedThumbnails.length);
    let cachedFrame: RenderedFrame | null = null;

    for (const thumbnail of requestedThumbnails) {
      const renderedFrame = await renderFrameAsync(
        video,
        thumbnail.targetTime,
        renderer,
        cachedFrame
      );
      cachedFrame = renderedFrame;
      thumbnails[thumbnail.originalIndex] = renderer.render(
        thumbnail.requestedTime,
        renderedFrame.actualTime,
        renderedFrame.uri
      );
    }
...
async function resolveVideoSourceAsync(source: ThumbnailSource): Promise<ResolvedVideoSource> {
...
function createVideoElement(sourceUri: string, shouldUseAnonymousCors: boolean): HTMLVideoElement {
...
async function waitForVideoMetadataAsync(video: HTMLVideoElement): Promise<void> {
...
async function waitForVideoFrameAsync(
...
function createThumbnailRenderer(
  video: HTMLVideoElement,
  sizeLimit: ThumbnailSizeLimit | null
): ThumbnailRenderer {
  ...
  return {
    ...
    renderUri(): string {
      ...
      try {
        ...
        throw new Error(
          'Failed to generate a thumbnail because the video source is not readable on web. Make sure the source is same-origin or CORS-enabled.'
        );
      }
    },
  };
...
function cleanupVideoElement(video: HTMLVideoElement) {
  try {
    video.pause();
...
function normalizeRequestedTimes(times: number | number[]): number[] {
...
function prepareRequestedThumbnails(
...
async function renderFrameAsync(
  ...
  cachedFrame: RenderedFrame | null
...
function resolveSizeLimit(options: VideoThumbnailOptions): ThumbnailSizeLimit | null {
  if (options.maxWidth == null && options.maxHeight == null) {
    return null;
  }

  if (
    (options.maxWidth != null && (!Number.isFinite(options.maxWidth) || options.maxWidth < 1)) ||
...
function clampSeekTimeToDuration(requestedTimeSeconds: number, durationSeconds: number): number {
