...
class NowPlayingManager: VideoPlayerObserverDelegate {
  ...
  private let imageCache = NowPlayingArtworkCache()
  ...
  private func updateNowPlayingInfo() {
    ...
    let artworkUrlString = notificationData?.imageUrl

    if let urlString = notificationData?.imageUrl, let url = NSURL(string: urlString), let player = mostRecentInteractionPlayer, currentArtwork == nil {
      imageCache.load(url: url, item: urlString) { [weak self] _, artwork in
