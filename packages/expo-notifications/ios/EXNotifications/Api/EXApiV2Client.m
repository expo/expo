...
- (nullable NSURLSessionTask *)callRemoteMethod:(NSString *)methodPath
  ...
  NSURLSessionTask *task = [_urlSession
      dataTaskWithRequest:request
        completionHandler:^(NSData *_Nullable data, NSURLResponse *_Nullable response,
          ...
          NSAssert([response isKindOfClass:[NSHTTPURLResponse class]],
                   @"The API response must be an HTTP response");
          NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;

          // Malformed response errors
          if (!data) {
            NSError *responseError = [NSError
          ...
          if (!object) {
            NSError *responseError =
                [NSError errorWithDomain:EXApiErrorDomain
                                    code:EXApiErrorCodeMalformedJson
                                userInfo:@{
                                  NSLocalizedDescriptionKey :
                                      @"The Expo server's response wasn't valid JSON",
          ...
          if (![object isKindOfClass:[NSDictionary class]]) {
            NSError *responseError =
                [NSError errorWithDomain:EXApiErrorDomain
                                ...
                                userInfo:@{
                                  NSLocalizedDescriptionKey :
                                      @"The Expo server's response wasn't a JSON object",
...
- (NSData *)_requestBodyForMethod:(NSString *)methodPath
    ...
    *error = [NSError errorWithDomain:EXApiErrorDomain
                             ...
                             userInfo:@{
                               NSLocalizedDescriptionKey : [NSString
                                   stringWithFormat:@"The arguments for the remote API call to "
  ...
  if (!jsonData) {
    *error = [NSError errorWithDomain:EXApiErrorDomain
                             ...
                             userInfo:@{
                               NSLocalizedDescriptionKey : [NSString
                                   stringWithFormat:@"Something went wrong encoding the arguments "
                                                    @"for the remote API call to \"%@\"",
                                                    methodPath],
                               NSUnderlyingErrorKey : jsonError,
                             }];
    return nil;
...
- (NSError *)_errorFromDictionary:(NSDictionary *)errorObject {
  ...
  errorInfo[NSLocalizedDescriptionKey] =
      errorObject[@"message"] ? [errorObject[@"message"] description]
                              : @"Something went wrong communicating with the Expo server.";

  errorInfo[EXApiErrorCodeKey] =
      errorObject[@"code"] ? [errorObject[@"code"] description] : @"UNKNOWN";
