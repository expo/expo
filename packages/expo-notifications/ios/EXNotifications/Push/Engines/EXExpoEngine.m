...
- (NSString *)generateTokenForAppId:(NSString *)appId withToken:(NSString *)token
{
  ...
  [[EXApiV2Client sharedClient] getExpoPushTokenForExperience:appId deviceToken:token inDevelopmentMode:_inDevelopment completionHandler:^(NSString * _Nullable expoPushToken, NSError * _Nullable error) {
...
- (void)sendTokenToServer:(NSString *)token
{
  [self checkIfInDevelopment];
  dispatch_semaphore_t sem = dispatch_semaphore_create(0);
  [[EXApiV2Client sharedClient] updateDeviceToken:token inDevelopmentMode:_inDevelopment completionHandler:^(NSError * _Nullable postError) {
    dispatch_semaphore_signal(sem);
  }];
  dispatch_semaphore_wait(sem, DISPATCH_TIME_FOREVER);
}
