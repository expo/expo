...
Time it takes `expo-notifications` to fulfill a `Promise` returned by `getDevicePushTokenAsync` or `getExpoPushTokenAsync` may vary and sometimes may be very long on iOS. Citing Apple's [“Troubleshooting Push Notifications” Technical Note](https://developer.apple.com/library/archive/technotes/tn2265/_index.html):
...
As mentioned, probably the most common reason for this this is a flaky or incompatible Internet connection (fetching a push token requires an Internet connection to register the device in the push notifications service provider) or an invalid configuration of the App ID or Provisioning Profile, but looking at proposed solutions there may be more to it.

When it comes to solutions historically there are many different ways people have allegedly solved this problem, let's mention at least some of them -- maybe you'll be lucky to solve the problem too!
...
- Go read the Apple's [Technical Note on troubleshooting push notifications](https://developer.apple.com/library/archive/technotes/tn2265/_index.html)! This the single most reliable source of information on this problem. To help you grasp what they're suggesting:
  - Make sure the device has a reliable connection to the Internet (try turning off Wi-Fi or switching to another network, disabling firewall block on port 5223, also suggested in [this SO answer](https://stackoverflow.com/a/34332047/1123156)).
  - Make sure your app configuration is set properly for registering for push notifications (for bare workflow check out [this guide](https://developer.apple.com/library/ios/documentation/IDEs/Conceptual/AppDistributionGuide/AddingCapabilities/AddingCapabilities.html#//apple_ref/doc/uid/TP40012582-CH26-SW6), for managed workflow this should be done automatically for you by `expo-cli`) as also suggested by [this StackOverflow answer](https://stackoverflow.com/a/10791240/1123156).
  - If you're in bare workflow you may also want to try to debug this even further by logging persistent connection debug information as outlined by the note and [this StackOverflow answer](https://stackoverflow.com/a/8036052/1123156) if nothing else works.
- Sometimes APNS servers where the device wants to register may be down as suggested by [this forum thread](https://developer.apple.com/forums/thread/52224). Take a walk and try again!
...
> **Note:** Especially on iOS, `Promise` returned by this method may take longer periods of time to fulfill (this method uses [`getDevicePushTokenAsync`](#getdevicepushtokenasync-devicepushtoken) under the hood). For more information see [Fetching a push token takes a long time on iOS](#fetching-a-push-token-takes-a-long-time-on-ios) section of the documentation.
...
> **Note:** Especially on iOS, `Promise` returned by this method may take longer periods of time to fulfill. For more information see [Fetching a push token takes a long time on iOS](#fetching-a-push-token-takes-a-long-time-on-ios) section of the documentation.
