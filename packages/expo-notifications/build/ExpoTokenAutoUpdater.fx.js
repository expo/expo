 ...
 */
async function updateActiveRegistrationAsync(token) {
...
InstallationIdProvider.getRegistrationsAsync?.()
    ...
    if (persistedRegistrations.length > 0) {
        await setAutoTokenRegistrationAsync(persistedRegistrations[persistedRegistrations.length - 1]);
    }
})
 ...
 */
export async function setAutoTokenRegistrationAsync(registration) {
    ...
    try {
        ...
        console.warn('[expo-notifications] Could not have removed the active registration information for automatic token update.', e);
    }
