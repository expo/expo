...
- [android] Add opt-in `alarmClock` option to `DateTriggerInput` that delivers the notification via `AlarmManager.setAlarmClock()`, which OEM battery policies do not defer. Intended for time-critical, user-facing alarms. ([#PLACEHOLDER](https://github.com/expo/expo/pull/PLACEHOLDER) by [@capt-muji](https://github.com/capt-muji))
