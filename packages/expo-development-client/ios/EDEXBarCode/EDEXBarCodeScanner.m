...
@interface EDEXBarCodeScanner() <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
...
NSString *const EDEX_BARCODE_TYPES_KEY = @"barCodeTypes";

@implementation EDEXBarCodeScanner
...
- (void)setSettings:(NSDictionary<NSString *, id> *)settings
{
  for (NSString *key in settings) {
    if ([key isEqualToString:EDEX_BARCODE_TYPES_KEY]) {
      NSArray<NSString *> *value = settings[key];
      NSSet *previousTypes = [NSSet setWithArray:_settings[EDEX_BARCODE_TYPES_KEY]];
      ...
      if (![previousTypes isEqualToSet:newTypes]) {
        ...
        nextSettings[EDEX_BARCODE_TYPES_KEY] = value;
...
- (void)maybeStartBarCodeScanning
{
  ...
  if (_settings && _settings[EDEX_BARCODE_TYPES_KEY]) {
    requestedObjectTypes = [[NSArray alloc] initWithArray:_settings[EDEX_BARCODE_TYPES_KEY]];
  }
...
{
  if (!_settings || !_settings[EDEX_BARCODE_TYPES_KEY] || !_metadataOutput) {
    return;
  }
  ...
  for (AVMetadataObject *metadata in metadataObjects) {
    if ([metadata isKindOfClass:[AVMetadataMachineReadableCodeObject class]]) {
      ...
      for (id barcodeType in _settings[EDEX_BARCODE_TYPES_KEY]) {
...
//
//  if (result) {
//    NSString* type = [EDEXBarCodeScanner zxingFormatToString:result.barcodeFormat];
...
{
  return @{
           EDEX_BARCODE_TYPES_KEY: [[EDEXBarCodeScannerUtils validBarCodeTypes] allValues],
           };
}
