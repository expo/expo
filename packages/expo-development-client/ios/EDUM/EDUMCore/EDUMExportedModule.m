...
#define EDUM_IS_METHOD_EXPORTED(methodName) \
[methodName hasPrefix:@EXPAND_AND_QUOTE(EDUM_EXPORTED_METHODS_PREFIX)]

static const NSString *noNameExceptionName = @"No custom +(const NSString *)exportedModuleName implementation.";
static const NSString *noNameExceptionReasonFormat = @"You've subclassed an EDUMExportedModule in %@, but didn't override the +(const NSString *)exportedModuleName method. Override this method and return a name for your exported module.";
...
@implementation EDUMExportedModule
  ...
  while (klass) {
    ...
    @try {
      for(int i = 0; i < methodsCount; i++) {
        ...
        if (EDUM_IS_METHOD_EXPORTED(methodName)) {
          IMP imp = method_getImplementation(method);
          const EDUMMethodInfo *info = ((const EDUMMethodInfo *(*)(id, SEL))imp)(klass, methodSelector);
...
- (void)callExportedMethod:(NSString *)methodName withArguments:(NSArray *)arguments resolver:(EDUMPromiseResolveBlock)resolve rejecter:(EDUMPromiseRejectBlock)reject
{
  ...
  if (methodSignature == nil) {
    ...
    // it means that it has been exported with EDUM_IMPORT_METHOD and if we cannot find method signature
