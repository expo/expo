...
@interface EDUMUtilities ()

@property (nonatomic, nullable, weak) EDUMModuleRegistry *moduleRegistry;

@end

@protocol EDUMUtilService
...
@implementation EDUMUtilities

EDUM_REGISTER_MODULE();

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(EDUMUtilitiesInterface)];
}
...
- (nullable NSDictionary *)launchOptions
{
  id<EDUMUtilService> utilService = [_moduleRegistry getSingletonModuleForName:@"Util"];
...
+ (UIColor *)UIColor:(id)json
{
  ...
  if ([json isKindOfClass:[NSArray class]]) {
    ...
    EDUMLogInfo(@"%@ cannot be converted to a UIColor", json);
    return nil;
  }
}
...
+ (NSDate *)NSDate:(id)json
{
  if ([json isKindOfClass:[NSNumber class]]) {
    ...
    if (!date) {
      EDUMLogError(@"JSON String '%@' could not be interpreted as a date. "
                  "Expected format: YYYY-MM-DD'T'HH:mm:ss.sssZ", json);
    }
    return date;
  } else if (json) {
    EDUMLogError(json, @"a date");
  }
  return nil;
}
...
UIApplication * EDUMSharedApplication(void)
{
...
NSError *EDUMErrorWithMessage(NSString *message)
{
  NSDictionary<NSString *, id> *errorInfo = @{NSLocalizedDescriptionKey: message};
  return [[NSError alloc] initWithDomain:@"EDUMModulesErrorDomain" code:0 userInfo:errorInfo];
}
