...
export function requireNativeViewManager<P = any>(viewName: string): React.ComponentType<P> {
  if (__DEV__) {
    const { ExpoDevelopmentClientNativeUnimoduleProxy } = NativeModules;
    if (!ExpoDevelopmentClientNativeUnimoduleProxy.viewManagersNames.includes(viewName)) {
      const exportedViewManagerNames = ExpoDevelopmentClientNativeUnimoduleProxy.viewManagersNames.join(', ');
      console.warn(
