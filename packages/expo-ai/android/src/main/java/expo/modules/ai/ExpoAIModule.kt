...
class ExpoAIModule : Module() {
  internal var backendFactory: () -> LanguageModelBackend = { defaultLanguageModelBackend(Build.VERSION.SDK_INT) }
      ...
      tasks.start(
        ...
        try {
          when (backend.status()) {
            ...
            ModelStatus.UNAVAILABLE -> throw LanguageModelException("ERR_MODEL_UNAVAILABLE", unavailableSessionMessage(backend))
          }
          request.ensureActive()
