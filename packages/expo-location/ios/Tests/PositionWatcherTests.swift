...
struct PositionWatcherTests {
  ...
  func `pause suppresses buffered positions and pending errors`(bufferPosition: Bool) async throws {
    ...
    watcher.send = { [weak watcher] payload in
      ...
      guard let watcher, !watcher.status().isPaused else {
        return
      }
      // Queue the next result while the consumer is still handling the first one.
      if bufferPosition {
        source.continuations.last?.yield(Self.location(latitude: 2, secondsAgo: 0))
      }
      source.continuations.last?.finish(throwing: LocationServicesDisabledGlobally())
      watcher.pause()
      pausedContinuation.finish()
    }
    watcher.start()
    _ = await source.nextProfile()
    source.continuations.last?.yield(Self.location(latitude: 1, secondsAgo: 1))

    for await _ in paused {}
    try await Task.sleep(for: .milliseconds(50))
    let sent = await payloads.collect(after: {})
