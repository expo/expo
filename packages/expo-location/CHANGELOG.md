...
- [Android] Stop requiring foreground location permissions for `geocodeAsync` and `reverseGeocodeAsync`. `android.location.Geocoder` needs no location permission, and iOS never required one, so the check only made the two platforms disagree. ([#48676](https://github.com/expo/expo/pull/48676) by [@ide](https://github.com/ide))
