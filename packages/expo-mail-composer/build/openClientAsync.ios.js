 ...
 */
const getAvailableMailAppsAsync = async () => (await Promise.all(mailClients.map(async (app) => ((await canOpenURL(app.url)) ? app : null)))).filter(Boolean);
/**
 * Prompts the user to choose a mail app from the list of available mail apps.
 * @param title
 * @param cancelLabel
 */
async function askMailAppChoiceAsync({ title, cancelLabel, }) {
    ...
    return new Promise((resolve) => {
        ...
        }, (buttonIndex) => resolve(buttonIndex === availableMailApps.length ? null : availableMailApps[buttonIndex]));
    });
}
/**
 ...
 * @param options
 */
export async function openClientAsyncIos(options) {
    const mailApp = await askMailAppChoiceAsync(options);
    if (mailApp) {
        await openURL(mailApp.url);
    }
}
//# sourceMappingURL=openClientAsync.ios.js.map