...
open class JavaScriptRuntime: Equatable, Identifiable, @unchecked Sendable {
  ...
  /// Transitional variant of ``AsyncFunctionClosure`` that receives owned values and runs entirely
  /// in asynchronous context. Kept so code generated by the current `expo-modules-macros` release
  /// keeps compiling; it will be deprecated and removed once the macros emit the two-phase shape.
  /// New code should use ``AsyncFunctionClosure`` — decoding synchronously avoids copying `this`
  /// and the arguments into the scheduled task.
  public typealias LegacyAsyncFunctionClosure =
    @JavaScriptActor (
      _ this: JavaScriptValue,
      _ arguments: consuming JavaScriptValuesBuffer
    ) async throws -> JavaScriptValue

  /// Owns the copied `this` and arguments of a ``LegacyAsyncFunctionClosure`` call across the
  /// asynchronous boundary. Registered with the runtime's ``LongLivedObjectCollection`` so a call
  /// abandoned when the runtime tears down (e.g. on reload) has its `jsi::Value`s released by the
  /// teardown sweep on the JS thread, instead of wherever the dropped task frame dies (#47716).
  @JavaScriptActor
  private final class LegacyAsyncFunctionCall: LongLivedObject {
    let this = JavaScriptValue.Ref()
    let arguments = JavaScriptRef<JavaScriptValuesBuffer>()
  ...
  /// Creates an asynchronous host function from a ``LegacyAsyncFunctionClosure``. See its
  /// documentation — this overload exists for compatibility with the current `expo-modules-macros`
  /// output and copies `this` and the arguments into a runtime-owned box for safe async access.
  @JavaScriptActor
  public func createAsyncFunction(_ name: String, _ function: sending @escaping LegacyAsyncFunctionClosure)
    -> JavaScriptFunction
  {
    return createAsyncFunction(name) { this, arguments in
      // Local so the body below captures the collection, not `self`: the body crosses the
      // asynchronous boundary, and a captured runtime wrapper could outlive its runtime inside a
      // dropped task, tearing down wrapper-held JSI state (e.g. cached `PropNameID`s) against
      // freed memory.
      let longLivedObjects = self.longLivedObjects

      // The legacy closure owns its values in asynchronous context, so they must be copied into
      // a box owned by the long-lived object collection: a task dropped after the runtime tears
      // down (e.g. on reload) then has nothing to release against the freed runtime.
      let pendingCall = LegacyAsyncFunctionCall()
      pendingCall.this.reset(this.copied(in: self))
      pendingCall.arguments.reset(arguments.copy())
      longLivedObjects.add(pendingCall)

      return {
        // The task runs on the JS thread while the runtime is alive, so take the values back out
        // and deregister the box: a stream of calls must not pin their arguments until teardown.
        longLivedObjects.remove(pendingCall)
        return try await function(pendingCall.this.take(), pendingCall.arguments.take())
      }
    }
  }
