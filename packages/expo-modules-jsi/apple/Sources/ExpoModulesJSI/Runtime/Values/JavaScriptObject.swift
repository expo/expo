...
public struct JavaScriptObject: JavaScriptType, Sendable, ~Copyable {
  ...
  /// Sets a property to an asynchronous host function created from a
  /// ``JavaScriptRuntime/LegacyAsyncFunctionClosure``. A transitional overload kept for
  /// compatibility with code generated by the current `expo-modules-macros` release; the `async`
  /// closure body selects it. See ``JavaScriptRuntime/createAsyncFunction(_:_:)-(String,LegacyAsyncFunctionClosure)``.
  @JavaScriptActor
  public func setProperty(_ name: String, function: sending @escaping JavaScriptRuntime.LegacyAsyncFunctionClosure) {
    guard let runtime else {
      FatalError.runtimeLost()
    }
    setProperty(name, value: runtime.createAsyncFunction(name, function))
