    ...
    .target(
      ...
      swiftSettings: [
        ...
        .unsafeFlags([
          ...
          // Xcode 27 reports a warning left on a SwiftCompile task as "the following command
          // failed with exit code 0 but produced no further output" and fails the host app's
          // build phase, with nothing pointing at the cause. This group is the one that has
          // actually bitten us, so reject it at the line that causes it.
