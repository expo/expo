...
- [iOS] Drop `SWIFT_RETURNS_RETAINED` from the `RuntimeScheduler` constructors: Xcode 26.2's Swift rejects the attribute on a constructor ("cannot be annotated with either SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED because it is not returning a SWIFT_SHARED_REFERENCE type"), which failed every iOS build in the ExpoModulesJSI xcframework step. The class-level `SWIFT_SHARED_REFERENCE` declaration already governs ownership. ([#49214](https://github.com/expo/expo/issues/49214), [#49426](https://github.com/expo/expo/issues/49426), [#49667](https://github.com/expo/expo/issues/49667) by [@iankurrathi](https://github.com/iankurrathi))
