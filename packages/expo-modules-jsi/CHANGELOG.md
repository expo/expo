...
- [iOS] `JavaScriptValuesBuffer` holds its runtime as `unowned(unsafe)` instead of `unowned`, dropping per-call reference-counting from the synchronous host-call hot path. ([#47835](https://github.com/expo/expo/pull/47835) by [@tsapeta](https://github.com/tsapeta))
