...
Shipped [observed — 2026-08-22]: `exagent deploy --native` uploads the gzip source tarball (create-launch's ignore rules + 500 MB check; `--upload-root` for monorepos → `x-project-root`) as the signed-in user and prints the launch URL as the browser handoff, 8 h expiry noted (hardcoded — the service returns no `expiresAt`; asking www to add one is a recorded follow-up). `--platform`/`--profile` are typed errors now (a launch covers both platforms). Web rail unchanged. Hermetic e2e against a local HTTP stub; the unauthenticated path is an eval scenario grading exit code + the `cli:error` event.
