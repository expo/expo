...
Register exception [confirmed — Kudo, 2026-08-26]: **`eas register` does not exist** [observed — eas-cli 22.5.0, `eas register --help` answers `Error: Command register not found`], so the rule above has nothing to apply. `register` keeps the `npx expo` rung the other three gave up — `project-expo → runner-expo`, through `resolvePackageRunner` so a Bun user gets `bunx expo` — and pays the SDK download that motivated this whole section.

That is not an inconsistency, it is the same cost/benefit landing the other way. The download is worth avoiding for `whoami` because an agent asks it on every run and the answer is a line in a file; it is worth paying for `register` because the command creates an account — once per person, interactively, in a browser — and no other CLI on the machine can do it. The alternative considered and rejected was to keep failing with a message telling the reader to open expo.dev/signup by hand: that spends the same minute of their time and delivers less. stdio stays inherited, which is the hard constraint either way. A line on stderr names the CLI and warns about the download, so the reader is not surprised by a minute of silence [observed — the notice is `authFallbackNotice`'s `runner-expo` branch].
...
- **A bare group is answerable.** `exagent runtime` prints the group's actions and exits 0; a group that declares a `defaultAction` runs it instead (`exagent skills` syncs, `exagent doctor` checks), and `exagent <group> --help` is always the listing.
