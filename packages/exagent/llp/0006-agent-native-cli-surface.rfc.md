...
Grouping rule [confirmed — Kudo, 2026-08-22]: **a capability with several actions is one colon group, `<group>:<action>`, spelled the way `eas-cli` spells its own commands** (`runtime:eval`, `skills:list`, `checkpoint:undo`, `agents:setup`). Rationale [inferred]: the flat namespace was going to collide with `expo`'s as both grow, and a colon makes the boundary legible in one glance — an `exagent` capability is on the left of the colon, an `expo` command never has one. That last property is load-bearing for the forwarding half of the naming rule: a name with a colon is always ours, so it is never forwarded, and an unknown group or action is an error naming what does exist instead of an `expo` invocation that could not have meant anything.

The rule is implemented as data, not as string matching [observed — 2026-08-22, `src/commandRegistry.ts`]. Two lazy maps — `topLevelCommands` (name → module loader) and `commandGroups` (group → `{ summary, defaultAction?, actions }`) — are the whole surface; one pure `resolveCommand(command, argv)` splits on the first colon, looks the group and action up, and answers with one of five cases (`command`, `group-help`, `unknown-action`, `unknown-group`, `passthrough`) that `cli.ts` acts on without deciding anything again. Consequences, all generic rather than per command:
...
- **The help cannot drift.** `helpSections` groups the surface by the job at hand, and a unit test pins that every registered name appears in exactly one section — a new command cannot ship undiscoverable.
- **Adding a command is one entry.** A new action is a line in a group's `actions`; a new group is a key in `commandGroups`. No `switch`, no `if (command === 'runtime:eval')`, and no CLI framework: the resolution is ~40 lines over the two maps.
