...
- It never changes exit codes, and it is suppressible (`--no-followups` / `EXAGENT_NO_FOLLOWUPS`).
...
cannot, it names `exagent navigate / --print-url`, which covers every development build, whose scheme
...
4. **Version-pinned doc pointers.** Follow-ups may reference the installed module's co-located skill or SDK-pinned docs ([[0003-knowledge-tools-and-skills]]), as in "read `exagent skills:show expo-camera` before using it".
...
1. Do follow-ups also surface in `expo`-family subprocess output that exagent relays, or only for exagent's own commands? v1 does own commands only.
