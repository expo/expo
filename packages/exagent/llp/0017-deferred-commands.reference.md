...
**Date:** 2026-08-26
**Related:** [[0016-v1-scope]], [[0010-agent-conventions]], [[0008-guardrails]], [[0005-runtime-loop-tools]], [[0004-smart-start-and-project-state]], [[0006-agent-native-cli-surface]], [[0009-smart-followups]], [[0002-testing-and-evals]]

## What this document is

[[0016-v1-scope]] took five areas out of the v1 surface and moved their code, verbatim, to
`src/deferred/<area>/`. This is where their **design** went: one section per area, each carrying
what was built, why it left, what would bring it back, and which directory holds the code.
...
restored from its shelf directory *and* this section — the code is the mechanism and this is why the
...
**What is not here.** A deferred area's findings that still govern **live** code stayed in the LLP
that made them, and every section below names which and where. That split is the whole care this
consolidation takes: `dev:wait`'s bundle check is `smoke`'s bundle check, and moving its design onto
a shelf would have shelved a document that describes running code.
...
and the entry bundle compiled; `smoke` proves those and then opens the app, evaluates in it and
...
deferral rather than a cost of it ([[0016-v1-scope]] §Where a deferred command's callers were sent).
...
What returns is the command layer below; the library it called never left.
...
**Re-entry criteria:** a driving agent is observed losing work this would have held — which means a
case where the agent's own git was not enough, not merely a case where a snapshot would also have
worked. The mechanism below is what it returns as, and `checkpoint:undo`'s documented limit (it
...
So the honesty had to travel with the artifact, not stay in a document. Two things follow, and both were built:
...
waiting on a local `expo run:ios` and on an EAS build — so that `exagent build --wait` is one answer
rather than two commands wearing one name. The table below is what it returns as.

**What stayed in [[0010-agent-conventions]]:** the `20`–`29` outcome band itself, which this command
was the first into and which every command that stayed still uses; the rule that no `eas --json`
payload can contain a `null`; and the rule that the convention does not reach a forwarded exit code.
All three are in §Exit codes there.
...
and effectively unavailable on Expo Go — the two refusals below are not edge cases but the usual
answer there, so the command's most common outcome was an explanation of why it could not answer.
...
**What stayed in [[0005-runtime-loop-tools]]:** the §Candidates bullet that proposed it, as the
record of what was asked for; the shared runtime target selection and the Expo-Go-for-Android
...
**Why it left** [confirmed — Kudo, 2026-08-26]: the split it opens with — expo-doctor diagnoses,
...
deleting command in a first release for the sake of completeness. `doctor` / `doctor:check` ship
unchanged and are still the v1 answer to "what is wrong with this project".

**Re-entry criteria:** the v1 surface has been used enough to say which resets agents actually reach
for, and a tier table can be cut down to those; and the checkpoint question is answered, because
this command's whole subject is gitignored and no checkpoint holds it (§Deleting what a checkpoint
cannot hold above). It returns as an action of the `doctor` group, with the dry-run default
unchanged.

**What stayed in [[0010-agent-conventions]]:** its two upstream asks — `expo cache:clear` and
`expo-doctor --json` with stable check ids — which are recorded in §Upstream asks there and are
