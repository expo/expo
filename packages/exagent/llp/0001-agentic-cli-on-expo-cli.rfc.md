...
- [[0024-cli-ui]] — the CLI's own surface, read as a UI: one enforced help template per command (purpose, usage, options, examples with their outcomes, `Typically next`, the `--json` contract), a workflow map above the top-level listing, and `npx exagent help how-to` as the on-ramp that teaches the loop, the exit-code bands and the `--json` rule in one screen. The help is data rather than prose, so a unit test walks the registry and holds every command to the shape. Plus the palette, off under `--json`, off outside a TTY, off under `NO_COLOR`.
