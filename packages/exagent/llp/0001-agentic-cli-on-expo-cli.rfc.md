...
- [[0023-fingerprint-caching]] — paying for one fingerprint instead of three: an in-process memo, a cross-run record under `.expo/` revalidated against the files that can move a hash, the bare-project stat walk and why it is cheap enough to keep, the 24-hour ceiling that bounds what a dynamic `app.config.js` can hide, and the measured 38% off `status --explain`. Also the finding that a default `status` is bound by its dev-server port scan and not by the fingerprint.
