  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        '',
        chalk`  {bold It never force-stops an app it can see.} With a runtime connected and the broadcast`,
        chalk`  unable to reach it, this exits {bold 20} and says so, instead of restarting an app you did`,
        chalk`  not ask it to restart. The cheapest way in from there is to edit a file the app has`,
        chalk`  loaded: the dev server pushes that on its own.`,
        '',
        ...
        '',
        chalk`  {bold --cloud reloads by relaunching the app on the session.} The broadcast does {bold not}`,
        chalk`  reach a cloud simulator: the tunnel carries the {bold bundle}, and the app holds no client`,
        chalk`  on this dev server's command socket through it — so a broadcast there reaches nobody`,
        chalk`  even while the dev server's target list names the app. The flag therefore changes the`,
        chalk`  ladder, not just a fallback: {bold auto} runs one controller verb,`,
        chalk`  {bold open <app-id> <url> --relaunch}, which terminates the app and launches it again on`,
        chalk`  the tunnel URL. It needs a tunnelled dev server — {bold dev --detach --tunnel} — and it`,
        chalk`  refuses before touching the app when the dev server is only reachable from here.`,
        '',
        ...
        chalk`  On a cloud session there is no debugger target to wait for — a simulator there`,
        chalk`  registers none — so the observation is the dev server's own: a {bold Bundled} line in its`,
        chalk`  captured output that was not there before the relaunch, which means the app fetched`,
        chalk`  the served bundle again. Start the dev server with {bold dev --detach} so there is output`,
        chalk`  to read; without it a cloud reload can only report that the verb ran.`,
        '',
