  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        '',
        chalk`  {bold --cloud changes only the fallback.} The broadcast already reaches an EAS Simulator`,
        chalk`  session: it goes out over this dev server's socket, and a cloud session has to reach`,
        chalk`  this dev server through a tunnel to be running the bundle at all. The flag is for the`,
        chalk`  device method, which becomes the session controller's {bold close} and {bold open} verbs.`,
        '',
        chalk`  {bold What it checks first.} The project's entry bundle, the same way {bold dev:wait} does. A`,
