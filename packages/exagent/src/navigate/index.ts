  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        chalk`  and stopped by the EAS CLI — {bold npx eas simulator --platform ios --type}`,
        chalk`  {bold agent-device --expo-go} — and it bills until {bold npx eas simulator:stop}. It needs a`,
        chalk`  signed-in account; without one this command exits {bold 7} and says who has to sign in.`,
        chalk`  {bold --expo-go} matters: a session started without it has no app installed, and every`,
        chalk`  link opened on it is refused. Pass {bold --build-id <id>} instead for a development build.`,
        '',
