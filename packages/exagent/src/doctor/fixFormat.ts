...
/** Width of the label column, matching `exagent status` and `doctor:check`. */
...
function closing(payload: FixPlanPayload): string {
  if (!payload.applied) {
    return payload.steps.length
      ? chalk`Nothing was deleted. Run {bold npx exagent doctor:fix --tier ${payload.tier} --apply} to do it.`
