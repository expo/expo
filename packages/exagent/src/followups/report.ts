...
// agent, and the `Next:` section for a human terminal. Nothing here can change an exit code.
...
export interface ReportFollowUpsOptions {
  /** The command prints one JSON object on stdout, so the `Next:` section is left out. */
  json?: boolean;
  /**
