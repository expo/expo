...
// A command `exagent` does not implement is one of the `expo` CLI's own, so it is forwarded there
// verbatim: `exagent prebuild --clean` is `expo prebuild --clean`. Nothing is added — no probe, no
// skill sync, no follow-ups — and nothing is interpreted, so the arguments, the output, the errors
// and the exit code all stay the Expo CLI's. A command neither CLI knows is not special either:
// `expo` reports it, and its exit code is forwarded like any other.
 ...
 *
 * @param command The subcommand `exagent` has no implementation for, e.g. `prebuild`.
 */
