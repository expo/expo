...
- "exagent-fingerprint.json": remembers the native fingerprint of this project per platform, so a command that needs it does not spend a second hashing an unchanged project. Every entry records the size and modification time of the files it was computed against — the lockfiles, the app config, "eas.json", "package.json", the fingerprint's own settings and the assets they point at — and is only believed again while all of them are unchanged, and never for longer than ten minutes. It does not look inside "ios" or "android", so the ten minutes is what covers a native edit; "exagent dev" deletes the whole file after any step that changes the project. A report answered from here says so, with the check it used and the age of the answer. Delete it, or pass "--no-fingerprint-cache", to hash the project again.
