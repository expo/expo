...
- "exagent-fingerprint.json": remembers the native fingerprint of this project per platform, so a command that needs it does not spend a second hashing an unchanged project. Every entry records the files it was computed against — the lockfiles, the app config, "eas.json", "package.json", the fingerprint's own settings, and "ios"/"android" when they exist — and is only believed again while all of them are unchanged, and never for longer than a day. A report answered from here says so. Delete it, or pass "--no-fingerprint-cache", to hash the project again.
...
- "dev/logs/dev-detached.log": everything a dev server started with "exagent dev --detach" printed, since that run has no terminal to print it to. Read it with "exagent dev:logs". One file per project, truncated on each detached run, because a project has at most one detached dev server.
