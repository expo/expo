  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        '',
        chalk`  {bold Every build this command plans is a local build}, run on this machine by`,
        chalk`  {bold expo run:ios} or {bold expo run:android}: it needs Xcode for ios or the Android SDK for`,
        chalk`  android, and it takes many minutes the first time. The plan says which of its steps`,
        chalk`  build and marks them {bold local}, and it says up front when this machine has no such`,
        chalk`  toolchain rather than stopping at the compiler minutes later.`,
        chalk`  The other route is a cloud build on EAS, which needs an Expo account instead:`,
        chalk`    {dim $} npx eas build --platform ios --profile development`,
        '',
