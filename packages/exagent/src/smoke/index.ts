  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        chalk`  was wrong.`,
        '',
        chalk`  {bold A red screen is defined as what the app reports.} An uncaught exception in the window`,
        chalk`  fails the run; a {bold console.error} is counted and reported separately, because a project`,
        chalk`  that logs one on purpose would otherwise never pass. LogBox is not read: reaching it`,
        chalk`  from an evaluated expression needs a module id this command does not have.`,
        '',
