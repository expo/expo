...
- `exagent start` — get the app onto a device: probe the project, print the plan it decided on (`expo start`, `expo prebuild`, `expo run:ios`/`run:android`), then run it, with agent-friendly output and skills sync. `--plan` prints the plan and exits, `--passthrough` runs `expo start` and nothing else, `--yes` answers the confirmation a terminal is asked for before a plan that builds.
...
- `exagent checkpoint` — snapshot the files git tracks in this project, so a later change can be undone (`--label`, `--json`). `install`, `setup` and a `start` plan that prebuilds take one first; `--no-checkpoint` or `EXAGENT_NO_CHECKPOINT` skips it.
