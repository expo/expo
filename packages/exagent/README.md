...
`deploy --native` stops at the EAS Build page: it starts the cloud build and reports its URL. Native delivery through launch.expo.dev is the plan of `llp/0007`, and its integration is still pending, so the command says so instead of printing a link it cannot produce yet.
