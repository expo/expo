...
struct SceneEventForwarder {
  ...
  /// while `body` ran. Always calls `notify` when `url` is `nil`. Announces `url` itself when
  /// `notify` left `RCTLinkingManager` silent.
  ///
  ...
  private func notifyLinkingManagerUnlessAlreadyNotified(
    ...
    if !observer.wasNotified {
      notify()
      if !observer.wasNotified {
        Self.announceOpenURL(url)
      }
    }
  }

  /// Announces `url` the way `RCTLinkingManager` does, so React Native's `Linking` module turns it
  /// into the JS `url` event.
  ///
  /// React Native 0.88 ignores the app-delegate entry points called above when the app adopts the
  /// scene life cycle, which every app with an `ExpoAppSceneDelegate` does: they return without
  /// announcing anything. Its scene entry points need a `UIScene` and a `UIOpenURLContext`, which
  /// the app-delegate events re-fed here don't carry, and each one only posts this notification.
  private static func announceOpenURL(_ url: URL) {
