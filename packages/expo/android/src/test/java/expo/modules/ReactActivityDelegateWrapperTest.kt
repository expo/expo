...
internal class ReactActivityDelegateWrapperTest {
  ...
  fun `onNewIntent should call each handler's callback just once`() {
    ...
    val delegateWrapper = ReactActivityDelegateWrapper(activity, delegate)
    every { mockPackage0.reactActivityLifecycleListener.onNewIntent(intent, any()) } returns false
    every { mockPackage1.reactActivityLifecycleListener.onNewIntent(intent, any()) } returns true
    every { delegate.onNewIntent(intent) } returns false

    delegateWrapper.onNewIntent(intent)

    verify(exactly = 1) { mockPackage0.reactActivityLifecycleListener.onNewIntent(any(), any()) }
    verify(exactly = 1) { mockPackage1.reactActivityLifecycleListener.onNewIntent(any(), any()) }
