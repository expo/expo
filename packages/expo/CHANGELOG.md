...
- [iOS] Simplify the SwiftPM autolinking plugin's internals: resolve each pod's identity once, pass manifest arguments as named options, and share the helpers its lookups, diagnostics and tests repeated. The generated packages are unchanged.
...
- [iOS] The SwiftPM autolinking plugin now takes a module's root from the autolinking data React Native passes it when the prebuilt-metadata document does not cover that module, instead of walking the filesystem for it. It also warns when a module is installed twice in two different directories, and when the app declares extra CocoaPods dependencies (`extraPods`) that the plugin cannot install. ([#50237](https://github.com/expo/expo/pull/50237) by [@chrfalch](https://github.com/chrfalch))
