...
- [iOS] Fix `expo/fetch` logging an error-level `Invalid state` message when a body stream is cancelled after it already settled, which spec-legal consumer cleanup does on every request. ([#49758](https://github.com/expo/expo/pull/49758) by [@LizunovSergey](https://github.com/LizunovSergey))
