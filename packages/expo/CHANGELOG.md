...
- Propagate `AbortSignal` cancellation into the `expo/fetch` body stream, extending [#44909](https://github.com/expo/expo/pull/44909) to the abort path. Aborting now rejects the in-flight read with an `AbortError` and drops late native events; previously it canceled only the native request, leaving the JS `ReadableStream` controller open — the pending read hung forever and a late `didReceiveResponseData` was enqueued onto an abandoned controller (`The stream is not in a state that permits enqueue`). Also guard the body-stream teardown against a stale `bodyStreamClosed` flag: a rejected `pull()` (failed native `startStreaming` call) errors the stream through the streams implementation's internal error routine without the flag ever being set, so the trailing `didComplete` called `close()` on an errored stream and threw `The stream is not in a state that permits close` out of the event listener, where no consumer `try`/`catch` can reach. ([#47573](https://github.com/expo/expo/pull/47573) by [@idoyana](https://github.com/idoyana))
