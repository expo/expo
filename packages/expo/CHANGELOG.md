...
- [iOS] Fix `expo/fetch` logging an error-level `Invalid state` message when a body stream is cancelled after it already settled, or before it was ever read, which spec-legal consumer cleanup does on every request. Cancelling before the first read now also stops the discarded body from buffering in memory. ([#49758](https://github.com/expo/expo/pull/49758) by [@LizunovSergey](https://github.com/LizunovSergey))
