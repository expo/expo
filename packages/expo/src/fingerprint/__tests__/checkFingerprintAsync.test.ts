...
describe(checkFingerprintAsync, () => {
  ...
  // The server's advice wins, but only when it is well-formed.
  itNative.each([
    {
  ...
  itNative(`resolves rather than rejects when the bundle origin cannot be read`, async () => {
    // `new URL(...)` throws on a malformed script URL, and this promise never rejects.
  ...
  // Every known failure resolves to `unknown`. The last row throws from fetch, hence the thunks.
  itNative.each([
    {
