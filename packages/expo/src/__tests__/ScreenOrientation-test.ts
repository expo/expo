...
it (`throws when supportsOrientationLockAsync is called with an unsupported type`, async () => {
    await expect(ScreenOrientation.supportsOrientationLockAsync(NaN as any)).rejects.toThrowError(TypeError);
    await expect(ScreenOrientation.supportsOrientationLockAsync('test' as any)).rejects.toThrowError(TypeError);
});
...
describe(`Android screen orientation`, () => {
    ...
    it(`calls NativeModules.lockPlatformAsync with only Android properties`, async () => {
      ...
      expect(NativeModules.ExpoScreenOrientation.lockPlatformAsync).toBeCalledWith(screenOrientationConstantAndroid);
    });

    it(`throws when lockPlatformAsync is called with unsupported types in its Android properties`, async () => {
        await expect(ScreenOrientation.lockPlatformAsync({screenOrientationConstantAndroid: NaN as any})).rejects.toThrowError(TypeError);
        await expect(ScreenOrientation.lockPlatformAsync({screenOrientationConstantAndroid: 'test' as any})).rejects.toThrowError(TypeError);
    });
});
...
describe(`iOS screen orientation`, () => {
    ...
    it(`calls NativeModules.lockPlatformAsync with only iOS properties`, async () => {
      ...
      expect(NativeModules.ExpoScreenOrientation.lockPlatformAsync).toBeCalledWith(screenOrientationArrayIOS);
    });

    it(`throws when lockPlatformAsync is called with unsupported types in its iOS properties`, async () => {
        await expect(ScreenOrientation.lockPlatformAsync({screenOrientationArrayIOS: (3 as any)})).rejects.toThrowError(TypeError);
        await expect(ScreenOrientation.lockPlatformAsync({screenOrientationArrayIOS: ['foo' as any]})).rejects.toThrowError(TypeError);
    });
});