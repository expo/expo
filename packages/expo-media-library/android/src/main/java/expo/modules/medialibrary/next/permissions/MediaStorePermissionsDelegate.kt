...
class MediaStorePermissionsDelegate(val appContext: AppContext) {
  ...
  suspend fun requestMediaLibraryWritePermission(uris: Iterable<Uri>) {
    ...
    // MediaStore.createWriteRequest creates a prompt with all given uris, whi
    val urisWithoutPermission = uris.filterNot { uri ->
      hasWritePermissionForUri(uri)
    }
    // Launching MediaStore.createWriteRequest with empty array returns granted = false would lead to
    // PermissionsException, therefore this check is mandatory
