...
- [iOS] Include GPS `location` in `getAssetsAsync` batch results when `resolveWithFullInfo: true` and return numeric latitude/longitude from `exportLocation` (matching the documented `Location` type). ([#48630](https://github.com/expo/expo/pull/48630) by [@hsource](https://github.com/hsource) and [@fractalbeauty](https://github.com/fractalbeauty))
