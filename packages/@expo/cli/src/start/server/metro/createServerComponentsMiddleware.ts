 ...
 *
 * Output keys can be:
 * - Relative paths: "./components/Button.tsx" (resolved from serverRoot)
 * - node_modules paths: "pkg/lib/index.js" (resolved via require.resolve)
 * - Legacy relative paths: "./../../node_modules/pkg/..." (needs special handling)
 */
function resolveOutputKeyToPath(
  outputKey: string,
  projectRoot: string,
  serverRoot: string
): string {
  // Normalize to POSIX for consistent path operations
  const posixOutputKey = toPosixPath(outputKey);

  // Handle relative paths that contain /node_modules/ (legacy format from asset-transformer)
  // These are relative to projectRoot but contain node_modules, extract the package path
  if (posixOutputKey.includes('/node_modules/')) {
    const nodeModulesIndex = posixOutputKey.lastIndexOf('/node_modules/');
    const packagePath = posixOutputKey.slice(nodeModulesIndex + '/node_modules/'.length);
    try {
      return require.resolve(packagePath, { paths: [projectRoot] });
    } catch {
      // Fallback to direct path in serverRoot's node_modules
      return path.resolve(serverRoot, 'node_modules', packagePath);
    }
  }

  // Relative paths starting with ./ or ../ : resolve from projectRoot (app directory)
  // These paths are relative to projectRoot as generated by babel client-module-proxy plugin
  if (posixOutputKey.startsWith('./') || posixOutputKey.startsWith('../')) {
    return path.resolve(projectRoot, outputKey);
  }

  // node_modules paths without prefix: use require.resolve to handle monorepo/pnpm structures
  try {
 ...
 */
function generateOutputKeyFromPath(filePath: string, serverRoot: string): string {
  return './' + toPosixPath(path.relative(serverRoot, filePath));
}
...
export function createServerComponentsMiddleware(
  ...
  async function exportServerActionsAsync(
    ...
    async function processEntryPoint(entryPoint: string) {
      ...
      // Use serverRoot (monorepo root) to match SSR manifest key format
      const serverRoot = getMetroServerRootMemo(projectRoot);
      const publicModuleId = generateOutputKeyFromPath(entryPoint, serverRoot);
  ...
  function getResolveClientEntry(context: {
    ...
    return (file: string, isServer: boolean) => {
      ...
      const filePath = resolveOutputKeyToPath(outputKey, projectRoot, serverRoot);

      if (isExporting) {
        assert(context.ssrManifest, 'SSR manifest must exist when exporting');

        // Compute the manifest key from the file path
        // Must match the in-memory ssrManifest format from MetroBundlerDevServer.ts:
        // Simple relative path from serverRoot (e.g., "node_modules/pkg/..." or "packages/expo-router/...")
        const manifestKey = toPosixPath(path.relative(serverRoot, filePath));
