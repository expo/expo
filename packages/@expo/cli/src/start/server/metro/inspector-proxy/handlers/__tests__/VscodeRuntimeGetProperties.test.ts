...
it('does not respond on non-vscode debugger type', () => {
  ...
  expect(
    handler.onDeviceMessage({ id: 420, result: descriptors }, { debuggerType: 'generic' })
  ).toBe(false);
...
it('mutates `Runtime.getProperties` device response with `description` properties', () => {
  ...
  expect(handler.onDebuggerMessage(message, debuggerInfo)).toBe(false);
  ...
  expect(handler.onDeviceMessage({ id: 420, result: descriptors }, debuggerInfo)).toBe(false);
