...
export interface FingerprintResponse {
  ...
  /** Present only when the request announced a hash that differs from `hash`. */
...
export interface FingerprintMiddlewareOptions {
  ...
  /** Record the hash a client announced. Advice on a mismatch, `null` on a match. */
 ...
 * running app can compare it against the one embedded at build time. The optional
 * `expo-fingerprint` request header announces the caller's own hash.
 *
 * `CorsMiddleware` sends no `Access-Control-Allow-Headers`, so a browser cannot send that
 * header. Native callers are unaffected, and web has no native build to check.
 */
export class FingerprintMiddleware extends ExpoMiddleware {
  ...
  async handleRequestAsync(req: ServerRequest, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      // A broken config plugin throws here. Letting it reach `ExpoMiddleware` would print a red
      // error and a 500 on every reload, since every launch announces and failures are uncached.
      res.statusCode = 503;
      res.end(
        JSON.stringify({
