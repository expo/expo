  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        chalk`--scheme <scheme>               Custom URI protocol to use when launching an app`,
        chalk`-p, --port <number>             Port to start the dev server on (does not apply to web or tunnel). {dim Default: 8081}`,
        ``,
        chalk`--bundler <bundler>             Use an alternative native bundler ({dim metro}, rollipop). {dim Default: metro}`,
        ``,
