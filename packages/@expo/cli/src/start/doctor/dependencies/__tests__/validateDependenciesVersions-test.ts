...
describe('getVersionedDependenciesAsync (TV)', () => {
  ...
  beforeEach(() => {
    ...
    jest.mocked(resolveFrom).mockImplementation((fromDirectory, request) => {
      // Use the `fs` imported at the top of this file so we share the same
      // memfs instance as `vol` — re-requiring `fs` inside this closure can
      // return a fresh memfs after earlier tests' `jest.resetModules()`.
