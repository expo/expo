...
export function upsertGitIgnoreContents(
  ...
  if (targetGitIgnore.includes(generatedHeaderPrefix)) {
    ...
    if (indexes.start > -1 && indexes.end > -1 && indexes.start < indexes.end) {
      // Extract existing content from the generated section (skip header and footer)
      // start + 1: skip the header line (# @generated expo-cli ...)
      // start + 2: skip the comment line (# The following patterns were generated by expo-cli)
      // end: exclude the footer line (# @end expo-cli)
      const existingContentLines = indexes.contents.slice(indexes.start + 2, indexes.end);
      const existingContent = existingContentLines.join('\n').trim();
      const combinedContent =
        existingContentLines.length > 0 ? `${existingContent}\n${contents}` : contents;

      // Create new header with combined content
      const newHeader = createGeneratedHeaderComment(combinedContent);

      // Replace the entire generated section
      const beforeSection = indexes.contents.slice(0, indexes.start).join('\n');
      const afterSection = indexes.contents.slice(indexes.end + 1).join('\n');
      ...
      fs.writeFileSync(targetGitIgnorePath, newContents);
      return { contents: newContents, didClear: false, didMerge: true };
    }
  }
