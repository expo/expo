...
describe(expoUpdatesCommandAsync, () => {
  ...
  it.each<{ mode?: EnvMode; expectedMode: EnvMode }>([
    { expectedMode: 'development' },
    { mode: 'production', expectedMode: 'production' },
  ])('passes $expectedMode without inherited dotenv values', async ({ mode, expectedMode }) => {
    await expect(expoUpdatesCommandAsync('/app', ['runtimeversion:resolve'], mode)).resolves.toBe(
      'output'
    );
    ...
    expect(spawnAsync).toHaveBeenCalledWith(
      ...
      ['runtimeversion:resolve'],
      {
        ...
        env: expect.objectContaining({
          NODE_ENV: 'development',
          SHELL_VALUE: 'from-shell',
          __EXPO_CONFIG_MODE: expectedMode,
        }),
      }
    );
