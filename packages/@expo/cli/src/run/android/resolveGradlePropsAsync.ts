...
export async function resolveGradlePropsAsync(
  ...
  const [buildType, flavors] = variant.split(/(?=D|R)/).map((v) => v).reverse();
  const apkVariantDirectory = path.join(apkDirectory, flavors ?? '', buildType.toLowerCase());
  const architectures = await getConnectedDeviceABIS(options.allArch, buildType, device);
...
async function getConnectedDeviceABIS(
  ...
  if (device) {
    // We have a device, just get it's abi
