...
// The Android Gradle bundle task always passes `--sourcemap-output` so it can compose the Hermes
// source map. That source map is written outside the app binary. DOM component source maps are
// written next to the DOM JS bundle inside the binary, so they must never be emitted here.
// See: https://github.com/expo/expo/issues/49449
