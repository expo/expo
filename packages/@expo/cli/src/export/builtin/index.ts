...
export const expoExportBuiltin: Command = async (argv) => {
  ...
  if (args['--help']) {
    printHelp(
      ...
      [
        ...
        chalk`--pkg <pkg>           package to bundle`,
  ...
  const { exportBuiltinAsync } = await import('./exportBuiltinAsync.js');
  return exportBuiltinAsync(projectRoot, options).catch(logCmdError);
};
