...
describe('@expo/agent-cli runtime:reload', () => {
  ...
  // @ref llp/0005-runtime-loop-tools.rfc.md §What proves a reload
  ...
  // the peer churn's own count was not in the payload at all. The rule (llp/0021 §The rules,
        ...
        try {
          ...
          const evidence: Record<string, number> = {
            ...
            'app-relaunch': report.attempts.filter(
              (attempt: { method: string; ok: boolean }) =>
                attempt.method === 'device' && attempt.ok
            ).length,
          };
...
// @ref llp/0005-runtime-loop-tools.rfc.md §How it reloads — Kudo's cloud loop, K2.
//
