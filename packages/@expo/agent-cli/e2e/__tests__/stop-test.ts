...
describe('@expo/agent-cli dev:stop', () => {
  ...
  it('prints one JSON object with a stable set of keys', async () => {
    ...
    expect(Object.keys(JSON.parse(result.stdout)).sort()).toEqual([
      ...
      // this command signalled, and the port number it was using (F48-10, llp/0005 §Stopping the app
      // is not one listener).
      'portStillAnswering',
      'processStillRunning',
      'reason',
  ...
  // @ref llp/0021-honest-reports.rfc.md §How they show up — friction run 7,
