...
// builds is the argv a whole `@expo/agent-cli` process actually spawns, in order, with the session's
...
describe('@expo/agent-cli navigate --cloud', () => {
  ...
  // @ref llp/0005-runtime-loop-tools.rfc.md §Cloud simulator — S10.
  //
...
describe('@expo/agent-cli runtime:stop --cloud', () => {
  ...
  it(`closes the named app on the session, and never the session itself`, async () => {
    ...
    // command must not claim the app it named had been running (llp/0005 §Cloud simulator).
    // Null, never true.
  ...
  it(`never reaches for a session that was not named`, async () => {
    ...
    // ever spawned is.
    await executeAgentCliAsync(projectRoot, ['runtime:stop', '--ios'], { reject: false });
...
describe('@expo/agent-cli smoke --cloud', () => {
  ...
  // @ref llp/0021-honest-reports.rfc.md §The rules — F96, the first live-cloud run.
  //
...
describe('@expo/agent-cli runtime:reload --cloud', () => {
  ...
  // whole process is pinned: restart the app with no URL on the launch, then deep-link the route
  ...
  // finds the URL unusable is how a billed session was left with nothing on it.
  it(`refuses a dev server the session cannot reach, before any verb`, async () => {
    const projectRoot = await setupAsync('go-app');
    await writeSessionFileAsync(projectRoot, 'sess-e2e');
  ...
  it(`says how to start a session when --cloud finds none`, async () => {
    ...
    try {
      ...
      // Nothing was attempted, so this is the missing device rather than a failed reload: exit 1
      // with the whole of what to do about it, and never a report claiming a reload.
      expect(result.exitCode).toBe(1);
