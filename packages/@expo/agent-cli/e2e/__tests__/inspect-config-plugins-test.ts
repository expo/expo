...
describe('@expo/agent-cli inspect:config-plugins', () => {
  ...
  it('suppresses the follow-ups on request', async () => {
    ...
    const withFollowups = await executeAgentCliAsync(projectRoot, ['inspect:config-plugins'], {
      env,
    });
    const without = await executeAgentCliAsync(
      projectRoot,
      ['inspect:config-plugins', '--no-followups'],
      {
        env,
      }
    );
  ...
  it('prints its own help, naming the colon form', async () => {
    ...
    const result = await executeAgentCliAsync(projectRoot, ['inspect:config-plugins', '--help'], {
      env,
    });
...
// @ref llp/0010-agent-conventions.rfc.md §Registry rules. The unit test pins the
