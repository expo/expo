...
**Type:** RFC
**Status:** Final
**Systems:** `packages/@expo/cli`; JSONL events; `@expo/agent-cli` launcher (new)
...
- **AGENTS.md generation** [confirmed — Kudo accepted, 2026-08-20; design inferred]. `@expo/agent-cli agents:setup` writes and maintains a managed section in the project's `AGENTS.md`: SDK version, targets, the right commands, project quirks. It orients every agent, including ones that never call a tool.
...
The reserved bins (`@expo/agent-cli` / `ai-expo` [observed — npm, reserved by kudochien 2026-08-18]) ship as a model-free CLI. The surface:
