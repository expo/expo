...
**Type:** RFC
**Status:** Active
**Systems:** `packages/@expo/agent-cli`; `packages/@expo/cli`
...
The package declares one bin, `expo-agent-cli` → `bin/cli.js`. It is plumbing, not a name. It is documented nowhere: not in `--help`, not in the README, not in a `Try:` line, not in the `AGENTS.md` managed block. It exists because `npx <package>` requires a declared bin. There is no alias on `$PATH`. `@expo/agent-cli` is the final name, and the only one [confirmed — Kudo, 2026-09-02].
