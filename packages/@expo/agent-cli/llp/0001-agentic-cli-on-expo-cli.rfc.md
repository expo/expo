...
**Type:** RFC
**Status:** Final
**Systems:** `packages/@expo/agent-cli` (new); `packages/@expo/cli`; `expo-mcp` (external repo)
...
The `@expo/agent-cli` bin ships in v1 as a thin, model-free launcher. An embedded-loop agent stays a possible later wrapper, not v1. Shipping is gated on heavy tests and evals.
...
2. The command can be an entirely new bin (`npx ai-expo`, `npx @expo/agent-cli`, or similar), not necessarily an `expo` subcommand.
...
Decision [confirmed — Kudo, 2026-08-20]: `exagent` is the bin and package name. **Superseded before
the first publish** — see below. Those two words are the only place in this package where the old
name still appears, because they are what is reserved on npm and a rename would make the paragraph
below unactionable.

**Final [confirmed — Kudo, 2026-08-28]: the package is `@expo/agent-cli`, in
`packages/@expo/agent-cli/`, and every command this repository prints or teaches is
`npx @expo/agent-cli <command>`.**

**Why scoped.** The unscoped name was chosen while this was a spike, and it reads as a coinage the
reader has to be told about — nothing in the old coinage says Expo, says CLI, or says which of its
two halves carries the meaning. `@expo/agent-cli` says all three, and it says them in the shape the
monorepo already uses for a tool the framework owns: `@expo/cli`, `@expo/fingerprint`,
`@expo/package-manager`. The scope is also the trust signal. An agent that has been handed
`npx @expo/agent-cli status` can see from the name alone that the package comes from the Expo org,
which an unscoped coinage cannot show and which matters more for a command an agent is told to run
than for one a person types.

The cost is length, paid on every printed line. It is real and it is accepted: these lines are
read and pasted far more often by machines than typed by people, and the alternative was a name
that has to be explained once per reader.

**The bin is plumbing, not a name.** The package declares exactly one bin, `expo-agent-cli` →
`bin/cli.js`, and it is documented nowhere: not in `--help`, not in the README, not in a `Try:`
line, not in the `AGENTS.md` managed block. It exists because `npx <package>` requires the package
to declare a bin, and that is the whole of its job. There is deliberately **no** bin under the old
short name and no alias on the PATH — one spelling is the point of the rename, and a second one on
`$PATH` would be a second thing to teach and a second thing to keep in sync. `@expo/cli` sets the
precedent: its bin is `expo-internal`, a name no document tells anybody to run.

Nobody is expected to add this package to their dependencies, so nothing depends on the bin being
pleasant. `npx @expo/agent-cli <command>` is the invocation, and it is the only one.

**Recommendation, not done here:** publish `exagent` and `ai-expo` as thin alias packages whose
own bin re-execs `@expo/agent-cli`, so an agent that learned the reserved short name from an early
thread still lands on the real CLI rather than on a 404 or, worse, on somebody else's package. The
aliases are a publishing task and a squatting defence; they are not a second supported spelling,
and no document should teach them.
...
- New capabilities land as `@expo/agent-cli` commands and Expo skills, not as an agent loop [updated per the 2026-08-22 self-serve clarification].
...
- The reserved bins (`@expo/agent-cli` / `ai-expo`) can still ship as a thin, model-free launcher: start and connect the MCP server, install skills into the user's agents, print project context. [inferred]
...
2. **`expo-mcp` stays untouched; @expo/agent-cli is self-serve.** Clarified decision [confirmed — Kudo, 2026-08-22, superseding the 2026-08-18 reading]: "reuse MCP infra" means @expo/agent-cli may act as an MCP _client_ or add MCP features. It does NOT mean new tools land in the `expo-mcp` codebase. Runtime capabilities (CDP evaluate, error feed, deep-link navigation) are implemented **inside `packages/@expo/agent-cli`** as CLI commands, agent-callable via shell. The existing published `expo-mcp` tools (`automation_*`, `collect_app_logs`, `expo_router_sitemap`, `open_devtools` [observed]) stay available to agents that connect to them. `@expo/mcp-tunnel` stays the remote-transport option for the chat-driven case ([[0007-deploy-and-headless]]).
...
- [[0011-impact-and-freshness]] — `@expo/agent-cli impact`: what a change costs (reload, Metro restart, native build) from the fingerprint diff, and, separately from the `runtimeVersion` policy, whether it can ship over the air.
...
- [[0015-backend-selection-and-config]] — which of the two build backends a plan uses, decided at planning time from the host and the toolchain, plus the developer config (`package.json` › `expo.agentCli`) that overrides it.
