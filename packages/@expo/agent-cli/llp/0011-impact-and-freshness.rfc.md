...
**package**, asking "what must rerun after installing this?". `src/impact/` classifies one
**change**. They are the same question at two scales, and [[0004-smart-start-and-project-state]]
§Sub-features already named the first one that. The module paths are what tell them apart.
`classifyInstallImpactAsync` was left where it is rather than renamed, because it is referenced by
...
That leaves one rule, and it is the reason the fixtures are real rather than synthesized: **the
...
real fingerprint falls through to `unknown` [observed — `classify-test.ts`], so a drift between the
...
`app-config`, `build-config`, `build-scripts`, or `unknown`. That is what decides the *follow-up*,
...
a cheap class, on no evidence, for a project that has no build at all.
...
So the fall-through condition is `fingerprintChanged === false` rather than merely falsy, and the
...
`.env*`, `tsconfig.json`, a lockfile) needs Metro restarted. Everything else is picked up by
...
That is about 56 KB for a two-platform record. No gzip, no subset, no second file: only
`JSON.stringify` without indentation, which gives a third of the bytes back for a file nobody reads
by hand. A project where this grows unreasonably loses a *comparison* and never a command, because
the record is advisory by the same contract llp/0004 gave it.
...
how old it is, because that cache pins its files by size and modification time and does not look at
`ios/` or `android/` at all.
...
--non-interactive`. This is server ground truth and needs no local record, so it answers for a build
this machine never ran. That is the more useful mode, because most builds are made in the cloud and
...
`--platform ios --build <id>` fills it in, because then the caller said so. The alternative, looping
the platforms, would have spawned the identical command twice and reported one answer as two.
...
is the build's fingerprint and `fingerprint2` is the one it computed from the working directory. For
two positional hashes it is the same shape with the same ordering. The guess this section recorded,
that eas-cli depends on `@expo/fingerprint` so the payload would be a `FingerprintDiffItem[]`, **was
wrong.** Against the real service the parser found neither a diff nor a hash under any of the keys it
tried, so `impact --build <id>` answered with a caveat carrying a megabyte of payload and nothing
else. That is worse than the failure this design anticipated. The fallback preserved "whether", and
here even "whether" was lost, because the hashes were not where the fallback looked either.
...
diff that could not be produced, because the project has no `@expo/fingerprint`, costs the detail
and not the verdict. `fingerprintChanged` still comes from the hashes, and the failure rides along as
a caveat.

The older guesses stay in the parser as fallbacks, and the tail-carrying caveat stays behind them.
That is not because the shape is still in doubt. It is because this is another CLI's payload across a
process boundary, and a shape that moves again should degrade to "whether" rather than to nothing.
That is the lesson this round actually taught.
...
module tree. That is an answer that looks exact and is not, which is worse than no answer.
`--build <id>` answers the same question exactly, and the error names it. The upstream ask that would
make this cheap is `@expo/fingerprint --git-ref`, already recorded in [[0010-agent-conventions]]
§Upstream asks.
...
into an error. The nullable `resolveEasCli` exists for exactly this, beside the throwing
...
reporting `stale` and making the reader run a second command to learn what to do about it. That is a
`git status` that tells you a file changed and sends you to another tool for the diff.
[[0004-smart-start-and-project-state]] §The impact headline is free, the explanation is not has the
cost argument and the measurements. The split was that `status` is the reflex you run constantly, and
...
- The report answers `class: null` where nothing was established, whether that is no recorded build,
  a v1 record holding only a hash, or no fingerprint CLI. Its `unknown`s are load-bearing everywhere
  else in it (`auth`, `device`, the bundler's readiness).
...
`status`. The directory keeps its name because it is named after the *question*, what a change
costs, and that question is what these modules answer whichever command asks it. `index.ts`,
...
ask" together on purpose, because it is *decorating* a report that is complete without either, so
both lead to the same follow-up ladder. `status` cannot. Its whole contract is that a reader knows
whether a missing answer was established or merely assumed, which is the same rule its `auth`,
`device` and `ready` lines already follow.

So `lookUpCachedBuildAsync` is the primitive, and it answers `{ state: 'found', build }`,
`{ state: 'none' }`, or `{ state: 'unknown', reason }`. `findCachedBuildAsync` is the two-state
...
- **A fresh `status --explain`** found the finished build and printed
  `eas build  ios: finished build (development-simulator, 2026-08-26) · android: none · npx eas build:download --build-id a8df367c…`. That includes the `none` for the platform that genuinely had no
...
- **The costs are about as measured.** 5.4 s for the two-platform fresh lookup, and 2.5 s for the
  cached run.

One caveat the same session turned up, and it bears on the dominance argument above.
`--platform ios` does **not** isolate the iOS app config. Adding `android.package` to `app.json`
moved the iOS hash from `665388c1…` to `d24f1e75…`, even though nothing iOS touched it. That does not
break the claim the cache relies on, because the whole-project hash still dominates, so an unchanged
project hash still implies unchanged per-platform hashes. It does mean the per-platform hashes
...
failure this command exists to catch. A tool that derived `safe` from `class` would report it
...
- **`null` is not `false`.** A report that could not see the policy has not established that an
  ...
  This is the same discipline as `bundle.ok: null` in [[0010-agent-conventions]] §The gate has to ask
  about the _project_.
...
One thing found while wiring it is worth recording, because it would have failed silently. **The Expo
...
- **A fingerprint change is not the same as "needs prebuild".** `.easignore`, `eas.json` and
  ...
  `needs-native-build`, because a new cloud build is genuinely required, and the `ChangeKind` and the
...
- **A v1 record answers "whether", not "what".** Stated above, and it is a caveat too.
...
payload is why, and losing either makes the gate unreadable.
...
unreleased member of each prefix family, and the exact-beats-prefix case. The full `runtimeVersion` ×
`fingerprintChanged` matrix, meaning every policy against `true`, `false` and `null`. The option
...
**E2E.** A stub `fingerprint` covering **both** commands, so the temp-file dance of
...
including one asserting that the full report still prints on the `20`. The v1 record and the v2 record
each get one, because backwards compatibility is a property of the file on disk. The `cli:impact`
event is asserted from the JSONL stream, and `--json` is fed to `JSON.parse` rather than matched as a
substring.

**Eval (tier 1/2):** "I added expo-camera, do I need a new build?", graded on `class` in `--json`
and on `ota.safe` rather than on wording.
