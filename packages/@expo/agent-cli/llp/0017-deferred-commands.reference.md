...
(`@expo/agent-cli dev --plan` and `src/dev/planConsent.ts`), the permission tiers, and the
...
At `moderate` and above, a run watched from a terminal stops after the plan is on screen and prints the command that runs it — `Run it: npx @expo/agent-cli doctor:fix --apply --tier moderate --yes`. It used to be the question `Run this <tier> reset?`; wave 41 converted it with every other prompt in the package, though nothing here ships in v1 [see [[0008-guardrails]] §Consent is a re-run, never a prompt]. The pattern is `src/dev/planConsent.ts`'s, including who never stops: `--yes`, `--json`, and every non-interactive run.

The trigger is the **tier**, not the time class `dev` uses. Everything in `safe` is regenerated by the next command, which is what the tier means, and stopping to be asked again about deleting `.expo/web/cache` is a stop people learn to skip past without reading.
