...
const UNLINKED_OUTPUT = [
  ...
  '',
  'Accounts you can create projects in: kudochien, expo, expo-services, kudo1',
...
describe(classifyEasDeployFailure, () => {
  ...
  // @ref llp/0007-deploy-and-headless.rfc.md §Cross-platform deploy — **F143.** `npx eas init` on
  ...
  it(`should quote the accounts the EAS CLI listed`, () => {
    expect(classifyEasDeployFailure(UNLINKED_OUTPUT)?.how).toContain(
      'kudochien, expo, expo-services, kudo1'
    );
  });
  ...
  it(`should fill the account in when the EAS CLI named exactly one`, () => {
    const cause = classifyEasDeployFailure(
      ['EAS project not configured.', 'Accounts you can create projects in: kudo1'].join('\n')
    );

    expect(cause?.command).toBe('npx eas init --account kudo1 --non-interactive');
  });
