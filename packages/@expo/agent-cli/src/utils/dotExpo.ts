...
- "agent-skill-links.json": remembers which coding agents were selected for the "@expo/agent-cli skills" command.
- "agent-cli-last-build.json": remembers the native fingerprint of the last successful "@expo/agent-cli dev" build per platform, so the next plan can skip an unnecessary rebuild.
- "agent-cli-eas-builds.json": remembers a finished EAS build that "@expo/agent-cli status --explain" found for this project's current fingerprint, so the next "@expo/agent-cli status" can report it without calling EAS again. The entry is dropped as soon as the project fingerprint changes.
- "agent-cli-fingerprint.json": remembers the native fingerprint of this project per platform, so a command that needs it does not spend a second hashing an unchanged project. Every entry records the size and modification time of the files it was computed against — the lockfiles, the app config, "eas.json", "package.json", the fingerprint's own settings and the assets they point at — and is only believed again while all of them are unchanged, and never for longer than ten minutes. It does not look inside "ios" or "android", so the ten minutes is what covers a native edit; "@expo/agent-cli dev" deletes the whole file after any step that changes the project. A report answered from here says so, with the check it used and the age of the answer. Delete it, or pass "--no-fingerprint-cache", to hash the project again.
- "agent-cli-dev-server.sock": a socket the "@expo/agent-cli start" and "@expo/agent-cli dev" commands listen on while they run a dev server, so other commands can ask which port it is on instead of scanning for it. It holds no data on disk: connecting to it is what answers. A leftover file from a stopped dev server answers nothing and is replaced by the next one.
- "dev/logs/": contains structured JSONL event logs from CLI commands (e.g. start.log, export.log). These are truncated on each run.
- "dev/logs/dev-detached.log": everything a dev server started with "@expo/agent-cli dev --detach" printed, since that run has no terminal to print it to. Read it with "@expo/agent-cli dev:logs". One file per project, truncated on each detached run, because a project has at most one detached dev server.
