...
export function generateAgentsMdBlock({ state, projectName, skillsDirs }: AgentsMdContext): string {
  return [
    ...
    '- `npx exagent runtime:tree` — what is on the screen of the running app, by testID',
    '- `npx exagent runtime:tap <testID>` — tap an element; `--verify` reports what changed',
    '- `npx exagent runtime:type <text> --testID <id>` — type into an input',
