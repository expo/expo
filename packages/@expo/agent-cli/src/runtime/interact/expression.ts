...
//
// Five properties this is built around, each measured by the spike (llp/0014 §What the implementer
// must not lose):
//
 ...
 * agent asked for. Bounded because the fiber depth of a real screen is around 150 (llp/0014
...
const PRELUDE = `
   ...
   * every \`Offscreen\` fiber still reports \`mode: "visible"\` (llp/0014 §What the walk sees).
   */
  function screenOf(f) {
   ...
   * on every single call (llp/0014 §Verdict 3).
   */
   ...
   * \`event.nativeEvent\` at all would throw on \`undefined\` (llp/0014 §Verdict 3).
   */
...
function callPrologue(prop: string, extraFields: string = ''): string {
  ...
  // control the user cannot use — a false pass an agent cannot see (llp/0014 §\`disabled\`).
