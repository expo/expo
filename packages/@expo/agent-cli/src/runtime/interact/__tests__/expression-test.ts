...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose 8. The one guard: a runtime with no
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose 2. The load-bearing case, replayed from
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose 1.
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose 3. `RectButton` puts `onPress` on six
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose — the one correctness bug the spike found.
describe('a disabled element', () => {
  const fixture = JSON.parse(fs.readFileSync(path.join(FIXTURES, 'out-15-disabled.json'), 'utf8'))
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose. Three screens are mounted at once, and a
...
describe('the focus filter', () => {
  ...
  // I could not tell" rather than an error or an empty tree (llp/0018-interaction-commands.rfc.md).
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose 6. The default is bounded by what it
...
describe('the projections', () => {
  ...
  // @ref llp/0018 §Eight shipped decisions — friction run 7, F74. `nodeCount: 42` beside four returned
...
// @ref llp/0018 §Eight shipped decisions — friction run 7, F63.
//
...
describe('the text of a node', () => {
  ...
  // @ref llp/0018 §Eight shipped decisions — friction run 7, F70. A placeholder is what the input says
...
// @ref llp/0018 §Eight shipped decisions — friction run 7, F69.
//
...
// @ref llp/0018-interaction-commands.rfc.md §Must not lose 4. React's numeric fiber tags are
...
describe('the type call', () => {
  ...
  // @ref llp/0018 §Eight shipped decisions — friction run 7, F80. Two `shared-id` Pressables,
