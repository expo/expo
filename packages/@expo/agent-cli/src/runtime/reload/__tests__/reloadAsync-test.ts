...
describe(reloadAsync, () => {
  ...
  // @ref llp/0010-agent-conventions.rfc.md §Other gates, in brief — friction run 5, F48-6. "0
...
describe(`${reloadAsync.name} and the platform it checks the bundle for`, () => {
  ...
  it(`asks the dev server for the platform the connected app is on`, async () => {
    ...
    const { socket } = fakeSocket(
      [{ 'socket#1': 'role=android' }, { 'socket#4': 'role=android' }],
...
describe('an app the command socket cannot see', () => {
  ...
  it(`still force-stops it when --method device says so`, async () => {
    ...
    await expect(reloadAsync(projectRoot, options({ json: true, method: 'device' }))).resolves.toBe(
      EXIT_OK
    );
...
describe('reloading an app on a cloud simulator session', () => {
  ...
  it(`takes the host from the manifest when no flag named a dev server`, async () => {
    writeCloudProject();
    mockDevServer([], { manifestOrigin: 'https://chx3ba8-alice-8303.exp.direct' });
    ...
    expect(JSON.parse(printed()).url).toBe('exp://chx3ba8-alice-8303.exp.direct/--/?');
  });
  ...
  // @ref llp/0005 §What proves a reload. Never one number for two lists: "Apps connected 1" off
  // `/json/list` while broadcasting into an empty peer set is the reading that cost the loop.
  ...
  // @ref llp/0005 §Cloud simulator. The refusal has to come **before** the
  ...
  // @ref llp/0005 §Cloud simulator — S14. The controller names the session holding the device,
...
//
// The rule this block pins (llp/0021 §The rules): `verifiedBy` may name only a
...
describe('the verification payload', () => {
  ...
  it('backs fresh-debugger-target with appsReconnected on the runtime rung', async () => {
    ...
    jest
      .spyOn(require('../../cdpClient'), 'CdpClient')
      .mockImplementation(
        () => ({ evaluateAsync: jest.fn(async () => ({ value: 'ready', type: 'string' })) }) as any
      );
    setTimeout(() => server.listing([RELOADED_EXPO_GO_TARGET]), 10).unref();
