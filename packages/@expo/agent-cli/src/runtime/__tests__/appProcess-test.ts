...
describe('what an Android stop can establish about the app it stopped', () => {
  ...
  it(`asks the device whether the app was running, and says it was`, async () => {
    const calls = mockSpawnSequence([{ stdout: '3933\n', exitCode: 0 }, { exitCode: 0 }]);
...
describe('stopping the app on a cloud simulator session', () => {
  ...
  it(`reports the verb as having run when the controller closed the app`, async () => {
    cloudProject();
    mockSpawn({
      stdout: '{"success":true,"data":{"session":"default","message":"Closed: default"}}',
    });
  ...
  // with what the tool printed in it (llp/0005 §Cloud simulator
  // backend).
  ...
  it(`quotes the controller's own refusal when it gave one`, async () => {
    ...
    await expect(stopAppOnDeviceAsync(cloudParams)).resolves.toMatchObject({
      ok: false,
      reason:
        "the session's controller answered SESSION_NOT_FOUND: No active session. Run open first.",
    });
  });
...
// `adb shell <words>` joins the words and hands the line to a shell on the device, so every value
// in one is quoted for that shell — the URL of an `am start` already was (`../../navigate/deepLink`).
describe('Quoting the application id for the device shell', () => {
  it('should quote the app id of an Android force-stop', () => {
    expect(
      buildStopAppCommand({
        platform: 'android',
        deviceId: 'emulator-5554',
        appId: 'com.x;toybox id;#',
      }).args
    ).toEqual(['-s', 'emulator-5554', 'shell', 'am', 'force-stop', `'com.x;toybox id;#'`]);
  });

  it('should not quote the app id of an iOS terminate, which starts no shell', () => {
    expect(
      buildStopAppCommand({ platform: 'ios', deviceId: 'UDID', appId: 'com.example.demo' }).args
    ).toEqual(['simctl', 'terminate', 'UDID', 'com.example.demo']);
  });

  it('should quote the app id of the Android liveness probe', async () => {
    mockSpawn({ stdout: '' });

    await stopAppOnDeviceAsync({
      platform: 'android',
      deviceId: 'emulator-5554',
      appId: 'com.x;toybox id;#',
    });

    const probe = jest.mocked(spawn).mock.calls[0]?.[1] as string[];
    expect(probe).toEqual(['-s', 'emulator-5554', 'shell', 'pidof', `'com.x;toybox id;#'`]);
  });
});
