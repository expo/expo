...
// The `Next:` rungs of `exagent runtime:network`, lifted out of `src/followups/runtime.ts`
...
export function buildRuntimeNetworkFollowUps({
  ...
  if (failedCount > 0) {
    return capFollowUps([
      ...
      },
      {
        ...
        command: `npx exagent runtime:network --duration ${durationMs}`,
  ...
  if (pendingCount > 0) {
    return capFollowUps([
      ...
      {
        id: 'runtime-network-rerun',
        command: `npx exagent runtime:network --duration ${durationMs * 2}`,
