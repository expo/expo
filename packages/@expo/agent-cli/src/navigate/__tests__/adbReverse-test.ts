...
//
// The tunnel host and the port forward compose by construction, and this is where that is pinned:
// a tunnelled dev server is already reachable from the device, and reversing its port would point
// the emulator at *itself*. The decision is read off the URL that is about to be opened, so a run
// whose URL carries a tunnel host never reverses however the dev server was found.
describe(`${loopbackPortOfUrl.name} — a tunnelled run`, () => {
  it.each([
    ['a ws tunnel host', 'exp://znakdiwe5j2n5o0.boltexpo.dev/--/?'],
    ['a signed tunnel host', 'exp://kudo-notes.on.staging.expo.app/--/notes'],
    [
      'a dev launcher URL carrying a tunnel origin',
      'myapp://expo-development-client/?url=https%3A%2F%2Fabc.boltexpo.dev',
    ],
  ])(`reverses nothing for %s`, (_name, url) => {
    expect(loopbackPortOfUrl(url)).toBeNull();
  });

  // The other half, unchanged: a link at this machine's loopback still needs the forward (F50).
  it(`still reverses the port of a loopback link`, () => {
    expect(loopbackPortOfUrl('exp://127.0.0.1:8081/--/?')).toBe(8081);
  });
});
