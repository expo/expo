...
// @ref llp/0018-interaction-commands.rfc.md §Follow-ups
//
...
export function buildTreeFollowUps({ nodes, testID, platform }: TreeFollowUpInput): FollowUp[] {
  ...
  const toTap = testID != null ? (target && tappable(target) ? target : undefined) : nodes.find(tappable);
  const toType = testID != null ? (target && typeable(target) ? target : undefined) : nodes.find(typeable);
