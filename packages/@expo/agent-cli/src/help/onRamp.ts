...
//
// A module of its own with no imports, for two reasons. **One string.** The topic is named in the
// top-level screen, at the foot of every command's help, and in the recoveries for the spellings a
// caller guesses instead — so a rename has to be one edit rather than a search, and the name went
// from `how-to` to `workflow` once already [confirmed — Kudo, 2026-08-28]. **No cycle.** The
// registry prints the pointer and the topic reads the registry's workflow data, so the constant
