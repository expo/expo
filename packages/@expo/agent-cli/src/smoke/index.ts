...
export const smokeHelp: CommandHelp = {
  ...
  notes: [
    ...
    `This run brings its own environment: it starts a dev server and boots a device when there is`,
    `none, and stops exactly what it started. Their two phases show only on the runs that did them.`,
    `What was already running is used and left alone, so start one yourself to keep it afterwards.`,
    `It does not build: a project whose development build is missing or stale is refused and`,
    `pointed at "${PROGRAM_PREFIX} dev". Expo Go projects need no build and bootstrap in full.`,
