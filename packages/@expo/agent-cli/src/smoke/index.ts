...
export const smokeHelp: CommandHelp = {
  ...
  notes: [
    ...
    `An app already running is reloaded first: it holds the bundle from before your edit. An`,
    `unproved reload is 22, never a pass. --no-reload reads the app where it is instead.`,
    `This run brings its own environment: it starts a dev server and boots a device when there`,
    `is none, and stops exactly what it started. A cold start is paid by it, not by --timeout.`,
    `It does not build, and boots the simulator that has the app: a machine with none is refused.`,
