...
export const smokeHelp: CommandHelp = {
  ...
  notes: [
    ...
    `Coming up is not reading: a first compile, a first launch and the wait for the app to answer`,
    `are paid by the run that caused them, so --timeout is left for the phases that read the app.`,
    `It does not build: a project needing a development build is pointed at "${PROGRAM_PREFIX} dev".`,
