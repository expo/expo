...
dialog answered and live in `llp/0022-live-tier.plan.md`. A suite that needed somebody at the screen would never run.
...
EXPO_STAGING=1 AGENT_CLI_LIVE_EAS_PROJECT=~/path/to/eas-app \
  pnpm test:live:eas                        # ~1 min, one web deployment
...
payload). The evidence and the design each one forced are in `llp/0022-live-tier.plan.md` §Limits
