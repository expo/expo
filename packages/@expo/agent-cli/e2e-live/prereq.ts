// @ref llp/0022-live-tier.plan.md §A suite that cannot run refuses
//
...
export function androidDeviceGate(): { gate: Gate; device: AndroidDevice | null } {
  ...
  if (attached.length > 0) {
    ...
    try {
      ...
      return {
        gate: missing(
          `"${adb} -s ${serial} shell pm list packages" could not run: ${error.message}`
        ),
        device: null,
      };
    }
...
export function devClientProjectGate(): {
  ...
  try {
    ...
    return {
      gate: missing(`${root} has no readable package.json: ${error.message}`),
      project: null,
    };
  }
...
export function androidDevBuildGate(
  project: DevClientProject | null,
  ...
  try {
    ...
    return missing(
      `"${device.adb} -s ${device.serial} shell pm list packages" could not run: ${error.message}`
    );
  }
...
/** Write the livecheck link into a copied fixture or a scaffolded app. */
export function writeLivecheckLink(projectRoot: string): void {
  const link = livecheckLink();
  const appJsonPath = path.join(projectRoot, 'app.json');
  const appJson = JSON.parse(fs.readFileSync(appJsonPath, 'utf8'));
  appJson.expo.owner = link.owner;
  appJson.expo.slug = link.slug;
  appJson.expo.extra = {
    ...appJson.expo.extra,
    eas: { projectId: link.projectId },
  };
