/* eslint-env jest */
// @ref llp/0022-live-tier.plan.md §live-android
...
describeAndroid('live-android: the loop on a real Android emulator', () => {
   ...
   * asserting a timing, which llp/0022 §What a live assertion may be rules out.
   */
  ...
  // @ref llp/0005 §Android — and `e2e/utils.ts`'s `no-debugger` socket, which
  ...
  it.each([
    ...
    expectExit(
      result,
      1,
      `${name} must refuse rather than answer from a runtime that cannot answer`
    );
    const report = parseJson(result);
  ...
  it('runtime:errors reports the window as untrusted and falls back to the dev server log', async () => {
    ...
    // carries the fact that watching established nothing (llp/0005 §runtime:errors
    // cannot answer).
    expectExit(result, 0);
    const report = parseJson(result);
  ...
  it('a gate with no window and no log exits 22, never 0', async () => {
    ...
    // it. @ref llp/0005 §runtime:errors.
  ...
  it('runtime:reload stops on the command-socket rung, verified by two facts and no CDP', async () => {
    ...
    // @ref llp/0005 §What proves a reload. The verification is honest without CDP because
  ...
  it('smoke --android passes what it can prove and refuses to decide the rest', async () => {
    ...
    // `runtime` phase cannot measure, and llp/0010 §Other gates, in brief
  ...
  // `/json/list` names no platform (llp/0005 §Android), and the
  ...
  it('runtime:stop is a success for an app that is already stopped, and says nothing ran', async () => {
    // llp/0010 §Other gates, in brief: the subject of this command is a *state*, so an agent that
