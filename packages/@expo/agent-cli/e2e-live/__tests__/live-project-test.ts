/* eslint-env jest */
// @ref llp/0022-live-tier.plan.md §live-project
// @ref llp/0022-live-tier.plan.md §Coverage matrix — the `open — stub-only, by choice`
...
describeLive('live-project', gate)(
  ...
  () => {
    ...
    describe('install', () => {
      ...
      it('says a package the runtime does not carry needs a build', async () => {
        const result = await runLiveAsync(
          run,
          projectRoot,
          ['install', 'react-native-mmkv', '--json'],
          {
            label: 'install-mmkv',
          }
        );
    ...
    describe('agents:setup', () => {
      ...
      it('refuses a block with no end marker rather than rewriting past it', async () => {
        ...
        fs.writeFileSync(
          filePath,
          good.replace('<!-- END EXPO AGENT CLI MANAGED BLOCK -->', '<!-- gone -->')
        );
    ...
    describe('skills', () => {
      ...
      it('prints the SKILL.md with skills:show, and refuses a name it does not have', async () => {
        ...
        const missing = await runLiveAsync(
          run,
          projectRoot,
          ['skills:show', 'expo-haptics', 'nope'],
          {
      ...
      it('cleans only the links it made', async () => {
        ...
        expectExit(result, 0);
        expect(parseJson(result).removed).toContain(
          path.join('.claude', 'skills', 'expo-clipboard')
        );
    ...
    describe('inspect:config-plugins', () => {
      it('reads the real introspected config, plugins and autolinked modules included', async () => {
        ...
        expectExit(result, 0);
        const payload = parseJson(result);
        expect(payload.source.command).toEqual([
          'expo',
          'config',
          '--type',
          'introspect',
        ...
        expect(
          payload.platforms.android.manifest.manifest['uses-permission'].length
        ).toBeGreaterThan(1);
    ...
    describe('start', () => {
      ...
      it('starts a real dev server and syncs the skills a few seconds later', async () => {
        ...
        // llp/0003 §Skills shipped from Expo modules: the sync runs a few seconds after the dev server is up, from
