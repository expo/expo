...
- `smoke` no longer trusts an installed Expo Go without checking its version. The release the project's SDK ships is compared exactly, the way `@expo/cli`'s `ExpoGoInstaller` does, and any other version — older, newer, or from another release line — is replaced with the right one rather than reported. The expected release comes from the `expo-go` CLI as a subprocess. Under `--no-start` nothing is installed and a wrong version blocks the pass instead. ([#49654](https://github.com/expo/expo/pull/49654) by [@kudo](https://github.com/kudo))
