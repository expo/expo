...
export function captureStackForServerLogs() {
  ...
  HMRClient.log = (level: string, data: unknown[]) => {
    ...
    const hasComponentStack = data.some(
      (item) =>
        (typeof item === 'string' && isComponentStack(item)) ||
        (hasStringKey(item, 'message') && isComponentStack(item.message))
    );
...
export function withoutANSIColorStyles(message: string): string {
