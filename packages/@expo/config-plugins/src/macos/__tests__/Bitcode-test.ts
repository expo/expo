...
describe(setBitcodeWithConfig, () => {
  ...
  it('defaults to not modifying the bitcode settings', async () => {
    setBitcodeEnabledForRoot({ macos: {} }, projectRoot, validateDefaultBitcode);
  });

  it('enables bitcode for everything', async () => {
    setBitcodeEnabledForRoot({ macos: { bitcode: true } }, projectRoot, (project) => {
  ...
  it('disables bitcode for everything', async () => {
    setBitcodeEnabledForRoot({ macos: { bitcode: false } }, projectRoot, (project) => {
      const configurations = getConfigurations(project);
      for (const [, configuration] of configurations) {
  ...
  it('enables bitcode on specific configuration', async () => {
    setBitcodeEnabledForRoot({ macos: { bitcode: 'Debug' } }, projectRoot, (project) => {
      const configurations = getConfigurations(project);
      for (const [, configuration] of configurations) {
  ...
  it('warns when enabling bitcode on an invalid configuration', async () => {
    setBitcodeEnabledForRoot({ macos: { bitcode: 'Bacon' } }, projectRoot, validateDefaultBitcode);
...
function setBitcodeEnabledForRoot(
  config: { macos?: { bitcode?: boolean | string } & any },
  projectRoot: string,
