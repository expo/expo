...
describe(getRunnableSchemesFromXcodeproj, () => {
  beforeAll(async () => {
    vol.fromJSON(
      {
        'macos/project.xcodeproj/project.pbxproj': fsReal.readFileSync(
  ...
  it(`parses for runnable schemes`, async () => {
    ...
    expect(schemes).toStrictEqual([
      { name: 'multitarget', osType: 'macOS', type: 'com.apple.product-type.application' },
      { name: 'shareextension', osType: 'macOS', type: 'com.apple.product-type.app-extension' },
    ]);
  });
});
...
describe(getApplicationTargetNameForSchemeAsync, () => {
  describe('single build action entry', () => {
    beforeAll(async () => {
      vol.fromJSON(
        {
          'macos/testproject.xcodeproj/xcshareddata/xcschemes/testproject.xcscheme':
