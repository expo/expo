...
describe('BundleIdentifier module', () => {
  describe(getBundleIdentifier, () => {
    ...
    it('returns the bundleIdentifier if provided', () => {
      expect(
        getBundleIdentifier({ ...baseExpoConfig, macos: { bundleIdentifier: 'com.example.xyz' } })
  ...
  describe(getBundleIdentifierFromPbxproj, () => {
    ...
    describe('multi-target project', () => {
      it('returns correct bundle identifier for the default build configuration (Release)', () => {
        vol.fromJSON(
          {
            'macos/testproject.xcodeproj/project.pbxproj': originalFs.readFileSync(
              path.join(__dirname, 'fixtures/project-multitarget.pbxproj'),
              'utf-8'
            ),
            'macos/testproject.xcodeproj/xcshareddata/xcschemes/multitarget.xcscheme':
  ...
  describe(setBundleIdentifierForPbxproj, () => {
    const projectRoot = '/testproject';
    const pbxProjPath = 'macos/testproject.xcodeproj/project.pbxproj';
    const otherPbxProjPath = 'macos/otherproject.xcodeproj/project.pbxproj';

    beforeEach(() => {
      vol.fromJSON(
        {
          [pbxProjPath]: rnFixture['macos/HelloWorld.xcodeproj/project.pbxproj'],
          [otherPbxProjPath]: rnFixture['macos/HelloWorld.xcodeproj/project.pbxproj'],
        },
        projectRoot
      );
    });
