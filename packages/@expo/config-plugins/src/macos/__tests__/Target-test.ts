...
describe(findApplicationTargetWithDependenciesAsync, () => {
  ...
  it('also reads dependency dependencies', async () => {
    vol.fromJSON(
      {
        'macos/easwatchtest.xcodeproj/project.pbxproj': originalFs.readFileSync(
          path.join(__dirname, 'fixtures/watch.pbxproj'),
          'utf-8'
        ),
        'macos/easwatchtest.xcodeproj/xcshareddata/xcschemes/easwatchtest.xcscheme':
  ...
  it('marks framework targets as non-signable', async () => {
    vol.fromJSON(
      {
        'macos/myapp.xcodeproj/project.pbxproj': originalFs.readFileSync(
          path.join(__dirname, 'fixtures/project-with-framework.pbxproj'),
          'utf-8'
        ),
        'macos/myapp.xcodeproj/xcshareddata/xcschemes/myapp.xcscheme': originalFs.readFileSync(
