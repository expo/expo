...
import { withInfoPlist, withAppDelegate } from '../plugins/ios-plugins';
...
const debug = require('debug')('expo:config-plugins:ios:scene-delegate') as typeof console.log;
 ...
 */
export const withSceneDelegate: ConfigPlugin = (config) => {
...
const withSceneDelegateAppDelegate: ConfigPlugin = (config) => {
  return withAppDelegate(config, (config) => {
    ...
    try {
      ...
      if (error.code === 'ERR_NO_MATCH') {
        throw new Error(
          `Cannot adopt the UIScene life cycle in ${fileName}: the app delegate doesn't ` +
            `match the expected template. Migrate it manually following Apple Technote TN3187.`
        );
      }
      throw error;
    }
 ...
 */
export function removeWindowStartup(src: string): MergeResults {
  ...
  const contents = src.replace(blockMatcher, (match, indent: string) => {
  ...
  return { contents, didMerge: true, didClear: false };
}
 ...
 */
export function addLaunchOptionsProperty(src: string): MergeResults {
 ...
 */
export function addSceneConfigurationMethod(src: string): MergeResults {
  const newSrc = [
