...
exports.createRunOncePlugin = exports.withRunOnce = void 0;
...
const withRunOnce = (config, { plugin, name, version }) => {
    ...
    if ((0, history_1.getHistoryItem)(config, name)) {
        return config;
    }
    // Push the history item so duplicates cannot be run.
    config = (0, history_1.addHistoryItem)(config, { name, version });
...
function createRunOncePlugin(plugin, name, version) {
    return (config, props) => {
        return (0, exports.withRunOnce)(config, { plugin: (config) => plugin(config, props), name, version });
    };
}
