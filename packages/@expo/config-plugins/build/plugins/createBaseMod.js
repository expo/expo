...
function createBaseMod({
  ...
  const withUnknown = (config, _props) => {
    let _props$skipEmptyMod, _props$saveToInternal;
    ...
    return (0, _withMod().withBaseMod)(config, {
      ...
      mod: modName,
      skipEmptyMod:
        (_props$skipEmptyMod = props.skipEmptyMod) !== null && _props$skipEmptyMod !== void 0
          ? _props$skipEmptyMod
          : true,
      saveToInternal:
        (_props$saveToInternal = props.saveToInternal) !== null && _props$saveToInternal !== void 0
          ? _props$saveToInternal
          : false,
      isProvider: true,
      isIntrospective,
      ...
      async action({ modRequest: { nextMod, ...modRequest }, ...config }) {
        try {
          ...
          results = await nextMod({ ...results, modResults, modRequest });
...
function assertModResults(results, platformName, modName) {
  ...
  if (
    !ensuredResults ||
    typeof ensuredResults !== 'object' ||
    !(ensuredResults !== null && ensuredResults !== void 0 && ensuredResults.mods)
  ) {
    throw new Error(
