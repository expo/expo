...
const withMaps = applePlatform => config => {
  ...
  config = withMapsCocoaPods(applePlatform)(config, {
    useGoogleMaps: !!apiKey
  });

  // Adds/Removes AppDelegate setup for Google Maps API on iOS
  config = withGoogleMapsAppDelegate(applePlatform)(config, {
    apiKey
  });
  return config;
};
...
const withMapsCocoaPods = applePlatform => (config, {
  useGoogleMaps
}) => {
  return (0, _applePlugins().withPodfile)(applePlatform)(config, async config => {
...
const withGoogleMapsAppDelegate = applePlatform => (config, {
  apiKey
}) => {
  return (0, _applePlugins().withAppDelegate)(applePlatform)(config, config => {
