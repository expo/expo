...
const {
  getProjectName
} = _index().IOSConfig.XcodeUtils;
const IMAGE_CACHE_NAME = 'icons';
const IMAGESET_PATH = 'Images.xcassets/AppIcon.appiconset';
const withIosIcons = config => {
  return (0, _index().withDangerousMod)(config, ['ios', async config => {
...
async function setIconsAsync(config, projectRoot) {
  ...
  if (!icon) {
    _index().WarningAggregator.addWarningIOS('icon', 'This is the image that your app uses on your home screen, you will need to configure it manually.');
    return;
  }
