...
let Config = exports.Config = /*#__PURE__*/function (Config) {
  ...
  Config["PATCH_SUPPORT"] = "expo.modules.updates.ENABLE_PATCH_SUPPORT";
  return Config;
...
async function setUpdatesConfigAsync(projectRoot, config, androidManifest, expoUpdatesPackageVersion) {
  ...
  const patchSupport = (0, _Updates().getUpdatesPatchSupportEnabled)(config);
  if (!patchSupport) {
    (0, _Manifest().addMetaDataItemToMainApplication)(mainApplication, Config.PATCH_SUPPORT, 'false');
  } else {
    (0, _Manifest().removeMetaDataItemFromMainApplication)(mainApplication, Config.PATCH_SUPPORT);
  }
