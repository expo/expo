...
function getAppDelegateHeaderFilePath(projectRoot) {
  const [using, ...extra] = (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/*/AppDelegate.h', {
    absolute: true,
    cwd: projectRoot,
    ignore: ignoredPaths
  }));
...
function getAppDelegateFilePath(projectRoot) {
  const [using, ...extra] = (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/*/AppDelegate.@(m|mm|swift)', {
    absolute: true,
    cwd: projectRoot,
    ignore: ignoredPaths
  }));
...
function getPodfilePath(projectRoot) {
  const [using, ...extra] = (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/Podfile', {
    absolute: true,
    cwd: projectRoot,
    ignore: ignoredPaths
  }));
...
function findSchemePaths(projectRoot) {
  return (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/*.xcodeproj/xcshareddata/xcschemes/*.xcscheme', {
    absolute: true,
    cwd: projectRoot,
    ignore: ignoredPaths
  }));
...
function getAllXcodeProjectPaths(projectRoot) {
  const iosFolder = 'ios';
  const pbxprojPaths = (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/**/*.xcodeproj', {
    cwd: projectRoot,
    ignore: ignoredPaths
  }))
  // drop leading slash from relative glob paths (since glob@9)
  .map(value => value.replace(/^\//, '')).filter(project => !/test|example|sample/i.test(project) || path().dirname(project) === iosFolder).sort((a, b) => {
...
function getAllInfoPlistPaths(projectRoot) {
  const paths = (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/*/Info.plist', {
    absolute: true,
    cwd: projectRoot,
    ignore: ignoredPaths
  }))
...
function getAllEntitlementsPaths(projectRoot) {
  return (0, _glob2().withSortedGlobResult)((0, _glob().sync)('ios/*/*.entitlements', {
    absolute: true,
    cwd: projectRoot,
    ignore: ignoredPaths
  }));
