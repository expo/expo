...
function getEntitlementsPath(projectRoot, platform, {
  ...
  const entitlementsPath = getEntitlementsPathFromBuildConfiguration(projectRoot, platform, xcBuildConfiguration);
...
function getEntitlementsPathFromBuildConfiguration(projectRoot, platform, xcBuildConfiguration) {
  ...
  if (entitlementsPathRaw) {
    return _path().default.normalize(_path().default.join(projectRoot, platform, (0, _string().trimQuotes)(entitlementsPathRaw)));
  } else {
    return null;
  }
}
function ensureApplicationTargetEntitlementsFileConfigured(projectRoot, platform) {
  const project = (0, _Xcodeproj().getPbxproj)(projectRoot, platform);
  ...
  for (const [, xcBuildConfiguration] of buildConfigurations) {
    const oldEntitlementPath = getEntitlementsPathFromBuildConfiguration(projectRoot, platform, xcBuildConfiguration);
    ...
    const entitlementsPath = _path().default.normalize(_path().default.join(projectRoot, platform, entitlementsRelativePath));
