...
const withRequiresFullScreen = (0, _iosPlugins().createInfoPlistPlugin)(
...
function getRequiresFullScreen(config) {
  ...
  if (
    ...
    _config$ios !== void 0 &&
    _config$ios.hasOwnProperty('requireFullScreen')
  ) {
    return !!config.ios.requireFullScreen;
  } else {
    // In SDK 43, the `requireFullScreen` default has been changed to false.
    if (
      (0, _versions().gteSdkVersion)(config, '43.0.0') // TODO: Uncomment after SDK 43 is released.
      // || !config.sdkVersion
    ) {
      return false;
    }
