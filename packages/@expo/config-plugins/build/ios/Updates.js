...
async function setUpdatesConfig(projectRoot, config, expoPlist, expoUpdatesPackageVersion) {
  const newExpoPlist = {
    ...expoPlist,
  ...
  return await setVersionsConfig(projectRoot, config, newExpoPlist);
}
async function setVersionsConfig(projectRoot, config, expoPlist) {
  const newExpoPlist = {
    ...expoPlist
  };
  const runtimeVersion = await (0, _Updates().getRuntimeVersionNullable)(projectRoot, config, 'ios');
...
async function isPlistConfigurationSynced(projectRoot, config, expoPlist) {
  return (0, _Updates().getUpdateUrl)(config) === expoPlist.EXUpdatesURL && (0, _Updates().getUpdatesEnabled)(config) === expoPlist.EXUpdatesEnabled && (0, _Updates().getUpdatesTimeout)(config) === expoPlist.EXUpdatesLaunchWaitMs && (0, _Updates().getUpdatesCheckOnLaunch)(config) === expoPlist.EXUpdatesCheckOnLaunch && (0, _Updates().getUpdatesCodeSigningCertificate)(projectRoot, config) === expoPlist.EXUpdatesCodeSigningCertificate && (0, _Updates().getUpdatesCodeSigningMetadata)(config) === expoPlist.EXUpdatesCodeSigningMetadata && (await isPlistVersionConfigurationSynced(projectRoot, config, expoPlist));
}
async function isPlistVersionConfigurationSynced(projectRoot, config, expoPlist) {
  var _expoPlist$EXUpdatesR, _expoPlist$EXUpdatesS;
  const expectedRuntimeVersion = await (0, _Updates().getRuntimeVersionNullable)(projectRoot, config, 'ios');
