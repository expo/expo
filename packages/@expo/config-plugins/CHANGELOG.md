...
- [Android] Mark the `expo_runtime_version` string resource as `translatable="false"`. Localization tooling — including Google Play's automatic app string translation, which rewrites strings server-side after upload — treated it as user-facing copy and emitted a locale-qualified copy of it. Android prefers the locale-qualified value, so devices in that locale reported a stale runtime version and silently stopped receiving updates. ([#PRNUMBER](https://github.com/expo/expo/pull/PRNUMBER) by [@ahmdshrif](https://github.com/ahmdshrif))
