...
it('is not manually edited', async () => {
  ...
  const expected = await fs.promises.readFile(path.join(__dirname, 'fixtures/ExpoConfig.backup.ts'), 'utf8');
  ...
  try {
    ...
    error.message = 'Expected src/ExpoConfig.ts and src/__tests__/fixtures/ExpoConfig.ts to match.\n'
