...
// WARN: This is a generated file that has to be updated by Expo staff!
// Do not edit this file manually. This file is created using the "pnpm generate" script from the XDL schema API endpoint.
// If you are submitting an XDL schema change, please open an issue instead
...
export interface Web {
  /**
   * Sets the export method for the web app for both `expo start` and `expo export`. `static` statically renders HTML files for every route in the `app/` directory, which is available only in Expo Router apps. `single` outputs a Single Page Application (SPA), with a single `index.html` in the output folder, and has no statically indexable HTML. `server` outputs a server bundle that renders HTML at request time and serves API routes. Requires deployment to a server. Defaults to `single`.
   */
