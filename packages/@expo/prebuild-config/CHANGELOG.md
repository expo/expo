...
- [inline modules] Write the on-disk iOS project name to `expo.inlineModules.xcodeProjectTargets` in Podfile.properties.json instead of deriving it from the app name, so it always matches the real main target.
