...
const withEdgeToEdge = config => {
  ...
  if (edgeToEdgeConfigPlugin === null) {
    ...
    config = withEdgeToEdgeEnabledGradleProperties(config, {
...
function constructWarning(pluginIndex, config) {
  ...
  if (hasEnabledEdgeToEdge(config) && config.edgeToEdgeEnabled === false) {
    return `You have configured the \`react-native-edge-to-edge\` plugin in your config file, while also setting the \`edgeToEdgeEnabled\` ` + `field to \`false\`. The value of \`edgeToEdgeEnabled\` field will be ignored`;
  }
  return null;
}
