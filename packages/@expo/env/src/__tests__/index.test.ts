...
describe(consumeConfigEnvMode, () => {
  it('returns undefined when no config mode was passed', () => {
    expect(consumeConfigEnvMode()).toBeUndefined();
  });
  ...
  it('rejects and removes an invalid __EXPO_CONFIG_MODE value', () => {
...
describe(loadProjectEnv, () => {
  it('uses the mode without changing NODE_ENV', () => {
    process.env.NODE_ENV = 'test';
    vol.fromJSON(
      {
...
describe(getOriginalEnv, () => {
  ...
  it('removes dotenv values inherited from a parent process', () => {
    ...
    expect(getOriginalEnv(inheritedEnv)).toEqual({ EAS_VALUE: 'explicit' });
  ...
  it('removes inherited dotenv values after a forced local load', () => {
    const inheritedEnv = {
      PARENT_VALUE: 'from-parent-dotenv',
      [LOADED_ENV_NAME]: JSON.stringify(['PARENT_VALUE']),
    };
    vol.fromJSON({ '.env': 'LOCAL_VALUE=from-current-dotenv' }, '/');
...
describe(getOriginalEnvValue, () => {
  ...
  it('returns undefined for a dotenv value inherited from a parent process', () => {
    ...
    expect(getOriginalEnvValue('DOTENV_VALUE', inheritedEnv)).toBeUndefined();
    expect(getOriginalEnvValue('EAS_VALUE', inheritedEnv)).toBe('explicit');
