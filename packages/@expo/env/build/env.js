...
exports.getFiles = exports.createControlledEnvironment = exports.isEnabled = void 0;
/**
...
function createControlledEnvironment() {
    ...
    function _getForce(projectRoot, options = {}) {
        ...
        dotenvFiles.forEach((dotenvFile) => {
            ...
            try {
                const results = (0, dotenv_expand_1.expand)(dotenv.config({
                    debug: IS_DEBUG,
    ...
    function load(projectRoot, options = {}) {
        ...
        if (!options.force) {
            ...
            if (keys.length) {
                console.log(chalk_1.default.gray('env: load', envInfo.files.map((file) => path.basename(file)).join(' ')));
                console.log(chalk_1.default.gray('env: export', keys.join(' ')));
            }
        }
...
function getFiles(mode, { silent = false } = {}) {
    ...
    if (!mode) {
        if (silent) {
            ...
            console.error(chalk_1.default.red('The NODE_ENV environment variable is required but was not specified. Ensure the project is bundled with Expo CLI or NODE_ENV is set.'));
            console.error(chalk_1.default.red('Proceeding without mode-specific .env'));
        }
    }
