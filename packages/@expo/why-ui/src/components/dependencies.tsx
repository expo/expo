...
import { BaconCode } from './code';
import { MetroJsonModule } from './data';
import { Link } from 'expo-router';

export function DependenciesList({ modules }: { modules: MetroJsonModule[] }) {
  ...
  const selected = null; // index != null ? deps[index] : null;

  return (
    <div className="bg-[#191A20] flex flex-1 flex-row absolute top-0 left-0 bottom-0 right-0">
      <div className="flex flex-1 flex-col" style={{ flex: 1, maxWidth: selected ? 200 : '100%' }}>
        <FlatList
          data={modules}
          renderItem={({ item, index }) => (
            <Link href={{ pathname: '/module/[id]', params: { id: item.path } }}>
              <div className="border-b border-b-[#ffffff1a] p-2 gap-2">
                <p className="text-slate-50 font-bold">{item.path}</p>
                <div className="flex flex-row gap-2">
                  <p className="text-slate-50 opacity-60">Size: {formatSize(item.size)}</p>
                  <p className="text-slate-50 opacity-60">Type: {item.output[0].type}</p>
                </div>
              </div>
            </Link>
          )}
      ...
      {selected && <DependencyInfo {...selected} />}
    </div>
  );
}
...
export function DependencyInfo({ getSource, inverseDependencies, output, path }: MetroJsonModule) {
  ...
  return (
    <div className="flex flex-1 bg-[#191A20] border-l border-l-[#ffffff1a]">
      <View style={{ flex: 1 }}>
        {/* Header */}
        <span className="flex p-3 gap-2 border-b border-b-[#ffffff1a] flex-row justify-between">
          {/* <FileIcon className="w-4 text-slate-50" /> */}
          <h3 className="text-slate-50 font-bold">{path}</h3>
          <h3
        ...
        <div className="flex relative">
          <div className="absolute top-0 left-0 bottom-0 right-0 flex flex-1 flex-row">
            {items.map((code, i) => (
              <div className="max-w-[50%] w-[50%] h-full">{code}</div>
            ))}
          </div>
        </div>
      </View>
