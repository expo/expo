...
function createJsInspectorMiddleware() {
    return async function (req, res, next) {
        const { origin, searchParams } = new url_1.URL(req.url ?? '/', getServerBase(req));
        ...
        if (!app) {
            ...
            console.warn(chalk_1.default.yellow('No compatible apps connected. JavaScript Debugging can only be used with the Hermes engine.'));
            return;
        }
        if (req.method === 'GET') {
...
function getServerBase(req) {
    ...
    const address = localAddress && net_1.default.isIPv6(localAddress) ? `[${localAddress}]` : localAddress;
