...
function serializeOpenGraph(tags: MetadataTag[], openGraph: Metadata['openGraph'] | undefined) {
  ...
  pushMetaContent(
    ...
    'property',
    'og:ttl',
    openGraph.ttl != null ? String(openGraph.ttl) : undefined
  );
  ...
  for (const video of normalizeArray<MetadataVideo>(openGraph.videos)) {
    ...
    pushMetaContent(
      ...
      'property',
      'og:video:width',
      video.width != null ? String(video.width) : undefined
    );
    pushMetaContent(
      tags,
      'property',
      'og:video:height',
      video.height != null ? String(video.height) : undefined
    );
  }

  for (const audio of normalizeArray<MetadataAudio>(openGraph.audio)) {
...
function serializeTwitter(tags: MetadataTag[], twitter: Metadata['twitter'] | undefined) {
  ...
  for (const player of normalizeArray(twitter.players)) {
    ...
    pushMetaContent(
      ...
      player.width != null ? String(player.width) : undefined
    );
    pushMetaContent(
      tags,
      'name',
      'twitter:player:height',
      player.height != null ? String(player.height) : undefined
    );
...
function serializeFormatDetection(
  tags: MetadataTag[],
  ...
  if (formatDetection.date != null) parts.push(`date=${formatDetection.date ? 'yes' : 'no'}`);
  ...
  if (formatDetection.url != null) parts.push(`url=${formatDetection.url ? 'yes' : 'no'}`);
...
function serializeVerification(
  ...
  const pushValues = (key: string, value: string | string[] | undefined) => {
    if (!value) return;
...
function serializeAppleWebApp(
  tags: MetadataTag[],
  appleWebApp: Metadata['appleWebApp'] | undefined
) {
  if (!appleWebApp) return;

  if (appleWebApp.capable !== false) {
    pushMetaContent(
      tags,
      'name',
    ...
    pushMetaContent(tags, 'name', 'mobile-web-app-capable', 'no');
  }
  ...
  if (appleWebApp.startupImage) {
    ...
    for (const image of images) {
      tags.push({
        ...
        attributes:
          typeof image === 'string'
            ? { rel: 'apple-touch-startup-image', href: image }
            : { rel: 'apple-touch-startup-image', href: image.url, media: image.media },
      });
    }
  }
}

function serializeItunes(tags: MetadataTag[], itunes: Metadata['itunes'] | undefined) {
  if (!itunes) return;
...
function serializeFacebook(tags: MetadataTag[], facebook: Metadata['facebook'] | undefined) {
  if (!facebook) return;
...
function serializePinterest(tags: MetadataTag[], pinterest: Metadata['pinterest'] | undefined) {
  if (!pinterest) return;
...
function serializeAppLinks(tags: MetadataTag[], appLinks: Metadata['appLinks'] | undefined) {
  if (!appLinks) return;
  ...
  if (appLinks.web) {
    ...
    if (appLinks.web.shouldFallback != null) {
      pushMetaContent(
        ...
        appLinks.web.shouldFallback ? 'true' : 'false'
      );
    }
  }
}

function pushLinkArray(tags: MetadataTag[], rel: string, urls: string[] | undefined) {
  if (!urls) return;
...
function serializeAuthors(tags: MetadataTag[], authors: Metadata['authors'] | undefined) {
  if (!authors) return;
...
export function serializeMetadataToTags(metadata: Metadata): MetadataTag[] {
  ...
  if (typeof metadata.robots === 'object' && metadata.robots.googleBot) {
    pushMetaContent(
      ...
      'name',
      'googlebot',
      serializeRobotsValue(metadata.robots.googleBot)
    );
  }
