import type { DynamicConvention, MiddlewareNode, RouteNode } from 'expo-router/src/Route';
 ...
 */
function hasUrlProtocolPrefix(href: string): boolean {
  return /^([\w\d_+.-]+:)?\/\//.test(href);
}

function isWellKnownUri(href: string): boolean {
