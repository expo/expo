 ...
 */
export function defaultCSSInterop(jsx, type, { ...props }, key, experimental = false) {
 ...
 */
const DevOnlyCSSInteropWrapper = React.forwardRef(function DevOnlyCSSInteropWrapper({ __component: Component, __styleKeys, __experimental, ...props }, ref) {
    ...
    props = classNameToStyle(props);
    // If the styles are dynamic, we need to wrap the component with the CSSInteropWrapper to handle style updates.
    return areStylesDynamic(props.style) ? (React.createElement(CSSInteropWrapper, { ...props, ref: ref, __component: Component, __styleKeys: __styleKeys, __skipCssInterop: true, __experimental: __experimental })) : (React.createElement(Component, { ...props, ref: ref, __skipCssInterop: true }));
});
/**
 ...
 */
const CSSInteropWrapper = React.forwardRef(function CSSInteropWrapper({ __component: Component, __styleKeys, __experimental: experimental, ...$props }, ref) {
     ...
     * to update the state.
     */
    const [$interopMeta, setInteropMeta] = useState(initialMeta);
    ...
    const variables = useMemo(() => Object.assign({}, inheritedVariables, $interopMeta.variables), [inheritedVariables, $interopMeta.variables]);
    const containers = useMemo(() => Object.assign({}, inheritedContainers, $interopMeta.containers), [inheritedContainers, $interopMeta.containers]);
    ...
    if ($interopMeta.hasInlineVariables) {
        children = React.createElement(VariableContext.Provider, { value: variables }, children);
    }
    if ($interopMeta.hasInlineContainers) {
        children = React.createElement(ContainerContext.Provider, { value: containers }, children);
    }
    if (experimental && $interopMeta.animationInteropKey) {
        return (React.createElement(AnimationInterop, { ...props, ref: ref, key: $interopMeta.animationInteropKey, __component: Component, __variables: variables, __containers: inheritedContainers, __interaction: interaction, __interopMeta: $interopMeta, __skipCssInterop: true }, children));
    }
    else {
        return (React.createElement(Component, { ...props, ref: ref, __skipCssInterop: true }, children));
    }
});
/**
