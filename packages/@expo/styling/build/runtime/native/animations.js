 ...
 */
export const AnimationInterop = forwardRef(function Animated({ __component: Component, __propEntries, __interaction: interaction, __variables, __containers, __interopMeta: interopMeta, ...props }, ref) {
...
function useIsLayoutReady(interopMeta, interaction) {
    const [layoutReady, setLayoutReady] = useState(interopMeta.requiresLayout ? interaction.layout.width.get() !== 0 : true);
    useEffect(() => {
        if (layoutReady) {
            return undefined;
        }
...
function useAnimationAndTransitions(style, variables, interaction, isLayoutReady) {
    const { animations: { name: animationNames = emptyArray, duration: animationDurations = emptyArray, iterationCount: animationIterationCounts = emptyArray, } = {}, transition: { property: transitions = emptyArray, duration: transitionDurations = emptyArray, } = {}, } = styleMetaMap.get(style) || {};
