...
- Skip the `default` key in the `_interopNamespace` live-bindings helper, so a namespace import of a CommonJS module with an own `default` export no longer throws `Cannot assign to property 'default' which has only a getter`
