...
- Fix `resolver.useWatchman: true` never re-enabling Watchman: neutralize Metro's `useWatchman` default before merging the user config instead of rewriting the merged value, so an explicit opt-in survives while the Node watcher stays the default. ([#47662](https://github.com/expo/expo/issues/47662) by [@isaka1022](https://github.com/isaka1022))
