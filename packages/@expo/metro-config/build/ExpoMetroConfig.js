...
function patchMetroTransformToSupportCSS() {
    ...
    if (!original_transformFile.__patched) {
        ...
        Transformer.prototype.transformFile = function (filePath, transformOptions, fileBuffer) {
            this.dependencies.forEach((dependency) => {
                // Find any dependencies that have been marked as `skipCache` and ensure they are invalidated.
                // `skipCache` is set when a CSS module is found by PostCSS.
                if (dependency.output.find((file) => file.data.css?.skipCache) &&
    ...
    metro.getBundler().getBundler().transformFile = async function (filePath, transformOptions, fileBuffer) {
        return originalTransformFile(filePath, pruneCustomTransformOptions(filePath, 
