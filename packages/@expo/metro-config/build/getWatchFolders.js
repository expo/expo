...
function readJsonFile(filePath) {
  // Read with fs
  const file = _fs().default.readFileSync(filePath, 'utf8'); // Parse with JSON.parse
...
function resolveAllWorkspacePackageJsonPaths(workspaceProjectRoot) {
  try {
    ...
    const rootPackageJson = readJsonFile(rootPackageJsonFilePath); // Extract the "packages" array or use "workspaces" as packages array (yarn workspaces spec).
