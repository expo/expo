...
describe(getRewriteRequestUrl, () => {
  ...
  it(`rewrites expo request paths to entry point`, () => {
    ...
    expect(
      ...
      '/index.bundle?platform=ios&dev=true&minify=false&modulesOnly=false&runModule=true&app=com.bacon.test-custom-entry&transform.routerRoot=app&transform.engine=hermes'
    );
  });
  ...
  it(`rewrites expo request paths to entry point with custom expo router entry`, () => {
    ...
    expect(rewrite('/.expo/.virtual-metro-entry.bundle?platform=ios')).toBe(
      '/index.bundle?platform=ios&transform.routerRoot=foobar&transform.engine=hermes'
    );
  });
  ...
  it('rewrites expo request urls to entry point, with host and port', () => {
    ...
    expect(
      ...
      'http://127.0.0.1:8081/index.bundle?platform=ios&dev=true&minify=false&modulesOnly=false&runModule=true&app=com.bacon.test-custom-entry&transform.routerRoot=app&transform.engine=hermes'
    );
  });
});
