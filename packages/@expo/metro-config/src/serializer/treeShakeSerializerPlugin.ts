...
function getExportsThatAreNotUsedInModule(ast: types.File) {
  const exportedIdentifiers = new Map<string, string[]>();
  const usedIdentifiers = new Set<string>();
  const unusedExports = new Map<string, string[]>();

  const recordIdentifier = (local: string, exported: string) => {
  ...
  traverse(ast, {
    Declaration(path) {
      ...
      if (types.isVariableDeclaration(declaration)) {
        ...
        } else if (types.isExportDefaultDeclaration(path.parent)) {
          recordIdentifier(declaration.id.name, 'default');
        }
      }
    },
    ExportSpecifier(path) {
      const exportedIdentifier = types.isIdentifier(path.node.exported)
        ? path.node.exported.name
        : path.node.exported.value;
      recordIdentifier(path.node.local.name, exportedIdentifier);
    },
    ReferencedIdentifier(path) {
      if (types.isExportSpecifier(path.parent)) {
        const exportedIdentifier = types.isIdentifier(path.parent.exported)
          ? path.parent.exported.name
          : path.parent.exported.value;
        recordIdentifier(path.parent.local.name, exportedIdentifier);
      } else if (types.isExportDefaultSpecifier(path.parent)) {
      ...
      traverse(ast, {
        ...
        VariableDeclarator: {
          enter(path) {
            ...
            if (isModuleScope && types.isIdentifier(path.node.id)) {
