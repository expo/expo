...
export abstract class BasePackageManager implements PackageManager {
  ...
  async versionAsync() {
    return await this.executeAsync(['--version']).then(({ stdout }) => stdout.trim());
  }

  async getConfigAsync(key: string) {
    return await this.executeAsync(['config', 'get', key]).then(({ stdout }) => stdout.trim());
  }
  ...
  installAsync(flags: string[] = []): SpawnPromise<SpawnResult> | PendingSpawnPromise<SpawnResult> {
    return this.executeAsync(['install', ...flags]);
  }
