...
describe('installAsync', () => {
  it(`does pod repo update automatically when the Podfile.lock is malformed`, async () => {
    ...
    manager.runAsync = jest.fn((commands: string[]) => {
    ...
    expect(manager.runAsync).toHaveBeenNthCalledWith(1, ['install']);
    expect(manager.runAsync).toHaveBeenNthCalledWith(2, ['update', 'EXFileSystem']);
    expect(manager.runAsync).toHaveBeenNthCalledWith(3, ['install', '--repo-update']);
    expect(manager.runAsync).toBeCalledTimes(3);
  });
  ...
  it(`auto updates malformed package versions`, async () => {
    ...
    expect(manager.runAsync).toHaveBeenNthCalledWith(1, ['install']);
    expect(manager.runAsync).toHaveBeenNthCalledWith(2, ['update', 'EXFileSystem']);
    expect(manager.runAsync).toBeCalledTimes(2);
  });
  ...
  it(`runs install as expected`, async () => {
    ...
    // `pod install` > success
    expect(manager.runAsync).toHaveBeenNthCalledWith(1, ['install']);
    expect(manager.runAsync).toBeCalledTimes(1);
  });
});
