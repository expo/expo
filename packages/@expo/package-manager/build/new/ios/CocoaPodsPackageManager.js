...
const spawnSudo_1 = require("../utils/spawnSudo");
...
class CocoaPodsPackageManager {
    ...
    static async gemInstallCLIAsync(nonInteractive = false, spawnOptions = { stdio: 'inherit' }) {
        ...
        try {
            ...
            await (0, spawnSudo_1.spawnSudoAsync)(['gem', ...options], spawnOptions);
        }
    }
    ...
    async installAsync(flags = [], { spinner } = {}) {
    ...
    async runInstallTypeCommandAsync(command, { formatWarning, ...props } = {}) {
        try {
            return await this.runAsync(command);
        }
        catch (error) {
            if (formatWarning) {
    ...
    async podRepoUpdateAsync() {
        try {
            await this.runAsync(['repo', 'update']);
        }
        catch (error) {
