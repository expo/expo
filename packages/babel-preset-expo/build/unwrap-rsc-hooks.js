...
function effectHasSideEffectDeps(call) {
    if (call.arguments.length !== 2) {
        return false;
    }
    // We can't optimize if the effect has a function call as a dependency:
    // useEffect(() => {}, x())
    if (core_1.types.isCallExpression(call.arguments[1])) {
        return true;
    }
    // As well as:
    // useEffect(() => {}, [x()])
    if (core_1.types.isArrayExpression(call.arguments[1])) {
        for (const elem of call.arguments[1].elements) {
            if (core_1.types.isCallExpression(elem)) {
                return true;
            }
        }
    }
    ...
    return {
        ...
        visitor: {
            ...
            CallExpression(path, state) {
                ...
                if (t.isIdentifier(node.callee, { name: 'useMemo' }) ||
            ...
            VariableDeclarator(path, state) {
                ...
                if (t.isArrayPattern(node.id) && node.id.elements.length === 2) {
                    if (state.useStateIdent &&
                        ...
                        if (t.isLiteral(argExpr) ||
                            t.isObjectExpression(argExpr) ||
                            t.isArrayExpression(argExpr)) {
                            path.replaceWith(t.variableDeclarator(node.id, t.arrayExpression([
                                argExpr,
...
exports.unwrapRscHooks = unwrapRscHooks;
