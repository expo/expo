...
describe('forbidden server imports', () => {
  describe('react server mode', () => {
    ...
    it(`asserts importing client-side modules in server components`, () => {
      ...
      ).toThrowMatchingSnapshot();
      expect(() => runServerPass(`require("client-only")`)).toThrow();
      expect(() => runServerPass(`require.resolveWeak("client-only")`)).toThrow();
      expect(() => runServerPass(`require.importAll("client-only")`)).toThrow();
      expect(() => runServerPass(`require.importDefault("client-only")`)).toThrow();
      expect(() => runServerPass(`export * from "client-only"`)).toThrow();
      expect(() => runServerPass(`export { default } from "client-only"`)).toThrow();
    });
    ...
    it(`does not assert importing server-side modules in server components (react server mode)`, () => {
      expect(() => runServerPass(`import {} from 'server-only';`)).not.toThrow();
      expect(() => runServerPass(`require("server-only")`)).not.toThrow();
      expect(() => runServerPass(`require.resolveWeak("server-only")`)).not.toThrow();
      expect(() => runServerPass(`require.importAll("server-only")`)).not.toThrow();
      expect(() => runServerPass(`require.importDefault("server-only")`)).not.toThrow();
      expect(() => runServerPass(`export * from "server-only"`)).not.toThrow();
      expect(() => runServerPass(`export { default } from "server-only"`)).not.toThrow();
    });
  });
  describe('client mode (default)', () => {
    ...
    it(`asserts importing server-side modules in client components`, () => {
      ...
      expect(() => runServerPass(`require("server-only")`)).toThrow();
      expect(() => runServerPass(`require.resolveWeak("server-only")`)).toThrow();
      expect(() => runServerPass(`require.importAll("server-only")`)).toThrow();
      expect(() => runServerPass(`require.importDefault("server-only")`)).toThrow();
      expect(() => runServerPass(`export * from "server-only"`)).toThrow();
      expect(() => runServerPass(`export { default } from "server-only"`)).toThrow();
    });

    it(`does not assert importing server-side modules in server components (react server mode)`, () => {
      expect(() => runServerPass(`import {} from 'client-only';`)).not.toThrow();
      expect(() => runServerPass(`require("client-only")`)).not.toThrow();
      expect(() => runServerPass(`require.resolveWeak("client-only")`)).not.toThrow();
      expect(() => runServerPass(`require.importAll("client-only")`)).not.toThrow();
      expect(() => runServerPass(`require.importDefault("client-only")`)).not.toThrow();
      expect(() => runServerPass(`export * from "client-only"`)).not.toThrow();
      expect(() => runServerPass(`export { default } from "client-only"`)).not.toThrow();
    });
  });
});
