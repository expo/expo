...
- Expo Go accepts launcher commands as reserved `__expo_*` query params on launch URLs (`__expo_url`, `__expo_disable_onboarding`, `__expo_disable_fab`, `__expo_disable_auto_launch`) and removes them before the URL becomes the project URL. `__expo_disable_fab` and `__expo_disable_auto_launch` update the saved dev menu settings.
