...
public class ExponentFileSystem extends ReactContextBaseJavaModule {
  ...
  public ExponentFileSystem(ReactApplicationContext reactContext, JSONObject manifest) {
    super(reactContext);

    try {
      String experienceId = manifest.getString(ExponentManifest.MANIFEST_ID_KEY);
      mRootPath = reactContext.getFilesDir() + "/ExperienceData/" +
          URLEncoder.encode(experienceId, "UTF-8");
