      ...
      faceDetector?.process(image)
        ?.addOnCompleteListener(
          faceDetectionHandler(
            { face ->
              var result = FaceDetectorUtils.serializeFace(face, scaleX, scaleY)
              if (mirrored) {
                result = if (rotation == 270 || rotation == 90) {
                  FaceDetectorUtils.rotateFaceX(result, height, scaleX)
                } else {
                  FaceDetectorUtils.rotateFaceX(result, width, scaleX)
                }
              }
              result
            },
