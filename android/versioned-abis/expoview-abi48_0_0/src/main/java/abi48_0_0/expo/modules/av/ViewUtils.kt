package abi48_0_0.expo.modules.av
...
import abi48_0_0.expo.modules.av.video.VideoView
import abi48_0_0.expo.modules.av.video.VideoViewWrapper
...
object ViewUtils {
  ...
  @UiThread
  private fun tryRunWithVideoViewOnUiThread(moduleRegistry: ModuleRegistry, viewTag: Int, callback: VideoViewCallback, promise: abi48_0_0.expo.modules.kotlin.Promise) {
    try {
      val videoWrapperView = moduleRegistry.getModule(UIManager::class.java).resolveView(viewTag) as VideoViewWrapper?
      val videoView = videoWrapperView?.videoViewInstance
      if (videoView != null) {
  ...
  @AnyThread
  @Deprecated("Use `dispatchCommands` in favor of finding view with imperative calls")
  fun tryRunWithVideoView(moduleRegistry: ModuleRegistry, viewTag: Int, callback: VideoViewCallback, promise: abi48_0_0.expo.modules.kotlin.Promise) {
    if (UiThreadUtil.isOnUiThread()) {
      tryRunWithVideoViewOnUiThread(moduleRegistry, viewTag, callback, promise)
    } else {
      UiThreadUtil.runOnUiThread {
