package abi17_0_0.host.exp.exponent.modules.api.av.video;
...
import abi17_0_0.com.facebook.react.uimanager.events.RCTEventEmitter;
import com.yqritc.scalablevideoview.ScalableType;
import com.yqritc.scalablevideoview.ScaleManager;
import com.yqritc.scalablevideoview.Size;

import host.exp.expoview.Exponent;
import abi17_0_0.host.exp.exponent.modules.api.av.AVModule;
import abi17_0_0.host.exp.exponent.modules.api.av.AudioEventHandler;
import abi17_0_0.host.exp.exponent.modules.api.av.PlayerData;
import abi17_0_0.host.exp.exponent.modules.api.av.video.VideoViewManager.Events;
...
public class VideoView extends TextureView implements
  ...
  private final PlayerData.PlayerDataStatusUpdateListener mStatusUpdateListener = new PlayerData.PlayerDataStatusUpdateListener() {
    @Override
  ...
  private void updateMediaControllerIfNecessary() {
  ...
  public void setUri(final Uri uri, final ReadableMap initialStatus, final Promise promise) {
    ...
    mPlayerData.setVideoSizeUpdateListener(new PlayerData.PlayerDataVideoSizeUpdateListener() {
      @Override
  ...
  public void start() {
    final WritableMap map = Arguments.createMap();
    map.putBoolean(PlayerData.PLAYER_DATA_STATUS_SHOULD_PLAY_KEY_PATH, true);
    setStatus(map, null);
  }
  ...
  public void pause() {
    final WritableMap map = Arguments.createMap();
    map.putBoolean(PlayerData.PLAYER_DATA_STATUS_SHOULD_PLAY_KEY_PATH, false);
    setStatus(map, null);
  }

  @Override
  public int getDuration() {
    final ReadableMap status = getStatus();
    return status.getBoolean(PlayerData.PLAYER_DATA_STATUS_IS_LOADED_KEY_PATH)
        ? status.getInt(PlayerData.PLAYER_DATA_STATUS_DURATION_MILLIS_KEY_PATH) : 0;
  }
  ...
  public int getCurrentPosition() {
    final ReadableMap status = getStatus();
    return status.getBoolean(PlayerData.PLAYER_DATA_STATUS_IS_LOADED_KEY_PATH)
        ? status.getInt(PlayerData.PLAYER_DATA_STATUS_POSITION_MILLIS_KEY_PATH) : 0;
  }

  @Override
  public void seekTo(final int msec) {
    final WritableMap map = Arguments.createMap();
    map.putDouble(PlayerData.PLAYER_DATA_STATUS_POSITION_MILLIS_KEY_PATH, msec);
    setStatus(map, null);
  }
  ...
  public boolean isPlaying() {
    final ReadableMap status = getStatus();
    return status.getBoolean(PlayerData.PLAYER_DATA_STATUS_IS_LOADED_KEY_PATH)
        && status.getBoolean(PlayerData.PLAYER_DATA_STATUS_IS_PLAYING_KEY_PATH);
  }

  @Override
  public int getBufferPercentage() {
    final ReadableMap status = getStatus();
    if (status.getBoolean(PlayerData.PLAYER_DATA_STATUS_IS_LOADED_KEY_PATH)) {
      final double duration = status.getInt(PlayerData.PLAYER_DATA_STATUS_DURATION_MILLIS_KEY_PATH);
      final double playableDuration = status.getInt(PlayerData.PLAYER_DATA_STATUS_PLAYABLE_DURATION_MILLIS_KEY_PATH);
