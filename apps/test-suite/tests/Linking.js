    ...
    if (!Constants.isDevice) {
      t.describe('addListener', () => {
        ...
        if (Platform.OS === 'android') {
          ...
          t.it(
            'listener gets called with a proper URL when opened from a web browser',
            async () => {
              let handlerCalled = false;
              const handler = ({ url }) => {
                t.expect(url).toEqual(
                  Linking.makeUrl('++message=Redirected automatically by timer')
                );
                handlerCalled = true;
              };
              Linking.addEventListener('url', handler);
              // We cannot run this in device farm as it will invoke an "Open With" dialog for choosing a default browser
        ...
        t.it(
          'listener gets called with a proper URL when opened with Linking.openURL',
          async () => {
            let handlerCalled = false;
            const handler = ({ url }) => {
              handlerCalled = true;
