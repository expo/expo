...
export function test({ describe, expect, it, ...t }: JasmineInterface) {
  describe('Transportation tests', () => {
    ...
    it('should support plaintext messages', async () => {
      ...
      const client = requireNotNull(context.client, 'context.client');
...
function setupContext(
  ...
  t.afterAll(async () => {
    await requireNotNull(context.browserEchoClient, 'context.browserEchoClient').closeAsync();
    await requireNotNull(context.client, 'context.client').closeAsync();
  });
}
