...
const ASCII_SUPERSETS = THE_ENCODING.concat(LEGACY_ENCODINGS)
      .filter(function(e) {
...
export function test({ describe, it, expect }) {
  describe('TextDecoder', () => {
    ...
    it('Byte-order marks', () => {
      ...
      expect(new TextDecoder('utf-16le').decode(new Uint8Array([...utf16le_bom, ...utf16le]))).toBe(string);
      expect(new TextDecoder('utf-16be').decode(new Uint8Array([...utf16be_bom, ...utf16be]))).toBe(string);
    
      // mismatching BOMs
      expect(new TextDecoder('utf-8').decode(new Uint8Array([...utf16le_bom, ...utf8]))).not.toBe(string);
      expect(new TextDecoder('utf-8').decode(new Uint8Array([...utf16be_bom, ...utf8]))).not.toBe(string);
      
      expect(new TextDecoder('utf-16le').decode(new Uint8Array([...utf8_bom, ...utf16le]))).not.toBe(string);
      expect(new TextDecoder('utf-16le').decode(new Uint8Array([...utf16be_bom, ...utf16le]))).not.toBe(string);
      
      expect(new TextDecoder('utf-16be').decode(new Uint8Array([...utf8_bom, ...utf16be]))).not.toBe(string);
      expect(new TextDecoder('utf-16be').decode(new Uint8Array([...utf16le_bom, ...utf16be]))).not.toBe(string);

      // ignore BOMs
      expect(new TextDecoder('utf-8', {ignoreBOM: true}).decode(new Uint8Array([...utf8_bom, ...utf8]))).toBe('\uFEFF' + string);
      expect(new TextDecoder('utf-16le', {ignoreBOM: true}).decode(new Uint8Array([...utf16le_bom, ...utf16le]))).toBe('\uFEFF' + string);
      expect(new TextDecoder('utf-16be', {ignoreBOM: true}).decode(new Uint8Array([...utf16be_bom, ...utf16be]))).toBe('\uFEFF' + string);
    ...
    describe('ArrayBuffer, ArrayBufferView and buffer offsets', () => {
      it('should decode correctly from various buffer views', () => {
        ...
        types.forEach(typeName => {
          ...
          const subset = new TypeConstructor(buffer, TypeConstructor.BYTES_PER_ELEMENT, 8 / TypeConstructor.BYTES_PER_ELEMENT);
          expect(decoder.decode(subset)).toBe(chars.substring(TypeConstructor.BYTES_PER_ELEMENT, TypeConstructor.BYTES_PER_ELEMENT + 8));
        });
      });
    });
    ...
    it('GB 18030 2000 vs 2005: U+1E3F, U+E7C7 (decoding)', () => {
      const result = new TextDecoder('gb18030').decode(new Uint8Array([0xA8, 0xBC, 0x81, 0x35, 0xF4, 0x37]));
