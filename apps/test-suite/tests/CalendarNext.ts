    ...
    describeWithUiTest('Calendar UI Integration', () => {
      ...
      t.it('creates an event via UI', async () => {
        ...
        if (Platform.OS === 'ios') {
          ...
          const storedEvent = ExpoCalendarEvent.get(requireNotNull(result.id, 'result.id'));
      ...
      t.describe('Reminder', () => {
        t.describe('Reminder.get()', () => {
          ...
          t.it('returns a reminder by its ID', async () => {
            const fetchedReminder = await ExpoCalendarReminder.get(
              requireNotNull(reminder.id, 'reminder.id')
            );
        ...
        t.describe('Reminder.update()', () => {
          ...
          t.it('updates the listed reminder', async () => {
            ...
            const newTitle = `New title ${new Date().toISOString()}`;
            await requireNotNull(found, 'found').update({
              title: newTitle,
