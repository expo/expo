  ...
  describe('AES Crypto', () => {
    describe('EncryptionKey', () => {
      ...
      it('generates a 128-bit key when specified', async () => {
        const key = await AESEncryptionKey.generate(AESKeySize.AES128);
        expect(key).toBeDefined();
      ...
      if (Platform.OS !== 'web') {
        it('generates a 192-bit key when specified', async () => {
          const key = await AESEncryptionKey.generate(AESKeySize.AES192);
          expect(key).toBeDefined();
      ...
      it('imports a key from Uint8Array - 128 bit', async () => {
        const key = await AESEncryptionKey.import(test128BitKey);
        expect(key).toBeDefined();
        expect(key.size).toBe(128);
      });
      ...
      if (Platform.OS !== 'web') {
        it('imports a key from Uint8Array - 192 bit', async () => {
          const key = await AESEncryptionKey.import(test192BitKey);
          expect(key).toBeDefined();
          expect(key.size).toBe(192);
        });
      }
      ...
      it('imports a key from Uint8Array - 256 bit', async () => {
        const key = await AESEncryptionKey.import(test256BitKey);
        expect(key).toBeDefined();
        expect(key.size).toBe(256);
      });
      ...
      it('imports a key from hex string', async () => {
        ...
        const key = await AESEncryptionKey.import(hexKey, 'hex');
        expect(key).toBeDefined();
        expect(key.size).toBe(256);
      });
      ...
      it('imports a key from base64 string', async () => {
        ...
        const key = await AESEncryptionKey.import(base64Key, 'base64');
        expect(key).toBeDefined();
        expect(key.size).toBe(256);
      });
      ...
      it('throws error for invalid imported key size', async () => {
        ...
        let error = null;
        try {
          await AESEncryptionKey.import(invalidKey);
        } catch (e) {
          error = e;
        }
      ...
      it('exports key as bytes', async () => {
        ...
        const key = await AESEncryptionKey.import(originalBytes);
    ...
    describe('encryptAsync() and decryptAsync()', () => {
      ...
      it('encrypts and decrypts string data', async () => {
        ...
        const decrypted = await aesDecryptAsync(sealedData, key, { output: 'base64' });
      ...
      it('fails decryption with wrong AAD', async () => {
        ...
        let error = null;
        try {
          await aesDecryptAsync(sealedData, key, {
      ...
      it('fails decryption with wrong key', async () => {
        const plaintext = new Uint8Array([1, 2, 3, 4]);
        const wrongKey = await AESEncryptionKey.generate();

        const sealedData = await aesEncryptAsync(plaintext, key);

        let error = null;
        try {
          await aesDecryptAsync(sealedData, wrongKey);
        } catch (e) {
          error = e;
        }
        expect(error).not.toBeNull();
    ...
    describe('SealedData', () => {
      ...
      it('creates from combined format', async () => {
        const combined = await sealedData.combined();
        const reconstructed = AESSealedData.fromCombined(combined);
      ...
      it('creates from combined format with custom config', async () => {
        ...
        const plaintext = new Uint8Array([1, 2, 3, 4, 5]);
        const customSealedData = await aesEncryptAsync(plaintext, key, {
          nonce: { length: customIvLength },
    ...
    describe('Key size compatibility', () => {
      it('works with all supported key sizes', async () => {
        const plaintext = new Uint8Array([1, 2, 3, 4, 5]);
        const keySizes = [AESKeySize.AES128, AESKeySize.AES192, AESKeySize.AES256];

        for (const keySize of keySizes) {
          if (keySize === AESKeySize.AES192 && Platform.OS === 'web') continue;
