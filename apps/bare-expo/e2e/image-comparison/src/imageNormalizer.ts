...
export async function getOptimalTargetDimensions(
  ...
  // Choose a strategy based on aspect ratio similarity
 ...
 */
export async function normalizeImagesForComparison(
  image1Path: string,
  image2Path: string,
  options: NormalizationOptions
): Promise<NormalizedImages> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'image-normalizer-'));
  ...
  await Promise.all([
    normalizeImage(image1Path, normalizedImage1Path, options),
    normalizeImage(image2Path, normalizedImage2Path, options),
  ]);
...
async function normalizeImage(
  ...
  // Read original image
  const originalPng = await readPNG(inputPath);
  ...
  await writePNG(targetPng, outputPath);
}
