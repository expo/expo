...
import BenchmarkView, { LegacyBenchmarkView } from './BenchmarkView';
...
/** Which of the two native benchmark views is mounted and driven. */
type BenchmarkViewKind = 'legacy' | 'decoded';

const VIEW_KINDS: { kind: BenchmarkViewKind; label: string; description: string }[] = [
  {
    kind: 'legacy',
    label: 'Legacy',
    description: 'LegacyBenchmarkView: props lowered to a dictionary, decoded on the main thread',
  },
  {
    kind: 'decoded',
    label: 'Decoded',
    description: 'BenchmarkView: props decoded from JS values on the JS thread',
  },
];
...
export default function ViewPropsBenchmarkScreen() {
  ...
  // One result per view kind, so a legacy run and a decoded run sit side by side.
  const [results, setResults] = React.useState<Partial<Record<BenchmarkViewKind, Result>>>({});
  const [mode, setMode] = React.useState<BenchMode>('all');
  // The native counters are process-wide, so only the selected view is mounted and driven.
  const [viewKind, setViewKind] = React.useState<BenchmarkViewKind>('legacy');
  ...
  const run = React.useCallback((runMode: BenchMode) => {
    setResults((prev) => ({ ...prev, [viewKind]: undefined }));
    setMode(runMode);
    setRunning(true);
    let i = 0;
    ...
    const tick = () => {
      ...
      if (i <= WARMUP + ITERATIONS) {
        ...
        requestAnimationFrame(() => {
          ...
          setResults((prev) => ({ ...prev, [viewKind]: measured }));
          setRunning(false);
        });
      }
    };
  ...
  const decoded = results.decoded;
  // Only compare like with like: both runs in the same mode.
  const comparable = legacy && decoded && legacy.mode === decoded.mode ? { legacy, decoded } : null;
  const cardStyle = {
  ...
  return (
    <ScrollView
      ...
      <Text style={[styles.note, { color: theme.text.default }]}>
        Two native views take the same props by different routes. Pick one, run a mode, then run
        the same mode on the other to compare. "All props" changes every prop each pass (worst
        case); "1 prop" changes one per pass (realistic update).
      </Text>
      ...
      <Text style={[styles.hint, { color: theme.text.secondary }]}>
        {VIEW_KINDS.find((v) => v.kind === viewKind)?.description}
      </Text>

      <View style={[styles.card, cardStyle, styles.previewCard]}>
        {viewKind === 'decoded' ? (
          <BenchmarkView key="decoded" style={styles.benchView} {...props} />
        ) : (
          <LegacyBenchmarkView key="legacy" style={styles.benchView} {...props} />
        )}
      </View>
      ...
      {comparable ? <ComparisonCard {...comparable} theme={theme} cardStyle={cardStyle} /> : null}

      {VIEW_KINDS.map(({ kind, label }) => {
        const result = results[kind];
        return result ? (
          <ResultCard key={kind} label={label} result={result} theme={theme} cardStyle={cardStyle} />
        ) : null;
      })}
    </ScrollView>
  );
...
function ResultCard({
  ...
  // A decode pass only ever runs under the JSI descriptor, so this doubles as a check that the
  // per-class routing did what the view asked for.
  const decodedViaJSI = result.decodePassCount > 0;
  return (
    <View style={[styles.card, cardStyle]}>
      <View style={styles.resultHeader}>
        <Text style={[styles.resultTitle, { color: theme.text.default }]}>
          {label} · {result.mode === 'all' ? 'All props' : '1 prop'} ·{' '}
          {ITERATIONS.toLocaleString()} passes
        </Text>
        <View
          style={[
            styles.badge,
      ...
      <Section title="Counts" theme={theme} last>
        ...
        <Row
          label="legacy props (presented)"
          value={`${result.legacyPresentedPropCount}`}
          theme={theme}
        />
        <Row
...
/** Legacy vs decoded, same mode. Main-thread apply is the number the decoded path exists to cut. */
function ComparisonCard({
  ...
  const applyDelta = percentChange(legacy.applyUsPerPass, decoded.applyUsPerPass);
  // The decoded path moves work to the JS thread rather than removing all of it, so show the
  // total per-pass cost too.
  const legacyTotal = legacy.decodeUsPerPass + legacy.applyUsPerPass;
  const decodedTotal = decoded.decodeUsPerPass + decoded.applyUsPerPass;
  const totalDelta = percentChange(legacyTotal, decodedTotal);
  return (
    <View style={[styles.card, cardStyle]}>
      <View style={styles.resultHeader}>
        <Text style={[styles.resultTitle, { color: theme.text.default }]}>
          Legacy vs decoded · {legacy.mode === 'all' ? 'All props' : '1 prop'}
        </Text>
      </View>
      <Section title="Per pass" theme={theme} last>
        <Row
          label="main thread apply"
          value={`${legacy.applyUsPerPass.toFixed(1)} → ${decoded.applyUsPerPass.toFixed(1)} µs  (${applyDelta})`}
          theme={theme}
          emphasize
        />
        <Row
          label="JS thread decode"
          value={`${legacy.decodeUsPerPass.toFixed(1)} → ${decoded.decodeUsPerPass.toFixed(1)} µs`}
          theme={theme}
        />
        <Row
          label="both threads"
          value={`${legacyTotal.toFixed(1)} → ${decodedTotal.toFixed(1)} µs  (${totalDelta})`}
          theme={theme}
        />
      </Section>
    </View>
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.button,
        {
          backgroundColor: selected ? theme.background.subtle : theme.background.element,
          borderColor: selected ? theme.text.link : theme.border.default,
      ...
      <Text
        style={[
          styles.buttonText,
          { color: selected ? theme.text.link : theme.text.secondary },
        ]}>
        {label}
      </Text>
    </Pressable>
