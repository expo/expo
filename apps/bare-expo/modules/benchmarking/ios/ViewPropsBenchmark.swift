...
/// `folly::dynamic` / `NSDictionary` path. Fed by `BenchmarkView`, which brackets its
/// `updateProps(_:)` / `applyDecodedProps(_:)` overrides (main-thread apply), and by the
/// `ViewPropsJSIDecoder.onDecodePass` observer (JS-thread decode). No timing is wired into core
/// itself.
///
...
enum ViewPropsBenchmark {
  ...
  /// Number of apply passes that ran on the main thread (one per `updateProps` / `applyDecodedProps` call).
