...
if [[ "$EAS_BUILD_PROFILE" == "publish-client" ]]; then
  ...
  if [[ "$EAS_BUILD_PLATFORM" == "android" ]]; then
    notify_slack "Successful Expo Go Android APK build. Run et eas android-apk-upload to upload on GitHub" "$MESSAGE"
  elif [[ "$EAS_BUILD_PLATFORM" == "ios" ]]; then
    notify_slack "Successful Expo Go iOS simulator build. Run et eas ios-simulator-upload to upload on GitHub" "$MESSAGE"
  fi
fi
