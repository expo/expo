...
struct LocalNetworkPermissionView: View {
  ...
  private var isDenied: Bool {
    serverService.permissionStatus == .denied
  }
  ...
  var body: some View {
    ZStack {
      ...
      VStack(spacing: 24) {
        ...
        Text("Expo Go needs to access your local network to discover development servers running on your computer.")
          .font(.body)
  ...
  private func checkAccessButton(label: String, onResult: @escaping (Bool) -> Void) -> some View {
    Button {
      ...
      Task {
        let hasAccess = await serverService.checkLocalNetworkAccess()
