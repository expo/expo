...
class AuthenticationService: ObservableObject {
  ...
  /// A partner actor cannot ask whether its own session is alive, because `meUserActor` is null
  /// either way. The stored expiry is the only signal, and only partner sessions write it.
