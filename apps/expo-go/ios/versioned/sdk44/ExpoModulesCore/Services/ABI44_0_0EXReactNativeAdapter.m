...
#import <ABI44_0_0React/ABI44_0_0RCTAppState.h>
#import <ABI44_0_0React/ABI44_0_0RCTImageLoader.h>

#import <ABI44_0_0ExpoModulesCore/ABI44_0_0EXReactNativeAdapter.h>

@interface ABI44_0_0EXReactNativeAdapter ()
...
@implementation ABI44_0_0EXReactNativeAdapter

ABI44_0_0EX_REGISTER_MODULE();

+ (NSString *)moduleName
{
  return @"ABI44_0_0EXReactNativeAdapter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI44_0_0EXAppLifecycleService), @protocol(ABI44_0_0EXUIManager), @protocol(ABI44_0_0EXJavaScriptContextProvider)];
}
...
{
  __weak ABI44_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI44_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI44_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI44_0_0RCTJSThread];
}
...
# pragma mark - ABI44_0_0EXJavaScriptContextProvider
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI44_0_0RCTContentDidAppearNotification,
                           ABI44_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI44_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI44_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI44_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI44_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
