...
@implementation ABI44_0_0EXUpdatesDatabaseIntegrityCheck
...
{
  NSError *err;
  NSArray<ABI44_0_0EXUpdatesAsset *> *assets = [database allAssetsWithError:&err];
  if (err) {
    *error = err;
    return NO;
  }

  NSMutableArray<ABI44_0_0EXUpdatesAsset *> *missingAssets = [NSMutableArray new];
  dispatch_sync([ABI44_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
  ...
  NSArray<ABI44_0_0EXUpdatesUpdate *> *updatesWithEmbeddedStatus = [database allUpdatesWithStatus:ABI44_0_0EXUpdatesUpdateStatusEmbedded config:config error:&err];
  if (err) {
    *error = err;
    return NO;
  }
  ...
  for (ABI44_0_0EXUpdatesUpdate *update in updatesWithEmbeddedStatus) {
