...
@interface ABI43_0_0EXUpdatesDatabase ()
...
- (BOOL)openDatabaseInDirectory:(NSURL *)directory withError:(NSError ** _Nullable)error
{
  ...
  sqlite3 *db;
  if (![ABI43_0_0EXUpdatesDatabaseInitialization initializeDatabaseWithLatestSchemaInDirectory:directory database:&db error:error]) {
    return NO;
  }
...
{
  if (update.status != ABI43_0_0EXUpdatesUpdateStatusDevelopment) {
    update.status = ABI43_0_0EXUpdatesUpdateStatusReady;
  }
...
- (void)markMissingAssets:(NSArray<ABI43_0_0EXUpdatesAsset *> *)assets error:(NSError ** _Nullable)error
{
  ...
  for (ABI43_0_0EXUpdatesAsset *asset in assets) {
    if ([self _executeSql:updatesSql withArgs:@[@(ABI43_0_0EXUpdatesUpdateStatusPending), @(asset.assetId)] error:error] == nil) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
      return;
    }
  }
...
- (nullable NSArray<ABI43_0_0EXUpdatesUpdate *> *)allUpdatesWithConfig:(ABI43_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI43_0_0EXUpdatesUpdate *> *launchableUpdates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI43_0_0EXUpdatesUpdate *> *)allUpdatesWithStatus:(ABI43_0_0EXUpdatesUpdateStatus)status config:(ABI43_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI43_0_0EXUpdatesUpdate *> *updates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI43_0_0EXUpdatesAsset *> *)allAssetsWithError:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI43_0_0EXUpdatesAsset *> *assets = [NSMutableArray arrayWithCapacity:rows.count];
...
{
  return [self _jsonDataWithKey:ABI43_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey error:error];
}
...
{
  [self _setJsonData:manifestFilters withKey:ABI43_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey isInTransaction:NO error:error];
}
...
- (void)setMetadataWithManifest:(ABI43_0_0EXUpdatesUpdate *)updateManifest error:(NSError ** _Nullable)error
{
  ...
  if (updateManifest.serverDefinedHeaders) {
    if (![self _setJsonData:updateManifest.serverDefinedHeaders
                   withKey:ABI43_0_0EXUpdatesDatabaseServerDefinedHeadersKey
  ...
  if (updateManifest.manifestFilters) {
    if (![self _setJsonData:updateManifest.manifestFilters
                   withKey:ABI43_0_0EXUpdatesDatabaseManifestFiltersKey
                  scopeKey:updateManifest.scopeKey
           isInTransaction:YES
                     error:error]) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
...
- (nullable NSArray<NSDictionary *> *)_executeSql:(NSString *)sql withArgs:(nullable NSArray *)args error:(NSError ** _Nullable)error
{
  ...
  dispatch_assert_queue(_databaseQueue);
  return [ABI43_0_0EXUpdatesDatabaseUtils executeSql:sql withArgs:args onDatabase:_db error:error];
}

- (NSError *)_errorFromSqlite:(struct sqlite3 *)db
{
  return [ABI43_0_0EXUpdatesDatabaseUtils errorFromSqlite:db];
}

- (ABI43_0_0EXUpdatesUpdate *)_updateWithRow:(NSDictionary *)row config:(ABI43_0_0EXUpdatesConfig *)config
{
  NSError *error;
  id manifest = nil;
  ...
  ABI43_0_0EXUpdatesUpdate *update = [ABI43_0_0EXUpdatesUpdate updateWithId:row[@"id"]
                                                 scopeKey:row[@"scope_key"]
                                               commitTime:[ABI43_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"commit_time"]]
                                           runtimeVersion:row[@"runtime_version"]
                                                 manifest:manifest
                                                   status:(ABI43_0_0EXUpdatesUpdateStatus)[(NSNumber *)row[@"status"] integerValue]
