...
@interface ABI43_0_0EXLegacyRemoteNotificationPermissionRequester ()
...
@property (nonatomic, weak) id<ABI43_0_0EXPermissionsRequester> userNotificationPermissionRequester;
@property (nonatomic, weak) dispatch_queue_t methodQueue;
@property (nonatomic, weak) id<ABI43_0_0EXRemoteNotificationPermissionProgressPublisher> permissionProgressPublisher;

@end

@implementation ABI43_0_0EXLegacyRemoteNotificationPermissionRequester
...
{
  __block ABI43_0_0EXPermissionStatus status;
  [ABI43_0_0EXUtilities performSynchronouslyOnMainThread:^{
    status = (ABI43_0_0UMSharedApplication().isRegisteredForRemoteNotifications) ?
...
- (void)requestPermissionsWithResolver:(ABI43_0_0EXPromiseResolveBlock)resolve rejecter:(ABI43_0_0EXPromiseRejectBlock)reject
{
  ...
  [ABI43_0_0EXUtilities performSynchronouslyOnMainThread:^{
    isRegisteredForRemoteNotifications = ABI43_0_0UMSharedApplication().isRegisteredForRemoteNotifications;
  }];
  ...
  if (isRegisteredForRemoteNotifications) {
    ...
    [_userNotificationPermissionRequester requestPermissionsWithResolver:^(NSDictionary *permission){
      ...
      ABI43_0_0EXPermissionStatus localNotificationsStatus = [[permission objectForKey:@"status"] intValue];
      // We may assume that `ABI43_0_0EXLocalNotificationRequester`'s permission request will always finish
      ...
      if (localNotificationsStatus == ABI43_0_0EXPermissionStatusDenied) {
        [self _clearObserver];
        [self _maybeConsumeResolverWithCurrentPermissions];
      } else {
        self.remoteNotificationsRegistrationIsPending = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
          [ABI43_0_0UMSharedApplication() registerForRemoteNotifications];
        });
      }
