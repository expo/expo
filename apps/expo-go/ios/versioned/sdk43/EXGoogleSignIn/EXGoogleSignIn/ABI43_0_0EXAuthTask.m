...
@interface ABI43_0_0EXAuthTask ()

@property (nonatomic, strong) ABI43_0_0EXPromiseResolveBlock resolver;
@property (nonatomic, strong) ABI43_0_0EXPromiseRejectBlock rejecter;
@property (nonatomic, strong) NSString *tag;

@end

@implementation ABI43_0_0EXAuthTask
      ...
      rejecter:(ABI43_0_0EXPromiseRejectBlock)reject
{
  if (!_resolver) {
    ...
    reject(ABI43_0_0EX_E_CONCURRENT_TASK_IN_PROGRESS, [NSString stringWithFormat:@"Failed to start %@ as a concurrent GoogleSignIn task is already running", tag], nil);
    return NO;
  }
}
...
- (void)parse:(NSDictionary *)user error:(NSError *)error
{
  if (error) {
    ...
    [self reject:[ABI43_0_0EXGoogleSignIn jsonFromGIDSignInErrorCode:error.code] error:error];
  } else {
    [self resolve:ABI43_0_0EXNullIfNil(user)];
  }
}
