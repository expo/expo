...
#import <ABI44_0_0React/ABI44_0_0RCTSurfaceRootView.h>
...
@implementation ABI44_0_0RCTFabricSurface {
  // Immutable
  __weak ABI44_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI44_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}

@synthesize delegate = _delegate;
@synthesize rootTag = _rootTag;

- (instancetype)initWithSurfacePresenter:(ABI44_0_0RCTSurfacePresenter *)surfacePresenter
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI44_0_0RCTAllocateRootViewTag() integerValue];
...
- (BOOL)start
{
  if (![self _setStage:ABI44_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI44_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI44_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI44_0_0RCTSurfaceView alloc] initWithSurface:(ABI44_0_0RCTSurface *)self];
...
- (BOOL)_setStage:(ABI44_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:YES];
}

- (BOOL)_unsetStage:(ABI44_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:NO];
}

- (BOOL)_setStage:(ABI44_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
...
- (void)_propagateStageChange:(ABI44_0_0RCTSurfaceStage)stage
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI44_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI44_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
