...
#import "ABI44_0_0RCTSurfaceTouchHandler.h"
...
#import "ABI44_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI44_0_0RCTTouchEventType) {
  ABI44_0_0RCTTouchEventTypeTouchStart,
  ABI44_0_0RCTTouchEventTypeTouchMove,
  ABI44_0_0RCTTouchEventTypeTouchEnd,
  ABI44_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI44_0_0RCTComponentViewProtocol> *componentView = nil;
    ...
    CGPoint rootViewOriginOffset)
{
  ...
  activeTouch.touch.offsetPoint = ABI44_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI44_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI44_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI44_0_0RCTForceTouchAvailable()) {
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView, CGPoint rootViewOriginOffset)
{
  ...
  while (componentView) {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      activeTouch.eventEmitter = [(id<ABI44_0_0RCTTouchableComponentViewProtocol>)componentView
...
@interface ABI44_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI44_0_0RCTSurfaceTouchHandler {
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI44_0_0React Native,
...
ABI44_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithTarget : (id)target action : (SEL)action)

- (void)attachToView:(UIView *)view
{
  ABI44_0_0RCTAssert(self.view == nil, @"ABI44_0_0RCTTouchHandler already has attached view.");
...
{
  ABI44_0_0RCTAssertParam(view);
  ABI44_0_0RCTAssert(self.view == view, @"ABI44_0_0RCTTouchHandler attached to another view.");
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI44_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI44_0_0RCTTouchEventTypeTouchEnd || eventType == ABI44_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitters) {
    ...
    switch (eventType) {
      case ABI44_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI44_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI44_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI44_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI44_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI44_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI44_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI44_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (!_activeTouches.empty()) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI44_0_0RCTTouchEventTypeTouchCancel];
