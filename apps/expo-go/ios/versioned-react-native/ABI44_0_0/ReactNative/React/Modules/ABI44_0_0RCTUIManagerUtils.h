 ...
 *  1. `ABI44_0_0RCTAssertUIManagerQueue` is okay with execution on both actual UIManager and
 *     Pseudo UIManager queues.
 *  2. Both `ABI44_0_0RCTExecuteOnUIManagerQueue` and `ABI44_0_0RCTUnsafeExecuteOnUIManagerQueueSync`
 *     execute given block *synchronously* if they were called on actual UIManager
 *     or Pseudo UIManager queues.
 *  3. `ABI44_0_0RCTExecuteOnMainQueue` executes given block *synchronously* if we already on
 *     the Main queue.
 *  4. `ABI44_0_0RCTUnsafeExecuteOnUIManagerQueueSync` is smart enough to do the trick:
 ...
 * *explicitly* block the Main queue via calling `ABI44_0_0RCTUnsafeExecuteOnMainQueueSync`.
 ...
 */
ABI44_0_0RCT_EXTERN dispatch_queue_t ABI44_0_0RCTGetUIManagerQueue(void);

/**
 * Default name for the UIManager queue.
 */
ABI44_0_0RCT_EXTERN char *const ABI44_0_0RCTUIManagerQueueName;

/**
 * Check if we are currently on UIManager queue.
 * Please do not use this unless you really know what you're doing.
 */
ABI44_0_0RCT_EXTERN BOOL ABI44_0_0RCTIsUIManagerQueue(void);

/**
 * Check if we are currently on Pseudo UIManager queue.
 * Please do not use this unless you really know what you're doing.
 */
ABI44_0_0RCT_EXTERN BOOL ABI44_0_0RCTIsPseudoUIManagerQueue(void);
 ...
 */
ABI44_0_0RCT_EXTERN void ABI44_0_0RCTExecuteOnUIManagerQueue(dispatch_block_t block);
 ...
 * Please do not use this unless you really know what you're doing.
 */
ABI44_0_0RCT_EXTERN void ABI44_0_0RCTUnsafeExecuteOnUIManagerQueueSync(dispatch_block_t block);

/**
 * Convenience macro for asserting that we're running on UIManager queue.
 */
#define ABI44_0_0RCTAssertUIManagerQueue() \
  ABI44_0_0RCTAssert(                      \
      ABI44_0_0RCTIsUIManagerQueue() || ABI44_0_0RCTIsPseudoUIManagerQueue(), @"This function must be called on the UIManager queue")

/**
 * Returns new unique root view tag.
 */
ABI44_0_0RCT_EXTERN NSNumber *ABI44_0_0RCTAllocateRootViewTag(void);
