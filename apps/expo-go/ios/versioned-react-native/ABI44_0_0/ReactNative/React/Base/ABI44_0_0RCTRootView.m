...
NSString *const ABI44_0_0RCTContentDidAppearNotification = @"ABI44_0_0RCTContentDidAppearNotification";

@interface ABI44_0_0RCTUIManager (ABI44_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI44_0_0RCTRootView {
  ABI44_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  ABI44_0_0RCTRootContentView *_contentView;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI44_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI44_0_0RCTRootView");
  ABI44_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI44_0_0RCTRootView");

  ABI44_0_0RCT_PROFILE_BEGIN_EVENT(ABI44_0_0RCTProfileTagAlways, @"-[ABI44_0_0RCTRootView init]", nil);
  if (!bridge.isLoading) {
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _sizeFlexibility = ABI44_0_0RCTRootViewSizeFlexibilityNone;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI44_0_0RCTContentDidAppearNotification
                                               object:self];
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  fitSize = CGSizeMake(
      _sizeFlexibility & ABI44_0_0RCTRootViewSizeFlexibilityWidth ? fitSize.width : currentSize.width,
      _sizeFlexibility & ABI44_0_0RCTRootViewSizeFlexibilityHeight ? fitSize.height : currentSize.height);
...
- (void)setMinimumSize:(CGSize)minimumSize
{
  ...
  __block NSNumber *tag = self.ABI44_0_0ReactTag;
  __weak typeof(self) weakSelf = self;
  ABI44_0_0RCTExecuteOnUIManagerQueue(^{
    __strong typeof(self) strongSelf = weakSelf;
    if (strongSelf && strongSelf->_bridge.isValid) {
      ABI44_0_0RCTRootShadowView *shadowView = (ABI44_0_0RCTRootShadowView *)[strongSelf->_bridge.uiManager shadowViewForABI44_0_0ReactTag:tag];
...
{
  ABI44_0_0RCTAssertMainQueue();
  if (!super.ABI44_0_0ReactTag) {
    /**
     * Every root view that is created must have a unique ABI44_0_0React tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ABI44_0_0React tag must be re-assigned every time a new UIManager is created.
     */
    self.ABI44_0_0ReactTag = ABI44_0_0RCTAllocateRootViewTag();
  }
  return super.ABI44_0_0ReactTag;
}

- (void)bridgeDidReload
{
  ABI44_0_0RCTAssertMainQueue();
  // Clear the ABI44_0_0ReactTag so it can be re-assigned
  self.ABI44_0_0ReactTag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI44_0_0RCTRootContentView is scoped to the right bridge
...
- (void)bundleFinishedLoading:(ABI44_0_0RCTBridge *)bridge
{
  ABI44_0_0RCTAssert(bridge != nil, @"Bridge cannot be nil");
  if (!bridge.valid) {
    return;
  }

  [_contentView removeFromSuperview];
  _contentView = [[ABI44_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
  ...
  if (_sizeFlexibility == ABI44_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI44_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag" : _contentView.ABI44_0_0ReactTag,
    @"initialProps" : _appProperties ?: @{},
  };

  ABI44_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
@implementation ABI44_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI44_0_0RCTLogWarn(@"Calling deprecated `[-ABI44_0_0RCTRootView intrinsicSize]`.");
  return self.intrinsicContentSize;
}

- (void)cancelTouches
{
  ABI44_0_0RCTLogWarn(@"`-[ABI44_0_0RCTRootView cancelTouches]` is deprecated and will be deleted soon.");
