...
#import "ABI44_0_0RCTSurfaceRootShadowView.h"
#import "ABI44_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI44_0_0RCTSurface () <ABI44_0_0RCTSurfaceRootShadowViewDelegate, ABI44_0_0RCTUIManagerObserver>
@end

@implementation ABI44_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI44_0_0RCTBridge *_batchedBridge;
  ABI44_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI44_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI44_0_0RCTSurfaceView *_Nullable _view;
  ABI44_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI44_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instantiate `ABI44_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI44_0_0RCTAllocateRootViewTag();
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBridgeWillLoadJavaScriptNotification:)
                                                 name:ABI44_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    _stage = ABI44_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI44_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI44_0_0RCTSurfaceView *)view
{
  ABI44_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI44_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI44_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI44_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI44_0_0RCTSurfaceRootView *rootView = (ABI44_0_0RCTSurfaceRootView *)[self._uiManager viewForABI44_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI44_0_0RCTAssert(
      [rootView isKindOfClass:[ABI44_0_0RCTSurfaceRootView class]],
      @"Received root view is not an instance of `ABI44_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI44_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI44_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI44_0_0RCTSurfaceStage)stage
{
  ABI44_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI44_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI44_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI44_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI44_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI44_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI44_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI44_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI44_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI44_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI44_0_0RCTIsMainQueue() ? ABI44_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI44_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI44_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI44_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI44_0_0ReactTag:self->_rootViewTag];
    ABI44_0_0RCTAssert(
        [rootShadowView isKindOfClass:[ABI44_0_0RCTSurfaceRootShadowView class]],
        @"Received shadow view is not an instance of `ABI44_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI44_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI44_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI44_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI44_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI44_0_0ReactTag:self->_rootViewTag];
...
{
  // `viewportOffset` is intentionally zero because `ABI44_0_0RCTSurface` ignores it.
  // However, it is needed in `ABI44_0_0RCTFabricSurface`.
...
{
  if (ABI44_0_0RCTIsUIManagerQueue()) {
    ABI44_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI44_0_0RCTIsMainQueue() && (stage & ABI44_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI44_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI44_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI44_0_0RCTAssert(
          NO,
          @"Only waiting for `ABI44_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI44_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI44_0_0RCTIsMainQueue() && (stage & ABI44_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI44_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI44_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI44_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI44_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI44_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI44_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI44_0_0RCTUIManager *)manager performMountingWithBlock:(ABI44_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI44_0_0RCTUIManager *)manager
{
  if (self.stage & ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI44_0_0RCTSurfaceStageSurfaceDidInitialMounting];
