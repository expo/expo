...
@implementation ABI44_0_0RCTBaseTextInputShadowView
{
...
- (instancetype)initWithBridge:(ABI44_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI44_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI44_0_0RCTBaseTextInputShadowViewMeasure);
    ABI44_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI44_0_0RCTTextInputShadowViewBaseline);
  }
...
- (void)uiManagerWillPerformMounting
{
  ...
  ABI44_0_0RCTTextAttributes *textAttributes = [self.textAttributes copy];
  ...
  [attributedText removeAttribute:ABI44_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                            range:NSMakeRange(0, attributedText.length)];

  [attributedText removeAttribute:ABI44_0_0RCTTextAttributesTagAttributeName
                            range:NSMakeRange(0, attributedText.length)];
  ...
  [_bridge.uiManager addUIBlock:^(ABI44_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI44_0_0RCTBaseTextInputView *baseTextInputView = (ABI44_0_0RCTBaseTextInputView *)viewRegistry[tag];
    if (!baseTextInputView) {
      return;
    }

    baseTextInputView.textAttributes = textAttributes;
    baseTextInputView.ABI44_0_0ReactBorderInsets = borderInsets;
    baseTextInputView.ABI44_0_0ReactPaddingInsets = paddingInsets;
...
- (CGSize)sizeThatFitsMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  return (CGSize){
    MAX(minimumSize.width, MIN(ABI44_0_0RCTCeilPixelValue(size.width), maximumSize.width)),
    MAX(minimumSize.height, MIN(ABI44_0_0RCTCeilPixelValue(size.height), maximumSize.height))
  };
}
...
static ABI44_0_0YGSize ABI44_0_0RCTBaseTextInputShadowViewMeasure(ABI44_0_0YGNodeRef node, float width, ABI44_0_0YGMeasureMode widthMode, float height, ABI44_0_0YGMeasureMode heightMode)
{
  ...
  return (ABI44_0_0YGSize){
    ABI44_0_0RCTYogaFloatFromCoreGraphicsFloat(measuredSize.width),
    ABI44_0_0RCTYogaFloatFromCoreGraphicsFloat(measuredSize.height)
  };
}

static float ABI44_0_0RCTTextInputShadowViewBaseline(ABI44_0_0YGNodeRef node, const float width, const float height)
{
  ABI44_0_0RCTBaseTextInputShadowView *shadowTextView = (__bridge ABI44_0_0RCTBaseTextInputShadowView *)ABI44_0_0YGNodeGetContext(node);
