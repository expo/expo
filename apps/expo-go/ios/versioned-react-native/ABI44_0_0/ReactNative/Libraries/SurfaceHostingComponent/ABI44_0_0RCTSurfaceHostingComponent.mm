...
#import "ABI44_0_0RCTSurfaceHostingComponent.h"
#import "ABI44_0_0RCTSurfaceHostingComponent+Internal.h"
...
#import "ABI44_0_0RCTSurfaceHostingComponentController.h"
#import "ABI44_0_0RCTSurfaceHostingComponentState.h"

@implementation ABI44_0_0RCTSurfaceHostingComponent

+ (Class<CKComponentControllerProtocol>)controllerClass
{
  return [ABI44_0_0RCTSurfaceHostingComponentController class];
}

+ (id)initialState
{
  return [ABI44_0_0RCTSurfaceHostingComponentState new];
}
...
+ (instancetype)newWithSurface:(ABI44_0_0RCTSurface *)surface options:(ABI44_0_0RCTSurfaceHostingComponentOptions)options
{
  ...
  ABI44_0_0RCTSurfaceHostingComponentState *const state = scope.state();

  ABI44_0_0RCTSurfaceHostingComponentState *const newState =
    [ABI44_0_0RCTSurfaceHostingComponentState newWithStage:surface.stage
  ...
  ABI44_0_0RCTSurfaceHostingComponent *const component =
...
- (CKComponentLayout)computeLayoutThatFits:(CKSizeRange)constrainedSize
{
  ...
  // just to provide layout constraints to ABI44_0_0React Native as early as possible.
  // ABI44_0_0React Native *may* use this info later during applying the own state and
  ...
  // interop compatibilities of ABI44_0_0React Native which will enable us granularly
  // control ABI44_0_0React Native mounting blocks and, as a result, implement
  // truly synchronous mounting stage between ABI44_0_0React Native and ComponentKit.
  [_surface setMinimumSize:constrainedSize.min
               maximumSize:constrainedSize.max];

  // Just in case of the very first building pass, we give ABI44_0_0React Native a chance
  // to prepare its internals for coming synchronous measuring.
  [_surface synchronouslyWaitForStage:ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout
                              timeout:_options.synchronousLayoutingTimeout];

  CGSize fittingSize = CGSizeZero;
  if (_surface.stage & ABI44_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
...
- (CKComponentBoundsAnimation)boundsAnimationFromPreviousComponent:(ABI44_0_0RCTSurfaceHostingComponent *)previousComponent
{
