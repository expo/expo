...
TEST_F(ShadowNodeTest, handleShadowNodeCreation) {
  ABI44_0_0EXPECT_FALSE(nodeZ_->getSealed());
  ABI44_0_0EXPECT_STREQ(nodeZ_->getComponentName(), "Test");
  ABI44_0_0EXPECT_EQ(nodeZ_->getTag(), 18);
  ABI44_0_0EXPECT_EQ(nodeZ_->getSurfaceId(), surfaceId_);
  ABI44_0_0EXPECT_EQ(nodeZ_->getEventEmitter(), nullptr);
  ABI44_0_0EXPECT_EQ(nodeZ_->getChildren().size(), 0);
}

TEST_F(ShadowNodeTest, handleSealRecusive) {
  nodeZ_->sealRecursive();
  ABI44_0_0EXPECT_TRUE(nodeZ_->getSealed());
  ABI44_0_0EXPECT_TRUE(nodeZ_->getProps()->getSealed());
}
...
TEST_F(ShadowNodeTest, handleShadowNodeSimpleCloning) {
  ...
  ABI44_0_0EXPECT_STREQ(nodeA_->getComponentName(), nodeARevision2->getComponentName());
  ABI44_0_0EXPECT_EQ(nodeA_->getTag(), nodeARevision2->getTag());
  ABI44_0_0EXPECT_EQ(nodeA_->getSurfaceId(), nodeARevision2->getSurfaceId());
  ABI44_0_0EXPECT_EQ(nodeA_->getEventEmitter(), nodeARevision2->getEventEmitter());
}

TEST_F(ShadowNodeTest, handleShadowNodeMutation) {
  auto nodeABChildren = nodeAB_->getChildren();
  ABI44_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI44_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABA_);
  ABI44_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);
  ...
  ABI44_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI44_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABArevision2);
  ABI44_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);

  // Seal the entire tree.
  nodeAB_->sealRecursive();
  ABI44_0_0EXPECT_TRUE(nodeAB_->getSealed());
  ABI44_0_0EXPECT_TRUE(nodeABArevision2->getSealed());
  ABI44_0_0EXPECT_TRUE(nodeABB_->getSealed());
}
...
TEST_F(ShadowNodeTest, handleCloneFunction) {
  ...
  ABI44_0_0EXPECT_NE(nodeAB_, nodeABClone);
  ...
  ABI44_0_0EXPECT_EQ(nodeAB_->getTag(), nodeABClone->getTag());
  ABI44_0_0EXPECT_EQ(nodeAB_->getSurfaceId(), nodeABClone->getSurfaceId());
  ABI44_0_0EXPECT_EQ(nodeAB_->getProps(), nodeABClone->getProps());
}
...
TEST_F(ShadowNodeTest, handleState) {
  ...
  ABI44_0_0EXPECT_EQ(firstNode->getState(), secondNode->getState());
  ABI44_0_0EXPECT_NE(firstNode->getState(), thirdNode->getState());
  secondNode->setStateData(TestState{42});
  ABI44_0_0EXPECT_NE(firstNode->getState(), secondNode->getState());
