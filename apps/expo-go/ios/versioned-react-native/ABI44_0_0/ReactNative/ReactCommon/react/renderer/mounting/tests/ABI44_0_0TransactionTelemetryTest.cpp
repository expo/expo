...
#define ABI44_0_0EXPECT_EQ_WITH_THRESHOLD(a, b, threshold) \
  ABI44_0_0EXPECT_TRUE((a >= b - threshold) && (a <= b + threshold))
...
TEST(TransactionTelemetryTest, timepoints) {
  ...
  ABI44_0_0EXPECT_EQ_WITH_THRESHOLD(duration, 100, threshold);
}
...
TEST(TransactionTelemetryTest, normalUseCase) {
  ...
  ABI44_0_0EXPECT_EQ_WITH_THRESHOLD(commitDuration, 400, threshold);
  ABI44_0_0EXPECT_EQ_WITH_THRESHOLD(layoutDuration, 200, threshold);
  ABI44_0_0EXPECT_EQ_WITH_THRESHOLD(mountDuration, 100, threshold);

  ABI44_0_0EXPECT_EQ(telemetry.getNumberOfTextMeasurements(), 3);
  ABI44_0_0EXPECT_EQ(telemetry.getRevisionNumber(), 42);
}
