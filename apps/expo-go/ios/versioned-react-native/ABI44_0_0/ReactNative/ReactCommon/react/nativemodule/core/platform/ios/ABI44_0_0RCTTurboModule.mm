...
#import <ABI44_0_0React/ABI44_0_0RCTCxxConvert.h>
#import <ABI44_0_0React/ABI44_0_0RCTManagedPointer.h>
...
#import <ABI44_0_0ReactCommon/ABI44_0_0LongLivedObject.h>
...
static ABI44_0_0RCTResponseSenderBlock
...
convertJSIFunctionToCallback(jsi::Runtime &runtime, const jsi::Function &value, std::shared_ptr<CallInvoker> jsInvoker)
{
  ...
  ABI44_0_0RCTResponseSenderBlock callback = ^(NSArray *responses) {
    if (wrapperWasCalled) {
  ...
  if (ABI44_0_0RCTTurboModuleBlockCopyEnabled()) {
 ...
 * This serves as a backward-compatible support for ABI44_0_0RCTBridgeModule's methodQueue API.
 *
 * In the future:
    ...
    NSMutableArray *retainedObjectsForInvocation)
{
  ...
  __weak id<ABI44_0_0RCTTurboModule> weakModule = instance_;
  ...
  void (^block)() = ^{
    id<ABI44_0_0RCTTurboModule> strongModule = weakModule;
    if (!strongModule) {
      return;
    }
  ...
  if (!methodArgumentTypeNames_) {
    ...
    if (methods) {
      for (unsigned int i = 0; i < numberOfMethods; i++) {
        ...
        ABI44_0_0RCTMethodInfo *(*getMethodInfo)(id, SEL) = (__typeof__(getMethodInfo))objc_msgSend;
        ABI44_0_0RCTMethodInfo *methodInfo = getMethodInfo(cls, s);

        NSArray<ABI44_0_0RCTMethodArgument *> *arguments;
        NSString *otherMethodName = ABI44_0_0RCTParseMethodSignature(methodInfo->objcName, &arguments);
...
{
  const char *moduleName = name_.c_str();
  const id<ABI44_0_0RCTTurboModule> module = instance_;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (objCArg) {
      ...
      /**
       * Convert objects using ABI44_0_0RCTConvert.
       */
      if (objCArgType == @encode(id)) {
        ...
        if (argumentType != nil) {
          ...
          if ([ABI44_0_0RCTConvert respondsToSelector:rctConvertSelector]) {
            // Message dispatch logic from old infra
            id (*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
            id convertedObjCArg = convert([ABI44_0_0RCTConvert class], rctConvertSelector, objCArg);
      ...
      /**
       * Convert objects using ABI44_0_0RCTCxxConvert to structs.
       */
      if ([objCArg isKindOfClass:[NSDictionary class]] && hasMethodArgConversionSelector(methodNameNSString, i)) {
        ...
        ABI44_0_0RCTManagedPointer *box = convert([ABI44_0_0RCTCxxConvert class], methodArgConversionSelector, objCArg);
    ...
    size_t count)
{
  ...
  jsi::Value returnValue = returnType == PromiseKind
      ? createPromise(
            runtime,
            jsInvoker_,
            ^(ABI44_0_0RCTPromiseResolveBlock resolveBlock, ABI44_0_0RCTPromiseRejectBlock rejectBlock) {
              ABI44_0_0RCTPromiseResolveBlock resolveCopy = resolveBlock;
              ABI44_0_0RCTPromiseRejectBlock rejectCopy = rejectBlock;
