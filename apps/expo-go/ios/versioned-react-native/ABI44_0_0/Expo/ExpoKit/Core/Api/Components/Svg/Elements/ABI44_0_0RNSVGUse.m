...
@implementation ABI44_0_0RNSVGUse
...
- (void)setUsewidth:(ABI44_0_0RNSVGLength *)usewidth
{
...
- (void)setUseheight:(ABI44_0_0RNSVGLength *)useheight
{
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    ABI44_0_0RNSVGNode* template = [self.svgView getDefinedTemplate:self.href];
    if (template) {
        [self beginTransparencyLayer:context];
        [self clip:context];

        if ([template isKindOfClass:[ABI44_0_0RNSVGRenderable class]]) {
            [(ABI44_0_0RNSVGRenderable*)template mergeProperties:self];
        }

        if ([template class] == [ABI44_0_0RNSVGSymbol class]) {
            ABI44_0_0RNSVGSymbol *symbol = (ABI44_0_0RNSVGSymbol*)template;
        ...
        if ([template isKindOfClass:[ABI44_0_0RNSVGRenderable class]]) {
            [(ABI44_0_0RNSVGRenderable*)template resetProperties];
        }

        [self endTransparencyLayer:context];
    } else if (self.href) {
        // TODO: calling yellow box here
        ABI44_0_0RCTLogWarn(@"`Use` element expected a pre-defined svg template as `href` prop, template named: %@ is not defined.", self.href);
        return;
    } else {
        return;
    }
...
- (ABI44_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    ABI44_0_0RNSVGNode const* template = [self.svgView getDefinedTemplate:self.href];
    if (event) {
        self.active = NO;
    } else if (self.active) {
        return self;
    }
    ABI44_0_0RNSVGPlatformView const* hitChild = [template hitTest:transformed withEvent:event];
    if (hitChild) {
        self.active = YES;
        return self;
    }
