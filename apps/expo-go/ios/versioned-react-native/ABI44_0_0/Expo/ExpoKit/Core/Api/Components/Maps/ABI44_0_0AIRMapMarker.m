...
NSInteger const ABI44_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE = 999;

@implementation ABI44_0_0AIREmptyCalloutBackgroundView
@end

@implementation ABI44_0_0AIRMapMarker {
    BOOL _hasSetCalloutOffset;
    ABI44_0_0RCTImageLoaderCancellationBlock _reloadImageCancellationBlock;
...
- (void)ABI44_0_0ReactSetFrame:(CGRect)frame
{
    ...
    if (isnan(center.x) || isnan(center.y) ||
        ...
        ABI44_0_0RCTLogError(@"Invalid layout for (%@)%@. position: %@. bounds: %@",
                self.ABI44_0_0ReactTag, self, NSStringFromCGPoint(center), NSStringFromCGRect(bounds));
        return;
    }
...
- (void)insertABI44_0_0ReactSubview:(id<ABI44_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    if ([subview isKindOfClass:[ABI44_0_0AIRMapCallout class]]) {
        self.calloutView = (ABI44_0_0AIRMapCallout *)subview;
    } else {
        [super insertABI44_0_0ReactSubview:(UIView *)subview atIndex:atIndex];
    }
}

- (void)removeABI44_0_0ReactSubview:(id<ABI44_0_0RCTComponent>)subview {
    if ([subview isKindOfClass:[ABI44_0_0AIRMapCallout class]] && self.calloutView == subview) {
        self.calloutView = nil;
    } else {
        [super removeABI44_0_0ReactSubview:(UIView *)subview];
    }
}
...
- (MKAnnotationView *)getAnnotationView
{
    if ([self shouldUsePinView]) {
        ...
        // If it has subviews, it means we are wanting to render a custom marker with arbitrary ABI44_0_0React views.
        // if it has a non-null image, it means we want to render a custom marker with the image.
        // In either case, we want to return the ABI44_0_0AIRMapMarker since it is both an MKAnnotation and an
...
- (void)fillCalloutView:(ABI44_0_0SMCalloutView *)calloutView
{
    ...
    if (self.calloutView) {
        ...
        if (self.calloutView.tooltip) {
            // if tooltip is true, then the user wants their ABI44_0_0React view to be the "tooltip" as wwell, so we set
            // the background view to something empty/transparent
            calloutView.backgroundView = [ABI44_0_0AIREmptyCalloutBackgroundView new];
        } else {
            // the default tooltip look is wanted, and the user is just filling the content with their ABI44_0_0React subviews.
            // as a result, we use the default "masked" background view.
            calloutView.backgroundView = [ABI44_0_0SMCalloutMaskedBackgroundView new];
        }

        // when this is set, the callout's content will be whatever ABI44_0_0React views the user has put as the callout's
        // children.
...
- (void)_handleTap:(UITapGestureRecognizer *)recognizer {
    ABI44_0_0AIRMapMarker *marker = self;
    if (!marker) return;
...
{
    return self.ABI44_0_0ReactSubviews.count == 0 && !self.imageSrc;
}
...
- (void)setImageSrc:(NSString *)imageSrc
{
    ...
    _reloadImageCancellationBlock = [[_bridge moduleForName:@"ImageLoader"] loadImageWithURLRequest:[ABI44_0_0RCTConvert NSURLRequest:_imageSrc]
                                                                            size:self.bounds.size
                                                                           scale:ABI44_0_0RCTScreenScale()
                                                                         clipped:YES
                                                                      resizeMode:ABI44_0_0RCTResizeModeCenter
                                                                   progressBlock:nil
...
- (void)setZIndex:(NSInteger)zIndex
{
    ...
    _zIndex = _calloutIsOpen ? zIndex + ABI44_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE : zIndex;
