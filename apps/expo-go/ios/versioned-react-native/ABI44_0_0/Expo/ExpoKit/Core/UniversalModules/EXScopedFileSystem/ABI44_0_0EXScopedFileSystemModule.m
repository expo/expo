...
NSString * const ABI44_0_0EXShellManifestResourceName = @"shell-app-manifest";

@implementation ABI44_0_0EXScopedFileSystemModule
...
- (NSArray<NSString *> *)bundledAssets
{
  ...
  dispatch_once(&once, ^{
    NSString *manifestBundlePath = [[NSBundle mainBundle] pathForResource:ABI44_0_0EXShellManifestResourceName ofType:@"json"];
    ...
    if (error) {
      ABI44_0_0EXLogError(@"Error parsing bundled manifest: %@", error);
      return;
    }
