 ...
 */
@interface ABI42_0_0EXUpdatesDatabaseIntegrityCheckMockingAssetExists : ABI42_0_0EXUpdatesDatabaseIntegrityCheck

@end

@implementation ABI42_0_0EXUpdatesDatabaseIntegrityCheckMockingAssetExists
...
@interface ABI42_0_0EXUpdatesDatabaseIntegrityCheckTests : XCTestCase
...
@implementation ABI42_0_0EXUpdatesDatabaseIntegrityCheckTests
...
- (void)testFilterEmbeddedUpdates
{
  ...
  ABI42_0_0EXUpdatesUpdate *update1 = [ABI42_0_0EXUpdatesUpdate updateWithId:NSUUID.UUID scopeKey:scopeKey commitTime:[NSDate dateWithTimeIntervalSince1970:1608667851] runtimeVersion:runtimeVersion manifest:nil status:ABI42_0_0EXUpdatesUpdateStatusReady keep:YES config:config database:_db];
  ABI42_0_0EXUpdatesUpdate *update2 = [ABI42_0_0EXUpdatesUpdate updateWithId:NSUUID.UUID scopeKey:scopeKey commitTime:[NSDate dateWithTimeIntervalSince1970:1608667852] runtimeVersion:runtimeVersion manifest:nil status:ABI42_0_0EXUpdatesUpdateStatusReady keep:YES config:config database:_db];

  update1.status = ABI42_0_0EXUpdatesUpdateStatusEmbedded;
  update2.status = ABI42_0_0EXUpdatesUpdateStatusEmbedded;
  ...
  dispatch_sync(_db.databaseQueue, ^{
    ...
    NSError *error3;
    [ABI42_0_0EXUpdatesDatabaseIntegrityCheck runWithDatabase:_db directory:_testDatabaseDir config:config embeddedUpdate:update2 error:&error3];
    XCTAssertNil(error3);

    NSArray<ABI42_0_0EXUpdatesUpdate *> *allUpdates = [_db allUpdatesWithConfig:config error:nil];
...
{
  ABI42_0_0EXUpdatesAsset *asset1 = [[ABI42_0_0EXUpdatesAsset alloc] initWithKey:@"asset1" type:@"png"];
  asset1.downloadTime = [NSDate date];
  asset1.contentHash = @"hash1";
  ABI42_0_0EXUpdatesAsset *asset2 = [[ABI42_0_0EXUpdatesAsset alloc] initWithKey:@"asset2" type:@"png"];
  ...
  update1.status = ABI42_0_0EXUpdatesUpdateStatusReady;
  update2.status = ABI42_0_0EXUpdatesUpdateStatusReady;
  ...
  dispatch_sync(_db.databaseQueue, ^{
    ...
    NSError *error5;
    [ABI42_0_0EXUpdatesDatabaseIntegrityCheckMockingAssetExists runWithDatabase:_db directory:_testDatabaseDir config:config embeddedUpdate:nil error:&error5];
    XCTAssertNil(error5);

    NSArray<ABI42_0_0EXUpdatesUpdate *> *allUpdates = [_db allUpdatesWithConfig:config error:nil];
    NSArray<ABI42_0_0EXUpdatesAsset *> *allAssets = [_db allAssetsWithError:nil];
    XCTAssertEqual(2, allUpdates.count);
    XCTAssertEqual(2, allAssets.count);

    NSArray<ABI42_0_0EXUpdatesUpdate *> *sortedUpdates = [allUpdates sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"commitTime" ascending:YES]]];
    XCTAssertEqual(ABI42_0_0EXUpdatesUpdateStatusReady, sortedUpdates[0].status);
    XCTAssertEqual(ABI42_0_0EXUpdatesUpdateStatusPending, sortedUpdates[1].status);
  });
}
