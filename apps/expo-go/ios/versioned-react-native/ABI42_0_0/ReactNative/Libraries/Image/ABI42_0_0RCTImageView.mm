...
#import <ABI42_0_0React/ABI42_0_0RCTImageBlurUtils.h>
 ...
 * at `idealSize`.
 */
static BOOL ABI42_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
/**
 * See ABI42_0_0RCTConvert (ImageSource). We want to send down the source as a similar
 * JSON parameter.
 */
static NSDictionary *onLoadParamsForSource(ABI42_0_0RCTImageSource *source)
{
  NSDictionary *dict = @{
...
@interface ABI42_0_0RCTImageView ()

@property (nonatomic, copy) ABI42_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI42_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI42_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI42_0_0RCTDirectEventBlock onPartialLoad;
@property (nonatomic, copy) ABI42_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI42_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI42_0_0RCTImageView
{
  ...
  ABI42_0_0RCTImageSource *_imageSource;

  // The image source that's being loaded from the network
  ABI42_0_0RCTImageSource *_pendingImageSource;
  ...
  ABI42_0_0RCTUIImageViewAnimated *_imageView;
}
...
- (instancetype)initWithBridge:(ABI42_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _imageView = [[ABI42_0_0RCTUIImageViewAnimated alloc] init];
...
- (void)updateWithImage:(UIImage *)image
{
  ...
  if (_resizeMode == ABI42_0_0RCTResizeModeRepeat) {
...
- (void)setImageSources:(NSArray<ABI42_0_0RCTImageSource *> *)imageSources
{
...
- (void)setResizeMode:(ABI42_0_0RCTResizeMode)resizeMode
{
...
- (ABI42_0_0RCTImageSource *)imageSourceForSize:(CGSize)size
{
  ...
  const CGFloat scale = ABI42_0_0RCTScreenScale();
  const CGFloat targetImagePixels = size.width * size.height * scale * scale;

  ABI42_0_0RCTImageSource *bestSource = nil;
  CGFloat bestFit = CGFLOAT_MAX;
  for (ABI42_0_0RCTImageSource *source in _imageSources) {
...
- (BOOL)shouldChangeImageSource
{
  ...
  ABI42_0_0RCTImageSource *desiredImageSource = [self imageSourceForSize:self.frame.size];
...
- (void)reloadImage
{
  ...
  ABI42_0_0RCTImageSource *source = [self imageSourceForSize:self.frame.size];
  ...
  if (source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI42_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (_onProgress) {
    ...
    __weak ABI42_0_0RCTImageView *weakSelf = self;
    ABI42_0_0RCTImageLoaderPartialLoadBlock partialLoadHandler = ^(UIImage *image) {
      [weakSelf imageLoaderLoadedImage:image error:nil forImageSource:source partial:YES];
    };

    CGSize imageSize = self.bounds.size;
    CGFloat imageScale = ABI42_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    ABI42_0_0RCTImageLoaderCompletionBlock completionHandler = ^(NSError *error, UIImage *loadedImage) {
      [weakSelf imageLoaderLoadedImage:loadedImage error:error forImageSource:source partial:NO];
    };

    id<ABI42_0_0RCTImageLoaderWithAttributionProtocol> imageLoader = [_bridge moduleForName:@"ImageLoader"
                                                             lazilyLoadIfNecessary:YES];
    ABI42_0_0RCTImageURLLoaderRequest *loaderRequest = [imageLoader loadImageWithURLRequest:source.request
                                                                              size:imageSize
                                                                             scale:imageScale
                                                                           clipped:NO
                                                                        resizeMode:_resizeMode
                                                                       attribution:{
                                                                                   .nativeViewTag = [self.ABI42_0_0ReactTag intValue],
...
- (void)imageLoaderLoadedImage:(UIImage *)loadedImage error:(NSError *)error forImageSource:(ABI42_0_0RCTImageSource *)source partial:(BOOL)isPartialLoad
{
  ...
  void (^setImageBlock)(UIImage *) = ^(UIImage *image) {
    ...
    if (isPartialLoad) {
      ...
      if (self->_onLoad) {
        ABI42_0_0RCTImageSource *sourceLoaded = [source imageSourceWithSize:image.size scale:source.scale];
  ...
  if (_blurRadius > __FLT_EPSILON__) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      UIImage *blurredImage = ABI42_0_0RCTBlurredImageWithRadius(loadedImage, self->_blurRadius);
      ABI42_0_0RCTExecuteOnMainQueue(^{
...
- (void)ABI42_0_0ReactSetFrame:(CGRect)frame
{
  ...
  if ([self shouldChangeImageSource]) {
    ...
    CGSize idealSize = ABI42_0_0RCTTargetSize(imageSize, imageScale, frame.size, ABI42_0_0RCTScreenScale(),
                                     (ABI42_0_0RCTResizeMode)self.contentMode, YES);

    // Don't reload if the current image or target image size is close enough
    if (!ABI42_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize) ||
        !ABI42_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
      return;
    }
    ...
    ABI42_0_0RCTLogInfo(@"Reloading image %@ as size %@", _imageSource.request.URL.absoluteString, NSStringFromCGSize(idealSize));
