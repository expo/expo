 ...
 */
static NSRegularExpression *ABI42_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI42_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI42_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI42_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI42_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI42_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI42_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI42_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI42_0_0RCTNilIfNull(dict[@"collapse"]) integerValue]];
}

+ (NSArray<ABI42_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI42_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI42_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI42_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
