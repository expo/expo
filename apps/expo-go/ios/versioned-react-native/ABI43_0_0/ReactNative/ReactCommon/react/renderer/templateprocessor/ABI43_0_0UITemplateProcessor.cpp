...
#include "ABI43_0_0UITemplateProcessor.h"
...
SharedShadowNode UITemplateProcessor::runCommand(
    ...
    const ComponentDescriptorRegistry &componentDescriptorRegistry,
    const NativeModuleRegistry &nativeModuleRegistry,
    const std::shared_ptr<const ABI43_0_0ReactNativeConfig> ABI43_0_0ReactNativeConfig) {
  ...
  if (opcode == "createNode") {
    ...
    registers[registerNumber] = ABI43_0_0ReactNativeConfig->getBool(param);
...
SharedShadowNode UITemplateProcessor::buildShadowTree(
  ...
  LOG(ERROR) << "ABI43_0_0React ui template missing returnRoot command :(";
