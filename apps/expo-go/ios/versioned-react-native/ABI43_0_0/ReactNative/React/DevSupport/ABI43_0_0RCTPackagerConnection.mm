...
@interface ABI43_0_0RCTPackagerConnection () <ABI43_0_0RCTReconnectingWebSocketDelegate>
@end
...
@implementation ABI43_0_0RCTPackagerConnection {
  std::mutex _mutex; // protects all ivars
  ABI43_0_0RCTReconnectingWebSocket *_socket;
  BOOL _socketConnected;
  NSString *_serverHostPortForSocket;
  id _bundleURLChangeObserver;
  uint32_t _nextToken;
  std::vector<Registration<ABI43_0_0RCTNotificationHandler>> _notificationRegistrations;
  std::vector<Registration<ABI43_0_0RCTRequestHandler>> _requestRegistrations;
  std::vector<Registration<ABI43_0_0RCTConnectedHandler>> _connectedRegistrations;
}

+ (instancetype)sharedPackagerConnection
{
  static ABI43_0_0RCTPackagerConnection *connection;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    connection = [ABI43_0_0RCTPackagerConnection new];
  });
  return connection;
}
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _serverHostPortForSocket = [[ABI43_0_0RCTBundleURLProvider sharedSettings] packagerServerHostPort];
    _socket = socketForLocation(_serverHostPortForSocket);
    _socket.delegate = self;
    [_socket start];

    ABI43_0_0RCTPackagerConnection *const __weak weakSelf = self;
    _bundleURLChangeObserver =
        [[NSNotificationCenter defaultCenter] addObserverForName:ABI43_0_0RCTBundleURLProviderUpdatedNotification
                                                          object:nil
...
static ABI43_0_0RCTReconnectingWebSocket *socketForLocation(NSString *const serverHostPort)
{
  NSString *serverHost;
  NSString *serverPort;
  ...
  components.port = serverPort ? @(serverPort.integerValue) : @(kABI43_0_0RCTBundleURLProviderDefaultPort);
  components.path = @"/message";
  components.queryItems = @[ [NSURLQueryItem queryItemWithName:@"role" value:@"ios"] ];
  static dispatch_queue_t queue;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    queue = dispatch_queue_create("com.facebook.ABI43_0_0RCTPackagerConnectionQueue", DISPATCH_QUEUE_SERIAL);
  });
  return [[ABI43_0_0RCTReconnectingWebSocket alloc] initWithURL:components.URL queue:queue];
}

// NOTE(brentvatne): mainly copied over from socketForLocation to support passing in a bundleURL
static ABI43_0_0RCTReconnectingWebSocket *socketForURL(NSURL *url)
{
  NSURLComponents *const components = [NSURLComponents new];
...
- (void)bundleURLSettingsChanged
{
  ...
  NSString *const serverHostPort = [[ABI43_0_0RCTBundleURLProvider sharedSettings] packagerServerHostPort];
...
- (ABI43_0_0RCTHandlerToken)addNotificationHandler:(ABI43_0_0RCTNotificationHandler)handler
...
- (ABI43_0_0RCTHandlerToken)addRequestHandler:(ABI43_0_0RCTRequestHandler)handler
                               queue:(dispatch_queue_t)queue
                           forMethod:(NSString *)method
{
  std::lock_guard<std::mutex> l(_mutex);
...
- (ABI43_0_0RCTHandlerToken)addConnectedHandler:(ABI43_0_0RCTConnectedHandler)handler queue:(dispatch_queue_t)queue
{
  std::lock_guard<std::mutex> l(_mutex);
  if (_socketConnected) {
...
- (void)removeHandler:(ABI43_0_0RCTHandlerToken)token
{
  std::lock_guard<std::mutex> l(_mutex);
...
template <typename Handler>
static void eraseRegistrationsWithToken(std::vector<Registration<Handler>> &registrations, ABI43_0_0RCTHandlerToken token)
{
  registrations.erase(
      std::remove_if(
...
- (void)addHandler:(id<ABI43_0_0RCTPackagerClientMethod>)handler forMethod:(NSString *)method
{
  dispatch_queue_t queue =
  ...
  [self
      addNotificationHandler:^(NSDictionary<NSString *, id> *notification) {
        [handler handleNotification:notification];
                   ...
                   forMethod:method];
  [self
      addRequestHandler:^(NSDictionary<NSString *, id> *request, ABI43_0_0RCTPackagerClientResponder *responder) {
...
{
  NSArray<NSNumber *> *const kSupportedVersions = @[ @(ABI43_0_0RCT_PACKAGER_CLIENT_PROTOCOL_VERSION) ];
  return [kSupportedVersions containsObject:version];
}

#pragma mark - ABI43_0_0RCTReconnectingWebSocketDelegate

- (void)reconnectingWebSocketDidOpen:(__unused ABI43_0_0RCTReconnectingWebSocket *)webSocket
{
  std::vector<Registration<ABI43_0_0RCTConnectedHandler>> registrations;
  {
    std::lock_guard<std::mutex> l(_mutex);
  ...
  for (const auto &registration : registrations) {
    ...
    ABI43_0_0RCTConnectedHandler handler = registration.handler;
...
- (void)reconnectingWebSocket:(ABI43_0_0RCTReconnectingWebSocket *)webSocket didReceiveMessage:(id)message
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *msg = ABI43_0_0RCTJSONParse(message, &error);

  if (error) {
    ABI43_0_0RCTLogError(@"%@ failed to parse message with error %@\n<message>\n%@\n</message>", [self class], error, msg);
    return;
  }

  if (!isSupportedVersion(msg[@"version"])) {
    ABI43_0_0RCTLogError(@"%@ received message with not supported version %@", [self class], msg[@"version"]);
    return;
  }
  ...
  if (messageId) { // Request
    const std::vector<Registration<ABI43_0_0RCTRequestHandler>> registrations(
        registrationsWithMethod(_mutex, _requestRegistrations, method));
    if (registrations.empty()) {
      ABI43_0_0RCTLogError(@"No handler found for packager method %@", msg[@"method"]);
      [[[ABI43_0_0RCTPackagerClientResponder alloc] initWithId:messageId socket:webSocket]
      ...
      ABI43_0_0RCTRequestHandler handler = registrations.back().handler;
      dispatch_async(registrations.back().queue, ^{
        handler(params, [[ABI43_0_0RCTPackagerClientResponder alloc] initWithId:messageId socket:webSocket]);
      });
    }
  } else { // Notification
    const std::vector<Registration<ABI43_0_0RCTNotificationHandler>> registrations(
        registrationsWithMethod(_mutex, _notificationRegistrations, method));
    for (const auto &registration : registrations) {
      // Beware: don't capture the reference to handler in a dispatched block!
      ABI43_0_0RCTNotificationHandler handler = registration.handler;
      dispatch_async(registration.queue, ^{
        handler(params);
      });
    }
...
- (void)reconnectingWebSocketDidClose:(__unused ABI43_0_0RCTReconnectingWebSocket *)webSocket
{
  std::lock_guard<std::mutex> l(_mutex);
  _socketConnected = NO;
}
