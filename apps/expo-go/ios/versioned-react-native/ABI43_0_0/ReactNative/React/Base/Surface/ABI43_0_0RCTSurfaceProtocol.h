...
@protocol ABI43_0_0RCTSurfaceProtocol <NSObject>

@property (atomic, readonly) ABI43_0_0RCTSurfaceStage stage;
@property (atomic, readonly) NSString *moduleName;
@property (atomic, readwrite, weak, nullable) id<ABI43_0_0RCTSurfaceDelegate> delegate;
 ...
 * `viewportOffset` is ignored in `ABI43_0_0RCTSurface` but used in `ABI43_0_0RCTFabricSurface`.
 */
 ...
 * Those methods are a no-op for regular ABI43_0_0RCTSurface (for now), but all call sites must call them appropriately.
 */
- (BOOL)start;
- (BOOL)stop;
