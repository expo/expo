...
#import "ABI43_0_0RCTSurfaceDelegate.h"
#import "ABI43_0_0RCTSurfaceRootShadowView.h"
#import "ABI43_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI43_0_0RCTSurface () <ABI43_0_0RCTSurfaceRootShadowViewDelegate, ABI43_0_0RCTUIManagerObserver>
@end

@implementation ABI43_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI43_0_0RCTBridge *_batchedBridge;
  ABI43_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI43_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI43_0_0RCTSurfaceView *_Nullable _view;
  ABI43_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI43_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instantiate `ABI43_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI43_0_0RCTAllocateRootViewTag();
    ...
    if (!bridge.loading) {
      _stage = _stage | ABI43_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI43_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI43_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI43_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI43_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI43_0_0RCTSurfaceRootView *rootView = (ABI43_0_0RCTSurfaceRootView *)[self._uiManager viewForABI43_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI43_0_0RCTAssert(
      [rootView isKindOfClass:[ABI43_0_0RCTSurfaceRootView class]],
      @"Received root view is not an instance of `ABI43_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI43_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI43_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI43_0_0RCTSurfaceStage)stage
{
...
- (void)_run
{
  ABI43_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI43_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI43_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI43_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI43_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI43_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI43_0_0RCTIsMainQueue() ? ABI43_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI43_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI43_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI43_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI43_0_0ReactTag:self->_rootViewTag];
    ABI43_0_0RCTAssert(
        [rootShadowView isKindOfClass:[ABI43_0_0RCTSurfaceRootShadowView class]],
        @"Received shadow view is not an instance of `ABI43_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI43_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI43_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI43_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI43_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI43_0_0ReactTag:self->_rootViewTag];
...
{
  // `viewportOffset` is intentionally zero because `ABI43_0_0RCTSurface` ignores it.
  // However, it is needed in `ABI43_0_0RCTFabricSurface`.
...
- (BOOL)synchronouslyWaitForStage:(ABI43_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI43_0_0RCTIsUIManagerQueue()) {
    ABI43_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI43_0_0RCTIsMainQueue() && (stage & ABI43_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI43_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI43_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI43_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI43_0_0RCTAssert(
          NO,
          @"Only waiting for `ABI43_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI43_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI43_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI43_0_0RCTIsMainQueue() && (stage & ABI43_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI43_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI43_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI43_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI43_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI43_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI43_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI43_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI43_0_0RCTUIManager *)manager performMountingWithBlock:(ABI43_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI43_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI43_0_0RCTUIManager *)manager
{
  if (self.stage & ABI43_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI43_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ABI43_0_0React components

- (void)mountABI43_0_0ReactComponentWithBridge:(ABI43_0_0RCTBridge *)bridge
                           moduleName:(NSString *)moduleName
...
- (void)unmountABI43_0_0ReactComponentWithBridge:(ABI43_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
