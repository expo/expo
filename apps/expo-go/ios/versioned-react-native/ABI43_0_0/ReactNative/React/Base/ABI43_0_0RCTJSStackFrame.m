 ...
 */
static NSRegularExpression *ABI43_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI43_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI43_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI43_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI43_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI43_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI43_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI43_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI43_0_0RCTNilIfNull(dict[@"collapse"]) boolValue]];
}

+ (NSArray<ABI43_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI43_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI43_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI43_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
