...
typedef void (^ABI43_0_0RCTPropBlock)(id<ABI43_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI43_0_0RCTPropBlock> ABI43_0_0RCTPropBlockDictionary;
...
@implementation ABI43_0_0RCTComponentData {
  id<ABI43_0_0RCTComponent> _defaultView; // Only needed for ABI43_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI43_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI43_0_0RCTPropBlockDictionary *_shadowPropBlocks;
...
- (ABI43_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI43_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI43_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI43_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI43_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI43_0_0RCTPropBlock
createEventSetter(NSString *propName, SEL setter, InterceptorBlock eventInterceptor, ABI43_0_0RCTBridge *bridge)
{
  __weak ABI43_0_0RCTBridge *weakBridge = bridge;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI43_0_0RCTConvert BOOL:json]) {
      __weak id<ABI43_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI43_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI43_0_0ReactTag);
        } else {
          ABI43_0_0RCTComponentEvent *componentEvent = [[ABI43_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI43_0_0ReactTag
                                                                                 body:event];
...
static ABI43_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI43_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI43_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI43_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI43_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI43_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock = createEventSetter(name, setter, self.eventInterceptor, _bridge);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI43_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI43_0_0RCTLogError(@"No +[ABI43_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI43_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI43_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI43_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI43_0_0RCT_CASE(_C_SEL, SEL)
        ABI43_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI43_0_0RCT_CASE(_C_CHR, char)
        ABI43_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI43_0_0RCT_CASE(_C_SHT, short)
        ABI43_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI43_0_0RCT_CASE(_C_INT, int)
        ABI43_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI43_0_0RCT_CASE(_C_LNG, long)
        ABI43_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI43_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI43_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI43_0_0RCT_CASE(_C_FLT, float)
        ABI43_0_0RCT_CASE(_C_DBL, double)
        ABI43_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI43_0_0RCT_CASE(_C_PTR, void *)
        ABI43_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI43_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI43_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI43_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI43_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI43_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI43_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI43_0_0ReactTag];
      ABI43_0_0RCTPerformBlockWithLogPrefix(
          ^{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI43_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI43_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI43_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
    ...
    if ([type isEqualToString:@"ABI43_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI43_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI43_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI43_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
...
{
  // Hackety hack, this partially re-implements ABI43_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI43_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI43_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI43_0_0EX_REMOVE_VERSION(name);
  ...
  ABI43_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
