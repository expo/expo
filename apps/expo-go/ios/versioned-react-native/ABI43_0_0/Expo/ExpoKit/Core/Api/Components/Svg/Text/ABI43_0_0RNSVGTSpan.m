...
#import "ABI43_0_0RNSVGTopAlignedLabel.h"
#import "ABI43_0_0RNSVGPathMeasure.h"
#import "ABI43_0_0RNSVGFontData.h"

static NSCharacterSet *ABI43_0_0RNSVGTSpan_separators = nil;
static CGFloat ABI43_0_0RNSVGTSpan_radToDeg = 180 / (CGFloat)M_PI;

@implementation ABI43_0_0RNSVGTSpan
{
    CGFloat startOffset;
    ABI43_0_0RNSVGTextPath *textPath;
    ...
    CGFloat firstX;
    CGFloat firstY;
    ABI43_0_0RNSVGPathMeasure *measure;
}
...
- (id)init
{
    ...
    if (ABI43_0_0RNSVGTSpan_separators == nil) {
        ABI43_0_0RNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
    }

    emoji = [NSMutableArray arrayWithCapacity:0];
    emojiTransform = [NSMutableArray arrayWithCapacity:0];
    measure = [[ABI43_0_0RNSVGPathMeasure alloc]init];
...
{
    if (self.content) {
        ABI43_0_0RNSVGGlyphContext* gc = [self.textRoot getGlyphContext];
        if (self.inlineSize != nil && self.inlineSize.value != 0) {
            CGColorRef color;
            if (self.fill) {
                if (self.fill.class == ABI43_0_0RNSVGBrush.class) {
            ...
            if (self.stroke) {
                if (self.stroke.class == ABI43_0_0RNSVGBrush.class) {
                    color = [self.tintColor CGColor];
                    [self drawWrappedText:context gc:gc rect:rect color:color];
                } else {
            ...
            if (self.path) {
                ...
                for (NSUInteger i = 0; i < count; i++) {
                    ABI43_0_0RNSVGPlatformView *label = [emoji objectAtIndex:i];
...
- (NSMutableDictionary *)getAttributes:(ABI43_0_0RNSVGFontData *)fontdata {
    ...
    bool allowOptionalLigatures = letterSpacing == 0 && fontdata->fontVariantLigatures == ABI43_0_0RNSVGFontVariantLigaturesNormal;
...
static ABI43_0_0RNSVGTopAlignedLabel *label;
- (void)drawWrappedText:(CGContextRef)context gc:(ABI43_0_0RNSVGGlyphContext *)gc rect:(CGRect)rect color:(CGColorRef)color {
    [self pushGlyphContext];
    if (fontRef != nil) {
         CFRelease(fontRef);
     }
    fontRef = [self getFontFromContext];
    ABI43_0_0RNSVGFontData* fontdata = [gc getFont];
    ...
    enum ABI43_0_0RNSVGTextAnchor textAnchor = fontdata->textAnchor;
    NSTextAlignment align;
    switch (textAnchor) {
        case ABI43_0_0RNSVGTextAnchorStart:
            align = NSTextAlignmentLeft;
            break;

        case ABI43_0_0RNSVGTextAnchorMiddle:
            align = NSTextAlignmentCenter;
            break;

        case ABI43_0_0RNSVGTextAnchorEnd:
    ...
    if (!label) {
        label = [[ABI43_0_0RNSVGTopAlignedLabel alloc] init];
    }
    label.attributedText = (__bridge NSAttributedString * _Nullable)(attrString);
    label.lineBreakMode = NSLineBreakByWordWrapping;
    label.backgroundColor = ABI43_0_0RNSVGColor.clearColor;
...
#endif
    label.font = font;
    label.textColor = [ABI43_0_0RNSVGColor colorWithCGColor:color];

    CGFloat fontSize = [gc getFontSize];
    CGFloat height = CGRectGetHeight(rect);
    CGFloat width = [ABI43_0_0RNSVGPropHelper fromRelative:self.inlineSize
...
- (CGFloat)getSubtreeTextChunksTotalAdvance
{
    ...
    if (!str) {
        for (ABI43_0_0RNSVGView *node in self.subviews) {
            if ([node isKindOfClass:[ABI43_0_0RNSVGText class]]) {
                ABI43_0_0RNSVGText *text = (ABI43_0_0RNSVGText*)node;
...
- (CGPathRef)getLinePath:(NSString *)str context:(CGContextRef)context
{
    ...
    ABI43_0_0RNSVGGlyphContext *gc = [self.textRoot getGlyphContext];
    ABI43_0_0RNSVGFontData *font = [gc getFont];
     ...
     */
    bool allowOptionalLigatures = letterSpacing == 0 && font->fontVariantLigatures == ABI43_0_0RNSVGFontVariantLigaturesNormal;
     ...
     */
    enum ABI43_0_0RNSVGTextAnchor textAnchor = font->textAnchor;
    ABI43_0_0RNSVGText *anchorRoot = [self getTextAnchorRoot];
    CGFloat textMeasure = [anchorRoot getSubtreeTextChunksTotalAdvance];
    CGFloat offset = [ABI43_0_0RNSVGTSpan getTextAnchorOffset:textAnchor width:textMeasure];
    ...
    if (hasTextPath) {
        //sharpMidLine = ABI43_0_0RNSVGTextPathMidLineFromString([textPath midLine]) == ABI43_0_0RNSVGTextPathMidLineSharp;
        /*
         Name
         side
         Value
         ...
         */
        side = ABI43_0_0RNSVGTextPathSideFromString([textPath side]) == ABI43_0_0RNSVGTextPathSideRight ? -1 : 1;
        /*
         Name
         startOffset
         Value
         ...
         the path is reached.
         */
        CGFloat absoluteStartOffset = [ABI43_0_0RNSVGPropHelper fromRelative:textPath.startOffset
                                                          relative:measure.pathLength
                                                          fontSize:fontSize];
        offset += absoluteStartOffset;
        if (measure.isClosed) {
            CGFloat halfPathDistance = measure.pathLength / 2;
            startOfRendering = absoluteStartOffset + (textAnchor == ABI43_0_0RNSVGTextAnchorMiddle ? -halfPathDistance : 0);
            endOfRendering = startOfRendering + measure.pathLength;
        }
        /*
         ABI43_0_0RNSVGTextPathSpacing spacing = textPath.getSpacing();
         if (spacing == ABI43_0_0RNSVGTextPathSpacing.auto) {
         // Hmm, what to do here?
    ...
    ABI43_0_0RNSVGLength *mTextLength = [self textLength];
    enum ABI43_0_0RNSVGTextLengthAdjust mLengthAdjust = ABI43_0_0RNSVGTextLengthAdjustFromString([self lengthAdjust]);
    if (mTextLength != nil) {
        CGFloat author = [ABI43_0_0RNSVGPropHelper fromRelative:mTextLength
                                             relative:[gc getWidth]
                                             fontSize:fontSize];
        if (author < 0) {
        ...
        switch (mLengthAdjust) {
            default:
            case ABI43_0_0RNSVGTextLengthAdjustSpacing:
                // TODO account for ligatures
                letterSpacing += (author - textMeasure) / (n - 1);
                break;
            case ABI43_0_0RNSVGTextLengthAdjustSpacingAndGlyphs:
    ...
    */
    ...
    enum ABI43_0_0RNSVGAlignmentBaseline baseline = ABI43_0_0RNSVGAlignmentBaselineFromString(self.alignmentBaseline);
    if (baseline != ABI43_0_0RNSVGAlignmentBaselineBaseline) {
        ...
        switch (baseline) {
            ...
            default:
            case ABI43_0_0RNSVGAlignmentBaselineBaseline:
                // Use the dominant baseline choice of the parent.
                // Match the box’s corresponding baseline to that of its parent.
                baselineShift = 0;
                break;

            case ABI43_0_0RNSVGAlignmentBaselineTextBottom:
            case ABI43_0_0RNSVGAlignmentBaselineAfterEdge:
            case ABI43_0_0RNSVGAlignmentBaselineTextAfterEdge:
            ...
            case ABI43_0_0RNSVGAlignmentBaselineCentral:
            ...
            case ABI43_0_0RNSVGAlignmentBaselineMathematical:
            ...
            case ABI43_0_0RNSVGAlignmentBaselineHanging:
                baselineShift = (CGFloat)0.8 * ascenderHeight;
                break;

            case ABI43_0_0RNSVGAlignmentBaselineTextTop:
            case ABI43_0_0RNSVGAlignmentBaselineBeforeEdge:
            case ABI43_0_0RNSVGAlignmentBaselineTextBeforeEdge:
            ...
            case ABI43_0_0RNSVGAlignmentBaselineBottom:
                // Align the top of the aligned subtree with the top of the line box.
                baselineShift = bottom;
                break;

            case ABI43_0_0RNSVGAlignmentBaselineCenter:
                // Align the center of the aligned subtree with the center of the line box.
                baselineShift = totalHeight / 2;
                break;

            case ABI43_0_0RNSVGAlignmentBaselineTop:
        ...
        switch (baseline) {
            ...
            default:
                if (fontData != nil && [baselineShiftString isEqualToString:@"sub"]) {
                    ...
                    baselineShift -= [ABI43_0_0RNSVGPropHelper fromRelativeWithNSString:baselineShiftString
                                                                   relative:fontSize
                                                                   fontSize:fontSize];
                }
                break;
    ...
    for (CFIndex ri = 0; ri < runEnd; ri++) {
        ...
        for(CFIndex g = 0; g < runGlyphCount; g++) {
            ...
            bool isWordSeparator = [ABI43_0_0RNSVGTSpan_separators characterIsMember:currentChar];
            ...
            CGFloat r = [gc nextRotation] / ABI43_0_0RNSVGTSpan_radToDeg;
            ...
            if (width == 0) { // Render unicode emoji
                ABI43_0_0RNSVGTextView *label = [[ABI43_0_0RNSVGTextView alloc] init];
...
+ (CGFloat)getTextAnchorOffset:(ABI43_0_0RNSVGTextAnchor)textAnchor width:(CGFloat) width
{
    switch (textAnchor) {
        case ABI43_0_0RNSVGTextAnchorStart:
            return 0;
...
{
    textPath = nil;
    ABI43_0_0RNSVGText *parent = (ABI43_0_0RNSVGText*)[self superview];
    CGPathRef path = nil;
    while (parent) {
        if ([parent class] == [ABI43_0_0RNSVGTextPath class]) {
            textPath = (ABI43_0_0RNSVGTextPath*) parent;
            ABI43_0_0RNSVGNode *template = [self.svgView getDefinedTemplate:textPath.href];
            path = [template getPath:context];
            [measure extractPathData:path];
            break;
        } else if (![parent isKindOfClass:[ABI43_0_0RNSVGText class]]) {
            break;
        }
        parent = (ABI43_0_0RNSVGText*)[parent superview];
    }
    if (!path) {
        [measure reset];
    }
