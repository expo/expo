...
const fs = {
  ...
  readFileSync(p) {
    touches.push('readFileSync ' + p);
    throw enoent('open', p);
  },
  readdirSync(p) {
    touches.push('readdirSync ' + p);
    throw enoent('scandir', p);
  },
  realpathSync: Object.assign(
    function realpathSync(p) {
      touches.push('realpathSync ' + p);
      return String(p);
    },
    {
      native: function (p) {
      ...
      get(_t, key) {
        ...
        return function () {
          return Promise.reject(
            new Error('fs.promises.' + String(key) + ' is not available in the Hermes runtime')
          );
        };
      },
    }
