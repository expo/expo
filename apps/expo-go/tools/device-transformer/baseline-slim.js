...
console.log(
  'PROBES ' +
    (normal.code === isolatedCode
      ? 'DO NOT AFFECT OUTPUT (safe to skip in Hermes)'
      : 'CHANGE OUTPUT (must bundle plugins)')
);
if (normal.code !== isolatedCode) {
  let i = 0;
  while (
    i < Math.min(normal.code.length, isolatedCode.length) &&
    normal.code[i] === isolatedCode[i]
  )
    i++;
