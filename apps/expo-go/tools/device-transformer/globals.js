...
(function () {
  ...
  if (typeof g.setTimeout !== 'function') {
    ...
    g.setInterval = () => {
      throw new Error('setInterval is not available in the Hermes runtime');
    };
    g.clearInterval = () => {};
  ...
  function base64Decode(str) {
    ...
    for (let i = 0; i < str.length; i += 4) {
      const n =
        ...
        ((B64.indexOf(str[i + 2]) & 63) << 6) |
        (B64.indexOf(str[i + 3]) & 63);
      out.push((n >> 16) & 255);
  ...
  if (typeof g.Buffer === 'undefined') {
    class Buffer extends Uint8Array {
      ...
      toString(enc, start, end) {
        ...
        if (enc === 'latin1' || enc === 'binary' || enc === 'ascii') {
          ...
          for (let i = start; i < end; i++)
            out += String.fromCharCode(enc === 'ascii' ? this[i] & 127 : this[i]);
          return out;
        }
      ...
      write() {
        throw new Error('Buffer.write not supported in Hermes runtime');
      }
    }
    g.Buffer = Buffer;
  }
  ...
  if (typeof g.process === 'undefined') {
    ...
    const proc = {
      ...
      listeners: () => [],
      binding: () => {
        throw new Error('process.binding not supported');
      },
      stdout: {
        write: (s) => {
          g.console.log(String(s).replace(/\n$/, ''));
          return true;
        },
        isTTY: false,
      },
  ...
  if (typeof g.require === 'undefined') {
    const globalRequire = function require(id) {
      ...
      const e = new Error(
        "Cannot find module '" + id + "' (unregistered dynamic require in Hermes runtime)"
      );
    ...
    globalRequire.resolve = function resolve(id) {
      ...
      const e = new Error(
        "Cannot find module '" + id + "' (unregistered require.resolve in Hermes runtime)"
      );
      e.code = 'MODULE_NOT_FOUND';
      throw e;
    };
          ...
          return function () {
            throw new Error(
              "node builtin '" +
                ...
                '.' +
                String(key) +
                "' is not available in the Hermes runtime"
            );
          };
        },
      }
