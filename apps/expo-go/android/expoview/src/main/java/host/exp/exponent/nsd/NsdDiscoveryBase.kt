  ...
  private fun stopDiscoveryOnly() {
    ...
    if (listener != null) {
      runCatching {
        ...
        Log.e("ExpoGo", "Failed to stop NSD discovery", it)
      }
    }
    cancelServiceJobs()
    healthCheckActive.value = false
...
/**
 * An extension which converts OkHttp requests to coroutines.
 */
internal suspend inline fun Request.await(okHttpClient: OkHttpClient): Response {
