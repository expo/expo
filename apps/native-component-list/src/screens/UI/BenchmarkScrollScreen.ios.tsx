      ...
      <ExpoScrollView>
        <VStack spacing={8}>
          {Array.from({ length: count }, (_, i) => (
            <ZStack
              key={i}
              modifiers={[frame({ height: 60 }), padding({ horizontal: 16 })]}>
              <RoundedRectangle
                cornerRadius={8}
    ...
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.note}>
          expo-ui SwiftUI rendering continues after JS commit. For UI-thread FPS and memory, profile
          with Xcode Instruments on a physical device.
        </Text>
        <View style={styles.row}>
          <SegButton label="expo-ui" active={variant === 'expo-ui'} onPress={() => setVariant('expo-ui')} />
