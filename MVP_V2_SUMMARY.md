# MiniApp Container MVP v2 - 完全解耦方案

## 🎉 重大改进：独立的开发启动器包

基于您的建议，我们创建了一个**完全独立**的 `expo-dev-miniapp-launcher` 包，彻底解决了更新冲突问题！

---

## 📦 新架构概览

### 之前的方案 (v1) ❌

```
修改 expo-dev-launcher
  ├─ HomeScreen.kt (修改)
  └─ 问题：更新 expo 仓库时会有冲突
```

### 现在的方案 (v2) ✅

```
创建 expo-dev-miniapp-launcher (独立包)
  ├─ 完全独立的代码库
  ├─ 自己的 UI 实现
  ├─ 独立的版本控制
  └─ 零冲突！
```

---

## 🏗️ 包结构

### 新包：expo-dev-miniapp-launcher

```
expo-miniapp/expo-dev-miniapp-launcher/
├── android/
│   ├── build.gradle
│   └── src/
│       ├── main/
│       │   ├── AndroidManifest.xml
│       │   ├── DevMiniAppLauncherModule.kt      # Expo 模块
│       │   └── DevMiniAppLauncherPackage.kt     # 包定义
│       └── debug/
│           └── ui/
│               └── MiniAppHomeScreen.kt         # 🎨 自定义 UI
│
├── ios/                                         # TODO: iOS 实现
│
├── plugin/
│   └── src/
│       └── index.ts                             # Config plugin
│
├── package.json
├── expo-module.config.json
└── README.md
```

---

## 🎨 自定义 UI 特性

### MiniAppHomeScreen 包含：

1. **渐变欢迎横幅**
   - 🚀 品牌标识
   - 紫色到紫罗兰渐变
   - 清晰的功能说明

2. **扫描二维码**
   - 📷 大按钮，易于点击
   - 连接到开发服务器或加载小程序

3. **手动输入 URL**
   - ✍️ 输入框 + 连接按钮
   - 占位符示例：`http://192.168.1.100:8081`

4. **最近使用列表**
   - 📚 显示最近连接的应用
   - 快速重新连接

5. **提示卡片**
   - 💡 网络连接提示
   - 使用指南

### UI 预览

```
┌─────────────────────────────────────┐
│  🚀                                 │
│  MiniApp Container                  │
│  动态加载小程序，随时随地运行         │
│  完全兼容 Expo 开发模式              │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│  📷  扫描二维码                      │
│      连接到开发服务器或加载小程序      │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│  手动输入开发服务器地址               │
│  ┌─────────────────────────────┐   │
│  │ http://192.168.1.100:8081   │   │
│  └─────────────────────────────┘   │
│  [         连接          ]         │
└─────────────────────────────────────┘

最近使用
┌─────────────────────────────────────┐
│  📱 http://192.168.1.100:8081      │
└─────────────────────────────────────┘
```

---

## 🚀 使用方法

### 1. 创建新项目

```bash
npx create-expo-miniapp-container my-container
```

生成的项目自动配置了 `expo-dev-miniapp-launcher`！

### 2. 生成的 package.json

```json
{
  "dependencies": {
    "expo": "~52.0.0",
    "expo-dev-miniapp-launcher": "file:../../expo/expo-miniapp/expo-dev-miniapp-launcher",
    "react": "18.3.1",
    "react-native": "0.76.5"
  }
}
```

### 3. 生成的 app.json

```json
{
  "expo": {
    "plugins": [
      [
        "expo-dev-miniapp-launcher",
        {
          "enabled": true,
          "launchMode": "launcher"
        }
      ]
    ]
  }
}
```

### 4. 运行项目

```bash
cd my-container
npm install
npx expo prebuild
npx expo run:android
```

---

## ✅ 优势对比

| 特性 | 修改 expo-dev-launcher | expo-dev-miniapp-launcher |
|------|----------------------|---------------------------|
| **更新冲突** | ❌ 有冲突 | ✅ 零冲突 |
| **自定义程度** | ⚠️ 有限 | ✅ 完全控制 |
| **版本管理** | ❌ 依赖上游 | ✅ 独立版本 |
| **维护性** | ⚠️ 需要同步 | ✅ 独立维护 |
| **测试** | ⚠️ 复杂 | ✅ 简单 |
| **发布** | ❌ 不能单独发布 | ✅ 独立发布 |

---

## 🔧 开发工作流

### 修改 UI

1. 编辑 `expo-miniapp/expo-dev-miniapp-launcher/android/src/debug/ui/MiniAppHomeScreen.kt`
2. 重新构建项目
3. 运行测试项目查看效果

### 测试新功能

```bash
# 1. 构建包
cd expo-miniapp/expo-dev-miniapp-launcher/android
./gradlew assembleDebug

# 2. 在测试项目中使用
cd my-test-container
npm install
npx expo run:android
```

### 更新 Expo 仓库

```bash
# 再也不用担心冲突了！
git pull upstream main
# 你的 expo-dev-miniapp-launcher 完全独立，不受影响
```

---

## 📊 代码统计

```
新增文件：12 个
新增代码：614 行
删除冲突代码：47 行

expo-miniapp/expo-dev-miniapp-launcher/
├── Android 模块：3 个文件
├── UI 组件：1 个文件 (250+ 行)
├── Config 插件：2 个文件
└── 文档：1 个文件 (150+ 行)
```

---

## 🎯 实现细节

### 1. Expo 模块集成

```kotlin
// DevMiniAppLauncherModule.kt
class DevMiniAppLauncherModule : Module() {
  override fun definition() = ModuleDefinition {
    Name("DevMiniAppLauncher")

    Constants(
      "VERSION" to BuildConfig.VERSION
    )

    Function("openLauncher") {
      // 打开启动器
    }
  }
}
```

### 2. Compose UI

```kotlin
@Composable
fun MiniAppHomeScreen(
    onScanQRCode: () -> Unit,
    onConnectToServer: (String) -> Unit,
    recentApps: List<String>
) {
    // 完全自定义的 UI
    // 使用 Material Design 3
    // Jetpack Compose
}
```

### 3. Config Plugin

```typescript
const withMiniAppLauncher: ConfigPlugin = (config, props) => {
  // 配置 app.json
  config.extra.miniAppLauncher = {
    launchMode: props.launchMode,
    enabled: props.enabled,
  };
  return config;
};
```

---

## 🚦 下一步计划

### Phase 1: 完善基础功能 ✅ (当前)

- [x] 创建独立包结构
- [x] 实现自定义 Android UI
- [x] 创建 Config Plugin
- [x] 更新 CLI 工具
- [x] 编写文档

### Phase 2: 功能增强

- [ ] iOS UI 实现 (SwiftUI)
- [ ] QR 码扫描功能
- [ ] 开发服务器自动发现
- [ ] 最近应用持久化

### Phase 3: 小程序加载

- [ ] Manifest 解析器
- [ ] Bundle 加载器
- [ ] 离线包安装
- [ ] 权限管理

### Phase 4: 高级功能

- [ ] Hot Reload
- [ ] DevTools 集成
- [ ] 性能监控
- [ ] 错误追踪

---

## 📝 使用示例

### 从 QR 码加载

```bash
# 终端 1：启动开发服务器
cd my-miniapp
npx expo start

# 终端 2：在容器中扫码
cd my-container
npx expo run:android
# 点击 "扫描二维码" 按钮
# 扫描终端 1 显示的二维码
```

### 手动连接

```bash
# 在容器中手动输入
http://192.168.1.100:8081
# 点击 "连接" 按钮
```

---

## 🔍 关键文件说明

### MiniAppHomeScreen.kt (250+ 行)

这是核心 UI 文件，包含：
- Composable 函数
- Material Design 组件
- 事件处理
- 状态管理

**特点：**
- 纯 Kotlin + Compose
- 响应式设计
- 支持 Preview
- 易于测试

### DevMiniAppLauncherModule.kt

Expo 模块定义：
- 暴露给 JS 的 API
- 常量定义
- 函数实现

### Config Plugin (plugin/src/index.ts)

配置插件：
- 读取 app.json 配置
- 修改原生项目配置
- 注入运行时参数

---

## 💡 最佳实践

### 1. 本地开发

```bash
# 使用 file: 协议链接本地包
"expo-dev-miniapp-launcher": "file:../../expo/expo-miniapp/expo-dev-miniapp-launcher"
```

### 2. 发布到 npm

```bash
cd expo-miniapp/expo-dev-miniapp-launcher
npm publish
```

### 3. 版本管理

遵循语义化版本：
- `0.1.0` - MVP
- `0.2.0` - iOS 支持
- `1.0.0` - 生产就绪

---

## 🎊 成果总结

我们成功创建了一个：

1. ✅ **完全独立**的开发启动器包
2. ✅ **零冲突**的更新策略
3. ✅ **美观现代**的自定义 UI
4. ✅ **易于使用**的 CLI 工具
5. ✅ **文档齐全**的项目结构

**最重要的是：再也不用担心更新 Expo 仓库时的冲突了！** 🎉

---

## 📚 相关文档

- [expo-dev-miniapp-launcher README](./expo-miniapp/expo-dev-miniapp-launcher/README.md)
- [create-expo-miniapp-container 使用指南](./expo-miniapp/create-expo-miniapp-container/README.md)
- [测试指南](./expo-miniapp/create-expo-miniapp-container/TESTING.md)

---

## 🔗 Git 信息

**分支:** `claude/miniapp-container-mvp-01CqTgFZ1wJ6EbFTWwqr5FvH`

**提交:**
- `beaaa7b9` - 文档
- `65450c08` - MVP v1 (已废弃)
- `35ee6591` - MVP v2 (当前，独立包方案)

**推送:**
```bash
git push -u origin claude/miniapp-container-mvp-01CqTgFZ1wJ6EbFTWwqr5FvH
```

---

**MVP v2 状态:** ✅ **完成并准备测试**

**主要改进:** 从修改上游包改为创建独立包，彻底解决冲突问题！
