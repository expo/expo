...
5. **Environment overreach.** This runs on a hosted iOS simulator. Anything concluded about Android, about physical hardware, or about a Release build that was never made must be stated as inference, not observation. Check the caveats actually cover what the report concludes. A Windows conclusion needs a Windows measurement: the run had `create_gha_sandbox` (a hosted Windows VM) available, so a Windows-specific claim backed only by Linux-sandbox evidence is overreach unless the report says the Windows VM was refused or why it was not needed. The reverse is also a defect: spending a Windows VM on behavior the Linux sandbox exhibits identically is padding.
