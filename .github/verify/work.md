...
When the changed behavior is Windows-specific — file watching, path separators or 8.3 short names, case-insensitive filesystems, `EPERM`-on-delete, CRLF — the Linux sandbox cannot falsify the claim. Attach one Windows VM to your session with `create_gha_sandbox` and run the focused checks there too (the FallbackWatcher change in [#48954](https://github.com/expo/expo/pull/48954) is the canonical case). This is the exception, not the default: for anything the Linux sandbox can exhibit, stay on it. Windows VM mechanics: the VM is EMPTY (clone and install by the project's own lockfile; node, npm, and git are on the image, bun/pnpm/yarn are not); `sandbox_exec` there is `cmd /c`, not bash or PowerShell (wrap PowerShell explicitly as `powershell -NoProfile -Command "..."`), with paths relative to the job workdir; route with `on: "gha"` when the session also has the Linux sandbox; one per session, ~30-minute job lifetime, billed while idle, torn down by `destroy_sandbox`. If the server refuses for lack of GHA access, report the Windows arm as not run — do not present Linux results as Windows evidence.
