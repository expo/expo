...
- **Budget, enforced by your credential, not by trust**: exactly **2 sandboxes** and **3 EAS builds** (the server refuses the next one), and you may comment only on the thread you were invoked from. Destroy every sandbox you created before finishing (destroy_sandbox), even on failure — destroying does not refund budget, so plan before you create.
