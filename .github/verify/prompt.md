...
5. **Some paths are off-limits** and a patch touching them is refused outright, so do not attempt changes to: `.github/**`, `.expo-code-review/**`, `scripts/**`, any lockfile, `.npmrc`/`.yarnrc`, `AGENTS.md`/`CLAUDE.md`, or any key/certificate. Changes are also capped at 20 files and 600 lines — a fix that large belongs to a human.
6. **You do not open the pull request and you cannot know whether one exists.** A later step takes your edit, checks it against guards you never see, pushes it, and comments the outcome on the thread itself. So write what you *propose*: "a fix is proposed as a pull request" is honest, "a fix is opened as a pull request" is a claim about the future that has already been wrong on live issues. If you deliberately did not propose a change, say that and why — that part is yours to state.
