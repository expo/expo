---
title: Patterns to deploy your app
---
...
There are a three parts that make up a successful deployment process that we'll need to consider:

1. Creating builds
   1. We can create builds for production use only.
   2. We can create builds for production use and separate builds for testing use.
2. Testing changes
   1. We can test changes with TestFlight and Play Store Internal Track.
   2. We can test changes with an internal distribution build.
   3. We can test changes with Expo Go or a development app.
3. Publishing updates
   1. We can publish updates without considering branch names.
   2. We can create update branches that are environment based, like `production` and `staging`.
   3. We can create update branches that are version based, like `version-1.0`, which enables us to promote updates from one channel to another.
...
Below, we've outline four patterns on how to deploy a project with Expo and EAS. We found that successful Expo projects often use one of these patterns, or a variation of them.
...
Publishing updates: (a) Publish updates without considering branch names.
...
This flow focuses on de-risking your deployments and preserving previous versions of your app. It's great for medium- to large-sized projects. Here are the parts of the deployment process above that make up this flow:
...
- Use the website UI or CLI to point the `staging` channel at the EAS Update branch `version-1.0`. Test the update by opening the apps on TestFlight and the Play Store Internal Track.
- When ready, use the website UI or CLI to point the `production` channel at the EAS Update branch `version-1.0`.
...
- When ready, use the website UI or CLI to point the `production` channel at the EAS Update branch `version-1.1`.
...
This flow is a slight variation of the "Branch flow". The main difference is that it does not require bookkeeping branches. Instead, we'll have a persistent `staging` and `production` branches we can merge into forever. It's great for medium- projects to large-sized projects. Here are the parts of the deployment process above that make up this flow:
...
- This flow is an even balance between safety and speed.
...
This flow is for projects that need to build and update their iOS and Android apps separately all the time. It will result in separate commands for delivering updates to the iOS and Android apps. Here are the parts of the deployment process above that make up this flow:
