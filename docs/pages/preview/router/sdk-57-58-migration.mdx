...
SDK 58 changes how Expo Router constructs navigation state and integrates with navigators. Apps that use `Stack`, `Link`, and `useRouter` may need a few changes. Apps that import from `expo-router/react-navigation`, access navigation state, or implement custom routers and navigators require closer review.

Use this checklist to find the sections that apply to your app:

- For common app migrations, prefer [`useRouter`](#prefer-userouter-in-components), replace nested navigation with [hrefs](#navigate-with-complete-hrefs), and review the [layout and navigation changes](#move-initialroutename-to-unstable_settings).
...
Every queued JavaScript navigation, including the first navigation to a route, runs in a React transition. When the destination suspends, the current screen stays visible instead of showing `SuspenseFallback`. Consecutive navigation calls are processed by the same queue.
...
The hook reports queued calls such as `router.back()`, but it does not report synchronous dispatches or native back gestures. It returns `false` outside an Expo Router root.
...
Most apps that use protected routes do not need to make changes. Protected routes remain registered in their navigator in SDK 58, and a route whose guard fails renders a redirect instead of being removed from the route tree.
...
The `freezeOnBlur` option has no effect in SDK 58. Remove it from your screen configuration. To preserve state while cleaning up effects after a screen loses focus, wrap the screen contents in the unstable `NavigationAwareActivity` component:
...
The `hideWhenNestedAtLevel` value of `1` hides the activity when the screen loses focus. Its default value of `2` keeps the previous screen visible and hides it when another screen is pushed on top.

{/* TODO: Replace the manual wrapper when a navigator option for NavigationAwareActivity is available. */}

### Prevent a screen from being removed

Existing `usePreventRemove` calls only need changes when their callback repeats or replaces the blocked action. In that case, set the prevention condition to `false` and call the returned `disablePrevention` function before navigating again.
