...
The session type determines which interface EAS provisions with the remote device. On iOS, every type includes a web preview. Choose agent-device, Argent, or Appium to add programmatic control, or choose `serve-sim` for the web preview only:
...
<Terminal
  cmd={[
    '$ eas simulator:start --platform ios --type serve-sim --name "Design review" --non-interactive',
  ]}
/>
...
| `--device`                  | iOS Simulator name or UDID, or Android virtual device hardware profile. The runner chooses a device when omitted                                                        |
...
| `--type`                    | Session type. On iOS, every type includes a web preview. Use `agent-device`, `appium`, or `argent` to add programmatic control, or `serve-sim` for the web preview only |
...
| `--type`     | `agent-device`, `appium`, `argent`, `serve-sim` |
