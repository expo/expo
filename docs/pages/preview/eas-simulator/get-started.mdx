...
Choose the controller you plan to use for installation and device actions. On iOS, each controller-backed session also includes a web preview. If you only need the web preview, use `serve-sim` without a controller. Pass the session type explicitly when you start the session.
...
Use `--platform android` for an Android Emulator. Android sessions do not currently include a live browser preview. Use `--device` to select a specific iOS Simulator name or UDID, or an Android virtual device hardware profile. If the flag is omitted, the remote runner chooses a device.
...
The start command waits for the selected session type to become ready, then writes **.env.eas-simulator**. It also prints:
