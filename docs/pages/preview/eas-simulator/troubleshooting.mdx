...
### A recently documented flag is rejected

The EAS Simulator preview changes quickly, and an older EAS CLI can recognize `simulator:start` without recognizing newer flags such as `--name`, `--device`, or the application launch options. Update EAS CLI, then compare the command with the installed help:
...
<Terminal
  cmd={['$ eas simulator:list --status in-progress', '$ eas simulator:stop --id <old-session-id>']}
/>
...
- `--build-id <build-id>` for an EAS Build
- `--application-archive-url <url>` for a remote application archive
...
Run the command from an Expo project with a valid app config, or pass `--sdk-version <version>` together with `--expo-go`. The SDK override is not valid with `--build-id` or `--application-archive-url`.
