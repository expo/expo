...
**Choose at most one application source:** `buildId`, `applicationArchiveUrl`, or `expoGo: true`. These options are mutually exclusive. Providing more than one returns `400`. You can omit all three to start with a blank device.
...
| `buildId`               | string (UUID) | No       | An EAS Build to download, install, and launch. Use an Android **.apk** or an iOS Simulator build. Cannot be combined with `applicationArchiveUrl` or `expoGo: true`.                                                               |
| `applicationArchiveUrl` | string        | No       | An HTTP or HTTPS URL for an installable application archive, up to 2,048 characters. Cannot be combined with `buildId` or `expoGo: true`.                                                                                          |
| `expoGo`                | boolean       | No       | Set to `true` to install and launch Expo Go. When `true`, cannot be combined with `buildId` or `applicationArchiveUrl`.                                                                                                            |
...
Automatic application setup applies to `agent-device`, `argent`, and `serve-sim`. For `appium`, install and launch the app through your Appium client.
