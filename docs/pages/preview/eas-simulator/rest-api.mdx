...
| `type`                  | string        | No       | `agent-device`, `appium`, `argent`, or `web-preview-only`. Defaults to `agent-device`. The removed `serve-sim` type returns `400`. Use `web-preview-only` instead.                                                                 |
...
A `web-preview-only` session provides the web preview without a controller. It does not support an idle timeout. Other session types provide a controller and include a web preview. Sessions created before `serve-sim` was removed still return `"type": "serve-sim"`.
...
Open `webPreviewUrl` in a desktop browser to view the live iOS Simulator or Android Emulator. Do not open it on the remote device.
