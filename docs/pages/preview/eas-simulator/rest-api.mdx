...
A `web-preview-only` session provides the web preview without a controller. It does not support an idle timeout. Other session types provide a controller and include a web preview. The deprecated `serve-sim` type is iOS-only and returns `previewUrl` instead of `webPreviewUrl`.
...
Open `webPreviewUrl` (or `previewUrl` for a deprecated `serve-sim` session) in a desktop browser to view the live iOS Simulator or Android Emulator. Do not open it on the remote device.
