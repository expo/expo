...
> **warning** Session responses can include controller credentials. Treat connection details as secrets: do not commit them, include them in logs, or expose them in a public response. Successful responses carry `Cache-Control: private, no-store` and `Pragma: no-cache`. Do not cache session responses.
...
| `type`                  | string        | No       | `agent-device`, `appium`, `argent`, or `web-preview-only`. Defaults to `agent-device`. `serve-sim` is deprecated. Use `web-preview-only` instead.                                                                                  |
...
| `maxIdleTimeMinutes`    | integer       | No       | Stop after this many minutes without session activity. Must be positive and less than 115. It must be smaller than `maxDurationMinutes` when both are set. Omit to disable the idle timeout. Not supported for `web-preview-only`. |
...
The application source options install and launch the app for `agent-device`, `argent`, and `web-preview-only` sessions. For `appium`, install and launch the app through your Appium client.

A `web-preview-only` session provides the web preview without a controller. It does not support an idle timeout. Other session types provide a controller and include a web preview on supported iOS sessions. The deprecated `serve-sim` type is iOS-only and returns `previewUrl` instead of `webPreviewUrl`.
...
<Terminal
  cmdCopy={`curl -X POST "https://api.expo.dev/v2/device-run-sessions" -H "Authorization: Bearer $EXPO_TOKEN" -H "Content-Type: application/json" -d '{"appId":"a415eac6-231a-4b38-b481-3255a59f13b8","platform":"ios","type":"agent-device","name":"Checkout flow screenshots","buildId":"f9609423-5072-4ea2-a0a5-c345eedf2c2a","maxDurationMinutes":30,"maxIdleTimeMinutes":5}'`}
  cmd={[
...
<Terminal
  cmdCopy={`curl "https://api.expo.dev/v2/device-run-sessions/$SESSION_ID" -H "Authorization: Bearer $EXPO_TOKEN"`}
  cmd={[
...
| `web-preview-only`       | `webPreviewUrl`. This session type has no controller.                                        |
| `serve-sim` (deprecated) | `previewUrl`. This session type has no controller.                                           |
...
Open `webPreviewUrl` (or `previewUrl` for a deprecated `serve-sim` session) in a desktop browser to view supported iOS sessions. Do not open it on the remote device. Android sessions do not currently provide a browser preview.
...
<Terminal
  cmdCopy={`curl -X POST "https://api.expo.dev/v2/device-run-sessions/$SESSION_ID/stop" -H "Authorization: Bearer $EXPO_TOKEN"`}
  cmd={[
