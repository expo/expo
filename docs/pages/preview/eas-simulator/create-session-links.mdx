...
A create session link starts an iOS Simulator session from an EAS Build, build fingerprint, or application archive and opens its browser preview in fullscreen mode when it is ready. Use one to add an EAS Simulator action to a pull request, CI result, or an internal tool without integrating the [REST API](/preview/eas-simulator/rest-api/).
...
After authentication, expo.dev validates the parameters and build, creates the session, and shows its startup progress while reserving a simulator and preparing the web preview. This can take about a minute. When the web preview becomes available, expo.dev redirects directly to its `webPreviewUrl` with `fullscreen=1`, so the preview fills the page.
