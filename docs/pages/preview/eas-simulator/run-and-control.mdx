...
Choose how you plan to control or view the device before starting the session and pass its type explicitly. For programmatic control, use the matching controller's commands for the rest of the session.
...
| Browser only | `--type serve-sim`    | Interactive iOS stream without programmatic control                   |
