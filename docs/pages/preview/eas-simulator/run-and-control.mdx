...
> **info** **Typical use case:** A coding agent starts an agent-device session, opens Expo Go or a development build connected to Metro, and iterates with Fast Refresh while you follow the same iOS session in the web preview. Ask the agent to use the [EAS Simulator skill](https://github.com/expo/skills/blob/main/plugins/expo/skills/eas-simulator/SKILL.md), which handles the build workflow, remote session, device control, evidence collection, and cleanup. The sections below document the same workflow for manual use.
...
| Run a compatible project without native code | Expo Go connected to Metro                  |
...
Choose how you plan to control the device before starting the session and pass its type explicitly. On iOS, agent-device, Appium, and Argent sessions include both the selected controller and a web preview. Use `serve-sim` when you only need the web preview without programmatic control.
...
| Argent           | `--type argent`       | [Argent device tools and MCP integration](/agents/argent/)            |
| Web preview only | `--type serve-sim`    | Interactive iOS stream without programmatic control                   |
...
The start command can only receive a remote EAS Build or application archive. For a local development `.app`, start a blank agent-device session, upload it with `install`, and use agent-device to open the development-client URL.
