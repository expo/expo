...
| Name      | Description                                                                                                                               |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `navigate` | Navigate to a route using the default behavior. `navigate` only pushes a new screen if the new route is different (not including search parameters or the hash). Otherwise, the current screen rerenders with the new parameters. If you navigate to a route that is in the history, the stack will reset to that route. |
| `push`    | The same as `navigate`, except a new screen is always created. You can push the current route multiple times or with new parameters. |
