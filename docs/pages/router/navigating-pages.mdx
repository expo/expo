...
> **caution** `relativeToDirectory` will be released in the next version of `expo-router`
...
<Link href="./article" relativeToDirectory>Goto article</Link>
...
> **info** Relative URL's without a `./` prefix such as `article` are not supported
