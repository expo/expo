...
To handle backwards navigation, Expo Router offers multiple functions: [`back`](/versions/latest/sdk/router/#back) moves you backward in the stack's history. At the same time, [`dismiss`](/versions/latest/sdk/router/#dismiss) only goes back within the current stack.
