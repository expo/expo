...
| `switch`   | Only available within a tabbed navigator. Will switch the navigator to the tab that can render the Href.                                                                                                                                                                                                                                  |

### `switch`

The `switch` method is only available within a tabbed navigator and is used to change the currently rendered tab. Unlike the other methods, `switch` does not guarantee that the provided href will be rendered, it changes to the tab that **could** render the href. This is useful when a tab may have internal state and you are unsure of its currently rendered screen.

If you wish to change tabs and guarantee that the current screen is the href provided, you should use `navigate` or `replace`
