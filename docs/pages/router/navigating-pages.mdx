...
> Expo Router v4 changed `navigate` to be an alias for `push`. To retain the old behavior, please use [`dismissTo`](/versions/v52.0.0/sdk/router/#dismissTo)

Expo Router uses a stack-based navigation model, where each new route you navigate to is added to a stack.

For example, navigating from /feed to /profile results in the stack being `['/feed', '/profile']`. If you then navigate to /settings, the stack becomes `['/feed', '/profile', '/settings']`. While this may resemble traditional web navigation, a key distinction is that screens can have multiple child stacks, each independently controlled. This is known as parallel routing.
...
In the example above, app/_layout introduces a `<Tabs />` navigator, while the other two _layout files each render a `<Stack />`. Navigating through each page would result in a history like `[['/apple', '/orange'], ['/carrot', '/potato']]`. Notice that instead of a single stack, there’s a parent stack containing two child stacks.

While advancing in a stack is straightforward, moving backwards can be more complex due to the non-linear history. This behavior is familiar when using your phone: after clicking a link to open an app, pressing back might show you a screen you haven’t visited yet. Only pressing back again exits the app. This is an example of an anchor route (also known as an initial route), demonstrating how stacks manage their own history. This is why `goBack()` is sometimes called a "pop" (the reverse of "push") or "unwinding the stack".

To handle backwards navigation, Expo Router offers multiple functions: [`goBack`](/versions/v52.0.0/sdk/router/#goback) moves you backwards in the stack's history, while [`dismiss`](/versions/v52.0.0/sdk/router/#dismiss) only goes back within the current stack.

The [`dismissTo`](/versions/v52.0.0/sdk/router/#dismissto) function is unique—it can move you forwards or backwards. Calling dismissTo for a previously visited route will dismiss screens until that route is reached. If the route isn’t in the history, it will push forward to that route.

Additionally, the [`replace`](/versions/v52.0.0/sdk/router/#replace) method allows you to replace the current route in the stack with a new one, without adding to the stack.
