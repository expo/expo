...
To navigate, you can provide a full path like `/profile/settings`, a relative path like `../settings` or by passing an object like `{ pathname: 'profile', params: { id: '123' } }`.
...
<Table>
  ...
  <tbody>
    <Row>
      <Cell>`navigate`</Cell>
      <Cell>
        Navigate to a route using the default behavior. Navigate will only push a new screen if the
        new route is different (not including search parameters or the hash), otherwise the current
        screen will rerender with the new parameters. If you navigate to a route is that is in the
        history, the stack will unwind to that route.
      </Cell>
    </Row>
    <Row>
      <Cell>`push`</Cell>
      <Cell>
        The same as `navigate`, except a new screen will always be created. You can push the current
        route multiple times, or with new parameters.
      </Cell>
    </Row>
    <Row>
      <Cell>replace</Cell>
      <Cell>
        Remove the current route from the history and replace it with the specifed URL. This is
...
You can also navigate imperatively using the `router` object. This is useful when you need to perform a navigation action outside of a React component, such as in a event handler or a utility function.
