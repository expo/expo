...
- **Client directory output**: Assets are placed in `dist/client/assets/` making them publicly accessible and cacheable by CDNs.
...
A common use case for assets in API routes is generating dynamic Open Graph images. This example uses [Satori](https://github.com/vercel/satori) to render React components as SVG images, with fonts imported directly:
...
The font is imported as an asset, which ensures it is bundled and exported with your application. At runtime, `fetch()` retrieves the font data from the bundled asset URL.
