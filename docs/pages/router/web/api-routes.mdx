...
Expo Router enables you to write secure server code for all platforms, right in your **app** directory inside **src**.
...
An API route is created in the **src/app** directory. For example, add the following route handler. It is executed when the route `/hello` is matched.
