...
The function can be asynchronous and receives `undefined` as its first argument and the route's parameters as its second argument. Static rendering does not provide a request, including during development. See [metadata with server rendering](/router/web/server-rendering/#metadata) for supported fields and dynamic examples.
...
When upgrading from SDK 57, follow [Migrate static page metadata](/router/migrate/sdk-57-to-58/#migrate-static-page-metadata) to move existing `Head` tags and document attributes.
