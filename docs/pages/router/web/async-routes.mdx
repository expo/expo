...
The loading state can be customized by exporting a `SuspenseFallback` component from a layout file or by passing the `suspenseFallback` prop to a navigator. See [Loading states with Suspense fallback](/router/error-handling/#loading-states-with-suspense-fallback). A layout's exported fallback applies once the layout module has loaded. While the layout module is loading, the nearest parent's fallback is shown.
