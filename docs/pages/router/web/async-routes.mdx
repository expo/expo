...
> **important** Async routes remains experimental. In SDK 58 and later, it is enabled by default on web in development and production. Native async routes remain opt-in and are supported only in development. See [Caveats](#caveats) for current limitations.
