...
In SDK 58 and later, middleware also supports `web.output: "static"` when `apiRoutes: true` is set in the `expo-router` config plugin. This allows middleware to run before [prerendered HTML and API routes](/router/web/api-routes/#create-an-api-route).
