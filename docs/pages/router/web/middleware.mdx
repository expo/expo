...
> **important** Server middleware is stable in SDK 58 and later. For SDK 54–57, enable `unstable_useServerMiddleware` in the `expo-router` config plugin. Server middleware requires a [deployed server](/router/web/api-routes/#deployment) for production use.

In SDK 58 and later, `unstable_useServerMiddleware` is deprecated and has no effect. Remove it from your config to avoid the deprecation warning.
...
In SDK 58 and later, you can use middleware with `web.output: "static"` when `apiRoutes: true` is set in the `expo-router` config plugin. This allows middleware to run before [prerendered HTML and API routes](/router/web/api-routes/#create-an-api-route).
...
- Client-side navigation using [`<Link />`](/versions/latest/sdk/router/link/#link) or [`router`](/versions/latest/sdk/router/#imperativerouter)
