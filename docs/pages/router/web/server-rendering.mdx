...
> **important** Server rendering is stable in SDK 58 and later. For SDK 55–57, enable `unstable_useServerRendering` in the `expo-router` config plugin. It requires a [deployed server](/router/web/api-routes/#deployment) for production use.
...
To keep the behavior of `web.output: "server"` from SDK 57 and earlier, use `web.output: "static"` with `apiRoutes: true` in the `expo-router` config plugin. This combines [prerendered HTML with API routes](/router/web/api-routes/#create-an-api-route).

<Step label="1">

In SDK 58 and later, Expo Router uses server rendering when `web.output` is omitted. New projects created with the default or tabs template also use server output. To switch an existing project, set `web.output` to `server` in your [app config](/versions/latest/config/app/):
...
- `bodyNodes`: React nodes for the `<body>` element (fonts and JavaScript bundles required for hydration). Omitting these prevents the app from hydrating and becoming interactive.
...
| Metadata           |     | [`<Head>`](#using-head-with-server-rendering) component                               | [`generateMetadata`](#metadata) |
