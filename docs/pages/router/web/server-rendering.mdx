...
To keep the behavior of `web.output: "server"` from SDK 57 and earlier without experimental server rendering, use `web.output: "static"` with `apiRoutes: true` in the `expo-router` config plugin. This combines [prerendered HTML with API routes](/router/web/api-routes/#create-an-api-route).

<Step label="1">

Set `web.output` to `server` in your project's [app config](/versions/latest/config/app/) to enable server rendering:
