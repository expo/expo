...
The `loader` function executes on the server, and its return value is serialized and passed to your component. This means you can safely use server-side secrets, database connections, and other resources that should not be exposed to the client.

### Using Suspense

[`useLoaderData`](/versions/unversioned/sdk/router/#useloaderdata) does not need to be called at the top level of your route component.
...
In the above example, [`useLoaderData`](/versions/unversioned/sdk/router/#useloaderdata) is lower in the component tree and wrapped it with `<Suspense>` to show a loading state.
...
Loaders can return data in several ways:

### Plain objects

The simplest approach is to return a plain JavaScript object, which is easily serialized to JSON:
...
Data loaders have full access to the [Runtime API](/router/web/api-routes/#runtime-api) from `expo-server`. This includes utilities for setting response headers, throwing HTTP errors, and running background tasks:
...
<Collapsible summary="Can I access the request in static rendering mode?">

No. In static rendering mode, the `request` parameter is `undefined` because loaders execute at build-time when there is no HTTP request. If you need access to request headers, cookies, or other request data, use server rendering instead.
...
No. Loader functions are automatically stripped from the client bundle. This means your server-side code, API keys, and database connections remain on the server.
