...
> **important** Data loaders are stable in SDK 58 and later. For SDK 55–57, enable `unstable_useServerDataLoaders` in the `expo-router` config plugin. They require either [static rendering](/router/web/static-rendering) or [server rendering](/router/web/server-rendering).
...
Data loaders are enabled by default. If you use [server rendering](/router/web/server-rendering), enable it in your project's [app config](/versions/latest/config/app/):
...
You can also type loaders directly using the [`LoaderFunction`](/versions/latest/sdk/server/#loaderfunctionrequest-params) type from `expo-router/server`. This gives you full control over the function signature, including both `request` and `params`:
