...
- Files named **index** adds no path segment to the URL. For example, **app/index.js** matches `/`.
- The following file extensions are supported by default: **.js**, **.jsx**, **.ts**, **.ts**.
- Metro platform extensions (for example, **.ios.js**, **.native.ts**) are not supported.
...
- **app/blog/[slug].js** &mdash; Matches `/blog/123` in the app and the browser.
- **app/blog/[...rest].js** &mdash; Matches `/blog/123/settings` in the app and the browser.
...
Multiple slugs can be matched in a single route by using the rest syntax (`...`). For example, **app/blog/[...id].js** will match `/blog/123/settings` in the app and the browser.
...
<BoxLink
  title="Navigation"
  Icon={BookOpen02Icon}
  description="Learn how to use the Expo Router to navigate between pages in your app."
  href="/router/routing/navigation/"
/>
