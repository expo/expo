...
The `Stack` component is a native stack: each screen is pushed onto the platform's own navigation stack, so you get native transition animations, swipe-back gestures, and header behavior on Android and iOS. You configure it with the [Stack screen options](/versions/latest/sdk/router/stack/). However, you do not have to define the pages specifically inside the navigator. The files inside the directory will be automatically treated as eligible routes in the stack. However, if you want to define screen options, you can add a `Stack.Screen` component inside the `Stack` component. The `name` prop should match the route name, but you do not need to supply a `component` prop; Expo Router will map this automatically:
...
This will cause the **index.tsx**, **feed.tsx**, and **profile.tsx** files to appear together in the same bottom tabs navigator. The `Tabs` component renders a bottom tab bar and keeps each tab's screen mounted so switching between them is instant. You configure the tab bar and each tab with the [Tabs screen options](/versions/latest/sdk/router/tabs/).
