...
Routes starting with a `+` have special significance to Expo Router. One example is `+not-found`, which catches any requests that don't match a route in your app.
...
- **app/(tabs)/index.tsx** is the default route for the **(tabs)** folder, so it will be the initially-focused tab, and will match the `/` URL.
