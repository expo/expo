...
To prefetch imperatively instead of on render, call `router.prefetch(href)`. This is useful when you want to prepare a screen in response to an interaction, such as a scroll or a hover:
...
export default function Page() {
  const router = useRouter();
  return <Button title="View details" onPress={() => router.prefetch('/details')} />;
}
```
...
For more information, see the [prefetching reference](/router/reference/prefetching/).
