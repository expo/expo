...
Expo Router calls this operation `prefetch` in its public APIs. The underlying navigator handles a prefetch request by preloading the route, so internal state and React Navigation APIs use the term `preload`.
