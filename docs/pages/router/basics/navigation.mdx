...
Prefetching allows you to load a screen in the background before navigating to it, enabling smoother transitions. Expo Router provides three ways to prefetch routes.

### Using the Link component

The `prefetch` prop on a `<Link />` component enables prefetching of the target screen when the component is rendered:
...
You can prefetch a route programmatically using `router.prefetch()`. This is useful when you want to trigger prefetching based on custom logic or events:

{/* prettier-ignore */}
```tsx
import { router } from 'expo-router';
/* @info Import the <CODE>router</CODE> object to prefetch routes imperatively. */
import { useEffect } from 'react';
/* @end */

export default function Page() {
  useEffect(() => {
    /* @info Prefetch the about page when this component mounts. */
...
You can use the `<Prefetch />` component to preload a route when the component is rendered on a focused screen:
...
export default function Page() {
  return (
    <>
      {/* @info The about page will be prefetched when this screen is focused. */}
      <Prefetch href="/about" />
      /* @end */
...
The `<Prefetch />` component only triggers prefetching when the screen it's rendered on is focused. This makes it useful for preparing routes that the user is likely to navigate to next.
