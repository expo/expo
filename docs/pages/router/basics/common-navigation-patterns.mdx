...
A `Link` to a URL that belongs to another tab switches tabs, and the back button then pops the stack inside that tab. In the example above, opening `/feed/123` from the Settings tab lands on the post in the Feed tab, and going back returns to `feed/index` — not to Settings.

With [native tabs](/router/advanced/native-tabs/), every tab is loaded as soon as the tab navigator mounts, so a tab that contains a stack is already sitting at that stack's first screen before you link into it. That screen ends up underneath the one you linked to whether or not you passed `withAnchor`.
...
`/feed/123` is now a screen of the root stack rather than of the Feed tab, so it is pushed on top of whichever tab is open and going back returns to that tab. This is also how detail screens usually behave in iOS apps.
