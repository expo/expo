...
export default function Layout() {
  return (
    <Stack screenErrorBoundary={ScreenErrorBoundary}>
      <Stack.Screen name="profile" errorBoundary={ScreenErrorBoundary} />
    </Stack>
  );
}
```

An individual route's `ErrorBoundary` export takes precedence, followed by its `unstable_settings.screenErrorBoundary`, `Stack.Screen`'s `errorBoundary`, the navigator's `screenErrorBoundary`, and then the nearest layout's `unstable_settings.screenErrorBoundary`. Layout settings apply to descendant screens, while navigator boundaries apply only to that navigator's screens.
