...
A screen error boundary keeps navigator UI, such as headers and tab bars, mounted when a screen throws. Configure it on a layout, a navigator, or an individual screen:
...
export default function Layout() {
  return (
    <Stack unstable_screenErrorBoundary={ScreenErrorBoundary}>
      <Stack.Screen name="profile" unstable_errorBoundary={ScreenErrorBoundary} />
    </Stack>
  );
}
```
...
2. Screen-level prop (for example, `Stack.Screen`)
3. Navigator-level prop (for example, `Stack`)
4. `unstable_settings.screenErrorBoundary` declared in the **\_layout.tsx** file
