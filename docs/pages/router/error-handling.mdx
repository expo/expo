...
You can keep navigator UI such as headers and tab bars mounted when a screen fails by providing an error boundary for the screens it renders. Configure it on a layout, navigator, or individual screen:
...
An individual route's `ErrorBoundary` export takes precedence, followed by its `unstable_settings.screenErrorBoundary`, `Stack.Screen`'s `unstable_errorBoundary`, the navigator's `unstable_screenErrorBoundary`, and then the nearest layout's `unstable_settings.screenErrorBoundary`. Layout settings apply to descendant screens, while navigator boundaries apply only to that navigator's screens.
