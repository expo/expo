...
Expo Router wraps each route in a [React Suspense](https://react.dev/reference/react/Suspense) boundary. You can export a [`SuspenseFallback`](/versions/unversioned/sdk/router/#suspensefallback) component from any route or layout file to customize the loading UI shown while a component is suspended:
...
> If a route exports its own [`SuspenseFallback`](/versions/unversioned/sdk/router/#suspensefallback), it overrides the fallback defined in its parent layout. When multiple parent layouts define a fallback, the nearest parent wins.
