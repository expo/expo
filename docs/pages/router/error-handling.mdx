...
### Boundaries for screens in a layout
...
// Choose this to configure the boundary for the layout's direct child screens.
...
Alternatively, configure the boundary on the navigator instead:
...
// Choose this to configure the boundary for the navigator's direct child screens.
export default function Layout() {
...
`unstable_settings.screenErrorBoundary` applies to the layout's direct child screens. A nested layout starts a new boundary scope; define the setting in that nested layout to handle its screens.
