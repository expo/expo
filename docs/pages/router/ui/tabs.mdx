...
The `<TabSlot />` component renders the current route. `<TabSlot />` be can be nested inside other components.
...
`<TabList />` needs to be rendered, but not necessary displayed. `<TabList style={{ display: 'none' }} />` will hide the `<TabList />` and you create create your own visual display using `<TabTrigger />`
