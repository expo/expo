...
description: Learn how to use headless Tabs, TabList, TabTrigger, and TabSlot components to create custom tab layouts in Expo Router.
sidebar_title: Tabs
---
...
> **warning** Experimentally available in SDK 52 and above. For the React Navigation styled tabs, see [Tabs](/router/advanced/tabs/).
...
## Anatomy of headless Tabs components

There are four components offered by `expo-router/ui` to create custom tab layouts:

| Component    | Definition                                            |
| ------------ | ----------------------------------------------------- |
| `Tabs`       | The containeing `<View>` for the tabs.                |
| `TabList`    | The containing `<View>` for the list of tab triggers. |
...
> **Note:** `name` can be any `string`. This is a user-defined name for the Tab.
...
The `reset` prop from `TabTrigger` can be used to control when a tab resets its navigation state. The options are `always`, `onLongPress` and `never`.
...
A `TabTrigger` with a relative href are relative to the local path name `Tabs` was rendered on. This is different to normal relative hrefs which are relative to the current displayed route. For example, the `<TabTrigger href="./profile" />` will resolve to `/directory/profile`, even when the `/directory/page` route is showing. Expo recommends against using relative hrefs.
