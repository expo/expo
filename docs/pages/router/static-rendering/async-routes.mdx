...
Enable the feature by setting the `asyncRoutes` option to `development` in your **app.json**:
...
You can set platform-specific settings for `asyncRoutes` using an object, `ios`, `android`, `web`, `default`:
