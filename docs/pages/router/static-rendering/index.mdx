...
First, enable static rendering in the project's `app.json` file (only available with `web.bundler` set to `metro`).
...
Next, if you have a `metro.config.js` file in your project, ensure it extends `expo/metro-config`:
...
This will create a `dist` folder with your statically rendered website.

You can test this production build locally by running the following, and opening the linked URL in your browser:
...
The `static` output will generate HTML files for each route. This means that dynamic routes (`app/[id].tsx`) will not work out of the box. You can generate known routes ahead of time using the `generateStaticParams` function.
...
You can customize the root HTML file by creating a **app/+html.tsx** file in your project. This file exports a React component that only ever runs in Node.js, meaning global CSS cannot be imported inside of it. The component will wrap all routes in the **app** directory. This is useful for adding global `<head>` elements or disabling body scrolling.

> Notice: Global context providers should go in the [Root Layout](/router/advanced/root-layout) component, not the Root HTML component.
...
- Global CSS should not be imported in this file, instead use the [Root Layout](/router/advanced/root-layout) component.
- Browser APIs like `window.location` are not available in this component as it only runs in Node.js during static rendering.
