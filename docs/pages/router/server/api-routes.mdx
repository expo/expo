...
Using `expo-server` is not limited to API routes and it can be used in any other server code as well, for example, in [server middleware](/router/server/middleware/).
...
For example, you may need to add metadata in [server middleware](/router/server/middleware/) to a `Response` before your API route code is running.
