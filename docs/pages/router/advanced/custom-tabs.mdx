...
The `href` values provided to `<abTrigger` cannot be ambagious and must always point to a single route. The href `/route` is not allowed, as it could refer to either `/(one)/route` or `/(two)/route`.
