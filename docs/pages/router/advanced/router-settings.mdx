...
The `initialRouteName` is only used when deep-linking to a route. During app navigation, the route you are navigating to will be the initial route. To also load the `initialRouteName` route below it, use the `withAnchor` prop on the `<Link />` component or pass the option to the imperative APIs.
