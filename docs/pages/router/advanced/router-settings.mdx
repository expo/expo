...
The `anchor` is only used when deep-linking to a route. During app navigation, the route you are navigating to will be the initial route. To also load the anchor route below the target route, use the `withAnchor` prop on the `<Link />` component. The imperative APIs accept the same option.
