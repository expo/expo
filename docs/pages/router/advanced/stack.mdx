...
Expo Router uses the `@react-navigation/native-stack` package for the `<Stack />` layout, but you may want to use the JavaScript-powered `@react-navigation/stack` library instead. To do so, you can create a custom layout component that wraps the `@react-navigation/stack` library.
...
For options and info about `@react-navigation/stack`, refer to the [React Navigation docs](https://reactnavigation.org/docs/stack-navigator).
