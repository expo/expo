...
export default function Layout() {
  return (
    <Stack
      // See the Stack API reference for the full list of available screenOptions: /versions/latest/sdk/router/stack/
      screenOptions={{
        headerStyle: {
...
For the full list of options, see the [Stack API reference](/versions/latest/sdk/router/stack/).

The `Stack` navigator is a native stack. It renders each screen with the platform's own navigation primitives: a `UINavigationController` on iOS and a fragment-based stack on Android. This gives you native transition animations, gestures, and header behavior instead of a JavaScript reimplementation.
...
Call `router.dismissAll()` to return to the first screen in the closest stack, dismissing every screen pushed after it in a single action.
