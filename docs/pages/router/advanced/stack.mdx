...
You can use the `<Stack.Screen name="[user]" getId={} />` component in the layout component route to modify the pushing behavior. In the following example, we use the `getId` function to push a new page every time the user navigates to a profile.
