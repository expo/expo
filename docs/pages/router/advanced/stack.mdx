...
## Preserve state while hiding deeply nested screens

Set `activityEnabled` on a stack to wrap its route content in [React Activity](https://react.dev/reference/react/Activity). Hidden content runs effect cleanups and releases resources while preserving local state. By default, a stack hides a route when two screens are above it:
...
Pass a number to change the hiding threshold. You can override the setting for a statically declared route in the layout, including setting it to `false` to keep that route active:
...
export default function Layout() {
  return (
    <Stack activityEnabled={3}>
      <Stack.Screen name="map" activityEnabled={false} />
    </Stack>
  );
}
```

When navigators are nested, screens above a route in each navigator are added together. The nearest explicit navigator or screen setting applies.
...
> **important** Screen composition API is in [alpha](/more/release-statuses/#alpha) and available in SDK 55 and later.
...
Create a [development build](/develop/development-builds/introduction/#how-would-you-like-to-build-your-development-build) and set the [`UIDesignRequiresCompatibility`](https://developer.apple.com/documentation/BundleResources/Information-Property-List/UIDesignRequiresCompatibility) property to `true` in [app config](/workflow/configuration/):
...
The `expo-router/js-stack` entry point is the SDK 56 replacement for the `@react-navigation/stack` library. See the [SDK 55 to 56 migration guide](/router/migrate/sdk-55-to-56) for more information.
