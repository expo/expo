...
## Preserve state while hiding inactive tabs

Set `activityEnabled` on a tabs layout to wrap each route's content in [React Activity](https://react.dev/reference/react/Activity). An inactive tab then runs effect cleanups and releases resources while preserving local state:
...
export default function TabLayout() {
  return (
    <Tabs activityEnabled>
      ...
      <Tabs.Screen name="map" activityEnabled={false} />
    </Tabs>
  );
}
```

For tabs, `true` hides a route as soon as its tab becomes inactive. Pass a number to require that many screens above a route instead. You can override the setting for a statically declared screen in the layout, including setting it to `false` to keep that route active.
