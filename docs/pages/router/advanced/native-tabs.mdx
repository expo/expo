...
On iOS 26 and later, the system draws the tab bar with Liquid Glass and derives its background from the content behind it. The `backgroundColor`, `blurEffect`, `shadowColor`, and `disableTransparentOnScrollEdge` props only apply to iOS 18 and earlier.
...
To make the tab bar match a dark UI on iOS 26, make the content behind the tab bar dark, as described in the previous section.
