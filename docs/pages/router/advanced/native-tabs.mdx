...
On iOS, you can use images from the Xcode asset catalog as tab icons with the `xcasset` prop. This is useful when you want to manage your icons through Xcode's asset catalog instead of bundling image files. Both image sets and symbol sets are supported.
...
> **info** Asset catalog icons don't support the `renderingMode` prop. Their rendering mode comes from the asset's **Render As** setting in the asset catalog. Set it to **Template Image** for the icon to be tinted with `iconColor` and `selectedIconColor`. Setting `renderingMode` alongside `xcasset` has no effect and logs a warning.
...
<Collapsible summary="The tab bar background props have no effect on iOS 26">
...
This also means the tab bar does not follow a color scheme that exists only in JavaScript. Setting the color scheme with [`Appearance.setColorScheme`](https://reactnative.dev/docs/appearance#setcolorscheme) changes the interface style that other native views resolve against, but it does not change the tab bar background either.

To make the tab bar match a dark UI on iOS 26, make the content behind the tab bar dark, as described in [White background flashes when switching tabs on iOS 26](#white-background-flashes-when-switching-tabs-on).
