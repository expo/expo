...
> **info** This feature is available on iOS 26 and later.

When a native tab uses `role="search"` and the screen renders a native search bar that can integrate with the toolbar, you can add native toolbar items next to the system search control. These items use UIKit rendering, preserve the native search expansion and tab bar minimize animations, and do not replace the native tab bar with a custom JavaScript bar.

Use `NativeTabs.Trigger.ToolbarItem` inside the search tab trigger. The item supports SF Symbols, accessibility labels and hints, disabled state, badge values, and press handlers.
    ...
    <NativeTabs minimizeBehavior="onScrollDown">
      ...
      <NativeTabs.Trigger name="search" role="search">
        ...
        <NativeTabs.Trigger.ToolbarItem
          ...
          badgeValue={activeFilterCount}
          onPress={() => {
            // Open your filters UI.
          }}
        />
      </NativeTabs.Trigger>
    </NativeTabs>
...
Set `badgeValue` to `null` or `undefined` to hide the badge. A value of `0` displays a badge with `0`.
