...
> **info** Asset catalog icons don't support the `renderingMode` prop. Their rendering mode comes from the asset's **Render As** setting in the asset catalog. Set it to **Template Image** for the icon to be tinted with `iconColor` and `selectedIconColor`.
