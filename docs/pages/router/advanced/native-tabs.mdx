...
> **info** On iOS, a tab's default and selected icons must use the same rendering mode. When they resolve to different modes, for example when only `selectedIconColor` is set, both icons use the default icon's rendering mode and a warning is logged.
