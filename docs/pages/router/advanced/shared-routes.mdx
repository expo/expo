...
Instead of defining the same route multiple times with different layouts, use the array syntax `(,)` to duplicate the children of a group. For example, `src/app/(home,search)/[user].tsx` &mdash; creates `src/app/(home)/[user].tsx` and `src/app/(search)/[user].tsx` in memory.
