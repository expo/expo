...
> **info** `createProps` receives the processed Expo Router state and low-level navigation helpers. These APIs may have small breaking changes between releases. Prefer the standard `state` and `actions` when they provide what you need. If something is missing, [open an issue on GitHub](https://github.com/expo/expo/issues).
...
Both helpers provide `baseRouter`, `options`, and `nextKey`. Use `baseRouter` to delegate existing behavior and `nextKey` when adding a route to the state.
