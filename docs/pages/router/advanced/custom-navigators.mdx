...
Both `createStandardRouterNavigator` and `integrateWithRouter` accept an optional `options` object as their third argument:
...
### Extend router behavior

Use `extendRouterActions` to add custom action handling without replacing any other router methods. The original router handles each action first, and your extension receives its result. Return that result unchanged for actions you do not want to modify:
...
const CustomTabRouter = extendRouterActions(TabRouter, (state, action, options, result) => {
  if (action.type === 'FOCUS_FIRST_TAB') {
    return {
      state: { ...state, index: 0 },
...
export const Tabs = unstable_createStandardRouterNavigator(TabsContent, CustomTabRouter);
```

The original result is `null` when the router does not handle the action. Returning `null` preserves normal action bubbling.

Use `extendRouter` when you need to override other parts of a router implementation. The extension receives the original router and its factory options, and returns only the methods that should change:

```tsx components/Tabs.tsx
import { extendRouter, TabRouter } from 'expo-router';

const CustomTabRouter = extendRouter(TabRouter, router => ({
  shouldActionChangeFocus(action) {
    return action.type === 'FOCUS_FIRST_TAB' || router.shouldActionChangeFocus(action);
  },
}));
```

`extendStackRouter` and `extendTabRouter` provide shortcuts for extending the corresponding router without passing its factory explicitly.
