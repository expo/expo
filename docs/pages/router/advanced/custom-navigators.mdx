...
`state.routes` is a subset of the navigator's screens: a screen only appears once it has loaded (it becomes focused, or you preload it). To render a tab for every declared screen from the first frame, preload them. Expose a `preload` action with `createProps` and call it on mount:
...
// The first type argument is the options you can set per screen, the third is the props
// injected by `createProps` below.
type TabsContentProps = NavigatorContentProps<
  { title?: string },
  Record<string, never>,
  { routeNames: string[]; preload: (name: string) => void }
>;

function TabsContent({ state, descriptors, actions, routeNames, preload }: TabsContentProps) {
  const focusedRoute = state.routes[state.index];

  // Preload every declared screen so the tab bar can show all tabs from the first frame.
  useEffect(() => {
    routeNames.forEach(preload);
  }, [routeNames, preload]);
