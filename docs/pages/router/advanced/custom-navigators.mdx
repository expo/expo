...
<FileTree
  files={[
    [
      "./src/TabsContent.tsx",
      "Navigator UI implementing the standard navigator API",
    ],
    ["./src/index.ts", "Root entry — exports the framework-agnostic navigator"],
    [
      "./src/react-navigation.ts",
      "React Navigation entry — integrates the same navigator",
    ],
    [
      "./src/expo-router.ts",
      "Expo Router entry — integrateWithRouter(navigator, ...)",
    ],
