...
function ModalStackContent({
  state,
  descriptors,
  actions,
}: NavigatorContentProps<Options>) {
  ...
  const nativeStackDescriptors =
    descriptors as unknown as NativeStackDescriptorMap;

  if (!hasModals) {
    return (
      <NativeStackView
        state={state as NativeStackViewState}
        descriptors={nativeStackDescriptors}
      />
    );
  }
...
export const ModalStack = createStandardRouterNavigator(
  ModalStackContent,
  StackRouter,
);
