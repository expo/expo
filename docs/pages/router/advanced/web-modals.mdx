...
You can achieve the above web modal behavior by setting a route's `presentation` option to `'transparentModal'`, styling the overlay and modal content, and utilizing [`react-native-reanimated`](/versions/latest/sdk/reanimated/#installation) to animate the modal's presentation.

With `presentation: 'transparentModal'`, the route is presented over the previous screen instead of replacing it. The previous screen stays mounted and visible underneath, and the modal screen renders with a transparent background so anything it does not paint shows through. This is what enables overlay UI: you render a semi-transparent dimming layer plus the modal content on top of the still-visible page below, rather than a full opaque screen.
