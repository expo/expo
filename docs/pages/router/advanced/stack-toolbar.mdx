...
Flexible spacers (`<Stack.Toolbar.Spacer />` with no `width`) are iOS-only. On Android, a `Stack.Toolbar.Spacer` without a `width` renders nothing &mdash; pass a fixed `width` such as `<Stack.Toolbar.Spacer width={24} />` in every placement.
