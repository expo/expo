...
## Expo Router's Modal component

Expo Router provides a Modal component that can be used as a replacement for the React Native Modal. You can use this component similarly to the React Native Modal, by placing it anywhere in your app. For example:
...
export default function SomeScreen() {
  ...
  return (
    <View>
      ...
      <Modal visible={modalVisible} onDidClose={() => setModalVisible(false)}>
        <View>
...
With Expo Router's Modal, you can also open multiple modals on top of each other:
...
export default function SomeScreen() {
  ...
  return (
    <View>
      <Button title="Open Modal" onPress={() => setIsFirstModalVisible(true)} />
      <Modal visible={isFirstModalVisible} onDidClose={() => setIsFirstModalVisible(false)}>
        <View>
          <Text>This is first modal!</Text>
          <Button title="Close Modal" onPress={() => setIsFirstModalVisible(false)} />
          <Button title="Open Inner Modal" onPress={() => setIsSecondModalVisible(true)} />
        </View>
      </Modal>
      <Modal visible={isSecondModalVisible} onDidClose={() => setIsSecondModalVisible(false)}>
        <View>
          <Text>This is second modal!</Text>
          <Button title="Close Modal" onPress={() => setIsFirstModalVisible(false)} />
          <Button title="Close Inner Modal" onPress={() => setIsSecondModalVisible(false)} />
        </View>
      </Modal>
    </View>
  );
...
**Note:** When you close a modal that is not the topmost one, it will close the modals above it (i.e., in the example above, when both modals are open and the first modal is closed, the second modal will also close).

The full API reference for Expo Router's `Modal` component can be found in the [API reference](...).

**Note:** Similarly to [Modal screen](#modal-screen-using-expo-router) there is currently no way to prevent native dismissal of the modal.
