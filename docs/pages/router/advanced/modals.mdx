...
## Traditional web modals

Below is an example of a common web modal:

<ContentSpotlight file="expo-router/web-modal.mp4" />

You can achieve this behavior by using the [`transparentModal`](https://reactnavigation.org/docs/stack-navigator/#presentation) presentation mode, styling the overlay and modal content, and utilizing [`react-native-reanimated`](/versions/latest/sdk/reanimated/#installation) to animate the modal's presentation.
    ...
    <Stack>
      ...
      <Stack.Screen
        ...
        options={{
          ...
          /* @end */
          /* @info Set the <CODE>animation</CODE> to <CODE>fade</CODE> this is optional.*/
          animation: 'fade',
          /* @end */
...
The above example sets the `index` screen as the [`initialRouteName`](/router/advanced/router-settings/#initialroutename) using `unstable_settings`, ensuring the transparent modal is always rendered on top of the current screen, even when users navigate to the modal screen via a direct link.

Finally, style the overlay and modal content.
