...
description: Learn how to use the zoom transition to create fluid animations between screens.
hasVideoLink: false
---

import { ContentSpotlight } from '~/ui/components/ContentSpotlight';

> **important** Zoom transition is currently experimental and available in canary releases of `expo-router`.

> **info** In order to enable zoom transitions, run `enableZoomTransition();` in your `app/_layout.tsx` file.
...
Zoom transitions provide a fluid animation effect when navigating between screens by zooming from a source element to the destination screen. This feature leverages iOS 18+'s native zoom transition API to create smooth, interactive transitions that make your app feel more polished and responsive.

## Get started

To implement zoom transitions, you'll use the `Link.AppleZoom` component to mark the source element and optionally `Link.AppleZoom.Target` to specify the target alignment on the destination screen.

### Basic example

Here's a simple example showing how to create a zoom transition from an image to a preview screen:
...
### Gallery with zoom transitions

Here's a more complex example showing a gallery grid with zoom transitions to detail views:
...
function Thumbnail({ index }: { index: number }) {
  ...
  return (
    <Link
      href={{
        pathname: `/image/[id]`,
        // We need to pass the image size to the detail page, so that layout can be measured during first render
...
We recommend avoiding the use of zoom transitions when navigating between screens that have a header (navigation bar).
There are known issues with the native iOS zoom transition API that can lead to visual glitches or unexpected behavior when headers are involved.
