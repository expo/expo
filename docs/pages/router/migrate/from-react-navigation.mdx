...
Along with all the benefits of React Navigation, Expo Router enables automatic deep linking, [type safety](/router/reference/typed-routes), [deferred bundling](/router/server/async-routes), [static rendering on web](/router/server/static-rendering), and more.
...
On web, returning `null` from the root will cause [static rendering](/router/server/static-rendering) to skip all of the children, resulting in no searchable content. This can be tested by using "View Page Source" in Chrome, or by disabling JavaScript and reloading the page.
...
Use the [Head component](/router/server/static-rendering#meta-tags) to set the webpage title.
