...
To embed an isolated navigation tree inside a screen, install `@react-navigation/native` and render that package's `NavigationContainer` in the screen with linking disabled. Expo Router remains responsible for deep links and, on web, the URL. Deep links into the embedded tree must be bridged by the host screen, and Expo Router `Link` cannot target routes inside it. Expo Router hooks rendered in the embedded tree still address the host router.
