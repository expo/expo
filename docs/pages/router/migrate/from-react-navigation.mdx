...
Both React Navigation and Expo Router are Expo frameworks for routing and navigation. Expo Router is a wrapper around React Navigation, many of the concepts are the same.
...
- Rename the initial route to `index`. Expo Router considers the route that is opened on launch to match `/`, React Navigation users will generally use something like "Home" for the initial route.

### Refactor search parameters

Refactor screens to use serializable top-level query parameters, we recommend this in React Navigation as well: [what should be in params?](https://reactnavigation.org/docs/params/#what-should-be-in-params)

In Expo Router, search parameters can only serializable top-level values like `number`, `boolean`, `string`. React Navigation doesn't have the same restrictions, so users can sometimes pass invalid parameters like Functions, Objects, Maps, etc.
...
Consider restructuring so the function can be accessed from the "followers" screen. In this case we can just access the router and push directly from the "followers" screen.
...
Layout the structure of your app, by creating files acording to the [creating files guide](/routing/create-files). Best practice dictates that you use kebab-case, and lowercase letters for filenames.
...
In Expo Router, you can either migrate to [shared routes](/router/advance/shared-routes) or create multiple files and re-export the same component from them.

When you use groups or shared routes, you can navigate to specific tabs by using the fully qualified route name, e.g. `/(home)/settings` instead of `/settings`.
...
Refer to the [platform-specific modules](/router/advance/platform-specific-modules) guide for info on switching UI based on the platform.
