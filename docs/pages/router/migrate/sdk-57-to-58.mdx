...
- For web apps, review the [async route defaults](#review-async-route-defaults), the [static metadata migration](#migrate-static-page-metadata) for `web.output: "static"`, and the [server rendering changes](#review-server-rendering-behavior) for `web.output: "server"`.
...
### Migrate static page metadata

In SDK 58, apps using `web.output: "static"` without React Server Components use the streaming renderer to generate HTML. Tags rendered through `expo-router/head` are no longer included in the initial static HTML. The `Head` component still updates metadata after JavaScript loads.

Move metadata that must be present in the exported HTML to a `generateMetadata()` export in each route:

<DiffBlock
  raw={`diff --git a/src/app/about.tsx b/src/app/about.tsx
--- a/src/app/about.tsx
+++ b/src/app/about.tsx
@@ -1,14 +1,13 @@
...
`generateMetadata()` can be asynchronous. For static rendering, its first argument is `undefined` and its second argument contains the route parameters. This also applies during development. Server rendering with `web.output: "server"` continues to provide the incoming request.

Metadata is resolved from the matched route, not its layouts. If a layout previously used `Head` for shared metadata, move that metadata into each route's `generateMetadata()` or add shared tags directly to the `<head>` in **src/app/+html.tsx**.

Move any `<html>` or `<body>` attributes previously set through `Head` onto those elements in your [Root HTML component](/router/web/static-rendering/#root-html). For example, set `lang` on `<html>` in **src/app/+html.tsx**.

See [Static rendering: Meta tags](/router/web/static-rendering/#meta-tags) for the current metadata API.

### Review async route defaults

In SDK 58, async routes are enabled by default on web in development and production. Native defaults are unchanged, and native production builds continue to load routes synchronously.

Async routes use the default loading fallback instead of custom [`SuspenseFallback`](/router/error-handling/#loading-states-with-suspense-fallback) exports from layouts. If your web app relies on a custom fallback, disable async routes on web in the `expo-router` config plugin in your [app config](/workflow/configuration/):
...
This restores synchronous route loading and support for custom loading fallbacks on web. Existing explicit `asyncRoutes` settings continue to take precedence over the defaults. See [Async routes](/router/web/async-routes/) for configuration options and limitations.

After upgrading or changing `asyncRoutes`, clear the Metro cache with `expo start --clear` or `expo export --clear`.
