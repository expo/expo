...
When `activityEnabled` is `true`, stacks hide content when two screens are above it, while tabs and drawers hide content as soon as it loses focus. Stack navigators and screens also accept a positive number to override that threshold.
...
npx expo install @expo/ui # Android Stack.Toolbar
```
...
In SDK 58, create custom routers by extending `StackRouter` or `TabRouter` with `extendRouter` or `extendRouterActions`. Expo Router creates the complete initial state, while the wrapper preserves the base router's behavior and state invariants.
...
See [Custom navigators](/router/advanced/custom-navigators/) for standard navigator concepts. Its options example applies to SDK 57: omit `useOnlyUserDefinedScreens` and declare props returned by `createProps` in the fourth `NavigatorContentProps` type argument in SDK 58.
