...
Expo Router supports [static rendering on web](/router/server/static-rendering), which enables search engine optimization (SEO), social media previews, and faster loading times, unlike Expo Webpack. Along with the benefits of React Navigation, it enables automatic deep linking, [type safety](/router/reference/typed-routes), [deferred bundling](/router/server/async-routes), [modular HTML templates](/router/server/static-rendering#root-html), [static rendering on web](/router/server/static-rendering), and more.
...
- **Recommended**: `web.output: "static"` which outputs a new HTML file for each route in the app. This approach lets you [dynamically generate the entire HTML template using the **app/+html.js** file](/router/server/static-rendering#root-html).
