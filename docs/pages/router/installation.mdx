...
The default template creates a project with the [`src/app/`](/router/reference/src-directory/) directory for routes and layout files, [native tabs](/router/advanced/native-tabs/) on mobile platforms, [custom tabs](/router/advanced/custom-tabs/) on web, [typed routes](/router/reference/typed-routes/), and the [React Compiler](https://react.dev/learn/react-compiler) enabled.
