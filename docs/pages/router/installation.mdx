...
In SDK 58 and later, `expo-symbols` and `@expo/ui` are optional peer dependencies of Expo Router. Install only the dependency required by the APIs your app uses:
...
Add a deep linking `scheme`, add the [`expo-router` config plugin](/versions/latest/sdk/router/#configuration-in-app-config), and enable [typed routes](/router/reference/typed-routes/) in your [app config](/workflow/configuration/):
