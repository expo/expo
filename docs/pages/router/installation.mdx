...
In SDK 58 and later, Expo Router defaults to [server rendering](/router/web/server-rendering) when `web.output` is omitted. Use `static` for [static rendering](/router/web/static-rendering) or `single` for a single-page application.
