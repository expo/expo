...
Your non-route files such as components and hooks cannot be placed within the `app` directory. For smaller projects may introduce seemly extra folder structure, but this rule helps prevents refactors as you adopt more advanced navigation patterns.

For example, this file structure has two different route files that both map to `/profile` and both show an identical "profile" screen. The difference between the two files are their containing `_layout`.
...
> **info** **Tip**: Expo recommends sorting components and hooks by "feature" and routes by "navigation pattern". While the two concepts can overlap, there is enough difference to structure them differently.
