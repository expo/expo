...
> **info** **Tip**: Expo recommends sorting components and hooks by "feature" and routes by "navigation pattern". While the two concepts can overlap, there are enough differences to structure them differently. Additionally, routes will inevitably change. However, components generally stay within the same feature set. Keeping routes separate will greatly simplify your refactors.<br /><br /> <ExampleStructure />
> In the example above, both `/sign-in` and `/sign-out` are authentication routes and share components from the **components/authentication** file. However, `/profile/[user]` is complex and renders components that belong to the authentication, profile, and task features. If you place your components within the **app** directory, it's not obvious where these components belong. By separating components from routes, you are creating an organized hierarchy with the philosophy that a route is a collection of reusable components, yet a component does not belong to an individual route. <br /><br />
> Another project optimization is to utilize [path aliases](/guides/typescript/#path-aliases-optional). Path aliases keep your routes project structure agnostic, as imports within your routes are no longer dependent on the **app** directory structure. This avoids large `import` statement changes when adding new directories (such as new groups) and prevents lengthy `import` paths.
