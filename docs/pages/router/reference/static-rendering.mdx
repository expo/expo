...
Expo Font has automatic static optimization for font loading in Expo Router. This means when you load a font with `expo-font`, Expo CLI will automatically extract the font resource and embed it in the page's HTML, enabling preloading, faster hydration, and reduced layout shift.

The following block will load Inter into the namespace and statically optimize on web:
...
This generates the following HTML in the static HTML:
...
- Static font optimization requires the font to be loaded syncronously. If the font isn't statically optimized, it could be because it was loaded inside a `useEffect`, deferred component, or async function.
- Static optimization is only supported with `Font.loadAsync` and `Font.useFonts` from `expo-fonts`. Wrapper functions are supported, so long as the wrappers are synchronous.
