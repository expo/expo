...
Android Material colors respond to the system's light/dark mode, and dynamic colors additionally change when the user updates their Material You colors (for example, by changing the wallpaper). To make your component re-render in both cases, use the `useRouterColor()` hook and read colors from the object it returns.
