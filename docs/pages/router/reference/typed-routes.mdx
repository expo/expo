...
> Expo Router supports standard TypeScript out of the box. [Learn how to setup TypeScript today](/guides/typescript/).
...
Now run `npx expo start` to start the development server. You should now be able to use autocomplete in the Expo Router `<Link />` component's `href` prop.
...
With typed routes enabled, Expo CLI will also augment the `react-native` types to support React Native Web. The following changes are made:
