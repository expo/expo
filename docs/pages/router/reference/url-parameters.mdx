...
{/* prettier-ignore */}
{/*
REMOVE COMMENT WHEN https://github.com/expo/expo/pull/30268 IS PUBLISHED
...
Parameter names with a `[]` suffix will always be returned as an array.

```tsx app/hash.tsx
import { router, useLocalSearchParams } from 'expo-router';

export default function Route() {
  // If the current URL is `/route?myParam[]=1
  const { myParam } = useLocalSearchParams();
  // myParam === ["1"]
}
```
