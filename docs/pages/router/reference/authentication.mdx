...
Next, we'll create a nested [Layout Route](/routing/layouts) which will be used to protect all of the children. This layout will be used to redirect users to the sign in screen if they are not authenticated.
...
We now have an app that can present a loading state while it checks the initial authentication state, and redirect to the sign-in screen if the user is not authenticated. If a user deep links to any protect routes, they'll be redirect back to the sign-in screen.
...
This can occur when the app attempts to perform navigation without a navigator mounted in the [root layout](/router/advanced/root-layout). You can fix this by adding a group and moving conditional logic up a level.
...
Traditionally, websites may leverage some form of server-side redirection to protect routes. As of Expo Router v2, Expo Router on web currently only supports build-time static generation and has no support for custom middleware or serving. This can be added in the future to provide a more optimal web experience, in the meantime, authentication can be implemented by using client-side redirects and a loading state.
