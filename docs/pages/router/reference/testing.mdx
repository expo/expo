...
[`@testing-library/jest-native`](https://testing-library.com/docs/ecosystem-jest-native/) are a set of custom Jest matchers that you can use to extend the functionality of `@testing-library/react-native`. If installed, Expo Router will automatically perform `@testing-library/jest-native`'s setup.
