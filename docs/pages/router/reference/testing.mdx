...
Before you proceed, ensure you have set up `jest-expo` according to the [Unit testing with Jest](/develop/unit-testing/) and [`@testing-library/react-native`](https://callstack.github.io/react-native-testing-library/docs/start/quick-start) in your project. `expo-router/testing-library` requires `@testing-library/react-native` version 14 or newer.
...
Like `render`, `renderRouter` is asynchronous and returns a promise. Always `await` it, and `await` any `act`, `fireEvent`, `rerender`, or `unmount` call that follows it.
...
it('my-test', async () => {
  await renderRouter(['index', 'directory/a', '(group)/b'], {
    initialUrl: '/directory/a',
  });
...
it('my-test', async () => {
  const MockComponent = jest.fn(() => <View />);
  await renderRouter('./my-test-fixture');
});
```
...
For more complex testing scenarios, `renderRouter` can use both directory path and inline-mocking methods simultaneously. The `appDir` parameter takes a string representing a pathname to a directory. The overrides parameter is an inline mock that can be used to override specific paths within the `appDir`. This combination allows for fine-tuned control over the mock environment.
