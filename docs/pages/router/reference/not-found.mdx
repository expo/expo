...
404s can be handled by using a `+not-found.js` route. This route matches all unmatched routes from a nested level, this is akin to `[...wild].tsx`, but unlike the deep dynamic routes, `+not-found.js` is matched after API routes.

On web, the server will first serve files in the following order:

1. Static files in the `public/` folder.
