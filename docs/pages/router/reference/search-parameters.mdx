...
The URL (hash)[https://developer.mozilla.org/en-US/docs/Web/API/URL/hash] is a string that follows the `#` symbol in a URL. It is commonly used on websites link to a specific section of a page, but it can also be used to store data. Expo Router treats the hash as a special search parameter using the name `#`, and it can be accessed and modified using the same hooks and APIs are search parameters.
