...
When the `app/[user]` route is matched, the `user` parameter is passed to the component and never a nullish value. Both search and route parameters can be used together and accessed with the `useLocalSearchParams` and `useGlobalSearchParams` hooks:
