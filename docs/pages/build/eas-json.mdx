...
You may alternatively prefer for your development build to [run in an iOS simulator](/build-reference/simulators.mdx). To do this, use the following configuration for `development` profile:
...
Similar to [development builds](#development-builds), you can configure your preview build to run in the [iOS simulator](/build-reference/simulators.mdx) or create a variant of your preview profile for that purpose. [No such configuration is required to run an Android APK on your device and in an emulator](/build-reference/apk.mdx); the same APK will work in both circumstances.
...
It's common to have development and production builds installed simultaneously on the same device. [Learn about "installing app variants on the same device"](../build-reference/variants.mdx).
