...
**eas.json** is the configuration file for EAS CLI and services. It is located at the root of your project next to your **package.json**. Configuration for EAS Build all belongs under the `build` key. A minimal **eas.json** will look as:
...
Inside each build profile, you can specify the `android` and `ios` fields that contain platform-specific configuration for the build. Fields that are available to both platforms can be provided on the platform-specific configuration object or on the root of the profile.
...
It's common to have development and production builds installed simultaneously on the same device. See [installing app variants on the same device](/build-reference/variants/).
