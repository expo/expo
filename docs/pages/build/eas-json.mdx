...
Similar to [development builds](#development-builds), you can configure your preview build to run on the [iOS Simulator](/build-reference/simulators.mdx) or create a variant of your preview profile for that purpose. [No such configuration is required to run an Android APK on your device and in an emulator](/build-reference/apk.mdx); the same APK will work in both circumstances.
