...
- `schemeBuildConfiguration` is the configuration to use when building the app. When set to `Release` or `Debug`, overrides the native project configuration. When set to `Auto`, uses the value set in the scheme. Defaults to `Release` thus overrides the native project configuration.
