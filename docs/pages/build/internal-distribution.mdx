...
> **warning** **A newly registered device may not be immediately installable.** Registering a device with Expo does not register it with Apple. The device is added to your Apple Developer Portal only when it's first included in a provisioning profile, which happens when you create a new internal build or re-sign an existing one. For new or recently renewed Apple Developer Program memberships, Apple can take [up to 24–72 hours](https://developer.apple.com/help/account/reference/device-registration-updates/) to finish processing a newly registered device, and during that time the device can't be added to provisioning profiles. As a result, the first build or re-sign that includes a new device may fail. Wait for Apple to finish processing, then create a new build or re-sign again to produce an installable build that includes the device.
