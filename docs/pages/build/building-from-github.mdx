...
#### Automatic Store Submission with EAS Submit

Once your build completes, you can automatically submit your app to the digital stores using EAS Submit. This feature streamlines the process, reducing the manual steps required to publish your app.
...
- Select "Edit trigger" and in the dialog that appears, check the option "Submit to store after
  build".
...
Once enabled, every time a build is triggered from this configuration, it will automatically be submitted to the store(s) you have configured in your `eas.json` under the `submit` field.

> **Note:** Ensure that your `eas.json` is properly configured for submission, including specifying
> the correct store credentials and submission profiles. For more information, see the [EAS Submit
> documentation](/submit/eas-json/).
