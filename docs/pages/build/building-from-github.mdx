...
### Build automatically when you push your repository

You can take your build automation further by automatically building your Expo project when you push code to GitHub.

#### Prerequisites

To kick off builds automatically from GitHub, the following must be true: 

- An Expo user in the organization must have a linked GitHub user with access to the target repository. Check your [user settings page](https://expo.dev/settings#connections) to make sure your GitHub user is linked.
...
- You must have valid credentials either configured locally in your repository or hosted on EAS that
  you set up when running your first build with `eas-cli`

#### Set up build triggers

To configure when EAS builds your app from GitHub, you can set up build triggers. We allow you to build when pushing to a branch, pull request, and Git tag.  
...
When you click the "New Build Trigger" button, you'll be presented with a form to configure how this build should run.

These patterns can include wildcards represented by asterisks (`*`), which can match any character
and number of characters inside the pattern. For example, `releases/*` can match `releases/`,
`release/1234`, `release/genesis`, and so on. If you specify the pattern as a sole asterisk (`*`),
...
When you push to a branch or tag, you can find the builds by looking at the “Checks” section on a
commit.
...
For pull requests, you can configure a “target branch pattern”. This is the destination branch of
the pull request you want to build. The same rules apply for wildcards here as well.
...
> on the GitHub repository. If you want to build pull requests from external contributors, apply a
> PR Label.

#### Managing build triggers

On your project's GitHub settings page on expo.dev, you can click the options button to the right of
a build trigger row to manage that trigger. You can disable, edit, or delete the trigger from here.
...
- When things go wrong, we'll comment on the commit we attempted to build with some error information. We also show the latest result in the build triggers UI, which includes error information when you hover the “Error” tag.
...
- It's also important to double-check that everything in the [Prerequisites](https://www.notion.so/Prerequisites-f9d476ee187b419ba505217ccb39a809?pvs=21) section is true when trying to build.
- Confirm that your configured base directory is accurate if you're using a monorepo setup.
- Is your configured build profile correct? If a matching profile can't be found in eas.json, the build will not dispatch.