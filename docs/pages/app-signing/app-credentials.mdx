...
You can use `eas build:resign` to codesign an existing **.ipa** for iOS to a new adhoc provisioning profile to internally distribute your project. This reduces time when you have multiple test iOS devices or want to share a build with your team members.
...
Select a new device, and the command will run the EAS Build again. Note that the build triggered is not a full build. It will reuse the artifacts from the selected build and only codesign them to the new provisioning profile.

Once the process of codesign is complete, you can use this new build link to install the **.ipa** on the iOS device whose provisioning profile was selected during the process.