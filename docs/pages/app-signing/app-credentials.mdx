...
You can use `eas build:resign` to resign an existing **.ipa** for iOS to a new ad hoc provisioning profile. This helps reduce time when distributing internally &mdash; for example, if you want to add a new test device to an existing build, you can use this command to update the provisioning profile to include the device without rebuilding the entire app from scratch.
...
Select a new device, and the command will run the EAS Build again. Note that the build triggered this time reuses the application artifact from the selected build and resigns it with the new provisioning profile. Once the process of resigning is complete, you can use this new build link to install the **.ipa** on the iOS device that was added to the provisioning profile during the process.
