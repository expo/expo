...
### Resigning new credentials

You can use `eas build:resign` to codesign an existing **.ipa** for iOS to a new ad hoc provisioning profile. This helps reduce time when distributing internally, for example, when you have multiple test iOS devices or want to share a build with your team members.

Running the command will ask you to select a build that you want to resign. For example, running the command in an example project shows an available build:
...
Select a new device, and the command will run the EAS Build again. Note that the build triggered this time is not a full build. It reuses the artifacts from the selected build and only codesign them to the new provisioning profile. Once the process of codesign is complete, you can use this new build link to install the **.ipa** on the iOS device whose provisioning profile was selected during the process.