...
description: Learn how to open and handle a URL from other apps based on the scheme for your by creating a deep link.
---
...
Additionally, many messaging apps do not autolink URLs with custom schemes. For example, `exp://u.expo.dev/[project-id]?channel-name=[channel-name]&runtime-version=[runtime-version]` might appear as plain text in your browser rather than as a link ([exp://u.expo.dev/[project-id]?channel-name=[channel-name]&runtime-version=[runtime-version]](#)). A common example is the Gmail app which strips the `href` attribute from the links of most apps.

One solution is to use a regular HTTPS URL instead of your app's custom scheme. This will open the user's web browser, where the `href` property is usually preserved. You can host a file online that redirects the user to your app's custom scheme.
