...
If you're building a [statically rendered website with Expo Router](/router/server/static-rendering), then add the HTML tag to the `<head>` component in your [**app/+html.js** file](/router/server/static-rendering#root-html).
