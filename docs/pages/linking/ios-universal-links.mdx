...
description: Learn how to configure iOS Universal Links in your Expo apps to open your app from a standard web URL.
---
...
iOS Universal Links differ from [standard deep links](/linking/into-your-app/), as they use regular HTTP and HTTPS schemes and are exclusive to iOS devices. They allow your app to always open when a user clicks a link instead of presenting a dialog for the user to choose between the browser or another handler. If a user doesn't have your app installed, the link takes them to the associated website.

This guide provides steps to configure iOS Universal Links in your project.

## Set up two-way association

You need to set up the **two-way association** between the website and app for iOS. This is done in two steps:

- **Website verification:** This requires creating a **apple-app-site-association** file inside the **/.well-known** directory and hosting it on the target website. This file is used to verify that the app opened from a given link is the correct app.
    ...
    "details": [
      {
        ...
        /* @info Replace <CODE>QQ57RJ5UTD.com.acme.myapp</CODE> with your own Apple Team ID and bundle identifier. */
        "appID": "QQ57RJ5UTD.com.acme.myapp",
        /* @end */
...
- Any links to `https://www.myapp.io/records/*` (with wildcard matching for the record ID) should be opened directly by the app with a matching bundle identifier on an iOS device. It is a combination of the Apple Team ID and the bundle identifier. The Apple Team ID can be found under the membership details in the Apple Developer account.
...
As of iOS 13, the [`details` format is supported](https://developer.apple.com/documentation/xcode/supporting-associated-domains) which allows you to specify:
...
After deploying your **apple-app-site-association** (AASA) file, configure your app to use your associated domain by adding [`ios.associatedDomains`](/versions/latest/config/app/#associateddomains) to your [app config](/workflow/configuration/).

Make sure to follow [Apple's specified format](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_developer_associated-domains) and **not** include the protocol (`https`) in your URL. This is a common mistake that will result in the universal links not working.
...
## Test and debug iOS Universal Links locally
...
You can now visit the your custom domain link on your device to open your app for the URL.
