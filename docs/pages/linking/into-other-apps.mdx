...
You can use [`Linking.createURL`](/versions/latest/sdk/linking/#linkingcreateurlpath-namedparameters) to create a URL that can be used to open or redirect back to your app. This method resolves to the following:

- **Production and development builds**: `myapp://`, where `myapp` is the [custom scheme](/linking/into-your-app/#add-a-custom-scheme-in-app-config) defined in the app config
