...
> If you know the custom scheme for the app you want to open from your app, you can link to it from your app using the method described in [Usage](#usage).
...
In the above example, it can be possible that the user might not have the Uber app installed on their device. In this case, your app can open Google Play Store or Apple App Store, and let the user know that they need to install the desired app. We recommend using the [`react-native-app-link`](https://github.com/FiberJW/react-native-app-link) library to handle these scenarios.

### Specify custom schemes for iOS

On iOS, you can use [`Linking.canOpenURL()`](/versions/latest/sdk/linking/#linkingcanopenurlurl) to query other apps's linking schemes. It requires additional configuration to specify a list of schemes in **InfoPlist**. You can use the [`ios.infoPlist`](/versions/latest/config/app/#infoplist) property in your app config to specify a list of schemes your app needs to query. For example:
