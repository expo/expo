...
The following example shows a basic configuration that enables your app to appear in the standard Android dialog as an option for handling any links to the `webapp.io` domain. It also uses regular `https` scheme since [Android App Links](/linking/overview/#android-app-links) are different from [standard deep links](/linking/into-other-apps/).
...
To setup **two-way association** between the website and app for Android, you need to perform the following steps:
...
Install the app on an Android device to trigger the [Android app verification](https://developer.android.com/training/app-links/verify-android-applinks#web-assoc) process. This process takes at least 20 seconds.
...
You can use the following `adb` command to start the intent activity and open the link on your app or type the custom domain link in your device's web browser.
