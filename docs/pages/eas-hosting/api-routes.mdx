...
> **info** This page is for EAS Hosting specific details about API routes. For general documentation about API routes, see the [API routes](/router/reference/api-routes/) under Expo Router.
...
All logs from API routes (`console.log`, `console.info`, `console.error` and so on) are recorded on the **Logs** page. These are only available per deployment at **Deployments** > _select a deployment_ > **Logs**.
