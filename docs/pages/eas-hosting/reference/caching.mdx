...
To differentiate between what a browser and EAS Hosting may cache, the `s-maxage` directive may be used. For example, responding with the `s-maxage=3600` directive will allow EAS Hosting to cache the response for 3,600 seconds, while the user’s browser won’t cache it at all.
...
export async function GET(request) {
  return Response.json({ ... }, {
    headers: {
	    ...
	    'CDN-Cache-Control': 'max-age=3600', // EAS Hosting should cache for 3,600s
    },
  });
}
```
