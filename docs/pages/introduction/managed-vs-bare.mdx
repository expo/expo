...
Developers build managed workflow apps using [expo-cli](/workflow/expo-cli.mdx) on their computer and a development client on their mobile devices (either the Expo Go app for more simple projects or a [development build](/development/introduction.mdx) when your project grows). Managed workflows apps typically use one or more Expo services, such as [push notifications](/push-notifications/overview.mdx), [build](/archive/classic-updates/building-standalone-apps.mdx), and [updates](/eas-update/getting-started.mdx).
...
While you can do a lot with the managed workflow, you can't do _everything_ with it, so what are your options when you encounter a [limitation](/introduction/why-not-expo.mdx)?
