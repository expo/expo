...
Expo's Metro config automatically supports the `compilerOptions.paths` and `compilerOptions.baseUrl` fields in your `tsconfig.json` (or `jsconfig.json`) file. This allows you to use absolute imports and aliases in your project. Learn more in the [TypeScript guide](/guides/typescript).
