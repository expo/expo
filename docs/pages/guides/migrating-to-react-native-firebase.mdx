...
- Replacing the use of Expo Go for testing with a [development build](/development/introduction/) customized to your project's needs
- Testing the updated code on your development build

> **info** These instructions assume you have already been successfully using the Expo Firebase packages. If you're adding Firebase to your app for the first time, see the [Using Firebase](/guides/using-firebase) guide.
...
If you are using [Expo Prebuild](/guides/adopting-prebuild) (also known as Managed Workflow) and building your app for iOS, install the `expo-build-properties` plugin and apply the following configuration to your [Expo config](/workflow/configuration/) file.

<Terminal cmd={["$ npx expo install expo-build-properties"]} />

### Add Config plugin to Expo config file

In the Expo config file, add add `expo-community-flipper` to the `plugins` array with the following configuration:
...
If you have previously used Expo Go for development and testing, at this point, you will need to migrate to a [development build](/development/introduction/#from-expo-go-to-development-builds), as Expo Go does not include the native code used in React Native Firebase.

[Follow these instructions for creating a development build](/development/getting-started/). Once you are finished, you should have an **eas.json** file that includes a build profile that looks like this:
...
```
If you wish to run the [development build on an iOS simulator](/build-reference/simulators/) instead of a device, include the `ios.simulator` in your profile:
