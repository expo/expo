---
title: Migrating to React Native Firebase
---

import { Terminal } from '~/ui/components/Snippet';

The `expo-firebase-analytics` and `expo-firebase-recaptcha` packages were created to allow use of some Firebase features across Classic Builds, Expo Go, and Expo for Web. However, they had the disadvantage of being tied to specific versions of the [Firebase](https://firebase.google.com/) native iOS and Android SDK's, preventing developers from upgrading to the latest version of [React Native Firebase](https://rnfirebase.io/) when they were ready to use additional Firebase features. With EAS Build and Development Builds, it is much easier to use React Native Firebase in your entire development workflow, so these packages have been deprecated. Read on for advice on migrating from the Expo Firebase packages to React Native Firebase.
...
```
If you wish to run a development build on an iOS simulator instead of a device, your profile may also include:
```
{
  "ios": {
    "simulator": true
...
```
`expo-dev-client` should be present in your **package.json**, as well. Once you run `eas build` with your development build profile and platfrom of choice, you will have an app binary file that you can install on your device or simulator.

If you are already using a development build, just rebuild your app, as your native code dependencies have changed.
...
React Native Firebase only supports iOS and Android. If you previously used `expo-firebase-analytics` to send events from an Expo for web client, you will need to use the Firebase JavaScript SDK. In, the Firebase Console, go to Project Settings, and then your web app, and copy the code under "SDK setup and configuration." Then you can switch between React Native Firebase and Firebase JS requests using the `Platform` component:
```
