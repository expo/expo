...
To do so, use the [`expo-localization`](/versions/latest/sdk/localization/#installation) config plugin and define the `extra.supportedLocales` property in the app config:
...
If you need to specify different locale values for Android and iOS, use the `supportedLocales.ios` and `supportedLocales.android` fields:
