...
Several regions around the world write text from right to left. To make your app look as expected in RTL languages, you need to make sure it handles these layout and text direction changes accordingly.

RTL support is enabled by default. Layout direction follows React Native's [`I18nManager`](https://reactnative.dev/docs/i18nmanager): the app renders in RTL when the device is set to an RTL language (such as Arabic or Hebrew), and in LTR otherwise. This applies in Expo Go, in the dev client, and in apps built with EAS Build or `npx expo prebuild`.
...
{
  "expo": {
    "plugins": [["expo-localization", { "supportsRTL": false }]]
  }
}
```
...
You can force the RTL layout for testing, or for applications that are only localized for RTL locales, by setting the `forcesRTL` option to `true` on the `expo-localization` config plugin:

```json app.json
{
  "expo": {
    "plugins": [["expo-localization", { "forcesRTL": true }]]
  }
}
```
