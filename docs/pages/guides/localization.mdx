...
To enable RTL support you have to install the [`expo-localization`](/versions/latest/sdk/localization/#installation) library and enable `extra.supportsRTL` property in app config:
...
You can also force the RTL layout for testing or for applications that are only localized for RTL locales. To do so, also set the `extra.forcesRTL` property in app config:
