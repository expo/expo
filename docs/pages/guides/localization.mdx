...
### Changing Expo Router direction at runtime

If your app lets users change languages without restarting, wrap the navigator in `LocaleProvider` in the root **app/\_layout.tsx**. Pass the direction for the selected locale to update navigation headers, transitions, and gestures.
...
`LocaleProvider` configures Expo Router's navigation components. Continue to set the layout direction for your app content separately.
