...
Run the following command to regenerate the `/ios` and `/android` directories based on the app config (**app.json**) configuration:
...
If your project has any native modifications (changes to the **ios** or **android** directories, such as app icon configuration or splash screen) then you'll need to configure your app config (**app.json**) to reflect those native changes.
