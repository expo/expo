...
Here is a quick example. If you are rendering a text input with the following bold text:
...
Then, you append a fifth letter `a`, the position of your cursor should determine if the new letter is a part of the bold string or not. Unfortunately, the callback only returns `aaaaa`.
