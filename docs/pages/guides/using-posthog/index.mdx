...
Render `<ReleaseTagger />` inside `<PostHogProvider>`. Every subsequent `capture()` carries these properties, so you can filter or group events by `eas/update_id` in PostHog. `eas/account` has a value only when your [app config](/workflow/configuration/) sets `owner`.
