...
- [Release tagging](#release-tagging), [standard EAS properties](#standard-eas-properties), [feature flags](#feature-flags), and [troubleshooting](#troubleshooting)
...
Map each captured event back to the update, project, and account that produced it by registering [super properties](https://posthog.com/docs/libraries/react-native#super-properties) from [`expo-updates`](/versions/latest/sdk/updates/) and [`expo-constants`](/versions/latest/sdk/constants/):
...
Render `<ReleaseTagger />` inside `<PostHogProvider>`. Every subsequent `capture()` carries these properties, so you can filter or group events by `eas/update_id` in PostHog. `eas/account` has a value only when your [app config](/workflow/configuration/) sets `owner`. `eas/update_id` and `eas/channel` are set only in release and preview builds. In Expo Go and development builds they are `null`, so events from those builds do not carry them.

## Standard EAS properties

PostHog recognizes these `eas/` names. It shows the Expo logo next to each and links the ones that carry an identifier to their page on expo.dev. Where each value comes from depends on whether the event fires in your app or in a workflow:

| Property              | In the app       | In a workflow         | Opens in PostHog           |
| --------------------- | ---------------- | --------------------- | -------------------------- |
| `eas/update_id`       | `expo-updates`   | update job            | `expo.dev/updates/<id>`    |
| `eas/build_id`        | not available    | build job             | `expo.dev/builds/<id>`     |
| `eas/channel`         | `expo-updates`   | build job             | no link                    |
| `eas/runtime_version` | `expo-updates`   | build or update job   | no link                    |
| `eas/project_id`      | `expo-constants` | `${{ app.id }}`       | `expo.dev/projects/<id>`   |
| `eas/account`         | `expo-constants` | `${{ account.name }}` | `expo.dev/accounts/<name>` |
| `eas/workflow_id`     | not available    | `${{ workflow.id }}`  | `expo.dev/workflows/<id>`  |

The `ReleaseTagger` snippet sets the in-app values from `expo-updates` and `expo-constants`. In a workflow, set the same names on [`eas/posthog_capture_event`](/eas/workflows/syntax/#easposthog_capture_event): `eas/account`, `eas/project_id`, and `eas/workflow_id` come from the [`account`](/eas/workflows/syntax/#account), [`app`](/eas/workflows/syntax/#app), and [`workflow`](/eas/workflows/syntax/#workflow) contexts, which every job can read. The build and update fields come from a build or update job's outputs. The [recipes](/guides/using-posthog/recipes) show this in context. `eas/update_id` is the individual update ID in the app and the update group ID in a workflow, where one publish produces a group of platform-specific updates. Both open the update on expo.dev.
