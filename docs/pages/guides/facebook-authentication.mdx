...
The [`react-native-fbsdk-next`](https://github.com/thebergamo/react-native-fbsdk-next/) library provides a wrapper around Android and iOS Facebook SDKs. It allows integrating Facebook authentication into your Expo project and provide access to native components.

This guide provides additional information on configuring the library with Expo for Android.

## Prerequisites

`react-native-fbsdk-next` requires [custom native code](/workflow/customizing/) and cannot be used with Expo Go. Before proceeding, you can either use:

- Android Studio or Xcode to build your project locally (see [Prebuild](/workflow/prebuild/) for more information)
...
Adding Android as a platform in your Facebook project requires you to have your app approved by Google Play Store, and the [`package`](/versions/latest/config/app/#package) name associated with your app has a valid Play Store URL. Otherwise, you'll run into the following error:
