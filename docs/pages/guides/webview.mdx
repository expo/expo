...
You can send type-safe native functions to webview components using the `actions` prop:
...
export default function MyComponent({ actions }: { actions: { hello: (data: string) => void } }) {
  return <p onClick={() => actions.hello('world')}>Click me</p>;
}
```
...
You can also send and receive serializable data between your native app and webviews using the `useBridge` API from `expo/webview`:
