---
title: Deep linking
description: Learn how to use universal/app links to open your app from a standard web URL.
---
...
Unlike standard deep links which use a custom URL scheme, universal links are regular HTTPS links that point to a specific route in your app. If your app isn't installed on when a user opens the link, they'll be taken to the associated website. This allows you to send notification emails with links that work as expected in the web browser on a desktop, while opening the content in your app on mobile. Android refers to this concept as "app links" and iOS refers to it as "universal links". In this section, universal links that do not use a custom URL scheme are specifically discussed.
...
Before you can use App/Universal Links, you must setup the two-way association between website and app, for both iOS and Android:
...
After the two-way association is setup, you need to setup runtime routing of the link in your app. This is done in JavaScript and must be configured for every route, then coordinated between web and native. Expo offers a fully automated solution for this called [Expo Router](/router/introduction).
...
For example, if an associated website is `https://expo.dev/`, the app links will be:
...
On the web-side, you must host a config file from `/.well-known/apple-app-site-association` (with no extension). This file contains JSON which specifies your Apple Developer Team ID, Bundler ID, and a list of supported paths to redirect to the native app.
...
If you're using [Expo Router](/router/introduction) to build your website (or another modern React framework like Remix, Next.js, etc.), create the file at `public/.well-known/apple-app-site-association`, legacy Expo Webpack projects can create the file at `web/.well-known/apple-app-site-association`.
...
If you're building a [statically rendered website with Expo Router](/router/reference/static-rendering), then you should add this HTML tag to the **<head>** component in your [**app/+html.js** file](/router/reference/static-rendering#root-html).
...
The next time you deploy your website, the banner should appear when you visit it on a mobile device that doesn't have your app installed.
...
Create a JSON file for the website verification (aka [digital asset links](https://developers.google.com/digital-asset-links/v1/getting-started) file) at `public/.well-known/assetlinks.json` (or `web/.well-known/assetlinks.json` for legacy Expo Webpack websites), and collect the following information:
...
- If you update your web files, be sure to rebuild the native app to trigger a server update on the vendor-side (Apple, Google).
