...
<Step label="2">
  Add `"autoVerify": true` to the intent filter in your [app config](/workflow/configuration/). This
  tells Android to check for your **assetlinks.json** on your server and register your app as the
  automatic handler for the specified paths:
</Step>
  ...
  "expo": {
    "android": {
      "intentFilters": [
        {
          "action": "VIEW",
          /* @info Add autoVerify property to the intent filter in app config.*/
          "autoVerify": true,
          /* @end */
          "data": [
            {
