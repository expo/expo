...
> **info** Deferred deep links can be implemented with [`react-native-branch`](https://github.com/expo/config-plugins/tree/main/packages/react-native-branch).
...
> **info** The `*` wildcard does **not** match domain or path separators (periods and slashes).
