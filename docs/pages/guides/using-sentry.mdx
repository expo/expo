...
After you publish an update with `eas update`, run `npx sentry-expo-upload-sourcemaps dist` (where `dist` is the directory containing your bundles and source maps) to create a new release on Sentry and upload the source maps.
