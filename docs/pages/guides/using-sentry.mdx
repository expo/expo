...
- Running `eas update` or `npx expo export` will generate a **dist** folder in your project root, which contains your JavaScript bundles and sourcemaps. This command will also output the 'Android update ID' and  'iOS update ID' that we'll need in the next step.
- Copy or rename the bundle names in the **dist/bundles** folder to match **main.jsbundle** (iOS) or **index.android.bundle** (Android)/ 
...
These steps define a new 'dist' on Sentry, under the same release as your full app build, and associate the sourcemaps with this dist. If you want to customize this behavior, you can pass in your own values for `release` and `dist` when you initialize Sentry in your code. For example:
