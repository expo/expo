...
<ImageSpotlight alt="Sentry update tags" src="/static/images/guides/sentry-tags.png" />
...
> `sentry-expo` has been merged into `@sentry/react-native` and is now depreacted. We recommend upgrading to SDK 50 to use `@sentry/react-native` for the best experience. If you're already using `sentry-expo`, [learn how to migrate](https://expo.fyi/sentry-expo-migration).
...
### Configuring Expo Router

If your app uses [Expo Router](/router/introduction), then you can configure Sentry to automatically capture the current route and pass it along with your error reports.

Open the [Root Layout route](/router/advanced/root-layout) (**app/\_layout.tsx**) and configure Sentry to capture error information:
