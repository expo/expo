  ...
  const [request, response, promptAsync] = useAuthRequest(
    {
      ...
      scopes: ['openid'],
      redirectUri: makeRedirectUri({
        /* @info The URI <code>[scheme]://</code> to be used in bare and standalone. If undefined, the <code>scheme</code> property in your app.json or app.config.js is used instead. */
        scheme: 'your.app'
        /* @end */
      }),
    },
...
- If **Manual** is selected, an authentication URL is returned as part of a JSON response. No redirects are needed and do not require web service authentication. The result is a completely silent OAuth 2.0 authentication using Passkeys. Since the challenge is packaged as part of the authentication URL, following the URL will result in the same behavior as if an Invocation Type of **Automatic** were selected.
