...
<Grid>
  ...
  <GridItem
    ...
    image={ASSETS.apple}
  />
  <GridItem title="Beyond Identity" protocol={['OAuth 2', 'OpenID']} image={ASSETS.beyondidentity} />
  ...
  <GridItem title="Facebook" protocol={['OAuth 2']} image={ASSETS.facebook} />
  ...
  <GridItem title="Google" protocol={['OAuth 2', 'OpenID']} image={ASSETS.google} />
...
| Website                             | Provider | PKCE      | Auto Discovery |
| ----------------------------------- | -------- | --------- | -------------- |
| [Sign Up][c-beyondidentity] | OpenID   | Supported | Available      |

[c-beyondidentity]: https://www.beyondidentity.com/developers/signup
[c-beyondidentity-docs]: https://developer.beyondidentity.com
[c-beyondidentity-github]: https://github.com/gobeyondidentity/bi-sdk-react-native
[c-beyondidentity-authenticator-config]: https://developer.beyondidentity.com/docs/v1/platform-overview/authenticator-config

> Beyond Identity allows developers to implement strong passwordless authentication based on public-private key pairs called "Universal Passkeys". All keys are cryptographically linked to the user and can be centrally managed using the Beyond Identity APIs. You will need a Universal Passkey before you can authenticate. 

> For information on Universal Passkeys, documentation and guides visit the [developer documentation][c-beyondidentity-docs].

> For a complete example app and installation instructions visit the sdk on [github][c-beyondidentity-github].

<Tabs tabs={["Auth Code w/ automatic invocation", "Auth Code w/ manual invocation"]}>
<Tab>
Set your Beyond Identity [Authenticator Config][c-beyondidentity-authenticator-config]'s Invocation Type to <code>"Automatic"</code>. If "Automatic" is selected, Beyond Identity will automatically redirect to your application using the Invoke URL (the App Scheme or Univeral URL pointing to your application).
{/* prettier-ignore */}
```tsx Beyond Identity Auth Code w/ Invocation Type "Automatic"
import * as React from 'react';
import { makeRedirectUri, useAuthRequest, useAutoDiscovery } from 'expo-auth-session';
import { Button } from 'react-native';
/* @info Install the Beyond Identity Embedded SDK. This can only be used in standalone and bare workflow native apps. This SDK cannot be used in the Expo Go app. */
import { Embedded } from '@beyondidentity/bi-sdk-react-native';
/* @end */

export default function App() {
  // Endpoint
  const discovery = useAutoDiscovery(`https://auth-${REGION}.beyondidentity.com/v1/tenants/${TENANT_ID}/realms/${REALM_ID}/applications/${APPLICATION_ID}`);
  // Request
  const [request, response, promptAsync] = useAuthRequest(
    {
      clientId: `${CLIENT_ID}`,
  ...
  React.useEffect(() => {
    const authenticate = async (url) => {
     ...
     /* @info The response does not need to be of type 'success' if it has a url. The state value is stored in a JWT in the url 'request' paramater and the url will be validated through the Beyond Identity Embedded SDK.  */
     if (response?.url) {
     /* @end */
      authenticate(url)
    }
...
<Box
  name="Facebook"
  createUrl="https://developers.facebook.com/apps/"
...
<Box
  name="Google"
  createUrl="https://console.developers.google.com/apis/credentials"
  image={ASSETS.google}
>
