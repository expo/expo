...
description: Learn how to configure status bar using expo-status-bar library.
---
...
<ImageSpotlight
  src="/static/images/status-bar-style-comparison.png"
...
`expo-status-bar` builds on top of the `StatusBar` component that React Native provides to give you better defaults when you're building an app with Expo tools.
...
To do this, we recommend using [`react-native-safe-area-context`](/versions/latest/sdk/safe-area-context) to find the safe area insets and add padding or margins to your layout accordingly.
...
You can place an empty `View` on top of your screen with a background color to act as a status bar or set top padding.
You can get the height of the status bar (and notch, if there is one) by using the top inset value provided by [`react-native-safe-area-context`](/versions/latest/sdk/safe-area-context).
