...
To configure the status bar while the splash screen is visible on Android is available through the `androidStatusBar` object in the [Expo config](/workflow/configuration). The options available are similar to those provided by [`expo-status-bar`](/versions/latest/sdk/status-bar).
