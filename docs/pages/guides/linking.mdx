...
- _Production and development builds_: `myapp://` (see [#linking-to-your-app](#linking-to-your-app))
- _Development in Expo Go_: `exp://127.0.0.1:8081`. For SDK 48 and lower, the port number is `19000`.

When loading published updates in Expo Go, the URL that is created by `Linking.createURL()` depends on the manner in which the project was launched and shouldn't be relied upon to be consistent or stable. For applications that require a stable URL (authorization provider redirects, for example), you should use a development build with a custom scheme instead (see [#linking-to-your-app](#linking-to-your-app)).

- _Project loaded via QR code or via menus in Expo Go_: `exp://u.expo.dev/[project-id]/group/[update-group-id]` or `exp://u.expo.dev/[project-id]/update/[update-id]`. The project ID is stable but the update group ID and update ID are not.
...
- _Production and development builds_: `myapp://path/into/app?hello=world`
