...
If you don't specify this list, `Linking.canOpenURL` may return `false` regardless of whether the device has the app installed. Note that this configuration can only be tested in [development builds](/develop/development-builds/introduction/), because it requires native changes that will not be applied when testing in [Expo Go][expo-go].
...
- _Builds and development builds_: `myapp://` (see [#linking-to-your-app](#linking-to-your-app))
- _Development in Expo Go_: `exp://127.0.0.1:8081`. For SDK 48 and lower, the port number is `19000`.

When loading published updates in Expo Go, the URL that is created in `createURL` depdends on the manner in which the project was launched and shouldn't be relied upon to be consistent or stable. For applications that require a stable URL (authorization provider redirects for example), a development build and custom scheme should be used (see [#linking-to-your-app](#linking-to-your-app)).

- _App loaded via QR code or via menus in Expo Go_: `exp://u.expo.dev/[project-id]/group/[update-group-id]` or `exp://u.expo.dev/[project-id]/update/[update-id]`. The project ID is stable but the update group ID and update ID are not.
...
- _Builds and development builds_: `myapp://path/into/app?hello=world`
- _Development in Expo Go_: `exp://127.0.0.1:8081/--/path/into/app?hello=world`. For SDK 48 and lower, the port number is `19000`.

Again, when loading published updates in Expo Go this behavior is undefined, but may look something like the following:

- _Published app in Expo Go_: `exp://u.expo.dev/[project-id]/group/[update-group-id]/--/path/into/app?hello=world`
...
To link to your [development build](/develop/development-builds/introduction/) or standalone app, you need to specify a custom URL scheme for your app. You can register a scheme in your app config (**app.json**, **app.config.js**) by adding a string under the `scheme` key:
