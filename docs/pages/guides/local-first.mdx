...
It works with Expo React Native (via [`react-native-mmkv`](https://github.com/mrousavy/react-native-mmkv)) out of the box! This makes it a perfect match for building local-first web and mobile apps! Get started by using the [Legend-State Supabase example](https://github.com/expo/examples/tree/master/with-legend-state-supabase): `npx create-expo-app --example with-legend-state-supabase`.
