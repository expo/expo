...
[Jazz.tools](https://jazz.tools/) is a framework for building local-first apps. It is open source, provides first-class support for Expo and you can [self-host](https://jazz.tools/docs/getting-started/server-setup#self-hosted-database-server) it to start quickly. To learn more, check out the [examples](https://github.com/garden-co/jazz/tree/main/examples/todo-client-localfirst-expo) or see the [Getting Started Guide](https://jazz.tools/docs/install/client#jazz-framework-expo) for detailed instructions.
