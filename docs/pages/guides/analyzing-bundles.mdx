...
sidebar_title: Analyzing JavaScript bundles
...
You can also use Expo Atlas when generating a production bundle for your app or EAS Update. Atlas generates a **.expo/atlas.jsonl** file during export, which you can share and open without having access to the project. This file contains the original and transformed source code of every bundled module, including the values of inlined [`EXPO_PUBLIC_` environment variables](/guides/environment-variables/). Treat it like your source code and only share it with people you trust.
