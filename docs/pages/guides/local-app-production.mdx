...
To submit the **.aab** file, upload it manually through Google Play Console. Alternatively, [EAS Submit](/deploy/submit-to-app-stores/#can-i-submit-builds-that-were-not) accepts locally built binaries with `eas submit --platform android --path ./my-app.aab`.
