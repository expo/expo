...
While `process.env.EXPO_OS` will always be web in a DOM component, you can detect the _top_ platform with `process.env.EXPO_TOP_OS`. This will be either `ios`, `android`, depending on the OS of the top-most native platform, and `undefined` on web.
