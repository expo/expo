...
While `process.env.EXPO_OS` will always be web in a DOM component, you can detect the owner platform with `process.env.EXPO_DOM_OS`. This will be either `ios`, `android`, depending on the parent OS, and `undefined` on web.
