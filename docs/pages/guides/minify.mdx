...
You may want to remove console logs from your production build. This can be done by using the `drop_console` option in the Terser minifier config.
...
config.transformer.minifierConfig = {
  compress: {
    // The option below removes console logs from production
    drop_console: true,
  },
};
