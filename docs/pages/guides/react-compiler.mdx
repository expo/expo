...
> **Note:** As of React Compiler 1.0, the compiler's lint rules have been merged into `eslint-plugin-react-hooks`. If you are using React Compiler 1.0 or later, follow the updated instructions below. For older setups (React Compiler beta / SDK 53 and earlier), use `eslint-plugin-react-compiler` instead.

<Step label="1">

Run [`npx expo lint`](/guides/using-eslint/#eslint) to ensure ESLint is setup in your app, then install the ESLint plugin:

<Tabs tabs={['React Compiler 1.0+ (SDK 54 and later)', 'React Compiler beta (SDK 53 and earlier)']}>
  <Tab>
