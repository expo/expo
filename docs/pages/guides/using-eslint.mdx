...
To integrate with ESlint, update your `.eslintrc.js`:
...
Now when you running `npx expo lint`, anything not alighned with prettier formatting will be caught as an error. Side note: you may want to do `"prettier/prettier": "warn"` instead if you prefer these to be warnings instead of errors.
