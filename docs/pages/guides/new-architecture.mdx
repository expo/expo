...
> **info** We recommend using SDK 51 or newer for the best experience with the New Architecture.
...
**As of SDK 52**, all new projects will be initialized with the New Architecture enabled by default. To create a new project using SDK 52 beta:

<Terminal cmd={['$ npx create-expo-app@latest -t default@sdk-52']} />

**For SDK 51 and below**, the easiest way to get started with the New Architecture is to use the `with-new-arch` template when creating a new project:
...
- **expo-notifications**: handling notifications in the background is not yet supported.
