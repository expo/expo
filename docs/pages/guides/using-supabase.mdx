...
After you have created your [Expo project](/get-started/create-a-project/), you can install supabase-js and dependencies using the following command:

<Terminal cmd={['$ npx expo install @supabase/supabase-js @react-native-async-storage/async-storage react-native-elements react-native-url-polyfill']} />
</Step>

<Step label="2">
#### Initialize supabase-js in your project

Simply create a helper file to initialize the Supabase client.
We need the API URL and the `anon` key that you copied [earlier](#get-the-api-keys).
These variables are safe to expose in your Expo app since we have
[Row Level Security](https://supabase.com/docs/guides/auth/row-level-security?utm_source=expo&utm_medium=referral&utm_term=expo-react-native) enabled on our Database.

```js utils/supabase.ts
...
That's it, now you can `import { supabase } from '/utils/supabase'` throughout your application to leverage the full power of Supabase!
...
<BoxLink
  title="Deep Linking for OAuth & Magic Links"
  description="When performing OAuth or sending magic link emails from native mobile applications we need to configure deep linking for our iOS and Android applications."
