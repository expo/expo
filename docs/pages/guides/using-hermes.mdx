...
> **info** Hermes is supported for apps built with [EAS Build](/build/introduction). There are no plans to backport support to `expo build`. [Jump to "Limitations"](#limitations).
...
> **info** Hermes for Android is supported from SDK 42 and above in Expo Go, [development builds](/development/introduction.mdx) using `expo-dev-client` and standalone apps built with EAS Build. For bare apps created before SDK 42, [follow these instructions to update your project configuration](https://expo.fyi/hermes-android-config).
...
> **info** Hermes for iOS is supported from SDK 43 and above in [development builds](/development/introduction.mdx) using `expo-dev-client` and standalone apps built with EAS Build. Hermes is not supported in Expo Go for iOS. For bare apps created before SDK 43, [follow these instructions to update your project configuration](https://expo.fyi/hermes-ios-config).
...
> **info** `ld: building for iOS Simulator, but linking in dylib built for iOS, file '/path/to/projectName/ios/Pods/hermes-engine/destroot/Library/Frameworks/iphoneos/hermes.framework/hermes' for architecture arm64`
...
> **info** [Development builds](/development/introduction.mdx) built with `expo-dev-client` simplify this process by integrating directly with the JavaScript inspector in Hermes.
