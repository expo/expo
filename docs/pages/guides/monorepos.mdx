...
config.resolver.nodeModulesPaths = [
  ...
  path.resolve(workspaceRoot, 'node_modules'),e
];
...
As your monorepo increases in size, watching all files within the monorepo becomes slower. You can speed things up by only watching the packages your app uses. Typically, these are the ones that are installed with an asterik (*) in your package.json. For example:
