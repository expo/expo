...
In a monorepo, your app will typically be in a sub-directory of your repository and your package manager is configured to allow you to add dependencies to other packages from within your monorepo.
...
Packages that aren't native modules can break when they're duplicated too. A library that creates a React context is a common example. To apply the same checks and resolution to those packages, see [Checking packages that aren't native modules](/modules/autolinking/#checking-packages-that-arent-native-modules).
