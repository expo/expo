...
Packages that aren't native modules can break when they're duplicated too, for example a library that creates a React context. To apply the same checks and resolution to them, see [Checking packages that aren't native modules](/modules/autolinking/#checking-packages-that-arent-native-modules).
