...
> **info** Expo's automatic monorepo setup adds all workspaces in your monorepo as `watchFolders` to Metro. This might slow Metro down for larger monorepos. You can also manually configure which folders Metro should watch by changing the `watchFolders` in your **metro.config.js**, see [Configure Metro](#configure-metro).
...
Expo's Metro config has monorepo support for bun, npm, yarn, and pnpm. By default, if you use Expo CLI and do not have a **metro.config.js** file, you be will using the config from `expo/metro-config`, and you can proceed to the next step. 

[If you customize your **metro.config.js**](/guides/customizing-metro), we strongly recommend that you extend `expo/metro-config` to get the best experience with monorepos and Expo tools in general.

If for some reason this is not possible, or if you extend `expo/metro-config` and want to ensure that you are not overwriting configuration that is relevant to monorepos, there are two main configuration changes to be aware of:
...
<Collapsible summary="Example metro.config.js">

The following **metro.config.js** demonstrates configuration changes `expo/metro-config` applies automatically relevant to monorepos, to demonstrate what is happening under the hood. You do not need to copy this code into your project, but use it as a reference when you are customizing your **metro.config.js** if you change any of the config properties referenced below.
...
In the [Configure Metro](#configure-metro) step, we instructed Metro to do a couple of this, specifically:
