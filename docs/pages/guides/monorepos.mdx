...
Since SDK 52, Expo's Metro config configures itself automatically for monorepos. You don't have to manually configure Metro when using monorepos if you use the config from [`expo/metro-config`](/guides/customizing-metro/).
...
After modifying these options, you'll need to run Expo with `expo start --clear` once to erase the outdated Metro cache. If your app continues working as expected afterwards, it's a regular Node monorepo and won't need any special configuration going forward.
...
Prior to SDK 52, to configure a monorepo with Metro manually, there were two manual changes:

1. Metro had to be configured to watch code within the monorepo manually (for example, not just **apps/cool-app**.)
2. Metro's resolution has to be adjusted to find packages in other workspaces and multiple `node_modules` folders (for example, **apps/cool-app/node_modules** or **node_modules**.)

The configuration had to be adjusted by [creating a **metro.config.js**](/guides/customizing-metro/#customizing) with the following content:
...
For [pnpm](https://pnpm.io/workspaces), you'll instead have to create a [**pnpm-workspace.yaml**](https://pnpm.io/pnpm-workspace_yaml):
...
Many package managers also support specifying workspace dependencies using `"workspace:*"` instead of `"*"`. This will ensure that the workspace package never instead resolves a published package of the same name from the npm registry.

After adding the the package as a dependency, install your dependencies with your package manager from the root directory of your monorepo to check for common warnings once again.
...
Monorepos are not for everyone. They require in-depth knowledge of the used tooling, add more complexity, and often require specific tooling configuration. You take on increased complexity and need to solve issues you most likely will run into. Here are a couple of common issues you might encounter.

### Package managers with isolated dependencies

> **info** From **SDK 54**, Expo officially supports isolated dependencies and isolated installations.<br />
...
From **SDK 54**, you can set the `EXPO_USE_STICKY_RESOLVER=1` environment variable to apply auto-linking to Expo CLI and Metro bundler automatically. This will force dependencies that Metro resolves to match the native modules that [auto-linking](/modules/autolinking/) links for your native builds.
