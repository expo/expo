...
## Production like compilation for testing

You can compile and run a production build on a device or a simulator using the commands from the following sections. This build type is not code-signed or ready to submit to the app stores. Compiling this type of build is useful to test for bugs that may show up in production.

### Android

To compile an Android for production, use the `--variant` flag with `release` as its value:

<Terminal cmd={['$ npx expo run:android --variant release']} />

You can also add the `--device` flag to create and run the build on a connected device.

### iOS

To compile an iOS app for production, use the `--configuration` flag with `Release` as its value:
...
To learn more about how compilation works with Expo CLI, see the following guides:
...
To learn how to natively code-sign your app, see Production builds:
