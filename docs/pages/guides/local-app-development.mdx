...
## Production build compilation for testing

You can [compile](/more/expo-cli/#compiling) and run a production build on a device or a simulator using the commands from the following sections. This build type is not code-signed or ready to submit to the app stores. Compiling this type of build is useful to test for bugs that may show up in production.

### Android

To compile an Android app for production, use the `--variant` flag with `release` as its value:
...
To learn how to natively code-sign and release your app, see Production builds:
