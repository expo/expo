...
Enable the flag in the project Expo config:
...
Ensure the entry module is `expo-router/entry` (default).
...
> Complex built-in layouts such as Stack, and Tabs are not supported yet. They'll fallback to a `<Slot />`.

Layout routes (`app/_layout.js`) can be used to wrap all routes in a common layout.
...
To detect which platform your code is bundled for, use the `process.env.EXPO_OS` envrionment variable. For example `process.env.EXPO_OS === 'ios'`. Prefer this to `Platform.OS` as `react-native` is not fully optimized for React Server Components yet and therefore won't work as expected.
