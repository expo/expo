...
To use a custom icon font, you have to make sure you import them into your project. Only after a font has loaded, you can create an Icon set. [Learn more about loading custom fonts](using-custom-fonts.mdx).
...
export default function App() {
  ...
  /* @info See <a href="/home/develop/user-interface/fonts/#wait-for-fonts-to-load"> Waiting for fonts to Load section</a> for more information on how to display a splash screen when fonts are loaded. */
  if (!fontsLoaded) {
    return null;
  }
  /* @end */
