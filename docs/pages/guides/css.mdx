...
> Experimentally available in **SDK 55 and later**. This is a beta release and is subject to breaking changes.
...
- Only class selectors are supported (for example, `.my-class { color: red; }`), element selectors (for example, `div { color: red; }`) and ID selectors (for example, `#my-id { color: red; }`) are not supported.
...
Enable the experimental Functional CSS feature in your [app config](/workflow/configuration/) file:
...
Now run `npx expo start` command in a terminal window to start the project. You should see a white screen with black text. On web, this will fall back to standard CSS behavior.
...
- CSS has dynamic functionality that would normally require React hooks. React hooks are limited as they don't support server rendering, and you cannot call them conditionally. This means you need to refactor components to optimize them.
- High-performance animations are built in. We use the native Reanimated animation library on native and CSS as-is on web. This results in smooth max-FPS animations on all platforms.
