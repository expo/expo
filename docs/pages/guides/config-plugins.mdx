...
> **info** This guide applies to SDK 41+ projects. The Expo Go app doesn't support custom native modules.
...
> **info** Check out all the different ways you can import `plugins`: [plugin module resolution](#plugin-module-resolution).
...
> **info** Any resolution pattern that isn't specified below is unexpected behavior, and subject to breaking changes.
...
> **info** Use [modifier previews](https://github.com/expo/vscode-expo#expo-preview-modifier) to debug the results of your plugin live.
...
> **info** SDK 47 is scheduled to be released in Q3 2022
...
> **info** Introspection only works on safe modifiers (static files like JSON, XML, plist, properties), with the exception of `ios.xcodeproj` which often requires file system changes, making it non idempotent.
...
> **info** This example uses `ts-node` for simple local TypeScript support, this isn't strictly necessary. [Learn more](/guides/typescript/#appconfigjs).
