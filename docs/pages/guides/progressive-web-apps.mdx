...
Now link the manifest in your HTML file. The method here depends on the output mode of your website (configured with `web.output` in **app.json**). In SDK 58 and later, Expo Router defaults to `server`; apps without Expo Router default to `single`.
...
Now register the service worker in the HTML file. The method here depends on the output mode of your website (configured with `web.output` in **app.json**). In SDK 58 and later, Expo Router defaults to `server`; apps without Expo Router default to `single`.
