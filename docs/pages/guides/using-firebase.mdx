...
The following sub-sections assume you are using `firebase@11.x.x` or above version of the Firebase JS SDK.
...
Firebase JS SDK library loads both ES modules (ESM) and Common JS modules (CJS) when bundled in your project. To work around this, ensure Metro is configured to support ESM or CJS modules but not both. The following configuration ensures that Metro uses ESM format for the Firebase JS SDK:
...
Alternatively, you can disable [`package.json:exports`](/versions/latest/config/metro/#packagejsonexports) support in the metro config but that might cause issues with other libraries and is not recommended.
...
You do not have to install other plugins or configurations to use Firebase JS SDK. You can directly import any service you want to use from the `firebase` package. For example, if you want to use an authentication service in your project, you can import the `auth` module from the `firebase/auth` package.
