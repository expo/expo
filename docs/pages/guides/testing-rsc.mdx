...
React Server Components run in a Node.js environment, unlike a client-based Jest testing environment. This means you can emulate the server-side rendering environment closely with Jest.

### Setup

While standard server rendering is web-only, Expo's universal RSC bundles custom server renderers for each platform. For example, when writing server components for an iOS app, the platform-specific extensions such as **\*.ios.js** and **\*.native.ts** will be resolved.
...
Behind the scenes, these methods handle a part of the framework operation needed to render RSC. The component is streamed to a string and compared all at once. You can alternatively stream it manually to observe the progress.

If a component fails to render, the matcher will throw an error to fail the test. An `E:` line will be present in your flight string in real time. This will be sent to the client, where the error will be thrown.
