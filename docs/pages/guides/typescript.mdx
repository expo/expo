...
Expo CLI supports [path aliases](https://www.typescriptlang.org/docs/handbook/module-resolution.html#path-mapping) in your project's **tsconfig.json** automatically. This allows you to import modules using a custom alias instead of a relative path.

For example, if you have a file at `src/components/Button.tsx` you can import it using the alias `@/components/Button`:

```tsx
import Button from '@/components/Button';
```

The default alias is `@/`, but you can change this in your **tsconfig.json**:
...
- Expo CLI must be restarted to update path aliases when you change the **tsconfig.json**.
...
- Aliases can be disabled by setting the environment variable `EXPO_METRO_NO_PATH_ALIASES=1`.
- Path aliases are Metro-only and not supported by Expo's webpack config.

## Absolute Imports

To enable absolute imports in your project, you can add the following to your **tsconfig.json**:
...
Now you can import modules using an "absolute" path relative to the root of your project:
...
- `compilerOptions.baseUrl` is automatically set to `.` when the `tsconfig.json` or `jsconfig.json` files exist.
- Node modules take precedence over absolute imports, so you cannot overwrite a node module import with an absolute import.
- Expo CLI must be restarted to update `compilerOptions.baseUrl` when you change the **tsconfig.json**.
- `jsconfig.json` can be used instead of `tsconfig.json` if you are not using TypeScript.
- Base URL support can be disabled by setting the environment variable `EXPO_METRO_NO_BASE_URL=1`.
- Absolute imports are Metro-only and not supported by Expo's webpack config.
