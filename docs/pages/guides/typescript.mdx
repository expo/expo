...
Then, run `npx tsc` or `yarn tsc` to type check the project.
...
To get started in an existing project, create a **tsconfig.json** in your project root:

<Terminal cmd={['$ touch tsconfig.json']} />

**For SDK 48 and higher**, running `npx expo start` prompts you to install the required dependencies, such as `typescript` and `@types/react`, and automatically configure your **tsconfig.json**. **For SDK 47 and below**, the command also prompts to install `@types/react-native` as an additional dependency.

Rename files to convert them to TypeScript. For example, rename **App.js** to **App.tsx**. Use the **.tsx** extension if the file includes React components (JSX). If the file did not include any JSX, you can use the **.ts** file extension.

<Terminal cmd={['$ mv App.js App.tsx']} />

Then, you can use `npx tsc` or `yarn tsc` to type check the project.
...
| ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`"allowJs"`]: `true`                                                                                 | Allow JavaScript files to be compiled. If your project requires more strictness, you can disable this.                                                        |
| `"esModuleInterop"`: `true`                                                                           | Improve Babel ecosystem compatibility. This also sets `allowSyntheticDefaultImports` to `true`, allowing default imports from modules with no default export. |
| [`"jsx"`](https://www.typescriptlang.org/docs/handbook/jsx.html): `"react-native"`                    | Preserve JSX, and converts the `jsx` extension to `js`. This is optimized for bundlers that transform the JSX internally (such as Metro).                     |
| `"lib"`: `["DOM", "ESNext"]`                                                                          | Allow using the latest [ECMAScript proposed features and libraries](https://github.com/tc39/proposals).                                                       |
| [`"moduleResolution"`](https://www.typescriptlang.org/docs/handbook/module-resolution.html): `"node"` | Emulate how Metro and webpack resolve modules.                                                                                                                |
| `"noEmit"`: `true`                                                                                    | Only use the TypeScript compiler (TSC) to check the code. The Metro bundler is responsible for compiling TypeScript to JavaScript.                            |
| `"resolveJsonModule"`: `true`                                                                         | Enables importing **.json** files. Metro's default behavior is to allow importing JSON files as JS objects.                                                   |
| `"skipLibCheck"`: `true`                                                                              | Skip type checking of all declaration files (**\*.d.ts**).                                                                                                    |
| `"target"`: `"ESNext"`                                                                                | Compile to the latest version of ECMAScript.                                                                                                                  |
...
The default configuration for TypeScript is user-friendly and encourage adoption. However, if if you prefer strict type checking, you can enable it by adding `"strict": true` to the `compilerOptions`. We recommend enabling this to minimize the chance of introducing runtime errors.
