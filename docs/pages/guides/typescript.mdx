...
You can automatically generate a **tsconfig.json** file by running: `npx expo customize tsconfig.json`
...
Some Expo packages provide both static types and type generation capabilities. These types will automatically be generated when the project is built or by running the `npx expo customize tsconfig.json` command.
