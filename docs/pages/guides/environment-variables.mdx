...
Environment variables in Expo CLI have two parts, both can be disabled:

1. Expo CLI will load the `.env` files into the global process. This can be disabled by setting the environment variable `EXPO_NO_DOTENV` before any Expo CLI command is run, like `EXPO_NO_DOTENV=1`.
2. Expo's Metro config will inline environment variables in the client JavaScript bundle during serialization. This can be disabled with `EXPO_NO_CLIENT_ENV_VARS`, like `EXPO_NO_CLIENT_ENV_VARS=1`.

If you're experiencing issues with environment variables, you can try disabling one or both of these features. Consider contacting support on GitHub or Discord if you're still having issues.
