...
> If you've published [previews](/review/share-previews-with-your-team/) or created a [build](/deploy/build-project/) before, you may have already set up updates.

To set up updates, run the following [EAS CLI](/develop/tools/#eas-cli) command:

<Terminal cmd={['$ eas update:configure']} />

Once the command completes, you'll need to make new builds.
...
This command will create an update and make it available to builds of your app that are configured to receive updates on the `production` channel, which is defined in [**eas.json**](/eas/json/#channel).
