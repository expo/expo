...
EAS Submit uploads your **.aab** to Google Play Console and places it in the track you choose (internal, alpha, beta, or production). For a brand-new app, Google's API only accepts the first release as a draft: either upload the first build manually through Play Console, or set [`releaseStatus`](/eas/json/#releasestatus) to `draft` in **eas.json** and submit with `eas submit`. After the first release exists, `eas submit` handles every subsequent release.
