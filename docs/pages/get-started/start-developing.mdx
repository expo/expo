...
> **info** **Using Expo Go on a physical iOS device?** Expo Go opens your project only when Expo CLI and Expo Go are signed in to the same Expo account. Run `npx expo login` on your computer, then sign in to Expo Go with the same account. See [Set up your environment](/get-started/set-up-your-environment/?platform=ios&device=physical&mode=expo-go).

<Collapsible summary="Having problems?">

Make sure you are on the same Wi-Fi network on your computer and your device.

If you see an error about accounts on a physical iOS device, the message tells you what to fix. To sign in to Expo CLI, run `npx expo login` on your computer. To sign in to Expo Go, tap the account icon in the top-right corner. Use the same account for both, then press **Try Again**. See [Expo Go sign-in required](/troubleshooting/expo-go-sign-in-required/) for every message.
...
- If you see **Fast Refresh** enabled, toggle it. If you see **Disable Fast Refresh**, dismiss the developer menu. Now try making another change.
  ...
  <ContentSpotlight
    ...
    src="/static/images/get-started/developer-menu.webp"
