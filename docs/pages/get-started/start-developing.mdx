...
If you see an error about accounts on a physical iOS device, read which side the message names. Run `npx expo login` for Expo CLI, or sign in from the account icon in Expo Go, then press **Try Again**. See [Expo Go sign-in required](/troubleshooting/expo-go-sign-in-required/) for every message.
