...
> **info** **Using Expo Go on a physical iOS device?** Expo Go opens your project only when Expo CLI and Expo Go are signed in to the same Expo account. Run `npx expo login` on your computer, then sign in to Expo Go with the same account. See [Set up your environment](/get-started/set-up-your-environment/?platform=ios&device=physical&mode=expo-go). This requirement does not apply to the iOS Simulator or to Android.

<Collapsible summary="Having problems?">

Make sure you are on the same Wi-Fi network on your computer and your device.

If you are using Expo Go on a physical iOS device and see an error about accounts, sign in to Expo CLI and Expo Go with the same Expo account. Run `npx expo login` on your computer, sign in to Expo Go from the account icon in the top-right corner, then press **Try Again**.
