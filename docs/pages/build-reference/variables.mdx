...
If you're using a dotenv file for storing your secrets locally, you can use the `eas secret:sync` command to import all of them to EAS:

<Terminal
  cmd={[
    '$ eas secret:sync --scope project --env-file ./eas/.env',
...
You can use the `eas secret:sync` command to integrate EAS with your [Doppler](https://doppler.com/) project:

<Terminal
  cmd={[
    '$ doppler run --mount ./eas/.env -- eas secret:sync --scope project --env-file ./eas/.env',
  ]}
/>
