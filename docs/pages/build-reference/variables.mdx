...
Add any applicable **.env** files to your **.easignore** file so they are not uploaded with your EAS build job:
```bash .easignore
...
> `EXPO_PUBLIC_` variable resolution is available in SDK 49 and higher. See notes for [SDK 48 and lower](/guides/environment-variables#environment-variables-in-sdk-48-and-lower).
...
> All environment variables in your **eas.json** profile are available when evaluating **app.config.js**. It's a good practice to only use the `EXPO_PUBLIC_` prefix for variables used within your application code.
...
Environment variables defined in a **.env** file are only considered by the Expo CLI. Therefore, if you upload a **.env** file to EAS Build, it will be used to inline `EXPO_PUBLIC_` variables into your application code, but will not be using for evaluating your **app.config.js** or for any other build steps.

### How do I share environment variables between my local development environment, EAS Update, and EAS Build?
Environment variables defined in **eas.json** are only available when running an EAS Build job. However, you may wish to change variables used within your application code based on the build profile without duplicating values you might keep in an **.env** file for local development or for when publishing to EAS Update.
...
3. **.env** files committed to source control/ not in **.easignore**
