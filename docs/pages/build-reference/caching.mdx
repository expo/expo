...
By default, Node packages will be installed with your preferred package manager's immutable lockfile flag/command (e.g. `yarn --frozen-lockfile` or `npm ci`). If you would like to disable this, you can set the `EAS_NO_FROZEN_LOCKFILE` environment variable to `"1"` in **eas.json**.
