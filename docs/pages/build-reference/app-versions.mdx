...
> **warning** The `remote` version source is the default behavior from EAS CLI version `11.0.0`. If you are using an older version of the EAS CLI, `local` is the default.

EAS servers can store and manage your app's developer-facing build version (`android.versionCode` and `ios.buildNumber`) remotely by default. To enable it, you need to set `cli.appVersionSource` to `remote` in **eas.json**. Then, under the `production` build profile, you can set the `autoIncrement` property to `true`.
