...
> **warning** The `remote` version source is the default behavior from EAS CLI version `12.0.0`. If you are using an older version of the EAS CLI, `local` is the default.
...
> **warning** The `remote` version source as a default behavior has been introduced in `eas-cli` version `12.0.0`. If you are using an older version of the CLI, you **don't need to specify the version source explicitly to `local`**.
