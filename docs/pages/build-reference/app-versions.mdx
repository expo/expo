...
EAS Build can manage automatically incrementing these versions for you if you use the `remote` app version source, which is the default behaviour. Optionally, you can choose to use a `local` app version source, which means you control versions manually in their respective config files.

> **warning** The `remote` version source as a default behavior has been introduced in `eas-cli` version `10.0.0`. If you are using an older version of CLI, be aware that **`local` will be the default behavior**.
...
{
  "cli": {
    "appVersionSource": "remote" // you don't need to specify this explicitly, as "remote" is the default
  },
  "build": {
    "staging": {
...
By default, you can configure your project to rely on EAS servers to store and manage the version of your app. Add `{ "cli": { "appVersionSource": "remote" } }` in your **eas.json**, or don't specify it at all to use `remote` as a default option. The remote version will be initialized with the value from the local project. If you would like to explicitly set the value directly, or EAS CLI is not able to detect what version the app is on, you can use the `eas build:version:set` command. EAS stores version information scoped by account, slug, platform, and application ID/bundle identifier. For example, if you are building variants with different application IDs or bundle identifiers, versioning will be independent for each of them.

> **warning** The `remote` version source as a default behavior has been introduced in `eas-cli` version `10.0.0`. If you are using an older version of the CLI, be aware that **`local` will be the default behavior**.
...
You can also configure your project so that the source of truth for project versions is the local project source code itself. Add `{ "cli": { "appVersionSource": "local" } }` in your **eas.json**. In this case, EAS does not write to the project, it reads the values and builds projects as they are.

> **warning** The `remote` version source as a default behavior has been introduced in `eas-cli` version `10.0.0`. If you are using an older version of the CLI, you **don't need to specify the version source explicitly to `local`**.
