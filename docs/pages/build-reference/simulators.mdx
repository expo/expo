...
To build your app for installation on an iOS Simulator, you can create a new profile in **eas.json** and set the `ios.simulator` value to `true`:
...
To run your build run `eas build -p ios --profile preview`. Remember that you can name the profile whatever you like. In the example, it is named `"preview"`, however, you can call it `"local"`,`"simulator"`, or whatever makes the most sense for you.
