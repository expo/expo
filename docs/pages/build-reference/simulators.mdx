...
To install a build of your app on an iOS Simulator, you need to create a new or modify an existing build profile in **eas.json**. In this profile, set the `ios.simulator` property's value to `true`. This will tell EAS Build to build your app for the iOS Simulator.

For example, in the following **eas.json**, the `preview` profile is configured to build for the iOS Simulator:
...
Once your build is completed, run the `eas build:run` command to download the build and automatically extract it on the iOS Simulator:
...
<ImageSpotlight
  ...
  src="/static/images/eas-build/eas-build-run-on-ios.jpg"
  style={{ maxWidth: 720 }}
/>
