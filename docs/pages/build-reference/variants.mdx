...
When creating [development, preview, and production builds](/build/eas-json/#common-use-cases), installing each build variant on the same device is common. This allows working in development, previewing the next version of the app, and running the production version on the same device without needing to uninstall and reinstall the app.

This guide provides steps required to configure multiple (development and production) variants to install and use them on the same device.
...
Let's say you want to create a development build and a production build of a managed Expo project. The configuration in the **app.json** may look as shown below:
...
If your project has EAS Build configured, your project's **eas.json** has a similar configuration:
