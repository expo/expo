...
When creating [development, preview, and production builds](/build/eas-json/#common-use-cases), it's common to install each variant of the build on the same device. This allows doing development work, previewing the next version of the app, and running the production version all on the same device, without needing to uninstall and reinstall the app.

This guide explains the workflow you need to follow to install and use multiple build variants (development and production) on the same device.
...
**If you have a bare project**, this can be accomplished by using flavors (Android) and targets (iOS). To configure which flavor is used, use the `gradleCommand` field on your build profile. To configure which target is used, use the `scheme` field for iOS.

### In managed project

Let's say you want a development build and production build of a managed Expo project. The **eas.json** in your project may look like the following configuration:
...
The configuration in the **app.json** may look as shown below:
...
In **app.config.js**, switch the `android.package` and `ios.bundleIdentifier` based on an environment variable called `IS_DEV`:
...
In the above example, the environment variable `IS_DEV` is used to differentiate between the development and production environment. Based on its value, the different Application IDs or Bundle Identifiers are set for each instance.
