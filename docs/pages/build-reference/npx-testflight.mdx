...
description: A single command that allows you to build, sign, and submit your iOS app to TestFlight from any device.
---
...
[`npx testflight`](https://www.npmjs.com/package/testflight) is a single command that walks you through building, signing, and submitting your iOS app to TestFlight with no other dependencies required.

## Prerequisites

- A paid [Apple Developer Account](https://developer.apple.com/account/) &mdash; must-have from Apple to enable TestFlight builds
...
The command workflow is interactive and walks you through the following prompts:

- **Run EAS commands automatically.** Uses the latest EAS CLI version and runs it without running it globally.
...
- **Sign in to Apple Developer.** Provide your Apple ID, complete two-factor authentication, and allow the CLI to reuse existing distribution certificates or provisioning profiles when available.
...
- **Verify App Store Connect access.** The submit step checks for an [App Store Connect API key](https://expo.fyi/creating-asc-api-key). You can reuse an existing key or generate a new one.
...
Yes, when you provide `ascAppId` in the `submit` profile in the **eas.json**, the `npx testflight` command will bypass the process of ensuring your app exists on App Store Connect.
