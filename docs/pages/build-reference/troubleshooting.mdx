...
You can verify that your project builds on your local machine with the `npx expo run:ios` and `npx expo run:android` commands, with variant/configuration flags set to release to most faithfully reproduce what executes on EAS Build. For more information, see [Android build process](/build-reference/android-builds) and [iOS build process](/build-reference/ios-builds).
