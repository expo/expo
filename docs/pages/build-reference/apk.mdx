...
By default, EAS Build produces Android App Bundle, you can change it by modifying the [eas.json](/build/eas-json.mdx) file by either:

- setting `buildType` to `apk` or
- setting `developmentClient` to `true` or
...
Now, to run your build run `eas build -p android --profile preview1`. Remember that you can name the profile whatever you like; we named the profile `preview1`, but you could call it `local`, `simulator` or whatever makes most sense for you.
