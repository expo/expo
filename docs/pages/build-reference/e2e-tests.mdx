...
To verify that these flows work as expected, install your app on a local simulator or emulator. Navigate to the Maestro folder in your terminal, then run the following commands to start the tests using Maestro CLI:
...
To install the Maestro CLI, follow the instructions in [Installing Maestro](https://maestro.mobile.dev/getting-started/installing-maestro).
