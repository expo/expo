...
> **info** This guide will evolve over time as support for testing workflows in EAS Build improves.
...
In the following steps, the example demonstrates how to configure your EAS Build Maestro E2E tests workflow using the [default Expo template](/more/create-expo/#--template). For your own app, you will need to adjust the flows to match your app's UI.
...
Inside it, create a new flow called **home.yaml**. This flow will launch the app and assert that the text "Welcome!" is visible on the home screen.
...
When the flow fails, any Maestro artifacts are automatically uploaded as build artifacts. This includes screenshots saved at `/Users/expo/.maestro/tests` (the default destination). You can download them from the build page.
