...
Communicating directly with FCM and APNs is much more complicated than sending notifications through [Expo's push notification service]/push-notifications/sending-notifications/). You should only use this feature if you're prepared to undertake that complexity. Here are a few things you'll have to handle yourself if you choose to write your server for FCM and APNs:

- Differentiating between native Android & iOS device tokens on your backend
...
Sometimes you need finer-grained control over your notifications, so communicating directly with FCM and APNs is necessary. The Expo platform does not lock you into using Expo's application services, and the `expo-notifications` API is push-service agnostic.

## How do I write my own FCM & APNs servers
...
Communicating with FCM is as simple as sending a POST request. However, before sending or receiving any notifications, you'll need to follow the steps to [configure FCM](/push-notifications/push-notifications-setup/#android) to configure FCM and get your `FCM-SERVER-KEY`.
...
> This documentation is based on [Apple's documentation](https://developer.apple.com/documentation/usernotifications#/apple_ref/doc/uid/TP40008194-CH8-SW1), and this section covers the basics to get you started.
...
APNs provide their full list of supported fields in the [notification payload](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/PayloadKeyReference.html#/apple_ref/doc/uid/TP40008194-CH17-SW1).
