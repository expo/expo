...
**Note on `sound` and `badge`**: On iOS, both fields need the matching permission. The system plays the sound only if the user allows sounds. The system sets the badge only if the user allows badges. `getPermissionsAsync()` reports these two settings as `ios.allowsSound` and `ios.allowsBadge`. The top-level `status` stays `granted` when either one is off. Check the two `ios` fields when a notification arrives without a sound or without a badge. The user answers these when your app asks for permission the first time, and changes them later in the Settings app.
