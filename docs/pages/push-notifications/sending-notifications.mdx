...
The Expo push notification service does not have an SLA and the FCM and APNs services also may have occasional outages. If you follow the guidance above, your app can handle temporary service interruptions.
...
| `contentAvailable`  | iOS Only        | `boolean`                                                 | When this is set to true, the notification will cause the iOS app to start in the background to run a [background task](/versions/latest/sdk/notifications/#headless-background-notifications). Your app needs to be [configured](/versions/latest/sdk/notifications/#background-notification-configuration) to support this. Maps to [`aps.content-available`](https://developer.apple.com/documentation/usernotifications/pushing-background-updates-to-your-app#Create-a-background-notification).                                                                                                                                                                     |
...
| `ttl`               | Android and iOS | `number`                                                  | Time to Live: the number of seconds for which the message may be kept around for redelivery if it hasn't been delivered yet. Omit this field to use each provider's own default - 4 weeks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `targetContentId`   | iOS Only        | `string`                                                  | The identifier of the window brought forward. The value of this key will be populated on the `UNNotificationContent` object created from the push payload. Maps to [`aps.target-content-id`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference). The `expo-notifications` package exposes the received value as `targetContentIdentifier`.                                                                                                                                                                                                                                                               |
| `relevanceScore`    | iOS Only        | `number`                                                  | A score between 0 and 1 that the system uses to pick which notification to feature in a summary. Higher values indicate higher relevance. Maps to [`aps.relevance-score`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference).                                                                                                                                                                                                                                                                                                                                                                            |
| `filterCriteria`    | iOS Only        | `string`                                                  | The criteria the system evaluates to determine if it displays the notification in the current Focus. Maps to [`aps.filter-criteria`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference).                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `threadId`          | iOS Only        | `string`                                                  | An identifier by which the system visually groups notifications together. Notifications that share a `threadId` are stacked into a single group. Unlike `collapseId`, no notification is replaced or removed. Maps to [`aps.thread-id`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference).                                                                                                                                                                                                                                                                                                              |
...
**Note on `contentAvailable`**: This field replaces the deprecated `_contentAvailable` field, which is still accepted for backwards compatibility. If you specify both, `contentAvailable` takes precedence.
