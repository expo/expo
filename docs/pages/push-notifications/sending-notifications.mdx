...
sidebar_title: Send notifications with the Expo Push Service
description: Learn how to call Expo's Push Service API to send push notifications from your server.
---

import { ContentSpotlight } from '~/ui/components/ContentSpotlight';

The [`expo-notifications`](/versions/latest/sdk/notifications) library provides all the client-side functionality for push notifications. Expo also handles sending push notifications off to FCM and APNs which then send them to particular devices. All you need to do is send a request to Expo's Push API with the `ExpoPushToken` you obtain with [`getExpoPushTokenAsync`](/versions/latest/sdk/notifications/#getexpopushtokenasyncoptions).
...
Each of the example servers above is a wrapper around Expo's Push Service API.
...
| `sound`             | iOS Only        | `'default' \| null`                                       | Play a sound when the recipient receives this notification. Specify `default` to play the device's default notification sound, or omit this field to play no sound. Custom sounds are not supported.                                                                                                                                                                    |
