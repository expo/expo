...
- `InvalidCredentials`: Your push notification credentials for your standalone app are invalid (ex: you may have revoked them).
  - **Android**: Be sure that you have correctly applied the server key from the Firebase Console as specified in [the instructions for uploading FCM server credentials](using-fcm/#uploading-server-credentials).
