...
**Note on `contentAvailable`**: This field replaces the deprecated `_contentAvailable` field, which is still accepted for backwards compatibility. If you specify both, `contentAvailable` takes precedence. Unlike `_contentAvailable`, `contentAvailable` accepts a boolean only.
