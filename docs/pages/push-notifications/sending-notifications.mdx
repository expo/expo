...
> **info** If you're just getting started and want to focus on the front-end for now, you can skip this step and just use [Expo's push notification tool](https://expo.dev/notifications) to send notifications with the click of a button.
...
> **info** If you'd rather build a server that communicates with APNs and FCM directly, check out [this guide](sending-notifications-custom.mdx) (this is more complicated than using Expo's push notification service).
...
> **info** If you're **not** testing in the Expo Go app, make sure you've [generated the proper push credentials](push-notifications-setup.mdx#credentials) before proceeding! If you haven't, push notifications will not work.
...
> **info** A status of `ok` along with a receipt ID means that the message was received by Expo's servers, **not** that it was received by the user (for that you will need to check the [push receipt](#push-receipts)).
...
> **info** For a better understanding of iOS credentials, including push notification credentials, read our [App Signing docs](/app-signing/app-credentials.mdx#ios)
