...
- `InvalidCredentials`: Your push notification credentials for your standalone app are invalid (ex: you may have revoked them).
  - **Android**: Make sure that you have correctly uploaded the server key from the Firebase Console as specified in the [uploading FCM server credentials](using-fcm/#uploading-server-credentials).
