...
/* @info Sends a notification to Expo. Expo's push notification tool at https://expo.dev/notifications may also be used. */
...
async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    try {
    /* @info This provides the ExpoPushToken using the unique project ID. */
...
export default function App() {
  ...
  useEffect(() => {
    /* @info Gets the push token and shows it in the UI, or shows the error if one occurs */
