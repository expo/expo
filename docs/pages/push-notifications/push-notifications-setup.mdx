...
Run the following command to install `expo-notifications`, `expo-device` and `expo-constants` libraries:
...
It is automatically set when you create a development build, however, **we recommend setting it manually in your project's code**. To do so, you can use [`expo-constants`](/versions/latest/sdk/constants/) to get the `projectId` value from **app.json**:
