...
You'll need to use [`expo-notifications`](/versions/latest/sdk/notifications) library to request for user's permission and fetch the `ExpoPushToken`. You'll also use [`expo-device`](/versions/latest/sdk/device) to check whether the app is running on a physical device. The `expo-notifications` library is not supported on an Android Emulator or an iOS simulator.
...
For Android and iOS, there are different requirements to setup your credentials.

### Android

For Android, you need to configure **Firebase Cloud Messaging (FCM)** to get your own credentials and setup your Expo project. It is required for all Android apps using Expo SDK (unless you are running your project in Expo Go).
