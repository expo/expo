...
4. In the **app.json**, add an `android.googleServicesFile` field with the relative path to the downloaded **google-services.json** file. If place it in the root directory, the path is:
...
6. Firebase also creates an API key in the Google Cloud Platform Credentials console with a name like **Android key (auto created by Firebase)**. This is could be a different key than found in **google-services.json**.
...
> If you are using bare workflow and not EAS Build, you will need to run `eas credentials` manually.
