...
To get the client-side ready for push notifications, the main 2 things we need are:

- The user's permission to send them push notifications
- The user's ExpoPushToken. If push notifications were mail, then the ExpoPushToken is the user's address.

Getting the permissions is easy- just use the [`expo-permissions` module](../../sdk/permissions)! As for the push token, the `expo-notifications` module provides a method exactly for that: [`getExpoPushTokenAsync`](../../sdk/notifications/#getexpopushtokenasyncoptions-expotokenoptions-expopushtoken).
...
registerForPushNotificationsAsync = async () => {
  /* @info We should also make sure the app is running on a physical device, since push notification won't work on a simulator. */
