...
Used to define the path to a JS/TS module implementing the function. Each function is **required** to define either a `command` or a `path` to JS/TS module implementing the function. For example, the path `./greetings` is used to execute a `greetings` function declared in `greetings` module:
...
Learn more about custom TS/JS functions TODO.
...
{/* TODO: examples with inline credentials */}
