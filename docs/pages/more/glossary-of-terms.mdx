...
The process of generating the temporary native `/ios` and `/android` folders for a React Native project based on the [app Config](#app-config). This process is performed by running the command `npx expo prebuild` from [Expo CLI](#expo-cli) in a project directory.
