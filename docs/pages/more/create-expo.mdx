...
Skips generating **AGENTS.md** and **.claude/settings.json**. By default, `create-expo-app` generates **AGENTS.md** so AI coding agents have Expo-specific context. When Claude Code is installed, it also generates **.claude/settings.json** to configure the [`expo` skills plugin](https://expo.dev/expo-skills) automatically. The generated **AGENTS.md** points to the versioned Expo docs matching your project's SDK version.
