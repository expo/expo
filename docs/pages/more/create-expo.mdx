...
When `create-expo-app` runs inside a coding agent such as Claude Code, Codex, or Cursor, it also prints the command that installs the Expo plugin or Expo Skills for that agent after the project is created, unless the plugin is already installed. This option skips that message too.
