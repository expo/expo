...
When run from inside a coding agent such as Claude Code, Codex, or Cursor, the command also prints how to install the Expo plugin or Expo Skills for that agent after the project is created, unless the plugin is already installed. This option skips that message too.
