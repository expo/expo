...
Yarn Modern on EAS requires enabling [Corepack](https://nodejs.org/api/corepack.html) for the build. Set [`corepack`](/eas/json/#corepack) to `true` in your build profile in **eas.json**:
...
With these in place, Corepack automatically downloads and uses the pinned Yarn version when EAS installs dependencies.
