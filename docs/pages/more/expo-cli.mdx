...
There may be circumstances where you need to use a version of a package that is outside the range of safe versions recognized by `npx expo install`. In SDK 49 and higher projects, you can exclude specific packages from the version checks, while still allowing the `install` command to install, check, and fix any other dependencies. This configuration extends to the checking done by `npx expo-doctor`, suppressing warnings related to these specific packages during the Expo Doctor step in EAS Build.
