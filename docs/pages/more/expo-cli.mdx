...
This uses a generic Xcode destination (`generic/platform=iOS Simulator`) instead of a specific simulator UDID, which is useful for:
...
You can combine this with `--configuration Release` to create a production simulator build:

<Terminal cmd={['$ npx expo run:ios --configuration Release --device generic']} />
