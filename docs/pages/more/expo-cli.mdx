...
You can configure the prefix for static assets by setting the `assetPrefix` field in your `app.json`:
...
This will export the website with all static assets prefixed with `/my-root`. For example, an image at `assets/image.png` will be expected to be hosted at `/my-root/assets/image.png`.
