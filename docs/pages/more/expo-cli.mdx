...
| `EXPO_USE_DEPRECATED_POLYFILL_SORTING` | **boolean** | <div className="flex items-center pb-1.5"><StatusTag status="SDK 52" /></div>Enable the deprecated `serializer.getModulesRunBeforeMainModule` system which injects polyfill modules before running the main module. This has been removed in favor of simply importing the required polyfills in the `expo` module.                                                                                                             |
