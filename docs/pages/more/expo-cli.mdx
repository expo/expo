...
| `EXPO_USE_STICKY_RESOLVER`           | **boolean** | <div className="flex items-center pb-1.5"><StatusTag status="experimental" /><StatusTag status="SDK 54+" /></div>Enable [auto-linking](/modules/autolinking/) to also be used for dependency resolution in Metro to prevent duplicate or mismatchig native modules between native builds and JavaScript bundles.                                                                                                                |
