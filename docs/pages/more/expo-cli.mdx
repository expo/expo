...
| `EXPO_DANGEROUSLY_ALLOW_REMOTE_DEBUGGING` | **boolean** | Allow the debugger middleware to accept non-loopback connections (for example, over a tunnel). When enabled, remote connections will be accepted with a warning logged for each non-local connection. **Use with caution**: this exposes the debugger to your network. |
