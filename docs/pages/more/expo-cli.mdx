...
- `--port`: Port to start the dev server on (does not apply to webpack or [tunnel URLs](#tunneling)). Use `--port 0` to automatically use the first available port. Default: **8081**. For SDK 48 and lower, defaults to `19000`.
...
| `EXPO_NO_INSPECTOR_PROXY`           | **boolean** | <StatusTag status="deprecated" /><br/>Disable the customized inspector proxy with improved support for the Chrome DevTools protocol.<br/>This includes support for the network inspector.                                                                                                                                                                                            |
...
| `EXPO_NO_TELEMETRY_DETACH`          | **boolean** | <StatusTag status="SDK 51+" /><br/>Send telemetry events in the main thread of `@expo/cli`. This causes the CLI to slow down as it waits for all the events to be sent.                                                                                                                                                                                                              |
| `EXPO_UNSTABLE_ATLAS`               | **boolean** | <StatusTag status="experimental" /><StatusTag status="SDK 51+" /><br/>Gather Metro bundle information during development or export.                                                                                                                                                                                                                                                  |
| `EXPO_NO_BUNDLE_SPLITTING`          | **boolean** | <StatusTag status="experimental" /><StatusTag status="SDK 51+" /><br/>Disable Metro splitting chunks on async imports in production (web-only).                                                                                                                                                                                                                                      |
