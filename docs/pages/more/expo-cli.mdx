...
Expo Go on a physical iOS device also checks this account. It opens a project from your development server only when Expo CLI and Expo Go are signed in to the same account. See [Expo Go sign-in required](/troubleshooting/expo-go-sign-in-required/) when the accounts do not match.
