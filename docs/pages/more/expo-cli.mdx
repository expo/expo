...
The dev server exposes `/_expo/open` so external tools — cloud agents, distributed preview services like [Appetize](https://appetize.io/), CI scripts — can introspect the deep links the CLI would use, and optionally trigger the same action as pressing <kbd>I</kbd> / <kbd>A</kbd> / <kbd>W</kbd> in the **Terminal UI**. It supplements the legacy `/_expo/link` endpoint, which returns a `307` redirect to a deep link scheme that non-mobile clients can't follow.
...
- `runtime`: choose how the URL is resolved.
  - `default` (omitted): mirrors what pressing <kbd>I</kbd> / <kbd>A</kbd> does — picks the dev client when the server was started with `--dev-client`, falls back to a disambiguation page when the project has both Expo Go and a development build, otherwise opens Expo Go.
...
```json
{
  ...
  "availableRuntimes": ["expo", "custom"],
  "platforms": {
    "ios":     { "url": "http://192.168.1.71:8081/_expo/loading?platform=ios", "appId": "com.example.app" },
    "android": { "url": "http://192.168.1.71:8081/_expo/loading?platform=android", "appId": "com.example.app" },
