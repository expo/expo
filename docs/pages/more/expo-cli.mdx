...
You can configure the prefix for static assets by setting the `experiments.basePath` field in your [app config](/workflow/configuration/):
...
Expo Router has built-in support for `basePath`. When using the `Link` and `router` APIs, the `basePath` will be automatically prepended to the URL.
...
If you use `<a>`, React Navigation, or the `Linking` API directly, you'll need to manually prepend the `basePath`.

The `basePath` functionality is production-only and must be set before exporting the website. If you change the value, you must re-export the website.

Images and other assets will work automatically if you `require` or `import` them. If you directly reference a resource URL then you will need to append the **basePath** manually.
...
| `EXPO_NO_INLINE_DEV_SOURCE_MAPS`    | **boolean** | <StatusTag note="SDK 50+" /><br/>Prevent using inline source maps in development. We recommend using either no source maps or inline source maps for the best performance and reliability in development.                                                                                                                                                                            |
