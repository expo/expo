...
| `EXPO_NO_DEV_MENU`                   | **boolean** | Appends the reserved `__expo_*` query parameters that keep the developer menu closed, hide the tools button and skip onboarding to every Expo Go and development build launch URL for that session. The parameters update the saved dev menu settings on the device.                                                                                                                                                            |
