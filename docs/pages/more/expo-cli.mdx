...
<Terminal cmd={['$ npx expo needs-rebuild']} />

`npx expo needs-rebuild` checks whether the app installed on your simulator or device still matches your project. It compares the [fingerprint](/versions/latest/sdk/fingerprint/) embedded in the app at build time against the current project fingerprint, and reports the result through its exit code, so scripts and agents can act on it:
...
| `3`       | Cannot determine — no device, app not installed, no embedded fingerprint, or the check failed. |

By default, the command checks every platform with a reachable device. Use `--platform <all|android|ios>` to check specific platforms, `--device <name|udid|serial>` to check one specific simulator or device, `--app-id <id>` to look for a specific application ID (for example, with build flavors; requires a single `--platform`), and `--json` for machine-readable output that includes the recommended commands, the reason for the verdict, and the sources that changed.

The check requires a baseline: the app must be a debug build created with a version of `expo-constants` that embeds the fingerprint (an **app.fingerprint** file next to the embedded app config). Release builds, older builds, and builds created with `EXPO_SKIP_FINGERPRINT_EMBED=1` do not carry the fingerprint and report exit code `3`. Reading the fingerprint from physical iOS devices is not supported; iOS checks use the booted simulator.

The app config is evaluated in development mode, matching local development builds. To check a build whose config depends on production environment variables, run `EXPO_CONFIG_MODE=production npx expo needs-rebuild`.

`npx expo prebuild` additionally records what the native directories were generated from. When the app config or a config plugin changes afterwards, `needs-rebuild` exits with code `2` — a plain rebuild would compile the stale directories, so regenerate them first.
