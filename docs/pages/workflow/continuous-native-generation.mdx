...
As native your project grows, the complexity from third-party dependencies increases, complicating upgrades and slowing down developer momentum. This discourages adding advanced native functionality and leads to less powerful apps. In cross-platform apps, this complexity is multiplied across each platform.

To address this, we introduced **Continuous Native Generation** as a concept, which generates short-lived native projects from inputs like configuration and code, resulting in a compatible native project that can be compiled into a native app.

This concept describes the process of [Expo Prebuild](#usage), which is a command to create native projects for React Native apps, provided by [Expo CLI](/more/expo-cli/). However, CNG is not limited to React Native or cross-platform apps.

## CNG in React Native apps

React Native apps can use **CNG** by using [Prebuild](#usage) to automate upgrades, install or uninstall libraries, white-labeling at scale, share configuration across multiple apps, reduce [orphaned code](#how-does-prebuild-help-with-orphaned-code), and more.
...
This boils down to a workflow where a developer can express any native application with the app config and generate that project continuously &mdash; by running `npx expo prebuild`.
...
If native directories are absent, `npx expo prebuild` will run once for the specific platform. On subsequent uses of these `run` commands, manually run `npx expo prebuild` to ensure the native code is freshly synchronized with your local configuration.

## Platform support

Prebuild currently supports Android and iOS. Web support is not required because there is no native project to generate for the web and the web app it runs in a web browser.Use the `--platform` option to build for individual platforms:
...
When using the `--clean` option, you'll be warned to have a clean git status due its destructive nature. This prompt is optional and will be skipped when encountered in CI. You can disable this check by enabling the environment variable `EXPO_NO_GIT_STATUS=1`.
...
Prebuild is optional and works seamlessly with all Expo tools and services. For existing React Native projects do not use `npx expo prebuild` &mdash; developers can make ([direct changes to their native projects](/more/glossary-of-terms/#bare-workflow)), rather than continuously generating them on demand as with prebuild command.
...
Absolutely! CNG is an abstract concept that can be applied to any operating system. Take Expo for example, although Expo Prebuild officially implements CNG for Android and iOS, it also provides abstract platform support for developers to create implementations for additional platforms.
...
<Collapsible summary="How Prebuild helps with sensible project upgrades?">
...
When bootstrapping a native app, there's initial code and configuration that you may not understand but you own it. Eventually, you'll need to understand this code to upgrade your app safely. This challenge often leads developers to upgrade incorrectly or start a new app, copying existing source code.
