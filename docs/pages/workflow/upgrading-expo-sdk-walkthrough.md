...
Expo maintains ~6 months of backward compatibility. Once an SDK version has been deprecated, you will no longer be able to use the Expo Go app for development or build new binaries via the classic build service (`expo build`). However, you will still be able to publish updates via `eas update`, and build using `eas build`. Deprecations **will not** affect standalone apps you have in production.
