...
The purpose of the prompt is to encourage users to add the **android** and **ios** directories to the project's **.gitignore**, ensuring that the folders are always generated by prebuild. However, this can make custom config plugins harder to develop so we haven't introduced any mechanism to enforce this behavior.
...
Prebuilding is completely optional and it works great with all tools and services offered by Expo. There are existing React Native projects that do not use `npx expo prebuild` — where developers make ([direct changes to their native projects](/more/glossary-of-terms/#bare-workflow)), rather than continuously generating them on demand as with prebuild.

Everything offered by Expo including [EAS][eas], Expo CLI, and the libraries in the Expo SDK are built to **fully support** bare React Native projects as this is a minimum requirement for supporting projects using `npx expo prebuild`. The only exception is the [Expo Go][expo-go] app, which can load arbitrary React Native projects, but only if they are structured to provide JavaScript fallbacks for native code that does not exist in the Expo Go runtime.
