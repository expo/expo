...
- `EXPO_METRO_NO_PATH_ALIASES` (**boolean**) prevent Metro from using the `compilerOptions.paths` feature from `tsconfig.json` (or `jsconfig.json`) to enable import aliases.
- `EXPO_METRO_NO_BASE_URL` (**boolean**) prevent Metro from using the `compilerOptions.baseUrl` feature from `tsconfig.json` (or `jsconfig.json`) to enable absolute imports.
