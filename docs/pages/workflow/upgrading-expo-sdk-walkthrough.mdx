...
Expo maintains ~6 months of backward compatibility. Once an SDK version has been deprecated, you will no longer be able to use the Expo Go app. However, you will still be able to publish updates via `eas update`, and build using `eas build`. Deprecations **will not** affect the standalone apps you have in production or your [development builds](/develop/development-builds/introduction/).
...
<Terminal cmd={['$ npm install expo@latest']} />

</Tab>

<Tab label="yarn">

<Terminal cmd={['$ yarn install expo@latest']} />
...
## Upgrading native projects
If you manage your own native projects (bare workflow), you will need to manually apply any changes to your native Android and iOS project folders. Our [bare upgrade advisor](/bare/upgrade) will show you a diff that describes the changes you need to make. Alternatively, you could consider [adopting prebuild](/guides/adopting-prebuild) for easier upgrades in the future.
