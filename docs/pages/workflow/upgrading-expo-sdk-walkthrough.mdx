  ...
  <Tab label="npm">
    <Terminal
      cmd={[
        ...
        '',
        '# or, install a specific SDK version (for example, SDK 53)',
        '$ npm install --save expo@^53.0.0',
      ]}
    />
  </Tab>
  <Tab label="Yarn">
...
Upgrade all dependencies to match the installed SDK version. It's also advisable to run [`expo-doctor`](/develop/tools/#expo-doctor) to check for common problems after upgrading.
