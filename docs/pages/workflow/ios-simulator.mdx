...
Sometimes the iOS Simulator doesn't respond to the open command. If it seems stuck on this prompt, start a simulator yourself, then run your app again.

<Terminal
  cmd={[
    '# List the simulators installed on your Mac',
    '$ xcrun simctl list devices available',
    '',
    '# Start one of them by name',
    '$ xcrun simctl boot "iPhone 17"',
  ]}
/>

Xcode 27 replaces the Simulator app with Device Hub. Run `open -a DeviceHub` to open it. Each running device appears in the sidebar. On Xcode 26, run `open -a Simulator` instead.

<ContentSpotlight
  alt="Device Hub on macOS lists a running iPhone 17 simulator in the sidebar."
  src="/static/images/ios-simulator/device-hub.webp"
/>

You can run more than one simulator at the same time. Expo CLI always targets the most recently opened simulator.
...
- If that doesn't help, select the simulator in Device Hub and choose **Device** &gt; **Reset Content and Settings...**<br/>
  The Simulator app on Xcode 26 calls this menu item **Erase All Content and Settings...**<br/>
