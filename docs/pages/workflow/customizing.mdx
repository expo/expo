...
Another option is to use [React Native's Core Native Modules API](https://reactnative.dev/docs/native-modules-intro) that may require some C++ knowledge in addition to Objective-C and Java. Most React Native modules in the ecosystem are built using this API because it is and always has been part of React Native, while the Expo Module API is new and intended to solve many of the pain points of using the core API.
