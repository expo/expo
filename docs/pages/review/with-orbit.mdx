...
When you launch an update, Orbit will look for the latest development build that matches the runtime version and target platform of the update. If a compatible build is found, it will be automatically installed on the target device and launched with a deep link that points to the update.
...
> **Note**: Launching updates using Expo Orbit is not supported on physical iOS devices. It is supported on iOS simulators and Android devices/emulators.
