...
In this chapter, we'll learn Expo Router's fundamentals to create stack and bottom tabs navigators with two routes.
...
- **app directory**: It is a special directory. Any files added to this directory become a screen inside our native app and a page on the web.
...
- **File name conventions**: File name, such as **index.tsx**, matches the parent directory and do not add a path segment. For example, the **index.tsx** file in the **app** directory matches `/` route.
...
Let's create a new file **about.tsx** inside the **app** directory. It displays the screen name when the user navigates to `/about` route.
...
We'll use Expo Router's `Link` component to navigate from `/index` route to `/about` route. It is a React component that renders a `<Text>` with a given `href` prop.
...
1. Create a new file **+not-found.tsx** inside the app directory to add `NotFoundScreen` component.
2. Add `options` prop from `Stack.Screen` to display a custom screen title for this route.
...
Update `Stack` in **app/\_layout.tsx** file to add `+not-found` route:
...
To test this, navigate to `http:localhost:8081/123` URL in the web browser since it is easy to change the URL path there. The app should display `NotFoundScreen` component:
...
1. Inside **app** directory, add a **(tabs)** subdirectory. This special directory is used to group routes together and display them in a bottom tab bar.
...
3. Move the existing **index.tsx** and **about.tsx** files inside **(tabs)** directory. The structure of **app** directory will look like this now:
...
Inside **(tabs)/\_layout.tsx**, add `Tabs` component to define the bottom tab layout:
