...
<Prerequisites>
  <Requirement title="Expo Go on a physical device">
    Install [Expo Go](https://expo.dev/go) on a physical Android or iOS device. On a physical iOS
    device, you also need a free [Expo account](https://expo.dev/signup) and must sign in to it in
    both Expo CLI and Expo Go.
  </Requirement>
  ...
  <Requirement title="A developer machine with a terminal">
    A macOS, Linux, or Windows (PowerShell and [WSL2](https://expo.fyi/wsl)) with a terminal window
    open.
  </Requirement>
</Prerequisites>
...
After running the above command, there are two files (**index.tsx** and **\_layout.tsx**) left inside the **src/app** directory. The previous files from the **src** directory (including **components**, **constants**, and **hooks**) are moved inside the **example** directory by the script. We'll create our own directories and component files as we go along.
...
> **info** On a physical iOS device, Expo Go opens your project only when Expo CLI and Expo Go are signed in to the same Expo account. Run `npx expo login` on your computer, then sign in to Expo Go with the same account. If the error remains, see [Expo Go sign-in required](/troubleshooting/expo-go-sign-in-required/).
...
The **src/app/index.tsx** file defines the text displayed on the app's screen. It is the entry point of our app and executes when the development server starts. It uses core React Native components such as `<View>` and `<Text>` to display background and text.
...
{/* prettier-ignore */}
```tsx src/app/index.tsx|collapseHeight=340
import { Text, View, /* @tutinfo Import `StyleSheet` to define styles. */ StyleSheet/* @end */ } from 'react-native';

export default function Index() {
  return (
    <View style={styles.container}>
      /* @tutinfo This used to say "Edit src/app/index.tsx to edit this screen". Now it says, "Home screen". */
...
<Collapsible summary="What is the blue gear icon?">

The blue gear icon is the **Tools button** for Android and iOS platforms. It's a floating shortcut that opens the [Developer menu](/debugging/tools/#developer-menu), which provides access to debugging functions such as reloading the app and toggling Fast Refresh. It appears only while developing and is never included in a production build.

The remaining screenshots in this tutorial will have it hidden. To hide it:

1. Open the Developer menu. Shake your device, or press <kbd>M</kbd> in the terminal running the development server if you're using an emulator, a simulator, or a device connected with USB.
2. Find the **Tools button** option and toggle it off.

Hiding it doesn't disable the Developer menu. You can still open it the way described in Step 1 whenever you need it.
