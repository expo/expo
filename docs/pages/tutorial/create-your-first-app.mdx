...
The tutorial assumes that you are familiar with TypeScript and React. If you are not familiar with these tools, first check out the [TypeScript Handbook](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html) and [React's official tutorial](https://react.dev/learn).
...
This command will create a new project directory named StickerSmash, using the [default template](/more/create-expo/#--template). This template has essential boilerplate code and libraries needed to build our app, including Expo Router. We'll continue to add more libraries throughout this tutorial as needed.
...
1. The developement server will start, and you'll see a QR code inside the terminal window.
