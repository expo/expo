...
After running the above command, there are two files (**index.tsx** and **\_layout.tsx**) left inside the **src/app** directory. The previous files from **src** and other directories (**components**, **constants**, and **hooks** — containing boilerplate code) are moved inside the **example** directory by the script. We'll create our own directories and component files as we go along.
