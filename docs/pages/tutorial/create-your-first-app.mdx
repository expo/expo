...
> With the release of SDK 51, we updated the default template to use [Expo Router](/router/introduction/). This tutorial only has a single screen, so we create the app with the `blank` template instead. After you complete the tutorial, check out the [Expo Router guide](/router/introduction/) to see how to create an app with many screens.
