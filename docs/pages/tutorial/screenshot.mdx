...
export default function Index() {
  /* @tutinfo Add this statement to import the permissions status and <CODE>requestPermission()</CODE> method from the hook. */const [permissionResponse, requestPermission] = MediaLibrary.usePermissions();/* @end */
  // ...rest of the code remains same
...
In the above snippet, the `collapsable` prop is set to `false`. This allows `<View>` component to screenshot only of the background image and emoji sticker.
...
<ProgressTracker
  ...
  name="GET_STARTED"
  summary="We've successfully used react-native-view-shot and expo-media-library to capture a screenshot and save it on the device's library."
