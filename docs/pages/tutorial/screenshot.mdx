...
The `captureRef()` method also returns a promise that fulfills with the screenshot's URI. We will pass this URI as a parameter to [`MediaLibrary.Asset.create()`](/versions/v57.0.0/sdk/media-library/#createfilepath-album) and save the screenshot to the device's media library.
