...
1. Declare a state variable called `showAppOptions` to show or hide buttons that open the modal alongside a few other options. It is a boolean. When the app screen loads, we'll set it to `false` so that the options are not shown before picking an image. When the user picks an image or uses the placeholder image, we'll set it to `true`.
2. Modify the `pickImageAsync()` function to set the value of `showAppOptions` to `true` after the user picks an image.
...
In the above snippet, we're also conditionally rendering the `Button` component based on the value of `showAppOptions` and moving the buttons in the conditional operator block. When the value of `showAppOptions` is `true`, render an empty `<View>` component. We'll address this state in the next step.

Now, we can remove the `alert` on the `Button` component and update the `onPress` prop when rendering the second button in **components/Button.tsx**:
...
Inside the **components** directory, create a new file called **CircleButton.tsx** with the following code:
...
2. Add three placeholder functions for these buttons. The `onReset()` function is called when the user presses the reset button. When this button is pressed, we'll show the image picker button again. For the other two functions, we'll add the functionality later.
...
2. Create an `isModalVisible` state variable with the `useState` hook. It has a default value of `false` to ensure that the modal is hidden until the user presses the button to open it.
...
4. Create a`onModalClose()` function to update the `isModalVisible` state variable.
...
Create a file named **EmojiList.tsx** file in the **components** directory and add the following code:
...
Now, modify the `app/index.tsx` to import the `<EmojiList>` component and replace the comments where the `<EmojiPicker>` component is used with the following code snippet:
...
Import this component in the **app/index.tsx** file and update the `Index` component to display the emoji sticker on the image conditionally. We'll do this by checking if the `pickedEmoji` state is not `undefined`.
