...
React Native provides a [`<Modal>` component](https://reactnative.dev/docs/modal) that presents content above the rest of your app. In general, modals are used to draw a user's attention toward critical information or guide them to take action. For example, in the [third chapter](/tutorial/build-a-screen/#step-7-enhance-the-reusable-button-component), after pressing the button, we used a modal to a placeholder text. That's how a modal component displays an overlay.
...
1. Declare a state variable, `showAppOptions`, to show or hide buttons that open the modal alongside a few other options. It is a boolean. When the app screen loads, we'll set it to `false` so the options are not shown before picking an image. When the user picks an image or uses the placeholder image, we'll set it to `true`.
...
export default function Index() {
  ...
  /* @info Create a state variable inside the <CODE>Index</CODE> component. */
  ...
  const pickImageAsync = async () => {
    ...
    if (!result.canceled) {
      ...
      /* @info After selecting the image, set the app options to true. */
  ...
  return (
    <View style={styles.container}>
      ...
      /* @info Based on the value of <CODE>showAppOptions</CODE>, the buttons will be displayed. Also, move the existing buttons in the conditional operator block. */
      {showAppOptions ? (
        <View />
      /* @end */
      ) : (
        <View style={styles.footerContainer}>
          ...
          /* @info Replace the <CODE>alert()</CODE> with the <CODE>onPress</CODE>.*/
...
In the above snippet, we're rendering the `Button` component based on the value of `showAppOptions` and moving the buttons in the conditional operator block. When the value of `showAppOptions` is `true`, render an empty `<View>` component. We'll address this state in the next step.
...
<Pressable style={styles.button} /* @info Replace the <CODE>alert()</CODE> with the <CODE>onPress</CODE>.*/ onPress={onPress}/* @end */ >
```
...
1. Import `CircleButton` and `IconButton` components to display them.
2. Add three placeholder functions for these buttons. The `onReset()` function invokes when the user presses the reset button and we'll show the image picker button again. We'll add the functionality for the other two functions later.
...
import Button from '@/components/Button';
import ImageViewer from '@/components/ImageViewer';
/* @info Import <CODE>IconButton</CODE> and <CODE>CircleButton</CODE> components.*/
...
export default function Index() {
  ...
  return (
    <View style={styles.container}>
      ...
      {showAppOptions ? (
        /* @info Replace empty <CODE>View</CODE> component with this snippet to display App option buttons. */
...
const styles = StyleSheet.create({
  ...
  /* @info Add styles for the new <CODE>View</CODE> components. */
  optionsContainer: {
    position: 'absolute',
    bottom: 80,
  },
...
- `isVisible`: a boolean determining the state of the modal's visibility.
...
Now, let's modify the **app/index.tsx**:
...
import IconButton from '@/components/IconButton';
import CircleButton from '@/components/CircleButton';
/* @info import the <CODE>EmojiPicker</CODE> component. */
...
export default function Index() {
  ...
  return (
    <View style={styles.container}>
      ...
      /* @info Render the <CODE>EmojiPicker</CODE> component at the bottom of the <CODE>Index</CODE> component. */
...
- The `<FlatList>` component above renders all the emoji images using an `Image` component, wrapped by a `<Pressable>`. Later, we will improve it so that the user can tap an emoji on the screen to make it appear as a sticker on the image.
- The `<FlatList>` component takes an array of items provided by the `emoji` array variable as the value of the `data` prop. The `renderItem` prop takes the item from the `data` and returns the item in the list. Finally, we added `Image` and the `<Pressable>` components to display this item.
...
Now, update the `app/index.tsx` to import the `<EmojiList>` component and replace the comments inside `<EmojiPicker>` component with the following code snippet:
...
import CircleButton from '@/components/CircleButton';
import EmojiPicker from '@/components/EmojiPicker';
/* @info Import the <CODE>EmojiList</CODE> component. */
...
export default function Index() {
  ...
  return (
    <View style={styles.container}>
      ...
      <EmojiPicker isVisible={isModalVisible} onClose={onModalClose}>
        /* @info Render the <CODE>EmojiList</CODE> component inside the <CODE>EmojiPicker</CODE> component. */
...
Import this component in the app/index.tsx file and update the `Index` component to display the emoji sticker on the image. We'll check if the `pickedEmoji` state is not `undefined`:
...
import EmojiPicker from '@/components/EmojiPicker';
import EmojiList from '@/components/EmojiList';
/* @info Import the <CODE>EmojiSticker</CODE> component. */
...
export default function Index() {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.imageContainer}>
        <ImageViewer imgSource={PlaceholderImage} selectedImage={selectedImage} />
        /* @info Add this line to display the emoji sticker on the image. */
