...
In **app.json**, the `expo-splash-screen` plugin object is already configured to use the app's icon as the splash screen image. We can replace the placeholder image with our own splash screen image. We need to replace `app-icon` in the `image` property with the `icon`:
