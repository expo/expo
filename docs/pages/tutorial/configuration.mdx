...
The splash screen is configured by defining a path to the `"splash.image"` property in the <LinkBase href="/workflow/configuration/" openInNewTab>**app.json** file</LinkBase>. It has a current value of `"./assets/splash.png"` path. This is already done by default when a new Expo project is created.
...
The next section of the tutorial will guide you toward resources to learn more about concepts we've covered here and others we have mentioned briefly. [Continue to find out how you can learn more](/tutorial/follow-up).
