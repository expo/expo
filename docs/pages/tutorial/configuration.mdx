...
We've to configure the status bar, add a splash screen, and an app icon to consider the app in a launchable state. In this chapter, we'll learn how to do all of this.

<Step label="1">

## Configure the status bar

[`expo-status-bar`](/versions/latest/sdk/status-bar/) library comes pre-installed in every project created using `create-expo-app`. This library provides a `<StatusBar>` component to configure the app's status bar by changing the text color, background color, making it translucent, and so on.
...
A splash screen is visible before the app's contents have had a chance to load. It hides once the app's content is ready for display.

Defining a path to the `"splash.image"` property in the [**app.json**](/workflow/configuration/) file configures a splash screen. By default, a new Expo project already defines the path. We already have **splash.png** in the **assets/images** directory. It looks as shown below:
...
You can display the splash screen for longer intervals. Run running the following command to install `expo-splash-screen`, a library that provides a `preventAutoHideAsync()` method to delay hiding the splash screen:
...
Depending on the device's resolution, a white bar is visible on the edges of an Android device's screen. To resolve this, set the `backgroundColor` for the splash screen in app.json:
