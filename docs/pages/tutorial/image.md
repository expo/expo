...
We have displayed text using the `Text` component from React Native, and we can display the image using the `Image` component. When creating an `Image` component for Web you will need to explicitly specify a width and height, or it will default to 0x0.
