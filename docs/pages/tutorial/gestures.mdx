...
Stop the development server by pressing <kbd>Ctrl</kbd> + <kbd>c</kbd> in the terminal. Then, install the required dependencies:

<Terminal cmd={['$ npx expo install react-native-gesture-handler react-native-reanimated']} />

The [`npx expo install`](/more/expo-cli/#installation) command will install the libraries and add them to the project's dependencies in **package.json**.

In the project directory, run the following command to start the development server again.
