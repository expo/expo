...
To animate the transition, let's use a spring-based animation. This will make it feel alive because it's based on the real-world physics of a spring. We will use the `withSpring()` hook provided by `react-native-reanimated`.
...
In the previous step, we triggered the `onActive()` callback for the tap gesture inside the `useAnimatedGestureHandler()` function. Similarly, for the pan gesture, we have to specify two callbacks:
...
Then add the `containerStyle` from the above snippet on the `<AnimatedView>` component to apply the transform styles. Also, update the `<EmojiSticker>` component so that the `<PanGestureHandler>` component becomes the top-level component.
