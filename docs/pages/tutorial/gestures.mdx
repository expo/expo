...
1. Open the **EmojiSticker.tsx** file in the **src/components** directory. Inside it, import `Animated` from the `react-native-reanimated` library to use animated components.
2. Replace the `Image` component with `<Animated.Image>`.

{/* prettier-ignore */}
```tsx src/components/EmojiSticker.tsx
...
In the **src/components/EmojiSticker.tsx** file:
...
To recognize a dragging gesture on the sticker and to track its movement, we'll use a pan gesture. In the **src/components/EmojiSticker.tsx**:
