...
The "Choose a photo" button requires different styling than the "Use this photo" button, so we will add a new button theme prop that will allow us to apply a `primary` theme. This button also has an icon before the label. We will use an icon from the <LinkBase href="/guides/icons/#expovector-icons" openInNewTab>`@expo/vector-icons` library</LinkBase> that icons from popular icon sets.
