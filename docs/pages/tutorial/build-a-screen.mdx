...
`Image` component takes the source of an image as its value. The source can be either a [static asset](https://reactnative.dev/docs/images#static-image-resources) or a URL. For example, the source required from **./assets/images** directory is static. It can also come from [Network](https://reactnative.dev/docs/images#network-images) as a `uri` property.
...
Import `ImageViewer` and use it in the **app/index.tsx**:
...
React Native provides various components to handle touch events on native platforms. For this tutorial, we'll use the [`<Pressable>`](https://reactnative.dev/docs/pressable). It can detect various stages of interactions, from basic single-tap events to advanced events such as long press.

In the design, there are two buttons we need to create. Each has different styles and labels. Let's start by creating a reusable component for these buttons. Create a **Button.tsx** file inside the components directory with the following code:
...
The app displays an alert when the user taps any of the buttons on the screen. It happens because `<Pressable>` calls an `alert()` on its `onPress` prop. Let's import this component into app/index.tsx file and add styles for `<View>` that encapsulates these buttons:
...
The "Choose a photo" button requires different styling than the "Use this photo" button, so we will add a new button theme prop that will allow us to apply a `primary` theme. This button also has an icon before the label. We will use an icon from `@expo/vector-icons` library.
...
- `<Pressable>` component in the primary theme uses a `backgroundColor` property of `#fff` to set the button's background color. If we add this property to the `styles.button`, the background color value will be set for both the primary theme and the unstyled one.
