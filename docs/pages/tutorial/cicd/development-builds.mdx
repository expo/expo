...
On the EAS dashboard, notice that it says "Triggered manually" under the **Workflow graph** tab. Any time the **Trigger** is set to **Manual**, the workflow was triggered using the `eas workflow:run` command.
...
The logs for each build appear inside the workflow interface. Clicking the build ID (for example, `ef90d1f` is the Android build ID above) opens the EAS Build page.
...
Wait for both builds to complete before moving to the next step. Once they complete, we can use the **Install** or **Open with [Orbit](/build/orbit/)** buttons, or download from **Artifacts** to install on our device/emulator/simulator.
