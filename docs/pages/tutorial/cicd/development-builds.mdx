...
The `build` job type is one of the [pre-packaged jobs](/tutorial/cicd/first-workflow/#types-of-jobs-in-eas-workflows) from EAS Workflows. Instead of defining each step of the build process, we use the `build` job type to run an EAS Build for Android and iOS.
...
jobs:
  ...
  fingerprint:
    name: Fingerprint
    # @info Add this to the fingerprint job. It runs only after tests pass; if tests fail, nothing builds. #
    needs: [run_tests]
    # @end #
    type: fingerprint
    environment: development
