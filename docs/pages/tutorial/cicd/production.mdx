...
Rebuilding the app for every production change is resourceful when most changes are TypeScript/JavaScript only.
...
EAS CLI prompts to generate credentials the first time we trigger a build for a profile. Trigger the production build manually for both Android and iOS so the credentials exist before the workflow runs using the command below:
