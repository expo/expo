...
<ProgressTracker
  ...
  name="CICD_TUTORIAL"
  summary="We configured the Expo project for static web export, added a deploy job to the preview workflow for non-production web URLs, and added a deploy job with `prod: true` to the production workflow so every release ships the web build with the native release."
