...
5. Add the webhook URL as an EAS [environment variable](/eas/environment-variables/manage/) named `SLACK_WEBHOOK_URL` for the `preview` environment. Set its [visibility](/eas/environment-variables/#visibility-settings-for-environment-variables) to **secret** since only EAS servers need to read it.
