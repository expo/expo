...
We'll use [`expo-image-picker`](/versions/latest/sdk/image picker), a library from Expo SDK.
...
> info Tip: Any time we install a new library in our project, stop the development server by pressing <kbd>Ctrl</kbd> + <kbd>c</kbd> in the terminal and then run the installation command. After the installation, we can start the development server again by running `npx expo start` from the same terminal window.
...
- The `launchImageLibraryAsync()` receives an object to specify different options. This object is an [`ImagePickerOptions`](/versions/latest/sdk/imagepicker/#imagepickeroptions) object. We can pass the object to specify different options when invoking the method.
