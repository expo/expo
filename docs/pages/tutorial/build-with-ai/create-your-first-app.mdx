...
The agent takes a few minutes to download the template and install its dependencies. When it finishes, it should report that the project is ready, with a minimal **src/app** folder containing a couple of files.
...
> **info** On a physical iOS device, Expo Go opens your project only when Expo CLI and Expo Go are signed in to the same Expo account. You did both in the previous chapter. If you see an account error, see [Expo Go sign-in required](/troubleshooting/expo-go-sign-in-required/).
