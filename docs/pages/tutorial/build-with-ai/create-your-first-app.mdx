...
In this chapter, your agent creates a new Expo app, you get it running on your phone, and you make your first change — the loop you'll repeat for the rest of the tutorial.
...
<Terminal cmd={['$ mkdir StickerSmash', '$ cd StickerSmash']} />
...
The agent will take a few minutes to download the template and install its dependencies. When it finishes, it should report that the project is ready, with a minimal **app** folder containing a couple of files.
...
<Terminal cmd={['$ cd StickerSmash', '$ npx expo start']} />
...
- **Paste errors verbatim.** If your phone shows a red error screen or the terminal prints an error, copy the whole message into the agent. Error messages are written for programmers — your agent is one.
...
You created an app, ran it on your phone, and changed it with a single prompt. Next, you'll turn this empty screen into the real thing.

<BoxLink
  title="Next: Build the home screen"
  Icon={BookOpen02Icon}
  description="Add navigation tabs, the photo viewer, and an image picker."
  href="/tutorial/build-with-ai/build-the-home-screen/"
/>
