...
In this chapter, we're going to learn how to set up internal distribution builds. Creating an internal build is quite similar to creating a development build. However, the difference is how it used and distributed.
...
This is all we need to create our first internal distribution build. The `preview` build profile from the above snippet has `distribution` property whose value is set to `internal`. This value allows us to share our build URLs with anyone so they to install it on their device and do not require a development server to run the app.
