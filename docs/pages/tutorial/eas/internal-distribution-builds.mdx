...
On the other hand, Apple has stricter rules for app distribution on iOS devices. We need an ad hoc provisioning profile that explicitly lists the devices allowed to run the app. Alternatively, to ad hoc provisioning profiles, you can use enterprise provisioning, which requires a special Apple Developer Program membership. It costs $299 USD per year and allows running the app on any device without device registration.
...
- In projects generated with Expo CLI, use **app.config.js** to dynamically switch identifiers.
- In existing React Native projects, create a separate `scheme` for each bundle identifier and specify the scheme name in separate build profiles.
