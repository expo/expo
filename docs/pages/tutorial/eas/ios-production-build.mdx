...
The only required configuration to create our production build for iOS is to have a `production` build profile in **eas.json** which is added by default.
...
The `production` profile in the EAS configuration is the default profile. We don't have to pass it using the `--profile` flag.
...
<ProgressTracker currentChapterIndex={8} name="EAS_TUTORIAL" />
