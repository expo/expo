...
> **info** Creating the first release manually is not required: `eas submit` can create your app's first release directly. In this tutorial, we walk through Play Console to get familiar with how releases work.
...
From now on, we can use [EAS Submit](/submit/android/) to automate releases and avoid the manual process. To do that, we need to add the service account key to our project's credentials.
