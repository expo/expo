...
EAS Build can assist us in automating these values by utilizing the [remote version resource](/build-reference/app-versions/#remote-version-source). This feature automatically increments developer-facing values and keeps track of them whenever a new production release is made. This behavior is enabled by default, but we can also explicitly set the `cli.appVersionSource` and `build.production.autoIncrement` values.
...
We can also manage the app versions manually. To enable that we need to set the `cli.appVersionSource` to `local`.

In **eas.json**:

- Add `cli.appVersionSource` and set it to `local`
...
```json eas.json
{
  "cli": {
    /* @hide ... */ /* @end */
    /* @info Add <CODE>appVersionSource</CODE> and set it to <CODE>local</CODE>. */
