...
In **eas.json**, add a new build profile called `ios-simulator` with the [`ios.simulator`](/eas/json/#simulator) property. Set its value to `true`:
...
- **What would you like your iOS bundle identifier to be?** Press <kbd>Return</kbd> to select the default value provided for this prompt. This will add [`ios.bundleIdentifier`](/versions/latest/config/app/#bundleidentifier) in **app.json**.
