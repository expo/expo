...
You can also use a [fingerprint workflow](/eas/workflows/examples/deploy-to-production/) to create builds only when native code changes in your Android and iOS projects. Otherwise, for JavaScript-only changes, the workflow will skip creating a new build and publish updates through EAS Update.

build or update selectively for both Android and iOS platforms. depending on whether a compatible native runtime already exists for your project.
