...
[Production, Enterprise, and Legacy plans](/billing/plans/#plans) subscribers receive credits for EAS Build. These credits can be used to offset the cost of builds. They are reset at the start of the billing cycle and expire at the end of that billing cycle.
...
Monthly updated users reflect the number of unique users who download an update in a billing cycle. At the same time, global edge bandwidth represents the total amount of bandwidth used for all update downloads. Moreover, each monthly updated user includes 40 MiB of global edge bandwidth, up to that plan's global edge bandwidth limit.

> **Note**: A monthly updated user counts only once per billing cycle, regardless of how many updates this user downloads.
...
For example, let's consider a subscriber to the On-demand plan who deploys 5 updates of 20 MiB each via EAS Update to 10,000 users. The subscription to the plan includes 1,000 monthly updated users and 100 GiB per month. As a result, the subscriber's overage bill will be:
...
| ------------------------------------------------- | ----------------------------------- | ------------ |
| Bandwidth used to send updates                    | 5 updates \* 20 MiB \* 10,000 users | 976.5625 GiB |
|                                                   |
...
> **Note**: If the subscriber then sends another 20 MiB update to the same 10,000 users in the current billing cycle, they will only pay for any bandwidth overage used. This is because Expo only charges for [unique monthly updated users](/eas-update/faq/#how-are-monthly-updated-users-counted-for-a-billing-cycle).

If the same subscriber is on a Production plan, they will pay $0 as the Production plan includes 20,000 monthly updated users and 1 TiB (1024 GiB). As such, there is no overage.
...
To see detailed EAS Build usage for a current or previous billing cycle:
...
To see detailed EAS Update usage for a current or previous billing cycle:
...
You can manage certain assets to include or exclude when using EAS Update. It reduces the number of assets uploaded or downloaded from the updates server and the global edge bandwidth used.

To optimize the update usage, you can choose to exclude assets that haven't been modified. For example, images or videos that haven't been changed can be excluded. Excluded assets won't be uploaded to the update server and won't be downloaded by the app. However, it's important to make sure that assets that are not part of an update are included in the native build of the app.
