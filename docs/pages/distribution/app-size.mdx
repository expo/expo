...
The above options correspond to the `expo.gif.enabled` and `expo.webp.enabled` Gradle properties in `android/gradle.properties`. The animated WebP decoder (`expo.webp.animated`) is disabled by default because React Native's `<Image>` on iOS doesn't support animated WebP.
