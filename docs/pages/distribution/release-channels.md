...
You can edit the native project's release channel by modifying the `EXUpdatesReleaseChannel` key in `Expo.plist` (iOS) or the `releaseChannel` meta-data tag value in `AndroidManifest.xml` (Android). [Read this guide](/bare/updating-your-app.md) for more information on configuring OTA Updates in a bare app.
