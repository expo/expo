...
You need to create a custom server to host the websites with API routes. Learn more about [serving locally with Express](https://docs.expo.dev/router/reference/api-routes/#express).
...
<Terminal cmd={['$ git init', '$ git remote add origin https://github.com/username/expo-gh-pages.git' ]} />

The above commands initialize a new git repository and configure git to know which repository on GitHub to push your source code to.
...
Files in the **public** directory will be copied to the **dist** directory when you build your project.

> You also need to run `gh-pages` with the `-t` flag to ensure files starting with `.` are included in the deployment, which is handled in the scripts added to **package.json** in the next step.
...
This will publish a build of your app to the `gh-pages` branch of your GitHub repository. This branch will only have your build code in the root directory. It will not contain your development source code.

</Step>

<Step label="7">

Now that your app is published to the `gh-pages` branch, you need to configure GitHub Pages to serve the app from that branch.

On your GitHub repository, navigate to the **Settings** tab and scroll down to the **Pages** section. Make sure **Source** is set to "Deploy from a branch". For the **Branch** section, select the **gh-pages** branch and the **root** directory.
...
Once the app is published and the GitHub Pages configuration is set, a GitHub action will start to deploy your website. You can check its progress by navigating to the **Actions** tab in your repository.

Once this job is complete, your web app will be available at the URL `http://{username on GitHub}.github.io/{repo-name}`.

In the future, you can run the `deploy` command and the GitHub action to update your page will start automatically.
