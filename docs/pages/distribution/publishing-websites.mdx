...
> **info** For more information on unlimited hosting, check out their [pricing](https://vercel.com/pricing).
...
> **info** To support routers that use the HTML 5 `pushState` API, you'll need to rename the **web-build/index.html** to **web/200.html** before deploying.
...
> **info** We'll use `yarn` but you can use `npm` if you want.
...
> **info** In projects running SDK 45 or lower, use `expo build:web` instead of `expo export:web`.
...
4. **Configure your **package.json** for web hosting**
   ...
   > **info** In projects running SDK 45 or lower, use `expo build:web` instead of `expo export:web`. `predeploy` is automatically run before `deploy`.
...
5. **Generate a _production build_ of your app, and deploy it to GitHub Pages.** (2 minutes)
   ...
   > **info** When you publish code to `gh-pages`, it will create and push the code to a branch in your repo called `gh-pages`. This branch will have your built code but not your development source code.
