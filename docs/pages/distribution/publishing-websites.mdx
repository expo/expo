...
The project files will be output in the **dist** folder. Any files inside the **public** directory will also be copied to the **dist** folder.
...
The project files will be output in the **web-build** folder. Any files inside the **web** directory will also be copied to the **web-build** folder.
...
Then, open [`http://localhost:5000`](http://localhost:5000) to see your project in action. This method is **HTTP only**, so permissions, camera, location, and many other secure features won't work.
...
`expo.web.output: 'single'` generates a single-page application. This means there's only one **dist/index.html** file which all requests must be redirected to. This can be done in Netlify by creating a `./public/_redirects` file and redirecting all requests to `/index.html`.
...
> For more information on hosting, check out [Vercel's pricing](https://vercel.com/pricing).
...
<Step label="2">
  ...
  <Tabs>
    <Tab label="Expo Router">
      ...
      `expo.web.output: 'single'` generates a single-page application. This means there's only one **dist/index.html** file which all requests must be redirected to. This can be done in Vercel by creating a `./public/vercel.json` file and redirecting all requests to `/index.html`.
    ...
    <Tab label="Webpack">
      If your app implements any navigation, you'll need to configure Vercel to redirect requests to the single **web-build/index.html** file. This can be done in Vercel by creating a `./public/vercel.json` file and redirecting all requests to `/index.html`.
...
<Step label="4">
  ...
  <Tabs>
    <Tab label="Expo Router">
      <Terminal cmd={['$ vercel deploy dist --prebuilt']} />
    </Tab>
    <Tab label="Webpack">
      <Terminal cmd={['$ vercel deploy web-build --prebuilt']} />
    </Tab>
  </Tabs>
...
> Expo Router websites do not support deployment to GitHub Pages yet.
