...
Custom builds allow customizing the build process for your project by running commands before, during, or after the build process.

<Step label="1">

## Create a custom build config

To get started, create a directories and a file named **.eas/build/hello-world.yml** at the same level as **eas.json**. The location and name of both directories are important for EAS Build to identify that a project contains a custom build config.
...
<Terminal cmd={['$ eas build -p android -e production']} />
