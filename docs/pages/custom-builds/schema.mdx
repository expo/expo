...
|                     | **Note:** available Simulators are different for every image, so if you change the image, the Simulator for given name may become unavailable.                |
|                     | For instance, an Xcode 14 image will have iPhone 14 Simulators while an Xcode 15 image will have iPhone 15 simulators. We encourage not providing this input. |
