...
| Input             | Description                                                                                                              |
|-------------------|--------------------------------------------------------------------------------------------------------------------------|
| `maestro_version` | **string** Maestro version to install (e.g. 1.35.0). If not provided, `install_maestro` will install the latest version. |
...
| Input                  | Description                                                                                                                                             |
|------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| `device_name`          | **string** Name for the created device. You may wish to customize it if starting multiple emulators.                                                    |
| `system_image_package` | **string** Android package path you wish to use for the emulator, e.g. `system-images;android-30;default;x86_64`.                                       |
|                        | To get list of available system images, run `sdkmanager --list` on your computer ([command docs](https://developer.android.com/tools/sdkmanager#list)). |
|                        | VMs run on x86_64 architecture, so always choose `x86_64` package variants.                                                                             |
|                        | The `sdkmanager` tool comes from Android SDK command-line tools ([installation instructions](https://developer.android.com/tools/sdkmanager)).          |
...
| Input               | Description                                                                                                                                      |
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|
| `device_identifier` | **string** Name or UDID of the Simulator you want to start.                                                                                      |
|                     | Examples include `iPhone [XY] Pro`, `AEF997BB-222C-4379-89BA-D21070B1D787`.                                                                      |
|                     | **Note:** available Simulators are different for every image, so if you change the image, the Simulator for a given name may become unavailable. |
|                     | For instance, an Xcode 14 image will have iPhone 14 Simulators while an Xcode 15 image will have iPhone 15 simulators.                           |
|                     | In general we encourage not providing this input.                                                                                                |
|                     | Learn more about [runner images](/build/eas-json/#selecting-a-base-image).                                                                       |
