...
| `system_image_package` | **string** Android package path you wish to use for the emulator, e.g. `system-images;android-30;default;x86_64`.<br />To get list of available system images, run `sdkmanager --list` on your computer ([command docs](https://developer.android.com/tools/sdkmanager#list)). VMs run on x86_64 architecture, so always choose `x86_64` package variants. The `sdkmanager` tool comes from Android SDK command-line tools ([installation instructions](https://developer.android.com/tools/sdkmanager)). |
