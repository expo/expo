...
`create-dev-plugin` will setup a new plugin project for you. Run the following command to create a new plugin project:

<Terminal cmd={['$ npx create-dev-plugin']} />

`create-dev-plugin` will prompt you for a name of your plugin, a description, and the name of the hook that will be used by consumers of your plugin.

The plugin project will contain the following folders:
...
### Customize your plugin's functionality
...
Listens for a message matching the type string and invokes the callback with the message data.
...
Edit the hook in the **src** folder to customize what diagnostic information is sent to the plugin or how the app should respond to any messages from the web user interface:
...
if (process.env.NODE_ENV !== 'production') {
  useTestPlugin = require('./useTestPlugin').useTestPlugin;
} else {
  useTestPlugin = () => ({
+    sendPing: () => {},
  });
...
To prepare your plugin for distribution or for use within your monorepo, you will need to build the plugin with the following command:

<Terminal cmd={['$ npm run prepublishOnly']} />

This command will build the hook code into the **build** folder, and the web user interface into the **dist** folder.
...
export default function App() {
  const { sendPing } = useTestPlugin();

  return (
    <View style={styles.container}>
      <Button title="Ping" onPress={() => { sendPing(); }}>
    </View>
  );
}
```