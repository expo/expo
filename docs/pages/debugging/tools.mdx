...
React Native Debugger relies on [Remote JS Debugging](https://reactnative.dev/docs/other-debugging-methods#remote-javascript-debugging-deprecated) workflow, which is deprecated since React Native 0.73. To use Remote JS Debugging, you have to install additional packages and disable the default debugging workflow.

First, you need to install the `@react-native-community/cli-server-api` package in your project. This package contains the Remote JS Debugging functionality.

<Terminal cmd={['$ npx expo install @react-native-community/cli-server-api']} />

Once the package is installed you need to disable the default debugging workflow, as described in the [Remote JS Debugging documentation](https://reactnative.dev/docs/other-debugging-methods#remote-javascript-debugging-deprecated). You can do this by adding the following code to your app entry file:

```js App.js
import NativeDevSettings from 'react-native/Libraries/NativeModules/specs/NativeDevSettings';

// Only enable Remote JS Debugging in development
if (__DEV__) {
