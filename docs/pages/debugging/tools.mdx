...
For automation or agent-driven runs, keep the developer menu out of the way. Run `EXPO_NO_DEV_MENU=1 npx expo start` to launch with the menu closed, the tools button hidden, and onboarding skipped. These parameters update the saved dev menu settings, which you can re-enable from the dev menu. See [Deep linking to an update's URL](/develop/development-builds/development-workflows/#deep-linking-to-an-updates-url) for the individual query parameters.
