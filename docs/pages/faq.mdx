...
Expo CLI can be used simultaneously with React Native Community CLI (sometimes referred to as "React Native CLI"). Expo CLI provides the same core functionality as React Native CLI with additional functionality such as automatic [TypeScript setup](/guides/typescript), [web support](/workflow/web), [auto installing compatible libraries](/more/expo-cli/#install), [improved native build commands](/more/expo-cli#compiling), [tunneling](/more/expo-cli#tunneling), [prebuild](/workflow/prebuild), [and more](/more/expo-cli).
...
Learn more about why you might want to use [Expo CLI](/bare/using-expo-cli/) and [use a framework to build React Native apps](https://reactnative.dev/blog/2024/06/25/use-a-framework-to-build-react-native-apps).
