...
When ready, you can create a production build (**.aab** and **.ipa**) to submit to the app stores. You can build your app in a single command with [EAS Build](/build/introduction) and submit it to the stores with [EAS Submit](/deploy/submit-to-app-stores/).
...
Traditionally you needed a macOS to develop iOS apps, however, you can use [EAS Build](/build/introduction) to build your app in the cloud. You can also use [EAS Submit](/deploy/submit-to-app-stores/) to submit your app to the stores. Testing can be done on a physical iOS device using [Expo Go](https://expo.dev/go) or a [development build](/develop/development-builds/introduction/).
...
- Learn about the benefits of migrating to Expo CLI, such as better app performance, faster releases, and closer collaboration across your team, in [this blog post](https://expo.dev/blog/from-rnc-cli-to-expo).
