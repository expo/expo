...
The following instructions apply to installing the latest version of Expo modules in React Native {latestSdkVersionValues['react-native']}. For previous versions, check the [native upgrade helper](/bare/upgrade) to see how these files are customized.
...
Save all of your changes and update your iOS Deployment Target in Xcode to <code>iOS {latestSdkVersionValues.ios.replace('+', '')}</code>:

- Open **your-project-name.xcworkspace** in Xcode, select your project in the left sidebar.
- Select **Targets** > **your-project-name** > **Build Settings** > **iOS Deployment Target** and set it to <code>iOS {latestSdkVersionValues.ios.replace('+', '')}</code>.
