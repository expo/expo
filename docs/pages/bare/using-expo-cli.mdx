...
**In SDK 49 and higher**, all you need to do to use Expo CLI in any React Native app is to install the `expo` package.

**In SDK 48 and lower**, you will need to install the native module side (`npx install-expo-modules@latest`) or explicitly opt out of with autolinking. when you install the `expo` package with `npx install-expo-modules`. If you just want to try out Expo CLI for now, you can install the `expo` package with `npm install` and then configure your **react-native.config.js** to exclude the package from autolinking:
