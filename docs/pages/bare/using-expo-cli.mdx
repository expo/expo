...
- [First-class TypeScript support](/guides/typescript.mdx).
- Support for `tsconfig.json` aliases with `paths` and `baseUrl` [built-in to Metro](/guides/typescript#path-aliases).
- [Web support](/guides/customizing-metro.mdx#adding-web-support-to-metro) with Metro. Fully typed for React Native Web.
