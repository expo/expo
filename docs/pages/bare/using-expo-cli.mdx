...
- Supports [Continuous Native Generation](/workflow/prebuild.md) (CNG) with `npx expo prebuild` for easy upgrades, white-labeling, simple third-party package setup, and improved maintainibility of your codebase (by reducing the surface area).
...
- Improved native build log formatting by using Expo's own `xcpretty`-style tool that is built specifically for React Native apps. For example, when compiling a Pod you can see which Node module included it.
