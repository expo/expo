...
This package includes the Expo Modules API and Expo CLI. For a complete explanation of how to install this, see the [Install Expo modules](/bare/installing-expo-modules.mdx) guide. For most apps, it's as simple as running the following command in your project directory:
...
- Support for [Continuous Native Generation](/workflow/prebuild/) (CNG) with `npx expo prebuild` for easy upgrades, white-labeling, simple third-party package setup,
...
- [Automatic TypeScript support](/guides/typescript/).
- [Automatic web support](/guides/customizing-metro/#adding-web-support-to-metro) with Metro.
- Out of the box [support for monorepos](/guides/monorepos/).
- Support for Expo tooling such as [expo-dev-client](/develop/development-builds/introduction/), the [Expo Updates protocol](/technical-specs/expo-updates-1/)
  and [EAS Update](/eas-update/introduction/).
...
Once you have the `expo` package installed, you can use the `npx expo run:android` and `npx expo run:ios` commands as alternatives to `npx react-native run-android` and `npx react-native run-ios`. You can pass in the `--device` flag to select a device/simulator to build for &mdash; this also works for any connected iOS device.

## Start the bundler

`npx expo run:[android|ios]` will start the bundler/development server. You can skip that by passing in the `--no-bundler` flag.
