...
> **info** If you later adopt CNG, you can configure the channel with [`updates.requestHeaders`](/versions/latest/config/app/#requestheaders) in **app.json** instead. Running `npx expo prebuild` in your pipeline then writes the channel into these files. See [Configure update channels in app.json](/eas-update/getting-started/#configure-update-channels-in-appjson).
