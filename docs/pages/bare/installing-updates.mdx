...
This property is also ultimately set in the **AndroidManifest.xml** and **Expo.plist** files that we modified above. It is usually set right before build, so you can flexibly switch between production and non-production builds. If you build your project with EAS Build, the channel is set in the **eas.json** build profile and then applied to these files right before the native build step.

When you build locally or with another service, you will need to set the channel yourself in these files:

### Android

In **android/app/src/main/AndroidManifest.xml**, add the request headers `meta-data` entry:
...
In **Expo.plist**, add the `EXUpdatesRequestHeaders` dictionary:
...
> **info** If you later adopt CNG, you can configure the channel with [`updates.requestHeaders`](/versions/latest/config/app/#requestheaders) in **app.json** instead. Then, your channel will be configured automatically when you run `npx expo prebuild` in your pipeline. See [Configure update channels in app.json](/eas-update/getting-started/#configure-update-channels-in-appjson).
