...
description: Add Expo and React Native to existing native apps using the "integrated" approach.
hideTOC: true
---
...
React Native offers a flexible development strategy. Whether you're launching a new app or looking to integrate an existing native app, it provides a powerful solution. You can leverage its capabilities to introduce new features or views incrementally, allowing your team to adopt modern, cross-platform development without a complete rewrite.

This guide will walk you through the steps to add a React Native view into an existing native app using the "integrated" approach.

## Prerequisites

To integrate React Native into your existing application, you'll need to set up a JavaScript development environment. This includes installing Node.js to run the Expo CLI and Yarn to manage the project's JavaScript dependencies.
...
- **[iOS only]** [CocoaPods](https://cocoapods.org/): one of the dependency management system available for iOS. CocoaPods is a Ruby [gem](https://en.wikipedia.org/wiki/RubyGems). You can install CocoaPods using the version of Ruby that ships with the latest version of macOS.
...
### Approach 1: Move your existing native project files

Move your existing Android and iOS projects to the **android** or **ios** sub-directories, depending on the platform you are integrating with. For example:

<Tabs>

<Tab label="Android">

<Terminal cmd={['$ mkdir exp/android', '$ mv /path/to/your/android-project exp/android/']} />

</Tab>
<Tab label="iOS">

<Terminal cmd={['$ mkdir exp/ios', '$ mv /path/to/your/ios-project exp/ios/']} />

</Tab>
</Tabs>

### Approach 2: Set up a monorepo
...
Then run `yarn install` to install the dependencies. This will ensure **node_modules** are installed at the root of your project, and that native scripts can interact with React Native code. Make sure to change `["exp"]` to the name of the Expo project you created in the previous step.
...
+expoAutolinking {
+  projectRoot = new File(rootDir, "exp")
+}
