...
### Shipping the artifacts as a Swift Package

Pass the `--package [name]` flag to wrap the built artifacts into a self-contained Swift Package instead of leaving them as loose `.xcframework` directories. Consumers can then add the package to their host app via Xcode's **Add Package Dependencies → Add Local…** flow rather than dragging individual frameworks in and configuring **Embed & Sign** by hand.

<Terminal cmd={['$ npx expo-brownfield build:ios --release --package MyAppPackage']} />

The flag accepts an optional name; if you omit it, the package name defaults to **\{TargetName\}Artifacts**. The output directory becomes a complete Swift Package laid out like this:
...
The build output you integrate depends on whether you are using just XCFrameworks or SPM.
...
When `build:ios` produced a Swift Package (for example, **./artifacts/MyAppPackage-release**), add it to your host app via **File → Add Package Dependencies → Add Local…** in Xcode and select the package directory. Xcode links every bundled `.xcframework` automatically through the single aggregate library product.

If you build both **--debug** and **--release**, point your host app at the matching flavor for each configuration.
