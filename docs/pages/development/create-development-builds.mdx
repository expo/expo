...
We also recommend to install EAS CLI as a global npm dependency by running the following command:
...
An EAS Build is created by running the `eas build` command. It also creates an [**eas.json**](https://docs.expo.dev/build/eas-json/) file at the root of your project directory with three build profiles automatically: `"development"`, `"preview"` and `"production"`. A minimal configuration is shown below:
...
> If you have created a development build for Android Emulator, you do not need to create it separately the device. You can skip this step since the same **.apk** will work in both scenarios.
