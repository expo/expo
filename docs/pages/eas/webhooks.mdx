---
title: Webhooks
sidebar_title: Configure EAS webhooks
...
EAS can alert you as soon as your build or submission has been completed via a webhook. Webhooks need to be configured per project.

For example, if you want to be alerted for both `@johndoe/awesomeApp` and `@johndoe/coolApp`, you need to the following command in each directory:
...
> If you want to test the above webhook locally, you have to use a service such as [ngrok](https://ngrok.com/docs) to forward `localhost:8080` via a tunnel and make it publicly accessible with the URL `ngrok` gives you.

You can always change your webhook URL and/or webhook secret by running:

<Terminal cmd={['$ eas webhook:update --id WEBHOOK_ID']} />

You can find the webhook ID by running:
...
app.post('/webhook', (req, res) => {
  ...
  if (!safeCompare(expoSignature, hash)) {
    ...
    // do something here, such as send a notification to Slack!
