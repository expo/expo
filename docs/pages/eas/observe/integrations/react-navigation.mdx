...
> **info** `ObserveNavigationContainer` is a drop-in replacement for `NavigationContainer`. The integration builds each screen's path by joining the focused route names from the root navigator to the screen (for example, `/Tabs/Sessions`). Since route names are developer-chosen, the values differ from the file-based segment patterns the Expo Router integration emits, but both follow the same stable, params-free shape.
