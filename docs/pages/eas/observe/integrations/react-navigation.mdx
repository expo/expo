...
> **info** If the integration is disabled or `@react-navigation/native` is not installed, `useObserve()` falls back to the global `Observe.markInteractive`. You can leave the hook in place regardless of integration state.
