...
<Prerequisites>
  ...
  <Requirement title="@react-navigation/native 7 installed in the app">
...
Replace your top-level `<NavigationContainer>` with `<ObserveNavigationContainer>`. It wraps the stock container, accepts the same props, and forwards the same ref, while subscribing to navigation state changes so it can record per-screen render timings.
...
- The integration only activates if `@react-navigation/native` is installed at runtime. If it is not installed, `useObserve()` continues to work but no per-screen navigation metrics are emitted, and rendering `<ObserveNavigationContainer>` throws.
