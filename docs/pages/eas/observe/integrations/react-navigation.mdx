...
For [dynamic configuration](https://reactnavigation.org/docs/dynamic-configuration/), replace your top-level `<NavigationContainer>` with `<ObserveNavigationContainer>`. It wraps the stock container, accepts the same props, and forwards the same ref. It also subscribes to navigation state changes so it can record per-screen render timings.
...
For [static configuration](https://reactnavigation.org/docs/static-configuration/), there is no `NavigationContainer` to replace — `createStaticNavigation()` renders one for you. Instead, create the navigation ref yourself and wrap the returned `<Navigation>` element in `<ObserveNavigationProvider>`, passing it the same ref.
...
const linking: LinkingOptions<ReactNavigation.RootParamList> = {
  prefixes: ['myapp://'],
  // Maps your navigators to URL paths. The integration uses this to resolve a
  // human-readable path (for example, `/sessions/abc`) for each screen.
  config: {
    screens: {
      Home: '',
...
> **info** `ObserveNavigationProvider` does not render a container of its own — it listens to the `navigationRef` you pass and provides per-screen context to the navigators rendered inside it. Pass the same `linking` config to both `<Navigation>` and the provider so the integration can resolve a human-readable path for each screen; without it, screens fall back to their `route.name`.
