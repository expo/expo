...
<Prerequisites>
  ...
  <Requirement title="`expo-router` installed in the app">
    The integration soft-requires `expo-router`. If the package is not installed, the integration
...
## Wrap your root with `ObserveRoot`

`ObserveRoot` includes `AppMetricsRoot` plus the integration context that holds navigation timing state. Replace `AppMetricsRoot.wrap(...)` with `ObserveRoot.wrap(...)`:
...
### Per-route time to first render (`cold_ttr`)

**What it measures:** Time from when a navigation action is dispatched - for example link click - to when the destination screen first becomes focused. For the very first focus after app launch, the measurement is taken from when the js bundle is loaded, and the event includes `isAppLaunch: true`.

Emitted at most once per `(route, screenId)` pair within a session.
...
| `isAppLaunch` | `boolean` | `true` when measured against process start, `false` for later navigations. |

### Per-route warm render (`warm_ttr`)

**What it measures:** Same as `cold_ttr`, but for screens that were already rendered before focus — typically because they were preloaded via [`<Link prefetch />`](/router/basics/navigation/#prefetching) or the back navigation happened.
