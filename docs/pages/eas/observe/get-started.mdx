...
EAS Observe tracks your app's startup performance in production. This guide walks you through installation, setup, and viewing your first metrics.
...
<Collapsible summary="Expo SDK 55 or later">
...
<Collapsible summary="An EAS project">
...
Wrap your root layout with `AppMetricsRoot`. This higher-order component (HOC) automatically measures Time to First Render for you.
...
> **info** `markInteractive()` can safely be called multiple times per session, only the first call records the measurement. If your app has multiple entry screens (for example, an onboarding flow, login flow, or deep link targets), call `markInteractive` on **every one of these screens**. If you only place it on one screen, TTI will not be recorded when the app opens via a deep link to a different screen.
...
In the [EAS dashboard](https://expo.dev), open your project and select **Observe** from the navigation menu to view metrics from your app.
