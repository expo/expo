...
Completed network requests are recorded as trace spans by default. Each span carries the request URL, so you may want to narrow what gets recorded, or turn it off:
...
Pass an object to record only the requests you care about. Hosts match exactly and case-insensitively, so subdomains are not implied, and an omitted field matches every request:
...
This controls recording rather than only export: a request that is turned off or filtered out never reaches the local database. The setting applies to future requests, so spans recorded earlier in the launch are still sent. It also persists across launches, which means requests observed before `configure` runs on the next launch follow the last value you set.

URLs are redacted before they are stored. Userinfo credentials and the query parameters that commonly carry signed-URL secrets are replaced with `REDACTED`.
