...
For all available options and their defaults, see the [`configure()`](/versions/latest/sdk/observe/#configureconfig) and [`ObserveConfig`](/versions/latest/sdk/observe/#observeconfig) API references.

## `dispatchEvents()`

Events are dispatched automatically when the app moves to the background. To flush events manually (for example, during testing or to ensure events are sent before a specific point), call `dispatchEvents()`:
...
For details on the automatic dispatch behavior on each platform, see the [`dispatchEvents()`](/versions/latest/sdk/observe/#dispatchevents) API reference.
