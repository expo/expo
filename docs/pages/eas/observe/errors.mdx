...
> **important** Error reporting in EAS Observe is in [preview](/more/release-statuses/#preview). JavaScript errors require SDK 57 or later. Native crashes require SDK 58 or later. Source maps for EAS Update and more are still to come.
...
The **Crash-free sessions** card shows the share of sessions in the selected time range that finished without a fatal error, together with crash-free users, fatal and non-fatal counts, and the number of affected users.
...
A native crash terminates the app before your JavaScript can react to it, so the report is written on the device and dispatched the next time the app launches. Native crashes are recorded automatically on Android and iOS, and no setup is required. They appear with the **Native** source in the dashboard.
...
On iOS, EAS Observe uses MetricKit to collect the crash reports the system produces for your app. These cover Mach exceptions such as `EXC_BAD_ACCESS` and `EXC_BREAKPOINT`, Unix signals such as `SIGSEGV`, `SIGBUS`, and `SIGTRAP`, and, on iOS 17 and later, uncaught Objective-C and Swift exceptions.
...
- **Symbolication for native crashes**: uploading debug symbols, such as iOS dSYMs and Android ProGuard mappings, is not supported yet.
