...
By default, all installations dispatch their metrics. To dispatch from a fraction of installations instead, set [`sampleRate`](/eas/observe/configuration/#sampling) when calling `configure()`. The decision is deterministic per installation, so an installation is consistently in-sample or out-of-sample across app launches.
