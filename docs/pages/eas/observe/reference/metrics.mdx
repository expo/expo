...
Instead of calling `markInteractive()` from an effect, you can render the `<InteractiveMarker />` component at the point your screen becomes interactive, such as once its initial data has loaded. It calls `markInteractive()` once when it mounts and renders nothing.

> **info** `InteractiveMarker` is available in SDK 56 and later.
