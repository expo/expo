...
- **Error reporting** (in preview): [Record JavaScript errors and native crashes](/eas/observe/errors/) and investigate their stack traces in the dashboard
...
| Track native crashes in production (in preview)     | <YesIcon />    |
...
**Native crashes**: On SDK 57 with `expo-observe` 57.0.21 and later, EAS Observe records native crashes on Android and iOS and shows them in the dashboard alongside JavaScript errors. Native stack traces are not symbolicated in the dashboard. See [Error reporting](/eas/observe/errors/) for details.
