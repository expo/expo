...
> **important** **Expo Observe** is in [preview](/more/release-statuses/#preview) and subject to breaking changes. While in preview, it is free to use.
...
**Custom analytics and event tracking**: Observe will have some support for custom events in the future. For a first-class solution, choose an analytics provider from the [React Native analytics guide](/guides/using-analytics/), for example PostHog, Amplitude, or Firebase Analytics.
