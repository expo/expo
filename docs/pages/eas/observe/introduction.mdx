...
By default, metrics collected from debug builds are dropped without being dispatched. You can dispatch them anyway for testing by setting `dispatchInDebug` to `true` via [`configure()`](/eas/observe/configuration/#configure). See [Configuration](/eas/observe/configuration/#enable-metrics-in-development) for details.
