...
description: EAS Workflows is React Native CI/CD service for automating builds, updates, submissions, and tests for React Native and Expo apps.
hasVideoLink: true
---
...
EAS Workflows run in managed cloud environments with pre-packaged job types designed specifically for mobile app development. Teams can trigger workflows from GitHub events (push, pull request, labels), schedules (cron), or manually via the EAS CLI.
...
- **Faster iteration**: Use fingerprint-based builds to skip redundant native builds and send OTA updates instead
...
- **Fingerprint-based builds**: Calculate fingerprints of your project to skip redundant native builds
  **Repack**: Repack builds into a single binary for faster distribution
...
Yes. Workflows support [EAS environment variables](/eas/environment-variables/) (production, preview, development) and inline `env` values. Environment variables can be referenced using `${{ env.VARIABLE_NAME }}` syntax.

</Collapsible>

<Collapsible summary="What are the current limitations?">

No shared workflow configurations (each workflow must be defined independently) - No matrix builds (cannot run multiple variations with different configurations in parallel). See [Limitations](/eas/workflows/limitations) for more details and updates.
...
<BoxLink
  ...
  description="Use ready-to-use jobs to build, submit, update, and test your app."
