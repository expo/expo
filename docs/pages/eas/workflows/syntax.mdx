...
jobs:
  my_job:
    ...
    params:
      ...
      profile: string # optional, default: production
      groups: string[] # (iOS only) optional, array of TestFlight internal group names to add the build to
```
...
> **Note:** The `groups` parameter is only used for iOS submissions. It allows you to specify which [TestFlight internal groups](https://developer.apple.com/help/app-store-connect/test-a-beta-version/add-internal-testers) the build should be added to after submission. The group names must match those configured in App Store Connect. If omitted, the build will not be automatically added to any internal groups.
