...
> **Note:** The `groups` parameter is only used for iOS submissions. It allows you to specify which [TestFlight internal groups](https://developer.apple.com/help/app-store-connect/test-a-beta-version/add-internal-testers) the build should be added to after submission. On top of the groups you provide as `groups` parameter, the build will be automatically added to the groups that have been created with "Enable automatic distribution" App Store Connect setting.
