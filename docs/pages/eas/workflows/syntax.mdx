...
You can customize behavior of workflow — commands executed, control flow, environment variables, build profile, app version — based on the workflow run context.

### `after` and `needs`

If you specify a job in `after` or `needs`, you can use its outputs in a downstream job. All jobs expose a `status` property. Most pre-packaged jobs also expose some outputs. You can [set outputs in custom jobs using the `set-output` function](#jobsjob_idoutputs).

```json
{
  "status": "success" | "failure" | "skipped",
  /* @info Every job outputs a different set of outputs. See the specific job's documentation for details. */
  "outputs": {}
  /* @end */
}
```
...
To ease the migration from GitHub Actions to EAS Workflows we expose some context you may find useful.
...
`fromJSON()` is `JSON.parse`, `toJSON` is `JSON.stringify`. You can use them to save / process JSON output.
...
jobs:
  ...
  print_debug:
    ...
    steps:
      - name: Print debug
        run: |
          echo "Updates published: ${{ fromJSON(needs.publish_update.outputs.updates_json)[0].group }}, ${{ fromJSON(needs.publish_update.outputs.updates_json)[1].group }}"
```
...
```json
{
  "first_update_group_id": string | null,
...
Outputs of the jobs are later accessible in [interpolation context](#Interpolation) under `needs.<job_id>.outputs.<output_name>` or `after.<job_id>.outputs.<output_name>`.
