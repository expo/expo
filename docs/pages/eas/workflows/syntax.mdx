...
Specifies the VM image to use for the job. See [Infrastructure](/build-reference/infrastructure/) for available images. For macOS images, each image's Details list the default iOS Simulator runtime and pre-warmed Simulator; see [iOS Simulators](/build-reference/infrastructure/#ios-simulators) for how to target a different runtime or device.
