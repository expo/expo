...
You can customize the behavior of a workflow &mdash; commands executed, control flow, environment variables, build profile, app version &mdash; based on the workflow run context.
...
`fromJSON()` is `JSON.parse`, `toJSON` is `JSON.stringify`. You can use them to consume or produce JSON outputs.
...
```json
{
  "first_update_group_id": string, // ID of the first update group. You can use it to e.g. construct the update URL for development client deep link.
