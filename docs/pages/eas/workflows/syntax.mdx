...
Specifies the VM image to use for the job. See [Infrastructure](/build-reference/infrastructure/) for available images. For macOS jobs, that page lists each runner image's default iOS Simulator runtime and pre-warmed device model under each image tag. See [iOS Simulators](/build-reference/infrastructure/#ios-simulators) for how to target a different runtime or device.
