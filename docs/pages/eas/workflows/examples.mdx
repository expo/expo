...
To get started, you'll need to configure your project and devices to build and run development builds.

<BoxLink
  title="Development build prerequisites"
  description="Get your project ready for development builds."
  href="/develop/development-builds/create-a-build/#prerequisites"
  Icon={BookOpen02Icon}
/>
...
```yaml .eas/workflows/development-build.yml
...
<Terminal cmd={['$ eas workflow:run .eas/workflows/development-build.yml']} />
...
Your project needs have [EAS Update](/eas-update/introduction/) setup to publish preview updates. You can set up your project with:

<Terminal cmd={['$ eas update:configure']} />

After that, you'll need to create new [development builds](/develop/development-builds/introduction/).
...
When you're ready to deliver changes to your users, you can build and submit to the app stores or you can send and over-the-air update. The following workflow detects if you need new builds, and if so, it sends them to the app stores. If new builds are not required, it will send an over-the-air update.
