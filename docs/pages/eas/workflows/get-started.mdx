...
export default function App() {
  ...
  const addDebt = () => {
    if (name && amount) {
      setList([...list, { id: Date.now().toString(), name, amount, paid: false }]);
      setName('');
      setAmount('');
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>مارکیت ابو عبدالله</Text>
      ...
      <Button title="زیادکردن" onPress={addDebt} />

      <FlatList
        data={list}
        renderItem={({ item }) => (
          <Text
            style={[styles.item, { color: item.paid ? 'green' : 'red' }]}
            onPress={() => togglePaid(item.id)}
          >
            {item.name} - {item.amount} دینار ({item.paid ? 'پارەدراوە' : 'قەرزە'})
          </Text>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  container: { padding: 20, marginTop: 40, backgroundColor: '#fff8dc', flex: 1 },
  title: { fontSize: 22, marginBottom: 10, color: '#b8860b' },
  input: { borderWidth: 1, marginBottom: 10, padding: 8, borderColor: '#b8860b' },
