...
| groups    | string[] | Optional. Array of TestFlight internal group names to add the build to (only affects iOS submissions; for Android, this param is ignored). Overrides `groups` in the [submit profile](/eas/json/#ios-specific-options-1). Note: on top of the groups you provide here, the build will be automatically added to the groups that have been created with "Enable automatic distribution" App Store Connect setting. |
