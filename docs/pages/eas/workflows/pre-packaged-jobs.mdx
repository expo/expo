...
If you need certain environment variables during the build process, you can include them in your
[eas.json](/eas/json/#environment), for your specified build `profile`. They will be pulled from
[EAS environment variables](/eas/environment-variables/).
...
You can pass a list of environment variables into the `env` section. These environment variables will be pulled from
[EAS environment variables](/eas/environment-variables/). The passed `environment` parameter will be used for the
Environment variable's environment, useful for when the same environment variable is defined for different environments.
