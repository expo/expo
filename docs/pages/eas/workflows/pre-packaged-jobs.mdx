...
| groups    | string[] | Optional. Array of TestFlight internal group names to add the build to (only affects iOS submissions; for Android, this param is ignored). Note: on top of the groups you provide here, the build will be automatically added to the groups that have been created with "Enable automatic distribution" App Store Connect setting. |
