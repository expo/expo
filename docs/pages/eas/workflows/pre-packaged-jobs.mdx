...
| private_key_path | string | Optional. Path to a private key file or an [EAS Environment variable](/eas/environment-variables/) containing the private key to use for code signing the update. This gets passed as `--private-key-path` to the EAS CLI.                                          |
