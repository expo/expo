...
| `build_info`                           | Get details of a specific EAS build                                  | "get the status of my latest iOS build"              | Server                                 |
| `build_list`                           | List EAS builds for a project                                        | "list the recent builds for this project"            | Server                                 |
| `build_logs`                           | Fetch logs for a completed EAS build                                 | "show me the logs for the failed build"              | Server                                 |
| `build_run`                            | Trigger a new EAS build from a git reference                         | "run a production build for iOS"                     | Server                                 |
| `build_cancel`                         | Cancel an EAS build that is queued or in progress                    | "cancel the build that is currently in progress"     | Server                                 |
| `build_submit`                         | Submit an EAS build to app stores                                    | "submit the latest build to the App Store"           | Server                                 |
| `workflow_create`                      | Create a new EAS workflow YAML file or learn workflow syntax         | "create a CI/CD workflow for building and deploying" | Server                                 |
| `workflow_info`                        | Get details of a specific EAS workflow run                           | "get the status of the latest workflow run"          | Server                                 |
| `workflow_list`                        | List recent EAS workflow runs for a project                          | "list the recent workflow runs"                      | Server                                 |
| `workflow_logs`                        | Fetch logs for a specific job in an EAS workflow run                 | "show me the logs for the build job in the workflow" | Server                                 |
| `workflow_run`                         | Trigger an EAS workflow run from a git reference                     | "run the build-and-deploy workflow"                  | Server                                 |
| `workflow_cancel`                      | Cancel a running EAS workflow                                        | "cancel the running workflow"                        | Server                                 |
| `workflow_validate`                    | Validate EAS workflow YAML syntax and configuration                  | "validate my workflow file"                          | Server                                 |
