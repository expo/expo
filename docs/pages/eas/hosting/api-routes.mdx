...
A crash is a statement that throws in the program execution, for example,`throw new Error("An error!")`. Crashes may be viewed on [Hosting Crashes](https://expo.dev/accounts/[accountName]/projects/[projectName]/hosting/crashes) page.
...
All logs from API routes (`console.log`, `console.info`, `console.error` and so on) are recorded on the deployment level logs page. [Hosting Deployments](https://expo.dev/accounts/[accountName]/projects/[projectName]/hosting/deployments) > _select a deployment_ > **Logs**.
