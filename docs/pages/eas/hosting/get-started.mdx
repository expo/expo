...
If using an existing project, ensure that in your project's app config, the web output ([`expo.web.output`](/versions/latest/config/app/#output)) set to either `static` or `server` (single page apps are not supported on EAS Hosting).
