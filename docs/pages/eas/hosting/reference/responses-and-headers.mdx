...
**EAS Hosting** will add a couple of headers to every request, before they're forwarded to your API routes. These headers generally add more information about who made the request.
...
For example, while a client may make a request to an alias URL such as `https://my-app--staging.expo.app/`, the worker deployment that may receive the request will have a URL containing its deployment ID, such as `https://my-app--or1170q9ix.expo.app/`.
...
export async function GET(request) {
  const url = request.url; // 'https://my-app--or1170q9ix.expo.app/'
  const origin = request.headers.get('Origin'); // 'https://my-app--staging.expo.app/'
  const host = request.headers.get('X-Forwarded-Host'); // 'my-app--staging.expo.app'
}
```
...
- `X-Forwarded-For` only contains a comma-separated list of IP addressed. Each entry in the list represents a proxy that forwarded the request as well.
