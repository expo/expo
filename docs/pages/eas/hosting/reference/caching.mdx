...
- `public` &mdash; Indicates any cache (including EAS Hosting) may store the response. Without it, it's implied that the response is not shared between multiple requests.
- `private` &mdash; Indicates the response is intended for a single user and may only be cached by a browser.
- `no-store` or `no-cache` &mdash; Indicates that this response may never be cached or stored.
