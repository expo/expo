...
## Account-scoped environment variables

Account-scoped environment variables are environment variables that are available across all of your projects in your EAS account.
...
They are available to your build jobs, workflow jobs and updates, together with project-scoped variables for this project.
...
The environment variables defined on EAS servers can be assigned to mutiple environments and have the same value across all of them, or be created for a single environemnt, so they can have different values for each environment.
...
| Secret     | Not readable outside of the EAS servers, including on the website and in EAS CLI, obfuscated in logs.                        |
...
One of the differences between using environment variables in Expo framework and EAS is that, in oposition to Expo CLI, EAS CLI itself does not support loading **.env** files to set environment variables when resolving the expo config used to create your build jobs or updates.
