...
> **info** On **Production** and **Enterprise** plans, you can create custom environments by clicking the plus (`+`) icon next to Environments when creating an environment variable. Custom environment names can contain letters, digits, underscores and hyphens, and must be between 3-100 characters. Once created, custom environments will be available for all subsequent environment variables in your project.
...
To set the environment for your EAS Build jobs, you can use the `environment` option in **eas.json** and set it to one of the available environments. By default, projects have `development`, `preview`, and `production` environments. On **Production** and **Enterprise** plans, you can also use custom environments that you've created.
...
- `EAS_BUILD_PROJECT_ID`: the EAS app (project) ID, for example, `bd2f7e21-1ee7-47f2-8357-d7c4b50622fb`
...
- You can create up to 150 account-wide environment variables per environment and 200 project-specific environment variables per environment.
- Custom environments (available on **Production** and **Enterprise** plans) are limited to 10 per project. Custom environment names can contain letters, digits, underscores and hyphens, and must be between 3-100 characters.
