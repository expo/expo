...
また、`captureRef()` メソッドはスクリーンショットの URI で解決される Promise を返します。この URI を [`MediaLibrary.Asset.create()`](/versions/v57.0.0/sdk/media-library/#createfilepath-album) のパラメータとして渡し、デバイスのメディアライブラリにスクリーンショットを保存します。
