...
<Prerequisites>
  <Requirement title="実機での Expo Go">
    Android または iOS の実機に [Expo Go](https://expo.dev/go) をインストールします。iOS
    の実機では、無料の [Expo アカウント](https://expo.dev/signup) も必要で、Expo CLI と Expo Go
    の両方でそのアカウントにサインインする必要があります。
  </Requirement>
  ...
  <Requirement title="ターミナルが使える開発マシン">
    macOS、Linux、または Windows (PowerShell および [WSL2](https://expo.fyi/wsl))
    でターミナルウィンドウを開けるマシン。
  </Requirement>
</Prerequisites>
...
`reset-project` スクリプトは、プロジェクト内の **src/app** ディレクトリ構造をリセットし、プロジェクトの **src** ディレクトリにあった以前のボイラープレートファイルを、**example** という別のサブディレクトリに移動します。これはメインアプリの構造の一部ではないため、削除して構いません。
...
> **info** iOS の実機では、Expo CLI と Expo Go の両方が同じ Expo アカウントでサインインしている場合にのみ、Expo Go はプロジェクトを開きます。コンピュータで `npx expo login` を実行し、同じアカウントで Expo Go にサインインしてください。エラーが解決しない場合は、[Expo Go のサインインが必要](/troubleshooting/expo-go-sign-in-required/)を参照してください。
...
import { Text, View, /* @tutinfo スタイルを定義するため `StyleSheet` をインポートします。 */ StyleSheet/* @end */ } from 'react-native';

export default function Index() {
  return (
    <View style={styles.container}>
      /* @tutinfo 以前は "Edit src/app/index.tsx to edit this screen" と表示されていましたが、現在は "Home screen" と表示されます。 */
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    /* @tutinfo `backgroundColor` プロパティに `'#25292e'` の値を設定します。*/
...
<Collapsible summary="青い歯車アイコンとは何ですか？">

青い歯車アイコンは、Android と iOS プラットフォームの **Tools button** です。これは [開発者メニュー](/debugging/tools/#developer-menu) を開くフローティングショートカットで、アプリのリロードや Fast Refresh の切り替えなどのデバッグ機能を利用できます。開発中にのみ表示され、本番ビルドには含まれません。

このチュートリアルの以降のスクリーンショットでは、このアイコンを非表示にします。非表示にするには:

1. 開発者メニューを開きます。デバイスを振るか、エミュレーター、シミュレーター、または USB 接続したデバイスを使用している場合は、開発サーバーを実行しているターミナルで <kbd>M</kbd> を押します。
2. **Tools button** の項目を見つけて、オフに切り替えます。
