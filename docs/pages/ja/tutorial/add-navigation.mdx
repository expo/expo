...
export default function RootLayout() {
  return (
    /* @tutinfo <CODE>Stack</CODE> ナビゲーターの内部に画面をラップします。<CODE>/index</CODE> ルートに <CODE>options</CODE> プロパティを追加します。<CODE>/about</CODE> ルートを追加するため、もう一つの <CODE>Stack.Screen</CODE> コンポーネントを追加します。 */
    <Stack>
...
import { Text, View, StyleSheet } from 'react-native';
/* @tutinfo <CODE>expo-router</CODE> から <CODE>Link</CODE> コンポーネントをインポートします。 */ import { Link } from 'expo-router'; /* @end */

export default function Index() {
  return (
    <View style={styles.container}>
      <Text style={styles.text}>Home screen</Text>
      /* @tutinfo <CODE>Text</CODE> コンポーネントの後に <CODE>Link</CODE> コンポーネントを追加し、<CODE>href</CODE> プロパティに <CODE>/about</CODE> ルートを渡します。 */
...
const styles = StyleSheet.create({
  ...
  /* @tutinfo <CODE>Link</CODE> コンポーネントに <CODE>fontSize</CODE>、<CODE>textDecorationLine</CODE>、<CODE>color</CODE> のスタイルを追加します。 */
  button: {
    fontSize: 20,
...
export default function NotFoundScreen() {
  return (
    <>
      /* @tutinfo <CODE>+not-found</CODE> ルートのタイトルを更新するため、<CODE>options</CODE> プロパティを持つ <CODE>Stack.Screen</CODE> コンポーネントを追加します。 */
...
export default function RootLayout() {
  return (
    <Stack>
      /* @tutinfo これは、特に Root レイアウトが親のスタックナビゲーターで構成されている場合に、タブナビゲーターをスタックナビゲーターの内部にネストする方法です。また、タブナビゲーターのヘッダーを非表示にするため、<CODE>headerShown</CODE> オプションを <CODE>false</CODE> に設定しています。そうしないと、各画面に 2 つのヘッダーが表示されてしまいます。 */
...
export default function TabLayout() {
  return (
    <Tabs>
      /* @tutinfo <CODE>Tabs.Screen</CODE> コンポーネントは <CODE>Stack.Screen</CODE> コンポーネントと同様に動作し、同じプロパティを受け取ります。唯一の違いは、デバイス上のナビゲーションパターンです。 */
...
```tsx app/(tabs)/_layout.tsx
import { Tabs } from 'expo-router';
/* @tutinfo <CODE>Ionicons</CODE> アイコンセットをインポートします。*/
...
export default function TabLayout() {
  return (
    <Tabs
      /* @tutinfo タブバーをカスタマイズするために使用できる <CODE>screenOptions</CODE> は多数あります。ここではアクティブなタブの色をカスタム値に変更しており、この値はアプリの後の部分でも使用します。 */
      screenOptions={{
      ...
      <Tabs.Screen
        ...
        options={{
          ...
          tabBarIcon: ({ color, focused }) => (
            /* @tutinfo <CODE>focused</CODE> パラメーターを使用すると、タブがアクティブまたは非アクティブのときのアイコンとラベルの動作を変更できます。*/
