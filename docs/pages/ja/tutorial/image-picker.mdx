...
**src/app/(tabs)/index.tsx** で `expo-image-picker` ライブラリをインポートし、`Index` コンポーネント内に `pickImageAsync()` 関数を作成します。

{/* prettier-ignore */}
```tsx src/app/(tabs)/index.tsx|collapseHeight=440
// ...rest of the import statements remain unchanged
/* @tutinfo `ImagePicker` をインポートします。 */ import * as ImagePicker from 'expo-image-picker';/* @end */

export default function Index() {
  /* @tutinfo `launchImageLibraryAsync()` に画像ピッカーのオプションを渡します */
...
primary ボタンを押したときに、`Button` コンポーネントから `pickImageAsync()` 関数を呼び出します。**src/components/button.tsx** で `Button` コンポーネントの `onPress` プロパティを更新します。
...
**src/app/(tabs)/index.tsx** で、1 つ目の `<Button>` の `onPress` プロパティに `pickImageAsync()` 関数を追加します。
...
**src/app/(tabs)/index.tsx** ファイルを変更します。
...
import { View, StyleSheet } from 'react-native';
import * as ImagePicker from 'expo-image-picker';
/* @tutinfo `react` から `useState` フックをインポートします。*/
...
export default function Index() {
  ...
  const pickImageAsync = async () => {
    ...
    if (!result.canceled) {
      /* @tutinfo `assets` 配列から最初の uri を取得します。また、一度に選択される画像は 1 つだけなので、ここを変更する必要はありません。 */
  ...
  return (
    <View style={styles.container}>
      <View style={styles.imageContainer}>
        /* @tutinfo 選択された画像の URI を `ImageViewer` コンポーネントに渡します。 */
...
1. **src/components/image-viewer.tsx** ファイルを変更して、`selectedImage` プロパティを受け取れるようにします。
...
export default function ImageViewer({ imgSource, /* @tutinfo `selectedImage` プロパティを渡します。*/selectedImage/* @end */ }: Props) {
  /* @tutinfo 選択された画像が null でない場合はデバイスからの画像を表示し、そうでない場合はプレースホルダー画像を表示します。 */
  const imageSource = selectedImage ? { uri: selectedImage } : imgSource;
  /* @end */

  /* @tutinfo `imgSource` を `imageSource` に置き換えます。 */
