...
次に、**src/components/button.tsx** で 2 つ目のボタンをレンダリングする際の `Button` コンポーネントから `alert` を削除し、`onPress` プロパティを更新します。
...
**src/components** ディレクトリ内に、次のコードで新しい **circle-button.tsx** ファイルを作成します。
...
他の 2 つのボタンも `<MaterialIcons>` を使い、縦に並んだテキストラベルとアイコンを表示します。**src/components** ディレクトリ内に **icon-button.tsx** という名前のファイルを作成します。このコンポーネントは 3 つのプロパティを受け取ります。
...
モーダルでは、利用可能な絵文字のリストからユーザーが絵文字を選択できます。**src/components** ディレクトリ内に **emoji-picker.tsx** ファイルを作成します。このコンポーネントは 3 つのプロパティを受け取ります。
...
{/* prettier-ignore */}
```tsx src/components/emoji-picker.tsx|collapseHeight=430
...
それでは、**src/app/(tabs)/index.tsx** を変更しましょう。
...
**src/components** ディレクトリ内に **emoji-list.tsx** ファイルを作成し、次のコードを追加します。
...
次に、**src/app/(tabs)/index.tsx** を更新して `<EmojiList>` コンポーネントをインポートし、`<EmojiPicker>` コンポーネント内のコメントを次のコードスニペットに置き換えます。

{/* prettier-ignore */}
```tsx src/app/(tabs)/index.tsx|collapseHeight=440
...
次に、画像の上に絵文字ステッカーを配置します。**src/components** ディレクトリに新しいファイルを作成し、**emoji-sticker.tsx** という名前にします。そして次のコードを追加します。

```tsx src/components/emoji-sticker.tsx|collapseHeight=300
...
このコンポーネントを **src/app/(tabs)/index.tsx** ファイルでインポートし、`Index` コンポーネントを更新して画像上に絵文字ステッカーを表示します。`pickedEmoji` 状態が `undefined` でないかをチェックします。
...
import EmojiPicker from '@/components/emoji-picker';
import EmojiList from '@/components/emoji-list';
/* @tutinfo `EmojiSticker` コンポーネントをインポートします。 */import EmojiSticker from '@/components/emoji-sticker';/* @end */
