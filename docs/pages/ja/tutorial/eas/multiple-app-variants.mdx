...
**app.json** はアプリ関連の設定を JSON ファイルに持ちます。これは静的なので、[一部のプロパティに動的な値を使いたい](/ja/workflow/configuration/#動的な設定) 場合には向きません。ここでは [環境変数](/ja/workflow/configuration/#環境に応じて設定を切り替える) に基づいて、すべてのビルドプロファイルごとに異なる Android Application ID と iOS Bundle Identifier を設定します。
