...
This guide walks through archiving an iOS app with Xcode and uploading it to [App Store Connect](https://appstoreconnect.apple.com/), which is useful if you aren't using EAS, or as a fallback if EAS Submit is temporarily unavailable. If you already have a **.ipa** file, skip to [Upload an existing build with Transporter](#upload-an-existing-build-with-transporter). For most teams, [EAS Submit](/submit/ios/) is simpler and works from any OS.
