...
## TestFlight internal groups (`groups`)

You can use the `groups` property in the iOS submit profile to specify which [TestFlight internal groups](https://developer.apple.com/help/app-store-connect/test-a-beta-version/add-internal-testers) the build should be added to after submission. This is useful for distributing your app to specific testers within your team.

The value should be an array of group names (as strings) that match the internal groups configured in App Store Connect.
...
> **Note:** If you do not specify `groups`, the build will not be automatically added to any internal TestFlight groups. You can still add it manually in App Store Connect.

> **Additional details:**
>
> - The submitted build will be automatically added to internal testers' groups that have the App Store Connect setting to receive new builds automatically enabled.
> - Only internal group names are supported right now; external groups are not supported.
