...
<Prerequisites numberOfRequirements={4}>
  ...
  <Requirement number={2} title="Install EAS CLI and authenticate with your Expo account">
    Install EAS CLI login with your Expo account:
    
    <Terminal cmd={['$ npm install -g eas-cli && eas login']} />
  </Requirement>
  <Requirement number={3} title="Include a bundle identifier in app.json">
...
<Collapsible summary="How to find appleId and appleTeamId">
  1. Sign in to [App Store Connect](https://appstoreconnect.apple.com/) and select your team.
  2. Click on your account name in the top right corner and select, then select your team in the dropdown menu that appears.
  3. In the **General** tab, you'll find your `appleId` under **Apple account** and your `appleTeamId` under **Team ID**.

  <ContentSpotlight
    alt="Apple ID and Apple Team ID in App Store Connect"
    src="/static/images/eas-submit/finding-appleId-and-appleTeamId.png"
...
<Prerequisites numberOfRequirements={5}>
  ...
  <Requirement number={3} title="Configure your App Store Connect API Key">
    ...
    3. Select "App Store Cononect: Manage your API Key"
...
Once you have completed all the prerequisites, you can set up a CI/CD pipeline to submit your app to the Apple App Store. You can use [EAS Workflows](/eas-workflows/get-started/) to build and submit your app automatically.
...
2. Inside **submit-ios.yml**, you can use the following workflow to kick off a job that submits an iOS app:
   ...
   jobs:
     ...
     submit_ios:
       ...
       params:
         platform: ios
         build_id: ${{ jobs.needs.build_ios.outputs.id }}
     # @end #
   ```
