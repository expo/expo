...
<Prerequisites numberOfRequirements={6}>
  ...
  <Requirement number={3} title="Upload your app manually at least once">
    Google Play Store requires every app to be uploaded manually through Play Console one time before it will accept API-based submissions.

    <BoxLink
      title="Manually submit an Android app for the first time"
      description="Follow the step-by-step guide to create your first Play Console release."
      href="/submit/android-manual/"
      Icon={GoogleAppStoreIcon}
    />

  </Requirement>
  <Requirement number={4} title="Create a Google Service Account key">
  ...
  <Requirement number={5} title="Include a package name in app config">
  ...
  <Requirement number={6} title="Install EAS CLI and authenticate with your Expo account">
