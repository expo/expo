...
<Prerequisites numberOfRequirements={8}>
  ...
  <Requirement number={6} title="Add draft release">
    Add a `releaseStatus`(/eas/json/#android-specific-options-1) in the `production` submission profile in **eas.json**:
...
<Prerequisites numberOfRequirements={8}>
  ...
  <Requirement number={6} title="Provide a submission profile in eas.json">
    Then, you'll need to provide a submission profile in **eas.json** that includes your app's `serviceAccountKeyPath` and `releaseStatus`:
