...
A splash screen and an app icon are fundamental elements of a mobile app. They play an important role in the user experience and branding of the app. This guide provides instructions on how to create and add both to your app.
...
Android screen sizes vary greatly with the massive variety of devices on the market. To deal with this, take a look at [Material Design blog post](https://material.io/blog/device-metrics) for more information on how to find the right device metrics for any screen.
...
You can test your new splash image by starting the development sever with `npx expo start` and using a development build or Expo Go to preview it.
...
You can update the value of this property to be the same as your splash image background color. For example:
...
After creating an app icon, export it as **.png** and save it in the **assets/images** directory. By default, Expo uses **icon.png** as the file name. If you decide to its file name, make sure to use that in the next step.
...
- Use a **.png** file.
- 1024x1024 is a good size. If you have an Expo project created using `npx create-expo-app`, [EAS Build](/build/setup) will generate the other sizes for you. If you are using a bare workflow, generate the icons on your own. The largest size EAS Build generates is 1024x1024.
