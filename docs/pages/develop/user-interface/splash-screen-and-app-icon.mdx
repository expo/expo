...
The [expo-splash-screen module](/versions/latest/sdk/splash-screen) has a built-in [config plugin](/config-plugins/introduction) that lets you configure properties such as the splash icon and background color. Learn more about [configurable splash screen properties](/versions/latest/sdk/splash-screen/#configurable-properties).
...
<Collapsible summary="Splash screen icon recommendations">

- Use a 1024x1024 image.
- Use a **.png** file.
- Use a transparent background.
- Use the [Figma template](https://www.figma.com/community/file/1466490409418563617) to create your splash screen icon.
...
Open the app config file, under plugins set the corresponding properties as follows:
...
[expo-splash-screen](/versions/latest/sdk/splash-screen) also supports `ios` and `android` properties for configuring the splash screen for a specific platform. See the following example:
...
- Use `ios.icon` to specify different icons for various system appearances (e.g., dark, tinted) can be provided. If specified, this overrides the top-level icon key. See the example below:
