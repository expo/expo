  ...
  "expo": {
    "plugins": [
      [
        ...
        {
          "fonts": [{
            "path": "./assets/fonts/inter_black.otf",
            "family": "Inter",
            "weight": 900
          }]
        }
...
The `fonts` option takes an array of one or more font objects to link to the native project. The path to each font file is relative to the project's root.

[Create a new native build](/develop/development-builds/create-a-build/), and you can now use the font in your project with the `fontFamily` style prop. On Android, the font family name will be the name of the provided "family" key in the font option. On iOS, the font family name and weight will be read from the font file itself. We recommend naming the family key the same as the font family so the family name is consistent on both platforms.
...
Note that weights are not supported when loading fonts at runtime thus needing to provide a different family name for every font weight.
...
```json app.json
{
  "plugins": [
    [
      ...
      {
        "fonts": [{
            "path": "node_modules/@expo-google-fonts/inter/Inter_100Thin.ttf",
            "family": "Inter",
            "weight": 100
          }]
      }
