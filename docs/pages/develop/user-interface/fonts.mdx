...
The map syntax used in the example above treats each key as an independent font family, so on
web there's no way for the browser to tell that `'Inter-Bold'` and `'Inter-BoldItalic'` are
related faces of the same typeface. To load several font files (for example regular, bold, and
italic cuts) under a single `fontFamily` name and let the browser select the correct face using
the CSS `font-weight` and `font-style` properties, pass `useFonts` an array of
[`FontFamilyDefinition`](/versions/latest/sdk/font/#fontfamilydefinition)s instead of a map:
...
> **Note:** `weight` and `style` only affect the `@font-face` CSS generated on web. On Android and
> iOS, only the first `fontDefinitions` entry for a given `fontFamily` is loaded &mdash; to select
> different weights and styles natively, use separate `fontFamily` names, or embed them with the
> [config plugin's per-platform `fontDefinitions`](#with-expo-font-config-plugin) instead.
