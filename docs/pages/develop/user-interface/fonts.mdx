...
Run `npx expo prebuild --clean` to apply the config plugin. Start a new build of the app (`npx expo run:[android|ios]`) &mdash; the font files are embedded into the app binary during build. Install the app on your device or Android Emulator or iOS Simulator.
