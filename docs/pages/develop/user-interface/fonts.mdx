...
A variable font holds every weight in one file, along a `wght` axis. `fontWeight` renders the weight you ask for on web and Android:

- On web, the browser applies the axis.
- On Android, the font is instanced at each weight. This requires Android 8.0 (API level 26) or higher when you embed the font with the [`expo-font` config plugin](/versions/latest/sdk/font/#configuration-in-app-config), and Android 10 (API level 29) or higher when you load it with the [`useFonts`](/versions/latest/sdk/font/#usefontsmap) hook. Earlier versions synthesize bold from the font's default instance instead of instancing the axis.

On iOS, embed variable fonts with the config plugin: the system then registers every instance the font declares. Reference the font by its family name so that all of them are available. See [how to determine which font family name to use](#how-to-determine-which-font-family-name-to-use). If you load a variable font at runtime and its weights don't render, embed it with the config plugin instead, or use a separate static font file for each weight.

When you embed a variable font with the config plugin, list the same font file once per weight under `fontDefinitions` so that every weight you need is available.

Axes other than `wght`, such as `wdth` or `opsz`, render at the font's default value. To use a specific axis configuration, use a utility such as [fontTools](https://fonttools.readthedocs.io/en/latest/varLib/mutator.html) to extract it from the variable font and save it as a separate font file.
