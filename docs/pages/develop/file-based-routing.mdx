...
Expo Router is a navigation library for React Native and web applications. It allows you to manage navigation between screens in your app and use the same components on multiple platforms (Android, iOS and web). It uses a file-based method to determine routes inside your app. It also provides native navigation and is built on top of [React Navigation](https://reactnavigation.org/).
...
Files named **index** match the parent directory and do not add a path segment. For example, if you expand the app's file structure by adding **app/settings/index.js**, it will match the `/settings` route.
...
Traditionally, React Native projects are structured with a single root component (defined as **App.js** or **index.js**). Similarly, the first layout file (**\_layout.tsx**) inside the **app** directory is considered to be a single root component.
...
To allow navigation between two routes (`/` and `/details`), update the Root Layout file and a `Stack` component to it:
...
A group is created to organize similar routes or a section of the app. Each group has its own layout file and the grouped directory requires a name inside parentheses `(group)`.
