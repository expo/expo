...
<BoxLink title="Expo Router API Routes" href="/router/server/api-routes" Icon={BookOpen02Icon} />
...
[Expo Router API Routes](/router/server/api-routes) allow you to write server-side logic directly inside your Expo app. You can define functions that handle requests just like an Express or Next.js backend, no need for an external server.
