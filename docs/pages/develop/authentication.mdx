...
The biggest advantage of these services is how easy they are to integrate. They usually offer clear documentation, starter kits, and prebuilt components that save you time.

Also worth noting: adding email and password authentication is usually enough to pass App Store and Play Store review. You can submit your app with this method first. If you include “Sign in with Google,” Apple may reject your app unless you also support “Sign in with Apple.” The same rule applies in reverse on Google Play.
...
[React Navigation](https://reactnavigation.org/) also supports deep linking, but you will need to configure it manually. See their [Deep Linking guide](https://reactnavigation.org/docs/deep-linking/) for more details.
...
> **info** Magic links and passcodes are both valid authentication methods for App Store and Play Store review. You can submit your app with either of these methods as the only option and get approved, even before adding social or OAuth login options.
...
[OAuth 2.0](https://oauth.net/2) is a widely used, secure protocol that allows your app to access user information from another service, without needing to handle passwords. It allows your users to to log in with a single tap, which saves time, builds trust, and removes the need to manage passwords.
...
If you are looking for full control or want to understand how OAuth works under the hood, keep reading. This section shows how to implement a complete OAuth flow yourself using **Expo**.
...
<ContentSpotlight src="/static/images/develop/authentication/oauth-flow.png" />

> **info** In the above diagram, the term "client" does not imply any particular implementation characteristics (for example, whether the application executes on a server, a desktop, mobile app, or other devices).
...
<ContentSpotlight src="/static/images/develop/authentication/api-routes-oauth-flow.png" />

Expo makes it possible to implement the full OAuth flow directly inside your app using:

- [Expo Router](/router/introduction)
- [Expo API Routes](/router/reference/api-routes)
- [Expo AuthSession](/versions/latest/sdk/auth-session) for the client side

> **info** Some providers offer native APIs to handle the sign-in flow directly within the app. <br/>
> Google offers a native Sign in with Google experience on Android. If you're looking for a native implementation, see the [Google Authentication guide](/guides/google-authentication). <br/>
> Apple provides Sign in with Apple, which uses a native bottom sheet and Face ID on iOS. See [Expo Apple Authentication](/versions/latest/sdk/apple-authentication) to learn more.

This setup gives you full control over the login experience across Android, iOS, and Web.

#### What are Expo API Routes?

[Expo API Routes](/router/reference/api-routes) allow you to write server-side logic directly inside your Expo app. You can define functions that handle requests just like an Express or Next.js backend, no need for an external server.
...
> **info** You're essentially building a lightweight custom auth server scoped to your own application, all using just your Expo project.
...
It's the tool that kicks off the flow and talks to your API Route once the user has authorized access. See [Authentication with OAuth or OpenID providers](https://docs.expo.dev/guides/authentication/) for more info.
...
In our tutorials, we demonstrate exactly how to handle this. After receiving the ID token from a provider like Google or Apple, we generate a custom JWT on the server using Expo API Routes.
...
- For native apps, you can store it securely using [Expo Secure Store](/versions/latest/sdk/secure-store)
...
We cover all of this in the video tutorials linked above, including:
...
Modern solutions like OAuth, biometrics, and passkeys are not required, but they can be excellent additions once your core system is in place. The key is to build authentication that fits your current needs, while staying flexible enough to grow with your product.

It's perfectly okay to start with a third-party service and refine your approach as your app matures.
...
<BoxLink
  title="Google Sign-In with Expo Router API Routes"
  description="Learn how to implement Google Sign-In with Expo Router API Routes"
