...
<Collapsible summary="What versions of Expo SDK are supported in Expo Go?">

Expo Go supports multiple SDK versions. A new Expo SDK is released approximately three times a year. Each time a new SDK version is added to Expo Go, the oldest version is dropped. You can find the supported SDK versions in the [Expo SDK versions](/versions/latest/#each-expo-sdk-version-depends-on-a-react-native-version) section.
