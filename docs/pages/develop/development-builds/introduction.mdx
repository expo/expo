...
When you created your first React Native project with `npx create-expo-app` and ran it with `npx expo start`, you most likely started off using the [Expo Go](https://expo.dev/go) app for development. Expo Go is a native app that we (as in Expo) built and submitted to the [Google Play](https://play.google.com/store/apps/details?id=host.exp.exponent) and [Apple App](https://apps.apple.com/us/app/expo-go/id982107779) so you could get coding quickly. It is a sandbox app with a number of native libraries included within (see the [dependencies list](https://github.com/expo/expo/blob/main/apps/expo-go/package.json#L23)). This means that developers may update your app's JavaScript code of the app on their local machine and see the changes on Expo Go.
...
The Expo Dev Client library includes the launcher UI (shown in the screenshots below), dev menu, extensions to test updates, and more. The Expo Go app has the dev menu built in, we'll need to install it separately for a development build.
