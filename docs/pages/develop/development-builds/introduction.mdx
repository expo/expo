...
A **development build** is a debug build of your app that is optimized for quick iterations when creating an app. It contains the the [`expo-dev-client`](/versions/latest/sdk/dev-client/) library. By using development builds, you gain full control over the native runtime, so you can [install any native libraries](#use-libraries-with-native-code-that-arent-in-expo-go), [modify any project configuration](/config-plugins/introduction/), or [write your own native code](/modules/get-started/). With Expo development builds, you are building your own native app.
