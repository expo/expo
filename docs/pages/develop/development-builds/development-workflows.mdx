...
You can load your app on a device that has a compatible build of your custom client by opening a launch URL. Launch URLs carry launcher commands in reserved query parameters that start with `__expo_`. Any URL that has at least one `__expo_*` parameter is handled by the launcher before it reaches your app.

```text
{scheme}://?__expo_url={manifestUrl}
```
...
```text
exp+app-slug://?__expo_url=https%3A%2F%2Fu.expo.dev%2F767ADF57-B487-4D8F-9522-85549C39F43F%2F%3Fchannel-name%3Dmain
```
...
All reserved parameters are optional and independent. The launcher removes them before your app receives the URL, so they never reach your app's deep link handling or the development server.

| parameter                      | value                                                                                                   |
| ------------------------------ | ------------------------------------------------------------------------------------------------------- |
| `__expo_url`                   | URL-encoded URL of the project or update manifest to load.                                              |
| `__expo_disable_onboarding=1`  | Skips the onboarding screen that appears on the first launch of a development build after installation. |
| `__expo_disable_fab=1`         | Hides the Tools button (the floating button that opens the dev menu) for this launch.                   |
| `__expo_disable_auto_launch=1` | Prevents the dev menu from opening when the app starts, for this launch.                                |

`__expo_disable_fab` and `__expo_disable_auto_launch` apply to the launched process only and never change the saved dev menu settings, so a person who opens the app later still gets the menu and the Tools button. The legacy `disableFab=1` and `disableAutoLaunch=1` parameters keep updating the saved settings.

The same parameters work on Expo Go launch URLs, for example `exp://192.168.1.10:8081?__expo_disable_auto_launch=1`.

> **Note:** The legacy form `{scheme}://expo-development-client/?url={manifestUrl}` keeps working, together with its `disableOnboarding=1`, `disableFab=1` and `disableAutoLaunch=1` parameters placed next to `url`. `npx expo start` still emits this form.

#### Using launch URLs in automation scenarios

When launching a development build with automation, such as in a CI/CD workflow, an end-to-end test or an agent loop, add `__expo_disable_onboarding=1`, `__expo_disable_fab=1` and `__expo_disable_auto_launch=1` to the launch URL. Set `EXPO_NO_DEV_MENU=1` when you start Expo CLI to add these parameters to every launch URL, QR code and redirect for that session.
