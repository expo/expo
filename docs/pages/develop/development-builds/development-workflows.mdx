...
| `__expo_disable_fab=1`         | Hides the Tools button (the floating button that opens the dev menu).                                   |
| `__expo_disable_auto_launch=1` | Prevents the dev menu from opening when the app starts.                                                 |

`__expo_disable_fab` and `__expo_disable_auto_launch` update the saved dev menu settings. This is useful on a physical device, where automation cannot change those settings from the development machine. You can re-enable either setting from the dev menu.
