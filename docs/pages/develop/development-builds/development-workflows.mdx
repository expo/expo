...
| `__expo_disable_onboarding=1`  | Skip the onboarding screen that appears on the first launch of a development build after installation.                                 |
| `__expo_show_menu_at_launch=0` | Do not open the developer menu at launch. The menu stays available through gestures and the tools button. Applies to this launch only. |
| `__expo_tools_button=0`        | Hide the floating tools button. Applies to this launch only.                                                                           |

The same parameters work on Expo Go launch URLs, for example `exp://192.168.1.10:8081?__expo_show_menu_at_launch=0`.

> **Note:** The legacy form `{scheme}://expo-development-client/?url={manifestUrl}&disableOnboarding=1` keeps working. `npx expo start` still emits this form.

#### Using launch URLs in automation scenarios

When launching a development build on an emulator or simulator using automation, such as in a CI/CD workflow or an agent loop, add `__expo_disable_onboarding=1`, `__expo_show_menu_at_launch=0` and `__expo_tools_button=0` to the launch URL. Set `EXPO_NO_DEV_MENU=1` when you start Expo CLI to add these parameters to every launch URL, QR code and redirect for that session.
