...
When launching a development build with automation, such as in a CI/CD workflow or an end-to-end test, you can add the following query parameters to the URL:
...
`disableFab` and `disableAutoLaunch` have to be on the deep link itself, next to `url`, rather than inside the encoded `url` value. They apply only to the launch they are passed on and leave your saved dev menu settings unchanged, which is useful on a physical device where automation cannot change those settings from the host.
