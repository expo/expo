...
When launching an update URL in a development build on an emulator or simulator using automation, such as in a CI/CD workflow, you can set the `disableOnboarding=1` query parameter to skip the onboarding screen that appears on the first launch of a development build after installation.
