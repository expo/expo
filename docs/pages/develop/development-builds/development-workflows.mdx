...
`disableFab` and `disableAutoLaunch` have to be on the deep link itself, next to `url`, rather than inside the encoded `url` value. They update the saved dev menu settings, which is useful on a physical device where automation cannot change those settings from the host. You can re-enable either setting from the dev menu.
