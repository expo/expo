...
| `disableFab=1`        | Hides the floating action button that opens the dev menu.                                               |
| `disableAutoLaunch=1` | Prevents the dev menu from opening when the app starts.                                                 |

`disableFab` and `disableAutoLaunch` apply only to the current launch and leave your saved dev menu settings unchanged. They are useful on a physical device, where automation cannot change those settings from the host.
