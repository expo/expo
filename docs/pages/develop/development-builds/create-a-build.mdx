...
In this guide, you'll find information on creating a development build with EAS and installing it on an emulator/simulator or a physical device to continue developing your app. If you are looking to create a development build locally you can [create local builds with `expo-dev-client` library](/guides/local-app-development/#local-builds-with-expo-dev-client) or [with EAS](/build-reference/local-builds/).
...
EAS Build is created by running the `eas build` command. It automatically creates an [**eas.json**](/build/eas-json/) file at the root of your project directory with three build profiles &mdash; `development`, `preview`, and `production`. A minimal configuration is shown below:
...
- [`developmentClient`](/build-reference/eas-json/#developmentclient) to `true` to create a Debug build. It allows the `expo-dev-client` library to choose the update to load in your app and provides tools to help you develop it. It also generates a build artifact you can install on a device or emulator/simulator.
