...
description: Learn how to set up and configure the jest-expo library to write unit and snapshot tests for a project.
---
...
[Jest](https://jestjs.io) is the most widely used JavaScript testing framework. In this guide, you will learn how to set up Jest in your project, write a unit test, write a snapshot test, and best practices for structuring your tests when using Jest with React Native.

You will also use the [`jest-expo`](https://github.com/expo/expo/tree/main/packages/jest-expo) library, which is a Jest preset and mocks the native part of the Expo SDK and handles most of the configuration.
...
The default `jest-expo` provides a `transformIgnorePatterns` configuration to specify which files Babel will transform. This configuration covers most of your needs.
...
To add additional patterns, copy and modify the above configuration. Then, add it to the `transformIgnorePatterns` array under `jest` in **package.json**.
