...
In file-based routing libraries such as Expo Router, a **dynamic route** allows matching one or multiple paths based on a dynamic segment embedded in the URL. This segment is in the form of a variable such as a unique identifier and your app doesn't know the exact segment ahead of time.

This guide explains how to handle dynamic routes with Expo Router.

> This guide continues to build on top of the example and the **app** directory structure used in the previous [File-based routing](/develop/file-based-routing/) guide.
...
Dynamic segments of a URL are accessible with a [search parameter](/router/reference/search-parameters/) in the route component. For example, you can use the [`useLocalSearchParams`](/router/reference/hooks/#uselocalsearchparams) hook which returns the search parameters for the selected route.
