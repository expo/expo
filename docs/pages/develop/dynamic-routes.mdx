...
In file-based routing libraries such as Expo Router, a **dynamic route** allows matching one or multiple paths based on a dynamic segment embedded in the URL. This segment is in the form of a variable such as a unique identifier and your app doesn't know the exact segment of the pattern ahead of time.

This guide explains how to handle dynamic routes with Expo Router.

> This guide continues to build on top of the example and the **app** directory structure used in the previous [File-based routing] guide.

## Dynamic route convention

A dynamic segment of a route can be created by wrapping a file's name in square brackets. For example, **[id].tsx**.

> **What is a dynamic segment?** Any segment of a path in a URL that is dynamic. For example, in the list of users app, you might have a path such as `/details/[id]` where the `[id]` is the dynamic segment and displays details based on the `id` of the user.
...
Navigating from one route to a dynamic route is done by providing query parameters to the `Link` component statically or using the `href` object.
...
Dynamic segments of a URL are accessible with a [search parameter](/router/reference/search-parameters/) in the route component. For example, you can use the `useLocalSearchParams()` hook which returns the search parameters for the selected route.

```tsx app/(home)/details/[id].tsx
