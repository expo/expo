...
Grok Build is xAI's terminal-based AI coding agent. It can read and write files across your project, run terminal commands, and browse the web. Expo projects created with `create-expo-app` are scaffolded with an **AGENTS.md** file that Grok Build reads directly. Installing the official Expo plugin from the Grok Build plugin marketplace adds Expo Skills and the Expo MCP Server, so Grok Build can apply known-good Expo patterns, fetch documentation from the Expo Model Context Protocol (MCP) Server, manage your EAS deployment workflow, and more.
...
Install the official Expo plugin from the Grok Build plugin marketplace. The plugin bundles Expo Skills and the Expo MCP Server, so this single install teaches Grok Build known-good Expo patterns and gives it live access to Expo documentation and EAS.
