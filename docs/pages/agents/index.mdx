...
<BoxLink
  title="Grok Build"
  description="Set up xAI's terminal agent with Expo, including project context and example prompts."
  href="/agents/grok/"
  Icon={XAILogoIcon}
/>
...
| ------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **AGENTS.md**             | Codex, Cursor, and Grok Build directly. Claude Code through **CLAUDE.md**. | Points the agent to the Expo documentation matching your project's SDK. This is the source of truth for project-level instructions. |
...
Each file targets a different agent convention, so the same project works with Claude Code, Codex, Cursor, or Grok Build without per-agent configuration.
...
If you use Claude Code, create **CLAUDE.md** with a single line that imports **AGENTS.md**:
...
Commit all three files so every agent session on your team starts from the same project context. Codex and Cursor need only **AGENTS.md**, so steps 2 and 3 apply to Claude Code only.
