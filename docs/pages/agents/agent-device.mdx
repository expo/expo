...
- In an [EAS Simulator](/preview/eas-simulator/introduction/) Android session, boot the emulator headless. Set `AGENT_DEVICE_HEADLESS=1`, or pass `--headless` to `agent-device boot`. A windowed emulator does not start there.
