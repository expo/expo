...
`agent-device` runs from the terminal in Codex, Claude Code, Cursor, and other coding agents, with optional [Model Context Protocol (MCP)](/mcp/) integration. One command model covers iOS Simulators, Android Emulators, physical devices, tvOS, Android TV, macOS, Linux, and web.
