...
- On a machine without a display, such as CI, a Linux server, or an [EAS Simulator](/preview/eas-simulator/introduction/) session, boot Android Emulators headless. Set `AGENT_DEVICE_HEADLESS=1`, or pass `--headless` to `agent-device boot`. A windowed emulator does not start there.
