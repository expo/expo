...
- On a machine without a display, such as CI or a Linux server, set `ARGENT_EMULATOR_NO_WINDOW=1` before you start Argent, so it boots Android Emulators headless.
