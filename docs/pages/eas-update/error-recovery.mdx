...
- [`Updates.readLogEntriesAsync()`](/versions/latest/sdk/updates/#updatesreadlogentriesasyncmaxage), which returns the error that `expo-updates` serialized when it caught the error. Log entries older than one day are removed.
...
`expo-updates` does not launch a failed update on that device again, and a restart does not clear this. [`Updates.checkForUpdateAsync()`](/versions/latest/sdk/updates/#updatescheckforupdateasync) reports the update as unavailable with the reason `updatePreviouslyFailed`. To recover a device in this state, publish a new update or reinstall the app.

One frequent cause of an update that fails to launch, when the build it was published for works, is configuration that is present at build time but missing at publish time. For example, Metro inlines `EXPO_PUBLIC_*` environment variables into the bundle when it creates the bundle, so these variables must be set for `eas update` as well as for `eas build`.
