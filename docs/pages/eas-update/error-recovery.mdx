...
If you can identify an older update that is safe to roll back to, you can do so using EAS Update's `republish` option from the [website](https://expo.dev/accounts/[account]/projects/[project]/updates) or [CLI](/eas-update/eas-cli/#republish-a-previous-update-within-a-branch).
