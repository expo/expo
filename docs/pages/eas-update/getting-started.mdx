...
If you created your project with `npx create-expo-app`, or you don't call `registerRootComponent` in your app at all (eg: it's handled by Expo Router), then you are all set. The following applies to projects created with other tools, such as React Native Community CLI.
...
After making that change, update your [MainActivity](/versions/latest/sdk/register-root-component/#manual-android-setup) and [AppDelegate](/versions/latest/sdk/register-root-component/#manual-ios-setup) to use the module name `"main"` instead of your app name.
