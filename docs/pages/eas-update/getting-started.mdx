...
To publish an update with changes from your project, you can use the `eas update` command, and specify a name for the channel and a `message` to describe the update:

<Terminal cmd={['$ eas update --channel [channel-name] --message "[message]"']} />

<Collapsible summary="How does publishing an update work?">

When you publish an update with the `eas update` command, it generates a new update bundle and uploads it to the EAS servers. The channel name is used to locate the branch of updates in which to publish the new update. It is similar to how Git commit works, where every commit is on a Git branch.
