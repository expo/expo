...
You can use [`expo-background-task`](/versions/latest/sdk/background-task/) to check for updates while the app is backgrounded. This is useful for apps that are used in a way when the app is in the background for extended periods of time, such as weather apps or other information apps that are not actively used but are running in the background.
...
<Collapsible summary="Example: Check for updates in the background">
