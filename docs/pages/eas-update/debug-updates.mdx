...
We also provide a [step by step guide](/pages/eas-update/eas-update-with-local-build) to quickly try out EAS Update in a local development environment using Android Studio or Xcode, with either release or debug builds of the app.
