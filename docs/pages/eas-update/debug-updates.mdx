...
We also provide a [step-by-step guide to try out EAS Update quickly](/eas-update/local-build) in a local development environment using Android Studio or Xcode, with either release or debug builds of the app.
