...
description: Learn how to preview updates in both preview and development builds.
---
...
// TODO: actually move this to "develop-faster" which we should rename to "preview" and move this current page back to dev-builds or whatever, then create new page about preview builds

Before deploying an update to production, it's often prudent to test it in a production-like environment. As a developer, an easy way to accomplish this is to load your update in a development build, and this works great for loading previews from pull requests or directly from the EAS dashboard. Non-technical users will typically not want to interact with a development build, and they will want to test changes from a preview build on an [App store testing track](/review/overview/#app-store-testing-tracks) or [internal distribution](/review/overview/#internal-distribution-with-eas-build). This guide will show you how to preview updates in both preview and development builds.

## Preview builds

> **Note:** this feature is currently experimental and requires using a `next` version of `expo-updates`.


## Development builds

[`expo-dev-client`](/develop/development-builds/introduction/) library allows launching different versions of a project by creating a development build. Any update can be previewed in a development build with a compatible runtime version.
