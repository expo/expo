...
Bundle diffing works in two situations. Devices already running a published update get a patch when a newer one is available. Devices on a fresh install can also get a patch on their very first update check, instead of downloading the full new bundle, when the embedded bundle from the build has been uploaded. Both are enabled by default on recent SDK versions.
...
<Tab label="SDK 58 and later">

EAS Build uploads the embedded bundle when the build completes. Updates published later to the same channel can then be served as patches against it. No configuration is required.

To disable the upload, set the `EAS_UPDATE_UPLOAD_EMBEDDED_BUNDLE` environment variable to `0` under `env` in a build profile in **eas.json**:
...
```json eas.json
{
  "build": {
    "production": {
      "env": {
        "EAS_UPDATE_UPLOAD_EMBEDDED_BUNDLE": "0"
      }
    }
  }
}
...
<Tab label="SDK 57 and below">

> **info** On SDK 57 and below, this mode is [experimental](/more/release-statuses/#experimental) and opt-in. The behavior may change.

To enable, set the `EAS_UPDATE_UPLOAD_EMBEDDED_BUNDLE` environment variable to `1` under `env` in a build profile in **eas.json**:
...
```json eas.json
{
  "build": {
    "production": {
      "env": {
        "EAS_UPDATE_UPLOAD_EMBEDDED_BUNDLE": "1"
      }
    }
  }
}
...
When the build completes, EAS Build uploads the embedded bundle. Updates published later to the same channel can then be served as patches against it.

</Tab>

</Tabs>

If you don't use EAS Build, you can upload an embedded bundle yourself. Point `--bundle` at the JavaScript bundle and `--manifest` at the **app.manifest** produced by the native build:
...
- **Fresh installs receive the full bundle on their first update unless the build's embedded bundle was uploaded.** This happens by default on SDK 58 and later. See [patches from the embedded bundle](#patches-from-the-embedded-bundle).
