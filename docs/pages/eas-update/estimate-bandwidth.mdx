...
## Estimating bundle diff patch size
...
Under normal circumstances, most users download patches instead of full bundles, but EAS Update may fall back to serving the full bundle in some cases (see [current limitations](/eas-update/bundle-diffing/#current-limitations)). The actual impact of bundle diffing also varies based on whether diffing is enabled for the embedded bundle.
