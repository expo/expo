...
## Estimating bundle diff size

If your project uses [bundle diffing](/eas-update/bundle-diffing), most users download a patch rather than the full bundle. A patch contains only the differences between the bundle already on the device and the new bundle, so it is usually much smaller than the full update. Incorporating the impact of bundle diffing can give you a more realistic estimate for bandwidth usage.

EAS Update generates patches with the [bsdiff algorithm](https://en.wikipedia.org/wiki/Bsdiff). You can run the same algorithm locally to compare two of your own bundles. First, install bsdiff:

<Terminal cmd={['$ brew install bsdiff']} />

Next, export your app twice: once at the version your users currently run, and once with your new changes. For each export, follow the steps in [Measuring your actual update size](#measuring-your-actual-update-size). Rename the two **.hbc** files to **old.hbc** and **new.hbc**.

Then generate the patch and check its size:

<Terminal
  cmd={['$ bsdiff old.hbc new.hbc patch.bin', '$ ls -lh patch.bin']}
  cmdCopy="bsdiff old.hbc new.hbc patch.bin && ls -lh patch.bin"
/>

The size of **patch.bin** is the approximate download size for a user who moves from the old bundle to the new one. The diff will already be compressed, so you do not need to factor in the compression algorithm.

Every patch is different, so you may want to try a variety of smaller and larger code changes to get a better idea of the potential impact of bundle diffing.

Under normal circumstances, most users will download diffs instead of full bundles, but EAS Update may fall back to serving the full bundle in some circumstance. See [current limitations](/eas-update/bundle-diffing/#current-limitations) for the full list. The actual impact of bundle diffing will also vary based on whether diffing is enabled for the embedded bundle.
