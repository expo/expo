...
Next, export your app twice: once at the version your users currently run, and once with your new changes. For each export, run `npx expo export` to generate the bundle inside the **dist** folder. Rename the two **.hbc** files to **old.hbc** and **new.hbc**.
