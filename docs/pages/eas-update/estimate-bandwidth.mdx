...
Under normal circumstances, most users will download patches instead of full bundles, but EAS Update may fall back to serving the full bundle in some circumstance (see [current limitations](/eas-update/bundle-diffing/#current-limitations)). The actual impact of bundle diffing will also vary based on whether diffing is enabled for the embedded bundle.
