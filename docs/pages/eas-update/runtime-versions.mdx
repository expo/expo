...
This policy is great for projects that contain custom native code and want to use updates safely without native conflicts.

Differently than "nativeVersion", this policy automatically checks the project native dependencies and update the runtimeVersion accordingly.

It's important to notice that fingerprinting the project might take a while and it will slow down the build process.
