...
[`expo-dev-client`](/develop/development-builds/introduction/) library allows launching different versions of a project. It supports previewing published EAS updates inside a development build through channels that are linked with branches.
...
Make non-native changes locally in your project and then [publish them using `eas update`](/eas-update/publish/). The branch specified when running this command should correspond to a channel. You can run `eas channel:list` to view the channels linked to branches.
...
As an alternative to using the **Extensions** tab, you can construct a specific URL to open your EAS Update in the development build. The URL will look like the following:
