...
In the default EAS Update flow, an updates URL and updates channel is fixed at build time. When building on EAS, this channel is set in your **eas.json** build profile. This channel is sent as the `expo-channel-name` request header to the updates URL and determines from which channel your app will receive updates. Channel surfing allows you to override that request header in your JavaScript code, allowing your app to request updates from a different channel.
...
- Load updates from more than one channel on the same native runtime.
