...
To avoid this issue, you can use the [`Updates.isEmbeddedLaunch`](/versions/latest/sdk/updates/#updatesisembeddedlaunch) property to determine whether the app is running an embedded update or one downloaded from the server. If [`Updates.isEmbeddedLaunch`](/versions/latest/sdk/updates/#updatesisembeddedlaunch) is `true`, the currently running update is embedded in the build, which means it won't be available in the [Expo dashboard](https://expo.dev/accounts/[account]).
