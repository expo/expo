...
- Builds on that channel still check for updates, but no update is returned to them. A response with no update can be a bodyless HTTP `204`, so these checks use very little bandwidth.
