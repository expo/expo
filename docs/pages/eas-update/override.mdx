...
For apps using EAS Update, the primary use case for this feature is [channel surfing](/eas-update/channel-surfing/). Channel surfing switches the `expo-channel-name` request header at runtime as part of a complete channel switching workflow.
...
> **warning** Switching channels can cause compatibility issues when updates use different migrations or data shapes. See [Risks and considerations when switching channels](/eas-update/channel-surfing/#risks-and-considerations-when-switching-channels).
...
You can override request headers by calling [`Updates.setUpdateRequestHeadersOverride`](/versions/latest/sdk/updates/#updatessetupdaterequestheadersoverriderequestheaders). With EAS Update, overriding the `expo-channel-name` request header makes the app request updates from the specified channel.

The object you pass to `setUpdateRequestHeadersOverride()` replaces the custom request headers configured in the build. Include any other custom request headers that the app still needs while the override is active.

Provide a way for users to trigger the request header change in your app. This may be a hidden menu that only trusted users can access or another mechanism that fits your use case. After changing the request headers, call [`fetchUpdateAsync()`](/versions/latest/sdk/updates/#updatesfetchupdateasync) to fetch the update and [`reloadAsync()`](/versions/latest/sdk/updates/#updatesreloadasyncoptions) to reload the app. You can also wait for the next launch to fetch and install the update automatically.
...
> **info** You can only override a request header if the native build includes its name. When a build profile in **eas.json** defines `channel`, EAS Build includes the `expo-channel-name` header and sets it to that `channel` value. For example, a build created with the `production` profile and `"channel": "production"` requests updates from the `production` channel. If you build without EAS Build, add the header to [`updates.requestHeaders`](/eas-update/getting-started/#configure-update-channels-in-appjson) or the native project, then rebuild the app.
