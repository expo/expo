...
To learn more about the differences between CodePush and EAS Update, see [Conceptual differences between CodePush and EAS Update](#conceptual-differences-between-codepush-and-eas-update).
...
CodePush and EAS Update are both services that allow you to send hotfixes to the JavaScript code of your app, but they take slightly different approaches for how streams of updates are managed.
...
A key distinction between CodePush and EAS Update that can impact your release process is the aforementioned difference between how a specific deployment is targeted: with CodePush, the client controls this at runtime, and with EAS Update, this is only partially controlled at build time and otherwise determined it is controlled on the server side. This means that you can't include code in your app using EAS Update to instruct it to load a different stream of updates depending on runtime criteria, such as the current user role (eg: distribute beta releases to employees). This feature is commonly used in staging environments to allow non-technical stakeholders to test features from a single build on Google Play Beta / TestFlight. The current alternative for this with EAS Update is to use [development builds](/eas-update/expo-dev-client/).
