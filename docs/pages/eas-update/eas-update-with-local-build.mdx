...
description: Using EAS update directly with a local build environment
---
...
In this step by step guide, you'll create a new Expo app, run it in your local development environment, publish an update, and show that the app downloads and runs the update when restarted.

This guide goes into detail on some of the Expo config properties used in EAS Update and their functions.
...
The example app shown here is based on SDK 48, which supports the new [`requestHeaders`](/versions/latest/config/app/#requestheaders) property in Expo config.
...
> In the [introductory guide](/pages/eas-update/getting-started), this property is set by EAS Build and is different for different EAS build profiles. Since this is a local build, it is necessary to set this request header explicitly in the Expo config.
...
> If you do not see the above lines added to these files, it means that you are using SDK 47 or earlier, and you should add these lines manually to **AndroidManifest.xml** and **Expo.plist**.
