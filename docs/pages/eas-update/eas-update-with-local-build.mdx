...
> In the [introductory guide](/pages/eas-update/getting-started), the channel name is set by EAS Build and is different for different EAS build profiles. Since this is a local build, it is necessary to set this request header explicitly in the Expo config.
