...
When creating a new production build, increment your app version every time to ensure it has a unique runtime version.
...
**If you have already published the fix to staging and verified it there**, ensure that you are republishing from the same commit. We recommend using identical environment variables and code signing configuration on staging as on production, in order to ensure that updates verified in staging work exactly the same in production.
