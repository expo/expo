...
Access for members is managed through a role-based system. Users can have the _owner_, _admin_, _release manager_, _developer_, or _viewer_ role within an Organization account. Roles are hierarchical. Each role can do everything the roles below it can do.
...
> **info** Protected channels are in private beta and will be available on the Enterprise plan. Any account can assign the Release Manager role, but until protected channels are enabled for an account, a Release Manager has the same access as a Developer.
