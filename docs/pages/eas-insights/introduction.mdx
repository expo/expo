...
For those already using [EAS Update](/eas-update/introduction.mdx), we're able to provide certain high-level usage insights without any additional client-side changes. This is possible because by aggregating data from client requests to check for an update into a limited Insights view that shows usage over time, as well as usage broken down by platform.

### Using the `expo-insights` module

Developers can add the `expo-insights` module to their project and gain more precise usage metrics (than what can be provided by just aggregating update requests), as well as additional breakdowns by app store version.  Currently, the module is limited to sending client events only pertaining to cold starts of the app, but in the future we will expand `expo-insights` to offer new types of events and payloads, to support more advanced functionality.
