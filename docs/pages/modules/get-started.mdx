...
If you need to extend the capabilities of your application, you can install expo packages or third party react native libraries.
Sometimes, this is not enough. You may need to write custom native code to tap into obscure platform APIs or use custom Android or iOS dependencies.

Luckly, Expo has first class support for this in the form of Expo Modules.
...
<BoxLink
  ...
  description="Learn more about the design considerations behind Expo Modules."
  href="./overview"
  Icon={BookOpen02Icon}
/>
<BoxLink
  title="Expo Modules API Reference"
  description="A list of building blocks you can use to create your own Expo Module."
  href="./module-api"
  Icon={Grid01Icon}
/>
...
We provide two recommended flows for creating a new Expo Module. You can either:

- add a new module to an existing Expo application and use it to test and develop your module, or
- create a new module in isolation with a generated example project, if you want to reuse it in multiple projects or publish to npm.

The next two sections describe the two flows in detail.
...
The recommended way to create a local Expo Module is with the `create-expo-module --local` command.

Navigate to your project directory (the one that contains the `package.json` file) and run:

<Terminal cmd={[`$ npx create-expo-module --local`]} />

It's best to provide a meaningful module name, but you can accept the default suggestions for the rest of the questions.

</Step>
<Step label="2">
### Using the module

If you have an `ios` folder in your project that you created using `expo prebuild`, you need to reinstall the pods using `pod install --project-directory=ios`.
...
You can now import the module in your application:
...
If you have an `android` directory in your project that you created using `expo prebuild`, you can open the directory in Android Studio.

You can always just edit the files in the `modules/my-module/android/src/main/java/expo/modules/mymodule/` directory directly in your favourite text editor.
...
If you have an `ios` directory in your project that you created using `expo prebuild`, you can open the directory in Xcode.

You can always just edit the files in the `modules/my-module/ios/` directory directly in your favourite text editor.
...
> For example, you can use a monorepo or publish to npm, as described by the [How to use a standalone Expo module in your project](./use-standalone-expo-module-in-your-project) guide.
