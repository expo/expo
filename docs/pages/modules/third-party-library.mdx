...
Alternatively, if you want to create an empty Expo module to be published on npm and used in any Expo application, run:
...
Next, you can open the newly created `expo-radial-chart` directory and start editing the native code.

### Inside an existing application

If you want to use our new module as a view inside the existing project, you can run this command inside your project directory:

<Terminal cmd={['$ npx create-expo-module --local expo-radial-chart']} />

Next, you can open the newly created `modules/expo-radial-chart` directory and start editing the native code.
...
Now we can add the native dependencies to the module by editing the `android/build.gradle` and `ios/ExpoRadialChart.podspec` files.
...
To use our module in the application without mistakes we'll define the types for our props.
Our module accepts a list of series, each with a color and a percentage value.
...
Since we won't implement the module on the web in this example, we also need to replace the `src/ExpoRadialChartView.web.tsx` file.
...
We can now implement the native functionality by editing the placeholder files.

We create a `PieChart` instance and set its layoutParams to match the parent view.
Then we add it to the view hierarchy using the `addView` function.

We define a `setChartData` function that accepts a list of `Series` objects.
We iterate over the list and create a `PieEntry` for each series. We also store the colors in a separate list.
Then we create a `PieDataSet`, use it to create a `PieData` object and set it as data on the `PieChart` instance.
...
We also need to use the [`Prop`](/modules/module-api/#prop) function to define the `data` prop and call the native `setChartData` function when the prop changes.
...
We create a new `PieChartView` instance and use the `addSubview` function to add it to the view hierarchy.

We need to set the `clipsToBounds` property and override the `layoutSubviews` function to make sure the chart view is always the same size as the parent view.

Finally, we create a `setChartData` function that accepts a list of series, creates a `PieChartDataSet` instance with the data and assigns it to the `data` property of the `PieChartView` instance.
...
## Write an example application to use our module

Now we can update the app inside the `example` directory to to test our module. We'll use the `ExpoRadialChartView` component to render a pie chart with three slices.
...
> **Tip**: If you created the module inside an existing application, make sure to import it directly from your `modules` directory. It's easiest to use a relative import. `import { ExpoRadialChartView } from '../modules/expo-radial-chart';`
...
Rerun the build commands from step 2 and make sure your application builds successfully on both platforms.

You should see a pie chart with three slices:

![A PieChart module on iOS and Android](/static/images/modules/third-party-library/result.webp)

Congratulations, you have created your first simple wrapper around two separate native libraries using Expo Modules! Learn more about the API in the [Expo Module API reference](/modules/module-api/).
