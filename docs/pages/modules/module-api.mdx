...
Defines a value associated with a JavaScript object representing the native module.

To declare a read-only property, you can use a shorthanded syntax that requires only two arguments:
...
In the case of the mutable property, both the getter and the setter closure are needed (using that syntax is also possible to declare a property with only a setter):

- **name**: `String` — Name of the property that you'll use from JavaScript.
- **getter**: `() -> ProperyType` — The closure to run when the getter for a property was called.
- **setter**: `(newValue: ProperyType) -> void` — The closure to run when the setter for a property was called.
