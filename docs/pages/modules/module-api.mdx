...
> **warning** **[Deprecated](/more/release-statuses/#deprecated):** Use [`Constant`](#constant) instead.
...
- **name**: `String` &mdash; Name of the function that you'll call from JavaScript.
- **body**: `(args...) -> ReturnType` &mdash; The closure to run when the function is called.
...
- **name**: `String` &mdash; Name of the property that you'll use from JavaScript.
- **getter**: `() -> PropertyType` &mdash; The closure to run when the getter for a property was called.
...
- **name**: `String` &mdash; Name of the property that you'll use from JavaScript.
- **getter**: `() -> PropertyType` &mdash; The closure to run when the getter for a property was called.
- **setter**: `(newValue: PropertyType) -> void` &mdash; The closure to run when the setter for a property was called.
...
- **viewType** &mdash; The class of the native view that will be rendered. Note: On Android, the provided class must inherit from the [`ExpoView`](#expoview), on iOS it's optional. See [`Extending ExpoView`](#extending--expoview).
- **definition**: `() -> ViewDefinition` &mdash; A builder of the view definition.
...
> **Note:** This function is not available on Android &mdash; you may want to use [`OnActivityEntersForeground`](#onactivityentersforeground) instead.
...
> **Note:** This function is not available on Android &mdash; you may want to use [`OnActivityEntersBackground`](#onactivityentersbackground) instead.
...
> **Note:** This function is not available on iOS &mdash; you may want to use [`OnAppEntersForeground`](#onappentersforeground) instead.
...
> **Note:** This function is not available on iOS &mdash; you may want to use [`OnAppEntersBackground`](#onappentersbackground) instead.
...
- **activity** &mdash; The Android activity that received the result.
- **payload** &mdash; An object containing data about the activity result.
  - **requestCode**: `Int` &mdash; The request code originally supplied to `startActivityForResult`, used to identify the source of the result.
  - **resultCode**: `Int` &mdash; The result code returned by the child activity (for example, `Activity.RESULT_OK` or `Activity.RESULT_CANCELED`).
  - **data** &mdash; An optional intent that carries the result data returned from the launched activity. Can be `null`.
...
- **intent**: `Intent` &mdash; The new intent was delivered to the activity. For more information about the `Intent` type, visit: [https://developer.android.com/reference/android/content/Intent](https://developer.android.com/reference/android/content/Intent).
...
- **name**: `String` &mdash; Name of view prop that you want to define a setter.
- **defaultValue**: `ValueType` &mdash; Optional default value used when the setter is called with `null`.
- **setter**: `(view: ViewType, value: ValueType) -> ()` &mdash; Closure that is invoked when the view rerenders.
...
- **viewType** &mdash; The class of the native view. Note that the provided class must inherit from the Android `ViewGroup`.
- **definition**: `() -> ViewGroupDefinition` &mdash; A builder of the view group definition.
...
- **action**: `(parent: ParentType, child: ChildType, index: Int) -> ()` &mdash; An action that adds a child view to the view group.
...
- **action**: `(parent: ParentType) -> Int` &mdash; A function that returns number of child views.
...
- **action**: `(parent: ParentType, index: Int) -> ChildType` &mdash; A function that retrieves a child view at a specific index from the view group.
...
- **action**: `(parent: ParentType, child: ChildType) -> ()` &mdash; A function that remove a specific child view from the view group.
...
- **action**: `(parent: ParentType, child: ChildType) -> ()` &mdash; A function that removes a child view at a specific index from the view group.
...
> **important** **This feature is [experimental](/more/release-statuses/#experimental).**
...
- `Either<FirstType, SecondType>` &mdash; A container for one of two types.
- `EitherOfThree<FirstType, SecondType, ThirdType>` &mdash; A container for one of three types.
- `EitherOfFour<FirstType, SecondType, ThirdType, FourthType>` &mdash; A container for one of four types.
