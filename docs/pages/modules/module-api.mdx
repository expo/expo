...
While JavaScript to Native communication is mostly covered by native functions, you might also want to let the JavaScript code know about certain system events, for example, when the clipboard content changes.
...
To subscribe to these events in JavaScript, you need to wrap the native module with `EventEmitter` class as shown:
