...
- **isUndefined**: `Bool` — Returns `true` if the JavaScript value was `undefined`, `false` otherwise.
- **optional**: `InnerType?` — Returns the unwrapped value if present, or `null` if the value was `undefined`.
