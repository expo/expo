...
- `devtools` — Config with options specific to Expo CLI DevTools plugins
  ...
  - `serverEntryPoint` — A package-local JavaScript file that default-exports a `handler(request)` function. Expo CLI calls it for requests to the plugin endpoint (with the endpoint prefix stripped from the URL) before falling back to `webpageRoot` static serving, so a plugin can serve dynamic responses such as API endpoints for its web interface. Return a [`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response), or `null` to fall through to static serving.
