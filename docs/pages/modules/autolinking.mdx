...
The following config in **package.json** will exclude `expo-random` and `third-party-native-module` from autolinking on Android:
...
```json package.json
{
  "expo": {
    "autolinking": {
      "android": {
        "exclude": ["expo-random", "third-party-native-module"]
      }
    }
  }
}
