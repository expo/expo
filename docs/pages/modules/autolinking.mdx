...
Packages in this list are not linked into your native build. They are only added to the duplicate checks and to Metro's module resolution.
...
`npx expo-doctor` and the `verify` command now report duplicates for `third-party-library`, and `experiments.autolinkingModuleResolution` resolves it to a single copy when bundling. Nothing is added to your native build, so listing a package that contains no native code is safe.
