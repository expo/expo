...
Usually, when you're developing a native mobile app and want to install a 3rd-party library, you're asked to add the dependency to the manifest files of your package managers (**build.gradle** on Android, **Podfile** for CocoaPods on iOS, **Package.swift** for SwiftPM on iOS).
In Expo and React Native in general, you already need to do that with your **package.json** file by installing the package from [npm](https://www.npmjs.com) registry. Since most of the React Native libraries come with some native (platform-specific) code,
installing a library would require configuring even up to three different package managers!

Expo Autolinking is a mechanism that automates this process and narrows down the library installation process to the minimum — usually just installing the package from `npm` and re-running `pod install`.
...
Searching is the first phase of resolving the Expo modules installed in the project. Its implementation is common for all platforms and is intended to
just find which modules should be taken as Expo modules and figure out which version is of the highest precedence (in case of duplicates).

<Terminal cmd={['npx expo-modules-autolinking search']} />

Returns an object in JSON format with modules that have been found as dependencies:
...
```json
{
  "expo-random": {
    ...
    "version": "13.0.0",
    "config": { // Contents of `expo-module.config.json`
...
Resolving is the second phase that is based on the results from the `search` command. It resolves each search result to an object with more (platform-specific) details,
such as the path to the podspec or **build.gradle** files and module classes to link.

<Terminal cmd={['npx expo-modules-autolinking resolve --platform <ios|android>']} />

For example, with `--platform ios` option it returns an object in JSON format with an array of modules and resolved details for iOS platform:
...
Verifies the search results by checking whether there are no multiple revisions (duplicates) of the same package, otherwise an appropriate warning is shown.
...
It defaults to a list of all **node_modules** folders found when traversing up through your monorepo starting from the app's root directory.
...
The following config in **package.json** would exclude `expo-random` from autolinking on Android:
...
CocoaPods flags to pass to each autolinked pod. `inhibit_warnings` is probably the only one you may want to use to inhibit Xcode warnings produced when compiling the autolinked modules.
...
All projects created with `npx create-expo-app` command are already configured to use Expo Autolinking. If your project was created using different tools,
please refer to [Installing Expo modules](/bare/installing-expo-modules) to make sure your project includes all necessary changes.
...
It's also significantly faster, even though the module resolution algorithm is more complex in order to be more reliable and match the Node's module resolution.
Expo module resolution is also capable of detecting the duplicates which is a common issue in monorepos. Last but not least, it integrates well with all the features offered by Expo Modules APIs.
