...
A list of additional package names to verify for deduplication. This is useful for utility libraries that should not be duplicated because of singleton or internal state, even when they aren't native modules. For when to reach for this option, see [Checking packages that aren't native modules](#checking-packages-that-arent-native-modules).
...
Unlike other options, per-platform `include` lists are merged with the root list rather than overridden. The configuration below verifies a third-party library on every platform and additionally checks a platform-specific library only on Android:
...
Packages in this list are not linked into your native build. They are only added to the duplicate checks and, when `experiments.autolinkingModuleResolution` is enabled, to Metro's module resolution.
...
A list of package names to opt out of prebuilt Expo modules. For complete reference, see [Prebuilt Expo Modules for Android](/guides/prebuilt-expo-modules/#disabling-specific-modules-via-expo-autolinking).
...
<Terminal
  cmd={{
    npm: ['$ npx expo-modules-autolinking search'],
    yarn: ['$ yarn dlx expo-modules-autolinking search'],
    pnpm: ['$ pnpm dlx expo-modules-autolinking search'],
    bun: ['$ bunx expo-modules-autolinking search'],
  }}
/>
...
<Terminal
  cmd={{
    npm: ['$ npx expo-modules-autolinking resolve --platform <apple|android>'],
    yarn: ['$ yarn dlx expo-modules-autolinking resolve --platform <apple|android>'],
    pnpm: ['$ pnpm dlx expo-modules-autolinking resolve --platform <apple|android>'],
    bun: ['$ bunx expo-modules-autolinking resolve --platform <apple|android>'],
  }}
/>
...
<Terminal
  cmd={{
    npm: ['$ npx expo-modules-autolinking verify'],
    yarn: ['$ yarn dlx expo-modules-autolinking verify'],
    pnpm: ['$ pnpm dlx expo-modules-autolinking verify'],
    bun: ['$ bunx expo-modules-autolinking verify'],
  }}
/>
...
<Terminal
  cmd={{
    npm: ['$ npx expo-modules-autolinking react-native-config'],
    yarn: ['$ yarn dlx expo-modules-autolinking react-native-config'],
    pnpm: ['$ pnpm dlx expo-modules-autolinking react-native-config'],
    bun: ['$ bunx expo-modules-autolinking react-native-config'],
  }}
/>
...
[`expo-doctor`](/develop/tools/#expo-doctor) lists every duplicated native module in your project, with the path of each installation:
...
The [`verify`](#verify) command reports the same duplicates without running the other project checks. Deduplicating your dependencies is the complete fix. For the steps for each package manager, see [Resolving dependency issues](https://expo.fyi/resolving-dependency-issues).
...
The duplicate installation stays in your **node_modules** directory, so this flag is a workaround rather than a replacement for deduplicating your dependencies. Use it for the duplicates you cannot remove.

### Checking packages that aren't native modules

The duplicate checks and `experiments.autolinkingModuleResolution` act on the native modules that autolinking resolves, plus a built-in list of packages that must not be duplicated. Packages that aren't native modules can break when they're duplicated as well. A library that creates a React context or keeps state in a module-level variable is a common example.

Add these packages to the [`include`](#include) option to treat them the same way:
...
`npx expo-doctor` and the `verify` command now report duplicates for `third-party-library`. When bundling, `experiments.autolinkingModuleResolution` resolves that package to a single copy. Nothing is added to your native build, so listing a package that contains no native code is safe.
