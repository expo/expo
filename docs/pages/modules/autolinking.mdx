...
Expo Autolinking is a mechanism that automates this process and reduces the library installation process to the minimum &mdash; usually just installing the package from `npm` and re-running `pod install`.
...
It's also necessary to include supported platforms in the `platforms` array &mdash; if the platform for which the autolinking algorithm is run is not present in this array, it's just skipped in the search results.
