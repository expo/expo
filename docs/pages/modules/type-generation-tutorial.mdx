...
Looking at the "generated" file, we can see that an `ExpoWebViewProps` interface has been generated, it has the props and events from `FirstInlineView`. The Swift `URL` type of the `url` prop correctly maps to `string` in TypeScript.

Types that the tool cannot map or find a definition for are different. When a type is not one of the known convertible types and is not defined in the provided files (or the tool failed to parse its definition), the tool generates an `export type SomeType = unknown;` alias for it.
...
The `*.types.ts` file contains the definitions of all types defined in the module. In our case we've declared a `Theme` enum which has correctly been put in the file, together with an `ExpoSettingsEvents` type describing the module's events. Note that the tool doesn't resolve the payload types of events, so the payload is typed as a generic object.
