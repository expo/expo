...
> **warning** **Deprecated:** The `Video` and `Audio` APIs from `expo-av` have now been deprecated and replaced by improved versions in [`expo-video`](video.mdx) and [`expo-audio`](audio.mdx). We recommend using those libraries instead. `expo-av` is not receiving patches and will be removed in SDK 55.
