...
If you want to avoid the performance penalty on older platforms you can use the `dimezisBlurViewSdk31Plus` [BlurMethod](#blurmethod-1) which will only blur on newer versions of Android and fall back to the [`none`](#blurmethod-1) on older versions.
