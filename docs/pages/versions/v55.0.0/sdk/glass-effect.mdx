...
- Avoid `opacity` values less than `1` on `GlassView` or its parent views, as this causes effects to render incorrectly. Use the `animate` and `animationDuration` props in `glassEffectStyle` to fade in/out the effect instead. See the [opacity animation workaround](#opacity-animation-workaround) example, [Apple's documentation](https://developer.apple.com/documentation/uikit/uivisualeffectview#Set-the-correct-alpha-value), and [GitHub issue #41024](https://github.com/expo/expo/issues/41024) for more information.
...
Since `opacity` values less than `1` cause `GlassView` to render incorrectly, you can use Reanimated to animate a wrapper view's opacity while toggling the `glassEffectStyle` between the desired style and `'none'`.
