...
First, create a Widget using `createWidget` function and pass the widget component marked with the `'widget'` directive.
...
The simplest way to update a widget is to use the `updateSnapshot` method. This creates a widget timeline with a single entry that displays immediately.
