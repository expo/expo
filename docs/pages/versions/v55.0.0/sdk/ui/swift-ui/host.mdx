...
Use `ignoreSafeArea="keyboard"` when React Native is already handling keyboard avoidance (for example, with [`react-native-keyboard-controller`](https://docs.expo.dev/versions/latest/sdk/keyboard-controller/)), to prevent the SwiftUI host from applying its own keyboard inset on top.
...
Use `ignoreSafeArea="all"` when you want SwiftUI content to extend behind the status bar, home indicator, and keyboard — useful for full-screen overlays or backgrounds.

```tsx IgnoreAllSafeAreasExample.tsx
import { Host, Rectangle } from '@expo/ui/swift-ui';
import { frame, background } from '@expo/ui/swift-ui/modifiers';
