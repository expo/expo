...
A background notification (iOS) or a data-only notifiation (Android) is a remote notification that does not display an alert, play a sound, or add a badge to your app's icon. The purpose of a background notification is to give you a way to wake up your app to trigger an app data refresh in the background.
