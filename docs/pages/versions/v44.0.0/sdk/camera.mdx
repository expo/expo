...
**(_function_)** Callback invoked with results of face detection on the preview. See [FaceDetector documentation](facedetector.mdx#event-shape) for details.

### `faceDetectorSettings`

**(_Object_)** A settings object passed directly to an underlying module providing face detection features. See [FaceDetector documentation](facedetector.mdx#settings) for details.
...
**(_object_)** Settings exposed by [`BarCodeScanner`](bar-code-scanner.mdx) module. Supported settings: [**barCodeTypes**].
...
On native platforms, the local image URI is temporary. Use [`FileSystem.copyAsync`](filesystem.mdx#filesystemcopyasyncoptions) to make a permanent copy of the image.
