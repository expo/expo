...
**Usage:**
On iOS, the application bundle is read-only and direct access may be restricted due to sandboxing permissions. The recommended approach is to copy the file to a writable location (like `Paths.cache`,paths.document) before reading it.
...
const readNativeBundle = async () => {
  try {
    ...
    if (Platform.OS === 'ios') {
      // Copy only for iOS
      const tempFile = new File(Paths.cacheDirectory, fileName);

      if (tempFile.exists) {
        await tempFile.delete();
      }

      await bundleFile.copy(tempFile);
      fileToRead = tempFile;
    }
