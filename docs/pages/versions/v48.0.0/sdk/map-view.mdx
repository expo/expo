...
- Rebuild the app binary (or re-submit to the Google Play Store in case your app is already uploaded). An easy way to test if the configuration was successful is to do an [emulator build](/home/develop/development-builds/create-a-build/#create-a-development-build-for-emulatorsimulator).
...
- In your code, import `{ PROVIDER_GOOGLE }` from `react-native-maps` and add the property `provider=PROVIDER_GOOGLE` to your `<MapView>`. This property works on both Android and iOS.
- Rebuild the app binary. An easy way to test if the configuration was successful is to do a [simulator build](/home/develop/development-builds/create-a-build/#create-a-development-build-for-emulatorsimulator).
