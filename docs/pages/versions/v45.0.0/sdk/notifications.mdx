...
Custom notification sounds are only supported when using [EAS Build](/build/introduction), or in the bare workflow.
...
Fetches the number currently set as the badge of the app icon on the device's home screen. A `0` value means that the badge is not displayed.
