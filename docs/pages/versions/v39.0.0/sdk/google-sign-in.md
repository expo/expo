...
2. In `app.json`, set `expo.devs.config.googleSignIn.reservedClientId` to your reversed client ID.
...
5. In `app.json`, set your `expo.devs.config.googleSignIn.reservedClientId` to the value of `REVERSED_CLIENT_ID` in the `GoogleService-Info.plist`.
6. Also in `app.json`, set `expo.devs.googleServicesFile` to the relative path of your `GoogleService-Info.plist`. Make sure the file is located somewhere in your Expo project.
