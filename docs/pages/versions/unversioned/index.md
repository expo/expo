...
The Expo SDK provides access to device and system functionality such as contacts, camera, gyroscope sensor, GPS location, etc. To use these functionalities and may other, you can install any of the Expo SDK package using `expo-cli` with the `expo install` command. For example, in the command below, three different package are installed:
