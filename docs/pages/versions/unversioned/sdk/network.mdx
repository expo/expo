...
> **Android only:** The signal strength APIs (`getCellSignalStrengthAsync`, `getWifiSignalStrengthAsync`, `addCellSignalStrengthListener`, `addWifiSignalStrengthListener`, and the `useCellSignalStrength`, `useWifiSignalStrength`, `useActiveSignalStrength` hooks) are only available on Android. They return a value in the range `[0, 4]`, where `0` is the weakest and `4` is the strongest signal. The constant `INVALID_SIGNAL_STRENGTH` (`-1`) is returned when the value is unavailable or when the active network type does not support signal strength (for example, Ethernet or VPN).
