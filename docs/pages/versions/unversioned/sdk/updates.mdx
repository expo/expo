...
To avoid this issue, you can use the [`Updates.isEmbeddedLaunch`](/#updatesisembeddedlaunch) property to determine if the app is running an embedded update or one downloaded from the server. If `isEmbeddedLaunch` is `true`, the currently running update is embedded in the build, meaning it won't be available in the [Expo dashboard](https://expo.dev/accounts/[account]).
