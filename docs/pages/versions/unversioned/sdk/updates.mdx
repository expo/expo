...
`updates.maxUpdatesToKeep` controls how many updates to keep, including the one currently running. For example, setting it to `3` keeps the running update and up to two older updates. Updates that match the manifest filters take priority, with the newest updates kept first.
