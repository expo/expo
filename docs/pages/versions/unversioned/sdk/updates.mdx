...
| [`updates.maxUpdatesToKeep`](../config/app/#maxupdatestokeep)                       | `2`       | <NoIcon />  | `EXUpdatesMaxUpdatesToKeep`            | `expo.modules.updates.EXPO_UPDATES_MAX_UPDATES_TO_KEEP`          | `maxUpdatesToKeep`            |

`updates.maxUpdatesToKeep` retains the launched update and up to that number minus one older updates, preferring updates that match the manifest filters. Embedded updates, updates with the same or a newer commit time, and updates from other scopes are protected from deletion, so the total cache can exceed this value.
