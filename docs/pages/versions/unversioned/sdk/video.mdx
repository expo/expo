...
### Video playlists&ensp;<PlatformTags platforms={['ios']} />

`VideoPlaylist` manages a list of video sources while exposing one stable [`VideoPlayer`](#videoplayer). Pass `playlist.player` to [`VideoView`](#videoview); the player object stays the same when the playlist advances.

The `sources` option passed to [`useVideoPlaylist`](#usevideoplaylistoptions) is used only as the initial playlist contents. After creation, update the playlist with `add`, `insert`, `remove`, `clear`, or `replaceAll`.

> **Note:** `VideoPlaylist` is currently available on iOS. Android and web throw unsupported-platform errors until their playlist implementations are added.
...
export default function PlaylistScreen() {
  ...
  return (
    <View style={styles.contentContainer}>
      <VideoView player={playlist.player} style={styles.video} />
      <Text>
        Video {status.currentIndex + 1} of {status.sourceCount}
      </Text>
      <View style={styles.controlsContainer}>
        <Button title="Previous" disabled={!status.canPlayPrevious} onPress={() => playlist.previous()} />
        <Button
        ...
        />
        <Button title="Next" disabled={!status.canPlayNext} onPress={() => playlist.next()} />
      </View>
    </View>
  );
}
