...
## Caching videos (experimental)
> **Info** Caching was introduced during the final stages of the beta phase for `expo-video`, and it has not been thoroughly tested by the community yet.
> If you encounter any issues, please report them at the [expo/expo](https://github.com/expo/expo) repository.

If your app frequently replays the same video, caching can be utilized to minimize network usage and enhance user experience, albeit at the cost of increased device storage usage.
`expo-video` supports video caching on `Android` and `iOS` platforms. This feature can be activated by setting the [`useCache`](#videosource) property of a [`VideoSource`](#videosource) object to `true`.
...
> The cache cannot be used with HLS video sources and DRM-protected videos.
...
- All cached videos can be cleared using the [`cleanVideoCacheAsync`](#videocleanvideocacheasync) function.
