...
<Collapsible summary={"This is a native `expo-video` feature meant for advanced users. Expand to learn more."}>
#### Introduction
...
`VideoAssetTransportProvider` lets you control:
...
`VideoAssetLoadPlan` lets you control how `expo-video` constructs and manages the asset via the following arguments:
...
2. Register the provider - the recommended place to register a provider is an Expo module's `OnCreate` block. This ensures the provider is available before videos start loading.
...
- Basic Dash Support Provider - //TODO@behenate - add a link to the local module in bare expo which provides a limited DASH support for expo-video on iOS
- Expo-video iOS caching - //TODO@behenate - add a link to relevant github code once the PR is merged
