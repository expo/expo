...
Apps also have **read-only** access to bundled assets:

- [`FileSystem.bundleDirectory`](#filesystembundledirectory) - Contains assets bundled with the app (read-only)
  - On iOS: Maps to `Bundle.main.bundlePath`
  - On Android: Uses the `asset://` protocol

> **Prerequisite:** To access a file here, it must be bundled into the native build (e.g., via `bundledAssets` in `app.json`). Files simply existing in your project structure are not automatically available in the bundle directory.
...
To read a bundled asset like `data.json` or a binary file from your bundled assets, use `expo-asset` to obtain a proper file URI:

```ts
import { Asset } from 'expo-asset';
import * as FileSystem from 'expo-file-system/legacy';

// Step 1: Register the asset so the packager provides a usable URI
const asset = Asset.fromModule(require('./assets/data.json'));
await asset.downloadAsync();

// Step 2: Resolve local URI
const fileUri = asset.localUri || asset.uri;

// Read text content
const content = await FileSystem.readAsStringAsync(fileUri);

// Read binary data (e.g., for ArrayBuffer conversion)
...
> **Note**: Bundled assets are read-only. To modify a bundled file, first copy it to a writable location using `copyAsync()`.
