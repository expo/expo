...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      ...
      platform: 'android',
      description:
        'Controls which `READ_MEDIA_*` permissions are added to **AndroidManifest.xml**. By default, all are included. Set it to `false` to exclude them entirely (for example, if you only use `saveToLibraryAsync`), or pass an object like `{ images: true, video: true }` to include only specific permissions.'
    },
  ]}
/>
