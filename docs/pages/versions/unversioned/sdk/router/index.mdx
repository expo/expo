...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      name: 'asyncRoutes',
      description:
        'Enable experimental async routes (lazy loading). In SDK 58 and later, enabled by default on web in development and production, and disabled by default on native. Can be a boolean, a string (`"development"` or `"production"`), or an object with platform-specific values (`{ android, ios, web, default }`). Set `false`, `{ web: false }`, or `{ default: false }` to opt out on web. An explicit platform value takes precedence over `default`. Production async routes are web-only.',
      default: '{ web: true }',
    },
    {
      name: 'platformRoutes',
