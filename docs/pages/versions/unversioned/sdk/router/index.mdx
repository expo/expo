...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      name: 'asyncRoutes',
      description:
        'Enable experimental async routes (lazy loading). In SDK 58 and later, this is enabled by default on web in development and production, and disabled by default on native. Can be a boolean, a string (`"development"` or `"production"`), or an object with platform-specific values (`{ android, ios, web, default }`). Explicit platform values take precedence over `default`. Production async routes are web-only.',
      default: '{ web: true }',
    },
    {
      name: 'platformRoutes',
