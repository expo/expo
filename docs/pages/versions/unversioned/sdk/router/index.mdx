...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      name: 'asyncRoutes',
      description:
        'Enable async routes (lazy loading). Stable and enabled by default on web in SDK 58 and later. Experimental and disabled by default on native. Can be an [`AsyncRouteOption`](#asyncrouteoption) or an object with platform-specific [`AsyncRouteOption`](#asyncrouteoption) values (`{ android, ios, web, default }`). Explicit platform values take precedence over `default`. Production async routes are web-only.',
      default: '{ web: true }',
    },
    {
      name: 'platformRoutes',
    ...
    },
    {
      name: 'redirects',
      description:
        'An array of static redirect rules. Each rule is a [`RedirectConfig`](#redirectconfig) with `source`, `destination`, and optionally `permanent` (defaults to `false`) and `methods` (HTTP methods to redirect).',
      default: 'undefined',
    },
    {
      name: 'rewrites',
      description:
        'An array of static rewrite rules. Each rule is a [`RewriteConfig`](#rewriteconfig) with `source`, `destination`, and optionally `methods` (HTTP methods to rewrite).',
      default: 'undefined',
    },
    {
      name: 'headers',
    ...
    },
    {
      name: 'pageHeaders',
      description:
        "An array of header rules that are set on a specific page's response from the server. Each rule is a [`PageHeadersConfig`](#pageheadersconfig) with `source` and `headers`. Rules only apply to page responses, not API routes or data loaders.",
      default: 'undefined',
    },
    {
