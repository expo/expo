...
<ConfigPluginProperties
  properties={[
    ...
    {
      name: 'disableScrollViewDetection',
      description:
        'Disable the development-only warning shown when a screen inside a native stack or native tabs renders a scroll view that is not on the first child path. Native header collapse, scroll edge effects, and scroll-to-top on tab re-selection only apply to the scroll view found on that path.',
      default: 'false',
    },
    {
