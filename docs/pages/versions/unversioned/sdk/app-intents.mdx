...
On iOS 18.4 and later, `appEntityIdentifier()` tells Siri which entity an [`@expo/ui`](/versions/latest/sdk/ui/) SwiftUI view represents. Use the same entity kind and identifier in the native registration, JavaScript catalog, and view modifier.
...
The view still renders normally on earlier iOS versions or when the kind is not registered. In these cases, the system cannot associate the view with the entity.
