...
With `expo-app-intents`, you can implement Apple Intelligence features, including [on-screen intelligence](https://developer.apple.com/documentation/appintents/providing-contextual-cues-to-apple-intelligence-and-siri). [`appEntityIdentifier()`](#appintentsappentityidentifierentity-id) marks the entity represented by a view so Siri can act on the visible content. You can apply it to [`@expo/ui`](/versions/latest/sdk/ui/) views on iOS 18.4 and later, in a project compiled with Xcode 27 or later. In a project built with an earlier Xcode, the modifier does not apply the identifier and `expo-app-intents` logs a warning once.
...
On iOS 18.4 and later, `appEntityIdentifier()` tells Siri which entity an [`@expo/ui`](/versions/latest/sdk/ui/) SwiftUI view represents. Compile your project with Xcode 27 or later. Use the same entity kind and identifier in the native registration, JavaScript catalog, and view modifier.
...
With an earlier iOS or Xcode version, the view still renders without an entity association. The same applies when the kind is not registered.
