...
On Android, **lock screen controls are required for sustained background playback**. Without them, audio will automatically pause when the app moves to the background (typically after about 3 minutes due to OS limitations). When lock screen controls are active:
