...
export default function AudioPlayerScreen() {
  ...
  const handlePlay = () => {
    ...
    player.setActiveForLockScreen(true, {
      ...
      artworkUrl: 'https://example.com/artwork.jpg', // Remote HTTP/HTTPS URL only
    });
...
> **Note**: The `artworkUrl` field in `AudioMetadata` must be a **remote HTTP or HTTPS URL** (for example, `'https://example.com/artwork.jpg'`). Local assets resolved via `require()` or `Image.resolveAssetSource()` are not supported on Android release builds and may cause the lock screen media player to not appear. Use a remotely hosted image for artwork.
