...
`expo-background-tasks` has a dev tools plugin that can be used to trigger background tasks.

Add the following code to your app to use the plugin:

```tsx
import { useExpoBackgroundTaskDevPlugin } from 'expo-background-task/devtools';

export default App() {
  useExpoBackgroundTaskDevPlugin();
  return (/* rest of your app */)
}
```

To trigger background tasks, start the dev server with `npx expo start` and press `shift + m` to open the list of available dev tools plugins. Select `Open expo-background-task`, and a new Chrome Window will be opened.
