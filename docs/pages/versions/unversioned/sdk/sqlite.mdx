...
<Step label="2">
Use [`getSharedContainer`](filesystem-next/#getsharedcontainerappgroup) from the [`expo-file-system`](filesystem-next/) module to retrieve the path to the shared container:
...
export default function App() {
  const dbDirectory = useMemo(() => {
    if (Platform.OS === 'ios') {
      return Directory.getSharedContainer('group.com.myapp')?.path;
    }
