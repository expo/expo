...
The `expo-sqlite` library provides [`AsyncStorage`](#sqliteasyncstorage) as a drop-in replacement for the [`@react-native-async-storage/async-storage`](https://github.com/react-native-async-storage/async-storage) library. This key-value storage is backed by SQLite. If your project already uses `expo-sqlite`, you can leverage `AsyncStorage` from `expo-sqlite` without needing to add another dependency.

[`AsyncStorage`](#sqliteasyncstorage) offers the same API as the `@react-native-async-storage/async-storage`:

```ts Using the Storage
...
A key benefit of using `AsyncStorage` from `expo-sqlite` is the availability of synchronous APIs for added convenience:

```ts Using the Storage with synchronous APIs
import { AsyncStorage } from 'expo-sqlite';
