...
By default the web backend holds an exclusive [OPFS](https://developer.mozilla.org/en-US/docs/Web/API/File_System_API/Origin_private_file_system) access handle for the whole origin, so opening a second tab of your app fails with `NoModificationAllowedError`. Pass `enableMultiTabSupport` to store the database with a backend that supports concurrent connections:
...
- The two backends lay the database out differently in OPFS, so a database created with `enableMultiTabSupport` is not readable without it, and the other way round. Set the option consistently for a given database.
- Writes are slower, because the default backend relies on exclusive locking that concurrent connections cannot use.
- A transaction cannot span more than one attached database.
