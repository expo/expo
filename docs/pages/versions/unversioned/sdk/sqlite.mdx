...
For convenience and improved developer experience, expo-sqlite provides a Bun-inspired tagged template literals API through the `db.sql` property. This API automatically escapes parameters to prevent SQL injection attacks, and provides automatic type inference based on the query type.
