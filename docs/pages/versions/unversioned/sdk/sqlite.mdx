...
The `expo-sqlite/kv-store` library provides a drop-in replacement for the [`localStorage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage) API. This is useful if you're currently using `localStorage` in your project. To use it, you just need to import the `expo-sqlite/kv-store/installGlobal` module:

```ts Install globalThis.localStorage
import 'expo-sqlite/kv-store/installGlobal';
