...
To share a database with other apps/extensions in the same App Group you can use shared containers. Follow these steps:
<Step label="1">
Configure the App Group in `app.json`:
```json app.json
{
  "expo": {
    "ios": {
...
<Step label="2">
Use [`getSharedContainerUriAsync`](filesystem/#filesystemgetsharedcontaineruriasyncappgroup) from the [`expo-file-system`](filesystem/) module to retrieve the path to the shared container:
...
export default function App() {
  ...
  useEffect(() => {
    const getDbDirectory = async () => {
      if (Platform.OS === 'ios') {
        return await FS.getSharedContainerUriAsync('group.com.myapp');
      }
      return defaultDatabaseDirectory;
    };
    getDbDirectory().then(setDbDirectory);
  }, []);
