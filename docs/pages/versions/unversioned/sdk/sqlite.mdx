...
<Step label="2">
Use [`getSharedContainerUri`](filesystem-next/#getsharedcontaineruriappgroup) from the [`expo-file-system`](filesystem-next/) module to retrieve the path to the shared container:
...
export default function App() {
  const dbDirectory = useMemo(() => {
    if (Platform.OS === 'ios') {
      return Directory.getSharedContainerUri('group.com.myapp')?.path;
    }
