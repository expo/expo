...
Google Play only reports age signals to accounts it has enabled, so your test accounts may not produce the age range you want to test. Use [`setFakeAgeSignals`](#agerangesetfakeagesignalsfake) to report [`FakeAgeSignalsManager`](https://developer.android.com/google/play/age-signals/test-age-signals-api) signals instead of the real ones. Do not call it in production builds.

The requests are unchanged, so the rest of your app works the same way:
...
| `ERR_AGE_RANGE_FAKE_SIGNALS_CONFLICT` | <PlatformTags platforms={['android']} /> | `setFakeAgeSignals` was passed both an `errorCode` and age signals.                                                |
