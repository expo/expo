...
- **ios (_optional_) (_object_)** -- notification configuration specific to iOS.
  - **sound** (_optional_) (_boolean_ or _string_) -- if `true`, notifications posted to this channel will play a sound (the system's default sound). If a string, the specified custom audio file will be played (you will also have to make some changes to `app.json`, see [below](#use-custom-notification-sound-files)). Default: `false`.
