...
`BlurView` supports the `borderRadius` style property on both Android and iOS. The blur effect will be clipped to the rounded corners automatically.

### Non-uniform corner radii

Non-uniform corner radii (e.g., `borderTopLeftRadius`, `borderBottomRightRadius`) are supported on:

- **iOS 26.0+** - Uses the native `UICornerRadius` API with full support for all corner properties
- **Android 11+ (API 30)** - Uses path-based outline clipping for individual corner radii
- **Older versions** - Fall back to uniform `borderRadius` using the smallest corner radius value
