...
<SnackInline label='Get Camera Cutout Info' dependencies={['expo-device']}>
...
export default function App() {
  const [cutoutInfo, setCutoutInfo] = useState(null);

  useEffect(() => {
    const fetchCutoutInfo = async () => {
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
      <Text>Has Cutout: {cutoutInfo?.hasCameraCutout ? 'Yes' : 'No'}</Text>
      {cutoutInfo?.hasCameraCutout && <Text>Type: {cutoutInfo.type}</Text>}
    </View>
  );
}
```
