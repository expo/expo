  ...
  properties={[
    {
      ...
      description: [
        ...
        '',
        'Any file type can be linked. These types are placed in a type-specific location in the native project:',
        '* Images: `.png`, `.jpg`, `.gif`',
        '* Media: `.mp4`, `.mp3`, `.lottie`, `.riv`',
        '',
        'Every other type, such as `.db`, `.glb`, or a model file used by a native library, is copied into the app bundle on iOS and into the `assets` directory on Android.',
        '',
        'Fonts are not linked by this plugin. Use `expo-font` instead.',
        '',
