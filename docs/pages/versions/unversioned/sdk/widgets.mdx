...
On iOS 17 and later, WidgetKit expects widgets to declare a container background. When your layout doesn't declare one, `expo-widgets` applies a default system fill so the widget still renders.

To use a custom background, apply the `containerBackground` modifier from [`@expo/ui`](/versions/latest/sdk/ui/swift-ui/) to any view in the layout.
...
const BackgroundWidget = () => {
  'widget';
  return (
    <VStack modifiers={[containerBackground('#1e293b', 'widget')]}>
      <Text>Hello</Text>
    </VStack>
  );
};

export default createWidget('BackgroundWidget', BackgroundWidget);
```

For a widget that draws its own artwork edge to edge, pass the `clear` color to opt out of any background:
