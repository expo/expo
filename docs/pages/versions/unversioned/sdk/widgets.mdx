...
Use the `widgetURL` modifier from `@expo/ui/swift-ui/modifiers` to make the widget open a single URL when tapped. If you need multiple tappable links, use [`Link`](./ui/swift-ui/link.mdx) inside the widget layout.
...
The push-to-start token and per-activity push token are APNs tokens, not Expo push tokens. Send them to your server and use Apple's ActivityKit APNs endpoint to start, update, or end Live Activities remotely. `expo-widgets` exposes the tokens and renders the content state and it does not send APNs requests for you.
