...
`expo-widgets` enables the creation of iOS home screen widgets and Live Activities using Expo UI components - without the need to write native code. It provides a simple API for creating and updating widgets timeline, as well as starting and managing Live Activities. Layout can be built using expo/ui components and modifiers.
...
The widget name (`'MyWidget'`) must match the `name` field in your widget configuration in **app.json**.
