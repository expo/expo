...
- **(_iOS only_)** `"location"` background mode must be specified in **Info.plist** file. See [background tasks configuration guide](/versions/latest/sdk/task-manager/#configuration-for-standalone-apps).
