...
> **warning** The `Audio` component from `expo-av`, which is documented on this page, has now been depraceted and replaced by an improved version in `expo-audio`. [Learn about `expo-audio`](./audio/).
