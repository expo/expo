...
### Loading multiple weights and styles per font family on web

By default, the CSS generated for the web export doesn't distinguish weights or styles for
fonts loaded through the map syntax above &mdash; each key is treated as a completely separate
font family. To load several files (for example regular, bold, and italic cuts) under one
`fontFamily` and let the browser pick the right face with the CSS `font-weight` and `font-style`
properties, pass `useFonts` (or [`loadAsync`](#loadasyncfontfamilyorfontmap-source)) an array of
[`FontFamilyDefinition`](#fontfamilydefinition)s instead:
...
You can then select the loaded face with the `fontWeight` and `fontStyle` style props, the same
way you would with a system font:
...
> **Note:** `weight` and `style` on [`FontFaceDefinition`](#fontfacedefinition) only affect the
> `@font-face` CSS generated on web. On Android and iOS, only the first `fontDefinitions` entry
> for a given `fontFamily` is loaded &mdash; use separate `fontFamily` names (or the
> [config plugin's per-platform `fontDefinitions`](#configuration-in-app-config)) to select
> different weights and styles natively.
