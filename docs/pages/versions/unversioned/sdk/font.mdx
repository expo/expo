...
A font family can also hold several static font files, for example regular, bold, and italic cuts. To load them under a single `fontFamily` name, pass [`useFonts`](#usefonts) or [`loadAsync`](#loadasyncfontfamilyorfontmap-source) an array of [`FontFamilyDefinition`](#fontfamilydefinition)s instead of a map, then select a face with the `fontWeight` and `fontStyle` style props. To learn more, see [Loading multiple weights and styles for one font family](/develop/user-interface/fonts/#loading-multiple-weights-and-styles-for-one-font-family) in the Fonts guide.
