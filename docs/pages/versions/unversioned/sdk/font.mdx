...
To load multiple weights and styles of the same font family and have the browser pick the correct face on web using the CSS `font-weight` and `font-style` properties, pass `useFonts` an array of [`FontFamilyDefinition`](#fontfamilydefinition)s. See [Loading multiple weights and styles for one font family on web](/develop/user-interface/fonts/#loading-multiple-weights-and-styles-for-one-font-family-on-web).
