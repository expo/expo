...
The preferred way to add fonts to your app is through the provided [config plugin](/config-plugins/introduction/) if you use config plugins in your project ([EAS Build](/build/introduction) or `npx expo run:[android|ios]`). The plugin allows you to embed font files at build time which is more efficient than using [`loadAsync`](#loadasyncfontfamilyorfontmap-source). Learn more in the [fonts guide](/develop/user-interface/fonts/#with-expo-font-config-plugin).
