...
AuthSession allows browser-based authentication (using OAuth or OpenID Connect) for Android, iOS, and web. We recommend implementing authentication using native libraries for third-party providers with development builds. For example, use the [`@react-native-google-signin/google-signin`](/guides/google-authentication/) library for Google authentication and [`react-native-fbsdk-next`](/guides/facebook-authentication/) for Facebook. For more information, see [Authentication](/develop/authentication/).
