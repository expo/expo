...
This is now the preferred method of configuring the splash screen. The other methods are now considered legacy and will be removed in the future.
...
Expo splashscreen provides an out of the box fade animation. It can be configured using the `setOptions` method.
...
If you would prefer to use a custom animation, see the [with-splash-screen](https://github.com/expo/examples/tree/master/with-splash-screen) example on how to apply any arbitrary animations to your splash screen. You can initialize a new project from this example by running `npx create-react-native-app -t with-splash-screen`.
