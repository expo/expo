...
Starting from iOS 11, you can detect when a user starts or stops screen recording your app. This is useful for protecting sensitive content or implementing security measures.

### `addRecordingListener(listener)`

Adds a listener that will fire whenever the app starts or stops being recorded.
...
export default function useScreenRecordingCallback() {
  const addRecordingListener = async () => {
