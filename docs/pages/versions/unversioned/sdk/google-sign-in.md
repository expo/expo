...
2. In `app.json`, set `expo.ios.config.googleSignIn.reversedClientId` to your reversed client ID.
...
5. In `app.json`, set your `expo.ios.config.googleSignIn.reversedClientId` to the value of `REVERSED_CLIENT_ID` in the `GoogleService-Info.plist`.
    ...
    "ios": {
      ...
      "config": {
        "googleSignIn": {
          ...
          "reversedClientId": "<YOUR_REVERSED_IOS_CLIENT_ID>"
        }
      },
