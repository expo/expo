...
> **warning** The `Video` component from `expo-av`, which is documented on this page, has now been depraceted and replaced by an improved version in `expo-video`. [Learn about `expo-video`](video.mdx).
