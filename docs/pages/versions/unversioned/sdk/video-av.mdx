...
> **warning** The `Video` component from expo-av, which is documented on this page, will be replaced by an improved version in `expo-video` in an upcoming release when the new library is stable. [Learn about `expo-video`](video.mdx).
