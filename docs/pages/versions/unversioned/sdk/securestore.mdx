...
**The `requireAuthentication` option is not supported in Expo Go when Face ID is available due to a missing `NSFaceIDUsageDescription` key.**
...
Additionally, any data protected with the `requireAuthentication` option set to `'biometry'` will become inaccessible if there are changes to the user's biometric settings, such as adding a new fingerprint.
