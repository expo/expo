...
sourceCodeUrl: 'https://github.com/expo/expo/tree/main/packages/expo-calendar/src'
...
> **important** To provide quicker updates, `expo-calendar` is currently unsupported in Expo Go and Snack. To use it, create a [development build](/develop/development-builds/create-a-build/).
...
To only add events without reading existing calendar data, enable the `writeOnlyAccess` option in the [config plugin](#configuration-in-app-config) and request write-only access by passing `true` to [`requestCalendarPermissions`](#requestcalendarpermissions). This is sufficient for methods like [`ExpoCalendar.createEvent`](#createeventdetails). Methods that read calendar data, such as [`getCalendars`](#getcalendars), [`listEvents`](#listevents), or [`presentPicker`](#presentpicker), require full calendar access.
