...
Only the alpha channel of the `maskElement` matters: opaque pixels reveal content, transparent pixels hide it. Use a `LinearGradient` (from `expo-linear-gradient`) that goes from opaque to transparent &mdash; `'black'` (for example) to `'transparent'` below &mdash; to fade content out along an axis.
