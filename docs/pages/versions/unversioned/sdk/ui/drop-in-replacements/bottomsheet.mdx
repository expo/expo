...
`@gorhom/bottom-sheet` renders inline at the bottom of its parent view. This component uses native modal presentation on Android and iOS, and an HTML `<dialog>` on web.
...
| `BottomSheetBackdrop`      | <NoIcon />  | The native sheet or HTML dialog handles the backdrop      |
| `BottomSheetHandle`        | <NoIcon />  | The native sheet or HTML dialog handles the drag indicator |
