...
> **info** For a cross-platform picker, see [`Picker`](../universal/picker) &mdash; built on top of `ExposedDropdownMenuBox` on Android.
...
> When using an uncontrolled `TextField` as the anchor, pass `key={selected}` to force a remount when the selected value changes &mdash; `defaultValue` is only read on mount.
