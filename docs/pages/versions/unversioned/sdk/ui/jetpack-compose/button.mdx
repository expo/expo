...
export default function ButtonWithIconsExample() {
  return (
    <Host matchContents>
      {/* Leading icon — use contentPadding to match M3 spec */}
      <Button onClick={() => {}} contentPadding={{ start: 16 }}>
      ...
      <FilledTonalButton onClick={() => {}} contentPadding={{ start: 16 }}>
