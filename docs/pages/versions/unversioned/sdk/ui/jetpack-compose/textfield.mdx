...
```tsx FilledTextFieldExample.tsx
import { useState } from 'react';
import { Host, TextField, Text } from '@expo/ui/jetpack-compose';

export default function FilledTextFieldExample() {
...
`BasicTextField` is the unstyled Compose primitive, with no container, indicator, or padding. Style it yourself with [modifiers](modifiers) and supply decoration such as a placeholder through `DecorationBox`, placing `InnerTextField` where the editable text should render.

```tsx BasicTextFieldExample.tsx
import { useState } from 'react';
import { Host, BasicTextField, Box, Text } from '@expo/ui/jetpack-compose';
import {
  background,
  clip,
  fillMaxWidth,
    ...
    <Host matchContents>
      <BasicTextField
        ...
        <BasicTextField.DecorationBox>
          <Box>
            {value.length === 0 ? <Text color="#9ca3af">Search…</Text> : null}
