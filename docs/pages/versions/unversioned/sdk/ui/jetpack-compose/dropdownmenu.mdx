...
Use `shadowElevation` to set the popup shadow elevation in dp. Set it to `0` to remove the shadow, or omit it to use Material3's default. Modifiers on `DropdownMenu` apply to the trigger container, so shadow modifiers do not change the popup shadow.
