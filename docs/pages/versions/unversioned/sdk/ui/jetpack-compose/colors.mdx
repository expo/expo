...
- **Seeded from a color:** When you pass a `seedColor`, the full palette is derived using the same algorithm that Material 3 Dynamic Colors use for wallpaper-based colors. It is independent of the wallpaper and works on Android 8.0 (API 26) and above.

> **Note:** Custom colors require Android 8.0 (API 26) or newer. On Android 7 (API 24 and 25), a `seedColor` and component color props are ignored. The palette falls back to the Material 3 baseline, and components use their default colors.
