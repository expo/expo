...
`@expo/ui/jetpack-compose` exposes the [Material 3 color palette](https://m3.material.io/styles/color/system/overview) used by Jetpack Compose so you can pick a source (wallpaper, light/dark, or a seed color) and have every component under a [`<Host>`](./host) theme from it consistently.
...
- **Seeded from a color** — when you pass a `seedColor`, the full palette is derived using the same algorithm Material 3 Dynamic Colors use for wallpaper-based colors. Works on every Android API level and is independent of the wallpaper.
...
Pass args to [`useMaterialColors()`](#usematerialcolorsoptions) to compute a palette on demand, even outside a `<Host>`. `scheme` takes `'light'` or `'dark'`; omit it to follow the system.
