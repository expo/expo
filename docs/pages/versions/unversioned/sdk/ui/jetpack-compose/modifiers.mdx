...
Makes the component respond to both short-tap and long-press gestures. Wraps Compose's `Modifier.combinedClickable`. Useful for opening a [`DropdownMenu`](./dropdownmenu.mdx) on long-press while keeping a separate short-tap action on the same view.
