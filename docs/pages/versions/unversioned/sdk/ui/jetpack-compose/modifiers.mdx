...
> **info** You can also create custom modifiers that work with any Expo UI component. See the [Custom Jetpack Compose components](extending) guide for details.
...
> **info** Works only inside Android widgets built with `expo-widgets`, on Android 12 (API level 31) and later.

Jetpack Compose has no exact equivalent of Glance's `cornerRadius`, so Expo UI ignores this modifier. Use [`clip(Shapes.RoundedCorner(radius))`](#clipshape) to round corners in an app.
