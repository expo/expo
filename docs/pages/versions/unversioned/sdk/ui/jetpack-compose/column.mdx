...
```tsx
import { Host, Column, Text, useMaterialColors } from '@expo/ui/jetpack-compose';
  ...
  return (
    <Host style={{ flex: 1 }}>
      <Column
        ...
        <Text color={colors.onBackground}>First</Text>
        <Text color={colors.onBackground}>Second</Text>
        <Text color={colors.onBackground}>Third</Text>
      </Column>
    </Host>
  );
}
