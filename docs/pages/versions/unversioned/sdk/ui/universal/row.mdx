...
A horizontal layout container that arranges its children from start to end. Delegates to SwiftUI's [`HStack`](../swift-ui/hstack) on iOS, Jetpack Compose's [`Row`](../jetpack-compose/row) on Android, and a flex `View` on web.
