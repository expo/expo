...
A text input that routes to [`TextField`](../swift-ui/textfield) from `@expo/ui/swift-ui` on iOS, [`TextField`](../jetpack-compose/textfield) from `@expo/ui/jetpack-compose` on Android, and React Native's [`TextInput`](https://reactnative.dev/docs/textinput) on web.

The API mirrors React Native's `TextInput`, with two changes: [`value`](#value) and [`selection`](#selection) are observable state objects (created with `useNativeState`), and [`onChangeText`](#onchangetext) can be a worklet for synchronously updating the state on the UI thread.
