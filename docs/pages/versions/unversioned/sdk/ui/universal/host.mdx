...
### Automatic hosting and performance

Universal components can be rendered without an explicit `Host`; if no parent host boundary exists, the component creates one with default sizing. This keeps standalone controls concise:

```tsx StandaloneButtonExample.tsx
import { Button } from '@expo/ui';

export default function StandaloneButtonExample() {
  return <Button label="Press me" onPress={() => alert('Pressed')} />;
}
```

When rendering multiple adjacent universal components, prefer wrapping the group in one `Host`. This avoids creating one platform-native host view per standalone component.

```tsx GroupedButtonsExample.tsx
import { Host, Column, Button } from '@expo/ui';

export default function GroupedButtonsExample() {
  return (
    <Host matchContents>
      <Column spacing={8}>
        <Button label="First" onPress={() => {}} />
        <Button label="Second" onPress={() => {}} />
        <Button label="Third" onPress={() => {}} />
      </Column>
    </Host>
  );
}
