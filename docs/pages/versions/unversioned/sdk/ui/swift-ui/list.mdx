...
Enable selection, deletion, and reordering of list items using the `ForEach` component with `onDelete` and `onMove` props. Use the [`environment`](modifiers/#environmentkey-value) modifier to enable edit mode, the [`tag`](modifiers/#tagtag) modifier to identify items, and the [`selection`](#selection) prop to control selected items. Use [`moveDisabled`](modifiers/#movedisableddisabled) and [`deleteDisabled`](modifiers/#deletedisableddisabled) modifiers to disable these actions on individual items.
  ...
  return (
    <Host style={{ flex: 1 }}>
      <List
        ...
        modifiers={[environment(EnvironmentKey.editMode, editMode ? 'active' : 'inactive')]}>
        <Section title="Settings">
