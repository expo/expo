...
Use `ignoreSafeArea="keyboard"` when React Native is already handling keyboard avoidance (for example, with `KeyboardAvoidingView`), to prevent the SwiftUI host from applying its own keyboard inset on top.
...
export default function IgnoreAllSafeAreasExample() {
  return (
    <Host ignoreSafeArea="all" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
      <VStack>
