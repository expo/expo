...
Use the [`presentationDetents`](/versions/unversioned/sdk/ui/swift-ui/modifiers/#presentationdetentsdetents) modifier on `BottomSheet.Content` to control the available heights. You can use:
  ...
  return (
    <Host style={{ flex: 1 }}>
      <VStack>
        ...
        <BottomSheet isPresented={isPresented} onIsPresentedChange={setIsPresented}>
          <BottomSheet.Content
            modifiers={[presentationDetents(['medium', 'large', { fraction: 0.3 }, { height: 200 }])]}>
...
Use the [`presentationBackgroundInteraction`](/versions/unversioned/sdk/ui/swift-ui/modifiers/#presentationbackgroundinteractioninteraction) modifier to allow interactions with content behind the sheet.
...
Use the [`interactiveDismissDisabled`](/versions/unversioned/sdk/ui/swift-ui/modifiers/#interactivedismissdisabledisdisabled) modifier to prevent users from dismissing the sheet by swiping.
...
When using React Native content with `flex: 1`, omit the `matchContents` prop on `RNHostView` and use [`presentationDetents`](/versions/unversioned/sdk/ui/swift-ui/modifiers/#presentationdetentsdetents) to control the sheet height.
