...
Use the `searchable` modifier to attach a native SwiftUI search field to a view. Pass a [`useNativeState`](/versions/latest/sdk/ui/swift-ui/usenativestate/) observable to hold the search text, which maps to SwiftUI's `Binding<String>`. Provide `onChange` to update your data as the search text changes.

```tsx SearchableExample.tsx
