...
When React Native views are placed inside SwiftUI components like [`BottomSheet`](bottomsheet) or [`Popover`](popover) or [`HStack`](hstack) etc, the layout systems need to communicate. `RNHostView` bridges this gap:
