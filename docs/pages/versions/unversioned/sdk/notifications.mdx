...
<ConfigPluginProperties
  properties={[
    ...
    {
      name: 'enableBigPictureStyle',
      default: 'false',
      platform: 'android',
      description:
        "Whether a remote notification's image fills the notification when the user expands it, using Android's `NotificationCompat.BigPictureStyle`. When `false`, the image appears only as the notification's large icon. This style replaces the expanded text layout, so the full notification body no longer appears.",
    },
    {
      name: 'sounds',
      description:
