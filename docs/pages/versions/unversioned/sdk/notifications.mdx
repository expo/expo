...
- Fetch a native device push token, so you can send push notifications with FCM (for Android) and APNs (for iOS)
- Fetch an Expo push token, so you can send push notifications with [Expo Push API](/push-notifications/sending-notifications/)
...
| ---------------------------------- | ---------- | ----------------------------------------------------------------------- |
| false                              | Foreground | `NotificationReceivedListener` & `BackgroundNotificationTask`           |
...
Please refer to the `ExpoPushMessage` type in the [`expo-server-sdk`](https://github.com/expo/expo-server-sdk-node/blob/b470314023d8393073ecdbc07927b32d163af790/src/ExpoClient.ts#L346) package.
