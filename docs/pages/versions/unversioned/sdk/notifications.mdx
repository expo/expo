...
> **info** When the app is not running or killed, and is restarted by tapping on a notification, your `NotificationResponseReceivedListener` may or may not be triggered. To reliably capture the response, we recommend using `useLastNotificationResponse`. 
...
- In the array under `ios.infoPlist.UIBackgroundModes` key in your [app config](/versions/latest/config/app/), add the values `remote-notification` and `processing`.
