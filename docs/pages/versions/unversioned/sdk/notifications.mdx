...
- **projectId (_string_)** -- The ID of the project to which the token should be attributed. Defaults to [`Constants.expoConfig.extra.eas.projectId`](/versions/latest/sdk/constants/#easconfig) exposed by `expo-constants`. If you are not using EAS Build, it will fallback to [`Constants.manifest.projectId`](/versions/latest/sdk/constants/#constantsmanifest).
- **experienceId (_string_)** -- **deprecated**. The ID of the experience to which the token should be attributed. Defaults to [`Constants.manifest.id`](/versions/latest/sdk/constants/#constantsmanifest) exposed by `expo-constants`. When building with EAS Build, or in the bare workflow, **this is required** and you must provide a value which takes the shape `@username/projectSlug`, where `username` is the Expo account that the project is associated with, and `projectSlug` is your [`slug`](../config/app.mdx#slug) from Expo config.
