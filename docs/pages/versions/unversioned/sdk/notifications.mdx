...
- Starting from Android 12 (API level 31), to schedule the notification that triggers at the exact time, the `SCHEDULE_EXACT_ALARM` permission has to be granted.
  Since Android 14 (API level 34) this permission is denied by default and requieres an explicit opt-in from the user. You should use the
  [`requestScheduleExactPermissionsAsync`](#requestscheduleexactpermissionsasync) method to request this permission. If the permission isn't granted the
  notification will be delivered, but the time will be approximate.
  You can read more about the exact alarm permission in the [official documentation](https://developer.android.com/about/versions/14/changes/schedule-exact-alarms).
