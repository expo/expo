...
- Starting from Android 12 (API level 31), to schedule a notification that triggers at an exact time, you need to add
  ...
  The `alarmClock` trigger option also requires an exact alarm permission. On Android 13 (API level 33) and higher, alarm clock, timer, and calendar apps
  can declare `<uses-permission android:name="android.permission.USE_EXACT_ALARM"/>` instead, which the system grants without a user prompt.
  [Google Play restricts](https://support.google.com/googleplay/android-developer/answer/13161072) `USE_EXACT_ALARM` to those app categories.
...
<AndroidPermissions permissions={['RECEIVE_BOOT_COMPLETED', 'SCHEDULE_EXACT_ALARM', 'USE_EXACT_ALARM']} />
