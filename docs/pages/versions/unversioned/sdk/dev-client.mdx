...
These properties set build-time defaults. To change the behavior for one launch, add `__expo_disable_onboarding=1`, `__expo_show_menu_at_launch=0` or `__expo_tools_button=0` to the launch URL. See [Deep linking to an update's URL](/develop/development-builds/development-workflows/#deep-linking-to-an-updates-url).
