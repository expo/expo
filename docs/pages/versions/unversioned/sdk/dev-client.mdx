...
These properties set build-time defaults. To change the saved settings from a launch URL, add `__expo_disable_onboarding=1`, `__expo_disable_fab=1` or `__expo_disable_auto_launch=1` to it. See [Deep linking to an update's URL](/develop/development-builds/development-workflows/#deep-linking-to-an-updates-url).
