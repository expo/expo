...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      name: 'toolsButton',
      description:
        'Whether to show the floating tools button by default. Use `setToolsButtonVisible` from `expo-dev-client` to change it at runtime.',
      default: 'true',
    },
    {
      name: 'embeddedBundle',
      description:
        'Whether to enable loading an embedded JavaScript bundle from the launcher. When enabled and a bundle file is present in the app, a "Load embedded bundle" option appears in the launcher.',
      default: 'false',
    },
    {
      name: 'skipOnboarding',
...
These properties set build-time defaults. To skip onboarding, hide the tools button, or keep the dev menu closed for one launch, add `__expo_disable_onboarding=1`, `__expo_disable_fab=1` or `__expo_disable_auto_launch=1` to the launch URL. See [Deep linking to an update's URL](/develop/development-builds/development-workflows/#deep-linking-to-an-updates-url).
