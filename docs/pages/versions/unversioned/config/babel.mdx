...
- `process.env.EXPO_OS`: The operating system of the device. This can be used as a replacement for `Platform.OS`. One of `android`, `ios`, `web`.
...
> This option has an effect only when the `disableImportExportTransform` option is set to `false`. On Android and iOS, `disableImportExportTransform` defaults to `false`, and on web it defaults to `true` to allow for tree shaking.
...
> **info** This section is for advanced developers who want to use `babel-preset-expo` with a custom bundler implementation.
