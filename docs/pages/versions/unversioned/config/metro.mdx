...
> **info** Standard Tailwind does not support native platforms, this is web-only. Use a compatability library like [Nativewind](https://www.nativewind.dev/) for universal support.

[Tailwind](https://tailwindcss.com/) can be used with Metro for web. The following files are modified:

<FileTree files={['app.json', 'package.json', 'global.css', 'tailwind.config.js', 'index.js']} />

First, ensure you're using Metro for web in your **app.json**:
...
Then simply configure Tailwind according to the [Tailwind PostCSS docs](https://tailwindcss.com/docs/installation/using-postcss).

<Step label="1">

Install `tailwindcss` and its peer dependencies, and create your **tailwind.config.js** file.
...
> Expo Router users can use `./index.js` or `./app/_layout.js`, they both work the same.

</Step>

<Step label="5">

Start your project, you may want to clear the transform cache for good measure:
...
If you have a custom `config.cacheStores` in your **metro.config.js**, you'll need to extend the Expo superclass of `FileStore`:
...
> Transformations are heavily caches in Metro. If you update something, use the `--clear` flag to see updates, e.g. `npx expo start --clear`.
...
If the caller doesn't have `engine`, `platform`, `bundler`, etc. then ensure you are using `@expo/metro-config/babel-transformer` for the transformer. If you're using a custom transformer then it may need to extend the Expo transformer.
