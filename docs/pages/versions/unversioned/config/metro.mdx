...
Project's that don't use [Expo Prebuild](/workflow/prebuild), must configure native files to ensure Expo Metro config is always used to bundle the project.
...
+     cliFile = new File(["node", "--print", "require.resolve('@expo/cli')"].execute(null, rootDir).text.trim()).getAbsolutePath()
