...
The `debugId` is a deterministic hash of the bundle's contents without the external bundle splitting references. This is effectively the same value used to create a chunks filename.

`@expo/metro-config` only injects `debugId` during `npx expo export`, which is currently used for web build, and EAS Update. The initial native build however, is performed with the `npx expo export:embed` command which does not currently inject a `debugId`.

`debugId` can be used with services like [Sentry](https://sentry.io/welcome/) for error reporting.
