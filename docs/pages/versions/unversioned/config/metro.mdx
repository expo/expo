...
[PostCSS](https://github.com/postcss/postcss) can be customized by adding a `postcss.config.js` file to the root of your project. This file should export a function that returns a PostCSS configuration object. For example:
...
This can be used to enable scss syntax, for example:
...
You will additionally need to add the `scss` extension to your `metro.config.js`:
...
// Add the scss extension to the list of supported extensions.
config.resolver.sourceExts.push('scss');

module.exports = config;
```

The Expo Metro transformer can currently only support `css`, `scss`, and `sass` extensions. `scss` and `sass` must be added to the `sourceExts` array in `metro.config.js` to be observed by the file watcher.
