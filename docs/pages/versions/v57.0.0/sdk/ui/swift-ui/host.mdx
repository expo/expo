...
Use `ignoreSafeArea="all"` when React Native is already positioning the host, for example with `react-native-keyboard-controller`. It stops the SwiftUI host from applying its own keyboard inset and moving the content a second time.

`ignoreSafeArea="keyboard"` removes only the keyboard inset and still applies the container inset. A `matchContents` host pinned over the home indicator therefore grows by the bottom inset, which leaves empty space below the content.
...
Use `ignoreSafeArea="container"` to remove only the container safe area (notch, home indicator, status and navigation bars). The keyboard safe area still applies. This is the default, so the example below sets a value it would get anyway. Use `ignoreSafeArea="none"` to apply every safe area region instead.
