...
> **Note:** `matchContents` only works when the child has a natural size of its own. If the child just fills whatever space it is given (`flex: 1`, percentage widths, or text that wraps to the available width), Jetpack Compose and Yoga keep resizing each other and layout can loop forever. Let the child size to its content instead, for example with `alignSelf: 'flex-start'`, and give long text a `maxWidth`.
