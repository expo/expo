...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      ...
      default: '{appBundleIdentifier}.expo-sharing-extension',
    },
    {
      name: 'ios.appGroupId',
    ...
    },
    {
      ...
      default: '{ supportsText: true, supportsWebUrlWithMaxCount: 1 }',
    },
    {
...
> **info** On iOS, the system shows your app in the share sheet only when the source app offers an item that matches the activation rule. The rule matches the [uniform type identifier](https://developer.apple.com/documentation/uniformtypeidentifiers) of the shared item, not the subject of the content. Many apps share a link to a video or a photo instead of the file itself. For example, `supportsMovieWithMaxCount` matches a video file, but it does not match a link to a video. To receive those shares, also set `supportsWebUrlWithMaxCount` and `supportsText`.
