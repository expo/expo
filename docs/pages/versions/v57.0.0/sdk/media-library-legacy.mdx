...
Enabling `resolveWithFullInfo: true` in `getAssetsAsync` significantly increases request time on Android (~5×), because the library fetches EXIF and location data per photo. On iOS, the impact is much smaller (~10–20%), because the query results already include the data.
