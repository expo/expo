...
Enabling `resolveWithFullInfo: true` in `getAssetsAsync` significantly increases request time on Android (~5×), because EXIF and location data must be fetched per photo. On iOS, the impact is much smaller (~10–20%), because the data is already included in the query results.
