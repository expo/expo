...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      ...
      platform: 'ios',
      description:
        'A string to set the [`NSMotionUsageDescription`](#permission-nsmotionusagedescription) permission message. `getMotionActivityAsync` and `watchMotionActivityAsync` require this key. Set this property to `false` to delete the key from **Info.plist**. `false` only changes **Info.plist**. It does not remove the Core Motion code from the iOS build, so App Store Connect can reject the archive with `ITMS-90683`. To use your own message, pass a string here, or omit this property and set `ios.infoPlist.NSMotionUsageDescription`. Do not combine `false` with `ios.infoPlist`, because the plugin deletes the key that `ios.infoPlist` adds.',
    ...
    {
      name: 'isAndroidMotionActivityEnabled',
      platform: 'android',
      description:
        'A boolean to enable the [`ACTIVITY_RECOGNITION`](#permission-activity_recognition) and `com.google.android.gms.permission.ACTIVITY_RECOGNITION` permissions. `getMotionActivityAsync` and `watchMotionActivityAsync` require these permissions.',
      default: 'false',
    },
    {
...
<ContentSpotlight
  ...
  src="/static/images/sdk/location/ios-simulator-location.webp"
