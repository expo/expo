...
On Android and iOS, recordings are saved in the app's cache directory by default. The system may delete cache files when the device runs low on storage. To keep new recordings in a more persistent location, pass `{ ...RecordingPresets.HIGH_QUALITY, directory: 'document' }` to `useAudioRecorder` or `prepareToRecordAsync()` to store them in the app's document directory. For existing recordings already saved in cache, move them with [`expo-file-system`](./filesystem/).
