...
// Define the task at module scope so the background runtime can find it.
// This needs to be called in the global scope, not in a React component.
...
> **info** **Note:** [`TaskManager.defineTask`](#taskmanagerdefinetasktaskname-taskexecutor) must be called at module scope, not inside a React component. The example above places it at module scope inside the screen file, which works for non-router apps. In Expo Router projects, move it to a separate file (for example **tasks.ts**) and import that file at the top of your root layout (**app/\_layout.tsx**) so the task is registered before any navigation runs.
