...
Pass [`snapPoints`](#snappoints) to let the user drag the sheet between multiple resting heights. Use the semantic values `'half'` and `'full'` for cross-platform parity; the `{ fraction }` and `{ height }` forms are honored precisely on iOS and web.
