...
- [`Snackbar`](#snackbar) is a styling-only child of [`SnackbarHost`](#snackbarhost). Pass one to override colors or place the action on a new line. Maps to the `snackbar` lambda parameter of `SnackbarHost(hostState) { data -> Snackbar(data, ...) }` in Compose.
