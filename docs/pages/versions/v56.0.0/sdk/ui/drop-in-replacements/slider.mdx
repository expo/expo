...
- On iOS, `maximumTrackTintColor` and `thumbTintColor` have no visual effect because SwiftUI's `Slider` only exposes the minimum (active) track tint. `minimumTrackTintColor` works on both platforms.
