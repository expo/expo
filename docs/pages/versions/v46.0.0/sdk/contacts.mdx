...
Async `boolean`, indicating whether the Contacts API is available on the current device. Currently, this resolves to `true` on Android and iOS only.
...
> **Note**: For Android users, the Expo Go app does not have the required `WRITE_CONTACTS` permission to write to Contacts. To do this, you must build a [standalone app](/archive/classic-updates/building-standalone-apps) and add permission through there.
