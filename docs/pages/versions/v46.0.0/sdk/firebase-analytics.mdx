...
### Expo Go: limitations and configuration
...
> This limitation only applies to the Expo Go app in the App and Play stores; standalone builds, custom clients and bare apps support the full native Firebase Analytics experience.
 ...
 * are even using it.
 *
 * If lots of users are opening the card and then swiping through photos, just
...
onPressProfileButton = uid => {
  Analytics.logEvent('ExpandProfile', {
    /*
     * We want to know if the user came from the swipe card as
