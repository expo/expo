...
```tsx
import { useState } from 'react';
import { DateRangePicker, type DateRangeSelection, Host } from '@expo/ui/jetpack-compose';
...
import { DateRangePickerDialog, type DateRangeSelection, Host } from '@expo/ui/jetpack-compose';
...
export default function DateRangePickerDialogExample() {
  ...
  return (
    <>
      <Button title="Select dates" onPress={() => setVisible(true)} />
      {visible && (
        <Host>
