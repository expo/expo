...
This guide explains how to create custom Jetpack Compose components and modifiers that integrate with Expo UI using the [Expo Modules API](/modules/overview/).
...
1. **Props data class**: annotated with `@OptimizedComposeProps`, implements `ComposeProps`, and includes a `modifiers: ModifierList` field for the [`modifiers`](modifiers) prop.
