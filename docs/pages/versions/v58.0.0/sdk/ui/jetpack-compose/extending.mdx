...
sidebar_title: Custom Jetpack Compose components
sidebar_order: 1
...
<Prerequisites>
  <Requirement title={<><CODE>@expo/ui</CODE> installed</>}>
    See [Jetpack Compose components in Expo UI](../jetpack-compose) for more information.
  ...
  <Requirement title="Basic familiarity with the Expo Modules API and Jetpack Compose">
...
The `register` lambda receives the raw map sent from JavaScript, the current `ComposableScope` (use it for scope-dependent modifiers like `weight` or `align`), a `ConverterContext`, and an event dispatcher. Most modifiers only need the map and the `ConverterContext`, which they pass to `recordFromMap<T>(map, converterContext)`.
...
- Use the [built-in Jetpack Compose components](../jetpack-compose) that come with Expo UI.
