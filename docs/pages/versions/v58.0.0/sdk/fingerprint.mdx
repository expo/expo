...
sourceCodeUrl: 'https://github.com/expo/expo/tree/sdk-58/packages/@expo/fingerprint'
...
Placed in your project root, **fingerprint.config.js** allows you to specify custom hash calculation configuration beyond what is configurable in the **.fingerprintignore**. For supported configurations, see [Config](#config), [`FingerprintPreset`](#fingerprintpreset), and [`SourceSkips`](#sourceskips).
...
const config = {
  // Uncomment to switch presets (the default is 'balanced'). See FingerprintPreset for the options.
  // preset: 'strict',
  sourceSkips: [
