---
title: Third-party libraries supported in Expo Go
sidebar_title: Overview
description: A set of third-party libraries which support for is included by default in Expo Go environment.
inExpoGo: true
hideTOC: true
---

import { CodeSquare01Icon } from '@expo/styleguide-icons/outline/CodeSquare01Icon';
import { TerminalBrowserIcon } from '@expo/styleguide-icons/outline/TerminalBrowserIcon';

import { BoxLink } from '~/ui/components/BoxLink';

The Expo Go playground enables students and learners to quickly try out building native Android and iOS apps. It supports a curated list of third-party libraries that are community-driven, provide APIs for common app functionalities, and are tested with each Expo SDK release.

The difference between libraries listed in this section and any other third-party library is that support for the former is built into the Expo Go environment. You may use any library of your choice with development builds.

## Learn more

<BoxLink
  title="Using other third-party libraries"
  description="Learn how to use other third-party npm libraries in your project."
  Icon={CodeSquare01Icon}
  href="/workflow/using-libraries/#third-party-libraries"
/>

<BoxLink
  title="Introduction to development builds"
  description="Learn why use development builds, and how to get started."
  Icon={TerminalBrowserIcon}
  href="/develop/development-builds/introduction/"
/>
