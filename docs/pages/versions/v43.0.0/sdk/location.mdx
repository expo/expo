...
- `Permissions.LOCATION` permission must be granted. On iOS it must be granted with `Always` option — see [Permissions.LOCATION](permissions.mdx#permissionslocation) for more details.
- `"location"` background mode must be specified in **Info.plist** file. See [background tasks configuration guide](task-manager.mdx#configuration). **(_iOS only_)**
...
- `Permissions.LOCATION` permission must be granted. On iOS it must be granted with `Always` option — see [Permissions.LOCATION](permissions.mdx#permissionslocation) for more details.
- Geofencing task must be defined in the top-level scope, using [TaskManager.defineTask](task-manager.mdx#taskmanagerdefinetasktaskname-taskexecutor).
...
To make it work in the background, it uses [TaskManager](task-manager.mdx) Native API under the hood. Make sure you've followed the required steps detailed [here](#configuration).
...
`LocationPermissionResponse` extends [PermissionResponse](permissions.mdx#permissionresponse) type exported by `unimodules-permission-interface` and contains additional platform-specific fields:
