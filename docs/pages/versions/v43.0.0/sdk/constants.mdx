...
> ⚠️ **This property is deprecated and will be removed in SDK 44. Please implement it on your own using `expo-application`'s [`androidId`](application.mdx#applicationandroidid) on Android and a storage API such as [`expo-secure-store`](securestore.mdx) on iOS and `localStorage` on Web.**
