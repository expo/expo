...
> **warning** **Warning**: `patch-project` is an experimental feature.
...
`patch-project` uses an approach to generate and automatically apply patches that is inspired by Git. Using this command line tool requires the following steps in your project:
...
Once you have generated patches, they are automatically applied when subsequently running `npx expo prebuild` command. The `patch-project` config plugin detects the existing patches and applies them to restore your customizations.
...
- **Patches are applied automatically** when running `npx expo prebuild` command subsequently. This is an advantage over tools like `patch-package` (commonly used for generating patches for npm libraries), which do not preserve and automatically apply patches during the prebuild process.
