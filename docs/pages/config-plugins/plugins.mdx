...
<BoxLink
  title="What are config plugins?"
  description="For information about config plugins terminology, and their characteristics, check out the introduction."
...
While it is recommended that you write config plugins in TypeScript, as this will proide intellisense for the configuration objects, your app config is ultimately evaluated by Node.js, which does not recognize TypeScript code by default. You need to add a parser for the TypeScript files from the the **plugins** directory to **app.config.ts** file.
...
The `plugins` array uses `withPlugins` method under the hood to chain the plugins. If you are using so many config plugins with complex configuration that becomes difficult to read and edit your dynamic app config, you can use the `withPlugins` method directly to chain plugins together and execute them in order.
...
Expo config plugins come mostly from Node.js modules. You can install them just like other libraries in your project.
