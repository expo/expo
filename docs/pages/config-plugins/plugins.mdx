...
While it is convenient to write config plugins in TypeScript, because you can benefit from intellisense on the configuration objects, your app config is evaluated by Node.js, which does not recognize TypeScript by default. You need to add a parser for the TypeScript files from the the **plugins** directory to **app.config.ts** file.
