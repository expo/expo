...
- **Instructions in your README**: If the plugin is tied to a React Native module, then you should document manual setup instructions for the package. If anything goes wrong with the plugin, developers should be able to manually add the project modifications which were automated by the plugin. This also allows you to support projects that are not using [CNG](/workflow/continuous-native-generation/).
...
When a node module is installed with the `npx expo install` command, if it includes a config plugin, it will be added to the project's app config automatically. This makes setup a easier and helps prevent users from forgetting to add a plugin. However, this does come with a couple of caveats:
