...
1. Packages can use both use a root **app.plugin.js** file or use the `main` entry point to ship a config plugin. `expo install` only adds config plugins using **app.config.js** automatically to the app manifest. This rule was added added to prevent popular packages like `lodash` from being mistaken for a config plugin and breaking the prebuild.
