...
- **Need to modify text with regexes or replace functions**: You need to make complex text changes that existing mod plugins do not support. Expo uses dangerous mods internally for large file system refactoring, for example, when a library's name changes.
