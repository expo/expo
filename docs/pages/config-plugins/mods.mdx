...
Config plugins use "mods" to modify native project files during the prebuild process. Mods are powerful tools that allow you to make changes to platform-specific files such as **AndroidManifest.xml**, **Info.plist**, and other native configuration files without having to manually edit them.

This guide explains what mods are, how they work, and how to use them effectively when creating config plugins for your Expo project.

## What are mods

A modifier (mod for short) is an asynchronous function that accepts a config and a data object, then manipulates and returns both as an object.
...
after the initial reading, which means you can use it to perform actions _during_ code generation. If possible, you should attempt to use basic plugins instead of mods, as they're simpler to work with.
...
Instead you should use the mod plugins provided by `expo/config-plugins`:
...
For example, you want to write a mod to update the Xcode Project's "product name". You'll create a config plugin file that uses the `withXcodeProject` mod plugin.
...
1. **Project-integrated plugins**: These plugins live locally within your project, making them easy to customize and maintain alongside your app's code. They are ideal for project-specific customizations.
...
With project-integrated plugins, you can implement plugins directly in your project in several ways:
...
Standalone package plugins are published to npm and can be shared across multiple projects, making them ideal for reusable functionality.
...
Sometimes you want your package to export React components and also support a plugin. To do this, multiple entry points need to be used because the transpilation (Babel preset) could be different. If an **app.plugin.js** file is present in the root of a Node module's directory, it'll be used instead of the package's `main` file.
