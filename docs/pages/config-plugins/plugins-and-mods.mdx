...
If an **app.plugin.js** file is present in the root of a Node module's directory, it'll be used instead of the package's `main` file. If your package.json uses [exports](https://nodejs.org/api/packages.html#package-entry-points), remember to include the **app.plugin.js** file.
