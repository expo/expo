...
On a physical iOS device, Expo Go opens a project from a development server only when Expo CLI and Expo Go are signed in to the same Expo account.
...
> **info** This requirement applies to physical iOS devices only. It does not apply to the iOS Simulator or to Android.
