...
This library was made to provide full Google authentication for Standalone Expo Apps. This module cannot be used in the Expo client as the native `GoogleSignIn` library expects your `REVERSE_CLIENT_ID` in the `info.plist` at build-time. To use Google authentication in the Expo Client, check out `Expo.Google`.
