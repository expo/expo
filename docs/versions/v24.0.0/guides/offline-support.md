...
When you [publish](../../workflow/publishing/) an update to your app, your users will receieve the new version of your JS over the air. The new version will download either next time the app starts, or next time you call [Util.reload()](../../sdk/util/). This behavior also applies the very first time the user opens your app.
...
- On Android, this option is always enabled. You can use [Util.addNewVersionListenerExperimental()](../../sdk/util/) to perform an action when the background download is complete.
...
To bundle images in your binary, use the [assetBundlePatterns](../../workflow/configuration/) key in `app.json` to provide a list of image paths in your project directory:
