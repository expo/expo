...
let from = args._[0] || "/Users/ccheever/projects/react-native-website";
let to = args._[1] || "..";
...
let mainAsync = async () => {
    ...
    // await fsExtra.ensureDir(guidesTo);
    // await fsExtra.ensureDir(componentsTo);
    // await fsExtra.ensureDir(apisTo);
    // await fsExtra.ensureDir(basicsTo);
    ...
    let transformFileAysnc = async (src, dest) => {
        ...
        for (let l of lines) {
            ...
            l = l.replace("(more-resources.md)", '(https://facebook.github.io/react-native/docs/more-resources.html)')

            // TODO: Make these global replaces instead of just first instance
            l = l.replace('<img src="/react-native/docs/assets/', '<img src="https://facebook.github.io/react-native/docs/assets/')
            l = l .replace('![](/react-native/docs/', '![](https://facebook.github.io/react-native/docs/');


            l = l.replace(/!\[([^]]*)\]\(\/react-native\/docs\//g, (match, $1) => '![' + $1 + '](https://facebook.github.io/react-native/docs/');

            let r = new RegExp('!\\[(.*)\\]\\(\\/react\\-native\\/docs\\/', 'g');
            l = l.replace(r, (match, $1) => ('![' + $1 + '](https://facebook.github.io/react-native/docs/'));
            ...
            switch (basename) {
                // case 'button.md':
                // case 'datepickerios.md':
                // case 'drawerlayoutandroid.md':
                // case 'image.md':
                // case 'keyboardavoidingview.md':
                // case 'maskedviewios.md':
                // case 'listview.md':
                // case 'navigatorios.md':
                // case 'progressbarandroid.md':
                // case 'segmentedcontrolios.md':
                // case 'statusbar.md':
                // case 'touchablenativefeedback.md':
                // case 'touchablehighlight.md':
                default:
                    if (/^```/.test(l)) {
                        if (!inCodeBlock) {
                            if (l === '```') {
...
// I
            ...
            if (basename === 'javascript-environment.md') {
                l = l.replace("`` var who = 'world'; var str = `Hello ${who}`; ``", "``` var who = 'world'; var str = `Hello ${who}`; ```");
            }
    ...
    for (let [x, dest] of [[guides, guidesTo], [components, componentsTo], [basics, basicsTo], [apis, apisTo]]) {
        // await copyFilesAsync(x, dest);
        await copyFilesAsync(x, reactNative);
    }

    console.log("#guides", guides.length, "#components", components.length, "#apis", apis.length, "#basics", basics.length);
