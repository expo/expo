...
async function action(options: ActionOptions) {
  ...
  for (const step of moduleConfig.steps) {
    ...
    if (executeIOS && step.sourceIosPath && step.targetIosPath) {
      ...
      for (const objcFile of objcFiles) {
        const objcFileRelativePath = path.relative(sourceDir, objcFile);
        const objcFileTargetPath = path.join(targetDir, objcFileRelativePath);
        
        await fs.mkdirs(path.dirname(objcFileTargetPath));
        await fs.copy(objcFile, objcFileTargetPath);
      }
    ...
    if (executeAndroid && step.sourceAndroidPath && step.targetAndroidPath && step.sourceAndroidPackage && step.targetAndroidPackage) {
      ...
      for (const javaFile of javaFiles) {
        const javaFileRelativePath = path.relative(sourceDir, javaFile);
        const javaFileTargetPath = path.join(targetDir, javaFileRelativePath);
        
        await fs.mkdirs(path.dirname(javaFileTargetPath));
        await fs.copy(javaFile, javaFileTargetPath);
      }
