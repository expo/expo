...
function checkInventory() {
  ...
  for (const relative of new Set([...baselinePaths, ...currentPaths])) {
    ...
    const packageRoot = path.join(repo, path.dirname(relative));
    if (fs.existsSync(path.join(packageRoot, 'Package.swift'))) {
      optedIn.add(JSON.parse(fs.readFileSync(path.join(packageRoot, 'package.json'), 'utf8')).name);
    }
  }
  console.log(
...
async function main() {
  if (!values.base) {
    const error = new Error(
      'Unable to check SwiftPM manifest collateral: --base is required.\n' +
        'Why: comparing manifests requires an explicit baseline commit.\n' +
        'How to fix: run node tools/scripts/check-spm-manifest-collateral.cjs --base "$(git merge-base origin/main HEAD)".'
    );
    error.stack = error.message;
    throw error;
  }
