    ...
    for (const pkg of packages) {
      ...
      if (options.generate || performAllSteps) {
        if (options.cleanGenerated) {
          ...
          await SPMGenerator.cleanGeneratedSourceCodeFolderAsync(pkg);
        }

        // Generate source code structure for the package
        const targets = await SPMGenerator.generateSourceCodeFolderAsync(pkg);

        // Generate Package.swift and vfs overlay
        await SPMGenerator.genereateSwiftPackageAsync(pkg, resolvedBuildFlavor, targets);
      }
    }
    ...
    for (const pkg of packages) {
      if (options.build || performAllSteps) {
        if (options.cleanBuild) {
          ...
          await SPMBuild.cleanBuildFolderAsync(pkg);
        }

        // Build the swift package
        await SPMBuild.buildSwiftPackageAsync(pkg, resolvedBuildFlavor, options);
      }

      // Create xcframeworks from built frameworks
      if (options.compose || performAllSteps) {
        // Let's compose those xcframeworks!
        await Frameworks.composeXCFrameworkFromBuiltFrameworksAsync(
          pkg,
          resolvedBuildFlavor,
          options
        );
      ...
      if (options.verify || performAllSteps) {
        ...
        await SPMVerify.verifyPackageXCFrameworksAsync(pkg, resolvedBuildFlavor);
      }
    }
  } catch (error) {
    logger.log(
