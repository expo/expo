...
export function getAndroidPlatformPackageId(compileSdk: string): string {
  return `platforms;android-${Number(compileSdk) >= 37 && !compileSdk.includes('.') ? `${compileSdk}.0` : compileSdk}`;
}
...
export async function prepareAndroidPrecompileToolchainAsync(): Promise<void> {
  ...
  const components = [
    getAndroidPlatformPackageId(versions.compileSdk),
