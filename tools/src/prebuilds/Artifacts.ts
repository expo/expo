...
export const Artifacts = {
    ...
    try {
      ...
      if (skipArtifacts) {
        ...
        if (!fs.existsSync(config.getValidationPath(outputPath))) {
          spinner.fail(`${config.name}: Artifacts not found, cannot skip download.`);
          throw new Error(
      ...
      if (!tarballPath) {
        ...
        if (version === 'nightly') {
          spinner.text = `${config.name}: Resolving latest nightly version...`;
          version = await getNightlyVersionFromNPMAsync(config.npmPackageName);
        }

        spinner.text = `${config.name}: Checking artifact availability...`;
        const sourceType = await determineSourceTypeAsync(config, options, version);

        spinner.text = `${config.name}: Downloading ${version} (${buildType})...`;
        ...
        spinner.text = `${config.name}: Using local tarball`;
      ...
      spinner.text = `${config.name}: Extracting...`;
  ...
  HermesArtifactConfig: {
    ...
    getTarballFilename: (version: string, buildType: BuildFlavor) => {
      return `hermes-ios-${version}-hermes-ios-${buildType.toLowerCase()}.tar.gz`;
    },

    getNightlyTarballUrl: async (version: string, buildType: BuildFlavor) => {
      const artifactCoordinate = 'hermes-ios';
      const artifactName = `hermes-ios-${buildType.toLowerCase()}.tar.gz`;
      return computeNightlyTarballURL(
        version,
        buildType,
        'hermes',
        artifactCoordinate,
  ...
  ReactNativeDependenciesArtifactConfig: {
    ...
    getTarballFilename: (version: string, buildType: BuildFlavor) => {
      return `react-native-artifacts-${version}-reactnative-dependencies-${buildType.toLowerCase()}.tar.gz`;
    },
    ...
    getValidationPath: (outputPath: string) => {
      return path.join(outputPath, 'ReactNativeDependencies.xcframework');
    },
  ...
  ReactNativeArtifactConfig: {
    ...
    getTarballFilename: (version: string, buildType: BuildFlavor) => {
      return `react-native-artifacts-${version}-reactnative-core-${buildType.toLowerCase()}.tar.gz`;
    },

    getNightlyTarballUrl: async (version: string, buildType: BuildFlavor) => {
      const artifactCoordinate = 'react-native-artifacts';
      const artifactName = `reactnative-core-${buildType.toLowerCase()}.tar.gz`;
      return computeNightlyTarballURL(
        version,
        buildType,
        'react',
    ...
    getValidationPath: (outputPath: string) => {
      ...
      return path.join(outputPath, 'React.xcframework');
    },
  } as ArtifactConfig,
};
        ...
        const readableStream = new ReadableStream({
          async start(controller) {
            while (true) {
              ...
              spinner.text = `${config.name}: Downloading... ${progress}% (${downloadedMB}/${totalMB} MB)`;
