...
function generateExportsFile(
  ...
  const internalDependencyImports = (target.dependencies ?? [])
    .filter((dependency) => internalTargetNames.has(dependency))
    .map((dependency) => `@_exported import ${dependency}`);
  const allImports = [...frameworkImports, ...internalDependencyImports];
  if (allImports.length === 0) return false;
  return writeFileIfChanged(
    path.join(targetDestination, `${product.name}+Exports.swift`),
...
export const SPMGenerator = {
  ...
  generateIsolatedSourcesForTargetsAsync: async (pkg: SPMPackageSource, product: SPMProduct) => {
    ...
    if (checkedInRoot) {
      ...
      for (const target of targets) {
        ...
        const targetDestination = path.join(
          SPMGenerator.getGeneratedProductFilesPath(pkg, product),
          target.name
        );
        await fs.ensureDir(targetDestination);
        const retained = new Set([
          'src',
          ...target.sources.filter((source) => !source.startsWith('src/')),
        ]);
        for (const entry of await fs.readdir(targetDestination)) {
          if (!retained.has(entry)) await fs.remove(path.join(targetDestination, entry));
        }
        const sourceLink = path.join(targetDestination, 'src');
        await fs.remove(sourceLink).catch(() => {});
        await fs.symlink(target.sourceRoot, sourceLink);
    ...
    for (const target of product.targets) {
      ...
      if (!target.path) {
        throw new Error(
          `Cannot generate sources for product "${product.name}", target "${target.name}": the ` +
            `target declares no "path", and ${pkg.packageName} is not building from a checked-in ` +
            `Package.swift, so nothing says where its sources are. A target may omit "path" only ` +
            `when a checked-in manifest declares the package layout. Give the target a "path" in ` +
            `its spm.config.json, or add Package.swift to ${pkg.path}.`
        );
      }
      ...
      if (target.type === 'swift') {
        if (generateExportsFile(targetDestination, product, target)) {
          spinner.info(
