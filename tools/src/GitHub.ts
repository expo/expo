...
export async function uploadReleaseAssetAsync(
  repoOwner: string,
  repoName: string,
  releaseId: number,
  assetName: string,
...
export async function updateReleaseTitle(
  repoOwner: string,
  repoName: string,
  releaseId: number,
  appVersion: string,
...
/**
 * Lists open pull requests for the repo.
 */
export async function listOpenPullRequestsAsync(options?: {
  per_page?: number;
  page?: number;
  sort?: string;
  direction?: 'asc' | 'desc';
...
/**
 * Lists recent issues with date filtering.
 */
export async function listRecentIssuesAsync(options: {
  since?: string;
