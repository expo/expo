...
export function addDevMenu(packageName: string) {
  const projectRoot = getProjectRoot(packageName);
  const targetName = getTargetName(packageName);
  const pathToAppDelegate = path.resolve(projectRoot, 'ios', targetName, 'AppDelegate.m');

  let contents = fs.readFileSync(pathToAppDelegate, 'utf-8');
...
#import <React/RCTUtils.h>`
  ...
  // Add DevMenu back
  if (
    !contents.includes('[extraModules addObject:(id<RCTBridgeModule>)[[RCTDevMenu alloc] init]];')
  ) {
    contents = contents.replace(
...
self.moduleRegistryAdapter = [[UMModuleRegistryAdapter alloc]`
  ...
  fs.writeFileSync(pathToAppDelegate, contents);
}
