...
import { Alert, NativeModules, Platform, StyleSheet, Text, TouchableOpacity, View, findNodeHandle } from 'react-native';
...
class OptionsButtonAndroid extends React.Component {
  ...
  _handlePress = () => {
    let handle = findNodeHandle(this._anchor);
    NativeModules.UIManager.showPopupMenu(handle, ['Report this user'], err => {}, (action, selectedIndex) => {
      if (selectedIndex === 0) {
        Alert.alert('Thank you for your report', 'We will investigate the case as soon as we can.');
      }
    });
  };
}
