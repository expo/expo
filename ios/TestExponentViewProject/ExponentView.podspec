# require 'json'
# package = JSON.parse(File.read(File.join('../../react-native-lab/react-native', 'package.json')))
...
Pod::Spec.new do |s|
  ...
  s.requires_arc = true
  s.platform = :ios, "8.0"
  s.source = { :git => "https://github.com/exponentjs/exponent.git" }
  s.source_files = "ios/Exponent/**/*.{h,m}"
  s.preserve_paths = "ios/Exponent/**/*.{h,m}"
  s.exclude_files = "ios/Exponent/EXAppDelegate.*", "ios/Exponent/EXRootViewController.h"

  s.dependency 'CocoaLumberjack', '~> 2.3'
  s.dependency 'Crashlytics', '~> 3.8'
  s.dependency 'Fabric', '~> 1.6'
  # s.dependency 'SimulatorStatusMagic', '~> 1.9', :configurations => ['Debug']
