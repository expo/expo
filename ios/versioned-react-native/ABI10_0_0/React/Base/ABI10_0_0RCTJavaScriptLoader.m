...
uint32_t const ABI10_0_0RCTRAMBundleMagicNumber = 0xFB0BD1E5;

NSString *const ABI10_0_0RCTJavaScriptLoaderErrorDomain = @"ABI10_0_0RCTJavaScriptLoaderErrorDomain";

@implementation ABI10_0_0RCTJavaScriptLoader

ABI10_0_0RCT_NOT_IMPLEMENTED(- (instancetype)init)

+ (void)loadBundleAtURL:(NSURL *)scriptURL onComplete:(ABI10_0_0RCTSourceLoadBlock)onComplete
{
  ...
  [error.domain isEqualToString:ABI10_0_0RCTJavaScriptLoaderErrorDomain]
  && error.code == ABI10_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI10_0_0RCTJavaScriptLoaderErrorNoScriptURL
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI10_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI10_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI10_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (magicNumber != ABI10_0_0RCTRAMBundleMagicNumber) {
    if (error) {
      *error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI10_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI10_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                               userInfo:@{NSLocalizedDescriptionKey:
...
static void attemptAsynchronousLoadOfBundleAtURL(NSURL *scriptURL, ABI10_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
   ...
   ^(NSData *data, NSURLResponse *response, NSError *error) {
     ...
     if (error) {
       if ([error.domain isEqualToString:NSURLErrorDomain]) {
         error = [NSError errorWithDomain:ABI10_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI10_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                                 userInfo:
                  @{
                    NSLocalizedDescriptionKey:
                      [@"Could not connect to development server.\n\n"
                       "Ensure the following:\n"
                       "- Node server is running and available on the same network - run 'npm start' from ReactABI10_0_0-native root\n"
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI10_0_0RCTConvert NSURL:url.absoluteString];
}

static NSDictionary *userInfoForRawResponse(NSString *rawText)
{
  NSDictionary *parsedResponse = ABI10_0_0RCTJSONParse(rawText, nil);
