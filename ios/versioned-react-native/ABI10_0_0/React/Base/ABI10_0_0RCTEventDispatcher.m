...
const NSInteger ABI10_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI10_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI10_0_0RCTGetEventID(id<ABI10_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI10_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI10_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI10_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI10_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
{
  [_bridge enqueueJSCall:@"ABI10_0_0RCTDeviceEventEmitter"
                  method:@"emit"
                    args:body ? @[name, body] : @[name]
              completion:NULL];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI10_0_0RCT_DEBUG) {
    ABI10_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI10_0_0 tag");
  }

  name = ABI10_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI10_0_0RCTEventEmitter"
                  method:@"receiveEvent"
                    args:body ? @[body[@"target"], name, body] : @[body[@"target"], name]
              completion:NULL];
}

- (void)sendTextEventWithType:(ABI10_0_0RCTTextEventType)type
...
- (void)sendEvent:(id<ABI10_0_0RCTEvent>)event
{
  [_observersLock lock];

  BOOL eventHandled = NO;
  for (id<ABI10_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI10_0_0RCTGetEventID(event);

  id<ABI10_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI10_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
  ...
  if (scheduleEventsDispatch) {
    ...
    } queue:ABI10_0_0RCTJSThread];
  }
}

- (void)addDispatchObserver:(id<ABI10_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI10_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)dispatchEvent:(id<ABI10_0_0RCTEvent>)event
{
...
@implementation ABI10_0_0RCTBridge (ABI10_0_0RCTEventDispatcher)

- (ABI10_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI10_0_0RCTEventDispatcher class]];
}
