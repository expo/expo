...
static NSRegularExpression *ABI10_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI10_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI10_0_0RCTNullIfNil(self.methodName),
     @"file": ABI10_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI10_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
+ (NSArray<ABI10_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI10_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI10_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI10_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
