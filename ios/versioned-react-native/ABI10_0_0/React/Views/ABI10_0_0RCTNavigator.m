...
typedef NS_ENUM(NSUInteger, ABI10_0_0RCTNavigationLock) {
  ABI10_0_0RCTNavigationLockNone,
  ABI10_0_0RCTNavigationLockNative,
  ABI10_0_0RCTNavigationLockJavaScript
};

// By default the interactive pop gesture will be enabled when the navigation bar is displayed
// and disabled when hidden
// ABI10_0_0RCTPopGestureStateDefault maps to the default behavior (mentioned above). Once popGestureState
...
// back to it.
// ABI10_0_0RCTPopGestureStateEnabled will enable the gesture independent of nav bar visibility
// ABI10_0_0RCTPopGestureStateDisabled will disable the gesture independent of nav bar visibility
typedef NS_ENUM(NSUInteger, ABI10_0_0RCTPopGestureState) {
  ABI10_0_0RCTPopGestureStateDefault = 0,
  ABI10_0_0RCTPopGestureStateEnabled,
  ABI10_0_0RCTPopGestureStateDisabled
};
...
@interface ABI10_0_0RCTNavigationController : UINavigationController <UINavigationBarDelegate>
{
  dispatch_block_t _scrollCallback;
}

@property (nonatomic, assign) ABI10_0_0RCTNavigationLock navigationLock;

@end

/**
 * In general, `ABI10_0_0RCTNavigator` examines `_currentViews` (which are ReactABI10_0_0 child
 ...
 * ahead by adding/removing ReactABI10_0_0 subviews. Native gets ahead by swiping back,
 * or tapping the back button. In both cases, the other system is initially
 * unaware. And in both cases, `ABI10_0_0RCTNavigator` helps the other side "catch up".
 *
 * If `ABI10_0_0RCTNavigator` sees the number of ReactABI10_0_0 children have changed, it
 * pushes/pops accordingly. If `ABI10_0_0RCTNavigator` sees a `UIKit` driven push/pop, it
 ...
 *
 */
@implementation ABI10_0_0RCTNavigationController

/**
 * @param callback Callback that is invoked when a "scroll" interaction begins
 * so that `ABI10_0_0RCTNavigator` can notify `JavaScript`.
 */
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
  if (self.interactivePopGestureRecognizer.state == UIGestureRecognizerStateBegan) {
    if (self.navigationLock == ABI10_0_0RCTNavigationLockNone) {
      self.navigationLock = ABI10_0_0RCTNavigationLockNative;
      if (_scrollCallback) {
        _scrollCallback();
      }
    } else if (self.navigationLock == ABI10_0_0RCTNavigationLockJavaScript) {
      // This should never happen because we disable/enable the gesture
      // recognizer when we lock the navigation.
      ABI10_0_0RCTAssert(NO, @"Should never receive gesture start while JS locks navigator");
    }
  } else {
    if (self.navigationLock == ABI10_0_0RCTNavigationLockNone) {
      ...
      // length (`currentReactABI10_0_0Count` - 1).
...
@interface ABI10_0_0RCTNavigator() <ABI10_0_0RCTWrapperViewControllerNavigationListener, UINavigationControllerDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, copy) ABI10_0_0RCTDirectEventBlock onNavigationProgress;
@property (nonatomic, copy) ABI10_0_0RCTBubblingEventBlock onNavigationComplete;

@property (nonatomic, assign) NSInteger previousRequestedTopOfStack;

@property (nonatomic, assign) ABI10_0_0RCTPopGestureState popGestureState;

// Previous views are only mainted in order to detect incorrect
// addition/removal of views below the `requestedTopOfStack`
@property (nonatomic, copy, readwrite) NSArray<ABI10_0_0RCTNavItem *> *previousViews;
@property (nonatomic, readwrite, strong) ABI10_0_0RCTNavigationController *navigationController;
/**
 ...
 * - We use `invalidate` to remove the `ABI10_0_0RCTNavigator`'s reference to the
 ...
 * remove it from the run loop when the view will be destroyed by ReactABI10_0_0.
 *
 * +----------+              +--------------+
...
@implementation ABI10_0_0RCTNavigator
{
...
- (instancetype)initWithBridge:(ABI10_0_0RCTBridge *)bridge
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _previousViews = @[];
    __weak ABI10_0_0RCTNavigator *weakSelf = self;
    _navigationController = [[ABI10_0_0RCTNavigationController alloc] initWithScrollCallback:^{
      [weakSelf dispatchFakeScrollEvent];
    }];
    _navigationController.delegate = self;
    ABI10_0_0RCTAssert([self requestSchedulingJavaScriptNavigation], @"Could not acquire JS navigation lock on init");
...
- (void)didUpdateFrame:(__unused ABI10_0_0RCTFrameUpdate *)update
{
...
- (void)setInteractivePopGestureEnabled:(BOOL)interactivePopGestureEnabled
{
  ...
  _popGestureState = interactivePopGestureEnabled ? ABI10_0_0RCTPopGestureStateEnabled : ABI10_0_0RCTPopGestureStateDisabled;
}
                    ...
                    animated:(__unused BOOL)animated
{
  ...
  [tc animateAlongsideTransition: ^(id<UIViewControllerTransitionCoordinatorContext> context) {
    ABI10_0_0RCTWrapperViewController *fromController =
      (ABI10_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey];
    ABI10_0_0RCTWrapperViewController *toController =
      (ABI10_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    NSUInteger indexOfFrom = [self.ReactABI10_0_0Subviews indexOfObject:fromController.navItem];
    NSUInteger indexOfTo = [self.ReactABI10_0_0Subviews indexOfObject:toController.navItem];
...
{
  if (_navigationController.navigationLock == ABI10_0_0RCTNavigationLockNone) {
    _navigationController.navigationLock = ABI10_0_0RCTNavigationLockJavaScript;
...
- (void)freeLock
{
  _navigationController.navigationLock = ABI10_0_0RCTNavigationLockNone;

  // Unless the pop gesture has been explicitly disabled (ABI10_0_0RCTPopGestureStateDisabled),
  // Set interactivePopGestureRecognizer.enabled to YES
  // If the popGestureState is ABI10_0_0RCTPopGestureStateDefault the default behavior will be maintained
  _navigationController.interactivePopGestureRecognizer.enabled = self.popGestureState != ABI10_0_0RCTPopGestureStateDisabled;
}

/**
 * A ReactABI10_0_0 subview can be inserted/removed at any time, however if the
 * `requestedTopOfStack` changes, there had better be enough subviews present
 * to satisfy the push/pop.
 */
- (void)insertReactABI10_0_0Subview:(ABI10_0_0RCTNavItem *)view atIndex:(NSInteger)atIndex
{
  ABI10_0_0RCTAssert([view isKindOfClass:[ABI10_0_0RCTNavItem class]], @"ABI10_0_0RCTNavigator only accepts ABI10_0_0RCTNavItem subviews");
  ABI10_0_0RCTAssert(
    _navigationController.navigationLock == ABI10_0_0RCTNavigationLockJavaScript,
...
- (void)layoutSubviews
{
  ...
  [self ReactABI10_0_0AddControllerToClosestParent:_navigationController];
  _navigationController.view.frame = self.bounds;
}

- (void)removeReactABI10_0_0Subview:(ABI10_0_0RCTNavItem *)subview
{
  if (self.ReactABI10_0_0Subviews.count <= 0 || subview == self.ReactABI10_0_0Subviews[0]) {
    ABI10_0_0RCTLogError(@"Attempting to remove invalid ABI10_0_0RCT subview of ABI10_0_0RCTNavigator");
    return;
  }
...
{
  [_bridge.eventDispatcher sendFakeScrollEvent:self.ReactABI10_0_0Tag];
}
...
{
  ABI10_0_0RCTAssert(!_bridge.isValid || self.superview != nil, @"put ReactABI10_0_0NavSuperviewLink back");
  UIView *superview = [super ReactABI10_0_0Superview];
  return superview ?: self.ReactABI10_0_0NavSuperviewLink;
}
...
- (void)ReactABI10_0_0BridgeDidFinishTransaction
{
  ...
  // The "ReactABI10_0_0 count" is the count of views that are visible on the navigation
  // stack.  There may be more beyond this - that aren't visible, and may be
  // deleted/purged soon.
  NSUInteger previousReactABI10_0_0Count =
    _previousRequestedTopOfStack == kNeverRequested ? 0 : _previousRequestedTopOfStack + 1;
  NSUInteger currentReactABI10_0_0Count = _requestedTopOfStack + 1;

  BOOL jsGettingAhead =
    //    ----- previously caught up ------          ------ no longer caught up -------
    viewControllerCount == previousReactABI10_0_0Count && currentReactABI10_0_0Count != viewControllerCount;
  BOOL jsCatchingUp =
    //    --- previously not caught up ----          --------- now caught up ----------
    viewControllerCount != previousReactABI10_0_0Count && currentReactABI10_0_0Count == viewControllerCount;
  BOOL jsMakingNoProgressButNeedsToCatchUp =
    //    --- previously not caught up ----          ------- still the same -----------
    viewControllerCount != previousReactABI10_0_0Count && currentReactABI10_0_0Count == previousReactABI10_0_0Count;
...
BOOL jsGettingtooSlow =
  ...
  BOOL ReactABI10_0_0PushOne = jsGettingAhead && currentReactABI10_0_0Count == previousReactABI10_0_0Count + 1;
  BOOL ReactABI10_0_0PopN = jsGettingAhead && currentReactABI10_0_0Count < previousReactABI10_0_0Count;
  ...
  if (!(jsGettingAhead ||
        ...
        jsGettingtooSlow)) {
    ABI10_0_0RCTLogError(@"JS has only made partial progress to catch up to UIKit");
  }
  if (currentReactABI10_0_0Count > self.ReactABI10_0_0Subviews.count) {
    ABI10_0_0RCTLogError(@"Cannot adjust current top of stack beyond available views");
  }

  // Views before the previous ReactABI10_0_0 count must not have changed. Views greater than previousReactABI10_0_0Count
  // up to currentReactABI10_0_0Count may have changed.
  for (NSUInteger i = 0; i < MIN(self.ReactABI10_0_0Subviews.count, MIN(_previousViews.count, previousReactABI10_0_0Count)); i++) {
    if (self.ReactABI10_0_0Subviews[i] != _previousViews[i]) {
      ABI10_0_0RCTLogError(@"current view should equal previous view");
    }
  }
  if (currentReactABI10_0_0Count < 1) {
    ABI10_0_0RCTLogError(@"should be at least one current view");
  }
  if (jsGettingAhead) {
    if (ReactABI10_0_0PushOne) {
      UIView *lastView = self.ReactABI10_0_0Subviews.lastObject;
      ABI10_0_0RCTWrapperViewController *vc = [[ABI10_0_0RCTWrapperViewController alloc] initWithNavItem:(ABI10_0_0RCTNavItem *)lastView];
      vc.navigationListener = self;
      _numberOfViewControllerMovesToIgnore = 1;
      [_navigationController pushViewController:vc animated:(currentReactABI10_0_0Count > 1)];
    } else if (ReactABI10_0_0PopN) {
      UIViewController *viewControllerToPopTo = _navigationController.viewControllers[(currentReactABI10_0_0Count - 1)];
      _numberOfViewControllerMovesToIgnore = viewControllerCount - currentReactABI10_0_0Count;
      [_navigationController popToViewController:viewControllerToPopTo animated:YES];
    } else {
      ABI10_0_0RCTLogError(@"Pushing or popping more than one view at a time from JS");
    }
  ...
  _previousViews = [self.ReactABI10_0_0Subviews copy];
...
didMoveToNavigationController:(UINavigationController *)navigationController
{
  ...
  ABI10_0_0RCTAssert(_navigationController.navigationLock == ABI10_0_0RCTNavigationLockJavaScript ||
