...
typedef void (^ABI10_0_0RCTPropBlock)(id<ABI10_0_0RCTComponent> view, id json);

@interface ABI10_0_0RCTComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) ABI10_0_0RCTPropBlock propBlock;

@end

@implementation ABI10_0_0RCTComponentProp
...
@implementation ABI10_0_0RCTComponentData
{
  id<ABI10_0_0RCTComponent> _defaultView; // Only needed for ABI10_0_0RCT_CUSTOM_VIEW_PROPERTY
  NSMutableDictionary<NSString *, ABI10_0_0RCTPropBlock> *_viewPropBlocks;
  NSMutableDictionary<NSString *, ABI10_0_0RCTPropBlock> *_shadowPropBlocks;
                              ...
                              bridge:(ABI10_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    _name = ABI10_0_0RCTBridgeModuleNameForClass(_managerClass);
    ABI10_0_0RCTAssert(_name.length, @"Invalid moduleName '%@'", _name);
    ...
    while (cls != [ABI10_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI10_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI10_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI10_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI10_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ABI10_0_0RCTPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, ABI10_0_0RCTPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  ABI10_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {

    __weak ABI10_0_0RCTComponentData *weakSelf = self;
    ...
    if ([_managerClass respondsToSelector:selector]) {
      ...
      type = ABI10_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<ABI10_0_0RCTComponent> view, id json) {
        ABI10_0_0RCTComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        json = ABI10_0_0RCTNilIfNull(json);
      ...
      if (type == NSSelectorFromString(@"ABI10_0_0RCTBubblingEventBlock:") ||
          type == NSSelectorFromString(@"ABI10_0_0RCTDirectEventBlock:")) {

        // Special case for event handlers
        __weak ABI10_0_0RCTViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<ABI10_0_0RCTComponent> weakTarget = target;
          ((void (*)(id, SEL, id))objc_msgSend)(target, setter, [ABI10_0_0RCTConvert BOOL:json] ? ^(NSDictionary *body) {
            body = [NSMutableDictionary dictionaryWithDictionary:body];
            ((NSMutableDictionary *)body)[@"target"] = weakTarget.ReactABI10_0_0Tag;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            [weakManager.bridge.eventDispatcher sendInputEventWithName:ABI10_0_0RCTNormalizeInputEventName(name) body:body];
#pragma clang diagnostic pop
        ...
        NSMethodSignature *typeSignature = [[ABI10_0_0RCTConvert class] methodSignatureForSelector:type];
        if (!typeSignature) {
          ABI10_0_0RCTLogError(@"No +[ABI10_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
          return ^(__unused id<ABI10_0_0RCTComponent> view, __unused id json){};
        }
  ...
  #define ABI10_0_0RCT_CASE(_value, _type) \
          case _value: { \
            ...
            setterBlock = ^(id target, id json) { \
              if (json) { \
                ...
                set(target, setter, convert([ABI10_0_0RCTConvert class], type, json)); \
            ...
            ABI10_0_0RCT_CASE(_C_SEL, SEL)
            ABI10_0_0RCT_CASE(_C_CHARPTR, const char *)
            ...
            ABI10_0_0RCT_CASE(_C_PTR, void *)
            ABI10_0_0RCT_CASE(_C_ID, id)
      ...
      propBlock = ^(__unused id view, __unused id json) {
        ...
        setterBlock(target, ABI10_0_0RCTNilIfNull(json));
      };
    }
    ...
    if (ABI10_0_0RCT_DEBUG) {
      ...
      ABI10_0_0RCTPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<ABI10_0_0RCTComponent> view, id json) {
        NSString *logPrefix = [NSString stringWithFormat:
                               @"Error setting property '%@' of %@ with tag #%@: ",
                               name, weakSelf.name, view.ReactABI10_0_0Tag];

        ABI10_0_0RCTPerformBlockWithLogPrefix(^{ unwrappedBlock(view, json); }, logPrefix);
      };
    }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  if (ABI10_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customDirectEventTypes))) {
    NSArray<NSString *> *events = [self.manager customDirectEventTypes];
#pragma clang diagnostic pop
    if (ABI10_0_0RCT_DEBUG) {
      ABI10_0_0RCTAssert(!events || [events isKindOfClass:[NSArray class]],
        @"customDirectEventTypes must return an array, but %@ returned %@",
        _managerClass, [events class]);
    }
    for (NSString *event in events) {
      [directEvents addObject:ABI10_0_0RCTNormalizeInputEventName(event)];
    }
  }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  if (ABI10_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
#pragma clang diagnostic pop
    ...
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI10_0_0RCTNormalizeInputEventName(event)];
    }
  }
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if ([methodName hasPrefix:@"propConfig"]) {
      ...
      if (nameRange.length) {
        ...
        if (ABI10_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
          ABI10_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                      "to '%@'", name, _name, propTypes[name], type);
        }

        if ([type isEqualToString:@"ABI10_0_0RCTBubblingEventBlock"]) {
          [bubblingEvents addObject:ABI10_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else if ([type isEqualToString:@"ABI10_0_0RCTDirectEventBlock"]) {
          [directEvents addObject:ABI10_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else {
          propTypes[name] = type;
        }
  ...
  if (ABI10_0_0RCT_DEBUG) {
    for (NSString *event in directEvents) {
      if ([bubblingEvents containsObject:event]) {
        ABI10_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
