...
#import "ABI10_0_0RCTAutoInsetsProtocol.h"
#import "ABI10_0_0RCTBorderDrawing.h"
#import "ABI10_0_0RCTConvert.h"
#import "ABI10_0_0RCTLog.h"
#import "ABI10_0_0RCTUtils.h"
#import "UIView+ReactABI10_0_0.h"

@implementation UIView (ABI10_0_0RCTViewUnmounting)

- (void)ReactABI10_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI10_0_0_remountAllSubviews];
  }
}

- (void)ReactABI10_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI10_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI10_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI10_0_0RootView]) {
      break;
    }
...
static NSString *ABI10_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI10_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI10_0_0RCTView
{
  UIColor *_backgroundColor;
}

@synthesize ReactABI10_0_0ZIndex = _ReactABI10_0_0ZIndex;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI10_0_0RCTBorderStyleSolid;
...
ABI10_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI10_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI10_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI10_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI10_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI10_0_0RCTPointerEventsNone || _pointerEvents == ABI10_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI10_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI10_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI10_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI10_0_0Tag: %@;", self.ReactABI10_0_0Tag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI10_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ReactABI10_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI10_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.sortedReactABI10_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI10_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI10_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI10_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI10_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.sortedReactABI10_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI10_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI10_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI10_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI10_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI10_0_0_findClipView];
  if (clipView) {
    [self ReactABI10_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI10_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI10_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI10_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI10_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI10_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI10_0_0RCTCornerRadii){
...
{
  return (ABI10_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI10_0_0RCTUpdateShadowPathForView(self);

  const ABI10_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI10_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI10_0_0RCTRunningInTestEnvironment() &&
  ABI10_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI10_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI10_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI10_0_0RCTBorderStyleSolid &&

  // iOS draws borders in front of the content whereas ABI10_0_0CSS draws them behind
  // the content. For this reason, only use iOS border drawing when clipping
  // or when the border is hidden.

  (borderInsets.top == 0 || CGColorGetAlpha(borderColors.top) == 0 || self.clipsToBounds);

  // iOS clips to the outside of the border, but ABI10_0_0CSS clips to the inside. To
  ...
  UIImage *image = ABI10_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
  ...
  if (ABI10_0_0RCTRunningInTestEnvironment()) {
    const CGSize size = self.bounds.size;
...
static BOOL ABI10_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI10_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI10_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI10_0_0RCTUpdateShadowPathForView(ABI10_0_0RCTView *view)
{
  if (ABI10_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI10_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI10_0_0RCTCornerInsets cornerInsets = ABI10_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI10_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI10_0_0RCTLogWarn(@"View #%@ of type %@ has a shadow set but cannot calculate "
                 "shadow efficiently. Consider setting a background color to "
                 "fix this, or apply the shadow to a more specific component.",
                 view.ReactABI10_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI10_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI10_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI10_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI10_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI10_0_0RCTBorderStyle)style \
  {                                                    \
