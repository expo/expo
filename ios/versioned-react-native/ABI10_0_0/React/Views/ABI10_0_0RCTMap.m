...
const CLLocationDegrees ABI10_0_0RCTMapDefaultSpan = 0.005;
const NSTimeInterval ABI10_0_0RCTMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI10_0_0RCTMapZoomBoundBuffer = 0.01;

@implementation ABI10_0_0RCTMap
{
  UIView *_legalLabel;
...
- (void)setAnnotations:(NSArray<ABI10_0_0RCTMapAnnotation *> *)annotations
{
  ...
  NSMutableArray<ABI10_0_0RCTMapAnnotation *> *annotationsToDelete = [NSMutableArray new];
  NSMutableArray<ABI10_0_0RCTMapAnnotation *> *annotationsToAdd = [NSMutableArray new];

  for (ABI10_0_0RCTMapAnnotation *annotation in annotations) {
  ...
  for (ABI10_0_0RCTMapAnnotation *annotation in self.annotations) {
    if (![annotation isKindOfClass:[ABI10_0_0RCTMapAnnotation class]]) {
      continue;
    }
...
- (void)setOverlays:(NSArray<ABI10_0_0RCTMapOverlay *> *)overlays
{
  ...
  for (ABI10_0_0RCTMapOverlay *overlay in overlays) {
  ...
  for (ABI10_0_0RCTMapOverlay *overlay in self.overlays) {
    if (![overlay isKindOfClass:[ABI10_0_0RCTMapOverlay class]]) {
      continue;
    }
