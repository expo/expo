...
#import "ABI10_0_0RCTErrorInfo.h"
#import "ABI10_0_0RCTUtils.h"
#import "ABI10_0_0RCTJSStackFrame.h"

#if ABI10_0_0RCT_DEV

@class ABI10_0_0RCTRedBoxWindow;

@protocol ABI10_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI10_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI10_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI10_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI10_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI10_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI10_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI10_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI10_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI10_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI10_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI10_0_0RCTJSStackFrame *)stackFrame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI10_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI10_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI10_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI10_0_0RCTKeyCommands for this, but since
...
@interface ABI10_0_0RCTRedBox () <ABI10_0_0RCTInvalidating, ABI10_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI10_0_0RCTRedBox
{
  ABI10_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI10_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (void)registerErrorCustomizer:(id<ABI10_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI10_0_0RCTErrorInfo *)_customizeError:(ABI10_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI10_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI10_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI10_0_0RCTJSStackFrame *> *stack = [ABI10_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  [self _showErrorMessage:message withStack:[ABI10_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:isUpdate];
}

- (void)_showErrorMessage:(NSString *)message withStack:(NSArray<ABI10_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI10_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI10_0_0RCTErrorInfo *errorInfo = [[ABI10_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI10_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI10_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI10_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI10_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI10_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI10_0_0RCTRedBoxWindow *)redBoxWindow {
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI10_0_0RCTReloadNotification object:_bridge.baseBridge userInfo:nil];
}

@end

@implementation ABI10_0_0RCTBridge (ABI10_0_0RCTRedBox)

- (ABI10_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI10_0_0RCTRedBox class]];
}
...
- (ABI10_0_0RCTRedBox *)redBox { return nil; }
