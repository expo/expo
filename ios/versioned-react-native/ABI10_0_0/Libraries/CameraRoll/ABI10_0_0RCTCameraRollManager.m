...
#import "ABI10_0_0RCTCameraRollManager.h"

#import <CoreLocation/CoreLocation.h>
#import <Foundation/Foundation.h>
#import <UIKit/UIKit.h>

#import "ABI10_0_0RCTAssetsLibraryRequestHandler.h"
...
@implementation ABI10_0_0RCTConvert (ALAssetGroup)

ABI10_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI10_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI10_0_0RCTCameraRollManager

ABI10_0_0RCT_EXPORT_MODULE()

@synthesize bridge = _bridge;

NSString *const ABI10_0_0RCTErrorUnableToLoad = @"E_UNABLE_TO_LOAD";
NSString *const ABI10_0_0RCTErrorUnableToSave = @"E_UNABLE_TO_SAVE";

ABI10_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
  ...
  if ([type isEqualToString:@"video"]) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [self->_bridge.assetsLibrary writeVideoAtPathToSavedPhotosAlbum:request.URL completionBlock:^(NSURL *assetURL, NSError *saveError) {
        if (saveError) {
          reject(ABI10_0_0RCTErrorUnableToSave, nil, saveError);
        } else {
    ...
    [_bridge.imageLoader loadImageWithURLRequest:request
      ...
      if (loadError) {
        reject(ABI10_0_0RCTErrorUnableToLoad, nil, loadError);
        return;
      }
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        [self->_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
          if (saveError) {
            ABI10_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
            reject(ABI10_0_0RCTErrorUnableToSave, nil, saveError);
          } else {
            resolve(assetURL.absoluteString);
          }
...
static void ABI10_0_0RCTResolvePromise(ABI10_0_0RCTPromiseResolveBlock resolve,
...
ABI10_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI10_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI10_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger first = [ABI10_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI10_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI10_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI10_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI10_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI10_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI10_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI10_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
    reject(ABI10_0_0RCTErrorUnableToLoad, nil, error);
  }];
}
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI10_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
