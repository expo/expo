...
#import "ABI44_0_0EXLinkingManager.h"
#import "ABI44_0_0EXScopedModuleRegistry.h"
#import "ABI44_0_0EXUtil.h"
...
NSString * const ABI44_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI44_0_0EXLinkingManager ()

@property (nonatomic, weak) id <ABI44_0_0EXLinkingManagerScopedModuleDelegate> kernelLinkingDelegate;
@property (nonatomic, strong) NSURL *initialUrl;
@property (nonatomic) BOOL hasListeners;

@end

@implementation ABI44_0_0EXLinkingManager

ABI44_0_0EX_EXPORT_SCOPED_MODULE(ABI44_0_0RCTLinkingManager, KernelLinkingManager);
...
#pragma mark - ABI44_0_0RCTEventEmitter methods

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI44_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI44_0_0RCTFatal(ABI44_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI44_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}
                  ...
                  reject:(ABI44_0_0RCTPromiseRejectBlock)reject)
{
  if ([_kernelLinkingDelegate linkingModule:self shouldOpenExpoUrl:URL]) {
    ...
    [ABI44_0_0EXUtil performSynchronouslyOnMainThread:^{
                  ...
                  reject:(__unused ABI44_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!canOpen) {
    [ABI44_0_0EXUtil performSynchronouslyOnMainThread:^{
      canOpen = [ABI44_0_0RCTSharedApplication() canOpenURL:URL];
    }];
  }
  resolve(@(canOpen));
}
...
ABI44_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI44_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI44_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI44_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
