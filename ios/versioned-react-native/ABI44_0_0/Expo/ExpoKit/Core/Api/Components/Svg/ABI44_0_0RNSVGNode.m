...
@interface ABI44_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI44_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI44_0_0RNSVGGroup *textRoot;
@end

@implementation ABI44_0_0RNSVGNode
{
    ABI44_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI44_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI44_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI44_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)insertABI44_0_0ReactSubview:(ABI44_0_0RNSVGView *)subview atIndex:(NSInteger)atIndex
{
...
- (void)removeABI44_0_0ReactSubview:(ABI44_0_0RNSVGView *)subview
{
...
{
    // Do nothing, as subviews are inserted by insertABI44_0_0ReactSubview:
}
...
- (void)invalidate
{
    ...
    _dirty = true;
    id<ABI44_0_0RNSVGContainer> container = (id<ABI44_0_0RNSVGContainer>)self.superview;
...
{
    [self clearPath];
    for (__kindof ABI44_0_0RNSVGNode *node in self.subviews) {
...
{
    ABI44_0_0RNSVGNode* node = self;
    while (node != nil) {
        ABI44_0_0RNSVGPlatformView* parent = [node superview];

        if (![parent isKindOfClass:[ABI44_0_0RNSVGNode class]]) {
            return;
        }
        node = (ABI44_0_0RNSVGNode*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (ABI44_0_0RNSVGGroup *)textRoot
{
    ...
    ABI44_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI44_0_0RNSVGGroup class]] && [((ABI44_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI44_0_0RNSVGGroup*)node;
            break;
        }
...
- (ABI44_0_0RNSVGGroup *)getParentTextRoot
{
    ABI44_0_0RNSVGNode* parent = (ABI44_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI44_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI44_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI44_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)ABI44_0_0ReactSetInheritedBackgroundColor:(ABI44_0_0RNSVGColor *)inheritedBackgroundColor
{
...
- (void)setPointerEvents:(ABI44_0_0RCTPointerEvents)pointerEvents
{
  ...
  self.userInteractionEnabled = (pointerEvents != ABI44_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI44_0_0RCTPointerEventsBoxNone) {
#if TARGET_OS_OSX
...
- (void)setClipRule:(ABI44_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI44_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if (_cachedClipPath) {
            CGPathRelease(_cachedClipPath);
        }
...
- (ABI44_0_0RNSVGSvgView *)svgView
{
    if (_svgView) {
        return _svgView;
    }

    __kindof ABI44_0_0RNSVGPlatformView *parent = self.superview;

    if ([parent class] == [ABI44_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI44_0_0RNSVGNode class]]) {
        _svgView = ((ABI44_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI44_0_0RCTLogError(@"ABI44_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI44_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)relativeOn:(ABI44_0_0RNSVGLength *)length relative:(CGFloat)relative
{
    ABI44_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)relativeOnWidth:(ABI44_0_0RNSVGLength *)length
{
    ABI44_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)relativeOnHeight:(ABI44_0_0RNSVGLength *)length
{
    ABI44_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)relativeOnOther:(ABI44_0_0RNSVGLength *)length
{
    ABI44_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI44_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI44_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
...
- (void)traverseSubviews:(BOOL (^)(__kindof ABI44_0_0RNSVGView *node))block
{
