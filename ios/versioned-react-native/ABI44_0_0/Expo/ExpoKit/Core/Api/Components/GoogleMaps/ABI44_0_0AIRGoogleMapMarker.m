...
@interface ABI44_0_0AIRGoogleMapMarker ()
- (id)eventFromMarker:(ABI44_0_0AIRGMSMarker*)marker;
@end

@implementation ABI44_0_0AIRGoogleMapMarker {
...
{
  if ((self = [super init])) {
    _realMarker = [[ABI44_0_0AIRGMSMarker alloc] init];
...
- (void)insertABI44_0_0ReactSubview:(id<ABI44_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  if ([subview isKindOfClass:[ABI44_0_0AIRGoogleMapCallout class]]) {
    self.calloutView = (ABI44_0_0AIRGoogleMapCallout *)subview;
  } else { // a child view of the marker
    [self iconViewInsertSubview:(UIView*)subview atIndex:atIndex+1];
  }
  ABI44_0_0AIRDummyView *dummySubview = [[ABI44_0_0AIRDummyView alloc] initWithView:(UIView *)subview];
  [super insertABI44_0_0ReactSubview:(UIView*)dummySubview atIndex:atIndex];
}

- (void)removeABI44_0_0ReactSubview:(id<ABI44_0_0RCTComponent>)dummySubview {
  UIView* subview = ((ABI44_0_0AIRDummyView*)dummySubview).view;
  ...
  [super removeABI44_0_0ReactSubview:(UIView*)dummySubview];
}
...
- (void)didTapInfoWindowOfMarker:(ABI44_0_0AIRGMSMarker *)marker point:(CGPoint)point frame:(CGRect)frame {
...
- (void)didTapInfoWindowOfMarker:(ABI44_0_0AIRGMSMarker *)marker {
    [self didTapInfoWindowOfMarker:marker point:CGPointMake(-1, -1) frame:CGRectZero];
}

- (void)didTapInfoWindowOfMarker:(ABI44_0_0AIRGMSMarker *)marker subview:(ABI44_0_0AIRGoogleMapCalloutSubview*)subview point:(CGPoint)point frame:(CGRect)frame {
...
- (void)didBeginDraggingMarker:(ABI44_0_0AIRGMSMarker *)marker {
  if (!self.onDragStart) return;
  self.onDragStart([self eventFromMarker:marker]);
}

- (void)didEndDraggingMarker:(ABI44_0_0AIRGMSMarker *)marker {
  if (!self.onDragEnd) return;
  self.onDragEnd([self eventFromMarker:marker]);
}

- (void)didDragMarker:(ABI44_0_0AIRGMSMarker *)marker {
...
- (ABI44_0_0RCTBubblingEventBlock)onPress {
...
- (void)setImageSrc:(NSString *)imageSrc
{
  ...
  if (!_iconImageView) {
    // prevent glitch with marker (cf. https://github.com/ABI44_0_0React-native-maps/ABI44_0_0React-native-maps/issues/738)
...
//                                                                   }
                                                                   ...
                                                                   //       Shouldn't required images have size info automatically via ABI44_0_0RN?
...
- (void)setIconSrc:(NSString *)iconSrc
{
  ...
  _reloadImageCancellationBlock =
  [[_bridge moduleForName:@"ImageLoader"] loadImageWithURLRequest:[ABI44_0_0RCTConvert NSURLRequest:_iconSrc]
                                          size:self.bounds.size
                                         scale:ABI44_0_0RCTScreenScale()
                                       clipped:YES
                                    resizeMode:ABI44_0_0RCTResizeModeCenter
