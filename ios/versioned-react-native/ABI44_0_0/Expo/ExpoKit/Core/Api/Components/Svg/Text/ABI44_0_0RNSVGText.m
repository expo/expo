...
@implementation ABI44_0_0RNSVGText
{
...
- (void)setInlineSize:(ABI44_0_0RNSVGLength *)inlineSize
{
...
- (void)setTextLength:(ABI44_0_0RNSVGLength *)textLength
{
...
- (void)setDeltaX:(NSArray<ABI44_0_0RNSVGLength *> *)deltaX
{
    if (deltaX == _deltaX) {
        return;
    }
...
- (void)setDeltaY:(NSArray<ABI44_0_0RNSVGLength *> *)deltaY
{
    if (deltaY == _deltaY) {
        return;
    }
...
- (void)setPositionX:(NSArray<ABI44_0_0RNSVGLength *>*)positionX
{
...
- (void)setPositionY:(NSArray<ABI44_0_0RNSVGLength *>*)positionY
{
...
- (void)setRotate:(NSArray<ABI44_0_0RNSVGLength *> *)rotate
{
    if (rotate == _rotate) {
        return;
    }
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI44_0_0RNSVGGlyphContext alloc] initWithWidth:size.width
                                                      height:size.height];
}
...
- (ABI44_0_0RNSVGText *)textRoot
{
    ABI44_0_0RNSVGText *root = self;
    while (root && [root class] != [ABI44_0_0RNSVGText class]) {
        if (![root isKindOfClass:[ABI44_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        root = (ABI44_0_0RNSVGText*)[root superview];
    }
...
- (NSString *)alignmentBaseline
{
    ...
    while (parent != nil) {
        if ([parent isKindOfClass:[ABI44_0_0RNSVGText class]]) {
            ABI44_0_0RNSVGText* node = (ABI44_0_0RNSVGText*)parent;
    ...
    if (_alignmentBaseline == nil) {
        _alignmentBaseline = ABI44_0_0RNSVGAlignmentBaselineStrings[0];
    }
    return _alignmentBaseline;
}
...
- (ABI44_0_0RNSVGText*)getTextAnchorRoot
{
    ...
    ABI44_0_0RNSVGText* node = self;
    ABI44_0_0RNSVGPlatformView* parent = [self superview];
    for (NSInteger i = [font count] - 1; i >= 0; i--) {
        ABI44_0_0RNSVGFontData* fontData = [font objectAtIndex:i];
        if (![parent isKindOfClass:[ABI44_0_0RNSVGText class]] ||
            fontData->textAnchor == ABI44_0_0RNSVGTextAnchorStart ||
            node.positionX != nil) {
            return node;
        }
        node = (ABI44_0_0RNSVGText*) parent;
