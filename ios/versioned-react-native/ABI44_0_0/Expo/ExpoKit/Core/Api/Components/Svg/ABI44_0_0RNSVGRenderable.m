...
#import "ABI44_0_0RNSVGViewBox.h"
#import "ABI44_0_0RNSVGVectorEffect.h"
#import "ABI44_0_0RNSVGBezierElement.h"
#import "ABI44_0_0RNSVGMarker.h"
#import "ABI44_0_0RNSVGMarkerPosition.h"

@implementation ABI44_0_0RNSVGRenderable
{
    NSMutableDictionary *_originProperties;
    NSArray<NSString *> *_lastMergedList;
    NSArray<NSString *> *_attributeList;
    NSArray<ABI44_0_0RNSVGLength *> *_sourceStrokeDashArray;
    CGFloat *_strokeDashArrayData;
    CGPathRef _srcHitPath;
}

static ABI44_0_0RNSVGRenderable * _contextElement;
+ (ABI44_0_0RNSVGRenderable *)contextElement { return _contextElement; }
+ (void)setContextElement:(ABI44_0_0RNSVGRenderable *)contextElement { _contextElement = contextElement; }
...
- (id)init
{
    if (self = [super init]) {
        _fillOpacity = 1;
        _strokeOpacity = 1;
        _strokeWidth = [ABI44_0_0RNSVGLength lengthWithNumber:1];
...
- (void)setFill:(ABI44_0_0RNSVGBrush *)fill
{
    if (fill == _fill) {
        return;
    }
...
- (void)setFillRule:(ABI44_0_0RNSVGCGFCRule)fillRule
{
...
- (void)setStroke:(ABI44_0_0RNSVGBrush *)stroke
{
    if (stroke == _stroke) {
        return;
    }
...
- (void)setStrokeWidth:(ABI44_0_0RNSVGLength*)strokeWidth
{
...
- (void)setStrokeDasharray:(NSArray<ABI44_0_0RNSVGLength *> *)strokeDasharray
{
...
- (void)setVectorEffect:(ABI44_0_0RNSVGVectorEffect)vectorEffect
{
...
UInt32 ABI44_0_0saturate(CGFloat value) {
...
- (void)renderTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (self.mask) {
        ...
        ABI44_0_0RNSVGMask *_maskNode = (ABI44_0_0RNSVGMask*)[self.svgView getDefinedMask:self.mask];
        ...
        for (NSUInteger i = 0; i < npixels; i++) {
            ...
            *currentPixel = ABI44_0_0saturate(luma) << 24;
            currentPixel++;
        }
...
- (void)prepareStrokeDash:(NSUInteger)count strokeDasharray:(NSArray<ABI44_0_0RNSVGLength *> *)strokeDasharray {
...
- (void)renderMarkers:(CGContextRef)context path:(CGPathRef)path rect:(const CGRect *)rect {
    ABI44_0_0RNSVGMarker *markerStart = (ABI44_0_0RNSVGMarker*)[self.svgView getDefinedMarker:self.markerStart];
    ABI44_0_0RNSVGMarker *markerMid = (ABI44_0_0RNSVGMarker*)[self.svgView getDefinedMarker:self.markerMid];
    ABI44_0_0RNSVGMarker *markerEnd = (ABI44_0_0RNSVGMarker*)[self.svgView getDefinedMarker:self.markerEnd];
    if (markerStart || markerMid || markerEnd) {
        _contextElement = self;
        NSArray<ABI44_0_0RNSVGMarkerPosition*>* positions = [ABI44_0_0RNSVGMarkerPosition fromCGPath:path];
        ...
        for (ABI44_0_0RNSVGMarkerPosition* position in positions) {
            ABI44_0_0RNSVGMarkerType type = [position type];
            ABI44_0_0RNSVGMarker *marker;
            switch (type) {
                case kStartMarker:
                    marker = markerStart;
                    break;
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (self.stroke) {
        ...
        NSArray<ABI44_0_0RNSVGLength *>* strokeDasharray = self.strokeDasharray;
...
- (ABI44_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    BOOL canReceiveTouchEvents = (self.pointerEvents != ABI44_0_0RCTPointerEventsNone && ![self isHidden]);
    ...
    if (self.clipPath) {
        ABI44_0_0RNSVGClipPath *clipNode = (ABI44_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if ([clipNode isSimpleClipPath]) {
            ...
            ABI44_0_0RNSVGRenderable *clipGroup = (ABI44_0_0RNSVGRenderable*)clipNode;
...
- (void)mergeProperties:(__kindof ABI44_0_0RNSVGRenderable *)target
{
