...
#import "ABI44_0_0RNSVGPainterBrush.h"
#import "ABI44_0_0RNSVGSolidColorBrush.h"
#import "ABI44_0_0RNSVGContextBrush.h"
#import <ABI44_0_0React/ABI44_0_0RCTLog.h>
#import <ABI44_0_0React/ABI44_0_0RCTFont.h>

NSRegularExpression *ABI44_0_0RNSVGDigitRegEx;

@implementation ABI44_0_0RCTConvert (ABI44_0_0RNSVG)

ABI44_0_0RCT_ENUM_CONVERTER(ABI44_0_0RNSVGCGFCRule, (@{
                                     @"evenodd": @(kRNSVGCGFCRuleEvenodd),
                                     @"nonzero": @(kRNSVGCGFCRuleNonzero),
                                     }), kRNSVGCGFCRuleNonzero, intValue)

ABI44_0_0RCT_ENUM_CONVERTER(ABI44_0_0RNSVGVBMOS, (@{
                                  @"meet": @(kRNSVGVBMOSMeet),
                                  @"slice": @(kRNSVGVBMOSSlice),
                                  @"none": @(kRNSVGVBMOSNone)
                                  }), kRNSVGVBMOSMeet, intValue)

ABI44_0_0RCT_ENUM_CONVERTER(ABI44_0_0RNSVGUnits, (@{
...
+ (ABI44_0_0RNSVGBrush *)ABI44_0_0RNSVGBrush:(id)json
{
    if ([json isKindOfClass:[NSNumber class]]) {
        return [[ABI44_0_0RNSVGSolidColorBrush alloc] initWithNumber:json];
    }
    if ([json isKindOfClass:[NSString class]]) {
        NSString *value = [self NSString:json];
        if (!ABI44_0_0RNSVGDigitRegEx) {
            ABI44_0_0RNSVGDigitRegEx = [NSRegularExpression regularExpressionWithPattern:@"[0-9.-]+" options:NSRegularExpressionCaseInsensitive error:nil];
        }
        NSArray<NSTextCheckingResult*> *_matches = [ABI44_0_0RNSVGDigitRegEx matchesInString:value options:0 range:NSMakeRange(0, [value length])];
        ...
        return [[ABI44_0_0RNSVGSolidColorBrush alloc] initWithArray:output];
    }
    ...
    switch (type) {
        case 0: // solid color
            ...
            return [[ABI44_0_0RNSVGSolidColorBrush alloc] initWithArray:arr];
        case 1: // brush
            return [[ABI44_0_0RNSVGPainterBrush alloc] initWithArray:arr];
        case 2: // currentColor
            return [[ABI44_0_0RNSVGBrush alloc] initWithArray:nil];
        case 3: // context-fill
            return [[ABI44_0_0RNSVGContextBrush alloc] initFill];
        case 4: // context-stroke
            return [[ABI44_0_0RNSVGContextBrush alloc] initStroke];
        default:
            ABI44_0_0RCTLogError(@"Unknown brush type: %zd", (unsigned long)type);
            return nil;
    }
}
...
{
    return [[ABI44_0_0RNSVGPathParser alloc] initWithPathString: d];
}

+ (ABI44_0_0RNSVGLength *)ABI44_0_0RNSVGLength:(id)json
{
    if ([json isKindOfClass:[NSNumber class]]) {
        return [ABI44_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
    } else if ([json isKindOfClass:[NSString class]]) {
        NSString *stringValue = (NSString *)json;
        return [ABI44_0_0RNSVGLength lengthWithString:stringValue];
    } else {
        return [[ABI44_0_0RNSVGLength alloc] init];
    }
}
...
{
    if ([json isKindOfClass:[NSNumber class]]) {
        ABI44_0_0RNSVGLength* length = [ABI44_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
        return [NSArray arrayWithObject:length];
    } else if ([json isKindOfClass:[NSArray class]]) {
        NSArray *arrayValue = (NSArray*)json;
        NSMutableArray<ABI44_0_0RNSVGLength*>* lengths = [NSMutableArray arrayWithCapacity:[arrayValue count]];
        for (id obj in arrayValue) {
            [lengths addObject:[self ABI44_0_0RNSVGLength:obj]];
        }
        return lengths;
    }  else if ([json isKindOfClass:[NSString class]]) {
        NSString *stringValue = (NSString *)json;
        ABI44_0_0RNSVGLength* length = [ABI44_0_0RNSVGLength lengthWithString:stringValue];
        return [NSArray arrayWithObject:length];
    } else {
        return nil;
    }
...
+ (CGRect)ABI44_0_0RNSVGCGRect:(id)json offset:(NSUInteger)offset
{
    ...
    if (arr.count < offset + 4) {
        ABI44_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", (ssize_t)(4 + offset), arr);
        return CGRectZero;
    }
    return (CGRect){
