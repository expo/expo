...
@implementation ABI44_0_0RCTLogBoxView {
...
- (instancetype)initWithFrame:(CGRect)frame bridge:(ABI44_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _surface = [[ABI44_0_0RCTSurface alloc] initWithBridge:bridge moduleName:@"LogBox" initialProperties:@{}];

    [_surface start];
    [_surface setSize:frame.size];

    if (![_surface synchronouslyWaitForStage:ABI44_0_0RCTSurfaceStageSurfaceDidInitialMounting timeout:1]) {
      ABI44_0_0RCTLogInfo(@"Failed to mount LogBox within 1s");
    }
...
- (void)dealloc
{
  [ABI44_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
