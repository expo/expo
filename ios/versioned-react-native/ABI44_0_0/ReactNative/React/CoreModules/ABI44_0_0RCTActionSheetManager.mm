...
#import <ABI44_0_0React/ABI44_0_0RCTActionSheetManager.h>
...
@interface ABI44_0_0RCTActionSheetManager () <UIActionSheetDelegate, ABI44_0_0NativeActionSheetManagerSpec>
@end

@implementation ABI44_0_0RCTActionSheetManager {
                ...
                anchorViewTag:(NSNumber *)anchorViewTag
{
  ...
  if (anchorViewTag) {
    sourceView = [self.bridge.uiManager viewForABI44_0_0ReactTag:anchorViewTag];
  } else {
                  ...
                  : (ABI44_0_0RCTResponseSenderBlock)callback)
{
  if (ABI44_0_0RCTRunningInAppExtension()) {
    ABI44_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  }
  ...
  NSArray<NSString *> *buttons = ABI44_0_0RCTConvertOptionalVecToArray(options.options(), ^id(NSString *element) {
    return element;
  });
  NSArray<NSNumber *> *disabledButtonIndices;
  NSInteger cancelButtonIndex =
      options.cancelButtonIndex() ? [ABI44_0_0RCTConvert NSInteger:@(*options.cancelButtonIndex())] : -1;
  NSArray<NSNumber *> *destructiveButtonIndices;
  if (options.disabledButtonIndices()) {
    disabledButtonIndices = ABI44_0_0RCTConvertVecToArray(*options.disabledButtonIndices(), ^id(double element) {
      return @(element);
    });
  }
  if (options.destructiveButtonIndices()) {
    destructiveButtonIndices = ABI44_0_0RCTConvertVecToArray(*options.destructiveButtonIndices(), ^id(double element) {
      return @(element);
    });
  } else {
  ...
  UIViewController *controller = ABI44_0_0RCTPresentedViewController();
  NSNumber *anchor = [ABI44_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  UIColor *tintColor = [ABI44_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];

  if (controller == nil) {
    ABI44_0_0RCTLogError(@"Tried to display action sheet but there is no application window. options: %@", @{
      @"title" : title,
      @"message" : message,
      @"options" : buttons,
  ...
  if (disabledButtonIndices) {
    for (NSNumber *disabledButtonIndex in disabledButtonIndices) {
      if ([disabledButtonIndex integerValue] < buttons.count) {
        ...
        ABI44_0_0RCTLogError(@"Index %@ from `disabledButtonIndices` is out of bounds. Maximum index value is %@.", @([disabledButtonIndex integerValue]), @(buttons.count - 1));
        return;
      }
    }
  }
  ...
  if (@available(iOS 13.0, *)) {
    NSString *userInterfaceStyle = [ABI44_0_0RCTConvert NSString:options.userInterfaceStyle()];
                  ...
                  : (ABI44_0_0RCTResponseSenderBlock)successCallback)
{
  ...
  NSURL *URL = [ABI44_0_0RCTConvert NSURL:options.url()];
  if (URL) {
    if ([URL.scheme.lowercaseString isEqualToString:@"data"]) {
      NSError *error;
      NSData *data = [NSData dataWithContentsOfURL:URL options:(NSDataReadingOptions)0 error:&error];
      if (!data) {
        failureCallback(@[ ABI44_0_0RCTJSErrorFromNSError(error) ]);
        return;
      }
  ...
  if (items.count == 0) {
    ABI44_0_0RCTLogError(@"No `url` or `message` to share");
    return;
  }
  ...
  NSArray *excludedActivityTypes =
      ABI44_0_0RCTConvertOptionalVecToArray(options.excludedActivityTypes(), ^id(NSString *element) {
        return element;
      });
  ...
  shareController.completionWithItemsHandler =
      ^(NSString *activityType, BOOL completed, __unused NSArray *returnedItems, NSError *activityError) {
        if (activityError) {
          failureCallback(@[ ABI44_0_0RCTJSErrorFromNSError(activityError) ]);
        } else if (completed || activityType == nil) {
          successCallback(@[ @(completed), ABI44_0_0RCTNullIfNil(activityType) ]);
        }
      };

  NSNumber *anchorViewTag = [ABI44_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  shareController.view.tintColor = [ABI44_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];
...
Class ABI44_0_0RCTActionSheetManagerCls(void)
{
  return ABI44_0_0RCTActionSheetManager.class;
}
