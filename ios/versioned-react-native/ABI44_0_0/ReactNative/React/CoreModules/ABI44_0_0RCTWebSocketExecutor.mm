...
#import <ABI44_0_0React/ABI44_0_0RCTWebSocketExecutor.h>
...
typedef void (^ABI44_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI44_0_0RCTWebSocketExecutor () <ABI44_0_0RCTSRWebSocketDelegate, ABI44_0_0RCTTurboModule>

@end

@implementation ABI44_0_0RCTWebSocketExecutor {
  ABI44_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI44_0_0RCTWSMessageCallback> *_callbacks;
...
{
  if (!_url) {
    NSInteger port = [[[_bridge bundleURL] port] integerValue] ?: ABI44_0_0RCT_METRO_PORT;
    ...
    _url = [ABI44_0_0RCTConvert NSURL:URLString];
  }

  _jsQueue = dispatch_queue_create("com.facebook.ABI44_0_0React.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  _socket = [[ABI44_0_0RCTSRWebSocket alloc] initWithURL:_url];
  ...
  if (![self connectToProxy]) {
    ...
    NSString *error = [NSString stringWithFormat:
                                    @"Connection to %@ timed out. Are you "
                                     ...
                                     "you have the right IP address in `ABI44_0_0RCTWebSocketExecutor.m`.",
                                    _url];
    _setupError = ABI44_0_0RCTErrorWithMessage(error);
    ABI44_0_0RCTFatal(_setupError);
    return;
  }
...
- (BOOL)connectToProxy
{
  ...
  return connected == 0 && _socket.readyState == ABI44_0_0RCTSR_OPEN;
}
...
- (BOOL)prepareJSRuntime
{
  ...
  if (initError) {
    ABI44_0_0RCTLogInfo(@"Websocket runtime setup failed: %@", initError);
  }
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI44_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI44_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
{
  dispatch_semaphore_signal(_socketOpenSemaphore);
  ABI44_0_0RCTLogInfo(@"WebSocket connection failed with error %@", error);
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message onReply:(ABI44_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    if (!self.valid) {
      callback(ABI44_0_0RCTErrorWithMessage(@"Runtime is not ready for debugging. Make sure Metro is running."), nil);
      return;
    }
    ...
    [self->_socket send:ABI44_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}
                      ...
                      onComplete:(ABI44_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ...
  // from completely invalidating the bridge and preventing an endless barage of ABI44_0_0RCTLog.logIfNoNativeHook
  ...
  NSDictionary<NSString *, id> *message = @{
    ...
    @"url" : ABI44_0_0RCTNullIfNil(URL.absoluteString),
  ...
  [self sendMessage:message
            onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
              if (socketError) {
                ...
                onComplete(error ? ABI44_0_0RCTErrorWithMessage(error) : nil);
              }
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI44_0_0RCTJavaScriptCallback)onComplete
{
  ABI44_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  ...
  [self sendMessage:message
            onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
              ...
              NSError *jsonError;
              id result = ABI44_0_0RCTJSONParse(reply[@"result"], &jsonError);
              NSString *error = reply[@"error"];
              onComplete(result, error ? ABI44_0_0RCTErrorWithMessage(error) : jsonError);
            }];
}
...
{
  ABI44_0_0RCTExecuteOnMainQueue(block);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI44_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI44_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
...
Class ABI44_0_0RCTWebSocketExecutorCls(void)
{
#if ABI44_0_0RCT_DEV
  return ABI44_0_0RCTWebSocketExecutor.class;
#else
  return nil;
#endif
}
