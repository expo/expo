...
#import "ABI44_0_0RCTExceptionsManager.h"
...
@interface ABI44_0_0RCTExceptionsManager () <ABI44_0_0NativeExceptionsManagerSpec>

@end

@implementation ABI44_0_0RCTExceptionsManager
...
- (instancetype)initWithDelegate:(id<ABI44_0_0RCTExceptionsManagerDelegate>)delegate
{
  if ((self = [self init])) {
    _delegate = delegate;
  }
    ...
    suppressRedBox:(BOOL)suppressRedBox
{
  if (!suppressRedBox) {
    ...
    if (_bridge) {
      ...
      ABI44_0_0RCTRedBox *redbox = [_turboModuleRegistry moduleForName:"ABI44_0_0RCTRedBox"];
     ...
     suppressRedBox:(BOOL)suppressRedBox
{
  ...
  if (!ABI44_0_0RCT_DEBUG && reloadRetries < _maxReloadAttempts) {
    reloadRetries++;
    ABI44_0_0RCTTriggerReloadCommandListeners(@"JS Crash Reload");
  } else if (!ABI44_0_0RCT_DEV || !suppressRedBox) {
    NSString *description = [@"Unhandled JS Exception: " stringByAppendingString:message];
    NSDictionary *errorInfo = @{NSLocalizedDescriptionKey : description, ABI44_0_0RCTJSStackTraceKey : stack};
    ABI44_0_0RCTFatal([NSError errorWithDomain:ABI44_0_0RCTErrorDomain code:0 userInfo:errorInfo]);
  }
}

ABI44_0_0RCT_EXPORT_METHOD(reportSoftException
...
ABI44_0_0RCT_EXPORT_METHOD(reportFatalException
                  : (NSString *)message stack
                  : (NSArray<NSDictionary *> *)stack exceptionId
                  : (double)exceptionId)
{
...
ABI44_0_0RCT_EXPORT_METHOD(updateExceptionMessage
                  : (NSString *)message stack
                  : (NSArray<NSDictionary *> *)stack exceptionId
                  : (double)exceptionId)
{
...
ABI44_0_0RCT_EXPORT_METHOD(reportUnhandledException : (NSString *)message stack : (NSArray<NSDictionary *> *)stack)
{
...
{
  return std::make_shared<ABI44_0_0facebook::ABI44_0_0React::NativeExceptionsManagerSpecJSI>(params);
}

@end

Class ABI44_0_0RCTExceptionsManagerCls(void)
{
  return ABI44_0_0RCTExceptionsManager.class;
}
