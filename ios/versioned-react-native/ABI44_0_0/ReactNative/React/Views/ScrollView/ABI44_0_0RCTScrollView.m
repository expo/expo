...
#import "ABI44_0_0RCTScrollEvent.h"
 ...
 */
@interface ABI44_0_0RCTCustomScrollView : UIScrollView <UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) UIView<ABI44_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
@property (nonatomic, assign) BOOL pinchGestureEnabled;

@end

@implementation ABI44_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI44_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI44_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI44_0_0RCTUIManager JSResponder] isKindOfClass:[ABI44_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI44_0_0React scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
      ABI44_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
      ABI44_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
- (void)setCustomRefreshControl:(UIView<ABI44_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
  ...
  // `ABI44_0_0RCTCustomRefreshContolProtocol`'s superview will always be of class
...
@interface ABI44_0_0RCTScrollView () <ABI44_0_0RCTUIManagerObserver>

@end

@implementation ABI44_0_0RCTScrollView {
  id<ABI44_0_0RCTEventDispatcherProtocol> _eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI44_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(id<ABI44_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  ABI44_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI44_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI44_0_0RCTApplyTransformationAccordingLayoutDirection(
    UIView *view,
...
- (void)setABI44_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  [super setABI44_0_0ReactLayoutDirection:layoutDirection];

  ABI44_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI44_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI44_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI44_0_0ReactSubview:view atIndex:atIndex];
  if ([view conformsToProtocol:@protocol(ABI44_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI44_0_0RCTCustomRefreshContolProtocol> *)view];
    ...
    ABI44_0_0RCTAssert(
        _contentView == nil,
        @"ABI44_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@",
        [_contentView ABI44_0_0React_recursiveDescription]);
    _contentView = view;
    ABI44_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI44_0_0ReactLayoutDirection);
...
- (void)removeABI44_0_0ReactSubview:(UIView *)subview
{
  ...
  if ([subview conformsToProtocol:@protocol(ABI44_0_0RCTCustomRefreshContolProtocol)]) {
    ...
    ABI44_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI44_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI44_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI44_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI44_0_0React_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ABI44_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI44_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:NO];
...
{
  [ABI44_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:YES];
}

#pragma mark - ScrollView delegate

#define ABI44_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData)                                    \
  {                                                                                     \
...
#define ABI44_0_0RCT_FORWARD_SCROLL_EVENT(call)                                            \
...
#define ABI44_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
  -(void)delegateMethod : (UIScrollView *)scrollView        \
  {                                                         \
    ABI44_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                  \
    ABI44_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod : scrollView);  \
  }

ABI44_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI44_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI44_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    ABI44_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll : scrollView);
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  _allowNextScrollNoMatterWhat = YES; // Ensure next scroll event is recorded, regardless of throttle
  ABI44_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging : scrollView);
}
              ...
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI44_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging
...
{
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging : scrollView willDecelerate : decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI44_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming : scrollView withView : view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI44_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming : scrollView withView : view atScale : scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI44_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating : scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI44_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI44_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation : scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI44_0_0RCTView contentInsetsForView:self];
...
{
  ABI44_0_0RCTAssertUIManagerQueue();
  [manager
      prependUIBlock:^(__unused ABI44_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI44_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI44_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI44_0_0RCTScrollEvent *scrollEvent = [[ABI44_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ABI44_0_0ReactTag:self.ABI44_0_0ReactTag
...
void ABI44_0_0RCTSendFakeScrollEvent(id<ABI44_0_0RCTEventDispatcherProtocol> eventDispatcher, NSNumber *ABI44_0_0ReactTag)
{
  ...
  ABI44_0_0RCTScrollEvent *fakeScrollEvent = [[ABI44_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI44_0_0ReactTag:ABI44_0_0ReactTag
