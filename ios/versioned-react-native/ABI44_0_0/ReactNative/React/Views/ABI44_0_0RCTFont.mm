...
typedef CGFloat ABI44_0_0RCTFontWeight;
static ABI44_0_0RCTFontWeight weightOfFont(UIFont *font)
{
  ...
  for (NSInteger i = 0; i < 0 || i < (unsigned)fontNames.count; i++) {
    if ([font.fontName.lowercaseString hasSuffix:fontNames[i]]) {
      return (ABI44_0_0RCTFontWeight)[fontWeights[i] doubleValue];
    }
  }

  NSDictionary *traits = [font.fontDescriptor objectForKey:UIFontDescriptorTraitsAttribute];
  return (ABI44_0_0RCTFontWeight)[traits[UIFontWeightTrait] doubleValue];
}
...
static ABI44_0_0RCTFontHandler defaultFontHandler;

void ABI44_0_0RCTSetDefaultFontHandler(ABI44_0_0RCTFontHandler handler)
{
  defaultFontHandler = handler;
}

BOOL ABI44_0_0RCTHasFontHandlerSet()
{
...
static NSString *FontWeightDescriptionFromUIFontWeight(UIFontWeight fontWeight)
{
  ...
  ABI44_0_0RCTAssert(NO, @"Unknown UIFontWeight passed in: %f", fontWeight);
  return @"regular";
}

static UIFont *cachedSystemFont(CGFloat size, ABI44_0_0RCTFontWeight weight)
{
...
@implementation ABI44_0_0RCTConvert (ABI44_0_0RCTFont)

+ (UIFont *)UIFont:(id)json
{
  json = [self NSDictionary:json];
  return [ABI44_0_0RCTFont updateFont:nil
                  withFamily:[ABI44_0_0RCTConvert NSString:json[@"fontFamily"]]
                        size:[ABI44_0_0RCTConvert NSNumber:json[@"fontSize"]]
                      weight:[ABI44_0_0RCTConvert NSString:json[@"fontWeight"]]
                       style:[ABI44_0_0RCTConvert NSString:json[@"fontStyle"]]
                     variant:[ABI44_0_0RCTConvert NSStringArray:json[@"fontVariant"]]
             scaleMultiplier:1];
}

ABI44_0_0RCT_ENUM_CONVERTER(
    ABI44_0_0RCTFontWeight,
    (@{
...
typedef BOOL ABI44_0_0RCTFontStyle;
ABI44_0_0RCT_ENUM_CONVERTER(
    ABI44_0_0RCTFontStyle,
    (@{
      @"normal" : @NO,
      @"italic" : @YES,
      @"oblique" : @YES,
...
typedef NSDictionary ABI44_0_0RCTFontVariantDescriptor;
+ (ABI44_0_0RCTFontVariantDescriptor *)ABI44_0_0RCTFontVariantDescriptor:(id)json
{
  ...
  ABI44_0_0RCTFontVariantDescriptor *value = mapping[json];
  if (ABI44_0_0RCT_DEBUG && !value && [json description].length > 0) {
    ABI44_0_0RCTLogError(
        @"Invalid ABI44_0_0RCTFontVariantDescriptor '%@'. should be one of: %@",
        json,
...
ABI44_0_0RCT_ARRAY_CONVERTER(ABI44_0_0RCTFontVariantDescriptor)

@end

@implementation ABI44_0_0RCTFont
...
+ (UIFont *)updateFont:(UIFont *)font
            withFamily:(NSString *)family
               ...
               variant:(NSArray<ABI44_0_0RCTFontVariantDescriptor *> *)variant
       scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  const ABI44_0_0RCTFontWeight defaultFontWeight = UIFontWeightRegular;
  const CGFloat defaultFontSize = 14;

  // Initialize properties to defaults
  CGFloat fontSize = defaultFontSize;
  ABI44_0_0RCTFontWeight fontWeight = defaultFontWeight;
  ...
  // Get font attributes
  fontSize = [ABI44_0_0RCTConvert CGFloat:size] ?: fontSize;
  if (scaleMultiplier > 0.0 && scaleMultiplier != 1.0) {
    fontSize = round(fontSize * scaleMultiplier);
  }
  familyName = [ABI44_0_0RCTConvert NSString:family] ?: familyName;
  isItalic = style ? [ABI44_0_0RCTConvert ABI44_0_0RCTFontStyle:style] : isItalic;
  fontWeight = weight ? [ABI44_0_0RCTConvert ABI44_0_0RCTFontWeight:weight] : fontWeight;
  ...
  if (!didFindFont && [UIFont fontNamesForFamilyName:familyName].count == 0) {
    ...
    if (font) {
      ...
      ABI44_0_0RCTLogError(@"Unrecognized font family '%@'", familyName);
  ...
  if (variant) {
    NSArray *fontFeatures = [ABI44_0_0RCTConvert ABI44_0_0RCTFontVariantDescriptorArray:variant];
    UIFontDescriptor *fontDescriptor = [font.fontDescriptor
