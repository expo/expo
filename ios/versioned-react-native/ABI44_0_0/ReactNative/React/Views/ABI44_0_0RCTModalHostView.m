...
@implementation ABI44_0_0RCTModalHostView {
  __weak ABI44_0_0RCTBridge *_bridge;
  BOOL _isPresented;
  ABI44_0_0RCTModalHostViewController *_modalViewController;
  ABI44_0_0RCTTouchHandler *_touchHandler;
  UIView *_ABI44_0_0ReactSubview;
  UIInterfaceOrientation _lastKnownOrientation;
}

ABI44_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithFrame : (CGRect)frame)
ABI44_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : coder)

- (instancetype)initWithBridge:(ABI44_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _modalViewController = [ABI44_0_0RCTModalHostViewController new];
    UIView *containerView = [UIView new];
    containerView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _modalViewController.view = containerView;
    _touchHandler = [[ABI44_0_0RCTTouchHandler alloc] initWithBridge:bridge];
    _isPresented = NO;
...
{
  if (_ABI44_0_0ReactSubview && _isPresented) {
    [_bridge.uiManager setSize:newBounds.size forView:_ABI44_0_0ReactSubview];
...
- (void)notifyForOrientationChange
{
  ...
  UIInterfaceOrientation currentOrientation = [ABI44_0_0RCTSharedApplication() statusBarOrientation];
...
{
  ABI44_0_0RCTAssert(_ABI44_0_0ReactSubview == nil, @"Modal view can only have one subview");
  [super insertABI44_0_0ReactSubview:subview atIndex:atIndex];
  [_touchHandler attachToView:subview];

  [_modalViewController.view insertSubview:subview atIndex:0];
  _ABI44_0_0ReactSubview = subview;
}

- (void)removeABI44_0_0ReactSubview:(UIView *)subview
{
  ABI44_0_0RCTAssert(subview == _ABI44_0_0ReactSubview, @"Cannot remove view other than modal view");
  // Superclass (category) removes the `subview` from actual `superview`.
  [super removeABI44_0_0ReactSubview:subview];
  [_touchHandler detachFromView:subview];
  _ABI44_0_0ReactSubview = nil;
}

- (void)didUpdateABI44_0_0ReactSubviews
{
  // Do nothing, as subview (singular) is managed by `insertABI44_0_0ReactSubview:atIndex:`
}
...
- (void)didMoveToWindow
{
  ...
  if (!self.userInteractionEnabled && ![self.superview.ABI44_0_0ReactSubviews containsObject:self]) {
    return;
  }

  if (!_isPresented && self.window) {
    ABI44_0_0RCTAssert(self.ABI44_0_0ReactViewController, @"Can't present modal view controller without a presenting view controller");
