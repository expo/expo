...
#import "ABI44_0_0RCTBorderDrawing.h"
...
UIAccessibilityTraits const ABI44_0_0SwitchAccessibilityTrait = 0x20000000000001;

@implementation UIView (ABI44_0_0RCTViewUnmounting)
...
- (void)ABI44_0_0React_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI44_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI44_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI44_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ABI44_0_0React_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isABI44_0_0ReactRootView]) {
      break;
    }
...
@implementation ABI44_0_0RCTView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI44_0_0RCTBorderStyleSolid;
...
ABI44_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : unused)

- (void)setABI44_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ABI44_0_0ReactLayoutDirection != layoutDirection) {
    _ABI44_0_0ReactLayoutDirection = layoutDirection;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI44_0_0RCTPointerEventsNone || _pointerEvents == ABI44_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ABI44_0_0ReactZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI44_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI44_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI44_0_0RCTLogError(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  ...
  if (actionObject) {
    _onAccessibilityAction(@{@"actionName" : actionObject[@"name"], @"actionTarget" : self.ABI44_0_0ReactTag});
  }
  return YES;
}
...
- (NSString *)accessibilityValue
{
  ...
  // Handle Switch.
  if ((self.accessibilityTraits & ABI44_0_0SwitchAccessibilityTrait) == ABI44_0_0SwitchAccessibilityTrait) {
  ...
  // Handle states which haven't already been handled in ABI44_0_0RCTViewManager.
  for (NSString *state in self.accessibilityState) {
    id val = self.accessibilityState[state];
    if (!val) {
      continue;
...
- (UIView *)ABI44_0_0ReactAccessibilityElement
{
  return self;
}
...
{
  if (self.ABI44_0_0ReactAccessibilityElement == self) {
...
- (BOOL)performAccessibilityAction:(NSString *)name
{
  if (_onAccessibilityAction && accessibilityActionsNameMap[name]) {
    _onAccessibilityAction(@{@"actionName" : name, @"actionTarget" : self.ABI44_0_0ReactTag});
    return YES;
  }
  return NO;
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ABI44_0_0ReactTag: %@;", self.ABI44_0_0ReactTag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI44_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ABI44_0_0ReactViewController;
    if (controller) {
...
- (void)ABI44_0_0React_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ABI44_0_0ReactSubviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ABI44_0_0React_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ABI44_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI44_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ABI44_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ABI44_0_0ReactSubviews.count == 0) {
  ...
  for (UIView *view in self.ABI44_0_0ReactSubviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ABI44_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ABI44_0_0React_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ABI44_0_0React_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI44_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x)
{
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ABI44_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI44_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI44_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI44_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets){
        ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
        ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
        ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
        ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets){
      ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, ABI44_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
      ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI44_0_0RCTDefaultIfNegativeTo(borderWidth, ABI44_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI44_0_0RCTCornerRadii)cornerRadii
{
  ...
  if ([[ABI44_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI44_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, _borderTopStartRadius);
    const CGFloat topEndRadius = ABI44_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, _borderTopEndRadius);
    const CGFloat bottomStartRadius = ABI44_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, _borderBottomStartRadius);
    const CGFloat bottomEndRadius = ABI44_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, _borderBottomEndRadius);
    ...
    topLeftRadius = ABI44_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI44_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI44_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI44_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius =
        ABI44_0_0RCTDefaultIfNegativeTo(radius, ABI44_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius =
        ABI44_0_0RCTDefaultIfNegativeTo(radius, ABI44_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius =
        ABI44_0_0RCTDefaultIfNegativeTo(radius, ABI44_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI44_0_0RCTDefaultIfNegativeTo(
        radius, ABI44_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI44_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI44_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI44_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI44_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI44_0_0RCTCornerRadii){
...
- (ABI44_0_0RCTBorderColors)borderColors
{
  ...
  if ([[ABI44_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    ...
    return (ABI44_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI44_0_0RCTUpdateShadowPathForView(self);

  const ABI44_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI44_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering = ABI44_0_0RCTCornerRadiiAreEqual(cornerRadii) && ABI44_0_0RCTBorderInsetsAreEqual(borderInsets) &&
      ABI44_0_0RCTBorderColorsAreEqual(borderColors) && _borderStyle == ABI44_0_0RCTBorderStyleSolid &&
...
static BOOL ABI44_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI44_0_0RCTUpdateShadowPathForView(ABI44_0_0RCTView *view)
{
  if (ABI44_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {
      // If view has a solid background color, calculate shadow path from border
      const ABI44_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI44_0_0RCTCornerInsets cornerInsets = ABI44_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI44_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    const ABI44_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI44_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      cornerRadius = cornerRadii.topLeft;

    } else {
      CAShapeLayer *shapeLayer = [CAShapeLayer layer];
      CGPathRef path =
          ABI44_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI44_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                            \
  -(void)setBorder##side##Style : (ABI44_0_0RCTBorderStyle)style \
  {                                                     \
