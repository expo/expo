 ...
 */
static NSRegularExpression *ABI44_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI44_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI44_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI44_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI44_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI44_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI44_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI44_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI44_0_0RCTNilIfNull(dict[@"collapse"]) boolValue]];
}

+ (NSArray<ABI44_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI44_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI44_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI44_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
