...
#import "ABI44_0_0RCTJavaScriptLoader.h"
...
#import "ABI44_0_0RCTMultipartDataTask.h"
#import "ABI44_0_0RCTPerformanceLogger.h"
#import "ABI44_0_0RCTUtils.h"

NSString *const ABI44_0_0RCTJavaScriptLoaderErrorDomain = @"ABI44_0_0RCTJavaScriptLoaderErrorDomain";

const UInt32 ABI44_0_0RCT_BYTECODE_ALIGNMENT = 4;
UInt32 ABI44_0_0RCTReadUInt32LE(NSData *script, UInt32 offset)
{
  return [script length] < offset + 4 ? 0 : CFSwapInt32LittleToHost(*(((uint32_t *)[script bytes]) + offset / 4));
}

bool ABI44_0_0RCTIsBytecodeBundle(NSData *script)
{
  static const UInt32 BYTECODE_BUNDLE_MAGIC_NUMBER = 0xffe7c3c3;
  return (
      [script length] > 8 && ABI44_0_0RCTReadUInt32LE(script, 0) == BYTECODE_BUNDLE_MAGIC_NUMBER &&
      ABI44_0_0RCTReadUInt32LE(script, 4) > 0);
}

@interface ABI44_0_0RCTSource () {
 @public
  NSURL *_url;
  NSData *_data;
  NSUInteger _length;
...
@implementation ABI44_0_0RCTSource

static ABI44_0_0RCTSource *ABI44_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  ABI44_0_0RCTSource *source = [ABI44_0_0RCTSource new];
  source->_url = url;
  // Multipart responses may give us an unaligned view into the buffer. This ensures memory is aligned.
  source->_data = (ABI44_0_0RCTIsBytecodeBundle(data) && ((long)[data bytes] % ABI44_0_0RCT_BYTECODE_ALIGNMENT))
      ? [[NSData alloc] initWithData:data]
      : data;
  source->_length = length;
  source->_filesChangedCount = ABI44_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI44_0_0RCTLoadingProgress
...
@implementation ABI44_0_0RCTJavaScriptLoader
...
+ (void)loadBundleAtURL:(NSURL *)scriptURL
             ...
             onComplete:(ABI44_0_0RCTSourceLoadBlock)onComplete
{
  int64_t sourceLength;
  NSError *error;
  NSData *data = [self attemptSynchronousLoadOfBundleAtURL:scriptURL sourceLength:&sourceLength error:&error];
  if (data) {
    onComplete(nil, ABI44_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError = [error.domain isEqualToString:ABI44_0_0RCTJavaScriptLoaderErrorDomain] &&
      error.code == ABI44_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI44_0_0RCTJavaScriptLoaderErrorNoScriptURL
                 userInfo:@{
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI44_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI44_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                 userInfo:@{
  ...
  ABI44_0_0facebook::ABI44_0_0React::BundleHeader header;
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI44_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                 userInfo:@{
  ...
  ABI44_0_0facebook::ABI44_0_0React::ScriptTag tag = ABI44_0_0facebook::ABI44_0_0React::parseTypeFromHeader(header);
  switch (tag) {
    case ABI44_0_0facebook::ABI44_0_0React::ScriptTag::RAMBundle:
      break;

    case ABI44_0_0facebook::ABI44_0_0React::ScriptTag::String: {
      ...
      if (error) {
        *error =
            [NSError errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                                code:ABI44_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI44_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                 userInfo:@{
...
static void parseHeaders(NSDictionary *headers, ABI44_0_0RCTSource *source)
{
...
static void attemptAsynchronousLoadOfBundleAtURL(
    NSURL *scriptURL,
    ABI44_0_0RCTSourceLoadProgressBlock onProgress,
    ABI44_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      NSError *error = nil;
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path options:NSDataReadingMappedIfSafe error:&error];
      onComplete(error, ABI44_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI44_0_0RCTMultipartDataTask *task = [[ABI44_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL
      partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
        ...
        if (error) {
          if ([error.domain isEqualToString:NSURLErrorDomain]) {
            error = [NSError
                errorWithDomain:ABI44_0_0RCTJavaScriptLoaderErrorDomain
                           code:ABI44_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                       userInfo:@{
                         NSLocalizedDescriptionKey :
                             [@"Could not connect to development server.\n\n"
                               "Ensure the following:\n"
                               "- Node server is running and available on the same network - run 'npm start' from ABI44_0_0React-native root\n"
        ...
        ABI44_0_0RCTSource *source = ABI44_0_0RCTSourceCreate(scriptURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI44_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI44_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI44_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI44_0_0RCTLoadingProgress *progress = [ABI44_0_0RCTLoadingProgress new];
...
static ABI44_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI44_0_0RCTLoadingProgress *progress = [ABI44_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI44_0_0RCTJSONParse(rawText, nil);
