...
#import <ABI44_0_0React/ABI44_0_0renderer/components/iostextinput/TextInputComponentDescriptor.h>
...
#import <ABI44_0_0React/ABI44_0_0RCTUITextField.h>
#import <ABI44_0_0React/ABI44_0_0RCTUITextView.h>
#import <ABI44_0_0React/ABI44_0_0RCTUtils.h>

#import "ABI44_0_0RCTConversions.h"
#import "ABI44_0_0RCTTextInputNativeCommands.h"
#import "ABI44_0_0RCTTextInputUtils.h"

#import "ABI44_0_0RCTFabricComponentsPlugins.h"

using namespace ABI44_0_0facebook::ABI44_0_0React;

@interface ABI44_0_0RCTTextInputComponentView () <ABI44_0_0RCTBackedTextInputDelegate, ABI44_0_0RCTTextInputViewProtocol>
@end

@implementation ABI44_0_0RCTTextInputComponentView {
  TextInputShadowNode::ConcreteStateTeller _stateTeller;
  UIView<ABI44_0_0RCTBackedTextInputViewProtocol> *_backedTextInputView;
   ...
   * if UITextView is backing text input view, inside `-[ABI44_0_0RCTTextInputComponentView textInputDidChangeSelection]` we make
   * sure to call `onChange` before `onSelectionChange` and ignore next `-[ABI44_0_0RCTTextInputComponentView
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _backedTextInputView = props.traits.multiline ? [[ABI44_0_0RCTUITextView alloc] init] : [[ABI44_0_0RCTUITextField alloc] init];
...
#pragma mark - ABI44_0_0RCTViewComponentView overrides
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  if (newTextInputProps.traits.autocapitalizationType != oldTextInputProps.traits.autocapitalizationType) {
    _backedTextInputView.autocapitalizationType =
        ABI44_0_0RCTUITextAutocapitalizationTypeFromAutocapitalizationType(newTextInputProps.traits.autocapitalizationType);
  }

  if (newTextInputProps.traits.autoCorrect != oldTextInputProps.traits.autoCorrect) {
    _backedTextInputView.autocorrectionType =
        ABI44_0_0RCTUITextAutocorrectionTypeFromOptionalBool(newTextInputProps.traits.autoCorrect);
  }
  ...
  if (newTextInputProps.traits.keyboardAppearance != oldTextInputProps.traits.keyboardAppearance) {
    _backedTextInputView.keyboardAppearance =
        ABI44_0_0RCTUIKeyboardAppearanceFromKeyboardAppearance(newTextInputProps.traits.keyboardAppearance);
  }

  if (newTextInputProps.traits.spellCheck != oldTextInputProps.traits.spellCheck) {
    _backedTextInputView.spellCheckingType =
        ABI44_0_0RCTUITextSpellCheckingTypeFromOptionalBool(newTextInputProps.traits.spellCheck);
  }
  ...
  if (newTextInputProps.traits.clearButtonMode != oldTextInputProps.traits.clearButtonMode) {
    _backedTextInputView.clearButtonMode =
        ABI44_0_0RCTUITextFieldViewModeFromTextInputAccessoryVisibilityMode(newTextInputProps.traits.clearButtonMode);
  }
  ...
  if (newTextInputProps.traits.keyboardType != oldTextInputProps.traits.keyboardType) {
    _backedTextInputView.keyboardType = ABI44_0_0RCTUIKeyboardTypeFromKeyboardType(newTextInputProps.traits.keyboardType);
  }

  if (newTextInputProps.traits.returnKeyType != oldTextInputProps.traits.returnKeyType) {
    _backedTextInputView.returnKeyType = ABI44_0_0RCTUIReturnKeyTypeFromReturnKeyType(newTextInputProps.traits.returnKeyType);
  }

  if (newTextInputProps.traits.textContentType != oldTextInputProps.traits.textContentType) {
    _backedTextInputView.textContentType = ABI44_0_0RCTUITextContentTypeFromString(newTextInputProps.traits.textContentType);
  }

  if (newTextInputProps.traits.passwordRules != oldTextInputProps.traits.passwordRules) {
    if (@available(iOS 12.0, *)) {
      _backedTextInputView.passwordRules =
          ABI44_0_0RCTUITextInputPasswordRulesFromString(newTextInputProps.traits.passwordRules);
    }
  }
  ...
  if (newTextInputProps.placeholder != oldTextInputProps.placeholder) {
    _backedTextInputView.placeholder = ABI44_0_0RCTNSStringFromString(newTextInputProps.placeholder);
  }

  if (newTextInputProps.placeholderTextColor != oldTextInputProps.placeholderTextColor) {
    _backedTextInputView.placeholderColor = ABI44_0_0RCTUIColorFromSharedColor(newTextInputProps.placeholderTextColor);
  }

  if (newTextInputProps.textAttributes != oldTextInputProps.textAttributes) {
    _backedTextInputView.defaultTextAttributes =
        ABI44_0_0RCTNSTextAttributesFromTextAttributes(newTextInputProps.getEffectiveTextAttributes(ABI44_0_0RCTFontSizeMultiplier()));
  }

  if (newTextInputProps.selectionColor != oldTextInputProps.selectionColor) {
    _backedTextInputView.tintColor = ABI44_0_0RCTUIColorFromSharedColor(newTextInputProps.selectionColor);
  }

  if (newTextInputProps.inputAccessoryViewID != oldTextInputProps.inputAccessoryViewID) {
    _backedTextInputView.inputAccessoryViewID = ABI44_0_0RCTNSStringFromString(newTextInputProps.inputAccessoryViewID);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  if (_mostRecentEventCount == data.mostRecentEventCount) {
    _comingFromJS = YES;
    [self _setAttributedString:ABI44_0_0RCTNSAttributedStringFromAttributedStringBox(data.attributedStringBox)];
    _comingFromJS = NO;
  }
}
           ...
           oldLayoutMetrics:(LayoutMetrics const &)oldLayoutMetrics
{
  ...
  _backedTextInputView.frame =
      UIEdgeInsetsInsetRect(self.bounds, ABI44_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.borderWidth));
  _backedTextInputView.textContainerInset =
      ABI44_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.contentInsets - layoutMetrics.borderWidth);
}
...
#pragma mark - ABI44_0_0RCTBackedTextInputDelegate
...
- (NSString *)textInputShouldChangeText:(NSString *)text inRange:(NSRange)range
{
  if (!_backedTextInputView.textWasPasted) {
    if (_eventEmitter) {
      ...
      keyPressMetrics.text = ABI44_0_0RCTStringFromNSString(text);
...
{
  ABI44_0_0RCTTextInputHandleCommand(self, commandName, args);
}
...
- (TextInputMetrics)_textInputMetrics
{
  ...
  metrics.text = ABI44_0_0RCTStringFromNSString(_backedTextInputView.attributedText.string);
...
- (void)_updateState
{
  ...
  data.attributedStringBox = ABI44_0_0RCTAttributedStringBoxFromNSAttributedString(attributedString);
...
- (void)_setMultiline:(BOOL)multiline
{
  ...
  UIView<ABI44_0_0RCTBackedTextInputViewProtocol> *backedTextInputView =
      multiline ? [[ABI44_0_0RCTUITextView alloc] init] : [[ABI44_0_0RCTUITextField alloc] init];
  backedTextInputView.frame = _backedTextInputView.frame;
  ABI44_0_0RCTCopyBackedTextInput(_backedTextInputView, backedTextInputView);
...
Class<ABI44_0_0RCTComponentViewProtocol> ABI44_0_0RCTTextInputCls(void)
{
  return ABI44_0_0RCTTextInputComponentView.class;
}
