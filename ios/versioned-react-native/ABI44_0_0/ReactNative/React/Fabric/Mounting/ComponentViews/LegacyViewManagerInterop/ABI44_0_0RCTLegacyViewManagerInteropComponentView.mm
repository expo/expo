...
#import <ABI44_0_0React/ABI44_0_0renderer/components/legacyviewmanagerinterop/LegacyViewManagerInteropComponentDescriptor.h>
#import <ABI44_0_0React/ABI44_0_0renderer/components/legacyviewmanagerinterop/LegacyViewManagerInteropViewProps.h>
#import <ABI44_0_0React/ABI44_0_0utils/ManagedObjectWrapper.h>
#import "ABI44_0_0RCTLegacyViewManagerInteropCoordinatorAdapter.h"

using namespace ABI44_0_0facebook::ABI44_0_0React;

static NSString *const kABI44_0_0RCTLegacyInteropChildComponentKey = @"childComponentView";
static NSString *const kABI44_0_0RCTLegacyInteropChildIndexKey = @"index";

@implementation ABI44_0_0RCTLegacyViewManagerInteropComponentView {
  NSMutableArray<NSDictionary *> *_viewsToBeMounted;
  NSMutableArray<UIView *> *_viewsToBeUnmounted;
  ABI44_0_0RCTLegacyViewManagerInteropCoordinatorAdapter *_adapter;
...
{
  return [[ABI44_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] containsObject:componentName];
}

+ (void)supportLegacyViewManagerWithName:(NSString *)componentName
{
  [[ABI44_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] addObject:componentName];
}

- (ABI44_0_0RCTLegacyViewManagerInteropCoordinator *)coordinator
{
...
- (void)mountChildComponentView:(UIView<ABI44_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  [_viewsToBeMounted addObject:@{
    kABI44_0_0RCTLegacyInteropChildIndexKey : [NSNumber numberWithInteger:index],
    kABI44_0_0RCTLegacyInteropChildComponentKey : childComponentView
  }];
}
...
{
  if (_adapter) {
    [_adapter.paperView removeABI44_0_0ReactSubview:childComponentView];
  } else {
...
- (void)finalizeUpdates:(ABI44_0_0RNComponentViewUpdateMask)updateMask
{
  ...
  if (!_adapter) {
    _adapter = [[ABI44_0_0RCTLegacyViewManagerInteropCoordinatorAdapter alloc] initWithCoordinator:self.coordinator
                                                                                 ABI44_0_0ReactTag:self.tag];
  ...
  for (NSDictionary *mountInstruction in _viewsToBeMounted) {
    NSNumber *index = mountInstruction[kABI44_0_0RCTLegacyInteropChildIndexKey];
    UIView *childView = mountInstruction[kABI44_0_0RCTLegacyInteropChildComponentKey];
    [_adapter.paperView insertABI44_0_0ReactSubview:childView atIndex:index.integerValue];
  }

  [_viewsToBeMounted removeAllObjects];

  for (UIView *view in _viewsToBeUnmounted) {
    [_adapter.paperView removeABI44_0_0ReactSubview:view];
  }

  [_viewsToBeUnmounted removeAllObjects];

  [_adapter.paperView didUpdateABI44_0_0ReactSubviews];

  if (updateMask & ABI44_0_0RNComponentViewUpdateMaskProps) {
