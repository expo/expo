...
#include "ABI44_0_0UITemplateProcessor.h"
...
SharedShadowNode UITemplateProcessor::runCommand(
  ...
  if (opcode == "createNode") {
    ...
    registers[registerNumber] = ABI44_0_0ReactNativeConfig->getBool(param);
...
SharedShadowNode UITemplateProcessor::buildShadowTree(
  ...
  LOG(ERROR) << "ABI44_0_0React ui template missing returnRoot command :(";
