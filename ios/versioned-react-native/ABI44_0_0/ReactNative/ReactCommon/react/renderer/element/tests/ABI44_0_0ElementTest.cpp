...
TEST(ElementTest, testNormalCases) {
  ...
  ABI44_0_0EXPECT_EQ(shadowNode, shadowNodeA);

  // Tags
  ABI44_0_0EXPECT_EQ(shadowNodeA->getTag(), 1);
  ABI44_0_0EXPECT_EQ(shadowNodeAA->getTag(), 2);
  ABI44_0_0EXPECT_EQ(shadowNodeAB->getTag(), 3);
  ABI44_0_0EXPECT_EQ(shadowNodeABA->getTag(), 4);

  // Children
  ABI44_0_0EXPECT_EQ(shadowNodeA->getChildren().size(), 2);
  ABI44_0_0EXPECT_EQ(shadowNodeAA->getChildren().size(), 0);
  ABI44_0_0EXPECT_EQ(shadowNodeAB->getChildren().size(), 1);
  ABI44_0_0EXPECT_EQ(shadowNodeABA->getChildren().size(), 0);
  ABI44_0_0EXPECT_EQ(
  ...
  // Props
  ABI44_0_0EXPECT_EQ(shadowNodeA->getProps()->nativeId, "node A");
  ABI44_0_0EXPECT_EQ(shadowNodeABA->getProps()->nativeId, "node ABA");
  ABI44_0_0EXPECT_EQ(shadowNodeAA->getProps(), propsAA);

  // Finalize
  ABI44_0_0EXPECT_TRUE(shadowNodeA->getSealed());
  ABI44_0_0EXPECT_TRUE(shadowNodeAA->getSealed());
  ABI44_0_0EXPECT_TRUE(shadowNodeAB->getSealed());
  ABI44_0_0EXPECT_TRUE(shadowNodeABA->getSealed());
}
