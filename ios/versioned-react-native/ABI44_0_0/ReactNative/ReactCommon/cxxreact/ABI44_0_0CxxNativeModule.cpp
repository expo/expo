...
#include "ABI44_0_0CxxNativeModule.h"
...
#include "ABI44_0_0JsArgumentHelpers.h"
#include "ABI44_0_0MessageQueueThread.h"
#include "ABI44_0_0SystraceSection.h"

using ABI44_0_0facebook::xplat::module::CxxModule;
...
std::string CxxNativeModule::getSyncMethodName(unsigned int ABI44_0_0ReactMethodId) {
  if (ABI44_0_0ReactMethodId >= methods_.size()) {
    throw std::invalid_argument(folly::to<std::string>(
        "methodId ",
        ABI44_0_0ReactMethodId,
        " out of range [0..",
        methods_.size(),
        "]"));
  }
  return methods_[ABI44_0_0ReactMethodId].name;
}
...
void CxxNativeModule::invoke(
  ...
  const auto &method = methods_[ABI44_0_0ReactMethodId];
  ...
  messageQueueThread_->runOnQueue(
      [method, params = std::move(params), first, second, callId]() {
        ...
        } catch (const ABI44_0_0facebook::xplat::JsArgumentException &ex) {
          throw;
