...
@implementation ABI44_0_0RCTPropsAnimatedNode
{
     ...
     surfacePresenter:(id<ABI44_0_0RCTSurfacePresenterStub> )surfacePresenter
{
  ...
  _managedByFabric = ABI44_0_0RCTUIManagerTypeForTagIsFabric(viewTag);
  _rootTag = nil;
}
...
- (void)performUpdate
{
  ...
  for (NSNumber *parentTag in self.parentNodes.keyEnumerator) {
    ABI44_0_0RCTAnimatedNode *parentNode = [self.parentNodes objectForKey:parentTag];
    if ([parentNode isKindOfClass:[ABI44_0_0RCTStyleAnimatedNode class]]) {
      [self->_propsDictionary addEntriesFromDictionary:[(ABI44_0_0RCTStyleAnimatedNode *)parentNode propsDictionary]];

    } else if ([parentNode isKindOfClass:[ABI44_0_0RCTValueAnimatedNode class]]) {
      NSString *property = [self propertyNameForParentTag:parentTag];
      id animatedObject = [(ABI44_0_0RCTValueAnimatedNode *)parentNode animatedObject];
      if (animatedObject) {
        self->_propsDictionary[property] = animatedObject;
      } else {
        CGFloat value = [(ABI44_0_0RCTValueAnimatedNode *)parentNode value];
