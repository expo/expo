...
@implementation ABI44_0_0RCTWrapperShadowView
{
  __weak ABI44_0_0RCTBridge *_bridge;
  ABI44_0_0RCTWrapperMeasureBlock _measureBlock;
...
- (instancetype)initWithBridge:(ABI44_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
    ABI44_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI44_0_0RCTWrapperShadowViewMeasure);
  }

  return self;
}

static ABI44_0_0YGSize ABI44_0_0RCTWrapperShadowViewMeasure(ABI44_0_0YGNodeRef node, float width, ABI44_0_0YGMeasureMode widthMode, float height, ABI44_0_0YGMeasureMode heightMode)
{
  ...
  ABI44_0_0RCTWrapperShadowView *shadowView = (__bridge ABI44_0_0RCTWrapperShadowView *)ABI44_0_0YGNodeGetContext(node);
  CGSize size = [shadowView measureWithMinimumSize:minimumSize maximumSize:maximumSize];

  return (ABI44_0_0YGSize){
    ABI44_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width),
    ABI44_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height)
  };
}
...
- (CGSize)measureWithMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  if (!_measureBlock) {
    ...
    __block ABI44_0_0RCTWrapperMeasureBlock measureBlock;
    NSNumber *ABI44_0_0ReactTag = self.ABI44_0_0ReactTag;

    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);

    dispatch_async(dispatch_get_main_queue(), ^{
      ABI44_0_0RCTUIManager *uiManager = bridge.uiManager;
      ABI44_0_0RCTWrapperView *view = (ABI44_0_0RCTWrapperView *)[uiManager viewForABI44_0_0ReactTag:ABI44_0_0ReactTag];
    ...
    if (dispatch_semaphore_wait(semaphore, timeout)) {
      ABI44_0_0RCTLogError(@"Unable to retrieve `measureBlock` for view (%@) because the main thread is busy.", self);
    }
  ...
  if (dispatch_semaphore_wait(semaphore, timeout)) {
    ABI44_0_0RCTLogError(@"Unable to compute layout for view (%@) because the main thread is busy.", self);
  }
