...
@interface ABI44_0_0RCTSurfaceHostingComponentController() <ABI44_0_0RCTSurfaceDelegate>
@end

@implementation ABI44_0_0RCTSurfaceHostingComponentController {
  ABI44_0_0RCTSurface *_surface;
}

- (instancetype)initWithComponent:(ABI44_0_0RCTSurfaceHostingComponent *)component
{
...
- (void)didUpdateComponent
{
  ...
  [self updateSurfaceWithComponent:(ABI44_0_0RCTSurfaceHostingComponent *)self.component];
}
...
- (void)updateSurfaceWithComponent:(ABI44_0_0RCTSurfaceHostingComponent *)component
{
  // Updating `surface`
  ABI44_0_0RCTSurface *const surface = component.surface;
  if (surface != _surface) {
...
{
  [self.component updateState:^(ABI44_0_0RCTSurfaceHostingComponentState *state) {
    return [ABI44_0_0RCTSurfaceHostingComponentState newWithStage:state.stage
...
{
  [self.component updateState:^(ABI44_0_0RCTSurfaceHostingComponentState *state) {
    return [ABI44_0_0RCTSurfaceHostingComponentState newWithStage:stage
...
{
  return ((ABI44_0_0RCTSurfaceHostingComponent *)self.component).options.synchronousStateUpdates && ABI44_0_0RCTIsMainQueue() ? CKUpdateModeSynchronous : CKUpdateModeAsynchronous;
}
...
- (void)mountSurfaceView
{
  ...
  ABI44_0_0RCTAssert([superview.subviews count] <= 1, @"Should never have more than a single stateful subview.");
