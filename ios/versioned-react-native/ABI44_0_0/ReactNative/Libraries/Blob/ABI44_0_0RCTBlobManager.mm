...
#import "ABI44_0_0RCTBlobCollector.h"

static NSString *const kBlobURIScheme = @"blob";

@interface ABI44_0_0RCTBlobManager () <ABI44_0_0RCTNetworkingRequestHandler, ABI44_0_0RCTNetworkingResponseHandler, ABI44_0_0RCTWebSocketContentHandler, ABI44_0_0NativeBlobModuleSpec>

@end

@implementation ABI44_0_0RCTBlobManager
{
...
ABI44_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)setBridge:(ABI44_0_0RCTBridge *)bridge
{
  ...
  ABI44_0_0facebook::ABI44_0_0React::ABI44_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI44_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI44_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI44_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI44_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
  ABI44_0_0RCTNetworking *const networking = _bridge ? _bridge.networking : [_turboModuleRegistry moduleForName:"ABI44_0_0RCTNetworking"];
...
ABI44_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(double)socketID)
{
...
ABI44_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(double)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI44_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(double)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI44_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI44_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI44_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI44_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI44_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI44_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI44_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI44_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI44_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI44_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI44_0_0RCTWebSocketContentHandler methods
...
{
  return std::make_shared<ABI44_0_0facebook::ABI44_0_0React::NativeBlobModuleSpecJSI>(params);
}

@end

Class ABI44_0_0RCTBlobManagerCls(void) {
  return ABI44_0_0RCTBlobManager.class;
}
