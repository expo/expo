...
static NSString *const ABI44_0_0RCTImageStoreURLScheme = @"rct-image-store";

@interface ABI44_0_0RCTImageStoreManager() <ABI44_0_0NativeImageStoreIOSSpec>
@end

@implementation ABI44_0_0RCTImageStoreManager
{
...
{
  ABI44_0_0RCTAssertThread(_methodQueue, @"Must be called on ABI44_0_0RCTImageStoreManager thread");
  ...
  NSString *imageTag = [NSString stringWithFormat:@"%@://%tu", ABI44_0_0RCTImageStoreURLScheme, _id++];
...
- (void)storeImage:(UIImage *)image withBlock:(void (^)(NSString *imageTag))block
{
  ABI44_0_0RCTAssertParam(block);
  dispatch_async(_methodQueue, ^{
    NSString *imageTag = [self _storeImageData:ABI44_0_0RCTGetImageData(image, 0.75)];
...
ABI44_0_0RCT_EXPORT_METHOD(removeImageForTag:(NSString *)imageTag)
{
  [_store removeObjectForKey:imageTag];
}

ABI44_0_0RCT_EXPORT_METHOD(hasImageForTag:(NSString *)imageTag
...
ABI44_0_0RCT_EXPORT_METHOD(getBase64ForTag:(NSString *)imageTag
                  ...
                  errorCallback:(ABI44_0_0RCTResponseSenderBlock)errorCallback)
{
  ...
  if (!imageData) {
    errorCallback(@[ABI44_0_0RCTJSErrorFromNSError(ABI44_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]))]);
    return;
  }
...
ABI44_0_0RCT_EXPORT_METHOD(addImageFromBase64:(NSString *)base64String
                  successCallback:(ABI44_0_0RCTResponseSenderBlock)successCallback
                  errorCallback:(ABI44_0_0RCTResponseSenderBlock)errorCallback)

{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (imageData) {
      ...
      errorCallback(@[ABI44_0_0RCTJSErrorFromNSError(ABI44_0_0RCTErrorWithMessage(@"Failed to add image from base64String"))]);
    }
  });
}

#pragma mark - ABI44_0_0RCTURLRequestHandler

- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  return [request.URL.scheme caseInsensitiveCompare:ABI44_0_0RCTImageStoreURLScheme] == NSOrderedSame;
}

- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI44_0_0RCTURLRequestDelegate>)delegate
{
  ...
  dispatch_async(_methodQueue, ^{
    ...
    if (!imageData) {
      NSError *error = ABI44_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]);
    ...
    if (!sourceRef) {
      NSError *error = ABI44_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to decode data for imageTag: %@", imageTag]);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
      return;
    }
...
@implementation ABI44_0_0RCTImageStoreManager (Deprecated)

- (NSString *)storeImage:(UIImage *)image
{
  ABI44_0_0RCTAssertMainQueue();
  ABI44_0_0RCTLogWarn(@"ABI44_0_0RCTImageStoreManager.storeImage() is deprecated and has poor performance. Use an alternative method instead.");
  __block NSString *imageTag;
  dispatch_sync(_methodQueue, ^{
    imageTag = [self _storeImageData:ABI44_0_0RCTGetImageData(image, 0.75)];
  });
  return imageTag;
}
...
{
  ABI44_0_0RCTAssertMainQueue();
  ABI44_0_0RCTLogWarn(@"ABI44_0_0RCTImageStoreManager.imageForTag() is deprecated and has poor performance. Use an alternative method instead.");
...
{
  return std::make_shared<ABI44_0_0facebook::ABI44_0_0React::NativeImageStoreIOSSpecJSI>(params);
}

@end

@implementation ABI44_0_0RCTBridge (ABI44_0_0RCTImageStoreManager)

- (ABI44_0_0RCTImageStoreManager *)imageStoreManager
{
  return [self moduleForClass:[ABI44_0_0RCTImageStoreManager class]];
}

@end

Class ABI44_0_0RCTImageStoreManagerCls(void) {
  return ABI44_0_0RCTImageStoreManager.class;
}
