...
static CGFloat ABI44_0_0RCTCeilValue(CGFloat value, CGFloat scale)
{
  return ceil(value * scale) / scale;
}

static CGFloat ABI44_0_0RCTFloorValue(CGFloat value, CGFloat scale)
{
  return floor(value * scale) / scale;
}

static CGSize ABI44_0_0RCTCeilSize(CGSize size, CGFloat scale)
{
  return (CGSize){
    ABI44_0_0RCTCeilValue(size.width, scale),
    ABI44_0_0RCTCeilValue(size.height, scale)
  };
}
...
CGRect ABI44_0_0RCTTargetRect(CGSize sourceSize, CGSize destSize,
                     CGFloat destScale, ABI44_0_0RCTResizeMode resizeMode)
{
  ...
  if (resizeMode != ABI44_0_0RCTResizeModeCenter &&
  ...
  switch (resizeMode) {
    case ABI44_0_0RCTResizeModeStretch:
    case ABI44_0_0RCTResizeModeRepeat:

      return (CGRect){CGPointZero, ABI44_0_0RCTCeilSize(destSize, destScale)};

    case ABI44_0_0RCTResizeModeContain:
      ...
      return (CGRect){
        {
          ABI44_0_0RCTFloorValue((destSize.width - sourceSize.width) / 2, destScale),
          ABI44_0_0RCTFloorValue((destSize.height - sourceSize.height) / 2, destScale),
        },
        ABI44_0_0RCTCeilSize(sourceSize, destScale)
      };

    case ABI44_0_0RCTResizeModeCover:
      ...
      if (targetAspect <= aspect) { // target is taller than content
        ...
        return (CGRect){
          {ABI44_0_0RCTFloorValue((destSize.width - sourceSize.width) / 2, destScale), 0},
          ABI44_0_0RCTCeilSize(sourceSize, destScale)
        };
        ...
        return (CGRect){
          {0, ABI44_0_0RCTFloorValue((destSize.height - sourceSize.height) / 2, destScale)},
          ABI44_0_0RCTCeilSize(sourceSize, destScale)
        };
      }

    case ABI44_0_0RCTResizeModeCenter:
...
CGAffineTransform ABI44_0_0RCTTransformFromTargetRect(CGSize sourceSize, CGRect targetRect)
{
...
CGSize ABI44_0_0RCTTargetSize(CGSize sourceSize, CGFloat sourceScale,
                     CGSize destSize, CGFloat destScale,
                     ABI44_0_0RCTResizeMode resizeMode,
                     BOOL allowUpscaling)
{
  switch (resizeMode) {
    case ABI44_0_0RCTResizeModeCenter:

      return ABI44_0_0RCTTargetRect(sourceSize, destSize, destScale, resizeMode).size;
    ...
    case ABI44_0_0RCTResizeModeStretch:
      ...
      return ABI44_0_0RCTCeilSize(destSize, destScale);

    default: {

      // Get target size
      CGSize size = ABI44_0_0RCTTargetRect(sourceSize, destSize, destScale, resizeMode).size;
      if (!allowUpscaling) {
...
BOOL ABI44_0_0RCTUpscalingRequired(CGSize sourceSize, CGFloat sourceScale,
                          CGSize destSize, CGFloat destScale,
                          ABI44_0_0RCTResizeMode resizeMode)
{
  if (CGSizeEqualToSize(destSize, CGSizeZero)) {
...
UIImage *__nullable ABI44_0_0RCTDecodeImageWithData(NSData *data,
                                           CGSize destSize,
                                           CGFloat destScale,
                                           ABI44_0_0RCTResizeMode resizeMode)
{
  ...
  if (CGSizeEqualToSize(destSize, CGSizeZero)) {
    ...
    destScale = ABI44_0_0RCTScreenScale();
  }

  if (resizeMode == UIViewContentModeScaleToFill) {
    // Decoder cannot change aspect ratio, so ABI44_0_0RCTResizeModeStretch is equivalent
    // to ABI44_0_0RCTResizeModeCover for our purposes
    resizeMode = ABI44_0_0RCTResizeModeCover;
  }

  // Calculate target size
  CGSize targetSize = ABI44_0_0RCTTargetSize(sourceSize, 1, destSize, destScale, resizeMode, NO);
  CGSize targetPixelSize = ABI44_0_0RCTSizeInPixels(targetSize, destScale);
...
NSDictionary<NSString *, id> *__nullable ABI44_0_0RCTGetImageMetadata(NSData *data)
{
  CGImageSourceRef sourceRef = CGImageSourceCreateWithData((__bridge CFDataRef)data, NULL);
  if (!sourceRef) {
    return nil;
...
NSData *__nullable ABI44_0_0RCTGetImageData(UIImage *image, float quality)
{
  ...
  if (ABI44_0_0RCTImageHasAlpha(cgImage)) {
    // get png data
...
UIImage *__nullable ABI44_0_0RCTTransformImage(UIImage *image,
                                      CGSize destSize,
                                      CGFloat destScale,
                                      CGAffineTransform transform)
{
  ...
  BOOL opaque = !ABI44_0_0RCTImageHasAlpha(image.CGImage);
...
BOOL ABI44_0_0RCTImageHasAlpha(CGImageRef image)
{
