...
NSString * const ABI39_0_0EXShellManifestResourceName = @"shell-app-manifest";

@implementation ABI39_0_0EXScopedFileSystemModule
...
- (NSArray<NSString *> *)bundledAssets
{
  ...
  dispatch_once(&once, ^{
    NSString *manifestBundlePath = [[NSBundle mainBundle] pathForResource:ABI39_0_0EXShellManifestResourceName ofType:@"json"];
    ...
    if (error) {
      ABI39_0_0UMLogError(@"Error parsing bundled manifest: %@", error);
      return;
    }
