...
@implementation ABI39_0_0RNSVGRenderable
{
    NSMutableDictionary *_originProperties;
    NSArray<NSString *> *_lastMergedList;
    NSArray<NSString *> *_attributeList;
    NSArray<ABI39_0_0RNSVGLength *> *_sourceStrokeDashArray;
    CGFloat *_strokeDashArrayData;
    CGPathRef _srcHitPath;
}

static ABI39_0_0RNSVGRenderable * _contextElement;
+ (ABI39_0_0RNSVGRenderable *)contextElement { return _contextElement; }
+ (void)setContextElement:(ABI39_0_0RNSVGRenderable *)contextElement { _contextElement = contextElement; }
...
- (id)init
{
    if (self = [super init]) {
        _fillOpacity = 1;
        _strokeOpacity = 1;
        _strokeWidth = [ABI39_0_0RNSVGLength lengthWithNumber:1];
...
- (void)setFill:(ABI39_0_0RNSVGBrush *)fill
{
    if (fill == _fill) {
        return;
    }
...
- (void)setFillRule:(ABI39_0_0RNSVGCGFCRule)fillRule
{
...
- (void)setStroke:(ABI39_0_0RNSVGBrush *)stroke
{
    if (stroke == _stroke) {
        return;
    }
...
- (void)setStrokeWidth:(ABI39_0_0RNSVGLength*)strokeWidth
{
...
- (void)setStrokeDasharray:(NSArray<ABI39_0_0RNSVGLength *> *)strokeDasharray
{
...
- (void)setVectorEffect:(ABI39_0_0RNSVGVectorEffect)vectorEffect
{
...
UInt32 ABI39_0_0saturate(CGFloat value) {
...
- (void)renderTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (self.mask) {
        ...
        ABI39_0_0RNSVGMask *_maskNode = (ABI39_0_0RNSVGMask*)[self.svgView getDefinedMask:self.mask];
        ...
        for (NSUInteger i = 0; i < npixels; i++) {
            ...
            *currentPixel = ABI39_0_0saturate(luma) << 24;
            currentPixel++;
        }
...
- (void)prepareStrokeDash:(NSUInteger)count strokeDasharray:(NSArray<ABI39_0_0RNSVGLength *> *)strokeDasharray {
...
- (void)renderMarkers:(CGContextRef)context path:(CGPathRef)path rect:(const CGRect *)rect {
    ABI39_0_0RNSVGMarker *markerStart = (ABI39_0_0RNSVGMarker*)[self.svgView getDefinedMarker:self.markerStart];
    ABI39_0_0RNSVGMarker *markerMid = (ABI39_0_0RNSVGMarker*)[self.svgView getDefinedMarker:self.markerMid];
    ABI39_0_0RNSVGMarker *markerEnd = (ABI39_0_0RNSVGMarker*)[self.svgView getDefinedMarker:self.markerEnd];
    if (markerStart || markerMid || markerEnd) {
        _contextElement = self;
        NSArray<ABI39_0_0RNSVGMarkerPosition*>* positions = [ABI39_0_0RNSVGMarkerPosition fromCGPath:path];
        ...
        for (ABI39_0_0RNSVGMarkerPosition* position in positions) {
            ABI39_0_0RNSVGMarkerType type = [position type];
            ABI39_0_0RNSVGMarker *marker;
            switch (type) {
                case kStartMarker:
                    marker = markerStart;
                    break;
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (self.fill) {
        if (self.fill.class == ABI39_0_0RNSVGBrush.class) {
    ...
    if (self.stroke) {
        ...
        NSArray<ABI39_0_0RNSVGLength *>* strokeDasharray = self.strokeDasharray;
        ...
        if (self.stroke.class == ABI39_0_0RNSVGBrush.class) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    BOOL canReceiveTouchEvents = (self.pointerEvents != ABI39_0_0RCTPointerEventsNone && ![self isHidden]);
    ...
    if (self.clipPath) {
        ABI39_0_0RNSVGClipPath *clipNode = (ABI39_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if ([clipNode isSimpleClipPath]) {
            ...
            ABI39_0_0RNSVGRenderable *clipGroup = (ABI39_0_0RNSVGRenderable*)clipNode;
