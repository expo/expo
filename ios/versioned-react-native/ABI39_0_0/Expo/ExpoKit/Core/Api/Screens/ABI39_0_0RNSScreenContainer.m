...
@interface ABI39_0_0RNSScreenContainerManager : ABI39_0_0RCTViewManager

- (void)markUpdated:(ABI39_0_0RNSScreenContainerView *)screen;

@end

@interface ABI39_0_0RNSScreenContainerView () <ABI39_0_0RCTInvalidating>

@property (nonatomic, retain) UIViewController *controller;
@property (nonatomic, retain) NSMutableSet<ABI39_0_0RNSScreenView *> *activeScreens;
@property (nonatomic, retain) NSMutableArray<ABI39_0_0RNSScreenView *> *ABI39_0_0ReactSubviews;

- (void)updateContainer;

@end

@implementation ABI39_0_0RNSScreenContainerView {
  BOOL _needUpdate;
  __weak ABI39_0_0RNSScreenContainerManager *_manager;
}

- (instancetype)initWithManager:(ABI39_0_0RNSScreenContainerManager *)manager
{
  if (self = [super init]) {
...
- (void)insertABI39_0_0ReactSubview:(ABI39_0_0RNSScreenView *)subview atIndex:(NSInteger)atIndex
{
  subview.ABI39_0_0ReactSuperview = self;
  [_ABI39_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  [subview ABI39_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
}

- (void)removeABI39_0_0ReactSubview:(ABI39_0_0RNSScreenView *)subview
{
  subview.ABI39_0_0ReactSuperview = nil;
...
- (UIViewController*)findChildControllerForScreen:(ABI39_0_0RNSScreenView*)screen
{
...
- (void)prepareDetach:(ABI39_0_0RNSScreenView *)screen
{
  [[self findChildControllerForScreen:screen] willMoveToParentViewController:nil];
}

- (void)detachScreen:(ABI39_0_0RNSScreenView *)screen
{
...
- (void)attachScreen:(ABI39_0_0RNSScreenView *)screen atIndex:(NSInteger)index
{
  [_controller addChildViewController:screen.controller];
  // the frame is already set at this moment because we adjust it in insertABI39_0_0ReactSubview. We don't
  // want to update it here as ABI39_0_0React-driven animations may already run and hence changing the frame
...
- (void)updateContainer
{
  ...
  for (ABI39_0_0RNSScreenView *screen in _ABI39_0_0ReactSubviews) {
  ...
  for (ABI39_0_0RNSScreenView *screen in orphaned) {
    activeScreenRemoved = YES;
    [self detachScreen:screen];
  }
  ...
  if ((activeScreenRemoved || activeScreenAdded) && _activeScreens.count == 1) {
    ABI39_0_0RNSScreenView *singleActiveScreen = [_activeScreens anyObject];
...
{
  if (self.window) {
    [self ABI39_0_0ReactAddControllerToClosestParent:_controller];
  }
}
...
- (void)layoutSubviews
{
  ...
  for (ABI39_0_0RNSScreenView *subview in _ABI39_0_0ReactSubviews) {
    [subview ABI39_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
@implementation ABI39_0_0RNSScreenContainerManager {
  NSMutableArray<ABI39_0_0RNSScreenContainerView *> *_markedContainers;
}
...
- (UIView *)view
{
  ...
  return [[ABI39_0_0RNSScreenContainerView alloc] initWithManager:self];
}
...
- (void)markUpdated:(ABI39_0_0RNSScreenContainerView *)screen
{
  ...
  if ([_markedContainers count] == 1) {
    ...
    ABI39_0_0RCTExecuteOnMainQueue(^{
      for (ABI39_0_0RNSScreenContainerView *container in self->_markedContainers) {
