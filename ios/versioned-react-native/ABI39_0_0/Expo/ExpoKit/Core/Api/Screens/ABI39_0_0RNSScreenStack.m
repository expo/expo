...
@interface ABI39_0_0RNSScreenStackView () <UINavigationControllerDelegate, UIAdaptivePresentationControllerDelegate, UIGestureRecognizerDelegate>
...
@interface ABI39_0_0RNSScreenStackAnimator : NSObject <UIViewControllerAnimatedTransitioning>
- (instancetype)initWithOperation:(UINavigationControllerOperation)operation;
@end

@implementation ABI39_0_0RNSScreenStackView {
  UINavigationController *_controller;
  NSMutableArray<ABI39_0_0RNSScreenView *> *_ABI39_0_0ReactSubviews;
  __weak ABI39_0_0RNSScreenStackManager *_manager;
}

- (instancetype)initWithManager:(ABI39_0_0RNSScreenStackManager*)manager
{
  if (self = [super init]) {
    _manager = manager;
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
  ...
  [ABI39_0_0RNSScreenStackHeaderConfig willShowViewController:viewController animated:animated withConfig:config];
}
...
- (void)presentationControllerDidDismiss:(UIPresentationController *)presentationController
{
  ...
  if ([screenView isKindOfClass:[ABI39_0_0RNSScreenView class]]) {
    ...
    if (self.onFinishTransitioning) {
      ...
      // finish event to arrive later than onDismiss (see ABI39_0_0RNSScreen#notifyDismiss)
...
{
  ABI39_0_0RNSScreenView *screen;
  if (operation == UINavigationControllerOperationPush) {
    screen = (ABI39_0_0RNSScreenView *) toVC.view;
  } else if (operation == UINavigationControllerOperationPop) {
    screen = (ABI39_0_0RNSScreenView *) fromVC.view;
  }
  if (screen != nil && (screen.stackAnimation == ABI39_0_0RNSScreenStackAnimationFade || screen.stackAnimation == ABI39_0_0RNSScreenStackAnimationNone)) {
    return  [[ABI39_0_0RNSScreenStackAnimator alloc] initWithOperation:operation];
  }
  return nil;
}
...
{
  // cancel touches in parent, this is needed to cancel ABI39_0_0RN touch events. For example when Touchable
  ...
  while (parent != nil && ![parent isKindOfClass:[ABI39_0_0RCTRootContentView class]]) parent = parent.superview;
  ABI39_0_0RCTRootContentView *rootView = (ABI39_0_0RCTRootContentView *)parent;
  [rootView.touchHandler cancel];

  ABI39_0_0RNSScreenView *topScreen = (ABI39_0_0RNSScreenView *)_controller.viewControllers.lastObject.view;
...
{
  if (![subview isKindOfClass:[ABI39_0_0RNSScreenView class]]) {
    ABI39_0_0RCTLogError(@"ScreenStack only accepts children of type Screen");
    return;
  }
...
- (void)ABI39_0_0ReactAddControllerToClosestParent:(UIViewController *)controller
{
  if (!controller.parentViewController) {
    UIView *parentView = (UIView *)self.ABI39_0_0ReactSuperview;
    while (parentView) {
      if (parentView.ABI39_0_0ReactViewController) {
        [parentView.ABI39_0_0ReactViewController addChildViewController:controller];
        [self addSubview:controller.view];
        _controller.interactivePopGestureRecognizer.delegate = self;
        [controller didMoveToParentViewController:parentView.ABI39_0_0ReactViewController];
      ...
      parentView = (UIView *)parentView.ABI39_0_0ReactSuperview;
    }
    return;
  }
}
...
- (void)setModalViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  for (NSUInteger i = changeRootIndex; i < controllers.count; i++) {
    if ([_presentedModals containsObject:controllers[i]]) {
      ABI39_0_0RCTAssert(false, @"Modally presented controllers are being reshuffled, this is not allowed");
    }
  }

  __weak ABI39_0_0RNSScreenStackView *weakSelf = self;
  ...
  void (^finish)(void) = ^{
    ...
    if (!isAttached || changeRootIndex >= controllers.count) {
      ...
      for (NSUInteger i = changeRootIndex; i < controllers.count; i++) {
        ...
        BOOL shouldAnimate = lastModal && [next isKindOfClass:[ABI39_0_0RNSScreen class]] && ((ABI39_0_0RNSScreenView *) next.view).stackAnimation != ABI39_0_0RNSScreenStackAnimationNone;
  ...
  if (changeRootController.presentedViewController != nil
    ...
    BOOL shouldAnimate = changeRootIndex == controllers.count && [changeRootController.presentedViewController isKindOfClass:[ABI39_0_0RNSScreen class]] && ((ABI39_0_0RNSScreenView *) changeRootController.presentedViewController.view).stackAnimation != ABI39_0_0RNSScreenStackAnimationNone;
...
- (void)setPushViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  BOOL firstTimePush = ![lastTop isKindOfClass:[ABI39_0_0RNSScreen class]];

  BOOL shouldAnimate = !firstTimePush && ((ABI39_0_0RNSScreenView *) lastTop.view).stackAnimation != ABI39_0_0RNSScreenStackAnimationNone;
  ...
  if (firstTimePush) {
    ...
    if (![controllers containsObject:lastTop]) {
      ...
      if ( ![_controller.viewControllers containsObject:top] && ((ABI39_0_0RNSScreenView *) top.view).replaceAnimation == ABI39_0_0RNSScreenReplaceAnimationPush) {
  ...
  for (ABI39_0_0RNSScreenView *screen in _ABI39_0_0ReactSubviews) {
    if (!screen.dismissed && screen.controller != nil) {
      if (pushControllers.count == 0) {
        ...
        if (screen.stackPresentation == ABI39_0_0RNSScreenStackPresentationPush) {
          [pushControllers addObject:screen.controller];
        } else {
...
@implementation ABI39_0_0RNSScreenStackManager {
  NSPointerArray *_stacks;
}

ABI39_0_0RCT_EXPORT_MODULE()

ABI39_0_0RCT_EXPORT_VIEW_PROPERTY(onFinishTransitioning, ABI39_0_0RCTDirectEventBlock);

- (UIView *)view
{
  ABI39_0_0RNSScreenStackView *view = [[ABI39_0_0RNSScreenStackView alloc] initWithManager:self];
  if (!_stacks) {
...
- (void)invalidate
{
 for (ABI39_0_0RNSScreenStackView *stack in _stacks) {
...
@implementation ABI39_0_0RNSScreenStackAnimator {
...
- (NSTimeInterval)transitionDuration:(id <UIViewControllerContextTransitioning>)transitionContext
{
  ...
  if (_operation == UINavigationControllerOperationPush) {
    ...
    screen = (ABI39_0_0RNSScreenView *)toViewController.view;
  } else if (_operation == UINavigationControllerOperationPop) {
    UIViewController* fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    screen = (ABI39_0_0RNSScreenView *)fromViewController.view;
  }

  if (screen != nil && screen.stackAnimation == ABI39_0_0RNSScreenStackAnimationNone) {
    return 0;
  }
