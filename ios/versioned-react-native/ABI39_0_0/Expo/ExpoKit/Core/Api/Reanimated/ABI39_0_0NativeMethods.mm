...
std::vector<std::pair<std::string,double>> measure(int viewTag, ABI39_0_0RCTUIManager *uiManager) {
  UIView *view = [uiManager viewForABI39_0_0ReactTag:@(viewTag)];
  ...
  if (rootView == nil || (![rootView isABI39_0_0ReactRootView])) {
    return std::vector<std::pair<std::string, double>>(0, std::make_pair("x", -1234567.0));
  }
...
void scrollTo(int scrollViewTag, ABI39_0_0RCTUIManager *uiManager, double x, double y, bool animated) {
  UIView *view = [uiManager viewForABI39_0_0ReactTag:@(scrollViewTag)];
  ABI39_0_0RCTScrollView *scrollView = (ABI39_0_0RCTScrollView *) view;
  ABI39_0_0RCTEventDispatcher* oldEventDispatcher = [scrollView valueForKey:eventDispatcherKey];
