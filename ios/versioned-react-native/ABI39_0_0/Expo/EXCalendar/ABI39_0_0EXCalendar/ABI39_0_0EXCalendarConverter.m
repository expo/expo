...
@implementation ABI39_0_0EXCalendarConverter
...
+ (NSDictionary *)serializeSource:(EKSource *)source
{
  ...
  return @{
       ...
       @"type": [ABI39_0_0EXCalendarConverter _sourceType:source.sourceType],
       @"name": ABI39_0_0UMNullIfNil(source.title)
       };
}
...
+ (NSMutableDictionary *)serializeCalendar:(EKCalendar *)calendar
{
  ...
  serializedCalendar[@"id"] = ABI39_0_0UMNullIfNil(calendar.calendarIdentifier);
  serializedCalendar[@"title"] = ABI39_0_0UMNullIfNil(calendar.title);
  serializedCalendar[@"source"] = ABI39_0_0UMNullIfNil([ABI39_0_0EXCalendarConverter serializeSource:calendar.source]);
  serializedCalendar[@"entityType"] = [ABI39_0_0EXCalendarConverter _entityType:calendar.allowedEntityTypes];
  serializedCalendar[@"color"] = calendar.CGColor ? [ABI39_0_0UMUtilities hexStringWithCGColor:calendar.CGColor] : [NSNull null];
  serializedCalendar[@"type"] = [ABI39_0_0EXCalendarConverter _calendarType:calendar.type];
  serializedCalendar[@"allowsModifications"] = @(calendar.allowsContentModifications);
  serializedCalendar[@"allowedAvailabilities"] = [ABI39_0_0EXCalendarConverter _calendarSupportedAvailabilitiesFromMask:calendar.supportedEventAvailabilities];
...
+ (NSArray *)serializeCalendars:(NSArray *)calendars
{
  ...
  for (EKCalendar *calendar in calendars) {
    [serializedCalendars addObject:[ABI39_0_0EXCalendarConverter serializeCalendar:calendar]];
  }
...
+ (NSMutableDictionary *)_serializeCalendarItem:(EKCalendarItem *)item withDateFormatter:(NSDateFormatter *)dateFormatter
{
  ...
  serializedItem[@"timeZone"] = ABI39_0_0UMNullIfNil(item.timeZone.name);
  serializedItem[@"url"] = ABI39_0_0UMNullIfNil(item.URL.absoluteString.stringByRemovingPercentEncoding);
  serializedItem[@"notes"] = ABI39_0_0UMNullIfNil(item.notes);
  serializedItem[@"alarms"] = [ABI39_0_0EXCalendarConverter _serializeAlarms:item.alarms withDateFormatter:dateFormatter];

  if (item.hasRecurrenceRules) {
    EKRecurrenceRule *rule = [item.recurrenceRules objectAtIndex:0];
    NSString *frequencyType = [ABI39_0_0EXCalendarConverter _recurrenceFrequency:[rule frequency]];
...
+ (NSArray *)serializeCalendarEvents:(NSArray *)calendarEvents
{
  ...
  for (EKEvent *event in calendarEvents) {
    ...
    [serializedCalendarEvents addObject:[ABI39_0_0EXCalendarConverter serializeCalendarEvent:event]];
  }
...
+ (NSDictionary *)serializeCalendarEvent:(EKEvent *)event
{
  ...
  NSMutableDictionary *formedCalendarEvent = [ABI39_0_0EXCalendarConverter _serializeCalendarItem:event withDateFormatter:dateFormatter];
  ...
  formedCalendarEvent[@"availability"] = [ABI39_0_0EXCalendarConverter _eventAvailability:event.availability];
  formedCalendarEvent[@"status"] = [ABI39_0_0EXCalendarConverter _eventStatus:event.status];
  formedCalendarEvent[@"organizer"] = ABI39_0_0UMNullIfNil([ABI39_0_0EXCalendarConverter _serializeAttendee:event.organizer]);
...
+ (NSArray *)serializeReminders:(NSArray<EKReminder *> *)reminders
{
  ...
  for (EKReminder *reminder in reminders) {
    [serializedReminders addObject:[ABI39_0_0EXCalendarConverter serializeReminder:reminder]];
  }
...
+ (NSDictionary *)serializeReminder:(EKReminder *)reminder
{
  ...
  NSMutableDictionary *formedReminder = [ABI39_0_0EXCalendarConverter _serializeCalendarItem:reminder withDateFormatter:dateFormatter];
...
+ (NSArray *)serializeAttendees:(NSArray<EKParticipant *> *)attendees
{
  ...
  for (EKParticipant *attendee in attendees) {
    [serializedAttendees addObject:[ABI39_0_0EXCalendarConverter _serializeAttendee:attendee]];
  }
...
+ (NSDictionary *)_serializeAttendee:(EKParticipant *)attendee
{
  ...
  if (attendee.participantRole) {
    formedAttendee[@"role"] = [ABI39_0_0EXCalendarConverter _participantRole:attendee.participantRole];
  }

  if (attendee.participantStatus) {
    formedAttendee[@"status"] = [ABI39_0_0EXCalendarConverter _participantStatus:attendee.participantStatus];
  }

  if (attendee.participantType) {
    formedAttendee[@"type"] = [ABI39_0_0EXCalendarConverter _participantType:attendee.participantType];
  }
