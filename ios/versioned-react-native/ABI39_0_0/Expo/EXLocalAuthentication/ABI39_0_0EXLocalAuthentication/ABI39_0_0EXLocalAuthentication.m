...
#import <ABI39_0_0EXLocalAuthentication/ABI39_0_0EXLocalAuthentication.h>

typedef NS_ENUM(NSInteger, ABI39_0_0EXAuthenticationType) {
  ABI39_0_0EXAuthenticationTypeFingerprint = 1,
  ABI39_0_0EXAuthenticationTypeFacialRecognition = 2,
};

@implementation ABI39_0_0EXLocalAuthentication

ABI39_0_0UM_EXPORT_MODULE(ExpoLocalAuthentication)

ABI39_0_0UM_EXPORT_METHOD_AS(supportedAuthenticationTypesAsync,
                    supportedAuthenticationTypesAsync:(ABI39_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI39_0_0UMPromiseRejectBlock)reject)
{
  NSMutableArray *results = [NSMutableArray array];
  if ([[self class] isTouchIdDevice]) {
    [results addObject:@(ABI39_0_0EXAuthenticationTypeFingerprint)];
  }
  if ([[self class] isFaceIdDevice]) {
    [results addObject:@(ABI39_0_0EXAuthenticationTypeFacialRecognition)];
  }
  resolve(results);
}

ABI39_0_0UM_EXPORT_METHOD_AS(hasHardwareAsync,
                    hasHardwareAsync:(ABI39_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI39_0_0UMPromiseRejectBlock)reject)
{
...
ABI39_0_0UM_EXPORT_METHOD_AS(isEnrolledAsync,
                    isEnrolledAsync:(ABI39_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI39_0_0UMPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;
          ...
          localizedReason:reason
                    reply:^(BOOL success, NSError *error) {
                      resolve(@{
                                ...
                                @"warning": ABI39_0_0UMNullIfNil(warningMessage),
                                });
                    }];
  } else {
