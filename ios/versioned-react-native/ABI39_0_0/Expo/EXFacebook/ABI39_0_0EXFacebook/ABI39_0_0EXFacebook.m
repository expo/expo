...
NSString * const ABI39_0_0EXFacebookMisconfiguredErrorDomain = @"ERR_FACEBOOK_MISCONFIGURED";
NSString * const ABI39_0_0EXFacebookLoginErrorDomain = @"ERR_FACEBOOK_LOGIN";

@implementation ABI39_0_0EXFacebook

ABI39_0_0UM_EXPORT_MODULE(ExponentFacebook)

ABI39_0_0UM_EXPORT_METHOD_AS(setAutoLogAppEventsEnabledAsync,
...
ABI39_0_0UM_EXPORT_METHOD_AS(setAutoInitEnabledAsync,
                    setAutoInitEnabled:(BOOL)enabled
                    resolver:(ABI39_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI39_0_0UMPromiseRejectBlock)reject)
{
  ABI39_0_0UMLogWarn(@"The `autoInitEnabled` option has been removed from Facebook SDK — we recommend to explicitly use `initializeAsync` instead.");
  resolve(nil);
}

ABI39_0_0UM_EXPORT_METHOD_AS(initializeAsync,
                    ...
                    rejecter:(ABI39_0_0UMPromiseRejectBlock)reject)
{
  ...
  if (![FBSDKSettings appID]) {
    reject(ABI39_0_0EXFacebookMisconfiguredErrorDomain, @"No FacebookAppId configured, required for initialization. Please ensure that you're either providing `appId` to `initializeAsync` as an argument or inside Info.plist.", nil);
    return;
  }
  // Caller overrides buildtime settings
  if (options[@"appName"]) {
...
ABI39_0_0UM_EXPORT_METHOD_AS(setAdvertiserIDCollectionEnabledAsync,
...
ABI39_0_0UM_EXPORT_METHOD_AS(logOutAsync,
                    logOutAsync:(ABI39_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI39_0_0UMPromiseRejectBlock)reject)
{
...
ABI39_0_0UM_EXPORT_METHOD_AS(getAuthenticationCredentialAsync,
                    getAuthenticationCredentialAsync:(ABI39_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI39_0_0UMPromiseRejectBlock)reject)
{
  FBSDKAccessToken *currentAccessToken = [FBSDKAccessToken currentAccessToken];
  resolve(ABI39_0_0UMNullIfNil([ABI39_0_0EXFacebook accessTokenNativeToJSON:currentAccessToken]));
}

ABI39_0_0UM_EXPORT_METHOD_AS(logInWithReadPermissionsAsync,
                    logInWithReadPermissionsWithConfig:(NSDictionary *)config
                    resolver:(ABI39_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI39_0_0UMPromiseRejectBlock)reject)
{
  if (![FBSDKSettings appID]) {
    reject(ABI39_0_0EXFacebookMisconfiguredErrorDomain, @"No appId configured, required for initialization. Please ensure that you're either providing `appId` to `initializeAsync` as an argument or inside Info.plist.", nil);
    return;
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    @try {
      [loginManager logInWithPermissions:permissions fromViewController:nil handler:^(FBSDKLoginManagerLoginResult *result, NSError *error) {
        if (error) {
          reject(ABI39_0_0EXFacebookLoginErrorDomain, @"Error with Facebook login", error);
          return;
        }
        ...
        NSMutableDictionary *accessToken = [NSMutableDictionary dictionaryWithDictionary:[ABI39_0_0EXFacebook accessTokenNativeToJSON:result.token]];
        accessToken[@"type"] = @"success";
        resolve(accessToken);
      }];
    }
    @catch (NSException *exception) {
      NSError *error = [[NSError alloc] initWithDomain:ABI39_0_0EXFacebookLoginErrorDomain code:650 userInfo:@{
