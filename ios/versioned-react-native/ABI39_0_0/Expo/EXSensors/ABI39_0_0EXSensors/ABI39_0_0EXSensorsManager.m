...
#import <ABI39_0_0EXSensors/ABI39_0_0EXSensorsManager.h>
#import <CoreMotion/CoreMotion.h>

@interface ABI39_0_0EXSensorsManager ()
...
@implementation ABI39_0_0EXSensorsManager
                                            ...
                                            withHandler:(void (^)(NSDictionary *event))handlerBlock
{
  ...
  if (![[self manager] isAccelerometerActive]) {
    ...
    __weak ABI39_0_0EXSensorsManager *weakSelf = self;
    [[self manager] startAccelerometerUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMAccelerometerData *data, NSError *error) {
      __strong ABI39_0_0EXSensorsManager *strongSelf = weakSelf;
      if (strongSelf) {
  ...
  [[self manager] startDeviceMotionUpdatesUsingReferenceFrame:CMAttitudeReferenceFrameXArbitraryCorrectedZVertical toQueue:[NSOperationQueue mainQueue] withHandler:^(CMDeviceMotion *data, NSError *error) {
    ...
    NSMutableDictionary *result = [NSMutableDictionary dictionaryWithDictionary:@{
                             ...
                             @"accelerationIncludingGravity": @{
                                 @"x": @((data.userAcceleration.x + data.gravity.x) * ABI39_0_0EXGravity),
                                 @"y": @((data.userAcceleration.y + data.gravity.y) * ABI39_0_0EXGravity),
                                 @"z": @((data.userAcceleration.z + data.gravity.z) * ABI39_0_0EXGravity)
                                 },
                             @"rotation": @{
    ...
    if ([[strongSelf manager] isGyroAvailable]) {
      result[@"acceleration"] = @{
        @"x": @(data.userAcceleration.x * ABI39_0_0EXGravity),
        @"y": @(data.userAcceleration.y * ABI39_0_0EXGravity),
        @"z": @(data.userAcceleration.z * ABI39_0_0EXGravity)
      };
