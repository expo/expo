...
#import <ABI39_0_0UMReactNativeAdapter/ABI39_0_0UMReactNativeAdapter.h>
...
@interface ABI39_0_0UMReactNativeAdapter ()
...
@implementation ABI39_0_0UMReactNativeAdapter
...
{
  return @[@protocol(ABI39_0_0UMAppLifecycleService), @protocol(ABI39_0_0UMUIManager), @protocol(ABI39_0_0UMJavaScriptContextProvider)];
}
...
{
  __weak ABI39_0_0UMReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI39_0_0UMReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI39_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI39_0_0RCTJSThread];
}
...
# pragma mark - ABI39_0_0UMJavaScriptContextProvider
...
- (JSGlobalContextRef)javaScriptContextRef
{
  if ([_bridge respondsToSelector:@selector(jsContextRef)]) {
    ...
    // In ABI39_0_0React-native 0.59 vm is abstracted by JSI and all JSC specific references are removed
    ...
    // WARNING: This is temporary solution that may break with new ABI39_0_0React-native releases.
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI39_0_0RCTContentDidAppearNotification,
                           ABI39_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI39_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI39_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI39_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI39_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
