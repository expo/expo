...
@property (nonatomic, strong) NSMutableArray<ABI39_0_0EXUpdatesAsset *> *assetsToLoad;
@property (nonatomic, strong) NSMutableArray<ABI39_0_0EXUpdatesAsset *> *erroredAssets;
@property (nonatomic, strong) NSMutableArray<ABI39_0_0EXUpdatesAsset *> *finishedAssets;
@property (nonatomic, strong) NSMutableArray<ABI39_0_0EXUpdatesAsset *> *existingAssets;
...
static NSString * const ABI39_0_0EXUpdatesAppLoaderErrorDomain = @"ABI39_0_0EXUpdatesAppLoader";

@implementation ABI39_0_0EXUpdatesAppLoader
...
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call ABI39_0_0EXUpdatesAppLoader#loadUpdate -- use a subclass instead" userInfo:nil];
}
...
- (void)startLoadingFromManifest:(ABI39_0_0EXUpdatesUpdate *)updateManifest
{
  ...
  if (updateManifest.isDevelopmentMode) {
    dispatch_async(_database.databaseQueue, ^{
      ...
      ABI39_0_0EXUpdatesAppLoaderSuccessBlock successBlock;
  ...
  dispatch_async(_database.databaseQueue, ^{
    ...
    ABI39_0_0EXUpdatesUpdate *existingUpdate = [self->_database updateWithId:updateManifest.updateId config:self->_config error:&existingUpdateError];
    ...
    if (existingUpdate && ![existingUpdate.scopeKey isEqualToString:updateManifest.scopeKey]) {
      ...
      NSLog(@"ABI39_0_0EXUpdatesAppLoader: Loaded an update with the same ID but a different scopeKey than one we already have on disk. This is a server error. Overwriting the scopeKey and loading the existing update.");
    }

    if (existingUpdate && existingUpdate.status == ABI39_0_0EXUpdatesUpdateStatusReady) {
      if (self->_successBlock) {
        dispatch_async(self->_completionQueue, ^{
    ...
    if (self->_updateManifest.assets && self->_updateManifest.assets.count > 0) {
      ...
      for (ABI39_0_0EXUpdatesAsset *asset in self->_updateManifest.assets) {
        // before downloading, check to see if we already have this asset in the database
        NSError *matchingAssetError;
        ABI39_0_0EXUpdatesAsset *matchingDbEntry = [self->_database assetWithKey:asset.key error:&matchingAssetError];
...
- (void)handleAssetDownloadWithData:(NSData *)data response:(nullable NSURLResponse *)response asset:(ABI39_0_0EXUpdatesAsset *)asset
{
  ...
  asset.contentHash = [ABI39_0_0EXUpdatesUtils sha256WithData:data];
...
- (BOOL)_shouldStartLoadingUpdate:(ABI39_0_0EXUpdatesUpdate *)updateManifest
{
 ...
 */
- (void)_notifyProgressWithAsset:(ABI39_0_0EXUpdatesAsset *)asset
{
  if (_assetBlock) {
    _assetBlock(asset,
...
- (void)_finish
{
  dispatch_async(_database.databaseQueue, ^{
    ...
    for (ABI39_0_0EXUpdatesAsset *existingAsset in self->_existingAssets) {
      NSError *error;
      ...
      if (!existingAssetFound) {
        ...
        existingAsset.contentHash = [ABI39_0_0EXUpdatesUtils sha256WithData:contents];
    ...
    ABI39_0_0EXUpdatesAppLoaderSuccessBlock successBlock;
    ABI39_0_0EXUpdatesAppLoaderErrorBlock errorBlock;
    ...
    dispatch_async(self->_completionQueue, ^{
      if (errorBlock) {
        errorBlock([NSError errorWithDomain:ABI39_0_0EXUpdatesAppLoaderErrorDomain
                                       code:1012
