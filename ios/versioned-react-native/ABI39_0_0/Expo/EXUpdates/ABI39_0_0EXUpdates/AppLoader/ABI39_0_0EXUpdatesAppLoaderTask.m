...
@property (nonatomic, strong) id<ABI39_0_0EXUpdatesSelectionPolicy> selectionPolicy;
@property (nonatomic, strong) dispatch_queue_t delegateQueue;

@property (nonatomic, strong) id<ABI39_0_0EXUpdatesAppLauncher> launcher;
...
- (void)_finishWithError:(nullable NSError *)error
{
  ...
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
      if (self->_isReadyToLaunch && (self->_launcher.launchAssetUrl || self->_launcher.launchedUpdate.status == ABI39_0_0EXUpdatesUpdateStatusDevelopment)) {
  ...
  dispatch_async(_loaderTaskQueue, ^{
    ...
    if (update) {
      if (!self->_hasLaunched) {
        ABI39_0_0EXUpdatesAppLauncherWithDatabase *launcher = [[ABI39_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:self->_config database:self->_database directory:self->_directory completionQueue:self->_loaderTaskQueue];
