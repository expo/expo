...
static NSString * const ABI39_0_0EXUpdatesCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const ABI39_0_0EXUpdatesCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const ABI39_0_0EXUpdatesCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation ABI39_0_0EXUpdatesCrypto
...
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"ABI39_0_0EXUpdatesCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }

  NSURL *cachedPublicKeyUrl = [cacheDirectory URLByAppendingPathComponent:ABI39_0_0EXUpdatesCryptoPublicKeyFilename];
  if (useCache) {
    ...
    ABI39_0_0EXUpdatesFileDownloader *fileDownloader = [[ABI39_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
    [fileDownloader downloadFileFromURL:[NSURL URLWithString:ABI39_0_0EXUpdatesCryptoPublicKeyUrl]
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(ABI39_0_0EXUpdatesVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[ABI39_0_0EXUpdatesCryptoPublicKeyTag UTF8String] length:[ABI39_0_0EXUpdatesCryptoPublicKeyTag length]];
