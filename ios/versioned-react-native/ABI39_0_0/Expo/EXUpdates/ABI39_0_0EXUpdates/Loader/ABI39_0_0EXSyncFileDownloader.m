...
NSString * const ABI39_0_0EXSyncFileDownloaderErrorDomain = @"ABI39_0_0EXSyncFileDownloader";
NSTimeInterval const ABI39_0_0EXSyncDefaultTimeoutInterval = 60;

@interface ABI39_0_0EXSyncFileDownloader () <NSURLSessionDataDelegate>
...
@implementation ABI39_0_0EXSyncFileDownloader
                 ...
                 errorBlock:(ABI39_0_0EXSyncFileDownloaderErrorBlock)errorBlock
{
  [self downloadDataFromURL:url successBlock:^(NSData *data, NSURLResponse *response) {
    ...
    if ([data writeToFile:destinationPath options:NSDataWritingAtomic error:&error]) {
      ...
      errorBlock([NSError errorWithDomain:ABI39_0_0EXSyncFileDownloaderErrorDomain
                                     code:1002
                                 userInfo:@{
                     ...
                     errorBlock:(ABI39_0_0EXSyncFileDownloaderErrorBlock)errorBlock
{
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url
                                                     ...
                                                     timeoutInterval:ABI39_0_0EXSyncDefaultTimeoutInterval];
  ...
  [self _downloadDataWithRequest:request successBlock:^(NSData *data, NSURLResponse *response) {
    ...
    if (isSigned) {
      ...
      [ABI39_0_0EXSyncCrypto verifySignatureWithData:(NSString *)innerManifestString
                                ...
                                cacheDirectory:cacheDirectory
                                  successBlock:^(BOOL isValid) {
                                                  if (isValid) {
                                                    ...
                                                    ABI39_0_0EXSyncManifest *update = [ABI39_0_0EXSyncManifest updateWithManifest:[mutableInnerManifest copy]
                                                                                                           config:self->_config
                                                                                                         database:database];
                                                    successBlock(update);
                                                  } else {
                                                    NSError *error = [NSError errorWithDomain:ABI39_0_0EXSyncFileDownloaderErrorDomain code:1003 userInfo:@{NSLocalizedDescriptionKey: @"Manifest verification failed"}];
      ...
      ABI39_0_0EXSyncManifest *update = [ABI39_0_0EXSyncManifest updateWithManifest:(NSDictionary *)manifest
                                                             config:self->_config
                                                           database:database];
      successBlock(update);
    }
                 ...
                 errorBlock:(ABI39_0_0EXSyncFileDownloaderErrorBlock)errorBlock
{
  ...
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:cachePolicy timeoutInterval:ABI39_0_0EXSyncDefaultTimeoutInterval];
...
- (nullable NSDictionary *)_extractManifest:(id)parsedJson error:(NSError **)error
{
  ...
  if (error) {
    *error = [NSError errorWithDomain:ABI39_0_0EXSyncFileDownloaderErrorDomain code:1009 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"No compatible update found at %@. Only %@ are supported.", _config.updateUrl.absoluteString, _config.sdkVersion]}];
  }
  return nil;
}
...
- (void)_setManifestHTTPHeaderFields:(NSMutableURLRequest *)request
{
  ...
  NSString *previousFatalError = [ABI39_0_0EXSyncLauncherNoDatabase consumeError];
...
- (NSError *)_errorFromResponse:(NSHTTPURLResponse *)response body:(NSString *)body
{
  ...
  return [NSError errorWithDomain:ABI39_0_0EXSyncFileDownloaderErrorDomain code:response.statusCode userInfo:userInfo];
}
