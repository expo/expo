...
static NSString * const ABI39_0_0EXSyncCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const ABI39_0_0EXSyncCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const ABI39_0_0EXSyncCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation ABI39_0_0EXSyncCrypto
...
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"ABI39_0_0EXSyncCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }

  NSURL *cachedPublicKeyUrl = [cacheDirectory URLByAppendingPathComponent:ABI39_0_0EXSyncCryptoPublicKeyFilename];
  if (useCache) {
    ...
    ABI39_0_0EXSyncFileDownloader *fileDownloader = [[ABI39_0_0EXSyncFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
    [fileDownloader downloadFileFromURL:[NSURL URLWithString:ABI39_0_0EXSyncCryptoPublicKeyUrl]
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(ABI39_0_0EXSyncVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[ABI39_0_0EXSyncCryptoPublicKeyTag UTF8String] length:[ABI39_0_0EXSyncCryptoPublicKeyTag length]];
