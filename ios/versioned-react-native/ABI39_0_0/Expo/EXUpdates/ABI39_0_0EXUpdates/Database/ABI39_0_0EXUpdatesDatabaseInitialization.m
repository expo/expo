...
#import <ABI39_0_0EXUpdates/ABI39_0_0EXUpdatesDatabaseInitialization+Tests.h>
...
#import <ABI39_0_0EXUpdates/ABI39_0_0EXUpdatesDatabaseUtils.h>

NS_ASSUME_NONNULL_BEGIN

static NSString * const ABI39_0_0EXUpdatesDatabaseInitializationErrorDomain = @"ABI39_0_0EXUpdatesDatabaseInitialization";
static NSString * const ABI39_0_0EXUpdatesDatabaseLatestFilename = @"expo-v6.db";

static NSString * const ABI39_0_0EXUpdatesDatabaseInitializationLatestSchema = @"\
...
@implementation ABI39_0_0EXUpdatesDatabaseInitialization
                                                ...
                                                error:(NSError ** _Nullable)error
{
  return [[self class] initializeDatabaseWithLatestSchemaInDirectory:directory
                                                            database:database
                                                          migrations:[ABI39_0_0EXUpdatesDatabaseMigrationRegistry migrations]
                                                               error:error];
}

+ (BOOL)initializeDatabaseWithLatestSchemaInDirectory:(NSURL *)directory
                                             database:(struct sqlite3 * _Nullable * _Nonnull)database
                                           migrations:(NSArray<id<ABI39_0_0EXUpdatesDatabaseMigration>> *)migrations
                                                error:(NSError ** _Nullable)error
{
  return [[self class] initializeDatabaseWithSchema:ABI39_0_0EXUpdatesDatabaseInitializationLatestSchema
                                           filename:ABI39_0_0EXUpdatesDatabaseLatestFilename
                               ...
                               error:(NSError ** _Nullable)error
{
  ...
  if (!success) {
    ...
    if ([NSFileManager.defaultManager fileExistsAtPath:dbUrl.path] &&
      ...
      if (error != nil) {
        ...
        *error = [NSError errorWithDomain:ABI39_0_0EXUpdatesDatabaseInitializationErrorDomain
                                     code:1022
  ...
  if (resultCode != SQLITE_OK) {
    NSLog(@"Error opening SQLite db: %@", [ABI39_0_0EXUpdatesDatabaseUtils errorFromSqlite:db].localizedDescription);
    sqlite3_close(db);
    ...
    if (resultCode == SQLITE_CORRUPT || resultCode == SQLITE_NOTADB) {
      ...
      if ([[NSFileManager defaultManager] moveItemAtURL:dbUrl toURL:destinationUrl error:&err]) {
        ...
        if (sqlite3_open([[dbUrl absoluteString] UTF8String], &db) != SQLITE_OK) {
          if (error != nil) {
            *error = [ABI39_0_0EXUpdatesDatabaseUtils errorFromSqlite:db];
          }
          return NO;
        }
        shouldInitializeDatabaseSchema = YES;
  ...
  if (![ABI39_0_0EXUpdatesDatabaseUtils executeSql:@"PRAGMA foreign_keys=ON;" withArgs:nil onDatabase:db error:&pragmaForeignKeysError]) {
...
+ (BOOL)_migrateDatabaseInDirectory:(NSURL *)directory withMigrations:(NSArray<id<ABI39_0_0EXUpdatesDatabaseMigration>> *)migrations
{
  NSURL *latestURL = [directory URLByAppendingPathComponent:ABI39_0_0EXUpdatesDatabaseLatestFilename];
  ...
  [migrations enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id<ABI39_0_0EXUpdatesDatabaseMigration> migration, NSUInteger idx, BOOL *stop) {
  ...
  if (existingURL) {
    ...
    if (sqlite3_open(latestURL.absoluteString.UTF8String, &db) != SQLITE_OK) {
      NSLog(@"Error opening migrated SQLite db: %@", [ABI39_0_0EXUpdatesDatabaseUtils errorFromSqlite:db].localizedDescription);
      sqlite3_close(db);
      return NO;
    }
    ...
    for (NSUInteger i = startingMigrationIndex; i < migrations.count; i++) {
      ...
      id<ABI39_0_0EXUpdatesDatabaseMigration> migration = migrations[i];
      if (![migration runMigrationOnDatabase:db error:&migrationError]) {
        NSLog(@"Error migrating SQLite db: %@", [ABI39_0_0EXUpdatesDatabaseUtils errorFromSqlite:db].localizedDescription);
        sqlite3_close(db);
        return NO;
      }
    }
