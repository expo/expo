...
static NSString * const ABI39_0_0EXUpdatesErrorLogFile = @"expo-error.log";

@interface ABI39_0_0EXUpdatesAppLauncherNoDatabase ()
...
@implementation ABI39_0_0EXUpdatesAppLauncherNoDatabase

- (void)launchUpdateWithConfig:(ABI39_0_0EXUpdatesConfig *)config
{
  _launchedUpdate = [ABI39_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:config database:nil];
  if (_launchedUpdate) {
    if (_launchedUpdate.status == ABI39_0_0EXUpdatesUpdateStatusEmbedded) {
      ...
      _launchAssetUrl = [[NSBundle mainBundle] URLForResource:ABI39_0_0EXUpdatesEmbeddedBundleFilename withExtension:ABI39_0_0EXUpdatesEmbeddedBundleFileType];
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:ABI39_0_0EXUpdatesErrorLogFile] path];
}
