...
@interface ABI39_0_0EXUpdatesAppController ()
...
@implementation ABI39_0_0EXUpdatesAppController

+ (instancetype)sharedInstance
{
  static ABI39_0_0EXUpdatesAppController *theController;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!theController) {
      theController = [[ABI39_0_0EXUpdatesAppController alloc] init];
    }
  });
...
- (void)start
{
  ...
  if (!_config.updateUrl) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"expo-updates is enabled, but no valid URL is configured under ABI39_0_0EXUpdatesURL. If you are making a release build for the first time, make sure you have run `expo publish` at least once."
                                 userInfo:@{}];
  }
...
- (void)requestRelaunchWithCompletion:(ABI39_0_0EXUpdatesAppRelaunchCompletionBlock)completion
{
  if (_bridge) {
    ...
    NSLog(@"ABI39_0_0EXUpdatesAppController: Failed to reload because bridge was nil. Did you set the bridge property on the controller singleton?");
    completion(NO);
  }
}
...
# pragma mark - ABI39_0_0EXUpdatesAppLoaderTaskDelegate

- (BOOL)appLoaderTask:(ABI39_0_0EXUpdatesAppLoaderTask *)appLoaderTask didLoadCachedUpdate:(nonnull ABI39_0_0EXUpdatesUpdate *)update
{
  return YES;
}
...
- (void)appLoaderTask:(ABI39_0_0EXUpdatesAppLoaderTask *)appLoaderTask didFinishWithLauncher:(id<ABI39_0_0EXUpdatesAppLauncher>)launcher isUpToDate:(BOOL)isUpToDate
{
  _launcher = launcher;
  if (self->_delegate) {
    [ABI39_0_0EXUpdatesUtils runBlockOnMainThread:^{
