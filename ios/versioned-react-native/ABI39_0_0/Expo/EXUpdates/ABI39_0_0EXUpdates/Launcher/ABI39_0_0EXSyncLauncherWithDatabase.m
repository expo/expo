...
@interface ABI39_0_0EXSyncLauncherWithDatabase ()

@property (nullable, nonatomic, strong, readwrite) ABI39_0_0EXSyncManifest *launchedUpdate;
...
@property (nonatomic, copy) ABI39_0_0EXSyncLauncherCompletionBlock completion;
...
static NSString * const ABI39_0_0EXSyncLauncherErrorDomain = @"AppLauncher";

@implementation ABI39_0_0EXSyncLauncherWithDatabase
                   ...
                   completionQueue:(dispatch_queue_t)completionQueue
{
  dispatch_async(database.databaseQueue, ^{
    NSError *error;
    NSArray<ABI39_0_0EXSyncManifest *> *launchableUpdates = [database launchableUpdatesWithConfig:config error:&error];
    dispatch_async(completionQueue, ^{
      ...
      ABI39_0_0EXSyncManifest *embeddedManifest = [ABI39_0_0EXSyncEmbeddedLoader embeddedManifestWithConfig:config database:database];
      NSMutableArray<ABI39_0_0EXSyncManifest *>*filteredLaunchableUpdates = [NSMutableArray new];
      for (ABI39_0_0EXSyncManifest *update in launchableUpdates) {
        if (update.status == ABI39_0_0EXSyncManifestStatusEmbedded) {
...
{
  NSAssert(!_completion, @"ABI39_0_0EXSyncLauncher:launchUpdateWithSelectionPolicy:successBlock should not be called twice on the same instance");
  _completion = completion;

  if (!_launchedUpdate) {
    [[self class] launchableUpdateWithConfig:_config database:_database selectionPolicy:selectionPolicy completion:^(NSError * _Nullable error, ABI39_0_0EXSyncManifest * _Nullable launchableUpdate) {
      if (error || !launchableUpdate) {
        if (self->_completion) {
          dispatch_async(self->_completionQueue, ^{
            ...
            self->_completion([NSError errorWithDomain:ABI39_0_0EXSyncLauncherErrorDomain code:1011 userInfo:userInfo], NO);
          });
        }
      } else {
...
{
  if (_launchedUpdate.status == ABI39_0_0EXSyncManifestStatusEmbedded) {
    NSAssert(_assetFilesMap == nil, @"assetFilesMap should be null for embedded updates");
    _launchAssetUrl = [[NSBundle mainBundle] URLForResource:ABI39_0_0EXSyncBareEmbeddedBundleFilename withExtension:ABI39_0_0EXSyncBareEmbeddedBundleFileType];
    ...
    return;
  } else if (_launchedUpdate.status == ABI39_0_0EXSyncManifestStatusDevelopment) {
    dispatch_async(self->_completionQueue, ^{
  ...
  if (_launchedUpdate) {
    ...
    for (ABI39_0_0EXSyncAsset *asset in _launchedUpdate.assets) {
...
- (void)_ensureAssetExists:(ABI39_0_0EXSyncAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
  [self _checkExistenceOfAsset:asset withLocalUrl:assetLocalUrl completion:^(BOOL exists) {
    ...
    [self _maybeCopyAssetFromMainBundle:asset withLocalUrl:assetLocalUrl completion:^(BOOL success, NSError * _Nullable error) {
      ...
      [self _downloadAsset:asset withLocalUrl:assetLocalUrl completion:^(NSError * _Nullable error, ABI39_0_0EXSyncAsset *asset, NSURL *assetLocalUrl) {
        if (error) {
          if (asset.isLaunchAsset) {
...
- (void)_checkExistenceOfAsset:(ABI39_0_0EXSyncAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
...
- (void)_maybeCopyAssetFromMainBundle:(ABI39_0_0EXSyncAsset *)asset
                         withLocalUrl:(NSURL *)assetLocalUrl
                           completion:(void (^)(BOOL success, NSError * _Nullable error))completion
{
  ABI39_0_0EXSyncManifest *embeddedManifest = [ABI39_0_0EXSyncEmbeddedLoader embeddedManifestWithConfig:_config database:_database];
  if (embeddedManifest) {
    ABI39_0_0EXSyncAsset *matchingAsset;
    for (ABI39_0_0EXSyncAsset *embeddedAsset in embeddedManifest.assets) {
...
- (void)_downloadAsset:(ABI39_0_0EXSyncAsset *)asset
          withLocalUrl:(NSURL *)assetLocalUrl
            completion:(void (^)(NSError * _Nullable error, ABI39_0_0EXSyncAsset *asset, NSURL *assetLocalUrl))completion
{
  if (!asset.url) {
    completion([NSError errorWithDomain:ABI39_0_0EXSyncLauncherErrorDomain code:1007 userInfo:@{NSLocalizedDescriptionKey: @"Failed to download asset with no URL provided"}], asset, assetLocalUrl);
  }
  dispatch_async([ABI39_0_0EXSyncFileDownloader assetFilesQueue], ^{
...
- (ABI39_0_0EXSyncFileDownloader *)downloader
{
  if (!_downloader) {
    _downloader = [[ABI39_0_0EXSyncFileDownloader alloc] initWithUpdatesConfig:_config];
  }
  return _downloader;
}
