...
#define ABI39_0_0UM_IS_METHOD_EXPORTED(methodName) \
[methodName hasPrefix:@ABI39_0_0EXPAND_AND_QUOTE(ABI39_0_0UM_EXPORTED_METHODS_PREFIX)]

static const NSString *noNameExceptionName = @"No custom +(const NSString *)exportedModuleName implementation.";
static const NSString *noNameExceptionReasonFormat = @"You've subclassed an ABI39_0_0UMExportedModule in %@, but didn't override the +(const NSString *)exportedModuleName method. Override this method and return a name for your exported module.";
...
@interface ABI39_0_0UMExportedModule ()
...
@implementation ABI39_0_0UMExportedModule
  ...
  while (klass) {
    ...
    @try {
      for(int i = 0; i < methodsCount; i++) {
        ...
        if (ABI39_0_0UM_IS_METHOD_EXPORTED(methodName)) {
          IMP imp = method_getImplementation(method);
          const ABI39_0_0UMMethodInfo *info = ((const ABI39_0_0UMMethodInfo *(*)(id, SEL))imp)(klass, methodSelector);
...
- (void)callExportedMethod:(NSString *)methodName withArguments:(NSArray *)arguments resolver:(ABI39_0_0UMPromiseResolveBlock)resolve rejecter:(ABI39_0_0UMPromiseRejectBlock)reject
{
  ...
  if (methodSignature == nil) {
    ...
    // it means that it has been exported with ABI39_0_0UM_IMPORT_METHOD and if we cannot find method signature
