...
#import "ABI39_0_0RCTAttributedTextUtils.h"

#include <ABI39_0_0React/core/LayoutableShadowNode.h>
#include <ABI39_0_0React/textlayoutmanager/ABI39_0_0RCTFontProperties.h>
#include <ABI39_0_0React/textlayoutmanager/ABI39_0_0RCTFontUtils.h>
#include <ABI39_0_0React/textlayoutmanager/ABI39_0_0RCTTextPrimitivesConversions.h>
#include <ABI39_0_0React/utils/ManagedObjectWrapper.h>

using namespace ABI39_0_0facebook::ABI39_0_0React;

@implementation ABI39_0_0RCTWeakEventEmitterWrapper {
...
inline static UIFontWeight ABI39_0_0RCTUIFontWeightFromInteger(NSInteger fontWeight)
{
...
inline static UIFont *ABI39_0_0RCTEffectiveFontFromTextAttributes(const TextAttributes &textAttributes)
{
  NSString *fontFamily = [NSString stringWithCString:textAttributes.fontFamily.c_str() encoding:NSUTF8StringEncoding];

  ABI39_0_0RCTFontProperties fontProperties;
  ...
  fontProperties.style = textAttributes.fontStyle.hasValue()
      ? ABI39_0_0RCTFontStyleFromFontStyle(textAttributes.fontStyle.value())
      : ABI39_0_0RCTFontStyleUndefined;
  fontProperties.variant = textAttributes.fontVariant.hasValue()
      ? ABI39_0_0RCTFontVariantFromFontVariant(textAttributes.fontVariant.value())
      : ABI39_0_0RCTFontVariantUndefined;
  fontProperties.weight = textAttributes.fontWeight.hasValue()
      ? ABI39_0_0RCTUIFontWeightFromInteger((NSInteger)textAttributes.fontWeight.value())
      : NAN;
  fontProperties.sizeMultiplier = textAttributes.fontSizeMultiplier;

  return ABI39_0_0RCTFontWithFontProperties(fontProperties);
}

inline static CGFloat ABI39_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(const TextAttributes &textAttributes)
{
...
inline static UIColor *ABI39_0_0RCTEffectiveForegroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveForegroundColor = ABI39_0_0RCTUIColorFromSharedColor(textAttributes.foregroundColor) ?: [UIColor blackColor];
...
inline static UIColor *ABI39_0_0RCTEffectiveBackgroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveBackgroundColor = ABI39_0_0RCTUIColorFromSharedColor(textAttributes.backgroundColor);
...
NSDictionary<NSAttributedStringKey, id> *ABI39_0_0RCTNSTextAttributesFromTextAttributes(TextAttributes const &textAttributes)
{
  NSMutableDictionary<NSAttributedStringKey, id> *attributes = [NSMutableDictionary dictionaryWithCapacity:10];

  // Font
  UIFont *font = ABI39_0_0RCTEffectiveFontFromTextAttributes(textAttributes);
  if (font) {
    attributes[NSFontAttributeName] = font;
  }

  // Colors
  UIColor *effectiveForegroundColor = ABI39_0_0RCTEffectiveForegroundColorFromTextAttributes(textAttributes);
  ...
  if (textAttributes.backgroundColor || !isnan(textAttributes.opacity)) {
    attributes[NSBackgroundColorAttributeName] = ABI39_0_0RCTEffectiveBackgroundColorFromTextAttributes(textAttributes);
  }
  ...
  if (textAttributes.alignment.hasValue()) {
    ...
    paragraphStyle.alignment = ABI39_0_0RCTNSTextAlignmentFromTextAlignment(textAlignment);
    isParagraphStyleUsed = YES;
  }

  if (textAttributes.baseWritingDirection.hasValue()) {
    paragraphStyle.baseWritingDirection =
        ABI39_0_0RCTNSWritingDirectionFromWritingDirection(textAttributes.baseWritingDirection.value());
    isParagraphStyleUsed = YES;
  }

  if (!isnan(textAttributes.lineHeight)) {
    CGFloat lineHeight = textAttributes.lineHeight * ABI39_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(textAttributes);
  ...
  if (textAttributes.textDecorationLineType.value_or(TextDecorationLineType::None) != TextDecorationLineType::None) {
    ...
    NSUnderlineStyle style = ABI39_0_0RCTNSUnderlineStyleFromStyleAndPattern(
    ...
    UIColor *textDecorationColor = ABI39_0_0RCTUIColorFromSharedColor(textAttributes.textDecorationColor);
  ...
  if (textAttributes.textShadowOffset.hasValue()) {
    ...
    shadow.shadowColor = ABI39_0_0RCTUIColorFromSharedColor(textAttributes.textShadowColor);
    attributes[NSShadowAttributeName] = shadow;
  }

  // Special
  if (textAttributes.isHighlighted) {
    attributes[ABI39_0_0RCTAttributedStringIsHighlightedAttributeName] = @YES;
  }

  return [attributes copy];
}

NSAttributedString *ABI39_0_0RCTNSAttributedStringFromAttributedString(const AttributedString &attributedString)
{
    ...
    if (fragment.isAttachment()) {
      ...
      nsAttributedStringFragment = [[NSMutableAttributedString alloc]
          initWithString:string
              attributes:ABI39_0_0RCTNSTextAttributesFromTextAttributes(fragment.textAttributes)];
    }

    if (fragment.parentShadowView.componentHandle) {
      ABI39_0_0RCTWeakEventEmitterWrapper *eventEmitterWrapper = [ABI39_0_0RCTWeakEventEmitterWrapper new];
      eventEmitterWrapper.eventEmitter = fragment.parentShadowView.eventEmitter;

      NSDictionary<NSAttributedStringKey, id> *additionalTextAttributes =
          @{ABI39_0_0RCTAttributedStringEventEmitterKey : eventEmitterWrapper};
...
NSAttributedString *ABI39_0_0RCTNSAttributedStringFromAttributedStringBox(AttributedStringBox const &attributedStringBox)
{
  switch (attributedStringBox.getMode()) {
    case AttributedStringBox::Mode::Value:
      return ABI39_0_0RCTNSAttributedStringFromAttributedString(attributedStringBox.getValue());
    case AttributedStringBox::Mode::OpaquePointer:
      return (NSAttributedString *)unwrapManagedObject(attributedStringBox.getOpaquePointer());
  }
}

AttributedStringBox ABI39_0_0RCTAttributedStringBoxFromNSAttributedString(NSAttributedString *nsAttributedString)
{
