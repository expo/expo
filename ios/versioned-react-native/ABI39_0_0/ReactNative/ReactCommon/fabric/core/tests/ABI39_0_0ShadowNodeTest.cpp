...
#include "ABI39_0_0TestComponent.h"
...
TEST_F(ShadowNodeTest, handleShadowNodeCreation) {
  ABI39_0_0EXPECT_FALSE(nodeZ_->getSealed());
  ABI39_0_0EXPECT_STREQ(nodeZ_->getComponentName(), "Test");
  ABI39_0_0EXPECT_EQ(nodeZ_->getTag(), 18);
  ABI39_0_0EXPECT_EQ(nodeZ_->getSurfaceId(), surfaceId_);
  ABI39_0_0EXPECT_EQ(nodeZ_->getEventEmitter(), nullptr);
  ABI39_0_0EXPECT_EQ(nodeZ_->getChildren().size(), 0);
}

TEST_F(ShadowNodeTest, handleSealRecusive) {
  nodeZ_->sealRecursive();
  ABI39_0_0EXPECT_TRUE(nodeZ_->getSealed());
  ABI39_0_0EXPECT_TRUE(nodeZ_->getProps()->getSealed());
}
...
TEST_F(ShadowNodeTest, handleShadowNodeSimpleCloning) {
  ...
  ABI39_0_0EXPECT_STREQ(nodeA_->getComponentName(), nodeARevision2->getComponentName());
  ABI39_0_0EXPECT_EQ(nodeA_->getTag(), nodeARevision2->getTag());
  ABI39_0_0EXPECT_EQ(nodeA_->getSurfaceId(), nodeARevision2->getSurfaceId());
  ABI39_0_0EXPECT_EQ(nodeA_->getEventEmitter(), nodeARevision2->getEventEmitter());
}

TEST_F(ShadowNodeTest, handleShadowNodeMutation) {
  auto nodeABChildren = nodeAB_->getChildren();
  ABI39_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI39_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABA_);
  ABI39_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);
  ...
  ABI39_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI39_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABArevision2);
  ABI39_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);

  // Seal the entire tree.
  nodeAB_->sealRecursive();
  ABI39_0_0EXPECT_TRUE(nodeAB_->getSealed());
  ABI39_0_0EXPECT_TRUE(nodeABArevision2->getSealed());
  ABI39_0_0EXPECT_TRUE(nodeABB_->getSealed());
}
...
TEST_F(ShadowNodeTest, handleCloneFunction) {
  ...
  ABI39_0_0EXPECT_NE(nodeAB_, nodeABClone);
  ...
  ABI39_0_0EXPECT_EQ(nodeAB_->getTag(), nodeABClone->getTag());
  ABI39_0_0EXPECT_EQ(nodeAB_->getSurfaceId(), nodeABClone->getSurfaceId());
  ABI39_0_0EXPECT_EQ(nodeAB_->getProps(), nodeABClone->getProps());
}
...
TEST_F(ShadowNodeTest, handleState) {
  ...
  ABI39_0_0EXPECT_EQ(firstNode->getState(), secondNode->getState());
  ABI39_0_0EXPECT_NE(firstNode->getState(), thirdNode->getState());
  secondNode->setStateData(TestState{42});
  ABI39_0_0EXPECT_NE(firstNode->getState(), secondNode->getState());

  // State cannot be changed for sealed shadow node.
  secondNode->sealRecursive();
  ABI39_0_0EXPECT_ANY_THROW(secondNode->setStateData(TestState{42}));
}
