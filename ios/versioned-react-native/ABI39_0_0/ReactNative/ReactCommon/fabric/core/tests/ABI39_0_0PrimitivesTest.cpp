...
TEST(SealableTest, sealObjectCorrectly) {
  Sealable obj;
  ABI39_0_0EXPECT_FALSE(obj.getSealed());

  obj.seal();
  ABI39_0_0EXPECT_TRUE(obj.getSealed());
}

TEST(SealableTest, handleAssignmentsCorrectly) {
  Sealable obj;
  Sealable other;
  ABI39_0_0EXPECT_NO_THROW(obj = other);

  // Assignment after getting sealed is not allowed.
  obj.seal();
  Sealable other2;
  ABI39_0_0EXPECT_THROW(obj = other2, std::runtime_error);
  ...
  Sealable other3(obj);
  ABI39_0_0EXPECT_FALSE(other3.getSealed());
}
