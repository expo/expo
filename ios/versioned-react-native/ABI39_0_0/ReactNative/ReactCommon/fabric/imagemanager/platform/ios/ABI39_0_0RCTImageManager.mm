...
#import <ABI39_0_0React/utils/SharedFunction.h>
...
#import "ABI39_0_0RCTImagePrimitivesConversions.h"

using namespace ABI39_0_0facebook::ABI39_0_0React;

@implementation ABI39_0_0RCTImageManager {
  id<ABI39_0_0RCTImageLoaderWithAttributionProtocol> _imageLoader;
...
- (instancetype)initWithImageLoader:(id<ABI39_0_0RCTImageLoaderWithAttributionProtocol>)imageLoader
{
  if (self = [super init]) {
    ...
    _backgroundSerialQueue =
        dispatch_queue_create("com.facebook.ABI39_0_0React-native.image-manager-queue", DISPATCH_QUEUE_SERIAL);
  }
...
{
  SystraceSection s("ABI39_0_0RCTImageManager::requestImage");

  auto imageInstrumentation = std::make_shared<ABI39_0_0RCTImageInstrumentationProxy>(_imageLoader);
   ...
   * work (such as creating an `NSURLRequest` object and some obscure logic inside `ABI39_0_0RCTImageLoader`) can take a couple
   ...
   *
   * Technically, we don't need to dispatch this to *serial* queue. The interface of `ABI39_0_0RCTImageLoader` promises to be
  ...
  dispatch_async(_backgroundSerialQueue, ^{
    ...
    ABI39_0_0RCTImageURLLoaderRequest *loaderRequest =
    ...
    ABI39_0_0RCTImageLoaderCancellationBlock cancelationBlock = loaderRequest.cancellationBlock;
