...
#import <ABI39_0_0ReactCommon/ABI39_0_0LongLivedObject.h>
...
static id convertJSIValueToObjCObject(
    ...
    std::shared_ptr<ABI39_0_0React::CallInvoker> jsInvoker);
...
static NSArray *
convertJSIArrayToNSArray(jsi::Runtime &runtime, const jsi::Array &value, std::shared_ptr<ABI39_0_0React::CallInvoker> jsInvoker)
{
...
static ABI39_0_0RCTResponseSenderBlock convertJSIFunctionToCallback(
    jsi::Runtime &runtime,
...
static ABI39_0_0RCTResponseSenderBlock convertJSIFunctionToCallback(
    ...
    std::shared_ptr<ABI39_0_0React::CallInvoker> jsInvoker)
{
  auto weakWrapper = ABI39_0_0React::CallbackWrapper::createWeak(value.getFunction(runtime), runtime, jsInvoker);
  ...
  jsi::Function fn = jsi::Function::createFromHostFunction(
      ...
      [invokeCopy, jsInvoker](jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args, size_t count) {
        ...
        auto weakResolveWrapper =
            ABI39_0_0React::CallbackWrapper::createWeak(args[0].getObject(rt).getFunction(rt), rt, jsInvoker);
        auto weakRejectWrapper =
            ABI39_0_0React::CallbackWrapper::createWeak(args[1].getObject(rt).getFunction(rt), rt, jsInvoker);

        __block BOOL resolveWasCalled = NO;
        __block BOOL rejectWasCalled = NO;

        ABI39_0_0RCTPromiseResolveBlock resolveBlock = ^(id result) {
          if (rejectWasCalled) {
        ...
        ABI39_0_0RCTPromiseRejectBlock rejectBlock = ^(NSString *code, NSString *message, NSError *error) {
          if (resolveWasCalled) {
          ...
          NSDictionary *jsError = ABI39_0_0RCTJSErrorFromCodeMessageAndNSError(code, message, error);
 ...
 * This serves as a backward-compatible support for ABI39_0_0RCTBridgeModule's methodQueue API.
 *
 * In the future:
    ...
    MethodCallId methodCallId)
{
  ...
  __weak id<ABI39_0_0RCTTurboModule> weakModule = instance_;
  id<ABI39_0_0RCTTurboModulePerformanceLogger> performanceLogger = performanceLogger_;
  ...
  void (^block)() = ^{
    ...
    id<ABI39_0_0RCTTurboModule> strongModule = weakModule;

    if (isSync) {
      [performanceLogger syncABI39_0_0RCTTurboModuleMethodCallStart:moduleName methodName:methodName methodCallId:methodCallId];
    } else {
      [performanceLogger asyncABI39_0_0RCTTurboModuleMethodCallStart:moduleName methodName:methodName methodCallId:methodCallId];
    }
    ...
    if (returnType == VoidKind) {
      [performanceLogger asyncABI39_0_0RCTTurboModuleMethodCallEnd:moduleName methodName:methodName methodCallId:methodCallId];
      return;
    }
    void *rawResult;
    [inv getReturnValue:&rawResult];
    result = (__bridge id)rawResult;
    [performanceLogger syncABI39_0_0RCTTurboModuleMethodCallEnd:moduleName methodName:methodName methodCallId:methodCallId];
  };
  ...
  if (!methodArgumentTypeNames_) {
    ...
    if (methods) {
      for (unsigned int i = 0; i < numberOfMethods; i++) {
        ...
        ABI39_0_0RCTMethodInfo *(*getMethodInfo)(id, SEL) = (__typeof__(getMethodInfo))objc_msgSend;
        ABI39_0_0RCTMethodInfo *methodInfo = getMethodInfo(cls, s);

        NSArray<ABI39_0_0RCTMethodArgument *> *arguments;
        NSString *otherMethodName = ABI39_0_0RCTParseMethodSignature(methodInfo->objcName, &arguments);
    ...
    MethodCallId methodCallId)
{
  const bool isSync = returnType != VoidKind && returnType != PromiseKind;
  const char *moduleName = name_.c_str();
  const id<ABI39_0_0RCTTurboModule> module = instance_;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (objCArg) {
      ...
      /**
       * Convert objects using ABI39_0_0RCTConvert.
       */
      if (objCArgType == @encode(id)) {
        ...
        if (argumentType != nil) {
          ...
          if ([ABI39_0_0RCTConvert respondsToSelector:rctConvertSelector]) {
            // Message dispatch logic from old infra
            id (*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
            id convertedObjCArg = convert([ABI39_0_0RCTConvert class], rctConvertSelector, objCArg);
      ...
      /**
       * Convert objects using ABI39_0_0RCTCxxConvert to structs.
       */
      if ([objCArg isKindOfClass:[NSDictionary class]] && hasMethodArgConversionSelector(methodNameNSString, i)) {
        ...
        ABI39_0_0RCTManagedPointer *box = convert([ABI39_0_0RCTCxxConvert class], methodArgConversionSelector, objCArg);
    ...
    size_t count)
{
  ...
  jsi::Value returnValue = returnType == PromiseKind
      ? createPromise(
            runtime,
            jsInvoker_,
            ^(ABI39_0_0RCTPromiseResolveBlock resolveBlock, ABI39_0_0RCTPromiseRejectBlock rejectBlock) {
