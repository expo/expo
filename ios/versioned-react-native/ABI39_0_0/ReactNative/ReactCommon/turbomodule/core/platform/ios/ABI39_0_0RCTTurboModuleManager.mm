...
#import "ABI39_0_0RCTTurboModuleManager.h"
...
#import <ABI39_0_0ReactCommon/ABI39_0_0TurboCxxModule.h>
#import <ABI39_0_0ReactCommon/ABI39_0_0TurboModuleBinding.h>

using namespace ABI39_0_0facebook;

/**
 * A global variable whose address we use to associate method queues to id<ABI39_0_0RCTTurboModule> objects.
 */
static char kAssociatedMethodQueueKey;

namespace {
class MethodQueueNativeCallInvoker : public ABI39_0_0facebook::ABI39_0_0React::CallInvoker {
 private:
 ...
 public:
  ...
  {
    if (methodQueue_ == ABI39_0_0RCTJSThread) {
      work();
      return;
    }
...
// Fallback lookup since ABI39_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI39_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI39_0_0RCTTurboModuleManager {
  jsi::Runtime *_runtime;
  std::shared_ptr<ABI39_0_0facebook::ABI39_0_0React::CallInvoker> _jsInvoker;
  id<ABI39_0_0RCTTurboModulePerformanceLogger> _performanceLogger;
  __weak id<ABI39_0_0RCTTurboModuleManagerDelegate> _delegate;
   ...
   */
  std::unordered_map<std::string, id<ABI39_0_0RCTTurboModule>> _rctTurboModuleCache;
  std::unordered_map<std::string, std::shared_ptr<ABI39_0_0React::TurboModule>> _turboModuleCache;

  /**
   * _rctTurboModuleCache can be accessed by multiple threads at once via
   * the provideABI39_0_0RCTTurboModule method. This can lead to races. Therefore, we
             ...
             performanceLogger:(id<ABI39_0_0RCTTurboModulePerformanceLogger>)performanceLogger
{
  if (self = [super init]) {
    ...
    [bridge setABI39_0_0RCTTurboModuleLookupDelegate:self];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeWillInvalidateModules:)
                                                 name:ABI39_0_0RCTBridgeWillInvalidateModulesNotification
                                               object:_bridge.parentBridge];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidInvalidateModules:)
                                                 name:ABI39_0_0RCTBridgeDidInvalidateModulesNotification
                                               object:_bridge.parentBridge];
  }
  return self;
}
...
- (void)notifyAboutTurboModuleSetup:(const char *)name
{
  ...
  if (moduleName) {
    int64_t setupTime = [self->_bridge.performanceLogger durationForTag:ABI39_0_0RCTPLTurboModuleSetup];
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI39_0_0RCTDidSetupModuleNotification
                                                        object:nil
                                                      userInfo:@{
                                                        ABI39_0_0RCTDidSetupModuleNotificationModuleNameKey : moduleName,
                                                        ABI39_0_0RCTDidSetupModuleNotificationSetupTimeKey : @(setupTime)
                                                      }];
  }
}
...
- (std::shared_ptr<ABI39_0_0React::TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  [_performanceLogger createABI39_0_0RCTTurboModuleStart:moduleName];
  id<ABI39_0_0RCTTurboModule> module = [self provideABI39_0_0RCTTurboModule:moduleName];
  [_performanceLogger createABI39_0_0RCTTurboModuleEnd:moduleName];

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI39_0_0RCTTurboModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  /**
   * Have ABI39_0_0RCTCxxBridge decorate native CallInvoker, so that it's aware of TurboModule async method calls.
  ...
  // If ABI39_0_0RCTTurboModule supports creating its own C++ TurboModule object,
  // allow it to do so.
  if ([module respondsToSelector:@selector(getTurboModuleWithJsInvoker:nativeInvoker:perfLogger:)]) {
    [_performanceLogger getTurboModuleFromABI39_0_0RCTTurboModuleStart:moduleName];
    auto turboModule = [module getTurboModuleWithJsInvoker:_jsInvoker
                                             nativeInvoker:nativeInvoker
                                                perfLogger:_performanceLogger];
    [_performanceLogger getTurboModuleFromABI39_0_0RCTTurboModuleEnd:moduleName];
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI39_0_0RCTCxxModule.class]) {
    ...
    [_performanceLogger getTurboModuleFromABI39_0_0RCTCxxModuleStart:moduleName];
    auto turboModule = std::make_shared<ABI39_0_0React::TurboCxxModule>([((ABI39_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    [_performanceLogger getTurboModuleFromABI39_0_0RCTCxxModuleEnd:moduleName];
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
 ...
 * (for now).
 */
- (id<ABI39_0_0RCTTurboModule>)provideABI39_0_0RCTTurboModule:(const char *)moduleName
{
  Class moduleClass;
  id<ABI39_0_0RCTTurboModule> module = nil;

  {
    ...
    if (rctTurboModuleCacheLookup != _rctTurboModuleCache.end()) {
      [_performanceLogger createABI39_0_0RCTTurboModuleCacheHit:moduleName];
     ...
     */
    [_performanceLogger getABI39_0_0RCTTurboModuleClassStart:moduleName];
    ...
    [_performanceLogger getABI39_0_0RCTTurboModuleClassEnd:moduleName];

    if (![moduleClass conformsToProtocol:@protocol(ABI39_0_0RCTTurboModule)]) {
      return nil;
    }
     ...
     */
    [_performanceLogger getABI39_0_0RCTTurboModuleInstanceStart:moduleName];
    ...
    [_performanceLogger getABI39_0_0RCTTurboModuleInstanceEnd:moduleName];
  ...
  [self setUpABI39_0_0RCTTurboModule:module moduleName:moduleName];
  return module;
}

- (void)setUpABI39_0_0RCTTurboModule:(id<ABI39_0_0RCTTurboModule>)module moduleName:(const char *)moduleName
{
  __weak id<ABI39_0_0RCTBridgeModule> weakModule = (id<ABI39_0_0RCTBridgeModule>)module;
  __weak ABI39_0_0RCTBridge *weakBridge = _bridge;
  id<ABI39_0_0RCTTurboModulePerformanceLogger> performanceLogger = _performanceLogger;
  ...
  auto setUpTurboModule = ^{
    ...
    [performanceLogger setupABI39_0_0RCTTurboModuleStart:moduleName];

    id<ABI39_0_0RCTBridgeModule> strongModule = weakModule;
    ABI39_0_0RCTBridge *strongBridge = weakBridge;
     ...
     * implementation, and a `- (ABI39_0_0RCTBridge *) bridge { ... }` method won't be
    ...
    if ([strongModule respondsToSelector:@selector(bridge)] && strongBridge) {
      [performanceLogger attachABI39_0_0RCTBridgeToABI39_0_0RCTTurboModuleStart:moduleName];
      ...
      @try {
        /**
         * ABI39_0_0RCTBridgeModule declares the bridge property as readonly.
         * Therefore, when authors of NativeModules synthesize the bridge
         * via @synthesize bridge = bridge;, the ObjC runtime generates
         * only a - (ABI39_0_0RCTBridge *) bridge: { ... } method. No setter is
        ...
        ABI39_0_0RCTLogError(
            ...
            ABI39_0_0RCTBridgeModuleNameForClass([strongModule class]));
      }

      [performanceLogger attachABI39_0_0RCTBridgeToABI39_0_0RCTTurboModuleEnd:moduleName];
    }
    ...
    [performanceLogger attachMethodQueueToABI39_0_0RCTTurboModuleStart:moduleName];
    ...
    /**
     * Note: ABI39_0_0RCTJSThread, which is a valid method queue, is defined as (id)kCFNull. It should rightfully not enter the
     * following if condition's block.
     */
    if (!methodQueue) {
      NSString *methodQueueName = [NSString stringWithFormat:@"com.facebook.ABI39_0_0React.%sQueue", moduleName];
    ...
    /**
     * Attach method queue to id<ABI39_0_0RCTTurboModule> object.
     * This is necessary because the id<ABI39_0_0RCTTurboModule> object can be eagerly created/initialized before the method
     * queue is required. The method queue is required for an id<ABI39_0_0RCTTurboModule> for JS -> Native calls. So, we need it
     * before we create the id<ABI39_0_0RCTTurboModule>'s TurboModule jsi::HostObject in provideTurboModule:.
     */
    objc_setAssociatedObject(strongModule, &kAssociatedMethodQueueKey, methodQueue, OBJC_ASSOCIATION_RETAIN);

    [performanceLogger attachMethodQueueToABI39_0_0RCTTurboModuleEnd:moduleName];

    /**
     * NativeModules that implement the ABI39_0_0RCTFrameUpdateObserver protocol
     * require registration with ABI39_0_0RCTDisplayLink.
     *
     * TODO(T55504345): Investigate whether we can improve this after TM
     * rollout.
     */
    if (strongBridge) {
      [performanceLogger registerABI39_0_0RCTTurboModuleForFrameUpdatesStart:moduleName];
      ABI39_0_0RCTModuleData *data = [[ABI39_0_0RCTModuleData alloc] initWithModuleInstance:strongModule bridge:strongBridge];
      [strongBridge registerModuleForFrameUpdates:strongModule withModuleData:data];
      [performanceLogger registerABI39_0_0RCTTurboModuleForFrameUpdatesEnd:moduleName];
    }
     ...
     * rollout.
     */
    [performanceLogger dispatchDidInitializeModuleNotificationForABI39_0_0RCTTurboModuleStart:moduleName];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:ABI39_0_0RCTDidInitializeModuleNotification
                      object:strongBridge
                    userInfo:@{@"module" : module, @"bridge" : ABI39_0_0RCTNullIfNil([strongBridge parentBridge])}];
    [performanceLogger dispatchDidInitializeModuleNotificationForABI39_0_0RCTTurboModuleEnd:moduleName];

    [performanceLogger setupABI39_0_0RCTTurboModuleEnd:moduleName];
  };
   ...
   *    concurrently run setUpABI39_0_0RCTTurboModule:
   */
  if ([[module class] respondsToSelector:@selector(requiresMainQueueSetup)] &&
     ...
     */
    [_performanceLogger setupABI39_0_0RCTTurboModuleDispatch:moduleName];
    ABI39_0_0RCTUnsafeExecuteOnMainQueueSync(setUpTurboModule);
  } else {
    setUpTurboModule();
  }
}
...
- (void)installJSBindingWithRuntime:(jsi::Runtime *)runtime
{
  ...
  ABI39_0_0React::TurboModuleBinding::install(
      *_runtime,
      [weakSelf,
       performanceLogger = _performanceLogger](const std::string &name) -> std::shared_ptr<ABI39_0_0React::TurboModule> {
        if (!weakSelf) {
          return nullptr;
        }
        ...
        if (moduleWasNotInitialized) {
          [strongSelf->_bridge.performanceLogger markStartForTag:ABI39_0_0RCTPLTurboModuleSetup];
        }
        ...
        if (moduleWasNotInitialized && [strongSelf moduleIsInitialized:moduleName]) {
          [strongSelf->_bridge.performanceLogger markStopForTag:ABI39_0_0RCTPLTurboModuleSetup];
...
#pragma mark ABI39_0_0RCTTurboModuleLookupDelegate
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  if (warnOnLookupFailure && !module) {
    ABI39_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
- (void)bridgeDidInvalidateModules:(NSNotification *)notification
{
  ...
  std::unordered_map<std::string, id<ABI39_0_0RCTTurboModule>> rctCacheCopy;
  {
    std::unique_lock<std::mutex> lock(_rctTurboModuleCacheLock);
    rctCacheCopy.insert(_rctTurboModuleCache.begin(), _rctTurboModuleCache.end());
  }

  // Backward-compatibility: ABI39_0_0RCTInvalidating handling.
  dispatch_group_t moduleInvalidationGroup = dispatch_group_create();
  for (const auto &p : rctCacheCopy) {
    id<ABI39_0_0RCTTurboModule> module = p.second;
    if ([module respondsToSelector:@selector(invalidate)]) {
      if ([module respondsToSelector:@selector(methodQueue)]) {
        ...
        if (methodQueue) {
          ...
          [bridge
              dispatchBlock:^{
                [((id<ABI39_0_0RCTInvalidating>)module) invalidate];
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI39_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
...
- (void)invalidate
{
  ...
  // Backward-compatibility: ABI39_0_0RCTInvalidating handling, but not adhering to desired methodQueue.
  for (const auto &p : rctCacheCopy) {
    id<ABI39_0_0RCTTurboModule> module = p.second;
    if ([module respondsToSelector:@selector(invalidate)]) {
      [((id<ABI39_0_0RCTInvalidating>)module) invalidate];
