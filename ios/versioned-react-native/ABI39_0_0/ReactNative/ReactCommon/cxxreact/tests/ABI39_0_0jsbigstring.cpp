...
#include <ABI39_0_0cxxreact/ABI39_0_0JSBigString.h>
...
TEST(JSBigFileString, MapPartTest) {
  ...
  // Test
  ABI39_0_0EXPECT_EQ(needle.length(), bigStr.size());
  for (unsigned int i = 0; i < needle.length(); ++i) {
    ABI39_0_0EXPECT_EQ(needle[i], bigStr.c_str()[i]);
  }
}
...
TEST(JSBigFileString, RemapTest) {
  ...
  ABI39_0_0EXPECT_EQ(pageSize * 2, bigStr.size());
  auto remapped = bigStr.c_str();
  size_t i = 0;
  for (; i < pageSize; ++i) {
    ABI39_0_0EXPECT_EQ(0x22, remapped[i]);
  }
  for (; i < pageSize * 2; ++i) {
    ABI39_0_0EXPECT_EQ(0x11, remapped[i]);
  }
}
