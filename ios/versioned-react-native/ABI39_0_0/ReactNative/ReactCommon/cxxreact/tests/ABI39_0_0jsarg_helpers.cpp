...
#include <ABI39_0_0cxxreact/ABI39_0_0JsArgumentHelpers.h>
...
using namespace ABI39_0_0facebook::xplat;

#define ABI39_0_0EXPECT_JSAE(statement, exstr)                                         \
  do {                                                                        \
    try {                                                                     \
      statement;                                                              \
      FAIL() << "Expected JsArgumentException(" << (exstr) << ") not thrown"; \
    } catch (const JsArgumentException &ex) {                                 \
      ABI39_0_0EXPECT_EQ(ex.what(), std::string(exstr));                               \
    }                                                                         \
...
TEST(JsArgumentHelpersTest, args) {
  ...
  ABI39_0_0EXPECT_EQ(jsArgAsBool(args, 0), aBool);
  ABI39_0_0EXPECT_EQ(jsArgAsInt(args, 1), anInt);
  ABI39_0_0EXPECT_EQ(jsArgAsDouble(args, 2), aDouble);
  ABI39_0_0EXPECT_EQ(jsArgAsString(args, 3), aString);
  ABI39_0_0EXPECT_EQ(jsArgAsArray(args, 4), anArray);
  ABI39_0_0EXPECT_EQ(jsArgAsObject(args, 5), anObject);

  // const args
  const folly::dynamic &cargs = args;
  const folly::dynamic &a4 = jsArgAsArray(cargs, 4);
  ABI39_0_0EXPECT_EQ(a4, anArray);
  ABI39_0_0EXPECT_EQ(jsArgAsObject(cargs, 5), anObject);

  // helpers returning dynamic should return same object without copying
  ABI39_0_0EXPECT_EQ(&jsArgAsArray(args, 4), &(args[4]));
  ABI39_0_0EXPECT_EQ(&jsArgAsArray(cargs, 4), &(args[4]));
  ...
  ABI39_0_0EXPECT_EQ(jsArgN(args, 3, &folly::dynamic::getString), aString);
  ABI39_0_0EXPECT_EQ(jsArg(args[3], &folly::dynamic::getString), aString);

  // conversions
  ABI39_0_0EXPECT_EQ(jsArgAsDouble(args, 1), anInt * 1.0);
  ABI39_0_0EXPECT_EQ(jsArgAsString(args, 1), aNumericString);
  ABI39_0_0EXPECT_EQ(jsArgAsInt(args, 6), anInt);

  // Test exception messages.

  // out_of_range
  ABI39_0_0EXPECT_JSAE(
      jsArgAsBool(args, 7),
  ...
  try {
    ...
    ABI39_0_0EXPECT_GT(exwhat.size(), exhead.size());
    ABI39_0_0EXPECT_GT(exwhat.size(), extail.size());

    ABI39_0_0EXPECT_TRUE(std::equal(exhead.cbegin(), exhead.cend(), exwhat.cbegin()))
        << "JsArgumentException('" << exwhat << "') does not begin with '"
        << exhead << "'";
    ABI39_0_0EXPECT_TRUE(std::equal(extail.crbegin(), extail.crend(), exwhat.crbegin()))
