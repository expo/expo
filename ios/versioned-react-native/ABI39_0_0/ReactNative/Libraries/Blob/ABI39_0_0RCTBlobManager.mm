...
#import "ABI39_0_0RCTBlobCollector.h"

static NSString *const kBlobURIScheme = @"blob";

@interface ABI39_0_0RCTBlobManager () <ABI39_0_0RCTNetworkingRequestHandler, ABI39_0_0RCTNetworkingResponseHandler, ABI39_0_0RCTWebSocketContentHandler, ABI39_0_0NativeBlobModuleSpec>

@end

@implementation ABI39_0_0RCTBlobManager
{
...
ABI39_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)setBridge:(ABI39_0_0RCTBridge *)bridge
{
  ...
  ABI39_0_0facebook::ABI39_0_0React::ABI39_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI39_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI39_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI39_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI39_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
  ABI39_0_0RCTNetworking *const networking = _bridge ? _bridge.networking : [_turboModuleLookupDelegate moduleForName:"ABI39_0_0RCTNetworking"];
...
ABI39_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(double)socketID)
{
...
ABI39_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(double)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI39_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(double)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI39_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI39_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI39_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI39_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI39_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI39_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI39_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI39_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI39_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI39_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI39_0_0RCTWebSocketContentHandler methods
...
{
  return std::make_shared<ABI39_0_0facebook::ABI39_0_0React::NativeBlobModuleSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

Class ABI39_0_0RCTBlobManagerCls(void) {
  return ABI39_0_0RCTBlobManager.class;
}
