...
@interface ABI39_0_0RCTHTTPRequestHandler () <NSURLSessionDataDelegate, ABI39_0_0RCTTurboModule>

@end

@implementation ABI39_0_0RCTHTTPRequestHandler
{
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  dispatch_once(&onceToken, ^{
    // technically, ABI39_0_0RCTHTTPRequestHandler can handle file:// as well,
    // but it's less efficient than using ABI39_0_0RCTFileRequestHandler
                         ...
                         withDelegate:(id<ABI39_0_0RCTURLRequestDelegate>)delegate
{
  ...
  if (!_session && [self isValid]) {
    ...
    //  by providing the following key to your ABI39_0_0RN project (edit ios/project/Info.plist file in Xcode):
    // <key>ABI39_0_0ReactNetworkForceWifiOnly</key>    <true/>
    ...
    NSNumber *useWifiOnly = [infoDictionary objectForKey:@"ABI39_0_0ReactNetworkForceWifiOnly"];

    NSOperationQueue *callbackQueue = [NSOperationQueue new];
    callbackQueue.maxConcurrentOperationCount = 1;
    callbackQueue.underlyingQueue = [[_bridge networking] methodQueue];
    NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
    // Set allowsCellularAccess to NO ONLY if key ABI39_0_0ReactNetworkForceWifiOnly exists AND its value is YES
    if (useWifiOnly) {
...
{
  id<ABI39_0_0RCTURLRequestDelegate> delegate;
  {
    std::lock_guard<std::mutex> lock(_mutex);
...
Class ABI39_0_0RCTHTTPRequestHandlerCls(void) {
  return ABI39_0_0RCTHTTPRequestHandler.class;
}
