...
@interface ABI39_0_0RCTAssetsLibraryRequestHandler() <ABI39_0_0RCTTurboModule>
@end

@implementation ABI39_0_0RCTAssetsLibraryRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  return [request.URL.scheme caseInsensitiveCompare:@"assets-library"] == NSOrderedSame
    ...
    || [request.URL.scheme caseInsensitiveCompare:ABI39_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
}
     ...
     withDelegate:(id<ABI39_0_0RCTURLRequestDelegate>)delegate
{
  ...
  BOOL isPHUpload = [requestURL.scheme caseInsensitiveCompare:ABI39_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
  if (isPHUpload) {
    requestURL = [NSURL URLWithString:[@"ph" stringByAppendingString:[requestURL.absoluteString substringFromIndex:ABI39_0_0RCTNetworkingPHUploadHackScheme.length]]];
  }

  if (!requestURL) {
    NSString *const msg = [NSString stringWithFormat:@"Cannot send request without URL"];
    [delegate URLRequest:cancellationBlock didCompleteWithError:ABI39_0_0RCTErrorWithMessage(msg)];
  ...
  if (![fetchResult firstObject]) {
    ...
    NSError *error = ABI39_0_0RCTErrorWithMessage(errorMessage);
  ...
  if (asset.mediaType == PHAssetMediaTypeVideo && isPHUpload) {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:requestOptions resultHandler:^(AVAsset * _Nullable avAsset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
      ...
      if (![avAsset isKindOfClass:[AVURLAsset class]]) {
        error = [NSError errorWithDomain:ABI39_0_0RCTErrorDomain code:0 userInfo:
        @{
...
Class ABI39_0_0RCTAssetsLibraryRequestHandlerCls(void) {
  return ABI39_0_0RCTAssetsLibraryRequestHandler.class;
}
