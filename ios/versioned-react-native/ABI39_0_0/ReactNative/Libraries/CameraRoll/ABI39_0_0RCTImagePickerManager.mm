...
#import "ABI39_0_0RCTImagePickerManager.h"
...
@interface ABI39_0_0RCTImagePickerController : UIImagePickerController

@property (nonatomic, assign) BOOL unmirrorFrontFacingCamera;

@end

@implementation ABI39_0_0RCTImagePickerController

@end

@interface ABI39_0_0RCTImagePickerManager () <UIImagePickerControllerDelegate, UINavigationControllerDelegate, ABI39_0_0NativeImagePickerIOSSpec>
@end

@implementation ABI39_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI39_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI39_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
  NSMutableDictionary<NSString *, NSDictionary<NSString *, id> *> *_pendingVideoInfo;
}

ABI39_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI39_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI39_0_0RCTResponseSenderBlock)callback)
{
...
ABI39_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI39_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI39_0_0RCT_EXPORT_METHOD(openCameraDialog:(JS::NativeImagePickerIOS::SpecOpenCameraDialogConfig &)config
                  successCallback:(ABI39_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI39_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  ABI39_0_0RCTImagePickerController *imagePicker = [ABI39_0_0RCTImagePickerController new];
...
ABI39_0_0RCT_EXPORT_METHOD(openSelectDialog:(JS::NativeImagePickerIOS::SpecOpenSelectDialogConfig &)config
                  successCallback:(ABI39_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI39_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI39_0_0RCTRunningInAppExtension()) {
...
ABI39_0_0RCT_EXPORT_METHOD(clearAllPendingVideos)
{
...
ABI39_0_0RCT_EXPORT_METHOD(removePendingVideo:(NSString *)url)
{
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    ...
    [self _dismissPicker:picker args:@[imageURLString, ABI39_0_0RCTNullIfNil(height), ABI39_0_0RCTNullIfNil(width)]];
    return;
  }
  ...
  [_bridge.imageStoreManager storeImage:originalImage withBlock:^(NSString *tempImageTag) {
    [self _dismissPicker:picker args:tempImageTag ? @[tempImageTag, ABI39_0_0RCTNullIfNil(height), ABI39_0_0RCTNullIfNil(width)] : nil];
  }];
}
        ...
        cancelCallback:(ABI39_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI39_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI39_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI39_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
...
- (void)cameraChanged:(NSNotification *)notification
{
  for (UIImagePickerController *picker in _pickers) {
    ...
    if ([picker isKindOfClass:[ABI39_0_0RCTImagePickerController class]]
      && ((ABI39_0_0RCTImagePickerController *)picker).unmirrorFrontFacingCamera
...
{
  return std::make_shared<ABI39_0_0facebook::ABI39_0_0React::NativeImagePickerIOSSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

Class ABI39_0_0RCTImagePickerManagerCls(void) {
  return ABI39_0_0RCTImagePickerManager.class;
}
