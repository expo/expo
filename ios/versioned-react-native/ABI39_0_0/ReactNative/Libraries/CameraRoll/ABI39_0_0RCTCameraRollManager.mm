...
#import "ABI39_0_0RCTCameraRollManager.h"
...
#import "ABI39_0_0RCTAssetsLibraryRequestHandler.h"

@implementation ABI39_0_0RCTConvert (PHAssetCollectionSubtype)

ABI39_0_0RCT_ENUM_CONVERTER(PHAssetCollectionSubtype, (@{
...
@implementation ABI39_0_0RCTConvert (PHFetchOptions)
...
+ (PHFetchOptions *)PHFetchOptionsFromMediaType:(NSString *)mediaType
{
  ...
  if ([lowercase isEqualToString:@"photos"]) {
    ...
    if (![lowercase isEqualToString:@"all"]) {
      ABI39_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
...
@interface ABI39_0_0RCTCameraRollManager() <ABI39_0_0NativeCameraRollManagerSpec>
@end

@implementation ABI39_0_0RCTCameraRollManager
...
static void requestPhotoLibraryAccess(ABI39_0_0RCTPromiseRejectBlock reject, PhotosAuthorizedBlock authorizedBlock) {
...
ABI39_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
                  ...
                  reject:(ABI39_0_0RCTPromiseRejectBlock)reject)
{
  ...
  void (^loadBlock)(void) = ^void() {
    if ([type isEqualToString:@"video"]) {
      ...
      [[self.bridge moduleForClass:[ABI39_0_0RCTImageLoader class]] loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
        if (error) {
...
static void ABI39_0_0RCTResolvePromise(ABI39_0_0RCTPromiseResolveBlock resolve,
...
ABI39_0_0RCT_EXPORT_METHOD(getPhotos:(JS::NativeCameraRollManager::GetPhotosParams &)params
                  resolve:(ABI39_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI39_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger const first = [ABI39_0_0RCTConvert NSInteger:[NSNumber numberWithDouble:params.first()]];
  NSString *const afterCursor = [ABI39_0_0RCTConvert NSString:params.after()];
  NSString *const groupName = [ABI39_0_0RCTConvert NSString:params.groupName()];
  NSString *const groupTypes = [[ABI39_0_0RCTConvert NSString:params.groupTypes()] lowercaseString];
  NSString *const mediaType = [ABI39_0_0RCTConvert NSString:params.assetType()];
  NSArray<NSString *> *const mimeTypes = [ABI39_0_0RCTConvert NSStringArray:ABI39_0_0RCTConvertOptionalVecToArray(params.mimeTypes())];
  ...
  PHAssetCollectionSubtype const collectionSubtype = [ABI39_0_0RCTConvert PHAssetCollectionSubtype:groupTypes];

  // Predicate for fetching assets within a collection
  PHFetchOptions *const assetFetchOptions = [ABI39_0_0RCTConvert PHFetchOptionsFromMediaType:mediaType];
    ...
    [assetCollectionFetchResult enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull assetCollection, NSUInteger collectionIdx, BOOL * _Nonnull stopCollections) {
      ...
      [assetsFetchResult enumerateObjectsUsingBlock:^(PHAsset * _Nonnull asset, NSUInteger assetIdx, BOOL * _Nonnull stopAssets) {
        ...
        if (first == assets.count) {
          ...
          *stopCollections = YES;
          hasNextPage = YES;
          ABI39_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
          ABI39_0_0RCTResolvePromise(resolve, assets, hasNextPage);
          resolvedPromise = YES;
          return;
        }
...
ABI39_0_0RCT_EXPORT_METHOD(deletePhotos:(NSArray<NSString *>*)assets
                  resolve:(ABI39_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI39_0_0RCTPromiseRejectBlock)reject)
{
  NSArray<NSURL *> *assets_ = [ABI39_0_0RCTConvert NSURLArray:assets];
  [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI39_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
...
{
  return std::make_shared<ABI39_0_0facebook::ABI39_0_0React::NativeCameraRollManagerSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

Class ABI39_0_0RCTCameraRollManagerCls(void) {
  return ABI39_0_0RCTCameraRollManager.class;
}
