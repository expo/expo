...
typedef void (^ABI39_0_0RCTPropBlock)(id<ABI39_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI39_0_0RCTPropBlock> ABI39_0_0RCTPropBlockDictionary;
...
@implementation ABI39_0_0RCTComponentData {
  id<ABI39_0_0RCTComponent> _defaultView; // Only needed for ABI39_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI39_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI39_0_0RCTPropBlockDictionary *_shadowPropBlocks;
  __weak ABI39_0_0RCTBridge *_bridge;
}

@synthesize manager = _manager;

- (instancetype)initWithManagerClass:(Class)managerClass bridge:(ABI39_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _bridge = bridge;
...
- (ABI39_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI39_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI39_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI39_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI39_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI39_0_0RCTPropBlock
createEventSetter(NSString *propName, SEL setter, InterceptorBlock eventInterceptor, ABI39_0_0RCTBridge *bridge)
{
  __weak ABI39_0_0RCTBridge *weakBridge = bridge;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI39_0_0RCTConvert BOOL:json]) {
      __weak id<ABI39_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI39_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI39_0_0ReactTag);
        } else {
          ABI39_0_0RCTComponentEvent *componentEvent = [[ABI39_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI39_0_0ReactTag
                                                                                 body:event];
...
static ABI39_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI39_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI39_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI39_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI39_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI39_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock = createEventSetter(name, setter, self.eventInterceptor, _bridge);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI39_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI39_0_0RCTLogError(@"No +[ABI39_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI39_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI39_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI39_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI39_0_0RCT_CASE(_C_SEL, SEL)
        ABI39_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI39_0_0RCT_CASE(_C_CHR, char)
        ABI39_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI39_0_0RCT_CASE(_C_SHT, short)
        ABI39_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI39_0_0RCT_CASE(_C_INT, int)
        ABI39_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI39_0_0RCT_CASE(_C_LNG, long)
        ABI39_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI39_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI39_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI39_0_0RCT_CASE(_C_FLT, float)
        ABI39_0_0RCT_CASE(_C_DBL, double)
        ABI39_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI39_0_0RCT_CASE(_C_PTR, void *)
        ABI39_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI39_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI39_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI39_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI39_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI39_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI39_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI39_0_0ReactTag];
      ABI39_0_0RCTPerformBlockWithLogPrefix(
          ^{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI39_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI39_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI39_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
    ...
    if ([type isEqualToString:@"ABI39_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI39_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI39_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI39_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
...
{
  // Hackety hack, this partially re-implements ABI39_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI39_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI39_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI39_0_0EX_REMOVE_VERSION(name);
  ...
  ABI39_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
