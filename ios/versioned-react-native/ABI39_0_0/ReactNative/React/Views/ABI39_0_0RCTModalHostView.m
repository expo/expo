...
@implementation ABI39_0_0RCTModalHostView {
  __weak ABI39_0_0RCTBridge *_bridge;
  BOOL _isPresented;
  ABI39_0_0RCTModalHostViewController *_modalViewController;
  ABI39_0_0RCTTouchHandler *_touchHandler;
  UIView *_ABI39_0_0ReactSubview;
#if TARGET_OS_TV
...
ABI39_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : coder)

- (instancetype)initWithBridge:(ABI39_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _modalViewController = [ABI39_0_0RCTModalHostViewController new];
    UIView *containerView = [UIView new];
    containerView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _modalViewController.view = containerView;
    _touchHandler = [[ABI39_0_0RCTTouchHandler alloc] initWithBridge:bridge];
#if TARGET_OS_TV
...
- (void)setOnRequestClose:(ABI39_0_0RCTDirectEventBlock)onRequestClose
{
  _onRequestClose = onRequestClose;
  if (_ABI39_0_0ReactSubview) {
    if (_onRequestClose && _menuButtonGestureRecognizer) {
      [_ABI39_0_0ReactSubview addGestureRecognizer:_menuButtonGestureRecognizer];
    } else {
      [_ABI39_0_0ReactSubview removeGestureRecognizer:_menuButtonGestureRecognizer];
    }
  }
}
#endif
...
{
  if (_ABI39_0_0ReactSubview && _isPresented) {
    [_bridge.uiManager setSize:newBounds.size forView:_ABI39_0_0ReactSubview];
...
- (void)notifyForOrientationChange
{
  ...
  UIInterfaceOrientation currentOrientation = [ABI39_0_0RCTSharedApplication() statusBarOrientation];
...
{
  ABI39_0_0RCTAssert(_ABI39_0_0ReactSubview == nil, @"Modal view can only have one subview");
  ...
  for (NSString *key in [self.tvRemoteHandler.tvRemoteGestureRecognizers allKeys]) {
    if (![key isEqualToString:ABI39_0_0RCTTVRemoteEventMenu]) {
  ...
  _ABI39_0_0ReactSubview = subview;
}

- (void)removeABI39_0_0ReactSubview:(UIView *)subview
{
  ABI39_0_0RCTAssert(subview == _ABI39_0_0ReactSubview, @"Cannot remove view other than modal view");
...
#endif
  _ABI39_0_0ReactSubview = nil;
}

- (void)didUpdateABI39_0_0ReactSubviews
{
  // Do nothing, as subview (singular) is managed by `insertABI39_0_0ReactSubview:atIndex:`
}
...
- (void)didMoveToWindow
{
  ...
  if (!self.userInteractionEnabled && ![self.superview.ABI39_0_0ReactSubviews containsObject:self]) {
    return;
  }

  if (!_isPresented && self.window) {
    ABI39_0_0RCTAssert(self.ABI39_0_0ReactViewController, @"Can't present modal view controller without a presenting view controller");
