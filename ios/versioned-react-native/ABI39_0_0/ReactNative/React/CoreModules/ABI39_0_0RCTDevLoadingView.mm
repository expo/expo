...
@interface ABI39_0_0RCTDevLoadingView () <ABI39_0_0NativeDevLoadingViewSpec>
@end

#if ABI39_0_0RCT_DEV | ABI39_0_0RCT_ENABLE_LOADING_VIEW

@implementation ABI39_0_0RCTDevLoadingView {
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
}
...
{
  ABI39_0_0RCTDevLoadingViewSetEnabled(enabled);
}
...
- (void)setBridge:(ABI39_0_0RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(hide)
                                               name:ABI39_0_0RCTJavaScriptDidFailToLoadNotification
                                             object:nil];
...
{
  if (!ABI39_0_0RCTDevLoadingViewGetEnabled()) {
    return;
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    self->_showDate = [NSDate date];
    if (!self->_window && !ABI39_0_0RCTRunningInTestEnvironment()) {
      CGSize screenSize = [UIScreen mainScreen].bounds.size;

      if (@available(iOS 11.0, *)) {
        UIWindow *window = ABI39_0_0RCTSharedApplication().keyWindow;
        self->_window =
    ...
    if (@available(iOS 13.0, *)) {
      UIWindowScene *scene = (UIWindowScene *)ABI39_0_0RCTSharedApplication().connectedScenes.anyObject;
...
ABI39_0_0RCT_EXPORT_METHOD(showMessage
                  : (NSString *)message withColor
                  : (NSNumber *__nonnull)color withBackgroundColor
                  : (NSNumber *__nonnull)backgroundColor)
{
  [self showMessage:message color:[ABI39_0_0RCTConvert UIColor:color] backgroundColor:[ABI39_0_0RCTConvert UIColor:backgroundColor]];
}
...
- (void)showWithURL:(NSURL *)URL
{
  ...
  if (URL.fileURL) {
    ...
    message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI39_0_0RCT_PACKAGER_NAME];
  } else {
    color = [UIColor whiteColor];
...
- (void)updateProgress:(ABI39_0_0RCTLoadingProgress *)progress
{
  if (!progress) {
    return;
  }
...
Class ABI39_0_0RCTDevLoadingViewCls(void)
{
  return ABI39_0_0RCTDevLoadingView.class;
}
