...
NSString *const ABI39_0_0RCTShowDevMenuNotification = @"ABI39_0_0RCTShowDevMenuNotification";

@implementation UIWindow (ABI39_0_0RCTDevMenu)

- (void)ABI39_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI39_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

@implementation ABI39_0_0RCTDevMenuItem {
  ABI39_0_0RCTDevMenuItemTitleBlock _titleBlock;
  dispatch_block_t _handler;
}

- (instancetype)initWithTitleBlock:(ABI39_0_0RCTDevMenuItemTitleBlock)titleBlock handler:(dispatch_block_t)handler
{
  if ((self = [super init])) {
...
typedef void (^ABI39_0_0RCTDevMenuAlertActionHandler)(UIAlertAction *action);

@interface ABI39_0_0RCTDevMenu () <ABI39_0_0RCTBridgeModule, ABI39_0_0RCTInvalidating, ABI39_0_0NativeDevMenuSpec>

@end

@implementation ABI39_0_0RCTDevMenu {
  UIAlertController *_actionSheet;
  NSMutableArray<ABI39_0_0RCTDevMenuItem *> *_extraMenuItems;
}
  ...
  if ((self = [super init])) {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showOnShake)
                                                 name:ABI39_0_0RCTShowDevMenuNotification
                                               object:nil];
    _extraMenuItems = [NSMutableArray new];

#if TARGET_OS_SIMULATOR || TARGET_OS_MACCATALYST
    ABI39_0_0RCTKeyCommands *commands = [ABI39_0_0RCTKeyCommands sharedInstance];
...
{
  [self addItem:[ABI39_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
{
  [[ABI39_0_0RCTBundleURLProvider sharedSettings] resetToDefaults];
  self->_bridge.bundleURL = [[ABI39_0_0RCTBundleURLProvider sharedSettings] jsBundleURLForFallbackResource:nil
                                                                                fallbackExtension:nil];
  ABI39_0_0RCTTriggerReloadCommandListeners(@"Dev menu - reset to default");
}

- (NSArray<ABI39_0_0RCTDevMenuItem *> *)_menuItemsToPresent
{
  NSMutableArray<ABI39_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items
  __weak ABI39_0_0RCTBridge *bridge = _bridge;
  __weak ABI39_0_0RCTDevSettings *devSettings = _bridge.devSettings;
  __weak ABI39_0_0RCTDevMenu *weakSelf = self;

  [items addObject:[ABI39_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload"
                                               handler:^{
                                                 ABI39_0_0RCTTriggerReloadCommandListeners(@"Dev menu - reload");
                                               }]];

  if (!devSettings.isProfilingEnabled) {
    if (!devSettings.isRemoteDebuggingAvailable) {
      [items
          addObject:[ABI39_0_0RCTDevMenuItem
                        buttonItemWithTitle:@"Debugger Unavailable"
                                    handler:^{
                                      NSString *message = ABI39_0_0RCTTurboModuleEnabled()
                                          ? @"Debugging is not currently supported when TurboModule is enabled."
                                          : @"Include the ABI39_0_0RCTWebSocket library to enable JavaScript debugging.";
                                      ...
                                      [ABI39_0_0RCTPresentedViewController() presentViewController:alertController
                                                                                 animated:YES
                                                                               completion:NULL];
                                    }]];
    } else {
      [items addObject:[ABI39_0_0RCTDevMenuItem
  ...
  [items
      addObject:[ABI39_0_0RCTDevMenuItem
                    ...
                    handler:^{
                          ...
                          addAction:[UIAlertAction
                                        actionWithTitle:@"Apply Changes"
                                                ...
                                                handler:^(__unused UIAlertAction *action) {
                                                  ...
                                                  if (portNumber == nil) {
                                                    portNumber = [NSNumber numberWithInt:ABI39_0_0RCT_METRO_PORT];
                                                  }
                                                  [ABI39_0_0RCTBundleURLProvider sharedSettings].jsLocation = [NSString
                                                      stringWithFormat:@"%@:%d", ipTextField.text, portNumber.intValue];
                                                  __strong ABI39_0_0RCTBridge *strongBridge = bridge;
                                                  if (strongBridge) {
                                                    NSURL *bundleURL = bundleRoot.length
                                                        ? [[ABI39_0_0RCTBundleURLProvider sharedSettings]
                                                    ...
                                                    ABI39_0_0RCTTriggerReloadCommandListeners(@"Dev menu - apply changes");
                                                  }
                                                }]];
                      ...
                      [ABI39_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
                    }]];
...
{
  if (_actionSheet || !_bridge || ABI39_0_0RCTRunningInAppExtension()) {
    return;
  }
  ...
  _actionSheet = [UIAlertController alertControllerWithTitle:@"ABI39_0_0React Native Debug Menu"
                                                     message:description
                                              preferredStyle:style];

  NSArray<ABI39_0_0RCTDevMenuItem *> *items = [self _menuItemsToPresent];
  for (ABI39_0_0RCTDevMenuItem *item in items) {
  ...
  [ABI39_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:nil];

  [_bridge enqueueJSCall:@"ABI39_0_0RCTNativeAppEventEmitter" method:@"emit" args:@[ @"ABI39_0_0RCTDevMenuShown" ] completion:NULL];
}

- (ABI39_0_0RCTDevMenuAlertActionHandler)alertActionHandlerForDevItem:(ABI39_0_0RCTDevMenuItem *__nullable)item
{
...
#define WARN_DEPRECATED_DEV_MENU_EXPORT() \
  ABI39_0_0RCTLogWarn(@"Using deprecated method %s, use ABI39_0_0RCTDevSettings instead", __func__)
...
ABI39_0_0RCT_EXPORT_METHOD(reload)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  ABI39_0_0RCTTriggerReloadCommandListeners(@"Unknown from JS");
}

ABI39_0_0RCT_EXPORT_METHOD(debugRemotely : (BOOL)enableDebug)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  _bridge.devSettings.isDebuggingRemotely = enableDebug;
}

ABI39_0_0RCT_EXPORT_METHOD(setProfilingEnabled : (BOOL)enabled)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
...
ABI39_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled : (BOOL)enabled)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
...
{
  return std::make_shared<ABI39_0_0facebook::ABI39_0_0React::NativeDevMenuSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}
...
@interface ABI39_0_0RCTDevMenu () <ABI39_0_0NativeDevMenuSpec>
@end
...
- (void)addItem:(ABI39_0_0RCTDevMenu *)item
{
}
...
@implementation ABI39_0_0RCTBridge (ABI39_0_0RCTDevMenu)

- (ABI39_0_0RCTDevMenu *)devMenu
{
#if ABI39_0_0RCT_DEV_MENU
  return [self moduleForClass:[ABI39_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
...
Class ABI39_0_0RCTDevMenuCls(void)
{
  return ABI39_0_0RCTDevMenu.class;
}
