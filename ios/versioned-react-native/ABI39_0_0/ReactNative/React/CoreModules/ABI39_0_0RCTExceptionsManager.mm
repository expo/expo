...
#import "ABI39_0_0RCTExceptionsManager.h"
...
@interface ABI39_0_0RCTExceptionsManager () <ABI39_0_0NativeExceptionsManagerSpec>

@end

@implementation ABI39_0_0RCTExceptionsManager

@synthesize bridge = _bridge;

ABI39_0_0RCT_EXPORT_MODULE()

- (instancetype)initWithDelegate:(id<ABI39_0_0RCTExceptionsManagerDelegate>)delegate
{
  if ((self = [self init])) {
    _delegate = delegate;
  }
     ...
     suppressRedBox:(BOOL)suppressRedBox
{
  ...
  if (!ABI39_0_0RCT_DEBUG && reloadRetries < _maxReloadAttempts) {
    reloadRetries++;
    ABI39_0_0RCTTriggerReloadCommandListeners(@"JS Crash Reload");
  } else if (!ABI39_0_0RCT_DEV || !suppressRedBox) {
    NSString *description = [@"Unhandled JS Exception: " stringByAppendingString:message];
    NSDictionary *errorInfo = @{NSLocalizedDescriptionKey : description, ABI39_0_0RCTJSStackTraceKey : stack};
    ABI39_0_0RCTFatal([NSError errorWithDomain:ABI39_0_0RCTErrorDomain code:0 userInfo:errorInfo]);
  }
}

ABI39_0_0RCT_EXPORT_METHOD(reportSoftException
...
ABI39_0_0RCT_EXPORT_METHOD(reportFatalException
                  : (NSString *)message stack
                  : (NSArray<NSDictionary *> *)stack exceptionId
                  : (double)exceptionId)
{
...
ABI39_0_0RCT_EXPORT_METHOD(updateExceptionMessage
                  : (NSString *)message stack
                  : (NSArray<NSDictionary *> *)stack exceptionId
                  : (double)exceptionId)
{
...
ABI39_0_0RCT_EXPORT_METHOD(reportUnhandledException : (NSString *)message stack : (NSArray<NSDictionary *> *)stack)
{
  [self reportFatalException:message stack:stack exceptionId:-1];
}

ABI39_0_0RCT_EXPORT_METHOD(dismissRedbox) {}

ABI39_0_0RCT_EXPORT_METHOD(reportException : (JS::NativeExceptionsManager::ExceptionData &)data)
{
...
{
  return std::make_shared<ABI39_0_0facebook::ABI39_0_0React::NativeExceptionsManagerSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

Class ABI39_0_0RCTExceptionsManagerCls(void)
{
  return ABI39_0_0RCTExceptionsManager.class;
}
