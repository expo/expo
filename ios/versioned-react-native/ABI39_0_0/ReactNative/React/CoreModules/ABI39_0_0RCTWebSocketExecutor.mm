...
#if ABI39_0_0RCT_DEV // Debug executors are only supported in dev mode

typedef void (^ABI39_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI39_0_0RCTWebSocketExecutor () <ABI39_0_0RCTSRWebSocketDelegate, ABI39_0_0RCTTurboModule>

@end

@implementation ABI39_0_0RCTWebSocketExecutor {
  ABI39_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI39_0_0RCTWSMessageCallback> *_callbacks;
...
{
  if (!_url) {
    NSInteger port = [[[_bridge bundleURL] port] integerValue] ?: ABI39_0_0RCT_METRO_PORT;
  ...
  if (![self connectToProxy]) {
    ...
    NSString *error = [NSString stringWithFormat:
                                    @"Connection to %@ timed out. Are you "
                                     ...
                                     "you have the right IP address in `ABI39_0_0RCTWebSocketExecutor.m`.",
                                    _url];
    _setupError = ABI39_0_0RCTErrorWithMessage(error);
    ABI39_0_0RCTFatal(_setupError);
    return;
  }
...
- (BOOL)connectToProxy
{
  ...
  return connected == 0 && _socket.readyState == ABI39_0_0RCTSR_OPEN;
}
...
- (BOOL)prepareJSRuntime
{
  ...
  if (initError) {
    ABI39_0_0RCTLogInfo(@"Websocket runtime setup failed: %@", initError);
  }
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI39_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI39_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
{
  dispatch_semaphore_signal(_socketOpenSemaphore);
  ABI39_0_0RCTLogInfo(@"WebSocket connection failed with error %@", error);
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message onReply:(ABI39_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    if (!self.valid) {
      callback(ABI39_0_0RCTErrorWithMessage(@"Runtime is not ready for debugging. Make sure Packager server is running."), nil);
      return;
    }
    ...
    [self->_socket send:ABI39_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}
                      ...
                      onComplete:(ABI39_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ...
  // from completely invalidating the bridge and preventing an endless barage of ABI39_0_0RCTLog.logIfNoNativeHook
  ...
  NSDictionary<NSString *, id> *message = @{
    ...
    @"url" : ABI39_0_0RCTNullIfNil(URL.absoluteString),
  ...
  [self sendMessage:message
            onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
              if (socketError) {
                ...
                onComplete(error ? ABI39_0_0RCTErrorWithMessage(error) : nil);
              }
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI39_0_0RCTJavaScriptCallback)onComplete
{
  ABI39_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  ...
  [self sendMessage:message
            onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
              ...
              NSError *jsonError;
              id result = ABI39_0_0RCTJSONParse(reply[@"result"], &jsonError);
              NSString *error = reply[@"error"];
              onComplete(result, error ? ABI39_0_0RCTErrorWithMessage(error) : jsonError);
            }];
}
...
{
  ABI39_0_0RCTExecuteOnMainQueue(block);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI39_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI39_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
...
Class ABI39_0_0RCTWebSocketExecutorCls(void)
{
#if ABI39_0_0RCT_DEV
  return ABI39_0_0RCTWebSocketExecutor.class;
#else
  return nil;
#endif
}
