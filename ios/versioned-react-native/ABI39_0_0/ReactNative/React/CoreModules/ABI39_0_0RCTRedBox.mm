...
@class ABI39_0_0RCTRedBoxWindow;

@interface UIButton (ABI39_0_0RCTRedBox)

@property (nonatomic) ABI39_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI39_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI39_0_0RCTRedBox)

- (ABI39_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI39_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI39_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI39_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI39_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI39_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI39_0_0RCTRedBoxWindow : NSObject <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UIViewController *rootViewController;
@property (nonatomic, weak) id<ABI39_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI39_0_0RCTRedBoxWindow {
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI39_0_0RCTJSStackFrame *> *_lastStackTrace;
  int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame
           customButtonTitles:(NSArray<NSString *> *)customButtonTitles
         customButtonHandlers:(NSArray<ABI39_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
  if (self = [super init]) {
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title
    accessibilityIdentifier:(NSString *)accessibilityIdentifier
                   selector:(SEL)selector
                      block:(ABI39_0_0RCTRedBoxButtonPressHandler)block
{
...
- (NSInteger)bottomSafeViewHeight
{
  if (@available(iOS 11.0, *)) {
    return ABI39_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
  } else {
    return 0;
  }
}
...
- (void)showErrorMessage:(NSString *)message
               withStack:(NSArray<ABI39_0_0RCTJSStackFrame *> *)stack
    ...
    if (!isRootViewControllerPresented) {
      [_stackTraceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]
                                  atScrollPosition:UITableViewScrollPositionTop
                                          animated:NO];
      [ABI39_0_0RCTKeyWindow().rootViewController presentViewController:self.rootViewController animated:YES completion:nil];
    }
  }
}
...
- (void)copyStack
{
  ...
  for (ABI39_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI39_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI39_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI39_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI39_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI39_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI39_0_0RCTKeyCommands for this, but since
...
@interface ABI39_0_0RCTRedBox () <
    ABI39_0_0RCTInvalidating,
    ABI39_0_0RCTRedBoxWindowActionDelegate,
    ABI39_0_0RCTRedBoxExtraDataActionDelegate,
    ABI39_0_0NativeRedBoxSpec>
@end

@implementation ABI39_0_0RCTRedBox {
  ABI39_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI39_0_0RCTErrorCustomizer>> *_errorCustomizers;
  ABI39_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
  NSMutableArray<NSString *> *_customButtonTitles;
  NSMutableArray<ABI39_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (ABI39_0_0RCTErrorInfo *)_customizeError:(ABI39_0_0RCTErrorInfo *)error
{
  ABI39_0_0RCTAssertMainQueue();
  if (!self->_errorCustomizers) {
    return error;
  }
  for (id<ABI39_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI39_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI39_0_0RCTJSStackTraceKey]
             errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message
             withDetails:(NSString *)details
                   stack:(NSArray<ABI39_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
...
{
  NSArray<ABI39_0_0RCTJSStackFrame *> *stack = [ABI39_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
  [self showErrorMessage:message
         withParsedStack:[ABI39_0_0RCTJSStackFrame stackFramesWithDictionaries:stack]
                isUpdate:NO
             ...
             errorCookie:(int)errorCookie
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self->_extraDataViewController == nil) {
      self->_extraDataViewController = [ABI39_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
    ...
    if (!self->_window) {
      self->_window = [[ABI39_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds
    ...
    ABI39_0_0RCTErrorInfo *errorInfo = [[ABI39_0_0RCTErrorInfo alloc] initWithErrorMessage:message stack:stack];
...
ABI39_0_0RCT_EXPORT_METHOD(setExtraData : (NSDictionary *)extraData forIdentifier : (NSString *)identifier)
{
  [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI39_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI39_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI39_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI39_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI39_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI39_0_0RCTRedBoxWindow *)redBoxWindow
{
  if (_overrideReloadAction) {
    _overrideReloadAction();
  } else {
    ABI39_0_0RCTTriggerReloadCommandListeners(@"Redbox");
  }
  [self dismiss];
}
...
{
  return std::make_shared<ABI39_0_0facebook::ABI39_0_0React::NativeRedBoxSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

@implementation ABI39_0_0RCTBridge (ABI39_0_0RCTRedBox)

- (ABI39_0_0RCTRedBox *)redBox
{
  return ABI39_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI39_0_0RCTRedBox class]] : nil;
}
...
@interface ABI39_0_0RCTRedBox () <ABI39_0_0NativeRedBoxSpec>
@end
...
Class ABI39_0_0RCTRedBoxCls(void)
{
  return ABI39_0_0RCTRedBox.class;
}
