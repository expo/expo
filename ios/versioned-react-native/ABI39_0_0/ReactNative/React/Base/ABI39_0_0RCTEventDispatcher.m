...
const NSInteger ABI39_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI39_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI39_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI39_0_0RCTUniqueCoalescingKeyGenerator = 0;

@implementation ABI39_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI39_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI39_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI39_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI39_0_0RCTComponentEvent *event = [[ABI39_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI39_0_0ReactTag body:body];
...
- (void)sendEvent:(id<ABI39_0_0RCTEvent>)event
{
  ...
  for (id<ABI39_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI39_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI39_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI39_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI39_0_0RCTGetEventID(event.viewTag, event.eventName, ABI39_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI39_0_0RCTAssert(
        previousEvent == nil,
...
@implementation ABI39_0_0RCTBridge (ABI39_0_0RCTEventDispatcher)

- (ABI39_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI39_0_0RCTEventDispatcher class]];
}
