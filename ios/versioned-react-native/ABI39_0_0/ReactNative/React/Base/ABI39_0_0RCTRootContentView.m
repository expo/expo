...
@implementation ABI39_0_0RCTRootContentView

- (instancetype)initWithFrame:(CGRect)frame
                       bridge:(ABI39_0_0RCTBridge *)bridge
                     ABI39_0_0ReactTag:(NSNumber *)ABI39_0_0ReactTag
               sizeFlexiblity:(ABI39_0_0RCTRootViewSizeFlexibility)sizeFlexibility
{
  if ((self = [super initWithFrame:frame])) {
    _bridge = bridge;
    self.ABI39_0_0ReactTag = ABI39_0_0ReactTag;
...
ABI39_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : (nonnull NSCoder *)aDecoder)
...
- (void)insertABI39_0_0ReactSubview:(UIView *)subview atIndex:(NSInteger)atIndex
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_contentHasAppeared) {
      ...
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI39_0_0RCTContentDidAppearNotification object:self.superview];
    }
  });
}
...
- (CGSize)availableSize
{
  ...
  return CGSizeMake(
      _sizeFlexibility & ABI39_0_0RCTRootViewSizeFlexibilityWidth ? INFINITY : size.width,
      _sizeFlexibility & ABI39_0_0RCTRootViewSizeFlexibilityHeight ? INFINITY : size.height);
}

- (void)updateAvailableSize
{
  if (!self.ABI39_0_0ReactTag || !_bridge.isValid) {
    return;
  }
...
- (void)invalidate
{
  if (self.userInteractionEnabled) {
    ...
    [(ABI39_0_0RCTRootView *)self.superview contentViewInvalidated];

    [_bridge enqueueJSCall:@"AppRegistry"
                    method:@"unmountApplicationComponentAtRootTag"
                      args:@[ self.ABI39_0_0ReactTag ]
                completion:NULL];
  }
}
