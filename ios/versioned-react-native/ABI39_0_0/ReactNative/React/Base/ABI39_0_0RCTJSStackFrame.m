...
#import "ABI39_0_0RCTJSStackFrame.h"
 ...
 */
static NSRegularExpression *ABI39_0_0RCTJSStackFrameRegex()
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    if (regexError) {
      ABI39_0_0RCTLogError(@"Failed to build regex: %@", [regexError localizedDescription]);
    }
  });
  return _regex;
}

@implementation ABI39_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI39_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI39_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI39_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI39_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI39_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI39_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI39_0_0RCTNilIfNull(dict[@"collapse"]) integerValue]];
}

+ (NSArray<ABI39_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI39_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI39_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI39_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
