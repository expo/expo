...
@class ABI39_0_0RCTSurfaceView;
@protocol ABI39_0_0RCTSurfaceDelegate;

/**
 * ABI39_0_0RCTSurface instance represents ABI39_0_0React Native-powered piece of a user interface
 * which can be a full-screen app, separate modal view controller,
 * or even small widget.
 * It is called "Surface".
 *
 * The ABI39_0_0RCTSurface instance is completely thread-safe by design;
 ...
 *
 * The primary goals of the ABI39_0_0RCTSurface are:
 ...
 */
@interface ABI39_0_0RCTSurface : NSObject

@property (atomic, readonly) ABI39_0_0RCTSurfaceStage stage;
@property (atomic, readonly) NSString *moduleName;
@property (atomic, readonly) NSNumber *rootViewTag;

@property (atomic, readwrite, weak, nullable) id<ABI39_0_0RCTSurfaceDelegate> delegate;
 ...
 * ABI39_0_0RCTSurface does not control (or influence in any way) the size or origin
...
/**
 * Simple shortcut to `-[ABI39_0_0RCTSurface setMinimumSize:size maximumSize:size]`.
 */
 ...
 *  - Calling on the main queue with `ABI39_0_0RCTSurfaceStageSurfaceDidInitialMounting`
 *    stage temporary is not supported; in this case the stage will be
 *    downgraded to `ABI39_0_0RCTSurfaceStageSurfaceDidInitialLayout`.
 */
- (BOOL)synchronouslyWaitForStage:(ABI39_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout;

#pragma mark - Start & Stop

/**
 * Starts or stops the Surface.
 * Those methods are a no-op for regular ABI39_0_0RCTSurface (for now), but all call sites must call them appropriately.
 */
- (BOOL)start;
- (BOOL)stop;

#pragma mark - Mounting/Unmounting of ABI39_0_0React components

/**
 * Mount the ABI39_0_0React component specified by the given moduleName. This is typically
 * calling runApplication.js from the native side.
 */
- (void)mountABI39_0_0ReactComponentWithBridge:(ABI39_0_0RCTBridge *)bridge
                           moduleName:(NSString *)moduleName
                               params:(NSDictionary *)params;

/**
 * Unmount the ABI39_0_0React component specified by the given rootViewTag, called from native.
 */
- (void)unmountABI39_0_0ReactComponentWithBridge:(ABI39_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag;
