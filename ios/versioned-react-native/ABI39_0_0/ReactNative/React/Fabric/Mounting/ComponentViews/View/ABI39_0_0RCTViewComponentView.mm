...
#import <ABI39_0_0React/components/view/ViewComponentDescriptor.h>
#import <ABI39_0_0React/components/view/ViewEventEmitter.h>
#import <ABI39_0_0React/components/view/ViewProps.h>

#import "ABI39_0_0RCTConversions.h"

using namespace ABI39_0_0facebook::ABI39_0_0React;

@implementation ABI39_0_0RCTViewComponentView {
...
- (void)layoutSubviews
{
  ...
  // by ABI39_0_0React Native Renderer within `CATransaction`.
...
{
  ABI39_0_0RCTAssert(
      self == [ABI39_0_0RCTViewComponentView class],
      @"`+[ABI39_0_0RCTComponentViewProtocol componentDescriptorProvider]` must be implemented for all subclasses (and `%@` particularly).",
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  ABI39_0_0RCTAssert(
      propsRawPtr &&
          ([self class] == [ABI39_0_0RCTViewComponentView class] ||
           typeid(*propsRawPtr).hash_code() != typeid(ViewProps const).hash_code()),
      @"`ABI39_0_0RCTViewComponentView` subclasses (and `%@` particularly) must setup `_props`"
  ...
  if (oldViewProps.backgroundColor != newViewProps.backgroundColor) {
    self.backgroundColor = ABI39_0_0RCTUIColorFromSharedColor(newViewProps.backgroundColor);
    needsInvalidateLayer = YES;
  }

  // `foregroundColor`
  if (oldViewProps.foregroundColor != newViewProps.foregroundColor) {
    self.foregroundColor = ABI39_0_0RCTUIColorFromSharedColor(newViewProps.foregroundColor);
  }

  // `shadowColor`
  if (oldViewProps.shadowColor != newViewProps.shadowColor) {
    CGColorRef shadowColor = ABI39_0_0RCTCGColorRefFromSharedColor(newViewProps.shadowColor);
  ...
  if (oldViewProps.shadowOffset != newViewProps.shadowOffset) {
    self.layer.shadowOffset = ABI39_0_0RCTCGSizeFromSize(newViewProps.shadowOffset);
    needsInvalidateLayer = YES;
  }
  ...
  if (oldViewProps.transform != newViewProps.transform) {
    self.layer.transform = ABI39_0_0RCTCATransform3DFromTransformMatrix(newViewProps.transform);
    self.layer.allowsEdgeAntialiasing = newViewProps.transform != Transform::Identity();
  }

  // `hitSlop`
  if (oldViewProps.hitSlop != newViewProps.hitSlop) {
    self.hitTestEdgeInsets = ABI39_0_0RCTUIEdgeInsetsFromEdgeInsets(newViewProps.hitSlop);
  }
  ...
  if (oldViewProps.nativeId != newViewProps.nativeId) {
    self.nativeId = ABI39_0_0RCTNSStringFromStringNilIfEmpty(newViewProps.nativeId);
  }
  ...
  if (oldViewProps.accessibilityLabel != newViewProps.accessibilityLabel) {
    self.accessibilityElement.accessibilityLabel = ABI39_0_0RCTNSStringFromStringNilIfEmpty(newViewProps.accessibilityLabel);
  }

  // `accessibilityHint`
  if (oldViewProps.accessibilityHint != newViewProps.accessibilityHint) {
    self.accessibilityElement.accessibilityHint = ABI39_0_0RCTNSStringFromStringNilIfEmpty(newViewProps.accessibilityHint);
  }
  ...
  if (oldViewProps.accessibilityTraits != newViewProps.accessibilityTraits) {
    self.accessibilityElement.accessibilityTraits =
        ABI39_0_0RCTUIAccessibilityTraitsFromAccessibilityTraits(newViewProps.accessibilityTraits);
  }
           ...
           oldLayoutMetrics:(LayoutMetrics const &)oldLayoutMetrics
{
  ...
  if (_contentView) {
    _contentView.frame = ABI39_0_0RCTCGRectFromRect(_layoutMetrics.getContentFrame());
  }
}
...
static ABI39_0_0RCTCornerRadii ABI39_0_0RCTCornerRadiiFromBorderRadii(BorderRadii borderRadii)
{
  return ABI39_0_0RCTCornerRadii{.topLeft = (CGFloat)borderRadii.topLeft,
...
static ABI39_0_0RCTBorderColors ABI39_0_0RCTBorderColorsFromBorderColors(BorderColors borderColors)
{
  return ABI39_0_0RCTBorderColors{.left = ABI39_0_0RCTCGColorRefUnretainedFromSharedColor(borderColors.left),
                         .top = ABI39_0_0RCTCGColorRefUnretainedFromSharedColor(borderColors.top),
                         .bottom = ABI39_0_0RCTCGColorRefUnretainedFromSharedColor(borderColors.bottom),
                         .right = ABI39_0_0RCTCGColorRefUnretainedFromSharedColor(borderColors.right)};
}

static ABI39_0_0RCTBorderStyle ABI39_0_0RCTBorderStyleFromBorderStyle(BorderStyle borderStyle)
{
  switch (borderStyle) {
    case BorderStyle::Solid:
      return ABI39_0_0RCTBorderStyleSolid;
    case BorderStyle::Dotted:
      return ABI39_0_0RCTBorderStyleDotted;
    case BorderStyle::Dashed:
      return ABI39_0_0RCTBorderStyleDashed;
  }
}
...
- (void)invalidateLayer
{
  ...
  if (layerHasShadow) {
    if (CGColorGetAlpha(_backgroundColor.CGColor) > 0.999) {
      ...
      ABI39_0_0RCTCornerInsets const cornerInsets =
          ABI39_0_0RCTGetCornerInsets(ABI39_0_0RCTCornerRadiiFromBorderRadii(borderMetrics.borderRadii), UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI39_0_0RCTPathCreateWithRoundedRect(self.bounds, cornerInsets, nil);
  ...
  if (useCoreAnimationBorderRendering) {
    ...
    CGColorRef borderColor = ABI39_0_0RCTCGColorRefFromSharedColor(borderMetrics.borderColors.left);
    ...
    UIImage *image = ABI39_0_0RCTGetBorderImage(
        ABI39_0_0RCTBorderStyleFromBorderStyle(borderMetrics.borderStyles.left),
        layer.bounds.size,
        ABI39_0_0RCTCornerRadiiFromBorderRadii(borderMetrics.borderRadii),
        ABI39_0_0RCTUIEdgeInsetsFromEdgeInsets(borderMetrics.borderWidths),
        ABI39_0_0RCTBorderColorsFromBorderColors(borderMetrics.borderColors),
    ...
    if (self.clipsToBounds) {
      if (borderMetrics.borderRadii.isUniform()) {
        ...
        CGPathRef path = ABI39_0_0RCTPathCreateWithRoundedRect(
            self.bounds,
            ABI39_0_0RCTGetCornerInsets(ABI39_0_0RCTCornerRadiiFromBorderRadii(borderMetrics.borderRadii), UIEdgeInsetsZero),
            nil);
...
static NSString *ABI39_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *result = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI39_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI39_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (NSArray<UIAccessibilityCustomAction *> *)accessibilityCustomActions
{
  ...
  for (auto const &accessibilityAction : accessibilityActions) {
    [customActions
        addObject:[[UIAccessibilityCustomAction alloc] initWithName:ABI39_0_0RCTNSStringFromString(accessibilityAction)
                                                             target:self
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  if (_eventEmitter && _props->onAccessibilityAction) {
    _eventEmitter->onAccessibilityAction(ABI39_0_0RCTStringFromNSString(action.name));
    return YES;
  } else {
    return NO;
  }
...
{
  return ABI39_0_0RCTNSStringFromString([[self class] componentDescriptorProvider].name);
}
