...
@implementation ABI39_0_0RCTFabricSurface {
  // Immutable
  ABI39_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI39_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}

- (instancetype)initWithSurfacePresenter:(ABI39_0_0RCTSurfacePresenter *)surfacePresenter
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI39_0_0RCTAllocateRootViewTag() integerValue];

    _minimumSize = CGSizeZero;
    // FIXME: Replace with `_maximumSize = CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX);`.
    _maximumSize = ABI39_0_0RCTScreenSize();
...
- (BOOL)start
{
  if (![self _setStage:ABI39_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI39_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI39_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI39_0_0RCTSurfaceView alloc] initWithSurface:(ABI39_0_0RCTSurface *)self];
...
- (BOOL)_setStage:(ABI39_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:YES];
}

- (BOOL)_unsetStage:(ABI39_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:NO];
}

- (BOOL)_setStage:(ABI39_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
...
- (void)_propagateStageChange:(ABI39_0_0RCTSurfaceStage)stage
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI39_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI39_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
