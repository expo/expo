...
#include "ABI46_0_0StubClock.h"
#include "ABI46_0_0StubErrorUtils.h"
#include "ABI46_0_0StubQueue.h"
...
TEST_F(RuntimeSchedulerTest, now) {
  ...
  ABI46_0_0EXPECT_EQ(runtimeScheduler_->now(), RuntimeSchedulerTimePoint(1ms));

  stubClock_->advanceTimeBy(10ms);

  ABI46_0_0EXPECT_EQ(runtimeScheduler_->now(), RuntimeSchedulerTimePoint(11ms));

  stubClock_->advanceTimeBy(6s);

  ABI46_0_0EXPECT_EQ(runtimeScheduler_->now(), RuntimeSchedulerTimePoint(6011ms));
}

TEST_F(RuntimeSchedulerTest, getShouldYield) {
  // Always returns false for now.
  ABI46_0_0EXPECT_FALSE(runtimeScheduler_->getShouldYield());
}
...
TEST_F(RuntimeSchedulerTest, scheduleSingleTask) {
  ...
  auto callback =
      createHostFunctionFromLambda([&didRunTask](bool didUserCallbackTimeout) {
        didRunTask = true;
        ABI46_0_0EXPECT_FALSE(didUserCallbackTimeout);
  ...
  ABI46_0_0EXPECT_FALSE(didRunTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI46_0_0EXPECT_TRUE(didRunTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, scheduleTwoTasksWithSamePriority) {
  ...
  ABI46_0_0EXPECT_EQ(firstTaskCallOrder, 0);
  ABI46_0_0EXPECT_EQ(secondTaskCallOrder, 0);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI46_0_0EXPECT_EQ(firstTaskCallOrder, 1);
  ABI46_0_0EXPECT_EQ(secondTaskCallOrder, 2);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
  ABI46_0_0EXPECT_EQ(hostFunctionCallCount_, 2);
}
...
TEST_F(RuntimeSchedulerTest, scheduleTwoTasksWithDifferentPriorities) {
  ...
  ABI46_0_0EXPECT_EQ(lowPriorityTaskCallOrder, 0);
  ABI46_0_0EXPECT_EQ(userBlockingPriorityTaskCallOrder, 0);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI46_0_0EXPECT_EQ(lowPriorityTaskCallOrder, 2);
  ABI46_0_0EXPECT_EQ(userBlockingPriorityTaskCallOrder, 1);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
  ABI46_0_0EXPECT_EQ(hostFunctionCallCount_, 2);
}
...
TEST_F(RuntimeSchedulerTest, continuationTask) {
  ...
  ABI46_0_0EXPECT_TRUE(didRunTask);
  ABI46_0_0EXPECT_TRUE(didContinuationTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, scheduleWork) {
  ...
  ABI46_0_0EXPECT_FALSE(wasCalled);
  ...
  ABI46_0_0EXPECT_TRUE(wasCalled);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, scheduleWorkWithYielding) {
  ...
  ABI46_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
...
TEST_F(RuntimeSchedulerTest, normalTaskYieldsToPlatformEvent) {
  ...
  auto callback = createHostFunctionFromLambda([&](bool) {
    ...
    ABI46_0_0EXPECT_TRUE(didRunPlatformWork);
    return jsi::Value::undefined();
  });
  ...
  runtimeScheduler_->scheduleWork([&](jsi::Runtime const &) {
    ...
    ABI46_0_0EXPECT_FALSE(didRunJavaScriptTask);
    ABI46_0_0EXPECT_FALSE(runtimeScheduler_->getShouldYield());
  });

  ABI46_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 2);
...
TEST_F(RuntimeSchedulerTest, expiredTaskDoesntYieldToPlatformEvent) {
  ...
  auto callback = createHostFunctionFromLambda([&](bool) {
    didRunJavaScriptTask = true;
    ABI46_0_0EXPECT_FALSE(didRunPlatformWork);
    return jsi::Value::undefined();
  });
  ...
  runtimeScheduler_->scheduleWork([&](jsi::Runtime const &) {
    didRunPlatformWork = true;
    ABI46_0_0EXPECT_TRUE(didRunJavaScriptTask);
  });
...
TEST_F(RuntimeSchedulerTest, scheduleTaskFromTask) {
  ...
  ABI46_0_0EXPECT_FALSE(didRunFirstTask);
  ABI46_0_0EXPECT_FALSE(didRunSecondTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI46_0_0EXPECT_TRUE(didRunFirstTask);
  ABI46_0_0EXPECT_TRUE(didRunSecondTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, handlingError) {
  ...
  ABI46_0_0EXPECT_TRUE(didRunTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 0);
  ABI46_0_0EXPECT_EQ(stubErrorUtils_->getReportFatalCallCount(), 1);
}
...
TEST_F(RuntimeSchedulerTest, basicSameThreadExecution) {
  ...
  std::thread t1([this, &didRunSynchronousTask]() {
    runtimeScheduler_->executeNowOnTheSameThread(
        [this, &didRunSynchronousTask](jsi::Runtime &rt) {
          ABI46_0_0EXPECT_TRUE(runtimeScheduler_->getIsSynchronous());
          didRunSynchronousTask = true;
        });
    ABI46_0_0EXPECT_FALSE(runtimeScheduler_->getIsSynchronous());
  });

  auto hasTask = stubQueue_->waitForTask(1ms);

  ABI46_0_0EXPECT_TRUE(hasTask);
  ABI46_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);
  ...
  ABI46_0_0EXPECT_TRUE(didRunSynchronousTask);
}
...
TEST_F(RuntimeSchedulerTest, sameThreadTaskCreatesImmediatePriorityTask) {
  ...
  ABI46_0_0EXPECT_TRUE(hasTask);
  ABI46_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI46_0_0EXPECT_FALSE(didRunSubsequentTask);
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);
  ...
  ABI46_0_0EXPECT_TRUE(didRunSynchronousTask);
  ABI46_0_0EXPECT_TRUE(didRunSubsequentTask);
}
...
TEST_F(RuntimeSchedulerTest, twoThreadsRequestAccessToTheRuntime) {
  ...
  ABI46_0_0EXPECT_TRUE(hasTask);
  ABI46_0_0EXPECT_FALSE(didRunWork);
  ABI46_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI46_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 2);
  ...
  ABI46_0_0EXPECT_TRUE(didRunWork);
  ABI46_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI46_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
  ABI46_0_0EXPECT_EQ(stubQueue_->size(), 1);
