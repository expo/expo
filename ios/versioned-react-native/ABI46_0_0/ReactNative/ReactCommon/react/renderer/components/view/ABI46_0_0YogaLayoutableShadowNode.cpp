...
#include "ABI46_0_0YogaLayoutableShadowNode.h"
#include <ABI46_0_0logger/ABI46_0_0React_native_log.h>
#include <ABI46_0_0React/ABI46_0_0debug/flags.h>
...
#include <ABI46_0_0React/ABI46_0_0renderer/components/view/conversions.h>
...
#include <ABI46_0_0React/ABI46_0_0renderer/debug/DebugStringConvertibleItem.h>
#include <ABI46_0_0React/ABI46_0_0renderer/debug/SystraceSection.h>
#include <ABI46_0_0yoga/ABI46_0_0Yoga.h>
#include <algorithm>
#include <limits>
#include <memory>
...
static int FabricDefaultYogaLog(
    const ABI46_0_0YGConfigRef,
    const ABI46_0_0YGNodeRef,
  ...
  switch (level) {
    case ABI46_0_0YGLogLevelError:
      ABI46_0_0React_native_log_error(buffer.data());
      break;
    case ABI46_0_0YGLogLevelFatal:
      ABI46_0_0React_native_log_fatal(buffer.data());
      break;
    case ABI46_0_0YGLogLevelWarn:
      ABI46_0_0React_native_log_warn(buffer.data());
      break;
    case ABI46_0_0YGLogLevelInfo:
    case ABI46_0_0YGLogLevelDebug:
    case ABI46_0_0YGLogLevelVerbose:
    default:
      ABI46_0_0React_native_log_info(buffer.data());
  }
...
YogaLayoutableShadowNode::YogaLayoutableShadowNode(
  ...
  // This is not a default for `ABI46_0_0YGNode`.
...
YogaLayoutableShadowNode::YogaLayoutableShadowNode(
  ...
  // Note, cloned `ABI46_0_0YGNode` instance (copied using copy-constructor) inherits
...
void YogaLayoutableShadowNode::adoptYogaChild(size_t index) {
  ...
  // Overflow checks.
  ABI46_0_0React_native_assert(children.size() > index);
  ABI46_0_0React_native_assert(children.size() >= yogaNode_.getChildren().size());
  ...
  // address). ABI46_0_0React_native_assert(layoutableChildNode.yogaNode_.getOwner() !=
  // &yogaNode_);
  ...
  if (layoutableChildNode.yogaNode_.getOwner() == nullptr) {
    ...
    // ABI46_0_0React_native_assert(layoutableChildNode.yogaNode_.isDirty());
  } else {
...
void YogaLayoutableShadowNode::appendChild(
  ...
  // (e.g. ABI46_0_0RCTRawText). This used to throw an error, but we are ignoring it
  ...
  if (traitCast<YogaLayoutableShadowNode const *>(childNode.get())) {
    ...
    ABI46_0_0React_native_log_error(
...
void YogaLayoutableShadowNode::updateYogaChildrenOwnersIfNeeded() {
  for (auto &childYogaNode : yogaNode_.getChildren()) {
    if (childYogaNode->getOwner() == &yogaNode_) {
      childYogaNode->setOwner(reinterpret_cast<ABI46_0_0YGNodeRef>(0xBADC0FFEE0DDF00D));
    }
  }
}
...
void YogaLayoutableShadowNode::updateYogaChildren() {
  ...
  auto oldYogaChildren = isClean ? yogaNode_.getChildren() : ABI46_0_0YGVector{};
  ...
  ABI46_0_0React_native_assert(getChildren().size() == yogaNode_.getChildren().size());
...
void YogaLayoutableShadowNode::setSize(Size size) const {
  ...
  style.dimensions()[ABI46_0_0YGDimensionWidth] = yogaStyleValueFromFloat(size.width);
  style.dimensions()[ABI46_0_0YGDimensionHeight] = yogaStyleValueFromFloat(size.height);
...
void YogaLayoutableShadowNode::setPadding(RectangleEdges<Float> padding) const {
  ...
  if (leftPadding != style.padding()[ABI46_0_0YGEdgeLeft] ||
      topPadding != style.padding()[ABI46_0_0YGEdgeTop] ||
      rightPadding != style.padding()[ABI46_0_0YGEdgeRight] ||
      bottomPadding != style.padding()[ABI46_0_0YGEdgeBottom]) {
    style.padding()[ABI46_0_0YGEdgeTop] = yogaStyleValueFromFloat(padding.top);
    style.padding()[ABI46_0_0YGEdgeLeft] = yogaStyleValueFromFloat(padding.left);
    style.padding()[ABI46_0_0YGEdgeRight] = yogaStyleValueFromFloat(padding.right);
    style.padding()[ABI46_0_0YGEdgeBottom] = yogaStyleValueFromFloat(padding.bottom);
    yogaNode_.setStyle(style);
    yogaNode_.setDirty(true);
  }
}
...
void YogaLayoutableShadowNode::layoutTree(
  ...
  ABI46_0_0React_native_assert(!std::isnan(minimumSize.width));
  ABI46_0_0React_native_assert(!std::isnan(minimumSize.height));
  ABI46_0_0React_native_assert(!std::isnan(maximumSize.width));
  ABI46_0_0React_native_assert(!std::isnan(maximumSize.height));
  // Values cannot be negative.
  ABI46_0_0React_native_assert(minimumSize.width >= 0);
  ABI46_0_0React_native_assert(minimumSize.height >= 0);
  ABI46_0_0React_native_assert(maximumSize.width >= 0);
  ABI46_0_0React_native_assert(maximumSize.height >= 0);
  // Mimimum size cannot be infinity.
  ABI46_0_0React_native_assert(!std::isinf(minimumSize.width));
  ABI46_0_0React_native_assert(!std::isinf(minimumSize.height));
  ...
  // Module`. Yoga C++ API (and `ABI46_0_0YGNodeCalculateLayout` function particularly)
  ...
  yogaStyle.maxDimensions()[ABI46_0_0YGDimensionWidth] = std::isfinite(maximumSize.width)
      ? yogaStyleValueFromFloat(maximumSize.width)
      : ABI46_0_0YGValueUndefined;

  yogaStyle.maxDimensions()[ABI46_0_0YGDimensionHeight] =
      std::isfinite(maximumSize.height)
      ? yogaStyleValueFromFloat(maximumSize.height)
      : ABI46_0_0YGValueUndefined;

  yogaStyle.minDimensions()[ABI46_0_0YGDimensionWidth] = minimumSize.width > 0
      ? yogaStyleValueFromFloat(minimumSize.width)
      : ABI46_0_0YGValueUndefined;

  yogaStyle.minDimensions()[ABI46_0_0YGDimensionHeight] = minimumSize.height > 0
  ...
  {
    SystraceSection s("YogaLayoutableShadowNode::ABI46_0_0YGNodeCalculateLayout");
    ABI46_0_0YGNodeCalculateLayout(&yogaNode_, ownerWidth, ownerHeight, direction);
  }
...
void YogaLayoutableShadowNode::layout(LayoutContext layoutContext) {
  ...
  ABI46_0_0React_native_assert(!yogaNode_.isDirty());
  ...
  for (auto childYogaNode : yogaNode_.getChildren()) {
    ...
    ABI46_0_0React_native_assert(&childNode.yogaNode_ == childYogaNode);

    if (childYogaNode->getHasNewLayout()) {
      childYogaNode->setHasNewLayout(false);

      // Reading data from a dirtied node does not make sense.
      ABI46_0_0React_native_assert(!childYogaNode->isDirty());

      // We must copy layout metrics from Yoga node only once (when the parent
      // node exclusively ownes the child node).
      ABI46_0_0React_native_assert(childYogaNode->getOwner() == &yogaNode_);
  ...
  if (yogaNode_.getStyle().overflow() == ABI46_0_0YGOverflowVisible) {
...
ABI46_0_0YGNode *YogaLayoutableShadowNode::yogaNodeCloneCallbackConnector(
    ABI46_0_0YGNode *oldYogaNode,
    ABI46_0_0YGNode *parentYogaNode,
    int childIndex) {
...
ABI46_0_0YGSize YogaLayoutableShadowNode::yogaNodeMeasureCallbackConnector(
    ABI46_0_0YGNode *yogaNode,
    float width,
...
ABI46_0_0YGConfig &YogaLayoutableShadowNode::initializeYogaConfig(ABI46_0_0YGConfig &config) {
  config.setCloneNodeCallback(
      YogaLayoutableShadowNode::yogaNodeCloneCallbackConnector);
  config.useLegacyStretchBehaviour = true;
#ifdef ABI46_0_0RN_DEBUG_YOGA_LOGGER
...
void YogaLayoutableShadowNode::swapLeftAndRightInYogaStyleProps(
  ...
  ABI46_0_0YGStyle::Edges const &position = yogaStyle.position();
  ABI46_0_0YGStyle::Edges const &padding = yogaStyle.padding();
  ABI46_0_0YGStyle::Edges const &margin = yogaStyle.margin();

  // Swap Yoga node values, position, padding and margin.

  if (yogaStyle.position()[ABI46_0_0YGEdgeLeft] != ABI46_0_0YGValueUndefined) {
    yogaStyle.position()[ABI46_0_0YGEdgeStart] = position[ABI46_0_0YGEdgeLeft];
    yogaStyle.position()[ABI46_0_0YGEdgeLeft] = ABI46_0_0YGValueUndefined;
  }

  if (yogaStyle.position()[ABI46_0_0YGEdgeRight] != ABI46_0_0YGValueUndefined) {
    yogaStyle.position()[ABI46_0_0YGEdgeEnd] = position[ABI46_0_0YGEdgeRight];
    yogaStyle.position()[ABI46_0_0YGEdgeRight] = ABI46_0_0YGValueUndefined;
  }

  if (yogaStyle.padding()[ABI46_0_0YGEdgeLeft] != ABI46_0_0YGValueUndefined) {
    yogaStyle.padding()[ABI46_0_0YGEdgeStart] = padding[ABI46_0_0YGEdgeLeft];
    yogaStyle.padding()[ABI46_0_0YGEdgeLeft] = ABI46_0_0YGValueUndefined;
  }

  if (yogaStyle.padding()[ABI46_0_0YGEdgeRight] != ABI46_0_0YGValueUndefined) {
    yogaStyle.padding()[ABI46_0_0YGEdgeEnd] = padding[ABI46_0_0YGEdgeRight];
    yogaStyle.padding()[ABI46_0_0YGEdgeRight] = ABI46_0_0YGValueUndefined;
  }

  if (yogaStyle.margin()[ABI46_0_0YGEdgeLeft] != ABI46_0_0YGValueUndefined) {
    yogaStyle.margin()[ABI46_0_0YGEdgeStart] = margin[ABI46_0_0YGEdgeLeft];
    yogaStyle.margin()[ABI46_0_0YGEdgeLeft] = ABI46_0_0YGValueUndefined;
  }

  if (yogaStyle.margin()[ABI46_0_0YGEdgeRight] != ABI46_0_0YGValueUndefined) {
    yogaStyle.margin()[ABI46_0_0YGEdgeEnd] = margin[ABI46_0_0YGEdgeRight];
    yogaStyle.margin()[ABI46_0_0YGEdgeRight] = ABI46_0_0YGValueUndefined;
  }
...
void YogaLayoutableShadowNode::swapLeftAndRightInViewProps(
  ...
  ABI46_0_0YGStyle::Edges const &border = props.yogaStyle.border();

  if (props.yogaStyle.border()[ABI46_0_0YGEdgeLeft] != ABI46_0_0YGValueUndefined) {
    props.yogaStyle.border()[ABI46_0_0YGEdgeStart] = border[ABI46_0_0YGEdgeLeft];
    props.yogaStyle.border()[ABI46_0_0YGEdgeLeft] = ABI46_0_0YGValueUndefined;
  }

  if (props.yogaStyle.border()[ABI46_0_0YGEdgeRight] != ABI46_0_0YGValueUndefined) {
    props.yogaStyle.border()[ABI46_0_0YGEdgeEnd] = border[ABI46_0_0YGEdgeRight];
    props.yogaStyle.border()[ABI46_0_0YGEdgeRight] = ABI46_0_0YGValueUndefined;
  }
}
...
void YogaLayoutableShadowNode::ensureYogaChildrenOwnersConsistency() const {
#ifdef ABI46_0_0REACT_NATIVE_DEBUG
  ...
  if (!yogaChildren.empty()) {
    ...
    for (auto const &child : yogaChildren) {
      ABI46_0_0React_native_assert(child->getOwner() == owner);
    }
  }
#endif
}
...
void YogaLayoutableShadowNode::ensureYogaChildrenLookFine() const {
  ...
  for (auto const &yogaChild : yogaChildren) {
    ABI46_0_0React_native_assert(yogaChild->getContext());
    ABI46_0_0React_native_assert(yogaChild->getChildren().size() < 16384);
    if (!yogaChild->getChildren().empty()) {
      ABI46_0_0React_native_assert(!yogaChild->hasMeasureFunc());
    }
  }
#endif
}
...
void YogaLayoutableShadowNode::ensureYogaChildrenAlighment() const {
  ...
  if (getTraits().check(ShadowNodeTraits::Trait::LeafYogaNode)) {
    ABI46_0_0React_native_assert(yogaChildren.empty());
    return;
  }

  ABI46_0_0React_native_assert(yogaChildren.size() == children.size());
