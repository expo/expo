...
#include "ABI46_0_0RCTLegacyViewManagerInteropCoordinator.h"
#include <ABI46_0_0React/ABI46_0_0RCTBridge+Private.h>
#include <ABI46_0_0React/ABI46_0_0RCTBridgeMethod.h>
#include <ABI46_0_0React/ABI46_0_0RCTComponentData.h>
#include <ABI46_0_0React/ABI46_0_0RCTEventDispatcherProtocol.h>
#include <ABI46_0_0React/ABI46_0_0RCTFollyConvert.h>
#include <ABI46_0_0React/ABI46_0_0RCTModuleData.h>
#include <ABI46_0_0React/ABI46_0_0RCTModuleMethod.h>
#include <ABI46_0_0React/ABI46_0_0RCTUIManager.h>
#include <ABI46_0_0React/ABI46_0_0RCTUIManagerUtils.h>
#include <ABI46_0_0React/ABI46_0_0RCTUtils.h>
#include <folly/json.h>
#include <objc/runtime.h>

using namespace ABI46_0_0facebook::ABI46_0_0React;

@implementation ABI46_0_0RCTLegacyViewManagerInteropCoordinator {
  ABI46_0_0RCTComponentData *_componentData;
  __weak ABI46_0_0RCTBridge *_bridge;
  __weak ABI46_0_0RCTBridgeModuleDecorator *_bridgelessInteropData;
  /*
   Each instance of `ABI46_0_0RCTLegacyViewManagerInteropComponentView` registers a block to which events are dispatched.
  ...
  /*
   * In bridgeless mode, instead of using the bridge to look up ABI46_0_0RCTModuleData,
   * store that information locally.
   */
  NSMutableArray<id<ABI46_0_0RCTBridgeMethod>> *_moduleMethods;
  NSMutableDictionary<NSString *, id<ABI46_0_0RCTBridgeMethod>> *_moduleMethodsByName;
}
                ...
                bridgelessInteropData:(ABI46_0_0RCTBridgeModuleDecorator *)bridgelessInteropData;
{
  if (self = [super init]) {
    ...
    if (bridgelessInteropData) {
      //  During bridge mode, ABI46_0_0RCTBridgeModules will be decorated with these APIs by the bridge.
      ABI46_0_0RCTAssert(
          _bridge == nil,
          @"ABI46_0_0RCTLegacyViewManagerInteropCoordinator should not be intialized with ABI46_0_0RCTBridgeModuleDecorator in bridge mode.");
    }

    _eventInterceptors = [NSMutableDictionary new];

    __weak __typeof(self) weakSelf = self;
    _componentData.eventInterceptor = ^(NSString *eventName, NSDictionary *event, NSNumber *ABI46_0_0ReactTag) {
      __typeof(self) strongSelf = weakSelf;
      if (strongSelf) {
        InterceptorBlock block = [strongSelf->_eventInterceptors objectForKey:ABI46_0_0ReactTag];
        if (block) {
          block(std::string([ABI46_0_0RCTNormalizeInputEventName(eventName) UTF8String]), convertIdToFollyDynamic(event ?: @{}));
        }
      }
    };
  }
...
- (UIView *)createPaperViewWithTag:(NSInteger)tag;
{
  ...
  [_bridgelessInteropData attachInteropAPIsToModule:(id<ABI46_0_0RCTBridgeModule>)_componentData.bridgelessViewManager];
  return view;
}
...
{
  return ABI46_0_0RCTDropABI46_0_0ReactPrefixes(_componentData.name);
}
...
- (void)handleCommand:(NSString *)commandName args:(NSArray *)args ABI46_0_0ReactTag:(NSInteger)tag
{
  ...
  ABI46_0_0RCTModuleData *moduleData = [_bridge.batchedBridge moduleDataForName:ABI46_0_0RCTBridgeModuleNameForClass(managerClass)];
  ...
  if ([commandName isKindOfClass:[NSNumber class]]) {
    ...
    if (method == nil) {
      ABI46_0_0RCTLogError(@"No command found with name \"%@\"", commandName);
    }
  } else {
    ...
    [_bridge.batchedBridge
        dispatchBlock:^{
          ...
          [self->_bridge.uiManager setNeedsLayout];
                ...
                queue:ABI46_0_0RCTGetUIManagerQueue()];
  } else {
...
// This is copy-pasta from ABI46_0_0RCTModuleData.
    ...
    while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
      ...
      for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
          ...
          auto exportedMethod = ((const ABI46_0_0RCTMethodInfo *(*)(id, SEL))imp)(_componentData.managerClass, selector);
          id<ABI46_0_0RCTBridgeMethod> moduleMethod =
              [[ABI46_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod moduleClass:_componentData.managerClass];
