...
#include "ABI46_0_0LegacyViewManagerInteropComponentDescriptor.h"
#include <ABI46_0_0React/ABI46_0_0RCTBridge.h>
#include <ABI46_0_0React/ABI46_0_0RCTBridgeModuleDecorator.h>
#include <ABI46_0_0React/ABI46_0_0RCTComponentData.h>
#include <ABI46_0_0React/ABI46_0_0RCTEventDispatcher.h>
...
static std::string moduleNameFromComponentName(const std::string &componentName)
{
  ...
  if (componentName == "FDSTooltipView") {
    return "FBABI46_0_0ReactFDSTooltipViewManager";
  }
  ...
  std::string artPrefix("ABI46_0_0ART");
  if (std::mismatch(artPrefix.begin(), artPrefix.end(), componentName.begin()).first == artPrefix.end()) {
    return componentName + "Manager";
  }

  std::string rnPrefix("ABI46_0_0RN");
  if (std::mismatch(rnPrefix.begin(), rnPrefix.end(), componentName.begin()).first == rnPrefix.end()) {
    return componentName + "Manager";
  }

  return "ABI46_0_0RCT" + componentName + "Manager";
}

inline NSString *ABI46_0_0RCTNSStringFromString(const std::string &string)
{
    ...
    ComponentDescriptor::Flavor const &flavor)
{
  ...
  Class module = NSClassFromString(ABI46_0_0RCTNSStringFromString(moduleName));
  assert(module);
  ...
  auto optionalEventDispatcher = contextContainer->find<std::shared_ptr<void>>("ABI46_0_0RCTEventDispatcher");
  ABI46_0_0RCTEventDispatcher *eventDispatcher;
  if (optionalEventDispatcher) {
    eventDispatcher = unwrapManagedObject(optionalEventDispatcher.value());
  }

  auto optionalModuleDecorator = contextContainer->find<std::shared_ptr<void>>("ABI46_0_0RCTBridgeModuleDecorator");
  ...
  ABI46_0_0RCTComponentData *componentData = [[ABI46_0_0RCTComponentData alloc] initWithManagerClass:module
                                                                            bridge:bridge
                                                                   eventDispatcher:eventDispatcher];
  return wrapManagedObject([[ABI46_0_0RCTLegacyViewManagerInteropCoordinator alloc]
