...
TEST(AttributedStringTest, testToDynamic) {
  ...
  ABI46_0_0EXPECT_EQ(result["string"], fragment.string);
  auto textAttribute = result["fragments"][0]["textAttributes"];
  ABI46_0_0EXPECT_EQ(textAttribute["foregroundColor"], toDynamic(text.foregroundColor));
  ABI46_0_0EXPECT_EQ(textAttribute["opacity"], text.opacity);
  ABI46_0_0EXPECT_EQ(textAttribute["fontStyle"], toString(text.fontStyle.value()));
  ABI46_0_0EXPECT_EQ(textAttribute["fontWeight"], toString(text.fontWeight.value()));
}
