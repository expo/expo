...
#include "ABI46_0_0UITemplateProcessor.h"
...
SharedShadowNode UITemplateProcessor::runCommand(
    ...
    const ComponentDescriptorRegistry &componentDescriptorRegistry,
    const NativeModuleRegistry &nativeModuleRegistry,
    std::shared_ptr<const ABI46_0_0ReactNativeConfig> const &ABI46_0_0ReactNativeConfig) {
  ...
  if (opcode == "createNode") {
    ...
    registers[registerNumber] = ABI46_0_0ReactNativeConfig->getBool(param);
...
SharedShadowNode UITemplateProcessor::buildShadowTree(
  ...
  LOG(ERROR) << "ABI46_0_0React ui template missing returnRoot command :(";
