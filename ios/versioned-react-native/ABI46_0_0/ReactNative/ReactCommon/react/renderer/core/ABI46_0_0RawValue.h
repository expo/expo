 ...
 *
 * The main intention of the class is to abstract ABI46_0_0React props parsing infra from
 ...
 private:
  ...
  static bool checkValueType(
    ...
    for (const auto &item : dynamic.items()) {
      ABI46_0_0React_native_assert(item.first.isString());
  ...
  static std::vector<T> castValue(
      const folly::dynamic &dynamic,
      std::vector<T> *type) noexcept {
    ABI46_0_0React_native_assert(dynamic.isArray());
  ...
  static butter::map<std::string, T> castValue(
      const folly::dynamic &dynamic,
      butter::map<std::string, T> *type) noexcept {
    ABI46_0_0React_native_assert(dynamic.isObject());
