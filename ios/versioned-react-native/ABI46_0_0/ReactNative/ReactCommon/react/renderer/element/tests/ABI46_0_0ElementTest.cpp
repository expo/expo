...
TEST(ElementTest, testNormalCases) {
  ...
  ABI46_0_0EXPECT_EQ(shadowNode, shadowNodeA);

  // Tags
  ABI46_0_0EXPECT_EQ(shadowNodeA->getTag(), 1);
  ABI46_0_0EXPECT_EQ(shadowNodeAA->getTag(), 2);
  ABI46_0_0EXPECT_EQ(shadowNodeAB->getTag(), 3);
  ABI46_0_0EXPECT_EQ(shadowNodeABA->getTag(), 4);

  // Children
  ABI46_0_0EXPECT_EQ(shadowNodeA->getChildren().size(), 2);
  ABI46_0_0EXPECT_EQ(shadowNodeAA->getChildren().size(), 0);
  ABI46_0_0EXPECT_EQ(shadowNodeAB->getChildren().size(), 1);
  ABI46_0_0EXPECT_EQ(shadowNodeABA->getChildren().size(), 0);
  ABI46_0_0EXPECT_EQ(
  ...
  // Props
  ABI46_0_0EXPECT_EQ(shadowNodeA->getProps()->nativeId, "node A");
  ABI46_0_0EXPECT_EQ(shadowNodeABA->getProps()->nativeId, "node ABA");
  ABI46_0_0EXPECT_EQ(shadowNodeAA->getProps(), propsAA);

  // Finalize
  ABI46_0_0EXPECT_TRUE(shadowNodeA->getSealed());
  ABI46_0_0EXPECT_TRUE(shadowNodeAA->getSealed());
  ABI46_0_0EXPECT_TRUE(shadowNodeAB->getSealed());
  ABI46_0_0EXPECT_TRUE(shadowNodeABA->getSealed());
}
