...
class ContextContainer final {
 public:
   ...
   * example if the type `T` is `std::shared_ptr<const ABI46_0_0ReactNativeConfig>`,
   * then one would use `"ABI46_0_0ReactNativeConfig"` for the `key`, even if the
   * instance is actually a `shared_ptr` of derived class
   *`EmptyABI46_0_0ReactNativeConfig`.
   */
  template <typename T>
