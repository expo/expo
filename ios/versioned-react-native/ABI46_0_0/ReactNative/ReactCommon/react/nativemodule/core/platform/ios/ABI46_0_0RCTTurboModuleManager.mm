...
#import "ABI46_0_0RCTTurboModuleManager.h"
...
#import <ABI46_0_0ReactCommon/ABI46_0_0TurboCxxModule.h>
#import <ABI46_0_0ReactCommon/ABI46_0_0TurboModuleBinding.h>
#import <ABI46_0_0ReactCommon/ABI46_0_0TurboModulePerfLogger.h>
...
/**
 * A global variable whose address we use to associate method queues to id<ABI46_0_0RCTTurboModule> objects.
 */
...
class TurboModuleHolder {
 private:
  ...
  id<ABI46_0_0RCTTurboModule> module_;
 ...
 public:
  ...
  void setModule(id<ABI46_0_0RCTTurboModule> module)
  {
    module_ = module;
  }

  id<ABI46_0_0RCTTurboModule> getModule() const
  {
    return module_;
  }
...
class MethodQueueNativeCallInvoker : public CallInvoker {
 ...
 public:
  ...
  {
    if (methodQueue_ == ABI46_0_0RCTJSThread) {
      work();
      return;
    }
...
// Fallback lookup since ABI46_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI46_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI46_0_0RCTTurboModuleManager {
  std::shared_ptr<CallInvoker> _jsInvoker;
  __weak id<ABI46_0_0RCTTurboModuleManagerDelegate> _delegate;
  ...
  ABI46_0_0RCTRetainJSCallback _retainJSCallback;
  ...
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidInvalidateModules:)
                                                 name:ABI46_0_0RCTBridgeDidInvalidateModulesNotification
                                               object:_bridge.parentBridge];

    if (ABI46_0_0RCTGetTurboModuleCleanupMode() == kABI46_0_0RCTGlobalScope) {
      // Use LongLivedObjectCollection singleton regularly
      _retainJSCallback = nil;
    } else if (ABI46_0_0RCTGetTurboModuleCleanupMode() == kABI46_0_0RCTGlobalScopeUsingRetainJSCallback) {
    ...
    } else if (ABI46_0_0RCTGetTurboModuleCleanupMode() == kABI46_0_0RCTTurboModuleManagerScope) {
...
- (void)notifyAboutTurboModuleSetup:(const char *)name
{
  ...
  if (moduleName) {
    int64_t setupTime = [self->_bridge.performanceLogger durationForTag:ABI46_0_0RCTPLTurboModuleSetup];
...
- (std::shared_ptr<TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  id<ABI46_0_0RCTTurboModule> module = [self provideABI46_0_0RCTTurboModule:moduleName];

  TurboModulePerfLogger::moduleJSRequireEndingStart(moduleName);

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI46_0_0RCTTurboModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  if (methodQueue == nil) {
    ABI46_0_0RCTLogError(@"TurboModule \"%@\" was not associated with a method queue.", moduleClass);
  }
  ...
  /**
   * Have ABI46_0_0RCTCxxBridge decorate native CallInvoker, so that it's aware of TurboModule async method calls.
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI46_0_0RCTCxxModule.class]) {
    ...
    auto turboModule = std::make_shared<TurboCxxModule>([((ABI46_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
  ...
  ObjCTurboModule::InitParams params = {
      ...
      .isSyncModule = methodQueue == ABI46_0_0RCTJSThread,
  ...
  if (turboModule == nullptr) {
    ABI46_0_0RCTLogError(@"TurboModule \"%@\"'s getTurboModule: method returned nil.", moduleClass);
  }
  _turboModuleCache.insert({moduleName, turboModule});
  return turboModule;
}
...
{
  if (ABI46_0_0RCTTurboModuleSharedMutexInitEnabled()) {
    {
 ...
 * (for now).
 */
- (id<ABI46_0_0RCTTurboModule>)provideABI46_0_0RCTTurboModule:(const char *)moduleName
{
  if (strncmp("ABI46_0_0RCT", moduleName, 3) == 0) {
  ...
  id<ABI46_0_0RCTTurboModule> module = [self _provideABI46_0_0RCTTurboModule:moduleName moduleHolder:moduleHolder shouldPerfLog:YES];
...
- (id<ABI46_0_0RCTTurboModule>)_provideABI46_0_0RCTTurboModule:(const char *)moduleName
                               ...
                               shouldPerfLog:(BOOL)shouldPerfLog
{
  ...
  if (shouldCreateModule) {
    ...
    if ([_delegate respondsToSelector:@selector(getModuleClassFromName:)]) {
      if (ABI46_0_0RCTTurboModuleManagerDelegateLockingDisabled()) {
    ...
    __block id<ABI46_0_0RCTTurboModule> module = nil;

    if ([moduleClass conformsToProtocol:@protocol(ABI46_0_0RCTTurboModule)]) {
      ...
      dispatch_block_t work = ^{
        ...
        module = [strongSelf _createAndSetUpABI46_0_0RCTTurboModule:moduleClass
                                                moduleName:moduleName
      ...
      if ([self _requiresMainQueueSetup:moduleClass]) {
         ...
         */
        if (ABI46_0_0RCTTurboModuleEagerInitEnabled() && !ABI46_0_0RCTIsMainQueue()) {
          ABI46_0_0RCTLogWarn(
              @"TurboModule \"%@\" requires synchronous dispatch onto the main queue to be initialized. This may lead to deadlock.",
              moduleClass);
        }

        ABI46_0_0RCTUnsafeExecuteOnMainQueueSync(work);
      } else {
        work();
      }
    }
 ...
 *  - The thread that calls provideABI46_0_0RCTTurboModule:
 *  - The main thread (if the TurboModule requires main queue init), blocking the thread that calls
 * provideABI46_0_0RCTTurboModule:.
 */
- (id<ABI46_0_0RCTTurboModule>)_createAndSetUpABI46_0_0RCTTurboModule:(Class)moduleClass
                                         moduleName:(const char *)moduleName
                                           moduleId:(int32_t)moduleId
{
  id<ABI46_0_0RCTTurboModule> module = nil;
   ...
   * implementation, and a `- (ABI46_0_0RCTBridge *) bridge { ... }` method won't be
  ...
  if ([module respondsToSelector:@selector(bridge)] && _bridge) {
    ...
    @try {
      /**
       * ABI46_0_0RCTBridgeModule declares the bridge property as readonly.
       * Therefore, when authors of NativeModules synthesize the bridge
       * via @synthesize bridge = bridge;, the ObjC runtime generates
       * only a - (ABI46_0_0RCTBridge *) bridge: { ... } method. No setter is
  ...
  if (moduleHasMethodQueueGetter) {
    methodQueue = [(id<ABI46_0_0RCTBridgeModule>)module methodQueue];
  }

  /**
   * Note: ABI46_0_0RCTJSThread, which is a valid method queue, is defined as (id)kCFNull. It should rightfully not enter the
   * following if condition's block.
   */
  if (!methodQueue) {
    NSString *methodQueueName = [NSString stringWithFormat:@"com.facebook.ABI46_0_0React.%sQueue", moduleName];
  ...
  /**
   * If the TurboModule conforms to ABI46_0_0RCTInitializing, invoke its initialize method.
   */
  if ([module respondsToSelector:@selector(initialize)]) {
    [(id<ABI46_0_0RCTInitializing>)module initialize];
  }

  /**
   * Attach method queue to id<ABI46_0_0RCTTurboModule> object.
   * This is necessary because the id<ABI46_0_0RCTTurboModule> object can be eagerly created/initialized before the method
   * queue is required. The method queue is required for an id<ABI46_0_0RCTTurboModule> for JS -> Native calls. So, we need it
   * before we create the id<ABI46_0_0RCTTurboModule>'s TurboModule jsi::HostObject in provideTurboModule:.
   */
  objc_setAssociatedObject(module, &kAssociatedMethodQueueKey, methodQueue, OBJC_ASSOCIATION_RETAIN);

  /**
   * NativeModules that implement the ABI46_0_0RCTFrameUpdateObserver protocol
   * require registration with ABI46_0_0RCTDisplayLink.
   *
   * TODO(T55504345): Investigate whether we can improve this after TM
   * rollout.
   */
  if (_bridge) {
    ABI46_0_0RCTModuleData *data = [[ABI46_0_0RCTModuleData alloc] initWithModuleInstance:(id<ABI46_0_0RCTBridgeModule>)module
                                                                 bridge:_bridge
                                                         moduleRegistry:_bridge.moduleRegistry
                                                viewRegistry_DEPRECATED:nil
                                                          bundleManager:nil
                                                      callableJSModules:nil];
    [_bridge registerModuleForFrameUpdates:(id<ABI46_0_0RCTBridgeModule>)module withModuleData:data];
  }
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI46_0_0RCTDidInitializeModuleNotification
                    object:_bridge
                  userInfo:@{@"module" : module, @"bridge" : ABI46_0_0RCTNullIfNil([_bridge parentBridge])}];
  ...
  if (requiresMainQueueSetup) {
    ABI46_0_0RCTLogWarn(
        @"Module %@ requires main queue setup since it overrides `%s` but doesn't implement "
         "`requiresMainQueueSetup`. In a future release ABI46_0_0React Native will default to initializing all native modules "
...
- (void)installJSBindingWithRuntimeExecutor:(ABI46_0_0facebook::ABI46_0_0React::RuntimeExecutor)runtimeExecutor
{
   ...
   */
  auto turboModuleProvider = [self](const std::string &name) -> std::shared_ptr<ABI46_0_0React::TurboModule> {
    auto moduleName = name.c_str();

    TurboModulePerfLogger::moduleJSRequireBeginningStart(moduleName);
    auto moduleWasNotInitialized = ![self moduleIsInitialized:moduleName];
    if (moduleWasNotInitialized) {
      [self->_bridge.performanceLogger markStartForTag:ABI46_0_0RCTPLTurboModuleSetup];
    }
    ...
    if (moduleWasNotInitialized && [self moduleIsInitialized:moduleName]) {
      [self->_bridge.performanceLogger markStopForTag:ABI46_0_0RCTPLTurboModuleSetup];
  ...
  if (ABI46_0_0RCTGetTurboModuleCleanupMode() == kABI46_0_0RCTGlobalScope ||
      ABI46_0_0RCTGetTurboModuleCleanupMode() == kABI46_0_0RCTGlobalScopeUsingRetainJSCallback) {
    runtimeExecutor([turboModuleProvider = std::move(turboModuleProvider)](jsi::Runtime &runtime) {
      ABI46_0_0React::TurboModuleBinding::install(runtime, std::move(turboModuleProvider));
    });
  } else if (ABI46_0_0RCTGetTurboModuleCleanupMode() == kABI46_0_0RCTTurboModuleManagerScope) {
    runtimeExecutor([turboModuleProvider = std::move(turboModuleProvider),
      ...
      ABI46_0_0React::TurboModuleBinding::install(runtime, std::move(turboModuleProvider), longLivedObjectCollection);
    });
  }
}

#pragma mark ABI46_0_0RCTTurboModuleRegistry
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  if (warnOnLookupFailure && !module) {
    ABI46_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
{
  // Backward-compatibility: ABI46_0_0RCTInvalidating handling.
  ...
  for (auto &pair : _turboModuleHolders) {
    ...
    /**
     * We could start tearing down ABI46_0_0ReactNative before a TurboModule is fully initialized. In this case, we should wait
     * for TurboModule init to finish before calling invalidate on it. So, we call _provideABI46_0_0RCTTurboModule:moduleHolder,
     * because it's guaranteed to return a fully initialized NativeModule.
     */
    id<ABI46_0_0RCTTurboModule> module = [self _provideABI46_0_0RCTTurboModule:moduleName.c_str()
    ...
    if ([module respondsToSelector:@selector(invalidate)]) {
      ...
      dispatch_block_t invalidateModule = ^{
        [((id<ABI46_0_0RCTInvalidating>)module) invalidate];
      ...
      if (_bridge) {
        ...
        // Bridgeless mode
        if (methodQueue == ABI46_0_0RCTJSThread) {
          invalidateModule();
        } else {
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI46_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
