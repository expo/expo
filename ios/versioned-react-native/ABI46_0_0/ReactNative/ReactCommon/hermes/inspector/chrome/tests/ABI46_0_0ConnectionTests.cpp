...
#include "ABI46_0_0AsyncHermesRuntime.h"
...
#include <ABI46_0_0hermes/ABI46_0_0DebuggerAPI.h>
...
void expectCallFrames(
  ...
  ABI46_0_0EXPECT_EQ(frames.size(), infos.size());

  int i = 0;
  for (const FrameInfo &info : infos) {
    m::debugger::CallFrame frame = frames[i];

    ABI46_0_0EXPECT_EQ(frame.callFrameId, folly::to<std::string>(i));
    ABI46_0_0EXPECT_EQ(frame.functionName, info.functionName);
    ABI46_0_0EXPECT_GE(frame.location.lineNumber, info.lineNumberMin);
    ABI46_0_0EXPECT_LE(frame.location.lineNumber, info.lineNumberMax);

    if (info.columnNumber != debugger::kInvalidLocation) {
      ABI46_0_0EXPECT_EQ(frame.location.columnNumber, info.columnNumber);
    }

    if (info.scriptId.size() > 0) {
      ABI46_0_0EXPECT_EQ(frame.location.scriptId, info.scriptId);
    }

    // TODO: make expectation more specific once Hermes gives us something other
    // than kInvalidBreakpoint for the file id
    ABI46_0_0EXPECT_FALSE(frame.location.scriptId.empty());

    if (info.scopeCount > 0) {
      ABI46_0_0EXPECT_EQ(frame.scopeChain.size(), info.scopeCount);

      for (int j = 0; j < info.scopeCount; j++) {
        ABI46_0_0EXPECT_TRUE(frame.scopeChain[j].object.objectId.hasValue());
      }
    }
...
m::runtime::ExecutionContextCreatedNotification expectExecutionContextCreated(
  ...
  ABI46_0_0EXPECT_EQ(note.context.id, 1);
  ABI46_0_0EXPECT_EQ(note.context.origin, "");
  ABI46_0_0EXPECT_EQ(note.context.name, "hermes");
...
m::debugger::ScriptParsedNotification expectScriptParsed(
  ...
  ABI46_0_0EXPECT_EQ(note.url, url);
  ABI46_0_0EXPECT_GT(note.scriptId.size(), 0);

  if (sourceMapURL.empty()) {
    ABI46_0_0EXPECT_FALSE(note.sourceMapURL.hasValue());
  } else {
    ABI46_0_0EXPECT_EQ(note.sourceMapURL.value(), sourceMapURL);
  }
...
m::debugger::PausedNotification expectPaused(
  ...
  ABI46_0_0EXPECT_EQ(note.reason, reason);
...
m::debugger::BreakpointId expectBreakpointResponse(
  ...
  ABI46_0_0EXPECT_EQ(resp.id, id);
  ABI46_0_0EXPECT_FALSE(resp.breakpointId.empty());
  ABI46_0_0EXPECT_NE(
      resp.breakpointId,
      folly::to<std::string>(ABI46_0_0facebook::ABI46_0_0hermes::debugger::kInvalidBreakpoint));

  if (line == -1) {
    ABI46_0_0EXPECT_EQ(resp.locations.size(), 0);
  } else {
    ABI46_0_0EXPECT_EQ(resp.locations.size(), 1);
    ABI46_0_0EXPECT_EQ(resp.locations[0].lineNumber, resolvedLine);
  }
...
void expectEvalResponse(
  ...
  ABI46_0_0EXPECT_EQ(resp.id, id);
  ABI46_0_0EXPECT_EQ(resp.result.type, "string");
  ABI46_0_0EXPECT_EQ(resp.result.value, expectedValue);
  ABI46_0_0EXPECT_FALSE(resp.exceptionDetails.hasValue());
}
...
void expectEvalResponse(SyncConnection &conn, int id, bool expectedValue) {
  ...
  ABI46_0_0EXPECT_EQ(resp.id, id);
  ABI46_0_0EXPECT_EQ(resp.result.type, "boolean");
  ABI46_0_0EXPECT_EQ(resp.result.value, expectedValue);
  ABI46_0_0EXPECT_FALSE(resp.exceptionDetails.hasValue());
}
...
void expectEvalResponse(SyncConnection &conn, int id, int expectedValue) {
  ...
  ABI46_0_0EXPECT_EQ(resp.id, id);
  ABI46_0_0EXPECT_EQ(resp.result.type, "number");
  ABI46_0_0EXPECT_EQ(resp.result.value, expectedValue);
  ABI46_0_0EXPECT_FALSE(resp.exceptionDetails.hasValue());
}
...
void expectEvalException(
  ...
  ABI46_0_0EXPECT_EQ(resp.id, id);
  ABI46_0_0EXPECT_TRUE(resp.exceptionDetails.hasValue());

  m::runtime::ExceptionDetails &details = resp.exceptionDetails.value();
  ABI46_0_0EXPECT_EQ(details.text, exceptionText);

  // TODO: Hermes doesn't seem to populate the line number for the exception?
  ABI46_0_0EXPECT_EQ(details.lineNumber, 0);

  ABI46_0_0EXPECT_TRUE(details.stackTrace.hasValue());

  m::runtime::StackTrace &stackTrace = details.stackTrace.value();
  ABI46_0_0EXPECT_EQ(stackTrace.callFrames.size(), infos.size());

  int i = 0;
  for (const FrameInfo &info : infos) {
    const m::runtime::CallFrame &callFrame = stackTrace.callFrames[i];

    ABI46_0_0EXPECT_GE(callFrame.lineNumber, info.lineNumberMin);
    ABI46_0_0EXPECT_LE(callFrame.lineNumber, info.lineNumberMax);
    ABI46_0_0EXPECT_EQ(callFrame.functionName, info.functionName);
...
std::unordered_map<std::string, std::string> expectProps(
  ...
  ABI46_0_0EXPECT_EQ(resp.result.size(), infos.size());

  for (int i = 0; i < resp.result.size(); i++) {
    m::runtime::PropertyDescriptor &desc = resp.result[i];

    auto infoIt = infos.find(desc.name);
    ABI46_0_0EXPECT_FALSE(infoIt == infos.end()) << desc.name;

    if (infoIt != infos.end()) {
      const PropInfo &info = infoIt->second;

      ABI46_0_0EXPECT_TRUE(desc.value.hasValue());

      m::runtime::RemoteObject &remoteObj = desc.value.value();
      ABI46_0_0EXPECT_EQ(remoteObj.type, info.type);

      if (info.subtype.hasValue()) {
        ABI46_0_0EXPECT_TRUE(remoteObj.subtype.hasValue());
        ABI46_0_0EXPECT_EQ(remoteObj.subtype.value(), info.subtype.value());
      }

      if (info.value.hasValue()) {
        ABI46_0_0EXPECT_TRUE(remoteObj.value.hasValue());
        ABI46_0_0EXPECT_EQ(remoteObj.value.value(), info.value.value());
      }

      if (info.unserializableValue.hasValue()) {
        ABI46_0_0EXPECT_TRUE(remoteObj.unserializableValue.hasValue());
        ABI46_0_0EXPECT_EQ(
      ...
      if ((info.type == "object" && info.subtype != "null") ||
          info.type == "function") {
        ABI46_0_0EXPECT_TRUE(remoteObj.objectId.hasValue());
...
void expectEvalResponse(
  ...
  ABI46_0_0EXPECT_EQ(resp.id, id);
  ABI46_0_0EXPECT_EQ(resp.result.type, "object");
  ABI46_0_0EXPECT_FALSE(resp.exceptionDetails.hasValue());

  ABI46_0_0EXPECT_TRUE(resp.result.objectId.hasValue());
...
TEST(ConnectionTests, testSetBreakpointById) {
  ...
  ABI46_0_0EXPECT_EQ(resp.actualLocation.scriptId, script.scriptId);
  ABI46_0_0EXPECT_EQ(resp.actualLocation.lineNumber, 2);
  ABI46_0_0EXPECT_EQ(resp.actualLocation.columnNumber.value(), 4);
...
TEST(ConnectionTests, testRemoveBreakpoint) {
  ...
  ABI46_0_0EXPECT_EQ(finalValue.asNumber(), 4);
}
...
TEST(ConnectionTests, testRuntimeEvaluateReturnByValue) {
  ...
  ABI46_0_0EXPECT_EQ(folly::toJson(resp.result.value.value()), object);
...
TEST(ConnectionTests, testGetProperties) {
  ...
  ABI46_0_0EXPECT_TRUE(scopeObj.objectId.hasValue());
  ...
  ABI46_0_0EXPECT_EQ(scopeChildren.size(), 3);

  ABI46_0_0EXPECT_EQ(scopeChildren.count("obj"), 1);
  ...
  auto objChildren = expectProps(
      ...
      {{"depth", PropInfo("number").setValue(0)},
       ...
       {"__proto__", PropInfo("object")}});
  ABI46_0_0EXPECT_EQ(objChildren.size(), 2);

  ABI46_0_0EXPECT_EQ(objChildren.count("value"), 1);
  ...
  auto valueChildren = expectProps(
      ...
      {{"a", PropInfo("number").setUnserializableValue("-Infinity")},
       ...
       {"__proto__", PropInfo("object")}});
  ABI46_0_0EXPECT_EQ(valueChildren.size(), 1);
...
TEST(ConnectionTests, testSetPauseOnExceptionsAll) {
  ...
  expectNotification<m::debugger::ResumedNotification>(conn);
  asyncRuntime.wait();
  ABI46_0_0EXPECT_EQ(asyncRuntime.getNumberOfExceptions(), 1);
  ABI46_0_0EXPECT_EQ(asyncRuntime.getLastThrownExceptionMessage(), "Uncaught exception");
}
...
TEST(ConnectionTests, testScopeVariables) {
  ...
  ABI46_0_0EXPECT_EQ(scopeChain.size(), 2);

  // [2] inspect local scope
  ABI46_0_0EXPECT_EQ(scopeChain.at(0).type, "local");
  ...
  ABI46_0_0EXPECT_EQ(scopeChain.at(1).type, "global");
  ...
  ABI46_0_0EXPECT_GE(globalProperties.size(), 3);

  // globalString should be of type "string" and have value "global-string".
  ABI46_0_0EXPECT_EQ(globalProperties.count("globalString"), 1);
  ABI46_0_0EXPECT_TRUE(globalProperties["globalString"].hasValue());
  ABI46_0_0EXPECT_EQ(globalProperties["globalString"].value().type, "string");
  ABI46_0_0EXPECT_EQ(
      globalProperties["globalString"].value().value.value(), "global-string");

  // func should be of type "function".
  ABI46_0_0EXPECT_EQ(globalProperties.count("func"), 1);
  ABI46_0_0EXPECT_TRUE(globalProperties["func"].hasValue());
  ABI46_0_0EXPECT_EQ(globalProperties["func"].value().type, "function");

  // globalObject should be of type "object" with "number" and "bool"
  // properties.
  ABI46_0_0EXPECT_EQ(globalProperties.count("globalObject"), 1);
  ABI46_0_0EXPECT_TRUE(globalProperties["globalObject"].hasValue());
  ABI46_0_0EXPECT_EQ(globalProperties["globalObject"].value().type, "object");
  expectProps(
      conn,
      msgId++,
...
TEST(ConnectionTests, testRuntimeCallFunctionOnObject) {
  ...
  auto addMember = [&](const m::runtime::RemoteObjectId id,
    ...
    ABI46_0_0EXPECT_TRUE(allowRedefinition || it.second)
  ...
  auto verifyObjShape = [&](const m::runtime::RemoteObjectId &objId)
    ...
    ABI46_0_0EXPECT_TRUE(objProps.count("__proto__"));
    auto objPropIt = objProps.find("self_ref");
    if (objPropIt == objProps.end()) {
      ABI46_0_0EXPECT_TRUE(false) << "missing \"self_ref\" property.";
      return {};
    }
...
TEST(ConnectionTests, testRuntimeCallFunctionOnExecutionContext) {
  ...
  // sanity-check that our test fields don't exist in global this.
  {
    ...
    for (const auto &test : tests) {
      ABI46_0_0EXPECT_EQ(currProps.count(test.propName), 0) << test.propName;
    }
  }
...

  {
    ...
    for (const auto &test : tests) {
      ...
      if (it->second->value.hasValue()) {
        ...
        ABI46_0_0EXPECT_EQ(*actual, *expected) << test.propName;
...
TEST(ConnectionTests, testConsoleLog) {
  ...
  for (size_t i = 0; i < 2; ++i) {
    conn.waitForNotification([&receivedConsoleNotification,
      ...
      if (method == "Runtime.consoleAPICalled") {
        ...
        ABI46_0_0EXPECT_EQ(note.type, "warning");
        ABI46_0_0EXPECT_EQ(note.args.size(), 3);

        ABI46_0_0EXPECT_EQ(note.args[0].type, "string");
        ABI46_0_0EXPECT_EQ(note.args[0].value, "string value");

        ABI46_0_0EXPECT_EQ(note.args[1].type, "object");
        expectProps(
            conn,
            msgId++,
        ...
        ABI46_0_0EXPECT_EQ(note.args[2].type, "object");
        expectProps(
            conn,
            msgId++,
        ...
        ABI46_0_0EXPECT_EQ(note.reason, "other");
        ABI46_0_0EXPECT_EQ(note.callFrames.size(), 1);
        ABI46_0_0EXPECT_EQ(note.callFrames[0].functionName, "global");
        ABI46_0_0EXPECT_EQ(note.callFrames[0].location.lineNumber, 5);
      } else {
  ...
  ABI46_0_0EXPECT_TRUE(receivedConsoleNotification);
  ABI46_0_0EXPECT_TRUE(receivedPausedNotification);
...
TEST(ConnectionTests, testThisObject) {
  ...
  // globalString should be of type "string" and have value "global-string".
  ABI46_0_0EXPECT_EQ(properties.count("globalString"), 1);
  ABI46_0_0EXPECT_TRUE(properties["globalString"].hasValue());
  ABI46_0_0EXPECT_EQ(properties["globalString"].value().type, "string");
  ABI46_0_0EXPECT_EQ(properties["globalString"].value().value.value(), "global-string");
...
TEST(ConnectionTests, canBreakOnScriptsWithSourceMap) {
  ...
  ABI46_0_0EXPECT_EQ(note.hitBreakpoints->at(0), bpId);

  // Continue and verify that the JS code has now executed
  send<m::debugger::ResumeRequest>(conn, msgId++);
  expectNotification<m::debugger::ResumedNotification>(conn);
  expectNotification<m::debugger::PausedNotification>(conn);
  ABI46_0_0EXPECT_EQ(asyncRuntime.awaitStoredValue().asNumber(), 42);
...
TEST(ConnectionTests, runIfWaitingForDebugger) {
  ...
  expectNotification<m::debugger::PausedNotification>(conn);
  ABI46_0_0EXPECT_EQ(1, asyncRuntime.awaitStoredValue().asNumber());
...
TEST(ConnectionTests, heapProfilerSampling) {
  ...
  ABI46_0_0EXPECT_NE(resp.profile.samples.size(), 0);
...
TEST(ConnectionTests, heapSnapshotRemoteObject) {
  ...
  auto testObject = [&msgId, &conn](
    ...
    ABI46_0_0EXPECT_EQ(resp.result.type, type);
    ABI46_0_0EXPECT_EQ(resp.result.className, className);
    ABI46_0_0EXPECT_EQ(resp.result.description, description);
    if (subtype) {
      ABI46_0_0EXPECT_EQ(resp.result.subtype, subtype);
    }
    ...
    ABI46_0_0EXPECT_EQ(atoi(idResp.heapSnapshotObjectId.c_str()), objID);
  };
...
TEST(ConnectionTests, testBasicProfilerOperation) {
  ...
  // Being re-attached to the VM, send the stop sampling profile request.
  {
    ...
    ABI46_0_0EXPECT_GT(profile.nodes.size(), 0);
    ABI46_0_0EXPECT_LT(profile.startTime, profile.endTime);
    ASSERT_TRUE(profile.samples);
    ABI46_0_0EXPECT_FALSE(profile.samples->empty());
    ASSERT_TRUE(profile.timeDeltas);
    ABI46_0_0EXPECT_EQ(profile.samples->size(), profile.timeDeltas->size());
  }
