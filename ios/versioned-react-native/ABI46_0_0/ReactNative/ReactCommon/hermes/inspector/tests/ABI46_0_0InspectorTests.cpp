...
#include <ABI46_0_0hermes/ABI46_0_0inspector/Inspector.h>
...
TEST(InspectorTests, testStepOver) {
  ...
  OnPauseFunction onPauseFunc = [&futures](
    ...
    switch (pauseCount) {
      case 1: {
        ...
        ABI46_0_0EXPECT_EQ(getCurrentLine(state), 3);
      ...
      case 2: {
        ABI46_0_0EXPECT_EQ(state.getPauseReason(), debugger::PauseReason::StepFinish);
        ABI46_0_0EXPECT_EQ(getCurrentLine(state), 4);
      ...
      case 3: {
        ABI46_0_0EXPECT_EQ(state.getPauseReason(), debugger::PauseReason::StepFinish);
        ABI46_0_0EXPECT_EQ(getCurrentLine(state), 5);
  ...
  ABI46_0_0EXPECT_EQ(observer.getPauseCount(), 3);
}
...
TEST(InspectorTests, testSetBreakpoint) {
  ...
  OnPauseFunction onPauseFunc = [&futures](
    ...
    switch (pauseCount) {
      ...
      case 2: {
        ...
        futures.emplace_back(std::move(breakpointFuture)
                                 .thenValue([](debugger::BreakpointInfo info) {
                                   ABI46_0_0EXPECT_EQ(info.resolvedLocation.line, 6);
                                 }));
      ...
      case 3: {
        ABI46_0_0EXPECT_EQ(state.getPauseReason(), debugger::PauseReason::Breakpoint);
        ABI46_0_0EXPECT_EQ(getCurrentLine(state), 6);
...
TEST(InspectorTests, testAsyncSetBreakpoint) {
  ...
  auto breakpointPromise = context->inspector.setBreakpoint(locationForLine(4))
                               .thenValue([](debugger::BreakpointInfo info) {
                                 ABI46_0_0EXPECT_EQ(info.resolvedLocation.line, 4);
                               });
  ...
  ABI46_0_0EXPECT_EQ(observer.getPauseCount(), 2);
}
    ...
    switch (pauseCount) {
      ...
      case 2: {
        ...
        futures.emplace_back(inspector.setBreakpoint(locationForLine(7))
                                 .thenValue([](debugger::BreakpointInfo info) {
                                   ABI46_0_0EXPECT_EQ(info.resolvedLocation.line, 7);
                                 }));
