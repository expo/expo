...
@interface ABI46_0_0RCTSurfaceHostingView ()
...
@implementation ABI46_0_0RCTSurfaceHostingView {
...
{
  return [[ABI46_0_0RCTSurface alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProperties];
}
...
{
  id<ABI46_0_0RCTSurfaceProtocol> surface = [[self class] createSurfaceWithBridge:bridge
                ...
                sizeMeasureMode:(ABI46_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    // For backward compatibility with ABI46_0_0RCTRootView, set a color here instead of transparent (OS default).
...
- (void)setFrame:(CGRect)frame
{
  ...
  ABI46_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(
...
{
  if (ABI46_0_0RCTSurfaceStageIsPreparing(_stage)) {
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  ABI46_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(size, _sizeMeasureMode, &minimumSize, &maximumSize);
...
- (void)setStage:(ABI46_0_0RCTSurfaceStage)stage
{
  ...
  BOOL shouldInvalidateLayout = ABI46_0_0RCTSurfaceStageIsRunning(stage) != ABI46_0_0RCTSurfaceStageIsRunning(_stage) ||
      ABI46_0_0RCTSurfaceStageIsPreparing(stage) != ABI46_0_0RCTSurfaceStageIsPreparing(_stage);
...
- (void)setSizeMeasureMode:(ABI46_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
...
- (void)setActivityIndicatorViewFactory:(ABI46_0_0RCTSurfaceHostingViewActivityIndicatorViewFactory)activityIndicatorViewFactory
{
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection
{
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI46_0_0RCTUserInterfaceStyleDidChangeNotification
                    object:self
                  userInfo:@{
                    ABI46_0_0RCTUserInterfaceStyleDidChangeNotificationTraitCollectionKey : self.traitCollection,
                  }];
}
...
- (void)_updateViews
{
  self.isSurfaceViewVisible = ABI46_0_0RCTSurfaceStageIsRunning(_stage);
  self.isActivityIndicatorViewVisible = ABI46_0_0RCTSurfaceStageIsPreparing(_stage);
}
...
- (void)surface:(__unused ABI46_0_0RCTSurface *)surface didChangeStage:(ABI46_0_0RCTSurfaceStage)stage
{
...
- (void)surface:(__unused ABI46_0_0RCTSurface *)surface didChangeIntrinsicSize:(__unused CGSize)intrinsicSize
{
  ABI46_0_0RCTExecuteOnMainQueue(^{
    [self _invalidateLayout];
  });
