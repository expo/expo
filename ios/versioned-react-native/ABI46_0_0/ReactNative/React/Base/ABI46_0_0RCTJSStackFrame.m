 ...
 */
static NSRegularExpression *ABI46_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI46_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI46_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI46_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI46_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI46_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI46_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI46_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI46_0_0RCTNilIfNull(dict[@"collapse"]) boolValue]];
}
...
+ (NSArray<ABI46_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI46_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI46_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI46_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
