...
#import "ABI46_0_0RCTBridgeModuleDecorator.h"
#import "ABI46_0_0RCTInitializing.h"
...
void ABI46_0_0RCTSetIsMainQueueExecutionOfConstantsToExportDisabled(BOOL val)
{
  isMainQueueExecutionOfConstantToExportDisabled = val;
}

BOOL ABI46_0_0RCTIsMainQueueExecutionOfConstantsToExportDisabled()
{
  return isMainQueueExecutionOfConstantToExportDisabled;
}

@implementation ABI46_0_0RCTModuleData {
  NSDictionary<NSString *, id> *_constantsToExport;
  NSString *_queueName;
  __weak ABI46_0_0RCTBridge *_bridge;
  ABI46_0_0RCTBridgeModuleProvider _moduleProvider;
...
- (instancetype)initWithModuleClass:(Class)moduleClass
            ...
            viewRegistry_DEPRECATED:(ABI46_0_0RCTViewRegistry *)viewRegistry_DEPRECATED
                  ...
                  callableJSModules:(ABI46_0_0RCTCallableJSModules *)callableJSModules
{
  return [self initWithModuleClass:moduleClass
                    moduleProvider:^id<ABI46_0_0RCTBridgeModule> {
...
- (instancetype)initWithModuleClass:(Class)moduleClass
                     moduleProvider:(ABI46_0_0RCTBridgeModuleProvider)moduleProvider
                             bridge:(ABI46_0_0RCTBridge *)bridge
                     moduleRegistry:(ABI46_0_0RCTModuleRegistry *)moduleRegistry
            viewRegistry_DEPRECATED:(ABI46_0_0RCTViewRegistry *)viewRegistry_DEPRECATED
                      bundleManager:(ABI46_0_0RCTBundleManager *)bundleManager
...
- (instancetype)initWithModuleInstance:(id<ABI46_0_0RCTBridgeModule>)instance
                                bridge:(ABI46_0_0RCTBridge *)bridge
                        moduleRegistry:(ABI46_0_0RCTModuleRegistry *)moduleRegistry
               viewRegistry_DEPRECATED:(ABI46_0_0RCTViewRegistry *)viewRegistry_DEPRECATED
                         bundleManager:(ABI46_0_0RCTBundleManager *)bundleManager
...
- (void)setUpInstanceAndBridge:(int32_t)requestId
{
  ...
  ABI46_0_0RCT_PROFILE_BEGIN_EVENT(
      ...
      @"[ABI46_0_0RCTModuleData setUpInstanceAndBridge]",
      @{@"moduleClass" : NSStringFromClass(_moduleClass)});
  {
    ...
    if (shouldSetup) {
      if (!_instance) {
        if (ABI46_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI46_0_0RCTAssertMainQueue();
        }
        ABI46_0_0RCT_PROFILE_BEGIN_EVENT(ABI46_0_0RCTProfileTagAlways, @"[ABI46_0_0RCTModuleData setUpInstanceAndBridge] Create module", nil);
      ...
      if (_instance && ABI46_0_0RCTProfileIsProfiling()) {
        ABI46_0_0RCTProfileHookInstance(_instance);
      }
    }
    ...
    if (shouldSetup) {
      ...
      ABI46_0_0RCTBridgeModuleDecorator *moduleDecorator =
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI46_0_0RCT_PROFILE_BEGIN_EVENT(ABI46_0_0RCTProfileTagAlways, @"[ABI46_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
...
- (void)_initializeModule
{
  if (!_isInitialized && [_instance respondsToSelector:@selector(initialize)]) {
    _isInitialized = YES;
    [(id<ABI46_0_0RCTInitializing>)_instance initialize];
  }
}
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI46_0_0RCT_PROFILE_BEGIN_EVENT(ABI46_0_0RCTProfileTagAlways, @"[ABI46_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:ABI46_0_0RCTDidInitializeModuleNotification
                      object:_bridge
                    userInfo:@{@"module" : _instance, @"bridge" : ABI46_0_0RCTNullIfNil(_bridge.parentBridge)}];
    ABI46_0_0RCT_PROFILE_END_EVENT(ABI46_0_0RCTProfileTagAlways, @"");
  }
}
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI46_0_0RCT_PROFILE_BEGIN_EVENT(ABI46_0_0RCTProfileTagAlways, @"[ABI46_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ABI46_0_0React.%@Queue", self.name];
...
- (void)calculateMethods
{
  ...
  NSMutableArray<id<ABI46_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
  NSMutableDictionary<NSString *, id<ABI46_0_0RCTBridgeMethod>> *moduleMethodsByName = [NSMutableDictionary new];
  ...
  while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
      ...
      if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
        ...
        auto exportedMethod = ((const ABI46_0_0RCTMethodInfo *(*)(id, SEL))imp)(_moduleClass, selector);
        id<ABI46_0_0RCTBridgeMethod> moduleMethod = [[ABI46_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod
...
- (id<ABI46_0_0RCTBridgeModule>)instance
{
  NSString *moduleName = [self name];
  ...
  if (!_setupComplete) {
    ABI46_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI46_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI46_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI46_0_0RCT_PROFILE_BEGIN_EVENT(ABI46_0_0RCTProfileTagAlways, @"[ABI46_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI46_0_0RCTIsMainQueue()) {
        ABI46_0_0RCTLogWarn(@"ABI46_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI46_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI46_0_0RCTBridgeMethod>> *)methods
{
  [self calculateMethods];
  return _methods;
}

- (NSDictionary<NSString *, id<ABI46_0_0RCTBridgeMethod>> *)methodsByName
{
  [self calculateMethods];
...
- (void)gatherConstantsAndSignalJSRequireEnding:(BOOL)startMarkers
{
  ...
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI46_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI46_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI46_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    ...
    if (startMarkers) {
       ...
       *  - This delegates to ABI46_0_0RCTNativeModule::getConstants(), which calls ABI46_0_0RCTModuleData gatherConstants().
    ...
    if (!ABI46_0_0RCTIsMainQueueExecutionOfConstantsToExportDisabled() && _requiresMainQueueSetup) {
      if (!ABI46_0_0RCTIsMainQueue()) {
        ABI46_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
      }
...
- (dispatch_queue_t)methodQueue
{
  if (_bridge.valid) {
    ...
    ABI46_0_0RCTAssert(_methodQueue != nullptr, @"Module %@ has no methodQueue (instance: %@)", self, instance);
  }
  return _methodQueue;
}
