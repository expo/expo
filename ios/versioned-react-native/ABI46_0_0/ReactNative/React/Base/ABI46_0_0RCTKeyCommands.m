...
@interface ABI46_0_0RCTKeyCommand : NSObject <NSCopying>
...
@implementation ABI46_0_0RCTKeyCommand
...
- (BOOL)isEqual:(ABI46_0_0RCTKeyCommand *)object
{
  if (![object isKindOfClass:[ABI46_0_0RCTKeyCommand class]]) {
    return NO;
  }
...
@interface ABI46_0_0RCTKeyCommands ()

@property (nonatomic, strong) NSMutableSet<ABI46_0_0RCTKeyCommand *> *commands;

@end

@implementation ABI46_0_0RCTKeyCommands
...
- (void)handleKeyUIEventSwizzle:(UIEvent *)event
{
  ...
  if (isKeyDown && modifiedInput.length > 0 && interactionEnabled) {
    ...
    if (!firstResponder) {
      [self ABI46_0_0RCT_handleKeyCommand:modifiedInput flags:modifierFlags];
    }
  }
};
...
- (void)ABI46_0_0RCT_handleKeyCommand:(NSString *)input flags:(UIKeyModifierFlags)modifierFlags
{
  for (ABI46_0_0RCTKeyCommand *command in [ABI46_0_0RCTKeyCommands sharedInstance].commands) {
...
{
  static ABI46_0_0RCTKeyCommands *sharedInstance;
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  ABI46_0_0RCTKeyCommand *keyCommand = [[ABI46_0_0RCTKeyCommand alloc] init:input flags:flags block:block];
...
- (void)unregisterKeyCommandWithInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI46_0_0RCTKeyCommand *command in _commands.allObjects) {
...
- (BOOL)isKeyCommandRegisteredForInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI46_0_0RCTKeyCommand *command in _commands) {
    if ([command matchesInput:input flags:flags]) {
      return YES;
    }
  }
