...
#import "ABI46_0_0RCTComponentEvent.h"
...
typedef void (^ABI46_0_0RCTPropBlock)(id<ABI46_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI46_0_0RCTPropBlock> ABI46_0_0RCTPropBlockDictionary;
...
static SEL selectorForType(NSString *type)
{
  ...
  return NSSelectorFromString([ABI46_0_0RCTParseType(&input) stringByAppendingString:@":"]);
}

@implementation ABI46_0_0RCTComponentData {
  id<ABI46_0_0RCTComponent> _defaultView; // Only needed for ABI46_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI46_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI46_0_0RCTPropBlockDictionary *_shadowPropBlocks;
...
- (instancetype)initWithManagerClass:(Class)managerClass
                     ...
                     eventDispatcher:(id<ABI46_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  if ((self = [super init])) {
    _bridge = bridge;
...
- (ABI46_0_0RCTViewManager *)manager
{
  if (!_manager && _bridge) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI46_0_0RCTDidInitializeModuleNotification
                                                        object:nil
...
{
  ABI46_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI46_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI46_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI46_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI46_0_0RCTPropBlock createEventSetter(
    NSString *propName,
    SEL setter,
    InterceptorBlock eventInterceptor,
    id<ABI46_0_0RCTEventDispatcherProtocol> eventDispatcher)
{
  __weak id<ABI46_0_0RCTEventDispatcherProtocol> weakEventDispatcher = eventDispatcher;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI46_0_0RCTConvert BOOL:json]) {
      __weak id<ABI46_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI46_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI46_0_0ReactTag);
        } else {
          ABI46_0_0RCTComponentEvent *componentEvent = [[ABI46_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI46_0_0ReactTag
                                                                                 body:event];
...
static ABI46_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
  NSInvocation *typeInvocation = [NSInvocation invocationWithMethodSignature:typeSignature];
  typeInvocation.selector = type;
  typeInvocation.target = [ABI46_0_0RCTConvert class];
...
- (ABI46_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI46_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI46_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI46_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI46_0_0RCTDirectEventBlock:") ||
        type == NSSelectorFromString(@"ABI46_0_0RCTCapturingEventBlock:")) {
      // Special case for event handlers
      setterBlock =
          createEventSetter(name, setter, self.eventInterceptor, _bridge ? _bridge.eventDispatcher : _eventDispatcher);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI46_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI46_0_0RCTLogError(@"No +[ABI46_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI46_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI46_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI46_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI46_0_0RCT_CASE(_C_SEL, SEL)
        ABI46_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI46_0_0RCT_CASE(_C_CHR, char)
        ABI46_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI46_0_0RCT_CASE(_C_SHT, short)
        ABI46_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI46_0_0RCT_CASE(_C_INT, int)
        ABI46_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI46_0_0RCT_CASE(_C_LNG, long)
        ABI46_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI46_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI46_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI46_0_0RCT_CASE(_C_FLT, float)
        ABI46_0_0RCT_CASE(_C_DBL, double)
        ABI46_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI46_0_0RCT_CASE(_C_PTR, void *)
        ABI46_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI46_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI46_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI46_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI46_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI46_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI46_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI46_0_0ReactTag];
      ABI46_0_0RCTPerformBlockWithLogPrefix(
          ^{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI46_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI46_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI46_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
    ...
    if ([type isEqualToString:@"ABI46_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI46_0_0RCTNormalizeInputEventName(name)];

      // TODO(109509380): Remove this gating
      if (!ABI46_0_0RCTViewConfigEventValidAttributesDisabled()) {
        propTypes[name] = @"BOOL";
      }
    } else if ([type isEqualToString:@"ABI46_0_0RCTCapturingEventBlock"]) {
      [capturingEvents addObject:ABI46_0_0RCTNormalizeInputEventName(name)];

      // TODO(109509380): Remove this gating
      if (!ABI46_0_0RCTViewConfigEventValidAttributesDisabled()) {
        propTypes[name] = @"BOOL";
      }
    } else if ([type isEqualToString:@"ABI46_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI46_0_0RCTNormalizeInputEventName(name)];
...
{
  // Hackety hack, this partially re-implements ABI46_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI46_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI46_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI46_0_0EX_REMOVE_VERSION(name);
  ...
  ABI46_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
