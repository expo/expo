...
#import <ABI46_0_0React/ABI46_0_0RCTMockDef.h>

#import "ABI46_0_0RCTAutoInsetsProtocol.h"
#import "ABI46_0_0RCTBorderDrawing.h"
#import "ABI46_0_0RCTI18nUtil.h"
#import "ABI46_0_0RCTLog.h"
#import "ABI46_0_0RCTViewUtils.h"
#import "ABI46_0_0UIView+React.h"

ABI46_0_0RCT_MOCK_DEF(ABI46_0_0RCTView, ABI46_0_0RCTContentInsets);
#define ABI46_0_0RCTContentInsets ABI46_0_0RCT_MOCK_USE(ABI46_0_0RCTView, ABI46_0_0RCTContentInsets)

UIAccessibilityTraits const ABI46_0_0SwitchAccessibilityTrait = 0x20000000000001;

@implementation UIView (ABI46_0_0RCTViewUnmounting)
...
- (void)ABI46_0_0React_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI46_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI46_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI46_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ABI46_0_0React_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isABI46_0_0ReactRootView]) {
      break;
    }
...
static NSString *ABI46_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI46_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI46_0_0RCTView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI46_0_0RCTBorderStyleSolid;
...
ABI46_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : unused)

- (void)setABI46_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ABI46_0_0ReactLayoutDirection != layoutDirection) {
    _ABI46_0_0ReactLayoutDirection = layoutDirection;
...
- (void)setPointerEvents:(ABI46_0_0RCTPointerEvents)pointerEvents
{
  ...
  self.userInteractionEnabled = (pointerEvents != ABI46_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI46_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI46_0_0RCTPointerEventsNone || _pointerEvents == ABI46_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ABI46_0_0ReactZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI46_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI46_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI46_0_0RCTLogError(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI46_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  ...
  if (actionObject) {
    _onAccessibilityAction(@{@"actionName" : actionObject[@"name"], @"actionTarget" : self.ABI46_0_0ReactTag});
  }
  return YES;
}
...
- (NSString *)accessibilityValue
{
  ...
  // Handle Switch.
  if ((self.accessibilityTraits & ABI46_0_0SwitchAccessibilityTrait) == ABI46_0_0SwitchAccessibilityTrait) {
  ...
  // Handle states which haven't already been handled in ABI46_0_0RCTViewManager.
  for (NSString *state in self.accessibilityState) {
    id val = self.accessibilityState[state];
    if (!val) {
      continue;
...
{
  if (self.ABI46_0_0ReactAccessibilityElement == self) {
...
- (BOOL)performAccessibilityAction:(NSString *)name
{
  if (_onAccessibilityAction && accessibilityActionsNameMap[name]) {
    _onAccessibilityAction(@{@"actionName" : name, @"actionTarget" : self.ABI46_0_0ReactTag});
    return YES;
  }
  return NO;
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ABI46_0_0ReactTag: %@;", self.ABI46_0_0ReactTag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI46_0_0RCTAutoInsetsProtocol> *)parentView
                   ...
                   updateOffset:(BOOL)updateOffset
{
  ...
  if (parentView.automaticallyAdjustContentInsets) {
    UIEdgeInsets autoInset = ABI46_0_0RCTContentInsets(parentView);
...
- (void)ABI46_0_0React_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ABI46_0_0ReactSubviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ABI46_0_0React_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ABI46_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI46_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ABI46_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ABI46_0_0ReactSubviews.count == 0) {
  ...
  for (UIView *view in self.ABI46_0_0ReactSubviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ABI46_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ABI46_0_0React_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI46_0_0React_findClipView];
  if (clipView) {
    [self ABI46_0_0React_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI46_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x)
{
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ABI46_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI46_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI46_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI46_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets){
        ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
        ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
        ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
        ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets){
      ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, ABI46_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
      ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI46_0_0RCTDefaultIfNegativeTo(borderWidth, ABI46_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI46_0_0RCTCornerRadii)cornerRadii
{
  ...
  if ([[ABI46_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI46_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, _borderTopStartRadius);
    const CGFloat topEndRadius = ABI46_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, _borderTopEndRadius);
    const CGFloat bottomStartRadius = ABI46_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, _borderBottomStartRadius);
    const CGFloat bottomEndRadius = ABI46_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, _borderBottomEndRadius);
    ...
    topLeftRadius = ABI46_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI46_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI46_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI46_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius =
        ABI46_0_0RCTDefaultIfNegativeTo(radius, ABI46_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius =
        ABI46_0_0RCTDefaultIfNegativeTo(radius, ABI46_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius =
        ABI46_0_0RCTDefaultIfNegativeTo(radius, ABI46_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI46_0_0RCTDefaultIfNegativeTo(
        radius, ABI46_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI46_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI46_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI46_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI46_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI46_0_0RCTCornerRadii){
...
- (ABI46_0_0RCTBorderColors)borderColorsWithTraitCollection:(UITraitCollection *)traitCollection
{
  const BOOL isRTL = _ABI46_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;
  ...
  return (ABI46_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI46_0_0RCTUpdateShadowPathForView(self);

  const ABI46_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI46_0_0RCTBorderColors borderColors = [self borderColorsWithTraitCollection:self.traitCollection];

  BOOL useIOSBorderRendering = ABI46_0_0RCTCornerRadiiAreEqual(cornerRadii) && ABI46_0_0RCTBorderInsetsAreEqual(borderInsets) &&
      ABI46_0_0RCTBorderColorsAreEqual(borderColors) && _borderStyle == ABI46_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI46_0_0RCTGetBorderImage(
...
static BOOL ABI46_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI46_0_0RCTUpdateShadowPathForView(ABI46_0_0RCTView *view)
{
  if (ABI46_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {
      // If view has a solid background color, calculate shadow path from border
      const ABI46_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI46_0_0RCTCornerInsets cornerInsets = ABI46_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI46_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    const ABI46_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI46_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      cornerRadius = cornerRadii.topLeft;

    } else {
      CAShapeLayer *shapeLayer = [CAShapeLayer layer];
      CGPathRef path =
          ABI46_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI46_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                            \
  -(void)setBorder##side##Style : (ABI46_0_0RCTBorderStyle)style \
  {                                                     \
