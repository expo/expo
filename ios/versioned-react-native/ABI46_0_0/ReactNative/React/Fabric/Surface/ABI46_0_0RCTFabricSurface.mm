...
#import <ABI46_0_0React/ABI46_0_0RCTSurfaceRootView.h>
...
@implementation ABI46_0_0RCTFabricSurface {
  __weak ABI46_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI46_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _surfaceHandler =
        SurfaceHandler{ABI46_0_0RCTStringFromNSString(moduleName), (SurfaceId)[ABI46_0_0RCTAllocateRootViewTag() integerValue]};
    _surfaceHandler->setProps(convertIdToFollyDynamic(initialProperties));

    [_surfacePresenter registerSurface:self];

    [self setMinimumSize:CGSizeZero maximumSize:ABI46_0_0RCTViewportSize()];
...
- (NSString *)moduleName
{
  return ABI46_0_0RCTNSStringFromString(_surfaceHandler->getModuleName());
}
...
- (ABI46_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI46_0_0RCTSurfaceView alloc] initWithSurface:(ABI46_0_0RCTSurface *)self];
...
{
  return _surfaceHandler->getStatus() == SurfaceHandler::Status::Running ? ABI46_0_0RCTSurfaceStageRunning
                                                                         : ABI46_0_0RCTSurfaceStagePreparing;
}

- (void)_propagateStageChange
{
  ABI46_0_0RCTSurfaceStage stage = self.stage;

  // Notifying the `delegate`
  id<ABI46_0_0RCTSurfaceDelegate> delegate = self.delegate;
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI46_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)_updateLayoutContext
{
  ...
  layoutConstraints.layoutDirection = ABI46_0_0RCTLayoutDirection([[ABI46_0_0RCTI18nUtil sharedInstance] isRTL]);

  auto layoutContext = _surfaceHandler->getLayoutContext();

  layoutContext.pointScaleFactor = ABI46_0_0RCTScreenScale();
  layoutContext.swapLeftAndRightInRTL =
      [[ABI46_0_0RCTI18nUtil sharedInstance] isRTL] && [[ABI46_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL];
  layoutContext.fontSizeMultiplier = ABI46_0_0RCTFontSizeMultiplier();
...
- (void)setMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize viewportOffset:(CGPoint)viewportOffset
{
  ...
  layoutConstraints.minimumSize = ABI46_0_0RCTSizeFromCGSize(minimumSize);
  layoutConstraints.maximumSize = ABI46_0_0RCTSizeFromCGSize(maximumSize);

  if (!isnan(viewportOffset.x) && !isnan(viewportOffset.y)) {
    layoutContext.viewportOffset = ABI46_0_0RCTPointFromCGPoint(viewportOffset);
  }
...
- (CGSize)sizeThatFitsMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  return ABI46_0_0RCTCGSizeFromSize(_surfaceHandler->measure(layoutConstraints, layoutContext));
}

- (CGSize)minimumSize
{
  return ABI46_0_0RCTCGSizeFromSize(_surfaceHandler->getLayoutConstraints().minimumSize);
}

- (CGSize)maximumSize
{
  return ABI46_0_0RCTCGSizeFromSize(_surfaceHandler->getLayoutConstraints().maximumSize);
}

- (CGPoint)viewportOffset
{
  return ABI46_0_0RCTCGPointFromPoint(_surfaceHandler->getLayoutContext().viewportOffset);
}
