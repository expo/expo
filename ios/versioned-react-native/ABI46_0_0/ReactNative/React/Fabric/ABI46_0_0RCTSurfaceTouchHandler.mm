...
#import "ABI46_0_0RCTSurfaceTouchHandler.h"
...
#import "ABI46_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI46_0_0RCTTouchEventType) {
  ABI46_0_0RCTTouchEventTypeTouchStart,
  ABI46_0_0RCTTouchEventTypeTouchMove,
  ABI46_0_0RCTTouchEventTypeTouchEnd,
  ABI46_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI46_0_0RCTComponentViewProtocol> *componentView = nil;
    ...
    CGPoint rootViewOriginOffset)
{
  ...
  activeTouch.touch.offsetPoint = ABI46_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI46_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI46_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI46_0_0RCTForceTouchAvailable()) {
    activeTouch.touch.force = ABI46_0_0RCTZeroIfNaN(uiTouch.force / uiTouch.maximumPossibleForce);
  }
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView, CGPoint rootViewOriginOffset)
{
  ...
  while (componentView) {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      activeTouch.eventEmitter = [(id<ABI46_0_0RCTTouchableComponentViewProtocol>)componentView
...
{
  return [(id<ABI46_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitterAtPoint:point];
}
...
CreatePointerEventFromIncompleteHoverData(UIView *view, CGPoint clientLocation, NSTimeInterval timestamp)
{
  ...
  event.clientPoint = ABI46_0_0RCTPointFromCGPoint(clientLocation);
...
@interface ABI46_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI46_0_0RCTSurfaceTouchHandler {
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI46_0_0React Native,
...
- (void)attachToView:(UIView *)view
{
  ...
  if (ABI46_0_0RCTGetDispatchW3CPointerEvents()) {
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI46_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI46_0_0RCTTouchEventTypeTouchEnd || eventType == ABI46_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &activeTouch : activeTouches) {
    ...
    if (ABI46_0_0RCTGetDispatchW3CPointerEvents()) {
      ...
      switch (eventType) {
        case ABI46_0_0RCTTouchEventTypeTouchStart:
          activeTouch.eventEmitter->onPointerDown(pointerEvent);
          break;
        case ABI46_0_0RCTTouchEventTypeTouchMove:
          activeTouch.eventEmitter->onPointerMove2(pointerEvent);
          break;
        case ABI46_0_0RCTTouchEventTypeTouchEnd:
          activeTouch.eventEmitter->onPointerUp(pointerEvent);
          break;
        case ABI46_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI46_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI46_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI46_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI46_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (!_activeTouches.empty()) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI46_0_0RCTTouchEventTypeTouchCancel];
