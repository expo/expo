...
#import "ABI46_0_0RCTScrollViewComponentView.h"
...
#import <ABI46_0_0React/ABI46_0_0RCTScrollEvent.h>

#import <ABI46_0_0React/ABI46_0_0renderer/components/scrollview/ABI46_0_0RCTComponentViewHelpers.h>
#import <ABI46_0_0React/ABI46_0_0renderer/components/scrollview/ScrollViewComponentDescriptor.h>
#import <ABI46_0_0React/ABI46_0_0renderer/components/scrollview/ScrollViewEventEmitter.h>
#import <ABI46_0_0React/ABI46_0_0renderer/components/scrollview/ScrollViewProps.h>
#import <ABI46_0_0React/ABI46_0_0renderer/components/scrollview/ScrollViewState.h>
#import <ABI46_0_0React/ABI46_0_0renderer/components/scrollview/conversions.h>
...
static UIScrollViewKeyboardDismissMode ABI46_0_0RCTUIKeyboardDismissModeFromProps(ScrollViewProps const &props)
{
...
static UIScrollViewIndicatorStyle ABI46_0_0RCTUIScrollViewIndicatorStyleFromProps(ScrollViewProps const &props)
{
...
// TODO: T116850910 [Fabric][iOS] Make Fabric not use legacy ABI46_0_0RCTEventDispatcher for native-driven AnimatedEvents
static void ABI46_0_0RCTSendScrollEventForNativeAnimations_DEPRECATED(UIScrollView *scrollView, NSInteger tag)
{
  static uint16_t coalescingKey = 0;
  ABI46_0_0RCTScrollEvent *scrollEvent = [[ABI46_0_0RCTScrollEvent alloc] initWithEventName:@"onScroll"
                                                                 ABI46_0_0ReactTag:[NSNumber numberWithInt:tag]
  ...
  if (bridge) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ABI46_0_0RCTSendEventToLegacyEventDispatcher"
                                                        object:nil
                                                      userInfo:userInfo];
  }
}

@interface ABI46_0_0RCTScrollViewComponentView () <
    UIScrollViewDelegate,
    ABI46_0_0RCTScrollViewProtocol,
    ABI46_0_0RCTScrollableProtocol,
    ABI46_0_0RCTEnhancedScrollViewOverridingDelegate>

@end

@implementation ABI46_0_0RCTScrollViewComponentView {
...
+ (ABI46_0_0RCTScrollViewComponentView *_Nullable)findScrollViewComponentViewForView:(UIView *)view
{
  do {
    view = view.superview;
  } while (view != nil && ![view isKindOfClass:[ABI46_0_0RCTScrollViewComponentView class]]);
  return (ABI46_0_0RCTScrollViewComponentView *)view;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _scrollView = [[ABI46_0_0RCTEnhancedScrollView alloc] initWithFrame:self.bounds];
    _scrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    _scrollView.delaysContentTouches = NO;
    ((ABI46_0_0RCTEnhancedScrollView *)_scrollView).overridingDelegate = self;
...
{
  return ((ABI46_0_0RCTEnhancedScrollView *)_scrollView).delegateSplitter;
}

#pragma mark - ABI46_0_0RCTMountingTransactionObserving
...
#define REMAP_PROP(ABI46_0_0ReactName, localName, target)                      \
  if (oldScrollViewProps.ABI46_0_0ReactName != newScrollViewProps.ABI46_0_0ReactName) { \
    target.localName = newScrollViewProps.ABI46_0_0ReactName;                  \
  }

#define REMAP_VIEW_PROP(ABI46_0_0ReactName, localName) REMAP_PROP(ABI46_0_0ReactName, localName, self)
#define MAP_VIEW_PROP(name) REMAP_VIEW_PROP(name, name)
#define REMAP_SCROLL_VIEW_PROP(ABI46_0_0ReactName, localName) \
  REMAP_PROP(ABI46_0_0ReactName, localName, ((ABI46_0_0RCTEnhancedScrollView *)_scrollView))
#define MAP_SCROLL_VIEW_PROP(name) REMAP_SCROLL_VIEW_PROP(name, name)
  ...
  if (oldScrollViewProps.scrollIndicatorInsets != newScrollViewProps.scrollIndicatorInsets) {
    _scrollView.scrollIndicatorInsets = ABI46_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.scrollIndicatorInsets);
  }

  if (oldScrollViewProps.indicatorStyle != newScrollViewProps.indicatorStyle) {
    _scrollView.indicatorStyle = ABI46_0_0RCTUIScrollViewIndicatorStyleFromProps(newScrollViewProps);
  }
  ...
  if (oldScrollViewProps.contentInset != newScrollViewProps.contentInset) {
    _scrollView.contentInset = ABI46_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.contentInset);
  }

  ABI46_0_0RCTEnhancedScrollView *scrollView = (ABI46_0_0RCTEnhancedScrollView *)_scrollView;
  if (oldScrollViewProps.contentOffset != newScrollViewProps.contentOffset) {
    _scrollView.contentOffset = ABI46_0_0RCTCGPointFromPoint(newScrollViewProps.contentOffset);
  }

  if (oldScrollViewProps.snapToAlignment != newScrollViewProps.snapToAlignment) {
    scrollView.snapToAlignment = ABI46_0_0RCTNSStringFromString(toString(newScrollViewProps.snapToAlignment));
  }
  ...
  if (oldScrollViewProps.keyboardDismissMode != newScrollViewProps.keyboardDismissMode) {
    scrollView.keyboardDismissMode = ABI46_0_0RCTUIKeyboardDismissModeFromProps(newScrollViewProps);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  auto contentOffset = ABI46_0_0RCTCGPointFromPoint(data.contentOffset);
  if (!oldState && !CGPointEqualToPoint(contentOffset, CGPointZero)) {
    _scrollView.contentOffset = contentOffset;
  }

  CGSize contentSize = ABI46_0_0RCTCGSizeFromSize(data.getContentSize());
  ...
  _containerView.frame = CGRect{ABI46_0_0RCTCGPointFromPoint(data.contentBoundingRect.origin), contentSize};
...
- (void)_preserveContentOffsetIfNeededWithBlock:(void (^)())block
{
  ...
  [((ABI46_0_0RCTEnhancedScrollView *)_scrollView) preserveContentOffsetWithBlock:block];
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  while (ancestorView) {
    if ([ancestorView respondsToSelector:@selector(isJSResponder)]) {
      BOOL isJSResponder = ((UIView<ABI46_0_0RCTComponentViewProtocol> *)ancestorView).isJSResponder;
      if (isJSResponder) {
        return YES;
      }
    }
...
- (ScrollViewMetrics)_scrollViewMetrics
{
  ...
  metrics.contentSize = ABI46_0_0RCTSizeFromCGSize(_scrollView.contentSize);
  metrics.contentOffset = ABI46_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
  metrics.contentInset = ABI46_0_0RCTEdgeInsetsFromUIEdgeInsets(_scrollView.contentInset);
  metrics.containerSize = ABI46_0_0RCTSizeFromCGSize(_scrollView.bounds.size);
...
- (void)_updateStateWithContentOffset
{
  ...
  auto contentOffset = ABI46_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
...
- (void)prepareForRecycle
{
  ...
  _scrollView.contentOffset = ABI46_0_0RCTCGPointFromPoint(props.contentOffset);
...
{
  // Historically, `UIScrollView`s in ABI46_0_0React Native do not cancel touches
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if ((_lastScrollEventDispatchTime == 0) || (now - _lastScrollEventDispatchTime > _scrollEventThrottle)) {
    ...
    ABI46_0_0RCTSendScrollEventForNativeAnimations_DEPRECATED(scrollView, self.tag);
  }
...
{
  ABI46_0_0RCTScrollViewHandleCommand(self, commandName, args);
}
...
#pragma mark - ABI46_0_0RCTScrollableProtocol
...
Class<ABI46_0_0RCTComponentViewProtocol> ABI46_0_0RCTScrollViewCls(void)
{
  return ABI46_0_0RCTScrollViewComponentView.class;
}
