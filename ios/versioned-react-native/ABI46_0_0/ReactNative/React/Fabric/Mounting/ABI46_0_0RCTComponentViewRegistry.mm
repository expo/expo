...
const NSInteger ABI46_0_0RCTComponentViewRegistryRecyclePoolMaxSize = 1024;

@implementation ABI46_0_0RCTComponentViewRegistry {
  butter::map<Tag, ABI46_0_0RCTComponentViewDescriptor> _registry;
  butter::map<ComponentHandle, std::vector<ABI46_0_0RCTComponentViewDescriptor>> _recyclePool;
}

- (instancetype)init
{
  if (self = [super init]) {
    _componentViewFactory = [ABI46_0_0RCTComponentViewFactory currentComponentViewFactory];
...
{
  if (ABI46_0_0RCTExperimentGetPreemptiveViewAllocationDisabled()) {
    return;
  }
  ...
  std::vector<std::pair<ComponentHandle, float>> componentMagnitudes = {
      {[ABI46_0_0RCTViewComponentView componentDescriptorProvider].handle, 1},
      {[ABI46_0_0RCTImageComponentView componentDescriptorProvider].handle, 0.3},
      {[ABI46_0_0RCTParagraphComponentView componentDescriptorProvider].handle, 0.3},
  };
...
- (ABI46_0_0RCTComponentViewDescriptor const &)dequeueComponentViewWithComponentHandle:(ComponentHandle)componentHandle
                                                                          tag:(Tag)tag
{
  ...
  ABI46_0_0RCTAssert(
      ...
      @"ABI46_0_0RCTComponentViewRegistry: Attempt to dequeue already registered component.");
                        ...
                        componentViewDescriptor:(ABI46_0_0RCTComponentViewDescriptor)componentViewDescriptor
{
  ...
  ABI46_0_0RCTAssert(
      _registry.find(tag) != _registry.end(), @"ABI46_0_0RCTComponentViewRegistry: Attempt to enqueue unregistered component.");
...
- (ABI46_0_0RCTComponentViewDescriptor const &)componentViewDescriptorWithTag:(Tag)tag
{
  ABI46_0_0RCTAssertMainQueue();
  auto iterator = _registry.find(tag);
  ABI46_0_0RCTAssert(iterator != _registry.end(), @"ABI46_0_0RCTComponentViewRegistry: Attempt to query unregistered component.");
  return iterator->second;
}

- (nullable UIView<ABI46_0_0RCTComponentViewProtocol> *)findComponentViewWithTag:(Tag)tag
{
  ABI46_0_0RCTAssertMainQueue();
  auto iterator = _registry.find(tag);
...
- (ABI46_0_0RCTComponentViewDescriptor)_dequeueComponentViewWithComponentHandle:(ComponentHandle)componentHandle
{
  ABI46_0_0RCTAssertMainQueue();
                         ...
                         componentViewDescriptor:(ABI46_0_0RCTComponentViewDescriptor)componentViewDescriptor
{
  ...
  if (recycledViews.size() > ABI46_0_0RCTComponentViewRegistryRecyclePoolMaxSize) {
    return;
  }

  ABI46_0_0RCTAssert(
      componentViewDescriptor.view.superview == nil, @"ABI46_0_0RCTComponentViewRegistry: Attempt to recycle a mounted view.");
