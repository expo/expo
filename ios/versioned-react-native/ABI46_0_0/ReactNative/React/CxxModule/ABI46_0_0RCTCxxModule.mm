...
@implementation ABI46_0_0RCTCxxModule {
  std::unique_ptr<ABI46_0_0facebook::xplat::module::CxxModule> _module;
}
...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI46_0_0RCTAssert(
          [ABI46_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
          @"CxxModule class name %@ does not match runtime name %s",
          ABI46_0_0RCTBridgeModuleNameForClass([self class]),
...
{
  ABI46_0_0RCTAssert(NO, @"Subclass %@ must override createModule", [self class]);
  return nullptr;
}

- (NSArray<id<ABI46_0_0RCTBridgeMethod>> *)methodsToExport
{
  [self lazyInit];
  if (!_module) {
    return nil;
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI46_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
