...
#import "ABI46_0_0DispatchMessageQueueThread.h"
#import "ABI46_0_0RCTCxxModule.h"
#import "ABI46_0_0RCTNativeModule.h"

namespace ABI46_0_0facebook {
namespace ABI46_0_0React {

using ABI46_0_0facebook::jsi::JSError;
...
createNativeModules(NSArray<ABI46_0_0RCTModuleData *> *modules, ABI46_0_0RCTBridge *bridge, const std::shared_ptr<Instance> &instance)
{
  ...
  for (ABI46_0_0RCTModuleData *moduleData in modules) {
    if ([moduleData.moduleClass isSubclassOfClass:[ABI46_0_0RCTCxxModule class]]) {
      nativeModules.emplace_back(std::make_unique<CxxNativeModule>(
          instance,
          [moduleData.name UTF8String],
          [moduleData] { return [(ABI46_0_0RCTCxxModule *)(moduleData.instance) createModule]; },
          std::make_shared<DispatchMessageQueueThread>(moduleData)));
    } else {
      nativeModules.emplace_back(std::make_unique<ABI46_0_0RCTNativeModule>(bridge, moduleData));
    }
  }
...
static NSError *errorWithException(const std::exception &e)
{
  ...
  if (jsError) {
    errorInfo[ABI46_0_0RCTJSRawStackTraceKey] = @(jsError->getStack().c_str());
  ...
  return [NSError errorWithDomain:ABI46_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}
...
NSError *tryAndReturnError(const std::function<void()> &func)
{
  try {
    @try {
      ...
      return ABI46_0_0RCTErrorWithNSException(exception);
    } @catch (id exception) {
      // This will catch any other ObjC exception, but no C++ exceptions
      return ABI46_0_0RCTErrorWithMessage(@"non-std ObjC Exception");
    }
    ...
    return ABI46_0_0RCTErrorWithMessage(@"non-std C++ exception");
  }
}
