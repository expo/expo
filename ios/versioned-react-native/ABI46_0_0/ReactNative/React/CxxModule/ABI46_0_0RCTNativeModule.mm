...
#import "ABI46_0_0RCTNativeModule.h"
...
#import <ABI46_0_0React/ABI46_0_0RCTCxxUtils.h>
...
#import <ABI46_0_0Reactperflogger/ABI46_0_0BridgeNativeModulePerfLogger.h>
...
static MethodCallResult invokeInner(
    ...
    ABI46_0_0RCTModuleData *moduleData,
    unsigned int methodId,
    const folly::dynamic &params,
    int callId,
    SchedulingContext context);

ABI46_0_0RCTNativeModule::ABI46_0_0RCTNativeModule(ABI46_0_0RCTBridge *bridge, ABI46_0_0RCTModuleData *moduleData)
    : m_bridge(bridge), m_moduleData(moduleData)
{
}

std::string ABI46_0_0RCTNativeModule::getName()
{
  return [m_moduleData.name UTF8String];
}

std::string ABI46_0_0RCTNativeModule::getSyncMethodName(unsigned int methodId)
{
  return m_moduleData.methods[methodId].JSMethodName;
}

std::vector<MethodDescriptor> ABI46_0_0RCTNativeModule::getMethods()
{
  std::vector<MethodDescriptor> descs;

  for (id<ABI46_0_0RCTBridgeMethod> method in m_moduleData.methods) {
    descs.emplace_back(method.JSMethodName, ABI46_0_0RCTFunctionDescriptorFromType(method.functionType));
  }

  return descs;
}

folly::dynamic ABI46_0_0RCTNativeModule::getConstants()
{
  ABI46_0_0RCT_PROFILE_BEGIN_EVENT(ABI46_0_0RCTProfileTagAlways, @"[ABI46_0_0RCTNativeModule getConstants] moduleData.exportedConstants", nil);
...
void ABI46_0_0RCTNativeModule::invoke(unsigned int methodId, folly::dynamic &&params, int callId)
{
  ...
  const bool isSyncModule = queue == ABI46_0_0RCTJSThread;
  ...
  __weak ABI46_0_0RCTBridge *weakBridge = m_bridge;
  __weak ABI46_0_0RCTModuleData *weakModuleData = m_moduleData;
...
MethodCallResult ABI46_0_0RCTNativeModule::callSerializableNativeHook(unsigned int ABI46_0_0ReactMethodId, folly::dynamic &&params)
{
  return invokeInner(m_bridge, m_moduleData, ABI46_0_0ReactMethodId, params, 0, Sync);
}
    ...
    SchedulingContext context)
{
  ...
  id<ABI46_0_0RCTBridgeMethod> method = moduleData.methods[methodId];
  if (ABI46_0_0RCT_DEBUG && !method) {
    ABI46_0_0RCTLogError(@"Unknown methodID: %ud for module: %@", methodId, moduleData.name);
  }
  ...
  @try {
    ...
    if ([exception.name hasPrefix:ABI46_0_0RCTFatalExceptionName]) {
      @throw exception;
    }
    ...
    ABI46_0_0RCTFatal(ABI46_0_0RCTErrorWithMessage(message));
#else
    ABI46_0_0RCTFatalException(exception);
#endif
  }
