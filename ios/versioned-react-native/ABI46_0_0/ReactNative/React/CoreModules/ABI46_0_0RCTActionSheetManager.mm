...
#import <ABI46_0_0React/ABI46_0_0RCTActionSheetManager.h>
...
@interface ABI46_0_0RCTActionSheetManager () <UIActionSheetDelegate, ABI46_0_0NativeActionSheetManagerSpec>

@property (nonatomic, strong) NSMutableArray<UIAlertController *> *alertControllers;

@end

@implementation ABI46_0_0RCTActionSheetManager
                ...
                anchorViewTag:(NSNumber *)anchorViewTag
{
  ...
  if (anchorViewTag) {
    sourceView = [self.viewRegistry_DEPRECATED viewForABI46_0_0ReactTag:anchorViewTag];
  } else {
...
ABI46_0_0RCT_EXPORT_METHOD(showActionSheetWithOptions
                  : (ABI46_0_0JS::NativeActionSheetManager::SpecShowActionSheetWithOptionsOptions &)options callback
                  : (ABI46_0_0RCTResponseSenderBlock)callback)
{
  if (ABI46_0_0RCTRunningInAppExtension()) {
    ABI46_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  }

  NSString *title = options.title();
  NSString *message = options.message();
  NSArray<NSString *> *buttons = ABI46_0_0RCTConvertOptionalVecToArray(options.options(), ^id(NSString *element) {
    return element;
  });
  NSArray<NSNumber *> *disabledButtonIndices;
  NSInteger cancelButtonIndex =
      options.cancelButtonIndex() ? [ABI46_0_0RCTConvert NSInteger:@(*options.cancelButtonIndex())] : -1;
  NSArray<NSNumber *> *destructiveButtonIndices;
  if (options.disabledButtonIndices()) {
    disabledButtonIndices = ABI46_0_0RCTConvertVecToArray(*options.disabledButtonIndices(), ^id(double element) {
      return @(element);
    });
  }
  if (options.destructiveButtonIndices()) {
    destructiveButtonIndices = ABI46_0_0RCTConvertVecToArray(*options.destructiveButtonIndices(), ^id(double element) {
      return @(element);
    });
  } else {
  ...
  UIViewController *controller = ABI46_0_0RCTPresentedViewController();
  NSNumber *anchor = [ABI46_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  UIColor *tintColor = [ABI46_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];
  UIColor *cancelButtonTintColor =
      [ABI46_0_0RCTConvert UIColor:options.cancelButtonTintColor() ? @(*options.cancelButtonTintColor()) : nil];
  ...
  // the action sheet on iPad. ABI46_0_0RCTResponseSenderBlock can only be called once so
  ...
  if (@available(iOS 13.0, *)) {
    NSString *userInterfaceStyle = [ABI46_0_0RCTConvert NSString:options.userInterfaceStyle()];
...
ABI46_0_0RCT_EXPORT_METHOD(dismissActionSheet)
{
  if (_alertControllers.count == 0) {
    ABI46_0_0RCTLogWarn(@"Unable to dismiss action sheet");
  }
...
ABI46_0_0RCT_EXPORT_METHOD(showShareActionSheetWithOptions
                  : (ABI46_0_0JS::NativeActionSheetManager::SpecShowShareActionSheetWithOptionsOptions &)options failureCallback
                  : (ABI46_0_0RCTResponseSenderBlock)failureCallback successCallback
                  : (ABI46_0_0RCTResponseSenderBlock)successCallback)
{
  if (ABI46_0_0RCTRunningInAppExtension()) {
    ABI46_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  ...
  NSURL *URL = [ABI46_0_0RCTConvert NSURL:options.url()];
  if (URL) {
    if ([URL.scheme.lowercaseString isEqualToString:@"data"]) {
      NSError *error;
      NSData *data = [NSData dataWithContentsOfURL:URL options:(NSDataReadingOptions)0 error:&error];
      if (!data) {
        failureCallback(@[ ABI46_0_0RCTJSErrorFromNSError(error) ]);
        return;
      }
  ...
  if (items.count == 0) {
    ABI46_0_0RCTLogError(@"No `url` or `message` to share");
    return;
  }
  ...
  NSArray *excludedActivityTypes =
      ABI46_0_0RCTConvertOptionalVecToArray(options.excludedActivityTypes(), ^id(NSString *element) {
        return element;
      });
  ...
  shareController.completionWithItemsHandler =
      ^(NSString *activityType, BOOL completed, __unused NSArray *returnedItems, NSError *activityError) {
        if (activityError) {
          failureCallback(@[ ABI46_0_0RCTJSErrorFromNSError(activityError) ]);
        } else if (completed || activityType == nil) {
          successCallback(@[ @(completed), ABI46_0_0RCTNullIfNil(activityType) ]);
        }
      };

  NSNumber *anchorViewTag = [ABI46_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  shareController.view.tintColor = [ABI46_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];
...
Class ABI46_0_0RCTActionSheetManagerCls(void)
{
  return ABI46_0_0RCTActionSheetManager.class;
}
