...
#import <ABI46_0_0React/ABI46_0_0RCTErrorInfo.h>
#import <ABI46_0_0React/ABI46_0_0RCTEventDispatcherProtocol.h>
#import <ABI46_0_0React/ABI46_0_0RCTJSStackFrame.h>
#import <ABI46_0_0React/ABI46_0_0RCTRedBoxExtraDataViewController.h>
...
@class ABI46_0_0RCTRedBoxWindow;

@interface UIButton (ABI46_0_0RCTRedBox)

@property (nonatomic) ABI46_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI46_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI46_0_0RCTRedBox)

- (ABI46_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI46_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI46_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI46_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI46_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI46_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI46_0_0RCTRedBoxWindow : NSObject <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UIViewController *rootViewController;
@property (nonatomic, weak) id<ABI46_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI46_0_0RCTRedBoxWindow {
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI46_0_0RCTJSStackFrame *> *_lastStackTrace;
  int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame
           customButtonTitles:(NSArray<NSString *> *)customButtonTitles
         customButtonHandlers:(NSArray<ABI46_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
  if (self = [super init]) {
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title
    accessibilityIdentifier:(NSString *)accessibilityIdentifier
                   selector:(SEL)selector
                      block:(ABI46_0_0RCTRedBoxButtonPressHandler)block
{
...
{
  return ABI46_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
}
...
- (void)showErrorMessage:(NSString *)message
               withStack:(NSArray<ABI46_0_0RCTJSStackFrame *> *)stack
    ...
    if (!isRootViewControllerPresented) {
      [_stackTraceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]
                                  atScrollPosition:UITableViewScrollPositionTop
                                          animated:NO];
      [ABI46_0_0RCTKeyWindow().rootViewController presentViewController:self.rootViewController animated:YES completion:nil];
    }
  }
}
...
- (void)copyStack
{
  ...
  for (ABI46_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI46_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI46_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI46_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI46_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI46_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI46_0_0RCTKeyCommands for this, but since
...
@interface ABI46_0_0RCTRedBox () <
    ABI46_0_0RCTInvalidating,
    ABI46_0_0RCTRedBoxWindowActionDelegate,
    ABI46_0_0RCTRedBoxExtraDataActionDelegate,
    ABI46_0_0NativeRedBoxSpec>
@end

@implementation ABI46_0_0RCTRedBox {
  ABI46_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI46_0_0RCTErrorCustomizer>> *_errorCustomizers;
  ABI46_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
  NSMutableArray<NSString *> *_customButtonTitles;
  NSMutableArray<ABI46_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (void)registerErrorCustomizer:(id<ABI46_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI46_0_0RCTErrorInfo *)_customizeError:(ABI46_0_0RCTErrorInfo *)error
{
  ABI46_0_0RCTAssertMainQueue();
  if (!self->_errorCustomizers) {
    return error;
  }
  for (id<ABI46_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI46_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI46_0_0RCTJSStackTraceKey]
             errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message
             withDetails:(NSString *)details
                   stack:(NSArray<ABI46_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
...
{
  NSArray<ABI46_0_0RCTJSStackFrame *> *stack = [ABI46_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
  [self showErrorMessage:message
         withParsedStack:[ABI46_0_0RCTJSStackFrame stackFramesWithDictionaries:stack]
                isUpdate:NO
...
- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI46_0_0RCTJSStackFrame *> *)stack
{
  [self showErrorMessage:message withParsedStack:stack errorCookie:-1];
}

- (void)updateErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI46_0_0RCTJSStackFrame *> *)stack
{
  [self updateErrorMessage:message withParsedStack:stack errorCookie:-1];
}

- (void)showErrorMessage:(NSString *)message
         withParsedStack:(NSArray<ABI46_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
  [self showErrorMessage:message withParsedStack:stack isUpdate:NO errorCookie:errorCookie];
}
             ...
             errorCookie:(int)errorCookie
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self->_extraDataViewController == nil) {
      self->_extraDataViewController = [ABI46_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
    ...
    if (!self->_window) {
      self->_window = [[ABI46_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds
    ...
    ABI46_0_0RCTErrorInfo *errorInfo = [[ABI46_0_0RCTErrorInfo alloc] initWithErrorMessage:message stack:stack];
...
ABI46_0_0RCT_EXPORT_METHOD(setExtraData : (NSDictionary *)extraData forIdentifier : (NSString *)identifier)
{
  [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI46_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI46_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI46_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bundleManager.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI46_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI46_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI46_0_0RCTRedBoxWindow *)redBoxWindow
{
  if (_overrideReloadAction) {
    _overrideReloadAction();
  } else {
    ABI46_0_0RCTTriggerReloadCommandListeners(@"Redbox");
  }
  [self dismiss];
}

- (void)addCustomButton:(NSString *)title onPressHandler:(ABI46_0_0RCTRedBoxButtonPressHandler)handler
{
...
{
  return std::make_shared<ABI46_0_0facebook::ABI46_0_0React::NativeRedBoxSpecJSI>(params);
}

@end

@implementation ABI46_0_0RCTBridge (ABI46_0_0RCTRedBox)

- (ABI46_0_0RCTRedBox *)redBox
{
  return ABI46_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI46_0_0RCTRedBox class]] : nil;
}
...
@interface ABI46_0_0RCTRedBox () <ABI46_0_0NativeRedBoxSpec>
@end
...
Class ABI46_0_0RCTRedBoxCls(void)
{
  return ABI46_0_0RCTRedBox.class;
}
