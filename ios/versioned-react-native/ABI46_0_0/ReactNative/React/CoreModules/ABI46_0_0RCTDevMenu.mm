...
#import <ABI46_0_0React/ABI46_0_0RCTKeyCommands.h>
...
NSString *const ABI46_0_0RCTShowDevMenuNotification = @"ABI46_0_0RCTShowDevMenuNotification";

@implementation UIWindow (ABI46_0_0RCTDevMenu)

- (void)ABI46_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI46_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

@implementation ABI46_0_0RCTDevMenuItem {
  ABI46_0_0RCTDevMenuItemTitleBlock _titleBlock;
  dispatch_block_t _handler;
}

- (instancetype)initWithTitleBlock:(ABI46_0_0RCTDevMenuItemTitleBlock)titleBlock handler:(dispatch_block_t)handler
{
  if ((self = [super init])) {
...
typedef void (^ABI46_0_0RCTDevMenuAlertActionHandler)(UIAlertAction *action);

@interface ABI46_0_0RCTDevMenu () <ABI46_0_0RCTBridgeModule, ABI46_0_0RCTInvalidating, ABI46_0_0NativeDevMenuSpec>

@end

@implementation ABI46_0_0RCTDevMenu {
  UIAlertController *_actionSheet;
  NSMutableArray<ABI46_0_0RCTDevMenuItem *> *_extraMenuItems;
}
  ...
  if ((self = [super init])) {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showOnShake)
                                                 name:ABI46_0_0RCTShowDevMenuNotification
                                               object:nil];
...
- (void)registerHotkeys
{
  ...
  ABI46_0_0RCTKeyCommands *commands = [ABI46_0_0RCTKeyCommands sharedInstance];
  ...
  [commands registerKeyCommandWithInput:@"i"
                          modifierFlags:UIKeyModifierCommand
                                 action:^(__unused UIKeyCommand *command) {
                                   [(ABI46_0_0RCTDevSettings *)[weakSelf.moduleRegistry moduleForName:"DevSettings"]
...
- (void)showOnShake
{
  if ([((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]) isShakeToShowDevMenuEnabled]) {
    for (UIWindow *window in [ABI46_0_0RCTSharedApplication() windows]) {
      NSString *recursiveDescription = [window valueForKey:@"recursiveDescription"];
      if ([recursiveDescription containsString:@"ABI46_0_0RCTView"]) {
        [self show];
        return;
      }
    }
...
{
  [self addItem:[ABI46_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
{
  [[ABI46_0_0RCTBundleURLProvider sharedSettings] resetToDefaults];
  self->_bundleManager.bundleURL = [[ABI46_0_0RCTBundleURLProvider sharedSettings] jsBundleURLForFallbackExtension:nil];
  ABI46_0_0RCTTriggerReloadCommandListeners(@"Dev menu - reset to default");
}

- (NSArray<ABI46_0_0RCTDevMenuItem *> *)_menuItemsToPresent
{
  NSMutableArray<ABI46_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items
  __weak ABI46_0_0RCTDevSettings *devSettings = [_moduleRegistry moduleForName:"DevSettings"];
  __weak ABI46_0_0RCTDevMenu *weakSelf = self;
  __weak ABI46_0_0RCTBundleManager *bundleManager = _bundleManager;

  [items addObject:[ABI46_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload"
                                               handler:^{
                                                 ABI46_0_0RCTTriggerReloadCommandListeners(@"Dev menu - reload");
                                               }]];
  ...
  if (!devSettings.isProfilingEnabled) {
    ...
    if (devSettings.isDeviceDebuggingAvailable) {
      ...
      [items addObject:[ABI46_0_0RCTDevMenuItem
                           buttonItemWithTitleBlock:^NSString * {
                             return @"Open Debugger";
                           }
                           handler:^{
                             [ABI46_0_0RCTInspectorDevServerHelper
                                          openURL:@"flipper://null/Hermesdebuggerrn?device=ABI46_0_0React%20Native"
      ...
      [items addObject:[ABI46_0_0RCTDevMenuItem
                           buttonItemWithTitleBlock:^NSString * {
                             return @"Open ABI46_0_0React DevTools";
                           }
                           handler:^{
                             [ABI46_0_0RCTInspectorDevServerHelper
                                          openURL:@"flipper://null/ABI46_0_0React?device=ABI46_0_0React%20Native"
                                    withBundleURL:bundleManager.bundleURL
                                 withErrorMessage:@"Failed to open Flipper. Please check that Metro is runnning."];
                           }]];
    ...
    if (devSettings.isRemoteDebuggingAvailable) {
      ...
      [items
          addObject:[ABI46_0_0RCTDevMenuItem
                        buttonItemWithTitle:@"Debugger Unavailable"
                                    handler:^{
                                      NSString *message = ABI46_0_0RCTTurboModuleEnabled()
                                          ? @"Debugging with Chrome is not supported when TurboModules are enabled."
                                          : @"Include the ABI46_0_0RCTWebSocket library to enable JavaScript debugging.";
                                      ...
                                      [ABI46_0_0RCTPresentedViewController() presentViewController:alertController
                                                                                 animated:YES
                                                                               completion:NULL];
                                    }]];
    }
  ...
  [items
      addObject:[ABI46_0_0RCTDevMenuItem
                    ...
                    handler:^{
                          ...
                          addAction:[UIAlertAction
                                        actionWithTitle:@"Apply Changes"
                                                ...
                                                handler:^(__unused UIAlertAction *action) {
                                                  ...
                                                  if (portNumber == nil) {
                                                    portNumber = [NSNumber numberWithInt:ABI46_0_0RCT_METRO_PORT];
                                                  }
                                                  [ABI46_0_0RCTBundleURLProvider sharedSettings].jsLocation = [NSString
                                                      stringWithFormat:@"%@:%d", ipTextField.text, portNumber.intValue];
                                                  if (bundleRoot.length == 0) {
                                                    [bundleManager resetBundleURL];
                                                  } else {
                                                    bundleManager.bundleURL = [[ABI46_0_0RCTBundleURLProvider sharedSettings]
                                                        jsBundleURLForBundleRoot:bundleRoot];
                                                  }

                                                  ABI46_0_0RCTTriggerReloadCommandListeners(@"Dev menu - apply changes");
                                                }]];
                      ...
                      [ABI46_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
                    }]];
...
{
  if (_actionSheet || ABI46_0_0RCTRunningInAppExtension()) {
    return;
  }
  ...
  NSString *debugMenuTitle = [NSString stringWithFormat:@"ABI46_0_0React Native Debug Menu (%@)", debugMenuType];

  _actionSheet = [UIAlertController alertControllerWithTitle:debugMenuTitle message:description preferredStyle:style];

  NSArray<ABI46_0_0RCTDevMenuItem *> *items = [self _menuItemsToPresent];
  for (ABI46_0_0RCTDevMenuItem *item in items) {
  ...
  [ABI46_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:nil];

  [_callableJSModules invokeModule:@"ABI46_0_0RCTNativeAppEventEmitter" method:@"emit" withArgs:@[ @"ABI46_0_0RCTDevMenuShown" ]];
}

- (ABI46_0_0RCTDevMenuAlertActionHandler)alertActionHandlerForDevItem:(ABI46_0_0RCTDevMenuItem *__nullable)item
{
...
#define WARN_DEPRECATED_DEV_MENU_EXPORT() \
  ABI46_0_0RCTLogWarn(@"Using deprecated method %s, use ABI46_0_0RCTDevSettings instead", __func__)

- (void)setShakeToShow:(BOOL)shakeToShow
{
  ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isShakeToShowDevMenuEnabled = shakeToShow;
}

- (BOOL)shakeToShow
{
  return ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isShakeToShowDevMenuEnabled;
}
...
ABI46_0_0RCT_EXPORT_METHOD(debugRemotely : (BOOL)enableDebug)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isDebuggingRemotely = enableDebug;
}

ABI46_0_0RCT_EXPORT_METHOD(setProfilingEnabled : (BOOL)enabled)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isProfilingEnabled = enabled;
}

- (BOOL)profilingEnabled
{
  return ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isProfilingEnabled;
}

ABI46_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled : (BOOL)enabled)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isHotLoadingEnabled = enabled;
}

- (BOOL)hotLoadingEnabled
{
  return ((ABI46_0_0RCTDevSettings *)[_moduleRegistry moduleForName:"DevSettings"]).isHotLoadingEnabled;
}
...
{
  return std::make_shared<ABI46_0_0facebook::ABI46_0_0React::NativeDevMenuSpecJSI>(params);
}
...
@interface ABI46_0_0RCTDevMenu () <ABI46_0_0NativeDevMenuSpec>
@end
...
- (void)addItem:(ABI46_0_0RCTDevMenu *)item
{
}
...
@implementation ABI46_0_0RCTBridge (ABI46_0_0RCTDevMenu)

- (ABI46_0_0RCTDevMenu *)devMenu
{
#if ABI46_0_0RCT_DEV_MENU
  return [self moduleForClass:[ABI46_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
...
Class ABI46_0_0RCTDevMenuCls(void)
{
  return ABI46_0_0RCTDevMenu.class;
}
