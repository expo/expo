...
// ABI46_0_0ReactAndroid/src/main/java/com/facebook/ABI46_0_0React/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int ABI46_0_0RECONNECT_DELAY_MS = 2000;

@implementation ABI46_0_0RCTBundleStatus
@end

@interface ABI46_0_0RCTInspectorPackagerConnection () <ABI46_0_0RCTSRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI46_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  ABI46_0_0RCTSRWebSocket *_webSocket;
  dispatch_queue_t _jsQueue;
  BOOL _closed;
  BOOL _suppressConnectionErrors;
  ABI46_0_0RCTBundleStatusProvider _bundleStatusProvider;
}
@end

@interface ABI46_0_0RCTInspectorRemoteConnection () {
  __weak ABI46_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI46_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI46_0_0RCTInspectorPackagerConnection
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI46_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  ABI46_0_0RCTInspectorLocalConnection *existingConnection = _inspectorConnections[pageId];
  if (existingConnection) {
    [_inspectorConnections removeObjectForKey:pageId];
    [existingConnection disconnect];
    ABI46_0_0RCTLogWarn(@"Already connected: %@", pageId);
    return;
  }

  ABI46_0_0RCTInspectorRemoteConnection *remoteConnection =
      [[ABI46_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self pageId:pageId];

  ABI46_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI46_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI46_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (void)handleWrappedEvent:(NSDictionary *)payload
{
  ...
  ABI46_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
  if (!inspectorConnection) {
    ABI46_0_0RCTLogWarn(@"Not connected to page: %@ , failed trying to handle event: %@", pageId, wrappedEvent);
    return;
  }
...
- (NSArray *)pages
{
  NSArray<ABI46_0_0RCTInspectorPage *> *pages = [ABI46_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];

  ABI46_0_0RCTBundleStatusProvider statusProvider = _bundleStatusProvider;
  ABI46_0_0RCTBundleStatus *bundleStatus = statusProvider == nil ? nil : statusProvider();

  for (ABI46_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(__unused ABI46_0_0RCTSRWebSocket *)webSocket didFailWithError:(NSError *)error
{
  if (_webSocket) {
...
- (void)webSocket:(__unused ABI46_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  // warn but don't die on unrecognized messages
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI46_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI46_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI46_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@", messageText);
    return;
  }
...
- (void)webSocket:(__unused ABI46_0_0RCTSRWebSocket *)webSocket
...
{
  if (_closed) {
    ABI46_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresponding android code has a lot of custom config options for
  // timeouts, but it appears the iOS ABI46_0_0RCTSRWebSocket API doesn't have the same
  // implemented options.
  _webSocket = [[ABI46_0_0RCTSRWebSocket alloc] initWithURL:_url];
...
{
  if (_closed) {
    ABI46_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI46_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI46_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, ABI46_0_0RECONNECT_DELAY_MS * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
    ABI46_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(_jsQueue, ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI46_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI46_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
...
- (void)abort:(NSString *)message withCause:(NSError *)cause
{
  ...
  if (![cause.domain isEqual:NSPOSIXErrorDomain] || cause.code != ECONNREFUSED) {
    ABI46_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
  }
...
@implementation ABI46_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI46_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
