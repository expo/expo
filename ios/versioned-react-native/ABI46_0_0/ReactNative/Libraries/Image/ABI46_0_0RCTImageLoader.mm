...
BOOL ABI46_0_0RCTImageLoadingPerfInstrumentationEnabled(void)
{
  return imagePerfInstrumentationEnabled;
}

void ABI46_0_0RCTEnableImageLoadingPerfInstrumentation(BOOL enabled)
{
  imagePerfInstrumentationEnabled = enabled;
}

static NSInteger ABI46_0_0RCTImageBytesForImage(UIImage *image)
{
...
@interface ABI46_0_0RCTImageLoader() <ABI46_0_0NativeImageLoaderIOSSpec, ABI46_0_0RCTImageLoaderWithAttributionProtocol>

@end

@implementation UIImage (ABI46_0_0React)

- (NSInteger)ABI46_0_0ReactDecodedImageBytes
{
  NSNumber *imageBytes = objc_getAssociatedObject(self, _cmd);
  if (!imageBytes) {
    imageBytes = @(ABI46_0_0RCTImageBytesForImage(self));
  }
  return [imageBytes integerValue];
}

- (void)setABI46_0_0ReactDecodedImageBytes:(NSInteger)bytes
{
  objc_setAssociatedObject(self, @selector(ABI46_0_0ReactDecodedImageBytes), @(bytes), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

@end

@implementation ABI46_0_0RCTImageLoader
{
  NSArray<id<ABI46_0_0RCTImageURLLoader>> * (^_loadersProvider)(ABI46_0_0RCTModuleRegistry *);
  NSArray<id<ABI46_0_0RCTImageDataDecoder>> * (^_decodersProvider)(ABI46_0_0RCTModuleRegistry *);
  NSArray<id<ABI46_0_0RCTImageURLLoader>> *_loaders;
  NSArray<id<ABI46_0_0RCTImageDataDecoder>> *_decoders;
  NSOperationQueue *_imageDecodeQueue;
  dispatch_queue_t _URLRequestQueue;
  id<ABI46_0_0RCTImageCache> _imageCache;
  ...
  __weak id<ABI46_0_0RCTImageRedirectProtocol> _redirectDelegate;
}
...
- (instancetype)initWithRedirectDelegate:(id<ABI46_0_0RCTImageRedirectProtocol>)redirectDelegate
                        ...
                        decodersProvider:(NSArray<id<ABI46_0_0RCTImageDataDecoder>> * (^)(ABI46_0_0RCTModuleRegistry *))getHandlers
{
...
- (void)setUp
{
  ...
  _URLRequestQueue = dispatch_queue_create("com.facebook.ABI46_0_0React.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
#pragma mark - ABI46_0_0RCTImageLoaderProtocol 1/3

- (id<ABI46_0_0RCTImageCache>)imageCache
{
  if (!_imageCache) {
    //set up with default cache
    _imageCache = [ABI46_0_0RCTImageCache new];
  }
  return _imageCache;
}
...
{
  if (_imageCache) {
    ABI46_0_0RCTLogWarn(@"ABI46_0_0RCTImageCache was already set and has now been overridden.");
  }
  _imageCache = cache;
}

- (id<ABI46_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
  ...
  if (!_loaders) {
    ...
    if (!_loaders) {
      ...
      if (_loadersProvider) {
        ...
        ABI46_0_0RCTAssert(_bridge, @"Trying to find ABI46_0_0RCTImageURLLoaders and bridge not set.");
        _loaders = [_bridge modulesConformingToProtocol:@protocol(ABI46_0_0RCTImageURLLoader)];
      }

      _loaders = [_loaders sortedArrayUsingComparator:^NSComparisonResult(id<ABI46_0_0RCTImageURLLoader> a, id<ABI46_0_0RCTImageURLLoader> b) {
  ...
  if (ABI46_0_0RCT_DEBUG) {
    ...
    id<ABI46_0_0RCTImageURLLoader> previousLoader = nil;
    for (id<ABI46_0_0RCTImageURLLoader> loader in _loaders) {
      ...
      if ([loader canLoadImageURL:URL]) {
        if (previousLoader) {
          if (priority == previousPriority) {
            ABI46_0_0RCTLogError(@"The ABI46_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI46_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
  if (!_maxConcurrentLoadingTasks) {
    [self setUp];
  }
  ...
  if (!_decoders) {
    ...
    if (_decodersProvider) {
      ...
      ABI46_0_0RCTAssert(_bridge, @"Trying to find ABI46_0_0RCTImageDataDecoders and bridge not set.");
      _decoders = [_bridge modulesConformingToProtocol:@protocol(ABI46_0_0RCTImageDataDecoder)];
    }

    _decoders = [_decoders sortedArrayUsingComparator:^NSComparisonResult(id<ABI46_0_0RCTImageDataDecoder> a, id<ABI46_0_0RCTImageDataDecoder> b) {
  ...
  if (ABI46_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI46_0_0RCTImageDataDecoder> previousDecoder = nil;
    for (id<ABI46_0_0RCTImageDataDecoder> decoder in _decoders) {
      ...
      if ([decoder canDecodeImageData:data]) {
        if (previousDecoder) {
          if (priority == previousPriority) {
            ABI46_0_0RCTLogError(@"The ABI46_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI46_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI46_0_0RCTResizeMode resizeMode)
{
  ...
  CGRect targetSize = ABI46_0_0RCTTargetRect(image.size, size, scale, resizeMode);
  CGAffineTransform transform = ABI46_0_0RCTTransformFromTargetRect(image.size, targetSize);
  image = ABI46_0_0RCTTransformImage(image, size, scale, transform);
  return image;
}

#pragma mark - ABI46_0_0RCTImageLoaderProtocol 2/3
...
- (nullable ABI46_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                   ...
                                                   partialLoadBlock:(ABI46_0_0RCTImageLoaderPartialLoadBlock)partialLoadBlock
                                                    completionBlock:(ABI46_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  ABI46_0_0RCTImageURLLoaderRequest *request = [self loadImageWithURLRequest:imageURLRequest
                                                               size:size
                                                              scale:scale
                                                            clipped:clipped
                                                         resizeMode:resizeMode
...
- (void)dequeueTasks
{
  dispatch_async(_URLRequestQueue, ^{
    ...
    for (ABI46_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
      switch (task.status) {
        case ABI46_0_0RCTNetworkTaskFinished:
          if (!tasksToRemove) {
          ...
          self->_activeTasks--;
          break;
        case ABI46_0_0RCTNetworkTaskPending:
          break;
        case ABI46_0_0RCTNetworkTaskInProgress:
          // Check task isn't "stuck"
          if (task.requestToken == nil) {
            ABI46_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
            if (!tasksToRemove) {
              tasksToRemove = [NSMutableArray new];
            }
            [tasksToRemove addObject:task];
    ...
    // Start queued tasks
    for (ABI46_0_0RCTNetworkTask *task in self->_pendingTasks) {
      if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
        break;
      }
      if (task.status == ABI46_0_0RCTNetworkTaskPending) {
        [task start];
        self->_activeTasks++;
      }
    }
 ...
 */
- (ABI46_0_0RCTImageURLLoaderRequest *)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                        size:(CGSize)size
                                                       scale:(CGFloat)scale
                                                  resizeMode:(ABI46_0_0RCTResizeMode)resizeMode
                                                    priority:(ABI46_0_0RCTImageLoaderPriority)priority
                                                 attribution:(const ImageURLLoaderAttribution &)attribution
                                               progressBlock:(ABI46_0_0RCTImageLoaderProgressBlock)progressHandler
                                            partialLoadBlock:(ABI46_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
...
{
  {
    ...
    [NSURLProtocol setProperty:@"ABI46_0_0RCTImageLoader"
                        forKey:@"trackingName"
  ...
  id<ABI46_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
  ...
  void (^completionHandler)(NSError *, id, id, NSURLResponse *) = ^(NSError *error, id imageOrData, id imageMetadata, NSURLResponse *response) {
    ...
    if (ABI46_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
  ...
  if (loadHandler && !requiresScheduling) {
    if ([loadHandler conformsToProtocol:@protocol(ABI46_0_0RCTImageURLLoaderWithAttribution)]) {
      return [(id<ABI46_0_0RCTImageURLLoaderWithAttribution>)loadHandler loadImageForURL:request.URL
                                                                           size:size
                                                                          scale:scale
                                                                     resizeMode:resizeMode
                                                                      requestId:requestId
    ...
    ABI46_0_0RCTImageLoaderCancellationBlock cb = [loadHandler loadImageForURL:request.URL
                                                                 size:size
                                                                scale:scale
                                                           resizeMode:resizeMode
                                                      progressHandler:progressHandler
    ...
    return [[ABI46_0_0RCTImageURLLoaderRequest alloc] initWithRequestId:nil imageURL:request.URL cancellationBlock:cb];
  }
  ...
  __weak ABI46_0_0RCTImageLoader *weakSelf = self;
  dispatch_async(_URLRequestQueue, ^{
  ...
  if (atomic_load(cancelled.get()) || !strongSelf) {
    ...
    if (loadHandler) {
      ...
      if ([loadHandler conformsToProtocol:@protocol(ABI46_0_0RCTImageURLLoaderWithAttribution)]) {
        ABI46_0_0RCTImageURLLoaderRequest *loaderRequest = [(id<ABI46_0_0RCTImageURLLoaderWithAttribution>)loadHandler loadImageForURL:request.URL
                                                                                                                size:size
                                                                                                               scale:scale
                                                                                                          resizeMode:resizeMode
                                                                                                           requestId:requestId
  ...
  return [[ABI46_0_0RCTImageURLLoaderRequest alloc] initWithRequestId:requestId imageURL:request.URL cancellationBlock:^{
...
- (ABI46_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI46_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSURLResponse *response))completionHandler
{
  ABI46_0_0RCTNetworking *networking = [_moduleRegistry moduleForName:"Networking"];
  if (ABI46_0_0RCT_DEBUG && !networking) {
    ABI46_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                " import the ABI46_0_0RCTNetwork library in order to load images.",
                request.URL.absoluteString);
    return NULL;
  }

  // Check if networking module can load image
  if (ABI46_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
    ABI46_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
    return NULL;
  }

  // Use networking module to load image
  ABI46_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    // Check for system errors
    if (error) {
      completionHandler(error, nil, response);
      return;
    } else if (!response) {
      completionHandler(ABI46_0_0RCTErrorWithMessage(@"Response metadata error"), nil, response);
      return;
    } else if (!data) {
      completionHandler(ABI46_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, response);
      return;
    }
  ...
  __block ABI46_0_0RCTNetworkTask *task =
                     ...
                     completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                       ...
                       if (error || !response || !data) {
                         ...
                         if (error) {
                           ...
                           someError = ABI46_0_0RCTErrorWithMessage(@"Response metadata error");
                         } else {
                           someError = ABI46_0_0RCTErrorWithMessage(@"Unknown image download error");
                         }
...
#pragma mark - ABI46_0_0RCTImageLoaderWithAttributionProtocol
                                      ...
                                      completionBlock:(ABI46_0_0RCTImageLoaderCompletionBlockWithMetadata)completionBlock
{
  ...
  void (^completionHandler)(NSError *, id, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, id imageMetadata, BOOL cacheResult, NSURLResponse *response) {
    ...
    ABI46_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
  ...
  ABI46_0_0RCTImageURLLoaderRequest *loaderRequest = [self _loadImageOrDataWithURLRequest:imageURLRequest
                                                                             size:size
                                                                            scale:scale
                                                                       resizeMode:resizeMode
                                                                         priority:priority
  ...
  return [[ABI46_0_0RCTImageURLLoaderRequest alloc] initWithRequestId:loaderRequest.requestId imageURL:imageURLRequest.URL cancellationBlock:cancellationBlock];
}

- (BOOL)shouldEnablePerfLoggingForRequestUrl:(NSURL *)url {
  id<ABI46_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:url];
  if ([loadHandler respondsToSelector:@selector(shouldEnablePerfLogging)]) {
    return [(id<ABI46_0_0RCTImageURLLoaderWithAttribution>)loadHandler shouldEnablePerfLogging];
  }
  return NO;
}
...
- (void)trackURLImageVisibilityForRequest:(ABI46_0_0RCTImageURLLoaderRequest *)loaderRequest imageView:(UIView *)imageView
{
  ...
  id<ABI46_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:loaderRequest.imageURL];
  if ([loadHandler respondsToSelector:@selector(trackURLImageVisibilityForRequest:imageView:)]) {
    [(id<ABI46_0_0RCTImageURLLoaderWithAttribution>)loadHandler trackURLImageVisibilityForRequest:loaderRequest imageView:imageView];
  }
}
...
- (void)trackURLImageRequestDidDestroy:(ABI46_0_0RCTImageURLLoaderRequest *)loaderRequest
{
  ...
  if ([loadHandler respondsToSelector:@selector(trackURLImageRequestDidDestroy:)]) {
    [(id<ABI46_0_0RCTImageURLLoaderWithAttribution>)loadHandler trackURLImageRequestDidDestroy:loaderRequest];
  }
}
...
- (void)trackURLImageDidDestroy:(ABI46_0_0RCTImageURLLoaderRequest *)loaderRequest
{
  ...
  if ([loadHandler respondsToSelector:@selector(trackURLImageDidDestroy:)]) {
    [(id<ABI46_0_0RCTImageURLLoaderWithAttribution>)loadHandler trackURLImageDidDestroy:loaderRequest];
  }
}

#pragma mark - ABI46_0_0RCTImageLoaderProtocol 3/3

- (ABI46_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI46_0_0RCTResizeMode)resizeMode
...
{
  if (data.length == 0) {
    completionBlock(ABI46_0_0RCTErrorWithMessage(@"No image data"), nil);
    return ^{};
  }
  ...
  void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
    if (ABI46_0_0RCTIsMainQueue()) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          completionBlock(error, clipped ? ABI46_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
        }
      });
    } else if (!std::atomic_load(cancelled.get())) {
      completionBlock(error, clipped ? ABI46_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
  ...
  id<ABI46_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
  if (imageDecoder) {
    ...
    dispatch_block_t decodeBlock = ^{
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          ...
          UIImage *image = ABI46_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI46_0_0RCT_DEV
          CGSize imagePixelSize = ABI46_0_0RCTSizeInPixels(image.size, image.scale);
          CGSize screenPixelSize = ABI46_0_0RCTSizeInPixels(ABI46_0_0RCTScreenSize(), ABI46_0_0RCTScreenScale());
          if (imagePixelSize.width * imagePixelSize.height >
              screenPixelSize.width * screenPixelSize.height) {
            ABI46_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
          ...
          if (image) {
            ...
            NSError *finalError = ABI46_0_0RCTErrorWithMessage(errorMessage);
                                                       ...
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
  void (^completion)(NSError *, id, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, id imageMetadata, BOOL cacheResult, NSURLResponse *response) {
    ...
    if ([imageOrData isKindOfClass:[NSData class]]) {
      NSDictionary *meta = ABI46_0_0RCTGetImageMetadata(imageOrData);
...
- (NSDictionary *)getImageCacheStatus:(NSArray *)requests
{
  ...
  for (id request in requests) {
    NSURLRequest *urlRequest = [ABI46_0_0RCTConvert NSURLRequest:request];
    if (urlRequest) {
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  // Even better may be to implement this on the ABI46_0_0RCTImageURLLoader that would try to load it,
  // but we'd have to run the logic both in ABI46_0_0RCTPhotoLibraryImageLoader and
  // ABI46_0_0RCTAssetsLibraryRequestHandler. Once we drop iOS7 though, we'd drop
  // ABI46_0_0RCTAssetsLibraryRequestHandler and can move it there.
  ...
  for (id<ABI46_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI46_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI46_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI46_0_0RCTURLRequestHandler)] &&
...
{
  __block ABI46_0_0RCTImageLoaderCancellationBlock requestToken;
  requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
    ...
    if (ABI46_0_0RCTImageHasAlpha(image.CGImage)) {
      mimeType = @"image/png";
...
{
  if (requestToken) {
    ((ABI46_0_0RCTImageLoaderCancellationBlock)requestToken)();
  }
}
...
{
  return std::make_shared<ABI46_0_0facebook::ABI46_0_0React::NativeImageLoaderIOSSpecJSI>(params);
}

ABI46_0_0RCT_EXPORT_METHOD(getSize:(NSString *)uri resolve:(ABI46_0_0RCTPromiseResolveBlock)resolve reject:(ABI46_0_0RCTPromiseRejectBlock)reject)
{
  NSURLRequest *request = [ABI46_0_0RCTConvert NSURLRequest:uri];
...
ABI46_0_0RCT_EXPORT_METHOD(getSizeWithHeaders:(NSString *)uri
                  headers:(NSDictionary *)headers
                  resolve:(ABI46_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI46_0_0RCTPromiseRejectBlock)reject)
{
  NSURL *URL = [ABI46_0_0RCTConvert NSURL:uri];
...
ABI46_0_0RCT_EXPORT_METHOD(prefetchImage:(NSString *)uri
              resolve:(ABI46_0_0RCTPromiseResolveBlock)resolve
               reject:(ABI46_0_0RCTPromiseRejectBlock)reject)
{
...
ABI46_0_0RCT_EXPORT_METHOD(prefetchImageWithMetadata:(NSString *)uri
...
ABI46_0_0RCT_EXPORT_METHOD(queryCache:(NSArray *)uris
                  resolve:(ABI46_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI46_0_0RCTPromiseRejectBlock)reject)
{
 ...
 */
@implementation ABI46_0_0RCTBridge (ABI46_0_0RCTImageLoader)

- (ABI46_0_0RCTImageLoader *)imageLoader
{
  ABI46_0_0RCTLogWarn(@"Calling bridge.imageLoader is deprecated and will not work in newer versions of ABI46_0_0RN. Please update to the "
             "moduleForClass API or turboModuleRegistry API.");
  return [self moduleForClass:[ABI46_0_0RCTImageLoader class]];
}

@end

Class ABI46_0_0RCTImageLoaderCls(void)
{
  return ABI46_0_0RCTImageLoader.class;
}
