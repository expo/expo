...
#import "ABI46_0_0NSTextStorage+FontScaling.h"
#import <ABI46_0_0React/ABI46_0_0RCTTextView.h>

@implementation ABI46_0_0RCTTextShadowView
{
...
- (instancetype)initWithBridge:(ABI46_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI46_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI46_0_0RCTTextShadowViewMeasure);
    ABI46_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI46_0_0RCTTextShadowViewBaseline);
  }
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  // because the background color is drawn on the ABI46_0_0RCTTextView itself, as well
  // as on the glphy background draw step. By setting this to nil, we allow
  // the ABI46_0_0RCTTextView backgroundColor to be used, without affecting nested Text
  // components.
...
{
  if (ABI46_0_0YGNodeIsDirty(self.yogaNode)) {
    return;
  }
  ...
  NSNumber *tag = self.ABI46_0_0ReactTag;
  NSMutableArray<NSNumber *> *descendantViewTags = [NSMutableArray new];
  [textStorage enumerateAttribute:ABI46_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:
   ^(ABI46_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
     if (!shadowView) {
       return;
     }

     [descendantViewTags addObject:shadowView.ABI46_0_0ReactTag];
   }
  ];

  [_bridge.uiManager addUIBlock:^(ABI46_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI46_0_0RCTTextView *textView = (ABI46_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI46_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName range:NSMakeRange(0, textStorage.length)];
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI46_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                               inRange:NSMakeRange(0, attributedText.length)
                               options:0
                            usingBlock:
    ^(ABI46_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                                        ...
                                        exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage abi46_0_0_rct_scaleFontSizeToFitSize:size
...
- (void)layoutWithMetrics:(ABI46_0_0RCTLayoutMetrics)layoutMetrics
...
- (void)layoutSubviewsWithContext:(ABI46_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage enumerateAttribute:ABI46_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          ...
                          options:0
                       usingBlock:
    ^(ABI46_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
      if (!shadowView) {
        return;
      }
      ...
      CGRect frame = {{
        ABI46_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI46_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)
      }, {
        ABI46_0_0RCTRoundPixelValue(attachmentSize.width),
        ABI46_0_0RCTRoundPixelValue(attachmentSize.height)
      }};
      
      NSRange truncatedGlyphRange = [layoutManager truncatedGlyphRangeInLineFragmentForGlyphAtIndex:range.location];
      BOOL viewIsTruncated = NSIntersectionRange(range, truncatedGlyphRange).length != 0;

      ABI46_0_0RCTLayoutContext localLayoutContext = layoutContext;
      ...
      ABI46_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
      localLayoutMetrics.frame.origin = frame.origin; // Reinforcing a proper frame origin for the Shadow View.
      if (viewIsTruncated) {
        localLayoutMetrics.displayType = ABI46_0_0RCTDisplayTypeNone;
      }
...
static ABI46_0_0YGSize ABI46_0_0RCTTextShadowViewMeasure(ABI46_0_0YGNodeRef node, float width, ABI46_0_0YGMeasureMode widthMode, float height, ABI46_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
    widthMode == ABI46_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI46_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    heightMode == ABI46_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI46_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI46_0_0RCTTextShadowView *shadowTextView = (__bridge ABI46_0_0RCTTextShadowView *)ABI46_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
    MIN(ABI46_0_0RCTCeilPixelValue(size.width), maximumSize.width),
    MIN(ABI46_0_0RCTCeilPixelValue(size.height), maximumSize.height)
  };
  ...
  return (ABI46_0_0YGSize){
    ABI46_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
    ABI46_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)
  };
}

static float ABI46_0_0RCTTextShadowViewBaseline(ABI46_0_0YGNodeRef node, const float width, const float height)
{
  ABI46_0_0RCTTextShadowView *shadowTextView = (__bridge ABI46_0_0RCTTextShadowView *)ABI46_0_0YGNodeGetContext(node);

  CGSize size = (CGSize){
    ABI46_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    ABI46_0_0RCTCoreGraphicsFloatFromYogaFloat(height)
  };

  CGFloat lastBaseline = [shadowTextView lastBaselineForSize:size];

  return ABI46_0_0RCTYogaFloatFromCoreGraphicsFloat(lastBaseline);
}
