...
#import "ABI46_0_0RCTNetworkPlugins.h"

@interface ABI46_0_0RCTHTTPRequestHandler () <NSURLSessionDataDelegate, ABI46_0_0RCTTurboModule>

@end

static NSURLSessionConfigurationProvider urlSessionConfigurationProvider;

void ABI46_0_0RCTSetCustomNSURLSessionConfigurationProvider(NSURLSessionConfigurationProvider provider) {
  urlSessionConfigurationProvider = provider;
}

@implementation ABI46_0_0RCTHTTPRequestHandler
{
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  dispatch_once(&onceToken, ^{
    // technically, ABI46_0_0RCTHTTPRequestHandler can handle file:// as well,
    // but it's less efficient than using ABI46_0_0RCTFileRequestHandler
...
- (NSURLSessionDataTask *)sendRequest:(NSURLRequest *)request
                         withDelegate:(id<ABI46_0_0RCTURLRequestDelegate>)delegate
{
  std::lock_guard<std::mutex> lock(_mutex);
  // Lazy setup
  if (!_session && [self isValid]) {
    // You can override default NSURLSession instance property allowsCellularAccess (default value YES)
    //  by providing the following key to your ABI46_0_0RN project (edit ios/project/Info.plist file in Xcode):
    // <key>ABI46_0_0ReactNetworkForceWifiOnly</key>    <true/>
    ...
    NSNumber *useWifiOnly = [infoDictionary objectForKey:@"ABI46_0_0ReactNetworkForceWifiOnly"];
    ...
    if (urlSessionConfigurationProvider) {
      ...
      // Set allowsCellularAccess to NO ONLY if key ABI46_0_0ReactNetworkForceWifiOnly exists AND its value is YES
      if (useWifiOnly) {
...
{
  id<ABI46_0_0RCTURLRequestDelegate> delegate;
  {
    std::lock_guard<std::mutex> lock(_mutex);
...
Class ABI46_0_0RCTHTTPRequestHandlerCls(void) {
  return ABI46_0_0RCTHTTPRequestHandler.class;
}
