...
#import <ABI46_0_0React/ABI46_0_0RCTLinkingManager.h>
...
#import "ABI46_0_0RCTLinkingPlugins.h"

static NSString *const kOpenURLNotification = @"ABI46_0_0RCTOpenURLNotification";
...
@interface ABI46_0_0RCTLinkingManager() <ABI46_0_0NativeLinkingManagerSpec>
@end

@implementation ABI46_0_0RCTLinkingManager
    ...
    if (success) {
      resolve(@YES);
        ...
        if([URL.absoluteString hasPrefix:@"tel:"]){
          ABI46_0_0RCTLogWarn(@"Unable to open the Phone app in the simulator for telephone URLs. URL:  %@", URL);
          resolve(@NO);
        } else {
                  ...
                  reject:(__unused ABI46_0_0RCTPromiseRejectBlock)reject)
{
  ...
  BOOL canOpen = [ABI46_0_0RCTSharedApplication() canOpenURL:URL];
  ...
  if (canOpen) {
    ...
    if (querySchemes != nil && ([querySchemes containsObject:scheme] || [querySchemes containsObject:[scheme lowercaseString]])) {
      ...
      reject(ABI46_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@. Add %@ to LSApplicationQueriesSchemes in your Info.plist.", URL, scheme], nil);
    }
  } else {
    resolve(@NO);
  }
                  ...
                  reject:(__unused ABI46_0_0RCTPromiseRejectBlock)reject)
{
  ...
  resolve(ABI46_0_0RCTNullIfNil(initialURL.absoluteString));
}
...
ABI46_0_0RCT_EXPORT_METHOD(sendIntent:(NSString *)action
...
{
  return std::make_shared<ABI46_0_0facebook::ABI46_0_0React::NativeLinkingManagerSpecJSI>(params);
}

@end

Class ABI46_0_0RCTLinkingManagerCls(void) {
  return ABI46_0_0RCTLinkingManager.class;
}
