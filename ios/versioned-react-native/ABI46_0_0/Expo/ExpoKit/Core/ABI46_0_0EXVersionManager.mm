...
#import "ABI46_0_0EXVersionManager.h"
...
#import <ABI46_0_0React/ABI46_0_0RCTExceptionsManager.h>
...
#import <ABI46_0_0React/ABI46_0_0RCTAsyncLocalStorage.h>
...
#import <ABI46_0_0EXMediaLibrary/ABI46_0_0EXMediaLibraryImageLoader.h>
...
ABI46_0_0RCT_EXTERN NSDictionary<NSString *, NSDictionary *> *ABI46_0_0EXGetScopedModuleClasses(void);
ABI46_0_0RCT_EXTERN void ABI46_0_0EXRegisterScopedModule(Class, ...);

@interface ABI46_0_0RCTEventDispatcher (ABI46_0_0REAnimated)

- (void)setBridge:(ABI46_0_0RCTBridge*)bridge;

@end

// this is needed because ABI46_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI46_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
@interface ABI46_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI46_0_0EXVersionManager () <ABI46_0_0RCTTurboModuleManagerDelegate>

// is this the first time this ABI has been touched at runtime?
@property (nonatomic, assign) BOOL isFirstLoad;
@property (nonatomic, strong) NSDictionary *params;
@property (nonatomic, strong) ABI46_0_0EXManifestsManifest *manifest;
@property (nonatomic, strong) ABI46_0_0RCTTurboModuleManager *turboModuleManager;

@end

@implementation ABI46_0_0EXVersionManager
 ...
 *    @ABI46_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI46_0_0EXKernel *kernel
...
+ (void)load
{
  ...
  // don't have access to ABI46_0_0EXScopedModuleRegistry and so they can't register themselves.
  ABI46_0_0EXRegisterScopedModule([ABI46_0_0RNCWebViewManager class], ABI46_0_0EX_KERNEL_SERVICE_NONE, nil);
}

- (void)bridgeWillStartLoading:(id)bridge
{
  // We need to check DEBUG flag here because in ejected projects ABI46_0_0RCT_DEV is set only for ABI46_0_0React and not for ExpoKit to which this file belongs to.
  // It can be changed to just ABI46_0_0RCT_DEV once we deprecate ExpoKit and set that flag for the entire standalone project.
#if DEBUG || ABI46_0_0RCT_DEV
  if ([self _isDevModeEnabledForBridge:bridge]) {
    ...
    [[ABI46_0_0RCTPackagerConnection sharedPackagerConnection] reconnect:packagerServerHostPort];
  }
#endif

  // Manually send a "start loading" notif, since the real one happened uselessly inside the ABI46_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI46_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}

- (void)bridgeFinishedLoading:(id)bridge
{
  // Override the "Reload" button from Redbox to reload the app from manifest
  // Keep in mind that it is possible this will return a ABI46_0_0EXDisabledRedBox
  ABI46_0_0RCTRedBox *redBox = [self _moduleInstanceForBridge:bridge named:@"RedBox"];
...
{
  ABI46_0_0RCTDevSettings *devSettings = (ABI46_0_0RCTDevSettings *)[self _moduleInstanceForBridge:bridge named:@"DevSettings"];
  ...
  if (devSettings.isRemoteDebuggingAvailable && isDevModeEnabled) {
    ...
    items[@"dev-remote-debug"] =  @{
      ...
      @"isEnabled": @NO,
      @"detail": ABI46_0_0RCTTurboModuleEnabled() ? @"Remote debugging is unavailable while Turbo Modules are enabled. To debug remotely, please set `turboModules` to false in app.json." : [NSNull null]
    };
  }
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI46_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI46_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI46_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return [[ABI46_0_0RCTPackagerConnection sharedPackagerConnection] addNotificationHandler:handler queue:queue forMethod:method];
}
#endif
...
{
  return ([ABI46_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI46_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI46_0_0RCTEnableTurboModule([self.manifest.experiments[@"turboModules"] boolValue]);
  ABI46_0_0RCTSetFatalHandler(fatalHandler);
  ABI46_0_0RCTSetLogThreshold((ABI46_0_0RCTLogLevel) threshold);
...
- (NSArray *)extraModulesForBridge:(id)bridge
{
  ...
  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI46_0_0EXAppState alloc] init],
                                    [[ABI46_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI46_0_0EXStatusBarManager alloc] init],
                                    ]];
  ...
  if (params[@"testEnvironment"]) {
    ABI46_0_0EXTestEnvironment testEnvironment = (ABI46_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI46_0_0EXTestEnvironmentNone) {
      ABI46_0_0EXTest *testModule = [[ABI46_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  ABI46_0_0EXModuleRegistryProvider *moduleRegistryProvider = [[ABI46_0_0EXModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI46_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI46_0_0EXModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ABI46_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI46_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];
#pragma clang diagnostic pop

  ABI46_0_0EXModuleRegistry *moduleRegistry = [moduleRegistryAdapter moduleRegistryForParams:params
                                                        forExperienceStableLegacyId:self.manifest.stableLegacyId
                                                                           scopeKey:self.manifest.scopeKey
                                                                           manifest:self.manifest
                                                                 withKernelServices:services];

  // Adding ABI46_0_0EXNativeModulesProxy with the custom moduleRegistry.
  ABI46_0_0EXNativeModulesProxy *expoNativeModulesProxy = [[ABI46_0_0EXNativeModulesProxy alloc] initWithCustomModuleRegistry:moduleRegistry];
  ...
  if (!ABI46_0_0RCTTurboModuleEnabled()) {
    ...
    id exceptionsManager = [self getModuleInstanceFromClass:ABI46_0_0RCTExceptionsManagerCls()];
    ...
    [extraModules addObject:[self getModuleInstanceFromClass:ABI46_0_0RCTAsyncLocalStorageCls()]];
  }
...
- (NSArray *)_newScopedModulesForServices:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  NSDictionary<NSString *, NSDictionary *> *ABI46_0_0EXScopedModuleClasses = ABI46_0_0EXGetScopedModuleClasses();
  if (ABI46_0_0EXScopedModuleClasses) {
    [ABI46_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
...
- (Class)getModuleClassFromName:(const char *)name
{
  if (std::string(name) == "DevSettings") {
    return ABI46_0_0EXDevSettings.class;
  }
  if (std::string(name) == "DevMenu") {
    if (![_params[@"isStandardDevMenuAllowed"] boolValue] || ![_params[@"isDeveloper"] boolValue]) {
      // non-kernel, or non-development kernel, uses expo menu instead of ABI46_0_0RCTDevMenu
      return ABI46_0_0EXDisabledDevMenu.class;
    }
  }
  if (std::string(name) == "RedBox") {
    if (![_params[@"isDeveloper"] boolValue]) {
      // user-facing (not debugging).
      // additionally disable ABI46_0_0RCTRedBox
      return ABI46_0_0EXDisabledRedBox.class;
    }
  }
  return ABI46_0_0RCTCoreModulesClassProvider(name);
}
...
- (id<ABI46_0_0RCTTurboModule>)getModuleInstanceFromClass:(Class)moduleClass
{
  ...
  if (moduleClass == ABI46_0_0RCTImageLoader.class) {
    return [[moduleClass alloc] initWithRedirectDelegate:nil loadersProvider:^NSArray<id<ABI46_0_0RCTImageURLLoader>> *(ABI46_0_0RCTModuleRegistry *) {
      return @[[ABI46_0_0RCTLocalAssetImageLoader new], [ABI46_0_0EXMediaLibraryImageLoader new]];
    } decodersProvider:^NSArray<id<ABI46_0_0RCTImageDataDecoder>> *(ABI46_0_0RCTModuleRegistry *) {
      return @[[ABI46_0_0RCTGIFImageDecoder new]];
    }];
  } else if (moduleClass == ABI46_0_0RCTNetworking.class) {
    return [[moduleClass alloc] initWithHandlersProvider:^NSArray<id<ABI46_0_0RCTURLRequestHandler>> *(ABI46_0_0RCTModuleRegistry *) {
      return @[
  ...
  // Expo-specific
  if (moduleClass == ABI46_0_0EXDevSettings.class) {
  ...
  } else if (moduleClass == ABI46_0_0RCTExceptionsManagerCls()) {
    ...
    if (exceptionsManagerDelegate) {
      ...
      ABI46_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
    }
  } else if (moduleClass == ABI46_0_0RCTAsyncLocalStorageCls()) {
...
- (std::shared_ptr<ABI46_0_0facebook::ABI46_0_0React::TurboModule>)getTurboModule:(const std::string &)name
                                                     initParams:(const ABI46_0_0facebook::ABI46_0_0React::ObjCTurboModule::InitParams &)params
{
  // TODO: ADD
  return nullptr;
}
...
- (void *)versionedJsExecutorFactoryForBridge:(ABI46_0_0RCTBridge *)bridge
{
  ...
  const auto executor = [ABI46_0_0EXWeak_self, bridge](ABI46_0_0facebook::jsi::Runtime &runtime) {
    if (!bridge) {
      return;
    }
