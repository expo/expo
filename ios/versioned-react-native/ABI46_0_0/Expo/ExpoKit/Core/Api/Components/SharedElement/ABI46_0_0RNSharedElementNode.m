//
//  ABI46_0_0RNSharedElementNode.m
...
@interface ABI46_0_0RNSharedElementNodeResolvedSource : NSObject
...
@end
@implementation ABI46_0_0RNSharedElementNodeResolvedSource
- (instancetype)initWithView:(UIView*) view hasChildImageView:(BOOL)hasChildImageView
{
  _view = view;
...
+ (instancetype)sourceWithView:(UIView*) view {
  return [[ABI46_0_0RNSharedElementNodeResolvedSource alloc]initWithView:view hasChildImageView:NO];
}
+ (instancetype)sourceWithView:(UIView*) view hasChildImageView:(BOOL)hasChildImageView {
  return [[ABI46_0_0RNSharedElementNodeResolvedSource alloc]initWithView:view hasChildImageView:hasChildImageView];
}
- (UIView*) contentView {
...
@implementation ABI46_0_0RNSharedElementNode
{
  long _refCount;
  long _hideRefCount;
  
  NSMutableArray* _contentRequests;
  ABI46_0_0RNSharedElementContent* _contentCache;
  CFTimeInterval _contentCacheTimeInterval;
  
  NSMutableArray* _styleRequests;
  ABI46_0_0RNSharedElementStyle* _styleCache;
  CFTimeInterval _styleCacheTimeInterval;
  
  CADisplayLink* _displayLink;
  
  __weak UIView* _sourceView;
  ABI46_0_0RNSharedElementNodeResolvedSource* _resolvedSource;
}
...
{
  _ABI46_0_0ReactTag = ABI46_0_0ReactTag;
  _sourceView = view;
  _isParent = isParent;
  _refCount = 1;
  _hideRefCount = 0;
  ...
  _displayLink = nil;
  _resolvedSource = [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:nil];
...
{
  ABI46_0_0RNSharedElementNodeResolvedSource* resolvedSource = noReset
  ? [ABI46_0_0RNSharedElementNode resolveSource:_isParent ? _sourceView.subviews.firstObject : _sourceView]
  : [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:nil];
...
+ (ABI46_0_0RNSharedElementNodeResolvedSource*) resolveSource:(UIView*) view
{
  if (view == nil || _imageResolvers == nil) return [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:view];
  
  // If the view is an ImageView, then use that.
  if ([ABI46_0_0RNSharedElementContent isKindOfImageView:view]) {
    return [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:view];
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if ([ABI46_0_0RNSharedElementContent isKindOfImageView:subview]) {
      ...
      if (CGRectEqualToRect(subview.frame, bounds)) {
        ...
        return [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:view hasChildImageView:YES];
      }
    }
  }
  
  // Resolve the underlying ImageViews of well known
  // ABI46_0_0React-native libs (e.g. ABI46_0_0React-native-fast-image)
  for (NSArray* imageResolver in _imageResolvers) {
    ...
    if (foundImageView != nil) {
      return [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:foundImageView];
    }
  }
  return [ABI46_0_0RNSharedElementNodeResolvedSource sourceWithView:view];
}
...
{
  if ([ABI46_0_0RNSharedElementContent isKindOfImageView:view]) {
    UIImageView* imageView = [ABI46_0_0RNSharedElementContent imageViewFromView:view];
...
- (void) requestContent:(__weak id <ABI46_0_0RNSharedElementDelegate>) delegate
{
...
- (void) updateContent
{
  ...
  [self updateResolvedSource:YES];
  ABI46_0_0RNSharedElementNodeResolvedSource* resolvedSource = _resolvedSource;
  ...
  ABI46_0_0RNSharedElementContent* content;
  if ([ABI46_0_0RNSharedElementContent isKindOfImageView:contentView]) {
    UIImageView* imageView = [ABI46_0_0RNSharedElementContent imageViewFromView:contentView];
    ...
    content = [[ABI46_0_0RNSharedElementContent alloc]initWithData:image type:ABI46_0_0RNSharedElementContentTypeRawImage insets:imageInsets];
  }
  else if ([NSStringFromClass(view.class) isEqualToString:@"ABI46_0_0RCTView"] && !view.subviews.count) {
    UIView* dummyView = [[UIView alloc]init];
    content = [[ABI46_0_0RNSharedElementContent alloc]initWithData:dummyView type:ABI46_0_0RNSharedElementContentTypeSnapshotView insets:UIEdgeInsetsZero];
  }
  else {
  ...
  /*else {
   ...
   BOOL res = [view drawViewHierarchyInRect:bounds afterScreenUpdates:NO]; // NEVER USE YES, IT CREATED VISUAL ABI46_0_0ARTEFACTS ON THE CREEN
   UIImage* image = res ? UIGraphicsGetImageFromCurrentImageContext() : nil;
   UIGraphicsEndImageContext();
   NSLog(@"drawViewHierarchyInRect: RESULT: %li", res);
   content = image;
   contentType = ABI46_0_0RNSharedElementContentTypeSnapshotImage;
   }*/
  ...
  for (__weak id <ABI46_0_0RNSharedElementDelegate> delegate in delegates) {
    if (delegate != nil) {
...
- (void) requestStyle:(__weak id <ABI46_0_0RNSharedElementDelegate>) delegate
{
...
- (void) updateStyle
{
  ...
  // Create style
  ABI46_0_0RNSharedElementStyle* style = [[ABI46_0_0RNSharedElementStyle alloc]initWithView:view];
  style.layout = layout;
  if ([ABI46_0_0RNSharedElementContent isKindOfImageView:contentView]) {
    UIImageView* imageView = [ABI46_0_0RNSharedElementContent imageViewFromView:contentView];
