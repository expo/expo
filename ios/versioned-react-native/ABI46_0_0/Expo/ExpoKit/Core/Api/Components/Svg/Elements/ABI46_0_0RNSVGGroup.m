...
@implementation ABI46_0_0RNSVGGroup
{
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI46_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI46_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI46_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI46_0_0RNSVGMask class]]) {
...
- (ABI46_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI46_0_0RNSVGNode class]];
    ...
    for (ABI46_0_0RNSVGView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI46_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI46_0_0RNSVGMask class]]) {
                continue;
            }
            ABI46_0_0RNSVGNode* svgNode = (ABI46_0_0RNSVGNode*)node;
            if (event) {
            ...
            ABI46_0_0RNSVGPlatformView *hitChild = [svgNode hitTest:transformed withEvent:event];
            if (hitChild) {
                svgNode.active = YES;
            ...
            ABI46_0_0RNSVGSvgView* svgView = (ABI46_0_0RNSVGSvgView*)node;
            ABI46_0_0RNSVGPlatformView *hitChild = [svgView hitTest:transformed withEvent:event];
            if (hitChild) {
                return hitChild;
            }
        }
    ...
    ABI46_0_0RNSVGPlatformView *hitSelf = [super hitTest:transformed withEvent:event];
    if (hitSelf) {
        return hitSelf;
    }
...
{
    [self traverseSubviews:^(__kindof ABI46_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI46_0_0RNSVGRenderable class]]) {
            [(ABI46_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
