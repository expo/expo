...
@interface ABI46_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI46_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI46_0_0RNSVGGroup *textRoot;
@end

@implementation ABI46_0_0RNSVGNode
{
    ABI46_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI46_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI46_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI46_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)insertABI46_0_0ReactSubview:(ABI46_0_0RNSVGView *)subview atIndex:(NSInteger)atIndex
{
...
- (void)removeABI46_0_0ReactSubview:(ABI46_0_0RNSVGView *)subview
{
...
{
    // Do nothing, as subviews are inserted by insertABI46_0_0ReactSubview:
}
...
- (void)invalidate
{
    ...
    _dirty = true;
    id<ABI46_0_0RNSVGContainer> container = (id<ABI46_0_0RNSVGContainer>)self.superview;
...
{
    [self clearPath];
    for (__kindof ABI46_0_0RNSVGNode *node in self.subviews) {
...
{
    ABI46_0_0RNSVGNode* node = self;
    while (node != nil) {
        ABI46_0_0RNSVGPlatformView* parent = [node superview];

        if (![parent isKindOfClass:[ABI46_0_0RNSVGNode class]]) {
            return;
        }
        node = (ABI46_0_0RNSVGNode*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (ABI46_0_0RNSVGGroup *)textRoot
{
    ...
    ABI46_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI46_0_0RNSVGGroup class]] && [((ABI46_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI46_0_0RNSVGGroup*)node;
            break;
        }
...
{
    ABI46_0_0RNSVGNode* parent = (ABI46_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI46_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI46_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI46_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)ABI46_0_0ReactSetInheritedBackgroundColor:(ABI46_0_0RNSVGColor *)inheritedBackgroundColor
{
...
- (void)setClipRule:(ABI46_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI46_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if (_cachedClipPath) {
            CGPathRelease(_cachedClipPath);
        }
...
- (ABI46_0_0RNSVGSvgView *)svgView
{
    if (_svgView) {
        return _svgView;
    }

    __kindof ABI46_0_0RNSVGPlatformView *parent = self.superview;

    if ([parent class] == [ABI46_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI46_0_0RNSVGNode class]]) {
        _svgView = ((ABI46_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI46_0_0RCTLogError(@"ABI46_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI46_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI46_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI46_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
