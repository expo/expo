...
const CLLocationDegrees ABI46_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI46_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI46_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI46_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI46_0_0AIRMap
{
    UIView *_legalLabel;
    ...
    // Array to manually track ABI46_0_0RN subviews
    //
    // ABI46_0_0AIRMap implicitly creates subviews that aren't regular ABI46_0_0RN children
    // (ABI46_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI46_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI46_0_0React-native/blob/v0.16.0/ABI46_0_0React/Modules/ABI46_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI46_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI46_0_0React-native/blob/v0.16.0/Libraries/Text/ABI46_0_0RCTTextField.m#L20
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI46_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI46_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI46_0_0ReactSubview:(id<ABI46_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI46_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI46_0_0AIRMapPolyline class]]) {
        ((ABI46_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI46_0_0AIRMapPolygon class]]) {
        ((ABI46_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI46_0_0AIRMapCircle class]]) {
        ((ABI46_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI46_0_0AIRMapUrlTile class]]) {
        ((ABI46_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI46_0_0AIRMapWMSTile class]]) {
        ((ABI46_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI46_0_0AIRMapLocalTile class]]) {
        ((ABI46_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI46_0_0AIRMapOverlay class]]) {
        ((ABI46_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ...
        return [marker isKindOfClass:[ABI46_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI46_0_0AIRMapMarker*) markerForCallout:(ABI46_0_0AIRMapCallout*)callout {
    ABI46_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI46_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI46_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI46_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI46_0_0AIRMapCalloutSubview* calloutSubview = nil;
        ABI46_0_0AIRMapCallout* callout = nil;
        ABI46_0_0AIRMapMarker* marker = nil;
        
        UIView* tmp = touchedView;
        while (tmp && tmp != win && tmp != self.calloutView) {
            if ([tmp respondsToSelector:@selector(onPress)]) {
                calloutSubview = (ABI46_0_0AIRMapCalloutSubview*) tmp;
            }
            if ([tmp isKindOfClass:[ABI46_0_0AIRMapCallout class]]) {
                callout = (ABI46_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI46_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI46_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
...
- (void)cacheViewIfNeeded {
    // https://github.com/ABI46_0_0React-native-maps/ABI46_0_0React-native-maps/issues/3100
