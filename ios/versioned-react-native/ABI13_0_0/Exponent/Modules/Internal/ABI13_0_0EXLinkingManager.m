...
NSString * const ABI13_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI13_0_0EXLinkingManager ()
...
@implementation ABI13_0_0EXLinkingManager

+ (NSString *)moduleName { return @"ABI13_0_0RCTLinkingManager"; }
...
#pragma mark - ABI13_0_0RCTEventEmitter methods

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI13_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI13_0_0RCTFatal(ABI13_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI13_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}

ABI13_0_0RCT_EXPORT_METHOD(openURL:(NSURL *)URL
                  ...
                  reject:(ABI13_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _isExponentUrl:URL]) {
    ...
    BOOL opened = [ABI13_0_0RCTSharedApplication() openURL:URL];
    if (opened) {
      resolve(nil);
    } else {
      reject(ABI13_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@", URL], nil);
    }
  }
}

ABI13_0_0RCT_EXPORT_METHOD(canOpenURL:(NSURL *)URL
                  resolve:(ABI13_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI13_0_0RCTPromiseRejectBlock)reject)
{
  BOOL canOpen = [self _isExponentUrl:URL];
  if (!canOpen) {
    canOpen = [ABI13_0_0RCTSharedApplication() canOpenURL:URL];
  }
  resolve(@(canOpen));
}

ABI13_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI13_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI13_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI13_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
...
- (BOOL)_isExponentUrl: (NSURL *)url
{
  ...
  NSString *versionsPath = [[NSBundle mainBundle] pathForResource:@"ABI13_0_0EXSDKVersions" ofType:@"plist"];
