...
#include <cxxReactABI13_0_0/ABI13_0_0MethodCall.h>
...
using namespace facebook::ReactABI13_0_0;
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI13_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI13_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI13_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
