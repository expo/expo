...
#define ABI13_0_0EXPECT_JSAE(statement, exstr) do {                              \
    try {                                                               \
      statement;                                                        \
      FAIL() << "Expected JsArgumentException(" << (exstr) << ") not thrown"; \
    } catch (const JsArgumentException& ex) {                           \
      ABI13_0_0EXPECT_EQ(ex.what(), std::string(exstr));                         \
    }                                                                   \
...
TEST(JsArgumentHelpersTest, args) {
  ...
  ABI13_0_0EXPECT_EQ(jsArgAsBool(args, 0), aBool);
  ABI13_0_0EXPECT_EQ(jsArgAsInt(args, 1), anInt);
  ABI13_0_0EXPECT_EQ(jsArgAsDouble(args, 2), aDouble);
  ABI13_0_0EXPECT_EQ(jsArgAsString(args, 3), aString);
  ABI13_0_0EXPECT_EQ(jsArgAsArray(args, 4), anArray);
  ABI13_0_0EXPECT_EQ(jsArgAsObject(args, 5), anObject);

  // const args
  const folly::dynamic& cargs = args;
  const folly::dynamic& a4 = jsArgAsArray(cargs, 4);
  ABI13_0_0EXPECT_EQ(a4, anArray);
  ABI13_0_0EXPECT_EQ(jsArgAsObject(cargs, 5), anObject);

  // helpers returning dynamic should return same object without copying
  ABI13_0_0EXPECT_EQ(&jsArgAsArray(args, 4), &(args[4]));
  ABI13_0_0EXPECT_EQ(&jsArgAsArray(cargs, 4), &(args[4]));
  ...
  ABI13_0_0EXPECT_EQ(jsArgN(args, 3, &folly::dynamic::getString), aString);
  ABI13_0_0EXPECT_EQ(jsArg(args[3], &folly::dynamic::getString), aString);

  // conversions
  ABI13_0_0EXPECT_EQ(jsArgAsDouble(args, 1), anInt * 1.0);
  ABI13_0_0EXPECT_EQ(jsArgAsString(args, 1), aNumericString);
  ABI13_0_0EXPECT_EQ(jsArgAsInt(args, 6), anInt);

  // Test exception messages.

  // out_of_range
  ABI13_0_0EXPECT_JSAE(jsArgAsBool(args, 7),
  ...
  try {
    ...
    ABI13_0_0EXPECT_GT(exwhat.size(), exhead.size());
    ABI13_0_0EXPECT_GT(exwhat.size(), extail.size());

    ABI13_0_0EXPECT_TRUE(std::equal(exhead.cbegin(), exhead.cend(), exwhat.cbegin()))
      << "JsArgumentException('" << exwhat << "') does not begin with '" << exhead << "'";
    ABI13_0_0EXPECT_TRUE(std::equal(extail.crbegin(), extail.crend(), exwhat.crbegin()))
      << "JsArgumentException('" << exwhat << "') does not end with '" << extail << "'";
  }
  // inconvertible types
  ABI13_0_0EXPECT_JSAE(jsArgAsArray(args, 2),
              "Argument 3 of type double is not required type Array");
  ABI13_0_0EXPECT_JSAE(jsArgAsInt(args, 4),
  ...
  ABI13_0_0EXPECT_JSAE(jsArgAsObject(args, 4),
