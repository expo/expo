...
#include "ABI13_0_0CxxNativeModule.h"
...
#include <cxxReactABI13_0_0/ABI13_0_0JsArgumentHelpers.h>
...
void CxxNativeModule::invoke(ExecutorToken token, unsigned int ReactABI13_0_0MethodId, folly::dynamic&& params) {
  if (ReactABI13_0_0MethodId >= methods_.size()) {
    throw std::invalid_argument(
      folly::to<std::string>("methodId ", ReactABI13_0_0MethodId, " out of range [0..", methods_.size(), "]"));
  }
  if (!params.isArray()) {
    throw std::invalid_argument(
  ...
  const auto& method = methods_[ReactABI13_0_0MethodId];
