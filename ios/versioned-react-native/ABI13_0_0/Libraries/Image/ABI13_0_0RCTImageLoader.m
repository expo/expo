...
#import <ReactABI13_0_0/ABI13_0_0RCTImageLoader.h>
#import <ReactABI13_0_0/ABI13_0_0RCTLog.h>
#import <ReactABI13_0_0/ABI13_0_0RCTNetworking.h>
#import <ReactABI13_0_0/ABI13_0_0RCTUtils.h>

#import "ABI13_0_0RCTImageCache.h"
#import "ABI13_0_0RCTImageUtils.h"

@implementation UIImage (ReactABI13_0_0)

- (CAKeyframeAnimation *)ReactABI13_0_0KeyframeAnimation
{
  return objc_getAssociatedObject(self, _cmd);
}

- (void)setReactABI13_0_0KeyframeAnimation:(CAKeyframeAnimation *)ReactABI13_0_0KeyframeAnimation
{
  objc_setAssociatedObject(self, @selector(ReactABI13_0_0KeyframeAnimation), ReactABI13_0_0KeyframeAnimation, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

@end

@implementation ABI13_0_0RCTImageLoader
{
  NSArray<id<ABI13_0_0RCTImageURLLoader>> *_loaders;
  NSArray<id<ABI13_0_0RCTImageDataDecoder>> *_decoders;
  NSOperationQueue *_imageDecodeQueue;
  dispatch_queue_t _URLRequestQueue;
  id<ABI13_0_0RCTImageCache> _imageCache;
...
- (void)setUp
{
  ...
  _URLRequestQueue = dispatch_queue_create("com.facebook.ReactABI13_0_0.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (id<ABI13_0_0RCTImageCache>)imageCache
{
  if (!_imageCache) {
    //set up with default cache
    _imageCache = [ABI13_0_0RCTImageCache new];
  }
  return _imageCache;
}

- (void)setImageCache:(id<ABI13_0_0RCTImageCache>)cache
{
  if (_imageCache) {
    ABI13_0_0RCTLogWarn(@"ABI13_0_0RCTImageCache was already set and has now been overriden.");
  }
  _imageCache = cache;
}

- (id<ABI13_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
  ...
  if (!_loaders) {
    ...
    ABI13_0_0RCTAssert(_bridge, @"Bridge not set");
    _loaders = [[_bridge modulesConformingToProtocol:@protocol(ABI13_0_0RCTImageURLLoader)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI13_0_0RCTImageURLLoader> a, id<ABI13_0_0RCTImageURLLoader> b) {
  ...
  if (ABI13_0_0RCT_DEBUG) {
    ...
    id<ABI13_0_0RCTImageURLLoader> previousLoader = nil;
    for (id<ABI13_0_0RCTImageURLLoader> loader in _loaders) {
      ...
      if ([loader canLoadImageURL:URL]) {
        if (previousLoader) {
          if (priority == previousPriority) {
            ABI13_0_0RCTLogError(@"The ABI13_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI13_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
  if (!_maxConcurrentLoadingTasks) {
    [self setUp];
  }
  ...
  if (!_decoders) {
    ...
    ABI13_0_0RCTAssert(_bridge, @"Bridge not set");
    _decoders = [[_bridge modulesConformingToProtocol:@protocol(ABI13_0_0RCTImageDataDecoder)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI13_0_0RCTImageDataDecoder> a, id<ABI13_0_0RCTImageDataDecoder> b) {
  ...
  if (ABI13_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI13_0_0RCTImageDataDecoder> previousDecoder = nil;
    for (id<ABI13_0_0RCTImageDataDecoder> decoder in _decoders) {
      ...
      if ([decoder canDecodeImageData:data]) {
        if (previousDecoder) {
          if (priority == previousPriority) {
            ABI13_0_0RCTLogError(@"The ABI13_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI13_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI13_0_0RCTResizeMode resizeMode)
{
  ...
  CAKeyframeAnimation *animation = image.ReactABI13_0_0KeyframeAnimation;
  CGRect targetSize = ABI13_0_0RCTTargetRect(image.size, size, scale, resizeMode);
  CGAffineTransform transform = ABI13_0_0RCTTransformFromTargetRect(image.size, targetSize);
  image = ABI13_0_0RCTTransformImage(image, size, scale, transform);
  image.ReactABI13_0_0KeyframeAnimation = animation;
  return image;
}

- (ABI13_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                  callback:(ABI13_0_0RCTImageLoaderCompletionBlock)callback
{
  return [self loadImageWithURLRequest:imageURLRequest
                                  size:CGSizeZero
                                 scale:1
                               clipped:YES
                            resizeMode:ABI13_0_0RCTResizeModeStretch
                         progressBlock:nil
...
- (void)dequeueTasks
{
  dispatch_async(_URLRequestQueue, ^{
    ...
    for (ABI13_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
      switch (task.status) {
        case ABI13_0_0RCTNetworkTaskFinished:
          if (!tasksToRemove) {
          ...
          self->_activeTasks--;
          break;
        case ABI13_0_0RCTNetworkTaskPending:
          break;
        case ABI13_0_0RCTNetworkTaskInProgress:
          // Check task isn't "stuck"
          if (task.requestToken == nil) {
            ABI13_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
            if (!tasksToRemove) {
              tasksToRemove = [NSMutableArray new];
            }
            [tasksToRemove addObject:task];
    ...
    // Start queued tasks
    for (ABI13_0_0RCTNetworkTask *task in self->_pendingTasks) {
      if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
        break;
      }
      if (task.status == ABI13_0_0RCTNetworkTaskPending) {
        [task start];
        self->_activeTasks++;
      }
    }
 ...
 */
- (ABI13_0_0RCTImageLoaderCancellationBlock)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                             size:(CGSize)size
                                                            scale:(CGFloat)scale
                                                       resizeMode:(ABI13_0_0RCTResizeMode)resizeMode
                                                    progressBlock:(ABI13_0_0RCTImageLoaderProgressBlock)progressHandler
                                                 partialLoadBlock:(ABI13_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
                                                  completionBlock:(void (^)(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate))completionBlock
{
  {
    NSMutableURLRequest *mutableRequest = [request mutableCopy];
    [NSURLProtocol setProperty:@"ABI13_0_0RCTImageLoader"
                        forKey:@"trackingName"
  ...
  id<ABI13_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
  ...
  void (^completionHandler)(NSError *, id, NSString *) = ^(NSError *error, id imageOrData, NSString *fetchDate) {
    ...
    if (ABI13_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
  ...
  __weak ABI13_0_0RCTImageLoader *weakSelf = self;
  dispatch_async(_URLRequestQueue, ^{
...
- (ABI13_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI13_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSString *fetchDate))completionHandler
{
  // Check if networking module is available
  if (ABI13_0_0RCT_DEBUG && ![_bridge respondsToSelector:@selector(networking)]) {
    ABI13_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                " import the ABI13_0_0RCTNetwork library in order to load images.",
                request.URL.absoluteString);
    return NULL;
  }

  ABI13_0_0RCTNetworking *networking = [_bridge networking];

  // Check if networking module can load image
  if (ABI13_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
    ABI13_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
    return NULL;
  }

  // Use networking module to load image
  ABI13_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    // Check for system errors
    if (error) {
      completionHandler(error, nil, nil);
      return;
    } else if (!response) {
      completionHandler(ABI13_0_0RCTErrorWithMessage(@"Response metadata error"), nil, nil);
      return;
    } else if (!data) {
      completionHandler(ABI13_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, nil);
      return;
    }
  ...
  __block ABI13_0_0RCTNetworkTask *task =
  [networking networkTaskWithRequest:request
    ...
    if (error || !response || !data) {
      ...
      if (error) {
        ...
        someError = ABI13_0_0RCTErrorWithMessage(@"Response metadata error");
      } else {
        someError = ABI13_0_0RCTErrorWithMessage(@"Unknown image download error");
      }
...
- (ABI13_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                ...
                                                resizeMode:(ABI13_0_0RCTResizeMode)resizeMode
                                             progressBlock:(ABI13_0_0RCTImageLoaderProgressBlock)progressBlock
                                          partialLoadBlock:(ABI13_0_0RCTImageLoaderPartialLoadBlock)partialLoadBlock
                                           completionBlock:(ABI13_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  __block volatile uint32_t cancelled = 0;
  __block dispatch_block_t cancelLoad = nil;
  ...
  void (^completionHandler)(NSError *, id, BOOL, NSString *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate) {
    ...
    ABI13_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
...
- (ABI13_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI13_0_0RCTResizeMode)resizeMode
...
{
  if (data.length == 0) {
    completionBlock(ABI13_0_0RCTErrorWithMessage(@"No image data"), nil);
    return ^{};
  }
  ...
  void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
    if (ABI13_0_0RCTIsMainQueue()) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!cancelled) {
          completionBlock(error, clipped ? ABI13_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
        }
      });
    } else if (!cancelled) {
      completionBlock(error, clipped ? ABI13_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
  ...
  id<ABI13_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
  if (imageDecoder) {
    ...
    dispatch_block_t decodeBlock = ^{
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!cancelled) {
          ...
          UIImage *image = ABI13_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI13_0_0RCT_DEV
          CGSize imagePixelSize = ABI13_0_0RCTSizeInPixels(image.size, image.scale);
          CGSize screenPixelSize = ABI13_0_0RCTSizeInPixels(ABI13_0_0RCTScreenSize(), ABI13_0_0RCTScreenScale());
          if (imagePixelSize.width * imagePixelSize.height >
              screenPixelSize.width * screenPixelSize.height) {
            ABI13_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
          ...
          if (image) {
            ...
            NSError *finalError = ABI13_0_0RCTErrorWithMessage(errorMessage);
...
- (ABI13_0_0RCTImageLoaderCancellationBlock)getImageSizeForURLRequest:(NSURLRequest *)imageURLRequest
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
  void (^completion)(NSError *, id, BOOL, NSString *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate) {
    ...
    if ([imageOrData isKindOfClass:[NSData class]]) {
      NSDictionary *meta = ABI13_0_0RCTGetImageMetadata(imageOrData);
      size = (CGSize){
...
#pragma mark - ABI13_0_0RCTURLRequestHandler

- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  NSURL *requestURL = request.URL;
  for (id<ABI13_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI13_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI13_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI13_0_0RCTURLRequestHandler)] &&
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI13_0_0RCTURLRequestDelegate>)delegate
{
  __block ABI13_0_0RCTImageLoaderCancellationBlock requestToken;
  requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
    ...
    if (ABI13_0_0RCTImageHasAlpha(image.CGImage)) {
      mimeType = @"image/png";
...
{
  if (requestToken) {
    ((ABI13_0_0RCTImageLoaderCancellationBlock)requestToken)();
  }
}

@end

@implementation ABI13_0_0RCTBridge (ABI13_0_0RCTImageLoader)

- (ABI13_0_0RCTImageLoader *)imageLoader
{
  return [self moduleForClass:[ABI13_0_0RCTImageLoader class]];
}
