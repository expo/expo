...
NSString *const ABI13_0_0RCTShadowViewAttributeName = @"ABI13_0_0RCTShadowViewAttributeName";
NSString *const ABI13_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI13_0_0RCTReactABI13_0_0TagAttributeName = @"ReactABI13_0_0TagAttributeName";

CGFloat const ABI13_0_0RCTTextAutoSizeDefaultMinimumFontScale       = 0.5f;
CGFloat const ABI13_0_0RCTTextAutoSizeWidthErrorMargin              = 0.05f;
CGFloat const ABI13_0_0RCTTextAutoSizeHeightErrorMargin             = 0.025f;
CGFloat const ABI13_0_0RCTTextAutoSizeGranularity                   = 0.001f;

@implementation ABI13_0_0RCTShadowText
{
...
static ABI13_0_0YGSize ABI13_0_0RCTMeasure(ABI13_0_0YGNodeRef node, float width, ABI13_0_0YGMeasureMode widthMode, float height, ABI13_0_0YGMeasureMode heightMode)
{
  ABI13_0_0RCTShadowText *shadowText = (__bridge ABI13_0_0RCTShadowText *)ABI13_0_0YGNodeGetContext(node);
  ...
  ABI13_0_0YGSize result;
  result.width = ABI13_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI13_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI13_0_0YGNodeSetMeasureFunc(self.cssNode, ABI13_0_0RCTMeasure);
...
{
  ABI13_0_0YGNodeMarkDirty(self.cssNode);
  [self dirtyText];
}
...
{
  if ([[self ReactABI13_0_0Superview] isKindOfClass:[ABI13_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI13_0_0Superview] ReactABI13_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:width widthMode:ABI13_0_0YGMeasureModeExactly];
  CGRect textFrame = [self calculateTextFrame:textStorage];
  BOOL selectable = _selectable;
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI13_0_0RCTText *view = (ABI13_0_0RCTText *)viewRegistry[self.ReactABI13_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI13_0_0RCTShadowText
     * to the ABI13_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI13_0_0RCTTextView
     * and its contaned ABI13_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI13_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  NSTextStorage *textStorage = [self buildTextStorageForWidth:self.frame.size.width widthMode:ABI13_0_0YGMeasureModeExactly];
  NSLayoutManager *layoutManager = textStorage.layoutManagers.firstObject;
  NSTextContainer *textContainer = layoutManager.textContainers.firstObject;
  NSRange glyphRange = [layoutManager glyphRangeForTextContainer:textContainer];
  NSRange characterRange = [layoutManager characterRangeForGlyphRange:glyphRange actualGlyphRange:NULL];
  [layoutManager.textStorage enumerateAttribute:ABI13_0_0RCTShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI13_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI13_0_0YGNodeRef childNode = child.cssNode;
      float width = ABI13_0_0YGNodeStyleGetWidth(childNode);
      float height = ABI13_0_0YGNodeStyleGetHeight(childNode);
      if (ABI13_0_0YGValueIsUndefined(width) || ABI13_0_0YGValueIsUndefined(height)) {
        ABI13_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI13_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI13_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI13_0_0RCTRoundPixelValue(width),
        ABI13_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI13_0_0YGMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){widthMode == ABI13_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : width, CGFLOAT_MAX};
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI13_0_0RCTFont updateFont:nil
  ...
  for (ABI13_0_0RCTShadowView *child in [self ReactABI13_0_0Subviews]) {
    if ([child isKindOfClass:[ABI13_0_0RCTShadowText class]]) {
      ABI13_0_0RCTShadowText *shadowText = (ABI13_0_0RCTShadowText *)child;
      ...
      ABI13_0_0RCTShadowRawText *shadowRawText = (ABI13_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI13_0_0YGNodeStyleGetWidth(child.cssNode);
      float height = ABI13_0_0YGNodeStyleGetHeight(child.cssNode);
      if (ABI13_0_0YGValueIsUndefined(width) || ABI13_0_0YGValueIsUndefined(height)) {
        ABI13_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      [attachmentString addAttribute:ABI13_0_0RCTShadowViewAttributeName value:child range:(NSRange){0, attachmentString.length}];
      [attributedString appendAttributedString:attachmentString];
      if (height > heightOfTallestSubview) {
        heightOfTallestSubview = height;
      }
      // Don't call setTextComputed on this child. ABI13_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI13_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI13_0_0RCTReactABI13_0_0TagAttributeName withValue:self.ReactABI13_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  if (newTextAlign == NSTextAlignmentRight || newTextAlign == NSTextAlignmentLeft) {
    ABI13_0_0RCTShadowView *view = self;
    while (view != nil && ABI13_0_0YGNodeStyleGetDirection(view.cssNode) == ABI13_0_0YGDirectionInherit) {
      view = [view ReactABI13_0_0Superview];
    }
    if (view != nil && ABI13_0_0YGNodeStyleGetDirection(view.cssNode) == ABI13_0_0YGDirectionRTL) {
  ...
  // Text decoration
  if (_textDecorationLine == ABI13_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI13_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI13_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI13_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
- (CGRect)updateStorage:(NSTextStorage *)textStorage toFitFrame:(CGRect)frame
{
  ...
  frame.origin.y = self.paddingAsInsets.top + ABI13_0_0RCTRoundPixelValue((CGRectGetHeight(frame) - requiredSize.height) / 2.0f);
  return frame;
}
                               ...
                               prevMid:(CGFloat)prevMid
{
  ...
  if (round((prevMid / ABI13_0_0RCTTextAutoSizeGranularity)) == round((midScale / ABI13_0_0RCTTextAutoSizeGranularity))) {
    //Bail because we can't meet error margin.
    return [self calculateSize:textStorage];
  } else {
    ABI13_0_0RCTSizeComparison comparison = [self attemptScale:midScale
                                            inStorage:textStorage
                                             forFrame:frame];
    if (comparison == ABI13_0_0RCTSizeWithinRange) {
      return [self calculateSize:textStorage];
    } else if (comparison == ABI13_0_0RCTSizeTooLarge) {
      return [self calculateOptimumScaleInFrame:frame
                                     forStorage:textStorage
                                       minScale:minScale
                                       maxScale:midScale - ABI13_0_0RCTTextAutoSizeGranularity
                                        prevMid:midScale];
    } else {
      return [self calculateOptimumScaleInFrame:frame
                                     forStorage:textStorage
                                       minScale:midScale + ABI13_0_0RCTTextAutoSizeGranularity
                                       maxScale:maxScale
                                        prevMid:midScale];
    }
  }
...
- (ABI13_0_0RCTSizeComparison)attemptScale:(CGFloat)scale
                         ...
                         forFrame:(CGRect)frame
{
  ...
  if (fitLines && fitSize) {
    if ((requiredSize.width + (CGRectGetWidth(frame) * ABI13_0_0RCTTextAutoSizeWidthErrorMargin)) > CGRectGetWidth(frame) &&
        (requiredSize.height + (CGRectGetHeight(frame) * ABI13_0_0RCTTextAutoSizeHeightErrorMargin)) > CGRectGetHeight(frame))
    {
      return ABI13_0_0RCTSizeWithinRange;
    } else {
      return ABI13_0_0RCTSizeTooSmall;
    }
  } else {
    return ABI13_0_0RCTSizeTooLarge;
  }
}
...
#define ABI13_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI13_0_0RCT_TEXT_PROPERTY(AdjustsFontSizeToFit, _adjustsFontSizeToFit, BOOL)
ABI13_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI13_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI13_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI13_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI13_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI13_0_0RCT_TEXT_PROPERTY(FontVariant, _fontVariant, NSArray *)
ABI13_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI13_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI13_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI13_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI13_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI13_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI13_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI13_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI13_0_0RCTTextDecorationLineType);
ABI13_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI13_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI13_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI13_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI13_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI13_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI13_0_0RCTShadowView *child in [self ReactABI13_0_0Subviews]) {
    if ([child isKindOfClass:[ABI13_0_0RCTShadowText class]]) {
      ((ABI13_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  if (_fontSizeMultiplier == 0) {
    ABI13_0_0RCTLogError(@"fontSizeMultiplier value must be > zero.");
    _fontSizeMultiplier = 1.0;
  }
  for (ABI13_0_0RCTShadowView *child in [self ReactABI13_0_0Subviews]) {
    if ([child isKindOfClass:[ABI13_0_0RCTShadowText class]]) {
      ((ABI13_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
