...
@interface ABI13_0_0RCTScrollEvent : NSObject <ABI13_0_0RCTEvent>
...
@implementation ABI13_0_0RCTScrollEvent
{
...
{
  ABI13_0_0RCTAssertParam(ReactABI13_0_0Tag);
...
- (ABI13_0_0RCTScrollEvent *)coalesceWithEvent:(ABI13_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI13_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI13_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI13_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, copy) NSIndexSet *stickyHeaderIndices;
@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI13_0_0RCTRefreshControl *rctRefreshControl;
#endif

@end


@implementation ABI13_0_0RCTCustomScrollView
{
...
- (UIView *)contentView
{
  return ((ABI13_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI13_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI13_0_0RCTUIManager JSResponder] isKindOfClass:[ABI13_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI13_0_0 scroll
...
- (void)dockClosestSectionHeader
{
  ...
  NSUInteger subviewCount = contentView.ReactABI13_0_0Subviews.count;
  [_stickyHeaderIndices enumerateIndexesWithOptions:0 usingBlock:
   ^(NSUInteger idx, BOOL *stop) {
    ...
    UIView *header = contentView.ReactABI13_0_0Subviews[idx];
...
- (void)setBounds:(CGRect)bounds
{
  if (isRectInvalid(bounds)) {
    ABI13_0_0RCTLogError(@"Attempted to set an invalid bounds to inner scrollview: %@", NSStringFromCGRect(bounds));
    return;
  }
...
- (void)setFrame:(CGRect)frame
{
  if (isRectInvalid(frame)) {
    ABI13_0_0RCTLogError(@"Attempted to set an invalid frame to inner scrollview: %@", NSStringFromCGRect(frame));
    return;
  }
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI13_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI13_0_0RCTScrollView
{
  ABI13_0_0RCTEventDispatcher *_eventDispatcher;
  ABI13_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI13_0_0RCTEventDispatcher *)eventDispatcher
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _scrollView = [[ABI13_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI13_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI13_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI13_0_0RCTAssert(_contentView == nil, @"ABI13_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI13_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI13_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI13_0_0Subview:atIndex:`
}
...
{
  ABI13_0_0RCTAssert(_scrollView.contentSize.width <= self.frame.size.width,
...
- (void)layoutSubviews
{
  ...
  ABI13_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI13_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI13_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI13_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI13_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI13_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI13_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI13_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI13_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI13_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI13_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI13_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI13_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI13_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI13_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI13_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI13_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI13_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI13_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI13_0_0RCTView contentInsetsForView:self];
...
- (void)ReactABI13_0_0BridgeDidFinishTransaction
{
  ...
  if (ABI13_0_0RCT_DEBUG) {
    ...
    NSUInteger subviewCount = _scrollView.contentView.ReactABI13_0_0Subviews.count;
    ...
    if (lastIndex != NSNotFound && lastIndex >= subviewCount) {
      ABI13_0_0RCTLogWarn(@"Sticky header index %zd was outside the range {0, %zd}",
...
#define ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI13_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI13_0_0RCTScrollEvent *scrollEvent = [[ABI13_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI13_0_0RCTEventDispatcher (ABI13_0_0RCTScrollView)
...
- (void)sendFakeScrollEvent:(NSNumber *)ReactABI13_0_0Tag
{
  ...
  ABI13_0_0RCTScrollEvent *fakeScrollEvent = [[ABI13_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI13_0_0Tag:ReactABI13_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
