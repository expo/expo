...
#import "ABI13_0_0RCTAutoInsetsProtocol.h"
#import "ABI13_0_0RCTBorderDrawing.h"
#import "ABI13_0_0RCTConvert.h"
#import "ABI13_0_0RCTLog.h"
#import "ABI13_0_0RCTUtils.h"
#import "UIView+ReactABI13_0_0.h"

@implementation UIView (ABI13_0_0RCTViewUnmounting)

- (void)ReactABI13_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI13_0_0_remountAllSubviews];
  }
}

- (void)ReactABI13_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI13_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI13_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI13_0_0RootView]) {
      break;
    }
...
static NSString *ABI13_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI13_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI13_0_0RCTView
{
  UIColor *_backgroundColor;
}

@synthesize ReactABI13_0_0ZIndex = _ReactABI13_0_0ZIndex;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI13_0_0RCTBorderStyleSolid;
...
ABI13_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI13_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI13_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI13_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI13_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI13_0_0RCTPointerEventsNone || _pointerEvents == ABI13_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI13_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI13_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI13_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI13_0_0Tag: %@;", self.ReactABI13_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI13_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI13_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.sortedReactABI13_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI13_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI13_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI13_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI13_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ReactABI13_0_0Subviews.count == 0) {
  ...
  for (UIView *view in self.sortedReactABI13_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI13_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI13_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI13_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI13_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI13_0_0_findClipView];
  if (clipView) {
    [self ReactABI13_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI13_0_0RCTCornerRadii)cornerRadii
{
  // Get corner radii
  ...
  const CGFloat topScaleFactor = ABI13_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI13_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI13_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI13_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI13_0_0RCTCornerRadii){
...
- (ABI13_0_0RCTBorderColors)borderColors
{
  return (ABI13_0_0RCTBorderColors){
...
- (void)ReactABI13_0_0SetFrame:(CGRect)frame
{
  ...
  [super ReactABI13_0_0SetFrame:frame];
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI13_0_0RCTUpdateShadowPathForView(self);

  const ABI13_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI13_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI13_0_0RCTRunningInTestEnvironment() &&
  ABI13_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI13_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI13_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI13_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI13_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
  ...
  if (ABI13_0_0RCTRunningInTestEnvironment()) {
    const CGSize size = self.bounds.size;
...
static BOOL ABI13_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI13_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI13_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI13_0_0RCTUpdateShadowPathForView(ABI13_0_0RCTView *view)
{
  if (ABI13_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI13_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI13_0_0RCTCornerInsets cornerInsets = ABI13_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI13_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI13_0_0RCTLogWarn(@"View #%@ of type %@ has a shadow set but cannot calculate "
                 "shadow efficiently. Consider setting a background color to "
                 "fix this, or apply the shadow to a more specific component.",
                 view.ReactABI13_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI13_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI13_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI13_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI13_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI13_0_0RCTBorderStyle)style \
  {                                                    \
