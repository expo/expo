...
const CLLocationDegrees ABI13_0_0RCTMapDefaultSpan = 0.005;
const NSTimeInterval ABI13_0_0RCTMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI13_0_0RCTMapZoomBoundBuffer = 0.01;

@implementation ABI13_0_0RCTMap
{
  UIView *_legalLabel;
...
- (void)setAnnotations:(NSArray<ABI13_0_0RCTMapAnnotation *> *)annotations
{
  ...
  NSMutableArray<ABI13_0_0RCTMapAnnotation *> *annotationsToDelete = [NSMutableArray new];
  NSMutableArray<ABI13_0_0RCTMapAnnotation *> *annotationsToAdd = [NSMutableArray new];

  for (ABI13_0_0RCTMapAnnotation *annotation in annotations) {
  ...
  for (ABI13_0_0RCTMapAnnotation *annotation in self.annotations) {
    if (![annotation isKindOfClass:[ABI13_0_0RCTMapAnnotation class]]) {
      continue;
    }
...
- (void)setOverlays:(NSArray<ABI13_0_0RCTMapOverlay *> *)overlays
{
  ...
  for (ABI13_0_0RCTMapOverlay *overlay in overlays) {
    if (![overlay isKindOfClass:[ABI13_0_0RCTMapOverlay class]]) {
      continue;
    }
  ...
  for (ABI13_0_0RCTMapOverlay *overlay in self.overlays) {
    if (![overlay isKindOfClass:[ABI13_0_0RCTMapOverlay class]]) {
      continue;
    }
