...
#import "ABI13_0_0RCTErrorInfo.h"
#import "ABI13_0_0RCTJSStackFrame.h"
#import "ABI13_0_0RCTUtils.h"

#if ABI13_0_0RCT_DEBUG

@class ABI13_0_0RCTRedBoxWindow;

@protocol ABI13_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI13_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI13_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI13_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI13_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI13_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI13_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI13_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI13_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI13_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI13_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI13_0_0RCTJSStackFrame *)stackFrame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI13_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI13_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI13_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI13_0_0RCTKeyCommands for this, but since
...
@interface ABI13_0_0RCTRedBox () <ABI13_0_0RCTInvalidating, ABI13_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI13_0_0RCTRedBox
{
  ABI13_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI13_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (void)registerErrorCustomizer:(id<ABI13_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI13_0_0RCTErrorInfo *)_customizeError:(ABI13_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI13_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI13_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI13_0_0RCTJSStackFrame *> *stack = [ABI13_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI13_0_0RCTJSStackFrame class]]) {
    stack = [ABI13_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI13_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI13_0_0RCTErrorInfo *errorInfo = [[ABI13_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI13_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI13_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI13_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI13_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI13_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI13_0_0RCTRedBoxWindow *)redBoxWindow
{
  [_bridge reload];
}
...
@implementation ABI13_0_0RCTBridge (ABI13_0_0RCTRedBox)

- (ABI13_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI13_0_0RCTRedBox class]];
}
...
- (ABI13_0_0RCTRedBox *)redBox { return nil; }
