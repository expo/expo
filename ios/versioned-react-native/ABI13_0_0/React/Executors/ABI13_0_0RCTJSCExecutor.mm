...
#import "ABI13_0_0RCTJSCErrorHandling.h"
...
NSString *const ABI13_0_0RCTJSCThreadName = @"com.facebook.ReactABI13_0_0.JavaScript";
NSString *const ABI13_0_0RCTJavaScriptContextCreatedNotification = @"ABI13_0_0RCTJavaScriptContextCreatedNotification";
ABI13_0_0RCT_EXTERN NSString *const ABI13_0_0RCTFBJSContextClassKey = @"_ABI13_0_0RCTFBJSContextClassKey";
ABI13_0_0RCT_EXTERN NSString *const ABI13_0_0RCTFBJSValueClassKey = @"_ABI13_0_0RCTFBJSValueClassKey";

static NSString *const ABI13_0_0RCTJSCProfilerEnabledDefaultsKey = @"ABI13_0_0RCTJSCProfilerEnabled";
...
struct TaggedScript {
  const facebook::ReactABI13_0_0::ScriptTag tag;
  const NSData *script;
};

#if ABI13_0_0RCT_PROFILE
@interface ABI13_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI13_0_0RCTCookieMap @end
#endif

struct ABI13_0_0RCTJSContextData {
...
@interface ABI13_0_0RCTJSContextProvider ()
/** May only be called once, or deadlock will result. */
- (ABI13_0_0RCTJSContextData)data;
@end

@interface ABI13_0_0RCTJavaScriptContext : NSObject <ABI13_0_0RCTInvalidating>
...
@implementation ABI13_0_0RCTJavaScriptContext
{
  ABI13_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    _context = context;
    _context.name = @"ABI13_0_0RCTJSContext";
    _javaScriptThread = javaScriptThread;

    /**
     * Explicitly introduce a retain cycle here - The ABI13_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI13_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI13_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI13_0_0RCTPerformanceLogger *_performanceLogger;
  ABI13_0_0RCTJavaScriptContext *_context;
...
#if ABI13_0_0RCT_DEV
static void ABI13_0_0RCTInstallJSCProfiler(ABI13_0_0RCTBridge *bridge, JSContextRef context)
{
  if (ABI13_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI13_0_0RCTDevMenuItem toggleItemWithKey:ABI13_0_0RCTJSCProfilerEnabledDefaultsKey title:@"Start Profiling" selectedTitle:@"Stop Profiling" handler:^(BOOL shouldStart) {
      if (shouldStart != ABI13_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI13_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI13_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI13_0_0RCTProfileSendResult(bridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
+ (void)runRunLoopThread
{
  @autoreleasepool {
    ...
    while (kCFRunLoopRunStopped != CFRunLoopRunInMode(kCFRunLoopDefaultMode, ((NSDate *)[NSDate distantFuture]).timeIntervalSinceReferenceDate, NO)) {
      ABI13_0_0RCTAssert(NO, @"not reached assertion"); // runloop spun. that's bad.
    }
  }
}
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI13_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI13_0_0RCTJSCThreadName;
...
{
  ABI13_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI13_0_0RCTJSCExecutor init]", nil);
...
+ (instancetype)initializedExecutorWithContextProvider:(ABI13_0_0RCTJSContextProvider *)JSContextProvider
                                             JSContext:(JSContext **)JSContext
{
  const ABI13_0_0RCTJSContextData data = JSContextProvider.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  return [[ABI13_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
}

- (instancetype)initWithJSContextData:(const ABI13_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    ...
    _context = [[ABI13_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
                                         ...
                                         sourceURL:(NSURL *)sourceURL
{
  ...
  if (taggedScript.tag == facebook::ReactABI13_0_0::ScriptTag::RAMBundle) {
...
- (ABI13_0_0RCTJavaScriptContext *)context
{
  ABI13_0_0RCTAssertThread(_javaScriptThread, @"Must be called on JS thread.");
  if (!self.isValid) {
    return nil;
  }
  ABI13_0_0RCTAssert(_context != nil, @"Fetching context while valid, but before it is created");
  return _context;
}
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI13_0_0RCTProfileDidStartProfiling, ABI13_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_context) {
      ...
      self->_context = [[ABI13_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI13_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];
    ...
    if (!threadDictionary[ABI13_0_0RCTFBJSContextClassKey] || !threadDictionary[ABI13_0_0RCTFBJSValueClassKey]) {
      threadDictionary[ABI13_0_0RCTFBJSContextClassKey] = JSC_JSContext(contextRef);
      threadDictionary[ABI13_0_0RCTFBJSValueClassKey] = JSC_JSValue(contextRef);
    }

    __weak ABI13_0_0RCTJSCExecutor *weakSelf = self;
    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI13_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI13_0_0RCT_PROFILE_BEGIN_EVENT(ABI13_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI13_0_0RCT_PROFILE_END_EVENT(ABI13_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI13_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI13_0_0RCT_PROFILE_BEGIN_EVENT(ABI13_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
      [strongSelf->_bridge handleBuffer:calls batchEnded:NO];
      ABI13_0_0RCT_PROFILE_END_EVENT(ABI13_0_0RCTProfileTagAlways, @"js_call");
    };
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI13_0_0RCT_PROFILE_BEGIN_EVENT(ABI13_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI13_0_0RCT_PROFILE
    __weak ABI13_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI13_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI13_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI13_0_0RCTProfileIsProfiling()) {
        ...
        _ABI13_0_0RCTProfileEndFlowEvent(newCookie);
    ...
    if (JSC_JSSamplingProfilerEnabled(context.JSGlobalContextRef)) {
      ...
      [self->_bridge.devMenu addItem:[ABI13_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
        ABI13_0_0RCTJSCExecutor *strongSelf = weakSelf;
        ...
        if (JSC_JSValueGetType(ctx, jsResult) != kJSTypeNull) {
          ...
          ABI13_0_0JSCSamplingProfiler *profilerModule = [strongSelf->_bridge moduleForClass:[ABI13_0_0JSCSamplingProfiler class]];
...
#if ABI13_0_0RCT_DEV
    ABI13_0_0RCTInstallJSCProfiler(self->_bridge, context.JSGlobalContextRef);
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI13_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ABI13_0_0RCTLogLevel level = ABI13_0_0RCTLogLevelInfo;
    if (logLevel) {
      level = MAX(level, (ABI13_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI13_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI13_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI13_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI13_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI13_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI13_0_0RCTCookieMap *cookieMap = [ABI13_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI13_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI13_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI13_0_0RCTProfileDidStartProfiling];
...
- (int32_t)bytecodeFileFormatVersion
{
  return (_useCustomJSCLibrary && _tryBytecode)
    ? facebook::ReactABI13_0_0::customJSCWrapper()->JSBytecodeFileFormatVersion
...
ABI13_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)contextName)
{
...
- (void)flushedQueue:(ABI13_0_0RCTJavaScriptCallback)onComplete
{
...
- (void)_callFunctionOnModule:(NSString *)module
                 ...
                 unwrapResult:(BOOL)unwrapResult
                     callback:(ABI13_0_0RCTJavaScriptCallback)onComplete
{
  // TODO: Make this function handle first class instead of dynamically dispatching it. #9317773
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI13_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI13_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI13_0_0RCTJavaScriptCallback)onComplete
{
  ABI13_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI13_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI13_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI13_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, batchedBridgeRef) != kJSTypeUndefined) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, methodJSRef) != kJSTypeUndefined) {
        ...
        if (!errorJSRef && JSC_JSValueGetType(ctx, methodJSRef) == kJSTypeUndefined) {
          error = ABI13_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSC_JSValueGetType(ctx, batchedBridgeRef) == kJSTypeUndefined) {
        error = ABI13_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI13_0_0RCTNSErrorFromJSError([JSC_JSValue(ctx) valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI13_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
- (void)executeApplicationScript:(NSData *)script
                       sourceURL:(NSURL *)sourceURL
                      onComplete:(ABI13_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ABI13_0_0RCTAssertParam(script);
  ABI13_0_0RCTAssertParam(sourceURL);
...
static TaggedScript loadTaggedScript(NSData *script,
                                     NSURL *sourceURL,
                                     ABI13_0_0RCTPerformanceLogger *performanceLogger,
                                     RandomAccessBundleData &randomAccessBundle,
                                     NSError **error)
{
  ABI13_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);
  ...
  switch (tag) {
    case facebook::ReactABI13_0_0::ScriptTag::RAMBundle:
      [performanceLogger markStartForTag:ABI13_0_0RCTPLRAMBundleLoad];

      loadedScript = loadRAMBundle(sourceURL, error, randomAccessBundle);

      [performanceLogger markStopForTag:ABI13_0_0RCTPLRAMBundleLoad];
      [performanceLogger setValue:loadedScript.length forTag:ABI13_0_0RCTPLRAMStartupCodeSize];
      break;
...
static void registerNativeRequire(JSContext *context, ABI13_0_0RCTJSCExecutor *executor)
{
  __weak ABI13_0_0RCTJSCExecutor *weakExecutor = executor;
...
{
  ABI13_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(taggedScript.script.length)
  }));

  [performanceLogger markStartForTag:ABI13_0_0RCTPLScriptExecution];
  ...
  [performanceLogger markStopForTag:ABI13_0_0RCTPLScriptExecution];

  NSError *error = jsError
    ? ABI13_0_0RCTNSErrorFromJSErrorRef(jsError, ctx)
    : nil;
...
- (void)injectJSONText:(NSString *)script
   asGlobalObjectNamed:(NSString *)objectName
              callback:(ABI13_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI13_0_0RCT_DEBUG) {
    ABI13_0_0RCTAssert(ABI13_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI13_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    JSGlobalContextRef ctx = strongSelf->_context.context.JSGlobalContextRef;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI13_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI13_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI13_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
      }
    }
    ABI13_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI13_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI13_0_0RCTFatal(ABI13_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    NSError *error = ABI13_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI13_0_0RCTFatal(error);
      [executor invalidate];
    });
  }
}
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI13_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI13_0_0RCTPLRAMNativeRequires];
  ABI13_0_0RCT_PROFILE_BEGIN_EVENT(ABI13_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI13_0_0RCT_PROFILE_END_EVENT(ABI13_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI13_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI13_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI13_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
@implementation ABI13_0_0RCTJSContextProvider
{
