...
static const NSUInteger ABI21_0_0RCTMaxCachableDecodedImageSizeInBytes = 1048576; // 1MB

static NSString *ABI21_0_0RCTCacheKeyForImage(NSString *imageTag, CGSize size, CGFloat scale,
                                     ABI21_0_0RCTResizeMode resizeMode, NSString *responseDate)
{
    return [NSString stringWithFormat:@"%@|%g|%g|%g|%zd|%@",
            imageTag, size.width, size.height, scale, resizeMode, responseDate];
}

@implementation ABI21_0_0RCTImageCache
{
                 ...
                 forKey:(NSString *)cacheKey
{
  ...
  if (bytes <= ABI21_0_0RCTMaxCachableDecodedImageSizeInBytes) {
...
{
  NSString *cacheKey = ABI21_0_0RCTCacheKeyForImage(url, size, scale, resizeMode, responseDate);
