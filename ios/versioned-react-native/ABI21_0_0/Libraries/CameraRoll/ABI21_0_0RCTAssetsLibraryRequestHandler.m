...
@implementation ABI21_0_0RCTAssetsLibraryRequestHandler
{
     ...
     withDelegate:(id<ABI21_0_0RCTURLRequestDelegate>)delegate
{
  ...
  [[self assetsLibrary] assetForURL:request.URL resultBlock:^(ALAsset *asset) {
    ...
    if (asset) {
      ...
      NSError *error = ABI21_0_0RCTErrorWithMessage(errorMessage);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
    }
...
@implementation ABI21_0_0RCTBridge (ABI21_0_0RCTAssetsLibraryImageLoader)

- (ALAssetsLibrary *)assetsLibrary
{
  return [[self moduleForClass:[ABI21_0_0RCTAssetsLibraryRequestHandler class]] assetsLibrary];
}
