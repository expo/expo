...
NSString *const ABI21_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI21_0_0RCTReactABI21_0_0TagAttributeName = @"ReactABI21_0_0TagAttributeName";

static NSString *const kShadowViewAttributeName = @"ABI21_0_0RCTShadowViewAttributeName";
...
@implementation ABI21_0_0RCTShadowText
{
...
static ABI21_0_0YGSize ABI21_0_0RCTMeasure(ABI21_0_0YGNodeRef node, float width, ABI21_0_0YGMeasureMode widthMode, float height, ABI21_0_0YGMeasureMode heightMode)
{
  ABI21_0_0RCTShadowText *shadowText = (__bridge ABI21_0_0RCTShadowText *)ABI21_0_0YGNodeGetContext(node);
  ...
  result.width = ABI21_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI21_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI21_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI21_0_0RCTMeasure);

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(contentSizeMultiplierDidChange:)
                                                 name:ABI21_0_0RCTUIManagerWillUpdateViewsDueToContentSizeMultiplierChangeNotification
                                               object:nil];
  }
  return self;
}
...
{
  ABI21_0_0YGNodeMarkDirty(self.yogaNode);
  [self dirtyText];
}
...
{
  if ([[self ReactABI21_0_0Superview] isKindOfClass:[ABI21_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI21_0_0Superview] ReactABI21_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:width widthMode:ABI21_0_0YGMeasureModeExactly];
  CGRect textFrame = [self calculateTextFrame:textStorage];
  BOOL selectable = _selectable;
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI21_0_0RCTText *view = (ABI21_0_0RCTText *)viewRegistry[self.ReactABI21_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI21_0_0RCTShadowText
     * to the ABI21_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI21_0_0RCTTextView
     * and its contaned ABI21_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI21_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  NSTextStorage *textStorage = [self buildTextStorageForWidth:self.frame.size.width widthMode:ABI21_0_0YGMeasureModeExactly];
  NSLayoutManager *layoutManager = textStorage.layoutManagers.firstObject;
  NSTextContainer *textContainer = layoutManager.textContainers.firstObject;
  NSRange glyphRange = [layoutManager glyphRangeForTextContainer:textContainer];
  NSRange characterRange = [layoutManager characterRangeForGlyphRange:glyphRange actualGlyphRange:NULL];
  [layoutManager.textStorage enumerateAttribute:kShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI21_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI21_0_0YGNodeRef childNode = child.yogaNode;
      float width = ABI21_0_0YGNodeStyleGetWidth(childNode).value;
      float height = ABI21_0_0YGNodeStyleGetHeight(childNode).value;
      if (ABI21_0_0YGFloatIsUndefined(width) || ABI21_0_0YGFloatIsUndefined(height)) {
        ABI21_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI21_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI21_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI21_0_0RCTRoundPixelValue(width),
        ABI21_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI21_0_0YGMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){widthMode == ABI21_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : width, CGFLOAT_MAX};
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI21_0_0RCTFont updateFont:nil
  ...
  for (ABI21_0_0RCTShadowView *child in [self ReactABI21_0_0Subviews]) {
    if ([child isKindOfClass:[ABI21_0_0RCTShadowText class]]) {
      ABI21_0_0RCTShadowText *shadowText = (ABI21_0_0RCTShadowText *)child;
      ...
      ABI21_0_0RCTShadowRawText *shadowRawText = (ABI21_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI21_0_0YGNodeStyleGetWidth(child.yogaNode).value;
      float height = ABI21_0_0YGNodeStyleGetHeight(child.yogaNode).value;
      if (ABI21_0_0YGFloatIsUndefined(width) || ABI21_0_0YGFloatIsUndefined(height)) {
        ABI21_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      // Don't call setTextComputed on this child. ABI21_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI21_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI21_0_0RCTReactABI21_0_0TagAttributeName withValue:self.ReactABI21_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  // Text decoration
  if (_textDecorationLine == ABI21_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI21_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI21_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI21_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
- (CGRect)updateStorage:(NSTextStorage *)textStorage toFitFrame:(CGRect)frame
{
  ...
  frame.origin.y = self.paddingAsInsets.top + ABI21_0_0RCTRoundPixelValue((CGRectGetHeight(frame) - requiredSize.height) / 2.0f);
  return frame;
}
                               ...
                               prevMid:(CGFloat)prevMid
{
  ...
  if (round((prevMid / kAutoSizeGranularity)) == round((midScale / kAutoSizeGranularity))) {
    ...
    ABI21_0_0RCTSizeComparison comparison = [self attemptScale:midScale
                                            inStorage:textStorage
                                             forFrame:frame];
    if (comparison == ABI21_0_0RCTSizeWithinRange) {
      return [self calculateSize:textStorage];
    } else if (comparison == ABI21_0_0RCTSizeTooLarge) {
...
- (ABI21_0_0RCTSizeComparison)attemptScale:(CGFloat)scale
                         ...
                         forFrame:(CGRect)frame
{
  ...
  if (fitLines && fitSize) {
    ...
    {
      return ABI21_0_0RCTSizeWithinRange;
    } else {
      return ABI21_0_0RCTSizeTooSmall;
    }
  } else {
    return ABI21_0_0RCTSizeTooLarge;
  }
}
...
#define ABI21_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI21_0_0RCT_TEXT_PROPERTY(AdjustsFontSizeToFit, _adjustsFontSizeToFit, BOOL)
ABI21_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI21_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI21_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI21_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI21_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI21_0_0RCT_TEXT_PROPERTY(FontVariant, _fontVariant, NSArray *)
ABI21_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI21_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI21_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI21_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI21_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI21_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI21_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI21_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI21_0_0RCTTextDecorationLineType);
ABI21_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI21_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI21_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI21_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI21_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI21_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI21_0_0RCTShadowView *child in [self ReactABI21_0_0Subviews]) {
    if ([child isKindOfClass:[ABI21_0_0RCTShadowText class]]) {
      ((ABI21_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  if (_fontSizeMultiplier == 0) {
    ABI21_0_0RCTLogError(@"fontSizeMultiplier value must be > zero.");
    _fontSizeMultiplier = 1.0;
  }
  for (ABI21_0_0RCTShadowView *child in [self ReactABI21_0_0Subviews]) {
    if ([child isKindOfClass:[ABI21_0_0RCTShadowText class]]) {
      ((ABI21_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
