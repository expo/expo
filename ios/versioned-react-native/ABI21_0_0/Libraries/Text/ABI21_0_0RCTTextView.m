...
@interface ABI21_0_0RCTTextView () <ABI21_0_0RCTBackedTextInputDelegate>

@end

@implementation ABI21_0_0RCTTextView
{
  ABI21_0_0RCTUITextView *_backedTextInput;
  ABI21_0_0RCTText *_richTextView;
...
- (instancetype)initWithBridge:(ABI21_0_0RCTBridge *)bridge
{
  ...
  if (self = [super initWithBridge:bridge]) {
    ...
    _backedTextInput = [[ABI21_0_0RCTUITextView alloc] initWithFrame:self.bounds];
...
#pragma mark - ABI21_0_0RCTComponent

- (void)insertReactABI21_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI21_0_0Subview:subview atIndex:index];

  if ([subview isKindOfClass:[ABI21_0_0RCTText class]]) {
    if (_richTextView) {
      ABI21_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI21_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI21_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI21_0_0RCTReactABI21_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI21_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI21_0_0TagFromString(_pendingAttributedText);
...
- (void)setText:(NSString *)text
{
  ...
  if (eventLag == 0 && ![text isEqualToString:_backedTextInput.text]) {
    ...
    ABI21_0_0RCTLogWarn(@"Native TextInput(%@) is %zd events ahead of JS - try to make your JS faster.", self.text, eventLag);
  }
}
...
- (void)textInputDidChange
{
  ...
  if (!self.ReactABI21_0_0Tag || !_onChange) {
    return;
  }
