...
#import "ABI21_0_0EXAppLoadingManager.h"
...
#define ABI21_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05
#define ABI21_0_0EX_FRAME_ORIENTATION_USE_MANIFEST 0

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI21_0_0EXFrameReactABI21_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI21_0_0EXFrameReactABI21_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI21_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI21_0_0Bridge;

@end

@interface ABI21_0_0EXFrame () <ABI21_0_0RCTInvalidating, ABI21_0_0RCTExceptionsManagerDelegate>

@property (nonatomic, assign) BOOL sourceSet;
@property (nonatomic, assign) BOOL needsReload;

@property (nonatomic, copy) ABI21_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI21_0_0RCTDirectEventBlock onLoadingProgress;
@property (nonatomic, copy) ABI21_0_0RCTDirectEventBlock onLoadingFinish;
@property (nonatomic, copy) ABI21_0_0RCTDirectEventBlock onLoadingError;
...
@implementation ABI21_0_0EXFrame

@synthesize supportedInterfaceOrientations = _supportedInterfaceOrientations;

ABI21_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI21_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");

    _appManager = [[unversionedAppManagerClass alloc] initWithEXFrame:self];
    [_appManager setDelegate:self];
    _supportedInterfaceOrientations = ABI21_0_0EX_FRAME_ORIENTATION_USE_MANIFEST;
  }
  return self;
}
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI21_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI21_0_0RootView]) {
    [_appManager ReactABI21_0_0RootView].frame = self.bounds;
  }
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
  if (_supportedInterfaceOrientations != ABI21_0_0EX_FRAME_ORIENTATION_USE_MANIFEST) {
...
#pragma mark - ABI21_0_0EXReactABI21_0_0AppManagerDelegate

- (void)ReactABI21_0_0AppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI21_0_0RootView = [appManager ReactABI21_0_0RootView];
  ReactABI21_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI21_0_0RootView];
}

- (void)ReactABI21_0_0AppManagerDidDestroyApp:(id)appManager
{
  if (_tmrReload) {
    [_tmrReload invalidate];
    _tmrReload = nil;
...
- (void)ReactABI21_0_0AppManagerStartedLoadingJavaScript:(id)appManager
{
  if (_onLoadingStart) {
    _onLoadingStart(nil);
  }
...
- (void)ReactABI21_0_0AppManager:(id)appManager loadedJavaScriptWithProgress:(ABI21_0_0RCTLoadingProgress *)progress;
{
  if (_onLoadingProgress) {
    _onLoadingProgress(@{
      @"status": ABI21_0_0RCTNullIfNil(progress.status),
      @"done": ABI21_0_0RCTNullIfNil(progress.done),
      @"total": ABI21_0_0RCTNullIfNil(progress.total),
    });
  }
}

- (void)ReactABI21_0_0AppManagerFinishedLoadingJavaScript:(id)appManager
{
  if (_viewTestTimer) {
    [_viewTestTimer invalidate];
  }
...
- (void)ReactABI21_0_0AppManager:(id)appManager failedToLoadJavaScriptWithError:(NSError *)error
{
  [self _sendLoadingError:error];
}

- (void)ReactABI21_0_0AppManager:(id)appManager failedToDownloadBundleWithError:(NSError *)error
{
  
}

- (void)ReactABI21_0_0AppManagerDidForeground:(id)appManager
{
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI21_0_0RootView] &&
      [_appManager ReactABI21_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI21_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI21_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI21_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI21_0_0RCTErrorWithMessage(message)];
