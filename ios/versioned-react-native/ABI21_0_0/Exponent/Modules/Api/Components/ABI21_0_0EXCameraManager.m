...
#import "ABI21_0_0EXCamera.h"
#import "ABI21_0_0EXCameraManager.h"
...
@property (nonatomic, strong) ABI21_0_0RCTPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI21_0_0RCTPromiseRejectBlock videoRecordedReject;

@end

@implementation ABI21_0_0EXCameraManager

ABI21_0_0RCT_EXPORT_MODULE(ExponentCameraManager);
ABI21_0_0RCT_EXPORT_VIEW_PROPERTY(onCameraReady, ABI21_0_0RCTDirectEventBlock);
...
- (UIView *)view
{
  ...
  if (!self.camera) {
    self.camera = [[ABI21_0_0EXCamera alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
- (NSDictionary *)constantsToExport
{
  return @{
           @"Type" :
             @{@"front" : @(ABI21_0_0EXCameraTypeFront), @"back" : @(ABI21_0_0EXCameraTypeBack)},
           @"FlashMode" : @{
               @"off" : @(ABI21_0_0EXCameraFlashModeOff),
               @"on" : @(ABI21_0_0EXCameraFlashModeOn),
               @"auto" : @(ABI21_0_0EXCameraFlashModeAuto),
               @"torch" : @(ABI21_0_0EXCameraFlashModeTorch)
               },
           @"AutoFocus" :
             @{@"on" : @(ABI21_0_0EXCameraAutoFocusOn), @"off" : @(ABI21_0_0EXCameraAutoFocusOff)},
           @"WhiteBalance" : @{
               @"auto" : @(ABI21_0_0EXCameraWhiteBalanceAuto),
               @"sunny" : @(ABI21_0_0EXCameraWhiteBalanceSunny),
               @"cloudy" : @(ABI21_0_0EXCameraWhiteBalanceCloudy),
               @"shadow" : @(ABI21_0_0EXCameraWhiteBalanceShadow),
               @"incandescent" : @(ABI21_0_0EXCameraWhiteBalanceIncandescent),
               @"fluorescent" : @(ABI21_0_0EXCameraWhiteBalanceFluorescent)
               },
           @"VideoQuality": @{
               @"2160p": @(ABI21_0_0EXCameraVideo2160p),
               @"1080p": @(ABI21_0_0EXCameraVideo1080p),
               @"720p": @(ABI21_0_0EXCameraVideo720p),
               @"480p": @(ABI21_0_0EXCameraVideo4x3),
               @"4:3": @(ABI21_0_0EXCameraVideo4x3),
               }
           };
}
...
ABI21_0_0RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, ABI21_0_0EXCamera)
{
...
ABI21_0_0RCT_CUSTOM_VIEW_PROPERTY(flashMode, NSInteger, ABI21_0_0EXCamera)
{
  self.flashMode = [ABI21_0_0RCTConvert NSInteger:json];
  [self updateFlashMode];
}
...
- (void)updateFlashMode {
  dispatch_async(self.sessionQueue, ^{
    ...
    if (self.flashMode == ABI21_0_0EXCameraFlashModeTorch) {
      if (![device hasTorch])
        return;
...
ABI21_0_0RCT_CUSTOM_VIEW_PROPERTY(autoFocus, NSInteger, ABI21_0_0EXCamera)
{
  self.autoFocus = [ABI21_0_0RCTConvert NSInteger:json];
...
ABI21_0_0RCT_CUSTOM_VIEW_PROPERTY(focusDepth, NSNumber, ABI21_0_0EXCamera)
{
  self.focusDepth = [ABI21_0_0RCTConvert float:json];
...
- (void)updateFocusDepth {
  ...
  if (device.focusMode != ABI21_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if (![device respondsToSelector:@selector(isLockingFocusWithCustomLensPositionSupported)] || ![device isLockingFocusWithCustomLensPositionSupported]) {
    ABI21_0_0RCTLogWarn(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
    return;
  }
...
ABI21_0_0RCT_CUSTOM_VIEW_PROPERTY(zoom, NSNumber, ABI21_0_0EXCamera)
{
  self.zoom = [ABI21_0_0RCTConvert CGFloat:json];
  [self updateZoom];
}
...
ABI21_0_0RCT_CUSTOM_VIEW_PROPERTY(whiteBalance, NSInteger, ABI21_0_0EXCamera)
{
  self.whiteBalance = [ABI21_0_0RCTConvert NSInteger:json];
...
- (void)updateWhiteBalance {
  ...
  if (self.whiteBalance == ABI21_0_0EXCameraWhiteBalanceAuto) {
...
ABI21_0_0RCT_REMAP_METHOD(takePicture,
...
ABI21_0_0RCT_REMAP_METHOD(record,
  ...
  if (self.movieFileOutput != nil && !self.movieFileOutput.isRecording) {
    ...
    if (options[@"quality"]) {
      [self updateSessionPreset:[[self class] captureSessionPresetForVideoResolution:(ABI21_0_0EXCameraVideoResolution)options[@"quality"]]];
    }
...
ABI21_0_0RCT_EXPORT_METHOD(stopRecording) {
...
- (void)startSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI21_0_0EXCameraManager *weakSelf = self;
    [self setRuntimeErrorHandlingObserver:
     [NSNotificationCenter.defaultCenter
      ...
      usingBlock:^(NSNotification *note) {
        ABI21_0_0EXCameraManager *strongSelf = weakSelf;
...
- (void)initializeCaptureSessionInput:(NSString *)type
{
  dispatch_async(self.sessionQueue, ^{
    ...
    if (error || captureDeviceInput == nil) {
      ABI21_0_0RCTLog(@"%s: %@", __func__, error);
      return;
    }
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    if (!isMuted) {
      ...
      if (error || audioDeviceInput == nil) {
        ABI21_0_0RCTLogWarn(@"%s: %@", __func__, error);
        return;
      }
...
+ (float)temperatureForWhiteBalance:(ABI21_0_0EXCameraWhiteBalance)whiteBalance
{
  switch (whiteBalance) {
    case ABI21_0_0EXCameraWhiteBalanceSunny: default:
      return 5200;
    case ABI21_0_0EXCameraWhiteBalanceCloudy:
      return 6000;
    case ABI21_0_0EXCameraWhiteBalanceShadow:
      return 7000;
    case ABI21_0_0EXCameraWhiteBalanceIncandescent:
      return 3000;
    case ABI21_0_0EXCameraWhiteBalanceFluorescent:
      return 4200;
  }
}

+ (NSString *)captureSessionPresetForVideoResolution:(ABI21_0_0EXCameraVideoResolution)resolution
{
  switch (resolution) {
    case ABI21_0_0EXCameraVideo2160p:
      return AVCaptureSessionPreset3840x2160;
    case ABI21_0_0EXCameraVideo1080p:
      return AVCaptureSessionPreset1920x1080;
    case ABI21_0_0EXCameraVideo720p:
      return AVCaptureSessionPreset1280x720;
    case ABI21_0_0EXCameraVideo4x3:
