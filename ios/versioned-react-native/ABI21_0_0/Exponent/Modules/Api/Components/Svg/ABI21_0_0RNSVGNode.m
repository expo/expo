...
@implementation ABI21_0_0RNSVGNode
{
    BOOL _transparent;
    CGPathRef _cachedClipPath;
    ABI21_0_0RNSVGSvgView *_svgView;
}
...
- (void)invalidate
{
    id<ABI21_0_0RNSVGContainer> container = (id<ABI21_0_0RNSVGContainer>)self.superview;
...
- (ABI21_0_0RNSVGSvgView *)getSvgView
{
    if (_svgView) {
        return _svgView;
    }
    ...
    if ([parent class] == [ABI21_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI21_0_0RNSVGNode class]]) {
        ABI21_0_0RNSVGNode *node = parent;
        _svgView = [node getSvgView];
    } else {
        ABI21_0_0RCTLogError(@"ABI21_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI21_0_0RNSVGPercentageConverter stringToFloat:length relative:[self getContextWidth] offset:0];
}

- (CGFloat)relativeOnHeight:(NSString *)length
{
    return [ABI21_0_0RNSVGPercentageConverter stringToFloat:length relative:[self getContextHeight] offset:0];
}
...
- (void)traverseSubviews:(BOOL (^)(__kindof ABI21_0_0RNSVGNode *node))block
{
