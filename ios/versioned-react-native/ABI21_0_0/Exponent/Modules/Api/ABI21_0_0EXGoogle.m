...
NSString *ABI21_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI21_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI21_0_0EXGoogle
{
  ABI21_0_0RCTPromiseResolveBlock _logInResolve;
  ABI21_0_0RCTPromiseRejectBlock _logInReject;
}

ABI21_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI21_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI21_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI21_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      if (authState.isAuthorized) {
        _logInResolve(@{
          @"type": @"success",
          @"accessToken": ABI21_0_0RCTNullIfNil(authState.lastTokenResponse.accessToken),
          @"idToken": ABI21_0_0RCTNullIfNil(authState.lastTokenResponse.idToken),
          @"refreshToken": ABI21_0_0RCTNullIfNil(authState.lastTokenResponse.refreshToken),
          @"serverAuthCode": ABI21_0_0RCTNullIfNil(authState.lastAuthorizationResponse.authorizationCode),
        });
      } else {
        _logInResolve(@{@"type": @"cancel"});
      }
    } else {
      _logInReject(ABI21_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI21_0_0RCTPresentedViewController()
                                                   callback:callback];
...
-(void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error
{
  if (error != nil) {
    ...
    _logInResolve(@{
      @"type": @"success",
      @"accessToken": ABI21_0_0RCTNullIfNil(user.authentication.accessToken),
      @"serverAuthCode": ABI21_0_0RCTNullIfNil(user.serverAuthCode),
      @"idToken": ABI21_0_0RCTNullIfNil(user.authentication.idToken),
      @"refreshToken": ABI21_0_0RCTNullIfNil(user.authentication.refreshToken),
      @"user": @{
        @"id": ABI21_0_0RCTNullIfNil(user.userID),
        @"name": ABI21_0_0RCTNullIfNil(user.profile.name),
        @"familyName": ABI21_0_0RCTNullIfNil(user.profile.familyName),
        @"givenName": ABI21_0_0RCTNullIfNil(user.profile.givenName),
        @"email": ABI21_0_0RCTNullIfNil(user.profile.email),
...
{
  [ABI21_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI21_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
