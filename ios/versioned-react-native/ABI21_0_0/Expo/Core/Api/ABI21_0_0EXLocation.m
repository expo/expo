...
NSString * const ABI21_0_0EXLocationChangedEventName = @"Exponent.locationChanged";
NSString * const ABI21_0_0EXHeadingChangedEventName = @"Exponent.headingChanged";

@interface ABI21_0_0EXLocationDelegate : NSObject <CLLocationManagerDelegate>
...
@implementation ABI21_0_0EXLocationDelegate
...
@interface ABI21_0_0EXLocation ()

@property (nonatomic, strong) NSMutableDictionary<NSNumber *, ABI21_0_0EXLocationDelegate*> *delegates;
...
@implementation ABI21_0_0EXLocation

ABI21_0_0EX_EXPORT_SCOPED_MODULE(ExponentLocation, PermissionsManager)
...
{
  return @[ABI21_0_0EXLocationChangedEventName, ABI21_0_0EXHeadingChangedEventName];
}

ABI21_0_0RCT_REMAP_METHOD(getProviderStatusAsync,
...
ABI21_0_0RCT_REMAP_METHOD(getCurrentPositionAsync,
...
ABI21_0_0RCT_REMAP_METHOD(watchPositionImplAsync,
                 ...
                 rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject)
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    locMgr.distanceFilter = options[@"distanceInterval"] ? [ABI21_0_0RCTConvert double:options[@"distanceInterval"]] ?: kCLDistanceFilterNone : kCLLocationAccuracyHundredMeters;
    locMgr.desiredAccuracy = [ABI21_0_0RCTConvert BOOL:options[@"enableHighAccuracy"]] ? kCLLocationAccuracyBest : kCLLocationAccuracyHundredMeters;
    
    ABI21_0_0EXLocationDelegate *delegate = [[ABI21_0_0EXLocationDelegate alloc] initWithId:watchId withLocMgr:locMgr onUpdateLocations:^(NSArray<CLLocation *> *locations) {
      if (locations.lastObject) {
        ...
        [weakSelf sendEventWithName:ABI21_0_0EXLocationChangedEventName body:body];
      }
...
ABI21_0_0RCT_REMAP_METHOD(watchDeviceHeading,
                 watchId:(nonnull NSNumber *)watchId
                 watchDeviceHeading_resolver:(ABI21_0_0RCTPromiseResolveBlock)resolve
                 watchDeviceHeading_rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject) {
  if (!([CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedAlways || [CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedWhenInUse ||
        [_kernelPermissionsServiceDelegate hasGrantedPermission:@"location" forExperience:self.experienceId])) {
    reject(@"E_LOCATION_UNAUTHORIZED", @"Not authorized to use location services", nil);
    return;
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    ABI21_0_0EXLocationDelegate *delegate = [[ABI21_0_0EXLocationDelegate alloc] initWithId:watchId withLocMgr:locMgr onUpdateLocations: nil onUpdateHeadings:^(CLHeading *newHeading) {
      if (newHeading) {
        NSNumber *accuracy;
        ...
        [weakSelf sendEventWithName:ABI21_0_0EXHeadingChangedEventName body:body];
      }
...
ABI21_0_0RCT_REMAP_METHOD(removeWatchAsync,
                 watchId:(nonnull NSNumber *)watchId
                 resolver:(ABI21_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject)
{
  ABI21_0_0EXLocationDelegate *delegate = _delegates[watchId];
  if (delegate) {
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
...
ABI21_0_0RCT_REMAP_METHOD(geocodeAsync,
...
ABI21_0_0RCT_REMAP_METHOD(reverseGeocodeAsync,
