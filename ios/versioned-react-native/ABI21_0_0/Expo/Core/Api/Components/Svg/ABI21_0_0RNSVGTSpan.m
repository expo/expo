...
@implementation ABI21_0_0RNSVGTSpan
{
    ABI21_0_0RNSVGBezierTransformer *_bezierTransformer;
    CGPathRef _cache;
}
...
{
    __block ABI21_0_0RNSVGBezierTransformer *bezierTransformer;
    [self traverseTextSuperviews:^(__kindof ABI21_0_0RNSVGText *node) {
        if ([node class] == [ABI21_0_0RNSVGTextPath class]) {
            bezierTransformer = [(ABI21_0_0RNSVGTextPath*)node getBezierTransformer];
            return NO;
        }
        return YES;
    }];
...
- (void)traverseTextSuperviews:(BOOL (^)(__kindof ABI21_0_0RNSVGText *node))block
{
    ABI21_0_0RNSVGText *targetView = self;
    BOOL result = block(self);
    
    while (targetView && [targetView class] != [ABI21_0_0RNSVGText class] && result) {
        if (![targetView isKindOfClass:[ABI21_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        
        targetView = (ABI21_0_0RNSVGText*)[targetView superview];
