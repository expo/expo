...
@interface ABI21_0_0EXUtil ()

@property (nonatomic, weak) id<ABI21_0_0EXUtilScopedModuleDelegate> kernelUpdatesServiceDelegate;
@property (nonatomic, weak) id<ABI21_0_0EXUtilService> kernelUtilService;

@end

ABI21_0_0EX_DEFINE_SCOPED_MODULE_GETTER(ABI21_0_0EXUtil, util)

@implementation ABI21_0_0EXUtil

@synthesize bridge = _bridge;

// delegate to kernel linking manager because our only kernel work (right now)
// is refreshing the foreground task.
ABI21_0_0EX_EXPORT_SCOPED_MULTISERVICE_MODULE(ExponentUtil, @"UpdatesManager", @"UtilService");
...
ABI21_0_0RCT_REMAP_METHOD(getCurrentLocaleAsync,
                 getCurrentLocaleWithResolver:(ABI21_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (preferredLanguages.count > 0) {
    ...
    reject(@"E_NO_PREFERRED_LOCALE", errMsg, ABI21_0_0RCTErrorWithMessage(errMsg));
  }
}

ABI21_0_0RCT_REMAP_METHOD(getCurrentDeviceCountryAsync,
                 getCurrentDeviceCountryWithResolver:(ABI21_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (countryCode) {
    ...
    reject(@"E_NO_DEVICE_COUNTRY", errMsg, ABI21_0_0RCTErrorWithMessage(errMsg));
  }
}

ABI21_0_0RCT_REMAP_METHOD(getCurrentTimeZoneAsync,
                 getCurrentTimeZoneWithResolver:(ABI21_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI21_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (currentTimeZone) {
    ...
    reject(@"E_NO_DEVICE_TIMEZONE", errMsg, ABI21_0_0RCTErrorWithMessage(errMsg));
  }
}
