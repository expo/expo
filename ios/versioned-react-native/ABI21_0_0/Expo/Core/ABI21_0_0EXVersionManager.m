...
#import "ABI21_0_0EXDisabledDevMenu.h"
#import "ABI21_0_0EXDisabledRedBox.h"
#import "ABI21_0_0EXFileSystem.h"
#import "ABI21_0_0EXFrameExceptionsManager.h"
#import "ABI21_0_0EXKernelModule.h"
#import "ABI21_0_0EXVersionManager.h"
...
#import <ReactABI21_0_0/ABI21_0_0RCTAsyncLocalStorage.h>

#import <objc/message.h>

static NSNumber *ABI21_0_0EXVersionManagerIsFirstLoad;

// used for initializing scoped modules which don't tie in to any kernel service.
#define ABI21_0_0EX_KERNEL_SERVICE_NONE @"ABI21_0_0EXKernelServiceNone"

// this is needed because ABI21_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI21_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSString *> *ABI21_0_0EXScopedModuleClasses;
void ABI21_0_0EXRegisterScopedModule(Class, NSString *);
void ABI21_0_0EXRegisterScopedModule(Class moduleClass, NSString *kernelServiceClassName)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI21_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  
  NSString *unversionedKernelServiceClassName;
  if ([kernelServiceClassName isEqualToString:@"nil"]) {
    unversionedKernelServiceClassName = ABI21_0_0EX_KERNEL_SERVICE_NONE;
  } else {
  ...
  if (moduleClassName) {
    ABI21_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassName;
  }
}

@interface ABI21_0_0EXVersionManager ()
...
@implementation ABI21_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI21_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI21_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI21_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
#ifdef ABI21_0_0EX_ENABLE_UNSAFE_SYSTRACE
    items[@"dev-profiler"] = @{
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI21_0_0RCTDevMenu *)devMenu) show];
  }
}
...
- (id<ABI21_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI21_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  if (ABI21_0_0EXVersionManagerIsFirstLoad == nil) {
    // first time initializing this RN version at runtime
    _isFirstLoad = YES;
  }
  ABI21_0_0EXVersionManagerIsFirstLoad = @(NO);
  ABI21_0_0RCTSetFatalHandler(fatalHandler);
  ABI21_0_0RCTSetLogThreshold(threshold);
  ABI21_0_0RCTSetLogFunction(logFunction);
}
 ...
 *    @ABI21_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI21_0_0EXKernel *kernel
 *    NSArray *supportedSdkVersions
 *    id exceptionsManagerDelegate
 *
 * Frame-only:
 *    ABI21_0_0EXFrame *frame
 */
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSString *localStorageDirectory = [[ABI21_0_0EXFileSystem documentDirectoryForExperienceId:experienceId] stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];

  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI21_0_0EXAppState alloc] init],
                                    [[ABI21_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:isDeveloper],
                                    [[ABI21_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI21_0_0EXStatusBarManager alloc] init],
                                    [[ABI21_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory],
                                    ]];
  ...
  if (params[@"frame"]) {
    [extraModules addObject:[[ABI21_0_0EXFrameExceptionsManager alloc] initWithDelegate:params[@"frame"]]];
  } else {
    id exceptionsManagerDelegate = params[@"exceptionsManagerDelegate"];
    if (exceptionsManagerDelegate) {
      ABI21_0_0RCTExceptionsManager *exceptionsManager = [[ABI21_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
      [extraModules addObject:exceptionsManager];
    } else {
      ABI21_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
    }
  }
  
  if (params[@"testEnvironment"]) {
    ABI21_0_0EXTestEnvironment testEnvironment = (ABI21_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI21_0_0EXTestEnvironmentNone) {
      ABI21_0_0EXTest *testModule = [[ABI21_0_0EXTest alloc] initWithEnvironment:testEnvironment];
      [extraModules addObject:testModule];
    }
  }
  
  if (params[@"kernel"]) {
    ABI21_0_0EXKernelModule *kernel = [[ABI21_0_0EXKernelModule alloc] initWithExperienceId:experienceId
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI21_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI21_0_0RCTDevMenu
    [extraModules addObject:[[ABI21_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI21_0_0RCTRedBox
    [extraModules addObject:[[ABI21_0_0EXDisabledRedBox alloc] init]];
  }
  return extraModules;
}
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI21_0_0EXScopedModuleClasses) {
    [ABI21_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSString * _Nonnull kernelServiceClassName, BOOL * _Nonnull stop) {
      id service = ([kernelServiceClassName isEqualToString:ABI21_0_0EX_KERNEL_SERVICE_NONE]) ? nil : services[kernelServiceClassName];
