...
@class ABI21_0_0RCTRedBoxWindow;

@protocol ABI21_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI21_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI21_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI21_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI21_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI21_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI21_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI21_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI21_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI21_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI21_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI21_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI21_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI21_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI21_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI21_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI21_0_0RCTKeyCommands for this, but since
...
@interface ABI21_0_0RCTRedBox () <ABI21_0_0RCTInvalidating, ABI21_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI21_0_0RCTRedBox
{
  ABI21_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI21_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (void)registerErrorCustomizer:(id<ABI21_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI21_0_0RCTErrorInfo *)_customizeError:(ABI21_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI21_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI21_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI21_0_0RCTJSStackTraceKey]];
}
...
{
  NSArray<ABI21_0_0RCTJSStackFrame *> *stack = [ABI21_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI21_0_0RCTJSStackFrame class]]) {
    stack = [ABI21_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI21_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI21_0_0RCTErrorInfo *errorInfo = [[ABI21_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI21_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI21_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI21_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI21_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI21_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI21_0_0RCTRedBoxWindow *)redBoxWindow
{
  [_bridge reload];
  [self dismiss];
}
...
@implementation ABI21_0_0RCTBridge (ABI21_0_0RCTRedBox)

- (ABI21_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI21_0_0RCTRedBox class]];
}
...
- (ABI21_0_0RCTRedBox *)redBox { return nil; }
