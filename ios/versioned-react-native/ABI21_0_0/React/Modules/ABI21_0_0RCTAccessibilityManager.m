...
NSString *const ABI21_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI21_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI21_0_0RCTAccessibilityManager ()
...
@implementation ABI21_0_0RCTAccessibilityManager
  ...
  if ((self = [super init])) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didReceiveNewContentSizeCategory:)
                                               ...
                                               object:ABI21_0_0RCTSharedApplication()];
    ...
    self.contentSizeCategory = ABI21_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI21_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI21_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI21_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI21_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI21_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI21_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI21_0_0Tag:ReactABI21_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI21_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI21_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI21_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI21_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI21_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI21_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI21_0_0RCTBridge (ABI21_0_0RCTAccessibilityManager)

- (ABI21_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI21_0_0RCTAccessibilityManager class]];
}
