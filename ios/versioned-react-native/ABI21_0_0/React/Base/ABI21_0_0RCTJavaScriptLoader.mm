...
NSString *const ABI21_0_0RCTJavaScriptLoaderErrorDomain = @"ABI21_0_0RCTJavaScriptLoaderErrorDomain";

@implementation ABI21_0_0RCTLoadingProgress
...
@implementation ABI21_0_0RCTJavaScriptLoader
...
+ (void)loadBundleAtURL:(NSURL *)scriptURL onProgress:(ABI21_0_0RCTSourceLoadProgressBlock)onProgress onComplete:(ABI21_0_0RCTSourceLoadBlock)onComplete
{
  ...
  [error.domain isEqualToString:ABI21_0_0RCTJavaScriptLoaderErrorDomain]
  && error.code == ABI21_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI21_0_0RCTJavaScriptLoaderErrorNoScriptURL
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI21_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI21_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI21_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  case facebook::ReactABI21_0_0::ScriptTag::BCBundle:
    if (runtimeBCVersion == JSNoBytecodeFileFormatVersion || runtimeBCVersion < 0) {
      if (error) {
        *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI21_0_0RCTJavaScriptLoaderErrorBCNotSupported
                                 userInfo:@{NSLocalizedDescriptionKey:
      ...
      if (error) {
        ...
        *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI21_0_0RCTJavaScriptLoaderErrorBCVersion
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI21_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                               userInfo:@{NSLocalizedDescriptionKey:
...
static void attemptAsynchronousLoadOfBundleAtURL(NSURL *scriptURL, ABI21_0_0RCTSourceLoadProgressBlock onProgress, ABI21_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  ABI21_0_0RCTMultipartDataTask *task = [[ABI21_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
    if (!done) {
      if (onProgress) {
    ...
    if (error) {
      if ([error.domain isEqualToString:NSURLErrorDomain]) {
        error = [NSError errorWithDomain:ABI21_0_0RCTJavaScriptLoaderErrorDomain
                                    code:ABI21_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                                userInfo:
                 @{
                   NSLocalizedDescriptionKey:
                     [@"Could not connect to development server.\n\n"
                      "Ensure the following:\n"
                      "- Node server is running and available on the same network - run 'npm start' from ReactABI21_0_0-native root\n"
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI21_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI21_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI21_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI21_0_0RCTLoadingProgress *progress = [ABI21_0_0RCTLoadingProgress new];
...
static ABI21_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI21_0_0RCTLoadingProgress *progress = [ABI21_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI21_0_0RCTJSONParse(rawText, nil);
