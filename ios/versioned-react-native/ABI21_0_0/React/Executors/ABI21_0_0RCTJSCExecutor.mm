...
#import <ReactABI21_0_0/ABI21_0_0RCTJSCErrorHandling.h>
...
#if (ABI21_0_0RCT_PROFILE || ABI21_0_0RCT_DEV) && __has_include("ABI21_0_0RCTDevMenu.h")
#import "ABI21_0_0RCTDevMenu.h"
#endif

NSString *const ABI21_0_0RCTJSCThreadName = @"com.facebook.ReactABI21_0_0.JavaScript";
NSString *const ABI21_0_0RCTJavaScriptContextCreatedNotification = @"ABI21_0_0RCTJavaScriptContextCreatedNotification";
...
struct TaggedScript {
  const facebook::ReactABI21_0_0::ScriptTag tag;
  const NSData *script;
};

#if ABI21_0_0RCT_PROFILE
@interface ABI21_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI21_0_0RCTCookieMap @end
#endif

struct ABI21_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
};

@interface ABI21_0_0RCTJavaScriptContext : NSObject <ABI21_0_0RCTInvalidating>
...
@implementation ABI21_0_0RCTJavaScriptContext
{
  ABI21_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    _context = context;
    _context.name = @"ABI21_0_0RCTJSContext";
    _javaScriptThread = javaScriptThread;

    /**
     * Explicitly introduce a retain cycle here - The ABI21_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI21_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI21_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI21_0_0RCTPerformanceLogger *_performanceLogger;
  ABI21_0_0RCTJavaScriptContext *_context;
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI21_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI21_0_0RCTJSCThreadName;
...
{
  ABI21_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI21_0_0RCTJSCExecutor init]", nil);
...
- (instancetype)initWithJSContextData:(const ABI21_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    _useCustomJSCLibrary = data.useCustomJSCLibrary;
    _valid = YES;
    _javaScriptThread = data.javaScriptThread;
    _context = [[ABI21_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
                                         ...
                                         sourceURL:(NSURL *)sourceURL
{
  ...
  if (taggedScript.tag == facebook::ReactABI21_0_0::ScriptTag::RAMBundle) {
...
- (ABI21_0_0RCTJavaScriptContext *)context
{
  if (!self.isValid) {
    return nil;
  }
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI21_0_0RCTProfileDidStartProfiling, ABI21_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_context) {
      ...
      if (self->_useCustomJSCLibrary) {
        JSC_configureJSCForIOS(true, ABI21_0_0RCTJSONStringify(@{
      ...
      self->_context = [[ABI21_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI21_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];

      installBasicSynchronousHooksOnContext(context);
    }

    ABI21_0_0RCTFBQuickPerformanceLoggerConfigureHooks(context.JSGlobalContextRef);

    __weak ABI21_0_0RCTJSCExecutor *weakSelf = self;
    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI21_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI21_0_0RCT_PROFILE_BEGIN_EVENT(ABI21_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI21_0_0RCT_PROFILE_END_EVENT(ABI21_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI21_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI21_0_0RCT_PROFILE_BEGIN_EVENT(ABI21_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI21_0_0RCT_PROFILE_BEGIN_EVENT(ABI21_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI21_0_0RCT_PROFILE
    __weak ABI21_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI21_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI21_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI21_0_0RCTProfileIsProfiling()) {
        ...
        _ABI21_0_0RCTProfileEndFlowEvent(newCookie);
...
#if __has_include("ABI21_0_0RCTDevMenu.h")
      // Allow to toggle the sampling profiler through RN's dev menu
      [self->_bridge.devMenu addItem:[ABI21_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
        ABI21_0_0RCTJSCExecutor *strongSelf = weakSelf;
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI21_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ...
    if (logLevel) {
      level = MAX(level, (ABI21_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI21_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI21_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI21_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI21_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI21_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI21_0_0RCTCookieMap *cookieMap = [ABI21_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI21_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI21_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI21_0_0RCTProfileDidStartProfiling];
...
- (int32_t)bytecodeFileFormatVersion
{
  return _useCustomJSCLibrary
    ? facebook::ReactABI21_0_0::customJSCWrapper()->JSBytecodeFileFormatVersion
...
ABI21_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)contextName)
{
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI21_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI21_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI21_0_0RCTJavaScriptCallback)onComplete
{
  ABI21_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI21_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI21_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI21_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, batchedBridgeRef) != kJSTypeUndefined) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, methodJSRef) != kJSTypeUndefined) {
        ...
        if (!errorJSRef && JSC_JSValueGetType(ctx, methodJSRef) == kJSTypeUndefined) {
          error = ABI21_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSC_JSValueGetType(ctx, batchedBridgeRef) == kJSTypeUndefined) {
        error = ABI21_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined. This can happen "
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI21_0_0RCTNSErrorFromJSError([JSC_JSValue(ctx) valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI21_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
{
  ABI21_0_0RCTAssertParam(script);
  ABI21_0_0RCTAssertParam(sourceURL);
...
static TaggedScript loadTaggedScript(NSData *script,
                                     NSURL *sourceURL,
                                     ABI21_0_0RCTPerformanceLogger *performanceLogger,
                                     RandomAccessBundleData &randomAccessBundle,
                                     NSError **error)
{
  ABI21_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);

  facebook::ReactABI21_0_0::BundleHeader header;
  [script getBytes:&header length:sizeof(header)];
  facebook::ReactABI21_0_0::ScriptTag tag = facebook::ReactABI21_0_0::parseTypeFromHeader(header);

  NSData *loadedScript = NULL;
  switch (tag) {
    case facebook::ReactABI21_0_0::ScriptTag::RAMBundle:
      [performanceLogger markStartForTag:ABI21_0_0RCTPLRAMBundleLoad];

      loadedScript = loadRAMBundle(sourceURL, error, randomAccessBundle);

      [performanceLogger markStopForTag:ABI21_0_0RCTPLRAMBundleLoad];
      [performanceLogger setValue:loadedScript.length forTag:ABI21_0_0RCTPLRAMStartupCodeSize];
      break;

    case facebook::ReactABI21_0_0::ScriptTag::BCBundle:
      loadedScript = script;
      break;

    case facebook::ReactABI21_0_0::ScriptTag::String: {
...
static void registerNativeRequire(JSContext *context, ABI21_0_0RCTJSCExecutor *executor)
{
  __weak ABI21_0_0RCTJSCExecutor *weakExecutor = executor;
...
{
  ABI21_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(taggedScript.script.length)
  }));

  [performanceLogger markStartForTag:ABI21_0_0RCTPLScriptExecution];
  ...
  [performanceLogger markStopForTag:ABI21_0_0RCTPLScriptExecution];

  NSError *error = jsError
    ? ABI21_0_0RCTNSErrorFromJSErrorRef(jsError, ctx)
    : nil;
              ...
              callback:(ABI21_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI21_0_0RCT_DEBUG) {
    ABI21_0_0RCTAssert(ABI21_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI21_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    JSGlobalContextRef ctx = strongSelf->_context.context.JSGlobalContextRef;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI21_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI21_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI21_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
      }
    }
    ABI21_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI21_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI21_0_0RCTFatal(ABI21_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    NSError *error = ABI21_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI21_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI21_0_0RCTPLRAMNativeRequires];
  ABI21_0_0RCT_PROFILE_BEGIN_EVENT(ABI21_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI21_0_0RCT_PROFILE_END_EVENT(ABI21_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI21_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI21_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI21_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
@implementation ABI21_0_0RCTJSContextProvider
{
...
- (ABI21_0_0RCTJSContextData)data
{
...
- (ABI21_0_0RCTJSCExecutor *)createExecutorWithContext:(JSContext **)JSContext
{
  const ABI21_0_0RCTJSContextData data = self.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  return [[ABI21_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
}
