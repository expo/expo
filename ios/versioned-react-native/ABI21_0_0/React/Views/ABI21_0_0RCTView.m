...
#import "ABI21_0_0RCTAutoInsetsProtocol.h"
...
@implementation UIView (ABI21_0_0RCTViewUnmounting)

- (void)ReactABI21_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI21_0_0_remountAllSubviews];
  }
}

- (void)ReactABI21_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI21_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI21_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI21_0_0RootView]) {
      break;
    }
...
static NSString *ABI21_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI21_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI21_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI21_0_0RCTBorderStyleSolid;
...
ABI21_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)

- (void)setReactABI21_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  _ReactABI21_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI21_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI21_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI21_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI21_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI21_0_0RCTPointerEventsNone || _pointerEvents == ABI21_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ReactABI21_0_0ZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI21_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI21_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI21_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
{
  if (self.ReactABI21_0_0AccessibilityElement == self) {
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI21_0_0Tag: %@;", self.ReactABI21_0_0Tag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI21_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ReactABI21_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI21_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ReactABI21_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI21_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI21_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI21_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI21_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.ReactABI21_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI21_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI21_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI21_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI21_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI21_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI21_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI21_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI21_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI21_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI21_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI21_0_0RCTCornerRadii){
...
{
  return (ABI21_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI21_0_0RCTUpdateShadowPathForView(self);

  const ABI21_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI21_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI21_0_0RCTRunningInTestEnvironment() &&
  ABI21_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI21_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI21_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI21_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI21_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
  ...
  if (ABI21_0_0RCTRunningInTestEnvironment()) {
    const CGSize size = self.bounds.size;
...
static BOOL ABI21_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI21_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI21_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI21_0_0RCTUpdateShadowPathForView(ABI21_0_0RCTView *view)
{
  if (ABI21_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI21_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI21_0_0RCTCornerInsets cornerInsets = ABI21_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI21_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI21_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI21_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI21_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI21_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI21_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI21_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI21_0_0RCTBorderStyle)style \
  {                                                    \
