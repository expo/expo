...
#import "ABI20_0_0EXDisabledDevLoadingView.h"
...
#import "ABI20_0_0EXKernelModule.h"
#import "ABI20_0_0EXVersionManager.h"
#import "ABI20_0_0EXStatusBarManager.h"
...
#import <ReactABI20_0_0/ABI20_0_0RCTAsyncLocalStorage.h>

#import <objc/message.h>

static NSNumber *ABI20_0_0EXVersionManagerIsFirstLoad;

// used for initializing scoped modules which don't tie in to any kernel service.
#define ABI20_0_0EX_KERNEL_SERVICE_NONE @"ABI20_0_0EXKernelServiceNone"

// this is needed because ABI20_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI20_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSString *> *ABI20_0_0EXScopedModuleClasses;
void ABI20_0_0EXRegisterScopedModule(Class, NSString *);
void ABI20_0_0EXRegisterScopedModule(Class moduleClass, NSString *kernelServiceClassName)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI20_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  
  NSString *unversionedKernelServiceClassName;
  if ([kernelServiceClassName isEqualToString:@"nil"]) {
    unversionedKernelServiceClassName = ABI20_0_0EX_KERNEL_SERVICE_NONE;
  } else {
  ...
  if (moduleClassName) {
    ABI20_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassName;
  }
}

@interface ABI20_0_0EXVersionManager ()
...
@implementation ABI20_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI20_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI20_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI20_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI20_0_0RCTDevMenu *)devMenu) show];
  }
}
...
- (id<ABI20_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI20_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  if (ABI20_0_0EXVersionManagerIsFirstLoad == nil) {
    // first time initializing this RN version at runtime
    _isFirstLoad = YES;
  }
  ABI20_0_0EXVersionManagerIsFirstLoad = @(NO);
  ABI20_0_0RCTSetFatalHandler(fatalHandler);
  ABI20_0_0RCTSetLogThreshold(threshold);
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSString *localStorageDirectory = [[ABI20_0_0EXFileSystem documentDirectoryForExperienceId:experienceId] stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];

  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI20_0_0EXAppState alloc] init],
                                    [[ABI20_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:isDeveloper],
                                    [[ABI20_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI20_0_0EXStatusBarManager alloc] init],
                                    [[ABI20_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory],
                                    ]];
  ...
  if (params[@"frame"]) {
    [extraModules addObject:[[ABI20_0_0EXFrameExceptionsManager alloc] initWithDelegate:params[@"frame"]]];
  } else {
    id exceptionsManagerDelegate = params[@"exceptionsManagerDelegate"];
    if (exceptionsManagerDelegate) {
      ABI20_0_0RCTExceptionsManager *exceptionsManager = [[ABI20_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
      [extraModules addObject:exceptionsManager];
    } else {
      ABI20_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
    }
  }
  
  if (params[@"kernel"]) {
    ABI20_0_0EXKernelModule *kernel = [[ABI20_0_0EXKernelModule alloc] initWithExperienceId:experienceId
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI20_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI20_0_0RCTDevMenu
    [extraModules addObject:[[ABI20_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI20_0_0RCTRedBox
    [extraModules addObject:[[ABI20_0_0EXDisabledRedBox alloc] init]];
  }
  return extraModules;
}
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI20_0_0EXScopedModuleClasses) {
    [ABI20_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSString * _Nonnull kernelServiceClassName, BOOL * _Nonnull stop) {
      id service = ([kernelServiceClassName isEqualToString:ABI20_0_0EX_KERNEL_SERVICE_NONE]) ? nil : services[kernelServiceClassName];
