...
#import "ABI20_0_0EXGoogle.h"
#import "ABI20_0_0EXOAuthViewController.h"
...
NSString *ABI20_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI20_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI20_0_0EXGoogle
{
  ABI20_0_0RCTPromiseResolveBlock _logInResolve;
  ABI20_0_0RCTPromiseRejectBlock _logInReject;
}

ABI20_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI20_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI20_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI20_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      ...
      _logInReject(ABI20_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI20_0_0RCTPresentedViewController()
                                                   callback:callback];
...
{
  [ABI20_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}
