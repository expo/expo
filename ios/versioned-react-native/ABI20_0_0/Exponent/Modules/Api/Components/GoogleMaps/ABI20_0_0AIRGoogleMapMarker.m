...
#import "ABI20_0_0AIRGoogleMapCallout.h"
#import "ABI20_0_0AIRDummyView.h"
...
@interface ABI20_0_0AIRGoogleMapMarker ()
- (id)eventFromMarker:(ABI20_0_0AIRGMSMarker*)marker;
@end

@implementation ABI20_0_0AIRGoogleMapMarker {
...
{
  if ((self = [super init])) {
    _realMarker = [[ABI20_0_0AIRGMSMarker alloc] init];
...
- (void)insertReactABI20_0_0Subview:(id<ABI20_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  if ([subview isKindOfClass:[ABI20_0_0AIRGoogleMapCallout class]]) {
    self.calloutView = (ABI20_0_0AIRGoogleMapCallout *)subview;
  } else { // a child view of the marker
    [self iconViewInsertSubview:(UIView*)subview atIndex:atIndex+1];
  }
  ABI20_0_0AIRDummyView *dummySubview = [[ABI20_0_0AIRDummyView alloc] initWithView:(UIView *)subview];
  [super insertReactABI20_0_0Subview:(UIView*)dummySubview atIndex:atIndex];
}

- (void)removeReactABI20_0_0Subview:(id<ABI20_0_0RCTComponent>)dummySubview {
  UIView* subview = ((ABI20_0_0AIRDummyView*)dummySubview).view;
  ...
  [super removeReactABI20_0_0Subview:(UIView*)dummySubview];
}
...
- (void)didTapInfoWindowOfMarker:(ABI20_0_0AIRGMSMarker *)marker {
...
- (void)didBeginDraggingMarker:(ABI20_0_0AIRGMSMarker *)marker {
  if (!self.onDragStart) return;
  self.onDragStart([self eventFromMarker:marker]);
}

- (void)didEndDraggingMarker:(ABI20_0_0AIRGMSMarker *)marker {
  if (!self.onDragEnd) return;
  self.onDragEnd([self eventFromMarker:marker]);
}

- (void)didDragMarker:(ABI20_0_0AIRGMSMarker *)marker {
...
- (void)setOnPress:(ABI20_0_0RCTBubblingEventBlock)onPress {
  _realMarker.onPress = onPress;
}

- (ABI20_0_0RCTBubblingEventBlock)onPress {
...
- (void)setImageSrc:(NSString *)imageSrc
{
  ...
  if (!_iconImageView) {
    // prevent glitch with marker (cf. https://github.com/airbnb/ReactABI20_0_0-native-maps/issues/738)
...
//                                                                   }
                                                                   ...
                                                                   //       Shouldn't required images have size info automatically via ABI20_0_0RN?
