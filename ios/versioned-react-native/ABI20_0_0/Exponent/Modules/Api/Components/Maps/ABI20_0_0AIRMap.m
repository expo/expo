...
const CLLocationDegrees ABI20_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI20_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI20_0_0AIRMapZoomBoundBuffer = 0.01;
...
@implementation ABI20_0_0AIRMap
{
    UIView *_legalLabel;
    CLLocationManager *_locationManager;
    BOOL _initialRegionSet;

    // Array to manually track ABI20_0_0RN subviews
    //
    // ABI20_0_0AIRMap implicitly creates subviews that aren't regular ABI20_0_0RN children
    // (ABI20_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI20_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI20_0_0-native/blob/v0.16.0/ReactABI20_0_0/Modules/ABI20_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI20_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI20_0_0-native/blob/v0.16.0/Libraries/Text/ABI20_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ReactABI20_0_0Subviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _ReactABI20_0_0Subviews = [NSMutableArray new];
        ...
        self.calloutView = [ABI20_0_0SMCalloutView platformCalloutView];
...
- (void)insertReactABI20_0_0Subview:(id<ABI20_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI20_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI20_0_0AIRMapPolyline class]]) {
        ((ABI20_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI20_0_0AIRMapPolygon class]]) {
        ((ABI20_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI20_0_0AIRMapCircle class]]) {
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI20_0_0AIRMapUrlTile class]]) {
        ((ABI20_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI20_0_0RCTComponent>> *childSubviews = [subview ReactABI20_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertReactABI20_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ReactABI20_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI20_0_0Subview:(id<ABI20_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI20_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI20_0_0RCTComponent>> *childSubviews = [subview ReactABI20_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeReactABI20_0_0Subview:(UIView *)childSubviews[i]];
        }
    }
    [_ReactABI20_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
#pragma mark ABI20_0_0SMCalloutViewDelegate
