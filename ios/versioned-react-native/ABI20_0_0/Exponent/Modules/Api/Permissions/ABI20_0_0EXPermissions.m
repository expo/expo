...
ABI20_0_0RCT_EXPORT_MODULE(ExponentPermissions);
                 ...
                 rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  if ([type isEqualToString:@"remoteNotifications"]) {
    resolve([ABI20_0_0EXRemoteNotificationRequester permissions]);
  } else if ([type isEqualToString:@"location"]) {
    resolve([ABI20_0_0EXLocationRequester permissions]);
  } else if ([type isEqualToString:@"camera"]) {
    resolve([ABI20_0_0EXCameraPermissionRequester permissions]);
  } else if ([type isEqualToString:@"contacts"]) {
    resolve([ABI20_0_0EXContactsRequester permissions]);
  } else if ([type isEqualToString:@"audioRecording"]) {
    resolve([ABI20_0_0EXAudioRecordingPermissionRequester permissions]);
  } else {
                 ...
                 rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  [self getCurrentPermissionsWithType:type resolver:^(NSDictionary *result) {
    if (result && [result[@"status"] isEqualToString:[ABI20_0_0EXPermissions permissionStringForStatus:ABI20_0_0EXPermissionStatusGranted]]) {
