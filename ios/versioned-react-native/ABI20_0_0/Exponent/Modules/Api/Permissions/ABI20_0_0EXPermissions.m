...
#import "ABI20_0_0EXCameraPermissionRequester.h"
#import "ABI20_0_0EXContactsRequester.h"
#import "ABI20_0_0EXLocationRequester.h"
#import "ABI20_0_0EXPermissions.h"
#import "ABI20_0_0EXRemoteNotificationRequester.h"
#import "ABI20_0_0EXScopedModuleRegistry.h"
#import <ABI20_0_0React/ABI20_0_0RCTUtils.h>

NSString * const ABI20_0_0EXPermissionExpiresNever = @"never";

@interface ABI20_0_0EXPermissions ()
...
@implementation ABI20_0_0EXPermissions

ABI20_0_0EX_EXPORT_SCOPED_MODULE(ExponentPermissions, PermissionsManager);
...
ABI20_0_0RCT_REMAP_METHOD(getAsync,
                 ...
                 rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (![type isEqualToString:@"notifications"] && ![type isEqualToString:@"userFacingNotifications"] &&
      [globalPermissionsResult[@"status"] isEqualToString:[[self class] permissionStringForStatus:ABI20_0_0EXPermissionStatusGranted]]) {
    if (![_kernelPermissionsServiceDelegate hasGrantedPermission:type forExperience:self.experienceId]) {
      response[@"status"] = [[self class] permissionStringForStatus:ABI20_0_0EXPermissionStatusDenied];
    }
  }
  resolve(response);
}

ABI20_0_0RCT_REMAP_METHOD(askAsync,
                 ...
                 rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([systemPermissions[@"status"] isEqualToString:[ABI20_0_0EXPermissions permissionStringForStatus:ABI20_0_0EXPermissionStatusGranted]]) {
    // temporarily exclude notifactions from permissions per experience
    if (![type isEqualToString:@"notifications"] && ![type isEqualToString:@"userFacingNotifications"] &&
      ...
      UIAlertAction *allow = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        if ([_kernelPermissionsServiceDelegate savePermission:systemPermissions ofType:type forExperience:self.experienceId]) {
          ...
          deniedResult[@"status"] = [[weakSelf class] permissionStringForStatus:ABI20_0_0EXPermissionStatusDenied];
    ...
    NSObject<ABI20_0_0EXPermissionRequester> *requester;
    if ([type isEqualToString:@"remoteNotifications"]) {
      requester = [[ABI20_0_0EXRemoteNotificationRequester alloc] init];
    } else if ([type isEqualToString:@"location"]) {
      requester = [[ABI20_0_0EXLocationRequester alloc] init];
    } else if ([type isEqualToString:@"camera"]) {
      requester = [[ABI20_0_0EXCameraPermissionRequester alloc] init];
    } else if ([type isEqualToString:@"contacts"]) {
      requester = [[ABI20_0_0EXContactsRequester alloc] init];
    } else if ([type isEqualToString:@"audioRecording"]) {
      requester = [[ABI20_0_0EXAudioRecordingPermissionRequester alloc] init];
    } else {
...
{
  if ([type isEqualToString:@"remoteNotifications"]) {
    return [ABI20_0_0EXRemoteNotificationRequester permissions];
  } else if ([type isEqualToString:@"location"]) {
    return [ABI20_0_0EXLocationRequester permissions];
  } else if ([type isEqualToString:@"camera"]) {
    return [ABI20_0_0EXCameraPermissionRequester permissions];
  } else if ([type isEqualToString:@"contacts"]) {
    return [ABI20_0_0EXContactsRequester permissions];
  } else if ([type isEqualToString:@"audioRecording"]) {
    return [ABI20_0_0EXAudioRecordingPermissionRequester permissions];
  } else if ([type isEqualToString:@"systemBrightness"]) {
    return [ABI20_0_0EXPermissions alwaysGrantedPermissions]; // permission is implicit
  }
  return nil;
}
...
- (void)showPermissionRequestAlert:(NSString *)type withAllowAction:(UIAlertAction *)allow withDenyAction:(UIAlertAction *)deny
{
  ...
  [ABI20_0_0RCTPresentedViewController() presentViewController:alert animated:YES completion:nil];
}


+ (NSDictionary *)alwaysGrantedPermissions {
  return @{
    @"status": [ABI20_0_0EXPermissions permissionStringForStatus:ABI20_0_0EXPermissionStatusGranted],
...
{
  switch (status) {
    case ABI20_0_0EXPermissionStatusGranted:
      return @"granted";
    case ABI20_0_0EXPermissionStatusDenied:
      return @"denied";
    default:
      return @"undetermined";
  }
...
+ (ABI20_0_0EXPermissionStatus)statusForPermissions:(NSDictionary *)permissions
{
  ...
  if ([status isEqualToString:@"granted"]) {
    return ABI20_0_0EXPermissionStatusGranted;
  } else if ([status isEqualToString:@"denied"]) {
    return ABI20_0_0EXPermissionStatusDenied;
  } else {
    return ABI20_0_0EXPermissionStatusUndetermined;
  }
}
...
+ (NSString *)typeForRequester:(NSObject<ABI20_0_0EXPermissionRequester> *)requester
{
  if ([requester isKindOfClass:[ABI20_0_0EXLocationRequester class]]) {
    return @"location";
  } else if ([requester isKindOfClass:[ABI20_0_0EXCameraPermissionRequester class]]) {
    return @"camera";
  } else if ([requester isKindOfClass:[ABI20_0_0EXContactsRequester class]]) {
    return @"contacts";
  } else if ([requester isKindOfClass:[ABI20_0_0EXAudioRecordingPermissionRequester class]]) {
