...
@interface ABI20_0_0EXGLView ()

@property (nonatomic, weak) ABI20_0_0EXGLViewManager *viewManager;
...
@implementation ABI20_0_0EXGLView
...
- (instancetype)initWithManager:(ABI20_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
    _viewManager = viewManager;

    self.contentScaleFactor = ABI20_0_0RCTScreenScale();
...
- (void)setOnSurfaceCreate:(ABI20_0_0RCTDirectEventBlock)onSurfaceCreate
{
  _onSurfaceCreate = onSurfaceCreate;
  if (_onSurfaceCreate) {
    // Got non-empty onSurfaceCreate callback -- set up JS binding -- only possible on JavaScriptCore
    ABI20_0_0RCTBridge *bridge = _viewManager.bridge;
    if (!bridge.executorClass || [NSStringFromClass(bridge.executorClass) isEqualToString:@"ABI20_0_0RCTJSCExecutor"]) {
      // On JS thread, extract JavaScriptCore context, create ABI20_0_0EXGL context, call JS callback
      ...
      [bridge dispatchBlock:^{
        ...
        if (!jsContextRef) {
          ABI20_0_0RCTLogError(@"ABI20_0_0EXGL: The ReactABI20_0_0 Native bridge unexpectedly does not have a JavaScriptCore context.");
          return;
        }
      ...
      ABI20_0_0RCTLog(@"ABI20_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI20_0_0-native/docs/debugging.html)? ABI20_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI20_0_0EXGL.");
    }
  }
}
...
- (void)draw
{
  ...
  // called on the JS thread to create the ABI20_0_0EXGL context and save its id (see init method). In
