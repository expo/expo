...
#import "ABI20_0_0EXFontLoader.h"
...
static NSMutableDictionary *ABI20_0_0EXFonts = nil;

static const char *ABI20_0_0EXFontAssocKey = "ABI20_0_0EXFont";

@interface ABI20_0_0EXFont : NSObject
...
@implementation ABI20_0_0EXFont
...
- (UIFont *) UIFontWithSize:(CGFloat) fsize
{
  ...
  _sizes[size] = uiFont;
  objc_setAssociatedObject(uiFont, ABI20_0_0EXFontAssocKey, self, OBJC_ASSOCIATION_ASSIGN);
  return uiFont;
}
...
@implementation ABI20_0_0RCTFont (ABI20_0_0EXFontLoader)

// Will swap this with +[ABI20_0_0RCTFont updateFont: ...]
          ...
          scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  ABI20_0_0EXFont *exFont = nil;

  // Did we get a new family, and if so, is it associated with an ABI20_0_0EXFont?
  if ([family hasPrefix:exponentPrefix] && ABI20_0_0EXFonts) {
    NSString *suffix = [family substringFromIndex:exponentPrefix.length];
    exFont = ABI20_0_0EXFonts[suffix];
  }

  // Did the passed-in UIFont come from an ABI20_0_0EXFont?
  if (!exFont && uiFont) {
    exFont = objc_getAssociatedObject(uiFont, ABI20_0_0EXFontAssocKey);
  }

  // If it's an ABI20_0_0EXFont, generate the corresponding UIFont, else fallback to ReactABI20_0_0 Native's built-in method
  if (exFont) {
    return [exFont UIFontWithSize:[ABI20_0_0RCTConvert CGFloat:size] ?: uiFont.pointSize ?: defaultFontSize];
  } else {
...
@implementation ABI20_0_0EXFontLoader

ABI20_0_0RCT_EXPORT_MODULE(ExponentFontLoader);
...
+ (void)initialize {
  ...
  method_exchangeImplementations(class_getClassMethod([ABI20_0_0RCTFont class], a),
                                 class_getClassMethod([ABI20_0_0RCTFont class], b));
}

ABI20_0_0RCT_REMAP_METHOD(loadAsync,
...
{
  if (ABI20_0_0EXFonts && ABI20_0_0EXFonts[fontFamilyName]) {
  ...
  if (!ABI20_0_0EXFonts) {
    ABI20_0_0EXFonts = [NSMutableDictionary dictionary];
  }
  ABI20_0_0EXFonts[fontFamilyName] = [[ABI20_0_0EXFont alloc] initWithCGFont:font];
  resolve(nil);
}
