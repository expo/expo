#import "ABI20_0_0EXImagePicker.h"
...
@interface ABI20_0_0EXImagePicker ()
...
@implementation ABI20_0_0EXImagePicker

ABI20_0_0EX_EXPORT_SCOPED_MODULE(ExponentImagePicker, PermissionsManager);
...
ABI20_0_0RCT_EXPORT_METHOD(launchCameraAsync:(NSDictionary *)options
                  resolver:(ABI20_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI20_0_0EXPermissions statusForPermissions:[ABI20_0_0EXCameraPermissionRequester permissions]] != ABI20_0_0EXPermissionStatusGranted ||
  ...
  self.reject = reject;
  [self launchImagePicker:ABI20_0_0RNImagePickerTargetCamera options:options];
}

ABI20_0_0RCT_EXPORT_METHOD(launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI20_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI20_0_0RCTPromiseRejectBlock)reject)
{
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI20_0_0RNImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI20_0_0RNImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI20_0_0RNImagePickerTarget)target
{
  self.picker = [[UIImagePickerController alloc] init];

  if (target == ABI20_0_0RNImagePickerTargetCamera) {
#if TARGET_IPHONE_SIMULATOR
    self.reject(ABI20_0_0RCTErrorUnspecified, @"Camera not available on simulator", nil);
    return;
#else
...
#endif
  } else { // ABI20_0_0RNImagePickerTargetLibrarySingleImage
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
  dispatch_block_t dismissCompletionBlock = ^{
    ...
    ABI20_0_0RCTAssert([mediaType isEqualToString:(NSString *)kUTTypeImage], @"Expected image response from `UIImagePickerController`");
