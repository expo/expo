...
#import "ABI20_0_0RCTErrorInfo.h"
#import "ABI20_0_0RCTJSStackFrame.h"
#import "ABI20_0_0RCTUtils.h"

#if ABI20_0_0RCT_DEV

@class ABI20_0_0RCTRedBoxWindow;

@protocol ABI20_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI20_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI20_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI20_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI20_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI20_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI20_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI20_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI20_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI20_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI20_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI20_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI20_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI20_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI20_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI20_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI20_0_0RCTKeyCommands for this, but since
...
@interface ABI20_0_0RCTRedBox () <ABI20_0_0RCTInvalidating, ABI20_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI20_0_0RCTRedBox
{
  ABI20_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI20_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI20_0_0RCTErrorInfo *)_customizeError:(ABI20_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI20_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI20_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI20_0_0RCTJSStackTraceKey]];
}
...
{
  NSArray<ABI20_0_0RCTJSStackFrame *> *stack = [ABI20_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI20_0_0RCTJSStackFrame class]]) {
    stack = [ABI20_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI20_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI20_0_0RCTErrorInfo *errorInfo = [[ABI20_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI20_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI20_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI20_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI20_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI20_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI20_0_0RCTRedBoxWindow *)redBoxWindow
{
  [_bridge reload];
  [self dismiss];
}
...
@implementation ABI20_0_0RCTBridge (ABI20_0_0RCTRedBox)

- (ABI20_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI20_0_0RCTRedBox class]];
}
...
- (ABI20_0_0RCTRedBox *)redBox { return nil; }
