...
NSString *const ABI20_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI20_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI20_0_0RCTAccessibilityManager ()
...
@implementation ABI20_0_0RCTAccessibilityManager
  ...
  if ((self = [super init])) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didReceiveNewContentSizeCategory:)
                                               ...
                                               object:ABI20_0_0RCTSharedApplication()];
    ...
    self.contentSizeCategory = ABI20_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI20_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI20_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI20_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI20_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI20_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI20_0_0Tag)
{
...
ABI20_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI20_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI20_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI20_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI20_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI20_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI20_0_0RCTBridge (ABI20_0_0RCTAccessibilityManager)

- (ABI20_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI20_0_0RCTAccessibilityManager class]];
}
