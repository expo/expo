...
typedef NS_ENUM(NSUInteger, ABI20_0_0RCTNavigationLock) {
  ABI20_0_0RCTNavigationLockNone,
  ABI20_0_0RCTNavigationLockNative,
  ABI20_0_0RCTNavigationLockJavaScript
};

// By default the interactive pop gesture will be enabled when the navigation bar is displayed
// and disabled when hidden
// ABI20_0_0RCTPopGestureStateDefault maps to the default behavior (mentioned above). Once popGestureState
...
// back to it.
// ABI20_0_0RCTPopGestureStateEnabled will enable the gesture independent of nav bar visibility
// ABI20_0_0RCTPopGestureStateDisabled will disable the gesture independent of nav bar visibility
typedef NS_ENUM(NSUInteger, ABI20_0_0RCTPopGestureState) {
  ABI20_0_0RCTPopGestureStateDefault = 0,
  ABI20_0_0RCTPopGestureStateEnabled,
  ABI20_0_0RCTPopGestureStateDisabled
};
...
@interface ABI20_0_0RCTNavigationController : UINavigationController <UINavigationBarDelegate>
{
  dispatch_block_t _scrollCallback;
}

@property (nonatomic, assign) ABI20_0_0RCTNavigationLock navigationLock;

@end

/**
 * In general, `ABI20_0_0RCTNavigator` examines `_currentViews` (which are ReactABI20_0_0 child
 ...
 * ahead by adding/removing ReactABI20_0_0 subviews. Native gets ahead by swiping back,
 * or tapping the back button. In both cases, the other system is initially
 * unaware. And in both cases, `ABI20_0_0RCTNavigator` helps the other side "catch up".
 *
 * If `ABI20_0_0RCTNavigator` sees the number of ReactABI20_0_0 children have changed, it
 * pushes/pops accordingly. If `ABI20_0_0RCTNavigator` sees a `UIKit` driven push/pop, it
 ...
 *
 */
@implementation ABI20_0_0RCTNavigationController

/**
 * @param callback Callback that is invoked when a "scroll" interaction begins
 * so that `ABI20_0_0RCTNavigator` can notify `JavaScript`.
 */
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
  ...
  if (self.interactivePopGestureRecognizer.state == UIGestureRecognizerStateBegan) {
    if (self.navigationLock == ABI20_0_0RCTNavigationLockNone) {
      self.navigationLock = ABI20_0_0RCTNavigationLockNative;
      if (_scrollCallback) {
        _scrollCallback();
      }
    } else if (self.navigationLock == ABI20_0_0RCTNavigationLockJavaScript) {
      // This should never happen because we disable/enable the gesture
      // recognizer when we lock the navigation.
      ABI20_0_0RCTAssert(NO, @"Should never receive gesture start while JS locks navigator");
    }
  } else
#endif //TARGET_OS_TV
  {
    if (self.navigationLock == ABI20_0_0RCTNavigationLockNone) {
      ...
      // length (`currentReactABI20_0_0Count` - 1).
...
@interface ABI20_0_0RCTNavigator() <ABI20_0_0RCTWrapperViewControllerNavigationListener, UINavigationControllerDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, copy) ABI20_0_0RCTDirectEventBlock onNavigationProgress;
@property (nonatomic, copy) ABI20_0_0RCTBubblingEventBlock onNavigationComplete;

@property (nonatomic, assign) NSInteger previousRequestedTopOfStack;

@property (nonatomic, assign) ABI20_0_0RCTPopGestureState popGestureState;

// Previous views are only mainted in order to detect incorrect
// addition/removal of views below the `requestedTopOfStack`
@property (nonatomic, copy, readwrite) NSArray<ABI20_0_0RCTNavItem *> *previousViews;
@property (nonatomic, readwrite, strong) ABI20_0_0RCTNavigationController *navigationController;
/**
 ...
 * - We use `invalidate` to remove the `ABI20_0_0RCTNavigator`'s reference to the
 ...
 * remove it from the run loop when the view will be destroyed by ReactABI20_0_0.
 *
 * +----------+              +--------------+
...
@implementation ABI20_0_0RCTNavigator
{
...
- (instancetype)initWithBridge:(ABI20_0_0RCTBridge *)bridge
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _previousViews = @[];
    __weak ABI20_0_0RCTNavigator *weakSelf = self;
    _navigationController = [[ABI20_0_0RCTNavigationController alloc] initWithScrollCallback:^{
      [weakSelf dispatchFakeScrollEvent];
    }];
    _navigationController.delegate = self;
    ABI20_0_0RCTAssert([self requestSchedulingJavaScriptNavigation], @"Could not acquire JS navigation lock on init");
...
- (void)didUpdateFrame:(__unused ABI20_0_0RCTFrameUpdate *)update
{
...
- (void)setInteractivePopGestureEnabled:(BOOL)interactivePopGestureEnabled
{
  ...
  _popGestureState = interactivePopGestureEnabled ? ABI20_0_0RCTPopGestureStateEnabled : ABI20_0_0RCTPopGestureStateDisabled;
#endif
}
                    ...
                    animated:(__unused BOOL)animated
{
  ...
  [tc animateAlongsideTransition: ^(id<UIViewControllerTransitionCoordinatorContext> context) {
    ABI20_0_0RCTWrapperViewController *fromController =
      (ABI20_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey];
    ABI20_0_0RCTWrapperViewController *toController =
      (ABI20_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    NSUInteger indexOfFrom = [self.ReactABI20_0_0Subviews indexOfObject:fromController.navItem];
    NSUInteger indexOfTo = [self.ReactABI20_0_0Subviews indexOfObject:toController.navItem];
...
{
  if (_navigationController.navigationLock == ABI20_0_0RCTNavigationLockNone) {
    _navigationController.navigationLock = ABI20_0_0RCTNavigationLockJavaScript;
#if !TARGET_OS_TV
...
- (void)freeLock
{
  _navigationController.navigationLock = ABI20_0_0RCTNavigationLockNone;

  // Unless the pop gesture has been explicitly disabled (ABI20_0_0RCTPopGestureStateDisabled),
  // Set interactivePopGestureRecognizer.enabled to YES
  // If the popGestureState is ABI20_0_0RCTPopGestureStateDefault the default behavior will be maintained
#if !TARGET_OS_TV
  _navigationController.interactivePopGestureRecognizer.enabled = self.popGestureState != ABI20_0_0RCTPopGestureStateDisabled;
#endif
}

/**
 * A ReactABI20_0_0 subview can be inserted/removed at any time, however if the
 * `requestedTopOfStack` changes, there had better be enough subviews present
 * to satisfy the push/pop.
 */
- (void)insertReactABI20_0_0Subview:(ABI20_0_0RCTNavItem *)view atIndex:(NSInteger)atIndex
{
  ABI20_0_0RCTAssert([view isKindOfClass:[ABI20_0_0RCTNavItem class]], @"ABI20_0_0RCTNavigator only accepts ABI20_0_0RCTNavItem subviews");
  ABI20_0_0RCTAssert(
    _navigationController.navigationLock == ABI20_0_0RCTNavigationLockJavaScript,
...
- (void)layoutSubviews
{
  ...
  [self ReactABI20_0_0AddControllerToClosestParent:_navigationController];
  _navigationController.view.frame = self.bounds;
}

- (void)removeReactABI20_0_0Subview:(ABI20_0_0RCTNavItem *)subview
{
  if (self.ReactABI20_0_0Subviews.count <= 0 || subview == self.ReactABI20_0_0Subviews[0]) {
    ABI20_0_0RCTLogError(@"Attempting to remove invalid ABI20_0_0RCT subview of ABI20_0_0RCTNavigator");
    return;
  }
...
{
  [_bridge.eventDispatcher sendFakeScrollEvent:self.ReactABI20_0_0Tag];
}
 ...
 */
- (UIView *)ReactABI20_0_0Superview
{
  ABI20_0_0RCTAssert(!_bridge.isValid || self.superview != nil, @"put ReactABI20_0_0NavSuperviewLink back");
  UIView *superview = [super ReactABI20_0_0Superview];
  return superview ?: self.ReactABI20_0_0NavSuperviewLink;
}
...
- (void)ReactABI20_0_0BridgeDidFinishTransaction
{
  ...
  // The "ReactABI20_0_0 count" is the count of views that are visible on the navigation
  // stack.  There may be more beyond this - that aren't visible, and may be
  // deleted/purged soon.
  NSUInteger previousReactABI20_0_0Count =
    _previousRequestedTopOfStack == kNeverRequested ? 0 : _previousRequestedTopOfStack + 1;
  NSUInteger currentReactABI20_0_0Count = _requestedTopOfStack + 1;

  BOOL jsGettingAhead =
    //    ----- previously caught up ------          ------ no longer caught up -------
    viewControllerCount == previousReactABI20_0_0Count && currentReactABI20_0_0Count != viewControllerCount;
  BOOL jsCatchingUp =
    //    --- previously not caught up ----          --------- now caught up ----------
    viewControllerCount != previousReactABI20_0_0Count && currentReactABI20_0_0Count == viewControllerCount;
  BOOL jsMakingNoProgressButNeedsToCatchUp =
    //    --- previously not caught up ----          ------- still the same -----------
    viewControllerCount != previousReactABI20_0_0Count && currentReactABI20_0_0Count == previousReactABI20_0_0Count;
...
BOOL jsGettingtooSlow =
  ...
  BOOL ReactABI20_0_0PushOne = jsGettingAhead && currentReactABI20_0_0Count == previousReactABI20_0_0Count + 1;
  BOOL ReactABI20_0_0PopN = jsGettingAhead && currentReactABI20_0_0Count < previousReactABI20_0_0Count;
  ...
  if (!(jsGettingAhead ||
        ...
        jsGettingtooSlow)) {
    ABI20_0_0RCTLogError(@"JS has only made partial progress to catch up to UIKit");
  }
  if (currentReactABI20_0_0Count > self.ReactABI20_0_0Subviews.count) {
    ABI20_0_0RCTLogError(@"Cannot adjust current top of stack beyond available views");
  }

  // Views before the previous ReactABI20_0_0 count must not have changed. Views greater than previousReactABI20_0_0Count
  // up to currentReactABI20_0_0Count may have changed.
  for (NSUInteger i = 0; i < MIN(self.ReactABI20_0_0Subviews.count, MIN(_previousViews.count, previousReactABI20_0_0Count)); i++) {
    if (self.ReactABI20_0_0Subviews[i] != _previousViews[i]) {
      ABI20_0_0RCTLogError(@"current view should equal previous view");
    }
  }
  if (currentReactABI20_0_0Count < 1) {
    ABI20_0_0RCTLogError(@"should be at least one current view");
  }
  if (jsGettingAhead) {
    if (ReactABI20_0_0PushOne) {
      UIView *lastView = self.ReactABI20_0_0Subviews.lastObject;
      ABI20_0_0RCTWrapperViewController *vc = [[ABI20_0_0RCTWrapperViewController alloc] initWithNavItem:(ABI20_0_0RCTNavItem *)lastView];
      vc.navigationListener = self;
      _numberOfViewControllerMovesToIgnore = 1;
      [_navigationController pushViewController:vc animated:(currentReactABI20_0_0Count > 1)];
    } else if (ReactABI20_0_0PopN) {
      UIViewController *viewControllerToPopTo = _navigationController.viewControllers[(currentReactABI20_0_0Count - 1)];
      _numberOfViewControllerMovesToIgnore = viewControllerCount - currentReactABI20_0_0Count;
      [_navigationController popToViewController:viewControllerToPopTo animated:YES];
    } else {
      ABI20_0_0RCTLogError(@"Pushing or popping more than one view at a time from JS");
    }
  ...
  // otherwise we would unnecessarily retain a reference to view(s) no longer on the ReactABI20_0_0 navigation stack:
  NSUInteger expectedCount = MIN(currentReactABI20_0_0Count, self.ReactABI20_0_0Subviews.count);
  _previousViews = [[self.ReactABI20_0_0Subviews subarrayWithRange: NSMakeRange(0, expectedCount)] copy];
...
didMoveToNavigationController:(UINavigationController *)navigationController
{
  ...
  ABI20_0_0RCTAssert(_navigationController.navigationLock == ABI20_0_0RCTNavigationLockJavaScript ||
