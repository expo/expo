...
@interface ABI20_0_0RCTScrollEvent : NSObject <ABI20_0_0RCTEvent>
...
@implementation ABI20_0_0RCTScrollEvent
{
...
{
  ABI20_0_0RCTAssertParam(ReactABI20_0_0Tag);
...
- (ABI20_0_0RCTScrollEvent *)coalesceWithEvent:(ABI20_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI20_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI20_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI20_0_0RCTRefreshControl *rctRefreshControl;
#endif

@end


@implementation ABI20_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI20_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI20_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI20_0_0RCTUIManager JSResponder] isKindOfClass:[ABI20_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI20_0_0 scroll
...
- (void)setBounds:(CGRect)bounds
{
  if (isRectInvalid(bounds)) {
    ABI20_0_0RCTLogError(@"Attempted to set an invalid bounds to inner scrollview: %@", NSStringFromCGRect(bounds));
    return;
  }
...
- (void)setFrame:(CGRect)frame
{
  if (isRectInvalid(frame)) {
    ABI20_0_0RCTLogError(@"Attempted to set an invalid frame to inner scrollview: %@", NSStringFromCGRect(frame));
    return;
  }
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI20_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI20_0_0RCTScrollView
{
  ABI20_0_0RCTEventDispatcher *_eventDispatcher;
  ABI20_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI20_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI20_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI20_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI20_0_0RCTApplyTranformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
- (void)setReactABI20_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  [super setReactABI20_0_0LayoutDirection:layoutDirection];

  ABI20_0_0RCTApplyTranformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI20_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI20_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI20_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI20_0_0RCTAssert(_contentView == nil, @"ABI20_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI20_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, self.ReactABI20_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI20_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI20_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI20_0_0Subview:atIndex:`
}
...
- (void)layoutSubviews
{
  ...
  ABI20_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI20_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI20_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI20_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI20_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI20_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI20_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI20_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI20_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI20_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI20_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI20_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI20_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI20_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI20_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI20_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI20_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI20_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI20_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI20_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI20_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI20_0_0RCTView contentInsetsForView:self];
...
#define ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI20_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI20_0_0RCTScrollEvent *scrollEvent = [[ABI20_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI20_0_0RCTEventDispatcher (ABI20_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI20_0_0Tag
{
  ...
  ABI20_0_0RCTScrollEvent *fakeScrollEvent = [[ABI20_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI20_0_0Tag:ReactABI20_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
