...
@implementation UIView (ABI20_0_0RCTViewUnmounting)
...
- (void)ReactABI20_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI20_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI20_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI20_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ReactABI20_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI20_0_0RootView]) {
      break;
    }
...
static NSString *ABI20_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI20_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI20_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI20_0_0RCTBorderStyleSolid;
...
{
  _ReactABI20_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI20_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI20_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI20_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI20_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI20_0_0RCTPointerEventsNone || _pointerEvents == ABI20_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ReactABI20_0_0ZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI20_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI20_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI20_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (UIView *)ReactABI20_0_0AccessibilityElement
{
  return self;
}
...
{
  if (self.ReactABI20_0_0AccessibilityElement == self) {
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI20_0_0Tag: %@;", self.ReactABI20_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI20_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI20_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ReactABI20_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI20_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI20_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI20_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI20_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.ReactABI20_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI20_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI20_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI20_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI20_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI20_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI20_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI20_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI20_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI20_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI20_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI20_0_0RCTCornerRadii){
...
{
  return (ABI20_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI20_0_0RCTUpdateShadowPathForView(self);

  const ABI20_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI20_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI20_0_0RCTRunningInTestEnvironment() &&
  ABI20_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI20_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI20_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI20_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI20_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI20_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI20_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI20_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI20_0_0RCTUpdateShadowPathForView(ABI20_0_0RCTView *view)
{
  if (ABI20_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI20_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI20_0_0RCTCornerInsets cornerInsets = ABI20_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI20_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI20_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI20_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI20_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI20_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI20_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI20_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI20_0_0RCTBorderStyle)style \
  {                                                    \
