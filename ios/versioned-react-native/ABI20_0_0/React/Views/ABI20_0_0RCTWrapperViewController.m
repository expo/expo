...
@implementation ABI20_0_0RCTWrapperViewController
{
  UIView *_wrapperView;
  UIView *_contentView;
...
{
  ABI20_0_0RCTAssertParam(contentView);
...
ABI20_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithNibName:(NSString *)nn bundle:(NSBundle *)nb)
...
static BOOL ABI20_0_0RCTFindScrollViewAndRefreshContentInsetInView(UIView *view)
{
  if ([view conformsToProtocol:@protocol(ABI20_0_0RCTAutoInsetsProtocol)]) {
    [(id <ABI20_0_0RCTAutoInsetsProtocol>) view refreshContentInset];
    return YES;
  }
  for (UIView *subview in view.subviews) {
    if (ABI20_0_0RCTFindScrollViewAndRefreshContentInsetInView(subview)) {
      return YES;
    }
  }
  return NO;
...
- (void)viewDidLayoutSubviews
{
  ...
  if (_previousTopLayoutLength != _currentTopLayoutGuide.length ||
    ...
    ABI20_0_0RCTFindScrollViewAndRefreshContentInsetInView(_contentView);
    _previousTopLayoutLength = _currentTopLayoutGuide.length;
    _previousBottomLayoutLength = _currentBottomLayoutGuide.length;
  }
}

static UIView *ABI20_0_0RCTFindNavBarShadowViewInView(UIView *view)
{
  if ([view isKindOfClass:[UIImageView class]] && view.bounds.size.height <= 1) {
    return view;
  }
  for (UIView *subview in view.subviews) {
    UIView *shadowView = ABI20_0_0RCTFindNavBarShadowViewInView(subview);
    if (shadowView) {
      return shadowView;
    }
  }
...
- (void)viewWillAppear:(BOOL)animated
{
  ...
  if ([self.parentViewController isKindOfClass:[UINavigationController class]])
  {
    ...
    ABI20_0_0RCTFindNavBarShadowViewInView(bar).hidden = _navItem.shadowHidden;
...
- (void)loadView
{
  ...
  //`contentView` which is a ReactABI20_0_0-managed view.
