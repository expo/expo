...
@implementation ABI20_0_0RCTDevLoadingView
{
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
...
- (void)setBridge:(ABI20_0_0RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(hide)
                                               name:ABI20_0_0RCTJavaScriptDidFailToLoadNotification
                                             object:nil];
...
ABI20_0_0RCT_EXPORT_METHOD(showMessage:(NSString *)message color:(UIColor *)color backgroundColor:(UIColor *)backgroundColor)
{
  if (!isEnabled) {
    return;
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!self->_window && !ABI20_0_0RCTRunningInTestEnvironment()) {
...
ABI20_0_0RCT_EXPORT_METHOD(hide)
{
  if (!isEnabled) {
    return;
  }
...
- (void)updateProgress:(ABI20_0_0RCTLoadingProgress *)progress
{
  if (!progress) {
    return;
  }
