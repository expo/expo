...
#include "ABI20_0_0OpaqueJSString.h"
#endif
...
TEST(Value, Undefined) {
  ...
  ABI20_0_0EXPECT_EQ("undefined", s.str());
...
TEST(Value, FromJSON) {
  ...
  ABI20_0_0EXPECT_TRUE(v.isObject());
  JSC_JSGlobalContextRelease(ctx);
}
...
TEST(Value, ToJSONString) {
  ...
  ABI20_0_0EXPECT_TRUE(dyn.isObject());
  auto val = dyn.at("a");
  ASSERT_NE(nullptr, val);
  ASSERT_TRUE(val.isInt());
  ABI20_0_0EXPECT_EQ(4, val.getInt());
  ABI20_0_0EXPECT_EQ(4.0f, val.asDouble());
