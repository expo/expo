...
#import "ABI20_0_0RCTUITextView.h"

@implementation ABI20_0_0RCTTextView
{
  ABI20_0_0RCTUITextView *_textView;
  ABI20_0_0RCTText *_richTextView;
...
- (instancetype)initWithBridge:(ABI20_0_0RCTBridge *)bridge
{
  ...
  if (self = [super initWithBridge:bridge]) {
    ...
    _textView = [[ABI20_0_0RCTUITextView alloc] initWithFrame:self.bounds];
...
#pragma mark - ABI20_0_0RCTComponent

- (void)insertReactABI20_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI20_0_0Subview:subview atIndex:index];

  if ([subview isKindOfClass:[ABI20_0_0RCTText class]]) {
    if (_richTextView) {
      ABI20_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI20_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI20_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI20_0_0RCTReactABI20_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI20_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI20_0_0TagFromString(_pendingAttributedText);
...
- (BOOL)textView:(ABI20_0_0RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (!textView.textWasPasted) {
    ...
    if (_blurOnSubmit && [text isEqualToString:@"\n"]) {
      // TODO: the purpose of blurOnSubmit on ABI20_0_0RCTextField is to decide if the
      // field should lose focus when return is pressed or not. We're cheating a
      // bit here by using it on ABI20_0_0RCTextView to decide if return character should
...
- (void)textViewDidChangeSelection:(ABI20_0_0RCTUITextView *)textView
{
...
- (void)textViewDidChange:(UITextView *)textView
{
  ...
  if (!self.ReactABI20_0_0Tag || !_onChange) {
    return;
  }
