...
@implementation ABI20_0_0RCTAssetsLibraryRequestHandler
{
     ...
     withDelegate:(id<ABI20_0_0RCTURLRequestDelegate>)delegate
{
  ...
  [[self assetsLibrary] assetForURL:request.URL resultBlock:^(ALAsset *asset) {
    ...
    if (asset) {
      ...
      NSError *error = ABI20_0_0RCTErrorWithMessage(errorMessage);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
    }
...
@implementation ABI20_0_0RCTBridge (ABI20_0_0RCTAssetsLibraryImageLoader)

- (ALAssetsLibrary *)assetsLibrary
{
  return [[self moduleForClass:[ABI20_0_0RCTAssetsLibraryRequestHandler class]] assetsLibrary];
}
