...
NSString *const ABI29_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI29_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI29_0_0RCTAccessibilityManager ()
...
@implementation ABI29_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI29_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI29_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI29_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI29_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI29_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI29_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI29_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI29_0_0Tag:ReactABI29_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI29_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI29_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI29_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI29_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI29_0_0RCTResponseSenderBlock)callback
...
@implementation ABI29_0_0RCTBridge (ABI29_0_0RCTAccessibilityManager)

- (ABI29_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI29_0_0RCTAccessibilityManager class]];
}
