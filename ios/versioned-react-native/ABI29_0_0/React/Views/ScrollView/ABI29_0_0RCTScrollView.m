...
@interface ABI29_0_0RCTScrollEvent : NSObject <ABI29_0_0RCTEvent>
...
@implementation ABI29_0_0RCTScrollEvent
{
...
{
  ABI29_0_0RCTAssertParam(ReactABI29_0_0Tag);
...
- (ABI29_0_0RCTScrollEvent *)coalesceWithEvent:(ABI29_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI29_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI29_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI29_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI29_0_0RCTRefreshControl *rctRefreshControl;
...
@implementation ABI29_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI29_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI29_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI29_0_0RCTUIManager JSResponder] isKindOfClass:[ABI29_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI29_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI29_0_0RCTRefreshControl *)refreshControl
{
...
@interface ABI29_0_0RCTScrollView () <ABI29_0_0RCTUIManagerObserver>

@end

@implementation ABI29_0_0RCTScrollView
{
  ABI29_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI29_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI29_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI29_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI29_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI29_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI29_0_0LayoutDirection:layoutDirection];

  ABI29_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI29_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertReactABI29_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertReactABI29_0_0Subview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI29_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI29_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI29_0_0RCTAssert(_contentView == nil, @"ABI29_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI29_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ReactABI29_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI29_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI29_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI29_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI29_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  ABI29_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI29_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI29_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI29_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI29_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI29_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI29_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI29_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI29_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI29_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI29_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI29_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI29_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI29_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI29_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI29_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI29_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI29_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI29_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI29_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI29_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI29_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(ABI29_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(ABI29_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI29_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI29_0_0RCTScrollEvent *scrollEvent = [[ABI29_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI29_0_0RCTEventDispatcher (ABI29_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI29_0_0Tag
{
  ...
  ABI29_0_0RCTScrollEvent *fakeScrollEvent = [[ABI29_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI29_0_0Tag:ReactABI29_0_0Tag
