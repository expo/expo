...
#include <cxxReactABI29_0_0/ABI29_0_0MethodCall.h>
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI29_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI29_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI29_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
