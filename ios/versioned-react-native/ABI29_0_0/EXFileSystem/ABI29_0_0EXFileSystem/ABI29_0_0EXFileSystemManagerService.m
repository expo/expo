...
NSString * const ABI29_0_0EXShellManifestResourceName = @"shell-app-manifest";

@implementation ABI29_0_0EXFileSystemManagerService
...
- (NSArray<NSString *> *)bundledAssetsForExperienceId:(NSString *)experienceId
{
  ...
  dispatch_once(&once, ^{
    NSString *manifestBundlePath = [[NSBundle mainBundle] pathForResource:ABI29_0_0EXShellManifestResourceName ofType:@"json"];
    ...
    if (error) {
      ABI29_0_0EXLogError(@"Error parsing bundled manifest: %@", error);
      return;
    }
