...
typedef NS_OPTIONS(NSInteger, ABI29_0_0RCTTextSizeComparisonOptions) {
  ABI29_0_0RCTTextSizeComparisonSmaller     = 1 << 0,
  ABI29_0_0RCTTextSizeComparisonLarger      = 1 << 1,
  ABI29_0_0RCTTextSizeComparisonWithinRange = 1 << 2,
};
               ...
               maximumFontSize:(CGFloat)maximumFontSize
{
  ...
  while (true) {
    ...
    if (
        ...
        (comparsion & ABI29_0_0RCTTextSizeComparisonSmaller)
    ) {
      return;
    } else if (comparsion & ABI29_0_0RCTTextSizeComparisonSmaller) {
      bottomRatio = ratio;
...
- (ABI29_0_0RCTTextSizeComparisonOptions)ABI29_0_0compareToSize:(CGSize)size thresholdRatio:(CGFloat)thresholdRatio
{
  ...
  if (truncatedGlyphRange.location != NSNotFound) {
    return ABI29_0_0RCTTextSizeComparisonLarger;
  }
  ...
  ABI29_0_0RCTTextSizeComparisonOptions result = 0;

  result |= (fitsSize) ? ABI29_0_0RCTTextSizeComparisonSmaller : ABI29_0_0RCTTextSizeComparisonLarger;

  if (ABS(measuredSize.width - size.width) < thresholdSize.width) {
    result = result | ABI29_0_0RCTTextSizeComparisonWithinRange;
  }
  ...
  [self enumerateAttribute:NSFontAttributeName
    ...
    ^(UIFont *_Nullable font, NSRange range, BOOL *_Nonnull stop) {
      ...
      if (scaledFont) {
        ...
        ABI29_0_0RCTLogError(@"Font \"%@"" doesn't support automatic scaling.", font.familyName);
      }
    }
  ];
