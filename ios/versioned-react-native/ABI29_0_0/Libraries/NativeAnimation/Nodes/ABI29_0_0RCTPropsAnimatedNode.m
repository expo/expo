...
@implementation ABI29_0_0RCTPropsAnimatedNode
{
...
- (void)performUpdate
{
  ...
  for (NSNumber *parentTag in self.parentNodes.keyEnumerator) {
    ABI29_0_0RCTAnimatedNode *parentNode = [self.parentNodes objectForKey:parentTag];
    if ([parentNode isKindOfClass:[ABI29_0_0RCTStyleAnimatedNode class]]) {
      [self->_propsDictionary addEntriesFromDictionary:[(ABI29_0_0RCTStyleAnimatedNode *)parentNode propsDictionary]];
      
    } else if ([parentNode isKindOfClass:[ABI29_0_0RCTValueAnimatedNode class]]) {
      NSString *property = [self propertyNameForParentTag:parentTag];
      CGFloat value = [(ABI29_0_0RCTValueAnimatedNode *)parentNode value];
