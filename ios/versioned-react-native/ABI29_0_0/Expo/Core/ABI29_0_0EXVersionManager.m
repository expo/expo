...
#import "ABI29_0_0EXDisabledDevLoadingView.h"
#import "ABI29_0_0EXDisabledDevMenu.h"
#import "ABI29_0_0EXDisabledRedBox.h"
#import "ABI29_0_0EXFileSystem.h"
#import "ABI29_0_0EXVersionManager.h"
#import "ABI29_0_0EXScopedBridgeModule.h"
#import "ABI29_0_0EXStatusBarManager.h"
...
#import <ReactABI29_0_0/ABI29_0_0RCTExceptionsManager.h>
...
#import <ReactABI29_0_0/ABI29_0_0RCTAsyncLocalStorage.h>
...
#define ABI29_0_0EX_KERNEL_SERVICE_NONE @"ABI29_0_0EXKernelServiceNone"

// this is needed because ABI29_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI29_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSDictionary *> *ABI29_0_0EXScopedModuleClasses;
void ABI29_0_0EXRegisterScopedModule(Class, ...);
void ABI29_0_0EXRegisterScopedModule(Class moduleClass, ...)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI29_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  ...
  va_start(argumentList, moduleClass);
    while ((kernelServiceClassName = va_arg(argumentList, NSString*))) {
      if ([kernelServiceClassName isEqualToString:@"nil"]) {
        unversionedKernelServiceClassNames[kernelServiceClassName] = ABI29_0_0EX_KERNEL_SERVICE_NONE;
      } else {
  ...
  if (moduleClassName) {
    ABI29_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassNames;
  }
}

@interface ABI29_0_0EXVersionManager ()
...
@implementation ABI29_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI29_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI29_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI29_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
#ifdef ABI29_0_0EX_ENABLE_UNSAFE_SYSTRACE
    items[@"dev-profiler"] = @{
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI29_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return ([ABI29_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI29_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI29_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI29_0_0RCTSetFatalHandler(fatalHandler);
  ABI29_0_0RCTSetLogThreshold(threshold);
 ...
 *    @ABI29_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI29_0_0EXKernel *kernel
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSString *localStorageDirectory = [[ABI29_0_0EXFileSystem documentDirectoryForExperienceId:experienceId] stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];

  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI29_0_0EXAppState alloc] init],
                                    [[ABI29_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:isDeveloper],
                                    [[ABI29_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI29_0_0EXStatusBarManager alloc] init],
                                    [[ABI29_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory],
                                    ]];
  ...
  if (exceptionsManagerDelegate) {
    ABI29_0_0RCTExceptionsManager *exceptionsManager = [[ABI29_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
    [extraModules addObject:exceptionsManager];
  } else {
    ABI29_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
  }
  
  if (params[@"testEnvironment"]) {
    ABI29_0_0EXTestEnvironment testEnvironment = (ABI29_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI29_0_0EXTestEnvironmentNone) {
      ABI29_0_0EXTest *testModule = [[ABI29_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI29_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI29_0_0RCTDevMenu
    [extraModules addObject:[[ABI29_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
  ...
  id<ABI29_0_0EXModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

  ABI29_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI29_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];

  NSArray<id<ABI29_0_0RCTBridgeModule>> *expoModules = [moduleRegistryAdapter extraModulesForParams:params andExperience:experienceId withScopedModulesArray:extraModules withKernelServices:services];
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI29_0_0EXScopedModuleClasses) {
    [ABI29_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI29_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
