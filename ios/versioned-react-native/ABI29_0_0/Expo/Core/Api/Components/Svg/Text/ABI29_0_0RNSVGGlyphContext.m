...
@interface ABI29_0_0RNSVGGlyphContext () {
@public
    ...
    double mFontSize_;
    ABI29_0_0RNSVGFontData *topFont_;
...
@implementation ABI29_0_0RNSVGGlyphContext
...
- (CTFontRef)getGlyphFont
{
    ...
    NSString * fontWeight = [ABI29_0_0RNSVGFontWeightToString(topFont_->fontWeight) lowercaseString];
    NSString * fontStyle = ABI29_0_0RNSVGFontStyleStrings[topFont_->fontStyle];
    ...
    return (__bridge CTFontRef)[ABI29_0_0RCTFont updateFont:nil
...
void ABI29_0_0pushIndices(ABI29_0_0RNSVGGlyphContext *self) {
...
- (instancetype)initWithScale:(float)scale_
    ...
    self->mFontSize_ = ABI29_0_0RNSVGFontData_DEFAULT_FONT_SIZE;
    self->topFont_ = [ABI29_0_0RNSVGFontData Defaults];
...
ABI29_0_0RNSVGFontData *ABI29_0_0getTopOrParentFont(ABI29_0_0RNSVGGlyphContext *self, ABI29_0_0RNSVGGroup* child) {
    if (self->mTop_ > 0) {
        return self->topFont_;
    } else {
        ABI29_0_0RNSVGGroup* parentRoot = [child getParentTextRoot];
        ABI29_0_0RNSVGFontData* Defaults = [ABI29_0_0RNSVGFontData Defaults];
        while (parentRoot != nil) {
            ABI29_0_0RNSVGFontData *map = [[parentRoot getGlyphContext] getFont];
            if (map != Defaults) {
                return map;
            }
...
void ABI29_0_0pushNodeAndFont(ABI29_0_0RNSVGGlyphContext *self, ABI29_0_0RNSVGGroup* node, NSDictionary* font) {
    ABI29_0_0RNSVGFontData *parent = ABI29_0_0getTopOrParentFont(self, node);
    self->mTop_++;
    if (font == nil) {
        [self->mFontContext_ addObject:parent];
        return;
    }
    ABI29_0_0RNSVGFontData *data = [ABI29_0_0RNSVGFontData initWithNSDictionary:font
                                                 parent:parent
                                                  scale:self->mScale_];
...
- (void)pushContext:(ABI29_0_0RNSVGGroup*)node
               font:(NSDictionary*)font {
    ABI29_0_0pushNodeAndFont(self, node, font);
    ABI29_0_0pushIndices(self);
}
...
- (void)pushContext:(ABI29_0_0RNSVGText*)node
             ...
             deltaY:(NSArray*)deltaY
             rotate:(NSArray*)rotate {
    ABI29_0_0pushNodeAndFont(self, (ABI29_0_0RNSVGGroup*)node, font);
    if (x != nil && [x count] != 0) {
        mXsIndex_++;
        mXIndex_ = -1;
...
void ABI29_0_0incrementIndices(NSMutableArray *indices, long topIndex) {
...
- (double)nextXWithDouble:(double)advance {
    ABI29_0_0incrementIndices(mXIndices_, mXsIndex_);
    long nextIndex = mXIndex_ + 1;
    if (nextIndex < [mXs_ count]) {
        mDX_ = 0;
        mXIndex_ = nextIndex;
        NSString *string = [mXs_ objectAtIndex:nextIndex];
        mX_ = [ABI29_0_0RNSVGPropHelper fromRelativeWithNSString:string
                                            relative:mWidth_
                                              offset:0
                                               scale:mScale_
                                            fontSize:mFontSize_];
...
- (double)nextY {
    ABI29_0_0incrementIndices(mYIndices_, mYsIndex_);
    long nextIndex = mYIndex_ + 1;
    if (nextIndex < [mYs_ count]) {
        mDY_ = 0;
        mYIndex_ = nextIndex;
        NSString *string = [mYs_ objectAtIndex:nextIndex];
        mY_ = [ABI29_0_0RNSVGPropHelper fromRelativeWithNSString:string
                                            relative:mHeight_
                                              offset:0
                                               scale:mScale_
                                            fontSize:mFontSize_];
...
- (double)nextDeltaX {
    ABI29_0_0incrementIndices(mDXIndices_, mDXsIndex_);
    long nextIndex = mDXIndex_ + 1;
    if (nextIndex < [mDXs_ count]) {
        mDXIndex_ = nextIndex;
        NSString *string = [mDXs_ objectAtIndex:nextIndex];
        double val = [ABI29_0_0RNSVGPropHelper fromRelativeWithNSString:string
                                                   relative:mWidth_
                                                     offset:0
                                                      scale:mScale_
                                                   fontSize:mFontSize_];
...
- (double)nextDeltaY {
    ABI29_0_0incrementIndices(mDYIndices_, mDYsIndex_);
...
- (NSNumber*)nextRotation {
    ABI29_0_0incrementIndices(mRIndices_, mRsIndex_);
