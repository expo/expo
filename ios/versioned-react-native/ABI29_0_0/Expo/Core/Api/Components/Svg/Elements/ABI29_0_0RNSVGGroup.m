...
@implementation ABI29_0_0RNSVGGroup
{
    ABI29_0_0RNSVGGlyphContext *_glyphContext;
}
...
- (void)renderGroupTo:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI29_0_0RNSVGNode *node) {
        if (node.responsible && !svg.responsible) {
            svg.responsible = YES;
        }

        if ([node isKindOfClass:[ABI29_0_0RNSVGRenderable class]]) {
            [(ABI29_0_0RNSVGRenderable*)node mergeProperties:self];
        }

        [node renderTo:context];

        if ([node isKindOfClass:[ABI29_0_0RNSVGRenderable class]]) {
            [(ABI29_0_0RNSVGRenderable*)node resetProperties];
        }
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI29_0_0RNSVGGlyphContext alloc] initWithScale:1 width:width
                                                   height:height];
}
...
- (void)parseReference
{
    ...
    [self traverseSubviews:^(__kindof ABI29_0_0RNSVGNode *node) {
        [node parseReference];
        return YES;
    }];
}
