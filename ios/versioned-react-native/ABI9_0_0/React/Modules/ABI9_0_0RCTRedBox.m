...
@class ABI9_0_0RCTRedBoxWindow;

@protocol ABI9_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI9_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI9_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI9_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI9_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI9_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI9_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI9_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI9_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI9_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI9_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI9_0_0RCTJSStackFrame *)stackFrame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI9_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI9_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI9_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI9_0_0RCTKeyCommands for this, but since
...
@interface ABI9_0_0RCTRedBox () <ABI9_0_0RCTInvalidating, ABI9_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI9_0_0RCTRedBox
{
  ABI9_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI9_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (void)registerErrorCustomizer:(id<ABI9_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI9_0_0RCTErrorInfo *)_customizeError:(ABI9_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI9_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI9_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI9_0_0RCTJSStackFrame *> *stack = [ABI9_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  [self _showErrorMessage:message withStack:[ABI9_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:isUpdate];
}

- (void)_showErrorMessage:(NSString *)message withStack:(NSArray<ABI9_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI9_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI9_0_0RCTErrorInfo *errorInfo = [[ABI9_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI9_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI9_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI9_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI9_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI9_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI9_0_0RCTRedBoxWindow *)redBoxWindow {
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI9_0_0RCTReloadNotification object:_bridge.baseBridge userInfo:nil];
}

@end

@implementation ABI9_0_0RCTBridge (ABI9_0_0RCTRedBox)

- (ABI9_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI9_0_0RCTRedBox class]];
}
...
- (ABI9_0_0RCTRedBox *)redBox { return nil; }
