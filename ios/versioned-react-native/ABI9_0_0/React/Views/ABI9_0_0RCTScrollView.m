...
#import "ABI9_0_0RCTRefreshControl.h"
...
@interface ABI9_0_0RCTScrollEvent : NSObject <ABI9_0_0RCTEvent>
...
@implementation ABI9_0_0RCTScrollEvent
{
...
{
  ABI9_0_0RCTAssertParam(ReactABI9_0_0Tag);

  if ((self = [super init])) {
    _eventName = [eventName copy];
    _viewTag = ReactABI9_0_0Tag;
...
- (ABI9_0_0RCTScrollEvent *)coalesceWithEvent:(ABI9_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI9_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI9_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI9_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, copy) NSIndexSet *stickyHeaderIndices;
@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) ABI9_0_0RCTRefreshControl *rctRefreshControl;

@end


@implementation ABI9_0_0RCTCustomScrollView
{
...
- (UIView *)contentView
{
  return ((ABI9_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI9_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI9_0_0RCTUIManager JSResponder] isKindOfClass:[ABI9_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI9_0_0 scroll
...
- (void)dockClosestSectionHeader
{
  ...
  NSUInteger subviewCount = contentView.ReactABI9_0_0Subviews.count;
  [_stickyHeaderIndices enumerateIndexesWithOptions:0 usingBlock:
   ^(NSUInteger idx, BOOL *stop) {
    ...
    UIView *header = contentView.ReactABI9_0_0Subviews[idx];
...
- (void)setRctRefreshControl:(ABI9_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI9_0_0RCTScrollView
{
  ABI9_0_0RCTEventDispatcher *_eventDispatcher;
  ABI9_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
{
  ABI9_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;
    _scrollView = [[ABI9_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
- (void)insertReactABI9_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertReactABI9_0_0Subview:view atIndex:atIndex];
  if ([view isKindOfClass:[ABI9_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI9_0_0RCTRefreshControl *)view];
  } else {
    ABI9_0_0RCTAssert(_contentView == nil, @"ABI9_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
...
- (void)removeReactABI9_0_0Subview:(UIView *)subview
{
  ...
  if ([subview isKindOfClass:[ABI9_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else {
    ABI9_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI9_0_0Subview:atIndex:`
}
...
{
  ABI9_0_0RCTAssert(_scrollView.contentSize.width <= self.frame.size.width,
...
- (void)layoutSubviews
{
  ...
  ABI9_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI9_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI9_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI9_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI9_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI9_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI9_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI9_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI9_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI9_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI9_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI9_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI9_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI9_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI9_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI9_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI9_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI9_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI9_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI9_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI9_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI9_0_0RCTView contentInsetsForView:self];
...
- (void)ReactABI9_0_0BridgeDidFinishTransaction
{
  ...
  if (ABI9_0_0RCT_DEBUG) {
    ...
    NSUInteger subviewCount = _scrollView.contentView.ReactABI9_0_0Subviews.count;
    ...
    if (lastIndex != NSNotFound && lastIndex >= subviewCount) {
      ABI9_0_0RCTLogWarn(@"Sticky header index %zd was outside the range {0, %zd}",
...
#define ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI9_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI9_0_0RCTScrollEvent *scrollEvent = [[ABI9_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI9_0_0RCTEventDispatcher (ABI9_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI9_0_0Tag
{
  ...
  ABI9_0_0RCTScrollEvent *fakeScrollEvent = [[ABI9_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI9_0_0Tag:ReactABI9_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
