...
#import "ABI9_0_0RCTBorderDrawing.h"
#import "ABI9_0_0RCTConvert.h"
#import "ABI9_0_0RCTLog.h"
#import "ABI9_0_0RCTUtils.h"
#import "UIView+ReactABI9_0_0.h"

@implementation UIView (ABI9_0_0RCTViewUnmounting)

- (void)ReactABI9_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI9_0_0_remountAllSubviews];
  }
}

- (void)ReactABI9_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI9_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI9_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI9_0_0RootView]) {
      break;
    }
...
static NSString *ABI9_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI9_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI9_0_0RCTView
{
  UIColor *_backgroundColor;
}

@synthesize ReactABI9_0_0ZIndex = _ReactABI9_0_0ZIndex;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI9_0_0RCTBorderStyleSolid;
...
ABI9_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI9_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI9_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI9_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI9_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI9_0_0RCTPointerEventsNone || _pointerEvents == ABI9_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI9_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI9_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI9_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI9_0_0Tag: %@;", self.ReactABI9_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI9_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI9_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.sortedReactABI9_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI9_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI9_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI9_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI9_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.sortedReactABI9_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI9_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI9_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI9_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI9_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI9_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI9_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI9_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI9_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI9_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI9_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI9_0_0RCTCornerRadii){
...
{
  return (ABI9_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI9_0_0RCTUpdateShadowPathForView(self);

  const ABI9_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI9_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI9_0_0RCTRunningInTestEnvironment() &&
  ABI9_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI9_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI9_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI9_0_0RCTBorderStyleSolid &&

  // iOS draws borders in front of the content whereas ABI9_0_0CSS draws them behind
  // the content. For this reason, only use iOS border drawing when clipping
  // or when the border is hidden.

  (borderInsets.top == 0 || CGColorGetAlpha(borderColors.top) == 0 || self.clipsToBounds);

  // iOS clips to the outside of the border, but ABI9_0_0CSS clips to the inside. To
  ...
  UIImage *image = ABI9_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI9_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI9_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI9_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI9_0_0RCTUpdateShadowPathForView(ABI9_0_0RCTView *view)
{
  if (ABI9_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI9_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI9_0_0RCTCornerInsets cornerInsets = ABI9_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI9_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI9_0_0RCTLogWarn(@"View #%@ of type %@ has a shadow set but cannot calculate "
                 "shadow efficiently. Consider setting a background color to "
                 "fix this, or apply the shadow to a more specific component.",
                 view.ReactABI9_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI9_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI9_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI9_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI9_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI9_0_0RCTBorderStyle)style \
  {                                                    \
