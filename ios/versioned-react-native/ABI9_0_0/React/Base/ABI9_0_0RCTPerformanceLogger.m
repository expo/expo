...
@interface ABI9_0_0RCTPerformanceLogger ()
{
  int64_t _data[ABI9_0_0RCTPLSize][2];
  NSUInteger _cookies[ABI9_0_0RCTPLSize];
}
...
@implementation ABI9_0_0RCTPerformanceLogger
...
- (void)markStartForTag:(ABI9_0_0RCTPLTag)tag
{
  if (ABI9_0_0RCTProfileIsProfiling()) {
    ...
    _cookies[tag] = ABI9_0_0RCTProfileBeginAsyncEvent(ABI9_0_0RCTProfileTagAlways, label, nil);
  }
...
- (void)markStopForTag:(ABI9_0_0RCTPLTag)tag
{
  if (ABI9_0_0RCTProfileIsProfiling()) {
    NSString *label =_labelsForTags[tag];
    ABI9_0_0RCTProfileEndAsyncEvent(ABI9_0_0RCTProfileTagAlways, @"native", _cookies[tag], label, @"ABI9_0_0RCTPerformanceLogger", nil);
  }
  if (_data[tag][0] != 0 && _data[tag][1] == 0) {
    _data[tag][1] = CACurrentMediaTime() * 1000;
  } else {
    ABI9_0_0RCTLogInfo(@"Unbalanced calls start/end for tag %li", (unsigned long)tag);
  }
}
...
- (NSArray<NSNumber *> *)valuesForTags
{
  ...
  for (NSUInteger index = 0; index < ABI9_0_0RCTPLSize; index++) {
