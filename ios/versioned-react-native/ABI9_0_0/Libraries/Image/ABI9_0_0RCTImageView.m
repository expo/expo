...
#import "ABI9_0_0RCTImageView.h"

#import "ABI9_0_0RCTBridge.h"
#import "ABI9_0_0RCTConvert.h"
#import "ABI9_0_0RCTEventDispatcher.h"
#import "ABI9_0_0RCTImageLoader.h"
#import "ABI9_0_0RCTImageSource.h"
#import "ABI9_0_0RCTImageUtils.h"
#import "ABI9_0_0RCTUtils.h"
#import "ABI9_0_0RCTImageBlurUtils.h"
 ...
 * at `idealSize`.
 */
static BOOL ABI9_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
@interface ABI9_0_0RCTImageView ()

@property (nonatomic, copy) ABI9_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI9_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI9_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI9_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI9_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI9_0_0RCTImageView
{
   ...
   * if any.
   */
  ABI9_0_0RCTImageLoaderCancellationBlock _reloadImageCancellationBlock;
}
...
- (void)updateImage
{
  ...
  if (_resizeMode == ABI9_0_0RCTResizeModeRepeat) {
...
- (void)setSource:(ABI9_0_0RCTImageSource *)source
{
...
- (void)setResizeMode:(ABI9_0_0RCTResizeMode)resizeMode
{
...
{
  ABI9_0_0RCTImageLoaderCancellationBlock previousCancellationBlock = _reloadImageCancellationBlock;
...
- (void)reloadImage
{
  ...
  if (_source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI9_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (_onProgress) {
    ...
    CGFloat imageScale = ABI9_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    ABI9_0_0RCTImageSource *source = _source;
    CGFloat blurRadius = _blurRadius;
    __weak ABI9_0_0RCTImageView *weakSelf = self;
    ...
    [_bridge.imageLoader loadImageWithURLRequest:_source.request
      ...
      ABI9_0_0RCTImageView *strongSelf = weakSelf;
      void (^setImageBlock)(UIImage *) = ^(UIImage *image) {
        if (![source isEqual:strongSelf.source]) {
          // Bail out if source has changed since we started loading
          return;
        }
        if (image.ReactABI9_0_0KeyframeAnimation) {
          [strongSelf.layer addAnimation:image.ReactABI9_0_0KeyframeAnimation forKey:@"contents"];
        } else {
      ...
      if (blurRadius > __FLT_EPSILON__) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          UIImage *image = ABI9_0_0RCTBlurredImageWithRadius(loadedImage, blurRadius);
...
{
  [super ReactABI9_0_0SetFrame:frame];
  ...
  if (!self.image || self.image == _defaultImage) {
    ...
    CGSize idealSize = ABI9_0_0RCTTargetSize(imageSize, self.image.scale, frame.size,
                                     ABI9_0_0RCTScreenScale(), (ABI9_0_0RCTResizeMode)self.contentMode, YES);

    if (ABI9_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize)) {
      if (ABI9_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
        ABI9_0_0RCTLogInfo(@"[PERF IMAGEVIEW] Reloading image %@ as size %@", _source.request.URL.absoluteString, NSStringFromCGSize(idealSize));
