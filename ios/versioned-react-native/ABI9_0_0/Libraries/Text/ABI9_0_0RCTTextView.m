...
@interface ABI9_0_0RCTUITextView : UITextView

@property (nonatomic, assign) BOOL textWasPasted;

@end

@implementation ABI9_0_0RCTUITextView
{
...
- (void)didMoveToWindow
{
  if (_jsRequestingFirstResponder) {
    ...
    [self ReactABI9_0_0DidMakeFirstResponder];
  }
}

@end

@implementation ABI9_0_0RCTTextView
{
...
- (instancetype)initWithEventDispatcher:(ABI9_0_0RCTEventDispatcher *)eventDispatcher
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _textView = [[ABI9_0_0RCTUITextView alloc] initWithFrame:CGRectZero];
...
- (void)insertReactABI9_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI9_0_0Subview:subview atIndex:index];
  if ([subview isKindOfClass:[ABI9_0_0RCTText class]]) {
    if (_richTextView) {
      ABI9_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI9_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI9_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI9_0_0RCTReactABI9_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI9_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI9_0_0TagFromString(_pendingAttributedText);
...
- (void)updateFrames
{
  ...
  // ABI9_0_0RCTTextField defaults, using the system defaults of font size 17 and a
  // height of 31 points.
  //
...
- (BOOL)textView:(ABI9_0_0RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (textView.textWasPasted) {
    ...
    if (_blurOnSubmit && [text isEqualToString:@"\n"]) {
      // TODO: the purpose of blurOnSubmit on ABI9_0_0RCTextField is to decide if the
      // field should lose focus when return is pressed or not. We're cheating a
      // bit here by using it on ABI9_0_0RCTextView to decide if return character should
...
- (void)textViewDidChangeSelection:(ABI9_0_0RCTUITextView *)textView
{
...
- (void)textViewDidChange:(UITextView *)textView
{
  ...
  if (!self.ReactABI9_0_0Tag || !_onChange) {
    return;
  }
...
- (void)ReactABI9_0_0WillMakeFirstResponder
{
  [_textView ReactABI9_0_0WillMakeFirstResponder];
}
...
- (void)ReactABI9_0_0DidMakeFirstResponder
{
  [_textView ReactABI9_0_0DidMakeFirstResponder];
}
