...
NSString *const ABI9_0_0RCTShadowViewAttributeName = @"ABI9_0_0RCTShadowViewAttributeName";
NSString *const ABI9_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI9_0_0RCTReactABI9_0_0TagAttributeName = @"ReactABI9_0_0TagAttributeName";

@implementation ABI9_0_0RCTShadowText
{
...
static ABI9_0_0CSSSize ABI9_0_0RCTMeasure(void *context, float width, ABI9_0_0CSSMeasureMode widthMode, float height, ABI9_0_0CSSMeasureMode heightMode)
{
  ABI9_0_0RCTShadowText *shadowText = (__bridge ABI9_0_0RCTShadowText *)context;
  ...
  ABI9_0_0CSSSize result;
  result.width = ABI9_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI9_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI9_0_0CSSNodeSetMeasureFunc(self.cssNode, ABI9_0_0RCTMeasure);
...
{
  if ([[self ReactABI9_0_0Superview] isKindOfClass:[ABI9_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI9_0_0Superview] ReactABI9_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:width widthMode:ABI9_0_0CSSMeasureModeExactly];
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI9_0_0RCTText *view = (ABI9_0_0RCTText *)viewRegistry[self.ReactABI9_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI9_0_0RCTShadowText
     * to the ABI9_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI9_0_0RCTTextView
     * and its contaned ABI9_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI9_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  NSTextStorage *textStorage = [self buildTextStorageForWidth:self.frame.size.width widthMode:ABI9_0_0CSSMeasureModeExactly];
  NSLayoutManager *layoutManager = textStorage.layoutManagers.firstObject;
  NSTextContainer *textContainer = layoutManager.textContainers.firstObject;
  NSRange glyphRange = [layoutManager glyphRangeForTextContainer:textContainer];
  NSRange characterRange = [layoutManager characterRangeForGlyphRange:glyphRange actualGlyphRange:NULL];
  [layoutManager.textStorage enumerateAttribute:ABI9_0_0RCTShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI9_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI9_0_0CSSNodeRef childNode = child.cssNode;
      float width = ABI9_0_0CSSNodeStyleGetWidth(childNode);
      float height = ABI9_0_0CSSNodeStyleGetHeight(childNode);
      if (isUndefined(width) || isUndefined(height)) {
        ABI9_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI9_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI9_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI9_0_0RCTRoundPixelValue(width),
        ABI9_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI9_0_0CSSMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){widthMode == ABI9_0_0CSSMeasureModeUndefined ? CGFLOAT_MAX : width, CGFLOAT_MAX};
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI9_0_0RCTConvert UIFont:nil withFamily:fontFamily
  ...
  for (ABI9_0_0RCTShadowView *child in [self ReactABI9_0_0Subviews]) {
    if ([child isKindOfClass:[ABI9_0_0RCTShadowText class]]) {
      ABI9_0_0RCTShadowText *shadowText = (ABI9_0_0RCTShadowText *)child;
      ...
      ABI9_0_0RCTShadowRawText *shadowRawText = (ABI9_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI9_0_0CSSNodeStyleGetWidth(child.cssNode);
      float height = ABI9_0_0CSSNodeStyleGetHeight(child.cssNode);
      if (isUndefined(width) || isUndefined(height)) {
        ABI9_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      [attachmentString addAttribute:ABI9_0_0RCTShadowViewAttributeName value:child range:(NSRange){0, attachmentString.length}];
      [attributedString appendAttributedString:attachmentString];
      if (height > heightOfTallestSubview) {
        heightOfTallestSubview = height;
      }
      // Don't call setTextComputed on this child. ABI9_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI9_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI9_0_0RCTReactABI9_0_0TagAttributeName withValue:self.ReactABI9_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  if (newTextAlign == NSTextAlignmentRight || newTextAlign == NSTextAlignmentLeft) {
    ABI9_0_0RCTShadowView *view = self;
    while (view != nil && ABI9_0_0CSSNodeStyleGetDirection(view.cssNode) == ABI9_0_0CSSDirectionInherit) {
      view = [view ReactABI9_0_0Superview];
    }
    if (view != nil && ABI9_0_0CSSNodeStyleGetDirection(view.cssNode) == ABI9_0_0CSSDirectionRTL) {
  ...
  // Text decoration
  if (_textDecorationLine == ABI9_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI9_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI9_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI9_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
#define ABI9_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI9_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI9_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI9_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI9_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI9_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI9_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI9_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI9_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI9_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI9_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI9_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI9_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI9_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI9_0_0RCTTextDecorationLineType);
ABI9_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI9_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI9_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI9_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI9_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI9_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI9_0_0RCTShadowView *child in [self ReactABI9_0_0Subviews]) {
    if ([child isKindOfClass:[ABI9_0_0RCTShadowText class]]) {
      ((ABI9_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  if (_fontSizeMultiplier == 0) {
    ABI9_0_0RCTLogError(@"fontSizeMultiplier value must be > zero.");
    _fontSizeMultiplier = 1.0;
  }
  for (ABI9_0_0RCTShadowView *child in [self ReactABI9_0_0Subviews]) {
    if ([child isKindOfClass:[ABI9_0_0RCTShadowText class]]) {
      ((ABI9_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
