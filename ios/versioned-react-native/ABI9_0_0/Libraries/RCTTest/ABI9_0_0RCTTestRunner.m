...
#import "ABI9_0_0RCTTestRunner.h"
...
@implementation ABI9_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI9_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI9_0_0RCTAssertParam(app);
  ABI9_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI9_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI9_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
configurationBlock:(void(^)(ABI9_0_0RCTRootView *rootView))configurationBlock
{
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI9_0_0RCTSetLogFunction(^(ABI9_0_0RCTLogLevel level, ABI9_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI9_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI9_0_0RCTBridge *bridge = [[ABI9_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI9_0_0RCTRootView *rootView = [[ABI9_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
    rootView.frame = CGRectMake(0, 0, 320, 2000); // Constant size for testing on multiple devices

    ABI9_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI9_0_0RCTTestModule class]];
    ABI9_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI9_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI9_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI9_0_0RCTSetLogFunction(ABI9_0_0RCTDefaultLogFunction);
    ...
    ABI9_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI9_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI9_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI9_0_0RCTAssert(testModule.status != ABI9_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI9_0_0RCTAssert(testModule.status == ABI9_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI9_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
