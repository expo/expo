...
#import "ABI9_0_0RCTImagePickerManager.h"
...
@interface ABI9_0_0RCTImagePickerManager ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI9_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI9_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI9_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
}

ABI9_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI9_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI9_0_0RCTResponseSenderBlock)callback)
{
...
ABI9_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI9_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI9_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI9_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI9_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  if ([ABI9_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI9_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI9_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI9_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI9_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI9_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI9_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    [self _dismissPicker:picker args:@[imageURL.absoluteString, ABI9_0_0RCTNullIfNil(height), ABI9_0_0RCTNullIfNil(width)]];
    return;
  }
        ...
        cancelCallback:(ABI9_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI9_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI9_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI9_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
