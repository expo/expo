...
#import "ABI23_0_0EXDownloadDelegate.h"
...
typedef NS_OPTIONS(unsigned int, ABI23_0_0EXFileSystemPermissionFlags) {
  ABI23_0_0EXFileSystemPermissionNone = 0,
  ABI23_0_0EXFileSystemPermissionRead = 1 << 1,
  ABI23_0_0EXFileSystemPermissionWrite = 1 << 2,
};

NSString * const ABI23_0_0EXDownloadProgressEventName = @"Exponent.downloadProgress";

@interface ABI23_0_0EXDownloadResumable : NSObject

@property (nonatomic, strong) NSString *uuid;
@property (nonatomic, strong) NSURLSession *session;
@property (nonatomic, strong) ABI23_0_0EXDownloadDelegate *delegate;

@end

@implementation ABI23_0_0EXDownloadResumable

- (instancetype)initWithId:(NSString *)uuid
               withSession:(NSURLSession *)session
              withDelegate:(ABI23_0_0EXDownloadDelegate *)delegate;
  {
    if ((self = [super init])) {
      _uuid = uuid;
      _session = session;
...
@interface ABI23_0_0EXFileSystem ()

@property (nonatomic, strong) NSMutableDictionary<NSString *, ABI23_0_0EXDownloadResumable*> *downloadObjects;

@end

ABI23_0_0EX_DEFINE_SCOPED_MODULE_GETTER(ABI23_0_0EXFileSystem, fileSystem)

@implementation NSData (ABI23_0_0EXFileSystem)
...
@implementation ABI23_0_0EXFileSystem

ABI23_0_0EX_EXPORT_SCOPED_MODULE(ExponentFileSystem, nil);
...
- (instancetype)initWithExperienceId:(NSString *)experienceId kernelServiceDelegate:(id)kernelServiceInstance params:(NSDictionary *)params
{
  if (self = [super initWithExperienceId:experienceId kernelServiceDelegate:kernelServiceInstance params:params]) {
    ...
    [ABI23_0_0EXFileSystem ensureDirExistsWithPath:_documentDirectory];
    [ABI23_0_0EXFileSystem ensureDirExistsWithPath:_cachesDirectory];
  }
  return self;
}
...
{
  return @[ABI23_0_0EXDownloadProgressEventName];
}

ABI23_0_0RCT_REMAP_METHOD(getInfoAsync,
...
{
  if (!([self _permissionsForURI:uri] & ABI23_0_0EXFileSystemPermissionRead)) {
  ...
  if ([uri.scheme isEqualToString:@"file"]) {
    [ABI23_0_0EXFileSystemLocalFileHandler getInfoForFile:uri withOptions:options resolver:resolve rejecter:reject];
  } else if ([uri.scheme isEqualToString:@"assets-library"]) {
    [ABI23_0_0EXFileSystemAssetLibraryHandler getInfoForFile:uri withOptions:options resolver:resolve rejecter:reject];
  } else {
...
ABI23_0_0RCT_REMAP_METHOD(readAsStringAsync,
...
ABI23_0_0RCT_REMAP_METHOD(writeAsStringAsync,
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if (!([self _permissionsForURI:uri] & ABI23_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI23_0_0RCT_REMAP_METHOD(deleteAsync,
                 deleteAsyncWithURI:(NSURL *)uri
                 withOptions:(NSDictionary *)options
                 resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if (!([self _permissionsForURI:[uri URLByAppendingPathComponent:@".."]] & ABI23_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI23_0_0RCT_REMAP_METHOD(moveAsync,
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!([self _permissionsForURI:[from URLByAppendingPathComponent:@".."]] & ABI23_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  if (!([self _permissionsForURI:to] & ABI23_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI23_0_0RCT_REMAP_METHOD(copyAsync,
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!([self _permissionsForURI:from] & ABI23_0_0EXFileSystemPermissionRead)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  if ([from.scheme isEqualToString:@"file"]) {
    [ABI23_0_0EXFileSystemLocalFileHandler copyFrom:from to:to resolver:resolve rejecter:reject];
  } else if ([from.scheme isEqualToString:@"assets-library"]) {
    [ABI23_0_0EXFileSystemAssetLibraryHandler copyFrom:from to:to resolver:resolve rejecter:reject];
  } else {
    reject(@"E_FILESYSTEM_INVALID_URI",
           [NSString stringWithFormat:@"Unsupported URI scheme for '%@'", from],
           nil);
...
ABI23_0_0RCT_REMAP_METHOD(makeDirectoryAsync,
...
ABI23_0_0RCT_REMAP_METHOD(readDirectoryAsync,
...
ABI23_0_0RCT_REMAP_METHOD(downloadAsync,
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if (!([self _permissionsForURI:localUri] & ABI23_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI23_0_0RCT_REMAP_METHOD(downloadResumableStartAsync,
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!([self _permissionsForPath:path] & ABI23_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  NSData *resumeData = data ? [ABI23_0_0RCTConvert NSData:data]:nil;
...
ABI23_0_0RCT_REMAP_METHOD(downloadResumablePauseAsync,
                 downloadResumablePauseAsyncWithUUID:(NSString *)uuid
                 resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ABI23_0_0EXDownloadResumable *downloadResumable = (ABI23_0_0EXDownloadResumable *)self.downloadObjects[uuid];
...
- (void)_downloadResumableCreateSessionWithUrl:(NSURL *)url withScopedPath:(NSString *)scopedPath withUUID:(NSString *)uuid withOptions:(NSDictionary *)options withResumeData:(NSData * _Nullable)resumeData withResolver:(ABI23_0_0RCTPromiseResolveBlock)resolve withRejecter:(ABI23_0_0RCTPromiseRejectBlock)reject
{
  NSURLSessionConfiguration *sessionConfiguration = [NSURLSessionConfiguration defaultSessionConfiguration];
  sessionConfiguration.requestCachePolicy = NSURLRequestReloadIgnoringLocalCacheData;
  sessionConfiguration.URLCache = nil;
  ...
  ABI23_0_0EXDownloadDelegate *downloadDelegate = [[ABI23_0_0EXDownloadDelegate alloc] initWithId:uuid
                                                                        onWrite:^(NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
    if(bytesWritten > 0)
      [weakSelf sendEventWithName:ABI23_0_0EXDownloadProgressEventName
                             body:@{@"uuid":uuid,
                                    @"data":@{
    ...
    if (!data) {
      reject(@"E_UNABLE_TO_SAVE",
             nil,
             ABI23_0_0RCTErrorWithMessage(@"Unable to save file to local URI"));
      return;
    }
    NSMutableDictionary *result = [NSMutableDictionary dictionary];
  ...
  ABI23_0_0EXDownloadResumable *downloadResumable = [[ABI23_0_0EXDownloadResumable alloc] initWithId:uuid
                                                                       withSession:session
...
- (ABI23_0_0EXFileSystemPermissionFlags)_permissionsForURI:(NSURL *)uri
{
  if ([uri.scheme isEqualToString:@"assets-library"]) {
    return ABI23_0_0EXFileSystemPermissionRead;
  }
  if ([uri.scheme isEqualToString:@"file"]) {
    return [self _permissionsForPath:uri.path];
  }
  return ABI23_0_0EXFileSystemPermissionNone;
}

- (ABI23_0_0EXFileSystemPermissionFlags)_permissionsForPath:(NSString *)path
{
  path = [path stringByStandardizingPath];
  if ([path hasPrefix:[_documentDirectory stringByAppendingString:@"/"]]) {
    return ABI23_0_0EXFileSystemPermissionRead | ABI23_0_0EXFileSystemPermissionWrite;
  }
...
{
  NSString *subdir = [ABI23_0_0EXUtil escapedResourceName:experienceId];
