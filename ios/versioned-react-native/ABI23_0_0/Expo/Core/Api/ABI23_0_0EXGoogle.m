...
NSString *ABI23_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI23_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI23_0_0EXGoogle
{
  ABI23_0_0RCTPromiseResolveBlock _logInResolve;
  ABI23_0_0RCTPromiseRejectBlock _logInReject;
}

ABI23_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI23_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI23_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI23_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      if (authState.isAuthorized) {
        _logInResolve(@{
          @"type": @"success",
          @"accessToken": ABI23_0_0RCTNullIfNil(authState.lastTokenResponse.accessToken),
          @"idToken": ABI23_0_0RCTNullIfNil(authState.lastTokenResponse.idToken),
          @"refreshToken": ABI23_0_0RCTNullIfNil(authState.lastTokenResponse.refreshToken),
          @"serverAuthCode": ABI23_0_0RCTNullIfNil(authState.lastAuthorizationResponse.authorizationCode),
        });
      } else {
        _logInResolve(@{@"type": @"cancel"});
      }
    } else {
      _logInReject(ABI23_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI23_0_0RCTPresentedViewController()
                                                   callback:callback];
...
-(void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error
{
  if (error != nil) {
    ...
    _logInResolve(@{
      @"type": @"success",
      @"accessToken": ABI23_0_0RCTNullIfNil(user.authentication.accessToken),
      @"serverAuthCode": ABI23_0_0RCTNullIfNil(user.serverAuthCode),
      @"idToken": ABI23_0_0RCTNullIfNil(user.authentication.idToken),
      @"refreshToken": ABI23_0_0RCTNullIfNil(user.authentication.refreshToken),
      @"user": @{
        @"id": ABI23_0_0RCTNullIfNil(user.userID),
        @"name": ABI23_0_0RCTNullIfNil(user.profile.name),
        @"familyName": ABI23_0_0RCTNullIfNil(user.profile.familyName),
        @"givenName": ABI23_0_0RCTNullIfNil(user.profile.givenName),
        @"email": ABI23_0_0RCTNullIfNil(user.profile.email),
...
{
  [ABI23_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI23_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
