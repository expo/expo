...
#if __has_include("ABI23_0_0EXGLARSessionManager.h")
#import "ABI23_0_0EXGLARSessionManager.h"
#else
#import "ABI23_0_0EXGLARSessionManagerStub.h"
#endif

@interface ABI23_0_0EXGLView ()

@property (nonatomic, weak) ABI23_0_0EXGLViewManager *viewManager;
...
@implementation ABI23_0_0EXGLView
...
- (instancetype)initWithManager:(ABI23_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
    _viewManager = viewManager;

    self.contentScaleFactor = ABI23_0_0RCTScreenScale();
...
- (void)setOnSurfaceCreate:(ABI23_0_0RCTDirectEventBlock)onSurfaceCreate
{
  _onSurfaceCreate = onSurfaceCreate;
  if (_onSurfaceCreate) {
    // Got non-empty onSurfaceCreate callback -- set up JS binding -- only possible on JavaScriptCore
    ABI23_0_0RCTBridge *bridge = _viewManager.bridge;
    if (!bridge.executorClass || [NSStringFromClass(bridge.executorClass) isEqualToString:@"ABI23_0_0RCTJSCExecutor"]) {
      // On JS thread, extract JavaScriptCore context, create ABI23_0_0EXGL context, call JS callback
      ...
      [bridge dispatchBlock:^{
        ...
        if (!jsContextRef) {
          ABI23_0_0RCTLogError(@"ABI23_0_0EXGL: The ReactABI23_0_0 Native bridge unexpectedly does not have a JavaScriptCore context.");
          return;
        }
      ...
      ABI23_0_0RCTLog(@"ABI23_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI23_0_0-native/docs/debugging.html)? ABI23_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI23_0_0EXGL.");
    }
  }
}
...
- (void)draw
{
  ...
  // called on the JS thread to create the ABI23_0_0EXGL context and save its id (see init method). In
...
{
  Class sessionManagerClass = NSClassFromString(@"ABI23_0_0EXGLARSessionManager");
