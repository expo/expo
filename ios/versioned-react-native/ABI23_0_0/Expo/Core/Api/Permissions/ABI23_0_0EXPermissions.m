...
#import <ABI23_0_0React/ABI23_0_0RCTUtils.h>
...
ABI23_0_0EX_EXPORT_SCOPED_MODULE(ExponentPermissions, PermissionsManager);
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ...
  NSMutableDictionary *response = [[NSMutableDictionary alloc] initWithDictionary:globalPermissionsResult];
    if (![[[self class] excludedScopedPermissions] containsObject:type] &&
      [globalPermissionsResult[@"status"] isEqualToString:[[self class] permissionStringForStatus:ABI23_0_0EXPermissionStatusGranted]]) {
    if (![_kernelPermissionsServiceDelegate hasGrantedPermission:type forExperience:self.experienceId]) {
      response[@"status"] = [[self class] permissionStringForStatus:ABI23_0_0EXPermissionStatusDenied];
    }
  }
  resolve(response);
}
                 ...
                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([systemPermissions[@"status"] isEqualToString:[ABI23_0_0EXPermissions permissionStringForStatus:ABI23_0_0EXPermissionStatusGranted]]) {
    if (![[[self class] excludedScopedPermissions] containsObject:type] &&
      ...
      UIAlertAction *allow = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        if ([_kernelPermissionsServiceDelegate savePermission:systemPermissions ofType:type forExperience:self.experienceId]) {
          ...
          deniedResult[@"status"] = [[weakSelf class] permissionStringForStatus:ABI23_0_0EXPermissionStatusDenied];
...
{
  if ([type isEqualToString:@"remoteNotifications"]) {
    return [ABI23_0_0EXRemoteNotificationRequester permissions];
  } else if ([type isEqualToString:@"location"]) {
    return [ABI23_0_0EXLocationRequester permissions];
  } else if ([type isEqualToString:@"camera"]) {
    return [ABI23_0_0EXCameraPermissionRequester permissions];
  } else if ([type isEqualToString:@"contacts"]) {
    return [ABI23_0_0EXContactsRequester permissions];
  } else if ([type isEqualToString:@"audioRecording"]) {
    return [ABI23_0_0EXAudioRecordingPermissionRequester permissions];
  } else if ([type isEqualToString:@"systemBrightness"]) {
    return [ABI23_0_0EXPermissions alwaysGrantedPermissions]; // permission is implicit
  }
  return nil;
}
...
- (void)showPermissionRequestAlert:(NSString *)type withAllowAction:(UIAlertAction *)allow withDenyAction:(UIAlertAction *)deny
{
  ...
  [ABI23_0_0RCTPresentedViewController() presentViewController:alert animated:YES completion:nil];
}
...
+ (NSString *)typeForRequester:(NSObject<ABI23_0_0EXPermissionRequester> *)requester
{
  if ([requester isKindOfClass:[ABI23_0_0EXLocationRequester class]]) {
    return @"location";
  } else if ([requester isKindOfClass:[ABI23_0_0EXCameraPermissionRequester class]]) {
    return @"camera";
  } else if ([requester isKindOfClass:[ABI23_0_0EXContactsRequester class]]) {
    return @"contacts";
  } else if ([requester isKindOfClass:[ABI23_0_0EXAudioRecordingPermissionRequester class]]) {
