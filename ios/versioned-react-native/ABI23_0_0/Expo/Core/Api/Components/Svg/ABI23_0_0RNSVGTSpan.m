...
@implementation ABI23_0_0RNSVGTSpan
{
    ABI23_0_0RNSVGBezierTransformer *_bezierTransformer;
    CGPathRef _cache;
}
...
{
    __block ABI23_0_0RNSVGBezierTransformer *bezierTransformer;
    [self traverseTextSuperviews:^(__kindof ABI23_0_0RNSVGText *node) {
        if ([node class] == [ABI23_0_0RNSVGTextPath class]) {
            bezierTransformer = [(ABI23_0_0RNSVGTextPath*)node getBezierTransformer];
            return NO;
        }
        return YES;
    }];
...
- (void)traverseTextSuperviews:(BOOL (^)(__kindof ABI23_0_0RNSVGText *node))block
{
    ABI23_0_0RNSVGText *targetView = self;
    BOOL result = block(self);
    
    while (targetView && [targetView class] != [ABI23_0_0RNSVGText class] && result) {
        if (![targetView isKindOfClass:[ABI23_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        
        targetView = (ABI23_0_0RNSVGText*)[targetView superview];
