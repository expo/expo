...
@property (nonatomic, strong) ABI23_0_0RCTPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI23_0_0RCTPromiseRejectBlock videoRecordedReject;

@end

@implementation ABI23_0_0EXCameraManager

ABI23_0_0RCT_EXPORT_MODULE(ExponentCameraManager);
ABI23_0_0RCT_EXPORT_VIEW_PROPERTY(onCameraReady, ABI23_0_0RCTDirectEventBlock);
ABI23_0_0RCT_EXPORT_VIEW_PROPERTY(onBarCodeRead, ABI23_0_0RCTDirectEventBlock);
...
- (UIView *)view
{
  ...
  if (!self.camera) {
    self.camera = [[ABI23_0_0EXCamera alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
- (NSDictionary *)constantsToExport
{
  return @{
           @"Type" :
             @{@"front" : @(ABI23_0_0EXCameraTypeFront), @"back" : @(ABI23_0_0EXCameraTypeBack)},
           @"FlashMode" : @{
               @"off" : @(ABI23_0_0EXCameraFlashModeOff),
               @"on" : @(ABI23_0_0EXCameraFlashModeOn),
               @"auto" : @(ABI23_0_0EXCameraFlashModeAuto),
               @"torch" : @(ABI23_0_0EXCameraFlashModeTorch)
               },
           @"AutoFocus" :
             @{@"on" : @(ABI23_0_0EXCameraAutoFocusOn), @"off" : @(ABI23_0_0EXCameraAutoFocusOff)},
           @"WhiteBalance" : @{
               @"auto" : @(ABI23_0_0EXCameraWhiteBalanceAuto),
               @"sunny" : @(ABI23_0_0EXCameraWhiteBalanceSunny),
               @"cloudy" : @(ABI23_0_0EXCameraWhiteBalanceCloudy),
               @"shadow" : @(ABI23_0_0EXCameraWhiteBalanceShadow),
               @"incandescent" : @(ABI23_0_0EXCameraWhiteBalanceIncandescent),
               @"fluorescent" : @(ABI23_0_0EXCameraWhiteBalanceFluorescent)
               },
           @"VideoQuality": @{
               @"2160p": @(ABI23_0_0EXCameraVideo2160p),
               @"1080p": @(ABI23_0_0EXCameraVideo1080p),
               @"720p": @(ABI23_0_0EXCameraVideo720p),
               @"480p": @(ABI23_0_0EXCameraVideo4x3),
               @"4:3": @(ABI23_0_0EXCameraVideo4x3),
               },
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, ABI23_0_0EXCamera)
{
  NSInteger type = [ABI23_0_0RCTConvert NSInteger:json];
  ...
  if (self.session.isRunning) {
    dispatch_async(self.sessionQueue, ^{
      ...
      if (error || captureDeviceInput == nil) {
        ABI23_0_0RCTLogError(@"%s: %@", __func__, error);
        return;
      }
      ...
      if ([self.session canAddInput:captureDeviceInput]) {
        ...
        if (self.videoCaptureDeviceInput) {
          ...
          ABI23_0_0RCTLogWarn(@"%s: Can't add null video capture device, doing nothing instead.", __func__);
        }
      }
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(flashMode, NSInteger, ABI23_0_0EXCamera)
{
  self.flashMode = [ABI23_0_0RCTConvert NSInteger:json];
  [self updateFlashMode];
}
...
- (void)updateFlashMode {
  dispatch_async(self.sessionQueue, ^{
    ...
    if (self.flashMode == ABI23_0_0EXCameraFlashModeTorch) {
      if (![device hasTorch])
        return;
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(autoFocus, NSInteger, ABI23_0_0EXCamera)
{
  self.autoFocus = [ABI23_0_0RCTConvert NSInteger:json];
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(focusDepth, NSNumber, ABI23_0_0EXCamera)
{
  self.focusDepth = [ABI23_0_0RCTConvert float:json];
...
- (void)updateFocusDepth {
  ...
  if (device.focusMode != ABI23_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if (![device respondsToSelector:@selector(isLockingFocusWithCustomLensPositionSupported)] || ![device isLockingFocusWithCustomLensPositionSupported]) {
    ABI23_0_0RCTLogWarn(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
    return;
  }
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(zoom, NSNumber, ABI23_0_0EXCamera)
{
  self.zoom = [ABI23_0_0RCTConvert CGFloat:json];
  [self updateZoom];
}
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(whiteBalance, NSInteger, ABI23_0_0EXCamera)
{
  self.whiteBalance = [ABI23_0_0RCTConvert NSInteger:json];
...
- (void)updateWhiteBalance {
  ...
  if (self.whiteBalance == ABI23_0_0EXCameraWhiteBalanceAuto) {
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(barCodeScannerEnabled, BOOL, ABI23_0_0EXCamera)
{
  self.scanning = [ABI23_0_0RCTConvert BOOL:json];
...
ABI23_0_0RCT_CUSTOM_VIEW_PROPERTY(barCodeTypes, NSArray, ABI23_0_0EXCamera)
{
  NSArray *types = [ABI23_0_0RCTConvert NSArray:json];
  NSSet *validTypes = [NSSet setWithArray:[[self class] validBarCodeTypes].allValues];
  for (id type in types) {
    if (![validTypes containsObject:type]) {
      ABI23_0_0RCTLogWarn(@"Unsupported BarCodeType: %@", type);
      return;
    }
  }
...
ABI23_0_0RCT_REMAP_METHOD(takePicture,
...
ABI23_0_0RCT_REMAP_METHOD(record,
  ...
  if (self.movieFileOutput != nil && !self.movieFileOutput.isRecording) {
    ...
    if (options[@"quality"]) {
      [self updateSessionPreset:[[self class] captureSessionPresetForVideoResolution:(ABI23_0_0EXCameraVideoResolution)[options[@"quality"] integerValue]]];
    }
...
ABI23_0_0RCT_EXPORT_METHOD(stopRecording) {
...
- (void)startSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI23_0_0EXCameraManager *weakSelf = self;
    [self setRuntimeErrorHandlingObserver:
     [NSNotificationCenter.defaultCenter
      ...
      usingBlock:^(NSNotification *note) {
        ABI23_0_0EXCameraManager *strongSelf = weakSelf;
...
- (void)initializeCaptureSessionInput:(NSString *)type
{
  dispatch_async(self.sessionQueue, ^{
    ...
    if (error || captureDeviceInput == nil) {
      ABI23_0_0RCTLog(@"%s: %@", __func__, error);
      return;
    }
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    if (!isMuted) {
      ...
      if (error || audioDeviceInput == nil) {
        ABI23_0_0RCTLogWarn(@"%s: %@", __func__, error);
        return;
      }
...
+ (float)temperatureForWhiteBalance:(ABI23_0_0EXCameraWhiteBalance)whiteBalance
{
  switch (whiteBalance) {
    case ABI23_0_0EXCameraWhiteBalanceSunny: default:
      return 5200;
    case ABI23_0_0EXCameraWhiteBalanceCloudy:
      return 6000;
    case ABI23_0_0EXCameraWhiteBalanceShadow:
      return 7000;
    case ABI23_0_0EXCameraWhiteBalanceIncandescent:
      return 3000;
    case ABI23_0_0EXCameraWhiteBalanceFluorescent:
      return 4200;
  }
}

+ (NSString *)captureSessionPresetForVideoResolution:(ABI23_0_0EXCameraVideoResolution)resolution
{
  switch (resolution) {
    case ABI23_0_0EXCameraVideo2160p:
      return AVCaptureSessionPreset3840x2160;
    case ABI23_0_0EXCameraVideo1080p:
      return AVCaptureSessionPreset1920x1080;
    case ABI23_0_0EXCameraVideo720p:
      return AVCaptureSessionPreset1280x720;
    case ABI23_0_0EXCameraVideo4x3:
