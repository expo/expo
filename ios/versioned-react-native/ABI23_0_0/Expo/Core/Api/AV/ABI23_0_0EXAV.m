...
#import "ABI23_0_0EXAVPlayerData.h"
...
NSString *const ABI23_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI23_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI23_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI23_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI23_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI23_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI23_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI23_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI23_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI23_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI23_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI23_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

@interface ABI23_0_0EXAV ()
...
@property (nonatomic, assign) ABI23_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI23_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI23_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, strong) NSMutableSet <NSObject<ABI23_0_0EXAVObject> *> *videoSet;
...
{
  if (self = [super initWithExperienceId:experienceId kernelServiceDelegate:kernelServiceInstances params:params]) {    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI23_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI23_0_0EXAudioInterruptionModeMixWithOthers;
...
- (void)_bridgeDidForeground:(NSNotification *)notification
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI23_0_0EXAVObject> *exAVObject) {
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI23_0_0EXAVObject> *exAVObject))block
{
  for (ABI23_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI23_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI23_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  BOOL allowsRecording = ((NSNumber *)mode[@"allowsRecordingIOS"]).boolValue;
  
  if (!playsInSilentMode && interruptionMode == ABI23_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI23_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and duckOthers cannot both be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI23_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and allowsRecording cannot both be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI23_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI23_0_0EXAVAudioSessionMode)audioSessionMode
{
  NSError *error;
  ABI23_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI23_0_0EXAVAudioSessionModeActiveMuted
    ? ABI23_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
  
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI23_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI23_0_0EXAudioInterruptionModeDoNotMix) {
    ...
    switch (activeInterruptionMode) {
      ...
      case ABI23_0_0EXAudioInterruptionModeDuckOthers:
        ...
        break;
      case ABI23_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI23_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI23_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI23_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI23_0_0EXAVObject> *exAVObject) {
    ABI23_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI23_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI23_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI23_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI23_0_0RCTErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded) {
    return ABI23_0_0RCTErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI23_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI23_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI23_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI23_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI23_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI23_0_0RCTPromiseRejectBlock)reject
{
  ABI23_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
    reject(@"E_AUDIO_NOPLAYER", nil, ABI23_0_0RCTErrorWithMessage(@"Player does not exist."));
  }
}
...
withEXVideoViewForTag:(nonnull NSNumber *)ReactABI23_0_0Tag
     withRejecter:(ABI23_0_0RCTPromiseRejectBlock)reject
{
  __weak ABI23_0_0EXAV *weakSelf = self;
  dispatch_async(dispatch_get_main_queue(), ^{
    __strong ABI23_0_0EXAV *strongSelf = weakSelf;
    if (strongSelf) {
      UIView *view = [strongSelf.bridge.uiManager viewForReactABI23_0_0Tag:ReactABI23_0_0Tag];
      if ([view isKindOfClass:[ABI23_0_0EXVideoView class]]) {
        dispatch_async(ABI23_0_0RCTGetUIManagerQueue(), ^{
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI23_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI23_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI23_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI23_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI23_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI23_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI23_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI23_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI23_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI23_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI23_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI23_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI23_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI23_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI23_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI23_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI23_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI23_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
      @([self _getFormatIDFromString:iosOptionsFromJS[ABI23_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI23_0_0RCTErrorWithMessage(@"Recorder already exists.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI23_0_0RCTPromiseRejectBlock)reject
{
  if (_audioRecorder == nil) {
    reject(@"E_AUDIO_NORECORDER", nil, ABI23_0_0RCTErrorWithMessage(@"Recorder does not exist."));
  }
...
ABI23_0_0EX_EXPORT_SCOPED_MULTISERVICE_MODULE(ExponentAV, @"AudioSessionManager", @"PermissionsManager");
...
ABI23_0_0RCT_EXPORT_METHOD(setAudioIsEnabled:(BOOL)value
...
ABI23_0_0RCT_EXPORT_METHOD(setAudioMode:(nonnull NSDictionary *)mode
                      resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                      rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
...
ABI23_0_0RCT_EXPORT_METHOD(loadForSound:(nonnull NSString *)uriString
                    withStatus:(nonnull NSDictionary *)status
                   withSuccess:(ABI23_0_0RCTResponseSenderBlock)loadSuccess
                     withError:(ABI23_0_0RCTResponseSenderBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);
  __weak __typeof__(self) weakSelf = self;
  ABI23_0_0EXAVPlayerData *data = [[ABI23_0_0EXAVPlayerData alloc] initWithEXAV:self
...
ABI23_0_0RCT_EXPORT_METHOD(unloadForSound:(nonnull NSNumber *)key
                        resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI23_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI23_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI23_0_0RCT_EXPORT_METHOD(setStatusForSound:(nonnull NSNumber *)key
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
...
ABI23_0_0RCT_EXPORT_METHOD(getStatusForSound:(nonnull NSNumber *)key
                           resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI23_0_0EXAVPlayerData *data) {
...
ABI23_0_0RCT_EXPORT_METHOD(setStatusUpdateCallbackForSound:(nonnull NSNumber *)key
                                     withCallback:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  ABI23_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    __block BOOL used = NO; // ABI23_0_0RCTResponseSenderBlock can only be used once
...
ABI23_0_0RCT_EXPORT_METHOD(setErrorCallbackForSound:(nonnull NSNumber *)key
                              withCallback:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  ABI23_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
...
ABI23_0_0RCT_EXPORT_METHOD(loadForVideo:(nonnull NSNumber *)ReactABI23_0_0Tag
...
ABI23_0_0RCT_EXPORT_METHOD(unloadForVideo:(nonnull NSNumber *)ReactABI23_0_0Tag
                        resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI23_0_0EXVideoView *view) {
...
ABI23_0_0RCT_EXPORT_METHOD(setStatusForVideo:(nonnull NSNumber *)ReactABI23_0_0Tag
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
...
ABI23_0_0RCT_EXPORT_METHOD(getStatusForVideo:(nonnull NSNumber *)ReactABI23_0_0Tag
                           resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI23_0_0EXVideoView *view) {
...
ABI23_0_0RCT_EXPORT_METHOD(prepareAudioRecorder:(nonnull NSDictionary *)options
                              resolver:(ABI23_0_0RCTPromiseResolveBlock)resolve
                              rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI23_0_0EXPermissions statusForPermissions:[ABI23_0_0EXAudioRecordingPermissionRequester permissions]] != ABI23_0_0EXPermissionStatusGranted ||
  ...
  if (_audioRecorder && !error) {
    ...
    if (error) {
      ...
      reject(@"E_AUDIO_RECORDERNOTCREATED", nil, ABI23_0_0RCTErrorWithMessage(@"Prepare encountered an error: recorder not prepared."));
    }
    _audioRecorderIsPreparing = false;
    [self demoteAudioSessionIfPossible];
  } else {
...
ABI23_0_0RCT_EXPORT_METHOD(startAudioRecording:(ABI23_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI23_0_0EXPermissions statusForPermissions:[ABI23_0_0EXAudioRecordingPermissionRequester permissions]] != ABI23_0_0EXPermissionStatusGranted ||
      ![_kernelPermissionsServiceDelegate hasGrantedPermission:@"audioRecording" forExperience:self.experienceId]) {
  ...
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (!_allowsAudioRecording) {
      reject(@"E_AUDIO_AUDIOMODE", nil, ABI23_0_0RCTErrorWithMessage(@"Recording not allowed on iOS."));
      ...
      if (!error) {
        if ([_audioRecorder record]) {
          ...
          reject(@"E_AUDIO_RECORDING", nil, ABI23_0_0RCTErrorWithMessage(@"Start encountered an error: recording not started."));
        }
      } else {
...
ABI23_0_0RCT_EXPORT_METHOD(pauseAudioRecording:(ABI23_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI23_0_0RCT_EXPORT_METHOD(stopAudioRecording:(ABI23_0_0RCTPromiseResolveBlock)resolve
                            rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI23_0_0RCT_EXPORT_METHOD(getAudioRecordingStatus:(ABI23_0_0RCTPromiseResolveBlock)resolve
                                 rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI23_0_0RCT_EXPORT_METHOD(unloadAudioRecorder:(ABI23_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
