...
#import <cxxReactABI23_0_0/ABI23_0_0JsArgumentHelpers.h>
...
@implementation ABI23_0_0RCTCxxMethod
{
...
- (ABI23_0_0RCTFunctionType)functionType
{
  std::string type(_method->getType());
  if (type == "sync") {
    return ABI23_0_0RCTFunctionTypeSync;
  } else if (type == "async") {
    return ABI23_0_0RCTFunctionTypeNormal;
  } else {
    return ABI23_0_0RCTFunctionTypePromise;
  }
}
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI23_0_0RCTLogError(@"Method %@.%s expects at least %zu arguments, but got %tu",
                ABI23_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI23_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
  ...
  try {
    ...
    ABI23_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI23_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
