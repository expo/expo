...
#import <cxxReactABI23_0_0/ABI23_0_0CxxNativeModule.h>
#import <ABI23_0_0jschelpers/ABI23_0_0Value.h>

#import "ABI23_0_0DispatchMessageQueueThread.h"
#import "ABI23_0_0RCTCxxModule.h"
#import "ABI23_0_0RCTNativeModule.h"
...
std::vector<std::unique_ptr<NativeModule>> createNativeModules(NSArray<ABI23_0_0RCTModuleData *> *modules, ABI23_0_0RCTBridge *bridge, const std::shared_ptr<Instance> &instance)
{
  ...
  for (ABI23_0_0RCTModuleData *moduleData in modules) {
    if ([moduleData.moduleClass isSubclassOfClass:[ABI23_0_0RCTCxxModule class]]) {
      nativeModules.emplace_back(std::make_unique<CxxNativeModule>(
        instance,
        [moduleData.name UTF8String],
        [moduleData] { return [(ABI23_0_0RCTCxxModule *)(moduleData.instance) createModule]; },
        std::make_shared<DispatchMessageQueueThread>(moduleData)));
    } else {
      nativeModules.emplace_back(std::make_unique<ABI23_0_0RCTNativeModule>(bridge, moduleData));
    }
  }
...
static NSError *errorWithException(const std::exception &e)
{
  ...
  if (jsException) {
    errorInfo[ABI23_0_0RCTJSRawStackTraceKey] = @(jsException->getStack().c_str());
  ...
  return [NSError errorWithDomain:ABI23_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}
...
NSError *tryAndReturnError(const std::function<void()>& func)
{
  try {
    ...
    @catch (NSException *exception) {
      ...
      return ABI23_0_0RCTErrorWithMessage(message);
    }
    @catch (id exception) {
      // This will catch any other ObjC exception, but no C++ exceptions
      return ABI23_0_0RCTErrorWithMessage(@"non-std ObjC Exception");
    }
    ...
    return ABI23_0_0RCTErrorWithMessage(@"non-std C++ exception");
  }
}
