...
#import "ABI23_0_0RCTAsyncLocalStorage.h"
...
static NSString *const ABI23_0_0RCTStorageDirectory = @"ABI23_0_0RCTAsyncLocalStorage_V1";
static NSString *const ABI23_0_0RCTManifestFileName = @"manifest.json";
static const NSUInteger ABI23_0_0RCTInlineValueThreshold = 1024;

#pragma mark - Static helper functions

static NSDictionary *ABI23_0_0RCTErrorForKey(NSString *key)
{
  if (![key isKindOfClass:[NSString class]]) {
    return ABI23_0_0RCTMakeAndLogError(@"Invalid key - must be a string.  Key: ", key, @{@"key": key});
  } else if (key.length < 1) {
    return ABI23_0_0RCTMakeAndLogError(@"Invalid key - must be at least one character.  Key: ", key, @{@"key": key});
  } else {
    return nil;
  }
}
...
static NSString *ABI23_0_0RCTReadFile(NSString *filePath, NSString *key, NSDictionary **errorOut)
{
  if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
    ...
    NSDictionary *extraData = @{@"key": ABI23_0_0RCTNullIfNil(key)};

    if (error) {
      if (errorOut) *errorOut = ABI23_0_0RCTMakeError(@"Failed to read storage file.", error, extraData);
      return nil;
    }

    if (encoding != NSUTF8StringEncoding) {
      if (errorOut) *errorOut = ABI23_0_0RCTMakeError(@"Incorrect encoding of storage file: ", @(encoding), extraData);
      return nil;
    }
    return entryString;
  }
...
static BOOL ABI23_0_0RCTMergeRecursive(NSMutableDictionary *destination, NSDictionary *source)
{
  BOOL modified = NO;
  for (NSString *key in source) {
    ...
    if ([sourceValue isKindOfClass:[NSDictionary class]]) {
      if ([destinationValue isKindOfClass:[NSDictionary class]]) {
        ...
        if (ABI23_0_0RCTMergeRecursive(destinationValue, sourceValue)) {
...
//   ABI23_0_0RCTGetStorageDirectory()
//   ABI23_0_0RCTGetManifestFilePath()
//   ABI23_0_0RCTGetMethodQueue()
//   ABI23_0_0RCTGetCache()
//   ABI23_0_0RCTDeleteStorageDirectory()

#define ABI23_0_0RCTGetStorageDirectory() _storageDirectory
#define ABI23_0_0RCTGetManifestFilePath() _manifestFilePath
#define ABI23_0_0RCTGetMethodQueue() self.methodQueue
#define ABI23_0_0RCTGetCache() self.cache

static NSDictionary *ABI23_0_0RCTDeleteStorageDirectory(NSString *storageDirectory)
{
  NSError *error;
  [[NSFileManager defaultManager] removeItemAtPath:storageDirectory error:&error];
  return error ? ABI23_0_0RCTMakeError(@"Failed to delete storage directory.", error, nil) : nil;
}
#define ABI23_0_0RCTDeleteStorageDirectory() ABI23_0_0RCTDeleteStorageDirectory(_storageDirectory)

#pragma mark - ABI23_0_0RCTAsyncLocalStorage

@interface ABI23_0_0RCTAsyncLocalStorage ()
...
@implementation ABI23_0_0RCTAsyncLocalStorage
{
  BOOL _haveSetup;
...
+ (NSString *)moduleName { return @"ABI23_0_0RCTAsyncLocalStorage"; }
- (instancetype)initWithStorageDirectory:(NSString *)storageDirectory
{
  if ((self = [super init])) {
    _storageDirectory = storageDirectory;
    _manifestFilePath = [ABI23_0_0RCTGetStorageDirectory() stringByAppendingPathComponent:ABI23_0_0RCTManifestFileName];
  }
  return self;
}
...
- (void)clearAllData
{
  dispatch_async(ABI23_0_0RCTGetMethodQueue(), ^{
    [self->_manifest removeAllObjects];
    [ABI23_0_0RCTGetCache() removeAllObjects];
    ABI23_0_0RCTDeleteStorageDirectory();
  });
}
...
- (NSDictionary *)_ensureSetup
{
  ...
  if (!_haveSetup) {
    ...
    NSString *serialized = ABI23_0_0RCTReadFile(ABI23_0_0RCTGetManifestFilePath(), ABI23_0_0RCTManifestFileName, &errorOut);
...
{
  NSError *error;
  NSString *serialized = ABI23_0_0RCTJSONStringify(_manifest, &error);
  [serialized writeToFile:ABI23_0_0RCTGetManifestFilePath() atomically:YES encoding:NSUTF8StringEncoding error:&error];
  NSDictionary *errorOut;
  if (error) {
    errorOut = ABI23_0_0RCTMakeError(@"Failed to write manifest file.", error, nil);
    ABI23_0_0RCTAppendError(errorOut, errors);
  }
  return errorOut;
}
...
{
  NSDictionary *errorOut = ABI23_0_0RCTErrorForKey(key);
  if (errorOut) {
    return errorOut;
  }
  NSString *value = [self _getValueForKey:key errorOut:&errorOut];
  [result addObject:@[key, ABI23_0_0RCTNullIfNil(value)]]; // Insert null if missing or failure.
  return errorOut;
}
...
{
  if (entry.count != 2) {
    return ABI23_0_0RCTMakeAndLogError(@"Entries must be arrays of the form [key: string, value: string], got: ", entry, nil);
  }
  ...
  [ABI23_0_0RCTGetCache() setObject:value forKey:key cost:value.length];
  if (error) {
    errorOut = ABI23_0_0RCTMakeError(@"Failed to write value.", error, @{@"key": key});
...
ABI23_0_0RCT_EXPORT_METHOD(multiSet:(NSArray<NSArray<NSString *> *> *)kvPairs
                  callback:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  NSDictionary *errorOut = [self _ensureSetup];
  if (errorOut) {
  ...
  callback(@[ABI23_0_0RCTNullIfNil(errors)]);
}

ABI23_0_0RCT_EXPORT_METHOD(multiMerge:(NSArray<NSArray<NSString *> *> *)kvPairs
                    callback:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  NSDictionary *errorOut = [self _ensureSetup];
  if (errorOut) {
  ...
  for (__strong NSArray<NSString *> *entry in kvPairs) {
    ...
    if (!keyError) {
      if (value) {
        NSError *jsonError;
        NSMutableDictionary *mergedVal = ABI23_0_0RCTJSONParseMutable(value, &jsonError);
        if (ABI23_0_0RCTMergeRecursive(mergedVal, ABI23_0_0RCTJSONParse(entry[1], &jsonError))) {
          entry = @[entry[0], ABI23_0_0RCTNullIfNil(ABI23_0_0RCTJSONStringify(mergedVal, NULL))];
        }
        if (jsonError) {
          keyError = ABI23_0_0RCTJSErrorFromNSError(jsonError);
        }
      }
      if (!keyError) {
...
ABI23_0_0RCT_EXPORT_METHOD(multiRemove:(NSArray<NSString *> *)keys
                  callback:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  NSDictionary *errorOut = [self _ensureSetup];
  if (errorOut) {
  ...
  BOOL changedManifest = NO;
  for (NSString *key in keys) {
    NSDictionary *keyError = ABI23_0_0RCTErrorForKey(key);
    if (!keyError) {
      if (_manifest[key] == (id)kCFNull) {
        NSString *filePath = [self _filePathForKey:key];
        [[NSFileManager defaultManager] removeItemAtPath:filePath error:nil];
...
ABI23_0_0RCT_EXPORT_METHOD(clear:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  [_manifest removeAllObjects];
  [ABI23_0_0RCTGetCache() removeAllObjects];
  NSDictionary *error = ABI23_0_0RCTDeleteStorageDirectory();
  callback(@[ABI23_0_0RCTNullIfNil(error)]);
}
