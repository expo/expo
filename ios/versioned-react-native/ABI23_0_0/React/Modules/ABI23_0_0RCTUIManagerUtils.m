...
char *const ABI23_0_0RCTUIManagerQueueName = "com.facebook.ReactABI23_0_0.ShadowQueue";

static BOOL pseudoUIManagerQueueFlag = NO;

dispatch_queue_t ABI23_0_0RCTGetUIManagerQueue(void)
{
  ...
  dispatch_once(&onceToken, ^{
    if ([NSOperation instancesRespondToSelector:@selector(qualityOfService)]) {
      ...
      shadowQueue = dispatch_queue_create(ABI23_0_0RCTUIManagerQueueName, attr);
    } else {
      shadowQueue = dispatch_queue_create(ABI23_0_0RCTUIManagerQueueName, DISPATCH_QUEUE_SERIAL);
...
BOOL ABI23_0_0RCTIsUIManagerQueue()
{
  static void *queueKey = &queueKey;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    dispatch_queue_set_specific(ABI23_0_0RCTGetUIManagerQueue(), queueKey, queueKey, NULL);
  });
  return dispatch_get_specific(queueKey) == queueKey;
}

BOOL ABI23_0_0RCTIsPseudoUIManagerQueue()
{
...
void ABI23_0_0RCTExecuteOnUIManagerQueue(dispatch_block_t block)
{
  if (ABI23_0_0RCTIsUIManagerQueue() || ABI23_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
...
void ABI23_0_0RCTUnsafeExecuteOnUIManagerQueueSync(dispatch_block_t block)
{
  if (ABI23_0_0RCTIsUIManagerQueue() || ABI23_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    if (ABI23_0_0RCTIsMainQueue()) {
      ...
      dispatch_sync(ABI23_0_0RCTGetUIManagerQueue(), ^{
        block();
      });
    }
  }
