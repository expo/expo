...
NSString *const ABI23_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI23_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI23_0_0RCTAccessibilityManager ()
...
@implementation ABI23_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    self.contentSizeCategory = ABI23_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI23_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI23_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI23_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI23_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI23_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI23_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI23_0_0Tag:ReactABI23_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI23_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI23_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI23_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI23_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI23_0_0RCTResponseSenderBlock)callback
...
@implementation ABI23_0_0RCTBridge (ABI23_0_0RCTAccessibilityManager)

- (ABI23_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI23_0_0RCTAccessibilityManager class]];
}
