...
const NSInteger ABI23_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI23_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI23_0_0RCTGetEventID(id<ABI23_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI23_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI23_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI23_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI23_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI23_0_0RCT_DEBUG) {
    ABI23_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI23_0_0 tag");
  }

  name = ABI23_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI23_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI23_0_0RCTEvent>)event
{
  ...
  for (id<ABI23_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI23_0_0RCTGetEventID(event);

  id<ABI23_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI23_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)dispatchEvent:(id<ABI23_0_0RCTEvent>)event
{
...
@implementation ABI23_0_0RCTBridge (ABI23_0_0RCTEventDispatcher)

- (ABI23_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI23_0_0RCTEventDispatcher class]];
}
