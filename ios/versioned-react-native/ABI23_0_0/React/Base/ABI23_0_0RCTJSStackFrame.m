...
#import "ABI23_0_0RCTJSStackFrame.h"

#import "ABI23_0_0RCTLog.h"
#import "ABI23_0_0RCTUtils.h"

static NSRegularExpression *ABI23_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI23_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI23_0_0RCTNullIfNil(self.methodName),
     @"file": ABI23_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI23_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
{
  return [[self alloc] initWithMethodName:ABI23_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI23_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI23_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}

+ (NSArray<ABI23_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI23_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI23_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI23_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
