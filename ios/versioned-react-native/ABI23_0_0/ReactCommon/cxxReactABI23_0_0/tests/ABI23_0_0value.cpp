...
#include "ABI23_0_0OpaqueJSString.h"
#endif
...
TEST(Value, Undefined) {
  ...
  ABI23_0_0EXPECT_EQ("undefined", s.str());
...
TEST(Value, FromJSON) {
  ...
  ABI23_0_0EXPECT_TRUE(v.isObject());
  JSC_JSGlobalContextRelease(ctx);
}
...
TEST(Value, ToJSONString) {
  ...
  ABI23_0_0EXPECT_TRUE(dyn.isObject());
  auto val = dyn.at("a");
  ASSERT_NE(nullptr, val);
  ASSERT_TRUE(val.isNumber());
  ABI23_0_0EXPECT_EQ(4, val.asInt());
  ABI23_0_0EXPECT_EQ(4.0f, val.asDouble());
