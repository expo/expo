...
#import "ABI23_0_0RCTTestRunner.h"
...
@implementation ABI23_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI23_0_0RCTBridgeModuleListProvider _moduleProvider;
  NSString *_appPath;
}
...
{
  ABI23_0_0RCTAssertParam(app);
  ABI23_0_0RCTAssertParam(referenceDirectory);
...
- (void)updateScript
{
  if (getenv("CI_USE_PACKAGER") || _useBundler) {
    ...
    _scriptURL = [[NSBundle bundleForClass:[ABI23_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
  }
  ABI23_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
}
...
configurationBlock:(void(^)(ABI23_0_0RCTRootView *rootView))configurationBlock
{
...
{
  __weak ABI23_0_0RCTBridge *batchedBridge;

  @autoreleasepool {
    __block NSString *error = nil;
    ABI23_0_0RCTLogFunction defaultLogFunction = ABI23_0_0RCTGetLogFunction();
    ABI23_0_0RCTSetLogFunction(^(ABI23_0_0RCTLogLevel level, ABI23_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      ...
      if (level >= ABI23_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI23_0_0RCTBridge *bridge = [[ABI23_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];
    [bridge.devSettings setIsDebuggingRemotely:_useJSDebugger];
    batchedBridge = [bridge batchedBridge];

    ABI23_0_0RCTRootView *rootView = [[ABI23_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI23_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI23_0_0RCTTestModule class]];
    ABI23_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    testModule.controller = _testController;
    testModule.testSelector = test;
    testModule.testSuffix = _testSuffix;
    testModule.view = rootView;

    UIViewController *vc = ABI23_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI23_0_0RCTTestStatusPending && error == nil) {
    ...
    ABI23_0_0RCTSetLogFunction(defaultLogFunction);
    ...
    ABI23_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);
#endif

    if (expectErrorBlock) {
      ABI23_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI23_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI23_0_0RCTAssert(testModule.status != ABI23_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI23_0_0RCTAssert(testModule.status == ABI23_0_0RCTTestStatusPassed, @"Test failed");
    }
  ...
  ABI23_0_0RCTAssert(batchedBridge == nil, @"Bridge should be deallocated after the test");
}
