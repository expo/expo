...
NSString *const ABI23_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI23_0_0RCTReactABI23_0_0TagAttributeName = @"ReactABI23_0_0TagAttributeName";

static NSString *const kShadowViewAttributeName = @"ABI23_0_0RCTShadowViewAttributeName";
...
@implementation ABI23_0_0RCTShadowText
{
...
static ABI23_0_0YGSize ABI23_0_0RCTMeasure(ABI23_0_0YGNodeRef node, float width, ABI23_0_0YGMeasureMode widthMode, float height, ABI23_0_0YGMeasureMode heightMode)
{
  ABI23_0_0RCTShadowText *shadowText = (__bridge ABI23_0_0RCTShadowText *)ABI23_0_0YGNodeGetContext(node);
  ...
  result.width = ABI23_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI23_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI23_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI23_0_0RCTMeasure);

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(contentSizeMultiplierDidChange:)
                                                 name:ABI23_0_0RCTUIManagerWillUpdateViewsDueToContentSizeMultiplierChangeNotification
                                               object:nil];
  }
  return self;
}
...
{
  ABI23_0_0YGNodeMarkDirty(self.yogaNode);
  [self dirtyText];
}
...
{
  if ([[self ReactABI23_0_0Superview] isKindOfClass:[ABI23_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI23_0_0Superview] ReactABI23_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:availableWidth widthMode:ABI23_0_0YGMeasureModeExactly];
  CGRect textFrame = [self calculateTextFrame:textStorage];
  BOOL selectable = _selectable;
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI23_0_0RCTText *view = (ABI23_0_0RCTText *)viewRegistry[self.ReactABI23_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI23_0_0RCTShadowText
     * to the ABI23_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI23_0_0RCTTextView
     * and its contaned ABI23_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI23_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  [layoutManager.textStorage enumerateAttribute:kShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI23_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI23_0_0YGNodeRef childNode = child.yogaNode;
      float width = ABI23_0_0YGNodeStyleGetWidth(childNode).value;
      float height = ABI23_0_0YGNodeStyleGetHeight(childNode).value;
      if (ABI23_0_0YGFloatIsUndefined(width) || ABI23_0_0YGFloatIsUndefined(height)) {
        ABI23_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI23_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI23_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI23_0_0RCTRoundPixelValue(width),
        ABI23_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI23_0_0YGMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){
    widthMode == ABI23_0_0YGMeasureModeUndefined || isnan(width) ? CGFLOAT_MAX : width,
    CGFLOAT_MAX
  };
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI23_0_0RCTFont updateFont:nil
  ...
  for (ABI23_0_0RCTShadowView *child in [self ReactABI23_0_0Subviews]) {
    if ([child isKindOfClass:[ABI23_0_0RCTShadowText class]]) {
      ABI23_0_0RCTShadowText *shadowText = (ABI23_0_0RCTShadowText *)child;
      ...
      ABI23_0_0RCTShadowRawText *shadowRawText = (ABI23_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI23_0_0YGNodeStyleGetWidth(child.yogaNode).value;
      float height = ABI23_0_0YGNodeStyleGetHeight(child.yogaNode).value;
      if (ABI23_0_0YGFloatIsUndefined(width) || ABI23_0_0YGFloatIsUndefined(height)) {
        ABI23_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      // Don't call setTextComputed on this child. ABI23_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI23_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI23_0_0RCTReactABI23_0_0TagAttributeName withValue:self.ReactABI23_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  // Text decoration
  if (_textDecorationLine == ABI23_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI23_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI23_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI23_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
- (CGRect)updateStorage:(NSTextStorage *)textStorage toFitFrame:(CGRect)frame
{
  ...
  frame.origin.y = self.compoundInsets.top + ABI23_0_0RCTRoundPixelValue((CGRectGetHeight(frame) - requiredSize.height) / 2.0f);
  return frame;
}
                               ...
                               prevMid:(CGFloat)prevMid
{
  ...
  if (round((prevMid / kAutoSizeGranularity)) == round((midScale / kAutoSizeGranularity))) {
    ...
    ABI23_0_0RCTSizeComparison comparison = [self attemptScale:midScale
                                            inStorage:textStorage
                                             forFrame:frame];
    if (comparison == ABI23_0_0RCTSizeWithinRange) {
      return [self calculateSize:textStorage];
    } else if (comparison == ABI23_0_0RCTSizeTooLarge) {
...
- (ABI23_0_0RCTSizeComparison)attemptScale:(CGFloat)scale
                         ...
                         forFrame:(CGRect)frame
{
                        ...
                        usingBlock:^(UIFont *font, NSRange range, BOOL *stop)
   {
     if (font) {
       ...
       if (newFont) {
         ...
         ABI23_0_0RCTLogError(@"Font '%@' doesn't support automatic scaling.", originalFont.familyName);
         *stop = YES;
       }
     }
   }];
  ...
  if (fitLines && fitSize) {
    ...
    {
      return ABI23_0_0RCTSizeWithinRange;
    } else {
      return ABI23_0_0RCTSizeTooSmall;
    }
  } else {
    return ABI23_0_0RCTSizeTooLarge;
  }
}
...
#define ABI23_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI23_0_0RCT_TEXT_PROPERTY(AdjustsFontSizeToFit, _adjustsFontSizeToFit, BOOL)
ABI23_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI23_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI23_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI23_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI23_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI23_0_0RCT_TEXT_PROPERTY(FontVariant, _fontVariant, NSArray *)
ABI23_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI23_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI23_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI23_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI23_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI23_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI23_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI23_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI23_0_0RCTTextDecorationLineType);
ABI23_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI23_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI23_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI23_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI23_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI23_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI23_0_0RCTShadowView *child in [self ReactABI23_0_0Subviews]) {
    if ([child isKindOfClass:[ABI23_0_0RCTShadowText class]]) {
      ((ABI23_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  for (ABI23_0_0RCTShadowView *child in [self ReactABI23_0_0Subviews]) {
    if ([child isKindOfClass:[ABI23_0_0RCTShadowText class]]) {
      ((ABI23_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
