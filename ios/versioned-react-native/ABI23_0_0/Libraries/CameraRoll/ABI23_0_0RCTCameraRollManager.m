...
#import "ABI23_0_0RCTCameraRollManager.h"
...
@implementation ABI23_0_0RCTConvert (ALAssetGroup)

ABI23_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI23_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI23_0_0RCTCameraRollManager
...
ABI23_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
    ...
    [_bridge.imageLoader loadImageWithURLRequest:request
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        [self->_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
          if (saveError) {
            ABI23_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
            reject(kErrorUnableToSave, nil, saveError);
          } else {
            resolve(assetURL.absoluteString);
          }
...
static void ABI23_0_0RCTResolvePromise(ABI23_0_0RCTPromiseResolveBlock resolve,
...
ABI23_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI23_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI23_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger first = [ABI23_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI23_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI23_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI23_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI23_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI23_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI23_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI23_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI23_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
