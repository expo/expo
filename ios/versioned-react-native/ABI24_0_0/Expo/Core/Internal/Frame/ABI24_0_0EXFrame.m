...
#define ABI24_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05
#define ABI24_0_0EX_FRAME_ORIENTATION_USE_MANIFEST 0

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI24_0_0EXFrameReactABI24_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI24_0_0EXFrameReactABI24_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI24_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI24_0_0Bridge;

@end

@interface ABI24_0_0EXFrame () <ABI24_0_0RCTInvalidating, ABI24_0_0RCTExceptionsManagerDelegate>

@property (nonatomic, assign) BOOL sourceSet;
@property (nonatomic, assign) BOOL needsReload;

@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoadingProgress;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoadingFinish;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoadingError;
...
@implementation ABI24_0_0EXFrame

@synthesize supportedInterfaceOrientations = _supportedInterfaceOrientations;

ABI24_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI24_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");

    _appManager = [[unversionedAppManagerClass alloc] initWithEXFrame:self];
    [_appManager setDelegate:self];
    _supportedInterfaceOrientations = ABI24_0_0EX_FRAME_ORIENTATION_USE_MANIFEST;
  }
  return self;
}
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI24_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI24_0_0RootView]) {
    [_appManager ReactABI24_0_0RootView].frame = self.bounds;
  }
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
  if (_supportedInterfaceOrientations != ABI24_0_0EX_FRAME_ORIENTATION_USE_MANIFEST) {
...
- (void)reactAppManager:(id)appManager loadedJavaScriptWithProgress:(ABI24_0_0RCTLoadingProgress *)progress;
{
  if (_onLoadingProgress) {
    _onLoadingProgress(@{
      @"status": ABI24_0_0RCTNullIfNil(progress.status),
      @"done": ABI24_0_0RCTNullIfNil(progress.done),
      @"total": ABI24_0_0RCTNullIfNil(progress.total),
    });
  }
}
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI24_0_0RootView] &&
      [_appManager ReactABI24_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI24_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI24_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI24_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI24_0_0RCTErrorWithMessage(message)];
