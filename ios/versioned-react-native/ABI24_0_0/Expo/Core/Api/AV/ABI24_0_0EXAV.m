...
NSString *const ABI24_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI24_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI24_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI24_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI24_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI24_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI24_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI24_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI24_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI24_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI24_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI24_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

NSString *const ABI24_0_0EXDidUpdatePlaybackStatusEventName = @"didUpdatePlaybackStatus";

@interface ABI24_0_0EXAV ()
...
@property (nonatomic, assign) ABI24_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI24_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI24_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, assign) BOOL isBeingObserved;
@property (nonatomic, strong) NSMutableSet <NSObject<ABI24_0_0EXAVObject> *> *videoSet;
...
- (instancetype)initWithExperienceId:(NSString *)experienceId kernelServiceDelegates:(NSDictionary *)kernelServiceInstances params:(NSDictionary *)params
{
  if (self = [super initWithExperienceId:experienceId kernelServiceDelegate:kernelServiceInstances params:params]) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI24_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI24_0_0EXAudioInterruptionModeMixWithOthers;
...
- (void)_bridgeDidForeground:(NSNotification *)notification
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI24_0_0EXAVObject> *exAVObject) {
...
#pragma mark - ABI24_0_0RCTEventEmitter
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI24_0_0EXAVObject> *exAVObject))block
{
  for (ABI24_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI24_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI24_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  BOOL allowsRecording = ((NSNumber *)mode[@"allowsRecordingIOS"]).boolValue;
  
  if (!playsInSilentMode && interruptionMode == ABI24_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI24_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and duckOthers cannot both be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI24_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and allowsRecording cannot both be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI24_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI24_0_0EXAVAudioSessionMode)audioSessionMode
{
  NSError *error;
  ABI24_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI24_0_0EXAVAudioSessionModeActiveMuted
    ? ABI24_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
  
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI24_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI24_0_0EXAudioInterruptionModeDoNotMix) {
    ...
    switch (activeInterruptionMode) {
      ...
      case ABI24_0_0EXAudioInterruptionModeDuckOthers:
        ...
        break;
      case ABI24_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI24_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI24_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI24_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI24_0_0EXAVObject> *exAVObject) {
    ABI24_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI24_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI24_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI24_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI24_0_0RCTErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded) {
    return ABI24_0_0RCTErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI24_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI24_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI24_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI24_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI24_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI24_0_0RCTPromiseRejectBlock)reject
{
  ABI24_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
    reject(@"E_AUDIO_NOPLAYER", nil, ABI24_0_0RCTErrorWithMessage(@"Player does not exist."));
  }
}
...
withEXVideoViewForTag:(nonnull NSNumber *)ReactABI24_0_0Tag
     withRejecter:(ABI24_0_0RCTPromiseRejectBlock)reject
{
  __weak ABI24_0_0EXAV *weakSelf = self;
  dispatch_async(dispatch_get_main_queue(), ^{
    __strong ABI24_0_0EXAV *strongSelf = weakSelf;
    if (strongSelf) {
      UIView *view = [strongSelf.bridge.uiManager viewForReactABI24_0_0Tag:ReactABI24_0_0Tag];
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI24_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI24_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI24_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI24_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI24_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI24_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI24_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI24_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI24_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI24_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI24_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI24_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI24_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI24_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI24_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI24_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI24_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI24_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
      @([self _getFormatIDFromString:iosOptionsFromJS[ABI24_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI24_0_0RCTErrorWithMessage(@"Recorder already exists.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI24_0_0RCTPromiseRejectBlock)reject
{
  if (_audioRecorder == nil) {
    reject(@"E_AUDIO_NORECORDER", nil, ABI24_0_0RCTErrorWithMessage(@"Recorder does not exist."));
  }
...
ABI24_0_0EX_EXPORT_SCOPED_MULTISERVICE_MODULE(ExponentAV, @"AudioSessionManager", @"PermissionsManager");

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI24_0_0EXDidUpdatePlaybackStatusEventName];
}

#pragma mark - Audio API: Global settings

ABI24_0_0RCT_EXPORT_METHOD(setAudioIsEnabled:(BOOL)value
...
ABI24_0_0RCT_EXPORT_METHOD(setAudioMode:(nonnull NSDictionary *)mode
                      resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                      rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
...
ABI24_0_0RCT_EXPORT_METHOD(loadForSound:(nonnull NSString *)uriString
                    withStatus:(nonnull NSDictionary *)status
                   withSuccess:(ABI24_0_0RCTResponseSenderBlock)loadSuccess
                     withError:(ABI24_0_0RCTResponseSenderBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);
  __weak __typeof__(self) weakSelf = self;
  ABI24_0_0EXAVPlayerData *data = [[ABI24_0_0EXAVPlayerData alloc] initWithEXAV:self
  ...
  data.statusUpdateCallback = ^(NSDictionary *status) {
    ...
    if (strongSelf && strongSelf.isBeingObserved) {
      ...
      [strongSelf sendEventWithName:ABI24_0_0EXDidUpdatePlaybackStatusEventName body:response];
    }
  };
...
ABI24_0_0RCT_EXPORT_METHOD(unloadForSound:(nonnull NSNumber *)key
                        resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI24_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI24_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI24_0_0RCT_EXPORT_METHOD(setStatusForSound:(nonnull NSNumber *)key
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
...
ABI24_0_0RCT_EXPORT_METHOD(getStatusForSound:(nonnull NSNumber *)key
                           resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI24_0_0EXAVPlayerData *data) {
...
ABI24_0_0RCT_EXPORT_METHOD(replaySound:(nonnull NSNumber *)key
                   withStatus:(nonnull NSDictionary *)status
                     resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                     rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
...
ABI24_0_0RCT_EXPORT_METHOD(setErrorCallbackForSound:(nonnull NSNumber *)key
                              withCallback:(ABI24_0_0RCTResponseSenderBlock)callback)
{
  ABI24_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    __block BOOL used = NO; // ABI24_0_0RCTResponseSenderBlock can only be used once
    __weak __typeof__(self) weakSelf = self;
    data.errorCallback = ^(NSString *error) {
      __strong __typeof__(self) strongSelf = weakSelf;
...
ABI24_0_0RCT_EXPORT_METHOD(loadForVideo:(nonnull NSNumber *)ReactABI24_0_0Tag
...
ABI24_0_0RCT_EXPORT_METHOD(unloadForVideo:(nonnull NSNumber *)ReactABI24_0_0Tag
                        resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI24_0_0EXVideoView *view) {
...
ABI24_0_0RCT_EXPORT_METHOD(setStatusForVideo:(nonnull NSNumber *)ReactABI24_0_0Tag
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
...
ABI24_0_0RCT_EXPORT_METHOD(replayVideo:(nonnull NSNumber *)ReactABI24_0_0Tag
                   withStatus:(nonnull NSDictionary *)status
                     resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                     rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
...
ABI24_0_0RCT_EXPORT_METHOD(getStatusForVideo:(nonnull NSNumber *)ReactABI24_0_0Tag
                           resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI24_0_0EXVideoView *view) {
...
ABI24_0_0RCT_EXPORT_METHOD(prepareAudioRecorder:(nonnull NSDictionary *)options
                              resolver:(ABI24_0_0RCTPromiseResolveBlock)resolve
                              rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI24_0_0EXPermissions statusForPermissions:[ABI24_0_0EXAudioRecordingPermissionRequester permissions]] != ABI24_0_0EXPermissionStatusGranted ||
  ...
  if (_audioRecorder && !error) {
    ...
    if (error) {
      ...
      reject(@"E_AUDIO_RECORDERNOTCREATED", nil, ABI24_0_0RCTErrorWithMessage(@"Prepare encountered an error: recorder not prepared."));
    }
    _audioRecorderIsPreparing = false;
    [self demoteAudioSessionIfPossible];
  } else {
...
ABI24_0_0RCT_EXPORT_METHOD(startAudioRecording:(ABI24_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI24_0_0EXPermissions statusForPermissions:[ABI24_0_0EXAudioRecordingPermissionRequester permissions]] != ABI24_0_0EXPermissionStatusGranted ||
      ![_kernelPermissionsServiceDelegate hasGrantedPermission:@"audioRecording" forExperience:self.experienceId]) {
  ...
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (!_allowsAudioRecording) {
      reject(@"E_AUDIO_AUDIOMODE", nil, ABI24_0_0RCTErrorWithMessage(@"Recording not allowed on iOS."));
      ...
      if (!error) {
        if ([_audioRecorder record]) {
          ...
          reject(@"E_AUDIO_RECORDING", nil, ABI24_0_0RCTErrorWithMessage(@"Start encountered an error: recording not started."));
        }
      } else {
...
ABI24_0_0RCT_EXPORT_METHOD(pauseAudioRecording:(ABI24_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI24_0_0RCT_EXPORT_METHOD(stopAudioRecording:(ABI24_0_0RCTPromiseResolveBlock)resolve
                            rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI24_0_0RCT_EXPORT_METHOD(getAudioRecordingStatus:(ABI24_0_0RCTPromiseResolveBlock)resolve
                                 rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI24_0_0RCT_EXPORT_METHOD(unloadAudioRecorder:(ABI24_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
