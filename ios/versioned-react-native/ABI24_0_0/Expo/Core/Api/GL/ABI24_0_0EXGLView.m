...
@interface ABI24_0_0EXGLView ()

@property (nonatomic, weak) ABI24_0_0EXGLViewManager *viewManager;
...
@implementation ABI24_0_0EXGLView
...
- (instancetype)initWithManager:(ABI24_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
    _viewManager = viewManager;

    self.contentScaleFactor = ABI24_0_0RCTScreenScale();
...
- (void)setOnSurfaceCreate:(ABI24_0_0RCTDirectEventBlock)onSurfaceCreate
{
  _onSurfaceCreate = onSurfaceCreate;
  if (_onSurfaceCreate) {
    // Got non-empty onSurfaceCreate callback -- set up JS binding -- only possible on JavaScriptCore
    ABI24_0_0RCTBridge *bridge = _viewManager.bridge;
    if (!bridge.executorClass || [NSStringFromClass(bridge.executorClass) isEqualToString:@"ABI24_0_0RCTJSCExecutor"]) {
      // On JS thread, extract JavaScriptCore context, create ABI24_0_0EXGL context, call JS callback
      ...
      [bridge dispatchBlock:^{
        ...
        if (!jsContextRef) {
          ABI24_0_0RCTLogError(@"ABI24_0_0EXGL: The ReactABI24_0_0 Native bridge unexpectedly does not have a JavaScriptCore context.");
          return;
        }
      ...
      ABI24_0_0RCTLog(@"ABI24_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI24_0_0-native/docs/debugging.html)? ABI24_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI24_0_0EXGL.");
    }
  }
}
...
- (void)draw
{
  ...
  // called on the JS thread to create the ABI24_0_0EXGL context and save its id (see init method). In
...
{
  Class sessionManagerClass = NSClassFromString(@"ABI24_0_0EXGLARSessionManager");
