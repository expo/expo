...
@implementation FBSDKInternalUtility (ABI24_0_0EXFacebook)
...
@implementation ABI24_0_0EXFacebook

@synthesize bridge = _bridge;

ABI24_0_0RCT_EXPORT_MODULE(ExponentFacebook)

ABI24_0_0RCT_REMAP_METHOD(logInWithReadPermissionsAsync,
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (loginMgr.loginBehavior != FBSDKLoginBehaviorWeb) {
      if ([_bridge.scopedModules.constants.appOwnership isEqualToString:@"expo"]) {
        ...
        reject(@"E_BEHAVIOR_NOT_PERMITTED", message, ABI24_0_0RCTErrorWithMessage(message));
        return;
      } else {
        if (![[self class] facebookAppIdFromNSBundle]) {
          ...
          reject(@"E_BEHAVIOR_NOT_SUPPORTED", message, ABI24_0_0RCTErrorWithMessage(message));
          return;
        }
      }
    }
