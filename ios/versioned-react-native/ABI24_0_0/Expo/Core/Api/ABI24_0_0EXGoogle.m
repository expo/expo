...
NSString *ABI24_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI24_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI24_0_0EXGoogle
{
  ABI24_0_0RCTPromiseResolveBlock _logInResolve;
  ABI24_0_0RCTPromiseRejectBlock _logInReject;
}

ABI24_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI24_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI24_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI24_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI24_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      if (authState.isAuthorized) {
        _logInResolve(@{
          @"type": @"success",
          @"accessToken": ABI24_0_0RCTNullIfNil(authState.lastTokenResponse.accessToken),
          @"idToken": ABI24_0_0RCTNullIfNil(authState.lastTokenResponse.idToken),
          @"refreshToken": ABI24_0_0RCTNullIfNil(authState.lastTokenResponse.refreshToken),
          @"serverAuthCode": ABI24_0_0RCTNullIfNil(authState.lastAuthorizationResponse.authorizationCode),
        });
      } else {
        _logInResolve(@{@"type": @"cancel"});
      }
    } else {
      _logInReject(ABI24_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI24_0_0RCTPresentedViewController()
                                                   callback:callback];
...
-(void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error
{
  if (error != nil) {
    ...
    _logInResolve(@{
      @"type": @"success",
      @"accessToken": ABI24_0_0RCTNullIfNil(user.authentication.accessToken),
      @"serverAuthCode": ABI24_0_0RCTNullIfNil(user.serverAuthCode),
      @"idToken": ABI24_0_0RCTNullIfNil(user.authentication.idToken),
      @"refreshToken": ABI24_0_0RCTNullIfNil(user.authentication.refreshToken),
      @"user": @{
        @"id": ABI24_0_0RCTNullIfNil(user.userID),
        @"name": ABI24_0_0RCTNullIfNil(user.profile.name),
        @"familyName": ABI24_0_0RCTNullIfNil(user.profile.familyName),
        @"givenName": ABI24_0_0RCTNullIfNil(user.profile.givenName),
        @"email": ABI24_0_0RCTNullIfNil(user.profile.email),
...
{
  [ABI24_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI24_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
