 ...
 * at `idealSize`.
 */
static BOOL ABI24_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
/**
 * See ABI24_0_0RCTConvert (ImageSource). We want to send down the source as a similar
 * JSON parameter.
 */
static NSDictionary *onLoadParamsForSource(ABI24_0_0RCTImageSource *source)
{
  NSDictionary *dict = @{
...
@interface ABI24_0_0RCTImageView ()

@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onPartialLoad;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI24_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI24_0_0RCTImageView
{
  ...
  ABI24_0_0RCTImageSource *_imageSource;

  // The image source that's being loaded from the network
  ABI24_0_0RCTImageSource *_pendingImageSource;
  ...
  ABI24_0_0RCTImageLoaderCancellationBlock _reloadImageCancellationBlock;
...
- (void)updateWithImage:(UIImage *)image
{
  ...
  if (_resizeMode == ABI24_0_0RCTResizeModeRepeat) {
...
- (void)setImageSources:(NSArray<ABI24_0_0RCTImageSource *> *)imageSources
{
...
- (void)setResizeMode:(ABI24_0_0RCTResizeMode)resizeMode
{
...
{
  ABI24_0_0RCTImageLoaderCancellationBlock previousCancellationBlock = _reloadImageCancellationBlock;
...
- (ABI24_0_0RCTImageSource *)imageSourceForSize:(CGSize)size
{
  ...
  const CGFloat scale = ABI24_0_0RCTScreenScale();
  const CGFloat targetImagePixels = size.width * size.height * scale * scale;

  ABI24_0_0RCTImageSource *bestSource = nil;
  CGFloat bestFit = CGFLOAT_MAX;
  for (ABI24_0_0RCTImageSource *source in _imageSources) {
...
- (BOOL)shouldChangeImageSource
{
  ...
  ABI24_0_0RCTImageSource *desiredImageSource = [self imageSourceForSize:self.frame.size];
...
- (void)reloadImage
{
  ...
  ABI24_0_0RCTImageSource *source = [self imageSourceForSize:self.frame.size];
  ...
  if (source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI24_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (_onProgress) {
    ...
    __weak ABI24_0_0RCTImageView *weakSelf = self;
    ABI24_0_0RCTImageLoaderPartialLoadBlock partialLoadHandler = ^(UIImage *image) {
      [weakSelf imageLoaderLoadedImage:image error:nil forImageSource:source partial:YES];
    };

    CGSize imageSize = self.bounds.size;
    CGFloat imageScale = ABI24_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    ABI24_0_0RCTImageLoaderCompletionBlock completionHandler = ^(NSError *error, UIImage *loadedImage) {
...
- (void)imageLoaderLoadedImage:(UIImage *)loadedImage error:(NSError *)error forImageSource:(ABI24_0_0RCTImageSource *)source partial:(BOOL)isPartialLoad
{
  ...
  void (^setImageBlock)(UIImage *) = ^(UIImage *image) {
    ...
    if (image.ReactABI24_0_0KeyframeAnimation) {
      [self.layer addAnimation:image.ReactABI24_0_0KeyframeAnimation forKey:@"contents"];
    } else {
      [self.layer removeAnimationForKey:@"contents"];
      self.image = image;
    }
    ...
    if (isPartialLoad) {
      ...
      if (self->_onLoad) {
        ABI24_0_0RCTImageSource *sourceLoaded = [source imageSourceWithSize:image.size scale:source.scale];
  ...
  if (_blurRadius > __FLT_EPSILON__) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      UIImage *blurredImage = ABI24_0_0RCTBlurredImageWithRadius(loadedImage, self->_blurRadius);
...
- (void)ReactABI24_0_0SetFrame:(CGRect)frame
{
  ...
  if ([self shouldChangeImageSource]) {
    ...
    CGSize idealSize = ABI24_0_0RCTTargetSize(imageSize, imageScale, frame.size, ABI24_0_0RCTScreenScale(),
                                     (ABI24_0_0RCTResizeMode)self.contentMode, YES);

    // Don't reload if the current image or target image size is close enough
    if (!ABI24_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize) ||
        !ABI24_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
      return;
    }
    ...
    ABI24_0_0RCTLogInfo(@"Reloading image %@ as size %@", _imageSource.request.URL.absoluteString, NSStringFromCGSize(idealSize));
