...
#import <ReactABI24_0_0/ABI24_0_0RCTReconnectingWebSocket.h>
#import <ReactABI24_0_0/ABI24_0_0RCTSRWebSocket.h>
...
static dispatch_queue_t ABI24_0_0RCTPackagerConnectionQueue()
{
  static dispatch_queue_t queue;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    queue = dispatch_queue_create("com.facebook.ABI24_0_0RCTPackagerConnectionQueue", DISPATCH_QUEUE_SERIAL);
  });
  return queue;
};

@interface ABI24_0_0RCTPackagerConnection () <ABI24_0_0RCTWebSocketProtocolDelegate>
@end

@implementation ABI24_0_0RCTPackagerConnection {
  NSURL *_packagerURL;
  ABI24_0_0RCTReconnectingWebSocket *_socket;
  NSMutableDictionary<NSString *, id<ABI24_0_0RCTPackagerClientMethod>> *_handlers;
}

+ (void)checkDefaultConnectionWithCallback:(void (^)(BOOL isRunning))callback
                                     queue:(dispatch_queue_t)queue
{
  ABI24_0_0RCTBundleURLProvider *const settings = [ABI24_0_0RCTBundleURLProvider sharedSettings];
  NSURLComponents *components = [NSURLComponents new];
  components.scheme = @"http";
  components.host = settings.jsLocation ?: @"localhost";
  components.port = @(kABI24_0_0RCTBundleURLProviderDefaultPort);
...
{
  ABI24_0_0RCTPackagerConnectionBridgeConfig *config = [[ABI24_0_0RCTPackagerConnectionBridgeConfig alloc] initWithBridge:bridge];
...
- (void)connect
{
  ...
  static NSMutableDictionary<NSString *, ABI24_0_0RCTReconnectingWebSocket *> *socketConnections = nil;
  ...
  if (!_socket) {
    _socket = [[ABI24_0_0RCTReconnectingWebSocket alloc] initWithURL:url];
    _socket.delegateDispatchQueue = ABI24_0_0RCTPackagerConnectionQueue();
    [_socket start];
...
- (id<ABI24_0_0RCTPackagerClientMethod>)handlerForMethod:(NSString *)name
{
  @synchronized(self) {
...
{
  NSArray<NSNumber *> *const kSupportedVersions = @[ @(ABI24_0_0RCT_PACKAGER_CLIENT_PROTOCOL_VERSION) ];
  return [kSupportedVersions containsObject:version];
}

#pragma mark - ABI24_0_0RCTWebSocketProtocolDelegate

- (void)webSocket:(ABI24_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)message
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *msg = ABI24_0_0RCTJSONParse(message, &error);

  if (error) {
    ABI24_0_0RCTLogError(@"%@ failed to parse message with error %@\n<message>\n%@\n</message>", [self class], error, msg);
    return;
  }

  if (!isSupportedVersion(msg[@"version"])) {
    ABI24_0_0RCTLogError(@"%@ received message with not supported version %@", [self class], msg[@"version"]);
    return;
  }

  id<ABI24_0_0RCTPackagerClientMethod> methodHandler = [self handlerForMethod:msg[@"method"]];
  if (!methodHandler) {
    if (msg[@"id"]) {
      NSString *errorMsg = [NSString stringWithFormat:@"%@ no handler found for method %@", [self class], msg[@"method"]];
      ABI24_0_0RCTLogError(errorMsg, msg[@"method"]);
      [[[ABI24_0_0RCTPackagerClientResponder alloc] initWithId:msg[@"id"]
  ...
  dispatch_async(methodQueue, ^{
    if (msg[@"id"]) {
      [methodHandler handleRequest:msg[@"params"]
                     withResponder:[[ABI24_0_0RCTPackagerClientResponder alloc] initWithId:msg[@"id"]
                                                                           socket:webSocket]];
    } else {
