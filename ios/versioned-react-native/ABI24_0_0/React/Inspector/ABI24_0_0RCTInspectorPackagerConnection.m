...
// ReactABI24_0_0Android/src/main/java/com/facebook/ReactABI24_0_0/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int RECONNECT_DELAY_MS = 2000;

@interface ABI24_0_0RCTInspectorPackagerConnection () <ABI24_0_0RCTSRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI24_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  ABI24_0_0RCTSRWebSocket *_webSocket;
...
@interface ABI24_0_0RCTInspectorRemoteConnection () {
  __weak ABI24_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI24_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI24_0_0RCTInspectorPackagerConnection
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI24_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  if (_inspectorConnections[pageId]) {
    ...
    ABI24_0_0RCTLogError(@"Already connected: %@", pageId);
    return;
  }

  ABI24_0_0RCTInspectorRemoteConnection *remoteConnection =
    [[ABI24_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self
                                                              pageId:pageId];

  ABI24_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI24_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI24_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (void)handleWrappedEvent:(NSDictionary *)payload
{
  ...
  ABI24_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
  if (!inspectorConnection) {
    ABI24_0_0RCTLogError(@"Not connected: %@", pageId);
    return;
  }
...
- (NSArray *)pages
{
  NSArray<ABI24_0_0RCTInspectorPage *> *pages = [ABI24_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];
  for (ABI24_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(ABI24_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  // warn but don't die on unrecognized messages
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI24_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI24_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI24_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@",
      messageText);
    return;
  }
...
- (void)webSocket:(ABI24_0_0RCTSRWebSocket *)webSocket didCloseWithCode:(NSInteger)code
...
{
  if (_closed) {
    ABI24_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresopnding android code has a lot of custom config options for
  // timeouts, but it appears the iOS ABI24_0_0RCTSRWebSocket API doesn't have the same
  // implemented options.
  _webSocket = [[ABI24_0_0RCTSRWebSocket alloc] initWithURL:_url];
...
{
  if (_closed) {
    ABI24_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI24_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI24_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(
    dispatch_time(DISPATCH_TIME_NOW, RECONNECT_DELAY_MS *NSEC_PER_MSEC),
    dispatch_get_main_queue(), ^{
      ABI24_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI24_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI24_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
...
{
  ABI24_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
...
@implementation ABI24_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI24_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
