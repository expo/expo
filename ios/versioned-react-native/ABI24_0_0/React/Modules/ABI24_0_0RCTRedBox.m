...
@class ABI24_0_0RCTRedBoxWindow;

@protocol ABI24_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI24_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI24_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI24_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI24_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI24_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI24_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI24_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI24_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI24_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI24_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI24_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI24_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI24_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI24_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI24_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI24_0_0RCTKeyCommands for this, but since
...
@interface ABI24_0_0RCTRedBox () <ABI24_0_0RCTInvalidating, ABI24_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI24_0_0RCTRedBox
{
  ABI24_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI24_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI24_0_0RCTErrorInfo *)_customizeError:(ABI24_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI24_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI24_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI24_0_0RCTJSStackTraceKey]];
}
...
- (void)showErrorMessage:(NSString *)message withDetails:(NSString *)details stack:(NSArray<ABI24_0_0RCTJSStackFrame *> *)stack {
...
{
  NSArray<ABI24_0_0RCTJSStackFrame *> *stack = [ABI24_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
  [self showErrorMessage:message withParsedStack:stack isUpdate:NO];
}

- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
  [self showErrorMessage:message withParsedStack:[ABI24_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:NO];
}

- (void)updateErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
  [self showErrorMessage:message withParsedStack:[ABI24_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:YES];
}
...
- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI24_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI24_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI24_0_0RCTErrorInfo *errorInfo = [[ABI24_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI24_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI24_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI24_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI24_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI24_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI24_0_0RCTRedBoxWindow *)redBoxWindow
{
...
@implementation ABI24_0_0RCTBridge (ABI24_0_0RCTRedBox)

- (ABI24_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI24_0_0RCTRedBox class]];
}
...
- (ABI24_0_0RCTRedBox *)redBox { return nil; }
