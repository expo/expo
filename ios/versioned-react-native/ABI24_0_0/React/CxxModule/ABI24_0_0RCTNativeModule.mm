...
#import "ABI24_0_0RCTNativeModule.h"
...
static MethodCallResult invokeInner(ABI24_0_0RCTBridge *bridge, ABI24_0_0RCTModuleData *moduleData, unsigned int methodId, const folly::dynamic &params);

ABI24_0_0RCTNativeModule::ABI24_0_0RCTNativeModule(ABI24_0_0RCTBridge *bridge, ABI24_0_0RCTModuleData *moduleData)
    : m_bridge(bridge)
    , m_moduleData(moduleData) {}

std::string ABI24_0_0RCTNativeModule::getName() {
  return [m_moduleData.name UTF8String];
}

std::vector<MethodDescriptor> ABI24_0_0RCTNativeModule::getMethods() {
  std::vector<MethodDescriptor> descs;

  for (id<ABI24_0_0RCTBridgeMethod> method in m_moduleData.methods) {
    descs.emplace_back(
      method.JSMethodName,
      ABI24_0_0RCTFunctionDescriptorFromType(method.functionType)
    );
  }
...
folly::dynamic ABI24_0_0RCTNativeModule::getConstants() {
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(ABI24_0_0RCTProfileTagAlways,
    @"[ABI24_0_0RCTNativeModule getConstants] moduleData.exportedConstants", nil);
...
void ABI24_0_0RCTNativeModule::invoke(unsigned int methodId, folly::dynamic &&params, int callId) {
  // capture by weak pointer so that we can safely use these variables in a callback
  __weak ABI24_0_0RCTBridge *weakBridge = m_bridge;
  __weak ABI24_0_0RCTModuleData *weakModuleData = m_moduleData;
...
MethodCallResult ABI24_0_0RCTNativeModule::callSerializableNativeHook(unsigned int ReactABI24_0_0MethodId, folly::dynamic &&params) {
  return invokeInner(m_bridge, m_moduleData, ReactABI24_0_0MethodId, params);
}
...
static MethodCallResult invokeInner(ABI24_0_0RCTBridge *bridge, ABI24_0_0RCTModuleData *moduleData, unsigned int methodId, const folly::dynamic &params) {
  ...
  if (ABI24_0_0RCT_DEBUG && !method) {
    ABI24_0_0RCTLogError(@"Unknown methodID: %ud for module: %@",
