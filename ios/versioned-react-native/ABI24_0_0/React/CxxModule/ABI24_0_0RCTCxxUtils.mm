...
#import "ABI24_0_0DispatchMessageQueueThread.h"
#import "ABI24_0_0RCTCxxModule.h"
...
std::vector<std::unique_ptr<NativeModule>> createNativeModules(NSArray<ABI24_0_0RCTModuleData *> *modules, ABI24_0_0RCTBridge *bridge, const std::shared_ptr<Instance> &instance)
{
  ...
  for (ABI24_0_0RCTModuleData *moduleData in modules) {
    if ([moduleData.moduleClass isSubclassOfClass:[ABI24_0_0RCTCxxModule class]]) {
      nativeModules.emplace_back(std::make_unique<CxxNativeModule>(
        instance,
        [moduleData.name UTF8String],
        [moduleData] { return [(ABI24_0_0RCTCxxModule *)(moduleData.instance) createModule]; },
        std::make_shared<DispatchMessageQueueThread>(moduleData)));
    } else {
      nativeModules.emplace_back(std::make_unique<ABI24_0_0RCTNativeModule>(bridge, moduleData));
    }
  }
...
static NSError *errorWithException(const std::exception &e)
{
  ...
  if (jsException) {
    errorInfo[ABI24_0_0RCTJSRawStackTraceKey] = @(jsException->getStack().c_str());
  ...
  return [NSError errorWithDomain:ABI24_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}
...
NSError *tryAndReturnError(const std::function<void()>& func)
{
  try {
    ...
    @catch (NSException *exception) {
      ...
      return ABI24_0_0RCTErrorWithMessage(message);
    }
    @catch (id exception) {
      // This will catch any other ObjC exception, but no C++ exceptions
      return ABI24_0_0RCTErrorWithMessage(@"non-std ObjC Exception");
    }
    ...
    return ABI24_0_0RCTErrorWithMessage(@"non-std C++ exception");
  }
}
