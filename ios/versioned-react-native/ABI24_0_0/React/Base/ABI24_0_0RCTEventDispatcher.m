...
const NSInteger ABI24_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI24_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI24_0_0RCTGetEventID(id<ABI24_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI24_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI24_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI24_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI24_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI24_0_0RCT_DEBUG) {
    ABI24_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI24_0_0 tag");
  }

  name = ABI24_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI24_0_0RCTEventEmitter"
                  method:@"receiveEvent"
                    args:body ? @[body[@"target"], name, body] : @[body[@"target"], name]
              completion:NULL];
}

- (void)sendTextEventWithType:(ABI24_0_0RCTTextEventType)type
...
- (void)sendEvent:(id<ABI24_0_0RCTEvent>)event
{
  [_observersLock lock];

  for (id<ABI24_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI24_0_0RCTGetEventID(event);

  id<ABI24_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI24_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)addDispatchObserver:(id<ABI24_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI24_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)dispatchEvent:(id<ABI24_0_0RCTEvent>)event
{
...
@implementation ABI24_0_0RCTBridge (ABI24_0_0RCTEventDispatcher)

- (ABI24_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI24_0_0RCTEventDispatcher class]];
}
