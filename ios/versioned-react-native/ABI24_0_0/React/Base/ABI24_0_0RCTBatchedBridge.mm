...
#import "ABI24_0_0RCTBridgeMethod.h"
...
#define ABI24_0_0RCTAssertJSThread() \
  ABI24_0_0RCTAssert(![NSStringFromClass([self->_javaScriptExecutor class]) isEqualToString:@"ABI24_0_0RCTJSCExecutor"] || \
              [[[NSThread currentThread] name] isEqualToString:ABI24_0_0RCTJSCThreadName], \
...
@implementation ABI24_0_0RCTBatchedBridge
{
  std::atomic_bool _wasBatchActive;
  NSMutableArray<dispatch_block_t> *_pendingCalls;
  NSDictionary<NSString *, ABI24_0_0RCTModuleData *> *_moduleDataByName;
  NSArray<ABI24_0_0RCTModuleData *> *_moduleDataByID;
...
- (void)start
{
  ...
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI24_0_0RCTBatchedBridge setUp]", nil);

  dispatch_queue_t bridgeQueue = dispatch_queue_create("com.facebook.ReactABI24_0_0.ABI24_0_0RCTBridgeQueue", DISPATCH_QUEUE_CONCURRENT);

  dispatch_group_t initModulesAndLoadSource = dispatch_group_create();

  // Asynchronously load source code
  dispatch_group_enter(initModulesAndLoadSource);
  __weak ABI24_0_0RCTBatchedBridge *weakSelf = self;
  __block NSData *sourceCode;
  [self loadSource:^(NSError *error, ABI24_0_0RCTSource *source) {
    if (error) {
      ABI24_0_0RCTLogWarn(@"Failed to load source: %@", error);
  ...
  ABI24_0_0RCTPerformanceLogger *performanceLogger = self->_performanceLogger;
  ...
  dispatch_async(bridgeQueue, ^{
    ...
    dispatch_group_async(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      [performanceLogger markStartForTag:ABI24_0_0RCTPLJSCExecutorSetup];
      [weakSelf setUpExecutor];
      [performanceLogger markStopForTag:ABI24_0_0RCTPLJSCExecutorSetup];
    });

    // Asynchronously gather the module config
    dispatch_group_async(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      if (weakSelf.valid) {
        ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI24_0_0RCTBatchedBridge moduleConfig", nil);
        [performanceLogger markStartForTag:ABI24_0_0RCTPLNativeModulePrepareConfig];
        config = [weakSelf moduleConfig];
        [performanceLogger markStopForTag:ABI24_0_0RCTPLNativeModulePrepareConfig];
    ...
    dispatch_group_notify(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      ...
      [performanceLogger markStartForTag:ABI24_0_0RCTPLNativeModuleInjectConfig];
      [weakSelf injectJSONConfiguration:config onComplete:^(NSError *error) {
        [performanceLogger markStopForTag:ABI24_0_0RCTPLNativeModuleInjectConfig];
        if (error) {
          ABI24_0_0RCTLogWarn(@"Failed to inject config: %@", error);
          dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf stopLoadingWithError:error];
          });
        }
  ...
  dispatch_group_notify(initModulesAndLoadSource, bridgeQueue, ^{
    ABI24_0_0RCTBatchedBridge *strongSelf = weakSelf;
...
{
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI24_0_0RCTBatchedBridge initModules]", nil);
  ...
  NSMutableArray<ABI24_0_0RCTModuleData *> *moduleDataByID = [NSMutableArray new];
  NSMutableDictionary<NSString *, ABI24_0_0RCTModuleData *> *moduleDataByName = [NSMutableDictionary new];

  // Set up moduleData for pre-initialized module instances
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"extraModules", nil);
  for (id<ABI24_0_0RCTBridgeModule> module in extraModules) {
    ...
    if (ABI24_0_0RCT_DEBUG) {
      ...
      ABI24_0_0RCTModuleData *moduleData = moduleDataByName[moduleName];
      if (moduleData) {
    ...
    ABI24_0_0RCTModuleData *moduleData = [[ABI24_0_0RCTModuleData alloc] initWithModuleInstance:module
                                                                       bridge:self];
    ...
    if (moduleClass == self.executorClass) {
      _javaScriptExecutor = (id<ABI24_0_0RCTJavaScriptExecutor>)module;
    }
  }
  ABI24_0_0RCT_PROFILE_END_EVENT(ABI24_0_0RCTProfileTagAlways, @"");
  ...
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"JavaScriptExecutor", nil);
  if (!_javaScriptExecutor) {
    id<ABI24_0_0RCTJavaScriptExecutor> executorModule = [self.executorClass new];
    ABI24_0_0RCTModuleData *moduleData = [[ABI24_0_0RCTModuleData alloc] initWithModuleInstance:executorModule
                                                                       bridge:self];
  ...
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"ModuleData", nil);
  for (Class moduleClass in ABI24_0_0RCTGetModuleClasses()) {
    NSString *moduleName = ABI24_0_0RCTBridgeModuleNameForClass(moduleClass);
    ...
    moduleData = [[ABI24_0_0RCTModuleData alloc] initWithModuleClass:moduleClass
                                                     bridge:self];
    moduleDataByName[moduleName] = moduleData;
    [moduleClassesByID addObject:moduleClass];
    [moduleDataByID addObject:moduleData];
  ...
  for (ABI24_0_0RCTModuleData *moduleData in _moduleDataByID) {
    if (moduleData.hasInstance &&
        (!moduleData.requiresMainQueueSetup || ABI24_0_0RCTIsMainQueue())) {
...
- (NSString *)moduleConfig
{
  ...
  for (ABI24_0_0RCTModuleData *moduleData in _moduleDataByID) {
    if (self.executorClass == [ABI24_0_0RCTJSCExecutor class]) {
      [config addObject:@[moduleData.name]];
    } else {
      [config addObject:ABI24_0_0RCTNullIfNil(moduleData.config)];
    }
  }

  return ABI24_0_0RCTJSONStringify(@{
...
- (void)executeSourceCode:(NSData *)sourceCode
{
  ...
  [self enqueueApplicationScript:sourceCode url:self.bundleURL onComplete:^(NSError *loadError) {
    ...
    if (loadError) {
      ABI24_0_0RCTLogWarn(@"Failed to execute source code: %@ %@", [loadError localizedDescription], [loadError localizedFailureReason]);
      dispatch_async(dispatch_get_main_queue(), ^{
    ...
    NSRunLoop *targetRunLoop = [self->_javaScriptExecutor isKindOfClass:[ABI24_0_0RCTJSCExecutor class]] ? [NSRunLoop currentRunLoop] : [NSRunLoop mainRunLoop];
    ...
    [self->_performanceLogger setValue:nativeRequiresCount forTag:ABI24_0_0RCTPLRAMStartupNativeRequiresCount];
    uint64_t nativeRequires = [self->_performanceLogger valueForTag:ABI24_0_0RCTPLRAMNativeRequires];
    [self->_performanceLogger setValue:nativeRequires forTag:ABI24_0_0RCTPLRAMStartupNativeRequires];

    [self->_performanceLogger markStopForTag:ABI24_0_0RCTPLBridgeStartup];
...
#if ABI24_0_0RCT_DEV
      ABI24_0_0RCTLogWarn(@"ABI24_0_0RCTBatchedBridge is deprecated and will be removed in a future ReactABI24_0_0 Native release. "
        "See https://fb.me/ReactABI24_0_0-cxx-bridge for upgrade instructions.");
#endif
    });
...
- (void)_flushPendingCalls
{
  ...
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"Processing pendingCalls", @{ @"count": @(_pendingCalls.count) });
  _loading = NO;
...
- (void)stopLoadingWithError:(NSError *)error
{
  ...
  if ([error userInfo][ABI24_0_0RCTJSStackTraceKey]) {
    [self.redBox showErrorMessage:[error localizedDescription]
                        withStack:[error userInfo][ABI24_0_0RCTJSStackTraceKey]];
  }
...
- (Class)executorClass
{
  return _parentBridge.executorClass ?: [ABI24_0_0RCTJSCExecutor class];
}
                ...
                queue:(dispatch_queue_t)queue
{
  if (queue == ABI24_0_0RCTJSThread) {
    ...
    ABI24_0_0RCTAssert(_javaScriptExecutor != nil, @"Need JS executor to schedule JS work");

    [_javaScriptExecutor executeBlockOnJavaScriptQueue:^{
      ABI24_0_0RCTProfileEndFlowEvent();

      ABI24_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI24_0_0RCTBatchedBridge dispatchBlock", @{ @"loading": @(self.loading) });

      if (self.loading) {
        ABI24_0_0RCTAssert(self->_pendingCalls != nil, @"Can't add pending call, bridge is no longer loading");
...
- (void)invalidate
{
  ...
  ABI24_0_0RCTAssertMainQueue();
  ABI24_0_0RCTAssert(_javaScriptExecutor != nil, @"Can't complete invalidation without a JS executor");
  ...
  for (ABI24_0_0RCTModuleData *moduleData in _moduleDataByID) {
    ...
    id<ABI24_0_0RCTBridgeModule> instance = nil;
    if ([moduleData hasInstance]) {
    ...
    if ([instance respondsToSelector:@selector(invalidate)]) {
      ...
      [self dispatchBlock:^{
        [(id<ABI24_0_0RCTInvalidating>)instance invalidate];
...
{
  if (ABI24_0_0RCT_DEBUG && [_javaScriptExecutor isValid]) {
...
- (void)enqueueJSCall:(NSString *)module method:(NSString *)method args:(NSArray *)args completion:(dispatch_block_t)completion
{
 ...
 /**
  ...
  ABI24_0_0RCT_PROFILE_BEGIN_EVENT(ABI24_0_0RCTProfileTagAlways, @"-[ABI24_0_0RCTBatchedBridge enqueueJSCall:]", nil);
...
- (JSValue *)callFunctionOnModule:(NSString *)module
                        ...
                        arguments:(NSArray *)arguments
                            error:(NSError **)error
{
  ABI24_0_0RCTJSCExecutor *jsExecutor = (ABI24_0_0RCTJSCExecutor *)_javaScriptExecutor;
  if (![jsExecutor isKindOfClass:[ABI24_0_0RCTJSCExecutor class]]) {
    ABI24_0_0RCTLogWarn(@"FBReactABI24_0_0BridgeJSExecutor is only supported when running in JSC");
    return nil;
  }
  ...
  [jsExecutor callFunctionOnModule:module
    ...
    ABI24_0_0RCTAssert([length isNumber] && [length toUInt32] == 2,
...
{
  ABI24_0_0RCTAssert(onComplete != nil, @"onComplete block passed in should be non-nil");
  ...
  [_javaScriptExecutor executeApplicationScript:script sourceURL:url onComplete:^(NSError *scriptLoadError) {
    ...
    ABI24_0_0RCT_PROFILE_BEGIN_EVENT(ABI24_0_0RCTProfileTagAlways, @"FetchApplicationScriptCallbacks", nil);
    [self->_javaScriptExecutor flushedQueue:^(id json, NSError *error)
     {
       ABI24_0_0RCT_PROFILE_END_EVENT(ABI24_0_0RCTProfileTagAlways, @"js_call,init");
...
{
  NSArray *requestsArray = [ABI24_0_0RCTConvert NSArray:buffer];

  if (ABI24_0_0RCT_DEBUG && requestsArray.count <= ABI24_0_0RCTBridgeFieldParams) {
    ABI24_0_0RCTLogError(@"Buffer should contain at least %tu sub-arrays. Only found %tu",
                ABI24_0_0RCTBridgeFieldParams + 1, requestsArray.count);
    return;
  }

  NSArray<NSNumber *> *moduleIDs = [ABI24_0_0RCTConvert NSNumberArray:requestsArray[ABI24_0_0RCTBridgeFieldRequestModuleIDs]];
  NSArray<NSNumber *> *methodIDs = [ABI24_0_0RCTConvert NSNumberArray:requestsArray[ABI24_0_0RCTBridgeFieldMethodIDs]];
  NSArray<NSArray *> *paramsArrays = [ABI24_0_0RCTConvert NSArrayArray:requestsArray[ABI24_0_0RCTBridgeFieldParams]];

  int64_t callID = -1;

  if (requestsArray.count > 3) {
    callID = [requestsArray[ABI24_0_0RCTBridgeFieldCallID] longLongValue];
  }

  if (ABI24_0_0RCT_DEBUG && (moduleIDs.count != methodIDs.count || moduleIDs.count != paramsArrays.count)) {
    ABI24_0_0RCTLogError(@"Invalid data message - all must be length: %zd", moduleIDs.count);
    return;
  }
  ...
  [moduleIDs enumerateObjectsUsingBlock:^(NSNumber *moduleID, NSUInteger i, __unused BOOL *stop) {
    ABI24_0_0RCTModuleData *moduleData = self->_moduleDataByID[moduleID.integerValue];
  ...
  for (dispatch_queue_t queue in buckets) {
    ...
    dispatch_block_t block = ^{
      ...
      ABI24_0_0RCT_PROFILE_BEGIN_EVENT(ABI24_0_0RCTProfileTagAlways, @"-[ABI24_0_0RCTBatchedBridge handleBuffer:]", (@{
        @"calls": @(calls.count),
      }));
      ...
      @autoreleasepool {
        for (NSNumber *indexObj in calls) {
          ...
          if (ABI24_0_0RCT_DEV && callID != -1 && self->_flowIDMap != NULL && ABI24_0_0RCTProfileIsProfiling()) {
            ...
            _ABI24_0_0RCTProfileEndFlowEvent(newFlowID);
      ...
      ABI24_0_0RCT_PROFILE_END_EVENT(ABI24_0_0RCTProfileTagAlways, @"objc_call,dispatch_async");
    };
...
{
  // TODO: batchDidComplete is only used by ABI24_0_0RCTUIManager - can we eliminate this special case?
  for (ABI24_0_0RCTModuleData *moduleData in _moduleDataByID) {
                ...
                params:(NSArray *)params
{
  ...
  ABI24_0_0RCTModuleData *moduleData = _moduleDataByID[moduleID];
  if (ABI24_0_0RCT_DEBUG && !moduleData) {
    ABI24_0_0RCTLogError(@"No module found for id '%zd'", moduleID);
    return nil;
  }

  id<ABI24_0_0RCTBridgeMethod> method = moduleData.methods[methodID];
  if (ABI24_0_0RCT_DEBUG && !method) {
    ABI24_0_0RCTLogError(@"Unknown methodID: %zd for module: %zd (%@)", methodID, moduleID, moduleData.name);
    return nil;
  }
  ...
  @catch (NSException *exception) {
    ...
    if ([exception.name hasPrefix:ABI24_0_0RCTFatalExceptionName]) {
      @throw exception;
    }
...
- (JSContext *)jsContext
{
  if ([_javaScriptExecutor isKindOfClass:[ABI24_0_0RCTJSCExecutor class]]) {
    return [(ABI24_0_0RCTJSCExecutor *)_javaScriptExecutor jsContext];
  } else {
    return nil;
  }
}
