...
@implementation ABI24_0_0RNSVGNode
{
    BOOL _transparent;
    CGPathRef _cachedClipPath;
    ABI24_0_0RNSVGSvgView *_svgView;
}
...
- (void)invalidate
{
    id<ABI24_0_0RNSVGContainer> container = (id<ABI24_0_0RNSVGContainer>)self.superview;
...
- (ABI24_0_0RNSVGSvgView *)getSvgView
{
    ...
    if ([parent class] == [ABI24_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI24_0_0RNSVGNode class]]) {
        ABI24_0_0RNSVGNode *node = parent;
        _svgView = [node getSvgView];
    } else {
        ABI24_0_0RCTLogError(@"ABI24_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI24_0_0RNSVGPercentageConverter stringToFloat:length relative:[self getContextWidth] offset:0];
}

- (CGFloat)relativeOnHeight:(NSString *)length
{
    return [ABI24_0_0RNSVGPercentageConverter stringToFloat:length relative:[self getContextHeight] offset:0];
}
