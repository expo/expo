...
#import "ABI40_0_0RCTSurfaceTouchHandler.h"

#import <ABI40_0_0React/ABI40_0_0RCTUtils.h>
#import <ABI40_0_0React/ABI40_0_0RCTViewComponentView.h>
#import <UIKit/UIGestureRecognizerSubclass.h>

#import "ABI40_0_0RCTConversions.h"
#import "ABI40_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI40_0_0RCTTouchEventType) {
  ABI40_0_0RCTTouchEventTypeTouchStart,
  ABI40_0_0RCTTouchEventTypeTouchMove,
  ABI40_0_0RCTTouchEventTypeTouchEnd,
  ABI40_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI40_0_0RCTComponentViewProtocol> *componentView = nil;
...
static void UpdateActiveTouchWithUITouch(ActiveTouch &activeTouch, UITouch *uiTouch, UIView *rootComponentView)
{
  ...
  activeTouch.touch.offsetPoint = ABI40_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI40_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI40_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI40_0_0RCTForceTouchAvailable()) {
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView)
{
  ...
  if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
    activeTouch.eventEmitter = [(id<ABI40_0_0RCTTouchableComponentViewProtocol>)componentView
...
@interface ABI40_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI40_0_0RCTSurfaceTouchHandler {
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI40_0_0React Native,
...
ABI40_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithTarget : (id)target action : (SEL)action)

- (void)attachToView:(UIView *)view
{
  ABI40_0_0RCTAssert(self.view == nil, @"ABI40_0_0RCTTouchHandler already has attached view.");
...
{
  ABI40_0_0RCTAssertParam(view);
  ABI40_0_0RCTAssert(self.view == view, @"ABI40_0_0RCTTouchHandler attached to another view.");
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI40_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI40_0_0RCTTouchEventTypeTouchEnd || eventType == ABI40_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitters) {
    ...
    switch (eventType) {
      case ABI40_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI40_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI40_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI40_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI40_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI40_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI40_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI40_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (_activeTouches.size() != 0) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI40_0_0RCTTouchEventTypeTouchCancel];
