...
#import <ABI40_0_0React/ABI40_0_0RCTSurfaceRootView.h>
...
@implementation ABI40_0_0RCTFabricSurface {
  // Immutable
  ABI40_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI40_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI40_0_0RCTAllocateRootViewTag() integerValue];

    _minimumSize = CGSizeZero;
    // FIXME: Replace with `_maximumSize = CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX);`.
    _maximumSize = ABI40_0_0RCTScreenSize();
...
- (BOOL)start
{
  if (![self _setStage:ABI40_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI40_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI40_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI40_0_0RCTSurfaceView alloc] initWithSurface:(ABI40_0_0RCTSurface *)self];
...
- (BOOL)_setStage:(ABI40_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
...
- (void)_propagateStageChange:(ABI40_0_0RCTSurfaceStage)stage
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI40_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI40_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
