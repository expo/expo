...
#import <ABI40_0_0React/components/iostextinput/TextInputComponentDescriptor.h>
...
#import <ABI40_0_0React/ABI40_0_0RCTUITextView.h>

#import "ABI40_0_0RCTConversions.h"
#import "ABI40_0_0RCTTextInputNativeCommands.h"
#import "ABI40_0_0RCTTextInputUtils.h"

using namespace ABI40_0_0facebook::ABI40_0_0React;

@interface ABI40_0_0RCTTextInputComponentView () <ABI40_0_0RCTBackedTextInputDelegate, ABI40_0_0RCTTextInputViewProtocol>
@end

@implementation ABI40_0_0RCTTextInputComponentView {
  TextInputShadowNode::ConcreteState::Shared _state;
  UIView<ABI40_0_0RCTBackedTextInputViewProtocol> *_backedTextInputView;
   ...
   * if UITextView is backing text input view, inside `-[ABI40_0_0RCTTextInputComponentView textInputDidChangeSelection]` we make
   * sure to call `onChange` before `onSelectionChange` and ignore next `-[ABI40_0_0RCTTextInputComponentView
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _backedTextInputView = props.traits.multiline ? [[ABI40_0_0RCTUITextView alloc] init] : [[ABI40_0_0RCTUITextField alloc] init];
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  if (newTextInputProps.traits.autocapitalizationType != oldTextInputProps.traits.autocapitalizationType) {
    _backedTextInputView.autocapitalizationType =
        ABI40_0_0RCTUITextAutocapitalizationTypeFromAutocapitalizationType(newTextInputProps.traits.autocapitalizationType);
  }

  if (newTextInputProps.traits.autoCorrect != oldTextInputProps.traits.autoCorrect) {
    _backedTextInputView.autocorrectionType =
        ABI40_0_0RCTUITextAutocorrectionTypeFromOptionalBool(newTextInputProps.traits.autoCorrect);
  }
  ...
  if (newTextInputProps.traits.keyboardAppearance != oldTextInputProps.traits.keyboardAppearance) {
    _backedTextInputView.keyboardAppearance =
        ABI40_0_0RCTUIKeyboardAppearanceFromKeyboardAppearance(newTextInputProps.traits.keyboardAppearance);
  }

  if (newTextInputProps.traits.spellCheck != oldTextInputProps.traits.spellCheck) {
    _backedTextInputView.spellCheckingType =
        ABI40_0_0RCTUITextSpellCheckingTypeFromOptionalBool(newTextInputProps.traits.spellCheck);
  }
  ...
  if (newTextInputProps.traits.clearButtonMode != oldTextInputProps.traits.clearButtonMode) {
    _backedTextInputView.clearButtonMode =
        ABI40_0_0RCTUITextFieldViewModeFromTextInputAccessoryVisibilityMode(newTextInputProps.traits.clearButtonMode);
  }
  ...
  if (newTextInputProps.traits.keyboardType != oldTextInputProps.traits.keyboardType) {
    _backedTextInputView.keyboardType = ABI40_0_0RCTUIKeyboardTypeFromKeyboardType(newTextInputProps.traits.keyboardType);
  }

  if (newTextInputProps.traits.returnKeyType != oldTextInputProps.traits.returnKeyType) {
    _backedTextInputView.returnKeyType = ABI40_0_0RCTUIReturnKeyTypeFromReturnKeyType(newTextInputProps.traits.returnKeyType);
  }

  if (newTextInputProps.traits.textContentType != oldTextInputProps.traits.textContentType) {
    if (@available(iOS 10.0, *)) {
      _backedTextInputView.textContentType = ABI40_0_0RCTUITextContentTypeFromString(newTextInputProps.traits.textContentType);
    }
  }
  ...
  if (newTextInputProps.traits.passwordRules != oldTextInputProps.traits.passwordRules) {
    if (@available(iOS 12.0, *)) {
      _backedTextInputView.passwordRules =
          ABI40_0_0RCTUITextInputPasswordRulesFromString(newTextInputProps.traits.passwordRules);
    }
  }
  ...
  if (newTextInputProps.placeholder != oldTextInputProps.placeholder) {
    _backedTextInputView.placeholder = ABI40_0_0RCTNSStringFromString(newTextInputProps.placeholder);
  }

  if (newTextInputProps.textAttributes != oldTextInputProps.textAttributes) {
    _backedTextInputView.defaultTextAttributes =
        ABI40_0_0RCTNSTextAttributesFromTextAttributes(newTextInputProps.getEffectiveTextAttributes());
  }

  if (newTextInputProps.selectionColor != oldTextInputProps.selectionColor) {
    _backedTextInputView.tintColor = ABI40_0_0RCTUIColorFromSharedColor(newTextInputProps.selectionColor);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  if (_mostRecentEventCount == _state->getData().mostRecentEventCount) {
    ...
    [self _setAttributedString:ABI40_0_0RCTNSAttributedStringFromAttributedStringBox(data.attributedStringBox)];
  }
}
           ...
           oldLayoutMetrics:(LayoutMetrics const &)oldLayoutMetrics
{
  ...
  _backedTextInputView.frame =
      UIEdgeInsetsInsetRect(self.bounds, ABI40_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.borderWidth));
  _backedTextInputView.textContainerInset =
      ABI40_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.contentInsets - layoutMetrics.borderWidth);
}
...
- (void)_setMultiline:(BOOL)multiline
{
  ...
  UIView<ABI40_0_0RCTBackedTextInputViewProtocol> *backedTextInputView =
      multiline ? [[ABI40_0_0RCTUITextView alloc] init] : [[ABI40_0_0RCTUITextField alloc] init];
  backedTextInputView.frame = _backedTextInputView.frame;
  ABI40_0_0RCTCopyBackedTextInput(_backedTextInputView, backedTextInputView);
  _backedTextInputView = backedTextInputView;
  [self addSubview:_backedTextInputView];
}

#pragma mark - ABI40_0_0RCTBackedTextInputDelegate
...
- (NSString *)textInputShouldChangeText:(NSString *)text inRange:(NSRange)range
{
  if (!_backedTextInputView.textWasPasted) {
    if (_eventEmitter) {
      ...
      keyPressMetrics.text = ABI40_0_0RCTStringFromNSString(text);
...
- (TextInputMetrics)_textInputMetrics
{
  ...
  metrics.text = ABI40_0_0RCTStringFromNSString(_backedTextInputView.attributedText.string);
...
- (void)_updateState
{
  ...
  auto data = _state->getData();
  _lastStringStateWasUpdatedWith = attributedString;
  data.attributedStringBox = ABI40_0_0RCTAttributedStringBoxFromNSAttributedString(attributedString);
...
{
  ABI40_0_0RCTTextInputHandleCommand(self, commandName, args);
}
