...
#import <ABI40_0_0React/components/legacyviewmanagerinterop/LegacyViewManagerInteropComponentDescriptor.h>
#import <ABI40_0_0React/components/legacyviewmanagerinterop/LegacyViewManagerInteropViewProps.h>
...
@implementation ABI40_0_0RCTLegacyViewManagerInteropComponentView {
  NSMutableDictionary<NSNumber *, UIView *> *_viewsToBeMounted;
  NSMutableArray<UIView *> *_viewsToBeUnmounted;
  ABI40_0_0RCTLegacyViewManagerInteropCoordinatorAdapter *_adapter;
...
{
  return [[ABI40_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] containsObject:componentName];
}

+ (void)supportLegacyViewManagerWithName:(NSString *)componentName
{
  [[ABI40_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] addObject:componentName];
}

- (ABI40_0_0RCTLegacyViewManagerInteropCoordinator *)coordinator
{
  if (_state != nullptr) {
...
{
  const auto &state = _state->getData();
  ABI40_0_0RCTLegacyViewManagerInteropCoordinator *coordinator = unwrapManagedObject(state.coordinator);
...
- (void)finalizeUpdates:(ABI40_0_0RNComponentViewUpdateMask)updateMask
{
  ...
  if (!_adapter) {
    _adapter = [[ABI40_0_0RCTLegacyViewManagerInteropCoordinatorAdapter alloc] initWithCoordinator:self.coordinator
                                                                                 ABI40_0_0ReactTag:self.tag];
  ...
  for (NSNumber *key in _viewsToBeMounted) {
    [_adapter.paperView insertABI40_0_0ReactSubview:_viewsToBeMounted[key] atIndex:key.integerValue];
  }

  [_viewsToBeMounted removeAllObjects];

  for (UIView *view in _viewsToBeUnmounted) {
    [_adapter.paperView removeABI40_0_0ReactSubview:view];
  }

  [_viewsToBeUnmounted removeAllObjects];

  [_adapter.paperView didUpdateABI40_0_0ReactSubviews];

  if (updateMask & ABI40_0_0RNComponentViewUpdateMaskProps) {
