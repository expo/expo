...
@interface ABI40_0_0RCTUIManager (Hack)
...
@implementation ABI40_0_0RCTUIManager (Hack)
...
+ (void)registerView:(UIView *)view
{
  ...
  ABI40_0_0RCTUIManager *uiManager = [[ABI40_0_0RCTBridge currentBridge] uiManager];
  view.ABI40_0_0ReactTag = @(view.tag);
...
const NSInteger ABI40_0_0RCTComponentViewRegistryRecyclePoolMaxSize = 1024;

@implementation ABI40_0_0RCTComponentViewRegistry {
  better::map<Tag, ABI40_0_0RCTComponentViewDescriptor> _registry;
  better::map<ComponentHandle, std::vector<ABI40_0_0RCTComponentViewDescriptor>> _recyclePool;
}

- (instancetype)init
{
  if (self = [super init]) {
    _componentViewFactory = [ABI40_0_0RCTComponentViewFactory standardComponentViewFactory];
...
- (void)preallocateViewComponents
{
  ...
  std::vector<std::pair<ComponentHandle, float>> componentMagnitudes = {
      {[ABI40_0_0RCTViewComponentView componentDescriptorProvider].handle, 1},
      {[ABI40_0_0RCTImageComponentView componentDescriptorProvider].handle, 0.3},
      {[ABI40_0_0RCTParagraphComponentView componentDescriptorProvider].handle, 0.3},
  };
...
- (ABI40_0_0RCTComponentViewDescriptor)dequeueComponentViewWithComponentHandle:(ComponentHandle)componentHandle tag:(Tag)tag
{
  ABI40_0_0RCTAssertMainQueue();

  ABI40_0_0RCTAssert(
      _registry.find(tag) == _registry.end(),
      @"ABI40_0_0RCTComponentViewRegistry: Attempt to dequeue already registered component.");
...
#ifdef LEGACY_UIMANAGER_INTEGRATION_ENABLED
  [ABI40_0_0RCTUIManager registerView:componentViewDescriptor.view];
#endif
                        ...
                        componentViewDescriptor:(ABI40_0_0RCTComponentViewDescriptor)componentViewDescriptor
{
  ...
  ABI40_0_0RCTAssert(
      _registry.find(tag) != _registry.end(), @"ABI40_0_0RCTComponentViewRegistry: Attempt to enqueue unregistered component.");

#ifdef LEGACY_UIMANAGER_INTEGRATION_ENABLED
  [ABI40_0_0RCTUIManager unregisterView:componentViewDescriptor.view];
#endif
...
- (ABI40_0_0RCTComponentViewDescriptor const &)componentViewDescriptorWithTag:(Tag)tag
{
  ABI40_0_0RCTAssertMainQueue();
  auto iterator = _registry.find(tag);
  ABI40_0_0RCTAssert(iterator != _registry.end(), @"ABI40_0_0RCTComponentViewRegistry: Attempt to query unregistered component.");
  return iterator->second;
}

- (nullable UIView<ABI40_0_0RCTComponentViewProtocol> *)findComponentViewWithTag:(Tag)tag
{
  ABI40_0_0RCTAssertMainQueue();
  auto iterator = _registry.find(tag);
...
- (ABI40_0_0RCTComponentViewDescriptor)_dequeueComponentViewWithComponentHandle:(ComponentHandle)componentHandle
{
  ABI40_0_0RCTAssertMainQueue();
                         ...
                         componentViewDescriptor:(ABI40_0_0RCTComponentViewDescriptor)componentViewDescriptor
{
  ...
  if (recycledViews.size() > ABI40_0_0RCTComponentViewRegistryRecyclePoolMaxSize) {
    return;
  }
