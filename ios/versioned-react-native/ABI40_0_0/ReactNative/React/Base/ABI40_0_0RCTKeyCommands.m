...
@interface ABI40_0_0RCTKeyCommand : NSObject <NSCopying>
...
@implementation ABI40_0_0RCTKeyCommand
...
- (BOOL)isEqual:(ABI40_0_0RCTKeyCommand *)object
{
  if (![object isKindOfClass:[ABI40_0_0RCTKeyCommand class]]) {
    return NO;
  }
...
@interface ABI40_0_0RCTKeyCommands ()

@property (nonatomic, strong) NSMutableSet<ABI40_0_0RCTKeyCommand *> *commands;

@end

@implementation UIResponder (ABI40_0_0RCTKeyCommands)

+ (UIResponder *)ABI40_0_0RCT_getFirstResponder:(UIResponder *)view
{
  ...
  if (view.isFirstResponder) {
    ...
    if ([(UIViewController *)view parentViewController]) {
      firstResponder = [UIResponder ABI40_0_0RCT_getFirstResponder:[(UIViewController *)view parentViewController]];
    }
    return firstResponder ? firstResponder : [UIResponder ABI40_0_0RCT_getFirstResponder:[(UIViewController *)view view]];
  } else if ([view isKindOfClass:[UIView class]]) {
    for (UIView *subview in [(UIView *)view subviews]) {
      firstResponder = [UIResponder ABI40_0_0RCT_getFirstResponder:subview];
      if (firstResponder) {
...
- (NSArray<UIKeyCommand *> *)ABI40_0_0RCT_keyCommands
{
  NSSet<ABI40_0_0RCTKeyCommand *> *commands = [ABI40_0_0RCTKeyCommands sharedInstance].commands;
 ...
 */
- (void)ABI40_0_0RCT_handleKeyCommand:(UIKeyCommand *)key
{
  // NOTE: throttle the key handler because on iOS 9 the handleKeyCommand:
  // method gets called repeatedly if the command key is held down.
  static NSTimeInterval lastCommand = 0;
  if (CACurrentMediaTime() - lastCommand > 0.5) {
    for (ABI40_0_0RCTKeyCommand *command in [ABI40_0_0RCTKeyCommands sharedInstance].commands) {
 ...
 */
- (void)ABI40_0_0RCT_handleDoublePressKeyCommand:(UIKeyCommand *)key
{
...
@implementation ABI40_0_0RCTKeyCommands

+ (instancetype)sharedInstance
{
  static ABI40_0_0RCTKeyCommands *sharedInstance;
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  UIKeyCommand *command = [UIKeyCommand keyCommandWithInput:input
                                              modifierFlags:flags
                                                     action:@selector(ABI40_0_0RCT_handleKeyCommand:)];

  ABI40_0_0RCTKeyCommand *keyCommand = [[ABI40_0_0RCTKeyCommand alloc] initWithKeyCommand:command block:block];
...
- (void)unregisterKeyCommandWithInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI40_0_0RCTKeyCommand *command in _commands.allObjects) {
...
- (BOOL)isKeyCommandRegisteredForInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI40_0_0RCTKeyCommand *command in _commands) {
    if ([command matchesInput:input flags:flags]) {
      return YES;
    }
  }
                                        ...
                                        action:(void (^)(UIKeyCommand *))block
{
  ...
  UIKeyCommand *command = [UIKeyCommand keyCommandWithInput:input
                                              modifierFlags:flags
                                                     action:@selector(ABI40_0_0RCT_handleDoublePressKeyCommand:)];
