...
#import "ABI40_0_0RCTJSStackFrame.h"
 ...
 */
static NSRegularExpression *ABI40_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI40_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI40_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI40_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI40_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI40_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI40_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI40_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI40_0_0RCTNilIfNull(dict[@"collapse"]) integerValue]];
}
...
+ (NSArray<ABI40_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI40_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI40_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI40_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
