...
#import "ABI40_0_0RCTComponentEvent.h"
#import "ABI40_0_0RCTProfile.h"
#import "ABI40_0_0RCTUtils.h"

const NSInteger ABI40_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI40_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI40_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI40_0_0RCTUniqueCoalescingKeyGenerator = 0;

@implementation ABI40_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI40_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI40_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI40_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI40_0_0RCTComponentEvent *event = [[ABI40_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI40_0_0ReactTag body:body];
...
- (void)sendEvent:(id<ABI40_0_0RCTEvent>)event
{
  ...
  for (id<ABI40_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI40_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI40_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI40_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI40_0_0RCTGetEventID(event.viewTag, event.eventName, ABI40_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI40_0_0RCTAssert(
        previousEvent == nil,
...
@implementation ABI40_0_0RCTBridge (ABI40_0_0RCTEventDispatcher)

- (ABI40_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI40_0_0RCTEventDispatcher class]];
}
