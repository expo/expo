...
@implementation ABI40_0_0RCTTVView {
          ...
          [UIView animateWithDuration:(pressDuration / 2)
              ...
              completion:^(__unused BOOL finished2) {
                [[NSNotificationCenter defaultCenter]
                    postNotificationName:ABI40_0_0RCTTVNavigationEventNotification
                                  object:@{@"eventType" : @"select", @"tag" : self.ABI40_0_0ReactTag}];
              }];
        }];

  } else {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI40_0_0RCTTVNavigationEventNotification
                                                        object:@{@"eventType" : @"select", @"tag" : self.ABI40_0_0ReactTag}];
  }
}
  ...
  if (context.nextFocusedView == self && self.isTVSelectable) {
    ...
    [coordinator
        addCoordinatedAnimations:^(void) {
          ...
          [[NSNotificationCenter defaultCenter]
              postNotificationName:ABI40_0_0RCTTVNavigationEventNotification
                            object:@{@"eventType" : @"focus", @"tag" : self.ABI40_0_0ReactTag}];
        }
                      completion:^(void){
                      }];
  } else {
    [coordinator
        addCoordinatedAnimations:^(void) {
          [[NSNotificationCenter defaultCenter] postNotificationName:ABI40_0_0RCTTVNavigationEventNotification
                                                              object:@{@"eventType" : @"blur", @"tag" : self.ABI40_0_0ReactTag}];
          [UIView animateWithDuration:0.2
                           animations:^{
...
- (void)setHasTVPreferredFocus:(BOOL)hasTVPreferredFocus
{
  ...
  if (hasTVPreferredFocus) {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      ...
      while (![rootview isABI40_0_0ReactRootView] && rootview != nil) {
