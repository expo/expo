...
#import "ABI40_0_0RCTScrollEvent.h"
...
#import "ABI40_0_0UIView+Private.h"
 ...
 */
@interface ABI40_0_0RCTCustomScrollView : UIScrollView <UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI40_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
@property (nonatomic, assign) BOOL pinchGestureEnabled;
#endif

@end

@implementation ABI40_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI40_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI40_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI40_0_0RCTUIManager JSResponder] isKindOfClass:[ABI40_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI40_0_0React scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
      ABI40_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
      ABI40_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI40_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI40_0_0RCTScrollView () <ABI40_0_0RCTUIManagerObserver>

@end

@implementation ABI40_0_0RCTScrollView {
  ABI40_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI40_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
{
  ABI40_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI40_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI40_0_0RCTApplyTransformationAccordingLayoutDirection(
    UIView *view,
...
- (void)setABI40_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  [super setABI40_0_0ReactLayoutDirection:layoutDirection];

  ABI40_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI40_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI40_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI40_0_0ReactSubview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI40_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI40_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI40_0_0RCTAssert(
        _contentView == nil,
        @"ABI40_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@",
        [_contentView ABI40_0_0React_recursiveDescription]);
    _contentView = view;
    ABI40_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI40_0_0ReactLayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI40_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI40_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI40_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI40_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI40_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI40_0_0React_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ABI40_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI40_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:NO];
...
{
  [ABI40_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:YES];
}

#pragma mark - ScrollView delegate

#define ABI40_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData)                                    \
  {                                                                                     \
...
#define ABI40_0_0RCT_FORWARD_SCROLL_EVENT(call)                                            \
...
#define ABI40_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
  -(void)delegateMethod : (UIScrollView *)scrollView        \
  {                                                         \
    ABI40_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                  \
    ABI40_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod : scrollView);  \
  }

ABI40_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI40_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI40_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI40_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames" : [self calculateChildFramesData]}));
    } else {
      ABI40_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll : scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
  NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
  [[_contentView ABI40_0_0ReactSubviews] enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, __unused BOOL *stop) {
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI40_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging : scrollView);
}
              ...
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI40_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging
...
{
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging : scrollView willDecelerate : decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI40_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming : scrollView withView : view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI40_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming : scrollView withView : view atScale : scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI40_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating : scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI40_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI40_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation : scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI40_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI40_0_0RCTUIManager *)manager
{
  ...
  [manager
      prependUIBlock:^(__unused ABI40_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI40_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI40_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI40_0_0RCTScrollEvent *scrollEvent = [[ABI40_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ABI40_0_0ReactTag:self.ABI40_0_0ReactTag
...
@implementation ABI40_0_0RCTEventDispatcher (ABI40_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ABI40_0_0ReactTag
{
  ...
  ABI40_0_0RCTScrollEvent *fakeScrollEvent = [[ABI40_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI40_0_0ReactTag:ABI40_0_0ReactTag
