...
#import <ABI40_0_0React/ABI40_0_0RCTDevLoadingView.h>
...
#import <ABI40_0_0React/ABI40_0_0RCTDevLoadingViewSetEnabled.h>
#import <ABI40_0_0React/ABI40_0_0RCTModalHostViewController.h>
...
@interface ABI40_0_0RCTDevLoadingView () <ABI40_0_0NativeDevLoadingViewSpec>
@end

#if ABI40_0_0RCT_DEV | ABI40_0_0RCT_ENABLE_LOADING_VIEW

@implementation ABI40_0_0RCTDevLoadingView {
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
}
...
{
  ABI40_0_0RCTDevLoadingViewSetEnabled(enabled);
}
...
- (void)setBridge:(ABI40_0_0RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(hide)
                                               name:ABI40_0_0RCTJavaScriptDidFailToLoadNotification
                                             object:nil];
...
{
  if (!ABI40_0_0RCTDevLoadingViewGetEnabled()) {
    return;
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    self->_showDate = [NSDate date];
    if (!self->_window && !ABI40_0_0RCTRunningInTestEnvironment()) {
      CGSize screenSize = [UIScreen mainScreen].bounds.size;

      if (@available(iOS 11.0, *)) {
        UIWindow *window = ABI40_0_0RCTSharedApplication().keyWindow;
        self->_window =
    ...
    if (@available(iOS 13.0, *)) {
      UIWindowScene *scene = (UIWindowScene *)ABI40_0_0RCTSharedApplication().connectedScenes.anyObject;
...
ABI40_0_0RCT_EXPORT_METHOD(showMessage
                  : (NSString *)message withColor
                  : (NSNumber *__nonnull)color withBackgroundColor
                  : (NSNumber *__nonnull)backgroundColor)
{
  [self showMessage:message color:[ABI40_0_0RCTConvert UIColor:color] backgroundColor:[ABI40_0_0RCTConvert UIColor:backgroundColor]];
}
...
- (void)showWithURL:(NSURL *)URL
{
  ...
  if (URL.fileURL) {
    ...
    message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI40_0_0RCT_PACKAGER_NAME];
  } else {
    color = [UIColor whiteColor];
...
- (void)updateProgress:(ABI40_0_0RCTLoadingProgress *)progress
{
  if (!progress) {
    return;
  }
...
Class ABI40_0_0RCTDevLoadingViewCls(void)
{
  return ABI40_0_0RCTDevLoadingView.class;
}
