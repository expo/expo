...
#import "ABI40_0_0RCTCameraRollManager.h"
...
#import "ABI40_0_0RCTAssetsLibraryRequestHandler.h"

@implementation ABI40_0_0RCTConvert (PHAssetCollectionSubtype)

ABI40_0_0RCT_ENUM_CONVERTER(PHAssetCollectionSubtype, (@{
...
@implementation ABI40_0_0RCTConvert (PHFetchOptions)
...
+ (PHFetchOptions *)PHFetchOptionsFromMediaType:(NSString *)mediaType
{
  ...
  if ([lowercase isEqualToString:@"photos"]) {
    ...
    if (![lowercase isEqualToString:@"all"]) {
      ABI40_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
...
@interface ABI40_0_0RCTCameraRollManager() <ABI40_0_0NativeCameraRollManagerSpec>
@end

@implementation ABI40_0_0RCTCameraRollManager
...
static void requestPhotoLibraryAccess(ABI40_0_0RCTPromiseRejectBlock reject, PhotosAuthorizedBlock authorizedBlock) {
...
ABI40_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
                  ...
                  reject:(ABI40_0_0RCTPromiseRejectBlock)reject)
{
  ...
  void (^loadBlock)(void) = ^void() {
    if ([type isEqualToString:@"video"]) {
      ...
      [[self.bridge moduleForClass:[ABI40_0_0RCTImageLoader class]] loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
        if (error) {
...
static void ABI40_0_0RCTResolvePromise(ABI40_0_0RCTPromiseResolveBlock resolve,
...
ABI40_0_0RCT_EXPORT_METHOD(getPhotos:(JS::NativeCameraRollManager::GetPhotosParams &)params
                  resolve:(ABI40_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI40_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger const first = [ABI40_0_0RCTConvert NSInteger:[NSNumber numberWithDouble:params.first()]];
  NSString *const afterCursor = [ABI40_0_0RCTConvert NSString:params.after()];
  NSString *const groupName = [ABI40_0_0RCTConvert NSString:params.groupName()];
  NSString *const groupTypes = [[ABI40_0_0RCTConvert NSString:params.groupTypes()] lowercaseString];
  NSString *const mediaType = [ABI40_0_0RCTConvert NSString:params.assetType()];
  NSArray<NSString *> *const mimeTypes = [ABI40_0_0RCTConvert NSStringArray:ABI40_0_0RCTConvertOptionalVecToArray(params.mimeTypes())];
  ...
  PHAssetCollectionSubtype const collectionSubtype = [ABI40_0_0RCTConvert PHAssetCollectionSubtype:groupTypes];

  // Predicate for fetching assets within a collection
  PHFetchOptions *const assetFetchOptions = [ABI40_0_0RCTConvert PHFetchOptionsFromMediaType:mediaType];
    ...
    [assetCollectionFetchResult enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull assetCollection, NSUInteger collectionIdx, BOOL * _Nonnull stopCollections) {
      ...
      [assetsFetchResult enumerateObjectsUsingBlock:^(PHAsset * _Nonnull asset, NSUInteger assetIdx, BOOL * _Nonnull stopAssets) {
        ...
        if (first == assets.count) {
          ...
          *stopCollections = YES;
          hasNextPage = YES;
          ABI40_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
          ABI40_0_0RCTResolvePromise(resolve, assets, hasNextPage);
          resolvedPromise = YES;
          return;
        }
...
ABI40_0_0RCT_EXPORT_METHOD(deletePhotos:(NSArray<NSString *>*)assets
                  resolve:(ABI40_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI40_0_0RCTPromiseRejectBlock)reject)
{
  NSArray<NSURL *> *assets_ = [ABI40_0_0RCTConvert NSURLArray:assets];
  [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI40_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
...
{
  return std::make_shared<ABI40_0_0facebook::ABI40_0_0React::NativeCameraRollManagerSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

Class ABI40_0_0RCTCameraRollManagerCls(void) {
  return ABI40_0_0RCTCameraRollManager.class;
}
