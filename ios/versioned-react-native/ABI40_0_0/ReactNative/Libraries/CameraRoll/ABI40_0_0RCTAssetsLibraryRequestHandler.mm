...
@interface ABI40_0_0RCTAssetsLibraryRequestHandler() <ABI40_0_0RCTTurboModule>
@end

@implementation ABI40_0_0RCTAssetsLibraryRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  return [request.URL.scheme caseInsensitiveCompare:@"assets-library"] == NSOrderedSame
    ...
    || [request.URL.scheme caseInsensitiveCompare:ABI40_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
}
     ...
     withDelegate:(id<ABI40_0_0RCTURLRequestDelegate>)delegate
{
  ...
  BOOL isPHUpload = [requestURL.scheme caseInsensitiveCompare:ABI40_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
  if (isPHUpload) {
    requestURL = [NSURL URLWithString:[@"ph" stringByAppendingString:[requestURL.absoluteString substringFromIndex:ABI40_0_0RCTNetworkingPHUploadHackScheme.length]]];
  }

  if (!requestURL) {
    NSString *const msg = [NSString stringWithFormat:@"Cannot send request without URL"];
    [delegate URLRequest:cancellationBlock didCompleteWithError:ABI40_0_0RCTErrorWithMessage(msg)];
  ...
  if (![fetchResult firstObject]) {
    ...
    NSError *error = ABI40_0_0RCTErrorWithMessage(errorMessage);
  ...
  if (asset.mediaType == PHAssetMediaTypeVideo && isPHUpload) {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:requestOptions resultHandler:^(AVAsset * _Nullable avAsset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
      ...
      if (![avAsset isKindOfClass:[AVURLAsset class]]) {
        error = [NSError errorWithDomain:ABI40_0_0RCTErrorDomain code:0 userInfo:
        @{
...
Class ABI40_0_0RCTAssetsLibraryRequestHandlerCls(void) {
  return ABI40_0_0RCTAssetsLibraryRequestHandler.class;
}
