...
@implementation ABI40_0_0RCTPropsAnimatedNode
{
               ...
               bridge:(ABI40_0_0RCTBridge *)bridge
{
  ...
  _managedByFabric = ABI40_0_0RCTUIManagerTypeForTagIsFabric(viewTag);
  _rootTag = nil;
}
...
- (void)performUpdate
{
  ...
  for (NSNumber *parentTag in self.parentNodes.keyEnumerator) {
    ABI40_0_0RCTAnimatedNode *parentNode = [self.parentNodes objectForKey:parentTag];
    if ([parentNode isKindOfClass:[ABI40_0_0RCTStyleAnimatedNode class]]) {
      [self->_propsDictionary addEntriesFromDictionary:[(ABI40_0_0RCTStyleAnimatedNode *)parentNode propsDictionary]];

    } else if ([parentNode isKindOfClass:[ABI40_0_0RCTValueAnimatedNode class]]) {
      NSString *property = [self propertyNameForParentTag:parentTag];
      id animatedObject = [(ABI40_0_0RCTValueAnimatedNode *)parentNode animatedObject];
      if (animatedObject) {
        self->_propsDictionary[property] = animatedObject;
      } else {
        CGFloat value = [(ABI40_0_0RCTValueAnimatedNode *)parentNode value];
