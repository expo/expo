...
static const NSUInteger ABI40_0_0RCTMaxCachableDecodedImageSizeInBytes = 2097152; // 2 MB

static NSString *ABI40_0_0RCTCacheKeyForImage(NSString *imageTag, CGSize size, CGFloat scale,
                                     ABI40_0_0RCTResizeMode resizeMode)
{
  return [NSString stringWithFormat:@"%@|%g|%g|%g|%lld",
          imageTag, size.width, size.height, scale, (long long)resizeMode];
}

@implementation ABI40_0_0RCTImageCache
{
                 ...
                 forKey:(NSString *)cacheKey
{
  ...
  NSInteger bytes = image.ABI40_0_0ReactDecodedImageBytes;
  if (bytes <= ABI40_0_0RCTMaxCachableDecodedImageSizeInBytes) {
...
{
  NSString *cacheKey = ABI40_0_0RCTCacheKeyForImage(url, size, scale, resizeMode);
  @synchronized(_cacheStaleTimes) {
