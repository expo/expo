...
@implementation ABI40_0_0RCTSurfaceHostingComponent

+ (Class<CKComponentControllerProtocol>)controllerClass
{
  return [ABI40_0_0RCTSurfaceHostingComponentController class];
}

+ (id)initialState
{
  return [ABI40_0_0RCTSurfaceHostingComponentState new];
}

+ (instancetype)newWithSurface:(ABI40_0_0RCTSurface *)surface options:(ABI40_0_0RCTSurfaceHostingComponentOptions)options
{
  CKComponentScope scope(self, surface);

  ABI40_0_0RCTSurfaceHostingComponentState *const state = scope.state();

  ABI40_0_0RCTSurfaceHostingComponentState *const newState =
    [ABI40_0_0RCTSurfaceHostingComponentState newWithStage:surface.stage
  ...
  ABI40_0_0RCTSurfaceHostingComponent *const component =
...
- (CKComponentLayout)computeLayoutThatFits:(CKSizeRange)constrainedSize
{
  ...
  // just to provide layout constraints to ABI40_0_0React Native as early as possible.
  // ABI40_0_0React Native *may* use this info later during applying the own state and
  ...
  // interop compatibilities of ABI40_0_0React Native which will enable us granularly
  // control ABI40_0_0React Native mounting blocks and, as a result, implement
  // truly synchronous mounting stage between ABI40_0_0React Native and ComponentKit.
  [_surface setMinimumSize:constrainedSize.min
               maximumSize:constrainedSize.max];

  // Just in case of the very first building pass, we give ABI40_0_0React Native a chance
  // to prepare its internals for coming synchronous measuring.
  [_surface synchronouslyWaitForStage:ABI40_0_0RCTSurfaceStageSurfaceDidInitialLayout
                              timeout:_options.synchronousLayoutingTimeout];

  CGSize fittingSize = CGSizeZero;
  if (_surface.stage & ABI40_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
...
- (CKComponentBoundsAnimation)boundsAnimationFromPreviousComponent:(ABI40_0_0RCTSurfaceHostingComponent *)previousComponent
{
