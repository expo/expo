...
#include <ABI40_0_0React/utils/Telemetry.h>

using namespace ABI40_0_0facebook::ABI40_0_0React;

#define ABI40_0_0EXPECT_EQ_WITH_THRESHOLD(a, b, threshold) \
  ABI40_0_0EXPECT_TRUE((a >= b - threshold) && (a <= b + threshold))
...
TEST(MountingTelemetryTest, timepoints) {
  ...
  ABI40_0_0EXPECT_EQ_WITH_THRESHOLD(duration, 100, threshold);
}
...
TEST(MountingTelemetryTest, normalUseCase) {
  ...
  ABI40_0_0EXPECT_EQ_WITH_THRESHOLD(commitDuration, 400, threshold * 2);
  ABI40_0_0EXPECT_EQ_WITH_THRESHOLD(layoutDuration, 200, threshold);
  ABI40_0_0EXPECT_EQ_WITH_THRESHOLD(mountDuration, 100, threshold);
}
