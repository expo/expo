...
#include <ABI40_0_0React/textlayoutmanager/ABI40_0_0RCTTextPrimitivesConversions.h>
#include <ABI40_0_0React/utils/ManagedObjectWrapper.h>

using namespace ABI40_0_0facebook::ABI40_0_0React;

@implementation ABI40_0_0RCTWeakEventEmitterWrapper {
...
inline static UIFontWeight ABI40_0_0RCTUIFontWeightFromInteger(NSInteger fontWeight)
{
...
inline static UIFont *ABI40_0_0RCTEffectiveFontFromTextAttributes(const TextAttributes &textAttributes)
{
  ...
  fontProperties.style = textAttributes.fontStyle.hasValue()
      ? ABI40_0_0RCTFontStyleFromFontStyle(textAttributes.fontStyle.value())
      : ABI40_0_0RCTFontStyleUndefined;
  fontProperties.variant = textAttributes.fontVariant.hasValue()
      ? ABI40_0_0RCTFontVariantFromFontVariant(textAttributes.fontVariant.value())
      : ABI40_0_0RCTFontVariantUndefined;
  fontProperties.weight = textAttributes.fontWeight.hasValue()
      ? ABI40_0_0RCTUIFontWeightFromInteger((NSInteger)textAttributes.fontWeight.value())
      : NAN;
  fontProperties.sizeMultiplier = textAttributes.fontSizeMultiplier;

  return ABI40_0_0RCTFontWithFontProperties(fontProperties);
}

inline static CGFloat ABI40_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(const TextAttributes &textAttributes)
{
...
inline static UIColor *ABI40_0_0RCTEffectiveForegroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveForegroundColor = ABI40_0_0RCTUIColorFromSharedColor(textAttributes.foregroundColor) ?: [UIColor blackColor];
...
inline static UIColor *ABI40_0_0RCTEffectiveBackgroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveBackgroundColor = ABI40_0_0RCTUIColorFromSharedColor(textAttributes.backgroundColor);
...
NSDictionary<NSAttributedStringKey, id> *ABI40_0_0RCTNSTextAttributesFromTextAttributes(TextAttributes const &textAttributes)
{
  NSMutableDictionary<NSAttributedStringKey, id> *attributes = [NSMutableDictionary dictionaryWithCapacity:10];

  // Font
  UIFont *font = ABI40_0_0RCTEffectiveFontFromTextAttributes(textAttributes);
  if (font) {
    attributes[NSFontAttributeName] = font;
  }

  // Colors
  UIColor *effectiveForegroundColor = ABI40_0_0RCTEffectiveForegroundColorFromTextAttributes(textAttributes);
  ...
  if (textAttributes.backgroundColor || !isnan(textAttributes.opacity)) {
    attributes[NSBackgroundColorAttributeName] = ABI40_0_0RCTEffectiveBackgroundColorFromTextAttributes(textAttributes);
  }
  ...
  if (textAttributes.alignment.hasValue()) {
    ...
    paragraphStyle.alignment = ABI40_0_0RCTNSTextAlignmentFromTextAlignment(textAlignment);
    isParagraphStyleUsed = YES;
  }

  if (textAttributes.baseWritingDirection.hasValue()) {
    paragraphStyle.baseWritingDirection =
        ABI40_0_0RCTNSWritingDirectionFromWritingDirection(textAttributes.baseWritingDirection.value());
    isParagraphStyleUsed = YES;
  }

  if (!isnan(textAttributes.lineHeight)) {
    CGFloat lineHeight = textAttributes.lineHeight * ABI40_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(textAttributes);
  ...
  if (textAttributes.textDecorationLineType.value_or(TextDecorationLineType::None) != TextDecorationLineType::None) {
    ...
    NSUnderlineStyle style = ABI40_0_0RCTNSUnderlineStyleFromStyleAndPattern(
    ...
    UIColor *textDecorationColor = ABI40_0_0RCTUIColorFromSharedColor(textAttributes.textDecorationColor);
  ...
  if (textAttributes.textShadowOffset.hasValue()) {
    ...
    shadow.shadowColor = ABI40_0_0RCTUIColorFromSharedColor(textAttributes.textShadowColor);
    attributes[NSShadowAttributeName] = shadow;
  }

  // Special
  if (textAttributes.isHighlighted) {
    attributes[ABI40_0_0RCTAttributedStringIsHighlightedAttributeName] = @YES;
  }

  return [attributes copy];
}

NSAttributedString *ABI40_0_0RCTNSAttributedStringFromAttributedString(const AttributedString &attributedString)
{
    ...
    if (fragment.isAttachment()) {
      ...
      nsAttributedStringFragment = [[NSMutableAttributedString alloc]
          initWithString:string
              attributes:ABI40_0_0RCTNSTextAttributesFromTextAttributes(fragment.textAttributes)];
    }

    if (fragment.parentShadowView.componentHandle) {
      ABI40_0_0RCTWeakEventEmitterWrapper *eventEmitterWrapper = [ABI40_0_0RCTWeakEventEmitterWrapper new];
      eventEmitterWrapper.eventEmitter = fragment.parentShadowView.eventEmitter;

      NSDictionary<NSAttributedStringKey, id> *additionalTextAttributes =
          @{ABI40_0_0RCTAttributedStringEventEmitterKey : eventEmitterWrapper};
...
NSAttributedString *ABI40_0_0RCTNSAttributedStringFromAttributedStringBox(AttributedStringBox const &attributedStringBox)
{
  switch (attributedStringBox.getMode()) {
    case AttributedStringBox::Mode::Value:
      return ABI40_0_0RCTNSAttributedStringFromAttributedString(attributedStringBox.getValue());
    case AttributedStringBox::Mode::OpaquePointer:
      return (NSAttributedString *)unwrapManagedObject(attributedStringBox.getOpaquePointer());
  }
}

AttributedStringBox ABI40_0_0RCTAttributedStringBoxFromNSAttributedString(NSAttributedString *nsAttributedString)
{
