...
TEST(ShadowNodeFamilyTest, sealObjectCorrectly) {
  ...
  ABI40_0_0EXPECT_EQ(ancestors1.size(), 0);

  // Positive case:
  auto ancestors2 = shadowNodeAAA->getFamily().getAncestors(*shadowNodeA);
  ABI40_0_0EXPECT_EQ(ancestors2.size(), 2);
  ABI40_0_0EXPECT_EQ(&ancestors2[0].first.get(), shadowNodeA.get());
  ABI40_0_0EXPECT_EQ(&ancestors2[1].first.get(), shadowNodeAA.get());
}
