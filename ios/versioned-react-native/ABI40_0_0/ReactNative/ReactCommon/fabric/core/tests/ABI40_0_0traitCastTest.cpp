...
#include <ABI40_0_0React/element/Element.h>
#include <ABI40_0_0React/element/testUtils.h>
...
TEST(traitCastTest, testOne) {
  ...
  ABI40_0_0EXPECT_FALSE(traitCast<LayoutableShadowNode const *>(nullptr));
  ABI40_0_0EXPECT_FALSE(traitCast<YogaLayoutableShadowNode const *>(nullptr));

  // `ViewShadowNode` is `LayoutableShadowNode` and `YogaLayoutableShadowNode`.
  ABI40_0_0EXPECT_TRUE(traitCast<LayoutableShadowNode const *>(viewShadowNode.get()));
  ABI40_0_0EXPECT_TRUE(
      traitCast<YogaLayoutableShadowNode const *>(viewShadowNode.get()));
  ABI40_0_0EXPECT_NO_FATAL_FAILURE(
  ...
  // `YogaLayoutableShadowNode`.
  ABI40_0_0EXPECT_FALSE(traitCast<LayoutableShadowNode const *>(textShadowNode.get()));
  ABI40_0_0EXPECT_FALSE(
      traitCast<YogaLayoutableShadowNode const *>(textShadowNode.get()));
  ABI40_0_0EXPECT_DEATH_IF_SUPPORTED(
