...
TEST(SealableTest, sealObjectCorrectly) {
  Sealable obj;
  ABI40_0_0EXPECT_FALSE(obj.getSealed());

  obj.seal();
  ABI40_0_0EXPECT_TRUE(obj.getSealed());
}

TEST(SealableTest, handleAssignmentsCorrectly) {
  Sealable obj;
  Sealable other;
  ABI40_0_0EXPECT_NO_THROW(obj = other);

  // Assignment after getting sealed is not allowed.
  obj.seal();
  Sealable other2;
  ABI40_0_0EXPECT_THROW(obj = other2, std::runtime_error);
  ...
  Sealable other3(obj);
  ABI40_0_0EXPECT_FALSE(other3.getSealed());
}
