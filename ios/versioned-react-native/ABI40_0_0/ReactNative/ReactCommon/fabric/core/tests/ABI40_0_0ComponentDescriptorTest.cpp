...
TEST(ComponentDescriptorTest, createShadowNode) {
  ...
  ABI40_0_0EXPECT_EQ(descriptor->getComponentHandle(), TestShadowNode::Handle());
  ABI40_0_0EXPECT_STREQ(descriptor->getComponentName(), TestShadowNode::Name());
  ABI40_0_0EXPECT_STREQ(descriptor->getComponentName(), "Test");
  ...
  ABI40_0_0EXPECT_EQ(node->getComponentHandle(), TestShadowNode::Handle());
  ABI40_0_0EXPECT_STREQ(node->getComponentName(), TestShadowNode::Name());
  ABI40_0_0EXPECT_STREQ(node->getComponentName(), "Test");
  ABI40_0_0EXPECT_EQ(node->getTag(), 9);
  ABI40_0_0EXPECT_EQ(node->getSurfaceId(), 1);
  ABI40_0_0EXPECT_STREQ(node->getProps()->nativeId.c_str(), "abc");
}
...
TEST(ComponentDescriptorTest, cloneShadowNode) {
  ...
  ABI40_0_0EXPECT_STREQ(cloned->getComponentName(), "Test");
  ABI40_0_0EXPECT_EQ(cloned->getTag(), 9);
  ABI40_0_0EXPECT_EQ(cloned->getSurfaceId(), 1);
  ABI40_0_0EXPECT_STREQ(cloned->getProps()->nativeId.c_str(), "abc");
}
...
TEST(ComponentDescriptorTest, appendChild) {
  ...
  ABI40_0_0EXPECT_EQ(node1Children.size(), 2);
  ABI40_0_0EXPECT_EQ(node1Children.at(0), node2);
  ABI40_0_0EXPECT_EQ(node1Children.at(1), node3);
}
