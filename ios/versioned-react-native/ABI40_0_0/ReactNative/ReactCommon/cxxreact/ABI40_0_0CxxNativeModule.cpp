...
#include "ABI40_0_0CxxNativeModule.h"
...
#include "ABI40_0_0JsArgumentHelpers.h"
#include "ABI40_0_0MessageQueueThread.h"
#include "ABI40_0_0SystraceSection.h"

using ABI40_0_0facebook::xplat::module::CxxModule;
...
void CxxNativeModule::invoke(
    ...
    int callId) {
  if (ABI40_0_0ReactMethodId >= methods_.size()) {
    throw std::invalid_argument(folly::to<std::string>(
        "methodId ",
        ABI40_0_0ReactMethodId,
        " out of range [0..",
        methods_.size(),
        "]"));
  }
  ...
  const auto &method = methods_[ABI40_0_0ReactMethodId];
  ...
  messageQueueThread_->runOnQueue(
      [method, params = std::move(params), first, second, callId]() {
        ...
        } catch (const ABI40_0_0facebook::xplat::JsArgumentException &ex) {
          throw;
