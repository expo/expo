...
static NSString * const ABI40_0_0EXUpdatesErrorLogFile = @"expo-error.log";

@interface ABI40_0_0EXUpdatesAppLauncherNoDatabase ()
...
@implementation ABI40_0_0EXUpdatesAppLauncherNoDatabase

- (void)launchUpdateWithConfig:(ABI40_0_0EXUpdatesConfig *)config
{
  _launchedUpdate = [ABI40_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:config database:nil];
  if (_launchedUpdate) {
    if (_launchedUpdate.status == ABI40_0_0EXUpdatesUpdateStatusEmbedded) {
      ...
      _launchAssetUrl = [[NSBundle mainBundle] URLForResource:ABI40_0_0EXUpdatesEmbeddedBundleFilename withExtension:ABI40_0_0EXUpdatesEmbeddedBundleFileType];
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:ABI40_0_0EXUpdatesErrorLogFile] path];
}
