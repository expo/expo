...
#import <ABI40_0_0EXUpdates/ABI40_0_0EXSyncLauncherWithDatabase.h>
...
@interface ABI40_0_0EXSyncLauncherWithDatabase ()
...
@property (nonatomic, copy) ABI40_0_0EXSyncLauncherCompletionBlock completion;
...
static NSString * const ABI40_0_0EXSyncLauncherErrorDomain = @"AppLauncher";

@implementation ABI40_0_0EXSyncLauncherWithDatabase
                   ...
                   completionQueue:(dispatch_queue_t)completionQueue
{
  dispatch_async(database.databaseQueue, ^{
    NSError *error;
    NSArray<ABI40_0_0EXSyncManifest *> *launchableUpdates = [database launchableUpdatesWithConfig:config error:&error];
    dispatch_async(completionQueue, ^{
      ...
      ABI40_0_0EXSyncManifest *embeddedManifest = [ABI40_0_0EXSyncEmbeddedLoader embeddedManifestWithConfig:config database:database];
      NSMutableArray<ABI40_0_0EXSyncManifest *>*filteredLaunchableUpdates = [NSMutableArray new];
      for (ABI40_0_0EXSyncManifest *update in launchableUpdates) {
        if (update.status == ABI40_0_0EXSyncManifestStatusEmbedded) {
...
{
  NSAssert(!_completion, @"ABI40_0_0EXSyncLauncher:launchUpdateWithSelectionPolicy:successBlock should not be called twice on the same instance");
  _completion = completion;

  if (!_launchedUpdate) {
    [[self class] launchableUpdateWithConfig:_config database:_database selectionPolicy:selectionPolicy completion:^(NSError * _Nullable error, ABI40_0_0EXSyncManifest * _Nullable launchableUpdate) {
      if (error || !launchableUpdate) {
        if (self->_completion) {
          dispatch_async(self->_completionQueue, ^{
            ...
            self->_completion([NSError errorWithDomain:ABI40_0_0EXSyncLauncherErrorDomain code:1011 userInfo:userInfo], NO);
          });
        }
      } else {
...
- (void)_ensureAllAssetsExist
{
  if (_launchedUpdate.status == ABI40_0_0EXSyncManifestStatusEmbedded) {
    ...
    return;
  } else if (_launchedUpdate.status == ABI40_0_0EXSyncManifestStatusDevelopment) {
    dispatch_async(self->_completionQueue, ^{
...
- (void)_ensureAssetExists:(ABI40_0_0EXSyncAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
  [self _checkExistenceOfAsset:asset withLocalUrl:assetLocalUrl completion:^(BOOL exists) {
    ...
    [self _maybeCopyAssetFromMainBundle:asset withLocalUrl:assetLocalUrl completion:^(BOOL success, NSError * _Nullable error) {
      ...
      [self _downloadAsset:asset withLocalUrl:assetLocalUrl completion:^(NSError * _Nullable error, ABI40_0_0EXSyncAsset *asset, NSURL *assetLocalUrl) {
        if (error) {
          if (asset.isLaunchAsset) {
...
- (void)_checkExistenceOfAsset:(ABI40_0_0EXSyncAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
...
- (void)_maybeCopyAssetFromMainBundle:(ABI40_0_0EXSyncAsset *)asset
                         withLocalUrl:(NSURL *)assetLocalUrl
                           completion:(void (^)(BOOL success, NSError * _Nullable error))completion
{
  ABI40_0_0EXSyncManifest *embeddedManifest = [ABI40_0_0EXSyncEmbeddedLoader embeddedManifestWithConfig:_config database:_database];
  if (embeddedManifest) {
    ABI40_0_0EXSyncAsset *matchingAsset;
    for (ABI40_0_0EXSyncAsset *embeddedAsset in embeddedManifest.assets) {
...
- (void)_downloadAsset:(ABI40_0_0EXSyncAsset *)asset
          withLocalUrl:(NSURL *)assetLocalUrl
            completion:(void (^)(NSError * _Nullable error, ABI40_0_0EXSyncAsset *asset, NSURL *assetLocalUrl))completion
{
  if (!asset.url) {
    completion([NSError errorWithDomain:ABI40_0_0EXSyncLauncherErrorDomain code:1007 userInfo:@{NSLocalizedDescriptionKey: @"Failed to download asset with no URL provided"}], asset, assetLocalUrl);
  }
  dispatch_async([ABI40_0_0EXSyncFileDownloader assetFilesQueue], ^{
    [self.downloader downloadFileFromURL:asset.url toPath:[assetLocalUrl path] successBlock:^(NSData *data, NSURLResponse *response) {
      dispatch_async(self->_launcherQueue, ^{
        ...
        asset.contentHash = [ABI40_0_0EXSyncUtils sha256WithData:data];
...
- (ABI40_0_0EXSyncFileDownloader *)downloader
{
  if (!_downloader) {
    _downloader = [[ABI40_0_0EXSyncFileDownloader alloc] initWithUpdatesConfig:_config];
  }
  return _downloader;
}
