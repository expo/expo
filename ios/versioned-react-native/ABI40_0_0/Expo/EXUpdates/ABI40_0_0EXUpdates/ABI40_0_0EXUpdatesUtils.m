...
static NSString * const ABI40_0_0EXUpdatesEventName = @"Expo.nativeUpdatesEvent";
static NSString * const ABI40_0_0EXUpdatesUtilsErrorDomain = @"ABI40_0_0EXUpdatesUtils";

@implementation ABI40_0_0EXUpdatesUtils
...
+ (nullable NSURL *)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  ...
  if (exists) {
    if (!isDir) {
      *error = [NSError errorWithDomain:ABI40_0_0EXUpdatesUtilsErrorDomain code:1005 userInfo:@{NSLocalizedDescriptionKey: @"Failed to create the Updates Directory; a file already exists with the required directory name"}];
      return nil;
    }
  } else {
    NSError *err;
...
+ (void)sendEventToBridge:(nullable ABI40_0_0RCTBridge *)bridge withType:(NSString *)eventType body:(NSDictionary *)body
{
  if (bridge) {
    ...
    [bridge enqueueJSCall:@"ABI40_0_0RCTDeviceEventEmitter.emit" args:@[ABI40_0_0EXUpdatesEventName, mutableBody]];
  } else {
...
+ (BOOL)shouldCheckForUpdateWithConfig:(ABI40_0_0EXUpdatesConfig *)config
{
  switch (config.checkOnLaunch) {
    case ABI40_0_0EXUpdatesCheckAutomaticallyConfigNever:
      return NO;
    case ABI40_0_0EXUpdatesCheckAutomaticallyConfigWifiOnly: {
    ...
    case ABI40_0_0EXUpdatesCheckAutomaticallyConfigAlways:
    default:
      return YES;
  }
}
