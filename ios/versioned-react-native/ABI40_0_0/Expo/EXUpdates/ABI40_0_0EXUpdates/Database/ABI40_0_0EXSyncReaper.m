...
@implementation ABI40_0_0EXSyncReaper
                     ...
                     launchedUpdate:(ABI40_0_0EXSyncManifest *)launchedUpdate
{
  dispatch_async(database.databaseQueue, ^{
    ...
    NSArray<ABI40_0_0EXSyncManifest *> *allUpdates = [database allUpdatesWithConfig:config error:&error];
    if (!allUpdates || error) {
      NSLog(@"Error reaping updates: %@", error.localizedDescription);
      return;
    }
    NSArray<ABI40_0_0EXSyncManifest *> *updatesToDelete = [selectionPolicy updatesToDeleteWithLaunchedUpdate:launchedUpdate updates:allUpdates];
    ...
    NSArray<ABI40_0_0EXSyncAsset *> *assetsForDeletion = [database deleteUnusedAssetsWithError:&error];
    if (error) {
      NSLog(@"Error reaping updates: %@", error.localizedDescription);
      return;
    }
    ...
    dispatch_async([ABI40_0_0EXSyncFileDownloader assetFilesQueue], ^{
      ...
      NSMutableArray<ABI40_0_0EXSyncAsset *> *erroredAssets = [NSMutableArray new];

      NSDate *beginDeleteAssets = [NSDate date];
      for (ABI40_0_0EXSyncAsset *asset in assetsForDeletion) {
      ...
      for (ABI40_0_0EXSyncAsset *asset in erroredAssets) {
        NSURL *localUrl = [directory URLByAppendingPathComponent:asset.filename];
        NSError *error;
        if ([NSFileManager.defaultManager fileExistsAtPath:localUrl.path] && ![NSFileManager.defaultManager removeItemAtURL:localUrl error:&error]) {
