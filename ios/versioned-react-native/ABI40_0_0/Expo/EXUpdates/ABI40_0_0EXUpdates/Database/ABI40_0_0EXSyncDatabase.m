...
@interface ABI40_0_0EXSyncDatabase ()
...
static NSString * const ABI40_0_0EXSyncDatabaseErrorDomain = @"ABI40_0_0EXSyncDatabase";
static NSString * const ABI40_0_0EXSyncDatabaseFilename = @"expo-v4.db";

@implementation ABI40_0_0EXSyncDatabase
...
{
  sqlite3 *db;
  NSURL *dbUrl = [directory URLByAppendingPathComponent:ABI40_0_0EXSyncDatabaseFilename];
  ...
  if (resultCode != SQLITE_OK) {
    ...
    if (resultCode == SQLITE_CORRUPT || resultCode == SQLITE_NOTADB) {
      NSString *archivedDbFilename = [NSString stringWithFormat:@"%f-%@", [[NSDate date] timeIntervalSince1970], ABI40_0_0EXSyncDatabaseFilename];
      ...
      if ([[NSFileManager defaultManager] moveItemAtURL:dbUrl toURL:destinationUrl error:&err]) {
        ...
        if (error != nil) {
          *error = [NSError errorWithDomain:ABI40_0_0EXSyncDatabaseErrorDomain
                                       code:1004
...
- (void)addUpdate:(ABI40_0_0EXSyncManifest *)update error:(NSError ** _Nullable)error
{
  ...
  [self _executeSql:sql
           withArgs:@[
                      ...
                      @(ABI40_0_0EXSyncManifestStatusPending)
                      ]
              error:error];
}
...
- (void)addNewAssets:(NSArray<ABI40_0_0EXSyncAsset *> *)assets toUpdateWithId:(NSUUID *)updateId error:(NSError ** _Nullable)error
{
  ...
  for (ABI40_0_0EXSyncAsset *asset in assets) {
    ...
    if ([self _executeSql:assetInsertSql
                 withArgs:@[
                          ...
                          asset.filename,
                          asset.contentHash,
                          @(ABI40_0_0EXSyncDatabaseHashTypeSha1)
                          ]
                    error:error] == nil) {
...
{
  if (update.status != ABI40_0_0EXSyncManifestStatusDevelopment) {
    update.status = ABI40_0_0EXSyncManifestStatusReady;
  }
...
- (nullable NSArray<ABI40_0_0EXSyncManifest *> *)allUpdatesWithConfig:(ABI40_0_0EXSyncConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI40_0_0EXSyncManifest *> *launchableUpdates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI40_0_0EXSyncManifest *> *)launchableUpdatesWithConfig:(ABI40_0_0EXSyncConfig *)config error:(NSError ** _Nullable)error
{
  ...
  FROM updates\
  WHERE scope_key = ?1\
  AND status IN (%li, %li, %li);", (long)ABI40_0_0EXSyncManifestStatusReady, (long)ABI40_0_0EXSyncManifestStatusEmbedded, (long)ABI40_0_0EXSyncManifestStatusDevelopment];
...
- (nullable NSArray<ABI40_0_0EXSyncAsset *> *)assetsWithUpdateId:(NSUUID *)updateId error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI40_0_0EXSyncAsset *> *assets = [NSMutableArray arrayWithCapacity:rows.count];
...
- (NSError *)_errorFromSqlite:(struct sqlite3 *)db
{
  ...
  return [NSError errorWithDomain:ABI40_0_0EXSyncDatabaseErrorDomain
                              code:extendedCode
...
- (ABI40_0_0EXSyncManifest *)_updateWithRow:(NSDictionary *)row config:(ABI40_0_0EXSyncConfig *)config
{
  NSError *error;
  id metadata = nil;
  ...
  ABI40_0_0EXSyncManifest *update = [ABI40_0_0EXSyncManifest updateWithId:row[@"id"]
                                                 scopeKey:row[@"scope_key"]
                                               commitTime:[NSDate dateWithTimeIntervalSince1970:[(NSNumber *)row[@"commit_time"] doubleValue] / 1000]
                                           runtimeVersion:row[@"runtime_version"]
                                                 metadata:metadata
                                                   status:(ABI40_0_0EXSyncManifestStatus)[(NSNumber *)row[@"status"] integerValue]
...
- (ABI40_0_0EXSyncAsset *)_assetWithRow:(NSDictionary *)row
{
  NSError *error;
  id metadata = nil;
  id rowMetadata = row[@"metadata"];
  ...
  ABI40_0_0EXSyncAsset *asset = [[ABI40_0_0EXSyncAsset alloc] initWithKey:row[@"key"] type:row[@"type"]];
  asset.url = url;
