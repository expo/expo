...
#import <ABI40_0_0EXUpdates/ABI40_0_0EXUpdatesDatabaseIntegrityCheck.h>
#import <ABI40_0_0EXUpdates/ABI40_0_0EXUpdatesFileDownloader.h>

NS_ASSUME_NONNULL_BEGIN

@implementation ABI40_0_0EXUpdatesDatabaseIntegrityCheck
...
{
  NSError *err;
  NSArray<ABI40_0_0EXUpdatesAsset *> *assets = [database allAssetsWithError:&err];
  if (err) {
    *error = err;
    return NO;
  }

  NSMutableArray<ABI40_0_0EXUpdatesAsset *> *missingAssets = [NSMutableArray new];
  dispatch_sync([ABI40_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
    for (ABI40_0_0EXUpdatesAsset *asset in assets) {
  ...
  NSArray<ABI40_0_0EXUpdatesUpdate *> *updatesWithEmbeddedStatus = [database allUpdatesWithStatus:ABI40_0_0EXUpdatesUpdateStatusEmbedded config:config error:&err];
  if (err) {
    *error = err;
    return NO;
  }
  ...
  for (ABI40_0_0EXUpdatesUpdate *update in updatesWithEmbeddedStatus) {
