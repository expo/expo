...
@property (nonatomic, strong) NSMutableArray<ABI40_0_0EXSyncAsset *> *assetsToLoad;
@property (nonatomic, strong) NSMutableArray<ABI40_0_0EXSyncAsset *> *erroredAssets;
@property (nonatomic, strong) NSMutableArray<ABI40_0_0EXSyncAsset *> *finishedAssets;
@property (nonatomic, strong) NSMutableArray<ABI40_0_0EXSyncAsset *> *existingAssets;
...
static NSString * const ABI40_0_0EXSyncLoaderErrorDomain = @"ABI40_0_0EXSyncLoader";

@implementation ABI40_0_0EXSyncLoader
...
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call ABI40_0_0EXSyncLoader#loadUpdate -- use a subclass instead" userInfo:nil];
}
...
- (void)startLoadingFromManifest:(ABI40_0_0EXSyncManifest *)updateManifest
{
  ...
  if (updateManifest.isDevelopmentMode) {
    dispatch_async(_database.databaseQueue, ^{
      ...
      ABI40_0_0EXSyncLoaderSuccessBlock successBlock;
  ...
  dispatch_async(_database.databaseQueue, ^{
    ...
    ABI40_0_0EXSyncManifest *existingUpdate = [self->_database updateWithId:updateManifest.updateId config:self->_config error:&existingUpdateError];
    ...
    if (existingUpdate && ![existingUpdate.scopeKey isEqualToString:updateManifest.scopeKey]) {
      ...
      NSLog(@"ABI40_0_0EXSyncLoader: Loaded an update with the same ID but a different scopeKey than one we already have on disk. This is a server error. Overwriting the scopeKey and loading the existing update.");
    }

    if (existingUpdate && existingUpdate.status == ABI40_0_0EXSyncManifestStatusReady) {
      if (self->_successBlock) {
        dispatch_async(self->_completionQueue, ^{
    ...
    if (self->_updateManifest.assets && self->_updateManifest.assets.count > 0) {
      ...
      for (ABI40_0_0EXSyncAsset *asset in self->_updateManifest.assets) {
        // before downloading, check to see if we already have this asset in the database
        NSError *matchingAssetError;
        ABI40_0_0EXSyncAsset *matchingDbEntry = [self->_database assetWithKey:asset.key error:&matchingAssetError];
...
- (BOOL)_shouldStartLoadingUpdate:(ABI40_0_0EXSyncManifest *)updateManifest
{
...
- (void)_finish
{
  dispatch_async(_database.databaseQueue, ^{
    ...
    for (ABI40_0_0EXSyncAsset *existingAsset in self->_existingAssets) {
      NSError *error;
      ...
      if (!existingAssetFound) {
        ...
        existingAsset.contentHash = [ABI40_0_0EXSyncUtils sha256WithData:contents];
    ...
    ABI40_0_0EXSyncLoaderSuccessBlock successBlock;
    ABI40_0_0EXSyncLoaderErrorBlock errorBlock;
    ...
    dispatch_async(self->_completionQueue, ^{
      if (errorBlock) {
        errorBlock([NSError errorWithDomain:ABI40_0_0EXSyncLoaderErrorDomain
                                       code:1012
