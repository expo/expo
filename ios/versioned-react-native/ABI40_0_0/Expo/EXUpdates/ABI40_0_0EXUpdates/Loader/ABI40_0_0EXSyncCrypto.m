...
static NSString * const ABI40_0_0EXSyncCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const ABI40_0_0EXSyncCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const ABI40_0_0EXSyncCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation ABI40_0_0EXSyncCrypto
...
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"ABI40_0_0EXSyncCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }

  NSURL *cachedPublicKeyUrl = [cacheDirectory URLByAppendingPathComponent:ABI40_0_0EXSyncCryptoPublicKeyFilename];
  if (useCache) {
    ...
    ABI40_0_0EXSyncFileDownloader *fileDownloader = [[ABI40_0_0EXSyncFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
    [fileDownloader downloadFileFromURL:[NSURL URLWithString:ABI40_0_0EXSyncCryptoPublicKeyUrl]
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(ABI40_0_0EXSyncVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[ABI40_0_0EXSyncCryptoPublicKeyTag UTF8String] length:[ABI40_0_0EXSyncCryptoPublicKeyTag length]];
