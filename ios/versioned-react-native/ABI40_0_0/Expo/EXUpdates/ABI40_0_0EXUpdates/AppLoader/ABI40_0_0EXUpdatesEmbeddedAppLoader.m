...
NSString * const ABI40_0_0EXUpdatesEmbeddedManifestName = @"app";
NSString * const ABI40_0_0EXUpdatesEmbeddedManifestType = @"manifest";
NSString * const ABI40_0_0EXUpdatesEmbeddedBundleFilename = @"app";
NSString * const ABI40_0_0EXUpdatesEmbeddedBundleFileType = @"bundle";
NSString * const ABI40_0_0EXUpdatesBareEmbeddedBundleFilename = @"main";
NSString * const ABI40_0_0EXUpdatesBareEmbeddedBundleFileType = @"jsbundle";

static NSString * const ABI40_0_0EXUpdatesEmbeddedAppLoaderErrorDomain = @"ABI40_0_0EXUpdatesEmbeddedAppLoader";

@implementation ABI40_0_0EXUpdatesEmbeddedAppLoader
...
{
  static ABI40_0_0EXUpdatesUpdate *embeddedManifest;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!config.hasEmbeddedUpdate) {
      embeddedManifest = nil;
    } else if (!embeddedManifest) {
      NSString *path = [[NSBundle mainBundle] pathForResource:ABI40_0_0EXUpdatesEmbeddedManifestName ofType:ABI40_0_0EXUpdatesEmbeddedManifestType];
      ...
      if (!manifest) {
        ...
        embeddedManifest = [ABI40_0_0EXUpdatesUpdate updateWithEmbeddedManifest:[mutableManifest copy]
                                                                config:config
                                                              database:database];
...
{
  ABI40_0_0EXUpdatesUpdate *embeddedManifest = [[self class] embeddedManifestWithConfig:self.config
  ...
  if (embeddedManifest) {
    ...
    error([NSError errorWithDomain:ABI40_0_0EXUpdatesEmbeddedAppLoaderErrorDomain
                              code:1008
...
- (void)loadUpdateFromUrl:(NSURL *)url
                  success:(ABI40_0_0EXUpdatesAppLoaderSuccessBlock)success
                    error:(ABI40_0_0EXUpdatesAppLoaderErrorBlock)error
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call ABI40_0_0EXUpdatesEmbeddedAppLoader#loadUpdateFromUrl" userInfo:nil];
}
