...
@interface ABI40_0_0EXAppleAuthenticationRequest ()
...
@implementation ABI40_0_0EXAppleAuthenticationRequest
...
- (void)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
{
  ...
  request.requestedScopes = [ABI40_0_0EXAppleAuthenticationMappings importScopes:requestedScopes];
...
+ (ABI40_0_0EXAppleAuthenticationRequest *)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
                                       withOptions:(NSDictionary *)options
                                      withCallback:(void(^)(NSDictionary *, NSError *))callback
{
  ABI40_0_0EXAppleAuthenticationRequest *authRequest = [[ABI40_0_0EXAppleAuthenticationRequest alloc] initWithOptions:options andCallback:callback];
   ...
   didCompleteWithAuthorization:(ASAuthorization *)authorization
{
  ...
  if (credential.fullName) {
    fullName = [ABI40_0_0EXAppleAuthenticationMappings exportFullName:credential.fullName];
  }
  NSString *authorizationCode;
  if (credential.authorizationCode) {
    authorizationCode = [ABI40_0_0EXAppleAuthenticationMappings exportData:credential.authorizationCode];
  }
  NSString *identityToken;
  if (credential.identityToken) {
    identityToken = [ABI40_0_0EXAppleAuthenticationMappings exportData:credential.identityToken];
  }

  NSDictionary *response = @{
                         @"fullName": ABI40_0_0UMNullIfNil(fullName),
                         @"email": ABI40_0_0UMNullIfNil(credential.email),
                         @"user": credential.user,
                         @"realUserStatus": [ABI40_0_0EXAppleAuthenticationMappings exportRealUserStatus:credential.realUserStatus],
                         @"state": ABI40_0_0UMNullIfNil(credential.state),
                         @"authorizationCode": ABI40_0_0UMNullIfNil(authorizationCode),
                         @"identityToken": ABI40_0_0UMNullIfNil(identityToken),
                         };
