...
@implementation ABI40_0_0EXFileSystemLocalFileHandler
              ...
              rejecter:(ABI40_0_0UMPromiseRejectBlock)reject
{
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:path isDirectory:&isDirectory]) {
    ...
    result[@"size"] = @([ABI40_0_0EXFileSystemLocalFileHandler getFileSize:path attributes:attributes]);
...
+ (unsigned long long)getFileSize:(NSString *)path attributes:(NSDictionary<NSFileAttributeKey, id> *)attributes
{
  ...
  while (file = [contentsEnumurator nextObject]) {
    ...
    folderSize += [ABI40_0_0EXFileSystemLocalFileHandler getFileSize:filePath attributes:fileAttributes];
  }
