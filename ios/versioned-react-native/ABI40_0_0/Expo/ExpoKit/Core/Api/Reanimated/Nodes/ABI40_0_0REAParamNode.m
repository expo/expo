...
@implementation ABI40_0_0REAParamNode {
  NSMutableArray<ABI40_0_0REANodeID> *_argstack;
...
- (instancetype)initWithID:(ABI40_0_0REANodeID)nodeID config:(NSDictionary<NSString *,id> *)config
{
  if ((self = [super initWithID:nodeID config:config])) {
    _argstack = [NSMutableArray<ABI40_0_0REANodeID> arrayWithCapacity:0];
  }
  return self;
}
...
{
  ABI40_0_0REANode *node = [self.nodesManager findNodeByID:[_argstack lastObject]];
  NSString *callID = self.updateContext.callID;
  self.updateContext.callID = _prevCallID;
  [(ABI40_0_0REAValueNode*)node setValue:value];
...
- (BOOL)isRunning
{
  ABI40_0_0REANode* node = [self.nodesManager findNodeByID:[_argstack lastObject]];
  if ([node isKindOfClass:[ABI40_0_0REAParamNode class]]) {
    return [(ABI40_0_0REAParamNode* )node isRunning];
  }
  return [(ABI40_0_0REAClockNode* )node isRunning];
}
