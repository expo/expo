...
- (id)evaluate
{
  ...
  for (NSDictionary *transformConfig in _transformConfigs) {
    ...
    ABI40_0_0REANodeID nodeID = [ABI40_0_0RCTConvert NSNumber:transformConfig[@"nodeID"]];
    NSNumber *value;
    if (nodeID) {
      ABI40_0_0REANode *node = [self.nodesManager findNodeByID:nodeID];
      value = [node value];
    } else {
