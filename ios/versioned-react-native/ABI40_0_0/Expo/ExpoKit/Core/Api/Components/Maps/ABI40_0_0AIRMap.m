...
const CLLocationDegrees ABI40_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI40_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI40_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI40_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI40_0_0AIRMap
{
    UIView *_legalLabel;
    ...
    // Array to manually track ABI40_0_0RN subviews
    //
    // ABI40_0_0AIRMap implicitly creates subviews that aren't regular ABI40_0_0RN children
    // (ABI40_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI40_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI40_0_0React-native/blob/v0.16.0/ABI40_0_0React/Modules/ABI40_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI40_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI40_0_0React-native/blob/v0.16.0/Libraries/Text/ABI40_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ABI40_0_0ReactSubviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI40_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI40_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI40_0_0ReactSubview:(id<ABI40_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI40_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI40_0_0AIRMapPolyline class]]) {
        ((ABI40_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI40_0_0AIRMapPolygon class]]) {
        ((ABI40_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI40_0_0AIRMapCircle class]]) {
        ((ABI40_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI40_0_0AIRMapUrlTile class]]) {
        ((ABI40_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI40_0_0AIRMapWMSTile class]]) {
        ((ABI40_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI40_0_0AIRMapLocalTile class]]) {
        ((ABI40_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI40_0_0AIRMapOverlay class]]) {
        ((ABI40_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI40_0_0RCTComponent>> *childSubviews = [subview ABI40_0_0ReactSubviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertABI40_0_0ReactSubview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ABI40_0_0ReactSubviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeABI40_0_0ReactSubview:(id<ABI40_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI40_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI40_0_0RCTComponent>> *childSubviews = [subview ABI40_0_0ReactSubviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeABI40_0_0ReactSubview:(UIView *)childSubviews[i]];
        }
    }
    [_ABI40_0_0ReactSubviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ...
        return [marker isKindOfClass:[ABI40_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI40_0_0AIRMapMarker*) markerForCallout:(ABI40_0_0AIRMapCallout*)callout {
    ABI40_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI40_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI40_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI40_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (ABI40_0_0AIRMapMarker*) markerAtPoint:(CGPoint)point {
    ABI40_0_0AIRMapMarker* mrk = nil;
    for (ABI40_0_0AIRMapMarker* mrkAnn in self.markers) {
        CGRect frame = [self frameForMarker:mrkAnn];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI40_0_0AIRMapCalloutSubview* calloutSubview = nil;
        ABI40_0_0AIRMapCallout* callout = nil;
        ABI40_0_0AIRMapMarker* marker = nil;
        
        UIView* tmp = touchedView;
        while (tmp && tmp != win && tmp != self.calloutView) {
            if ([tmp respondsToSelector:@selector(onPress)]) {
                calloutSubview = (ABI40_0_0AIRMapCalloutSubview*) tmp;
            }
            if ([tmp isKindOfClass:[ABI40_0_0AIRMapCallout class]]) {
                callout = (ABI40_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI40_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI40_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
...
- (void)cacheViewIfNeeded {
    // https://github.com/ABI40_0_0React-native-community/ABI40_0_0React-native-maps/issues/3100
