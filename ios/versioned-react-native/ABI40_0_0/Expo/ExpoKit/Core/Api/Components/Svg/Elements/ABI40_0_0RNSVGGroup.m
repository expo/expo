...
@implementation ABI40_0_0RNSVGGroup
{
    ABI40_0_0RNSVGGlyphContext *_glyphContext;
}
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI40_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI40_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI40_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI40_0_0RNSVGMask class]]) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (self.clipPath) {
        ABI40_0_0RNSVGClipPath *clipNode = (ABI40_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if ([clipNode isSimpleClipPath]) {
            ...
            ABI40_0_0RNSVGRenderable *clipGroup = (ABI40_0_0RNSVGRenderable*)clipNode;
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI40_0_0RNSVGNode class]];
    ...
    for (UIView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI40_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI40_0_0RNSVGMask class]]) {
                continue;
            }
            ABI40_0_0RNSVGNode* svgNode = (ABI40_0_0RNSVGNode*)node;
            if (event) {
...
{
    [self traverseSubviews:^(__kindof ABI40_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI40_0_0RNSVGRenderable class]]) {
            [(ABI40_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
