...
@interface ABI40_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI40_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI40_0_0RNSVGGroup *textRoot;
@end

@implementation ABI40_0_0RNSVGNode
{
    ABI40_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI40_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI40_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI40_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)invalidate
{
    ...
    _dirty = true;
    id<ABI40_0_0RNSVGContainer> container = (id<ABI40_0_0RNSVGContainer>)self.superview;
...
{
    [self clearPath];
    for (__kindof ABI40_0_0RNSVGNode *node in self.subviews) {
...
{
    ABI40_0_0RNSVGNode* node = self;
    while (node != nil) {
        UIView* parent = [node superview];

        if (![parent isKindOfClass:[ABI40_0_0RNSVGNode class]]) {
            return;
        }
        node = (ABI40_0_0RNSVGNode*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (ABI40_0_0RNSVGGroup *)textRoot
{
    ...
    ABI40_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI40_0_0RNSVGGroup class]] && [((ABI40_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI40_0_0RNSVGGroup*)node;
            break;
        }
...
- (ABI40_0_0RNSVGGroup *)getParentTextRoot
{
    ABI40_0_0RNSVGNode* parent = (ABI40_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI40_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI40_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI40_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)ABI40_0_0ReactSetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
...
- (void)setClipRule:(ABI40_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI40_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if (_cachedClipPath) {
            CGPathRelease(_cachedClipPath);
        }
...
- (ABI40_0_0RNSVGSvgView *)svgView
{
    ...
    if ([parent class] == [ABI40_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI40_0_0RNSVGNode class]]) {
        _svgView = ((ABI40_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI40_0_0RCTLogError(@"ABI40_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI40_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)relativeOn:(ABI40_0_0RNSVGLength *)length relative:(CGFloat)relative
{
    ABI40_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)relativeOnWidth:(ABI40_0_0RNSVGLength *)length
{
    ABI40_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)relativeOnHeight:(ABI40_0_0RNSVGLength *)length
{
    ABI40_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)relativeOnOther:(ABI40_0_0RNSVGLength *)length
{
    ABI40_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI40_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI40_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
