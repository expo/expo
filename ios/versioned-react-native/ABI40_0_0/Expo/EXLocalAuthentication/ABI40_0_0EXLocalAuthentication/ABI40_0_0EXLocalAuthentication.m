...
#import <ABI40_0_0EXLocalAuthentication/ABI40_0_0EXLocalAuthentication.h>

typedef NS_ENUM(NSInteger, ABI40_0_0EXAuthenticationType) {
  ABI40_0_0EXAuthenticationTypeFingerprint = 1,
  ABI40_0_0EXAuthenticationTypeFacialRecognition = 2,
};

@implementation ABI40_0_0EXLocalAuthentication

ABI40_0_0UM_EXPORT_MODULE(ExpoLocalAuthentication)

ABI40_0_0UM_EXPORT_METHOD_AS(supportedAuthenticationTypesAsync,
                    supportedAuthenticationTypesAsync:(ABI40_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI40_0_0UMPromiseRejectBlock)reject)
{
  NSMutableArray *results = [NSMutableArray array];
  if ([[self class] isTouchIdDevice]) {
    [results addObject:@(ABI40_0_0EXAuthenticationTypeFingerprint)];
  }
  if ([[self class] isFaceIdDevice]) {
    [results addObject:@(ABI40_0_0EXAuthenticationTypeFacialRecognition)];
  }
  resolve(results);
}

ABI40_0_0UM_EXPORT_METHOD_AS(hasHardwareAsync,
                    hasHardwareAsync:(ABI40_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI40_0_0UMPromiseRejectBlock)reject)
{
...
ABI40_0_0UM_EXPORT_METHOD_AS(isEnrolledAsync,
                    isEnrolledAsync:(ABI40_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI40_0_0UMPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;
          ...
          localizedReason:reason
                    reply:^(BOOL success, NSError *error) {
                      resolve(@{
                                ...
                                @"warning": ABI40_0_0UMNullIfNil(warningMessage),
                                });
                    }];
  } else {
