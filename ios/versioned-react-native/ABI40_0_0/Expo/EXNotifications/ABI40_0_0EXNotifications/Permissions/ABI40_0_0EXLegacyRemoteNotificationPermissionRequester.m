...
@interface ABI40_0_0EXLegacyRemoteNotificationPermissionRequester ()

@property (nonatomic, strong) ABI40_0_0UMPromiseResolveBlock resolve;
@property (nonatomic, strong) ABI40_0_0UMPromiseRejectBlock reject;
@property (nonatomic, assign) BOOL remoteNotificationsRegistrationIsPending;
@property (nonatomic, weak) id<ABI40_0_0UMPermissionsRequester> userNotificationPermissionRequester;
@property (nonatomic, weak) dispatch_queue_t methodQueue;
@property (nonatomic, weak) id<ABI40_0_0EXRemoteNotificationPermissionProgressPublisher> permissionProgressPublisher;

@end

@implementation ABI40_0_0EXLegacyRemoteNotificationPermissionRequester
...
{
  __block ABI40_0_0UMPermissionStatus status;
  [ABI40_0_0UMUtilities performSynchronouslyOnMainThread:^{
    status = (ABI40_0_0UMSharedApplication().isRegisteredForRemoteNotifications) ?
...
- (void)requestPermissionsWithResolver:(ABI40_0_0UMPromiseResolveBlock)resolve rejecter:(ABI40_0_0UMPromiseRejectBlock)reject
{
  ...
  [ABI40_0_0UMUtilities performSynchronouslyOnMainThread:^{
    isRegisteredForRemoteNotifications = ABI40_0_0UMSharedApplication().isRegisteredForRemoteNotifications;
  }];
  ...
  if (isRegisteredForRemoteNotifications) {
    ...
    [_userNotificationPermissionRequester requestPermissionsWithResolver:^(NSDictionary *permission){
      ...
      ABI40_0_0UMPermissionStatus localNotificationsStatus = [[permission objectForKey:@"status"] intValue];
      // We may assume that `ABI40_0_0EXLocalNotificationRequester`'s permission request will always finish
      ...
      if (localNotificationsStatus == ABI40_0_0UMPermissionStatusDenied) {
        [self _clearObserver];
        [self _maybeConsumeResolverWithCurrentPermissions];
      } else {
        self.remoteNotificationsRegistrationIsPending = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
          [ABI40_0_0UMSharedApplication() registerForRemoteNotifications];
        });
      }
