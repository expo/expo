...
@interface ABI40_0_0UMModuleRegistry ()

@property (nonatomic, weak) id<ABI40_0_0UMModuleRegistryDelegate> delegate;

@property NSMutableSet<id<ABI40_0_0UMInternalModule>> *internalModulesSet;
@property NSMapTable<Protocol *, id<ABI40_0_0UMInternalModule>> *internalModules;
@property NSMapTable<Protocol *, NSMutableArray<id<ABI40_0_0UMInternalModule>> *> *internalModulesPreResolution;
@property NSMapTable<Class, ABI40_0_0UMExportedModule *> *exportedModulesByClass;
@property NSMutableDictionary<const NSString *, ABI40_0_0UMExportedModule *> *exportedModules;
@property NSMutableDictionary<const NSString *, ABI40_0_0UMViewManager *> *viewManagerModules;
...
@implementation ABI40_0_0UMModuleRegistry
...
{
  if (self = [self init]) {
    for (id<ABI40_0_0UMInternalModule> internalModule in internalModules) {
      [self registerInternalModule:internalModule];
    }
    
    for (ABI40_0_0UMExportedModule *exportedModule in exportedModules) {
...
- (void)resolveInternalModulesConflicts
{
  ...
  for (Protocol *protocol in _internalModulesPreResolution) {
    NSArray<id<ABI40_0_0UMInternalModule>> *conflictingModules = [_internalModulesPreResolution objectForKey:protocol];

    id<ABI40_0_0UMInternalModule> resolvedModule;
...
- (void)initialize
{
  ...
  for (id<ABI40_0_0UMModuleRegistryConsumer> registryConsumer in _registryConsumers) {
...
- (void)registerInternalModule:(id<ABI40_0_0UMInternalModule>)internalModule
{
  for (Protocol *exportedInterface in [[internalModule class] exportedInterfaces]) {
    if (_internalModules) {
      id<ABI40_0_0UMInternalModule> resolvedModule = internalModule;
      if (_delegate && [_internalModules objectForKey:exportedInterface]) {
        id<ABI40_0_0UMInternalModule> oldModule = [_internalModules objectForKey:exportedInterface];
...
- (void)registerExportedModule:(ABI40_0_0UMExportedModule *)exportedModule
{
  ...
  if (_exportedModules[exportedModuleName]) {
    ABI40_0_0UMLogInfo(@"Universal module %@ overrides %@ as the module exported as %@.", exportedModule, _exportedModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerViewManager:(ABI40_0_0UMViewManager *)viewManager
{
  ...
  if (_viewManagerModules[exportedModuleName]) {
    ABI40_0_0UMLogInfo(@"Universal view manager %@ overrides %@ as the module exported as %@.", viewManager, _viewManagerModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerSingletonModule:(id)singletonModule
{
  if ([[singletonModule class] respondsToSelector:@selector(name)]) {
    ...
    ABI40_0_0UMLogWarn(@"One of the singleton modules does not respond to +(NSString *)name selector. This probably means you're either try to pass a strange object as a singleton module (it won't get registered in the module registry, sorry) or the ABI40_0_0UMSingletonModule interface and the ABI40_0_0UMModuleRegistry implementations versions are out of sync, which means things will probably not work as expected.");
  }
}

- (void)maybeAddRegistryConsumer:(id)maybeConsumer
{
  if ([maybeConsumer conformsToProtocol:@protocol(ABI40_0_0UMModuleRegistryConsumer)]) {
    [self addRegistryConsumer:(id<ABI40_0_0UMModuleRegistryConsumer>)maybeConsumer];
  }
}

- (void)addRegistryConsumer:(id<ABI40_0_0UMModuleRegistryConsumer>)registryConsumer
{
