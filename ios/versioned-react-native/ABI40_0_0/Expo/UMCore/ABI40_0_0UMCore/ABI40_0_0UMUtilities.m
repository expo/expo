...
@interface ABI40_0_0UMUtilities ()

@property (nonatomic, nullable, weak) ABI40_0_0UMModuleRegistry *moduleRegistry;

@end

@protocol ABI40_0_0UMUtilService
...
@implementation ABI40_0_0UMUtilities

ABI40_0_0UM_REGISTER_MODULE();

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI40_0_0UMUtilitiesInterface)];
}
...
- (nullable NSDictionary *)launchOptions
{
  id<ABI40_0_0UMUtilService> utilService = [_moduleRegistry getSingletonModuleForName:@"Util"];
...
// Kind of copied from ABI40_0_0RN to make UIColor:(id)json work
...
+ (UIColor *)UIColor:(id)json
{
  ...
  if ([json isKindOfClass:[NSArray class]]) {
    ...
    ABI40_0_0UMLogInfo(@"%@ cannot be converted to a UIColor", json);
    return nil;
  }
}
...
+ (NSDate *)NSDate:(id)json
{
  if ([json isKindOfClass:[NSNumber class]]) {
    ...
    if (!date) {
      ABI40_0_0UMLogError(@"JSON String '%@' could not be interpreted as a date. "
                  "Expected format: YYYY-MM-DD'T'HH:mm:ss.sssZ", json);
    }
    return date;
  } else if (json) {
    ABI40_0_0UMLogError(json, @"a date");
  }
  return nil;
}
...
UIApplication * ABI40_0_0UMSharedApplication(void)
{
...
NSError *ABI40_0_0UMErrorWithMessage(NSString *message)
{
  NSDictionary<NSString *, id> *errorInfo = @{NSLocalizedDescriptionKey: message};
  return [[NSError alloc] initWithDomain:@"ABI40_0_0UMModulesErrorDomain" code:0 userInfo:errorInfo];
}
