...
#define ABI40_0_0EXPAND_AND_QUOTE(str) QUOTE(str)

#define ABI40_0_0UM_IS_METHOD_PROPSETTER(methodName) \
  [methodName hasPrefix:@ABI40_0_0EXPAND_AND_QUOTE(ABI40_0_0UM_PROPSETTERS_PREFIX)]

#define ABI40_0_0UM_PROPSETTER_FOR_PROP(propName)\
  QUOTE(ABI40_0_0UM_PROPSETTERS_PREFIX)propName

static const NSString *noViewExceptionName = @"No custom -(UIView *)view implementation.";
static const NSString *noViewExceptionReason = @"You've subclassed an ABI40_0_0UMViewManager, but didn't override the -(UIView *)view method. Override this method and return a new view instance.";

static const NSString *noViewNameExceptionName = @"No custom -(NSString *)viewName implementation.";
static const NSString *noViewNameExceptionReasonFormat = @"You've subclassed an ABI40_0_0UMViewManager in %@, but didn't override the -(NSString *)viewName method. Override this method and return a name of the view component.";

@interface ABI40_0_0UMViewManager ()

@property NSDictionary<NSString *, NSString *> *propsNamesSelectors;

@end

@implementation ABI40_0_0UMViewManager
...
// Scans the class methods for methods with a certain prefix (see macro ABI40_0_0UM_PROPSETTERS_PREFIX),
...
- (NSDictionary<NSString *, NSString *> *)getPropsNames
{
  ...
  @try {
    for(int i = 0; i < methodsCount; i++) {
      ...
      if (ABI40_0_0UM_IS_METHOD_PROPSETTER(methodName)) {
        NSString *propNameWithArguments = [methodName substringFromIndex:[@ABI40_0_0EXPAND_AND_QUOTE(ABI40_0_0UM_PROPSETTERS_PREFIX) length]];
...
- (void)updateProp:(NSString *)propName withValue:(id)value onView:(UIView *)view
{
  if (_propsNamesSelectors[propName]) {
    ...
    if (methodSignature == nil) {
      ...
      ABI40_0_0UMLogError(@"View manager of view '%@' does not implement method for selector '%@'.", [self viewName], NSStringFromSelector(selector));
      return;
    }
    ...
    ABI40_0_0UMLogWarn(@"Tried to set property `%@` on view manager of view `%@` when the view manager does not export such prop.", propName, [self viewName]);
  }
}
