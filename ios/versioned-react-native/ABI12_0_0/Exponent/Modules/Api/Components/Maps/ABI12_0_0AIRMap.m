...
const CLLocationDegrees ABI12_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI12_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI12_0_0AIRMapZoomBoundBuffer = 0.01;
...
@implementation ABI12_0_0AIRMap
{
    UIView *_legalLabel;
    ...
    //
    // ABI12_0_0AIRMap implicitly creates subviews that aren't regular RN children
    // (ABI12_0_0SMCalloutView injects an overlay subview), which otherwise confuses RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI12_0_0-native/blob/v0.16.0/ReactABI12_0_0/Modules/ABI12_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI12_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI12_0_0-native/blob/v0.16.0/Libraries/Text/ABI12_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ReactABI12_0_0Subviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _ReactABI12_0_0Subviews = [NSMutableArray new];
        ...
        self.calloutView = [ABI12_0_0SMCalloutView platformCalloutView];
...
- (void)insertReactABI12_0_0Subview:(id<ABI12_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI12_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI12_0_0AIRMapPolyline class]]) {
        ((ABI12_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI12_0_0AIRMapPolygon class]]) {
        ((ABI12_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI12_0_0AIRMapCircle class]]) {
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI12_0_0AIRMapUrlTile class]]) {
        [self addOverlay:(id<MKOverlay>)subview];
    }
    [_ReactABI12_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI12_0_0Subview:(id<ABI12_0_0RCTComponent>)subview {
    ...
    [_ReactABI12_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
#pragma mark ABI12_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI12_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
