...
@property (nonatomic, weak) ABI12_0_0EXGLViewManager *viewManager;
...
@implementation ABI12_0_0EXGLView
...
- (instancetype)initWithManager:(ABI12_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
    ...
    _exglCtxId = 0;
    id<ABI12_0_0RCTJavaScriptExecutor> executor = [_viewManager.bridge valueForKey:@"javaScriptExecutor"];
    if ([executor isKindOfClass:NSClassFromString(@"ABI12_0_0RCTJSCExecutor")]) {
      // On JS thread, extract JavaScriptCore context, create ABI12_0_0EXGL context, call JS callback
      __weak __typeof__(self) weakSelf = self;
      __weak __typeof__(executor) weakExecutor = executor;
      [executor executeBlockOnJavaScriptQueue:^{
        __typeof__(self) self = weakSelf;
        ABI12_0_0RCTJSCExecutor *executor = weakExecutor;
        if (self && executor) {
      ...
      ABI12_0_0RCTLog(@"ABI12_0_0EXGL: Can only run on JavaScriptCore!");
    }
  }
  return self;
}
...
- (void)draw
{
  // _exglCtxId may be unset if we get here (on the UI thread) before ABI12_0_0EXGLContextCreate(...) is
  // called on the JS thread to create the ABI12_0_0EXGL context and save its id (see init method). In
