...
NSString * const ABI12_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI12_0_0EXLinkingManager ()
...
@implementation ABI12_0_0EXLinkingManager

+ (NSString *)moduleName { return @"ABI12_0_0RCTLinkingManager"; }
...
#pragma mark - ABI12_0_0RCTEventEmitter methods

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI12_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI12_0_0RCTFatal(ABI12_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI12_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}

ABI12_0_0RCT_EXPORT_METHOD(openURL:(NSURL *)URL
                  ...
                  reject:(ABI12_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _isExponentUrl:URL]) {
    ...
    BOOL opened = [ABI12_0_0RCTSharedApplication() openURL:URL];
    if (opened) {
      resolve(nil);
    } else {
      reject(ABI12_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@", URL], nil);
    }
  }
}

ABI12_0_0RCT_EXPORT_METHOD(canOpenURL:(NSURL *)URL
                  resolve:(ABI12_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI12_0_0RCTPromiseRejectBlock)reject)
{
  BOOL canOpen = [self _isExponentUrl:URL];
  if (!canOpen) {
    canOpen = [ABI12_0_0RCTSharedApplication() canOpenURL:URL];
  }
  resolve(@(canOpen));
}

ABI12_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI12_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI12_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI12_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
...
- (BOOL)_isExponentUrl: (NSURL *)url
{
  ...
  NSString *versionsPath = [[NSBundle mainBundle] pathForResource:@"ABI12_0_0EXSDKVersions" ofType:@"plist"];
