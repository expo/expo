...
#import "ABI12_0_0RCTCameraRollManager.h"

#import <CoreLocation/CoreLocation.h>
#import <Foundation/Foundation.h>
#import <UIKit/UIKit.h>

#import "ABI12_0_0RCTAssetsLibraryRequestHandler.h"
...
@implementation ABI12_0_0RCTConvert (ALAssetGroup)

ABI12_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI12_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI12_0_0RCTCameraRollManager

ABI12_0_0RCT_EXPORT_MODULE()

@synthesize bridge = _bridge;

NSString *const ABI12_0_0RCTErrorUnableToLoad = @"E_UNABLE_TO_LOAD";
NSString *const ABI12_0_0RCTErrorUnableToSave = @"E_UNABLE_TO_SAVE";

ABI12_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
  ...
  if ([type isEqualToString:@"video"]) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [self->_bridge.assetsLibrary writeVideoAtPathToSavedPhotosAlbum:request.URL completionBlock:^(NSURL *assetURL, NSError *saveError) {
        if (saveError) {
          reject(ABI12_0_0RCTErrorUnableToSave, nil, saveError);
        } else {
    ...
    [_bridge.imageLoader loadImageWithURLRequest:request
      ...
      if (loadError) {
        reject(ABI12_0_0RCTErrorUnableToLoad, nil, loadError);
        return;
      }
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        [self->_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
          if (saveError) {
            ABI12_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
            reject(ABI12_0_0RCTErrorUnableToSave, nil, saveError);
          } else {
            resolve(assetURL.absoluteString);
          }
...
static void ABI12_0_0RCTResolvePromise(ABI12_0_0RCTPromiseResolveBlock resolve,
...
ABI12_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI12_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI12_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger first = [ABI12_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI12_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI12_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI12_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI12_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI12_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI12_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI12_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
    reject(ABI12_0_0RCTErrorUnableToLoad, nil, error);
  }];
}
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI12_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
