...
#import "ABI12_0_0RCTImagePickerManager.h"
...
@interface ABI12_0_0RCTImagePickerManager ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI12_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI12_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI12_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
}

ABI12_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI12_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI12_0_0RCTResponseSenderBlock)callback)
{
...
ABI12_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI12_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI12_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI12_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI12_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  if ([ABI12_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI12_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI12_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI12_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI12_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI12_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI12_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    [self _dismissPicker:picker args:@[imageURL.absoluteString, ABI12_0_0RCTNullIfNil(height), ABI12_0_0RCTNullIfNil(width)]];
    return;
  }
        ...
        cancelCallback:(ABI12_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI12_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI12_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI12_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
