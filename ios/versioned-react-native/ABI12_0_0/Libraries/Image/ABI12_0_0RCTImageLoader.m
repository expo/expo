...
@implementation UIImage (ReactABI12_0_0)

- (CAKeyframeAnimation *)ReactABI12_0_0KeyframeAnimation
{
  return objc_getAssociatedObject(self, _cmd);
}

- (void)setReactABI12_0_0KeyframeAnimation:(CAKeyframeAnimation *)ReactABI12_0_0KeyframeAnimation
{
  objc_setAssociatedObject(self, @selector(ReactABI12_0_0KeyframeAnimation), ReactABI12_0_0KeyframeAnimation, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

@end

@implementation ABI12_0_0RCTImageLoader
{
  NSArray<id<ABI12_0_0RCTImageURLLoader>> *_loaders;
  NSArray<id<ABI12_0_0RCTImageDataDecoder>> *_decoders;
  NSOperationQueue *_imageDecodeQueue;
  dispatch_queue_t _URLRequestQueue;
  id<ABI12_0_0RCTImageCache> _imageCache;
...
- (void)setUp
{
  ...
  _URLRequestQueue = dispatch_queue_create("com.facebook.ReactABI12_0_0.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (id<ABI12_0_0RCTImageCache>)imageCache
{
  if (!_imageCache) {
    //set up with default cache
    _imageCache = [ABI12_0_0RCTImageCache new];
  }
  return _imageCache;
}
...
{
  if (_imageCache) {
    ABI12_0_0RCTLogWarn(@"ABI12_0_0RCTImageCache was already set and has now been overriden.");
  }
  _imageCache = cache;
}

- (id<ABI12_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
  ...
  if (!_loaders) {
    ...
    ABI12_0_0RCTAssert(_bridge, @"Bridge not set");
    _loaders = [[_bridge modulesConformingToProtocol:@protocol(ABI12_0_0RCTImageURLLoader)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI12_0_0RCTImageURLLoader> a, id<ABI12_0_0RCTImageURLLoader> b) {
  ...
  if (ABI12_0_0RCT_DEBUG) {
    ...
    id<ABI12_0_0RCTImageURLLoader> previousLoader = nil;
    for (id<ABI12_0_0RCTImageURLLoader> loader in _loaders) {
      ...
      if ([loader canLoadImageURL:URL]) {
        if (previousLoader) {
          if (priority == previousPriority) {
            ABI12_0_0RCTLogError(@"The ABI12_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI12_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
  if (!_maxConcurrentLoadingTasks) {
    [self setUp];
  }
  ...
  if (!_decoders) {
    ...
    ABI12_0_0RCTAssert(_bridge, @"Bridge not set");
    _decoders = [[_bridge modulesConformingToProtocol:@protocol(ABI12_0_0RCTImageDataDecoder)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI12_0_0RCTImageDataDecoder> a, id<ABI12_0_0RCTImageDataDecoder> b) {
  ...
  if (ABI12_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI12_0_0RCTImageDataDecoder> previousDecoder = nil;
    for (id<ABI12_0_0RCTImageDataDecoder> decoder in _decoders) {
      ...
      if ([decoder canDecodeImageData:data]) {
        if (previousDecoder) {
          if (priority == previousPriority) {
            ABI12_0_0RCTLogError(@"The ABI12_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI12_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI12_0_0RCTResizeMode resizeMode)
{
  ...
  CAKeyframeAnimation *animation = image.ReactABI12_0_0KeyframeAnimation;
  CGRect targetSize = ABI12_0_0RCTTargetRect(image.size, size, scale, resizeMode);
  CGAffineTransform transform = ABI12_0_0RCTTransformFromTargetRect(image.size, targetSize);
  image = ABI12_0_0RCTTransformImage(image, size, scale, transform);
  image.ReactABI12_0_0KeyframeAnimation = animation;
  return image;
}
...
- (void)dequeueTasks
{
  dispatch_async(_URLRequestQueue, ^{
    ...
    while (i < [self->_pendingTasks count]) {
      ABI12_0_0RCTNetworkTask *task = [self->_pendingTasks objectAtIndex:i];

      // Remove any completed tasks from the front of the array
      if (task.status == ABI12_0_0RCTNetworkTaskFinished) {
        [self->_pendingTasks removeObjectAtIndex:i];
      }

      // Start pending tasks up to the limit
      else if (task.status == ABI12_0_0RCTNetworkTaskPending) {
        if (i < maxTasks) {
          if (![task start]) {
            ABI12_0_0RCTAssert(NO, @"Task %p failed to start", task);
          } else {
            i++;
          }
        } else {
 ...
 */
- (ABI12_0_0RCTImageLoaderCancellationBlock)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                             size:(CGSize)size
                                                            scale:(CGFloat)scale
                                                       resizeMode:(ABI12_0_0RCTResizeMode)resizeMode
                                                    progressBlock:(ABI12_0_0RCTImageLoaderProgressBlock)progressHandler
                                                 partialLoadBlock:(ABI12_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
                                                  completionBlock:(void (^)(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate))completionBlock
{
  {
    NSMutableURLRequest *mutableRequest = [request mutableCopy];
    [NSURLProtocol setProperty:@"ABI12_0_0RCTImageLoader"
                        forKey:@"trackingName"
  ...
  id<ABI12_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
  ...
  void (^completionHandler)(NSError *, id, NSString *) = ^(NSError *error, id imageOrData, NSString *fetchDate) {
    ...
    if (ABI12_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
  ...
  __weak ABI12_0_0RCTImageLoader *weakSelf = self;
  dispatch_async(_URLRequestQueue, ^{
...
- (ABI12_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI12_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSString *fetchDate))completionHandler
{
  // Check if networking module is available
  if (ABI12_0_0RCT_DEBUG && ![_bridge respondsToSelector:@selector(networking)]) {
    ABI12_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                " import the ABI12_0_0RCTNetwork library in order to load images.",
                request.URL.absoluteString);
    return NULL;
  }

  ABI12_0_0RCTNetworking *networking = [_bridge networking];

  // Check if networking module can load image
  if (ABI12_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
    ABI12_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
    return NULL;
  }

  // Use networking module to load image
  ABI12_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    ...
    if (error) {
      ...
      completionHandler(ABI12_0_0RCTErrorWithMessage(@"Response metadata error"), nil, nil);
      return;
    } else if (!data) {
      completionHandler(ABI12_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, nil);
      return;
    }
  ...
  [networking networkTaskWithRequest:request
    ...
    if (error || !response || !data) {
      ...
      if (error) {
        ...
        someError = ABI12_0_0RCTErrorWithMessage(@"Response metadata error");
      } else {
        someError = ABI12_0_0RCTErrorWithMessage(@"Unknown image download error");
      }
                                           ...
                                           completionBlock:(ABI12_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  ...
  void (^completionHandler)(NSError *, id, BOOL, NSString *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate) {
    ...
    ABI12_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
...
- (ABI12_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI12_0_0RCTResizeMode)resizeMode
...
{
  if (data.length == 0) {
    completionBlock(ABI12_0_0RCTErrorWithMessage(@"No image data"), nil);
    return ^{};
  }
  ...
  void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
    if (ABI12_0_0RCTIsMainQueue()) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!cancelled) {
          completionBlock(error, clipped ? ABI12_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
        }
      });
    } else if (!cancelled) {
      completionBlock(error, clipped ? ABI12_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
  ...
  id<ABI12_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
  if (imageDecoder) {
    ...
    dispatch_block_t decodeBlock = ^{
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!cancelled) {
          ...
          UIImage *image = ABI12_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI12_0_0RCT_DEV
          CGSize imagePixelSize = ABI12_0_0RCTSizeInPixels(image.size, image.scale);
          CGSize screenPixelSize = ABI12_0_0RCTSizeInPixels(ABI12_0_0RCTScreenSize(), ABI12_0_0RCTScreenScale());
          if (imagePixelSize.width * imagePixelSize.height >
              screenPixelSize.width * screenPixelSize.height) {
            ABI12_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
          ...
          if (image) {
            ...
            NSError *finalError = ABI12_0_0RCTErrorWithMessage(errorMessage);
                                                       ...
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
  void (^completion)(NSError *, id, BOOL, NSString *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate) {
    ...
    if ([imageOrData isKindOfClass:[NSData class]]) {
      NSDictionary *meta = ABI12_0_0RCTGetImageMetadata(imageOrData);
      size = (CGSize){
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  for (id<ABI12_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI12_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI12_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI12_0_0RCTURLRequestHandler)] &&
...
{
  __block ABI12_0_0RCTImageLoaderCancellationBlock requestToken;
  requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
    ...
    if (ABI12_0_0RCTImageHasAlpha(image.CGImage)) {
      mimeType = @"image/png";
...
@implementation ABI12_0_0RCTBridge (ABI12_0_0RCTImageLoader)

- (ABI12_0_0RCTImageLoader *)imageLoader
{
  return [self moduleForClass:[ABI12_0_0RCTImageLoader class]];
}
