...
#import "ABI12_0_0RCTTestRunner.h"
...
@implementation ABI12_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI12_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI12_0_0RCTAssertParam(app);
  ABI12_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI12_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI12_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI12_0_0RCTSetLogFunction(^(ABI12_0_0RCTLogLevel level, ABI12_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI12_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI12_0_0RCTBridge *bridge = [[ABI12_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI12_0_0RCTRootView *rootView = [[ABI12_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
    rootView.frame = CGRectMake(0, 0, 320, 2000); // Constant size for testing on multiple devices

    ABI12_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI12_0_0RCTTestModule class]];
    ABI12_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI12_0_0RCTTestStatusPending && error == nil) {
    ...
    ABI12_0_0RCTSetLogFunction(ABI12_0_0RCTDefaultLogFunction);
    ...
    ABI12_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI12_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI12_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI12_0_0RCTAssert(testModule.status != ABI12_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI12_0_0RCTAssert(testModule.status == ABI12_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI12_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
