...
const CLLocationDegrees ABI12_0_0RCTMapDefaultSpan = 0.005;
const NSTimeInterval ABI12_0_0RCTMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI12_0_0RCTMapZoomBoundBuffer = 0.01;

@implementation ABI12_0_0RCTMap
{
  UIView *_legalLabel;
...
- (void)setAnnotations:(NSArray<ABI12_0_0RCTMapAnnotation *> *)annotations
{
  ...
  NSMutableArray<ABI12_0_0RCTMapAnnotation *> *annotationsToDelete = [NSMutableArray new];
  NSMutableArray<ABI12_0_0RCTMapAnnotation *> *annotationsToAdd = [NSMutableArray new];

  for (ABI12_0_0RCTMapAnnotation *annotation in annotations) {
  ...
  for (ABI12_0_0RCTMapAnnotation *annotation in self.annotations) {
    if (![annotation isKindOfClass:[ABI12_0_0RCTMapAnnotation class]]) {
      continue;
    }
...
- (void)setOverlays:(NSArray<ABI12_0_0RCTMapOverlay *> *)overlays
{
  ...
  for (ABI12_0_0RCTMapOverlay *overlay in overlays) {
  ...
  for (ABI12_0_0RCTMapOverlay *overlay in self.overlays) {
    if (![overlay isKindOfClass:[ABI12_0_0RCTMapOverlay class]]) {
      continue;
    }
