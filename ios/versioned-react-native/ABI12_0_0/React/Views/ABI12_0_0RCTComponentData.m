...
typedef void (^ABI12_0_0RCTPropBlock)(id<ABI12_0_0RCTComponent> view, id json);

@interface ABI12_0_0RCTComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) ABI12_0_0RCTPropBlock propBlock;

@end

@implementation ABI12_0_0RCTComponentProp
...
@implementation ABI12_0_0RCTComponentData
{
  id<ABI12_0_0RCTComponent> _defaultView; // Only needed for ABI12_0_0RCT_CUSTOM_VIEW_PROPERTY
  NSMutableDictionary<NSString *, ABI12_0_0RCTPropBlock> *_viewPropBlocks;
  NSMutableDictionary<NSString *, ABI12_0_0RCTPropBlock> *_shadowPropBlocks;
                              ...
                              bridge:(ABI12_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    // Hackety hack, this partially re-implements ABI12_0_0RCTBridgeModuleNameForClass
    // We want to get rid of ABI12_0_0RCT and RK prefixes, but a lot of JS code still references
    // view names by prefix. So, while ABI12_0_0RCTBridgeModuleNameForClass now drops these
    ...
    ABI12_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
    _name = name;

    _implementsUIBlockToAmendWithShadowViewRegistry = NO;
    Class cls = _managerClass;
    while (cls != [ABI12_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI12_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI12_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI12_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI12_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ABI12_0_0RCTPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, ABI12_0_0RCTPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  ABI12_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {

    __weak ABI12_0_0RCTComponentData *weakSelf = self;
    ...
    if ([_managerClass respondsToSelector:selector]) {
      ...
      type = ABI12_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<ABI12_0_0RCTComponent> view, id json) {
        ABI12_0_0RCTComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        json = ABI12_0_0RCTNilIfNull(json);
        if (!shadowView) {
      ...
      if (type == NSSelectorFromString(@"ABI12_0_0RCTBubblingEventBlock:") ||
          type == NSSelectorFromString(@"ABI12_0_0RCTDirectEventBlock:")) {

        // Special case for event handlers
        __weak ABI12_0_0RCTViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<ABI12_0_0RCTComponent> weakTarget = target;
          ((void (*)(id, SEL, id))objc_msgSend)(target, setter, [ABI12_0_0RCTConvert BOOL:json] ? ^(NSDictionary *body) {
            body = [NSMutableDictionary dictionaryWithDictionary:body];
            ((NSMutableDictionary *)body)[@"target"] = weakTarget.ReactABI12_0_0Tag;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            [weakManager.bridge.eventDispatcher sendInputEventWithName:ABI12_0_0RCTNormalizeInputEventName(name) body:body];
#pragma clang diagnostic pop
        ...
        NSMethodSignature *typeSignature = [[ABI12_0_0RCTConvert class] methodSignatureForSelector:type];
        if (!typeSignature) {
          ABI12_0_0RCTLogError(@"No +[ABI12_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
          return ^(__unused id<ABI12_0_0RCTComponent> view, __unused id json){};
        }
  ...
  #define ABI12_0_0RCT_CASE(_value, _type) \
          case _value: { \
            ...
            setterBlock = ^(id target, id json) { \
              if (json) { \
                ...
                set(target, setter, convert([ABI12_0_0RCTConvert class], type, json)); \
            ...
            ABI12_0_0RCT_CASE(_C_SEL, SEL)
            ABI12_0_0RCT_CASE(_C_CHARPTR, const char *)
            ...
            ABI12_0_0RCT_CASE(_C_PTR, void *)
            ABI12_0_0RCT_CASE(_C_ID, id)
      ...
      propBlock = ^(__unused id view, __unused id json) {
        ...
        setterBlock(target, ABI12_0_0RCTNilIfNull(json));
      };
    }
    ...
    if (ABI12_0_0RCT_DEBUG) {
      ...
      ABI12_0_0RCTPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<ABI12_0_0RCTComponent> view, id json) {
        NSString *logPrefix = [NSString stringWithFormat:
                               @"Error setting property '%@' of %@ with tag #%@: ",
                               name, weakSelf.name, view.ReactABI12_0_0Tag];

        ABI12_0_0RCTPerformBlockWithLogPrefix(^{ unwrappedBlock(view, json); }, logPrefix);
      };
    }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  if (ABI12_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customDirectEventTypes))) {
    NSArray<NSString *> *events = [self.manager customDirectEventTypes];
#pragma clang diagnostic pop
    if (ABI12_0_0RCT_DEBUG) {
      ABI12_0_0RCTAssert(!events || [events isKindOfClass:[NSArray class]],
        @"customDirectEventTypes must return an array, but %@ returned %@",
        _managerClass, [events class]);
    }
    for (NSString *event in events) {
      [directEvents addObject:ABI12_0_0RCTNormalizeInputEventName(event)];
    }
  }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  if (ABI12_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
#pragma clang diagnostic pop
    ...
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI12_0_0RCTNormalizeInputEventName(event)];
    }
  }
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if ([methodName hasPrefix:@"propConfig"]) {
      ...
      if (nameRange.length) {
        ...
        if (ABI12_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
          ABI12_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                      "to '%@'", name, _name, propTypes[name], type);
        }

        if ([type isEqualToString:@"ABI12_0_0RCTBubblingEventBlock"]) {
          [bubblingEvents addObject:ABI12_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else if ([type isEqualToString:@"ABI12_0_0RCTDirectEventBlock"]) {
          [directEvents addObject:ABI12_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else {
          propTypes[name] = type;
        }
  ...
  if (ABI12_0_0RCT_DEBUG) {
    for (NSString *event in directEvents) {
      if ([bubblingEvents containsObject:event]) {
        ABI12_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
