...
@interface ABI12_0_0RCTScrollEvent : NSObject <ABI12_0_0RCTEvent>
...
@implementation ABI12_0_0RCTScrollEvent
{
...
{
  ABI12_0_0RCTAssertParam(ReactABI12_0_0Tag);
...
- (ABI12_0_0RCTScrollEvent *)coalesceWithEvent:(ABI12_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI12_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI12_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI12_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, copy) NSIndexSet *stickyHeaderIndices;
@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI12_0_0RCTRefreshControl *rctRefreshControl;
#endif

@end


@implementation ABI12_0_0RCTCustomScrollView
{
...
- (UIView *)contentView
{
  return ((ABI12_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI12_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI12_0_0RCTUIManager JSResponder] isKindOfClass:[ABI12_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI12_0_0 scroll
...
- (void)dockClosestSectionHeader
{
  ...
  NSUInteger subviewCount = contentView.ReactABI12_0_0Subviews.count;
  [_stickyHeaderIndices enumerateIndexesWithOptions:0 usingBlock:
   ^(NSUInteger idx, BOOL *stop) {
    ...
    UIView *header = contentView.ReactABI12_0_0Subviews[idx];
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI12_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI12_0_0RCTScrollView
{
  ABI12_0_0RCTEventDispatcher *_eventDispatcher;
  ABI12_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI12_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI12_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;
    _scrollView = [[ABI12_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI12_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI12_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI12_0_0RCTAssert(_contentView == nil, @"ABI12_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI12_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI12_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI12_0_0Subview:atIndex:`
}
...
{
  ABI12_0_0RCTAssert(_scrollView.contentSize.width <= self.frame.size.width,
...
- (void)layoutSubviews
{
  ...
  ABI12_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI12_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI12_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI12_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI12_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI12_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI12_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI12_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI12_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI12_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI12_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI12_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI12_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI12_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI12_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI12_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI12_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI12_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI12_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI12_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI12_0_0RCTView contentInsetsForView:self];
...
- (void)ReactABI12_0_0BridgeDidFinishTransaction
{
  ...
  if (ABI12_0_0RCT_DEBUG) {
    ...
    NSUInteger subviewCount = _scrollView.contentView.ReactABI12_0_0Subviews.count;
    ...
    if (lastIndex != NSNotFound && lastIndex >= subviewCount) {
      ABI12_0_0RCTLogWarn(@"Sticky header index %zd was outside the range {0, %zd}",
...
#define ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI12_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI12_0_0RCTScrollEvent *scrollEvent = [[ABI12_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI12_0_0RCTEventDispatcher (ABI12_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI12_0_0Tag
{
  ...
  ABI12_0_0RCTScrollEvent *fakeScrollEvent = [[ABI12_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI12_0_0Tag:ReactABI12_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
