...
NSString *const ABI12_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI12_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI12_0_0RCTAccessibilityManager ()
...
@implementation ABI12_0_0RCTAccessibilityManager
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI12_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI12_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI12_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI12_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI12_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI12_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI12_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI12_0_0RCTResponseSenderBlock)callback
...
@implementation ABI12_0_0RCTBridge (ABI12_0_0RCTAccessibilityManager)

- (ABI12_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI12_0_0RCTAccessibilityManager class]];
}
