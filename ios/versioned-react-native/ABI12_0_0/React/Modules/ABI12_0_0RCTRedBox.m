...
#import "ABI12_0_0RCTErrorInfo.h"
#import "ABI12_0_0RCTUtils.h"
#import "ABI12_0_0RCTJSStackFrame.h"

#if ABI12_0_0RCT_DEBUG

@class ABI12_0_0RCTRedBoxWindow;

@protocol ABI12_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI12_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI12_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI12_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI12_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI12_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI12_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI12_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI12_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI12_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI12_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI12_0_0RCTJSStackFrame *)stackFrame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI12_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI12_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI12_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI12_0_0RCTKeyCommands for this, but since
...
@interface ABI12_0_0RCTRedBox () <ABI12_0_0RCTInvalidating, ABI12_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI12_0_0RCTRedBox
{
  ABI12_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI12_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI12_0_0RCTErrorInfo *)_customizeError:(ABI12_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI12_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI12_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI12_0_0RCTJSStackFrame *> *stack = [ABI12_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI12_0_0RCTJSStackFrame class]]) {
    stack = [ABI12_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI12_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI12_0_0RCTErrorInfo *errorInfo = [[ABI12_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI12_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI12_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI12_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI12_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI12_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI12_0_0RCTRedBoxWindow *)redBoxWindow {
  [_bridge requestReload];
}

@end

@implementation ABI12_0_0RCTBridge (ABI12_0_0RCTRedBox)

- (ABI12_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI12_0_0RCTRedBox class]];
}
...
- (ABI12_0_0RCTRedBox *)redBox { return nil; }
