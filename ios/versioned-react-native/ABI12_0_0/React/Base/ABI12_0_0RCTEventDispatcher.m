...
const NSInteger ABI12_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI12_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI12_0_0RCTGetEventID(id<ABI12_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI12_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI12_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI12_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI12_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI12_0_0RCT_DEBUG) {
    ABI12_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI12_0_0 tag");
  }

  name = ABI12_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI12_0_0RCTEventEmitter"
                  method:@"receiveEvent"
                    args:body ? @[body[@"target"], name, body] : @[body[@"target"], name]
              completion:NULL];
}

- (void)sendTextEventWithType:(ABI12_0_0RCTTextEventType)type
...
- (void)sendEvent:(id<ABI12_0_0RCTEvent>)event
{
  [_observersLock lock];

  BOOL eventHandled = NO;
  for (id<ABI12_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI12_0_0RCTGetEventID(event);

  id<ABI12_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI12_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)addDispatchObserver:(id<ABI12_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI12_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)dispatchEvent:(id<ABI12_0_0RCTEvent>)event
{
...
@implementation ABI12_0_0RCTBridge (ABI12_0_0RCTEventDispatcher)

- (ABI12_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI12_0_0RCTEventDispatcher class]];
}
