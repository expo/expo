...
@interface ABI27_0_0EXRemoteNotificationRequester ()
...
@property (nonatomic, strong) ABI27_0_0EXLocalNotificationRequester *localNotificationRequester;

@end

@implementation ABI27_0_0EXRemoteNotificationRequester
...
{
  ABI27_0_0EXPermissionStatus status = (ABI27_0_0RCTSharedApplication().isRegisteredForRemoteNotifications) ?
...
- (void)requestPermissionsWithResolver:(ABI27_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI27_0_0RCTPromiseRejectBlock)reject
{
  ...
  if (ABI27_0_0RCTSharedApplication().isRegisteredForRemoteNotifications) {
...
# pragma mark - ABI27_0_0EXPermissionRequesterDelegate
...
- (void)permissionsRequester:(NSObject<ABI27_0_0EXPermissionRequester> *)requester didFinishWithResult:(NSDictionary *)requestResult
{
  if (requester == _localNotificationRequester) {
    ...
    NSString *localNotificationsStatus = [[ABI27_0_0EXLocalNotificationRequester permissions] objectForKey:@"status"];
    // We may assume that `ABI27_0_0EXLocalNotificationRequester`'s permission request will always finish
    ...
    if ([localNotificationsStatus isEqualToString:[ABI27_0_0EXPermissions permissionStringForStatus:ABI27_0_0EXPermissionStatusDenied]]) {
      [self _clearObserver];
    }
    [self _maybeConsumeResolverWithCurrentPermissions];
  }
