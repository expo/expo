...
@implementation ABI27_0_0EXPermissions

ABI27_0_0EX_EXPORT_SCOPED_MODULE(ExponentPermissions, PermissionsManager);
...
ABI27_0_0RCT_REMAP_METHOD(getAsync,
                 ...
                 rejecter:(ABI27_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (![type isEqualToString:@"notifications"] && ![type isEqualToString:@"userFacingNotifications"] &&
      [globalPermissionsResult[@"status"] isEqualToString:[[self class] permissionStringForStatus:ABI27_0_0EXPermissionStatusGranted]]) {
    if (![_kernelPermissionsServiceDelegate hasGrantedPermission:type forExperience:self.experienceId]) {
      response[@"status"] = [[self class] permissionStringForStatus:ABI27_0_0EXPermissionStatusDenied];
    }
  }
  resolve(response);
}

ABI27_0_0RCT_REMAP_METHOD(askAsync,
                 ...
                 rejecter:(ABI27_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([systemPermissions[@"status"] isEqualToString:[ABI27_0_0EXPermissions permissionStringForStatus:ABI27_0_0EXPermissionStatusGranted]]) {
    // temporarily exclude notifactions from permissions per experience
    if (![type isEqualToString:@"notifications"] && ![type isEqualToString:@"userFacingNotifications"] &&
      ...
      UIAlertAction *allow = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        if ([_kernelPermissionsServiceDelegate savePermission:systemPermissions ofType:type forExperience:self.experienceId]) {
          ...
          deniedResult[@"status"] = [[weakSelf class] permissionStringForStatus:ABI27_0_0EXPermissionStatusDenied];
    ...
    id<ABI27_0_0EXPermissionRequester> requester;
    if ([type isEqualToString:@"notifications"]) {
      requester = [[ABI27_0_0EXRemoteNotificationRequester alloc] init];
    } else if ([type isEqualToString:@"userFacingNotifications"]) {
      requester = [[ABI27_0_0EXLocalNotificationRequester alloc] init];
    } else if ([type isEqualToString:@"notifications"]) {
      requester = [[ABI27_0_0EXRemoteNotificationRequester alloc] init];
    } else if ([type isEqualToString:@"userFacingNotifications"]) {
      requester = [[ABI27_0_0EXLocalNotificationRequester alloc] init];
    } else if ([type isEqualToString:@"location"]) {
      requester = [[ABI27_0_0EXLocationRequester alloc] init];
    } else if ([type isEqualToString:@"camera"]) {
      requester = [[ABI27_0_0EXCameraPermissionRequester alloc] init];
    } else if ([type isEqualToString:@"contacts"]) {
      requester = [[ABI27_0_0EXContactsRequester alloc] init];
    } else if ([type isEqualToString:@"audioRecording"]) {
      requester = [[ABI27_0_0EXAudioRecordingPermissionRequester alloc] init];
    } else if ([type isEqualToString:@"cameraRoll"]) {
      requester = [[ABI27_0_0EXCameraRollRequester alloc] init];
    } else if ([type isEqualToString:@"calendar"]) {
      requester = [[ABI27_0_0EXCalendarRequester alloc] init];
    } else if ([type isEqualToString:@"reminders"]) {
      requester = [[ABI27_0_0EXRemindersRequester alloc] init];
    } else {
...
{
  if ([type isEqualToString:@"notifications"]) {
    return [ABI27_0_0EXRemoteNotificationRequester permissions];
  } else if ([type isEqualToString:@"userFacingNotifications"]) {
    return [ABI27_0_0EXLocalNotificationRequester permissions];
  } else if ([type isEqualToString:@"location"]) {
    return [ABI27_0_0EXLocationRequester permissions];
  } else if ([type isEqualToString:@"camera"]) {
    return [ABI27_0_0EXCameraPermissionRequester permissions];
  } else if ([type isEqualToString:@"contacts"]) {
    return [ABI27_0_0EXContactsRequester permissions];
  } else if ([type isEqualToString:@"audioRecording"]) {
    return [ABI27_0_0EXAudioRecordingPermissionRequester permissions];
  } else if ([type isEqualToString:@"systemBrightness"]) {
    return [ABI27_0_0EXPermissions alwaysGrantedPermissions]; // permission is implicit
  } else if ([type isEqualToString:@"cameraRoll"]) {
    return [ABI27_0_0EXCameraRollRequester permissions];
  } else if ([type isEqualToString:@"calendar"]) {
    return [ABI27_0_0EXCalendarRequester permissions];
  } else if ([type isEqualToString:@"reminders"]) {
    return [ABI27_0_0EXRemindersRequester permissions];
  }
  return nil;
}
...
- (void)showPermissionRequestAlert:(NSString *)type withAllowAction:(UIAlertAction *)allow withDenyAction:(UIAlertAction *)deny
{
  ...
  [ABI27_0_0RCTPresentedViewController() presentViewController:alert animated:YES completion:nil];
}

+ (NSDictionary *)alwaysGrantedPermissions {
  return @{
    @"status": [ABI27_0_0EXPermissions permissionStringForStatus:ABI27_0_0EXPermissionStatusGranted],
...
{
  switch (status) {
    case ABI27_0_0EXPermissionStatusGranted:
      return @"granted";
    case ABI27_0_0EXPermissionStatusDenied:
      return @"denied";
    default:
      return @"undetermined";
  }
...
+ (ABI27_0_0EXPermissionStatus)statusForPermissions:(NSDictionary *)permissions
{
  ...
  if ([status isEqualToString:@"granted"]) {
    return ABI27_0_0EXPermissionStatusGranted;
  } else if ([status isEqualToString:@"denied"]) {
    return ABI27_0_0EXPermissionStatusDenied;
  } else {
    return ABI27_0_0EXPermissionStatusUndetermined;
  }
}
...
+ (NSString *)typeForRequester:(NSObject<ABI27_0_0EXPermissionRequester> *)requester
{
  if ([requester isKindOfClass:[ABI27_0_0EXLocationRequester class]]) {
    return @"location";
  } else if ([requester isKindOfClass:[ABI27_0_0EXCameraPermissionRequester class]]) {
    return @"camera";
  } else if ([requester isKindOfClass:[ABI27_0_0EXContactsRequester class]]) {
    return @"contacts";
  } else if ([requester isKindOfClass:[ABI27_0_0EXAudioRecordingPermissionRequester class]]) {
    return @"audioRecording";
  } else if ([requester isKindOfClass:[ABI27_0_0EXCameraRollRequester class]]) {
    return @"cameraRoll";
  } else if ([requester isKindOfClass:[ABI27_0_0EXCalendarRequester class]]) {
    return @"calendar";
  } else if ([requester isKindOfClass:[ABI27_0_0EXRemindersRequester class]]) {
    return @"reminders";
  }
  return @"unknown";
}
