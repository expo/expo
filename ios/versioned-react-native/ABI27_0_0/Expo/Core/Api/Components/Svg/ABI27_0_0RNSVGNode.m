...
#import "ABI27_0_0RNSVGClipPath.h"
#import "ABI27_0_0RNSVGGroup.h"
#import "ABI27_0_0RNSVGGlyphContext.h"

@implementation ABI27_0_0RNSVGNode
{
    ABI27_0_0RNSVGGroup *_textRoot;
    ABI27_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    CGPathRef _cachedClipPath;
    ABI27_0_0RNSVGSvgView *_svgView;
}

CGFloat const ReactABI27_0_0M_SQRT1_2l = 0.707106781186547524400844362104849039;
CGFloat const ABI27_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)invalidate
{
    id<ABI27_0_0RNSVGContainer> container = (id<ABI27_0_0RNSVGContainer>)self.superview;
    [container invalidate];
}

- (ABI27_0_0RNSVGGroup *)getTextRoot
{
    ABI27_0_0RNSVGNode* node = self;
    if (_textRoot == nil) {
        while (node != nil) {
            if ([node isKindOfClass:[ABI27_0_0RNSVGGroup class]] && [((ABI27_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
                _textRoot = (ABI27_0_0RNSVGGroup*)node;
                break;
            }

            UIView* parent = [node superview];

            if (![node isKindOfClass:[ABI27_0_0RNSVGNode class]]) {
                node = nil;
            } else {
                node = (ABI27_0_0RNSVGNode*)parent;
            }
        }
    }
...
- (ABI27_0_0RNSVGGroup *)getParentTextRoot
{
    ABI27_0_0RNSVGNode* parent = (ABI27_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI27_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI27_0_0RNSVGGroup* root = [self getTextRoot];
    if (root == nil) {
        return ABI27_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)ReactABI27_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
...
- (ABI27_0_0RNSVGSvgView *)getSvgView
{
    if (_svgView) {
        return _svgView;
    }
    ...
    if ([parent class] == [ABI27_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI27_0_0RNSVGNode class]]) {
        ABI27_0_0RNSVGNode *node = parent;
        _svgView = [node getSvgView];
    } else {
        ABI27_0_0RCTLogError(@"ABI27_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI27_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)relativeOnOther:(NSString *)length
{
    ...
    CGFloat r = sqrt(powX + powY) * ReactABI27_0_0M_SQRT1_2l;
    return [ABI27_0_0RNSVGPropHelper fromRelativeWithNSString:length
                                         relative:r
                                           offset:0
                                            scale:1
...
{
    ABI27_0_0RNSVGGroup * root = [self getTextRoot];
    ABI27_0_0RNSVGGlyphContext * gc = [root getGlyphContext];
    if (root == nil || gc == nil) {
...
- (void)traverseSubviews:(BOOL (^)(__kindof ABI27_0_0RNSVGNode *node))block
{
    for (ABI27_0_0RNSVGNode *node in self.subviews) {
        if ([node isKindOfClass:[ABI27_0_0RNSVGNode class]]) {
            if (!block(node)) {
                break;
            }
