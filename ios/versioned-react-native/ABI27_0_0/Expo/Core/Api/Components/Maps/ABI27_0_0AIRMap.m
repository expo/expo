...
const CLLocationDegrees ABI27_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI27_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI27_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI27_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI27_0_0AIRMap
{
    UIView *_legalLabel;
    CLLocationManager *_locationManager;
    BOOL _initialRegionSet;

    // Array to manually track ABI27_0_0RN subviews
    //
    // ABI27_0_0AIRMap implicitly creates subviews that aren't regular ABI27_0_0RN children
    // (ABI27_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI27_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI27_0_0-native/blob/v0.16.0/ReactABI27_0_0/Modules/ABI27_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI27_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI27_0_0-native/blob/v0.16.0/Libraries/Text/ABI27_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ReactABI27_0_0Subviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _ReactABI27_0_0Subviews = [NSMutableArray new];
        ...
        self.calloutView = [ABI27_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI27_0_0AIRMapMaxZoomLevel;
    }
    return self;
}
...
- (void)insertReactABI27_0_0Subview:(id<ABI27_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI27_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI27_0_0AIRMapPolyline class]]) {
        ((ABI27_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI27_0_0AIRMapPolygon class]]) {
        ((ABI27_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI27_0_0AIRMapCircle class]]) {
        ((ABI27_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI27_0_0AIRMapUrlTile class]]) {
        ((ABI27_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI27_0_0AIRMapLocalTile class]]) {
        ((ABI27_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI27_0_0AIRMapOverlay class]]) {
        ((ABI27_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI27_0_0RCTComponent>> *childSubviews = [subview ReactABI27_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertReactABI27_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ReactABI27_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI27_0_0Subview:(id<ABI27_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI27_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI27_0_0RCTComponent>> *childSubviews = [subview ReactABI27_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeReactABI27_0_0Subview:(UIView *)childSubviews[i]];
        }
    }
    [_ReactABI27_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (NSArray<id<ABI27_0_0RCTComponent>> *)ReactABI27_0_0Subviews {
  return _ReactABI27_0_0Subviews;
}
#pragma clang diagnostic pop
...
#pragma mark ABI27_0_0SMCalloutViewDelegate
