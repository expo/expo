...
NSInteger const ReactABI27_0_0CALLOUT_OPEN_ZINDEX_BASELINE = 999;

@implementation ABI27_0_0AIREmptyCalloutBackgroundView
bool ReactABI27_0_0_calloutIsOpen = NO;
NSInteger ReactABI27_0_0_zIndexBeforeOpen = 0;
@end

@implementation ABI27_0_0AIRMapMarker {
...
- (void)ReactABI27_0_0SetFrame:(CGRect)frame
{
    ...
    if (isnan(center.x) || isnan(center.y) ||
        ...
        ABI27_0_0RCTLogError(@"Invalid layout for (%@)%@. position: %@. bounds: %@",
                self.ReactABI27_0_0Tag, self, NSStringFromCGPoint(center), NSStringFromCGRect(bounds));
        return;
    }
...
- (void)insertReactABI27_0_0Subview:(id<ABI27_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    if ([subview isKindOfClass:[ABI27_0_0AIRMapCallout class]]) {
        self.calloutView = (ABI27_0_0AIRMapCallout *)subview;
    } else {
        [super insertReactABI27_0_0Subview:(UIView *)subview atIndex:atIndex];
    }
}

- (void)removeReactABI27_0_0Subview:(id<ABI27_0_0RCTComponent>)subview {
    if ([subview isKindOfClass:[ABI27_0_0AIRMapCallout class]] && self.calloutView == subview) {
        self.calloutView = nil;
    } else {
        [super removeReactABI27_0_0Subview:(UIView *)subview];
    }
}
...
- (MKAnnotationView *)getAnnotationView
{
    if ([self shouldUsePinView]) {
        ...
        // If it has subviews, it means we are wanting to render a custom marker with arbitrary ReactABI27_0_0 views.
        // if it has a non-null image, it means we want to render a custom marker with the image.
        // In either case, we want to return the ABI27_0_0AIRMapMarker since it is both an MKAnnotation and an
...
- (void)fillCalloutView:(ABI27_0_0SMCalloutView *)calloutView
{
    ...
    if (self.calloutView) {
        ...
        if (self.calloutView.tooltip) {
            // if tooltip is true, then the user wants their ReactABI27_0_0 view to be the "tooltip" as wwell, so we set
            // the background view to something empty/transparent
            calloutView.backgroundView = [ABI27_0_0AIREmptyCalloutBackgroundView new];
        } else {
            // the default tooltip look is wanted, and the user is just filling the content with their ReactABI27_0_0 subviews.
            // as a result, we use the default "masked" background view.
            calloutView.backgroundView = [ABI27_0_0SMCalloutMaskedBackgroundView new];
        }

        // when this is set, the callout's content will be whatever ReactABI27_0_0 views the user has put as the callout's
        // children.
...
{
    ReactABI27_0_0_calloutIsOpen = YES;
    [self setZIndex:ReactABI27_0_0_zIndexBeforeOpen];
...
- (void)_handleTap:(UITapGestureRecognizer *)recognizer {
    ABI27_0_0AIRMapMarker *marker = self;
    if (!marker) return;
...
{
    ReactABI27_0_0_calloutIsOpen = NO;
    [self setZIndex:ReactABI27_0_0_zIndexBeforeOpen];
...
{
    return self.ReactABI27_0_0Subviews.count == 0 && !self.imageSrc;
}
...
{
    ReactABI27_0_0_zIndexBeforeOpen = zIndex;
    _zIndex = ReactABI27_0_0_calloutIsOpen ? zIndex + ReactABI27_0_0CALLOUT_OPEN_ZINDEX_BASELINE : zIndex;
