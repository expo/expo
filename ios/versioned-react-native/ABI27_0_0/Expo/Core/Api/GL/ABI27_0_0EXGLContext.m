...
@interface ABI27_0_0EXGLContext ()

@property (nonatomic, strong) dispatch_queue_t glQueue;
@property (nonatomic, weak) ABI27_0_0EXGLObjectManager *manager;
...
@implementation ABI27_0_0EXGLContext
...
{
  ABI27_0_0RCTBridge *bridge = _manager.bridge;

  if (!bridge.executorClass || [NSStringFromClass(bridge.executorClass) isEqualToString:@"ABI27_0_0RCTJSCExecutor"]) {
    // On JS thread, extract JavaScriptCore context, create ABI27_0_0EXGL context, call JS callback
    ...
    [bridge dispatchBlock:^{
      ...
      if (!jsContextRef) {
        ABI27_0_0RCTLogError(@"ABI27_0_0EXGL: The ReactABI27_0_0 Native bridge unexpectedly does not have a JavaScriptCore context.");
        BLOCK_SAFE_RUN(callback, NO);
        return;
      }
    ...
    } queue:ABI27_0_0RCTJSThread];
  } else {
    BLOCK_SAFE_RUN(callback, NO);
    ABI27_0_0RCTLog(@"ABI27_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI27_0_0-native/docs/debugging.html)? ABI27_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI27_0_0EXGL.");
  }
}
  ...
  [self runAsync:^{
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI27_0_0RCTErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI27_0_0RCTErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = ABI27_0_0RCTScreenScale();
