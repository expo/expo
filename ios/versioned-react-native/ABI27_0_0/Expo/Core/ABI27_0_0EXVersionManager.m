...
#import "ABI27_0_0EXAppState.h"
#import "ABI27_0_0EXDevSettings.h"
#import "ABI27_0_0EXDisabledDevLoadingView.h"
#import "ABI27_0_0EXDisabledDevMenu.h"
#import "ABI27_0_0EXDisabledRedBox.h"
#import "ABI27_0_0EXFileSystem.h"
#import "ABI27_0_0EXVersionManager.h"
#import "ABI27_0_0EXStatusBarManager.h"
#import "ABI27_0_0EXUnversioned.h"
#import "ABI27_0_0EXTest.h"
...
#import <ReactABI27_0_0/ABI27_0_0RCTExceptionsManager.h>
...
#import <ReactABI27_0_0/ABI27_0_0RCTAsyncLocalStorage.h>

#import <objc/message.h>

static NSNumber *ABI27_0_0EXVersionManagerIsFirstLoad;

// used for initializing scoped modules which don't tie in to any kernel service.
#define ABI27_0_0EX_KERNEL_SERVICE_NONE @"ABI27_0_0EXKernelServiceNone"

// this is needed because ABI27_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI27_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSDictionary *> *ABI27_0_0EXScopedModuleClasses;
void ABI27_0_0EXRegisterScopedModule(Class, ...);
void ABI27_0_0EXRegisterScopedModule(Class moduleClass, ...)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI27_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  ...
  va_start(argumentList, moduleClass);
    while ((kernelServiceClassName = va_arg(argumentList, NSString*))) {
      if ([kernelServiceClassName isEqualToString:@"nil"]) {
        unversionedKernelServiceClassNames[kernelServiceClassName] = ABI27_0_0EX_KERNEL_SERVICE_NONE;
      } else {
  ...
  if (moduleClassName) {
    ABI27_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassNames;
  }
}

@interface ABI27_0_0EXVersionManager ()
...
@implementation ABI27_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI27_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI27_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI27_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
#ifdef ABI27_0_0EX_ENABLE_UNSAFE_SYSTRACE
    items[@"dev-profiler"] = @{
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI27_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return ([ABI27_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI27_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI27_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  if (ABI27_0_0EXVersionManagerIsFirstLoad == nil) {
    // first time initializing this RN version at runtime
    _isFirstLoad = YES;
  }
  ABI27_0_0EXVersionManagerIsFirstLoad = @(NO);
  ABI27_0_0RCTSetFatalHandler(fatalHandler);
  ABI27_0_0RCTSetLogThreshold(threshold);
  ABI27_0_0RCTSetLogFunction(logFunction);
}
 ...
 *    @ABI27_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI27_0_0EXKernel *kernel
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSString *localStorageDirectory = [[ABI27_0_0EXFileSystem documentDirectoryForExperienceId:experienceId] stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];

  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI27_0_0EXAppState alloc] init],
                                    [[ABI27_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:isDeveloper],
                                    [[ABI27_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI27_0_0EXStatusBarManager alloc] init],
                                    [[ABI27_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory],
                                    ]];
  ...
  if (exceptionsManagerDelegate) {
    ABI27_0_0RCTExceptionsManager *exceptionsManager = [[ABI27_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
    [extraModules addObject:exceptionsManager];
  } else {
    ABI27_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
  }
  
  if (params[@"testEnvironment"]) {
    ABI27_0_0EXTestEnvironment testEnvironment = (ABI27_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI27_0_0EXTestEnvironmentNone) {
      ABI27_0_0EXTest *testModule = [[ABI27_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  /**  todo: ben
   if (params[@"kernel"]) {
    ABI27_0_0EXHomeModule *homeModule = [[ABI27_0_0EXHomeModule alloc] initWithExperienceId:experienceId
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI27_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI27_0_0RCTDevMenu
    [extraModules addObject:[[ABI27_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI27_0_0RCTRedBox
    [extraModules addObject:[[ABI27_0_0EXDisabledRedBox alloc] init]];
  }
  return extraModules;
}
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI27_0_0EXScopedModuleClasses) {
    [ABI27_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI27_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
