//
//  ABI27_0_0EXFaceDetectorManager.m
//  Exponent
//
...
@interface ABI27_0_0EXFaceDetectorManager() <GMVDataOutputDelegate>
...
@property (nonatomic, weak) id <ABI27_0_0EXFaceDetectorDelegate> delegate;
...
@implementation ABI27_0_0EXFaceDetectorManager
...
- (void)setIsEnabled:(id)json
{
  ...
  BOOL newFaceDetecting = [ABI27_0_0RCTConvert BOOL:json];
...
- (void)tryEnablingFaceDetection
{
  ...
  if ([self isDetectingFaces]) {
    @try {
      ...
      ABI27_0_0RCTLogWarn(@"%@", [exception description]);
    }
  }
...
{
  long requestedValue = [ABI27_0_0RCTConvert NSInteger:json];
...
- (void)dataOutput:(GMVDataOutput *)dataOutput didFinishedDetection:(NSArray<__kindof GMVFeature *> *)results
{
  ...
  // width or height started happen after refactor: moving face detection logic from ABI27_0_0EXCameraManager to ABI27_0_0EXFaceDetectorManager.
  ...
  CGAffineTransform transform = [ABI27_0_0EXFaceDetectorUtils transformFromDeviceOutput:dataOutput toInterfaceVideoOrientation:interfaceVideoOrientation];
  
  ABI27_0_0EXFaceEncoder *faceEncoder = [[ABI27_0_0EXFaceEncoder alloc] initWithTransform:transform];
