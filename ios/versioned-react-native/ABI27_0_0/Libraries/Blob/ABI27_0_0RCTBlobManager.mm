...
#import "ABI27_0_0RCTBlobManager.h"
...
#import <ReactABI27_0_0/ABI27_0_0RCTWebSocketModule.h>

static NSString *const kBlobURIScheme = @"blob";

@interface ABI27_0_0RCTBlobManager () <ABI27_0_0RCTNetworkingRequestHandler, ABI27_0_0RCTNetworkingResponseHandler, ABI27_0_0RCTWebSocketContentHandler>

@end

@implementation ABI27_0_0RCTBlobManager
{
...
ABI27_0_0RCT_EXPORT_MODULE(BlobModule)
...
{
  NSString *blobId = [ABI27_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI27_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI27_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI27_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
...
ABI27_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(nonnull NSNumber *)socketID)
{
...
ABI27_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI27_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI27_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI27_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI27_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI27_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI27_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI27_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI27_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI27_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI27_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI27_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI27_0_0RCTWebSocketContentHandler methods
