...
#import "ABI27_0_0RCTTestRunner.h"
...
@implementation ABI27_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI27_0_0RCTBridgeModuleListProvider _moduleProvider;
  NSString *_appPath;
}
...
{
  ABI27_0_0RCTAssertParam(app);
  ABI27_0_0RCTAssertParam(referenceDirectory);
...
- (void)updateScript
{
  if (getenv("CI_USE_PACKAGER") || _useBundler) {
    ...
    _scriptURL = [[NSBundle bundleForClass:[ABI27_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
  }
  ABI27_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
}
...
configurationBlock:(void(^)(ABI27_0_0RCTRootView *rootView))configurationBlock
{
...
{
  __weak ABI27_0_0RCTBridge *batchedBridge;

  @autoreleasepool {
    __block NSMutableArray<NSString *> *errors = nil;
    ABI27_0_0RCTLogFunction defaultLogFunction = ABI27_0_0RCTGetLogFunction();
    ABI27_0_0RCTSetLogFunction(^(ABI27_0_0RCTLogLevel level, ABI27_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      ...
      if (level >= ABI27_0_0RCTLogLevelError) {
        if (errors == nil) {
    ...
    ABI27_0_0RCTBridge *bridge = [[ABI27_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];
    [bridge.devSettings setIsDebuggingRemotely:_useJSDebugger];
    batchedBridge = [bridge batchedBridge];

    ABI27_0_0RCTRootView *rootView = [[ABI27_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI27_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI27_0_0RCTTestModule class]];
    ABI27_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    testModule.controller = _testController;
    testModule.testSelector = test;
    testModule.testSuffix = _testSuffix;
    testModule.view = rootView;

    UIViewController *vc = ABI27_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI27_0_0RCTTestStatusPending && errors == nil) {
    ...
    ABI27_0_0RCTSetLogFunction(defaultLogFunction);
    ...
    ABI27_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);
#endif

    if (expectErrorBlock) {
      ABI27_0_0RCTAssert(expectErrorBlock(errors[0]), @"Expected an error but the first one was missing or did not match.");
    } else {
      ABI27_0_0RCTAssert(errors == nil, @"RedBox errors: %@", errors);
      ABI27_0_0RCTAssert(testModule.status != ABI27_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI27_0_0RCTAssert(testModule.status == ABI27_0_0RCTTestStatusPassed, @"Test failed");
    }
