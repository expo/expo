...
@implementation ABI27_0_0RCTAssetsLibraryRequestHandler
{
     ...
     withDelegate:(id<ABI27_0_0RCTURLRequestDelegate>)delegate
{
  ...
  [[self assetsLibrary] assetForURL:request.URL resultBlock:^(ALAsset *asset) {
    ...
    if (asset) {
      ...
      NSError *error = ABI27_0_0RCTErrorWithMessage(errorMessage);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
    }
...
@implementation ABI27_0_0RCTBridge (ABI27_0_0RCTAssetsLibraryImageLoader)

- (ALAssetsLibrary *)assetsLibrary
{
  return [[self moduleForClass:[ABI27_0_0RCTAssetsLibraryRequestHandler class]] assetsLibrary];
}
