...
@implementation ABI27_0_0RCTTextShadowView
{
...
- (instancetype)initWithBridge:(ABI27_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI27_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI27_0_0RCTTextShadowViewMeasure);
    ABI27_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI27_0_0RCTTextShadowViewBaseline);
  }
...
{
  if (ABI27_0_0YGNodeIsDirty(self.yogaNode)) {
    return;
  }
  ...
  NSNumber *tag = self.ReactABI27_0_0Tag;
  NSMutableArray<NSNumber *> *descendantViewTags = [NSMutableArray new];
  [textStorage enumerateAttribute:ABI27_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:
   ^(ABI27_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
     if (!shadowView) {
       return;
     }

     [descendantViewTags addObject:shadowView.ReactABI27_0_0Tag];
   }
  ];

  [_bridge.uiManager addUIBlock:^(ABI27_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI27_0_0RCTTextView *textView = (ABI27_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI27_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName range:NSMakeRange(0, textStorage.length)];
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI27_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                               inRange:NSMakeRange(0, attributedText.length)
                               options:0
                            usingBlock:
    ^(ABI27_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                                        ...
                                        exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage ABI27_0_0RCTscaleFontSizeToFitSize:size
...
- (void)layoutSubviewsWithContext:(ABI27_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage enumerateAttribute:ABI27_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          ...
                          options:0
                       usingBlock:
    ^(ABI27_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
      if (!shadowView) {
        return;
      }
      ...
      CGRect frame = {{
        ABI27_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI27_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)
      }, {
        ABI27_0_0RCTRoundPixelValue(attachmentSize.width),
        ABI27_0_0RCTRoundPixelValue(attachmentSize.height)
      }};

      ABI27_0_0RCTLayoutContext localLayoutContext = layoutContext;
      ...
      ABI27_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
...
static ABI27_0_0YGSize ABI27_0_0RCTTextShadowViewMeasure(ABI27_0_0YGNodeRef node, float width, ABI27_0_0YGMeasureMode widthMode, float height, ABI27_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
    widthMode == ABI27_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI27_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    heightMode == ABI27_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI27_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI27_0_0RCTTextShadowView *shadowTextView = (__bridge ABI27_0_0RCTTextShadowView *)ABI27_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
    MIN(ABI27_0_0RCTCeilPixelValue(size.width), maximumSize.width),
    MIN(ABI27_0_0RCTCeilPixelValue(size.height), maximumSize.height)
  };
  ...
  return (ABI27_0_0YGSize){
    ABI27_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
    ABI27_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)
  };
}

static float ABI27_0_0RCTTextShadowViewBaseline(ABI27_0_0YGNodeRef node, const float width, const float height)
{
  ABI27_0_0RCTTextShadowView *shadowTextView = (__bridge ABI27_0_0RCTTextShadowView *)ABI27_0_0YGNodeGetContext(node);

  CGSize size = (CGSize){
    ABI27_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    ABI27_0_0RCTCoreGraphicsFloatFromYogaFloat(height)
  };

  CGFloat lastBaseline = [shadowTextView lastBaselineForSize:size];

  return ABI27_0_0RCTYogaFloatFromCoreGraphicsFloat(lastBaseline);
}
