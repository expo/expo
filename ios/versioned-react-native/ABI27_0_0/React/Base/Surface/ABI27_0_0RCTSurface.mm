...
#import "ABI27_0_0RCTSurfaceView+Internal.h"
...
#import "ABI27_0_0RCTSurfaceRootShadowView.h"
#import "ABI27_0_0RCTSurfaceRootShadowViewDelegate.h"
#import "ABI27_0_0RCTSurfaceRootView.h"
...
@interface ABI27_0_0RCTSurface () <ABI27_0_0RCTSurfaceRootShadowViewDelegate, ABI27_0_0RCTUIManagerObserver>
@end

@implementation ABI27_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI27_0_0RCTBridge *_batchedBridge;
  ABI27_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI27_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI27_0_0RCTSurfaceView *_Nullable _view;
  ABI27_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI27_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI27_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI27_0_0RCTAllocateRootViewTag();
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBridgeWillLoadJavaScriptNotification:)
                                                 name:ABI27_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBridgeDidLoadJavaScriptNotification:)
                                                 name:ABI27_0_0RCTJavaScriptDidLoadNotification
                                               object:_bridge];

    _stage = ABI27_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI27_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI27_0_0RCTUIManager *)_uiManager
{
...
- (ABI27_0_0RCTSurfaceView *)view
{
  ABI27_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI27_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI27_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI27_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI27_0_0RCTSurfaceRootView *rootView =
    (ABI27_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI27_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI27_0_0RCTAssert([rootView isKindOfClass:[ABI27_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI27_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI27_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI27_0_0RCTSurfaceStageModuleDidLoad];

  ABI27_0_0RCTBridge *bridge = notification.userInfo[@"bridge"];
...
- (ABI27_0_0RCTSurfaceStage)stage
{
  std::lock_guard<std::mutex> lock(_mutex);
  return _stage;
}

- (void)_setStage:(ABI27_0_0RCTSurfaceStage)stage
{
  ABI27_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI27_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI27_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI27_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI27_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI27_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI27_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI27_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountReactABI27_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI27_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI27_0_0RCTIsMainQueue() ? ABI27_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI27_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI27_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI27_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI27_0_0Tag:self->_rootViewTag];
    ABI27_0_0RCTAssert([rootShadowView isKindOfClass:[ABI27_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI27_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI27_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI27_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI27_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI27_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI27_0_0Tag:self->_rootViewTag];
...
- (BOOL)synchronouslyWaitForStage:(ABI27_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI27_0_0RCTIsUIManagerQueue()) {
    ABI27_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI27_0_0RCTIsMainQueue() && (stage == ABI27_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI27_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI27_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI27_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI27_0_0RCTAssert(NO, @"Only waiting for `ABI27_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI27_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI27_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI27_0_0RCTIsMainQueue() && (stage == ABI27_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI27_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI27_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI27_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(ABI27_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI27_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(ABI27_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI27_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(ABI27_0_0RCTUIManager *)manager performMountingWithBlock:(ABI27_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI27_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(ABI27_0_0RCTUIManager *)manager
{
  if (self.stage & ABI27_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI27_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ReactABI27_0_0 components

- (void)mountReactABI27_0_0ComponentWithBridge:(ABI27_0_0RCTBridge *)bridge moduleName:(NSString *)moduleName params:(NSDictionary *)params
{
