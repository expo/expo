...
@interface ABI27_0_0RCTScrollEvent : NSObject <ABI27_0_0RCTEvent>
...
@implementation ABI27_0_0RCTScrollEvent
{
...
{
  ABI27_0_0RCTAssertParam(ReactABI27_0_0Tag);
...
- (ABI27_0_0RCTScrollEvent *)coalesceWithEvent:(ABI27_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI27_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI27_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI27_0_0RCTRefreshControl *rctRefreshControl;
...
@implementation ABI27_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI27_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI27_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI27_0_0RCTUIManager JSResponder] isKindOfClass:[ABI27_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI27_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI27_0_0RCTRefreshControl *)refreshControl
{
...
@interface ABI27_0_0RCTScrollView () <ABI27_0_0RCTUIManagerObserver>

@end

@implementation ABI27_0_0RCTScrollView
{
  ABI27_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI27_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI27_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI27_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI27_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI27_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI27_0_0LayoutDirection:layoutDirection];

  ABI27_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI27_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI27_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI27_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI27_0_0RCTAssert(_contentView == nil, @"ABI27_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI27_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ReactABI27_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI27_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI27_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI27_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI27_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  ABI27_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI27_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI27_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI27_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI27_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI27_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI27_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI27_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI27_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI27_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI27_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI27_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI27_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI27_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI27_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI27_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI27_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI27_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI27_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI27_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI27_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(ABI27_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(ABI27_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI27_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI27_0_0RCTScrollEvent *scrollEvent = [[ABI27_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI27_0_0RCTEventDispatcher (ABI27_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI27_0_0Tag
{
  ...
  ABI27_0_0RCTScrollEvent *fakeScrollEvent = [[ABI27_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI27_0_0Tag:ReactABI27_0_0Tag
