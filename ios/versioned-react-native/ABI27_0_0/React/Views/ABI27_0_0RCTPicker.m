...
@interface ABI27_0_0RCTPicker() <UIPickerViewDataSource, UIPickerViewDelegate>
@end

@implementation ABI27_0_0RCTPicker
...
{
  return [ABI27_0_0RCTConvert NSString:_items[row][@"label"]];
}
           ...
           reusingView:(UILabel *)label
{
  ...
  label.textColor = [ABI27_0_0RCTConvert UIColor:_items[row][@"textColor"]] ?: _color;
      ...
      didSelectRow:(NSInteger)row inComponent:(__unused NSInteger)component
{
  ...
  if (_onChange && _items.count > (NSUInteger)row) {
    _onChange(@{
      @"newIndex": @(row),
      @"newValue": ABI27_0_0RCTNullIfNil(_items[row][@"value"]),
    });
  }
}
