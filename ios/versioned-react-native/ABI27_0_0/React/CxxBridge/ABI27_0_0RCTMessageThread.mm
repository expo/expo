...
#include "ABI27_0_0RCTMessageThread.h"
...
ABI27_0_0RCTMessageThread::ABI27_0_0RCTMessageThread(NSRunLoop *runLoop, ABI27_0_0RCTJavaScriptCompleteBlock errorBlock)
...
ABI27_0_0RCTMessageThread::~ABI27_0_0RCTMessageThread() {
  CFRelease(m_cfRunLoop);
}

// This is analogous to dispatch_async
void ABI27_0_0RCTMessageThread::runAsync(std::function<void()> func) {
...
void ABI27_0_0RCTMessageThread::runSync(std::function<void()> func) {
...
void ABI27_0_0RCTMessageThread::tryFunc(const std::function<void()>& func) {
...
void ABI27_0_0RCTMessageThread::runOnQueue(std::function<void()>&& func) {
  if (m_shutdown) {
    return;
  }
...
void ABI27_0_0RCTMessageThread::runOnQueueSync(std::function<void()>&& func) {
  if (m_shutdown) {
    return;
  }
...
void ABI27_0_0RCTMessageThread::quitSynchronous() {
  m_shutdown = true;
  runSync([]{});
  CFRunLoopStop(m_cfRunLoop);
}

void ABI27_0_0RCTMessageThread::setRunLoop(NSRunLoop *runLoop) {
  CFRelease(m_cfRunLoop);
  m_cfRunLoop = [runLoop getCFRunLoop];
  CFRetain(m_cfRunLoop);
}
