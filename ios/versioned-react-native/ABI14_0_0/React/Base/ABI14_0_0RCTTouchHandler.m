...
@implementation ABI14_0_0RCTTouchHandler
{
  __weak ABI14_0_0RCTEventDispatcher *_eventDispatcher;

  /**
   * Arrays managed in parallel tracking native touch object along with the
   * native view that was touched, and the ReactABI14_0_0 touch data dictionary.
  ...
  NSMutableArray<NSMutableDictionary *> *_ReactABI14_0_0Touches;
...
{
  ABI14_0_0RCTAssertParam(bridge);

  if ((self = [super initWithTarget:self action:@selector(handleGestureUpdate:)])) {

    _eventDispatcher = [bridge moduleForClass:[ABI14_0_0RCTEventDispatcher class]];
    _dispatchedInitialTouches = NO;
    _nativeTouches = [NSMutableOrderedSet new];
    _ReactABI14_0_0Touches = [NSMutableArray new];
    _touchViews = [NSMutableArray new];

    // `cancelsTouchesInView` is needed in order to be used as a top level
    // event delegated recognizer. Otherwise, lower-level components not built
    // using ABI14_0_0RCT, will fail to recognize gestures.
    self.cancelsTouchesInView = NO;
  }
  return self;
}

ABI14_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithTarget:(id)target action:(SEL)action)

typedef NS_ENUM(NSInteger, ABI14_0_0RCTTouchEventType) {
  ABI14_0_0RCTTouchEventTypeStart,
  ABI14_0_0RCTTouchEventTypeMove,
  ABI14_0_0RCTTouchEventTypeEnd,
  ABI14_0_0RCTTouchEventTypeCancel
};
...
- (void)_recordNewTouches:(NSSet<UITouch *> *)touches
{
  for (UITouch *touch in touches) {
    ...
    ABI14_0_0RCTAssert(![_nativeTouches containsObject:touch],
              @"Touch is already recorded. This is a critical bug.");

    // Find closest ReactABI14_0_0-managed touchable view
    UIView *targetView = touch.view;
    while (targetView) {
      if (targetView.ReactABI14_0_0Tag && targetView.userInteractionEnabled &&
          [targetView ReactABI14_0_0RespondsToTouch:touch]) {
        break;
      }
      targetView = targetView.superview;
    }

    NSNumber *ReactABI14_0_0Tag = [targetView ReactABI14_0_0TagAtPoint:[touch locationInView:targetView]];
    if (!ReactABI14_0_0Tag || !targetView.userInteractionEnabled) {
      return;
    }

    // Get new, unique touch identifier for the ReactABI14_0_0 touch
    const NSUInteger ABI14_0_0RCTMaxTouches = 11; // This is the maximum supported by iDevices
    NSInteger touchID = ([_ReactABI14_0_0Touches.lastObject[@"identifier"] integerValue] + 1) % ABI14_0_0RCTMaxTouches;
    for (NSDictionary *ReactABI14_0_0Touch in _ReactABI14_0_0Touches) {
      NSInteger usedID = [ReactABI14_0_0Touch[@"identifier"] integerValue];
      if (usedID == touchID) {
    ...
    // Create touch
    NSMutableDictionary *ReactABI14_0_0Touch = [[NSMutableDictionary alloc] initWithCapacity:ABI14_0_0RCTMaxTouches];
    ReactABI14_0_0Touch[@"target"] = ReactABI14_0_0Tag;
    ReactABI14_0_0Touch[@"identifier"] = @(touchID);

    // Add to arrays
    [_touchViews addObject:targetView];
    [_nativeTouches addObject:touch];
    [_ReactABI14_0_0Touches addObject:ReactABI14_0_0Touch];
  }
}
...
- (void)_recordRemovedTouches:(NSSet<UITouch *> *)touches
{
  for (UITouch *touch in touches) {
    ...
    [_ReactABI14_0_0Touches removeObjectAtIndex:index];
  }
}

- (void)_updateReactABI14_0_0TouchAtIndex:(NSInteger)touchIndex
{
  ...
  NSMutableDictionary *ReactABI14_0_0Touch = _ReactABI14_0_0Touches[touchIndex];
  ReactABI14_0_0Touch[@"pageX"] = @(rootViewLocation.x);
  ReactABI14_0_0Touch[@"pageY"] = @(rootViewLocation.y);
  ReactABI14_0_0Touch[@"locationX"] = @(touchViewLocation.x);
  ReactABI14_0_0Touch[@"locationY"] = @(touchViewLocation.y);
  ReactABI14_0_0Touch[@"timestamp"] =  @(nativeTouch.timestamp * 1000); // in ms, for JS
  ...
  if (ABI14_0_0RCTForceTouchAvailable()) {
    ReactABI14_0_0Touch[@"force"] = @(ABI14_0_0RCTZeroIfNaN(nativeTouch.force / nativeTouch.maximumPossibleForce));
  }
}
                  ...
                  originatingTime:(__unused CFTimeInterval)originatingTime
{
  ...
  for (UITouch *touch in touches) {
    ...
    [self _updateReactABI14_0_0TouchAtIndex:index];
  ...
  NSMutableArray<NSDictionary *> *ReactABI14_0_0Touches =
    [[NSMutableArray alloc] initWithCapacity:_ReactABI14_0_0Touches.count];
  for (NSDictionary *touch in _ReactABI14_0_0Touches) {
    [ReactABI14_0_0Touches addObject:[touch copy]];
  }

  ABI14_0_0RCTTouchEvent *event = [[ABI14_0_0RCTTouchEvent alloc] initWithEventName:eventName
                                                         ReactABI14_0_0Tag:self.view.ReactABI14_0_0Tag
                                                     ReactABI14_0_0Touches:ReactABI14_0_0Touches
...
static BOOL ABI14_0_0RCTAllTouchesAreCancelledOrEnded(NSSet<UITouch *> *touches)
{
  for (UITouch *touch in touches) {
...
static BOOL ABI14_0_0RCTAnyTouchesChanged(NSSet<UITouch *> *touches)
{
  for (UITouch *touch in touches) {
    if (touch.phase == UITouchPhaseBegan ||
        touch.phase == UITouchPhaseMoved) {
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  if (_dispatchedInitialTouches) {
    ...
    if (ABI14_0_0RCTAllTouchesAreCancelledOrEnded(event.allTouches)) {
      self.state = UIGestureRecognizerStateEnded;
    } else if (ABI14_0_0RCTAnyTouchesChanged(event.allTouches)) {
      self.state = UIGestureRecognizerStateChanged;
    }
  }
