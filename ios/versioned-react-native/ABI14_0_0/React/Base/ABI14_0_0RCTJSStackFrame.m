...
static NSRegularExpression *ABI14_0_0RCTJSStackFrameRegex()
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    if (regexError) {
      ABI14_0_0RCTLogError(@"Failed to build regex: %@", [regexError localizedDescription]);
    }
  });
  return _regex;
}

@implementation ABI14_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI14_0_0RCTNullIfNil(self.methodName),
     @"file": ABI14_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI14_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
+ (instancetype)stackFrameWithDictionary:(NSDictionary *)dict
{
  return [[self alloc] initWithMethodName:dict[@"methodName"]
                                     file:dict[@"file"]
                               lineNumber:[ABI14_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI14_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}

+ (NSArray<ABI14_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI14_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI14_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI14_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
