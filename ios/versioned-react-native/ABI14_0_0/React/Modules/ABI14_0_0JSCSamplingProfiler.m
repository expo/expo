...
@implementation ABI14_0_0JSCSamplingProfiler

@synthesize methodQueue = _methodQueue;
@synthesize bridge = _bridge;

ABI14_0_0RCT_EXPORT_MODULE(JSCSamplingProfiler);

#ifdef ABI14_0_0RCT_PROFILE
ABI14_0_0RCT_EXPORT_METHOD(operationComplete:(int)token result:(id)profileData error:(id)error)
{
  if (error) {
    ABI14_0_0RCTLogError(@"JSC Sampling profiler ended with error: %@", error);
    return;
  }
  ...
  if (connection) {
    ABI14_0_0RCTLogInfo(@"JSC CPU Profile data sent successfully.");
  } else {
    ABI14_0_0RCTLogWarn(@"JSC CPU Profile data failed to send.");
  }
}
