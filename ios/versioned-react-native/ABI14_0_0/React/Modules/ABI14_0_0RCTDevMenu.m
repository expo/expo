...
#import "ABI14_0_0RCTWebSocketObserverProtocol.h"

#if ABI14_0_0RCT_DEV

static NSString *const ABI14_0_0RCTShowDevMenuNotification = @"ABI14_0_0RCTShowDevMenuNotification";
static NSString *const ABI14_0_0RCTDevMenuSettingsKey = @"ABI14_0_0RCTDevMenu";

@implementation UIWindow (ABI14_0_0RCTDevMenu)

- (void)ABI14_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI14_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

typedef NS_ENUM(NSInteger, ABI14_0_0RCTDevMenuType) {
  ABI14_0_0RCTDevMenuTypeButton,
  ABI14_0_0RCTDevMenuTypeToggle
};

@interface ABI14_0_0RCTDevMenuItem ()

@property (nonatomic, assign, readonly) ABI14_0_0RCTDevMenuType type;
@property (nonatomic, copy, readonly) NSString *key;
@property (nonatomic, copy) id value;

@end

@implementation ABI14_0_0RCTDevMenuItem
{
  id _handler; // block
...
- (instancetype)initWithType:(ABI14_0_0RCTDevMenuType)type
                         key:(NSString *)key
...
- (NSString *)title
{
  if (_type == ABI14_0_0RCTDevMenuTypeToggle && [_value boolValue]) {
...
{
  return [[self alloc] initWithType:ABI14_0_0RCTDevMenuTypeButton
                                key:nil
                              title:title
                      selectedTitle:nil
                            handler:handler];
...
{
  return [[self alloc] initWithType:ABI14_0_0RCTDevMenuTypeToggle
                                key:key
                              title:title
...
{
  switch (_type) {
    case ABI14_0_0RCTDevMenuTypeButton: {
      if (_handler) {
        ((void(^)())_handler)();
      }
      break;
    }
    case ABI14_0_0RCTDevMenuTypeToggle: {
      if (_handler) {
...
typedef void(^ABI14_0_0RCTDevMenuAlertActionHandler)(UIAlertAction *action);

@interface ABI14_0_0RCTDevMenu () <ABI14_0_0RCTBridgeModule, ABI14_0_0RCTInvalidating, ABI14_0_0RCTWebSocketObserverDelegate>

@property (nonatomic, strong) Class executorClass;

@end

@implementation ABI14_0_0RCTDevMenu
{
  ...
  BOOL _jsLoaded;
  NSMutableArray<ABI14_0_0RCTDevMenuItem *> *_extraMenuItems;
  ...
  if ((self = [super init])) {
    ...
    [notificationCenter addObserver:self
                           selector:@selector(showOnShake)
                               name:ABI14_0_0RCTShowDevMenuNotification
                             object:nil];
    ...
    _settings = [[NSMutableDictionary alloc] initWithDictionary:[_defaults objectForKey:ABI14_0_0RCTDevMenuSettingsKey]];
    _extraMenuItems = [NSMutableArray new];

    __weak ABI14_0_0RCTDevMenu *weakSelf = self;

    [_extraMenuItems addObject:[ABI14_0_0RCTDevMenuItem toggleItemWithKey:@"showInspector"
...
// TODO: Move non-UI logic into separate ABI14_0_0RCTDevSettings module
- (void)connectPackager
{
  ...
  Class webSocketObserverClass = objc_lookUpClass("ABI14_0_0RCTWebSocketObserver");
  if (webSocketObserverClass == Nil) {
    return;
  }

  // If multiple ABI14_0_0RCTDevMenus are created, the most recently connected one steals the ABI14_0_0RCTWebSocketObserver.
  // (Why this behavior exists is beyond me, as of this writing.)
  static NSMutableDictionary<NSString *, id<ABI14_0_0RCTWebSocketObserver>> *observers = nil;
  if (observers == nil) {
    observers = [NSMutableDictionary new];
  }

  NSString *key = [url absoluteString];
  id<ABI14_0_0RCTWebSocketObserver> existingObserver = observers[key];
  if (existingObserver) {
    existingObserver.delegate = self;
  } else {
    id<ABI14_0_0RCTWebSocketObserver> newObserver = [(id<ABI14_0_0RCTWebSocketObserver>)[webSocketObserverClass alloc] initWithURL:url];
  ...
  if ([target isEqualToString:@"bridge"]) {
    if ([action isEqualToString:@"reload"]) {
      if ([options[@"debug"] boolValue]) {
        _bridge.executorClass = objc_lookUpClass("ABI14_0_0RCTWebSocketExecutor");
      }
      [_bridge reload];
    }
  }
...
- (void)settingsDidChange
{
  ...
  __weak ABI14_0_0RCTDevMenu *weakSelf = self;
  NSDictionary *settings = [_defaults objectForKey:ABI14_0_0RCTDevMenuSettingsKey];
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateSettings:(NSDictionary *)settings
{
  ...
  for (ABI14_0_0RCTDevMenuItem *item in _extraMenuItems) {
    if (item.key) {
...
- (void)updateSetting:(NSString *)name value:(id)value
{
  ...
  [_defaults setObject:_settings forKey:ABI14_0_0RCTDevMenuSettingsKey];
...
{
  [self addItem:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
- (NSArray<ABI14_0_0RCTDevMenuItem *> *)menuItems
{
  NSMutableArray<ABI14_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items

  __weak ABI14_0_0RCTDevMenu *weakSelf = self;

  [items addObject:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload" handler:^{
    [weakSelf reload];
  }]];

  Class jsDebuggingExecutorClass = objc_lookUpClass("ABI14_0_0RCTWebSocketExecutor");
  if (!jsDebuggingExecutorClass) {
    [items addObject:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", _webSocketExecutorName] handler:^{
      UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", self->_webSocketExecutorName]
                                                                               message:[NSString stringWithFormat:@"You need to include the ABI14_0_0RCTWebSocket library to enable %@ debugging", self->_webSocketExecutorName]
                                                                        preferredStyle:UIAlertControllerStyleAlert];

      [ABI14_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
    }]];
  } else {
    ...
    [items addObject:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:debugTitleJS handler:^{
  ...
  if (_liveReloadURL) {
    ...
    [items addObject:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:liveReloadTitle handler:^{
    ...
    NSString *profilingTitle  = ABI14_0_0RCTProfileIsProfiling() ? @"Stop Systrace" : @"Start Systrace";
    [items addObject:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:profilingTitle handler:^{
      __typeof(self) strongSelf = weakSelf;
      if (strongSelf) {
  ...
  if ([self hotLoadingAvailable]) {
    ...
    [items addObject:[ABI14_0_0RCTDevMenuItem buttonItemWithTitle:hotLoadingTitle handler:^{
      __typeof(self) strongSelf = weakSelf;
      if (strongSelf) {
...
ABI14_0_0RCT_EXPORT_METHOD(show)
{
  if (_actionSheet || !_bridge || ABI14_0_0RCTRunningInAppExtension()) {
    return;
  }

  NSString *title = [NSString stringWithFormat:@"ReactABI14_0_0 Native: Development (%@)", [_bridge class]];
  ...
  NSArray<ABI14_0_0RCTDevMenuItem *> *items = [self menuItems];
  for (ABI14_0_0RCTDevMenuItem *item in items) {
  ...
  [ABI14_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:nil];
}

- (ABI14_0_0RCTDevMenuAlertActionHandler)alertActionHandlerForDevItem:(ABI14_0_0RCTDevMenuItem *__nullable)item
{
...
ABI14_0_0RCT_EXPORT_METHOD(debugRemotely:(BOOL)enableDebug)
{
  Class jsDebuggingExecutorClass = NSClassFromString(@"ABI14_0_0RCTWebSocketExecutor");
...
ABI14_0_0RCT_EXPORT_METHOD(setProfilingEnabled:(BOOL)enabled)
{
  _profilingEnabled = enabled;
  [self updateSetting:@"profilingEnabled" value:@(_profilingEnabled)];

  if (_liveReloadURL && enabled != ABI14_0_0RCTProfileIsProfiling()) {
    if (enabled) {
      [_bridge startProfiling];
    } else {
      [_bridge stopProfiling:^(NSData *logData) {
        ABI14_0_0RCTProfileSendResult(self->_bridge, @"systrace", logData);
      }];
    }
  }
}

ABI14_0_0RCT_EXPORT_METHOD(setLiveReloadEnabled:(BOOL)enabled)
{
...
ABI14_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled:(BOOL)enabled)
{
  ...
  if (ABI14_0_0RCTGetURLQueryParam(_bridge.bundleURL, @"hot").boolValue != actuallyEnabled) {
    _bridge.bundleURL = ABI14_0_0RCTURLByReplacingQueryParam(_bridge.bundleURL, @"hot",
...
- (void)checkForUpdates
{
  ...
  _updateTask = [[NSURLSession sharedSession] dataTaskWithURL:_liveReloadURL completionHandler:
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI14_0_0RCTDevMenu *strongSelf = weakSelf;
...
- (void)addItem:(ABI14_0_0RCTDevMenu *)item {}
...
@implementation  ABI14_0_0RCTBridge (ABI14_0_0RCTDevMenu)

- (ABI14_0_0RCTDevMenu *)devMenu
{
#if ABI14_0_0RCT_DEV
  return [self moduleForClass:[ABI14_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
