...
@interface ABI14_0_0RCTScrollEvent : NSObject <ABI14_0_0RCTEvent>
...
@implementation ABI14_0_0RCTScrollEvent
{
...
{
  ABI14_0_0RCTAssertParam(ReactABI14_0_0Tag);
...
- (ABI14_0_0RCTScrollEvent *)coalesceWithEvent:(ABI14_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI14_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI14_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI14_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, copy) NSIndexSet *stickyHeaderIndices;
@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI14_0_0RCTRefreshControl *rctRefreshControl;
#endif

@end


@implementation ABI14_0_0RCTCustomScrollView
{
...
- (UIView *)contentView
{
  return ((ABI14_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI14_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI14_0_0RCTUIManager JSResponder] isKindOfClass:[ABI14_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI14_0_0 scroll
...
- (void)dockClosestSectionHeader
{
  ...
  NSUInteger subviewCount = contentView.ReactABI14_0_0Subviews.count;
  [_stickyHeaderIndices enumerateIndexesWithOptions:0 usingBlock:
   ^(NSUInteger idx, BOOL *stop) {
    ...
    UIView *header = contentView.ReactABI14_0_0Subviews[idx];
...
- (void)setBounds:(CGRect)bounds
{
  if (isRectInvalid(bounds)) {
    ABI14_0_0RCTLogError(@"Attempted to set an invalid bounds to inner scrollview: %@", NSStringFromCGRect(bounds));
    return;
  }
...
- (void)setFrame:(CGRect)frame
{
  if (isRectInvalid(frame)) {
    ABI14_0_0RCTLogError(@"Attempted to set an invalid frame to inner scrollview: %@", NSStringFromCGRect(frame));
    return;
  }
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI14_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI14_0_0RCTScrollView
{
  ABI14_0_0RCTEventDispatcher *_eventDispatcher;
  ABI14_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI14_0_0RCTEventDispatcher *)eventDispatcher
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _scrollView = [[ABI14_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI14_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI14_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI14_0_0RCTAssert(_contentView == nil, @"ABI14_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI14_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI14_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI14_0_0Subview:atIndex:`
}
...
{
  ABI14_0_0RCTAssert(_scrollView.contentSize.width <= self.frame.size.width,
...
- (void)layoutSubviews
{
  ...
  ABI14_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI14_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI14_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI14_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI14_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI14_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI14_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI14_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI14_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI14_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI14_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI14_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI14_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI14_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI14_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI14_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI14_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI14_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI14_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI14_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI14_0_0RCTView contentInsetsForView:self];
...
- (void)ReactABI14_0_0BridgeDidFinishTransaction
{
  ...
  if (ABI14_0_0RCT_DEBUG) {
    ...
    NSUInteger subviewCount = _scrollView.contentView.ReactABI14_0_0Subviews.count;
    ...
    if (lastIndex != NSNotFound && lastIndex >= subviewCount) {
      ABI14_0_0RCTLogWarn(@"Sticky header index %zd was outside the range {0, %zd}",
...
#define ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI14_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI14_0_0RCTScrollEvent *scrollEvent = [[ABI14_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI14_0_0RCTEventDispatcher (ABI14_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI14_0_0Tag
{
  ...
  ABI14_0_0RCTScrollEvent *fakeScrollEvent = [[ABI14_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI14_0_0Tag:ReactABI14_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
