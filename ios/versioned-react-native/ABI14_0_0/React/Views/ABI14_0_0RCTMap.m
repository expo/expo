...
const CLLocationDegrees ABI14_0_0RCTMapDefaultSpan = 0.005;
const NSTimeInterval ABI14_0_0RCTMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI14_0_0RCTMapZoomBoundBuffer = 0.01;

@implementation ABI14_0_0RCTMap
{
  UIView *_legalLabel;
...
- (void)setAnnotations:(NSArray<ABI14_0_0RCTMapAnnotation *> *)annotations
{
  NSMutableArray<NSString *> *newAnnotationIDs = [NSMutableArray new];
  NSMutableArray<ABI14_0_0RCTMapAnnotation *> *annotationsToDelete = [NSMutableArray new];
  NSMutableArray<ABI14_0_0RCTMapAnnotation *> *annotationsToAdd = [NSMutableArray new];

  for (ABI14_0_0RCTMapAnnotation *annotation in annotations) {
  ...
  for (ABI14_0_0RCTMapAnnotation *annotation in self.annotations) {
    if (![annotation isKindOfClass:[ABI14_0_0RCTMapAnnotation class]]) {
      continue;
    }
...
// TODO: this doesn't preserve order. Should it? If so we should change the
// algorithm. If not, it would be more efficient to use an NSSet
- (void)setOverlays:(NSArray<ABI14_0_0RCTMapOverlay *> *)overlays
{
  ...
  for (ABI14_0_0RCTMapOverlay *overlay in overlays) {
    if (![overlay isKindOfClass:[ABI14_0_0RCTMapOverlay class]]) {
      continue;
    }
  ...
  for (ABI14_0_0RCTMapOverlay *overlay in self.overlays) {
    if (![overlay isKindOfClass:[ABI14_0_0RCTMapOverlay class]]) {
      continue;
    }
