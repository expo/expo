...
#import "ABI14_0_0RCTBridgeModule.h"
#import "ABI14_0_0RCTConvert.h"
#import "ABI14_0_0RCTShadowView.h"
#import "ABI14_0_0RCTUtils.h"
#import "UIView+ReactABI14_0_0.h"

typedef void (^ABI14_0_0RCTPropBlock)(id<ABI14_0_0RCTComponent> view, id json);

@interface ABI14_0_0RCTComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) ABI14_0_0RCTPropBlock propBlock;

@end

@implementation ABI14_0_0RCTComponentProp
...
@implementation ABI14_0_0RCTComponentData
{
  id<ABI14_0_0RCTComponent> _defaultView; // Only needed for ABI14_0_0RCT_CUSTOM_VIEW_PROPERTY
  NSMutableDictionary<NSString *, ABI14_0_0RCTPropBlock> *_viewPropBlocks;
  NSMutableDictionary<NSString *, ABI14_0_0RCTPropBlock> *_shadowPropBlocks;
                              ...
                              bridge:(ABI14_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    // Hackety hack, this partially re-implements ABI14_0_0RCTBridgeModuleNameForClass
    // We want to get rid of ABI14_0_0RCT and RK prefixes, but a lot of JS code still references
    // view names by prefix. So, while ABI14_0_0RCTBridgeModuleNameForClass now drops these
    ...
    name = ABI14_0_0EX_REMOVE_VERSION(name);
    ...
    ABI14_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
    _name = name;

    _implementsUIBlockToAmendWithShadowViewRegistry = NO;
    Class cls = _managerClass;
    while (cls != [ABI14_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI14_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI14_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI14_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI14_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ABI14_0_0RCTPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, ABI14_0_0RCTPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  ABI14_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {

    __weak ABI14_0_0RCTComponentData *weakSelf = self;
    ...
    if ([_managerClass respondsToSelector:selector]) {
      ...
      type = ABI14_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<ABI14_0_0RCTComponent> view, id json) {
        ABI14_0_0RCTComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        json = ABI14_0_0RCTNilIfNull(json);
        if (!shadowView) {
      ...
      if (type == NSSelectorFromString(@"ABI14_0_0RCTBubblingEventBlock:") ||
          type == NSSelectorFromString(@"ABI14_0_0RCTDirectEventBlock:")) {

        // Special case for event handlers
        __weak ABI14_0_0RCTViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<ABI14_0_0RCTComponent> weakTarget = target;
          ((void (*)(id, SEL, id))objc_msgSend)(target, setter, [ABI14_0_0RCTConvert BOOL:json] ? ^(NSDictionary *body) {
            body = [NSMutableDictionary dictionaryWithDictionary:body];
            ((NSMutableDictionary *)body)[@"target"] = weakTarget.ReactABI14_0_0Tag;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            [weakManager.bridge.eventDispatcher sendInputEventWithName:ABI14_0_0RCTNormalizeInputEventName(name) body:body];
#pragma clang diagnostic pop
        ...
        NSMethodSignature *typeSignature = [[ABI14_0_0RCTConvert class] methodSignatureForSelector:type];
        if (!typeSignature) {
          ABI14_0_0RCTLogError(@"No +[ABI14_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
          return ^(__unused id<ABI14_0_0RCTComponent> view, __unused id json){};
        }
  ...
  #define ABI14_0_0RCT_CASE(_value, _type) \
          case _value: { \
            ...
            setterBlock = ^(id target, id json) { \
              if (json) { \
                ...
                set(target, setter, convert([ABI14_0_0RCTConvert class], type, json)); \
            ...
            ABI14_0_0RCT_CASE(_C_SEL, SEL)
            ABI14_0_0RCT_CASE(_C_CHARPTR, const char *)
            ...
            ABI14_0_0RCT_CASE(_C_PTR, void *)
            ABI14_0_0RCT_CASE(_C_ID, id)
      ...
      propBlock = ^(__unused id view, __unused id json) {
        ...
        setterBlock(target, ABI14_0_0RCTNilIfNull(json));
      };
    }
    ...
    if (ABI14_0_0RCT_DEBUG) {
      ...
      ABI14_0_0RCTPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<ABI14_0_0RCTComponent> view, id json) {
        NSString *logPrefix = [NSString stringWithFormat:
                               @"Error setting property '%@' of %@ with tag #%@: ",
                               name, weakSelf.name, view.ReactABI14_0_0Tag];

        ABI14_0_0RCTPerformBlockWithLogPrefix(^{ unwrappedBlock(view, json); }, logPrefix);
      };
    }
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI14_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI14_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI14_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
      ABI14_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                  "to '%@'", name, _name, propTypes[name], type);
    }

    if ([type isEqualToString:@"ABI14_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI14_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI14_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI14_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
  ...
  for (NSString *event in bubblingEvents) {
    if ([directEvents containsObject:event]) {
      ABI14_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
