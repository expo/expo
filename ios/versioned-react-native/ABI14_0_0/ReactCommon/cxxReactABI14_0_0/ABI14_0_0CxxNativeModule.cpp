...
#include "ABI14_0_0CxxNativeModule.h"
...
void CxxNativeModule::invoke(ExecutorToken token, unsigned int ReactABI14_0_0MethodId, folly::dynamic&& params) {
  if (ReactABI14_0_0MethodId >= methods_.size()) {
    throw std::invalid_argument(
      folly::to<std::string>("methodId ", ReactABI14_0_0MethodId, " out of range [0..", methods_.size(), "]"));
  }
  if (!params.isArray()) {
    throw std::invalid_argument(
  ...
  const auto& method = methods_[ReactABI14_0_0MethodId];
