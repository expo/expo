...
typedef ABI14_0_0RCTURLRequestCancellationBlock (^ABI14_0_0RCTHTTPQueryResult)(NSError *error, NSDictionary<NSString *, id> *result);

@interface ABI14_0_0RCTNetworking ()

- (ABI14_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(NSDictionary<NSString *, id> *)data
                                                 callback:(ABI14_0_0RCTHTTPQueryResult)callback;
@end

/**
 * Helper to convert FormData payloads into multipart/formdata requests.
 */
@interface ABI14_0_0RCTHTTPFormDataHelper : NSObject

@property (nonatomic, weak) ABI14_0_0RCTNetworking *networker;

@end

@implementation ABI14_0_0RCTHTTPFormDataHelper
{
  NSMutableArray<NSDictionary<NSString *, id> *> *_parts;
  NSMutableData *_multipartBody;
  ABI14_0_0RCTHTTPQueryResult _callback;
  NSString *_boundary;
}

static NSString *ABI14_0_0RCTGenerateFormBoundary()
{
...
- (ABI14_0_0RCTURLRequestCancellationBlock)process:(NSArray<NSDictionary *> *)formData
                                 callback:(ABI14_0_0RCTHTTPQueryResult)callback
{
  ABI14_0_0RCTAssertThread(_networker.methodQueue, @"process: must be called on method queue");
  ...
  _boundary = ABI14_0_0RCTGenerateFormBoundary();
...
- (ABI14_0_0RCTURLRequestCancellationBlock)handleResult:(NSDictionary<NSString *, id> *)result
                                         error:(NSError *)error
{
  ABI14_0_0RCTAssertThread(_networker.methodQueue, @"handleResult: must be called on method queue");
 ...
 */
@implementation ABI14_0_0RCTNetworking
{
  NSMutableDictionary<NSNumber *, ABI14_0_0RCTNetworkTask *> *_tasksByRequestID;
  std::mutex _handlersLock;
  NSArray<id<ABI14_0_0RCTURLRequestHandler>> *_handlers;
}
...
- (id<ABI14_0_0RCTURLRequestHandler>)handlerForRequest:(NSURLRequest *)request
{
  if (!request.URL) {
    return nil;
  }

  {
    ...
    if (!_handlers) {
      ...
      _handlers = [[self.bridge modulesConformingToProtocol:@protocol(ABI14_0_0RCTURLRequestHandler)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI14_0_0RCTURLRequestHandler> a, id<ABI14_0_0RCTURLRequestHandler> b) {
  ...
  if (ABI14_0_0RCT_DEBUG) {
    ...
    id<ABI14_0_0RCTURLRequestHandler> previousHandler = nil;
    for (id<ABI14_0_0RCTURLRequestHandler> handler in _handlers) {
      ...
      if ([handler canHandleRequest:request]) {
        if (previousHandler) {
          if (priority == previousPriority) {
            ABI14_0_0RCTLogError(@"The ABI14_0_0RCTURLRequestHandlers %@ and %@ both reported that"
...
- (ABI14_0_0RCTURLRequestCancellationBlock)buildRequest:(NSDictionary<NSString *, id> *)query
                                 completionBlock:(void (^)(NSURLRequest *request))block
{
  ABI14_0_0RCTAssertThread(_methodQueue, @"buildRequest: must be called on method queue");

  NSURL *URL = [ABI14_0_0RCTConvert NSURL:query[@"url"]]; // this is marked as nullable in JS, but should not be null
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:URL];
  request.HTTPMethod = [ABI14_0_0RCTConvert NSString:ABI14_0_0RCTNilIfNull(query[@"method"])].uppercaseString ?: @"GET";
  request.allHTTPHeaderFields = [self stripNullsInRequestHeaders:[ABI14_0_0RCTConvert NSDictionary:query[@"headers"]]];
  request.timeoutInterval = [ABI14_0_0RCTConvert NSTimeInterval:query[@"timeout"]];
  NSDictionary<NSString *, id> *data = [ABI14_0_0RCTConvert NSDictionary:ABI14_0_0RCTNilIfNull(query[@"data"])];
  ...
  return [self processDataForHTTPQuery:data callback:^(NSError *error, NSDictionary<NSString *, id> *result) {
    if (error) {
      ABI14_0_0RCTLogError(@"Error processing request body: %@", error);
      // Ideally we'd circle back to JS here and notify an error/abort on the request.
      return (ABI14_0_0RCTURLRequestCancellationBlock)nil;
    }
    ...
    if ([request.allHTTPHeaderFields[@"Content-Encoding"] isEqualToString:@"gzip"]) {
      request.HTTPBody = ABI14_0_0RCTGzipData(request.HTTPBody, -1 /* default */);
 ...
 *
 */
- (ABI14_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(nullable NSDictionary<NSString *, id> *)query callback:
(ABI14_0_0RCTURLRequestCancellationBlock (^)(NSError *error, NSDictionary<NSString *, id> *result))callback
{
  ABI14_0_0RCTAssertThread(_methodQueue, @"processDataForHTTPQuery: must be called on method queue");

  if (!query) {
    return callback(nil, nil);
  }
  NSData *body = [ABI14_0_0RCTConvert NSData:query[@"string"]];
  if (body) {
    return callback(nil, @{@"body": body});
  }
  NSURLRequest *request = [ABI14_0_0RCTConvert NSURLRequest:query[@"uri"]];
  if (request) {

    __block ABI14_0_0RCTURLRequestCancellationBlock cancellationBlock = nil;
    ABI14_0_0RCTNetworkTask *task = [self networkTaskWithRequest:request completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
      dispatch_async(self->_methodQueue, ^{
        cancellationBlock = callback(error, data ? @{@"body": data, @"contentType": ABI14_0_0RCTNullIfNil(response.MIMEType)} : nil);
      });
    }];

    [task start];

    __weak ABI14_0_0RCTNetworkTask *weakTask = task;
    return ^{
      [weakTask cancel];
  ...
  NSArray<NSDictionary *> *formData = [ABI14_0_0RCTConvert NSDictionaryArray:query[@"formData"]];
  if (formData) {
    ABI14_0_0RCTHTTPFormDataHelper *formDataHelper = [ABI14_0_0RCTHTTPFormDataHelper new];
...
- (void)sendData:(NSData *)data
    responseType:(NSString *)responseType
         forTask:(ABI14_0_0RCTNetworkTask *)task
{
  ABI14_0_0RCTAssertThread(_methodQueue, @"sendData: must be called on method queue");
  ...
  if ([responseType isEqualToString:@"text"]) {
    ...
    responseString = [ABI14_0_0RCTNetworking decodeTextData:data fromResponse:task.response withCarryData:nil];
    if (!responseString) {
      ABI14_0_0RCTLogWarn(@"Received data was not a string, or was not a recognised encoding.");
      return;
    }
  } else if ([responseType isEqualToString:@"base64"]) {
    responseString = [data base64EncodedStringWithOptions:0];
  } else {
    ABI14_0_0RCTLogWarn(@"Invalid responseType: %@", responseType);
    return;
  }
...
- (void)sendRequest:(NSURLRequest *)request
 ...
 incrementalUpdates:(BOOL)incrementalUpdates
     responseSender:(ABI14_0_0RCTResponseSenderBlock)responseSender
{
  ABI14_0_0RCTAssertThread(_methodQueue, @"sendRequest: must be called on method queue");

  __block ABI14_0_0RCTNetworkTask *task;
  ABI14_0_0RCTURLRequestProgressBlock uploadProgressBlock = ^(int64_t progress, int64_t total) {
    NSArray *responseJSON = @[task.requestID, @((double)progress), @((double)total)];
    [self sendEventWithName:@"didSendNetworkData" body:responseJSON];
  };

  ABI14_0_0RCTURLRequestResponseBlock responseBlock = ^(NSURLResponse *response) {
  ...
  ABI14_0_0RCTURLRequestIncrementalDataBlock incrementalDataBlock = nil;
  ABI14_0_0RCTURLRequestProgressBlock downloadProgressBlock = nil;
  if (incrementalUpdates) {
    if ([responseType isEqualToString:@"text"]) {
      ...
      incrementalDataBlock = ^(NSData *data, int64_t progress, int64_t total) {
        ...
        NSString *responseString = [ABI14_0_0RCTNetworking decodeTextData:data
  ...
  ABI14_0_0RCTURLRequestCompletionBlock completionBlock =
  ^(NSURLResponse *response, NSData *data, NSError *error) {
    ...
    NSArray *responseJSON = @[task.requestID,
                              ABI14_0_0RCTNullIfNil(error.localizedDescription),
...
- (ABI14_0_0RCTNetworkTask *)networkTaskWithRequest:(NSURLRequest *)request completionBlock:(ABI14_0_0RCTURLRequestCompletionBlock)completionBlock
{
  id<ABI14_0_0RCTURLRequestHandler> handler = [self handlerForRequest:request];
  if (!handler) {
    ABI14_0_0RCTLogError(@"No suitable URL request handler found for %@", request.URL);
    return nil;
  }

  ABI14_0_0RCTNetworkTask *task = [[ABI14_0_0RCTNetworkTask alloc] initWithRequest:request
                                                         handler:handler
...
ABI14_0_0RCT_EXPORT_METHOD(sendRequest:(NSDictionary *)query
                  responseSender:(ABI14_0_0RCTResponseSenderBlock)responseSender)
{
  ...
  [self buildRequest:query completionBlock:^(NSURLRequest *request) {
    NSString *responseType = [ABI14_0_0RCTConvert NSString:query[@"responseType"]];
    BOOL incrementalUpdates = [ABI14_0_0RCTConvert BOOL:query[@"incrementalUpdates"]];
...
ABI14_0_0RCT_EXPORT_METHOD(abortRequest:(nonnull NSNumber *)requestID)
{
  [_tasksByRequestID[requestID] cancel];
  [_tasksByRequestID removeObjectForKey:requestID];
}

ABI14_0_0RCT_EXPORT_METHOD(clearCookies:(ABI14_0_0RCTResponseSenderBlock)responseSender)
{
...
@implementation ABI14_0_0RCTBridge (ABI14_0_0RCTNetworking)

- (ABI14_0_0RCTNetworking *)networking
{
  return [self moduleForClass:[ABI14_0_0RCTNetworking class]];
}
