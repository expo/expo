...
#import "ABI14_0_0RCTTestRunner.h"
...
@implementation ABI14_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI14_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI14_0_0RCTAssertParam(app);
  ABI14_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI14_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI14_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI14_0_0RCTSetLogFunction(^(ABI14_0_0RCTLogLevel level, ABI14_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI14_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI14_0_0RCTBridge *bridge = [[ABI14_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI14_0_0RCTRootView *rootView = [[ABI14_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI14_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI14_0_0RCTTestModule class]];
    ABI14_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI14_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI14_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI14_0_0RCTSetLogFunction(ABI14_0_0RCTDefaultLogFunction);
    ...
    ABI14_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI14_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI14_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI14_0_0RCTAssert(testModule.status != ABI14_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI14_0_0RCTAssert(testModule.status == ABI14_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI14_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
