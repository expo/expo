...
#import "ABI14_0_0RCTTextSelection.h"

@interface ABI14_0_0RCTUITextView : UITextView

@property (nonatomic, assign) BOOL textWasPasted;

@end

@implementation ABI14_0_0RCTUITextView
{
...
- (void)didMoveToWindow
{
  if (_jsRequestingFirstResponder) {
    ...
    [self ReactABI14_0_0DidMakeFirstResponder];
  }
}

@end

@implementation ABI14_0_0RCTTextView
{
  ABI14_0_0RCTEventDispatcher *_eventDispatcher;

  NSString *_placeholder;
  UITextView *_placeholderView;
  UITextView *_textView;
  ABI14_0_0RCTText *_richTextView;
...
- (instancetype)initWithEventDispatcher:(ABI14_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI14_0_0RCTAssertParam(eventDispatcher);
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _textView = [[ABI14_0_0RCTUITextView alloc] initWithFrame:CGRectZero];
...
- (void)insertReactABI14_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI14_0_0Subview:subview atIndex:index];
  if ([subview isKindOfClass:[ABI14_0_0RCTText class]]) {
    if (_richTextView) {
      ABI14_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI14_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI14_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI14_0_0RCTReactABI14_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI14_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI14_0_0TagFromString(_pendingAttributedText);
...
- (void)updateFrames
{
  ...
  // ABI14_0_0RCTTextField defaults, using the system defaults of font size 17 and a
  // height of 31 points.
  //
...
- (BOOL)textView:(ABI14_0_0RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (textView.textWasPasted) {
    textView.textWasPasted = NO;
  } else {
    [_eventDispatcher sendTextEventWithType:ABI14_0_0RCTTextEventTypeKeyPress
    ...
    if (_blurOnSubmit && [text isEqualToString:@"\n"]) {
      // TODO: the purpose of blurOnSubmit on ABI14_0_0RCTextField is to decide if the
      // field should lose focus when return is pressed or not. We're cheating a
      // bit here by using it on ABI14_0_0RCTextView to decide if return character should
      ...
      [_eventDispatcher sendTextEventWithType:ABI14_0_0RCTTextEventTypeSubmit
                                     ReactABI14_0_0Tag:self.ReactABI14_0_0Tag
                                         text:self.text
                                          key:nil
                                   eventCount:_nativeEventCount];
...
- (void)textViewDidChangeSelection:(ABI14_0_0RCTUITextView *)textView
{
...
- (void)setSelection:(ABI14_0_0RCTTextSelection *)selection
{
  if (!selection) {
    return;
  }
  ...
  } else if (eventLag > ABI14_0_0RCTTextUpdateLagWarningThreshold) {
    ABI14_0_0RCTLogWarn(@"Native TextInput(%@) is %zd events ahead of JS - try to make your JS faster.", self.text, eventLag);
  }
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
  ...
  [_eventDispatcher sendTextEventWithType:ABI14_0_0RCTTextEventTypeFocus
                                 ReactABI14_0_0Tag:self.ReactABI14_0_0Tag
                                     text:nil
                                      key:nil
                               eventCount:_nativeEventCount];
...
- (void)textViewDidChange:(UITextView *)textView
{
  ...
  if (!self.ReactABI14_0_0Tag || !_onChange) {
    return;
  }
...
{
  [_eventDispatcher sendTextEventWithType:ABI14_0_0RCTTextEventTypeEnd
                                 ReactABI14_0_0Tag:self.ReactABI14_0_0Tag
                                     text:textView.text
                                      key:nil
                               eventCount:_nativeEventCount];

  [_eventDispatcher sendTextEventWithType:ABI14_0_0RCTTextEventTypeBlur
                                 ReactABI14_0_0Tag:self.ReactABI14_0_0Tag
                                     text:nil
                                      key:nil
                               eventCount:_nativeEventCount];
...
- (void)ReactABI14_0_0WillMakeFirstResponder
{
  [_textView ReactABI14_0_0WillMakeFirstResponder];
}
...
- (void)ReactABI14_0_0DidMakeFirstResponder
{
  [_textView ReactABI14_0_0DidMakeFirstResponder];
}
