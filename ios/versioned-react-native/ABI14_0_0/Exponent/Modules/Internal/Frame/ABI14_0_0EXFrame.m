...
#define ABI14_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI14_0_0EXFrameReactABI14_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI14_0_0EXFrameReactABI14_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI14_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI14_0_0Bridge;

@end

@interface ABI14_0_0EXFrame () <ABI14_0_0RCTInvalidating, ABI14_0_0RCTExceptionsManagerDelegate>
...
@implementation ABI14_0_0EXFrame

ABI14_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI14_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI14_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI14_0_0RootView]) {
    [_appManager ReactABI14_0_0RootView].frame = self.bounds;
  }
}
...
#pragma mark - ABI14_0_0EXReactABI14_0_0AppManagerDelegate

- (void)ReactABI14_0_0AppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI14_0_0RootView = [appManager ReactABI14_0_0RootView];
  ReactABI14_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI14_0_0RootView];
}

- (void)ReactABI14_0_0AppManagerDidDestroyApp:(id)appManager
{
  if (_tmrReload) {
    [_tmrReload invalidate];
    _tmrReload = nil;
...
- (void)ReactABI14_0_0AppManagerStartedLoadingJavaScript:(id)appManager
{
  if (_onLoadingStart) {
    _onLoadingStart(nil);
  }
...
- (void)ReactABI14_0_0AppManagerFinishedLoadingJavaScript:(id)appManager
{
  if (_viewTestTimer) {
    [_viewTestTimer invalidate];
  }
...
- (void)ReactABI14_0_0AppManager:(id)appManager failedToLoadJavaScriptWithError:(NSError *)error
{
  [self _sendLoadingError:error];
}

- (void)ReactABI14_0_0AppManager:(id)appManager failedToDownloadBundleWithError:(NSError *)error
{
  
}

- (void)ReactABI14_0_0AppManagerDidForeground:(id)appManager
{
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI14_0_0RootView] &&
      [_appManager ReactABI14_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI14_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI14_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI14_0_0RCTExceptionsManagerDelegate
