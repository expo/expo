...
#import <ReactABI14_0_0/ABI14_0_0RCTJSCExecutor.h>

#import "ABI14_0_0EXUnversioned.h"
#import <EXGL.h>


@interface ABI14_0_0EXGLView ()

@property (nonatomic, weak) ABI14_0_0EXGLViewManager *viewManager;
...
@implementation ABI14_0_0EXGLView
...
- (instancetype)initWithManager:(ABI14_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
...
//    _displayLink.preferredFramesPerSecond = 60;
    ...
    _exglCtxId = 0;
    id<ABI14_0_0RCTJavaScriptExecutor> executor = [_viewManager.bridge valueForKey:@"javaScriptExecutor"];
    if ([executor isKindOfClass:NSClassFromString(@"ABI14_0_0RCTJSCExecutor")]) {
      // On JS thread, extract JavaScriptCore context, create ABI14_0_0EXGL context, call JS callback
      __weak __typeof__(self) weakSelf = self;
      __weak __typeof__(executor) weakExecutor = executor;
      [executor executeBlockOnJavaScriptQueue:^{
        __typeof__(self) self = weakSelf;
        ABI14_0_0RCTJSCExecutor *executor = weakExecutor;
        if (self && executor) {
      ...
      ABI14_0_0RCTLog(@"ABI14_0_0EXGL: Can only run on JavaScriptCore!");
    }
  }
  return self;
}
...
- (void)draw
{
  // _exglCtxId may be unset if we get here (on the UI thread) before ABI14_0_0EXGLContextCreate(...) is
  // called on the JS thread to create the ABI14_0_0EXGL context and save its id (see init method). In
