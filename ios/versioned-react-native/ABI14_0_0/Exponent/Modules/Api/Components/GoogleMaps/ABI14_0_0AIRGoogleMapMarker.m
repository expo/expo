...
@interface ABI14_0_0AIRGoogleMapMarker ()
- (id)eventFromMarker:(ABI14_0_0AIRGMSMarker*)marker;
@end

@implementation ABI14_0_0AIRGoogleMapMarker {
...
{
  if ((self = [super init])) {
    _realMarker = [[ABI14_0_0AIRGMSMarker alloc] init];
...
- (void)insertReactABI14_0_0Subview:(id<ABI14_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  if ([subview isKindOfClass:[ABI14_0_0AIRGoogleMapCallout class]]) {
    self.calloutView = (ABI14_0_0AIRGoogleMapCallout *)subview;
  } else { // a child view of the marker
    [self iconViewInsertSubview:(UIView*)subview atIndex:atIndex+1];
  }
  ABI14_0_0AIRDummyView *dummySubview = [[ABI14_0_0AIRDummyView alloc] initWithView:(UIView *)subview];
  [super insertReactABI14_0_0Subview:(UIView*)dummySubview atIndex:atIndex];
}

- (void)removeReactABI14_0_0Subview:(id<ABI14_0_0RCTComponent>)dummySubview {
  UIView* subview = ((ABI14_0_0AIRDummyView*)dummySubview).view;
  ...
  [super removeReactABI14_0_0Subview:(UIView*)dummySubview];
}
...
- (void)didTapInfoWindowOfMarker:(ABI14_0_0AIRGMSMarker *)marker {
...
- (void)didBeginDraggingMarker:(ABI14_0_0AIRGMSMarker *)marker {
  if (!self.onDragStart) return;
  self.onDragStart([self eventFromMarker:marker]);
}

- (void)didEndDraggingMarker:(ABI14_0_0AIRGMSMarker *)marker {
  if (!self.onDragEnd) return;
  self.onDragEnd([self eventFromMarker:marker]);
}

- (void)didDragMarker:(ABI14_0_0AIRGMSMarker *)marker {
...
- (void)setOnPress:(ABI14_0_0RCTBubblingEventBlock)onPress {
  _realMarker.onPress = onPress;
}

- (ABI14_0_0RCTBubblingEventBlock)onPress {
...
- (void)setImageSrc:(NSString *)imageSrc
{
  ...
  if (!_iconImageView) {
    // prevent glitch with marker (cf. https://github.com/airbnb/ReactABI14_0_0-native-maps/issues/738)
