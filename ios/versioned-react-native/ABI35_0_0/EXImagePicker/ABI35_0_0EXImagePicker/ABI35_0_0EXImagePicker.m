#import <ABI35_0_0EXImagePicker/ABI35_0_0EXImagePicker.h>
...
const CGFloat ABI35_0_0EXDefaultImageQuality = 0.2;

@interface ABI35_0_0EXImagePicker ()
...
@property (nonatomic, weak) id<ABI35_0_0UMPermissionsInterface> permissionsModule;
@property (nonatomic, assign) BOOL shouldRestoreStatusBarVisibility;

@end

@implementation ABI35_0_0EXImagePicker

ABI35_0_0UM_EXPORT_MODULE(ExponentImagePicker);
...
- (void)setModuleRegistry:(ABI35_0_0UMModuleRegistry *)moduleRegistry
{
  ...
  _permissionsModule = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI35_0_0UMPermissionsInterface)];
}

ABI35_0_0UM_EXPORT_METHOD_AS(launchCameraAsync, launchCameraAsync:(NSDictionary *)options
                  ...
                  rejecter:(ABI35_0_0UMPromiseRejectBlock)reject)
{
  ...
  self.reject = reject;
  [self launchImagePicker:ABI35_0_0EXImagePickerTargetCamera options:options];
}

ABI35_0_0UM_EXPORT_METHOD_AS(launchImageLibraryAsync, launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI35_0_0UMPromiseResolveBlock)resolve
                  rejecter:(ABI35_0_0UMPromiseRejectBlock)reject)
{
  ...
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI35_0_0EXImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI35_0_0EXImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI35_0_0EXImagePickerTarget)target
{
  dispatch_async(dispatch_get_main_queue(), ^{
    self.picker = [[UIImagePickerController alloc] init];

    if (target == ABI35_0_0EXImagePickerTargetCamera) {
    ...
    id<ABI35_0_0UMUtilitiesInterface> utils = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI35_0_0UMUtilitiesInterface)];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
  dispatch_block_t dismissCompletionBlock = ^{
    ...
    id<ABI35_0_0UMFileSystemInterface> fileSystem = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI35_0_0UMFileSystemInterface)];
    if (!fileSystem) {
          ...
          completionHandler:(void (^)(void))completionHandler
{
  ...
  NSData *data = UIImageJPEGRepresentation(image, quality == nil ? ABI35_0_0EXDefaultImageQuality : [quality floatValue]);
  ...
  if ([[self.options objectForKey:@"exif"] boolValue]) {
    ...
    if (metadata) {
      ...
      if (assets.count > 0) {
        ...
        ABI35_0_0UMLogInfo(@"Could not fetch metadata for image %@", [imageURL absoluteString]);
        completionHandler();
      }
    }
  } else {
