...
#import <ReactABI35_0_0/components/view/ViewShadowNode.h>
#import <ReactABI35_0_0/components/view/ViewProps.h>
#import <ReactABI35_0_0/components/view/ViewEventEmitter.h>
#import <objc/runtime.h>
#import <ReactABI35_0_0/ABI35_0_0RCTAssert.h>
#import <ReactABI35_0_0/ABI35_0_0RCTBorderDrawing.h>

#import "ABI35_0_0RCTConversions.h"

using namespace facebook::ReactABI35_0_0;

@implementation ABI35_0_0RCTViewComponentView
{
...
- (void)layoutSubviews
{
  ...
  if (_contentView) {
    _contentView.frame = ABI35_0_0RCTCGRectFromRect(_layoutMetrics.getContentFrame());
  }
}
...
{
  ABI35_0_0RCTAssert(
    self == [ABI35_0_0RCTViewComponentView class],
    @"`+[ABI35_0_0RCTComponentViewProtocol componentHandle]` must be implemented for all subclasses (and `%@` particularly).", NSStringFromClass([self class]));
           ...
           oldProps:(SharedProps)oldProps
{
  ...
  ABI35_0_0RCTAssert(
    propsRawPtr &&
    (
      [self class] == [ABI35_0_0RCTViewComponentView class] ||
      typeid(*propsRawPtr).hash_code() != typeid(const ViewProps).hash_code()
    ),
    @"`ABI35_0_0RCTViewComponentView` subclasses (and `%@` particularly) must setup `_props`"
  ...
  if (oldViewProps.backgroundColor != newViewProps.backgroundColor) {
    _backgroundColor = ABI35_0_0RCTUIColorFromSharedColor(newViewProps.backgroundColor);
    needsInvalidateLayer = YES;
  }

  // `foregroundColor`
  if (oldViewProps.foregroundColor != newViewProps.foregroundColor) {
    self.foregroundColor = ABI35_0_0RCTUIColorFromSharedColor(newViewProps.foregroundColor);
  }

  // `shadowColor`
  if (oldViewProps.shadowColor != newViewProps.shadowColor) {
    CGColorRef shadowColor = ABI35_0_0RCTCGColorRefFromSharedColor(newViewProps.shadowColor);
  ...
  if (oldViewProps.shadowOffset != newViewProps.shadowOffset) {
    self.layer.shadowOffset = ABI35_0_0RCTCGSizeFromSize(newViewProps.shadowOffset);
    needsInvalidateLayer = YES;
  }
  ...
  if (oldViewProps.transform != newViewProps.transform) {
    self.layer.transform = ABI35_0_0RCTCATransform3DFromTransformMatrix(newViewProps.transform);
    self.layer.allowsEdgeAntialiasing = newViewProps.transform != Transform::Identity();
  }

  // `hitSlop`
  if (oldViewProps.hitSlop != newViewProps.hitSlop) {
    self.hitTestEdgeInsets = ABI35_0_0RCTUIEdgeInsetsFromEdgeInsets(newViewProps.hitSlop);
  }

  // `overflow`
  if (oldViewProps.ABI35_0_0yogaStyle.overflow != newViewProps.ABI35_0_0yogaStyle.overflow) {
    self.clipsToBounds = newViewProps.ABI35_0_0yogaStyle.overflow != ABI35_0_0YGOverflowVisible;
    needsInvalidateLayer = YES;
  }
  ...
  if (oldViewProps.nativeId != newViewProps.nativeId) {
    self.nativeId = ABI35_0_0RCTNSStringFromStringNilIfEmpty(newViewProps.nativeId);
  }
  ...
  if (oldViewProps.accessibilityLabel != newViewProps.accessibilityLabel) {
    self.accessibilityElement.accessibilityLabel = ABI35_0_0RCTNSStringFromStringNilIfEmpty(newViewProps.accessibilityLabel);
  }

  // `accessibilityHint`
  if (oldViewProps.accessibilityHint != newViewProps.accessibilityHint) {
    self.accessibilityElement.accessibilityHint = ABI35_0_0RCTNSStringFromStringNilIfEmpty(newViewProps.accessibilityHint);
  }

  // `accessibilityTraits`
  if (oldViewProps.accessibilityTraits != newViewProps.accessibilityTraits) {
    self.accessibilityElement.accessibilityTraits = ABI35_0_0RCTUIAccessibilityTraitsFromAccessibilityTraits(newViewProps.accessibilityTraits);
  }
...
static ABI35_0_0RCTCornerRadii ABI35_0_0RCTCornerRadiiFromBorderRadii(BorderRadii borderRadii) {
...
static ABI35_0_0RCTBorderColors ABI35_0_0RCTBorderColorsFromBorderColors(BorderColors borderColors) {
  return ABI35_0_0RCTBorderColors {
    .left = ABI35_0_0RCTCGColorRefFromSharedColor(borderColors.left),
    .top = ABI35_0_0RCTCGColorRefFromSharedColor(borderColors.top),
    .bottom = ABI35_0_0RCTCGColorRefFromSharedColor(borderColors.bottom),
    .right = ABI35_0_0RCTCGColorRefFromSharedColor(borderColors.right)
  };
}
...
static ABI35_0_0RCTBorderStyle ABI35_0_0RCTBorderStyleFromBorderStyle(BorderStyle borderStyle) {
  switch (borderStyle) {
    case BorderStyle::Solid:
      return ABI35_0_0RCTBorderStyleSolid;
    case BorderStyle::Dotted:
      return ABI35_0_0RCTBorderStyleDotted;
    case BorderStyle::Dashed:
      return ABI35_0_0RCTBorderStyleDashed;
  }
}
...
- (void)invalidateLayer
{
  ...
  if (layerHasShadow) {
    if (CGColorGetAlpha(_backgroundColor.CGColor) > 0.999) {
      ...
      const ABI35_0_0RCTCornerInsets cornerInsets =
        ABI35_0_0RCTGetCornerInsets(ABI35_0_0RCTCornerRadiiFromBorderRadii(borderMetrics.borderRadii), UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI35_0_0RCTPathCreateWithRoundedRect(self.bounds, cornerInsets, nil);
  ...
  if (useCoreAnimationBorderRendering) {
    ...
    layer.borderColor = ABI35_0_0RCTCGColorRefFromSharedColor(borderMetrics.borderColors.left);
    ...
    UIImage *image = ABI35_0_0RCTGetBorderImage(
      ABI35_0_0RCTBorderStyleFromBorderStyle(borderMetrics.borderStyles.left),
      layer.bounds.size,
      ABI35_0_0RCTCornerRadiiFromBorderRadii(borderMetrics.borderRadii),
      UIEdgeInsetsFromBorderInsets(borderMetrics.borderWidths),
      ABI35_0_0RCTBorderColorsFromBorderColors(borderMetrics.borderColors),
    ...
    if (self.clipsToBounds) {
      if (borderMetrics.borderRadii.isUniform()) {
        ...
        CGPathRef path = ABI35_0_0RCTPathCreateWithRoundedRect(
          self.bounds,
          ABI35_0_0RCTGetCornerInsets(ABI35_0_0RCTCornerRadiiFromBorderRadii(borderMetrics.borderRadii), UIEdgeInsetsZero),
          nil
        );
...
- (NSArray<UIAccessibilityCustomAction *> *)accessibilityCustomActions
{
  ...
  for (const auto &accessibilityAction : accessibilityActions) {
    [customActions addObject:[[UIAccessibilityCustomAction alloc] initWithName:ABI35_0_0RCTNSStringFromString(accessibilityAction)
                                                                        target:self
...
{
  _eventEmitter->onAccessibilityAction(ABI35_0_0RCTStringFromNSString(action.name));
  return YES;
}
