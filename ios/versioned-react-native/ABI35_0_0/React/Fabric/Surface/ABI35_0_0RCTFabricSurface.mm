...
#import <ReactABI35_0_0/ABI35_0_0RCTSurfaceRootView.h>
#import <ReactABI35_0_0/ABI35_0_0RCTSurfaceTouchHandler.h>
...
@implementation ABI35_0_0RCTFabricSurface {
  // Immutable
  ABI35_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI35_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}

- (instancetype)initWithSurfacePresenter:(ABI35_0_0RCTSurfacePresenter *)surfacePresenter
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI35_0_0RCTAllocateRootViewTag() integerValue];

    _minimumSize = CGSizeZero;
    // FIXME: Replace with `_maximumSize = CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX);`.
    _maximumSize = ABI35_0_0RCTScreenSize();

    _touchHandler = [ABI35_0_0RCTSurfaceTouchHandler new];
...
- (BOOL)start
{
  if (![self _setStage:ABI35_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI35_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI35_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI35_0_0RCTSurfaceView alloc] initWithSurface:(ABI35_0_0RCTSurface *)self];
...
- (BOOL)_setStage:(ABI35_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:YES];
}

- (BOOL)_unsetStage:(ABI35_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:NO];
}

- (BOOL)_setStage:(ABI35_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
...
- (void)_propagateStageChange:(ABI35_0_0RCTSurfaceStage)stage
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI35_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI35_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
