...
NSString *const ABI35_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI35_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI35_0_0RCTAccessibilityManager ()
...
@implementation ABI35_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI35_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI35_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI35_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI35_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI35_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI35_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI35_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI35_0_0Tag:ReactABI35_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI35_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI35_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI35_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI35_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI35_0_0RCTResponseSenderBlock)callback
...
@implementation ABI35_0_0RCTBridge (ABI35_0_0RCTAccessibilityManager)

- (ABI35_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI35_0_0RCTAccessibilityManager class]];
}
