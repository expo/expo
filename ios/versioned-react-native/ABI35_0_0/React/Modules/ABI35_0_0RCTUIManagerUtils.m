...
char *const ABI35_0_0RCTUIManagerQueueName = "com.facebook.ReactABI35_0_0.ShadowQueue";

static BOOL pseudoUIManagerQueueFlag = NO;

dispatch_queue_t ABI35_0_0RCTGetUIManagerQueue(void)
{
  ...
  dispatch_once(&onceToken, ^{
    if ([NSOperation instancesRespondToSelector:@selector(qualityOfService)]) {
      ...
      shadowQueue = dispatch_queue_create(ABI35_0_0RCTUIManagerQueueName, attr);
    } else {
      shadowQueue = dispatch_queue_create(ABI35_0_0RCTUIManagerQueueName, DISPATCH_QUEUE_SERIAL);
...
BOOL ABI35_0_0RCTIsUIManagerQueue()
{
  static void *queueKey = &queueKey;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    dispatch_queue_set_specific(ABI35_0_0RCTGetUIManagerQueue(), queueKey, queueKey, NULL);
  });
  return dispatch_get_specific(queueKey) == queueKey;
}

BOOL ABI35_0_0RCTIsPseudoUIManagerQueue()
{
...
void ABI35_0_0RCTExecuteOnUIManagerQueue(dispatch_block_t block)
{
  if (ABI35_0_0RCTIsUIManagerQueue() || ABI35_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    dispatch_async(ABI35_0_0RCTGetUIManagerQueue(), ^{
      block();
    });
  }
}

void ABI35_0_0RCTUnsafeExecuteOnUIManagerQueueSync(dispatch_block_t block)
{
  if (ABI35_0_0RCTIsUIManagerQueue() || ABI35_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    if (ABI35_0_0RCTIsMainQueue()) {
      ...
      dispatch_sync(ABI35_0_0RCTGetUIManagerQueue(), ^{
        block();
      });
    }
  }
...
NSNumber *ABI35_0_0RCTAllocateRootViewTag()
{
