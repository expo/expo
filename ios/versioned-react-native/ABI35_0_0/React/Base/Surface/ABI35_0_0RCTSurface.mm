...
#import "ABI35_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI35_0_0RCTSurface () <ABI35_0_0RCTSurfaceRootShadowViewDelegate, ABI35_0_0RCTUIManagerObserver>
@end

@implementation ABI35_0_0RCTSurface {
  // Immutable
  ...
  ABI35_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI35_0_0RCTSurfaceView *_Nullable _view;
  ABI35_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI35_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI35_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI35_0_0RCTAllocateRootViewTag();
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBridgeWillLoadJavaScriptNotification:)
                                                 name:ABI35_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    _stage = ABI35_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI35_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI35_0_0RCTSurfaceView *)view
{
  ABI35_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI35_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI35_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI35_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI35_0_0RCTSurfaceRootView *rootView =
    (ABI35_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI35_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI35_0_0RCTAssert([rootView isKindOfClass:[ABI35_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI35_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI35_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI35_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI35_0_0RCTSurfaceStage)stage
{
  ABI35_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI35_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI35_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI35_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI35_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI35_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI35_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI35_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountReactABI35_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI35_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI35_0_0RCTIsMainQueue() ? ABI35_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI35_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI35_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI35_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI35_0_0Tag:self->_rootViewTag];
    ABI35_0_0RCTAssert([rootShadowView isKindOfClass:[ABI35_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI35_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI35_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI35_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI35_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI35_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI35_0_0Tag:self->_rootViewTag];
...
- (BOOL)synchronouslyWaitForStage:(ABI35_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI35_0_0RCTIsUIManagerQueue()) {
    ABI35_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI35_0_0RCTIsMainQueue() && (stage & ABI35_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI35_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI35_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI35_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI35_0_0RCTAssert(NO, @"Only waiting for `ABI35_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI35_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI35_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI35_0_0RCTIsMainQueue() && (stage & ABI35_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI35_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI35_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI35_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(ABI35_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI35_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(ABI35_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI35_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI35_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
{
  if (self.stage & ABI35_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI35_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ReactABI35_0_0 components

- (void)mountReactABI35_0_0ComponentWithBridge:(ABI35_0_0RCTBridge *)bridge moduleName:(NSString *)moduleName params:(NSDictionary *)params
{
  [bridge enqueueJSCall:@"AppRegistry" method:@"runApplication" args:@[moduleName, params] completion:NULL];
}

- (void)unmountReactABI35_0_0ComponentWithBridge:(ABI35_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
