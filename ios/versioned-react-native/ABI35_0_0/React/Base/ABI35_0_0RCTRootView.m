...
#if TARGET_OS_TV
#import "ABI35_0_0RCTTVRemoteHandler.h"
#import "ABI35_0_0RCTTVNavigationEventEmitter.h"
#endif

NSString *const ABI35_0_0RCTContentDidAppearNotification = @"ABI35_0_0RCTContentDidAppearNotification";

@interface ABI35_0_0RCTUIManager (ABI35_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI35_0_0RCTRootView
{
  ABI35_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  ABI35_0_0RCTRootContentView *_contentView;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI35_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI35_0_0RCTRootView");
  ABI35_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI35_0_0RCTRootView");

  ABI35_0_0RCT_PROFILE_BEGIN_EVENT(ABI35_0_0RCTProfileTagAlways, @"-[ABI35_0_0RCTRootView init]", nil);
  if (!bridge.isLoading) {
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _sizeFlexibility = ABI35_0_0RCTRootViewSizeFlexibilityNone;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI35_0_0RCTContentDidAppearNotification
                                               object:self];

#if TARGET_OS_TV
    self.tvRemoteHandler = [ABI35_0_0RCTTVRemoteHandler new];
...
{
  if (self.ReactABI35_0_0PreferredFocusedView) {
    return self.ReactABI35_0_0PreferredFocusedView;
  }
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  fitSize = CGSizeMake(
      _sizeFlexibility & ABI35_0_0RCTRootViewSizeFlexibilityWidth ? fitSize.width : currentSize.width,
      _sizeFlexibility & ABI35_0_0RCTRootViewSizeFlexibilityHeight ? fitSize.height : currentSize.height
    );
...
{
  ABI35_0_0RCTAssertMainQueue();
  if (!super.ReactABI35_0_0Tag) {
    /**
     * Every root view that is created must have a unique ReactABI35_0_0 tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ReactABI35_0_0 tag must be re-assigned every time a new UIManager is created.
     */
    self.ReactABI35_0_0Tag = ABI35_0_0RCTAllocateRootViewTag();
  }
  return super.ReactABI35_0_0Tag;
}

- (void)bridgeDidReload
{
  ABI35_0_0RCTAssertMainQueue();
  // Clear the ReactABI35_0_0Tag so it can be re-assigned
  self.ReactABI35_0_0Tag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI35_0_0RCTRootContentView is scoped to the right bridge
...
- (void)bundleFinishedLoading:(ABI35_0_0RCTBridge *)bridge
{
  ABI35_0_0RCTAssert(bridge != nil, @"Bridge cannot be nil");
  if (!bridge.valid) {
    return;
  }

  [_contentView removeFromSuperview];
  _contentView = [[ABI35_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
  ...
  if (_sizeFlexibility == ABI35_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI35_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag": _contentView.ReactABI35_0_0Tag,
    @"initialProps": _appProperties ?: @{},
  };

  ABI35_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
@implementation ABI35_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI35_0_0RCTLogWarn(@"Calling deprecated `[-ABI35_0_0RCTRootView intrinsicSize]`.");
