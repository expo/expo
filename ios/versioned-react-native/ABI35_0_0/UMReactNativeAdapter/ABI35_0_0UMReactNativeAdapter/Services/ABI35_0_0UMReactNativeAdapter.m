...
@implementation ABI35_0_0UMReactNativeAdapter
...
{
  return @[@protocol(ABI35_0_0UMAppLifecycleService), @protocol(ABI35_0_0UMUIManager), @protocol(ABI35_0_0UMJavaScriptContextProvider), @protocol(ABI35_0_0UMImageLoaderInterface)];
}
...
{
  __weak ABI35_0_0UMReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI35_0_0UMReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI35_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI35_0_0RCTJSThread];
}
...
# pragma mark - ABI35_0_0UMJavaScriptContextProvider
...
- (JSGlobalContextRef)javaScriptContextRef
{
  if ([_bridge respondsToSelector:@selector(jsContextRef)]) {
    ...
    // WARNING: This is temporary solution that may break with new ReactABI35_0_0-native releases.
...
# pragma mark - ABI35_0_0UMImageLoader
...
- (void)handleAppStateDidChange:(NSNotification *)notification
{
  if (
      _isForegrounded && (
       ...
       ABI35_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI35_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
