...
#import <ABI35_0_0EXLocalAuthentication/ABI35_0_0EXLocalAuthentication.h>

typedef NS_ENUM(NSInteger, ABI35_0_0EXAuthenticationType) {
  ABI35_0_0EXAuthenticationTypeFingerprint = 1,
  ABI35_0_0EXAuthenticationTypeFacialRecognition = 2,
};

@implementation ABI35_0_0EXLocalAuthentication

ABI35_0_0UM_EXPORT_MODULE(ExpoLocalAuthentication)

ABI35_0_0UM_EXPORT_METHOD_AS(supportedAuthenticationTypesAsync,
                    supportedAuthenticationTypesAsync:(ABI35_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI35_0_0UMPromiseRejectBlock)reject)
{
  NSMutableArray *results = [NSMutableArray array];
  if ([[self class] isTouchIdDevice]) {
    [results addObject:@(ABI35_0_0EXAuthenticationTypeFingerprint)];
  }
  if ([[self class] isFaceIdDevice]) {
    [results addObject:@(ABI35_0_0EXAuthenticationTypeFacialRecognition)];
  }
  resolve(results);
}

ABI35_0_0UM_EXPORT_METHOD_AS(hasHardwareAsync,
                    hasHardwareAsync:(ABI35_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI35_0_0UMPromiseRejectBlock)reject)
{
...
ABI35_0_0UM_EXPORT_METHOD_AS(isEnrolledAsync,
                    isEnrolledAsync:(ABI35_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI35_0_0UMPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;
...
ABI35_0_0UM_EXPORT_METHOD_AS(authenticateAsync,
          ...
          localizedReason:reason
                    reply:^(BOOL success, NSError *error) {
                      resolve(@{
                                ...
                                @"warning": ABI35_0_0UMNullIfNil(warningMessage),
                                });
                    }];
}
