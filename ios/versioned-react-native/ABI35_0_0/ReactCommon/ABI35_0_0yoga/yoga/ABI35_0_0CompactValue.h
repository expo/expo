...
static_assert(
    ...
    "facebook::ABI35_0_0yoga::detail::CompactValue only works with IEEE754 floats");
...
// This class stores ABI35_0_0YGValue in 32 bits.
...
public:
  ...
  template <ABI35_0_0YGUnit Unit>
  static CompactValue of(float value) noexcept {
    if (value == 0.0f || (value < LOWER_BOUND && value > -LOWER_BOUND)) {
      constexpr auto zero =
          Unit == ABI35_0_0YGUnitPercent ? ZERO_BITS_PERCENT : ZERO_BITS_POINT;
      return {Payload{zero}};
    }

    constexpr auto upperBound =
        Unit == ABI35_0_0YGUnitPercent ? UPPER_BOUND_PERCENT : UPPER_BOUND_POINT;
    if (value > upperBound || value < -upperBound) {
      value = copysignf(upperBound, value);
    }

    uint32_t unitBit = Unit == ABI35_0_0YGUnitPercent ? PERCENT_BIT : 0;
  ...
  CompactValue(const ABI35_0_0YGValue& x) noexcept : payload_(uint32_t{0}) {
    switch (x.unit) {
      ...
      case ABI35_0_0YGUnitPoint:
        *this = of<ABI35_0_0YGUnitPoint>(x.value);
        break;
      case ABI35_0_0YGUnitPercent:
        *this = of<ABI35_0_0YGUnitPercent>(x.value);
        break;
    }
  }

  operator ABI35_0_0YGValue() const noexcept {
    switch (payload_.repr) {
      case AUTO_BITS:
        return ABI35_0_0YGValueAuto;
      case ZERO_BITS_POINT:
        return ABI35_0_0YGValue{0.0f, ABI35_0_0YGUnitPoint};
      case ZERO_BITS_PERCENT:
        return ABI35_0_0YGValue{0.0f, ABI35_0_0YGUnitPercent};
    }
    ...
    return ABI35_0_0YGValue{data.value,
                   payload_.repr & 0x40000000 ? ABI35_0_0YGUnitPercent : ABI35_0_0YGUnitPoint};
  }
...
template <>
CompactValue CompactValue::of<ABI35_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::of<ABI35_0_0YGUnitAuto>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI35_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI35_0_0YGUnitAuto>(float) noexcept = delete;
