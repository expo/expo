...
#include "ABI35_0_0ComponentDescriptorRegistry.h"
...
static const std::string componentNameByReactABI35_0_0ViewName(std::string viewName) {
  // We need this function only for the transition period;
  // eventually, all names will be unified.

  std::string rctPrefix("ABI35_0_0RCT");
  if (std::mismatch(rctPrefix.begin(), rctPrefix.end(), viewName.begin())
          .first == rctPrefix.end()) {
    // If `viewName` has "ABI35_0_0RCT" prefix, remove it.
...
const ComponentDescriptor &ComponentDescriptorRegistry::at(
  ...
  auto unifiedComponentName = componentNameByReactABI35_0_0ViewName(componentName);
...
SharedShadowNode ComponentDescriptorRegistry::createNode(
  ...
  ComponentName componentName = componentNameByReactABI35_0_0ViewName(viewName);
