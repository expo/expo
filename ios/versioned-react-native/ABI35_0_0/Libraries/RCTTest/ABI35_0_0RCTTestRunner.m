...
#import "ABI35_0_0RCTTestRunner.h"
...
@implementation ABI35_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI35_0_0RCTBridgeModuleListProvider _moduleProvider;
  NSString *_appPath;
  __weak id<ABI35_0_0RCTBridgeDelegate> _bridgeDelegate;
}
...
- (instancetype)initWithApp:(NSString *)app
         referenceDirectory:(NSString *)referenceDirectory
             bridgeDelegate:(id<ABI35_0_0RCTBridgeDelegate>)bridgeDelegate
{
  return [self initWithApp:app
        referenceDirectory:referenceDirectory
            moduleProvider:nil
...
- (instancetype)initWithApp:(NSString *)app
         referenceDirectory:(NSString *)referenceDirectory
             ...
             bridgeDelegate:(id<ABI35_0_0RCTBridgeDelegate>)bridgeDelegate
{
  ABI35_0_0RCTAssertParam(app);
  ABI35_0_0RCTAssertParam(referenceDirectory);
...
- (NSURL *)defaultScriptURL
{
  if (getenv("CI_USE_PACKAGER") || _useBundler) {
    ...
    return [[NSBundle bundleForClass:[ABI35_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
  }
}
...
- (void)updateScript
{
  ...
  ABI35_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
}
...
configurationBlock:(void(^)(ABI35_0_0RCTRootView *rootView))configurationBlock
{
...
{
  __weak ABI35_0_0RCTBridge *batchedBridge;
  NSNumber *rootTag;
  ABI35_0_0RCTLogFunction defaultLogFunction = ABI35_0_0RCTGetLogFunction();
  ...
  ABI35_0_0RCTSetLogFunction(^(ABI35_0_0RCTLogLevel level, ABI35_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
    ...
    if (level >= ABI35_0_0RCTLogLevelError) {
      if (errors == nil) {
  ...
  @autoreleasepool {
    ABI35_0_0RCTBridge *bridge;
    if (_bridgeDelegate) {
      bridge = [[ABI35_0_0RCTBridge alloc] initWithDelegate:_bridgeDelegate launchOptions:nil];
    } else {
      bridge= [[ABI35_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
    ...
    UIViewController *vc = ABI35_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];

    ABI35_0_0RCTTestModule *testModule = [bridge moduleForClass:[ABI35_0_0RCTTestModule class]];
    ABI35_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    @autoreleasepool {
      ...
      ABI35_0_0RCTRootView *rootView = [[ABI35_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
      ...
      rootTag = rootView.ReactABI35_0_0Tag;
      ...
      while (date.timeIntervalSinceNow > 0 && testModule.status == ABI35_0_0RCTTestStatusPending && errors == nil) {
    ...
    ABI35_0_0RCTSetLogFunction(^(ABI35_0_0RCTLogLevel level, ABI35_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      defaultLogFunction(level, source, fileName, lineNumber, message);
      if (level >= ABI35_0_0RCTLogLevelFatal) {
        if (errors == nil) {
          errors = [NSMutableArray new];
        }
        [errors addObject:message];
    ...
    ABI35_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);
#endif

    if (expectErrorBlock) {
      ABI35_0_0RCTAssert(expectErrorBlock(errors[0]), @"Expected an error but the first one was missing or did not match.");
    } else {
      ABI35_0_0RCTAssert(errors == nil, @"RedBox errors: %@", errors);
      ABI35_0_0RCTAssert(testModule.status != ABI35_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI35_0_0RCTAssert(testModule.status == ABI35_0_0RCTTestStatusPassed, @"Test failed");
    }

    // Wait for the rootView to be deallocated completely before invalidating the bridge.
    ABI35_0_0RCTUIManager *uiManager = [bridge moduleForClass:[ABI35_0_0RCTUIManager class]];
    NSDate *date = [NSDate dateWithTimeIntervalSinceNow:5];
    while (date.timeIntervalSinceNow > 0 && [uiManager viewForReactABI35_0_0Tag:rootTag]) {
      [[NSRunLoop mainRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.1]];
      [[NSRunLoop mainRunLoop] runMode:NSRunLoopCommonModes beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.1]];
    }
    ABI35_0_0RCTAssert([uiManager viewForReactABI35_0_0Tag:rootTag] == nil, @"RootView should have been deallocated after removed.");
  ...
  ABI35_0_0RCTAssert(errors == nil, @"RedBox errors during bridge invalidation: %@", errors);
  ABI35_0_0RCTAssert(batchedBridge == nil, @"Bridge should be deallocated after the test");

  ABI35_0_0RCTSetLogFunction(defaultLogFunction);
}
