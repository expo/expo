...
#import "ABI35_0_0RCTBlobManager.h"

#import <mutex>

#import <ReactABI35_0_0/ABI35_0_0RCTConvert.h>
#import <ReactABI35_0_0/ABI35_0_0RCTNetworking.h>
#import <ReactABI35_0_0/ABI35_0_0RCTUtils.h>
#import <ReactABI35_0_0/ABI35_0_0RCTWebSocketModule.h>

static NSString *const kBlobURIScheme = @"blob";

@interface ABI35_0_0RCTBlobManager () <ABI35_0_0RCTNetworkingRequestHandler, ABI35_0_0RCTNetworkingResponseHandler, ABI35_0_0RCTWebSocketContentHandler>

@end

@implementation ABI35_0_0RCTBlobManager
{
...
ABI35_0_0RCT_EXPORT_MODULE(BlobModule)
...
{
  NSString *blobId = [ABI35_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI35_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI35_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI35_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
...
ABI35_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(nonnull NSNumber *)socketID)
{
...
ABI35_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI35_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI35_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI35_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI35_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI35_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI35_0_0RCTURLRequestHandler methods
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI35_0_0RCTURLRequestDelegate>)delegate
{
  // Lazy setup
  if (!_queue) {
...
#pragma mark - ABI35_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI35_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI35_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI35_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI35_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI35_0_0RCTWebSocketContentHandler methods
