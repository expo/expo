...
#import "ABI35_0_0RCTImagePickerManager.h"

#import <MobileCoreServices/UTCoreTypes.h>
#import <UIKit/UIKit.h>

#import <ReactABI35_0_0/ABI35_0_0RCTConvert.h>
#import <ReactABI35_0_0/ABI35_0_0RCTImageStoreManager.h>
#import <ReactABI35_0_0/ABI35_0_0RCTRootView.h>
#import <ReactABI35_0_0/ABI35_0_0RCTUtils.h>

@interface ABI35_0_0RCTImagePickerController : UIImagePickerController

@property (nonatomic, assign) BOOL unmirrorFrontFacingCamera;

@end

@implementation ABI35_0_0RCTImagePickerController

@end

@interface ABI35_0_0RCTImagePickerManager () <UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI35_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI35_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI35_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
}

ABI35_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI35_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI35_0_0RCTResponseSenderBlock)callback)
{
...
ABI35_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI35_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI35_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI35_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI35_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  ABI35_0_0RCTImagePickerController *imagePicker = [ABI35_0_0RCTImagePickerController new];
  imagePicker.delegate = self;
  imagePicker.sourceType = UIImagePickerControllerSourceTypeCamera;
  imagePicker.unmirrorFrontFacingCamera = [ABI35_0_0RCTConvert BOOL:config[@"unmirrorFrontFacingCamera"]];

  if ([ABI35_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI35_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI35_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI35_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI35_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI35_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI35_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    [self _dismissPicker:picker args:@[imageURL.absoluteString, ABI35_0_0RCTNullIfNil(height), ABI35_0_0RCTNullIfNil(width)]];
    return;
  }
  ...
  [_bridge.imageStoreManager storeImage:originalImage withBlock:^(NSString *tempImageTag) {
    [self _dismissPicker:picker args:tempImageTag ? @[tempImageTag, ABI35_0_0RCTNullIfNil(height), ABI35_0_0RCTNullIfNil(width)] : nil];
  }];
}
        ...
        cancelCallback:(ABI35_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI35_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI35_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI35_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
...
- (void)cameraChanged:(NSNotification *)notification
{
  for (UIImagePickerController *picker in _pickers) {
    if ([picker isKindOfClass:[ABI35_0_0RCTImagePickerController class]]
      && ((ABI35_0_0RCTImagePickerController *)picker).unmirrorFrontFacingCamera
