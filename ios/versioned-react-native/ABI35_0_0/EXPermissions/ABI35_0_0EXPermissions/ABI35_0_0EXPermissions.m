...
NSString * const ABI35_0_0EXPermissionExpiresNever = @"never";

@interface ABI35_0_0EXPermissions ()
...
@implementation ABI35_0_0EXPermissions

ABI35_0_0UM_EXPORT_MODULE(ExpoPermissions);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI35_0_0UMPermissionsInterface), @protocol(ABI35_0_0EXPermissionsModule)];
}

+ (NSDictionary<NSString *, Class> *)defaultRequesters
{
  return @{
           @"audioRecording": [ABI35_0_0EXAudioRecordingPermissionRequester class],
           @"calendar": [ABI35_0_0EXCalendarRequester class],
           @"camera": [ABI35_0_0EXCameraPermissionRequester class],
           @"cameraRoll": [ABI35_0_0EXCameraRollRequester class],
           @"contacts": [ABI35_0_0EXContactsRequester class],
           @"location": [ABI35_0_0EXLocationRequester class],
           @"notifications": [ABI35_0_0EXRemoteNotificationRequester class],
           @"reminders": [ABI35_0_0EXRemindersRequester class],
           @"userFacingNotifications": [ABI35_0_0EXUserNotificationRequester class],
           @"systemBrightness": [ABI35_0_0EXSystemBrightnessRequester class]
           };
}

- (instancetype)init
{
  return [self initWithRequesters:[ABI35_0_0EXPermissions defaultRequesters]];
}
...
ABI35_0_0UM_EXPORT_METHOD_AS(getAsync,
...
ABI35_0_0UM_EXPORT_METHOD_AS(askAsync,
                   ...
                   withRejecter:(ABI35_0_0UMPromiseRejectBlock)reject
{
  ...
  __block void (^askForNextPermission)(void) = ^() {
    ...
    id<ABI35_0_0EXPermissionRequester> requester = [self getPermissionRequesterForType:permissionType];
  ...
  customResolver = ^(NSDictionary *permission) {
    ...
    BOOL isGranted = [ABI35_0_0EXPermissions statusForPermissions:permission] == ABI35_0_0EXPermissionStatusGranted;
...
- (BOOL)hasGrantedPermission:(NSString *)permissionType
{
  ...
  if (!permissions) {
    ABI35_0_0UMLogWarn(@"Permission with type '%@' not found.", permissionType);
    return false;
  }
...
- (id<ABI35_0_0EXPermissionRequester>)getPermissionRequesterForType:(NSString *)type
{
  Class requesterClass = _requesters[type];
  if (requesterClass) {
...
{
  switch (status) {
    case ABI35_0_0EXPermissionStatusGranted:
      return @"granted";
    case ABI35_0_0EXPermissionStatusDenied:
      return @"denied";
    default:
      return @"undetermined";
  }
...
+ (ABI35_0_0EXPermissionStatus)statusForPermissions:(NSDictionary *)permissions
{
  ...
  if ([status isEqualToString:@"granted"]) {
    return ABI35_0_0EXPermissionStatusGranted;
  } else if ([status isEqualToString:@"denied"]) {
    return ABI35_0_0EXPermissionStatusDenied;
  } else {
    return ABI35_0_0EXPermissionStatusUndetermined;
  }
}
