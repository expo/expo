...
#import <ABI35_0_0EXAV/ABI35_0_0EXAVPlayerData.h>
#import <ABI35_0_0EXAV/ABI35_0_0EXVideoPlayerViewController.h>

static NSString *const ABI35_0_0EXVideoReadyForDisplayKeyPath = @"readyForDisplay";
static NSString *const ABI35_0_0EXVideoSourceURIKeyPath = @"uri";
static NSString *const ABI35_0_0EXVideoSourceHeadersKeyPath = @"headers";
static NSString *const ABI35_0_0EXVideoBoundsKeyPath = @"videoBounds";
static NSString *const ABI35_0_0EXAVFullScreenViewControllerClassName = @"AVFullScreenViewController";

@interface ABI35_0_0EXVideoView ()

@property (nonatomic, weak) ABI35_0_0EXAV *exAV;

@property (nonatomic, assign) BOOL playerHasLoaded;
@property (nonatomic, strong) ABI35_0_0EXAVPlayerData *data;
@property (nonatomic, strong) AVPlayerLayer *playerLayer;
@property (nonatomic, strong) ABI35_0_0EXVideoPlayerViewController *playerViewController;
...
@property (nonatomic, strong) ABI35_0_0EXVideoPlayerViewController *fullscreenPlayerViewController;
@property (nonatomic, strong) ABI35_0_0UMPromiseResolveBlock requestedFullscreenChangeResolver;
@property (nonatomic, strong) ABI35_0_0UMPromiseRejectBlock requestedFullscreenChangeRejecter;
...
@implementation ABI35_0_0EXVideoView

#pragma mark - ABI35_0_0EXVideoView interface methods

- (instancetype)initWithModuleRegistry:(ABI35_0_0UMModuleRegistry *)moduleRegistry
{
  if ((self = [super init])) {
    _moduleRegistry = moduleRegistry;
    _exAV = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI35_0_0EXAVInterface)];
...
- (void)_callFullscreenCallbackForUpdate:(ABI35_0_0EXVideoFullscreenUpdate)update
{
...
- (void)_tryUpdateDataStatus:(ABI35_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI35_0_0UMPromiseRejectBlock)reject
{
  if (_data) {
    ...
    resolve([ABI35_0_0EXAVPlayerData getUnloadedStatus]);
  }
}
...
- (void)_removePlayer
{
  if (_requestedFullscreenChangeRejecter) {
    ...
    _requestedFullscreenChangeRejecter(@"E_VIDEO_FULLSCREEN", errorMessage, ABI35_0_0UMErrorWithMessage(errorMessage));
    _requestedFullscreenChangeResolver = nil;
    _requestedFullscreenChangeRejecter = nil;
    _requestedFullscreenChange = NO;
  }
...
- (ABI35_0_0EXVideoPlayerViewController *)_createNewPlayerViewController
{
  if (_data == nil) {
    return nil;
  }
  ABI35_0_0EXVideoPlayerViewController *controller = [[ABI35_0_0EXVideoPlayerViewController alloc] init];
  ...
  [controller addObserver:self forKeyPath:ABI35_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
  return controller;
}
...
- (void)_usePlayerLayer
{
  if (_data) {
    ...
    [_playerLayer addObserver:self forKeyPath:ABI35_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)_removePlayerLayer
{
  if (_playerLayer) {
    ...
    [_playerLayer removeObserver:self forKeyPath:ABI35_0_0EXVideoReadyForDisplayKeyPath];
    _playerLayer = nil;
  }
}
...
- (void)_removeFullscreenPlayerViewController
{
  if (_fullscreenPlayerViewController) {
    [_fullscreenPlayerViewController removeObserver:self forKeyPath:ABI35_0_0EXVideoReadyForDisplayKeyPath];
    _fullscreenPlayerViewController = nil;
  }
}
...
- (void)_removePlayerViewController
{
  if (_playerViewController) {
    __weak ABI35_0_0EXVideoView *weakSelf = self;
    void (^block)(void) = ^ {
      __strong ABI35_0_0EXVideoView *strongSelf = weakSelf;
      if (strongSelf && strongSelf.playerViewController) {
        [strongSelf.playerViewController.view removeFromSuperview];
        [strongSelf.playerViewController removeObserver:strongSelf forKeyPath:ABI35_0_0EXVideoReadyForDisplayKeyPath];
        [strongSelf.playerViewController removeObserver:strongSelf forKeyPath:ABI35_0_0EXVideoBoundsKeyPath];
        strongSelf.playerViewController = nil;
      }
    };

    [ABI35_0_0UMUtilities performSynchronouslyOnMainThread:block];
  }
}
...
{
  if ((object == _playerLayer || object == _playerViewController || object == _fullscreenPlayerViewController) && [keyPath isEqualToString:ABI35_0_0EXVideoReadyForDisplayKeyPath]) {
  ...
  } else if (object == _playerViewController && [keyPath isEqualToString:ABI35_0_0EXVideoBoundsKeyPath]) {
    UIViewController *presentedViewController = [[_moduleRegistry getModuleImplementingProtocol:@protocol(ABI35_0_0UMUtilitiesInterface)] currentViewController];
    ...
    if ([[presentedViewController.class description] isEqualToString:ABI35_0_0EXAVFullScreenViewControllerClassName]) {
      // ABI35_0_0RCTPresentedViewController() is fullscreen
       fullscreenViewController = presentedViewController;
    } else if (presentedViewController.presentedViewController != nil && [[presentedViewController.presentedViewController.class description] isEqualToString:ABI35_0_0EXAVFullScreenViewControllerClassName]) {
      // ABI35_0_0RCTPresentedViewController().presentedViewController is fullscreen
    ...
    if (fullscreenViewController.isBeingDismissed && _fullscreenPlayerPresented && _nativeFullscreenPlayerViewController == fullscreenViewController) {
      ...
      _nativeFullscreenPlayerViewController = nil;
      [self _callFullscreenCallbackForUpdate:ABI35_0_0EXVideoFullscreenUpdatePlayerWillDismiss];
      [self _callFullscreenCallbackForUpdate:ABI35_0_0EXVideoFullscreenUpdatePlayerDidDismiss];
      ...
      [self _callFullscreenCallbackForUpdate:ABI35_0_0EXVideoFullscreenUpdatePlayerWillPresent];
      [self _callFullscreenCallbackForUpdate:ABI35_0_0EXVideoFullscreenUpdatePlayerDidPresent];
    } else {
      return;
    }
  } else {
         ...
         rejecter:(ABI35_0_0UMPromiseRejectBlock)reject
{
  ...
  _data = [[ABI35_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                                    withSource:source
                           ...
                           withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                             ...
                             if (strongSelf && success) {
                               ...
                               [strongSelf _removePlayer];
                               if (reject) {
                                 reject(@"E_VIDEO_NOTCREATED", error, ABI35_0_0UMErrorWithMessage(error));
                               }
                               [strongSelf _callErrorCallback:error];
                             }
                           }];
             ...
             rejecter:(ABI35_0_0UMPromiseRejectBlock)reject
{
  if (!_data) {
    ...
    if (reject) {
      ...
      reject(@"E_VIDEO_FULLSCREEN", errorMessage, ABI35_0_0UMErrorWithMessage(errorMessage));
    }
    return;
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (strongSelf) {
          ...
          [strongSelf.presentingViewController presentViewController:strongSelf.fullscreenPlayerViewController animated:YES completion:^{
            __strong ABI35_0_0EXVideoView *strongSelfInner = weakSelf;
            if (strongSelfInner) {
              strongSelfInner.fullscreenPlayerPresented = YES;
              [strongSelfInner _callFullscreenCallbackForUpdate:ABI35_0_0EXVideoFullscreenUpdatePlayerDidPresent];
              if (resolve) {
...
{
  return @{
           ABI35_0_0EXVideoSourceURIKeyPath: (_data != nil && _data.url != nil) ? _data.url.absoluteString : @"",
           ABI35_0_0EXVideoSourceHeadersKeyPath: _data.headers
           };
}
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (strongSelf && strongSelf.useNativeControls) {
      ...
      if (!strongSelf.playerViewController && strongSelf.data) {
        ...
        [strongSelf.playerViewController addObserver:self forKeyPath:ABI35_0_0EXVideoBoundsKeyPath options:NSKeyValueObservingOptionNew context:nil];
        // Resize mode must be set before layer is added
        // to prevent video from being animated when `resizeMode` is `cover`
...
- (NSDictionary *)getStatus
{
  if (_data) {
    ...
    return [ABI35_0_0EXAVPlayerData getUnloadedStatus];
  }
}

#pragma mark - ReactABI35_0_0 View Management
...
//  } else {
//    ABI35_0_0RCTLogError(@"video cannot have any subviews");
//  }
//}
...
#pragma mark - ABI35_0_0EXVideoPlayerViewControllerDelegate
...
#pragma mark - ABI35_0_0EXAVObject
...
- (ABI35_0_0EXAVAudioSessionMode)getAudioSessionModeRequired
{
  return _data == nil ? ABI35_0_0EXAVAudioSessionModeInactive : [_data getAudioSessionModeRequired];
}
