...
@implementation ABI35_0_0RNSVGGroup
{
    ABI35_0_0RNSVGGlyphContext *_glyphContext;
}
...
- (void)renderGroupTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    [self traverseSubviews:^(UIView *node) {
        if ([node isKindOfClass:[ABI35_0_0RNSVGMask class]]) {
            // no-op
        } else if ([node isKindOfClass:[ABI35_0_0RNSVGNode class]]) {
            ABI35_0_0RNSVGNode* svgNode = (ABI35_0_0RNSVGNode*)node;
            if (svgNode.responsible && !self.svgView.responsible) {
                self.svgView.responsible = YES;
            }

            if ([node isKindOfClass:[ABI35_0_0RNSVGRenderable class]]) {
                [(ABI35_0_0RNSVGRenderable*)node mergeProperties:self];
            }
            ...
            if ([node isKindOfClass:[ABI35_0_0RNSVGRenderable class]]) {
                [(ABI35_0_0RNSVGRenderable*)node resetProperties];
            }
        } else if ([node isKindOfClass:[ABI35_0_0RNSVGSvgView class]]) {
            ABI35_0_0RNSVGSvgView* svgView = (ABI35_0_0RNSVGSvgView*)node;
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI35_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}

- (ABI35_0_0RNSVGGlyphContext *)getGlyphContext
{
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI35_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI35_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI35_0_0RNSVGMask class]]) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI35_0_0RNSVGNode class]];
...
{
    [self traverseSubviews:^(__kindof ABI35_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI35_0_0RNSVGRenderable class]]) {
            [(ABI35_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
