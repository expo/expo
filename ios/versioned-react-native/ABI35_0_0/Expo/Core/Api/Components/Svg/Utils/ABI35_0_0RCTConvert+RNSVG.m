...
+ (ABI35_0_0RNSVGBrush *)ABI35_0_0RNSVGBrush:(id)json
{
    if ([json isKindOfClass:[NSString class]]) {
        ...
        if (!ABI35_0_0RNSVGDigitRegEx) {
            ABI35_0_0RNSVGDigitRegEx = [NSRegularExpression regularExpressionWithPattern:@"[0-9.-]+" options:NSRegularExpressionCaseInsensitive error:nil];
        }
        NSArray<NSTextCheckingResult*> *_matches = [ABI35_0_0RNSVGDigitRegEx matchesInString:value options:0 range:NSMakeRange(0, [value length])];
        ...
        return [[ABI35_0_0RNSVGSolidColorBrush alloc] initWithArray:output];
    }
    ...
    switch (type) {
        case 0: // solid color
            ...
            return [[ABI35_0_0RNSVGSolidColorBrush alloc] initWithArray:arr];
        case 1: // brush
            return [[ABI35_0_0RNSVGPainterBrush alloc] initWithArray:arr];
        case 2: // currentColor
            return [[ABI35_0_0RNSVGBrush alloc] initWithArray:nil];
        default:
            ABI35_0_0RCTLogError(@"Unknown brush type: %zd", (unsigned long)type);
            return nil;
    }
}
...
{
    return [[ABI35_0_0RNSVGPathParser alloc] initWithPathString: d];
}

+ (ABI35_0_0RNSVGLength *)ABI35_0_0RNSVGLength:(id)json
{
    if ([json isKindOfClass:[NSNumber class]]) {
        return [ABI35_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
    } else if ([json isKindOfClass:[NSString class]]) {
        NSString *stringValue = (NSString *)json;
        return [ABI35_0_0RNSVGLength lengthWithString:stringValue];
    } else {
        return [[ABI35_0_0RNSVGLength alloc] init];
    }
}
...
{
    if ([json isKindOfClass:[NSNumber class]]) {
        ABI35_0_0RNSVGLength* length = [ABI35_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
        return [NSArray arrayWithObject:length];
    } else if ([json isKindOfClass:[NSArray class]]) {
        NSArray *arrayValue = (NSArray*)json;
        NSMutableArray<ABI35_0_0RNSVGLength*>* lengths = [NSMutableArray arrayWithCapacity:[arrayValue count]];
        for (id obj in arrayValue) {
            [lengths addObject:[self ABI35_0_0RNSVGLength:obj]];
        }
        return lengths;
    }  else if ([json isKindOfClass:[NSString class]]) {
        NSString *stringValue = (NSString *)json;
        ABI35_0_0RNSVGLength* length = [ABI35_0_0RNSVGLength lengthWithString:stringValue];
        return [NSArray arrayWithObject:length];
    } else {
        return nil;
    }
...
+ (CGRect)ABI35_0_0RNSVGCGRect:(id)json offset:(NSUInteger)offset
{
    ...
    if (arr.count < offset + 4) {
        ABI35_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", (ssize_t)(4 + offset), arr);
        return CGRectZero;
    }
    return (CGRect){
