//
//  ABI35_0_0RNSharedElementNode.m
...
@implementation ABI35_0_0RNSharedElementNode
{
    long _refCount;
    long _hideRefCount;
    ...
    ABI35_0_0RNSharedElementContentType _contentTypeCache;
    
    NSMutableArray* _styleRequests;
    ABI35_0_0RNSharedElementStyle* _styleCache;
...
{
    _ReactABI35_0_0Tag = ReactABI35_0_0Tag;
    _sourceView = view;
    _isParent = isParent;
    _refCount = 1;
    _hideRefCount = 0;
    ...
    _contentCache = nil;
    _contentTypeCache = ABI35_0_0RNSharedElementContentTypeNone;
    _styleRequests = nil;
    _styleCache = nil;
    _displayLink = nil;
    self.view = [ABI35_0_0RNSharedElementNode resolveView:_isParent ? _sourceView.subviews.firstObject : _sourceView];
...
+ (UIView*) resolveView:(UIView*) view
{
    ...
    // ReactABI35_0_0-native libs (e.g. ReactABI35_0_0-native-fast-image)
...
- (void) requestContent:(__weak id <ABI35_0_0RNSharedElementDelegate>) delegate
{
...
- (void) updateContent
{
    ...
    NSObject* content;
    ABI35_0_0RNSharedElementContentType contentType;
    if ([view isKindOfClass:[UIImageView class]]) {
        UIImageView* imageView = (UIImageView*) view;
        UIImage* image = imageView.image;
        content = image;
        contentType = ABI35_0_0RNSharedElementContentTypeRawImage;
    }
    else if ([NSStringFromClass(view.class) isEqualToString:@"ABI35_0_0RCTView"] && !view.subviews.count) {
        content = [[UIView alloc]init];
        contentType = ABI35_0_0RNSharedElementContentTypeSnapshotView;
    }
    else {
    ...
    /*else {
        ...
        BOOL res = [view drawViewHierarchyInRect:bounds afterScreenUpdates:NO]; // NEVER USE YES, IT CREATED VISUAL ABI35_0_0ARTEFACTS ON THE CREEN
        UIImage* image = res ? UIGraphicsGetImageFromCurrentImageContext() : nil;
        UIGraphicsEndImageContext();
        NSLog(@"drawViewHierarchyInRect: RESULT: %li", res);
        content = image;
        contentType = ABI35_0_0RNSharedElementContentTypeSnapshotImage;
    }*/
    ...
    for (__weak id <ABI35_0_0RNSharedElementDelegate> delegate in delegates) {
        if (delegate != nil) {
...
- (void) requestStyle:(__weak id <ABI35_0_0RNSharedElementDelegate>) delegate
{
    if (_styleCache != nil) {
...
- (void) updateStyle
{
    ...
    // Create style
    ABI35_0_0RNSharedElementStyle* style = [[ABI35_0_0RNSharedElementStyle alloc]init];
    CALayer* layer = view.layer;
    style.view = view;
    style.layout = layout;
    style.size = view.bounds.size;
    style.transform = [ABI35_0_0RNSharedElementStyle getAbsoluteViewTransform:view];
    ...
    /*NSLog(@"Style fetched: %@, realSize: %@, opacity: %lf, transform: %@", NSStringFromCGRect(layout), NSStringFromCGSize(view.bounds.size), style.opacity, [ABI35_0_0RNSharedElementStyle stringFromTransform:style.transform]);*/
