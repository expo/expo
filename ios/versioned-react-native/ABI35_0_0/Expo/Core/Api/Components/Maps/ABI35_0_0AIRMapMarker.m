...
NSInteger const ABI35_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE = 999;

@implementation ABI35_0_0AIREmptyCalloutBackgroundView
@end

@implementation ABI35_0_0AIRMapMarker {
...
- (void)ReactABI35_0_0SetFrame:(CGRect)frame
{
    ...
    if (isnan(center.x) || isnan(center.y) ||
        ...
        ABI35_0_0RCTLogError(@"Invalid layout for (%@)%@. position: %@. bounds: %@",
                self.ReactABI35_0_0Tag, self, NSStringFromCGPoint(center), NSStringFromCGRect(bounds));
        return;
    }
...
- (void)insertReactABI35_0_0Subview:(id<ABI35_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    if ([subview isKindOfClass:[ABI35_0_0AIRMapCallout class]]) {
        self.calloutView = (ABI35_0_0AIRMapCallout *)subview;
    } else {
        [super insertReactABI35_0_0Subview:(UIView *)subview atIndex:atIndex];
    }
}

- (void)removeReactABI35_0_0Subview:(id<ABI35_0_0RCTComponent>)subview {
    if ([subview isKindOfClass:[ABI35_0_0AIRMapCallout class]] && self.calloutView == subview) {
        self.calloutView = nil;
    } else {
        [super removeReactABI35_0_0Subview:(UIView *)subview];
    }
}
...
- (MKAnnotationView *)getAnnotationView
{
    if ([self shouldUsePinView]) {
        ...
        // If it has subviews, it means we are wanting to render a custom marker with arbitrary ReactABI35_0_0 views.
        // if it has a non-null image, it means we want to render a custom marker with the image.
        // In either case, we want to return the ABI35_0_0AIRMapMarker since it is both an MKAnnotation and an
...
- (void)fillCalloutView:(ABI35_0_0SMCalloutView *)calloutView
{
    ...
    if (self.calloutView) {
        ...
        if (self.calloutView.tooltip) {
            // if tooltip is true, then the user wants their ReactABI35_0_0 view to be the "tooltip" as wwell, so we set
            // the background view to something empty/transparent
            calloutView.backgroundView = [ABI35_0_0AIREmptyCalloutBackgroundView new];
        } else {
            // the default tooltip look is wanted, and the user is just filling the content with their ReactABI35_0_0 subviews.
            // as a result, we use the default "masked" background view.
            calloutView.backgroundView = [ABI35_0_0SMCalloutMaskedBackgroundView new];
        }

        // when this is set, the callout's content will be whatever ReactABI35_0_0 views the user has put as the callout's
        // children.
...
- (void)_handleTap:(UITapGestureRecognizer *)recognizer {
    ABI35_0_0AIRMapMarker *marker = self;
    if (!marker) return;
...
{
    return self.ReactABI35_0_0Subviews.count == 0 && !self.imageSrc;
}
...
- (void)setImageSrc:(NSString *)imageSrc
{
    ...
    _reloadImageCancellationBlock = [_bridge.imageLoader loadImageWithURLRequest:[ABI35_0_0RCTConvert NSURLRequest:_imageSrc]
                                                                            size:self.bounds.size
                                                                           scale:ABI35_0_0RCTScreenScale()
                                                                         clipped:YES
                                                                      resizeMode:ABI35_0_0RCTResizeModeCenter
                                                                   progressBlock:nil
...
- (void)setZIndex:(NSInteger)zIndex
{
    ...
    _zIndex = _calloutIsOpen ? zIndex + ABI35_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE : zIndex;
