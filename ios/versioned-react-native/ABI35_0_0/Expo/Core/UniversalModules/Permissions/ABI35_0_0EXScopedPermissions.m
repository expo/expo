...
@interface ABI35_0_0EXPermissions (Protected)
...
@interface ABI35_0_0EXScopedPermissions ()

@property (nonatomic, strong) NSString *experienceId;
@property (nonatomic, weak) id<ABI35_0_0EXPermissionsScopedModuleDelegate> permissionsService;
@property (nonatomic, weak) id<ABI35_0_0UMUtilitiesInterface> utils;
@property (nonatomic, weak) ABI35_0_0EXConstantsBinding *constantsBinding;

@end

@implementation ABI35_0_0EXScopedPermissions
                      ...
                      withRejecter:(ABI35_0_0UMPromiseRejectBlock)reject
{
  ...
  // The result will be saved in ABI35_0_0EXPermissionsScopedModuleDelegate.
  //
...
- (BOOL)hasGrantedScopedPermission:(NSString *)permissionType
{
  ...
  return [_permissionsService getPermission:permissionType forExperience:_experienceId] == ABI35_0_0EXPermissionStatusGranted;
}
  ...
  if (![_constantsBinding.appOwnership isEqualToString:@"expo"]) {
    ...
    for (NSString *permissionType in permissionsTypes) {
      ...
      if (![self.permissionsService savePermission:results[permissionType] ofType:permissionType forExperience:self.experienceId]) {
        results[permissionType][@"status"] = [[self class] permissionStringForStatus:ABI35_0_0EXPermissionStatusDenied];
        results[@"granted"] = @(NO);
      }
    }
  ...
  __block void (^askForNextPermission)(void) = ^() {
    ...
    UIAlertAction *allowAction = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
      ...
      if (![self.permissionsService savePermission:permissions[permissionType] ofType:permissionType forExperience:self.experienceId]) {
        permissions[permissionType][@"status"] = [[self class] permissionStringForStatus:ABI35_0_0EXPermissionStatusDenied];
...
- (NSDictionary *)getPermissionsForResource:(NSString *)permissionType
{
  ...
  if ([self shouldVerifyScopedPermission:permissionType]
      && [ABI35_0_0EXPermissions statusForPermissions:permission] == ABI35_0_0EXPermissionStatusGranted
      && ![self hasGrantedScopedPermission:permissionType]) {
    permission[@"status"] = [[self class] permissionStringForStatus:ABI35_0_0EXPermissionStatusDenied];
  }
  return permission;
}
