...
NSString * const ABI35_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI35_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI35_0_0UMTaskServiceInterface> taskService;

@end

@implementation ABI35_0_0EXTaskManager

ABI35_0_0UM_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI35_0_0UMTaskManagerInterface)];
}
...
// TODO: Remove when adding bare ReactABI35_0_0 Native support
...
{
  return @{
           @"EVENT_NAME": ABI35_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI35_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI35_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI35_0_0UM_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI35_0_0UM_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI35_0_0UM_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI35_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI35_0_0UMPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI35_0_0UM_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI35_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI35_0_0UMPromiseRejectBlock)reject)
{
  resolve(ABI35_0_0UMNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI35_0_0UM_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI35_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI35_0_0UMPromiseRejectBlock)reject)
{
...
# pragma mark - ABI35_0_0UMTaskManagerInterface
...
{
  return [ABI35_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI35_0_0EXTaskManagerEventName body:body];
  } else {
