...
@interface ABI41_0_0EXNotificationCenterDelegate ()
...
@implementation ABI41_0_0EXNotificationCenterDelegate

ABI41_0_0UM_REGISTER_SINGLETON_MODULE(NotificationCenterDelegate);
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary<UIApplicationLaunchOptionsKey,id> *)launchOptions
{
  if ([UNUserNotificationCenter currentNotificationCenter].delegate) {
    ABI41_0_0UMLogWarn(@"[expo-notifications] ABI41_0_0EXNotificationCenterDelegate encountered already present delegate of UNUserNotificationCenter: %@. "
              "ABI41_0_0EXNotificationCenterDelegate will not overwrite the value not to break other features of your app. "
              "In return, expo-notifications may not work properly. "
              "To fix this problem either remove setting of the second delegate "
              "or set the delegate to an instance of ABI41_0_0EXNotificationCenterDelegate manually afterwards.",
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler
{
  ...
  for (int i = 0; i < _delegates.count; i++) {
    ...
    if ([pointer respondsToSelector:@selector(userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:)] 
      && ![NSStringFromClass([pointer class]) isEqual: @"ABI41_0_0EXUserNotificationManager"]) {
      // Remove ABI41_0_0EXUserNotificationManager check when LegacyNotifications are no longer supported
...
# pragma mark - ABI41_0_0EXNotificationCenterDelegate

- (void)addDelegate:(id<ABI41_0_0EXNotificationsDelegate>)delegate
{
  ...
  if ([delegate respondsToSelector:@selector(userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:)]) {
    ...
    // Remove ABI41_0_0EXUserNotificationManager check when LegacyNotifications are no longer supported
    if (![NSStringFromClass([delegate class]) isEqual:@"ABI41_0_0EXUserNotificationManager"]) {
      [_pendingNotificationResponses removeAllObjects];
    }
  }
}

- (void)removeDelegate:(id<ABI41_0_0EXNotificationsDelegate>)delegate
{
  for (int i = 0; i < _delegates.count; i++) {
    id pointer = [_delegates pointerAtIndex:i];
