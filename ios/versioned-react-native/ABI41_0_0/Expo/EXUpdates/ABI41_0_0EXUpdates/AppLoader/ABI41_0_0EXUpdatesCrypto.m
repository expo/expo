...
static NSString * const ABI41_0_0EXUpdatesCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const ABI41_0_0EXUpdatesCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const ABI41_0_0EXUpdatesCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation ABI41_0_0EXUpdatesCrypto

+ (void)verifySignatureWithData:(NSString *)data
                      signature:(NSString *)signature
                         config:(ABI41_0_0EXUpdatesConfig *)config
                   successBlock:(ABI41_0_0EXUpdatesVerifySignatureSuccessBlock)successBlock
                     errorBlock:(ABI41_0_0EXUpdatesVerifySignatureErrorBlock)errorBlock
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"ABI41_0_0EXUpdatesCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }
  ...
  ABI41_0_0EXUpdatesFileDownloader *fileDownloader = [[ABI41_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
  [fileDownloader downloadDataFromURL:[NSURL URLWithString:ABI41_0_0EXUpdatesCryptoPublicKeyUrl]
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(ABI41_0_0EXUpdatesVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[ABI41_0_0EXUpdatesCryptoPublicKeyTag UTF8String] length:[ABI41_0_0EXUpdatesCryptoPublicKeyTag length]];
