...
static NSString * const ABI41_0_0EXUpdatesErrorLogFile = @"expo-error.log";

@interface ABI41_0_0EXUpdatesAppLauncherNoDatabase ()
...
@implementation ABI41_0_0EXUpdatesAppLauncherNoDatabase

- (void)launchUpdateWithConfig:(ABI41_0_0EXUpdatesConfig *)config
{
  _launchedUpdate = [ABI41_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:config database:nil];
  if (_launchedUpdate) {
    if (_launchedUpdate.status == ABI41_0_0EXUpdatesUpdateStatusEmbedded) {
      ...
      _launchAssetUrl = [[NSBundle mainBundle] URLForResource:ABI41_0_0EXUpdatesEmbeddedBundleFilename withExtension:ABI41_0_0EXUpdatesEmbeddedBundleFileType];
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:ABI41_0_0EXUpdatesErrorLogFile] path];
}
