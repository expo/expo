...
static NSString * const ABI41_0_0EXSyncDatabaseInitializationErrorDomain = @"ABI41_0_0EXSyncDatabaseInitialization";
static NSString * const ABI41_0_0EXSyncDatabaseLatestFilename = @"expo-v5.db";

static NSString * const ABI41_0_0EXSyncDatabaseInitializationLatestSchema = @"\
...
@implementation ABI41_0_0EXSyncDatabaseInitialization
...
{
  return [[self class] initializeDatabaseWithSchema:ABI41_0_0EXSyncDatabaseInitializationLatestSchema
                                           filename:ABI41_0_0EXSyncDatabaseLatestFilename
                               ...
                               error:(NSError ** _Nullable)error
{
  ...
  if (!success) {
    ...
    if ([NSFileManager.defaultManager fileExistsAtPath:dbUrl.path] &&
      ...
      if (error != nil) {
        ...
        *error = [NSError errorWithDomain:ABI41_0_0EXSyncDatabaseInitializationErrorDomain
                                     code:1022
  ...
  if (resultCode != SQLITE_OK) {
    NSLog(@"Error opening SQLite db: %@", [ABI41_0_0EXSyncDatabaseUtils errorFromSqlite:db].localizedDescription);
    sqlite3_close(db);
    ...
    if (resultCode == SQLITE_CORRUPT || resultCode == SQLITE_NOTADB) {
      ...
      if ([[NSFileManager defaultManager] moveItemAtURL:dbUrl toURL:destinationUrl error:&err]) {
        ...
        if (sqlite3_open([[dbUrl absoluteString] UTF8String], &db) != SQLITE_OK) {
          if (error != nil) {
            *error = [ABI41_0_0EXSyncDatabaseUtils errorFromSqlite:db];
          }
          return NO;
        }
        shouldInitializeDatabaseSchema = YES;
  ...
  if (![ABI41_0_0EXSyncDatabaseUtils executeSql:@"PRAGMA foreign_keys=ON;" withArgs:nil onDatabase:db error:&pragmaForeignKeysError]) {
...
{
  NSURL *latestURL = [directory URLByAppendingPathComponent:ABI41_0_0EXSyncDatabaseLatestFilename];
  ...
  NSArray<id<ABI41_0_0EXSyncDatabaseMigration>> *migrations = [ABI41_0_0EXSyncDatabaseMigrationRegistry migrations];
  __block NSURL *existingURL;
  __block NSUInteger startingMigrationIndex;
  [migrations enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id<ABI41_0_0EXSyncDatabaseMigration> migration, NSUInteger idx, BOOL *stop) {
  ...
  if (existingURL) {
    ...
    if (sqlite3_open(latestURL.absoluteString.UTF8String, &db) != SQLITE_OK) {
      NSLog(@"Error opening migrated SQLite db: %@", [ABI41_0_0EXSyncDatabaseUtils errorFromSqlite:db].localizedDescription);
      sqlite3_close(db);
      return NO;
    }
    ...
    for (NSUInteger i = startingMigrationIndex; i < migrations.count; i++) {
      ...
      id<ABI41_0_0EXSyncDatabaseMigration> migration = migrations[i];
      if (![migration runMigrationOnDatabase:db error:&migrationError]) {
        NSLog(@"Error migrating SQLite db: %@", [ABI41_0_0EXSyncDatabaseUtils errorFromSqlite:db].localizedDescription);
        sqlite3_close(db);
        return NO;
      }
    }
