...
@implementation ABI41_0_0EXUpdatesReaper
                     ...
                     launchedUpdate:(ABI41_0_0EXUpdatesUpdate *)launchedUpdate
{
  dispatch_async(database.databaseQueue, ^{
    ...
    NSArray<ABI41_0_0EXUpdatesUpdate *> *allUpdates = [database allUpdatesWithConfig:config error:&error];
    if (!allUpdates || error) {
      NSLog(@"Error reaping updates: %@", error.localizedDescription);
      return;
    }
    ...
    NSArray<ABI41_0_0EXUpdatesUpdate *> *updatesToDelete = [selectionPolicy updatesToDeleteWithLaunchedUpdate:launchedUpdate updates:allUpdates filters:manifestFilters];
    ...
    NSArray<ABI41_0_0EXUpdatesAsset *> *assetsForDeletion = [database deleteUnusedAssetsWithError:&error];
    if (error) {
      NSLog(@"Error reaping updates: %@", error.localizedDescription);
      return;
    }
    ...
    dispatch_async([ABI41_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
      ...
      NSMutableArray<ABI41_0_0EXUpdatesAsset *> *erroredAssets = [NSMutableArray new];

      NSDate *beginDeleteAssets = [NSDate date];
      for (ABI41_0_0EXUpdatesAsset *asset in assetsForDeletion) {
      ...
      for (ABI41_0_0EXUpdatesAsset *asset in erroredAssets) {
        NSURL *localUrl = [directory URLByAppendingPathComponent:asset.filename];
        NSError *error;
        if ([NSFileManager.defaultManager fileExistsAtPath:localUrl.path] && ![NSFileManager.defaultManager removeItemAtURL:localUrl error:&error]) {
