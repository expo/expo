...
@implementation ABI41_0_0EXUpdatesDatabaseInitialization
                               ...
                               error:(NSError ** _Nullable)error
{
  ...
  if (!success) {
    ...
    if ([NSFileManager.defaultManager fileExistsAtPath:dbUrl.path] &&
      ...
      if (error != nil) {
        ...
        *error = [NSError errorWithDomain:ABI41_0_0EXUpdatesDatabaseInitializationErrorDomain
                                     code:1022
  ...
  if (resultCode != SQLITE_OK) {
    NSLog(@"Error opening SQLite db: %@", [ABI41_0_0EXUpdatesDatabaseUtils errorFromSqlite:db].localizedDescription);
    sqlite3_close(db);
    ...
    if (resultCode == SQLITE_CORRUPT || resultCode == SQLITE_NOTADB) {
      ...
      if ([[NSFileManager defaultManager] moveItemAtURL:dbUrl toURL:destinationUrl error:&err]) {
        ...
        if (sqlite3_open([[dbUrl absoluteString] UTF8String], &db) != SQLITE_OK) {
          if (error != nil) {
            *error = [ABI41_0_0EXUpdatesDatabaseUtils errorFromSqlite:db];
          }
          return NO;
        }
        shouldInitializeDatabaseSchema = YES;
  ...
  if (![ABI41_0_0EXUpdatesDatabaseUtils executeSql:@"PRAGMA foreign_keys=ON;" withArgs:nil onDatabase:db error:&pragmaForeignKeysError]) {
...
+ (BOOL)_migrateDatabaseInDirectory:(NSURL *)directory
{
  ...
  if (existingURL) {
    ...
    if (sqlite3_open(latestURL.absoluteString.UTF8String, &db) != SQLITE_OK) {
      NSLog(@"Error opening migrated SQLite db: %@", [ABI41_0_0EXUpdatesDatabaseUtils errorFromSqlite:db].localizedDescription);
      sqlite3_close(db);
      return NO;
    }
    ...
    for (NSUInteger i = startingMigrationIndex; i < migrations.count; i++) {
      ...
      id<ABI41_0_0EXUpdatesDatabaseMigration> migration = migrations[i];
      if (![migration runMigrationOnDatabase:db error:&migrationError]) {
        NSLog(@"Error migrating SQLite db: %@", [ABI41_0_0EXUpdatesDatabaseUtils errorFromSqlite:db].localizedDescription);
        sqlite3_close(db);
        return NO;
      }
    }
