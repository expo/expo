...
NSString * const ABI41_0_0EXSyncEmbeddedManifestName = @"app";
NSString * const ABI41_0_0EXSyncEmbeddedManifestType = @"manifest";
NSString * const ABI41_0_0EXSyncEmbeddedBundleFilename = @"app";
NSString * const ABI41_0_0EXSyncEmbeddedBundleFileType = @"bundle";
NSString * const ABI41_0_0EXSyncBareEmbeddedBundleFilename = @"main";
NSString * const ABI41_0_0EXSyncBareEmbeddedBundleFileType = @"jsbundle";

static NSString * const ABI41_0_0EXSyncEmbeddedLoaderErrorDomain = @"ABI41_0_0EXSyncEmbeddedLoader";

@implementation ABI41_0_0EXSyncEmbeddedLoader
...
{
  static ABI41_0_0EXSyncManifest *embeddedManifest;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!config.hasEmbeddedUpdate) {
      embeddedManifest = nil;
    } else if (!embeddedManifest) {
      NSString *path = [[NSBundle mainBundle] pathForResource:ABI41_0_0EXSyncEmbeddedManifestName ofType:ABI41_0_0EXSyncEmbeddedManifestType];
      ...
      if (!manifest) {
        ...
        embeddedManifest = [ABI41_0_0EXSyncManifest updateWithEmbeddedManifest:[mutableManifest copy]
                                                                config:config
                                                              database:database];
...
{
  ABI41_0_0EXSyncManifest *embeddedManifest = [[self class] embeddedManifestWithConfig:self.config
...
- (void)loadUpdateFromUrl:(NSURL *)url
                  success:(ABI41_0_0EXSyncLoaderSuccessBlock)success
                    error:(ABI41_0_0EXSyncLoaderErrorBlock)error
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call ABI41_0_0EXSyncEmbeddedLoader#loadUpdateFromUrl" userInfo:nil];
}
