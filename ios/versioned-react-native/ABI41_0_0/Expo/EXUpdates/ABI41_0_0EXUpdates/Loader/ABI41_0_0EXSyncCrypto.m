...
static NSString * const ABI41_0_0EXSyncCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const ABI41_0_0EXSyncCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const ABI41_0_0EXSyncCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation ABI41_0_0EXSyncCrypto
...
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"ABI41_0_0EXSyncCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }
  ...
  ABI41_0_0EXSyncFileDownloader *fileDownloader = [[ABI41_0_0EXSyncFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
  [fileDownloader downloadDataFromURL:[NSURL URLWithString:ABI41_0_0EXSyncCryptoPublicKeyUrl]
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(ABI41_0_0EXSyncVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[ABI41_0_0EXSyncCryptoPublicKeyTag UTF8String] length:[ABI41_0_0EXSyncCryptoPublicKeyTag length]];
