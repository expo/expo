...
#import <ABI41_0_0EXUpdates/ABI41_0_0EXSyncModule.h>
...
@interface ABI41_0_0EXSyncModule ()

@property (nonatomic, weak) id<ABI41_0_0EXSyncInterface> updatesService;

@end

@implementation ABI41_0_0EXSyncModule

ABI41_0_0UM_EXPORT_MODULE(ExpoUpdates);

- (void)setModuleRegistry:(ABI41_0_0UMModuleRegistry *)moduleRegistry
{
  _updatesService = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI41_0_0EXSyncInterface)];
}
...
- (NSDictionary *)constantsToExport
{
  ...
  ABI41_0_0EXSyncManifest *launchedUpdate = _updatesService.launchedUpdate;
  if (!launchedUpdate) {
    return @{
      @"isEnabled": @(NO),
      @"isMissingRuntimeVersion": @(_updatesService.config.isMissingRuntimeVersion)
                         ...
                         reject:(ABI41_0_0UMPromiseRejectBlock)reject)
{
  if (!_updatesService.canRelaunch) {
    reject(@"ERR_UPDATES_DISABLED", @"The updates module controller has not been properly initialized. If you're in development mode, you cannot use this method. Otherwise, make sure you have called [[ABI41_0_0EXSyncController sharedInstance] start].", nil);
    return;
  }
  ...
  [_updatesService requestRelaunchWithCompletion:^(BOOL success) {
    if (success) {
      ...
      reject(@"ERR_UPDATES_RELOAD", @"Could not reload application. Ensure you have set the `bridge` property of ABI41_0_0EXSyncController.", nil);
    }
  }];
}
...
{
  if (!_updatesService.isStarted) {
    reject(@"ERR_UPDATES_DISABLED", @"The updates module controller has not been properly initialized. If you're in development mode, you cannot check for updates. Otherwise, make sure you have called [[ABI41_0_0EXSyncController sharedInstance] start].", nil);
    return;
  }
  ...
  ABI41_0_0EXSyncFileDownloader *fileDownloader = [[ABI41_0_0EXSyncFileDownloader alloc] initWithUpdatesConfig:_updatesService.config];
  [fileDownloader downloadManifestFromURL:_updatesService.config.updateUrl
                             withDatabase:_updatesService.database
                             extraHeaders:extraHeaders
                             successBlock:^(ABI41_0_0EXSyncManifest *update) {
    ABI41_0_0EXSyncManifest *launchedUpdate = self->_updatesService.launchedUpdate;
    id<ABI41_0_0EXSyncSelectionPolicy> selectionPolicy = self->_updatesService.selectionPolicy;
...
{
  if (!_updatesService.isStarted) {
    reject(@"ERR_UPDATES_DISABLED", @"The updates module controller has not been properly initialized. If you're in development mode, you cannot fetch updates. Otherwise, make sure you have called [[ABI41_0_0EXSyncController sharedInstance] start].", nil);
    return;
  }

  ABI41_0_0EXSyncRemoteLoader *remoteAppLoader = [[ABI41_0_0EXSyncRemoteLoader alloc] initWithConfig:_updatesService.config database:_updatesService.database directory:_updatesService.directory completionQueue:self.methodQueue];
  [remoteAppLoader loadUpdateFromUrl:_updatesService.config.updateUrl onManifest:^BOOL(ABI41_0_0EXSyncManifest * _Nonnull update) {
