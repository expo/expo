...
// Prior to ABI41_0_0React Native 0.61, it contained `ABI41_0_0RCTAssetsLibraryRequestHandler` that handles loading data from URLs
// with `assets-library://` or `ph://` schemes. Due to lean core project, it's been moved to `@ABI41_0_0React-native-community/cameraroll`
// and that made it impossible to render assets using URLs returned by MediaLibrary without installing CameraRoll.
// Because it's still a unimodule and we need to export bare ABI41_0_0React Native module, we should make sure ABI41_0_0React Native is installed.
#if __has_include(<ABI41_0_0React/ABI41_0_0RCTImageURLLoader.h>)
...
@implementation ABI41_0_0EXMediaLibraryImageLoader

ABI41_0_0RCT_EXPORT_MODULE()

#pragma mark - ABI41_0_0RCTImageURLLoader
                                 ...
                                 completionHandler:(ABI41_0_0RCTImageLoaderCompletionBlock)completionHandler
{
  ...
  if (!imageURL) {
    completionHandler(ABI41_0_0RCTErrorWithMessage(@"Cannot load a photo library asset with no URL"), nil);
    return ^{};
  ...
  if (results.count == 0) {
    ...
    completionHandler(ABI41_0_0RCTErrorWithMessage(errorText), nil);
    return ^{};
  }
  ...
  if (resizeMode == ABI41_0_0RCTResizeModeContain) {
