...
#import <ABI41_0_0EXDocumentPicker/ABI41_0_0EXDocumentPickerModule.h>
...
#import <MobileCoreServices/MobileCoreServices.h>
  ...
  static UTType* ABI41_0_0EXConvertMimeTypeToUTType(NSString *mimeType)
  {
...
static NSString * ABI41_0_0EXConvertMimeTypeToUTI(NSString *mimeType)
{
  CFStringRef uti;
...
@interface ABI41_0_0EXDocumentPickerModule () <UIDocumentPickerDelegate, UIAdaptivePresentationControllerDelegate>
...
@implementation ABI41_0_0EXDocumentPickerModule

ABI41_0_0UM_EXPORT_MODULE(ExpoDocumentPicker);
...
ABI41_0_0UM_EXPORT_METHOD_AS(getDocumentAsync,
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 140000
      if (@available(iOS 14, *)) {
        UTType* utType = ABI41_0_0EXConvertMimeTypeToUTType(mimeType);
        documentPickerVC = [[UIDocumentPickerViewController alloc] initForOpeningContentTypes:@[utType] asCopy:YES];
      } else {
#endif
        NSString* type = ABI41_0_0EXConvertMimeTypeToUTI(mimeType);
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url
{
  ...
  unsigned long long fileSize = [ABI41_0_0EXDocumentPickerModule getFileSize:[url path] error:&fileSizeError];
  if (fileSizeError) {
...
+ (unsigned long long)getFileSize:(NSString *)path error:(NSError **)error
{
  ...
  while (file = [contentsEnumurator nextObject]) {
    folderSize += [ABI41_0_0EXDocumentPickerModule getFileSize:[path stringByAppendingPathComponent:file] error:error];
    if (*error) {
      return 0;
    }
  }
