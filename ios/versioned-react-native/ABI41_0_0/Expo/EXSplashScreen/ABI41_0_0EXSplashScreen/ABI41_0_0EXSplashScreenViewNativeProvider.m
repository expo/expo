...
@implementation ABI41_0_0EXSplashScreenViewNativeProvider
...
- (nonnull UIView *)createSplashScreenView
{
  ...
  @try {
    ...
    ABI41_0_0UMLogWarn([NSString stringWithFormat:@"'%@.storyboard' file is missing. Fallbacking to '%@.xib' file.", splashScreenFilename, splashScreenFilename]);
  }
  if (storyboard) {
    @try {
  ...
  @try {
    ...
    ABI41_0_0UMLogWarn([NSString stringWithFormat:@"'%@.xib' file is missing - 'expo-splash-screen' will not work as expected.", splashScreenFilename]);
  }
  if (views) {
