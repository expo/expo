...
@interface ABI41_0_0EXAppleAuthenticationRequest ()
...
@implementation ABI41_0_0EXAppleAuthenticationRequest
...
- (void)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
{
  ...
  request.requestedScopes = [ABI41_0_0EXAppleAuthenticationMappings importScopes:requestedScopes];
...
{
  ABI41_0_0EXAppleAuthenticationRequest *authRequest = [[ABI41_0_0EXAppleAuthenticationRequest alloc] initWithOptions:options andCallback:callback];
   ...
   didCompleteWithAuthorization:(ASAuthorization *)authorization
{
  ...
  if (credential.fullName) {
    fullName = [ABI41_0_0EXAppleAuthenticationMappings exportFullName:credential.fullName];
  }
  NSString *authorizationCode;
  if (credential.authorizationCode) {
    authorizationCode = [ABI41_0_0EXAppleAuthenticationMappings exportData:credential.authorizationCode];
  }
  NSString *identityToken;
  if (credential.identityToken) {
    identityToken = [ABI41_0_0EXAppleAuthenticationMappings exportData:credential.identityToken];
  }

  NSDictionary *response = @{
                         @"fullName": ABI41_0_0UMNullIfNil(fullName),
                         @"email": ABI41_0_0UMNullIfNil(credential.email),
                         @"user": credential.user,
                         @"realUserStatus": [ABI41_0_0EXAppleAuthenticationMappings exportRealUserStatus:credential.realUserStatus],
                         @"state": ABI41_0_0UMNullIfNil(credential.state),
                         @"authorizationCode": ABI41_0_0UMNullIfNil(authorizationCode),
                         @"identityToken": ABI41_0_0UMNullIfNil(identityToken),
                         };
