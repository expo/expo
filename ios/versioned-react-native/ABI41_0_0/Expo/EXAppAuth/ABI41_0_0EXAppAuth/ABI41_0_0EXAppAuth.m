...
static NSString *const ABI41_0_0EXAppAuthError = @"ERR_APP_AUTH";

@interface ABI41_0_0EXAppAuth ()
...
@property (nonatomic, weak) id<ABI41_0_0EXAppAuthSessionsManagerInterface> sessionsManager;

@end

@implementation ABI41_0_0EXAppAuth

#pragma mark - Expo

ABI41_0_0UM_EXPORT_MODULE(ExpoAppAuth);
...
{
  return @{
           @"OAuthRedirect": ABI41_0_0UMNullIfNil([self _getOAuthRedirect]),
           @"URLSchemes": ABI41_0_0UMNullIfNil([[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleURLTypes"]),
           };
}

ABI41_0_0UM_EXPORT_METHOD_AS(executeAsync,
                             ...
                             reject:(ABI41_0_0UMPromiseRejectBlock)reject
{
  ...
  [ABI41_0_0UMUtilities performSynchronouslyOnMainThread:^{
    ...
    __weak id<ABI41_0_0EXAppAuthSessionsManagerInterface> sessionsManager = self->_sessionsManager;
    OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
      ...
      if (authState) {
        NSDictionary *tokenResponse = [ABI41_0_0EXAppAuth _tokenResponseNativeToJSON:authState.lastTokenResponse request:options];
        resolve(tokenResponse);
      } else {
        ABI41_0_0EXrejectWithError(reject, error);
      }
    };
...
{
  NSDictionary *tokenResponse = [ABI41_0_0EXAppAuth tokenResponseNativeToJSON:input];
  ...
  [output setValue:ABI41_0_0UMNullIfNil(refreshToken) forKey:@"refreshToken"];
...
- (void)_refreshWithConfiguration:(OIDServiceConfiguration *)configuration
  ...
  OIDTokenCallback callback = ^(OIDTokenResponse *_Nullable response, NSError *_Nullable error) {
    if (response) {
      NSDictionary *tokenResponse = [ABI41_0_0EXAppAuth _tokenResponseNativeToJSON:response request:options];
      resolve(tokenResponse);
    } else {
      ABI41_0_0EXrejectWithError(reject, error);
    }
...
// ABI41_0_0EX prefix for versioning to work smoothly
void ABI41_0_0EXrejectWithError(ABI41_0_0UMPromiseRejectBlock reject, NSError *error) {
  NSString *errorMessage = [NSString stringWithFormat:@"%@: %@", ABI41_0_0EXAppAuthError, error.localizedDescription];
