...
const NSInteger ABI41_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI41_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI41_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI41_0_0RCTUniqueCoalescingKeyGenerator = 0;

@implementation ABI41_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI41_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI41_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI41_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendTextEventWithType:(ABI41_0_0RCTTextEventType)type
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI41_0_0RCTComponentEvent *event = [[ABI41_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI41_0_0ReactTag body:body];
...
- (void)sendEvent:(id<ABI41_0_0RCTEvent>)event
{
  ...
  for (id<ABI41_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI41_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI41_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI41_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI41_0_0RCTGetEventID(event.viewTag, event.eventName, ABI41_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI41_0_0RCTAssert(
        previousEvent == nil,
...
- (void)addDispatchObserver:(id<ABI41_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI41_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
@implementation ABI41_0_0RCTBridge (ABI41_0_0RCTEventDispatcher)

- (ABI41_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI41_0_0RCTEventDispatcher class]];
}
