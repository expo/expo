...
#import "ABI41_0_0RCTSurfaceView+Internal.h"
...
#import "ABI41_0_0RCTSurfaceDelegate.h"
#import "ABI41_0_0RCTSurfaceRootShadowView.h"
#import "ABI41_0_0RCTSurfaceRootShadowViewDelegate.h"
#import "ABI41_0_0RCTSurfaceRootView.h"
#import "ABI41_0_0RCTSurfaceView.h"
...
@interface ABI41_0_0RCTSurface () <ABI41_0_0RCTSurfaceRootShadowViewDelegate, ABI41_0_0RCTUIManagerObserver>
@end

@implementation ABI41_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI41_0_0RCTBridge *_batchedBridge;
  ABI41_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI41_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI41_0_0RCTSurfaceView *_Nullable _view;
  ABI41_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI41_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instantiate `ABI41_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI41_0_0RCTAllocateRootViewTag();
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBridgeWillLoadJavaScriptNotification:)
                                                 name:ABI41_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    _stage = ABI41_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI41_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI41_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI41_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI41_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI41_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI41_0_0RCTSurfaceRootView *rootView = (ABI41_0_0RCTSurfaceRootView *)[self._uiManager viewForABI41_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI41_0_0RCTAssert(
      [rootView isKindOfClass:[ABI41_0_0RCTSurfaceRootView class]],
      @"Received root view is not an instance of `ABI41_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI41_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI41_0_0RCTSurfaceStageModuleDidLoad];
...
- (ABI41_0_0RCTSurfaceStage)stage
{
  std::lock_guard<std::mutex> lock(_mutex);
  return _stage;
}

- (void)_setStage:(ABI41_0_0RCTSurfaceStage)stage
{
  ABI41_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI41_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI41_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI41_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI41_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI41_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI41_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI41_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI41_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI41_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI41_0_0RCTIsMainQueue() ? ABI41_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI41_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI41_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI41_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI41_0_0ReactTag:self->_rootViewTag];
    ABI41_0_0RCTAssert(
        [rootShadowView isKindOfClass:[ABI41_0_0RCTSurfaceRootShadowView class]],
        @"Received shadow view is not an instance of `ABI41_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI41_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI41_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI41_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI41_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI41_0_0ReactTag:self->_rootViewTag];
...
- (BOOL)synchronouslyWaitForStage:(ABI41_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI41_0_0RCTIsUIManagerQueue()) {
    ABI41_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI41_0_0RCTIsMainQueue() && (stage & ABI41_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI41_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI41_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI41_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI41_0_0RCTAssert(
          NO,
          @"Only waiting for `ABI41_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI41_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI41_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI41_0_0RCTIsMainQueue() && (stage & ABI41_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI41_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI41_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI41_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI41_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI41_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI41_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI41_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI41_0_0RCTUIManager *)manager performMountingWithBlock:(ABI41_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI41_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI41_0_0RCTUIManager *)manager
{
  if (self.stage & ABI41_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI41_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ABI41_0_0React components

- (void)mountABI41_0_0ReactComponentWithBridge:(ABI41_0_0RCTBridge *)bridge
                           moduleName:(NSString *)moduleName
...
- (void)unmountABI41_0_0ReactComponentWithBridge:(ABI41_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
