...
#import "ABI41_0_0RCTConstants.h"
...
@interface ABI41_0_0RCTSurfaceHostingView ()
...
@implementation ABI41_0_0RCTSurfaceHostingView {
...
{
  return [[ABI41_0_0RCTSurface alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProperties];
}

ABI41_0_0RCT_NOT_IMPLEMENTED(-(instancetype)init)
ABI41_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithFrame : (CGRect)frame)
ABI41_0_0RCT_NOT_IMPLEMENTED(-(nullable instancetype)initWithCoder : (NSCoder *)coder)
...
- (instancetype)initWithSurface:(ABI41_0_0RCTSurface *)surface sizeMeasureMode:(ABI41_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
...
- (void)setFrame:(CGRect)frame
{
  ...
  ABI41_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(
...
{
  if (ABI41_0_0RCTSurfaceStageIsPreparing(_stage)) {
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  ABI41_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(size, _sizeMeasureMode, &minimumSize, &maximumSize);

  return [_surface sizeThatFitsMinimumSize:minimumSize maximumSize:maximumSize];
}

- (void)setStage:(ABI41_0_0RCTSurfaceStage)stage
{
  if (stage == _stage) {
    return;
  }

  BOOL shouldInvalidateLayout = ABI41_0_0RCTSurfaceStageIsRunning(stage) != ABI41_0_0RCTSurfaceStageIsRunning(_stage) ||
      ABI41_0_0RCTSurfaceStageIsPreparing(stage) != ABI41_0_0RCTSurfaceStageIsPreparing(_stage);
...
- (void)setSizeMeasureMode:(ABI41_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
...
- (void)setActivityIndicatorViewFactory:(ABI41_0_0RCTSurfaceHostingViewActivityIndicatorViewFactory)activityIndicatorViewFactory
{
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection
{
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI41_0_0RCTUserInterfaceStyleDidChangeNotification
                    object:self
                  userInfo:@{
                    ABI41_0_0RCTUserInterfaceStyleDidChangeNotificationTraitCollectionKey : self.traitCollection,
                  }];
}
...
- (void)_updateViews
{
  self.isSurfaceViewVisible = ABI41_0_0RCTSurfaceStageIsRunning(_stage);
  self.isActivityIndicatorViewVisible = ABI41_0_0RCTSurfaceStageIsPreparing(_stage);
}
...
#pragma mark - ABI41_0_0RCTSurfaceDelegate

- (void)surface:(__unused ABI41_0_0RCTSurface *)surface didChangeStage:(ABI41_0_0RCTSurfaceStage)stage
{
...
- (void)surface:(__unused ABI41_0_0RCTSurface *)surface didChangeIntrinsicSize:(__unused CGSize)intrinsicSize
{
  ABI41_0_0RCTExecuteOnMainQueue(^{
    [self _invalidateLayout];
  });
