 ...
 */
static NSRegularExpression *ABI41_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI41_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI41_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI41_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI41_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI41_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI41_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI41_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI41_0_0RCTNilIfNull(dict[@"collapse"]) integerValue]];
}
...
+ (NSArray<ABI41_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI41_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI41_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI41_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
