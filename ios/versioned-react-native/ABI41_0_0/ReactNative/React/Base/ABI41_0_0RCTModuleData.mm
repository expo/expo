...
@implementation ABI41_0_0RCTModuleData {
  NSDictionary<NSString *, id> *_constantsToExport;
  NSString *_queueName;
  __weak ABI41_0_0RCTBridge *_bridge;
  ABI41_0_0RCTBridgeModuleProvider _moduleProvider;
    ...
    if (_requiresMainQueueSetup) {
      ...
      ABI41_0_0RCTLogWarn(
          @"Module %@ requires main queue setup since it overrides `%s` but doesn't implement "
           "`requiresMainQueueSetup`. In a future release ABI41_0_0React Native will default to initializing all native modules "
...
- (instancetype)initWithModuleClass:(Class)moduleClass bridge:(ABI41_0_0RCTBridge *)bridge
{
  return [self initWithModuleClass:moduleClass
                    moduleProvider:^id<ABI41_0_0RCTBridgeModule> {
...
- (instancetype)initWithModuleClass:(Class)moduleClass
                     moduleProvider:(ABI41_0_0RCTBridgeModuleProvider)moduleProvider
...
- (instancetype)initWithModuleInstance:(id<ABI41_0_0RCTBridgeModule>)instance bridge:(ABI41_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
    _instance = instance;
...
- (void)setUpInstanceAndBridge
{
  ABI41_0_0RCT_PROFILE_BEGIN_EVENT(
      ...
      @"[ABI41_0_0RCTModuleData setUpInstanceAndBridge]",
      @{@"moduleClass" : NSStringFromClass(_moduleClass)});
  {
    ...
    if (!_setupComplete && _bridge.valid) {
      if (!_instance) {
        if (ABI41_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI41_0_0RCTAssertMainQueue();
        }
        ABI41_0_0RCT_PROFILE_BEGIN_EVENT(ABI41_0_0RCTProfileTagAlways, @"[ABI41_0_0RCTModuleData setUpInstanceAndBridge] Create module", nil);
      ...
      if (_instance && ABI41_0_0RCTProfileIsProfiling()) {
        ABI41_0_0RCTProfileHookInstance(_instance);
      }
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI41_0_0RCT_PROFILE_BEGIN_EVENT(ABI41_0_0RCTProfileTagAlways, @"[ABI41_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI41_0_0RCT_PROFILE_BEGIN_EVENT(ABI41_0_0RCTProfileTagAlways, @"[ABI41_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:ABI41_0_0RCTDidInitializeModuleNotification
                      object:_bridge
                    userInfo:@{@"module" : _instance, @"bridge" : ABI41_0_0RCTNullIfNil(_bridge.parentBridge)}];
    ABI41_0_0RCT_PROFILE_END_EVENT(ABI41_0_0RCTProfileTagAlways, @"");
  }
}
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI41_0_0RCT_PROFILE_BEGIN_EVENT(ABI41_0_0RCTProfileTagAlways, @"[ABI41_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ABI41_0_0React.%@Queue", self.name];
...
- (void)calculateMethods
{
  ...
  NSMutableArray<id<ABI41_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
  NSMutableDictionary<NSString *, id<ABI41_0_0RCTBridgeMethod>> *moduleMethodsByName = [NSMutableDictionary new];
  ...
  while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
      ...
      if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
        ...
        auto exportedMethod = ((const ABI41_0_0RCTMethodInfo *(*)(id, SEL))imp)(_moduleClass, selector);
        id<ABI41_0_0RCTBridgeMethod> moduleMethod = [[ABI41_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod
...
- (id<ABI41_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI41_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI41_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI41_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI41_0_0RCT_PROFILE_BEGIN_EVENT(ABI41_0_0RCTProfileTagAlways, @"[ABI41_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI41_0_0RCTIsMainQueue()) {
        ABI41_0_0RCTLogWarn(@"ABI41_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI41_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI41_0_0RCTBridgeMethod>> *)methods
{
  [self calculateMethods];
  return _methods;
}

- (NSDictionary<NSString *, id<ABI41_0_0RCTBridgeMethod>> *)methodsByName
{
  [self calculateMethods];
...
- (void)gatherConstants
{
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI41_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI41_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI41_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    if (_requiresMainQueueSetup) {
      if (!ABI41_0_0RCTIsMainQueue()) {
        ABI41_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
      }
...
- (dispatch_queue_t)methodQueue
{
  if (_bridge.valid) {
    ...
    ABI41_0_0RCTAssert(_methodQueue != nullptr, @"Module %@ has no methodQueue (instance: %@)", self, instance);
  }
  return _methodQueue;
}
