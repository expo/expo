...
#import <ABI41_0_0React/ABI41_0_0RCTRedBoxExtraDataViewController.h>
...
@class ABI41_0_0RCTRedBoxWindow;

@interface UIButton (ABI41_0_0RCTRedBox)

@property (nonatomic) ABI41_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI41_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI41_0_0RCTRedBox)

- (ABI41_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI41_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI41_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI41_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI41_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI41_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI41_0_0RCTRedBoxWindow : NSObject <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UIViewController *rootViewController;
@property (nonatomic, weak) id<ABI41_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI41_0_0RCTRedBoxWindow {
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI41_0_0RCTJSStackFrame *> *_lastStackTrace;
  int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame
           customButtonTitles:(NSArray<NSString *> *)customButtonTitles
         customButtonHandlers:(NSArray<ABI41_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
  if (self = [super init]) {
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title
    accessibilityIdentifier:(NSString *)accessibilityIdentifier
                   selector:(SEL)selector
                      block:(ABI41_0_0RCTRedBoxButtonPressHandler)block
{
...
- (NSInteger)bottomSafeViewHeight
{
  if (@available(iOS 11.0, *)) {
    return ABI41_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
  } else {
    return 0;
  }
}
...
- (void)showErrorMessage:(NSString *)message
               withStack:(NSArray<ABI41_0_0RCTJSStackFrame *> *)stack
    ...
    if (!isRootViewControllerPresented) {
      [_stackTraceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]
                                  atScrollPosition:UITableViewScrollPositionTop
                                          animated:NO];
      [ABI41_0_0RCTKeyWindow().rootViewController presentViewController:self.rootViewController animated:YES completion:nil];
    }
  }
}
...
- (void)copyStack
{
  ...
  for (ABI41_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI41_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI41_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI41_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI41_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI41_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI41_0_0RCTKeyCommands for this, but since
...
@interface ABI41_0_0RCTRedBox () <
    ABI41_0_0RCTInvalidating,
    ABI41_0_0RCTRedBoxWindowActionDelegate,
    ABI41_0_0RCTRedBoxExtraDataActionDelegate,
    ABI41_0_0NativeRedBoxSpec>
@end

@implementation ABI41_0_0RCTRedBox {
  ABI41_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI41_0_0RCTErrorCustomizer>> *_errorCustomizers;
  ABI41_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
  NSMutableArray<NSString *> *_customButtonTitles;
  NSMutableArray<ABI41_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (ABI41_0_0RCTErrorInfo *)_customizeError:(ABI41_0_0RCTErrorInfo *)error
{
  ABI41_0_0RCTAssertMainQueue();
  if (!self->_errorCustomizers) {
    return error;
  }
  for (id<ABI41_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI41_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI41_0_0RCTJSStackTraceKey]
             errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message
             withDetails:(NSString *)details
                   stack:(NSArray<ABI41_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
...
{
  NSArray<ABI41_0_0RCTJSStackFrame *> *stack = [ABI41_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
  [self showErrorMessage:message
         withParsedStack:[ABI41_0_0RCTJSStackFrame stackFramesWithDictionaries:stack]
                isUpdate:NO
             ...
             errorCookie:(int)errorCookie
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self->_extraDataViewController == nil) {
      self->_extraDataViewController = [ABI41_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
    ...
    if (!self->_window) {
      self->_window = [[ABI41_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds
    ...
    ABI41_0_0RCTErrorInfo *errorInfo = [[ABI41_0_0RCTErrorInfo alloc] initWithErrorMessage:message stack:stack];
...
ABI41_0_0RCT_EXPORT_METHOD(setExtraData : (NSDictionary *)extraData forIdentifier : (NSString *)identifier)
{
  [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI41_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI41_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI41_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI41_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI41_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI41_0_0RCTRedBoxWindow *)redBoxWindow
{
  if (_overrideReloadAction) {
    _overrideReloadAction();
  } else {
    ABI41_0_0RCTTriggerReloadCommandListeners(@"Redbox");
  }
  [self dismiss];
}
...
{
  return std::make_shared<ABI41_0_0facebook::ABI41_0_0React::NativeRedBoxSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

@implementation ABI41_0_0RCTBridge (ABI41_0_0RCTRedBox)

- (ABI41_0_0RCTRedBox *)redBox
{
  return ABI41_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI41_0_0RCTRedBox class]] : nil;
}
...
@interface ABI41_0_0RCTRedBox () <ABI41_0_0NativeRedBoxSpec>
@end
...
Class ABI41_0_0RCTRedBoxCls(void)
{
  return ABI41_0_0RCTRedBox.class;
}
