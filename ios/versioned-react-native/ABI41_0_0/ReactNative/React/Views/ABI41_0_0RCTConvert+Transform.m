...
@implementation ABI41_0_0RCTConvert (Transform)
...
+ (CATransform3D)CATransform3DFromMatrix:(id)json
{
  ...
  for (NSUInteger i = 0; i < kMatrixArrayLength; i++) {
    ((CGFloat *)&transform)[i] = [ABI41_0_0RCTConvert CGFloat:json[i]];
  }
  return transform;
}
...
+ (CATransform3D)CATransform3D:(id)json
{
  ...
  if ([(NSArray *)json count] == kMatrixArrayLength && [json[0] isKindOfClass:[NSNumber class]]) {
    ABI41_0_0RCTLogWarn(
        @"[ABI41_0_0RCTConvert CATransform3D:] has deprecated a matrix as input. Pass an array of configs (which can contain a matrix key) instead.");
