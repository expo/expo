...
@interface ABI41_0_0RCTPicker () <UIPickerViewDataSource, UIPickerViewDelegate, UIPickerViewAccessibilityDelegate>
@end

@implementation ABI41_0_0RCTPicker
...
{
  return [ABI41_0_0RCTConvert NSString:_items[row][@"label"]];
}
           ...
           reusingView:(UILabel *)label
{
  ...
  label.textColor = [ABI41_0_0RCTConvert UIColor:_items[row][@"textColor"]] ?: _color;
       ...
       inComponent:(__unused NSInteger)component
{
  ...
  if (_onChange && _items.count > (NSUInteger)row) {
    _onChange(@{
      @"newIndex" : @(row),
      @"newValue" : ABI41_0_0RCTNullIfNil(_items[row][@"value"]),
    });
  }
}
