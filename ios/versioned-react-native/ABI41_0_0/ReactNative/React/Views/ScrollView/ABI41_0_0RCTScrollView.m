 ...
 */
@interface ABI41_0_0RCTCustomScrollView : UIScrollView <UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI41_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
@property (nonatomic, assign) BOOL pinchGestureEnabled;
#endif

@end

@implementation ABI41_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI41_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI41_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI41_0_0RCTUIManager JSResponder] isKindOfClass:[ABI41_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI41_0_0React scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
      ABI41_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
      ABI41_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI41_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI41_0_0RCTScrollView () <ABI41_0_0RCTUIManagerObserver>

@end

@implementation ABI41_0_0RCTScrollView {
  ABI41_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI41_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
{
  ABI41_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI41_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI41_0_0RCTApplyTransformationAccordingLayoutDirection(
    UIView *view,
...
{
  [super setABI41_0_0ReactLayoutDirection:layoutDirection];

  ABI41_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI41_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI41_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI41_0_0ReactSubview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI41_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI41_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI41_0_0RCTAssert(
        _contentView == nil,
        @"ABI41_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@",
        [_contentView ABI41_0_0React_recursiveDescription]);
    _contentView = view;
    ABI41_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI41_0_0ReactLayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI41_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI41_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI41_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI41_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI41_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI41_0_0React_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ABI41_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI41_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:NO];
...
{
  [ABI41_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:YES];
}

#pragma mark - ScrollView delegate

#define ABI41_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData)                                    \
  {                                                                                     \
...
#define ABI41_0_0RCT_FORWARD_SCROLL_EVENT(call)                                            \
...
#define ABI41_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
  -(void)delegateMethod : (UIScrollView *)scrollView        \
  {                                                         \
    ABI41_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                  \
    ABI41_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod : scrollView);  \
  }

ABI41_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI41_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI41_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI41_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames" : [self calculateChildFramesData]}));
    } else {
      ABI41_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll : scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
  NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
  [[_contentView ABI41_0_0ReactSubviews] enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, __unused BOOL *stop) {
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI41_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging : scrollView);
}
              ...
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI41_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging
...
{
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging : scrollView willDecelerate : decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI41_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming : scrollView withView : view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI41_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming : scrollView withView : view atScale : scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI41_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating : scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI41_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI41_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation : scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI41_0_0RCTView contentInsetsForView:self];
...
{
  ABI41_0_0RCTAssertUIManagerQueue();
  [manager
      prependUIBlock:^(__unused ABI41_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI41_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI41_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI41_0_0RCTScrollEvent *scrollEvent = [[ABI41_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ABI41_0_0ReactTag:self.ABI41_0_0ReactTag
...
@implementation ABI41_0_0RCTEventDispatcher (ABI41_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ABI41_0_0ReactTag
{
  ...
  ABI41_0_0RCTScrollEvent *fakeScrollEvent = [[ABI41_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI41_0_0ReactTag:ABI41_0_0ReactTag
