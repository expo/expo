...
#import "ABI41_0_0RCTComponentEvent.h"
...
typedef void (^ABI41_0_0RCTPropBlock)(id<ABI41_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI41_0_0RCTPropBlock> ABI41_0_0RCTPropBlockDictionary;
...
@implementation ABI41_0_0RCTComponentData {
  id<ABI41_0_0RCTComponent> _defaultView; // Only needed for ABI41_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI41_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI41_0_0RCTPropBlockDictionary *_shadowPropBlocks;
  __weak ABI41_0_0RCTBridge *_bridge;
}

@synthesize manager = _manager;

- (instancetype)initWithManagerClass:(Class)managerClass bridge:(ABI41_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _bridge = bridge;
...
- (ABI41_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI41_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI41_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI41_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI41_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI41_0_0RCTPropBlock
createEventSetter(NSString *propName, SEL setter, InterceptorBlock eventInterceptor, ABI41_0_0RCTBridge *bridge)
{
  __weak ABI41_0_0RCTBridge *weakBridge = bridge;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI41_0_0RCTConvert BOOL:json]) {
      __weak id<ABI41_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI41_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI41_0_0ReactTag);
        } else {
          ABI41_0_0RCTComponentEvent *componentEvent = [[ABI41_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI41_0_0ReactTag
                                                                                 body:event];
...
static ABI41_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI41_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI41_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI41_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI41_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI41_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock = createEventSetter(name, setter, self.eventInterceptor, _bridge);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI41_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI41_0_0RCTLogError(@"No +[ABI41_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI41_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI41_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI41_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI41_0_0RCT_CASE(_C_SEL, SEL)
        ABI41_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI41_0_0RCT_CASE(_C_CHR, char)
        ABI41_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI41_0_0RCT_CASE(_C_SHT, short)
        ABI41_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI41_0_0RCT_CASE(_C_INT, int)
        ABI41_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI41_0_0RCT_CASE(_C_LNG, long)
        ABI41_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI41_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI41_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI41_0_0RCT_CASE(_C_FLT, float)
        ABI41_0_0RCT_CASE(_C_DBL, double)
        ABI41_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI41_0_0RCT_CASE(_C_PTR, void *)
        ABI41_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI41_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI41_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI41_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI41_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI41_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI41_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI41_0_0ReactTag];
      ABI41_0_0RCTPerformBlockWithLogPrefix(
          ^{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI41_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI41_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI41_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
    ...
    if ([type isEqualToString:@"ABI41_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI41_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI41_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI41_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
...
{
  // Hackety hack, this partially re-implements ABI41_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI41_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI41_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI41_0_0EX_REMOVE_VERSION(name);
  ...
  ABI41_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
