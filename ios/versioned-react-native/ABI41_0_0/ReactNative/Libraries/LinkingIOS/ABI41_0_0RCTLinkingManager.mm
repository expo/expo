...
#import <ABI41_0_0React/ABI41_0_0RCTLinkingManager.h>
...
static NSString *const kOpenURLNotification = @"ABI41_0_0RCTOpenURLNotification";
...
@interface ABI41_0_0RCTLinkingManager() <ABI41_0_0NativeLinkingSpec>
@end

@implementation ABI41_0_0RCTLinkingManager
    ...
    if (success) {
      resolve(@YES);
        ...
        if([URL.absoluteString hasPrefix:@"tel:"]){
          ABI41_0_0RCTLogWarn(@"Unable to open the Phone app in the simulator for telephone URLs. URL:  %@", URL);
          resolve(@NO);
        } else {
                  ...
                  reject:(__unused ABI41_0_0RCTPromiseRejectBlock)reject)
{
  ...
  BOOL canOpen = [ABI41_0_0RCTSharedApplication() canOpenURL:URL];
  ...
  if (canOpen) {
    ...
    if (querySchemes != nil && ([querySchemes containsObject:scheme] || [querySchemes containsObject:[scheme lowercaseString]])) {
      ...
      reject(ABI41_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@. Add %@ to LSApplicationQueriesSchemes in your Info.plist.", URL, scheme], nil);
    }
  } else {
    resolve(@NO);
  }
                  ...
                  reject:(__unused ABI41_0_0RCTPromiseRejectBlock)reject)
{
  ...
  resolve(ABI41_0_0RCTNullIfNil(initialURL.absoluteString));
}
...
ABI41_0_0RCT_EXPORT_METHOD(sendIntent:(NSString *)action
...
{
  return std::make_shared<ABI41_0_0facebook::ABI41_0_0React::NativeLinkingSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

Class ABI41_0_0RCTLinkingManagerCls(void) {
  return ABI41_0_0RCTLinkingManager.class;
}
