...
static NSString *const ABI41_0_0RCTImageStoreURLScheme = @"rct-image-store";

@interface ABI41_0_0RCTImageStoreManager() <ABI41_0_0NativeImageStoreSpec>
@end

@implementation ABI41_0_0RCTImageStoreManager
{
...
{
  ABI41_0_0RCTAssertThread(_methodQueue, @"Must be called on ABI41_0_0RCTImageStoreManager thread");
  ...
  NSString *imageTag = [NSString stringWithFormat:@"%@://%tu", ABI41_0_0RCTImageStoreURLScheme, _id++];
...
- (void)storeImage:(UIImage *)image withBlock:(void (^)(NSString *imageTag))block
{
  ABI41_0_0RCTAssertParam(block);
  dispatch_async(_methodQueue, ^{
    NSString *imageTag = [self _storeImageData:ABI41_0_0RCTGetImageData(image, 0.75)];
...
ABI41_0_0RCT_EXPORT_METHOD(removeImageForTag:(NSString *)imageTag)
{
  [_store removeObjectForKey:imageTag];
}

ABI41_0_0RCT_EXPORT_METHOD(hasImageForTag:(NSString *)imageTag
...
ABI41_0_0RCT_EXPORT_METHOD(getBase64ForTag:(NSString *)imageTag
                  ...
                  errorCallback:(ABI41_0_0RCTResponseSenderBlock)errorCallback)
{
  ...
  if (!imageData) {
    errorCallback(@[ABI41_0_0RCTJSErrorFromNSError(ABI41_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]))]);
    return;
  }
...
ABI41_0_0RCT_EXPORT_METHOD(addImageFromBase64:(NSString *)base64String
                  successCallback:(ABI41_0_0RCTResponseSenderBlock)successCallback
                  errorCallback:(ABI41_0_0RCTResponseSenderBlock)errorCallback)

{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (imageData) {
      ...
      errorCallback(@[ABI41_0_0RCTJSErrorFromNSError(ABI41_0_0RCTErrorWithMessage(@"Failed to add image from base64String"))]);
    }
  });
}
...
{
  return [request.URL.scheme caseInsensitiveCompare:ABI41_0_0RCTImageStoreURLScheme] == NSOrderedSame;
}
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI41_0_0RCTURLRequestDelegate>)delegate
{
  ...
  dispatch_async(_methodQueue, ^{
    ...
    if (!imageData) {
      NSError *error = ABI41_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]);
    ...
    if (!sourceRef) {
      NSError *error = ABI41_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to decode data for imageTag: %@", imageTag]);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
      return;
    }
...
@implementation ABI41_0_0RCTImageStoreManager (Deprecated)

- (NSString *)storeImage:(UIImage *)image
{
  ABI41_0_0RCTAssertMainQueue();
  ABI41_0_0RCTLogWarn(@"ABI41_0_0RCTImageStoreManager.storeImage() is deprecated and has poor performance. Use an alternative method instead.");
  __block NSString *imageTag;
  dispatch_sync(_methodQueue, ^{
    imageTag = [self _storeImageData:ABI41_0_0RCTGetImageData(image, 0.75)];
  });
  return imageTag;
}
...
{
  ABI41_0_0RCTAssertMainQueue();
  ABI41_0_0RCTLogWarn(@"ABI41_0_0RCTImageStoreManager.imageForTag() is deprecated and has poor performance. Use an alternative method instead.");
...
{
  return std::make_shared<ABI41_0_0facebook::ABI41_0_0React::NativeImageStoreSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}

@end

@implementation ABI41_0_0RCTBridge (ABI41_0_0RCTImageStoreManager)

- (ABI41_0_0RCTImageStoreManager *)imageStoreManager
{
  return [self moduleForClass:[ABI41_0_0RCTImageStoreManager class]];
}

@end

Class ABI41_0_0RCTImageStoreManagerCls(void) {
  return ABI41_0_0RCTImageStoreManager.class;
}
