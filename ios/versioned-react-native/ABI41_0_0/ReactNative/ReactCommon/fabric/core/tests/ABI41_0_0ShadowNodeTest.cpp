...
TEST_F(ShadowNodeTest, handleShadowNodeCreation) {
  ABI41_0_0EXPECT_FALSE(nodeZ_->getSealed());
  ABI41_0_0EXPECT_STREQ(nodeZ_->getComponentName(), "Test");
  ABI41_0_0EXPECT_EQ(nodeZ_->getTag(), 18);
  ABI41_0_0EXPECT_EQ(nodeZ_->getSurfaceId(), surfaceId_);
  ABI41_0_0EXPECT_EQ(nodeZ_->getEventEmitter(), nullptr);
  ABI41_0_0EXPECT_EQ(nodeZ_->getChildren().size(), 0);
}

TEST_F(ShadowNodeTest, handleSealRecusive) {
  nodeZ_->sealRecursive();
  ABI41_0_0EXPECT_TRUE(nodeZ_->getSealed());
  ABI41_0_0EXPECT_TRUE(nodeZ_->getProps()->getSealed());
}
...
TEST_F(ShadowNodeTest, handleShadowNodeSimpleCloning) {
  ...
  ABI41_0_0EXPECT_STREQ(nodeA_->getComponentName(), nodeARevision2->getComponentName());
  ABI41_0_0EXPECT_EQ(nodeA_->getTag(), nodeARevision2->getTag());
  ABI41_0_0EXPECT_EQ(nodeA_->getSurfaceId(), nodeARevision2->getSurfaceId());
  ABI41_0_0EXPECT_EQ(nodeA_->getEventEmitter(), nodeARevision2->getEventEmitter());
}

TEST_F(ShadowNodeTest, handleShadowNodeMutation) {
  auto nodeABChildren = nodeAB_->getChildren();
  ABI41_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI41_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABA_);
  ABI41_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);
  ...
  ABI41_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI41_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABArevision2);
  ABI41_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);

  // Seal the entire tree.
  nodeAB_->sealRecursive();
  ABI41_0_0EXPECT_TRUE(nodeAB_->getSealed());
  ABI41_0_0EXPECT_TRUE(nodeABArevision2->getSealed());
  ABI41_0_0EXPECT_TRUE(nodeABB_->getSealed());
}
...
TEST_F(ShadowNodeTest, handleCloneFunction) {
  ...
  ABI41_0_0EXPECT_NE(nodeAB_, nodeABClone);
  ...
  ABI41_0_0EXPECT_EQ(nodeAB_->getTag(), nodeABClone->getTag());
  ABI41_0_0EXPECT_EQ(nodeAB_->getSurfaceId(), nodeABClone->getSurfaceId());
  ABI41_0_0EXPECT_EQ(nodeAB_->getProps(), nodeABClone->getProps());
}
...
TEST_F(ShadowNodeTest, handleState) {
  ...
  ABI41_0_0EXPECT_EQ(firstNode->getState(), secondNode->getState());
  ABI41_0_0EXPECT_NE(firstNode->getState(), thirdNode->getState());
  secondNode->setStateData(TestState{42});
  ABI41_0_0EXPECT_NE(firstNode->getState(), secondNode->getState());

  // State cannot be changed for sealed shadow node.
  secondNode->sealRecursive();
  ABI41_0_0EXPECT_ANY_THROW(secondNode->setStateData(TestState{42}));
}
