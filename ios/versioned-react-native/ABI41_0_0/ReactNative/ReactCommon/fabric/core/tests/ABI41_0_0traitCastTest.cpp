...
#include <ABI41_0_0React/components/text/ParagraphComponentDescriptor.h>
#include <ABI41_0_0React/components/text/RawTextComponentDescriptor.h>
#include <ABI41_0_0React/components/text/TextComponentDescriptor.h>
...
TEST(traitCastTest, testOne) {
  ...
  ABI41_0_0EXPECT_FALSE(traitCast<LayoutableShadowNode const *>(nullptr));
  ABI41_0_0EXPECT_FALSE(traitCast<YogaLayoutableShadowNode const *>(nullptr));

  // `ViewShadowNode` is `LayoutableShadowNode` and `YogaLayoutableShadowNode`.
  ABI41_0_0EXPECT_TRUE(traitCast<LayoutableShadowNode const *>(viewShadowNode.get()));
  ABI41_0_0EXPECT_TRUE(
      traitCast<YogaLayoutableShadowNode const *>(viewShadowNode.get()));
  ABI41_0_0EXPECT_NO_FATAL_FAILURE(
  ...
  // `YogaLayoutableShadowNode`.
  ABI41_0_0EXPECT_FALSE(traitCast<LayoutableShadowNode const *>(textShadowNode.get()));
