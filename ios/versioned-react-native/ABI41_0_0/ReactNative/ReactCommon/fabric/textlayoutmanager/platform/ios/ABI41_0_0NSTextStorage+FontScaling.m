...
typedef NS_OPTIONS(NSInteger, ABI41_0_0RCTTextSizeComparisonOptions) {
  ABI41_0_0RCTTextSizeComparisonSmaller = 1 << 0,
  ABI41_0_0RCTTextSizeComparisonLarger = 1 << 1,
  ABI41_0_0RCTTextSizeComparisonWithinRange = 1 << 2,
};

@implementation NSTextStorage (ABI41_0_0FontScaling)
               ...
               maximumFontSize:(CGFloat)maximumFontSize
{
  ...
  while (true) {
    [self abi41_0_0_rct_scaleFontSizeWithRatio:ratio minimumFontSize:minimumFontSize maximumFontSize:maximumFontSize];

    ABI41_0_0RCTTextSizeComparisonOptions comparsion = [self abi41_0_0_rct_compareToSize:size thresholdRatio:0.01];

    if ((comparsion & ABI41_0_0RCTTextSizeComparisonWithinRange) && (comparsion & ABI41_0_0RCTTextSizeComparisonSmaller)) {
      return;
    } else if (comparsion & ABI41_0_0RCTTextSizeComparisonSmaller) {
      bottomRatio = ratio;
    ...
    if (ABS(topRatio - bottomRatio) < kRatioThreshold || ABS(topRatio - ratio) < kRatioThreshold ||
      ...
      [self abi41_0_0_rct_scaleFontSizeWithRatio:lastRatioWhichFits minimumFontSize:minimumFontSize maximumFontSize:maximumFontSize];
      return;
    }
...
- (ABI41_0_0RCTTextSizeComparisonOptions)abi41_0_0_rct_compareToSize:(CGSize)size thresholdRatio:(CGFloat)thresholdRatio
{
  ...
  if (truncatedGlyphRange.location != NSNotFound) {
    return ABI41_0_0RCTTextSizeComparisonLarger;
  }
  ...
  ABI41_0_0RCTTextSizeComparisonOptions result = 0;

  result |= (fitsSize) ? ABI41_0_0RCTTextSizeComparisonSmaller : ABI41_0_0RCTTextSizeComparisonLarger;

  if (ABS(measuredSize.width - size.width) < thresholdSize.width) {
    result = result | ABI41_0_0RCTTextSizeComparisonWithinRange;
  }
