...
#include <ABI31_0_0fabric/ABI31_0_0textlayoutmanager/ABI31_0_0RCTTextPrimitivesConversions.h>

inline static UIFont *ABI31_0_0RCTEffectiveFontFromTextAttributes(const TextAttributes &textAttributes) {
  NSString *fontFamily = [NSString stringWithCString:textAttributes.fontFamily.c_str()
                                            encoding:NSUTF8StringEncoding];

  ABI31_0_0RCTFontProperties fontProperties;
  fontProperties.family = fontFamily;
  fontProperties.size = textAttributes.fontSize;
  fontProperties.style = textAttributes.fontStyle.hasValue() ? ABI31_0_0RCTFontStyleFromFontStyle(textAttributes.fontStyle.value()) : ABI31_0_0RCTFontStyleUndefined;
  fontProperties.variant = textAttributes.fontVariant.hasValue() ? ABI31_0_0RCTFontVariantFromFontVariant(textAttributes.fontVariant.value()) : ABI31_0_0RCTFontVariantDefault;
  ...
  return ABI31_0_0RCTFontWithFontProperties(fontProperties);
}

inline static CGFloat ABI31_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(const TextAttributes &textAttributes) {
...
inline static UIColor *ABI31_0_0RCTEffectiveForegroundColorFromTextAttributes(const TextAttributes &textAttributes) {
  UIColor *effectiveForegroundColor = ABI31_0_0RCTUIColorFromSharedColor(textAttributes.foregroundColor) ?: [UIColor blackColor];
...
inline static UIColor *ABI31_0_0RCTEffectiveBackgroundColorFromTextAttributes(const TextAttributes &textAttributes) {
  UIColor *effectiveBackgroundColor = ABI31_0_0RCTUIColorFromSharedColor(textAttributes.backgroundColor);
...
static NSDictionary<NSAttributedStringKey, id> *ABI31_0_0RCTNSTextAttributesFromTextAttributes(const TextAttributes &textAttributes) {
  NSMutableDictionary<NSAttributedStringKey, id> *attributes =
    [NSMutableDictionary dictionaryWithCapacity:10];

  // Font
  UIFont *font = ABI31_0_0RCTEffectiveFontFromTextAttributes(textAttributes);
  if (font) {
    attributes[NSFontAttributeName] = font;
  }

  // Colors
  UIColor *effectiveForegroundColor = ABI31_0_0RCTEffectiveForegroundColorFromTextAttributes(textAttributes);
  ...
  if (textAttributes.backgroundColor || !isnan(textAttributes.opacity)) {
    attributes[NSBackgroundColorAttributeName] = ABI31_0_0RCTEffectiveBackgroundColorFromTextAttributes(textAttributes);
  }
  ...
  if (textAttributes.alignment.hasValue()) {
    ...
    paragraphStyle.alignment =
      ABI31_0_0RCTNSTextAlignmentFromTextAlignment(textAlignment);
    isParagraphStyleUsed = YES;
  }

  if (textAttributes.baseWritingDirection.hasValue()) {
    paragraphStyle.baseWritingDirection =
      ABI31_0_0RCTNSWritingDirectionFromWritingDirection(textAttributes.baseWritingDirection.value());
    isParagraphStyleUsed = YES;
  }

  if (!isnan(textAttributes.lineHeight)) {
    CGFloat lineHeight =
      textAttributes.lineHeight * ABI31_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(textAttributes);
  ...
  if (textAttributes.textDecorationLineType.value_or(TextDecorationLineType::None) != TextDecorationLineType::None) {
    ...
    NSUnderlineStyle style =
      ABI31_0_0RCTNSUnderlineStyleFromStyleAndPattern(
    ...
    UIColor *textDecorationColor = ABI31_0_0RCTUIColorFromSharedColor(textAttributes.textDecorationColor);
  ...
  if (textAttributes.textShadowOffset.hasValue()) {
    ...
    shadow.shadowColor = ABI31_0_0RCTUIColorFromSharedColor(textAttributes.textShadowColor);
    attributes[NSShadowAttributeName] = shadow;
  }

  // Special
  if (textAttributes.isHighlighted) {
    attributes[ABI31_0_0RCTAttributedStringIsHighlightedAttributeName] = @YES;
  }

  return [attributes copy];
}

NSAttributedString *ABI31_0_0RCTNSAttributedStringFromAttributedString(const AttributedString &attributedString) {
  ...
  for (auto fragment : attributedString.getFragments()) {
    ...
    if (layoutableShadowNode) {
      ...
      nsAttributedStringFragment =
        [[NSAttributedString alloc] initWithString:string
                                        attributes:ABI31_0_0RCTNSTextAttributesFromTextAttributes(fragment.textAttributes)];
    }
    ...
    if (fragment.shadowNode) {
      NSDictionary<NSAttributedStringKey, id> *additionalTextAttributes = @{
        ABI31_0_0RCTAttributedStringReactABI31_0_0TagAttributeName: @(fragment.shadowNode->getTag())
      };
