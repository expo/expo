...
ABI31_0_0RCT_EXPORT_METHOD(removeProxiedListeners:(NSString *)moduleName count:(double)count)
{
  ...
  if (newModuleListenersCount == 0) {
    ...
    ABI31_0_0EXLogError(@"Attempted to remove more `%@` listeners than added", moduleName);
  ...
  if (_listenersCount - 1 < 0) {
    ABI31_0_0EXLogError(@"Attempted to remove more proxied event emitter listeners than added");
    _listenersCount = 0;
  } else {
    _listenersCount -= 1;
  }
