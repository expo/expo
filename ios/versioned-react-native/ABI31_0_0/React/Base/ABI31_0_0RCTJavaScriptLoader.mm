...
#import "ABI31_0_0RCTJavaScriptLoader.h"
...
#import "ABI31_0_0RCTMultipartDataTask.h"
#import "ABI31_0_0RCTPerformanceLogger.h"
#import "ABI31_0_0RCTUtils.h"

NSString *const ABI31_0_0RCTJavaScriptLoaderErrorDomain = @"ABI31_0_0RCTJavaScriptLoaderErrorDomain";

@interface ABI31_0_0RCTSource()
{
@public
  NSURL *_url;
  NSData *_data;
...
@implementation ABI31_0_0RCTSource

static ABI31_0_0RCTSource *ABI31_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  ABI31_0_0RCTSource *source = [ABI31_0_0RCTSource new];
  source->_url = url;
  source->_data = data;
  source->_length = length;
  source->_filesChangedCount = ABI31_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI31_0_0RCTLoadingProgress
...
@implementation ABI31_0_0RCTJavaScriptLoader
  ...
  NSData *data = [self attemptSynchronousLoadOfBundleAtURL:scriptURL
                                          runtimeBCVersion:JSNoBytecodeFileFormatVersion
                                              sourceLength:&sourceLength
                                                     error:&error];
  if (data) {
    onComplete(nil, ABI31_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError =
  [error.domain isEqualToString:ABI31_0_0RCTJavaScriptLoaderErrorDomain]
  && error.code == ABI31_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI31_0_0RCTJavaScriptLoaderErrorNoScriptURL
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI31_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI31_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  facebook::ReactABI31_0_0::BundleHeader header;
  size_t readResult = fread(&header, sizeof(header), 1, bundle);
  fclose(bundle);
  if (readResult != 1) {
    if (error) {
      *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI31_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  facebook::ReactABI31_0_0::ScriptTag tag = facebook::ReactABI31_0_0::parseTypeFromHeader(header);
  switch (tag) {
  case facebook::ReactABI31_0_0::ScriptTag::RAMBundle:
    break;

  case facebook::ReactABI31_0_0::ScriptTag::String: {
  ...
  case facebook::ReactABI31_0_0::ScriptTag::BCBundle:
    if (runtimeBCVersion == JSNoBytecodeFileFormatVersion || runtimeBCVersion < 0) {
      if (error) {
        *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI31_0_0RCTJavaScriptLoaderErrorBCNotSupported
                                 userInfo:@{NSLocalizedDescriptionKey:
      ...
      if (error) {
        ...
        *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI31_0_0RCTJavaScriptLoaderErrorBCVersion
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI31_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                               userInfo:@{NSLocalizedDescriptionKey:
...
static void parseHeaders(NSDictionary *headers, ABI31_0_0RCTSource *source) {
  source->_filesChangedCount = [headers[@"X-Metro-Files-Changed-Count"] integerValue];
}

static void attemptAsynchronousLoadOfBundleAtURL(NSURL *scriptURL, ABI31_0_0RCTSourceLoadProgressBlock onProgress, ABI31_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path
                                              options:NSDataReadingMappedIfSafe
                                                error:&error];
      onComplete(error, ABI31_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI31_0_0RCTMultipartDataTask *task = [[ABI31_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
    if (!done) {
      if (onProgress) {
    ...
    if (error) {
      if ([error.domain isEqualToString:NSURLErrorDomain]) {
        error = [NSError errorWithDomain:ABI31_0_0RCTJavaScriptLoaderErrorDomain
                                    code:ABI31_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                                userInfo:
                 @{
                   NSLocalizedDescriptionKey:
                     [@"Could not connect to development server.\n\n"
                      "Ensure the following:\n"
                      "- Node server is running and available on the same network - run 'npm start' from ReactABI31_0_0-native root\n"
    ...
    ABI31_0_0RCTSource *source = ABI31_0_0RCTSourceCreate(scriptURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI31_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI31_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI31_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI31_0_0RCTLoadingProgress *progress = [ABI31_0_0RCTLoadingProgress new];
...
static ABI31_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI31_0_0RCTLoadingProgress *progress = [ABI31_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI31_0_0RCTJSONParse(rawText, nil);
