...
*/
static NSRegularExpression *ABI31_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI31_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI31_0_0RCTNullIfNil(self.methodName),
     @"file": ABI31_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI31_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
{
  return [[self alloc] initWithMethodName:ABI31_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI31_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI31_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}

+ (NSArray<ABI31_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI31_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI31_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI31_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
