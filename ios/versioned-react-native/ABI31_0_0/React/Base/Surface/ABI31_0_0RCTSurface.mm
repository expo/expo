...
#import "ABI31_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI31_0_0RCTSurface () <ABI31_0_0RCTSurfaceRootShadowViewDelegate, ABI31_0_0RCTUIManagerObserver>
@end

@implementation ABI31_0_0RCTSurface {
  // Immutable
  ...
  ABI31_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI31_0_0RCTSurfaceView *_Nullable _view;
  ABI31_0_0RCTTouchHandler *_Nullable _touchHandler;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI31_0_0RCTAllocateRootViewTag();
    ...
    if (!bridge.loading) {
      _stage = _stage | ABI31_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI31_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI31_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI31_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI31_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI31_0_0RCTSurfaceRootView *rootView =
    (ABI31_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI31_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI31_0_0RCTAssert([rootView isKindOfClass:[ABI31_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI31_0_0RCTSurfaceRootView`.");
...
- (void)_run
{
  ABI31_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI31_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI31_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];
...
- (void)_stop
{
  [self unmountReactABI31_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI31_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI31_0_0RCTIsMainQueue() ? ABI31_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI31_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI31_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI31_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI31_0_0Tag:self->_rootViewTag];
    ABI31_0_0RCTAssert([rootShadowView isKindOfClass:[ABI31_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI31_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI31_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI31_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI31_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI31_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI31_0_0Tag:self->_rootViewTag];
...
{
  if (ABI31_0_0RCTIsUIManagerQueue()) {
    ABI31_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI31_0_0RCTIsMainQueue() && (stage == ABI31_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI31_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI31_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI31_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI31_0_0RCTAssert(NO, @"Only waiting for `ABI31_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI31_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI31_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI31_0_0RCTIsMainQueue() && (stage == ABI31_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI31_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI31_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI31_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
...
{
  [self _setStage:ABI31_0_0RCTSurfaceStageSurfaceDidInitialRendering];
...
{
  [self _setStage:ABI31_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI31_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
{
  if (self.stage & ABI31_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI31_0_0RCTSurfaceStageSurfaceDidInitialMounting];
