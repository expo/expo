...
@implementation ABI31_0_0RCTJSCSamplingProfiler

@synthesize methodQueue = _methodQueue;
@synthesize bridge = _bridge;

ABI31_0_0RCT_EXPORT_MODULE(ABI31_0_0RCTJSCSamplingProfiler);

#ifdef ABI31_0_0RCT_PROFILE
ABI31_0_0RCT_EXPORT_METHOD(operationComplete:(__unused int)token result:(id)profileData error:(id)error)
{
  if (error) {
    ABI31_0_0RCTLogError(@"JSC Sampling profiler ended with error: %@", error);
    return;
  }
  ...
  NSURLSessionDataTask *sessionDataTask = [session dataTaskWithRequest:request completionHandler:^(__unused NSData *data, __unused NSURLResponse *response, NSError *sessionError) {
    if (sessionError) {
      ABI31_0_0RCTLogWarn(@"JS CPU Profile data failed to send. Is the packager server running locally?\nDetails: %@", error);
    } else {
      ABI31_0_0RCTLogInfo(@"JS CPU Profile data sent successfully.");
    }
  }];
