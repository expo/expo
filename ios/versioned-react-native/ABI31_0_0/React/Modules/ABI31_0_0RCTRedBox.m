...
#import "ABI31_0_0RCTRedBoxExtraDataViewController.h"
#import "ABI31_0_0RCTUtils.h"

#if ABI31_0_0RCT_DEV

@class ABI31_0_0RCTRedBoxWindow;

@protocol ABI31_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI31_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI31_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI31_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI31_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI31_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI31_0_0RCTRedBoxWindow
{
    UITableView *_stackTraceTableView;
    NSString *_lastErrorMessage;
    NSArray<ABI31_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI31_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
    ...
    [ABI31_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
    ...
    for (ABI31_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI31_0_0RCTJSStackFrame *)stackFrame
{
    NSString *fileName = ABI31_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ABI31_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI31_0_0RCTJSStackFrame *)stackFrame
{
    if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 1) {
        ...
        ABI31_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
    // NOTE: We could use ABI31_0_0RCTKeyCommands for this, but since
...
@interface ABI31_0_0RCTRedBox () <ABI31_0_0RCTInvalidating, ABI31_0_0RCTRedBoxWindowActionDelegate, ABI31_0_0RCTRedBoxExtraDataActionDelegate>
@end

@implementation ABI31_0_0RCTRedBox
{
    ABI31_0_0RCTRedBoxWindow *_window;
    NSMutableArray<id<ABI31_0_0RCTErrorCustomizer>> *_errorCustomizers;
    ABI31_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
}
...
- (void)registerErrorCustomizer:(id<ABI31_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI31_0_0RCTErrorInfo *)_customizeError:(ABI31_0_0RCTErrorInfo *)error
{
    ABI31_0_0RCTAssertMainQueue();
    if (!self->_errorCustomizers) {
        return error;
    }
    for (id<ABI31_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
        ABI31_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
        if (newInfo) {
            error = newInfo;
        }
    }
...
- (void)showError:(NSError *)error
{
    [self showErrorMessage:error.localizedDescription
               withDetails:error.localizedFailureReason
                     stack:error.userInfo[ABI31_0_0RCTJSStackTraceKey]];
}
...
- (void)showErrorMessage:(NSString *)message withDetails:(NSString *)details stack:(NSArray<ABI31_0_0RCTJSStackFrame *> *)stack {
...
{
    NSArray<ABI31_0_0RCTJSStackFrame *> *stack = [ABI31_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
    [self showErrorMessage:message withParsedStack:stack isUpdate:NO];
}

- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
    [self showErrorMessage:message withParsedStack:[ABI31_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:NO];
}

- (void)updateErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
    [self showErrorMessage:message withParsedStack:[ABI31_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:YES];
}

- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI31_0_0RCTJSStackFrame *> *)stack
{
    [self showErrorMessage:message withParsedStack:stack isUpdate:NO];
}

- (void)updateErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI31_0_0RCTJSStackFrame *> *)stack
{
    [self showErrorMessage:message withParsedStack:stack isUpdate:YES];
}

- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI31_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_extraDataViewController == nil) {
            self->_extraDataViewController = [ABI31_0_0RCTRedBoxExtraDataViewController new];
        ...
        if (!self->_window) {
            self->_window = [[ABI31_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
            self->_window.actionDelegate = self;
        }

        ABI31_0_0RCTErrorInfo *errorInfo = [[ABI31_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                       stack:stack];
...
ABI31_0_0RCT_EXPORT_METHOD(setExtraData:(NSDictionary *)extraData forIdentifier:(NSString *)identifier) {
    [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI31_0_0RCT_EXPORT_METHOD(dismiss)
{
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI31_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI31_0_0RCTJSStackFrame *)stackFrame
{
    NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
    if (![bundleURL.scheme hasPrefix:@"http"]) {
        ABI31_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
        return;
    }

    NSData *stackFrameJSON = [ABI31_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI31_0_0RCTRedBoxWindow *)redBoxWindow
{
...
@implementation ABI31_0_0RCTBridge (ABI31_0_0RCTRedBox)

- (ABI31_0_0RCTRedBox *)redBox
{
    return [self moduleForClass:[ABI31_0_0RCTRedBox class]];
}
...
- (ABI31_0_0RCTRedBox *)redBox { return nil; }
