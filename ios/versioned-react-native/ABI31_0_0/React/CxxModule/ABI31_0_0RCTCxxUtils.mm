...
#import <cxxReactABI31_0_0/ABI31_0_0CxxNativeModule.h>
#import <ABI31_0_0jschelpers/ABI31_0_0Value.h>

#import "ABI31_0_0DispatchMessageQueueThread.h"
#import "ABI31_0_0RCTCxxModule.h"
#import "ABI31_0_0RCTNativeModule.h"
...
std::vector<std::unique_ptr<NativeModule>> createNativeModules(NSArray<ABI31_0_0RCTModuleData *> *modules, ABI31_0_0RCTBridge *bridge, const std::shared_ptr<Instance> &instance)
{
  ...
  for (ABI31_0_0RCTModuleData *moduleData in modules) {
    if ([moduleData.moduleClass isSubclassOfClass:[ABI31_0_0RCTCxxModule class]]) {
      nativeModules.emplace_back(std::make_unique<CxxNativeModule>(
        instance,
        [moduleData.name UTF8String],
        [moduleData] { return [(ABI31_0_0RCTCxxModule *)(moduleData.instance) createModule]; },
        std::make_shared<DispatchMessageQueueThread>(moduleData)));
    } else {
      nativeModules.emplace_back(std::make_unique<ABI31_0_0RCTNativeModule>(bridge, moduleData));
    }
  }
...
static NSError *errorWithException(const std::exception &e)
{
  ...
  if (jsException) {
    errorInfo[ABI31_0_0RCTJSRawStackTraceKey] = @(jsException->getStack().c_str());
  ...
  return [NSError errorWithDomain:ABI31_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}
...
NSError *tryAndReturnError(const std::function<void()>& func)
{
  try {
    ...
    @catch (id exception) {
      ...
      return ABI31_0_0RCTErrorWithMessage(@"non-std ObjC Exception");
    }
    ...
    return ABI31_0_0RCTErrorWithMessage(@"non-std C++ exception");
  }
}
