...
#import <cxxReactABI31_0_0/ABI31_0_0JsArgumentHelpers.h>
...
@implementation ABI31_0_0RCTCxxMethod
{
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI31_0_0RCTLogError(@"Method %@.%s expects at least %zu arguments, but got %tu",
                ABI31_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI31_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
  ...
  try {
    ...
    ABI31_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI31_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
