...
#import "ABI31_0_0RCTSurfaceTouchHandler.h"
...
@protocol ABI31_0_0RCTTouchableComponentViewProtocol <NSObject>
  - (SharedViewEventEmitter)touchEventEmitter;
@end

typedef NS_ENUM(NSInteger, ABI31_0_0RCTTouchEventType) {
  ABI31_0_0RCTTouchEventTypeTouchStart,
  ABI31_0_0RCTTouchEventTypeTouchMove,
  ABI31_0_0RCTTouchEventTypeTouchEnd,
  ABI31_0_0RCTTouchEventTypeTouchCancel,
};
...
static void UpdateActiveTouchWithUITouch(ActiveTouch &activeTouch, UITouch *uiTouch, UIView *rootComponentView) {
  ...
  activeTouch.touch.offsetPoint = ABI31_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI31_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI31_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI31_0_0RCTForceTouchAvailable()) {
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView) {
  ...
  if ([componentView respondsToSelector:@selector(touchEventEmitter)]) {
    activeTouch.eventEmitter = [(id<ABI31_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitter];
...
@interface ABI31_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI31_0_0RCTSurfaceTouchHandler {
  std::unordered_map<
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ReactABI31_0_0 Native,
...
ABI31_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithTarget:(id)target action:(SEL)action)

- (void)attachToView:(UIView *)view
{
  ABI31_0_0RCTAssert(self.view == nil, @"ABI31_0_0RCTTouchHandler already has attached view.");
...
{
  ABI31_0_0RCTAssertParam(view);
  ABI31_0_0RCTAssert(self.view == view, @"ABI31_0_0RCTTouchHandler attached to another view.");
...
- (void)_dispatchTouches:(NSSet<UITouch *> *)touches eventType:(ABI31_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI31_0_0RCTTouchEventTypeTouchEnd || eventType == ABI31_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitter) {
    ...
    switch (eventType) {
      case ABI31_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI31_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI31_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI31_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchTouches:touches eventType:ABI31_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchTouches:touches eventType:ABI31_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchTouches:touches eventType:ABI31_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchTouches:touches eventType:ABI31_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
