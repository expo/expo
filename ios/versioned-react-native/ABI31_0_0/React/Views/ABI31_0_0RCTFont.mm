...
typedef CGFloat ABI31_0_0RCTFontWeight;
static ABI31_0_0RCTFontWeight weightOfFont(UIFont *font)
{
  ...
  for (NSInteger i = 0; i < fontNames.count; i++) {
    if ([font.fontName.lowercaseString hasSuffix:fontNames[i]]) {
      return (ABI31_0_0RCTFontWeight)[fontWeights[i] doubleValue];
    }
  }

  NSDictionary *traits = [font.fontDescriptor objectForKey:UIFontDescriptorTraitsAttribute];
  return (ABI31_0_0RCTFontWeight)[traits[UIFontWeightTrait] doubleValue];
}
...
static ABI31_0_0RCTFontHandler defaultFontHandler;

void ABI31_0_0RCTSetDefaultFontHandler(ABI31_0_0RCTFontHandler handler) {
  defaultFontHandler = handler;
}

BOOL ABI31_0_0RCTHasFontHandlerSet() {
...
static NSString *FontWeightDescriptionFromUIFontWeight(UIFontWeight fontWeight)
{
  ...
  ABI31_0_0RCTAssert(NO, @"Unknown UIFontWeight passed in: %f", fontWeight);
  return @"regular";
}

static UIFont *cachedSystemFont(CGFloat size, ABI31_0_0RCTFontWeight weight)
{
...
@implementation ABI31_0_0RCTConvert (ABI31_0_0RCTFont)

+ (UIFont *)UIFont:(id)json
{
  json = [self NSDictionary:json];
  return [ABI31_0_0RCTFont updateFont:nil
                  withFamily:[ABI31_0_0RCTConvert NSString:json[@"fontFamily"]]
                        size:[ABI31_0_0RCTConvert NSNumber:json[@"fontSize"]]
                      weight:[ABI31_0_0RCTConvert NSString:json[@"fontWeight"]]
                       style:[ABI31_0_0RCTConvert NSString:json[@"fontStyle"]]
                     variant:[ABI31_0_0RCTConvert NSStringArray:json[@"fontVariant"]]
             scaleMultiplier:1];
}

ABI31_0_0RCT_ENUM_CONVERTER(ABI31_0_0RCTFontWeight, (@{
...
typedef BOOL ABI31_0_0RCTFontStyle;
ABI31_0_0RCT_ENUM_CONVERTER(ABI31_0_0RCTFontStyle, (@{
                                    @"normal": @NO,
                                    @"italic": @YES,
                                    @"oblique": @YES,
                                    }), NO, boolValue)

typedef NSDictionary ABI31_0_0RCTFontVariantDescriptor;
+ (ABI31_0_0RCTFontVariantDescriptor *)ABI31_0_0RCTFontVariantDescriptor:(id)json
{
  ...
  ABI31_0_0RCTFontVariantDescriptor *value = mapping[json];
  if (ABI31_0_0RCT_DEBUG && !value && [json description].length > 0) {
    ABI31_0_0RCTLogError(@"Invalid ABI31_0_0RCTFontVariantDescriptor '%@'. should be one of: %@", json,
                [[mapping allKeys] sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)]);
  }
  return value;
}

ABI31_0_0RCT_ARRAY_CONVERTER(ABI31_0_0RCTFontVariantDescriptor)

@end

@implementation ABI31_0_0RCTFont
...
+ (UIFont *)updateFont:(UIFont *)font
            withFamily:(NSString *)family
               ...
               variant:(NSArray<ABI31_0_0RCTFontVariantDescriptor *> *)variant
       scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  const ABI31_0_0RCTFontWeight defaultFontWeight = UIFontWeightRegular;
  const CGFloat defaultFontSize = 14;

  // Initialize properties to defaults
  CGFloat fontSize = defaultFontSize;
  ABI31_0_0RCTFontWeight fontWeight = defaultFontWeight;
  ...
  // Get font attributes
  fontSize = [ABI31_0_0RCTConvert CGFloat:size] ?: fontSize;
  if (scaleMultiplier > 0.0 && scaleMultiplier != 1.0) {
    fontSize = round(fontSize * scaleMultiplier);
  }
  familyName = [ABI31_0_0RCTConvert NSString:family] ?: familyName;
  isItalic = style ? [ABI31_0_0RCTConvert ABI31_0_0RCTFontStyle:style] : isItalic;
  fontWeight = weight ? [ABI31_0_0RCTConvert ABI31_0_0RCTFontWeight:weight] : fontWeight;
  ...
  if (!didFindFont && [UIFont fontNamesForFamilyName:familyName].count == 0) {
    ...
    if (font) {
      ...
      ABI31_0_0RCTLogError(@"Unrecognized font family '%@'", familyName);
  ...
  if (variant) {
    NSArray *fontFeatures = [ABI31_0_0RCTConvert ABI31_0_0RCTFontVariantDescriptorArray:variant];
