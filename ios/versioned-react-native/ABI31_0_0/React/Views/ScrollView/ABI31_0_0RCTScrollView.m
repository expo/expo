...
@interface ABI31_0_0RCTScrollEvent : NSObject <ABI31_0_0RCTEvent>
...
@implementation ABI31_0_0RCTScrollEvent
{
...
{
  ABI31_0_0RCTAssertParam(ReactABI31_0_0Tag);
...
- (ABI31_0_0RCTScrollEvent *)coalesceWithEvent:(ABI31_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI31_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI31_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI31_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI31_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
...
@implementation ABI31_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI31_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI31_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI31_0_0RCTUIManager JSResponder] isKindOfClass:[ABI31_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI31_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI31_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI31_0_0RCTScrollView () <ABI31_0_0RCTUIManagerObserver>

@end

@implementation ABI31_0_0RCTScrollView
{
  ABI31_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI31_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI31_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI31_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI31_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI31_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
- (void)setReactABI31_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  [super setReactABI31_0_0LayoutDirection:layoutDirection];

  ABI31_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI31_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertReactABI31_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertReactABI31_0_0Subview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI31_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI31_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI31_0_0RCTAssert(_contentView == nil, @"ABI31_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI31_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ReactABI31_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI31_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI31_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI31_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI31_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI31_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI31_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI31_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI31_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI31_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI31_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI31_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI31_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI31_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI31_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI31_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI31_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI31_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI31_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI31_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI31_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI31_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI31_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI31_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI31_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI31_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI31_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(ABI31_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(ABI31_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI31_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI31_0_0RCTScrollEvent *scrollEvent = [[ABI31_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ReactABI31_0_0Tag:self.ReactABI31_0_0Tag
...
@implementation ABI31_0_0RCTEventDispatcher (ABI31_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI31_0_0Tag
{
  ...
  ABI31_0_0RCTScrollEvent *fakeScrollEvent = [[ABI31_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI31_0_0Tag:ReactABI31_0_0Tag
