...
@implementation ABI31_0_0RCTModalHostView
{
  __weak ABI31_0_0RCTBridge *_bridge;
  BOOL _isPresented;
  ABI31_0_0RCTModalHostViewController *_modalViewController;
  ABI31_0_0RCTTouchHandler *_touchHandler;
  UIView *_ReactABI31_0_0Subview;
#if TARGET_OS_TV
...
ABI31_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:coder)

- (instancetype)initWithBridge:(ABI31_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _modalViewController = [ABI31_0_0RCTModalHostViewController new];
    UIView *containerView = [UIView new];
    containerView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _modalViewController.view = containerView;
    _touchHandler = [[ABI31_0_0RCTTouchHandler alloc] initWithBridge:bridge];
#if TARGET_OS_TV
...
- (void)setOnRequestClose:(ABI31_0_0RCTDirectEventBlock)onRequestClose
{
  _onRequestClose = onRequestClose;
  if (_ReactABI31_0_0Subview) {
    if (_onRequestClose && _menuButtonGestureRecognizer) {
      [_ReactABI31_0_0Subview addGestureRecognizer:_menuButtonGestureRecognizer];
    } else {
      [_ReactABI31_0_0Subview removeGestureRecognizer:_menuButtonGestureRecognizer];
    }
  }
}
#endif
...
{
  if (_ReactABI31_0_0Subview && _isPresented) {
    [_bridge.uiManager setSize:newBounds.size forView:_ReactABI31_0_0Subview];
...
- (void)notifyForOrientationChange
{
  ...
  UIInterfaceOrientation currentOrientation = [ABI31_0_0RCTSharedApplication() statusBarOrientation];
...
{
  ABI31_0_0RCTAssert(_ReactABI31_0_0Subview == nil, @"Modal view can only have one subview");
  ...
  for (NSString *key in [self.tvRemoteHandler.tvRemoteGestureRecognizers allKeys]) {
    if (![key isEqualToString:ABI31_0_0RCTTVRemoteEventMenu]) {
  ...
  _ReactABI31_0_0Subview = subview;
}

- (void)removeReactABI31_0_0Subview:(UIView *)subview
{
  ABI31_0_0RCTAssert(subview == _ReactABI31_0_0Subview, @"Cannot remove view other than modal view");
...
#endif
  _ReactABI31_0_0Subview = nil;
}

- (void)didUpdateReactABI31_0_0Subviews
{
  // Do nothing, as subview (singular) is managed by `insertReactABI31_0_0Subview:atIndex:`
}
...
- (void)didMoveToWindow
{
  ...
  if (!self.userInteractionEnabled && ![self.superview.ReactABI31_0_0Subviews containsObject:self]) {
    return;
  }

  if (!_isPresented && self.window) {
    ABI31_0_0RCTAssert(self.ReactABI31_0_0ViewController, @"Can't present modal view controller without a presenting view controller");
