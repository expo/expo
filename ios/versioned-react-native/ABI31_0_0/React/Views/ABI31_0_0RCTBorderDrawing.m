...
static const CGFloat ABI31_0_0RCTViewBorderThreshold = 0.001;

BOOL ABI31_0_0RCTBorderInsetsAreEqual(UIEdgeInsets borderInsets)
{
  return
  ABS(borderInsets.left - borderInsets.right) < ABI31_0_0RCTViewBorderThreshold &&
  ABS(borderInsets.left - borderInsets.bottom) < ABI31_0_0RCTViewBorderThreshold &&
  ABS(borderInsets.left - borderInsets.top) < ABI31_0_0RCTViewBorderThreshold;
}

BOOL ABI31_0_0RCTCornerRadiiAreEqual(ABI31_0_0RCTCornerRadii cornerRadii)
{
  return
  ABS(cornerRadii.topLeft - cornerRadii.topRight) < ABI31_0_0RCTViewBorderThreshold &&
  ABS(cornerRadii.topLeft - cornerRadii.bottomLeft) < ABI31_0_0RCTViewBorderThreshold &&
  ABS(cornerRadii.topLeft - cornerRadii.bottomRight) < ABI31_0_0RCTViewBorderThreshold;
}
...
{
  return (ABI31_0_0RCTCornerInsets) {
    {
...
static UIEdgeInsets ABI31_0_0RCTRoundInsetsToPixel(UIEdgeInsets edgeInsets) {
    edgeInsets.top = ABI31_0_0RCTRoundPixelValue(edgeInsets.top);
    edgeInsets.bottom = ABI31_0_0RCTRoundPixelValue(edgeInsets.bottom);
    edgeInsets.left = ABI31_0_0RCTRoundPixelValue(edgeInsets.left);
    edgeInsets.right = ABI31_0_0RCTRoundPixelValue(edgeInsets.right);

    return edgeInsets;
}

static void ABI31_0_0RCTPathAddEllipticArc(CGMutablePathRef path,
                                       ...
                                       const CGAffineTransform *transform)
{
  ...
  ABI31_0_0RCTPathAddEllipticArc(path, transform, (CGPoint){
...
static void ABI31_0_0RCTEllipseGetIntersectionsWithLine(CGRect ellipseBounds,
                                               CGPoint lineStart,
                                               CGPoint lineEnd,
...
NS_INLINE BOOL ABI31_0_0RCTCornerRadiiAreAboveThreshold(ABI31_0_0RCTCornerRadii cornerRadii) {
  return (cornerRadii.topLeft > ABI31_0_0RCTViewBorderThreshold ||
    cornerRadii.topRight > ABI31_0_0RCTViewBorderThreshold      ||
    cornerRadii.bottomLeft > ABI31_0_0RCTViewBorderThreshold    ||
    cornerRadii.bottomRight > ABI31_0_0RCTViewBorderThreshold);
}

static CGPathRef ABI31_0_0RCTPathCreateOuterOutline(BOOL drawToEdge, CGRect rect, ABI31_0_0RCTCornerRadii cornerRadii) {
  if (drawToEdge) {
    return CGPathCreateWithRect(rect, NULL);
  }

  return ABI31_0_0RCTPathCreateWithRoundedRect(rect, ABI31_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
}

static CGContextRef ABI31_0_0RCTUIGraphicsBeginImageContext(CGSize size, CGColorRef backgroundColor, BOOL hasCornerRadii, BOOL drawToEdge) {
...
static UIImage *ABI31_0_0RCTGetSolidBorderImage(ABI31_0_0RCTCornerRadii cornerRadii,
                                       CGSize viewSize,
                                       ...
                                       BOOL drawToEdge)
{
  const BOOL hasCornerRadii = ABI31_0_0RCTCornerRadiiAreAboveThreshold(cornerRadii);
  const ABI31_0_0RCTCornerInsets cornerInsets = ABI31_0_0RCTGetCornerInsets(cornerRadii, borderInsets);
  ...
  borderInsets = ABI31_0_0RCTRoundInsetsToPixel(borderInsets);
  ...
  CGContextRef ctx = ABI31_0_0RCTUIGraphicsBeginImageContext(size, backgroundColor, hasCornerRadii, drawToEdge);
  const CGRect rect = {.size = size};
  CGPathRef path = ABI31_0_0RCTPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
  ...
  CGPathRef insetPath = ABI31_0_0RCTPathCreateWithRoundedRect(UIEdgeInsetsInsetRect(rect, borderInsets), cornerInsets, NULL);

  CGContextAddPath(ctx, insetPath);
  CGContextEOClip(ctx);

  BOOL hasEqualColors = ABI31_0_0RCTBorderColorsAreEqual(borderColors);
  if ((drawToEdge || !hasCornerRadii) && hasEqualColors) {
    ...
    if (cornerInsets.topLeft.width > 0 && cornerInsets.topLeft.height > 0) {
      CGPoint points[2];
      ABI31_0_0RCTEllipseGetIntersectionsWithLine((CGRect){
...
static UIImage *ABI31_0_0RCTGetDashedOrDottedBorderImage(ABI31_0_0RCTBorderStyle borderStyle,
                                                ...
                                                BOOL drawToEdge)
{
  NSCParameterAssert(borderStyle == ABI31_0_0RCTBorderStyleDashed || borderStyle == ABI31_0_0RCTBorderStyleDotted);

  if (!ABI31_0_0RCTBorderColorsAreEqual(borderColors) || !ABI31_0_0RCTBorderInsetsAreEqual(borderInsets)) {
    ABI31_0_0RCTLogWarn(@"Unsupported dashed / dotted border style");
    return nil;
  }
  ...
  const BOOL hasCornerRadii = ABI31_0_0RCTCornerRadiiAreAboveThreshold(cornerRadii);
  CGContextRef ctx = ABI31_0_0RCTUIGraphicsBeginImageContext(viewSize, backgroundColor, hasCornerRadii, drawToEdge);
  const CGRect rect = {.size = viewSize};

  if (backgroundColor) {
    CGPathRef outerPath = ABI31_0_0RCTPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
  ...
  CGPathRef path = ABI31_0_0RCTPathCreateWithRoundedRect(pathRect, ABI31_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);

  CGFloat dashLengths[2];
  dashLengths[0] = dashLengths[1] = (borderStyle == ABI31_0_0RCTBorderStyleDashed ? 3 : 1) * lineWidth;
                           ...
                           BOOL drawToEdge)
{
  ...
  switch (borderStyle) {
    case ABI31_0_0RCTBorderStyleSolid:
      return ABI31_0_0RCTGetSolidBorderImage(cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
    case ABI31_0_0RCTBorderStyleDashed:
    case ABI31_0_0RCTBorderStyleDotted:
      return ABI31_0_0RCTGetDashedOrDottedBorderImage(borderStyle, cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
    case ABI31_0_0RCTBorderStyleUnset:
      break;
  }
