...
@interface ABI31_0_0EXModuleRegistry ()

@property (nonatomic, weak) id<ABI31_0_0EXModuleRegistryDelegate> delegate;

@property NSMutableSet<id<ABI31_0_0EXInternalModule>> *internalModulesSet;
@property NSMapTable<Protocol *, id<ABI31_0_0EXInternalModule>> *internalModules;
@property NSMapTable<Protocol *, NSMutableArray<id<ABI31_0_0EXInternalModule>> *> *internalModulesPreResolution;
@property NSMutableDictionary<Class, ABI31_0_0EXExportedModule *> *exportedModulesByClass;
@property NSMutableDictionary<const NSString *, ABI31_0_0EXExportedModule *> *exportedModules;
@property NSMutableDictionary<const NSString *, ABI31_0_0EXViewManager *> *viewManagerModules;
@property NSMutableDictionary<const NSString *, id> *singletonModules;

@property NSMutableSet<id<ABI31_0_0EXModuleRegistryConsumer>> *registryConsumers;

@end

@implementation ABI31_0_0EXModuleRegistry
...
{
  if (self = [self init]) {
    for (id<ABI31_0_0EXInternalModule> internalModule in internalModules) {
      [self registerInternalModule:internalModule];
    }
    
    for (ABI31_0_0EXExportedModule *exportedModule in exportedModules) {
      [self registerExportedModule:exportedModule];
    }

    for (ABI31_0_0EXViewManager *viewManager in viewManagers) {
...
- (void)resolveInternalModulesConflicts
{
  ...
  for (Protocol *protocol in _internalModulesPreResolution) {
    NSArray<id<ABI31_0_0EXInternalModule>> *conflictingModules = [_internalModulesPreResolution objectForKey:protocol];

    if ([conflictingModules count] > 1 && _delegate) {
      id<ABI31_0_0EXInternalModule> resolvedModule = [_delegate pickInternalModuleImplementingInterface:protocol fromAmongModules:conflictingModules];
...
- (void)initialize
{
  ...
  for (id<ABI31_0_0EXModuleRegistryConsumer> registryConsumer in _registryConsumers) {
...
{
  id<ABI31_0_0EXInternalModule> module = [_internalModules objectForKey:protocol];
...
- (void)registerExportedModule:(ABI31_0_0EXExportedModule *)exportedModule
{
  ...
  if (_exportedModules[exportedModuleName]) {
    ABI31_0_0EXLogWarn(@"Expo module %@ overrides %@ as the module exported as %@.", exportedModule, _exportedModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerViewManager:(ABI31_0_0EXViewManager *)viewManager
{
  ...
  if (_viewManagerModules[exportedModuleName]) {
    ABI31_0_0EXLogWarn(@"Expo view manager %@ overrides %@ as the module exported as %@.", viewManager, _viewManagerModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerSingletonModule:(id)singletonModule
{
  if ([[singletonModule class] respondsToSelector:@selector(name)]) {
    ...
    ABI31_0_0EXLogWarn(@"One of the singleton modules does not respond to +(NSString *)name selector. This probably means you're either try to pass a strange object as a singleton module (it won't get registered in the module registry, sorry) or the ABI31_0_0EXSingletonModule interface and the ABI31_0_0EXModuleRegistry implementations versions are out of sync, which means things will probably not work as expected.");
  }
}

- (void)maybeAddRegistryConsumer:(id)maybeConsumer
{
  if ([maybeConsumer conformsToProtocol:@protocol(ABI31_0_0EXModuleRegistryConsumer)]) {
    [self addRegistryConsumer:(id<ABI31_0_0EXModuleRegistryConsumer>)maybeConsumer];
  }
}

- (void)addRegistryConsumer:(id<ABI31_0_0EXModuleRegistryConsumer>)registryConsumer
{
