...
@implementation ABI31_0_0RCTBaseTextInputShadowView
{
...
- (instancetype)initWithBridge:(ABI31_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI31_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI31_0_0RCTBaseTextInputShadowViewMeasure);
    ABI31_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI31_0_0RCTTextInputShadowViewBaseline);
  }
...
- (void)uiManagerWillPerformMounting
{
  ...
  ABI31_0_0RCTTextAttributes *textAttributes = [self.textAttributes copy];
  ...
  [attributedText removeAttribute:ABI31_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                            range:NSMakeRange(0, attributedText.length)];

  [attributedText removeAttribute:ABI31_0_0RCTTextAttributesTagAttributeName
                            range:NSMakeRange(0, attributedText.length)];
  ...
  [_bridge.uiManager addUIBlock:^(ABI31_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI31_0_0RCTBaseTextInputView *baseTextInputView = (ABI31_0_0RCTBaseTextInputView *)viewRegistry[tag];
    if (!baseTextInputView) {
      return;
    }

    baseTextInputView.textAttributes = textAttributes;
    baseTextInputView.ReactABI31_0_0BorderInsets = borderInsets;
    baseTextInputView.ReactABI31_0_0PaddingInsets = paddingInsets;
...
- (CGSize)sizeThatFitsMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  return (CGSize){
    MAX(minimumSize.width, MIN(ABI31_0_0RCTCeilPixelValue(size.width), maximumSize.width)),
    MAX(minimumSize.height, MIN(ABI31_0_0RCTCeilPixelValue(size.height), maximumSize.height))
  };
}
...
static ABI31_0_0YGSize ABI31_0_0RCTBaseTextInputShadowViewMeasure(ABI31_0_0YGNodeRef node, float width, ABI31_0_0YGMeasureMode widthMode, float height, ABI31_0_0YGMeasureMode heightMode)
{
  ...
  return (ABI31_0_0YGSize){
    ABI31_0_0RCTYogaFloatFromCoreGraphicsFloat(measuredSize.width),
    ABI31_0_0RCTYogaFloatFromCoreGraphicsFloat(measuredSize.height)
  };
}

static float ABI31_0_0RCTTextInputShadowViewBaseline(ABI31_0_0YGNodeRef node, const float width, const float height)
{
  ABI31_0_0RCTBaseTextInputShadowView *shadowTextView = (__bridge ABI31_0_0RCTBaseTextInputShadowView *)ABI31_0_0YGNodeGetContext(node);
