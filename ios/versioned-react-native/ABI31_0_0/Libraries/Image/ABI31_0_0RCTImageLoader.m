...
@implementation UIImage (ReactABI31_0_0)

- (CAKeyframeAnimation *)ReactABI31_0_0KeyframeAnimation
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setReactABI31_0_0KeyframeAnimation:(CAKeyframeAnimation *)ReactABI31_0_0KeyframeAnimation
{
    objc_setAssociatedObject(self, @selector(ReactABI31_0_0KeyframeAnimation), ReactABI31_0_0KeyframeAnimation, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

@end

@implementation ABI31_0_0RCTImageLoader
{
    NSArray<id<ABI31_0_0RCTImageURLLoader>> *_loaders;
    NSArray<id<ABI31_0_0RCTImageDataDecoder>> *_decoders;
    NSOperationQueue *_imageDecodeQueue;
    dispatch_queue_t _URLRequestQueue;
    id<ABI31_0_0RCTImageCache> _imageCache;
    NSMutableArray *_pendingTasks;
    NSInteger _activeTasks;
    NSMutableArray *_pendingDecodes;
    NSInteger _scheduledDecodes;
    NSUInteger _activeBytes;
  __weak id<ABI31_0_0RCTImageRedirectProtocol> _redirectDelegate;
}
...
- (instancetype)initWithRedirectDelegate:(id<ABI31_0_0RCTImageRedirectProtocol>)redirectDelegate
{
    if (self = [super init]) {
...
- (void)setUp
{
    ...
    _URLRequestQueue = dispatch_queue_create("com.facebook.ReactABI31_0_0.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (id<ABI31_0_0RCTImageCache>)imageCache
{
    if (!_imageCache) {
        //set up with default cache
        _imageCache = [ABI31_0_0RCTImageCache new];
    }
    return _imageCache;
}
...
{
    if (_imageCache) {
        ABI31_0_0RCTLogWarn(@"ABI31_0_0RCTImageCache was already set and has now been overriden.");
    }
    _imageCache = cache;
}

- (id<ABI31_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
    ...
    if (!_loaders) {
        ...
        ABI31_0_0RCTAssert(_bridge, @"Bridge not set");
        _loaders = [[_bridge modulesConformingToProtocol:@protocol(ABI31_0_0RCTImageURLLoader)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI31_0_0RCTImageURLLoader> a, id<ABI31_0_0RCTImageURLLoader> b) {
    ...
    if (ABI31_0_0RCT_DEBUG) {
        ...
        id<ABI31_0_0RCTImageURLLoader> previousLoader = nil;
        for (id<ABI31_0_0RCTImageURLLoader> loader in _loaders) {
            ...
            if ([loader canLoadImageURL:URL]) {
                if (previousLoader) {
                    if (priority == previousPriority) {
                        ABI31_0_0RCTLogError(@"The ABI31_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI31_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
    if (!_maxConcurrentLoadingTasks) {
        [self setUp];
    }
    ...
    if (!_decoders) {
        ...
        ABI31_0_0RCTAssert(_bridge, @"Bridge not set");
        _decoders = [[_bridge modulesConformingToProtocol:@protocol(ABI31_0_0RCTImageDataDecoder)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI31_0_0RCTImageDataDecoder> a, id<ABI31_0_0RCTImageDataDecoder> b) {
    ...
    if (ABI31_0_0RCT_DEBUG) {
        // Check for handler conflicts
        float previousPriority = 0;
        id<ABI31_0_0RCTImageDataDecoder> previousDecoder = nil;
        for (id<ABI31_0_0RCTImageDataDecoder> decoder in _decoders) {
            ...
            if ([decoder canDecodeImageData:data]) {
                if (previousDecoder) {
                    if (priority == previousPriority) {
                        ABI31_0_0RCTLogError(@"The ABI31_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI31_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI31_0_0RCTResizeMode resizeMode)
{
    ...
    CAKeyframeAnimation *animation = image.ReactABI31_0_0KeyframeAnimation;
    CGRect targetSize = ABI31_0_0RCTTargetRect(image.size, size, scale, resizeMode);
    CGAffineTransform transform = ABI31_0_0RCTTransformFromTargetRect(image.size, targetSize);
    image = ABI31_0_0RCTTransformImage(image, size, scale, transform);
...
- (void)dequeueTasks
{
    dispatch_async(_URLRequestQueue, ^{
        ...
        for (ABI31_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
            switch (task.status) {
                case ABI31_0_0RCTNetworkTaskFinished:
                    if (!tasksToRemove) {
                    ...
                    self->_activeTasks--;
                    break;
                case ABI31_0_0RCTNetworkTaskPending:
                    break;
                case ABI31_0_0RCTNetworkTaskInProgress:
                    // Check task isn't "stuck"
                    if (task.requestToken == nil) {
                        ABI31_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
                        if (!tasksToRemove) {
                            tasksToRemove = [NSMutableArray new];
                        }
                        [tasksToRemove addObject:task];
        ...
        // Start queued tasks
        for (ABI31_0_0RCTNetworkTask *task in self->_pendingTasks) {
            if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
                break;
            }
            if (task.status == ABI31_0_0RCTNetworkTaskPending) {
                [task start];
                self->_activeTasks++;
            }
        }
 ...
 */
- (ABI31_0_0RCTImageLoaderCancellationBlock)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                             size:(CGSize)size
                                                            scale:(CGFloat)scale
                                                       resizeMode:(ABI31_0_0RCTResizeMode)resizeMode
                                                    progressBlock:(ABI31_0_0RCTImageLoaderProgressBlock)progressHandler
                                                 partialLoadBlock:(ABI31_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
                                                  completionBlock:(void (^)(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate))completionBlock
{
    {
        NSMutableURLRequest *mutableRequest = [request mutableCopy];
        [NSURLProtocol setProperty:@"ABI31_0_0RCTImageLoader"
                            forKey:@"trackingName"
    ...
    id<ABI31_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
    ...
    void (^completionHandler)(NSError *, id, NSString *) = ^(NSError *error, id imageOrData, NSString *fetchDate) {
        ...
        if (ABI31_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
    ...
    __weak ABI31_0_0RCTImageLoader *weakSelf = self;
    dispatch_async(_URLRequestQueue, ^{
...
- (ABI31_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI31_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSString *fetchDate))completionHandler
{
    // Check if networking module is available
    if (ABI31_0_0RCT_DEBUG && ![_bridge respondsToSelector:@selector(networking)]) {
        ABI31_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                    " import the ABI31_0_0RCTNetwork library in order to load images.",
                    request.URL.absoluteString);
        return NULL;
    }

    ABI31_0_0RCTNetworking *networking = [_bridge networking];

    // Check if networking module can load image
    if (ABI31_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
        ABI31_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
        return NULL;
    }

    // Use networking module to load image
    ABI31_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
        // Check for system errors
        if (error) {
            completionHandler(error, nil, nil);
            return;
        } else if (!response) {
            completionHandler(ABI31_0_0RCTErrorWithMessage(@"Response metadata error"), nil, nil);
            return;
        } else if (!data) {
            completionHandler(ABI31_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, nil);
            return;
        }
                       ...
                       completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                           ...
                           if (error || !response || !data) {
                               ...
                               if (error) {
                                   ...
                                   someError = ABI31_0_0RCTErrorWithMessage(@"Response metadata error");
                               } else {
                                   someError = ABI31_0_0RCTErrorWithMessage(@"Unknown image download error");
                               }
                                           ...
                                           completionBlock:(ABI31_0_0RCTImageLoaderCompletionBlock)completionBlock
{
    ...
    void (^completionHandler)(NSError *, id, BOOL, NSString *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate) {
        ...
        ABI31_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
...
- (ABI31_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI31_0_0RCTResizeMode)resizeMode
...
{
    if (data.length == 0) {
        completionBlock(ABI31_0_0RCTErrorWithMessage(@"No image data"), nil);
        return ^{};
    }
    ...
    void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
        if (ABI31_0_0RCTIsMainQueue()) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                if (!atomic_load(&cancelled)) {
                    completionBlock(error, clipped ? ABI31_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
                }
            });
        } else if (!atomic_load(&cancelled)) {
            completionBlock(error, clipped ? ABI31_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
    ...
    id<ABI31_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
    if (imageDecoder) {
        ...
        dispatch_block_t decodeBlock = ^{
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                if (!atomic_load(&cancelled)) {
                    ...
                    UIImage *image = ABI31_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI31_0_0RCT_DEV
                    CGSize imagePixelSize = ABI31_0_0RCTSizeInPixels(image.size, image.scale);
                    CGSize screenPixelSize = ABI31_0_0RCTSizeInPixels(ABI31_0_0RCTScreenSize(), ABI31_0_0RCTScreenScale());
                    if (imagePixelSize.width * imagePixelSize.height >
                        screenPixelSize.width * screenPixelSize.height) {
                        ABI31_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
                    ...
                    if (image) {
                        ...
                        NSError *finalError = ABI31_0_0RCTErrorWithMessage(errorMessage);
                                                       ...
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
    void (^completion)(NSError *, id, BOOL, NSString *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSString *fetchDate) {
        ...
        if ([imageOrData isKindOfClass:[NSData class]]) {
            NSDictionary *meta = ABI31_0_0RCTGetImageMetadata(imageOrData);
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
    ...
    // Even better may be to implement this on the ABI31_0_0RCTImageURLLoader that would try to load it,
    // but we'd have to run the logic both in ABI31_0_0RCTPhotoLibraryImageLoader and
    // ABI31_0_0RCTAssetsLibraryRequestHandler. Once we drop iOS7 though, we'd drop
    // ABI31_0_0RCTAssetsLibraryRequestHandler and can move it there.
    ...
    for (id<ABI31_0_0RCTImageURLLoader> loader in _loaders) {
        // Don't use ABI31_0_0RCTImageURLLoader protocol for modules that already conform to
        // ABI31_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
        // convert it back into data
        if (![loader conformsToProtocol:@protocol(ABI31_0_0RCTURLRequestHandler)] &&
...
{
    __block ABI31_0_0RCTImageLoaderCancellationBlock requestToken;
    requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
        ...
        if (ABI31_0_0RCTImageHasAlpha(image.CGImage)) {
            mimeType = @"image/png";
...
{
    if (requestToken) {
        ((ABI31_0_0RCTImageLoaderCancellationBlock)requestToken)();
    }
}

@end

@implementation ABI31_0_0RCTBridge (ABI31_0_0RCTImageLoader)

- (ABI31_0_0RCTImageLoader *)imageLoader
{
    return [self moduleForClass:[ABI31_0_0RCTImageLoader class]];
}
