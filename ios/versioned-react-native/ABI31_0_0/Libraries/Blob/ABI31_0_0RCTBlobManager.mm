...
#import <ReactABI31_0_0/ABI31_0_0RCTWebSocketModule.h>

static NSString *const kBlobURIScheme = @"blob";

@interface ABI31_0_0RCTBlobManager () <ABI31_0_0RCTNetworkingRequestHandler, ABI31_0_0RCTNetworkingResponseHandler, ABI31_0_0RCTWebSocketContentHandler>

@end

@implementation ABI31_0_0RCTBlobManager
{
...
ABI31_0_0RCT_EXPORT_MODULE(BlobModule)
...
{
  NSString *blobId = [ABI31_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI31_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI31_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI31_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
...
ABI31_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(nonnull NSNumber *)socketID)
{
...
ABI31_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI31_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI31_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI31_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI31_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI31_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI31_0_0RCTURLRequestHandler methods
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI31_0_0RCTURLRequestDelegate>)delegate
{
  // Lazy setup
  if (!_queue) {
...
#pragma mark - ABI31_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI31_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI31_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI31_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI31_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI31_0_0RCTWebSocketContentHandler methods
