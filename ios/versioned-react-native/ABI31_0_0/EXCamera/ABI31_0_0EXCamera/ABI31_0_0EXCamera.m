...
#import <ABI31_0_0EXBarCodeScannerInterface/ABI31_0_0EXBarCodeScannerProviderInterface.h>
...
#import <ABI31_0_0EXFaceDetectorInterface/ABI31_0_0EXFaceDetectorManagerProvider.h>
...
@property (nonatomic, strong) id<ABI31_0_0EXFaceDetectorManager> faceDetectorManager;
@property (nonatomic, strong) id<ABI31_0_0EXBarCodeScannerInterface> barCodeScanner;
...
@property (nonatomic, strong) ABI31_0_0EXPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI31_0_0EXPromiseRejectBlock videoRecordedReject;

@property (nonatomic, copy) ABI31_0_0EXDirectEventBlock onCameraReady;
@property (nonatomic, copy) ABI31_0_0EXDirectEventBlock onMountError;
@property (nonatomic, copy) ABI31_0_0EXDirectEventBlock onPictureSaved;

@property (nonatomic, copy) ABI31_0_0EXDirectEventBlock onBarCodeScanned;
@property (nonatomic, copy) ABI31_0_0EXDirectEventBlock onFacesDetected;

@end

@implementation ABI31_0_0EXCamera
...
- (void)updateFlashMode
{
  ...
  if (_flashMode == ABI31_0_0EXCameraFlashModeTorch) {
    if (![device hasTorch]) {
      return;
    }
    ...
    if (![device lockForConfiguration:&error]) {
      if (error) {
        ABI31_0_0EXLogInfo(@"%s: %@", __func__, error);
      }
      return;
    }
...
- (void)updateFocusDepth
{
  ...
  if (device == nil || device.focusMode != ABI31_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if ([device isLockingFocusWithCustomLensPositionSupported]) {
    ...
    ABI31_0_0EX_WEAKIFY(device);
    [device setFocusModeLockedWithLensPosition:_focusDepth completionHandler:^(CMTime syncTime) {
      ABI31_0_0EX_ENSURE_STRONGIFY(device);
      [device unlockForConfiguration];
    }];
    return;
  }

  ABI31_0_0EXLogInfo(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
  return;
}
...
- (void)updateWhiteBalance
{
  ...
  if (_whiteBalance == ABI31_0_0EXCameraWhiteBalanceAuto) {
    [device setWhiteBalanceMode:AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance];
    [device unlockForConfiguration];
  } else {
    AVCaptureWhiteBalanceTemperatureAndTintValues temperatureAndTint = {
      .temperature = [ABI31_0_0EXCameraUtils temperatureForWhiteBalance:_whiteBalance],
      .tint = 0,
    };
...
- (void)takePicture:(NSDictionary *)options resolve:(ABI31_0_0EXPromiseResolveBlock)resolve reject:(ABI31_0_0EXPromiseRejectBlock)reject
{
  ...
  [connection setVideoOrientation:[ABI31_0_0EXCameraUtils videoOrientationForDeviceOrientation:[[UIDevice currentDevice] orientation]]];
  ...
  [_stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
    ...
    takenImage = [ABI31_0_0EXCameraUtils cropImage:takenImage toRect:croppedSize];
    ...
    response[@"uri"] = [ABI31_0_0EXCameraUtils writeImage:takenImageData toPath:path];
    ...
    if ([options[@"exif"] boolValue]) {
      ...
      [ABI31_0_0EXCameraUtils updatePhotoMetadata:imageSampleBuffer withAdditionalData:@{ @"Orientation": @(imageRotation) } inResponse:response]; // TODO
    }
...
- (void)record:(NSDictionary *)options resolve:(ABI31_0_0EXPromiseResolveBlock)resolve reject:(ABI31_0_0EXPromiseRejectBlock)reject
{
  ...
  if (_movieFileOutput != nil && !_movieFileOutput.isRecording && _videoRecordedResolve == nil && _videoRecordedReject == nil) {
    ...
    if (options[@"quality"]) {
      ABI31_0_0EXCameraVideoResolution resolution = [options[@"quality"] integerValue];
      preset = [ABI31_0_0EXCameraUtils captureSessionPresetForVideoResolution:resolution];
    ...
    if (self.videoStabilizationMode != nil) {
      if (connection.isVideoStabilizationSupported == NO) {
        ABI31_0_0EXLogWarn(@"%s: Video Stabilization is not supported on this device.", __func__);
      } else {
        [connection setPreferredVideoStabilizationMode:self.videoStabilizationMode];
      }
    }
    [connection setVideoOrientation:[ABI31_0_0EXCameraUtils videoOrientationForInterfaceOrientation:[[UIApplication sharedApplication] statusBarOrientation]]];
...
- (void)initializeCaptureSessionInput
{
  ...
  AVCaptureVideoOrientation orientation = [ABI31_0_0EXCameraUtils videoOrientationForInterfaceOrientation:interfaceOrientation];
  ...
  dispatch_async(_sessionQueue, ^{
    ...
    NSError *error = nil;
    AVCaptureDevice *captureDevice = [ABI31_0_0EXCameraUtils deviceWithMediaType:AVMediaTypeVideo preferringPosition:self.presetCamera];
...
{
  ABI31_0_0EX_WEAKIFY(self);
  AVCaptureVideoOrientation videoOrientation = [ABI31_0_0EXCameraUtils videoOrientationForInterfaceOrientation:orientation];
  [ABI31_0_0EXUtilities performSynchronouslyOnMainThread:^{
    ABI31_0_0EX_ENSURE_STRONGIFY(self);
...
{
  id <ABI31_0_0EXFaceDetectorManagerProvider> faceDetectorProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI31_0_0EXFaceDetectorManagerProvider)];

  if (faceDetectorProvider) {
    id <ABI31_0_0EXFaceDetectorManager> faceDetector = [faceDetectorProvider createFaceDetectorManager];
    if (faceDetector) {
      ABI31_0_0EX_WEAKIFY(self);
...
{
  id<ABI31_0_0EXBarCodeScannerProviderInterface> barCodeScannerProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI31_0_0EXBarCodeScannerProviderInterface)];
  if (barCodeScannerProvider) {
    id<ABI31_0_0EXBarCodeScannerInterface> barCodeScanner = [barCodeScannerProvider createBarCodeScanner];
    if (barCodeScanner) {
      ABI31_0_0EX_WEAKIFY(self);
