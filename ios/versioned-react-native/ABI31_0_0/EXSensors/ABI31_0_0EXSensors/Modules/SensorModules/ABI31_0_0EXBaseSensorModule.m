...
@interface ABI31_0_0EXBaseSensorModule () <ABI31_0_0EXAppLifecycleListener>
...
@implementation ABI31_0_0EXBaseSensorModule

# pragma mark - ABI31_0_0EXBaseSensorModule

- (id)getSensorServiceFromModuleRegistry:(ABI31_0_0EXModuleRegistry *)moduleRegistry
{
  NSAssert(false, @"You've subclassed ABI31_0_0EXBaseSensorModule, but didn't override the `getSensorServiceFromModuleRegistry` method.");
  return nil;
}

- (void)setUpdateInterval:(double)updateInterval onSensorService:(id)sensorService
{
  NSAssert(false, @"You've subclassed ABI31_0_0EXBaseSensorModule, but didn't override the `setUpdateInterval:onSensorService:` method.");
}

- (void)subscribeToSensorService:(id)sensorService withHandler:(void (^)(NSDictionary *event))handlerBlock
{
  NSAssert(false, @"You've subclassed ABI31_0_0EXBaseSensorModule, but didn't override the `subscribeToSensorService:withHandler:` method.");
}

- (void)unsubscribeFromSensorService:(id)sensorService
{
  NSAssert(false, @"You've subclassed ABI31_0_0EXBaseSensorModule, but didn't override the `unsubscribeFromSensorService:` method.");
}

- (const NSString *)updateEventName
{
  NSAssert(false, @"You've subclassed ABI31_0_0EXBaseSensorModule, but didn't override the `updateEventName` method.");
  return nil;
}
...
- (void)startObserving {
  ...
  __weak ABI31_0_0EXBaseSensorModule *weakSelf = self;
  [self subscribeToSensorService:_sensorManager withHandler:^(NSDictionary *event) {
    __strong ABI31_0_0EXBaseSensorModule *strongSelf = weakSelf;
    if (strongSelf) {
...
ABI31_0_0EX_EXPORT_METHOD_AS(setUpdateInterval, setUpdateInterval:(nonnull NSNumber *)intervalMs resolve:(ABI31_0_0EXPromiseResolveBlock)resolve reject:(ABI31_0_0EXPromiseRejectBlock)rejecter) {
