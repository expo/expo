...
#import <ABI31_0_0EXSensors/ABI31_0_0EXSensorsManager.h>
#import <CoreMotion/CoreMotion.h>

@interface ABI31_0_0EXSensorsManager ()
...
@implementation ABI31_0_0EXSensorsManager
                                            ...
                                            withHandler:(void (^)(NSDictionary *event))handlerBlock
{
  ...
  if (![[self manager] isAccelerometerActive]) {
    ...
    __weak ABI31_0_0EXSensorsManager *weakSelf = self;
    [[self manager] startAccelerometerUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMAccelerometerData *data, NSError *error) {
      __strong ABI31_0_0EXSensorsManager *strongSelf = weakSelf;
      if (strongSelf) {
...
- (void)activateDeviceMotionUpdates
{
  ...
  [[self manager] startDeviceMotionUpdatesUsingReferenceFrame:CMAttitudeReferenceFrameXArbitraryCorrectedZVertical toQueue:[NSOperationQueue mainQueue] withHandler:^(CMDeviceMotion *data, NSError *error) {
    ...
    NSDictionary *result = @{
                             @"acceleration": @{
                                 @"x": @(data.userAcceleration.x * ABI31_0_0EXGravity),
                                 @"y": @(data.userAcceleration.y * ABI31_0_0EXGravity),
                                 @"z": @(data.userAcceleration.z * ABI31_0_0EXGravity)
                                 },
                             @"accelerationIncludingGravity": @{
                                 @"x": @((data.userAcceleration.x + data.gravity.x) * ABI31_0_0EXGravity),
                                 @"y": @((data.userAcceleration.y + data.gravity.y) * ABI31_0_0EXGravity),
                                 @"z": @((data.userAcceleration.z + data.gravity.z) * ABI31_0_0EXGravity)
                                 },
                             @"rotation": @{
