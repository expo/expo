...
#import <ABI31_0_0EXPermissions/ABI31_0_0EXLocationRequester.h>
...
NSString * const ABI31_0_0EXPermissionExpiresNever = @"never";

@interface ABI31_0_0EXPermissions ()

@property (nonatomic, strong) NSMutableArray *requests;
@property (nonatomic, weak) id<ABI31_0_0EXPermissionsServiceInterface> permissionsService;
@property (nonatomic, weak) id<ABI31_0_0EXUtilitiesInterface> utils;
@property (nonatomic, assign) NSString *experienceId;

@end

@implementation ABI31_0_0EXPermissions

ABI31_0_0EX_EXPORT_MODULE(ExpoPermissions);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI31_0_0EXPermissionsInterface)];
}
...
ABI31_0_0EX_EXPORT_METHOD_AS(getAsync,
  ...
  for (NSString *permissionType in permissionsTypes) {
    ...
    if (_permissionsService != nil) {
      if (![[self class] isExcludedScopedPermission:permissionType]
          && [permission[@"status"] isEqualToString:[[self class] permissionStringForStatus:ABI31_0_0EXPermissionStatusGranted]]
          && ![_permissionsService hasGrantedPermission:permissionType forExperience:_experienceId])
      {
        permission[@"status"] = [[self class] permissionStringForStatus:ABI31_0_0EXPermissionStatusDenied];
      }
    }
...
ABI31_0_0EX_EXPORT_METHOD_AS(askAsync,
...
- (void)askForGlobalPermissions:(NSArray<NSString *> *)permissionsTypes
                   ...
                   withRejecter:(ABI31_0_0EXPromiseRejectBlock)reject
{
  ...
  __block void (^askForNextPermission)(void) = ^() {
    ...
    id<ABI31_0_0EXPermissionRequester> requester = [[self class] getPermissionRequesterForType:permissionType];
  ...
  customResolver = ^(NSDictionary *permission) {
    ...
    {
      permissions[permissionType][@"status"] = [[self class] permissionStringForStatus:ABI31_0_0EXPermissionStatusDenied];
    }
...
+ (NSDictionary *)alwaysGrantedPermissions {
  return @{
           @"status": [ABI31_0_0EXPermissions permissionStringForStatus:ABI31_0_0EXPermissionStatusGranted],
...
- (NSDictionary *)getPermissionsForResource:(NSString *)type
{
  if ([type isEqualToString:@"notifications"]) {
    return [ABI31_0_0EXRemoteNotificationRequester permissions];
  } else if ([type isEqualToString:@"userFacingNotifications"]) {
    return [ABI31_0_0EXLocalNotificationRequester permissions];
  } else if ([type isEqualToString:@"location"]) {
    return [ABI31_0_0EXLocationRequester permissions];
  } else if ([type isEqualToString:@"camera"]) {
    return [ABI31_0_0EXCameraPermissionRequester permissions];
  } else if ([type isEqualToString:@"contacts"]) {
    return [ABI31_0_0EXContactsRequester permissions];
  } else if ([type isEqualToString:@"audioRecording"]) {
    return [ABI31_0_0EXAudioRecordingPermissionRequester permissions];
  } else if ([type isEqualToString:@"cameraRoll"]) {
    return [ABI31_0_0EXCameraRollRequester permissions];
  } else if ([type isEqualToString:@"calendar"]) {
    return [ABI31_0_0EXCalendarRequester permissions];
  } else if ([type isEqualToString:@"reminders"]) {
    return [ABI31_0_0EXRemindersRequester permissions];
  } else if ([[self class] isPermissionImplicitlyGranted:type]) {
    return [ABI31_0_0EXPermissions alwaysGrantedPermissions]; // permission is implicit
  } else {
    return nil;
  }
}
...
- (BOOL)hasGrantedPermission:(NSString *)permissionType
{
  ...
  if (!permissions) {
    ABI31_0_0EXLogWarn(@"Permission with type '%@' not found.", permissionType);
    return false;
  }
             ...
             withRejecter:(ABI31_0_0EXPromiseRejectBlock)reject
{
  return [self askForPermissionsWithTypes:permissionsTypes withResults:^(NSDictionary *results) {
    ...
    [results enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, NSDictionary *singleResult, BOOL * _Nonnull stop) {
      BOOL value = [singleResult[@"status"] isEqualToString:[ABI31_0_0EXPermissions permissionStringForStatus:ABI31_0_0EXPermissionStatusGranted]];
                      ...
                      withRejecter:(ABI31_0_0EXPromiseRejectBlock)reject
{
  ...
  for (NSString *permissionType in permissionsTypes) {
    ...
    if ([permission[@"status"] isEqualToString:[ABI31_0_0EXPermissions permissionStringForStatus:ABI31_0_0EXPermissionStatusGranted]]) {
...
+ (id<ABI31_0_0EXPermissionRequester>)getPermissionRequesterForType:(NSString *)type
{
  if ([type isEqualToString:@"notifications"]) {
    return [[ABI31_0_0EXRemoteNotificationRequester alloc] init];
  } else if ([type isEqualToString:@"userFacingNotifications"]) {
    return [[ABI31_0_0EXLocalNotificationRequester alloc] init];
  } else if ([type isEqualToString:@"location"]) {
    return [[ABI31_0_0EXLocationRequester alloc] init];
  } else if ([type isEqualToString:@"camera"]) {
    return [[ABI31_0_0EXCameraPermissionRequester alloc] init];
  } else if ([type isEqualToString:@"contacts"]) {
    return [[ABI31_0_0EXContactsRequester alloc] init];
  } else if ([type isEqualToString:@"audioRecording"]) {
    return [[ABI31_0_0EXAudioRecordingPermissionRequester alloc] init];
  } else if ([type isEqualToString:@"cameraRoll"]) {
    return [[ABI31_0_0EXCameraRollRequester alloc] init];
  } else if ([type isEqualToString:@"calendar"]) {
    return [[ABI31_0_0EXCalendarRequester alloc] init];
  } else if ([type isEqualToString:@"reminders"]) {
    return [[ABI31_0_0EXRemindersRequester alloc] init];
  } else {
    return nil;
  }
}
...
{
  switch (status) {
    case ABI31_0_0EXPermissionStatusGranted:
      return @"granted";
    case ABI31_0_0EXPermissionStatusDenied:
      return @"denied";
    default:
      return @"undetermined";
  }
...
+ (ABI31_0_0EXPermissionStatus)statusForPermissions:(NSDictionary *)permissions
{
  ...
  if ([status isEqualToString:@"granted"]) {
    return ABI31_0_0EXPermissionStatusGranted;
  } else if ([status isEqualToString:@"denied"]) {
    return ABI31_0_0EXPermissionStatusDenied;
  } else {
    return ABI31_0_0EXPermissionStatusUndetermined;
  }
}
