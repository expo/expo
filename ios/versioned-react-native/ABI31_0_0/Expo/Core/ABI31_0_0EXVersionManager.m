...
#import "ABI31_0_0EXFileSystem.h"
#import "ABI31_0_0EXVersionManager.h"
...
#import <ReactABI31_0_0/ABI31_0_0RCTExceptionsManager.h>
...
#import <ReactABI31_0_0/ABI31_0_0RCTAsyncLocalStorage.h>
...
#define ABI31_0_0EX_KERNEL_SERVICE_NONE @"ABI31_0_0EXKernelServiceNone"

// this is needed because ABI31_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI31_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSDictionary *> *ABI31_0_0EXScopedModuleClasses;
void ABI31_0_0EXRegisterScopedModule(Class, ...);
void ABI31_0_0EXRegisterScopedModule(Class moduleClass, ...)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI31_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  ...
  va_start(argumentList, moduleClass);
    while ((kernelServiceClassName = va_arg(argumentList, NSString*))) {
      if ([kernelServiceClassName isEqualToString:@"nil"]) {
        unversionedKernelServiceClassNames[kernelServiceClassName] = ABI31_0_0EX_KERNEL_SERVICE_NONE;
      } else {
  ...
  if (moduleClassName) {
    ABI31_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassNames;
  }
}

@interface ABI31_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI31_0_0EXVersionManager ()
...
@implementation ABI31_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI31_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI31_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI31_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
#ifdef ABI31_0_0EX_ENABLE_UNSAFE_SYSTRACE
    items[@"dev-profiler"] = @{
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI31_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI31_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI31_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return ([ABI31_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI31_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI31_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI31_0_0RCTSetFatalHandler(fatalHandler);
  ABI31_0_0RCTSetLogThreshold(threshold);
 ...
 *    @ABI31_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI31_0_0EXKernel *kernel
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSString *localStorageDirectory = [[ABI31_0_0EXFileSystem documentDirectoryForExperienceId:experienceId] stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];
  BOOL isOpeningHomeInProductionMode = params[@"browserModuleClass"] && !manifest[@"developer"];

  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI31_0_0EXAppState alloc] init],
                                    [[ABI31_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:(!isOpeningHomeInProductionMode && isDeveloper)],
                                    [[ABI31_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI31_0_0EXStatusBarManager alloc] init],
                                    [[ABI31_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory],
                                    ]];
  ...
  if (exceptionsManagerDelegate) {
    ABI31_0_0RCTExceptionsManager *exceptionsManager = [[ABI31_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
    [extraModules addObject:exceptionsManager];
  } else {
    ABI31_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
  }
  
  if (params[@"testEnvironment"]) {
    ABI31_0_0EXTestEnvironment testEnvironment = (ABI31_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI31_0_0EXTestEnvironmentNone) {
      ABI31_0_0EXTest *testModule = [[ABI31_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI31_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI31_0_0RCTDevMenu
    [extraModules addObject:[[ABI31_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI31_0_0RCTRedBox
    [extraModules addObject:[[ABI31_0_0EXDisabledRedBox alloc] init]];
  }

  ABI31_0_0EXModuleRegistryProvider *moduleRegistryProvider = [[ABI31_0_0EXModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI31_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI31_0_0EXModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

  ABI31_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI31_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];

  NSArray<id<ABI31_0_0RCTBridgeModule>> *expoModules = [moduleRegistryAdapter extraModulesForParams:params andExperience:experienceId withScopedModulesArray:extraModules withKernelServices:services];
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI31_0_0EXScopedModuleClasses) {
    [ABI31_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI31_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
