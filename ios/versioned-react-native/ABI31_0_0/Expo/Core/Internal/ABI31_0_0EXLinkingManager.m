...
#import "ABI31_0_0EXLinkingManager.h"
...
NSString * const ABI31_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI31_0_0EXLinkingManager ()

@property (nonatomic, weak) id <ABI31_0_0EXLinkingManagerScopedModuleDelegate> kernelLinkingDelegate;
@property (nonatomic, strong) NSURL *initialUrl;
@property (nonatomic) BOOL hasListeners;

@end

@implementation ABI31_0_0EXLinkingManager

ABI31_0_0EX_EXPORT_SCOPED_MODULE(ABI31_0_0RCTLinkingManager, KernelLinkingManager);
...
#pragma mark - ABI31_0_0RCTEventEmitter methods

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI31_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI31_0_0RCTFatal(ABI31_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI31_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}
                  ...
                  reject:(ABI31_0_0RCTPromiseRejectBlock)reject)
{
  if ([_kernelLinkingDelegate linkingModule:self shouldOpenExpoUrl:URL]) {
    ...
    [ABI31_0_0EXUtil performSynchronouslyOnMainThread:^{
      opened = [ABI31_0_0RCTSharedApplication() openURL:URL];
    }];
    if (opened) {
      resolve(nil);
    } else {
                  ...
                  reject:(__unused ABI31_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!canOpen) {
    [ABI31_0_0EXUtil performSynchronouslyOnMainThread:^{
      canOpen = [ABI31_0_0RCTSharedApplication() canOpenURL:URL];
    }];
  }
  resolve(@(canOpen));
}
...
ABI31_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI31_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI31_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI31_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
