#import "ABI31_0_0EXImagePicker.h"
...
#import "ABI31_0_0EXCameraPermissionRequester.h"
#import "ABI31_0_0EXCameraRollRequester.h"
...
const CGFloat ABI31_0_0EXDefaultImageQuality = 0.2;

@interface ABI31_0_0EXImagePicker ()
...
@implementation ABI31_0_0EXImagePicker

ABI31_0_0EX_EXPORT_SCOPED_MODULE(ExponentImagePicker, PermissionsManager);
...
ABI31_0_0RCT_EXPORT_METHOD(launchCameraAsync:(NSDictionary *)options
                  resolver:(ABI31_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI31_0_0EXPermissions statusForPermissions:[ABI31_0_0EXCameraRollRequester permissions]] != ABI31_0_0EXPermissionStatusGranted ||
      ![_kernelPermissionsServiceDelegate hasGrantedPermission:@"cameraRoll" forExperience:self.experienceId] ||
      [ABI31_0_0EXPermissions statusForPermissions:[ABI31_0_0EXCameraPermissionRequester permissions]] != ABI31_0_0EXPermissionStatusGranted ||
  ...
  self.reject = reject;
  [self launchImagePicker:ABI31_0_0RNImagePickerTargetCamera options:options];
}

ABI31_0_0RCT_EXPORT_METHOD(launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI31_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI31_0_0EXPermissions statusForPermissions:[ABI31_0_0EXCameraRollRequester permissions]] != ABI31_0_0EXPermissionStatusGranted ||
  ...
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI31_0_0RNImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI31_0_0RNImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI31_0_0RNImagePickerTarget)target
{
  self.picker = [[UIImagePickerController alloc] init];

  if (target == ABI31_0_0RNImagePickerTargetCamera) {
#if TARGET_IPHONE_SIMULATOR
    self.reject(ABI31_0_0RCTErrorUnspecified, @"Camera not available on simulator", nil);
    return;
#else
...
#endif
  } else { // ABI31_0_0RNImagePickerTargetLibrarySingleImage
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    id<ABI31_0_0EXUtilitiesInterface> utils = [self->_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI31_0_0EXUtilitiesInterface)];
          ...
          completionHandler:(void (^)(void))completionHandler
{
  ...
  NSData *data = UIImageJPEGRepresentation(image, quality == nil ? ABI31_0_0EXDefaultImageQuality : [quality floatValue]);
  ...
  if ([[imageURL absoluteString] containsString:@"ext=PNG"]) {
    ...
    ABI31_0_0RCTLogWarn(@"Unsupported format of the picked image. Using JPEG instead.");
  }
  ...
  if ([[self.options objectForKey:@"exif"] boolValue]) {
    ...
    if (metadata) {
      ...
      if (assets.count > 0) {
        ...
        ABI31_0_0RCTLogInfo(@"Could not fetch metadata for image %@", [imageURL absoluteString]);
        completionHandler();
      }
    }
  } else {
...
- (void)handleVideoWithInfo:(NSDictionary * _Nonnull)info saveAt:(NSString *)directory updateResponse:(NSMutableDictionary *)response
{
  ...
  if (assets.count > 0) {
    ...
    ABI31_0_0RCTLogInfo(@"Could not fetch metadata for video %@", [videoURL absoluteString]);
  }
