...
#import "ABI31_0_0EXImageManipulator.h"
...
@implementation ABI31_0_0EXImageManipulator

ABI31_0_0RCT_EXPORT_MODULE(ExponentImageManipulator);
...
ABI31_0_0RCT_EXPORT_METHOD(manipulate:(NSString *)uri
              ...
              rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject
{
  for (NSDictionary *options in actions) {
    if (options[@"resize"]) {
      ...
      image = [ABI31_0_0EXImageUtils cropImage:image toRect:cropDimensions];
    }
  }
  ...
  if ([format isEqualToString:@"jpeg"]) {
    ...
    ABI31_0_0RCTLogWarn(@"Unsupported format: %@, using JPEG instead.", format);
    imageData = UIImageJPEGRepresentation(image, compressionValue);
    extension = @".jpg";
  }
