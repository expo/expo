...
#import "ABI31_0_0EXDocumentPicker.h"
...
static NSString * ABI31_0_0EXConvertMimeTypeToUTI(NSString *mimeType)
{
  CFStringRef uti;
...
@interface ABI31_0_0EXDocumentPicker () <UIDocumentMenuDelegate, UIDocumentPickerDelegate>
...
@implementation ABI31_0_0EXDocumentPicker

@synthesize bridge = _bridge;

ABI31_0_0RCT_EXPORT_MODULE(ExponentDocumentPicker)

ABI31_0_0RCT_EXPORT_METHOD(getDocumentAsync:(NSDictionary *)options resolver:(ABI31_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject) {
  _resolve = resolve;
  _reject = reject;

  NSString *type = ABI31_0_0EXConvertMimeTypeToUTI(options[@"type"] ?: @"*/*");

  if (options[@"copyToCacheDirectory"] && [ABI31_0_0RCTConvert BOOL:options[@"copyToCacheDirectory"]] == NO) {
  ...
  if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
    documentMenuVC.popoverPresentationController.sourceRect = CGRectMake(CGRectGetMidX([ABI31_0_0RCTPresentedViewController().view frame]), CGRectGetMaxY([ABI31_0_0RCTPresentedViewController().view frame]), 0, 0);
    documentMenuVC.popoverPresentationController.sourceView = ABI31_0_0RCTPresentedViewController().view;
    documentMenuVC.modalPresentationStyle = UIModalPresentationPageSheet;
  }

  id<ABI31_0_0EXUtilitiesInterface> utils = [_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI31_0_0EXUtilitiesInterface)];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url
{
  ...
  if (_shouldCopyToCacheDirectory) {
    id<ABI31_0_0EXFileSystemInterface> fileSystem = [_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI31_0_0EXFileSystemInterface)];
    if (!fileSystem) {
