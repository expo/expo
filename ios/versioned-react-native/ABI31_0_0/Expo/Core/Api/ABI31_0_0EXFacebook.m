...
@implementation FBSDKInternalUtility (ABI31_0_0EXFacebook)
...
NSString * const ABI31_0_0EXFacebookLoginErrorDomain = @"E_FBLOGIN";
NSString * const ABI31_0_0EXFacebookLoginBehaviorErrorDomain = @"E_FBLOGIN_BEHAVIOR";

@implementation ABI31_0_0EXFacebook

@synthesize bridge = _bridge;

ABI31_0_0RCT_EXPORT_MODULE(ExponentFacebook)

ABI31_0_0RCT_REMAP_METHOD(logInWithReadPermissionsAsync,
                 ...
                 rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject)
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (loginMgr.loginBehavior != FBSDKLoginBehaviorWeb) {
      id<ABI31_0_0EXConstantsInterface> constants = [self->_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI31_0_0EXConstantsInterface)];
      ...
      if (![constants.appOwnership isEqualToString:@"expo"] && ![[self class] facebookAppIdFromNSBundle]) {
        ...
        reject(ABI31_0_0EXFacebookLoginBehaviorErrorDomain, message, ABI31_0_0RCTErrorWithMessage(message));
        return;
      }
    }
    ...
    @try {
      [loginMgr logInWithReadPermissions:permissions fromViewController:nil handler:^(FBSDKLoginManagerLoginResult *result, NSError *error) {
        if (error) {
          reject(ABI31_0_0EXFacebookLoginErrorDomain, @"Error with Facebook login", error);
          return;
        }
        ...
        if (![result.token.appID isEqualToString:appId]) {
          reject(ABI31_0_0EXFacebookLoginErrorDomain, @"Logged into wrong app, try again?", nil);
          return;
        }
    ...
    @catch (NSException *exception) {
      NSError *error = [[NSError alloc] initWithDomain:ABI31_0_0EXFacebookLoginErrorDomain code:650 userInfo:@{
