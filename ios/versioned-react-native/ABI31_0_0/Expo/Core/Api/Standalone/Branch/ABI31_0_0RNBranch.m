...
#import "ABI31_0_0BranchLinkProperties+RNBranch.h"
...
// - Add #import "ABI31_0_0EXConstants.h"
// - Use ABI31_0_0EX_EXPORT_SCOPED_MODULE(ABI31_0_0RNBranch, BranchManager); instead of ABI31_0_0RCT_EXPORT_MODULE
// - Add scoped module init (initWithExperienceId)
// - Add setBridge
// - Move code from init to setBridge and delete init

NSString * const ABI31_0_0RNBranchLinkOpenedNotification = @"ABI31_0_0RNBranchLinkOpenedNotification";
NSString * const ABI31_0_0RNBranchLinkOpenedNotificationErrorKey = @"error";
NSString * const ABI31_0_0RNBranchLinkOpenedNotificationParamsKey = @"params";
NSString * const ABI31_0_0RNBranchLinkOpenedNotificationUriKey = @"uri";
NSString * const ABI31_0_0RNBranchLinkOpenedNotificationBranchUniversalObjectKey = @"branch_universal_object";
NSString * const ABI31_0_0RNBranchLinkOpenedNotificationLinkPropertiesKey = @"link_properties";
...
static NSString * const ABI31_0_0RNBranchErrorDomain = @"ABI31_0_0RNBranchErrorDomain";
static NSInteger const ABI31_0_0RNBranchUniversalObjectNotFoundError = 1;


#pragma mark - Private ABI31_0_0RNBranch declarations

@interface ABI31_0_0RNBranch()
@property (nonatomic, readonly) UIViewController *currentViewController;
@property (nonatomic) ABI31_0_0RNBranchAgingDictionary<NSString *, BranchUniversalObject *> *universalObjectMap;
@end

#pragma mark - ABI31_0_0RNBranch implementation

@implementation ABI31_0_0RNBranch
...
+ (Branch *)branch
{
    @synchronized(self) {
        ...
        dispatch_once(&once, ^{
            ABI31_0_0RNBranchConfig *config = ABI31_0_0RNBranchConfig.instance;

            // YES if either [ABI31_0_0RNBranch useTestInstance] was called or useTestInstance: true is present in branch.json.
...
{
    ABI31_0_0RCTLogInfo(@"Initializing Branch SDK v. %@", BNC_SDK_VERSION);
    ABI31_0_0RNBranchConfig *config = ABI31_0_0RNBranchConfig.instance;
    if (config.debugMode) {
        [instance setDebug];
    }
...
- (NSDictionary<NSString *, NSString *> *)constantsToExport {
    return @{
             // ABI31_0_0RN events transmitted to JS by event emitter
             @"INIT_SESSION_SUCCESS": ABI31_0_0RNBranchInitSessionSuccess,
             @"INIT_SESSION_ERROR": ABI31_0_0RNBranchInitSessionError,
...
//Called by AppDelegate.m -- stores initSession result in static variables and posts ABI31_0_0RNBranchLinkOpened event that's captured by the ABI31_0_0RNBranch instance to emit it to ReactABI31_0_0 Native
+ (void)initSessionWithLaunchOptions:(NSDictionary *)launchOptions isReferrable:(BOOL)isReferrable {
    [self.branch initSessionWithLaunchOptions:launchOptions isReferrable:isReferrable andRegisterDeepLinkHandler:^(NSDictionary *params, NSError *error) {
        ...
        if (error) result[ABI31_0_0RNBranchLinkOpenedNotificationErrorKey] = error;
        if (params) {
            result[ABI31_0_0RNBranchLinkOpenedNotificationParamsKey] = params;
            ...
            if (clickedBranchLink) {
                ...
                if (branchUniversalObject) result[ABI31_0_0RNBranchLinkOpenedNotificationBranchUniversalObjectKey] = branchUniversalObject;

                BranchLinkProperties *linkProperties = [BranchLinkProperties getBranchLinkPropertiesFromDictionary:params];
                if (linkProperties) result[ABI31_0_0RNBranchLinkOpenedNotificationLinkPropertiesKey] = linkProperties;

                if (params[@"~referring_link"]) {
                    result[ABI31_0_0RNBranchLinkOpenedNotificationUriKey] = [NSURL URLWithString:params[@"~referring_link"]];
                }
            }
            else if (params[@"+non_branch_link"]) {
                result[ABI31_0_0RNBranchLinkOpenedNotificationUriKey] = [NSURL URLWithString:params[@"+non_branch_link"]];
            }
        }

        [[NSNotificationCenter defaultCenter] postNotificationName:ABI31_0_0RNBranchLinkOpenedNotification object:nil userInfo:result];
    }];
}
...
- (void)setBridge:(ABI31_0_0RCTBridge *)bridge
{
  ...
  if ([constants.appOwnership isEqualToString:@"standalone"]) {
    _universalObjectMap = [ABI31_0_0RNBranchAgingDictionary dictionaryWithTtl:3600.0];

    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onInitSessionFinished:) name:ABI31_0_0RNBranchLinkOpenedNotification object:nil];
  }
}
...
- (void) onInitSessionFinished:(NSNotification*) notification {
    NSURL *uri = notification.userInfo[ABI31_0_0RNBranchLinkOpenedNotificationUriKey];
    NSError *error = notification.userInfo[ABI31_0_0RNBranchLinkOpenedNotificationErrorKey];
    NSDictionary *params = notification.userInfo[ABI31_0_0RNBranchLinkOpenedNotificationParamsKey];

    initSessionWithLaunchOptionsResult = @{
                                           ABI31_0_0RNBranchLinkOpenedNotificationErrorKey: error.localizedDescription ?: NSNull.null,
                                           ABI31_0_0RNBranchLinkOpenedNotificationParamsKey: params ?: NSNull.null,
                                           ABI31_0_0RNBranchLinkOpenedNotificationUriKey: uri.absoluteString ?: NSNull.null
                                           };

    // If there is an error, fire error event
    if (error) {
        [ABI31_0_0RNBranchEventEmitter initSessionDidEncounterErrorWithPayload:initSessionWithLaunchOptionsResult];
    }

    // otherwise notify the session is finished
    else {
        [ABI31_0_0RNBranchEventEmitter initSessionDidSucceedWithPayload:initSessionWithLaunchOptionsResult];
    }
}
...
- (BranchUniversalObject *)findUniversalObjectWithIdent:(NSString *)ident rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject
{
    ...
    if (!universalObject) {
        ...
        NSError *error = [NSError errorWithDomain:ABI31_0_0RNBranchErrorDomain
                                             code:ABI31_0_0RNBranchUniversalObjectNotFoundError
                                         userInfo:@{IdentFieldName : ident,
                                                    NSLocalizedDescriptionKey: errorMessage
                                                    }];

        reject(@"ABI31_0_0RNBranch::Error::BUONotFound", errorMessage, error);
    }

    return universalObject;
}

#pragma mark - Methods exported to ReactABI31_0_0 Native

#pragma mark redeemInitSessionResult
ABI31_0_0RCT_EXPORT_METHOD(
                  redeemInitSessionResult:(ABI31_0_0RCTPromiseResolveBlock)resolve
...
ABI31_0_0RCT_EXPORT_METHOD(
                  getLatestReferringParams:(ABI31_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(__unused ABI31_0_0RCTPromiseRejectBlock)reject
                  ) {
...
ABI31_0_0RCT_EXPORT_METHOD(
                  getFirstReferringParams:(ABI31_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(__unused ABI31_0_0RCTPromiseRejectBlock)reject
                  ) {
...
ABI31_0_0RCT_EXPORT_METHOD(
    ...
    [branchUniversalObject getShortUrlWithLinkProperties:linkProperties andCallback:^(NSString *url, NSError *error) {
        if (!error) {
            ABI31_0_0RCTLogInfo(@"ABI31_0_0RNBranch Success");
            resolve(@{ @"url": url });
        }
        else if (error.code == BNCDuplicateResourceError) {
            reject(@"ABI31_0_0RNBranch::Error::DuplicateResourceError", error.localizedDescription, error);
        }
        else {
            reject(@"ABI31_0_0RNBranch::Error", error.localizedDescription, error);
        }
    }];
}
...
ABI31_0_0RCT_EXPORT_METHOD(
    ...
    [self.class.branch getShortURLWithParams:linkPropertiesMap
                              ...
                              andCallback:^(NSString *url, NSError *error) {
                                  if (error) {
                                      ABI31_0_0RCTLogError(@"ABI31_0_0RNBranch::Error: %@", error.localizedDescription);
                                      reject(@"ABI31_0_0RNBranch::Error", @"getShortURLWithParams", error);
                                  }
                                  resolve(url);
                              }];
}
...
ABI31_0_0RCT_EXPORT_METHOD(
    ...
    [self.class.branch loadRewardsWithCallback:^(BOOL changed, NSError *error) {
        if(!error) {
            ...
            ABI31_0_0RCTLogError(@"Load Rewards Error: %@", error.localizedDescription);
            reject(@"ABI31_0_0RNBranch::Error::loadRewardsWithCallback", @"loadRewardsWithCallback", error);
        }
    }];
}
    ...
    if (bucket) {
        [self.class.branch redeemRewards:amount forBucket:bucket callback:^(BOOL changed, NSError *error) {
            if (!error) {
                ...
                ABI31_0_0RCTLogError(@"Redeem Rewards Error: %@", error.localizedDescription);
                reject(@"ABI31_0_0RNBranch::Error::redeemRewards", error.localizedDescription, error);
            }
        }];
    } else {
...
ABI31_0_0RCT_EXPORT_METHOD(
                  getCreditHistory:(ABI31_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject
                  ) {
    [self.class.branch getCreditHistoryWithCallback:^(NSArray *list, NSError *error) {
        if (!error) {
            ...
            ABI31_0_0RCTLogError(@"Credit History Error: %@", error.localizedDescription);
            reject(@"ABI31_0_0RNBranch::Error::getCreditHistory", error.localizedDescription, error);
        }
    }];
}
