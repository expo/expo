...
#import "ABI31_0_0EXInterstitialAdManager.h"
...
@implementation ABI31_0_0EXInterstitialAdManager

@synthesize bridge = _bridge;

ABI31_0_0RCT_EXPORT_MODULE(CTKInterstitialAdManager)
...
{
  ABI31_0_0RCTAssert(_resolve == nil && _reject == nil, @"Only one `showAd` can be called at once");
  ABI31_0_0RCTAssert(_isBackground == false, @"`showAd` can be called only when experience is running in foreground");
...
{
  [_interstitialAd showAdFromRootViewController:ABI31_0_0RCTPresentedViewController()];
}
...
- (void)bridgeDidForeground:(NSNotification *)notification
{
  ...
  if (_adViewController) {
    [ABI31_0_0RCTPresentedViewController() presentViewController:_adViewController animated:NO completion:nil];
...
- (void)bridgeDidBackground:(NSNotification *)notification
{
  ...
  if (_interstitialAd) {
    _adViewController = ABI31_0_0RCTPresentedViewController();
