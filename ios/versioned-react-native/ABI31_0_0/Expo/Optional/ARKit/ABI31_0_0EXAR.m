...
#import "ABI31_0_0EXAR.h"
#import "ABI31_0_0EXUnversioned.h"
#import "ABI31_0_0EXGLView.h"
#import <ReactABI31_0_0/ABI31_0_0RCTUIManager.h>
#import "ABI31_0_0EXGLARSessionManager.h"

@interface ABI31_0_0EXAR () <ABI31_0_0EXGLARSessionManagerDelegate>

@property (nonatomic, strong) id arSessionManager;

@end

@implementation ABI31_0_0EXAR

ABI31_0_0RCT_EXPORT_MODULE(ExponentAR);
...
+ (NSMutableArray *)serializeARVideoFormats:(NSArray<ARVideoFormat *>*)videoFormats  API_AVAILABLE(ios(11.3))
{
  ...
  if (@available(iOS 11.3, *)) {
    for (ARVideoFormat *videoFormat in videoFormats) {
      [output addObject:[ABI31_0_0EXAR serializeARVideoFormat: videoFormat]];
    }
  }
...
- (NSDictionary<NSString *, NSString *> *)constantsToExport
{
  ...
  if (@available(iOS 11.3, *)) {
    [output addEntriesFromDictionary:@{
                                       @"ARKitVersion": @"1.5",
//                                       @"FaceTrackingVideoFormats": [ABI31_0_0EXAR serializeARVideoFormats:[ARFaceTrackingConfiguration supportedVideoFormats]],
                                       @"WorldTrackingVideoFormats": [ABI31_0_0EXAR serializeARVideoFormats:[ARWorldTrackingConfiguration supportedVideoFormats]],
                                       @"OrientationTrackingVideoFormats": [ABI31_0_0EXAR serializeARVideoFormats:[AROrientationTrackingConfiguration supportedVideoFormats]],
                                       }];
  }
...
ABI31_0_0RCT_REMAP_METHOD(startAsync,
                 startAsyncWithReactABI31_0_0Tag:(nonnull NSNumber *)tag
                 ...
                 rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI31_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ...
    if (![view isKindOfClass:[ABI31_0_0EXGLView class]]) {
      reject(@"E_GLVIEW_MANAGER_BAD_VIEW_TAG", nil, ABI31_0_0RCTErrorWithMessage(@"ABI31_0_0EXAR.startARSessionAsync: Expected an ABI31_0_0EXGLView"));
      return;
    }
    ...
    ABI31_0_0EXGLView *exglView = (ABI31_0_0EXGLView *)view;
    
    Class sessionManagerClass = NSClassFromString(@"ABI31_0_0EXGLARSessionManager");
    if (sessionManagerClass) {
      ...
      if (response[@"error"]) {
        reject(@"ERR_ARKIT_FAILED_TO_INIT", response[@"error"], ABI31_0_0RCTErrorWithMessage(response[@"error"]));
      } else {
        resolve(response);
      }
    } else {
      NSString *response = @"ARKIT capabilities were not included with this build.";
      reject(@"ERR_ARKIT_FAILED_TO_INIT", response, ABI31_0_0RCTErrorWithMessage(response));
    }
  }];
}

ABI31_0_0RCT_REMAP_METHOD(stopAsync,
                 resolver:(ABI31_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI31_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI31_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
ABI31_0_0RCT_EXPORT_METHOD(pause)
{
  [_arSessionManager pause];
}

ABI31_0_0RCT_EXPORT_METHOD(resume)
{
  [_arSessionManager resume];
}

ABI31_0_0RCT_EXPORT_METHOD(reset)
{
  [_arSessionManager reset];
}

ABI31_0_0RCT_REMAP_METHOD(setProvidesAudioData,
                 providesAudioData:(BOOL)providesAudioData)
{
  [_arSessionManager setProvidesAudioData:providesAudioData];
}

ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getProvidesAudioData, NSNumber *, getProvidesAudioData)
{
  return @([_arSessionManager providesAudioData]);
}

ABI31_0_0RCT_REMAP_METHOD(setLightEstimationEnabled,
                 lightEstimationEnabled:(BOOL)lightEstimationEnabled)
{
  [_arSessionManager setLightEstimationEnabled:lightEstimationEnabled];
}

ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getLightEstimationEnabled, NSNumber *, getLightEstimationEnabled)
{
  return @([_arSessionManager lightEstimationEnabled]);
}

ABI31_0_0RCT_REMAP_METHOD(setAutoFocusEnabled,
                 isAutoFocusEnabled:(BOOL)isAutoFocusEnabled)
{
  [_arSessionManager setAutoFocusEnabled:isAutoFocusEnabled];
}

ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getAutoFocusEnabled, NSNumber *, getAutoFocusEnabled)
{
  return @([_arSessionManager autoFocusEnabled]);
}

ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getPlaneDetection, NSString *, getPlaneDetection)
{
...
ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(performHitTest,
...
ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getWorldAlignment, NSString *, getWorldAlignment)
{
  NSArray *items = @[@"gravity", @"gravityAndHeading", @"alignmentCamera"];
  return [items objectAtIndex:[_arSessionManager worldAlignment]];
}

ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCurrentFrame,
                                      nullable NSDictionary *,
...
ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getARMatrices,
                                      nullable NSDictionary *,
...
ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getVideoFormat, nullable NSDictionary *, getVideoFormat)
{
  if (!_arSessionManager) {
    return nil;
  }
  if (@available(iOS 11.3, *)) {
    ...
    return [ABI31_0_0EXAR serializeARVideoFormat:videoFormat];
  }
  return nil;
}

ABI31_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCameraTexture, NSNumber *, getCameraTexture)
{
  return @([_arSessionManager cameraTexture]);
}

ABI31_0_0RCT_REMAP_METHOD(setShouldAttemptRelocalization,
...
ABI31_0_0RCT_REMAP_METHOD(setWorldOriginAsync,
...
ABI31_0_0RCT_REMAP_METHOD(setConfigurationAsync,
...
ABI31_0_0RCT_REMAP_METHOD(setDetectionImagesAsync,
