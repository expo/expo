...
@interface ABI31_0_0EXFaceDetectorManager() <GMVDataOutputDelegate>
...
- (void)setIsEnabled:(BOOL)newFaceDetecting
{
  ...
  if ([self isDetectingFaces] != newFaceDetecting) {
    ...
    __weak ABI31_0_0EXFaceDetectorManager *weakSelf = self;
    [self _runBlockIfQueueIsPresent:^{
      __strong ABI31_0_0EXFaceDetectorManager *strongSelf = weakSelf;
      if (strongSelf) {
...
- (void)dataOutput:(GMVDataOutput *)dataOutput didFinishedDetection:(NSArray<__kindof GMVFeature *> *)results
{
  ...
  // width or height started happen after refactor: moving face detection logic from ABI31_0_0EXCameraManager to ABI31_0_0EXFaceDetectorManager.
  ...
  CGAffineTransform transform = [ABI31_0_0EXFaceDetectorUtils transformFromDeviceOutput:dataOutput toInterfaceVideoOrientation:interfaceVideoOrientation];
  
  ABI31_0_0EXFaceEncoder *faceEncoder = [[ABI31_0_0EXFaceEncoder alloc] initWithTransform:transform];
