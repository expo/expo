...
typedef void (^ABI6_0_0RCTPropBlock)(id<ABI6_0_0RCTComponent> view, id json);

@interface ABI6_0_0RCTComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) ABI6_0_0RCTPropBlock propBlock;

@end

@implementation ABI6_0_0RCTComponentProp
...
@implementation ABI6_0_0RCTComponentData
{
  id<ABI6_0_0RCTComponent> _defaultView; // Only needed for ABI6_0_0RCT_CUSTOM_VIEW_PROPERTY
  NSMutableDictionary<NSString *, ABI6_0_0RCTPropBlock> *_viewPropBlocks;
  NSMutableDictionary<NSString *, ABI6_0_0RCTPropBlock> *_shadowPropBlocks;
                              ...
                              bridge:(ABI6_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    _name = ABI6_0_0RCTBridgeModuleNameForClass(_managerClass);
    ABI6_0_0RCTAssert(_name.length, @"Invalid moduleName '%@'", _name);
    ...
    while (cls != [ABI6_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI6_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI6_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI6_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI6_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ABI6_0_0RCTPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, ABI6_0_0RCTPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  ABI6_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {

    __weak ABI6_0_0RCTComponentData *weakSelf = self;
    ...
    if ([_managerClass respondsToSelector:selector]) {
      ...
      type = ABI6_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<ABI6_0_0RCTComponent> view, id json) {
        ABI6_0_0RCTComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        json = ABI6_0_0RCTNilIfNull(json);
      ...
      if (type == NSSelectorFromString(@"ABI6_0_0RCTBubblingEventBlock:") ||
          type == NSSelectorFromString(@"ABI6_0_0RCTDirectEventBlock:")) {

        // Special case for event handlers
        __weak ABI6_0_0RCTViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<ABI6_0_0RCTComponent> weakTarget = target;
          ((void (*)(id, SEL, id))objc_msgSend)(target, setter, [ABI6_0_0RCTConvert BOOL:json] ? ^(NSDictionary *body) {
            body = [NSMutableDictionary dictionaryWithDictionary:body];
            ((NSMutableDictionary *)body)[@"target"] = weakTarget.ReactABI6_0_0Tag;
            [weakManager.bridge.eventDispatcher sendInputEventWithName:ABI6_0_0RCTNormalizeInputEventName(name) body:body];
          } : nil);
        };
        ...
        NSMethodSignature *typeSignature = [[ABI6_0_0RCTConvert class] methodSignatureForSelector:type];
        if (!typeSignature) {
          ABI6_0_0RCTLogError(@"No +[ABI6_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
          return ^(__unused id<ABI6_0_0RCTComponent> view, __unused id json){};
        }
  ...
  #define ABI6_0_0RCT_CASE(_value, _type) \
          case _value: { \
            ...
            setterBlock = ^(id target, id json) { \
              if (json) { \
                ...
                set(target, setter, convert([ABI6_0_0RCTConvert class], type, json)); \
            ...
            ABI6_0_0RCT_CASE(_C_SEL, SEL)
            ABI6_0_0RCT_CASE(_C_CHARPTR, const char *)
            ...
            ABI6_0_0RCT_CASE(_C_PTR, void *)
            ABI6_0_0RCT_CASE(_C_ID, id)
      ...
      propBlock = ^(__unused id view, __unused id json) {
        ...
        setterBlock(target, ABI6_0_0RCTNilIfNull(json));
      };
    }
    ...
    if (ABI6_0_0RCT_DEBUG) {
      ...
      ABI6_0_0RCTPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<ABI6_0_0RCTComponent> view, id json) {
        NSString *logPrefix = [NSString stringWithFormat:
                               @"Error setting property '%@' of %@ with tag #%@: ",
                               name, weakSelf.name, view.ReactABI6_0_0Tag];

        ABI6_0_0RCTPerformBlockWithLogPrefix(^{ unwrappedBlock(view, json); }, logPrefix);
      };
    }
...
- (NSDictionary<NSString *, id> *)viewConfig
{
  ...
  if (ABI6_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customDirectEventTypes))) {
    NSArray<NSString *> *events = [self.manager customDirectEventTypes];
    if (ABI6_0_0RCT_DEBUG) {
      ABI6_0_0RCTAssert(!events || [events isKindOfClass:[NSArray class]],
        @"customDirectEventTypes must return an array, but %@ returned %@",
        _managerClass, [events class]);
    }
    for (NSString *event in events) {
      [directEvents addObject:ABI6_0_0RCTNormalizeInputEventName(event)];
    }
  }

  NSMutableArray<NSString *> *bubblingEvents = [NSMutableArray new];
  if (ABI6_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    ...
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI6_0_0RCTNormalizeInputEventName(event)];
    }
  }
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if ([methodName hasPrefix:@"propConfig"]) {
      ...
      if (nameRange.length) {
        ...
        if (ABI6_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
          ABI6_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                      "to '%@'", name, _name, propTypes[name], type);
        }

        if ([type isEqualToString:@"ABI6_0_0RCTBubblingEventBlock"]) {
          [bubblingEvents addObject:ABI6_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else if ([type isEqualToString:@"ABI6_0_0RCTDirectEventBlock"]) {
          [directEvents addObject:ABI6_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else {
          propTypes[name] = type;
        }
  ...
  if (ABI6_0_0RCT_DEBUG) {
    for (NSString *event in directEvents) {
      if ([bubblingEvents containsObject:event]) {
        ABI6_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
