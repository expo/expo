...
#import "ABI6_0_0UIView+Private.h"
#import "UIView+ReactABI6_0_0.h"

CGFloat const ZINDEX_DEFAULT = 0;
CGFloat const ZINDEX_STICKY_HEADER = 50;

@interface ABI6_0_0RCTScrollEvent : NSObject <ABI6_0_0RCTEvent>

- (instancetype)initWithEventName:(NSString *)eventName
                         ReactABI6_0_0Tag:(NSNumber *)ReactABI6_0_0Tag
...
@implementation ABI6_0_0RCTScrollEvent
{
...
{
  ABI6_0_0RCTAssertParam(ReactABI6_0_0Tag);

  if ((self = [super init])) {
    _eventName = [eventName copy];
    _viewTag = ReactABI6_0_0Tag;
...
- (ABI6_0_0RCTScrollEvent *)coalesceWithEvent:(ABI6_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI6_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI6_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI6_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, copy) NSIndexSet *stickyHeaderIndices;
@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) ABI6_0_0RCTRefreshControl *reactRefreshControl;

@end


@implementation ABI6_0_0RCTCustomScrollView
{
...
- (UIView *)contentView
{
  return ((ABI6_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI6_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI6_0_0RCTUIManager JSResponder] isKindOfClass:[ABI6_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI6_0_0 scroll
...
- (void)dockClosestSectionHeader
{
  ...
  NSUInteger subviewCount = contentView.ReactABI6_0_0Subviews.count;
  [_stickyHeaderIndices enumerateIndexesWithOptions:0 usingBlock:
   ^(NSUInteger idx, BOOL *stop) {
    ...
    UIView *header = contentView.ReactABI6_0_0Subviews[idx];
...
- (void)setReactRefreshControl:(ABI6_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI6_0_0RCTScrollView
{
  ABI6_0_0RCTEventDispatcher *_eventDispatcher;
  ABI6_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI6_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI6_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;
    _scrollView = [[ABI6_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
- (void)insertReactABI6_0_0Subview:(UIView *)view atIndex:(__unused NSInteger)atIndex
{
  if ([view isKindOfClass:[ABI6_0_0RCTRefreshControl class]]) {
    _scrollView.reactRefreshControl = (ABI6_0_0RCTRefreshControl*)view;
  } else {
    ABI6_0_0RCTAssert(_contentView == nil, @"ABI6_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
...
{
  if ([subview isKindOfClass:[ABI6_0_0RCTRefreshControl class]]) {
    _scrollView.reactRefreshControl = nil;
  } else {
    ABI6_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
...
{
  ABI6_0_0RCTAssert(_scrollView.contentSize.width <= self.frame.size.width,
...
- (void)layoutSubviews
{
  ...
  ABI6_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI6_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI6_0_0RCTRefreshControl *refreshControl = _scrollView.reactRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI6_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI6_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI6_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI6_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI6_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI6_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI6_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI6_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI6_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI6_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI6_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI6_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI6_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI6_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI6_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI6_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI6_0_0RCTView contentInsetsForView:self];
...
- (void)ReactABI6_0_0BridgeDidFinishTransaction
{
  ...
  if (ABI6_0_0RCT_DEBUG) {
    ...
    NSUInteger subviewCount = _scrollView.contentView.ReactABI6_0_0Subviews.count;
    ...
    if (lastIndex != NSNotFound && lastIndex >= subviewCount) {
      ABI6_0_0RCTLogWarn(@"Sticky header index %zd was outside the range {0, %zd}",
...
#define ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI6_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);

- (void)setOnRefreshStart:(ABI6_0_0RCTDirectEventBlock)onRefreshStart
{
  if (!onRefreshStart) {
    _onRefreshStart = nil;
    _scrollView.reactRefreshControl = nil;
  ...
  if (!_scrollView.reactRefreshControl) {
    ABI6_0_0RCTRefreshControl *refreshControl = [ABI6_0_0RCTRefreshControl new];
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI6_0_0RCTScrollEvent *scrollEvent = [[ABI6_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ReactABI6_0_0Tag:self.ReactABI6_0_0Tag
...
@implementation ABI6_0_0RCTEventDispatcher (ABI6_0_0RCTScrollView)
...
- (void)sendFakeScrollEvent:(NSNumber *)ReactABI6_0_0Tag
{
  ...
  ABI6_0_0RCTScrollEvent *fakeScrollEvent = [[ABI6_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI6_0_0Tag:ReactABI6_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
