...
#import "ABI6_0_0RCTAlertManager.h"
...
@implementation ABI6_0_0RCTConvert (UIAlertViewStyle)

ABI6_0_0RCT_ENUM_CONVERTER(UIAlertViewStyle, (@{
...
@interface ABI6_0_0RCTAlertManager() <UIAlertViewDelegate>

@end

@implementation ABI6_0_0RCTAlertManager
{
  NSMutableArray<UIAlertView *> *_alerts;
  NSMutableArray<UIAlertController *> *_alertControllers;
  NSMutableArray<ABI6_0_0RCTResponseSenderBlock> *_alertCallbacks;
 ...
 */
ABI6_0_0RCT_EXPORT_METHOD(alertWithArgs:(NSDictionary *)args
                  callback:(ABI6_0_0RCTResponseSenderBlock)callback)
{
  NSString *title = [ABI6_0_0RCTConvert NSString:args[@"title"]];
  NSString *message = [ABI6_0_0RCTConvert NSString:args[@"message"]];
  UIAlertViewStyle type = [ABI6_0_0RCTConvert UIAlertViewStyle:args[@"type"]];
  NSArray<NSDictionary *> *buttons = [ABI6_0_0RCTConvert NSDictionaryArray:args[@"buttons"]];
  NSString *defaultValue = [ABI6_0_0RCTConvert NSString:args[@"defaultValue"]];
  NSString *cancelButtonKey = [ABI6_0_0RCTConvert NSString:args[@"cancelButtonKey"]];
  NSString *destructiveButtonKey = [ABI6_0_0RCTConvert NSString:args[@"destructiveButtonKey"]];

  if (!title && !message) {
    ABI6_0_0RCTLogError(@"Must specify either an alert title, or message, or both");
    return;
  }

  if (buttons.count == 0) {
    if (type == UIAlertViewStyleDefault) {
      buttons = @[@{@"0": ABI6_0_0RCTUIKitLocalizedString(@"OK")}];
      cancelButtonKey = @"0";
    } else {
      buttons = @[
        @{@"0": ABI6_0_0RCTUIKitLocalizedString(@"OK")},
        @{@"1": ABI6_0_0RCTUIKitLocalizedString(@"Cancel")},
      ];
      cancelButtonKey = @"1";
    }
  }
  ...
  BOOL preferAlertView = (!ABI6_0_0RCTRunningInAppExtension() &&
                          !destructiveButtonKey &&
                          UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);

  if (preferAlertView || [UIAlertController class] == nil) {

    UIAlertView *alertView = ABI6_0_0RCTAlertView(title, nil, self, nil, nil);
    ...
    for (NSDictionary<NSString *, id> *button in buttons) {
      if (button.count != 1) {
        ABI6_0_0RCTLogError(@"Button definitions should have exactly one key.");
      }
      NSString *buttonKey = button.allKeys.firstObject;
      NSString *buttonTitle = [ABI6_0_0RCTConvert NSString:button[buttonKey]];
  ...
  {
    UIViewController *presentingController = ABI6_0_0RCTKeyWindow().rootViewController;
    if (presentingController == nil) {
      ABI6_0_0RCTLogError(@"Tried to display alert view but there is no application window. args: %@", args);
      return;
    }
    ...
    switch (type) {
      ...
      case UIAlertViewStyleSecureTextInput: {
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder = ABI6_0_0RCTUIKitLocalizedString(@"Password");
      ...
      case UIAlertViewStyleLoginAndPasswordInput: {
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder = ABI6_0_0RCTUIKitLocalizedString(@"Login");
          textField.text = defaultValue;
        }];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder = ABI6_0_0RCTUIKitLocalizedString(@"Password");
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  ABI6_0_0RCTAssert(index != NSNotFound, @"Dismissed alert was not recognised");

  ABI6_0_0RCTResponseSenderBlock callback = _alertCallbacks[index];
