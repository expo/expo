...
@class ABI6_0_0RCTRedBoxWindow;

@protocol ABI6_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI6_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(NSDictionary *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI6_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI6_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI6_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI6_0_0RCTRedBoxWindow
{
...
- (void)dismiss
{
  ...
  [ABI6_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
{
  // NOTE: We could use ABI6_0_0RCTKeyCommands for this, but since
...
@interface ABI6_0_0RCTRedBox () <ABI6_0_0RCTInvalidating, ABI6_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI6_0_0RCTRedBox
{
  ABI6_0_0RCTRedBoxWindow *_window;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack isUpdate:(BOOL)isUpdate
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!_window) {
      _window = [[ABI6_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
ABI6_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [_window dismiss];
  });
...
- (void)redBoxWindow:(ABI6_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(NSDictionary *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI6_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI6_0_0RCTJSONStringify(stackFrame, NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(ABI6_0_0RCTRedBoxWindow *)redBoxWindow {
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI6_0_0RCTReloadNotification object:_bridge.baseBridge userInfo:nil];
}

@end

@implementation ABI6_0_0RCTBridge (ABI6_0_0RCTRedBox)

- (ABI6_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI6_0_0RCTRedBox class]];
}
...
- (ABI6_0_0RCTRedBox *)redBox { return nil; }
