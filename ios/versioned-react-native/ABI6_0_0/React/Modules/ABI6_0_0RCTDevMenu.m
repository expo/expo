...
static NSString *const ABI6_0_0RCTShowDevMenuNotification = @"ABI6_0_0RCTShowDevMenuNotification";
static NSString *const ABI6_0_0RCTDevMenuSettingsKey = @"ABI6_0_0RCTDevMenu";

@implementation UIWindow (ABI6_0_0RCTDevMenu)

- (void)ABI6_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI6_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

typedef NS_ENUM(NSInteger, ABI6_0_0RCTDevMenuType) {
  ABI6_0_0RCTDevMenuTypeButton,
  ABI6_0_0RCTDevMenuTypeToggle
};

@interface ABI6_0_0RCTDevMenuItem ()

@property (nonatomic, assign, readonly) ABI6_0_0RCTDevMenuType type;
...
@implementation ABI6_0_0RCTDevMenuItem
{
  id _handler; // block
}

- (instancetype)initWithType:(ABI6_0_0RCTDevMenuType)type
                         key:(NSString *)key
...
{
  return [[self alloc] initWithType:ABI6_0_0RCTDevMenuTypeButton
                                key:nil
                              title:title
                      selectedTitle:nil
                            handler:handler];
...
{
  return [[self alloc] initWithType:ABI6_0_0RCTDevMenuTypeToggle
                                key:key
                              title:title
...
{
  switch (_type) {
    case ABI6_0_0RCTDevMenuTypeButton: {
      if (_handler) {
        ((void(^)())_handler)();
      }
      break;
    }
    case ABI6_0_0RCTDevMenuTypeToggle: {
      if (_handler) {
...
@interface ABI6_0_0RCTDevMenu () <ABI6_0_0RCTBridgeModule, UIActionSheetDelegate, ABI6_0_0RCTInvalidating, ABI6_0_0RCTWebSocketProxyDelegate>

@property (nonatomic, strong) Class executorClass;

@end

@implementation ABI6_0_0RCTDevMenu
{
  ...
  BOOL _jsLoaded;
  NSArray<ABI6_0_0RCTDevMenuItem *> *_presentedItems;
  NSMutableArray<ABI6_0_0RCTDevMenuItem *> *_extraMenuItems;
...
+ (void)initialize
{
  ...
  ABI6_0_0RCTSwapInstanceMethods([UIWindow class], @selector(motionEnded:withEvent:), @selector(ABI6_0_0RCT_motionEnded:withEvent:));
}
  ...
  if ((self = [super init])) {
    ...
    [notificationCenter addObserver:self
                           selector:@selector(showOnShake)
                               name:ABI6_0_0RCTShowDevMenuNotification
                             object:nil];
    ...
    _settings = [[NSMutableDictionary alloc] initWithDictionary:[_defaults objectForKey:ABI6_0_0RCTDevMenuSettingsKey]];
    _extraMenuItems = [NSMutableArray new];

    __weak ABI6_0_0RCTDevMenu *weakSelf = self;

    [_extraMenuItems addObject:[ABI6_0_0RCTDevMenuItem toggleItemWithKey:@"showInspector"
...
// TODO: Move non-UI logic into separate ABI6_0_0RCTDevSettings module
- (void)connectPackager
{
  Class webSocketManagerClass = NSClassFromString(@"ABI6_0_0RCTWebSocketManager");
  id<ABI6_0_0RCTWebSocketProxy> webSocketManager = (id <ABI6_0_0RCTWebSocketProxy>)[webSocketManagerClass sharedInstance];
...
- (void)socketProxy:(__unused id<ABI6_0_0RCTWebSocketProxy>)sender didReceiveMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([target isEqualToString:@"bridge"]) {
    if ([action isEqualToString:@"reload"]) {
      if ([options[@"debug"] boolValue]) {
        _bridge.executorClass = NSClassFromString(@"ABI6_0_0RCTWebSocketExecutor");
      }
      [_bridge reload];
    }
  }
...
- (void)settingsDidChange
{
  ...
  __weak ABI6_0_0RCTDevMenu *weakSelf = self;
  NSDictionary *settings = [_defaults objectForKey:ABI6_0_0RCTDevMenuSettingsKey];
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateSettings:(NSDictionary *)settings
{
  ...
  for (ABI6_0_0RCTDevMenuItem *item in _extraMenuItems) {
    if (item.key) {
...
- (void)updateSetting:(NSString *)name value:(id)value
{
  ...
  [_defaults setObject:_settings forKey:ABI6_0_0RCTDevMenuSettingsKey];
...
- (void)jsLoaded:(NSNotification *)notification
{
  ...
  _liveReloadURL = nil;
  ABI6_0_0RCTSourceCode *sourceCodeModule = [_bridge moduleForClass:[ABI6_0_0RCTSourceCode class]];
  if (!sourceCodeModule.scriptURL) {
    if (!sourceCodeModule) {
      ABI6_0_0RCTLogWarn(@"ABI6_0_0RCTSourceCode module not found");
    } else if (!ABI6_0_0RCTRunningInTestEnvironment()) {
      ABI6_0_0RCTLogWarn(@"ABI6_0_0RCTSourceCode module scriptURL has not been set");
    }
...
{
  [self addItem:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
- (NSArray<ABI6_0_0RCTDevMenuItem *> *)menuItems
{
  NSMutableArray<ABI6_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items

  __weak ABI6_0_0RCTDevMenu *weakSelf = self;

  [items addObject:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload" handler:^{
    [weakSelf reload];
  }]];

  Class jsDebuggingExecutorClass = NSClassFromString(@"ABI6_0_0RCTWebSocketExecutor");
  if (!jsDebuggingExecutorClass) {
    [items addObject:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", _webSocketExecutorName] handler:^{
      UIAlertView *alert = ABI6_0_0RCTAlertView(
        [NSString stringWithFormat:@"%@ Debugger Unavailable", _webSocketExecutorName],
        [NSString stringWithFormat:@"You need to include the ABI6_0_0RCTWebSocket library to enable %@ debugging", _webSocketExecutorName],
        nil,
        @"OK",
        nil);
      [alert show];
    ...
    [items addObject:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:debugTitleJS handler:^{
  ...
  if (_liveReloadURL) {
    ...
    [items addObject:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:liveReloadTitle handler:^{
      weakSelf.liveReloadEnabled = !_liveReloadEnabled;
    }]];

    NSString *profilingTitle  = ABI6_0_0RCTProfileIsProfiling() ? @"Stop Systrace" : @"Start Systrace";
    [items addObject:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:profilingTitle handler:^{
  ...
  if ([self hotLoadingAvailable]) {
    ...
    [items addObject:[ABI6_0_0RCTDevMenuItem buttonItemWithTitle:hotLoadingTitle handler:^{
...
ABI6_0_0RCT_EXPORT_METHOD(show)
{
  if (_actionSheet || !_bridge || ABI6_0_0RCTRunningInAppExtension()) {
    return;
  }

  UIActionSheet *actionSheet = [UIActionSheet new];
  actionSheet.title = @"ReactABI6_0_0 Native: Development";
  actionSheet.delegate = self;

  NSArray<ABI6_0_0RCTDevMenuItem *> *items = [self menuItems];
  for (ABI6_0_0RCTDevMenuItem *item in items) {
    switch (item.type) {
      case ABI6_0_0RCTDevMenuTypeButton: {
  ...
  [actionSheet showInView:ABI6_0_0RCTKeyWindow().rootViewController.view];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  ABI6_0_0RCTDevMenuItem *item = _presentedItems[buttonIndex];
  switch (item.type) {
    case ABI6_0_0RCTDevMenuTypeButton: {
      [item callHandler];
      break;
...
ABI6_0_0RCT_EXPORT_METHOD(reload)
{
...
- (void)setProfilingEnabled:(BOOL)enabled
{
  ...
  if (_liveReloadURL && enabled != ABI6_0_0RCTProfileIsProfiling()) {
    if (enabled) {
      [_bridge startProfiling];
    } else {
      [_bridge stopProfiling:^(NSData *logData) {
        ABI6_0_0RCTProfileSendResult(_bridge, @"systrace", logData);
      }];
    }
  }
}
...
- (void)setHotLoadingEnabled:(BOOL)enabled
{
  ...
  if (ABI6_0_0RCTGetURLQueryParam(_bridge.bundleURL, @"hot").boolValue != actuallyEnabled) {
    _bridge.bundleURL = ABI6_0_0RCTURLByReplacingQueryParam(_bridge.bundleURL, @"hot",
...
- (void)checkForUpdates
{
  ...
  _updateTask = [[NSURLSession sharedSession] dataTaskWithURL:_liveReloadURL completionHandler:
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI6_0_0RCTDevMenu *strongSelf = weakSelf;
...
- (void)addItem:(ABI6_0_0RCTDevMenu *)item {}

@end

#endif

@implementation  ABI6_0_0RCTBridge (ABI6_0_0RCTDevMenu)

- (ABI6_0_0RCTDevMenu *)devMenu
{
#if ABI6_0_0RCT_DEV
  return [self moduleForClass:[ABI6_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
