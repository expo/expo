...
const NSInteger ABI6_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI6_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI6_0_0RCTGetEventID(id<ABI6_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI6_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI6_0_0RCTEvent>> *_events;
...
{
  [_bridge enqueueJSCall:@"ABI6_0_0RCTNativeAppEventEmitter.emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI6_0_0RCT_DEBUG) {
    ABI6_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI6_0_0 tag");
  }

  name = ABI6_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI6_0_0RCTEventEmitter.receiveEvent"
                    args:body ? @[body[@"target"], name, body] : @[body[@"target"], name]];
}

- (void)sendTextEventWithType:(ABI6_0_0RCTTextEventType)type
...
- (void)sendEvent:(id<ABI6_0_0RCTEvent>)event
{
  [_eventQueueLock lock];

  NSNumber *eventID = ABI6_0_0RCTGetEventID(event);

  id<ABI6_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI6_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
  ...
  if (scheduleEventsDispatch) {
    ...
    } queue:ABI6_0_0RCTJSThread];
  }
}

- (void)dispatchEvent:(id<ABI6_0_0RCTEvent>)event
{
...
@implementation ABI6_0_0RCTBridge (ABI6_0_0RCTEventDispatcher)

- (ABI6_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI6_0_0RCTEventDispatcher class]];
}
