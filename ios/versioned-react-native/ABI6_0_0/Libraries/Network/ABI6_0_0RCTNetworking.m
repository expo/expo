...
#import "ABI6_0_0RCTHTTPRequestHandler.h"
#import "ABI6_0_0RCTLog.h"
#import "ABI6_0_0RCTUtils.h"

typedef ABI6_0_0RCTURLRequestCancellationBlock (^ABI6_0_0RCTHTTPQueryResult)(NSError *error, NSDictionary<NSString *, id> *result);

@interface ABI6_0_0RCTNetworking ()

- (ABI6_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(NSDictionary<NSString *, id> *)data
                                                 callback:(ABI6_0_0RCTHTTPQueryResult)callback;
@end

/**
 * Helper to convert FormData payloads into multipart/formdata requests.
 */
@interface ABI6_0_0RCTHTTPFormDataHelper : NSObject

@property (nonatomic, weak) ABI6_0_0RCTNetworking *networker;

@end

@implementation ABI6_0_0RCTHTTPFormDataHelper
{
  NSMutableArray<NSDictionary<NSString *, id> *> *_parts;
  NSMutableData *_multipartBody;
  ABI6_0_0RCTHTTPQueryResult _callback;
  NSString *_boundary;
}

static NSString *ABI6_0_0RCTGenerateFormBoundary()
{
...
- (ABI6_0_0RCTURLRequestCancellationBlock)process:(NSArray<NSDictionary *> *)formData
                                 callback:(ABI6_0_0RCTHTTPQueryResult)callback
{
  ABI6_0_0RCTAssertThread(_networker.methodQueue, @"process: must be called on method queue");
  ...
  _boundary = ABI6_0_0RCTGenerateFormBoundary();
...
- (ABI6_0_0RCTURLRequestCancellationBlock)handleResult:(NSDictionary<NSString *, id> *)result
                                         error:(NSError *)error
{
  ABI6_0_0RCTAssertThread(_networker.methodQueue, @"handleResult: must be called on method queue");
 ...
 */
@implementation ABI6_0_0RCTNetworking
{
  NSMutableDictionary<NSNumber *, ABI6_0_0RCTNetworkTask *> *_tasksByRequestID;
  NSArray<id<ABI6_0_0RCTURLRequestHandler>> *_handlers;
}
...
- (id<ABI6_0_0RCTURLRequestHandler>)handlerForRequest:(NSURLRequest *)request
{
  if (!request.URL) {
    return nil;
  }
  ...
  if (!_handlers) {
    ...
    _handlers = [[_bridge modulesConformingToProtocol:@protocol(ABI6_0_0RCTURLRequestHandler)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI6_0_0RCTURLRequestHandler> a, id<ABI6_0_0RCTURLRequestHandler> b) {
  ...
  if (ABI6_0_0RCT_DEBUG) {
    ...
    id<ABI6_0_0RCTURLRequestHandler> previousHandler = nil;
    for (id<ABI6_0_0RCTURLRequestHandler> handler in _handlers) {
      ...
      if ([handler canHandleRequest:request]) {
        if (previousHandler) {
          if (priority == previousPriority) {
            ABI6_0_0RCTLogError(@"The ABI6_0_0RCTURLRequestHandlers %@ and %@ both reported that"
...
- (ABI6_0_0RCTURLRequestCancellationBlock)buildRequest:(NSDictionary<NSString *, id> *)query
                                 completionBlock:(void (^)(NSURLRequest *request))block
{
  ABI6_0_0RCTAssertThread(_methodQueue, @"buildRequest: must be called on method queue");

  NSURL *URL = [ABI6_0_0RCTConvert NSURL:query[@"url"]]; // this is marked as nullable in JS, but should not be null
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:URL];
  request.HTTPMethod = [ABI6_0_0RCTConvert NSString:ABI6_0_0RCTNilIfNull(query[@"method"])].uppercaseString ?: @"GET";
  request.allHTTPHeaderFields = [self stripNullsInRequestHeaders:[ABI6_0_0RCTConvert NSDictionary:query[@"headers"]]];
  request.timeoutInterval = [ABI6_0_0RCTConvert NSTimeInterval:query[@"timeout"]];
  NSDictionary<NSString *, id> *data = [ABI6_0_0RCTConvert NSDictionary:ABI6_0_0RCTNilIfNull(query[@"data"])];
  return [self processDataForHTTPQuery:data callback:^(NSError *error, NSDictionary<NSString *, id> *result) {
    if (error) {
      ABI6_0_0RCTLogError(@"Error processing request body: %@", error);
      // Ideally we'd circle back to JS here and notify an error/abort on the request.
      return (ABI6_0_0RCTURLRequestCancellationBlock)nil;
    }
    ...
    if ([request.allHTTPHeaderFields[@"Content-Encoding"] isEqualToString:@"gzip"]) {
      request.HTTPBody = ABI6_0_0RCTGzipData(request.HTTPBody, -1 /* default */);
 ...
 *
 */
- (ABI6_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(nullable NSDictionary<NSString *, id> *)query callback:
(ABI6_0_0RCTURLRequestCancellationBlock (^)(NSError *error, NSDictionary<NSString *, id> *result))callback
{
  ABI6_0_0RCTAssertThread(_methodQueue, @"processDataForHTTPQuery: must be called on method queue");

  if (!query) {
    return callback(nil, nil);
  }
  NSData *body = [ABI6_0_0RCTConvert NSData:query[@"string"]];
  if (body) {
    return callback(nil, @{@"body": body});
  }
  NSURLRequest *request = [ABI6_0_0RCTConvert NSURLRequest:query[@"uri"]];
  if (request) {

    __block ABI6_0_0RCTURLRequestCancellationBlock cancellationBlock = nil;
    ABI6_0_0RCTNetworkTask *task = [self networkTaskWithRequest:request completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
      dispatch_async(_methodQueue, ^{
        cancellationBlock = callback(error, data ? @{@"body": data, @"contentType": ABI6_0_0RCTNullIfNil(response.MIMEType)} : nil);
      });
    }];

    [task start];

    __weak ABI6_0_0RCTNetworkTask *weakTask = task;
    return ^{
      [weakTask cancel];
  ...
  NSArray<NSDictionary *> *formData = [ABI6_0_0RCTConvert NSDictionaryArray:query[@"formData"]];
  if (formData) {
    ABI6_0_0RCTHTTPFormDataHelper *formDataHelper = [ABI6_0_0RCTHTTPFormDataHelper new];
...
- (void)sendData:(NSData *)data forTask:(ABI6_0_0RCTNetworkTask *)task
{
  ABI6_0_0RCTAssertThread(_methodQueue, @"sendData: must be called on method queue");
  ...
  if (!responseText && data.length) {
    ...
    if (!responseText) {
      ABI6_0_0RCTLogWarn(@"Received data was not a string, or was not a recognised encoding.");
      return;
    }
  }
...
- (void)sendRequest:(NSURLRequest *)request
 incrementalUpdates:(BOOL)incrementalUpdates
     responseSender:(ABI6_0_0RCTResponseSenderBlock)responseSender
{
  ABI6_0_0RCTAssertThread(_methodQueue, @"sendRequest: must be called on method queue");

  __block ABI6_0_0RCTNetworkTask *task;

  ABI6_0_0RCTURLRequestProgressBlock uploadProgressBlock = ^(int64_t progress, int64_t total) {
    dispatch_async(_methodQueue, ^{
  ...
  ABI6_0_0RCTURLRequestCompletionBlock completionBlock =
  ^(NSURLResponse *response, NSData *data, NSError *error) {
    dispatch_async(_methodQueue, ^{
      ...
      NSArray *responseJSON = @[task.requestID,
                                ABI6_0_0RCTNullIfNil(error.localizedDescription),
...
{
  id<ABI6_0_0RCTURLRequestHandler> handler = [self handlerForRequest:request];
  if (!handler) {
    ABI6_0_0RCTLogError(@"No suitable URL request handler found for %@", request.URL);
    return nil;
  }

  return [[ABI6_0_0RCTNetworkTask alloc] initWithRequest:request
                                          handler:handler
...
ABI6_0_0RCT_EXPORT_METHOD(sendRequest:(NSDictionary *)query
                  responseSender:(ABI6_0_0RCTResponseSenderBlock)responseSender)
{
  ...
  [self buildRequest:query completionBlock:^(NSURLRequest *request) {
    ...
    BOOL incrementalUpdates = [ABI6_0_0RCTConvert BOOL:query[@"incrementalUpdates"]];
...
ABI6_0_0RCT_EXPORT_METHOD(cancelRequest:(nonnull NSNumber *)requestID)
{
...
@implementation ABI6_0_0RCTBridge (ABI6_0_0RCTNetworking)

- (ABI6_0_0RCTNetworking *)networking
{
  return [self moduleForClass:[ABI6_0_0RCTNetworking class]];
}
