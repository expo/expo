...
#import "ABI6_0_0RCTImagePickerManager.h"
...
@interface ABI6_0_0RCTImagePickerManager ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI6_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI6_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI6_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
}

ABI6_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI6_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI6_0_0RCTResponseSenderBlock)callback)
{
...
ABI6_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI6_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI6_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI6_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI6_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  if ([ABI6_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI6_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI6_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI6_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI6_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI6_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI6_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    [self _dismissPicker:picker args:@[imageURL.absoluteString, ABI6_0_0RCTNullIfNil(height), ABI6_0_0RCTNullIfNil(width)]];
    return;
  }
        ...
        cancelCallback:(ABI6_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI6_0_0RCTKeyWindow().rootViewController;
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI6_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI6_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
