...
#import "ABI6_0_0RCTCameraRollManager.h"
...
@implementation ABI6_0_0RCTConvert (ALAssetGroup)

ABI6_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI6_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI6_0_0RCTCameraRollManager

ABI6_0_0RCT_EXPORT_MODULE()

@synthesize bridge = _bridge;

NSString *const ABI6_0_0RCTErrorUnableToLoad = @"E_UNABLE_TO_LOAD";
NSString *const ABI6_0_0RCTErrorUnableToSave = @"E_UNABLE_TO_SAVE";

ABI6_0_0RCT_EXPORT_METHOD(saveImageWithTag:(NSString *)imageTag
                  ...
                  reject:(ABI6_0_0RCTPromiseRejectBlock)reject)
{
  [_bridge.imageLoader loadImageWithTag:imageTag callback:^(NSError *loadError, UIImage *loadedImage) {
    if (loadError) {
      reject(ABI6_0_0RCTErrorUnableToLoad, nil, loadError);
      return;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
        if (saveError) {
          ABI6_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
          reject(ABI6_0_0RCTErrorUnableToSave, nil, saveError);
        } else {
...
static void ABI6_0_0RCTResolvePromise(ABI6_0_0RCTPromiseResolveBlock resolve,
...
ABI6_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI6_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI6_0_0RCTPromiseRejectBlock)reject)
{
  NSUInteger first = [ABI6_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI6_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI6_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI6_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI6_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI6_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI6_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI6_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
    reject(ABI6_0_0RCTErrorUnableToLoad, nil, error);
  }];
}
