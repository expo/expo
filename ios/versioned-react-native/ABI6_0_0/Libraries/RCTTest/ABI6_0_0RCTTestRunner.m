...
#import "ABI6_0_0RCTTestRunner.h"
...
@implementation ABI6_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI6_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI6_0_0RCTAssertParam(app);
  ABI6_0_0RCTAssertParam(referenceDirectory);
...
#if RUNNING_ON_CI
    _scriptURL = [[NSBundle bundleForClass:[ABI6_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    ABI6_0_0RCTAssert(_scriptURL != nil, @"Could not locate main.jsBundle");
#else
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI6_0_0RCTSetLogFunction(^(ABI6_0_0RCTLogLevel level, ABI6_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI6_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI6_0_0RCTBridge *bridge = [[ABI6_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI6_0_0RCTRootView *rootView = [[ABI6_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
    rootView.frame = CGRectMake(0, 0, 320, 2000); // Constant size for testing on multiple devices

    ABI6_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI6_0_0RCTTestModule class]];
    ABI6_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI6_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI6_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI6_0_0RCTSetLogFunction(ABI6_0_0RCTDefaultLogFunction);
    ...
    ABI6_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI6_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI6_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI6_0_0RCTAssert(testModule.status != ABI6_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI6_0_0RCTAssert(testModule.status == ABI6_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI6_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
