...
#import "ABI26_0_0RCTWebSocketExecutor.h"
...
typedef void (^ABI26_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI26_0_0RCTWebSocketExecutor () <ABI26_0_0RCTSRWebSocketDelegate>

@end

@implementation ABI26_0_0RCTWebSocketExecutor
{
  ABI26_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI26_0_0RCTWSMessageCallback> *_callbacks;
...
{
  if (!_url) {
    NSInteger port = [[[_bridge bundleURL] port] integerValue] ?: ABI26_0_0RCT_METRO_PORT;
  ...
  if (![self connectToProxy]) {
    ...
    NSString *error = [NSString stringWithFormat:@"Connection to %@ timed out. Are you "
                       ...
                       "you have the right IP address in `ABI26_0_0RCTWebSocketExecutor.m`.", _url];
    _setupError = ABI26_0_0RCTErrorWithMessage(error);
    ABI26_0_0RCTFatal(_setupError);
    return;
  }
...
- (BOOL)connectToProxy
{
  ...
  return connected == 0 && _socket.readyState == ABI26_0_0RCTSR_OPEN;
}
...
- (BOOL)prepareJSRuntime
{
  ...
  if (initError) {
    ABI26_0_0RCTLogInfo(@"Websocket runtime setup failed: %@", initError);
  }
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI26_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI26_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
{
  dispatch_semaphore_signal(_socketOpenSemaphore);
  ABI26_0_0RCTLogInfo(@"WebSocket connection failed with error %@", error);
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message onReply:(ABI26_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    if (!self.valid) {
      callback(ABI26_0_0RCTErrorWithMessage(@"Runtime is not ready for debugging. Make sure Packager server is running."), nil);
      return;
    }
    ...
    [self->_socket send:ABI26_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}

- (void)executeApplicationScript:(NSData *)script sourceURL:(NSURL *)URL onComplete:(ABI26_0_0RCTJavaScriptCompleteBlock)onComplete
{
  // Hack: the bridge transitions out of loading state as soon as this method returns, which prevents us
  // from completely invalidating the bridge and preventing an endless barage of ABI26_0_0RCTLog.logIfNoNativeHook
  ...
  NSDictionary<NSString *, id> *message = @{
    ...
    @"url": ABI26_0_0RCTNullIfNil(URL.absoluteString),
  ...
  [self sendMessage:message onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    if (socketError) {
      ...
      onComplete(error ? ABI26_0_0RCTErrorWithMessage(error) : nil);
    }
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI26_0_0RCTJavaScriptCallback)onComplete
{
  ABI26_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  NSDictionary<NSString *, id> *message = @{
    @"method": method,
    @"arguments": arguments
  };
  [self sendMessage:message onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    ...
    NSError *jsonError;
    id result = ABI26_0_0RCTJSONParse(reply[@"result"], &jsonError);
    NSString *error = reply[@"error"];
    onComplete(result, error ? ABI26_0_0RCTErrorWithMessage(error) : jsonError);
  }];
}

- (void)injectJSONText:(NSString *)script asGlobalObjectNamed:(NSString *)objectName callback:(ABI26_0_0RCTJavaScriptCompleteBlock)onComplete
{
  dispatch_async(_jsQueue, ^{
...
{
  ABI26_0_0RCTExecuteOnMainQueue(block);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI26_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI26_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
