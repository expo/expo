...
@interface ABI26_0_0RCTMultilineTextInputView () <ABI26_0_0RCTBackedTextInputDelegate>

@end

@implementation ABI26_0_0RCTMultilineTextInputView
{
  ABI26_0_0RCTUITextView *_backedTextInput;
  ABI26_0_0RCTTextView *_richTextView;
...
- (instancetype)initWithBridge:(ABI26_0_0RCTBridge *)bridge
{
  ...
  if (self = [super initWithBridge:bridge]) {
    ...
    _backedTextInput = [[ABI26_0_0RCTUITextView alloc] initWithFrame:self.bounds];
...
- (void)insertReactABI26_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI26_0_0Subview:subview atIndex:index];

  if ([subview isKindOfClass:[ABI26_0_0RCTTextView class]]) {
    if (_richTextView) {
      ABI26_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI26_0_0RCTTextView *)subview;
...
static NSAttributedString *removeReactABI26_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI26_0_0RCTReactABI26_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI26_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI26_0_0TagFromString(_pendingAttributedText);
...
- (void)setText:(NSString *)text
{
  ...
  if (eventLag == 0 && ![text isEqualToString:_backedTextInput.text]) {
    ...
    ABI26_0_0RCTLogWarn(@"Native TextInput(%@) is %lld events ahead of JS - try to make your JS faster.", self.text, (long long)eventLag);
  }
}
...
- (void)textInputDidChange
{
  ...
  if (!self.ReactABI26_0_0Tag || !_onChange) {
    return;
  }
