...
@interface ABI26_0_0EXBannerView () <FBAdViewDelegate>

@property (nonatomic, strong) FBAdView *adView;

@end

@implementation ABI26_0_0EXBannerView
...
- (void)createViewIfCan
{
  ...
  FBAdView *adView = [[FBAdView alloc] initWithPlacementID:_placementId
                                                    adSize:fbAdSize
                                        rootViewController:ABI26_0_0RCTPresentedViewController()];
...
{
  if (_onAdError) {
    _onAdError(ABI26_0_0RCTJSErrorFromNSError(error));
  } else {
    ABI26_0_0RCTMakeAndLogError(error.localizedDescription, nil, error.userInfo);
  }
}
