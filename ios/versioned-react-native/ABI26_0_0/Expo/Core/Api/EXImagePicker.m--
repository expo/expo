#import "ABI26_0_0EXImagePicker.h"
...
@interface ABI26_0_0EXImagePicker ()
...
@implementation ABI26_0_0EXImagePicker

ABI26_0_0RCT_EXPORT_MODULE(ExponentImagePicker);
...
ABI26_0_0RCT_EXPORT_METHOD(launchCameraAsync:(NSDictionary *)options
                  resolver:(ABI26_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI26_0_0RCTPromiseRejectBlock)reject)
{
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI26_0_0RNImagePickerTargetCamera options:options];
}

ABI26_0_0RCT_EXPORT_METHOD(launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI26_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI26_0_0RCTPromiseRejectBlock)reject)
{
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI26_0_0RNImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI26_0_0RNImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI26_0_0RNImagePickerTarget)target
{
  self.picker = [[UIImagePickerController alloc] init];

  if (target == ABI26_0_0RNImagePickerTargetCamera) {
#if TARGET_IPHONE_SIMULATOR
    self.reject(ABI26_0_0RCTErrorUnspecified, @"Camera not available on simulator", nil);
    return;
#else
...
#endif
  } else { // ABI26_0_0RNImagePickerTargetLibrarySingleImage
          ...
          completionHandler:(void (^)(void))completionHandler
{
  ...
  if ([[imageURL absoluteString] containsString:@"ext=PNG"]) {
    ...
    ABI26_0_0RCTLogWarn(@"Unsupported format of the picked image. Using JPEG instead.");
  }
  ...
  if ([[self.options objectForKey:@"exif"] boolValue]) {
    ...
    if (metadata) {
      ...
      if (assets.count > 0) {
        ...
        ABI26_0_0RCTLogInfo(@"Could not fetch metadata for image %@", [imageURL absoluteString]);
        completionHandler();
      }
    }
  } else {
...
- (void)handleVideoWithInfo:(NSDictionary * _Nonnull)info saveAt:(NSString *)directory updateResponse:(NSMutableDictionary *)response
{
  ...
  if (assets.count > 0) {
    ...
    ABI26_0_0RCTLogInfo(@"Could not fetch metadata for video %@", [videoURL absoluteString]);
  }
  ...
  response[@"type"] = @"video";
  NSError *error = nil;
  NSString *path = [ABI26_0_0EXFileSystem generatePathInDirectory:directory withExtension:@".mov"];
