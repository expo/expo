...
#import <ABI26_0_0React/ABI26_0_0RCTUtils.h>
...
ABI26_0_0EX_EXPORT_SCOPED_MODULE(ExponentPermissions, PermissionsManager);
    ...
    if (![type isEqualToString:@"notifications"] && ![type isEqualToString:@"userFacingNotifications"] &&
      ...
      UIAlertAction *allow = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        if ([_kernelPermissionsServiceDelegate savePermission:systemPermissions ofType:type forExperience:self.experienceId]) {
          ...
          deniedResult[@"status"] = [[weakSelf class] permissionStringForStatus:ABI26_0_0EXPermissionStatusDenied];
...
{
  if ([type isEqualToString:@"notifications"]) {
    return [ABI26_0_0EXRemoteNotificationRequester permissions];
  } else if ([type isEqualToString:@"userFacingNotifications"]) {
    return [ABI26_0_0EXLocalNotificationRequester permissions];
  } else if ([type isEqualToString:@"location"]) {
    return [ABI26_0_0EXLocationRequester permissions];
  } else if ([type isEqualToString:@"camera"]) {
    return [ABI26_0_0EXCameraPermissionRequester permissions];
  } else if ([type isEqualToString:@"contacts"]) {
    return [ABI26_0_0EXContactsRequester permissions];
  } else if ([type isEqualToString:@"audioRecording"]) {
    return [ABI26_0_0EXAudioRecordingPermissionRequester permissions];
  } else if ([type isEqualToString:@"systemBrightness"]) {
    return [ABI26_0_0EXPermissions alwaysGrantedPermissions]; // permission is implicit
  } else if ([type isEqualToString:@"cameraRoll"]) {
    return [ABI26_0_0EXCameraRollRequester permissions];
  } else if ([type isEqualToString:@"calendar"]) {
    return [ABI26_0_0EXCalendarRequester permissions];
  } else if ([type isEqualToString:@"reminders"]) {
    return [ABI26_0_0EXRemindersRequester permissions];
  }
  return nil;
}
...
- (void)showPermissionRequestAlert:(NSString *)type withAllowAction:(UIAlertAction *)allow withDenyAction:(UIAlertAction *)deny
{
  ...
  [ABI26_0_0RCTPresentedViewController() presentViewController:alert animated:YES completion:nil];
}
...
+ (NSString *)typeForRequester:(NSObject<ABI26_0_0EXPermissionRequester> *)requester
{
  if ([requester isKindOfClass:[ABI26_0_0EXLocationRequester class]]) {
    return @"location";
  } else if ([requester isKindOfClass:[ABI26_0_0EXCameraPermissionRequester class]]) {
    return @"camera";
  } else if ([requester isKindOfClass:[ABI26_0_0EXContactsRequester class]]) {
    return @"contacts";
  } else if ([requester isKindOfClass:[ABI26_0_0EXAudioRecordingPermissionRequester class]]) {
    return @"audioRecording";
  } else if ([requester isKindOfClass:[ABI26_0_0EXCameraRollRequester class]]) {
    return @"cameraRoll";
  } else if ([requester isKindOfClass:[ABI26_0_0EXCalendarRequester class]]) {
    return @"calendar";
  } else if ([requester isKindOfClass:[ABI26_0_0EXRemindersRequester class]]) {
    return @"reminders";
  }
  return @"unknown";
}
