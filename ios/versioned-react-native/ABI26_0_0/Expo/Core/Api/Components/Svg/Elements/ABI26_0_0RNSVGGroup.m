...
@implementation ABI26_0_0RNSVGGroup
{
...
- (void)renderGroupTo:(CGContextRef)context
{
    ...
    ABI26_0_0RNSVGSvgView* svg = [self getSvgView];
    [self traverseSubviews:^(ABI26_0_0RNSVGNode *node) {
        if (node.responsible && !svg.responsible) {
            svg.responsible = YES;
        }

        if ([node isKindOfClass:[ABI26_0_0RNSVGRenderable class]]) {
            [(ABI26_0_0RNSVGRenderable*)node mergeProperties:self];
        }

        [node renderTo:context];

        if ([node isKindOfClass:[ABI26_0_0RNSVGRenderable class]]) {
            [(ABI26_0_0RNSVGRenderable*)node resetProperties];
        }
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI26_0_0RNSVGGlyphContext alloc] initWithScale:1 width:width
                                                   height:height];
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event withTransform:(CGAffineTransform)transform
{
    ...
    for (ABI26_0_0RNSVGNode *node in [self.subviews reverseObjectEnumerator]) {
        if (![node isKindOfClass:[ABI26_0_0RNSVGNode class]]) {
            continue;
        }
...
- (void)parseReference
{
    ...
    [self traverseSubviews:^(__kindof ABI26_0_0RNSVGNode *node) {
        [node parseReference];
        return YES;
    }];
}
