...
#define ABI26_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05
#define ABI26_0_0EX_FRAME_ORIENTATION_USE_MANIFEST 0

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI26_0_0EXFrameReactABI26_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI26_0_0EXFrameReactABI26_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI26_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI26_0_0Bridge;

@end

@interface ABI26_0_0EXFrame () <ABI26_0_0RCTInvalidating, ABI26_0_0RCTExceptionsManagerDelegate>

@property (nonatomic, assign) BOOL sourceSet;
@property (nonatomic, assign) BOOL needsReload;

@property (nonatomic, copy) ABI26_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI26_0_0RCTDirectEventBlock onLoadingProgress;
...
@implementation ABI26_0_0EXFrame
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    ABI26_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");

    _appManager = [[unversionedAppManagerClass alloc] initWithEXFrame:self];
    [_appManager setDelegate:self];
    _supportedInterfaceOrientations = ABI26_0_0EX_FRAME_ORIENTATION_USE_MANIFEST;
  }
  return self;
}
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI26_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI26_0_0RootView]) {
    [_appManager ReactABI26_0_0RootView].frame = self.bounds;
  }
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
  if (_supportedInterfaceOrientations != ABI26_0_0EX_FRAME_ORIENTATION_USE_MANIFEST) {
...
#pragma mark - ABI26_0_0EXReactABI26_0_0AppManagerDelegate

- (void)reactAppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI26_0_0RootView = [appManager ReactABI26_0_0RootView];
  ReactABI26_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI26_0_0RootView];
}
...
- (void)reactAppManager:(id)appManager loadedJavaScriptWithProgress:(ABI26_0_0RCTLoadingProgress *)progress;
{
  if (_onLoadingProgress) {
    _onLoadingProgress(@{
      @"status": ABI26_0_0RCTNullIfNil(progress.status),
      @"done": ABI26_0_0RCTNullIfNil(progress.done),
      @"total": ABI26_0_0RCTNullIfNil(progress.total),
    });
  }
}
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI26_0_0RootView] &&
      [_appManager ReactABI26_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI26_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI26_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI26_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI26_0_0RCTErrorWithMessage(message)];
