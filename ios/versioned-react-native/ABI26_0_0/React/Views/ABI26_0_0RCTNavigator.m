...
#import "ABI26_0_0RCTWrapperViewController.h"
#import "UIView+ReactABI26_0_0.h"

typedef NS_ENUM(NSUInteger, ABI26_0_0RCTNavigationLock) {
  ABI26_0_0RCTNavigationLockNone,
  ABI26_0_0RCTNavigationLockNative,
  ABI26_0_0RCTNavigationLockJavaScript
};

// By default the interactive pop gesture will be enabled when the navigation bar is displayed
// and disabled when hidden
// ABI26_0_0RCTPopGestureStateDefault maps to the default behavior (mentioned above). Once popGestureState
...
// back to it.
// ABI26_0_0RCTPopGestureStateEnabled will enable the gesture independent of nav bar visibility
// ABI26_0_0RCTPopGestureStateDisabled will disable the gesture independent of nav bar visibility
typedef NS_ENUM(NSUInteger, ABI26_0_0RCTPopGestureState) {
  ABI26_0_0RCTPopGestureStateDefault = 0,
  ABI26_0_0RCTPopGestureStateEnabled,
  ABI26_0_0RCTPopGestureStateDisabled
};
...
@interface ABI26_0_0RCTNavigationController : UINavigationController <UINavigationBarDelegate>
{
  dispatch_block_t _scrollCallback;
}

@property (nonatomic, assign) ABI26_0_0RCTNavigationLock navigationLock;

@end

/**
 * In general, `ABI26_0_0RCTNavigator` examines `_currentViews` (which are ReactABI26_0_0 child
 ...
 * ahead by adding/removing ReactABI26_0_0 subviews. Native gets ahead by swiping back,
 * or tapping the back button. In both cases, the other system is initially
 * unaware. And in both cases, `ABI26_0_0RCTNavigator` helps the other side "catch up".
 *
 * If `ABI26_0_0RCTNavigator` sees the number of ReactABI26_0_0 children have changed, it
 * pushes/pops accordingly. If `ABI26_0_0RCTNavigator` sees a `UIKit` driven push/pop, it
 ...
 *
 */
@implementation ABI26_0_0RCTNavigationController

/**
 * @param callback Callback that is invoked when a "scroll" interaction begins
 * so that `ABI26_0_0RCTNavigator` can notify `JavaScript`.
 */
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
  ...
  if (self.interactivePopGestureRecognizer.state == UIGestureRecognizerStateBegan) {
    if (self.navigationLock == ABI26_0_0RCTNavigationLockNone) {
      self.navigationLock = ABI26_0_0RCTNavigationLockNative;
      if (_scrollCallback) {
        _scrollCallback();
      }
    } else if (self.navigationLock == ABI26_0_0RCTNavigationLockJavaScript) {
      // This should never happen because we disable/enable the gesture
      // recognizer when we lock the navigation.
      ABI26_0_0RCTAssert(NO, @"Should never receive gesture start while JS locks navigator");
    }
  } else
#endif //TARGET_OS_TV
  {
    if (self.navigationLock == ABI26_0_0RCTNavigationLockNone) {
      ...
      // length (`currentReactABI26_0_0Count` - 1).
...
@interface ABI26_0_0RCTNavigator() <ABI26_0_0RCTWrapperViewControllerNavigationListener, UINavigationControllerDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, copy) ABI26_0_0RCTDirectEventBlock onNavigationProgress;
@property (nonatomic, copy) ABI26_0_0RCTBubblingEventBlock onNavigationComplete;

@property (nonatomic, assign) NSInteger previousRequestedTopOfStack;

@property (nonatomic, assign) ABI26_0_0RCTPopGestureState popGestureState;

// Previous views are only mainted in order to detect incorrect
// addition/removal of views below the `requestedTopOfStack`
@property (nonatomic, copy, readwrite) NSArray<ABI26_0_0RCTNavItem *> *previousViews;
@property (nonatomic, readwrite, strong) ABI26_0_0RCTNavigationController *navigationController;
/**
 ...
 * - We use `invalidate` to remove the `ABI26_0_0RCTNavigator`'s reference to the
 ...
 * remove it from the run loop when the view will be destroyed by ReactABI26_0_0.
 *
 * +----------+              +--------------+
...
@implementation ABI26_0_0RCTNavigator
{
...
- (instancetype)initWithBridge:(ABI26_0_0RCTBridge *)bridge
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _previousViews = @[];
    __weak ABI26_0_0RCTNavigator *weakSelf = self;
    _navigationController = [[ABI26_0_0RCTNavigationController alloc] initWithScrollCallback:^{
      [weakSelf dispatchFakeScrollEvent];
    }];
    _navigationController.delegate = self;
    ABI26_0_0RCTAssert([self requestSchedulingJavaScriptNavigation], @"Could not acquire JS navigation lock on init");
...
- (void)didUpdateFrame:(__unused ABI26_0_0RCTFrameUpdate *)update
{
...
- (void)setInteractivePopGestureEnabled:(BOOL)interactivePopGestureEnabled
{
  ...
  _popGestureState = interactivePopGestureEnabled ? ABI26_0_0RCTPopGestureStateEnabled : ABI26_0_0RCTPopGestureStateDisabled;
#endif
}
                    ...
                    animated:(__unused BOOL)animated
{
  ...
  [tc animateAlongsideTransition: ^(id<UIViewControllerTransitionCoordinatorContext> context) {
    ABI26_0_0RCTWrapperViewController *fromController =
      (ABI26_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey];
    ABI26_0_0RCTWrapperViewController *toController =
      (ABI26_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    NSUInteger indexOfFrom = [self.ReactABI26_0_0Subviews indexOfObject:fromController.navItem];
    NSUInteger indexOfTo = [self.ReactABI26_0_0Subviews indexOfObject:toController.navItem];
...
{
  if (_navigationController.navigationLock == ABI26_0_0RCTNavigationLockNone) {
    _navigationController.navigationLock = ABI26_0_0RCTNavigationLockJavaScript;
#if !TARGET_OS_TV
...
- (void)freeLock
{
  _navigationController.navigationLock = ABI26_0_0RCTNavigationLockNone;

  // Unless the pop gesture has been explicitly disabled (ABI26_0_0RCTPopGestureStateDisabled),
  // Set interactivePopGestureRecognizer.enabled to YES
  // If the popGestureState is ABI26_0_0RCTPopGestureStateDefault the default behavior will be maintained
#if !TARGET_OS_TV
  _navigationController.interactivePopGestureRecognizer.enabled = self.popGestureState != ABI26_0_0RCTPopGestureStateDisabled;
#endif
}

/**
 * A ReactABI26_0_0 subview can be inserted/removed at any time, however if the
 * `requestedTopOfStack` changes, there had better be enough subviews present
 * to satisfy the push/pop.
 */
- (void)insertReactABI26_0_0Subview:(ABI26_0_0RCTNavItem *)view atIndex:(NSInteger)atIndex
{
  ABI26_0_0RCTAssert([view isKindOfClass:[ABI26_0_0RCTNavItem class]], @"ABI26_0_0RCTNavigator only accepts ABI26_0_0RCTNavItem subviews");
  ABI26_0_0RCTAssert(
    _navigationController.navigationLock == ABI26_0_0RCTNavigationLockJavaScript,
    @"Cannot change subviews from JS without first locking."
  );
  [super insertReactABI26_0_0Subview:view atIndex:atIndex];
}
...
- (void)layoutSubviews
{
  ...
  [self ReactABI26_0_0AddControllerToClosestParent:_navigationController];
  _navigationController.view.frame = self.bounds;
}

- (void)removeReactABI26_0_0Subview:(ABI26_0_0RCTNavItem *)subview
{
  if (self.ReactABI26_0_0Subviews.count <= 0 || subview == self.ReactABI26_0_0Subviews[0]) {
    ABI26_0_0RCTLogError(@"Attempting to remove invalid ABI26_0_0RCT subview of ABI26_0_0RCTNavigator");
    return;
  }
...
{
  [_bridge.eventDispatcher sendFakeScrollEvent:self.ReactABI26_0_0Tag];
}
 ...
 */
- (UIView *)ReactABI26_0_0Superview
{
  ABI26_0_0RCTAssert(!_bridge.isValid || self.superview != nil, @"put ReactABI26_0_0NavSuperviewLink back");
  UIView *superview = [super ReactABI26_0_0Superview];
  return superview ?: self.ReactABI26_0_0NavSuperviewLink;
}
...
- (void)uiManagerDidPerformMounting
{
  ...
  // The "ReactABI26_0_0 count" is the count of views that are visible on the navigation
  // stack.  There may be more beyond this - that aren't visible, and may be
  // deleted/purged soon.
  NSUInteger previousReactABI26_0_0Count =
    _previousRequestedTopOfStack == kNeverRequested ? 0 : _previousRequestedTopOfStack + 1;
  NSUInteger currentReactABI26_0_0Count = _requestedTopOfStack + 1;

  BOOL jsGettingAhead =
    //    ----- previously caught up ------          ------ no longer caught up -------
    viewControllerCount == previousReactABI26_0_0Count && currentReactABI26_0_0Count != viewControllerCount;
  BOOL jsCatchingUp =
    //    --- previously not caught up ----          --------- now caught up ----------
    viewControllerCount != previousReactABI26_0_0Count && currentReactABI26_0_0Count == viewControllerCount;
  BOOL jsMakingNoProgressButNeedsToCatchUp =
    //    --- previously not caught up ----          ------- still the same -----------
    viewControllerCount != previousReactABI26_0_0Count && currentReactABI26_0_0Count == previousReactABI26_0_0Count;
...
BOOL jsGettingtooSlow =
  ...
  BOOL ReactABI26_0_0PushOne = jsGettingAhead && currentReactABI26_0_0Count == previousReactABI26_0_0Count + 1;
  BOOL ReactABI26_0_0PopN = jsGettingAhead && currentReactABI26_0_0Count < previousReactABI26_0_0Count;
  ...
  if (!(jsGettingAhead ||
        ...
        jsGettingtooSlow)) {
    ABI26_0_0RCTLogError(@"JS has only made partial progress to catch up to UIKit");
  }
  if (currentReactABI26_0_0Count > self.ReactABI26_0_0Subviews.count) {
    ABI26_0_0RCTLogError(@"Cannot adjust current top of stack beyond available views");
  }

  // Views before the previous ReactABI26_0_0 count must not have changed. Views greater than previousReactABI26_0_0Count
  // up to currentReactABI26_0_0Count may have changed.
  for (NSUInteger i = 0; i < MIN(self.ReactABI26_0_0Subviews.count, MIN(_previousViews.count, previousReactABI26_0_0Count)); i++) {
    if (self.ReactABI26_0_0Subviews[i] != _previousViews[i]) {
      ABI26_0_0RCTLogError(@"current view should equal previous view");
    }
  }
  if (currentReactABI26_0_0Count < 1) {
    ABI26_0_0RCTLogError(@"should be at least one current view");
  }
  if (jsGettingAhead) {
    if (ReactABI26_0_0PushOne) {
      UIView *lastView = self.ReactABI26_0_0Subviews.lastObject;
      ABI26_0_0RCTWrapperViewController *vc = [[ABI26_0_0RCTWrapperViewController alloc] initWithNavItem:(ABI26_0_0RCTNavItem *)lastView];
      vc.navigationListener = self;
      _numberOfViewControllerMovesToIgnore = 1;
      [_navigationController pushViewController:vc animated:(currentReactABI26_0_0Count > 1)];
    } else if (ReactABI26_0_0PopN) {
      UIViewController *viewControllerToPopTo = _navigationController.viewControllers[(currentReactABI26_0_0Count - 1)];
      _numberOfViewControllerMovesToIgnore = viewControllerCount - currentReactABI26_0_0Count;
      [_navigationController popToViewController:viewControllerToPopTo animated:YES];
    } else {
      ABI26_0_0RCTLogError(@"Pushing or popping more than one view at a time from JS");
    }
  ...
  // otherwise we would unnecessarily retain a reference to view(s) no longer on the ReactABI26_0_0 navigation stack:
  NSUInteger expectedCount = MIN(currentReactABI26_0_0Count, self.ReactABI26_0_0Subviews.count);
  _previousViews = [[self.ReactABI26_0_0Subviews subarrayWithRange: NSMakeRange(0, expectedCount)] copy];
...
didMoveToNavigationController:(UINavigationController *)navigationController
{
  ...
  ABI26_0_0RCTAssert(_navigationController.navigationLock == ABI26_0_0RCTNavigationLockJavaScript ||
