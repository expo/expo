...
@interface ABI26_0_0RCTScrollEvent : NSObject <ABI26_0_0RCTEvent>
...
@implementation ABI26_0_0RCTScrollEvent
{
...
{
  ABI26_0_0RCTAssertParam(ReactABI26_0_0Tag);
...
- (ABI26_0_0RCTScrollEvent *)coalesceWithEvent:(ABI26_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI26_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI26_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI26_0_0RCTRefreshControl *rctRefreshControl;
...
@implementation ABI26_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI26_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI26_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI26_0_0RCTUIManager JSResponder] isKindOfClass:[ABI26_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI26_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI26_0_0RCTRefreshControl *)refreshControl
{
...
- (instancetype)initWithEventDispatcher:(ABI26_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI26_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI26_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
- (void)setReactABI26_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
...
- (void)insertReactABI26_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertReactABI26_0_0Subview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI26_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI26_0_0RCTRefreshControl *)view];
  } else
#endif
  {
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI26_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI26_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI26_0_0Subview:atIndex:`
}
...
- (void)layoutSubviews
{
  ...
  ABI26_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI26_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  ABI26_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI26_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI26_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI26_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI26_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI26_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI26_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI26_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI26_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI26_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI26_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI26_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
...
- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    ...
    [[_contentView ReactABI26_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI26_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI26_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI26_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI26_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI26_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI26_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI26_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI26_0_0RCTView contentInsetsForView:self];
...
#define ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI26_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI26_0_0RCTScrollEvent *scrollEvent = [[ABI26_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ReactABI26_0_0Tag:self.ReactABI26_0_0Tag
...
@implementation ABI26_0_0RCTEventDispatcher (ABI26_0_0RCTScrollView)
...
- (void)sendFakeScrollEvent:(NSNumber *)ReactABI26_0_0Tag
{
  ...
  ABI26_0_0RCTScrollEvent *fakeScrollEvent = [[ABI26_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI26_0_0Tag:ReactABI26_0_0Tag
