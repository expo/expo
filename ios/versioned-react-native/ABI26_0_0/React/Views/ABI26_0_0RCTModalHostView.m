...
@implementation ABI26_0_0RCTModalHostView
{
  __weak ABI26_0_0RCTBridge *_bridge;
  BOOL _isPresented;
  ABI26_0_0RCTModalHostViewController *_modalViewController;
  ABI26_0_0RCTTouchHandler *_touchHandler;
  UIView *_ReactABI26_0_0Subview;
#if TARGET_OS_TV
...
ABI26_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:coder)

- (instancetype)initWithBridge:(ABI26_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _modalViewController = [ABI26_0_0RCTModalHostViewController new];
    UIView *containerView = [UIView new];
    containerView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _modalViewController.view = containerView;
    _touchHandler = [[ABI26_0_0RCTTouchHandler alloc] initWithBridge:bridge];
#if TARGET_OS_TV
...
- (void)setOnRequestClose:(ABI26_0_0RCTDirectEventBlock)onRequestClose
{
  _onRequestClose = onRequestClose;
  if (_ReactABI26_0_0Subview) {
    if (_onRequestClose && _menuButtonGestureRecognizer) {
      [_ReactABI26_0_0Subview addGestureRecognizer:_menuButtonGestureRecognizer];
    } else {
      [_ReactABI26_0_0Subview removeGestureRecognizer:_menuButtonGestureRecognizer];
    }
  }
}
#endif
...
{
  if (_ReactABI26_0_0Subview && _isPresented) {
    [_bridge.uiManager setSize:newBounds.size forView:_ReactABI26_0_0Subview];
...
- (void)notifyForOrientationChange
{
  ...
  UIInterfaceOrientation currentOrientation = [ABI26_0_0RCTSharedApplication() statusBarOrientation];
...
{
  ABI26_0_0RCTAssert(_ReactABI26_0_0Subview == nil, @"Modal view can only have one subview");
  ...
  for (NSString *key in [self.tvRemoteHandler.tvRemoteGestureRecognizers allKeys]) {
    if (![key isEqualToString:ABI26_0_0RCTTVRemoteEventMenu]) {
  ...
  _ReactABI26_0_0Subview = subview;
}

- (void)removeReactABI26_0_0Subview:(UIView *)subview
{
  ABI26_0_0RCTAssert(subview == _ReactABI26_0_0Subview, @"Cannot remove view other than modal view");
...
#endif
  _ReactABI26_0_0Subview = nil;
}

- (void)didUpdateReactABI26_0_0Subviews
{
  // Do nothing, as subview (singular) is managed by `insertReactABI26_0_0Subview:atIndex:`
}
...
- (void)didMoveToWindow
{
  ...
  if (!self.userInteractionEnabled && ![self.superview.ReactABI26_0_0Subviews containsObject:self]) {
    return;
  }

  if (!_isPresented && self.window) {
    ABI26_0_0RCTAssert(self.ReactABI26_0_0ViewController, @"Can't present modal view controller without a presenting view controller");
