...
static_assert(
    ...
    "ABI36_0_0facebook::yoga::detail::CompactValue only works with IEEE754 floats");
...
// This class stores ABI36_0_0YGValue in 32 bits.
...
public:
  ...
  template <ABI36_0_0YGUnit Unit>
  static CompactValue of(float value) noexcept {
    if (value == 0.0f || (value < LOWER_BOUND && value > -LOWER_BOUND)) {
      constexpr auto zero =
          Unit == ABI36_0_0YGUnitPercent ? ZERO_BITS_PERCENT : ZERO_BITS_POINT;
      return {Payload{zero}};
    }

    constexpr auto upperBound =
        Unit == ABI36_0_0YGUnitPercent ? UPPER_BOUND_PERCENT : UPPER_BOUND_POINT;
    if (value > upperBound || value < -upperBound) {
      value = copysignf(upperBound, value);
    }

    uint32_t unitBit = Unit == ABI36_0_0YGUnitPercent ? PERCENT_BIT : 0;
  ...
  CompactValue(const ABI36_0_0YGValue& x) noexcept : payload_(uint32_t{0}) {
    switch (x.unit) {
      ...
      case ABI36_0_0YGUnitPoint:
        *this = of<ABI36_0_0YGUnitPoint>(x.value);
        break;
      case ABI36_0_0YGUnitPercent:
        *this = of<ABI36_0_0YGUnitPercent>(x.value);
        break;
    }
  }

  operator ABI36_0_0YGValue() const noexcept {
    switch (payload_.repr) {
      case AUTO_BITS:
        return ABI36_0_0YGValueAuto;
      case ZERO_BITS_POINT:
        return ABI36_0_0YGValue{0.0f, ABI36_0_0YGUnitPoint};
      case ZERO_BITS_PERCENT:
        return ABI36_0_0YGValue{0.0f, ABI36_0_0YGUnitPercent};
    }
    ...
    return ABI36_0_0YGValue{data.value,
                   payload_.repr & 0x40000000 ? ABI36_0_0YGUnitPercent : ABI36_0_0YGUnitPoint};
  }
...
template <>
CompactValue CompactValue::of<ABI36_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::of<ABI36_0_0YGUnitAuto>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI36_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI36_0_0YGUnitAuto>(float) noexcept = delete;
