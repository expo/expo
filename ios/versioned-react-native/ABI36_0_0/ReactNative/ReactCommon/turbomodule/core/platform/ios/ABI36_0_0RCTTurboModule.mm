...
#import <ABI36_0_0ReactCommon/ABI36_0_0LongLivedObject.h>
#import <ABI36_0_0ReactCommon/ABI36_0_0TurboModule.h>
#import <ABI36_0_0ReactCommon/ABI36_0_0TurboModuleUtils.h>
...
static ABI36_0_0RCTResponseSenderBlock convertJSIFunctionToCallback(
    jsi::Runtime &runtime,
...
static ABI36_0_0RCTResponseSenderBlock convertJSIFunctionToCallback(
    ...
    std::shared_ptr<ABI36_0_0React::JSCallInvoker> jsInvoker)
{
  __block auto wrapper = std::make_shared<ABI36_0_0React::CallbackWrapper>(value.getFunction(runtime), runtime, jsInvoker);
  return ^(NSArray *responses) {
    if (wrapper == nullptr) {
      throw std::runtime_error("callback arg cannot be called more than once");
    }

    std::shared_ptr<ABI36_0_0React::CallbackWrapper> rw = wrapper;
...
struct PromiseWrapper : public ABI36_0_0React::LongLivedObject {
      ...
      std::shared_ptr<ABI36_0_0React::JSCallInvoker> jsInvoker)
  {
    ...
    ABI36_0_0React::LongLivedObjectCollection::get().add(instance);
    return instance;
  }
  ...
  PromiseWrapper(
      ...
      jsi::Runtime &runtime,
      std::shared_ptr<ABI36_0_0React::JSCallInvoker> jsInvoker)
      : resolveWrapper(std::make_shared<ABI36_0_0React::CallbackWrapper>(std::move(resolve), runtime, jsInvoker)),
        rejectWrapper(std::make_shared<ABI36_0_0React::CallbackWrapper>(std::move(reject), runtime, jsInvoker)),
        runtime(runtime),
        jsInvoker(jsInvoker)
  {
  }

  ABI36_0_0RCTPromiseResolveBlock resolveBlock()
  {
    return ^(id result) {
      ...
      std::shared_ptr<ABI36_0_0React::CallbackWrapper> retainedWrapper = resolveWrapper;
  ...
  ABI36_0_0RCTPromiseRejectBlock rejectBlock()
  {
    return ^(NSString *code, NSString *message, NSError *error) {
      ...
      // Retain the resolveWrapper so that it stays alive inside the lambda.
      std::shared_ptr<ABI36_0_0React::CallbackWrapper> retainedWrapper = rejectWrapper;
      NSDictionary *jsError = ABI36_0_0RCTJSErrorFromCodeMessageAndNSError(code, message, error);
  ...
  std::shared_ptr<ABI36_0_0React::CallbackWrapper> resolveWrapper;
  std::shared_ptr<ABI36_0_0React::CallbackWrapper> rejectWrapper;
  jsi::Runtime &runtime;
  std::shared_ptr<ABI36_0_0React::JSCallInvoker> jsInvoker;
};
...
static jsi::Value
createPromise(jsi::Runtime &runtime, std::shared_ptr<ABI36_0_0React::JSCallInvoker> jsInvoker, PromiseInvocationBlock invoke)
{
  if (!invoke) {
    return jsi::Value::undefined();
  }
 ...
 * This serves as a backward-compatible support for ABI36_0_0RCTBridgeModule's methodQueue API.
 *
 * In the future:
    ...
    NSMutableArray *retainedObjectsForInvocation)
{
  ...
  if ([module conformsToProtocol:@protocol(ABI36_0_0RCTBridgeModule)] && [module respondsToSelector:@selector(methodQueue)]) {
    methodQueue = [module performSelector:@selector(methodQueue)];
  }

  if (methodQueue == NULL || methodQueue == ABI36_0_0RCTJSThread) {
    ...
    if (valueKind == VoidKind) {
      ...
      ABI36_0_0RCTUnsafeExecuteOnMainQueueSync(block);
    }
  } else {
    if (valueKind == VoidKind) {
  ...
  if (!methodArgumentTypeNames_) {
    ...
    if (methods) {
      for (unsigned int i = 0; i < numberOfMethods; i++) {
        ...
        ABI36_0_0RCTMethodInfo *(*getMethodInfo)(id, SEL) = (__typeof__(getMethodInfo))objc_msgSend;
        ABI36_0_0RCTMethodInfo *methodInfo = getMethodInfo(cls, s);

        NSArray<ABI36_0_0RCTMethodArgument *> *arguments;
        NSString *otherMethodName = ABI36_0_0RCTParseMethodSignature(methodInfo->objcName, &arguments);
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (arg->isNumber()) {
      ...
      if (objCArgType[0] == _C_ID) {
        ...
        /**
         * Convert numbers using ABI36_0_0RCTConvert if possible.
         */
        ...
        if (argumentType != nil) {
          ...
          if ([ABI36_0_0RCTConvert respondsToSelector:rctConvertSelector]) {
            // Message dispatch logic from old infra
            id (*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
            id convertedObjCArg = convert([ABI36_0_0RCTConvert class], rctConvertSelector, objCArg);
    ...
    if (objCArg) {
      ...
      /**
       * Convert objects using ABI36_0_0RCTConvert.
       */
      if (objCArgType[0] == _C_ID) {
        NSString *argumentType = getArgumentTypeName(methodNameNSString, i);
        if (argumentType != nil) {
      ...
      /**
       * Convert objects using ABI36_0_0RCTCxxConvert to structs.
       */
      if ([objCArg isKindOfClass:[NSDictionary class]] && hasMethodArgConversionSelector(methodNameNSString, i)) {
        ...
        ABI36_0_0RCTManagedPointer *box = convert([ABI36_0_0RCTCxxConvert class], methodArgConversionSelector, objCArg);
    ...
    size_t count)
{
  ...
  if (valueKind == PromiseKind) {
    ...
    return createPromise(runtime, jsInvoker_, ^(jsi::Runtime &rt, std::shared_ptr<PromiseWrapper> wrapper) {
      ABI36_0_0RCTPromiseResolveBlock resolveBlock = wrapper->resolveBlock();
      ABI36_0_0RCTPromiseRejectBlock rejectBlock = wrapper->rejectBlock();
