...
#include <ABI36_0_0React/textlayoutmanager/ABI36_0_0RCTTextPrimitivesConversions.h>

using namespace ABI36_0_0facebook::ABI36_0_0React;

@implementation ABI36_0_0RCTWeakEventEmitterWrapper {
...
inline static UIFontWeight ABI36_0_0RCTUIFontWeightFromInteger(NSInteger fontWeight)
{
...
inline static UIFont *ABI36_0_0RCTEffectiveFontFromTextAttributes(const TextAttributes &textAttributes)
{
  ...
  fontProperties.style = textAttributes.fontStyle.hasValue()
      ? ABI36_0_0RCTFontStyleFromFontStyle(textAttributes.fontStyle.value())
      : ABI36_0_0RCTFontStyleUndefined;
  fontProperties.variant = textAttributes.fontVariant.hasValue()
      ? ABI36_0_0RCTFontVariantFromFontVariant(textAttributes.fontVariant.value())
      : ABI36_0_0RCTFontVariantUndefined;
  fontProperties.weight = textAttributes.fontWeight.hasValue()
      ? ABI36_0_0RCTUIFontWeightFromInteger((NSInteger)textAttributes.fontWeight.value())
      : NAN;
  fontProperties.sizeMultiplier = textAttributes.fontSizeMultiplier;

  return ABI36_0_0RCTFontWithFontProperties(fontProperties);
}

inline static CGFloat ABI36_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(const TextAttributes &textAttributes)
{
...
inline static UIColor *ABI36_0_0RCTEffectiveForegroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveForegroundColor = ABI36_0_0RCTUIColorFromSharedColor(textAttributes.foregroundColor) ?: [UIColor blackColor];
...
inline static UIColor *ABI36_0_0RCTEffectiveBackgroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveBackgroundColor = ABI36_0_0RCTUIColorFromSharedColor(textAttributes.backgroundColor);
...
static NSDictionary<NSAttributedStringKey, id> *ABI36_0_0RCTNSTextAttributesFromTextAttributes(
    const TextAttributes &textAttributes)
{
  NSMutableDictionary<NSAttributedStringKey, id> *attributes = [NSMutableDictionary dictionaryWithCapacity:10];

  // Font
  UIFont *font = ABI36_0_0RCTEffectiveFontFromTextAttributes(textAttributes);
  if (font) {
    attributes[NSFontAttributeName] = font;
  }

  // Colors
  UIColor *effectiveForegroundColor = ABI36_0_0RCTEffectiveForegroundColorFromTextAttributes(textAttributes);
  ...
  if (textAttributes.backgroundColor || !isnan(textAttributes.opacity)) {
    attributes[NSBackgroundColorAttributeName] = ABI36_0_0RCTEffectiveBackgroundColorFromTextAttributes(textAttributes);
  }
  ...
  if (textAttributes.alignment.hasValue()) {
    ...
    paragraphStyle.alignment = ABI36_0_0RCTNSTextAlignmentFromTextAlignment(textAlignment);
    isParagraphStyleUsed = YES;
  }

  if (textAttributes.baseWritingDirection.hasValue()) {
    paragraphStyle.baseWritingDirection =
        ABI36_0_0RCTNSWritingDirectionFromWritingDirection(textAttributes.baseWritingDirection.value());
    isParagraphStyleUsed = YES;
  }

  if (!isnan(textAttributes.lineHeight)) {
    CGFloat lineHeight = textAttributes.lineHeight * ABI36_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(textAttributes);
  ...
  if (textAttributes.textDecorationLineType.value_or(TextDecorationLineType::None) != TextDecorationLineType::None) {
    ...
    NSUnderlineStyle style = ABI36_0_0RCTNSUnderlineStyleFromStyleAndPattern(
    ...
    UIColor *textDecorationColor = ABI36_0_0RCTUIColorFromSharedColor(textAttributes.textDecorationColor);
  ...
  if (textAttributes.textShadowOffset.hasValue()) {
    ...
    shadow.shadowColor = ABI36_0_0RCTUIColorFromSharedColor(textAttributes.textShadowColor);
    attributes[NSShadowAttributeName] = shadow;
  }

  // Special
  if (textAttributes.isHighlighted) {
    attributes[ABI36_0_0RCTAttributedStringIsHighlightedAttributeName] = @YES;
  }

  return [attributes copy];
}

NSAttributedString *ABI36_0_0RCTNSAttributedStringFromAttributedString(const AttributedString &attributedString)
{
  ...
  for (auto fragment : attributedString.getFragments()) {
    ...
    if (layoutMetrics != EmptyLayoutMetrics) {
      ...
      nsAttributedStringFragment =
          [[NSAttributedString alloc] initWithString:string
                                          attributes:ABI36_0_0RCTNSTextAttributesFromTextAttributes(fragment.textAttributes)];
    }
    ...
    if (fragment.parentShadowView.componentHandle) {
      ABI36_0_0RCTWeakEventEmitterWrapper *eventEmitterWrapper = [ABI36_0_0RCTWeakEventEmitterWrapper new];
      eventEmitterWrapper.eventEmitter = fragment.parentShadowView.eventEmitter;

      NSDictionary<NSAttributedStringKey, id> *additionalTextAttributes =
          @{ABI36_0_0RCTAttributedStringEventEmitterKey : eventEmitterWrapper};
