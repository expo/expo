...
@implementation ABI36_0_0RCTTextShadowView
{
...
- (instancetype)initWithBridge:(ABI36_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI36_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI36_0_0RCTTextShadowViewMeasure);
    ABI36_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI36_0_0RCTTextShadowViewBaseline);
  }
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  // because the background color is drawn on the ABI36_0_0RCTTextView itself, as well
  // as on the glphy background draw step. By setting this to nil, we allow
  // the ABI36_0_0RCTTextView backgroundColor to be used, without affecting nested Text
  // components.
...
- (void)uiManagerWillPerformMounting
{
  ...
  [textStorage enumerateAttribute:ABI36_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:
   ^(ABI36_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
     if (!shadowView) {
       return;
     }

     [descendantViewTags addObject:shadowView.ABI36_0_0ReactTag];
   }
  ];

  [_bridge.uiManager addUIBlock:^(ABI36_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI36_0_0RCTTextView *textView = (ABI36_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI36_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName range:NSMakeRange(0, textStorage.length)];
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI36_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                               inRange:NSMakeRange(0, attributedText.length)
                               options:0
                            usingBlock:
    ^(ABI36_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                                        ...
                                        exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage abi36_0_0_rct_scaleFontSizeToFitSize:size
...
- (void)layoutSubviewsWithContext:(ABI36_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage enumerateAttribute:ABI36_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          ...
                          options:0
                       usingBlock:
    ^(ABI36_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
      if (!shadowView) {
        return;
      }
      ...
      CGRect frame = {{
        ABI36_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI36_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)
      }, {
        ABI36_0_0RCTRoundPixelValue(attachmentSize.width),
        ABI36_0_0RCTRoundPixelValue(attachmentSize.height)
      }};
      
      NSRange truncatedGlyphRange = [layoutManager truncatedGlyphRangeInLineFragmentForGlyphAtIndex:range.location];
      BOOL viewIsTruncated = NSIntersectionRange(range, truncatedGlyphRange).length != 0;

      ABI36_0_0RCTLayoutContext localLayoutContext = layoutContext;
      ...
      ABI36_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
      localLayoutMetrics.frame.origin = frame.origin; // Reinforcing a proper frame origin for the Shadow View.
      if (viewIsTruncated) {
        localLayoutMetrics.displayType = ABI36_0_0RCTDisplayTypeNone;
      }
...
static ABI36_0_0YGSize ABI36_0_0RCTTextShadowViewMeasure(ABI36_0_0YGNodeRef node, float width, ABI36_0_0YGMeasureMode widthMode, float height, ABI36_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
    widthMode == ABI36_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI36_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    heightMode == ABI36_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI36_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI36_0_0RCTTextShadowView *shadowTextView = (__bridge ABI36_0_0RCTTextShadowView *)ABI36_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
    MIN(ABI36_0_0RCTCeilPixelValue(size.width), maximumSize.width),
    MIN(ABI36_0_0RCTCeilPixelValue(size.height), maximumSize.height)
  };
  ...
  return (ABI36_0_0YGSize){
    ABI36_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
    ABI36_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)
  };
}

static float ABI36_0_0RCTTextShadowViewBaseline(ABI36_0_0YGNodeRef node, const float width, const float height)
{
  ABI36_0_0RCTTextShadowView *shadowTextView = (__bridge ABI36_0_0RCTTextShadowView *)ABI36_0_0YGNodeGetContext(node);
