...
#import "ABI36_0_0RCTSurfaceBackedComponent.h"
...
@implementation ABI36_0_0RCTSurfaceBackedComponent

+ (id)initialState
{
  return [ABI36_0_0RCTSurfaceBackedComponentState new];
}
                      ...
                      options:(ABI36_0_0RCTSurfaceHostingComponentOptions)options
{
  ...
  ABI36_0_0RCTSurfaceBackedComponentState *state = scope.state();

  if (state.surface == nil || ![state.surface.moduleName isEqualToString:moduleName]) {
    ABI36_0_0RCTSurface *surface =
      [[ABI36_0_0RCTSurface alloc] initWithBridge:bridge
                              moduleName:moduleName
                       initialProperties:properties];

    [surface start];

    state = [ABI36_0_0RCTSurfaceBackedComponentState newWithSurface:surface];
  ...
  ABI36_0_0RCTSurfaceHostingComponent *surfaceHostingComponent =
    [ABI36_0_0RCTSurfaceHostingComponent newWithSurface:state.surface
                                       options:options];

  CKComponent *component;
  if (options.activityIndicatorComponentFactory == nil || ABI36_0_0RCTSurfaceStageIsRunning(state.surface.stage)) {
