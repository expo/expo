...
@interface ABI36_0_0RCTSpringAnimation ()
...
const NSTimeInterval ABI36_0_0MAX_DELTA_TIME = 0.064;

@implementation ABI36_0_0RCTSpringAnimation
{
  CGFloat _toValue;
  CGFloat _fromValue;
                  ...
                  callBack:(nullable ABI36_0_0RCTResponseSenderBlock)callback
{
  if ((self = [super init])) {
    ...
    _lastVelocity = [ABI36_0_0RCTConvert CGFloat:config[@"initialVelocity"]];
...
- (void)resetAnimationConfig:(NSDictionary *)config
{
  ...
  _toValue = [ABI36_0_0RCTConvert CGFloat:config[@"toValue"]];
  _overshootClamping = [ABI36_0_0RCTConvert BOOL:config[@"overshootClamping"]];
  _restDisplacementThreshold = [ABI36_0_0RCTConvert CGFloat:config[@"restDisplacementThreshold"]];
  _restSpeedThreshold = [ABI36_0_0RCTConvert CGFloat:config[@"restSpeedThreshold"]];
  _stiffness = [ABI36_0_0RCTConvert CGFloat:config[@"stiffness"]];
  _damping = [ABI36_0_0RCTConvert CGFloat:config[@"damping"]];
  _mass = [ABI36_0_0RCTConvert CGFloat:config[@"mass"]];
...
- (void)stepAnimationWithTime:(NSTimeInterval)currentTime
{
  ...
  if(_animationStartTime == -1) {
    ...
    // Handle frame drops, and only advance dt by a max of ABI36_0_0MAX_DELTA_TIME
    NSTimeInterval deltaTime = MIN(ABI36_0_0MAX_DELTA_TIME, currentTime - _animationCurrentTime);
    _t = _t + deltaTime / ABI36_0_0RCTAnimationDragCoefficient();
  }
