...
static NSString *const kOpenURLNotification = @"ABI36_0_0RCTOpenURLNotification";
...
@implementation ABI36_0_0RCTLinkingManager
                  ...
                  reject:(ABI36_0_0RCTPromiseRejectBlock)reject)
{
  if (@available(iOS 10.0, *)) {
    ...
    BOOL opened = [ABI36_0_0RCTSharedApplication() openURL:URL];
    if (opened) {
      resolve(@YES);
    } else {
      reject(ABI36_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@", URL], nil);
                  ...
                  reject:(__unused ABI36_0_0RCTPromiseRejectBlock)reject)
{
  ...
  BOOL canOpen = [ABI36_0_0RCTSharedApplication() canOpenURL:URL];
  ...
  if (canOpen) {
    ...
    if (querySchemes != nil && ([querySchemes containsObject:scheme] || [querySchemes containsObject:[scheme lowercaseString]])) {
      ...
      reject(ABI36_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@. Add %@ to LSApplicationQueriesSchemes in your Info.plist.", URL, scheme], nil);
    }
  } else {
    resolve(@NO);
  }
                  ...
                  reject:(__unused ABI36_0_0RCTPromiseRejectBlock)reject)
{
  ...
  resolve(ABI36_0_0RCTNullIfNil(initialURL.absoluteString));
}

ABI36_0_0RCT_EXPORT_METHOD(openSettings:(ABI36_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI36_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (@available(iOS 10.0, *)) {
    [ABI36_0_0RCTSharedApplication() openURL:url options:@{} completionHandler:^(BOOL success) {
      if (success) {
        ...
        reject(ABI36_0_0RCTErrorUnspecified, @"Unable to open app settings", nil);
      }
    }];
  } else {
#if !TARGET_OS_UIKITFORMAC
