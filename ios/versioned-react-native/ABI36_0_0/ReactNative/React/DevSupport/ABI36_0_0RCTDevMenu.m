...
NSString *const ABI36_0_0RCTShowDevMenuNotification = @"ABI36_0_0RCTShowDevMenuNotification";

@implementation UIWindow (ABI36_0_0RCTDevMenu)

- (void)ABI36_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI36_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

@implementation ABI36_0_0RCTDevMenuItem
{
  ABI36_0_0RCTDevMenuItemTitleBlock _titleBlock;
  dispatch_block_t _handler;
}

- (instancetype)initWithTitleBlock:(ABI36_0_0RCTDevMenuItemTitleBlock)titleBlock
...
typedef void(^ABI36_0_0RCTDevMenuAlertActionHandler)(UIAlertAction *action);

@interface ABI36_0_0RCTDevMenu () <ABI36_0_0RCTBridgeModule, ABI36_0_0RCTInvalidating>

@end

@implementation ABI36_0_0RCTDevMenu
{
  UIAlertController *_actionSheet;
  NSMutableArray<ABI36_0_0RCTDevMenuItem *> *_extraMenuItems;
}
  ...
  if ((self = [super init])) {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showOnShake)
                                                 name:ABI36_0_0RCTShowDevMenuNotification
                                               object:nil];
    _extraMenuItems = [NSMutableArray new];

#if TARGET_OS_SIMULATOR
    ABI36_0_0RCTKeyCommands *commands = [ABI36_0_0RCTKeyCommands sharedInstance];
...
{
  [self addItem:[ABI36_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
- (void)setDefaultJSBundle {
  [[ABI36_0_0RCTBundleURLProvider sharedSettings] resetToDefaults];
  self->_bridge.bundleURL = [[ABI36_0_0RCTBundleURLProvider sharedSettings] jsBundleURLForFallbackResource:nil fallbackExtension:nil];
  [self->_bridge reload];
}

- (NSArray<ABI36_0_0RCTDevMenuItem *> *)_menuItemsToPresent
{
  NSMutableArray<ABI36_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items
  __weak ABI36_0_0RCTBridge *bridge = _bridge;
  __weak ABI36_0_0RCTDevSettings *devSettings = _bridge.devSettings;
  __weak ABI36_0_0RCTDevMenu *weakSelf = self;

  [items addObject:[ABI36_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload" handler:^{
    [bridge reload];
  }]];

  if (!devSettings.isProfilingEnabled) {
    if (!devSettings.isRemoteDebuggingAvailable) {
      [items addObject:[ABI36_0_0RCTDevMenuItem buttonItemWithTitle:@"Debugger Unavailable" handler:^{
        NSString *message = ABI36_0_0RCTTurboModuleEnabled() ?
            @"Debugging is not currently supported when TurboModule is enabled." :
            @"Include the ABI36_0_0RCTWebSocket library to enable JavaScript debugging.";
        ...
        [ABI36_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
      }]];
    } else {
      [items addObject:[ABI36_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
    ...
    if (devSettings.isNuclideDebuggingAvailable && !devSettings.isDebuggingRemotely) {
      [items addObject:[ABI36_0_0RCTDevMenuItem buttonItemWithTitle:@"Debug with Nuclide" handler:^{
  #if ABI36_0_0RCT_ENABLE_INSPECTOR && !TARGET_OS_UIKITFORMAC
        [ABI36_0_0RCTInspectorDevServerHelper attachDebugger:@"ABI36_0_0ReactNative" withBundleURL:bridge.bundleURL withView: ABI36_0_0RCTPresentedViewController()];
  #endif
      }]];
    }
  }
  ...
  [items addObject:[ABI36_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"Apply Changes" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
      ...
      if (portNumber == nil) {
        portNumber = [NSNumber numberWithInt: ABI36_0_0RCT_METRO_PORT];
      }
      [ABI36_0_0RCTBundleURLProvider sharedSettings].jsLocation = [NSString stringWithFormat:@"%@:%d",
                                                          ipTextField.text, portNumber.intValue];
      __strong ABI36_0_0RCTBridge *strongBridge = bridge;
      if (strongBridge) {
        NSURL *bundleURL = bundleRoot.length
          ? [[ABI36_0_0RCTBundleURLProvider sharedSettings] jsBundleURLForBundleRoot:bundleRoot fallbackResource:nil]
...
ABI36_0_0RCT_EXPORT_METHOD(show)
{
  if (_actionSheet || !_bridge || ABI36_0_0RCTRunningInAppExtension()) {
    return;
  }
  ...
  _actionSheet = [UIAlertController alertControllerWithTitle:@"ABI36_0_0React Native Debug Menu"
                                                     message:description
                                              preferredStyle:style];

  NSArray<ABI36_0_0RCTDevMenuItem *> *items = [self _menuItemsToPresent];
  for (ABI36_0_0RCTDevMenuItem *item in items) {
  ...
  [ABI36_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:nil];
}

- (ABI36_0_0RCTDevMenuAlertActionHandler)alertActionHandlerForDevItem:(ABI36_0_0RCTDevMenuItem *__nullable)item
{
...
#define WARN_DEPRECATED_DEV_MENU_EXPORT() ABI36_0_0RCTLogWarn(@"Using deprecated method %s, use ABI36_0_0RCTDevSettings instead", __func__)
...
ABI36_0_0RCT_EXPORT_METHOD(debugRemotely:(BOOL)enableDebug)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
...
- (void)addItem:(ABI36_0_0RCTDevMenu *)item {}
...
@implementation  ABI36_0_0RCTBridge (ABI36_0_0RCTDevMenu)

- (ABI36_0_0RCTDevMenu *)devMenu
{
#if ABI36_0_0RCT_DEV
  return [self moduleForClass:[ABI36_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
