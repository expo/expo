...
#if ABI36_0_0RCT_DEV | ABI36_0_0RCT_ENABLE_LOADING_VIEW

static BOOL isEnabled = YES;

@implementation ABI36_0_0RCTDevLoadingView
{
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
...
- (void)setBridge:(ABI36_0_0RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(hide)
                                               name:ABI36_0_0RCTJavaScriptDidFailToLoadNotification
                                             object:nil];
...
ABI36_0_0RCT_EXPORT_METHOD(showMessage:(NSString *)message color:(UIColor *)color backgroundColor:(UIColor *)backgroundColor)
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!self->_window && !ABI36_0_0RCTRunningInTestEnvironment()) {
...
- (void)showWithURL:(NSURL *)URL
{
  ...
  if (URL.fileURL) {
    ...
    backgroundColor = [UIColor blackColor];
      message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI36_0_0RCT_PACKAGER_NAME];
  } else {
    color = [UIColor whiteColor];
