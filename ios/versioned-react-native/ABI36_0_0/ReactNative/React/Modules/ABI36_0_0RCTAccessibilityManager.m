...
NSString *const ABI36_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI36_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI36_0_0RCTAccessibilityManager ()
...
@implementation ABI36_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI36_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI36_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI36_0_0RCTLogError(@"Can't determine multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI36_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI36_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI36_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ABI36_0_0ReactTag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForABI36_0_0ReactTag:ABI36_0_0ReactTag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI36_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI36_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI36_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI36_0_0RCT_EXPORT_METHOD(getCurrentBoldTextState:(ABI36_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI36_0_0RCTResponseSenderBlock)error)
{
  callback(@[@(_isBoldTextEnabled)]);
}

ABI36_0_0RCT_EXPORT_METHOD(getCurrentGrayscaleState:(ABI36_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI36_0_0RCTResponseSenderBlock)error)
{
  callback(@[@(_isGrayscaleEnabled)]);
}

ABI36_0_0RCT_EXPORT_METHOD(getCurrentInvertColorsState:(ABI36_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI36_0_0RCTResponseSenderBlock)error)
{
  callback(@[@(_isInvertColorsEnabled)]);
}

ABI36_0_0RCT_EXPORT_METHOD(getCurrentReduceMotionState:(ABI36_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI36_0_0RCTResponseSenderBlock)error)
{
  callback(@[@(_isReduceMotionEnabled)]);
}

ABI36_0_0RCT_EXPORT_METHOD(getCurrentReduceTransparencyState:(ABI36_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI36_0_0RCTResponseSenderBlock)error)
{
  callback(@[@(_isReduceTransparencyEnabled)]);
}

ABI36_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI36_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI36_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI36_0_0RCTBridge (ABI36_0_0RCTAccessibilityManager)

- (ABI36_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI36_0_0RCTAccessibilityManager class]];
}
