...
@interface ABI36_0_0RCTPerformanceLogger ()
{
  int64_t _data[ABI36_0_0RCTPLSize][2];
  NSUInteger _cookies[ABI36_0_0RCTPLSize];
}
...
@implementation ABI36_0_0RCTPerformanceLogger

- (instancetype)init
{
  if (self = [super init]) {
    // Keep this in sync with ABI36_0_0RCTPLTag
    _labelsForTags = @[
      @"ScriptDownload",
      @"ScriptExecution",
      @"RAMBundleLoad",
...
- (void)markStartForTag:(ABI36_0_0RCTPLTag)tag
{
  ...
  if (ABI36_0_0RCTProfileIsProfiling()) {
    ...
    _cookies[tag] = ABI36_0_0RCTProfileBeginAsyncEvent(ABI36_0_0RCTProfileTagAlways, label, nil);
  }
#endif
...
- (void)markStopForTag:(ABI36_0_0RCTPLTag)tag
{
  ...
  if (ABI36_0_0RCTProfileIsProfiling()) {
    NSString *label =_labelsForTags[tag];
    ABI36_0_0RCTProfileEndAsyncEvent(ABI36_0_0RCTProfileTagAlways, @"native", _cookies[tag], label, @"ABI36_0_0RCTPerformanceLogger");
  }
#endif
  if (_data[tag][0] != 0 && _data[tag][1] == 0) {
    _data[tag][1] = CACurrentMediaTime() * 1000;
  } else {
    ABI36_0_0RCTLogInfo(@"Unbalanced calls start/end for tag %li", (unsigned long)tag);
  }
}
...
- (NSArray<NSNumber *> *)valuesForTags
{
  ...
  for (NSUInteger index = 0; index < ABI36_0_0RCTPLSize; index++) {
