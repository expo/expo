...
*/
static NSRegularExpression *ABI36_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI36_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI36_0_0RCTNullIfNil(self.methodName),
     @"file": ABI36_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI36_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
{
  return [[self alloc] initWithMethodName:ABI36_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI36_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI36_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}
...
+ (NSArray<ABI36_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI36_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI36_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI36_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
