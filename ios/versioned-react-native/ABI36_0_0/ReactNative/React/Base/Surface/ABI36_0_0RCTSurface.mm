...
#import "ABI36_0_0RCTSurfaceView+Internal.h"
...
#import "ABI36_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI36_0_0RCTSurface () <ABI36_0_0RCTSurfaceRootShadowViewDelegate, ABI36_0_0RCTUIManagerObserver>
@end

@implementation ABI36_0_0RCTSurface {
  // Immutable
  ...
  ABI36_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI36_0_0RCTSurfaceView *_Nullable _view;
  ABI36_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI36_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI36_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI36_0_0RCTAllocateRootViewTag();
    ...
    _stage = ABI36_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI36_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI36_0_0RCTSurfaceView *)view
{
  ABI36_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI36_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI36_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI36_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI36_0_0RCTSurfaceRootView *rootView =
    (ABI36_0_0RCTSurfaceRootView *)[self._uiManager viewForABI36_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI36_0_0RCTAssert([rootView isKindOfClass:[ABI36_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instance of `ABI36_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI36_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI36_0_0RCTSurfaceStageModuleDidLoad];
...
- (ABI36_0_0RCTSurfaceStage)stage
{
  std::lock_guard<std::mutex> lock(_mutex);
  return _stage;
}

- (void)_setStage:(ABI36_0_0RCTSurfaceStage)stage
{
  ABI36_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI36_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI36_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI36_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI36_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI36_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI36_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI36_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI36_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI36_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI36_0_0RCTIsMainQueue() ? ABI36_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI36_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI36_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI36_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI36_0_0ReactTag:self->_rootViewTag];
    ABI36_0_0RCTAssert([rootShadowView isKindOfClass:[ABI36_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instance of `ABI36_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI36_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI36_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI36_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI36_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI36_0_0ReactTag:self->_rootViewTag];
...
- (BOOL)synchronouslyWaitForStage:(ABI36_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI36_0_0RCTIsUIManagerQueue()) {
    ABI36_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI36_0_0RCTIsMainQueue() && (stage & ABI36_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI36_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI36_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI36_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI36_0_0RCTAssert(NO, @"Only waiting for `ABI36_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI36_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI36_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI36_0_0RCTIsMainQueue() && (stage & ABI36_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI36_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI36_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI36_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI36_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI36_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI36_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI36_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI36_0_0RCTUIManager *) manager performMountingWithBlock:(ABI36_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI36_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI36_0_0RCTUIManager *)manager
{
  if (self.stage & ABI36_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI36_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ABI36_0_0React components

- (void)mountABI36_0_0ReactComponentWithBridge:(ABI36_0_0RCTBridge *)bridge moduleName:(NSString *)moduleName params:(NSDictionary *)params
{
  [bridge enqueueJSCall:@"AppRegistry" method:@"runApplication" args:@[moduleName, params] completion:NULL];
}

- (void)unmountABI36_0_0ReactComponentWithBridge:(ABI36_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
