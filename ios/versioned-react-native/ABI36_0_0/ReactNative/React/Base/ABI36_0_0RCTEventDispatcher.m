...
const NSInteger ABI36_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI36_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI36_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(
    viewTag.intValue |
...
static uint16_t ABI36_0_0RCTUniqueCoalescingKeyGenerator = 0;

@implementation ABI36_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI36_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI36_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI36_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI36_0_0RCTComponentEvent *event = [[ABI36_0_0RCTComponentEvent alloc] initWithName:events[type]
                                                             viewTag:ABI36_0_0ReactTag
                                                                body:body];
...
- (void)sendEvent:(id<ABI36_0_0RCTEvent>)event
{
  ...
  for (id<ABI36_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI36_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI36_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI36_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI36_0_0RCTGetEventID(event.viewTag, event.eventName, ABI36_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI36_0_0RCTAssert(previousEvent == nil, @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
    [_eventQueue addObject:eventID];
  }
...
@implementation ABI36_0_0RCTBridge (ABI36_0_0RCTEventDispatcher)

- (ABI36_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI36_0_0RCTEventDispatcher class]];
}
