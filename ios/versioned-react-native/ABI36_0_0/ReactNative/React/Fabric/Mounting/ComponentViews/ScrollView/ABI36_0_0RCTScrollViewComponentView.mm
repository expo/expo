...
#import <ABI36_0_0React/components/scrollview/ScrollViewComponentDescriptor.h>
#import <ABI36_0_0React/components/scrollview/ScrollViewEventEmitter.h>
#import <ABI36_0_0React/components/scrollview/ScrollViewProps.h>
#import <ABI36_0_0React/components/scrollview/ScrollViewState.h>
...
@interface ABI36_0_0RCTScrollViewComponentView () <UIScrollViewDelegate>

@property (nonatomic, assign) CGFloat scrollEventThrottle;

@end

@implementation ABI36_0_0RCTScrollViewComponentView {
  ScrollViewShadowNode::ConcreteState::Shared _state;
  CGSize _contentSize;
}

+ (ABI36_0_0RCTScrollViewComponentView *_Nullable)findScrollViewComponentViewForView:(UIView *)view
{
  do {
    view = view.superview;
  } while (view != nil && ![view isKindOfClass:[ABI36_0_0RCTScrollViewComponentView class]]);
  return (ABI36_0_0RCTScrollViewComponentView *)view;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _scrollView = [[ABI36_0_0RCTEnhancedScrollView alloc] initWithFrame:self.bounds];
    ...
    _scrollViewDelegateSplitter = [[ABI36_0_0RCTGenericDelegateSplitter alloc] initWithDelegateUpdateBlock:^(id delegate) {
...
#define REMAP_PROP(ABI36_0_0ReactName, localName, target)                      \
  if (oldScrollViewProps.ABI36_0_0ReactName != newScrollViewProps.ABI36_0_0ReactName) { \
    target.localName = newScrollViewProps.ABI36_0_0ReactName;                  \
  }

#define REMAP_VIEW_PROP(ABI36_0_0ReactName, localName) REMAP_PROP(ABI36_0_0ReactName, localName, self)
#define MAP_VIEW_PROP(name) REMAP_VIEW_PROP(name, name)
#define REMAP_SCROLL_VIEW_PROP(ABI36_0_0ReactName, localName) \
  REMAP_PROP(ABI36_0_0ReactName, localName, ((ABI36_0_0RCTEnhancedScrollView *)_scrollView))
#define MAP_SCROLL_VIEW_PROP(name) REMAP_SCROLL_VIEW_PROP(name, name)
  ...
  if (oldScrollViewProps.contentInset != newScrollViewProps.contentInset) {
    _scrollView.contentInset = ABI36_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.contentInset);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  CGSize contentSize = ABI36_0_0RCTCGSizeFromSize(_state->getData().getContentSize());
...
{
  ABI36_0_0RCTAssert(childComponentView.superview == _containerView, @"Attempt to unmount improperly mounted component view.");
...
- (ScrollViewMetrics)_scrollViewMetrics
{
  ...
  metrics.contentSize = ABI36_0_0RCTSizeFromCGSize(_scrollView.contentSize);
  metrics.contentOffset = ABI36_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
  metrics.contentInset = ABI36_0_0RCTEdgeInsetsFromUIEdgeInsets(_scrollView.contentInset);
  metrics.containerSize = ABI36_0_0RCTSizeFromCGSize(_scrollView.bounds.size);
...
{
  auto contentOffset = ABI36_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
...
@implementation ABI36_0_0RCTScrollViewComponentView (ScrollableProtocol)
