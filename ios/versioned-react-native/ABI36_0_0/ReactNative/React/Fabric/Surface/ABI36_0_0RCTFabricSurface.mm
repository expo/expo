...
#import <ABI36_0_0React/ABI36_0_0RCTSurfaceTouchHandler.h>
...
@implementation ABI36_0_0RCTFabricSurface {
  // Immutable
  ABI36_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI36_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI36_0_0RCTAllocateRootViewTag() integerValue];

    _minimumSize = CGSizeZero;
    // FIXME: Replace with `_maximumSize = CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX);`.
    _maximumSize = ABI36_0_0RCTScreenSize();

    _touchHandler = [ABI36_0_0RCTSurfaceTouchHandler new];
...
- (BOOL)start
{
  if (![self _setStage:ABI36_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI36_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI36_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI36_0_0RCTSurfaceView alloc] initWithSurface:(ABI36_0_0RCTSurface *)self];
...
- (BOOL)_setStage:(ABI36_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
...
- (void)_propagateStageChange:(ABI36_0_0RCTSurfaceStage)stage
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI36_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI36_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
