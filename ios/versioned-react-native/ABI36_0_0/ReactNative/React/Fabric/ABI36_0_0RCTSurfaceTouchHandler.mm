...
#import "ABI36_0_0RCTSurfaceTouchHandler.h"
...
#import "ABI36_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI36_0_0RCTTouchEventType) {
  ABI36_0_0RCTTouchEventTypeTouchStart,
  ABI36_0_0RCTTouchEventTypeTouchMove,
  ABI36_0_0RCTTouchEventTypeTouchEnd,
  ABI36_0_0RCTTouchEventTypeTouchCancel,
};
...
static void UpdateActiveTouchWithUITouch(ActiveTouch &activeTouch, UITouch *uiTouch, UIView *rootComponentView) {
  ...
  activeTouch.touch.offsetPoint = ABI36_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI36_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI36_0_0RCTPointFromCGPoint(pagePoint);
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView) {
  ...
  if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
    activeTouch.eventEmitter = [(id<ABI36_0_0RCTTouchableComponentViewProtocol>)componentView
...
@interface ABI36_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI36_0_0RCTSurfaceTouchHandler {
  std::unordered_map<
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI36_0_0React Native,
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI36_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI36_0_0RCTTouchEventTypeTouchEnd || eventType == ABI36_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitters) {
    ...
    switch (eventType) {
      case ABI36_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI36_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI36_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI36_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI36_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI36_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI36_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI36_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
