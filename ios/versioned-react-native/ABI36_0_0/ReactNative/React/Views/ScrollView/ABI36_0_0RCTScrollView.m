...
@interface ABI36_0_0RCTScrollEvent : NSObject <ABI36_0_0RCTEvent>
...
@implementation ABI36_0_0RCTScrollEvent
{
...
- (ABI36_0_0RCTScrollEvent *)coalesceWithEvent:(ABI36_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI36_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI36_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI36_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
...
@implementation ABI36_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI36_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI36_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI36_0_0RCTUIManager JSResponder] isKindOfClass:[ABI36_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI36_0_0React scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
    ABI36_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
    ABI36_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI36_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI36_0_0RCTScrollView () <ABI36_0_0RCTUIManagerObserver>

@end

@implementation ABI36_0_0RCTScrollView
{
  ABI36_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI36_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
{
  ABI36_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI36_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI36_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setABI36_0_0ReactLayoutDirection:layoutDirection];

  ABI36_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI36_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI36_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI36_0_0ReactSubview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI36_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI36_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI36_0_0RCTAssert(_contentView == nil, @"ABI36_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@", [_contentView ABI36_0_0React_recursiveDescription]);
    _contentView = view;
    ABI36_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI36_0_0ReactLayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI36_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI36_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI36_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI36_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI36_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI36_0_0React_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ABI36_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI36_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI36_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI36_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI36_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI36_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI36_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI36_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI36_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI36_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI36_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ABI36_0_0ReactSubviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI36_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI36_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI36_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI36_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI36_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI36_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI36_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI36_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI36_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(__unused ABI36_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI36_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI36_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI36_0_0RCTScrollEvent *scrollEvent = [[ABI36_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI36_0_0RCTEventDispatcher (ABI36_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ABI36_0_0ReactTag
{
  ...
  ABI36_0_0RCTScrollEvent *fakeScrollEvent = [[ABI36_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI36_0_0ReactTag:ABI36_0_0ReactTag
