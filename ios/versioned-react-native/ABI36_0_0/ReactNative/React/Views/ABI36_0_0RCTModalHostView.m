...
@implementation ABI36_0_0RCTModalHostView
{
  __weak ABI36_0_0RCTBridge *_bridge;
  BOOL _isPresented;
  ABI36_0_0RCTModalHostViewController *_modalViewController;
  ABI36_0_0RCTTouchHandler *_touchHandler;
  UIView *_ABI36_0_0ReactSubview;
#if TARGET_OS_TV
...
ABI36_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:coder)

- (instancetype)initWithBridge:(ABI36_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _modalViewController = [ABI36_0_0RCTModalHostViewController new];
    UIView *containerView = [UIView new];
    containerView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _modalViewController.view = containerView;
    _touchHandler = [[ABI36_0_0RCTTouchHandler alloc] initWithBridge:bridge];
#if TARGET_OS_TV
...
- (void)setOnRequestClose:(ABI36_0_0RCTDirectEventBlock)onRequestClose
{
  _onRequestClose = onRequestClose;
  if (_ABI36_0_0ReactSubview) {
    if (_onRequestClose && _menuButtonGestureRecognizer) {
      [_ABI36_0_0ReactSubview addGestureRecognizer:_menuButtonGestureRecognizer];
    } else {
      [_ABI36_0_0ReactSubview removeGestureRecognizer:_menuButtonGestureRecognizer];
    }
  }
}
#endif
...
{
  if (_ABI36_0_0ReactSubview && _isPresented) {
    [_bridge.uiManager setSize:newBounds.size forView:_ABI36_0_0ReactSubview];
...
- (void)notifyForOrientationChange
{
  ...
  UIInterfaceOrientation currentOrientation = [ABI36_0_0RCTSharedApplication() statusBarOrientation];
...
{
  ABI36_0_0RCTAssert(_ABI36_0_0ReactSubview == nil, @"Modal view can only have one subview");
  ...
  for (NSString *key in [self.tvRemoteHandler.tvRemoteGestureRecognizers allKeys]) {
    if (![key isEqualToString:ABI36_0_0RCTTVRemoteEventMenu]) {
  ...
  _ABI36_0_0ReactSubview = subview;
}

- (void)removeABI36_0_0ReactSubview:(UIView *)subview
{
  ABI36_0_0RCTAssert(subview == _ABI36_0_0ReactSubview, @"Cannot remove view other than modal view");
...
#endif
  _ABI36_0_0ReactSubview = nil;
}

- (void)didUpdateABI36_0_0ReactSubviews
{
  // Do nothing, as subview (singular) is managed by `insertABI36_0_0ReactSubview:atIndex:`
}
...
- (void)didMoveToWindow
{
  ...
  if (!self.userInteractionEnabled && ![self.superview.ABI36_0_0ReactSubviews containsObject:self]) {
    return;
  }

  if (!_isPresented && self.window) {
    ABI36_0_0RCTAssert(self.ABI36_0_0ReactViewController, @"Can't present modal view controller without a presenting view controller");
