...
// ABI36_0_0ReactAndroid/src/main/java/com/facebook/ABI36_0_0React/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int ABI36_0_0RECONNECT_DELAY_MS = 2000;

@implementation ABI36_0_0RCTBundleStatus
@end

@interface ABI36_0_0RCTInspectorPackagerConnection () <ABI36_0_0RCTSRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI36_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  ABI36_0_0RCTSRWebSocket *_webSocket;
  dispatch_queue_t _jsQueue;
  BOOL _closed;
  BOOL _suppressConnectionErrors;
  ABI36_0_0RCTBundleStatusProvider _bundleStatusProvider;
}
@end

@interface ABI36_0_0RCTInspectorRemoteConnection () {
  __weak ABI36_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI36_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI36_0_0RCTInspectorPackagerConnection
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI36_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  ABI36_0_0RCTInspectorLocalConnection *existingConnection = _inspectorConnections[pageId];
  if (existingConnection) {
    [_inspectorConnections removeObjectForKey:pageId];
    [existingConnection disconnect];
    ABI36_0_0RCTLogWarn(@"Already connected: %@", pageId);
    return;
  }

  ABI36_0_0RCTInspectorRemoteConnection *remoteConnection =
    [[ABI36_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self
                                                              pageId:pageId];

  ABI36_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI36_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI36_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (NSArray *)pages
{
  NSArray<ABI36_0_0RCTInspectorPage *> *pages = [ABI36_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];

  ABI36_0_0RCTBundleStatusProvider statusProvider = _bundleStatusProvider;
  ABI36_0_0RCTBundleStatus *bundleStatus = statusProvider == nil
    ? nil
    : statusProvider();

  for (ABI36_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(__unused ABI36_0_0RCTSRWebSocket *)webSocket didFailWithError:(NSError *)error
{
  if (_webSocket) {
...
- (void)webSocket:(__unused ABI36_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  // warn but don't die on unrecognized messages
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI36_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI36_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI36_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@",
      messageText);
    return;
  }
...
- (void)webSocket:(__unused ABI36_0_0RCTSRWebSocket *)webSocket didCloseWithCode:(__unused NSInteger)code
...
{
  if (_closed) {
    ABI36_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresponding android code has a lot of custom config options for
  // timeouts, but it appears the iOS ABI36_0_0RCTSRWebSocket API doesn't have the same
  // implemented options.
  _webSocket = [[ABI36_0_0RCTSRWebSocket alloc] initWithURL:_url];
...
{
  if (_closed) {
    ABI36_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI36_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI36_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(
    dispatch_time(DISPATCH_TIME_NOW, ABI36_0_0RECONNECT_DELAY_MS *NSEC_PER_MSEC),
    dispatch_get_main_queue(), ^{
      ABI36_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI36_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI36_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
    ...
    withCause:(NSError *)cause
{
  ...
  if (![cause.domain isEqual:NSPOSIXErrorDomain] || cause.code != ECONNREFUSED) {
    ABI36_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
  }
...
@implementation ABI36_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI36_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
