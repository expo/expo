...
#import "ABI36_0_0EXAR.h"
#import "ABI36_0_0EXUnversioned.h"
#import "ABI36_0_0EXGLView.h"
#import <ABI36_0_0React/ABI36_0_0RCTUIManager.h>
#import "ABI36_0_0EXGLARSessionManager.h"

@interface ABI36_0_0EXAR () <ABI36_0_0EXGLARSessionManagerDelegate>

@property (nonatomic, strong) id arSessionManager;

@end

@implementation ABI36_0_0EXAR

ABI36_0_0RCT_EXPORT_MODULE(ExponentAR);
...
+ (NSMutableArray *)serializeARVideoFormats:(NSArray<ARVideoFormat *>*)videoFormats  API_AVAILABLE(ios(11.3))
{
  ...
  if (@available(iOS 11.3, *)) {
    for (ARVideoFormat *videoFormat in videoFormats) {
      [output addObject:[ABI36_0_0EXAR serializeARVideoFormat: videoFormat]];
    }
  }
...
- (NSDictionary<NSString *, NSString *> *)constantsToExport
{
  ...
  if (@available(iOS 11.3, *)) {
    [output addEntriesFromDictionary:@{
                                       @"ARKitVersion": @"1.5",
//                                       @"FaceTrackingVideoFormats": [ABI36_0_0EXAR serializeARVideoFormats:[ARFaceTrackingConfiguration supportedVideoFormats]],
                                       @"WorldTrackingVideoFormats": [ABI36_0_0EXAR serializeARVideoFormats:[ARWorldTrackingConfiguration supportedVideoFormats]],
                                       @"OrientationTrackingVideoFormats": [ABI36_0_0EXAR serializeARVideoFormats:[AROrientationTrackingConfiguration supportedVideoFormats]],
                                       }];
  }
...
ABI36_0_0RCT_REMAP_METHOD(startAsync,
                 startAsyncWithABI36_0_0ReactTag:(nonnull NSNumber *)tag
                 ...
                 rejecter:(ABI36_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI36_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ...
    if (![view isKindOfClass:[ABI36_0_0EXGLView class]]) {
      reject(@"E_GLVIEW_MANAGER_BAD_VIEW_TAG", nil, ABI36_0_0RCTErrorWithMessage(@"ABI36_0_0EXAR.startARSessionAsync: Expected an ABI36_0_0EXGLView"));
      return;
    }
    ...
    ABI36_0_0EXGLView *exglView = (ABI36_0_0EXGLView *)view;
    
    Class sessionManagerClass = NSClassFromString(@"ABI36_0_0EXGLARSessionManager");
    if (sessionManagerClass) {
      ...
      if (response[@"error"]) {
        reject(@"ERR_ARKIT_FAILED_TO_INIT", response[@"error"], ABI36_0_0RCTErrorWithMessage(response[@"error"]));
      } else {
        resolve(response);
      }
    } else {
      NSString *response = @"ARKIT capabilities were not included with this build.";
      reject(@"ERR_ARKIT_FAILED_TO_INIT", response, ABI36_0_0RCTErrorWithMessage(response));
    }
  }];
}

ABI36_0_0RCT_REMAP_METHOD(stopAsync,
                 resolver:(ABI36_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI36_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI36_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
ABI36_0_0RCT_EXPORT_METHOD(pause)
{
  [_arSessionManager pause];
}

ABI36_0_0RCT_EXPORT_METHOD(resume)
{
  [_arSessionManager resume];
}

ABI36_0_0RCT_EXPORT_METHOD(reset)
{
  [_arSessionManager reset];
}

ABI36_0_0RCT_REMAP_METHOD(setProvidesAudioData,
                 providesAudioData:(BOOL)providesAudioData)
{
  [_arSessionManager setProvidesAudioData:providesAudioData];
}

ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getProvidesAudioData, NSNumber *, getProvidesAudioData)
{
  return @([_arSessionManager providesAudioData]);
}

ABI36_0_0RCT_REMAP_METHOD(setLightEstimationEnabled,
                 lightEstimationEnabled:(BOOL)lightEstimationEnabled)
{
  [_arSessionManager setLightEstimationEnabled:lightEstimationEnabled];
}

ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getLightEstimationEnabled, NSNumber *, getLightEstimationEnabled)
{
  return @([_arSessionManager lightEstimationEnabled]);
}

ABI36_0_0RCT_REMAP_METHOD(setAutoFocusEnabled,
                 isAutoFocusEnabled:(BOOL)isAutoFocusEnabled)
{
  [_arSessionManager setAutoFocusEnabled:isAutoFocusEnabled];
}

ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getAutoFocusEnabled, NSNumber *, getAutoFocusEnabled)
{
  return @([_arSessionManager autoFocusEnabled]);
}

ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getPlaneDetection, NSString *, getPlaneDetection)
{
...
ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(performHitTest,
...
ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getWorldAlignment, NSString *, getWorldAlignment)
{
  NSArray *items = @[@"gravity", @"gravityAndHeading", @"alignmentCamera"];
  return [items objectAtIndex:[_arSessionManager worldAlignment]];
}

ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCurrentFrame,
                                      nullable NSDictionary *,
...
ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getARMatrices,
                                      nullable NSDictionary *,
...
ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getVideoFormat, nullable NSDictionary *, getVideoFormat)
{
  if (!_arSessionManager) {
    return nil;
  }
  if (@available(iOS 11.3, *)) {
    ...
    return [ABI36_0_0EXAR serializeARVideoFormat:videoFormat];
  }
  return nil;
}

ABI36_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCameraTexture, NSNumber *, getCameraTexture)
{
  return @([_arSessionManager cameraTexture]);
}

ABI36_0_0RCT_REMAP_METHOD(setShouldAttemptRelocalization,
...
ABI36_0_0RCT_REMAP_METHOD(setWorldOriginAsync,
...
ABI36_0_0RCT_REMAP_METHOD(setConfigurationAsync,
...
ABI36_0_0RCT_REMAP_METHOD(setDetectionImagesAsync,
