...
NSString * const ABI36_0_0EXShellManifestResourceName = @"shell-app-manifest";

@implementation ABI36_0_0EXScopedFileSystemModule

- (instancetype)initWithExperienceId:(NSString *)experienceId andConstantsBinding:(ABI36_0_0EXConstantsBinding *)constantsBinding
{
  NSString *escapedExperienceId = [ABI36_0_0EXScopedFileSystemModule escapedResourceName:experienceId];
...
- (NSArray<NSString *> *)bundledAssets
{
  ...
  dispatch_once(&once, ^{
    NSString *manifestBundlePath = [[NSBundle mainBundle] pathForResource:ABI36_0_0EXShellManifestResourceName ofType:@"json"];
    ...
    if (error) {
      ABI36_0_0UMLogError(@"Error parsing bundled manifest: %@", error);
      return;
    }
