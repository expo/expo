...
@interface ABI36_0_0EXFacebook (ExportedMethods)
...
@interface ABI36_0_0EXScopedFacebook ()
...
// Expo client-only ABI36_0_0EXFacebook module, which ensures that Facebook SDK configurations
// of different experiences don't collide.

@implementation ABI36_0_0EXScopedFacebook : ABI36_0_0EXFacebook
...
- (instancetype)initWithExperienceId:(NSString *)experienceId andParams:(NSDictionary *)params
{
  if (self = [super init]) {
    ...
    if (hasPreviouslySetAutoInitEnabled || manifestDefinesAutoInitEnabled) {
      ...
      // the mechanism behind ABI36_0_0EXFacebookAppDelegate.
      if (facebookAppId) {
        ...
        ABI36_0_0UMLogWarn(@"FacebookAutoInit is enabled, but no FacebookAppId has been provided. Facebook SDK initialization aborted.");
      }
    }
  }
  return self;
...
- (void)setAutoInitEnabled:(BOOL)enabled resolver:(ABI36_0_0UMPromiseResolveBlock)resolve rejecter:(ABI36_0_0UMPromiseRejectBlock)reject
{
  if (enabled) {
...
- (void)logInWithReadPermissionsWithConfig:(NSDictionary *)config resolver:(ABI36_0_0UMPromiseResolveBlock)resolve rejecter:(ABI36_0_0UMPromiseRejectBlock)reject
{
...
{
  id<ABI36_0_0UMAppLifecycleService> appLifecycleService = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI36_0_0UMAppLifecycleService)];
