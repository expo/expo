...
const CLLocationDegrees ABI36_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI36_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI36_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI36_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI36_0_0AIRMap
{
    UIView *_legalLabel;
    ...
    // Array to manually track ABI36_0_0RN subviews
    //
    // ABI36_0_0AIRMap implicitly creates subviews that aren't regular ABI36_0_0RN children
    // (ABI36_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI36_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI36_0_0React-native/blob/v0.16.0/ABI36_0_0React/Modules/ABI36_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI36_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI36_0_0React-native/blob/v0.16.0/Libraries/Text/ABI36_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ABI36_0_0ReactSubviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI36_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI36_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI36_0_0ReactSubview:(id<ABI36_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI36_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI36_0_0AIRMapPolyline class]]) {
        ((ABI36_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI36_0_0AIRMapPolygon class]]) {
        ((ABI36_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI36_0_0AIRMapCircle class]]) {
        ((ABI36_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI36_0_0AIRMapUrlTile class]]) {
        ((ABI36_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI36_0_0AIRMapWMSTile class]]) {
        ((ABI36_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI36_0_0AIRMapLocalTile class]]) {
        ((ABI36_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI36_0_0AIRMapOverlay class]]) {
        ((ABI36_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI36_0_0RCTComponent>> *childSubviews = [subview ABI36_0_0ReactSubviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertABI36_0_0ReactSubview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ABI36_0_0ReactSubviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeABI36_0_0ReactSubview:(id<ABI36_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI36_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI36_0_0RCTComponent>> *childSubviews = [subview ABI36_0_0ReactSubviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeABI36_0_0ReactSubview:(UIView *)childSubviews[i]];
        }
    }
    [_ABI36_0_0ReactSubviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ABI36_0_0AIRMapMarker *marker = (ABI36_0_0AIRMapMarker *)evaluatedObject;
        return [marker isKindOfClass:[ABI36_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI36_0_0AIRMapMarker*) markerForCallout:(ABI36_0_0AIRMapCallout*)callout {
    ABI36_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI36_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI36_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI36_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI36_0_0AIRMapCalloutSubview* calloutSubview = nil;
        ABI36_0_0AIRMapCallout* callout = nil;
        ABI36_0_0AIRMapMarker* marker = nil;
        
        UIView* tmp = touchedView;
        while (tmp && tmp != win && tmp != self.calloutView) {
            if ([tmp respondsToSelector:@selector(onPress)]) {
                calloutSubview = (ABI36_0_0AIRMapCalloutSubview*) tmp;
            }
            if ([tmp isKindOfClass:[ABI36_0_0AIRMapCallout class]]) {
                callout = (ABI36_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI36_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI36_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
