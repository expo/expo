...
#ifdef ABI36_0_0HAVE_GOOGLE_MAPS_UTILS
...
#define REQUIRES_GOOGLE_MAPS_UTILS(feature) do { \
 [NSException raise:@"ABI36_0_0ReactNativeMapsDependencyMissing" \
...
@interface ABI36_0_0AIRGoogleMap ()
...
@implementation ABI36_0_0AIRGoogleMap
{
...
- (void)insertABI36_0_0ReactSubview:(id<ABI36_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  ...
  if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapMarker class]]) {
    ABI36_0_0AIRGoogleMapMarker *marker = (ABI36_0_0AIRGoogleMapMarker*)subview;
    marker.realMarker.map = self;
    [self.markers addObject:marker];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapPolygon class]]) {
    ABI36_0_0AIRGoogleMapPolygon *polygon = (ABI36_0_0AIRGoogleMapPolygon*)subview;
    polygon.polygon.map = self;
    [self.polygons addObject:polygon];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapPolyline class]]) {
    ABI36_0_0AIRGoogleMapPolyline *polyline = (ABI36_0_0AIRGoogleMapPolyline*)subview;
    polyline.polyline.map = self;
    [self.polylines addObject:polyline];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapCircle class]]) {
    ABI36_0_0AIRGoogleMapCircle *circle = (ABI36_0_0AIRGoogleMapCircle*)subview;
    circle.circle.map = self;
    [self.circles addObject:circle];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapUrlTile class]]) {
    ABI36_0_0AIRGoogleMapUrlTile *tile = (ABI36_0_0AIRGoogleMapUrlTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapWMSTile class]]) {
    ABI36_0_0AIRGoogleMapWMSTile *tile = (ABI36_0_0AIRGoogleMapWMSTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapOverlay class]]) {
    ABI36_0_0AIRGoogleMapOverlay *overlay = (ABI36_0_0AIRGoogleMapOverlay*)subview;
    overlay.overlay.map = self;
    [self.overlays addObject:overlay];
  } else if ([subview isKindOfClass:[ABI36_0_0AIRGoogleMapHeatmap class]]){
    ABI36_0_0AIRGoogleMapHeatmap *heatmap = (ABI36_0_0AIRGoogleMapHeatmap*)subview;
...
- (void)didMoveToWindow {
  ...
  if (_initialCamera != nil) {
    ...
    self.camera = [ABI36_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_initialRegion];
  } else if (_region.span.latitudeDelta != 0.0 &&
      _region.span.longitudeDelta != 0.0) {
    self.camera = [ABI36_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_region];
  }
...
- (void)setInitialRegion:(MKCoordinateRegion)initialRegion {
  ...
  self.camera = [ABI36_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:initialRegion];
}
...
- (void)setRegion:(MKCoordinateRegion)region {
  ...
  _region = region;
  self.camera = [ABI36_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self  andMKCoordinateRegion:region];
}
...
- (BOOL)didTapMarker:(GMSMarker *)marker {
  ABI36_0_0AIRGMSMarker *airMarker = (ABI36_0_0AIRGMSMarker *)marker;
...
- (void)didTapPolyline:(GMSOverlay *)polyline {
  ABI36_0_0AIRGMSPolyline *airPolyline = (ABI36_0_0AIRGMSPolyline *)polyline;
...
- (void)didTapPolygon:(GMSOverlay *)polygon {
    ABI36_0_0AIRGMSPolygon *airPolygon = (ABI36_0_0AIRGMSPolygon *)polygon;
...
- (void)didChangeCameraPosition:(GMSCameraPosition *)position {
  id event = @{@"continuous": @YES,
               @"region": regionAsJSON([ABI36_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andGMSCameraPosition:position]),
               };
...
- (CGRect) frameForMarker:(ABI36_0_0AIRGoogleMapMarker*) mrkView {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI36_0_0AIRGoogleMapMarker* mrkView in self.markers) {
...
- (ABI36_0_0AIRGoogleMapMarker*) markerAtPoint:(CGPoint)point {
    ABI36_0_0AIRGoogleMapMarker* mrk = nil;
    for (ABI36_0_0AIRGoogleMapMarker* mrkView in self.markers) {
        CGRect frame = [self frameForMarker:mrkView];
...
- (id)extendedMapGestureHandler:(UIGestureRecognizer*)gestureRecognizer {
    ...
    if (!tapTouch)
        ...
        if (isTapInsideBubble) {
            ...
            ABI36_0_0AIRGoogleMapMarker* markerView = nil;
            ABI36_0_0AIRGMSMarker* marker = nil;
            for (ABI36_0_0AIRGoogleMapMarker* mrk in self.markers) {
            ...
            UIView* realSubview = [(ABI36_0_0RCTView*)bubbleView hitTest:tapPointInBubble withEvent:nil];
            ABI36_0_0AIRGoogleMapCalloutSubview* realPressableSubview = nil;
            if (realSubview) {
                UIView* tmp = realSubview;
                while (tmp && tmp != win && tmp != bubbleView) {
                    if ([tmp respondsToSelector:@selector(onPress)]) {
                        realPressableSubview = (ABI36_0_0AIRGoogleMapCalloutSubview*) tmp;
                        break;
                    }
                    tmp = tmp.superview;
                }
            ...
            if (markerView) {
                ...
                if (isInsideCallout) {
                    ...
                    ABI36_0_0AIRGoogleMapMarker* markerAtTapPoint = [self markerAtPoint:tapPoint];
...
- (void)setKmlSrc:(NSString *)kmlUrl {
  ...
  for (GMUPlacemark *place in parser.placemarks) {
    ...
    ABI36_0_0AIRGoogleMapMarker *marker = (ABI36_0_0AIRGoogleMapMarker *)[[ABI36_0_0AIRGoogleMapMarkerManager alloc] view];
    if (!marker.bridge) {
    ...
    marker.imageSrc = [ABI36_0_0AIRGoogleMap GetIconUrl:place parser:parser];
    marker.layer.backgroundColor = [UIColor clearColor].CGColor;
    marker.layer.position = CGPointZero;

    [self insertABI36_0_0ReactSubview:(UIView *) marker atIndex:index];
