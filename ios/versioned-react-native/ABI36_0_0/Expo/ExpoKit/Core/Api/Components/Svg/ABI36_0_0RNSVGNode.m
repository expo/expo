...
@interface ABI36_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI36_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI36_0_0RNSVGGroup *textRoot;
@end

@implementation ABI36_0_0RNSVGNode
{
    ABI36_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI36_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI36_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI36_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)invalidate
{
    ...
    _dirty = true;
    id<ABI36_0_0RNSVGContainer> container = (id<ABI36_0_0RNSVGContainer>)self.superview;
...
- (void)clearParentCache
{
    ...
    while (node != nil) {
        ...
        node = (ABI36_0_0RNSVGNode*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (ABI36_0_0RNSVGGroup *)textRoot
{
    ...
    ABI36_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI36_0_0RNSVGGroup class]] && [((ABI36_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI36_0_0RNSVGGroup*)node;
            break;
        }
...
- (ABI36_0_0RNSVGGroup *)getParentTextRoot
{
    ABI36_0_0RNSVGNode* parent = (ABI36_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI36_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI36_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI36_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)setClipRule:(ABI36_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI36_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if (_cachedClipPath) {
            CGPathRelease(_cachedClipPath);
        }
...
- (ABI36_0_0RNSVGSvgView *)svgView
{
    ...
    if ([parent class] == [ABI36_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI36_0_0RNSVGNode class]]) {
        _svgView = ((ABI36_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI36_0_0RCTLogError(@"ABI36_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI36_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)relativeOn:(ABI36_0_0RNSVGLength *)length relative:(CGFloat)relative
{
    ABI36_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)relativeOnWidth:(ABI36_0_0RNSVGLength *)length
{
    ABI36_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)relativeOnHeight:(ABI36_0_0RNSVGLength *)length
{
    ABI36_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)relativeOnOther:(ABI36_0_0RNSVGLength *)length
{
    ABI36_0_0RNSVGLengthUnitType unit = length.unit;
    if (unit == SVG_LENGTHTYPE_NUMBER){
        return length.value;
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI36_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI36_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
