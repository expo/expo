...
static NSCharacterSet *ABI36_0_0RNSVGTSpan_separators = nil;
static CGFloat ABI36_0_0RNSVGTSpan_radToDeg = 180 / (CGFloat)M_PI;

@interface ABI36_0_0TopAlignedLabel : UILabel

@end


@implementation ABI36_0_0TopAlignedLabel
...
@implementation ABI36_0_0RNSVGTSpan
{
    CGFloat startOffset;
    ABI36_0_0RNSVGTextPath *textPath;
    ...
    CGFloat firstX;
    CGFloat firstY;
    ABI36_0_0RNSVGPathMeasure *measure;
}
...
- (id)init
{
    ...
    if (ABI36_0_0RNSVGTSpan_separators == nil) {
        ABI36_0_0RNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
    }

    emoji = [NSMutableArray arrayWithCapacity:0];
    emojiTransform = [NSMutableArray arrayWithCapacity:0];
    measure = [[ABI36_0_0RNSVGPathMeasure alloc]init];
...
{
    if (self.content) {
        ABI36_0_0RNSVGGlyphContext* gc = [self.textRoot getGlyphContext];
...
- (NSMutableDictionary *)getAttributes:(ABI36_0_0RNSVGFontData *)fontdata {
    ...
    bool allowOptionalLigatures = letterSpacing == 0 && fontdata->fontVariantLigatures == ABI36_0_0RNSVGFontVariantLigaturesNormal;
...
static ABI36_0_0TopAlignedLabel *label;
- (void)drawWrappedText:(CGContextRef)context gc:(ABI36_0_0RNSVGGlyphContext *)gc rect:(CGRect)rect {
    [self pushGlyphContext];
    fontRef = [self getFontFromContext];
    ABI36_0_0RNSVGFontData* fontdata = [gc getFont];
    ...
    enum ABI36_0_0RNSVGTextAnchor textAnchor = fontdata->textAnchor;
    NSTextAlignment align;
    switch (textAnchor) {
        case ABI36_0_0RNSVGTextAnchorStart:
            align = NSTextAlignmentLeft;
            break;

        case ABI36_0_0RNSVGTextAnchorMiddle:
            align = NSTextAlignmentCenter;
            break;

        case ABI36_0_0RNSVGTextAnchorEnd:
    ...
    if (!label) {
        label = [[ABI36_0_0TopAlignedLabel alloc] init];
    }
    ...
    CGFloat width = [ABI36_0_0RNSVGPropHelper fromRelative:self.inlineSize
...
- (CGFloat)getSubtreeTextChunksTotalAdvance
{
    ...
    if (!str) {
        for (UIView *node in self.subviews) {
            if ([node isKindOfClass:[ABI36_0_0RNSVGText class]]) {
                ABI36_0_0RNSVGText *text = (ABI36_0_0RNSVGText*)node;
...
- (CGPathRef)getLinePath:(NSString *)str context:(CGContextRef)context
{
    ...
    ABI36_0_0RNSVGGlyphContext *gc = [self.textRoot getGlyphContext];
    ABI36_0_0RNSVGFontData *font = [gc getFont];
     ...
     */
    bool allowOptionalLigatures = letterSpacing == 0 && font->fontVariantLigatures == ABI36_0_0RNSVGFontVariantLigaturesNormal;
     ...
     */
    enum ABI36_0_0RNSVGTextAnchor textAnchor = font->textAnchor;
    ABI36_0_0RNSVGText *anchorRoot = [self getTextAnchorRoot];
    CGFloat textMeasure = [anchorRoot getSubtreeTextChunksTotalAdvance];
    CGFloat offset = [ABI36_0_0RNSVGTSpan getTextAnchorOffset:textAnchor width:textMeasure];
    ...
    if (hasTextPath) {
        //sharpMidLine = ABI36_0_0RNSVGTextPathMidLineFromString([textPath midLine]) == ABI36_0_0RNSVGTextPathMidLineSharp;
        /*
         Name
         side
         Value
         ...
         */
        side = ABI36_0_0RNSVGTextPathSideFromString([textPath side]) == ABI36_0_0RNSVGTextPathSideRight ? -1 : 1;
        /*
         Name
         startOffset
         Value
         ...
         the path is reached.
         */
        CGFloat absoluteStartOffset = [ABI36_0_0RNSVGPropHelper fromRelative:textPath.startOffset
                                                          relative:measure.pathLength
                                                          fontSize:fontSize];
        offset += absoluteStartOffset;
        if (measure.isClosed) {
            CGFloat halfPathDistance = measure.pathLength / 2;
            startOfRendering = absoluteStartOffset + (textAnchor == ABI36_0_0RNSVGTextAnchorMiddle ? -halfPathDistance : 0);
            endOfRendering = startOfRendering + measure.pathLength;
        }
        /*
         ABI36_0_0RNSVGTextPathSpacing spacing = textPath.getSpacing();
         if (spacing == ABI36_0_0RNSVGTextPathSpacing.auto) {
         // Hmm, what to do here?
    ...
    ABI36_0_0RNSVGLength *mTextLength = [self textLength];
    enum ABI36_0_0RNSVGTextLengthAdjust mLengthAdjust = ABI36_0_0RNSVGTextLengthAdjustFromString([self lengthAdjust]);
    if (mTextLength != nil) {
        CGFloat author = [ABI36_0_0RNSVGPropHelper fromRelative:mTextLength
                                             relative:[gc getWidth]
                                             fontSize:fontSize];
        if (author < 0) {
        ...
        switch (mLengthAdjust) {
            default:
            case ABI36_0_0RNSVGTextLengthAdjustSpacing:
                // TODO account for ligatures
                letterSpacing += (author - textMeasure) / (n - 1);
                break;
            case ABI36_0_0RNSVGTextLengthAdjustSpacingAndGlyphs:
    ...
    */
    ...
    enum ABI36_0_0RNSVGAlignmentBaseline baseline = ABI36_0_0RNSVGAlignmentBaselineFromString(self.alignmentBaseline);
    if (baseline != ABI36_0_0RNSVGAlignmentBaselineBaseline) {
        ...
        switch (baseline) {
            ...
            default:
            case ABI36_0_0RNSVGAlignmentBaselineBaseline:
                // Use the dominant baseline choice of the parent.
                // Match the box’s corresponding baseline to that of its parent.
                baselineShift = 0;
                break;

            case ABI36_0_0RNSVGAlignmentBaselineTextBottom:
            case ABI36_0_0RNSVGAlignmentBaselineAfterEdge:
            case ABI36_0_0RNSVGAlignmentBaselineTextAfterEdge:
            ...
            case ABI36_0_0RNSVGAlignmentBaselineCentral:
            ...
            case ABI36_0_0RNSVGAlignmentBaselineMathematical:
            ...
            case ABI36_0_0RNSVGAlignmentBaselineHanging:
                baselineShift = (CGFloat)0.8 * ascenderHeight;
                break;

            case ABI36_0_0RNSVGAlignmentBaselineTextTop:
            case ABI36_0_0RNSVGAlignmentBaselineBeforeEdge:
            case ABI36_0_0RNSVGAlignmentBaselineTextBeforeEdge:
            ...
            case ABI36_0_0RNSVGAlignmentBaselineBottom:
                // Align the top of the aligned subtree with the top of the line box.
                baselineShift = bottom;
                break;

            case ABI36_0_0RNSVGAlignmentBaselineCenter:
                // Align the center of the aligned subtree with the center of the line box.
                baselineShift = totalHeight / 2;
                break;

            case ABI36_0_0RNSVGAlignmentBaselineTop:
        ...
        switch (baseline) {
            ...
            default:
                if (fontData != nil && [baselineShiftString isEqualToString:@"sub"]) {
                    ...
                    baselineShift -= [ABI36_0_0RNSVGPropHelper fromRelativeWithNSString:baselineShiftString
                                                                   relative:fontSize
                                                                   fontSize:fontSize];
                }
                break;
    ...
    for (CFIndex ri = 0; ri < runEnd; ri++) {
        ...
        for(CFIndex g = 0; g < runGlyphCount; g++) {
            ...
            bool isWordSeparator = [ABI36_0_0RNSVGTSpan_separators characterIsMember:currentChar];
            ...
            CGFloat r = [gc nextRotation] / ABI36_0_0RNSVGTSpan_radToDeg;
...
+ (CGFloat)getTextAnchorOffset:(ABI36_0_0RNSVGTextAnchor)textAnchor width:(CGFloat) width
{
    switch (textAnchor) {
        case ABI36_0_0RNSVGTextAnchorStart:
            return 0;
...
{
    textPath = nil;
    ABI36_0_0RNSVGText *parent = (ABI36_0_0RNSVGText*)[self superview];
    CGPathRef path = nil;
    while (parent) {
        if ([parent class] == [ABI36_0_0RNSVGTextPath class]) {
            textPath = (ABI36_0_0RNSVGTextPath*) parent;
            ABI36_0_0RNSVGNode *template = [self.svgView getDefinedTemplate:textPath.href];
            path = [template getPath:context];
            [measure extractPathData:path];
            break;
        } else if (![parent isKindOfClass:[ABI36_0_0RNSVGText class]]) {
            break;
        }
        parent = (ABI36_0_0RNSVGText*)[parent superview];
    }
    if (!path) {
        [measure reset];
    }
