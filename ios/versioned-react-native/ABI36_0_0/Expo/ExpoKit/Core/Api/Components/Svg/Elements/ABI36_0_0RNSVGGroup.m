...
@implementation ABI36_0_0RNSVGGroup
{
...
- (void)renderGroupTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    [self traverseSubviews:^(UIView *node) {
        if ([node isKindOfClass:[ABI36_0_0RNSVGMask class]] || [node isKindOfClass:[ABI36_0_0RNSVGClipPath class]]) {
            // no-op
        } else if ([node isKindOfClass:[ABI36_0_0RNSVGNode class]]) {
            ABI36_0_0RNSVGNode* svgNode = (ABI36_0_0RNSVGNode*)node;
            if (svgNode.responsible && !self.svgView.responsible) {
                self.svgView.responsible = YES;
            }

            if ([node isKindOfClass:[ABI36_0_0RNSVGRenderable class]]) {
                [(ABI36_0_0RNSVGRenderable*)node mergeProperties:self];
            }
            ...
            if ([node isKindOfClass:[ABI36_0_0RNSVGRenderable class]]) {
                [(ABI36_0_0RNSVGRenderable*)node resetProperties];
            }
        } else if ([node isKindOfClass:[ABI36_0_0RNSVGSvgView class]]) {
            ABI36_0_0RNSVGSvgView* svgView = (ABI36_0_0RNSVGSvgView*)node;
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI36_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI36_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI36_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI36_0_0RNSVGMask class]]) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (self.clipPath) {
        ABI36_0_0RNSVGClipPath *clipNode = (ABI36_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if ([clipNode isSimpleClipPath]) {
            ...
            ABI36_0_0RNSVGRenderable *clipGroup = (ABI36_0_0RNSVGRenderable*)clipNode;
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI36_0_0RNSVGNode class]];
    ...
    for (UIView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI36_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI36_0_0RNSVGMask class]]) {
                continue;
            }
            ABI36_0_0RNSVGNode* svgNode = (ABI36_0_0RNSVGNode*)node;
            if (event) {
...
{
    [self traverseSubviews:^(__kindof ABI36_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI36_0_0RNSVGRenderable class]]) {
            [(ABI36_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
