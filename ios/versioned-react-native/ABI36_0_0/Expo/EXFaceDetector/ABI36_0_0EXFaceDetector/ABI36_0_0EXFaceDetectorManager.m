//
//  ABI36_0_0EXFaceDetectorManager.m
//  Exponent
//
...
@interface ABI36_0_0EXFaceDetectorManager() <AVCaptureVideoDataOutputSampleBufferDelegate>
...
@property ABI36_0_0EXFaceDetector *faceDetector;
@property NSInteger timeIntervalMillis;

@end

@implementation ABI36_0_0EXFaceDetectorManager

- (instancetype)init
{
  return [self initWithOptions:[ABI36_0_0EXFaceDetectorUtils defaultFaceDetectorOptions]];
}
...
- (instancetype)initWithOptions:(NSDictionary*)options
{
  if (self = [super init]) {
    ...
    _faceDetectorOptions = [ABI36_0_0EXFaceDetectorUtils mapOptions:options];
...
{
  FIRVisionFaceDetectorOptions* newOptions = [ABI36_0_0EXFaceDetectorUtils newOptions:self.faceDetectorOptions withValues:settings];
  if(![ABI36_0_0EXFaceDetectorUtils areOptionsEqual:newOptions to:self.faceDetectorOptions])
  {
...
- (void)tryEnablingFaceDetection
{
  ...
  if ([self isDetectingFaceEnabled]) {
    @try {
      self.faceDetector = [[ABI36_0_0EXFaceDetector alloc] initWithOptions:_faceDetectorOptions];
      ...
      if([_session canAddOutput:output]) {
        ...
        ABI36_0_0UMLogError(@"Unable to add output to camera session! Face detection aborted!");
      }
    } @catch (NSException *exception) {
      ABI36_0_0UMLogWarn(@"%@", [exception description]);
    }
  }
...
- (void)_notifyOfFaces:(NSArray<FIRVisionFace *> *)faces withEncoder:(ABI36_0_0EXFaceEncoder*)encoder
{
...
- (void)captureOutput:(AVCaptureVideoDataOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
  ...
  if(timePassedMillis > self.timeIntervalMillis) {
    ...
    ABI36_0_0EXFaceDetectionAngleTransformBlock angleTransform = ^(float angle) { return -angle; };
    
    CGAffineTransform transformation = [ABI36_0_0EXCSBufferOrientationCalculator pointTransformForInterfaceOrientation:_interfaceOrientation
    ...
    FIRVisionImageMetadata* metadata = [ABI36_0_0EXFaceDetectorManager metadataForInterfaceOrientation:_interfaceOrientation andMirrored:_mirroredImageSession];
    ...
    [_faceDetector detectFromBuffer:sampleBuffer metadata:metadata completionListener:^(NSArray<FIRVisionFace *> * _Nonnull faces, NSError * _Nonnull error) {
      if(error != nil) {
        ...
        [self _notifyOfFaces:faces withEncoder:[[ABI36_0_0EXFaceEncoder alloc] initWithTransform:transformation withRotationTransform:angleTransform]];
      }
