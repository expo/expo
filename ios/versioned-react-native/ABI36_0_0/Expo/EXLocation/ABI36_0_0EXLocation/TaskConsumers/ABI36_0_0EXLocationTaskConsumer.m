...
@interface ABI36_0_0EXLocationTaskConsumer ()
...
@implementation ABI36_0_0EXLocationTaskConsumer
...
# pragma mark - ABI36_0_0UMTaskConsumerInterface
...
- (void)didRegisterTask:(id<ABI36_0_0UMTaskInterface>)task
{
...
- (void)setOptions:(NSDictionary *)options
{
  [ABI36_0_0UMUtilities performSynchronouslyOnMainThread:^{
    ...
    locationManager.desiredAccuracy = [ABI36_0_0EXLocation CLLocationAccuracyFromOption:accuracy];
    locationManager.distanceFilter = [options[@"distanceInterval"] doubleValue] ?: kCLDistanceFilterNone;
    locationManager.activityType = [ABI36_0_0EXLocation CLActivityTypeFromOption:[options[@"activityType"] integerValue]];
...
- (void)executeTaskWithDeferredLocations
{
  ...
  NSDictionary *data = @{ @"locations": [ABI36_0_0EXLocationTaskConsumer _exportLocations:_deferredLocations] };
...
+ (NSArray<NSDictionary *> *)_exportLocations:(NSArray<CLLocation *> *)locations
{
  ...
  for (CLLocation *location in locations) {
    [result addObject:[ABI36_0_0EXLocation exportLocation:location]];
  }
  return result;
}
