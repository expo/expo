...
#import <ABI36_0_0EXAV/ABI36_0_0EXAVPlayerData.h>
#import <ABI36_0_0EXAV/ABI36_0_0EXVideoView.h>
#import <ABI36_0_0EXAV/ABI36_0_0EXAudioRecordingPermissionRequester.h>

NSString *const ABI36_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI36_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI36_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI36_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI36_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI36_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI36_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI36_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI36_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI36_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI36_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI36_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

NSString *const ABI36_0_0EXDidUpdatePlaybackStatusEventName = @"didUpdatePlaybackStatus";

@interface ABI36_0_0EXAV ()
...
@property (nonatomic, assign) ABI36_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI36_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI36_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, assign) BOOL isBeingObserved;
@property (nonatomic, strong) NSMutableSet <NSObject<ABI36_0_0EXAVObject> *> *videoSet;
...
@implementation ABI36_0_0EXAV

ABI36_0_0UM_EXPORT_MODULE(ExponentAV);

- (instancetype)init
{
  if (self = [super init]) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI36_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI36_0_0EXAudioInterruptionModeMixWithOthers;
...
{
  return @[@protocol(ABI36_0_0EXAVInterface)];
}
...
{
  [[_moduleRegistry getModuleImplementingProtocol:@protocol(ABI36_0_0UMAppLifecycleService)] unregisterAppLifecycleListener:self];
...
//  [_kernelAudioSessionManagerDelegate moduleDidForeground:self];
  [[_moduleRegistry getModuleImplementingProtocol:@protocol(ABI36_0_0UMAppLifecycleService)] registerAppLifecycleListener:self];
  _permissionsManager = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI36_0_0UMPermissionsInterface)];
  [ABI36_0_0UMPermissionsMethodsDelegate registerRequesters:@[[ABI36_0_0EXAudioRecordingPermissionRequester new]] withPermissionsManager:_permissionsManager];
}
...
- (void)onAppForegrounded
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI36_0_0EXAVObject> *exAVObject) {
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI36_0_0EXAVObject> *exAVObject))block
{
  for (ABI36_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI36_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI36_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  ...
  if (!playsInSilentMode && interruptionMode == ABI36_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI36_0_0UMErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and duckOthers == true cannot be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI36_0_0UMErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and allowsRecording == true cannot be set on iOS.");
  } else if (!playsInSilentMode && shouldPlayInBackground) {
    return ABI36_0_0UMErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and staysActiveInBackground == true cannot be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI36_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI36_0_0EXAVAudioSessionMode)audioSessionMode
{
  ...
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI36_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI36_0_0EXAudioInterruptionModeDoNotMix) {
      requiredAudioCategory = AVAudioSessionCategorySoloAmbient;
    } else {
      requiredAudioCategory = AVAudioSessionCategoryAmbient;
    }
  } else {
    ABI36_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI36_0_0EXAVAudioSessionModeActiveMuted ? ABI36_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
    ...
    switch (activeInterruptionMode) {
      case ABI36_0_0EXAudioInterruptionModeDoNotMix:
        break;
      case ABI36_0_0EXAudioInterruptionModeDuckOthers:
        requiredAudioCategoryOptions = AVAudioSessionCategoryOptionDuckOthers;
        break;
      case ABI36_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI36_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI36_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI36_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI36_0_0EXAVObject> *exAVObject) {
    ABI36_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI36_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI36_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI36_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI36_0_0UMErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded && ![_kernelAudioSessionManagerDelegate isActiveForModule:self]) {
    return ABI36_0_0UMErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI36_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI36_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI36_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI36_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI36_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI36_0_0UMPromiseRejectBlock)reject
{
  ABI36_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
    reject(@"E_AUDIO_NOPLAYER", nil, ABI36_0_0UMErrorWithMessage(@"Player does not exist."));
  }
}
...
withEXVideoViewForTag:(nonnull NSNumber *)ABI36_0_0ReactTag
     withRejecter:(ABI36_0_0UMPromiseRejectBlock)reject
{
  ...
  } forView:ABI36_0_0ReactTag ofClass:[ABI36_0_0EXVideoView class]];
}
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI36_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI36_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI36_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI36_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI36_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI36_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI36_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI36_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI36_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI36_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI36_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI36_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI36_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI36_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI36_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI36_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI36_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI36_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
      @([self _getFormatIDFromString:iosOptionsFromJS[ABI36_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI36_0_0UMErrorWithMessage(@"Recorder already exists.");
  }
  ...
  if (!fileSystem) {
    return ABI36_0_0UMErrorWithMessage(@"No FileSystem module.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI36_0_0UMPromiseRejectBlock)reject
{
  if (_audioRecorder == nil) {
    reject(@"E_AUDIO_NORECORDER", nil, ABI36_0_0UMErrorWithMessage(@"Recorder does not exist."));
  }
...
{
  return @[ABI36_0_0EXDidUpdatePlaybackStatusEventName, @"ExponentAV.onError"];
}

#pragma mark - Audio API: Global settings

ABI36_0_0UM_EXPORT_METHOD_AS(setAudioIsEnabled,
...
ABI36_0_0UM_EXPORT_METHOD_AS(setAudioMode,
...
ABI36_0_0UM_EXPORT_METHOD_AS(loadForSound,
                    loadForSound:(NSDictionary *)source
                    withStatus:(NSDictionary *)status
                    resolver:(ABI36_0_0UMPromiseResolveBlock)loadSuccess
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);

  __weak __typeof__(self) weakSelf = self;
  ABI36_0_0EXAVPlayerData *data = [[ABI36_0_0EXAVPlayerData alloc] initWithEXAV:self
                                                   withSource:source
                                                   withStatus:status
                                         withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                                           if (success) {
                                             ...
                                             loadError(@"ABI36_0_0EXAV", error, nil);
                                           }
                                         }];
  ...
  data.statusUpdateCallback = ^(NSDictionary *status) {
    ...
    if (strongSelf && strongSelf.isBeingObserved) {
      ...
      [strongSelf sendEventWithName:ABI36_0_0EXDidUpdatePlaybackStatusEventName body:response];
    }
  };
...
{
  [[_moduleRegistry getModuleImplementingProtocol:@protocol(ABI36_0_0UMEventEmitterService)] sendEventWithName:eventName body:body];
}

ABI36_0_0UM_EXPORT_METHOD_AS(unloadForSound,
                    unloadForSound:(NSNumber *)key
                    resolver:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI36_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI36_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI36_0_0UM_EXPORT_METHOD_AS(setStatusForSound,
...
ABI36_0_0UM_EXPORT_METHOD_AS(getStatusForSound,
...
ABI36_0_0UM_EXPORT_METHOD_AS(replaySound,
...
ABI36_0_0UM_EXPORT_METHOD_AS(loadForVideo,
                    loadForVideo:(NSNumber *)ABI36_0_0ReactTag
                    ...
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  ...
  } withEXVideoViewForTag:ABI36_0_0ReactTag withRejecter:reject];
}

ABI36_0_0UM_EXPORT_METHOD_AS(unloadForVideo,
                    unloadForVideo:(NSNumber *)ABI36_0_0ReactTag
                    resolver:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI36_0_0EXVideoView *view) {
...
ABI36_0_0UM_EXPORT_METHOD_AS(setStatusForVideo,
                    setStatusForVideo:(NSNumber *)ABI36_0_0ReactTag
                    withStatus:(NSDictionary *)status
                    resolver:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
...
ABI36_0_0UM_EXPORT_METHOD_AS(replayVideo,
                    replayVideo:(NSNumber *)ABI36_0_0ReactTag
                    withStatus:(NSDictionary *)status
                    resolver:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
...
ABI36_0_0UM_EXPORT_METHOD_AS(getStatusForVideo,
                    getStatusForVideo:(NSNumber *)ABI36_0_0ReactTag
                    resolver:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI36_0_0EXVideoView *view) {
                    ...
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  [ABI36_0_0UMPermissionsMethodsDelegate getPermissionWithPermissionsManager:_permissionsManager
                                                      withRequester:[ABI36_0_0EXAudioRecordingPermissionRequester class]
                                                            resolve:resolve
                                                             reject:reject];
}
...
ABI36_0_0UM_EXPORT_METHOD_AS(prepareAudioRecorder,
                    prepareAudioRecorder:(NSDictionary *)options
                    resolver:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI36_0_0EXAudioRecordingPermissionRequester class]]) {
    reject(@"E_MISSING_PERMISSION", @"Missing audio recording permission.", nil);
    return;
  }
  if (!_allowsAudioRecording) {
    reject(@"E_AUDIO_AUDIOMODE", nil, ABI36_0_0UMErrorWithMessage(@"Recording not allowed on iOS. Enable with Audio.setAudioModeAsync"));
    return;
  }
  ...
  if (_audioRecorder && !error) {
    ...
    if (error) {
      ...
      reject(@"E_AUDIO_RECORDERNOTCREATED", nil, ABI36_0_0UMErrorWithMessage(@"Prepare encountered an error: recorder not prepared."));
    }
    _audioRecorderIsPreparing = false;
    [self demoteAudioSessionIfPossible];
  } else {
...
ABI36_0_0UM_EXPORT_METHOD_AS(startAudioRecording,
                    startAudioRecording:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI36_0_0EXAudioRecordingPermissionRequester class]]) {
    reject(@"E_MISSING_PERMISSION", @"Missing audio recording permission.", nil);
    ...
    if (!_allowsAudioRecording) {
      ...
      if (!error) {
        if ([_audioRecorder record]) {
          ...
          reject(@"E_AUDIO_RECORDING", nil, ABI36_0_0UMErrorWithMessage(@"Start encountered an error: recording not started."));
        }
      } else {
...
ABI36_0_0UM_EXPORT_METHOD_AS(pauseAudioRecording,
                    pauseAudioRecording:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI36_0_0UM_EXPORT_METHOD_AS(stopAudioRecording,
                    stopAudioRecording:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI36_0_0UM_EXPORT_METHOD_AS(getAudioRecordingStatus,
                    getAudioRecordingStatus:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI36_0_0UM_EXPORT_METHOD_AS(unloadAudioRecorder,
                    unloadAudioRecorder:(ABI36_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
