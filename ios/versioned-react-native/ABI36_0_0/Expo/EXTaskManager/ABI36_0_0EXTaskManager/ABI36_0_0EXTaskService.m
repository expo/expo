...
@interface ABI36_0_0EXTaskService ()

// Array of task requests that are being executed.
@property (nonatomic, strong) NSMutableArray<ABI36_0_0EXTaskExecutionRequest *> *requests;

// Table of registered tasks. Schema: { "<appId>": { "<taskName>": ABI36_0_0EXTask } }
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSMutableDictionary<NSString *, ABI36_0_0EXTask *> *> *tasks;

// Dictionary with app records of running background apps. Schema: { "<appId>": ABI36_0_0EXAppRecordInterface }
@property (nonatomic, strong) NSMutableDictionary<NSString *, id<ABI36_0_0EXAppRecordInterface>> *appRecords;

// MapTable with task managers of running (foregrounded) apps. Schema: { "<appId>": ABI36_0_0UMTaskManagerInterface }
@property (nonatomic, strong) NSMapTable<NSString *, id<ABI36_0_0UMTaskManagerInterface>> *taskManagers;

// Same as above but for headless (backgrounded) apps.
@property (nonatomic, strong) NSMapTable<NSString *, id<ABI36_0_0UMTaskManagerInterface>> *headlessTaskManagers;
...
@implementation ABI36_0_0EXTaskService

ABI36_0_0UM_REGISTER_SINGLETON_MODULE(TaskService)
...
# pragma mark - ABI36_0_0UMTaskServiceInterface
...
{
  id<ABI36_0_0UMTaskInterface> task = [self _getTaskWithName:taskName forAppId:appId];
  return task != nil;
}

/**
 *  Creates a new task, registers it and saves to the config stored in user defaults.
 *  It can throw an exception if given consumer class doesn't conform to ABI36_0_0UMTaskConsumerInterface protocol
                     ...
                     options:(NSDictionary *)options
{
  ...
  // Given consumer class doesn't conform to ABI36_0_0UMTaskConsumerInterface protocol
  if (![unversionedConsumerClass conformsToProtocol:@protocol(ABI36_0_0UMTaskConsumerInterface)]) {
    NSString *reason = @"Invalid `consumer` argument. It must be a class that conforms to ABI36_0_0UMTaskConsumerInterface protocol.";
...
- (void)unregisterTaskWithName:(NSString *)taskName
                 ...
                 consumerClass:(Class)consumerClass
{
  ABI36_0_0EXTask *task = (ABI36_0_0EXTask *)[self _getTaskWithName:taskName forAppId:appId];
...
- (void)unregisterAllTasksForAppId:(NSString *)appId
{
  ...
  if (appTasks) {
    ...
    for (ABI36_0_0EXTask *task in [appTasks allValues]) {
      if ([task.consumer respondsToSelector:@selector(didUnregister)]) {
        [task.consumer didUnregister];
      }
    }
...
{
  NSDictionary<NSString *, id<ABI36_0_0UMTaskInterface>> *tasks = [self _getTasksForAppId:appId];
  NSMutableArray *results = [NSMutableArray new];

  for (NSString *taskName in tasks) {
    id<ABI36_0_0UMTaskInterface> task = tasks[taskName];
     ...
     didFinishWithResponse:(NSDictionary *)response
{
  ...
  for (ABI36_0_0EXTaskExecutionRequest *request in [_requests copy]) {
...
- (void)setTaskManager:(id<ABI36_0_0UMTaskManagerInterface>)taskManager
              forAppId:(NSString *)appId
...
# pragma mark - ABI36_0_0EXTaskDelegate

- (void)executeTask:(nonnull id<ABI36_0_0UMTaskInterface>)task
           withData:(nullable NSDictionary *)data
          withError:(nullable NSError *)error
{
  id<ABI36_0_0UMTaskManagerInterface> taskManager = [self _taskManagerForAppId:task.appId];
  NSDictionary *executionInfo = [self _executionInfoForTask:task];
  NSDictionary *body = @{
                         @"executionInfo": executionInfo,
                         @"data": data ?: @{},
                         @"error": ABI36_0_0UMNullIfNil([self _exportError:error]),
                         };

  NSLog(@"ABI36_0_0EXTaskService: Executing task '%@' for app '%@'.", task.name, task.appId);
...
- (void)applicationDidFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
  ...
  ABI36_0_0UMTaskLaunchReason launchReason = [self _launchReasonForLaunchOptions:launchOptions];
 ...
 */
- (id<ABI36_0_0UMTaskInterface>)_getTaskWithName:(NSString *)taskName
                               forAppId:(NSString *)appId
{
 ...
 */
- (NSDictionary<NSString *, ABI36_0_0EXTask *> *)_getTasksForAppId:(NSString *)appId
{
  return _tasks[appId];
}
 ...
 */
- (ABI36_0_0EXTask *)_internalRegisterTaskWithName:(nonnull NSString *)taskName
                                  ...
                                  options:(nullable NSDictionary *)options
{
  ...
  ABI36_0_0EXTask *task = [[ABI36_0_0EXTask alloc] initWithName:taskName
                                        appId:appId
                                       appUrl:appUrl
 ...
 */
- (void)_addTaskToConfig:(nonnull id<ABI36_0_0UMTaskInterface>)task
{
...
- (void)_iterateTasksUsingBlock:(void(^)(id<ABI36_0_0UMTaskInterface> task))block
{
 ...
 *        "<taskName>": {
 *          "name": "task's name",
 *          "consumerClass": "name of consumer class, e.g. ABI36_0_0EXLocationTaskConsumer",
 *          "consumerVersion": 1,
 *          "options": {},
 *        },
 *      }
 ...
 */
- (nullable NSDictionary *)_dictionaryFromTask:(id<ABI36_0_0UMTaskInterface>)task
{
  return @{
           @"name": task.name,
           ...
           @"options": ABI36_0_0UMNullIfNil([task options]),
           };
}

- (void)_runTasksSupportingLaunchReason:(ABI36_0_0UMTaskLaunchReason)launchReason
                               userInfo:(nullable NSDictionary *)userInfo
                               callback:(void(^)(NSArray * _Nonnull results))callback
{
  __block ABI36_0_0EXTaskExecutionRequest *request;

  request = [[ABI36_0_0EXTaskExecutionRequest alloc] initWithCallback:^(NSArray * _Nonnull results) {
    if (callback != nil) {
      callback(results);
    }
  ...
  [self _iterateTasksUsingBlock:^(id<ABI36_0_0UMTaskInterface> task) {
...
- (void)_loadAppWithId:(nonnull NSString *)appId
                appUrl:(nonnull NSString *)appUrl
{
  id<ABI36_0_0EXAppLoaderInterface> appLoader = [[ABI36_0_0EXAppLoaderProvider sharedInstance] createAppLoader:@"ABI36_0_0React-native-experience"];

  if (appLoader != nil && appUrl != nil) {
    __block id<ABI36_0_0EXAppRecordInterface> appRecord;

    NSLog(@"ABI36_0_0EXTaskService: Loading headless app '%@' with url '%@'.", appId, appUrl);

    appRecord = [appLoader loadAppWithUrl:appUrl options:nil callback:^(BOOL success, NSError *error) {
      if (!success) {
        NSLog(@"ABI36_0_0EXTaskService: Loading app '%@' from url '%@' failed. Error description: %@", appId, appUrl, error.description);
        [self->_events removeObjectForKey:appId];
 ...
 */
- (id<ABI36_0_0UMTaskManagerInterface>)_taskManagerForAppId:(NSString *)appId
{
  id<ABI36_0_0UMTaskManagerInterface> taskManager = [_taskManagers objectForKey:appId];
...
- (void)_restoreTasks
{
  ...
  if (config) {
    ...
    NSLog(@"ABI36_0_0EXTaskService: Restoring tasks configuration: %@", config.description);
      ...
      for (NSString *taskName in tasksConfig) {
        ...
        if (consumerClass != nil) {
          ...
          if (currentConsumerVersion == previousConsumerVersion) {
            ...
            ABI36_0_0UMLogWarn(
                      @"ABI36_0_0EXTaskService: Task consumer '%@' has version '%d' that is not compatible with the saved version '%d'.",
                      consumerClassName,
          ...
          ABI36_0_0UMLogWarn(@"ABI36_0_0EXTaskService: Cannot restore task '%@' because consumer class doesn't exist.", taskName);
          [self _removeTaskFromConfig:taskName appId:appId];
        }
      }
    }
...
- (void)_addTask:(id<ABI36_0_0UMTaskInterface>)task toRequest:(ABI36_0_0EXTaskExecutionRequest *)request
{
...
- (NSDictionary *)_executionInfoForTask:(nonnull id<ABI36_0_0UMTaskInterface>)task
{
...
{
  id<ABI36_0_0EXAppRecordInterface> appRecord = _appRecords[appId];
...
- (ABI36_0_0UMTaskLaunchReason)_launchReasonForLaunchOptions:(nullable NSDictionary *)launchOptions
{
  if (launchOptions == nil) {
    return ABI36_0_0UMTaskLaunchReasonUser;
  }
  if (launchOptions[UIApplicationLaunchOptionsBluetoothCentralsKey]) {
    return ABI36_0_0UMTaskLaunchReasonBluetoothCentrals;
  }
  if (launchOptions[UIApplicationLaunchOptionsBluetoothPeripheralsKey]) {
    return ABI36_0_0UMTaskLaunchReasonBluetoothPeripherals;
  }
  if (launchOptions[UIApplicationLaunchOptionsLocationKey]) {
    return ABI36_0_0UMTaskLaunchReasonLocation;
  }
  if (launchOptions[UIApplicationLaunchOptionsNewsstandDownloadsKey]) {
    return ABI36_0_0UMTaskLaunchReasonNewsstandDownloads;
  }
  if (launchOptions[UIApplicationLaunchOptionsRemoteNotificationKey]) {
    return ABI36_0_0UMTaskLaunchReasonRemoteNotification;
  }
  return ABI36_0_0UMTaskLaunchReasonUnrecognized;
}
