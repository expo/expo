...
NSString * const ABI36_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI36_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI36_0_0UMTaskServiceInterface> taskService;

@end

@implementation ABI36_0_0EXTaskManager

ABI36_0_0UM_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI36_0_0UMTaskManagerInterface)];
}
...
// TODO: Remove when adding bare ABI36_0_0React Native support
...
{
  return @{
           @"EVENT_NAME": ABI36_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI36_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI36_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI36_0_0UM_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI36_0_0UM_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI36_0_0UM_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI36_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI36_0_0UM_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI36_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI36_0_0UMPromiseRejectBlock)reject)
{
  resolve(ABI36_0_0UMNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI36_0_0UM_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI36_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI36_0_0UMPromiseRejectBlock)reject)
{
...
# pragma mark - ABI36_0_0UMTaskManagerInterface
...
{
  return [ABI36_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI36_0_0EXTaskManagerEventName body:body];
  } else {
...
{
  // TODO(@tsapeta): find app url for vanilla ABI36_0_0RN apps
