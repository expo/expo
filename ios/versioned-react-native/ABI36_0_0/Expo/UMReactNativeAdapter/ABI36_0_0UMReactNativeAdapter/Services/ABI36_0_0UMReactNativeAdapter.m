...
#import <ABI36_0_0UMReactNativeAdapter/ABI36_0_0UMReactNativeAdapter.h>
...
@interface ABI36_0_0UMReactNativeAdapter ()

@property (nonatomic, weak) ABI36_0_0RCTBridge *bridge;
@property (nonatomic, weak) ABI36_0_0UMNativeModulesProxy *modulesProxy;
...
@implementation ABI36_0_0UMReactNativeAdapter
...
{
  return @[@protocol(ABI36_0_0UMAppLifecycleService), @protocol(ABI36_0_0UMUIManager), @protocol(ABI36_0_0UMJavaScriptContextProvider), @protocol(ABI36_0_0UMImageLoaderInterface)];
}
...
{
  __weak ABI36_0_0UMReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI36_0_0UMReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI36_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI36_0_0RCTJSThread];
}
...
# pragma mark - ABI36_0_0UMJavaScriptContextProvider
...
- (void)handleAppStateDidChange:(NSNotification *)notification
{
  if (
      _isForegrounded && (
       ...
       ABI36_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI36_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
