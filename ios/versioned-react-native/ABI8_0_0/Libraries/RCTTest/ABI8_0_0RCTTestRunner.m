...
#import "ABI8_0_0RCTTestRunner.h"
...
@implementation ABI8_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI8_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI8_0_0RCTAssertParam(app);
  ABI8_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI8_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI8_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
configurationBlock:(void(^)(ABI8_0_0RCTRootView *rootView))configurationBlock
{
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI8_0_0RCTSetLogFunction(^(ABI8_0_0RCTLogLevel level, ABI8_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI8_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI8_0_0RCTBridge *bridge = [[ABI8_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI8_0_0RCTRootView *rootView = [[ABI8_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
    rootView.frame = CGRectMake(0, 0, 320, 2000); // Constant size for testing on multiple devices

    ABI8_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI8_0_0RCTTestModule class]];
    ABI8_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI8_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI8_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI8_0_0RCTSetLogFunction(ABI8_0_0RCTDefaultLogFunction);
    ...
    ABI8_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI8_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI8_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI8_0_0RCTAssert(testModule.status != ABI8_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI8_0_0RCTAssert(testModule.status == ABI8_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI8_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
