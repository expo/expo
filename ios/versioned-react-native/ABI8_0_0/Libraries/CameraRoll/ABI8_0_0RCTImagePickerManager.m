...
#import "ABI8_0_0RCTImagePickerManager.h"
#import "ABI8_0_0RCTImageStoreManager.h"
...
@interface ABI8_0_0RCTImagePickerManager ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI8_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI8_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI8_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
}

ABI8_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI8_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI8_0_0RCTResponseSenderBlock)callback)
{
...
ABI8_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI8_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI8_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI8_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI8_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  if ([ABI8_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI8_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI8_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI8_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI8_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI8_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI8_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    [self _dismissPicker:picker args:@[imageURL.absoluteString, ABI8_0_0RCTNullIfNil(height), ABI8_0_0RCTNullIfNil(width)]];
    return;
  }
        ...
        cancelCallback:(ABI8_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI8_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI8_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI8_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
