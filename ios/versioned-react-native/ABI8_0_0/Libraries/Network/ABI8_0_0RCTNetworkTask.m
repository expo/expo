...
@implementation ABI8_0_0RCTNetworkTask
{
  NSMutableData *_data;
  id<ABI8_0_0RCTURLRequestHandler> _handler;
  ABI8_0_0RCTNetworkTask *_selfReference;
}
...
{
  ABI8_0_0RCTAssertParam(request);
  ABI8_0_0RCTAssertParam(handler);
  ABI8_0_0RCTAssertParam(completionBlock);
  ...
  if ((self = [super init])) {
    ...
    _status = ABI8_0_0RCTNetworkTaskPending;
  }
  return self;
}
...
- (void)start
{
  if (_requestToken == nil) {
    if ([self validateRequestToken:[_handler sendRequest:_request
                                            withDelegate:self]]) {
      _selfReference = self;
      _status = ABI8_0_0RCTNetworkTaskInProgress;
    }
  }
}
...
- (void)cancel
{
  _status = ABI8_0_0RCTNetworkTaskFinished;
  ...
  if (_requestToken == nil) {
    if (requestToken == nil) {
      if (ABI8_0_0RCT_DEBUG) {
        ABI8_0_0RCTLogError(@"Missing request token for request: %@", _request);
      }
      valid = NO;
    }
    ...
    if (ABI8_0_0RCT_DEBUG) {
      ABI8_0_0RCTLogError(@"Unrecognized request token: %@ expected: %@", requestToken, _requestToken);
    }
    valid = NO;
  }
  if (!valid) {
    _status = ABI8_0_0RCTNetworkTaskFinished;
    if (_completionBlock) {
      _completionBlock(_response, nil, [NSError errorWithDomain:ABI8_0_0RCTErrorDomain code:0
