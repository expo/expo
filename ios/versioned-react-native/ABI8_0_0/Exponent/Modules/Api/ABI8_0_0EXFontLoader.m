...
static NSMutableDictionary *ABI8_0_0EXFonts = nil;

@interface ABI8_0_0EXFont : NSObject
...
@implementation ABI8_0_0RCTConvert (ABI8_0_0EXFontLoader)

// Will swap this with UIFont:withFamily:size:weight:style:scaleMultiplier:

+ (UIFont *)ABI8_0_0EXFont:(UIFont *)font withFamily:(id)family
                    size:(id)size weight:(id)weight style:(id)style
         scaleMultiplier:(CGFloat)scaleMultiplier
{
  if ([family hasPrefix:@"ExponentFont-"] && ABI8_0_0EXFonts) {
    NSString *suffix = [family substringFromIndex:[@"ExponentFont-" length]];
    if (ABI8_0_0EXFonts[suffix]) {
      return [ABI8_0_0EXFonts[suffix] UIFontWithSize:[self CGFloat:size] ?: 14];
    }
  }
  return [self ABI8_0_0EXFont:font withFamily:family size:size weight:weight style:style scaleMultiplier:scaleMultiplier];
}

@end


@implementation ABI8_0_0EXFontLoader

ABI8_0_0RCT_EXPORT_MODULE(ExponentFontLoader);

+ (void)initialize {
  SEL a = @selector(ABI8_0_0EXFont:withFamily:size:weight:style:scaleMultiplier:);
  SEL b = @selector(UIFont:withFamily:size:weight:style:scaleMultiplier:);
  method_exchangeImplementations(class_getClassMethod([ABI8_0_0RCTConvert class], a),
                                 class_getClassMethod([ABI8_0_0RCTConvert class], b));
}

ABI8_0_0RCT_REMAP_METHOD(loadAsync,
...
{
  if (ABI8_0_0EXFonts && ABI8_0_0EXFonts[fontFamilyName]) {
  ...
  if (!ABI8_0_0EXFonts) {
    ABI8_0_0EXFonts = [NSMutableDictionary dictionary];
  }
  ABI8_0_0EXFonts[fontFamilyName] = [[ABI8_0_0EXFont alloc] initWithCGFont:font];
  resolve(nil);
}
