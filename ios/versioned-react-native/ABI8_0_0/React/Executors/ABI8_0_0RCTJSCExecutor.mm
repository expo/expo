...
#import "ABI8_0_0RCTJSCWrapper.h"

NSString *const ABI8_0_0RCTJSCThreadName = @"com.facebook.ReactABI8_0_0.JavaScript";

NSString *const ABI8_0_0RCTJavaScriptContextCreatedNotification = @"ABI8_0_0RCTJavaScriptContextCreatedNotification";

static NSString *const ABI8_0_0RCTJSCProfilerEnabledDefaultsKey = @"ABI8_0_0RCTJSCProfilerEnabled";
...
@interface ABI8_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI8_0_0RCTCookieMap @end
#endif

struct ABI8_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
  ABI8_0_0RCTJSCWrapper *jscWrapper;
};

@interface ABI8_0_0RCTJSContextProvider ()
/** May only be called once, or deadlock will result. */
- (ABI8_0_0RCTJSContextData)data;
@end

@interface ABI8_0_0RCTJavaScriptContext : NSObject <ABI8_0_0RCTInvalidating>
...
@implementation ABI8_0_0RCTJavaScriptContext
{
  ABI8_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    ...
    /**
     * Explicitly introduce a retain cycle here - The ABI8_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI8_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI8_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  ABI8_0_0RCTJSCWrapper *_jscWrapper;
  ABI8_0_0RCTJavaScriptContext *_context;
...
static NSString *ABI8_0_0RCTJSValueToNSString(ABI8_0_0RCTJSCWrapper *jscWrapper, JSContextRef context, JSValueRef value, JSValueRef *exception)
{
...
static NSString *ABI8_0_0RCTJSValueToJSONString(ABI8_0_0RCTJSCWrapper *jscWrapper, JSContextRef context, JSValueRef value, JSValueRef *exception, unsigned indent)
{
...
static NSError *ABI8_0_0RCTNSErrorFromJSError(ABI8_0_0RCTJSCWrapper *jscWrapper, JSContextRef context, JSValueRef jsError)
{
  NSMutableDictionary *errorInfo = [NSMutableDictionary new];

  NSString *description = jsError ? ABI8_0_0RCTJSValueToNSString(jscWrapper, context, jsError, NULL) : @"Unknown JS error";
  errorInfo[NSLocalizedDescriptionKey] = [@"Unhandled JS Exception: " stringByAppendingString:description];

  NSString *details = jsError ? ABI8_0_0RCTJSValueToJSONString(jscWrapper, context, jsError, NULL, 0) : nil;
  if (details) {
    errorInfo[NSLocalizedFailureReasonErrorKey] = details;

    // Format stack as used in ABI8_0_0RCTFormatError
    id json = ABI8_0_0RCTJSONParse(details, NULL);
    if ([json isKindOfClass:[NSDictionary class]]) {
      if (json[@"stack"]) {
        ...
        if ([stackTrace count]) {
          errorInfo[ABI8_0_0RCTJSStackTraceKey] = stackTrace;
        }
      }

      // Fall back to just logging the line number
      if (!errorInfo[ABI8_0_0RCTJSStackTraceKey] && json[@"line"]) {
        errorInfo[ABI8_0_0RCTJSStackTraceKey] = @[@{
          @"methodName": @"",
          @"file": ABI8_0_0RCTNullIfNil(json[@"sourceURL"]),
          @"lineNumber": ABI8_0_0RCTNullIfNil(json[@"line"]),
          @"column": @0,
        }];
      }
    }
  ...
  return [NSError errorWithDomain:ABI8_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}

- (NSError *)errorForJSError:(JSValue *)jsError
{
  return ABI8_0_0RCTNSErrorFromJSError(_jscWrapper, jsError.context.JSGlobalContextRef, jsError.JSValueRef);
}

#if ABI8_0_0RCT_DEV

static void ABI8_0_0RCTInstallJSCProfiler(ABI8_0_0RCTBridge *bridge, JSContextRef context)
{
  if (ABI8_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI8_0_0RCTDevMenuItem toggleItemWithKey:ABI8_0_0RCTJSCProfilerEnabledDefaultsKey title:@"Start Profiling" selectedTitle:@"Stop Profiling" handler:^(BOOL shouldStart) {
      if (shouldStart != ABI8_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI8_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI8_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI8_0_0RCTProfileSendResult(bridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
+ (void)runRunLoopThread
{
  @autoreleasepool {
    ...
    while (kCFRunLoopRunStopped != CFRunLoopRunInMode(kCFRunLoopDefaultMode, ((NSDate *)[NSDate distantFuture]).timeIntervalSinceReferenceDate, NO)) {
      ABI8_0_0RCTAssert(NO, @"not reached assertion"); // runloop spun. that's bad.
    }
  }
}
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI8_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI8_0_0RCTJSCThreadName;
...
{
  ABI8_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI8_0_0RCTJSCExecutor init]", nil);
...
+ (instancetype)initializedExecutorWithContextProvider:(ABI8_0_0RCTJSContextProvider *)JSContextProvider
                                     applicationScript:(NSData *)applicationScript
                                             ...
                                             JSContext:(JSContext **)JSContext
                                                 error:(NSError **)error
{
  const ABI8_0_0RCTJSContextData data = JSContextProvider.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  ABI8_0_0RCTJSCExecutor *executor = [[ABI8_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
...
- (instancetype)initWithJSContextData:(const ABI8_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    ...
    _context = [[ABI8_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
...
- (ABI8_0_0RCTJavaScriptContext *)context
{
  ABI8_0_0RCTAssertThread(_javaScriptThread, @"Must be called on JS thread.");
  if (!self.isValid) {
    return nil;
  }
  ABI8_0_0RCTAssert(_context != nil, @"Fetching context while valid, but before it is created");
  return _context;
}
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI8_0_0RCTProfileDidStartProfiling, ABI8_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_jscWrapper) {
      ABI8_0_0RCTAssert(self->_context != nil, @"If wrapper was pre-initialized, context should be too");
      context = self->_context.context;
    } else {
      [self->_performanceLogger markStartForTag:ABI8_0_0RCTPLJSCWrapperOpenLibrary];
      self->_jscWrapper = ABI8_0_0RCTJSCWrapperCreate(self->_useCustomJSCLibrary);
      [self->_performanceLogger markStopForTag:ABI8_0_0RCTPLJSCWrapperOpenLibrary];

      ABI8_0_0RCTAssert(self->_context == nil, @"Didn't expect to set up twice");
      context = [self->_jscWrapper->JSContext new];
      self->_context = [[ABI8_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI8_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];
    ...
    __weak ABI8_0_0RCTJSCExecutor *weakSelf = self;

    context[@"nativeRequireModuleConfig"] = ^NSString *(NSString *moduleName) {
      ABI8_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", nil);
      NSArray *config = [strongSelf->_bridge configForModuleName:moduleName];
      NSString *result = config ? ABI8_0_0RCTJSONStringify(config, NULL) : nil;
      ABI8_0_0RCT_PROFILE_END_EVENT(ABI8_0_0RCTProfileTagAlways, @"js_call,config", @{ @"moduleName": moduleName });
      return result;
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
      [strongSelf->_bridge handleBuffer:calls batchEnded:NO];
      ABI8_0_0RCT_PROFILE_END_EVENT(ABI8_0_0RCTProfileTagAlways, @"js_call", nil);
    };

#if ABI8_0_0RCT_PROFILE
    __weak ABI8_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI8_0_0RCTProfileIsProfiling()) {
        ...
        int64_t newCookie = [_ABI8_0_0RCTProfileBeginFlowEvent() longLongValue];
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI8_0_0RCTProfileIsProfiling()) {
        ...
        _ABI8_0_0RCTProfileEndFlowEvent(@(newCookie));
...
#if ABI8_0_0RCT_DEV
    ABI8_0_0RCTInstallJSCProfiler(self->_bridge, context.JSGlobalContextRef);
    ...
    context[@"nativeInjectHMRUpdate"] = ^(NSString *sourceCode, NSString *sourceCodeURL) {
      ...
      ABI8_0_0RCTJSCWrapper *jscWrapper = strongSelf->_jscWrapper;
...
static void configureCacheOnContext(JSContext *context, ABI8_0_0RCTJSCWrapper *jscWrapper)
{
  if (jscWrapper->configureJSContextForIOS != NULL) {
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
    ABI8_0_0RCTAssert(cachesPath != nil, @"cachesPath should not be nil");
    if (cachesPath) {
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI8_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  ...
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ABI8_0_0RCTLogLevel level = ABI8_0_0RCTLogLevelInfo;
    if (logLevel) {
      level = MAX(level, (ABI8_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI8_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI8_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI8_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI8_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console", nil);
  };
  ABI8_0_0RCTCookieMap *cookieMap = [ABI8_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI8_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI8_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async", nil);
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI8_0_0RCTProfileDidStartProfiling];
...
- (void)dealloc
{
  ...
  if (_jscWrapper) {
    ABI8_0_0RCTJSCWrapperRelease(_jscWrapper);
    _jscWrapper = NULL;
  }
}
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI8_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args flushQueue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI8_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI8_0_0RCTJavaScriptCallback)onComplete
{
  ABI8_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI8_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI8_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", @{@"method": method, @"args": arguments});
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && !jscWrapper->JSValueIsUndefined(contextJSRef, batchedBridgeRef)) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && !jscWrapper->JSValueIsUndefined(contextJSRef, methodJSRef)) {
        ...
        if (!errorJSRef && jscWrapper->JSValueIsUndefined(contextJSRef, methodJSRef)) {
          error = ABI8_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && jscWrapper->JSValueIsUndefined(contextJSRef, batchedBridgeRef)) {
        error = ABI8_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI8_0_0RCTNSErrorFromJSError(jscWrapper, contextJSRef, errorJSRef);
      }
    } else {
    ...
    ABI8_0_0RCT_PROFILE_END_EVENT(0, @"js_call", nil);
...
{
  ABI8_0_0RCTAssertParam(script);
  ABI8_0_0RCTAssertParam(sourceURL);
...
static NSData *loadPossiblyBundledApplicationScript(NSData *script, NSURL *sourceURL,
                                                    ABI8_0_0RCTPerformanceLogger *performanceLogger,
                                                    BOOL &isRAMBundle, RandomAccessBundleData &randomAccessBundle,
                                                    NSError **error)
{
  ABI8_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);

  // The RAM bundle has a magic number in the 4 first bytes `(0xFB0BD1E5)`.
  uint32_t magicNumber = 0;
  [script getBytes:&magicNumber length:sizeof(magicNumber)];
  isRAMBundle = NSSwapLittleIntToHost(magicNumber) == ABI8_0_0RCTRAMBundleMagicNumber;
  if (isRAMBundle) {
    [performanceLogger markStartForTag:ABI8_0_0RCTPLRAMBundleLoad];
    script = loadRAMBundle(sourceURL, error, randomAccessBundle);
    [performanceLogger markStopForTag:ABI8_0_0RCTPLRAMBundleLoad];
    [performanceLogger setValue:script.length forTag:ABI8_0_0RCTPLRAMStartupCodeSize];

    // Reset the counters that the native require implementation uses
    [performanceLogger setValue:0 forTag:ABI8_0_0RCTPLRAMNativeRequires];
    [performanceLogger setValue:0 forTag:ABI8_0_0RCTPLRAMNativeRequiresCount];
    [performanceLogger setValue:0 forTag:ABI8_0_0RCTPLRAMNativeRequiresSize];
  } else {
...
static void registerNativeRequire(JSContext *context, ABI8_0_0RCTJSCExecutor *executor)
{
  __weak ABI8_0_0RCTJSCExecutor *weakExecutor = executor;
  context[@"nativeRequire"] = ^(NSNumber *moduleID) { [weakExecutor _nativeRequire:moduleID]; };
}

static NSError *executeApplicationScript(NSData *script, NSURL *sourceURL, ABI8_0_0RCTJSCWrapper *jscWrapper,
                                         ABI8_0_0RCTPerformanceLogger *performanceLogger, JSGlobalContextRef ctx)
{
  ABI8_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script",
                          @{ @"url": sourceURL.absoluteString, @"size": @(script.length) });
  [performanceLogger markStartForTag:ABI8_0_0RCTPLScriptExecution];
  ...
  jscWrapper->JSStringRelease(execJSString);
  [performanceLogger markStopForTag:ABI8_0_0RCTPLScriptExecution];
  NSError *error = result ? nil : ABI8_0_0RCTNSErrorFromJSError(jscWrapper, ctx, jsError);
  ABI8_0_0RCT_PROFILE_END_EVENT(0, @"js_call", nil);
  return error;
}
              ...
              callback:(ABI8_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI8_0_0RCT_DEBUG) {
    ABI8_0_0RCTAssert(ABI8_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    ABI8_0_0RCTJSCWrapper *jscWrapper = strongSelf->_jscWrapper;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI8_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ...
      if (jsError) {
        error = ABI8_0_0RCTNSErrorFromJSError(jscWrapper, ctx, jsError);
      }
    }
    ABI8_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call", nil);
...
static void executeRandomAccessModule(ABI8_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI8_0_0RCTFatal(ABI8_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  ABI8_0_0RCTJSCWrapper *jscWrapper = executor->_jscWrapper;
  ...
  if (!result) {
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI8_0_0RCTFatal(ABI8_0_0RCTNSErrorFromJSError(jscWrapper, ctx, jsError));
      [executor invalidate];
    });
  }
}
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI8_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI8_0_0RCTPLRAMNativeRequires];
  ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways,
  ...
  if (ID < _randomAccessBundle.numTableEntries) {
    ...
    [_performanceLogger addValue:size forTag:ABI8_0_0RCTPLRAMNativeRequiresSize];
    executeRandomAccessModule(self, ID, NSSwapLittleIntToHost(moduleData->offset), size);
  }

  ABI8_0_0RCT_PROFILE_END_EVENT(ABI8_0_0RCTProfileTagAlways, @"js_call", nil);
  [_performanceLogger appendStopForTag:ABI8_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI8_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI8_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
ABI8_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)name)
{
...
@implementation ABI8_0_0RCTJSContextProvider
{
...
- (void)_createContext
{
  _jscWrapper = ABI8_0_0RCTJSCWrapperCreate(_useCustomJSCLibrary);
