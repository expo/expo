...
static void *ABI8_0_0RCTCustomLibraryHandler(void)
{
  ...
  dispatch_once(&token, ^{
    ...
    if (path) {
      ...
      if (!handler) {
        ABI8_0_0RCTLogWarn(@"Can't load custom JSC library: %s", dlerror());
      }
    }
  });
  return handler;
...
static void ABI8_0_0RCTSetUpSystemLibraryPointers(ABI8_0_0RCTJSCWrapper *wrapper)
{
...
static void ABI8_0_0RCTSetUpCustomLibraryPointers(ABI8_0_0RCTJSCWrapper *wrapper)
{
  void *libraryHandle = ABI8_0_0RCTCustomLibraryHandler();
  if (!libraryHandle) {
    ABI8_0_0RCTSetUpSystemLibraryPointers(wrapper);
    return;
  }
...
ABI8_0_0RCTJSCWrapper *ABI8_0_0RCTJSCWrapperCreate(BOOL useCustomJSC)
{
  ABI8_0_0RCTJSCWrapper *wrapper = (ABI8_0_0RCTJSCWrapper *)malloc(sizeof(ABI8_0_0RCTJSCWrapper));
  if (useCustomJSC && [UIDevice currentDevice].systemVersion.floatValue >= 8) {
    ABI8_0_0RCTSetUpCustomLibraryPointers(wrapper);
  } else {
    ABI8_0_0RCTSetUpSystemLibraryPointers(wrapper);
  }
  return wrapper;
...
void ABI8_0_0RCTJSCWrapperRelease(ABI8_0_0RCTJSCWrapper *wrapper)
{
  if (wrapper) {
    free(wrapper);
  }
