...
@implementation ABI8_0_0RCTModuleData
{
...
- (void)setUp
{
  ...
  _hasConstantsToExport = ABI8_0_0RCTClassOverridesInstanceMethod(_moduleClass, @selector(constantsToExport));
}
...
- (instancetype)initWithModuleInstance:(id<ABI8_0_0RCTBridgeModule>)instance
                                bridge:(ABI8_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _bridge = bridge;
...
{
  ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"[ABI8_0_0RCTModuleData setUpInstanceAndBridge] [_instanceLock lock]", nil);
  [_instanceLock lock];
  if (!_setupComplete && _bridge.valid) {
    if (!_instance) {
      if (ABI8_0_0RCT_DEBUG && _requiresMainQueueSetup) {
        ABI8_0_0RCTAssertMainQueue();
      }
      ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"[ABI8_0_0RCTModuleData setUpInstanceAndBridge] [_moduleClass new]", nil);
      ...
      if (!_instance) {
        ...
        _setupComplete = YES;
        ABI8_0_0RCTLogWarn(@"The module %@ is returning nil from its constructor. You "
                   "may need to instantiate it yourself and pass it into the "
                   "bridge.", _moduleClass);
      }
    }

    if (_instance && ABI8_0_0RCTProfileIsProfiling()) {
      ABI8_0_0RCTProfileHookInstance(_instance);
    }
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"[ABI8_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
      [(id)_instance setValue:_bridge forKey:@"bridge"];
    }
    @catch (NSException *exception) {
      ABI8_0_0RCTLogError(@"%@ has no setter or ivar for its bridge, which is not "
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"[ABI8_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI8_0_0RCTDidInitializeModuleNotification
                                                        object:_bridge
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"[ABI8_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ReactABI8_0_0.%@Queue", self.name];
      ...
      if (implementsMethodQueue) {
        ...
        @catch (NSException *exception) {
          ABI8_0_0RCTLogError(@"%@ is returning nil for its methodQueue, which is not "
...
- (id<ABI8_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI8_0_0RCTModuleData instanceForClass:%@]", _moduleClass], nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, @"[ABI8_0_0RCTModuleData instance] main thread setup", nil);
      ABI8_0_0RCTExecuteOnMainThread(^{
...
- (NSString *)name
{
  return ABI8_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI8_0_0RCTBridgeMethod>> *)methods
{
  if (!_methods) {
    NSMutableArray<id<ABI8_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
    ...
    while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
      ...
      for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
          ...
          id<ABI8_0_0RCTBridgeMethod> moduleMethod =
            [[ABI8_0_0RCTModuleMethod alloc] initWithMethodSignature:entries[1]
...
- (void)gatherConstants
{
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI8_0_0RCTModuleData gatherConstants] %@", _moduleClass], nil);
    (void)[self instance];
    ABI8_0_0RCTExecuteOnMainThread(^{
...
- (NSArray *)config
{
  ...
  ABI8_0_0RCT_PROFILE_BEGIN_EVENT(ABI8_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI8_0_0RCTModuleData config] %@", _moduleClass], nil);

  NSMutableArray<NSString *> *methods = self.methods.count ? [NSMutableArray new] : nil;
  NSMutableArray<NSNumber *> *asyncMethods = nil;
  for (id<ABI8_0_0RCTBridgeMethod> method in self.methods) {
    if (method.functionType == ABI8_0_0RCTFunctionTypePromise) {
      if (!asyncMethods) {
  ...
  ABI8_0_0RCT_PROFILE_END_EVENT(ABI8_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI8_0_0RCTModuleData config] %@", _moduleClass], nil);
  return config;
}
