...
const NSInteger ABI8_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI8_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI8_0_0RCTGetEventID(id<ABI8_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI8_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI8_0_0RCTEvent>> *_events;
...
{
  [_bridge enqueueJSCall:@"ABI8_0_0RCTNativeAppEventEmitter.emit"
                    args:body ? @[name, body] : @[name]];
}

- (void)sendDeviceEventWithName:(NSString *)name body:(id)body
{
  [_bridge enqueueJSCall:@"ABI8_0_0RCTDeviceEventEmitter.emit"
                    args:body ? @[name, body] : @[name]];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI8_0_0RCT_DEBUG) {
    ABI8_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI8_0_0 tag");
  }

  name = ABI8_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI8_0_0RCTEventEmitter.receiveEvent"
...
- (void)sendEvent:(id<ABI8_0_0RCTEvent>)event
{
  ...
  NSNumber *eventID = ABI8_0_0RCTGetEventID(event);

  id<ABI8_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI8_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)dispatchEvent:(id<ABI8_0_0RCTEvent>)event
{
...
@implementation ABI8_0_0RCTBridge (ABI8_0_0RCTEventDispatcher)

- (ABI8_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI8_0_0RCTEventDispatcher class]];
}
