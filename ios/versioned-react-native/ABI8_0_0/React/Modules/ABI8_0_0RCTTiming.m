...
@interface _ABI8_0_0RCTTimer : NSObject
...
@implementation _ABI8_0_0RCTTimer
...
@interface _ABI8_0_0RCTTimingProxy : NSObject

@end

// NSTimer retains its target, insert this class to break potential retain cycles
@implementation _ABI8_0_0RCTTimingProxy
{
  __weak id _target;
}
...
{
  _ABI8_0_0RCTTimingProxy *proxy = [self new];
  if (proxy) {
...
@implementation ABI8_0_0RCTTiming
{
  NSMutableDictionary<NSNumber *, _ABI8_0_0RCTTimer *> *_timers;
  NSTimer *_sleepTimer;
...
{
  ABI8_0_0RCTAssert(!_bridge, @"Should never be initialized twice!");
...
- (void)didUpdateFrame:(__unused ABI8_0_0RCTFrameUpdate *)update
{
  ...
  for (_ABI8_0_0RCTTimer *timer in _timers.allValues) {
  ...
  if (!_sendIdleEvents && timersToCall.count == 0) {
    // No need to call the pauseCallback as ABI8_0_0RCTDisplayLink will ask us about our paused
...
- (void)scheduleSleepTimer:(NSDate *)sleepTarget
{
  if (!_sleepTimer || !_sleepTimer.valid) {
    _sleepTimer = [[NSTimer alloc] initWithFireDate:sleepTarget
                                           interval:0
                                             target:[_ABI8_0_0RCTTimingProxy proxyWithTarget:self]
 ...
 */
ABI8_0_0RCT_EXPORT_METHOD(createTimer:(nonnull NSNumber *)callbackID
                  ...
                  repeats:(BOOL)repeats)
{
  ...
  _ABI8_0_0RCTTimer *timer = [[_ABI8_0_0RCTTimer alloc] initWithCallbackID:callbackID
                                                  interval:jsDuration
...
ABI8_0_0RCT_EXPORT_METHOD(deleteTimer:(nonnull NSNumber *)timerID)
{
...
ABI8_0_0RCT_EXPORT_METHOD(setSendIdleEvents:(BOOL)sendIdleEvents)
{
