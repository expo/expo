...
@class ABI8_0_0RCTRedBoxWindow;

@protocol ABI8_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI8_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI8_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI8_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI8_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI8_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI8_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI8_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI8_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI8_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI8_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI8_0_0RCTJSStackFrame *)stackFrame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI8_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI8_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI8_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI8_0_0RCTKeyCommands for this, but since
...
@interface ABI8_0_0RCTRedBox () <ABI8_0_0RCTInvalidating, ABI8_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI8_0_0RCTRedBox
{
  ABI8_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI8_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI8_0_0RCTErrorInfo *)_customizeError:(ABI8_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI8_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI8_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI8_0_0RCTJSStackFrame *> *stack = [ABI8_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  [self _showErrorMessage:message withStack:[ABI8_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:isUpdate];
}

- (void)_showErrorMessage:(NSString *)message withStack:(NSArray<ABI8_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI8_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI8_0_0RCTErrorInfo *errorInfo = [[ABI8_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI8_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI8_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI8_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI8_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI8_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI8_0_0RCTRedBoxWindow *)redBoxWindow {
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI8_0_0RCTReloadNotification object:_bridge.baseBridge userInfo:nil];
}

@end

@implementation ABI8_0_0RCTBridge (ABI8_0_0RCTRedBox)

- (ABI8_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI8_0_0RCTRedBox class]];
}
...
- (ABI8_0_0RCTRedBox *)redBox { return nil; }
