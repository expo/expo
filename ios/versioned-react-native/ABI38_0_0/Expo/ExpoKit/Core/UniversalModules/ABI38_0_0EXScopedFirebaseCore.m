...
#import <ABI38_0_0EXFirebaseCore/ABI38_0_0EXFirebaseCore+FIROptions.h>

@interface NSObject (Private)
- (NSString*)_methodDescription;
@end

@implementation ABI38_0_0EXScopedFirebaseCore {
  ...
  for (NSString* name in names) {
    if (!protectedAppNames[name] && (!options || ![name isEqualToString:appName])) {
      [[FIRApp appNamed:name] deleteApp:^(BOOL success) {
        if (!success) {
          ABI38_0_0UMLogWarn(@"Failed to delete Firebase app: %@", name);
        }
      }];
    }
  }
...
+ (nullable NSDictionary *)googleServicesFileFromConstantsManifest:(nullable id<ABI38_0_0UMConstantsInterface>)constants
{
  ...
  @try {
    ...
    if (error) ABI38_0_0UMLogWarn(@"Invalid googleServicesFile: %@", error);
    return plist;
  }
  @catch(NSException* exception) {
    ABI38_0_0UMLogWarn(@"Invalid googleServicesFile: %@", exception);
    return nil;
  }
}
