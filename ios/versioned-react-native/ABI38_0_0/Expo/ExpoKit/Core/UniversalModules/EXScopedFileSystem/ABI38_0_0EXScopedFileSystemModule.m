...
NSString * const ABI38_0_0EXShellManifestResourceName = @"shell-app-manifest";

@implementation ABI38_0_0EXScopedFileSystemModule

- (instancetype)initWithExperienceId:(NSString *)experienceId andConstantsBinding:(ABI38_0_0EXConstantsBinding *)constantsBinding
{
  NSString *escapedExperienceId = [ABI38_0_0EXScopedFileSystemModule escapedResourceName:experienceId];
...
- (NSArray<NSString *> *)bundledAssets
{
  ...
  dispatch_once(&once, ^{
    NSString *manifestBundlePath = [[NSBundle mainBundle] pathForResource:ABI38_0_0EXShellManifestResourceName ofType:@"json"];
    ...
    if (error) {
      ABI38_0_0UMLogError(@"Error parsing bundled manifest: %@", error);
      return;
    }
