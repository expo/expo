...
NSInteger const ABI38_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE = 999;

@implementation ABI38_0_0AIREmptyCalloutBackgroundView
@end

@implementation ABI38_0_0AIRMapMarker {
...
- (void)ABI38_0_0ReactSetFrame:(CGRect)frame
{
    ...
    if (isnan(center.x) || isnan(center.y) ||
        ...
        ABI38_0_0RCTLogError(@"Invalid layout for (%@)%@. position: %@. bounds: %@",
                self.ABI38_0_0ReactTag, self, NSStringFromCGPoint(center), NSStringFromCGRect(bounds));
        return;
    }
...
- (void)insertABI38_0_0ReactSubview:(id<ABI38_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    if ([subview isKindOfClass:[ABI38_0_0AIRMapCallout class]]) {
        self.calloutView = (ABI38_0_0AIRMapCallout *)subview;
    } else {
        [super insertABI38_0_0ReactSubview:(UIView *)subview atIndex:atIndex];
    }
}

- (void)removeABI38_0_0ReactSubview:(id<ABI38_0_0RCTComponent>)subview {
    if ([subview isKindOfClass:[ABI38_0_0AIRMapCallout class]] && self.calloutView == subview) {
        self.calloutView = nil;
    } else {
        [super removeABI38_0_0ReactSubview:(UIView *)subview];
    }
}
...
- (MKAnnotationView *)getAnnotationView
{
    if ([self shouldUsePinView]) {
        ...
        // If it has subviews, it means we are wanting to render a custom marker with arbitrary ABI38_0_0React views.
        // if it has a non-null image, it means we want to render a custom marker with the image.
        // In either case, we want to return the ABI38_0_0AIRMapMarker since it is both an MKAnnotation and an
...
- (void)fillCalloutView:(ABI38_0_0SMCalloutView *)calloutView
{
    ...
    if (self.calloutView) {
        ...
        if (self.calloutView.tooltip) {
            // if tooltip is true, then the user wants their ABI38_0_0React view to be the "tooltip" as wwell, so we set
            // the background view to something empty/transparent
            calloutView.backgroundView = [ABI38_0_0AIREmptyCalloutBackgroundView new];
        } else {
            // the default tooltip look is wanted, and the user is just filling the content with their ABI38_0_0React subviews.
            // as a result, we use the default "masked" background view.
            calloutView.backgroundView = [ABI38_0_0SMCalloutMaskedBackgroundView new];
        }

        // when this is set, the callout's content will be whatever ABI38_0_0React views the user has put as the callout's
        // children.
...
- (void)_handleTap:(UITapGestureRecognizer *)recognizer {
    ABI38_0_0AIRMapMarker *marker = self;
    if (!marker) return;
...
{
    return self.ABI38_0_0ReactSubviews.count == 0 && !self.imageSrc;
}
...
- (void)setZIndex:(NSInteger)zIndex
{
    ...
    _zIndex = _calloutIsOpen ? zIndex + ABI38_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE : zIndex;
