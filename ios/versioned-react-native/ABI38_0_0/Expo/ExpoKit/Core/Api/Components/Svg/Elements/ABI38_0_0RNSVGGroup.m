...
@implementation ABI38_0_0RNSVGGroup
{
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI38_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI38_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI38_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI38_0_0RNSVGMask class]]) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI38_0_0RNSVGNode class]];
    ...
    for (UIView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI38_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI38_0_0RNSVGMask class]]) {
                continue;
            }
            ABI38_0_0RNSVGNode* svgNode = (ABI38_0_0RNSVGNode*)node;
            if (event) {
...
{
    [self traverseSubviews:^(__kindof ABI38_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI38_0_0RNSVGRenderable class]]) {
            [(ABI38_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
