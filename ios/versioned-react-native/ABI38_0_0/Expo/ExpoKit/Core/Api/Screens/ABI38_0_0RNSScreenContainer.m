...
@interface ABI38_0_0RNSScreenContainerManager : ABI38_0_0RCTViewManager

- (void)markUpdated:(ABI38_0_0RNSScreenContainerView *)screen;

@end

@interface ABI38_0_0RNSScreenContainerView () <ABI38_0_0RCTInvalidating>

@property (nonatomic, retain) UIViewController *controller;
@property (nonatomic, retain) NSMutableSet<ABI38_0_0RNSScreenView *> *activeScreens;
@property (nonatomic, retain) NSMutableArray<ABI38_0_0RNSScreenView *> *ABI38_0_0ReactSubviews;

- (void)updateContainer;

@end

@implementation ABI38_0_0RNSScreenContainerView {
  BOOL _needUpdate;
  __weak ABI38_0_0RNSScreenContainerManager *_manager;
}

- (instancetype)initWithManager:(ABI38_0_0RNSScreenContainerManager *)manager
{
  if (self = [super init]) {
...
- (void)insertABI38_0_0ReactSubview:(ABI38_0_0RNSScreenView *)subview atIndex:(NSInteger)atIndex
{
  ...
  [subview ABI38_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
}
...
- (UIViewController*)findChildControllerForScreen:(ABI38_0_0RNSScreenView*)screen
{
...
- (void)prepareDetach:(ABI38_0_0RNSScreenView *)screen
{
  [[self findChildControllerForScreen:screen] willMoveToParentViewController:nil];
}

- (void)detachScreen:(ABI38_0_0RNSScreenView *)screen
{
...
- (void)attachScreen:(ABI38_0_0RNSScreenView *)screen atIndex:(NSInteger)index
{
  [_controller addChildViewController:screen.controller];
  // the frame is already set at this moment because we adjust it in insertABI38_0_0ReactSubview. We don't
  // want to update it here as ABI38_0_0React-driven animations may already run and hence changing the frame
...
- (void)updateContainer
{
  ...
  for (ABI38_0_0RNSScreenView *screen in orphaned) {
    activeScreenRemoved = YES;
    [self detachScreen:screen];
  }
  ...
  if ((activeScreenRemoved || activeScreenAdded) && _activeScreens.count == 1) {
    ABI38_0_0RNSScreenView *singleActiveScreen = [_activeScreens anyObject];
...
- (void)layoutSubviews
{
  ...
  for (ABI38_0_0RNSScreenView *subview in _ABI38_0_0ReactSubviews) {
    [subview ABI38_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
@implementation ABI38_0_0RNSScreenContainerManager {
  NSMutableArray<ABI38_0_0RNSScreenContainerView *> *_markedContainers;
}
...
- (UIView *)view
{
  ...
  return [[ABI38_0_0RNSScreenContainerView alloc] initWithManager:self];
}
...
- (void)markUpdated:(ABI38_0_0RNSScreenContainerView *)screen
{
  ...
  if ([_markedContainers count] == 1) {
    ...
    ABI38_0_0RCTExecuteOnMainQueue(^{
      for (ABI38_0_0RNSScreenContainerView *container in self->_markedContainers) {
