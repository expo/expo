...
#import "ABI38_0_0EXDisabledDevLoadingView.h"
#import "ABI38_0_0EXDisabledDevMenu.h"
#import "ABI38_0_0EXDisabledRedBox.h"
#import "ABI38_0_0EXFileSystem.h"
#import "ABI38_0_0EXVersionManager.h"
#import "ABI38_0_0EXScopedBridgeModule.h"
#import "ABI38_0_0EXStatusBarManager.h"
...
#import <ABI38_0_0React/ABI38_0_0RCTExceptionsManager.h>
...
#import <ABI38_0_0React/ABI38_0_0RCTAsyncLocalStorage.h>
...
#define ABI38_0_0EX_KERNEL_SERVICE_NONE @"ABI38_0_0EXKernelServiceNone"

// this is needed because ABI38_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI38_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSDictionary *> *ABI38_0_0EXScopedModuleClasses;
void ABI38_0_0EXRegisterScopedModule(Class, ...);
void ABI38_0_0EXRegisterScopedModule(Class moduleClass, ...)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI38_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  ...
  va_start(argumentList, moduleClass);
    while ((kernelServiceClassName = va_arg(argumentList, NSString*))) {
      if ([kernelServiceClassName isEqualToString:@"nil"]) {
        unversionedKernelServiceClassNames[kernelServiceClassName] = ABI38_0_0EX_KERNEL_SERVICE_NONE;
      } else {
  ...
  if (moduleClassName) {
    ABI38_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassNames;
  }
}

@interface ABI38_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI38_0_0EXVersionManager ()
...
- (void)bridgeWillStartLoading:(id)bridge
{
  ...
  // Keep in mind that it is possible this will return a ABI38_0_0EXDisabledRedBox
  ABI38_0_0RCTRedBox *redBox = [self _moduleInstanceForBridge:bridge named:@"RedBox"];
  [redBox setOverrideReloadAction:^{
      [[NSNotificationCenter defaultCenter]
     postNotificationName:@"EXReloadActiveAppRequest" object:nil];
  }];

  // We need to check DEBUG flag here because in ejected projects ABI38_0_0RCT_DEV is set only for ABI38_0_0React and not for ExpoKit to which this file belongs to.
  // It can be changed to just ABI38_0_0RCT_DEV once we deprecate ExpoKit and set that flag for the entire standalone project.
#if DEBUG || ABI38_0_0RCT_DEV
  if ([self _isDevModeEnabledForBridge:bridge]) {
    // Set the bundle url for the packager connection manually
    [[ABI38_0_0RCTPackagerConnection sharedPackagerConnection] setBundleURL:[bridge bundleURL]];
  }
#endif

  // Manually send a "start loading" notif, since the real one happened uselessly inside the ABI38_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
...
{
  ABI38_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI38_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI38_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI38_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return ([ABI38_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI38_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI38_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI38_0_0RCTSetFatalHandler(fatalHandler);
  ABI38_0_0RCTSetLogThreshold(threshold);
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI38_0_0EXAppState alloc] init],
                                    [[ABI38_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:(!isOpeningHomeInProductionMode && isDeveloper)],
                                    [[ABI38_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI38_0_0EXStatusBarManager alloc] init],
                                    ]];
  ...
  if (exceptionsManagerDelegate) {
    ABI38_0_0RCTExceptionsManager *exceptionsManager = [[ABI38_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
    [extraModules addObject:exceptionsManager];
  } else {
    ABI38_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
  }
  
  if (params[@"testEnvironment"]) {
    ABI38_0_0EXTestEnvironment testEnvironment = (ABI38_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI38_0_0EXTestEnvironmentNone) {
      ABI38_0_0EXTest *testModule = [[ABI38_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI38_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI38_0_0RCTDevMenu
    [extraModules addObject:[[ABI38_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI38_0_0RCTRedBox
    [extraModules addObject:[[ABI38_0_0EXDisabledRedBox alloc] init]];
  }

  ABI38_0_0UMModuleRegistryProvider *moduleRegistryProvider = [[ABI38_0_0UMModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI38_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI38_0_0UMModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

  ABI38_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI38_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];
  ABI38_0_0UMModuleRegistry *moduleRegistry = [moduleRegistryAdapter moduleRegistryForParams:params forExperienceId:experienceId withKernelServices:services];
  NSArray<id<ABI38_0_0RCTBridgeModule>> *expoModules = [moduleRegistryAdapter extraModulesForModuleRegistry:moduleRegistry];
  [extraModules addObjectsFromArray:expoModules];

  id<ABI38_0_0UMFileSystemInterface> fileSystemModule = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI38_0_0UMFileSystemInterface)];
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI38_0_0EXScopedModuleClasses) {
    [ABI38_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI38_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
