...
#import <ABI38_0_0UMReactNativeAdapter/ABI38_0_0UMReactNativeEventEmitter.h>
...
@interface ABI38_0_0UMReactNativeEventEmitter ()
...
@implementation ABI38_0_0UMReactNativeEventEmitter
...
+ (NSString *)moduleName
{
  return @"ABI38_0_0UMReactNativeEventEmitter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI38_0_0UMEventEmitterService)];
}

- (NSArray<NSString *> *)supportedEvents
{
  NSMutableSet<NSString *> *eventsAccumulator = [NSMutableSet set];
  for (ABI38_0_0UMExportedModule *exportedModule in [_moduleRegistry getAllExportedModules]) {
    if ([exportedModule conformsToProtocol:@protocol(ABI38_0_0UMEventEmitter)]) {
      id<ABI38_0_0UMEventEmitter> eventEmitter = (id<ABI38_0_0UMEventEmitter>)exportedModule;
...
ABI38_0_0RCT_EXPORT_METHOD(addProxiedListener:(NSString *)moduleName eventName:(NSString *)eventName)
{
  [self addListener:eventName];
  // Validate module
  ABI38_0_0UMExportedModule *module = [_moduleRegistry getExportedModuleForName:moduleName];
  
  if (ABI38_0_0RCT_DEBUG && module == nil) {
    ABI38_0_0UMLogError(@"Module for name `%@` has not been found.", moduleName);
    return;
  } else if (ABI38_0_0RCT_DEBUG && ![module conformsToProtocol:@protocol(ABI38_0_0UMEventEmitter)]) {
    ABI38_0_0UMLogError(@"Module `%@` is not an ABI38_0_0UMEventEmitter, thus it cannot be subscribed to.", moduleName);
    return;
  }

  // Validate eventEmitter
  id<ABI38_0_0UMEventEmitter> eventEmitter = (id<ABI38_0_0UMEventEmitter>)module;

  if (ABI38_0_0RCT_DEBUG && ![[eventEmitter supportedEvents] containsObject:eventName]) {
    ABI38_0_0UMLogError(@"`%@` is not a supported event type for %@. Supported events are: `%@`",
...
ABI38_0_0RCT_EXPORT_METHOD(removeProxiedListeners:(NSString *)moduleName count:(double)count)
{
  ...
  if (newModuleListenersCount == 0) {
    ...
    ABI38_0_0UMLogError(@"Attempted to remove more `%@` listeners than added", moduleName);
  ...
  if (_listenersCount - 1 < 0) {
    ABI38_0_0UMLogError(@"Attempted to remove more proxied event emitter listeners than added");
    _listenersCount = 0;
  } else {
    _listenersCount -= 1;
  }
