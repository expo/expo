...
@interface ABI38_0_0EXAuthTask ()

@property (nonatomic, strong) ABI38_0_0UMPromiseResolveBlock resolver;
@property (nonatomic, strong) ABI38_0_0UMPromiseRejectBlock rejecter;
@property (nonatomic, strong) NSString *tag;

@end

@implementation ABI38_0_0EXAuthTask
      ...
      rejecter:(ABI38_0_0UMPromiseRejectBlock)reject
{
  if (!_resolver) {
    ...
    reject(ABI38_0_0EX_E_CONCURRENT_TASK_IN_PROGRESS, [NSString stringWithFormat:@"Failed to start %@ as a concurrent GoogleSignIn task is already running", tag], nil);
    return NO;
  }
}
...
- (void)parse:(NSDictionary *)user error:(NSError *)error
{
  if (error) {
    ...
    [self reject:[ABI38_0_0EXGoogleSignIn jsonFromGIDSignInErrorCode:error.code] error:error];
  } else {
    [self resolve:ABI38_0_0UMNullIfNil(user)];
  }
}
