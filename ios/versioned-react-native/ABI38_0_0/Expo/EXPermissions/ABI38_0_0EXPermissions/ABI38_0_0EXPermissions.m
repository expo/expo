...
NSString * const ABI38_0_0EXStatusKey = @"status";
NSString * const ABI38_0_0EXExpiresKey = @"expires";
NSString * const ABI38_0_0EXGrantedKey = @"granted";
NSString * const ABI38_0_0EXCanAskAgain = @"canAskAgain";

NSString * const ABI38_0_0EXPermissionExpiresNever = @"never";

@interface ABI38_0_0EXPermissions ()

@property (nonatomic, strong) NSMutableDictionary<NSString *, id<ABI38_0_0UMPermissionsRequester>> *requesters;
@property (nonatomic, strong) NSMapTable<Class, id<ABI38_0_0UMPermissionsRequester>> *requestersByClass;
@property (nonatomic, weak) ABI38_0_0UMModuleRegistry *moduleRegistry;

@end

@implementation ABI38_0_0EXPermissions

ABI38_0_0UM_EXPORT_MODULE(ExpoPermissions);

- (instancetype)init
{
  if (self = [super init]) {
    _requesters = [NSMutableDictionary<NSString *, id<ABI38_0_0UMPermissionsRequester>> new];
    _requestersByClass = [NSMapTable<Class, id<ABI38_0_0UMPermissionsRequester>> new];
  }
  return self;
}
...
{
  return @[@protocol(ABI38_0_0UMPermissionsInterface)];
}

- (void)registerRequesters:(NSArray<id<ABI38_0_0UMPermissionsRequester>> *)newRequesters {
  for (id<ABI38_0_0UMPermissionsRequester> requester in newRequesters) {
...
ABI38_0_0UM_EXPORT_METHOD_AS(getAsync,
...
{
  id<ABI38_0_0UMPermissionsRequester> requester = [self getPermissionRequesterForType:permissionType];
  if (requester == nil) {
...
ABI38_0_0UM_EXPORT_METHOD_AS(askAsync,
...
- (NSDictionary *)getPermissionUsingRequester:(id<ABI38_0_0UMPermissionsRequester>)requester
{
  if (requester) {
    return [ABI38_0_0EXPermissions parsePermissionFromRequester:[requester getPermissions]];
  }
  return nil;
}
...
- (BOOL)hasGrantedPermissionUsingRequesterClass:(Class)requesterClass
{
  ...
  if (!permissions) {
    ABI38_0_0UMLogWarn(@"Permission requester '%@' not found.", NSStringFromClass(requesterClass));
    return false;
  }
  
  return [permissions[ABI38_0_0EXStatusKey] isEqualToString:@"granted"];
}

- (void)askForPermissionUsingRequesterClass:(Class)requesterClass
                                    resolve:(ABI38_0_0UMPromiseResolveBlock)onResult
                                     reject:(ABI38_0_0UMPromiseRejectBlock)reject
{
  ...
  BOOL isGranted = [ABI38_0_0EXPermissions statusForPermission:permission] == ABI38_0_0UMPermissionStatusGranted;
...
{
  id<ABI38_0_0UMPermissionsRequester> requester = [self getPermissionRequesterForClass:requesterClass];
  if (requester == nil) {
  ...
  void (^permissionParser)(NSDictionary *) = ^(NSDictionary * permission){
    resolver([ABI38_0_0EXPermissions parsePermissionFromRequester:permission]);
  };
...
+ (NSDictionary *)parsePermissionFromRequester:(NSDictionary *)permission
{
  ...
  ABI38_0_0UMPermissionStatus status = (ABI38_0_0UMPermissionStatus)[permission[ABI38_0_0EXStatusKey] intValue];
  BOOL isGranted = status == ABI38_0_0UMPermissionStatusGranted;
  BOOL canAskAgain = status != ABI38_0_0UMPermissionStatusDenied;
  
  [parsedPermission setValue:[[self class] permissionStringForStatus:status] forKey:ABI38_0_0EXStatusKey];
  [parsedPermission setValue:ABI38_0_0EXPermissionExpiresNever forKey:ABI38_0_0EXExpiresKey];
  [parsedPermission setValue:@(isGranted) forKey:ABI38_0_0EXGrantedKey];
  [parsedPermission setValue:@(canAskAgain) forKey:ABI38_0_0EXCanAskAgain];
...
{
  switch (status) {
    case ABI38_0_0UMPermissionStatusGranted:
      return @"granted";
    case ABI38_0_0UMPermissionStatusDenied:
      return @"denied";
    default:
      return @"undetermined";
  }
...
{
  NSString *status = permission[ABI38_0_0EXStatusKey];
  if ([status isEqualToString:@"granted"]) {
    return ABI38_0_0UMPermissionStatusGranted;
  } else if ([status isEqualToString:@"denied"]) {
    return ABI38_0_0UMPermissionStatusDenied;
  } else {
    return ABI38_0_0UMPermissionStatusUndetermined;
  }
}

- (id<ABI38_0_0UMPermissionsRequester>)getPermissionRequesterForType:(NSString *)type
{
  return _requesters[type];
}

- (id<ABI38_0_0UMPermissionsRequester>)getPermissionRequesterForClass:(Class)requesterClass
{
