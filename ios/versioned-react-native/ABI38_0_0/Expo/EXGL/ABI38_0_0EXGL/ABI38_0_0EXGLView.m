...
@implementation ABI38_0_0EXGLView
...
- (instancetype)initWithModuleRegistry:(ABI38_0_0UMModuleRegistry *)moduleRegistry
{
  if ((self = [super init])) {
    ...
    self.contentScaleFactor = [ABI38_0_0UMUtilities screenScale];
    ...
    _glContext = [[ABI38_0_0EXGLContext alloc] initWithDelegate:self andModuleRegistry:moduleRegistry];
...
- (void)maybeCallSurfaceCreated
{
  ...
  if (_onSurfaceCreate && _glContext.isInitialized && _isLayouted) {
    ...
    UEXGLContextSetDefaultFramebuffer__Legacy(exglCtxId, _msaaFramebuffer);
...
- (void)setOnSurfaceCreate:(ABI38_0_0UMDirectEventBlock)onSurfaceCreate
{
...
- (void)resizeViewBuffersToWidth:(short)width height:(short)height
{
  ...
  [_glContext runAsync:^{
    ...
    if (self->_glContext.isInitialized) {
      UEXGLContextSetDefaultFramebuffer__Legacy(self->_glContext.contextId, self->_msaaFramebuffer);
    }
...
- (void)draw
{
  ...
  // called on the JS thread to create the ABI38_0_0EXGL context and save its id (see ABI38_0_0EXGLContext.initializeContextWithBridge method).
