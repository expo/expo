...
static NSString *const ABI38_0_0EXAppAuthError = @"ERR_APP_AUTH";

@interface ABI38_0_0EXAppAuth ()
...
@property (nonatomic, weak) id<ABI38_0_0EXAppAuthSessionsManagerInterface> sessionsManager;

@end

@implementation ABI38_0_0EXAppAuth

#pragma mark - Expo

ABI38_0_0UM_EXPORT_MODULE(ExpoAppAuth);
...
{
  return @{
           @"OAuthRedirect": ABI38_0_0UMNullIfNil([self _getOAuthRedirect]),
           @"URLSchemes": ABI38_0_0UMNullIfNil([[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleURLTypes"]),
           };
}

ABI38_0_0UM_EXPORT_METHOD_AS(executeAsync,
                             ...
                             reject:(ABI38_0_0UMPromiseRejectBlock)reject
{
  ...
  [ABI38_0_0UMUtilities performSynchronouslyOnMainThread:^{
    ...
    __weak id<ABI38_0_0EXAppAuthSessionsManagerInterface> sessionsManager = self->_sessionsManager;
    OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
      ...
      if (authState) {
        NSDictionary *tokenResponse = [ABI38_0_0EXAppAuth _tokenResponseNativeToJSON:authState.lastTokenResponse request:options];
        resolve(tokenResponse);
      } else {
        ABI38_0_0EXrejectWithError(reject, error);
      }
    };
...
{
  NSDictionary *tokenResponse = [ABI38_0_0EXAppAuth tokenResponseNativeToJSON:input];
  ...
  [output setValue:ABI38_0_0UMNullIfNil(refreshToken) forKey:@"refreshToken"];
...
- (void)_refreshWithConfiguration:(OIDServiceConfiguration *)configuration
  ...
  OIDTokenCallback callback = ^(OIDTokenResponse *_Nullable response, NSError *_Nullable error) {
    if (response) {
      NSDictionary *tokenResponse = [ABI38_0_0EXAppAuth _tokenResponseNativeToJSON:response request:options];
      resolve(tokenResponse);
    } else {
      ABI38_0_0EXrejectWithError(reject, error);
    }
...
// ABI38_0_0EX prefix for versioning to work smoothly
void ABI38_0_0EXrejectWithError(ABI38_0_0UMPromiseRejectBlock reject, NSError *error) {
  NSString *errorMessage = [NSString stringWithFormat:@"%@: %@", ABI38_0_0EXAppAuthError, error.localizedDescription];
