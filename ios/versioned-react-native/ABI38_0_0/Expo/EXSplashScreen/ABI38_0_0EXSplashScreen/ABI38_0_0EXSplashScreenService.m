...
@interface ABI38_0_0EXSplashScreenService ()

@property (nonatomic, strong) NSMapTable<UIViewController *, ABI38_0_0EXSplashScreenController *> *splashScreenControllers;

@end

@implementation ABI38_0_0EXSplashScreenService

ABI38_0_0UM_REGISTER_SINGLETON_MODULE(SplashScreen);
...
{
  id<ABI38_0_0EXSplashScreenViewProvider> splashScreenViewProvider = [ABI38_0_0EXSplashScreenViewNativeProvider new];
  return [self showSplashScreenFor:viewController
          splashScreenViewProvider:splashScreenViewProvider
                   successCallback:^{}
                   failureCallback:^(NSString *message){ ABI38_0_0UMLogWarn(@"%@", message); }];
}
            ...
            failureCallback:(void (^)(NSString * _Nonnull))failureCallback
{
  ...
  ABI38_0_0EXSplashScreenController *splashScreenController = [[ABI38_0_0EXSplashScreenController alloc] initWithViewController:viewController
                                                                                     splashScreenViewProvider:splashScreenViewProvider];
...
{
  if (![self.splashScreenControllers objectForKey:viewController]) {
    ABI38_0_0UMLogWarn(@"No native splash screen registered for given view controller. Call 'SplashScreen.show' for given view controller first.");
  }
