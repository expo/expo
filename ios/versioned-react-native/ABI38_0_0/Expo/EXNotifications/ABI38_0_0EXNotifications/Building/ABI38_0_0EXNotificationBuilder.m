...
@implementation ABI38_0_0EXNotificationBuilder

ABI38_0_0UM_REGISTER_MODULE();

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI38_0_0EXNotificationBuilder)];
}
...
- (UNNotificationAttachment *)attachmentFromRequest:(NSDictionary *)request
{
  ...
  if (error) {
    ABI38_0_0UMLogWarn(@"[expo-notifications] Could not have created a notification attachment out of request: %@. Error: %@.", [request description], [error description]);
    return nil;
  }
  return attachment;
}
