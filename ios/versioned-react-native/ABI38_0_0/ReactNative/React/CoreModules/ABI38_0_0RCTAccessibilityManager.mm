...
#import "ABI38_0_0RCTAccessibilityManager.h"
...
@implementation ABI38_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI38_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI38_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI38_0_0RCTLogError(@"Can't determine multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI38_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(JS::NativeAccessibilityManager::SpecSetAccessibilityContentSizeMultipliersJSMultipliers &)JSMultipliers)
{
...
ABI38_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(double)ABI38_0_0ReactTag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForABI38_0_0ReactTag:@(ABI38_0_0ReactTag)];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI38_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI38_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI38_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI38_0_0RCT_EXPORT_METHOD(getCurrentBoldTextState:(ABI38_0_0RCTResponseSenderBlock)onSuccess
                  onError:(__unused ABI38_0_0RCTResponseSenderBlock)onError)
{
  onSuccess(@[@(_isBoldTextEnabled)]);
}

ABI38_0_0RCT_EXPORT_METHOD(getCurrentGrayscaleState:(ABI38_0_0RCTResponseSenderBlock)onSuccess
                  onError:(__unused ABI38_0_0RCTResponseSenderBlock)onError)
{
  onSuccess(@[@(_isGrayscaleEnabled)]);
}

ABI38_0_0RCT_EXPORT_METHOD(getCurrentInvertColorsState:(ABI38_0_0RCTResponseSenderBlock)onSuccess
                  onError:(__unused ABI38_0_0RCTResponseSenderBlock)onError)
{
  onSuccess(@[@(_isInvertColorsEnabled)]);
}

ABI38_0_0RCT_EXPORT_METHOD(getCurrentReduceMotionState:(ABI38_0_0RCTResponseSenderBlock)onSuccess
                  onError:(__unused ABI38_0_0RCTResponseSenderBlock)onError)
{
  onSuccess(@[@(_isReduceMotionEnabled)]);
}

ABI38_0_0RCT_EXPORT_METHOD(getCurrentReduceTransparencyState:(ABI38_0_0RCTResponseSenderBlock)onSuccess
                  onError:(__unused ABI38_0_0RCTResponseSenderBlock)onError)
{
  onSuccess(@[@(_isReduceTransparencyEnabled)]);
}

ABI38_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI38_0_0RCTResponseSenderBlock)onSuccess
                  onError:(__unused ABI38_0_0RCTResponseSenderBlock)onError)
{
...
{
  return std::make_shared<ABI38_0_0facebook::ABI38_0_0React::NativeAccessibilityManagerSpecJSI>(self, jsInvoker);
}

@end

@implementation ABI38_0_0RCTBridge (ABI38_0_0RCTAccessibilityManager)

- (ABI38_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI38_0_0RCTAccessibilityManager class]];
}

@end

Class ABI38_0_0RCTAccessibilityManagerCls(void)
{
  return ABI38_0_0RCTAccessibilityManager.class;
}
