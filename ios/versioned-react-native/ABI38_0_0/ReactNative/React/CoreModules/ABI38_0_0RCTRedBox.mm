...
#import <ABI38_0_0React/ABI38_0_0RCTRedBoxExtraDataViewController.h>
...
@class ABI38_0_0RCTRedBoxWindow;

@interface UIButton (ABI38_0_0RCTRedBox)

@property (nonatomic) ABI38_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI38_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI38_0_0RCTRedBox)

- (ABI38_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI38_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI38_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI38_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI38_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI38_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI38_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI38_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI38_0_0RCTRedBoxWindow
{
    UITableView *_stackTraceTableView;
    NSString *_lastErrorMessage;
    NSArray<ABI38_0_0RCTJSStackFrame *> *_lastStackTrace;
    int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame customButtonTitles:(NSArray<NSString *>*)customButtonTitles customButtonHandlers:(NSArray<ABI38_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title accessibilityIdentifier:(NSString *)accessibilityIdentifier selector:(SEL)selector block:(ABI38_0_0RCTRedBoxButtonPressHandler)block
{
...
- (NSInteger)bottomSafeViewHeight
{
    if (@available(iOS 11.0, *)) {
        return ABI38_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
    } else {
        return 0;
    }
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate errorCookie:(int)errorCookie
{
...
- (void)copyStack
{
    ...
    for (ABI38_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI38_0_0RCTJSStackFrame *)stackFrame
{
    NSString *fileName = ABI38_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ABI38_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI38_0_0RCTJSStackFrame *)stackFrame
{
    if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 1) {
        ...
        ABI38_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
    // NOTE: We could use ABI38_0_0RCTKeyCommands for this, but since
...
{
    ABI38_0_0RCTRedBoxWindow *_window;
    NSMutableArray<id<ABI38_0_0RCTErrorCustomizer>> *_errorCustomizers;
    ABI38_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
    NSMutableArray<NSString *> *_customButtonTitles;
    NSMutableArray<ABI38_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (void)registerErrorCustomizer:(id<ABI38_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI38_0_0RCTErrorInfo *)_customizeError:(ABI38_0_0RCTErrorInfo *)error
{
    ABI38_0_0RCTAssertMainQueue();
    if (!self->_errorCustomizers) {
        return error;
    }
    for (id<ABI38_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
        ABI38_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
        if (newInfo) {
            error = newInfo;
        }
    }
...
- (void)showError:(NSError *)error
{
    [self showErrorMessage:error.localizedDescription
               withDetails:error.localizedFailureReason
                     stack:error.userInfo[ABI38_0_0RCTJSStackTraceKey]
                     errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message withDetails:(NSString *)details stack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack errorCookie:(int)errorCookie {
...
{
    NSArray<ABI38_0_0RCTJSStackFrame *> *stack = [ABI38_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
    [self showErrorMessage:message withParsedStack:[ABI38_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:NO errorCookie:errorCookie];
}

- (void)updateErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
    [self showErrorMessage:message withParsedStack:[ABI38_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:YES errorCookie:errorCookie];
}

- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack
{
    [self showErrorMessage:message withParsedStack:stack errorCookie:-1];
}

- (void)updateErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack
{
    [self updateErrorMessage:message withParsedStack:stack errorCookie:-1];
}

- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack errorCookie:(int)errorCookie
{
    [self showErrorMessage:message withParsedStack:stack isUpdate:NO errorCookie:errorCookie];
}

- (void)updateErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack errorCookie:(int)errorCookie
{
    [self showErrorMessage:message withParsedStack:stack isUpdate:YES errorCookie:errorCookie];
}

- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI38_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate errorCookie:(int)errorCookie
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_extraDataViewController == nil) {
            self->_extraDataViewController = [ABI38_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
        ...
        if (!self->_window) {
            self->_window = [[ABI38_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds customButtonTitles:self->_customButtonTitles customButtonHandlers:self->_customButtonHandlers];
            self->_window.actionDelegate = self;
        }

        ABI38_0_0RCTErrorInfo *errorInfo = [[ABI38_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                       stack:stack];
...
ABI38_0_0RCT_EXPORT_METHOD(setExtraData:(NSDictionary *)extraData forIdentifier:(NSString *)identifier) {
    [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI38_0_0RCT_EXPORT_METHOD(dismiss)
{
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI38_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI38_0_0RCTJSStackFrame *)stackFrame
{
    NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
    if (![bundleURL.scheme hasPrefix:@"http"]) {
        ABI38_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
        return;
    }

    NSData *stackFrameJSON = [ABI38_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI38_0_0RCTRedBoxWindow *)redBoxWindow
{
    if (_overrideReloadAction) {
        _overrideReloadAction();
    } else {
      ABI38_0_0RCTTriggerReloadCommandListeners(@"Redbox");
    }
    [self dismiss];
}

- (void)addCustomButton:(NSString *)title onPressHandler:(ABI38_0_0RCTRedBoxButtonPressHandler)handler
{
...
{
  return std::make_shared<ABI38_0_0facebook::ABI38_0_0React::NativeRedBoxSpecJSI>(self, jsInvoker);
}

@end

@implementation ABI38_0_0RCTBridge (ABI38_0_0RCTRedBox)

- (ABI38_0_0RCTRedBox *)redBox
{
  return ABI38_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI38_0_0RCTRedBox class]] : nil;
}
...
- (ABI38_0_0RCTRedBox *)redBox { return nil; }

@end

#endif

Class ABI38_0_0RCTRedBoxCls(void) {
  return ABI38_0_0RCTRedBox.class;
}
