...
#import <ABI38_0_0React/ABI38_0_0RCTSurfaceRootView.h>
#import <ABI38_0_0React/ABI38_0_0RCTSurfaceTouchHandler.h>
...
@implementation ABI38_0_0RCTFabricSurface {
  // Immutable
  ABI38_0_0RCTSurfacePresenter *_surfacePresenter;
  NSString *_moduleName;

  // Protected by the `_mutex`
  std::mutex _mutex;
  ABI38_0_0RCTSurfaceStage _stage;
  ...
  ABI38_0_0RCTSurfaceView *_Nullable _view;
  ABI38_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}

- (instancetype)initWithSurfacePresenter:(ABI38_0_0RCTSurfacePresenter *)surfacePresenter
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI38_0_0RCTAllocateRootViewTag() integerValue];

    _minimumSize = CGSizeZero;
    // FIXME: Replace with `_maximumSize = CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX);`.
    _maximumSize = ABI38_0_0RCTScreenSize();

    _touchHandler = [ABI38_0_0RCTSurfaceTouchHandler new];

    _stage = ABI38_0_0RCTSurfaceStageSurfaceDidInitialize;
  }
...
- (BOOL)start
{
  if (![self _setStage:ABI38_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI38_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI38_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI38_0_0RCTSurfaceView alloc] initWithSurface:(ABI38_0_0RCTSurface *)self];
...
- (ABI38_0_0RCTSurfaceStage)stage
{
  std::lock_guard<std::mutex> lock(_mutex);
  return _stage;
}

- (BOOL)_setStage:(ABI38_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:YES];
}

- (BOOL)_unsetStage:(ABI38_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:NO];
}

- (BOOL)_setStage:(ABI38_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
  ABI38_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);

    if (setOrUnset) {
      updatedStage = (ABI38_0_0RCTSurfaceStage)(_stage | stage);
    } else {
      updatedStage = (ABI38_0_0RCTSurfaceStage)(_stage & ~stage);
    }
...
- (void)_propagateStageChange:(ABI38_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI38_0_0RCTSurfaceDelegate> delegate = self.delegate;
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI38_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI38_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
