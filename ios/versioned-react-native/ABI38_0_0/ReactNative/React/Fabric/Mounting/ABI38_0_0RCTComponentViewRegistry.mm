...
@interface ABI38_0_0RCTUIManager (Hack)
...
@implementation ABI38_0_0RCTUIManager (Hack)
...
+ (void)registerView:(UIView *)view
{
  ...
  ABI38_0_0RCTUIManager *uiManager = [[ABI38_0_0RCTBridge currentBridge] uiManager];
  view.ABI38_0_0ReactTag = @(view.tag);
...
const NSInteger ABI38_0_0RCTComponentViewRegistryRecyclePoolMaxSize = 1024;

@implementation ABI38_0_0RCTComponentViewRegistry {
  better::map<Tag, ABI38_0_0RCTComponentViewDescriptor> _registry;
  better::map<ComponentHandle, std::vector<ABI38_0_0RCTComponentViewDescriptor>> _recyclePool;
}

- (instancetype)init
{
  if (self = [super init]) {
    _componentViewFactory = [ABI38_0_0RCTComponentViewFactory standardComponentViewFactory];
...
- (void)preallocateViewComponents
{
  ...
  std::vector<std::pair<ComponentHandle, float>> componentMagnitudes = {
      {[ABI38_0_0RCTViewComponentView componentDescriptorProvider].handle, 1},
      {[ABI38_0_0RCTImageComponentView componentDescriptorProvider].handle, 0.3},
      {[ABI38_0_0RCTParagraphComponentView componentDescriptorProvider].handle, 0.3},
  };
...
- (ABI38_0_0RCTComponentViewDescriptor)dequeueComponentViewWithComponentHandle:(ComponentHandle)componentHandle tag:(Tag)tag
{
  ABI38_0_0RCTAssertMainQueue();

  ABI38_0_0RCTAssert(
      _registry.find(tag) == _registry.end(),
      @"ABI38_0_0RCTComponentViewRegistry: Attempt to dequeue already registered component.");
...
#ifdef LEGACY_UIMANAGER_INTEGRATION_ENABLED
  [ABI38_0_0RCTUIManager registerView:componentViewDescriptor.view];
#endif
                        ...
                        componentViewDescriptor:(ABI38_0_0RCTComponentViewDescriptor)componentViewDescriptor
{
  ...
  ABI38_0_0RCTAssert(
      _registry.find(tag) != _registry.end(), @"ABI38_0_0RCTComponentViewRegistry: Attempt to enqueue unregistered component.");

#ifdef LEGACY_UIMANAGER_INTEGRATION_ENABLED
  [ABI38_0_0RCTUIManager unregisterView:componentViewDescriptor.view];
#endif
...
- (ABI38_0_0RCTComponentViewDescriptor const &)componentViewDescriptorWithTag:(Tag)tag
{
  ABI38_0_0RCTAssertMainQueue();
  auto iterator = _registry.find(tag);
  ABI38_0_0RCTAssert(iterator != _registry.end(), @"ABI38_0_0RCTComponentViewRegistry: Attempt to query unregistered component.");
  return iterator->second;
}

- (nullable UIView<ABI38_0_0RCTComponentViewProtocol> *)findComponentViewWithTag:(Tag)tag
{
  ABI38_0_0RCTAssertMainQueue();
  auto iterator = _registry.find(tag);
...
- (ABI38_0_0RCTComponentViewDescriptor)_dequeueComponentViewWithComponentHandle:(ComponentHandle)componentHandle
{
  ABI38_0_0RCTAssertMainQueue();
                         ...
                         componentViewDescriptor:(ABI38_0_0RCTComponentViewDescriptor)componentViewDescriptor
{
  ...
  if (recycledViews.size() > ABI38_0_0RCTComponentViewRegistryRecyclePoolMaxSize) {
    return;
  }
