...
#import <ABI38_0_0React/components/legacyviewmanagerinterop/LegacyViewManagerInteropComponentDescriptor.h>
#import <ABI38_0_0React/components/legacyviewmanagerinterop/LegacyViewManagerInteropViewProps.h>
#import <ABI38_0_0React/utils/ManagedObjectWrapper.h>
#import "ABI38_0_0RCTLegacyViewManagerInteropCoordinatorAdapter.h"

using namespace ABI38_0_0facebook::ABI38_0_0React;

@implementation ABI38_0_0RCTLegacyViewManagerInteropComponentView {
  NSMutableDictionary<NSNumber *, UIView *> *_viewsToBeMounted;
  NSMutableArray<UIView *> *_viewsToBeUnmounted;
  ABI38_0_0RCTLegacyViewManagerInteropCoordinatorAdapter *_adapter;
...
{
  return [[ABI38_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] containsObject:componentName];
}

+ (void)supportLegacyViewManagerWithName:(NSString *)componentName
{
  [[ABI38_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] addObject:componentName];
}

- (ABI38_0_0RCTLegacyViewManagerInteropCoordinator *)coordinator
{
  if (_state != nullptr) {
...
{
  const auto &state = _state->getData();
  ABI38_0_0RCTLegacyViewManagerInteropCoordinator *coordinator = unwrapManagedObject(state.coordinator);
...
- (void)finalizeUpdates:(ABI38_0_0RNComponentViewUpdateMask)updateMask
{
  ...
  if (!_adapter) {
    _adapter = [[ABI38_0_0RCTLegacyViewManagerInteropCoordinatorAdapter alloc] initWithCoordinator:self.coordinator
                                                                                 ABI38_0_0ReactTag:self.tag];
  ...
  for (NSNumber *key in _viewsToBeMounted) {
    [_adapter.paperView insertABI38_0_0ReactSubview:_viewsToBeMounted[key] atIndex:key.integerValue];
  }

  [_viewsToBeMounted removeAllObjects];

  for (UIView *view in _viewsToBeUnmounted) {
    [_adapter.paperView removeABI38_0_0ReactSubview:view];
  }

  [_viewsToBeUnmounted removeAllObjects];

  [_adapter.paperView didUpdateABI38_0_0ReactSubviews];

  if (updateMask & ABI38_0_0RNComponentViewUpdateMaskProps) {
