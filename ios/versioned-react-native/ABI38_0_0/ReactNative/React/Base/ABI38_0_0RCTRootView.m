...
NSString *const ABI38_0_0RCTContentDidAppearNotification = @"ABI38_0_0RCTContentDidAppearNotification";

@interface ABI38_0_0RCTUIManager (ABI38_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI38_0_0RCTRootView
{
  ABI38_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  ABI38_0_0RCTRootContentView *_contentView;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI38_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI38_0_0RCTRootView");
  ABI38_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI38_0_0RCTRootView");

  ABI38_0_0RCT_PROFILE_BEGIN_EVENT(ABI38_0_0RCTProfileTagAlways, @"-[ABI38_0_0RCTRootView init]", nil);
  if (!bridge.isLoading) {
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _sizeFlexibility = ABI38_0_0RCTRootViewSizeFlexibilityNone;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI38_0_0RCTContentDidAppearNotification
                                               object:self];
...
{
  if (self.ABI38_0_0ReactPreferredFocusedView) {
    return self.ABI38_0_0ReactPreferredFocusedView;
  }
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  fitSize = CGSizeMake(
      _sizeFlexibility & ABI38_0_0RCTRootViewSizeFlexibilityWidth ? fitSize.width : currentSize.width,
      _sizeFlexibility & ABI38_0_0RCTRootViewSizeFlexibilityHeight ? fitSize.height : currentSize.height
    );
...
{
  ABI38_0_0RCTAssertMainQueue();
  if (!super.ABI38_0_0ReactTag) {
    /**
     * Every root view that is created must have a unique ABI38_0_0React tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ABI38_0_0React tag must be re-assigned every time a new UIManager is created.
     */
    self.ABI38_0_0ReactTag = ABI38_0_0RCTAllocateRootViewTag();
  }
  return super.ABI38_0_0ReactTag;
}

- (void)bridgeDidReload
{
  ABI38_0_0RCTAssertMainQueue();
  // Clear the ABI38_0_0ReactTag so it can be re-assigned
  self.ABI38_0_0ReactTag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI38_0_0RCTRootContentView is scoped to the right bridge
...
- (void)bundleFinishedLoading:(ABI38_0_0RCTBridge *)bridge
{
  ABI38_0_0RCTAssert(bridge != nil, @"Bridge cannot be nil");
  if (!bridge.valid) {
    return;
  }

  [_contentView removeFromSuperview];
  _contentView = [[ABI38_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
  ...
  if (_sizeFlexibility == ABI38_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI38_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag": _contentView.ABI38_0_0ReactTag,
    @"initialProps": _appProperties ?: @{},
  };

  ABI38_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
@implementation ABI38_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI38_0_0RCTLogWarn(@"Calling deprecated `[-ABI38_0_0RCTRootView intrinsicSize]`.");
  return self.intrinsicContentSize;
}

- (void)cancelTouches
{
  ABI38_0_0RCTLogWarn(@"`-[ABI38_0_0RCTRootView cancelTouches]` is deprecated and will be deleted soon.");
