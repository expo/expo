...
#import "ABI38_0_0RCTComponentEvent.h"
#import "ABI38_0_0RCTProfile.h"
#import "ABI38_0_0RCTUtils.h"

const NSInteger ABI38_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI38_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI38_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(
    viewTag.intValue |
...
static uint16_t ABI38_0_0RCTUniqueCoalescingKeyGenerator = 0;

@implementation ABI38_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI38_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI38_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI38_0_0RCTDeviceEventEmitter"
                  method:@"emit"
                    args:body ? @[name, body] : @[name]
              completion:NULL];
}

- (void)sendTextEventWithType:(ABI38_0_0RCTTextEventType)type
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI38_0_0RCTComponentEvent *event = [[ABI38_0_0RCTComponentEvent alloc] initWithName:events[type]
                                                             viewTag:ABI38_0_0ReactTag
                                                                body:body];
  [self sendEvent:event];
}

- (void)sendEvent:(id<ABI38_0_0RCTEvent>)event
{
  [_observersLock lock];

  for (id<ABI38_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI38_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI38_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI38_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI38_0_0RCTGetEventID(event.viewTag, event.eventName, ABI38_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI38_0_0RCTAssert(previousEvent == nil, @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
    [_eventQueue addObject:eventID];
  }
...
- (void)addDispatchObserver:(id<ABI38_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI38_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
@implementation ABI38_0_0RCTBridge (ABI38_0_0RCTEventDispatcher)

- (ABI38_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI38_0_0RCTEventDispatcher class]];
}
