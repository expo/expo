...
#import "ABI38_0_0RCTJavaScriptLoader.h"
...
#import "ABI38_0_0RCTMultipartDataTask.h"
#import "ABI38_0_0RCTPerformanceLogger.h"
#import "ABI38_0_0RCTUtils.h"

NSString *const ABI38_0_0RCTJavaScriptLoaderErrorDomain = @"ABI38_0_0RCTJavaScriptLoaderErrorDomain";

static const int32_t JSNoBytecodeFileFormatVersion = -1;

@interface ABI38_0_0RCTSource()
{
@public
  NSURL *_url;
  NSData *_data;
...
@implementation ABI38_0_0RCTSource

static ABI38_0_0RCTSource *ABI38_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  ABI38_0_0RCTSource *source = [ABI38_0_0RCTSource new];
  source->_url = url;
  source->_data = data;
  source->_length = length;
  source->_filesChangedCount = ABI38_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI38_0_0RCTLoadingProgress
...
@implementation ABI38_0_0RCTJavaScriptLoader

ABI38_0_0RCT_NOT_IMPLEMENTED(- (instancetype)init)

+ (void)loadBundleAtURL:(NSURL *)scriptURL onProgress:(ABI38_0_0RCTSourceLoadProgressBlock)onProgress onComplete:(ABI38_0_0RCTSourceLoadBlock)onComplete
{
  ...
  NSData *data = [self attemptSynchronousLoadOfBundleAtURL:scriptURL
                                          runtimeBCVersion:JSNoBytecodeFileFormatVersion
                                              sourceLength:&sourceLength
                                                     error:&error];
  if (data) {
    onComplete(nil, ABI38_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError =
  [error.domain isEqualToString:ABI38_0_0RCTJavaScriptLoaderErrorDomain]
  && error.code == ABI38_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI38_0_0RCTJavaScriptLoaderErrorNoScriptURL
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI38_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI38_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  ABI38_0_0facebook::ABI38_0_0React::BundleHeader header;
  size_t readResult = fread(&header, sizeof(header), 1, bundle);
  fclose(bundle);
  if (readResult != 1) {
    if (error) {
      *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI38_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  ABI38_0_0facebook::ABI38_0_0React::ScriptTag tag = ABI38_0_0facebook::ABI38_0_0React::parseTypeFromHeader(header);
  switch (tag) {
  case ABI38_0_0facebook::ABI38_0_0React::ScriptTag::RAMBundle:
    break;

  case ABI38_0_0facebook::ABI38_0_0React::ScriptTag::String: {
  ...
  case ABI38_0_0facebook::ABI38_0_0React::ScriptTag::BCBundle:
    if (runtimeBCVersion == JSNoBytecodeFileFormatVersion || runtimeBCVersion < 0) {
      if (error) {
        *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI38_0_0RCTJavaScriptLoaderErrorBCNotSupported
                                 userInfo:@{NSLocalizedDescriptionKey:
      ...
      if (error) {
        ...
        *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI38_0_0RCTJavaScriptLoaderErrorBCVersion
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI38_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                               userInfo:@{NSLocalizedDescriptionKey:
...
static void parseHeaders(NSDictionary *headers, ABI38_0_0RCTSource *source) {
  source->_filesChangedCount = [headers[@"X-Metro-Files-Changed-Count"] integerValue];
}

static void attemptAsynchronousLoadOfBundleAtURL(NSURL *scriptURL, ABI38_0_0RCTSourceLoadProgressBlock onProgress, ABI38_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path
                                              options:NSDataReadingMappedIfSafe
                                                error:&error];
      onComplete(error, ABI38_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI38_0_0RCTMultipartDataTask *task = [[ABI38_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
    if (!done) {
      if (onProgress) {
    ...
    if (error) {
      if ([error.domain isEqualToString:NSURLErrorDomain]) {
        error = [NSError errorWithDomain:ABI38_0_0RCTJavaScriptLoaderErrorDomain
                                    code:ABI38_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                                userInfo:
                 @{
                   NSLocalizedDescriptionKey:
                     [@"Could not connect to development server.\n\n"
                      "Ensure the following:\n"
                      "- Node server is running and available on the same network - run 'npm start' from ABI38_0_0React-native root\n"
    ...
    ABI38_0_0RCTSource *source = ABI38_0_0RCTSourceCreate(scriptURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI38_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI38_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI38_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI38_0_0RCTLoadingProgress *progress = [ABI38_0_0RCTLoadingProgress new];
...
static ABI38_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI38_0_0RCTLoadingProgress *progress = [ABI38_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI38_0_0RCTJSONParse(rawText, nil);
