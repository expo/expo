...
@interface ABI38_0_0RCTKeyCommand : NSObject <NSCopying>
...
@implementation ABI38_0_0RCTKeyCommand
...
- (BOOL)isEqual:(ABI38_0_0RCTKeyCommand *)object
{
  if (![object isKindOfClass:[ABI38_0_0RCTKeyCommand class]]) {
    return NO;
  }
...
@interface ABI38_0_0RCTKeyCommands ()

@property (nonatomic, strong) NSMutableSet<ABI38_0_0RCTKeyCommand *> *commands;

@end

@implementation UIResponder (ABI38_0_0RCTKeyCommands)

+ (UIResponder *)ABI38_0_0RCT_getFirstResponder:(UIResponder *)view
{
  ...
  if (view.isFirstResponder) {
    ...
    if ([(UIViewController *)view parentViewController]) {
      firstResponder = [UIResponder ABI38_0_0RCT_getFirstResponder: [(UIViewController *)view parentViewController]];
    }
    return firstResponder ? firstResponder : [UIResponder ABI38_0_0RCT_getFirstResponder: [(UIViewController *)view view]];
  } else if ([view isKindOfClass:[UIView class]]) {
    for (UIView *subview in [(UIView *)view subviews]) {
      firstResponder = [UIResponder ABI38_0_0RCT_getFirstResponder: subview];
      if (firstResponder) {
...
- (NSArray<UIKeyCommand *> *)ABI38_0_0RCT_keyCommands
{
  NSSet<ABI38_0_0RCTKeyCommand *> *commands = [ABI38_0_0RCTKeyCommands sharedInstance].commands;
 ...
 */
- (void)ABI38_0_0RCT_handleKeyCommand:(UIKeyCommand *)key
{
  // NOTE: throttle the key handler because on iOS 9 the handleKeyCommand:
  // method gets called repeatedly if the command key is held down.
  static NSTimeInterval lastCommand = 0;
  if (CACurrentMediaTime() - lastCommand > 0.5) {
    for (ABI38_0_0RCTKeyCommand *command in [ABI38_0_0RCTKeyCommands sharedInstance].commands) {
 ...
 */
- (void)ABI38_0_0RCT_handleDoublePressKeyCommand:(UIKeyCommand *)key
{
...
@implementation ABI38_0_0RCTKeyCommands

+ (instancetype)sharedInstance
{
  static ABI38_0_0RCTKeyCommands *sharedInstance;
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  UIKeyCommand *command = [UIKeyCommand keyCommandWithInput:input
                                              modifierFlags:flags
                                                     action:@selector(ABI38_0_0RCT_handleKeyCommand:)];

  ABI38_0_0RCTKeyCommand *keyCommand = [[ABI38_0_0RCTKeyCommand alloc] initWithKeyCommand:command block:block];
                        ...
                        modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI38_0_0RCTKeyCommand *command in _commands.allObjects) {
                         ...
                         modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI38_0_0RCTKeyCommand *command in _commands) {
    if ([command matchesInput:input flags:flags]) {
      return YES;
    }
  }
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  UIKeyCommand *command = [UIKeyCommand keyCommandWithInput:input
                                              modifierFlags:flags
                                                     action:@selector(ABI38_0_0RCT_handleDoublePressKeyCommand:)];
