...
#import <ABI38_0_0cxxreact/ABI38_0_0JsArgumentHelpers.h>
...
@implementation ABI38_0_0RCTCxxMethod
{
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI38_0_0RCTLogError(@"Method %@.%s expects at least %zu arguments, but got %tu",
                ABI38_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI38_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
  ...
  try {
    ...
    ABI38_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI38_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
