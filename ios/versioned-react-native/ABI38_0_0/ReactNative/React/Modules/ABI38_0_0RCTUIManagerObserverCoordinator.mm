...
@implementation ABI38_0_0RCTUIManagerObserverCoordinator {
  NSHashTable<id<ABI38_0_0RCTUIManagerObserver>> *_observers;
  std::mutex _mutex;
}
...
- (void)addObserver:(id<ABI38_0_0RCTUIManagerObserver>)observer
{
  std::lock_guard<std::mutex> lock(_mutex);
  [self->_observers addObject:observer];
}

- (void)removeObserver:(id<ABI38_0_0RCTUIManagerObserver>)observer
{
  std::lock_guard<std::mutex> lock(_mutex);
...
- (void)uiManagerWillPerformLayout:(ABI38_0_0RCTUIManager *)manager
{
  std::lock_guard<std::mutex> lock(_mutex);

  for (id<ABI38_0_0RCTUIManagerObserver> observer in _observers) {
...
- (void)uiManagerDidPerformLayout:(ABI38_0_0RCTUIManager *)manager
{
  std::lock_guard<std::mutex> lock(_mutex);
...
- (BOOL)uiManager:(ABI38_0_0RCTUIManager *)manager performMountingWithBlock:(ABI38_0_0RCTUIManagerMountingBlock)block
{
  std::lock_guard<std::mutex> lock(_mutex);
...
- (void)uiManagerDidPerformMounting:(ABI38_0_0RCTUIManager *)manager
{
  std::lock_guard<std::mutex> lock(_mutex);
