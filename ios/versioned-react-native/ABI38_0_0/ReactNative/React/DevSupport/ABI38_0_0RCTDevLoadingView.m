...
#if ABI38_0_0RCT_DEV | ABI38_0_0RCT_ENABLE_LOADING_VIEW

static BOOL isEnabled = YES;

@implementation ABI38_0_0RCTDevLoadingView
{
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
...
- (void)setBridge:(ABI38_0_0RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(hide)
                                               name:ABI38_0_0RCTJavaScriptDidFailToLoadNotification
                                             object:nil];
...
ABI38_0_0RCT_EXPORT_METHOD(showMessage:(NSString *)message color:(UIColor *)color backgroundColor:(UIColor *)backgroundColor)
{
  if (!isEnabled) {
    return;
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!self->_window && !ABI38_0_0RCTRunningInTestEnvironment()) {
      CGSize screenSize = [UIScreen mainScreen].bounds.size;

      if (@available(iOS 11.0, *)) {
        UIWindow *window = ABI38_0_0RCTSharedApplication().keyWindow;
...
- (void)showWithURL:(NSURL *)URL
{
  ...
  if (URL.fileURL) {
    ...
    backgroundColor = [UIColor blackColor];
      message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI38_0_0RCT_PACKAGER_NAME];
  } else {
    color = [UIColor whiteColor];
...
- (void)updateProgress:(ABI38_0_0RCTLoadingProgress *)progress
{
  if (!progress) {
    return;
  }
