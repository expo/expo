...
BOOL ABI38_0_0RCTImageLoadingInstrumentationEnabled(void)
{
  return imageInstrumentationEnabled;
}

BOOL ABI38_0_0RCTImageLoadingPerfInstrumentationEnabled(void)
{
  return imagePerfInstrumentationEnabled;
}

void ABI38_0_0RCTEnableImageLoadingInstrumentation(BOOL enabled)
{
  imageInstrumentationEnabled = enabled;
}

void ABI38_0_0RCTEnableImageLoadingPerfInstrumentation(BOOL enabled)
{
  imagePerfInstrumentationEnabled = enabled;
}

static NSInteger ABI38_0_0RCTImageBytesForImage(UIImage *image)
{
  NSInteger singleImageBytes = image.size.width * image.size.height * image.scale * image.scale * 4;
  return image.images ? image.images.count * singleImageBytes : singleImageBytes;
}

@interface ABI38_0_0RCTImageLoader() <ABI38_0_0NativeImageLoaderIOSSpec, ABI38_0_0RCTImageLoaderWithAttributionProtocol>

@end

@implementation UIImage (ABI38_0_0React)

- (NSInteger)ABI38_0_0ReactDecodedImageBytes
{
  NSNumber *imageBytes = objc_getAssociatedObject(self, _cmd);
  if (!imageBytes) {
    imageBytes = @(ABI38_0_0RCTImageBytesForImage(self));
  }
  return [imageBytes integerValue];
}

- (void)setABI38_0_0ReactDecodedImageBytes:(NSInteger)bytes
{
  objc_setAssociatedObject(self, @selector(ABI38_0_0ReactDecodedImageBytes), @(bytes), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

@end

@implementation ABI38_0_0RCTImageLoader
{
  NSArray<id<ABI38_0_0RCTImageURLLoader>> * (^_loadersProvider)(void);
  NSArray<id<ABI38_0_0RCTImageDataDecoder>> * (^_decodersProvider)(void);
  NSArray<id<ABI38_0_0RCTImageURLLoader>> *_loaders;
  NSArray<id<ABI38_0_0RCTImageDataDecoder>> *_decoders;
  NSOperationQueue *_imageDecodeQueue;
  dispatch_queue_t _URLRequestQueue;
  id<ABI38_0_0RCTImageCache> _imageCache;
  NSMutableArray *_pendingTasks;
  NSInteger _activeTasks;
  NSMutableArray *_pendingDecodes;
  NSInteger _scheduledDecodes;
  NSUInteger _activeBytes;
  __weak id<ABI38_0_0RCTImageRedirectProtocol> _redirectDelegate;
}
...
- (instancetype)initWithRedirectDelegate:(id<ABI38_0_0RCTImageRedirectProtocol>)redirectDelegate
                        ...
                        decodersProvider:(NSArray<id<ABI38_0_0RCTImageDataDecoder>> * (^)(void))getHandlers
{
...
- (void)setUp
{
  ...
  _URLRequestQueue = dispatch_queue_create("com.facebook.ABI38_0_0React.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (id<ABI38_0_0RCTImageCache>)imageCache
{
  if (!_imageCache) {
    ...
    _imageCache = [ABI38_0_0RCTImageCache new];
  }
  return _imageCache;
}

- (void)setImageCache:(id<ABI38_0_0RCTImageCache>)cache
{
  if (_imageCache) {
    ABI38_0_0RCTLogWarn(@"ABI38_0_0RCTImageCache was already set and has now been overridden.");
  }
  _imageCache = cache;
}

- (id<ABI38_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
  ...
  if (!_loaders) {
    ...
    ABI38_0_0RCTAssert(_bridge, @"Bridge not set");

    if (_loadersProvider) {
      _loaders = _loadersProvider();
    } else {
      _loaders = [_bridge modulesConformingToProtocol:@protocol(ABI38_0_0RCTImageURLLoader)];
    }

    _loaders = [_loaders sortedArrayUsingComparator:^NSComparisonResult(id<ABI38_0_0RCTImageURLLoader> a, id<ABI38_0_0RCTImageURLLoader> b) {
  ...
  if (ABI38_0_0RCT_DEBUG) {
    ...
    id<ABI38_0_0RCTImageURLLoader> previousLoader = nil;
    for (id<ABI38_0_0RCTImageURLLoader> loader in _loaders) {
      ...
      if ([loader canLoadImageURL:URL]) {
        if (previousLoader) {
          if (priority == previousPriority) {
            ABI38_0_0RCTLogError(@"The ABI38_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI38_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
  if (!_maxConcurrentLoadingTasks) {
    [self setUp];
  }
  ...
  if (!_decoders) {
    ...
    if (_decodersProvider) {
      ...
      _decoders = [_bridge modulesConformingToProtocol:@protocol(ABI38_0_0RCTImageDataDecoder)];
    }

    _decoders = [[_bridge modulesConformingToProtocol:@protocol(ABI38_0_0RCTImageDataDecoder)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI38_0_0RCTImageDataDecoder> a, id<ABI38_0_0RCTImageDataDecoder> b) {
  ...
  if (ABI38_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI38_0_0RCTImageDataDecoder> previousDecoder = nil;
    for (id<ABI38_0_0RCTImageDataDecoder> decoder in _decoders) {
      ...
      if ([decoder canDecodeImageData:data]) {
        if (previousDecoder) {
          if (priority == previousPriority) {
            ABI38_0_0RCTLogError(@"The ABI38_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI38_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI38_0_0RCTResizeMode resizeMode)
{
  ...
  CGRect targetSize = ABI38_0_0RCTTargetRect(image.size, size, scale, resizeMode);
  CGAffineTransform transform = ABI38_0_0RCTTransformFromTargetRect(image.size, targetSize);
  image = ABI38_0_0RCTTransformImage(image, size, scale, transform);
  return image;
}

- (ABI38_0_0RCTImageLoaderCancellationBlock) loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                   callback:(ABI38_0_0RCTImageLoaderCompletionBlock)callback
{
...
- (void)dequeueTasks
{
  dispatch_async(_URLRequestQueue, ^{
    ...
    for (ABI38_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
      switch (task.status) {
        case ABI38_0_0RCTNetworkTaskFinished:
          if (!tasksToRemove) {
          ...
          self->_activeTasks--;
          break;
        case ABI38_0_0RCTNetworkTaskPending:
          break;
        case ABI38_0_0RCTNetworkTaskInProgress:
          // Check task isn't "stuck"
          if (task.requestToken == nil) {
            ABI38_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
            if (!tasksToRemove) {
              tasksToRemove = [NSMutableArray new];
            }
            [tasksToRemove addObject:task];
    ...
    // Start queued tasks
    for (ABI38_0_0RCTNetworkTask *task in self->_pendingTasks) {
      if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
        break;
      }
      if (task.status == ABI38_0_0RCTNetworkTaskPending) {
        [task start];
        self->_activeTasks++;
      }
    }
 ...
 */
- (ABI38_0_0RCTImageLoaderCancellationBlock)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                             size:(CGSize)size
                                                            scale:(CGFloat)scale
                                                       resizeMode:(ABI38_0_0RCTResizeMode)resizeMode
                                                      attribution:(const ImageURLLoaderAttribution &)attribution
                                                    progressBlock:(ABI38_0_0RCTImageLoaderProgressBlock)progressHandler
                                                 partialLoadBlock:(ABI38_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
                                                  completionBlock:(void (^)(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response))completionBlock
{
  {
    NSMutableURLRequest *mutableRequest = [request mutableCopy];
    [NSURLProtocol setProperty:@"ABI38_0_0RCTImageLoader"
                        forKey:@"trackingName"
  ...
  id<ABI38_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
  ...
  void (^completionHandler)(NSError *, id, NSURLResponse *) = ^(NSError *error, id imageOrData, NSURLResponse *response) {
    ...
    if (ABI38_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
  ...
  if (loadHandler && !requiresScheduling) {
    if ([loadHandler conformsToProtocol:@protocol(ABI38_0_0RCTImageURLLoaderWithAttribution)]) {
      return [(id<ABI38_0_0RCTImageURLLoaderWithAttribution>)loadHandler loadImageForURL:request.URL
                                                                           size:size
                                                                          scale:scale
                                                                     resizeMode:resizeMode
  ...
  __weak ABI38_0_0RCTImageLoader *weakSelf = self;
  dispatch_async(_URLRequestQueue, ^{
  ...
  if (atomic_load(cancelled.get()) || !strongSelf) {
    ...
    if (loadHandler) {
      ...
      if ([loadHandler conformsToProtocol:@protocol(ABI38_0_0RCTImageURLLoaderWithAttribution)]) {
        cancelLoadLocal = [(id<ABI38_0_0RCTImageURLLoaderWithAttribution>)loadHandler loadImageForURL:request.URL
                                                                                        size:size
                                                                                       scale:scale
                                                                                  resizeMode:resizeMode
                                                                                 attribution:attributionCopy
...
- (ABI38_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI38_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSURLResponse *response))completionHandler
{
  // Check if networking module is available
  if (ABI38_0_0RCT_DEBUG && ![_bridge respondsToSelector:@selector(networking)]) {
    ABI38_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                " import the ABI38_0_0RCTNetwork library in order to load images.",
                request.URL.absoluteString);
    return NULL;
  }

  ABI38_0_0RCTNetworking *networking = [_bridge networking];

  // Check if networking module can load image
  if (ABI38_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
    ABI38_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
    return NULL;
  }

  // Use networking module to load image
  ABI38_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    // Check for system errors
    if (error) {
      completionHandler(error, nil, response);
      return;
    } else if (!response) {
      completionHandler(ABI38_0_0RCTErrorWithMessage(@"Response metadata error"), nil, response);
      return;
    } else if (!data) {
      completionHandler(ABI38_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, response);
      return;
    }
  ...
  __block ABI38_0_0RCTNetworkTask *task =
                     ...
                     completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                       ...
                       if (error || !response || !data) {
                         ...
                         if (error) {
                           ...
                           someError = ABI38_0_0RCTErrorWithMessage(@"Response metadata error");
                         } else {
                           someError = ABI38_0_0RCTErrorWithMessage(@"Unknown image download error");
                         }
                                           ...
                                           completionBlock:(ABI38_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  ...
  void (^completionHandler)(NSError *, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response) {
    ...
    ABI38_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
...
- (ABI38_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI38_0_0RCTResizeMode)resizeMode
...
{
  if (data.length == 0) {
    completionBlock(ABI38_0_0RCTErrorWithMessage(@"No image data"), nil);
    return ^{};
  }
  ...
  void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
    if (ABI38_0_0RCTIsMainQueue()) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          completionBlock(error, clipped ? ABI38_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
        }
      });
    } else if (!std::atomic_load(cancelled.get())) {
      completionBlock(error, clipped ? ABI38_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
  ...
  id<ABI38_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
  if (imageDecoder) {
    ...
    dispatch_block_t decodeBlock = ^{
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          ...
          UIImage *image = ABI38_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI38_0_0RCT_DEV
          CGSize imagePixelSize = ABI38_0_0RCTSizeInPixels(image.size, image.scale);
          CGSize screenPixelSize = ABI38_0_0RCTSizeInPixels(ABI38_0_0RCTScreenSize(), ABI38_0_0RCTScreenScale());
          if (imagePixelSize.width * imagePixelSize.height >
              screenPixelSize.width * screenPixelSize.height) {
            ABI38_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
          ...
          if (image) {
            ...
            NSError *finalError = ABI38_0_0RCTErrorWithMessage(errorMessage);
...
- (ABI38_0_0RCTImageLoaderCancellationBlock)getImageSizeForURLRequest:(NSURLRequest *)imageURLRequest
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
  void (^completion)(NSError *, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response) {
    ...
    if ([imageOrData isKindOfClass:[NSData class]]) {
      NSDictionary *meta = ABI38_0_0RCTGetImageMetadata(imageOrData);
...
- (NSDictionary *)getImageCacheStatus:(NSArray *)requests
{
  ...
  for (id request in requests) {
    NSURLRequest *urlRequest = [ABI38_0_0RCTConvert NSURLRequest:request];
    if (urlRequest) {
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  // Even better may be to implement this on the ABI38_0_0RCTImageURLLoader that would try to load it,
  // but we'd have to run the logic both in ABI38_0_0RCTPhotoLibraryImageLoader and
  // ABI38_0_0RCTAssetsLibraryRequestHandler. Once we drop iOS7 though, we'd drop
  // ABI38_0_0RCTAssetsLibraryRequestHandler and can move it there.
  ...
  for (id<ABI38_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI38_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI38_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI38_0_0RCTURLRequestHandler)] &&
...
{
  __block ABI38_0_0RCTImageLoaderCancellationBlock requestToken;
  requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
    ...
    if (ABI38_0_0RCTImageHasAlpha(image.CGImage)) {
      mimeType = @"image/png";
...
{
  if (requestToken) {
    ((ABI38_0_0RCTImageLoaderCancellationBlock)requestToken)();
  }
}
...
{
  return std::make_shared<ABI38_0_0facebook::ABI38_0_0React::NativeImageLoaderIOSSpecJSI>(self, jsInvoker);
}

ABI38_0_0RCT_EXPORT_METHOD(getSize:(NSString *)uri resolve:(ABI38_0_0RCTPromiseResolveBlock)resolve reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
  NSURLRequest *request = [ABI38_0_0RCTConvert NSURLRequest:uri];
...
ABI38_0_0RCT_EXPORT_METHOD(getSizeWithHeaders:(NSString *)uri
...
ABI38_0_0RCT_EXPORT_METHOD(prefetchImage:(NSString *)uri
              resolve:(ABI38_0_0RCTPromiseResolveBlock)resolve
               reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
  NSURLRequest *request = [ABI38_0_0RCTConvert NSURLRequest:uri];
...
ABI38_0_0RCT_EXPORT_METHOD(queryCache:(NSArray *)uris
                  resolve:(ABI38_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
 ...
 */
@implementation ABI38_0_0RCTBridge (ABI38_0_0RCTImageLoader)

- (ABI38_0_0RCTImageLoader *)imageLoader
{
  ABI38_0_0RCTLogWarn(@"Calling bridge.imageLoader is deprecated and will not work in newer versions of ABI38_0_0RN. Please update to the "
             "moduleForClass API or turboModuleLookupDelegate API.");
  return [self moduleForClass:[ABI38_0_0RCTImageLoader class]];
}

@end

Class ABI38_0_0RCTImageLoaderCls(void)
{
  return ABI38_0_0RCTImageLoader.class;
}
