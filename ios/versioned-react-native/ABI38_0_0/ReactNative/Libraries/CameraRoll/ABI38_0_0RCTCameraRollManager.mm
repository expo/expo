...
#import "ABI38_0_0RCTCameraRollManager.h"
...
@implementation ABI38_0_0RCTConvert (PHAssetCollectionSubtype)

ABI38_0_0RCT_ENUM_CONVERTER(PHAssetCollectionSubtype, (@{
...
@implementation ABI38_0_0RCTConvert (PHFetchOptions)
...
+ (PHFetchOptions *)PHFetchOptionsFromMediaType:(NSString *)mediaType
{
  ...
  if ([lowercase isEqualToString:@"photos"]) {
    ...
    if (![lowercase isEqualToString:@"all"]) {
      ABI38_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
...
static void requestPhotoLibraryAccess(ABI38_0_0RCTPromiseRejectBlock reject, PhotosAuthorizedBlock authorizedBlock) {
...
ABI38_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
                  ...
                  reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
  ...
  void (^loadBlock)(void) = ^void() {
    if ([type isEqualToString:@"video"]) {
      ...
      [[self.bridge moduleForClass:[ABI38_0_0RCTImageLoader class]] loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
        if (error) {
...
static void ABI38_0_0RCTResolvePromise(ABI38_0_0RCTPromiseResolveBlock resolve,
...
ABI38_0_0RCT_EXPORT_METHOD(getPhotos:(JS::NativeCameraRollManager::GetPhotosParams &)params
                  resolve:(ABI38_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger const first = [ABI38_0_0RCTConvert NSInteger:[NSNumber numberWithDouble:params.first()]];
  NSString *const afterCursor = [ABI38_0_0RCTConvert NSString:params.after()];
  NSString *const groupName = [ABI38_0_0RCTConvert NSString:params.groupName()];
  NSString *const groupTypes = [[ABI38_0_0RCTConvert NSString:params.groupTypes()] lowercaseString];
  NSString *const mediaType = [ABI38_0_0RCTConvert NSString:params.assetType()];
  NSArray<NSString *> *const mimeTypes = [ABI38_0_0RCTConvert NSStringArray:ABI38_0_0RCTConvertOptionalVecToArray(params.mimeTypes())];
  ...
  PHAssetCollectionSubtype const collectionSubtype = [ABI38_0_0RCTConvert PHAssetCollectionSubtype:groupTypes];

  // Predicate for fetching assets within a collection
  PHFetchOptions *const assetFetchOptions = [ABI38_0_0RCTConvert PHFetchOptionsFromMediaType:mediaType];
    ...
    [assetCollectionFetchResult enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull assetCollection, NSUInteger collectionIdx, BOOL * _Nonnull stopCollections) {
      ...
      [assetsFetchResult enumerateObjectsUsingBlock:^(PHAsset * _Nonnull asset, NSUInteger assetIdx, BOOL * _Nonnull stopAssets) {
        ...
        if (first == assets.count) {
          ...
          *stopCollections = YES;
          hasNextPage = YES;
          ABI38_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
          ABI38_0_0RCTResolvePromise(resolve, assets, hasNextPage);
          resolvedPromise = YES;
          return;
        }
...
ABI38_0_0RCT_EXPORT_METHOD(deletePhotos:(NSArray<NSString *>*)assets
                  resolve:(ABI38_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
  NSArray<NSURL *> *assets_ = [ABI38_0_0RCTConvert NSURLArray:assets];
  [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI38_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
...
{
  return std::make_shared<ABI38_0_0facebook::ABI38_0_0React::NativeCameraRollManagerSpecJSI>(self, jsInvoker);
}

@end

Class ABI38_0_0RCTCameraRollManagerCls(void) {
  return ABI38_0_0RCTCameraRollManager.class;
}
