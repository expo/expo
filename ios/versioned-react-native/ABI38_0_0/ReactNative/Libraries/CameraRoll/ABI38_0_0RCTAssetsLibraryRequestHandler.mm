...
#import "ABI38_0_0RCTAssetsLibraryRequestHandler.h"
...
@interface ABI38_0_0RCTAssetsLibraryRequestHandler() <ABI38_0_0RCTTurboModule>
@end

@implementation ABI38_0_0RCTAssetsLibraryRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  return [request.URL.scheme caseInsensitiveCompare:@"assets-library"] == NSOrderedSame
    ...
    || [request.URL.scheme caseInsensitiveCompare:ABI38_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
}

- (id)sendRequest:(NSURLRequest *)request
     withDelegate:(id<ABI38_0_0RCTURLRequestDelegate>)delegate
{
  ...
  BOOL isPHUpload = [requestURL.scheme caseInsensitiveCompare:ABI38_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
  if (isPHUpload) {
    requestURL = [NSURL URLWithString:[@"ph" stringByAppendingString:[requestURL.absoluteString substringFromIndex:ABI38_0_0RCTNetworkingPHUploadHackScheme.length]]];
  }

  if (!requestURL) {
    NSString *const msg = [NSString stringWithFormat:@"Cannot send request without URL"];
    [delegate URLRequest:cancellationBlock didCompleteWithError:ABI38_0_0RCTErrorWithMessage(msg)];
  ...
  if (![fetchResult firstObject]) {
    ...
    NSError *error = ABI38_0_0RCTErrorWithMessage(errorMessage);
  ...
  if (asset.mediaType == PHAssetMediaTypeVideo && isPHUpload) {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:requestOptions resultHandler:^(AVAsset * _Nullable avAsset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
      ...
      if (![avAsset isKindOfClass:[AVURLAsset class]]) {
        error = [NSError errorWithDomain:ABI38_0_0RCTErrorDomain code:0 userInfo:
        @{
...
Class ABI38_0_0RCTAssetsLibraryRequestHandlerCls(void) {
  return ABI38_0_0RCTAssetsLibraryRequestHandler.class;
}
