...
#import <ABI38_0_0React/ABI38_0_0RCTNetworking.h>
...
#import "ABI38_0_0RCTBlobCollector.h"
...
@implementation ABI38_0_0RCTBlobManager
{
...
ABI38_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)setBridge:(ABI38_0_0RCTBridge *)bridge
{
  ...
  ABI38_0_0facebook::ABI38_0_0React::ABI38_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI38_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI38_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI38_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI38_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
...
ABI38_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(double)socketID)
{
...
ABI38_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(double)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI38_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(double)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI38_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI38_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI38_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI38_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI38_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI38_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI38_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI38_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI38_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI38_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI38_0_0RCTWebSocketContentHandler methods
...
{
  return std::make_shared<ABI38_0_0facebook::ABI38_0_0React::NativeBlobModuleSpecJSI>(self, jsInvoker);
}

@end

Class ABI38_0_0RCTBlobManagerCls(void) {
  return ABI38_0_0RCTBlobManager.class;
}
