...
@interface ABI38_0_0RCTSpringAnimation ()
...
const NSTimeInterval ABI38_0_0MAX_DELTA_TIME = 0.064;

@implementation ABI38_0_0RCTSpringAnimation
{
  CGFloat _toValue;
  CGFloat _fromValue;
                  ...
                  callBack:(nullable ABI38_0_0RCTResponseSenderBlock)callback
{
  if ((self = [super init])) {
    ...
    _lastVelocity = [ABI38_0_0RCTConvert CGFloat:config[@"initialVelocity"]];
...
- (void)resetAnimationConfig:(NSDictionary *)config
{
  ...
  _toValue = [ABI38_0_0RCTConvert CGFloat:config[@"toValue"]];
  _overshootClamping = [ABI38_0_0RCTConvert BOOL:config[@"overshootClamping"]];
  _restDisplacementThreshold = [ABI38_0_0RCTConvert CGFloat:config[@"restDisplacementThreshold"]];
  _restSpeedThreshold = [ABI38_0_0RCTConvert CGFloat:config[@"restSpeedThreshold"]];
  _stiffness = [ABI38_0_0RCTConvert CGFloat:config[@"stiffness"]];
  _damping = [ABI38_0_0RCTConvert CGFloat:config[@"damping"]];
  _mass = [ABI38_0_0RCTConvert CGFloat:config[@"mass"]];
...
- (void)stepAnimationWithTime:(NSTimeInterval)currentTime
{
  ...
  if(_animationStartTime == -1) {
    ...
    // Handle frame drops, and only advance dt by a max of ABI38_0_0MAX_DELTA_TIME
    NSTimeInterval deltaTime = MIN(ABI38_0_0MAX_DELTA_TIME, currentTime - _animationCurrentTime);
    _t = _t + deltaTime / ABI38_0_0RCTAnimationDragCoefficient();
  }
