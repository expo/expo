...
#import <ABI38_0_0React/ABI38_0_0RCTLinkingManager.h>
...
static NSString *const kOpenURLNotification = @"ABI38_0_0RCTOpenURLNotification";
                  ...
                  reject:(ABI38_0_0RCTPromiseRejectBlock)reject)
{
  if (@available(iOS 10.0, *)) {
      ...
      if (success) {
        resolve(@YES);
          ...
          if([URL.absoluteString hasPrefix:@"tel:"]){
            ABI38_0_0RCTLogWarn(@"Unable to open the Phone app in the simulator for telephone URLs. URL:  %@", URL);
            resolve(@NO);
          } else {
    ...
    BOOL opened = [ABI38_0_0RCTSharedApplication() openURL:URL];
    if (opened) {
      resolve(@YES);
    } else {
      #if TARGET_OS_SIMULATOR
                  ...
                  reject:(__unused ABI38_0_0RCTPromiseRejectBlock)reject)
{
  ...
  BOOL canOpen = [ABI38_0_0RCTSharedApplication() canOpenURL:URL];
  ...
  if (canOpen) {
    ...
    if (querySchemes != nil && ([querySchemes containsObject:scheme] || [querySchemes containsObject:[scheme lowercaseString]])) {
      ...
      reject(ABI38_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@. Add %@ to LSApplicationQueriesSchemes in your Info.plist.", URL, scheme], nil);
    }
  } else {
    resolve(@NO);
  }
                  ...
                  reject:(__unused ABI38_0_0RCTPromiseRejectBlock)reject)
{
  ...
  resolve(ABI38_0_0RCTNullIfNil(initialURL.absoluteString));
}
...
ABI38_0_0RCT_EXPORT_METHOD(sendIntent:(NSString *)action
...
{
  return std::make_shared<ABI38_0_0facebook::ABI38_0_0React::NativeLinkingSpecJSI>(self, jsInvoker);
}

@end

Class ABI38_0_0RCTLinkingManagerCls(void) {
  return ABI38_0_0RCTLinkingManager.class;
}
