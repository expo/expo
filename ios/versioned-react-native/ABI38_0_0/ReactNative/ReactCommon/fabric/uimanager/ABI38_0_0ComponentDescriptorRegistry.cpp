...
static std::string componentNameByABI38_0_0ReactViewName(std::string viewName) {
  // We need this function only for the transition period;
  // eventually, all names will be unified.

  std::string rctPrefix("ABI38_0_0RCT");
  if (std::mismatch(rctPrefix.begin(), rctPrefix.end(), viewName.begin())
          .first == rctPrefix.end()) {
    // If `viewName` has "ABI38_0_0RCT" prefix, remove it.
...
ComponentDescriptor const &ComponentDescriptorRegistry::at(
  ...
  auto unifiedComponentName = componentNameByABI38_0_0ReactViewName(componentName);
...
SharedShadowNode ComponentDescriptorRegistry::createNode(
  ...
  auto unifiedComponentName = componentNameByABI38_0_0ReactViewName(viewName);
