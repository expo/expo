...
#import "ABI38_0_0RCTAttributedTextUtils.h"

#include <ABI38_0_0React/core/LayoutableShadowNode.h>
#include <ABI38_0_0React/textlayoutmanager/ABI38_0_0RCTFontProperties.h>
#include <ABI38_0_0React/textlayoutmanager/ABI38_0_0RCTFontUtils.h>
#include <ABI38_0_0React/textlayoutmanager/ABI38_0_0RCTTextPrimitivesConversions.h>

using namespace ABI38_0_0facebook::ABI38_0_0React;

@implementation ABI38_0_0RCTWeakEventEmitterWrapper {
...
inline static UIFontWeight ABI38_0_0RCTUIFontWeightFromInteger(NSInteger fontWeight)
{
...
inline static UIFont *ABI38_0_0RCTEffectiveFontFromTextAttributes(const TextAttributes &textAttributes)
{
  NSString *fontFamily = [NSString stringWithCString:textAttributes.fontFamily.c_str() encoding:NSUTF8StringEncoding];

  ABI38_0_0RCTFontProperties fontProperties;
  ...
  fontProperties.style = textAttributes.fontStyle.hasValue()
      ? ABI38_0_0RCTFontStyleFromFontStyle(textAttributes.fontStyle.value())
      : ABI38_0_0RCTFontStyleUndefined;
  fontProperties.variant = textAttributes.fontVariant.hasValue()
      ? ABI38_0_0RCTFontVariantFromFontVariant(textAttributes.fontVariant.value())
      : ABI38_0_0RCTFontVariantUndefined;
  fontProperties.weight = textAttributes.fontWeight.hasValue()
      ? ABI38_0_0RCTUIFontWeightFromInteger((NSInteger)textAttributes.fontWeight.value())
      : NAN;
  fontProperties.sizeMultiplier = textAttributes.fontSizeMultiplier;

  return ABI38_0_0RCTFontWithFontProperties(fontProperties);
}

inline static CGFloat ABI38_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(const TextAttributes &textAttributes)
{
...
inline static UIColor *ABI38_0_0RCTEffectiveForegroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveForegroundColor = ABI38_0_0RCTUIColorFromSharedColor(textAttributes.foregroundColor) ?: [UIColor blackColor];
...
inline static UIColor *ABI38_0_0RCTEffectiveBackgroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveBackgroundColor = ABI38_0_0RCTUIColorFromSharedColor(textAttributes.backgroundColor);
...
static NSDictionary<NSAttributedStringKey, id> *ABI38_0_0RCTNSTextAttributesFromTextAttributes(
    const TextAttributes &textAttributes)
{
  NSMutableDictionary<NSAttributedStringKey, id> *attributes = [NSMutableDictionary dictionaryWithCapacity:10];

  // Font
  UIFont *font = ABI38_0_0RCTEffectiveFontFromTextAttributes(textAttributes);
  if (font) {
    attributes[NSFontAttributeName] = font;
  }

  // Colors
  UIColor *effectiveForegroundColor = ABI38_0_0RCTEffectiveForegroundColorFromTextAttributes(textAttributes);
  ...
  if (textAttributes.backgroundColor || !isnan(textAttributes.opacity)) {
    attributes[NSBackgroundColorAttributeName] = ABI38_0_0RCTEffectiveBackgroundColorFromTextAttributes(textAttributes);
  }
  ...
  if (textAttributes.alignment.hasValue()) {
    ...
    paragraphStyle.alignment = ABI38_0_0RCTNSTextAlignmentFromTextAlignment(textAlignment);
    isParagraphStyleUsed = YES;
  }

  if (textAttributes.baseWritingDirection.hasValue()) {
    paragraphStyle.baseWritingDirection =
        ABI38_0_0RCTNSWritingDirectionFromWritingDirection(textAttributes.baseWritingDirection.value());
    isParagraphStyleUsed = YES;
  }

  if (!isnan(textAttributes.lineHeight)) {
    CGFloat lineHeight = textAttributes.lineHeight * ABI38_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(textAttributes);
  ...
  if (textAttributes.textDecorationLineType.value_or(TextDecorationLineType::None) != TextDecorationLineType::None) {
    ...
    NSUnderlineStyle style = ABI38_0_0RCTNSUnderlineStyleFromStyleAndPattern(
    ...
    UIColor *textDecorationColor = ABI38_0_0RCTUIColorFromSharedColor(textAttributes.textDecorationColor);
  ...
  if (textAttributes.textShadowOffset.hasValue()) {
    ...
    shadow.shadowColor = ABI38_0_0RCTUIColorFromSharedColor(textAttributes.textShadowColor);
    attributes[NSShadowAttributeName] = shadow;
  }

  // Special
  if (textAttributes.isHighlighted) {
    attributes[ABI38_0_0RCTAttributedStringIsHighlightedAttributeName] = @YES;
  }

  return [attributes copy];
}

NSAttributedString *ABI38_0_0RCTNSAttributedStringFromAttributedString(const AttributedString &attributedString)
{
  ...
  for (auto fragment : attributedString.getFragments()) {
    ...
    if (fragment.isAttachment()) {
      ...
      nsAttributedStringFragment =
          [[NSAttributedString alloc] initWithString:string
                                          attributes:ABI38_0_0RCTNSTextAttributesFromTextAttributes(fragment.textAttributes)];
    }
    ...
    if (fragment.parentShadowView.componentHandle) {
      ABI38_0_0RCTWeakEventEmitterWrapper *eventEmitterWrapper = [ABI38_0_0RCTWeakEventEmitterWrapper new];
      eventEmitterWrapper.eventEmitter = fragment.parentShadowView.eventEmitter;

      NSDictionary<NSAttributedStringKey, id> *additionalTextAttributes =
          @{ABI38_0_0RCTAttributedStringEventEmitterKey : eventEmitterWrapper};
