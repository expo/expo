...
typedef NS_OPTIONS(NSInteger, ABI38_0_0RCTTextSizeComparisonOptions) {
  ABI38_0_0RCTTextSizeComparisonSmaller = 1 << 0,
  ABI38_0_0RCTTextSizeComparisonLarger = 1 << 1,
  ABI38_0_0RCTTextSizeComparisonWithinRange = 1 << 2,
};

@implementation NSTextStorage (ABI38_0_0FontScaling)

- (void)abi38_0_0_rct_scaleFontSizeToFitSize:(CGSize)size
  ...
  while (true) {
    [self abi38_0_0_rct_scaleFontSizeWithRatio:ratio
                 minimumFontSize:minimumFontSize
                 maximumFontSize:maximumFontSize];

    ABI38_0_0RCTTextSizeComparisonOptions comparsion = [self abi38_0_0_rct_compareToSize:size
                                                   thresholdRatio:0.01];

    if ((comparsion & ABI38_0_0RCTTextSizeComparisonWithinRange) &&
        (comparsion & ABI38_0_0RCTTextSizeComparisonSmaller)) {
      return;
    } else if (comparsion & ABI38_0_0RCTTextSizeComparisonSmaller) {
      bottomRatio = ratio;
    ...
    if (ABS(topRatio - bottomRatio) < kRatioThreshold ||
      ...
      [self abi38_0_0_rct_scaleFontSizeWithRatio:lastRatioWhichFits
                   minimumFontSize:minimumFontSize
                   maximumFontSize:maximumFontSize];
      return;
    }
...
- (ABI38_0_0RCTTextSizeComparisonOptions)abi38_0_0_rct_compareToSize:(CGSize)size
  ...
  if (truncatedGlyphRange.location != NSNotFound) {
    return ABI38_0_0RCTTextSizeComparisonLarger;
  }
  ...
  ABI38_0_0RCTTextSizeComparisonOptions result = 0;

  result |=
      (fitsSize) ? ABI38_0_0RCTTextSizeComparisonSmaller : ABI38_0_0RCTTextSizeComparisonLarger;

  if (ABS(measuredSize.width - size.width) < thresholdSize.width) {
    result = result | ABI38_0_0RCTTextSizeComparisonWithinRange;
  }

  return result;
}

- (void)abi38_0_0_rct_scaleFontSizeWithRatio:(CGFloat)ratio
               minimumFontSize:(CGFloat)minimumFontSize
               maximumFontSize:(CGFloat)maximumFontSize {
  [self beginEditing];
