...
#include <ABI38_0_0cxxreact/ABI38_0_0MethodCall.h>
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI38_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI38_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI38_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
