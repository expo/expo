...
NSString *const ABI7_0_0RCTJSCThreadName = @"com.facebook.ReactABI7_0_0.JavaScript";

NSString *const ABI7_0_0RCTJavaScriptContextCreatedNotification = @"ABI7_0_0RCTJavaScriptContextCreatedNotification";

static NSString *const ABI7_0_0RCTJSCProfilerEnabledDefaultsKey = @"ABI7_0_0RCTJSCProfilerEnabled";
...
@interface ABI7_0_0RCTJavaScriptContext : NSObject <ABI7_0_0RCTInvalidating>
...
@implementation ABI7_0_0RCTJavaScriptContext
{
  ABI7_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    ...
    /**
     * Explicitly introduce a retain cycle here - The ABI7_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI7_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI7_0_0RCTJSCExecutor
{
  ABI7_0_0RCTJavaScriptContext *_context;
  NSThread *_javaScriptThread;
...
static NSString *ABI7_0_0RCTJSValueToNSString(JSContextRef context, JSValueRef value, JSValueRef *exception)
{
...
static NSString *ABI7_0_0RCTJSValueToJSONString(JSContextRef context, JSValueRef value, JSValueRef *exception, unsigned indent)
{
...
NSError *ABI7_0_0RCTNSErrorFromJSError(JSContextRef context, JSValueRef jsError)
{
  NSMutableDictionary *errorInfo = [NSMutableDictionary new];

  NSString *description = jsError ? ABI7_0_0RCTJSValueToNSString(context, jsError, NULL) : @"Unknown JS error";
  errorInfo[NSLocalizedDescriptionKey] = [@"Unhandled JS Exception: " stringByAppendingString:description];

  NSString *details = jsError ? ABI7_0_0RCTJSValueToJSONString(context, jsError, NULL, 0) : nil;
  if (details) {
    errorInfo[NSLocalizedFailureReasonErrorKey] = details;

    // Format stack as used in ABI7_0_0RCTFormatError
    id json = ABI7_0_0RCTJSONParse(details, NULL);
    if ([json isKindOfClass:[NSDictionary class]]) {
      if (json[@"stack"]) {
        NSError *regexError;
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"^([^@]+)@(.*):(\\d+):(\\d+)$" options:0 error:&regexError];
        if (regexError) {
          ABI7_0_0RCTLogError(@"Failed to build regex: %@", [regexError localizedDescription]);
        }
        ...
        if ([stackTrace count]) {
          errorInfo[ABI7_0_0RCTJSStackTraceKey] = stackTrace;
        }
      }

      // Fall back to just logging the line number
      if (!errorInfo[ABI7_0_0RCTJSStackTraceKey] && json[@"line"]) {
        errorInfo[ABI7_0_0RCTJSStackTraceKey] = @[@{
          @"methodName": @"",
          @"file": ABI7_0_0RCTNullIfNil(json[@"sourceURL"]),
          @"lineNumber": ABI7_0_0RCTNullIfNil(json[@"line"]),
          @"column": @0,
        }];
      }
    }
  ...
  return [NSError errorWithDomain:ABI7_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}

#if ABI7_0_0RCT_DEV

static void ABI7_0_0RCTInstallJSCProfiler(ABI7_0_0RCTBridge *bridge, JSContextRef context)
{
  if (ABI7_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI7_0_0RCTDevMenuItem toggleItemWithKey:ABI7_0_0RCTJSCProfilerEnabledDefaultsKey title:@"Start Profiling" selectedTitle:@"Stop Profiling" handler:^(BOOL shouldStart) {
      if (shouldStart != ABI7_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI7_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI7_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI7_0_0RCTProfileSendResult(bridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
+ (void)runRunLoopThread
{
  @autoreleasepool {
    ...
    while (kCFRunLoopRunStopped != CFRunLoopRunInMode(kCFRunLoopDefaultMode, ((NSDate *)[NSDate distantFuture]).timeIntervalSinceReferenceDate, NO)) {
      ABI7_0_0RCTAssert(NO, @"not reached assertion"); // runloop spun. that's bad.
    }
  }
}
  ...
  if (self = [super init]) {
    ...
    _javaScriptThread = [[NSThread alloc] initWithTarget:[self class]
                                                selector:@selector(runRunLoopThread)
                                                  object:nil];
    _javaScriptThread.name = ABI7_0_0RCTJSCThreadName;
...
- (ABI7_0_0RCTJavaScriptContext *)context
{
  ABI7_0_0RCTAssertThread(_javaScriptThread, @"Must be called on JS thread.");
  ...
  if (!_context) {
    ...
    _context = [[ABI7_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:_javaScriptThread];

    [[NSNotificationCenter defaultCenter] postNotificationName:ABI7_0_0RCTJavaScriptContextCreatedNotification
                                                        object:context];
  }
...
{
  __weak ABI7_0_0RCTJSCExecutor *weakSelf = self;
...
- (void)setUp
{
...
#ifdef WITH_FB_JSC_TUNING
  [self executeBlockOnJavaScriptQueue:^{
    ABI7_0_0RCTJSCExecutor *strongSelf = weakSelf;
    if (!strongSelf.valid) {
      return;
    }

    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
    ABI7_0_0RCTAssert(cachesPath != nil, @"cachesPath should not be nil");
    if (cachesPath) {
  ...
  [self addSynchronousHookWithName:@"nativeLoggingHook" usingBlock:^(NSString *message, NSNumber *logLevel) {
    ABI7_0_0RCTLogLevel level = ABI7_0_0RCTLogLevelInfo;
    if (logLevel) {
      level = MAX(level, (ABI7_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI7_0_0RCTLogJavaScriptInternal(level, message);
  }];
  ...
  [self addSynchronousHookWithName:@"nativeRequireModuleConfig" usingBlock:^NSString *(NSString *moduleName) {
    ...
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", nil);
    NSArray *config = [strongSelf->_bridge configForModuleName:moduleName];
    NSString *result = config ? ABI7_0_0RCTJSONStringify(config, NULL) : nil;
    ABI7_0_0RCT_PROFILE_END_EVENT(ABI7_0_0RCTProfileTagAlways, @"js_call,config", @{ @"moduleName": moduleName });
    return result;
  }];
  ...
  [self addSynchronousHookWithName:@"nativeFlushQueueImmediate" usingBlock:^(NSArray<NSArray *> *calls){
    ...
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
    [strongSelf->_bridge handleBuffer:calls batchEnded:NO];
    ABI7_0_0RCT_PROFILE_END_EVENT(ABI7_0_0RCTProfileTagAlways, @"js_call", nil);
  }];
  ...
  if (ABI7_0_0RCTProfileIsProfiling()) {
    ...
    [self addSynchronousHookWithName:@"__ABI7_0_0RCTProfileIsProfiling" usingBlock:@YES];
  }
  ...
  [self addSynchronousHookWithName:@"nativeTraceBeginAsyncSection" usingBlock:^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    NSUInteger newCookie = ABI7_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  [self addSynchronousHookWithName:@"nativeTraceEndAsyncSection" usingBlock:^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI7_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async", nil);
  ...
  [self addSynchronousHookWithName:@"nativeTraceBeginSection" usingBlock:^(NSNumber *tag, NSString *profileName){
    ...
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, nil);
  }];

  [self addSynchronousHookWithName:@"nativeTraceEndSection" usingBlock:^(NSNumber *tag) {
    ABI7_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console", nil);
  }];

  __weak ABI7_0_0RCTBridge *weakBridge = _bridge;
  ...
  [self addSynchronousHookWithName:@"nativeTraceBeginAsyncFlow" usingBlock:^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
    if (ABI7_0_0RCTProfileIsProfiling()) {
      ...
      int64_t newCookie = [_ABI7_0_0RCTProfileBeginFlowEvent() longLongValue];
  ...
  [self addSynchronousHookWithName:@"nativeTraceEndAsyncFlow" usingBlock:^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
    if (ABI7_0_0RCTProfileIsProfiling()) {
      ...
      _ABI7_0_0RCTProfileEndFlowEvent(@(newCookie));
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI7_0_0RCTInstallJSCProfiler(_bridge, context.JSGlobalContextRef);
  }];

  for (NSString *event in @[ABI7_0_0RCTProfileDidStartProfiling, ABI7_0_0RCTProfileDidEndProfiling]) {
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI7_0_0RCTProfileDidStartProfiling];
...
- (void)_executeJSCall:(NSString *)method
             arguments:(NSArray *)arguments
              callback:(ABI7_0_0RCTJavaScriptCallback)onComplete
{
  ABI7_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI7_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:ABI7_0_0RCTProfileBlock((^{
    ABI7_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    if (moduleJSRef != NULL && errorJSRef == NULL && !JSValueIsUndefined(contextJSRef, moduleJSRef)) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && !JSValueIsUndefined(contextJSRef, methodJSRef)) {
        ...
        if (!errorJSRef && JSValueIsUndefined(contextJSRef, methodJSRef)) {
          error = ABI7_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSValueIsUndefined(contextJSRef, moduleJSRef)) {
        error = ABI7_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }

    if (errorJSRef || error) {
      if (!error) {
        error = ABI7_0_0RCTNSErrorFromJSError(contextJSRef, errorJSRef);
      }
    ...
    // see [ABI7_0_0RCTJSCExecutorTests testDeserializationPerf]
    id objcValue;
...
{
  ABI7_0_0RCTAssertParam(script);
  ABI7_0_0RCTAssertParam(sourceURL);

  // The RAM bundle has a magic number in the 4 first bytes `(0xFB0BD1E5)`.
  uint32_t magicNumber = NSSwapLittleIntToHost(*((uint32_t *)script.bytes));
  BOOL isRAMBundle = magicNumber == ABI7_0_0RCTRAMBundleMagicNumber;
  if (isRAMBundle) {
    NSError *error;
  ...
  [self executeBlockOnJavaScriptQueue:ABI7_0_0RCTProfileBlock((^{
    ...
    ABI7_0_0RCTPerformanceLoggerStart(ABI7_0_0RCTPLScriptExecution);
    ...
    JSStringRelease(execJSString);
    ABI7_0_0RCTPerformanceLoggerEnd(ABI7_0_0RCTPLScriptExecution);

    if (onComplete) {
      NSError *error;
      if (!result) {
        error = ABI7_0_0RCTNSErrorFromJSError(strongSelf->_context.ctx, jsError);
      }
      onComplete(error);
    }
              ...
              callback:(ABI7_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI7_0_0RCT_DEBUG) {
    ABI7_0_0RCTAssert(ABI7_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:ABI7_0_0RCTProfileBlock((^{
    ...
    if (!valueToInject) {
      ...
      ABI7_0_0RCTLogError(@"%@", errorDesc);

      if (onComplete) {
        NSError *error = [NSError errorWithDomain:ABI7_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorDesc}];
        onComplete(error);
      }
      return;
    }
...
static void executeRandomAccessModule(ABI7_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::unique_ptr<char[]>(new char[size]);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI7_0_0RCTFatal(ABI7_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI7_0_0RCTFatal(ABI7_0_0RCTNSErrorFromJSError(executor->_context.ctx, jsError));
      [executor invalidate];
    });
  }
}
...
{
  ABI7_0_0RCTPerformanceLoggerSet(ABI7_0_0RCTPLRAMNativeRequires, 0);
  ABI7_0_0RCTPerformanceLoggerSet(ABI7_0_0RCTPLRAMNativeRequiresCount, 0);
  ABI7_0_0RCTPerformanceLoggerSet(ABI7_0_0RCTPLRAMNativeRequiresSize, 0);
  ...
  [self addSynchronousHookWithName:@"nativeRequire" usingBlock:^(NSNumber *moduleID) {
    ...
    ABI7_0_0RCTPerformanceLoggerAdd(ABI7_0_0RCTPLRAMNativeRequiresCount, 1);
    ABI7_0_0RCTPerformanceLoggerAppendStart(ABI7_0_0RCTPLRAMNativeRequires);
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, 
    ...
    if (ID < strongSelf->_randomAccessBundle.numTableEntries) {
      ...
      ABI7_0_0RCTPerformanceLoggerAdd(ABI7_0_0RCTPLRAMNativeRequiresSize, size);
      executeRandomAccessModule(strongSelf, ID, NSSwapLittleIntToHost(moduleData->offset), size);
    }

    ABI7_0_0RCT_PROFILE_END_EVENT(ABI7_0_0RCTProfileTagAlways, @"js_call", nil);
    ABI7_0_0RCTPerformanceLoggerAppendEnd(ABI7_0_0RCTPLRAMNativeRequires);
  }];
}
...
{
  ABI7_0_0RCTPerformanceLoggerStart(ABI7_0_0RCTPLRAMBundleLoad);
  file_ptr bundle(fopen(sourceURL.path.UTF8String, "r"), fclose);
  if (!bundle) {
    if (error) {
      *error = ABI7_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI7_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }

  ABI7_0_0RCTPerformanceLoggerEnd(ABI7_0_0RCTPLRAMBundleLoad);
  ABI7_0_0RCTPerformanceLoggerSet(ABI7_0_0RCTPLRAMStartupCodeSize, startupCode.size);
  return [NSData dataWithBytesNoCopy:startupCode.code.release() length:startupCode.size freeWhenDone:YES];
}

ABI7_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)name)
{
