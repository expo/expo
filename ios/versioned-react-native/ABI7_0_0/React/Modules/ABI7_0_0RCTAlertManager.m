...
#import "ABI7_0_0RCTAlertManager.h"
...
@implementation ABI7_0_0RCTConvert (UIAlertViewStyle)

ABI7_0_0RCT_ENUM_CONVERTER(UIAlertViewStyle, (@{
...
@interface ABI7_0_0RCTAlertManager() <UIAlertViewDelegate>

@end

@implementation ABI7_0_0RCTAlertManager
{
  NSMutableArray<UIAlertView *> *_alerts;
  NSMutableArray<UIAlertController *> *_alertControllers;
  NSMutableArray<ABI7_0_0RCTResponseSenderBlock> *_alertCallbacks;
 ...
 */
ABI7_0_0RCT_EXPORT_METHOD(alertWithArgs:(NSDictionary *)args
                  callback:(ABI7_0_0RCTResponseSenderBlock)callback)
{
  NSString *title = [ABI7_0_0RCTConvert NSString:args[@"title"]];
  NSString *message = [ABI7_0_0RCTConvert NSString:args[@"message"]];
  UIAlertViewStyle type = [ABI7_0_0RCTConvert UIAlertViewStyle:args[@"type"]];
  NSArray<NSDictionary *> *buttons = [ABI7_0_0RCTConvert NSDictionaryArray:args[@"buttons"]];
  NSString *defaultValue = [ABI7_0_0RCTConvert NSString:args[@"defaultValue"]];
  NSString *cancelButtonKey = [ABI7_0_0RCTConvert NSString:args[@"cancelButtonKey"]];
  NSString *destructiveButtonKey = [ABI7_0_0RCTConvert NSString:args[@"destructiveButtonKey"]];

  if (!title && !message) {
    ABI7_0_0RCTLogError(@"Must specify either an alert title, or message, or both");
    return;
  }

  if (buttons.count == 0) {
    if (type == UIAlertViewStyleDefault) {
      buttons = @[@{@"0": ABI7_0_0RCTUIKitLocalizedString(@"OK")}];
      cancelButtonKey = @"0";
    } else {
      buttons = @[
        @{@"0": ABI7_0_0RCTUIKitLocalizedString(@"OK")},
        @{@"1": ABI7_0_0RCTUIKitLocalizedString(@"Cancel")},
      ];
      cancelButtonKey = @"1";
    }
  }
  ...
  BOOL preferAlertView = (!ABI7_0_0RCTRunningInAppExtension() &&
                          !destructiveButtonKey &&
                          UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);

  if (preferAlertView || [UIAlertController class] == nil) {

    UIAlertView *alertView = ABI7_0_0RCTAlertView(title, nil, self, nil, nil);
    ...
    for (NSDictionary<NSString *, id> *button in buttons) {
      if (button.count != 1) {
        ABI7_0_0RCTLogError(@"Button definitions should have exactly one key.");
      }
      NSString *buttonKey = button.allKeys.firstObject;
      NSString *buttonTitle = [ABI7_0_0RCTConvert NSString:button[buttonKey]];
  ...
  {
    UIViewController *presentingController = ABI7_0_0RCTKeyWindow().rootViewController;
    if (presentingController == nil) {
      ABI7_0_0RCTLogError(@"Tried to display alert view but there is no application window. args: %@", args);
      return;
    }
    ...
    switch (type) {
      ...
      case UIAlertViewStyleSecureTextInput: {
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder = ABI7_0_0RCTUIKitLocalizedString(@"Password");
      ...
      case UIAlertViewStyleLoginAndPasswordInput: {
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder = ABI7_0_0RCTUIKitLocalizedString(@"Login");
          textField.text = defaultValue;
        }];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder = ABI7_0_0RCTUIKitLocalizedString(@"Password");
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  ABI7_0_0RCTAssert(index != NSNotFound, @"Dismissed alert was not recognised");

  ABI7_0_0RCTResponseSenderBlock callback = _alertCallbacks[index];
