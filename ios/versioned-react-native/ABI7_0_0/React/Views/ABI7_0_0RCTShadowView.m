...
typedef void (^ABI7_0_0RCTActionBlock)(ABI7_0_0RCTShadowView *shadowViewSelf, id value);
typedef void (^ABI7_0_0RCTResetActionBlock)(ABI7_0_0RCTShadowView *shadowViewSelf);

static NSString *const ABI7_0_0RCTBackgroundColorProp = @"backgroundColor";
...
@implementation ABI7_0_0RCTShadowView
{
  ABI7_0_0RCTUpdateLifecycle _propagationLifecycle;
  ABI7_0_0RCTUpdateLifecycle _textLifecycle;
  NSDictionary *_lastParentProperties;
  NSMutableArray<ABI7_0_0RCTShadowView *> *_ReactABI7_0_0Subviews;
...
@synthesize ReactABI7_0_0Tag = _ReactABI7_0_0Tag;

// css_node api

static void ABI7_0_0RCTPrint(void *context)
{
  ABI7_0_0RCTShadowView *shadowView = (__bridge ABI7_0_0RCTShadowView *)context;
  printf("%s(%zd), ", shadowView.viewName.UTF8String, shadowView.ReactABI7_0_0Tag.integerValue);
}

static css_node_t *ABI7_0_0RCTGetChild(void *context, int i)
{
  ABI7_0_0RCTShadowView *shadowView = (__bridge ABI7_0_0RCTShadowView *)context;
  ABI7_0_0RCTShadowView *child = [shadowView ReactABI7_0_0Subviews][i];
  return child->_cssNode;
}

static bool ABI7_0_0RCTIsDirty(void *context)
{
  ABI7_0_0RCTShadowView *shadowView = (__bridge ABI7_0_0RCTShadowView *)context;
...
static void ABI7_0_0RCTProcessMetaProps(const float metaProps[META_PROP_COUNT], float style[CSS_POSITION_COUNT]) {
...
{
  node->children_count = (int)_ReactABI7_0_0Subviews.count;
}
       ...
       absolutePosition:(CGPoint)absolutePosition
{
  ...
  _layoutLifecycle = ABI7_0_0RCTUpdateLifecycleComputed;
  ...
  CGRect frame = {{
    ABI7_0_0RCTRoundPixelValue(node->layout.position[CSS_LEFT]),
    ABI7_0_0RCTRoundPixelValue(node->layout.position[CSS_TOP]),
  }, {
    ABI7_0_0RCTRoundPixelValue(absoluteBottomRight.x - absoluteTopLeft.x),
    ABI7_0_0RCTRoundPixelValue(absoluteBottomRight.y - absoluteTopLeft.y)
  }};
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  for (int i = 0; i < node->children_count; ++i) {
    ABI7_0_0RCTShadowView *child = (ABI7_0_0RCTShadowView *)_ReactABI7_0_0Subviews[i];
                                          ...
                                          parentProperties:(NSDictionary<NSString *, id> *)parentProperties
{
  ...
  if (!_backgroundColor) {
    UIColor *parentBackgroundColor = parentProperties[ABI7_0_0RCTBackgroundColorProp];
    if (parentBackgroundColor) {
      [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        UIView *view = viewRegistry[_ReactABI7_0_0Tag];
        [view ReactABI7_0_0SetInheritedBackgroundColor:parentBackgroundColor];
      }];
    }
  } else {
    ...
    if (alpha < 1.0) {
      ...
      properties[ABI7_0_0RCTBackgroundColorProp] = [UIColor clearColor];
    } else {
      properties[ABI7_0_0RCTBackgroundColorProp] = _backgroundColor;
    }
    return properties;
  }
...
- (void)collectUpdatedProperties:(NSMutableSet<ABI7_0_0RCTApplierBlock> *)applierBlocks
                parentProperties:(NSDictionary<NSString *, id> *)parentProperties
{
  if (_propagationLifecycle == ABI7_0_0RCTUpdateLifecycleComputed && [parentProperties isEqualToDictionary:_lastParentProperties]) {
    return;
  }
  _propagationLifecycle = ABI7_0_0RCTUpdateLifecycleComputed;
  _lastParentProperties = parentProperties;
  NSDictionary<NSString *, id> *nextProps = [self processUpdatedProperties:applierBlocks parentProperties:parentProperties];
  for (ABI7_0_0RCTShadowView *child in _ReactABI7_0_0Subviews) {
            ...
            absolutePosition:(CGPoint)absolutePosition
{
  ...
  if (!CGRectEqualToRect(frame, _frame)) {
    ...
    _layoutLifecycle = ABI7_0_0RCTUpdateLifecycleDirtied;
  }
...
- (CGRect)measureLayoutRelativeToAncestor:(ABI7_0_0RCTShadowView *)ancestor
{
  ...
  ABI7_0_0RCTShadowView *shadowView = self;
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    _newView = YES;
    _layoutLifecycle = ABI7_0_0RCTUpdateLifecycleUninitialized;
    _propagationLifecycle = ABI7_0_0RCTUpdateLifecycleUninitialized;
    _textLifecycle = ABI7_0_0RCTUpdateLifecycleUninitialized;

    _ReactABI7_0_0Subviews = [NSMutableArray array];

    _cssNode = new_css_node();
    _cssNode->context = (__bridge void *)self;
    _cssNode->print = ABI7_0_0RCTPrint;
    _cssNode->get_child = ABI7_0_0RCTGetChild;
    _cssNode->is_dirty = ABI7_0_0RCTIsDirty;
    [self fillCSSNode:_cssNode];
  }
  return self;
}
...
- (void)dirtyLayout
{
  if (_layoutLifecycle != ABI7_0_0RCTUpdateLifecycleDirtied) {
    _layoutLifecycle = ABI7_0_0RCTUpdateLifecycleDirtied;
...
- (BOOL)isLayoutDirty
{
  return _layoutLifecycle != ABI7_0_0RCTUpdateLifecycleComputed;
}

- (void)dirtyPropagation
{
  if (_propagationLifecycle != ABI7_0_0RCTUpdateLifecycleDirtied) {
    _propagationLifecycle = ABI7_0_0RCTUpdateLifecycleDirtied;
...
{
  return _propagationLifecycle != ABI7_0_0RCTUpdateLifecycleComputed;
}

- (void)dirtyText
{
  if (_textLifecycle != ABI7_0_0RCTUpdateLifecycleDirtied) {
    _textLifecycle = ABI7_0_0RCTUpdateLifecycleDirtied;
...
- (BOOL)isTextDirty
{
  return _textLifecycle != ABI7_0_0RCTUpdateLifecycleComputed;
}

- (void)setTextComputed
{
  _textLifecycle = ABI7_0_0RCTUpdateLifecycleComputed;
}
...
- (void)insertReactABI7_0_0Subview:(ABI7_0_0RCTShadowView *)subview atIndex:(NSInteger)atIndex
{
  ...
  _cssNode->children_count = (int)_ReactABI7_0_0Subviews.count;
...
{
  for (ABI7_0_0RCTShadowView *shadowView in _ReactABI7_0_0Subviews) {
    if (CGRectContainsPoint(shadowView.frame, point)) {
      CGPoint relativePoint = point;
      CGPoint origin = shadowView.frame.origin;
      relativePoint.x -= origin.x;
      relativePoint.y -= origin.y;
      return [shadowView ReactABI7_0_0TagAtPoint:relativePoint];
    }
  }
  return self.ReactABI7_0_0Tag;
}

- (NSString *)description
{
  NSString *description = super.description;
  description = [[description substringToIndex:description.length - 1] stringByAppendingFormat:@"; viewName: %@; ReactABI7_0_0Tag: %@; frame: %@>", self.viewName, self.ReactABI7_0_0Tag, NSStringFromCGRect(self.frame)];
  return description;
}
...
- (void)addRecursiveDescriptionToString:(NSMutableString *)string atLevel:(NSUInteger)level
{
  ...
  for (ABI7_0_0RCTShadowView *subview in _ReactABI7_0_0Subviews) {
...
#define ABI7_0_0RCT_MARGIN_PROPERTY(prop, metaProp)       \
...
ABI7_0_0RCT_MARGIN_PROPERTY(, ALL)
ABI7_0_0RCT_MARGIN_PROPERTY(Vertical, VERTICAL)
ABI7_0_0RCT_MARGIN_PROPERTY(Horizontal, HORIZONTAL)
ABI7_0_0RCT_MARGIN_PROPERTY(Top, TOP)
ABI7_0_0RCT_MARGIN_PROPERTY(Left, LEFT)
ABI7_0_0RCT_MARGIN_PROPERTY(Bottom, BOTTOM)
ABI7_0_0RCT_MARGIN_PROPERTY(Right, RIGHT)

// Padding

#define ABI7_0_0RCT_PADDING_PROPERTY(prop, metaProp)       \
...
ABI7_0_0RCT_PADDING_PROPERTY(, ALL)
ABI7_0_0RCT_PADDING_PROPERTY(Vertical, VERTICAL)
ABI7_0_0RCT_PADDING_PROPERTY(Horizontal, HORIZONTAL)
ABI7_0_0RCT_PADDING_PROPERTY(Top, TOP)
ABI7_0_0RCT_PADDING_PROPERTY(Left, LEFT)
ABI7_0_0RCT_PADDING_PROPERTY(Bottom, BOTTOM)
ABI7_0_0RCT_PADDING_PROPERTY(Right, RIGHT)
...
#define ABI7_0_0RCT_BORDER_PROPERTY(prop, metaProp)            \
...
ABI7_0_0RCT_BORDER_PROPERTY(, ALL)
ABI7_0_0RCT_BORDER_PROPERTY(Top, TOP)
ABI7_0_0RCT_BORDER_PROPERTY(Left, LEFT)
ABI7_0_0RCT_BORDER_PROPERTY(Bottom, BOTTOM)
ABI7_0_0RCT_BORDER_PROPERTY(Right, RIGHT)

// Dimensions

#define ABI7_0_0RCT_DIMENSIONS_PROPERTY(setProp, getProp, cssProp, dimensions) \
...
ABI7_0_0RCT_DIMENSIONS_PROPERTY(Width, width, WIDTH, dimensions)
ABI7_0_0RCT_DIMENSIONS_PROPERTY(Height, height, HEIGHT, dimensions)

// Position

#define ABI7_0_0RCT_POSITION_PROPERTY(setProp, getProp, cssProp) \
- (void)set##setProp:(CGFloat)value                      \
{                                                        \
...
ABI7_0_0RCT_POSITION_PROPERTY(Top, top, TOP)
ABI7_0_0RCT_POSITION_PROPERTY(Right, right, RIGHT)
ABI7_0_0RCT_POSITION_PROPERTY(Bottom, bottom, BOTTOM)
ABI7_0_0RCT_POSITION_PROPERTY(Left, left, LEFT)
...
static inline BOOL ABI7_0_0RCTAssignSuggestedDimension(css_node_t *css_node, int dimension, CGFloat amount)
{
...
- (void)setIntrinsicContentSize:(CGSize)size
{
  if (_cssNode->style.flex == 0) {
    BOOL dirty = NO;
    dirty |= ABI7_0_0RCTAssignSuggestedDimension(_cssNode, CSS_HEIGHT, size.height);
    dirty |= ABI7_0_0RCTAssignSuggestedDimension(_cssNode, CSS_WIDTH, size.width);
    if (dirty) {
      [self dirtyLayout];
    }
  }
...
#define ABI7_0_0RCT_STYLE_PROPERTY(setProp, getProp, cssProp, type) \
...
ABI7_0_0RCT_STYLE_PROPERTY(Flex, flex, flex, CGFloat)
ABI7_0_0RCT_STYLE_PROPERTY(FlexDirection, flexDirection, flex_direction, css_flex_direction_t)
ABI7_0_0RCT_STYLE_PROPERTY(JustifyContent, justifyContent, justify_content, css_justify_t)
ABI7_0_0RCT_STYLE_PROPERTY(AlignSelf, alignSelf, align_self, css_align_t)
ABI7_0_0RCT_STYLE_PROPERTY(AlignItems, alignItems, align_items, css_align_t)
ABI7_0_0RCT_STYLE_PROPERTY(Position, position, position_type, css_position_type_t)
ABI7_0_0RCT_STYLE_PROPERTY(FlexWrap, flexWrap, flex_wrap, css_wrap_type_t)
...
- (void)didSetProps:(__unused NSArray<NSString *> *)changedProps
{
  if (_recomputePadding) {
    ABI7_0_0RCTProcessMetaProps(_paddingMetaProps, _cssNode->style.padding);
  }
  if (_recomputeMargin) {
    ABI7_0_0RCTProcessMetaProps(_marginMetaProps, _cssNode->style.margin);
  }
  if (_recomputeBorder) {
    ABI7_0_0RCTProcessMetaProps(_borderMetaProps, _cssNode->style.border);
  }
