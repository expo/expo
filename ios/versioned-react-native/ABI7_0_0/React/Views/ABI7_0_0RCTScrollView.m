...
@implementation ABI7_0_0RCTScrollEvent
{
...
{
  ABI7_0_0RCTAssertParam(ReactABI7_0_0Tag);

  if ((self = [super init])) {
    _eventName = [eventName copy];
    _viewTag = ReactABI7_0_0Tag;
...
- (ABI7_0_0RCTScrollEvent *)coalesceWithEvent:(ABI7_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI7_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI7_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI7_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, copy) NSIndexSet *stickyHeaderIndices;
@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) ABI7_0_0RCTRefreshControl *reactRefreshControl;

@end


@implementation ABI7_0_0RCTCustomScrollView
{
...
- (UIView *)contentView
{
  return ((ABI7_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI7_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI7_0_0RCTUIManager JSResponder] isKindOfClass:[ABI7_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI7_0_0 scroll
...
- (void)dockClosestSectionHeader
{
  ...
  NSUInteger subviewCount = contentView.ReactABI7_0_0Subviews.count;
  [_stickyHeaderIndices enumerateIndexesWithOptions:0 usingBlock:
   ^(NSUInteger idx, BOOL *stop) {
    ...
    UIView *header = contentView.ReactABI7_0_0Subviews[idx];
...
- (void)setReactRefreshControl:(ABI7_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI7_0_0RCTScrollView
{
  ABI7_0_0RCTEventDispatcher *_eventDispatcher;
  ABI7_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI7_0_0RCTEventDispatcher *)eventDispatcher
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _scrollView = [[ABI7_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
- (void)insertReactABI7_0_0Subview:(UIView *)view atIndex:(__unused NSInteger)atIndex
{
  if ([view isKindOfClass:[ABI7_0_0RCTRefreshControl class]]) {
    _scrollView.reactRefreshControl = (ABI7_0_0RCTRefreshControl*)view;
  } else {
    ABI7_0_0RCTAssert(_contentView == nil, @"ABI7_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
...
{
  if ([subview isKindOfClass:[ABI7_0_0RCTRefreshControl class]]) {
    _scrollView.reactRefreshControl = nil;
  } else {
    ABI7_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
...
{
  ABI7_0_0RCTAssert(_scrollView.contentSize.width <= self.frame.size.width,
...
- (void)layoutSubviews
{
  ...
  ABI7_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI7_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI7_0_0RCTRefreshControl *refreshControl = _scrollView.reactRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI7_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI7_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI7_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI7_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI7_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI7_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI7_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI7_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI7_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI7_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI7_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI7_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI7_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI7_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI7_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI7_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI7_0_0RCTView contentInsetsForView:self];
...
- (void)ReactABI7_0_0BridgeDidFinishTransaction
{
  ...
  if (ABI7_0_0RCT_DEBUG) {
    ...
    NSUInteger subviewCount = _scrollView.contentView.ReactABI7_0_0Subviews.count;
    ...
    if (lastIndex != NSNotFound && lastIndex >= subviewCount) {
      ABI7_0_0RCTLogWarn(@"Sticky header index %zd was outside the range {0, %zd}",
...
#define ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI7_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);

- (void)setOnRefreshStart:(ABI7_0_0RCTDirectEventBlock)onRefreshStart
{
  if (!onRefreshStart) {
    _onRefreshStart = nil;
    _scrollView.reactRefreshControl = nil;
  ...
  if (!_scrollView.reactRefreshControl) {
    ABI7_0_0RCTRefreshControl *refreshControl = [ABI7_0_0RCTRefreshControl new];
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI7_0_0RCTScrollEvent *scrollEvent = [[ABI7_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI7_0_0RCTEventDispatcher (ABI7_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI7_0_0Tag
{
  ...
  ABI7_0_0RCTScrollEvent *fakeScrollEvent = [[ABI7_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI7_0_0Tag:ReactABI7_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
