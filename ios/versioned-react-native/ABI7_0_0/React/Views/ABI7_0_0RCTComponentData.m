...
typedef void (^ABI7_0_0RCTPropBlock)(id<ABI7_0_0RCTComponent> view, id json);

@interface ABI7_0_0RCTComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) ABI7_0_0RCTPropBlock propBlock;

@end

@implementation ABI7_0_0RCTComponentProp
...
@implementation ABI7_0_0RCTComponentData
{
  id<ABI7_0_0RCTComponent> _defaultView; // Only needed for ABI7_0_0RCT_CUSTOM_VIEW_PROPERTY
  NSMutableDictionary<NSString *, ABI7_0_0RCTPropBlock> *_viewPropBlocks;
  NSMutableDictionary<NSString *, ABI7_0_0RCTPropBlock> *_shadowPropBlocks;
...
- (instancetype)initWithManagerClass:(Class)managerClass
                              bridge:(ABI7_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _bridge = bridge;
    ...
    _name = ABI7_0_0RCTBridgeModuleNameForClass(_managerClass);
    ABI7_0_0RCTAssert(_name.length, @"Invalid moduleName '%@'", _name);
    ...
    while (cls != [ABI7_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI7_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI7_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
- (ABI7_0_0RCTShadowView *)createShadowViewWithTag:(NSNumber *)tag
{
  ABI7_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI7_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ABI7_0_0RCTPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, ABI7_0_0RCTPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  ABI7_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {

    __weak ABI7_0_0RCTComponentData *weakSelf = self;
    ...
    if ([_managerClass respondsToSelector:selector]) {
      ...
      type = ABI7_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<ABI7_0_0RCTComponent> view, id json) {
        ABI7_0_0RCTComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        json = ABI7_0_0RCTNilIfNull(json);
      ...
      if (type == NSSelectorFromString(@"ABI7_0_0RCTBubblingEventBlock:") ||
          type == NSSelectorFromString(@"ABI7_0_0RCTDirectEventBlock:")) {

        // Special case for event handlers
        __weak ABI7_0_0RCTViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<ABI7_0_0RCTComponent> weakTarget = target;
          ((void (*)(id, SEL, id))objc_msgSend)(target, setter, [ABI7_0_0RCTConvert BOOL:json] ? ^(NSDictionary *body) {
            body = [NSMutableDictionary dictionaryWithDictionary:body];
            ((NSMutableDictionary *)body)[@"target"] = weakTarget.ReactABI7_0_0Tag;
            [weakManager.bridge.eventDispatcher sendInputEventWithName:ABI7_0_0RCTNormalizeInputEventName(name) body:body];
          } : nil);
        };
        ...
        NSMethodSignature *typeSignature = [[ABI7_0_0RCTConvert class] methodSignatureForSelector:type];
        if (!typeSignature) {
          ABI7_0_0RCTLogError(@"No +[ABI7_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
          return ^(__unused id<ABI7_0_0RCTComponent> view, __unused id json){};
        }
  ...
  #define ABI7_0_0RCT_CASE(_value, _type) \
          case _value: { \
            ...
            setterBlock = ^(id target, id json) { \
              if (json) { \
                ...
                set(target, setter, convert([ABI7_0_0RCTConvert class], type, json)); \
            ...
            ABI7_0_0RCT_CASE(_C_SEL, SEL)
            ABI7_0_0RCT_CASE(_C_CHARPTR, const char *)
            ...
            ABI7_0_0RCT_CASE(_C_PTR, void *)
            ABI7_0_0RCT_CASE(_C_ID, id)
      ...
      propBlock = ^(__unused id view, __unused id json) {
        ...
        setterBlock(target, ABI7_0_0RCTNilIfNull(json));
      };
    }
    ...
    if (ABI7_0_0RCT_DEBUG) {
      ...
      ABI7_0_0RCTPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<ABI7_0_0RCTComponent> view, id json) {
        NSString *logPrefix = [NSString stringWithFormat:
                               @"Error setting property '%@' of %@ with tag #%@: ",
                               name, weakSelf.name, view.ReactABI7_0_0Tag];

        ABI7_0_0RCTPerformBlockWithLogPrefix(^{ unwrappedBlock(view, json); }, logPrefix);
      };
    }
...
- (void)setProps:(NSDictionary<NSString *, id> *)props forView:(id<ABI7_0_0RCTComponent>)view
{
  if (!view) {
    return;
  }
...
- (void)setProps:(NSDictionary<NSString *, id> *)props forShadowView:(ABI7_0_0RCTShadowView *)shadowView
{
  if (!shadowView) {
    return;
  }
...
- (NSDictionary<NSString *, id> *)viewConfig
{
  ...
  if (ABI7_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customDirectEventTypes))) {
    NSArray<NSString *> *events = [self.manager customDirectEventTypes];
    if (ABI7_0_0RCT_DEBUG) {
      ABI7_0_0RCTAssert(!events || [events isKindOfClass:[NSArray class]],
        @"customDirectEventTypes must return an array, but %@ returned %@",
        _managerClass, [events class]);
    }
    for (NSString *event in events) {
      [directEvents addObject:ABI7_0_0RCTNormalizeInputEventName(event)];
    }
  }

  NSMutableArray<NSString *> *bubblingEvents = [NSMutableArray new];
  if (ABI7_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    ...
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI7_0_0RCTNormalizeInputEventName(event)];
    }
  }
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if ([methodName hasPrefix:@"propConfig"]) {
      ...
      if (nameRange.length) {
        ...
        if (ABI7_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
          ABI7_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                      "to '%@'", name, _name, propTypes[name], type);
        }

        if ([type isEqualToString:@"ABI7_0_0RCTBubblingEventBlock"]) {
          [bubblingEvents addObject:ABI7_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else if ([type isEqualToString:@"ABI7_0_0RCTDirectEventBlock"]) {
          [directEvents addObject:ABI7_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else {
          propTypes[name] = type;
        }
  ...
  if (ABI7_0_0RCT_DEBUG) {
    for (NSString *event in directEvents) {
      if ([bubblingEvents containsObject:event]) {
        ABI7_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
...
- (ABI7_0_0RCTViewManagerUIBlock)uiBlockToAmendWithShadowViewRegistry:(NSDictionary<NSNumber *, ABI7_0_0RCTShadowView *> *)registry
{
