...
typedef NS_ENUM(NSUInteger, ABI7_0_0RCTNavigationLock) {
  ABI7_0_0RCTNavigationLockNone,
  ABI7_0_0RCTNavigationLockNative,
  ABI7_0_0RCTNavigationLockJavaScript
};

// By default the interactive pop gesture will be enabled when the navigation bar is displayed
// and disabled when hidden
// ABI7_0_0RCTPopGestureStateDefault maps to the default behavior (mentioned above). Once popGestureState
...
// back to it.
// ABI7_0_0RCTPopGestureStateEnabled will enable the gesture independent of nav bar visibility
// ABI7_0_0RCTPopGestureStateDisabled will disable the gesture independent of nav bar visibility
typedef NS_ENUM(NSUInteger, ABI7_0_0RCTPopGestureState) {
  ABI7_0_0RCTPopGestureStateDefault = 0,
  ABI7_0_0RCTPopGestureStateEnabled,
  ABI7_0_0RCTPopGestureStateDisabled
};
...
@interface ABI7_0_0RCTNavigationController : UINavigationController <UINavigationBarDelegate>
{
  dispatch_block_t _scrollCallback;
}

@property (nonatomic, assign) ABI7_0_0RCTNavigationLock navigationLock;

@end

/**
 * In general, `ABI7_0_0RCTNavigator` examines `_currentViews` (which are ReactABI7_0_0 child
 ...
 * ahead by adding/removing ReactABI7_0_0 subviews. Native gets ahead by swiping back,
 * or tapping the back button. In both cases, the other system is initially
 * unaware. And in both cases, `ABI7_0_0RCTNavigator` helps the other side "catch up".
 *
 * If `ABI7_0_0RCTNavigator` sees the number of ReactABI7_0_0 children have changed, it
 * pushes/pops accordingly. If `ABI7_0_0RCTNavigator` sees a `UIKit` driven push/pop, it
 ...
 *
 */
@implementation ABI7_0_0RCTNavigationController

/**
 * @param callback Callback that is invoked when a "scroll" interaction begins
 * so that `ABI7_0_0RCTNavigator` can notify `JavaScript`.
 */
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
  if (self.interactivePopGestureRecognizer.state == UIGestureRecognizerStateBegan) {
    if (self.navigationLock == ABI7_0_0RCTNavigationLockNone) {
      self.navigationLock = ABI7_0_0RCTNavigationLockNative;
      if (_scrollCallback) {
        _scrollCallback();
      }
    } else if (self.navigationLock == ABI7_0_0RCTNavigationLockJavaScript) {
      // This should never happen because we disable/enable the gesture
      // recognizer when we lock the navigation.
      ABI7_0_0RCTAssert(NO, @"Should never receive gesture start while JS locks navigator");
    }
  } else {
    if (self.navigationLock == ABI7_0_0RCTNavigationLockNone) {
      ...
      // length (`currentReactABI7_0_0Count` - 1).
...
@interface ABI7_0_0RCTNavigator() <ABI7_0_0RCTWrapperViewControllerNavigationListener, UINavigationControllerDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, copy) ABI7_0_0RCTDirectEventBlock onNavigationProgress;
@property (nonatomic, copy) ABI7_0_0RCTBubblingEventBlock onNavigationComplete;

@property (nonatomic, assign) NSInteger previousRequestedTopOfStack;

@property (nonatomic, assign) ABI7_0_0RCTPopGestureState popGestureState;

// Previous views are only mainted in order to detect incorrect
// addition/removal of views below the `requestedTopOfStack`
@property (nonatomic, copy, readwrite) NSArray<ABI7_0_0RCTNavItem *> *previousViews;
@property (nonatomic, readwrite, strong) NSMutableArray<ABI7_0_0RCTNavItem *> *currentViews;
@property (nonatomic, readwrite, strong) ABI7_0_0RCTNavigationController *navigationController;
/**
 ...
 * - We use `invalidate` to remove the `ABI7_0_0RCTNavigator`'s reference to the
 ...
 * remove it from the run loop when the view will be destroyed by ReactABI7_0_0.
 *
 * +----------+              +--------------+
...
@implementation ABI7_0_0RCTNavigator
{
...
- (instancetype)initWithBridge:(ABI7_0_0RCTBridge *)bridge
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    __weak ABI7_0_0RCTNavigator *weakSelf = self;
    _navigationController = [[ABI7_0_0RCTNavigationController alloc] initWithScrollCallback:^{
      [weakSelf dispatchFakeScrollEvent];
    }];
    _navigationController.delegate = self;
    ABI7_0_0RCTAssert([self requestSchedulingJavaScriptNavigation], @"Could not acquire JS navigation lock on init");
...
- (void)setInteractivePopGestureEnabled:(BOOL)interactivePopGestureEnabled
{
  ...
  _popGestureState = interactivePopGestureEnabled ? ABI7_0_0RCTPopGestureStateEnabled : ABI7_0_0RCTPopGestureStateDisabled;
}
                    ...
                    animated:(__unused BOOL)animated
{
  ...
  [tc animateAlongsideTransition: ^(id<UIViewControllerTransitionCoordinatorContext> context) {
    ABI7_0_0RCTWrapperViewController *fromController =
      (ABI7_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey];
    ABI7_0_0RCTWrapperViewController *toController =
      (ABI7_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey];
...
{
  if (_navigationController.navigationLock == ABI7_0_0RCTNavigationLockNone) {
    _navigationController.navigationLock = ABI7_0_0RCTNavigationLockJavaScript;
...
- (void)freeLock
{
  _navigationController.navigationLock = ABI7_0_0RCTNavigationLockNone;
  
  // Unless the pop gesture has been explicitly disabled (ABI7_0_0RCTPopGestureStateDisabled),
  // Set interactivePopGestureRecognizer.enabled to YES
  // If the popGestureState is ABI7_0_0RCTPopGestureStateDefault the default behavior will be maintained
  _navigationController.interactivePopGestureRecognizer.enabled = self.popGestureState != ABI7_0_0RCTPopGestureStateDisabled;
}

/**
 * A ReactABI7_0_0 subview can be inserted/removed at any time, however if the
 * `requestedTopOfStack` changes, there had better be enough subviews present
 * to satisfy the push/pop.
 */
- (void)insertReactABI7_0_0Subview:(ABI7_0_0RCTNavItem *)view atIndex:(NSInteger)atIndex
{
  ABI7_0_0RCTAssert([view isKindOfClass:[ABI7_0_0RCTNavItem class]], @"ABI7_0_0RCTNavigator only accepts ABI7_0_0RCTNavItem subviews");
  ABI7_0_0RCTAssert(
    _navigationController.navigationLock == ABI7_0_0RCTNavigationLockJavaScript,
...
- (NSArray<ABI7_0_0RCTNavItem *> *)ReactABI7_0_0Subviews
{
...
- (void)layoutSubviews
{
  ...
  [self ReactABI7_0_0AddControllerToClosestParent:_navigationController];
  _navigationController.view.frame = self.bounds;
}

- (void)removeReactABI7_0_0Subview:(ABI7_0_0RCTNavItem *)subview
{
  if (_currentViews.count <= 0 || subview == _currentViews[0]) {
    ABI7_0_0RCTLogError(@"Attempting to remove invalid ABI7_0_0RCT subview of ABI7_0_0RCTNavigator");
    return;
  }
...
{
  [_bridge.eventDispatcher sendFakeScrollEvent:self.ReactABI7_0_0Tag];
}
...
{
  ABI7_0_0RCTAssert(!_bridge.isValid || self.superview != nil, @"put ReactABI7_0_0NavSuperviewLink back");
  return self.superview ? self.superview : self.ReactABI7_0_0NavSuperviewLink;
}
...
- (void)ReactABI7_0_0BridgeDidFinishTransaction
{
  ...
  // The "ReactABI7_0_0 count" is the count of views that are visible on the navigation
  // stack.  There may be more beyond this - that aren't visible, and may be
  // deleted/purged soon.
  NSUInteger previousReactABI7_0_0Count =
    _previousRequestedTopOfStack == kNeverRequested ? 0 : _previousRequestedTopOfStack + 1;
  NSUInteger currentReactABI7_0_0Count = _requestedTopOfStack + 1;

  BOOL jsGettingAhead =
    //    ----- previously caught up ------          ------ no longer caught up -------
    viewControllerCount == previousReactABI7_0_0Count && currentReactABI7_0_0Count != viewControllerCount;
  BOOL jsCatchingUp =
    //    --- previously not caught up ----          --------- now caught up ----------
    viewControllerCount != previousReactABI7_0_0Count && currentReactABI7_0_0Count == viewControllerCount;
  BOOL jsMakingNoProgressButNeedsToCatchUp =
    //    --- previously not caught up ----          ------- still the same -----------
    viewControllerCount != previousReactABI7_0_0Count && currentReactABI7_0_0Count == previousReactABI7_0_0Count;
...
BOOL jsGettingtooSlow =
  ...
  BOOL ReactABI7_0_0PushOne = jsGettingAhead && currentReactABI7_0_0Count == previousReactABI7_0_0Count + 1;
  BOOL ReactABI7_0_0PopN = jsGettingAhead && currentReactABI7_0_0Count < previousReactABI7_0_0Count;
  ...
  if (!(jsGettingAhead ||
        ...
        jsGettingtooSlow)) {
    ABI7_0_0RCTLogError(@"JS has only made partial progress to catch up to UIKit");
  }
  if (currentReactABI7_0_0Count > _currentViews.count) {
    ABI7_0_0RCTLogError(@"Cannot adjust current top of stack beyond available views");
  }

  // Views before the previous ReactABI7_0_0 count must not have changed. Views greater than previousReactABI7_0_0Count
  // up to currentReactABI7_0_0Count may have changed.
  for (NSUInteger i = 0; i < MIN(_currentViews.count, MIN(_previousViews.count, previousReactABI7_0_0Count)); i++) {
    if (_currentViews[i] != _previousViews[i]) {
      ABI7_0_0RCTLogError(@"current view should equal previous view");
    }
  }
  if (currentReactABI7_0_0Count < 1) {
    ABI7_0_0RCTLogError(@"should be at least one current view");
  }
  if (jsGettingAhead) {
    if (ReactABI7_0_0PushOne) {
      UIView *lastView = _currentViews.lastObject;
      ABI7_0_0RCTWrapperViewController *vc = [[ABI7_0_0RCTWrapperViewController alloc] initWithNavItem:(ABI7_0_0RCTNavItem *)lastView];
      vc.navigationListener = self;
      _numberOfViewControllerMovesToIgnore = 1;
      [_navigationController pushViewController:vc animated:(currentReactABI7_0_0Count > 1)];
    } else if (ReactABI7_0_0PopN) {
      UIViewController *viewControllerToPopTo = _navigationController.viewControllers[(currentReactABI7_0_0Count - 1)];
      _numberOfViewControllerMovesToIgnore = viewControllerCount - currentReactABI7_0_0Count;
      [_navigationController popToViewController:viewControllerToPopTo animated:YES];
    } else {
      ABI7_0_0RCTLogError(@"Pushing or popping more than one view at a time from JS");
    }
...
didMoveToNavigationController:(UINavigationController *)navigationController
{
  ...
  ABI7_0_0RCTAssert(_navigationController.navigationLock == ABI7_0_0RCTNavigationLockJavaScript ||
