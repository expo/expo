...
#import "ABI7_0_0RCTAutoInsetsProtocol.h"
#import "ABI7_0_0RCTBorderDrawing.h"
#import "ABI7_0_0RCTConvert.h"
#import "ABI7_0_0RCTLog.h"
#import "ABI7_0_0RCTUtils.h"
#import "UIView+ReactABI7_0_0.h"

@implementation UIView (ABI7_0_0RCTViewUnmounting)
...
- (void)ReactABI7_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI7_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI7_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI7_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ReactABI7_0_0_findClipView
{
  ...
  while (testView && !passedRootView) {
    ...
    if ([testView isReactABI7_0_0RootView]) {
      passedRootView = YES;
    }
...
static NSString *ABI7_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI7_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI7_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI7_0_0RCTBorderStyleSolid;
...
ABI7_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI7_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI7_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI7_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI7_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI7_0_0RCTPointerEventsNone || _pointerEvents == ABI7_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI7_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI7_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI7_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI7_0_0Tag: %@;", self.ReactABI7_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI7_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI7_0_0_remountAllSubviews
{
  if (_ReactABI7_0_0Subviews) {
    NSUInteger index = 0;
    for (UIView *view in _ReactABI7_0_0Subviews) {
      if (view.superview != self) {
        ...
        [view ReactABI7_0_0_remountAllSubviews];
      }
      index++;
    }
  } else {
    // If ReactABI7_0_0_subviews is nil, we must already be showing all subviews
    [super ReactABI7_0_0_remountAllSubviews];
  }
}
...
- (void)remountSubview:(UIView *)view
{
  ...
  NSInteger index = 0;
  for (UIView *subview in _ReactABI7_0_0Subviews) {
    if (subview == view) {
      [self insertSubview:view atIndex:index];
      break;
    }
  ...
  if (view.clipsToBounds) {
    ...
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI7_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)ReactABI7_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (_ReactABI7_0_0Subviews == nil) {
    // Use default behavior if unmounting is disabled
    return [super ReactABI7_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (_ReactABI7_0_0Subviews.count == 0) {
...
{
  if (removeClippedSubviews && !_ReactABI7_0_0Subviews) {
    _ReactABI7_0_0Subviews = [self.subviews mutableCopy];
  } else if (!removeClippedSubviews && _ReactABI7_0_0Subviews) {
    [self ReactABI7_0_0_remountAllSubviews];
    _ReactABI7_0_0Subviews = nil;
  }
}

- (BOOL)removeClippedSubviews
{
  return _ReactABI7_0_0Subviews != nil;
}
...
- (void)insertReactABI7_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  if (_ReactABI7_0_0Subviews == nil) {
    ...
    UIView *clipView = [self ReactABI7_0_0_findClipView];
    if (clipView) {
...
{
  // The _ReactABI7_0_0Subviews array is only used when we have hidden
  // offscreen views. If _ReactABI7_0_0Subviews is nil, we can assume
  // that [self ReactABI7_0_0Subviews] and [self subviews] are the same

  return _ReactABI7_0_0Subviews ?: self.subviews;
}
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI7_0_0_findClipView];
  if (clipView) {
    [self ReactABI7_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI7_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI7_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI7_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI7_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI7_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI7_0_0RCTCornerRadii){
...
{
  return (ABI7_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI7_0_0RCTUpdateShadowPathForView(self);

  const ABI7_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI7_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI7_0_0RCTRunningInTestEnvironment() &&
  ABI7_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI7_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI7_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI7_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI7_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI7_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI7_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI7_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI7_0_0RCTUpdateShadowPathForView(ABI7_0_0RCTView *view)
{
  if (ABI7_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI7_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI7_0_0RCTCornerInsets cornerInsets = ABI7_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI7_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI7_0_0RCTLogWarn(@"View #%@ of type %@ has a shadow set but cannot calculate "
                 "shadow efficiently. Consider setting a background color to "
                 "fix this, or apply the shadow to a more specific component.",
                 view.ReactABI7_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI7_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI7_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI7_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI7_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI7_0_0RCTBorderStyle)style \
  {                                                    \
