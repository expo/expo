...
const NSInteger ABI7_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI7_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI7_0_0RCTGetEventID(id<ABI7_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI7_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI7_0_0RCTEvent>> *_events;
...
{
  [_bridge enqueueJSCall:@"ABI7_0_0RCTNativeAppEventEmitter.emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI7_0_0RCT_DEBUG) {
    ABI7_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI7_0_0 tag");
  }

  name = ABI7_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI7_0_0RCTEventEmitter.receiveEvent"
...
- (void)sendEvent:(id<ABI7_0_0RCTEvent>)event
{
  ...
  NSNumber *eventID = ABI7_0_0RCTGetEventID(event);

  id<ABI7_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI7_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
  ...
  if (scheduleEventsDispatch) {
    ...
    } queue:ABI7_0_0RCTJSThread];
  }
}

- (void)dispatchEvent:(id<ABI7_0_0RCTEvent>)event
{
...
@implementation ABI7_0_0RCTBridge (ABI7_0_0RCTEventDispatcher)

- (ABI7_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI7_0_0RCTEventDispatcher class]];
}
