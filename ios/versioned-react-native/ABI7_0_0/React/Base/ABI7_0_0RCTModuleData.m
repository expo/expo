...
@implementation ABI7_0_0RCTModuleData
{
...
- (void)setUp
{
  ...
  _hasConstantsToExport = ABI7_0_0RCTClassOverridesInstanceMethod(_moduleClass, @selector(constantsToExport));
}
...
- (instancetype)initWithModuleInstance:(id<ABI7_0_0RCTBridgeModule>)instance
                                bridge:(ABI7_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _bridge = bridge;
...
{
  ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"[ABI7_0_0RCTModuleData setUpInstanceAndBridge] [_instanceLock lock]", nil);
  [_instanceLock lock];
  if (!_setupComplete && _bridge.valid) {
    if (!_instance) {
      if (ABI7_0_0RCT_DEBUG && _requiresMainThreadSetup) {
        ABI7_0_0RCTAssertMainThread();
      }
      ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"[ABI7_0_0RCTModuleData setUpInstanceAndBridge] [_moduleClass new]", nil);
      ...
      if (!_instance) {
        ...
        _setupComplete = YES;
        ABI7_0_0RCTLogWarn(@"The module %@ is returning nil from its constructor. You "
    ...
    if (ABI7_0_0RCTProfileIsProfiling()) {
      ABI7_0_0RCTProfileHookInstance(_instance);
    }
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"[ABI7_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
      [(id)_instance setValue:_bridge forKey:@"bridge"];
    }
    @catch (NSException *exception) {
      ABI7_0_0RCTLogError(@"%@ has no setter or ivar for its bridge, which is not "
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"[ABI7_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI7_0_0RCTDidInitializeModuleNotification
                                                        object:_bridge
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"[ABI7_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ReactABI7_0_0.%@Queue", self.name];
      ...
      if (implementsMethodQueue) {
        ...
        @catch (NSException *exception) {
          ABI7_0_0RCTLogError(@"%@ is returning nil for its methodQueue, which is not "
...
- (id<ABI7_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI7_0_0RCTModuleData instanceForClass:%@]", _moduleClass], nil);
    if (_requiresMainThreadSetup) {
      ...
      ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, @"[ABI7_0_0RCTModuleData instance] main thread setup", nil);
...
- (NSString *)name
{
  return ABI7_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI7_0_0RCTBridgeMethod>> *)methods
{
  if (!_methods) {
    NSMutableArray<id<ABI7_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
    ...
    while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
      ...
      for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
          ...
          id<ABI7_0_0RCTBridgeMethod> moduleMethod =
            [[ABI7_0_0RCTModuleMethod alloc] initWithMethodSignature:entries[1]
...
- (void)gatherConstants
{
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI7_0_0RCTModuleData gatherConstants] %@", _moduleClass], nil);
    (void)[self instance];
    ABI7_0_0RCTExecuteOnMainThread(^{
...
- (NSArray *)config
{
  ...
  ABI7_0_0RCT_PROFILE_BEGIN_EVENT(ABI7_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI7_0_0RCTModuleData config] %@", _moduleClass], nil);

  NSMutableArray<NSString *> *methods = self.methods.count ? [NSMutableArray new] : nil;
  NSMutableArray<NSNumber *> *asyncMethods = nil;
  for (id<ABI7_0_0RCTBridgeMethod> method in self.methods) {
    if (method.functionType == ABI7_0_0RCTFunctionTypePromise) {
      if (!asyncMethods) {
  ...
  ABI7_0_0RCT_PROFILE_END_EVENT(ABI7_0_0RCTProfileTagAlways, [NSString stringWithFormat:@"[ABI7_0_0RCTModuleData config] %@", _moduleClass], nil);
  return config;
}
