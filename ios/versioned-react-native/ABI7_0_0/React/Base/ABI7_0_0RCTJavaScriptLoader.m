...
uint32_t const ABI7_0_0RCTRAMBundleMagicNumber = 0xFB0BD1E5;

@implementation ABI7_0_0RCTJavaScriptLoader

ABI7_0_0RCT_NOT_IMPLEMENTED(- (instancetype)init)

+ (void)loadBundleAtURL:(NSURL *)scriptURL onComplete:(ABI7_0_0RCTSourceLoadBlock)onComplete
{
  // Sanitize the script URL
  scriptURL = [ABI7_0_0RCTConvert NSURL:scriptURL.absoluteString];
  ...
  if (scriptURL.fileURL) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      if (!bundle) {
        onComplete(ABI7_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Error opening bundle %@", scriptURL.path]), source);
        return;
      }
      ...
      if (fread(&magicNumber, sizeof(magicNumber), 1, bundle) != 1) {
        fclose(bundle);
        onComplete(ABI7_0_0RCTErrorWithMessage(@"Error reading bundle"), source);
        return;
      }
      ...
      ABI7_0_0RCTPerformanceLoggerSet(ABI7_0_0RCTPLBundleSize, sourceLength);
      fclose(bundle);
  ...
  NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithURL:scriptURL completionHandler:
    ...
    if (error) {
      if ([error.domain isEqualToString:NSURLErrorDomain]) {
        NSString *desc = [@"Could not connect to development server.\n\nEnsure the following:\n- Node server is running and available on the same network - run 'npm start' from ReactABI7_0_0-native root\n- Node server URL is correctly set in AppDelegate\n\nURL: " stringByAppendingString:scriptURL.absoluteString];
    ...
    if ([response isKindOfClass:[NSHTTPURLResponse class]] && ((NSHTTPURLResponse *)response).statusCode != 200) {
      ...
      NSDictionary *userInfo;
      NSDictionary *errorDetails = ABI7_0_0RCTJSONParse(rawText, nil);
    ...
    ABI7_0_0RCTPerformanceLoggerSet(ABI7_0_0RCTPLBundleSize, data.length);
    onComplete(nil, data);
  }];
