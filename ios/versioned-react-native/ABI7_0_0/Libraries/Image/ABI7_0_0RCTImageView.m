...
#import "ABI7_0_0RCTImageBlurUtils.h"
 ...
 * at `idealSize`.
 */
static BOOL ABI7_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
@interface ABI7_0_0RCTImageView ()

@property (nonatomic, copy) ABI7_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI7_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI7_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI7_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI7_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI7_0_0RCTImageView
{
   ...
   * if any.
   */
  ABI7_0_0RCTImageLoaderCancellationBlock _reloadImageCancellationBlock;
}
...
- (void)setSource:(ABI7_0_0RCTImageSource *)source
{
...
{
  ABI7_0_0RCTImageLoaderCancellationBlock previousCancellationBlock = _reloadImageCancellationBlock;
...
- (void)reloadImage
{
  ...
  if (_source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI7_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (_onProgress) {
    ...
    CGFloat imageScale = ABI7_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    ABI7_0_0RCTImageSource *source = _source;
    CGFloat blurRadius = _blurRadius;
    __weak ABI7_0_0RCTImageView *weakSelf = self;
    _reloadImageCancellationBlock = [_bridge.imageLoader loadImageWithoutClipping:_source.imageURL.absoluteString
                                                                             size:imageSize
                                                                            scale:imageScale
                                                                       resizeMode:(ABI7_0_0RCTResizeMode)self.contentMode
                                                                    progressBlock:progressHandler
                                                                  completionBlock:^(NSError *error, UIImage *loadedImage) {
      ABI7_0_0RCTImageView *strongSelf = weakSelf;
      void (^setImageBlock)(UIImage *) = ^(UIImage *image) {
        if (![source isEqual:strongSelf.source]) {
          // Bail out if source has changed since we started loading
          return;
        }
        if (image.ReactABI7_0_0KeyframeAnimation) {
          [strongSelf.layer addAnimation:image.ReactABI7_0_0KeyframeAnimation forKey:@"contents"];
        } else {
      ...
      if (blurRadius > __FLT_EPSILON__) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          UIImage *image = ABI7_0_0RCTBlurredImageWithRadius(loadedImage, blurRadius);
...
{
  [super ReactABI7_0_0SetFrame:frame];
  ...
  if (!self.image || self.image == _defaultImage) {
    ...
    CGSize idealSize = ABI7_0_0RCTTargetSize(imageSize, self.image.scale, frame.size,
                                     ABI7_0_0RCTScreenScale(), (ABI7_0_0RCTResizeMode)self.contentMode, YES);

    if (ABI7_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize)) {
      if (ABI7_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
        ABI7_0_0RCTLogInfo(@"[PERF IMAGEVIEW] Reloading image %@ as size %@", _source.imageURL, NSStringFromCGSize(idealSize));
