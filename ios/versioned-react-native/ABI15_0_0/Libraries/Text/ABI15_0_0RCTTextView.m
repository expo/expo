...
@interface ABI15_0_0RCTUITextView : UITextView

@property (nonatomic, assign) BOOL textWasPasted;

@end

@implementation ABI15_0_0RCTUITextView
{
...
- (void)didMoveToWindow
{
  if (_jsRequestingFirstResponder) {
    ...
    [self ReactABI15_0_0DidMakeFirstResponder];
  }
}

@end

@implementation ABI15_0_0RCTTextView
{
  ...
  ABI15_0_0RCTText *_richTextView;
...
{
  ABI15_0_0RCTAssertParam(eventDispatcher);
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _textView = [[ABI15_0_0RCTUITextView alloc] initWithFrame:CGRectZero];
...
- (void)insertReactABI15_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI15_0_0Subview:subview atIndex:index];
  if ([subview isKindOfClass:[ABI15_0_0RCTText class]]) {
    if (_richTextView) {
      ABI15_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI15_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI15_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI15_0_0RCTReactABI15_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI15_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI15_0_0TagFromString(_pendingAttributedText);
...
- (void)updateFrames
{
  ...
  // ABI15_0_0RCTTextField defaults, using the system defaults of font size 17 and a
  // height of 31 points.
  //
...
- (BOOL)textView:(ABI15_0_0RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (textView.textWasPasted) {
    ...
    if (_blurOnSubmit && [text isEqualToString:@"\n"]) {
      // TODO: the purpose of blurOnSubmit on ABI15_0_0RCTextField is to decide if the
      // field should lose focus when return is pressed or not. We're cheating a
      // bit here by using it on ABI15_0_0RCTextView to decide if return character should
...
- (void)textViewDidChangeSelection:(ABI15_0_0RCTUITextView *)textView
{
...
- (void)textViewDidChange:(UITextView *)textView
{
  ...
  if (!self.ReactABI15_0_0Tag || !_onChange) {
    return;
  }
...
- (void)ReactABI15_0_0WillMakeFirstResponder
{
  [_textView ReactABI15_0_0WillMakeFirstResponder];
}
...
- (void)ReactABI15_0_0DidMakeFirstResponder
{
  [_textView ReactABI15_0_0DidMakeFirstResponder];
}
