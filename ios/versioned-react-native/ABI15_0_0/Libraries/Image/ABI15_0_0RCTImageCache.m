...
static const NSUInteger ABI15_0_0RCTMaxCachableDecodedImageSizeInBytes = 1048576; // 1MB

static NSString *ABI15_0_0RCTCacheKeyForImage(NSString *imageTag, CGSize size, CGFloat scale,
                                     ABI15_0_0RCTResizeMode resizeMode, NSString *responseDate)
{
    return [NSString stringWithFormat:@"%@|%g|%g|%g|%zd|%@",
            imageTag, size.width, size.height, scale, resizeMode, responseDate];
}

@implementation ABI15_0_0RCTImageCache
{
                 ...
                 forKey:(NSString *)cacheKey
{
  ...
  if (bytes <= ABI15_0_0RCTMaxCachableDecodedImageSizeInBytes) {
...
{
  NSString *cacheKey = ABI15_0_0RCTCacheKeyForImage(url, size, scale, resizeMode, responseDate);
