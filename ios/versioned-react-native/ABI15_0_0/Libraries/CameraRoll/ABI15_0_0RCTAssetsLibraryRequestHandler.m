...
@implementation ABI15_0_0RCTAssetsLibraryRequestHandler
{
     ...
     withDelegate:(id<ABI15_0_0RCTURLRequestDelegate>)delegate
{
  ...
  [[self assetsLibrary] assetForURL:request.URL resultBlock:^(ALAsset *asset) {
    ...
    if (asset) {
      ...
      NSError *error = ABI15_0_0RCTErrorWithMessage(errorMessage);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
    }
...
@implementation ABI15_0_0RCTBridge (ABI15_0_0RCTAssetsLibraryImageLoader)

- (ALAssetsLibrary *)assetsLibrary
{
  return [[self moduleForClass:[ABI15_0_0RCTAssetsLibraryRequestHandler class]] assetsLibrary];
}
