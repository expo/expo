...
#import "ABI15_0_0RCTCameraRollManager.h"
...
#import <ReactABI15_0_0/ABI15_0_0RCTImageLoader.h>
#import <ReactABI15_0_0/ABI15_0_0RCTLog.h>
#import <ReactABI15_0_0/ABI15_0_0RCTUtils.h>

#import "ABI15_0_0RCTAssetsLibraryRequestHandler.h"

@implementation ABI15_0_0RCTConvert (ALAssetGroup)

ABI15_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI15_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI15_0_0RCTCameraRollManager

ABI15_0_0RCT_EXPORT_MODULE()

@synthesize bridge = _bridge;

NSString *const ABI15_0_0RCTErrorUnableToLoad = @"E_UNABLE_TO_LOAD";
NSString *const ABI15_0_0RCTErrorUnableToSave = @"E_UNABLE_TO_SAVE";

ABI15_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
  ...
  if ([type isEqualToString:@"video"]) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [self->_bridge.assetsLibrary writeVideoAtPathToSavedPhotosAlbum:request.URL completionBlock:^(NSURL *assetURL, NSError *saveError) {
        if (saveError) {
          reject(ABI15_0_0RCTErrorUnableToSave, nil, saveError);
        } else {
    ...
    [_bridge.imageLoader loadImageWithURLRequest:request
      ...
      if (loadError) {
        reject(ABI15_0_0RCTErrorUnableToLoad, nil, loadError);
        return;
      }
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        [self->_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
          if (saveError) {
            ABI15_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
            reject(ABI15_0_0RCTErrorUnableToSave, nil, saveError);
          } else {
            resolve(assetURL.absoluteString);
          }
...
static void ABI15_0_0RCTResolvePromise(ABI15_0_0RCTPromiseResolveBlock resolve,
...
ABI15_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI15_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI15_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger first = [ABI15_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI15_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI15_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI15_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI15_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI15_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI15_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI15_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
    reject(ABI15_0_0RCTErrorUnableToLoad, nil, error);
  }];
}
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI15_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
