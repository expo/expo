...
#import "ABI15_0_0RCTWebSocketExecutor.h"
...
typedef void (^ABI15_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI15_0_0RCTWebSocketExecutor () <ABI15_0_0RCTSRWebSocketDelegate>

@end

@implementation ABI15_0_0RCTWebSocketExecutor
{
  ABI15_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI15_0_0RCTWSMessageCallback> *_callbacks;
...
- (void)setUp
{
  if (!_url) {
    ...
    _url = [ABI15_0_0RCTConvert NSURL:URLString];
  }

  _jsQueue = dispatch_queue_create("com.facebook.ReactABI15_0_0.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  ...
  if (![self connectToProxy]) {
    ...
    NSString *error = [NSString stringWithFormat:@"Connection to %@ timed out. Are you "
                       ...
                       "you have the right IP address in `ABI15_0_0RCTWebSocketExecutor.m`.", _url];
    _setupError = ABI15_0_0RCTErrorWithMessage(error);
    ABI15_0_0RCTFatal(_setupError);
    return;
  }
...
- (BOOL)connectToProxy
{
  ...
  return connected == 0 && _socket.readyState == ABI15_0_0RCTSR_OPEN;
}
...
- (BOOL)prepareJSRuntime
{
  ...
  if (initError) {
    ABI15_0_0RCTLogInfo(@"Websocket runtime setup failed: %@", initError);
  }
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI15_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI15_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
{
  dispatch_semaphore_signal(_socketOpenSemaphore);
  ABI15_0_0RCTLogInfo(@"WebSocket connection failed with error %@", error);
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message onReply:(ABI15_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    ...
    [self->_socket send:ABI15_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}

- (void)executeApplicationScript:(NSData *)script sourceURL:(NSURL *)URL onComplete:(ABI15_0_0RCTJavaScriptCompleteBlock)onComplete
{
  NSDictionary<NSString *, id> *message = @{
    @"method": @"executeApplicationScript",
    @"url": ABI15_0_0RCTNullIfNil(URL.absoluteString),
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI15_0_0RCTJavaScriptCallback)onComplete
{
  ABI15_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  NSDictionary<NSString *, id> *message = @{
    @"method": method,
    @"arguments": arguments
  };
  [self sendMessage:message onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    ...
    id objcValue = ABI15_0_0RCTJSONParse(result, NULL);
    onComplete(objcValue, nil);
  }];
}

- (void)injectJSONText:(NSString *)script asGlobalObjectNamed:(NSString *)objectName callback:(ABI15_0_0RCTJavaScriptCompleteBlock)onComplete
{
  dispatch_async(_jsQueue, ^{
...
{
  ABI15_0_0RCTExecuteOnMainQueue(block);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI15_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI15_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
