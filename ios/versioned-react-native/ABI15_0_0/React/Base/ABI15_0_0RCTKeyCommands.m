...
static BOOL ABI15_0_0RCTIsIOS8OrEarlier()
{
  return [UIDevice currentDevice].systemVersion.floatValue < 9;
}

@interface ABI15_0_0RCTKeyCommand : NSObject <NSCopying>
...
@implementation ABI15_0_0RCTKeyCommand
...
- (BOOL)isEqual:(ABI15_0_0RCTKeyCommand *)object
{
  if (![object isKindOfClass:[ABI15_0_0RCTKeyCommand class]]) {
    return NO;
  }
...
@interface ABI15_0_0RCTKeyCommands ()

@property (nonatomic, strong) NSMutableSet<ABI15_0_0RCTKeyCommand *> *commands;

@end

@implementation UIResponder (ABI15_0_0RCTKeyCommands)

+ (UIResponder *)ABI15_0_0RCT_getFirstResponder:(UIResponder *)view
{
  ...
  if (view.isFirstResponder) {
    ...
    if ([(UIViewController *)view parentViewController]) {
      firstResponder = [UIResponder ABI15_0_0RCT_getFirstResponder: [(UIViewController *)view parentViewController]];
    }
    return firstResponder ? firstResponder : [UIResponder ABI15_0_0RCT_getFirstResponder: [(UIViewController *)view view]];
  } else if ([view isKindOfClass:[UIView class]]) {
    for (UIView *subview in [(UIView *)view subviews]) {
      firstResponder = [UIResponder ABI15_0_0RCT_getFirstResponder: subview];
      if (firstResponder) {
...
- (NSArray<UIKeyCommand *> *)ABI15_0_0RCT_keyCommands
{
  NSSet<ABI15_0_0RCTKeyCommand *> *commands = [ABI15_0_0RCTKeyCommands sharedInstance].commands;
 ...
 */
- (void)ABI15_0_0RCT_handleKeyCommand:(UIKeyCommand *)key
{
  // NOTE: throttle the key handler because on iOS 9 the handleKeyCommand:
  // method gets called repeatedly if the command key is held down.
  static NSTimeInterval lastCommand = 0;
  if (ABI15_0_0RCTIsIOS8OrEarlier() || CACurrentMediaTime() - lastCommand > 0.5) {
    for (ABI15_0_0RCTKeyCommand *command in [ABI15_0_0RCTKeyCommands sharedInstance].commands) {
 ...
 */
- (void)ABI15_0_0RCT_handleDoublePressKeyCommand:(UIKeyCommand *)key
{
    ...
    if (CACurrentMediaTime() - lastCommand < 0.2 &&
      ...
      for (ABI15_0_0RCTKeyCommand *command in [ABI15_0_0RCTKeyCommands sharedInstance].commands) {
        if ([command.keyCommand.input isEqualToString:key.input] &&
          ...
          // NOTE: throttle the key handler because on iOS 9 the handleKeyCommand:
          // method gets called repeatedly if the command key is held down.
          if (ABI15_0_0RCTIsIOS8OrEarlier() || CACurrentMediaTime() - lastDoubleCommand > 0.5) {
            command.block(key);
...
@implementation UIApplication (ABI15_0_0RCTKeyCommands)

// Required for iOS 8.x
- (BOOL)ABI15_0_0RCT_sendAction:(SEL)action to:(id)target from:(id)sender forEvent:(UIEvent *)event
{
  if (action == @selector(ABI15_0_0RCT_handleKeyCommand:)) {
    [self ABI15_0_0RCT_handleKeyCommand:sender];
    return YES;
  } else if (action == @selector(ABI15_0_0RCT_handleDoublePressKeyCommand:)) {
    [self ABI15_0_0RCT_handleDoublePressKeyCommand:sender];
    return YES;
  }
  return [self ABI15_0_0RCT_sendAction:action to:target from:sender forEvent:event];
}
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  if (input.length && flags && ABI15_0_0RCTIsIOS8OrEarlier()) {
  ...
  UIKeyCommand *command = [UIKeyCommand keyCommandWithInput:input
                                              modifierFlags:flags
                                                     action:@selector(ABI15_0_0RCT_handleKeyCommand:)];

  ABI15_0_0RCTKeyCommand *keyCommand = [[ABI15_0_0RCTKeyCommand alloc] initWithKeyCommand:command block:block];
                        ...
                        modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI15_0_0RCTKeyCommand *command in _commands.allObjects) {
                         ...
                         modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI15_0_0RCTKeyCommand *command in _commands) {
    if ([command matchesInput:input flags:flags]) {
      return YES;
    }
  }
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  UIKeyCommand *command = [UIKeyCommand keyCommandWithInput:input
                                              modifierFlags:flags
                                                     action:@selector(ABI15_0_0RCT_handleDoublePressKeyCommand:)];
