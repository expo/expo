...
const NSInteger ABI15_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI15_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI15_0_0RCTGetEventID(id<ABI15_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI15_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI15_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI15_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI15_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
{
  [_bridge enqueueJSCall:@"ABI15_0_0RCTDeviceEventEmitter"
                  method:@"emit"
                    args:body ? @[name, body] : @[name]
              completion:NULL];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI15_0_0RCT_DEBUG) {
    ABI15_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI15_0_0 tag");
  }

  name = ABI15_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI15_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI15_0_0RCTEvent>)event
{
  ...
  for (id<ABI15_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI15_0_0RCTGetEventID(event);

  id<ABI15_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI15_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)dispatchEvent:(id<ABI15_0_0RCTEvent>)event
{
...
@implementation ABI15_0_0RCTBridge (ABI15_0_0RCTEventDispatcher)

- (ABI15_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI15_0_0RCTEventDispatcher class]];
}
