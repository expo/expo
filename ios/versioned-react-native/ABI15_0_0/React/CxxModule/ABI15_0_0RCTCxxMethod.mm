...
#import <cxxReactABI15_0_0/ABI15_0_0JsArgumentHelpers.h>
...
@implementation ABI15_0_0RCTCxxMethod
{
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI15_0_0RCTLogError(@"Method %@.%s expects at least %lu arguments, but got %tu",
                ABI15_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI15_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
    ...
    if (_method->callbacks == 2) {
      ...
      second = ^(std::vector<folly::dynamic> args) {
        [bridge enqueueCallback:id2 args:ABI15_0_0RCTConvertFollyDynamic(folly::dynamic(args.begin(), args.end()))];
      };
    } else {
    ...
    first = ^(std::vector<folly::dynamic> args) {
      [bridge enqueueCallback:id1 args:ABI15_0_0RCTConvertFollyDynamic(folly::dynamic(args.begin(), args.end()))];
    };
  }

  folly::dynamic args = [ABI15_0_0RCTConvert folly_dynamic:arguments];
  ...
  try {
    if (_method->func) {
      ...
      return ABI15_0_0RCTConvertFollyDynamic(result);
    }
  } catch (const facebook::xplat::JsArgumentException &ex) {
    ABI15_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI15_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
...
- (ABI15_0_0RCTFunctionType)functionType
{
  ...
  return _method->syncFunc ? ABI15_0_0RCTFunctionTypeSync : ABI15_0_0RCTFunctionTypeNormal;
}
