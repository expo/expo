...
static NSString *const ABI15_0_0RCTShowDevMenuNotification = @"ABI15_0_0RCTShowDevMenuNotification";

@implementation UIWindow (ABI15_0_0RCTDevMenu)

- (void)ABI15_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI15_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

@implementation ABI15_0_0RCTDevMenuItem
{
  ABI15_0_0RCTDevMenuItemTitleBlock _titleBlock;
  dispatch_block_t _handler;
}

- (instancetype)initWithTitleBlock:(ABI15_0_0RCTDevMenuItemTitleBlock)titleBlock
...
typedef void(^ABI15_0_0RCTDevMenuAlertActionHandler)(UIAlertAction *action);

@interface ABI15_0_0RCTDevMenu () <ABI15_0_0RCTBridgeModule, ABI15_0_0RCTInvalidating>

@end

@implementation ABI15_0_0RCTDevMenu
{
  UIAlertController *_actionSheet;
  NSMutableArray<ABI15_0_0RCTDevMenuItem *> *_extraMenuItems;
}
  ...
  if ((self = [super init])) {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showOnShake)
                                                 name:ABI15_0_0RCTShowDevMenuNotification
                                               object:nil];
    _extraMenuItems = [NSMutableArray new];

#if TARGET_IPHONE_SIMULATOR
    ABI15_0_0RCTKeyCommands *commands = [ABI15_0_0RCTKeyCommands sharedInstance];
...
{
  [self addItem:[ABI15_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
- (NSArray<ABI15_0_0RCTDevMenuItem *> *)_menuItemsToPresent
{
  NSMutableArray<ABI15_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items
  __weak ABI15_0_0RCTBridge *bridge = _bridge;
  __weak ABI15_0_0RCTDevSettings *devSettings = _bridge.devSettings;

  [items addObject:[ABI15_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload" handler:^{
    [bridge reload];
  }]];

  NSString *executorName = devSettings.websocketExecutorName ?: @"Remote JS";
  if (!devSettings.isRemoteDebuggingAvailable) {
    [items addObject:[ABI15_0_0RCTDevMenuItem buttonItemWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", executorName] handler:^{
      UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", executorName]
        message:[NSString stringWithFormat:@"You need to include the ABI15_0_0RCTWebSocket library to enable %@ debugging", executorName]
        preferredStyle:UIAlertControllerStyleAlert];
      [ABI15_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
    }]];
  } else {
    [items addObject:[ABI15_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
  ...
  if (devSettings.isJSCSamplingProfilerAvailable) {
    ...
    // duplicated in ABI15_0_0RCTJSCExecutor
    [items addObject:[ABI15_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
...
ABI15_0_0RCT_EXPORT_METHOD(show)
{
  if (_actionSheet || !_bridge || ABI15_0_0RCTRunningInAppExtension()) {
    return;
  }

  NSString *title = [NSString stringWithFormat:@"ReactABI15_0_0 Native: Development (%@)", [_bridge class]];
  ...
  NSArray<ABI15_0_0RCTDevMenuItem *> *items = [self _menuItemsToPresent];
  for (ABI15_0_0RCTDevMenuItem *item in items) {
  ...
  [ABI15_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:nil];
}

- (ABI15_0_0RCTDevMenuAlertActionHandler)alertActionHandlerForDevItem:(ABI15_0_0RCTDevMenuItem *__nullable)item
{
...
#define WARN_DEPRECATED_DEV_MENU_EXPORT() ABI15_0_0RCTLogWarn(@"Using deprecated method %s, use ABI15_0_0RCTDevSettings instead", __func__)
...
ABI15_0_0RCT_EXPORT_METHOD(debugRemotely:(BOOL)enableDebug)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
...
- (void)addItem:(ABI15_0_0RCTDevMenu *)item {}
...
@implementation  ABI15_0_0RCTBridge (ABI15_0_0RCTDevMenu)

- (ABI15_0_0RCTDevMenu *)devMenu
{
#if ABI15_0_0RCT_DEV
  return [self moduleForClass:[ABI15_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
