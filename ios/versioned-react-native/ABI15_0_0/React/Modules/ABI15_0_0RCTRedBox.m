...
@protocol ABI15_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI15_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI15_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI15_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI15_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI15_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI15_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI15_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI15_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI15_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI15_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI15_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI15_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI15_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI15_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI15_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI15_0_0RCTKeyCommands for this, but since
...
@interface ABI15_0_0RCTRedBox () <ABI15_0_0RCTInvalidating, ABI15_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI15_0_0RCTRedBox
{
  ABI15_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI15_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (void)registerErrorCustomizer:(id<ABI15_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI15_0_0RCTErrorInfo *)_customizeError:(ABI15_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI15_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI15_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI15_0_0RCTJSStackFrame *> *stack = [ABI15_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI15_0_0RCTJSStackFrame class]]) {
    stack = [ABI15_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI15_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI15_0_0RCTErrorInfo *errorInfo = [[ABI15_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI15_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI15_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI15_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI15_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI15_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI15_0_0RCTRedBoxWindow *)redBoxWindow
{
  [_bridge reload];
  [self dismiss];
}
...
@implementation ABI15_0_0RCTBridge (ABI15_0_0RCTRedBox)

- (ABI15_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI15_0_0RCTRedBox class]];
}
...
- (ABI15_0_0RCTRedBox *)redBox { return nil; }
