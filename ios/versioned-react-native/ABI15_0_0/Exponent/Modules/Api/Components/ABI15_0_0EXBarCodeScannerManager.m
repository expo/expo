...
#import "ABI15_0_0EXBarCodeScanner.h"
#import "ABI15_0_0EXBarCodeScannerManager.h"
...
@implementation ABI15_0_0EXBarCodeScannerManager

ABI15_0_0RCT_EXPORT_MODULE(ExponentBarCodeScannerManager);
ABI15_0_0RCT_EXPORT_VIEW_PROPERTY(onBarCodeRead, ABI15_0_0RCTDirectEventBlock);
...
- (UIView *)view
{
  ...
  if (!self.camera) {
    self.camera = [[ABI15_0_0EXBarCodeScanner alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
- (NSDictionary *)constantsToExport
{
  return @{
    ...
    @"Type" :
        @{@"front" : @(ABI15_0_0EXBarCodeScannerTypeFront), @"back" : @(ABI15_0_0EXBarCodeScannerTypeBack)},
    @"TorchMode" : @{
      @"off" : @(ABI15_0_0EXBarCodeScannerTorchModeOff),
      @"on" : @(ABI15_0_0EXBarCodeScannerTorchModeOn),
      @"auto" : @(ABI15_0_0EXBarCodeScannerTorchModeAuto)
    }
  };
}
...
ABI15_0_0RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, ABI15_0_0EXBarCodeScanner)
{
  NSInteger type = [ABI15_0_0RCTConvert NSInteger:json];
    ...
    dispatch_async(self.sessionQueue, ^{
      ...
      if ([self.session canAddInput:captureDeviceInput]) {
        ...
        if (self.videoCaptureDeviceInput) {
          ...
          ABI15_0_0RCTLogWarn(@"%s: Can't add null video capture device, doing nothing instead.", __func__);
        }
      }
...
ABI15_0_0RCT_CUSTOM_VIEW_PROPERTY(torchMode, NSInteger, ABI15_0_0EXBarCodeScanner)
{
  dispatch_async(self.sessionQueue, ^{
    NSInteger torchMode = [ABI15_0_0RCTConvert NSInteger:json];
...
ABI15_0_0RCT_CUSTOM_VIEW_PROPERTY(barCodeTypes, NSArray, ABI15_0_0EXBarCodeScanner)
{
  NSArray *types = [ABI15_0_0RCTConvert NSArray:json];
  NSSet *validTypes = [NSSet setWithArray:[[self class] validBarCodeTypes].allValues];
  for (id type in types) {
    if (![validTypes containsObject:type]) {
      ABI15_0_0RCTLogWarn(@"Unsupported BarCodeType: %@", type);
      return;
    }
  }
...
- (void)startSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI15_0_0EXBarCodeScannerManager *weakSelf = self;
              ...
              [NSNotificationCenter.defaultCenter
                  addObserverForName:AVCaptureSessionRuntimeErrorNotification
                          ...
                          usingBlock:^(NSNotification *note) {
                            ABI15_0_0EXBarCodeScannerManager *strongSelf = weakSelf;
