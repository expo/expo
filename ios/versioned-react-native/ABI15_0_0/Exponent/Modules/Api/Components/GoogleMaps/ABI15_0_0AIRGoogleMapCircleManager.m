//
//  ABI15_0_0AIRGoogleMapCircleManager.m
//
//  Created by Nick Italiano on 10/24/16.
//

#import "ABI15_0_0AIRGoogleMapCircleManager.h"
#import "ABI15_0_0AIRGoogleMapCircle.h"
#import <ReactABI15_0_0/ABI15_0_0RCTBridge.h>
#import <ReactABI15_0_0/UIView+ReactABI15_0_0.h>

@interface ABI15_0_0AIRGoogleMapCircleManager()

@end

@implementation ABI15_0_0AIRGoogleMapCircleManager

ABI15_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  ABI15_0_0AIRGoogleMapCircle *circle = [ABI15_0_0AIRGoogleMapCircle new];
  return circle;
}

ABI15_0_0RCT_EXPORT_VIEW_PROPERTY(radius, double)
ABI15_0_0RCT_REMAP_VIEW_PROPERTY(center, centerCoordinate, CLLocationCoordinate2D)
ABI15_0_0RCT_EXPORT_VIEW_PROPERTY(strokeColor, UIColor)
ABI15_0_0RCT_EXPORT_VIEW_PROPERTY(strokeWidth, double)
ABI15_0_0RCT_EXPORT_VIEW_PROPERTY(fillColor, UIColor)
ABI15_0_0RCT_EXPORT_VIEW_PROPERTY(zIndex, int)

@end
