...
@property (nonatomic, weak) ABI15_0_0EXGLViewManager *viewManager;
...
@implementation ABI15_0_0EXGLView
...
- (instancetype)initWithManager:(ABI15_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
    ...
    _exglCtxId = 0;
    id<ABI15_0_0RCTJavaScriptExecutor> executor = [_viewManager.bridge valueForKey:@"javaScriptExecutor"];
    if ([executor isKindOfClass:NSClassFromString(@"ABI15_0_0RCTJSCExecutor")]) {
      // On JS thread, extract JavaScriptCore context, create ABI15_0_0EXGL context, call JS callback
      __weak __typeof__(self) weakSelf = self;
      __weak __typeof__(executor) weakExecutor = executor;
      [executor executeBlockOnJavaScriptQueue:^{
        __typeof__(self) self = weakSelf;
        ABI15_0_0RCTJSCExecutor *executor = weakExecutor;
        if (self && executor) {
      ...
      ABI15_0_0RCTLog(@"ABI15_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI15_0_0-native/docs/debugging.html)? ABI15_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI15_0_0EXGL.");
    }
  }
  return self;
}
...
- (void)draw
{
  // _exglCtxId may be unset if we get here (on the UI thread) before ABI15_0_0EXGLContextCreate(...) is
  // called on the JS thread to create the ABI15_0_0EXGL context and save its id (see init method). In
  ...
  if (_exglCtxId != 0 && _viewFramebuffer != 0) {
    [EAGLContext setCurrentContext:_eaglCtx];
    EXGLContextSetDefaultFramebuffer(_exglCtxId, _msaaFramebuffer);
