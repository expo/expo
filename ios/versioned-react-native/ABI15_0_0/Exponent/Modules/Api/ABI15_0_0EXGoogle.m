...
NSString *ABI15_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI15_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@end

@implementation ABI15_0_0EXGoogle
{
  ABI15_0_0RCTPromiseResolveBlock _logInResolve;
  ABI15_0_0RCTPromiseRejectBlock _logInReject;
}

ABI15_0_0RCT_EXPORT_MODULE(ExponentGoogle)
                 ...
                 rejecter:(ABI15_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI15_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI15_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI15_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      ...
      _logInReject(ABI15_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI15_0_0RCTPresentedViewController()
                                                   callback:callback];
...
{
  [ABI15_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI15_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
