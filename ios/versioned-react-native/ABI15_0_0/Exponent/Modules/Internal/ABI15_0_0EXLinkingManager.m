...
NSString * const ABI15_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI15_0_0EXLinkingManager ()
...
@implementation ABI15_0_0EXLinkingManager

+ (NSString *)moduleName { return @"ABI15_0_0RCTLinkingManager"; }
...
#pragma mark - ABI15_0_0RCTEventEmitter methods

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI15_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI15_0_0RCTFatal(ABI15_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI15_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}
                  ...
                  reject:(__unused ABI15_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!canOpen) {
    canOpen = [ABI15_0_0RCTSharedApplication() canOpenURL:URL];
  }
  resolve(@(canOpen));
}
...
ABI15_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI15_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI15_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI15_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
...
- (BOOL)_isExponentUrl: (NSURL *)url
{
  ...
  NSString *versionsPath = [[NSBundle mainBundle] pathForResource:@"ABI15_0_0EXSDKVersions" ofType:@"plist"];
