...
#import "ABI25_0_0EXNativeModuleIntrospection.h"
...
@implementation ABI25_0_0EXNativeModuleIntrospection

@synthesize bridge = _bridge;

ABI25_0_0RCT_EXPORT_MODULE(ExpoNativeModuleIntrospection)

ABI25_0_0RCT_REMAP_METHOD(getNativeModuleNamesAsync,
                 nativeModuleNamesWithResolver:(ABI25_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject) {
  NSError *error;
  NSDictionary<NSString *, ABI25_0_0RCTModuleData *> *moduleDataByName = [self _moduleDataFromBridge:&error];
  if (!moduleDataByName) {
...
ABI25_0_0RCT_REMAP_METHOD(introspectNativeModuleAsync,
  ...
  ABI25_0_0RCTModuleData *moduleData = moduleDataByName[name];
  if (!moduleData) {
    resolve(nil);
    return;
  }
  
  NSArray<id<ABI25_0_0RCTBridgeMethod>> *moduleMethods = moduleData.methods;
  ...
  for (id<ABI25_0_0RCTBridgeMethod> method in moduleMethods) {
    NSString *methodName = [NSString stringWithCString:method.JSMethodName encoding:NSASCIIStringEncoding];
    NSString *functionType = [NSString stringWithCString:ABI25_0_0RCTFunctionDescriptorFromType(method.functionType)
...
- (NSDictionary<NSString *, ABI25_0_0RCTModuleData *> *)_moduleDataFromBridge:(NSError **)error {
  ABI25_0_0RCTBridge *bridge = _bridge;
  if (![NSStringFromClass(bridge.class) isEqualToString:@"ABI25_0_0RCTCxxBridge"]) {
    if (error) {
  ...
  NSDictionary<NSString *, ABI25_0_0RCTModuleData *> *moduleDataByName;
  @try {
