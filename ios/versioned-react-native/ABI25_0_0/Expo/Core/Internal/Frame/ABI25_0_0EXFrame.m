...
#define ABI25_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05
#define ABI25_0_0EX_FRAME_ORIENTATION_USE_MANIFEST 0

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI25_0_0EXFrameReactABI25_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI25_0_0EXFrameReactABI25_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI25_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI25_0_0Bridge;

@end

@interface ABI25_0_0EXFrame () <ABI25_0_0RCTInvalidating, ABI25_0_0RCTExceptionsManagerDelegate>

@property (nonatomic, assign) BOOL sourceSet;
@property (nonatomic, assign) BOOL needsReload;

@property (nonatomic, copy) ABI25_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI25_0_0RCTDirectEventBlock onLoadingProgress;
...
@implementation ABI25_0_0EXFrame

@synthesize supportedInterfaceOrientations = _supportedInterfaceOrientations;

ABI25_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI25_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");

    _appManager = [[unversionedAppManagerClass alloc] initWithEXFrame:self];
    [_appManager setDelegate:self];
    _supportedInterfaceOrientations = ABI25_0_0EX_FRAME_ORIENTATION_USE_MANIFEST;
  }
  return self;
}
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI25_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI25_0_0RootView]) {
    [_appManager ReactABI25_0_0RootView].frame = self.bounds;
  }
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
  if (_supportedInterfaceOrientations != ABI25_0_0EX_FRAME_ORIENTATION_USE_MANIFEST) {
...
#pragma mark - ABI25_0_0EXReactABI25_0_0AppManagerDelegate

- (void)ReactABI25_0_0AppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI25_0_0RootView = [appManager ReactABI25_0_0RootView];
  ReactABI25_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI25_0_0RootView];
}

- (void)ReactABI25_0_0AppManagerDidDestroyApp:(id)appManager
{
  if (_tmrReload) {
    [_tmrReload invalidate];
    _tmrReload = nil;
...
- (void)ReactABI25_0_0AppManagerStartedLoadingJavaScript:(id)appManager
{
  if (_onLoadingStart) {
    _onLoadingStart(nil);
  }
...
- (void)ReactABI25_0_0AppManager:(id)appManager loadedJavaScriptWithProgress:(ABI25_0_0RCTLoadingProgress *)progress;
{
  if (_onLoadingProgress) {
    _onLoadingProgress(@{
      @"status": ABI25_0_0RCTNullIfNil(progress.status),
      @"done": ABI25_0_0RCTNullIfNil(progress.done),
      @"total": ABI25_0_0RCTNullIfNil(progress.total),
    });
  }
}

- (void)ReactABI25_0_0AppManagerFinishedLoadingJavaScript:(id)appManager
{
  if (_viewTestTimer) {
    [_viewTestTimer invalidate];
  }
...
- (void)ReactABI25_0_0AppManager:(id)appManager failedToLoadJavaScriptWithError:(NSError *)error
{
  [self _sendLoadingError:error];
}

- (void)ReactABI25_0_0AppManager:(id)appManager failedToDownloadBundleWithError:(NSError *)error
{
  
}

- (void)ReactABI25_0_0AppManagerDidForeground:(id)appManager
{
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI25_0_0RootView] &&
      [_appManager ReactABI25_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI25_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI25_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI25_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI25_0_0RCTErrorWithMessage(message)];
