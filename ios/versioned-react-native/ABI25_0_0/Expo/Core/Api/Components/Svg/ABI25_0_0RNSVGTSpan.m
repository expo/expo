...
#import "ABI25_0_0RNSVGTSpan.h"
#import "ABI25_0_0RNSVGBezierTransformer.h"
#import "ABI25_0_0RNSVGText.h"
#import "ABI25_0_0RNSVGTextPath.h"

@implementation ABI25_0_0RNSVGTSpan
{
    ABI25_0_0RNSVGBezierTransformer *_bezierTransformer;
    CGPathRef _cache;
}
...
{
    __block ABI25_0_0RNSVGBezierTransformer *bezierTransformer;
    [self traverseTextSuperviews:^(__kindof ABI25_0_0RNSVGText *node) {
        if ([node class] == [ABI25_0_0RNSVGTextPath class]) {
            bezierTransformer = [(ABI25_0_0RNSVGTextPath*)node getBezierTransformer];
            return NO;
        }
        return YES;
    }];
...
- (void)traverseTextSuperviews:(BOOL (^)(__kindof ABI25_0_0RNSVGText *node))block
{
    ABI25_0_0RNSVGText *targetView = self;
    BOOL result = block(self);
    
    while (targetView && [targetView class] != [ABI25_0_0RNSVGText class] && result) {
        if (![targetView isKindOfClass:[ABI25_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        
        targetView = (ABI25_0_0RNSVGText*)[targetView superview];
