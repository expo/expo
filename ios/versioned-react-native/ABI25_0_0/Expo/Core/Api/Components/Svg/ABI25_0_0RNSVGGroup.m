...
@implementation ABI25_0_0RNSVGGroup
...
- (void)renderGroupTo:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI25_0_0RNSVGNode *node) {
        if (node.responsible && !svg.responsible) {
            svg.responsible = YES;
        }
        
        if ([node isKindOfClass:[ABI25_0_0RNSVGRenderable class]]) {
            [(ABI25_0_0RNSVGRenderable*)node mergeProperties:self];
        }
        
        [node renderTo:context];
        
        if ([node isKindOfClass:[ABI25_0_0RNSVGRenderable class]]) {
            [(ABI25_0_0RNSVGRenderable*)node resetProperties];
        }
...
- (void)parseReference
{
    ...
    [self traverseSubviews:^(__kindof ABI25_0_0RNSVGNode *node) {
        [node parseReference];
        return YES;
    }];
}
