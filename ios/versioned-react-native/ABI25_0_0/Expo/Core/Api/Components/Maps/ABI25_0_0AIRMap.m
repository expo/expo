...
const CLLocationDegrees ABI25_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI25_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI25_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI25_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI25_0_0AIRMap
{
    UIView *_legalLabel;
    CLLocationManager *_locationManager;
    BOOL _initialRegionSet;

    // Array to manually track ABI25_0_0RN subviews
    //
    // ABI25_0_0AIRMap implicitly creates subviews that aren't regular ABI25_0_0RN children
    // (ABI25_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI25_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI25_0_0-native/blob/v0.16.0/ReactABI25_0_0/Modules/ABI25_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI25_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI25_0_0-native/blob/v0.16.0/Libraries/Text/ABI25_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ReactABI25_0_0Subviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _ReactABI25_0_0Subviews = [NSMutableArray new];
        ...
        self.calloutView = [ABI25_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI25_0_0AIRMapMaxZoomLevel;
    }
    return self;
}
...
- (void)insertReactABI25_0_0Subview:(id<ABI25_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI25_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI25_0_0AIRMapPolyline class]]) {
        ((ABI25_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI25_0_0AIRMapPolygon class]]) {
        ((ABI25_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI25_0_0AIRMapCircle class]]) {
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI25_0_0AIRMapUrlTile class]]) {
        ((ABI25_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI25_0_0AIRMapLocalTile class]]) {
        ((ABI25_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI25_0_0RCTComponent>> *childSubviews = [subview ReactABI25_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertReactABI25_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ReactABI25_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI25_0_0Subview:(id<ABI25_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI25_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI25_0_0RCTComponent>> *childSubviews = [subview ReactABI25_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeReactABI25_0_0Subview:(UIView *)childSubviews[i]];
        }
    }
    [_ReactABI25_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
#pragma mark ABI25_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI25_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
