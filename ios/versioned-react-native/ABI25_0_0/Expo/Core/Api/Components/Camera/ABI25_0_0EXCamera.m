...
@interface ABI25_0_0EXCamera ()<GMVDataOutputDelegate>

@property (nonatomic, weak) ABI25_0_0RCTBridge *bridge;

@property (nonatomic, assign, getter=isSessionPaused) BOOL paused;

@property (nonatomic, strong) ABI25_0_0RCTPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI25_0_0RCTPromiseRejectBlock videoRecordedReject;
@property (nonatomic, strong) id faceDetectorManager;

@property (nonatomic, copy) ABI25_0_0RCTDirectEventBlock onCameraReady;
@property (nonatomic, copy) ABI25_0_0RCTDirectEventBlock onMountError;
@property (nonatomic, copy) ABI25_0_0RCTDirectEventBlock onBarCodeRead;
@property (nonatomic, copy) ABI25_0_0RCTDirectEventBlock onFacesDetected;

@end

@implementation ABI25_0_0EXCamera
...
- (void)updateFlashMode
{
  ...
  if (self.flashMode == ABI25_0_0EXCameraFlashModeTorch) {
    if (![device hasTorch])
      return;
    if (![device lockForConfiguration:&error]) {
      if (error) {
        ABI25_0_0RCTLogError(@"%s: %@", __func__, error);
      }
      return;
    }
...
- (void)updateFocusDepth
{
  ...
  if (device.focusMode != ABI25_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if (![device respondsToSelector:@selector(isLockingFocusWithCustomLensPositionSupported)] || ![device isLockingFocusWithCustomLensPositionSupported]) {
    ABI25_0_0RCTLogWarn(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
    return;
  }
...
- (void)updateWhiteBalance
{
  ...
  if (self.whiteBalance == ABI25_0_0EXCameraWhiteBalanceAuto) {
    [device setWhiteBalanceMode:AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance];
    [device unlockForConfiguration];
  } else {
    AVCaptureWhiteBalanceTemperatureAndTintValues temperatureAndTint = {
      .temperature = [ABI25_0_0EXCameraUtils temperatureForWhiteBalance:self.whiteBalance],
      .tint = 0,
    };
...
- (void)takePicture:(NSDictionary *)options resolve:(ABI25_0_0RCTPromiseResolveBlock)resolve reject:(ABI25_0_0RCTPromiseRejectBlock)reject
{
  ...
  [connection setVideoOrientation:[ABI25_0_0EXCameraUtils videoOrientationForDeviceOrientation:[[UIDevice currentDevice] orientation]]];
  [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
    if (imageSampleBuffer && !error) {
      ...
      takenImage = [ABI25_0_0EXImageUtils cropImage:takenImage toRect:cropRect];
      ...
      response[@"uri"] = [ABI25_0_0EXImageUtils writeImage:takenImageData toPath:path];
      ...
      if ([options[@"exif"] boolValue]) {
        ...
        [ABI25_0_0EXImageUtils updatePhotoMetadata:imageSampleBuffer withAdditionalData:@{ @"Orientation": @(imageRotation) } inResponse:response]; // TODO
      }
...
- (void)record:(NSDictionary *)options resolve:(ABI25_0_0RCTPromiseResolveBlock)resolve reject:(ABI25_0_0RCTPromiseRejectBlock)reject
{
  ...
  if (self.movieFileOutput != nil && !self.movieFileOutput.isRecording && _videoRecordedResolve == nil && _videoRecordedReject == nil) {
    ...
    if (options[@"quality"]) {
      [self updateSessionPreset:[ABI25_0_0EXCameraUtils captureSessionPresetForVideoResolution:(ABI25_0_0EXCameraVideoResolution)[options[@"quality"] integerValue]]];
    }
    
    [self updateSessionAudioIsMuted:!!options[@"mute"]];
    
    AVCaptureConnection *connection = [self.movieFileOutput connectionWithMediaType:AVMediaTypeVideo];
    [connection setVideoOrientation:[ABI25_0_0EXCameraUtils videoOrientationForInterfaceOrientation:[[UIApplication sharedApplication] statusBarOrientation]]];
    
    dispatch_async(self.sessionQueue, ^{
      NSString *path = [ABI25_0_0EXFileSystem generatePathInDirectory:[self.bridge.scopedModules.fileSystem.cachesDirectory stringByAppendingPathComponent:@"Camera"] withExtension:@".mov"];
...
- (void)startSession
{
  ...
  return;
#endif
  NSDictionary *cameraPermissions = [ABI25_0_0EXCameraPermissionRequester permissions];
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI25_0_0EXCamera *weakSelf = self;
    [self setRuntimeErrorHandlingObserver:
     [NSNotificationCenter.defaultCenter addObserverForName:AVCaptureSessionRuntimeErrorNotification object:self.session queue:nil usingBlock:^(NSNotification *note) {
      ABI25_0_0EXCamera *strongSelf = weakSelf;
...
- (void)initializeCaptureSessionInput
{
  ...
  AVCaptureVideoOrientation orientation = [ABI25_0_0EXCameraUtils videoOrientationForInterfaceOrientation:interfaceOrientation];
  dispatch_async(self.sessionQueue, ^{
    [self.session beginConfiguration];
    
    NSError *error = nil;
    AVCaptureDevice *captureDevice = [ABI25_0_0EXCameraUtils deviceWithMediaType:AVMediaTypeVideo preferringPosition:self.presetCamera];
    AVCaptureDeviceInput *captureDeviceInput = [AVCaptureDeviceInput deviceInputWithDevice:captureDevice error:&error];
    
    if (error || captureDeviceInput == nil) {
      ABI25_0_0RCTLog(@"%s: %@", __func__, error);
      return;
    }
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    if (!isMuted) {
      ...
      if (error || audioDeviceInput == nil) {
        ABI25_0_0RCTLogWarn(@"%s: %@", __func__, error);
        return;
      }
...
- (void)changePreviewOrientation:(UIInterfaceOrientation)orientation
{
  ...
  AVCaptureVideoOrientation videoOrientation = [ABI25_0_0EXCameraUtils videoOrientationForInterfaceOrientation:orientation];
...
{
  Class faceDetectorManagerClass = NSClassFromString(@"ABI25_0_0EXFaceDetectorManager");
  Class faceDetectorManagerStubClass = NSClassFromString(@"ABI25_0_0EXFaceDetectorManagerStub");
