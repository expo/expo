...
#import "ABI25_0_0EXVideoPlayerViewController.h"

static NSString *const ABI25_0_0EXVideoReadyForDisplayKeyPath = @"readyForDisplay";
static NSString *const ABI25_0_0EXVideoSourceURIKeyPath = @"uri";

@interface ABI25_0_0EXVideoView ()

@property (nonatomic, weak) ABI25_0_0EXAV *exAV;

@property (nonatomic, assign) BOOL playerHasLoaded;
@property (nonatomic, strong) ABI25_0_0EXAVPlayerData *data;
@property (nonatomic, strong) AVPlayerLayer *playerLayer;
@property (nonatomic, strong) ABI25_0_0EXVideoPlayerViewController *playerViewController;

@property (nonatomic, assign) BOOL fullscreenPlayerIsDismissing;
@property (nonatomic, strong) ABI25_0_0EXVideoPlayerViewController *fullscreenPlayerViewController;
@property (nonatomic, strong) ABI25_0_0RCTPromiseResolveBlock requestedFullscreenChangeResolver;
@property (nonatomic, strong) ABI25_0_0RCTPromiseRejectBlock requestedFullscreenChangeRejecter;
...
@implementation ABI25_0_0EXVideoView

#pragma mark - ABI25_0_0EXVideoView interface methods

- (instancetype)initWithBridge:(ABI25_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _exAV = [bridge moduleForClass:[ABI25_0_0EXAV class]];
...
- (void)_callFullscreenCallbackForUpdate:(ABI25_0_0EXVideoFullscreenUpdate)update
{
...
- (void)_tryUpdateDataStatus:(ABI25_0_0RCTPromiseResolveBlock)resolve
...
- (void)_removePlayer
{
  if (_requestedFullscreenChangeRejecter) {
    ...
    _requestedFullscreenChangeRejecter(@"E_VIDEO_FULLSCREEN", errorMessage, ABI25_0_0RCTErrorWithMessage(errorMessage));
    _requestedFullscreenChangeResolver = nil;
    _requestedFullscreenChangeRejecter = nil;
    _requestedFullscreenChange = NO;
  }
...
- (ABI25_0_0EXVideoPlayerViewController *)_createNewPlayerViewController
{
  if (_data == nil) {
    return nil;
  }
  ABI25_0_0EXVideoPlayerViewController *controller = [[ABI25_0_0EXVideoPlayerViewController alloc] init];
  ...
  [controller addObserver:self forKeyPath:ABI25_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
  return controller;
}
...
- (void)_usePlayerLayer
{
  if (_data) {
    ...
    [_playerLayer addObserver:self forKeyPath:ABI25_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)_removePlayerLayer
{
  if (_playerLayer) {
    ...
    [_playerLayer removeObserver:self forKeyPath:ABI25_0_0EXVideoReadyForDisplayKeyPath];
    _playerLayer = nil;
  }
}
...
- (void)_removeFullscreenPlayerViewController
{
  if (_fullscreenPlayerViewController) {
    [_fullscreenPlayerViewController removeObserver:self forKeyPath:ABI25_0_0EXVideoReadyForDisplayKeyPath];
    _fullscreenPlayerViewController = nil;
  }
}
...
- (void)_removePlayerViewController
{
  if (_playerViewController) {
    ...
    [_playerViewController removeObserver:self forKeyPath:ABI25_0_0EXVideoReadyForDisplayKeyPath];
    _playerViewController = nil;
  }
}
...
{
  if ((object == _playerLayer || object == _playerViewController || object == _fullscreenPlayerViewController) && [keyPath isEqualToString:ABI25_0_0EXVideoReadyForDisplayKeyPath]) {
         ...
         rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject
{
  ...
  __weak ABI25_0_0EXVideoView *weakSelf = self;
  
  void (^statusUpdateCallback)(NSDictionary *) = ^(NSDictionary *status) {
    __strong ABI25_0_0EXVideoView *strongSelf = weakSelf;
  ...
  _data = [[ABI25_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                                    withSource:source
                           ...
                           withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                             ...
                             if (strongSelf && success) {
                               ...
                               [strongSelf _removePlayer];
                               if (reject) {
                                 reject(@"E_VIDEO_NOTCREATED", error, ABI25_0_0RCTErrorWithMessage(error));
                               }
                               [strongSelf _callErrorCallback:error];
                             }
                           }];
             ...
             rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject
{
  if (!_data) {
    ...
    if (reject) {
      ...
      reject(@"E_VIDEO_FULLSCREEN", errorMessage, ABI25_0_0RCTErrorWithMessage(errorMessage));
    }
    return;
    ...
    if (value && !_fullscreenPlayerPresented && !_fullscreenPlayerViewController) {
      ...
      _presentingViewController = ABI25_0_0RCTPresentedViewController();
      [self _callFullscreenCallbackForUpdate:ABI25_0_0EXVideoFullscreenUpdatePlayerWillPresent];
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (strongSelf) {
          ...
          [strongSelf.presentingViewController presentViewController:strongSelf.fullscreenPlayerViewController animated:YES completion:^{
            __strong ABI25_0_0EXVideoView *strongSelfInner = weakSelf;
            if (strongSelfInner) {
              strongSelfInner.fullscreenPlayerPresented = YES;
              [strongSelfInner _callFullscreenCallbackForUpdate:ABI25_0_0EXVideoFullscreenUpdatePlayerDidPresent];
              if (resolve) {
...
{
  return @{
           ABI25_0_0EXVideoSourceURIKeyPath: (_data != nil && _data.url != nil) ? _data.url.absoluteString : @""
           };
}
...
#pragma mark - ReactABI25_0_0 View Management
...
- (void)insertReactABI25_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  ...
  if (_useNativeControls && _playerViewController) {
    ...
    ABI25_0_0RCTLogError(@"video cannot have any subviews");
  }
}
...
#pragma mark - ABI25_0_0EXVideoPlayerViewControllerDelegate
...
- (void)videoPlayerViewControllerWillDismiss:(AVPlayerViewController *)playerViewController
{
  if (_fullscreenPlayerViewController == playerViewController && _fullscreenPlayerPresented) {
    ...
    [self _callFullscreenCallbackForUpdate:ABI25_0_0EXVideoFullscreenUpdatePlayerWillDismiss];
  }
}
...
- (void)videoPlayerViewControllerDidDismiss:(AVPlayerViewController *)playerViewController
{
  if (_fullscreenPlayerViewController == playerViewController && _fullscreenPlayerPresented) {
    ...
    [self _removeFullscreenPlayerViewController];
    [self setUseNativeControls:_useNativeControls];
    [self _callFullscreenCallbackForUpdate:ABI25_0_0EXVideoFullscreenUpdatePlayerDidDismiss];
  }
}
...
{
  return _data == nil ? ABI25_0_0EXAVAudioSessionModeInactive : [_data getAudioSessionModeRequired];
}
