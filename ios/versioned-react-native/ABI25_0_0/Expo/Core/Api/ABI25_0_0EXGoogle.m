...
#import "ABI25_0_0EXGoogle.h"
#import "ABI25_0_0EXOAuthViewController.h"
...
NSString *ABI25_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI25_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI25_0_0EXGoogle
{
  ABI25_0_0RCTPromiseResolveBlock _logInResolve;
  ABI25_0_0RCTPromiseRejectBlock _logInReject;
}

ABI25_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI25_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI25_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI25_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      if (authState.isAuthorized) {
        _logInResolve(@{
          @"type": @"success",
          @"accessToken": ABI25_0_0RCTNullIfNil(authState.lastTokenResponse.accessToken),
          @"idToken": ABI25_0_0RCTNullIfNil(authState.lastTokenResponse.idToken),
          @"refreshToken": ABI25_0_0RCTNullIfNil(authState.lastTokenResponse.refreshToken),
          @"serverAuthCode": ABI25_0_0RCTNullIfNil(authState.lastAuthorizationResponse.authorizationCode),
        });
      } else {
        _logInResolve(@{@"type": @"cancel"});
      }
    } else {
      _logInReject(ABI25_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI25_0_0RCTPresentedViewController()
                                                   callback:callback];
...
-(void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error
{
  if (error != nil) {
    ...
    _logInResolve(@{
      @"type": @"success",
      @"accessToken": ABI25_0_0RCTNullIfNil(user.authentication.accessToken),
      @"serverAuthCode": ABI25_0_0RCTNullIfNil(user.serverAuthCode),
      @"idToken": ABI25_0_0RCTNullIfNil(user.authentication.idToken),
      @"refreshToken": ABI25_0_0RCTNullIfNil(user.authentication.refreshToken),
      @"user": @{
        @"id": ABI25_0_0RCTNullIfNil(user.userID),
        @"name": ABI25_0_0RCTNullIfNil(user.profile.name),
        @"familyName": ABI25_0_0RCTNullIfNil(user.profile.familyName),
        @"givenName": ABI25_0_0RCTNullIfNil(user.profile.givenName),
        @"email": ABI25_0_0RCTNullIfNil(user.profile.email),
...
{
  [ABI25_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI25_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
