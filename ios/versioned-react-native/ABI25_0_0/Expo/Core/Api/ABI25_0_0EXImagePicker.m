#import "ABI25_0_0EXImagePicker.h"
...
@interface ABI25_0_0EXImagePicker ()
...
ABI25_0_0RCT_EXPORT_METHOD(launchCameraAsync:(NSDictionary *)options
                  resolver:(ABI25_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI25_0_0EXPermissions statusForPermissions:[ABI25_0_0EXCameraRollRequester permissions]] != ABI25_0_0EXPermissionStatusGranted ||
      ![_kernelPermissionsServiceDelegate hasGrantedPermission:@"cameraRoll" forExperience:self.experienceId] ||
      [ABI25_0_0EXPermissions statusForPermissions:[ABI25_0_0EXCameraPermissionRequester permissions]] != ABI25_0_0EXPermissionStatusGranted ||
  ...
  self.reject = reject;
  [self launchImagePicker:ABI25_0_0RNImagePickerTargetCamera options:options];
}

ABI25_0_0RCT_EXPORT_METHOD(launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI25_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI25_0_0EXPermissions statusForPermissions:[ABI25_0_0EXCameraRollRequester permissions]] != ABI25_0_0EXPermissionStatusGranted ||
  ...
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI25_0_0RNImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI25_0_0RNImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI25_0_0RNImagePickerTarget)target
{
  self.picker = [[UIImagePickerController alloc] init];

  if (target == ABI25_0_0RNImagePickerTargetCamera) {
#if TARGET_IPHONE_SIMULATOR
    self.reject(ABI25_0_0RCTErrorUnspecified, @"Camera not available on simulator", nil);
    return;
#else
...
#endif
  } else { // ABI25_0_0RNImagePickerTargetLibrarySingleImage
          ...
          completionHandler:(void (^)(void))completionHandler
{
  ...
  if ([[imageURL absoluteString] containsString:@"ext=PNG"]) {
    ...
    ABI25_0_0RCTLogWarn(@"Unsupported format of the picked image. Using JPEG instead.");
  }

  NSString *path = [ABI25_0_0EXFileSystem generatePathInDirectory:directory withExtension:extension];
  ...
  if ([[self.options objectForKey:@"exif"] boolValue]) {
    ...
    if (metadata) {
      ...
      if (assets.count > 0) {
        ...
        ABI25_0_0RCTLogInfo(@"Could not fetch metadata for image %@", [imageURL absoluteString]);
        completionHandler();
      }
    }
  } else {
...
- (void)handleVideoWithInfo:(NSDictionary * _Nonnull)info saveAt:(NSString *)directory updateResponse:(NSMutableDictionary *)response
{
  ...
  if (assets.count > 0) {
    ...
    ABI25_0_0RCTLogInfo(@"Could not fetch metadata for video %@", [videoURL absoluteString]);
  }
  ...
  response[@"type"] = @"video";
  NSError *error = nil;
  NSString *path = [ABI25_0_0EXFileSystem generatePathInDirectory:directory withExtension:@".mov"];
