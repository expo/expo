...
NSString * const ABI25_0_0EXLocationChangedEventName = @"Exponent.locationChanged";
NSString * const ABI25_0_0EXHeadingChangedEventName = @"Exponent.headingChanged";

@interface ABI25_0_0EXLocationDelegate : NSObject <CLLocationManagerDelegate>
...
@implementation ABI25_0_0EXLocationDelegate
...
@interface ABI25_0_0EXLocation ()

@property (nonatomic, strong) NSMutableDictionary<NSNumber *, ABI25_0_0EXLocationDelegate*> *delegates;
...
@implementation ABI25_0_0EXLocation

ABI25_0_0EX_EXPORT_SCOPED_MODULE(ExponentLocation, PermissionsManager)
...
{
  return @[ABI25_0_0EXLocationChangedEventName, ABI25_0_0EXHeadingChangedEventName];
}

ABI25_0_0RCT_REMAP_METHOD(getProviderStatusAsync,
...
ABI25_0_0RCT_REMAP_METHOD(getCurrentPositionAsync,
...
ABI25_0_0RCT_REMAP_METHOD(watchPositionImplAsync,
                 ...
                 rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject)
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    locMgr.distanceFilter = options[@"distanceInterval"] ? [ABI25_0_0RCTConvert double:options[@"distanceInterval"]] ?: kCLDistanceFilterNone : kCLLocationAccuracyHundredMeters;
    locMgr.desiredAccuracy = [ABI25_0_0RCTConvert BOOL:options[@"enableHighAccuracy"]] ? kCLLocationAccuracyBest : kCLLocationAccuracyHundredMeters;
    
    ABI25_0_0EXLocationDelegate *delegate = [[ABI25_0_0EXLocationDelegate alloc] initWithId:watchId withLocMgr:locMgr onUpdateLocations:^(NSArray<CLLocation *> *locations) {
      if (locations.lastObject) {
        ...
        [weakSelf sendEventWithName:ABI25_0_0EXLocationChangedEventName body:body];
      }
...
ABI25_0_0RCT_REMAP_METHOD(watchDeviceHeading,
                 watchId:(nonnull NSNumber *)watchId
                 watchDeviceHeading_resolver:(ABI25_0_0RCTPromiseResolveBlock)resolve
                 watchDeviceHeading_rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject) {
  if (!([CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedAlways || [CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedWhenInUse ||
        [_kernelPermissionsServiceDelegate hasGrantedPermission:@"location" forExperience:self.experienceId])) {
    reject(@"E_LOCATION_UNAUTHORIZED", @"Not authorized to use location services", nil);
    return;
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    ABI25_0_0EXLocationDelegate *delegate = [[ABI25_0_0EXLocationDelegate alloc] initWithId:watchId withLocMgr:locMgr onUpdateLocations: nil onUpdateHeadings:^(CLHeading *newHeading) {
      if (newHeading) {
        NSNumber *accuracy;
        ...
        [weakSelf sendEventWithName:ABI25_0_0EXHeadingChangedEventName body:body];
      }
...
ABI25_0_0RCT_REMAP_METHOD(removeWatchAsync,
                 watchId:(nonnull NSNumber *)watchId
                 resolver:(ABI25_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI25_0_0RCTPromiseRejectBlock)reject)
{
  ABI25_0_0EXLocationDelegate *delegate = _delegates[watchId];
  if (delegate) {
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
...
ABI25_0_0RCT_REMAP_METHOD(geocodeAsync,
...
ABI25_0_0RCT_REMAP_METHOD(reverseGeocodeAsync,
