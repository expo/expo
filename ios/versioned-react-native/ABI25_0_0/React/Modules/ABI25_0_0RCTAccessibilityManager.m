...
NSString *const ABI25_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI25_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI25_0_0RCTAccessibilityManager ()
...
@implementation ABI25_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    self.contentSizeCategory = ABI25_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI25_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI25_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI25_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI25_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI25_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI25_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI25_0_0Tag:ReactABI25_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI25_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI25_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI25_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI25_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI25_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI25_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI25_0_0RCTBridge (ABI25_0_0RCTAccessibilityManager)

- (ABI25_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI25_0_0RCTAccessibilityManager class]];
}
