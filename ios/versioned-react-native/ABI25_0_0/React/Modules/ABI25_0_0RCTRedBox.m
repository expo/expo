...
@class ABI25_0_0RCTRedBoxWindow;

@protocol ABI25_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI25_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI25_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI25_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI25_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI25_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI25_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI25_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI25_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI25_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI25_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI25_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI25_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI25_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI25_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI25_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI25_0_0RCTKeyCommands for this, but since
...
@interface ABI25_0_0RCTRedBox () <ABI25_0_0RCTInvalidating, ABI25_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI25_0_0RCTRedBox
{
  ABI25_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI25_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI25_0_0RCTErrorInfo *)_customizeError:(ABI25_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI25_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI25_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI25_0_0RCTJSStackTraceKey]];
}
...
- (void)showErrorMessage:(NSString *)message withDetails:(NSString *)details stack:(NSArray<ABI25_0_0RCTJSStackFrame *> *)stack {
...
{
  NSArray<ABI25_0_0RCTJSStackFrame *> *stack = [ABI25_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
  [self showErrorMessage:message withParsedStack:stack isUpdate:NO];
}

- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
  [self showErrorMessage:message withParsedStack:[ABI25_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:NO];
}

- (void)updateErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
  [self showErrorMessage:message withParsedStack:[ABI25_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:YES];
}
...
- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI25_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI25_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI25_0_0RCTErrorInfo *errorInfo = [[ABI25_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI25_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI25_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI25_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI25_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI25_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI25_0_0RCTRedBoxWindow *)redBoxWindow
{
...
@implementation ABI25_0_0RCTBridge (ABI25_0_0RCTRedBox)

- (ABI25_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI25_0_0RCTRedBox class]];
}
...
- (ABI25_0_0RCTRedBox *)redBox { return nil; }
