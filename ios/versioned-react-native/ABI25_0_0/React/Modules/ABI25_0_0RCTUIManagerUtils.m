...
char *const ABI25_0_0RCTUIManagerQueueName = "com.facebook.ReactABI25_0_0.ShadowQueue";

static BOOL pseudoUIManagerQueueFlag = NO;

dispatch_queue_t ABI25_0_0RCTGetUIManagerQueue(void)
{
  ...
  dispatch_once(&onceToken, ^{
    if ([NSOperation instancesRespondToSelector:@selector(qualityOfService)]) {
      ...
      shadowQueue = dispatch_queue_create(ABI25_0_0RCTUIManagerQueueName, attr);
    } else {
      shadowQueue = dispatch_queue_create(ABI25_0_0RCTUIManagerQueueName, DISPATCH_QUEUE_SERIAL);
...
BOOL ABI25_0_0RCTIsUIManagerQueue()
{
  static void *queueKey = &queueKey;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    dispatch_queue_set_specific(ABI25_0_0RCTGetUIManagerQueue(), queueKey, queueKey, NULL);
  });
  return dispatch_get_specific(queueKey) == queueKey;
}

BOOL ABI25_0_0RCTIsPseudoUIManagerQueue()
{
...
void ABI25_0_0RCTExecuteOnUIManagerQueue(dispatch_block_t block)
{
  if (ABI25_0_0RCTIsUIManagerQueue() || ABI25_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
...
void ABI25_0_0RCTUnsafeExecuteOnUIManagerQueueSync(dispatch_block_t block)
{
  if (ABI25_0_0RCTIsUIManagerQueue() || ABI25_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    if (ABI25_0_0RCTIsMainQueue()) {
      ...
      dispatch_sync(ABI25_0_0RCTGetUIManagerQueue(), ^{
        block();
      });
    }
  }
...
NSNumber *ABI25_0_0RCTAllocateRootViewTag()
{
