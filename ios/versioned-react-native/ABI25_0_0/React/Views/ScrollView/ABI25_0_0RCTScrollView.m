...
@interface ABI25_0_0RCTScrollEvent : NSObject <ABI25_0_0RCTEvent>
...
@implementation ABI25_0_0RCTScrollEvent
{
...
{
  ABI25_0_0RCTAssertParam(ReactABI25_0_0Tag);
...
- (ABI25_0_0RCTScrollEvent *)coalesceWithEvent:(ABI25_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI25_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI25_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI25_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI25_0_0RCTRefreshControl *rctRefreshControl;
...
@implementation ABI25_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI25_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI25_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI25_0_0RCTUIManager JSResponder] isKindOfClass:[ABI25_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI25_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI25_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI25_0_0RCTScrollView
{
  ABI25_0_0RCTEventDispatcher *_eventDispatcher;
  ABI25_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI25_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI25_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI25_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI25_0_0RCTApplyTranformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI25_0_0LayoutDirection:layoutDirection];

  ABI25_0_0RCTApplyTranformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI25_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertReactABI25_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertReactABI25_0_0Subview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI25_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI25_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI25_0_0RCTAssert(_contentView == nil, @"ABI25_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI25_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, self.ReactABI25_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI25_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI25_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI25_0_0Subview:atIndex:`
}
...
- (void)layoutSubviews
{
  ...
  ABI25_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI25_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  ABI25_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI25_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI25_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI25_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI25_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI25_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI25_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI25_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI25_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI25_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI25_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI25_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI25_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI25_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI25_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI25_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI25_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI25_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI25_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI25_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI25_0_0RCTView contentInsetsForView:self];
...
#define ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI25_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI25_0_0RCTScrollEvent *scrollEvent = [[ABI25_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI25_0_0RCTEventDispatcher (ABI25_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI25_0_0Tag
{
  ...
  ABI25_0_0RCTScrollEvent *fakeScrollEvent = [[ABI25_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI25_0_0Tag:ReactABI25_0_0Tag
