...
static NSRegularExpression *ABI25_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI25_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI25_0_0RCTNullIfNil(self.methodName),
     @"file": ABI25_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI25_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
{
  return [[self alloc] initWithMethodName:ABI25_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI25_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI25_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}

+ (NSArray<ABI25_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI25_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI25_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI25_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
