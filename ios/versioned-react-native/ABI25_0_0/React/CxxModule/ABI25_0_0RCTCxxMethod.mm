...
#import <cxxReactABI25_0_0/ABI25_0_0JsArgumentHelpers.h>
...
@implementation ABI25_0_0RCTCxxMethod
{
...
- (ABI25_0_0RCTFunctionType)functionType
{
  std::string type(_method->getType());
  if (type == "sync") {
    return ABI25_0_0RCTFunctionTypeSync;
  } else if (type == "async") {
    return ABI25_0_0RCTFunctionTypeNormal;
  } else {
    return ABI25_0_0RCTFunctionTypePromise;
  }
}
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI25_0_0RCTLogError(@"Method %@.%s expects at least %zu arguments, but got %tu",
                ABI25_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI25_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
  ...
  try {
    ...
    ABI25_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI25_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
