...
#include <cxxReactABI25_0_0/ABI25_0_0MethodCall.h>
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI25_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI25_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI25_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
