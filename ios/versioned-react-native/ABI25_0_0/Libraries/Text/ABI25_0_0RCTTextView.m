...
@interface ABI25_0_0RCTTextView () <ABI25_0_0RCTBackedTextInputDelegate>

@end

@implementation ABI25_0_0RCTTextView
{
  ABI25_0_0RCTUITextView *_backedTextInput;
  ABI25_0_0RCTText *_richTextView;
...
- (instancetype)initWithBridge:(ABI25_0_0RCTBridge *)bridge
{
  ...
  if (self = [super initWithBridge:bridge]) {
    ...
    _backedTextInput = [[ABI25_0_0RCTUITextView alloc] initWithFrame:self.bounds];
...
#pragma mark - ABI25_0_0RCTComponent

- (void)insertReactABI25_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI25_0_0Subview:subview atIndex:index];

  if ([subview isKindOfClass:[ABI25_0_0RCTText class]]) {
    if (_richTextView) {
      ABI25_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI25_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI25_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI25_0_0RCTReactABI25_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI25_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI25_0_0TagFromString(_pendingAttributedText);
...
- (void)setText:(NSString *)text
{
  ...
  if (eventLag == 0 && ![text isEqualToString:_backedTextInput.text]) {
    ...
    ABI25_0_0RCTLogWarn(@"Native TextInput(%@) is %lld events ahead of JS - try to make your JS faster.", self.text, (long long)eventLag);
  }
}
...
- (BOOL)textInputShouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (!_backedTextInput.textWasPasted) {
    [_eventDispatcher sendTextEventWithType:ABI25_0_0RCTTextEventTypeKeyPress
...
- (void)textInputDidChange
{
  ...
  if (!self.ReactABI25_0_0Tag || !_onChange) {
    return;
  }
