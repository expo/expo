...
#import "ABI25_0_0RCTTestRunner.h"
...
#import "ABI25_0_0FBSnapshotTestController.h"
#import "ABI25_0_0RCTTestModule.h"

static const NSTimeInterval kTestTimeoutSeconds = 120;

@implementation ABI25_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI25_0_0RCTBridgeModuleListProvider _moduleProvider;
  NSString *_appPath;
}
...
{
  ABI25_0_0RCTAssertParam(app);
  ABI25_0_0RCTAssertParam(referenceDirectory);
...
- (void)updateScript
{
  if (getenv("CI_USE_PACKAGER") || _useBundler) {
    ...
    _scriptURL = [[NSBundle bundleForClass:[ABI25_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
  }
  ABI25_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
}
...
configurationBlock:(void(^)(ABI25_0_0RCTRootView *rootView))configurationBlock
{
...
{
  __weak ABI25_0_0RCTBridge *batchedBridge;

  @autoreleasepool {
    __block NSMutableArray<NSString *> *errors = nil;
    ABI25_0_0RCTLogFunction defaultLogFunction = ABI25_0_0RCTGetLogFunction();
    ABI25_0_0RCTSetLogFunction(^(ABI25_0_0RCTLogLevel level, ABI25_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      defaultLogFunction(level, source, fileName, lineNumber, message);
      if (level >= ABI25_0_0RCTLogLevelError) {
        if (errors == nil) {
    ...
    ABI25_0_0RCTBridge *bridge = [[ABI25_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];
    [bridge.devSettings setIsDebuggingRemotely:_useJSDebugger];
    batchedBridge = [bridge batchedBridge];

    ABI25_0_0RCTRootView *rootView = [[ABI25_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI25_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI25_0_0RCTTestModule class]];
    ABI25_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    testModule.controller = _testController;
    testModule.testSelector = test;
    testModule.testSuffix = _testSuffix;
    testModule.view = rootView;

    UIViewController *vc = ABI25_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI25_0_0RCTTestStatusPending && errors == nil) {
    ...
    ABI25_0_0RCTSetLogFunction(defaultLogFunction);
    ...
    ABI25_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);
#endif

    if (expectErrorBlock) {
      ABI25_0_0RCTAssert(expectErrorBlock(errors[0]), @"Expected an error but the first one was missing or did not match.");
    } else {
      ABI25_0_0RCTAssert(errors == nil, @"RedBox errors: %@", errors);
      ABI25_0_0RCTAssert(testModule.status != ABI25_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI25_0_0RCTAssert(testModule.status == ABI25_0_0RCTTestStatusPassed, @"Test failed");
    }
  ...
  ABI25_0_0RCTAssert(batchedBridge == nil, @"Bridge should be deallocated after the test");
}
