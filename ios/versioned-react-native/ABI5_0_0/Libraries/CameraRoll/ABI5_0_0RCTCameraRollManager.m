...
#import "ABI5_0_0RCTCameraRollManager.h"
...
@implementation ABI5_0_0RCTConvert (ALAssetGroup)

ABI5_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI5_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI5_0_0RCTCameraRollManager

ABI5_0_0RCT_EXPORT_MODULE()

@synthesize bridge = _bridge;

NSString *const ABI5_0_0RCTErrorUnableToLoad = @"E_UNABLE_TO_LOAD";
NSString *const ABI5_0_0RCTErrorUnableToSave = @"E_UNABLE_TO_SAVE";

ABI5_0_0RCT_EXPORT_METHOD(saveImageWithTag:(NSString *)imageTag
                  ...
                  reject:(ABI5_0_0RCTPromiseRejectBlock)reject)
{
  [_bridge.imageLoader loadImageWithTag:imageTag callback:^(NSError *loadError, UIImage *loadedImage) {
    if (loadError) {
      reject(ABI5_0_0RCTErrorUnableToLoad, nil, loadError);
      return;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
        if (saveError) {
          ABI5_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
          reject(ABI5_0_0RCTErrorUnableToSave, nil, saveError);
        } else {
...
static void ABI5_0_0RCTResolvePromise(ABI5_0_0RCTPromiseResolveBlock resolve,
...
ABI5_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI5_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI5_0_0RCTPromiseRejectBlock)reject)
{
  NSUInteger first = [ABI5_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI5_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI5_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI5_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI5_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI5_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI5_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI5_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
    reject(ABI5_0_0RCTErrorUnableToLoad, nil, error);
  }];
}
