...
#import "ABI5_0_0RCTWebSocketExecutor.h"
...
typedef void (^ABI5_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI5_0_0RCTWebSocketExecutor () <ABI5_0_0RCTSRWebSocketDelegate>

@end

@implementation ABI5_0_0RCTWebSocketExecutor
{
  ABI5_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI5_0_0RCTWSMessageCallback> *_callbacks;
...
- (void)setUp
{
  if (!_url) {
    ...
    _url = [ABI5_0_0RCTConvert NSURL:URLString];
  }

  _jsQueue = dispatch_queue_create("com.facebook.ReactABI5_0_0.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  _socket = [[ABI5_0_0RCTSRWebSocket alloc] initWithURL:_url];
  ...
  if (![self connectToProxy]) {
    ABI5_0_0RCTLogError(@"Connection to %@ timed out. Are you running node proxy? If "
                 "you are running on the device, check if you have the right IP "
                 "address in `ABI5_0_0RCTWebSocketExecutor.m`.", _url);
    [self invalidate];
    return;
  }
  ...
  if (!runtimeIsReady) {
    ABI5_0_0RCTLogError(@"Runtime is not ready for debugging.\n "
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI5_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI5_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
- (void)webSocket:(ABI5_0_0RCTSRWebSocket *)webSocket didFailWithError:(NSError *)error
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    ABI5_0_0RCTLogError(@"WebSocket connection failed with error %@", error);
  });
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message waitForReply:(ABI5_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    ...
    [_socket send:ABI5_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}

- (void)executeApplicationScript:(NSData *)script sourceURL:(NSURL *)URL onComplete:(ABI5_0_0RCTJavaScriptCompleteBlock)onComplete
{
  NSDictionary<NSString *, id> *message = @{
    @"method": @"executeApplicationScript",
    @"url": ABI5_0_0RCTNullIfNil(URL.absoluteString),
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI5_0_0RCTJavaScriptCallback)onComplete
{
  ABI5_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  NSDictionary<NSString *, id> *message = @{
    @"method": method,
    @"arguments": arguments
  };
  [self sendMessage:message waitForReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    ...
    id objcValue = ABI5_0_0RCTJSONParse(result, NULL);
    onComplete(objcValue, nil);
  }];
}

- (void)injectJSONText:(NSString *)script asGlobalObjectNamed:(NSString *)objectName callback:(ABI5_0_0RCTJavaScriptCompleteBlock)onComplete
{
  dispatch_async(_jsQueue, ^{
...
{
  ABI5_0_0RCTExecuteOnMainThread(block, NO);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI5_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI5_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
