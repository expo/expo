...
#import "ABI5_0_0RCTTestRunner.h"
...
@implementation ABI5_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI5_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI5_0_0RCTAssertParam(app);
  ABI5_0_0RCTAssertParam(referenceDirectory);
...
#if RUNNING_ON_CI
    _scriptURL = [[NSBundle bundleForClass:[ABI5_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    ABI5_0_0RCTAssert(_scriptURL != nil, @"Could not locate main.jsBundle");
#else
...
configurationBlock:(void(^)(ABI5_0_0RCTRootView *rootView))configurationBlock
{
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI5_0_0RCTSetLogFunction(^(ABI5_0_0RCTLogLevel level, ABI5_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI5_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI5_0_0RCTBridge *bridge = [[ABI5_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI5_0_0RCTRootView *rootView = [[ABI5_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
    rootView.frame = CGRectMake(0, 0, 320, 2000); // Constant size for testing on multiple devices

    ABI5_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI5_0_0RCTTestModule class]];
    ABI5_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI5_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI5_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI5_0_0RCTSetLogFunction(ABI5_0_0RCTDefaultLogFunction);
    ...
    ABI5_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI5_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI5_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI5_0_0RCTAssert(testModule.status != ABI5_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI5_0_0RCTAssert(testModule.status == ABI5_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI5_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
