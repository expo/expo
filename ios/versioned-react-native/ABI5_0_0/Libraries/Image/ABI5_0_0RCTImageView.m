 ...
 * at `idealSize`.
 */
static BOOL ABI5_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
@interface ABI5_0_0RCTImageView ()

@property (nonatomic, copy) ABI5_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI5_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI5_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI5_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI5_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI5_0_0RCTImageView
{
   ...
   * if any.
   */
  ABI5_0_0RCTImageLoaderCancellationBlock _reloadImageCancellationBlock;
}
...
- (void)setSource:(ABI5_0_0RCTImageSource *)source
{
...
{
  ABI5_0_0RCTImageLoaderCancellationBlock previousCancellationBlock = _reloadImageCancellationBlock;
...
- (void)reloadImage
{
  ...
  if (_source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI5_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (_onProgress) {
    ...
    CGFloat imageScale = ABI5_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    _reloadImageCancellationBlock = [_bridge.imageLoader loadImageWithoutClipping:_source.imageURL.absoluteString
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (image.ReactABI5_0_0KeyframeAnimation) {
          [strongSelf.layer addAnimation:image.ReactABI5_0_0KeyframeAnimation forKey:@"contents"];
        } else {
...
- (void)ReactABI5_0_0SetFrame:(CGRect)frame
{
  ...
  if (!self.image || self.image == _defaultImage) {
    ...
    CGSize idealSize = ABI5_0_0RCTTargetSize(imageSize, self.image.scale, frame.size,
                                     ABI5_0_0RCTScreenScale(), (ABI5_0_0RCTResizeMode)self.contentMode, YES);

    if (ABI5_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize)) {
      if (ABI5_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
        ABI5_0_0RCTLogInfo(@"[PERF IMAGEVIEW] Reloading image %@ as size %@", _source.imageURL, NSStringFromCGSize(idealSize));
