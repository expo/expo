...
@implementation UIView (ABI5_0_0RCTViewUnmounting)
...
- (void)ReactABI5_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI5_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI5_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI5_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
static NSString *ABI5_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI5_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI5_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI5_0_0RCTBorderStyleSolid;
...
ABI5_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI5_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI5_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI5_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI5_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI5_0_0RCTPointerEventsNone || _pointerEvents == ABI5_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI5_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI5_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI5_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI5_0_0Tag: %@;", self.ReactABI5_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI5_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI5_0_0_remountAllSubviews
{
  if (_ReactABI5_0_0Subviews) {
    NSUInteger index = 0;
    for (UIView *view in _ReactABI5_0_0Subviews) {
      if (view.superview != self) {
        ...
        [view ReactABI5_0_0_remountAllSubviews];
      }
      index++;
    }
  } else {
    // If ReactABI5_0_0_subviews is nil, we must already be showing all subviews
    [super ReactABI5_0_0_remountAllSubviews];
  }
}
...
- (void)remountSubview:(UIView *)view
{
  ...
  NSInteger index = 0;
  for (UIView *subview in _ReactABI5_0_0Subviews) {
    if (subview == view) {
      [self insertSubview:view atIndex:index];
      break;
    }
  ...
  if (view.clipsToBounds) {
    ...
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI5_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)ReactABI5_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (_ReactABI5_0_0Subviews == nil) {
    // Use default behavior if unmounting is disabled
    return [super ReactABI5_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (_ReactABI5_0_0Subviews.count == 0) {
...
{
  if (removeClippedSubviews && !_ReactABI5_0_0Subviews) {
    _ReactABI5_0_0Subviews = [self.subviews mutableCopy];
  } else if (!removeClippedSubviews && _ReactABI5_0_0Subviews) {
    [self ReactABI5_0_0_remountAllSubviews];
    _ReactABI5_0_0Subviews = nil;
  }
}

- (BOOL)removeClippedSubviews
{
  return _ReactABI5_0_0Subviews != nil;
}

- (void)insertReactABI5_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  if (_ReactABI5_0_0Subviews == nil) {
    [self insertSubview:view atIndex:atIndex];
  } else {
    [_ReactABI5_0_0Subviews insertObject:view atIndex:atIndex];
...
{
  // The _ReactABI5_0_0Subviews array is only used when we have hidden
  // offscreen views. If _ReactABI5_0_0Subviews is nil, we can assume
  // that [self ReactABI5_0_0Subviews] and [self subviews] are the same

  return _ReactABI5_0_0Subviews ?: self.subviews;
}
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI5_0_0_findClipView];
  if (clipView) {
    [self ReactABI5_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI5_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI5_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI5_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI5_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI5_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI5_0_0RCTCornerRadii){
...
{
  return (ABI5_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI5_0_0RCTUpdateShadowPathForView(self);

  const ABI5_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI5_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI5_0_0RCTRunningInTestEnvironment() &&
  ABI5_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI5_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI5_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI5_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI5_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI5_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI5_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI5_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI5_0_0RCTUpdateShadowPathForView(ABI5_0_0RCTView *view)
{
  if (ABI5_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI5_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI5_0_0RCTCornerInsets cornerInsets = ABI5_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI5_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI5_0_0RCTLogWarn(@"View #%@ of type %@ has a shadow set but cannot calculate "
                 "shadow efficiently. Consider setting a background color to "
                 "fix this, or apply the shadow to a more specific component.",
                 view.ReactABI5_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI5_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI5_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI5_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI5_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI5_0_0RCTBorderStyle)style \
  {                                                    \
