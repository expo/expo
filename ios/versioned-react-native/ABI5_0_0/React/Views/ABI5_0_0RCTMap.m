...
#import "ABI5_0_0RCTMap.h"

#import "ABI5_0_0RCTEventDispatcher.h"
#import "ABI5_0_0RCTLog.h"
#import "ABI5_0_0RCTMapAnnotation.h"
#import "ABI5_0_0RCTMapOverlay.h"
#import "ABI5_0_0RCTUtils.h"

const CLLocationDegrees ABI5_0_0RCTMapDefaultSpan = 0.005;
const NSTimeInterval ABI5_0_0RCTMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI5_0_0RCTMapZoomBoundBuffer = 0.01;

@implementation ABI5_0_0RCTMap
{
  UIView *_legalLabel;
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    _ReactABI5_0_0Subviews = [NSMutableArray new];
...
- (void)setAnnotations:(NSArray<ABI5_0_0RCTMapAnnotation *> *)annotations
{
  NSMutableArray<NSString *> *newAnnotationIDs = [NSMutableArray new];
  NSMutableArray<ABI5_0_0RCTMapAnnotation *> *annotationsToDelete = [NSMutableArray new];
  NSMutableArray<ABI5_0_0RCTMapAnnotation *> *annotationsToAdd = [NSMutableArray new];

  for (ABI5_0_0RCTMapAnnotation *annotation in annotations) {
    if (![annotation isKindOfClass:[ABI5_0_0RCTMapAnnotation class]]) {
      continue;
    }
  ...
  for (ABI5_0_0RCTMapAnnotation *annotation in self.annotations) {
    if (![annotation isKindOfClass:[ABI5_0_0RCTMapAnnotation class]]) {
      continue;
    }
...
// TODO: this doesn't preserve order. Should it? If so we should change the
// algorithm. If not, it would be more efficient to use an NSSet
- (void)setOverlays:(NSArray<ABI5_0_0RCTMapOverlay *> *)overlays
{
  ...
  for (ABI5_0_0RCTMapOverlay *overlay in overlays) {
    if (![overlay isKindOfClass:[ABI5_0_0RCTMapOverlay class]]) {
      continue;
    }
  ...
  for (ABI5_0_0RCTMapOverlay *overlay in self.overlays) {
    if (![overlay isKindOfClass:[ABI5_0_0RCTMapOverlay class]]) {
      continue;
    }
