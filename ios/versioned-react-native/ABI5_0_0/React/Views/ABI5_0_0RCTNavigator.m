...
typedef NS_ENUM(NSUInteger, ABI5_0_0RCTNavigationLock) {
  ABI5_0_0RCTNavigationLockNone,
  ABI5_0_0RCTNavigationLockNative,
  ABI5_0_0RCTNavigationLockJavaScript
};
...
@interface ABI5_0_0RCTNavigationController : UINavigationController <UINavigationBarDelegate>
{
  dispatch_block_t _scrollCallback;
}

@property (nonatomic, assign) ABI5_0_0RCTNavigationLock navigationLock;

@end

/**
 * In general, `ABI5_0_0RCTNavigator` examines `_currentViews` (which are ReactABI5_0_0 child
 ...
 * ahead by adding/removing ReactABI5_0_0 subviews. Native gets ahead by swiping back,
 * or tapping the back button. In both cases, the other system is initially
 * unaware. And in both cases, `ABI5_0_0RCTNavigator` helps the other side "catch up".
 *
 * If `ABI5_0_0RCTNavigator` sees the number of ReactABI5_0_0 children have changed, it
 * pushes/pops accordingly. If `ABI5_0_0RCTNavigator` sees a `UIKit` driven push/pop, it
 ...
 *
 */
@implementation ABI5_0_0RCTNavigationController

/**
 * @param callback Callback that is invoked when a "scroll" interaction begins
 * so that `ABI5_0_0RCTNavigator` can notify `JavaScript`.
 */
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
  if (self.interactivePopGestureRecognizer.state == UIGestureRecognizerStateBegan) {
    if (self.navigationLock == ABI5_0_0RCTNavigationLockNone) {
      self.navigationLock = ABI5_0_0RCTNavigationLockNative;
      if (_scrollCallback) {
        _scrollCallback();
      }
    } else if (self.navigationLock == ABI5_0_0RCTNavigationLockJavaScript) {
      // This should never happen because we disable/enable the gesture
      // recognizer when we lock the navigation.
      ABI5_0_0RCTAssert(NO, @"Should never receive gesture start while JS locks navigator");
    }
  } else {
    if (self.navigationLock == ABI5_0_0RCTNavigationLockNone) {
      ...
      // length (`currentReactABI5_0_0Count` - 1).
...
@interface ABI5_0_0RCTNavigator() <ABI5_0_0RCTWrapperViewControllerNavigationListener, UINavigationControllerDelegate>

@property (nonatomic, copy) ABI5_0_0RCTDirectEventBlock onNavigationProgress;
@property (nonatomic, copy) ABI5_0_0RCTBubblingEventBlock onNavigationComplete;
...
@property (nonatomic, copy, readwrite) NSArray<ABI5_0_0RCTNavItem *> *previousViews;
@property (nonatomic, readwrite, strong) NSMutableArray<ABI5_0_0RCTNavItem *> *currentViews;
@property (nonatomic, readwrite, strong) ABI5_0_0RCTNavigationController *navigationController;
/**
 ...
 * - We use `invalidate` to remove the `ABI5_0_0RCTNavigator`'s reference to the
 ...
 * remove it from the run loop when the view will be destroyed by ReactABI5_0_0.
 *
 * +----------+              +--------------+
...
@implementation ABI5_0_0RCTNavigator
{
...
- (instancetype)initWithBridge:(ABI5_0_0RCTBridge *)bridge
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    __weak ABI5_0_0RCTNavigator *weakSelf = self;
    _navigationController = [[ABI5_0_0RCTNavigationController alloc] initWithScrollCallback:^{
      [weakSelf dispatchFakeScrollEvent];
    }];
    _navigationController.delegate = self;
    ABI5_0_0RCTAssert([self requestSchedulingJavaScriptNavigation], @"Could not acquire JS navigation lock on init");
                    ...
                    animated:(__unused BOOL)animated
{
  ...
  [tc animateAlongsideTransition: ^(id<UIViewControllerTransitionCoordinatorContext> context) {
    ABI5_0_0RCTWrapperViewController *fromController =
      (ABI5_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey];
    ABI5_0_0RCTWrapperViewController *toController =
      (ABI5_0_0RCTWrapperViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey];
...
{
  if (_navigationController.navigationLock == ABI5_0_0RCTNavigationLockNone) {
    _navigationController.navigationLock = ABI5_0_0RCTNavigationLockJavaScript;
...
- (void)freeLock
{
  _navigationController.navigationLock = ABI5_0_0RCTNavigationLockNone;
  _navigationController.interactivePopGestureRecognizer.enabled = YES;
}

/**
 * A ReactABI5_0_0 subview can be inserted/removed at any time, however if the
 * `requestedTopOfStack` changes, there had better be enough subviews present
 * to satisfy the push/pop.
 */
- (void)insertReactABI5_0_0Subview:(ABI5_0_0RCTNavItem *)view atIndex:(NSInteger)atIndex
{
  ABI5_0_0RCTAssert([view isKindOfClass:[ABI5_0_0RCTNavItem class]], @"ABI5_0_0RCTNavigator only accepts ABI5_0_0RCTNavItem subviews");
  ABI5_0_0RCTAssert(
    _navigationController.navigationLock == ABI5_0_0RCTNavigationLockJavaScript,
...
- (NSArray<ABI5_0_0RCTNavItem *> *)ReactABI5_0_0Subviews
{
...
- (void)layoutSubviews
{
  ...
  [self ReactABI5_0_0AddControllerToClosestParent:_navigationController];
  _navigationController.view.frame = self.bounds;
}

- (void)removeReactABI5_0_0Subview:(ABI5_0_0RCTNavItem *)subview
{
  if (_currentViews.count <= 0 || subview == _currentViews[0]) {
    ABI5_0_0RCTLogError(@"Attempting to remove invalid ABI5_0_0RCT subview of ABI5_0_0RCTNavigator");
    return;
  }
...
{
  [_bridge.eventDispatcher sendFakeScrollEvent:self.ReactABI5_0_0Tag];
}
...
{
  ABI5_0_0RCTAssert(!_bridge.isValid || self.superview != nil, @"put ReactABI5_0_0NavSuperviewLink back");
  return self.superview ? self.superview : self.ReactABI5_0_0NavSuperviewLink;
}
...
- (void)ReactABI5_0_0BridgeDidFinishTransaction
{
  ...
  // The "ReactABI5_0_0 count" is the count of views that are visible on the navigation
  // stack.  There may be more beyond this - that aren't visible, and may be
  // deleted/purged soon.
  NSUInteger previousReactABI5_0_0Count =
    _previousRequestedTopOfStack == kNeverRequested ? 0 : _previousRequestedTopOfStack + 1;
  NSUInteger currentReactABI5_0_0Count = _requestedTopOfStack + 1;

  BOOL jsGettingAhead =
    //    ----- previously caught up ------          ------ no longer caught up -------
    viewControllerCount == previousReactABI5_0_0Count && currentReactABI5_0_0Count != viewControllerCount;
  BOOL jsCatchingUp =
    //    --- previously not caught up ----          --------- now caught up ----------
    viewControllerCount != previousReactABI5_0_0Count && currentReactABI5_0_0Count == viewControllerCount;
  BOOL jsMakingNoProgressButNeedsToCatchUp =
    //    --- previously not caught up ----          ------- still the same -----------
    viewControllerCount != previousReactABI5_0_0Count && currentReactABI5_0_0Count == previousReactABI5_0_0Count;
...
BOOL jsGettingtooSlow =
  ...
  BOOL ReactABI5_0_0PushOne = jsGettingAhead && currentReactABI5_0_0Count == previousReactABI5_0_0Count + 1;
  BOOL ReactABI5_0_0PopN = jsGettingAhead && currentReactABI5_0_0Count < previousReactABI5_0_0Count;
  ...
  if (!(jsGettingAhead ||
        ...
        jsGettingtooSlow)) {
    ABI5_0_0RCTLogError(@"JS has only made partial progress to catch up to UIKit");
  }
  if (currentReactABI5_0_0Count > _currentViews.count) {
    ABI5_0_0RCTLogError(@"Cannot adjust current top of stack beyond available views");
  }

  // Views before the previous ReactABI5_0_0 count must not have changed. Views greater than previousReactABI5_0_0Count
  // up to currentReactABI5_0_0Count may have changed.
  for (NSUInteger i = 0; i < MIN(_currentViews.count, MIN(_previousViews.count, previousReactABI5_0_0Count)); i++) {
    if (_currentViews[i] != _previousViews[i]) {
      ABI5_0_0RCTLogError(@"current view should equal previous view");
    }
  }
  if (currentReactABI5_0_0Count < 1) {
    ABI5_0_0RCTLogError(@"should be at least one current view");
  }
  if (jsGettingAhead) {
    if (ReactABI5_0_0PushOne) {
      UIView *lastView = _currentViews.lastObject;
      ABI5_0_0RCTWrapperViewController *vc = [[ABI5_0_0RCTWrapperViewController alloc] initWithNavItem:(ABI5_0_0RCTNavItem *)lastView];
      vc.navigationListener = self;
      _numberOfViewControllerMovesToIgnore = 1;
      [_navigationController pushViewController:vc animated:(currentReactABI5_0_0Count > 1)];
    } else if (ReactABI5_0_0PopN) {
      UIViewController *viewControllerToPopTo = _navigationController.viewControllers[(currentReactABI5_0_0Count - 1)];
      _numberOfViewControllerMovesToIgnore = viewControllerCount - currentReactABI5_0_0Count;
      [_navigationController popToViewController:viewControllerToPopTo animated:YES];
    } else {
      ABI5_0_0RCTLogError(@"Pushing or popping more than one view at a time from JS");
    }
...
didMoveToNavigationController:(UINavigationController *)navigationController
{
  ...
  ABI5_0_0RCTAssert(_navigationController.navigationLock == ABI5_0_0RCTNavigationLockJavaScript ||
