...
typedef void (^ABI5_0_0RCTPropBlock)(id<ABI5_0_0RCTComponent> view, id json);

@interface ABI5_0_0RCTComponentProp : NSObject

@property (nonatomic, copy, readonly) NSString *type;
@property (nonatomic, copy) ABI5_0_0RCTPropBlock propBlock;

@end

@implementation ABI5_0_0RCTComponentProp
...
@implementation ABI5_0_0RCTComponentData
{
  id<ABI5_0_0RCTComponent> _defaultView; // Only needed for ABI5_0_0RCT_CUSTOM_VIEW_PROPERTY
  NSMutableDictionary<NSString *, ABI5_0_0RCTPropBlock> *_viewPropBlocks;
  NSMutableDictionary<NSString *, ABI5_0_0RCTPropBlock> *_shadowPropBlocks;
                              ...
                              bridge:(ABI5_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    _name = ABI5_0_0RCTBridgeModuleNameForClass(_managerClass);
    ABI5_0_0RCTAssert(_name.length, @"Invalid moduleName '%@'", _name);
    ...
    while (cls != [ABI5_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI5_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI5_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
- (ABI5_0_0RCTShadowView *)createShadowViewWithTag:(NSNumber *)tag
{
  ABI5_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI5_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (ABI5_0_0RCTPropBlock)propBlockForKey:(NSString *)name
                   inDictionary:(NSMutableDictionary<NSString *, ABI5_0_0RCTPropBlock> *)propBlocks
{
  BOOL shadowView = (propBlocks == _shadowPropBlocks);
  ABI5_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {

    __weak ABI5_0_0RCTComponentData *weakSelf = self;
    ...
    if ([_managerClass respondsToSelector:selector]) {
      ...
      type = ABI5_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
    ...
    if ([keyPath isEqualToString:@"__custom__"]) {
      ...
      propBlock = ^(id<ABI5_0_0RCTComponent> view, id json) {
        ABI5_0_0RCTComponentData *strongSelf = weakSelf;
        if (!strongSelf) {
          return;
        }
        json = ABI5_0_0RCTNilIfNull(json);
      ...
      if (type == NSSelectorFromString(@"ABI5_0_0RCTBubblingEventBlock:") ||
          type == NSSelectorFromString(@"ABI5_0_0RCTDirectEventBlock:")) {

        // Special case for event handlers
        __weak ABI5_0_0RCTViewManager *weakManager = self.manager;
        setterBlock = ^(id target, id json) {
          __weak id<ABI5_0_0RCTComponent> weakTarget = target;
          ((void (*)(id, SEL, id))objc_msgSend)(target, setter, [ABI5_0_0RCTConvert BOOL:json] ? ^(NSDictionary *body) {
            body = [NSMutableDictionary dictionaryWithDictionary:body];
            ((NSMutableDictionary *)body)[@"target"] = weakTarget.ReactABI5_0_0Tag;
            [weakManager.bridge.eventDispatcher sendInputEventWithName:ABI5_0_0RCTNormalizeInputEventName(name) body:body];
          } : nil);
        };
        ...
        NSMethodSignature *typeSignature = [[ABI5_0_0RCTConvert class] methodSignatureForSelector:type];
        if (!typeSignature) {
          ABI5_0_0RCTLogError(@"No +[ABI5_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
          return ^(__unused id<ABI5_0_0RCTComponent> view, __unused id json){};
        }
        switch (typeSignature.methodReturnType[0]) {

  #define ABI5_0_0RCT_CASE(_value, _type) \
          case _value: { \
            ...
            setterBlock = ^(id target, id json) { \
              if (json) { \
                ...
                set(target, setter, convert([ABI5_0_0RCTConvert class], type, json)); \
            ...
            ABI5_0_0RCT_CASE(_C_SEL, SEL)
            ABI5_0_0RCT_CASE(_C_CHARPTR, const char *)
            ABI5_0_0RCT_CASE(_C_CHR, char)
            ABI5_0_0RCT_CASE(_C_UCHR, unsigned char)
            ABI5_0_0RCT_CASE(_C_SHT, short)
            ABI5_0_0RCT_CASE(_C_USHT, unsigned short)
            ABI5_0_0RCT_CASE(_C_INT, int)
            ABI5_0_0RCT_CASE(_C_UINT, unsigned int)
            ABI5_0_0RCT_CASE(_C_LNG, long)
            ABI5_0_0RCT_CASE(_C_ULNG, unsigned long)
            ABI5_0_0RCT_CASE(_C_LNG_LNG, long long)
            ABI5_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
            ABI5_0_0RCT_CASE(_C_FLT, float)
            ABI5_0_0RCT_CASE(_C_DBL, double)
            ABI5_0_0RCT_CASE(_C_BOOL, BOOL)
            ABI5_0_0RCT_CASE(_C_PTR, void *)
            ABI5_0_0RCT_CASE(_C_ID, id)
          ...
          default: {
            ...
            typeInvocation.target = [ABI5_0_0RCTConvert class];
      ...
      propBlock = ^(__unused id view, __unused id json) {
        ...
        setterBlock(target, ABI5_0_0RCTNilIfNull(json));
      };
    }
    ...
    if (ABI5_0_0RCT_DEBUG) {
      ...
      ABI5_0_0RCTPropBlock unwrappedBlock = propBlock;
      propBlock = ^(id<ABI5_0_0RCTComponent> view, id json) {
        NSString *logPrefix = [NSString stringWithFormat:
                               @"Error setting property '%@' of %@ with tag #%@: ",
                               name, weakSelf.name, view.ReactABI5_0_0Tag];

        ABI5_0_0RCTPerformBlockWithLogPrefix(^{ unwrappedBlock(view, json); }, logPrefix);
      };
    }
...
- (void)setProps:(NSDictionary<NSString *, id> *)props forView:(id<ABI5_0_0RCTComponent>)view
{
  if (!view) {
    return;
  }
...
- (void)setProps:(NSDictionary<NSString *, id> *)props forShadowView:(ABI5_0_0RCTShadowView *)shadowView
{
  if (!shadowView) {
    return;
  }
...
- (NSDictionary<NSString *, id> *)viewConfig
{
  ...
  if (ABI5_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customDirectEventTypes))) {
    NSArray<NSString *> *events = [self.manager customDirectEventTypes];
    if (ABI5_0_0RCT_DEBUG) {
      ABI5_0_0RCTAssert(!events || [events isKindOfClass:[NSArray class]],
        @"customDirectEventTypes must return an array, but %@ returned %@",
        _managerClass, [events class]);
    }
    for (NSString *event in events) {
      [directEvents addObject:ABI5_0_0RCTNormalizeInputEventName(event)];
    }
  }

  NSMutableArray<NSString *> *bubblingEvents = [NSMutableArray new];
  if (ABI5_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    ...
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI5_0_0RCTNormalizeInputEventName(event)];
    }
  }
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if ([methodName hasPrefix:@"propConfig"]) {
      ...
      if (nameRange.length) {
        ...
        if (ABI5_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
          ABI5_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                      "to '%@'", name, _name, propTypes[name], type);
        }

        if ([type isEqualToString:@"ABI5_0_0RCTBubblingEventBlock"]) {
          [bubblingEvents addObject:ABI5_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else if ([type isEqualToString:@"ABI5_0_0RCTDirectEventBlock"]) {
          [directEvents addObject:ABI5_0_0RCTNormalizeInputEventName(name)];
          propTypes[name] = @"BOOL";
        } else {
          propTypes[name] = type;
        }
  ...
  if (ABI5_0_0RCT_DEBUG) {
    for (NSString *event in directEvents) {
      if ([bubblingEvents containsObject:event]) {
        ABI5_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
...
- (ABI5_0_0RCTViewManagerUIBlock)uiBlockToAmendWithShadowViewRegistry:(NSDictionary<NSNumber *, ABI5_0_0RCTShadowView *> *)registry
{
