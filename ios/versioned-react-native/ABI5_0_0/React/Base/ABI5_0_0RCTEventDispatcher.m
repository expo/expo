...
const NSInteger ABI5_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI5_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI5_0_0RCTGetEventID(id<ABI5_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI5_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI5_0_0RCTEvent>> *_events;
...
{
  [_bridge enqueueJSCall:@"ABI5_0_0RCTNativeAppEventEmitter.emit"
                    args:body ? @[name, body] : @[name]];
}

- (void)sendDeviceEventWithName:(NSString *)name body:(id)body
{
  [_bridge enqueueJSCall:@"ABI5_0_0RCTDeviceEventEmitter.emit"
                    args:body ? @[name, body] : @[name]];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI5_0_0RCT_DEBUG) {
    ABI5_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI5_0_0 tag");
  }

  name = ABI5_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI5_0_0RCTEventEmitter.receiveEvent"
...
- (void)sendEvent:(id<ABI5_0_0RCTEvent>)event
{
  ...
  NSNumber *eventID = ABI5_0_0RCTGetEventID(event);

  id<ABI5_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI5_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
  ...
  if (scheduleEventsDispatch) {
    ...
    } queue:ABI5_0_0RCTJSThread];
  }
}

- (void)dispatchEvent:(id<ABI5_0_0RCTEvent>)event
{
