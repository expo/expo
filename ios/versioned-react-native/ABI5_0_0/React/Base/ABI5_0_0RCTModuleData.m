...
@implementation ABI5_0_0RCTModuleData
{
...
- (void)setUp
{
  ...
  _hasConstantsToExport = ABI5_0_0RCTClassOverridesInstanceMethod(_moduleClass, @selector(constantsToExport));
}
...
- (instancetype)initWithModuleInstance:(id<ABI5_0_0RCTBridgeModule>)instance
                                bridge:(ABI5_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _bridge = bridge;
...
- (void)setUpInstanceAndBridge
{
  ...
  if (!_setupComplete && _bridge.valid) {
    if (!_instance) {
      if (ABI5_0_0RCT_DEBUG && _requiresMainThreadSetup) {
      ...
      if (!_instance) {
        ...
        _setupComplete = YES;
        ABI5_0_0RCTLogWarn(@"The module %@ is returning nil from its constructor. You "
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ...
    @catch (NSException *exception) {
      ABI5_0_0RCTLogError(@"%@ has no setter or ivar for its bridge, which is not "
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI5_0_0RCTDidInitializeModuleNotification
                                                        object:_bridge
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ReactABI5_0_0.%@Queue", self.name];
      ...
      if (implementsMethodQueue) {
        ...
        @catch (NSException *exception) {
          ABI5_0_0RCTLogError(@"%@ is returning nil for its methodQueue, which is not "
...
- (id<ABI5_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI5_0_0RCT_PROFILE_BEGIN_EVENT(0, [NSString stringWithFormat:@"[ABI5_0_0RCTModuleData instanceForClass:%@]", _moduleClass], nil);
...
- (NSString *)name
{
  return ABI5_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI5_0_0RCTBridgeMethod>> *)methods
{
  if (!_methods) {
    NSMutableArray<id<ABI5_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
      ...
      if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
        ...
        id<ABI5_0_0RCTBridgeMethod> moduleMethod =
          [[ABI5_0_0RCTModuleMethod alloc] initWithMethodSignature:entries[1]
...
- (NSArray *)config
{
  ...
  for (id<ABI5_0_0RCTBridgeMethod> method in self.methods) {
    if (method.functionType == ABI5_0_0RCTFunctionTypePromise) {
      if (!asyncMethods) {
