...
@interface ABI5_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@end

@implementation ABI5_0_0RCTRedBoxWindow
{
...
- (instancetype)initWithFrame:(CGRect)frame bridge:(ABI5_0_0RCTBridge *)bridge
{
...
{
  NSData *stackFrameJSON = [ABI5_0_0RCTJSONStringify(stackFrame, NULL) dataUsingEncoding:NSUTF8StringEncoding];
  NSString *postLength = [NSString stringWithFormat:@"%tu", stackFrameJSON.length];
  NSMutableURLRequest *request = [NSMutableURLRequest new];
  request.URL = [ABI5_0_0RCTConvert NSURL:@"http://localhost:8081/open-stack-frame"];
...
- (void)dismiss
{
  ...
  [ABI5_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}

- (void)reload
{
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI5_0_0RCTReloadNotification object:_bridge.baseBridge userInfo:nil];
}
...
{
  // NOTE: We could use ABI5_0_0RCTKeyCommands for this, but since
...
@interface ABI5_0_0RCTRedBox () <ABI5_0_0RCTInvalidating>
@end

@implementation ABI5_0_0RCTRedBox
{
  ABI5_0_0RCTRedBoxWindow *_window;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack showIfHidden:(BOOL)shouldShow
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!_window) {
      _window = [[ABI5_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds bridge:_bridge];
    }
    [_window showErrorMessage:message withStack:stack showIfHidden:shouldShow];
  });
}

ABI5_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [_window dismiss];
  });
...
@implementation ABI5_0_0RCTBridge (ABI5_0_0RCTRedBox)

- (ABI5_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI5_0_0RCTRedBox class]];
}
...
- (ABI5_0_0RCTRedBox *)redBox { return nil; }
