...
NSString *const ABI5_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI5_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI5_0_0RCTAccessibilityManager ()
...
@implementation ABI5_0_0RCTAccessibilityManager
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI5_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI5_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI5_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI5_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI5_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI5_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI5_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI5_0_0RCTResponseSenderBlock)callback
...
@implementation ABI5_0_0RCTBridge (ABI5_0_0RCTAccessibilityManager)

- (ABI5_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI5_0_0RCTAccessibilityManager class]];
}
