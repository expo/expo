...
static NSString * const ABI42_0_0EXUpdatesAppControllerErrorDomain = @"ABI42_0_0EXUpdatesAppController";

static NSString * const ABI42_0_0EXUpdatesUpdateAvailableEventName = @"updateAvailable";
static NSString * const ABI42_0_0EXUpdatesNoUpdateAvailableEventName = @"noUpdateAvailable";
static NSString * const ABI42_0_0EXUpdatesErrorEventName = @"error";
...
@implementation ABI42_0_0EXUpdatesAppController

+ (instancetype)sharedInstance
{
  static ABI42_0_0EXUpdatesAppController *theController;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!theController) {
      theController = [[ABI42_0_0EXUpdatesAppController alloc] init];
    }
  });
...
{
  if (self = [super init]) {
    _config = [ABI42_0_0EXUpdatesConfig configWithExpoPlist];
    _database = [[ABI42_0_0EXUpdatesDatabase alloc] init];
    _defaultSelectionPolicy = [ABI42_0_0EXUpdatesSelectionPolicyFactory filterAwarePolicyWithRuntimeVersion:[ABI42_0_0EXUpdatesUtils getRuntimeVersionWithConfig:_config]];
...
- (void)setConfiguration:(NSDictionary *)configuration
{
  if (_isStarted) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"ABI42_0_0EXUpdatesAppController:setConfiguration should not be called after start"
                                 userInfo:@{}];
  }
...
- (void)start
{
  NSAssert(!_isStarted, @"ABI42_0_0EXUpdatesAppController:start should only be called once per instance");

  if (!_config.isEnabled) {
    ABI42_0_0EXUpdatesAppLauncherNoDatabase *launcher = [[ABI42_0_0EXUpdatesAppLauncherNoDatabase alloc] init];
    _launcher = launcher;
  ...
  ABI42_0_0EXUpdatesAppLoaderTask *loaderTask = [[ABI42_0_0EXUpdatesAppLoaderTask alloc] initWithConfig:_config
                                                                             database:_database
...
# pragma mark - ABI42_0_0EXUpdatesAppLoaderTaskDelegate

- (BOOL)appLoaderTask:(ABI42_0_0EXUpdatesAppLoaderTask *)appLoaderTask didLoadCachedUpdate:(nonnull ABI42_0_0EXUpdatesUpdate *)update
{
  return YES;
}
...
- (void)appLoaderTask:(ABI42_0_0EXUpdatesAppLoaderTask *)appLoaderTask didFinishWithLauncher:(id<ABI42_0_0EXUpdatesAppLauncher>)launcher isUpToDate:(BOOL)isUpToDate
{
  _launcher = launcher;
  if (self->_delegate) {
    [ABI42_0_0EXUpdatesUtils runBlockOnMainThread:^{
...
{
  if (status == ABI42_0_0EXUpdatesBackgroundUpdateStatusError) {
...
# pragma mark - ABI42_0_0EXUpdatesAppController+Internal

- (BOOL)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  _updatesDirectory = [ABI42_0_0EXUpdatesUtils initializeUpdatesDirectoryWithError:error];
