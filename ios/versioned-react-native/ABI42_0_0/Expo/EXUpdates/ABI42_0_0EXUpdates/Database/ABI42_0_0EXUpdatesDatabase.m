...
#import <ABI42_0_0EXUpdates/ABI42_0_0EXUpdatesDatabaseInitialization.h>
...
@interface ABI42_0_0EXUpdatesDatabase ()

@property (nonatomic, assign) sqlite3 *db;

@end

static NSString * const ABI42_0_0EXUpdatesDatabaseManifestFiltersKey = @"manifestFilters";
static NSString * const ABI42_0_0EXUpdatesDatabaseServerDefinedHeadersKey = @"serverDefinedHeaders";

@implementation ABI42_0_0EXUpdatesDatabase
...
- (BOOL)openDatabaseInDirectory:(NSURL *)directory withError:(NSError ** _Nullable)error
{
  ...
  sqlite3 *db;
  if (![ABI42_0_0EXUpdatesDatabaseInitialization initializeDatabaseWithLatestSchemaInDirectory:directory database:&db error:error]) {
    return NO;
  }
  ...
  for (ABI42_0_0EXUpdatesAsset *asset in assets) {
    ...
    if ([self _executeSql:assetInsertSql
                 withArgs:@[
                          ...
                          asset.filename,
                          asset.contentHash,
                          @(ABI42_0_0EXUpdatesDatabaseHashTypeSha1)
                          ]
                    error:error] == nil) {
...
{
  if (update.status != ABI42_0_0EXUpdatesUpdateStatusDevelopment) {
    update.status = ABI42_0_0EXUpdatesUpdateStatusReady;
  }
...
- (void)markMissingAssets:(NSArray<ABI42_0_0EXUpdatesAsset *> *)assets error:(NSError ** _Nullable)error
{
  ...
  for (ABI42_0_0EXUpdatesAsset *asset in assets) {
    if ([self _executeSql:updatesSql withArgs:@[@(ABI42_0_0EXUpdatesUpdateStatusPending), @(asset.assetId)] error:error] == nil) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
      return;
    }
  }
...
- (nullable NSArray<ABI42_0_0EXUpdatesUpdate *> *)allUpdatesWithConfig:(ABI42_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI42_0_0EXUpdatesUpdate *> *launchableUpdates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI42_0_0EXUpdatesUpdate *> *)allUpdatesWithStatus:(ABI42_0_0EXUpdatesUpdateStatus)status config:(ABI42_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI42_0_0EXUpdatesUpdate *> *updates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI42_0_0EXUpdatesUpdate *> *)launchableUpdatesWithConfig:(ABI42_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  FROM updates\
  WHERE scope_key = ?1\
  AND status IN (%li, %li, %li);", (long)ABI42_0_0EXUpdatesUpdateStatusReady, (long)ABI42_0_0EXUpdatesUpdateStatusEmbedded, (long)ABI42_0_0EXUpdatesUpdateStatusDevelopment];
...
- (nullable NSArray<ABI42_0_0EXUpdatesAsset *> *)allAssetsWithError:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI42_0_0EXUpdatesAsset *> *assets = [NSMutableArray arrayWithCapacity:rows.count];
...
{
  return [self _jsonDataWithKey:ABI42_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey error:error];
}

- (nullable NSDictionary *)manifestFiltersWithScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  return [self _jsonDataWithKey:ABI42_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey error:error];
}
...
{
  [self _setJsonData:serverDefinedHeaders withKey:ABI42_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey isInTransaction:NO error:error];
}

- (void)setManifestFilters:(NSDictionary *)manifestFilters withScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  [self _setJsonData:manifestFilters withKey:ABI42_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey isInTransaction:NO error:error];
}
...
- (void)setMetadataWithManifest:(ABI42_0_0EXUpdatesUpdate *)updateManifest error:(NSError ** _Nullable)error
{
  ...
  if (updateManifest.serverDefinedHeaders) {
    if (![self _setJsonData:updateManifest.serverDefinedHeaders
                   withKey:ABI42_0_0EXUpdatesDatabaseServerDefinedHeadersKey
  ...
  if (updateManifest.manifestFilters) {
    if (![self _setJsonData:updateManifest.manifestFilters
                   withKey:ABI42_0_0EXUpdatesDatabaseManifestFiltersKey
                  scopeKey:updateManifest.scopeKey
           isInTransaction:YES
                     error:error]) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
...
- (nullable NSArray<NSDictionary *> *)_executeSql:(NSString *)sql withArgs:(nullable NSArray *)args error:(NSError ** _Nullable)error
{
  ...
  dispatch_assert_queue(_databaseQueue);
  return [ABI42_0_0EXUpdatesDatabaseUtils executeSql:sql withArgs:args onDatabase:_db error:error];
}

- (NSError *)_errorFromSqlite:(struct sqlite3 *)db
{
  return [ABI42_0_0EXUpdatesDatabaseUtils errorFromSqlite:db];
}

- (ABI42_0_0EXUpdatesUpdate *)_updateWithRow:(NSDictionary *)row config:(ABI42_0_0EXUpdatesConfig *)config
{
  NSError *error;
  id manifest = nil;
  ...
  ABI42_0_0EXUpdatesUpdate *update = [ABI42_0_0EXUpdatesUpdate updateWithId:row[@"id"]
                                                 scopeKey:row[@"scope_key"]
                                               commitTime:[ABI42_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"commit_time"]]
                                           runtimeVersion:row[@"runtime_version"]
                                                 manifest:manifest
                                                   status:(ABI42_0_0EXUpdatesUpdateStatus)[(NSNumber *)row[@"status"] integerValue]
                                                     keep:[(NSNumber *)row[@"keep"] boolValue]
                                                   config:config
                                                 database:self];
  update.lastAccessed = [ABI42_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"last_accessed"]];
  return update;
}

- (ABI42_0_0EXUpdatesAsset *)_assetWithRow:(NSDictionary *)row
{
  NSError *error;
  id metadata = nil;
  ...
  ABI42_0_0EXUpdatesAsset *asset = [[ABI42_0_0EXUpdatesAsset alloc] initWithKey:key type:row[@"type"]];
  asset.assetId = [(NSNumber *)row[@"id"] unsignedIntegerValue];
  asset.url = url;
  asset.downloadTime = [ABI42_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"download_time"]];
