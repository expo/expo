...
#import <ABI42_0_0EXFirebaseCore/ABI42_0_0EXFirebaseCore+FIROptions.h>
...
@implementation ABI42_0_0EXScopedFirebaseCore {
  NSDictionary* _protectedAppNames;
}

- (instancetype)initWithScopeKey:(NSString *)scopeKey manifest:(ABI42_0_0EXManifestsRawManifest *)manifest constantsBinding:(ABI42_0_0EXConstantsBinding *)constantsBinding
{
  ...
  for (NSString* name in names) {
    if (!protectedAppNames[name] && (!options || ![name isEqualToString:appName])) {
      [[FIRApp appNamed:name] deleteApp:^(BOOL success) {
        if (!success) {
          ABI42_0_0UMLogWarn(@"Failed to delete Firebase app: %@", name);
        }
      }];
    }
  }
...
+ (nullable NSDictionary *)googleServicesFileFromManifest:(ABI42_0_0EXManifestsRawManifest *)manifest
{
  ...
  @try {
    ...
    if (error) ABI42_0_0UMLogWarn(@"Invalid googleServicesFile: %@", error);
    return plist;
  }
  @catch(NSException* exception) {
    ABI42_0_0UMLogWarn(@"Invalid googleServicesFile: %@", exception);
    return nil;
  }
}
