//
//  ABI42_0_0RNSharedElementNode.m
...
@interface ABI42_0_0RNSharedElementNodeResolvedSource : NSObject
...
@end
@implementation ABI42_0_0RNSharedElementNodeResolvedSource
- (instancetype)initWithView:(UIView*) view hasChildImageView:(BOOL)hasChildImageView
{
  _view = view;
...
+ (instancetype)sourceWithView:(UIView*) view {
  return [[ABI42_0_0RNSharedElementNodeResolvedSource alloc]initWithView:view hasChildImageView:NO];
}
+ (instancetype)sourceWithView:(UIView*) view hasChildImageView:(BOOL)hasChildImageView {
  return [[ABI42_0_0RNSharedElementNodeResolvedSource alloc]initWithView:view hasChildImageView:hasChildImageView];
}
- (UIView*) contentView {
...
@implementation ABI42_0_0RNSharedElementNode
{
  long _refCount;
  long _hideRefCount;
  
  NSMutableArray* _contentRequests;
  ABI42_0_0RNSharedElementContent* _contentCache;
  CFTimeInterval _contentCacheTimeInterval;
  
  NSMutableArray* _styleRequests;
  ABI42_0_0RNSharedElementStyle* _styleCache;
  CFTimeInterval _styleCacheTimeInterval;
  
  CADisplayLink* _displayLink;
  
  __weak UIView* _sourceView;
  ABI42_0_0RNSharedElementNodeResolvedSource* _resolvedSource;
}
...
{
  _ABI42_0_0ReactTag = ABI42_0_0ReactTag;
  _sourceView = view;
  _isParent = isParent;
  _refCount = 1;
  _hideRefCount = 0;
  ...
  _displayLink = nil;
  _resolvedSource = [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:nil];
...
{
  ABI42_0_0RNSharedElementNodeResolvedSource* resolvedSource = noReset
  ? [ABI42_0_0RNSharedElementNode resolveSource:_isParent ? _sourceView.subviews.firstObject : _sourceView]
  : [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:nil];
...
+ (ABI42_0_0RNSharedElementNodeResolvedSource*) resolveSource:(UIView*) view
{
  if (view == nil || _imageResolvers == nil) return [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:view];
  
  // If the view is an ImageView, then use that.
  if ([ABI42_0_0RNSharedElementContent isKindOfImageView:view]) {
    return [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:view];
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if ([ABI42_0_0RNSharedElementContent isKindOfImageView:subview]) {
      ...
      if (CGRectEqualToRect(subview.frame, bounds)) {
        ...
        return [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:view hasChildImageView:YES];
      }
    }
  }
  
  // Resolve the underlying ImageViews of well known
  // ABI42_0_0React-native libs (e.g. ABI42_0_0React-native-fast-image)
  for (NSArray* imageResolver in _imageResolvers) {
    ...
    if (foundImageView != nil) {
      return [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:foundImageView];
    }
  }
  return [ABI42_0_0RNSharedElementNodeResolvedSource sourceWithView:view];
}
...
{
  if ([ABI42_0_0RNSharedElementContent isKindOfImageView:view]) {
    UIImageView* imageView = [ABI42_0_0RNSharedElementContent imageViewFromView:view];
...
- (void) requestContent:(__weak id <ABI42_0_0RNSharedElementDelegate>) delegate
{
...
- (void) updateContent
{
  ...
  [self updateResolvedSource:YES];
  ABI42_0_0RNSharedElementNodeResolvedSource* resolvedSource = _resolvedSource;
  ...
  ABI42_0_0RNSharedElementContent* content;
  if ([ABI42_0_0RNSharedElementContent isKindOfImageView:contentView]) {
    UIImageView* imageView = [ABI42_0_0RNSharedElementContent imageViewFromView:contentView];
    ...
    content = [[ABI42_0_0RNSharedElementContent alloc]initWithData:image type:ABI42_0_0RNSharedElementContentTypeRawImage insets:imageInsets];
  }
  else if ([NSStringFromClass(view.class) isEqualToString:@"ABI42_0_0RCTView"] && !view.subviews.count) {
    UIView* dummyView = [[UIView alloc]init];
    content = [[ABI42_0_0RNSharedElementContent alloc]initWithData:dummyView type:ABI42_0_0RNSharedElementContentTypeSnapshotView insets:UIEdgeInsetsZero];
  }
  else {
    UIView* snapshotView = [view snapshotViewAfterScreenUpdates:NO];
    content = [[ABI42_0_0RNSharedElementContent alloc]initWithData:snapshotView type:ABI42_0_0RNSharedElementContentTypeSnapshotView insets:UIEdgeInsetsZero];
  }
  /*else {
   NSLog(@"drawViewHierarchyInRect: bounds: %@", NSStringFromCGRect(bounds));
   UIGraphicsBeginImageContextWithOptions(bounds.size, NO, 0.0f);
   BOOL res = [view drawViewHierarchyInRect:bounds afterScreenUpdates:NO]; // NEVER USE YES, IT CREATED VISUAL ABI42_0_0ARTEFACTS ON THE CREEN
   UIImage* image = res ? UIGraphicsGetImageFromCurrentImageContext() : nil;
   UIGraphicsEndImageContext();
   NSLog(@"drawViewHierarchyInRect: RESULT: %li", res);
   content = image;
   contentType = ABI42_0_0RNSharedElementContentTypeSnapshotImage;
   }*/
  ...
  for (__weak id <ABI42_0_0RNSharedElementDelegate> delegate in delegates) {
    if (delegate != nil) {
...
- (void) requestStyle:(__weak id <ABI42_0_0RNSharedElementDelegate>) delegate
{
...
- (void) updateStyle
{
  ...
  // Create style
  ABI42_0_0RNSharedElementStyle* style = [[ABI42_0_0RNSharedElementStyle alloc]initWithView:view];
  style.layout = layout;
  if ([ABI42_0_0RNSharedElementContent isKindOfImageView:contentView]) {
    UIImageView* imageView = [ABI42_0_0RNSharedElementContent imageViewFromView:contentView];
  ...
  /*if (_isParent) {
   NSLog(@"Style fetched: %@, realSize: %@, opacity: %lf, transform: %@, borderWidth: %lf, contentMode: %ld", NSStringFromCGRect(layout), NSStringFromCGSize(style.size), style.opacity, [ABI42_0_0RNSharedElementStyle stringFromTransform:style.transform], style.borderWidth, style.contentMode);
   }*/
