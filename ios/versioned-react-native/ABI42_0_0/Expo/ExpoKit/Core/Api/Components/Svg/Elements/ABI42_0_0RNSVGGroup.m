...
@implementation ABI42_0_0RNSVGGroup
{
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI42_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI42_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI42_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI42_0_0RNSVGMask class]]) {
...
- (ABI42_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI42_0_0RNSVGNode class]];
    ...
    for (ABI42_0_0RNSVGView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI42_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI42_0_0RNSVGMask class]]) {
                continue;
            }
            ABI42_0_0RNSVGNode* svgNode = (ABI42_0_0RNSVGNode*)node;
            if (event) {
            ...
            ABI42_0_0RNSVGPlatformView *hitChild = [svgNode hitTest:transformed withEvent:event];
            if (hitChild) {
                svgNode.active = YES;
            ...
            ABI42_0_0RNSVGSvgView* svgView = (ABI42_0_0RNSVGSvgView*)node;
            ABI42_0_0RNSVGPlatformView *hitChild = [svgView hitTest:transformed withEvent:event];
            if (hitChild) {
                return hitChild;
            }
        }
    ...
    ABI42_0_0RNSVGPlatformView *hitSelf = [super hitTest:transformed withEvent:event];
    if (hitSelf) {
        return hitSelf;
    }
...
{
    [self traverseSubviews:^(__kindof ABI42_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI42_0_0RNSVGRenderable class]]) {
            [(ABI42_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
