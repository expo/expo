...
#define ABI42_0_0EXPECT_EQ_WITH_THRESHOLD(a, b, threshold) \
  ABI42_0_0EXPECT_TRUE((a >= b - threshold) && (a <= b + threshold))
...
TEST(MountingTelemetryTest, timepoints) {
  ...
  ABI42_0_0EXPECT_EQ_WITH_THRESHOLD(duration, 100, threshold);
}
...
TEST(MountingTelemetryTest, normalUseCase) {
  ...
  ABI42_0_0EXPECT_EQ_WITH_THRESHOLD(commitDuration, 400, threshold * 2);
  ABI42_0_0EXPECT_EQ_WITH_THRESHOLD(layoutDuration, 200, threshold);
  ABI42_0_0EXPECT_EQ_WITH_THRESHOLD(mountDuration, 100, threshold);
}
