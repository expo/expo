...
@implementation ABI42_0_0RCTImageManager {
...
- (instancetype)initWithImageLoader:(id<ABI42_0_0RCTImageLoaderWithAttributionProtocol>)imageLoader
{
  if (self = [super init]) {
    ...
    _backgroundSerialQueue =
        dispatch_queue_create("com.facebook.ABI42_0_0React-native.image-manager-queue", DISPATCH_QUEUE_SERIAL);
  }
...
{
  SystraceSection s("ABI42_0_0RCTImageManager::requestImage");

  auto imageInstrumentation = std::make_shared<ABI42_0_0RCTImageInstrumentationProxy>(_imageLoader);
   ...
   * work (such as creating an `NSURLRequest` object and some obscure logic inside `ABI42_0_0RCTImageLoader`) can take a couple
   ...
   *
   * Technically, we don't need to dispatch this to *serial* queue. The interface of `ABI42_0_0RCTImageLoader` promises to be
