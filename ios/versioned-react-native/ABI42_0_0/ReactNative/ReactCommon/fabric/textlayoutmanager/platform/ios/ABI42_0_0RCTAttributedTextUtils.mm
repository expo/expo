...
#include <ABI42_0_0React/textlayoutmanager/ABI42_0_0RCTTextPrimitivesConversions.h>
#include <ABI42_0_0React/utils/ManagedObjectWrapper.h>

using namespace ABI42_0_0facebook::ABI42_0_0React;

@implementation ABI42_0_0RCTWeakEventEmitterWrapper {
...
inline static UIFontWeight ABI42_0_0RCTUIFontWeightFromInteger(NSInteger fontWeight)
{
...
inline static UIFont *ABI42_0_0RCTEffectiveFontFromTextAttributes(const TextAttributes &textAttributes)
{
  ...
  fontProperties.style = textAttributes.fontStyle.hasValue()
      ? ABI42_0_0RCTFontStyleFromFontStyle(textAttributes.fontStyle.value())
      : ABI42_0_0RCTFontStyleUndefined;
  fontProperties.variant = textAttributes.fontVariant.hasValue()
      ? ABI42_0_0RCTFontVariantFromFontVariant(textAttributes.fontVariant.value())
      : ABI42_0_0RCTFontVariantUndefined;
  fontProperties.weight = textAttributes.fontWeight.hasValue()
      ? ABI42_0_0RCTUIFontWeightFromInteger((NSInteger)textAttributes.fontWeight.value())
      : NAN;
  fontProperties.sizeMultiplier = textAttributes.fontSizeMultiplier;

  return ABI42_0_0RCTFontWithFontProperties(fontProperties);
}

inline static CGFloat ABI42_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(const TextAttributes &textAttributes)
{
...
inline static UIColor *ABI42_0_0RCTEffectiveForegroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveForegroundColor = ABI42_0_0RCTUIColorFromSharedColor(textAttributes.foregroundColor) ?: [UIColor blackColor];
...
inline static UIColor *ABI42_0_0RCTEffectiveBackgroundColorFromTextAttributes(const TextAttributes &textAttributes)
{
  UIColor *effectiveBackgroundColor = ABI42_0_0RCTUIColorFromSharedColor(textAttributes.backgroundColor);
...
NSDictionary<NSAttributedStringKey, id> *ABI42_0_0RCTNSTextAttributesFromTextAttributes(TextAttributes const &textAttributes)
{
  NSMutableDictionary<NSAttributedStringKey, id> *attributes = [NSMutableDictionary dictionaryWithCapacity:10];

  // Font
  UIFont *font = ABI42_0_0RCTEffectiveFontFromTextAttributes(textAttributes);
  if (font) {
    attributes[NSFontAttributeName] = font;
  }

  // Colors
  UIColor *effectiveForegroundColor = ABI42_0_0RCTEffectiveForegroundColorFromTextAttributes(textAttributes);
  ...
  if (textAttributes.backgroundColor || !isnan(textAttributes.opacity)) {
    attributes[NSBackgroundColorAttributeName] = ABI42_0_0RCTEffectiveBackgroundColorFromTextAttributes(textAttributes);
  }
  ...
  if (textAttributes.alignment.hasValue()) {
    ...
    paragraphStyle.alignment = ABI42_0_0RCTNSTextAlignmentFromTextAlignment(textAlignment);
    isParagraphStyleUsed = YES;
  }

  if (textAttributes.baseWritingDirection.hasValue()) {
    paragraphStyle.baseWritingDirection =
        ABI42_0_0RCTNSWritingDirectionFromWritingDirection(textAttributes.baseWritingDirection.value());
    isParagraphStyleUsed = YES;
  }

  if (!isnan(textAttributes.lineHeight)) {
    CGFloat lineHeight = textAttributes.lineHeight * ABI42_0_0RCTEffectiveFontSizeMultiplierFromTextAttributes(textAttributes);
  ...
  if (textAttributes.textDecorationLineType.value_or(TextDecorationLineType::None) != TextDecorationLineType::None) {
    ...
    NSUnderlineStyle style = ABI42_0_0RCTNSUnderlineStyleFromStyleAndPattern(
    ...
    UIColor *textDecorationColor = ABI42_0_0RCTUIColorFromSharedColor(textAttributes.textDecorationColor);
  ...
  if (textAttributes.textShadowOffset.hasValue()) {
    ...
    shadow.shadowColor = ABI42_0_0RCTUIColorFromSharedColor(textAttributes.textShadowColor);
    attributes[NSShadowAttributeName] = shadow;
  }

  // Special
  if (textAttributes.isHighlighted) {
    attributes[ABI42_0_0RCTAttributedStringIsHighlightedAttributeName] = @YES;
  }

  return [attributes copy];
}

NSAttributedString *ABI42_0_0RCTNSAttributedStringFromAttributedString(const AttributedString &attributedString)
{
    ...
    if (fragment.isAttachment()) {
      ...
      nsAttributedStringFragment = [[NSMutableAttributedString alloc]
          initWithString:string
              attributes:ABI42_0_0RCTNSTextAttributesFromTextAttributes(fragment.textAttributes)];
    }

    if (fragment.parentShadowView.componentHandle) {
      ABI42_0_0RCTWeakEventEmitterWrapper *eventEmitterWrapper = [ABI42_0_0RCTWeakEventEmitterWrapper new];
      eventEmitterWrapper.eventEmitter = fragment.parentShadowView.eventEmitter;

      NSDictionary<NSAttributedStringKey, id> *additionalTextAttributes =
          @{ABI42_0_0RCTAttributedStringEventEmitterKey : eventEmitterWrapper};
...
NSAttributedString *ABI42_0_0RCTNSAttributedStringFromAttributedStringBox(AttributedStringBox const &attributedStringBox)
{
  switch (attributedStringBox.getMode()) {
    case AttributedStringBox::Mode::Value:
      return ABI42_0_0RCTNSAttributedStringFromAttributedString(attributedStringBox.getValue());
    case AttributedStringBox::Mode::OpaquePointer:
      return (NSAttributedString *)unwrapManagedObject(attributedStringBox.getOpaquePointer());
  }
}

AttributedStringBox ABI42_0_0RCTAttributedStringBoxFromNSAttributedString(NSAttributedString *nsAttributedString)
{
