...
TEST(ComponentDescriptorTest, createShadowNode) {
  ...
  ABI42_0_0EXPECT_EQ(descriptor->getComponentHandle(), TestShadowNode::Handle());
  ABI42_0_0EXPECT_STREQ(descriptor->getComponentName(), TestShadowNode::Name());
  ABI42_0_0EXPECT_STREQ(descriptor->getComponentName(), "Test");
  ...
  ABI42_0_0EXPECT_EQ(node->getComponentHandle(), TestShadowNode::Handle());
  ABI42_0_0EXPECT_STREQ(node->getComponentName(), TestShadowNode::Name());
  ABI42_0_0EXPECT_STREQ(node->getComponentName(), "Test");
  ABI42_0_0EXPECT_EQ(node->getTag(), 9);
  ABI42_0_0EXPECT_EQ(node->getSurfaceId(), 1);
  ABI42_0_0EXPECT_STREQ(node->getProps()->nativeId.c_str(), "abc");
}
...
TEST(ComponentDescriptorTest, cloneShadowNode) {
  ...
  ABI42_0_0EXPECT_STREQ(cloned->getComponentName(), "Test");
  ABI42_0_0EXPECT_EQ(cloned->getTag(), 9);
  ABI42_0_0EXPECT_EQ(cloned->getSurfaceId(), 1);
  ABI42_0_0EXPECT_STREQ(cloned->getProps()->nativeId.c_str(), "abc");
}
...
TEST(ComponentDescriptorTest, appendChild) {
  ...
  ABI42_0_0EXPECT_EQ(node1Children.size(), 2);
  ABI42_0_0EXPECT_EQ(node1Children.at(0), node2);
  ABI42_0_0EXPECT_EQ(node1Children.at(1), node3);
}
