...
#import "ABI42_0_0RCTSurfaceTouchHandler.h"
...
#import "ABI42_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI42_0_0RCTTouchEventType) {
  ABI42_0_0RCTTouchEventTypeTouchStart,
  ABI42_0_0RCTTouchEventTypeTouchMove,
  ABI42_0_0RCTTouchEventTypeTouchEnd,
  ABI42_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI42_0_0RCTComponentViewProtocol> *componentView = nil;
...
static void UpdateActiveTouchWithUITouch(ActiveTouch &activeTouch, UITouch *uiTouch, UIView *rootComponentView)
{
  ...
  activeTouch.touch.offsetPoint = ABI42_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI42_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI42_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI42_0_0RCTForceTouchAvailable()) {
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView)
{
  ...
  if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
    activeTouch.eventEmitter = [(id<ABI42_0_0RCTTouchableComponentViewProtocol>)componentView
...
@interface ABI42_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI42_0_0RCTSurfaceTouchHandler {
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI42_0_0React Native,
...
ABI42_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithTarget : (id)target action : (SEL)action)

- (void)attachToView:(UIView *)view
{
  ABI42_0_0RCTAssert(self.view == nil, @"ABI42_0_0RCTTouchHandler already has attached view.");
...
{
  ABI42_0_0RCTAssertParam(view);
  ABI42_0_0RCTAssert(self.view == view, @"ABI42_0_0RCTTouchHandler attached to another view.");
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI42_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI42_0_0RCTTouchEventTypeTouchEnd || eventType == ABI42_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitters) {
    ...
    switch (eventType) {
      case ABI42_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI42_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI42_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI42_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI42_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI42_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI42_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI42_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (_activeTouches.size() != 0) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI42_0_0RCTTouchEventTypeTouchCancel];
