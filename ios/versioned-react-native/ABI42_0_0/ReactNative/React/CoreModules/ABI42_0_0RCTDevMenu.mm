...
#import <ABI42_0_0React/ABI42_0_0RCTBundleURLProvider.h>
#import <ABI42_0_0React/ABI42_0_0RCTDefines.h>
#import <ABI42_0_0React/ABI42_0_0RCTDevSettings.h>
#import <ABI42_0_0React/ABI42_0_0RCTKeyCommands.h>
...
#import <ABI42_0_0React/ABI42_0_0RCTInspectorDevServerHelper.h>
#endif

NSString *const ABI42_0_0RCTShowDevMenuNotification = @"ABI42_0_0RCTShowDevMenuNotification";

@implementation UIWindow (ABI42_0_0RCTDevMenu)

- (void)ABI42_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
...
@implementation ABI42_0_0RCTDevMenuItem {
  ABI42_0_0RCTDevMenuItemTitleBlock _titleBlock;
  dispatch_block_t _handler;
}

- (instancetype)initWithTitleBlock:(ABI42_0_0RCTDevMenuItemTitleBlock)titleBlock handler:(dispatch_block_t)handler
{
  if ((self = [super init])) {
...
typedef void (^ABI42_0_0RCTDevMenuAlertActionHandler)(UIAlertAction *action);

@interface ABI42_0_0RCTDevMenu () <ABI42_0_0RCTBridgeModule, ABI42_0_0RCTInvalidating, ABI42_0_0NativeDevMenuSpec>

@end

@implementation ABI42_0_0RCTDevMenu {
  UIAlertController *_actionSheet;
  NSMutableArray<ABI42_0_0RCTDevMenuItem *> *_extraMenuItems;
}
  ...
  if ((self = [super init])) {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showOnShake)
                                                 name:ABI42_0_0RCTShowDevMenuNotification
                                               object:nil];
    _extraMenuItems = [NSMutableArray new];

#if TARGET_OS_SIMULATOR || TARGET_OS_MACCATALYST
    ABI42_0_0RCTKeyCommands *commands = [ABI42_0_0RCTKeyCommands sharedInstance];
...
{
  [self addItem:[ABI42_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
{
  [[ABI42_0_0RCTBundleURLProvider sharedSettings] resetToDefaults];
  self->_bridge.bundleURL = [[ABI42_0_0RCTBundleURLProvider sharedSettings] jsBundleURLForFallbackResource:nil
                                                                                fallbackExtension:nil];
  ABI42_0_0RCTTriggerReloadCommandListeners(@"Dev menu - reset to default");
}

- (NSArray<ABI42_0_0RCTDevMenuItem *> *)_menuItemsToPresent
{
  NSMutableArray<ABI42_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items
  __weak ABI42_0_0RCTBridge *bridge = _bridge;
  __weak ABI42_0_0RCTDevSettings *devSettings = _bridge.devSettings;
  __weak ABI42_0_0RCTDevMenu *weakSelf = self;

  [items addObject:[ABI42_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload"
                                               handler:^{
                                                 ABI42_0_0RCTTriggerReloadCommandListeners(@"Dev menu - reload");
                                               }]];

  if (!devSettings.isProfilingEnabled) {
    if (!devSettings.isRemoteDebuggingAvailable) {
      [items
          addObject:[ABI42_0_0RCTDevMenuItem
                        buttonItemWithTitle:@"Debugger Unavailable"
                                    handler:^{
                                      NSString *message = ABI42_0_0RCTTurboModuleEnabled()
                                          ? @"Debugging is not currently supported when TurboModule is enabled."
                                          : @"Include the ABI42_0_0RCTWebSocket library to enable JavaScript debugging.";
                                      ...
                                      [ABI42_0_0RCTPresentedViewController() presentViewController:alertController
                                                                                 animated:YES
                                                                               completion:NULL];
                                    }]];
    } else {
      [items addObject:[ABI42_0_0RCTDevMenuItem
  ...
  [items
      addObject:[ABI42_0_0RCTDevMenuItem
                    ...
                    handler:^{
                          ...
                          addAction:[UIAlertAction
                                        actionWithTitle:@"Apply Changes"
                                                ...
                                                handler:^(__unused UIAlertAction *action) {
                                                  ...
                                                  if (portNumber == nil) {
                                                    portNumber = [NSNumber numberWithInt:ABI42_0_0RCT_METRO_PORT];
                                                  }
                                                  [ABI42_0_0RCTBundleURLProvider sharedSettings].jsLocation = [NSString
                                                      stringWithFormat:@"%@:%d", ipTextField.text, portNumber.intValue];
                                                  __strong ABI42_0_0RCTBridge *strongBridge = bridge;
                                                  if (strongBridge) {
                                                    NSURL *bundleURL = bundleRoot.length
                                                        ? [[ABI42_0_0RCTBundleURLProvider sharedSettings]
                                                    ...
                                                    ABI42_0_0RCTTriggerReloadCommandListeners(@"Dev menu - apply changes");
                                                  }
                                                }]];
                      ...
                      [ABI42_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
                    }]];
...
{
  if (_actionSheet || !_bridge || ABI42_0_0RCTRunningInAppExtension()) {
    return;
  }
  ...
  _actionSheet = [UIAlertController alertControllerWithTitle:@"ABI42_0_0React Native Debug Menu"
                                                     message:description
                                              preferredStyle:style];

  NSArray<ABI42_0_0RCTDevMenuItem *> *items = [self _menuItemsToPresent];
  for (ABI42_0_0RCTDevMenuItem *item in items) {
  ...
  [ABI42_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:nil];

  [_bridge enqueueJSCall:@"ABI42_0_0RCTNativeAppEventEmitter" method:@"emit" args:@[ @"ABI42_0_0RCTDevMenuShown" ] completion:NULL];
}

- (ABI42_0_0RCTDevMenuAlertActionHandler)alertActionHandlerForDevItem:(ABI42_0_0RCTDevMenuItem *__nullable)item
{
...
#define WARN_DEPRECATED_DEV_MENU_EXPORT() \
  ABI42_0_0RCTLogWarn(@"Using deprecated method %s, use ABI42_0_0RCTDevSettings instead", __func__)
...
ABI42_0_0RCT_EXPORT_METHOD(reload)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  ABI42_0_0RCTTriggerReloadCommandListeners(@"Unknown from JS");
}

ABI42_0_0RCT_EXPORT_METHOD(debugRemotely : (BOOL)enableDebug)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
  _bridge.devSettings.isDebuggingRemotely = enableDebug;
}

ABI42_0_0RCT_EXPORT_METHOD(setProfilingEnabled : (BOOL)enabled)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
...
ABI42_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled : (BOOL)enabled)
{
  WARN_DEPRECATED_DEV_MENU_EXPORT();
...
{
  return std::make_shared<ABI42_0_0facebook::ABI42_0_0React::NativeDevMenuSpecJSI>(self, jsInvoker, nativeInvoker, perfLogger);
}
...
@interface ABI42_0_0RCTDevMenu () <ABI42_0_0NativeDevMenuSpec>
@end
...
- (void)addItem:(ABI42_0_0RCTDevMenu *)item
{
}
...
@implementation ABI42_0_0RCTBridge (ABI42_0_0RCTDevMenu)

- (ABI42_0_0RCTDevMenu *)devMenu
{
#if ABI42_0_0RCT_DEV_MENU
  return [self moduleForClass:[ABI42_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
...
Class ABI42_0_0RCTDevMenuCls(void)
{
  return ABI42_0_0RCTDevMenu.class;
}
