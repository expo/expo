...
#import <ABI42_0_0React/ABI42_0_0RCTActionSheetManager.h>
...
@interface ABI42_0_0RCTActionSheetManager () <UIActionSheetDelegate, ABI42_0_0NativeActionSheetManagerSpec>
@end

@implementation ABI42_0_0RCTActionSheetManager {
                ...
                anchorViewTag:(NSNumber *)anchorViewTag
{
  ...
  if (anchorViewTag) {
    sourceView = [self.bridge.uiManager viewForABI42_0_0ReactTag:anchorViewTag];
  } else {
...
ABI42_0_0RCT_EXPORT_METHOD(showActionSheetWithOptions
                  ...
                  : (ABI42_0_0RCTResponseSenderBlock)callback)
{
  if (ABI42_0_0RCTRunningInAppExtension()) {
    ABI42_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  }
  ...
  NSArray<NSString *> *buttons = ABI42_0_0RCTConvertOptionalVecToArray(options.options(), ^id(NSString *element) {
    return element;
  });
  NSInteger cancelButtonIndex =
      options.cancelButtonIndex() ? [ABI42_0_0RCTConvert NSInteger:@(*options.cancelButtonIndex())] : -1;
  NSArray<NSNumber *> *destructiveButtonIndices;
  if (options.destructiveButtonIndices()) {
    destructiveButtonIndices = ABI42_0_0RCTConvertVecToArray(*options.destructiveButtonIndices(), ^id(double element) {
      return @(element);
    });
  } else {
  ...
  UIViewController *controller = ABI42_0_0RCTPresentedViewController();
  NSNumber *anchor = [ABI42_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  UIColor *tintColor = [ABI42_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];

  if (controller == nil) {
    ABI42_0_0RCTLogError(@"Tried to display action sheet but there is no application window. options: %@", @{
      @"title" : title,
      @"message" : message,
      @"options" : buttons,
  ...
  if (@available(iOS 13.0, *)) {
    NSString *userInterfaceStyle = [ABI42_0_0RCTConvert NSString:options.userInterfaceStyle()];
...
ABI42_0_0RCT_EXPORT_METHOD(showShareActionSheetWithOptions
                  : (JS::NativeActionSheetManager::SpecShowShareActionSheetWithOptionsOptions &)options failureCallback
                  : (ABI42_0_0RCTResponseSenderBlock)failureCallback successCallback
                  : (ABI42_0_0RCTResponseSenderBlock)successCallback)
{
  if (ABI42_0_0RCTRunningInAppExtension()) {
    ABI42_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  ...
  NSURL *URL = [ABI42_0_0RCTConvert NSURL:options.url()];
  if (URL) {
    if ([URL.scheme.lowercaseString isEqualToString:@"data"]) {
      NSError *error;
      NSData *data = [NSData dataWithContentsOfURL:URL options:(NSDataReadingOptions)0 error:&error];
      if (!data) {
        failureCallback(@[ ABI42_0_0RCTJSErrorFromNSError(error) ]);
        return;
      }
  ...
  if (items.count == 0) {
    ABI42_0_0RCTLogError(@"No `url` or `message` to share");
    return;
  }
  ...
  NSArray *excludedActivityTypes =
      ABI42_0_0RCTConvertOptionalVecToArray(options.excludedActivityTypes(), ^id(NSString *element) {
        return element;
      });
  ...
  shareController.completionWithItemsHandler =
      ^(NSString *activityType, BOOL completed, __unused NSArray *returnedItems, NSError *activityError) {
        if (activityError) {
          failureCallback(@[ ABI42_0_0RCTJSErrorFromNSError(activityError) ]);
        } else if (completed || activityType == nil) {
          successCallback(@[ @(completed), ABI42_0_0RCTNullIfNil(activityType) ]);
        }
      };

  NSNumber *anchorViewTag = [ABI42_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  shareController.view.tintColor = [ABI42_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];
...
Class ABI42_0_0RCTActionSheetManagerCls(void)
{
  return ABI42_0_0RCTActionSheetManager.class;
}
