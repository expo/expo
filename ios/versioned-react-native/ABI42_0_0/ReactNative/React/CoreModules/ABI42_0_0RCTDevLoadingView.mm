...
#import <ABI42_0_0React/ABI42_0_0RCTDevLoadingView.h>
...
#import <ABI42_0_0React/ABI42_0_0RCTDevLoadingViewSetEnabled.h>
#import <ABI42_0_0React/ABI42_0_0RCTModalHostViewController.h>
...
@interface ABI42_0_0RCTDevLoadingView () <ABI42_0_0NativeDevLoadingViewSpec>
@end

#if ABI42_0_0RCT_DEV | ABI42_0_0RCT_ENABLE_LOADING_VIEW

@implementation ABI42_0_0RCTDevLoadingView {
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
}
...
{
  ABI42_0_0RCTDevLoadingViewSetEnabled(enabled);
}
...
- (void)setBridge:(ABI42_0_0RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(hide)
                                               name:ABI42_0_0RCTJavaScriptDidFailToLoadNotification
                                             object:nil];
...
{
  if (!ABI42_0_0RCTDevLoadingViewGetEnabled()) {
    return;
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    self->_showDate = [NSDate date];
    if (!self->_window && !ABI42_0_0RCTRunningInTestEnvironment()) {
      CGSize screenSize = [UIScreen mainScreen].bounds.size;

      if (@available(iOS 11.0, *)) {
        UIWindow *window = ABI42_0_0RCTSharedApplication().keyWindow;
        self->_window =
    ...
    if (@available(iOS 13.0, *)) {
      UIWindowScene *scene = (UIWindowScene *)ABI42_0_0RCTSharedApplication().connectedScenes.anyObject;
...
ABI42_0_0RCT_EXPORT_METHOD(showMessage
                  : (NSString *)message withColor
                  : (NSNumber *__nonnull)color withBackgroundColor
                  : (NSNumber *__nonnull)backgroundColor)
{
  [self showMessage:message color:[ABI42_0_0RCTConvert UIColor:color] backgroundColor:[ABI42_0_0RCTConvert UIColor:backgroundColor]];
}
...
- (void)showWithURL:(NSURL *)URL
{
  ...
  if (URL.fileURL) {
    ...
    message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI42_0_0RCT_PACKAGER_NAME];
  } else {
    color = [UIColor whiteColor];
...
- (void)updateProgress:(ABI42_0_0RCTLoadingProgress *)progress
{
  if (!progress) {
    return;
  }
...
Class ABI42_0_0RCTDevLoadingViewCls(void)
{
  return ABI42_0_0RCTDevLoadingView.class;
}
