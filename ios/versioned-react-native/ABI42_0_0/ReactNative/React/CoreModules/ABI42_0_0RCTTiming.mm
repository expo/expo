...
@interface _ABI42_0_0RCTTimer : NSObject
...
@implementation _ABI42_0_0RCTTimer
...
@interface _ABI42_0_0RCTTimingProxy : NSObject

@end

// NSTimer retains its target, insert this class to break potential retain cycles
@implementation _ABI42_0_0RCTTimingProxy {
  __weak id _target;
}

+ (instancetype)proxyWithTarget:(id)target
{
  _ABI42_0_0RCTTimingProxy *proxy = [self new];
  if (proxy) {
...
@implementation ABI42_0_0RCTTiming {
  NSMutableDictionary<NSNumber *, _ABI42_0_0RCTTimer *> *_timers;
  NSTimer *_sleepTimer;
  BOOL _sendIdleEvents;
  BOOL _inBackground;
  id<ABI42_0_0RCTTimingDelegate> _timingDelegate;
}
...
{
  ABI42_0_0RCTAssert(!_bridge, @"Should never be initialized twice!");
  [self setup];
  _bridge = bridge;
}
...
- (void)didUpdateFrame:(ABI42_0_0RCTFrameUpdate *)update
{
  ...
  NSMutableArray<_ABI42_0_0RCTTimer *> *timersToCall = [NSMutableArray new];
  NSDate *now = [NSDate date]; // compare all the timers to the same base time
  @synchronized(_timers) {
    for (_ABI42_0_0RCTTimer *timer in _timers.allValues) {
  ...
  if (timersToCall.count > 0) {
    NSArray<NSNumber *> *sortedTimers = [[timersToCall sortedArrayUsingComparator:^(_ABI42_0_0RCTTimer *a, _ABI42_0_0RCTTimer *b) {
  ...
  for (_ABI42_0_0RCTTimer *timer in timersToCall) {
    if (timer.repeats) {
      [timer reschedule];
      nextScheduledTarget = [nextScheduledTarget earlierDate:timer.target];
    } else {
  ...
  if (_inBackground) {
    ...
    // No need to call the pauseCallback as ABI42_0_0RCTDisplayLink will ask us about our paused
  ...
  @synchronized(self) {
    if (!_sleepTimer || !_sleepTimer.valid) {
      _sleepTimer = [[NSTimer alloc] initWithFireDate:sleepTarget
                                             interval:0
                                               target:[_ABI42_0_0RCTTimingProxy proxyWithTarget:self]
 ...
 */
ABI42_0_0RCT_EXPORT_METHOD(createTimer
                  ...
                  : (BOOL)repeats)
{
  ...
  NSDate *schedulingTime = [ABI42_0_0RCTConvert NSDate:[NSNumber numberWithDouble:jsSchedulingTime]];
                        ...
                        repeats:(BOOL)repeats
{
  ...
  _ABI42_0_0RCTTimer *timer = [[_ABI42_0_0RCTTimer alloc] initWithCallbackID:callbackID
                                                  interval:jsDuration
...
ABI42_0_0RCT_EXPORT_METHOD(deleteTimer : (double)timerID)
{
  @synchronized(_timers) {
...
ABI42_0_0RCT_EXPORT_METHOD(setSendIdleEvents : (BOOL)sendIdleEvents)
{
...
Class ABI42_0_0RCTTimingCls(void)
{
  return ABI42_0_0RCTTiming.class;
}
