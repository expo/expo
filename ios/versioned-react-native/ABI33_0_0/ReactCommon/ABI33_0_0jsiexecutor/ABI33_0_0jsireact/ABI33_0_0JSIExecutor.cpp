...
#include <cxxReactABI33_0_0/ABI33_0_0SystraceSection.h>
...
class ABI33_0_0JSIExecutor::NativeModuleProxy : public ABI33_0_0jsi::HostObject {
 public:
  NativeModuleProxy(ABI33_0_0JSIExecutor& executor) : executor_(executor) {}

  Value get(Runtime& rt, const PropNameID& name) override {
    if (name.utf8(rt) == "name") {
      return ABI33_0_0jsi::String::createFromAscii(rt, "NativeModules");
    }
 ...
 private:
  ABI33_0_0JSIExecutor& executor_;
};
...
ABI33_0_0JSIExecutor::ABI33_0_0JSIExecutor(
    std::shared_ptr<ABI33_0_0jsi::Runtime> runtime,
    std::shared_ptr<ExecutorDelegate> delegate,
    Logger logger,
    const ABI33_0_0JSIScopedTimeoutInvoker& scopedTimeoutInvoker,
  ...
  runtime_->global().setProperty(
      *runtime, "__ABI33_0_0jsiExecutorDescription", runtime->description());
}

void ABI33_0_0JSIExecutor::loadApplicationScript(
    std::unique_ptr<const JSBigString> script,
    std::string sourceURL) {
  SystraceSection s("ABI33_0_0JSIExecutor::loadApplicationScript");
  ...
  if (hasLogger) {
    ReactABI33_0_0Marker::logTaggedMarker(
        ReactABI33_0_0Marker::RUN_JS_BUNDLE_START, scriptName.c_str());
  }
  runtime_->evaluateJavaScript(
      std::make_unique<BigStringBuffer>(std::move(script)), sourceURL);
  flush();
  if (hasLogger) {
    ReactABI33_0_0Marker::logMarker(ReactABI33_0_0Marker::CREATE_REACT_CONTEXT_STOP);
    ReactABI33_0_0Marker::logTaggedMarker(
        ReactABI33_0_0Marker::RUN_JS_BUNDLE_STOP, scriptName.c_str());
  }
}

void ABI33_0_0JSIExecutor::setBundleRegistry(std::unique_ptr<RAMBundleRegistry> r) {
  if (!bundleRegistry_) {
    runtime_->global().setProperty(
        *runtime_,
        "nativeRequire",
        Function::createFromHostFunction(
            *runtime_,
            ...
            [this](
                Runtime& rt,
                const facebook::ABI33_0_0jsi::Value&,
                const facebook::ABI33_0_0jsi::Value* args,
                size_t count) { return nativeRequire(args, count); }));
  }
  bundleRegistry_ = std::move(r);
}

void ABI33_0_0JSIExecutor::registerBundle(
    uint32_t bundleId,
    const std::string& bundlePath) {
  const auto tag = folly::to<std::string>(bundleId);
  ReactABI33_0_0Marker::logTaggedMarker(
      ReactABI33_0_0Marker::REGISTER_JS_SEGMENT_START, tag.c_str());
  if (bundleRegistry_) {
  ...
  ReactABI33_0_0Marker::logTaggedMarker(
      ReactABI33_0_0Marker::REGISTER_JS_SEGMENT_STOP, tag.c_str());
}

void ABI33_0_0JSIExecutor::callFunction(
    const std::string& moduleId,
    const std::string& methodId,
    const folly::dynamic& arguments) {
  SystraceSection s(
      "ABI33_0_0JSIExecutor::callFunction", "moduleId", moduleId, "methodId", methodId);
...
void ABI33_0_0JSIExecutor::invokeCallback(
    const double callbackId,
    const folly::dynamic& arguments) {
  SystraceSection s("ABI33_0_0JSIExecutor::invokeCallback", "callbackId", callbackId);
...
void ABI33_0_0JSIExecutor::setGlobalVariable(
    std::string propName,
    std::unique_ptr<const JSBigString> jsonValue) {
  SystraceSection s("ABI33_0_0JSIExecutor::setGlobalVariable", "propName", propName);
  runtime_->global().setProperty(
      *runtime_,
      propName.c_str(),
...
std::string ABI33_0_0JSIExecutor::getDescription() {
  return "ABI33_0_0JSI " + runtime_->description();
}

void* ABI33_0_0JSIExecutor::getJavaScriptContext() {
  return runtime_.get();
}

bool ABI33_0_0JSIExecutor::isInspectable() {
  return runtime_->isInspectable();
}

void ABI33_0_0JSIExecutor::bindBridge() {
  std::call_once(bindFlag_, [this] {
    SystraceSection s("ABI33_0_0JSIExecutor::bindBridge (once)");
    ...
    if (batchedBridgeValue.isUndefined()) {
      ...
      if (batchedBridgeValue.isUndefined()) {
        throw ABI33_0_0JSINativeException(
...
void ABI33_0_0JSIExecutor::callNativeModules(const Value& queue, bool isEndOfBatch) {
  SystraceSection s("ABI33_0_0JSIExecutor::callNativeModules");
...
void ABI33_0_0JSIExecutor::flush() {
  SystraceSection s("ABI33_0_0JSIExecutor::flush");
  if (flushedQueue_) {
...
Value ABI33_0_0JSIExecutor::nativeRequire(const Value* args, size_t count) {
  if (count > 2 || count == 0) {
  ...
  return facebook::ABI33_0_0jsi::Value();
}

Value ABI33_0_0JSIExecutor::nativeCallSyncHook(const Value* args, size_t count) {
  if (count != 3) {
