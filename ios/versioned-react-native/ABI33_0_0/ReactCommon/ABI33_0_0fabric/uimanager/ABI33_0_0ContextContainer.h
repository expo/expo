...
class ContextContainer final {
 public:
   ...
   * example if the type `T` is `std::shared_ptr<const ReactABI33_0_0NativeConfig>`,
   * then one would use `"ReactABI33_0_0NativeConfig"` for the `key`, even if the
   * instance is actually a `shared_ptr` of derived class
   *`EmptyReactABI33_0_0NativeConfig`.
   */
  template <typename T>
