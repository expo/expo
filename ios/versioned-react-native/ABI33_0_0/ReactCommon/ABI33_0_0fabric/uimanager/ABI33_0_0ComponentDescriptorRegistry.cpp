...
static const std::string componentNameByReactABI33_0_0ViewName(std::string viewName) {
  // We need this function only for the transition period;
  // eventually, all names will be unified.

  std::string rctPrefix("ABI33_0_0RCT");
  if (std::mismatch(rctPrefix.begin(), rctPrefix.end(), viewName.begin())
          .first == rctPrefix.end()) {
    // If `viewName` has "ABI33_0_0RCT" prefix, remove it.
...
const ComponentDescriptor &ComponentDescriptorRegistry::at(
  ...
  auto unifiedComponentName = componentNameByReactABI33_0_0ViewName(componentName);
...
SharedShadowNode ComponentDescriptorRegistry::createNode(
  ...
  ComponentName componentName = componentNameByReactABI33_0_0ViewName(viewName);
