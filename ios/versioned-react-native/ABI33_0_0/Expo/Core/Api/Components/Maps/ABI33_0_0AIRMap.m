...
const CLLocationDegrees ABI33_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI33_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI33_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI33_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI33_0_0AIRMap
{
    UIView *_legalLabel;
    ...
    // Array to manually track ABI33_0_0RN subviews
    //
    // ABI33_0_0AIRMap implicitly creates subviews that aren't regular ABI33_0_0RN children
    // (ABI33_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI33_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI33_0_0-native/blob/v0.16.0/ReactABI33_0_0/Modules/ABI33_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI33_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI33_0_0-native/blob/v0.16.0/Libraries/Text/ABI33_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ReactABI33_0_0Subviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _ReactABI33_0_0Subviews = [NSMutableArray new];
        ...
        self.calloutView = [ABI33_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI33_0_0AIRMapMaxZoomLevel;
...
- (void)insertReactABI33_0_0Subview:(id<ABI33_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI33_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI33_0_0AIRMapPolyline class]]) {
        ((ABI33_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI33_0_0AIRMapPolygon class]]) {
        ((ABI33_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI33_0_0AIRMapCircle class]]) {
        ((ABI33_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI33_0_0AIRMapUrlTile class]]) {
        ((ABI33_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI33_0_0AIRMapWMSTile class]]) {
        ((ABI33_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI33_0_0AIRMapLocalTile class]]) {
        ((ABI33_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI33_0_0AIRMapOverlay class]]) {
        ((ABI33_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI33_0_0RCTComponent>> *childSubviews = [subview ReactABI33_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertReactABI33_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ReactABI33_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI33_0_0Subview:(id<ABI33_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI33_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI33_0_0RCTComponent>> *childSubviews = [subview ReactABI33_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeReactABI33_0_0Subview:(UIView *)childSubviews[i]];
        }
    }
    [_ReactABI33_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ABI33_0_0AIRMapMarker *marker = (ABI33_0_0AIRMapMarker *)evaluatedObject;
        return [marker isKindOfClass:[ABI33_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI33_0_0AIRMapMarker*) markerForCallout:(ABI33_0_0AIRMapCallout*)callout {
    ABI33_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI33_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI33_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI33_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (ABI33_0_0AIRMapMarker*) markerAtPoint:(CGPoint)point {
    ABI33_0_0AIRMapMarker* mrk = nil;
    for (ABI33_0_0AIRMapMarker* mrkAnn in self.markers) {
        CGRect frame = [self frameForMarker:mrkAnn];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI33_0_0AIRMapCalloutSubview* calloutSubview = nil;
        ABI33_0_0AIRMapCallout* callout = nil;
        ABI33_0_0AIRMapMarker* marker = nil;
        
        UIView* tmp = touchedView;
        while (tmp && tmp != win && tmp != self.calloutView) {
            if ([tmp respondsToSelector:@selector(onPress)]) {
                calloutSubview = (ABI33_0_0AIRMapCalloutSubview*) tmp;
            }
            if ([tmp isKindOfClass:[ABI33_0_0AIRMapCallout class]]) {
                callout = (ABI33_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI33_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI33_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
