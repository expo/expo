...
@interface ABI33_0_0EXWKSnapshotPDFRenderer ()

@end

@implementation ABI33_0_0EXWKSnapshotPDFRenderer
...
- (void)takeSnapshotForPage:(int)pageIndex ofPages:(int)pagesCount ofWebView:(WKWebView *)webView withScrollHeight:(CGFloat)scrollHeight withCompletionHandler:(void (^ _Nullable)(NSError * _Nullable error))completionHandler
{
  ...
  if (@available(iOS 11.0, *)) {
    ...
    NSError *error = ABI33_0_0UMErrorWithMessage(@"Unexpected error occurred - on iOS under 11.0 use ABI33_0_0EXWKViewPDFRenderer.");
    completionHandler(error);
  }
}
