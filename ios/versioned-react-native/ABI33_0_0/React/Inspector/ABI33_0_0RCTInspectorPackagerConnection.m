...
// ReactABI33_0_0Android/src/main/java/com/facebook/ReactABI33_0_0/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int RECONNECT_DELAY_MS = 2000;

@implementation ABI33_0_0RCTBundleStatus
@end

@interface ABI33_0_0RCTInspectorPackagerConnection () <ABI33_0_0RCTSRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI33_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  ABI33_0_0RCTSRWebSocket *_webSocket;
  dispatch_queue_t _jsQueue;
  BOOL _closed;
  BOOL _suppressConnectionErrors;
  ABI33_0_0RCTBundleStatusProvider _bundleStatusProvider;
}
@end

@interface ABI33_0_0RCTInspectorRemoteConnection () {
  __weak ABI33_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI33_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI33_0_0RCTInspectorPackagerConnection
...
- (instancetype)initWithURL:(NSURL *)url
{
  if (self = [super init]) {
    ...
    _jsQueue = dispatch_queue_create("com.facebook.ReactABI33_0_0.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  }
  return self;
}
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI33_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  ABI33_0_0RCTInspectorLocalConnection *existingConnection = _inspectorConnections[pageId];
  if (existingConnection) {
    [_inspectorConnections removeObjectForKey:pageId];
    [existingConnection disconnect];
    ABI33_0_0RCTLogWarn(@"Already connected: %@", pageId);
    return;
  }

  ABI33_0_0RCTInspectorRemoteConnection *remoteConnection =
    [[ABI33_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self
                                                              pageId:pageId];

  ABI33_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI33_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI33_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (NSArray *)pages
{
  NSArray<ABI33_0_0RCTInspectorPage *> *pages = [ABI33_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];

  ABI33_0_0RCTBundleStatusProvider statusProvider = _bundleStatusProvider;
  ABI33_0_0RCTBundleStatus *bundleStatus = statusProvider == nil
    ? nil
    : statusProvider();

  for (ABI33_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(ABI33_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  // warn but don't die on unrecognized messages
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI33_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI33_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI33_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@",
      messageText);
    return;
  }
...
- (void)webSocket:(ABI33_0_0RCTSRWebSocket *)webSocket didCloseWithCode:(NSInteger)code
...
{
  if (_closed) {
    ABI33_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresopnding android code has a lot of custom config options for
  // timeouts, but it appears the iOS ABI33_0_0RCTSRWebSocket API doesn't have the same
  // implemented options.
  _webSocket = [[ABI33_0_0RCTSRWebSocket alloc] initWithURL:_url];
...
{
  if (_closed) {
    ABI33_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI33_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI33_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(
    dispatch_time(DISPATCH_TIME_NOW, RECONNECT_DELAY_MS *NSEC_PER_MSEC),
    dispatch_get_main_queue(), ^{
      ABI33_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI33_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI33_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
    ...
    withCause:(NSError *)cause
{
  ...
  if (![cause.domain isEqual:NSPOSIXErrorDomain] || cause.code != ECONNREFUSED) {
    ABI33_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
  }
...
@implementation ABI33_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI33_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
