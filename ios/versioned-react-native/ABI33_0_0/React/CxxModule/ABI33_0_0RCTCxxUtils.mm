...
#import "ABI33_0_0DispatchMessageQueueThread.h"
...
std::vector<std::unique_ptr<NativeModule>> createNativeModules(NSArray<ABI33_0_0RCTModuleData *> *modules, ABI33_0_0RCTBridge *bridge, const std::shared_ptr<Instance> &instance)
{
  ...
  for (ABI33_0_0RCTModuleData *moduleData in modules) {
    if ([moduleData.moduleClass isSubclassOfClass:[ABI33_0_0RCTCxxModule class]]) {
      nativeModules.emplace_back(std::make_unique<CxxNativeModule>(
        instance,
        [moduleData.name UTF8String],
        [moduleData] { return [(ABI33_0_0RCTCxxModule *)(moduleData.instance) createModule]; },
        std::make_shared<DispatchMessageQueueThread>(moduleData)));
    } else {
      nativeModules.emplace_back(std::make_unique<ABI33_0_0RCTNativeModule>(bridge, moduleData));
    }
  }
...
static NSError *errorWithException(const std::exception &e)
{
  ...
  if (jsError) {
    errorInfo[ABI33_0_0RCTJSRawStackTraceKey] = @(jsError->getStack().c_str());
  ...
  return [NSError errorWithDomain:ABI33_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}
...
NSError *tryAndReturnError(const std::function<void()>& func)
{
  try {
    ...
    @catch (NSException *exception) {
      ...
      return ABI33_0_0RCTErrorWithMessage(message);
    }
    @catch (id exception) {
      // This will catch any other ObjC exception, but no C++ exceptions
      return ABI33_0_0RCTErrorWithMessage(@"non-std ObjC Exception");
    }
    ...
    return ABI33_0_0RCTErrorWithMessage(@"non-std C++ exception");
  }
}
