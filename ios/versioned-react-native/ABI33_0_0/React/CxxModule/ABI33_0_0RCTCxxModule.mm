...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI33_0_0RCTAssert([ABI33_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
                @"CxxModule class name %@ does not match runtime name %s",
                ABI33_0_0RCTBridgeModuleNameForClass([self class]), _module->getName().c_str());
    }
  }
}
...
- (NSArray<id<ABI33_0_0RCTBridgeMethod>> *)methodsToExport;
{
  [self lazyInit];
  if (!_module) {
    return nil;
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI33_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
