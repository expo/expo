...
#import <ReactABI33_0_0/ABI33_0_0RCTSurfaceRootView.h>
#import <ReactABI33_0_0/ABI33_0_0RCTSurfaceTouchHandler.h>
...
@implementation ABI33_0_0RCTFabricSurface {
  // Immutable
  ABI33_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI33_0_0RCTSurfaceView *_Nullable _view;
  ABI33_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}

- (instancetype)initWithSurfacePresenter:(ABI33_0_0RCTSurfacePresenter *)surfacePresenter
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _rootTag = [ABI33_0_0RCTAllocateRootViewTag() integerValue];

    _minimumSize = CGSizeZero;
    // FIXME: Replace with `_maximumSize = CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX);`.
    _maximumSize = ABI33_0_0RCTScreenSize();

    _touchHandler = [ABI33_0_0RCTSurfaceTouchHandler new];

    _stage = ABI33_0_0RCTSurfaceStageSurfaceDidInitialize;
...
- (BOOL)start
{
  if (![self _setStage:ABI33_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (BOOL)stop
{
  if (![self _unsetStage:ABI33_0_0RCTSurfaceStageStarted]) {
    return NO;
  }
...
- (ABI33_0_0RCTSurfaceView *)view
{
  ABI33_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI33_0_0RCTSurfaceView alloc] initWithSurface:(ABI33_0_0RCTSurface *)self];
...
- (ABI33_0_0RCTSurfaceStage)stage
{
  std::lock_guard<std::mutex> lock(_mutex);
  return _stage;
}

- (BOOL)_setStage:(ABI33_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:YES];
}

- (BOOL)_unsetStage:(ABI33_0_0RCTSurfaceStage)stage
{
  return [self _setStage:stage setOrUnset:NO];
}

- (BOOL)_setStage:(ABI33_0_0RCTSurfaceStage)stage setOrUnset:(BOOL)setOrUnset
{
  ABI33_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);

    if (setOrUnset) {
      updatedStage = (ABI33_0_0RCTSurfaceStage)(_stage | stage);
    } else {
      updatedStage = (ABI33_0_0RCTSurfaceStage)(_stage & ~stage);
    }
...
- (void)_propagateStageChange:(ABI33_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI33_0_0RCTSurfaceDelegate> delegate = self.delegate;
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI33_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  if ([delegate respondsToSelector:@selector(surface:didChangeIntrinsicSize:)]) {
    [delegate surface:(ABI33_0_0RCTSurface *)(id)self didChangeIntrinsicSize:intrinsicSize];
  }
}
...
- (BOOL)synchronouslyWaitForStage:(ABI33_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
