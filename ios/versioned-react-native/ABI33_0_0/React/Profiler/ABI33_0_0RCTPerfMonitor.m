...
#import "ABI33_0_0RCTFPSGraph.h"
#import "ABI33_0_0RCTInvalidating.h"
#import "ABI33_0_0RCTJavaScriptExecutor.h"
...
static NSString *const ABI33_0_0RCTPerfMonitorCellIdentifier = @"ABI33_0_0RCTPerfMonitorCellIdentifier";
...
static vm_size_t ABI33_0_0RCTGetResidentMemorySize(void)
{
...
@interface ABI33_0_0RCTPerfMonitor : NSObject <ABI33_0_0RCTBridgeModule, ABI33_0_0RCTInvalidating, UITableViewDataSource, UITableViewDelegate>

#if __has_include("ABI33_0_0RCTDevMenu.h")
@property (nonatomic, strong, readonly) ABI33_0_0RCTDevMenuItem *devMenuItem;
#endif
...
@property (nonatomic, strong, readonly) ABI33_0_0RCTFPSGraph *jsGraph;
@property (nonatomic, strong, readonly) ABI33_0_0RCTFPSGraph *uiGraph;
@property (nonatomic, strong, readonly) UILabel *jsGraphLabel;
@property (nonatomic, strong, readonly) UILabel *uiGraphLabel;

@end

@implementation ABI33_0_0RCTPerfMonitor {
#if __has_include("ABI33_0_0RCTDevMenu.h")
  ABI33_0_0RCTDevMenuItem *_devMenuItem;
#endif
  ...
  ABI33_0_0RCTFPSGraph *_uiGraph;
  ABI33_0_0RCTFPSGraph *_jsGraph;
...
#if __has_include("ABI33_0_0RCTDevMenu.h")
- (ABI33_0_0RCTDevMenuItem *)devMenuItem
{
  if (!_devMenuItem) {
    __weak __typeof__(self) weakSelf = self;
    __weak ABI33_0_0RCTDevSettings *devSettings = self.bridge.devSettings;
    if (devSettings.isPerfMonitorShown) {
      [weakSelf show];
    }
    _devMenuItem =
    [ABI33_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
...
{
  if (!_container) {
    _container = [[UIView alloc] initWithFrame:CGRectMake(10, 25, 180, ABI33_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_memory) {
    _memory = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 44, ABI33_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_heap) {
    _heap = [[UILabel alloc] initWithFrame:CGRectMake(44, 0, 44, ABI33_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_views) {
    _views = [[UILabel alloc] initWithFrame:CGRectMake(88, 0, 44, ABI33_0_0RCTPerfMonitorBarHeight)];
...
- (ABI33_0_0RCTFPSGraph *)uiGraph
{
  if (!_uiGraph) {
    _uiGraph = [[ABI33_0_0RCTFPSGraph alloc] initWithFrame:CGRectMake(134, 14, 40, 30)
                                            color:[UIColor lightGrayColor]];
  }
  return _uiGraph;
}

- (ABI33_0_0RCTFPSGraph *)jsGraph
{
  if (!_jsGraph) {
    _jsGraph = [[ABI33_0_0RCTFPSGraph alloc] initWithFrame:CGRectMake(178, 14, 40, 30)
                                            color:[UIColor lightGrayColor]];
  }
  return _jsGraph;
}
...
- (UITableView *)metrics
{
  if (!_metrics) {
    _metrics = [[UITableView alloc] initWithFrame:CGRectMake(
      0,
      ABI33_0_0RCTPerfMonitorBarHeight,
      self.container.frame.size.width,
      self.container.frame.size.height - ABI33_0_0RCTPerfMonitorBarHeight
    )];
    ...
    [_metrics registerClass:[UITableViewCell class] forCellReuseIdentifier:ABI33_0_0RCTPerfMonitorCellIdentifier];
  }
...
- (void)show
{
  ...
  UIWindow *window = ABI33_0_0RCTSharedApplication().delegate.window;
...
- (void)redirectLogs
{
  ...
  __weak __typeof__(self) weakSelf = self;
  _queue = dispatch_queue_create("com.facebook.ReactABI33_0_0.ABI33_0_0RCTPerfMonitor", DISPATCH_QUEUE_SERIAL);
...
- (void)tap
{
  ...
  if (CGRectIsEmpty(_storedMonitorFrame)) {
    _storedMonitorFrame = CGRectMake(0, 20, self.container.window.frame.size.width, ABI33_0_0RCTPerfMonitorExpandHeight);
...
{
  ABI33_0_0RCTFPSGraph *graph = displayLink == _jsDisplayLink ? _jsGraph : _uiGraph;
...
- (void)loadPerformanceLoggerData
{
  ...
  ABI33_0_0RCTPerformanceLogger *performanceLogger = [_bridge performanceLogger];
...
{
  UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ABI33_0_0RCTPerfMonitorCellIdentifier
                                                          forIndexPath:indexPath];

  if (!cell) {
    cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                  reuseIdentifier:ABI33_0_0RCTPerfMonitorCellIdentifier];
  }
