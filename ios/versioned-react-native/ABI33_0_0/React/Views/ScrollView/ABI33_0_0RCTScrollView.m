...
@interface ABI33_0_0RCTScrollEvent : NSObject <ABI33_0_0RCTEvent>
...
@implementation ABI33_0_0RCTScrollEvent
{
...
{
  ABI33_0_0RCTAssertParam(ReactABI33_0_0Tag);

  if ((self = [super init])) {
    _eventName = [eventName copy];
    _viewTag = ReactABI33_0_0Tag;
...
- (ABI33_0_0RCTScrollEvent *)coalesceWithEvent:(ABI33_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI33_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI33_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI33_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI33_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
...
@implementation ABI33_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI33_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI33_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI33_0_0RCTUIManager JSResponder] isKindOfClass:[ABI33_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI33_0_0 scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
    ABI33_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
    ABI33_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI33_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI33_0_0RCTScrollView () <ABI33_0_0RCTUIManagerObserver>

@end

@implementation ABI33_0_0RCTScrollView
{
  ABI33_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI33_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI33_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI33_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI33_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI33_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI33_0_0LayoutDirection:layoutDirection];

  ABI33_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI33_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertReactABI33_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertReactABI33_0_0Subview:view atIndex:atIndex];
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI33_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI33_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI33_0_0RCTAssert(_contentView == nil, @"ABI33_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI33_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ReactABI33_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI33_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI33_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI33_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI33_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI33_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI33_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI33_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI33_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI33_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI33_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI33_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI33_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI33_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI33_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI33_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI33_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI33_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI33_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI33_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI33_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI33_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI33_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI33_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI33_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI33_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI33_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI33_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(ABI33_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI33_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI33_0_0RCTScrollEvent *scrollEvent = [[ABI33_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ReactABI33_0_0Tag:self.ReactABI33_0_0Tag
...
@implementation ABI33_0_0RCTEventDispatcher (ABI33_0_0RCTScrollView)
...
- (void)sendFakeScrollEvent:(NSNumber *)ReactABI33_0_0Tag
{
  ...
  ABI33_0_0RCTScrollEvent *fakeScrollEvent = [[ABI33_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI33_0_0Tag:ReactABI33_0_0Tag
