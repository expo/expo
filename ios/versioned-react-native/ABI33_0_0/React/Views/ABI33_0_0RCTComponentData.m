...
typedef void (^ABI33_0_0RCTPropBlock)(id<ABI33_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI33_0_0RCTPropBlock> ABI33_0_0RCTPropBlockDictionary;
...
static SEL selectorForType(NSString *type)
{
  ...
  return NSSelectorFromString([ABI33_0_0RCTParseType(&input) stringByAppendingString:@":"]);
}


@implementation ABI33_0_0RCTComponentData
{
  id<ABI33_0_0RCTComponent> _defaultView; // Only needed for ABI33_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI33_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI33_0_0RCTPropBlockDictionary *_shadowPropBlocks;
...
- (ABI33_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI33_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI33_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI33_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI33_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI33_0_0RCTPropBlock createEventSetter(NSString *propName, SEL setter, ABI33_0_0RCTBridge *bridge)
{
  __weak ABI33_0_0RCTBridge *weakBridge = bridge;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI33_0_0RCTConvert BOOL:json]) {
      __weak id<ABI33_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI33_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        NSMutableDictionary *mutableEvent = [NSMutableDictionary dictionaryWithDictionary:event];
        mutableEvent[@"target"] = strongTarget.ReactABI33_0_0Tag;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [weakBridge.eventDispatcher sendInputEventWithName:ABI33_0_0RCTNormalizeInputEventName(propName) body:mutableEvent];
...
static ABI33_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
  NSInvocation *typeInvocation = [NSInvocation invocationWithMethodSignature:typeSignature];
  typeInvocation.selector = type;
  typeInvocation.target = [ABI33_0_0RCTConvert class];
...
- (ABI33_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI33_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI33_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI33_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI33_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock = createEventSetter(name, setter, _bridge);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI33_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI33_0_0RCTLogError(@"No +[ABI33_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI33_0_0RCTComponent> view, __unused id json){};
      }
      switch (typeSignature.methodReturnType[0]) {

#define ABI33_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI33_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI33_0_0RCT_CASE(_C_SEL, SEL)
        ABI33_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI33_0_0RCT_CASE(_C_CHR, char)
        ABI33_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI33_0_0RCT_CASE(_C_SHT, short)
        ABI33_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI33_0_0RCT_CASE(_C_INT, int)
        ABI33_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI33_0_0RCT_CASE(_C_LNG, long)
        ABI33_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI33_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI33_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI33_0_0RCT_CASE(_C_FLT, float)
        ABI33_0_0RCT_CASE(_C_DBL, double)
        ABI33_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI33_0_0RCT_CASE(_C_PTR, void *)
        ABI33_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI33_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI33_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI33_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI33_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI33_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI33_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ",
                             name, weakSelf.name, view.ReactABI33_0_0Tag];
      ABI33_0_0RCTPerformBlockWithLogPrefix(^{
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI33_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI33_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI33_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
      ABI33_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                  "to '%@'", name, _name, propTypes[name], type);
    }

    if ([type isEqualToString:@"ABI33_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI33_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI33_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI33_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
  ...
  for (NSString *event in bubblingEvents) {
    if ([directEvents containsObject:event]) {
      ABI33_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
...
{
  // Hackety hack, this partially re-implements ABI33_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI33_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI33_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI33_0_0EX_REMOVE_VERSION(name);
  ...
  ABI33_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
