...
*/
static NSRegularExpression *ABI33_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI33_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI33_0_0RCTNullIfNil(self.methodName),
     @"file": ABI33_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI33_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
{
  return [[self alloc] initWithMethodName:ABI33_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI33_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI33_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}
...
+ (NSArray<ABI33_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI33_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI33_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI33_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
