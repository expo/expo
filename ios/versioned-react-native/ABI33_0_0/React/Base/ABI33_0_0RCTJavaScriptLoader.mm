...
#import "ABI33_0_0RCTJavaScriptLoader.h"
...
NSString *const ABI33_0_0RCTJavaScriptLoaderErrorDomain = @"ABI33_0_0RCTJavaScriptLoaderErrorDomain";

static const int32_t JSNoBytecodeFileFormatVersion = -1;

@interface ABI33_0_0RCTSource()
{
@public
  NSURL *_url;
  NSData *_data;
...
@implementation ABI33_0_0RCTSource

static ABI33_0_0RCTSource *ABI33_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  ABI33_0_0RCTSource *source = [ABI33_0_0RCTSource new];
  source->_url = url;
  source->_data = data;
  source->_length = length;
  source->_filesChangedCount = ABI33_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI33_0_0RCTLoadingProgress
...
@implementation ABI33_0_0RCTJavaScriptLoader

ABI33_0_0RCT_NOT_IMPLEMENTED(- (instancetype)init)

+ (void)loadBundleAtURL:(NSURL *)scriptURL onProgress:(ABI33_0_0RCTSourceLoadProgressBlock)onProgress onComplete:(ABI33_0_0RCTSourceLoadBlock)onComplete
{
  ...
  NSData *data = [self attemptSynchronousLoadOfBundleAtURL:scriptURL
                                          runtimeBCVersion:JSNoBytecodeFileFormatVersion
                                              sourceLength:&sourceLength
                                                     error:&error];
  if (data) {
    onComplete(nil, ABI33_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError =
  [error.domain isEqualToString:ABI33_0_0RCTJavaScriptLoaderErrorDomain]
  && error.code == ABI33_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI33_0_0RCTJavaScriptLoaderErrorNoScriptURL
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI33_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI33_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI33_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                               userInfo:@{NSLocalizedDescriptionKey:
  ...
  case facebook::ReactABI33_0_0::ScriptTag::BCBundle:
    if (runtimeBCVersion == JSNoBytecodeFileFormatVersion || runtimeBCVersion < 0) {
      if (error) {
        *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI33_0_0RCTJavaScriptLoaderErrorBCNotSupported
                                 userInfo:@{NSLocalizedDescriptionKey:
      ...
      if (error) {
        ...
        *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                     code:ABI33_0_0RCTJavaScriptLoaderErrorBCVersion
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI33_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                               userInfo:@{NSLocalizedDescriptionKey:
...
static void parseHeaders(NSDictionary *headers, ABI33_0_0RCTSource *source) {
  source->_filesChangedCount = [headers[@"X-Metro-Files-Changed-Count"] integerValue];
}

static void attemptAsynchronousLoadOfBundleAtURL(NSURL *scriptURL, ABI33_0_0RCTSourceLoadProgressBlock onProgress, ABI33_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path
                                              options:NSDataReadingMappedIfSafe
                                                error:&error];
      onComplete(error, ABI33_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI33_0_0RCTMultipartDataTask *task = [[ABI33_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
    if (!done) {
      if (onProgress) {
    ...
    if (error) {
      if ([error.domain isEqualToString:NSURLErrorDomain]) {
        error = [NSError errorWithDomain:ABI33_0_0RCTJavaScriptLoaderErrorDomain
                                    code:ABI33_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                                userInfo:
                 @{
                   NSLocalizedDescriptionKey:
                     [@"Could not connect to development server.\n\n"
                      "Ensure the following:\n"
                      "- Node server is running and available on the same network - run 'npm start' from ReactABI33_0_0-native root\n"
    ...
    ABI33_0_0RCTSource *source = ABI33_0_0RCTSourceCreate(scriptURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI33_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI33_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI33_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI33_0_0RCTLoadingProgress *progress = [ABI33_0_0RCTLoadingProgress new];
...
static ABI33_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI33_0_0RCTLoadingProgress *progress = [ABI33_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI33_0_0RCTJSONParse(rawText, nil);
