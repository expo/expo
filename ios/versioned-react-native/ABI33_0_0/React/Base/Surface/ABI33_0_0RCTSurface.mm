...
#import "ABI33_0_0RCTSurfaceView+Internal.h"
...
#import "ABI33_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI33_0_0RCTSurface () <ABI33_0_0RCTSurfaceRootShadowViewDelegate, ABI33_0_0RCTUIManagerObserver>
@end

@implementation ABI33_0_0RCTSurface {
  // Immutable
  ...
  ABI33_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI33_0_0RCTSurfaceView *_Nullable _view;
  ABI33_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI33_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI33_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI33_0_0RCTAllocateRootViewTag();
    ...
    if (!bridge.loading) {
      _stage = _stage | ABI33_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI33_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI33_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI33_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI33_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI33_0_0RCTSurfaceRootView *rootView =
    (ABI33_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI33_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI33_0_0RCTAssert([rootView isKindOfClass:[ABI33_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI33_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI33_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI33_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI33_0_0RCTSurfaceStage)stage
{
...
- (void)_run
{
  ABI33_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI33_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI33_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI33_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountReactABI33_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI33_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI33_0_0RCTIsMainQueue() ? ABI33_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI33_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI33_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI33_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI33_0_0Tag:self->_rootViewTag];
    ABI33_0_0RCTAssert([rootShadowView isKindOfClass:[ABI33_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI33_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI33_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI33_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI33_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI33_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI33_0_0Tag:self->_rootViewTag];
...
{
  if (ABI33_0_0RCTIsUIManagerQueue()) {
    ABI33_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI33_0_0RCTIsMainQueue() && (stage & ABI33_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI33_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI33_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI33_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI33_0_0RCTAssert(NO, @"Only waiting for `ABI33_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI33_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI33_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI33_0_0RCTIsMainQueue() && (stage & ABI33_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI33_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI33_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI33_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
...
{
  [self _setStage:ABI33_0_0RCTSurfaceStageSurfaceDidInitialRendering];
...
{
  [self _setStage:ABI33_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI33_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
{
  if (self.stage & ABI33_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI33_0_0RCTSurfaceStageSurfaceDidInitialMounting];
