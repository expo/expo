...
const NSInteger ABI33_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI33_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI33_0_0RCTGetEventID(id<ABI33_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI33_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI33_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI33_0_0RCTEventDispatcherObserver>> *_observers;
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI33_0_0RCT_DEBUG) {
    ABI33_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI33_0_0 tag");
  }
...
- (void)sendTextEventWithType:(ABI33_0_0RCTTextEventType)type
...
- (void)sendEvent:(id<ABI33_0_0RCTEvent>)event
{
  [_observersLock lock];

  for (id<ABI33_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI33_0_0RCTGetEventID(event);

  id<ABI33_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI33_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)addDispatchObserver:(id<ABI33_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI33_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
@implementation ABI33_0_0RCTBridge (ABI33_0_0RCTEventDispatcher)

- (ABI33_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI33_0_0RCTEventDispatcher class]];
}
