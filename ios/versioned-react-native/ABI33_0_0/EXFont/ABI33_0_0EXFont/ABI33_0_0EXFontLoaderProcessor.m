...
@implementation ABI33_0_0EXFontLoaderProcessor
         ...
         scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  ABI33_0_0EXFont *exFont = nil;
  
  // Did we get a new family, and if so, is it associated with an ABI33_0_0EXFont?
  if ([family hasPrefix:exponentPrefix]) {
    NSString *suffix = [family substringFromIndex:exponentPrefix.length];
    exFont = [ABI33_0_0EXFontManager getFontForName:suffix];
  }
  
  // Did the passed-in UIFont come from an ABI33_0_0EXFont?
  if (!exFont && uiFont) {
    exFont = objc_getAssociatedObject(uiFont, ABI33_0_0EXFontAssocKey);
  }
  
  // If it's an ABI33_0_0EXFont, generate the corresponding UIFont, else fallback to ReactABI33_0_0 Native's built-in method
  if (exFont) {
