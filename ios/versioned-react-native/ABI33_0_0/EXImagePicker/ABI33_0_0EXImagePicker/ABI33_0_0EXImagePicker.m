#import <ABI33_0_0EXImagePicker/ABI33_0_0EXImagePicker.h>
...
const CGFloat ABI33_0_0EXDefaultImageQuality = 0.2;

@interface ABI33_0_0EXImagePicker ()
...
@property (nonatomic, weak) id<ABI33_0_0UMPermissionsInterface> permissionsModule;
@property (nonatomic, assign) BOOL shouldRestoreStatusBarVisibility;

@end

@implementation ABI33_0_0EXImagePicker

ABI33_0_0UM_EXPORT_MODULE(ExponentImagePicker);
...
- (void)setModuleRegistry:(ABI33_0_0UMModuleRegistry *)moduleRegistry
{
  ...
  _permissionsModule = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI33_0_0UMPermissionsInterface)];
}

ABI33_0_0UM_EXPORT_METHOD_AS(launchCameraAsync, launchCameraAsync:(NSDictionary *)options
                  ...
                  rejecter:(ABI33_0_0UMPromiseRejectBlock)reject)
{
  ...
  self.reject = reject;
  [self launchImagePicker:ABI33_0_0EXImagePickerTargetCamera options:options];
}

ABI33_0_0UM_EXPORT_METHOD_AS(launchImageLibraryAsync, launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI33_0_0UMPromiseResolveBlock)resolve
                  rejecter:(ABI33_0_0UMPromiseRejectBlock)reject)
{
  ...
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI33_0_0EXImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI33_0_0EXImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI33_0_0EXImagePickerTarget)target
{
  self.picker = [[UIImagePickerController alloc] init];

  if (target == ABI33_0_0EXImagePickerTargetCamera) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    id<ABI33_0_0UMUtilitiesInterface> utils = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI33_0_0UMUtilitiesInterface)];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
  dispatch_block_t dismissCompletionBlock = ^{
    ...
    id<ABI33_0_0UMFileSystemInterface> fileSystem = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI33_0_0UMFileSystemInterface)];
    if (!fileSystem) {
          ...
          completionHandler:(void (^)(void))completionHandler
{
  ...
  NSData *data = UIImageJPEGRepresentation(image, quality == nil ? ABI33_0_0EXDefaultImageQuality : [quality floatValue]);
  ...
  if ([[self.options objectForKey:@"exif"] boolValue]) {
    ...
    if (metadata) {
      ...
      if (assets.count > 0) {
        ...
        ABI33_0_0UMLogInfo(@"Could not fetch metadata for image %@", [imageURL absoluteString]);
        completionHandler();
      }
    }
  } else {
...
- (void)handleVideoWithInfo:(NSDictionary * _Nonnull)info saveAt:(NSString *)directory updateResponse:(NSMutableDictionary *)response
{
  ...
  if (assets.count > 0) {
    ...
    ABI33_0_0UMLogInfo(@"Could not fetch metadata for video %@", [videoURL absoluteString]);
  }
