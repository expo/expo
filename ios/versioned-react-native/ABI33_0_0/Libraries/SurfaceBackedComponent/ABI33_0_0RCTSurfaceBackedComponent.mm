...
#import "ABI33_0_0RCTSurfaceBackedComponent.h"
...
@implementation ABI33_0_0RCTSurfaceBackedComponent

+ (id)initialState
{
  return [ABI33_0_0RCTSurfaceBackedComponentState new];
}
                      ...
                      options:(ABI33_0_0RCTSurfaceHostingComponentOptions)options
{
  ...
  ABI33_0_0RCTSurfaceBackedComponentState *state = scope.state();

  if (state.surface == nil || ![state.surface.moduleName isEqualToString:moduleName]) {
    ABI33_0_0RCTSurface *surface =
      [[ABI33_0_0RCTSurface alloc] initWithBridge:bridge
                              moduleName:moduleName
                       initialProperties:properties];

    [surface start];

    state = [ABI33_0_0RCTSurfaceBackedComponentState newWithSurface:surface];
  ...
  ABI33_0_0RCTSurfaceHostingComponent *surfaceHostingComponent =
    [ABI33_0_0RCTSurfaceHostingComponent newWithSurface:state.surface
                                       options:options];

  CKComponent *component;
  if (options.activityIndicatorComponentFactory == nil || ABI33_0_0RCTSurfaceStageIsRunning(state.surface.stage)) {
