...
#import <ReactABI33_0_0/ABI33_0_0RCTNetworking.h>
#import <ReactABI33_0_0/ABI33_0_0RCTUtils.h>

#import "ABI33_0_0RCTImageCache.h"
#import "ABI33_0_0RCTImageUtils.h"

static NSInteger ABI33_0_0RCTImageBytesForImage(UIImage *image)
{
  CAKeyframeAnimation *keyFrameAnimation = [image ReactABI33_0_0KeyframeAnimation];
...
@implementation UIImage (ReactABI33_0_0)

- (CAKeyframeAnimation *)ReactABI33_0_0KeyframeAnimation
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setReactABI33_0_0KeyframeAnimation:(CAKeyframeAnimation *)ReactABI33_0_0KeyframeAnimation
{
    objc_setAssociatedObject(self, @selector(ReactABI33_0_0KeyframeAnimation), ReactABI33_0_0KeyframeAnimation, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (NSInteger)ReactABI33_0_0DecodedImageBytes
{
  NSNumber *imageBytes = objc_getAssociatedObject(self, _cmd);
  if (!imageBytes) {
    imageBytes = @(ABI33_0_0RCTImageBytesForImage(self));
  }
  return [imageBytes integerValue];
}

- (void)setReactABI33_0_0DecodedImageBytes:(NSInteger)bytes
{
  objc_setAssociatedObject(self, @selector(ReactABI33_0_0DecodedImageBytes), @(bytes), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

@end

@implementation ABI33_0_0RCTImageLoader
{
    NSArray<id<ABI33_0_0RCTImageURLLoader>> *_loaders;
    NSArray<id<ABI33_0_0RCTImageDataDecoder>> *_decoders;
    NSOperationQueue *_imageDecodeQueue;
    dispatch_queue_t _URLRequestQueue;
    id<ABI33_0_0RCTImageCache> _imageCache;
    NSMutableArray *_pendingTasks;
    NSInteger _activeTasks;
    NSMutableArray *_pendingDecodes;
    NSInteger _scheduledDecodes;
    NSUInteger _activeBytes;
  __weak id<ABI33_0_0RCTImageRedirectProtocol> _redirectDelegate;
}
...
- (instancetype)initWithRedirectDelegate:(id<ABI33_0_0RCTImageRedirectProtocol>)redirectDelegate
{
    if (self = [super init]) {
...
- (void)setUp
{
    ...
    _URLRequestQueue = dispatch_queue_create("com.facebook.ReactABI33_0_0.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (id<ABI33_0_0RCTImageCache>)imageCache
{
    if (!_imageCache) {
        //set up with default cache
        _imageCache = [ABI33_0_0RCTImageCache new];
    }
    return _imageCache;
}

- (void)setImageCache:(id<ABI33_0_0RCTImageCache>)cache
{
    if (_imageCache) {
        ABI33_0_0RCTLogWarn(@"ABI33_0_0RCTImageCache was already set and has now been overriden.");
    }
    _imageCache = cache;
}

- (id<ABI33_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
    ...
    if (!_loaders) {
        ...
        ABI33_0_0RCTAssert(_bridge, @"Bridge not set");
        _loaders = [[_bridge modulesConformingToProtocol:@protocol(ABI33_0_0RCTImageURLLoader)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI33_0_0RCTImageURLLoader> a, id<ABI33_0_0RCTImageURLLoader> b) {
    ...
    if (ABI33_0_0RCT_DEBUG) {
        ...
        id<ABI33_0_0RCTImageURLLoader> previousLoader = nil;
        for (id<ABI33_0_0RCTImageURLLoader> loader in _loaders) {
            ...
            if ([loader canLoadImageURL:URL]) {
                if (previousLoader) {
                    if (priority == previousPriority) {
                        ABI33_0_0RCTLogError(@"The ABI33_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI33_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
    if (!_maxConcurrentLoadingTasks) {
        [self setUp];
    }
    ...
    if (!_decoders) {
        ...
        ABI33_0_0RCTAssert(_bridge, @"Bridge not set");
        _decoders = [[_bridge modulesConformingToProtocol:@protocol(ABI33_0_0RCTImageDataDecoder)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI33_0_0RCTImageDataDecoder> a, id<ABI33_0_0RCTImageDataDecoder> b) {
    ...
    if (ABI33_0_0RCT_DEBUG) {
        // Check for handler conflicts
        float previousPriority = 0;
        id<ABI33_0_0RCTImageDataDecoder> previousDecoder = nil;
        for (id<ABI33_0_0RCTImageDataDecoder> decoder in _decoders) {
            ...
            if ([decoder canDecodeImageData:data]) {
                if (previousDecoder) {
                    if (priority == previousPriority) {
                        ABI33_0_0RCTLogError(@"The ABI33_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI33_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI33_0_0RCTResizeMode resizeMode)
{
    ...
    CAKeyframeAnimation *animation = image.ReactABI33_0_0KeyframeAnimation;
    CGRect targetSize = ABI33_0_0RCTTargetRect(image.size, size, scale, resizeMode);
    CGAffineTransform transform = ABI33_0_0RCTTransformFromTargetRect(image.size, targetSize);
    image = ABI33_0_0RCTTransformImage(image, size, scale, transform);
    image.ReactABI33_0_0KeyframeAnimation = animation;
    return image;
}

- (ABI33_0_0RCTImageLoaderCancellationBlock) loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                  callback:(ABI33_0_0RCTImageLoaderCompletionBlock)callback
{
    return [self loadImageWithURLRequest:imageURLRequest
                                    size:CGSizeZero
                                   scale:1
                                 clipped:YES
                              resizeMode:ABI33_0_0RCTResizeModeStretch
                           progressBlock:nil
...
- (void)dequeueTasks
{
    dispatch_async(_URLRequestQueue, ^{
        ...
        for (ABI33_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
            switch (task.status) {
                case ABI33_0_0RCTNetworkTaskFinished:
                    if (!tasksToRemove) {
                    ...
                    self->_activeTasks--;
                    break;
                case ABI33_0_0RCTNetworkTaskPending:
                    break;
                case ABI33_0_0RCTNetworkTaskInProgress:
                    // Check task isn't "stuck"
                    if (task.requestToken == nil) {
                        ABI33_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
                        if (!tasksToRemove) {
                            tasksToRemove = [NSMutableArray new];
                        }
                        [tasksToRemove addObject:task];
        ...
        // Start queued tasks
        for (ABI33_0_0RCTNetworkTask *task in self->_pendingTasks) {
            if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
                break;
            }
            if (task.status == ABI33_0_0RCTNetworkTaskPending) {
                [task start];
                self->_activeTasks++;
            }
        }
 ...
 */
- (ABI33_0_0RCTImageLoaderCancellationBlock)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                             size:(CGSize)size
                                                            scale:(CGFloat)scale
                                                       resizeMode:(ABI33_0_0RCTResizeMode)resizeMode
                                                    progressBlock:(ABI33_0_0RCTImageLoaderProgressBlock)progressHandler
                                                 partialLoadBlock:(ABI33_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
                                                  completionBlock:(void (^)(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response))completionBlock
{
    {
        NSMutableURLRequest *mutableRequest = [request mutableCopy];
        [NSURLProtocol setProperty:@"ABI33_0_0RCTImageLoader"
                            forKey:@"trackingName"
    ...
    id<ABI33_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
    ...
    void (^completionHandler)(NSError *, id, NSURLResponse *) = ^(NSError *error, id imageOrData, NSURLResponse *response) {
        ...
        if (ABI33_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
    ...
    __weak ABI33_0_0RCTImageLoader *weakSelf = self;
    dispatch_async(_URLRequestQueue, ^{
...
- (ABI33_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI33_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSURLResponse *response))completionHandler
{
    // Check if networking module is available
    if (ABI33_0_0RCT_DEBUG && ![_bridge respondsToSelector:@selector(networking)]) {
        ABI33_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                    " import the ABI33_0_0RCTNetwork library in order to load images.",
                    request.URL.absoluteString);
        return NULL;
    }

    ABI33_0_0RCTNetworking *networking = [_bridge networking];

    // Check if networking module can load image
    if (ABI33_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
        ABI33_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
        return NULL;
    }

    // Use networking module to load image
    ABI33_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
        // Check for system errors
        if (error) {
            completionHandler(error, nil, response);
            return;
        } else if (!response) {
            completionHandler(ABI33_0_0RCTErrorWithMessage(@"Response metadata error"), nil, response);
            return;
        } else if (!data) {
            completionHandler(ABI33_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, response);
            return;
        }
    ...
    __block ABI33_0_0RCTNetworkTask *task =
                       ...
                       completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                           ...
                           if (error || !response || !data) {
                               ...
                               if (error) {
                                   ...
                                   someError = ABI33_0_0RCTErrorWithMessage(@"Response metadata error");
                               } else {
                                   someError = ABI33_0_0RCTErrorWithMessage(@"Unknown image download error");
                               }
...
- (ABI33_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                ...
                                                resizeMode:(ABI33_0_0RCTResizeMode)resizeMode
                                             progressBlock:(ABI33_0_0RCTImageLoaderProgressBlock)progressBlock
                                          partialLoadBlock:(ABI33_0_0RCTImageLoaderPartialLoadBlock)partialLoadBlock
                                           completionBlock:(ABI33_0_0RCTImageLoaderCompletionBlock)completionBlock
{
    __block atomic_bool cancelled = ATOMIC_VAR_INIT(NO);
    // TODO: Protect this variable shared between threads.
    __block dispatch_block_t cancelLoad = nil;
    ...
    void (^completionHandler)(NSError *, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response) {
        ...
        ABI33_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
...
- (ABI33_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI33_0_0RCTResizeMode)resizeMode
...
{
    if (data.length == 0) {
        completionBlock(ABI33_0_0RCTErrorWithMessage(@"No image data"), nil);
        return ^{};
    }
    ...
    void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
        if (ABI33_0_0RCTIsMainQueue()) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                if (!atomic_load(&cancelled)) {
                    completionBlock(error, clipped ? ABI33_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
                }
            });
        } else if (!atomic_load(&cancelled)) {
            completionBlock(error, clipped ? ABI33_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
    ...
    id<ABI33_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
    if (imageDecoder) {
        ...
        dispatch_block_t decodeBlock = ^{
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                if (!atomic_load(&cancelled)) {
                    ...
                    UIImage *image = ABI33_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI33_0_0RCT_DEV
                    CGSize imagePixelSize = ABI33_0_0RCTSizeInPixels(image.size, image.scale);
                    CGSize screenPixelSize = ABI33_0_0RCTSizeInPixels(ABI33_0_0RCTScreenSize(), ABI33_0_0RCTScreenScale());
                    if (imagePixelSize.width * imagePixelSize.height >
                        screenPixelSize.width * screenPixelSize.height) {
                        ABI33_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
                    ...
                    if (image) {
                        ...
                        NSError *finalError = ABI33_0_0RCTErrorWithMessage(errorMessage);
...
- (ABI33_0_0RCTImageLoaderCancellationBlock)getImageSizeForURLRequest:(NSURLRequest *)imageURLRequest
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
    void (^completion)(NSError *, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response) {
        ...
        if ([imageOrData isKindOfClass:[NSData class]]) {
            NSDictionary *meta = ABI33_0_0RCTGetImageMetadata(imageOrData);
...
- (NSDictionary *)getImageCacheStatus:(NSArray *)requests
{
  ...
  for (id request in requests) {
    NSURLRequest *urlRequest = [ABI33_0_0RCTConvert NSURLRequest:request];
    if (urlRequest) {
...
#pragma mark - ABI33_0_0RCTURLRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  // Even better may be to implement this on the ABI33_0_0RCTImageURLLoader that would try to load it,
  // but we'd have to run the logic both in ABI33_0_0RCTPhotoLibraryImageLoader and
  // ABI33_0_0RCTAssetsLibraryRequestHandler. Once we drop iOS7 though, we'd drop
  // ABI33_0_0RCTAssetsLibraryRequestHandler and can move it there.
  ...
  for (id<ABI33_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI33_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI33_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI33_0_0RCTURLRequestHandler)] &&
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI33_0_0RCTURLRequestDelegate>)delegate
{
    __block ABI33_0_0RCTImageLoaderCancellationBlock requestToken;
    requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
        ...
        if (ABI33_0_0RCTImageHasAlpha(image.CGImage)) {
            mimeType = @"image/png";
...
{
    if (requestToken) {
        ((ABI33_0_0RCTImageLoaderCancellationBlock)requestToken)();
    }
}

@end

@implementation ABI33_0_0RCTBridge (ABI33_0_0RCTImageLoader)

- (ABI33_0_0RCTImageLoader *)imageLoader
{
    return [self moduleForClass:[ABI33_0_0RCTImageLoader class]];
}
