...
@implementation ABI33_0_0EXCalendarConverter
...
+ (NSDictionary *)serializeSource:(EKSource *)source
{
  ...
  return @{
       ...
       @"type": [ABI33_0_0EXCalendarConverter _sourceType:source.sourceType],
       @"name": source.title
       };
}
...
+ (NSMutableDictionary *)_serializeCalendar:(EKCalendar *)calendar
{
  ...
  serializedCalendar[@"id"] = ABI33_0_0UMNullIfNil(calendar.calendarIdentifier);
  serializedCalendar[@"title"] = ABI33_0_0UMNullIfNil(calendar.title);
  serializedCalendar[@"source"] = ABI33_0_0UMNullIfNil([ABI33_0_0EXCalendarConverter serializeSource:calendar.source]);
  serializedCalendar[@"entityType"] = [ABI33_0_0EXCalendarConverter _entityType:calendar.allowedEntityTypes];
  serializedCalendar[@"color"] = calendar.CGColor ? [ABI33_0_0UMUtilities hexStringWithCGColor:calendar.CGColor] : [NSNull null];
  serializedCalendar[@"type"] = [ABI33_0_0EXCalendarConverter _calendarType:calendar.type];
  serializedCalendar[@"allowsModifications"] = @(calendar.allowsContentModifications);
  serializedCalendar[@"allowedAvailabilities"] = [ABI33_0_0EXCalendarConverter _calendarSupportedAvailabilitiesFromMask:calendar.supportedEventAvailabilities];
...
+ (NSArray *)serializeCalendars:(NSArray *)calendars
{
  ...
  for (EKCalendar *calendar in calendars) {
    [serializedCalendars addObject:[ABI33_0_0EXCalendarConverter _serializeCalendar:calendar]];
  }
...
+ (NSMutableDictionary *)_serializeCalendarItem:(EKCalendarItem *)item withDateFormatter:(NSDateFormatter *)dateFormatter
{
  ...
  serializedItem[@"timeZone"] = ABI33_0_0UMNullIfNil(item.timeZone.name);
  serializedItem[@"url"] = ABI33_0_0UMNullIfNil(item.URL.absoluteString.stringByRemovingPercentEncoding);
  serializedItem[@"notes"] = ABI33_0_0UMNullIfNil(item.notes);
  serializedItem[@"alarms"] = [ABI33_0_0EXCalendarConverter _serializeAlarms:item.alarms withDateFormatter:dateFormatter];

  if (item.hasRecurrenceRules) {
    EKRecurrenceRule *rule = [item.recurrenceRules objectAtIndex:0];
    NSString *frequencyType = [ABI33_0_0EXCalendarConverter _recurrenceFrequency:[rule frequency]];
...
+ (NSArray *)serializeCalendarEvents:(NSArray *)calendarEvents
{
  ...
  for (EKEvent *event in calendarEvents) {
    ...
    [serializedCalendarEvents addObject:[ABI33_0_0EXCalendarConverter serializeCalendarEvent:event]];
  }
...
+ (NSDictionary *)serializeCalendarEvent:(EKEvent *)event
{
  ...
  NSMutableDictionary *formedCalendarEvent = [ABI33_0_0EXCalendarConverter _serializeCalendarItem:event withDateFormatter:dateFormatter];
  ...
  formedCalendarEvent[@"availability"] = [ABI33_0_0EXCalendarConverter _eventAvailability:event.availability];
  formedCalendarEvent[@"status"] = [ABI33_0_0EXCalendarConverter _eventStatus:event.status];
  formedCalendarEvent[@"organizer"] = ABI33_0_0UMNullIfNil([ABI33_0_0EXCalendarConverter _serializeAttendee:event.organizer]);
...
+ (NSArray *)serializeReminders:(NSArray<EKReminder *> *)reminders
{
  ...
  for (EKReminder *reminder in reminders) {
    [serializedReminders addObject:[ABI33_0_0EXCalendarConverter serializeReminder:reminder]];
  }
...
+ (NSDictionary *)serializeReminder:(EKReminder *)reminder
{
  ...
  NSMutableDictionary *formedReminder = [ABI33_0_0EXCalendarConverter _serializeCalendarItem:reminder withDateFormatter:dateFormatter];
...
+ (NSArray *)serializeAttendees:(NSArray<EKParticipant *> *)attendees
{
  ...
  for (EKParticipant *attendee in attendees) {
    [serializedAttendees addObject:[ABI33_0_0EXCalendarConverter _serializeAttendee:attendee]];
  }
...
+ (NSDictionary *)_serializeAttendee:(EKParticipant *)attendee
{
  ...
  if (attendee.participantRole) {
    formedAttendee[@"role"] = [ABI33_0_0EXCalendarConverter _participantRole:attendee.participantRole];
  }

  if (attendee.participantStatus) {
    formedAttendee[@"status"] = [ABI33_0_0EXCalendarConverter _participantStatus:attendee.participantStatus];
  }

  if (attendee.participantType) {
    formedAttendee[@"type"] = [ABI33_0_0EXCalendarConverter _participantType:attendee.participantType];
  }
