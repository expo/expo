...
@interface ABI33_0_0EXGLContext ()

@property (nonatomic, strong) dispatch_queue_t glQueue;
@property (nonatomic, weak) ABI33_0_0UMModuleRegistry *moduleRegistry;
@property (nonatomic, weak) ABI33_0_0EXGLObjectManager *objectManager;

@end

@implementation ABI33_0_0EXGLContext
...
- (instancetype)initWithDelegate:(id<ABI33_0_0EXGLContextDelegate>)delegate andModuleRegistry:(nonnull ABI33_0_0UMModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    ...
    _objectManager = (ABI33_0_0EXGLObjectManager *)[_moduleRegistry getExportedModuleOfClass:[ABI33_0_0EXGLObjectManager class]];
...
- (void)initialize:(void(^)(BOOL))callback
{
  ...
  id<ABI33_0_0UMJavaScriptContextProvider> jsContextProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI33_0_0UMJavaScriptContextProvider)];
  ...
  if (jsRuntimePtr) {
    ...
    [uiManager dispatchOnClientThread:^{
      ABI33_0_0EXGLContext *self = weakSelf;
      id<ABI33_0_0UMUIManager> uiManager = weakUIManager;
    ...
    BLOCK_SAFE_RUN(callback, NO);
    ABI33_0_0UMLogWarn(@"ABI33_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI33_0_0-native/docs/debugging.html)? ABI33_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI33_0_0EXGL.");
  }
}
  ...
  [self runAsync:^{
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI33_0_0UMErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI33_0_0UMErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = [ABI33_0_0UMUtilities screenScale];
