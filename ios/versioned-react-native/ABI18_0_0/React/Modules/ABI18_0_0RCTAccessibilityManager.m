...
NSString *const ABI18_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI18_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI18_0_0RCTAccessibilityManager ()
...
@implementation ABI18_0_0RCTAccessibilityManager
  ...
  if ((self = [super init])) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didReceiveNewContentSizeCategory:)
                                               ...
                                               object:ABI18_0_0RCTSharedApplication()];
    ...
    self.contentSizeCategory = ABI18_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI18_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI18_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI18_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI18_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI18_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI18_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI18_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI18_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI18_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI18_0_0RCTBridge (ABI18_0_0RCTAccessibilityManager)

- (ABI18_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI18_0_0RCTAccessibilityManager class]];
}
