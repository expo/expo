...
#import "ABI18_0_0RCTJSCExecutor.h"
...
#import <ReactABI18_0_0/ABI18_0_0RCTJSCErrorHandling.h>
...
#if (ABI18_0_0RCT_PROFILE || ABI18_0_0RCT_DEV) && __has_include("ABI18_0_0RCTDevMenu.h")
#import "ABI18_0_0RCTDevMenu.h"
#endif

NSString *const ABI18_0_0RCTJSCThreadName = @"com.facebook.ReactABI18_0_0.JavaScript";
NSString *const ABI18_0_0RCTJavaScriptContextCreatedNotification = @"ABI18_0_0RCTJavaScriptContextCreatedNotification";
...
struct TaggedScript {
  const facebook::ReactABI18_0_0::ScriptTag tag;
  const NSData *script;
};

#if ABI18_0_0RCT_PROFILE
@interface ABI18_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI18_0_0RCTCookieMap @end
#endif

struct ABI18_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
};

@interface ABI18_0_0RCTJavaScriptContext : NSObject <ABI18_0_0RCTInvalidating>
...
@implementation ABI18_0_0RCTJavaScriptContext
{
  ABI18_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    _context = context;
    _context.name = @"ABI18_0_0RCTJSContext";
    _javaScriptThread = javaScriptThread;

    /**
     * Explicitly introduce a retain cycle here - The ABI18_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI18_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI18_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI18_0_0RCTPerformanceLogger *_performanceLogger;
  ABI18_0_0RCTJavaScriptContext *_context;
...
#if ABI18_0_0RCT_DEV
static void ABI18_0_0RCTInstallJSCProfiler(ABI18_0_0RCTBridge *bridge, JSContextRef context)
{
#if __has_include("ABI18_0_0RCTDevMenu.h")
  __weak ABI18_0_0RCTBridge *weakBridge = bridge;
  __weak ABI18_0_0RCTDevSettings *devSettings = bridge.devSettings;
  if (ABI18_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI18_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
      ...
      if (shouldStart != ABI18_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI18_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI18_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI18_0_0RCTProfileSendResult(weakBridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI18_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI18_0_0RCTJSCThreadName;
...
{
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI18_0_0RCTJSCExecutor init]", nil);
...
- (instancetype)initWithJSContextData:(const ABI18_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    _useCustomJSCLibrary = data.useCustomJSCLibrary;
    _valid = YES;
    _javaScriptThread = data.javaScriptThread;
    _context = [[ABI18_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
                                         ...
                                         sourceURL:(NSURL *)sourceURL
{
  ...
  if (taggedScript.tag == facebook::ReactABI18_0_0::ScriptTag::RAMBundle) {
...
- (ABI18_0_0RCTJavaScriptContext *)context
{
  if (!self.isValid) {
    return nil;
  }
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI18_0_0RCTProfileDidStartProfiling, ABI18_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_context) {
      ...
      if (self->_useCustomJSCLibrary) {
        JSC_configureJSCForIOS(true, ABI18_0_0RCTJSONStringify(@{
      ...
      self->_context = [[ABI18_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI18_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];

      installBasicSynchronousHooksOnContext(context);
    }

    ABI18_0_0RCTFBQuickPerformanceLoggerConfigureHooks(context.JSGlobalContextRef);

    __weak ABI18_0_0RCTJSCExecutor *weakSelf = self;
    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI18_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI18_0_0RCT_PROFILE_END_EVENT(ABI18_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI18_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI18_0_0RCT_PROFILE
    __weak ABI18_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI18_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI18_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI18_0_0RCTProfileIsProfiling()) {
        ...
        _ABI18_0_0RCTProfileEndFlowEvent(newCookie);
    ...
    if (JSC_JSSamplingProfilerEnabled(context.JSGlobalContextRef)) {
      ...
      [self->_bridge.devMenu addItem:[ABI18_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
        ABI18_0_0RCTJSCExecutor *strongSelf = weakSelf;
...
#if ABI18_0_0RCT_DEV
    ABI18_0_0RCTInstallJSCProfiler(self->_bridge, context.JSGlobalContextRef);
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI18_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ...
    if (logLevel) {
      level = MAX(level, (ABI18_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI18_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI18_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI18_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI18_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI18_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI18_0_0RCTCookieMap *cookieMap = [ABI18_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI18_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI18_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI18_0_0RCTProfileDidStartProfiling];
...
- (int32_t)bytecodeFileFormatVersion
{
  return _useCustomJSCLibrary
    ? facebook::ReactABI18_0_0::customJSCWrapper()->JSBytecodeFileFormatVersion
...
ABI18_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)contextName)
{
...
- (void)flushedQueue:(ABI18_0_0RCTJavaScriptCallback)onComplete
{
...
- (void)_callFunctionOnModule:(NSString *)module
                 ...
                 unwrapResult:(BOOL)unwrapResult
                     callback:(ABI18_0_0RCTJavaScriptCallback)onComplete
{
  // TODO: Make this function handle first class instead of dynamically dispatching it. #9317773
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI18_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI18_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI18_0_0RCTJavaScriptCallback)onComplete
{
  ABI18_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI18_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI18_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, batchedBridgeRef) != kJSTypeUndefined) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, methodJSRef) != kJSTypeUndefined) {
        ...
        if (!errorJSRef && JSC_JSValueGetType(ctx, methodJSRef) == kJSTypeUndefined) {
          error = ABI18_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSC_JSValueGetType(ctx, batchedBridgeRef) == kJSTypeUndefined) {
        error = ABI18_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI18_0_0RCTNSErrorFromJSError([JSC_JSValue(ctx) valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI18_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
- (void)executeApplicationScript:(NSData *)script
                       sourceURL:(NSURL *)sourceURL
                      onComplete:(ABI18_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ABI18_0_0RCTAssertParam(script);
  ABI18_0_0RCTAssertParam(sourceURL);
...
static TaggedScript loadTaggedScript(NSData *script,
                                     NSURL *sourceURL,
                                     ABI18_0_0RCTPerformanceLogger *performanceLogger,
                                     RandomAccessBundleData &randomAccessBundle,
                                     NSError **error)
{
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);

  facebook::ReactABI18_0_0::BundleHeader header;
  [script getBytes:&header length:sizeof(header)];
  facebook::ReactABI18_0_0::ScriptTag tag = facebook::ReactABI18_0_0::parseTypeFromHeader(header);

  NSData *loadedScript = NULL;
  switch (tag) {
    case facebook::ReactABI18_0_0::ScriptTag::RAMBundle:
      [performanceLogger markStartForTag:ABI18_0_0RCTPLRAMBundleLoad];

      loadedScript = loadRAMBundle(sourceURL, error, randomAccessBundle);

      [performanceLogger markStopForTag:ABI18_0_0RCTPLRAMBundleLoad];
      [performanceLogger setValue:loadedScript.length forTag:ABI18_0_0RCTPLRAMStartupCodeSize];
      break;

    case facebook::ReactABI18_0_0::ScriptTag::BCBundle:
      loadedScript = script;
      break;

    case facebook::ReactABI18_0_0::ScriptTag::String: {
...
static void registerNativeRequire(JSContext *context, ABI18_0_0RCTJSCExecutor *executor)
{
  __weak ABI18_0_0RCTJSCExecutor *weakExecutor = executor;
...
{
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(taggedScript.script.length)
  }));

  [performanceLogger markStartForTag:ABI18_0_0RCTPLScriptExecution];
  ...
  [performanceLogger markStopForTag:ABI18_0_0RCTPLScriptExecution];

  NSError *error = jsError
    ? ABI18_0_0RCTNSErrorFromJSErrorRef(jsError, ctx)
    : nil;
...
- (void)injectJSONText:(NSString *)script
   asGlobalObjectNamed:(NSString *)objectName
              callback:(ABI18_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI18_0_0RCT_DEBUG) {
    ABI18_0_0RCTAssert(ABI18_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    JSGlobalContextRef ctx = strongSelf->_context.context.JSGlobalContextRef;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI18_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI18_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI18_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
      }
    }
    ABI18_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI18_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI18_0_0RCTFatal(ABI18_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    NSError *error = ABI18_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI18_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI18_0_0RCTPLRAMNativeRequires];
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI18_0_0RCT_PROFILE_END_EVENT(ABI18_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI18_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI18_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI18_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
@implementation ABI18_0_0RCTJSContextProvider
{
...
- (ABI18_0_0RCTJSContextData)data
{
...
- (ABI18_0_0RCTJSCExecutor *)createExecutorWithContext:(JSContext **)JSContext
{
  const ABI18_0_0RCTJSContextData data = self.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  return [[ABI18_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
}
