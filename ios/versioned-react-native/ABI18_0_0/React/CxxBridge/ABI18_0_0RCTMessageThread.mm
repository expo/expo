...
#include "ABI18_0_0RCTMessageThread.h"
...
ABI18_0_0RCTMessageThread::ABI18_0_0RCTMessageThread(NSRunLoop *runLoop, ABI18_0_0RCTJavaScriptCompleteBlock errorBlock)
...
ABI18_0_0RCTMessageThread::~ABI18_0_0RCTMessageThread() {
  CFRelease(m_cfRunLoop);
}

// This is analogous to dispatch_async
void ABI18_0_0RCTMessageThread::runAsync(std::function<void()> func) {
...
void ABI18_0_0RCTMessageThread::runSync(std::function<void()> func) {
...
void ABI18_0_0RCTMessageThread::tryFunc(const std::function<void()>& func) {
...
void ABI18_0_0RCTMessageThread::runOnQueue(std::function<void()>&& func) {
  if (m_shutdown) {
    return;
  }
...
void ABI18_0_0RCTMessageThread::runOnQueueSync(std::function<void()>&& func) {
  if (m_shutdown) {
    return;
  }
...
void ABI18_0_0RCTMessageThread::quitSynchronous() {
  m_shutdown = true;
  runSync([]{});
