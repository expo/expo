...
#import "ABI18_0_0RCTRootViewDelegate.h"
...
#import "ABI18_0_0RCTKeyCommands.h"
...
#if TARGET_OS_TV
#import "ABI18_0_0RCTTVRemoteHandler.h"
#import "ABI18_0_0RCTTVNavigationEventEmitter.h"
#endif

NSString *const ABI18_0_0RCTContentDidAppearNotification = @"ABI18_0_0RCTContentDidAppearNotification";

@interface ABI18_0_0RCTUIManager (ABI18_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI18_0_0RCTRootView
{
  ABI18_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  NSDictionary *_launchOptions;
  ABI18_0_0RCTRootContentView *_contentView;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI18_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI18_0_0RCTRootView");
  ABI18_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI18_0_0RCTRootView");

  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"-[ABI18_0_0RCTRootView init]", nil);
  if (!bridge.isLoading) {
    [bridge.performanceLogger markStartForTag:ABI18_0_0RCTPLTTI];
  }
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _sizeFlexibility = ABI18_0_0RCTRootViewSizeFlexibilityNone;

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidReload)
                                                 name:ABI18_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI18_0_0RCTContentDidAppearNotification
                                               object:self];

#if TARGET_OS_TV
    self.tvRemoteHandler = [ABI18_0_0RCTTVRemoteHandler new];
...
{
  ABI18_0_0RCTBridge *bridge = [[ABI18_0_0RCTBridge alloc] initWithBundleURL:bundleURL
                                            moduleProvider:nil
...
{
  if (self.ReactABI18_0_0PreferredFocusedView) {
    return self.ReactABI18_0_0PreferredFocusedView;
  }
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  fitSize = CGSizeMake(
      _sizeFlexibility & ABI18_0_0RCTRootViewSizeFlexibilityWidth ? fitSize.width : currentSize.width,
      _sizeFlexibility & ABI18_0_0RCTRootViewSizeFlexibilityHeight ? fitSize.height : currentSize.height
    );
...
{
  return _ReactABI18_0_0ViewController ?: [super ReactABI18_0_0ViewController];
}
...
{
  ABI18_0_0RCTAssertMainQueue();
  if (!super.ReactABI18_0_0Tag) {
    /**
     * Every root view that is created must have a unique ReactABI18_0_0 tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ReactABI18_0_0 tag must be re-assigned every time a new UIManager is created.
     */
    self.ReactABI18_0_0Tag = [_bridge.uiManager allocateRootTag];
  }
  return super.ReactABI18_0_0Tag;
}

- (void)bridgeDidReload
{
  ABI18_0_0RCTAssertMainQueue();
  // Clear the ReactABI18_0_0Tag so it can be re-assigned
  self.ReactABI18_0_0Tag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI18_0_0RCTRootContentView is scoped to the right bridge
  ABI18_0_0RCTBridge *bridge = notification.userInfo[@"bridge"];
  if (bridge != _contentView.bridge) {
    [self bundleFinishedLoading:bridge];
  }
}

- (void)bundleFinishedLoading:(ABI18_0_0RCTBridge *)bridge
{
  ABI18_0_0RCTAssert(bridge != nil, @"Bridge cannot be nil");
  if (!bridge.valid) {
    return;
  }

  [_contentView removeFromSuperview];
  _contentView = [[ABI18_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
  ...
  if (_sizeFlexibility == ABI18_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI18_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag": _contentView.ReactABI18_0_0Tag,
    @"initialProps": _appProperties ?: @{},
  };

  ABI18_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
@implementation ABI18_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI18_0_0RCTLogWarn(@"Calling deprecated `[-ABI18_0_0RCTRootView intrinsicSize]`.");
  return self.intrinsicContentSize;
}

@end

@implementation ABI18_0_0RCTUIManager (ABI18_0_0RCTRootView)
