...
static NSRegularExpression *ABI18_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI18_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI18_0_0RCTNullIfNil(self.methodName),
     @"file": ABI18_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI18_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
+ (instancetype)stackFrameWithDictionary:(NSDictionary *)dict
{
  return [[self alloc] initWithMethodName:dict[@"methodName"]
                                     file:dict[@"file"]
                               lineNumber:[ABI18_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI18_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}

+ (NSArray<ABI18_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI18_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI18_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI18_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
