...
#import "ABI18_0_0RCTBridgeMethod.h"
#import "ABI18_0_0RCTConvert.h"
#import "ABI18_0_0RCTDisplayLink.h"
#import "ABI18_0_0RCTJSCExecutor.h"
#import "ABI18_0_0RCTJavaScriptLoader.h"
...
#if ABI18_0_0RCT_DEV && __has_include("ABI18_0_0RCTDevLoadingView.h")
#import "ABI18_0_0RCTDevLoadingView.h"
#endif

#define ABI18_0_0RCTAssertJSThread() \
  ABI18_0_0RCTAssert(![NSStringFromClass([self->_javaScriptExecutor class]) isEqualToString:@"ABI18_0_0RCTJSCExecutor"] || \
              [[[NSThread currentThread] name] isEqualToString:ABI18_0_0RCTJSCThreadName], \
...
@implementation ABI18_0_0RCTBatchedBridge
{
  std::atomic_bool _wasBatchActive;
  NSMutableArray<dispatch_block_t> *_pendingCalls;
  NSDictionary<NSString *, ABI18_0_0RCTModuleData *> *_moduleDataByName;
...
- (void)start
{
  ...
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI18_0_0RCTBatchedBridge setUp]", nil);

  dispatch_queue_t bridgeQueue = dispatch_queue_create("com.facebook.ReactABI18_0_0.ABI18_0_0RCTBridgeQueue", DISPATCH_QUEUE_CONCURRENT);

  dispatch_group_t initModulesAndLoadSource = dispatch_group_create();

  // Asynchronously load source code
  dispatch_group_enter(initModulesAndLoadSource);
  __weak ABI18_0_0RCTBatchedBridge *weakSelf = self;
  __block NSData *sourceCode;
  [self loadSource:^(NSError *error, NSData *source, __unused int64_t sourceLength) {
    if (error) {
      ABI18_0_0RCTLogWarn(@"Failed to load source: %@", error);
  ...
  ABI18_0_0RCTPerformanceLogger *performanceLogger = self->_performanceLogger;
  ...
  dispatch_async(bridgeQueue, ^{
    ...
    dispatch_group_async(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      [performanceLogger markStartForTag:ABI18_0_0RCTPLJSCExecutorSetup];
      [weakSelf setUpExecutor];
      [performanceLogger markStopForTag:ABI18_0_0RCTPLJSCExecutorSetup];
    });

    // Asynchronously gather the module config
    dispatch_group_async(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      if (weakSelf.valid) {
        ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI18_0_0RCTBatchedBridge moduleConfig", nil);
        [performanceLogger markStartForTag:ABI18_0_0RCTPLNativeModulePrepareConfig];
        config = [weakSelf moduleConfig];
        [performanceLogger markStopForTag:ABI18_0_0RCTPLNativeModulePrepareConfig];
    ...
    dispatch_group_notify(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      ...
      [performanceLogger markStartForTag:ABI18_0_0RCTPLNativeModuleInjectConfig];
      [weakSelf injectJSONConfiguration:config onComplete:^(NSError *error) {
        [performanceLogger markStopForTag:ABI18_0_0RCTPLNativeModuleInjectConfig];
        if (error) {
          ABI18_0_0RCTLogWarn(@"Failed to inject config: %@", error);
          dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf stopLoadingWithError:error];
          });
        }
  ...
  dispatch_group_notify(initModulesAndLoadSource, bridgeQueue, ^{
    ABI18_0_0RCTBatchedBridge *strongSelf = weakSelf;
...
{
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI18_0_0RCTBatchedBridge initModules]", nil);
  ...
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"extraModules", nil);
  for (id<ABI18_0_0RCTBridgeModule> module in extraModules) {
    ...
    if (moduleClass == self.executorClass) {
      _javaScriptExecutor = (id<ABI18_0_0RCTJavaScriptExecutor>)module;
    }
  }
  ABI18_0_0RCT_PROFILE_END_EVENT(ABI18_0_0RCTProfileTagAlways, @"");
  ...
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"JavaScriptExecutor", nil);
  if (!_javaScriptExecutor) {
    id<ABI18_0_0RCTJavaScriptExecutor> executorModule = [self.executorClass new];
    ABI18_0_0RCTModuleData *moduleData = [[ABI18_0_0RCTModuleData alloc] initWithModuleInstance:executorModule
                                                                       bridge:self];
  ...
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"ModuleData", nil);
...
- (NSString *)moduleConfig
{
  ...
  for (ABI18_0_0RCTModuleData *moduleData in _moduleDataByID) {
    if (self.executorClass == [ABI18_0_0RCTJSCExecutor class]) {
      [config addObject:@[moduleData.name]];
    } else {
      [config addObject:ABI18_0_0RCTNullIfNil(moduleData.config)];
    }
  }

  return ABI18_0_0RCTJSONStringify(@{
...
- (void)executeSourceCode:(NSData *)sourceCode
{
  ...
  [self enqueueApplicationScript:sourceCode url:self.bundleURL onComplete:^(NSError *loadError) {
    ...
    if (loadError) {
      ABI18_0_0RCTLogWarn(@"Failed to execute source code: %@ %@", [loadError localizedDescription], [loadError localizedFailureReason]);
      dispatch_async(dispatch_get_main_queue(), ^{
    ...
    NSRunLoop *targetRunLoop = [self->_javaScriptExecutor isKindOfClass:[ABI18_0_0RCTJSCExecutor class]] ? [NSRunLoop currentRunLoop] : [NSRunLoop mainRunLoop];
    ...
    [self->_performanceLogger setValue:nativeRequiresCount forTag:ABI18_0_0RCTPLRAMStartupNativeRequiresCount];
    uint64_t nativeRequires = [self->_performanceLogger valueForTag:ABI18_0_0RCTPLRAMNativeRequires];
    [self->_performanceLogger setValue:nativeRequires forTag:ABI18_0_0RCTPLRAMStartupNativeRequires];

    [self->_performanceLogger markStopForTag:ABI18_0_0RCTPLBridgeStartup];
...
- (void)stopLoadingWithError:(NSError *)error
{
  ...
  if ([error userInfo][ABI18_0_0RCTJSStackTraceKey]) {
    [self.redBox showErrorMessage:[error localizedDescription]
                        withStack:[error userInfo][ABI18_0_0RCTJSStackTraceKey]];
  }
...
- (Class)executorClass
{
  return _parentBridge.executorClass ?: [ABI18_0_0RCTJSCExecutor class];
}
                ...
                queue:(dispatch_queue_t)queue
{
  if (queue == ABI18_0_0RCTJSThread) {
    ...
    ABI18_0_0RCTAssert(_javaScriptExecutor != nil, @"Need JS executor to schedule JS work");

    [_javaScriptExecutor executeBlockOnJavaScriptQueue:^{
      ABI18_0_0RCTProfileEndFlowEvent();

      ABI18_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI18_0_0RCTBatchedBridge dispatchBlock", @{ @"loading": @(self.loading) });

      if (self.loading) {
        ABI18_0_0RCTAssert(self->_pendingCalls != nil, @"Can't add pending call, bridge is no longer loading");
...
- (void)invalidate
{
  ...
  ABI18_0_0RCTAssertMainQueue();
  ABI18_0_0RCTAssert(_javaScriptExecutor != nil, @"Can't complete invalidation without a JS executor");
  ...
  for (ABI18_0_0RCTModuleData *moduleData in _moduleDataByID) {
    ...
    id<ABI18_0_0RCTBridgeModule> instance = nil;
    if ([moduleData hasInstance]) {
    ...
    if ([instance respondsToSelector:@selector(invalidate)]) {
      ...
      [self dispatchBlock:^{
        [(id<ABI18_0_0RCTInvalidating>)instance invalidate];
...
{
  if (ABI18_0_0RCT_DEBUG && [_javaScriptExecutor isValid]) {
...
{
 module = ABI18_0_0EX_REMOVE_VERSION(module);
 /**
   * AnyThread
   */
  if (!_valid) {
  ...
  ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"-[ABI18_0_0RCTBatchedBridge enqueueJSCall:]", nil);
...
- (JSValue *)callFunctionOnModule:(NSString *)module
                        ...
                        arguments:(NSArray *)arguments
                            error:(NSError **)error
{
  ABI18_0_0RCTJSCExecutor *jsExecutor = (ABI18_0_0RCTJSCExecutor *)_javaScriptExecutor;
  if (![jsExecutor isKindOfClass:[ABI18_0_0RCTJSCExecutor class]]) {
    ABI18_0_0RCTLogWarn(@"FBReactABI18_0_0BridgeJSExecutor is only supported when running in JSC");
    return nil;
  }
  ...
  [jsExecutor callFunctionOnModule:module
    ...
    ABI18_0_0RCTAssert([length isNumber] && [length toUInt32] == 2,
                      ...
                      onComplete:(ABI18_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ...
  [_javaScriptExecutor executeApplicationScript:script sourceURL:url onComplete:^(NSError *scriptLoadError) {
    ...
    ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"FetchApplicationScriptCallbacks", nil);
    [self->_javaScriptExecutor flushedQueue:^(id json, NSError *error)
     {
       ABI18_0_0RCT_PROFILE_END_EVENT(ABI18_0_0RCTProfileTagAlways, @"js_call,init");
...
{
  NSArray *requestsArray = [ABI18_0_0RCTConvert NSArray:buffer];

  if (ABI18_0_0RCT_DEBUG && requestsArray.count <= ABI18_0_0RCTBridgeFieldParams) {
    ABI18_0_0RCTLogError(@"Buffer should contain at least %tu sub-arrays. Only found %tu",
                ABI18_0_0RCTBridgeFieldParams + 1, requestsArray.count);
    return;
  }

  NSArray<NSNumber *> *moduleIDs = [ABI18_0_0RCTConvert NSNumberArray:requestsArray[ABI18_0_0RCTBridgeFieldRequestModuleIDs]];
  NSArray<NSNumber *> *methodIDs = [ABI18_0_0RCTConvert NSNumberArray:requestsArray[ABI18_0_0RCTBridgeFieldMethodIDs]];
  NSArray<NSArray *> *paramsArrays = [ABI18_0_0RCTConvert NSArrayArray:requestsArray[ABI18_0_0RCTBridgeFieldParams]];

  int64_t callID = -1;

  if (requestsArray.count > 3) {
    callID = [requestsArray[ABI18_0_0RCTBridgeFieldCallID] longLongValue];
  }

  if (ABI18_0_0RCT_DEBUG && (moduleIDs.count != methodIDs.count || moduleIDs.count != paramsArrays.count)) {
    ABI18_0_0RCTLogError(@"Invalid data message - all must be length: %zd", moduleIDs.count);
    return;
  }
  ...
  [moduleIDs enumerateObjectsUsingBlock:^(NSNumber *moduleID, NSUInteger i, __unused BOOL *stop) {
    ABI18_0_0RCTModuleData *moduleData = self->_moduleDataByID[moduleID.integerValue];
  ...
  for (dispatch_queue_t queue in buckets) {
    ...
    dispatch_block_t block = ^{
      ...
      ABI18_0_0RCT_PROFILE_BEGIN_EVENT(ABI18_0_0RCTProfileTagAlways, @"-[ABI18_0_0RCTBatchedBridge handleBuffer:]", (@{
        @"calls": @(calls.count),
      }));
      ...
      @autoreleasepool {
        for (NSNumber *indexObj in calls) {
          ...
          if (ABI18_0_0RCT_DEV && callID != -1 && self->_flowIDMap != NULL && ABI18_0_0RCTProfileIsProfiling()) {
            ...
            _ABI18_0_0RCTProfileEndFlowEvent(newFlowID);
      ...
      ABI18_0_0RCT_PROFILE_END_EVENT(ABI18_0_0RCTProfileTagAlways, @"objc_call,dispatch_async");
    };
...
{
  // TODO: batchDidComplete is only used by ABI18_0_0RCTUIManager - can we eliminate this special case?
  for (ABI18_0_0RCTModuleData *moduleData in _moduleDataByID) {
                ...
                params:(NSArray *)params
{
  ...
  ABI18_0_0RCTModuleData *moduleData = _moduleDataByID[moduleID];
  if (ABI18_0_0RCT_DEBUG && !moduleData) {
    ABI18_0_0RCTLogError(@"No module found for id '%zd'", moduleID);
    return nil;
  }

  id<ABI18_0_0RCTBridgeMethod> method = moduleData.methods[methodID];
  if (ABI18_0_0RCT_DEBUG && !method) {
    ABI18_0_0RCTLogError(@"Unknown methodID: %zd for module: %zd (%@)", methodID, moduleID, moduleData.name);
    return nil;
  }
...
- (JSContext *)jsContext
{
  if ([_javaScriptExecutor isKindOfClass:[ABI18_0_0RCTJSCExecutor class]]) {
    return [(ABI18_0_0RCTJSCExecutor *)_javaScriptExecutor jsContext];
  } else {
    return nil;
  }
}
