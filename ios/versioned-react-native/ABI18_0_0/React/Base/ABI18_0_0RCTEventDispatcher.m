...
const NSInteger ABI18_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI18_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI18_0_0RCTGetEventID(id<ABI18_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI18_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI18_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI18_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI18_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
{
  [_bridge enqueueJSCall:@"ABI18_0_0RCTDeviceEventEmitter"
                  method:@"emit"
                    args:body ? @[name, body] : @[name]
              completion:NULL];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI18_0_0RCT_DEBUG) {
    ABI18_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI18_0_0 tag");
  }

  name = ABI18_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI18_0_0RCTEventEmitter"
                  method:@"receiveEvent"
                    args:body ? @[body[@"target"], name, body] : @[body[@"target"], name]
              completion:NULL];
}

- (void)sendTextEventWithType:(ABI18_0_0RCTTextEventType)type
...
- (void)sendEvent:(id<ABI18_0_0RCTEvent>)event
{
  [_observersLock lock];

  for (id<ABI18_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI18_0_0RCTGetEventID(event);

  id<ABI18_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI18_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)addDispatchObserver:(id<ABI18_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI18_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)dispatchEvent:(id<ABI18_0_0RCTEvent>)event
{
...
@implementation ABI18_0_0RCTBridge (ABI18_0_0RCTEventDispatcher)

- (ABI18_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI18_0_0RCTEventDispatcher class]];
}
