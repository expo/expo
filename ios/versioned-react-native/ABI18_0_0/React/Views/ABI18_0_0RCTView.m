...
@implementation UIView (ABI18_0_0RCTViewUnmounting)
...
- (void)ReactABI18_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI18_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI18_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI18_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ReactABI18_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI18_0_0RootView]) {
      break;
    }
...
static NSString *ABI18_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI18_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI18_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI18_0_0RCTBorderStyleSolid;
...
{
  _ReactABI18_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI18_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI18_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI18_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI18_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI18_0_0RCTPointerEventsNone || _pointerEvents == ABI18_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ReactABI18_0_0ZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI18_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI18_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI18_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI18_0_0Tag: %@;", self.ReactABI18_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI18_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI18_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ReactABI18_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI18_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI18_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI18_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI18_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ReactABI18_0_0Subviews.count == 0) {
  ...
  for (UIView *view in self.ReactABI18_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI18_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI18_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI18_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI18_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI18_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI18_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI18_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI18_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI18_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI18_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI18_0_0RCTCornerRadii){
...
{
  return (ABI18_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI18_0_0RCTUpdateShadowPathForView(self);

  const ABI18_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI18_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI18_0_0RCTRunningInTestEnvironment() &&
  ABI18_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI18_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI18_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI18_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI18_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI18_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI18_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI18_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI18_0_0RCTUpdateShadowPathForView(ABI18_0_0RCTView *view)
{
  if (ABI18_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI18_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI18_0_0RCTCornerInsets cornerInsets = ABI18_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI18_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI18_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI18_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI18_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI18_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI18_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI18_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI18_0_0RCTBorderStyle)style \
  {                                                    \
