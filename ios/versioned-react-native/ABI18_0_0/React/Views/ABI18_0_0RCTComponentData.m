...
typedef void (^ABI18_0_0RCTPropBlock)(id<ABI18_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI18_0_0RCTPropBlock> ABI18_0_0RCTPropBlockDictionary;

@implementation ABI18_0_0RCTComponentData
{
  id<ABI18_0_0RCTComponent> _defaultView; // Only needed for ABI18_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI18_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI18_0_0RCTPropBlockDictionary *_shadowPropBlocks;
                              ...
                              bridge:(ABI18_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    ...
    // Hackety hack, this partially re-implements ABI18_0_0RCTBridgeModuleNameForClass
    // We want to get rid of ABI18_0_0RCT and RK prefixes, but a lot of JS code still references
    // view names by prefix. So, while ABI18_0_0RCTBridgeModuleNameForClass now drops these
    ...
    ABI18_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
    _name = name;

    _implementsUIBlockToAmendWithShadowViewRegistry = NO;
    Class cls = _managerClass;
    while (cls != [ABI18_0_0RCTViewManager class]) {
      _implementsUIBlockToAmendWithShadowViewRegistry = _implementsUIBlockToAmendWithShadowViewRegistry ||
      ABI18_0_0RCTClassOverridesInstanceMethod(cls, @selector(uiBlockToAmendWithShadowViewRegistry:));
      cls = [cls superclass];
    }
  }
  return self;
...
- (ABI18_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI18_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI18_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI18_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI18_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI18_0_0RCTPropBlock createEventSetter(NSString *propName, SEL setter, ABI18_0_0RCTBridge *bridge)
{
  __weak ABI18_0_0RCTBridge *weakBridge = bridge;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI18_0_0RCTConvert BOOL:json]) {
      __weak id<ABI18_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI18_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        NSMutableDictionary *mutableEvent = [NSMutableDictionary dictionaryWithDictionary:event];
        mutableEvent[@"target"] = strongTarget.ReactABI18_0_0Tag;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [weakBridge.eventDispatcher sendInputEventWithName:ABI18_0_0RCTNormalizeInputEventName(propName) body:mutableEvent];
...
static ABI18_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI18_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([_managerClass respondsToSelector:selector]) {
    ...
    type = ABI18_0_0RCTConvertSelectorForType(typeAndKeyPath[0]);
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI18_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI18_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI18_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI18_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock = createEventSetter(name, setter, _bridge);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI18_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI18_0_0RCTLogError(@"No +[ABI18_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI18_0_0RCTComponent> view, __unused id json){};
      }
  ...
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI18_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI18_0_0RCT_CASE(_C_SEL, SEL)
        ABI18_0_0RCT_CASE(_C_CHARPTR, const char *)
        ...
        ABI18_0_0RCT_CASE(_C_PTR, void *)
        ABI18_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI18_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI18_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI18_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI18_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI18_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI18_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ",
                             name, weakSelf.name, view.ReactABI18_0_0Tag];
      ABI18_0_0RCTPerformBlockWithLogPrefix(^{
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI18_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI18_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI18_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
      ABI18_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                  "to '%@'", name, _name, propTypes[name], type);
    }

    if ([type isEqualToString:@"ABI18_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI18_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI18_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI18_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
  ...
  for (NSString *event in bubblingEvents) {
    if ([directEvents containsObject:event]) {
      ABI18_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
