...
@interface ABI18_0_0RCTPicker() <UIPickerViewDataSource, UIPickerViewDelegate>
@end

@implementation ABI18_0_0RCTPicker
...
{
  return [ABI18_0_0RCTConvert NSString:_items[row][@"label"]];
}
           ...
           reusingView:(UILabel *)label
{
  ...
  label.textColor = [ABI18_0_0RCTConvert UIColor:_items[row][@"textColor"]] ?: _color;
      ...
      didSelectRow:(NSInteger)row inComponent:(__unused NSInteger)component
{
  ...
  if (_onChange && _items.count > (NSUInteger)row) {
    _onChange(@{
      @"newIndex": @(row),
      @"newValue": ABI18_0_0RCTNullIfNil(_items[row][@"value"]),
    });
  }
}
