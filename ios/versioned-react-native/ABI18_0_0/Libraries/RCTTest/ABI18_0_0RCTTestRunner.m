...
#import "ABI18_0_0RCTTestRunner.h"
...
#import "ABI18_0_0FBSnapshotTestController.h"
#import "ABI18_0_0RCTTestModule.h"

static const NSTimeInterval kTestTimeoutSeconds = 120;

@implementation ABI18_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI18_0_0RCTBridgeModuleListProvider _moduleProvider;
}
...
{
  ABI18_0_0RCTAssertParam(app);
  ABI18_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI18_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI18_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
configurationBlock:(void(^)(ABI18_0_0RCTRootView *rootView))configurationBlock
{
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  @autoreleasepool {
    ...
    ABI18_0_0RCTLogFunction defaultLogFunction = ABI18_0_0RCTGetLogFunction();
    ABI18_0_0RCTSetLogFunction(^(ABI18_0_0RCTLogLevel level, ABI18_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI18_0_0RCTLogLevelError) {
        error = message;
      } else {
    ...
    ABI18_0_0RCTBridge *bridge = [[ABI18_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI18_0_0RCTRootView *rootView = [[ABI18_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI18_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI18_0_0RCTTestModule class]];
    ABI18_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    testModule.controller = _testController;
    testModule.testSelector = test;
    testModule.testSuffix = _testSuffix;
    testModule.view = rootView;

    UIViewController *vc = ABI18_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI18_0_0RCTTestStatusPending && error == nil) {
    ...
    ABI18_0_0RCTSetLogFunction(defaultLogFunction);
    ...
    ABI18_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI18_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI18_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI18_0_0RCTAssert(testModule.status != ABI18_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI18_0_0RCTAssert(testModule.status == ABI18_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
}
