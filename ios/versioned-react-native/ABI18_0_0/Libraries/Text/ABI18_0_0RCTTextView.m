...
#import "ABI18_0_0RCTUITextView.h"

@implementation ABI18_0_0RCTTextView
{
  ABI18_0_0RCTBridge *_bridge;
  ABI18_0_0RCTEventDispatcher *_eventDispatcher;

  ABI18_0_0RCTUITextView *_textView;
  ABI18_0_0RCTText *_richTextView;
...
- (instancetype)initWithBridge:(ABI18_0_0RCTBridge *)bridge
{
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _textView = [[ABI18_0_0RCTUITextView alloc] initWithFrame:self.bounds];
...
#pragma mark - ABI18_0_0RCTComponent

- (void)insertReactABI18_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI18_0_0Subview:subview atIndex:index];

  if ([subview isKindOfClass:[ABI18_0_0RCTText class]]) {
    if (_richTextView) {
      ABI18_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI18_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI18_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI18_0_0RCTReactABI18_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI18_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI18_0_0TagFromString(_pendingAttributedText);
...
- (void)setSelection:(ABI18_0_0RCTTextSelection *)selection
{
  if (!selection) {
    return;
  }
  ...
  } else if (eventLag > ABI18_0_0RCTTextUpdateLagWarningThreshold) {
    ABI18_0_0RCTLogWarn(@"Native TextInput(%@) is %zd events ahead of JS - try to make your JS faster.", self.text, eventLag);
  }
}
...
- (BOOL)textView:(ABI18_0_0RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (!textView.textWasPasted) {
    [_eventDispatcher sendTextEventWithType:ABI18_0_0RCTTextEventTypeKeyPress
    ...
    if (_blurOnSubmit && [text isEqualToString:@"\n"]) {
      // TODO: the purpose of blurOnSubmit on ABI18_0_0RCTextField is to decide if the
      // field should lose focus when return is pressed or not. We're cheating a
      // bit here by using it on ABI18_0_0RCTextView to decide if return character should
      ...
      [_eventDispatcher sendTextEventWithType:ABI18_0_0RCTTextEventTypeSubmit
                                     ReactABI18_0_0Tag:self.ReactABI18_0_0Tag
                                         text:self.text
                                          key:nil
                                   eventCount:_nativeEventCount];
...
- (void)textViewDidChangeSelection:(ABI18_0_0RCTUITextView *)textView
{
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
  ...
  [_eventDispatcher sendTextEventWithType:ABI18_0_0RCTTextEventTypeFocus
                                 ReactABI18_0_0Tag:self.ReactABI18_0_0Tag
                                     text:nil
                                      key:nil
                               eventCount:_nativeEventCount];
...
- (void)textViewDidChange:(UITextView *)textView
{
  ...
  if (!self.ReactABI18_0_0Tag || !_onChange) {
    return;
  }
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
  ...
  [_eventDispatcher sendTextEventWithType:ABI18_0_0RCTTextEventTypeEnd
                                 ReactABI18_0_0Tag:self.ReactABI18_0_0Tag
                                     text:textView.text
                                      key:nil
                               eventCount:_nativeEventCount];

  [_eventDispatcher sendTextEventWithType:ABI18_0_0RCTTextEventTypeBlur
                                 ReactABI18_0_0Tag:self.ReactABI18_0_0Tag
                                     text:nil
                                      key:nil
                               eventCount:_nativeEventCount];
...
{
  [_textView ReactABI18_0_0Focus];
}

- (void)ReactABI18_0_0Blur
{
  [_textView ReactABI18_0_0Blur];
}

- (void)didMoveToWindow
{
  [_textView ReactABI18_0_0FocusIfNeeded];
}
