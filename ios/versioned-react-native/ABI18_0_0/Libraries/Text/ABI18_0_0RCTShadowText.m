...
NSString *const ABI18_0_0RCTShadowViewAttributeName = @"ABI18_0_0RCTShadowViewAttributeName";
NSString *const ABI18_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI18_0_0RCTReactABI18_0_0TagAttributeName = @"ReactABI18_0_0TagAttributeName";

CGFloat const ABI18_0_0RCTTextAutoSizeDefaultMinimumFontScale       = 0.5f;
CGFloat const ABI18_0_0RCTTextAutoSizeWidthErrorMargin              = 0.05f;
CGFloat const ABI18_0_0RCTTextAutoSizeHeightErrorMargin             = 0.025f;
CGFloat const ABI18_0_0RCTTextAutoSizeGranularity                   = 0.001f;

@implementation ABI18_0_0RCTShadowText
{
...
static ABI18_0_0YGSize ABI18_0_0RCTMeasure(ABI18_0_0YGNodeRef node, float width, ABI18_0_0YGMeasureMode widthMode, float height, ABI18_0_0YGMeasureMode heightMode)
{
  ABI18_0_0RCTShadowText *shadowText = (__bridge ABI18_0_0RCTShadowText *)ABI18_0_0YGNodeGetContext(node);
  ...
  result.width = ABI18_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI18_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI18_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI18_0_0RCTMeasure);
...
{
  ABI18_0_0YGNodeMarkDirty(self.yogaNode);
  [self dirtyText];
}
...
{
  if ([[self ReactABI18_0_0Superview] isKindOfClass:[ABI18_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI18_0_0Superview] ReactABI18_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:width widthMode:ABI18_0_0YGMeasureModeExactly];
  CGRect textFrame = [self calculateTextFrame:textStorage];
  BOOL selectable = _selectable;
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI18_0_0RCTText *view = (ABI18_0_0RCTText *)viewRegistry[self.ReactABI18_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI18_0_0RCTShadowText
     * to the ABI18_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI18_0_0RCTTextView
     * and its contaned ABI18_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI18_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  NSTextStorage *textStorage = [self buildTextStorageForWidth:self.frame.size.width widthMode:ABI18_0_0YGMeasureModeExactly];
  NSLayoutManager *layoutManager = textStorage.layoutManagers.firstObject;
  NSTextContainer *textContainer = layoutManager.textContainers.firstObject;
  NSRange glyphRange = [layoutManager glyphRangeForTextContainer:textContainer];
  NSRange characterRange = [layoutManager characterRangeForGlyphRange:glyphRange actualGlyphRange:NULL];
  [layoutManager.textStorage enumerateAttribute:ABI18_0_0RCTShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI18_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI18_0_0YGNodeRef childNode = child.yogaNode;
      float width = ABI18_0_0YGNodeStyleGetWidth(childNode).value;
      float height = ABI18_0_0YGNodeStyleGetHeight(childNode).value;
      if (ABI18_0_0YGFloatIsUndefined(width) || ABI18_0_0YGFloatIsUndefined(height)) {
        ABI18_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI18_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI18_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI18_0_0RCTRoundPixelValue(width),
        ABI18_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI18_0_0YGMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){widthMode == ABI18_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : width, CGFLOAT_MAX};
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI18_0_0RCTFont updateFont:nil
  ...
  for (ABI18_0_0RCTShadowView *child in [self ReactABI18_0_0Subviews]) {
    if ([child isKindOfClass:[ABI18_0_0RCTShadowText class]]) {
      ABI18_0_0RCTShadowText *shadowText = (ABI18_0_0RCTShadowText *)child;
      ...
      ABI18_0_0RCTShadowRawText *shadowRawText = (ABI18_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI18_0_0YGNodeStyleGetWidth(child.yogaNode).value;
      float height = ABI18_0_0YGNodeStyleGetHeight(child.yogaNode).value;
      if (ABI18_0_0YGFloatIsUndefined(width) || ABI18_0_0YGFloatIsUndefined(height)) {
        ABI18_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      [attachmentString addAttribute:ABI18_0_0RCTShadowViewAttributeName value:child range:(NSRange){0, attachmentString.length}];
      [attributedString appendAttributedString:attachmentString];
      if (height > heightOfTallestSubview) {
        heightOfTallestSubview = height;
      }
      // Don't call setTextComputed on this child. ABI18_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI18_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI18_0_0RCTReactABI18_0_0TagAttributeName withValue:self.ReactABI18_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  // Text decoration
  if (_textDecorationLine == ABI18_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI18_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI18_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI18_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
- (CGRect)updateStorage:(NSTextStorage *)textStorage toFitFrame:(CGRect)frame
{
  ...
  frame.origin.y = self.paddingAsInsets.top + ABI18_0_0RCTRoundPixelValue((CGRectGetHeight(frame) - requiredSize.height) / 2.0f);
  return frame;
}
                               ...
                               prevMid:(CGFloat)prevMid
{
  ...
  if (round((prevMid / ABI18_0_0RCTTextAutoSizeGranularity)) == round((midScale / ABI18_0_0RCTTextAutoSizeGranularity))) {
    //Bail because we can't meet error margin.
    return [self calculateSize:textStorage];
  } else {
    ABI18_0_0RCTSizeComparison comparison = [self attemptScale:midScale
                                            inStorage:textStorage
                                             forFrame:frame];
    if (comparison == ABI18_0_0RCTSizeWithinRange) {
      return [self calculateSize:textStorage];
    } else if (comparison == ABI18_0_0RCTSizeTooLarge) {
      return [self calculateOptimumScaleInFrame:frame
                                     forStorage:textStorage
                                       minScale:minScale
                                       maxScale:midScale - ABI18_0_0RCTTextAutoSizeGranularity
                                        prevMid:midScale];
    } else {
      return [self calculateOptimumScaleInFrame:frame
                                     forStorage:textStorage
                                       minScale:midScale + ABI18_0_0RCTTextAutoSizeGranularity
                                       maxScale:maxScale
                                        prevMid:midScale];
    }
  }
...
- (ABI18_0_0RCTSizeComparison)attemptScale:(CGFloat)scale
                         ...
                         forFrame:(CGRect)frame
{
  ...
  if (fitLines && fitSize) {
    if ((requiredSize.width + (CGRectGetWidth(frame) * ABI18_0_0RCTTextAutoSizeWidthErrorMargin)) > CGRectGetWidth(frame) &&
        (requiredSize.height + (CGRectGetHeight(frame) * ABI18_0_0RCTTextAutoSizeHeightErrorMargin)) > CGRectGetHeight(frame))
    {
      return ABI18_0_0RCTSizeWithinRange;
    } else {
      return ABI18_0_0RCTSizeTooSmall;
    }
  } else {
    return ABI18_0_0RCTSizeTooLarge;
  }
}
...
#define ABI18_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI18_0_0RCT_TEXT_PROPERTY(AdjustsFontSizeToFit, _adjustsFontSizeToFit, BOOL)
ABI18_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI18_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI18_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI18_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI18_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI18_0_0RCT_TEXT_PROPERTY(FontVariant, _fontVariant, NSArray *)
ABI18_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI18_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI18_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI18_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI18_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI18_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI18_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI18_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI18_0_0RCTTextDecorationLineType);
ABI18_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI18_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI18_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI18_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI18_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI18_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI18_0_0RCTShadowView *child in [self ReactABI18_0_0Subviews]) {
    if ([child isKindOfClass:[ABI18_0_0RCTShadowText class]]) {
      ((ABI18_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  if (_fontSizeMultiplier == 0) {
    ABI18_0_0RCTLogError(@"fontSizeMultiplier value must be > zero.");
    _fontSizeMultiplier = 1.0;
  }
  for (ABI18_0_0RCTShadowView *child in [self ReactABI18_0_0Subviews]) {
    if ([child isKindOfClass:[ABI18_0_0RCTShadowText class]]) {
      ((ABI18_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
