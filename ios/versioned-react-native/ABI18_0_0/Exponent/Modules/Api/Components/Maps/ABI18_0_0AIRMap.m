...
const CLLocationDegrees ABI18_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI18_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI18_0_0AIRMapZoomBoundBuffer = 0.01;
...
@implementation ABI18_0_0AIRMap
{
    UIView *_legalLabel;
    CLLocationManager *_locationManager;
    BOOL _initialRegionSet;

    // Array to manually track ABI18_0_0RN subviews
    //
    // ABI18_0_0AIRMap implicitly creates subviews that aren't regular ABI18_0_0RN children
    // (ABI18_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI18_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI18_0_0-native/blob/v0.16.0/ReactABI18_0_0/Modules/ABI18_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI18_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI18_0_0-native/blob/v0.16.0/Libraries/Text/ABI18_0_0RCTTextField.m#L20
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI18_0_0SMCalloutView platformCalloutView];
...
- (void)insertReactABI18_0_0Subview:(id<ABI18_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI18_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI18_0_0AIRMapPolyline class]]) {
        ((ABI18_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI18_0_0AIRMapPolygon class]]) {
        ((ABI18_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI18_0_0AIRMapCircle class]]) {
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI18_0_0AIRMapUrlTile class]]) {
        ((ABI18_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
...
#pragma mark ABI18_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI18_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
