...
@implementation ABI18_0_0EXBarCodeScannerManager

ABI18_0_0RCT_EXPORT_MODULE(ExponentBarCodeScannerManager);
ABI18_0_0RCT_EXPORT_VIEW_PROPERTY(onBarCodeRead, ABI18_0_0RCTDirectEventBlock);
...
- (UIView *)view
{
  ...
  if (!self.camera) {
    self.camera = [[ABI18_0_0EXBarCodeScanner alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
- (NSDictionary *)constantsToExport
{
  return @{
    ...
    @"Type" :
        @{@"front" : @(ABI18_0_0EXBarCodeScannerTypeFront), @"back" : @(ABI18_0_0EXBarCodeScannerTypeBack)},
    @"TorchMode" : @{
      @"off" : @(ABI18_0_0EXBarCodeScannerTorchModeOff),
      @"on" : @(ABI18_0_0EXBarCodeScannerTorchModeOn),
      @"auto" : @(ABI18_0_0EXBarCodeScannerTorchModeAuto)
    }
  };
}
...
ABI18_0_0RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, ABI18_0_0EXBarCodeScanner)
{
  NSInteger type = [ABI18_0_0RCTConvert NSInteger:json];
    ...
    dispatch_async(self.sessionQueue, ^{
      ...
      if ([self.session canAddInput:captureDeviceInput]) {
        ...
        if (self.videoCaptureDeviceInput) {
          ...
          ABI18_0_0RCTLogWarn(@"%s: Can't add null video capture device, doing nothing instead.", __func__);
        }
      }
...
ABI18_0_0RCT_CUSTOM_VIEW_PROPERTY(torchMode, NSInteger, ABI18_0_0EXBarCodeScanner)
{
  self.torchMode = [ABI18_0_0RCTConvert NSInteger:json];
  [self setTorchMode];
}
...
ABI18_0_0RCT_CUSTOM_VIEW_PROPERTY(barCodeTypes, NSArray, ABI18_0_0EXBarCodeScanner)
{
  NSArray *types = [ABI18_0_0RCTConvert NSArray:json];
  NSSet *validTypes = [NSSet setWithArray:[[self class] validBarCodeTypes].allValues];
  for (id type in types) {
    if (![validTypes containsObject:type]) {
      ABI18_0_0RCTLogWarn(@"Unsupported BarCodeType: %@", type);
      return;
    }
  }
...
- (void)startSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI18_0_0EXBarCodeScannerManager *weakSelf = self;
              ...
              [NSNotificationCenter.defaultCenter
                  addObserverForName:AVCaptureSessionRuntimeErrorNotification
                          ...
                          usingBlock:^(NSNotification *note) {
                            ABI18_0_0EXBarCodeScannerManager *strongSelf = weakSelf;
