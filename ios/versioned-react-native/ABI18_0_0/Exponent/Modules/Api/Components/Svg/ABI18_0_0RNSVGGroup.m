...
@implementation ABI18_0_0RNSVGGroup
...
- (void)renderGroupTo:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI18_0_0RNSVGNode *node) {
        if (node.responsible && !svg.responsible) {
            svg.responsible = YES;
        }
        
        if ([node isKindOfClass:[ABI18_0_0RNSVGRenderable class]]) {
            [(ABI18_0_0RNSVGRenderable*)node mergeProperties:self];
        }
        
        [node renderTo:context];
        
        if ([node isKindOfClass:[ABI18_0_0RNSVGRenderable class]]) {
            [(ABI18_0_0RNSVGRenderable*)node resetProperties];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event withTransform:(CGAffineTransform)transform
{
    ...
    for (ABI18_0_0RNSVGNode *node in [self.subviews reverseObjectEnumerator]) {
...
- (void)parseReference
{
    ...
    [self traverseSubviews:^(__kindof ABI18_0_0RNSVGNode *node) {
        [node parseReference];
        return YES;
    }];
}
