...
@implementation ABI18_0_0RNSVGTSpan
{
    ABI18_0_0RNSVGBezierTransformer *_bezierTransformer;
    CGPathRef _cache;
}
...
{
    __block ABI18_0_0RNSVGBezierTransformer *bezierTransformer;
    [self traverseTextSuperviews:^(__kindof ABI18_0_0RNSVGText *node) {
        if ([node class] == [ABI18_0_0RNSVGTextPath class]) {
            bezierTransformer = [(ABI18_0_0RNSVGTextPath*)node getBezierTransformer];
            return NO;
        }
        return YES;
    }];
...
- (void)traverseTextSuperviews:(BOOL (^)(__kindof ABI18_0_0RNSVGText *node))block
{
    ABI18_0_0RNSVGText *targetView = self;
    BOOL result = block(self);
    
    while (targetView && [targetView class] != [ABI18_0_0RNSVGText class] && result) {
        if (![targetView isKindOfClass:[ABI18_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        
        targetView = (ABI18_0_0RNSVGText*)[targetView superview];
