...
#import "ABI18_0_0EXFontLoader.h"
...
static NSMutableDictionary *ABI18_0_0EXFonts = nil;

static const char *ABI18_0_0EXFontAssocKey = "ABI18_0_0EXFont";

@interface ABI18_0_0EXFont : NSObject
...
@implementation ABI18_0_0EXFont
...
- (UIFont *) UIFontWithSize:(CGFloat) fsize
{
  ...
  _sizes[size] = uiFont;
  objc_setAssociatedObject(uiFont, ABI18_0_0EXFontAssocKey, self, OBJC_ASSOCIATION_ASSIGN);
  return uiFont;
}
...
@implementation ABI18_0_0RCTFont (ABI18_0_0EXFontLoader)

// Will swap this with +[ABI18_0_0RCTFont updateFont: ...]
          ...
          scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  ABI18_0_0EXFont *exFont = nil;

  // Did we get a new family, and if so, is it associated with an ABI18_0_0EXFont?
  if ([family hasPrefix:exponentPrefix] && ABI18_0_0EXFonts) {
    NSString *suffix = [family substringFromIndex:exponentPrefix.length];
    exFont = ABI18_0_0EXFonts[suffix];
  }

  // Did the passed-in UIFont come from an ABI18_0_0EXFont?
  if (!exFont && uiFont) {
    exFont = objc_getAssociatedObject(uiFont, ABI18_0_0EXFontAssocKey);
  }

  // If it's an ABI18_0_0EXFont, generate the corresponding UIFont, else fallback to ReactABI18_0_0 Native's built-in method
  if (exFont) {
    return [exFont UIFontWithSize:[ABI18_0_0RCTConvert CGFloat:size] ?: uiFont.pointSize ?: defaultFontSize];
  } else {
...
@implementation ABI18_0_0EXFontLoader

ABI18_0_0RCT_EXPORT_MODULE(ExponentFontLoader);
...
+ (void)initialize {
  ...
  method_exchangeImplementations(class_getClassMethod([ABI18_0_0RCTFont class], a),
                                 class_getClassMethod([ABI18_0_0RCTFont class], b));
}

ABI18_0_0RCT_REMAP_METHOD(loadAsync,
...
{
  if (ABI18_0_0EXFonts && ABI18_0_0EXFonts[fontFamilyName]) {
  ...
  if (!ABI18_0_0EXFonts) {
    ABI18_0_0EXFonts = [NSMutableDictionary dictionary];
  }
  ABI18_0_0EXFonts[fontFamilyName] = [[ABI18_0_0EXFont alloc] initWithCGFont:font];
  resolve(nil);
}
