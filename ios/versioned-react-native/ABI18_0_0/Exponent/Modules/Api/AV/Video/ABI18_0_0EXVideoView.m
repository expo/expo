...
static NSString *const ABI18_0_0EXVideoReadyForDisplayKeyPath = @"readyForDisplay";

@interface ABI18_0_0EXVideoView ()

@property (nonatomic, weak) ABI18_0_0EXAV *exAV;

@property (nonatomic, strong) ABI18_0_0EXAVPlayerData *data;
@property (nonatomic, strong) AVPlayerLayer *playerLayer;
@property (nonatomic, strong) ABI18_0_0EXVideoPlayerViewController *playerViewController;
...
@implementation ABI18_0_0EXVideoView

#pragma mark - ABI18_0_0EXVideoView interface methods

- (instancetype)initWithBridge:(ABI18_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _exAV = [bridge moduleForClass:[ABI18_0_0EXAV class]];
...
- (void)_callFullscreenCallbackForUpdate:(ABI18_0_0EXVideoFullscreenUpdate)update
{
...
- (void)_tryUpdateDataStatus:(ABI18_0_0RCTPromiseResolveBlock)resolve
...
- (ABI18_0_0EXVideoPlayerViewController *)_getNewPlayerViewController
{
  if (_data == nil) {
    return nil;
  }
  ABI18_0_0EXVideoPlayerViewController *controller = [[ABI18_0_0EXVideoPlayerViewController alloc] init];
...
- (void)_usePlayerLayer
{
  if (_data) {
    ...
    [self setNativeResizeMode:_nativeResizeMode];
    [_playerLayer addObserver:self forKeyPath:ABI18_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)_removePlayerLayer
{
  ...
  [_playerLayer removeObserver:self forKeyPath:ABI18_0_0EXVideoReadyForDisplayKeyPath];
  _playerLayer = nil;
}
...
{
  if (object == _playerLayer && [keyPath isEqualToString:ABI18_0_0EXVideoReadyForDisplayKeyPath]) {
      ...
      rejecter:(ABI18_0_0RCTPromiseRejectBlock)reject
{
  ...
  _data = [[ABI18_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                          ...
                          withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                            if (success) {
                              ...
                              [self _removePlayer];
                              if (reject) {
                                reject(@"E_VIDEO_NOTCREATED", nil, ABI18_0_0RCTErrorWithMessage(error));
                              }
             ...
             rejecter:(ABI18_0_0RCTPromiseRejectBlock)reject
{
  if (!_data) {
    ...
    if (reject) {
      reject(@"E_VIDEO_FULLSCREEN", nil, ABI18_0_0RCTErrorWithMessage(@"Fullscreen encountered an error: video is not loaded."));
    }
    return;
  }
  ...
  if (value && !_fullscreenPlayerPresented) {
    ...
    _presentingViewController = ABI18_0_0RCTPresentedViewController();
    [self _callFullscreenCallbackForUpdate:ABI18_0_0EXVideoFullscreenUpdatePlayerWillPresent];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [_presentingViewController presentViewController:_playerViewController animated:YES completion:^{
        ...
        [weakSelf _callFullscreenCallbackForUpdate:ABI18_0_0EXVideoFullscreenUpdatePlayerDidPresent];
        if (resolve) {
...
#pragma mark - ReactABI18_0_0 View Management
...
- (void)insertReactABI18_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  ...
  if (_useNativeControls) {
    ...
    ABI18_0_0RCTLogError(@"video cannot have any subviews");
  }
  return;
}
...
#pragma mark - ABI18_0_0EXVideoPlayerViewControllerDelegate

- (void)videoPlayerViewControllerWillDismiss:(AVPlayerViewController *)playerViewController
{
  if (_playerViewController == playerViewController && _fullscreenPlayerPresented) {
    [self _callFullscreenCallbackForUpdate:ABI18_0_0EXVideoFullscreenUpdatePlayerWillDismiss];
  }
}
...
- (void)videoPlayerViewControllerDidDismiss:(AVPlayerViewController *)playerViewController
{
  if (_playerViewController == playerViewController && _fullscreenPlayerPresented) {
    ...
    _playerViewController = nil;
    [self setUseNativeControls:_useNativeControls];
    [self _callFullscreenCallbackForUpdate:ABI18_0_0EXVideoFullscreenUpdatePlayerDidDismiss];
  }
}
...
{
  return _data == nil ? ABI18_0_0EXAVAudioSessionModeInactive : [_data getAudioSessionModeRequired];
}
