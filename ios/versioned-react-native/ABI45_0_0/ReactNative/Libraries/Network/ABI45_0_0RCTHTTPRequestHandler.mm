...
@interface ABI45_0_0RCTHTTPRequestHandler () <NSURLSessionDataDelegate, ABI45_0_0RCTTurboModule>

@end

static NSURLSessionConfigurationProvider urlSessionConfigurationProvider;

void ABI45_0_0RCTSetCustomNSURLSessionConfigurationProvider(NSURLSessionConfigurationProvider provider) {
  urlSessionConfigurationProvider = provider;
}

@implementation ABI45_0_0RCTHTTPRequestHandler
{
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  dispatch_once(&onceToken, ^{
    // technically, ABI45_0_0RCTHTTPRequestHandler can handle file:// as well,
    // but it's less efficient than using ABI45_0_0RCTFileRequestHandler
                         ...
                         withDelegate:(id<ABI45_0_0RCTURLRequestDelegate>)delegate
{
  ...
  if (!_session && [self isValid]) {
    ...
    //  by providing the following key to your ABI45_0_0RN project (edit ios/project/Info.plist file in Xcode):
    // <key>ABI45_0_0ReactNetworkForceWifiOnly</key>    <true/>
    ...
    NSNumber *useWifiOnly = [infoDictionary objectForKey:@"ABI45_0_0ReactNetworkForceWifiOnly"];
    ...
    if (urlSessionConfigurationProvider) {
      ...
      // Set allowsCellularAccess to NO ONLY if key ABI45_0_0ReactNetworkForceWifiOnly exists AND its value is YES
      if (useWifiOnly) {
...
{
  id<ABI45_0_0RCTURLRequestDelegate> delegate;
  {
    std::lock_guard<std::mutex> lock(_mutex);
...
Class ABI45_0_0RCTHTTPRequestHandlerCls(void) {
  return ABI45_0_0RCTHTTPRequestHandler.class;
}
