               ...
               maximumFontSize:(CGFloat)maximumFontSize
{
  ...
  while (true) {
    [self abi45_0_0_rct_scaleFontSizeWithRatio:ratio
                 minimumFontSize:minimumFontSize
                 maximumFontSize:maximumFontSize];

    ABI45_0_0RCTTextSizeComparisonOptions comparsion =
      [self abi45_0_0_rct_compareToSize:size thresholdRatio:0.01];

    if (
        (comparsion & ABI45_0_0RCTTextSizeComparisonWithinRange) &&
        (comparsion & ABI45_0_0RCTTextSizeComparisonSmaller)
    ) {
      return;
    ...
    if (
      ...
      [self abi45_0_0_rct_scaleFontSizeWithRatio:lastRatioWhichFits
                   minimumFontSize:minimumFontSize
                   maximumFontSize:maximumFontSize];
      return;
    }
  ...
  [self enumerateAttribute:NSFontAttributeName
    ...
    ^(UIFont *_Nullable font, NSRange range, BOOL *_Nonnull stop) {
      ...
      if (scaledFont) {
        ...
        ABI45_0_0RCTLogError(@"Font \"%@"" doesn't support automatic scaling.", font.familyName);
      }
    }
  ];
