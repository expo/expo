...
@implementation ABI45_0_0RCTTextShadowView
{
...
- (instancetype)initWithBridge:(ABI45_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI45_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI45_0_0RCTTextShadowViewMeasure);
    ABI45_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI45_0_0RCTTextShadowViewBaseline);
  }
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  // because the background color is drawn on the ABI45_0_0RCTTextView itself, as well
  // as on the glphy background draw step. By setting this to nil, we allow
  // the ABI45_0_0RCTTextView backgroundColor to be used, without affecting nested Text
  // components.
...
{
  if (ABI45_0_0YGNodeIsDirty(self.yogaNode)) {
    return;
  }
  ...
  NSNumber *tag = self.ABI45_0_0ReactTag;
  NSMutableArray<NSNumber *> *descendantViewTags = [NSMutableArray new];
  [textStorage enumerateAttribute:ABI45_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:
   ^(ABI45_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
     if (!shadowView) {
       return;
     }

     [descendantViewTags addObject:shadowView.ABI45_0_0ReactTag];
   }
  ];

  [_bridge.uiManager addUIBlock:^(ABI45_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI45_0_0RCTTextView *textView = (ABI45_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI45_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName range:NSMakeRange(0, textStorage.length)];
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI45_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                               inRange:NSMakeRange(0, attributedText.length)
                               options:0
                            usingBlock:
    ^(ABI45_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                                        ...
                                        exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage abi45_0_0_rct_scaleFontSizeToFitSize:size
...
- (void)layoutWithMetrics:(ABI45_0_0RCTLayoutMetrics)layoutMetrics
...
- (void)layoutSubviewsWithContext:(ABI45_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage enumerateAttribute:ABI45_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          ...
                          options:0
                       usingBlock:
    ^(ABI45_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
      if (!shadowView) {
        return;
      }
      ...
      CGRect frame = {{
        ABI45_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI45_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)
      }, {
        ABI45_0_0RCTRoundPixelValue(attachmentSize.width),
        ABI45_0_0RCTRoundPixelValue(attachmentSize.height)
      }};
      
      NSRange truncatedGlyphRange = [layoutManager truncatedGlyphRangeInLineFragmentForGlyphAtIndex:range.location];
      BOOL viewIsTruncated = NSIntersectionRange(range, truncatedGlyphRange).length != 0;

      ABI45_0_0RCTLayoutContext localLayoutContext = layoutContext;
      ...
      ABI45_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
      localLayoutMetrics.frame.origin = frame.origin; // Reinforcing a proper frame origin for the Shadow View.
      if (viewIsTruncated) {
        localLayoutMetrics.displayType = ABI45_0_0RCTDisplayTypeNone;
      }
...
static ABI45_0_0YGSize ABI45_0_0RCTTextShadowViewMeasure(ABI45_0_0YGNodeRef node, float width, ABI45_0_0YGMeasureMode widthMode, float height, ABI45_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
    widthMode == ABI45_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI45_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    heightMode == ABI45_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI45_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI45_0_0RCTTextShadowView *shadowTextView = (__bridge ABI45_0_0RCTTextShadowView *)ABI45_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
    MIN(ABI45_0_0RCTCeilPixelValue(size.width), maximumSize.width),
    MIN(ABI45_0_0RCTCeilPixelValue(size.height), maximumSize.height)
  };
  ...
  return (ABI45_0_0YGSize){
    ABI45_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
    ABI45_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)
  };
}

static float ABI45_0_0RCTTextShadowViewBaseline(ABI45_0_0YGNodeRef node, const float width, const float height)
{
  ABI45_0_0RCTTextShadowView *shadowTextView = (__bridge ABI45_0_0RCTTextShadowView *)ABI45_0_0YGNodeGetContext(node);

  CGSize size = (CGSize){
    ABI45_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    ABI45_0_0RCTCoreGraphicsFloatFromYogaFloat(height)
  };

  CGFloat lastBaseline = [shadowTextView lastBaselineForSize:size];

  return ABI45_0_0RCTYogaFloatFromCoreGraphicsFloat(lastBaseline);
}
