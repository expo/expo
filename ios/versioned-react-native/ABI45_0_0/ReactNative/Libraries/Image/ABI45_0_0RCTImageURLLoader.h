...
typedef void (^ABI45_0_0RCTImageLoaderProgressBlock)(int64_t progress, int64_t total);
typedef void (^ABI45_0_0RCTImageLoaderPartialLoadBlock)(UIImage *image);
typedef void (^ABI45_0_0RCTImageLoaderCompletionBlock)(NSError * _Nullable error, UIImage * _Nullable image);
// Metadata is passed as a id in an additional parameter because there are forks of ABI45_0_0RN without this parameter,
// and the complexity of ABI45_0_0RCTImageLoader would make using protocols here difficult to typecheck.
typedef void (^ABI45_0_0RCTImageLoaderCompletionBlockWithMetadata)(NSError * _Nullable error, UIImage * _Nullable image, id _Nullable metadata);
typedef dispatch_block_t ABI45_0_0RCTImageLoaderCancellationBlock;
 ...
 */
@protocol ABI45_0_0RCTImageURLLoader <ABI45_0_0RCTBridgeModule>
...
/**
 * If more than one ABI45_0_0RCTImageURLLoader responds YES to `-canLoadImageURL:`
 ...
 *
 * Use with care: disabling scheduling will reduce ABI45_0_0RCTImageLoader's ability to throttle
 * network requests.
 */
