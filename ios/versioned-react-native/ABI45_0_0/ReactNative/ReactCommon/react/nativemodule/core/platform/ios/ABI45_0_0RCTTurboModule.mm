...
#import "ABI45_0_0RCTBlockGuard.h"
...
#import <ABI45_0_0ReactCommon/ABI45_0_0LongLivedObject.h>
...
static ABI45_0_0RCTResponseSenderBlock convertJSIFunctionToCallback(
    jsi::Runtime &runtime,
    ...
    ABI45_0_0RCTRetainJSCallback retainJSCallback)
{
  ...
  ABI45_0_0RCTBlockGuard *blockGuard = [[ABI45_0_0RCTBlockGuard alloc] initWithCleanup:^() {
  ...
  ABI45_0_0RCTResponseSenderBlock callback = ^(NSArray *responses) {
    if (wrapperWasCalled) {
  ...
  jsi::Function fn = jsi::Function::createFromHostFunction(
      ...
      [invokeCopy, jsInvoker = jsInvoker_, moduleName, methodName, retainJSCallback = retainJSCallback_](
        ...
        ABI45_0_0RCTPromiseResolveBlock resolveBlock = ^(id result) {
          if (rejectWasCalled) {
            ABI45_0_0RCTLogError(@"%s: Tried to resolve a promise after it's already been rejected.", moduleMethod.c_str());
            return;
          }

          if (resolveWasCalled) {
            ABI45_0_0RCTLogError(@"%s: Tried to resolve a promise more than once.", moduleMethod.c_str());
            return;
          }
        ...
        ABI45_0_0RCTPromiseRejectBlock rejectBlock = ^(NSString *code, NSString *message, NSError *error) {
          if (resolveWasCalled) {
            ABI45_0_0RCTLogError(@"%s: Tried to reject a promise after it's already been resolved.", moduleMethod.c_str());
            return;
          }

          if (rejectWasCalled) {
            ABI45_0_0RCTLogError(@"%s: Tried to reject a promise more than once.", moduleMethod.c_str());
            return;
          }
 ...
 * This serves as a backward-compatible support for ABI45_0_0RCTBridgeModule's methodQueue API.
 *
 * In the future:
    ...
    NSMutableArray *retainedObjectsForInvocation)
{
  ...
  __weak id<ABI45_0_0RCTTurboModule> weakModule = instance_;
  ...
  void (^block)() = ^{
    id<ABI45_0_0RCTTurboModule> strongModule = weakModule;
    if (!strongModule) {
      return;
    }
  ...
  if (!methodArgumentTypeNames_) {
    ...
    if (methods) {
      for (unsigned int i = 0; i < numberOfMethods; i++) {
        ...
        ABI45_0_0RCTMethodInfo *(*getMethodInfo)(id, SEL) = (__typeof__(getMethodInfo))objc_msgSend;
        ABI45_0_0RCTMethodInfo *methodInfo = getMethodInfo(cls, s);

        NSArray<ABI45_0_0RCTMethodArgument *> *arguments;
        NSString *otherMethodName = ABI45_0_0RCTParseMethodSignature(methodInfo->objcName, &arguments);
...
{
  const char *moduleName = name_.c_str();
  const id<ABI45_0_0RCTTurboModule> module = instance_;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (objCArg) {
      ...
      /**
       * Convert objects using ABI45_0_0RCTConvert.
       */
      if (objCArgType == @encode(id)) {
        ...
        if (argumentType != nil) {
          ...
          if ([ABI45_0_0RCTConvert respondsToSelector:rctConvertSelector]) {
            // Message dispatch logic from old infra
            id (*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
            id convertedObjCArg = convert([ABI45_0_0RCTConvert class], rctConvertSelector, objCArg);
      ...
      /**
       * Convert objects using ABI45_0_0RCTCxxConvert to structs.
       */
      if ([objCArg isKindOfClass:[NSDictionary class]] && hasMethodArgConversionSelector(methodNameNSString, i)) {
        ...
        ABI45_0_0RCTManagedPointer *(*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
        ABI45_0_0RCTManagedPointer *box = convert([ABI45_0_0RCTCxxConvert class], methodArgConversionSelector, objCArg);
    ...
    size_t count)
{
  ...
  jsi::Value returnValue = returnType == PromiseKind
      ? createPromise(
            runtime,
            methodNameStr,
            ^(ABI45_0_0RCTPromiseResolveBlock resolveBlock, ABI45_0_0RCTPromiseRejectBlock rejectBlock) {
              ABI45_0_0RCTPromiseResolveBlock resolveCopy = [resolveBlock copy];
              ABI45_0_0RCTPromiseRejectBlock rejectCopy = [rejectBlock copy];
