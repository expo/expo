...
#import "ABI45_0_0RCTTurboModuleManager.h"
...
#import <ABI45_0_0ReactCommon/ABI45_0_0TurboCxxModule.h>
#import <ABI45_0_0ReactCommon/ABI45_0_0TurboModuleBinding.h>
#import <ABI45_0_0ReactCommon/ABI45_0_0TurboModulePerfLogger.h>

using namespace ABI45_0_0facebook::ABI45_0_0React;

/**
 * A global variable whose address we use to associate method queues to id<ABI45_0_0RCTTurboModule> objects.
 */
...
class TurboModuleHolder {
 private:
  ...
  id<ABI45_0_0RCTTurboModule> module_;
 ...
 public:
  ...
  void setModule(id<ABI45_0_0RCTTurboModule> module)
  {
    module_ = module;
  }

  id<ABI45_0_0RCTTurboModule> getModule() const
  {
    return module_;
  }
...
class MethodQueueNativeCallInvoker : public CallInvoker {
 ...
 public:
  ...
  {
    if (methodQueue_ == ABI45_0_0RCTJSThread) {
      work();
      return;
    }
...
// Fallback lookup since ABI45_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI45_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI45_0_0RCTTurboModuleManager {
  std::shared_ptr<CallInvoker> _jsInvoker;
  __weak id<ABI45_0_0RCTTurboModuleManagerDelegate> _delegate;
  ...
  ABI45_0_0RCTRetainJSCallback _retainJSCallback;
  ...
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeWillInvalidateModules:)
                                                 name:ABI45_0_0RCTBridgeWillInvalidateModulesNotification
                                               object:_bridge.parentBridge];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidInvalidateModules:)
                                                 name:ABI45_0_0RCTBridgeDidInvalidateModulesNotification
                                               object:_bridge.parentBridge];

    if (ABI45_0_0RCTGetTurboModuleCleanupMode() == kABI45_0_0RCTGlobalScope) {
      // Use LongLivedObjectCollection singleton regularly
      _retainJSCallback = nil;
    } else if (ABI45_0_0RCTGetTurboModuleCleanupMode() == kABI45_0_0RCTGlobalScopeUsingRetainJSCallback) {
    ...
    } else if (ABI45_0_0RCTGetTurboModuleCleanupMode() == kABI45_0_0RCTTurboModuleManagerScope) {
...
- (void)notifyAboutTurboModuleSetup:(const char *)name
{
  ...
  if (moduleName) {
    int64_t setupTime = [self->_bridge.performanceLogger durationForTag:ABI45_0_0RCTPLTurboModuleSetup];
...
- (std::shared_ptr<TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  id<ABI45_0_0RCTTurboModule> module = [self provideABI45_0_0RCTTurboModule:moduleName];

  TurboModulePerfLogger::moduleJSRequireEndingStart(moduleName);

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI45_0_0RCTTurboModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  if (methodQueue == nil) {
    ABI45_0_0RCTLogError(@"TurboModule \"%@\" was not associated with a method queue.", moduleClass);
  }
  ...
  /**
   * Have ABI45_0_0RCTCxxBridge decorate native CallInvoker, so that it's aware of TurboModule async method calls.
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI45_0_0RCTCxxModule.class]) {
    ...
    auto turboModule = std::make_shared<TurboCxxModule>([((ABI45_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
  ...
  ObjCTurboModule::InitParams params = {
      ...
      .isSyncModule = methodQueue == ABI45_0_0RCTJSThread,
  ...
  if (turboModule == nullptr) {
    ABI45_0_0RCTLogError(@"TurboModule \"%@\"'s getTurboModule: method returned nil.", moduleClass);
  }
  _turboModuleCache.insert({moduleName, turboModule});
  return turboModule;
}
...
{
  if (ABI45_0_0RCTTurboModuleSharedMutexInitEnabled()) {
    {
 ...
 * (for now).
 */
- (id<ABI45_0_0RCTTurboModule>)provideABI45_0_0RCTTurboModule:(const char *)moduleName
{
  if (strncmp("ABI45_0_0RCT", moduleName, 3) == 0) {
  ...
  id<ABI45_0_0RCTTurboModule> module = [self _provideABI45_0_0RCTTurboModule:moduleName moduleHolder:moduleHolder shouldPerfLog:YES];
...
- (id<ABI45_0_0RCTTurboModule>)_provideABI45_0_0RCTTurboModule:(const char *)moduleName
                               ...
                               shouldPerfLog:(BOOL)shouldPerfLog
{
  ...
  if (shouldCreateModule) {
    ...
    if ([_delegate respondsToSelector:@selector(getModuleClassFromName:)]) {
      if (ABI45_0_0RCTTurboModuleManagerDelegateLockingDisabled()) {
    ...
    __block id<ABI45_0_0RCTTurboModule> module = nil;

    if ([moduleClass conformsToProtocol:@protocol(ABI45_0_0RCTTurboModule)]) {
      ...
      dispatch_block_t work = ^{
        ...
        module = [strongSelf _createAndSetUpABI45_0_0RCTTurboModule:moduleClass
                                                moduleName:moduleName
      ...
      if ([self _requiresMainQueueSetup:moduleClass]) {
         ...
         */
        if (ABI45_0_0RCTTurboModuleEagerInitEnabled() && !ABI45_0_0RCTIsMainQueue()) {
          ABI45_0_0RCTLogWarn(
              @"TurboModule \"%@\" requires synchronous dispatch onto the main queue to be initialized. This may lead to deadlock.",
              moduleClass);
        }

        ABI45_0_0RCTUnsafeExecuteOnMainQueueSync(work);
      } else {
        work();
      }
    }
 ...
 *  - The thread that calls provideABI45_0_0RCTTurboModule:
 *  - The main thread (if the TurboModule requires main queue init), blocking the thread that calls
 * provideABI45_0_0RCTTurboModule:.
 */
- (id<ABI45_0_0RCTTurboModule>)_createAndSetUpABI45_0_0RCTTurboModule:(Class)moduleClass
                                         moduleName:(const char *)moduleName
                                           moduleId:(int32_t)moduleId
{
  id<ABI45_0_0RCTTurboModule> module = nil;
   ...
   * implementation, and a `- (ABI45_0_0RCTBridge *) bridge { ... }` method won't be
  ...
  if ([module respondsToSelector:@selector(bridge)] && _bridge) {
    ...
    @try {
      /**
       * ABI45_0_0RCTBridgeModule declares the bridge property as readonly.
       * Therefore, when authors of NativeModules synthesize the bridge
       * via @synthesize bridge = bridge;, the ObjC runtime generates
       * only a - (ABI45_0_0RCTBridge *) bridge: { ... } method. No setter is
      ...
      ABI45_0_0RCTLogError(
          ...
          ABI45_0_0RCTBridgeModuleNameForClass([module class]));
    }
  }
  ...
  if (moduleHasMethodQueueGetter) {
    methodQueue = [(id<ABI45_0_0RCTBridgeModule>)module methodQueue];
  }

  /**
   * Note: ABI45_0_0RCTJSThread, which is a valid method queue, is defined as (id)kCFNull. It should rightfully not enter the
   * following if condition's block.
   */
  if (!methodQueue) {
    NSString *methodQueueName = [NSString stringWithFormat:@"com.facebook.ABI45_0_0React.%sQueue", moduleName];
  ...
  /**
   * If the TurboModule conforms to ABI45_0_0RCTInitializing, invoke its initialize method.
   */
  if ([module respondsToSelector:@selector(initialize)]) {
    [(id<ABI45_0_0RCTInitializing>)module initialize];
  }

  /**
   * Attach method queue to id<ABI45_0_0RCTTurboModule> object.
   * This is necessary because the id<ABI45_0_0RCTTurboModule> object can be eagerly created/initialized before the method
   * queue is required. The method queue is required for an id<ABI45_0_0RCTTurboModule> for JS -> Native calls. So, we need it
   * before we create the id<ABI45_0_0RCTTurboModule>'s TurboModule jsi::HostObject in provideTurboModule:.
   */
  objc_setAssociatedObject(module, &kAssociatedMethodQueueKey, methodQueue, OBJC_ASSOCIATION_RETAIN);

  /**
   * NativeModules that implement the ABI45_0_0RCTFrameUpdateObserver protocol
   * require registration with ABI45_0_0RCTDisplayLink.
   *
   * TODO(T55504345): Investigate whether we can improve this after TM
   * rollout.
   */
  if (_bridge) {
    ABI45_0_0RCTModuleData *data = [[ABI45_0_0RCTModuleData alloc] initWithModuleInstance:(id<ABI45_0_0RCTBridgeModule>)module
                                                                 bridge:_bridge
                                                         moduleRegistry:_bridge.moduleRegistry
                                                viewRegistry_DEPRECATED:nil
                                                          bundleManager:nil
                                                      callableJSModules:nil];
    [_bridge registerModuleForFrameUpdates:(id<ABI45_0_0RCTBridgeModule>)module withModuleData:data];
  }
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI45_0_0RCTDidInitializeModuleNotification
                    object:_bridge
                  userInfo:@{@"module" : module, @"bridge" : ABI45_0_0RCTNullIfNil([_bridge parentBridge])}];
...
- (void)installJSBindingWithRuntimeExecutor:(ABI45_0_0facebook::ABI45_0_0React::RuntimeExecutor)runtimeExecutor
{
   ...
   */
  auto turboModuleProvider = [self](const std::string &name) -> std::shared_ptr<ABI45_0_0React::TurboModule> {
    auto moduleName = name.c_str();

    TurboModulePerfLogger::moduleJSRequireBeginningStart(moduleName);
    auto moduleWasNotInitialized = ![self moduleIsInitialized:moduleName];
    if (moduleWasNotInitialized) {
      [self->_bridge.performanceLogger markStartForTag:ABI45_0_0RCTPLTurboModuleSetup];
    }
    ...
    if (moduleWasNotInitialized && [self moduleIsInitialized:moduleName]) {
      [self->_bridge.performanceLogger markStopForTag:ABI45_0_0RCTPLTurboModuleSetup];
  ...
  if (ABI45_0_0RCTGetTurboModuleCleanupMode() == kABI45_0_0RCTGlobalScope ||
      ABI45_0_0RCTGetTurboModuleCleanupMode() == kABI45_0_0RCTGlobalScopeUsingRetainJSCallback) {
    runtimeExecutor([turboModuleProvider = std::move(turboModuleProvider)](jsi::Runtime &runtime) {
      ABI45_0_0React::TurboModuleBinding::install(runtime, std::move(turboModuleProvider));
    });
  } else if (ABI45_0_0RCTGetTurboModuleCleanupMode() == kABI45_0_0RCTTurboModuleManagerScope) {
    runtimeExecutor([turboModuleProvider = std::move(turboModuleProvider),
      ...
      ABI45_0_0React::TurboModuleBinding::install(runtime, std::move(turboModuleProvider), longLivedObjectCollection);
    });
  }
}

#pragma mark ABI45_0_0RCTTurboModuleRegistry
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  if (warnOnLookupFailure && !module) {
    ABI45_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
{
  // Backward-compatibility: ABI45_0_0RCTInvalidating handling.
  ...
  for (auto &pair : _turboModuleHolders) {
    ...
    /**
     * We could start tearing down ABI45_0_0ReactNative before a TurboModule is fully initialized. In this case, we should wait
     * for TurboModule init to finish before calling invalidate on it. So, we call _provideABI45_0_0RCTTurboModule:moduleHolder,
     * because it's guaranteed to return a fully initialized NativeModule.
     */
    id<ABI45_0_0RCTTurboModule> module = [self _provideABI45_0_0RCTTurboModule:moduleName.c_str()
    ...
    if ([module respondsToSelector:@selector(invalidate)]) {
      ...
      dispatch_block_t invalidateModule = ^{
        [((id<ABI45_0_0RCTInvalidating>)module) invalidate];
      ...
      if (_bridge) {
        ...
        // Bridgeless mode
        if (methodQueue == ABI45_0_0RCTJSThread) {
          invalidateModule();
        } else {
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI45_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
