...
#include "ABI45_0_0UIManagerBinding.h"
...
void UIManagerBinding::dispatchEvent(
  ...
  auto instanceHandle = eventTarget
    ? [&]() {
      ...
      ABI45_0_0React_native_assert(payload.isObject());
  ...
  eventHandlerWrapper.callback.call(
      ...
      {std::move(instanceHandle),
       ...
       std::move(payload)});
  currentEventPriority_ = ABI45_0_0ReactEventPriority::Default;
}
...
jsi::Value UIManagerBinding::get(
  ...
  if (methodName == "unstable_DefaultEventPriority") {
    return jsi::Value(serialize(ABI45_0_0ReactEventPriority::Default));
  }

  if (methodName == "unstable_DiscreteEventPriority") {
    return jsi::Value(serialize(ABI45_0_0ReactEventPriority::Discrete));
  }
