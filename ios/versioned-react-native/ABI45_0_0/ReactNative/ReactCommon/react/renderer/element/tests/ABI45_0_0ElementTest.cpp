...
TEST(ElementTest, testNormalCases) {
  ...
  ABI45_0_0EXPECT_EQ(shadowNode, shadowNodeA);

  // Tags
  ABI45_0_0EXPECT_EQ(shadowNodeA->getTag(), 1);
  ABI45_0_0EXPECT_EQ(shadowNodeAA->getTag(), 2);
  ABI45_0_0EXPECT_EQ(shadowNodeAB->getTag(), 3);
  ABI45_0_0EXPECT_EQ(shadowNodeABA->getTag(), 4);

  // Children
  ABI45_0_0EXPECT_EQ(shadowNodeA->getChildren().size(), 2);
  ABI45_0_0EXPECT_EQ(shadowNodeAA->getChildren().size(), 0);
  ABI45_0_0EXPECT_EQ(shadowNodeAB->getChildren().size(), 1);
  ABI45_0_0EXPECT_EQ(shadowNodeABA->getChildren().size(), 0);
  ABI45_0_0EXPECT_EQ(
  ...
  // Props
  ABI45_0_0EXPECT_EQ(shadowNodeA->getProps()->nativeId, "node A");
  ABI45_0_0EXPECT_EQ(shadowNodeABA->getProps()->nativeId, "node ABA");
  ABI45_0_0EXPECT_EQ(shadowNodeAA->getProps(), propsAA);

  // Finalize
  ABI45_0_0EXPECT_TRUE(shadowNodeA->getSealed());
  ABI45_0_0EXPECT_TRUE(shadowNodeAA->getSealed());
  ABI45_0_0EXPECT_TRUE(shadowNodeAB->getSealed());
  ABI45_0_0EXPECT_TRUE(shadowNodeABA->getSealed());
}
