...
TEST(DynamicPropsUtilitiesTest, handleNestedObjects) {
  ...
  ABI45_0_0EXPECT_TRUE(result["style"].isObject());
  ABI45_0_0EXPECT_TRUE(result["style"]["backgroundColor"].isString());
  ABI45_0_0EXPECT_TRUE(result["style"]["color"].isString());
  ABI45_0_0EXPECT_TRUE(result["height"].isInt());

  ABI45_0_0EXPECT_EQ(result["style"]["backgroundColor"].asString(), "blue");
  ABI45_0_0EXPECT_EQ(result["style"]["color"], "black");
  ABI45_0_0EXPECT_EQ(result["height"], 100);
}
...
TEST(DynamicPropsUtilitiesTest, handleNull) {
  ...
  ABI45_0_0EXPECT_TRUE(result["height"].isNull());
}
