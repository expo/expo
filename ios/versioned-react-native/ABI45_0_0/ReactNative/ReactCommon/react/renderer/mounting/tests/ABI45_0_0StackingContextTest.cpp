...
TEST_F(StackingContextTest, defaultPropsMakeEverythingFlattened) {
  testViewTree_([](StubViewTree const &viewTree) {
    // 1 view in total.
    ABI45_0_0EXPECT_EQ(viewTree.size(), 1);

    // The root view has no subviews.
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.size(), 0);
  });
}
...
TEST_F(StackingContextTest, mostPropsDoNotForceViewsToMaterialize) {
  ...
  mutateViewShadowNodeProps_(nodeAA_, [](ViewProps &props) {
    ...
    yogaStyle.padding()[ABI45_0_0YGEdgeAll] = ABI45_0_0YGValue{42, ABI45_0_0YGUnitPoint};
    yogaStyle.margin()[ABI45_0_0YGEdgeAll] = ABI45_0_0YGValue{42, ABI45_0_0YGUnitPoint};
  ...
  mutateViewShadowNodeProps_(nodeBBA_, [](ViewProps &props) {
    auto &yogaStyle = props.yogaStyle;
    yogaStyle.positionType() = ABI45_0_0YGPositionTypeRelative;
...
TEST_F(StackingContextTest, somePropsForceViewsToMaterialize1) {
  ...
  testViewTree_([](StubViewTree const &viewTree) {
    // 4 views in total.
    ABI45_0_0EXPECT_EQ(viewTree.size(), 4);

    // The root view has all 3 subviews.
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.size(), 3);

    // The root view subviews are [3, 5, 7].
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(0)->tag, 3);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(1)->tag, 5);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(2)->tag, 7);
  });
}
...
TEST_F(StackingContextTest, somePropsForceViewsToMaterialize2) {
  ...
  testViewTree_([](StubViewTree const &viewTree) {
    // 10 views in total.
    ABI45_0_0EXPECT_EQ(viewTree.size(), 10);

    // The root view has all 9 subviews.
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.size(), 9);
  });
}
...
TEST_F(StackingContextTest, zIndexAndFlattenedNodes) {
  ...
  testViewTree_([](StubViewTree const &viewTree) {
    // 7 views in total.
    ABI45_0_0EXPECT_EQ(viewTree.size(), 7);

    // The root view has all 6 subviews.
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.size(), 6);

    // The root view subviews are [10, 9, 8, 7, 5, 3].
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(0)->tag, 10);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(1)->tag, 9);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(2)->tag, 8);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(3)->tag, 7);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(4)->tag, 5);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(5)->tag, 3);
  });
  ...
  testViewTree_([](StubViewTree const &viewTree) {
    // 8 views in total.
    ABI45_0_0EXPECT_EQ(viewTree.size(), 8);

    // The root view has 5 subviews.
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.size(), 5);

    // The root view subviews are [6, 10, 9, 5, 3].
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(0)->tag, 6);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(1)->tag, 10);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(2)->tag, 9);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(3)->tag, 5);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(4)->tag, 3);

    auto &view6 = viewTree.getStubView(6);
    ABI45_0_0EXPECT_EQ(view6.children.size(), 2);
    ABI45_0_0EXPECT_EQ(view6.children.at(0)->tag, 8);
    ABI45_0_0EXPECT_EQ(view6.children.at(1)->tag, 7);
  });
  ...
  mutateViewShadowNodeProps_(nodeBB_, [](ViewProps &props) {
    auto &yogaStyle = props.yogaStyle;
    yogaStyle.positionType() = ABI45_0_0YGPositionTypeStatic;
    props.zIndex = {};
  });
  ...
  mutateViewShadowNodeProps_(nodeBB_, [](ViewProps &props) {
    auto &yogaStyle = props.yogaStyle;
    yogaStyle.display() = ABI45_0_0YGDisplayNone;
  });

  testViewTree_([](StubViewTree const &viewTree) {
    // 5 views in total.
    ABI45_0_0EXPECT_EQ(viewTree.size(), 5);

    // The root view has all 4 subviews.
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.size(), 4);

    // The root view subviews are [10, 9, 5, 3].
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(0)->tag, 10);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(1)->tag, 9);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(2)->tag, 5);
    ABI45_0_0EXPECT_EQ(viewTree.getRootStubView().children.at(3)->tag, 3);
  });
}
