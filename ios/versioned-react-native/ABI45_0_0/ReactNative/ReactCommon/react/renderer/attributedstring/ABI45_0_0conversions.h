...
inline void fromRawValue(
    ...
    const RawValue &value,
    FontVariant &result) {
  ABI45_0_0React_native_assert(value.hasType<std::vector<std::string>>());
...
inline folly::dynamic toDynamic(const AttributedString &attributedString) {
  ...
  for (auto fragment : attributedString.getFragments()) {
    ...
    if (fragment.parentShadowView.componentHandle) {
      dynamicFragment["ABI45_0_0ReactTag"] = fragment.parentShadowView.tag;
    }
  ...
  value(
      "hash", std::hash<ABI45_0_0facebook::ABI45_0_0React::AttributedString>{}(attributedString));
...
inline MapBuffer toMapBuffer(const AttributedString &attributedString) {
  ...
  builder.putInt(
      AS_KEY_HASH,
      std::hash<ABI45_0_0facebook::ABI45_0_0React::AttributedString>{}(attributedString));
