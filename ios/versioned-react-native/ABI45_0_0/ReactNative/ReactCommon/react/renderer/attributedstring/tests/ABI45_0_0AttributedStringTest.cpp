...
TEST(AttributedStringTest, testToDynamic) {
  ...
  ABI45_0_0EXPECT_EQ(result["string"], fragment.string);
  auto textAttribute = result["fragments"][0]["textAttributes"];
  ABI45_0_0EXPECT_EQ(textAttribute["foregroundColor"], toDynamic(text.foregroundColor));
  ABI45_0_0EXPECT_EQ(textAttribute["opacity"], text.opacity);
  ABI45_0_0EXPECT_EQ(textAttribute["fontStyle"], toString(text.fontStyle.value()));
  ABI45_0_0EXPECT_EQ(textAttribute["fontWeight"], toString(text.fontWeight.value()));
}
