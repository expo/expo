...
#import <ABI45_0_0React/ABI45_0_0renderer/components/iostextinput/TextInputComponentDescriptor.h>
...
#import <ABI45_0_0React/ABI45_0_0RCTUITextView.h>
#import <ABI45_0_0React/ABI45_0_0RCTUtils.h>

#import "ABI45_0_0RCTConversions.h"
#import "ABI45_0_0RCTTextInputNativeCommands.h"
...
@interface ABI45_0_0RCTTextInputComponentView () <ABI45_0_0RCTBackedTextInputDelegate, ABI45_0_0RCTTextInputViewProtocol>
@end

@implementation ABI45_0_0RCTTextInputComponentView {
  TextInputShadowNode::ConcreteState::Shared _state;
  UIView<ABI45_0_0RCTBackedTextInputViewProtocol> *_backedTextInputView;
   ...
   * if UITextView is backing text input view, inside `-[ABI45_0_0RCTTextInputComponentView textInputDidChangeSelection]` we make
   * sure to call `onChange` before `onSelectionChange` and ignore next `-[ABI45_0_0RCTTextInputComponentView
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _backedTextInputView = props.traits.multiline ? [ABI45_0_0RCTUITextView new] : [ABI45_0_0RCTUITextField new];
...
#pragma mark - ABI45_0_0RCTViewComponentView overrides
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  if (newTextInputProps.traits.autocapitalizationType != oldTextInputProps.traits.autocapitalizationType) {
    _backedTextInputView.autocapitalizationType =
        ABI45_0_0RCTUITextAutocapitalizationTypeFromAutocapitalizationType(newTextInputProps.traits.autocapitalizationType);
  }

  if (newTextInputProps.traits.autoCorrect != oldTextInputProps.traits.autoCorrect) {
    _backedTextInputView.autocorrectionType =
        ABI45_0_0RCTUITextAutocorrectionTypeFromOptionalBool(newTextInputProps.traits.autoCorrect);
  }
  ...
  if (newTextInputProps.traits.keyboardAppearance != oldTextInputProps.traits.keyboardAppearance) {
    _backedTextInputView.keyboardAppearance =
        ABI45_0_0RCTUIKeyboardAppearanceFromKeyboardAppearance(newTextInputProps.traits.keyboardAppearance);
  }

  if (newTextInputProps.traits.spellCheck != oldTextInputProps.traits.spellCheck) {
    _backedTextInputView.spellCheckingType =
        ABI45_0_0RCTUITextSpellCheckingTypeFromOptionalBool(newTextInputProps.traits.spellCheck);
  }
  ...
  if (newTextInputProps.traits.clearButtonMode != oldTextInputProps.traits.clearButtonMode) {
    _backedTextInputView.clearButtonMode =
        ABI45_0_0RCTUITextFieldViewModeFromTextInputAccessoryVisibilityMode(newTextInputProps.traits.clearButtonMode);
  }
  ...
  if (newTextInputProps.traits.keyboardType != oldTextInputProps.traits.keyboardType) {
    _backedTextInputView.keyboardType = ABI45_0_0RCTUIKeyboardTypeFromKeyboardType(newTextInputProps.traits.keyboardType);
  }

  if (newTextInputProps.traits.returnKeyType != oldTextInputProps.traits.returnKeyType) {
    _backedTextInputView.returnKeyType = ABI45_0_0RCTUIReturnKeyTypeFromReturnKeyType(newTextInputProps.traits.returnKeyType);
  }

  if (newTextInputProps.traits.textContentType != oldTextInputProps.traits.textContentType) {
    _backedTextInputView.textContentType = ABI45_0_0RCTUITextContentTypeFromString(newTextInputProps.traits.textContentType);
  }

  if (newTextInputProps.traits.passwordRules != oldTextInputProps.traits.passwordRules) {
    if (@available(iOS 12.0, *)) {
      _backedTextInputView.passwordRules =
          ABI45_0_0RCTUITextInputPasswordRulesFromString(newTextInputProps.traits.passwordRules);
    }
  }
  ...
  if (newTextInputProps.placeholder != oldTextInputProps.placeholder) {
    _backedTextInputView.placeholder = ABI45_0_0RCTNSStringFromString(newTextInputProps.placeholder);
  }

  if (newTextInputProps.placeholderTextColor != oldTextInputProps.placeholderTextColor) {
    _backedTextInputView.placeholderColor = ABI45_0_0RCTUIColorFromSharedColor(newTextInputProps.placeholderTextColor);
  }

  if (newTextInputProps.textAttributes != oldTextInputProps.textAttributes) {
    _backedTextInputView.defaultTextAttributes =
        ABI45_0_0RCTNSTextAttributesFromTextAttributes(newTextInputProps.getEffectiveTextAttributes(ABI45_0_0RCTFontSizeMultiplier()));
  }

  if (newTextInputProps.selectionColor != oldTextInputProps.selectionColor) {
    _backedTextInputView.tintColor = ABI45_0_0RCTUIColorFromSharedColor(newTextInputProps.selectionColor);
  }

  if (newTextInputProps.inputAccessoryViewID != oldTextInputProps.inputAccessoryViewID) {
    _backedTextInputView.inputAccessoryViewID = ABI45_0_0RCTNSStringFromString(newTextInputProps.inputAccessoryViewID);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  if (_mostRecentEventCount == _state->getData().mostRecentEventCount) {
    _comingFromJS = YES;
    [self _setAttributedString:ABI45_0_0RCTNSAttributedStringFromAttributedStringBox(data.attributedStringBox)];
    _comingFromJS = NO;
  }
}
           ...
           oldLayoutMetrics:(LayoutMetrics const &)oldLayoutMetrics
{
  ...
  _backedTextInputView.frame =
      UIEdgeInsetsInsetRect(self.bounds, ABI45_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.borderWidth));
  _backedTextInputView.textContainerInset =
      ABI45_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.contentInsets - layoutMetrics.borderWidth);
}
...
- (NSString *)textInputShouldChangeText:(NSString *)text inRange:(NSRange)range
{
  ...
  if (!_backedTextInputView.textWasPasted) {
    if (_eventEmitter) {
      ...
      keyPressMetrics.text = ABI45_0_0RCTStringFromNSString(text);
...
#pragma mark - ABI45_0_0RCTBackedTextInputDelegate (UIScrollViewDelegate)
...
{
  ABI45_0_0RCTTextInputHandleCommand(self, commandName, args);
}
...
- (TextInputMetrics)_textInputMetrics
{
  ...
  metrics.text = ABI45_0_0RCTStringFromNSString(_backedTextInputView.attributedText.string);
...
- (void)_updateState
{
  ...
  data.attributedStringBox = ABI45_0_0RCTAttributedStringBoxFromNSAttributedString(attributedString);
...
- (void)_setMultiline:(BOOL)multiline
{
  ...
  UIView<ABI45_0_0RCTBackedTextInputViewProtocol> *backedTextInputView = multiline ? [ABI45_0_0RCTUITextView new] : [ABI45_0_0RCTUITextField new];
  backedTextInputView.frame = _backedTextInputView.frame;
  ABI45_0_0RCTCopyBackedTextInput(_backedTextInputView, backedTextInputView);
...
Class<ABI45_0_0RCTComponentViewProtocol> ABI45_0_0RCTTextInputCls(void)
{
  return ABI45_0_0RCTTextInputComponentView.class;
}
