...
#import "ABI45_0_0RCTScrollViewComponentView.h"
...
#import <ABI45_0_0React/ABI45_0_0RCTScrollEvent.h>

#import <ABI45_0_0React/ABI45_0_0renderer/components/scrollview/ABI45_0_0RCTComponentViewHelpers.h>
#import <ABI45_0_0React/ABI45_0_0renderer/components/scrollview/ScrollViewComponentDescriptor.h>
#import <ABI45_0_0React/ABI45_0_0renderer/components/scrollview/ScrollViewEventEmitter.h>
#import <ABI45_0_0React/ABI45_0_0renderer/components/scrollview/ScrollViewProps.h>
#import <ABI45_0_0React/ABI45_0_0renderer/components/scrollview/ScrollViewState.h>
#import <ABI45_0_0React/ABI45_0_0renderer/components/scrollview/conversions.h>
#import <ABI45_0_0React/ABI45_0_0renderer/graphics/Geometry.h>

#import "ABI45_0_0RCTConversions.h"
#import "ABI45_0_0RCTEnhancedScrollView.h"
...
static UIScrollViewKeyboardDismissMode ABI45_0_0RCTUIKeyboardDismissModeFromProps(ScrollViewProps const &props)
{
...
static UIScrollViewIndicatorStyle ABI45_0_0RCTUIScrollViewIndicatorStyleFromProps(ScrollViewProps const &props)
{
...
static void ABI45_0_0RCTSendPaperScrollEvent_DEPRECATED(UIScrollView *scrollView, NSInteger tag)
{
  static uint16_t coalescingKey = 0;
  ABI45_0_0RCTScrollEvent *scrollEvent = [[ABI45_0_0RCTScrollEvent alloc] initWithEventName:@"onScroll"
                                                                 ABI45_0_0ReactTag:[NSNumber numberWithInt:tag]
  ...
  [[ABI45_0_0RCTBridge currentBridge].eventDispatcher sendEvent:scrollEvent];
}

@interface ABI45_0_0RCTScrollViewComponentView () <
    UIScrollViewDelegate,
    ABI45_0_0RCTScrollViewProtocol,
    ABI45_0_0RCTScrollableProtocol,
    ABI45_0_0RCTEnhancedScrollViewOverridingDelegate>

@end

@implementation ABI45_0_0RCTScrollViewComponentView {
...
+ (ABI45_0_0RCTScrollViewComponentView *_Nullable)findScrollViewComponentViewForView:(UIView *)view
{
  do {
    view = view.superview;
  } while (view != nil && ![view isKindOfClass:[ABI45_0_0RCTScrollViewComponentView class]]);
  return (ABI45_0_0RCTScrollViewComponentView *)view;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _scrollView = [[ABI45_0_0RCTEnhancedScrollView alloc] initWithFrame:self.bounds];
    _scrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    _scrollView.delaysContentTouches = NO;
    ((ABI45_0_0RCTEnhancedScrollView *)_scrollView).overridingDelegate = self;
...
{
  return ((ABI45_0_0RCTEnhancedScrollView *)_scrollView).delegateSplitter;
}

#pragma mark - ABI45_0_0RCTMountingTransactionObserving
...
#define REMAP_PROP(ABI45_0_0ReactName, localName, target)                      \
  if (oldScrollViewProps.ABI45_0_0ReactName != newScrollViewProps.ABI45_0_0ReactName) { \
    target.localName = newScrollViewProps.ABI45_0_0ReactName;                  \
  }

#define REMAP_VIEW_PROP(ABI45_0_0ReactName, localName) REMAP_PROP(ABI45_0_0ReactName, localName, self)
#define MAP_VIEW_PROP(name) REMAP_VIEW_PROP(name, name)
#define REMAP_SCROLL_VIEW_PROP(ABI45_0_0ReactName, localName) \
  REMAP_PROP(ABI45_0_0ReactName, localName, ((ABI45_0_0RCTEnhancedScrollView *)_scrollView))
#define MAP_SCROLL_VIEW_PROP(name) REMAP_SCROLL_VIEW_PROP(name, name)
  ...
  if (oldScrollViewProps.scrollIndicatorInsets != newScrollViewProps.scrollIndicatorInsets) {
    _scrollView.scrollIndicatorInsets = ABI45_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.scrollIndicatorInsets);
  }

  if (oldScrollViewProps.indicatorStyle != newScrollViewProps.indicatorStyle) {
    _scrollView.indicatorStyle = ABI45_0_0RCTUIScrollViewIndicatorStyleFromProps(newScrollViewProps);
  }
  ...
  if (oldScrollViewProps.contentInset != newScrollViewProps.contentInset) {
    _scrollView.contentInset = ABI45_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.contentInset);
  }

  ABI45_0_0RCTEnhancedScrollView *scrollView = (ABI45_0_0RCTEnhancedScrollView *)_scrollView;
  if (oldScrollViewProps.contentOffset != newScrollViewProps.contentOffset) {
    _scrollView.contentOffset = ABI45_0_0RCTCGPointFromPoint(newScrollViewProps.contentOffset);
  }

  if (oldScrollViewProps.snapToAlignment != newScrollViewProps.snapToAlignment) {
    scrollView.snapToAlignment = ABI45_0_0RCTNSStringFromString(toString(newScrollViewProps.snapToAlignment));
  }
  ...
  if (oldScrollViewProps.keyboardDismissMode != newScrollViewProps.keyboardDismissMode) {
    scrollView.keyboardDismissMode = ABI45_0_0RCTUIKeyboardDismissModeFromProps(newScrollViewProps);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  auto contentOffset = ABI45_0_0RCTCGPointFromPoint(data.contentOffset);
  if (!oldState && !CGPointEqualToPoint(contentOffset, CGPointZero)) {
    _scrollView.contentOffset = contentOffset;
  }

  CGSize contentSize = ABI45_0_0RCTCGSizeFromSize(data.getContentSize());
  ...
  _containerView.frame = CGRect{ABI45_0_0RCTCGPointFromPoint(data.contentBoundingRect.origin), contentSize};
...
- (void)_preserveContentOffsetIfNeededWithBlock:(void (^)())block
{
  ...
  [((ABI45_0_0RCTEnhancedScrollView *)_scrollView) preserveContentOffsetWithBlock:block];
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  while (ancestorView) {
    if ([ancestorView respondsToSelector:@selector(isJSResponder)]) {
      BOOL isJSResponder = ((UIView<ABI45_0_0RCTComponentViewProtocol> *)ancestorView).isJSResponder;
      if (isJSResponder) {
        return YES;
      }
    }
...
- (ScrollViewMetrics)_scrollViewMetrics
{
  ...
  metrics.contentSize = ABI45_0_0RCTSizeFromCGSize(_scrollView.contentSize);
  metrics.contentOffset = ABI45_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
  metrics.contentInset = ABI45_0_0RCTEdgeInsetsFromUIEdgeInsets(_scrollView.contentInset);
  metrics.containerSize = ABI45_0_0RCTSizeFromCGSize(_scrollView.bounds.size);
...
- (void)_updateStateWithContentOffset
{
  ...
  auto contentOffset = ABI45_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
...
- (void)prepareForRecycle
{
  ...
  _scrollView.contentOffset = ABI45_0_0RCTCGPointFromPoint(props.contentOffset);
  _state.reset();
  _isUserTriggeredScrolling = NO;
...
{
  // Historically, `UIScrollView`s in ABI45_0_0React Native do not cancel touches
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if ((_lastScrollEventDispatchTime == 0) || (now - _lastScrollEventDispatchTime > _scrollEventThrottle)) {
    ...
    ABI45_0_0RCTSendPaperScrollEvent_DEPRECATED(scrollView, self.tag);
  }
...
{
  ABI45_0_0RCTScrollViewHandleCommand(self, commandName, args);
}
...
#pragma mark - ABI45_0_0RCTScrollableProtocol
...
Class<ABI45_0_0RCTComponentViewProtocol> ABI45_0_0RCTScrollViewCls(void)
{
  return ABI45_0_0RCTScrollViewComponentView.class;
}
