...
#import <ABI45_0_0React/ABI45_0_0RCTErrorInfo.h>
#import <ABI45_0_0React/ABI45_0_0RCTEventDispatcherProtocol.h>
#import <ABI45_0_0React/ABI45_0_0RCTJSStackFrame.h>
#import <ABI45_0_0React/ABI45_0_0RCTRedBoxExtraDataViewController.h>
...
@class ABI45_0_0RCTRedBoxWindow;

@interface UIButton (ABI45_0_0RCTRedBox)

@property (nonatomic) ABI45_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI45_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI45_0_0RCTRedBox)

- (ABI45_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI45_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI45_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI45_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI45_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI45_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI45_0_0RCTRedBoxWindow : NSObject <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UIViewController *rootViewController;
@property (nonatomic, weak) id<ABI45_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI45_0_0RCTRedBoxWindow {
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI45_0_0RCTJSStackFrame *> *_lastStackTrace;
  int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame
           customButtonTitles:(NSArray<NSString *> *)customButtonTitles
         customButtonHandlers:(NSArray<ABI45_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
  if (self = [super init]) {
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title
    accessibilityIdentifier:(NSString *)accessibilityIdentifier
                   selector:(SEL)selector
                      block:(ABI45_0_0RCTRedBoxButtonPressHandler)block
{
...
{
  return ABI45_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
}
...
- (void)showErrorMessage:(NSString *)message
               withStack:(NSArray<ABI45_0_0RCTJSStackFrame *> *)stack
    ...
    if (!isRootViewControllerPresented) {
      [_stackTraceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]
                                  atScrollPosition:UITableViewScrollPositionTop
                                          animated:NO];
      [ABI45_0_0RCTKeyWindow().rootViewController presentViewController:self.rootViewController animated:YES completion:nil];
    }
  }
}
...
- (void)copyStack
{
  ...
  for (ABI45_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI45_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI45_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI45_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI45_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI45_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI45_0_0RCTKeyCommands for this, but since
...
@interface ABI45_0_0RCTRedBox () <
    ABI45_0_0RCTInvalidating,
    ABI45_0_0RCTRedBoxWindowActionDelegate,
    ABI45_0_0RCTRedBoxExtraDataActionDelegate,
    ABI45_0_0NativeRedBoxSpec>
@end

@implementation ABI45_0_0RCTRedBox {
  ABI45_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI45_0_0RCTErrorCustomizer>> *_errorCustomizers;
  ABI45_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
  NSMutableArray<NSString *> *_customButtonTitles;
  NSMutableArray<ABI45_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (ABI45_0_0RCTErrorInfo *)_customizeError:(ABI45_0_0RCTErrorInfo *)error
{
  ABI45_0_0RCTAssertMainQueue();
  if (!self->_errorCustomizers) {
    return error;
  }
  for (id<ABI45_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI45_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI45_0_0RCTJSStackTraceKey]
             errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message
             withDetails:(NSString *)details
                   stack:(NSArray<ABI45_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
...
{
  NSArray<ABI45_0_0RCTJSStackFrame *> *stack = [ABI45_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
  [self showErrorMessage:message
         withParsedStack:[ABI45_0_0RCTJSStackFrame stackFramesWithDictionaries:stack]
                isUpdate:NO
             ...
             errorCookie:(int)errorCookie
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self->_extraDataViewController == nil) {
      self->_extraDataViewController = [ABI45_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
    ...
    if (!self->_window) {
      self->_window = [[ABI45_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds
    ...
    ABI45_0_0RCTErrorInfo *errorInfo = [[ABI45_0_0RCTErrorInfo alloc] initWithErrorMessage:message stack:stack];
...
ABI45_0_0RCT_EXPORT_METHOD(setExtraData : (NSDictionary *)extraData forIdentifier : (NSString *)identifier)
{
  [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI45_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI45_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI45_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bundleManager.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI45_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI45_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI45_0_0RCTRedBoxWindow *)redBoxWindow
{
  if (_overrideReloadAction) {
    _overrideReloadAction();
  } else {
    ABI45_0_0RCTTriggerReloadCommandListeners(@"Redbox");
  }
  [self dismiss];
}
...
{
  return std::make_shared<ABI45_0_0facebook::ABI45_0_0React::NativeRedBoxSpecJSI>(params);
}

@end

@implementation ABI45_0_0RCTBridge (ABI45_0_0RCTRedBox)

- (ABI45_0_0RCTRedBox *)redBox
{
  return ABI45_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI45_0_0RCTRedBox class]] : nil;
}
...
@interface ABI45_0_0RCTRedBox () <ABI45_0_0NativeRedBoxSpec>
@end
...
Class ABI45_0_0RCTRedBoxCls(void)
{
  return ABI45_0_0RCTRedBox.class;
}
