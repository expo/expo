...
#import "ABI45_0_0RCTDevSettings.h"
...
static NSString *const kABI45_0_0RCTDevSettingProfilingEnabled = @"profilingEnabled";
static NSString *const kABI45_0_0RCTDevSettingHotLoadingEnabled = @"hotLoadingEnabled";
static NSString *const kABI45_0_0RCTDevSettingIsInspectorShown = @"showInspector";
static NSString *const kABI45_0_0RCTDevSettingIsDebuggingRemotely = @"isDebuggingRemotely";
static NSString *const kABI45_0_0RCTDevSettingExecutorOverrideClass = @"executor-override";
static NSString *const kABI45_0_0RCTDevSettingShakeToShowDevMenu = @"shakeToShow";
static NSString *const kABI45_0_0RCTDevSettingIsPerfMonitorShown = @"ABI45_0_0RCTPerfMonitorKey";
...
void ABI45_0_0RCTDevSettingsSetEnabled(BOOL enabled)
{
...
@interface ABI45_0_0RCTDevSettingsUserDefaultsDataSource : NSObject <ABI45_0_0RCTDevSettingsDataSource>

@end

@implementation ABI45_0_0RCTDevSettingsUserDefaultsDataSource {
...
- (void)updateSettingWithValue:(id)value forKey:(NSString *)key
{
  ...
  [_userDefaults setObject:_settings forKey:kABI45_0_0RCTDevSettingsUserDefaultsKey];
}
...
{
  NSDictionary *existingSettings = [_userDefaults objectForKey:kABI45_0_0RCTDevSettingsUserDefaultsKey];
...
@property (nonatomic, readwrite, strong) id<ABI45_0_0RCTDevSettingsDataSource> dataSource;

@end

@implementation ABI45_0_0RCTDevSettings
...
- (instancetype)init
{
  ...
  ABI45_0_0RCTDevSettingsUserDefaultsDataSource *dataSource =
      [[ABI45_0_0RCTDevSettingsUserDefaultsDataSource alloc] initWithDefaultValues:defaultValues];
...
- (instancetype)initWithDataSource:(id<ABI45_0_0RCTDevSettingsDataSource>)dataSource
{
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(jsLoaded:)
                                                 name:@"ABI45_0_0RCTInstanceDidLoadBundle"
                                               object:nil];
  }
  return self;
}
...
- (void)initialize
{
    ...
    _bridgeExecutorOverrideToken = [[ABI45_0_0RCTPackagerConnection sharedPackagerConnection]
        addNotificationHandler:^(id params) {
          if (params != (id)kCFNull && [params[@"debug"] boolValue]) {
            weakBridge.executorClass = objc_lookUpClass("ABI45_0_0RCTWebSocketExecutor");
          }
        }
  ...
  if (numInitializedModules++ == 0) {
    reloadToken = [[ABI45_0_0RCTPackagerConnection sharedPackagerConnection]
        addNotificationHandler:^(id params) {
          ABI45_0_0RCTTriggerReloadCommandListeners(@"Global hotkey");
        }
                         queue:dispatch_get_main_queue()
                     forMethod:@"reload"];
  }
  ...
  if (self.bridge) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      id dispatchBlock = ^{
        ...
        [ABI45_0_0RCTInspectorDevServerHelper connectWithBundleURL:url];
      };
      [strongSelf.bridge dispatchBlock:dispatchBlock queue:ABI45_0_0RCTJSThread];
    });
  } else {
...
- (void)invalidate
{
  ...
  if (--numInitializedModules == 0) {
    [[ABI45_0_0RCTPackagerConnection sharedPackagerConnection] removeHandler:reloadToken];
  }
#endif
}
...
#else
  return false;
#endif // ABI45_0_0RCT_ENABLE_INSPECTOR
}
...
- (BOOL)isRemoteDebuggingAvailable
{
  ...
  Class jsDebuggingExecutorClass = objc_lookUpClass("ABI45_0_0RCTWebSocketExecutor");
...
ABI45_0_0RCT_EXPORT_METHOD(reloadWithReason : (NSString *)reason)
{
  ABI45_0_0RCTTriggerReloadCommandListeners(reason);
}

ABI45_0_0RCT_EXPORT_METHOD(onFastRefresh)
{
  [self.bridge onFastRefresh];
}

ABI45_0_0RCT_EXPORT_METHOD(setIsShakeToShowDevMenuEnabled : (BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI45_0_0RCTDevSettingShakeToShowDevMenu];
}

- (BOOL)isShakeToShowDevMenuEnabled
{
  return [[self settingForKey:kABI45_0_0RCTDevSettingShakeToShowDevMenu] boolValue];
}

ABI45_0_0RCT_EXPORT_METHOD(setIsDebuggingRemotely : (BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI45_0_0RCTDevSettingIsDebuggingRemotely];
  [self _remoteDebugSettingDidChange];
}

- (BOOL)isDebuggingRemotely
{
  return [[self settingForKey:kABI45_0_0RCTDevSettingIsDebuggingRemotely] boolValue];
}
...
- (void)_remoteDebugSettingDidChange
{
  ...
  NSString *executorOverride = [[NSUserDefaults standardUserDefaults] stringForKey:kABI45_0_0RCTDevSettingExecutorOverrideClass];
  ...
  if (executorOverrideClass) {
    ...
    self.executorClass = enabled ? objc_getClass("ABI45_0_0RCTWebSocketExecutor") : nil;
  }
}

ABI45_0_0RCT_EXPORT_METHOD(setProfilingEnabled : (BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI45_0_0RCTDevSettingProfilingEnabled];
  [self _profilingSettingDidChange];
}

- (BOOL)isProfilingEnabled
{
  return [[self settingForKey:kABI45_0_0RCTDevSettingProfilingEnabled] boolValue];
}

- (void)_profilingSettingDidChange
{
  BOOL enabled = self.isProfilingEnabled;
  if (self.isHotLoadingAvailable && enabled != ABI45_0_0RCTProfileIsProfiling()) {
    if (enabled) {
      ...
      [self.bridge stopProfiling:^(NSData *logData) {
        ...
        ABI45_0_0RCTProfileSendResult(strongSelf.bridge, @"systrace", logData);
      }];
    }
  }
}
...
ABI45_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled : (BOOL)enabled)
{
  if (self.isHotLoadingEnabled != enabled) {
    [self _updateSettingWithValue:@(enabled) forKey:kABI45_0_0RCTDevSettingHotLoadingEnabled];
    if (_isJSLoaded) {
...
{
  return [[self settingForKey:kABI45_0_0RCTDevSettingHotLoadingEnabled] boolValue];
}

ABI45_0_0RCT_EXPORT_METHOD(toggleElementInspector)
{
  BOOL value = [[self settingForKey:kABI45_0_0RCTDevSettingIsInspectorShown] boolValue];
  [self _updateSettingWithValue:@(!value) forKey:kABI45_0_0RCTDevSettingIsInspectorShown];
...
ABI45_0_0RCT_EXPORT_METHOD(addMenuItem : (NSString *)title)
{
  __weak __typeof(self) weakSelf = self;
  [(ABI45_0_0RCTDevMenu *)[self.moduleRegistry moduleForName:"DevMenu"]
      addItem:[ABI45_0_0RCTDevMenuItem buttonItemWithTitle:title
                                          handler:^{
...
{
  return [[self settingForKey:kABI45_0_0RCTDevSettingIsInspectorShown] boolValue];
}

- (void)setIsPerfMonitorShown:(BOOL)isPerfMonitorShown
{
  [self _updateSettingWithValue:@(isPerfMonitorShown) forKey:kABI45_0_0RCTDevSettingIsPerfMonitorShown];
}

- (BOOL)isPerfMonitorShown
{
  return [[self settingForKey:kABI45_0_0RCTDevSettingIsPerfMonitorShown] boolValue];
}
...
- (void)setExecutorClass:(Class)executorClass
{
  ...
  if (self.bridge.executorClass != executorClass) {
    ...
    if (executorClass == Nil && self.bridge.executorClass != objc_lookUpClass("ABI45_0_0RCTWebSocketExecutor")) {
      return;
    }

    self.bridge.executorClass = executorClass;
    ABI45_0_0RCTTriggerReloadCommandListeners(@"Custom executor class reset");
  }
}
  ...
  if (bundleURL && !bundleURL.fileURL) {
    ...
    if (self.callableJSModules) {
      [self.callableJSModules invokeModule:@"HMRClient"
                                    method:@"setup"
                                  withArgs:@[ @"ios", path, host, ABI45_0_0RCTNullIfNil(port), @(isHotLoadingEnabled), scheme ]];
    }
  }
}
...
- (void)jsLoaded:(NSNotification *)notification
{
  ...
  if ([notification.name isEqualToString:ABI45_0_0RCTJavaScriptDidLoadNotification] &&
...
{
  return std::make_shared<ABI45_0_0facebook::ABI45_0_0React::NativeDevSettingsSpecJSI>(params);
}

@end

#else // #if ABI45_0_0RCT_DEV_MENU

@interface ABI45_0_0RCTDevSettings () <ABI45_0_0NativeDevSettingsSpec>
@end
...
#endif // #if ABI45_0_0RCT_DEV_MENU

@implementation ABI45_0_0RCTBridge (ABI45_0_0RCTDevSettings)

- (ABI45_0_0RCTDevSettings *)devSettings
{
#if ABI45_0_0RCT_DEV_MENU
  return devSettingsMenuEnabled ? [self moduleForClass:[ABI45_0_0RCTDevSettings class]] : nil;
#else
  return nil;
#endif
}
...
Class ABI45_0_0RCTDevSettingsCls(void)
{
  return ABI45_0_0RCTDevSettings.class;
}
