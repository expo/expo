...
#import <ABI45_0_0React/ABI45_0_0RCTDevLoadingView.h>
...
#import <ABI45_0_0React/ABI45_0_0RCTDevLoadingViewSetEnabled.h>
...
@interface ABI45_0_0RCTDevLoadingView () <ABI45_0_0NativeDevLoadingViewSpec>
@end

#if ABI45_0_0RCT_DEV | ABI45_0_0RCT_ENABLE_LOADING_VIEW

@implementation ABI45_0_0RCTDevLoadingView {
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
  BOOL _hiding;
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hide)
                                                 name:ABI45_0_0RCTJavaScriptDidFailToLoadNotification
                                               object:nil];
  }
  return self;
}
...
{
  ABI45_0_0RCTDevLoadingViewSetEnabled(enabled);
}
...
{
  if (!ABI45_0_0RCTDevLoadingViewGetEnabled() || self->_hiding) {
    return;
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    self->_showDate = [NSDate date];
    if (!self->_window && !ABI45_0_0RCTRunningInTestEnvironment()) {
      CGSize screenSize = [UIScreen mainScreen].bounds.size;

      UIWindow *window = ABI45_0_0RCTSharedApplication().keyWindow;
      self->_window =
    ...
    if (@available(iOS 13.0, *)) {
      UIWindowScene *scene = (UIWindowScene *)ABI45_0_0RCTSharedApplication().connectedScenes.anyObject;
...
ABI45_0_0RCT_EXPORT_METHOD(showMessage
                  : (NSString *)message withColor
                  : (NSNumber *__nonnull)color withBackgroundColor
                  : (NSNumber *__nonnull)backgroundColor)
{
  [self showMessage:message color:[ABI45_0_0RCTConvert UIColor:color] backgroundColor:[ABI45_0_0RCTConvert UIColor:backgroundColor]];
}

ABI45_0_0RCT_EXPORT_METHOD(hide)
{
  if (!ABI45_0_0RCTDevLoadingViewGetEnabled()) {
    return;
  }
...
- (void)showOfflineMessage
{
  ...
  NSString *message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI45_0_0RCT_PACKAGER_NAME];
  [self showMessage:message color:color backgroundColor:backgroundColor];
}
...
- (BOOL)isDarkModeEnabled
{
  ...
  return [ABI45_0_0RCTColorSchemePreference(nil) isEqualToString:@"dark"];
}
- (void)showWithURL:(NSURL *)URL
{
  if (URL.fileURL) {
    ...
    [self showInitialMessageDelayed:^{
      NSString *message = [NSString stringWithFormat:@"Loading from %@\u2026", ABI45_0_0RCT_PACKAGER_NAME];
...
Class ABI45_0_0RCTDevLoadingViewCls(void)
{
  return ABI45_0_0RCTDevLoadingView.class;
}
