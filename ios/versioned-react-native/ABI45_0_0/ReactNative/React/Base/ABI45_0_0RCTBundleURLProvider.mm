...
#import "ABI45_0_0RCTBundleURLProvider.h"

#import "ABI45_0_0RCTConvert.h"
#import "ABI45_0_0RCTDefines.h"
#import "ABI45_0_0RCTLog.h"

NSString *const ABI45_0_0RCTBundleURLProviderUpdatedNotification = @"ABI45_0_0RCTBundleURLProviderUpdatedNotification";

const NSUInteger kABI45_0_0RCTBundleURLProviderDefaultPort = ABI45_0_0RCT_METRO_PORT;

#if ABI45_0_0RCT_DEV_MENU | ABI45_0_0RCT_PACKAGER_LOADING_FUNCTIONALITY
static BOOL kABI45_0_0RCTAllowPackagerAccess = YES;
void ABI45_0_0RCTBundleURLProviderAllowPackagerServerAccess(BOOL allowed)
{
  kABI45_0_0RCTAllowPackagerAccess = allowed;
}
#endif
static NSString *const kABI45_0_0RCTPackagerSchemeKey = @"ABI45_0_0RCT_packager_scheme";
static NSString *const kABI45_0_0RCTJsLocationKey = @"ABI45_0_0RCT_jsLocation";
static NSString *const kABI45_0_0RCTEnableDevKey = @"ABI45_0_0RCT_enableDev";
static NSString *const kABI45_0_0RCTEnableMinificationKey = @"ABI45_0_0RCT_enableMinification";

@implementation ABI45_0_0RCTBundleURLProvider
...
{
  return @{
    kABI45_0_0RCTEnableDevKey : @YES,
    kABI45_0_0RCTEnableMinificationKey : @NO,
  };
}

- (void)settingsUpdated
{
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI45_0_0RCTBundleURLProviderUpdatedNotification object:self];
}
...
static NSURL *serverRootWithHostPort(NSString *hostPort, NSString *scheme)
{
  ...
  return [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@:%lu/",
                                                         scheme,
                                                         hostPort,
                                                         (unsigned long)kABI45_0_0RCTBundleURLProviderDefaultPort]];
}

#if ABI45_0_0RCT_DEV_MENU
+ (BOOL)isPackagerRunning:(NSString *)hostPort
{
  return [ABI45_0_0RCTBundleURLProvider isPackagerRunning:hostPort scheme:nil];
}
...
- (NSString *)guessPackagerHost
{
  ...
  if ([ABI45_0_0RCTBundleURLProvider isPackagerRunning:host]) {
    return host;
  }
  return nil;
}
...
{
#if ABI45_0_0RCT_DEV_MENU | ABI45_0_0RCT_PACKAGER_LOADING_FUNCTIONALITY
  if (!kABI45_0_0RCTAllowPackagerAccess) {
    ABI45_0_0RCTLogInfo(@"Packager server access is disabled in this environment");
    return nil;
  }
#endif
  ...
  if ([location length] && ![ABI45_0_0RCTBundleURLProvider isPackagerRunning:location scheme:scheme]) {
    location = nil;
  }
#endif
  if (location != nil) {
...
- (NSURL *)jsBundleURLForBundleRoot:(NSString *)bundleRoot fallbackURLProvider:(NSURL * (^)(void))fallbackURLProvider
{
  ...
  if (!packagerServerHostPort) {
    ...
    return [ABI45_0_0RCTBundleURLProvider jsBundleURLForBundleRoot:bundleRoot
...
- (BOOL)enableDev
{
  return [[NSUserDefaults standardUserDefaults] boolForKey:kABI45_0_0RCTEnableDevKey];
}

- (BOOL)enableMinification
{
  return [[NSUserDefaults standardUserDefaults] boolForKey:kABI45_0_0RCTEnableMinificationKey];
}

- (NSString *)jsLocation
{
  return [[NSUserDefaults standardUserDefaults] stringForKey:kABI45_0_0RCTJsLocationKey];
}

- (NSString *)packagerScheme
{
  NSString *packagerScheme = [[NSUserDefaults standardUserDefaults] stringForKey:kABI45_0_0RCTPackagerSchemeKey];
...
{
  [self updateValue:@(enableDev) forKey:kABI45_0_0RCTEnableDevKey];
}

- (void)setJsLocation:(NSString *)jsLocation
{
  [self updateValue:jsLocation forKey:kABI45_0_0RCTJsLocationKey];
}

- (void)setEnableMinification:(BOOL)enableMinification
{
  [self updateValue:@(enableMinification) forKey:kABI45_0_0RCTEnableMinificationKey];
}

- (void)setPackagerScheme:(NSString *)packagerScheme
{
  [self updateValue:packagerScheme forKey:kABI45_0_0RCTPackagerSchemeKey];
}

+ (instancetype)sharedSettings
{
  static ABI45_0_0RCTBundleURLProvider *sharedInstance;
  static dispatch_once_t once_token;
  dispatch_once(&once_token, ^{
    sharedInstance = [ABI45_0_0RCTBundleURLProvider new];
  });
