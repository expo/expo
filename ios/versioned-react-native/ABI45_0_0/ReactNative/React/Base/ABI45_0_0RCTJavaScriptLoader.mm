...
#import "ABI45_0_0RCTJavaScriptLoader.h"
...
NSString *const ABI45_0_0RCTJavaScriptLoaderErrorDomain = @"ABI45_0_0RCTJavaScriptLoaderErrorDomain";

const UInt32 ABI45_0_0RCT_BYTECODE_ALIGNMENT = 4;
...
bool ABI45_0_0RCTIsBytecodeBundle(NSData *script)
{
  ...
  return (
      [script length] > 8 && ABI45_0_0RCTReadUInt32LE(script, 0) == BYTECODE_BUNDLE_MAGIC_NUMBER &&
      ABI45_0_0RCTReadUInt32LE(script, 4) > 0);
}

@interface ABI45_0_0RCTSource () {
 @public
  NSURL *_url;
  NSData *_data;
  NSUInteger _length;
...
@implementation ABI45_0_0RCTSource

static ABI45_0_0RCTSource *ABI45_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  ABI45_0_0RCTSource *source = [ABI45_0_0RCTSource new];
  source->_url = url;
  // Multipart responses may give us an unaligned view into the buffer. This ensures memory is aligned.
  source->_data = (ABI45_0_0RCTIsBytecodeBundle(data) && ((long)[data bytes] % ABI45_0_0RCT_BYTECODE_ALIGNMENT))
      ? [[NSData alloc] initWithData:data]
      : data;
  source->_length = length;
  source->_filesChangedCount = ABI45_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI45_0_0RCTLoadingProgress
...
@implementation ABI45_0_0RCTJavaScriptLoader
             ...
             onComplete:(ABI45_0_0RCTSourceLoadBlock)onComplete
{
  ...
  if (data) {
    onComplete(nil, ABI45_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError = [error.domain isEqualToString:ABI45_0_0RCTJavaScriptLoaderErrorDomain] &&
      error.code == ABI45_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI45_0_0RCTJavaScriptLoaderErrorNoScriptURL
                 userInfo:@{
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI45_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI45_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                 userInfo:@{
  ...
  ABI45_0_0facebook::ABI45_0_0React::BundleHeader header;
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI45_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                 userInfo:@{
  ...
  ABI45_0_0facebook::ABI45_0_0React::ScriptTag tag = ABI45_0_0facebook::ABI45_0_0React::parseTypeFromHeader(header);
  switch (tag) {
    case ABI45_0_0facebook::ABI45_0_0React::ScriptTag::HBCBundle:
    case ABI45_0_0facebook::ABI45_0_0React::ScriptTag::RAMBundle:
      break;

    case ABI45_0_0facebook::ABI45_0_0React::ScriptTag::String: {
      ...
      if (error) {
        *error =
            [NSError errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                                code:ABI45_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI45_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                 userInfo:@{
...
static void parseHeaders(NSDictionary *headers, ABI45_0_0RCTSource *source)
{
...
static void attemptAsynchronousLoadOfBundleAtURL(
    NSURL *scriptURL,
    ABI45_0_0RCTSourceLoadProgressBlock onProgress,
    ABI45_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      NSError *error = nil;
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path options:NSDataReadingMappedIfSafe error:&error];
      onComplete(error, ABI45_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI45_0_0RCTMultipartDataTask *task = [[ABI45_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL
      partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
        ...
        if (error) {
          if ([error.domain isEqualToString:NSURLErrorDomain]) {
            error = [NSError
                errorWithDomain:ABI45_0_0RCTJavaScriptLoaderErrorDomain
                           code:ABI45_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                       userInfo:@{
                         NSLocalizedDescriptionKey :
                             [@"Could not connect to development server.\n\n"
                               "Ensure the following:\n"
                               "- Node server is running and available on the same network - run 'npm start' from ABI45_0_0React-native root\n"
        ...
        ABI45_0_0RCTSource *source = ABI45_0_0RCTSourceCreate(scriptURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI45_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI45_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI45_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI45_0_0RCTLoadingProgress *progress = [ABI45_0_0RCTLoadingProgress new];
...
static ABI45_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI45_0_0RCTLoadingProgress *progress = [ABI45_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI45_0_0RCTJSONParse(rawText, nil);
