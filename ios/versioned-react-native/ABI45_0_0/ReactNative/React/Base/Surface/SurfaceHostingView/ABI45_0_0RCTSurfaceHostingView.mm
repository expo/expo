...
@interface ABI45_0_0RCTSurfaceHostingView ()
...
@implementation ABI45_0_0RCTSurfaceHostingView {
...
{
  return [[ABI45_0_0RCTSurface alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProperties];
}
...
{
  id<ABI45_0_0RCTSurfaceProtocol> surface = [[self class] createSurfaceWithBridge:bridge
                ...
                sizeMeasureMode:(ABI45_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    // For backward compatibility with ABI45_0_0RCTRootView, set a color here instead of transparent (OS default).
...
- (void)setFrame:(CGRect)frame
{
  ...
  ABI45_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(
...
{
  if (ABI45_0_0RCTSurfaceStageIsPreparing(_stage)) {
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  ABI45_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(size, _sizeMeasureMode, &minimumSize, &maximumSize);
...
- (void)setStage:(ABI45_0_0RCTSurfaceStage)stage
{
  ...
  BOOL shouldInvalidateLayout = ABI45_0_0RCTSurfaceStageIsRunning(stage) != ABI45_0_0RCTSurfaceStageIsRunning(_stage) ||
      ABI45_0_0RCTSurfaceStageIsPreparing(stage) != ABI45_0_0RCTSurfaceStageIsPreparing(_stage);
...
- (void)setSizeMeasureMode:(ABI45_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
...
- (void)setActivityIndicatorViewFactory:(ABI45_0_0RCTSurfaceHostingViewActivityIndicatorViewFactory)activityIndicatorViewFactory
{
...
- (void)_updateViews
{
  self.isSurfaceViewVisible = ABI45_0_0RCTSurfaceStageIsRunning(_stage);
  self.isActivityIndicatorViewVisible = ABI45_0_0RCTSurfaceStageIsPreparing(_stage);
}
...
- (void)surface:(__unused ABI45_0_0RCTSurface *)surface didChangeStage:(ABI45_0_0RCTSurfaceStage)stage
{
...
- (void)surface:(__unused ABI45_0_0RCTSurface *)surface didChangeIntrinsicSize:(__unused CGSize)intrinsicSize
{
  ABI45_0_0RCTExecuteOnMainQueue(^{
    [self _invalidateLayout];
  });
