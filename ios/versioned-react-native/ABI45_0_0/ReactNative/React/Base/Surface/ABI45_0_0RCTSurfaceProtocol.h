...
@protocol ABI45_0_0RCTSurfaceProtocol <NSObject>

@property (atomic, readonly) ABI45_0_0RCTSurfaceStage stage;
@property (atomic, readonly) NSString *moduleName;
@property (atomic, readwrite, weak, nullable) id<ABI45_0_0RCTSurfaceDelegate> delegate;
 ...
 * `viewportOffset` is ignored in `ABI45_0_0RCTSurface` but used in `ABI45_0_0RCTFabricSurface`.
 */
 ...
 * Those methods are a no-op for regular ABI45_0_0RCTSurface (for now), but all call sites must call them appropriately.
 */
- (void)start;
- (void)stop;
