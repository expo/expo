...
#import "ABI45_0_0RCTAutoInsetsProtocol.h"
#import "ABI45_0_0RCTBorderDrawing.h"
#import "ABI45_0_0RCTI18nUtil.h"
#import "ABI45_0_0RCTLog.h"
#import "ABI45_0_0RCTViewUtils.h"
#import "ABI45_0_0UIView+React.h"

UIAccessibilityTraits const ABI45_0_0SwitchAccessibilityTrait = 0x20000000000001;

@implementation UIView (ABI45_0_0RCTViewUnmounting)
...
- (void)ABI45_0_0React_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI45_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI45_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI45_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ABI45_0_0React_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isABI45_0_0ReactRootView]) {
      break;
    }
...
static NSString *ABI45_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI45_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI45_0_0RCTView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI45_0_0RCTBorderStyleSolid;
...
ABI45_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : unused)

- (void)setABI45_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ABI45_0_0ReactLayoutDirection != layoutDirection) {
    _ABI45_0_0ReactLayoutDirection = layoutDirection;
...
- (void)setPointerEvents:(ABI45_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI45_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI45_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI45_0_0RCTPointerEventsNone || _pointerEvents == ABI45_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ABI45_0_0ReactZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI45_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI45_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI45_0_0RCTLogError(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI45_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  ...
  if (actionObject) {
    _onAccessibilityAction(@{@"actionName" : actionObject[@"name"], @"actionTarget" : self.ABI45_0_0ReactTag});
  }
  return YES;
}
...
- (NSString *)accessibilityValue
{
  ...
  // Handle Switch.
  if ((self.accessibilityTraits & ABI45_0_0SwitchAccessibilityTrait) == ABI45_0_0SwitchAccessibilityTrait) {
  ...
  // Handle states which haven't already been handled in ABI45_0_0RCTViewManager.
  for (NSString *state in self.accessibilityState) {
    id val = self.accessibilityState[state];
    if (!val) {
      continue;
...
{
  if (self.ABI45_0_0ReactAccessibilityElement == self) {
...
- (BOOL)performAccessibilityAction:(NSString *)name
{
  if (_onAccessibilityAction && accessibilityActionsNameMap[name]) {
    _onAccessibilityAction(@{@"actionName" : name, @"actionTarget" : self.ABI45_0_0ReactTag});
    return YES;
  }
  return NO;
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ABI45_0_0ReactTag: %@;", self.ABI45_0_0ReactTag];
                   ...
                   updateOffset:(BOOL)updateOffset
{
  ...
  if (parentView.automaticallyAdjustContentInsets) {
    UIEdgeInsets autoInset = ABI45_0_0RCTContentInsets(parentView);
...
- (void)ABI45_0_0React_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ABI45_0_0ReactSubviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ABI45_0_0React_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ABI45_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI45_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ABI45_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ABI45_0_0ReactSubviews.count == 0) {
  ...
  for (UIView *view in self.ABI45_0_0ReactSubviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ABI45_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ABI45_0_0React_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI45_0_0React_findClipView];
  if (clipView) {
    [self ABI45_0_0React_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI45_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x)
{
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ABI45_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI45_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI45_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI45_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets){
        ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
        ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
        ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
        ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets){
      ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, ABI45_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
      ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI45_0_0RCTDefaultIfNegativeTo(borderWidth, ABI45_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI45_0_0RCTCornerRadii)cornerRadii
{
  ...
  if ([[ABI45_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI45_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, _borderTopStartRadius);
    const CGFloat topEndRadius = ABI45_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, _borderTopEndRadius);
    const CGFloat bottomStartRadius = ABI45_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, _borderBottomStartRadius);
    const CGFloat bottomEndRadius = ABI45_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, _borderBottomEndRadius);
    ...
    topLeftRadius = ABI45_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI45_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI45_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI45_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius =
        ABI45_0_0RCTDefaultIfNegativeTo(radius, ABI45_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius =
        ABI45_0_0RCTDefaultIfNegativeTo(radius, ABI45_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius =
        ABI45_0_0RCTDefaultIfNegativeTo(radius, ABI45_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI45_0_0RCTDefaultIfNegativeTo(
        radius, ABI45_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI45_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI45_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI45_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI45_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI45_0_0RCTCornerRadii){
...
- (ABI45_0_0RCTBorderColors)borderColors
{
  ...
  if ([[ABI45_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    ...
    return (ABI45_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI45_0_0RCTUpdateShadowPathForView(self);

  const ABI45_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI45_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering = ABI45_0_0RCTCornerRadiiAreEqual(cornerRadii) && ABI45_0_0RCTBorderInsetsAreEqual(borderInsets) &&
      ABI45_0_0RCTBorderColorsAreEqual(borderColors) && _borderStyle == ABI45_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI45_0_0RCTGetBorderImage(
...
static BOOL ABI45_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI45_0_0RCTUpdateShadowPathForView(ABI45_0_0RCTView *view)
{
  if (ABI45_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {
      // If view has a solid background color, calculate shadow path from border
      const ABI45_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI45_0_0RCTCornerInsets cornerInsets = ABI45_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI45_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    const ABI45_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI45_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      cornerRadius = cornerRadii.topLeft;

    } else {
      CAShapeLayer *shapeLayer = [CAShapeLayer layer];
      CGPathRef path =
          ABI45_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI45_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                            \
  -(void)setBorder##side##Style : (ABI45_0_0RCTBorderStyle)style \
  {                                                     \
