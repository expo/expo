...
#import "ABI45_0_0RCTFont.h"
#import "ABI45_0_0RCTAssert.h"
#import "ABI45_0_0RCTLog.h"

#import <CoreText/CoreText.h>

typedef CGFloat ABI45_0_0RCTFontWeight;
static ABI45_0_0RCTFontWeight weightOfFont(UIFont *font)
{
  ...
  for (NSString *suffix in weightSuffixes) {
    ...
    if (CFStringFind((CFStringRef)fontName, (CFStringRef)suffix, options).location != kCFNotFound) {
      return (ABI45_0_0RCTFontWeight)fontWeights[i].doubleValue;
    }
    i++;
  }

  auto traits = (__bridge_transfer NSDictionary *)CTFontCopyTraits((CTFontRef)font);
  return (ABI45_0_0RCTFontWeight)[traits[UIFontWeightTrait] doubleValue];
}
...
static ABI45_0_0RCTFontHandler defaultFontHandler;

void ABI45_0_0RCTSetDefaultFontHandler(ABI45_0_0RCTFontHandler handler)
{
  defaultFontHandler = handler;
}

BOOL ABI45_0_0RCTHasFontHandlerSet()
{
...
static NSString *FontWeightDescriptionFromUIFontWeight(UIFontWeight fontWeight)
{
  ...
  ABI45_0_0RCTAssert(NO, @"Unknown UIFontWeight passed in: %f", fontWeight);
  return @"regular";
}

static UIFont *cachedSystemFont(CGFloat size, ABI45_0_0RCTFontWeight weight)
{
  static NSCache<NSValue *, UIFont *> *fontCache = [NSCache new];

  struct __attribute__((__packed__)) CacheKey {
    CGFloat size;
    ABI45_0_0RCTFontWeight weight;
  };
...
@implementation ABI45_0_0RCTConvert (ABI45_0_0RCTFont)

+ (UIFont *)UIFont:(id)json
{
  json = [self NSDictionary:json];
  return [ABI45_0_0RCTFont updateFont:nil
                  withFamily:[ABI45_0_0RCTConvert NSString:json[@"fontFamily"]]
                        size:[ABI45_0_0RCTConvert NSNumber:json[@"fontSize"]]
                      weight:[ABI45_0_0RCTConvert NSString:json[@"fontWeight"]]
                       style:[ABI45_0_0RCTConvert NSString:json[@"fontStyle"]]
                     variant:[ABI45_0_0RCTConvert NSStringArray:json[@"fontVariant"]]
             scaleMultiplier:1];
}

ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RCTFontWeight,
    (@{
...
typedef BOOL ABI45_0_0RCTFontStyle;
ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RCTFontStyle,
    (@{
      @"normal" : @NO,
      @"italic" : @YES,
      @"oblique" : @YES,
...
typedef NSDictionary ABI45_0_0RCTFontVariantDescriptor;
+ (ABI45_0_0RCTFontVariantDescriptor *)ABI45_0_0RCTFontVariantDescriptor:(id)json
{
  ...
  ABI45_0_0RCTFontVariantDescriptor *value = mapping[json];
  if (ABI45_0_0RCT_DEBUG && !value && [json description].length > 0) {
    ABI45_0_0RCTLogError(
        @"Invalid ABI45_0_0RCTFontVariantDescriptor '%@'. should be one of: %@",
        json,
...
ABI45_0_0RCT_ARRAY_CONVERTER(ABI45_0_0RCTFontVariantDescriptor)

@end

@implementation ABI45_0_0RCTFont
...
+ (UIFont *)updateFont:(UIFont *)font
            withFamily:(NSString *)family
               ...
               variant:(NSArray<ABI45_0_0RCTFontVariantDescriptor *> *)variant
       scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  const ABI45_0_0RCTFontWeight defaultFontWeight = UIFontWeightRegular;
  const CGFloat defaultFontSize = 14;

  // Initialize properties to defaults
  CGFloat fontSize = defaultFontSize;
  ABI45_0_0RCTFontWeight fontWeight = defaultFontWeight;
  ...
  // Get font attributes
  fontSize = [ABI45_0_0RCTConvert CGFloat:size] ?: fontSize;
  if (scaleMultiplier > 0.0 && scaleMultiplier != 1.0) {
    fontSize = round(fontSize * scaleMultiplier);
  }
  familyName = [ABI45_0_0RCTConvert NSString:family] ?: familyName;
  isItalic = style ? [ABI45_0_0RCTConvert ABI45_0_0RCTFontStyle:style] : isItalic;
  fontWeight = weight ? [ABI45_0_0RCTConvert ABI45_0_0RCTFontWeight:weight] : fontWeight;
  ...
  if (!didFindFont && fontNamesForFamilyName(familyName).count == 0) {
    ...
    if (font) {
      ...
      ABI45_0_0RCTLogError(@"Unrecognized font family '%@'", familyName);
  ...
  if (variant) {
    NSArray *fontFeatures = [ABI45_0_0RCTConvert ABI45_0_0RCTFontVariantDescriptorArray:variant];
    UIFontDescriptor *fontDescriptor = [font.fontDescriptor
