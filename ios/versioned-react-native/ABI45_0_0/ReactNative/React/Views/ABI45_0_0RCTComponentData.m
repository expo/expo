...
typedef void (^ABI45_0_0RCTPropBlock)(id<ABI45_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI45_0_0RCTPropBlock> ABI45_0_0RCTPropBlockDictionary;
...
@implementation ABI45_0_0RCTComponentData {
  id<ABI45_0_0RCTComponent> _defaultView; // Only needed for ABI45_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI45_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI45_0_0RCTPropBlockDictionary *_shadowPropBlocks;
...
- (instancetype)initWithManagerClass:(Class)managerClass
                     ...
                     eventDispatcher:(id<ABI45_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  if ((self = [super init])) {
    _bridge = bridge;
...
- (ABI45_0_0RCTViewManager *)manager
{
  if (!_manager && _bridge) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI45_0_0RCTDidInitializeModuleNotification
                                                        object:nil
...
{
  ABI45_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI45_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI45_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI45_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI45_0_0RCTPropBlock createEventSetter(
    NSString *propName,
    SEL setter,
    InterceptorBlock eventInterceptor,
    id<ABI45_0_0RCTEventDispatcherProtocol> eventDispatcher)
{
  __weak id<ABI45_0_0RCTEventDispatcherProtocol> weakEventDispatcher = eventDispatcher;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI45_0_0RCTConvert BOOL:json]) {
      __weak id<ABI45_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI45_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI45_0_0ReactTag);
        } else {
          ABI45_0_0RCTComponentEvent *componentEvent = [[ABI45_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI45_0_0ReactTag
                                                                                 body:event];
...
static ABI45_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI45_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI45_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI45_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI45_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI45_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock =
          createEventSetter(name, setter, self.eventInterceptor, _bridge ? _bridge.eventDispatcher : _eventDispatcher);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI45_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI45_0_0RCTLogError(@"No +[ABI45_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI45_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI45_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI45_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI45_0_0RCT_CASE(_C_SEL, SEL)
        ABI45_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI45_0_0RCT_CASE(_C_CHR, char)
        ABI45_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI45_0_0RCT_CASE(_C_SHT, short)
        ABI45_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI45_0_0RCT_CASE(_C_INT, int)
        ABI45_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI45_0_0RCT_CASE(_C_LNG, long)
        ABI45_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI45_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI45_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI45_0_0RCT_CASE(_C_FLT, float)
        ABI45_0_0RCT_CASE(_C_DBL, double)
        ABI45_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI45_0_0RCT_CASE(_C_PTR, void *)
        ABI45_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI45_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI45_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI45_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI45_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI45_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI45_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI45_0_0ReactTag];
      ABI45_0_0RCTPerformBlockWithLogPrefix(
          ^{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI45_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI45_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI45_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
    ...
    if ([type isEqualToString:@"ABI45_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI45_0_0RCTNormalizeInputEventName(name)];

      // TODO(109509380): Remove this gating
      if (!ABI45_0_0RCTViewConfigEventValidAttributesDisabled()) {
        propTypes[name] = @"BOOL";
      }
    } else if ([type isEqualToString:@"ABI45_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI45_0_0RCTNormalizeInputEventName(name)];
...
{
  // Hackety hack, this partially re-implements ABI45_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI45_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI45_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI45_0_0EX_REMOVE_VERSION(name);
  ...
  ABI45_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
