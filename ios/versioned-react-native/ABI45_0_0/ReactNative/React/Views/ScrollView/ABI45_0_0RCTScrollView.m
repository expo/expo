...
#import "ABI45_0_0RCTScrollEvent.h"
 ...
 */
@interface ABI45_0_0RCTCustomScrollView : UIScrollView <UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) UIView<ABI45_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
@property (nonatomic, assign) BOOL pinchGestureEnabled;

@end

@implementation ABI45_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI45_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI45_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI45_0_0RCTUIManager JSResponder] isKindOfClass:[ABI45_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI45_0_0React scroll
...
- (void)setCustomRefreshControl:(UIView<ABI45_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
  ...
  // `ABI45_0_0RCTCustomRefreshContolProtocol`'s superview will always be of class
...
@interface ABI45_0_0RCTScrollView () <ABI45_0_0RCTUIManagerObserver>

@end

@implementation ABI45_0_0RCTScrollView {
  id<ABI45_0_0RCTEventDispatcherProtocol> _eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI45_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(id<ABI45_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  ABI45_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    [self _registerKeyboardListener];
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI45_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI45_0_0RCTApplyTransformationAccordingLayoutDirection(
    UIView *view,
...
{
  [super setABI45_0_0ReactLayoutDirection:layoutDirection];

  ABI45_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI45_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI45_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI45_0_0ReactSubview:view atIndex:atIndex];
  if ([view conformsToProtocol:@protocol(ABI45_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI45_0_0RCTCustomRefreshContolProtocol> *)view];
    ...
    ABI45_0_0RCTAssert(
        _contentView == nil,
        @"ABI45_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@",
        [_contentView ABI45_0_0React_recursiveDescription]);
    _contentView = view;
    ABI45_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI45_0_0ReactLayoutDirection);
...
- (void)removeABI45_0_0ReactSubview:(UIView *)subview
{
  ...
  if ([subview conformsToProtocol:@protocol(ABI45_0_0RCTCustomRefreshContolProtocol)]) {
    ...
    ABI45_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI45_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI45_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI45_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ABI45_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI45_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:NO];
...
{
  [ABI45_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:YES];
}

#pragma mark - ScrollView delegate

#define ABI45_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData)                                    \
  {                                                                                     \
...
#define ABI45_0_0RCT_FORWARD_SCROLL_EVENT(call)                                            \
...
#define ABI45_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
  -(void)delegateMethod : (UIScrollView *)scrollView        \
  {                                                         \
    ABI45_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                  \
    ABI45_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod : scrollView);  \
  }

ABI45_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI45_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI45_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    ABI45_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll : scrollView);
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  _allowNextScrollNoMatterWhat = YES; // Ensure next scroll event is recorded, regardless of throttle
  ABI45_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging : scrollView);
}
              ...
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI45_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging
...
{
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging : scrollView willDecelerate : decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI45_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming : scrollView withView : view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI45_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming : scrollView withView : view atScale : scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI45_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating : scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI45_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI45_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation : scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = ABI45_0_0RCTContentInsets(self);
...
- (void)uiManagerWillPerformMounting:(ABI45_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(
               __unused ABI45_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI45_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI45_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI45_0_0RCTScrollEvent *scrollEvent = [[ABI45_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
void ABI45_0_0RCTSendFakeScrollEvent(id<ABI45_0_0RCTEventDispatcherProtocol> eventDispatcher, NSNumber *ABI45_0_0ReactTag)
{
  ...
  ABI45_0_0RCTScrollEvent *fakeScrollEvent = [[ABI45_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI45_0_0ReactTag:ABI45_0_0ReactTag
