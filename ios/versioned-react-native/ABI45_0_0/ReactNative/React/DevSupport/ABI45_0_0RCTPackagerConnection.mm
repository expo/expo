...
@interface ABI45_0_0RCTPackagerConnection () <ABI45_0_0RCTReconnectingWebSocketDelegate>
@end
...
@implementation ABI45_0_0RCTPackagerConnection {
  std::mutex _mutex; // protects all ivars
  ABI45_0_0RCTReconnectingWebSocket *_socket;
  BOOL _socketConnected;
  NSString *_serverHostPortForSocket;
  NSString *_serverSchemeForSocket;
  id _bundleURLChangeObserver;
  uint32_t _nextToken;
  std::vector<Registration<ABI45_0_0RCTNotificationHandler>> _notificationRegistrations;
  std::vector<Registration<ABI45_0_0RCTRequestHandler>> _requestRegistrations;
  std::vector<Registration<ABI45_0_0RCTConnectedHandler>> _connectedRegistrations;
}

+ (instancetype)sharedPackagerConnection
{
  static ABI45_0_0RCTPackagerConnection *connection;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    connection = [ABI45_0_0RCTPackagerConnection new];
  });
  return connection;
}
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _serverHostPortForSocket = [[ABI45_0_0RCTBundleURLProvider sharedSettings] packagerServerHostPort];
    _serverSchemeForSocket = [[ABI45_0_0RCTBundleURLProvider sharedSettings] packagerScheme];
    _socket = socketForLocation(_serverHostPortForSocket, _serverSchemeForSocket);
    _socket.delegate = self;
    [_socket start];

    ABI45_0_0RCTPackagerConnection *const __weak weakSelf = self;
    _bundleURLChangeObserver =
        [[NSNotificationCenter defaultCenter] addObserverForName:ABI45_0_0RCTBundleURLProviderUpdatedNotification
                                                          object:nil
...
static ABI45_0_0RCTReconnectingWebSocket *socketForLocation(NSString *const serverHostPort, NSString *scheme)
{
  NSString *serverHost;
  NSString *serverPort;
  ...
  components.port = serverPort ? @(serverPort.integerValue) : @(kABI45_0_0RCTBundleURLProviderDefaultPort);
  components.path = @"/message";
  components.queryItems = @[ [NSURLQueryItem queryItemWithName:@"role" value:@"ios"] ];
  static dispatch_queue_t queue;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    queue = dispatch_queue_create("com.facebook.ABI45_0_0RCTPackagerConnectionQueue", DISPATCH_QUEUE_SERIAL);
  });
  return [[ABI45_0_0RCTReconnectingWebSocket alloc] initWithURL:components.URL queue:queue];
}
...
- (void)reconnect:(NSString *)packagerServerHostPort
{
  ...
  NSString *const serverScheme = [[ABI45_0_0RCTBundleURLProvider sharedSettings] packagerScheme];
...
{
  [self reconnect:[[ABI45_0_0RCTBundleURLProvider sharedSettings] packagerServerHostPort]];
}
...
template <typename Handler>
static void eraseRegistrationsWithToken(std::vector<Registration<Handler>> &registrations, ABI45_0_0RCTHandlerToken token)
{
  registrations.erase(
      std::remove_if(
...
- (void)addHandler:(id<ABI45_0_0RCTPackagerClientMethod>)handler forMethod:(NSString *)method
{
  dispatch_queue_t queue =
  ...
  [self
      addNotificationHandler:^(NSDictionary<NSString *, id> *notification) {
        [handler handleNotification:notification];
                   ...
                   forMethod:method];
  [self
      addRequestHandler:^(NSDictionary<NSString *, id> *request, ABI45_0_0RCTPackagerClientResponder *responder) {
...
{
  NSArray<NSNumber *> *const kSupportedVersions = @[ @(ABI45_0_0RCT_PACKAGER_CLIENT_PROTOCOL_VERSION) ];
  return [kSupportedVersions containsObject:version];
}

#pragma mark - ABI45_0_0RCTReconnectingWebSocketDelegate

- (void)reconnectingWebSocketDidOpen:(__unused ABI45_0_0RCTReconnectingWebSocket *)webSocket
{
  std::vector<Registration<ABI45_0_0RCTConnectedHandler>> registrations;
  {
    std::lock_guard<std::mutex> l(_mutex);
  ...
  for (const auto &registration : registrations) {
    ...
    ABI45_0_0RCTConnectedHandler handler = registration.handler;
...
- (void)reconnectingWebSocket:(ABI45_0_0RCTReconnectingWebSocket *)webSocket didReceiveMessage:(id)message
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *msg = ABI45_0_0RCTJSONParse(message, &error);

  if (error) {
    ABI45_0_0RCTLogError(@"%@ failed to parse message with error %@\n<message>\n%@\n</message>", [self class], error, msg);
    return;
  }

  if (!isSupportedVersion(msg[@"version"])) {
    ABI45_0_0RCTLogError(@"%@ received message with not supported version %@", [self class], msg[@"version"]);
    return;
  }
  ...
  if (messageId) { // Request
    const std::vector<Registration<ABI45_0_0RCTRequestHandler>> registrations(
        registrationsWithMethod(_mutex, _requestRegistrations, method));
    if (registrations.empty()) {
      ABI45_0_0RCTLogError(@"No handler found for packager method %@", msg[@"method"]);
      [[[ABI45_0_0RCTPackagerClientResponder alloc] initWithId:messageId socket:webSocket]
      ...
      ABI45_0_0RCTRequestHandler handler = registrations.back().handler;
      dispatch_async(registrations.back().queue, ^{
        handler(params, [[ABI45_0_0RCTPackagerClientResponder alloc] initWithId:messageId socket:webSocket]);
      });
    }
  } else { // Notification
    const std::vector<Registration<ABI45_0_0RCTNotificationHandler>> registrations(
        registrationsWithMethod(_mutex, _notificationRegistrations, method));
    for (const auto &registration : registrations) {
      // Beware: don't capture the reference to handler in a dispatched block!
      ABI45_0_0RCTNotificationHandler handler = registration.handler;
      dispatch_async(registration.queue, ^{
        handler(params);
      });
    }
...
- (void)reconnectingWebSocketDidClose:(__unused ABI45_0_0RCTReconnectingWebSocket *)webSocket
{
  std::lock_guard<std::mutex> l(_mutex);
  _socketConnected = NO;
}
