...
@implementation ABI45_0_0RNSVGText
{
    ABI45_0_0RNSVGGlyphContext *_glyphContext;
...
- (void)setInlineSize:(ABI45_0_0RNSVGLength *)inlineSize
{
...
- (void)setTextLength:(ABI45_0_0RNSVGLength *)textLength
{
...
- (void)setDeltaX:(NSArray<ABI45_0_0RNSVGLength *> *)deltaX
{
    if (deltaX == _deltaX) {
        return;
    }
...
- (void)setDeltaY:(NSArray<ABI45_0_0RNSVGLength *> *)deltaY
{
    if (deltaY == _deltaY) {
        return;
    }
...
- (void)setPositionX:(NSArray<ABI45_0_0RNSVGLength *>*)positionX
{
...
- (void)setPositionY:(NSArray<ABI45_0_0RNSVGLength *>*)positionY
{
...
- (void)setRotate:(NSArray<ABI45_0_0RNSVGLength *> *)rotate
{
    if (rotate == _rotate) {
        return;
    }
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI45_0_0RNSVGGlyphContext alloc] initWithWidth:size.width
                                                      height:size.height];
}
...
- (ABI45_0_0RNSVGText *)textRoot
{
    ABI45_0_0RNSVGText *root = self;
    while (root && [root class] != [ABI45_0_0RNSVGText class]) {
        if (![root isKindOfClass:[ABI45_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        root = (ABI45_0_0RNSVGText*)[root superview];
    }
...
- (NSString *)alignmentBaseline
{
    ...
    ABI45_0_0RNSVGPlatformView* parent = self.superview;
    while (parent != nil) {
        if ([parent isKindOfClass:[ABI45_0_0RNSVGText class]]) {
            ABI45_0_0RNSVGText* node = (ABI45_0_0RNSVGText*)parent;
    ...
    if (_alignmentBaseline == nil) {
        _alignmentBaseline = ABI45_0_0RNSVGAlignmentBaselineStrings[0];
    }
    return _alignmentBaseline;
}
...
- (ABI45_0_0RNSVGText*)getTextAnchorRoot
{
    ABI45_0_0RNSVGGlyphContext* gc = [self.textRoot getGlyphContext];
    NSArray* font = [gc getFontContext];
    ABI45_0_0RNSVGText* node = self;
    ABI45_0_0RNSVGPlatformView* parent = [self superview];
    for (NSInteger i = [font count] - 1; i >= 0; i--) {
        ABI45_0_0RNSVGFontData* fontData = [font objectAtIndex:i];
        if (![parent isKindOfClass:[ABI45_0_0RNSVGText class]] ||
            fontData->textAnchor == ABI45_0_0RNSVGTextAnchorStart ||
            node.positionX != nil) {
            return node;
        }
        node = (ABI45_0_0RNSVGText*) parent;
