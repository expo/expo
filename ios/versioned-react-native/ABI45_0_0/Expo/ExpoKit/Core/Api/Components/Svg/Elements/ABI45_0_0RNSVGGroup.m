...
@implementation ABI45_0_0RNSVGGroup
{
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI45_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI45_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI45_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI45_0_0RNSVGMask class]]) {
...
- (ABI45_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI45_0_0RNSVGNode class]];
    ...
    for (ABI45_0_0RNSVGView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI45_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI45_0_0RNSVGMask class]]) {
                continue;
            }
            ABI45_0_0RNSVGNode* svgNode = (ABI45_0_0RNSVGNode*)node;
            if (event) {
            ...
            ABI45_0_0RNSVGPlatformView *hitChild = [svgNode hitTest:transformed withEvent:event];
            if (hitChild) {
                svgNode.active = YES;
            ...
            ABI45_0_0RNSVGSvgView* svgView = (ABI45_0_0RNSVGSvgView*)node;
            ABI45_0_0RNSVGPlatformView *hitChild = [svgView hitTest:transformed withEvent:event];
            if (hitChild) {
                return hitChild;
            }
        }
    ...
    ABI45_0_0RNSVGPlatformView *hitSelf = [super hitTest:transformed withEvent:event];
    if (hitSelf) {
        return hitSelf;
    }
...
{
    [self traverseSubviews:^(__kindof ABI45_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI45_0_0RNSVGRenderable class]]) {
            [(ABI45_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
