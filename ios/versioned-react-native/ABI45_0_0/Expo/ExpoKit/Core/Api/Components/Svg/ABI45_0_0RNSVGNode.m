...
@interface ABI45_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI45_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI45_0_0RNSVGGroup *textRoot;
@end

@implementation ABI45_0_0RNSVGNode
{
    ABI45_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI45_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI45_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI45_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)insertABI45_0_0ReactSubview:(ABI45_0_0RNSVGView *)subview atIndex:(NSInteger)atIndex
{
...
- (void)removeABI45_0_0ReactSubview:(ABI45_0_0RNSVGView *)subview
{
...
{
    // Do nothing, as subviews are inserted by insertABI45_0_0ReactSubview:
}
...
- (void)invalidate
{
    ...
    _dirty = true;
    id<ABI45_0_0RNSVGContainer> container = (id<ABI45_0_0RNSVGContainer>)self.superview;
...
{
    [self clearPath];
    for (__kindof ABI45_0_0RNSVGNode *node in self.subviews) {
...
{
    ABI45_0_0RNSVGNode* node = self;
    while (node != nil) {
        ABI45_0_0RNSVGPlatformView* parent = [node superview];

        if (![parent isKindOfClass:[ABI45_0_0RNSVGNode class]]) {
            return;
        }
        node = (ABI45_0_0RNSVGNode*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (ABI45_0_0RNSVGGroup *)textRoot
{
    ...
    ABI45_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI45_0_0RNSVGGroup class]] && [((ABI45_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI45_0_0RNSVGGroup*)node;
            break;
        }
...
{
    ABI45_0_0RNSVGNode* parent = (ABI45_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI45_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI45_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI45_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)ABI45_0_0ReactSetInheritedBackgroundColor:(ABI45_0_0RNSVGColor *)inheritedBackgroundColor
{
...
- (void)setClipRule:(ABI45_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI45_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if (_cachedClipPath) {
            CGPathRelease(_cachedClipPath);
        }
...
- (ABI45_0_0RNSVGSvgView *)svgView
{
    if (_svgView) {
        return _svgView;
    }

    __kindof ABI45_0_0RNSVGPlatformView *parent = self.superview;

    if ([parent class] == [ABI45_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI45_0_0RNSVGNode class]]) {
        _svgView = ((ABI45_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI45_0_0RCTLogError(@"ABI45_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI45_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
{
    ABI45_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI45_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI45_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
