...
@interface ABI45_0_0RNCPicker() <UIPickerViewDataSource, UIPickerViewDelegate>
@end

@implementation ABI45_0_0RNCPicker
...
{
  return [ABI45_0_0RCTConvert NSString:_items[row][@"label"]];
}
           ...
           reusingView:(UIView *)view
{
  if (!view) {
      ...
      ABI45_0_0RNCPickerLabel* label = [[ABI45_0_0RNCPickerLabel alloc] initWithFrame:(CGRect) {
          CGPointZero,
          {
              rowWidth,
              rowHeight,
  ...
  ABI45_0_0RNCPickerLabel* label = view.subviews[0];
  label.font = _font;

  label.textColor = [ABI45_0_0RCTConvert UIColor:_items[row][@"textColor"]] ?: _color;
      ...
      didSelectRow:(NSInteger)row inComponent:(__unused NSInteger)component
{
  ...
  if (_onChange && _items.count > (NSUInteger)row) {
    _onChange(@{
      @"newIndex": @(row),
      @"newValue": ABI45_0_0RCTNullIfNil(_items[row][@"value"]),
    });
  }
}
