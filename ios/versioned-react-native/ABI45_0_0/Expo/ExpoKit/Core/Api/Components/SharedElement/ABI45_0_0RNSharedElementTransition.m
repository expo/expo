//
//  ABI45_0_0RNSharedElementTransition.m
...
@implementation ABI45_0_0RNSharedElementTransition
{
  NSArray* _items;
...
- (instancetype)initWithNodeManager:(ABI45_0_0RNSharedElementNodeManager*)nodeManager
{
  if ((self = [super init])) {
    _items = @[
      [[ABI45_0_0RNSharedElementTransitionItem alloc]initWithNodeManager:nodeManager name:@"startAncestor" isAncestor:YES],
      [[ABI45_0_0RNSharedElementTransitionItem alloc]initWithNodeManager:nodeManager name:@"endAncestor" isAncestor:YES],
      [[ABI45_0_0RNSharedElementTransitionItem alloc]initWithNodeManager:nodeManager name:@"startNode" isAncestor:NO],
      [[ABI45_0_0RNSharedElementTransitionItem alloc]initWithNodeManager:nodeManager name:@"endNode" isAncestor:NO]
    ];
    _nodePosition = 0.0f;
    _animation = ABI45_0_0RNSharedElementAnimationMove;
    _resize = ABI45_0_0RNSharedElementResizeStretch;
    _align = ABI45_0_0RNSharedElementAlignCenterCenter;
    _ABI45_0_0ReactFrameSet = NO;
...
- (void)removeFromSuperview
{
  ...
  for (ABI45_0_0RNSharedElementTransitionItem* item in _items) {
...
- (ABI45_0_0RNSharedElementTransitionItem*) findItemForNode:(ABI45_0_0RNSharedElementNode*) node
{
  for (ABI45_0_0RNSharedElementTransitionItem* item in _items) {
    if (item.node == node) {
      return item;
...
- (void)setStartNode:(ABI45_0_0RNSharedElementNode *)startNode
{
  ((ABI45_0_0RNSharedElementTransitionItem*)[_items objectAtIndex:ITEM_START]).node = startNode;
}

- (void)setEndNode:(ABI45_0_0RNSharedElementNode *)endNode
{
  ((ABI45_0_0RNSharedElementTransitionItem*)[_items objectAtIndex:ITEM_END]).node = endNode;
}

- (void)setStartAncestor:(ABI45_0_0RNSharedElementNode *)startNodeAncestor
{
  ((ABI45_0_0RNSharedElementTransitionItem*)[_items objectAtIndex:ITEM_START_ANCESTOR]).node = startNodeAncestor;
}

- (void)setEndAncestor:(ABI45_0_0RNSharedElementNode *)endNodeAncestor
{
  ((ABI45_0_0RNSharedElementTransitionItem*)[_items objectAtIndex:ITEM_END_ANCESTOR]).node = endNodeAncestor;
}
...
- (void) setAnimation:(ABI45_0_0RNSharedElementAnimation)animation
{
...
- (void) setResize:(ABI45_0_0RNSharedElementResize)resize
{
  if (_resize != resize) {
    _resize = resize;
    [self updateStyle];
...
- (void) setAlign:(ABI45_0_0RNSharedElementAlign)align
{
  if (_align != align) {
    _align = align;
    [self updateStyle];
...
- (void)updateNodeVisibility
{
  for (ABI45_0_0RNSharedElementTransitionItem* item in _items) {
    ...
    if (hidden && (_animation == ABI45_0_0RNSharedElementAnimationFadeIn) && [item.name isEqualToString:@"startNode"]) hidden = NO;
    if (hidden && (_animation == ABI45_0_0RNSharedElementAnimationFadeOut) && [item.name isEqualToString:@"endNode"]) hidden = NO;
    item.hidden = hidden;
  }
}
...
- (void) didLoadContent:(ABI45_0_0RNSharedElementContent*)content node:(id)node
{
  ...
  ABI45_0_0RNSharedElementTransitionItem* item = [self findItemForNode:node];
  if (item == nil) return;
  item.content = content;
  if ((content.type == ABI45_0_0RNSharedElementContentTypeSnapshotImage) || (content.type == ABI45_0_0RNSharedElementContentTypeRawImage)) {
    UIImage* image = (UIImage*) content.data;
    if (_animation == ABI45_0_0RNSharedElementAnimationMove) {
...
- (void) didLoadStyle:(ABI45_0_0RNSharedElementStyle *)style node:(ABI45_0_0RNSharedElementNode*)node
{
...
- (CGRect)normalizeLayout:(CGRect)layout
  compensateForTransforms:(BOOL)compensateForTransforms
                 ancestor:(ABI45_0_0RNSharedElementTransitionItem*)ancestor
            otherAncestor:(ABI45_0_0RNSharedElementTransitionItem*)otherAncestor

{
  ...
  if (compensateForTransforms && (ancestor.style != nil)) {
    ...
    ABI45_0_0RNSharedElementStyle* ancestorStyle = ancestor.style;
    ABI45_0_0RNSharedElementStyle* otherAncestorStyle = otherAncestor ? otherAncestor.style : nil;
...
- (CGFloat) getAncestorVisibility:(ABI45_0_0RNSharedElementStyle*)ancestorStyle
{
...
- (void) applyStyle:(ABI45_0_0RNSharedElementStyle*)style view:(UIView*)view
{
...
- (void) fireMeasureEvent:(ABI45_0_0RNSharedElementTransitionItem*) item layout:(CGRect)layout visibleLayout:(CGRect)visibleLayout contentLayout:(CGRect)contentLayout
{
  if (!self.onMeasureNode) return;
  ABI45_0_0RCTCornerRadii cornerRadii = [item.style.cornerRadii radiiForBounds:_outerStyleView.bounds];
...
- (void) updateStyle
{
  ...
  // Local data
  ABI45_0_0RNSharedElementTransitionItem* startItem = [_items objectAtIndex:ITEM_START];
  ABI45_0_0RNSharedElementTransitionItem* startAncestor = [_items objectAtIndex:ITEM_START_ANCESTOR];
  ABI45_0_0RNSharedElementTransitionItem* endItem = [_items objectAtIndex:ITEM_END];
  ABI45_0_0RNSharedElementTransitionItem* endAncestor = [_items objectAtIndex:ITEM_END_ANCESTOR];
  ABI45_0_0RNSharedElementStyle* startStyle = startItem.style;
  ABI45_0_0RNSharedElementStyle* endStyle = endItem.style;
  
  // Determine starting scene that is currently visible to the user
  if (_initialVisibleAncestorIndex < 0) {
    ABI45_0_0RNSharedElementStyle* startAncenstorStyle = startAncestor.style;
    ABI45_0_0RNSharedElementStyle* endAncestorStyle = endAncestor.style;
  ...
  ABI45_0_0RNSharedElementStyle* interpolatedStyle;
  ...
  if (startStyle && endStyle) {
    interpolatedStyle = [ABI45_0_0RNSharedElementStyle getInterpolatedStyle:startStyle style2:endStyle position:_nodePosition];
  ...
  [super ABI45_0_0ReactSetFrame:parentBounds];
  ...
  _innerClipView.layer.masksToBounds = _resize != ABI45_0_0RNSharedElementResizeNone;
  
  // Update content
  UIView* contentView1 = (startItem.content && startItem.content.type == ABI45_0_0RNSharedElementContentTypeSnapshotView) ? startItem.content.data : _primaryImageView;
  ...
  if (_animation == ABI45_0_0RNSharedElementAnimationMove) {
    ...
    UIView* contentView2 = (endItem.content && endItem.content.type == ABI45_0_0RNSharedElementContentTypeSnapshotView) ? endItem.content.data : _secondaryImageView;
    ...
    CGRect startContentLayout2 = startStyle ? [ABI45_0_0RNSharedElementContent layoutForRect:endStyle ? endContentLayout : startContentLayout content:startItem.content contentMode:startStyle.contentMode reverse:YES] : CGRectZero;
    CGRect endContentLayout1 = endStyle ? [ABI45_0_0RNSharedElementContent layoutForRect:startStyle ? startContentLayout : endContentLayout content:endItem.content contentMode:endStyle.contentMode reverse:YES] : CGRectZero;
    ...
    // Calculate new size
    switch (_resize) {
      case ABI45_0_0RNSharedElementResizeAuto:
        // Nothing to do
        break;
      case ABI45_0_0RNSharedElementResizeStretch:
        // TODO
        break;
      case ABI45_0_0RNSharedElementResizeClip:
      case ABI45_0_0RNSharedElementResizeNone:
    ...
    switch (_align) {
      case ABI45_0_0RNSharedElementAlignLeftTop:
        ...
        break;
      case ABI45_0_0RNSharedElementAlignLeftCenter:
        startInterpolatedContentLayout.origin.x = 0;
        ...
        break;
      case ABI45_0_0RNSharedElementAlignLeftBottom:
        startInterpolatedContentLayout.origin.x = 0;
        ...
        break;
      case ABI45_0_0RNSharedElementAlignRightTop:
        ...
        endInterpolatedContentLayout.origin.y = 0;
        break;
      case ABI45_0_0RNSharedElementAlignRightCenter:
        startInterpolatedContentLayout.origin.x = interpolatedLayout.size.width - startInterpolatedContentLayout.size.width;
        startInterpolatedContentLayout.origin.y = (interpolatedLayout.size.height - startInterpolatedContentLayout.size.height) / 2;
        endInterpolatedContentLayout.origin.x = interpolatedLayout.size.width - endInterpolatedContentLayout.size.width;
        endInterpolatedContentLayout.origin.y = (interpolatedLayout.size.height - endInterpolatedContentLayout.size.height) / 2;
        break;
      case ABI45_0_0RNSharedElementAlignRightBottom:
        startInterpolatedContentLayout.origin.x = interpolatedLayout.size.width - startInterpolatedContentLayout.size.width;
        startInterpolatedContentLayout.origin.y = interpolatedLayout.size.height - startInterpolatedContentLayout.size.height;
        endInterpolatedContentLayout.origin.x = interpolatedLayout.size.width - endInterpolatedContentLayout.size.width;
        endInterpolatedContentLayout.origin.y = interpolatedLayout.size.height - endInterpolatedContentLayout.size.height;
        break;
      case ABI45_0_0RNSharedElementAlignCenterTop:
        ...
        endInterpolatedContentLayout.origin.y = 0;
        break;
      case ABI45_0_0RNSharedElementAlignAuto:
      case ABI45_0_0RNSharedElementAlignCenterCenter:
        startInterpolatedContentLayout.origin.x = (interpolatedLayout.size.width - startInterpolatedContentLayout.size.width) / 2;
        startInterpolatedContentLayout.origin.y = (interpolatedLayout.size.height - startInterpolatedContentLayout.size.height) / 2;
        endInterpolatedContentLayout.origin.x = (interpolatedLayout.size.width - endInterpolatedContentLayout.size.width) / 2;
        endInterpolatedContentLayout.origin.y = (interpolatedLayout.size.height - endInterpolatedContentLayout.size.height) / 2;
        break;
      case ABI45_0_0RNSharedElementAlignCenterBottom:
        startInterpolatedContentLayout.origin.x = (interpolatedLayout.size.width - startInterpolatedContentLayout.size.width) / 2;
        startInterpolatedContentLayout.origin.y = interpolatedLayout.size.height - startInterpolatedContentLayout.size.height;
        endInterpolatedContentLayout.origin.x = (interpolatedLayout.size.width - endInterpolatedContentLayout.size.width) / 2;
        endInterpolatedContentLayout.origin.y = interpolatedLayout.size.height - endInterpolatedContentLayout.size.height;
    ...
    // Fade
    if (_animation == ABI45_0_0RNSharedElementAnimationFadeIn) {
      // Fade-in
      contentView1.layer.opacity = 0.0f;
      contentView2.layer.opacity = MIN(MAX(_nodePosition, 0.0f), 1.0f);
    }
    else if (_animation == ABI45_0_0RNSharedElementAnimationFadeOut) {
      // Fade-out
...
{
  // Only after the frame bounds have been set by the ABI45_0_0RN layout-system
  // we schedule a layout-fetch to run after these updates to ensure
  // that Yoga/UIManager has finished the initial layout pass.
  if (_ABI45_0_0ReactFrameSet == NO) {
    //NSLog(@"ABI45_0_0ReactSetFrame: %@", NSStringFromCGRect(frame));
    _ABI45_0_0ReactFrameSet = YES;
    dispatch_async(dispatch_get_main_queue(), ^{
      for (ABI45_0_0RNSharedElementTransitionItem* item in self->_items) {
        if (item.needsLayout) {
          item.needsLayout = NO;
          [item.node requestStyle:self];
        }
  ...
  // When ABI45_0_0React attempts to change the frame on this view,
