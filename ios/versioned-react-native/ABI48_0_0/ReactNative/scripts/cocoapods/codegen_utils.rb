...
class CodegenUtils
    ...
    def generate_react_codegen_podspec!(spec, codegen_output_dir)
        ...
        if @@REACT_CODEGEN_PODSPEC_GENERATED
          Pod::UI.puts "[Codegen] Skipping ABI48_0_0React-Codegen podspec generation."
          return
        end
        ...
        podspec_path = File.join(output_dir, 'ABI48_0_0React-Codegen.podspec.json')
    ...
    # It generates the podspec object that represents the `ABI48_0_0React-Codegen.podspec` file
    #
    # Parameters
    ...
    def get_react_codegen_spec(package_json_file, folly_version: '2021.07.22.00', fabric_enabled: false, hermes_enabled: true, script_phases: nil)
        ...
        spec = {
          ...
          'pod_target_xcconfig' => { "HEADER_SEARCH_PATHS" =>
            [
              "\"$(PODS_ROOT)/boost\"",
              "\"$(PODS_ROOT)/RCT-Folly\"",
              "\"${PODS_ROOT}/Headers/Public/ABI48_0_0React-Codegen/react/renderer/components\"",
              "\"$(PODS_ROOT)/Headers/Private/ABI48_0_0React-Fabric\"",
              "\"$(PODS_ROOT)/Headers/Private/ABI48_0_0React-RCTFabric\"",
            ].join(' ')
          },
          'dependencies': {
        ...
        if fabric_enabled
          spec[:'dependencies'].merge!({
            "ABI48_0_0React-graphics": [],
            "ABI48_0_0React-rncore":  [],
          });
        end

        if hermes_enabled
          spec[:'dependencies'].merge!({
            "ABI48_0_0hermes-engine": [], # let React Native decide which version of Hermes Engine to be used. Otherwise, this can create conflicts.
          });
        else
          spec[:'dependencies'].merge!({
            "ABI48_0_0React-jsc": [],
        ...
        if script_phases
          Pod::UI.puts "[Codegen] Adding script_phases to ABI48_0_0React-Codegen."
    ...
    def use_react_native_codegen_discovery!(
      ...
      # Generate ABI48_0_0React-Codegen podspec here to add the script phases.
