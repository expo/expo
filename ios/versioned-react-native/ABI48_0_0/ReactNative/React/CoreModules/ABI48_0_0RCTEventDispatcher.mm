...
static NSNumber *ABI48_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI48_0_0RCTUniqueCoalescingKeyGenerator = 0;

@interface ABI48_0_0RCTEventDispatcher () <ABI48_0_0RCTTurboModule>
@end

@implementation ABI48_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI48_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI48_0_0RCTEventDispatcherObserver>> *_observers;
...
- (void)sendViewEventWithName:(NSString *)name ABI48_0_0ReactTag:(NSNumber *)ABI48_0_0ReactTag
{
  [_callableJSModules invokeModule:@"ABI48_0_0RCTViewEventEmitter" method:@"emit" withArgs:@[ name, ABI48_0_0RCTNullIfNil(ABI48_0_0ReactTag) ]];
}

- (void)sendAppEventWithName:(NSString *)name body:(id)body
{
  [_callableJSModules invokeModule:@"ABI48_0_0RCTNativeAppEventEmitter"
                            method:@"emit"
...
{
  [_callableJSModules invokeModule:@"ABI48_0_0RCTDeviceEventEmitter" method:@"emit" withArgs:body ? @[ name, body ] : @[ name ]];
}

- (void)sendTextEventWithType:(ABI48_0_0RCTTextEventType)type
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI48_0_0RCTComponentEvent *event = [[ABI48_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI48_0_0ReactTag body:body];
  [self sendEvent:event];
}

- (void)notifyObserversOfEvent:(id<ABI48_0_0RCTEvent>)event
{
  [_observersLock lock];

  for (id<ABI48_0_0RCTEventDispatcherObserver> observer in _observers) {
...
- (void)sendEvent:(id<ABI48_0_0RCTEvent>)event
{
  ...
  if (event.canCoalesce) {
    eventID = ABI48_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI48_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI48_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI48_0_0RCTGetEventID(event.viewTag, event.eventName, ABI48_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI48_0_0RCTAssert(
        previousEvent == nil,
...
- (void)addDispatchObserver:(id<ABI48_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI48_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
Class ABI48_0_0RCTEventDispatcherCls(void)
{
  return ABI48_0_0RCTEventDispatcher.class;
}
