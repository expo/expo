...
#import <ABI48_0_0React/ABI48_0_0renderer/components/iostextinput/TextInputComponentDescriptor.h>
...
#import "ABI48_0_0RCTTextInputNativeCommands.h"
#import "ABI48_0_0RCTTextInputUtils.h"

#import "ABI48_0_0RCTFabricComponentsPlugins.h"

using namespace ABI48_0_0facebook::ABI48_0_0React;

@interface ABI48_0_0RCTTextInputComponentView () <ABI48_0_0RCTBackedTextInputDelegate, ABI48_0_0RCTTextInputViewProtocol>
@end

@implementation ABI48_0_0RCTTextInputComponentView {
  TextInputShadowNode::ConcreteState::Shared _state;
  UIView<ABI48_0_0RCTBackedTextInputViewProtocol> *_backedTextInputView;
   ...
   * if UITextView is backing text input view, inside `-[ABI48_0_0RCTTextInputComponentView textInputDidChangeSelection]` we make
   * sure to call `onChange` before `onSelectionChange` and ignore next `-[ABI48_0_0RCTTextInputComponentView
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _backedTextInputView = props.traits.multiline ? [ABI48_0_0RCTUITextView new] : [ABI48_0_0RCTUITextField new];
...
#pragma mark - ABI48_0_0RCTViewComponentView overrides
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  if (newTextInputProps.traits.autocapitalizationType != oldTextInputProps.traits.autocapitalizationType) {
    _backedTextInputView.autocapitalizationType =
        ABI48_0_0RCTUITextAutocapitalizationTypeFromAutocapitalizationType(newTextInputProps.traits.autocapitalizationType);
  }

  if (newTextInputProps.traits.autoCorrect != oldTextInputProps.traits.autoCorrect) {
    _backedTextInputView.autocorrectionType =
        ABI48_0_0RCTUITextAutocorrectionTypeFromOptionalBool(newTextInputProps.traits.autoCorrect);
  }
  ...
  if (newTextInputProps.traits.keyboardAppearance != oldTextInputProps.traits.keyboardAppearance) {
    _backedTextInputView.keyboardAppearance =
        ABI48_0_0RCTUIKeyboardAppearanceFromKeyboardAppearance(newTextInputProps.traits.keyboardAppearance);
  }

  if (newTextInputProps.traits.spellCheck != oldTextInputProps.traits.spellCheck) {
    _backedTextInputView.spellCheckingType =
        ABI48_0_0RCTUITextSpellCheckingTypeFromOptionalBool(newTextInputProps.traits.spellCheck);
  }
  ...
  if (newTextInputProps.traits.clearButtonMode != oldTextInputProps.traits.clearButtonMode) {
    _backedTextInputView.clearButtonMode =
        ABI48_0_0RCTUITextFieldViewModeFromTextInputAccessoryVisibilityMode(newTextInputProps.traits.clearButtonMode);
  }
  ...
  if (newTextInputProps.traits.keyboardType != oldTextInputProps.traits.keyboardType) {
    _backedTextInputView.keyboardType = ABI48_0_0RCTUIKeyboardTypeFromKeyboardType(newTextInputProps.traits.keyboardType);
  }

  if (newTextInputProps.traits.returnKeyType != oldTextInputProps.traits.returnKeyType) {
    _backedTextInputView.returnKeyType = ABI48_0_0RCTUIReturnKeyTypeFromReturnKeyType(newTextInputProps.traits.returnKeyType);
  }

  if (newTextInputProps.traits.textContentType != oldTextInputProps.traits.textContentType) {
    _backedTextInputView.textContentType = ABI48_0_0RCTUITextContentTypeFromString(newTextInputProps.traits.textContentType);
  }

  if (newTextInputProps.traits.passwordRules != oldTextInputProps.traits.passwordRules) {
    if (@available(iOS 12.0, *)) {
      _backedTextInputView.passwordRules =
          ABI48_0_0RCTUITextInputPasswordRulesFromString(newTextInputProps.traits.passwordRules);
    }
  }
  ...
  if (newTextInputProps.placeholder != oldTextInputProps.placeholder) {
    _backedTextInputView.placeholder = ABI48_0_0RCTNSStringFromString(newTextInputProps.placeholder);
  }

  if (newTextInputProps.placeholderTextColor != oldTextInputProps.placeholderTextColor) {
    _backedTextInputView.placeholderColor = ABI48_0_0RCTUIColorFromSharedColor(newTextInputProps.placeholderTextColor);
  }

  if (newTextInputProps.textAttributes != oldTextInputProps.textAttributes) {
    _backedTextInputView.defaultTextAttributes =
        ABI48_0_0RCTNSTextAttributesFromTextAttributes(newTextInputProps.getEffectiveTextAttributes(ABI48_0_0RCTFontSizeMultiplier()));
  }

  if (newTextInputProps.selectionColor != oldTextInputProps.selectionColor) {
    _backedTextInputView.tintColor = ABI48_0_0RCTUIColorFromSharedColor(newTextInputProps.selectionColor);
  }

  if (newTextInputProps.inputAccessoryViewID != oldTextInputProps.inputAccessoryViewID) {
    _backedTextInputView.inputAccessoryViewID = ABI48_0_0RCTNSStringFromString(newTextInputProps.inputAccessoryViewID);
  }
...
- (void)updateState:(State::Shared const &)state oldState:(State::Shared const &)oldState
{
  ...
  if (_mostRecentEventCount == _state->getData().mostRecentEventCount) {
    _comingFromJS = YES;
    [self _setAttributedString:ABI48_0_0RCTNSAttributedStringFromAttributedStringBox(data.attributedStringBox)];
    _comingFromJS = NO;
  }
}
           ...
           oldLayoutMetrics:(LayoutMetrics const &)oldLayoutMetrics
{
  ...
  _backedTextInputView.frame =
      UIEdgeInsetsInsetRect(self.bounds, ABI48_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.borderWidth));
  _backedTextInputView.textContainerInset =
      ABI48_0_0RCTUIEdgeInsetsFromEdgeInsets(layoutMetrics.contentInsets - layoutMetrics.borderWidth);
}
...
#pragma mark - ABI48_0_0RCTBackedTextInputDelegate
...
- (NSString *)textInputShouldChangeText:(NSString *)text inRange:(NSRange)range
{
  ...
  if (!_backedTextInputView.textWasPasted) {
    if (_eventEmitter) {
      ...
      keyPressMetrics.text = ABI48_0_0RCTStringFromNSString(text);
...
#pragma mark - ABI48_0_0RCTBackedTextInputDelegate (UIScrollViewDelegate)
...
{
  ABI48_0_0RCTTextInputHandleCommand(self, commandName, args);
}
...
- (TextInputMetrics)_textInputMetrics
{
  ...
  metrics.text = ABI48_0_0RCTStringFromNSString(_backedTextInputView.attributedText.string);
...
- (void)_updateState
{
  ...
  data.attributedStringBox = ABI48_0_0RCTAttributedStringBoxFromNSAttributedString(attributedString);
...
- (void)_setMultiline:(BOOL)multiline
{
  ...
  UIView<ABI48_0_0RCTBackedTextInputViewProtocol> *backedTextInputView = multiline ? [ABI48_0_0RCTUITextView new] : [ABI48_0_0RCTUITextField new];
  backedTextInputView.frame = _backedTextInputView.frame;
  ABI48_0_0RCTCopyBackedTextInput(_backedTextInputView, backedTextInputView);
...
Class<ABI48_0_0RCTComponentViewProtocol> ABI48_0_0RCTTextInputCls(void)
{
  return ABI48_0_0RCTTextInputComponentView.class;
}
