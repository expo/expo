...
#import "ABI48_0_0RCTSurfaceTouchHandler.h"

#import <ABI48_0_0React/ABI48_0_0RCTABI48_0_0ReactTaggedView.h>
...
#import "ABI48_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI48_0_0RCTTouchEventType) {
  ABI48_0_0RCTTouchEventTypeTouchStart,
  ABI48_0_0RCTTouchEventTypeTouchMove,
  ABI48_0_0RCTTouchEventTypeTouchEnd,
  ABI48_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI48_0_0RCTComponentViewProtocol> *componentView = nil;
    ...
    CGPoint rootViewOriginOffset)
{
  ...
  activeTouch.touch.offsetPoint = ABI48_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI48_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI48_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI48_0_0RCTForceTouchAvailable()) {
    activeTouch.touch.force = ABI48_0_0RCTZeroIfNaN(uiTouch.force / uiTouch.maximumPossibleForce);
  }
...
CreateTouchWithUITouch(UITouch *uiTouch, UIEvent *uiEvent, UIView *rootComponentView, CGPoint rootViewOriginOffset)
{
  ...
  while (componentView) {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      activeTouch.eventEmitter = [(id<ABI48_0_0RCTTouchableComponentViewProtocol>)componentView
...
static NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *GetTouchableViewsInPathToRoot(UIView *componentView)
{
  NSMutableOrderedSet *results = [NSMutableOrderedSet orderedSet];
  do {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      [results addObject:[ABI48_0_0RCTABI48_0_0ReactTaggedView wrap:componentView]];
    }
    componentView = componentView.superview;
  } while (componentView);
  return results;
...
{
  return [(id<ABI48_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitterAtPoint:point];
}
...
static PointerEvent CreatePointerEventFromActiveTouch(ActiveTouch activeTouch, ABI48_0_0RCTTouchEventType eventType)
{
  ...
  if (@available(iOS 13.4, *)) {
    if (activeTouch.touchType == UITouchTypeIndirectPointer) {
      ...
      event.pressure = eventType != ABI48_0_0RCTTouchEventTypeTouchEnd ? 0.5 : 0.0;
    }
  }
    ...
    UIKeyModifierFlags modifierFlags)
{
  ...
  event.clientPoint = ABI48_0_0RCTPointFromCGPoint(clientLocation);
  event.screenPoint = ABI48_0_0RCTPointFromCGPoint(screenLocation);
  event.offsetPoint = ABI48_0_0RCTPointFromCGPoint(offsetLocation);
  event.width = 1.0;
  event.height = 1.0;
  event.tiltX = 0;
  event.tiltY = 0;
...
static BOOL IsViewListeningToEvent(ABI48_0_0RCTABI48_0_0ReactTaggedView *taggedView, ViewEvents::Offset eventType)
{
  UIView *view = taggedView.view;
  if (view && [view.class conformsToProtocol:@protocol(ABI48_0_0RCTComponentViewProtocol)]) {
    auto props = ((id<ABI48_0_0RCTComponentViewProtocol>)view).props;
...
static BOOL IsAnyViewInPathListeningToEvent(NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *viewPath, ViewEvents::Offset eventType)
{
  for (ABI48_0_0RCTABI48_0_0ReactTaggedView *taggedView in viewPath) {
...
@interface ABI48_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI48_0_0RCTSurfaceTouchHandler {
  ...
  NSMutableDictionary<NSNumber *, NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *> *_currentlyHoveredViewsPerPointer;
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI48_0_0React Native,
...
- (void)attachToView:(UIView *)view
{
  ...
  if (ABI48_0_0RCTGetDispatchW3CPointerEvents()) {
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI48_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI48_0_0RCTTouchEventTypeTouchEnd || eventType == ABI48_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &activeTouch : activeTouches) {
    ...
    if (ABI48_0_0RCTGetDispatchW3CPointerEvents()) {
      ...
      if ((eventType == ABI48_0_0RCTTouchEventTypeTouchEnd && activeTouch.shouldLeaveWhenReleased)) {
        ...
        NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *eventPathViews = [self handleIncomingPointerEvent:pointerEvent
                                                                                       onView:targetView];

        switch (eventType) {
          case ABI48_0_0RCTTouchEventTypeTouchStart:
            activeTouch.eventEmitter->onPointerDown(pointerEvent);
            break;
          case ABI48_0_0RCTTouchEventTypeTouchMove: {
          ...
          case ABI48_0_0RCTTouchEventTypeTouchEnd:
            activeTouch.eventEmitter->onPointerUp(pointerEvent);
            break;
          case ABI48_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI48_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI48_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI48_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI48_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (!_activeTouches.empty()) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI48_0_0RCTTouchEventTypeTouchCancel];
...
- (void)hovering:(UIHoverGestureRecognizer *)recognizer API_AVAILABLE(ios(13.0))
{
  ...
  NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *eventPathViews = [self handleIncomingPointerEvent:event onView:targetView];
 ...
 * should be fired.
 */
- (NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *)handleIncomingPointerEvent:(PointerEvent)event
                                                            onView:(nullable UIView *)targetView
{
  ...
  NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *currentlyHoveredViews =
  ...
  ABI48_0_0RCTABI48_0_0ReactTaggedView *targetTaggedView = [ABI48_0_0RCTABI48_0_0ReactTaggedView wrap:targetView];
  ABI48_0_0RCTABI48_0_0ReactTaggedView *prevTargetTaggedView = [currentlyHoveredViews firstObject];
  UIView *prevTargetView = prevTargetTaggedView.view;

  NSOrderedSet<ABI48_0_0RCTABI48_0_0ReactTaggedView *> *eventPathViews = GetTouchableViewsInPathToRoot(targetView);
  ...
  for (ABI48_0_0RCTABI48_0_0ReactTaggedView *taggedView in [currentlyHoveredViews reverseObjectEnumerator]) {
  ...
  for (ABI48_0_0RCTABI48_0_0ReactTaggedView *taggedView in [eventPathViews reverseObjectEnumerator]) {
    UIView *componentView = taggedView.view;
