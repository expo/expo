...
@interface ABI48_0_0RCTSurfaceHostingView ()
...
@implementation ABI48_0_0RCTSurfaceHostingView {
...
{
  return [[ABI48_0_0RCTSurface alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProperties];
}

ABI48_0_0RCT_NOT_IMPLEMENTED(-(instancetype)init)
ABI48_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithFrame : (CGRect)frame)
ABI48_0_0RCT_NOT_IMPLEMENTED(-(nullable instancetype)initWithCoder : (NSCoder *)coder)
...
{
  id<ABI48_0_0RCTSurfaceProtocol> surface = [[self class] createSurfaceWithBridge:bridge
                ...
                sizeMeasureMode:(ABI48_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    // For backward compatibility with ABI48_0_0RCTRootView, set a color here instead of transparent (OS default).
...
- (void)setFrame:(CGRect)frame
{
  ...
  ABI48_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(
...
{
  if (ABI48_0_0RCTSurfaceStageIsPreparing(_stage)) {
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  ABI48_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(size, _sizeMeasureMode, &minimumSize, &maximumSize);

  return [_surface sizeThatFitsMinimumSize:minimumSize maximumSize:maximumSize];
}

- (void)setStage:(ABI48_0_0RCTSurfaceStage)stage
{
  if (stage == _stage) {
    return;
  }

  BOOL shouldInvalidateLayout = ABI48_0_0RCTSurfaceStageIsRunning(stage) != ABI48_0_0RCTSurfaceStageIsRunning(_stage) ||
      ABI48_0_0RCTSurfaceStageIsPreparing(stage) != ABI48_0_0RCTSurfaceStageIsPreparing(_stage);
...
- (void)setSizeMeasureMode:(ABI48_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
...
- (void)setActivityIndicatorViewFactory:(ABI48_0_0RCTSurfaceHostingViewActivityIndicatorViewFactory)activityIndicatorViewFactory
{
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection
{
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI48_0_0RCTUserInterfaceStyleDidChangeNotification
                    object:self
                  userInfo:@{
                    ABI48_0_0RCTUserInterfaceStyleDidChangeNotificationTraitCollectionKey : self.traitCollection,
                  }];
}
...
- (void)_updateViews
{
  self.isSurfaceViewVisible = ABI48_0_0RCTSurfaceStageIsRunning(_stage);
  self.isActivityIndicatorViewVisible = ABI48_0_0RCTSurfaceStageIsPreparing(_stage);
}
...
#pragma mark - ABI48_0_0RCTSurfaceDelegate

- (void)surface:(__unused ABI48_0_0RCTSurface *)surface didChangeStage:(ABI48_0_0RCTSurfaceStage)stage
{
...
- (void)surface:(__unused ABI48_0_0RCTSurface *)surface didChangeIntrinsicSize:(__unused CGSize)intrinsicSize
{
  ABI48_0_0RCTExecuteOnMainQueue(^{
    [self _invalidateLayout];
  });
