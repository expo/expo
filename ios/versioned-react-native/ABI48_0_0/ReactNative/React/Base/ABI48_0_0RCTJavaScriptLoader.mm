...
#import "ABI48_0_0RCTJavaScriptLoader.h"
...
#import "ABI48_0_0RCTMultipartDataTask.h"
#import "ABI48_0_0RCTPerformanceLogger.h"
#import "ABI48_0_0RCTUtils.h"

NSString *const ABI48_0_0RCTJavaScriptLoaderErrorDomain = @"ABI48_0_0RCTJavaScriptLoaderErrorDomain";

const uint32_t ABI48_0_0RCT_BYTECODE_ALIGNMENT = 4;

@interface ABI48_0_0RCTSource () {
 @public
  NSURL *_url;
  NSData *_data;
  NSUInteger _length;
...
@implementation ABI48_0_0RCTSource

static ABI48_0_0RCTSource *ABI48_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  using ABI48_0_0facebook::ABI48_0_0React::ScriptTag;
  ABI48_0_0facebook::ABI48_0_0React::BundleHeader header;
  [data getBytes:&header length:sizeof(header)];

  ABI48_0_0RCTSource *source = [ABI48_0_0RCTSource new];
  source->_url = url;
  // Multipart responses may give us an unaligned view into the buffer. This ensures memory is aligned.
  if (parseTypeFromHeader(header) == ScriptTag::MetroHBCBundle && ((long)[data bytes] % ABI48_0_0RCT_BYTECODE_ALIGNMENT)) {
    source->_data = [[NSData alloc] initWithData:data];
  } else {
    source->_data = data;
  }
  source->_length = length;
  source->_filesChangedCount = ABI48_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI48_0_0RCTLoadingProgress
...
@implementation ABI48_0_0RCTJavaScriptLoader

ABI48_0_0RCT_NOT_IMPLEMENTED(-(instancetype)init)

+ (void)loadBundleAtURL:(NSURL *)scriptURL
             onProgress:(ABI48_0_0RCTSourceLoadProgressBlock)onProgress
             onComplete:(ABI48_0_0RCTSourceLoadBlock)onComplete
{
  int64_t sourceLength;
  NSError *error;
  NSData *data = [self attemptSynchronousLoadOfBundleAtURL:scriptURL sourceLength:&sourceLength error:&error];
  if (data) {
    onComplete(nil, ABI48_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError = [error.domain isEqualToString:ABI48_0_0RCTJavaScriptLoaderErrorDomain] &&
      error.code == ABI48_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI48_0_0RCTJavaScriptLoaderErrorNoScriptURL
                 userInfo:@{
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI48_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI48_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                 userInfo:@{
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI48_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                 userInfo:@{
  ...
  ABI48_0_0facebook::ABI48_0_0React::ScriptTag tag = ABI48_0_0facebook::ABI48_0_0React::parseTypeFromHeader(header);
  switch (tag) {
    case ABI48_0_0facebook::ABI48_0_0React::ScriptTag::MetroHBCBundle:
    case ABI48_0_0facebook::ABI48_0_0React::ScriptTag::RAMBundle:
      break;

    case ABI48_0_0facebook::ABI48_0_0React::ScriptTag::String: {
      ...
      if (error) {
        *error =
            [NSError errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                                code:ABI48_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI48_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                 userInfo:@{
...
static void parseHeaders(NSDictionary *headers, ABI48_0_0RCTSource *source)
{
...
static void attemptAsynchronousLoadOfBundleAtURL(
    NSURL *scriptURL,
    ABI48_0_0RCTSourceLoadProgressBlock onProgress,
    ABI48_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      NSError *error = nil;
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path options:NSDataReadingMappedIfSafe error:&error];
      onComplete(error, ABI48_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI48_0_0RCTMultipartDataTask *task = [[ABI48_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL
      partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
        ...
        if (error) {
          if ([error.domain isEqualToString:NSURLErrorDomain]) {
            error = [NSError
                errorWithDomain:ABI48_0_0RCTJavaScriptLoaderErrorDomain
                           code:ABI48_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                       userInfo:@{
                         NSLocalizedDescriptionKey :
                             [@"Could not connect to development server.\n\n"
                               "Ensure the following:\n"
                               "- Node server is running and available on the same network - run 'npm start' from ABI48_0_0React-native root\n"
        ...
        ABI48_0_0RCTSource *source = ABI48_0_0RCTSourceCreate(scriptURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI48_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI48_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI48_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI48_0_0RCTLoadingProgress *progress = [ABI48_0_0RCTLoadingProgress new];
...
static ABI48_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI48_0_0RCTLoadingProgress *progress = [ABI48_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI48_0_0RCTJSONParse(rawText, nil);
