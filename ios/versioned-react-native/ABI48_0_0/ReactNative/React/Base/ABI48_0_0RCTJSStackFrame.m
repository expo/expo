 ...
 */
static NSRegularExpression *ABI48_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI48_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI48_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI48_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI48_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI48_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI48_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI48_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI48_0_0RCTNilIfNull(dict[@"collapse"]) boolValue]];
}
...
+ (NSArray<ABI48_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI48_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI48_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI48_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
