...
static MethodCallResult invokeInner(
    ...
    ABI48_0_0RCTModuleData *moduleData,
    unsigned int methodId,
    const folly::dynamic &params,
    int callId,
    SchedulingContext context);

ABI48_0_0RCTNativeModule::ABI48_0_0RCTNativeModule(ABI48_0_0RCTBridge *bridge, ABI48_0_0RCTModuleData *moduleData)
    : m_bridge(bridge), m_moduleData(moduleData)
{
}

std::string ABI48_0_0RCTNativeModule::getName()
{
  return [m_moduleData.name UTF8String];
}

std::string ABI48_0_0RCTNativeModule::getSyncMethodName(unsigned int methodId)
{
  return m_moduleData.methods[methodId].JSMethodName;
}

std::vector<MethodDescriptor> ABI48_0_0RCTNativeModule::getMethods()
{
  std::vector<MethodDescriptor> descs;

  for (id<ABI48_0_0RCTBridgeMethod> method in m_moduleData.methods) {
    descs.emplace_back(method.JSMethodName, ABI48_0_0RCTFunctionDescriptorFromType(method.functionType));
  }

  return descs;
}

folly::dynamic ABI48_0_0RCTNativeModule::getConstants()
{
  ABI48_0_0RCT_PROFILE_BEGIN_EVENT(ABI48_0_0RCTProfileTagAlways, @"[ABI48_0_0RCTNativeModule getConstants] moduleData.exportedConstants", nil);
...
void ABI48_0_0RCTNativeModule::invoke(unsigned int methodId, folly::dynamic &&params, int callId)
{
  id<ABI48_0_0RCTBridgeMethod> method = m_moduleData.methods[methodId];
  if (method) {
    ABI48_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI48_0_0RCTProfileTagAlways,
        @"[ABI48_0_0RCTNativeModule invoke]",
  ...
  const bool isSyncModule = queue == ABI48_0_0RCTJSThread;
  ...
  __weak ABI48_0_0RCTBridge *weakBridge = m_bridge;
  __weak ABI48_0_0RCTModuleData *weakModuleData = m_moduleData;
...
MethodCallResult ABI48_0_0RCTNativeModule::callSerializableNativeHook(unsigned int ABI48_0_0ReactMethodId, folly::dynamic &&params)
{
  return invokeInner(m_bridge, m_moduleData, ABI48_0_0ReactMethodId, params, 0, Sync);
}
    ...
    SchedulingContext context)
{
  ...
  id<ABI48_0_0RCTBridgeMethod> method = moduleData.methods[methodId];
  if (ABI48_0_0RCT_DEBUG && !method) {
    ABI48_0_0RCTLogError(@"Unknown methodID: %ud for module: %@", methodId, moduleData.name);
  }
  ...
  ABI48_0_0RCT_PROFILE_BEGIN_EVENT(
      ABI48_0_0RCTProfileTagAlways,
      @"[ABI48_0_0RCTNativeModule invokeInner]",
      @{@"method" : [NSString stringWithUTF8String:method.JSMethodName]});
  @try {
    if (context == Sync) {
    ...
    if ([exception.name hasPrefix:ABI48_0_0RCTFatalExceptionName]) {
      @throw exception;
    }
...
#else
    ABI48_0_0RCTFatalException(exception);
#endif
  } @finally {
    ABI48_0_0RCT_PROFILE_END_EVENT(ABI48_0_0RCTProfileTagAlways, @"");
  }
