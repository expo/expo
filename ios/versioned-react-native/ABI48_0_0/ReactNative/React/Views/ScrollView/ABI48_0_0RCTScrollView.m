...
#import "ABI48_0_0RCTViewUtils.h"
 ...
 */
@interface ABI48_0_0RCTCustomScrollView : UIScrollView <UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) UIView<ABI48_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
@property (nonatomic, assign) BOOL pinchGestureEnabled;

@end

@implementation ABI48_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI48_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI48_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI48_0_0RCTUIManager JSResponder] isKindOfClass:[ABI48_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI48_0_0React scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
      ABI48_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
      ABI48_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
- (void)setCustomRefreshControl:(UIView<ABI48_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
  ...
  // `ABI48_0_0RCTCustomRefreshContolProtocol`'s superview will always be of class
...
@interface ABI48_0_0RCTScrollView () <ABI48_0_0RCTUIManagerObserver>

@end

@implementation ABI48_0_0RCTScrollView {
  id<ABI48_0_0RCTEventDispatcherProtocol> _eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI48_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(id<ABI48_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  ABI48_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    [self _registerKeyboardListener];
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI48_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI48_0_0RCTApplyTransformationAccordingLayoutDirection(
    UIView *view,
...
- (void)setABI48_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  [super setABI48_0_0ReactLayoutDirection:layoutDirection];

  ABI48_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI48_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI48_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI48_0_0ReactSubview:view atIndex:atIndex];
  if ([view conformsToProtocol:@protocol(ABI48_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI48_0_0RCTCustomRefreshContolProtocol> *)view];
    ...
    ABI48_0_0RCTAssert(
        _contentView == nil,
        @"ABI48_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@",
        [_contentView ABI48_0_0React_recursiveDescription]);
    _contentView = view;
    ABI48_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI48_0_0ReactLayoutDirection);
...
- (void)removeABI48_0_0ReactSubview:(UIView *)subview
{
  ...
  if ([subview conformsToProtocol:@protocol(ABI48_0_0RCTCustomRefreshContolProtocol)]) {
    ...
    ABI48_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI48_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI48_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI48_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI48_0_0React_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ABI48_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI48_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:NO];
...
{
  [ABI48_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:YES];
}

#pragma mark - ScrollView delegate

#define ABI48_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData)                                    \
  {                                                                                     \
...
#define ABI48_0_0RCT_FORWARD_SCROLL_EVENT(call)                                            \
...
#define ABI48_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
  -(void)delegateMethod : (UIScrollView *)scrollView        \
  {                                                         \
    ABI48_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                  \
    ABI48_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod : scrollView);  \
  }

ABI48_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI48_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI48_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    ABI48_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll : scrollView);
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  _allowNextScrollNoMatterWhat = YES; // Ensure next scroll event is recorded, regardless of throttle
  ABI48_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging : scrollView);
}
              ...
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI48_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging
...
{
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging : scrollView willDecelerate : decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI48_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming : scrollView withView : view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI48_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming : scrollView withView : view atScale : scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI48_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating : scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI48_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI48_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation : scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = ABI48_0_0RCTContentInsets(self);
...
- (void)uiManagerWillPerformMounting:(ABI48_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(
               __unused ABI48_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI48_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI48_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI48_0_0RCTScrollEvent *scrollEvent = [[ABI48_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                 ABI48_0_0ReactTag:self.ABI48_0_0ReactTag
...
void ABI48_0_0RCTSendFakeScrollEvent(id<ABI48_0_0RCTEventDispatcherProtocol> eventDispatcher, NSNumber *ABI48_0_0ReactTag)
{
  ...
  ABI48_0_0RCTScrollEvent *fakeScrollEvent = [[ABI48_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI48_0_0ReactTag:ABI48_0_0ReactTag
