...
#import <ABI48_0_0React/ABI48_0_0RCTMockDef.h>

#import "ABI48_0_0RCTAutoInsetsProtocol.h"
...
ABI48_0_0RCT_MOCK_DEF(ABI48_0_0RCTView, ABI48_0_0RCTContentInsets);
#define ABI48_0_0RCTContentInsets ABI48_0_0RCT_MOCK_USE(ABI48_0_0RCTView, ABI48_0_0RCTContentInsets)

UIAccessibilityTraits const ABI48_0_0SwitchAccessibilityTrait = 0x20000000000001;

@implementation UIView (ABI48_0_0RCTViewUnmounting)
...
- (void)ABI48_0_0React_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI48_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI48_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI48_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ABI48_0_0React_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isABI48_0_0ReactRootView]) {
      break;
    }
...
static NSString *ABI48_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI48_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI48_0_0RCTView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderCurve = ABI48_0_0RCTBorderCurveCircular;
    _borderStyle = ABI48_0_0RCTBorderStyleSolid;
...
ABI48_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : unused)

- (void)setABI48_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ABI48_0_0ReactLayoutDirection != layoutDirection) {
    _ABI48_0_0ReactLayoutDirection = layoutDirection;
...
- (void)setPointerEvents:(ABI48_0_0RCTPointerEvents)pointerEvents
{
  ...
  self.userInteractionEnabled = (pointerEvents != ABI48_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI48_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI48_0_0RCTPointerEventsNone || _pointerEvents == ABI48_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI48_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI48_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI48_0_0RCTLogError(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI48_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  ...
  if (actionObject) {
    _onAccessibilityAction(@{@"actionName" : actionObject[@"name"], @"actionTarget" : self.ABI48_0_0ReactTag});
  }
  return YES;
}
...
- (NSString *)accessibilityValue
{
  ...
  // Handle Switch.
  if ((self.accessibilityTraits & ABI48_0_0SwitchAccessibilityTrait) == ABI48_0_0SwitchAccessibilityTrait) {
  ...
  // Handle states which haven't already been handled in ABI48_0_0RCTViewManager.
  for (NSString *state in self.accessibilityState) {
    id val = self.accessibilityState[state];
    if (!val) {
      continue;
...
- (UIView *)ABI48_0_0ReactAccessibilityElement
{
  return self;
}
...
{
  if (self.ABI48_0_0ReactAccessibilityElement == self) {
...
- (BOOL)performAccessibilityAction:(NSString *)name
{
  if (_onAccessibilityAction && accessibilityActionsNameMap[name]) {
    _onAccessibilityAction(@{@"actionName" : name, @"actionTarget" : self.ABI48_0_0ReactTag});
    return YES;
  }
  return NO;
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ABI48_0_0ReactTag: %@;", self.ABI48_0_0ReactTag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI48_0_0RCTAutoInsetsProtocol> *)parentView
                   ...
                   updateOffset:(BOOL)updateOffset
{
  ...
  if (parentView.automaticallyAdjustContentInsets) {
    UIEdgeInsets autoInset = ABI48_0_0RCTContentInsets(parentView);
...
- (void)ABI48_0_0React_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ABI48_0_0ReactSubviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ABI48_0_0React_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ABI48_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI48_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ABI48_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ABI48_0_0ReactSubviews.count == 0) {
  ...
  for (UIView *view in self.ABI48_0_0ReactSubviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ABI48_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ABI48_0_0React_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ABI48_0_0React_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI48_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x)
{
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ABI48_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI48_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI48_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI48_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets){
        ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
        ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
        ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
        ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets){
      ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, ABI48_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
      ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI48_0_0RCTDefaultIfNegativeTo(borderWidth, ABI48_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI48_0_0RCTCornerRadii)cornerRadii
{
  ...
  if ([[ABI48_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI48_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, _borderTopStartRadius);
    const CGFloat topEndRadius = ABI48_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, _borderTopEndRadius);
    const CGFloat bottomStartRadius = ABI48_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, _borderBottomStartRadius);
    const CGFloat bottomEndRadius = ABI48_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, _borderBottomEndRadius);
    ...
    topLeftRadius = ABI48_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI48_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI48_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI48_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius =
        ABI48_0_0RCTDefaultIfNegativeTo(radius, ABI48_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius =
        ABI48_0_0RCTDefaultIfNegativeTo(radius, ABI48_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius =
        ABI48_0_0RCTDefaultIfNegativeTo(radius, ABI48_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI48_0_0RCTDefaultIfNegativeTo(
        radius, ABI48_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI48_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI48_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI48_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI48_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI48_0_0RCTCornerRadii){
...
- (ABI48_0_0RCTBorderColors)borderColorsWithTraitCollection:(UITraitCollection *)traitCollection
{
  const BOOL isRTL = _ABI48_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;
  ...
  return (ABI48_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI48_0_0RCTUpdateShadowPathForView(self);

  const ABI48_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI48_0_0RCTBorderColors borderColors = [self borderColorsWithTraitCollection:self.traitCollection];

  BOOL useIOSBorderRendering = ABI48_0_0RCTCornerRadiiAreEqual(cornerRadii) && ABI48_0_0RCTBorderInsetsAreEqual(borderInsets) &&
      ABI48_0_0RCTBorderColorsAreEqual(borderColors) && _borderStyle == ABI48_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI48_0_0RCTGetBorderImage(
...
static BOOL ABI48_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI48_0_0RCTUpdateShadowPathForView(ABI48_0_0RCTView *view)
{
  if (ABI48_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {
      // If view has a solid background color, calculate shadow path from border
      const ABI48_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI48_0_0RCTCornerInsets cornerInsets = ABI48_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI48_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    const ABI48_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI48_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      cornerRadius = cornerRadii.topLeft;

    } else {
      CAShapeLayer *shapeLayer = [CAShapeLayer layer];
      CGPathRef path =
          ABI48_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI48_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderCurve(side)                            \
  -(void)setBorder##side##Curve : (ABI48_0_0RCTBorderCurve)curve \
  {                                                     \
...
#define setBorderStyle(side)                            \
  -(void)setBorder##side##Style : (ABI48_0_0RCTBorderStyle)style \
  {                                                     \
