...
typedef void (^ABI48_0_0RCTPropBlock)(id<ABI48_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI48_0_0RCTPropBlock> ABI48_0_0RCTPropBlockDictionary;
...
@implementation ABI48_0_0RCTComponentData {
  id<ABI48_0_0RCTComponent> _defaultView; // Only needed for ABI48_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI48_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI48_0_0RCTPropBlockDictionary *_shadowPropBlocks;
  __weak ABI48_0_0RCTBridge *_bridge;
  __weak id<ABI48_0_0RCTEventDispatcherProtocol> _eventDispatcher;
}
...
- (instancetype)initWithManagerClass:(Class)managerClass
                     ...
                     eventDispatcher:(id<ABI48_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  if ((self = [super init])) {
    _bridge = bridge;
...
- (ABI48_0_0RCTViewManager *)manager
{
  if (!_manager && _bridge) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI48_0_0RCTDidInitializeModuleNotification
                                                        object:nil
...
{
  ABI48_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI48_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI48_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI48_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI48_0_0RCTPropBlock createEventSetter(
    NSString *propName,
    SEL setter,
    InterceptorBlock eventInterceptor,
    id<ABI48_0_0RCTEventDispatcherProtocol> eventDispatcher)
{
  __weak id<ABI48_0_0RCTEventDispatcherProtocol> weakEventDispatcher = eventDispatcher;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI48_0_0RCTConvert BOOL:json]) {
      __weak id<ABI48_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI48_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI48_0_0ReactTag);
        } else {
          ABI48_0_0RCTComponentEvent *componentEvent = [[ABI48_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI48_0_0ReactTag
                                                                                 body:event];
...
static ABI48_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI48_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI48_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI48_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI48_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI48_0_0RCTDirectEventBlock:") ||
        type == NSSelectorFromString(@"ABI48_0_0RCTCapturingEventBlock:")) {
      // Special case for event handlers
      setterBlock =
          createEventSetter(name, setter, self.eventInterceptor, _bridge ? _bridge.eventDispatcher : _eventDispatcher);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI48_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI48_0_0RCTLogError(@"No +[ABI48_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI48_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI48_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI48_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI48_0_0RCT_CASE(_C_SEL, SEL)
        ABI48_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI48_0_0RCT_CASE(_C_CHR, char)
        ABI48_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI48_0_0RCT_CASE(_C_SHT, short)
        ABI48_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI48_0_0RCT_CASE(_C_INT, int)
        ABI48_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI48_0_0RCT_CASE(_C_LNG, long)
        ABI48_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI48_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI48_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI48_0_0RCT_CASE(_C_FLT, float)
        ABI48_0_0RCT_CASE(_C_DBL, double)
        ABI48_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI48_0_0RCT_CASE(_C_PTR, void *)
        ABI48_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI48_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI48_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI48_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI48_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI48_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI48_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI48_0_0ReactTag];
      ABI48_0_0RCTPerformBlockWithLogPrefix(
          ^{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI48_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI48_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI48_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
    ...
    if ([type isEqualToString:@"ABI48_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI48_0_0RCTNormalizeInputEventName(name)];

      // TODO(109509380): Remove this gating
      if (!ABI48_0_0RCTViewConfigEventValidAttributesDisabled()) {
        propTypes[name] = @"BOOL";
      }
    } else if ([type isEqualToString:@"ABI48_0_0RCTCapturingEventBlock"]) {
      [capturingEvents addObject:ABI48_0_0RCTNormalizeInputEventName(name)];

      // TODO(109509380): Remove this gating
      if (!ABI48_0_0RCTViewConfigEventValidAttributesDisabled()) {
        propTypes[name] = @"BOOL";
      }
    } else if ([type isEqualToString:@"ABI48_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI48_0_0RCTNormalizeInputEventName(name)];
...
{
  // Hackety hack, this partially re-implements ABI48_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI48_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI48_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI48_0_0EX_REMOVE_VERSION(name);
  ...
  ABI48_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
