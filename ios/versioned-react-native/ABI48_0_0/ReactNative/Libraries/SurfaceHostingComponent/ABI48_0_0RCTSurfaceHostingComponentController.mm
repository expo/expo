...
@interface ABI48_0_0RCTSurfaceHostingComponentController () <ABI48_0_0RCTSurfaceDelegate>
@end

@implementation ABI48_0_0RCTSurfaceHostingComponentController {
  id<ABI48_0_0RCTSurfaceProtocol> _surface;
}

- (instancetype)initWithComponent:(ABI48_0_0RCTSurfaceHostingComponent *)component
{
...
- (void)didUpdateComponent
{
  ...
  [self updateSurfaceWithComponent:(ABI48_0_0RCTSurfaceHostingComponent *)self.component];
}
...
- (void)updateSurfaceWithComponent:(ABI48_0_0RCTSurfaceHostingComponent *)component
{
  // Updating `surface`
  id<ABI48_0_0RCTSurfaceProtocol> const surface = component.surface;
  if (surface != _surface) {
...
- (void)setIntrinsicSize:(CGSize)intrinsicSize
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [strongSelf.component
        updateState:^(ABI48_0_0RCTSurfaceHostingComponentState *state) {
          return [ABI48_0_0RCTSurfaceHostingComponentState newWithStage:state.stage intrinsicSize:intrinsicSize];
        }
...
- (void)setStage:(ABI48_0_0RCTSurfaceStage)stage
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [strongSelf.component
        updateState:^(ABI48_0_0RCTSurfaceHostingComponentState *state) {
          return [ABI48_0_0RCTSurfaceHostingComponentState newWithStage:stage intrinsicSize:state.intrinsicSize];
        }
               mode:[strongSelf suitableStateUpdateMode]];
  });
}
...
{
  return ((ABI48_0_0RCTSurfaceHostingComponent *)self.component).options.synchronousStateUpdates && ABI48_0_0RCTIsMainQueue()
...
- (void)mountSurfaceView
{
  ...
  ABI48_0_0RCTAssert([superview.subviews count] <= 1, @"Should never have more than a single stateful subview.");
