...
static NSUInteger ABI48_0_0RCTMaxCachableDecodedImageSizeInBytes = 2 * 1024 * 1024;
static NSUInteger ABI48_0_0RCTImageCacheTotalCostLimit = 20 * 1024 * 1024;

void ABI48_0_0RCTSetImageCacheLimits(NSUInteger maxCachableDecodedImageSizeInBytes, NSUInteger imageCacheTotalCostLimit)
{
  ABI48_0_0RCTMaxCachableDecodedImageSizeInBytes = maxCachableDecodedImageSizeInBytes;
  ABI48_0_0RCTImageCacheTotalCostLimit = imageCacheTotalCostLimit;
}

static NSString *ABI48_0_0RCTCacheKeyForImage(NSString *imageTag, CGSize size, CGFloat scale, ABI48_0_0RCTResizeMode resizeMode)
{
  return
      [NSString stringWithFormat:@"%@|%g|%g|%g|%lld", imageTag, size.width, size.height, scale, (long long)resizeMode];
}

@implementation ABI48_0_0RCTImageCache {
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _decodedImageCache.totalCostLimit = ABI48_0_0RCTImageCacheTotalCostLimit;
...
- (void)addImageToCache:(UIImage *)image forKey:(NSString *)cacheKey
{
  ...
  NSInteger bytes = image.ABI48_0_0ReactDecodedImageBytes;
  if (bytes <= ABI48_0_0RCTMaxCachableDecodedImageSizeInBytes) {
...
{
  NSString *cacheKey = ABI48_0_0RCTCacheKeyForImage(url, size, scale, resizeMode);
  @synchronized(_cacheStaleTimes) {
