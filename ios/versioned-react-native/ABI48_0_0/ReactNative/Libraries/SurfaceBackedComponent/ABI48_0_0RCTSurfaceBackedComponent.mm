...
#import "ABI48_0_0RCTSurfaceBackedComponent.h"
...
@implementation ABI48_0_0RCTSurfaceBackedComponent

+ (id)initialState
{
  return [ABI48_0_0RCTSurfaceBackedComponentState new];
}
                      ...
                      options:(ABI48_0_0RCTSurfaceHostingComponentOptions)options
{
  ...
  ABI48_0_0RCTSurfaceBackedComponentState *state = scope.state();
  ...
  if (state.surface == nil || ![state.surface.moduleName isEqualToString:moduleName]) {
    id<ABI48_0_0RCTSurfaceProtocol> surface;
    if (surfacePresenter) {
      surface = [[ABI48_0_0RCTFabricSurface alloc] initWithSurfacePresenter:surfacePresenter
                                                        moduleName:moduleName
                                                 initialProperties:adjustedProperties];
    } else {
      surface = [[ABI48_0_0RCTSurface alloc] initWithBridge:bridge moduleName:moduleName initialProperties:adjustedProperties];
    }
    [surface start];

    state = [ABI48_0_0RCTSurfaceBackedComponentState newWithSurface:surface];
  ...
  ABI48_0_0RCTSurfaceHostingComponent *surfaceHostingComponent = [ABI48_0_0RCTSurfaceHostingComponent newWithSurface:state.surface
                                                                                           options:options];

  CKComponent *component;
  if (options.activityIndicatorComponentFactory == nil || ABI48_0_0RCTSurfaceStageIsRunning(state.surface.stage)) {
