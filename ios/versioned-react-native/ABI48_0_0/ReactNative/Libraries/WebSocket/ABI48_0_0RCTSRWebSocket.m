...
typedef NS_ENUM(NSInteger, ABI48_0_0RCTSROpCode) {
  ABI48_0_0RCTSROpCodeTextFrame = 0x1,
  ABI48_0_0RCTSROpCodeBinaryFrame = 0x2,
  // 3-7 reserved.
  ABI48_0_0RCTSROpCodeConnectionClose = 0x8,
  ABI48_0_0RCTSROpCodePing = 0x9,
  ABI48_0_0RCTSROpCodePong = 0xA,
  // B-F reserved.
};
...
static NSString *const ABI48_0_0RCTSRWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

//#define ABI48_0_0RCTSR_ENABLE_LOG
#ifdef ABI48_0_0RCTSR_ENABLE_LOG
#define ABI48_0_0RCTSRLog(format...) ABI48_0_0RCTLogInfo(format)
#else
#define ABI48_0_0RCTSRLog(...) \
  do {                \
  } while (0)
#endif
...
@interface NSData (ABI48_0_0RCTSRWebSocket)

@property (nonatomic, readonly, copy) NSString *stringBySHA1ThenBase64Encoding;

@end

@interface NSString (ABI48_0_0RCTSRWebSocket)

@property (nonatomic, readonly, copy) NSString *stringBySHA1ThenBase64Encoding;

@end

@interface NSURL (ABI48_0_0RCTSRWebSocket)

// The origin isn't really applicable for a native application.
// So instead, just map ws -> http and wss -> https.
@property (nonatomic, readonly, copy) NSString *ABI48_0_0RCTSR_origin;

@end

@interface _ABI48_0_0RCTSRRunLoopThread : NSThread
...
@implementation NSData (ABI48_0_0RCTSRWebSocket)
...
@implementation NSString (ABI48_0_0RCTSRWebSocket)
...
NSString *const ABI48_0_0RCTSRWebSocketErrorDomain = @"ABI48_0_0RCTSRWebSocketErrorDomain";
NSString *const ABI48_0_0RCTSRHTTPResponseErrorKey = @"HTTPResponseStatusCode";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(ABI48_0_0RCTSRWebSocket *webSocket, NSData *data);

@interface ABI48_0_0RCTSRIOConsumer : NSObject
...
@interface ABI48_0_0RCTSRIOConsumerPool : NSObject

- (instancetype)initWithBufferCapacity:(NSUInteger)poolSize NS_DESIGNATED_INITIALIZER;

- (ABI48_0_0RCTSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner
...
- (void)returnConsumer:(ABI48_0_0RCTSRIOConsumer *)consumer;

@end

@interface ABI48_0_0RCTSRWebSocket () <NSStreamDelegate>

@property (nonatomic, assign) ABI48_0_0RCTSRReadyState readyState;
...
@implementation ABI48_0_0RCTSRWebSocket {
  ...
  NSMutableArray<ABI48_0_0RCTSRIOConsumer *> *_consumers;
  ...
  __strong ABI48_0_0RCTSRWebSocket *_selfRetain;

  NSArray<NSString *> *_requestedProtocols;
  ABI48_0_0RCTSRIOConsumerPool *_consumerPool;
}

- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(NSArray<NSString *> *)protocols
{
  ABI48_0_0RCTAssertParam(request);
  ...
  if ((self = [super init])) {
    ...
    [self _ABI48_0_0RCTSR_commonInit];
  }
  return self;
}
...
- (void)_ABI48_0_0RCTSR_commonInit
{
  ...
  _readyState = ABI48_0_0RCTSR_CONNECTING;
  _consumerStopped = YES;
  _webSocketVersion = 13;

  _workQueue = dispatch_queue_create("com.facebook.ABI48_0_0React.SRWebSocket", DISPATCH_QUEUE_SERIAL);
  ...
  _consumerPool = [ABI48_0_0RCTSRIOConsumerPool new];
...
- (void)setReadyState:(ABI48_0_0RCTSRReadyState)aReadyState
{
...
{
  assert(_url);
  ABI48_0_0RCTAssert(_readyState == ABI48_0_0RCTSR_CONNECTING, @"Cannot call -(void)open on ABI48_0_0RCTSRWebSocket more than once");
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage
{
  ...
  NSString *concattedString = [_secKey stringByAppendingString:ABI48_0_0RCTSRWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish
{
  ...
  if (responseCode >= 400) {
    ABI48_0_0RCTSRLog(@"Request failed with response code %ld", responseCode);
    [self _failWithError:[NSError errorWithDomain:ABI48_0_0RCTSRWebSocketErrorDomain
                                             code:2132
                                         userInfo:@{
                                           NSLocalizedDescriptionKey :
                                               [NSString stringWithFormat:@"received bad response code from server %ld",
                                                                          (long)responseCode],
                                           ABI48_0_0RCTSRHTTPResponseErrorKey : @(responseCode)
                                         }]];
    return;
  }
  ...
  if (negotiatedProtocol) {
    ...
    if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
      [self _failWithError:
                [NSError errorWithDomain:ABI48_0_0RCTSRWebSocketErrorDomain
                                    code:2133
                                userInfo:@{
  ...
  self.readyState = ABI48_0_0RCTSR_OPEN;
...
- (void)_readHTTPHeader
{
  ...
  [self _readUntilHeaderCompleteWithCallback:^(ABI48_0_0RCTSRWebSocket *socket, NSData *data) {
...
- (void)didConnect
{
  ...
  CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Origin"), (__bridge CFStringRef)_url.ABI48_0_0RCTSR_origin);
...
- (void)_initializeStreams
{
  ...
  if (_secure) {
    ...
    if (_urlRequest.ABI48_0_0RCTSR_SSLPinnedCertificates.count) {
      [SSLOptions setValue:@NO forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
    }

#if DEBUG
    [SSLOptions setValue:@NO forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
    ABI48_0_0RCTLogInfo(@"SocketRocket: In debug mode.  Allowing connection to any root cert");
#endif
...
- (void)_connect
{
  if (!_scheduledRunloops.count) {
    [self scheduleInRunLoop:[NSRunLoop ABI48_0_0RCTSR_networkRunLoop] forMode:NSDefaultRunLoopMode];
  }
...
- (void)close
{
  [self closeWithCode:ABI48_0_0RCTSRStatusCodeNormal reason:nil];
}
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason
{
  ...
  dispatch_async(_workQueue, ^{
    if (self.readyState == ABI48_0_0RCTSR_CLOSING || self.readyState == ABI48_0_0RCTSR_CLOSED) {
      return;
    }

    BOOL wasConnecting = self.readyState == ABI48_0_0RCTSR_CONNECTING;

    self.readyState = ABI48_0_0RCTSR_CLOSING;

    ABI48_0_0RCTSRLog(@"Closing with code %ld reason %@", code, reason);
    ...
    [self _sendFrameWithOpcode:ABI48_0_0RCTSROpCodeConnectionClose data:payload];
  });
}
...
- (void)_closeWithProtocolError:(NSString *)message
{
  ...
  [self _performDelegateBlock:^{
    [self closeWithCode:ABI48_0_0RCTSRStatusCodeProtocolError reason:message];
...
{
  dispatch_async(_workQueue, ^{
    if (self.readyState != ABI48_0_0RCTSR_CLOSED) {
      self->_failed = YES;
      [self _performDelegateBlock:^{
      ...
      self.readyState = ABI48_0_0RCTSR_CLOSED;

      ABI48_0_0RCTSRLog(@"Failing with error %@", error.localizedDescription);
...
- (void)send:(id)data
{
  ABI48_0_0RCTAssert(self.readyState != ABI48_0_0RCTSR_CONNECTING, @"Invalid State: Cannot call send: until connection is open");
  if (nil == data) {
    return;
  }
  ...
  dispatch_async(_workQueue, ^{
    if ([data isKindOfClass:[NSString class]]) {
      [self _sendFrameWithOpcode:ABI48_0_0RCTSROpCodeTextFrame data:[(NSString *)data dataUsingEncoding:NSUTF8StringEncoding]];
    } else if ([data isKindOfClass:[NSData class]]) {
      [self _sendFrameWithOpcode:ABI48_0_0RCTSROpCodeBinaryFrame data:data];
    } else {
      assert(NO);
    }
  });
...
{
  ABI48_0_0RCTAssert(self.readyState == ABI48_0_0RCTSR_OPEN, @"Invalid State: Cannot call send: until connection is open");
  // TODO: maybe not copy this for performance
  data = [data copy] ?: [NSData data]; // It's okay for a ping to be empty
  dispatch_async(_workQueue, ^{
    [self _sendFrameWithOpcode:ABI48_0_0RCTSROpCodePing data:data];
  });
}
...
- (void)handlePing:(NSData *)pingData
{
  ...
  [self _performDelegateBlock:^{
    dispatch_async(self->_workQueue, ^{
      [self _sendFrameWithOpcode:ABI48_0_0RCTSROpCodePong data:pingData];
    });
  }];
}
...
{
  ABI48_0_0RCTSRLog(@"Received pong");
  [self _performDelegateBlock:^{
...
{
  ABI48_0_0RCTSRLog(@"Received message");
  [self _performDelegateBlock:^{
...
- (void)handleCloseWithData:(NSData *)data
{
  ...
  ABI48_0_0RCTSRLog(@"Received close frame");
  ...
  if (dataSize == 1) {
    ...
    _closeCode = ABI48_0_0RCTSRStatusNoStatusReceived;
  }

  [self assertOnWorkQueue];

  if (self.readyState == ABI48_0_0RCTSR_OPEN) {
...
{
  [self assertOnWorkQueue];
  ABI48_0_0RCTSRLog(@"Trying to disconnect");
...
- (void)_handleFrameWithData:(NSData *)frameData opCode:(NSInteger)opcode
{
  ...
  BOOL isControlFrame =
      (opcode == ABI48_0_0RCTSROpCodePing || opcode == ABI48_0_0RCTSROpCodePong || opcode == ABI48_0_0RCTSROpCodeConnectionClose);
  if (!isControlFrame) {
    [self _readFrameNew];
  } else {
    dispatch_async(_workQueue, ^{
  ...
  switch (opcode) {
    case ABI48_0_0RCTSROpCodeTextFrame: {
      NSString *str = [[NSString alloc] initWithData:frameData encoding:NSUTF8StringEncoding];
      if (str == nil && frameData) {
        [self closeWithCode:ABI48_0_0RCTSRStatusCodeInvalidUTF8 reason:@"Text frames must be valid UTF-8"];
        dispatch_async(_workQueue, ^{
          [self _disconnect];
        });
    ...
    case ABI48_0_0RCTSROpCodeBinaryFrame:
      [self _handleMessage:[frameData copy]];
      break;
    case ABI48_0_0RCTSROpCodeConnectionClose:
      [self handleCloseWithData:frameData];
      break;
    case ABI48_0_0RCTSROpCodePing:
      [self handlePing:frameData];
      break;
    case ABI48_0_0RCTSROpCodePong:
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData
{
  ...
  BOOL isControlFrame =
      (frame_header.opcode == ABI48_0_0RCTSROpCodePing || frame_header.opcode == ABI48_0_0RCTSROpCodePong ||
       frame_header.opcode == ABI48_0_0RCTSROpCodeConnectionClose);
  ...
  if (frame_header.payload_length == 0) {
    ...
    [self _addConsumerWithDataLength:(size_t)frame_header.payload_length
                            callback:^(ABI48_0_0RCTSRWebSocket *socket, NSData *newData) {
                              if (isControlFrame) {
...
static const uint8_t ABI48_0_0RCTSRFinMask = 0x80;
static const uint8_t ABI48_0_0RCTSROpCodeMask = 0x0F;
static const uint8_t ABI48_0_0RCTSRRsvMask = 0x70;
static const uint8_t ABI48_0_0RCTSRMaskMask = 0x80;
static const uint8_t ABI48_0_0RCTSRPayloadLenMask = 0x7F;
...
- (void)_readFrameContinue
{
  ...
  [self
      _addConsumerWithDataLength:2
                        callback:^(ABI48_0_0RCTSRWebSocket *socket, NSData *data) {
                          __block frame_header header = {0};

                          const uint8_t *headerBuffer = data.bytes;
                          assert(data.length >= 2);

                          if (headerBuffer[0] & ABI48_0_0RCTSRRsvMask) {
                            [socket _closeWithProtocolError:@"Server used RSV bits"];
                            return;
                          }

                          uint8_t receivedOpcode = (ABI48_0_0RCTSROpCodeMask & headerBuffer[0]);

                          BOOL isControlFrame =
                              (receivedOpcode == ABI48_0_0RCTSROpCodePing || receivedOpcode == ABI48_0_0RCTSROpCodePong ||
                               receivedOpcode == ABI48_0_0RCTSROpCodeConnectionClose);
                          ...
                          header.fin = !!(ABI48_0_0RCTSRFinMask & headerBuffer[0]);

                          header.masked = !!(ABI48_0_0RCTSRMaskMask & headerBuffer[1]);
                          header.payload_length = ABI48_0_0RCTSRPayloadLenMask & headerBuffer[1];
                          ...
                          if (extra_bytes_needed == 0) {
                            ...
                            [socket
                                _addConsumerWithDataLength:extra_bytes_needed
                                                  callback:^(ABI48_0_0RCTSRWebSocket *_socket, NSData *_data) {
...
- (BOOL)_innerPumpScanner
{
  ...
  if (self.readyState >= ABI48_0_0RCTSR_CLOSING) {
    return didWork;
  }
  ...
  ABI48_0_0RCTSRIOConsumer *consumer = _consumers[0];
  ...
  if (consumer.readToCurrentFrame || foundSize) {
    ...
    if (consumer.readToCurrentFrame) {
      ...
      if (_currentFrameOpcode == ABI48_0_0RCTSROpCodeTextFrame) {
        // Validate UTF8 stuff.
        size_t currentDataSize = _currentFrameData.length;
        if (_currentFrameOpcode == ABI48_0_0RCTSROpCodeTextFrame && currentDataSize > 0) {
...
static const size_t ABI48_0_0RCTSRFrameHeaderOverhead = 32;

- (void)_sendFrameWithOpcode:(ABI48_0_0RCTSROpCode)opcode data:(NSData *)data
{
  [self assertOnWorkQueue];
  ...
  NSMutableData *frame = [[NSMutableData alloc] initWithLength:payloadLength + ABI48_0_0RCTSRFrameHeaderOverhead];
  if (!frame) {
    [self closeWithCode:ABI48_0_0RCTSRStatusCodeMessageTooBig reason:@"Message too big"];
    return;
  }
  uint8_t *frame_buffer = (uint8_t *)frame.mutableBytes;

  // set fin
  frame_buffer[0] = ABI48_0_0RCTSRFinMask | opcode;
  ...
  if (useMask) {
    ...
    frame_buffer[1] |= ABI48_0_0RCTSRMaskMask;
  }
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode
{
  if (_secure && !_pinnedCertFound &&
    ...
    NSArray *sslCerts = _urlRequest.ABI48_0_0RCTSR_SSLPinnedCertificates;
    if (sslCerts) {
...
- (void)safeHandleEvent:(NSStreamEvent)eventCode stream:(NSStream *)aStream
{
  switch (eventCode) {
    case NSStreamEventOpenCompleted: {
      ABI48_0_0RCTSRLog(@"NSStreamEventOpenCompleted %@", aStream);
      if (self.readyState >= ABI48_0_0RCTSR_CLOSING) {
        return;
      }
      assert(self->_readBuffer);

      if (self.readyState == ABI48_0_0RCTSR_CONNECTING && aStream == self->_inputStream) {
        [self didConnect];
      }
      [self _pumpWriting];
      [self _pumpScanner];
    ...
    case NSStreamEventErrorOccurred: {
      ABI48_0_0RCTSRLog(@"NSStreamEventErrorOccurred %@ %@", aStream, [aStream.streamError copy]);
    ...
    case NSStreamEventEndEncountered: {
      [self _pumpScanner];
      ABI48_0_0RCTSRLog(@"NSStreamEventEndEncountered %@", aStream);
            ...
            [self _performDelegateBlock:^{
              if ([self.delegate respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
                [self.delegate webSocket:self
                        didCloseWithCode:ABI48_0_0RCTSRStatusCodeGoingAway
    ...
    case NSStreamEventHasBytesAvailable: {
      ABI48_0_0RCTSRLog(@"NSStreamEventHasBytesAvailable %@", aStream);
    ...
    case NSStreamEventHasSpaceAvailable: {
      ABI48_0_0RCTSRLog(@"NSStreamEventHasSpaceAvailable %@", aStream);
      [self _pumpWriting];
      break;
    }

    default:
      ABI48_0_0RCTSRLog(@"(default)  %@", aStream);
      break;
  }
}
...
- (void)_scheduleCleanup
{
  ...
  NSTimer *timer = [NSTimer timerWithTimeInterval:(0.0f)
                                         ...
                                         userInfo:nil
                                          repeats:NO];
  [[NSRunLoop ABI48_0_0RCTSR_networkRunLoop] addTimer:timer forMode:NSDefaultRunLoopMode];
}
...
@implementation ABI48_0_0RCTSRIOConsumer
...
@implementation ABI48_0_0RCTSRIOConsumerPool {
  NSUInteger _poolSize;
  NSMutableArray<ABI48_0_0RCTSRIOConsumer *> *_bufferedConsumers;
}
...
- (ABI48_0_0RCTSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner
                      ...
                      readToCurrentFrame:(BOOL)readToCurrentFrame
                             unmaskBytes:(BOOL)unmaskBytes
{
  ABI48_0_0RCTSRIOConsumer *consumer = nil;
  if (_bufferedConsumers.count) {
    consumer = _bufferedConsumers.lastObject;
    [_bufferedConsumers removeLastObject];
  } else {
    consumer = [ABI48_0_0RCTSRIOConsumer new];
  }
...
- (NSArray *)ABI48_0_0RCTSR_SSLPinnedCertificates
{
  return [NSURLProtocol propertyForKey:@"ABI48_0_0RCTSR_SSLPinnedCertificates" inRequest:self];
}
...
- (void)setABI48_0_0RCTSR_SSLPinnedCertificates:(NSArray *)ABI48_0_0RCTSR_SSLPinnedCertificates
{
  [NSURLProtocol setProperty:ABI48_0_0RCTSR_SSLPinnedCertificates forKey:@"ABI48_0_0RCTSR_SSLPinnedCertificates" inRequest:self];
}

@end

@implementation NSURL (ABI48_0_0RCTSRWebSocket)

- (NSString *)ABI48_0_0RCTSR_origin
{
...
static _ABI48_0_0RCTSRRunLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (ABI48_0_0RCTSRWebSocket)

+ (NSRunLoop *)ABI48_0_0RCTSR_networkRunLoop
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    networkThread = [_ABI48_0_0RCTSRRunLoopThread new];
...
@implementation _ABI48_0_0RCTSRRunLoopThread {
