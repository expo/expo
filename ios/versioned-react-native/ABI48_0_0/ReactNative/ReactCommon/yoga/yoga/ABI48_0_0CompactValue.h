...
static_assert(
    ...
    "ABI48_0_0facebook::yoga::detail::CompactValue only works with IEEE754 floats");
...
// This class stores ABI48_0_0YGValue in 32 bits.
...
public:
  ...
  template <ABI48_0_0YGUnit Unit>
  static CompactValue of(float value) noexcept {
    if (value == 0.0f || (value < LOWER_BOUND && value > -LOWER_BOUND)) {
      constexpr auto zero =
          Unit == ABI48_0_0YGUnitPercent ? ZERO_BITS_PERCENT : ZERO_BITS_POINT;
      return {zero};
    }

    constexpr auto upperBound =
        Unit == ABI48_0_0YGUnitPercent ? UPPER_BOUND_PERCENT : UPPER_BOUND_POINT;
    if (value > upperBound || value < -upperBound) {
      value = copysignf(upperBound, value);
    }

    uint32_t unitBit = Unit == ABI48_0_0YGUnitPercent ? PERCENT_BIT : 0;
  ...
  CompactValue(const ABI48_0_0YGValue& x) noexcept : repr_(uint32_t{0}) {
    switch (x.unit) {
      ...
      case ABI48_0_0YGUnitPoint:
        *this = of<ABI48_0_0YGUnitPoint>(x.value);
        break;
      case ABI48_0_0YGUnitPercent:
        *this = of<ABI48_0_0YGUnitPercent>(x.value);
        break;
    }
  }

  operator ABI48_0_0YGValue() const noexcept {
    switch (repr_) {
      case AUTO_BITS:
        return ABI48_0_0YGValueAuto;
      case ZERO_BITS_POINT:
        return ABI48_0_0YGValue{0.0f, ABI48_0_0YGUnitPoint};
      case ZERO_BITS_PERCENT:
        return ABI48_0_0YGValue{0.0f, ABI48_0_0YGUnitPercent};
    }

    if (std::isnan(asFloat(repr_))) {
      return ABI48_0_0YGValueUndefined;
    }
    ...
    return ABI48_0_0YGValue{
        asFloat(data), repr_ & 0x40000000 ? ABI48_0_0YGUnitPercent : ABI48_0_0YGUnitPoint};
  }
...
template <>
CompactValue CompactValue::of<ABI48_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::of<ABI48_0_0YGUnitAuto>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI48_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI48_0_0YGUnitAuto>(float) noexcept = delete;
