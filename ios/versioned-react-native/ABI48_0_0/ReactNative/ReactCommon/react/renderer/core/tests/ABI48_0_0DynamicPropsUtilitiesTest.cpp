...
TEST(DynamicPropsUtilitiesTest, handleNestedObjects) {
  ...
  ABI48_0_0EXPECT_TRUE(result["style"].isObject());
  ABI48_0_0EXPECT_TRUE(result["style"]["backgroundColor"].isString());
  ABI48_0_0EXPECT_TRUE(result["style"]["color"].isString());
  ABI48_0_0EXPECT_TRUE(result["height"].isInt());

  ABI48_0_0EXPECT_EQ(result["style"]["backgroundColor"].asString(), "blue");
  ABI48_0_0EXPECT_EQ(result["style"]["color"], "black");
  ABI48_0_0EXPECT_EQ(result["height"], 100);
}
...
TEST(DynamicPropsUtilitiesTest, handleNull) {
  ...
  ABI48_0_0EXPECT_TRUE(result["height"].isNull());
}
