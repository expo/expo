...
#include <ABI48_0_0React/ABI48_0_0renderer/animations/LayoutAnimationDriver.h>
    ...
    for (int j = 0; j < stages; j++) {
      ...
      for (int k = 0; k < animation_frames + 2; k++) {
        ...
        if (k != (animation_frames + 1)) {
          ABI48_0_0EXPECT_TRUE(animationDriver->shouldOverridePullTransaction());
        } else if (!commits_conflicting_mutations) {
          ABI48_0_0EXPECT_FALSE(animationDriver->shouldOverridePullTransaction());
        }
        ...
        ABI48_0_0EXPECT_TRUE(transaction.has_value() || k == animation_frames);
