...
#include "ABI48_0_0StubClock.h"
#include "ABI48_0_0StubErrorUtils.h"
#include "ABI48_0_0StubQueue.h"
...
TEST_F(RuntimeSchedulerTest, now) {
  ...
  ABI48_0_0EXPECT_EQ(runtimeScheduler_->now(), RuntimeSchedulerTimePoint(1ms));

  stubClock_->advanceTimeBy(10ms);

  ABI48_0_0EXPECT_EQ(runtimeScheduler_->now(), RuntimeSchedulerTimePoint(11ms));

  stubClock_->advanceTimeBy(6s);

  ABI48_0_0EXPECT_EQ(runtimeScheduler_->now(), RuntimeSchedulerTimePoint(6011ms));
}

TEST_F(RuntimeSchedulerTest, getShouldYield) {
  // Always returns false for now.
  ABI48_0_0EXPECT_FALSE(runtimeScheduler_->getShouldYield());
}
...
TEST_F(RuntimeSchedulerTest, scheduleSingleTask) {
  ...
  auto callback =
      createHostFunctionFromLambda([&didRunTask](bool didUserCallbackTimeout) {
        didRunTask = true;
        ABI48_0_0EXPECT_FALSE(didUserCallbackTimeout);
  ...
  ABI48_0_0EXPECT_FALSE(didRunTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI48_0_0EXPECT_TRUE(didRunTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, scheduleImmediatePriorityTask) {
  ...
  auto callback =
      createHostFunctionFromLambda([&didRunTask](bool didUserCallbackTimeout) {
        didRunTask = true;
        ABI48_0_0EXPECT_TRUE(didUserCallbackTimeout);
        return jsi::Value::undefined();
      });
...
TEST_F(RuntimeSchedulerTest, scheduleTwoTasksWithSamePriority) {
  ...
  ABI48_0_0EXPECT_EQ(firstTaskCallOrder, 0);
  ABI48_0_0EXPECT_EQ(secondTaskCallOrder, 0);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI48_0_0EXPECT_EQ(firstTaskCallOrder, 1);
  ABI48_0_0EXPECT_EQ(secondTaskCallOrder, 2);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
  ABI48_0_0EXPECT_EQ(hostFunctionCallCount_, 2);
}
...
TEST_F(RuntimeSchedulerTest, scheduleTwoTasksWithDifferentPriorities) {
  ...
  ABI48_0_0EXPECT_EQ(lowPriorityTaskCallOrder, 0);
  ABI48_0_0EXPECT_EQ(userBlockingPriorityTaskCallOrder, 0);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI48_0_0EXPECT_EQ(lowPriorityTaskCallOrder, 2);
  ABI48_0_0EXPECT_EQ(userBlockingPriorityTaskCallOrder, 1);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
  ABI48_0_0EXPECT_EQ(hostFunctionCallCount_, 2);
}
...
TEST_F(RuntimeSchedulerTest, continuationTask) {
  ...
  ABI48_0_0EXPECT_TRUE(didRunTask);
  ABI48_0_0EXPECT_TRUE(didContinuationTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, scheduleWorkWithYielding) {
  ...
  ABI48_0_0EXPECT_FALSE(wasCalled);

  ABI48_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());

  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI48_0_0EXPECT_TRUE(wasCalled);
  ABI48_0_0EXPECT_FALSE(runtimeScheduler_->getShouldYield());
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, normalTaskYieldsToPlatformEvent) {
  ...
  auto callback = createHostFunctionFromLambda([&](bool /*unused*/) {
    ...
    ABI48_0_0EXPECT_TRUE(didRunPlatformWork);
    return jsi::Value::undefined();
  });
  ...
  runtimeScheduler_->scheduleWork([&](jsi::Runtime const & /*unused*/) {
    ...
    ABI48_0_0EXPECT_FALSE(didRunJavaScriptTask);
    ABI48_0_0EXPECT_FALSE(runtimeScheduler_->getShouldYield());
  });

  ABI48_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 2);
...
TEST_F(RuntimeSchedulerTest, expiredTaskDoesntYieldToPlatformEvent) {
  ...
  auto callback = createHostFunctionFromLambda([&](bool /*unused*/) {
    didRunJavaScriptTask = true;
    ABI48_0_0EXPECT_FALSE(didRunPlatformWork);
    return jsi::Value::undefined();
  });
  ...
  runtimeScheduler_->scheduleWork([&](jsi::Runtime const & /*unused*/) {
    didRunPlatformWork = true;
    ABI48_0_0EXPECT_TRUE(didRunJavaScriptTask);
  });
...
TEST_F(RuntimeSchedulerTest, scheduleTaskFromTask) {
  ...
  ABI48_0_0EXPECT_FALSE(didRunFirstTask);
  ABI48_0_0EXPECT_FALSE(didRunSecondTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);

  stubQueue_->tick();

  ABI48_0_0EXPECT_TRUE(didRunFirstTask);
  ABI48_0_0EXPECT_TRUE(didRunSecondTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
}
...
TEST_F(RuntimeSchedulerTest, handlingError) {
  ...
  ABI48_0_0EXPECT_TRUE(didRunTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 0);
  ABI48_0_0EXPECT_EQ(stubErrorUtils_->getReportFatalCallCount(), 1);
}
...
TEST_F(RuntimeSchedulerTest, basicSameThreadExecution) {
  ...
  std::thread t1([this, &didRunSynchronousTask]() {
    runtimeScheduler_->executeNowOnTheSameThread(
        [this, &didRunSynchronousTask](jsi::Runtime & /*rt*/) {
          ABI48_0_0EXPECT_TRUE(runtimeScheduler_->getIsSynchronous());
          didRunSynchronousTask = true;
        });
    ABI48_0_0EXPECT_FALSE(runtimeScheduler_->getIsSynchronous());
  });

  auto hasTask = stubQueue_->waitForTask(1ms);

  ABI48_0_0EXPECT_TRUE(hasTask);
  ABI48_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);
  ...
  ABI48_0_0EXPECT_TRUE(didRunSynchronousTask);
}
...
TEST_F(RuntimeSchedulerTest, sameThreadTaskCreatesImmediatePriorityTask) {
  ...
  ABI48_0_0EXPECT_TRUE(hasTask);
  ABI48_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI48_0_0EXPECT_FALSE(didRunSubsequentTask);
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);
  ...
  ABI48_0_0EXPECT_TRUE(didRunSynchronousTask);
  ABI48_0_0EXPECT_TRUE(didRunSubsequentTask);
}
...
TEST_F(RuntimeSchedulerTest, twoThreadsRequestAccessToTheRuntime) {
  ...
  ABI48_0_0EXPECT_TRUE(hasTask);
  ABI48_0_0EXPECT_FALSE(didRunWork);
  ABI48_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI48_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 2);
  ...
  ABI48_0_0EXPECT_TRUE(didRunWork);
  ABI48_0_0EXPECT_FALSE(didRunSynchronousTask);
  ABI48_0_0EXPECT_TRUE(runtimeScheduler_->getShouldYield());
  ABI48_0_0EXPECT_EQ(stubQueue_->size(), 1);
