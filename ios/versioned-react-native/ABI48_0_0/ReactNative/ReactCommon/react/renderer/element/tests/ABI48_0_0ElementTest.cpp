...
TEST(ElementTest, testNormalCases) {
  ...
  ABI48_0_0EXPECT_EQ(shadowNode, shadowNodeA);

  // Tags
  ABI48_0_0EXPECT_EQ(shadowNodeA->getTag(), 1);
  ABI48_0_0EXPECT_EQ(shadowNodeAA->getTag(), 2);
  ABI48_0_0EXPECT_EQ(shadowNodeAB->getTag(), 3);
  ABI48_0_0EXPECT_EQ(shadowNodeABA->getTag(), 4);

  // Children
  ABI48_0_0EXPECT_EQ(shadowNodeA->getChildren().size(), 2);
  ABI48_0_0EXPECT_EQ(shadowNodeAA->getChildren().size(), 0);
  ABI48_0_0EXPECT_EQ(shadowNodeAB->getChildren().size(), 1);
  ABI48_0_0EXPECT_EQ(shadowNodeABA->getChildren().size(), 0);
  ABI48_0_0EXPECT_EQ(
  ...
  // Props
  ABI48_0_0EXPECT_EQ(shadowNodeA->getProps()->nativeId, "node A");
  ABI48_0_0EXPECT_EQ(shadowNodeABA->getProps()->nativeId, "node ABA");
  ABI48_0_0EXPECT_EQ(shadowNodeAA->getProps(), propsAA);

  // Finalize
  ABI48_0_0EXPECT_TRUE(shadowNodeA->getSealed());
  ABI48_0_0EXPECT_TRUE(shadowNodeAA->getSealed());
  ABI48_0_0EXPECT_TRUE(shadowNodeAB->getSealed());
  ABI48_0_0EXPECT_TRUE(shadowNodeABA->getSealed());
}
