...
#include <ABI48_0_0React/ABI48_0_0renderer/graphics/PlatformColorParser.h>
...
inline void fromRawValue(
  ...
  if (value.hasType<int>()) {
    ...
    ABI48_0_0React_native_assert(length == 3 || length == 4);
...
inline void fromRawValue(
  ...
  ABI48_0_0React_native_assert(value.hasType<std::vector<Float>>());
  if (value.hasType<std::vector<Float>>()) {
    auto array = (std::vector<Float>)value;
    ABI48_0_0React_native_assert(array.size() == 2);
    if (array.size() >= 2) {
  ...
  if (value.hasType<butter::map<std::string, Float>>()) {
    ...
    for (const auto &pair : map) {
      if (pair.first == "width") {
        ...
        ABI48_0_0React_native_assert(false);
      }
    }
    return;
  }
...
inline void fromRawValue(
  ...
  if (value.hasType<std::vector<Float>>()) {
    auto array = (std::vector<Float>)value;
    ABI48_0_0React_native_assert(array.size() == 4);
    if (array.size() >= 4) {
