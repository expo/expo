...
#include "ABI48_0_0TextInputShadowNode.h"
...
void TextInputShadowNode::updateStateIfNeeded(
  ...
  auto ABI48_0_0ReactTreeAttributedString = getAttributedString(layoutContext);
  auto const &state = getStateData();

  ABI48_0_0React_native_assert(textLayoutManager_);
  ABI48_0_0React_native_assert(
      (!state.layoutManager || state.layoutManager == textLayoutManager_) &&
      "`StateData` refers to a different `TextLayoutManager`");

  if (state.ABI48_0_0ReactTreeAttributedString == ABI48_0_0ReactTreeAttributedString &&
      state.layoutManager == textLayoutManager_) {
    return;
  }

  auto newState = TextInputState{};
  newState.attributedStringBox = AttributedStringBox{ABI48_0_0ReactTreeAttributedString};
  newState.paragraphAttributes = getConcreteProps().paragraphAttributes;
  newState.ABI48_0_0ReactTreeAttributedString = ABI48_0_0ReactTreeAttributedString;
