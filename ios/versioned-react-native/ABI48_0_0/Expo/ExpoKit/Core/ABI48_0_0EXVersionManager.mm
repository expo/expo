...
#import "ABI48_0_0EXDisabledDevMenu.h"
#import "ABI48_0_0EXDisabledRedBox.h"
#import "ABI48_0_0EXVersionManager.h"
#import "ABI48_0_0EXScopedBridgeModule.h"
#import "ABI48_0_0EXStatusBarManager.h"
...
#import <ABI48_0_0React/ABI48_0_0RCTExceptionsManager.h>
#import <ABI48_0_0React/ABI48_0_0RCTLog.h>
#import <ABI48_0_0React/ABI48_0_0RCTRedBox.h>
#import <ABI48_0_0React/ABI48_0_0RCTPackagerConnection.h>
...
#import <ABI48_0_0EXMediaLibrary/ABI48_0_0EXMediaLibraryImageLoader.h>
#import <ABI48_0_0EXFileSystem/ABI48_0_0EXFileSystem.h>
#import "ABI48_0_0EXScopedModuleRegistry.h"
#import "ABI48_0_0EXScopedModuleRegistryAdapter.h"
#import "ABI48_0_0EXScopedModuleRegistryDelegate.h"
...
#import <ABI48_0_0reacthermes/ABI48_0_0HermesExecutorFactory.h>
#import <ABI48_0_0React/ABI48_0_0JSCExecutorFactory.h>
#import <strings.h>

// Import 3rd party modules that need to be scoped.
#import <ABI48_0_0RNCAsyncStorage/ABI48_0_0RNCAsyncStorage.h>
#import "ABI48_0_0RNCWebViewManager.h"

ABI48_0_0RCT_EXTERN NSDictionary<NSString *, NSDictionary *> *ABI48_0_0EXGetScopedModuleClasses(void);
ABI48_0_0RCT_EXTERN void ABI48_0_0EXRegisterScopedModule(Class, ...);

@interface ABI48_0_0RCTEventDispatcher (ABI48_0_0REAnimated)

- (void)setBridge:(ABI48_0_0RCTBridge*)bridge;

@end

// this is needed because ABI48_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI48_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
@interface ABI48_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI48_0_0EXVersionManager () <ABI48_0_0RCTTurboModuleManagerDelegate>

// is this the first time this ABI has been touched at runtime?
@property (nonatomic, assign) BOOL isFirstLoad;
@property (nonatomic, strong) NSDictionary *params;
@property (nonatomic, strong) ABI48_0_0EXManifestsManifest *manifest;
@property (nonatomic, strong) ABI48_0_0RCTTurboModuleManager *turboModuleManager;

@end

@implementation ABI48_0_0EXVersionManager
 ...
 *    @ABI48_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI48_0_0EXKernel *kernel
...
+ (void)load
{
  ...
  // don't have access to ABI48_0_0EXScopedModuleRegistry and so they can't register themselves.
  ABI48_0_0EXRegisterScopedModule([ABI48_0_0RNCWebViewManager class], ABI48_0_0EX_KERNEL_SERVICE_NONE, nil);
}
...
- (void)bridgeWillStartLoading:(id)bridge
{
  if ([self _isDevModeEnabledForBridge:bridge]) {
    ...
    [[ABI48_0_0RCTPackagerConnection sharedPackagerConnection] reconnect:packagerServerHostPort];
    [ABI48_0_0RCTInspectorDevServerHelper connectWithBundleURL:bundleURL];
  }

  // Manually send a "start loading" notif, since the real one happened uselessly inside the ABI48_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI48_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}

- (void)bridgeFinishedLoading:(id)bridge
{
  // Override the "Reload" button from Redbox to reload the app from manifest
  // Keep in mind that it is possible this will return a ABI48_0_0EXDisabledRedBox
  ABI48_0_0RCTRedBox *redBox = [self _moduleInstanceForBridge:bridge named:@"RedBox"];
...
{
  ABI48_0_0RCTDevSettings *devSettings = (ABI48_0_0RCTDevSettings *)[self _moduleInstanceForBridge:bridge named:@"DevSettings"];
  ...
  if ([self _isBridgeInspectable:bridge] && isDevModeEnabled) {
    ...
    items[@"dev-remote-debug"] =  @{
      ...
      @"isEnabled": @NO,
      @"detail": ABI48_0_0RCTTurboModuleEnabled() ? @"Remote debugging is unavailable while Turbo Modules are enabled. To debug remotely, please set `turboModules` to false in app.json." : [NSNull null]
    };
  }
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI48_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI48_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI48_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return [[ABI48_0_0RCTPackagerConnection sharedPackagerConnection] addNotificationHandler:handler queue:queue forMethod:method];
}
...
{
  return ([ABI48_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}
...
{
  NSInteger port = [[[bridge bundleURL] port] integerValue] ?: ABI48_0_0RCT_METRO_PORT;
...
- (id<ABI48_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI48_0_0RCTEnableTurboModule([self.manifest.experiments[@"turboModules"] boolValue]);
  ABI48_0_0RCTSetFatalHandler(fatalHandler);
  ABI48_0_0RCTSetLogThreshold((ABI48_0_0RCTLogLevel) threshold);
...
- (NSArray *)extraModulesForBridge:(id)bridge
{
  ...
  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI48_0_0EXAppState alloc] init],
                                    [[ABI48_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI48_0_0EXStatusBarManager alloc] init],
                                    ]];
  ...
  if (params[@"testEnvironment"]) {
    ABI48_0_0EXTestEnvironment testEnvironment = (ABI48_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI48_0_0EXTestEnvironmentNone) {
      ABI48_0_0EXTest *testModule = [[ABI48_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  ABI48_0_0EXModuleRegistryProvider *moduleRegistryProvider = [[ABI48_0_0EXModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI48_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI48_0_0EXModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ABI48_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI48_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];
#pragma clang diagnostic pop

  ABI48_0_0EXModuleRegistry *moduleRegistry = [moduleRegistryAdapter moduleRegistryForParams:params
                                                        forExperienceStableLegacyId:self.manifest.stableLegacyId
                                                                           scopeKey:self.manifest.scopeKey
                                                                           manifest:self.manifest
                                                                 withKernelServices:services];

  // Adding ABI48_0_0EXNativeModulesProxy with the custom moduleRegistry.
  ABI48_0_0EXNativeModulesProxy *expoNativeModulesProxy = [[ABI48_0_0EXNativeModulesProxy alloc] initWithCustomModuleRegistry:moduleRegistry];
  ...
  if (!ABI48_0_0RCTTurboModuleEnabled()) {
    [extraModules addObject:[self getModuleInstanceFromClass:[self getModuleClassFromName:"DevSettings"]]];
    id exceptionsManager = [self getModuleInstanceFromClass:ABI48_0_0RCTExceptionsManagerCls()];
    ...
    [extraModules addObject:[self getModuleInstanceFromClass:ABI48_0_0RNCAsyncStorage.class]];
  }
...
- (NSArray *)_newScopedModulesForServices:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  NSDictionary<NSString *, NSDictionary *> *ABI48_0_0EXScopedModuleClasses = ABI48_0_0EXGetScopedModuleClasses();
  if (ABI48_0_0EXScopedModuleClasses) {
    [ABI48_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI48_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
...
- (Class)getModuleClassFromName:(const char *)name
{
  if (std::string(name) == "DevSettings") {
    return ABI48_0_0EXDevSettings.class;
  }
  if (std::string(name) == "DevMenu") {
    if (![_params[@"isStandardDevMenuAllowed"] boolValue] || ![_params[@"isDeveloper"] boolValue]) {
      // non-kernel, or non-development kernel, uses expo menu instead of ABI48_0_0RCTDevMenu
      return ABI48_0_0EXDisabledDevMenu.class;
    }
  }
  if (std::string(name) == "RedBox") {
    if (![_params[@"isDeveloper"] boolValue]) {
      // user-facing (not debugging).
      // additionally disable ABI48_0_0RCTRedBox
      return ABI48_0_0EXDisabledRedBox.class;
    }
  }
...
- (id<ABI48_0_0RCTTurboModule>)getModuleInstanceFromClass:(Class)moduleClass
{
  ...
  if (moduleClass == ABI48_0_0RCTImageLoader.class) {
    return [[moduleClass alloc] initWithRedirectDelegate:nil loadersProvider:^NSArray<id<ABI48_0_0RCTImageURLLoader>> *(ABI48_0_0RCTModuleRegistry *) {
      return @[[ABI48_0_0RCTLocalAssetImageLoader new], [ABI48_0_0EXMediaLibraryImageLoader new]];
    } decodersProvider:^NSArray<id<ABI48_0_0RCTImageDataDecoder>> *(ABI48_0_0RCTModuleRegistry *) {
      return @[[ABI48_0_0RCTGIFImageDecoder new]];
    }];
  } else if (moduleClass == ABI48_0_0RCTNetworking.class) {
    return [[moduleClass alloc] initWithHandlersProvider:^NSArray<id<ABI48_0_0RCTURLRequestHandler>> *(ABI48_0_0RCTModuleRegistry *) {
      return @[
  ...
  // Expo-specific
  if (moduleClass == ABI48_0_0EXDevSettings.class) {
  ...
  } else if (moduleClass == ABI48_0_0RCTExceptionsManagerCls()) {
    ...
    if (exceptionsManagerDelegate) {
      ...
      ABI48_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
    }
  } else if (moduleClass == ABI48_0_0RNCAsyncStorage.class) {
...
- (std::shared_ptr<ABI48_0_0facebook::ABI48_0_0React::TurboModule>)getTurboModule:(const std::string &)name
                                                     initParams:(const ABI48_0_0facebook::ABI48_0_0React::ObjCTurboModule::InitParams &)params
{
  // TODO: ADD
  return nullptr;
}
...
- (void *)versionedJsExecutorFactoryForBridge:(ABI48_0_0RCTBridge *)bridge
{
  ...
  const auto executor = [ABI48_0_0EXWeak_self, bridge](ABI48_0_0facebook::jsi::Runtime &runtime) {
    if (!bridge) {
      return;
    }
    ...
    runtime.global().setProperty(
        ...
        ABI48_0_0reanimated::getReanimatedVersionString(runtime));
  ...
  if ([self.manifest.jsEngine isEqualToString:@"hermes"]) {
    return new ABI48_0_0facebook::ABI48_0_0React::HermesExecutorFactory(ABI48_0_0RCTJSIExecutorRuntimeInstaller(executor));
  }
  return new ABI48_0_0facebook::ABI48_0_0React::JSCExecutorFactory(ABI48_0_0RCTJSIExecutorRuntimeInstaller(executor));
}
