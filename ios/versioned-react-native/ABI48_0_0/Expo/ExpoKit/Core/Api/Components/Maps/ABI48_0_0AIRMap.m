...
const CLLocationDegrees ABI48_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI48_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI48_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI48_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI48_0_0AIRMap
{
    UIView *_legalLabel;
    BOOL _initialRegionSet;
    BOOL _initialCameraSet;

    // Array to manually track ABI48_0_0RN subviews
    //
    // ABI48_0_0AIRMap implicitly creates subviews that aren't regular ABI48_0_0RN children
    // (ABI48_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI48_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI48_0_0React-native/blob/v0.16.0/ABI48_0_0React/Modules/ABI48_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI48_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI48_0_0React-native/blob/v0.16.0/Libraries/Text/ABI48_0_0RCTTextField.m#L20
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI48_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI48_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI48_0_0ReactSubview:(id<ABI48_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI48_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI48_0_0AIRMapPolyline class]]) {
        ((ABI48_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI48_0_0AIRMapPolygon class]]) {
        ((ABI48_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI48_0_0AIRMapCircle class]]) {
        ((ABI48_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI48_0_0AIRMapUrlTile class]]) {
        ((ABI48_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI48_0_0AIRMapWMSTile class]]) {
        ((ABI48_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI48_0_0AIRMapLocalTile class]]) {
        ((ABI48_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI48_0_0AIRMapOverlay class]]) {
        ((ABI48_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ...
        return [marker isKindOfClass:[ABI48_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI48_0_0AIRMapMarker*) markerForCallout:(ABI48_0_0AIRMapCallout*)callout {
    ABI48_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI48_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI48_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI48_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI48_0_0AIRMapCalloutSubview* calloutSubview = nil;
        ABI48_0_0AIRMapCallout* callout = nil;
        ABI48_0_0AIRMapMarker* marker = nil;
        
        UIView* tmp = touchedView;
        while (tmp && tmp != win && tmp != self.calloutView) {
            if ([tmp respondsToSelector:@selector(onPress)]) {
                calloutSubview = (ABI48_0_0AIRMapCalloutSubview*) tmp;
            }
            if ([tmp isKindOfClass:[ABI48_0_0AIRMapCallout class]]) {
                callout = (ABI48_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI48_0_0SMCalloutViewDelegate

- (NSTimeInterval)calloutView:(ABI48_0_0SMCalloutView *)calloutView delayForRepositionWithSize:(CGSize)offset {
...
- (void)cacheViewIfNeeded {
    // https://github.com/ABI48_0_0React-native-maps/ABI48_0_0React-native-maps/issues/3100
