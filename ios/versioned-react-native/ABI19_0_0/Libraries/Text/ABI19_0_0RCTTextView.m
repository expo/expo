...
#import "ABI19_0_0RCTUITextView.h"

@implementation ABI19_0_0RCTTextView
{
  ABI19_0_0RCTBridge *_bridge;
  ABI19_0_0RCTEventDispatcher *_eventDispatcher;

  ABI19_0_0RCTUITextView *_textView;
  ABI19_0_0RCTText *_richTextView;
...
- (instancetype)initWithBridge:(ABI19_0_0RCTBridge *)bridge
{
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _textView = [[ABI19_0_0RCTUITextView alloc] initWithFrame:self.bounds];
...
#pragma mark - ABI19_0_0RCTComponent

- (void)insertReactABI19_0_0Subview:(UIView *)subview atIndex:(NSInteger)index
{
  [super insertReactABI19_0_0Subview:subview atIndex:index];

  if ([subview isKindOfClass:[ABI19_0_0RCTText class]]) {
    if (_richTextView) {
      ABI19_0_0RCTLogError(@"Tried to insert a second <Text> into <TextInput> - there can only be one.");
    }
    _richTextView = (ABI19_0_0RCTText *)subview;
...
static NSAttributedString *removeReactABI19_0_0TagFromString(NSAttributedString *string)
{
  if (string.length == 0) {
    return string;
  } else {
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc] initWithAttributedString:string];
    [mutableString removeAttribute:ABI19_0_0RCTReactABI19_0_0TagAttributeName range:NSMakeRange(0, mutableString.length)];
...
- (void)performPendingTextUpdate
{
  ...
  // The underlying <Text> node that produces _pendingAttributedText has a ReactABI19_0_0 tag attribute on it that causes the
  ...
  _pendingAttributedText = removeReactABI19_0_0TagFromString(_pendingAttributedText);
...
- (void)setReactABI19_0_0PaddingInsets:(UIEdgeInsets)ReactABI19_0_0PaddingInsets
{
  ...
  _textView.textContainerInset = ReactABI19_0_0PaddingInsets;
  [self setNeedsLayout];
}
...
- (void)setReactABI19_0_0BorderInsets:(UIEdgeInsets)ReactABI19_0_0BorderInsets
{
  ...
  _textView.frame = UIEdgeInsetsInsetRect(self.bounds, ReactABI19_0_0BorderInsets);
  [self setNeedsLayout];
}
...
- (BOOL)textView:(ABI19_0_0RCTUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
  if (!textView.textWasPasted) {
    ...
    if (_blurOnSubmit && [text isEqualToString:@"\n"]) {
      // TODO: the purpose of blurOnSubmit on ABI19_0_0RCTextField is to decide if the
      // field should lose focus when return is pressed or not. We're cheating a
      // bit here by using it on ABI19_0_0RCTextView to decide if return character should
...
- (void)textViewDidChangeSelection:(ABI19_0_0RCTUITextView *)textView
{
...
- (void)textViewDidChange:(UITextView *)textView
{
  ...
  if (!self.ReactABI19_0_0Tag || !_onChange) {
    return;
  }

  _onChange(@{
    @"text": self.text,
    @"target": self.ReactABI19_0_0Tag,
    @"eventCount": @(_nativeEventCount),
  });
}
...
{
  [_textView ReactABI19_0_0Focus];
}

- (void)ReactABI19_0_0Blur
{
  [_textView ReactABI19_0_0Blur];
}

- (void)didMoveToWindow
{
  [_textView ReactABI19_0_0FocusIfNeeded];
}
