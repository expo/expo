 ...
 * at `idealSize`.
 */
static BOOL ABI19_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
/**
 * See ABI19_0_0RCTConvert (ImageSource). We want to send down the source as a similar
 * JSON parameter.
 */
static NSDictionary *onLoadParamsForSource(ABI19_0_0RCTImageSource *source)
{
  NSDictionary *dict = @{
...
@interface ABI19_0_0RCTImageView ()

@property (nonatomic, copy) ABI19_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI19_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI19_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI19_0_0RCTDirectEventBlock onPartialLoad;
@property (nonatomic, copy) ABI19_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI19_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI19_0_0RCTImageView
{
  __weak ABI19_0_0RCTBridge *_bridge;

  // The image source that's currently displayed
  ABI19_0_0RCTImageSource *_imageSource;

  // The image source that's being loaded from the network
  ABI19_0_0RCTImageSource *_pendingImageSource;
   ...
   * if any.
   */
  ABI19_0_0RCTImageLoaderCancellationBlock _reloadImageCancellationBlock;
}
...
- (void)updateWithImage:(UIImage *)image
{
  ...
  if (_resizeMode == ABI19_0_0RCTResizeModeRepeat) {
...
- (void)setImageSources:(NSArray<ABI19_0_0RCTImageSource *> *)imageSources
{
...
- (void)setResizeMode:(ABI19_0_0RCTResizeMode)resizeMode
{
...
{
  ABI19_0_0RCTImageLoaderCancellationBlock previousCancellationBlock = _reloadImageCancellationBlock;
...
- (ABI19_0_0RCTImageSource *)imageSourceForSize:(CGSize)size
{
  ...
  const CGFloat scale = ABI19_0_0RCTScreenScale();
  const CGFloat targetImagePixels = size.width * size.height * scale * scale;

  ABI19_0_0RCTImageSource *bestSource = nil;
  CGFloat bestFit = CGFLOAT_MAX;
  for (ABI19_0_0RCTImageSource *source in _imageSources) {
...
- (BOOL)shouldChangeImageSource
{
  ...
  ABI19_0_0RCTImageSource *desiredImageSource = [self imageSourceForSize:self.frame.size];
...
- (void)reloadImage
{
  ...
  ABI19_0_0RCTImageSource *source = [self imageSourceForSize:self.frame.size];
  ...
  if (source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI19_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (_onProgress) {
    ...
    __weak ABI19_0_0RCTImageView *weakSelf = self;
    ABI19_0_0RCTImageLoaderPartialLoadBlock partialLoadHandler = ^(UIImage *image) {
      [weakSelf imageLoaderLoadedImage:image error:nil forImageSource:source partial:YES];
    };

    CGSize imageSize = self.bounds.size;
    CGFloat imageScale = ABI19_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    ABI19_0_0RCTImageLoaderCompletionBlock completionHandler = ^(NSError *error, UIImage *loadedImage) {
...
- (void)imageLoaderLoadedImage:(UIImage *)loadedImage error:(NSError *)error forImageSource:(ABI19_0_0RCTImageSource *)source partial:(BOOL)isPartialLoad
{
  ...
  void (^setImageBlock)(UIImage *) = ^(UIImage *image) {
    ...
    if (image.ReactABI19_0_0KeyframeAnimation) {
      [self.layer addAnimation:image.ReactABI19_0_0KeyframeAnimation forKey:@"contents"];
    } else {
      [self.layer removeAnimationForKey:@"contents"];
      self.image = image;
    }
  ...
  if (_blurRadius > __FLT_EPSILON__) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      UIImage *blurredImage = ABI19_0_0RCTBlurredImageWithRadius(loadedImage, self->_blurRadius);
      ABI19_0_0RCTExecuteOnMainQueue(^{
...
- (void)ReactABI19_0_0SetFrame:(CGRect)frame
{
  ...
  if ([self shouldChangeImageSource]) {
    ...
    CGSize idealSize = ABI19_0_0RCTTargetSize(imageSize, imageScale, frame.size, ABI19_0_0RCTScreenScale(),
                                     (ABI19_0_0RCTResizeMode)self.contentMode, YES);

    // Don't reload if the current image or target image size is close enough
    if (!ABI19_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize) ||
        !ABI19_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
      return;
    }
    ...
    ABI19_0_0RCTLogInfo(@"Reloading image %@ as size %@", _imageSource.request.URL.absoluteString, NSStringFromCGSize(idealSize));
