...
#import "ABI19_0_0RCTTestRunner.h"
...
@implementation ABI19_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI19_0_0RCTBridgeModuleListProvider _moduleProvider;
}
...
{
  ABI19_0_0RCTAssertParam(app);
  ABI19_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI19_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI19_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  @autoreleasepool {
    ...
    ABI19_0_0RCTLogFunction defaultLogFunction = ABI19_0_0RCTGetLogFunction();
    ABI19_0_0RCTSetLogFunction(^(ABI19_0_0RCTLogLevel level, ABI19_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI19_0_0RCTLogLevelError) {
        error = message;
      } else {
    ...
    ABI19_0_0RCTBridge *bridge = [[ABI19_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI19_0_0RCTRootView *rootView = [[ABI19_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI19_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI19_0_0RCTTestModule class]];
    ABI19_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    testModule.controller = _testController;
    testModule.testSelector = test;
    testModule.testSuffix = _testSuffix;
    testModule.view = rootView;

    UIViewController *vc = ABI19_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI19_0_0RCTTestStatusPending && error == nil) {
    ...
    ABI19_0_0RCTSetLogFunction(defaultLogFunction);
    ...
    ABI19_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);
#endif
    
    if (expectErrorBlock) {
      ABI19_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI19_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI19_0_0RCTAssert(testModule.status != ABI19_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI19_0_0RCTAssert(testModule.status == ABI19_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
}
