...
#import "ABI19_0_0RCTJSCExecutor.h"
...
#import <ReactABI19_0_0/ABI19_0_0RCTJSCErrorHandling.h>
...
#if (ABI19_0_0RCT_PROFILE || ABI19_0_0RCT_DEV) && __has_include("ABI19_0_0RCTDevMenu.h")
#import "ABI19_0_0RCTDevMenu.h"
#endif

NSString *const ABI19_0_0RCTJSCThreadName = @"com.facebook.ReactABI19_0_0.JavaScript";
NSString *const ABI19_0_0RCTJavaScriptContextCreatedNotification = @"ABI19_0_0RCTJavaScriptContextCreatedNotification";
...
struct TaggedScript {
  const facebook::ReactABI19_0_0::ScriptTag tag;
  const NSData *script;
};

#if ABI19_0_0RCT_PROFILE
@interface ABI19_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI19_0_0RCTCookieMap @end
#endif

struct ABI19_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
};

@interface ABI19_0_0RCTJavaScriptContext : NSObject <ABI19_0_0RCTInvalidating>
...
@implementation ABI19_0_0RCTJavaScriptContext
{
  ABI19_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    _context = context;
    _context.name = @"ABI19_0_0RCTJSContext";
    _javaScriptThread = javaScriptThread;

    /**
     * Explicitly introduce a retain cycle here - The ABI19_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI19_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI19_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI19_0_0RCTPerformanceLogger *_performanceLogger;
  ABI19_0_0RCTJavaScriptContext *_context;
...
#if ABI19_0_0RCT_DEV
static void ABI19_0_0RCTInstallJSCProfiler(ABI19_0_0RCTBridge *bridge, JSContextRef context)
{
#if __has_include("ABI19_0_0RCTDevMenu.h")
  __weak ABI19_0_0RCTBridge *weakBridge = bridge;
  __weak ABI19_0_0RCTDevSettings *devSettings = bridge.devSettings;
  if (ABI19_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI19_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
      ...
      if (shouldStart != ABI19_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI19_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI19_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI19_0_0RCTProfileSendResult(weakBridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI19_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI19_0_0RCTJSCThreadName;
...
{
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI19_0_0RCTJSCExecutor init]", nil);
...
- (instancetype)initWithJSContextData:(const ABI19_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    _useCustomJSCLibrary = data.useCustomJSCLibrary;
    _valid = YES;
    _javaScriptThread = data.javaScriptThread;
    _context = [[ABI19_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
                                         ...
                                         sourceURL:(NSURL *)sourceURL
{
  ...
  if (taggedScript.tag == facebook::ReactABI19_0_0::ScriptTag::RAMBundle) {
...
- (ABI19_0_0RCTJavaScriptContext *)context
{
  if (!self.isValid) {
    return nil;
  }
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI19_0_0RCTProfileDidStartProfiling, ABI19_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_context) {
      ...
      if (self->_useCustomJSCLibrary) {
        JSC_configureJSCForIOS(true, ABI19_0_0RCTJSONStringify(@{
      ...
      self->_context = [[ABI19_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI19_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];

      installBasicSynchronousHooksOnContext(context);
    }

    ABI19_0_0RCTFBQuickPerformanceLoggerConfigureHooks(context.JSGlobalContextRef);

    __weak ABI19_0_0RCTJSCExecutor *weakSelf = self;
    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI19_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI19_0_0RCT_PROFILE_END_EVENT(ABI19_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI19_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI19_0_0RCT_PROFILE
    __weak ABI19_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI19_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI19_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI19_0_0RCTProfileIsProfiling()) {
        ...
        _ABI19_0_0RCTProfileEndFlowEvent(newCookie);
    ...
    if (JSC_JSSamplingProfilerEnabled(context.JSGlobalContextRef)) {
      ...
      [self->_bridge.devMenu addItem:[ABI19_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
        ABI19_0_0RCTJSCExecutor *strongSelf = weakSelf;
...
#if ABI19_0_0RCT_DEV
    ABI19_0_0RCTInstallJSCProfiler(self->_bridge, context.JSGlobalContextRef);
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI19_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ABI19_0_0RCTLogLevel level = ABI19_0_0RCTLogLevelInfo;
    if (logLevel) {
      level = MAX(level, (ABI19_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI19_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI19_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI19_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI19_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI19_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI19_0_0RCTCookieMap *cookieMap = [ABI19_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI19_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI19_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI19_0_0RCTProfileDidStartProfiling];
...
- (int32_t)bytecodeFileFormatVersion
{
  return _useCustomJSCLibrary
    ? facebook::ReactABI19_0_0::customJSCWrapper()->JSBytecodeFileFormatVersion
...
ABI19_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)contextName)
{
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI19_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI19_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI19_0_0RCTJavaScriptCallback)onComplete
{
  ABI19_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI19_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI19_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, batchedBridgeRef) != kJSTypeUndefined) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, methodJSRef) != kJSTypeUndefined) {
        ...
        if (!errorJSRef && JSC_JSValueGetType(ctx, methodJSRef) == kJSTypeUndefined) {
          error = ABI19_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSC_JSValueGetType(ctx, batchedBridgeRef) == kJSTypeUndefined) {
        error = ABI19_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI19_0_0RCTNSErrorFromJSError([JSC_JSValue(ctx) valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI19_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
- (void)executeApplicationScript:(NSData *)script
                       sourceURL:(NSURL *)sourceURL
                      onComplete:(ABI19_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ABI19_0_0RCTAssertParam(script);
  ABI19_0_0RCTAssertParam(sourceURL);
...
static TaggedScript loadTaggedScript(NSData *script,
                                     NSURL *sourceURL,
                                     ABI19_0_0RCTPerformanceLogger *performanceLogger,
                                     RandomAccessBundleData &randomAccessBundle,
                                     NSError **error)
{
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);
  ...
  switch (tag) {
    case facebook::ReactABI19_0_0::ScriptTag::RAMBundle:
      [performanceLogger markStartForTag:ABI19_0_0RCTPLRAMBundleLoad];

      loadedScript = loadRAMBundle(sourceURL, error, randomAccessBundle);

      [performanceLogger markStopForTag:ABI19_0_0RCTPLRAMBundleLoad];
      [performanceLogger setValue:loadedScript.length forTag:ABI19_0_0RCTPLRAMStartupCodeSize];
      break;
...
static void registerNativeRequire(JSContext *context, ABI19_0_0RCTJSCExecutor *executor)
{
  __weak ABI19_0_0RCTJSCExecutor *weakExecutor = executor;
...
{
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(taggedScript.script.length)
  }));

  [performanceLogger markStartForTag:ABI19_0_0RCTPLScriptExecution];
  ...
  [performanceLogger markStopForTag:ABI19_0_0RCTPLScriptExecution];

  NSError *error = jsError
    ? ABI19_0_0RCTNSErrorFromJSErrorRef(jsError, ctx)
    : nil;
...
- (void)injectJSONText:(NSString *)script
   asGlobalObjectNamed:(NSString *)objectName
              callback:(ABI19_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI19_0_0RCT_DEBUG) {
    ABI19_0_0RCTAssert(ABI19_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    JSGlobalContextRef ctx = strongSelf->_context.context.JSGlobalContextRef;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI19_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI19_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI19_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
      }
    }
    ABI19_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI19_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI19_0_0RCTFatal(ABI19_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    NSError *error = ABI19_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI19_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI19_0_0RCTPLRAMNativeRequires];
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI19_0_0RCT_PROFILE_END_EVENT(ABI19_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI19_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI19_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI19_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
@implementation ABI19_0_0RCTJSContextProvider
{
...
- (ABI19_0_0RCTJSContextData)data
{
...
{
  const ABI19_0_0RCTJSContextData data = self.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  return [[ABI19_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
}
