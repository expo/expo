...
#include "ABI19_0_0RCTJSCHelpers.h"
...
JSValueRef nativeLoggingHook(
  ...
  if (argumentCount > 1) {
    level = MAX(level, (ABI19_0_0RCTLogLevel)Value(ctx, arguments[1]).asNumber());
  }
  if (argumentCount > 0) {
    ...
    _ABI19_0_0RCTLogJavaScriptInternal(level, [msg toString]);
  }
...
void ABI19_0_0RCTPrepareJSCExecutor() {
  ReactABI19_0_0Marker::logTaggedMarker = [](const ReactABI19_0_0Marker::ReactABI19_0_0MarkerId, const char *tag) {};
  PerfLogging::installNativeHooks = ABI19_0_0RCTFBQuickPerformanceLoggerConfigureHooks;
