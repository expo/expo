...
const NSInteger ABI19_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI19_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI19_0_0RCTGetEventID(id<ABI19_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI19_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI19_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI19_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI19_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
{
  [_bridge enqueueJSCall:@"ABI19_0_0RCTDeviceEventEmitter"
                  method:@"emit"
                    args:body ? @[name, body] : @[name]
              completion:NULL];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI19_0_0RCT_DEBUG) {
    ABI19_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI19_0_0 tag");
  }

  name = ABI19_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI19_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI19_0_0RCTEvent>)event
{
  ...
  for (id<ABI19_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI19_0_0RCTGetEventID(event);

  id<ABI19_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI19_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)dispatchEvent:(id<ABI19_0_0RCTEvent>)event
{
...
@implementation ABI19_0_0RCTBridge (ABI19_0_0RCTEventDispatcher)

- (ABI19_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI19_0_0RCTEventDispatcher class]];
}
