...
#import "ABI19_0_0RCTBridgeMethod.h"
#import "ABI19_0_0RCTConvert.h"
#import "ABI19_0_0RCTDisplayLink.h"
...
#define ABI19_0_0RCTAssertJSThread() \
  ABI19_0_0RCTAssert(![NSStringFromClass([self->_javaScriptExecutor class]) isEqualToString:@"ABI19_0_0RCTJSCExecutor"] || \
              [[[NSThread currentThread] name] isEqualToString:ABI19_0_0RCTJSCThreadName], \
...
@implementation ABI19_0_0RCTBatchedBridge
{
  std::atomic_bool _wasBatchActive;
  NSMutableArray<dispatch_block_t> *_pendingCalls;
  NSDictionary<NSString *, ABI19_0_0RCTModuleData *> *_moduleDataByName;
...
- (void)start
{
  ...
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI19_0_0RCTBatchedBridge setUp]", nil);

  dispatch_queue_t bridgeQueue = dispatch_queue_create("com.facebook.ReactABI19_0_0.ABI19_0_0RCTBridgeQueue", DISPATCH_QUEUE_CONCURRENT);

  dispatch_group_t initModulesAndLoadSource = dispatch_group_create();

  // Asynchronously load source code
  dispatch_group_enter(initModulesAndLoadSource);
  __weak ABI19_0_0RCTBatchedBridge *weakSelf = self;
  __block NSData *sourceCode;
  [self loadSource:^(NSError *error, NSData *source, __unused int64_t sourceLength) {
    if (error) {
      ABI19_0_0RCTLogWarn(@"Failed to load source: %@", error);
  ...
  ABI19_0_0RCTPerformanceLogger *performanceLogger = self->_performanceLogger;
  ...
  dispatch_async(bridgeQueue, ^{
    ...
    dispatch_group_async(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      [performanceLogger markStartForTag:ABI19_0_0RCTPLJSCExecutorSetup];
      [weakSelf setUpExecutor];
      [performanceLogger markStopForTag:ABI19_0_0RCTPLJSCExecutorSetup];
    });

    // Asynchronously gather the module config
    dispatch_group_async(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      if (weakSelf.valid) {
        ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI19_0_0RCTBatchedBridge moduleConfig", nil);
        [performanceLogger markStartForTag:ABI19_0_0RCTPLNativeModulePrepareConfig];
        config = [weakSelf moduleConfig];
        [performanceLogger markStopForTag:ABI19_0_0RCTPLNativeModulePrepareConfig];
    ...
    dispatch_group_notify(setupJSExecutorAndModuleConfig, bridgeQueue, ^{
      ...
      [performanceLogger markStartForTag:ABI19_0_0RCTPLNativeModuleInjectConfig];
      [weakSelf injectJSONConfiguration:config onComplete:^(NSError *error) {
        [performanceLogger markStopForTag:ABI19_0_0RCTPLNativeModuleInjectConfig];
        if (error) {
          ABI19_0_0RCTLogWarn(@"Failed to inject config: %@", error);
          dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf stopLoadingWithError:error];
          });
        }
  ...
  dispatch_group_notify(initModulesAndLoadSource, bridgeQueue, ^{
    ABI19_0_0RCTBatchedBridge *strongSelf = weakSelf;
...
{
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI19_0_0RCTBatchedBridge initModules]", nil);
  ...
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"extraModules", nil);
  for (id<ABI19_0_0RCTBridgeModule> module in extraModules) {
    ...
    if (moduleClass == self.executorClass) {
      _javaScriptExecutor = (id<ABI19_0_0RCTJavaScriptExecutor>)module;
    }
  }
  ABI19_0_0RCT_PROFILE_END_EVENT(ABI19_0_0RCTProfileTagAlways, @"");
  ...
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"JavaScriptExecutor", nil);
  if (!_javaScriptExecutor) {
    id<ABI19_0_0RCTJavaScriptExecutor> executorModule = [self.executorClass new];
    ABI19_0_0RCTModuleData *moduleData = [[ABI19_0_0RCTModuleData alloc] initWithModuleInstance:executorModule
                                                                       bridge:self];
  ...
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"ModuleData", nil);
...
- (NSString *)moduleConfig
{
  ...
  for (ABI19_0_0RCTModuleData *moduleData in _moduleDataByID) {
    if (self.executorClass == [ABI19_0_0RCTJSCExecutor class]) {
      [config addObject:@[moduleData.name]];
    } else {
      [config addObject:ABI19_0_0RCTNullIfNil(moduleData.config)];
    }
  }

  return ABI19_0_0RCTJSONStringify(@{
...
- (void)executeSourceCode:(NSData *)sourceCode
{
  ...
  [self enqueueApplicationScript:sourceCode url:self.bundleURL onComplete:^(NSError *loadError) {
    ...
    if (loadError) {
      ABI19_0_0RCTLogWarn(@"Failed to execute source code: %@ %@", [loadError localizedDescription], [loadError localizedFailureReason]);
      dispatch_async(dispatch_get_main_queue(), ^{
    ...
    NSRunLoop *targetRunLoop = [self->_javaScriptExecutor isKindOfClass:[ABI19_0_0RCTJSCExecutor class]] ? [NSRunLoop currentRunLoop] : [NSRunLoop mainRunLoop];
    ...
    [self->_performanceLogger setValue:nativeRequiresCount forTag:ABI19_0_0RCTPLRAMStartupNativeRequiresCount];
    uint64_t nativeRequires = [self->_performanceLogger valueForTag:ABI19_0_0RCTPLRAMNativeRequires];
    [self->_performanceLogger setValue:nativeRequires forTag:ABI19_0_0RCTPLRAMStartupNativeRequires];

    [self->_performanceLogger markStopForTag:ABI19_0_0RCTPLBridgeStartup];
...
- (void)_flushPendingCalls
{
  ...
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"Processing pendingCalls", @{ @"count": @(_pendingCalls.count) });
  _loading = NO;
...
- (void)stopLoadingWithError:(NSError *)error
{
  ...
  if ([error userInfo][ABI19_0_0RCTJSStackTraceKey]) {
    [self.redBox showErrorMessage:[error localizedDescription]
                        withStack:[error userInfo][ABI19_0_0RCTJSStackTraceKey]];
  }
...
- (Class)executorClass
{
  return _parentBridge.executorClass ?: [ABI19_0_0RCTJSCExecutor class];
}
                ...
                queue:(dispatch_queue_t)queue
{
  if (queue == ABI19_0_0RCTJSThread) {
    ...
    ABI19_0_0RCTAssert(_javaScriptExecutor != nil, @"Need JS executor to schedule JS work");

    [_javaScriptExecutor executeBlockOnJavaScriptQueue:^{
      ABI19_0_0RCTProfileEndFlowEvent();

      ABI19_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI19_0_0RCTBatchedBridge dispatchBlock", @{ @"loading": @(self.loading) });

      if (self.loading) {
        ABI19_0_0RCTAssert(self->_pendingCalls != nil, @"Can't add pending call, bridge is no longer loading");
...
- (void)invalidate
{
  ...
  ABI19_0_0RCTAssertMainQueue();
  ABI19_0_0RCTAssert(_javaScriptExecutor != nil, @"Can't complete invalidation without a JS executor");
  ...
  for (ABI19_0_0RCTModuleData *moduleData in _moduleDataByID) {
    ...
    id<ABI19_0_0RCTBridgeModule> instance = nil;
    if ([moduleData hasInstance]) {
    ...
    if ([instance respondsToSelector:@selector(invalidate)]) {
      ...
      [self dispatchBlock:^{
        [(id<ABI19_0_0RCTInvalidating>)instance invalidate];
...
{
  if (ABI19_0_0RCT_DEBUG && [_javaScriptExecutor isValid]) {
...
{
 module = ABI19_0_0EX_REMOVE_VERSION(module);
 /**
   * AnyThread
   */
  if (!_valid) {
  ...
  ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, @"-[ABI19_0_0RCTBatchedBridge enqueueJSCall:]", nil);
...
- (JSValue *)callFunctionOnModule:(NSString *)module
                        ...
                        arguments:(NSArray *)arguments
                            error:(NSError **)error
{
  ABI19_0_0RCTJSCExecutor *jsExecutor = (ABI19_0_0RCTJSCExecutor *)_javaScriptExecutor;
  if (![jsExecutor isKindOfClass:[ABI19_0_0RCTJSCExecutor class]]) {
    ABI19_0_0RCTLogWarn(@"FBReactABI19_0_0BridgeJSExecutor is only supported when running in JSC");
    return nil;
  }
  ...
  [jsExecutor callFunctionOnModule:module
    ...
    ABI19_0_0RCTAssert([length isNumber] && [length toUInt32] == 2,
...
{
  ABI19_0_0RCTAssert(onComplete != nil, @"onComplete block passed in should be non-nil");
  ...
  [_javaScriptExecutor executeApplicationScript:script sourceURL:url onComplete:^(NSError *scriptLoadError) {
    ...
    ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, @"FetchApplicationScriptCallbacks", nil);
    [self->_javaScriptExecutor flushedQueue:^(id json, NSError *error)
     {
       ABI19_0_0RCT_PROFILE_END_EVENT(ABI19_0_0RCTProfileTagAlways, @"js_call,init");
...
{
  NSArray *requestsArray = [ABI19_0_0RCTConvert NSArray:buffer];

  if (ABI19_0_0RCT_DEBUG && requestsArray.count <= ABI19_0_0RCTBridgeFieldParams) {
    ABI19_0_0RCTLogError(@"Buffer should contain at least %tu sub-arrays. Only found %tu",
                ABI19_0_0RCTBridgeFieldParams + 1, requestsArray.count);
    return;
  }

  NSArray<NSNumber *> *moduleIDs = [ABI19_0_0RCTConvert NSNumberArray:requestsArray[ABI19_0_0RCTBridgeFieldRequestModuleIDs]];
  NSArray<NSNumber *> *methodIDs = [ABI19_0_0RCTConvert NSNumberArray:requestsArray[ABI19_0_0RCTBridgeFieldMethodIDs]];
  NSArray<NSArray *> *paramsArrays = [ABI19_0_0RCTConvert NSArrayArray:requestsArray[ABI19_0_0RCTBridgeFieldParams]];

  int64_t callID = -1;

  if (requestsArray.count > 3) {
    callID = [requestsArray[ABI19_0_0RCTBridgeFieldCallID] longLongValue];
  }

  if (ABI19_0_0RCT_DEBUG && (moduleIDs.count != methodIDs.count || moduleIDs.count != paramsArrays.count)) {
    ABI19_0_0RCTLogError(@"Invalid data message - all must be length: %zd", moduleIDs.count);
    return;
  }
  ...
  [moduleIDs enumerateObjectsUsingBlock:^(NSNumber *moduleID, NSUInteger i, __unused BOOL *stop) {
    ABI19_0_0RCTModuleData *moduleData = self->_moduleDataByID[moduleID.integerValue];
  ...
  for (dispatch_queue_t queue in buckets) {
    ...
    dispatch_block_t block = ^{
      ...
      ABI19_0_0RCT_PROFILE_BEGIN_EVENT(ABI19_0_0RCTProfileTagAlways, @"-[ABI19_0_0RCTBatchedBridge handleBuffer:]", (@{
        @"calls": @(calls.count),
      }));
      ...
      @autoreleasepool {
        for (NSNumber *indexObj in calls) {
          ...
          if (ABI19_0_0RCT_DEV && callID != -1 && self->_flowIDMap != NULL && ABI19_0_0RCTProfileIsProfiling()) {
            ...
            _ABI19_0_0RCTProfileEndFlowEvent(newFlowID);
      ...
      ABI19_0_0RCT_PROFILE_END_EVENT(ABI19_0_0RCTProfileTagAlways, @"objc_call,dispatch_async");
    };
...
{
  // TODO: batchDidComplete is only used by ABI19_0_0RCTUIManager - can we eliminate this special case?
  for (ABI19_0_0RCTModuleData *moduleData in _moduleDataByID) {
                ...
                params:(NSArray *)params
{
  ...
  ABI19_0_0RCTModuleData *moduleData = _moduleDataByID[moduleID];
  if (ABI19_0_0RCT_DEBUG && !moduleData) {
    ABI19_0_0RCTLogError(@"No module found for id '%zd'", moduleID);
    return nil;
  }

  id<ABI19_0_0RCTBridgeMethod> method = moduleData.methods[methodID];
  if (ABI19_0_0RCT_DEBUG && !method) {
    ABI19_0_0RCTLogError(@"Unknown methodID: %zd for module: %zd (%@)", methodID, moduleID, moduleData.name);
    return nil;
  }
...
- (JSContext *)jsContext
{
  if ([_javaScriptExecutor isKindOfClass:[ABI19_0_0RCTJSCExecutor class]]) {
    return [(ABI19_0_0RCTJSCExecutor *)_javaScriptExecutor jsContext];
  } else {
    return nil;
  }
}
