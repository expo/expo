...
@interface ABI19_0_0RCTScrollEvent : NSObject <ABI19_0_0RCTEvent>
...
@implementation ABI19_0_0RCTScrollEvent
{
...
{
  ABI19_0_0RCTAssertParam(ReactABI19_0_0Tag);
...
- (ABI19_0_0RCTScrollEvent *)coalesceWithEvent:(ABI19_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI19_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI19_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI19_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI19_0_0RCTRefreshControl *rctRefreshControl;
#endif

@end


@implementation ABI19_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI19_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI19_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI19_0_0RCTUIManager JSResponder] isKindOfClass:[ABI19_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI19_0_0 scroll
...
- (void)setBounds:(CGRect)bounds
{
  if (isRectInvalid(bounds)) {
    ABI19_0_0RCTLogError(@"Attempted to set an invalid bounds to inner scrollview: %@", NSStringFromCGRect(bounds));
    return;
  }
...
- (void)setFrame:(CGRect)frame
{
  if (isRectInvalid(frame)) {
    ABI19_0_0RCTLogError(@"Attempted to set an invalid frame to inner scrollview: %@", NSStringFromCGRect(frame));
    return;
  }
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI19_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI19_0_0RCTScrollView
{
  ABI19_0_0RCTEventDispatcher *_eventDispatcher;
  ABI19_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI19_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI19_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;
    _scrollView = [[ABI19_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI19_0_0RCTApplyTranformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI19_0_0LayoutDirection:layoutDirection];

  ABI19_0_0RCTApplyTranformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI19_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI19_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI19_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI19_0_0RCTAssert(_contentView == nil, @"ABI19_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI19_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, self.ReactABI19_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI19_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI19_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI19_0_0Subview:atIndex:`
}
...
- (void)layoutSubviews
{
  ...
  ABI19_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI19_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI19_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI19_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI19_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI19_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI19_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI19_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI19_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI19_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI19_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI19_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI19_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI19_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI19_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI19_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI19_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI19_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI19_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI19_0_0RCTView contentInsetsForView:self];
...
#define ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI19_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI19_0_0RCTScrollEvent *scrollEvent = [[ABI19_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI19_0_0RCTEventDispatcher (ABI19_0_0RCTScrollView)
...
- (void)sendFakeScrollEvent:(NSNumber *)ReactABI19_0_0Tag
{
  ...
  ABI19_0_0RCTScrollEvent *fakeScrollEvent = [[ABI19_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI19_0_0Tag:ReactABI19_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
