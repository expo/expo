...
NSString *const ABI19_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI19_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI19_0_0RCTAccessibilityManager ()
...
@implementation ABI19_0_0RCTAccessibilityManager
  ...
  if ((self = [super init])) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didReceiveNewContentSizeCategory:)
                                               ...
                                               object:ABI19_0_0RCTSharedApplication()];
    ...
    self.contentSizeCategory = ABI19_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI19_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI19_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI19_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI19_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI19_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI19_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI19_0_0Tag:ReactABI19_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI19_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI19_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI19_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI19_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI19_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI19_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI19_0_0RCTBridge (ABI19_0_0RCTAccessibilityManager)

- (ABI19_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI19_0_0RCTAccessibilityManager class]];
}
