...
NSString *const ABI19_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI19_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI19_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI19_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI19_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI19_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI19_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI19_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI19_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI19_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI19_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI19_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

@interface ABI19_0_0EXAV ()

@property (nonatomic, assign) BOOL audioIsEnabled;
@property (nonatomic, assign) ABI19_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI19_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI19_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, strong) NSMutableSet <NSObject<ABI19_0_0EXAVObject> *> *videoSet;
...
- (instancetype)init
{
  if ((self = [super init])) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI19_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI19_0_0EXAudioInterruptionModeMixWithOthers;
...
- (void)_bridgeDidForeground:(NSNotification *)notification
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI19_0_0EXAVObject> *exAVObject) {
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI19_0_0EXAVObject> *exAVObject))block
{
  for (ABI19_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI19_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI19_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  BOOL allowsRecording = ((NSNumber *)mode[@"allowsRecordingIOS"]).boolValue;
  
  if (!playsInSilentMode && interruptionMode == ABI19_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI19_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and duckOthers cannot both be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI19_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and allowsRecording cannot both be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI19_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategory:(AVAudioSession *)audioSession forAudioSessionMode:(ABI19_0_0EXAVAudioSessionMode)audioSessionMode
{
  NSError *error;
  ABI19_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI19_0_0EXAVAudioSessionModeActiveMuted
    ? ABI19_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
  
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI19_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI19_0_0EXAudioInterruptionModeDoNotMix) {
    ...
    switch (activeInterruptionMode) {
      case ABI19_0_0EXAudioInterruptionModeDoNotMix:
        [audioSession setCategory:category error:&error];
        break;
      case ABI19_0_0EXAudioInterruptionModeDuckOthers:
        [audioSession setCategory:category withOptions:AVAudioSessionCategoryOptionDuckOthers error:&error];
        break;
      case ABI19_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI19_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI19_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI19_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI19_0_0EXAVObject> *exAVObject) {
    ABI19_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI19_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI19_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI19_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI19_0_0RCTErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded) {
    return ABI19_0_0RCTErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI19_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI19_0_0EXAVAudioSessionModeActiveMuted) {
    ...
    if (!error) {
      _currentAudioSessionMode = ABI19_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI19_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI19_0_0RCTPromiseRejectBlock)reject
{
  ABI19_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
    reject(@"E_AUDIO_NOPLAYER", nil, ABI19_0_0RCTErrorWithMessage(@"Player does not exist."));
  }
}
     ...
     withRejecter:(ABI19_0_0RCTPromiseRejectBlock)reject
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [_bridge.uiManager viewForReactABI19_0_0Tag:ReactABI19_0_0Tag];
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI19_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI19_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI19_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI19_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI19_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI19_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI19_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI19_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI19_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI19_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI19_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI19_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI19_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}

- (void)_setNewAudioRecorderFilenameAndSettings:(NSDictionary *)optionsFromJS
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI19_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI19_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI19_0_0EXAudioRecordingOptionBitRateStrategyKey]];
...
{
  if (_audioRecorder) {
    return ABI19_0_0RCTErrorWithMessage(@"Recorder already exists.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI19_0_0RCTPromiseRejectBlock)reject
{
  if (_audioRecorder == nil) {
    reject(@"E_AUDIO_NORECORDER", nil, ABI19_0_0RCTErrorWithMessage(@"Recorder does not exist."));
  }
...
ABI19_0_0RCT_EXPORT_MODULE(ExponentAV);

#pragma mark - Audio API: Global settings

ABI19_0_0RCT_EXPORT_METHOD(setAudioIsEnabled:(BOOL)value
...
ABI19_0_0RCT_EXPORT_METHOD(setAudioMode:(nonnull NSDictionary *)mode
                      resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                      rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
...
ABI19_0_0RCT_EXPORT_METHOD(loadForSound:(nonnull NSString *)uriString
                    withStatus:(nonnull NSDictionary *)status
                   withSuccess:(ABI19_0_0RCTResponseSenderBlock)loadSuccess
                     withError:(ABI19_0_0RCTResponseSenderBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);
  __weak __typeof__(self) weakSelf = self;
  ABI19_0_0EXAVPlayerData *data = [[ABI19_0_0EXAVPlayerData alloc] initWithEXAV:self
...
ABI19_0_0RCT_EXPORT_METHOD(unloadForSound:(nonnull NSNumber *)key
                        resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI19_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI19_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI19_0_0RCT_EXPORT_METHOD(setStatusForSound:(nonnull NSNumber *)key
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
...
ABI19_0_0RCT_EXPORT_METHOD(getStatusForSound:(nonnull NSNumber *)key
                           resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI19_0_0EXAVPlayerData *data) {
...
ABI19_0_0RCT_EXPORT_METHOD(setStatusUpdateCallbackForSound:(nonnull NSNumber *)key
                                     withCallback:(ABI19_0_0RCTResponseSenderBlock)callback)
{
  ABI19_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    __block BOOL used = NO; // ABI19_0_0RCTResponseSenderBlock can only be used once
...
ABI19_0_0RCT_EXPORT_METHOD(setErrorCallbackForSound:(nonnull NSNumber *)key
                              withCallback:(ABI19_0_0RCTResponseSenderBlock)callback)
{
  ABI19_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
...
ABI19_0_0RCT_EXPORT_METHOD(loadForVideo:(nonnull NSNumber *)ReactABI19_0_0Tag
...
ABI19_0_0RCT_EXPORT_METHOD(unloadForVideo:(nonnull NSNumber *)ReactABI19_0_0Tag
                        resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI19_0_0EXVideoView *view) {
...
ABI19_0_0RCT_EXPORT_METHOD(setStatusForVideo:(nonnull NSNumber *)ReactABI19_0_0Tag
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
...
ABI19_0_0RCT_EXPORT_METHOD(getStatusForVideo:(nonnull NSNumber *)ReactABI19_0_0Tag
                           resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI19_0_0EXVideoView *view) {
...
ABI19_0_0RCT_EXPORT_METHOD(prepareAudioRecorder:(nonnull NSDictionary *)options
                              resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                              rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (_audioRecorder && !error) {
    ...
    if (error) {
      ...
      reject(@"E_AUDIO_RECORDERNOTCREATED", nil, ABI19_0_0RCTErrorWithMessage(@"Prepare encountered an error: recorder not prepared."));
    }
    _audioRecorderIsPreparing = false;
    [self demoteAudioSessionIfPossible];
  } else {
...
ABI19_0_0RCT_EXPORT_METHOD(startAudioRecording:(ABI19_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (!_allowsAudioRecording) {
      reject(@"E_AUDIO_AUDIOMODE", nil, ABI19_0_0RCTErrorWithMessage(@"Recording not allowed on iOS."));
      ...
      if (!error) {
        if ([_audioRecorder record]) {
          ...
          reject(@"E_AUDIO_RECORDING", nil, ABI19_0_0RCTErrorWithMessage(@"Start encountered an error: recording not started."));
        }
      } else {
...
ABI19_0_0RCT_EXPORT_METHOD(pauseAudioRecording:(ABI19_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI19_0_0RCT_EXPORT_METHOD(stopAudioRecording:(ABI19_0_0RCTPromiseResolveBlock)resolve
                            rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI19_0_0RCT_EXPORT_METHOD(getAudioRecordingStatus:(ABI19_0_0RCTPromiseResolveBlock)resolve
                                 rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI19_0_0RCT_EXPORT_METHOD(unloadAudioRecorder:(ABI19_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
