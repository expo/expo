...
@implementation ABI19_0_0EXBarCodeScannerManager

ABI19_0_0RCT_EXPORT_MODULE(ExponentBarCodeScannerManager);
ABI19_0_0RCT_EXPORT_VIEW_PROPERTY(onBarCodeRead, ABI19_0_0RCTDirectEventBlock);
...
- (UIView *)view
{
  ...
  if (!self.camera) {
    self.camera = [[ABI19_0_0EXBarCodeScanner alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
- (NSDictionary *)constantsToExport
{
  return @{
    ...
    @"Type" :
        @{@"front" : @(ABI19_0_0EXBarCodeScannerTypeFront), @"back" : @(ABI19_0_0EXBarCodeScannerTypeBack)},
    @"TorchMode" : @{
      @"off" : @(ABI19_0_0EXBarCodeScannerTorchModeOff),
      @"on" : @(ABI19_0_0EXBarCodeScannerTorchModeOn),
      @"auto" : @(ABI19_0_0EXBarCodeScannerTorchModeAuto)
    }
  };
}
...
ABI19_0_0RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, ABI19_0_0EXBarCodeScanner)
{
  NSInteger type = [ABI19_0_0RCTConvert NSInteger:json];
    ...
    dispatch_async(self.sessionQueue, ^{
      ...
      if ([self.session canAddInput:captureDeviceInput]) {
        ...
        if (self.videoCaptureDeviceInput) {
          ...
          ABI19_0_0RCTLogWarn(@"%s: Can't add null video capture device, doing nothing instead.", __func__);
        }
      }
...
ABI19_0_0RCT_CUSTOM_VIEW_PROPERTY(torchMode, NSInteger, ABI19_0_0EXBarCodeScanner)
{
  self.torchMode = [ABI19_0_0RCTConvert NSInteger:json];
  [self setTorchMode];
}
...
ABI19_0_0RCT_CUSTOM_VIEW_PROPERTY(barCodeTypes, NSArray, ABI19_0_0EXBarCodeScanner)
{
  NSArray *types = [ABI19_0_0RCTConvert NSArray:json];
  NSSet *validTypes = [NSSet setWithArray:[[self class] validBarCodeTypes].allValues];
  for (id type in types) {
    if (![validTypes containsObject:type]) {
      ABI19_0_0RCTLogWarn(@"Unsupported BarCodeType: %@", type);
      return;
    }
  }
...
- (void)startSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI19_0_0EXBarCodeScannerManager *weakSelf = self;
              ...
              [NSNotificationCenter.defaultCenter
                  addObserverForName:AVCaptureSessionRuntimeErrorNotification
                          ...
                          usingBlock:^(NSNotification *note) {
                            ABI19_0_0EXBarCodeScannerManager *strongSelf = weakSelf;
