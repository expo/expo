...
@interface ABI19_0_0AIRGoogleMap ()

- (id)eventFromCoordinate:(CLLocationCoordinate2D)coordinate;

@end

@implementation ABI19_0_0AIRGoogleMap
{
  NSMutableArray<UIView *> *_ReactABI19_0_0Subviews;
...
{
  if ((self = [super init])) {
    _ReactABI19_0_0Subviews = [NSMutableArray new];
...
- (void)insertReactABI19_0_0Subview:(id<ABI19_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  ...
  if ([subview isKindOfClass:[ABI19_0_0AIRGoogleMapMarker class]]) {
    ABI19_0_0AIRGoogleMapMarker *marker = (ABI19_0_0AIRGoogleMapMarker*)subview;
    marker.realMarker.map = self;
    [self.markers addObject:marker];
  } else if ([subview isKindOfClass:[ABI19_0_0AIRGoogleMapPolygon class]]) {
    ABI19_0_0AIRGoogleMapPolygon *polygon = (ABI19_0_0AIRGoogleMapPolygon*)subview;
    polygon.polygon.map = self;
    [self.polygons addObject:polygon];
  } else if ([subview isKindOfClass:[ABI19_0_0AIRGoogleMapPolyline class]]) {
    ABI19_0_0AIRGoogleMapPolyline *polyline = (ABI19_0_0AIRGoogleMapPolyline*)subview;
    polyline.polyline.map = self;
    [self.polylines addObject:polyline];
  } else if ([subview isKindOfClass:[ABI19_0_0AIRGoogleMapCircle class]]) {
    ABI19_0_0AIRGoogleMapCircle *circle = (ABI19_0_0AIRGoogleMapCircle*)subview;
    circle.circle.map = self;
    [self.circles addObject:circle];
  } else if ([subview isKindOfClass:[ABI19_0_0AIRGoogleMapUrlTile class]]) {
    ABI19_0_0AIRGoogleMapUrlTile *tile = (ABI19_0_0AIRGoogleMapUrlTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else {
    NSArray<id<ABI19_0_0RCTComponent>> *childSubviews = [subview ReactABI19_0_0Subviews];
    for (int i = 0; i < childSubviews.count; i++) {
      [self insertReactABI19_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
    }
  }
  [_ReactABI19_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wobjc-missing-super-calls"
- (void)removeReactABI19_0_0Subview:(id<ABI19_0_0RCTComponent>)subview {
  ...
  if ([subview isKindOfClass:[ABI19_0_0AIRGoogleMapMarker class]]) {
    ...
    NSArray<id<ABI19_0_0RCTComponent>> *childSubviews = [subview ReactABI19_0_0Subviews];
    for (int i = 0; i < childSubviews.count; i++) {
      [self removeReactABI19_0_0Subview:(UIView *)childSubviews[i]];
    }
  }
  [_ReactABI19_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wobjc-missing-super-calls"
- (NSArray<id<ABI19_0_0RCTComponent>> *)ReactABI19_0_0Subviews {
...
- (void)setInitialRegion:(MKCoordinateRegion)initialRegion {
  ...
  self.camera = [ABI19_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:initialRegion];
}

- (void)setRegion:(MKCoordinateRegion)region {
  // TODO: The JS component is repeatedly setting region unnecessarily. We might want to deal with that in here.
  self.camera = [ABI19_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self  andMKCoordinateRegion:region];
}

- (BOOL)didTapMarker:(GMSMarker *)marker {
  ABI19_0_0AIRGMSMarker *airMarker = (ABI19_0_0AIRGMSMarker *)marker;
...
- (void)didTapPolygon:(GMSOverlay *)polygon {
    ABI19_0_0AIRGMSPolygon *airPolygon = (ABI19_0_0AIRGMSPolygon *)polygon;
...
- (void)didChangeCameraPosition:(GMSCameraPosition *)position {
  id event = @{@"continuous": @YES,
               @"region": regionAsJSON([ABI19_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andGMSCameraPosition:position]),
               };
