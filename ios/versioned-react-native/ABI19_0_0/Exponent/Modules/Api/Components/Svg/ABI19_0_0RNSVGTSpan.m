...
@implementation ABI19_0_0RNSVGTSpan
{
    ABI19_0_0RNSVGBezierTransformer *_bezierTransformer;
    CGPathRef _cache;
}
...
{
    __block ABI19_0_0RNSVGBezierTransformer *bezierTransformer;
    [self traverseTextSuperviews:^(__kindof ABI19_0_0RNSVGText *node) {
        if ([node class] == [ABI19_0_0RNSVGTextPath class]) {
            bezierTransformer = [(ABI19_0_0RNSVGTextPath*)node getBezierTransformer];
            return NO;
        }
        return YES;
    }];
...
- (void)traverseTextSuperviews:(BOOL (^)(__kindof ABI19_0_0RNSVGText *node))block
{
    ABI19_0_0RNSVGText *targetView = self;
    BOOL result = block(self);
    
    while (targetView && [targetView class] != [ABI19_0_0RNSVGText class] && result) {
        if (![targetView isKindOfClass:[ABI19_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        
        targetView = (ABI19_0_0RNSVGText*)[targetView superview];
