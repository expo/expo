...
#import "ABI19_0_0EXLocationRequester.h"
#import "ABI19_0_0EXPermissions.h"
#import "ABI19_0_0EXRemoteNotificationRequester.h"

NSString * const ABI19_0_0EXPermissionExpiresNever = @"never";

@interface ABI19_0_0EXPermissions ()

@property (nonatomic, strong) NSMutableArray *requests;

@end

@implementation ABI19_0_0EXPermissions

ABI19_0_0RCT_EXPORT_MODULE(ExponentPermissions);
...
ABI19_0_0RCT_REMAP_METHOD(getAsync,
                 ...
                 rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  if ([type isEqualToString:@"remoteNotifications"]) {
    resolve([ABI19_0_0EXRemoteNotificationRequester permissions]);
  } else if ([type isEqualToString:@"location"]) {
    resolve([ABI19_0_0EXLocationRequester permissions]);
  } else if ([type isEqualToString:@"camera"]) {
    resolve([ABI19_0_0EXCameraPermissionRequester permissions]);
  } else if ([type isEqualToString:@"contacts"]) {
    resolve([ABI19_0_0EXContactsRequester permissions]);
  } else if ([type isEqualToString:@"audioRecording"]) {
    resolve([ABI19_0_0EXAudioRecordingPermissionRequester permissions]);
  } else {
    reject(@"E_PERMISSION_UNKNOWN", [NSString stringWithFormat:@"Unrecognized permission: %@", type], nil);
  }
}

ABI19_0_0RCT_REMAP_METHOD(askAsync,
                 askForPermissionsWithType:(NSString *)type
                 resolver:(ABI19_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI19_0_0RCTPromiseRejectBlock)reject)
{
  [self getCurrentPermissionsWithType:type resolver:^(NSDictionary *result) {
    if (result && [result[@"status"] isEqualToString:[ABI19_0_0EXPermissions permissionStringForStatus:ABI19_0_0EXPermissionStatusGranted]]) {
      // if we already have permission granted, resolve immediately with that
      resolve(result);
    } else {
      NSObject<ABI19_0_0EXPermissionRequester> *requester;
      if ([type isEqualToString:@"remoteNotifications"]) {
        requester = [[ABI19_0_0EXRemoteNotificationRequester alloc] init];
      } else if ([type isEqualToString:@"location"]) {
        requester = [[ABI19_0_0EXLocationRequester alloc] init];
      } else if ([type isEqualToString:@"camera"]) {
        requester = [[ABI19_0_0EXCameraPermissionRequester alloc] init];
      } else if ([type isEqualToString:@"contacts"]) {
        requester = [[ABI19_0_0EXContactsRequester alloc] init];
      } else if ([type isEqualToString:@"audioRecording"]) {
        requester = [[ABI19_0_0EXAudioRecordingPermissionRequester alloc] init];
      } else {
...
+ (NSDictionary *)alwaysGrantedPermissions {
  return @{
    @"status": [ABI19_0_0EXPermissions permissionStringForStatus:ABI19_0_0EXPermissionStatusGranted],
...
{
  switch (status) {
    case ABI19_0_0EXPermissionStatusGranted:
      return @"granted";
    case ABI19_0_0EXPermissionStatusDenied:
      return @"denied";
    default:
      return @"undetermined";
  }
...
+ (ABI19_0_0EXPermissionStatus)statusForPermissions:(NSDictionary *)permissions
{
  ...
  if ([status isEqualToString:@"granted"]) {
    return ABI19_0_0EXPermissionStatusGranted;
  } else if ([status isEqualToString:@"denied"]) {
    return ABI19_0_0EXPermissionStatusDenied;
  } else {
    return ABI19_0_0EXPermissionStatusUndetermined;
  }
}
