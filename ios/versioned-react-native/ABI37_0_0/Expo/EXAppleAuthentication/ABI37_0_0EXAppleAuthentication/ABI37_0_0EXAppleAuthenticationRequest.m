...
@interface ABI37_0_0EXAppleAuthenticationRequest ()
...
@implementation ABI37_0_0EXAppleAuthenticationRequest
...
- (void)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
{
  ...
  request.requestedScopes = [ABI37_0_0EXAppleAuthenticationMappings importScopes:requestedScopes];
...
+ (ABI37_0_0EXAppleAuthenticationRequest *)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
                                       withOptions:(NSDictionary *)options
                                      withCallback:(void(^)(NSDictionary *, NSError *))callback
{
  ABI37_0_0EXAppleAuthenticationRequest *authRequest = [[ABI37_0_0EXAppleAuthenticationRequest alloc] initWithOptions:options andCallback:callback];
   ...
   didCompleteWithAuthorization:(ASAuthorization *)authorization
{
  ...
  if (credential.fullName) {
    fullName = [ABI37_0_0EXAppleAuthenticationMappings exportFullName:credential.fullName];
  }
  NSString *authorizationCode;
  if (credential.authorizationCode) {
    authorizationCode = [ABI37_0_0EXAppleAuthenticationMappings exportData:credential.authorizationCode];
  }
  NSString *identityToken;
  if (credential.identityToken) {
    identityToken = [ABI37_0_0EXAppleAuthenticationMappings exportData:credential.identityToken];
  }

  NSDictionary *response = @{
                         @"fullName": ABI37_0_0UMNullIfNil(fullName),
                         @"email": ABI37_0_0UMNullIfNil(credential.email),
                         @"user": credential.user,
                         @"realUserStatus": [ABI37_0_0EXAppleAuthenticationMappings exportRealUserStatus:credential.realUserStatus],
                         @"state": ABI37_0_0UMNullIfNil(credential.state),
                         @"authorizationCode": ABI37_0_0UMNullIfNil(authorizationCode),
                         @"identityToken": ABI37_0_0UMNullIfNil(identityToken),
                         };
