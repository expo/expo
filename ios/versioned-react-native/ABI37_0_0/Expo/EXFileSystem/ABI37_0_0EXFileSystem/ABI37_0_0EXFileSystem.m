...
#import <ABI37_0_0EXFileSystem/ABI37_0_0EXDownloadDelegate.h>
#import <ABI37_0_0EXFileSystem/ABI37_0_0EXFileSystem.h>

#import <CommonCrypto/CommonDigest.h>

#import <ABI37_0_0EXFileSystem/ABI37_0_0EXFileSystemLocalFileHandler.h>
#import <ABI37_0_0EXFileSystem/ABI37_0_0EXFileSystemAssetLibraryHandler.h>
...
NSString * const ABI37_0_0EXDownloadProgressEventName = @"Exponent.downloadProgress";

@interface ABI37_0_0EXDownloadResumable : NSObject

@property (nonatomic, strong) NSString *uuid;
@property (nonatomic, strong) NSURLSession *session;
@property (nonatomic, strong) ABI37_0_0EXDownloadDelegate *delegate;

@end

@implementation ABI37_0_0EXDownloadResumable

- (instancetype)initWithId:(NSString *)uuid
               withSession:(NSURLSession *)session
              withDelegate:(ABI37_0_0EXDownloadDelegate *)delegate;
{
  if ((self = [super init])) {
    _uuid = uuid;
    _session = session;
...
@interface ABI37_0_0EXFileSystem ()

@property (nonatomic, strong) NSMutableDictionary<NSString *, ABI37_0_0EXDownloadResumable*> *downloadObjects;
...
@implementation NSData (ABI37_0_0EXFileSystem)
...
@implementation ABI37_0_0EXFileSystem
...
{
  return @[@protocol(ABI37_0_0UMFileSystemInterface)];
}
...
- (instancetype)initWithDocumentDirectory:(NSString *)documentDirectory cachesDirectory:(NSString *)cachesDirectory bundleDirectory:(NSString *)bundleDirectory
{
  if (self = [super init]) {
    ...
    [ABI37_0_0EXFileSystem ensureDirExistsWithPath:_documentDirectory];
    [ABI37_0_0EXFileSystem ensureDirExistsWithPath:_cachesDirectory];
  }
  return self;
}
...
- (void)setModuleRegistry:(ABI37_0_0UMModuleRegistry *)moduleRegistry
{
  ...
  _eventEmitter = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI37_0_0UMEventEmitterService)];
}
...
{
  return @[ABI37_0_0EXDownloadProgressEventName];
}
...
ABI37_0_0UM_EXPORT_METHOD_AS(getInfoAsync,
                    ...
                    rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:uri] & ABI37_0_0UMFileSystemPermissionRead)) {
  ...
  if ([uri.scheme isEqualToString:@"file"]) {
    [ABI37_0_0EXFileSystemLocalFileHandler getInfoForFile:uri withOptions:options resolver:resolve rejecter:reject];
  } else if ([uri.scheme isEqualToString:@"assets-library"] || [uri.scheme isEqualToString:@"ph"]) {
    [ABI37_0_0EXFileSystemAssetLibraryHandler getInfoForFile:uri withOptions:options resolver:resolve rejecter:reject];
  } else {
...
ABI37_0_0UM_EXPORT_METHOD_AS(readAsStringAsync,
...
ABI37_0_0UM_EXPORT_METHOD_AS(writeAsStringAsync,
...
{
  NSURL *uri = [NSURL URLWithString:uriString];
  if (!([self permissionsForURI:uri] & ABI37_0_0UMFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI37_0_0UM_EXPORT_METHOD_AS(deleteAsync,
...
{
  NSURL *uri = [NSURL URLWithString:uriString];
  if (!([self permissionsForURI:[uri URLByAppendingPathComponent:@".."]] & ABI37_0_0UMFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI37_0_0UM_EXPORT_METHOD_AS(moveAsync,
                    ...
                    rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:[from URLByAppendingPathComponent:@".."]] & ABI37_0_0UMFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  if (!([self permissionsForURI:to] & ABI37_0_0UMFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI37_0_0UM_EXPORT_METHOD_AS(copyAsync,
                    ...
                    rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:from] & ABI37_0_0UMFileSystemPermissionRead)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  if ([from.scheme isEqualToString:@"file"]) {
    [ABI37_0_0EXFileSystemLocalFileHandler copyFrom:from to:to resolver:resolve rejecter:reject];
  } else if ([from.scheme isEqualToString:@"assets-library"] || [from.scheme isEqualToString:@"ph"]) {
    [ABI37_0_0EXFileSystemAssetLibraryHandler copyFrom:from to:to resolver:resolve rejecter:reject];
  } else {
    reject(@"E_FILESYSTEM_INVALID_URI",
           [NSString stringWithFormat:@"Unsupported URI scheme for '%@'", from],
           nil);
...
ABI37_0_0UM_EXPORT_METHOD_AS(makeDirectoryAsync,
...
ABI37_0_0UM_EXPORT_METHOD_AS(readDirectoryAsync,
...
ABI37_0_0UM_EXPORT_METHOD_AS(downloadAsync,
                    ...
                    rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:localUri] & ABI37_0_0UMFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI37_0_0UM_EXPORT_METHOD_AS(downloadResumableStartAsync,
                    ...
                    rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
  ...
  if (!([self _permissionsForPath:path] & ABI37_0_0UMFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI37_0_0UM_EXPORT_METHOD_AS(downloadResumablePauseAsync,
                    downloadResumablePauseAsyncWithUUID:(NSString *)uuid
                    resolver:(ABI37_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
  ABI37_0_0EXDownloadResumable *downloadResumable = (ABI37_0_0EXDownloadResumable *)self.downloadObjects[uuid];
    ...
    [downloadResumable.session getTasksWithCompletionHandler:^(NSArray<NSURLSessionDataTask *> * _Nonnull dataTasks, NSArray<NSURLSessionUploadTask *> * _Nonnull uploadTasks, NSArray<NSURLSessionDownloadTask *> * _Nonnull downloadTasks) {
      ...
      if (downloadTask) {
        [downloadTask cancelByProducingResumeData:^(NSData * _Nullable resumeData) {
          resolve(@{ @"resumeData": ABI37_0_0UMNullIfNil([resumeData base64EncodedStringWithOptions:0]) });
        }];
      } else {
        reject(@"E_UNABLE_TO_PAUSE",
...
ABI37_0_0UM_EXPORT_METHOD_AS(getFreeDiskStorageAsync, getFreeDiskStorageAsyncWithResolver:(ABI37_0_0UMPromiseResolveBlock)resolve rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
...
ABI37_0_0UM_EXPORT_METHOD_AS(getTotalDiskCapacityAsync, getTotalDiskCapacityAsyncWithResolver:(ABI37_0_0UMPromiseResolveBlock)resolve rejecter:(ABI37_0_0UMPromiseRejectBlock)reject)
{
...
- (void)_downloadResumableCreateSessionWithUrl:(NSURL *)url withScopedPath:(NSString *)scopedPath withUUID:(NSString *)uuid withOptions:(NSDictionary *)options withResumeData:(NSData * _Nullable)resumeData withResolver:(ABI37_0_0UMPromiseResolveBlock)resolve withRejecter:(ABI37_0_0UMPromiseRejectBlock)reject
{
  NSURLSessionConfiguration *sessionConfiguration = [NSURLSessionConfiguration defaultSessionConfiguration];
  sessionConfiguration.requestCachePolicy = NSURLRequestReloadIgnoringLocalCacheData;
  sessionConfiguration.URLCache = nil;
  ...
  ABI37_0_0EXDownloadDelegateOnWriteCallback onWrite = ^(NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
    __strong ABI37_0_0EXFileSystem *strongSelf = weakSelf;
    if (strongSelf && bytesWritten > 0) {
      [strongSelf sendEventWithName:ABI37_0_0EXDownloadProgressEventName
                               body:@{@"uuid":uuid,
                                      @"data":@{
  ...
  ABI37_0_0EXDownloadDelegateOnDownloadCallback onDownload = ^(NSURLSessionDownloadTask *task, NSURL *location) {
  ...
  ABI37_0_0EXDownloadDelegateOnErrorCallback onError = ^(NSError *error) {
  ...
  ABI37_0_0EXDownloadDelegate *downloadDelegate = [[ABI37_0_0EXDownloadDelegate alloc] initWithId:uuid
                                                                        onWrite:onWrite
  ...
  ABI37_0_0EXDownloadResumable *downloadResumable = [[ABI37_0_0EXDownloadResumable alloc] initWithId:uuid
                                                                       withSession:session
...
- (ABI37_0_0UMFileSystemPermissionFlags)_permissionsForPath:(NSString *)path
{
  return [[_moduleRegistry getModuleImplementingProtocol:@protocol(ABI37_0_0UMFilePermissionModuleInterface)] getPathPermissions:(NSString *)path];
}
...
{
  return [ABI37_0_0EXFileSystem ensureDirExistsWithPath:path];
}
...
{
  return [ABI37_0_0EXFileSystem generatePathInDirectory:directory withExtension:extension];
}
...
+ (NSString *)generatePathInDirectory:(NSString *)directory withExtension:(NSString *)extension
{
  ...
  [ABI37_0_0EXFileSystem ensureDirExistsWithPath:directory];
