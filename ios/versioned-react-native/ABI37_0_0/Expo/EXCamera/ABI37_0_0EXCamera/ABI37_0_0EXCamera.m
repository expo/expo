...
#import <ABI37_0_0UMBarCodeScannerInterface/ABI37_0_0UMBarCodeScannerProviderInterface.h>
...
#import <ABI37_0_0UMFaceDetectorInterface/ABI37_0_0UMFaceDetectorManagerProvider.h>
...
@property (nonatomic, strong) id<ABI37_0_0UMFaceDetectorManager> faceDetectorManager;
@property (nonatomic, strong) id<ABI37_0_0UMBarCodeScannerInterface> barCodeScanner;
...
@property (nonatomic, strong) ABI37_0_0UMPromiseResolveBlock photoCapturedResolve;
@property (nonatomic, strong) ABI37_0_0UMPromiseRejectBlock photoCapturedReject;

@property (nonatomic, strong) ABI37_0_0UMPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI37_0_0UMPromiseRejectBlock videoRecordedReject;

@property (nonatomic, copy) ABI37_0_0UMDirectEventBlock onCameraReady;
@property (nonatomic, copy) ABI37_0_0UMDirectEventBlock onMountError;
@property (nonatomic, copy) ABI37_0_0UMDirectEventBlock onPictureSaved;

@property (nonatomic, copy) ABI37_0_0UMDirectEventBlock onBarCodeScanned;
@property (nonatomic, copy) ABI37_0_0UMDirectEventBlock onFacesDetected;

@end

@implementation ABI37_0_0EXCamera
...
- (void)dealloc
{
  // In very rare case ABI37_0_0EXCamera might be unmounted (and thus deallocated) after starting taking a photo,
...
- (void)updateFlashMode
{
  ...
  if (_flashMode == ABI37_0_0EXCameraFlashModeTorch) {
    if (![device hasTorch]) {
      return;
    }
    ...
    if (![device lockForConfiguration:&error]) {
      if (error) {
        ABI37_0_0UMLogInfo(@"%s: %@", __func__, error);
      }
      return;
    }
...
- (void)updateFocusDepth
{
  ...
  if (device == nil || device.focusMode != ABI37_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if ([device isLockingFocusWithCustomLensPositionSupported]) {
    ...
    ABI37_0_0UM_WEAKIFY(device);
    [device setFocusModeLockedWithLensPosition:_focusDepth completionHandler:^(CMTime syncTime) {
      ABI37_0_0UM_ENSURE_STRONGIFY(device);
      [device unlockForConfiguration];
    }];
    return;
  }

  ABI37_0_0UMLogInfo(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
  return;
}
...
- (void)updateWhiteBalance
{
  ...
  if (_whiteBalance == ABI37_0_0EXCameraWhiteBalanceAuto) {
    [device setWhiteBalanceMode:AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance];
    [device unlockForConfiguration];
  } else {
    AVCaptureWhiteBalanceTemperatureAndTintValues temperatureAndTint = {
      .temperature = [ABI37_0_0EXCameraUtils temperatureForWhiteBalance:_whiteBalance],
      .tint = 0,
    };
...
- (void)setIsScanningBarCodes:(BOOL)barCodeScanning
{
  if (_barCodeScanner) {
    ...
    ABI37_0_0UMLogError(@"BarCodeScanner module not found. Make sure `expo-barcode-scanner` is installed and linked correctly.");
  }
}
...
- (void)setIsDetectingFaces:(BOOL)faceDetecting
{
  if (_faceDetectorManager) {
    ...
    ABI37_0_0UMLogError(@"FaceDetector module not found. Make sure `expo-face-detector` is installed and linked correctly.");
  }
}
...
- (void)takePicture:(NSDictionary *)options resolve:(ABI37_0_0UMPromiseResolveBlock)resolve reject:(ABI37_0_0UMPromiseRejectBlock)reject
{
  ...
  [connection setVideoOrientation:[ABI37_0_0EXCameraUtils videoOrientationForDeviceOrientation:[[UIDevice currentDevice] orientation]]];
  ...
  switch (_flashMode) {
    case ABI37_0_0EXCameraFlashModeOff:
      requestedFlashMode = AVCaptureFlashModeOff;
      break;
    case ABI37_0_0EXCameraFlashModeAuto:
      requestedFlashMode = AVCaptureFlashModeAuto;
      break;
    case ABI37_0_0EXCameraFlashModeOn:
    case ABI37_0_0EXCameraFlashModeTorch:
                ...
                error:(NSError *)error
{
  ...
  ABI37_0_0UMPromiseRejectBlock reject = _photoCapturedReject;
  ABI37_0_0UMPromiseResolveBlock resolve = _photoCapturedResolve;
...
- (void)handleCapturedImageData:(NSData *)imageData metadata:(NSDictionary *)metadata options:(NSDictionary *)options resolver:(ABI37_0_0UMPromiseResolveBlock)resolve reject:(ABI37_0_0UMPromiseRejectBlock)reject
{
  ...
  takenImage = [ABI37_0_0EXCameraUtils cropImage:takenImage toRect:croppedSize];
  ...
  if ([options[@"exif"] boolValue]) {
    NSMutableDictionary *updatedExif = [ABI37_0_0EXCameraUtils updateExifMetadata:metadata[(NSString *)kCGImagePropertyExifDictionary] withAdditionalData:@{ @"Orientation": @([ABI37_0_0EXCameraUtils exportImageOrientation:takenImage.imageOrientation]) }];
    ...
    processedImageData = [ABI37_0_0EXCameraUtils dataFromImage:takenImage withMetadata:updatedMetadata imageQuality:quality];
  } else {
  ...
  response[@"uri"] = [ABI37_0_0EXCameraUtils writeImage:processedImageData toPath:path];
...
- (void)record:(NSDictionary *)options resolve:(ABI37_0_0UMPromiseResolveBlock)resolve reject:(ABI37_0_0UMPromiseRejectBlock)reject
{
  ...
  if (_movieFileOutput != nil && !_movieFileOutput.isRecording && _videoRecordedResolve == nil && _videoRecordedReject == nil) {
    ...
    if (options[@"quality"]) {
      ABI37_0_0EXCameraVideoResolution resolution = [options[@"quality"] integerValue];
      preset = [ABI37_0_0EXCameraUtils captureSessionPresetForVideoResolution:resolution];
    ...
    if (connection.isVideoStabilizationSupported == NO) {
      ABI37_0_0UMLogWarn(@"%s: Video Stabilization is not supported on this device.", __func__);
    } else {
...
#pragma clang diagnostic ignored "-Wunreachable-code"
  ...
  return;
#endif
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI37_0_0EXCameraPermissionRequester class]]) {
...
- (void)initializeCaptureSessionInput
{
  ...
  AVCaptureVideoOrientation orientation = [ABI37_0_0EXCameraUtils videoOrientationForInterfaceOrientation:interfaceOrientation];
  ...
  dispatch_async(_sessionQueue, ^{
    ...
    NSError *error = nil;
    AVCaptureDevice *captureDevice = [ABI37_0_0EXCameraUtils deviceWithMediaType:AVMediaTypeVideo preferringPosition:self.presetCamera];
...
{
  ABI37_0_0UM_WEAKIFY(self);
  AVCaptureVideoOrientation videoOrientation = [ABI37_0_0EXCameraUtils videoOrientationForInterfaceOrientation:orientation];
  [ABI37_0_0UMUtilities performSynchronouslyOnMainThread:^{
    ABI37_0_0UM_ENSURE_STRONGIFY(self);
...
{
  id <ABI37_0_0UMFaceDetectorManagerProvider> faceDetectorProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI37_0_0UMFaceDetectorManagerProvider)];

  if (faceDetectorProvider) {
    id <ABI37_0_0UMFaceDetectorManager> faceDetector = [faceDetectorProvider createFaceDetectorManager];
    if (faceDetector) {
      ABI37_0_0UM_WEAKIFY(self);
...
{
  id<ABI37_0_0UMBarCodeScannerProviderInterface> barCodeScannerProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI37_0_0UMBarCodeScannerProviderInterface)];
  if (barCodeScannerProvider) {
    id<ABI37_0_0UMBarCodeScannerInterface> barCodeScanner = [barCodeScannerProvider createBarCodeScanner];
    if (barCodeScanner) {
      ABI37_0_0UM_WEAKIFY(self);
