...
#import <ABI37_0_0UMReactNativeAdapter/ABI37_0_0UMReactNativeAdapter.h>
...
@interface ABI37_0_0UMReactNativeAdapter ()

@property (nonatomic, weak) ABI37_0_0RCTBridge *bridge;
@property (nonatomic, weak) ABI37_0_0UMNativeModulesProxy *modulesProxy;
...
@implementation ABI37_0_0UMReactNativeAdapter
...
{
  return @[@protocol(ABI37_0_0UMAppLifecycleService), @protocol(ABI37_0_0UMUIManager), @protocol(ABI37_0_0UMJavaScriptContextProvider)];
}
...
{
  __weak ABI37_0_0UMReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI37_0_0UMReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI37_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI37_0_0RCTJSThread];
}
...
# pragma mark - ABI37_0_0UMJavaScriptContextProvider
...
- (JSGlobalContextRef)javaScriptContextRef
{
  if ([_bridge respondsToSelector:@selector(jsContextRef)]) {
    ...
    // In ABI37_0_0React-native 0.59 vm is abstracted by JSI and all JSC specific references are removed
    ...
    // WARNING: This is temporary solution that may break with new ABI37_0_0React-native releases.
...
- (void)handleAppStateDidChange:(NSNotification *)notification
{
  if (
      _isForegrounded && (
       ...
       ABI37_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI37_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
