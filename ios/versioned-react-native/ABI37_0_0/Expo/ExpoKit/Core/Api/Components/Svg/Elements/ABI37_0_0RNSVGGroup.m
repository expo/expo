...
@implementation ABI37_0_0RNSVGGroup
{
    ABI37_0_0RNSVGGlyphContext *_glyphContext;
}
...
- (void)renderGroupTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    [self traverseSubviews:^(UIView *node) {
        if ([node isKindOfClass:[ABI37_0_0RNSVGMask class]] || [node isKindOfClass:[ABI37_0_0RNSVGClipPath class]]) {
            // no-op
        } else if ([node isKindOfClass:[ABI37_0_0RNSVGNode class]]) {
            ABI37_0_0RNSVGNode* svgNode = (ABI37_0_0RNSVGNode*)node;
            ...
            if ([node isKindOfClass:[ABI37_0_0RNSVGRenderable class]]) {
                [(ABI37_0_0RNSVGRenderable*)node mergeProperties:self];
            }
            ...
            if ([node isKindOfClass:[ABI37_0_0RNSVGRenderable class]]) {
                [(ABI37_0_0RNSVGRenderable*)node resetProperties];
            }
        } else if ([node isKindOfClass:[ABI37_0_0RNSVGSvgView class]]) {
            ABI37_0_0RNSVGSvgView* svgView = (ABI37_0_0RNSVGSvgView*)node;
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI37_0_0RNSVGGlyphContext alloc] initWithWidth:width
                                                      height:height];
}
...
- (CGPathRef)getPath:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI37_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI37_0_0RNSVGNode class]] && ![node isKindOfClass:[ABI37_0_0RNSVGMask class]]) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (!event) {
        NSPredicate *const anyActive = [NSPredicate predicateWithFormat:@"self isKindOfClass: %@ AND active == TRUE", [ABI37_0_0RNSVGNode class]];
    ...
    for (UIView *node in [self.subviews reverseObjectEnumerator]) {
        if ([node isKindOfClass:[ABI37_0_0RNSVGNode class]]) {
            if ([node isKindOfClass:[ABI37_0_0RNSVGMask class]]) {
                continue;
            }
            ABI37_0_0RNSVGNode* svgNode = (ABI37_0_0RNSVGNode*)node;
            if (event) {
...
{
    [self traverseSubviews:^(__kindof ABI37_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI37_0_0RNSVGRenderable class]]) {
            [(ABI37_0_0RNSVGRenderable*)node resetProperties];
        }
        return YES;
