...
@interface UIView (ABI37_0_0SMFrameAdditions)
...
@interface ABI37_0_0SMCalloutView ()
...
@property (nonatomic, assign) ABI37_0_0SMCalloutArrowDirection currentArrowDirection;
@property (nonatomic, assign) BOOL popupCancelled;
@end

@implementation ABI37_0_0SMCalloutView

+ (ABI37_0_0SMCalloutView *)platformCalloutView {

    // if you haven't compiled ABI37_0_0SMClassicCalloutView into your app, then we can't possibly create an instance of it!
    if (!NSClassFromString(@"ABI37_0_0SMClassicCalloutView"))
        return [ABI37_0_0SMCalloutView new];

    // ok we have both - so choose the best one based on current platform
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1)
        return [ABI37_0_0SMCalloutView new]; // iOS 7+
    else
        return [NSClassFromString(@"ABI37_0_0SMClassicCalloutView") new];
}

- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.permittedArrowDirection = ABI37_0_0SMCalloutArrowDirectionDown;
        self.presentAnimation = ABI37_0_0SMCalloutAnimationBounce;
        self.dismissAnimation = ABI37_0_0SMCalloutAnimationFade;
...
- (ABI37_0_0SMCalloutBackgroundView *)backgroundView {
...
- (ABI37_0_0SMCalloutBackgroundView *)defaultBackgroundView {
    return [ABI37_0_0SMCalloutMaskedBackgroundView new];
}
...
- (void)presentCalloutFromRect:(CGRect)rect inLayer:(CALayer *)layer ofView:(UIView *)view constrainedToLayer:(CALayer *)constrainedLayer animated:(BOOL)animated {
    ...
    ABI37_0_0SMCalloutArrowDirection bestDirection = ABI37_0_0SMCalloutArrowDirectionDown;

    // we'll point it up though if that's the only option you gave us.
    if (self.permittedArrowDirection == ABI37_0_0SMCalloutArrowDirectionUp)
        bestDirection = ABI37_0_0SMCalloutArrowDirectionUp;
    ...
    if (self.permittedArrowDirection == ABI37_0_0SMCalloutArrowDirectionAny && topSpace < self.calloutHeight && bottomSpace > topSpace)
        bestDirection = ABI37_0_0SMCalloutArrowDirectionUp;
    ...
    CGFloat anchorY = self.calloutOffset.y + (bestDirection == ABI37_0_0SMCalloutArrowDirectionDown ? CGRectGetMinY(rect) : CGRectGetMaxY(rect));
    ...
    CGPoint calloutOrigin = {
            .x = calloutX + adjustX,
            .y = bestDirection == ABI37_0_0SMCalloutArrowDirectionDown ? (anchorY - self.calloutHeight) : anchorY
    };
...
- (CAAnimation *)animationWithType:(ABI37_0_0SMCalloutAnimation)type presenting:(BOOL)presenting {
    CAAnimation *animation = nil;

    if (type == ABI37_0_0SMCalloutAnimationBounce) {
    ...
    else if (type == ABI37_0_0SMCalloutAnimationFade) {
        CABasicAnimation *fade = [CABasicAnimation animationWithKeyPath:@"opacity"];
    ...
    else if (type == ABI37_0_0SMCalloutAnimationStretch) {
...
- (void)layoutSubviews {
    ...
    CGFloat dy = self.currentArrowDirection == ABI37_0_0SMCalloutArrowDirectionUp ? TOP_ANCHOR_MARGIN : 0;
...
// import this known "private API" from ABI37_0_0SMCalloutBackgroundView
@interface ABI37_0_0SMCalloutBackgroundView (EmbeddedImages)
...
@interface ABI37_0_0SMCalloutMaskedBackgroundView ()
...
@implementation ABI37_0_0SMCalloutMaskedBackgroundView
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        if (!blackArrowImage) {
            blackArrowImage = [ABI37_0_0SMCalloutBackgroundView embeddedImageNamed:@"CalloutArrow"];
...
@implementation ABI37_0_0SMCalloutBackgroundView
...
@implementation UIView (ABI37_0_0SMFrameAdditions)
