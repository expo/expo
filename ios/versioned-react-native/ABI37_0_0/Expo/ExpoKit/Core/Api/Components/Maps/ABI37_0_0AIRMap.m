...
const CLLocationDegrees ABI37_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI37_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI37_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI37_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI37_0_0AIRMap
{
    UIView *_legalLabel;
    ...
    // Array to manually track ABI37_0_0RN subviews
    //
    // ABI37_0_0AIRMap implicitly creates subviews that aren't regular ABI37_0_0RN children
    // (ABI37_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI37_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI37_0_0React-native/blob/v0.16.0/ABI37_0_0React/Modules/ABI37_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI37_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI37_0_0React-native/blob/v0.16.0/Libraries/Text/ABI37_0_0RCTTextField.m#L20
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI37_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI37_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI37_0_0ReactSubview:(id<ABI37_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI37_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI37_0_0AIRMapPolyline class]]) {
        ((ABI37_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI37_0_0AIRMapPolygon class]]) {
        ((ABI37_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI37_0_0AIRMapCircle class]]) {
        ((ABI37_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI37_0_0AIRMapUrlTile class]]) {
        ((ABI37_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI37_0_0AIRMapWMSTile class]]) {
        ((ABI37_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI37_0_0AIRMapLocalTile class]]) {
        ((ABI37_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI37_0_0AIRMapOverlay class]]) {
        ((ABI37_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ...
        return [marker isKindOfClass:[ABI37_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI37_0_0AIRMapMarker*) markerForCallout:(ABI37_0_0AIRMapCallout*)callout {
    ABI37_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI37_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI37_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI37_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (ABI37_0_0AIRMapMarker*) markerAtPoint:(CGPoint)point {
    ABI37_0_0AIRMapMarker* mrk = nil;
    for (ABI37_0_0AIRMapMarker* mrkAnn in self.markers) {
        CGRect frame = [self frameForMarker:mrkAnn];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI37_0_0AIRMapCalloutSubview* calloutSubview = nil;
        ABI37_0_0AIRMapCallout* callout = nil;
        ABI37_0_0AIRMapMarker* marker = nil;
        
        UIView* tmp = touchedView;
        while (tmp && tmp != win && tmp != self.calloutView) {
            if ([tmp respondsToSelector:@selector(onPress)]) {
                calloutSubview = (ABI37_0_0AIRMapCalloutSubview*) tmp;
            }
            if ([tmp isKindOfClass:[ABI37_0_0AIRMapCallout class]]) {
                callout = (ABI37_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI37_0_0SMCalloutViewDelegate
