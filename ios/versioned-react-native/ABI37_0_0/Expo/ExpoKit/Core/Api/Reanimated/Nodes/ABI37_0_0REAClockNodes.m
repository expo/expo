...
@interface ABI37_0_0REAClockNode ()

@property (nonatomic) NSNumber *lastTimestampMs;

@end

@implementation ABI37_0_0REAClockNode
...
- (void)start
{
  ...
  __weak ABI37_0_0REAClockNode *weakSelf = self;
...
@implementation ABI37_0_0REAClockOpNode {
...
{
  if ((self = [super initWithID:nodeID config:config])) {
    _clockNodeID = [ABI37_0_0RCTConvert NSNumber:config[@"clock"]];
    ABI37_0_0REA_LOG_ERROR_IF_NIL(_clockNodeID, @"Reanimated: First argument passed to clock node is either of wrong type or is missing.");
  }
  return self;
}

- (ABI37_0_0REANode*)clockNode
{
  return (ABI37_0_0REANode*)[self.nodesManager findNodeByID:_clockNodeID];
}

@end

@implementation ABI37_0_0REAClockStartNode

- (id)evaluate
{
  ABI37_0_0REANode* node = [self clockNode];
...
@implementation ABI37_0_0REAClockStopNode
...
@implementation ABI37_0_0REAClockTestNode

- (id)evaluate
{
  ABI37_0_0REANode* node = [self clockNode];
  if ([node isKindOfClass:[ABI37_0_0REAParamNode class]]) {
    return @(((ABI37_0_0REAParamNode* )node).isRunning ? 1 : 0);
  }
  return @([(ABI37_0_0REAClockNode* )node isRunning] ? 1 : 0);
}
