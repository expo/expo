...
@implementation ABI37_0_0EXScopedFirebaseCore {
  ...
  for (NSString* name in names) {
    if (!protectedAppNames[name] && (!options || ![name isEqualToString:appName])) {
      [[FIRApp appNamed:name] deleteApp:^(BOOL success) {
        if (!success) {
          ABI37_0_0UMLogWarn(@"Failed to delete Firebase app: %@", name);
        }
      }];
    }
  }
...
+ (nullable NSDictionary *)googleServicesFileFromConstantsManifest:(nullable id<ABI37_0_0UMConstantsInterface>)constants
{
  ...
  @try {
    ...
    if (error) ABI37_0_0UMLogWarn(@"Invalid googleServicesFile: %@", error);
    return plist;
  }
  @catch(NSException* exception) {
    ABI37_0_0UMLogWarn(@"Invalid googleServicesFile: %@", exception);
    return nil;
  }
}
