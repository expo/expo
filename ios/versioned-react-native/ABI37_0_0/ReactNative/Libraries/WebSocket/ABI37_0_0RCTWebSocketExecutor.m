...
typedef void (^ABI37_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI37_0_0RCTWebSocketExecutor () <ABI37_0_0RCTSRWebSocketDelegate>

@end

@implementation ABI37_0_0RCTWebSocketExecutor
{
  ABI37_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI37_0_0RCTWSMessageCallback> *_callbacks;
...
{
  if (!_url) {
    NSInteger port = [[[_bridge bundleURL] port] integerValue] ?: ABI37_0_0RCT_METRO_PORT;
    ...
    _url = [ABI37_0_0RCTConvert NSURL:URLString];
  }

  _jsQueue = dispatch_queue_create("com.facebook.ABI37_0_0React.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  ...
  if (![self connectToProxy]) {
    ...
    NSString *error = [NSString stringWithFormat:@"Connection to %@ timed out. Are you "
                       ...
                       "you have the right IP address in `ABI37_0_0RCTWebSocketExecutor.m`.", _url];
    _setupError = ABI37_0_0RCTErrorWithMessage(error);
    ABI37_0_0RCTFatal(_setupError);
    return;
  }
...
- (BOOL)connectToProxy
{
  ...
  return connected == 0 && _socket.readyState == ABI37_0_0RCTSR_OPEN;
}
...
- (BOOL)prepareJSRuntime
{
  ...
  if (initError) {
    ABI37_0_0RCTLogInfo(@"Websocket runtime setup failed: %@", initError);
  }
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI37_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI37_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
{
  dispatch_semaphore_signal(_socketOpenSemaphore);
  ABI37_0_0RCTLogInfo(@"WebSocket connection failed with error %@", error);
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message onReply:(ABI37_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    if (!self.valid) {
      callback(ABI37_0_0RCTErrorWithMessage(@"Runtime is not ready for debugging. Make sure Packager server is running."), nil);
      return;
    }
    ...
    [self->_socket send:ABI37_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}

- (void)executeApplicationScript:(NSData *)script sourceURL:(NSURL *)URL onComplete:(ABI37_0_0RCTJavaScriptCompleteBlock)onComplete
{
  // Hack: the bridge transitions out of loading state as soon as this method returns, which prevents us
  // from completely invalidating the bridge and preventing an endless barage of ABI37_0_0RCTLog.logIfNoNativeHook
  ...
  NSDictionary<NSString *, id> *message = @{
    ...
    @"url": ABI37_0_0RCTNullIfNil(URL.absoluteString),
  ...
  [self sendMessage:message onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    if (socketError) {
      ...
      onComplete(error ? ABI37_0_0RCTErrorWithMessage(error) : nil);
    }
...
- (void)flushedQueue:(ABI37_0_0RCTJavaScriptCallback)onComplete
{
...
- (void)callFunctionOnModule:(NSString *)module
                   ...
                   arguments:(NSArray *)args
                    callback:(ABI37_0_0RCTJavaScriptCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI37_0_0RCTJavaScriptCallback)onComplete
{
  ABI37_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  NSDictionary<NSString *, id> *message = @{
    @"method": method,
    @"arguments": arguments
  };
  [self sendMessage:message onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    ...
    NSError *jsonError;
    id result = ABI37_0_0RCTJSONParse(reply[@"result"], &jsonError);
    NSString *error = reply[@"error"];
    onComplete(result, error ? ABI37_0_0RCTErrorWithMessage(error) : jsonError);
  }];
}

- (void)injectJSONText:(NSString *)script asGlobalObjectNamed:(NSString *)objectName callback:(ABI37_0_0RCTJavaScriptCompleteBlock)onComplete
{
  dispatch_async(_jsQueue, ^{
...
{
  ABI37_0_0RCTExecuteOnMainQueue(block);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI37_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI37_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
