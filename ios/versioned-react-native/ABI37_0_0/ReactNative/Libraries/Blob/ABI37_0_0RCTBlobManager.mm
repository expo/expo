...
@interface ABI37_0_0RCTBlobManager () <ABI37_0_0RCTNetworkingRequestHandler, ABI37_0_0RCTNetworkingResponseHandler, ABI37_0_0RCTWebSocketContentHandler>

@end

@implementation ABI37_0_0RCTBlobManager
{
...
ABI37_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)setBridge:(ABI37_0_0RCTBridge *)bridge
{
  ...
  ABI37_0_0facebook::ABI37_0_0React::ABI37_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI37_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI37_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI37_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI37_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
...
ABI37_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(nonnull NSNumber *)socketID)
{
...
ABI37_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI37_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(nonnull NSNumber *)socketID)
{
  dispatch_async(_bridge.webSocketModule.methodQueue, ^{
...
ABI37_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI37_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI37_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI37_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI37_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI37_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI37_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI37_0_0RCTConvert NSString:blob[@"type"]];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI37_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI37_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI37_0_0RCTWebSocketContentHandler methods
