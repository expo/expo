...
@implementation ABI37_0_0RCTAssetsLibraryRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  return [request.URL.scheme caseInsensitiveCompare:@"assets-library"] == NSOrderedSame
    ...
    || [request.URL.scheme caseInsensitiveCompare:ABI37_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
}
     ...
     withDelegate:(id<ABI37_0_0RCTURLRequestDelegate>)delegate
{
  ...
  BOOL isPHUpload = [requestURL.scheme caseInsensitiveCompare:ABI37_0_0RCTNetworkingPHUploadHackScheme] == NSOrderedSame;
  if (isPHUpload) {
    requestURL = [NSURL URLWithString:[@"ph" stringByAppendingString:[requestURL.absoluteString substringFromIndex:ABI37_0_0RCTNetworkingPHUploadHackScheme.length]]];
  }
  
  if (!requestURL) {
    NSString *const msg = [NSString stringWithFormat:@"Cannot send request without URL"];
    [delegate URLRequest:cancellationBlock didCompleteWithError:ABI37_0_0RCTErrorWithMessage(msg)];
  ...
  if (![fetchResult firstObject]) {
    ...
    NSError *error = ABI37_0_0RCTErrorWithMessage(errorMessage);
