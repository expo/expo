...
#import "ABI37_0_0NSTextStorage+FontScaling.h"
#import <ABI37_0_0React/ABI37_0_0RCTTextView.h>

@implementation ABI37_0_0RCTTextShadowView
{
...
- (instancetype)initWithBridge:(ABI37_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI37_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI37_0_0RCTTextShadowViewMeasure);
    ABI37_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI37_0_0RCTTextShadowViewBaseline);
  }
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  // because the background color is drawn on the ABI37_0_0RCTTextView itself, as well
  // as on the glphy background draw step. By setting this to nil, we allow
  // the ABI37_0_0RCTTextView backgroundColor to be used, without affecting nested Text
  // components.
...
{
  [super dirtyLayout];
  ABI37_0_0YGNodeMarkDirty(self.yogaNode);
...
{
  if (ABI37_0_0YGNodeIsDirty(self.yogaNode)) {
    return;
  }
  ...
  NSNumber *tag = self.ABI37_0_0ReactTag;
  NSMutableArray<NSNumber *> *descendantViewTags = [NSMutableArray new];
  [textStorage enumerateAttribute:ABI37_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:
   ^(ABI37_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
     if (!shadowView) {
       return;
     }

     [descendantViewTags addObject:shadowView.ABI37_0_0ReactTag];
   }
  ];

  [_bridge.uiManager addUIBlock:^(ABI37_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI37_0_0RCTTextView *textView = (ABI37_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI37_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName range:NSMakeRange(0, textStorage.length)];
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI37_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                               inRange:NSMakeRange(0, attributedText.length)
                               options:0
                            usingBlock:
    ^(ABI37_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                                        ...
                                        exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage abi37_0_0_rct_scaleFontSizeToFitSize:size
...
- (void)layoutSubviewsWithContext:(ABI37_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage enumerateAttribute:ABI37_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          ...
                          options:0
                       usingBlock:
    ^(ABI37_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
      if (!shadowView) {
        return;
      }
      ...
      CGRect frame = {{
        ABI37_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI37_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)
      }, {
        ABI37_0_0RCTRoundPixelValue(attachmentSize.width),
        ABI37_0_0RCTRoundPixelValue(attachmentSize.height)
      }};
      
      NSRange truncatedGlyphRange = [layoutManager truncatedGlyphRangeInLineFragmentForGlyphAtIndex:range.location];
      BOOL viewIsTruncated = NSIntersectionRange(range, truncatedGlyphRange).length != 0;

      ABI37_0_0RCTLayoutContext localLayoutContext = layoutContext;
      ...
      ABI37_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
      localLayoutMetrics.frame.origin = frame.origin; // Reinforcing a proper frame origin for the Shadow View.
      if (viewIsTruncated) {
        localLayoutMetrics.displayType = ABI37_0_0RCTDisplayTypeNone;
      }
...
static ABI37_0_0YGSize ABI37_0_0RCTTextShadowViewMeasure(ABI37_0_0YGNodeRef node, float width, ABI37_0_0YGMeasureMode widthMode, float height, ABI37_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
    widthMode == ABI37_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI37_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    heightMode == ABI37_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI37_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI37_0_0RCTTextShadowView *shadowTextView = (__bridge ABI37_0_0RCTTextShadowView *)ABI37_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
    MIN(ABI37_0_0RCTCeilPixelValue(size.width), maximumSize.width),
    MIN(ABI37_0_0RCTCeilPixelValue(size.height), maximumSize.height)
  };
  ...
  return (ABI37_0_0YGSize){
    ABI37_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
    ABI37_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)
  };
}

static float ABI37_0_0RCTTextShadowViewBaseline(ABI37_0_0YGNodeRef node, const float width, const float height)
{
  ABI37_0_0RCTTextShadowView *shadowTextView = (__bridge ABI37_0_0RCTTextShadowView *)ABI37_0_0YGNodeGetContext(node);

  CGSize size = (CGSize){
    ABI37_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    ABI37_0_0RCTCoreGraphicsFloatFromYogaFloat(height)
  };

  CGFloat lastBaseline = [shadowTextView lastBaselineForSize:size];

  return ABI37_0_0RCTYogaFloatFromCoreGraphicsFloat(lastBaseline);
}
