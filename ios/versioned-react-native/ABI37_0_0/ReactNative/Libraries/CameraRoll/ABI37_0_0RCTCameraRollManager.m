...
#import "ABI37_0_0RCTCameraRollManager.h"
...
#import "ABI37_0_0RCTAssetsLibraryRequestHandler.h"

@implementation ABI37_0_0RCTConvert (PHAssetCollectionSubtype)

ABI37_0_0RCT_ENUM_CONVERTER(PHAssetCollectionSubtype, (@{
...
@implementation ABI37_0_0RCTConvert (PHFetchOptions)
...
+ (PHFetchOptions *)PHFetchOptionsFromMediaType:(NSString *)mediaType
{
  ...
  if ([lowercase isEqualToString:@"photos"]) {
    ...
    if (![lowercase isEqualToString:@"all"]) {
      ABI37_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
...
@implementation ABI37_0_0RCTCameraRollManager
...
static void requestPhotoLibraryAccess(ABI37_0_0RCTPromiseRejectBlock reject, PhotosAuthorizedBlock authorizedBlock) {
...
ABI37_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
                  ...
                  reject:(ABI37_0_0RCTPromiseRejectBlock)reject)
{
  ...
  void (^loadBlock)(void) = ^void() {
    if ([type isEqualToString:@"video"]) {
      ...
      [[self.bridge moduleForClass:[ABI37_0_0RCTImageLoader class]] loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
        if (error) {
...
static void ABI37_0_0RCTResolvePromise(ABI37_0_0RCTPromiseResolveBlock resolve,
...
ABI37_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI37_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI37_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger const first = [ABI37_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *const afterCursor = [ABI37_0_0RCTConvert NSString:params[@"after"]];
  NSString *const groupName = [ABI37_0_0RCTConvert NSString:params[@"groupName"]];
  NSString *const groupTypes = [[ABI37_0_0RCTConvert NSString:params[@"groupTypes"]] lowercaseString];
  NSString *const mediaType = [ABI37_0_0RCTConvert NSString:params[@"assetType"]];
  NSArray<NSString *> *const mimeTypes = [ABI37_0_0RCTConvert NSStringArray:params[@"mimeTypes"]];
  ...
  PHAssetCollectionSubtype const collectionSubtype = [ABI37_0_0RCTConvert PHAssetCollectionSubtype:groupTypes];

  // Predicate for fetching assets within a collection
  PHFetchOptions *const assetFetchOptions = [ABI37_0_0RCTConvert PHFetchOptionsFromMediaType:mediaType];
    ...
    [assetCollectionFetchResult enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull assetCollection, NSUInteger collectionIdx, BOOL * _Nonnull stopCollections) {
      ...
      [assetsFetchResult enumerateObjectsUsingBlock:^(PHAsset * _Nonnull asset, NSUInteger assetIdx, BOOL * _Nonnull stopAssets) {
        ...
        if (first == assets.count) {
          ...
          *stopCollections = YES;
          hasNextPage = YES;
          ABI37_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
          ABI37_0_0RCTResolvePromise(resolve, assets, hasNextPage);
          resolvedPromise = YES;
          return;
        }
...
ABI37_0_0RCT_EXPORT_METHOD(deletePhotos:(NSArray<NSString *>*)assets
                  resolve:(ABI37_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI37_0_0RCTPromiseRejectBlock)reject)
{
  NSArray<NSURL *> *assets_ = [ABI37_0_0RCTConvert NSURLArray:assets];
  [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI37_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
