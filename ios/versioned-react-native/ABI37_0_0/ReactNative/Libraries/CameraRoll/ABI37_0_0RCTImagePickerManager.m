...
#import "ABI37_0_0RCTImagePickerManager.h"
...
@interface ABI37_0_0RCTImagePickerController : UIImagePickerController

@property (nonatomic, assign) BOOL unmirrorFrontFacingCamera;

@end

@implementation ABI37_0_0RCTImagePickerController

@end

@interface ABI37_0_0RCTImagePickerManager () <UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI37_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI37_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI37_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
  NSMutableDictionary<NSString *, NSDictionary<NSString *, id> *> *_pendingVideoInfo;
}

ABI37_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI37_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI37_0_0RCTResponseSenderBlock)callback)
{
...
ABI37_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI37_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI37_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI37_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI37_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  ABI37_0_0RCTImagePickerController *imagePicker = [ABI37_0_0RCTImagePickerController new];
  imagePicker.delegate = self;
  imagePicker.sourceType = UIImagePickerControllerSourceTypeCamera;
  NSArray<NSString *> *availableMediaTypes = [UIImagePickerController availableMediaTypesForSourceType:UIImagePickerControllerSourceTypeCamera];
  imagePicker.mediaTypes = availableMediaTypes;
  imagePicker.unmirrorFrontFacingCamera = [ABI37_0_0RCTConvert BOOL:config[@"unmirrorFrontFacingCamera"]];

  if ([ABI37_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI37_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI37_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI37_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI37_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI37_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI37_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
ABI37_0_0RCT_EXPORT_METHOD(clearAllPendingVideos)
{
...
ABI37_0_0RCT_EXPORT_METHOD(removePendingVideo:(NSString *)url)
{
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    ...
    [self _dismissPicker:picker args:@[imageURLString, ABI37_0_0RCTNullIfNil(height), ABI37_0_0RCTNullIfNil(width)]];
    return;
  }
  ...
  [_bridge.imageStoreManager storeImage:originalImage withBlock:^(NSString *tempImageTag) {
    [self _dismissPicker:picker args:tempImageTag ? @[tempImageTag, ABI37_0_0RCTNullIfNil(height), ABI37_0_0RCTNullIfNil(width)] : nil];
  }];
}
        ...
        cancelCallback:(ABI37_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI37_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI37_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI37_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
...
- (void)cameraChanged:(NSNotification *)notification
{
  for (UIImagePickerController *picker in _pickers) {
    if ([picker isKindOfClass:[ABI37_0_0RCTImagePickerController class]]
      && ((ABI37_0_0RCTImagePickerController *)picker).unmirrorFrontFacingCamera
