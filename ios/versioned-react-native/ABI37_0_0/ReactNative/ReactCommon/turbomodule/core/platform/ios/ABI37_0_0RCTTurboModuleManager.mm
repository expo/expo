...
#import "ABI37_0_0RCTTurboModuleManager.h"
...
#import <ABI37_0_0ReactCommon/ABI37_0_0BridgeJSCallInvoker.h>
#import <ABI37_0_0ReactCommon/ABI37_0_0TurboCxxModule.h>
#import <ABI37_0_0ReactCommon/ABI37_0_0TurboModuleBinding.h>

using namespace ABI37_0_0facebook;

// Fallback lookup since ABI37_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI37_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI37_0_0RCTTurboModuleManager {
  jsi::Runtime *_runtime;
  std::shared_ptr<ABI37_0_0facebook::ABI37_0_0React::JSCallInvoker> _jsInvoker;
  std::shared_ptr<ABI37_0_0React::TurboModuleBinding> _binding;
  __weak id<ABI37_0_0RCTTurboModuleManagerDelegate> _delegate;
   ...
   */
  std::unordered_map<std::string, id<ABI37_0_0RCTTurboModule>> _rctTurboModuleCache;
  std::unordered_map<std::string, std::shared_ptr<ABI37_0_0React::TurboModule>> _turboModuleCache;

  /**
   * _rctTurboModuleCache can be accessed by multiple threads at once via
   * the provideABI37_0_0RCTTurboModule method. This can lead to races. Therefore, we
...
{
  if (self = [super init]) {
    _jsInvoker = std::make_shared<ABI37_0_0React::BridgeJSCallInvoker>(bridge.ABI37_0_0ReactInstance);
    _delegate = delegate;
    _bridge = bridge;
    _invalidating = false;

    // Necessary to allow NativeModules to lookup TurboModules
    [bridge setABI37_0_0RCTTurboModuleLookupDelegate:self];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeWillInvalidateModules:)
                                                 name:ABI37_0_0RCTBridgeWillInvalidateModulesNotification
                                               object:_bridge.parentBridge];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidInvalidateModules:)
                                                 name:ABI37_0_0RCTBridgeDidInvalidateModulesNotification
                                               object:_bridge.parentBridge];

    __weak __typeof(self) weakSelf = self;

    auto moduleProvider = [weakSelf](const std::string &name) -> std::shared_ptr<ABI37_0_0React::TurboModule> {
      if (!weakSelf) {
        return nullptr;
      }
      ...
      if (moduleWasNotInitialized) {
        [strongSelf->_bridge.performanceLogger markStartForTag:ABI37_0_0RCTPLTurboModuleSetup];
      }
      ...
      if (moduleWasNotInitialized && [strongSelf moduleIsInitialized:moduleName]) {
        [strongSelf->_bridge.performanceLogger markStopForTag:ABI37_0_0RCTPLTurboModuleSetup];
    ...
    _binding = std::make_shared<ABI37_0_0React::TurboModuleBinding>(moduleProvider);
  }
  return self;
}
...
- (void)notifyAboutTurboModuleSetup:(const char *)name
{
  ...
  if (moduleName) {
    int64_t setupTime = [self->_bridge.performanceLogger durationForTag:ABI37_0_0RCTPLTurboModuleSetup];
...
- (std::shared_ptr<ABI37_0_0React::TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  id<ABI37_0_0RCTTurboModule> module = [self provideABI37_0_0RCTTurboModule:moduleName];

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI37_0_0RCTTurboModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  // If ABI37_0_0RCTTurboModule supports creating its own C++ TurboModule object,
  // allow it to do so.
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI37_0_0RCTCxxModule.class]) {
    ...
    auto turboModule = std::make_shared<ABI37_0_0React::TurboCxxModule>([((ABI37_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
 ...
 * (for now).
 */
- (id<ABI37_0_0RCTTurboModule>)provideABI37_0_0RCTTurboModule:(const char *)moduleName
{
  Class moduleClass;
  id<ABI37_0_0RCTTurboModule> module = nil;

  {
    ...
    if (![moduleClass conformsToProtocol:@protocol(ABI37_0_0RCTTurboModule)]) {
      return nil;
    }
   ...
   * implementation, and a `- (ABI37_0_0RCTBridge *) bridge { ... }` method won't be
  ...
  if ([module respondsToSelector:@selector(bridge)] && _bridge) {
    ...
    @try {
      /**
       * ABI37_0_0RCTBridgeModule declares the bridge property as readonly.
       * Therefore, when authors of NativeModules synthesize the bridge
       * via @synthesize bridge = bridge;, the ObjC runtime generates
       * only a - (ABI37_0_0RCTBridge *) bridge: { ... } method. No setter is
      ...
      ABI37_0_0RCTLogError(
          ...
          ABI37_0_0RCTBridgeModuleNameForClass(module));
    }
  }
  ...
  if ([module respondsToSelector:@selector(methodQueue)]) {
    ...
    if (!methodQueue) {
      ...
      NSString *queueName = [NSString stringWithFormat:@"com.facebook.ABI37_0_0React.%@Queue", moduleClassName];
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI37_0_0RCTDidInitializeModuleNotification
                    object:_bridge
                  userInfo:@{@"module" : module, @"bridge" : ABI37_0_0RCTNullIfNil(_bridge.parentBridge)}];
  return module;
}
...
- (void)installJSBindingWithRuntime:(jsi::Runtime *)runtime
{
  ...
  ABI37_0_0React::TurboModuleBinding::install(*_runtime, _binding);
}
...
#pragma mark ABI37_0_0RCTTurboModuleLookupDelegate
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  if (warnOnLookupFailure && !module) {
    ABI37_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
- (void)bridgeDidInvalidateModules:(NSNotification *)notification
{
  ...
  std::unordered_map<std::string, id<ABI37_0_0RCTTurboModule>> rctCacheCopy;
  {
    std::unique_lock<std::mutex> lock(_rctTurboModuleCacheLock);
    rctCacheCopy.insert(_rctTurboModuleCache.begin(), _rctTurboModuleCache.end());
  }

  // Backward-compatibility: ABI37_0_0RCTInvalidating handling.
  dispatch_group_t moduleInvalidationGroup = dispatch_group_create();
  for (const auto &p : rctCacheCopy) {
    id<ABI37_0_0RCTTurboModule> module = p.second;
    if ([module respondsToSelector:@selector(invalidate)]) {
      if ([module respondsToSelector:@selector(methodQueue)]) {
        ...
        if (methodQueue) {
          ...
          [bridge
              dispatchBlock:^{
                [((id<ABI37_0_0RCTInvalidating>)module) invalidate];
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI37_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
