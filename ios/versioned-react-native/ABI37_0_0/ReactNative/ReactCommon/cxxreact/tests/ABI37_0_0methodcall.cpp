...
#include <ABI37_0_0cxxreact/ABI37_0_0MethodCall.h>
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI37_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI37_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI37_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
