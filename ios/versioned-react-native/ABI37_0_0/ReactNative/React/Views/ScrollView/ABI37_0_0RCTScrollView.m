...
@interface ABI37_0_0RCTScrollEvent : NSObject <ABI37_0_0RCTEvent>
...
@implementation ABI37_0_0RCTScrollEvent
{
...
- (ABI37_0_0RCTScrollEvent *)coalesceWithEvent:(ABI37_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI37_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI37_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI37_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
...
@implementation ABI37_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI37_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI37_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI37_0_0RCTUIManager JSResponder] isKindOfClass:[ABI37_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI37_0_0React scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
    ABI37_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
    ABI37_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI37_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI37_0_0RCTScrollView () <ABI37_0_0RCTUIManagerObserver>

@end

@implementation ABI37_0_0RCTScrollView
{
  ABI37_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI37_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
{
  ABI37_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI37_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI37_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setABI37_0_0ReactLayoutDirection:layoutDirection];

  ABI37_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI37_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI37_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI37_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI37_0_0RCTAssert(_contentView == nil, @"ABI37_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@", [_contentView ABI37_0_0React_recursiveDescription]);
    _contentView = view;
    ABI37_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI37_0_0ReactLayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI37_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI37_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI37_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI37_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI37_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ABI37_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI37_0_0RCTView autoAdjustInsetsForView:self
...
#define ABI37_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI37_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI37_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI37_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI37_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI37_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI37_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI37_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI37_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ABI37_0_0ReactSubviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI37_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI37_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI37_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI37_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI37_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI37_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI37_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI37_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI37_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(__unused ABI37_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI37_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI37_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI37_0_0RCTScrollEvent *scrollEvent = [[ABI37_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI37_0_0RCTEventDispatcher (ABI37_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ABI37_0_0ReactTag
{
  ...
  ABI37_0_0RCTScrollEvent *fakeScrollEvent = [[ABI37_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI37_0_0ReactTag:ABI37_0_0ReactTag
