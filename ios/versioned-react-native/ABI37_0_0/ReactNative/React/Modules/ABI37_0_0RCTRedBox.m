...
#import "ABI37_0_0RCTRedBoxExtraDataViewController.h"
#import "ABI37_0_0RCTUtils.h"

#if ABI37_0_0RCT_DEV

@class ABI37_0_0RCTRedBoxWindow;

@protocol ABI37_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI37_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI37_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI37_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI37_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI37_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI37_0_0RCTRedBoxWindow
{
    UITableView *_stackTraceTableView;
    NSString *_lastErrorMessage;
    NSArray<ABI37_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (NSInteger)bottomSafeViewHeight
{
    if (@available(iOS 11.0, *)) {
        return ABI37_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
    } else {
        return 0;
    }
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI37_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
    ...
    [ABI37_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
    ...
    for (ABI37_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI37_0_0RCTJSStackFrame *)stackFrame
{
    NSString *fileName = ABI37_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ABI37_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI37_0_0RCTJSStackFrame *)stackFrame
{
    if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 1) {
        ...
        ABI37_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
    // NOTE: We could use ABI37_0_0RCTKeyCommands for this, but since
...
@interface ABI37_0_0RCTRedBox () <ABI37_0_0RCTInvalidating, ABI37_0_0RCTRedBoxWindowActionDelegate, ABI37_0_0RCTRedBoxExtraDataActionDelegate>
@end

@implementation ABI37_0_0RCTRedBox
{
    ABI37_0_0RCTRedBoxWindow *_window;
    NSMutableArray<id<ABI37_0_0RCTErrorCustomizer>> *_errorCustomizers;
    ABI37_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
}
...
- (ABI37_0_0RCTErrorInfo *)_customizeError:(ABI37_0_0RCTErrorInfo *)error
{
    ABI37_0_0RCTAssertMainQueue();
    if (!self->_errorCustomizers) {
        return error;
    }
    for (id<ABI37_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
        ABI37_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
        if (newInfo) {
            error = newInfo;
        }
    }
...
- (void)showError:(NSError *)error
{
    [self showErrorMessage:error.localizedDescription
               withDetails:error.localizedFailureReason
                     stack:error.userInfo[ABI37_0_0RCTJSStackTraceKey]];
}
...
- (void)showErrorMessage:(NSString *)message withDetails:(NSString *)details stack:(NSArray<ABI37_0_0RCTJSStackFrame *> *)stack {
...
{
    NSArray<ABI37_0_0RCTJSStackFrame *> *stack = [ABI37_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
    [self showErrorMessage:message withParsedStack:stack isUpdate:NO];
}

- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
    [self showErrorMessage:message withParsedStack:[ABI37_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:NO];
}

- (void)updateErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack
{
    [self showErrorMessage:message withParsedStack:[ABI37_0_0RCTJSStackFrame stackFramesWithDictionaries:stack] isUpdate:YES];
}
...
- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI37_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_extraDataViewController == nil) {
            self->_extraDataViewController = [ABI37_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
        ...
        if (!self->_window) {
            self->_window = [[ABI37_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
            self->_window.actionDelegate = self;
        }

        ABI37_0_0RCTErrorInfo *errorInfo = [[ABI37_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                       stack:stack];
...
ABI37_0_0RCT_EXPORT_METHOD(setExtraData:(NSDictionary *)extraData forIdentifier:(NSString *)identifier) {
    [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI37_0_0RCT_EXPORT_METHOD(dismiss)
{
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI37_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI37_0_0RCTJSStackFrame *)stackFrame
{
    NSURL *const bundleURL = _overrideBundleURL ?: _bridge.bundleURL;
    if (![bundleURL.scheme hasPrefix:@"http"]) {
        ABI37_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
        return;
    }

    NSData *stackFrameJSON = [ABI37_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI37_0_0RCTRedBoxWindow *)redBoxWindow
{
...
@implementation ABI37_0_0RCTBridge (ABI37_0_0RCTRedBox)

- (ABI37_0_0RCTRedBox *)redBox
{
    return [self moduleForClass:[ABI37_0_0RCTRedBox class]];
}
...
- (ABI37_0_0RCTRedBox *)redBox { return nil; }
