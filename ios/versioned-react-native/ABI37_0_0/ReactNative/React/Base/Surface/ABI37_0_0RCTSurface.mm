...
#import "ABI37_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI37_0_0RCTSurface () <ABI37_0_0RCTSurfaceRootShadowViewDelegate, ABI37_0_0RCTUIManagerObserver>
@end

@implementation ABI37_0_0RCTSurface {
  // Immutable
  ...
  ABI37_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI37_0_0RCTSurfaceView *_Nullable _view;
  ABI37_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI37_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI37_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI37_0_0RCTAllocateRootViewTag();
    ...
    if (!bridge.loading) {
      _stage = _stage | ABI37_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI37_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI37_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI37_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI37_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI37_0_0RCTSurfaceRootView *rootView =
    (ABI37_0_0RCTSurfaceRootView *)[self._uiManager viewForABI37_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI37_0_0RCTAssert([rootView isKindOfClass:[ABI37_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instance of `ABI37_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI37_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI37_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI37_0_0RCTSurfaceStage)stage
{
...
- (void)_run
{
  ABI37_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI37_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI37_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI37_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI37_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI37_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI37_0_0RCTIsMainQueue() ? ABI37_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI37_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI37_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI37_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI37_0_0ReactTag:self->_rootViewTag];
    ABI37_0_0RCTAssert([rootShadowView isKindOfClass:[ABI37_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instance of `ABI37_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI37_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI37_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI37_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI37_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI37_0_0ReactTag:self->_rootViewTag];
...
{
  if (ABI37_0_0RCTIsUIManagerQueue()) {
    ABI37_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI37_0_0RCTIsMainQueue() && (stage & ABI37_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI37_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI37_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI37_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI37_0_0RCTAssert(NO, @"Only waiting for `ABI37_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI37_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI37_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI37_0_0RCTIsMainQueue() && (stage & ABI37_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI37_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI37_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI37_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI37_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI37_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI37_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI37_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI37_0_0RCTUIManager *) manager performMountingWithBlock:(ABI37_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI37_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI37_0_0RCTUIManager *)manager
{
  if (self.stage & ABI37_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI37_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ABI37_0_0React components

- (void)mountABI37_0_0ReactComponentWithBridge:(ABI37_0_0RCTBridge *)bridge moduleName:(NSString *)moduleName params:(NSDictionary *)params
{
  [bridge enqueueJSCall:@"AppRegistry" method:@"runApplication" args:@[moduleName, params] completion:NULL];
}

- (void)unmountABI37_0_0ReactComponentWithBridge:(ABI37_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
