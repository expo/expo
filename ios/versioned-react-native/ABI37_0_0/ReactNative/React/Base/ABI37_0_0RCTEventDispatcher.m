...
const NSInteger ABI37_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI37_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI37_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(
    viewTag.intValue |
...
static uint16_t ABI37_0_0RCTUniqueCoalescingKeyGenerator = 0;

@implementation ABI37_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI37_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI37_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI37_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI37_0_0RCTComponentEvent *event = [[ABI37_0_0RCTComponentEvent alloc] initWithName:events[type]
                                                             viewTag:ABI37_0_0ReactTag
                                                                body:body];
...
- (void)sendEvent:(id<ABI37_0_0RCTEvent>)event
{
  ...
  for (id<ABI37_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI37_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI37_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI37_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI37_0_0RCTGetEventID(event.viewTag, event.eventName, ABI37_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI37_0_0RCTAssert(previousEvent == nil, @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
    [_eventQueue addObject:eventID];
  }
...
@implementation ABI37_0_0RCTBridge (ABI37_0_0RCTEventDispatcher)

- (ABI37_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI37_0_0RCTEventDispatcher class]];
}
