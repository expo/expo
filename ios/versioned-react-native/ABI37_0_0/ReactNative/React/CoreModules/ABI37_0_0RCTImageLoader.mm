...
static NSInteger ABI37_0_0RCTImageBytesForImage(UIImage *image)
{
  NSInteger singleImageBytes = image.size.width * image.size.height * image.scale * image.scale * 4;
  return image.images ? image.images.count * singleImageBytes : singleImageBytes;
}

@interface ABI37_0_0RCTImageLoader() <ABI37_0_0NativeImageLoaderSpec>

@end

@implementation UIImage (ABI37_0_0React)

- (NSInteger)ABI37_0_0ReactDecodedImageBytes
{
  NSNumber *imageBytes = objc_getAssociatedObject(self, _cmd);
  if (!imageBytes) {
    imageBytes = @(ABI37_0_0RCTImageBytesForImage(self));
  }
  return [imageBytes integerValue];
}

- (void)setABI37_0_0ReactDecodedImageBytes:(NSInteger)bytes
{
  objc_setAssociatedObject(self, @selector(ABI37_0_0ReactDecodedImageBytes), @(bytes), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

@end

@implementation ABI37_0_0RCTImageLoader
{
  NSArray<id<ABI37_0_0RCTImageURLLoader>> * (^_loadersProvider)(void);
  NSArray<id<ABI37_0_0RCTImageDataDecoder>> * (^_decodersProvider)(void);
  NSArray<id<ABI37_0_0RCTImageURLLoader>> *_loaders;
  NSArray<id<ABI37_0_0RCTImageDataDecoder>> *_decoders;
  NSOperationQueue *_imageDecodeQueue;
  dispatch_queue_t _URLRequestQueue;
  id<ABI37_0_0RCTImageCache> _imageCache;
  NSMutableArray *_pendingTasks;
  NSInteger _activeTasks;
  NSMutableArray *_pendingDecodes;
  NSInteger _scheduledDecodes;
  NSUInteger _activeBytes;
  __weak id<ABI37_0_0RCTImageRedirectProtocol> _redirectDelegate;
}
...
- (instancetype)initWithRedirectDelegate:(id<ABI37_0_0RCTImageRedirectProtocol>)redirectDelegate
                        ...
                        decodersProvider:(NSArray<id<ABI37_0_0RCTImageDataDecoder>> * (^)(void))getHandlers
{
...
- (void)setUp
{
  ...
  _URLRequestQueue = dispatch_queue_create("com.facebook.ABI37_0_0React.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (id<ABI37_0_0RCTImageCache>)imageCache
{
  if (!_imageCache) {
    ...
    _imageCache = [ABI37_0_0RCTImageCache new];
  }
  return _imageCache;
}

- (void)setImageCache:(id<ABI37_0_0RCTImageCache>)cache
{
  if (_imageCache) {
    ABI37_0_0RCTLogWarn(@"ABI37_0_0RCTImageCache was already set and has now been overridden.");
  }
  _imageCache = cache;
}

- (id<ABI37_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
  ...
  if (!_loaders) {
    ...
    ABI37_0_0RCTAssert(_bridge, @"Bridge not set");

    if (_loadersProvider) {
      _loaders = _loadersProvider();
    } else {
      _loaders = [_bridge modulesConformingToProtocol:@protocol(ABI37_0_0RCTImageURLLoader)];
    }

    _loaders = [_loaders sortedArrayUsingComparator:^NSComparisonResult(id<ABI37_0_0RCTImageURLLoader> a, id<ABI37_0_0RCTImageURLLoader> b) {
  ...
  if (ABI37_0_0RCT_DEBUG) {
    ...
    id<ABI37_0_0RCTImageURLLoader> previousLoader = nil;
    for (id<ABI37_0_0RCTImageURLLoader> loader in _loaders) {
      ...
      if ([loader canLoadImageURL:URL]) {
        if (previousLoader) {
          if (priority == previousPriority) {
            ABI37_0_0RCTLogError(@"The ABI37_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI37_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
  if (!_maxConcurrentLoadingTasks) {
    [self setUp];
  }
  ...
  if (!_decoders) {
    ...
    if (_decodersProvider) {
      ...
      _decoders = [_bridge modulesConformingToProtocol:@protocol(ABI37_0_0RCTImageDataDecoder)];
    }

    _decoders = [[_bridge modulesConformingToProtocol:@protocol(ABI37_0_0RCTImageDataDecoder)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI37_0_0RCTImageDataDecoder> a, id<ABI37_0_0RCTImageDataDecoder> b) {
  ...
  if (ABI37_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI37_0_0RCTImageDataDecoder> previousDecoder = nil;
    for (id<ABI37_0_0RCTImageDataDecoder> decoder in _decoders) {
      ...
      if ([decoder canDecodeImageData:data]) {
        if (previousDecoder) {
          if (priority == previousPriority) {
            ABI37_0_0RCTLogError(@"The ABI37_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI37_0_0RCTResizeImageIfNeeded(UIImage *image,
                                       CGSize size,
                                       CGFloat scale,
                                       ABI37_0_0RCTResizeMode resizeMode)
{
  ...
  CGRect targetSize = ABI37_0_0RCTTargetRect(image.size, size, scale, resizeMode);
  CGAffineTransform transform = ABI37_0_0RCTTransformFromTargetRect(image.size, targetSize);
  image = ABI37_0_0RCTTransformImage(image, size, scale, transform);
  return image;
}

- (ABI37_0_0RCTImageLoaderCancellationBlock) loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                   callback:(ABI37_0_0RCTImageLoaderCompletionBlock)callback
{
...
- (void)dequeueTasks
{
  dispatch_async(_URLRequestQueue, ^{
    ...
    for (ABI37_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
      switch (task.status) {
        case ABI37_0_0RCTNetworkTaskFinished:
          if (!tasksToRemove) {
          ...
          self->_activeTasks--;
          break;
        case ABI37_0_0RCTNetworkTaskPending:
          break;
        case ABI37_0_0RCTNetworkTaskInProgress:
          // Check task isn't "stuck"
          if (task.requestToken == nil) {
            ABI37_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
            if (!tasksToRemove) {
              tasksToRemove = [NSMutableArray new];
            }
            [tasksToRemove addObject:task];
    ...
    // Start queued tasks
    for (ABI37_0_0RCTNetworkTask *task in self->_pendingTasks) {
      if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
        break;
      }
      if (task.status == ABI37_0_0RCTNetworkTaskPending) {
        [task start];
        self->_activeTasks++;
      }
    }
 ...
 */
- (ABI37_0_0RCTImageLoaderCancellationBlock)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                             size:(CGSize)size
                                                            scale:(CGFloat)scale
                                                       resizeMode:(ABI37_0_0RCTResizeMode)resizeMode
                                                    progressBlock:(ABI37_0_0RCTImageLoaderProgressBlock)progressHandler
                                                 partialLoadBlock:(ABI37_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
                                                  completionBlock:(void (^)(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response))completionBlock
{
  {
    NSMutableURLRequest *mutableRequest = [request mutableCopy];
    [NSURLProtocol setProperty:@"ABI37_0_0RCTImageLoader"
                        forKey:@"trackingName"
  ...
  id<ABI37_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
  ...
  void (^completionHandler)(NSError *, id, NSURLResponse *) = ^(NSError *error, id imageOrData, NSURLResponse *response) {
    ...
    if (ABI37_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
  ...
  __weak ABI37_0_0RCTImageLoader *weakSelf = self;
  dispatch_async(_URLRequestQueue, ^{
...
- (ABI37_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI37_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSURLResponse *response))completionHandler
{
  // Check if networking module is available
  if (ABI37_0_0RCT_DEBUG && ![_bridge respondsToSelector:@selector(networking)]) {
    ABI37_0_0RCTLogError(@"No suitable image URL loader found for %@. You may need to "
                " import the ABI37_0_0RCTNetwork library in order to load images.",
                request.URL.absoluteString);
    return NULL;
  }

  ABI37_0_0RCTNetworking *networking = [_bridge networking];

  // Check if networking module can load image
  if (ABI37_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
    ABI37_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
    return NULL;
  }

  // Use networking module to load image
  ABI37_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    // Check for system errors
    if (error) {
      completionHandler(error, nil, response);
      return;
    } else if (!response) {
      completionHandler(ABI37_0_0RCTErrorWithMessage(@"Response metadata error"), nil, response);
      return;
    } else if (!data) {
      completionHandler(ABI37_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, response);
      return;
    }
  ...
  __block ABI37_0_0RCTNetworkTask *task =
                     ...
                     completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                       ...
                       if (error || !response || !data) {
                         ...
                         if (error) {
                           ...
                           someError = ABI37_0_0RCTErrorWithMessage(@"Response metadata error");
                         } else {
                           someError = ABI37_0_0RCTErrorWithMessage(@"Unknown image download error");
                         }
...
- (ABI37_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                ...
                                                resizeMode:(ABI37_0_0RCTResizeMode)resizeMode
                                             progressBlock:(ABI37_0_0RCTImageLoaderProgressBlock)progressBlock
                                          partialLoadBlock:(ABI37_0_0RCTImageLoaderPartialLoadBlock)partialLoadBlock
                                           completionBlock:(ABI37_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  auto cancelled = std::make_shared<std::atomic<int>>(0);
  __block dispatch_block_t cancelLoad = nil;
  __block NSLock *cancelLoadLock = [NSLock new];
  ...
  void (^completionHandler)(NSError *, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response) {
    ...
    ABI37_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
...
- (ABI37_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI37_0_0RCTResizeMode)resizeMode
...
{
  if (data.length == 0) {
    completionBlock(ABI37_0_0RCTErrorWithMessage(@"No image data"), nil);
    return ^{};
  }
  ...
  void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
    if (ABI37_0_0RCTIsMainQueue()) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          completionBlock(error, clipped ? ABI37_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
        }
      });
    } else if (!std::atomic_load(cancelled.get())) {
      completionBlock(error, clipped ? ABI37_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
  ...
  id<ABI37_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
  if (imageDecoder) {
    ...
    dispatch_block_t decodeBlock = ^{
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          ...
          UIImage *image = ABI37_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI37_0_0RCT_DEV
          CGSize imagePixelSize = ABI37_0_0RCTSizeInPixels(image.size, image.scale);
          CGSize screenPixelSize = ABI37_0_0RCTSizeInPixels(ABI37_0_0RCTScreenSize(), ABI37_0_0RCTScreenScale());
          if (imagePixelSize.width * imagePixelSize.height >
              screenPixelSize.width * screenPixelSize.height) {
            ABI37_0_0RCTLogInfo(@"[PERF ASSETS] Loading image at size %@, which is larger "
          ...
          if (image) {
            ...
            NSError *finalError = ABI37_0_0RCTErrorWithMessage(errorMessage);
...
- (ABI37_0_0RCTImageLoaderCancellationBlock)getImageSizeForURLRequest:(NSURLRequest *)imageURLRequest
                                                       block:(void(^)(NSError *error, CGSize size))callback
{
  void (^completion)(NSError *, id, BOOL, NSURLResponse *) = ^(NSError *error, id imageOrData, BOOL cacheResult, NSURLResponse *response) {
    ...
    if ([imageOrData isKindOfClass:[NSData class]]) {
      NSDictionary *meta = ABI37_0_0RCTGetImageMetadata(imageOrData);
...
- (NSDictionary *)getImageCacheStatus:(NSArray *)requests
{
  ...
  for (id request in requests) {
    NSURLRequest *urlRequest = [ABI37_0_0RCTConvert NSURLRequest:request];
    if (urlRequest) {
...
#pragma mark - ABI37_0_0RCTURLRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  // Even better may be to implement this on the ABI37_0_0RCTImageURLLoader that would try to load it,
  // but we'd have to run the logic both in ABI37_0_0RCTPhotoLibraryImageLoader and
  // ABI37_0_0RCTAssetsLibraryRequestHandler. Once we drop iOS7 though, we'd drop
  // ABI37_0_0RCTAssetsLibraryRequestHandler and can move it there.
  ...
  for (id<ABI37_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI37_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI37_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI37_0_0RCTURLRequestHandler)] &&
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI37_0_0RCTURLRequestDelegate>)delegate
{
  __block ABI37_0_0RCTImageLoaderCancellationBlock requestToken;
  requestToken = [self loadImageWithURLRequest:request callback:^(NSError *error, UIImage *image) {
    ...
    if (ABI37_0_0RCTImageHasAlpha(image.CGImage)) {
      mimeType = @"image/png";
...
{
  if (requestToken) {
    ((ABI37_0_0RCTImageLoaderCancellationBlock)requestToken)();
  }
}
...
{
  return std::make_shared<ABI37_0_0facebook::ABI37_0_0React::NativeImageLoaderSpecJSI>(self, jsInvoker);
}
 ...
 */
@implementation ABI37_0_0RCTBridge (ABI37_0_0RCTImageLoader)

- (ABI37_0_0RCTImageLoader *)imageLoader
{
  ABI37_0_0RCTLogWarn(@"Calling bridge.imageLoader is deprecated and will not work in newer versions of ABI37_0_0RN. Please update to the"
             "moduleForClass API or turboModuleLookupDelegate API.");
  return [self moduleForClass:[ABI37_0_0RCTImageLoader class]];
}
...
{
  return ABI37_0_0RCTImageLoader.class;
}
