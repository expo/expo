...
#import "ABI37_0_0RCTCxxMethod.h"

using namespace ABI37_0_0facebook::ABI37_0_0React;

@implementation ABI37_0_0RCTCxxModule
{
  std::unique_ptr<ABI37_0_0facebook::xplat::module::CxxModule> _module;
}
...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI37_0_0RCTAssert([ABI37_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
                @"CxxModule class name %@ does not match runtime name %s",
                ABI37_0_0RCTBridgeModuleNameForClass([self class]), _module->getName().c_str());
    }
  }
}

- (std::unique_ptr<ABI37_0_0facebook::xplat::module::CxxModule>)createModule
{
  ABI37_0_0RCTAssert(NO, @"Subclass %@ must override createModule", [self class]);
  return nullptr;
}
...
- (NSArray<id<ABI37_0_0RCTBridgeMethod>> *)methodsToExport;
{
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI37_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
