...
static NSString *const ABI28_0_0RCTImageStoreURLScheme = @"rct-image-store";

@implementation ABI28_0_0RCTImageStoreManager
{
...
{
  ABI28_0_0RCTAssertThread(_methodQueue, @"Must be called on ABI28_0_0RCTImageStoreManager thread");
  ...
  NSString *imageTag = [NSString stringWithFormat:@"%@://%tu", ABI28_0_0RCTImageStoreURLScheme, _id++];
...
- (void)storeImage:(UIImage *)image withBlock:(void (^)(NSString *imageTag))block
{
  ABI28_0_0RCTAssertParam(block);
  dispatch_async(_methodQueue, ^{
    NSString *imageTag = [self _storeImageData:ABI28_0_0RCTGetImageData(image, 0.75)];
...
ABI28_0_0RCT_EXPORT_METHOD(removeImageForTag:(NSString *)imageTag)
{
  [_store removeObjectForKey:imageTag];
}

ABI28_0_0RCT_EXPORT_METHOD(hasImageForTag:(NSString *)imageTag
...
ABI28_0_0RCT_EXPORT_METHOD(getBase64ForTag:(NSString *)imageTag
                  ...
                  errorCallback:(ABI28_0_0RCTResponseErrorBlock)errorCallback)
{
  ...
  if (!imageData) {
    errorCallback(ABI28_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]));
    return;
  }
...
ABI28_0_0RCT_EXPORT_METHOD(addImageFromBase64:(NSString *)base64String
                  successCallback:(ABI28_0_0RCTResponseSenderBlock)successCallback
                  errorCallback:(ABI28_0_0RCTResponseErrorBlock)errorCallback)

{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (imageData) {
      ...
      errorCallback(ABI28_0_0RCTErrorWithMessage(@"Failed to add image from base64String"));
    }
  });
}
...
{
  return [request.URL.scheme caseInsensitiveCompare:ABI28_0_0RCTImageStoreURLScheme] == NSOrderedSame;
}
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI28_0_0RCTURLRequestDelegate>)delegate
{
  ...
  dispatch_async(_methodQueue, ^{
    ...
    if (!imageData) {
      NSError *error = ABI28_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]);
    ...
    if (!sourceRef) {
      NSError *error = ABI28_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to decode data for imageTag: %@", imageTag]);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
      return;
    }
...
@implementation ABI28_0_0RCTImageStoreManager (Deprecated)

- (NSString *)storeImage:(UIImage *)image
{
  ABI28_0_0RCTAssertMainQueue();
  ABI28_0_0RCTLogWarn(@"ABI28_0_0RCTImageStoreManager.storeImage() is deprecated and has poor performance. Use an alternative method instead.");
  __block NSString *imageTag;
  dispatch_sync(_methodQueue, ^{
    imageTag = [self _storeImageData:ABI28_0_0RCTGetImageData(image, 0.75)];
  });
  return imageTag;
}
...
{
  ABI28_0_0RCTAssertMainQueue();
  ABI28_0_0RCTLogWarn(@"ABI28_0_0RCTImageStoreManager.imageForTag() is deprecated and has poor performance. Use an alternative method instead.");
...
@implementation ABI28_0_0RCTBridge (ABI28_0_0RCTImageStoreManager)

- (ABI28_0_0RCTImageStoreManager *)imageStoreManager
{
  return [self moduleForClass:[ABI28_0_0RCTImageStoreManager class]];
}
