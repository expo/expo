...
#import "ABI28_0_0RCTCxxMethod.h"

using namespace facebook::ReactABI28_0_0;

@implementation ABI28_0_0RCTCxxModule
{
...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI28_0_0RCTAssert([ABI28_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
                @"CxxModule class name %@ does not match runtime name %s",
                ABI28_0_0RCTBridgeModuleNameForClass([self class]), _module->getName().c_str());
    }
  }
}
...
{
  ABI28_0_0RCTAssert(NO, @"Subclass %@ must override createModule", [self class]);
  return nullptr;
}

- (NSArray<id<ABI28_0_0RCTBridgeMethod>> *)methodsToExport;
{
  [self lazyInit];
  if (!_module) {
    return nil;
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI28_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
