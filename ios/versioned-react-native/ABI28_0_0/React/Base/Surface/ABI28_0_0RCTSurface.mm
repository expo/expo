...
#import "ABI28_0_0RCTSurfaceView+Internal.h"
...
#import "ABI28_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI28_0_0RCTSurface () <ABI28_0_0RCTSurfaceRootShadowViewDelegate, ABI28_0_0RCTUIManagerObserver>
@end

@implementation ABI28_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI28_0_0RCTBridge *_batchedBridge;
  ABI28_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI28_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI28_0_0RCTSurfaceView *_Nullable _view;
  ABI28_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI28_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI28_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI28_0_0RCTAllocateRootViewTag();
    ...
    _stage = ABI28_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI28_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI28_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI28_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI28_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI28_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI28_0_0RCTSurfaceRootView *rootView =
    (ABI28_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI28_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI28_0_0RCTAssert([rootView isKindOfClass:[ABI28_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI28_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI28_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI28_0_0RCTSurfaceStageModuleDidLoad];
...
- (ABI28_0_0RCTSurfaceStage)stage
{
  std::lock_guard<std::mutex> lock(_mutex);
  return _stage;
}

- (void)_setStage:(ABI28_0_0RCTSurfaceStage)stage
{
  ABI28_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI28_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI28_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI28_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI28_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI28_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI28_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI28_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountReactABI28_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI28_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI28_0_0RCTIsMainQueue() ? ABI28_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI28_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI28_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI28_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI28_0_0Tag:self->_rootViewTag];
    ABI28_0_0RCTAssert([rootShadowView isKindOfClass:[ABI28_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI28_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI28_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI28_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI28_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI28_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI28_0_0Tag:self->_rootViewTag];
...
{
  if (ABI28_0_0RCTIsUIManagerQueue()) {
    ABI28_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI28_0_0RCTIsMainQueue() && (stage == ABI28_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI28_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI28_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI28_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI28_0_0RCTAssert(NO, @"Only waiting for `ABI28_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI28_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI28_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI28_0_0RCTIsMainQueue() && (stage == ABI28_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI28_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI28_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI28_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
...
{
  [self _setStage:ABI28_0_0RCTSurfaceStageSurfaceDidInitialRendering];
...
{
  [self _setStage:ABI28_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI28_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
{
  if (self.stage & ABI28_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI28_0_0RCTSurfaceStageSurfaceDidInitialMounting];
