...
static NSRegularExpression *ABI28_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI28_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI28_0_0RCTNullIfNil(self.methodName),
     @"file": ABI28_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI28_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
{
  return [[self alloc] initWithMethodName:ABI28_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI28_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI28_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}
...
+ (NSArray<ABI28_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI28_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI28_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI28_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
