...
const NSInteger ABI28_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI28_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI28_0_0RCTGetEventID(id<ABI28_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI28_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI28_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI28_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI28_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
{
  [_bridge enqueueJSCall:@"ABI28_0_0RCTDeviceEventEmitter"
                  method:@"emit"
                    args:body ? @[name, body] : @[name]
              completion:NULL];
}
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI28_0_0RCT_DEBUG) {
    ABI28_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI28_0_0 tag");
  }

  name = ABI28_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI28_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI28_0_0RCTEvent>)event
{
  ...
  for (id<ABI28_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI28_0_0RCTGetEventID(event);

  id<ABI28_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI28_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
@implementation ABI28_0_0RCTBridge (ABI28_0_0RCTEventDispatcher)

- (ABI28_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI28_0_0RCTEventDispatcher class]];
}
