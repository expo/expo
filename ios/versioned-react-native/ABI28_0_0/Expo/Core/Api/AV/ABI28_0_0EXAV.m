...
NSString *const ABI28_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI28_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI28_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI28_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI28_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI28_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI28_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI28_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI28_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI28_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI28_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI28_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

NSString *const ABI28_0_0EXDidUpdatePlaybackStatusEventName = @"didUpdatePlaybackStatus";

@interface ABI28_0_0EXAV ()
...
@property (nonatomic, assign) ABI28_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI28_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI28_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, assign) BOOL isBeingObserved;
@property (nonatomic, strong) NSMutableSet <NSObject<ABI28_0_0EXAVObject> *> *videoSet;
...
- (instancetype)initWithExperienceId:(NSString *)experienceId kernelServiceDelegates:(NSDictionary *)kernelServiceInstances params:(NSDictionary *)params
{
  if (self = [super initWithExperienceId:experienceId kernelServiceDelegates:kernelServiceInstances params:params]) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI28_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI28_0_0EXAudioInterruptionModeMixWithOthers;
...
- (void)_bridgeDidForeground:(NSNotification *)notification
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI28_0_0EXAVObject> *exAVObject) {
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI28_0_0EXAVObject> *exAVObject))block
{
  for (ABI28_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI28_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI28_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  BOOL allowsRecording = ((NSNumber *)mode[@"allowsRecordingIOS"]).boolValue;
  
  if (!playsInSilentMode && interruptionMode == ABI28_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI28_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and duckOthers cannot both be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI28_0_0RCTErrorWithMessage(@"Impossible audio mode: playsInSilentMode and allowsRecording cannot both be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI28_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI28_0_0EXAVAudioSessionMode)audioSessionMode
{
  NSError *error;
  ABI28_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI28_0_0EXAVAudioSessionModeActiveMuted
    ? ABI28_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
  
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI28_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI28_0_0EXAudioInterruptionModeDoNotMix) {
    ...
    switch (activeInterruptionMode) {
      ...
      case ABI28_0_0EXAudioInterruptionModeDuckOthers:
        ...
        break;
      case ABI28_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI28_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI28_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI28_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI28_0_0EXAVObject> *exAVObject) {
    ABI28_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI28_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI28_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI28_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI28_0_0RCTErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded) {
    return ABI28_0_0RCTErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI28_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI28_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI28_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI28_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI28_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI28_0_0RCTPromiseRejectBlock)reject
{
  ABI28_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
    reject(@"E_AUDIO_NOPLAYER", nil, ABI28_0_0RCTErrorWithMessage(@"Player does not exist."));
  }
}
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI28_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI28_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI28_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI28_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI28_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI28_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI28_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI28_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI28_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI28_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI28_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI28_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI28_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI28_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI28_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI28_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI28_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI28_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
      @([self _getFormatIDFromString:iosOptionsFromJS[ABI28_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI28_0_0RCTErrorWithMessage(@"Recorder already exists.");
  }
  
  NSString *directory = [self.bridge.scopedModules.fileSystem.cachesDirectory stringByAppendingPathComponent:@"AV"];
  [ABI28_0_0EXFileSystem ensureDirExistsWithPath:directory];
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI28_0_0RCTPromiseRejectBlock)reject
{
  if (_audioRecorder == nil) {
    reject(@"E_AUDIO_NORECORDER", nil, ABI28_0_0RCTErrorWithMessage(@"Recorder does not exist."));
  }
...
ABI28_0_0EX_EXPORT_SCOPED_MULTISERVICE_MODULE(ExponentAV, @"AudioSessionManager", @"PermissionsManager");

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI28_0_0EXDidUpdatePlaybackStatusEventName];
}

#pragma mark - Audio API: Global settings

ABI28_0_0RCT_EXPORT_METHOD(setAudioIsEnabled:(BOOL)value
...
ABI28_0_0RCT_EXPORT_METHOD(setAudioMode:(nonnull NSDictionary *)mode
                      resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                      rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
...
ABI28_0_0RCT_EXPORT_METHOD(loadForSound:(nonnull NSDictionary *)source
                    withStatus:(nonnull NSDictionary *)status
                   withSuccess:(ABI28_0_0RCTResponseSenderBlock)loadSuccess
                     withError:(ABI28_0_0RCTResponseSenderBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);

  __weak __typeof__(self) weakSelf = self;
  ABI28_0_0EXAVPlayerData *data = [[ABI28_0_0EXAVPlayerData alloc] initWithEXAV:self
                                                   withSource:source
                                                   withStatus:status
  ...
  data.statusUpdateCallback = ^(NSDictionary *status) {
    ...
    if (strongSelf && strongSelf.isBeingObserved) {
      ...
      [strongSelf sendEventWithName:ABI28_0_0EXDidUpdatePlaybackStatusEventName body:response];
    }
  };
...
ABI28_0_0RCT_EXPORT_METHOD(unloadForSound:(nonnull NSNumber *)key
                        resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI28_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI28_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI28_0_0RCT_EXPORT_METHOD(setStatusForSound:(nonnull NSNumber *)key
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
...
ABI28_0_0RCT_EXPORT_METHOD(getStatusForSound:(nonnull NSNumber *)key
                           resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI28_0_0EXAVPlayerData *data) {
...
ABI28_0_0RCT_EXPORT_METHOD(replaySound:(nonnull NSNumber *)key
                   withStatus:(nonnull NSDictionary *)status
                     resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                     rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
...
ABI28_0_0RCT_EXPORT_METHOD(setErrorCallbackForSound:(nonnull NSNumber *)key
                              withCallback:(ABI28_0_0RCTResponseSenderBlock)callback)
{
  ABI28_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    __block BOOL used = NO; // ABI28_0_0RCTResponseSenderBlock can only be used once
    __weak __typeof__(self) weakSelf = self;
    data.errorCallback = ^(NSString *error) {
...
ABI28_0_0RCT_EXPORT_METHOD(loadForVideo:(nonnull NSNumber *)ReactABI28_0_0Tag
...
ABI28_0_0RCT_EXPORT_METHOD(unloadForVideo:(nonnull NSNumber *)ReactABI28_0_0Tag
                        resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                        rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI28_0_0EXVideoView *view) {
...
ABI28_0_0RCT_EXPORT_METHOD(setStatusForVideo:(nonnull NSNumber *)ReactABI28_0_0Tag
                         withStatus:(nonnull NSDictionary *)status
                           resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
...
ABI28_0_0RCT_EXPORT_METHOD(replayVideo:(nonnull NSNumber *)ReactABI28_0_0Tag
                   withStatus:(nonnull NSDictionary *)status
                     resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                     rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
...
ABI28_0_0RCT_EXPORT_METHOD(getStatusForVideo:(nonnull NSNumber *)ReactABI28_0_0Tag
                           resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                           rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI28_0_0EXVideoView *view) {
...
ABI28_0_0RCT_EXPORT_METHOD(prepareAudioRecorder:(nonnull NSDictionary *)options
                              resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve
                              rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI28_0_0EXPermissions statusForPermissions:[ABI28_0_0EXAudioRecordingPermissionRequester permissions]] != ABI28_0_0EXPermissionStatusGranted ||
  ...
  if (_audioRecorder && !error) {
    ...
    if (error) {
      ...
      reject(@"E_AUDIO_RECORDERNOTCREATED", nil, ABI28_0_0RCTErrorWithMessage(@"Prepare encountered an error: recorder not prepared."));
    }
    _audioRecorderIsPreparing = false;
    [self demoteAudioSessionIfPossible];
  } else {
...
ABI28_0_0RCT_EXPORT_METHOD(startAudioRecording:(ABI28_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  if ([ABI28_0_0EXPermissions statusForPermissions:[ABI28_0_0EXAudioRecordingPermissionRequester permissions]] != ABI28_0_0EXPermissionStatusGranted ||
      ![_kernelPermissionsServiceDelegate hasGrantedPermission:@"audioRecording" forExperience:self.experienceId]) {
  ...
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (!_allowsAudioRecording) {
      reject(@"E_AUDIO_AUDIOMODE", nil, ABI28_0_0RCTErrorWithMessage(@"Recording not allowed on iOS."));
      ...
      if (!error) {
        if ([_audioRecorder record]) {
          ...
          reject(@"E_AUDIO_RECORDING", nil, ABI28_0_0RCTErrorWithMessage(@"Start encountered an error: recording not started."));
        }
      } else {
...
ABI28_0_0RCT_EXPORT_METHOD(pauseAudioRecording:(ABI28_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI28_0_0RCT_EXPORT_METHOD(stopAudioRecording:(ABI28_0_0RCTPromiseResolveBlock)resolve
                            rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI28_0_0RCT_EXPORT_METHOD(getAudioRecordingStatus:(ABI28_0_0RCTPromiseResolveBlock)resolve
                                 rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI28_0_0RCT_EXPORT_METHOD(unloadAudioRecorder:(ABI28_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
