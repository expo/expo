...
NSString *const ABI28_0_0EXPrintOrientationPortrait = @"portrait";
NSString *const ABI28_0_0EXPrintOrientationLandscape = @"landscape";

@interface ABI28_0_0EXPrint () <UIPrintInteractionControllerDelegate, UIPrinterPickerControllerDelegate>

@property (nonatomic, strong) NSMutableDictionary<NSString *, UIPrinter *> *printers;

@end

@implementation ABI28_0_0EXPrint
...
- (NSDictionary *)constantsToExport
{
  return @{
           @"Orientation": @{
               @"portrait": ABI28_0_0EXPrintOrientationPortrait,
               @"landscape": ABI28_0_0EXPrintOrientationLandscape,
               },
           };
}

ABI28_0_0RCT_EXPORT_MODULE(ExponentPrint);

ABI28_0_0RCT_EXPORT_METHOD(print:(NSDictionary *)options
                  ...
                  rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
  [self _getPrintingDataForOptions:options callback:^(NSData *printingData, NSDictionary *errorDetails) {
    if (errorDetails != nil) {
      reject(errorDetails[@"code"], errorDetails[@"message"], ABI28_0_0RCTErrorWithMessage(errorDetails[@"message"]));
      return;
    }
    ...
    if (printingData == nil) {
      ...
      if (options[@"markupFormatterIOS"]) {
        NSString *htmlString = [ABI28_0_0RCTConvert NSString:options[@"markupFormatterIOS"]];
        ...
        if (htmlString != nil) {
          ...
          reject(@"E_HTML_INVALID", message, ABI28_0_0RCTErrorWithMessage(message));
          return;
        }
      } else {
    ...
    if (options[@"printerUrl"]) {
      ...
      printerURL = [ABI28_0_0RCTConvert NSString:options[@"printerUrl"]];
...
ABI28_0_0RCT_EXPORT_METHOD(selectPrinter:(ABI28_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject)
{
...
ABI28_0_0RCT_REMAP_METHOD(printToFileAsync,
...
{
  NSString *format = [ABI28_0_0RCTConvert NSString:options[@"format"]];
  ...
  ABI28_0_0EXPrintPDFRenderTask *renderTask = [ABI28_0_0EXPrintPDFRenderTask new];
...
{
  NSString *uri = [ABI28_0_0RCTConvert NSString:options[@"uri"]];
...
- (void)_getPrintingDataForOptions:(nonnull NSDictionary *)options callback:(void(^)(NSData *, NSDictionary *))callback
{
  ...
  if (options[@"html"]) {
    __block ABI28_0_0EXPrintPDFRenderTask *renderTask = [ABI28_0_0EXPrintPDFRenderTask new];
...
{
  if ([orientation isEqualToString:ABI28_0_0EXPrintOrientationLandscape]) {
