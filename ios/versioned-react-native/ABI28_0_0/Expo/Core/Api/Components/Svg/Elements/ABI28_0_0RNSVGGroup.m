...
@implementation ABI28_0_0RNSVGGroup
{
...
- (void)renderGroupTo:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI28_0_0RNSVGNode *node) {
        if (node.responsible && !svg.responsible) {
            svg.responsible = YES;
        }

        if ([node isKindOfClass:[ABI28_0_0RNSVGRenderable class]]) {
            [(ABI28_0_0RNSVGRenderable*)node mergeProperties:self];
        }

        [node renderTo:context];

        if ([node isKindOfClass:[ABI28_0_0RNSVGRenderable class]]) {
            [(ABI28_0_0RNSVGRenderable*)node resetProperties];
        }
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI28_0_0RNSVGGlyphContext alloc] initWithScale:1 width:width
                                                   height:height];
}
...
- (void)parseReference
{
    ...
    [self traverseSubviews:^(__kindof ABI28_0_0RNSVGNode *node) {
        [node parseReference];
        return YES;
    }];
}
