...
@interface ABI28_0_0RNSVGGlyphContext () {
@public
    ...
    double mFontSize_;
    ABI28_0_0RNSVGFontData *topFont_;
...
@implementation ABI28_0_0RNSVGGlyphContext
...
- (CTFontRef)getGlyphFont
{
    ...
    NSString * fontWeight = [ABI28_0_0RNSVGFontWeightToString(topFont_->fontWeight) lowercaseString];
    NSString * fontStyle = ABI28_0_0RNSVGFontStyleStrings[topFont_->fontStyle];
    ...
    return (__bridge CTFontRef)[ABI28_0_0RCTFont updateFont:nil
...
void ABI28_0_0pushIndices(ABI28_0_0RNSVGGlyphContext *self) {
...
- (instancetype)initWithScale:(float)scale_
    ...
    self->mFontSize_ = ABI28_0_0RNSVGFontData_DEFAULT_FONT_SIZE;
    self->topFont_ = [ABI28_0_0RNSVGFontData Defaults];
...
ABI28_0_0RNSVGFontData *ABI28_0_0getTopOrParentFont(ABI28_0_0RNSVGGlyphContext *self, ABI28_0_0RNSVGGroup* child) {
    if (self->mTop_ > 0) {
        return self->topFont_;
    } else {
        ABI28_0_0RNSVGGroup* parentRoot = [child getParentTextRoot];
        ABI28_0_0RNSVGFontData* Defaults = [ABI28_0_0RNSVGFontData Defaults];
        while (parentRoot != nil) {
            ABI28_0_0RNSVGFontData *map = [[parentRoot getGlyphContext] getFont];
            if (map != Defaults) {
                return map;
            }
...
void ABI28_0_0pushNodeAndFont(ABI28_0_0RNSVGGlyphContext *self, ABI28_0_0RNSVGGroup* node, NSDictionary* font) {
    ABI28_0_0RNSVGFontData *parent = ABI28_0_0getTopOrParentFont(self, node);
    self->mTop_++;
    if (font == nil) {
        [self->mFontContext_ addObject:parent];
        return;
    }
    ABI28_0_0RNSVGFontData *data = [ABI28_0_0RNSVGFontData initWithNSDictionary:font
                                                 parent:parent
                                                  scale:self->mScale_];
...
- (void)pushContext:(ABI28_0_0RNSVGGroup*)node
               font:(NSDictionary*)font {
    ABI28_0_0pushNodeAndFont(self, node, font);
    ABI28_0_0pushIndices(self);
}
...
- (void)pushContext:(ABI28_0_0RNSVGText*)node
             ...
             deltaY:(NSArray*)deltaY
             rotate:(NSArray*)rotate {
    ABI28_0_0pushNodeAndFont(self, (ABI28_0_0RNSVGGroup*)node, font);
    if (x != nil && [x count] != 0) {
        mXsIndex_++;
        mXIndex_ = -1;
...
void ABI28_0_0incrementIndices(NSMutableArray *indices, long topIndex) {
...
- (double)nextXWithDouble:(double)advance {
    ABI28_0_0incrementIndices(mXIndices_, mXsIndex_);
    long nextIndex = mXIndex_ + 1;
    if (nextIndex < [mXs_ count]) {
        mDX_ = 0;
        mXIndex_ = nextIndex;
        NSString *string = [mXs_ objectAtIndex:nextIndex];
        mX_ = [ABI28_0_0RNSVGPropHelper fromRelativeWithNSString:string
                                            relative:mWidth_
                                              offset:0
                                               scale:mScale_
                                            fontSize:mFontSize_];
...
- (double)nextY {
    ABI28_0_0incrementIndices(mYIndices_, mYsIndex_);
    long nextIndex = mYIndex_ + 1;
    if (nextIndex < [mYs_ count]) {
        mDY_ = 0;
        mYIndex_ = nextIndex;
        NSString *string = [mYs_ objectAtIndex:nextIndex];
        mY_ = [ABI28_0_0RNSVGPropHelper fromRelativeWithNSString:string
                                            relative:mHeight_
                                              offset:0
                                               scale:mScale_
                                            fontSize:mFontSize_];
...
- (double)nextDeltaX {
    ABI28_0_0incrementIndices(mDXIndices_, mDXsIndex_);
    long nextIndex = mDXIndex_ + 1;
    if (nextIndex < [mDXs_ count]) {
        mDXIndex_ = nextIndex;
        NSString *string = [mDXs_ objectAtIndex:nextIndex];
        double val = [ABI28_0_0RNSVGPropHelper fromRelativeWithNSString:string
                                                   relative:mWidth_
                                                     offset:0
                                                      scale:mScale_
                                                   fontSize:mFontSize_];
...
- (double)nextDeltaY {
    ABI28_0_0incrementIndices(mDYIndices_, mDYsIndex_);
...
- (NSNumber*)nextRotation {
    ABI28_0_0incrementIndices(mRIndices_, mRsIndex_);
