...
const CLLocationDegrees ABI28_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI28_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI28_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI28_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI28_0_0AIRMap
{
    UIView *_legalLabel;
    CLLocationManager *_locationManager;
    BOOL _initialRegionSet;

    // Array to manually track ABI28_0_0RN subviews
    //
    // ABI28_0_0AIRMap implicitly creates subviews that aren't regular ABI28_0_0RN children
    // (ABI28_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI28_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ReactABI28_0_0-native/blob/v0.16.0/ReactABI28_0_0/Modules/ABI28_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI28_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ReactABI28_0_0-native/blob/v0.16.0/Libraries/Text/ABI28_0_0RCTTextField.m#L20
    NSMutableArray<UIView *> *_ReactABI28_0_0Subviews;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _ReactABI28_0_0Subviews = [NSMutableArray new];
        ...
        self.calloutView = [ABI28_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI28_0_0AIRMapMaxZoomLevel;
    }
    return self;
}
...
- (void)insertReactABI28_0_0Subview:(id<ABI28_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI28_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI28_0_0AIRMapPolyline class]]) {
        ((ABI28_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI28_0_0AIRMapPolygon class]]) {
        ((ABI28_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI28_0_0AIRMapCircle class]]) {
        ((ABI28_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI28_0_0AIRMapUrlTile class]]) {
        ((ABI28_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI28_0_0AIRMapLocalTile class]]) {
        ((ABI28_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI28_0_0AIRMapOverlay class]]) {
        ((ABI28_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI28_0_0RCTComponent>> *childSubviews = [subview ReactABI28_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertReactABI28_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ReactABI28_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI28_0_0Subview:(id<ABI28_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI28_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI28_0_0RCTComponent>> *childSubviews = [subview ReactABI28_0_0Subviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeReactABI28_0_0Subview:(UIView *)childSubviews[i]];
        }
    }
    [_ReactABI28_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
#pragma mark ABI28_0_0SMCalloutViewDelegate
