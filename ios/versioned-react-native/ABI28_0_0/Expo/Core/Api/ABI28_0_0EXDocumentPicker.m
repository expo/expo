...
#import "ABI28_0_0EXDocumentPicker.h"
...
static NSString * ABI28_0_0EXConvertMimeTypeToUTI(NSString *mimeType)
{
  CFStringRef uti;
...
@interface ABI28_0_0EXDocumentPicker () <UIDocumentMenuDelegate, UIDocumentPickerDelegate>
...
@implementation ABI28_0_0EXDocumentPicker

@synthesize bridge = _bridge;

ABI28_0_0RCT_EXPORT_MODULE(ExponentDocumentPicker)

ABI28_0_0RCT_EXPORT_METHOD(getDocumentAsync:(NSDictionary *)options resolver:(ABI28_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI28_0_0RCTPromiseRejectBlock)reject) {
  _resolve = resolve;
  _reject = reject;

  NSString *type = ABI28_0_0EXConvertMimeTypeToUTI(options[@"type"] ?: @"*/*");

  if (options[@"copyToCacheDirectory"] && [ABI28_0_0RCTConvert BOOL:options[@"copyToCacheDirectory"]] == NO) {
  ...
  if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
    documentMenuVC.popoverPresentationController.sourceRect = CGRectMake(CGRectGetMidX([ABI28_0_0RCTPresentedViewController().view frame]), CGRectGetMaxY([ABI28_0_0RCTPresentedViewController().view frame]), 0, 0);
    documentMenuVC.popoverPresentationController.sourceView = ABI28_0_0RCTPresentedViewController().view;
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url
{
  ...
  if (_shouldCopyToCacheDirectory) {
    ...
    NSString *path = [ABI28_0_0EXFileSystem generatePathInDirectory:directory withExtension:[extension isEqualToString:@""] ? extension : [@"." stringByAppendingString:extension]];
    NSError *error = nil;
