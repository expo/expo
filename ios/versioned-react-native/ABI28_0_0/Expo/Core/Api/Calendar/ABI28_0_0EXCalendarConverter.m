...
@implementation ABI28_0_0EXCalendarConverter
...
+ (NSDictionary *)serializeSource:(EKSource *)source
{
  return @{
       ...
       @"type": [ABI28_0_0EXCalendarConverter _sourceType:source.sourceType],
       @"name": source.title
       };
}
...
+ (NSMutableDictionary *)_serializeCalendar:(EKCalendar *)calendar
{
  ...
  if (calendar.source) {
    serializedCalendar[@"source"] = [ABI28_0_0EXCalendarConverter serializeSource:calendar.source];
  }
  ...
  if (calendar.CGColor) {
    serializedCalendar[@"color"] = [ABI28_0_0EXUtil hexStringWithCGColor:calendar.CGColor];
  }

  if (calendar.type) {
    serializedCalendar[@"type"] = [ABI28_0_0EXCalendarConverter _calendarType:calendar.type];
  }

  serializedCalendar[@"allowsModifications"] = @(calendar.allowsContentModifications);
  serializedCalendar[@"allowedAvailabilities"] = [ABI28_0_0EXCalendarConverter _calendarSupportedAvailabilitiesFromMask:calendar.supportedEventAvailabilities];
...
+ (NSArray *)serializeCalendars:(NSArray *)calendars
{
  ...
  for (EKCalendar *calendar in calendars) {
    [serializedCalendars addObject:[ABI28_0_0EXCalendarConverter _serializeCalendar:calendar]];
  }
...
+ (NSMutableDictionary *)_serializeCalendarItem:(EKCalendarItem *)item withDateFormatter:(NSDateFormatter *)dateFormatter
{
  ...
  if (item.hasRecurrenceRules) {
    ...
    NSString *frequencyType = [ABI28_0_0EXCalendarConverter _recurrenceFrequency:[rule frequency]];
...
+ (NSArray *)serializeCalendarEvents:(NSArray *)calendarEvents
{
  ...
  for (EKEvent *event in calendarEvents) {
    ...
    [serializedCalendarEvents addObject:[ABI28_0_0EXCalendarConverter serializeCalendarEvent:event]];
  }
...
+ (NSDictionary *)serializeCalendarEvent:(EKEvent *)event
{
  ...
  NSMutableDictionary *formedCalendarEvent = [ABI28_0_0EXCalendarConverter _serializeCalendarItem:event withDateFormatter:dateFormatter];
  ...
  if (event.availability) {
    formedCalendarEvent[@"availability"] = [ABI28_0_0EXCalendarConverter _eventAvailability:event.availability];
  }

  if (event.status) {
    formedCalendarEvent[@"status"] = [ABI28_0_0EXCalendarConverter _eventStatus:event.status];
  }

  if (event.organizer) {
    formedCalendarEvent[@"organizer"] = [ABI28_0_0EXCalendarConverter _serializeAttendee:event.organizer];
  }
...
+ (NSArray *)serializeReminders:(NSArray<EKReminder *> *)reminders
{
  ...
  for (EKReminder *reminder in reminders) {
    [serializedReminders addObject:[ABI28_0_0EXCalendarConverter serializeReminder:reminder]];
  }
...
+ (NSDictionary *)serializeReminder:(EKReminder *)reminder
{
  ...
  NSMutableDictionary *formedReminder = [ABI28_0_0EXCalendarConverter _serializeCalendarItem:reminder withDateFormatter:dateFormatter];
...
+ (NSArray *)serializeAttendees:(NSArray<EKParticipant *> *)attendees
{
  ...
  for (EKParticipant *attendee in attendees) {
    [serializedAttendees addObject:[ABI28_0_0EXCalendarConverter _serializeAttendee:attendee]];
  }
...
+ (NSDictionary *)_serializeAttendee:(EKParticipant *)attendee
{
  ...
  if (attendee.participantRole) {
    formedAttendee[@"role"] = [ABI28_0_0EXCalendarConverter _participantRole:attendee.participantRole];
  }

  if (attendee.participantStatus) {
    formedAttendee[@"status"] = [ABI28_0_0EXCalendarConverter _participantStatus:attendee.participantStatus];
  }

  if (attendee.participantType) {
    formedAttendee[@"type"] = [ABI28_0_0EXCalendarConverter _participantType:attendee.participantType];
  }
