...
#include <cxxReactABI28_0_0/ABI28_0_0MethodCall.h>
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI28_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI28_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI28_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
