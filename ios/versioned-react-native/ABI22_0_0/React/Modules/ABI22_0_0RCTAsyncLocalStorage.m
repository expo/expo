...
#import "ABI22_0_0RCTAsyncLocalStorage.h"
...
static NSString *const ABI22_0_0RCTStorageDirectory = @"ABI22_0_0RCTAsyncLocalStorage_V1";
static NSString *const ABI22_0_0RCTManifestFileName = @"manifest.json";
static const NSUInteger ABI22_0_0RCTInlineValueThreshold = 1024;

#pragma mark - Static helper functions

static NSDictionary *ABI22_0_0RCTErrorForKey(NSString *key)
{
  if (![key isKindOfClass:[NSString class]]) {
    return ABI22_0_0RCTMakeAndLogError(@"Invalid key - must be a string.  Key: ", key, @{@"key": key});
  } else if (key.length < 1) {
    return ABI22_0_0RCTMakeAndLogError(@"Invalid key - must be at least one character.  Key: ", key, @{@"key": key});
  } else {
    return nil;
  }
}
...
static BOOL ABI22_0_0RCTMergeRecursive(NSMutableDictionary *destination, NSDictionary *source)
{
  BOOL modified = NO;
  for (NSString *key in source) {
    ...
    if ([sourceValue isKindOfClass:[NSDictionary class]]) {
      if ([destinationValue isKindOfClass:[NSDictionary class]]) {
        ...
        if (ABI22_0_0RCTMergeRecursive(destinationValue, sourceValue)) {
...
//   ABI22_0_0RCTGetStorageDirectory()
//   ABI22_0_0RCTGetManifestFilePath()
//   ABI22_0_0RCTGetMethodQueue()
//   ABI22_0_0RCTGetCache()
//   ABI22_0_0RCTDeleteStorageDirectory()

#define ABI22_0_0RCTGetStorageDirectory() _storageDirectory
#define ABI22_0_0RCTGetManifestFilePath() _manifestFilePath
#define ABI22_0_0RCTGetMethodQueue() self.methodQueue
#define ABI22_0_0RCTGetCache() self.cache

static NSDictionary *ABI22_0_0RCTDeleteStorageDirectory(NSString *storageDirectory)
{
  NSError *error;
  [[NSFileManager defaultManager] removeItemAtPath:storageDirectory error:&error];
  return error ? ABI22_0_0RCTMakeError(@"Failed to delete storage directory.", error, nil) : nil;
}
#define ABI22_0_0RCTDeleteStorageDirectory() ABI22_0_0RCTDeleteStorageDirectory(_storageDirectory)

#pragma mark - ABI22_0_0RCTAsyncLocalStorage

@interface ABI22_0_0RCTAsyncLocalStorage ()
...
@implementation ABI22_0_0RCTAsyncLocalStorage
{
  BOOL _haveSetup;
...
+ (NSString *)moduleName { return @"ABI22_0_0RCTAsyncLocalStorage"; }
- (instancetype)initWithStorageDirectory:(NSString *)storageDirectory
{
  if ((self = [super init])) {
    _storageDirectory = storageDirectory;
    _manifestFilePath = [ABI22_0_0RCTGetStorageDirectory() stringByAppendingPathComponent:ABI22_0_0RCTManifestFileName];
  }
  return self;
}
...
- (void)clearAllData
{
  dispatch_async(ABI22_0_0RCTGetMethodQueue(), ^{
    [self->_manifest removeAllObjects];
    [ABI22_0_0RCTGetCache() removeAllObjects];
    ABI22_0_0RCTDeleteStorageDirectory();
  });
}
...
{
  NSError *error;
  NSString *serialized = ABI22_0_0RCTJSONStringify(_manifest, &error);
  [serialized writeToFile:ABI22_0_0RCTGetManifestFilePath() atomically:YES encoding:NSUTF8StringEncoding error:&error];
  NSDictionary *errorOut;
  if (error) {
    errorOut = ABI22_0_0RCTMakeError(@"Failed to write manifest file.", error, nil);
    ABI22_0_0RCTAppendError(errorOut, errors);
  }
  return errorOut;
}
...
{
  NSDictionary *errorOut = ABI22_0_0RCTErrorForKey(key);
  if (errorOut) {
    return errorOut;
  }
  NSString *value = [self _getValueForKey:key errorOut:&errorOut];
  [result addObject:@[key, ABI22_0_0RCTNullIfNil(value)]]; // Insert null if missing or failure.
  return errorOut;
}
...
{
  if (entry.count != 2) {
    return ABI22_0_0RCTMakeAndLogError(@"Entries must be arrays of the form [key: string, value: string], got: ", entry, nil);
  }
  ...
  [ABI22_0_0RCTGetCache() setObject:value forKey:key cost:value.length];
  if (error) {
    errorOut = ABI22_0_0RCTMakeError(@"Failed to write value.", error, @{@"key": key});
...
ABI22_0_0RCT_EXPORT_METHOD(multiSet:(NSArray<NSArray<NSString *> *> *)kvPairs
                  callback:(ABI22_0_0RCTResponseSenderBlock)callback)
{
  NSDictionary *errorOut = [self _ensureSetup];
  if (errorOut) {
  ...
  callback(@[ABI22_0_0RCTNullIfNil(errors)]);
}

ABI22_0_0RCT_EXPORT_METHOD(multiMerge:(NSArray<NSArray<NSString *> *> *)kvPairs
                    callback:(ABI22_0_0RCTResponseSenderBlock)callback)
{
  NSDictionary *errorOut = [self _ensureSetup];
  if (errorOut) {
  ...
  for (__strong NSArray<NSString *> *entry in kvPairs) {
    ...
    if (!keyError) {
      if (value) {
        NSError *jsonError;
        NSMutableDictionary *mergedVal = ABI22_0_0RCTJSONParseMutable(value, &jsonError);
        if (ABI22_0_0RCTMergeRecursive(mergedVal, ABI22_0_0RCTJSONParse(entry[1], &jsonError))) {
          entry = @[entry[0], ABI22_0_0RCTNullIfNil(ABI22_0_0RCTJSONStringify(mergedVal, NULL))];
        }
        if (jsonError) {
          keyError = ABI22_0_0RCTJSErrorFromNSError(jsonError);
        }
      }
      if (!keyError) {
...
ABI22_0_0RCT_EXPORT_METHOD(multiRemove:(NSArray<NSString *> *)keys
                  callback:(ABI22_0_0RCTResponseSenderBlock)callback)
{
  NSDictionary *errorOut = [self _ensureSetup];
  if (errorOut) {
  ...
  BOOL changedManifest = NO;
  for (NSString *key in keys) {
    NSDictionary *keyError = ABI22_0_0RCTErrorForKey(key);
    if (!keyError) {
      if (_manifest[key] == (id)kCFNull) {
        NSString *filePath = [self _filePathForKey:key];
        [[NSFileManager defaultManager] removeItemAtPath:filePath error:nil];
...
ABI22_0_0RCT_EXPORT_METHOD(clear:(ABI22_0_0RCTResponseSenderBlock)callback)
{
  [_manifest removeAllObjects];
  [ABI22_0_0RCTGetCache() removeAllObjects];
  NSDictionary *error = ABI22_0_0RCTDeleteStorageDirectory();
  callback(@[ABI22_0_0RCTNullIfNil(error)]);
}
