...
#import "ABI22_0_0RCTErrorInfo.h"
#import "ABI22_0_0RCTJSStackFrame.h"
#import "ABI22_0_0RCTUtils.h"

#if ABI22_0_0RCT_DEV

@class ABI22_0_0RCTRedBoxWindow;

@protocol ABI22_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI22_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI22_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI22_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI22_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI22_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI22_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI22_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI22_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI22_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI22_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI22_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI22_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI22_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI22_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI22_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI22_0_0RCTKeyCommands for this, but since
...
@interface ABI22_0_0RCTRedBox () <ABI22_0_0RCTInvalidating, ABI22_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI22_0_0RCTRedBox
{
  ABI22_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI22_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI22_0_0RCTErrorInfo *)_customizeError:(ABI22_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI22_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI22_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI22_0_0RCTJSStackTraceKey]];
}
...
{
  NSArray<ABI22_0_0RCTJSStackFrame *> *stack = [ABI22_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI22_0_0RCTJSStackFrame class]]) {
    stack = [ABI22_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI22_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI22_0_0RCTErrorInfo *errorInfo = [[ABI22_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI22_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI22_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI22_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI22_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI22_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI22_0_0RCTRedBoxWindow *)redBoxWindow
{
  [_bridge reload];
  [self dismiss];
}
...
@implementation ABI22_0_0RCTBridge (ABI22_0_0RCTRedBox)

- (ABI22_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI22_0_0RCTRedBox class]];
}
...
- (ABI22_0_0RCTRedBox *)redBox { return nil; }
