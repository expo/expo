...
NSString *const ABI22_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI22_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI22_0_0RCTAccessibilityManager ()
...
@implementation ABI22_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    self.contentSizeCategory = ABI22_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI22_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI22_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI22_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI22_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI22_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI22_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI22_0_0Tag:ReactABI22_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI22_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI22_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI22_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI22_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI22_0_0RCTResponseSenderBlock)callback
...
@implementation ABI22_0_0RCTBridge (ABI22_0_0RCTAccessibilityManager)

- (ABI22_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI22_0_0RCTAccessibilityManager class]];
}
