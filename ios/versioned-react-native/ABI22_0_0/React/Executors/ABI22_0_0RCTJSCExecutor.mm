...
#import <cxxReactABI22_0_0/ABI22_0_0JSBundleType.h>
...
#import <ReactABI22_0_0/ABI22_0_0RCTJSCErrorHandling.h>
#import <ReactABI22_0_0/ABI22_0_0RCTJavaScriptLoader.h>
#import <ReactABI22_0_0/ABI22_0_0RCTLog.h>
#import <ReactABI22_0_0/ABI22_0_0RCTPerformanceLogger.h>
#import <ReactABI22_0_0/ABI22_0_0RCTProfile.h>
#import <ReactABI22_0_0/ABI22_0_0RCTUtils.h>

#if (ABI22_0_0RCT_PROFILE || ABI22_0_0RCT_DEV) && __has_include("ABI22_0_0RCTDevMenu.h")
#import "ABI22_0_0RCTDevMenu.h"
#endif

NSString *const ABI22_0_0RCTJSCThreadName = @"com.facebook.ReactABI22_0_0.JavaScript";
NSString *const ABI22_0_0RCTJavaScriptContextCreatedNotification = @"ABI22_0_0RCTJavaScriptContextCreatedNotification";
...
struct TaggedScript {
  const facebook::ReactABI22_0_0::ScriptTag tag;
  const NSData *script;
};

#if ABI22_0_0RCT_PROFILE
@interface ABI22_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI22_0_0RCTCookieMap @end
#endif

struct ABI22_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
};

@interface ABI22_0_0RCTJavaScriptContext : NSObject <ABI22_0_0RCTInvalidating>
...
@implementation ABI22_0_0RCTJavaScriptContext
{
  ABI22_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    _context = context;
    _context.name = @"ABI22_0_0RCTJSContext";
    _javaScriptThread = javaScriptThread;

    /**
     * Explicitly introduce a retain cycle here - The ABI22_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI22_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI22_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI22_0_0RCTPerformanceLogger *_performanceLogger;
  ABI22_0_0RCTJavaScriptContext *_context;
...
+ (void)runRunLoopThread
{
  @autoreleasepool {
    ...
    while (kCFRunLoopRunStopped != CFRunLoopRunInMode(kCFRunLoopDefaultMode, ((NSDate *)[NSDate distantFuture]).timeIntervalSinceReferenceDate, NO)) {
      ABI22_0_0RCTAssert(NO, @"not reached assertion"); // runloop spun. that's bad.
    }
  }
}
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI22_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI22_0_0RCTJSCThreadName;
...
{
  ABI22_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI22_0_0RCTJSCExecutor init]", nil);
...
- (instancetype)initWithJSContextData:(const ABI22_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    _useCustomJSCLibrary = data.useCustomJSCLibrary;
    _valid = YES;
    _javaScriptThread = data.javaScriptThread;
    _context = [[ABI22_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
                                         ...
                                         sourceURL:(NSURL *)sourceURL
{
  ...
  if (taggedScript.tag == facebook::ReactABI22_0_0::ScriptTag::RAMBundle) {
...
- (ABI22_0_0RCTJavaScriptContext *)context
{
  if (!self.isValid) {
    return nil;
  }
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI22_0_0RCTProfileDidStartProfiling, ABI22_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_context) {
      ...
      if (self->_useCustomJSCLibrary) {
        JSC_configureJSCForIOS(true, ABI22_0_0RCTJSONStringify(@{
      ...
      self->_context = [[ABI22_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI22_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];

      installBasicSynchronousHooksOnContext(context);
    }

    ABI22_0_0RCTFBQuickPerformanceLoggerConfigureHooks(context.JSGlobalContextRef);

    __weak ABI22_0_0RCTJSCExecutor *weakSelf = self;
    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI22_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI22_0_0RCT_PROFILE_END_EVENT(ABI22_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI22_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI22_0_0RCT_PROFILE
    __weak ABI22_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI22_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI22_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI22_0_0RCTProfileIsProfiling()) {
        ...
        _ABI22_0_0RCTProfileEndFlowEvent(newCookie);
...
#if __has_include("ABI22_0_0RCTDevMenu.h")
      // Allow to toggle the sampling profiler through RN's dev menu
      [self->_bridge.devMenu addItem:[ABI22_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
        ABI22_0_0RCTJSCExecutor *strongSelf = weakSelf;
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI22_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ABI22_0_0RCTLogLevel level = ABI22_0_0RCTLogLevelInfo;
    if (logLevel) {
      level = MAX(level, (ABI22_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI22_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI22_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI22_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI22_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI22_0_0RCTCookieMap *cookieMap = [ABI22_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI22_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI22_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI22_0_0RCTProfileDidStartProfiling];
...
- (int32_t)bytecodeFileFormatVersion
{
  return _useCustomJSCLibrary
    ? facebook::ReactABI22_0_0::customJSCWrapper()->JSBytecodeFileFormatVersion
...
ABI22_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)contextName)
{
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI22_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI22_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI22_0_0RCTJavaScriptCallback)onComplete
{
  ABI22_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI22_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI22_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, batchedBridgeRef) != kJSTypeUndefined) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, methodJSRef) != kJSTypeUndefined) {
        ...
        if (!errorJSRef && JSC_JSValueGetType(ctx, methodJSRef) == kJSTypeUndefined) {
          error = ABI22_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSC_JSValueGetType(ctx, batchedBridgeRef) == kJSTypeUndefined) {
        error = ABI22_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined. This can happen "
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI22_0_0RCTNSErrorFromJSError([JSC_JSValue(ctx) valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI22_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
{
  ABI22_0_0RCTAssertParam(script);
  ABI22_0_0RCTAssertParam(sourceURL);
...
static TaggedScript loadTaggedScript(NSData *script,
                                     NSURL *sourceURL,
                                     ABI22_0_0RCTPerformanceLogger *performanceLogger,
                                     RandomAccessBundleData &randomAccessBundle,
                                     NSError **error)
{
  ABI22_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);

  facebook::ReactABI22_0_0::BundleHeader header;
  [script getBytes:&header length:sizeof(header)];
  facebook::ReactABI22_0_0::ScriptTag tag = facebook::ReactABI22_0_0::parseTypeFromHeader(header);

  NSData *loadedScript = NULL;
  switch (tag) {
    case facebook::ReactABI22_0_0::ScriptTag::RAMBundle:
      [performanceLogger markStartForTag:ABI22_0_0RCTPLRAMBundleLoad];

      loadedScript = loadRAMBundle(sourceURL, error, randomAccessBundle);

      [performanceLogger markStopForTag:ABI22_0_0RCTPLRAMBundleLoad];
      [performanceLogger setValue:loadedScript.length forTag:ABI22_0_0RCTPLRAMStartupCodeSize];
      break;

    case facebook::ReactABI22_0_0::ScriptTag::BCBundle:
      loadedScript = script;
      break;

    case facebook::ReactABI22_0_0::ScriptTag::String: {
...
static void registerNativeRequire(JSContext *context, ABI22_0_0RCTJSCExecutor *executor)
{
  __weak ABI22_0_0RCTJSCExecutor *weakExecutor = executor;
...
{
  ABI22_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(taggedScript.script.length)
  }));

  [performanceLogger markStartForTag:ABI22_0_0RCTPLScriptExecution];
  ...
  [performanceLogger markStopForTag:ABI22_0_0RCTPLScriptExecution];

  NSError *error = jsError
    ? ABI22_0_0RCTNSErrorFromJSErrorRef(jsError, ctx)
    : nil;
              ...
              callback:(ABI22_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI22_0_0RCT_DEBUG) {
    ABI22_0_0RCTAssert(ABI22_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    JSGlobalContextRef ctx = strongSelf->_context.context.JSGlobalContextRef;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI22_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI22_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI22_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
      }
    }
    ABI22_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI22_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI22_0_0RCTFatal(ABI22_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    NSError *error = ABI22_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI22_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI22_0_0RCTPLRAMNativeRequires];
  ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI22_0_0RCT_PROFILE_END_EVENT(ABI22_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI22_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI22_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI22_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
@implementation ABI22_0_0RCTJSContextProvider
{
...
- (ABI22_0_0RCTJSContextData)data
{
...
- (ABI22_0_0RCTJSCExecutor *)createExecutorWithContext:(JSContext **)JSContext
{
  const ABI22_0_0RCTJSContextData data = self.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  return [[ABI22_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
}
