...
#import <ReactABI22_0_0/ABI22_0_0RCTReconnectingWebSocket.h>
...
@interface ABI22_0_0RCTPackagerConnection () <ABI22_0_0RCTWebSocketProtocolDelegate>
@end

@implementation ABI22_0_0RCTPackagerConnection {
  NSURL *_packagerURL;
  ABI22_0_0RCTReconnectingWebSocket *_socket;
  NSMutableDictionary<NSString *, id<ABI22_0_0RCTPackagerClientMethod>> *_handlers;
}

+ (instancetype)connectionForBridge:(ABI22_0_0RCTBridge *)bridge
{
  ABI22_0_0RCTPackagerConnectionBridgeConfig *config = [[ABI22_0_0RCTPackagerConnectionBridgeConfig alloc] initWithBridge:bridge];
...
- (void)connect
{
  ...
  static NSMutableDictionary<NSString *, ABI22_0_0RCTReconnectingWebSocket *> *socketConnections = nil;
  if (socketConnections == nil) {
    socketConnections = [NSMutableDictionary new];
  }

  NSString *key = [url absoluteString];
  ABI22_0_0RCTReconnectingWebSocket *webSocket = socketConnections[key];
  if (!webSocket) {
    webSocket = [[ABI22_0_0RCTReconnectingWebSocket alloc] initWithURL:url];
    [webSocket start];
...
{
  NSArray<NSNumber *> *const kSupportedVersions = @[ @(ABI22_0_0RCT_PACKAGER_CLIENT_PROTOCOL_VERSION) ];
  return [kSupportedVersions containsObject:version];
}

#pragma mark - ABI22_0_0RCTWebSocketProtocolDelegate
...
- (void)webSocket:(ABI22_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)message
{
  ...
  if (error) {
    ABI22_0_0RCTLogError(@"%@ failed to parse message with error %@\n<message>\n%@\n</message>", [self class], error, msg);
    return;
  }

  if (!isSupportedVersion(msg[@"version"])) {
    ABI22_0_0RCTLogError(@"%@ received message with not supported version %@", [self class], msg[@"version"]);
    return;
  }

  id<ABI22_0_0RCTPackagerClientMethod> methodHandler = _handlers[msg[@"method"]];
  if (!methodHandler) {
    if (msg[@"id"]) {
      NSString *errorMsg = [NSString stringWithFormat:@"%@ no handler found for method %@", [self class], msg[@"method"]];
      ABI22_0_0RCTLogError(errorMsg, msg[@"method"]);
      [[[ABI22_0_0RCTPackagerClientResponder alloc] initWithId:msg[@"id"]
  ...
  if (msg[@"id"]) {
    [methodHandler handleRequest:msg[@"params"]
                   withResponder:[[ABI22_0_0RCTPackagerClientResponder alloc] initWithId:msg[@"id"]
                                                                         socket:webSocket]];
  } else {
