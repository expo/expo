...
const NSInteger ABI22_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI22_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI22_0_0RCTGetEventID(id<ABI22_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI22_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI22_0_0RCTEvent>> *_events;
  ...
  NSMutableArray<id<ABI22_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI22_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI22_0_0RCT_DEBUG) {
    ABI22_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI22_0_0 tag");
  }

  name = ABI22_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI22_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI22_0_0RCTEvent>)event
{
  ...
  for (id<ABI22_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI22_0_0RCTGetEventID(event);

  id<ABI22_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI22_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
- (void)dispatchEvent:(id<ABI22_0_0RCTEvent>)event
{
...
@implementation ABI22_0_0RCTBridge (ABI22_0_0RCTEventDispatcher)

- (ABI22_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI22_0_0RCTEventDispatcher class]];
}
