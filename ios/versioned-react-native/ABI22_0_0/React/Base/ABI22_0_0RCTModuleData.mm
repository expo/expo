...
@implementation ABI22_0_0RCTModuleData
{
  NSDictionary<NSString *, id> *_constantsToExport;
  NSString *_queueName;
  __weak ABI22_0_0RCTBridge *_bridge;
  ABI22_0_0RCTBridgeModuleProvider _moduleProvider;
...
- (void)setUp
{
  ...
  if (implementsRequireMainQueueSetup) {
    ...
    if (_requiresMainQueueSetup) {
      ...
      ABI22_0_0RCTLogWarn(@"Module %@ requires main queue setup since it overrides `%s` but doesn't implement "
        "`requiresMainQueueSetup. In a future release ReactABI22_0_0 Native will default to initializing all native modules "
                             ...
                             bridge:(ABI22_0_0RCTBridge *)bridge
{
  return [self initWithModuleClass:moduleClass
                    moduleProvider:^id<ABI22_0_0RCTBridgeModule>{ return [moduleClass new]; }
                            bridge:bridge];
}

- (instancetype)initWithModuleClass:(Class)moduleClass
                     moduleProvider:(ABI22_0_0RCTBridgeModuleProvider)moduleProvider
                             bridge:(ABI22_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
...
- (instancetype)initWithModuleInstance:(id<ABI22_0_0RCTBridgeModule>)instance
                                bridge:(ABI22_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
...
{
  ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"[ABI22_0_0RCTModuleData setUpInstanceAndBridge]", @{
  ...
  });
  {
    ...
    if (!_setupComplete && _bridge.valid) {
      if (!_instance) {
        if (ABI22_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI22_0_0RCTAssertMainQueue();
        }
        ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"[ABI22_0_0RCTModuleData setUpInstanceAndBridge] Create module", nil);
        ...
        if (!_instance) {
          ...
          _setupComplete = YES;
          ABI22_0_0RCTLogWarn(@"The module %@ is returning nil from its constructor. You "
                     "may need to instantiate it yourself and pass it into the "
                     "bridge.", _moduleClass);
        }
      }

      if (_instance && ABI22_0_0RCTProfileIsProfiling()) {
        ABI22_0_0RCTProfileHookInstance(_instance);
      }
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"[ABI22_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
      [(id)_instance setValue:_bridge forKey:@"bridge"];
    }
    @catch (NSException *exception) {
      ABI22_0_0RCTLogError(@"%@ has no setter or ivar for its bridge, which is not "
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"[ABI22_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI22_0_0RCTDidInitializeModuleNotification
                                                        object:_bridge
                                                      userInfo:@{@"module": _instance, @"bridge": ABI22_0_0RCTNullIfNil(_bridge.parentBridge)}];
    ABI22_0_0RCT_PROFILE_END_EVENT(ABI22_0_0RCTProfileTagAlways, @"");
  }
}
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"[ABI22_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ReactABI22_0_0.%@Queue", self.name];
      ...
      if (implementsMethodQueue) {
        ...
        @catch (NSException *exception) {
          ABI22_0_0RCTLogError(@"%@ is returning nil for its methodQueue, which is not "
...
- (id<ABI22_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI22_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, @"[ABI22_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI22_0_0RCTIsMainQueue()) {
        ABI22_0_0RCTLogWarn(@"ABI22_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI22_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI22_0_0RCTBridgeMethod>> *)methods
{
  if (!_methods) {
    NSMutableArray<id<ABI22_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
    ...
    while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
      ...
      for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
          ...
          auto exportedMethod = ((const ABI22_0_0RCTMethodInfo *(*)(id, SEL))imp)(_moduleClass, selector);
          id<ABI22_0_0RCTBridgeMethod> moduleMethod = [[ABI22_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod
...
- (void)gatherConstants
{
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI22_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    if (_requiresMainQueueSetup) {
      if (!ABI22_0_0RCTIsMainQueue()) {
        ABI22_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
      }
...
// TODO 10487027: this method can go once ABI22_0_0RCTBatchedBridge is gone
- (NSArray *)config
{
  ...
  ABI22_0_0RCT_PROFILE_BEGIN_EVENT(ABI22_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI22_0_0RCTModuleData config] %@", _moduleClass]), nil);
  ...
  for (id<ABI22_0_0RCTBridgeMethod> method in self.methods) {
    if (method.functionType == ABI22_0_0RCTFunctionTypePromise) {
      if (!promiseMethods) {
        promiseMethods = [NSMutableArray new];
      }
      [promiseMethods addObject:@(methods.count)];
    }
    else if (method.functionType == ABI22_0_0RCTFunctionTypeSync) {
      if (!syncMethods) {
  ...
  NSArray *config = @[
    self.name,
    ABI22_0_0RCTNullIfNil(constants),
    ABI22_0_0RCTNullIfNil(methods),
    ABI22_0_0RCTNullIfNil(promiseMethods),
    ABI22_0_0RCTNullIfNil(syncMethods)
  ];
  ABI22_0_0RCT_PROFILE_END_EVENT(ABI22_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI22_0_0RCTModuleData config] %@", _moduleClass]));
  return config;
}
...
{
  (void)[self instance];
  ABI22_0_0RCTAssert(_methodQueue != nullptr, @"Module %@ has no methodQueue (instance: %@, bridge.valid: %d)",
