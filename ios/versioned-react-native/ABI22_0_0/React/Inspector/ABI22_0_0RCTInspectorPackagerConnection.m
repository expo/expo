...
// ReactABI22_0_0Android/src/main/java/com/facebook/ReactABI22_0_0/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int RECONNECT_DELAY_MS = 2000;

@interface ABI22_0_0RCTInspectorPackagerConnection () <ABI22_0_0RCTSRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI22_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  ABI22_0_0RCTSRWebSocket *_webSocket;
...
@interface ABI22_0_0RCTInspectorRemoteConnection () {
  __weak ABI22_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI22_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI22_0_0RCTInspectorPackagerConnection
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI22_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  if (_inspectorConnections[pageId]) {
    ...
    ABI22_0_0RCTLogError(@"Already connected: %@", pageId);
    return;
  }

  ABI22_0_0RCTInspectorRemoteConnection *remoteConnection =
    [[ABI22_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self
                                                              pageId:pageId];

  ABI22_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI22_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI22_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (void)handleWrappedEvent:(NSDictionary *)payload
{
  ...
  ABI22_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
  if (!inspectorConnection) {
    ABI22_0_0RCTLogError(@"Not connected: %@", pageId);
    return;
  }
...
- (NSArray *)pages
{
  NSArray<ABI22_0_0RCTInspectorPage *> *pages = [ABI22_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];
  for (ABI22_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(ABI22_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  // warn but don't die on unrecognized messages
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI22_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI22_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI22_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@",
      messageText);
    return;
  }
...
- (void)webSocket:(ABI22_0_0RCTSRWebSocket *)webSocket didCloseWithCode:(NSInteger)code
...
{
  if (_closed) {
    ABI22_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresopnding android code has a lot of custom config options for
  // timeouts, but it appears the iOS ABI22_0_0RCTSRWebSocket API doesn't have the same
  // implemented options.
  _webSocket = [[ABI22_0_0RCTSRWebSocket alloc] initWithURL:_url];
...
{
  if (_closed) {
    ABI22_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI22_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI22_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(
    dispatch_time(DISPATCH_TIME_NOW, RECONNECT_DELAY_MS *NSEC_PER_MSEC),
    dispatch_get_main_queue(), ^{
      ABI22_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI22_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI22_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
...
{
  ABI22_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
...
@implementation ABI22_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI22_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
