...
#import "ABI22_0_0RCTCxxModule.h"
...
@implementation ABI22_0_0RCTCxxModule
{
...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI22_0_0RCTAssert([ABI22_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
                @"CxxModule class name %@ does not match runtime name %s",
                ABI22_0_0RCTBridgeModuleNameForClass([self class]), _module->getName().c_str());
    }
  }
}
...
{
  ABI22_0_0RCTAssert(NO, @"Subclass %@ must override createModule", [self class]);
  return nullptr;
}

- (NSArray<id<ABI22_0_0RCTBridgeMethod>> *)methodsToExport;
{
  [self lazyInit];
  if (!_module) {
    return nil;
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI22_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
