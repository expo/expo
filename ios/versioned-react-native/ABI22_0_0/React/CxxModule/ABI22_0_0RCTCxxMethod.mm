...
#import "ABI22_0_0RCTCxxMethod.h"
...
#import <cxxReactABI22_0_0/ABI22_0_0JsArgumentHelpers.h>
#import <folly/Memory.h>

#import "ABI22_0_0RCTCxxUtils.h"

using facebook::xplat::module::CxxModule;
using namespace facebook::ReactABI22_0_0;

@implementation ABI22_0_0RCTCxxMethod
{
...
- (ABI22_0_0RCTFunctionType)functionType
{
  std::string type(_method->getType());
  if (type == "sync") {
    return ABI22_0_0RCTFunctionTypeSync;
  } else if (type == "async") {
    return ABI22_0_0RCTFunctionTypeNormal;
  } else {
    return ABI22_0_0RCTFunctionTypePromise;
  }
}
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI22_0_0RCTLogError(@"Method %@.%s expects at least %zu arguments, but got %tu",
                ABI22_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI22_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
  ...
  try {
    ...
    ABI22_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI22_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
