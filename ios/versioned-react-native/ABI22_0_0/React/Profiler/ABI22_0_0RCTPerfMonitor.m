...
#import "ABI22_0_0RCTFPSGraph.h"
#import "ABI22_0_0RCTInvalidating.h"
#import "ABI22_0_0RCTJavaScriptExecutor.h"
...
static NSString *const ABI22_0_0RCTPerfMonitorCellIdentifier = @"ABI22_0_0RCTPerfMonitorCellIdentifier";

static CGFloat const ABI22_0_0RCTPerfMonitorBarHeight = 50;
static CGFloat const ABI22_0_0RCTPerfMonitorExpandHeight = 250;

typedef BOOL (*ABI22_0_0RCTJSCSetOptionType)(const char *);

static BOOL ABI22_0_0RCTJSCSetOption(const char *option)
{
  static ABI22_0_0RCTJSCSetOptionType setOption;
  ...
  dispatch_once(&onceToken, ^{
    ...
    if (ABI22_0_0RCT_DEBUG && setOption == NULL) {
      ABI22_0_0RCTLogWarn(@"The symbol used to enable JSC runtime options is not available in this iOS version");
    }
  });
...
static vm_size_t ABI22_0_0RCTGetResidentMemorySize(void)
{
...
@interface ABI22_0_0RCTPerfMonitor : NSObject <ABI22_0_0RCTBridgeModule, ABI22_0_0RCTInvalidating, UITableViewDataSource, UITableViewDelegate>

#if __has_include("ABI22_0_0RCTDevMenu.h")
@property (nonatomic, strong, readonly) ABI22_0_0RCTDevMenuItem *devMenuItem;
#endif
...
@property (nonatomic, strong, readonly) ABI22_0_0RCTFPSGraph *jsGraph;
@property (nonatomic, strong, readonly) ABI22_0_0RCTFPSGraph *uiGraph;
@property (nonatomic, strong, readonly) UILabel *jsGraphLabel;
@property (nonatomic, strong, readonly) UILabel *uiGraphLabel;

@end

@implementation ABI22_0_0RCTPerfMonitor {
#if __has_include("ABI22_0_0RCTDevMenu.h")
  ABI22_0_0RCTDevMenuItem *_devMenuItem;
#endif
  ...
  ABI22_0_0RCTFPSGraph *_uiGraph;
  ABI22_0_0RCTFPSGraph *_jsGraph;
...
#if __has_include("ABI22_0_0RCTDevMenu.h")
- (ABI22_0_0RCTDevMenuItem *)devMenuItem
{
  if (!_devMenuItem) {
    __weak __typeof__(self) weakSelf = self;
    __weak ABI22_0_0RCTDevSettings *devSettings = self.bridge.devSettings;
    _devMenuItem =
    [ABI22_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
...
{
  if (!_container) {
    _container = [[UIView alloc] initWithFrame:CGRectMake(10, 25, 180, ABI22_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_memory) {
    _memory = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 44, ABI22_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_heap) {
    _heap = [[UILabel alloc] initWithFrame:CGRectMake(44, 0, 44, ABI22_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_views) {
    _views = [[UILabel alloc] initWithFrame:CGRectMake(88, 0, 44, ABI22_0_0RCTPerfMonitorBarHeight)];
...
- (ABI22_0_0RCTFPSGraph *)uiGraph
{
  if (!_uiGraph) {
    _uiGraph = [[ABI22_0_0RCTFPSGraph alloc] initWithFrame:CGRectMake(134, 14, 40, 30)
                                            color:[UIColor lightGrayColor]];
  }
  return _uiGraph;
}

- (ABI22_0_0RCTFPSGraph *)jsGraph
{
  if (!_jsGraph) {
    _jsGraph = [[ABI22_0_0RCTFPSGraph alloc] initWithFrame:CGRectMake(178, 14, 40, 30)
                                            color:[UIColor lightGrayColor]];
  }
  return _jsGraph;
}
...
- (UITableView *)metrics
{
  if (!_metrics) {
    _metrics = [[UITableView alloc] initWithFrame:CGRectMake(
      0,
      ABI22_0_0RCTPerfMonitorBarHeight,
      self.container.frame.size.width,
      self.container.frame.size.height - ABI22_0_0RCTPerfMonitorBarHeight
    )];
    ...
    [_metrics registerClass:[UITableViewCell class] forCellReuseIdentifier:ABI22_0_0RCTPerfMonitorCellIdentifier];
  }
...
- (void)show
{
  ...
  ABI22_0_0RCTJSCSetOption("logGC=1");

  [self updateStats];

  UIWindow *window = ABI22_0_0RCTSharedApplication().delegate.window;
...
- (void)hide
{
  ...
  ABI22_0_0RCTJSCSetOption("logGC=0");
...
- (void)redirectLogs
{
  ...
  __weak __typeof__(self) weakSelf = self;
  _queue = dispatch_queue_create("com.facebook.ReactABI22_0_0.ABI22_0_0RCTPerfMonitor", DISPATCH_QUEUE_SERIAL);
...
- (void)updateStats
{
  ...
  double mem = (double)ABI22_0_0RCTGetResidentMemorySize() / 1024 / 1024;
...
- (void)tap
{
  ...
  if (CGRectIsEmpty(_storedMonitorFrame)) {
    _storedMonitorFrame = CGRectMake(0, 20, self.container.window.frame.size.width, ABI22_0_0RCTPerfMonitorExpandHeight);
...
{
  ABI22_0_0RCTFPSGraph *graph = displayLink == _jsDisplayLink ? _jsGraph : _uiGraph;
...
- (void)loadPerformanceLoggerData
{
  ...
  ABI22_0_0RCTPerformanceLogger *performanceLogger = [_bridge performanceLogger];
...
{
  UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ABI22_0_0RCTPerfMonitorCellIdentifier
                                                          forIndexPath:indexPath];

  if (!cell) {
    cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                  reuseIdentifier:ABI22_0_0RCTPerfMonitorCellIdentifier];
  }
