...
@interface ABI22_0_0RCTScrollEvent : NSObject <ABI22_0_0RCTEvent>
...
@implementation ABI22_0_0RCTScrollEvent
{
...
{
  ABI22_0_0RCTAssertParam(ReactABI22_0_0Tag);
...
- (ABI22_0_0RCTScrollEvent *)coalesceWithEvent:(ABI22_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI22_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI22_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI22_0_0RCTRefreshControl *rctRefreshControl;
...
@implementation ABI22_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI22_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI22_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI22_0_0RCTUIManager JSResponder] isKindOfClass:[ABI22_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI22_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI22_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI22_0_0RCTScrollView
{
  ABI22_0_0RCTEventDispatcher *_eventDispatcher;
  ABI22_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI22_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI22_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI22_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI22_0_0RCTApplyTranformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI22_0_0LayoutDirection:layoutDirection];

  ABI22_0_0RCTApplyTranformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI22_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI22_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI22_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI22_0_0RCTAssert(_contentView == nil, @"ABI22_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI22_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, self.ReactABI22_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI22_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI22_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI22_0_0Subview:atIndex:`
}
...
- (void)layoutSubviews
{
  ...
  ABI22_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI22_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  ABI22_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI22_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI22_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI22_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI22_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI22_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI22_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI22_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI22_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI22_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI22_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI22_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI22_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI22_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI22_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI22_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI22_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI22_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI22_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI22_0_0RCTView contentInsetsForView:self];
...
#define ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI22_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI22_0_0RCTScrollEvent *scrollEvent = [[ABI22_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI22_0_0RCTEventDispatcher (ABI22_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI22_0_0Tag
{
  ...
  ABI22_0_0RCTScrollEvent *fakeScrollEvent = [[ABI22_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI22_0_0Tag:ReactABI22_0_0Tag
