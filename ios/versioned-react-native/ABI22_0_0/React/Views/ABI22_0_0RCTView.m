...
@implementation UIView (ABI22_0_0RCTViewUnmounting)

- (void)ReactABI22_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI22_0_0_remountAllSubviews];
  }
}

- (void)ReactABI22_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI22_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI22_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI22_0_0RootView]) {
      break;
    }
...
static NSString *ABI22_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI22_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI22_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI22_0_0RCTBorderStyleSolid;
...
{
  _ReactABI22_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI22_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI22_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI22_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI22_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI22_0_0RCTPointerEventsNone || _pointerEvents == ABI22_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ReactABI22_0_0ZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI22_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI22_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI22_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
{
  if (self.ReactABI22_0_0AccessibilityElement == self) {
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI22_0_0Tag: %@;", self.ReactABI22_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI22_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI22_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ReactABI22_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI22_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI22_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI22_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI22_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.ReactABI22_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI22_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI22_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI22_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI22_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI22_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI22_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI22_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI22_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI22_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI22_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI22_0_0RCTCornerRadii){
...
{
  return (ABI22_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI22_0_0RCTUpdateShadowPathForView(self);

  const ABI22_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI22_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI22_0_0RCTRunningInTestEnvironment() &&
  ABI22_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI22_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI22_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI22_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI22_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI22_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI22_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI22_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI22_0_0RCTUpdateShadowPathForView(ABI22_0_0RCTView *view)
{
  if (ABI22_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI22_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI22_0_0RCTCornerInsets cornerInsets = ABI22_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI22_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI22_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI22_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI22_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI22_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI22_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI22_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI22_0_0RCTBorderStyle)style \
  {                                                    \
