...
#import "ABI22_0_0EXGoogle.h"
#import "ABI22_0_0EXOAuthViewController.h"
...
NSString *ABI22_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI22_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI22_0_0EXGoogle
{
  ABI22_0_0RCTPromiseResolveBlock _logInResolve;
  ABI22_0_0RCTPromiseRejectBlock _logInReject;
}

ABI22_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI22_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI22_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI22_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI22_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      if (authState.isAuthorized) {
        _logInResolve(@{
          @"type": @"success",
          @"accessToken": ABI22_0_0RCTNullIfNil(authState.lastTokenResponse.accessToken),
          @"idToken": ABI22_0_0RCTNullIfNil(authState.lastTokenResponse.idToken),
          @"refreshToken": ABI22_0_0RCTNullIfNil(authState.lastTokenResponse.refreshToken),
          @"serverAuthCode": ABI22_0_0RCTNullIfNil(authState.lastAuthorizationResponse.authorizationCode),
        });
      } else {
        _logInResolve(@{@"type": @"cancel"});
      }
    } else {
      _logInReject(ABI22_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI22_0_0RCTPresentedViewController()
                                                   callback:callback];
...
-(void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error
{
  if (error != nil) {
    ...
    _logInResolve(@{
      @"type": @"success",
      @"accessToken": ABI22_0_0RCTNullIfNil(user.authentication.accessToken),
      @"serverAuthCode": ABI22_0_0RCTNullIfNil(user.serverAuthCode),
      @"idToken": ABI22_0_0RCTNullIfNil(user.authentication.idToken),
      @"refreshToken": ABI22_0_0RCTNullIfNil(user.authentication.refreshToken),
      @"user": @{
        @"id": ABI22_0_0RCTNullIfNil(user.userID),
        @"name": ABI22_0_0RCTNullIfNil(user.profile.name),
        @"familyName": ABI22_0_0RCTNullIfNil(user.profile.familyName),
        @"givenName": ABI22_0_0RCTNullIfNil(user.profile.givenName),
        @"email": ABI22_0_0RCTNullIfNil(user.profile.email),
...
{
  [ABI22_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI22_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
