...
#import "ABI22_0_0RNBranchAgingDictionary.h"
#import "ABI22_0_0RNBranchEventEmitter.h"

#import "ABI22_0_0EXConstants.h"

NSString * const ABI22_0_0RNBranchLinkOpenedNotification = @"ABI22_0_0RNBranchLinkOpenedNotification";
NSString * const ABI22_0_0RNBranchLinkOpenedNotificationErrorKey = @"error";
NSString * const ABI22_0_0RNBranchLinkOpenedNotificationParamsKey = @"params";
NSString * const ABI22_0_0RNBranchLinkOpenedNotificationUriKey = @"uri";
NSString * const ABI22_0_0RNBranchLinkOpenedNotificationBranchUniversalObjectKey = @"branch_universal_object";
NSString * const ABI22_0_0RNBranchLinkOpenedNotificationLinkPropertiesKey = @"link_properties";
...
static NSString * const ABI22_0_0RNBranchErrorDomain = @"ABI22_0_0RNBranchErrorDomain";
static NSInteger const ABI22_0_0RNBranchUniversalObjectNotFoundError = 1;

#pragma mark - Private ABI22_0_0RNBranch declarations

@interface ABI22_0_0RNBranch()
@property (nonatomic, readonly) UIViewController *currentViewController;
@property (nonatomic) ABI22_0_0RNBranchAgingDictionary<NSString *, BranchUniversalObject *> *universalObjectMap;
@end

#pragma mark - ABI22_0_0RNBranch implementation

@implementation ABI22_0_0RNBranch

@synthesize bridge = _bridge;

ABI22_0_0EX_EXPORT_SCOPED_MODULE(RNBranch, BranchManager);

- (NSDictionary<NSString *, NSString *> *)constantsToExport {
    return @{
             // ABI22_0_0RN events transmitted to JS by event emitter
             @"INIT_SESSION_SUCCESS": ABI22_0_0RNBranchInitSessionSuccess,
             @"INIT_SESSION_ERROR": ABI22_0_0RNBranchInitSessionError,
...
//Called by AppDelegate.m -- stores initSession result in static variables and raises initSessionFinished event that's captured by the ABI22_0_0RNBranch instance to emit it to ReactABI22_0_0 Native
+ (void)initSessionWithLaunchOptions:(NSDictionary *)launchOptions isReferrable:(BOOL)isReferrable {
    ...
    [branchInstance initSessionWithLaunchOptions:launchOptions isReferrable:isReferrable andRegisterDeepLinkHandler:^(NSDictionary *params, NSError *error) {
        ...
        if (error) result[ABI22_0_0RNBranchLinkOpenedNotificationErrorKey] = error;
        if (params) {
            result[ABI22_0_0RNBranchLinkOpenedNotificationParamsKey] = params;

            if (params[@"~id"]) {
                BranchUniversalObject *branchUniversalObject = [BranchUniversalObject objectWithDictionary:params];
                if (branchUniversalObject) result[ABI22_0_0RNBranchLinkOpenedNotificationBranchUniversalObjectKey] = branchUniversalObject;

                BranchLinkProperties *linkProperties = [BranchLinkProperties getBranchLinkPropertiesFromDictionary:params];
                if (linkProperties) result[ABI22_0_0RNBranchLinkOpenedNotificationLinkPropertiesKey] = linkProperties;
            }
        }
        if (sourceUrl) result[ABI22_0_0RNBranchLinkOpenedNotificationUriKey] = sourceUrl;

        [[NSNotificationCenter defaultCenter] postNotificationName:ABI22_0_0RNBranchLinkOpenedNotification object:nil userInfo:result];
    }];
}
...
- (void)setBridge:(ABI22_0_0RCTBridge *)bridge
{
  ...
  if ([self.bridge.scopedModules.constants.appOwnership isEqualToString:@"standalone"]) {
      ...
      _universalObjectMap = [ABI22_0_0RNBranchAgingDictionary dictionaryWithTtl:3600.0];

      [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onInitSessionFinished:) name:ABI22_0_0RNBranchLinkOpenedNotification object:nil];
  }
}
...
- (void) onInitSessionFinished:(NSNotification*) notification {
    NSURL *uri = notification.userInfo[ABI22_0_0RNBranchLinkOpenedNotificationUriKey];
    NSError *error = notification.userInfo[ABI22_0_0RNBranchLinkOpenedNotificationErrorKey];
    NSDictionary *params = notification.userInfo[ABI22_0_0RNBranchLinkOpenedNotificationParamsKey];

    initSessionWithLaunchOptionsResult = @{
                                         ABI22_0_0RNBranchLinkOpenedNotificationErrorKey: error.localizedDescription ?: NSNull.null,
                                         ABI22_0_0RNBranchLinkOpenedNotificationParamsKey: params[@"~id"] ? params : NSNull.null,
                                         ABI22_0_0RNBranchLinkOpenedNotificationUriKey: uri.absoluteString ?: NSNull.null
                                         };

    // If there is an error, fire error event
    if (error) {
        [ABI22_0_0RNBranchEventEmitter initSessionDidEncounterErrorWithPayload:initSessionWithLaunchOptionsResult];
    }

    // otherwise notify the session is finished
    else {
        [ABI22_0_0RNBranchEventEmitter initSessionDidSucceedWithPayload:initSessionWithLaunchOptionsResult];
    }
}
...
- (BranchUniversalObject *)findUniversalObjectWithIdent:(NSString *)ident rejecter:(ABI22_0_0RCTPromiseRejectBlock)reject
{
    ...
    if (!universalObject) {
        ...
        NSError *error = [NSError errorWithDomain:ABI22_0_0RNBranchErrorDomain
                                             code:ABI22_0_0RNBranchUniversalObjectNotFoundError
                                         userInfo:@{IdentFieldName : ident,
                                                    NSLocalizedDescriptionKey: errorMessage
                                                    }];

        reject(@"ABI22_0_0RNBranch::Error::BUONotFound", errorMessage, error);
    }

    return universalObject;
}

#pragma mark - Methods exported to ReactABI22_0_0 Native

#pragma mark redeemInitSessionResult
ABI22_0_0RCT_EXPORT_METHOD(
                  redeemInitSessionResult:(ABI22_0_0RCTPromiseResolveBlock)resolve
...
ABI22_0_0RCT_EXPORT_METHOD(
                  getLatestReferringParams:(ABI22_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(__unused ABI22_0_0RCTPromiseRejectBlock)reject
                  ) {
...
ABI22_0_0RCT_EXPORT_METHOD(
                  getFirstReferringParams:(ABI22_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(__unused ABI22_0_0RCTPromiseRejectBlock)reject
                  ) {
...
ABI22_0_0RCT_EXPORT_METHOD(
    ...
    [branchUniversalObject getShortUrlWithLinkProperties:linkProperties andCallback:^(NSString *url, NSError *error) {
        if (!error) {
            ABI22_0_0RCTLogInfo(@"ABI22_0_0RNBranch Success");
            resolve(@{ @"url": url });
        } else {
            reject([NSString stringWithFormat: @"%lu", (long)error.code], error.localizedDescription, error);
        }
...
ABI22_0_0RCT_EXPORT_METHOD(
    ...
    [branchInstance getShortURLWithParams:linkPropertiesMap
                              ...
                              andCallback:^(NSString *url, NSError *error) {
                                  if (error) {
                                      ABI22_0_0RCTLogError(@"ABI22_0_0RNBranch::Error: %@", error.localizedDescription);
                                      reject(@"ABI22_0_0RNBranch::Error", @"getShortURLWithParams", error);
                                  }
                                  resolve(url);
                              }];
}
...
ABI22_0_0RCT_EXPORT_METHOD(
                  loadRewards:(ABI22_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI22_0_0RCTPromiseRejectBlock)reject
                  ) {
    [branchInstance loadRewardsWithCallback:^(BOOL changed, NSError *error) {
        if(!error) {
            ...
            ABI22_0_0RCTLogError(@"Load Rewards Error: %@", error.localizedDescription);
            reject(@"ABI22_0_0RNBranch::Error::loadRewardsWithCallback", @"loadRewardsWithCallback", error);
        }
    }];
}
    ...
    if (bucket) {
        [branchInstance redeemRewards:amount forBucket:bucket callback:^(BOOL changed, NSError *error) {
            if (!error) {
                ...
                ABI22_0_0RCTLogError(@"Redeem Rewards Error: %@", error.localizedDescription);
                reject(@"ABI22_0_0RNBranch::Error::redeemRewards", error.localizedDescription, error);
            }
        }];
    } else {
...
ABI22_0_0RCT_EXPORT_METHOD(
                  getCreditHistory:(ABI22_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI22_0_0RCTPromiseRejectBlock)reject
                  ) {
    [branchInstance getCreditHistoryWithCallback:^(NSArray *list, NSError *error) {
        if (!error) {
            ...
            ABI22_0_0RCTLogError(@"Credit History Error: %@", error.localizedDescription);
            reject(@"ABI22_0_0RNBranch::Error::getCreditHistory", error.localizedDescription, error);
        }
    }];
}
