...
@implementation NSObject(ABI22_0_0RNBranch)
...
- (void)setSupportedPropertiesWithMap:(NSDictionary *)map
{
    for (NSString *key in map.allKeys) {
        ABI22_0_0RNBranchProperty *property = self.class.supportedProperties[key];
        if (!property) {
            ABI22_0_0RCTLogWarn(@"\"%@\" is not a supported property for %@.", key, NSStringFromClass(self.class));
            continue;
        }
        ...
        if (![value isKindOfClass:type]) {
            ABI22_0_0RCTLogWarn(@"\"%@\" requires a value of type %@.", key, NSStringFromClass(type));
            continue;
        }
