...
@implementation ABI22_0_0RNSVGNode
{
    BOOL _transparent;
    CGPathRef _cachedClipPath;
    ABI22_0_0RNSVGSvgView *_svgView;
}
...
- (void)invalidate
{
    id<ABI22_0_0RNSVGContainer> container = (id<ABI22_0_0RNSVGContainer>)self.superview;
...
- (ABI22_0_0RNSVGSvgView *)getSvgView
{
    ...
    if ([parent class] == [ABI22_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI22_0_0RNSVGNode class]]) {
        ABI22_0_0RNSVGNode *node = parent;
        _svgView = [node getSvgView];
    } else {
        ABI22_0_0RCTLogError(@"ABI22_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI22_0_0RNSVGPercentageConverter stringToFloat:length relative:[self getContextWidth] offset:0];
}

- (CGFloat)relativeOnHeight:(NSString *)length
{
    return [ABI22_0_0RNSVGPercentageConverter stringToFloat:length relative:[self getContextHeight] offset:0];
}
