...
NSString *const ABI22_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI22_0_0RCTReactABI22_0_0TagAttributeName = @"ReactABI22_0_0TagAttributeName";

static NSString *const kShadowViewAttributeName = @"ABI22_0_0RCTShadowViewAttributeName";
...
@implementation ABI22_0_0RCTShadowText
{
...
static ABI22_0_0YGSize ABI22_0_0RCTMeasure(ABI22_0_0YGNodeRef node, float width, ABI22_0_0YGMeasureMode widthMode, float height, ABI22_0_0YGMeasureMode heightMode)
{
  ABI22_0_0RCTShadowText *shadowText = (__bridge ABI22_0_0RCTShadowText *)ABI22_0_0YGNodeGetContext(node);
  ...
  result.width = ABI22_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI22_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI22_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI22_0_0RCTMeasure);

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(contentSizeMultiplierDidChange:)
                                                 name:ABI22_0_0RCTUIManagerWillUpdateViewsDueToContentSizeMultiplierChangeNotification
                                               object:nil];
  }
  return self;
}
...
{
  ABI22_0_0YGNodeMarkDirty(self.yogaNode);
  [self dirtyText];
}
...
{
  if ([[self ReactABI22_0_0Superview] isKindOfClass:[ABI22_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI22_0_0Superview] ReactABI22_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:width widthMode:ABI22_0_0YGMeasureModeExactly];
  CGRect textFrame = [self calculateTextFrame:textStorage];
  BOOL selectable = _selectable;
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI22_0_0RCTText *view = (ABI22_0_0RCTText *)viewRegistry[self.ReactABI22_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI22_0_0RCTShadowText
     * to the ABI22_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI22_0_0RCTTextView
     * and its contaned ABI22_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI22_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  NSTextStorage *textStorage = [self buildTextStorageForWidth:self.frame.size.width widthMode:ABI22_0_0YGMeasureModeExactly];
  NSLayoutManager *layoutManager = textStorage.layoutManagers.firstObject;
  NSTextContainer *textContainer = layoutManager.textContainers.firstObject;
  NSRange glyphRange = [layoutManager glyphRangeForTextContainer:textContainer];
  NSRange characterRange = [layoutManager characterRangeForGlyphRange:glyphRange actualGlyphRange:NULL];
  [layoutManager.textStorage enumerateAttribute:kShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI22_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI22_0_0YGNodeRef childNode = child.yogaNode;
      float width = ABI22_0_0YGNodeStyleGetWidth(childNode).value;
      float height = ABI22_0_0YGNodeStyleGetHeight(childNode).value;
      if (ABI22_0_0YGFloatIsUndefined(width) || ABI22_0_0YGFloatIsUndefined(height)) {
        ABI22_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI22_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI22_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI22_0_0RCTRoundPixelValue(width),
        ABI22_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI22_0_0YGMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){widthMode == ABI22_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : width, CGFLOAT_MAX};
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI22_0_0RCTFont updateFont:nil
  ...
  for (ABI22_0_0RCTShadowView *child in [self ReactABI22_0_0Subviews]) {
    if ([child isKindOfClass:[ABI22_0_0RCTShadowText class]]) {
      ABI22_0_0RCTShadowText *shadowText = (ABI22_0_0RCTShadowText *)child;
      ...
      ABI22_0_0RCTShadowRawText *shadowRawText = (ABI22_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI22_0_0YGNodeStyleGetWidth(child.yogaNode).value;
      float height = ABI22_0_0YGNodeStyleGetHeight(child.yogaNode).value;
      if (ABI22_0_0YGFloatIsUndefined(width) || ABI22_0_0YGFloatIsUndefined(height)) {
        ABI22_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      // Don't call setTextComputed on this child. ABI22_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI22_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI22_0_0RCTReactABI22_0_0TagAttributeName withValue:self.ReactABI22_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  // Text decoration
  if (_textDecorationLine == ABI22_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI22_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI22_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI22_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
- (CGRect)updateStorage:(NSTextStorage *)textStorage toFitFrame:(CGRect)frame
{
  ...
  frame.origin.y = self.paddingAsInsets.top + ABI22_0_0RCTRoundPixelValue((CGRectGetHeight(frame) - requiredSize.height) / 2.0f);
  return frame;
}
                               ...
                               prevMid:(CGFloat)prevMid
{
  ...
  if (round((prevMid / kAutoSizeGranularity)) == round((midScale / kAutoSizeGranularity))) {
    ...
    ABI22_0_0RCTSizeComparison comparison = [self attemptScale:midScale
                                            inStorage:textStorage
                                             forFrame:frame];
    if (comparison == ABI22_0_0RCTSizeWithinRange) {
      return [self calculateSize:textStorage];
    } else if (comparison == ABI22_0_0RCTSizeTooLarge) {
...
- (ABI22_0_0RCTSizeComparison)attemptScale:(CGFloat)scale
                         ...
                         forFrame:(CGRect)frame
{
  ...
  if (fitLines && fitSize) {
    ...
    {
      return ABI22_0_0RCTSizeWithinRange;
    } else {
      return ABI22_0_0RCTSizeTooSmall;
    }
  } else {
    return ABI22_0_0RCTSizeTooLarge;
  }
}
...
#define ABI22_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI22_0_0RCT_TEXT_PROPERTY(AdjustsFontSizeToFit, _adjustsFontSizeToFit, BOOL)
ABI22_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI22_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI22_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI22_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI22_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI22_0_0RCT_TEXT_PROPERTY(FontVariant, _fontVariant, NSArray *)
ABI22_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI22_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI22_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI22_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI22_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI22_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI22_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI22_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI22_0_0RCTTextDecorationLineType);
ABI22_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI22_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI22_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI22_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI22_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI22_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI22_0_0RCTShadowView *child in [self ReactABI22_0_0Subviews]) {
    if ([child isKindOfClass:[ABI22_0_0RCTShadowText class]]) {
      ((ABI22_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  if (_fontSizeMultiplier == 0) {
    ABI22_0_0RCTLogError(@"fontSizeMultiplier value must be > zero.");
    _fontSizeMultiplier = 1.0;
  }
  for (ABI22_0_0RCTShadowView *child in [self ReactABI22_0_0Subviews]) {
    if ([child isKindOfClass:[ABI22_0_0RCTShadowText class]]) {
      ((ABI22_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
