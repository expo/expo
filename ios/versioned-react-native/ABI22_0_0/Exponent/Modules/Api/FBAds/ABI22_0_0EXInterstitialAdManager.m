...
#import "ABI22_0_0EXInterstitialAdManager.h"
...
@interface ABI22_0_0EXInterstitialAdManager () <FBInterstitialAdDelegate>
...
@implementation ABI22_0_0EXInterstitialAdManager

@synthesize bridge = _bridge;

ABI22_0_0RCT_EXPORT_MODULE(CTKInterstitialAdManager)
...
{
  ABI22_0_0RCTAssert(_resolve == nil && _reject == nil, @"Only one `showAd` can be called at once");
  ABI22_0_0RCTAssert(_isBackground == false, @"`showAd` can be called only when experience is running in foreground");
...
{
  [_interstitialAd showAdFromRootViewController:ABI22_0_0RCTPresentedViewController()];
}
...
- (void)bridgeDidForeground:(NSNotification *)notification
{
  ...
  if (_adViewController) {
    [ABI22_0_0RCTPresentedViewController() presentViewController:_adViewController animated:NO completion:nil];
...
- (void)bridgeDidBackground:(NSNotification *)notification
{
  ...
  if (_interstitialAd) {
    _adViewController = ABI22_0_0RCTPresentedViewController();
