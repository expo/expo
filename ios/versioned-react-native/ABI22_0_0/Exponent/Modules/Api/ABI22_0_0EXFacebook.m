...
@implementation FBSDKInternalUtility (ABI22_0_0EXFacebook)
...
@implementation ABI22_0_0EXFacebook

@synthesize bridge = _bridge;

ABI22_0_0RCT_EXPORT_MODULE(ExponentFacebook)

ABI22_0_0RCT_REMAP_METHOD(logInWithReadPermissionsAsync,
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (loginMgr.loginBehavior != FBSDKLoginBehaviorWeb) {
      if ([_bridge.scopedModules.constants.appOwnership isEqualToString:@"expo"]) {
        ...
        reject(@"E_BEHAVIOR_NOT_PERMITTED", message, ABI22_0_0RCTErrorWithMessage(message));
        return;
      } else {
        if (![[self class] facebookAppIdFromNSBundle]) {
          ...
          reject(@"E_BEHAVIOR_NOT_SUPPORTED", message, ABI22_0_0RCTErrorWithMessage(message));
          return;
        }
      }
    }
