...
@interface ABI22_0_0AIRGoogleMap ()

- (id)eventFromCoordinate:(CLLocationCoordinate2D)coordinate;

@end

@implementation ABI22_0_0AIRGoogleMap
{
...
- (void)insertReactABI22_0_0Subview:(id<ABI22_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  ...
  if ([subview isKindOfClass:[ABI22_0_0AIRGoogleMapMarker class]]) {
    ABI22_0_0AIRGoogleMapMarker *marker = (ABI22_0_0AIRGoogleMapMarker*)subview;
    marker.realMarker.map = self;
    [self.markers addObject:marker];
  } else if ([subview isKindOfClass:[ABI22_0_0AIRGoogleMapPolygon class]]) {
    ABI22_0_0AIRGoogleMapPolygon *polygon = (ABI22_0_0AIRGoogleMapPolygon*)subview;
    polygon.polygon.map = self;
    [self.polygons addObject:polygon];
  } else if ([subview isKindOfClass:[ABI22_0_0AIRGoogleMapPolyline class]]) {
    ABI22_0_0AIRGoogleMapPolyline *polyline = (ABI22_0_0AIRGoogleMapPolyline*)subview;
    polyline.polyline.map = self;
    [self.polylines addObject:polyline];
  } else if ([subview isKindOfClass:[ABI22_0_0AIRGoogleMapCircle class]]) {
    ABI22_0_0AIRGoogleMapCircle *circle = (ABI22_0_0AIRGoogleMapCircle*)subview;
    circle.circle.map = self;
    [self.circles addObject:circle];
  } else if ([subview isKindOfClass:[ABI22_0_0AIRGoogleMapUrlTile class]]) {
    ABI22_0_0AIRGoogleMapUrlTile *tile = (ABI22_0_0AIRGoogleMapUrlTile*)subview;
...
- (void)setInitialRegion:(MKCoordinateRegion)initialRegion {
  ...
  self.camera = [ABI22_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:initialRegion];
}

- (void)setRegion:(MKCoordinateRegion)region {
  // TODO: The JS component is repeatedly setting region unnecessarily. We might want to deal with that in here.
  self.camera = [ABI22_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self  andMKCoordinateRegion:region];
}

- (BOOL)didTapMarker:(GMSMarker *)marker {
  ABI22_0_0AIRGMSMarker *airMarker = (ABI22_0_0AIRGMSMarker *)marker;
...
- (void)didTapPolygon:(GMSOverlay *)polygon {
    ABI22_0_0AIRGMSPolygon *airPolygon = (ABI22_0_0AIRGMSPolygon *)polygon;
...
- (void)didChangeCameraPosition:(GMSCameraPosition *)position {
  id event = @{@"continuous": @YES,
               @"region": regionAsJSON([ABI22_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andGMSCameraPosition:position]),
               };
