...
#import "ABI22_0_0AIRGMSMarker.h"
#import "ABI22_0_0AIRGoogleMapCallout.h"
#import "ABI22_0_0AIRDummyView.h"
...
@interface ABI22_0_0AIRGoogleMapMarker ()
- (id)eventFromMarker:(ABI22_0_0AIRGMSMarker*)marker;
@end

@implementation ABI22_0_0AIRGoogleMapMarker {
...
{
  if ((self = [super init])) {
    _realMarker = [[ABI22_0_0AIRGMSMarker alloc] init];
...
- (void)insertReactABI22_0_0Subview:(id<ABI22_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  if ([subview isKindOfClass:[ABI22_0_0AIRGoogleMapCallout class]]) {
    self.calloutView = (ABI22_0_0AIRGoogleMapCallout *)subview;
  } else { // a child view of the marker
    [self iconViewInsertSubview:(UIView*)subview atIndex:atIndex+1];
  }
  ABI22_0_0AIRDummyView *dummySubview = [[ABI22_0_0AIRDummyView alloc] initWithView:(UIView *)subview];
  [super insertReactABI22_0_0Subview:(UIView*)dummySubview atIndex:atIndex];
}

- (void)removeReactABI22_0_0Subview:(id<ABI22_0_0RCTComponent>)dummySubview {
  UIView* subview = ((ABI22_0_0AIRDummyView*)dummySubview).view;
  ...
  [super removeReactABI22_0_0Subview:(UIView*)dummySubview];
}
...
- (void)didTapInfoWindowOfMarker:(ABI22_0_0AIRGMSMarker *)marker {
...
- (void)didBeginDraggingMarker:(ABI22_0_0AIRGMSMarker *)marker {
  if (!self.onDragStart) return;
  self.onDragStart([self eventFromMarker:marker]);
}

- (void)didEndDraggingMarker:(ABI22_0_0AIRGMSMarker *)marker {
  if (!self.onDragEnd) return;
  self.onDragEnd([self eventFromMarker:marker]);
}

- (void)didDragMarker:(ABI22_0_0AIRGMSMarker *)marker {
...
- (ABI22_0_0RCTBubblingEventBlock)onPress {
...
- (void)setImageSrc:(NSString *)imageSrc
{
  ...
  if (!_iconImageView) {
    // prevent glitch with marker (cf. https://github.com/airbnb/ReactABI22_0_0-native-maps/issues/738)
...
//                                                                   }
                                                                   ...
                                                                   //       Shouldn't required images have size info automatically via ABI22_0_0RN?
