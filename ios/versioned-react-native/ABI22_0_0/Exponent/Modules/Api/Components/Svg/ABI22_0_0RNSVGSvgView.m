...
#import "ABI22_0_0RNSVGViewBox.h"
#import "ABI22_0_0RNSVGNode.h"
#import <ReactABI22_0_0/ABI22_0_0RCTLog.h>

@implementation ABI22_0_0RNSVGSvgView
{
    NSMutableDictionary<NSString *, ABI22_0_0RNSVGNode *> *_clipPaths;
    NSMutableDictionary<NSString *, ABI22_0_0RNSVGNode *> *_templates;
    NSMutableDictionary<NSString *, ABI22_0_0RNSVGPainter *> *_painters;
    CGRect _boundingBox;
...
- (void)setMeetOrSlice:(ABI22_0_0RNSVGVBMOS)meetOrSlice
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.align) {
        _viewBoxTransform = [ABI22_0_0RNSVGViewBox getTransform:CGRectMake(self.minX, self.minY, self.vbWidth, self.vbHeight)
                                                 eRect:rect
                                                 align:self.align
    ...
    for (ABI22_0_0RNSVGNode *node in self.subviews) {
        if ([node isKindOfClass:[ABI22_0_0RNSVGNode class]]) {
...
{
    if (self.align) {
        for (ABI22_0_0RNSVGNode *node in [self.subviews reverseObjectEnumerator]) {
            if (![node isKindOfClass:[ABI22_0_0RNSVGNode class]]) {
                continue;
            }
...
- (void)defineClipPath:(__kindof ABI22_0_0RNSVGNode *)clipPath clipPathName:(NSString *)clipPathName
{
    if (!_clipPaths) {
...
- (ABI22_0_0RNSVGNode *)getDefinedClipPath:(NSString *)clipPathName
{
    return _clipPaths ? [_clipPaths objectForKey:clipPathName] : nil;
}

- (void)defineTemplate:(ABI22_0_0RNSVGNode *)template templateName:(NSString *)templateName
{
    if (!_templates) {
...
- (ABI22_0_0RNSVGNode *)getDefinedTemplate:(NSString *)templateName
{
    return _templates ? [_templates objectForKey:templateName] : nil;
}


- (void)definePainter:(ABI22_0_0RNSVGPainter *)painter painterName:(NSString *)painterName
{
    if (!_painters) {
...
- (ABI22_0_0RNSVGPainter *)getDefinedPainter:(NSString *)painterName;
{
