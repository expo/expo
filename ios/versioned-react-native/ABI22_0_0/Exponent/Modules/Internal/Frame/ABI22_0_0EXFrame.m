...
#import "ABI22_0_0EXAppLoadingManager.h"
...
#define ABI22_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05
#define ABI22_0_0EX_FRAME_ORIENTATION_USE_MANIFEST 0

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI22_0_0EXFrameReactABI22_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI22_0_0EXFrameReactABI22_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI22_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI22_0_0Bridge;

@end

@interface ABI22_0_0EXFrame () <ABI22_0_0RCTInvalidating, ABI22_0_0RCTExceptionsManagerDelegate>

@property (nonatomic, assign) BOOL sourceSet;
@property (nonatomic, assign) BOOL needsReload;

@property (nonatomic, copy) ABI22_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI22_0_0RCTDirectEventBlock onLoadingProgress;
@property (nonatomic, copy) ABI22_0_0RCTDirectEventBlock onLoadingFinish;
@property (nonatomic, copy) ABI22_0_0RCTDirectEventBlock onLoadingError;
@property (nonatomic, copy) ABI22_0_0RCTDirectEventBlock onError;
...
@implementation ABI22_0_0EXFrame

@synthesize supportedInterfaceOrientations = _supportedInterfaceOrientations;

ABI22_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI22_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");

    _appManager = [[unversionedAppManagerClass alloc] initWithEXFrame:self];
    [_appManager setDelegate:self];
    _supportedInterfaceOrientations = ABI22_0_0EX_FRAME_ORIENTATION_USE_MANIFEST;
  }
  return self;
}
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI22_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI22_0_0RootView]) {
    [_appManager ReactABI22_0_0RootView].frame = self.bounds;
  }
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
  if (_supportedInterfaceOrientations != ABI22_0_0EX_FRAME_ORIENTATION_USE_MANIFEST) {
...
#pragma mark - ABI22_0_0EXReactABI22_0_0AppManagerDelegate

- (void)ReactABI22_0_0AppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI22_0_0RootView = [appManager ReactABI22_0_0RootView];
  ReactABI22_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI22_0_0RootView];
}

- (void)ReactABI22_0_0AppManagerDidDestroyApp:(id)appManager
{
  if (_tmrReload) {
    [_tmrReload invalidate];
    _tmrReload = nil;
...
- (void)ReactABI22_0_0AppManagerStartedLoadingJavaScript:(id)appManager
{
  if (_onLoadingStart) {
    _onLoadingStart(nil);
  }
...
- (void)ReactABI22_0_0AppManager:(id)appManager loadedJavaScriptWithProgress:(ABI22_0_0RCTLoadingProgress *)progress;
{
  if (_onLoadingProgress) {
    _onLoadingProgress(@{
      @"status": ABI22_0_0RCTNullIfNil(progress.status),
      @"done": ABI22_0_0RCTNullIfNil(progress.done),
      @"total": ABI22_0_0RCTNullIfNil(progress.total),
    });
  }
}

- (void)ReactABI22_0_0AppManagerFinishedLoadingJavaScript:(id)appManager
{
  if (_viewTestTimer) {
    [_viewTestTimer invalidate];
  }
...
- (void)ReactABI22_0_0AppManager:(id)appManager failedToLoadJavaScriptWithError:(NSError *)error
{
  [self _sendLoadingError:error];
}

- (void)ReactABI22_0_0AppManager:(id)appManager failedToDownloadBundleWithError:(NSError *)error
{
  
}

- (void)ReactABI22_0_0AppManagerDidForeground:(id)appManager
{
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI22_0_0RootView] &&
      [_appManager ReactABI22_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI22_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI22_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
    if (!appLoading || !appLoading.started || appLoading.finished) {
      [[NSNotificationCenter defaultCenter] postNotificationName:@"EXKernelAppDidDisplay" object:self];
      if (_onLoadingFinish) {
...
# pragma mark - ABI22_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI22_0_0RCTErrorWithMessage(message)];
