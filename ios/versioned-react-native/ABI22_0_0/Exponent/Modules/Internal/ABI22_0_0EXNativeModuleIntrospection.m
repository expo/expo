...
#import "ABI22_0_0EXNativeModuleIntrospection.h"
...
@implementation ABI22_0_0EXNativeModuleIntrospection

@synthesize bridge = _bridge;

ABI22_0_0RCT_EXPORT_MODULE(ExpoNativeModuleIntrospection)

ABI22_0_0RCT_REMAP_METHOD(getNativeModuleNamesAsync,
                 nativeModuleNamesWithResolver:(ABI22_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI22_0_0RCTPromiseRejectBlock)reject) {
  NSError *error;
  NSDictionary<NSString *, ABI22_0_0RCTModuleData *> *moduleDataByName = [self _moduleDataFromBridge:&error];
  if (!moduleDataByName) {
...
ABI22_0_0RCT_REMAP_METHOD(introspectNativeModuleAsync,
  ...
  ABI22_0_0RCTModuleData *moduleData = moduleDataByName[name];
  if (!moduleData) {
    resolve(nil);
    return;
  }
  
  NSArray<id<ABI22_0_0RCTBridgeMethod>> *moduleMethods = moduleData.methods;
  ...
  for (id<ABI22_0_0RCTBridgeMethod> method in moduleMethods) {
    NSString *methodName = [NSString stringWithCString:method.JSMethodName encoding:NSASCIIStringEncoding];
    NSString *functionType = [NSString stringWithCString:ABI22_0_0RCTFunctionDescriptorFromType(method.functionType)
...
- (NSDictionary<NSString *, ABI22_0_0RCTModuleData *> *)_moduleDataFromBridge:(NSError **)error {
  ABI22_0_0RCTBridge *bridge = _bridge;
  if (![NSStringFromClass(bridge.class) isEqualToString:@"ABI22_0_0RCTCxxBridge"]) {
    if (error) {
  ...
  NSDictionary<NSString *, ABI22_0_0RCTModuleData *> *moduleDataByName;
  @try {
