...
#import <ABI43_0_0cxxreact/ABI43_0_0CxxNativeModule.h>
#import <ABI43_0_0jsi/ABI43_0_0jsi.h>

#import "ABI43_0_0DispatchMessageQueueThread.h"
...
using ABI43_0_0facebook::jsi::JSError;

std::vector<std::unique_ptr<NativeModule>>
createNativeModules(NSArray<ABI43_0_0RCTModuleData *> *modules, ABI43_0_0RCTBridge *bridge, const std::shared_ptr<Instance> &instance)
{
  std::vector<std::unique_ptr<NativeModule>> nativeModules;
  for (ABI43_0_0RCTModuleData *moduleData in modules) {
    if ([moduleData.moduleClass isSubclassOfClass:[ABI43_0_0RCTCxxModule class]]) {
      nativeModules.emplace_back(std::make_unique<CxxNativeModule>(
          instance,
          [moduleData.name UTF8String],
          [moduleData] { return [(ABI43_0_0RCTCxxModule *)(moduleData.instance) createModule]; },
          std::make_shared<DispatchMessageQueueThread>(moduleData)));
    } else {
      nativeModules.emplace_back(std::make_unique<ABI43_0_0RCTNativeModule>(bridge, moduleData));
    }
  }
...
static NSError *errorWithException(const std::exception &e)
{
  ...
  if (jsError) {
    errorInfo[ABI43_0_0RCTJSRawStackTraceKey] = @(jsError->getStack().c_str());
  ...
  return [NSError errorWithDomain:ABI43_0_0RCTErrorDomain code:1 userInfo:errorInfo];
}
...
NSError *tryAndReturnError(const std::function<void()> &func)
{
  try {
    @try {
      ...
      return ABI43_0_0RCTErrorWithNSException(exception);
    } @catch (id exception) {
      // This will catch any other ObjC exception, but no C++ exceptions
      return ABI43_0_0RCTErrorWithMessage(@"non-std ObjC Exception");
    }
    ...
    return ABI43_0_0RCTErrorWithMessage(@"non-std C++ exception");
  }
}
