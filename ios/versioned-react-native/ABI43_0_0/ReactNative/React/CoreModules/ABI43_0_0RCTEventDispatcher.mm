...
static NSNumber *ABI43_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI43_0_0RCTUniqueCoalescingKeyGenerator = 0;

@interface ABI43_0_0RCTEventDispatcher () <ABI43_0_0RCTTurboModule>
@end

@implementation ABI43_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI43_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI43_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  if (_bridge) {
    [_bridge enqueueJSCall:@"ABI43_0_0RCTNativeAppEventEmitter"
                    method:@"emit"
                      args:body ? @[ name, body ] : @[ name ]
                completion:NULL];
  } else if (_invokeJS) {
    _invokeJS(@"ABI43_0_0RCTNativeAppEventEmitter", @"emit", body ? @[ name, body ] : @[ name ]);
  }
}
...
- (void)sendDeviceEventWithName:(NSString *)name body:(id)body
{
  if (_bridge) {
    ...
    _invokeJS(@"ABI43_0_0RCTDeviceEventEmitter", @"emit", body ? @[ name, body ] : @[ name ]);
  }
}
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI43_0_0RCTComponentEvent *event = [[ABI43_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI43_0_0ReactTag body:body];
...
- (void)sendEvent:(id<ABI43_0_0RCTEvent>)event
{
  ...
  for (id<ABI43_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  if (event.canCoalesce) {
    eventID = ABI43_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI43_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI43_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI43_0_0RCTGetEventID(event.viewTag, event.eventName, ABI43_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI43_0_0RCTAssert(
        previousEvent == nil,
...
Class ABI43_0_0RCTEventDispatcherCls(void)
{
  return ABI43_0_0RCTEventDispatcher.class;
}
