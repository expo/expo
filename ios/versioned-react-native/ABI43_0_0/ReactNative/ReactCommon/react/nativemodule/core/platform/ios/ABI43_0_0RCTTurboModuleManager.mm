...
#import "ABI43_0_0RCTTurboModuleManager.h"
...
#import <ABI43_0_0ReactCommon/ABI43_0_0TurboCxxModule.h>
#import <ABI43_0_0ReactCommon/ABI43_0_0TurboModuleBinding.h>
#import <ABI43_0_0ReactCommon/ABI43_0_0TurboModulePerfLogger.h>

using namespace ABI43_0_0facebook::ABI43_0_0React;

/**
 * A global variable whose address we use to associate method queues to id<ABI43_0_0RCTTurboModule> objects.
 */
...
class TurboModuleHolder {
 private:
  ...
  id<ABI43_0_0RCTTurboModule> module_;
 ...
 public:
  ...
  void setModule(id<ABI43_0_0RCTTurboModule> module)
  {
    module_ = module;
  }

  id<ABI43_0_0RCTTurboModule> getModule() const
  {
    return module_;
  }
...
class MethodQueueNativeCallInvoker : public CallInvoker {
 ...
 public:
  ...
  {
    if (methodQueue_ == ABI43_0_0RCTJSThread) {
      work();
      return;
    }
...
// Fallback lookup since ABI43_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI43_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI43_0_0RCTTurboModuleManager {
  std::shared_ptr<CallInvoker> _jsInvoker;
  __weak id<ABI43_0_0RCTTurboModuleManagerDelegate> _delegate;
  ...
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeWillInvalidateModules:)
                                                 name:ABI43_0_0RCTBridgeWillInvalidateModulesNotification
                                               object:_bridge.parentBridge];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidInvalidateModules:)
                                                 name:ABI43_0_0RCTBridgeDidInvalidateModulesNotification
                                               object:_bridge.parentBridge];
  }
  return self;
}
...
- (void)notifyAboutTurboModuleSetup:(const char *)name
{
  ...
  if (moduleName) {
    int64_t setupTime = [self->_bridge.performanceLogger durationForTag:ABI43_0_0RCTPLTurboModuleSetup];
...
- (std::shared_ptr<TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  id<ABI43_0_0RCTTurboModule> module = [self provideABI43_0_0RCTTurboModule:moduleName];

  TurboModulePerfLogger::moduleJSRequireEndingStart(moduleName);

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI43_0_0RCTTurboModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  /**
   * Have ABI43_0_0RCTCxxBridge decorate native CallInvoker, so that it's aware of TurboModule async method calls.
  ...
  ObjCTurboModule::InitParams params = {
      ...
      .isSyncModule = methodQueue == ABI43_0_0RCTJSThread,
  };

  // If ABI43_0_0RCTTurboModule supports creating its own C++ TurboModule object,
  // allow it to do so.
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI43_0_0RCTCxxModule.class]) {
    ...
    auto turboModule = std::make_shared<TurboCxxModule>([((ABI43_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
...
{
  if (ABI43_0_0RCTTurboModuleSharedMutexInitEnabled()) {
    {
 ...
 * (for now).
 */
- (id<ABI43_0_0RCTTurboModule>)provideABI43_0_0RCTTurboModule:(const char *)moduleName
{
  ...
  id<ABI43_0_0RCTTurboModule> module = [self _provideABI43_0_0RCTTurboModule:moduleName moduleHolder:moduleHolder shouldPerfLog:YES];
...
- (id<ABI43_0_0RCTTurboModule>)_provideABI43_0_0RCTTurboModule:(const char *)moduleName
                               ...
                               shouldPerfLog:(BOOL)shouldPerfLog
{
  ...
  if (shouldCreateModule) {
    ...
    __block id<ABI43_0_0RCTTurboModule> module = nil;

    if ([moduleClass conformsToProtocol:@protocol(ABI43_0_0RCTTurboModule)]) {
      dispatch_block_t work = ^{
        module = [self _createAndSetUpABI43_0_0RCTTurboModule:moduleClass
                                          moduleName:moduleName
      ...
      if ([self _requiresMainQueueSetup:moduleClass]) {
         ...
         */
        if (ABI43_0_0RCTTurboModuleEagerInitEnabled() && !ABI43_0_0RCTIsMainQueue()) {
          ABI43_0_0RCTLogWarn(
              @"TurboModule \"%@\" requires synchronous dispatch onto the main queue to be initialized. This may lead to deadlock.",
              moduleClass);
        }

        ABI43_0_0RCTUnsafeExecuteOnMainQueueSync(work);
      } else {
        work();
      }
    }
 ...
 *  - The thread that calls provideABI43_0_0RCTTurboModule:
 *  - The main thread (if the TurboModule requires main queue init), blocking the thread that calls
 * provideABI43_0_0RCTTurboModule:.
 */
- (id<ABI43_0_0RCTTurboModule>)_createAndSetUpABI43_0_0RCTTurboModule:(Class)moduleClass
                                         moduleName:(const char *)moduleName
                                           moduleId:(int32_t)moduleId
{
  id<ABI43_0_0RCTTurboModule> module = nil;
   ...
   * implementation, and a `- (ABI43_0_0RCTBridge *) bridge { ... }` method won't be
  ...
  if ([module respondsToSelector:@selector(bridge)] && _bridge) {
    ...
    @try {
      /**
       * ABI43_0_0RCTBridgeModule declares the bridge property as readonly.
       * Therefore, when authors of NativeModules synthesize the bridge
       * via @synthesize bridge = bridge;, the ObjC runtime generates
       * only a - (ABI43_0_0RCTBridge *) bridge: { ... } method. No setter is
  ...
  if (moduleHasMethodQueueGetter) {
    methodQueue = [(id<ABI43_0_0RCTBridgeModule>)module methodQueue];
  }

  /**
   * Note: ABI43_0_0RCTJSThread, which is a valid method queue, is defined as (id)kCFNull. It should rightfully not enter the
   * following if condition's block.
   */
  if (!methodQueue) {
    NSString *methodQueueName = [NSString stringWithFormat:@"com.facebook.ABI43_0_0React.%sQueue", moduleName];
  ...
  /**
   * Attach method queue to id<ABI43_0_0RCTTurboModule> object.
   * This is necessary because the id<ABI43_0_0RCTTurboModule> object can be eagerly created/initialized before the method
   * queue is required. The method queue is required for an id<ABI43_0_0RCTTurboModule> for JS -> Native calls. So, we need it
   * before we create the id<ABI43_0_0RCTTurboModule>'s TurboModule jsi::HostObject in provideTurboModule:.
   */
  objc_setAssociatedObject(module, &kAssociatedMethodQueueKey, methodQueue, OBJC_ASSOCIATION_RETAIN);

  /**
   * NativeModules that implement the ABI43_0_0RCTFrameUpdateObserver protocol
   * require registration with ABI43_0_0RCTDisplayLink.
   *
   * TODO(T55504345): Investigate whether we can improve this after TM
   * rollout.
   */
  if (_bridge) {
    ABI43_0_0RCTModuleData *data = [[ABI43_0_0RCTModuleData alloc] initWithModuleInstance:(id<ABI43_0_0RCTBridgeModule>)module bridge:_bridge];
    [_bridge registerModuleForFrameUpdates:(id<ABI43_0_0RCTBridgeModule>)module withModuleData:data];
  }
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI43_0_0RCTDidInitializeModuleNotification
                    object:_bridge
                  userInfo:@{@"module" : module, @"bridge" : ABI43_0_0RCTNullIfNil([_bridge parentBridge])}];
...
- (void)installJSBindingWithRuntimeExecutor:(ABI43_0_0facebook::ABI43_0_0React::RuntimeExecutor)runtimeExecutor
{
  ...
  auto turboModuleProvider =
      [self](const std::string &name, const jsi::Value *schema) -> std::shared_ptr<ABI43_0_0React::TurboModule> {
    auto moduleName = name.c_str();

    TurboModulePerfLogger::moduleJSRequireBeginningStart(moduleName);
    auto moduleWasNotInitialized = ![self moduleIsInitialized:moduleName];
    if (moduleWasNotInitialized) {
      [self->_bridge.performanceLogger markStartForTag:ABI43_0_0RCTPLTurboModuleSetup];
    }
    ...
    if (moduleWasNotInitialized && [self moduleIsInitialized:moduleName]) {
      [self->_bridge.performanceLogger markStopForTag:ABI43_0_0RCTPLTurboModuleSetup];
  ...
  runtimeExecutor([turboModuleProvider = std::move(turboModuleProvider)](jsi::Runtime &runtime) {
    ABI43_0_0React::TurboModuleBinding::install(runtime, std::move(turboModuleProvider));
  });
}

#pragma mark ABI43_0_0RCTTurboModuleRegistry
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  if (warnOnLookupFailure && !module) {
    ABI43_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
- (void)bridgeDidInvalidateModules:(NSNotification *)notification
{
  ...
  // Backward-compatibility: ABI43_0_0RCTInvalidating handling.
  ...
  for (auto &pair : _turboModuleHolders) {
    ...
    /**
     * We could start tearing down ABI43_0_0ReactNative before a TurboModule is fully initialized. In this case, we should wait
     * for TurboModule init to finish before calling invalidate on it. So, we call _provideABI43_0_0RCTTurboModule:moduleHolder,
     * because it's guaranteed to return a fully initialized NativeModule.
     */
    id<ABI43_0_0RCTTurboModule> module = [self _provideABI43_0_0RCTTurboModule:moduleName.c_str()
    ...
    if ([module respondsToSelector:@selector(invalidate)]) {
      if ([module respondsToSelector:@selector(methodQueue)]) {
        ...
        if (methodQueue) {
          ...
          [bridge
              dispatchBlock:^{
                [((id<ABI43_0_0RCTInvalidating>)module) invalidate];
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI43_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
...
- (void)invalidate
{
  ...
  // Backward-compatibility: ABI43_0_0RCTInvalidating handling, but not adhering to desired methodQueue.
  for (const auto &p : _turboModuleHolders) {
    id<ABI43_0_0RCTTurboModule> module = p.second.getModule();
    if ([module respondsToSelector:@selector(invalidate)]) {
      [((id<ABI43_0_0RCTInvalidating>)module) invalidate];
    }
  }
