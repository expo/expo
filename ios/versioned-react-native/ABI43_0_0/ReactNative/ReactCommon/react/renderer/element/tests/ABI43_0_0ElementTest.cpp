...
TEST(ElementTest, testNormalCases) {
  ...
  ABI43_0_0EXPECT_EQ(shadowNode, shadowNodeA);

  // Tags
  ABI43_0_0EXPECT_EQ(shadowNodeA->getTag(), 1);
  ABI43_0_0EXPECT_EQ(shadowNodeAA->getTag(), 2);
  ABI43_0_0EXPECT_EQ(shadowNodeAB->getTag(), 3);
  ABI43_0_0EXPECT_EQ(shadowNodeABA->getTag(), 4);

  // Children
  ABI43_0_0EXPECT_EQ(shadowNodeA->getChildren().size(), 2);
  ABI43_0_0EXPECT_EQ(shadowNodeAA->getChildren().size(), 0);
  ABI43_0_0EXPECT_EQ(shadowNodeAB->getChildren().size(), 1);
  ABI43_0_0EXPECT_EQ(shadowNodeABA->getChildren().size(), 0);
  ABI43_0_0EXPECT_EQ(
  ...
  // Props
  ABI43_0_0EXPECT_EQ(shadowNodeA->getProps()->nativeId, "node A");
  ABI43_0_0EXPECT_EQ(shadowNodeABA->getProps()->nativeId, "node ABA");
  ABI43_0_0EXPECT_EQ(shadowNodeAA->getProps(), propsAA);

  // Finalize
  ABI43_0_0EXPECT_TRUE(shadowNodeA->getSealed());
  ABI43_0_0EXPECT_TRUE(shadowNodeAA->getSealed());
  ABI43_0_0EXPECT_TRUE(shadowNodeAB->getSealed());
  ABI43_0_0EXPECT_TRUE(shadowNodeABA->getSealed());
}
