...
#import "ABI43_0_0EXDisabledRedBox.h"
#import "ABI43_0_0EXVersionManager.h"
#import "ABI43_0_0EXScopedBridgeModule.h"
#import "ABI43_0_0EXStatusBarManager.h"
...
#import <ABI43_0_0React/ABI43_0_0RCTExceptionsManager.h>
...
#import <ABI43_0_0React/ABI43_0_0RCTDataRequestHandler.h>
#import <ABI43_0_0React/ABI43_0_0RCTFileRequestHandler.h>
...
#import <ABI43_0_0React/ABI43_0_0RCTAsyncLocalStorage.h>
...
#import <ABI43_0_0EXMediaLibrary/ABI43_0_0EXMediaLibraryImageLoader.h>
#import <ABI43_0_0EXFileSystem/ABI43_0_0EXFileSystem.h>
#import "ABI43_0_0EXScopedModuleRegistry.h"
#import "ABI43_0_0EXScopedModuleRegistryAdapter.h"
#import "ABI43_0_0EXScopedModuleRegistryDelegate.h"
...
#import <ABI43_0_0React/ABI43_0_0CoreModulesPlugins.h>
...
ABI43_0_0RCT_EXTERN NSDictionary<NSString *, NSDictionary *> *ABI43_0_0EXGetScopedModuleClasses(void);
ABI43_0_0RCT_EXTERN void ABI43_0_0EXRegisterScopedModule(Class, ...);

@interface ABI43_0_0RCTEventDispatcher (ABI43_0_0REAnimated)

- (void)setBridge:(ABI43_0_0RCTBridge*)bridge;

@end

// this is needed because ABI43_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI43_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
@interface ABI43_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI43_0_0EXVersionManager () <ABI43_0_0RCTTurboModuleManagerDelegate>

// is this the first time this ABI has been touched at runtime?
@property (nonatomic, assign) BOOL isFirstLoad;
@property (nonatomic, strong) NSDictionary *params;
@property (nonatomic, strong) ABI43_0_0EXManifestsManifest *manifest;
@property (nonatomic, strong) ABI43_0_0RCTTurboModuleManager *turboModuleManager;

@end

@implementation ABI43_0_0EXVersionManager
 ...
 *    @ABI43_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI43_0_0EXKernel *kernel
...
- (instancetype)initWithParams:(NSDictionary *)params
                  ...
                  fatalHandler:(void (^)(NSError *))fatalHandler
                   logFunction:(ABI43_0_0RCTLogFunction)logFunction
...
+ (void)load
{
  ...
  // don't have access to ABI43_0_0EXScopedModuleRegistry and so they can't register themselves.
  ABI43_0_0EXRegisterScopedModule([ABI43_0_0RNCWebViewManager class], ABI43_0_0EX_KERNEL_SERVICE_NONE, nil);
}

- (void)bridgeWillStartLoading:(id)bridge
{
  // We need to check DEBUG flag here because in ejected projects ABI43_0_0RCT_DEV is set only for ABI43_0_0React and not for ExpoKit to which this file belongs to.
  // It can be changed to just ABI43_0_0RCT_DEV once we deprecate ExpoKit and set that flag for the entire standalone project.
#if DEBUG || ABI43_0_0RCT_DEV
  if ([self _isDevModeEnabledForBridge:bridge]) {
    // Set the bundle url for the packager connection manually
    [[ABI43_0_0RCTPackagerConnection sharedPackagerConnection] setBundleURL:[bridge bundleURL]];
  }
#endif

  // Manually send a "start loading" notif, since the real one happened uselessly inside the ABI43_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI43_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}

- (void)bridgeFinishedLoading:(id)bridge
{
  // Override the "Reload" button from Redbox to reload the app from manifest
  // Keep in mind that it is possible this will return a ABI43_0_0EXDisabledRedBox
  ABI43_0_0RCTRedBox *redBox = [self _moduleInstanceForBridge:bridge named:@"RedBox"];
...
{
  ABI43_0_0RCTDevSettings *devSettings = (ABI43_0_0RCTDevSettings *)[self _moduleInstanceForBridge:bridge named:@"DevSettings"];
  ...
  if (devSettings.isRemoteDebuggingAvailable && isDevModeEnabled) {
    ...
    items[@"dev-remote-debug"] =  @{
      ...
      @"isEnabled": @NO,
      @"detail": ABI43_0_0RCTTurboModuleEnabled() ? @"Remote debugging is unavailable while Turbo Modules are enabled. To debug remotely, please set `turboModules` to false in app.json." : [NSNull null]
    };
  }
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI43_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI43_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI43_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return [[ABI43_0_0RCTPackagerConnection sharedPackagerConnection] addNotificationHandler:handler queue:queue forMethod:method];
}
#endif
...
{
  return ([ABI43_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI43_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI43_0_0RCTEnableTurboModule([self.manifest.experiments[@"turboModules"] boolValue]);
  ABI43_0_0RCTSetFatalHandler(fatalHandler);
  ABI43_0_0RCTSetLogThreshold((ABI43_0_0RCTLogLevel) threshold);
...
- (NSArray *)extraModulesForBridge:(id)bridge
{
  ...
  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI43_0_0EXAppState alloc] init],
                                    [[ABI43_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI43_0_0EXStatusBarManager alloc] init],
                                    ]];
  ...
  if (params[@"testEnvironment"]) {
    ABI43_0_0EXTestEnvironment testEnvironment = (ABI43_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI43_0_0EXTestEnvironmentNone) {
      ABI43_0_0EXTest *testModule = [[ABI43_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  ABI43_0_0EXModuleRegistryProvider *moduleRegistryProvider = [[ABI43_0_0EXModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI43_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI43_0_0EXModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ABI43_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI43_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];
#pragma clang diagnostic pop

  ABI43_0_0EXModuleRegistry *moduleRegistry = [moduleRegistryAdapter moduleRegistryForParams:params
                                                        forExperienceStableLegacyId:self.manifest.stableLegacyId
                                                                           scopeKey:self.manifest.scopeKey
                                                                           manifest:self.manifest
                                                                 withKernelServices:services];
  NSArray<id<ABI43_0_0RCTBridgeModule>> *expoModules = [moduleRegistryAdapter extraModulesForModuleRegistry:moduleRegistry];
  ...
  if (!ABI43_0_0RCTTurboModuleEnabled()) {
    ...
    id exceptionsManager = [self getModuleInstanceFromClass:ABI43_0_0RCTExceptionsManagerCls()];
    ...
    [extraModules addObject:[self getModuleInstanceFromClass:ABI43_0_0RCTAsyncLocalStorageCls()]];
  }
...
- (NSArray *)_newScopedModulesForServices:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  NSDictionary<NSString *, NSDictionary *> *ABI43_0_0EXScopedModuleClasses = ABI43_0_0EXGetScopedModuleClasses();
  if (ABI43_0_0EXScopedModuleClasses) {
    [ABI43_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI43_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
...
- (Class)getModuleClassFromName:(const char *)name
{
  if (std::string(name) == "DevSettings") {
    return ABI43_0_0EXDevSettings.class;
  }
  if (std::string(name) == "DevMenu") {
    if (![_params[@"isStandardDevMenuAllowed"] boolValue] || ![_params[@"isDeveloper"] boolValue]) {
      // non-kernel, or non-development kernel, uses expo menu instead of ABI43_0_0RCTDevMenu
      return ABI43_0_0EXDisabledDevMenu.class;
    }
  }
  if (std::string(name) == "RedBox") {
    if (![_params[@"isDeveloper"] boolValue]) {
      // user-facing (not debugging).
      // additionally disable ABI43_0_0RCTRedBox
      return ABI43_0_0EXDisabledRedBox.class;
    }
  }
  return ABI43_0_0RCTCoreModulesClassProvider(name);
}
...
{
  // Standard
  if (moduleClass == ABI43_0_0RCTImageLoader.class) {
    return [[moduleClass alloc] initWithRedirectDelegate:nil loadersProvider:^NSArray<id<ABI43_0_0RCTImageURLLoader>> *{
      return @[[ABI43_0_0RCTLocalAssetImageLoader new], [ABI43_0_0EXMediaLibraryImageLoader new]];
    } decodersProvider:^NSArray<id<ABI43_0_0RCTImageDataDecoder>> *{
      return @[[ABI43_0_0RCTGIFImageDecoder new]];
    }];
  } else if (moduleClass == ABI43_0_0RCTNetworking.class) {
    return [[moduleClass alloc] initWithHandlersProvider:^NSArray<id<ABI43_0_0RCTURLRequestHandler>> *{
      return @[
        [ABI43_0_0RCTHTTPRequestHandler new],
        [ABI43_0_0RCTDataRequestHandler new],
        [ABI43_0_0RCTFileRequestHandler new],
      ];
    }];
  }

  // Expo-specific
  if (moduleClass == ABI43_0_0EXDevSettings.class) {
  ...
  } else if (moduleClass == ABI43_0_0RCTExceptionsManagerCls()) {
    ...
    if (exceptionsManagerDelegate) {
      ...
      ABI43_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
    }
  } else if (moduleClass == ABI43_0_0RCTAsyncLocalStorageCls()) {
...
- (std::shared_ptr<ABI43_0_0facebook::ABI43_0_0React::TurboModule>)getTurboModule:(const std::string &)name
                                                     initParams:(const ABI43_0_0facebook::ABI43_0_0React::ObjCTurboModule::InitParams &)params
{
  // TODO: ADD
  return nullptr;
}
...
- (void *)versionedJsExecutorFactoryForBridge:(ABI43_0_0RCTBridge *)bridge
{
  ...
  const auto executor = [ABI43_0_0EXWeak_self, bridge](ABI43_0_0facebook::jsi::Runtime &runtime) {
    if (!bridge) {
      return;
    }
  ...
  return new ABI43_0_0facebook::ABI43_0_0React::JSCExecutorFactory(ABI43_0_0RCTJSIExecutorRuntimeInstaller(executor));
}
