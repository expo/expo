...
const NSTimeInterval ABI49_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI49_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI49_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI49_0_0AIRMap
{
    UIView *_legalLabel;
    BOOL _initialRegionSet;
    BOOL _initialCameraSet;

    // Array to manually track ABI49_0_0RN subviews
    //
    // ABI49_0_0AIRMap implicitly creates subviews that aren't regular ABI49_0_0RN children
    // (ABI49_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI49_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI49_0_0React-native/blob/v0.16.0/ABI49_0_0React/Modules/ABI49_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI49_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI49_0_0React-native/blob/v0.16.0/Libraries/Text/ABI49_0_0RCTTextField.m#L20
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI49_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI49_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI49_0_0ReactSubview:(id<ABI49_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI49_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI49_0_0AIRMapPolyline class]]) {
        ((ABI49_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI49_0_0AIRMapPolygon class]]) {
        ((ABI49_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI49_0_0AIRMapCircle class]]) {
        ((ABI49_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI49_0_0AIRMapUrlTile class]]) {
        ((ABI49_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI49_0_0AIRMapWMSTile class]]) {
        ((ABI49_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI49_0_0AIRMapLocalTile class]]) {
        ((ABI49_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI49_0_0AIRMapOverlay class]]) {
        ((ABI49_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ...
        return [marker isKindOfClass:[ABI49_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI49_0_0AIRMapMarker*) markerForCallout:(ABI49_0_0AIRMapCallout*)callout {
    ABI49_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI49_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI49_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI49_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (ABI49_0_0AIRMapMarker*) markerAtPoint:(CGPoint)point {
    ABI49_0_0AIRMapMarker* mrk = nil;
    for (ABI49_0_0AIRMapMarker* mrkAnn in self.markers) {
        CGRect frame = [self frameForMarker:mrkAnn];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI49_0_0AIRMapCallout* callout = nil;
        ABI49_0_0AIRMapMarker* marker = nil;
        ...
        while (tmp && tmp != win && tmp != self.calloutView) {
            ...
            if ([tmp isKindOfClass:[ABI49_0_0AIRMapCallout class]]) {
                callout = (ABI49_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI49_0_0SMCalloutViewDelegate
...
- (void)cacheViewIfNeeded {
    // https://github.com/ABI49_0_0React-native-maps/ABI49_0_0React-native-maps/issues/3100
