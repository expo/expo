...
@interface UIView (ABI49_0_0SMFrameAdditions)
...
@interface ABI49_0_0SMCalloutView ()
...
@property (nonatomic, assign) ABI49_0_0SMCalloutArrowDirection currentArrowDirection;
@property (nonatomic, assign) BOOL popupCancelled;
@end

@implementation ABI49_0_0SMCalloutView

+ (ABI49_0_0SMCalloutView *)platformCalloutView {

    // if you haven't compiled ABI49_0_0SMClassicCalloutView into your app, then we can't possibly create an instance of it!
    if (!NSClassFromString(@"ABI49_0_0SMClassicCalloutView"))
        return [ABI49_0_0SMCalloutView new];

    // ok we have both - so choose the best one based on current platform
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1)
        return [ABI49_0_0SMCalloutView new]; // iOS 7+
    else
        return [NSClassFromString(@"ABI49_0_0SMClassicCalloutView") new];
}

- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.permittedArrowDirection = ABI49_0_0SMCalloutArrowDirectionDown;
        self.presentAnimation = ABI49_0_0SMCalloutAnimationBounce;
        self.dismissAnimation = ABI49_0_0SMCalloutAnimationFade;
...
- (ABI49_0_0SMCalloutBackgroundView *)backgroundView {
...
- (ABI49_0_0SMCalloutBackgroundView *)defaultBackgroundView {
    return [ABI49_0_0SMCalloutMaskedBackgroundView new];
}
...
- (void)presentCalloutFromRect:(CGRect)rect inLayer:(CALayer *)layer ofView:(UIView *)view constrainedToLayer:(CALayer *)constrainedLayer animated:(BOOL)animated {
    ...
    ABI49_0_0SMCalloutArrowDirection bestDirection = ABI49_0_0SMCalloutArrowDirectionDown;

    // we'll point it up though if that's the only option you gave us.
    if (self.permittedArrowDirection == ABI49_0_0SMCalloutArrowDirectionUp)
        bestDirection = ABI49_0_0SMCalloutArrowDirectionUp;
    ...
    if (self.permittedArrowDirection == ABI49_0_0SMCalloutArrowDirectionAny && topSpace < self.calloutHeight && bottomSpace > topSpace)
        bestDirection = ABI49_0_0SMCalloutArrowDirectionUp;
    ...
    CGFloat anchorY = self.calloutOffset.y + (bestDirection == ABI49_0_0SMCalloutArrowDirectionDown ? CGRectGetMinY(rect) : CGRectGetMaxY(rect));
    ...
    CGPoint calloutOrigin = {
            .x = calloutX + adjustX,
            .y = bestDirection == ABI49_0_0SMCalloutArrowDirectionDown ? (anchorY - self.calloutHeight) : anchorY
    };
...
- (CAAnimation *)animationWithType:(ABI49_0_0SMCalloutAnimation)type presenting:(BOOL)presenting {
    CAAnimation *animation = nil;

    if (type == ABI49_0_0SMCalloutAnimationBounce) {
    ...
    else if (type == ABI49_0_0SMCalloutAnimationFade) {
        CABasicAnimation *fade = [CABasicAnimation animationWithKeyPath:@"opacity"];
    ...
    else if (type == ABI49_0_0SMCalloutAnimationStretch) {
...
- (void)layoutSubviews {
    ...
    CGFloat dy = self.currentArrowDirection == ABI49_0_0SMCalloutArrowDirectionUp ? TOP_ANCHOR_MARGIN : 0;
...
// import this known "private API" from ABI49_0_0SMCalloutBackgroundView
@interface ABI49_0_0SMCalloutBackgroundView (EmbeddedImages)
...
@interface ABI49_0_0SMCalloutMaskedBackgroundView ()
...
@implementation ABI49_0_0SMCalloutMaskedBackgroundView
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        if (!blackArrowImage) {
            blackArrowImage = [ABI49_0_0SMCalloutBackgroundView embeddedImageNamed:@"CalloutArrow"];
...
@implementation ABI49_0_0SMCalloutBackgroundView
...
@implementation UIView (ABI49_0_0SMFrameAdditions)
