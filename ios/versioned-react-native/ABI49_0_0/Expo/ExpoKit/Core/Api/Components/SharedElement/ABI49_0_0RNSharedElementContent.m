//
//  ABI49_0_0RNSharedElementContent_m
//  ABI49_0_0React-native-shared-element
//

#import "ABI49_0_0RNSharedElementContent.h"

@implementation ABI49_0_0RNSharedElementContent
{
  // nop
}
...
+ (BOOL) isKindOfImageView:(UIView*) view
{
  ...
  return (
          ...
          [className isEqualToString:@"ABI49_0_0RCTImageView"] ||
...
+ (UIImageView*) imageViewFromView:(UIView*) view
{
  ...
  } else if ([ABI49_0_0RNSharedElementContent isKindOfImageView:view]) {
    // Both ABI49_0_0React-native and expo-image have a UIImageView
    // as the first child.
...
{
  switch(_type) {
    case ABI49_0_0RNSharedElementContentTypeNone: return @"none";
    case ABI49_0_0RNSharedElementContentTypeRawImage: return @"image";
    case ABI49_0_0RNSharedElementContentTypeSnapshotView: return @"snapshotView";
    case ABI49_0_0RNSharedElementContentTypeSnapshotImage: return @"snapshotImage";
    default: return @"unknown";
  }
}

+ (CGSize) sizeForRect:(CGRect)layout content:(ABI49_0_0RNSharedElementContent*)content
{
  if (content == nil || content.data == nil) return layout.size;
  if (content.type != ABI49_0_0RNSharedElementContentTypeRawImage) return layout.size;
...
+ (CGRect) layoutForRect:(CGRect)layout content:(ABI49_0_0RNSharedElementContent*) content contentMode:(UIViewContentMode) contentMode reverse:(BOOL)reverse
{
  ...
  if (content.type != ABI49_0_0RNSharedElementContentTypeRawImage) return layout;
  CGSize size = layout.size;
  size.width -= (content.insets.left + content.insets.right);
  size.height -= (content.insets.top + content.insets.bottom);
  CGSize contentSize = [ABI49_0_0RNSharedElementContent sizeForRect:layout content:content];
