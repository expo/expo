...
#if __has_include(<ABI49_0_0React/ABI49_0_0RCTUIManagerUtils.h>)
#import <ABI49_0_0React/ABI49_0_0RCTUIManagerUtils.h>
#endif
#import <ABI49_0_0React/ABI49_0_0RCTBridge.h>

@implementation ABI49_0_0RNViewShot
...
ABI49_0_0RCT_EXPORT_METHOD(captureScreen: (NSDictionary *)options
...
ABI49_0_0RCT_EXPORT_METHOD(releaseCapture:(nonnull NSString *)uri)
{
  NSString *directory = [NSTemporaryDirectory() stringByAppendingPathComponent:@"ABI49_0_0ReactNative"];
...
ABI49_0_0RCT_EXPORT_METHOD(captureRef:(nonnull NSNumber *)target
                  ...
                  reject:(ABI49_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI49_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ...
    if (!view) {
      reject(ABI49_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"No view found with ABI49_0_0ReactTag: %@", target], nil);
      return;
    }

    // Get options
    CGSize size = [ABI49_0_0RCTConvert CGSize:options];
    NSString *format = [ABI49_0_0RCTConvert NSString:options[@"format"]];
    NSString *result = [ABI49_0_0RCTConvert NSString:options[@"result"]];
    BOOL renderInContext = [ABI49_0_0RCTConvert BOOL:options[@"useRenderInContext"]];
    BOOL snapshotContentContainer = [ABI49_0_0RCTConvert BOOL:options[@"snapshotContentContainer"]];
    ...
    if (snapshotContentContainer) {
      if (![view isKindOfClass:[ABI49_0_0RCTScrollView class]]) {
        reject(ABI49_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"snapshotContentContainer can only be used on a ABI49_0_0RCTScrollView. instead got: %@", view], nil);
        return;
      }
      ABI49_0_0RCTScrollView* rctScrollView = (ABI49_0_0RCTScrollView *)view;
    ...
    if (size.width < 0.1 || size.height < 0.1) {
      reject(ABI49_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"The content size must not be zero or negative. Got: (%g, %g)", size.width, size.height], nil);
      return;
    }
    ...
    if (!success) {
      reject(ABI49_0_0RCTErrorUnspecified, @"The view cannot be captured. drawViewHierarchyInRect was not successful. This is a potential technical or security limitation.", nil);
      return;
    }

    if (!image) {
      reject(ABI49_0_0RCTErrorUnspecified, @"Failed to capture view snapshot. UIGraphicsGetImageFromCurrentImageContext() returned nil!", nil);
      return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      if ([format isEqualToString:@"jpg"]) {
        CGFloat quality = [ABI49_0_0RCTConvert CGFloat:options[@"quality"]];
      ...
      if ([result isEqualToString:@"base64"]) {
        ...
        // Save to a temp file
        NSString *path = ABI49_0_0RCTTempFilePath(format, &error);
        if (path && !error) {
      ...
      if (error) reject(ABI49_0_0RCTErrorUnspecified, error.localizedDescription, error);
      else reject(ABI49_0_0RCTErrorUnspecified, @"viewshot unknown error", nil);
    });
  }];
}
