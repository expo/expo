...
#include "ABI49_0_0JSBundleType.h"
...
#include "ABI49_0_0RecoverableError.h"
#include "ABI49_0_0SystraceSection.h"

#include <ABI49_0_0cxxreact/ABI49_0_0JSIndexedRAMBundle.h>
    ...
    if (auto strongNativeToJsBridge = weakNativeToJsBridge.lock()) {
      strongNativeToJsBridge->runOnExecutorQueue(
          [callback = std::move(callback)](JSExecutor *executor) {
            ...
            try {
              ABI49_0_0React_native_assert(runtime != nullptr);
              callback(*runtime);
              executor->flush();
...
void Instance::JSCallInvoker::invokeAsync(std::function<void()> &&work) {
   ...
   *    is initialized. ABI49_0_0RCTDevLoadingView shows bundle download progress.
