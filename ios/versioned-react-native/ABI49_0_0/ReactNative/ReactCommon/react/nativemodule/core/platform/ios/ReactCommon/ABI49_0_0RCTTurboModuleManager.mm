...
#import "ABI49_0_0RCTTurboModuleManager.h"
...
#import <ABI49_0_0ReactCommon/ABI49_0_0TurboCxxModule.h>
...
void ABI49_0_0RCTTurboModuleSetBindingMode(TurboModuleBindingMode bindingMode)
{
  sTurboModuleBindingMode = bindingMode;
}

/**
 * A global variable whose address we use to associate method queues to id<ABI49_0_0RCTTurboModule> objects.
 */
...
class TurboModuleHolder {
 private:
  ...
  id<ABI49_0_0RCTTurboModule> module_;
 ...
 public:
  ...
  void setModule(id<ABI49_0_0RCTTurboModule> module)
  {
    module_ = module;
  }

  id<ABI49_0_0RCTTurboModule> getModule() const
  {
    return module_;
  }
...
class MethodQueueNativeCallInvoker : public CallInvoker {
 ...
 public:
  ...
  {
    if (methodQueue_ == ABI49_0_0RCTJSThread) {
      work();
      return;
    }
...
// Fallback lookup since ABI49_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI49_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI49_0_0RCTTurboModuleManager {
  std::shared_ptr<CallInvoker> _jsInvoker;
  __weak id<ABI49_0_0RCTTurboModuleManagerDelegate> _delegate;
  ...
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeWillInvalidateModules:)
                                                 name:ABI49_0_0RCTBridgeWillInvalidateModulesNotification
...
- (void)notifyAboutTurboModuleSetup:(const char *)name
{
  ...
  if (moduleName) {
    int64_t setupTime = [self->_bridge.performanceLogger durationForTag:ABI49_0_0RCTPLTurboModuleSetup];
...
- (std::shared_ptr<TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  id<ABI49_0_0RCTTurboModule> module = [self provideABI49_0_0RCTTurboModule:moduleName];

  TurboModulePerfLogger::moduleJSRequireEndingStart(moduleName);

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI49_0_0RCTTurboModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  if (methodQueue == nil) {
    ABI49_0_0RCTLogError(@"TurboModule \"%@\" was not associated with a method queue.", moduleClass);
  }
  ...
  /**
   * Have ABI49_0_0RCTCxxBridge decorate native CallInvoker, so that it's aware of TurboModule async method calls.
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI49_0_0RCTCxxModule.class]) {
    ...
    auto turboModule = std::make_shared<TurboCxxModule>([((ABI49_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
  ...
  ObjCTurboModule::InitParams params = {
      ...
      .isSyncModule = methodQueue == ABI49_0_0RCTJSThread,
  };
  ...
  if (turboModule == nullptr) {
    ABI49_0_0RCTLogError(@"TurboModule \"%@\"'s getTurboModule: method returned nil.", moduleClass);
  }
  _turboModuleCache.insert({moduleName, turboModule});
  return turboModule;
}
 ...
 * (for now).
 */
- (id<ABI49_0_0RCTTurboModule>)provideABI49_0_0RCTTurboModule:(const char *)moduleName
{
  if (strncmp("ABI49_0_0RCT", moduleName, 3) == 0) {
  ...
  id<ABI49_0_0RCTTurboModule> module = [self _provideABI49_0_0RCTTurboModule:moduleName moduleHolder:moduleHolder shouldPerfLog:YES];
...
- (id<ABI49_0_0RCTTurboModule>)_provideABI49_0_0RCTTurboModule:(const char *)moduleName
                               ...
                               shouldPerfLog:(BOOL)shouldPerfLog
{
  ...
  if (shouldCreateModule) {
    ...
    if ([_delegate respondsToSelector:@selector(getModuleClassFromName:)]) {
      if (ABI49_0_0RCTTurboModuleManagerDelegateLockingDisabled()) {
    ...
    __block id<ABI49_0_0RCTTurboModule> module = nil;

    if ([moduleClass conformsToProtocol:@protocol(ABI49_0_0RCTTurboModule)]) {
      ...
      dispatch_block_t work = ^{
        ...
        module = [strongSelf _createAndSetUpABI49_0_0RCTTurboModule:moduleClass
                                                moduleName:moduleName
      ...
      if ([self _requiresMainQueueSetup:moduleClass]) {
         ...
         */
        if (ABI49_0_0RCTTurboModuleEagerInitEnabled() && !ABI49_0_0RCTIsMainQueue()) {
          ABI49_0_0RCTLogWarn(
              @"TurboModule \"%@\" requires synchronous dispatch onto the main queue to be initialized. This may lead to deadlock.",
              moduleClass);
        }

        ABI49_0_0RCTUnsafeExecuteOnMainQueueSync(work);
      } else {
        work();
      }
    }
 ...
 *  - The thread that calls provideABI49_0_0RCTTurboModule:
 *  - The main thread (if the TurboModule requires main queue init), blocking the thread that calls
 * provideABI49_0_0RCTTurboModule:.
 */
- (id<ABI49_0_0RCTTurboModule>)_createAndSetUpABI49_0_0RCTTurboModule:(Class)moduleClass
                                         moduleName:(const char *)moduleName
                                           moduleId:(int32_t)moduleId
{
  id<ABI49_0_0RCTTurboModule> module = nil;
   ...
   * implementation, and a `- (ABI49_0_0RCTBridge *) bridge { ... }` method won't be
  ...
  if ([module respondsToSelector:@selector(bridge)] && _bridge) {
    ...
    @try {
      /**
       * ABI49_0_0RCTBridgeModule declares the bridge property as readonly.
       * Therefore, when authors of NativeModules synthesize the bridge
       * via @synthesize bridge = bridge;, the ObjC runtime generates
       * only a - (ABI49_0_0RCTBridge *) bridge: { ... } method. No setter is
      ...
      ABI49_0_0RCTLogError(
          ...
          ABI49_0_0RCTBridgeModuleNameForClass([module class]));
    }
  }
  ...
  if (moduleHasMethodQueueGetter) {
    methodQueue = [(id<ABI49_0_0RCTBridgeModule>)module methodQueue];
  }

  /**
   * Note: ABI49_0_0RCTJSThread, which is a valid method queue, is defined as (id)kCFNull. It should rightfully not enter the
   * following if condition's block.
   */
  if (!methodQueue) {
    NSString *methodQueueName = [NSString stringWithFormat:@"com.facebook.ABI49_0_0React.%sQueue", moduleName];
  ...
  /**
   * If the TurboModule conforms to ABI49_0_0RCTInitializing, invoke its initialize method.
   */
  if ([module respondsToSelector:@selector(initialize)]) {
    [(id<ABI49_0_0RCTInitializing>)module initialize];
  }

  /**
   * Attach method queue to id<ABI49_0_0RCTTurboModule> object.
   * This is necessary because the id<ABI49_0_0RCTTurboModule> object can be eagerly created/initialized before the method
   * queue is required. The method queue is required for an id<ABI49_0_0RCTTurboModule> for JS -> Native calls. So, we need it
   * before we create the id<ABI49_0_0RCTTurboModule>'s TurboModule jsi::HostObject in provideTurboModule:.
   */
  objc_setAssociatedObject(module, &kAssociatedMethodQueueKey, methodQueue, OBJC_ASSOCIATION_RETAIN);

  /**
   * NativeModules that implement the ABI49_0_0RCTFrameUpdateObserver protocol
   * require registration with ABI49_0_0RCTDisplayLink.
   *
   * TODO(T55504345): Investigate whether we can improve this after TM
   * rollout.
   */
  if (_bridge) {
    ABI49_0_0RCTModuleData *data = [[ABI49_0_0RCTModuleData alloc] initWithModuleInstance:(id<ABI49_0_0RCTBridgeModule>)module
                                                                 bridge:_bridge
                                                         moduleRegistry:_bridge.moduleRegistry
                                                viewRegistry_DEPRECATED:nil
                                                          bundleManager:nil
                                                      callableJSModules:nil];
    [_bridge registerModuleForFrameUpdates:(id<ABI49_0_0RCTBridgeModule>)module withModuleData:data];
  }
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI49_0_0RCTDidInitializeModuleNotification
                    object:_bridge
                  userInfo:@{@"module" : module, @"bridge" : ABI49_0_0RCTNullIfNil([_bridge parentBridge])}];
...
- (void)installJSBindingWithRuntimeExecutor:(ABI49_0_0facebook::ABI49_0_0React::RuntimeExecutor &)runtimeExecutor
{
   ...
   */
  auto turboModuleProvider = [self](const std::string &name) -> std::shared_ptr<ABI49_0_0React::TurboModule> {
    auto moduleName = name.c_str();

    TurboModulePerfLogger::moduleJSRequireBeginningStart(moduleName);
    auto moduleWasNotInitialized = ![self moduleIsInitialized:moduleName];
    if (moduleWasNotInitialized) {
      [self->_bridge.performanceLogger markStartForTag:ABI49_0_0RCTPLTurboModuleSetup];
    }
    ...
    if (moduleWasNotInitialized && [self moduleIsInitialized:moduleName]) {
      [self->_bridge.performanceLogger markStopForTag:ABI49_0_0RCTPLTurboModuleSetup];
...
#pragma mark ABI49_0_0RCTTurboModuleRegistry
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  if (warnOnLookupFailure && !module) {
    ABI49_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
{
  // Backward-compatibility: ABI49_0_0RCTInvalidating handling.
  ...
  for (auto &pair : _turboModuleHolders) {
    ...
    /**
     * We could start tearing down ABI49_0_0ReactNative before a TurboModule is fully initialized. In this case, we should wait
     * for TurboModule init to finish before calling invalidate on it. So, we call _provideABI49_0_0RCTTurboModule:moduleHolder,
     * because it's guaranteed to return a fully initialized NativeModule.
     */
    id<ABI49_0_0RCTTurboModule> module = [self _provideABI49_0_0RCTTurboModule:moduleName.c_str()
    ...
    if ([module respondsToSelector:@selector(invalidate)]) {
      ...
      dispatch_block_t invalidateModule = ^{
        [((id<ABI49_0_0RCTInvalidating>)module) invalidate];
      ...
      if (_bridge) {
        ...
        // Bridgeless mode
        if (methodQueue == ABI49_0_0RCTJSThread) {
          invalidateModule();
        } else {
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI49_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
