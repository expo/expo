...
#include "ABI49_0_0ViewPropsMapBuffer.h"

#include "ABI49_0_0viewPropConversions.h"
...
MapBuffer convertBorderWidths(ABI49_0_0YGStyle::Edges const &border) {
  MapBufferBuilder builder(7);
  putOptionalFloat(
      builder, EDGE_TOP, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeTop]));
  putOptionalFloat(
      builder, EDGE_RIGHT, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeRight]));
  putOptionalFloat(
      builder, EDGE_BOTTOM, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeBottom]));
  putOptionalFloat(
      builder, EDGE_LEFT, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeLeft]));
  putOptionalFloat(
      builder, EDGE_START, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeStart]));
  putOptionalFloat(
      builder, EDGE_END, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeEnd]));
  putOptionalFloat(
      builder, EDGE_ALL, optionalFloatFromYogaValue(border[ABI49_0_0YGEdgeAll]));
  ...
  if (oldProps.yogaStyle != newProps.yogaStyle) {
    ...
    if (!(oldStyle.border() == newStyle.border())) {
      builder.putMapBuffer(
          ABI49_0_0YG_BORDER_WIDTH, convertBorderWidths(newStyle.border()));
    }
    ...
    if (oldStyle.overflow() != newStyle.overflow()) {
      ...
      builder.putInt(ABI49_0_0YG_OVERFLOW, value);
    }
  }
}
