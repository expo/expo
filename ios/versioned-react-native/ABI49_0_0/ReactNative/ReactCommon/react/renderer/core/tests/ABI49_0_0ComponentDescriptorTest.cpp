...
TEST(ComponentDescriptorTest, createShadowNode) {
  ...
  ABI49_0_0EXPECT_EQ(descriptor->getComponentHandle(), TestShadowNode::Handle());
  ABI49_0_0EXPECT_STREQ(descriptor->getComponentName(), TestShadowNode::Name());
  ABI49_0_0EXPECT_STREQ(descriptor->getComponentName(), "Test");
  ...
  ABI49_0_0EXPECT_EQ(node->getComponentHandle(), TestShadowNode::Handle());
  ABI49_0_0EXPECT_STREQ(node->getComponentName(), TestShadowNode::Name());
  ABI49_0_0EXPECT_STREQ(node->getComponentName(), "Test");
  ABI49_0_0EXPECT_EQ(node->getTag(), 9);
  ABI49_0_0EXPECT_EQ(node->getSurfaceId(), 1);
  ABI49_0_0EXPECT_STREQ(node->getProps()->nativeId.c_str(), "abc");
}
...
TEST(ComponentDescriptorTest, cloneShadowNode) {
  ...
  ABI49_0_0EXPECT_STREQ(cloned->getComponentName(), "Test");
  ABI49_0_0EXPECT_EQ(cloned->getTag(), 9);
  ABI49_0_0EXPECT_EQ(cloned->getSurfaceId(), 1);
  ABI49_0_0EXPECT_STREQ(cloned->getProps()->nativeId.c_str(), "abc");

  auto clonedButSameProps =
      descriptor->cloneProps(parserContext, props, RawProps());
  ABI49_0_0EXPECT_NE(clonedButSameProps, props);
}
...
TEST(ComponentDescriptorTest, appendChild) {
  ...
  ABI49_0_0EXPECT_EQ(node1Children.size(), 2);
  ABI49_0_0EXPECT_EQ(node1Children.at(0), node2);
  ABI49_0_0EXPECT_EQ(node1Children.at(1), node3);
}
