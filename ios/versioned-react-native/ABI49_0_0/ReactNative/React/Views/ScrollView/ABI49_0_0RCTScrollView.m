 ...
 */
@interface ABI49_0_0RCTCustomScrollView : UIScrollView <UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
@property (nonatomic, strong) UIView<ABI49_0_0RCTCustomRefreshControlProtocol> *customRefreshControl;
@property (nonatomic, assign) BOOL pinchGestureEnabled;

@end

@implementation ABI49_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI49_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI49_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI49_0_0RCTUIManager JSResponder] isKindOfClass:[ABI49_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ABI49_0_0React scroll
...
- (void)setCustomRefreshControl:(UIView<ABI49_0_0RCTCustomRefreshControlProtocol> *)refreshControl
{
  ...
  // `ABI49_0_0RCTCustomRefreshControlProtocol`'s superview will always be of class
...
@interface ABI49_0_0RCTScrollView () <ABI49_0_0RCTUIManagerObserver>

@end

@implementation ABI49_0_0RCTScrollView {
  id<ABI49_0_0RCTEventDispatcherProtocol> _eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI49_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (void)_keyboardWillChangeFrame:(NSNotification *)notification
{
  ...
  if (@available(iOS 14.0, *)) {
    ...
    // Fixes similar issue we saw with https://github.com/facebook/ABI49_0_0React-native/pull/34503
...
- (instancetype)initWithEventDispatcher:(id<ABI49_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  ABI49_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    [self _registerKeyboardListener];
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI49_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI49_0_0RCTApplyTransformationAccordingLayoutDirection(
    UIView *view,
...
{
  [super setABI49_0_0ReactLayoutDirection:layoutDirection];

  ABI49_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI49_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
- (void)insertABI49_0_0ReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  [super insertABI49_0_0ReactSubview:view atIndex:atIndex];
  if ([view conformsToProtocol:@protocol(ABI49_0_0RCTCustomRefreshControlProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI49_0_0RCTCustomRefreshControlProtocol> *)view];
    ...
    ABI49_0_0RCTAssert(
        _contentView == nil,
        @"ABI49_0_0RCTScrollView may only contain a single subview, the already set subview looks like: %@",
        [_contentView ABI49_0_0React_recursiveDescription]);
    _contentView = view;
    ABI49_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ABI49_0_0ReactLayoutDirection);
...
- (void)removeABI49_0_0ReactSubview:(UIView *)subview
{
  ...
  if ([subview conformsToProtocol:@protocol(ABI49_0_0RCTCustomRefreshControlProtocol)]) {
    ...
    ABI49_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI49_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI49_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI49_0_0RCTCustomRefreshControlProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ABI49_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI49_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:NO];
...
{
  [ABI49_0_0RCTView autoAdjustInsetsForView:self withScrollView:_scrollView updateOffset:YES];
}

#pragma mark - ScrollView delegate

#define ABI49_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData)                                    \
  {                                                                                     \
...
#define ABI49_0_0RCT_FORWARD_SCROLL_EVENT(call)                                            \
...
#define ABI49_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
  -(void)delegateMethod : (UIScrollView *)scrollView        \
  {                                                         \
    ABI49_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                  \
    ABI49_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod : scrollView);  \
  }

ABI49_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI49_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI49_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    ABI49_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll : scrollView);
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  _allowNextScrollNoMatterWhat = YES; // Ensure next scroll event is recorded, regardless of throttle
  ABI49_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging : scrollView);
}
              ...
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI49_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging
...
{
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging : scrollView willDecelerate : decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI49_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming : scrollView withView : view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI49_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming : scrollView withView : view atScale : scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI49_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating : scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI49_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI49_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation : scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = ABI49_0_0RCTContentInsets(self);
...
- (void)uiManagerWillPerformMounting:(ABI49_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(
               __unused ABI49_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(__unused ABI49_0_0RCTUIManager *uiManager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI49_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI49_0_0RCTScrollEvent *scrollEvent = [[ABI49_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
void ABI49_0_0RCTSendFakeScrollEvent(id<ABI49_0_0RCTEventDispatcherProtocol> eventDispatcher, NSNumber *ABI49_0_0ReactTag)
{
  ...
  ABI49_0_0RCTScrollEvent *fakeScrollEvent = [[ABI49_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ABI49_0_0ReactTag:ABI49_0_0ReactTag
