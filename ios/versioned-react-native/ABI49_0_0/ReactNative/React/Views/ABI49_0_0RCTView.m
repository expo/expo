...
#import <ABI49_0_0React/ABI49_0_0RCTMockDef.h>

#import "ABI49_0_0RCTAutoInsetsProtocol.h"
#import "ABI49_0_0RCTBorderCurve.h"
#import "ABI49_0_0RCTBorderDrawing.h"
#import "ABI49_0_0RCTI18nUtil.h"
#import "ABI49_0_0RCTLog.h"
#import "ABI49_0_0RCTViewUtils.h"
#import "ABI49_0_0UIView+React.h"

ABI49_0_0RCT_MOCK_DEF(ABI49_0_0RCTView, ABI49_0_0RCTContentInsets);
#define ABI49_0_0RCTContentInsets ABI49_0_0RCT_MOCK_USE(ABI49_0_0RCTView, ABI49_0_0RCTContentInsets)

UIAccessibilityTraits const ABI49_0_0SwitchAccessibilityTrait = 0x20000000000001;

@implementation UIView (ABI49_0_0RCTViewUnmounting)

- (void)ABI49_0_0React_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI49_0_0React_remountAllSubviews];
  }
}

- (void)ABI49_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI49_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ABI49_0_0React_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isABI49_0_0ReactRootView]) {
      break;
    }
...
@implementation ABI49_0_0RCTView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderCurve = ABI49_0_0RCTBorderCurveCircular;
    _borderStyle = ABI49_0_0RCTBorderStyleSolid;
...
ABI49_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : unused)

- (void)setABI49_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ABI49_0_0ReactLayoutDirection != layoutDirection) {
    _ABI49_0_0ReactLayoutDirection = layoutDirection;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI49_0_0RCTPointerEventsNone || _pointerEvents == ABI49_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI49_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI49_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI49_0_0RCTLogInfo(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  ...
  if (actionObject) {
    _onAccessibilityAction(@{@"actionName" : actionObject[@"name"], @"actionTarget" : self.ABI49_0_0ReactTag});
  }
  return YES;
}
...
- (NSString *)accessibilityValue
{
  ...
  // Handle Switch.
  if ((self.accessibilityTraits & ABI49_0_0SwitchAccessibilityTrait) == ABI49_0_0SwitchAccessibilityTrait) {
  ...
  // Handle states which haven't already been handled in ABI49_0_0RCTViewManager.
  for (NSString *state in self.accessibilityState) {
    id val = self.accessibilityState[state];
    if (!val) {
      continue;
...
- (UIView *)ABI49_0_0ReactAccessibilityElement
{
  return self;
}
...
{
  if (self.ABI49_0_0ReactAccessibilityElement == self) {
...
- (BOOL)performAccessibilityAction:(NSString *)name
{
  if (_onAccessibilityAction && accessibilityActionsNameMap[name]) {
    _onAccessibilityAction(@{@"actionName" : name, @"actionTarget" : self.ABI49_0_0ReactTag});
    return YES;
  }
  return NO;
}
...
{
  return [[super description] stringByAppendingFormat:@" ABI49_0_0ReactTag: %@; frame = %@; layer = %@",
                   ...
                   updateOffset:(BOOL)updateOffset
{
  ...
  if (parentView.automaticallyAdjustContentInsets) {
    UIEdgeInsets autoInset = ABI49_0_0RCTContentInsets(parentView);
...
- (void)ABI49_0_0React_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ABI49_0_0ReactSubviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ABI49_0_0React_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ABI49_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI49_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ABI49_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ABI49_0_0ReactSubviews.count == 0) {
  ...
  for (UIView *view in self.ABI49_0_0ReactSubviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ABI49_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ABI49_0_0React_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI49_0_0React_findClipView];
  if (clipView) {
    [self ABI49_0_0React_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI49_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x)
{
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ABI49_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI49_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI49_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI49_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets){
        ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
        ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
        ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
        ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets){
      ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, ABI49_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
      ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI49_0_0RCTDefaultIfNegativeTo(borderWidth, ABI49_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI49_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat logicalTopStartRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderStartStartRadius, _borderTopStartRadius);
  const CGFloat logicalTopEndRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderStartEndRadius, _borderTopEndRadius);
  const CGFloat logicalBottomStartRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderEndStartRadius, _borderBottomStartRadius);
  const CGFloat logicalBottomEndRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderEndEndRadius, _borderBottomEndRadius);

  if ([[ABI49_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, logicalTopStartRadius);
    const CGFloat topEndRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, logicalTopEndRadius);
    const CGFloat bottomStartRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, logicalBottomStartRadius);
    const CGFloat bottomEndRadius = ABI49_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, logicalBottomEndRadius);
    ...
    topLeftRadius = ABI49_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI49_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI49_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI49_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius =
        ABI49_0_0RCTDefaultIfNegativeTo(radius, ABI49_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius =
        ABI49_0_0RCTDefaultIfNegativeTo(radius, ABI49_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius =
        ABI49_0_0RCTDefaultIfNegativeTo(radius, ABI49_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI49_0_0RCTDefaultIfNegativeTo(
        radius, ABI49_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI49_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI49_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI49_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI49_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));
...
- (ABI49_0_0RCTBorderColors)borderColorsWithTraitCollection:(UITraitCollection *)traitCollection
{
  const BOOL isRTL = _ABI49_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI49_0_0RCTUpdateShadowPathForView(self);

  const ABI49_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI49_0_0RCTBorderColors borderColors = [self borderColorsWithTraitCollection:self.traitCollection];

  BOOL useIOSBorderRendering = ABI49_0_0RCTCornerRadiiAreEqual(cornerRadii) && ABI49_0_0RCTBorderInsetsAreEqual(borderInsets) &&
      ABI49_0_0RCTBorderColorsAreEqual(borderColors) && _borderStyle == ABI49_0_0RCTBorderStyleSolid &&
...
static BOOL ABI49_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI49_0_0RCTUpdateShadowPathForView(ABI49_0_0RCTView *view)
{
  if (ABI49_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {
      // If view has a solid background color, calculate shadow path from border
      const ABI49_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI49_0_0RCTCornerInsets cornerInsets = ABI49_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI49_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    const ABI49_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI49_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      cornerRadius = cornerRadii.topLeft;

    } else {
      CAShapeLayer *shapeLayer = [CAShapeLayer layer];
      CGPathRef path =
          ABI49_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI49_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderCurve(side)                            \
  -(void)setBorder##side##Curve : (ABI49_0_0RCTBorderCurve)curve \
  {                                                     \
...
#define setBorderStyle(side)                            \
  -(void)setBorder##side##Style : (ABI49_0_0RCTBorderStyle)style \
  {                                                     \
