...
#import "ABI49_0_0RCTJavaScriptLoader.h"
...
#import "ABI49_0_0RCTMultipartDataTask.h"
#import "ABI49_0_0RCTPerformanceLogger.h"
#import "ABI49_0_0RCTUtils.h"

NSString *const ABI49_0_0RCTJavaScriptLoaderErrorDomain = @"ABI49_0_0RCTJavaScriptLoaderErrorDomain";

const uint32_t ABI49_0_0RCT_BYTECODE_ALIGNMENT = 4;

@interface ABI49_0_0RCTSource () {
 @public
  NSURL *_url;
  NSData *_data;
  NSUInteger _length;
...
@implementation ABI49_0_0RCTSource

static ABI49_0_0RCTSource *ABI49_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  using ABI49_0_0facebook::ABI49_0_0React::ScriptTag;
  ABI49_0_0facebook::ABI49_0_0React::BundleHeader header;
  [data getBytes:&header length:sizeof(header)];

  ABI49_0_0RCTSource *source = [ABI49_0_0RCTSource new];
  source->_url = url;
  // Multipart responses may give us an unaligned view into the buffer. This ensures memory is aligned.
  if (parseTypeFromHeader(header) == ScriptTag::MetroHBCBundle && ((long)[data bytes] % ABI49_0_0RCT_BYTECODE_ALIGNMENT)) {
    source->_data = [[NSData alloc] initWithData:data];
  } else {
    source->_data = data;
  }
  source->_length = length;
  source->_filesChangedCount = ABI49_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI49_0_0RCTLoadingProgress
...
@implementation ABI49_0_0RCTJavaScriptLoader
             ...
             onComplete:(ABI49_0_0RCTSourceLoadBlock)onComplete
{
  ...
  if (data) {
    onComplete(nil, ABI49_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError = [error.domain isEqualToString:ABI49_0_0RCTJavaScriptLoaderErrorDomain] &&
      error.code == ABI49_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI49_0_0RCTJavaScriptLoaderErrorNoScriptURL
                 userInfo:@{
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI49_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI49_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                 userInfo:@{
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI49_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                 userInfo:@{
  ...
  ABI49_0_0facebook::ABI49_0_0React::ScriptTag tag = ABI49_0_0facebook::ABI49_0_0React::parseTypeFromHeader(header);
  switch (tag) {
    case ABI49_0_0facebook::ABI49_0_0React::ScriptTag::MetroHBCBundle:
    case ABI49_0_0facebook::ABI49_0_0React::ScriptTag::RAMBundle:
      break;

    case ABI49_0_0facebook::ABI49_0_0React::ScriptTag::String: {
      ...
      if (error) {
        *error =
            [NSError errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                                code:ABI49_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI49_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                 userInfo:@{
...
static void parseHeaders(NSDictionary *headers, ABI49_0_0RCTSource *source)
{
...
static void attemptAsynchronousLoadOfBundleAtURL(
    NSURL *scriptURL,
    ABI49_0_0RCTSourceLoadProgressBlock onProgress,
    ABI49_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      NSError *error = nil;
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path options:NSDataReadingMappedIfSafe error:&error];
      onComplete(error, ABI49_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI49_0_0RCTMultipartDataTask *task = [[ABI49_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL
      partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
        ...
        if (error) {
          if ([error.domain isEqualToString:NSURLErrorDomain]) {
            error = [NSError
                errorWithDomain:ABI49_0_0RCTJavaScriptLoaderErrorDomain
                           code:ABI49_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                       userInfo:@{
                         NSLocalizedDescriptionKey :
                             [@"Could not connect to development server.\n\n"
                               "Ensure the following:\n"
                               "- Node server is running and available on the same network - run 'npm start' from ABI49_0_0React-native root\n"
        ...
        ABI49_0_0RCTSource *source = ABI49_0_0RCTSourceCreate(sourceURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI49_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI49_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI49_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI49_0_0RCTLoadingProgress *progress = [ABI49_0_0RCTLoadingProgress new];
...
static ABI49_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI49_0_0RCTLoadingProgress *progress = [ABI49_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI49_0_0RCTJSONParse(rawText, nil);
