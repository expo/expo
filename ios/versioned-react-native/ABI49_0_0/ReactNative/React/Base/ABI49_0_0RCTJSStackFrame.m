 ...
 */
static NSRegularExpression *ABI49_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI49_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI49_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI49_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI49_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI49_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI49_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI49_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI49_0_0RCTNilIfNull(dict[@"collapse"]) boolValue]];
}

+ (NSArray<ABI49_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI49_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI49_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI49_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
