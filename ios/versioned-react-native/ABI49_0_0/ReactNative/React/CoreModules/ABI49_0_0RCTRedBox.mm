...
#import <ABI49_0_0React/ABI49_0_0RCTErrorInfo.h>
#import <ABI49_0_0React/ABI49_0_0RCTEventDispatcherProtocol.h>
#import <ABI49_0_0React/ABI49_0_0RCTJSStackFrame.h>
#import <ABI49_0_0React/ABI49_0_0RCTRedBoxExtraDataViewController.h>
...
@class ABI49_0_0RCTRedBoxWindow;

@interface UIButton (ABI49_0_0RCTRedBox)

@property (nonatomic) ABI49_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI49_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI49_0_0RCTRedBox)

- (ABI49_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI49_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI49_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI49_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI49_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI49_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI49_0_0RCTRedBoxWindow : NSObject <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UIViewController *rootViewController;
@property (nonatomic, weak) id<ABI49_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI49_0_0RCTRedBoxWindow {
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI49_0_0RCTJSStackFrame *> *_lastStackTrace;
  int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame
           customButtonTitles:(NSArray<NSString *> *)customButtonTitles
         customButtonHandlers:(NSArray<ABI49_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
  if (self = [super init]) {
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title
    accessibilityIdentifier:(NSString *)accessibilityIdentifier
                   selector:(SEL)selector
                      block:(ABI49_0_0RCTRedBoxButtonPressHandler)block
{
...
{
  return ABI49_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
}
...
- (void)showErrorMessage:(NSString *)message
               withStack:(NSArray<ABI49_0_0RCTJSStackFrame *> *)stack
    ...
    if (!isRootViewControllerPresented) {
      [_stackTraceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]
                                  atScrollPosition:UITableViewScrollPositionTop
                                          animated:NO];
      [ABI49_0_0RCTKeyWindow().rootViewController presentViewController:self.rootViewController animated:YES completion:nil];
    }
  }
}
...
- (void)copyStack
{
  ...
  for (ABI49_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI49_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI49_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI49_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI49_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI49_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI49_0_0RCTKeyCommands for this, but since
...
@interface ABI49_0_0RCTRedBox () <
    ABI49_0_0RCTInvalidating,
    ABI49_0_0RCTRedBoxWindowActionDelegate,
    ABI49_0_0RCTRedBoxExtraDataActionDelegate,
    ABI49_0_0NativeRedBoxSpec>
@end

@implementation ABI49_0_0RCTRedBox {
  ABI49_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI49_0_0RCTErrorCustomizer>> *_errorCustomizers;
  ABI49_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
  NSMutableArray<NSString *> *_customButtonTitles;
  NSMutableArray<ABI49_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (void)registerErrorCustomizer:(id<ABI49_0_0RCTErrorCustomizer>)errorCustomizer
{
...
- (ABI49_0_0RCTErrorInfo *)_customizeError:(ABI49_0_0RCTErrorInfo *)error
{
  ABI49_0_0RCTAssertMainQueue();
  if (!self->_errorCustomizers) {
    return error;
  }
  for (id<ABI49_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI49_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI49_0_0RCTJSStackTraceKey]
             errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message
             withDetails:(NSString *)details
                   stack:(NSArray<ABI49_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
...
{
  NSArray<ABI49_0_0RCTJSStackFrame *> *stack = [ABI49_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
  [self showErrorMessage:message
         withParsedStack:[ABI49_0_0RCTJSStackFrame stackFramesWithDictionaries:stack]
                isUpdate:NO
...
- (void)showErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI49_0_0RCTJSStackFrame *> *)stack
{
  [self showErrorMessage:message withParsedStack:stack errorCookie:-1];
}

- (void)updateErrorMessage:(NSString *)message withParsedStack:(NSArray<ABI49_0_0RCTJSStackFrame *> *)stack
{
  [self updateErrorMessage:message withParsedStack:stack errorCookie:-1];
}

- (void)showErrorMessage:(NSString *)message
         withParsedStack:(NSArray<ABI49_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
  [self showErrorMessage:message withParsedStack:stack isUpdate:NO errorCookie:errorCookie];
}
             ...
             errorCookie:(int)errorCookie
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self->_extraDataViewController == nil) {
      self->_extraDataViewController = [ABI49_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
    ...
    if (!self->_window) {
      self->_window = [[ABI49_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds
    ...
    ABI49_0_0RCTErrorInfo *errorInfo = [[ABI49_0_0RCTErrorInfo alloc] initWithErrorMessage:message stack:stack];
...
ABI49_0_0RCT_EXPORT_METHOD(setExtraData : (NSDictionary *)extraData forIdentifier : (NSString *)identifier)
{
  [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI49_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI49_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI49_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bundleManager.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI49_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI49_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI49_0_0RCTRedBoxWindow *)redBoxWindow
{
  if (_overrideReloadAction) {
    _overrideReloadAction();
  } else {
    ABI49_0_0RCTTriggerReloadCommandListeners(@"Redbox");
  }
  [self dismiss];
}

- (void)addCustomButton:(NSString *)title onPressHandler:(ABI49_0_0RCTRedBoxButtonPressHandler)handler
{
...
{
  return std::make_shared<ABI49_0_0facebook::ABI49_0_0React::NativeRedBoxSpecJSI>(params);
}

@end

@implementation ABI49_0_0RCTBridge (ABI49_0_0RCTRedBox)

- (ABI49_0_0RCTRedBox *)redBox
{
  return ABI49_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI49_0_0RCTRedBox class]] : nil;
}
...
@interface ABI49_0_0RCTRedBox () <ABI49_0_0NativeRedBoxSpec>
@end
...
Class ABI49_0_0RCTRedBoxCls(void)
{
  return ABI49_0_0RCTRedBox.class;
}
