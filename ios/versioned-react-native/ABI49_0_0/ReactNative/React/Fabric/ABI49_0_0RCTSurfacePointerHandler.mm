...
#import "ABI49_0_0RCTSurfacePointerHandler.h"

#import <ABI49_0_0React/ABI49_0_0RCTIdentifierPool.h>
#import <ABI49_0_0React/ABI49_0_0RCTABI49_0_0ReactTaggedView.h>
...
#import "ABI49_0_0RCTTouchableComponentViewProtocol.h"

using namespace ABI49_0_0facebook::ABI49_0_0React;

typedef NS_ENUM(NSInteger, ABI49_0_0RCTPointerEventType) {
  ABI49_0_0RCTPointerEventTypeStart,
  ABI49_0_0RCTPointerEventTypeMove,
  ABI49_0_0RCTPointerEventTypeEnd,
  ABI49_0_0RCTPointerEventTypeCancel,
};
...
struct ActivePointer {
   ...
   */
  UIView<ABI49_0_0RCTComponentViewProtocol> *initialComponentView = nil;

  /*
   * The current target component view of the pointer
   */
  UIView<ABI49_0_0RCTComponentViewProtocol> *componentView = nil;
...
{
  return [(id<ABI49_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitterAtPoint:point];
}

static NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *GetTouchableViewsInPathToRoot(UIView *componentView)
{
  NSMutableOrderedSet *results = [NSMutableOrderedSet orderedSet];
  do {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      [results addObject:[ABI49_0_0RCTABI49_0_0ReactTaggedView wrap:componentView]];
    }
...
static PointerEvent CreatePointerEventFromActivePointer(
    ...
    ABI49_0_0RCTPointerEventType eventType,
    UIView *rootComponentView)
{
  ...
  if (eventType == ABI49_0_0RCTPointerEventTypeCancel) {
    event.clientPoint = ABI49_0_0RCTPointFromCGPoint(CGPointZero);
    event.screenPoint =
        ABI49_0_0RCTPointFromCGPoint([rootComponentView convertPoint:CGPointZero
                                          toCoordinateSpace:rootComponentView.window.screen.coordinateSpace]);
    event.offsetPoint = ABI49_0_0RCTPointFromCGPoint([rootComponentView convertPoint:CGPointZero
                                                                     toView:activePointer.componentView]);
  } else {
    event.clientPoint = ABI49_0_0RCTPointFromCGPoint(activePointer.clientPoint);
    event.screenPoint = ABI49_0_0RCTPointFromCGPoint(activePointer.screenPoint);
    event.offsetPoint = ABI49_0_0RCTPointFromCGPoint(activePointer.offsetPoint);
  }
  ...
  if (@available(iOS 13.4, *)) {
    if (activePointer.touchType == UITouchTypeIndirectPointer) {
      ...
      event.pressure = eventType != ABI49_0_0RCTPointerEventTypeEnd ? 0.5 : 0.0;
    }
  }
    ...
    UIKeyModifierFlags modifierFlags)
{
  ...
  event.clientPoint = ABI49_0_0RCTPointFromCGPoint(clientLocation);
  event.screenPoint = ABI49_0_0RCTPointFromCGPoint(screenLocation);
  event.offsetPoint = ABI49_0_0RCTPointFromCGPoint(offsetLocation);
  event.width = 1.0;
  event.height = 1.0;
  event.tiltX = 0;
  event.tiltY = 0;
    ...
    UIView *rootComponentView)
{
  ...
  activePointer.force = ABI49_0_0RCTZeroIfNaN(uiTouch.force / uiTouch.maximumPossibleForce);
...
static BOOL IsViewListeningToEvent(ABI49_0_0RCTABI49_0_0ReactTaggedView *taggedView, ViewEvents::Offset eventType)
{
  UIView *view = taggedView.view;
  if (view && [view.class conformsToProtocol:@protocol(ABI49_0_0RCTComponentViewProtocol)]) {
    auto props = ((id<ABI49_0_0RCTComponentViewProtocol>)view).props;
...
static BOOL IsAnyViewInPathListeningToEvent(NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *viewPath, ViewEvents::Offset eventType)
{
  for (ABI49_0_0RCTABI49_0_0ReactTaggedView *taggedView in viewPath) {
...
@interface ABI49_0_0RCTSurfacePointerHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI49_0_0RCTSurfacePointerHandler {
  ...
  ABI49_0_0RCTIdentifierPool<11> _identifierPool;
  ...
  NSMutableDictionary<NSNumber *, NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *> *_currentlyHoveredViewsPerPointer;
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI49_0_0React Native,
...
{
  ABI49_0_0RCTAssert(self.view == nil, @"ABI49_0_0RCTSurfacePointerHandler already has an attached view.");
...
- (void)detachFromView:(UIView *)view
{
  ...
  ABI49_0_0RCTAssert(self.view == view, @"ABI49_0_0RCTSufracePointerHandler attached to another view.");
...
- (void)_updateTouches:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  for (UITouch *touch in touches) {
    ...
    ABI49_0_0RCTAssert(iterator != _activePointers.end(), @"Inconsistency between local and UIKit touch registries");
...
- (void)_dispatchActivePointers:(std::vector<ActivePointer>)activePointers eventType:(ABI49_0_0RCTPointerEventType)eventType
{
  for (const auto &activePointer : activePointers) {
    ...
    NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *eventPathViews = [self handleIncomingPointerEvent:pointerEvent
    ...
    if (eventEmitter != nil) {
      switch (eventType) {
        case ABI49_0_0RCTPointerEventTypeStart: {
          eventEmitter->onPointerDown(pointerEvent);
          break;
        }
        case ABI49_0_0RCTPointerEventTypeMove: {
        ...
        case ABI49_0_0RCTPointerEventTypeEnd: {
        ...
        case ABI49_0_0RCTPointerEventTypeCancel: {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI49_0_0RCTPointerEventTypeStart];
}
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI49_0_0RCTPointerEventTypeMove];
}
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI49_0_0RCTPointerEventTypeEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI49_0_0RCTPointerEventTypeCancel];
  [self _unregisterTouches:touches];
}
...
- (void)reset
{
  ...
  if (!_activePointers.empty()) {
    ...
    [self _dispatchActivePointers:activePointers eventType:ABI49_0_0RCTPointerEventTypeCancel];
     ...
     pointerType:(std::string)pointerType API_AVAILABLE(ios(13.0))
{
  ...
  NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *eventPathViews = [self handleIncomingPointerEvent:event onView:targetView];
 ...
 * should be fired.
 */
- (NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *)handleIncomingPointerEvent:(PointerEvent)event
                                                            onView:(nullable UIView *)targetView
{
  ...
  NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *currentlyHoveredViews =
  ...
  ABI49_0_0RCTABI49_0_0ReactTaggedView *targetTaggedView = [ABI49_0_0RCTABI49_0_0ReactTaggedView wrap:targetView];
  ABI49_0_0RCTABI49_0_0ReactTaggedView *prevTargetTaggedView = [currentlyHoveredViews firstObject];
  UIView *prevTargetView = prevTargetTaggedView.view;

  NSOrderedSet<ABI49_0_0RCTABI49_0_0ReactTaggedView *> *eventPathViews = GetTouchableViewsInPathToRoot(targetView);
  ...
  for (ABI49_0_0RCTABI49_0_0ReactTaggedView *taggedView in [currentlyHoveredViews reverseObjectEnumerator]) {
  ...
  for (ABI49_0_0RCTABI49_0_0ReactTaggedView *taggedView in [eventPathViews reverseObjectEnumerator]) {
    UIView *componentView = taggedView.view;
