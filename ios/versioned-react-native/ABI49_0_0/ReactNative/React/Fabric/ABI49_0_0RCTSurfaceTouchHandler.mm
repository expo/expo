...
#import "ABI49_0_0RCTSurfaceTouchHandler.h"
...
typedef NS_ENUM(NSInteger, ABI49_0_0RCTTouchEventType) {
  ABI49_0_0RCTTouchEventTypeTouchStart,
  ABI49_0_0RCTTouchEventTypeTouchMove,
  ABI49_0_0RCTTouchEventTypeTouchEnd,
  ABI49_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI49_0_0RCTComponentViewProtocol> *componentView = nil;
    ...
    CGPoint rootViewOriginOffset)
{
  ...
  activeTouch.touch.offsetPoint = ABI49_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI49_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI49_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI49_0_0RCTForceTouchAvailable()) {
    activeTouch.touch.force = ABI49_0_0RCTZeroIfNaN(uiTouch.force / uiTouch.maximumPossibleForce);
  }
}
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView, CGPoint rootViewOriginOffset)
{
  ...
  while (componentView) {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      activeTouch.eventEmitter = [(id<ABI49_0_0RCTTouchableComponentViewProtocol>)componentView
...
@interface ABI49_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI49_0_0RCTSurfaceTouchHandler {
  ...
  ABI49_0_0RCTSurfacePointerHandler *_pointerHandler;
}
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    if (ABI49_0_0RCTGetDispatchW3CPointerEvents()) {
      _pointerHandler = [[ABI49_0_0RCTSurfacePointerHandler alloc] init];
    }
  }
...
- (void)_updateTouches:(NSSet<UITouch *> *)touches
{
  for (UITouch *touch in touches) {
    ...
    ABI49_0_0RCTAssert(iterator != _activeTouches.end(), @"Inconsistency between local and UIKit touch registries");
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI49_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI49_0_0RCTTouchEventTypeTouchEnd || eventType == ABI49_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitters) {
    ...
    switch (eventType) {
      case ABI49_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI49_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI49_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI49_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI49_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI49_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI49_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI49_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (!_activeTouches.empty()) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI49_0_0RCTTouchEventTypeTouchCancel];
