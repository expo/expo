...
#import "ABI49_0_0RCTLegacyViewManagerInteropComponentView.h"
...
#import <ABI49_0_0React/ABI49_0_0renderer/components/legacyviewmanagerinterop/LegacyViewManagerInteropComponentDescriptor.h>
#import <ABI49_0_0React/ABI49_0_0renderer/components/legacyviewmanagerinterop/LegacyViewManagerInteropViewProps.h>
...
static NSString *const kABI49_0_0RCTLegacyInteropChildComponentKey = @"childComponentView";
static NSString *const kABI49_0_0RCTLegacyInteropChildIndexKey = @"index";

@implementation ABI49_0_0RCTLegacyViewManagerInteropComponentView {
  NSMutableArray<NSDictionary *> *_viewsToBeMounted;
  NSMutableArray<UIView *> *_viewsToBeUnmounted;
  ABI49_0_0RCTLegacyViewManagerInteropCoordinatorAdapter *_adapter;
...
{
  const auto &state = _state->getData();
  ABI49_0_0RCTLegacyViewManagerInteropCoordinator *coordinator = unwrapManagedObject(state.coordinator);
...
- (void)forwardInvocation:(NSInvocation *)anInvocation
{
  if (!_hasInvokedForwardingWarning) {
    ...
    NSLog(
        @"Invoked unsupported method on ABI49_0_0RCTLegacyViewManagerInteropComponentView. Resulting to noop instead of a crash.");
  }
}
...
+ (BOOL)isSupported:(NSString *)componentName
{
  ...
  BOOL isComponentNameSupported =
      [[ABI49_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] containsObject:componentName];
  if (isComponentNameSupported) {
    return YES;
  }

  // Step 2: check if component has supported prefix.
  for (NSString *item in [ABI49_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagersPrefixes]) {
...
{
  [[ABI49_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagersPrefixes] addObject:prefix];
}

+ (void)supportLegacyViewManagerWithName:(NSString *)componentName
{
  [[ABI49_0_0RCTLegacyViewManagerInteropComponentView supportedViewManagers] addObject:componentName];
}
...
- (void)mountChildComponentView:(UIView<ABI49_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  [_viewsToBeMounted addObject:@{
    kABI49_0_0RCTLegacyInteropChildIndexKey : [NSNumber numberWithInteger:index],
    kABI49_0_0RCTLegacyInteropChildComponentKey : childComponentView
  }];
}
...
{
  if (_adapter) {
    [_adapter.paperView removeABI49_0_0ReactSubview:childComponentView];
  } else {
...
- (void)finalizeUpdates:(ABI49_0_0RNComponentViewUpdateMask)updateMask
{
  ...
  if (!_adapter) {
    _adapter = [[ABI49_0_0RCTLegacyViewManagerInteropCoordinatorAdapter alloc] initWithCoordinator:[self _coordinator]
                                                                                 ABI49_0_0ReactTag:self.tag];
  ...
  for (NSDictionary *mountInstruction in _viewsToBeMounted) {
    NSNumber *index = mountInstruction[kABI49_0_0RCTLegacyInteropChildIndexKey];
    UIView *childView = mountInstruction[kABI49_0_0RCTLegacyInteropChildComponentKey];
    if ([childView isKindOfClass:[ABI49_0_0RCTLegacyViewManagerInteropComponentView class]]) {
      UIView *target = ((ABI49_0_0RCTLegacyViewManagerInteropComponentView *)childView).contentView;
      [_adapter.paperView insertABI49_0_0ReactSubview:target atIndex:index.integerValue];
    } else {
      [_adapter.paperView insertABI49_0_0ReactSubview:childView atIndex:index.integerValue];
    }
  }
  ...
  for (UIView *view in _viewsToBeUnmounted) {
    [_adapter.paperView removeABI49_0_0ReactSubview:view];
  }

  [_viewsToBeUnmounted removeAllObjects];

  [_adapter.paperView didUpdateABI49_0_0ReactSubviews];

  if (updateMask & ABI49_0_0RNComponentViewUpdateMaskProps) {
