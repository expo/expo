...
@interface ABI49_0_0RCTSpringAnimation ()

@property (nonatomic, strong) NSNumber *animationId;
@property (nonatomic, strong) ABI49_0_0RCTValueAnimatedNode *valueNode;
@property (nonatomic, assign) BOOL animationHasBegun;
@property (nonatomic, assign) BOOL animationHasFinished;

@end

const NSTimeInterval ABI49_0_0MAX_DELTA_TIME = 0.064;

@implementation ABI49_0_0RCTSpringAnimation {
  CGFloat _toValue;
  CGFloat _fromValue;
  ...
  ABI49_0_0RCTResponseSenderBlock _callback;
                  ...
                  callBack:(nullable ABI49_0_0RCTResponseSenderBlock)callback
{
  if ((self = [super init])) {
    ...
    _lastVelocity = [ABI49_0_0RCTConvert CGFloat:config[@"initialVelocity"]];
...
{
  NSNumber *iterations = [ABI49_0_0RCTConvert NSNumber:config[@"iterations"]] ?: @1;
  _toValue = [ABI49_0_0RCTConvert CGFloat:config[@"toValue"]];
  _overshootClamping = [ABI49_0_0RCTConvert BOOL:config[@"overshootClamping"]];
  _restDisplacementThreshold = [ABI49_0_0RCTConvert CGFloat:config[@"restDisplacementThreshold"]];
  _restSpeedThreshold = [ABI49_0_0RCTConvert CGFloat:config[@"restSpeedThreshold"]];
  _stiffness = [ABI49_0_0RCTConvert CGFloat:config[@"stiffness"]];
  _damping = [ABI49_0_0RCTConvert CGFloat:config[@"damping"]];
  _mass = [ABI49_0_0RCTConvert CGFloat:config[@"mass"]];
...
- (void)stepAnimationWithTime:(NSTimeInterval)currentTime
{
  ...
  if (_animationStartTime == -1) {
    ...
    // Handle frame drops, and only advance dt by a max of ABI49_0_0MAX_DELTA_TIME
    NSTimeInterval deltaTime = MIN(ABI49_0_0MAX_DELTA_TIME, currentTime - _animationCurrentTime);
    _t = _t + deltaTime / ABI49_0_0RCTAnimationDragCoefficient();
  }
