...
@interface ABI49_0_0RCTHTTPRequestHandler () <NSURLSessionDataDelegate, ABI49_0_0RCTTurboModule>

@end

static NSURLSessionConfigurationProvider urlSessionConfigurationProvider;

void ABI49_0_0RCTSetCustomNSURLSessionConfigurationProvider(NSURLSessionConfigurationProvider provider)
{
  urlSessionConfigurationProvider = provider;
}

@implementation ABI49_0_0RCTHTTPRequestHandler {
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  dispatch_once(&onceToken, ^{
    // technically, ABI49_0_0RCTHTTPRequestHandler can handle file:// as well,
    // but it's less efficient than using ABI49_0_0RCTFileRequestHandler
    schemes = [[NSSet alloc] initWithObjects:@"http", @"https", nil];
  });
  return [schemes containsObject:request.URL.scheme.lowercaseString];
}

- (NSURLSessionDataTask *)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI49_0_0RCTURLRequestDelegate>)delegate
{
  std::lock_guard<std::mutex> lock(_mutex);
  // Lazy setup
  if (!_session && [self isValid]) {
    // You can override default NSURLSession instance property allowsCellularAccess (default value YES)
    //  by providing the following key to your ABI49_0_0RN project (edit ios/project/Info.plist file in Xcode):
    // <key>ABI49_0_0ReactNetworkForceWifiOnly</key>    <true/>
    ...
    NSNumber *useWifiOnly = [infoDictionary objectForKey:@"ABI49_0_0ReactNetworkForceWifiOnly"];
    ...
    if (urlSessionConfigurationProvider) {
      ...
      // Set allowsCellularAccess to NO ONLY if key ABI49_0_0ReactNetworkForceWifiOnly exists AND its value is YES
      if (useWifiOnly) {
...
{
  id<ABI49_0_0RCTURLRequestDelegate> delegate;
  {
    std::lock_guard<std::mutex> lock(_mutex);
...
Class ABI49_0_0RCTHTTPRequestHandlerCls(void)
{
  return ABI49_0_0RCTHTTPRequestHandler.class;
}
