...
typedef ABI49_0_0RCTURLRequestCancellationBlock (^ABI49_0_0RCTHTTPQueryResult)(NSError *error, NSDictionary<NSString *, id> *result);

NSString *const ABI49_0_0RCTNetworkingPHUploadHackScheme = @"ph-upload";

@interface ABI49_0_0RCTNetworking () <ABI49_0_0NativeNetworkingIOSSpec>

- (ABI49_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(NSDictionary<NSString *, id> *)data
                                                 callback:(ABI49_0_0RCTHTTPQueryResult)callback;
@end

/**
 * Helper to convert FormData payloads into multipart/formdata requests.
 */
@interface ABI49_0_0RCTHTTPFormDataHelper : NSObject

@property (nonatomic, weak) ABI49_0_0RCTNetworking *networker;

@end

@implementation ABI49_0_0RCTHTTPFormDataHelper {
  NSMutableArray<NSDictionary<NSString *, id> *> *_parts;
  NSMutableData *_multipartBody;
  ABI49_0_0RCTHTTPQueryResult _callback;
  NSString *_boundary;
}

static NSString *ABI49_0_0RCTGenerateFormBoundary()
{
...
- (ABI49_0_0RCTURLRequestCancellationBlock)process:(NSArray<NSDictionary *> *)formData callback:(ABI49_0_0RCTHTTPQueryResult)callback
{
  ABI49_0_0RCTAssertThread(_networker.methodQueue, @"process: must be called on method queue");
  ...
  _boundary = ABI49_0_0RCTGenerateFormBoundary();
  ...
  for (NSUInteger i = 0; i < _parts.count; i++) {
    ...
    if (uri && [[uri substringToIndex:@"ph:".length] caseInsensitiveCompare:@"ph:"] == NSOrderedSame) {
      uri = [ABI49_0_0RCTNetworkingPHUploadHackScheme stringByAppendingString:[uri substringFromIndex:@"ph".length]];
...
- (ABI49_0_0RCTURLRequestCancellationBlock)handleResult:(NSDictionary<NSString *, id> *)result error:(NSError *)error
{
  ABI49_0_0RCTAssertThread(_networker.methodQueue, @"handleResult: must be called on method queue");
 ...
 */
@implementation ABI49_0_0RCTNetworking {
  NSMutableDictionary<NSNumber *, ABI49_0_0RCTNetworkTask *> *_tasksByRequestID;
  std::mutex _handlersLock;
  NSArray<id<ABI49_0_0RCTURLRequestHandler>> *_handlers;
  NSArray<id<ABI49_0_0RCTURLRequestHandler>> * (^_handlersProvider)(ABI49_0_0RCTModuleRegistry *);
  NSMutableArray<id<ABI49_0_0RCTNetworkingRequestHandler>> *_requestHandlers;
  NSMutableArray<id<ABI49_0_0RCTNetworkingResponseHandler>> *_responseHandlers;
}
...
- (instancetype)initWithHandlersProvider:
    (NSArray<id<ABI49_0_0RCTURLRequestHandler>> * (^)(ABI49_0_0RCTModuleRegistry *moduleRegistry))getHandlers
{
...
- (id<ABI49_0_0RCTURLRequestHandler>)handlerForRequest:(NSURLRequest *)request
{
  if (!request.URL) {
    return nil;
  }
  NSArray<id<ABI49_0_0RCTURLRequestHandler>> *handlers = [self prioritizedHandlers];

  if (ABI49_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI49_0_0RCTURLRequestHandler> previousHandler = nil;
    for (id<ABI49_0_0RCTURLRequestHandler> handler in handlers) {
        ...
        if (previousHandler) {
          if (priority == previousPriority) {
            ABI49_0_0RCTLogError(
                @"The ABI49_0_0RCTURLRequestHandlers %@ and %@ both reported that"
...
- (NSArray<id<ABI49_0_0RCTURLRequestHandler>> *)prioritizedHandlers
{
  std::lock_guard<std::mutex> lock(_handlersLock);
  if (_handlers) {
    return _handlers;
  ...
  NSArray<id<ABI49_0_0RCTURLRequestHandler>> *newHandlers = _handlersProvider
      ? _handlersProvider(self.moduleRegistry)
      : [self.bridge modulesConformingToProtocol:@protocol(ABI49_0_0RCTURLRequestHandler)];

  // Get handlers, sorted in reverse priority order (highest priority first)
  newHandlers = [newHandlers
      sortedArrayUsingComparator:^NSComparisonResult(id<ABI49_0_0RCTURLRequestHandler> a, id<ABI49_0_0RCTURLRequestHandler> b) {
...
- (ABI49_0_0RCTURLRequestCancellationBlock)buildRequest:(NSDictionary<NSString *, id> *)query
                               completionBlock:(void (^)(NSURLRequest *request))block
{
  ABI49_0_0RCTAssertThread(_methodQueue, @"buildRequest: must be called on method queue");

  NSURL *URL = [ABI49_0_0RCTConvert NSURL:query[@"url"]]; // this is marked as nullable in JS, but should not be null
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:URL];
  request.HTTPMethod = [ABI49_0_0RCTConvert NSString:ABI49_0_0RCTNilIfNull(query[@"method"])].uppercaseString ?: @"GET";
  request.HTTPShouldHandleCookies = [ABI49_0_0RCTConvert BOOL:query[@"withCredentials"]];
  ...
  NSDictionary *headers = [ABI49_0_0RCTConvert NSDictionary:query[@"headers"]];
  ...
  request.timeoutInterval = [ABI49_0_0RCTConvert NSTimeInterval:query[@"timeout"]];
  NSDictionary<NSString *, id> *data = [ABI49_0_0RCTConvert NSDictionary:ABI49_0_0RCTNilIfNull(query[@"data"])];
  ...
  return [self processDataForHTTPQuery:data
                              callback:^(NSError *error, NSDictionary<NSString *, id> *result) {
                                if (error) {
                                  ABI49_0_0RCTLogError(@"Error processing request body: %@", error);
                                  // Ideally we'd circle back to JS here and notify an error/abort on the request.
                                  return (ABI49_0_0RCTURLRequestCancellationBlock)nil;
                                }
                                ...
                                if ([request.allHTTPHeaderFields[@"Content-Encoding"] isEqualToString:@"gzip"]) {
                                  request.HTTPBody = ABI49_0_0RCTGzipData(request.HTTPBody, -1 /* default */);
 ...
 *
 */
- (ABI49_0_0RCTURLRequestCancellationBlock)
    processDataForHTTPQuery:(nullable NSDictionary<NSString *, id> *)query
                   callback:(ABI49_0_0RCTURLRequestCancellationBlock (^)(NSError *error, NSDictionary<NSString *, id> *result))
                                callback
{
  ABI49_0_0RCTAssertThread(_methodQueue, @"processDataForHTTPQuery: must be called on method queue");

  if (!query) {
    return callback(nil, nil);
  }
  for (id<ABI49_0_0RCTNetworkingRequestHandler> handler in _requestHandlers) {
  ...
  NSData *body = [ABI49_0_0RCTConvert NSData:query[@"string"]];
  if (body) {
    return callback(nil, @{@"body" : body});
  }
  NSString *base64String = [ABI49_0_0RCTConvert NSString:query[@"base64"]];
  if (base64String) {
    NSData *data = [[NSData alloc] initWithBase64EncodedString:base64String options:0];
    return callback(nil, @{@"body" : data});
  }
  NSURLRequest *request = [ABI49_0_0RCTConvert NSURLRequest:query[@"uri"]];
  if (request) {
    __block ABI49_0_0RCTURLRequestCancellationBlock cancellationBlock = nil;
    ABI49_0_0RCTNetworkTask *task =
        [self networkTaskWithRequest:request
                     completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                       dispatch_async(self->_methodQueue, ^{
                         cancellationBlock = callback(
                             error, data ? @{@"body" : data, @"contentType" : ABI49_0_0RCTNullIfNil(response.MIMEType)} : nil);
                       });
                     }];

    [task start];

    __weak ABI49_0_0RCTNetworkTask *weakTask = task;
    return ^{
      [weakTask cancel];
  ...
  NSArray<NSDictionary *> *formData = [ABI49_0_0RCTConvert NSDictionaryArray:query[@"formData"]];
  if (formData) {
    ABI49_0_0RCTHTTPFormDataHelper *formDataHelper = [ABI49_0_0RCTHTTPFormDataHelper new];
...
- (void)sendData:(NSData *)data
    responseType:(NSString *)responseType
        response:(NSURLResponse *)response
         forTask:(ABI49_0_0RCTNetworkTask *)task
{
  ABI49_0_0RCTAssertThread(_methodQueue, @"sendData: must be called on method queue");

  id responseData = nil;
  for (id<ABI49_0_0RCTNetworkingResponseHandler> handler in _responseHandlers) {
  ...
  if (!responseData) {
    ...
    if ([responseType isEqualToString:@"text"]) {
      ...
      responseData = [ABI49_0_0RCTNetworking decodeTextData:data fromResponse:task.response withCarryData:nil];
      if (!responseData) {
        ABI49_0_0RCTLogWarn(@"Received data was not a string, or was not a recognised encoding.");
        return;
      }
    } else if ([responseType isEqualToString:@"base64"]) {
      responseData = [data base64EncodedStringWithOptions:0];
    } else {
      ABI49_0_0RCTLogWarn(@"Invalid responseType: %@", responseType);
      return;
    }
  }
...
- (void)sendRequest:(NSURLRequest *)request
    ...
    incrementalUpdates:(BOOL)incrementalUpdates
        responseSender:(ABI49_0_0RCTResponseSenderBlock)responseSender
{
  ABI49_0_0RCTAssertThread(_methodQueue, @"sendRequest: must be called on method queue");
  __weak __typeof(self) weakSelf = self;
  __block ABI49_0_0RCTNetworkTask *task;
  ABI49_0_0RCTURLRequestProgressBlock uploadProgressBlock = ^(int64_t progress, int64_t total) {
    NSArray *responseJSON = @[ task.requestID, @((double)progress), @((double)total) ];
    [weakSelf sendEventWithName:@"didSendNetworkData" body:responseJSON];
  };

  ABI49_0_0RCTURLRequestResponseBlock responseBlock = ^(NSURLResponse *response) {
  ...
  ABI49_0_0RCTURLRequestIncrementalDataBlock incrementalDataBlock = nil;
  ABI49_0_0RCTURLRequestProgressBlock downloadProgressBlock = nil;
  if (incrementalUpdates) {
    if ([responseType isEqualToString:@"text"]) {
      ...
      incrementalDataBlock = ^(NSData *data, int64_t progress, int64_t total) {
        ...
        NSString *responseString = [ABI49_0_0RCTNetworking decodeTextData:data
  ...
  ABI49_0_0RCTURLRequestCompletionBlock completionBlock = ^(NSURLResponse *response, NSData *data, NSError *error) {
    ...
    NSArray *responseJSON =
        @[ task.requestID, ABI49_0_0RCTNullIfNil(error.localizedDescription), error.code == kCFURLErrorTimedOut ? @YES : @NO ];
...
{
  id<ABI49_0_0RCTURLRequestHandler> handler = [self handlerForRequest:request];
  if (!handler) {
    ABI49_0_0RCTLogError(@"No suitable URL request handler found for %@", request.URL);
    return nil;
  }

  ABI49_0_0RCTNetworkTask *task = [[ABI49_0_0RCTNetworkTask alloc] initWithRequest:request handler:handler callbackQueue:_methodQueue];
...
ABI49_0_0RCT_EXPORT_METHOD(sendRequest
                  : (ABI49_0_0JS::NativeNetworkingIOS::SpecSendRequestQuery &)query callback
                  : (ABI49_0_0RCTResponseSenderBlock)responseSender)
{
  ...
  [self buildRequest:queryDict
      completionBlock:^(NSURLRequest *request) {
        NSString *responseType = [ABI49_0_0RCTConvert NSString:queryDict[@"responseType"]];
        BOOL incrementalUpdates = [ABI49_0_0RCTConvert BOOL:queryDict[@"incrementalUpdates"]];
...
ABI49_0_0RCT_EXPORT_METHOD(abortRequest : (double)requestID)
{
...
ABI49_0_0RCT_EXPORT_METHOD(clearCookies : (ABI49_0_0RCTResponseSenderBlock)responseSender)
{
...
{
  return std::make_shared<ABI49_0_0facebook::ABI49_0_0React::NativeNetworkingIOSSpecJSI>(params);
}

@end

@implementation ABI49_0_0RCTBridge (ABI49_0_0RCTNetworking)

- (ABI49_0_0RCTNetworking *)networking
{
  return [self moduleForClass:[ABI49_0_0RCTNetworking class]];
}

@end

Class ABI49_0_0RCTNetworkingCls(void)
{
  return ABI49_0_0RCTNetworking.class;
}
