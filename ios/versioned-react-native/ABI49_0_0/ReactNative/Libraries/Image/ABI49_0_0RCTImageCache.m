...
static NSUInteger ABI49_0_0RCTMaxCacheableDecodedImageSizeInBytes = 2 * 1024 * 1024;
static NSUInteger ABI49_0_0RCTImageCacheTotalCostLimit = 20 * 1024 * 1024;

void ABI49_0_0RCTSetImageCacheLimits(NSUInteger maxCacheableDecodedImageSizeInBytes, NSUInteger imageCacheTotalCostLimit)
{
  ABI49_0_0RCTMaxCacheableDecodedImageSizeInBytes = maxCacheableDecodedImageSizeInBytes;
  ABI49_0_0RCTImageCacheTotalCostLimit = imageCacheTotalCostLimit;
}

static NSString *ABI49_0_0RCTCacheKeyForImage(NSString *imageTag, CGSize size, CGFloat scale, ABI49_0_0RCTResizeMode resizeMode)
{
  return
      [NSString stringWithFormat:@"%@|%g|%g|%g|%lld", imageTag, size.width, size.height, scale, (long long)resizeMode];
}

@implementation ABI49_0_0RCTImageCache {
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _decodedImageCache.totalCostLimit = ABI49_0_0RCTImageCacheTotalCostLimit;
...
- (void)addImageToCache:(UIImage *)image forKey:(NSString *)cacheKey
{
  ...
  NSInteger bytes = image.ABI49_0_0ReactDecodedImageBytes;
  if (bytes <= ABI49_0_0RCTMaxCacheableDecodedImageSizeInBytes) {
    [self->_decodedImageCache setObject:image forKey:cacheKey cost:bytes];
  }
}

- (UIImage *)imageForUrl:(NSString *)url size:(CGSize)size scale:(CGFloat)scale resizeMode:(ABI49_0_0RCTResizeMode)resizeMode
{
  NSString *cacheKey = ABI49_0_0RCTCacheKeyForImage(url, size, scale, resizeMode);
  @synchronized(_cacheStaleTimes) {
