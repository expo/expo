...
static NSString *const ABI49_0_0RCTImageStoreURLScheme = @"rct-image-store";

@interface ABI49_0_0RCTImageStoreManager () <ABI49_0_0NativeImageStoreIOSSpec>
@end

@implementation ABI49_0_0RCTImageStoreManager {
...
{
  ABI49_0_0RCTAssertThread(_methodQueue, @"Must be called on ABI49_0_0RCTImageStoreManager thread");
  ...
  NSString *imageTag = [NSString stringWithFormat:@"%@://%tu", ABI49_0_0RCTImageStoreURLScheme, _id++];
...
- (void)storeImage:(UIImage *)image withBlock:(void (^)(NSString *imageTag))block
{
  ABI49_0_0RCTAssertParam(block);
  dispatch_async(_methodQueue, ^{
    NSString *imageTag = [self _storeImageData:ABI49_0_0RCTGetImageData(image, 0.75)];
...
ABI49_0_0RCT_EXPORT_METHOD(removeImageForTag : (NSString *)imageTag)
{
  [_store removeObjectForKey:imageTag];
}

ABI49_0_0RCT_EXPORT_METHOD(hasImageForTag : (NSString *)imageTag callback : (ABI49_0_0RCTResponseSenderBlock)callback)
{
  callback(@[ @(_store[imageTag] != nil) ]);
}

// TODO (#5906496): Name could be more explicit - something like getBase64EncodedDataForTag:?
ABI49_0_0RCT_EXPORT_METHOD(getBase64ForTag
                  ...
                  : (ABI49_0_0RCTResponseSenderBlock)errorCallback)
{
  ...
  if (!imageData) {
    errorCallback(
        @[ ABI49_0_0RCTJSErrorFromNSError(ABI49_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag])) ]);
    return;
  }
...
ABI49_0_0RCT_EXPORT_METHOD(addImageFromBase64
...

{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (imageData) {
      ...
      errorCallback(@[ ABI49_0_0RCTJSErrorFromNSError(ABI49_0_0RCTErrorWithMessage(@"Failed to add image from base64String")) ]);
    }
  });
}
...
{
  return [request.URL.scheme caseInsensitiveCompare:ABI49_0_0RCTImageStoreURLScheme] == NSOrderedSame;
}
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI49_0_0RCTURLRequestDelegate>)delegate
{
  ...
  dispatch_async(_methodQueue, ^{
    ...
    if (!imageData) {
      NSError *error = ABI49_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]);
    ...
    if (!sourceRef) {
      NSError *error =
          ABI49_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to decode data for imageTag: %@", imageTag]);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
      return;
    }
...
{
  return std::make_shared<ABI49_0_0facebook::ABI49_0_0React::NativeImageStoreIOSSpecJSI>(params);
}

@end

@implementation ABI49_0_0RCTImageStoreManager (Deprecated)

- (NSString *)storeImage:(UIImage *)image
{
  ABI49_0_0RCTAssertMainQueue();
  ABI49_0_0RCTLogWarn(
      @"ABI49_0_0RCTImageStoreManager.storeImage() is deprecated and has poor performance. Use an alternative method instead.");
  __block NSString *imageTag;
  dispatch_sync(_methodQueue, ^{
    imageTag = [self _storeImageData:ABI49_0_0RCTGetImageData(image, 0.75)];
  });
  return imageTag;
}
...
- (UIImage *)imageForTag:(NSString *)imageTag
{
  ABI49_0_0RCTAssertMainQueue();
  ABI49_0_0RCTLogWarn(
      @"ABI49_0_0RCTImageStoreManager.imageForTag() is deprecated and has poor performance. Use an alternative method instead.");
...
@implementation ABI49_0_0RCTBridge (ABI49_0_0RCTImageStoreManager)

- (ABI49_0_0RCTImageStoreManager *)imageStoreManager
{
  return [self moduleForClass:[ABI49_0_0RCTImageStoreManager class]];
}

@end

Class ABI49_0_0RCTImageStoreManagerCls(void)
{
  return ABI49_0_0RCTImageStoreManager.class;
}
