...
static CGFloat ABI49_0_0RCTCeilValue(CGFloat value, CGFloat scale)
{
  return ceil(value * scale) / scale;
}

static CGFloat ABI49_0_0RCTFloorValue(CGFloat value, CGFloat scale)
{
  return floor(value * scale) / scale;
}

static CGSize ABI49_0_0RCTCeilSize(CGSize size, CGFloat scale)
{
  return (CGSize){ABI49_0_0RCTCeilValue(size.width, scale), ABI49_0_0RCTCeilValue(size.height, scale)};
}
...
CGRect ABI49_0_0RCTTargetRect(CGSize sourceSize, CGSize destSize, CGFloat destScale, ABI49_0_0RCTResizeMode resizeMode)
{
  ...
  if (resizeMode != ABI49_0_0RCTResizeModeCenter && resizeMode != ABI49_0_0RCTResizeModeStretch) {
  ...
  switch (resizeMode) {
    case ABI49_0_0RCTResizeModeStretch:
    case ABI49_0_0RCTResizeModeRepeat:

      return (CGRect){CGPointZero, ABI49_0_0RCTCeilSize(destSize, destScale)};

    case ABI49_0_0RCTResizeModeContain:
      ...
      return (CGRect){
          {
              ABI49_0_0RCTFloorValue((destSize.width - sourceSize.width) / 2, destScale),
              ABI49_0_0RCTFloorValue((destSize.height - sourceSize.height) / 2, destScale),
          },
          ABI49_0_0RCTCeilSize(sourceSize, destScale)};

    case ABI49_0_0RCTResizeModeCover:
      ...
      if (targetAspect <= aspect) { // target is taller than content
        ...
        return (CGRect){
            {ABI49_0_0RCTFloorValue((destSize.width - sourceSize.width) / 2, destScale), 0}, ABI49_0_0RCTCeilSize(sourceSize, destScale)};
        ...
        return (CGRect){
            {0, ABI49_0_0RCTFloorValue((destSize.height - sourceSize.height) / 2, destScale)},
            ABI49_0_0RCTCeilSize(sourceSize, destScale)};
      }

    case ABI49_0_0RCTResizeModeCenter:
...
CGAffineTransform ABI49_0_0RCTTransformFromTargetRect(CGSize sourceSize, CGRect targetRect)
{
    ...
    BOOL allowUpscaling)
{
  switch (resizeMode) {
    case ABI49_0_0RCTResizeModeCenter:
      ...
      return ABI49_0_0RCTTargetRect(sourceSize, destSize, destScale, resizeMode).size;
    ...
    case ABI49_0_0RCTResizeModeStretch:
      ...
      return ABI49_0_0RCTCeilSize(destSize, destScale);

    default: {
      // Get target size
      CGSize size = ABI49_0_0RCTTargetRect(sourceSize, destSize, destScale, resizeMode).size;
      if (!allowUpscaling) {
...
BOOL ABI49_0_0RCTUpscalingRequired(
    CGSize sourceSize,
    CGFloat sourceScale,
    CGSize destSize,
    CGFloat destScale,
...
UIImage *__nullable ABI49_0_0RCTDecodeImageWithData(NSData *data, CGSize destSize, CGFloat destScale, ABI49_0_0RCTResizeMode resizeMode)
{
  ...
  if (CGSizeEqualToSize(destSize, CGSizeZero)) {
    ...
    destScale = ABI49_0_0RCTScreenScale();
  }

  if (resizeMode == UIViewContentModeScaleToFill) {
    // Decoder cannot change aspect ratio, so ABI49_0_0RCTResizeModeStretch is equivalent
    // to ABI49_0_0RCTResizeModeCover for our purposes
    resizeMode = ABI49_0_0RCTResizeModeCover;
  }

  // Calculate target size
  CGSize targetSize = ABI49_0_0RCTTargetSize(sourceSize, 1, destSize, destScale, resizeMode, NO);
  CGSize targetPixelSize = ABI49_0_0RCTSizeInPixels(targetSize, destScale);
...
NSDictionary<NSString *, id> *__nullable ABI49_0_0RCTGetImageMetadata(NSData *data)
{
  CGImageSourceRef sourceRef = CGImageSourceCreateWithData((__bridge CFDataRef)data, NULL);
  if (!sourceRef) {
    return nil;
...
NSData *__nullable ABI49_0_0RCTGetImageData(UIImage *image, float quality)
{
  ...
  if (ABI49_0_0RCTImageHasAlpha(cgImage)) {
    // get png data
...
UIImage *__nullable ABI49_0_0RCTTransformImage(UIImage *image, CGSize destSize, CGFloat destScale, CGAffineTransform transform)
{
  if (destSize.width <= 0 | destSize.height <= 0 || destScale <= 0) {
    return nil;
  }

  BOOL opaque = !ABI49_0_0RCTImageHasAlpha(image.CGImage);
...
BOOL ABI49_0_0RCTImageHasAlpha(CGImageRef image)
{
