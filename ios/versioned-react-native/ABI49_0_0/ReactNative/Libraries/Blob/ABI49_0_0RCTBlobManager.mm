...
#import "ABI49_0_0RCTBlobCollector.h"
#import "ABI49_0_0RCTBlobPlugins.h"

static NSString *const kBlobURIScheme = @"blob";

@interface ABI49_0_0RCTBlobManager () <
    ABI49_0_0RCTNetworkingRequestHandler,
    ABI49_0_0RCTNetworkingResponseHandler,
    ABI49_0_0RCTWebSocketContentHandler,
    ABI49_0_0NativeBlobModuleSpec>

@end

@implementation ABI49_0_0RCTBlobManager {
...
ABI49_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)initialize
{
  ...
  ABI49_0_0facebook::ABI49_0_0React::ABI49_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI49_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI49_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI49_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI49_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
  ABI49_0_0RCTNetworking *const networking = [_moduleRegistry moduleForName:"Networking"];
...
ABI49_0_0RCT_EXPORT_METHOD(addWebSocketHandler : (double)socketID)
{
  dispatch_async(((ABI49_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
    [[self->_moduleRegistry moduleForName:"WebSocketModule"] setContentHandler:self
                                                                   forSocketID:[NSNumber numberWithDouble:socketID]];
  });
}

ABI49_0_0RCT_EXPORT_METHOD(removeWebSocketHandler : (double)socketID)
{
  dispatch_async(((ABI49_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
...
ABI49_0_0RCT_EXPORT_METHOD(sendOverSocket : (NSDictionary *)blob socketID : (double)socketID)
{
  dispatch_async(((ABI49_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
...
ABI49_0_0RCT_EXPORT_METHOD(createFromParts : (NSArray<NSDictionary<NSString *, id> *> *)parts withId : (NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI49_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI49_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI49_0_0RCT_EXPORT_METHOD(release : (NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI49_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI49_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI49_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI49_0_0RCTConvert NSString:ABI49_0_0RCTNilIfNull(blob[@"type"])];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset" : @0,
    @"size" : @(data.length),
    @"name" : ABI49_0_0RCTNullIfNil([response suggestedFilename]),
    @"type" : ABI49_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI49_0_0RCTWebSocketContentHandler methods
...
{
  return std::make_shared<ABI49_0_0facebook::ABI49_0_0React::NativeBlobModuleSpecJSI>(params);
}

@end

Class ABI49_0_0RCTBlobManagerCls(void)
{
  return ABI49_0_0RCTBlobManager.class;
}
