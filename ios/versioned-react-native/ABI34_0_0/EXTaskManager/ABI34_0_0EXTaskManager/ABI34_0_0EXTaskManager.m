...
NSString * const ABI34_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI34_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI34_0_0UMTaskServiceInterface> taskService;

@end

@implementation ABI34_0_0EXTaskManager

ABI34_0_0UM_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI34_0_0UMTaskManagerInterface)];
}
...
// TODO: Remove when adding bare ReactABI34_0_0 Native support
...
{
  return @{
           @"EVENT_NAME": ABI34_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI34_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI34_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI34_0_0UM_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI34_0_0UM_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI34_0_0UM_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI34_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI34_0_0UMPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI34_0_0UM_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI34_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI34_0_0UMPromiseRejectBlock)reject)
{
  resolve(ABI34_0_0UMNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI34_0_0UM_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI34_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI34_0_0UMPromiseRejectBlock)reject)
{
...
# pragma mark - ABI34_0_0UMTaskManagerInterface
...
{
  return [ABI34_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI34_0_0EXTaskManagerEventName body:body];
  } else {
