...
@implementation ABI34_0_0UMReactNativeAdapter
...
{
  return @[@protocol(ABI34_0_0UMAppLifecycleService), @protocol(ABI34_0_0UMUIManager), @protocol(ABI34_0_0UMJavaScriptContextProvider), @protocol(ABI34_0_0UMImageLoaderInterface)];
}
...
{
  __weak ABI34_0_0UMReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI34_0_0UMReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI34_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI34_0_0RCTJSThread];
}
...
# pragma mark - ABI34_0_0UMJavaScriptContextProvider
...
- (JSGlobalContextRef)javaScriptContextRef
{
  if ([_bridge respondsToSelector:@selector(jsContextRef)]) {
    ...
    // In ReactABI34_0_0-native 0.59 vm is abstracted by JSI and all JSC specific references are removed
    ...
    // WARNING: This is temporary solution that may break with new ReactABI34_0_0-native releases.
...
- (void)handleAppStateDidChange:(NSNotification *)notification
{
  if (
      _isForegrounded && (
       ...
       ABI34_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI34_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
