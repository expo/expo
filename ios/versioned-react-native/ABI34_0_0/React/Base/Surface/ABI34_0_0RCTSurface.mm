...
#import "ABI34_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI34_0_0RCTSurface () <ABI34_0_0RCTSurfaceRootShadowViewDelegate, ABI34_0_0RCTUIManagerObserver>
@end

@implementation ABI34_0_0RCTSurface {
  // Immutable
  ...
  ABI34_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI34_0_0RCTSurfaceView *_Nullable _view;
  ABI34_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI34_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instanciate `ABI34_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI34_0_0RCTAllocateRootViewTag();
    ...
    _stage = ABI34_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI34_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI34_0_0RCTSurfaceView *)view
{
  ABI34_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI34_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI34_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI34_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI34_0_0RCTSurfaceRootView *rootView =
    (ABI34_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI34_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI34_0_0RCTAssert([rootView isKindOfClass:[ABI34_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI34_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI34_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI34_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI34_0_0RCTSurfaceStage)stage
{
  ABI34_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI34_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI34_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI34_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI34_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI34_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI34_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI34_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountReactABI34_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI34_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI34_0_0RCTIsMainQueue() ? ABI34_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI34_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI34_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI34_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI34_0_0Tag:self->_rootViewTag];
    ABI34_0_0RCTAssert([rootShadowView isKindOfClass:[ABI34_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI34_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI34_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI34_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI34_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI34_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI34_0_0Tag:self->_rootViewTag];
...
- (BOOL)synchronouslyWaitForStage:(ABI34_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI34_0_0RCTIsUIManagerQueue()) {
    ABI34_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI34_0_0RCTIsMainQueue() && (stage & ABI34_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI34_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI34_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI34_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI34_0_0RCTAssert(NO, @"Only waiting for `ABI34_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI34_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI34_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI34_0_0RCTIsMainQueue() && (stage & ABI34_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI34_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI34_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI34_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
...
{
  [self _setStage:ABI34_0_0RCTSurfaceStageSurfaceDidInitialRendering];
...
{
  [self _setStage:ABI34_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI34_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
{
  if (self.stage & ABI34_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI34_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ReactABI34_0_0 components

- (void)mountReactABI34_0_0ComponentWithBridge:(ABI34_0_0RCTBridge *)bridge moduleName:(NSString *)moduleName params:(NSDictionary *)params
{
  [bridge enqueueJSCall:@"AppRegistry" method:@"runApplication" args:@[moduleName, params] completion:NULL];
}

- (void)unmountReactABI34_0_0ComponentWithBridge:(ABI34_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
