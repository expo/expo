...
const NSInteger ABI34_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI34_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI34_0_0RCTGetEventID(id<ABI34_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI34_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI34_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI34_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI34_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI34_0_0RCT_DEBUG) {
    ABI34_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI34_0_0 tag");
  }

  name = ABI34_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI34_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI34_0_0RCTEvent>)event
{
  ...
  for (id<ABI34_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI34_0_0RCTGetEventID(event);

  id<ABI34_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI34_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
@implementation ABI34_0_0RCTBridge (ABI34_0_0RCTEventDispatcher)

- (ABI34_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI34_0_0RCTEventDispatcher class]];
}
