...
#include "ABI34_0_0RCTMessageThread.h"
...
ABI34_0_0RCTMessageThread::~ABI34_0_0RCTMessageThread() {
  CFRelease(m_cfRunLoop);
}

// This is analogous to dispatch_async
void ABI34_0_0RCTMessageThread::runAsync(std::function<void()> func) {
...
void ABI34_0_0RCTMessageThread::runSync(std::function<void()> func) {
...
void ABI34_0_0RCTMessageThread::tryFunc(const std::function<void()>& func) {
...
void ABI34_0_0RCTMessageThread::runOnQueue(std::function<void()>&& func) {
  if (m_shutdown) {
    return;
  }
...
void ABI34_0_0RCTMessageThread::runOnQueueSync(std::function<void()>&& func) {
  if (m_shutdown) {
    return;
  }
...
void ABI34_0_0RCTMessageThread::quitSynchronous() {
  m_shutdown = true;
  runSync([]{});
  CFRunLoopStop(m_cfRunLoop);
}

void ABI34_0_0RCTMessageThread::setRunLoop(NSRunLoop *runLoop) {
  CFRelease(m_cfRunLoop);
  m_cfRunLoop = [runLoop getCFRunLoop];
  CFRetain(m_cfRunLoop);
}
