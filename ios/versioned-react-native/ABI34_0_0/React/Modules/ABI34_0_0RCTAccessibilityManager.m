...
NSString *const ABI34_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI34_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI34_0_0RCTAccessibilityManager ()
...
@implementation ABI34_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI34_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI34_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI34_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI34_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI34_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI34_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI34_0_0Tag)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    UIView *view = [self.bridge.uiManager viewForReactABI34_0_0Tag:ReactABI34_0_0Tag];
    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, view);
  });
}

ABI34_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI34_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI34_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI34_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI34_0_0RCTResponseSenderBlock)callback
...
@implementation ABI34_0_0RCTBridge (ABI34_0_0RCTAccessibilityManager)

- (ABI34_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI34_0_0RCTAccessibilityManager class]];
}
