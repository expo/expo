...
@interface _ABI34_0_0RCTTimer : NSObject
...
@implementation _ABI34_0_0RCTTimer
...
@interface _ABI34_0_0RCTTimingProxy : NSObject

@end

// NSTimer retains its target, insert this class to break potential retain cycles
@implementation _ABI34_0_0RCTTimingProxy
{
  __weak id _target;
}
...
{
  _ABI34_0_0RCTTimingProxy *proxy = [self new];
  if (proxy) {
...
@implementation ABI34_0_0RCTTiming
{
  NSMutableDictionary<NSNumber *, _ABI34_0_0RCTTimer *> *_timers;
  NSTimer *_sleepTimer;
...
{
  ABI34_0_0RCTAssert(!_bridge, @"Should never be initialized twice!");
...
- (void)didUpdateFrame:(ABI34_0_0RCTFrameUpdate *)update
{
  NSDate *nextScheduledTarget = [NSDate distantFuture];
  NSMutableArray<_ABI34_0_0RCTTimer *> *timersToCall = [NSMutableArray new];
  NSDate *now = [NSDate date]; // compare all the timers to the same base time
  for (_ABI34_0_0RCTTimer *timer in _timers.allValues) {
  ...
  if (timersToCall.count > 0) {
    NSArray<NSNumber *> *sortedTimers = [[timersToCall sortedArrayUsingComparator:^(_ABI34_0_0RCTTimer *a, _ABI34_0_0RCTTimer *b) {
  ...
  for (_ABI34_0_0RCTTimer *timer in timersToCall) {
    if (timer.repeats) {
      [timer reschedule];
      nextScheduledTarget = [nextScheduledTarget earlierDate:timer.target];
    } else {
  ...
  if (!_sendIdleEvents && timersToCall.count == 0) {
    // No need to call the pauseCallback as ABI34_0_0RCTDisplayLink will ask us about our paused
...
- (void)scheduleSleepTimer:(NSDate *)sleepTarget
{
  if (!_sleepTimer || !_sleepTimer.valid) {
    _sleepTimer = [[NSTimer alloc] initWithFireDate:sleepTarget
                                           interval:0
                                             target:[_ABI34_0_0RCTTimingProxy proxyWithTarget:self]
 ...
 */
ABI34_0_0RCT_EXPORT_METHOD(createTimer:(nonnull NSNumber *)callbackID
                  ...
                  repeats:(BOOL)repeats)
{
  ...
  _ABI34_0_0RCTTimer *timer = [[_ABI34_0_0RCTTimer alloc] initWithCallbackID:callbackID
                                                  interval:jsDuration
...
ABI34_0_0RCT_EXPORT_METHOD(deleteTimer:(nonnull NSNumber *)timerID)
{
...
ABI34_0_0RCT_EXPORT_METHOD(setSendIdleEvents:(BOOL)sendIdleEvents)
{
