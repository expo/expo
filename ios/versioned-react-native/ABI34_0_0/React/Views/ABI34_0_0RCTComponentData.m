...
typedef void (^ABI34_0_0RCTPropBlock)(id<ABI34_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI34_0_0RCTPropBlock> ABI34_0_0RCTPropBlockDictionary;
...
@implementation ABI34_0_0RCTComponentData
{
  id<ABI34_0_0RCTComponent> _defaultView; // Only needed for ABI34_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI34_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI34_0_0RCTPropBlockDictionary *_shadowPropBlocks;
...
- (ABI34_0_0RCTViewManager *)manager
{
  if (!_manager) {
...
{
  ABI34_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ReactABI34_0_0Tag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI34_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI34_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI34_0_0RCTPropBlock createEventSetter(NSString *propName, SEL setter, ABI34_0_0RCTBridge *bridge)
{
  __weak ABI34_0_0RCTBridge *weakBridge = bridge;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI34_0_0RCTConvert BOOL:json]) {
      __weak id<ABI34_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI34_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        NSMutableDictionary *mutableEvent = [NSMutableDictionary dictionaryWithDictionary:event];
        mutableEvent[@"target"] = strongTarget.ReactABI34_0_0Tag;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [weakBridge.eventDispatcher sendInputEventWithName:ABI34_0_0RCTNormalizeInputEventName(propName) body:mutableEvent];
...
static ABI34_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI34_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI34_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI34_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI34_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI34_0_0RCTDirectEventBlock:")) {
      // Special case for event handlers
      setterBlock = createEventSetter(name, setter, _bridge);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI34_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI34_0_0RCTLogError(@"No +[ABI34_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI34_0_0RCTComponent> view, __unused id json){};
      }
      switch (typeSignature.methodReturnType[0]) {

#define ABI34_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI34_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI34_0_0RCT_CASE(_C_SEL, SEL)
        ABI34_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI34_0_0RCT_CASE(_C_CHR, char)
        ABI34_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI34_0_0RCT_CASE(_C_SHT, short)
        ABI34_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI34_0_0RCT_CASE(_C_INT, int)
        ABI34_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI34_0_0RCT_CASE(_C_LNG, long)
        ABI34_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI34_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI34_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI34_0_0RCT_CASE(_C_FLT, float)
        ABI34_0_0RCT_CASE(_C_DBL, double)
        ABI34_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI34_0_0RCT_CASE(_C_PTR, void *)
        ABI34_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI34_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI34_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI34_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI34_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI34_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI34_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ",
                             name, weakSelf.name, view.ReactABI34_0_0Tag];
      ABI34_0_0RCTPerformBlockWithLogPrefix(^{
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI34_0_0RCTClassOverridesInstanceMethod(_managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [self.manager customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI34_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI34_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
      ABI34_0_0RCTLogError(@"Property '%@' of component '%@' redefined from '%@' "
                  "to '%@'", name, _name, propTypes[name], type);
    }

    if ([type isEqualToString:@"ABI34_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI34_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI34_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI34_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
  ...
  for (NSString *event in bubblingEvents) {
    if ([directEvents containsObject:event]) {
      ABI34_0_0RCTLogError(@"Component '%@' registered '%@' as both a bubbling event "
...
{
  // Hackety hack, this partially re-implements ABI34_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI34_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI34_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI34_0_0EX_REMOVE_VERSION(name);
  ...
  ABI34_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
