...
@implementation UIView (ABI34_0_0RCTViewUnmounting)

- (void)ReactABI34_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI34_0_0_remountAllSubviews];
  }
}

- (void)ReactABI34_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI34_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI34_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI34_0_0RootView]) {
      break;
    }
...
static NSString *ABI34_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI34_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI34_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI34_0_0RCTBorderStyleSolid;
...
ABI34_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)

- (void)setReactABI34_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ReactABI34_0_0LayoutDirection != layoutDirection) {
    _ReactABI34_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI34_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (void)setPointerEvents:(ABI34_0_0RCTPointerEvents)pointerEvents
{
  ...
  self.userInteractionEnabled = (pointerEvents != ABI34_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI34_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI34_0_0RCTPointerEventsNone || _pointerEvents == ABI34_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ReactABI34_0_0ZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI34_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI34_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI34_0_0RCTLogError(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (UIView *)ReactABI34_0_0AccessibilityElement
{
  return self;
}
...
{
  if (self.ReactABI34_0_0AccessibilityElement == self) {
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI34_0_0Tag: %@;", self.ReactABI34_0_0Tag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI34_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ReactABI34_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI34_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ReactABI34_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI34_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI34_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI34_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI34_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ReactABI34_0_0Subviews.count == 0) {
  ...
  for (UIView *view in self.ReactABI34_0_0Subviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI34_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI34_0_0_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI34_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI34_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x) {
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ReactABI34_0_0LayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI34_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI34_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI34_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets) {
      ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
      ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets) {
    ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
    ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, ABI34_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
    ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
    ABI34_0_0RCTDefaultIfNegativeTo(borderWidth, ABI34_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI34_0_0RCTCornerRadii)cornerRadii
{
  ...
  if ([[ABI34_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI34_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, _borderTopStartRadius);
    const CGFloat topEndRadius = ABI34_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, _borderTopEndRadius);
    const CGFloat bottomStartRadius = ABI34_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, _borderBottomStartRadius);
    const CGFloat bottomEndRadius = ABI34_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, _borderBottomEndRadius);
    ...
    topLeftRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, ABI34_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, ABI34_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, ABI34_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI34_0_0RCTDefaultIfNegativeTo(radius, ABI34_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI34_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI34_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI34_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI34_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI34_0_0RCTCornerRadii){
...
- (ABI34_0_0RCTBorderColors)borderColors
{
  ...
  if ([[ABI34_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    ...
    return (ABI34_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI34_0_0RCTUpdateShadowPathForView(self);

  const ABI34_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI34_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI34_0_0RCTRunningInTestEnvironment() &&
  ABI34_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI34_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI34_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI34_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI34_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI34_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI34_0_0RCTUpdateShadowPathForView(ABI34_0_0RCTView *view)
{
  if (ABI34_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI34_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI34_0_0RCTCornerInsets cornerInsets = ABI34_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI34_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI34_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI34_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI34_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI34_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI34_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI34_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI34_0_0RCTBorderStyle)style \
  {                                                    \
