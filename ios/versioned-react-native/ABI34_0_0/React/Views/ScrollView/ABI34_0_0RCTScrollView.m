...
@interface ABI34_0_0RCTScrollEvent : NSObject <ABI34_0_0RCTEvent>
...
@implementation ABI34_0_0RCTScrollEvent
{
...
{
  ABI34_0_0RCTAssertParam(ReactABI34_0_0Tag);
...
- (ABI34_0_0RCTScrollEvent *)coalesceWithEvent:(ABI34_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI34_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI34_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI34_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI34_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
...
@implementation ABI34_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI34_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI34_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI34_0_0RCTUIManager JSResponder] isKindOfClass:[ABI34_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI34_0_0 scroll
...
- (void)setContentOffset:(CGPoint)contentOffset
{
  ...
  super.contentOffset = CGPointMake(
    ABI34_0_0RCTSanitizeNaNValue(contentOffset.x, @"scrollView.contentOffset.x"),
    ABI34_0_0RCTSanitizeNaNValue(contentOffset.y, @"scrollView.contentOffset.y"));
}
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI34_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI34_0_0RCTScrollView () <ABI34_0_0RCTUIManagerObserver>

@end

@implementation ABI34_0_0RCTScrollView
{
  ABI34_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI34_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI34_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI34_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI34_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI34_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
- (void)setReactABI34_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  [super setReactABI34_0_0LayoutDirection:layoutDirection];

  ABI34_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI34_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI34_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI34_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI34_0_0RCTAssert(_contentView == nil, @"ABI34_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI34_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ReactABI34_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI34_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI34_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI34_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI34_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI34_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI34_0_0_findClipView];
  if (!clipView) {
    return;
  }
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI34_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI34_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI34_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI34_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI34_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI34_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI34_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
ABI34_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidScrollToTop, onScrollToTop)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI34_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI34_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI34_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI34_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI34_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI34_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI34_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI34_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI34_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI34_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI34_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI34_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(ABI34_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI34_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI34_0_0RCTScrollEvent *scrollEvent = [[ABI34_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI34_0_0RCTEventDispatcher (ABI34_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI34_0_0Tag
{
  ...
  ABI34_0_0RCTScrollEvent *fakeScrollEvent = [[ABI34_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI34_0_0Tag:ReactABI34_0_0Tag
