...
#import "ABI34_0_0RCTSurfaceTouchHandler.h"
...
#import "ABI34_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI34_0_0RCTTouchEventType) {
  ABI34_0_0RCTTouchEventTypeTouchStart,
  ABI34_0_0RCTTouchEventTypeTouchMove,
  ABI34_0_0RCTTouchEventTypeTouchEnd,
  ABI34_0_0RCTTouchEventTypeTouchCancel,
};
...
static void UpdateActiveTouchWithUITouch(ActiveTouch &activeTouch, UITouch *uiTouch, UIView *rootComponentView) {
  ...
  activeTouch.touch.offsetPoint = ABI34_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI34_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI34_0_0RCTPointFromCGPoint(pagePoint);
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView) {
  ...
  if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
    activeTouch.eventEmitter = [(id<ABI34_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitterAtPoint:[uiTouch locationInView:componentView]];
...
@interface ABI34_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI34_0_0RCTSurfaceTouchHandler {
  std::unordered_map<
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ReactABI34_0_0 Native,
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI34_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI34_0_0RCTTouchEventTypeTouchEnd || eventType == ABI34_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitter) {
    ...
    switch (eventType) {
      case ABI34_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI34_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI34_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI34_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI34_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI34_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI34_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches]
                     eventType:ABI34_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
