...
@interface ABI34_0_0EXBarCodeScanner() <AVCaptureMetadataOutputObjectsDelegate>
...
NSString *const ABI34_0_0EX_BARCODE_TYPES_KEY = @"barCodeTypes";

@implementation ABI34_0_0EXBarCodeScanner
...
- (void)setSettings:(NSDictionary<NSString *, id> *)settings
{
  for (NSString *key in settings) {
    if ([key isEqualToString:ABI34_0_0EX_BARCODE_TYPES_KEY]) {
      NSArray<NSString *> *value = settings[key];
      NSSet *previousTypes = [NSSet setWithArray:_settings[ABI34_0_0EX_BARCODE_TYPES_KEY]];
      ...
      if (![previousTypes isEqualToSet:newTypes]) {
        ...
        nextSettings[ABI34_0_0EX_BARCODE_TYPES_KEY] = value;
...
- (void)maybeStartBarCodeScanning
{
  ...
  if (_settings && _settings[ABI34_0_0EX_BARCODE_TYPES_KEY]) {
    requestedObjectTypes = [[NSArray alloc] initWithArray:_settings[ABI34_0_0EX_BARCODE_TYPES_KEY]];
  }
...
{
  if (!_settings || !_settings[ABI34_0_0EX_BARCODE_TYPES_KEY] || !_metadataOutput) {
    return;
  }
...
{
  return @{
           ABI34_0_0EX_BARCODE_TYPES_KEY: [[ABI34_0_0EXBarCodeScannerUtils validBarCodeTypes] allValues],
           };
}
