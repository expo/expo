...
@interface ABI34_0_0EXAR () <ABI34_0_0EXGLARSessionManagerDelegate>

@property (nonatomic, strong) id arSessionManager;

@end

@implementation ABI34_0_0EXAR

ABI34_0_0RCT_EXPORT_MODULE(ExponentAR);
...
+ (NSMutableArray *)serializeARVideoFormats:(NSArray<ARVideoFormat *>*)videoFormats  API_AVAILABLE(ios(11.3))
{
  ...
  if (@available(iOS 11.3, *)) {
    for (ARVideoFormat *videoFormat in videoFormats) {
      [output addObject:[ABI34_0_0EXAR serializeARVideoFormat: videoFormat]];
    }
  }
...
- (NSDictionary<NSString *, NSString *> *)constantsToExport
{
  ...
  if (@available(iOS 11.3, *)) {
    [output addEntriesFromDictionary:@{
                                       @"ARKitVersion": @"1.5",
//                                       @"FaceTrackingVideoFormats": [ABI34_0_0EXAR serializeARVideoFormats:[ARFaceTrackingConfiguration supportedVideoFormats]],
                                       @"WorldTrackingVideoFormats": [ABI34_0_0EXAR serializeARVideoFormats:[ARWorldTrackingConfiguration supportedVideoFormats]],
                                       @"OrientationTrackingVideoFormats": [ABI34_0_0EXAR serializeARVideoFormats:[AROrientationTrackingConfiguration supportedVideoFormats]],
                                       }];
  }
...
ABI34_0_0RCT_REMAP_METHOD(startAsync,
                 startAsyncWithReactABI34_0_0Tag:(nonnull NSNumber *)tag
                 ...
                 rejecter:(ABI34_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI34_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ...
    if (![view isKindOfClass:[ABI34_0_0EXGLView class]]) {
      reject(@"E_GLVIEW_MANAGER_BAD_VIEW_TAG", nil, ABI34_0_0RCTErrorWithMessage(@"ABI34_0_0EXAR.startARSessionAsync: Expected an ABI34_0_0EXGLView"));
      return;
    }
    ...
    ABI34_0_0EXGLView *exglView = (ABI34_0_0EXGLView *)view;
    
    Class sessionManagerClass = NSClassFromString(@"ABI34_0_0EXGLARSessionManager");
    if (sessionManagerClass) {
      ...
      if (response[@"error"]) {
        reject(@"ERR_ARKIT_FAILED_TO_INIT", response[@"error"], ABI34_0_0RCTErrorWithMessage(response[@"error"]));
      } else {
        resolve(response);
      }
    } else {
      NSString *response = @"ARKIT capabilities were not included with this build.";
      reject(@"ERR_ARKIT_FAILED_TO_INIT", response, ABI34_0_0RCTErrorWithMessage(response));
    }
  }];
}

ABI34_0_0RCT_REMAP_METHOD(stopAsync,
                 resolver:(ABI34_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI34_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI34_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
ABI34_0_0RCT_EXPORT_METHOD(pause)
{
  [_arSessionManager pause];
}

ABI34_0_0RCT_EXPORT_METHOD(resume)
{
  [_arSessionManager resume];
}

ABI34_0_0RCT_EXPORT_METHOD(reset)
{
  [_arSessionManager reset];
}

ABI34_0_0RCT_REMAP_METHOD(setProvidesAudioData,
                 providesAudioData:(BOOL)providesAudioData)
{
  [_arSessionManager setProvidesAudioData:providesAudioData];
}

ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getProvidesAudioData, NSNumber *, getProvidesAudioData)
{
  return @([_arSessionManager providesAudioData]);
}

ABI34_0_0RCT_REMAP_METHOD(setLightEstimationEnabled,
                 lightEstimationEnabled:(BOOL)lightEstimationEnabled)
{
  [_arSessionManager setLightEstimationEnabled:lightEstimationEnabled];
}

ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getLightEstimationEnabled, NSNumber *, getLightEstimationEnabled)
{
  return @([_arSessionManager lightEstimationEnabled]);
}

ABI34_0_0RCT_REMAP_METHOD(setAutoFocusEnabled,
                 isAutoFocusEnabled:(BOOL)isAutoFocusEnabled)
{
  [_arSessionManager setAutoFocusEnabled:isAutoFocusEnabled];
}

ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getAutoFocusEnabled, NSNumber *, getAutoFocusEnabled)
{
  return @([_arSessionManager autoFocusEnabled]);
}

ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getPlaneDetection, NSString *, getPlaneDetection)
{
...
ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(performHitTest,
...
ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getWorldAlignment, NSString *, getWorldAlignment)
{
  NSArray *items = @[@"gravity", @"gravityAndHeading", @"alignmentCamera"];
  return [items objectAtIndex:[_arSessionManager worldAlignment]];
}

ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCurrentFrame,
                                      nullable NSDictionary *,
...
ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getARMatrices,
                                      nullable NSDictionary *,
...
ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getVideoFormat, nullable NSDictionary *, getVideoFormat)
{
  if (!_arSessionManager) {
    return nil;
  }
  if (@available(iOS 11.3, *)) {
    ...
    return [ABI34_0_0EXAR serializeARVideoFormat:videoFormat];
  }
  return nil;
}

ABI34_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCameraTexture, NSNumber *, getCameraTexture)
{
  return @([_arSessionManager cameraTexture]);
}

ABI34_0_0RCT_REMAP_METHOD(setShouldAttemptRelocalization,
...
ABI34_0_0RCT_REMAP_METHOD(setWorldOriginAsync,
...
ABI34_0_0RCT_REMAP_METHOD(setConfigurationAsync,
...
ABI34_0_0RCT_REMAP_METHOD(setDetectionImagesAsync,
