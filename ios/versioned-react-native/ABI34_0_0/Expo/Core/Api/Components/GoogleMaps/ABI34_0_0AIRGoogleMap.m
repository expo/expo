...
#ifdef ABI34_0_0HAVE_GOOGLE_MAPS_UTILS
...
#define REQUIRES_GOOGLE_MAPS_UTILS(feature) do { \
 [NSException raise:@"ReactABI34_0_0NativeMapsDependencyMissing" \
...
@interface ABI34_0_0AIRGoogleMap ()
...
@implementation ABI34_0_0AIRGoogleMap
{
  NSMutableArray<UIView *> *_ReactABI34_0_0Subviews;
...
- (void)insertReactABI34_0_0Subview:(id<ABI34_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  ...
  if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapMarker class]]) {
    ABI34_0_0AIRGoogleMapMarker *marker = (ABI34_0_0AIRGoogleMapMarker*)subview;
    marker.realMarker.map = self;
    [self.markers addObject:marker];
  } else if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapPolygon class]]) {
    ABI34_0_0AIRGoogleMapPolygon *polygon = (ABI34_0_0AIRGoogleMapPolygon*)subview;
    polygon.polygon.map = self;
    [self.polygons addObject:polygon];
  } else if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapPolyline class]]) {
    ABI34_0_0AIRGoogleMapPolyline *polyline = (ABI34_0_0AIRGoogleMapPolyline*)subview;
    polyline.polyline.map = self;
    [self.polylines addObject:polyline];
  } else if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapCircle class]]) {
    ABI34_0_0AIRGoogleMapCircle *circle = (ABI34_0_0AIRGoogleMapCircle*)subview;
    circle.circle.map = self;
    [self.circles addObject:circle];
  } else if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapUrlTile class]]) {
    ABI34_0_0AIRGoogleMapUrlTile *tile = (ABI34_0_0AIRGoogleMapUrlTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapWMSTile class]]) {
    ABI34_0_0AIRGoogleMapWMSTile *tile = (ABI34_0_0AIRGoogleMapWMSTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapOverlay class]]) {
    ABI34_0_0AIRGoogleMapOverlay *overlay = (ABI34_0_0AIRGoogleMapOverlay*)subview;
    overlay.overlay.map = self;
    [self.overlays addObject:overlay];
  } else {
    NSArray<id<ABI34_0_0RCTComponent>> *childSubviews = [subview ReactABI34_0_0Subviews];
    for (int i = 0; i < childSubviews.count; i++) {
      [self insertReactABI34_0_0Subview:(UIView *)childSubviews[i] atIndex:atIndex];
    }
  }
  [_ReactABI34_0_0Subviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeReactABI34_0_0Subview:(id<ABI34_0_0RCTComponent>)subview {
  ...
  if ([subview isKindOfClass:[ABI34_0_0AIRGoogleMapMarker class]]) {
    ...
    NSArray<id<ABI34_0_0RCTComponent>> *childSubviews = [subview ReactABI34_0_0Subviews];
    for (int i = 0; i < childSubviews.count; i++) {
      [self removeReactABI34_0_0Subview:(UIView *)childSubviews[i]];
    }
  }
  [_ReactABI34_0_0Subviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (void)didMoveToWindow {
  ...
  if (_initialCamera != nil) {
    ...
    self.camera = [ABI34_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_initialRegion];
  } else if (_region.span.latitudeDelta != 0.0 &&
      _region.span.longitudeDelta != 0.0) {
    self.camera = [ABI34_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_region];
  }
...
- (void)setInitialRegion:(MKCoordinateRegion)initialRegion {
  ...
  self.camera = [ABI34_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:initialRegion];
}
...
- (void)setRegion:(MKCoordinateRegion)region {
  ...
  _region = region;
  self.camera = [ABI34_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self  andMKCoordinateRegion:region];
}
...
- (BOOL)didTapMarker:(GMSMarker *)marker {
  ABI34_0_0AIRGMSMarker *airMarker = (ABI34_0_0AIRGMSMarker *)marker;
...
- (void)didTapPolyline:(GMSOverlay *)polyline {
  ABI34_0_0AIRGMSPolyline *airPolyline = (ABI34_0_0AIRGMSPolyline *)polyline;
...
- (void)didTapPolygon:(GMSOverlay *)polygon {
    ABI34_0_0AIRGMSPolygon *airPolygon = (ABI34_0_0AIRGMSPolygon *)polygon;
...
- (void)didChangeCameraPosition:(GMSCameraPosition *)position {
  id event = @{@"continuous": @YES,
               @"region": regionAsJSON([ABI34_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andGMSCameraPosition:position]),
               };
...
- (CGRect) frameForMarker:(ABI34_0_0AIRGoogleMapMarker*) mrkView {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI34_0_0AIRGoogleMapMarker* mrkView in self.markers) {
...
- (ABI34_0_0AIRGoogleMapMarker*) markerAtPoint:(CGPoint)point {
    ABI34_0_0AIRGoogleMapMarker* mrk = nil;
    for (ABI34_0_0AIRGoogleMapMarker* mrkView in self.markers) {
        CGRect frame = [self frameForMarker:mrkView];
...
- (id)extendedMapGestureHandler:(UIGestureRecognizer*)gestureRecognizer {
    ...
    if (!tapTouch)
        ...
        if (isTapInsideBubble) {
            ...
            ABI34_0_0AIRGoogleMapMarker* markerView = nil;
            ABI34_0_0AIRGMSMarker* marker = nil;
            for (ABI34_0_0AIRGoogleMapMarker* mrk in self.markers) {
            ...
            UIView* realSubview = [(ABI34_0_0RCTView*)bubbleView hitTest:tapPointInBubble withEvent:nil];
            ABI34_0_0AIRGoogleMapCalloutSubview* realPressableSubview = nil;
            if (realSubview) {
                UIView* tmp = realSubview;
                while (tmp && tmp != win && tmp != bubbleView) {
                    if ([tmp respondsToSelector:@selector(onPress)]) {
                        realPressableSubview = (ABI34_0_0AIRGoogleMapCalloutSubview*) tmp;
                        break;
                    }
                    tmp = tmp.superview;
                }
            ...
            if (markerView) {
                ...
                if (isInsideCallout) {
                    ...
                    ABI34_0_0AIRGoogleMapMarker* markerAtTapPoint = [self markerAtPoint:tapPoint];
...
- (void)setKmlSrc:(NSString *)kmlUrl {
  ...
  for (GMUPlacemark *place in parser.placemarks) {
    ...
    ABI34_0_0AIRGoogleMapMarker *marker = (ABI34_0_0AIRGoogleMapMarker *)[[ABI34_0_0AIRGoogleMapMarkerManager alloc] view];
    if (!marker.bridge) {
    ...
    marker.imageSrc = [ABI34_0_0AIRGoogleMap GetIconUrl:place parser:parser];
    marker.layer.backgroundColor = [UIColor clearColor].CGColor;
    marker.layer.position = CGPointZero;

    [self insertReactABI34_0_0Subview:(UIView *) marker atIndex:index];
