...
#import "ABI34_0_0RNCWKProcessPoolManager.h"
...
@interface ABI34_0_0_SwizzleHelperWK : NSObject @end
@implementation ABI34_0_0_SwizzleHelperWK
...
@interface ABI34_0_0RNCWKWebView () <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, UIScrollViewDelegate, ABI34_0_0RCTAutoInsetsProtocol>
...
@property (nonatomic, copy) ABI34_0_0RCTDirectEventBlock onLoadingProgress;
...
@property (nonatomic, copy) ABI34_0_0RCTDirectEventBlock onScroll;
@property (nonatomic, copy) WKWebView *webView;
@end

@implementation ABI34_0_0RNCWKWebView
{
  ...
  // https://github.com/ReactABI34_0_0-native-community/ReactABI34_0_0-native-webview/issues/62
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _savedStatusBarStyle = ABI34_0_0RCTSharedApplication().statusBarStyle;
    _savedStatusBarHidden = ABI34_0_0RCTSharedApplication().statusBarHidden;
  }
...
- (void)didMoveToWindow
{
  if (self.window != nil && _webView == nil) {
    ...
    if(self.useSharedProcessPool) {
      wkWebViewConfig.processPool = [[ABI34_0_0RNCWKProcessPoolManager sharedManager] sharedProcessPoolForExperienceId:self.experienceId];
    }
        ...
        */
        ...
        for (NSHTTPCookie *cookie in [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookies]) {
          [script appendFormat:@"document.cookie = %@ + '=' + %@",
            ABI34_0_0RCTJSONStringify(cookie.name, NULL),
            ABI34_0_0RCTJSONStringify(cookie.value, NULL)];
          if (cookie.path) {
            [script appendFormat:@" + '; Path=' + %@", ABI34_0_0RCTJSONStringify(cookie.path, NULL)];
          }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (!_isFullScreenVideoOpen) {
    ...
    ABI34_0_0RCTUnsafeExecuteOnMainQueueSync(^{
      [ABI34_0_0RCTSharedApplication() setStatusBarStyle:UIStatusBarStyleLightContent animated:YES];
    });
  } else {
    _isFullScreenVideoOpen = NO;
    ABI34_0_0RCTUnsafeExecuteOnMainQueueSync(^{
      [ABI34_0_0RCTSharedApplication() setStatusBarHidden:_savedStatusBarHidden animated:YES];
      [ABI34_0_0RCTSharedApplication() setStatusBarStyle:_savedStatusBarStyle animated:YES];
    });
  }
...
- (void)visitSource
{
    ...
    NSString *html = [ABI34_0_0RCTConvert NSString:_source[@"html"]];
    if (html) {
        NSURL *baseURL = [ABI34_0_0RCTConvert NSURL:_source[@"baseUrl"]];
        if (!baseURL) {
...
-(void)setHideKeyboardAccessoryView:(BOOL)hideKeyboardAccessoryView
{
    ...
    NSString* name = [NSString stringWithFormat:@"%@ABI34_0_0_SwizzleHelperWK", subview.class.superclass];
    ...
    if(newClass == nil)
    {
        ...
        Method method = class_getInstanceMethod([ABI34_0_0_SwizzleHelperWK class], @selector(inputAccessoryView));
...
- (void)layoutSubviews
{
  ...
  // Ensure webview takes the position and dimensions of ABI34_0_0RNCWKWebView
          ...
          thenCall: (void (^)(NSString*)) callback
{
  [self.webView evaluateJavaScript: js completionHandler: ^(id result, NSError *error) {
    if (error == nil) {
      ...
      ABI34_0_0RCTLogError(@"Error evaluating injectedJavaScript: This is possibly due to an unsupported return type. Try adding true to the end of your injectedJavaScript string.");
    }
  }];
}
