...
@interface ABI34_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI34_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI34_0_0RNSVGGroup *textRoot;
@end

@implementation ABI34_0_0RNSVGNode
{
    ABI34_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI34_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI34_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI34_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)invalidate
{
    ...
    _dirty = true;
    id<ABI34_0_0RNSVGContainer> container = (id<ABI34_0_0RNSVGContainer>)self.superview;
...
- (void)clearParentCache
{
    ...
    while (node != nil) {
        ...
        node = (ABI34_0_0RNSVGNode*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (ABI34_0_0RNSVGGroup *)textRoot
{
    ...
    ABI34_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI34_0_0RNSVGGroup class]] && [((ABI34_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI34_0_0RNSVGGroup*)node;
            break;
        }
...
{
    ABI34_0_0RNSVGNode* parent = (ABI34_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI34_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI34_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI34_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)setClipRule:(ABI34_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI34_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
        if (_cachedClipPath) {
            CGPathRelease(_cachedClipPath);
        }
...
- (ABI34_0_0RNSVGSvgView *)svgView
{
    ...
    if ([parent class] == [ABI34_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI34_0_0RNSVGNode class]]) {
        _svgView = ((ABI34_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI34_0_0RCTLogError(@"ABI34_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI34_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI34_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI34_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
