...
#import "ABI34_0_0RNSVGGlyphContext.h"
#import "ABI34_0_0RNSVGTextProperties.h"

@implementation ABI34_0_0RNSVGText
{
    ABI34_0_0RNSVGGlyphContext *_glyphContext;
...
- (void)setTextLength:(ABI34_0_0RNSVGLength *)textLength
{
...
- (void)setDeltaX:(NSArray<ABI34_0_0RNSVGLength *> *)deltaX
{
    if (deltaX == _deltaX) {
        return;
    }
...
- (void)setDeltaY:(NSArray<ABI34_0_0RNSVGLength *> *)deltaY
{
    if (deltaY == _deltaY) {
        return;
    }
...
- (void)setPositionX:(NSArray<ABI34_0_0RNSVGLength *>*)positionX
{
...
- (void)setPositionY:(NSArray<ABI34_0_0RNSVGLength *>*)positionY
{
...
- (void)setRotate:(NSArray<ABI34_0_0RNSVGLength *> *)rotate
{
    if (rotate == _rotate) {
        return;
    }
...
- (void)setupGlyphContext:(CGContextRef)context
{
    ...
    _glyphContext = [[ABI34_0_0RNSVGGlyphContext alloc] initWithWidth:size.width
                                                      height:size.height];
}
...
- (ABI34_0_0RNSVGText *)textRoot
{
    ABI34_0_0RNSVGText *root = self;
    while (root && [root class] != [ABI34_0_0RNSVGText class]) {
        if (![root isKindOfClass:[ABI34_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        root = (ABI34_0_0RNSVGText*)[root superview];
    }
...
- (NSString *)alignmentBaseline
{
    ...
    while (parent != nil) {
        if ([parent isKindOfClass:[ABI34_0_0RNSVGText class]]) {
            ABI34_0_0RNSVGText* node = (ABI34_0_0RNSVGText*)parent;
    ...
    if (_alignmentBaseline == nil) {
        _alignmentBaseline = ABI34_0_0RNSVGAlignmentBaselineStrings[0];
    }
    return _alignmentBaseline;
}
...
- (ABI34_0_0RNSVGGlyphContext *)getGlyphContext
{
...
- (ABI34_0_0RNSVGText*)getTextAnchorRoot
{
    ABI34_0_0RNSVGGlyphContext* gc = [self.textRoot getGlyphContext];
    NSArray* font = [gc getFontContext];
    ABI34_0_0RNSVGText* node = self;
    UIView* parent = [self superview];
    for (NSInteger i = [font count] - 1; i >= 0; i--) {
        ABI34_0_0RNSVGFontData* fontData = [font objectAtIndex:i];
        if (![parent isKindOfClass:[ABI34_0_0RNSVGText class]] ||
            fontData->textAnchor == ABI34_0_0RNSVGTextAnchorStart ||
            node.positionX != nil) {
            return node;
        }
        node = (ABI34_0_0RNSVGText*) parent;
...
- (CGFloat)getSubtreeTextChunksTotalAdvance
{
    ...
    for (UIView *node in self.subviews) {
        if ([node isKindOfClass:[ABI34_0_0RNSVGText class]]) {
            ABI34_0_0RNSVGText *text = (ABI34_0_0RNSVGText*)node;
