...
@implementation ABI34_0_0RNSVGUse
...
- (void)setUsewidth:(ABI34_0_0RNSVGLength *)usewidth
{
...
- (void)setUseheight:(ABI34_0_0RNSVGLength *)useheight
{
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    if (template) {
        ...
        if ([template isKindOfClass:[ABI34_0_0RNSVGRenderable class]]) {
            [(ABI34_0_0RNSVGRenderable*)template mergeProperties:self];
        }

        if ([template class] == [ABI34_0_0RNSVGSymbol class]) {
            ABI34_0_0RNSVGSymbol *symbol = (ABI34_0_0RNSVGSymbol*)template;
        ...
        if ([template isKindOfClass:[ABI34_0_0RNSVGRenderable class]]) {
            [(ABI34_0_0RNSVGRenderable*)template resetProperties];
        }

        [self endTransparencyLayer:context];
    } else if (self.href) {
        // TODO: calling yellow box here
        ABI34_0_0RCTLogWarn(@"`Use` element expected a pre-defined svg template as `href` prop, template named: %@ is not defined.", self.href);
        return;
    } else {
        return;
    }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    ABI34_0_0RNSVGNode const* template = [self.svgView getDefinedTemplate:self.href];
    if (event) {
        self.active = NO;
    } else if (self.active) {
        return self;
