...
#import "ABI34_0_0EXNativeModuleIntrospection.h"
...
@implementation ABI34_0_0EXNativeModuleIntrospection

@synthesize bridge = _bridge;

ABI34_0_0RCT_EXPORT_MODULE(ExpoNativeModuleIntrospection)

ABI34_0_0RCT_REMAP_METHOD(getNativeModuleNamesAsync,
                 nativeModuleNamesWithResolver:(ABI34_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI34_0_0RCTPromiseRejectBlock)reject) {
  NSError *error;
  NSDictionary<NSString *, ABI34_0_0RCTModuleData *> *moduleDataByName = [self _moduleDataFromBridge:&error];
  if (!moduleDataByName) {
...
ABI34_0_0RCT_REMAP_METHOD(introspectNativeModuleAsync,
  ...
  ABI34_0_0RCTModuleData *moduleData = moduleDataByName[name];
  if (!moduleData) {
    resolve(nil);
    return;
  }
  
  NSArray<id<ABI34_0_0RCTBridgeMethod>> *moduleMethods = moduleData.methods;
  ...
  for (id<ABI34_0_0RCTBridgeMethod> method in moduleMethods) {
    NSString *methodName = [NSString stringWithCString:method.JSMethodName encoding:NSASCIIStringEncoding];
    NSString *functionType = [NSString stringWithCString:ABI34_0_0RCTFunctionDescriptorFromType(method.functionType)
...
- (NSDictionary<NSString *, ABI34_0_0RCTModuleData *> *)_moduleDataFromBridge:(NSError **)error {
  ABI34_0_0RCTBridge *bridge = _bridge;
  if (![NSStringFromClass(bridge.class) isEqualToString:@"ABI34_0_0RCTCxxBridge"]) {
    if (error) {
  ...
  NSDictionary<NSString *, ABI34_0_0RCTModuleData *> *moduleDataByName;
  @try {
