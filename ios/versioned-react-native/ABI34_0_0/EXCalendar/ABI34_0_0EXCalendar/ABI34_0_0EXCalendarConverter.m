...
#import <ABI34_0_0EXCalendar/ABI34_0_0EXCalendarConverter.h>

@implementation ABI34_0_0EXCalendarConverter
...
+ (NSMutableDictionary *)_serializeCalendar:(EKCalendar *)calendar
{
  ...
  serializedCalendar[@"id"] = ABI34_0_0UMNullIfNil(calendar.calendarIdentifier);
  serializedCalendar[@"title"] = ABI34_0_0UMNullIfNil(calendar.title);
  serializedCalendar[@"source"] = ABI34_0_0UMNullIfNil([ABI34_0_0EXCalendarConverter serializeSource:calendar.source]);
  serializedCalendar[@"entityType"] = [ABI34_0_0EXCalendarConverter _entityType:calendar.allowedEntityTypes];
  serializedCalendar[@"color"] = calendar.CGColor ? [ABI34_0_0UMUtilities hexStringWithCGColor:calendar.CGColor] : [NSNull null];
  serializedCalendar[@"type"] = [ABI34_0_0EXCalendarConverter _calendarType:calendar.type];
  serializedCalendar[@"allowsModifications"] = @(calendar.allowsContentModifications);
  serializedCalendar[@"allowedAvailabilities"] = [ABI34_0_0EXCalendarConverter _calendarSupportedAvailabilitiesFromMask:calendar.supportedEventAvailabilities];
...
+ (NSArray *)serializeCalendars:(NSArray *)calendars
{
  ...
  for (EKCalendar *calendar in calendars) {
    [serializedCalendars addObject:[ABI34_0_0EXCalendarConverter _serializeCalendar:calendar]];
  }
...
+ (NSMutableDictionary *)_serializeCalendarItem:(EKCalendarItem *)item withDateFormatter:(NSDateFormatter *)dateFormatter
{
  ...
  serializedItem[@"timeZone"] = ABI34_0_0UMNullIfNil(item.timeZone.name);
  serializedItem[@"url"] = ABI34_0_0UMNullIfNil(item.URL.absoluteString.stringByRemovingPercentEncoding);
  serializedItem[@"notes"] = ABI34_0_0UMNullIfNil(item.notes);
  serializedItem[@"alarms"] = [ABI34_0_0EXCalendarConverter _serializeAlarms:item.alarms withDateFormatter:dateFormatter];

  if (item.hasRecurrenceRules) {
    EKRecurrenceRule *rule = [item.recurrenceRules objectAtIndex:0];
    NSString *frequencyType = [ABI34_0_0EXCalendarConverter _recurrenceFrequency:[rule frequency]];
...
+ (NSArray *)serializeCalendarEvents:(NSArray *)calendarEvents
{
  ...
  for (EKEvent *event in calendarEvents) {
    ...
    [serializedCalendarEvents addObject:[ABI34_0_0EXCalendarConverter serializeCalendarEvent:event]];
  }
...
+ (NSDictionary *)serializeCalendarEvent:(EKEvent *)event
{
  ...
  NSMutableDictionary *formedCalendarEvent = [ABI34_0_0EXCalendarConverter _serializeCalendarItem:event withDateFormatter:dateFormatter];
  ...
  formedCalendarEvent[@"availability"] = [ABI34_0_0EXCalendarConverter _eventAvailability:event.availability];
  formedCalendarEvent[@"status"] = [ABI34_0_0EXCalendarConverter _eventStatus:event.status];
  formedCalendarEvent[@"organizer"] = ABI34_0_0UMNullIfNil([ABI34_0_0EXCalendarConverter _serializeAttendee:event.organizer]);
...
+ (NSArray *)serializeReminders:(NSArray<EKReminder *> *)reminders
{
  ...
  for (EKReminder *reminder in reminders) {
    [serializedReminders addObject:[ABI34_0_0EXCalendarConverter serializeReminder:reminder]];
  }
...
+ (NSDictionary *)serializeReminder:(EKReminder *)reminder
{
  ...
  NSMutableDictionary *formedReminder = [ABI34_0_0EXCalendarConverter _serializeCalendarItem:reminder withDateFormatter:dateFormatter];
...
+ (NSArray *)serializeAttendees:(NSArray<EKParticipant *> *)attendees
{
  ...
  for (EKParticipant *attendee in attendees) {
    [serializedAttendees addObject:[ABI34_0_0EXCalendarConverter _serializeAttendee:attendee]];
  }
...
+ (NSDictionary *)_serializeAttendee:(EKParticipant *)attendee
{
  ...
  if (attendee.participantRole) {
    formedAttendee[@"role"] = [ABI34_0_0EXCalendarConverter _participantRole:attendee.participantRole];
  }

  if (attendee.participantStatus) {
    formedAttendee[@"status"] = [ABI34_0_0EXCalendarConverter _participantStatus:attendee.participantStatus];
  }

  if (attendee.participantType) {
    formedAttendee[@"type"] = [ABI34_0_0EXCalendarConverter _participantType:attendee.participantType];
  }
