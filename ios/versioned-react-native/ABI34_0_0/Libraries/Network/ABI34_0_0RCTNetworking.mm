...
#import <ReactABI34_0_0/ABI34_0_0RCTURLRequestHandler.h>
#import <ReactABI34_0_0/ABI34_0_0RCTUtils.h>

#import "ABI34_0_0RCTHTTPRequestHandler.h"

typedef ABI34_0_0RCTURLRequestCancellationBlock (^ABI34_0_0RCTHTTPQueryResult)(NSError *error, NSDictionary<NSString *, id> *result);

@interface ABI34_0_0RCTNetworking ()

- (ABI34_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(NSDictionary<NSString *, id> *)data
                                                 callback:(ABI34_0_0RCTHTTPQueryResult)callback;
@end

/**
 * Helper to convert FormData payloads into multipart/formdata requests.
 */
@interface ABI34_0_0RCTHTTPFormDataHelper : NSObject

@property (nonatomic, weak) ABI34_0_0RCTNetworking *networker;

@end

@implementation ABI34_0_0RCTHTTPFormDataHelper
{
  NSMutableArray<NSDictionary<NSString *, id> *> *_parts;
  NSMutableData *_multipartBody;
  ABI34_0_0RCTHTTPQueryResult _callback;
  NSString *_boundary;
}

static NSString *ABI34_0_0RCTGenerateFormBoundary()
{
...
- (ABI34_0_0RCTURLRequestCancellationBlock)process:(NSArray<NSDictionary *> *)formData
                                 callback:(ABI34_0_0RCTHTTPQueryResult)callback
{
  ABI34_0_0RCTAssertThread(_networker.methodQueue, @"process: must be called on method queue");
  ...
  _boundary = ABI34_0_0RCTGenerateFormBoundary();
...
- (ABI34_0_0RCTURLRequestCancellationBlock)handleResult:(NSDictionary<NSString *, id> *)result
                                         error:(NSError *)error
{
  ABI34_0_0RCTAssertThread(_networker.methodQueue, @"handleResult: must be called on method queue");
 ...
 */
@implementation ABI34_0_0RCTNetworking
{
  NSMutableDictionary<NSNumber *, ABI34_0_0RCTNetworkTask *> *_tasksByRequestID;
  std::mutex _handlersLock;
  NSArray<id<ABI34_0_0RCTURLRequestHandler>> *_handlers;
  NSMutableArray<id<ABI34_0_0RCTNetworkingRequestHandler>> *_requestHandlers;
  NSMutableArray<id<ABI34_0_0RCTNetworkingResponseHandler>> *_responseHandlers;
}
...
- (id<ABI34_0_0RCTURLRequestHandler>)handlerForRequest:(NSURLRequest *)request
{
  if (!request.URL) {
    return nil;
  }

  {
    ...
    if (!_handlers) {
      ...
      _handlers = [[self.bridge modulesConformingToProtocol:@protocol(ABI34_0_0RCTURLRequestHandler)] sortedArrayUsingComparator:^NSComparisonResult(id<ABI34_0_0RCTURLRequestHandler> a, id<ABI34_0_0RCTURLRequestHandler> b) {
  ...
  if (ABI34_0_0RCT_DEBUG) {
    ...
    id<ABI34_0_0RCTURLRequestHandler> previousHandler = nil;
    for (id<ABI34_0_0RCTURLRequestHandler> handler in _handlers) {
      ...
      if ([handler canHandleRequest:request]) {
        if (previousHandler) {
          if (priority == previousPriority) {
            ABI34_0_0RCTLogError(@"The ABI34_0_0RCTURLRequestHandlers %@ and %@ both reported that"
...
- (ABI34_0_0RCTURLRequestCancellationBlock)buildRequest:(NSDictionary<NSString *, id> *)query
                                 completionBlock:(void (^)(NSURLRequest *request))block
{
  ABI34_0_0RCTAssertThread(_methodQueue, @"buildRequest: must be called on method queue");

  NSURL *URL = [ABI34_0_0RCTConvert NSURL:query[@"url"]]; // this is marked as nullable in JS, but should not be null
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:URL];
  request.HTTPMethod = [ABI34_0_0RCTConvert NSString:ABI34_0_0RCTNilIfNull(query[@"method"])].uppercaseString ?: @"GET";
  ...
  NSDictionary *headers = [ABI34_0_0RCTConvert NSDictionary:query[@"headers"]];
  ...
  request.timeoutInterval = [ABI34_0_0RCTConvert NSTimeInterval:query[@"timeout"]];
  request.HTTPShouldHandleCookies = [ABI34_0_0RCTConvert BOOL:query[@"withCredentials"]];
  NSDictionary<NSString *, id> *data = [ABI34_0_0RCTConvert NSDictionary:ABI34_0_0RCTNilIfNull(query[@"data"])];
  ...
  return [self processDataForHTTPQuery:data callback:^(NSError *error, NSDictionary<NSString *, id> *result) {
    if (error) {
      ABI34_0_0RCTLogError(@"Error processing request body: %@", error);
      // Ideally we'd circle back to JS here and notify an error/abort on the request.
      return (ABI34_0_0RCTURLRequestCancellationBlock)nil;
    }
    ...
    if ([request.allHTTPHeaderFields[@"Content-Encoding"] isEqualToString:@"gzip"]) {
      request.HTTPBody = ABI34_0_0RCTGzipData(request.HTTPBody, -1 /* default */);
 ...
 *
 */
- (ABI34_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(nullable NSDictionary<NSString *, id> *)query callback:
(ABI34_0_0RCTURLRequestCancellationBlock (^)(NSError *error, NSDictionary<NSString *, id> *result))callback
{
  ABI34_0_0RCTAssertThread(_methodQueue, @"processDataForHTTPQuery: must be called on method queue");

  if (!query) {
    return callback(nil, nil);
  }
  for (id<ABI34_0_0RCTNetworkingRequestHandler> handler in _requestHandlers) {
  ...
  NSData *body = [ABI34_0_0RCTConvert NSData:query[@"string"]];
  if (body) {
    return callback(nil, @{@"body": body});
  }
  NSString *base64String = [ABI34_0_0RCTConvert NSString:query[@"base64"]];
  if (base64String) {
    NSData *data = [[NSData alloc] initWithBase64EncodedString:base64String options:0];
    return callback(nil, @{@"body": data});
  }
  NSURLRequest *request = [ABI34_0_0RCTConvert NSURLRequest:query[@"uri"]];
  if (request) {

    __block ABI34_0_0RCTURLRequestCancellationBlock cancellationBlock = nil;
    ABI34_0_0RCTNetworkTask *task = [self networkTaskWithRequest:request completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
      dispatch_async(self->_methodQueue, ^{
        cancellationBlock = callback(error, data ? @{@"body": data, @"contentType": ABI34_0_0RCTNullIfNil(response.MIMEType)} : nil);
      });
    }];

    [task start];

    __weak ABI34_0_0RCTNetworkTask *weakTask = task;
    return ^{
      [weakTask cancel];
  ...
  NSArray<NSDictionary *> *formData = [ABI34_0_0RCTConvert NSDictionaryArray:query[@"formData"]];
  if (formData) {
    ABI34_0_0RCTHTTPFormDataHelper *formDataHelper = [ABI34_0_0RCTHTTPFormDataHelper new];
...
- (void)sendData:(NSData *)data
    responseType:(NSString *)responseType
        response:(NSURLResponse *)response
         forTask:(ABI34_0_0RCTNetworkTask *)task
{
  ABI34_0_0RCTAssertThread(_methodQueue, @"sendData: must be called on method queue");

  id responseData = nil;
  for (id<ABI34_0_0RCTNetworkingResponseHandler> handler in _responseHandlers) {
  ...
  if (!responseData) {
    ...
    if ([responseType isEqualToString:@"text"]) {
      ...
      responseData = [ABI34_0_0RCTNetworking decodeTextData:data fromResponse:task.response withCarryData:nil];
      if (!responseData) {
        ABI34_0_0RCTLogWarn(@"Received data was not a string, or was not a recognised encoding.");
        return;
      }
    } else if ([responseType isEqualToString:@"base64"]) {
      responseData = [data base64EncodedStringWithOptions:0];
    } else {
      ABI34_0_0RCTLogWarn(@"Invalid responseType: %@", responseType);
      return;
    }
  }
...
- (void)sendRequest:(NSURLRequest *)request
 ...
 incrementalUpdates:(BOOL)incrementalUpdates
     responseSender:(ABI34_0_0RCTResponseSenderBlock)responseSender
{
  ABI34_0_0RCTAssertThread(_methodQueue, @"sendRequest: must be called on method queue");
  __weak __typeof(self) weakSelf = self;
  __block ABI34_0_0RCTNetworkTask *task;
  ABI34_0_0RCTURLRequestProgressBlock uploadProgressBlock = ^(int64_t progress, int64_t total) {
    NSArray *responseJSON = @[task.requestID, @((double)progress), @((double)total)];
    [weakSelf sendEventWithName:@"didSendNetworkData" body:responseJSON];
  };

  ABI34_0_0RCTURLRequestResponseBlock responseBlock = ^(NSURLResponse *response) {
  ...
  ABI34_0_0RCTURLRequestIncrementalDataBlock incrementalDataBlock = nil;
  ABI34_0_0RCTURLRequestProgressBlock downloadProgressBlock = nil;
  if (incrementalUpdates) {
    if ([responseType isEqualToString:@"text"]) {
      ...
      incrementalDataBlock = ^(NSData *data, int64_t progress, int64_t total) {
        ...
        NSString *responseString = [ABI34_0_0RCTNetworking decodeTextData:data
  ...
  ABI34_0_0RCTURLRequestCompletionBlock completionBlock =
  ^(NSURLResponse *response, NSData *data, NSError *error) {
    ...
    NSArray *responseJSON = @[task.requestID,
                              ABI34_0_0RCTNullIfNil(error.localizedDescription),
...
- (ABI34_0_0RCTNetworkTask *)networkTaskWithRequest:(NSURLRequest *)request completionBlock:(ABI34_0_0RCTURLRequestCompletionBlock)completionBlock
{
  id<ABI34_0_0RCTURLRequestHandler> handler = [self handlerForRequest:request];
  if (!handler) {
    ABI34_0_0RCTLogError(@"No suitable URL request handler found for %@", request.URL);
    return nil;
  }

  ABI34_0_0RCTNetworkTask *task = [[ABI34_0_0RCTNetworkTask alloc] initWithRequest:request
                                                         handler:handler
...
ABI34_0_0RCT_EXPORT_METHOD(sendRequest:(NSDictionary *)query
                  responseSender:(ABI34_0_0RCTResponseSenderBlock)responseSender)
{
  ...
  [self buildRequest:query completionBlock:^(NSURLRequest *request) {
    NSString *responseType = [ABI34_0_0RCTConvert NSString:query[@"responseType"]];
    BOOL incrementalUpdates = [ABI34_0_0RCTConvert BOOL:query[@"incrementalUpdates"]];
...
ABI34_0_0RCT_EXPORT_METHOD(abortRequest:(nonnull NSNumber *)requestID)
{
  [_tasksByRequestID[requestID] cancel];
  [_tasksByRequestID removeObjectForKey:requestID];
}

ABI34_0_0RCT_EXPORT_METHOD(clearCookies:(ABI34_0_0RCTResponseSenderBlock)responseSender)
{
...
@implementation ABI34_0_0RCTBridge (ABI34_0_0RCTNetworking)

- (ABI34_0_0RCTNetworking *)networking
{
  return [self moduleForClass:[ABI34_0_0RCTNetworking class]];
}
