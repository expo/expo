...
typedef NS_OPTIONS(NSInteger, ABI34_0_0RCTTextSizeComparisonOptions) {
  ABI34_0_0RCTTextSizeComparisonSmaller     = 1 << 0,
  ABI34_0_0RCTTextSizeComparisonLarger      = 1 << 1,
  ABI34_0_0RCTTextSizeComparisonWithinRange = 1 << 2,
};

@implementation NSTextStorage (ABI34_0_0FontScaling)

- (void)abi34_0_0_rct_scaleFontSizeToFitSize:(CGSize)size
               ...
               maximumFontSize:(CGFloat)maximumFontSize
{
  ...
  while (true) {
    [self abi34_0_0_rct_scaleFontSizeWithRatio:ratio
                 minimumFontSize:minimumFontSize
                 maximumFontSize:maximumFontSize];

    ABI34_0_0RCTTextSizeComparisonOptions comparsion =
      [self abi34_0_0_rct_compareToSize:size thresholdRatio:0.01];

    if (
        (comparsion & ABI34_0_0RCTTextSizeComparisonWithinRange) &&
        (comparsion & ABI34_0_0RCTTextSizeComparisonSmaller)
    ) {
      return;
    } else if (comparsion & ABI34_0_0RCTTextSizeComparisonSmaller) {
      bottomRatio = ratio;
    ...
    if (
      ...
      [self abi34_0_0_rct_scaleFontSizeWithRatio:lastRatioWhichFits
                   minimumFontSize:minimumFontSize
                   maximumFontSize:maximumFontSize];
      return;
    }
...
- (ABI34_0_0RCTTextSizeComparisonOptions)abi34_0_0_rct_compareToSize:(CGSize)size thresholdRatio:(CGFloat)thresholdRatio
{
  ...
  if (truncatedGlyphRange.location != NSNotFound) {
    return ABI34_0_0RCTTextSizeComparisonLarger;
  }
  ...
  ABI34_0_0RCTTextSizeComparisonOptions result = 0;

  result |= (fitsSize) ? ABI34_0_0RCTTextSizeComparisonSmaller : ABI34_0_0RCTTextSizeComparisonLarger;

  if (ABS(measuredSize.width - size.width) < thresholdSize.width) {
    result = result | ABI34_0_0RCTTextSizeComparisonWithinRange;
  }

  return result;
}

- (void)abi34_0_0_rct_scaleFontSizeWithRatio:(CGFloat)ratio
               minimumFontSize:(CGFloat)minimumFontSize
               maximumFontSize:(CGFloat)maximumFontSize
{
  [self beginEditing];
  ...
  [self enumerateAttribute:NSFontAttributeName
    ...
    ^(UIFont *_Nullable font, NSRange range, BOOL *_Nonnull stop) {
      ...
      if (scaledFont) {
        ...
        ABI34_0_0RCTLogError(@"Font \"%@"" doesn't support automatic scaling.", font.familyName);
      }
    }
  ];
