...
@implementation ABI34_0_0RCTPropsAnimatedNode
{
...
- (void)performUpdate
{
  ...
  for (NSNumber *parentTag in self.parentNodes.keyEnumerator) {
    ABI34_0_0RCTAnimatedNode *parentNode = [self.parentNodes objectForKey:parentTag];
    if ([parentNode isKindOfClass:[ABI34_0_0RCTStyleAnimatedNode class]]) {
      [self->_propsDictionary addEntriesFromDictionary:[(ABI34_0_0RCTStyleAnimatedNode *)parentNode propsDictionary]];
      
    } else if ([parentNode isKindOfClass:[ABI34_0_0RCTValueAnimatedNode class]]) {
      NSString *property = [self propertyNameForParentTag:parentTag];
      CGFloat value = [(ABI34_0_0RCTValueAnimatedNode *)parentNode value];
