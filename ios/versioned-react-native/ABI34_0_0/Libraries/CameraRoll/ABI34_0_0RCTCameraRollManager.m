...
#import "ABI34_0_0RCTCameraRollManager.h"
...
#import "ABI34_0_0RCTAssetsLibraryRequestHandler.h"

@implementation ABI34_0_0RCTConvert (PHAssetCollectionSubtype)

ABI34_0_0RCT_ENUM_CONVERTER(PHAssetCollectionSubtype, (@{
...
@implementation ABI34_0_0RCTConvert (PHFetchOptions)
...
+ (PHFetchOptions *)PHFetchOptionsFromMediaType:(NSString *)mediaType
{
  ...
  if ([lowercase isEqualToString:@"photos"]) {
    ...
    if (![lowercase isEqualToString:@"all"]) {
      ABI34_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
...
@implementation ABI34_0_0RCTCameraRollManager
...
static void requestPhotoLibraryAccess(ABI34_0_0RCTPromiseRejectBlock reject, PhotosAuthorizedBlock authorizedBlock) {
...
ABI34_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
...
static void ABI34_0_0RCTResolvePromise(ABI34_0_0RCTPromiseResolveBlock resolve,
...
ABI34_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI34_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI34_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger const first = [ABI34_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *const afterCursor = [ABI34_0_0RCTConvert NSString:params[@"after"]];
  NSString *const groupName = [ABI34_0_0RCTConvert NSString:params[@"groupName"]];
  NSString *const groupTypes = [[ABI34_0_0RCTConvert NSString:params[@"groupTypes"]] lowercaseString];
  NSString *const mediaType = [ABI34_0_0RCTConvert NSString:params[@"assetType"]];
  NSArray<NSString *> *const mimeTypes = [ABI34_0_0RCTConvert NSStringArray:params[@"mimeTypes"]];
  ...
  PHAssetCollectionSubtype const collectionSubtype = [ABI34_0_0RCTConvert PHAssetCollectionSubtype:groupTypes];

  // Predicate for fetching assets within a collection
  PHFetchOptions *const assetFetchOptions = [ABI34_0_0RCTConvert PHFetchOptionsFromMediaType:mediaType];
    ...
    [assetCollectionFetchResult enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull assetCollection, NSUInteger collectionIdx, BOOL * _Nonnull stopCollections) {
      ...
      [assetsFetchResult enumerateObjectsUsingBlock:^(PHAsset * _Nonnull asset, NSUInteger assetIdx, BOOL * _Nonnull stopAssets) {
        ...
        if (first == assets.count) {
          ...
          *stopCollections = YES;
          hasNextPage = YES;
          ABI34_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
          ABI34_0_0RCTResolvePromise(resolve, assets, hasNextPage);
          resolvedPromise = YES;
          return;
        }
...
ABI34_0_0RCT_EXPORT_METHOD(deletePhotos:(NSArray<NSString *>*)assets
                  resolve:(ABI34_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI34_0_0RCTPromiseRejectBlock)reject)
{
  NSArray<NSURL *> *assets_ = [ABI34_0_0RCTConvert NSURLArray:assets];
  [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI34_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
