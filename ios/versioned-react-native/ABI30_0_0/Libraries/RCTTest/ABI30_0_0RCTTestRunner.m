...
#import "ABI30_0_0RCTTestRunner.h"
...
@implementation ABI30_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI30_0_0RCTBridgeModuleListProvider _moduleProvider;
  NSString *_appPath;
}
...
{
  ABI30_0_0RCTAssertParam(app);
  ABI30_0_0RCTAssertParam(referenceDirectory);
...
- (void)updateScript
{
  if (getenv("CI_USE_PACKAGER") || _useBundler) {
    ...
    _scriptURL = [[NSBundle bundleForClass:[ABI30_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
  }
  ABI30_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
}
...
{
  __weak ABI30_0_0RCTBridge *batchedBridge;

  @autoreleasepool {
    __block NSMutableArray<NSString *> *errors = nil;
    ABI30_0_0RCTLogFunction defaultLogFunction = ABI30_0_0RCTGetLogFunction();
    ABI30_0_0RCTSetLogFunction(^(ABI30_0_0RCTLogLevel level, ABI30_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      ...
      if (level >= ABI30_0_0RCTLogLevelError) {
        if (errors == nil) {
    ...
    ABI30_0_0RCTBridge *bridge = [[ABI30_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];
    [bridge.devSettings setIsDebuggingRemotely:_useJSDebugger];
    batchedBridge = [bridge batchedBridge];

    ABI30_0_0RCTRootView *rootView = [[ABI30_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI30_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI30_0_0RCTTestModule class]];
    ABI30_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    testModule.controller = _testController;
    testModule.testSelector = test;
    testModule.testSuffix = _testSuffix;
    testModule.view = rootView;

    UIViewController *vc = ABI30_0_0RCTSharedApplication().delegate.window.rootViewController;
    vc.view = [UIView new];
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI30_0_0RCTTestStatusPending && errors == nil) {
    ...
    ABI30_0_0RCTSetLogFunction(defaultLogFunction);
    ...
    ABI30_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);
#endif

    if (expectErrorBlock) {
      ABI30_0_0RCTAssert(expectErrorBlock(errors[0]), @"Expected an error but the first one was missing or did not match.");
    } else {
      ABI30_0_0RCTAssert(errors == nil, @"RedBox errors: %@", errors);
      ABI30_0_0RCTAssert(testModule.status != ABI30_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI30_0_0RCTAssert(testModule.status == ABI30_0_0RCTTestStatusPassed, @"Test failed");
    }
