...
#include <cxxReactABI30_0_0/ABI30_0_0MethodCall.h>
...
TEST(parseMethodCalls, MapReturn) {
  ...
  ABI30_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI30_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI30_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
