...
// ReactABI30_0_0Android/src/main/java/com/facebook/ReactABI30_0_0/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int RECONNECT_DELAY_MS = 2000;

@implementation ABI30_0_0RCTBundleStatus
@end

@interface ABI30_0_0RCTInspectorPackagerConnection () <ABI30_0_0RCTSRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI30_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  ABI30_0_0RCTSRWebSocket *_webSocket;
  dispatch_queue_t _jsQueue;
  BOOL _closed;
  BOOL _suppressConnectionErrors;
  ABI30_0_0RCTBundleStatusProvider _bundleStatusProvider;
}
@end

@interface ABI30_0_0RCTInspectorRemoteConnection () {
  __weak ABI30_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI30_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI30_0_0RCTInspectorPackagerConnection
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI30_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  if (_inspectorConnections[pageId]) {
    ...
    ABI30_0_0RCTLogError(@"Already connected: %@", pageId);
    return;
  }

  ABI30_0_0RCTInspectorRemoteConnection *remoteConnection =
    [[ABI30_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self
                                                              pageId:pageId];

  ABI30_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI30_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI30_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (NSArray *)pages
{
  NSArray<ABI30_0_0RCTInspectorPage *> *pages = [ABI30_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];

  ABI30_0_0RCTBundleStatusProvider statusProvider = _bundleStatusProvider;
  ABI30_0_0RCTBundleStatus *bundleStatus = statusProvider == nil
    ? nil
    : statusProvider();

  for (ABI30_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(ABI30_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  // warn but don't die on unrecognized messages
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI30_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI30_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI30_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@",
      messageText);
    return;
  }
...
- (void)webSocket:(ABI30_0_0RCTSRWebSocket *)webSocket didCloseWithCode:(NSInteger)code
...
{
  if (_closed) {
    ABI30_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresopnding android code has a lot of custom config options for
  // timeouts, but it appears the iOS ABI30_0_0RCTSRWebSocket API doesn't have the same
  // implemented options.
  _webSocket = [[ABI30_0_0RCTSRWebSocket alloc] initWithURL:_url];
...
{
  if (_closed) {
    ABI30_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI30_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI30_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(
    dispatch_time(DISPATCH_TIME_NOW, RECONNECT_DELAY_MS *NSEC_PER_MSEC),
    dispatch_get_main_queue(), ^{
      ABI30_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI30_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI30_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
    ...
    withCause:(NSError *)cause
{
  ...
  if (![cause.domain isEqual:NSPOSIXErrorDomain] || cause.code != ECONNREFUSED) {
    ABI30_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
  }
...
@implementation ABI30_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI30_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
