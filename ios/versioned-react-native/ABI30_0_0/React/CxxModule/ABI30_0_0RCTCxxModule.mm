...
#import "ABI30_0_0RCTCxxMethod.h"

using namespace facebook::ReactABI30_0_0;

@implementation ABI30_0_0RCTCxxModule
{
...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI30_0_0RCTAssert([ABI30_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
                @"CxxModule class name %@ does not match runtime name %s",
                ABI30_0_0RCTBridgeModuleNameForClass([self class]), _module->getName().c_str());
    }
  }
}
...
{
  ABI30_0_0RCTAssert(NO, @"Subclass %@ must override createModule", [self class]);
  return nullptr;
}
...
- (NSArray<id<ABI30_0_0RCTBridgeMethod>> *)methodsToExport;
{
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI30_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
