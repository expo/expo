...
const NSInteger ABI30_0_0RCTTextUpdateLagWarningThreshold = 3;

NSString *ABI30_0_0RCTNormalizeInputEventName(NSString *eventName)
{
...
static NSNumber *ABI30_0_0RCTGetEventID(id<ABI30_0_0RCTEvent> event)
{
  return @(
...
@implementation ABI30_0_0RCTEventDispatcher
{
  ...
  NSMutableDictionary<NSNumber *, id<ABI30_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI30_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_bridge enqueueJSCall:@"ABI30_0_0RCTNativeAppEventEmitter"
                  method:@"emit"
...
- (void)sendInputEventWithName:(NSString *)name body:(NSDictionary *)body
{
  if (ABI30_0_0RCT_DEBUG) {
    ABI30_0_0RCTAssert([body[@"target"] isKindOfClass:[NSNumber class]],
      @"Event body dictionary must include a 'target' property containing a ReactABI30_0_0 tag");
  }

  name = ABI30_0_0RCTNormalizeInputEventName(name);
  [_bridge enqueueJSCall:@"ABI30_0_0RCTEventEmitter"
                  method:@"receiveEvent"
...
- (void)sendEvent:(id<ABI30_0_0RCTEvent>)event
{
  ...
  for (id<ABI30_0_0RCTEventDispatcherObserver> observer in _observers) {
  ...
  NSNumber *eventID = ABI30_0_0RCTGetEventID(event);

  id<ABI30_0_0RCTEvent> previousEvent = _events[eventID];
  if (previousEvent) {
    ABI30_0_0RCTAssert([event canCoalesce], @"Got event %@ which cannot be coalesced, but has the same eventID %@ as the previous event %@", event, eventID, previousEvent);
...
@implementation ABI30_0_0RCTBridge (ABI30_0_0RCTEventDispatcher)

- (ABI30_0_0RCTEventDispatcher *)eventDispatcher
{
  return [self moduleForClass:[ABI30_0_0RCTEventDispatcher class]];
}
