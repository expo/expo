...
NSString *const ABI30_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI30_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI30_0_0RCTAccessibilityManager ()
...
@implementation ABI30_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI30_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI30_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI30_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI30_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI30_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI30_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI30_0_0Tag)
{
...
ABI30_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI30_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI30_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI30_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI30_0_0RCTResponseSenderBlock)callback
...
@implementation ABI30_0_0RCTBridge (ABI30_0_0RCTAccessibilityManager)

- (ABI30_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI30_0_0RCTAccessibilityManager class]];
}
