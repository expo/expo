...
#define ABI30_0_0EX_IS_METHOD_EXPORTED(methodName) \
[methodName hasPrefix:@ABI30_0_0EXPAND_AND_QUOTE(ABI30_0_0EX_EXPORTED_METHODS_PREFIX)]

static const NSString *noNameExceptionName = @"No custom +(const NSString *)exportedModuleName implementation.";
static const NSString *noNameExceptionReasonFormat = @"You've subclassed an ABI30_0_0EXExportedModule in %@, but didn't override the +(const NSString *)exportedModuleName method. Override this method and return a name for your exported module.";
...
@interface ABI30_0_0EXExportedModule ()
...
@implementation ABI30_0_0EXExportedModule
...
{
  __weak ABI30_0_0EXExportedModule *weakSelf = self;
  dispatch_once(&_methodQueueSetupOnce, ^{
    __strong ABI30_0_0EXExportedModule *strongSelf = weakSelf;
    if (strongSelf) {
  ...
  while (klass) {
    ...
    @try {
      for(int i = 0; i < methodsCount; i++) {
        ...
        if (ABI30_0_0EX_IS_METHOD_EXPORTED(methodName)) {
          IMP imp = method_getImplementation(method);
          const ABI30_0_0EXMethodInfo *info = ((const ABI30_0_0EXMethodInfo *(*)(id, SEL))imp)(klass, methodSelector);
...
- (void)callExportedMethod:(NSString *)methodName withArguments:(NSArray *)arguments resolver:(ABI30_0_0EXPromiseResolveBlock)resolve rejecter:(ABI30_0_0EXPromiseRejectBlock)reject
{
  ...
  if (methodSignature == nil) {
    ...
    // it means that it has been exported with ABI30_0_0EX_EXPORT_METHOD and if we cannot find method signature
