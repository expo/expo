//
//  ABI30_0_0EXFaceDetectorManager.m
//  Exponent
//
...
@interface ABI30_0_0EXFaceDetectorManager() <GMVDataOutputDelegate>
...
@implementation ABI30_0_0EXFaceDetectorManager
...
- (void)setIsEnabled:(BOOL)newFaceDetecting
{
  ...
  if ([self isDetectingFaces] != newFaceDetecting) {
    ...
    __weak ABI30_0_0EXFaceDetectorManager *weakSelf = self;
    [self _runBlockIfQueueIsPresent:^{
      __strong ABI30_0_0EXFaceDetectorManager *strongSelf = weakSelf;
      if (strongSelf) {
...
- (void)tryEnablingFaceDetection
{
  ...
  if ([self isDetectingFaces]) {
    @try {
      ...
      ABI30_0_0EXLogWarn(@"%@", [exception description]);
    }
  }
...
- (void)dataOutput:(GMVDataOutput *)dataOutput didFinishedDetection:(NSArray<__kindof GMVFeature *> *)results
{
  ...
  // width or height started happen after refactor: moving face detection logic from ABI30_0_0EXCameraManager to ABI30_0_0EXFaceDetectorManager.
  ...
  CGAffineTransform transform = [ABI30_0_0EXFaceDetectorUtils transformFromDeviceOutput:dataOutput toInterfaceVideoOrientation:interfaceVideoOrientation];
  
  ABI30_0_0EXFaceEncoder *faceEncoder = [[ABI30_0_0EXFaceEncoder alloc] initWithTransform:transform];
