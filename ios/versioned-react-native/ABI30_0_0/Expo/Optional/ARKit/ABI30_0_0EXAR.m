...
#import "ABI30_0_0EXAR.h"
#import "ABI30_0_0EXUnversioned.h"
#import "ABI30_0_0EXGLView.h"
#import <ReactABI30_0_0/ABI30_0_0RCTUIManager.h>
#import "ABI30_0_0EXGLARSessionManager.h"

@interface ABI30_0_0EXAR () <ABI30_0_0EXGLARSessionManagerDelegate>

@property (nonatomic, strong) id arSessionManager;

@end

@implementation ABI30_0_0EXAR

ABI30_0_0RCT_EXPORT_MODULE(ExponentAR);
...
+ (NSMutableArray *)serializeARVideoFormats:(NSArray<ARVideoFormat *>*)videoFormats  API_AVAILABLE(ios(11.3))
{
  ...
  if (@available(iOS 11.3, *)) {
    for (ARVideoFormat *videoFormat in videoFormats) {
      [output addObject:[ABI30_0_0EXAR serializeARVideoFormat: videoFormat]];
    }
  }
...
- (NSDictionary<NSString *, NSString *> *)constantsToExport
{
  ...
  if (@available(iOS 11.3, *)) {
    [output addEntriesFromDictionary:@{
                                       @"ARKitVersion": @"1.5",
//                                       @"FaceTrackingVideoFormats": [ABI30_0_0EXAR serializeARVideoFormats:[ARFaceTrackingConfiguration supportedVideoFormats]],
                                       @"WorldTrackingVideoFormats": [ABI30_0_0EXAR serializeARVideoFormats:[ARWorldTrackingConfiguration supportedVideoFormats]],
                                       @"OrientationTrackingVideoFormats": [ABI30_0_0EXAR serializeARVideoFormats:[AROrientationTrackingConfiguration supportedVideoFormats]],
                                       }];
  }
...
ABI30_0_0RCT_REMAP_METHOD(startAsync,
                 startAsyncWithReactABI30_0_0Tag:(nonnull NSNumber *)tag
                 ...
                 rejecter:(ABI30_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI30_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ...
    if (![view isKindOfClass:[ABI30_0_0EXGLView class]]) {
      reject(@"E_GLVIEW_MANAGER_BAD_VIEW_TAG", nil, ABI30_0_0RCTErrorWithMessage(@"ABI30_0_0EXAR.startARSessionAsync: Expected an ABI30_0_0EXGLView"));
      return;
    }
    ...
    ABI30_0_0EXGLView *exglView = (ABI30_0_0EXGLView *)view;
    
    Class sessionManagerClass = NSClassFromString(@"ABI30_0_0EXGLARSessionManager");
    if (sessionManagerClass) {
      ...
      if (response[@"error"]) {
        reject(@"ERR_ARKIT_FAILED_TO_INIT", response[@"error"], ABI30_0_0RCTErrorWithMessage(response[@"error"]));
      } else {
        resolve(response);
      }
    } else {
      NSString *response = @"ARKIT capabilities were not included with this build.";
      reject(@"ERR_ARKIT_FAILED_TO_INIT", response, ABI30_0_0RCTErrorWithMessage(response));
    }
  }];
}

ABI30_0_0RCT_REMAP_METHOD(stopAsync,
                 resolver:(ABI30_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI30_0_0RCTPromiseRejectBlock)reject)
{
  [self.bridge.uiManager addUIBlock:^(__unused ABI30_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
ABI30_0_0RCT_EXPORT_METHOD(reset)
{
  [_arSessionManager reset];
}

ABI30_0_0RCT_REMAP_METHOD(setProvidesAudioData,
                 providesAudioData:(BOOL)providesAudioData)
{
  [_arSessionManager setProvidesAudioData:providesAudioData];
}

ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getProvidesAudioData, NSNumber *, getProvidesAudioData)
{
  return @([_arSessionManager providesAudioData]);
}

ABI30_0_0RCT_REMAP_METHOD(setLightEstimationEnabled,
                 lightEstimationEnabled:(BOOL)lightEstimationEnabled)
{
  [_arSessionManager setLightEstimationEnabled:lightEstimationEnabled];
}

ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getLightEstimationEnabled, NSNumber *, getLightEstimationEnabled)
{
  return @([_arSessionManager lightEstimationEnabled]);
}

ABI30_0_0RCT_REMAP_METHOD(setAutoFocusEnabled,
                 isAutoFocusEnabled:(BOOL)isAutoFocusEnabled)
{
  [_arSessionManager setAutoFocusEnabled:isAutoFocusEnabled];
}

ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getAutoFocusEnabled, NSNumber *, getAutoFocusEnabled)
{
  return @([_arSessionManager autoFocusEnabled]);
}

ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getPlaneDetection, NSString *, getPlaneDetection)
{
...
ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(performHitTest,
...
ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getWorldAlignment, NSString *, getWorldAlignment)
{
  NSArray *items = @[@"gravity", @"gravityAndHeading", @"alignmentCamera"];
  return [items objectAtIndex:[_arSessionManager worldAlignment]];
}

ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCurrentFrame,
                                      nullable NSDictionary *,
...
ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getARMatrices,
                                      nullable NSDictionary *,
...
ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getVideoFormat, nullable NSDictionary *, getVideoFormat)
{
  if (!_arSessionManager) {
    return nil;
  }
  if (@available(iOS 11.3, *)) {
    ...
    return [ABI30_0_0EXAR serializeARVideoFormat:videoFormat];
  }
  return nil;
}

ABI30_0_0RCT_REMAP_BLOCKING_SYNCHRONOUS_METHOD(getCameraTexture, NSNumber *, getCameraTexture)
{
  return @([_arSessionManager cameraTexture]);
}

ABI30_0_0RCT_REMAP_METHOD(setShouldAttemptRelocalization,
...
ABI30_0_0RCT_REMAP_METHOD(setWorldOriginAsync,
...
ABI30_0_0RCT_REMAP_METHOD(setConfigurationAsync,
...
ABI30_0_0RCT_REMAP_METHOD(setDetectionImagesAsync,
