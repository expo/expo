...
#import "ABI30_0_0EXFingerprint.h"
...
@implementation ABI30_0_0EXFingerprint

ABI30_0_0RCT_EXPORT_MODULE(ExponentFingerprint)

ABI30_0_0RCT_EXPORT_METHOD(hasHardwareAsync:(ABI30_0_0RCTPromiseResolveBlock)resolve
...
ABI30_0_0RCT_EXPORT_METHOD(isEnrolledAsync:(ABI30_0_0RCTPromiseResolveBlock)resolve
                  rejecter:(ABI30_0_0RCTPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;
...
ABI30_0_0RCT_EXPORT_METHOD(authenticateAsync:(NSString *)reason
                  ...
                  rejecter:(ABI30_0_0RCTPromiseRejectBlock)reject)
{
  ...
  [ABI30_0_0EXUtilities performSynchronouslyOnMainThread:^{
    isFaceIdDevice = ABI30_0_0EXIsFaceIDDevice();
  }];
  if (isFaceIdDevice) {
    NSString *usageDescription = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"NSFaceIDUsageDescription"];
    if (!usageDescription) {
      id<ABI30_0_0EXConstantsInterface> constants = [self.bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI30_0_0EXConstantsInterface)];
      ...
      reject(@"E_FACEID_NOT_CONFIGURED", errorMessage, ABI30_0_0RCTErrorWithMessage(errorMessage));
      return;
    }
  }
  LAContext *context = [LAContext new];
