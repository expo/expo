...
@implementation FBSDKInternalUtility (ABI30_0_0EXFacebook)
...
@implementation ABI30_0_0EXFacebook

@synthesize bridge = _bridge;

ABI30_0_0RCT_EXPORT_MODULE(ExponentFacebook)

ABI30_0_0RCT_REMAP_METHOD(logInWithReadPermissionsAsync,
                 ...
                 rejecter:(ABI30_0_0RCTPromiseRejectBlock)reject)
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (loginMgr.loginBehavior != FBSDKLoginBehaviorWeb) {
      id<ABI30_0_0EXConstantsInterface> constants = [self->_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI30_0_0EXConstantsInterface)];
      ...
      if ([constants.appOwnership isEqualToString:@"expo"]) {
        ...
        reject(@"E_BEHAVIOR_NOT_PERMITTED", message, ABI30_0_0RCTErrorWithMessage(message));
        return;
      } else {
        if (![[self class] facebookAppIdFromNSBundle]) {
          ...
          reject(@"E_BEHAVIOR_NOT_SUPPORTED", message, ABI30_0_0RCTErrorWithMessage(message));
          return;
        }
      }
    }
