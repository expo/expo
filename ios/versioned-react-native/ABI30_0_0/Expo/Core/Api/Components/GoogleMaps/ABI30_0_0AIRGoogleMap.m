...
@interface ABI30_0_0AIRGoogleMap ()

- (id)eventFromCoordinate:(CLLocationCoordinate2D)coordinate;

@end

@implementation ABI30_0_0AIRGoogleMap
{
...
- (void)insertReactABI30_0_0Subview:(id<ABI30_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  ...
  if ([subview isKindOfClass:[ABI30_0_0AIRGoogleMapMarker class]]) {
    ABI30_0_0AIRGoogleMapMarker *marker = (ABI30_0_0AIRGoogleMapMarker*)subview;
    marker.realMarker.map = self;
    [self.markers addObject:marker];
  } else if ([subview isKindOfClass:[ABI30_0_0AIRGoogleMapPolygon class]]) {
    ABI30_0_0AIRGoogleMapPolygon *polygon = (ABI30_0_0AIRGoogleMapPolygon*)subview;
    polygon.polygon.map = self;
    [self.polygons addObject:polygon];
  } else if ([subview isKindOfClass:[ABI30_0_0AIRGoogleMapPolyline class]]) {
    ABI30_0_0AIRGoogleMapPolyline *polyline = (ABI30_0_0AIRGoogleMapPolyline*)subview;
    polyline.polyline.map = self;
    [self.polylines addObject:polyline];
  } else if ([subview isKindOfClass:[ABI30_0_0AIRGoogleMapCircle class]]) {
    ABI30_0_0AIRGoogleMapCircle *circle = (ABI30_0_0AIRGoogleMapCircle*)subview;
    circle.circle.map = self;
    [self.circles addObject:circle];
  } else if ([subview isKindOfClass:[ABI30_0_0AIRGoogleMapUrlTile class]]) {
    ABI30_0_0AIRGoogleMapUrlTile *tile = (ABI30_0_0AIRGoogleMapUrlTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI30_0_0AIRGoogleMapOverlay class]]) {
    ABI30_0_0AIRGoogleMapOverlay *overlay = (ABI30_0_0AIRGoogleMapOverlay*)subview;
...
- (void)didMoveToWindow {
  ...
  if (_initialRegion.span.latitudeDelta != 0.0 &&
    ...
    self.camera = [ABI30_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_initialRegion];
  } else if (_region.span.latitudeDelta != 0.0 &&
      _region.span.longitudeDelta != 0.0) {
    self.camera = [ABI30_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_region];
  }
...
- (void)setInitialRegion:(MKCoordinateRegion)initialRegion {
  ...
  self.camera = [ABI30_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:initialRegion];
}

- (void)setRegion:(MKCoordinateRegion)region {
  // TODO: The JS component is repeatedly setting region unnecessarily. We might want to deal with that in here.
  _region = region;
  self.camera = [ABI30_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self  andMKCoordinateRegion:region];
}
...
- (BOOL)didTapMarker:(GMSMarker *)marker {
  ABI30_0_0AIRGMSMarker *airMarker = (ABI30_0_0AIRGMSMarker *)marker;
...
- (void)didTapPolyline:(GMSOverlay *)polyline {
  ABI30_0_0AIRGMSPolyline *airPolyline = (ABI30_0_0AIRGMSPolyline *)polyline;
...
- (void)didTapPolygon:(GMSOverlay *)polygon {
    ABI30_0_0AIRGMSPolygon *airPolygon = (ABI30_0_0AIRGMSPolygon *)polygon;
...
- (void)didChangeCameraPosition:(GMSCameraPosition *)position {
  id event = @{@"continuous": @YES,
               @"region": regionAsJSON([ABI30_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andGMSCameraPosition:position]),
               };
...
- (void)setKmlSrc:(NSString *)kmlUrl {
  ...
  for (GMUPlacemark *place in parser.placemarks) {
    ...
    ABI30_0_0AIRGoogleMapMarker *marker = (ABI30_0_0AIRGoogleMapMarker *)[[ABI30_0_0AIRGoogleMapMarkerManager alloc] view];
    if (!marker.bridge) {
    ...
    marker.imageSrc = [ABI30_0_0AIRGoogleMap GetIconUrl:place parser:parser];
    marker.layer.backgroundColor = [UIColor clearColor].CGColor;
    marker.layer.position = CGPointZero;

    [self insertReactABI30_0_0Subview:(UIView *) marker atIndex:index];
