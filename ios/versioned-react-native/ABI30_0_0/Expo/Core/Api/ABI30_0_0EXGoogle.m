...
NSString *ABI30_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI30_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@property (nonatomic, weak) id kernelGoogleAuthServiceDelegate;

@end

@implementation ABI30_0_0EXGoogle
{
  ABI30_0_0RCTPromiseResolveBlock _logInResolve;
  ABI30_0_0RCTPromiseRejectBlock _logInReject;
}

ABI30_0_0EX_EXPORT_SCOPED_MODULE(ExponentGoogle, GoogleAuthManager);
                 ...
                 rejecter:(ABI30_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI30_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI30_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI30_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
      ...
      if (authState) {
        if (authState.isAuthorized) {
          strongSelf->_logInResolve(@{
            @"type": @"success",
            @"accessToken": ABI30_0_0RCTNullIfNil(authState.lastTokenResponse.accessToken),
            @"idToken": ABI30_0_0RCTNullIfNil(authState.lastTokenResponse.idToken),
            @"refreshToken": ABI30_0_0RCTNullIfNil(authState.lastTokenResponse.refreshToken),
            @"serverAuthCode": ABI30_0_0RCTNullIfNil(authState.lastAuthorizationResponse.authorizationCode),
          });
        } else {
          strongSelf->_logInResolve(@{@"type": @"cancel"});
        }
      } else {
        strongSelf->_logInReject(ABI30_0_0EXGoogleErrorCode, @"Google sign in error", error);
      }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI30_0_0RCTPresentedViewController()
                                                   callback:callback];
...
-(void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error
{
  if (error != nil) {
    if (error.code == kGIDSignInErrorCodeCanceled) {
      ...
      _logInReject(ABI30_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  } else {
    _logInResolve(@{
      @"type": @"success",
      @"accessToken": ABI30_0_0RCTNullIfNil(user.authentication.accessToken),
      @"serverAuthCode": ABI30_0_0RCTNullIfNil(user.serverAuthCode),
      @"idToken": ABI30_0_0RCTNullIfNil(user.authentication.idToken),
      @"refreshToken": ABI30_0_0RCTNullIfNil(user.authentication.refreshToken),
      @"user": @{
        @"id": ABI30_0_0RCTNullIfNil(user.userID),
        @"name": ABI30_0_0RCTNullIfNil(user.profile.name),
        @"familyName": ABI30_0_0RCTNullIfNil(user.profile.familyName),
        @"givenName": ABI30_0_0RCTNullIfNil(user.profile.givenName),
        @"email": ABI30_0_0RCTNullIfNil(user.profile.email),
...
{
  [ABI30_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI30_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
