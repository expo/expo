...
TEST(JSCExecutor, CallFunction) {
  ...
  ABI17_0_0EXPECT_EQ(11, returnedCall.moduleId);
  ABI17_0_0EXPECT_EQ(10, returnedCall.methodId);
  ASSERT_EQ(4, returnedCall.arguments.size());
  ABI17_0_0EXPECT_EQ(args[0], returnedCall.arguments[0]);
  ABI17_0_0EXPECT_EQ(args[1], returnedCall.arguments[1]);
  ABI17_0_0EXPECT_EQ(args[2], returnedCall.arguments[2]);
  ABI17_0_0EXPECT_EQ(folly::dynamic(4.0), returnedCall.arguments[3]);
}
...
TEST(JSCExecutor, CallFunctionWithMap) {
  ...
  ABI17_0_0EXPECT_EQ("hello4true", returnedCall.arguments[0].getString());
}
...
TEST(JSCExecutor, CallFunctionReturningMap) {
  ...
  ABI17_0_0EXPECT_EQ(folly::dynamic(4.0), foo);
  auto bar = returnedMap.at("bar");
  ABI17_0_0EXPECT_EQ(folly::dynamic(true), bar);
}
...
TEST(JSCExecutor, CallFunctionWithArray) {
  ...
  auto returnedCall = returnedCalls[0];
  ASSERT_EQ(1, returnedCall.arguments.size());
  ABI17_0_0EXPECT_EQ("hello4true3", returnedCall.arguments[0].getString());
}
...
TEST(JSCExecutor, CallFunctionReturningNumberArray) {
  ...
  ABI17_0_0EXPECT_EQ(3, array.size());
  ABI17_0_0EXPECT_EQ(folly::dynamic(3.0), array[0]);
  ABI17_0_0EXPECT_EQ(folly::dynamic(4.0), array[2]);
}
...
TEST(JSCExecutor, SetObjectGlobalVariable) {
  ...
  auto& returnedMap = returnedCall.arguments[0];
  auto foo = returnedMap.at("foo");
  ABI17_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI17_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI17_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
