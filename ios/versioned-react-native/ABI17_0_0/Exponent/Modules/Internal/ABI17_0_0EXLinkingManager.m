...
NSString * const ABI17_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI17_0_0EXLinkingManager ()
...
@implementation ABI17_0_0EXLinkingManager

+ (NSString *)moduleName { return @"ABI17_0_0RCTLinkingManager"; }
...
{
  return @[ABI17_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI17_0_0RCTFatal(ABI17_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI17_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}
                  ...
                  reject:(__unused ABI17_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (!canOpen) {
    canOpen = [ABI17_0_0RCTSharedApplication() canOpenURL:URL];
  }
  resolve(@(canOpen));
}
...
ABI17_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI17_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI17_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI17_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
...
- (BOOL)_isExponentUrl: (NSURL *)url
{
  ...
  NSString *versionsPath = [[NSBundle mainBundle] pathForResource:@"ABI17_0_0EXSDKVersions" ofType:@"plist"];
