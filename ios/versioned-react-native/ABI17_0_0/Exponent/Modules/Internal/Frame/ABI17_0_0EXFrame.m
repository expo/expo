...
#import "ABI17_0_0EXAppLoadingManager.h"
...
#define ABI17_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI17_0_0EXFrameReactABI17_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI17_0_0EXFrameReactABI17_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI17_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI17_0_0Bridge;

@end

@interface ABI17_0_0EXFrame () <ABI17_0_0RCTInvalidating, ABI17_0_0RCTExceptionsManagerDelegate>

@property (nonatomic, assign) BOOL sourceSet;
@property (nonatomic, assign) BOOL needsReload;

@property (nonatomic, copy) ABI17_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI17_0_0RCTDirectEventBlock onLoadingFinish;
@property (nonatomic, copy) ABI17_0_0RCTDirectEventBlock onLoadingError;
...
@implementation ABI17_0_0EXFrame

ABI17_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI17_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");
...
- (BOOL)validateProps:(NSArray<NSString *> *)changedProps
{
  ...
  if ([changedProps containsObject:@"source"]) {
    // Performed in additon to JS-side validation because ABI17_0_0RN iOS websockets will crash without a port.
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI17_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI17_0_0RootView]) {
    [_appManager ReactABI17_0_0RootView].frame = self.bounds;
  }
}
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI17_0_0RootView] &&
      [_appManager ReactABI17_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI17_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI17_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI17_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI17_0_0RCTErrorWithMessage(message)];
