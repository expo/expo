...
#import "ABI17_0_0EXBarCodeScanner.h"
#import "ABI17_0_0EXBarCodeScannerManager.h"
...
@implementation ABI17_0_0EXBarCodeScannerManager

ABI17_0_0RCT_EXPORT_MODULE(ExponentBarCodeScannerManager);
ABI17_0_0RCT_EXPORT_VIEW_PROPERTY(onBarCodeRead, ABI17_0_0RCTDirectEventBlock);
...
- (UIView *)view
{
  ...
  if (!self.camera) {
    self.camera = [[ABI17_0_0EXBarCodeScanner alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
- (NSDictionary *)constantsToExport
{
  return @{
    ...
    @"Type" :
        @{@"front" : @(ABI17_0_0EXBarCodeScannerTypeFront), @"back" : @(ABI17_0_0EXBarCodeScannerTypeBack)},
    @"TorchMode" : @{
      @"off" : @(ABI17_0_0EXBarCodeScannerTorchModeOff),
      @"on" : @(ABI17_0_0EXBarCodeScannerTorchModeOn),
      @"auto" : @(ABI17_0_0EXBarCodeScannerTorchModeAuto)
    }
  };
}
...
ABI17_0_0RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, ABI17_0_0EXBarCodeScanner)
{
  NSInteger type = [ABI17_0_0RCTConvert NSInteger:json];
    ...
    dispatch_async(self.sessionQueue, ^{
      ...
      if ([self.session canAddInput:captureDeviceInput]) {
        ...
        if (self.videoCaptureDeviceInput) {
          ...
          ABI17_0_0RCTLogWarn(@"%s: Can't add null video capture device, doing nothing instead.", __func__);
        }
      }
...
ABI17_0_0RCT_CUSTOM_VIEW_PROPERTY(torchMode, NSInteger, ABI17_0_0EXBarCodeScanner)
{
  dispatch_async(self.sessionQueue, ^{
    NSInteger torchMode = [ABI17_0_0RCTConvert NSInteger:json];
...
ABI17_0_0RCT_CUSTOM_VIEW_PROPERTY(barCodeTypes, NSArray, ABI17_0_0EXBarCodeScanner)
{
  NSArray *types = [ABI17_0_0RCTConvert NSArray:json];
  NSSet *validTypes = [NSSet setWithArray:[[self class] validBarCodeTypes].allValues];
  for (id type in types) {
    if (![validTypes containsObject:type]) {
      ABI17_0_0RCTLogWarn(@"Unsupported BarCodeType: %@", type);
      return;
    }
  }
...
- (void)startSession
{
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak ABI17_0_0EXBarCodeScannerManager *weakSelf = self;
              ...
              [NSNotificationCenter.defaultCenter
                  addObserverForName:AVCaptureSessionRuntimeErrorNotification
                          ...
                          usingBlock:^(NSNotification *note) {
                            ABI17_0_0EXBarCodeScannerManager *strongSelf = weakSelf;
