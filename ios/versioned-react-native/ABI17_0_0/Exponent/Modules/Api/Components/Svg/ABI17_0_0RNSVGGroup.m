...
@implementation ABI17_0_0RNSVGGroup
...
- (void)renderGroupTo:(CGContextRef)context
{
    ...
    [self traverseSubviews:^(ABI17_0_0RNSVGNode *node) {
        if (node.responsible && !svg.responsible) {
            svg.responsible = YES;
        }
        
        if ([node isKindOfClass:[ABI17_0_0RNSVGRenderable class]]) {
            [(ABI17_0_0RNSVGRenderable*)node mergeProperties:self];
        }
        
        [node renderTo:context];
        
        if ([node isKindOfClass:[ABI17_0_0RNSVGRenderable class]]) {
            [(ABI17_0_0RNSVGRenderable*)node resetProperties];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event withTransform:(CGAffineTransform)transform
{
    ...
    for (ABI17_0_0RNSVGNode *node in [self.subviews reverseObjectEnumerator]) {
        if (![node isKindOfClass:[ABI17_0_0RNSVGNode class]]) {
            continue;
        }
...
- (void)parseReference
{
    ...
    [self traverseSubviews:^(__kindof ABI17_0_0RNSVGNode *node) {
        [node parseReference];
        return YES;
    }];
}
