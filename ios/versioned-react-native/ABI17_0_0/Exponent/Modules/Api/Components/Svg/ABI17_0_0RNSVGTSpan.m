...
@implementation ABI17_0_0RNSVGTSpan
{
    ABI17_0_0RNSVGBezierTransformer *_bezierTransformer;
    CGPathRef _cache;
}
...
{
    __block ABI17_0_0RNSVGBezierTransformer *bezierTransformer;
    [self traverseTextSuperviews:^(__kindof ABI17_0_0RNSVGText *node) {
        if ([node class] == [ABI17_0_0RNSVGTextPath class]) {
            bezierTransformer = [(ABI17_0_0RNSVGTextPath*)node getBezierTransformer];
            return NO;
        }
        return YES;
    }];
...
- (void)traverseTextSuperviews:(BOOL (^)(__kindof ABI17_0_0RNSVGText *node))block
{
    ABI17_0_0RNSVGText *targetView = self;
    BOOL result = block(self);
    
    while (targetView && [targetView class] != [ABI17_0_0RNSVGText class] && result) {
        if (![targetView isKindOfClass:[ABI17_0_0RNSVGText class]]) {
            //todo: throw exception here
            break;
        }
        
        targetView = (ABI17_0_0RNSVGText*)[targetView superview];
