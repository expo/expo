...
#import <ReactABI17_0_0/ABI17_0_0RCTJSCExecutor.h>
...
@interface ABI17_0_0EXGLView ()

@property (nonatomic, weak) ABI17_0_0EXGLViewManager *viewManager;
...
@implementation ABI17_0_0EXGLView
...
- (instancetype)initWithManager:(ABI17_0_0EXGLViewManager *)viewManager
{
  if ((self = [super init])) {
    _viewManager = viewManager;

    self.contentScaleFactor = ABI17_0_0RCTScreenScale();
...
- (void)setOnSurfaceCreate:(ABI17_0_0RCTDirectEventBlock)onSurfaceCreate
{
  _onSurfaceCreate = onSurfaceCreate;
  if (_onSurfaceCreate) {
    // Got non-empty onSurfaceCreate callback -- set up JS binding -- only possible on JavaScriptCore
    id<ABI17_0_0RCTJavaScriptExecutor> executor = [_viewManager.bridge valueForKey:@"javaScriptExecutor"];
    if ([executor isKindOfClass:NSClassFromString(@"ABI17_0_0RCTJSCExecutor")]) {
      // On JS thread, extract JavaScriptCore context, create ABI17_0_0EXGL context, call JS callback
      __weak __typeof__(self) weakSelf = self;
      __weak __typeof__(executor) weakExecutor = executor;
      [executor executeBlockOnJavaScriptQueue:^{
        __typeof__(self) self = weakSelf;
        ABI17_0_0RCTJSCExecutor *executor = weakExecutor;
        if (self && executor) {
      ...
      ABI17_0_0RCTLog(@"ABI17_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI17_0_0-native/docs/debugging.html)? ABI17_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI17_0_0EXGL.");
    }
  }
}
...
- (void)draw
{
  ...
  // called on the JS thread to create the ABI17_0_0EXGL context and save its id (see init method). In
