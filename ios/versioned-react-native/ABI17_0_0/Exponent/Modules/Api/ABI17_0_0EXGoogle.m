...
NSString *ABI17_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI17_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@end

@implementation ABI17_0_0EXGoogle
{
  ABI17_0_0RCTPromiseResolveBlock _logInResolve;
  ABI17_0_0RCTPromiseRejectBlock _logInReject;
}

ABI17_0_0RCT_EXPORT_MODULE(ExponentGoogle)
                 ...
                 rejecter:(ABI17_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI17_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI17_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI17_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      ...
      _logInReject(ABI17_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI17_0_0RCTPresentedViewController()
                                                   callback:callback];
...
{
  [ABI17_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI17_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
