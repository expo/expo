...
static NSString *const ABI17_0_0EXVideoReadyForDisplayKeyPath = @"readyForDisplay";

@interface ABI17_0_0EXVideoView ()

@property (nonatomic, weak) ABI17_0_0EXAV *exAV;

@property (nonatomic, strong) ABI17_0_0EXAVPlayerData *data;
@property (nonatomic, strong) AVPlayerLayer *playerLayer;
@property (nonatomic, strong) ABI17_0_0EXVideoPlayerViewController *playerViewController;
...
@implementation ABI17_0_0EXVideoView

#pragma mark - ABI17_0_0EXVideoView interface methods

- (instancetype)initWithBridge:(ABI17_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _exAV = [bridge moduleForClass:[ABI17_0_0EXAV class]];
...
- (void)_callFullscreenCallbackForUpdate:(ABI17_0_0EXVideoFullscreenUpdate)update
{
...
- (void)_tryUpdateDataStatus:(ABI17_0_0RCTPromiseResolveBlock)resolve
...
- (void)_usePlayerLayer
{
  if (_data) {
    ...
    [self setNativeResizeMode:_nativeResizeMode];
    [_playerLayer addObserver:self forKeyPath:ABI17_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)_removePlayerLayer
{
  ...
  [_playerLayer removeObserver:self forKeyPath:ABI17_0_0EXVideoReadyForDisplayKeyPath];
  _playerLayer = nil;
}
...
{
  if (object == _playerLayer && [keyPath isEqualToString:ABI17_0_0EXVideoReadyForDisplayKeyPath]) {
      ...
      rejecter:(ABI17_0_0RCTPromiseRejectBlock)reject
{
  ...
  _data = [[ABI17_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                          ...
                          withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                            if (success) {
                              ...
                              [self _removePlayer];
                              if (reject) {
                                reject(@"E_VIDEO_NOTCREATED", nil, ABI17_0_0RCTErrorWithMessage(error));
                              }
             ...
             rejecter:(ABI17_0_0RCTPromiseRejectBlock)reject
{
  if (!_data) {
    ...
    if (reject) {
      reject(@"E_VIDEO_FULLSCREEN", nil, ABI17_0_0RCTErrorWithMessage(@"Fullscreen encountered an error: video is not loaded."));
    }
    return;
  }
  ...
  if (value && !_fullscreenPlayerPresented) {
    ...
    _presentingViewController = ABI17_0_0RCTPresentedViewController();
    [self _callFullscreenCallbackForUpdate:ABI17_0_0EXVideoFullscreenUpdatePlayerWillPresent];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [_presentingViewController presentViewController:_playerViewController animated:YES completion:^{
        ...
        [weakSelf _callFullscreenCallbackForUpdate:ABI17_0_0EXVideoFullscreenUpdatePlayerDidPresent];
        if (resolve) {
...
#pragma mark - ReactABI17_0_0 View Management

- (void)insertReactABI17_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  ...
  if (_useNativeControls) {
    [super insertReactABI17_0_0Subview:view atIndex:atIndex];
    view.frame = self.bounds;
    [_playerViewController.contentOverlayView insertSubview:view atIndex:atIndex];
  } else {
    ABI17_0_0RCTLogError(@"video cannot have any subviews");
  }
  return;
}
...
#pragma mark - ABI17_0_0EXVideoPlayerViewControllerDelegate

- (void)videoPlayerViewControllerWillDismiss:(AVPlayerViewController *)playerViewController
{
  if (_playerViewController == playerViewController && _fullscreenPlayerPresented) {
    [self _callFullscreenCallbackForUpdate:ABI17_0_0EXVideoFullscreenUpdatePlayerWillDismiss];
  }
}
...
- (void)videoPlayerViewControllerDidDismiss:(AVPlayerViewController *)playerViewController
{
  if (_playerViewController == playerViewController && _fullscreenPlayerPresented) {
    ...
    _playerViewController = nil;
    [self setUseNativeControls:_useNativeControls];
    [self _callFullscreenCallbackForUpdate:ABI17_0_0EXVideoFullscreenUpdatePlayerDidDismiss];
  }
}
