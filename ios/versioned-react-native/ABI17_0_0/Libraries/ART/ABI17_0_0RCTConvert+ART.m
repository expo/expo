...
#import "ABI17_0_0ARTLinearGradient.h"
#import "ABI17_0_0ARTPattern.h"
#import "ABI17_0_0ARTRadialGradient.h"
#import "ABI17_0_0ARTSolidColor.h"

@implementation ABI17_0_0RCTConvert (ABI17_0_0ART)
...
#define NEXT_VALUE [self double:arr[i++]]
    ...
    while (i < count) {
      ...
      switch (type) {
        ...
        case 4:
          ...
          break;
        default:
          ABI17_0_0RCTLogError(@"Invalid CGPath type %zd at element %zd of %@", type, i, arr);
          CGPathRelease(path);
          return NULL;
      }
    }
  ...
  @catch (NSException *exception) {
    ABI17_0_0RCTLogError(@"Invalid CGPath format: %@", arr);
    CGPathRelease(path);
    return NULL;
  }
...
ABI17_0_0RCT_ENUM_CONVERTER(CTTextAlignment, (@{
...
// This prepares everything for rendering a frame of text in ABI17_0_0ARTText.
+ (ABI17_0_0ARTTextFrame)ABI17_0_0ARTTextFrame:(id)json
{
  NSDictionary *dict = [self NSDictionary:json];
  ABI17_0_0ARTTextFrame frame;
  frame.count = 0;
  ...
  frame.lineHeight = size * 1.1; // Base on ABI17_0_0ART canvas line height estimate
...
+ (ABI17_0_0ARTCGFloatArray)ABI17_0_0ARTCGFloatArray:(id)json
{
  ...
  ABI17_0_0ARTCGFloatArray array;
  array.count = count;
  array.array = NULL;
...
+ (ABI17_0_0ARTBrush *)ABI17_0_0ARTBrush:(id)json
{
  ...
  switch (type) {
    case 0: // solid color
      ...
      return [[ABI17_0_0ARTSolidColor alloc] initWithArray:arr];
    case 1: // linear gradient
      return [[ABI17_0_0ARTLinearGradient alloc] initWithArray:arr];
    case 2: // radial gradient
      return [[ABI17_0_0ARTRadialGradient alloc] initWithArray:arr];
    case 3: // pattern
      return [[ABI17_0_0ARTPattern alloc] initWithArray:arr];
    default:
      ABI17_0_0RCTLogError(@"Unknown brush type: %zd", type);
      return nil;
  }
}
...
+ (CGPoint)CGPoint:(id)json offset:(NSUInteger)offset
{
  ...
  if (arr.count < offset + 2) {
    ABI17_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", 2 + offset, arr);
    return CGPointZero;
  }
  return (CGPoint){
...
+ (CGRect)CGRect:(id)json offset:(NSUInteger)offset
{
  ...
  if (arr.count < offset + 4) {
    ABI17_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", 4 + offset, arr);
    return CGRectZero;
  }
  return (CGRect){
...
+ (CGGradientRef)CGGradient:(id)json offset:(NSUInteger)offset
{
  NSArray *arr = [self NSArray:json];
  if (arr.count < offset) {
    ABI17_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", offset, arr);
    return NULL;
  }
  arr = [arr subarrayWithRange:(NSRange){offset, arr.count - offset}];
  ABI17_0_0ARTCGFloatArray colorsAndOffsets = [self ABI17_0_0ARTCGFloatArray:arr];
