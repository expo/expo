...
#import <cxxReactABI17_0_0/ABI17_0_0CxxModule.h>

#import "ABI17_0_0RCTCxxMethod.h"

using namespace facebook::ReactABI17_0_0;

@implementation ABI17_0_0RCTCxxModule
{
...
- (void)lazyInit
{
  if (!_module) {
    ...
    if (_module) {
      ABI17_0_0RCTAssert([ABI17_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(_module->getName().c_str())],
                @"CxxModule class name %@ does not match runtime name %s",
                ABI17_0_0RCTBridgeModuleNameForClass([self class]), _module->getName().c_str());
    }
  }
}
...
{
  ABI17_0_0RCTAssert(NO, @"Subclass %@ must override createModule", [self class]);
  return nullptr;
}

- (NSArray<id<ABI17_0_0RCTBridgeMethod>> *)methodsToExport;
{
  [self lazyInit];
  if (!_module) {
    return nil;
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI17_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
