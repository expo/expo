...
#import <cxxReactABI17_0_0/ABI17_0_0JsArgumentHelpers.h>
...
@implementation ABI17_0_0RCTCxxMethod
{
             ...
             arguments:(NSArray *)arguments
{
  ...
  if (arguments.count < _method->callbacks) {
    ABI17_0_0RCTLogError(@"Method %@.%s expects at least %lu arguments, but got %tu",
                ABI17_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
  ...
  if (_method->callbacks >= 1) {
    if (![arguments[arguments.count - 1] isKindOfClass:[NSNumber class]]) {
      ABI17_0_0RCTLogError(@"Argument %tu (%@) of %@.%s should be a function",
  ...
  folly::dynamic args = [ABI17_0_0RCTConvert folly_dynamic:arguments];
  ...
  try {
    ...
    ABI17_0_0RCTLogError(@"Method %@.%s argument error: %s",
                ABI17_0_0RCTBridgeModuleNameForClass([module class]), _method->name.c_str(),
                ex.what());
    return nil;
  }
...
- (ABI17_0_0RCTFunctionType)functionType
{
  ...
  return _method->syncFunc ? ABI17_0_0RCTFunctionTypeSync : ABI17_0_0RCTFunctionTypeNormal;
}
