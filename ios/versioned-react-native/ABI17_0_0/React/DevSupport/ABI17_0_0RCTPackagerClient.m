...
#import <ReactABI17_0_0/ABI17_0_0RCTReconnectingWebSocket.h>
...
@interface ABI17_0_0RCTPackagerClient () <ABI17_0_0RCTWebSocketProtocolDelegate>
@end

@implementation ABI17_0_0RCTPackagerClient {
  ABI17_0_0RCTReconnectingWebSocket *_socket;
  NSMutableDictionary<NSString *, id<ABI17_0_0RCTPackagerClientMethod>> *_handlers;
}
...
{
  NSArray<NSNumber *> *const kSupportedVersions = @[ @(ABI17_0_0RCT_PACKAGER_CLIENT_PROTOCOL_VERSION) ];
...
- (void)webSocket:(ABI17_0_0RCTSRWebSocket *)webSocket didReceiveMessage:(id)message
{
  ...
  if (error) {
    ABI17_0_0RCTLogError(@"%@ failed to parse message with error %@\n<message>\n%@\n</message>", [self class], error, msg);
    return;
  }

  if (![self isSupportedVersion:msg[@"version"]]) {
    ABI17_0_0RCTLogError(@"%@ received message with not supported version %@", [self class], msg[@"version"]);
    return;
  }

  id<ABI17_0_0RCTPackagerClientMethod> methodHandler = _handlers[msg[@"method"]];
  if (!methodHandler) {
    if (msg[@"id"]) {
      NSString *errorMsg = [NSString stringWithFormat:@"%@ no handler found for method %@", [self class], msg[@"method"]];
      ABI17_0_0RCTLogError(errorMsg, msg[@"method"]);
      [[[ABI17_0_0RCTPackagerClientResponder alloc] initWithId:msg[@"id"]
  ...
  if (msg[@"id"]) {
    [methodHandler handleRequest:msg[@"params"]
                   withResponder:[[ABI17_0_0RCTPackagerClientResponder alloc] initWithId:msg[@"id"]
                                                                         socket:webSocket]];
  } else {
