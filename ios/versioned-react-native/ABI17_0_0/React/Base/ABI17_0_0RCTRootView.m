...
#import "ABI17_0_0RCTRootViewDelegate.h"
...
NSString *const ABI17_0_0RCTContentDidAppearNotification = @"ABI17_0_0RCTContentDidAppearNotification";

@interface ABI17_0_0RCTUIManager (ABI17_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI17_0_0RCTRootView
{
  ABI17_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  NSDictionary *_launchOptions;
  ABI17_0_0RCTRootContentView *_contentView;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI17_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI17_0_0RCTRootView");
  ABI17_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI17_0_0RCTRootView");

  ABI17_0_0RCT_PROFILE_BEGIN_EVENT(ABI17_0_0RCTProfileTagAlways, @"-[ABI17_0_0RCTRootView init]", nil);
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _sizeFlexibility = ABI17_0_0RCTRootViewSizeFlexibilityNone;

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidReload)
                                                 name:ABI17_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI17_0_0RCTContentDidAppearNotification
                                               object:self];

#if TARGET_OS_TV
    self.tvRemoteHandler = [ABI17_0_0RCTTVRemoteHandler new];
...
{
  ABI17_0_0RCTBridge *bridge = [[ABI17_0_0RCTBridge alloc] initWithBundleURL:bundleURL
                                            moduleProvider:nil
...
{
  if (self.ReactABI17_0_0PreferredFocusedView) {
    return self.ReactABI17_0_0PreferredFocusedView;
  }
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  fitSize = CGSizeMake(
      _sizeFlexibility & ABI17_0_0RCTRootViewSizeFlexibilityWidth ? fitSize.width : currentSize.width,
      _sizeFlexibility & ABI17_0_0RCTRootViewSizeFlexibilityHeight ? fitSize.height : currentSize.height
    );
...
{
  return _ReactABI17_0_0ViewController ?: [super ReactABI17_0_0ViewController];
}
...
{
  ABI17_0_0RCTAssertMainQueue();
  if (!super.ReactABI17_0_0Tag) {
    /**
     * Every root view that is created must have a unique ReactABI17_0_0 tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ReactABI17_0_0 tag must be re-assigned every time a new UIManager is created.
     */
    self.ReactABI17_0_0Tag = [_bridge.uiManager allocateRootTag];
  }
  return super.ReactABI17_0_0Tag;
}

- (void)bridgeDidReload
{
  ABI17_0_0RCTAssertMainQueue();
  // Clear the ReactABI17_0_0Tag so it can be re-assigned
  self.ReactABI17_0_0Tag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI17_0_0RCTRootContentView is scoped to the right bridge
  ABI17_0_0RCTBridge *bridge = notification.userInfo[@"bridge"];
  [self bundleFinishedLoading:bridge];
}

- (void)bundleFinishedLoading:(ABI17_0_0RCTBridge *)bridge
{
  if (!bridge.valid) {
    return;
  }
  ...
  _contentView = [[ABI17_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
                                                  ReactABI17_0_0Tag:self.ReactABI17_0_0Tag
  ...
  if (_sizeFlexibility == ABI17_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI17_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag": _contentView.ReactABI17_0_0Tag,
    @"initialProps": _appProperties ?: @{},
  };

  ABI17_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
@implementation ABI17_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI17_0_0RCTLogWarn(@"Calling deprecated `[-ABI17_0_0RCTRootView intrinsicSize]`.");
  return self.intrinsicContentSize;
}

@end

@implementation ABI17_0_0RCTUIManager (ABI17_0_0RCTRootView)
