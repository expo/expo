...
static NSRegularExpression *ABI17_0_0RCTJSStackFrameRegex()
{
...
@implementation ABI17_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI17_0_0RCTNullIfNil(self.methodName),
     @"file": ABI17_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI17_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
+ (instancetype)stackFrameWithDictionary:(NSDictionary *)dict
{
  return [[self alloc] initWithMethodName:dict[@"methodName"]
                                     file:dict[@"file"]
                               lineNumber:[ABI17_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI17_0_0RCTNilIfNull(dict[@"column"]) integerValue]];
}
...
+ (NSArray<ABI17_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  ...
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI17_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI17_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  ...
  for (NSDictionary *dict in dicts) {
    ABI17_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
