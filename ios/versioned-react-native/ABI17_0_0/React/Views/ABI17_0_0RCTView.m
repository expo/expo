...
#import "ABI17_0_0RCTAutoInsetsProtocol.h"
...
@implementation UIView (ABI17_0_0RCTViewUnmounting)
...
- (void)ReactABI17_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI17_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI17_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI17_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ReactABI17_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI17_0_0RootView]) {
      break;
    }
...
static NSString *ABI17_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI17_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI17_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI17_0_0RCTBorderStyleSolid;
...
ABI17_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)

- (void)setReactABI17_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  _ReactABI17_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI17_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI17_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI17_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI17_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  switch (_pointerEvents) {
    ...
    case ABI17_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI17_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI17_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI17_0_0Tag: %@;", self.ReactABI17_0_0Tag];
...
{
  while (view) {
    UIViewController *controller = view.ReactABI17_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI17_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    ...
    [super ReactABI17_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI17_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI17_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.sortedReactABI17_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI17_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI17_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI17_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI17_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI17_0_0_findClipView];
  if (clipView) {
    [self ReactABI17_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI17_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI17_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI17_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI17_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI17_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI17_0_0RCTCornerRadii){
...
{
  return (ABI17_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI17_0_0RCTUpdateShadowPathForView(self);

  const ABI17_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI17_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI17_0_0RCTRunningInTestEnvironment() &&
  ABI17_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI17_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI17_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI17_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI17_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI17_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI17_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI17_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI17_0_0RCTUpdateShadowPathForView(ABI17_0_0RCTView *view)
{
  if (ABI17_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI17_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI17_0_0RCTCornerInsets cornerInsets = ABI17_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI17_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI17_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI17_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI17_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI17_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI17_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI17_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI17_0_0RCTBorderStyle)style \
  {                                                    \
