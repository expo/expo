...
@interface ABI17_0_0RCTScrollEvent : NSObject <ABI17_0_0RCTEvent>
...
@implementation ABI17_0_0RCTScrollEvent
{
...
{
  ABI17_0_0RCTAssertParam(ReactABI17_0_0Tag);
...
- (ABI17_0_0RCTScrollEvent *)coalesceWithEvent:(ABI17_0_0RCTScrollEvent *)newEvent
{
...
{
  return @"ABI17_0_0RCTEventEmitter.receiveEvent";
}

- (NSArray *)arguments
{
  return @[self.viewTag, ABI17_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI17_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) ABI17_0_0RCTRefreshControl *rctRefreshControl;
#endif

@end


@implementation ABI17_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI17_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI17_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI17_0_0RCTUIManager JSResponder] isKindOfClass:[ABI17_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI17_0_0 scroll
...
- (void)setBounds:(CGRect)bounds
{
  if (isRectInvalid(bounds)) {
    ABI17_0_0RCTLogError(@"Attempted to set an invalid bounds to inner scrollview: %@", NSStringFromCGRect(bounds));
    return;
  }
...
- (void)setFrame:(CGRect)frame
{
  if (isRectInvalid(frame)) {
    ABI17_0_0RCTLogError(@"Attempted to set an invalid frame to inner scrollview: %@", NSStringFromCGRect(frame));
    return;
  }
...
#if !TARGET_OS_TV
- (void)setRctRefreshControl:(ABI17_0_0RCTRefreshControl *)refreshControl
{
...
@implementation ABI17_0_0RCTScrollView
{
  ABI17_0_0RCTEventDispatcher *_eventDispatcher;
  ABI17_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
{
  ABI17_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;
    _scrollView = [[ABI17_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI17_0_0RCTApplyTranformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI17_0_0LayoutDirection:layoutDirection];

  ABI17_0_0RCTApplyTranformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI17_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view isKindOfClass:[ABI17_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:(ABI17_0_0RCTRefreshControl *)view];
  } else
#endif
  {
    ABI17_0_0RCTAssert(_contentView == nil, @"ABI17_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI17_0_0RCTApplyTranformationAccordingLayoutDirection(_contentView, self.ReactABI17_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview isKindOfClass:[ABI17_0_0RCTRefreshControl class]]) {
    [_scrollView setRctRefreshControl:nil];
  } else
#endif
  {
    ABI17_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
{
  // Do nothing, as subviews are managed by `insertReactABI17_0_0Subview:atIndex:`
}
...
- (void)layoutSubviews
{
  ...
  ABI17_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI17_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");
  ...
  ABI17_0_0RCTRefreshControl *refreshControl = _scrollView.rctRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI17_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI17_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI17_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI17_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI17_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI17_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI17_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    // Dispatch event
    ABI17_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": childFrames}));

    // Update dispatch time
    _lastScrollDispatchTime = now;
    _allowNextScrollNoMatterWhat = NO;
  }
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI17_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI17_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI17_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI17_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI17_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI17_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI17_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil); //TODO: shouldn't this be onScrollAnimationEnd?
  ABI17_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI17_0_0RCTView contentInsetsForView:self];
...
#define ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI17_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI17_0_0RCTScrollEvent *scrollEvent = [[ABI17_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI17_0_0RCTEventDispatcher (ABI17_0_0RCTScrollView)

- (void)sendFakeScrollEvent:(NSNumber *)ReactABI17_0_0Tag
{
  ...
  ABI17_0_0RCTScrollEvent *fakeScrollEvent = [[ABI17_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI17_0_0Tag:ReactABI17_0_0Tag
                                                                   scrollView:nil
                                                                     userData:nil
                                                                coalescingKey:0];
