...
#import <ABI32_0_0EXReactNativeAdapter/ABI32_0_0EXReactNativeAdapter.h>
#import <ReactABI32_0_0/ABI32_0_0RCTUIManager.h>
#import <ReactABI32_0_0/ABI32_0_0RCTAppState.h>
#import <ReactABI32_0_0/ABI32_0_0RCTImageLoader.h>

@interface ABI32_0_0EXReactNativeAdapter ()

@property (nonatomic, weak) ABI32_0_0RCTBridge *bridge;
@property (nonatomic, weak) ABI32_0_0EXNativeModulesProxy *modulesProxy;
@property (nonatomic, assign) BOOL isForegrounded;
@property (nonatomic, strong) NSMutableSet<id<ABI32_0_0EXAppLifecycleListener>> *lifecycleListeners;
...
@implementation ABI32_0_0EXReactNativeAdapter
...
{
  return @[@protocol(ABI32_0_0EXAppLifecycleService), @protocol(ABI32_0_0EXUIManager), @protocol(ABI32_0_0EXJavaScriptContextProvider), @protocol(ABI32_0_0EXImageLoaderInterface)];
}
...
{
  [self.bridge dispatchBlock:block queue:ABI32_0_0RCTJSThread];
}
...
# pragma mark - ABI32_0_0EXJavaScriptContextProvider
...
# pragma mark - ABI32_0_0EXImageLoader
...
- (void)handleAppStateDidChange:(NSNotification *)notification
{
  if (
      _isForegrounded && (
       ...
       ABI32_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI32_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
...
{
  if (_isForegrounded) {
    [_lifecycleListeners enumerateObjectsUsingBlock:^(id<ABI32_0_0EXAppLifecycleListener>  _Nonnull obj, BOOL * _Nonnull stop) {
...
{
  __weak ABI32_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI32_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI32_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
extern void ABI32_0_0EXLogError(NSString *format, ...) {
  ...
  NSString *message = [[NSString alloc] initWithFormat:format arguments:args];
  va_end(args);
  ABI32_0_0RCTLogError(@"%@", message);
}

extern void ABI32_0_0EXFatal(NSError *error) {
  ABI32_0_0RCTFatal(error);
}

extern NSError * ABI32_0_0EXErrorWithMessage(NSString *message) {
  return ABI32_0_0RCTErrorWithMessage(message);
}

extern UIApplication *ABI32_0_0EXSharedApplication() {
  return ABI32_0_0RCTSharedApplication();
}
