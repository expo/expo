...
@interface ABI32_0_0EXGoogleSignIn ()
...
@property (nonatomic, strong) ABI32_0_0EXAuthTask *authTask;

@end

@implementation ABI32_0_0EXGoogleSignIn

ABI32_0_0EX_EXPORT_MODULE(ExpoGoogleSignIn);

- (instancetype)init
{
  if (self = [super init]) {
    _authTask = [[ABI32_0_0EXAuthTask alloc] init];
...
- (NSDictionary *)constantsToExport
{
  return @{
           @"ERRORS": @{
               ...
               @"TASK_IN_PROGRESS": ABI32_0_0EX_E_CONCURRENT_TASK_IN_PROGRESS,
               @"SIGN_IN_EXCEPTION": ABI32_0_0EX_E_EXCEPTION
               },
           @"TYPES": @{},
           @"SCOPES": @{
               @"PROFILE": @"profile",
               ...
               @"FITNESS_OXABI32_0_0YGEN_SATURATION_READ": @"https://www.googleapis.com/auth/fitness.oxygen_saturation.read",
               @"FITNESS_OXABI32_0_0YGEN_SATURATION_READ_WRITE": @"https://www.googleapis.com/auth/fitness.oxygen_saturation.write",
...
- (NSString *)_getNativeClientIdOrReject:(ABI32_0_0EXPromiseRejectBlock)reject
{
  NSString *path = [[NSBundle mainBundle] pathForResource:@"GoogleService-Info" ofType:@"plist"];
  
  if (!path) {
    reject(ABI32_0_0EX_E_EXCEPTION, @"Missing GoogleService-Info.plist", nil);
    return nil;
  }
  NSDictionary *plist = [[NSDictionary alloc] initWithContentsOfFile:path];
  NSString *clientId = plist[@"CLIENT_ID"];
  if (clientId != nil && ![clientId isEqualToString:@""]) return clientId;
  reject(ABI32_0_0EX_E_EXCEPTION, @"GoogleService-Info.plist `CLIENT_ID` is invalid", nil);
  return nil;
}
...
ABI32_0_0EX_EXPORT_METHOD_AS(initAsync,
...
ABI32_0_0EX_EXPORT_METHOD_AS(signInSilentlyAsync,
                    signInSilentlyAsync:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
...
ABI32_0_0EX_EXPORT_METHOD_AS(signInAsync,
                    signInAsync:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
...
ABI32_0_0EX_EXPORT_METHOD_AS(signOutAsync,
                    signOutAsync:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
...
ABI32_0_0EX_EXPORT_METHOD_AS(disconnectAsync,
                    disconnectAsync:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
...
ABI32_0_0EX_EXPORT_METHOD_AS(isConnectedAsync,
                    isConnectedAsync:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  resolve(@([[GIDSignIn sharedInstance] hasAuthInKeychain]));
}

ABI32_0_0EX_EXPORT_METHOD_AS(getCurrentUserAsync,
                    getCurrentUserAsync:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  GIDGoogleUser *currentUser = [GIDSignIn sharedInstance].currentUser;
  resolve(ABI32_0_0EXNullIfNil([ABI32_0_0EXGoogleSignIn jsonFromGIDGoogleUser:currentUser]));
}

ABI32_0_0EX_EXPORT_METHOD_AS(getPhotoAsync,
...
ABI32_0_0EX_EXPORT_METHOD_AS(getTokensAsync,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  GIDAuthenticationHandler handler = ^void(GIDAuthentication *authentication, NSError *error) {
    if (error) {
      reject(ABI32_0_0EX_E_EXCEPTION, error.localizedDescription, nil);
    } else {
      resolve(@{
...
- (void)signIn:(GIDSignIn *)signIn didSignInForUser:(GIDGoogleUser *)user withError:(NSError *)error {
  [self.authTask parse:[ABI32_0_0EXGoogleSignIn jsonFromGIDGoogleUser:user] error:error];
}
...
- (void)signIn:(GIDSignIn *)signIn presentViewController:(UIViewController *)viewController {
  __weak ABI32_0_0EXGoogleSignIn *weakSelf = self;
  
  [ABI32_0_0EXUtilities performSynchronouslyOnMainThread:^{
    __strong ABI32_0_0EXGoogleSignIn *strongSelf = weakSelf;
    if (strongSelf == nil) {
      return;
    }
