...
@interface ABI32_0_0EXGeofencingTaskConsumer ()
...
@implementation ABI32_0_0EXGeofencingTaskConsumer
...
- (void)startMonitoringRegionsForTask:(id<ABI32_0_0EXTaskInterface>)task
{
  [ABI32_0_0EXUtilities performSynchronouslyOnMainThread:^{
...
- (void)executeTaskWithRegion:(nonnull CLRegion *)region eventType:(ABI32_0_0EXGeofencingEventType)eventType
{
...
- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region
{
  if ([self regionStateForIdentifier:region.identifier] != CLRegionStateInside) {
    ...
    [self executeTaskWithRegion:region eventType:ABI32_0_0EXGeofencingEventTypeEnter];
  }
}
...
- (void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region
{
  if ([self regionStateForIdentifier:region.identifier] != CLRegionStateOutside) {
    ...
    [self executeTaskWithRegion:region eventType:ABI32_0_0EXGeofencingEventTypeExit];
  }
}
...
- (void)locationManager:(CLLocationManager *)manager didDetermineState:(CLRegionState)state forRegion:(CLRegion *)region
{
  if ([self regionStateForIdentifier:region.identifier] != state) {
    ABI32_0_0EXGeofencingEventType eventType = state == CLRegionStateInside ? ABI32_0_0EXGeofencingEventTypeEnter : ABI32_0_0EXGeofencingEventTypeExit;
...
+ (ABI32_0_0EXGeofencingRegionState)exportRegionState:(CLRegionState)regionState
{
  switch (regionState) {
    case CLRegionStateUnknown:
      return ABI32_0_0EXGeofencingRegionStateUnknown;
    case CLRegionStateInside:
      return ABI32_0_0EXGeofencingRegionStateInside;
    case CLRegionStateOutside:
      return ABI32_0_0EXGeofencingRegionStateOutside;
  }
}
