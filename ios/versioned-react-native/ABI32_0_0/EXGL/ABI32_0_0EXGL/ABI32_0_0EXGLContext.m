...
@interface ABI32_0_0EXGLContext ()

@property (nonatomic, strong) dispatch_queue_t glQueue;
@property (nonatomic, weak) ABI32_0_0EXModuleRegistry *moduleRegistry;
@property (nonatomic, weak) ABI32_0_0EXGLObjectManager *objectManager;

@end

@implementation ABI32_0_0EXGLContext
...
- (instancetype)initWithDelegate:(id<ABI32_0_0EXGLContextDelegate>)delegate andModuleRegistry:(nonnull ABI32_0_0EXModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    ...
    _objectManager = (ABI32_0_0EXGLObjectManager *)[_moduleRegistry getExportedModuleOfClass:[ABI32_0_0EXGLObjectManager class]];
...
{
  id<ABI32_0_0EXUIManager> uiManager = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXUIManager)];
  id<ABI32_0_0EXJavaScriptContextProvider> jsContextProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXJavaScriptContextProvider)];
  ...
  if (jsContextRef) {
    ...
    [uiManager dispatchOnClientThread:^{
      ABI32_0_0EXGLContext *self = weakSelf;
      id<ABI32_0_0EXUIManager> uiManager = weakUIManager;
    ...
    BLOCK_SAFE_RUN(callback, NO);
    ABI32_0_0EXLogWarn(@"ABI32_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://facebook.github.io/ReactABI32_0_0-native/docs/debugging.html)? ABI32_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI32_0_0EXGL.");
  }
}
  ...
  [self runAsync:^{
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI32_0_0EXErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI32_0_0EXErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = [ABI32_0_0EXUtilities screenScale];
