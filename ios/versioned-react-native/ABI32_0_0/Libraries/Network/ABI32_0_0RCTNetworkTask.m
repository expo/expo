...
@implementation ABI32_0_0RCTNetworkTask
{
  NSMutableData *_data;
  id<ABI32_0_0RCTURLRequestHandler> _handler;
  dispatch_queue_t _callbackQueue;

  ABI32_0_0RCTNetworkTask *_selfReference;
}
                  ...
                  callbackQueue:(dispatch_queue_t)callbackQueue
{
  ...
  ABI32_0_0RCTAssertParam(handler);
  ABI32_0_0RCTAssertParam(callbackQueue);
  ...
  if ((self = [super init])) {
    ...
    _status = ABI32_0_0RCTNetworkTaskPending;
...
- (void)start
{
  if (_status != ABI32_0_0RCTNetworkTaskPending) {
    ABI32_0_0RCTLogError(@"ABI32_0_0RCTNetworkTask was already started or completed");
    return;
  }
  ...
  if (_requestToken == nil) {
    ...
    if ([self validateRequestToken:token]) {
      _selfReference = self;
      _status = ABI32_0_0RCTNetworkTaskInProgress;
    }
  }
}
...
- (void)cancel
{
  if (_status == ABI32_0_0RCTNetworkTaskFinished) {
    return;
  }

  _status = ABI32_0_0RCTNetworkTaskFinished;
...
- (BOOL)validateRequestToken:(id)requestToken
{
  ...
  if (_requestToken == nil) {
    if (requestToken == nil) {
      if (ABI32_0_0RCT_DEBUG) {
        ABI32_0_0RCTLogError(@"Missing request token for request: %@", _request);
      }
      valid = NO;
    }
    ...
    if (ABI32_0_0RCT_DEBUG) {
      ABI32_0_0RCTLogError(@"Unrecognized request token: %@ expected: %@", requestToken, _requestToken);
    }
    valid = NO;
  }
  ...
  if (!valid) {
    _status = ABI32_0_0RCTNetworkTaskFinished;
    if (_completionBlock) {
      ABI32_0_0RCTURLRequestCompletionBlock completionBlock = _completionBlock;
      [self dispatchCallback:^{
        completionBlock(self->_response, nil, ABI32_0_0RCTErrorWithMessage(@"Invalid request token."));
      }];
    }
    [self invalidate];
  }
...
- (void)URLRequest:(id)requestToken didSendDataWithProgress:(int64_t)bytesSent
{
  ...
  if (_uploadProgressBlock) {
    ABI32_0_0RCTURLRequestProgressBlock uploadProgressBlock = _uploadProgressBlock;
...
- (void)URLRequest:(id)requestToken didReceiveResponse:(NSURLResponse *)response
{
  ...
  _response = response;
  if (_responseBlock) {
    ABI32_0_0RCTURLRequestResponseBlock responseBlock = _responseBlock;
    [self dispatchCallback:^{
...
- (void)URLRequest:(id)requestToken didReceiveData:(NSData *)data
{
  ...
  if (_incrementalDataBlock) {
    ABI32_0_0RCTURLRequestIncrementalDataBlock incrementalDataBlock = _incrementalDataBlock;
    [self dispatchCallback:^{
      incrementalDataBlock(data, length, total);
    }];
  }
  if (_downloadProgressBlock && total > 0) {
    ABI32_0_0RCTURLRequestProgressBlock downloadProgressBlock = _downloadProgressBlock;
    [self dispatchCallback:^{
