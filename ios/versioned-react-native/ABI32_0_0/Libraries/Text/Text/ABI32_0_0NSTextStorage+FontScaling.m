...
@implementation NSTextStorage (ABI32_0_0_RCT_FontScaling)
               ...
               maximumFontSize:(CGFloat)maximumFontSize
{
  ...
  while (true) {
    [self abi32_0_0_rct_scaleFontSizeWithRatio:ratio
                 minimumFontSize:minimumFontSize
                 maximumFontSize:maximumFontSize];

    ABI32_0_0RCTTextSizeComparisonOptions comparsion =
      [self abi32_0_0_rct_compareToSize:size thresholdRatio:0.01];
    ...
    if (
      ...
      [self abi32_0_0_rct_scaleFontSizeWithRatio:lastRatioWhichFits
                   minimumFontSize:minimumFontSize
                   maximumFontSize:maximumFontSize];
      return;
    }
...
- (ABI32_0_0RCTTextSizeComparisonOptions)abi32_0_0_rct_compareToSize:(CGSize)size thresholdRatio:(CGFloat)thresholdRatio
{
  ...
  [self enumerateAttribute:NSFontAttributeName
    ...
    ^(UIFont *_Nullable font, NSRange range, BOOL *_Nonnull stop) {
      ...
      if (scaledFont) {
        ...
        ABI32_0_0RCTLogError(@"Font \"%@"" doesn't support automatic scaling.", font.familyName);
      }
    }
  ];
