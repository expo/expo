...
@implementation ABI32_0_0RCTTextShadowView
{
...
- (instancetype)initWithBridge:(ABI32_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI32_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI32_0_0RCTTextShadowViewMeasure);
    ABI32_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI32_0_0RCTTextShadowViewBaseline);
  }
...
- (void)uiManagerWillPerformMounting
{
  ...
  [textStorage enumerateAttribute:ABI32_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:
   ^(ABI32_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
     if (!shadowView) {
       return;
     }

     [descendantViewTags addObject:shadowView.ReactABI32_0_0Tag];
   }
  ];

  [_bridge.uiManager addUIBlock:^(ABI32_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI32_0_0RCTTextView *textView = (ABI32_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI32_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName range:NSMakeRange(0, textStorage.length)];
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI32_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                               inRange:NSMakeRange(0, attributedText.length)
                               options:0
                            usingBlock:
    ^(ABI32_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                                        ...
                                        exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage abi32_0_0_rct_scaleFontSizeToFitSize:size
...
- (void)layoutSubviewsWithContext:(ABI32_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage enumerateAttribute:ABI32_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          ...
                          options:0
                       usingBlock:
    ^(ABI32_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
      if (!shadowView) {
        return;
      }
      ...
      CGRect frame = {{
        ABI32_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI32_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)
      }, {
        ABI32_0_0RCTRoundPixelValue(attachmentSize.width),
        ABI32_0_0RCTRoundPixelValue(attachmentSize.height)
      }};

      ABI32_0_0RCTLayoutContext localLayoutContext = layoutContext;
      ...
      ABI32_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
...
static ABI32_0_0YGSize ABI32_0_0RCTTextShadowViewMeasure(ABI32_0_0YGNodeRef node, float width, ABI32_0_0YGMeasureMode widthMode, float height, ABI32_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
    widthMode == ABI32_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI32_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
    heightMode == ABI32_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI32_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI32_0_0RCTTextShadowView *shadowTextView = (__bridge ABI32_0_0RCTTextShadowView *)ABI32_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
    MIN(ABI32_0_0RCTCeilPixelValue(size.width), maximumSize.width),
    MIN(ABI32_0_0RCTCeilPixelValue(size.height), maximumSize.height)
  };
  ...
  return (ABI32_0_0YGSize){
    ABI32_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
    ABI32_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)
  };
}

static float ABI32_0_0RCTTextShadowViewBaseline(ABI32_0_0YGNodeRef node, const float width, const float height)
{
  ABI32_0_0RCTTextShadowView *shadowTextView = (__bridge ABI32_0_0RCTTextShadowView *)ABI32_0_0YGNodeGetContext(node);
