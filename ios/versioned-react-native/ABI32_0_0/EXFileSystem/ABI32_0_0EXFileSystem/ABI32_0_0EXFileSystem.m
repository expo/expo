...
NSString * const ABI32_0_0EXDownloadProgressEventName = @"Exponent.downloadProgress";

@interface ABI32_0_0EXDownloadResumable : NSObject

@property (nonatomic, strong) NSString *uuid;
@property (nonatomic, strong) NSURLSession *session;
@property (nonatomic, strong) ABI32_0_0EXDownloadDelegate *delegate;

@end

@implementation ABI32_0_0EXDownloadResumable

- (instancetype)initWithId:(NSString *)uuid
               withSession:(NSURLSession *)session
              withDelegate:(ABI32_0_0EXDownloadDelegate *)delegate;
{
  if ((self = [super init])) {
    _uuid = uuid;
    _session = session;
...
@interface ABI32_0_0EXFileSystem ()

@property (nonatomic, strong) NSMutableDictionary<NSString *, ABI32_0_0EXDownloadResumable*> *downloadObjects;
...
@property (nonatomic, weak) id<ABI32_0_0EXFileSystemManager> fileSystemManager;

@end

@implementation NSData (ABI32_0_0EXFileSystem)
...
@implementation ABI32_0_0EXFileSystem
...
{
  return @[@protocol(ABI32_0_0EXFileSystemInterface)];
}
...
- (instancetype)initWithExperienceId:(NSString *)experienceId
{
  if (self = [super init]) {
    ...
    [ABI32_0_0EXFileSystem ensureDirExistsWithPath:_documentDirectory];
    [ABI32_0_0EXFileSystem ensureDirExistsWithPath:_cachesDirectory];
  }
  return self;
}
...
- (void)setModuleRegistry:(ABI32_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  _eventEmitter = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXEventEmitterService)];
  _fileSystemManager = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXFileSystemManager)];
}
...
{
  return @[ABI32_0_0EXDownloadProgressEventName];
}
...
ABI32_0_0EX_EXPORT_METHOD_AS(getInfoAsync,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:uri] & ABI32_0_0EXFileSystemPermissionRead)) {
  ...
  if ([uri.scheme isEqualToString:@"file"]) {
    [ABI32_0_0EXFileSystemLocalFileHandler getInfoForFile:uri withOptions:options resolver:resolve rejecter:reject];
  } else if ([uri.scheme isEqualToString:@"assets-library"]) {
    [ABI32_0_0EXFileSystemAssetLibraryHandler getInfoForFile:uri withOptions:options resolver:resolve rejecter:reject];
  } else {
...
ABI32_0_0EX_EXPORT_METHOD_AS(readAsStringAsync,
...
ABI32_0_0EX_EXPORT_METHOD_AS(writeAsStringAsync,
...
{
  NSURL *uri = [NSURL URLWithString:uriString];
  if (!([self permissionsForURI:uri] & ABI32_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI32_0_0EX_EXPORT_METHOD_AS(deleteAsync,
...
{
  NSURL *uri = [NSURL URLWithString:uriString];
  if (!([self permissionsForURI:[uri URLByAppendingPathComponent:@".."]] & ABI32_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI32_0_0EX_EXPORT_METHOD_AS(moveAsync,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:[from URLByAppendingPathComponent:@".."]] & ABI32_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  if (!([self permissionsForURI:to] & ABI32_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI32_0_0EX_EXPORT_METHOD_AS(copyAsync,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:from] & ABI32_0_0EXFileSystemPermissionRead)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
  ...
  if ([from.scheme isEqualToString:@"file"]) {
    [ABI32_0_0EXFileSystemLocalFileHandler copyFrom:from to:to resolver:resolve rejecter:reject];
  } else if ([from.scheme isEqualToString:@"assets-library"]) {
    [ABI32_0_0EXFileSystemAssetLibraryHandler copyFrom:from to:to resolver:resolve rejecter:reject];
  } else {
    reject(@"E_FILESYSTEM_INVALID_URI",
           [NSString stringWithFormat:@"Unsupported URI scheme for '%@'", from],
           nil);
...
ABI32_0_0EX_EXPORT_METHOD_AS(makeDirectoryAsync,
...
ABI32_0_0EX_EXPORT_METHOD_AS(readDirectoryAsync,
...
ABI32_0_0EX_EXPORT_METHOD_AS(downloadAsync,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  if (!([self permissionsForURI:localUri] & ABI32_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI32_0_0EX_EXPORT_METHOD_AS(downloadResumableStartAsync,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  if (!([self _permissionsForPath:path] & ABI32_0_0EXFileSystemPermissionWrite)) {
    reject(@"E_FILESYSTEM_PERMISSIONS",
...
ABI32_0_0EX_EXPORT_METHOD_AS(downloadResumablePauseAsync,
                    downloadResumablePauseAsyncWithUUID:(NSString *)uuid
                    resolver:(ABI32_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ABI32_0_0EXDownloadResumable *downloadResumable = (ABI32_0_0EXDownloadResumable *)self.downloadObjects[uuid];
    ...
    [downloadResumable.session getTasksWithCompletionHandler:^(NSArray<NSURLSessionDataTask *> * _Nonnull dataTasks, NSArray<NSURLSessionUploadTask *> * _Nonnull uploadTasks, NSArray<NSURLSessionDownloadTask *> * _Nonnull downloadTasks) {
      ...
      if (downloadTask) {
        [downloadTask cancelByProducingResumeData:^(NSData * _Nullable resumeData) {
          resolve(@{ @"resumeData": ABI32_0_0EXNullIfNil([resumeData base64EncodedStringWithOptions:0]) });
        }];
      } else {
        reject(@"E_UNABLE_TO_PAUSE",
...
- (void)_downloadResumableCreateSessionWithUrl:(NSURL *)url withScopedPath:(NSString *)scopedPath withUUID:(NSString *)uuid withOptions:(NSDictionary *)options withResumeData:(NSData * _Nullable)resumeData withResolver:(ABI32_0_0EXPromiseResolveBlock)resolve withRejecter:(ABI32_0_0EXPromiseRejectBlock)reject
{
  NSURLSessionConfiguration *sessionConfiguration = [NSURLSessionConfiguration defaultSessionConfiguration];
  sessionConfiguration.requestCachePolicy = NSURLRequestReloadIgnoringLocalCacheData;
  sessionConfiguration.URLCache = nil;
  ...
  ABI32_0_0EXDownloadDelegateOnWriteCallback onWrite = ^(NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
    __strong ABI32_0_0EXFileSystem *strongSelf = weakSelf;
    if (strongSelf && bytesWritten > 0) {
      [strongSelf sendEventWithName:ABI32_0_0EXDownloadProgressEventName
                               body:@{@"uuid":uuid,
                                      @"data":@{
  ...
  ABI32_0_0EXDownloadDelegateOnDownloadCallback onDownload = ^(NSURLSessionDownloadTask *task, NSURL *location) {
  ...
  ABI32_0_0EXDownloadDelegateOnErrorCallback onError = ^(NSError *error) {
  ...
  ABI32_0_0EXDownloadDelegate *downloadDelegate = [[ABI32_0_0EXDownloadDelegate alloc] initWithId:uuid
                                                                        onWrite:onWrite
  ...
  ABI32_0_0EXDownloadResumable *downloadResumable = [[ABI32_0_0EXDownloadResumable alloc] initWithId:uuid
                                                                       withSession:session
...
- (ABI32_0_0EXFileSystemPermissionFlags)_permissionsForPath:(NSString *)path
{
  return [[_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXFilePermissionModuleInterface)]
...
{
  return [ABI32_0_0EXFileSystem ensureDirExistsWithPath:path];
}
...
{
  return [ABI32_0_0EXFileSystem documentDirectoryForExperienceId:experienceId];
}
...
{
  return [ABI32_0_0EXFileSystem cachesDirectoryForExperienceId:experienceId];
}
...
{
  return [ABI32_0_0EXFileSystem generatePathInDirectory:directory withExtension:extension];
}
...
+ (NSString *)generatePathInDirectory:(NSString *)directory withExtension:(NSString *)extension
{
  ...
  [ABI32_0_0EXFileSystem ensureDirExistsWithPath:directory];
