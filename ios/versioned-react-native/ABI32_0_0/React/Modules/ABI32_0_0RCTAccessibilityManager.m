...
#import "ABI32_0_0RCTAccessibilityManager.h"
...
NSString *const ABI32_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI32_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";
...
@interface ABI32_0_0RCTAccessibilityManager ()
...
@implementation ABI32_0_0RCTAccessibilityManager
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    self.contentSizeCategory = ABI32_0_0RCTSharedApplication().preferredContentSizeCategory;
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI32_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI32_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI32_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI32_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI32_0_0RCT_EXPORT_METHOD(setAccessibilityFocus:(nonnull NSNumber *)ReactABI32_0_0Tag)
{
...
ABI32_0_0RCT_EXPORT_METHOD(announceForAccessibility:(NSString *)announcement)
{
  UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, announcement);
}

ABI32_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI32_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI32_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI32_0_0RCTResponseSenderBlock)callback
...
@implementation ABI32_0_0RCTBridge (ABI32_0_0RCTAccessibilityManager)

- (ABI32_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI32_0_0RCTAccessibilityManager class]];
}
