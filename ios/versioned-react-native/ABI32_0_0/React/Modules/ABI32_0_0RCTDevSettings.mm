...
#import "ABI32_0_0RCTDevSettings.h"
...
#import "ABI32_0_0RCTJSCSamplingProfiler.h"
#import "ABI32_0_0RCTLog.h"
#import "ABI32_0_0RCTProfile.h"
#import "ABI32_0_0RCTUtils.h"

static NSString *const kABI32_0_0RCTDevSettingProfilingEnabled = @"profilingEnabled";
static NSString *const kABI32_0_0RCTDevSettingHotLoadingEnabled = @"hotLoadingEnabled";
static NSString *const kABI32_0_0RCTDevSettingLiveReloadEnabled = @"liveReloadEnabled";
static NSString *const kABI32_0_0RCTDevSettingIsInspectorShown = @"showInspector";
static NSString *const kABI32_0_0RCTDevSettingIsDebuggingRemotely = @"isDebuggingRemotely";
static NSString *const kABI32_0_0RCTDevSettingExecutorOverrideClass = @"executor-override";
static NSString *const kABI32_0_0RCTDevSettingShakeToShowDevMenu = @"shakeToShow";
static NSString *const kABI32_0_0RCTDevSettingIsPerfMonitorShown = @"ABI32_0_0RCTPerfMonitorKey";
static NSString *const kABI32_0_0RCTDevSettingStartSamplingProfilerOnLaunch = @"startSamplingProfilerOnLaunch";
...
#import <ABI32_0_0jschelpers/ABI32_0_0JSCWrapper.h>
#endif

#if ABI32_0_0RCT_DEV

@interface ABI32_0_0RCTDevSettingsUserDefaultsDataSource : NSObject <ABI32_0_0RCTDevSettingsDataSource>

@end

@implementation ABI32_0_0RCTDevSettingsUserDefaultsDataSource {
...
{
  ABI32_0_0RCTAssert((key != nil), @"%@", [NSString stringWithFormat:@"%@: Tried to update nil key", [self class]]);
  ...
  [_userDefaults setObject:_settings forKey:kABI32_0_0RCTDevSettingsUserDefaultsKey];
}
...
{
  NSDictionary *existingSettings = [_userDefaults objectForKey:kABI32_0_0RCTDevSettingsUserDefaultsKey];
...
@interface ABI32_0_0RCTDevSettings () <ABI32_0_0RCTBridgeModule, ABI32_0_0RCTInvalidating>
{
...
@property (nonatomic, readwrite, strong) id<ABI32_0_0RCTDevSettingsDataSource> dataSource;

@end

@implementation ABI32_0_0RCTDevSettings
...
{
  return YES; // ABI32_0_0RCT_DEV-only
}
...
- (instancetype)init
{
  ...
  ABI32_0_0RCTDevSettingsUserDefaultsDataSource *dataSource = [[ABI32_0_0RCTDevSettingsUserDefaultsDataSource alloc] initWithDefaultValues:defaultValues];
  return [self initWithDataSource:dataSource];
}

- (instancetype)initWithDataSource:(id<ABI32_0_0RCTDevSettingsDataSource>)dataSource
{
  if (self = [super init]) {
...
- (void)setBridge:(ABI32_0_0RCTBridge *)bridge
{
  ...
  [[ABI32_0_0RCTPackagerConnection sharedPackagerConnection]
   addNotificationHandler:^(id params) {
     if (params != (id)kCFNull && [params[@"debug"] boolValue]) {
       weakBridge.executorClass = objc_lookUpClass("ABI32_0_0RCTWebSocketExecutor");
     }
     [weakBridge reload];
   }
  ...
  [[ABI32_0_0RCTPackagerConnection sharedPackagerConnection]
   addRequestHandler:^(NSDictionary<NSString *, id> *params, ABI32_0_0RCTPackagerClientResponder *responder) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [bridge dispatchBlock:^{
      [ABI32_0_0RCTInspectorDevServerHelper connectWithBundleURL:bridge.bundleURL];
    } queue:ABI32_0_0RCTJSThread];
  });
#endif
}
...
#else
  return false;
#endif // ABI32_0_0RCT_ENABLE_INSPECTOR
}

- (BOOL)isRemoteDebuggingAvailable
{
  Class jsDebuggingExecutorClass = objc_lookUpClass("ABI32_0_0RCTWebSocketExecutor");
...
ABI32_0_0RCT_EXPORT_METHOD(reload)
{
  [_bridge reload];
}

ABI32_0_0RCT_EXPORT_METHOD(setIsShakeToShowDevMenuEnabled:(BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI32_0_0RCTDevSettingShakeToShowDevMenu];
}

- (BOOL)isShakeToShowDevMenuEnabled
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingShakeToShowDevMenu] boolValue];
}

ABI32_0_0RCT_EXPORT_METHOD(setIsDebuggingRemotely:(BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI32_0_0RCTDevSettingIsDebuggingRemotely];
  [self _remoteDebugSettingDidChange];
}

- (BOOL)isDebuggingRemotely
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingIsDebuggingRemotely] boolValue];
}
...
- (void)_remoteDebugSettingDidChange
{
  ...
  NSString *executorOverride = [[NSUserDefaults standardUserDefaults] stringForKey:kABI32_0_0RCTDevSettingExecutorOverrideClass];
  ...
  if (executorOverrideClass) {
    ...
    self.executorClass = enabled ? objc_getClass("ABI32_0_0RCTWebSocketExecutor") : nil;
  }
}

ABI32_0_0RCT_EXPORT_METHOD(setProfilingEnabled:(BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI32_0_0RCTDevSettingProfilingEnabled];
  [self _profilingSettingDidChange];
}

- (BOOL)isProfilingEnabled
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingProfilingEnabled] boolValue];
}

- (void)_profilingSettingDidChange
{
  BOOL enabled = self.isProfilingEnabled;
  if (_liveReloadURL && enabled != ABI32_0_0RCTProfileIsProfiling()) {
    if (enabled) {
      [_bridge startProfiling];
    } else {
      [_bridge stopProfiling:^(NSData *logData) {
        ABI32_0_0RCTProfileSendResult(self->_bridge, @"systrace", logData);
      }];
    }
  }
}

ABI32_0_0RCT_EXPORT_METHOD(setLiveReloadEnabled:(BOOL)enabled)
{
  [self _updateSettingWithValue:@(enabled) forKey:kABI32_0_0RCTDevSettingLiveReloadEnabled];
  [self _liveReloadSettingDidChange];
}

- (BOOL)isLiveReloadEnabled
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingLiveReloadEnabled] boolValue];
}
...
ABI32_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled:(BOOL)enabled)
{
  if (self.isHotLoadingEnabled != enabled) {
    [self _updateSettingWithValue:@(enabled) forKey:kABI32_0_0RCTDevSettingHotLoadingEnabled];
    [_bridge reload];
  }
}
...
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingHotLoadingEnabled] boolValue];
}

ABI32_0_0RCT_EXPORT_METHOD(toggleElementInspector)
{
  BOOL value = [[self settingForKey:kABI32_0_0RCTDevSettingIsInspectorShown] boolValue];
  [self _updateSettingWithValue:@(!value) forKey:kABI32_0_0RCTDevSettingIsInspectorShown];
...
- (void)toggleJSCSamplingProfiler
{
  ...
  if (JSC_JSValueGetType(globalContext, jsResult) != kJSTypeNull) {
    JSContext *context = [JSC_JSContext(globalContext) contextWithJSGlobalContextRef:globalContext];
    NSString *results = [[JSC_JSValue(globalContext) valueWithJSValueRef:jsResult inContext:context] toObject];
    ABI32_0_0RCTJSCSamplingProfiler *profilerModule = [_bridge moduleForClass:[ABI32_0_0RCTJSCSamplingProfiler class]];
...
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingIsInspectorShown] boolValue];
}

- (void)setIsPerfMonitorShown:(BOOL)isPerfMonitorShown
{
  [self _updateSettingWithValue:@(isPerfMonitorShown) forKey:kABI32_0_0RCTDevSettingIsPerfMonitorShown];
}

- (BOOL)isPerfMonitorShown
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingIsPerfMonitorShown] boolValue];
}

- (void)setStartSamplingProfilerOnLaunch:(BOOL)startSamplingProfilerOnLaunch
{
  [self _updateSettingWithValue:@(startSamplingProfilerOnLaunch) forKey:kABI32_0_0RCTDevSettingStartSamplingProfilerOnLaunch];
}

- (BOOL)startSamplingProfilerOnLaunch
{
  return [[self settingForKey:kABI32_0_0RCTDevSettingStartSamplingProfilerOnLaunch] boolValue];
}
...
- (void)setExecutorClass:(Class)executorClass
{
  ...
  if (_bridge.executorClass != executorClass) {
    ...
    if (executorClass == Nil &&
        _bridge.executorClass != objc_lookUpClass("ABI32_0_0RCTWebSocketExecutor")) {
      return;
    }
...
{
  if (!_isJSLoaded || ![[self settingForKey:kABI32_0_0RCTDevSettingLiveReloadEnabled] boolValue] || !_liveReloadURL) {
    return;
  }
  ...
  __weak ABI32_0_0RCTDevSettings *weakSelf = self;
  _liveReloadUpdateTask = [[NSURLSession sharedSession] dataTaskWithURL:_liveReloadURL completionHandler:
                           ^(__unused NSData *data, NSURLResponse *response, NSError *error) {
                             ...
                             dispatch_async(dispatch_get_main_queue(), ^{
                               __strong ABI32_0_0RCTDevSettings *strongSelf = weakSelf;
                               if (strongSelf && [[strongSelf settingForKey:kABI32_0_0RCTDevSettingLiveReloadEnabled] boolValue]) {
...
#else // #if ABI32_0_0RCT_DEV

@implementation ABI32_0_0RCTDevSettings

- (instancetype)initWithDataSource:(id<ABI32_0_0RCTDevSettingsDataSource>)dataSource { return [super init]; }
...
@implementation ABI32_0_0RCTBridge (ABI32_0_0RCTDevSettings)

- (ABI32_0_0RCTDevSettings *)devSettings
{
#if ABI32_0_0RCT_DEV
  return [self moduleForClass:[ABI32_0_0RCTDevSettings class]];
#else
  return nil;
#endif
}
