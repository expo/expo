...
#import "ABI32_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI32_0_0RCTSurface () <ABI32_0_0RCTSurfaceRootShadowViewDelegate, ABI32_0_0RCTUIManagerObserver>
@end

@implementation ABI32_0_0RCTSurface {
  // Immutable
  ...
  ABI32_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI32_0_0RCTSurfaceView *_Nullable _view;
  ABI32_0_0RCTTouchHandler *_Nullable _touchHandler;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI32_0_0RCTAllocateRootViewTag();
    ...
    if (!bridge.loading) {
      _stage = _stage | ABI32_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI32_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI32_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI32_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI32_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI32_0_0RCTSurfaceRootView *rootView =
    (ABI32_0_0RCTSurfaceRootView *)[self._uiManager viewForReactABI32_0_0Tag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI32_0_0RCTAssert([rootView isKindOfClass:[ABI32_0_0RCTSurfaceRootView class]],
    @"Received root view is not an instanse of `ABI32_0_0RCTSurfaceRootView`.");
...
- (void)_run
{
  ABI32_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI32_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountReactABI32_0_0ComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];
...
- (void)_stop
{
  [self unmountReactABI32_0_0ComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI32_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI32_0_0RCTIsMainQueue() ? ABI32_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI32_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI32_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI32_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI32_0_0Tag:self->_rootViewTag];
    ABI32_0_0RCTAssert([rootShadowView isKindOfClass:[ABI32_0_0RCTSurfaceRootShadowView class]],
      @"Received shadow view is not an instanse of `ABI32_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI32_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI32_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI32_0_0RCTSurfaceRootShadowView *rootShadowView =
      (ABI32_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForReactABI32_0_0Tag:self->_rootViewTag];
...
{
  if (ABI32_0_0RCTIsUIManagerQueue()) {
    ABI32_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI32_0_0RCTIsMainQueue() && (stage == ABI32_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI32_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI32_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI32_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI32_0_0RCTAssert(NO, @"Only waiting for `ABI32_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI32_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI32_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI32_0_0RCTIsMainQueue() && (stage == ABI32_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI32_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI32_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(ABI32_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
...
{
  [self _setStage:ABI32_0_0RCTSurfaceStageSurfaceDidInitialRendering];
...
{
  [self _setStage:ABI32_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI32_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
{
  if (self.stage & ABI32_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI32_0_0RCTSurfaceStageSurfaceDidInitialMounting];
