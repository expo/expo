...
- (void)setUp
{
  ...
  if (implementsRequireMainQueueSetup) {
    ...
    if (_requiresMainQueueSetup) {
      ...
      ABI32_0_0RCTLogWarn(@"Module %@ requires main queue setup since it overrides `%s` but doesn't implement "
        "`requiresMainQueueSetup`. In a future release ReactABI32_0_0 Native will default to initializing all native modules "
                             ...
                             bridge:(ABI32_0_0RCTBridge *)bridge
{
  return [self initWithModuleClass:moduleClass
                    moduleProvider:^id<ABI32_0_0RCTBridgeModule>{ return [moduleClass new]; }
                            bridge:bridge];
}

- (instancetype)initWithModuleClass:(Class)moduleClass
                     moduleProvider:(ABI32_0_0RCTBridgeModuleProvider)moduleProvider
                             bridge:(ABI32_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
...
- (instancetype)initWithModuleInstance:(id<ABI32_0_0RCTBridgeModule>)instance
                                bridge:(ABI32_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
...
{
  ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, @"[ABI32_0_0RCTModuleData setUpInstanceAndBridge]", @{
  ...
  });
  {
    ...
    if (!_setupComplete && _bridge.valid) {
      if (!_instance) {
        if (ABI32_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI32_0_0RCTAssertMainQueue();
        }
        ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, @"[ABI32_0_0RCTModuleData setUpInstanceAndBridge] Create module", nil);
        ...
        if (!_instance) {
          ...
          _setupComplete = YES;
          ABI32_0_0RCTLogWarn(@"The module %@ is returning nil from its constructor. You "
                     "may need to instantiate it yourself and pass it into the "
                     "bridge.", _moduleClass);
        }
      }

      if (_instance && ABI32_0_0RCTProfileIsProfiling()) {
        ABI32_0_0RCTProfileHookInstance(_instance);
      }
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, @"[ABI32_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
      [(id)_instance setValue:_bridge forKey:@"bridge"];
    }
    @catch (NSException *exception) {
      ABI32_0_0RCTLogError(@"%@ has no setter or ivar for its bridge, which is not "
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, @"[ABI32_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI32_0_0RCTDidInitializeModuleNotification
                                                        object:_bridge
                                                      userInfo:@{@"module": _instance, @"bridge": ABI32_0_0RCTNullIfNil(_bridge.parentBridge)}];
    ABI32_0_0RCT_PROFILE_END_EVENT(ABI32_0_0RCTProfileTagAlways, @"");
  }
}
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, @"[ABI32_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ReactABI32_0_0.%@Queue", self.name];
      ...
      if (implementsMethodQueue) {
        ...
        @catch (NSException *exception) {
          ABI32_0_0RCTLogError(@"%@ is returning nil for its methodQueue, which is not "
...
- (id<ABI32_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI32_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, @"[ABI32_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI32_0_0RCTIsMainQueue()) {
        ABI32_0_0RCTLogWarn(@"ABI32_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI32_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI32_0_0RCTBridgeMethod>> *)methods
{
  if (!_methods) {
    NSMutableArray<id<ABI32_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
    ...
    while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
      ...
      for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
          ...
          auto exportedMethod = ((const ABI32_0_0RCTMethodInfo *(*)(id, SEL))imp)(_moduleClass, selector);
          id<ABI32_0_0RCTBridgeMethod> moduleMethod = [[ABI32_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod
...
- (void)gatherConstants
{
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI32_0_0RCT_PROFILE_BEGIN_EVENT(ABI32_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI32_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    if (_requiresMainQueueSetup) {
      if (!ABI32_0_0RCTIsMainQueue()) {
        ABI32_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
      }
...
{
  (void)[self instance];
  ABI32_0_0RCTAssert(_methodQueue != nullptr, @"Module %@ has no methodQueue (instance: %@, bridge.valid: %d)",
