...
@interface ABI32_0_0RCTScrollEvent : NSObject <ABI32_0_0RCTEvent>
...
@implementation ABI32_0_0RCTScrollEvent
{
...
{
  ABI32_0_0RCTAssertParam(ReactABI32_0_0Tag);
...
- (ABI32_0_0RCTScrollEvent *)coalesceWithEvent:(ABI32_0_0RCTScrollEvent *)newEvent
{
...
- (NSArray *)arguments
{
  return @[self.viewTag, ABI32_0_0RCTNormalizeInputEventName(self.eventName), [self body]];
}
 ...
 */
@interface ABI32_0_0RCTCustomScrollView : UIScrollView<UIGestureRecognizerDelegate>

@property (nonatomic, assign) BOOL centerContent;
#if !TARGET_OS_TV
@property (nonatomic, strong) UIView<ABI32_0_0RCTCustomRefreshContolProtocol> *customRefreshControl;
...
@implementation ABI32_0_0RCTCustomScrollView
...
- (UIView *)contentView
{
  return ((ABI32_0_0RCTScrollView *)self.superview).contentView;
}
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  UIView *JSResponder = [ABI32_0_0RCTUIManager JSResponder];
...
{
  if ([self _shouldDisableScrollInteraction] && ![[ABI32_0_0RCTUIManager JSResponder] isKindOfClass:[ABI32_0_0RCTScrollView class]]) {
 ...
 * touch started inside of a `UIControl` (`UIButton` etc). For ReactABI32_0_0 scroll
...
#if !TARGET_OS_TV
- (void)setCustomRefreshControl:(UIView<ABI32_0_0RCTCustomRefreshContolProtocol> *)refreshControl
{
...
@interface ABI32_0_0RCTScrollView () <ABI32_0_0RCTUIManagerObserver>

@end

@implementation ABI32_0_0RCTScrollView
{
  ABI32_0_0RCTEventDispatcher *_eventDispatcher;
  CGRect _prevFirstVisibleFrame;
  __weak UIView *_firstVisibleView;
  ABI32_0_0RCTCustomScrollView *_scrollView;
  UIView *_contentView;
...
- (instancetype)initWithEventDispatcher:(ABI32_0_0RCTEventDispatcher *)eventDispatcher
{
  ABI32_0_0RCTAssertParam(eventDispatcher);

  if ((self = [super initWithFrame:CGRectZero])) {
    _eventDispatcher = eventDispatcher;

    _scrollView = [[ABI32_0_0RCTCustomScrollView alloc] initWithFrame:CGRectZero];
...
static inline void ABI32_0_0RCTApplyTransformationAccordingLayoutDirection(UIView *view, UIUserInterfaceLayoutDirection layoutDirection) {
  view.transform =
...
{
  [super setReactABI32_0_0LayoutDirection:layoutDirection];

  ABI32_0_0RCTApplyTransformationAccordingLayoutDirection(_scrollView, layoutDirection);
  ABI32_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, layoutDirection);
}
...
#if !TARGET_OS_TV
  if ([view conformsToProtocol:@protocol(ABI32_0_0RCTCustomRefreshContolProtocol)]) {
    [_scrollView setCustomRefreshControl:(UIView<ABI32_0_0RCTCustomRefreshContolProtocol> *)view];
...
#endif
  {
    ABI32_0_0RCTAssert(_contentView == nil, @"ABI32_0_0RCTScrollView may only contain a single subview");
    _contentView = view;
    ABI32_0_0RCTApplyTransformationAccordingLayoutDirection(_contentView, self.ReactABI32_0_0LayoutDirection);
...
#if !TARGET_OS_TV
  if ([subview conformsToProtocol:@protocol(ABI32_0_0RCTCustomRefreshContolProtocol)]) {
...
#endif
  {
    ABI32_0_0RCTAssert(_contentView == subview, @"Attempted to remove non-existent subview");
    _contentView = nil;
  }
}
...
- (void)layoutSubviews
{
  ...
  ABI32_0_0RCTAssert(self.subviews.count == 1, @"we should only have exactly one subview");
  ABI32_0_0RCTAssert([self.subviews lastObject] == _scrollView, @"our only subview should be a scrollview");

#if !TARGET_OS_TV
  // Adjust the refresh control frame if the scrollview layout changes.
  UIView<ABI32_0_0RCTCustomRefreshContolProtocol> *refreshControl = _scrollView.customRefreshControl;
...
- (void)updateClippedSubviews
{
  ...
  if (shouldClipAgain) {
    ...
    [self ReactABI32_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
...
#define ABI32_0_0RCT_SEND_SCROLL_EVENT(_eventName, _userData) { \
...
#define ABI32_0_0RCT_FORWARD_SCROLL_EVENT(call) \
...
#define ABI32_0_0RCT_SCROLL_EVENT_HANDLER(delegateMethod, eventName) \
- (void)delegateMethod:(UIScrollView *)scrollView           \
{                                                           \
  ABI32_0_0RCT_SEND_SCROLL_EVENT(eventName, nil);                    \
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(delegateMethod:scrollView);      \
}

ABI32_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewWillBeginDecelerating, onMomentumScrollBegin)
ABI32_0_0RCT_SCROLL_EVENT_HANDLER(scrollViewDidZoom, onScroll)
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if (_allowNextScrollNoMatterWhat ||
    ...
    if (_DEPRECATED_sendUpdatedChildFrames) {
      ...
      ABI32_0_0RCT_SEND_SCROLL_EVENT(onScroll, (@{@"updatedChildFrames": [self calculateChildFramesData]}));
    } else {
      ABI32_0_0RCT_SEND_SCROLL_EVENT(onScroll, nil);
    }
  ...
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidScroll:scrollView);
}

- (NSArray<NSDictionary *> *)calculateChildFramesData
{
    NSMutableArray<NSDictionary *> *updatedChildFrames = [NSMutableArray new];
    [[_contentView ReactABI32_0_0Subviews] enumerateObjectsUsingBlock:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  ABI32_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginDragging:scrollView);
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  ABI32_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, userData);
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDragging:scrollView willDecelerate:decelerate);
}

- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
  ABI32_0_0RCT_SEND_SCROLL_EVENT(onScrollBeginDrag, nil);
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewWillBeginZooming:scrollView withView:view);
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale
{
  ABI32_0_0RCT_SEND_SCROLL_EVENT(onScrollEndDrag, nil);
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndZooming:scrollView withView:view atScale:scale);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
  ...
  ABI32_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndDecelerating:scrollView);
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
  ...
  // Fire the end deceleration event
  ABI32_0_0RCT_SEND_SCROLL_EVENT(onMomentumScrollEnd, nil);
  ABI32_0_0RCT_FORWARD_SCROLL_EVENT(scrollViewDidEndScrollingAnimation:scrollView);
}
...
- (CGSize)_calculateViewportSize
{
  ...
  if (_automaticallyAdjustContentInsets) {
    UIEdgeInsets contentInsets = [ABI32_0_0RCTView contentInsetsForView:self];
...
- (void)uiManagerWillPerformMounting:(ABI32_0_0RCTUIManager *)manager
{
  ...
  [manager prependUIBlock:^(ABI32_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
  ...
  [manager addUIBlock:^(ABI32_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
...
#define ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setter, getter, type) \
...
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceHorizontal, alwaysBounceHorizontal, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setAlwaysBounceVertical, alwaysBounceVertical, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setBounces, bounces, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setBouncesZoom, bouncesZoom, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setCanCancelContentTouches, canCancelContentTouches, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setDecelerationRate, decelerationRate, CGFloat)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setDirectionalLockEnabled, isDirectionalLockEnabled, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setIndicatorStyle, indicatorStyle, UIScrollViewIndicatorStyle)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setKeyboardDismissMode, keyboardDismissMode, UIScrollViewKeyboardDismissMode)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setMaximumZoomScale, maximumZoomScale, CGFloat)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setMinimumZoomScale, minimumZoomScale, CGFloat)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollEnabled, isScrollEnabled, BOOL)
#if !TARGET_OS_TV
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setPagingEnabled, isPagingEnabled, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollsToTop, scrollsToTop, BOOL)
#endif
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsHorizontalScrollIndicator, showsHorizontalScrollIndicator, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setShowsVerticalScrollIndicator, showsVerticalScrollIndicator, BOOL)
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setZoomScale, zoomScale, CGFloat);
ABI32_0_0RCT_SET_AND_PRESERVE_OFFSET(setScrollIndicatorInsets, scrollIndicatorInsets, UIEdgeInsets);
                       ...
                       userData:(NSDictionary *)userData
{
  ...
  ABI32_0_0RCTScrollEvent *scrollEvent = [[ABI32_0_0RCTScrollEvent alloc] initWithEventName:eventName
...
@implementation ABI32_0_0RCTEventDispatcher (ABI32_0_0RCTScrollView)
...
- (void)sendFakeScrollEvent:(NSNumber *)ReactABI32_0_0Tag
{
  ...
  ABI32_0_0RCTScrollEvent *fakeScrollEvent = [[ABI32_0_0RCTScrollEvent alloc] initWithEventName:eventName
                                                                     ReactABI32_0_0Tag:ReactABI32_0_0Tag
