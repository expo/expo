...
#import "ABI32_0_0RCTBorderDrawing.h"
...
@implementation UIView (ABI32_0_0RCTViewUnmounting)

- (void)ReactABI32_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI32_0_0_remountAllSubviews];
  }
}

- (void)ReactABI32_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI32_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI32_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI32_0_0RootView]) {
      break;
    }
...
static NSString *ABI32_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI32_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI32_0_0RCTView
{
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI32_0_0RCTBorderStyleSolid;
...
ABI32_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)

- (void)setReactABI32_0_0LayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ReactABI32_0_0LayoutDirection != layoutDirection) {
    _ReactABI32_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI32_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (void)setPointerEvents:(ABI32_0_0RCTPointerEvents)pointerEvents
{
  ...
  self.userInteractionEnabled = (pointerEvents != ABI32_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI32_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI32_0_0RCTPointerEventsNone || _pointerEvents == ABI32_0_0RCTPointerEventsBoxOnly);
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    // Take z-index into account when calculating the touch target.
    NSArray<UIView *> *sortedSubviews = [self ReactABI32_0_0ZIndexSortedSubviews];
  ...
  switch (_pointerEvents) {
    ...
    case ABI32_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI32_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI32_0_0RCTLogError(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
{
  if (self.ReactABI32_0_0AccessibilityElement == self) {
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI32_0_0Tag: %@;", self.ReactABI32_0_0Tag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI32_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ReactABI32_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI32_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.ReactABI32_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI32_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI32_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI32_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI32_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.ReactABI32_0_0Subviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI32_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI32_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI32_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI32_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ReactABI32_0_0_findClipView];
  if (clipView) {
    [self ReactABI32_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI32_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x) {
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ReactABI32_0_0LayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI32_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI32_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI32_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets) {
      ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
      ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets) {
    ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
    ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, ABI32_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
    ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
    ABI32_0_0RCTDefaultIfNegativeTo(borderWidth, ABI32_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI32_0_0RCTCornerRadii)cornerRadii
{
  ...
  if ([[ABI32_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI32_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, _borderTopStartRadius);
    const CGFloat topEndRadius = ABI32_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, _borderTopEndRadius);
    const CGFloat bottomStartRadius = ABI32_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, _borderBottomStartRadius);
    const CGFloat bottomEndRadius = ABI32_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, _borderBottomEndRadius);
    ...
    topLeftRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, ABI32_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, ABI32_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, ABI32_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI32_0_0RCTDefaultIfNegativeTo(radius, ABI32_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI32_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI32_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI32_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI32_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI32_0_0RCTCornerRadii){
...
- (ABI32_0_0RCTBorderColors)borderColors
{
  ...
  if ([[ABI32_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    ...
    return (ABI32_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI32_0_0RCTUpdateShadowPathForView(self);

  const ABI32_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI32_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI32_0_0RCTRunningInTestEnvironment() &&
  ABI32_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI32_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI32_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI32_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI32_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI32_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI32_0_0RCTUpdateShadowPathForView(ABI32_0_0RCTView *view)
{
  if (ABI32_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI32_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI32_0_0RCTCornerInsets cornerInsets = ABI32_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI32_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI32_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI32_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI32_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI32_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI32_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI32_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI32_0_0RCTBorderStyle)style \
  {                                                    \
