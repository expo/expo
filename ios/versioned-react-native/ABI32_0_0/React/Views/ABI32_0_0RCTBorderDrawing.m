...
static const CGFloat ABI32_0_0RCTViewBorderThreshold = 0.001;

BOOL ABI32_0_0RCTBorderInsetsAreEqual(UIEdgeInsets borderInsets)
{
  return
  ABS(borderInsets.left - borderInsets.right) < ABI32_0_0RCTViewBorderThreshold &&
  ABS(borderInsets.left - borderInsets.bottom) < ABI32_0_0RCTViewBorderThreshold &&
  ABS(borderInsets.left - borderInsets.top) < ABI32_0_0RCTViewBorderThreshold;
}

BOOL ABI32_0_0RCTCornerRadiiAreEqual(ABI32_0_0RCTCornerRadii cornerRadii)
{
  return
  ABS(cornerRadii.topLeft - cornerRadii.topRight) < ABI32_0_0RCTViewBorderThreshold &&
  ABS(cornerRadii.topLeft - cornerRadii.bottomLeft) < ABI32_0_0RCTViewBorderThreshold &&
  ABS(cornerRadii.topLeft - cornerRadii.bottomRight) < ABI32_0_0RCTViewBorderThreshold;
}
...
{
  return (ABI32_0_0RCTCornerInsets) {
    {
...
static UIEdgeInsets ABI32_0_0RCTRoundInsetsToPixel(UIEdgeInsets edgeInsets) {
    edgeInsets.top = ABI32_0_0RCTRoundPixelValue(edgeInsets.top);
    edgeInsets.bottom = ABI32_0_0RCTRoundPixelValue(edgeInsets.bottom);
    edgeInsets.left = ABI32_0_0RCTRoundPixelValue(edgeInsets.left);
    edgeInsets.right = ABI32_0_0RCTRoundPixelValue(edgeInsets.right);

    return edgeInsets;
}

static void ABI32_0_0RCTPathAddEllipticArc(CGMutablePathRef path,
                                       ...
                                       const CGAffineTransform *transform)
{
  ...
  ABI32_0_0RCTPathAddEllipticArc(path, transform, (CGPoint){
...
static void ABI32_0_0RCTEllipseGetIntersectionsWithLine(CGRect ellipseBounds,
                                               CGPoint lineStart,
                                               CGPoint lineEnd,
...
NS_INLINE BOOL ABI32_0_0RCTCornerRadiiAreAboveThreshold(ABI32_0_0RCTCornerRadii cornerRadii) {
  return (cornerRadii.topLeft > ABI32_0_0RCTViewBorderThreshold ||
    cornerRadii.topRight > ABI32_0_0RCTViewBorderThreshold      ||
    cornerRadii.bottomLeft > ABI32_0_0RCTViewBorderThreshold    ||
    cornerRadii.bottomRight > ABI32_0_0RCTViewBorderThreshold);
}

static CGPathRef ABI32_0_0RCTPathCreateOuterOutline(BOOL drawToEdge, CGRect rect, ABI32_0_0RCTCornerRadii cornerRadii) {
  if (drawToEdge) {
    return CGPathCreateWithRect(rect, NULL);
  }

  return ABI32_0_0RCTPathCreateWithRoundedRect(rect, ABI32_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
}

static CGContextRef ABI32_0_0RCTUIGraphicsBeginImageContext(CGSize size, CGColorRef backgroundColor, BOOL hasCornerRadii, BOOL drawToEdge) {
...
static UIImage *ABI32_0_0RCTGetSolidBorderImage(ABI32_0_0RCTCornerRadii cornerRadii,
                                       CGSize viewSize,
                                       ...
                                       BOOL drawToEdge)
{
  const BOOL hasCornerRadii = ABI32_0_0RCTCornerRadiiAreAboveThreshold(cornerRadii);
  const ABI32_0_0RCTCornerInsets cornerInsets = ABI32_0_0RCTGetCornerInsets(cornerRadii, borderInsets);
  ...
  borderInsets = ABI32_0_0RCTRoundInsetsToPixel(borderInsets);
  ...
  CGContextRef ctx = ABI32_0_0RCTUIGraphicsBeginImageContext(size, backgroundColor, hasCornerRadii, drawToEdge);
  const CGRect rect = {.size = size};
  CGPathRef path = ABI32_0_0RCTPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
  ...
  CGPathRef insetPath = ABI32_0_0RCTPathCreateWithRoundedRect(UIEdgeInsetsInsetRect(rect, borderInsets), cornerInsets, NULL);

  CGContextAddPath(ctx, insetPath);
  CGContextEOClip(ctx);

  BOOL hasEqualColors = ABI32_0_0RCTBorderColorsAreEqual(borderColors);
  if ((drawToEdge || !hasCornerRadii) && hasEqualColors) {
    ...
    if (cornerInsets.topLeft.width > 0 && cornerInsets.topLeft.height > 0) {
      CGPoint points[2];
      ABI32_0_0RCTEllipseGetIntersectionsWithLine((CGRect){
...
static UIImage *ABI32_0_0RCTGetDashedOrDottedBorderImage(ABI32_0_0RCTBorderStyle borderStyle,
                                                ...
                                                BOOL drawToEdge)
{
  NSCParameterAssert(borderStyle == ABI32_0_0RCTBorderStyleDashed || borderStyle == ABI32_0_0RCTBorderStyleDotted);

  if (!ABI32_0_0RCTBorderColorsAreEqual(borderColors) || !ABI32_0_0RCTBorderInsetsAreEqual(borderInsets)) {
    ABI32_0_0RCTLogWarn(@"Unsupported dashed / dotted border style");
    return nil;
  }
  ...
  const BOOL hasCornerRadii = ABI32_0_0RCTCornerRadiiAreAboveThreshold(cornerRadii);
  CGContextRef ctx = ABI32_0_0RCTUIGraphicsBeginImageContext(viewSize, backgroundColor, hasCornerRadii, drawToEdge);
  const CGRect rect = {.size = viewSize};

  if (backgroundColor) {
    CGPathRef outerPath = ABI32_0_0RCTPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
  ...
  CGPathRef path = ABI32_0_0RCTPathCreateWithRoundedRect(pathRect, ABI32_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);

  CGFloat dashLengths[2];
  dashLengths[0] = dashLengths[1] = (borderStyle == ABI32_0_0RCTBorderStyleDashed ? 3 : 1) * lineWidth;
                           ...
                           BOOL drawToEdge)
{
  ...
  switch (borderStyle) {
    case ABI32_0_0RCTBorderStyleSolid:
      return ABI32_0_0RCTGetSolidBorderImage(cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
    case ABI32_0_0RCTBorderStyleDashed:
    case ABI32_0_0RCTBorderStyleDotted:
      return ABI32_0_0RCTGetDashedOrDottedBorderImage(borderStyle, cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
    case ABI32_0_0RCTBorderStyleUnset:
      break;
  }
