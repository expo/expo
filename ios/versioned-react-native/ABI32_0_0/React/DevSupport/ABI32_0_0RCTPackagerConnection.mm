...
#import <ReactABI32_0_0/ABI32_0_0RCTBundleURLProvider.h>
...
#import <ReactABI32_0_0/ABI32_0_0RCTPackagerClient.h>
#import <ReactABI32_0_0/ABI32_0_0RCTReconnectingWebSocket.h>
#import <ReactABI32_0_0/ABI32_0_0RCTSRWebSocket.h>
#import <ReactABI32_0_0/ABI32_0_0RCTUtils.h>

#if ABI32_0_0RCT_DEV
@interface ABI32_0_0RCTPackagerConnection () <ABI32_0_0RCTReconnectingWebSocketDelegate>
@end
...
@implementation ABI32_0_0RCTPackagerConnection {
  std::mutex _mutex; // protects all ivars
  ABI32_0_0RCTReconnectingWebSocket *_socket;
  BOOL _socketConnected;
  NSString *_jsLocationForSocket;
  id _bundleURLChangeObserver;
  uint32_t _nextToken;
  std::vector<Registration<ABI32_0_0RCTNotificationHandler>> _notificationRegistrations;
  std::vector<Registration<ABI32_0_0RCTRequestHandler>> _requestRegistrations;
  std::vector<Registration<ABI32_0_0RCTConnectedHandler>> _connectedRegistrations;
}

+ (instancetype)sharedPackagerConnection
{
  static ABI32_0_0RCTPackagerConnection *connection;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    connection = [ABI32_0_0RCTPackagerConnection new];
  });
  return connection;
}
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _jsLocationForSocket = [ABI32_0_0RCTBundleURLProvider sharedSettings].jsLocation;
    _socket = socketForLocation(_jsLocationForSocket);
    _socket.delegate = self;
    [_socket start];

    ABI32_0_0RCTPackagerConnection *const __weak weakSelf = self;
    _bundleURLChangeObserver =
    [[NSNotificationCenter defaultCenter]
     addObserverForName:ABI32_0_0RCTBundleURLProviderUpdatedNotification
     object:nil
...
static ABI32_0_0RCTReconnectingWebSocket *socketForLocation(NSString *const jsLocation)
{
  NSURLComponents *const components = [NSURLComponents new];
  components.host = jsLocation ?: @"localhost";
  components.scheme = @"http";
  components.port = @(kABI32_0_0RCTBundleURLProviderDefaultPort);
  components.path = @"/message";
  components.queryItems = @[[NSURLQueryItem queryItemWithName:@"role" value:@"ios"]];
  static dispatch_queue_t queue;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    queue = dispatch_queue_create("com.facebook.ABI32_0_0RCTPackagerConnectionQueue", DISPATCH_QUEUE_SERIAL);
  });
  return [[ABI32_0_0RCTReconnectingWebSocket alloc] initWithURL:components.URL queue:queue];
}
...
- (void)bundleURLSettingsChanged
{
  ...
  NSString *const jsLocation = [ABI32_0_0RCTBundleURLProvider sharedSettings].jsLocation;
...
- (ABI32_0_0RCTHandlerToken)addNotificationHandler:(ABI32_0_0RCTNotificationHandler)handler queue:(dispatch_queue_t)queue forMethod:(NSString *)method
{
  std::lock_guard<std::mutex> l(_mutex);
...
- (ABI32_0_0RCTHandlerToken)addRequestHandler:(ABI32_0_0RCTRequestHandler)handler queue:(dispatch_queue_t)queue forMethod:(NSString *)method
{
  std::lock_guard<std::mutex> l(_mutex);
  const auto token = _nextToken++;
...
- (ABI32_0_0RCTHandlerToken)addConnectedHandler:(ABI32_0_0RCTConnectedHandler)handler queue:(dispatch_queue_t)queue
{
  std::lock_guard<std::mutex> l(_mutex);
  if (_socketConnected) {
...
- (void)removeHandler:(ABI32_0_0RCTHandlerToken)token
{
  std::lock_guard<std::mutex> l(_mutex);
...
template <typename Handler>
static void eraseRegistrationsWithToken(std::vector<Registration<Handler>> &registrations, ABI32_0_0RCTHandlerToken token)
{
...
- (void)addHandler:(id<ABI32_0_0RCTPackagerClientMethod>)handler forMethod:(NSString *)method
{
  ...
  [self addRequestHandler:^(NSDictionary<NSString *, id> *request, ABI32_0_0RCTPackagerClientResponder *responder) {
...
{
  NSArray<NSNumber *> *const kSupportedVersions = @[ @(ABI32_0_0RCT_PACKAGER_CLIENT_PROTOCOL_VERSION) ];
  return [kSupportedVersions containsObject:version];
}

#pragma mark - ABI32_0_0RCTReconnectingWebSocketDelegate

- (void)reconnectingWebSocketDidOpen:(ABI32_0_0RCTReconnectingWebSocket *)webSocket
{
  std::vector<Registration<ABI32_0_0RCTConnectedHandler>> registrations;
  {
    std::lock_guard<std::mutex> l(_mutex);
  ...
  for (const auto &registration : registrations) {
    ...
    ABI32_0_0RCTConnectedHandler handler = registration.handler;
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *msg = ABI32_0_0RCTJSONParse(message, &error);

  if (error) {
    ABI32_0_0RCTLogError(@"%@ failed to parse message with error %@\n<message>\n%@\n</message>", [self class], error, msg);
    return;
  }

  if (!isSupportedVersion(msg[@"version"])) {
    ABI32_0_0RCTLogError(@"%@ received message with not supported version %@", [self class], msg[@"version"]);
    return;
  }
  ...
  if (messageId) { // Request
    const std::vector<Registration<ABI32_0_0RCTRequestHandler>> registrations(registrationsWithMethod(_mutex, _requestRegistrations, method));
    if (registrations.empty()) {
      ABI32_0_0RCTLogError(@"No handler found for packager method %@", msg[@"method"]);
      [[[ABI32_0_0RCTPackagerClientResponder alloc] initWithId:messageId
                                               socket:webSocket]
       respondWithError:
      ...
      ABI32_0_0RCTRequestHandler handler = registrations.back().handler;
      dispatch_async(registrations.back().queue, ^{
        handler(params, [[ABI32_0_0RCTPackagerClientResponder alloc] initWithId:messageId socket:webSocket]);
      });
    }
  } else { // Notification
    const std::vector<Registration<ABI32_0_0RCTNotificationHandler>> registrations(registrationsWithMethod(_mutex, _notificationRegistrations, method));
    for (const auto &registration : registrations) {
      // Beware: don't capture the reference to handler in a dispatched block!
      ABI32_0_0RCTNotificationHandler handler = registration.handler;
