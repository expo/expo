...
#import "ABI32_0_0RCTSurfaceTouchHandler.h"
...
@protocol ABI32_0_0RCTTouchableComponentViewProtocol <NSObject>
  - (SharedViewEventEmitter)touchEventEmitter;
@end

typedef NS_ENUM(NSInteger, ABI32_0_0RCTTouchEventType) {
  ABI32_0_0RCTTouchEventTypeTouchStart,
  ABI32_0_0RCTTouchEventTypeTouchMove,
  ABI32_0_0RCTTouchEventTypeTouchEnd,
  ABI32_0_0RCTTouchEventTypeTouchCancel,
};
...
static void UpdateActiveTouchWithUITouch(ActiveTouch &activeTouch, UITouch *uiTouch, UIView *rootComponentView) {
  ...
  activeTouch.touch.offsetPoint = ABI32_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI32_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI32_0_0RCTPointFromCGPoint(pagePoint);
...
static ActiveTouch CreateTouchWithUITouch(UITouch *uiTouch, UIView *rootComponentView) {
  ...
  if ([componentView respondsToSelector:@selector(touchEventEmitter)]) {
    activeTouch.eventEmitter = [(id<ABI32_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitter];
...
@interface ABI32_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI32_0_0RCTSurfaceTouchHandler {
  std::unordered_map<
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ReactABI32_0_0 Native,
...
- (void)_dispatchTouches:(NSSet<UITouch *> *)touches eventType:(ABI32_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI32_0_0RCTTouchEventTypeTouchEnd || eventType == ABI32_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &eventEmitter : uniqueEventEmitter) {
    ...
    switch (eventType) {
      case ABI32_0_0RCTTouchEventTypeTouchStart:
        eventEmitter->onTouchStart(event);
        break;
      case ABI32_0_0RCTTouchEventTypeTouchMove:
        eventEmitter->onTouchMove(event);
        break;
      case ABI32_0_0RCTTouchEventTypeTouchEnd:
        eventEmitter->onTouchEnd(event);
        break;
      case ABI32_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchTouches:touches eventType:ABI32_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchTouches:touches eventType:ABI32_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchTouches:touches eventType:ABI32_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches];
  [self _dispatchTouches:touches eventType:ABI32_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
