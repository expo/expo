...
#import "ABI32_0_0RCTViewComponentView.h"

#import <ABI32_0_0fabric/ABI32_0_0components/view/ViewProps.h>
#import <ABI32_0_0fabric/ABI32_0_0components/view/ViewEventEmitter.h>

#import "ABI32_0_0RCTConversions.h"

using namespace facebook::ReactABI32_0_0;

@implementation ABI32_0_0RCTViewComponentView
{
...
- (void)layoutSubviews
{
  ...
  if (_contentView) {
    _contentView.frame = ABI32_0_0RCTCGRectFromRect(_layoutMetrics.getContentFrame());
  }
}
           ...
           oldProps:(SharedProps)oldProps
{
  ...
  if (oldViewProps.backgroundColor != newViewProps.backgroundColor) {
    self.backgroundColor = ABI32_0_0RCTUIColorFromSharedColor(newViewProps.backgroundColor);
  }

  // `foregroundColor`
  if (oldViewProps.foregroundColor != newViewProps.foregroundColor) {
    self.foregroundColor = ABI32_0_0RCTUIColorFromSharedColor(newViewProps.foregroundColor);
  }

  // `shadowColor`
  if (oldViewProps.shadowColor != newViewProps.shadowColor) {
    CGColorRef shadowColor = ABI32_0_0RCTCGColorRefFromSharedColor(newViewProps.shadowColor);
  ...
  if (oldViewProps.shadowOffset != newViewProps.shadowOffset) {
    self.layer.shadowOffset = ABI32_0_0RCTCGSizeFromSize(newViewProps.shadowOffset);
  }
  ...
  if (oldViewProps.transform != newViewProps.transform) {
    self.layer.transform = ABI32_0_0RCTCATransform3DFromTransformMatrix(newViewProps.transform);
    self.layer.allowsEdgeAntialiasing = newViewProps.transform != Transform::Identity();
  }

  // `hitSlop`
  if (oldViewProps.hitSlop != newViewProps.hitSlop) {
    self.hitTestEdgeInsets = ABI32_0_0RCTUIEdgeInsetsFromEdgeInsets(newViewProps.hitSlop);
  }

  // `overflow`
  if (oldViewProps.yogaStyle.overflow != newViewProps.yogaStyle.overflow) {
    self.clipsToBounds = newViewProps.yogaStyle.overflow != ABI32_0_0YGOverflowVisible;
  }
  ...
  if (oldViewProps.nativeId != newViewProps.nativeId) {
    self.nativeId = ABI32_0_0RCTNSStringFromString(newViewProps.nativeId);
  }
...
- (void)invalidateBorder
{
  ...
  if (useCoreAnimationBorderRendering) {
    ...
    layer.borderColor = ABI32_0_0RCTCGColorRefFromSharedColor(props.borderColor);
...
- (NSArray<UIAccessibilityCustomAction *> *)accessibilityCustomActions
{
  ...
  for (const auto &accessibilityAction : accessibilityProps.accessibilityActions) {
    [customActions addObject:[[UIAccessibilityCustomAction alloc] initWithName:ABI32_0_0RCTNSStringFromString(accessibilityAction)
                                                                        target:self
...
{
  _eventEmitter->onAccessibilityAction(ABI32_0_0RCTStringFromNSString(action.name));
  return YES;
}
