...
@implementation ABI32_0_0RCTConvert (ARPlaneDetection)

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 110300
ABI32_0_0RCT_ENUM_CONVERTER(ARPlaneDetection, (@{
...
@implementation ABI32_0_0RCTConvert (ARHitTestResultType)

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 110300
ABI32_0_0RCT_ENUM_CONVERTER(ARHitTestResultType, (@{
...
@implementation ABI32_0_0RCTConvert (ARWorldAlignment)

ABI32_0_0RCT_ENUM_CONVERTER(ARWorldAlignment, (@{
...
@interface ABI32_0_0EXGLARSessionManager () <ARSessionObserver, ARSessionDelegate>
{
...
@property (nonatomic, weak) ABI32_0_0EXGLView *glView;
...
@implementation ABI32_0_0EXGLARSessionManager
  ...
  if (@available(iOS 11.0, *)) {
    if ([anchor isKindOfClass:[ARAnchor class]]) {
      ...
      NSDictionary *anchorData = @{
                                   @"type": type,
                                   @"transform": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix:_anchor.transform],
      ...
      if ([anchor isKindOfClass:[ARPlaneAnchor class]]) {
        ...
        NSDictionary *planeData = [ABI32_0_0EXGLARSessionManager nsDictionaryForPlaneAnchor:planeAnchor];
        [output addEntriesFromDictionary:planeData];
      } else if (@available(iOS 11.3, *)) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 110300
        if ([anchor isKindOfClass:[ARImageAnchor class]]) {
          ARImageAnchor *imageAnchor = (ARImageAnchor *)anchor;
          NSDictionary *imageData = [ABI32_0_0EXGLARSessionManager nsDictionaryForImageAnchor:imageAnchor];
          [output addEntriesFromDictionary:imageData];
        } else {
          NSDictionary *data = [ABI32_0_0EXGLARSessionManager
...
+ (NSDictionary *)nsDictionaryForPlaneAnchor:(ARPlaneAnchor *)anchor
{
  ...
  return @{
           @"center": [ABI32_0_0EXGLARSessionManager nsDictionaryForVecFloat3:center],
           @"extent": @{
               @"width": @(extent[0]),
               @"length": @(extent[2])
               },
...
+ (NSDictionary *)nsDictionaryForImageAnchor:(ARImageAnchor *)anchor
{
  return @{
           @"image": @{
               ...
               @"size": [ABI32_0_0EXGLARSessionManager nsDictionaryForSize: anchor.referenceImage.physicalSize],
               },
           };
}
  ...
  if (@available(iOS 11.0, *)) {
    ...
    if (hasGeometry) {
      ...
      for (int i = 0; i < [geometry vertexCount]; i++) {
        [vertices addObject:[ABI32_0_0EXGLARSessionManager nsDictionaryForVecFloat3:[geometry vertices][i]]];
      }
      
      NSMutableArray *textureCoordinates = [NSMutableArray new];
      for (int i = 0; i < [geometry triangleCount]; i++) {
        [textureCoordinates addObject:[ABI32_0_0EXGLARSessionManager nsDictionaryForVecFloat2:[geometry textureCoordinates][i]]];
      }
...
+ (NSArray *)nsArrayForAnchors:(NSArray *)anchors props:(NSDictionary *)props
{
  ...
  if (@available(iOS 11.0, *)) {
    for (ARAnchor *anchor in anchors) {
      NSDictionary *anchorData = [ABI32_0_0EXGLARSessionManager nsDictionaryForAnchor:anchor props:props];
...
- (NSDictionary *)startWithGLView:(ABI32_0_0EXGLView *)glView trackingConfiguration:(NSString *) trackingConfiguration
{
  ...
  if (!self.configuration) {
    return @{
             @"error": @"Invalid ABI32_0_0ARTrackingConfiguration, ARKit may not be available on this device.",
             };
  }
...
- (void)_maybeSetupCameraTextureWithWidth:(int)width height:(int)height
{
  ...
  if (err) {
    NSLog(@"ABI32_0_0EXGLARSessionManager: Error from CVOpenGLESTextureCacheCreate(...): %d", err);
  }
  _arCamYTex = NULL;
  _arCamCbCrTex = NULL;
  ...
  // Create camera texture output ABI32_0_0EXGLObject
...
- (NSDictionary *)arMatricesWithZNear:(CGFloat)zNear zFar:(CGFloat)zFar
{
  ...
  return @{
           @"transform": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix:transform],
           @"viewMatrix": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix:viewMat],
           @"projectionMatrix": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix:projMat],
           };
}
...
- (nullable NSError *)startConfiguration:(NSString *)configuration
{
  ...
  if (!self.configuration) {
    ...
    return [NSError errorWithDomain:@"ABI32_0_0EXGLARConfiguration" code:0 userInfo: userInfo];
  }
  [self.session runWithConfiguration:self.configuration];
  return nil;
}
  ...
  if (@available(iOS 11.0, *)) {
    ...
    for (ARHitTestResult *result in results) {
      [hitTest addObject:@{
                           ...
                           @"localTransform": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix:result.localTransform],
                           @"worldTransform": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix:result.worldTransform],
                           @"anchor": [ABI32_0_0EXGLARSessionManager nsDictionaryForAnchor:result.anchor props:@{}]
                           }];
    }
  }
  return @{
...
//      return @{
//               @"intrinsicMatrix": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix3x3:cameraCalibrationData.intrinsicMatrix],
//               @"intrinsicMatrixReferenceDimensions": [ABI32_0_0EXGLARSessionManager nsDictionaryForSize:cameraCalibrationData.intrinsicMatrixReferenceDimensions],
//               @"extrinsicMatrix": [ABI32_0_0EXGLARSessionManager nsArrayForMatrix4x3:cameraCalibrationData.extrinsicMatrix],
...
//               @"lensDistortionCenter": [ABI32_0_0EXGLARSessionManager nsDictionaryForPoint:cameraCalibrationData.lensDistortionCenter]
//               };
//    }
//  }
//
...
- (NSDictionary *)getCurrentFrameWithAttributes:(NSDictionary *)attributes
{
  ...
  if (@available(iOS 11.0, *)) {
    ...
    if (attributes[@"anchors"] != nil) {
      ...
      NSArray *anchors = [ABI32_0_0EXGLARSessionManager nsArrayForAnchors:self.session.currentFrame.anchors props:props];
                                               ...
                                               height:(int)height
{
  ...
  if (error != kCVReturnSuccess) {
    ...
    texture = nil;
    NSLog(@"ABI32_0_0EXGLARSessionManager: Error creating texture from pixel buffer. CVOpenGLESTextureCacheCreateTextureFromImage %d", error);
  }
...
- (void)_updateAnchors:(NSArray<ARAnchor *> *)anchors eventType:(NSString *)eventType
{
  NSDictionary *event = @{
                          @"eventType": eventType,
                          @"anchors": [ABI32_0_0EXGLARSessionManager nsArrayForAnchors:anchors props:@{}]
                          };
...
{
  [_delegate didUpdateWithEvent:@"DID_FAIL_WITH_ERROR" payload:ABI32_0_0RCTJSErrorFromNSError(error)];
}
...
- (void)session:(ARSession *)session cameraDidChangeTrackingState:(ARCamera *)camera
{
  NSArray *trackingStates = @[
                              @"ABI32_0_0ARTrackingStateNotAvailable",
                              @"ABI32_0_0ARTrackingStateLimited",
                              @"ABI32_0_0ARTrackingStateNormal"
                              ];
  NSArray *trackingStateReasons = @[
                                    @"ABI32_0_0ARTrackingStateReasonNone",
                                    @"ABI32_0_0ARTrackingStateReasonInitializing",
                                    @"ABI32_0_0ARTrackingStateReasonExcessiveMotion",
                                    @"ABI32_0_0ARTrackingStateReasonInsufficientFeatures",
                                    @"ABI32_0_0ARTrackingStateReasonRelocalizing"
                                    ];
