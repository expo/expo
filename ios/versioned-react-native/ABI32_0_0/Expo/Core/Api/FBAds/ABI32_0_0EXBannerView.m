...
@implementation ABI32_0_0EXBannerView
...
- (void)createViewIfCan
{
  ...
  FBAdView *adView = [[FBAdView alloc] initWithPlacementID:_placementId
                                                    adSize:fbAdSize
                                        rootViewController:ABI32_0_0RCTPresentedViewController()];
...
{
  if (_onAdError) {
    _onAdError(ABI32_0_0RCTJSErrorFromNSError(error));
  } else {
    ABI32_0_0RCTMakeAndLogError(error.localizedDescription, nil, error.userInfo);
  }
}
