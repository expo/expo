...
#import "ABI32_0_0EXMailComposer.h"
...
@interface ABI32_0_0EXMailComposer ()
...
@implementation ABI32_0_0EXMailComposer

ABI32_0_0RCT_EXPORT_MODULE(ExponentMailComposer);
...
ABI32_0_0RCT_EXPORT_METHOD(composeAsync:(NSDictionary *)options
                  ...
                  rejecter:(ABI32_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if (options[@"attachments"] != nil) {
    for (NSString *uri in options[@"attachments"]) {
      ...
      if (!([fileSystem permissionsForURI:url] & ABI32_0_0EXFileSystemPermissionRead)) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [ABI32_0_0RCTPresentedViewController() presentViewController:composeController animated:YES completion:nil];
  });
}
