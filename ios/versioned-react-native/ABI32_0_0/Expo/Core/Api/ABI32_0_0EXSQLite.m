...
@interface ABI32_0_0EXSQLite ()

@property (nonatomic, copy) NSMutableDictionary *cachedDatabases;

@end

@implementation ABI32_0_0EXSQLite
...
ABI32_0_0RCT_EXPORT_MODULE(ExponentSQLite)
...
- (NSString *)pathForDatabaseName:(NSString *)name
{
  ...
  if (!fileSystem) {
    ABI32_0_0RCTLogError(@"No FileSystem module.");
    return nil;
  }
...
ABI32_0_0RCT_EXPORT_METHOD(exec:(NSString *)dbName
...
ABI32_0_0RCT_EXPORT_METHOD(close:(NSString *)dbName)
{
  @synchronized(self) {
    [cachedDatabases removeObjectForKey:dbName];
  }
                 ...
                 withDb:(sqlite3*)db withReadOnly:(BOOL)readOnly
{
  ...
  if (sqlite3_prepare_v2(db, [sql UTF8String], -1, &statement, NULL) != SQLITE_OK) {
    error = [ABI32_0_0EXSQLite convertSQLiteErrorToString:db];
    return @[error];
  }
