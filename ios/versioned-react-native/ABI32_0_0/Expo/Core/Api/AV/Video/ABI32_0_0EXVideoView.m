...
#import "ABI32_0_0EXVideoPlayerViewController.h"

static NSString *const ABI32_0_0EXVideoReadyForDisplayKeyPath = @"readyForDisplay";
static NSString *const ABI32_0_0EXVideoSourceURIKeyPath = @"uri";
static NSString *const ABI32_0_0EXVideoBoundsKeyPath = @"videoBounds";
static NSString *const ABI32_0_0EXAVFullScreenViewControllerClassName = @"AVFullScreenViewController";

@interface ABI32_0_0EXVideoView ()

@property (nonatomic, weak) ABI32_0_0EXAV *exAV;

@property (nonatomic, assign) BOOL playerHasLoaded;
@property (nonatomic, strong) ABI32_0_0EXAVPlayerData *data;
@property (nonatomic, strong) AVPlayerLayer *playerLayer;
@property (nonatomic, strong) ABI32_0_0EXVideoPlayerViewController *playerViewController;
...
@property (nonatomic, strong) ABI32_0_0EXVideoPlayerViewController *fullscreenPlayerViewController;
@property (nonatomic, strong) ABI32_0_0RCTPromiseResolveBlock requestedFullscreenChangeResolver;
@property (nonatomic, strong) ABI32_0_0RCTPromiseRejectBlock requestedFullscreenChangeRejecter;
...
@implementation ABI32_0_0EXVideoView

#pragma mark - ABI32_0_0EXVideoView interface methods

- (instancetype)initWithBridge:(ABI32_0_0RCTBridge *)bridge
{
  if ((self = [super init])) {
    _exAV = [bridge moduleForClass:[ABI32_0_0EXAV class]];
...
- (void)_callFullscreenCallbackForUpdate:(ABI32_0_0EXVideoFullscreenUpdate)update
{
...
- (void)_tryUpdateDataStatus:(ABI32_0_0RCTPromiseResolveBlock)resolve
...
- (void)_removePlayer
{
  if (_requestedFullscreenChangeRejecter) {
    ...
    _requestedFullscreenChangeRejecter(@"E_VIDEO_FULLSCREEN", errorMessage, ABI32_0_0RCTErrorWithMessage(errorMessage));
    _requestedFullscreenChangeResolver = nil;
    _requestedFullscreenChangeRejecter = nil;
    _requestedFullscreenChange = NO;
  }
...
- (ABI32_0_0EXVideoPlayerViewController *)_createNewPlayerViewController
{
  if (_data == nil) {
    return nil;
  }
  ABI32_0_0EXVideoPlayerViewController *controller = [[ABI32_0_0EXVideoPlayerViewController alloc] init];
  ...
  [controller addObserver:self forKeyPath:ABI32_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
  return controller;
}
...
- (void)_usePlayerLayer
{
  if (_data) {
    ...
    [_playerLayer addObserver:self forKeyPath:ABI32_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)_removePlayerLayer
{
  if (_playerLayer) {
    ...
    [_playerLayer removeObserver:self forKeyPath:ABI32_0_0EXVideoReadyForDisplayKeyPath];
    _playerLayer = nil;
  }
}
...
- (void)_removeFullscreenPlayerViewController
{
  if (_fullscreenPlayerViewController) {
    [_fullscreenPlayerViewController removeObserver:self forKeyPath:ABI32_0_0EXVideoReadyForDisplayKeyPath];
    _fullscreenPlayerViewController = nil;
  }
}
...
- (void)_removePlayerViewController
{
  if (_playerViewController) {
    __weak ABI32_0_0EXVideoView *weakSelf = self;
    void (^block)(void) = ^ {
      __strong ABI32_0_0EXVideoView *strongSelf = weakSelf;
      if (strongSelf && strongSelf.playerViewController) {
        [strongSelf.playerViewController.view removeFromSuperview];
        [strongSelf.playerViewController removeObserver:strongSelf forKeyPath:ABI32_0_0EXVideoReadyForDisplayKeyPath];
        [strongSelf.playerViewController removeObserver:strongSelf forKeyPath:ABI32_0_0EXVideoBoundsKeyPath];
        strongSelf.playerViewController = nil;
      }
    };

    [ABI32_0_0EXUtil performSynchronouslyOnMainThread:block];
  }
}
...
{
  if ((object == _playerLayer || object == _playerViewController || object == _fullscreenPlayerViewController) && [keyPath isEqualToString:ABI32_0_0EXVideoReadyForDisplayKeyPath]) {
  ...
  } else if (object == _playerViewController && [keyPath isEqualToString:ABI32_0_0EXVideoBoundsKeyPath]) {
    if (ABI32_0_0RCTPresentedViewController() == nil) {
      return;
    }
    ...
    if ([[ABI32_0_0RCTPresentedViewController().class description] isEqualToString:ABI32_0_0EXAVFullScreenViewControllerClassName]) {
      // ABI32_0_0RCTPresentedViewController() is fullscreen
       fullscreenViewController = ABI32_0_0RCTPresentedViewController();
    } else if (ABI32_0_0RCTPresentedViewController().presentedViewController != nil && [[ABI32_0_0RCTPresentedViewController().presentedViewController.class description] isEqualToString:ABI32_0_0EXAVFullScreenViewControllerClassName]) {
      // ABI32_0_0RCTPresentedViewController().presentedViewController is fullscreen
      fullscreenViewController = ABI32_0_0RCTPresentedViewController().presentedViewController;
    }
    ...
    if (fullscreenViewController.isBeingDismissed && _fullscreenPlayerPresented && _nativeFullscreenPlayerViewController == fullscreenViewController) {
      ...
      _nativeFullscreenPlayerViewController = nil;
      [self _callFullscreenCallbackForUpdate:ABI32_0_0EXVideoFullscreenUpdatePlayerWillDismiss];
      [self _callFullscreenCallbackForUpdate:ABI32_0_0EXVideoFullscreenUpdatePlayerDidDismiss];
      ...
      [self _callFullscreenCallbackForUpdate:ABI32_0_0EXVideoFullscreenUpdatePlayerWillPresent];
      [self _callFullscreenCallbackForUpdate:ABI32_0_0EXVideoFullscreenUpdatePlayerDidPresent];
    } else {
      return;
    }
  } else {
         ...
         rejecter:(ABI32_0_0RCTPromiseRejectBlock)reject
{
  ...
  _data = [[ABI32_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                                    withSource:source
                           ...
                           withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                             ...
                             if (strongSelf && success) {
                               ...
                               [strongSelf _removePlayer];
                               if (reject) {
                                 reject(@"E_VIDEO_NOTCREATED", error, ABI32_0_0RCTErrorWithMessage(error));
                               }
                               [strongSelf _callErrorCallback:error];
                             }
                           }];
             ...
             rejecter:(ABI32_0_0RCTPromiseRejectBlock)reject
{
  if (!_data) {
    ...
    if (reject) {
      ...
      reject(@"E_VIDEO_FULLSCREEN", errorMessage, ABI32_0_0RCTErrorWithMessage(errorMessage));
    }
    return;
    ...
    if (value && !_fullscreenPlayerPresented && !_fullscreenPlayerViewController) {
      ...
      _presentingViewController = ABI32_0_0RCTPresentedViewController();
      [self _callFullscreenCallbackForUpdate:ABI32_0_0EXVideoFullscreenUpdatePlayerWillPresent];
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (strongSelf) {
          ...
          [strongSelf.presentingViewController presentViewController:strongSelf.fullscreenPlayerViewController animated:YES completion:^{
            __strong ABI32_0_0EXVideoView *strongSelfInner = weakSelf;
            if (strongSelfInner) {
              strongSelfInner.fullscreenPlayerPresented = YES;
              [strongSelfInner _callFullscreenCallbackForUpdate:ABI32_0_0EXVideoFullscreenUpdatePlayerDidPresent];
              if (resolve) {
...
{
  return @{
           ABI32_0_0EXVideoSourceURIKeyPath: (_data != nil && _data.url != nil) ? _data.url.absoluteString : @""
           };
}
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (strongSelf && strongSelf.useNativeControls) {
      ...
      if (!strongSelf.playerViewController && strongSelf.data) {
        ...
        [strongSelf.playerViewController addObserver:self forKeyPath:ABI32_0_0EXVideoBoundsKeyPath options:NSKeyValueObservingOptionNew context:nil];
        // Resize mode must be set before layer is added
        // to prevent video from being animated when `resizeMode` is `cover`
...
#pragma mark - ReactABI32_0_0 View Management

- (void)insertReactABI32_0_0Subview:(UIView *)view atIndex:(NSInteger)atIndex
{
  ...
  if (_useNativeControls && _playerViewController) {
    [super insertReactABI32_0_0Subview:view atIndex:atIndex];
    [view setFrame:self.bounds];
    [_playerViewController.contentOverlayView insertSubview:view atIndex:atIndex];
  } else {
    ABI32_0_0RCTLogError(@"video cannot have any subviews");
  }
}
...
#pragma mark - ABI32_0_0EXVideoPlayerViewControllerDelegate
...
{
  return _data == nil ? ABI32_0_0EXAVAudioSessionModeInactive : [_data getAudioSessionModeRequired];
}
