...
+ (CGFloat*)ABI32_0_0RNSVGCGFloatArray:(id)json
{
...
+ (ABI32_0_0RNSVGBrush *)ABI32_0_0RNSVGBrush:(id)json
{
    if ([json isKindOfClass:[NSString class]]) {
        ...
        if (!ABI32_0_0RNSVGDigitRegEx) {
            ABI32_0_0RNSVGDigitRegEx = [NSRegularExpression regularExpressionWithPattern:@"[0-9.-]+" options:NSRegularExpressionCaseInsensitive error:nil];
        }
        NSArray<NSTextCheckingResult*> *_matches = [ABI32_0_0RNSVGDigitRegEx matchesInString:value options:0 range:NSMakeRange(0, [value length])];
        ...
        return [[ABI32_0_0RNSVGSolidColorBrush alloc] initWithArray:output];
    }
    ...
    switch (type) {
        case 0: // solid color
            ...
            return [[ABI32_0_0RNSVGSolidColorBrush alloc] initWithArray:arr];
        case 1: // brush
            return [[ABI32_0_0RNSVGPainterBrush alloc] initWithArray:arr];
        case 2: // currentColor
            return [[ABI32_0_0RNSVGBrush alloc] initWithArray:nil];
        default:
            ABI32_0_0RCTLogError(@"Unknown brush type: %zd", (unsigned long)type);
            return nil;
    }
}
...
{
    return [[ABI32_0_0RNSVGPathParser alloc] initWithPathString: d];
}

+ (ABI32_0_0RNSVGLength *)ABI32_0_0RNSVGLength:(id)json
{
    if ([json isKindOfClass:[NSNumber class]]) {
        return [ABI32_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
    } else if ([json isKindOfClass:[NSString class]]) {
        NSString *stringValue = (NSString *)json;
        return [ABI32_0_0RNSVGLength lengthWithString:stringValue];
    } else {
        return [[ABI32_0_0RNSVGLength alloc] init];
    }
}
...
{
    if ([json isKindOfClass:[NSNumber class]]) {
        ABI32_0_0RNSVGLength* length = [ABI32_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
        return [NSArray arrayWithObject:length];
    } else if ([json isKindOfClass:[NSArray class]]) {
        NSArray *arrayValue = (NSArray*)json;
        NSMutableArray<ABI32_0_0RNSVGLength*>* lengths = [NSMutableArray arrayWithCapacity:[arrayValue count]];
        for (id obj in arrayValue) {
            [lengths addObject:[self ABI32_0_0RNSVGLength:obj]];
        }
        return lengths;
    }  else if ([json isKindOfClass:[NSString class]]) {
        NSString *stringValue = (NSString *)json;
        ABI32_0_0RNSVGLength* length = [ABI32_0_0RNSVGLength lengthWithString:stringValue];
        return [NSArray arrayWithObject:length];
    } else {
        return nil;
    }
...
+ (CGRect)ABI32_0_0RNSVGCGRect:(id)json offset:(NSUInteger)offset
{
    ...
    if (arr.count < offset + 4) {
        ABI32_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", (ssize_t)(4 + offset), arr);
        return CGRectZero;
    }
    return (CGRect){
...
+ (CGGradientRef)ABI32_0_0RNSVGCGGradient:(id)json offset:(NSUInteger)offset
{
    NSArray *arr = [self NSArray:json];
    if (arr.count < offset) {
        ABI32_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", (unsigned long)offset, arr);
        return nil;
    }
    arr = [arr subarrayWithRange:(NSRange){offset, arr.count - offset}];
    CGFloat* colorsAndOffsets = [self ABI32_0_0RNSVGCGFloatArray:arr];
