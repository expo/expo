...
#import "ABI32_0_0RNSVGGlyphContext.h"

@interface ABI32_0_0RNSVGNode()
@property (nonatomic, readwrite, weak) ABI32_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI32_0_0RNSVGGroup *textRoot;
@end

@implementation ABI32_0_0RNSVGNode
{
    ABI32_0_0RNSVGGlyphContext *glyphContext;
    BOOL _transparent;
    ABI32_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI32_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI32_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)invalidate
{
    id<ABI32_0_0RNSVGContainer> container = (id<ABI32_0_0RNSVGContainer>)self.superview;
...
- (ABI32_0_0RNSVGGroup *)textRoot
{
    if (_textRoot) {
        return _textRoot;
    }

    ABI32_0_0RNSVGNode* node = self;
    while (node != nil) {
        if ([node isKindOfClass:[ABI32_0_0RNSVGGroup class]] && [((ABI32_0_0RNSVGGroup*) node) getGlyphContext] != nil) {
            _textRoot = (ABI32_0_0RNSVGGroup*)node;
            break;
        }
        ...
        if (![node isKindOfClass:[ABI32_0_0RNSVGNode class]]) {
            ...
            node = (ABI32_0_0RNSVGNode*)parent;
        }
    }
...
{
    ABI32_0_0RNSVGNode* parent = (ABI32_0_0RNSVGGroup*)[self superview];
    if (![parent isKindOfClass:[ABI32_0_0RNSVGGroup class]]) {
        return nil;
    } else {
...
{
    ABI32_0_0RNSVGGroup* root = self.textRoot;
    if (root == nil) {
        return ABI32_0_0RNSVG_DEFAULT_FONT_SIZE;
    }
...
- (void)setClipRule:(ABI32_0_0RNSVGCGFCRule)clipRule
{
...
{
    if (self.clipPath) {
        _clipNode = (ABI32_0_0RNSVGClipPath*)[self.svgView getDefinedClipPath:self.clipPath];
...
- (ABI32_0_0RNSVGSvgView *)svgView
{
    ...
    if ([parent class] == [ABI32_0_0RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[ABI32_0_0RNSVGNode class]]) {
        _svgView = ((ABI32_0_0RNSVGNode *)parent).svgView;
    } else {
        ABI32_0_0RCTLogError(@"ABI32_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
    }
...
{
    return [ABI32_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
{
    ABI32_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)getContextDiagonal {
    ...
    CGFloat r = sqrt(powX + powY) * ABI32_0_0RNSVG_M_SQRT1_2l;
    return r;
}
...
- (CGFloat) getCanvasDiagonal {
    ...
    CGFloat powX = width * width;
    CGFloat powY = height * height;
    canvasDiagonal = sqrt(powX + powY) * ABI32_0_0RNSVG_M_SQRT1_2l;
    return canvasDiagonal;
}
...
{
    [self clearPath];
    [self traverseSubviews:^BOOL(__kindof ABI32_0_0RNSVGNode *node) {
        if ([node isKindOfClass:[ABI32_0_0RNSVGNode class]]) {
