...
#import "ABI32_0_0EXDocumentPicker.h"
...
static NSString * ABI32_0_0EXConvertMimeTypeToUTI(NSString *mimeType)
{
  CFStringRef uti;
...
@interface ABI32_0_0EXDocumentPicker () <UIDocumentMenuDelegate, UIDocumentPickerDelegate>

@property (nonatomic, strong) ABI32_0_0RCTPromiseResolveBlock resolve;
@property (nonatomic, strong) ABI32_0_0RCTPromiseRejectBlock reject;

@property (nonatomic, assign) BOOL shouldCopyToCacheDirectory;

@end

@implementation ABI32_0_0EXDocumentPicker

@synthesize bridge = _bridge;

ABI32_0_0RCT_EXPORT_MODULE(ExponentDocumentPicker)

ABI32_0_0RCT_EXPORT_METHOD(getDocumentAsync:(NSDictionary *)options resolver:(ABI32_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI32_0_0RCTPromiseRejectBlock)reject) {
  _resolve = resolve;
  _reject = reject;

  NSString *type = ABI32_0_0EXConvertMimeTypeToUTI(options[@"type"] ?: @"*/*");

  if (options[@"copyToCacheDirectory"] && [ABI32_0_0RCTConvert BOOL:options[@"copyToCacheDirectory"]] == NO) {
  ...
  if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
    documentMenuVC.popoverPresentationController.sourceRect = CGRectMake(CGRectGetMidX([ABI32_0_0RCTPresentedViewController().view frame]), CGRectGetMaxY([ABI32_0_0RCTPresentedViewController().view frame]), 0, 0);
    documentMenuVC.popoverPresentationController.sourceView = ABI32_0_0RCTPresentedViewController().view;
    documentMenuVC.modalPresentationStyle = UIModalPresentationPageSheet;
  }

  id<ABI32_0_0EXUtilitiesInterface> utils = [_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXUtilitiesInterface)];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url
{
  ...
  if (_shouldCopyToCacheDirectory) {
    id<ABI32_0_0EXFileSystemInterface> fileSystem = [_bridge.scopedModules.moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXFileSystemInterface)];
    if (!fileSystem) {
