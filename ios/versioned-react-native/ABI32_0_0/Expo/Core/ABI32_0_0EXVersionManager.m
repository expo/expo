...
#import "ABI32_0_0EXDisabledDevLoadingView.h"
#import "ABI32_0_0EXDisabledDevMenu.h"
#import "ABI32_0_0EXDisabledRedBox.h"
#import "ABI32_0_0EXFileSystem.h"
#import "ABI32_0_0EXVersionManager.h"
#import "ABI32_0_0EXScopedBridgeModule.h"
#import "ABI32_0_0EXStatusBarManager.h"
...
#import <ReactABI32_0_0/ABI32_0_0RCTExceptionsManager.h>
...
#import <ReactABI32_0_0/ABI32_0_0RCTAsyncLocalStorage.h>
...
#import "ABI32_0_0EXScopedModuleRegistryDelegate.h"

// used for initializing scoped modules which don't tie in to any kernel service.
#define ABI32_0_0EX_KERNEL_SERVICE_NONE @"ABI32_0_0EXKernelServiceNone"

// this is needed because ABI32_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI32_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSDictionary *> *ABI32_0_0EXScopedModuleClasses;
void ABI32_0_0EXRegisterScopedModule(Class, ...);
void ABI32_0_0EXRegisterScopedModule(Class moduleClass, ...)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI32_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  ...
  va_start(argumentList, moduleClass);
    while ((kernelServiceClassName = va_arg(argumentList, NSString*))) {
      if ([kernelServiceClassName isEqualToString:@"nil"]) {
        unversionedKernelServiceClassNames[kernelServiceClassName] = ABI32_0_0EX_KERNEL_SERVICE_NONE;
      } else {
  ...
  if (moduleClassName) {
    ABI32_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassNames;
  }
}

@interface ABI32_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI32_0_0EXVersionManager ()
...
@implementation ABI32_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI32_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI32_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI32_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
#ifdef ABI32_0_0EX_ENABLE_UNSAFE_SYSTRACE
    items[@"dev-profiler"] = @{
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI32_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI32_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI32_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return ([ABI32_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI32_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI32_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI32_0_0RCTSetFatalHandler(fatalHandler);
  ABI32_0_0RCTSetLogThreshold(threshold);
 ...
 *    @ABI32_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI32_0_0EXKernel *kernel
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSString *localStorageDirectory = [[ABI32_0_0EXFileSystem documentDirectoryForExperienceId:experienceId] stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];
  BOOL isOpeningHomeInProductionMode = params[@"browserModuleClass"] && !manifest[@"developer"];

  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI32_0_0EXAppState alloc] init],
                                    [[ABI32_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:(!isOpeningHomeInProductionMode && isDeveloper)],
                                    [[ABI32_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI32_0_0EXStatusBarManager alloc] init],
                                    [[ABI32_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory],
                                    ]];
  ...
  if (exceptionsManagerDelegate) {
    ABI32_0_0RCTExceptionsManager *exceptionsManager = [[ABI32_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
    [extraModules addObject:exceptionsManager];
  } else {
    ABI32_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
  }
  
  if (params[@"testEnvironment"]) {
    ABI32_0_0EXTestEnvironment testEnvironment = (ABI32_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI32_0_0EXTestEnvironmentNone) {
      ABI32_0_0EXTest *testModule = [[ABI32_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI32_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI32_0_0RCTDevMenu
    [extraModules addObject:[[ABI32_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI32_0_0RCTRedBox
    [extraModules addObject:[[ABI32_0_0EXDisabledRedBox alloc] init]];
  }

  ABI32_0_0EXModuleRegistryProvider *moduleRegistryProvider = [[ABI32_0_0EXModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI32_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI32_0_0EXModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

  ABI32_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI32_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];

  NSArray<id<ABI32_0_0RCTBridgeModule>> *expoModules = [moduleRegistryAdapter extraModulesForParams:params andExperience:experienceId withScopedModulesArray:extraModules withKernelServices:services];
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI32_0_0EXScopedModuleClasses) {
    [ABI32_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI32_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
