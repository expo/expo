...
#import "ABI32_0_0RCTFontUtils.h"

#import <cmath>
#import <mutex>

static ABI32_0_0RCTFontProperties ABI32_0_0RCTDefaultFontProperties() {
  static ABI32_0_0RCTFontProperties defaultFontProperties;
  ...
  dispatch_once(&onceToken, ^{
    ...
    defaultFontProperties.style = ABI32_0_0RCTFontStyleNormal;
    defaultFontProperties.variant = ABI32_0_0RCTFontVariantDefault;
...
static ABI32_0_0RCTFontProperties ABI32_0_0RCTResolveFontProperties(ABI32_0_0RCTFontProperties fontProperties) {
  ABI32_0_0RCTFontProperties defaultFontProperties = ABI32_0_0RCTDefaultFontProperties();
  ...
  fontProperties.style = fontProperties.style != ABI32_0_0RCTFontStyleUndefined  ? fontProperties.style : defaultFontProperties.style;
  fontProperties.variant = fontProperties.variant != ABI32_0_0RCTFontVariantUndefined  ? fontProperties.variant : defaultFontProperties.variant;
  return fontProperties;
}

static UIFontWeight ABI32_0_0RCTGetFontWeight(UIFont *font) {
  NSDictionary *traits = [font.fontDescriptor objectForKey:UIFontDescriptorTraitsAttribute];
  return [traits[UIFontWeightTrait] doubleValue];
}

static ABI32_0_0RCTFontStyle ABI32_0_0RCTGetFontStyle(UIFont *font) {
  NSDictionary *traits = [font.fontDescriptor objectForKey:UIFontDescriptorTraitsAttribute];
  ...
  if (symbolicTraits & UIFontDescriptorTraitItalic) {
    return ABI32_0_0RCTFontStyleItalic;
  }

  return ABI32_0_0RCTFontStyleNormal;
}

static NSArray *ABI32_0_0RCTFontFeatures(ABI32_0_0RCTFontVariant fontVariant) {
  // FIXME:
  return @[];
}

static UIFontWeight ABI32_0_0RCTUIFontWeightFromFloat(CGFloat fontWeight) {
...
static UIFont *ABI32_0_0RCTDefaultFontWithFontProperties(ABI32_0_0RCTFontProperties fontProperties) {
  ...
  if (!font) {
    font = [UIFont systemFontOfSize:fontProperties.size
                             weight:ABI32_0_0RCTUIFontWeightFromFloat(fontProperties.weight)];

    if (fontProperties.variant == ABI32_0_0RCTFontStyleItalic) {
...
UIFont *ABI32_0_0RCTFontWithFontProperties(ABI32_0_0RCTFontProperties fontProperties) {
  ABI32_0_0RCTFontProperties defaultFontProperties = ABI32_0_0RCTDefaultFontProperties();
  fontProperties = ABI32_0_0RCTResolveFontProperties(fontProperties);
  ...
  if ([fontProperties.family isEqualToString:defaultFontProperties.family]) {
    ...
    font = ABI32_0_0RCTDefaultFontWithFontProperties(fontProperties);
  } else {
    ...
    if (fontNames.count == 0) {
      ...
      if (!font) {
        ...
        font = [UIFont systemFontOfSize:effectiveFontSize weight:ABI32_0_0RCTUIFontWeightFromFloat(fontProperties.weight)];
      }
    } else {
      ...
      for (NSString *name in fontNames) {
        ...
        if (ABI32_0_0RCTGetFontStyle(fontMatch) != fontProperties.style) {
          continue;
        }

        CGFloat testWeight = ABI32_0_0RCTGetFontWeight(fontMatch);
  ...
  if (fontProperties.variant != ABI32_0_0RCTFontVariantDefault) {
    NSArray *fontFeatures = ABI32_0_0RCTFontFeatures(fontProperties.variant);
