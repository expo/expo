...
TEST(JSCExecutor, CallFunction) {
  ...
  ABI32_0_0EXPECT_EQ(11, returnedCall.moduleId);
  ABI32_0_0EXPECT_EQ(10, returnedCall.methodId);
  ASSERT_EQ(4, returnedCall.arguments.size());
  ABI32_0_0EXPECT_EQ(args[0], returnedCall.arguments[0]);
  ABI32_0_0EXPECT_EQ(args[1], returnedCall.arguments[1]);
  ABI32_0_0EXPECT_EQ(args[2], returnedCall.arguments[2]);
  ABI32_0_0EXPECT_EQ(folly::dynamic(4.0), returnedCall.arguments[3]);
}
...
TEST(JSCExecutor, CallFunctionWithMap) {
  ...
  ABI32_0_0EXPECT_EQ("hello4true", returnedCall.arguments[0].getString());
}
...
TEST(JSCExecutor, CallFunctionReturningMap) {
  ...
  ABI32_0_0EXPECT_EQ(folly::dynamic(4.0), foo);
  auto bar = returnedMap.at("bar");
  ABI32_0_0EXPECT_EQ(folly::dynamic(true), bar);
}
...
TEST(JSCExecutor, CallFunctionWithArray) {
  ...
  auto returnedCall = returnedCalls[0];
  ASSERT_EQ(1, returnedCall.arguments.size());
  ABI32_0_0EXPECT_EQ("hello4true3", returnedCall.arguments[0].getString());
}
...
TEST(JSCExecutor, CallFunctionReturningNumberArray) {
  ...
  ABI32_0_0EXPECT_EQ(3, array.size());
  ABI32_0_0EXPECT_EQ(folly::dynamic(3.0), array[0]);
  ABI32_0_0EXPECT_EQ(folly::dynamic(4.0), array[2]);
}
