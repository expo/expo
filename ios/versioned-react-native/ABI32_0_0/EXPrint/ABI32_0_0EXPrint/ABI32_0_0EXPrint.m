...
#import <ABI32_0_0EXPrint/ABI32_0_0EXPrint.h>
#import <ABI32_0_0EXPrint/ABI32_0_0EXPrintPDFRenderTask.h>
...
NSString *const ABI32_0_0EXPrintOrientationPortrait = @"portrait";
NSString *const ABI32_0_0EXPrintOrientationLandscape = @"landscape";

@interface ABI32_0_0EXPrint () <UIPrintInteractionControllerDelegate, UIPrinterPickerControllerDelegate>
...
@implementation ABI32_0_0EXPrint

ABI32_0_0EX_EXPORT_MODULE(ExponentPrint);
...
- (NSDictionary *)constantsToExport
{
  return @{
           @"Orientation": @{
               @"portrait": ABI32_0_0EXPrintOrientationPortrait,
               @"landscape": ABI32_0_0EXPrintOrientationLandscape,
               },
           };
}

ABI32_0_0EX_EXPORT_METHOD_AS(print,
                    ...
                    rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  [self _getPrintingDataForOptions:options callback:^(NSData *printingData, NSDictionary *errorDetails) {
    if (errorDetails != nil) {
      reject(errorDetails[@"code"], errorDetails[@"message"], ABI32_0_0EXErrorWithMessage(errorDetails[@"message"]));
      return;
    }
    ...
    if (printingData == nil) {
      ...
      if (options[@"markupFormatterIOS"] && [options[@"markupFormatterIOS"] isKindOfClass:[NSString class]]) {
        ...
        if (htmlString != nil) {
          ...
          reject(@"E_HTML_INVALID", message, ABI32_0_0EXErrorWithMessage(message));
          return;
        }
      } else {
...
ABI32_0_0EX_EXPORT_METHOD_AS(selectPrinter,selectPrinter:(ABI32_0_0EXPromiseResolveBlock)resolve
                  rejecter:(ABI32_0_0EXPromiseRejectBlock)reject)
{
...
ABI32_0_0EX_EXPORT_METHOD_AS(printToFileAsync,
                    ...
                    reject:(ABI32_0_0EXPromiseRejectBlock)reject)
{
  ...
  ABI32_0_0EXPrintPDFRenderTask *renderTask = [ABI32_0_0EXPrintPDFRenderTask new];
...
{
  id<ABI32_0_0EXUtilitiesInterface> utils = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXUtilitiesInterface)];
...
- (void)_getPrintingDataForOptions:(nonnull NSDictionary *)options callback:(void(^)(NSData *, NSDictionary *))callback
{
  ...
  if (options[@"html"]) {
    __block ABI32_0_0EXPrintPDFRenderTask *renderTask = [ABI32_0_0EXPrintPDFRenderTask new];
...
{
  if ([orientation isEqualToString:ABI32_0_0EXPrintOrientationLandscape]) {
...
{
  id<ABI32_0_0EXFileSystemInterface> fileSystem = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI32_0_0EXFileSystemInterface)];
  if (!fileSystem) {
    return nil;
  }
