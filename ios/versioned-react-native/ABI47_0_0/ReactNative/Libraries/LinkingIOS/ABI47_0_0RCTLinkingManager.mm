...
#import <ABI47_0_0React/ABI47_0_0RCTLinkingManager.h>
...
#import "ABI47_0_0RCTLinkingPlugins.h"

static NSString *const kOpenURLNotification = @"ABI47_0_0RCTOpenURLNotification";
...
@interface ABI47_0_0RCTLinkingManager() <ABI47_0_0NativeLinkingManagerSpec>
@end

@implementation ABI47_0_0RCTLinkingManager
    ...
    if (success) {
      resolve(@YES);
        ...
        if([URL.absoluteString hasPrefix:@"tel:"]){
          ABI47_0_0RCTLogWarn(@"Unable to open the Phone app in the simulator for telephone URLs. URL:  %@", URL);
          resolve(@NO);
        } else {
                  ...
                  reject:(__unused ABI47_0_0RCTPromiseRejectBlock)reject)
{
  ...
  BOOL canOpen = [ABI47_0_0RCTSharedApplication() canOpenURL:URL];
  ...
  if (canOpen) {
    ...
    if (querySchemes != nil && ([querySchemes containsObject:scheme] || [querySchemes containsObject:[scheme lowercaseString]])) {
      ...
      reject(ABI47_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@. Add %@ to LSApplicationQueriesSchemes in your Info.plist.", URL, scheme], nil);
    }
  } else {
    resolve(@NO);
  }
                  ...
                  reject:(__unused ABI47_0_0RCTPromiseRejectBlock)reject)
{
  ...
  resolve(ABI47_0_0RCTNullIfNil(initialURL.absoluteString));
}
...
ABI47_0_0RCT_EXPORT_METHOD(sendIntent:(NSString *)action
...
{
  return std::make_shared<ABI47_0_0facebook::ABI47_0_0React::NativeLinkingManagerSpecJSI>(params);
}

@end

Class ABI47_0_0RCTLinkingManagerCls(void) {
  return ABI47_0_0RCTLinkingManager.class;
}
