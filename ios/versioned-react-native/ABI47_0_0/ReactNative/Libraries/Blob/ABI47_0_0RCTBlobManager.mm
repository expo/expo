...
@interface ABI47_0_0RCTBlobManager () <ABI47_0_0RCTNetworkingRequestHandler, ABI47_0_0RCTNetworkingResponseHandler, ABI47_0_0RCTWebSocketContentHandler, ABI47_0_0NativeBlobModuleSpec>

@end

@implementation ABI47_0_0RCTBlobManager
{
...
ABI47_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)initialize
{
  ...
  ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI47_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI47_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI47_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI47_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
  ABI47_0_0RCTNetworking *const networking = [_moduleRegistry moduleForName:"Networking"];
...
ABI47_0_0RCT_EXPORT_METHOD(addWebSocketHandler:(double)socketID)
{
  dispatch_async(((ABI47_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
    [[self->_moduleRegistry moduleForName:"WebSocketModule"] setContentHandler:self forSocketID:[NSNumber numberWithDouble:socketID]];
  });
}

ABI47_0_0RCT_EXPORT_METHOD(removeWebSocketHandler:(double)socketID)
{
  dispatch_async(((ABI47_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
...
ABI47_0_0RCT_EXPORT_METHOD(sendOverSocket:(NSDictionary *)blob socketID:(double)socketID)
{
  dispatch_async(((ABI47_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
...
ABI47_0_0RCT_EXPORT_METHOD(createFromParts:(NSArray<NSDictionary<NSString *, id> *> *)parts withId:(NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI47_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI47_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI47_0_0RCT_EXPORT_METHOD(release:(NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI47_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI47_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI47_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI47_0_0RCTConvert NSString:ABI47_0_0RCTNilIfNull(blob[@"type"])];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset": @0,
    @"size": @(data.length),
    @"name": ABI47_0_0RCTNullIfNil([response suggestedFilename]),
    @"type": ABI47_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI47_0_0RCTWebSocketContentHandler methods
...
{
  return std::make_shared<ABI47_0_0facebook::ABI47_0_0React::NativeBlobModuleSpecJSI>(params);
}

@end

Class ABI47_0_0RCTBlobManagerCls(void) {
  return ABI47_0_0RCTBlobManager.class;
}
