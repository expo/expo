...
@implementation ABI47_0_0RCTWrapperShadowView
{
  __weak ABI47_0_0RCTBridge *_bridge;
  ABI47_0_0RCTWrapperMeasureBlock _measureBlock;
...
- (instancetype)initWithBridge:(ABI47_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
    ABI47_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI47_0_0RCTWrapperShadowViewMeasure);
  }

  return self;
}

static ABI47_0_0YGSize ABI47_0_0RCTWrapperShadowViewMeasure(ABI47_0_0YGNodeRef node, float width, ABI47_0_0YGMeasureMode widthMode, float height, ABI47_0_0YGMeasureMode heightMode)
{
  ...
  ABI47_0_0RCTWrapperShadowView *shadowView = (__bridge ABI47_0_0RCTWrapperShadowView *)ABI47_0_0YGNodeGetContext(node);
...
- (CGSize)measureWithMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  if (!_measureBlock) {
    ...
    __block ABI47_0_0RCTWrapperMeasureBlock measureBlock;
    NSNumber *ABI47_0_0ReactTag = self.ABI47_0_0ReactTag;

    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);

    dispatch_async(dispatch_get_main_queue(), ^{
      ABI47_0_0RCTUIManager *uiManager = bridge.uiManager;
      ABI47_0_0RCTWrapperView *view = (ABI47_0_0RCTWrapperView *)[uiManager viewForABI47_0_0ReactTag:ABI47_0_0ReactTag];
    ...
    if (dispatch_semaphore_wait(semaphore, timeout)) {
      ABI47_0_0RCTLogError(@"Unable to retrieve `measureBlock` for view (%@) because the main thread is busy.", self);
    }
  ...
  if (dispatch_semaphore_wait(semaphore, timeout)) {
    ABI47_0_0RCTLogError(@"Unable to compute layout for view (%@) because the main thread is busy.", self);
  }
