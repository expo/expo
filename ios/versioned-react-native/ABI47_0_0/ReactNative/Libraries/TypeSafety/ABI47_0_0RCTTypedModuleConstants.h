...
/**
 * Interop type layer for ABI47_0_0RN's exportedConstants and the C++ structs generated by our typesafety codegen.
 *
 * The ABI47_0_0NativeModuleSpec will define a constantsToExport method which you can implement as follows:
 *
 * - (nonnull ModuleConstants<ABI47_0_0JS::Constants>)constantsToExport
 * {
 *   return typedConstants<ABI47_0_0JS::Constants>({ ... });
 * }
 */

// Internal container for module constants. Do not use yourself directly, instead use the typedConstants helpers below.
@interface _ABI47_0_0RCTTypedModuleConstants<StructType> : NSDictionary<NSString *, id>
...
template<typename T>
using ModuleConstants = _ABI47_0_0RCTTypedModuleConstants<void (^)(T)> *;

template<typename T>
ModuleConstants<T> typedConstants(typename T::Builder::Input &&value) {
  typename T::Builder builder(std::move(value));
  return [_ABI47_0_0RCTTypedModuleConstants newWithUnsafeDictionary:builder.buildUnsafeRawValue()];
}
