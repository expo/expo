...
@implementation ABI47_0_0RCTSurfaceHostingComponent

+ (Class<CKComponentControllerProtocol>)controllerClass
{
  return [ABI47_0_0RCTSurfaceHostingComponentController class];
}

+ (id)initialState
{
  return [ABI47_0_0RCTSurfaceHostingComponentState new];
}
...
+ (instancetype)newWithSurface:(id<ABI47_0_0RCTSurfaceProtocol>)surface options:(ABI47_0_0RCTSurfaceHostingComponentOptions)options
{
  ...
  ABI47_0_0RCTSurfaceHostingComponentState *const state = scope.state();

  ABI47_0_0RCTSurfaceHostingComponentState *const newState =
    [ABI47_0_0RCTSurfaceHostingComponentState newWithStage:surface.stage
  ...
  ABI47_0_0RCTSurfaceHostingComponent *const component =
...
- (RCLayout)computeLayoutThatFits:(CKSizeRange)constrainedSize
{
  ...
  // just to provide layout constraints to ABI47_0_0React Native as early as possible.
  // ABI47_0_0React Native *may* use this info later during applying the own state and
  ...
  // interop compatibilities of ABI47_0_0React Native which will enable us granularly
  // control ABI47_0_0React Native mounting blocks and, as a result, implement
  // truly synchronous mounting stage between ABI47_0_0React Native and ComponentKit.
  [_surface setMinimumSize:constrainedSize.min
               maximumSize:constrainedSize.max];

  // Just in case of the very first building pass, we give ABI47_0_0React Native a chance
  // to prepare its internals for coming synchronous measuring.
  if ([_surface isKindOfClass:[ABI47_0_0RCTSurface class]]) {
    // Legacy Pre-Fabric Surface
    [(ABI47_0_0RCTSurface *)_surface synchronouslyWaitForStage:ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout
                                      timeout:_options.synchronousLayoutingTimeout];
  } else if ([_surface isKindOfClass:[ABI47_0_0RCTFabricSurface class]]) {
    // Fabric Surface
    // Hack: Increase timeout because ABI47_0_0RCTFabricSurface stage will be ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout
    ...
    // TODO (T115399546) Allow ABI47_0_0RCTFabricSurface synchronouslyWaitFor to wait for mounting completion stage
    NSTimeInterval timeout = 20;
    [(ABI47_0_0RCTFabricSurface *)_surface synchronouslyWaitFor:timeout];
  }

  CGSize fittingSize = CGSizeZero;
  if (_surface.stage & ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
...
- (CKComponentBoundsAnimation)boundsAnimationFromPreviousComponent:(ABI47_0_0RCTSurfaceHostingComponent *)previousComponent
{
