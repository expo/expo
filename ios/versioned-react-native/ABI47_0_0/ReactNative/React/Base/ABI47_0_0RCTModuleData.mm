...
#import "ABI47_0_0RCTBridgeModuleDecorator.h"
#import "ABI47_0_0RCTInitializing.h"
...
void ABI47_0_0RCTSetIsMainQueueExecutionOfConstantsToExportDisabled(BOOL val)
{
  isMainQueueExecutionOfConstantToExportDisabled = val;
}

BOOL ABI47_0_0RCTIsMainQueueExecutionOfConstantsToExportDisabled()
{
  return isMainQueueExecutionOfConstantToExportDisabled;
}

@implementation ABI47_0_0RCTModuleData {
  NSDictionary<NSString *, id> *_constantsToExport;
  NSString *_queueName;
  __weak ABI47_0_0RCTBridge *_bridge;
  ABI47_0_0RCTBridgeModuleProvider _moduleProvider;
                  ...
                  callableJSModules:(ABI47_0_0RCTCallableJSModules *)callableJSModules
{
  return [self initWithModuleClass:moduleClass
                    moduleProvider:^id<ABI47_0_0RCTBridgeModule> {
...
- (void)setUpInstanceAndBridge:(int32_t)requestId
{
  ...
  ABI47_0_0RCT_PROFILE_BEGIN_EVENT(
      ...
      @"[ABI47_0_0RCTModuleData setUpInstanceAndBridge]",
      @{@"moduleClass" : NSStringFromClass(_moduleClass)});
  {
    ...
    if (shouldSetup) {
      if (!_instance) {
        if (ABI47_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI47_0_0RCTAssertMainQueue();
        }
        ABI47_0_0RCT_PROFILE_BEGIN_EVENT(ABI47_0_0RCTProfileTagAlways, @"[ABI47_0_0RCTModuleData setUpInstanceAndBridge] Create module", nil);
      ...
      if (_instance && ABI47_0_0RCTProfileIsProfiling()) {
        ABI47_0_0RCTProfileHookInstance(_instance);
      }
    }
    ...
    if (shouldSetup) {
      ...
      ABI47_0_0RCTBridgeModuleDecorator *moduleDecorator =
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI47_0_0RCT_PROFILE_BEGIN_EVENT(ABI47_0_0RCTProfileTagAlways, @"[ABI47_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
...
- (void)_initializeModule
{
  if (!_isInitialized && [_instance respondsToSelector:@selector(initialize)]) {
    _isInitialized = YES;
    [(id<ABI47_0_0RCTInitializing>)_instance initialize];
  }
}
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI47_0_0RCT_PROFILE_BEGIN_EVENT(ABI47_0_0RCTProfileTagAlways, @"[ABI47_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:ABI47_0_0RCTDidInitializeModuleNotification
                      object:_bridge
                    userInfo:@{@"module" : _instance, @"bridge" : ABI47_0_0RCTNullIfNil(_bridge.parentBridge)}];
    ABI47_0_0RCT_PROFILE_END_EVENT(ABI47_0_0RCTProfileTagAlways, @"");
  }
}
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI47_0_0RCT_PROFILE_BEGIN_EVENT(ABI47_0_0RCTProfileTagAlways, @"[ABI47_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ABI47_0_0React.%@Queue", self.name];
...
- (void)calculateMethods
{
  ...
  NSMutableArray<id<ABI47_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
  NSMutableDictionary<NSString *, id<ABI47_0_0RCTBridgeMethod>> *moduleMethodsByName = [NSMutableDictionary new];
  ...
  while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
      ...
      if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
        ...
        auto exportedMethod = ((const ABI47_0_0RCTMethodInfo *(*)(id, SEL))imp)(_moduleClass, selector);
        id<ABI47_0_0RCTBridgeMethod> moduleMethod = [[ABI47_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod
...
- (id<ABI47_0_0RCTBridgeModule>)instance
{
  NSString *moduleName = [self name];
  ...
  if (!_setupComplete) {
    ABI47_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI47_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI47_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI47_0_0RCT_PROFILE_BEGIN_EVENT(ABI47_0_0RCTProfileTagAlways, @"[ABI47_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI47_0_0RCTIsMainQueue()) {
        ABI47_0_0RCTLogWarn(@"ABI47_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI47_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI47_0_0RCTBridgeMethod>> *)methods
{
  [self calculateMethods];
  return _methods;
}

- (NSDictionary<NSString *, id<ABI47_0_0RCTBridgeMethod>> *)methodsByName
{
  [self calculateMethods];
...
- (void)gatherConstantsAndSignalJSRequireEnding:(BOOL)startMarkers
{
  ...
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI47_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI47_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI47_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    ...
    if (startMarkers) {
       ...
       *  - This delegates to ABI47_0_0RCTNativeModule::getConstants(), which calls ABI47_0_0RCTModuleData gatherConstants().
    ...
    if (!ABI47_0_0RCTIsMainQueueExecutionOfConstantsToExportDisabled() && _requiresMainQueueSetup) {
      if (!ABI47_0_0RCTIsMainQueue()) {
        ABI47_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
      }
...
- (dispatch_queue_t)methodQueue
{
  if (_bridge.valid) {
    ...
    ABI47_0_0RCTAssert(_methodQueue != nullptr, @"Module %@ has no methodQueue (instance: %@)", self, instance);
  }
  return _methodQueue;
}
