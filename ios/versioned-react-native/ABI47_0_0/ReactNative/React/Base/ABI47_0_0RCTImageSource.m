...
@interface ABI47_0_0RCTImageSource ()

@property (nonatomic, assign) BOOL packagerAsset;

@end

@implementation ABI47_0_0RCTImageSource
...
{
  ABI47_0_0RCTImageSource *imageSource = [[ABI47_0_0RCTImageSource alloc] initWithURLRequest:_request size:size scale:scale];
  imageSource.packagerAsset = _packagerAsset;
  return imageSource;
}

- (BOOL)isEqual:(ABI47_0_0RCTImageSource *)object
{
  if (![object isKindOfClass:[ABI47_0_0RCTImageSource class]]) {
    return NO;
  }
...
{
  return [NSString stringWithFormat:@"<ABI47_0_0RCTImageSource: %p URL=%@, size=%@, scale=%0.f>",
                                    self,
                                    _request.URL,
...
@implementation ABI47_0_0RCTConvert (ImageSource)

+ (ABI47_0_0RCTImageSource *)ABI47_0_0RCTImageSource:(id)json
{
  if (!json) {
    return nil;
  }
  ...
  if ([json isKindOfClass:[NSDictionary class]]) {
    ...
    ABI47_0_0RCTLogConvertError(json, @"an image. Did you forget to call resolveAssetSource() on the JS side?");
    return nil;
  }
...
ABI47_0_0RCT_ARRAY_CONVERTER(ABI47_0_0RCTImageSource)
