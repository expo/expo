...
@interface ABI47_0_0RCTKeyCommand : NSObject <NSCopying>
...
@implementation ABI47_0_0RCTKeyCommand
...
- (BOOL)isEqual:(ABI47_0_0RCTKeyCommand *)object
{
  if (![object isKindOfClass:[ABI47_0_0RCTKeyCommand class]]) {
    return NO;
  }
...
@interface ABI47_0_0RCTKeyCommands ()

@property (nonatomic, strong) NSMutableSet<ABI47_0_0RCTKeyCommand *> *commands;

@end

@implementation ABI47_0_0RCTKeyCommands
...
- (void)handleKeyUIEventSwizzle:(UIEvent *)event
{
  ...
  if (isKeyDown && modifiedInput.length > 0 && interactionEnabled) {
    ...
    if (!firstResponder) {
      [self ABI47_0_0RCT_handleKeyCommand:modifiedInput flags:modifierFlags];
    }
  }
};
...
- (void)ABI47_0_0RCT_handleKeyCommand:(NSString *)input flags:(UIKeyModifierFlags)modifierFlags
{
  for (ABI47_0_0RCTKeyCommand *command in [ABI47_0_0RCTKeyCommands sharedInstance].commands) {
...
{
  static ABI47_0_0RCTKeyCommands *sharedInstance;
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  ABI47_0_0RCTKeyCommand *keyCommand = [[ABI47_0_0RCTKeyCommand alloc] init:input flags:flags block:block];
...
- (void)unregisterKeyCommandWithInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI47_0_0RCTKeyCommand *command in _commands.allObjects) {
...
- (BOOL)isKeyCommandRegisteredForInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI47_0_0RCTKeyCommand *command in _commands) {
    if ([command matchesInput:input flags:flags]) {
      return YES;
    }
  }
