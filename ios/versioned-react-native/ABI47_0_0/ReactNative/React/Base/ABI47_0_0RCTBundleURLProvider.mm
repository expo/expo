...
#import "ABI47_0_0RCTBundleURLProvider.h"

#import "ABI47_0_0RCTConvert.h"
#import "ABI47_0_0RCTDefines.h"
#import "ABI47_0_0RCTLog.h"

NSString *const ABI47_0_0RCTBundleURLProviderUpdatedNotification = @"ABI47_0_0RCTBundleURLProviderUpdatedNotification";

const NSUInteger kABI47_0_0RCTBundleURLProviderDefaultPort = ABI47_0_0RCT_METRO_PORT;

#if ABI47_0_0RCT_DEV_MENU | ABI47_0_0RCT_PACKAGER_LOADING_FUNCTIONALITY
static BOOL kABI47_0_0RCTAllowPackagerAccess = YES;
void ABI47_0_0RCTBundleURLProviderAllowPackagerServerAccess(BOOL allowed)
{
  kABI47_0_0RCTAllowPackagerAccess = allowed;
}
#endif
static NSString *const kABI47_0_0RCTPackagerSchemeKey = @"ABI47_0_0RCT_packager_scheme";
static NSString *const kABI47_0_0RCTJsLocationKey = @"ABI47_0_0RCT_jsLocation";
static NSString *const kABI47_0_0RCTEnableDevKey = @"ABI47_0_0RCT_enableDev";
static NSString *const kABI47_0_0RCTEnableMinificationKey = @"ABI47_0_0RCT_enableMinification";

@implementation ABI47_0_0RCTBundleURLProvider
...
{
  return @{
    kABI47_0_0RCTEnableDevKey : @YES,
    kABI47_0_0RCTEnableMinificationKey : @NO,
  };
}

- (void)settingsUpdated
{
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI47_0_0RCTBundleURLProviderUpdatedNotification object:self];
}
...
static NSURL *serverRootWithHostPort(NSString *hostPort, NSString *scheme)
{
  ...
  return [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@:%lu/",
                                                         scheme,
                                                         hostPort,
                                                         (unsigned long)kABI47_0_0RCTBundleURLProviderDefaultPort]];
}

#if ABI47_0_0RCT_DEV_MENU
+ (BOOL)isPackagerRunning:(NSString *)hostPort
{
  return [ABI47_0_0RCTBundleURLProvider isPackagerRunning:hostPort scheme:nil];
}
...
- (NSString *)guessPackagerHost
{
  ...
  if ([ABI47_0_0RCTBundleURLProvider isPackagerRunning:host]) {
    return host;
  }
  return nil;
}
...
{
#if ABI47_0_0RCT_DEV_MENU | ABI47_0_0RCT_PACKAGER_LOADING_FUNCTIONALITY
  if (!kABI47_0_0RCTAllowPackagerAccess) {
    ABI47_0_0RCTLogInfo(@"Packager server access is disabled in this environment");
    return nil;
  }
#endif
  ...
  if ([location length] && ![ABI47_0_0RCTBundleURLProvider isPackagerRunning:location scheme:scheme]) {
    location = nil;
  }
#endif
  if (location != nil) {
...
- (NSURL *)jsBundleURLForBundleRoot:(NSString *)bundleRoot fallbackURLProvider:(NSURL * (^)(void))fallbackURLProvider
{
  ...
  if (!packagerServerHostPort) {
    ...
    return [ABI47_0_0RCTBundleURLProvider jsBundleURLForBundleRoot:bundleRoot
...
- (BOOL)enableDev
{
  return [[NSUserDefaults standardUserDefaults] boolForKey:kABI47_0_0RCTEnableDevKey];
}

- (BOOL)enableMinification
{
  return [[NSUserDefaults standardUserDefaults] boolForKey:kABI47_0_0RCTEnableMinificationKey];
}

- (NSString *)jsLocation
{
  return [[NSUserDefaults standardUserDefaults] stringForKey:kABI47_0_0RCTJsLocationKey];
}

- (NSString *)packagerScheme
{
  NSString *packagerScheme = [[NSUserDefaults standardUserDefaults] stringForKey:kABI47_0_0RCTPackagerSchemeKey];
...
{
  [self updateValue:@(enableDev) forKey:kABI47_0_0RCTEnableDevKey];
}

- (void)setJsLocation:(NSString *)jsLocation
{
  [self updateValue:jsLocation forKey:kABI47_0_0RCTJsLocationKey];
}

- (void)setEnableMinification:(BOOL)enableMinification
{
  [self updateValue:@(enableMinification) forKey:kABI47_0_0RCTEnableMinificationKey];
}

- (void)setPackagerScheme:(NSString *)packagerScheme
{
  [self updateValue:packagerScheme forKey:kABI47_0_0RCTPackagerSchemeKey];
}

+ (instancetype)sharedSettings
{
  static ABI47_0_0RCTBundleURLProvider *sharedInstance;
  static dispatch_once_t once_token;
  dispatch_once(&once_token, ^{
    sharedInstance = [ABI47_0_0RCTBundleURLProvider new];
  });
