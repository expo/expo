...
#import "ABI47_0_0RCTSurfaceRootShadowViewDelegate.h"
...
@interface ABI47_0_0RCTSurface () <ABI47_0_0RCTSurfaceRootShadowViewDelegate, ABI47_0_0RCTUIManagerObserver>
@end

@implementation ABI47_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI47_0_0RCTBridge *_batchedBridge;
  ABI47_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI47_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI47_0_0RCTSurfaceView *_Nullable _view;
  ABI47_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI47_0_0RCTErrorNewArchitectureValidation(ABI47_0_0RCTNotAllowedInFabricWithoutLegacy, @"ABI47_0_0RCTSurface", nil);
  ABI47_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instantiate `ABI47_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI47_0_0RCTAllocateRootViewTag();
    ...
    _stage = ABI47_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI47_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI47_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI47_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI47_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI47_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI47_0_0RCTSurfaceRootView *rootView = (ABI47_0_0RCTSurfaceRootView *)[self._uiManager viewForABI47_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI47_0_0RCTAssert(
      [rootView isKindOfClass:[ABI47_0_0RCTSurfaceRootView class]],
      @"Received root view is not an instance of `ABI47_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI47_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI47_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI47_0_0RCTSurfaceStage)stage
{
  ABI47_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI47_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI47_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI47_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ABI47_0_0RCTBridge *batchedBridge;
  NSDictionary *properties;
  ...
  ABI47_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI47_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI47_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI47_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI47_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI47_0_0RCTIsMainQueue() ? ABI47_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI47_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI47_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI47_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI47_0_0ReactTag:self->_rootViewTag];
    ABI47_0_0RCTAssert(
        [rootShadowView isKindOfClass:[ABI47_0_0RCTSurfaceRootShadowView class]],
        @"Received shadow view is not an instance of `ABI47_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI47_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI47_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI47_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI47_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI47_0_0ReactTag:self->_rootViewTag];
...
{
  // `viewportOffset` is intentionally zero because `ABI47_0_0RCTSurface` ignores it.
  // However, it is needed in `ABI47_0_0RCTFabricSurface`.
...
{
  if (ABI47_0_0RCTIsUIManagerQueue()) {
    ABI47_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI47_0_0RCTIsMainQueue() && (stage & ABI47_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI47_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI47_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI47_0_0RCTAssert(
          NO,
          @"Only waiting for `ABI47_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI47_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI47_0_0RCTIsMainQueue() && (stage & ABI47_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI47_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI47_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI47_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI47_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI47_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI47_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI47_0_0RCTUIManager *)manager performMountingWithBlock:(ABI47_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI47_0_0RCTUIManager *)manager
{
  if (self.stage & ABI47_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI47_0_0RCTSurfaceStageSurfaceDidInitialMounting];
