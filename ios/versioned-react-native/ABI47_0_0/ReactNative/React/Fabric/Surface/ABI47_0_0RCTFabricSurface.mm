...
#import <ABI47_0_0React/ABI47_0_0RCTSurfaceRootView.h>
...
@implementation ABI47_0_0RCTFabricSurface {
  __weak ABI47_0_0RCTSurfacePresenter *_surfacePresenter;
  ...
  ABI47_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _surfaceHandler =
        SurfaceHandler{ABI47_0_0RCTStringFromNSString(moduleName), (SurfaceId)[ABI47_0_0RCTAllocateRootViewTag() integerValue]};
    _surfaceHandler->setProps(convertIdToFollyDynamic(initialProperties));

    [_surfacePresenter registerSurface:self];

    [self setMinimumSize:CGSizeZero maximumSize:ABI47_0_0RCTViewportSize()];
...
- (NSString *)moduleName
{
  return ABI47_0_0RCTNSStringFromString(_surfaceHandler->getModuleName());
}
...
- (ABI47_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI47_0_0RCTSurfaceView alloc] initWithSurface:(ABI47_0_0RCTSurface *)self];
...
{
  return _surfaceHandler->getStatus() == SurfaceHandler::Status::Running ? ABI47_0_0RCTSurfaceStageRunning
                                                                         : ABI47_0_0RCTSurfaceStagePreparing;
}

- (void)_propagateStageChange
{
  ABI47_0_0RCTSurfaceStage stage = self.stage;

  // Notifying the `delegate`
  id<ABI47_0_0RCTSurfaceDelegate> delegate = self.delegate;
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI47_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)_updateLayoutContext
{
  ...
  layoutConstraints.layoutDirection = ABI47_0_0RCTLayoutDirection([[ABI47_0_0RCTI18nUtil sharedInstance] isRTL]);

  auto layoutContext = _surfaceHandler->getLayoutContext();

  layoutContext.pointScaleFactor = ABI47_0_0RCTScreenScale();
  layoutContext.swapLeftAndRightInRTL =
      [[ABI47_0_0RCTI18nUtil sharedInstance] isRTL] && [[ABI47_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL];
  layoutContext.fontSizeMultiplier = ABI47_0_0RCTFontSizeMultiplier();
...
- (void)setMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize viewportOffset:(CGPoint)viewportOffset
{
  ...
  layoutConstraints.minimumSize = ABI47_0_0RCTSizeFromCGSize(minimumSize);
  layoutConstraints.maximumSize = ABI47_0_0RCTSizeFromCGSize(maximumSize);

  if (!isnan(viewportOffset.x) && !isnan(viewportOffset.y)) {
    layoutContext.viewportOffset = ABI47_0_0RCTPointFromCGPoint(viewportOffset);
  }
...
- (CGSize)sizeThatFitsMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  return ABI47_0_0RCTCGSizeFromSize(_surfaceHandler->measure(layoutConstraints, layoutContext));
}

- (CGSize)minimumSize
{
  return ABI47_0_0RCTCGSizeFromSize(_surfaceHandler->getLayoutConstraints().minimumSize);
}

- (CGSize)maximumSize
{
  return ABI47_0_0RCTCGSizeFromSize(_surfaceHandler->getLayoutConstraints().maximumSize);
}

- (CGPoint)viewportOffset
{
  return ABI47_0_0RCTCGPointFromPoint(_surfaceHandler->getLayoutContext().viewportOffset);
}
