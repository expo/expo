...
#import "ABI47_0_0RCTSurfaceTouchHandler.h"
...
#import "ABI47_0_0RCTTouchableComponentViewProtocol.h"
...
typedef NS_ENUM(NSInteger, ABI47_0_0RCTTouchEventType) {
  ABI47_0_0RCTTouchEventTypeTouchStart,
  ABI47_0_0RCTTouchEventTypeTouchMove,
  ABI47_0_0RCTTouchEventTypeTouchEnd,
  ABI47_0_0RCTTouchEventTypeTouchCancel,
};
...
struct ActiveTouch {
   ...
   */
  __strong UIView<ABI47_0_0RCTComponentViewProtocol> *componentView = nil;
    ...
    CGPoint rootViewOriginOffset)
{
  ...
  activeTouch.touch.offsetPoint = ABI47_0_0RCTPointFromCGPoint(offsetPoint);
  activeTouch.touch.screenPoint = ABI47_0_0RCTPointFromCGPoint(screenPoint);
  activeTouch.touch.pagePoint = ABI47_0_0RCTPointFromCGPoint(pagePoint);

  activeTouch.touch.timestamp = uiTouch.timestamp;

  if (ABI47_0_0RCTForceTouchAvailable()) {
    activeTouch.touch.force = ABI47_0_0RCTZeroIfNaN(uiTouch.force / uiTouch.maximumPossibleForce);
  }
...
CreateTouchWithUITouch(UITouch *uiTouch, UIEvent *uiEvent, UIView *rootComponentView, CGPoint rootViewOriginOffset)
{
  ...
  while (componentView) {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      activeTouch.eventEmitter = [(id<ABI47_0_0RCTTouchableComponentViewProtocol>)componentView
...
{
  return [(id<ABI47_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitterAtPoint:point];
}
...
static PointerEvent CreatePointerEventFromActiveTouch(ActiveTouch activeTouch, ABI47_0_0RCTTouchEventType eventType)
{
  ...
  if (eventType == ABI47_0_0RCTTouchEventTypeTouchEnd) {
    event.buttons = 0;
  }
    ...
    NSTimeInterval timestamp)
{
  ...
  event.clientPoint = ABI47_0_0RCTPointFromCGPoint(clientLocation);
  event.screenPoint = ABI47_0_0RCTPointFromCGPoint(screenLocation);
  event.offsetPoint = ABI47_0_0RCTPointFromCGPoint(offsetLocation);
  event.width = 1.0;
  event.height = 1.0;
  event.tiltX = 0;
  event.tiltY = 0;
...
{
  if ([view.class conformsToProtocol:@protocol(ABI47_0_0RCTComponentViewProtocol)]) {
    auto props = ((id<ABI47_0_0RCTComponentViewProtocol>)view).props;
...
@interface ABI47_0_0RCTSurfaceTouchHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI47_0_0RCTSurfaceTouchHandler {
...
- (instancetype)init
{
  if (self = [super initWithTarget:nil action:nil]) {
    ...
    // Otherwise, lower-level components not built using ABI47_0_0React Native,
...
- (void)attachToView:(UIView *)view
{
  ...
  if (ABI47_0_0RCTGetDispatchW3CPointerEvents()) {
...
- (void)_dispatchActiveTouches:(std::vector<ActiveTouch>)activeTouches eventType:(ABI47_0_0RCTTouchEventType)eventType
{
  TouchEvent event = {};
  ...
  BOOL isEndishEventType = eventType == ABI47_0_0RCTTouchEventTypeTouchEnd || eventType == ABI47_0_0RCTTouchEventTypeTouchCancel;
  ...
  for (const auto &activeTouch : activeTouches) {
    ...
    if (ABI47_0_0RCTGetDispatchW3CPointerEvents()) {
      ...
      switch (eventType) {
        case ABI47_0_0RCTTouchEventTypeTouchStart:
          activeTouch.eventEmitter->onPointerDown(pointerEvent);
          break;
        case ABI47_0_0RCTTouchEventTypeTouchMove:
          activeTouch.eventEmitter->onPointerMove(pointerEvent);
          break;
        case ABI47_0_0RCTTouchEventTypeTouchEnd:
          activeTouch.eventEmitter->onPointerUp(pointerEvent);
          break;
        case ABI47_0_0RCTTouchEventTypeTouchCancel:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI47_0_0RCTTouchEventTypeTouchStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI47_0_0RCTTouchEventTypeTouchMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI47_0_0RCTTouchEventTypeTouchEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActiveTouches:[self _activeTouchesFromTouches:touches] eventType:ABI47_0_0RCTTouchEventTypeTouchCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (!_activeTouches.empty()) {
    ...
    [self _dispatchActiveTouches:activeTouches eventType:ABI47_0_0RCTTouchEventTypeTouchCancel];
