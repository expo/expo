...
#import "ABI47_0_0RCTFont.h"
#import "ABI47_0_0RCTAssert.h"
#import "ABI47_0_0RCTLog.h"

#import <CoreText/CoreText.h>

typedef CGFloat ABI47_0_0RCTFontWeight;
static ABI47_0_0RCTFontWeight weightOfFont(UIFont *font)
{
  ...
  for (NSString *suffix in weightSuffixes) {
    ...
    if (CFStringFind((CFStringRef)fontName, (CFStringRef)suffix, options).location != kCFNotFound) {
      return (ABI47_0_0RCTFontWeight)fontWeights[i].doubleValue;
    }
    i++;
  }

  auto traits = (__bridge_transfer NSDictionary *)CTFontCopyTraits((CTFontRef)font);
  return (ABI47_0_0RCTFontWeight)[traits[UIFontWeightTrait] doubleValue];
}
...
static ABI47_0_0RCTFontHandler defaultFontHandler;

void ABI47_0_0RCTSetDefaultFontHandler(ABI47_0_0RCTFontHandler handler)
{
  defaultFontHandler = handler;
}

BOOL ABI47_0_0RCTHasFontHandlerSet()
{
...
static NSString *FontWeightDescriptionFromUIFontWeight(UIFontWeight fontWeight)
{
  ...
  ABI47_0_0RCTAssert(NO, @"Unknown UIFontWeight passed in: %f", fontWeight);
  return @"regular";
}

static UIFont *cachedSystemFont(CGFloat size, ABI47_0_0RCTFontWeight weight)
{
  static NSCache<NSValue *, UIFont *> *fontCache = [NSCache new];

  struct __attribute__((__packed__)) CacheKey {
    CGFloat size;
    ABI47_0_0RCTFontWeight weight;
  };
...
@implementation ABI47_0_0RCTConvert (ABI47_0_0RCTFont)

+ (UIFont *)UIFont:(id)json
{
  json = [self NSDictionary:json];
  return [ABI47_0_0RCTFont updateFont:nil
                  withFamily:[ABI47_0_0RCTConvert NSString:json[@"fontFamily"]]
                        size:[ABI47_0_0RCTConvert NSNumber:json[@"fontSize"]]
                      weight:[ABI47_0_0RCTConvert NSString:json[@"fontWeight"]]
                       style:[ABI47_0_0RCTConvert NSString:json[@"fontStyle"]]
                     variant:[ABI47_0_0RCTConvert NSStringArray:json[@"fontVariant"]]
             scaleMultiplier:1];
}

ABI47_0_0RCT_ENUM_CONVERTER(
    ABI47_0_0RCTFontWeight,
    (@{
...
typedef BOOL ABI47_0_0RCTFontStyle;
ABI47_0_0RCT_ENUM_CONVERTER(
    ABI47_0_0RCTFontStyle,
    (@{
      @"normal" : @NO,
      @"italic" : @YES,
      @"oblique" : @YES,
...
typedef NSDictionary ABI47_0_0RCTFontVariantDescriptor;
+ (ABI47_0_0RCTFontVariantDescriptor *)ABI47_0_0RCTFontVariantDescriptor:(id)json
{
  ...
  ABI47_0_0RCTFontVariantDescriptor *value = mapping[json];
  if (ABI47_0_0RCT_DEBUG && !value && [json description].length > 0) {
    ABI47_0_0RCTLogError(
        @"Invalid ABI47_0_0RCTFontVariantDescriptor '%@'. should be one of: %@",
        json,
...
ABI47_0_0RCT_ARRAY_CONVERTER(ABI47_0_0RCTFontVariantDescriptor)

@end

@implementation ABI47_0_0RCTFont
...
+ (UIFont *)updateFont:(UIFont *)font
            withFamily:(NSString *)family
               ...
               variant:(NSArray<ABI47_0_0RCTFontVariantDescriptor *> *)variant
       scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  const ABI47_0_0RCTFontWeight defaultFontWeight = UIFontWeightRegular;
  const CGFloat defaultFontSize = 14;

  // Initialize properties to defaults
  CGFloat fontSize = defaultFontSize;
  ABI47_0_0RCTFontWeight fontWeight = defaultFontWeight;
  ...
  // Get font attributes
  fontSize = [ABI47_0_0RCTConvert CGFloat:size] ?: fontSize;
  if (scaleMultiplier > 0.0 && scaleMultiplier != 1.0) {
    fontSize = round(fontSize * scaleMultiplier);
  }
  familyName = [ABI47_0_0RCTConvert NSString:family] ?: familyName;
  isItalic = style ? [ABI47_0_0RCTConvert ABI47_0_0RCTFontStyle:style] : isItalic;
  fontWeight = weight ? [ABI47_0_0RCTConvert ABI47_0_0RCTFontWeight:weight] : fontWeight;
  ...
  if (!didFindFont && fontNamesForFamilyName(familyName).count == 0) {
    ...
    if (font) {
      ...
      ABI47_0_0RCTLogError(@"Unrecognized font family '%@'", familyName);
  ...
  if (variant) {
    NSArray *fontFeatures = [ABI47_0_0RCTConvert ABI47_0_0RCTFontVariantDescriptorArray:variant];
    UIFontDescriptor *fontDescriptor = [font.fontDescriptor
