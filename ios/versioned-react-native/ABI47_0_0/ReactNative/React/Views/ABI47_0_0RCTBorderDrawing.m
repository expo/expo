...
static const CGFloat ABI47_0_0RCTViewBorderThreshold = 0.001;

BOOL ABI47_0_0RCTBorderInsetsAreEqual(UIEdgeInsets borderInsets)
{
  return ABS(borderInsets.left - borderInsets.right) < ABI47_0_0RCTViewBorderThreshold &&
      ABS(borderInsets.left - borderInsets.bottom) < ABI47_0_0RCTViewBorderThreshold &&
      ABS(borderInsets.left - borderInsets.top) < ABI47_0_0RCTViewBorderThreshold;
}

BOOL ABI47_0_0RCTCornerRadiiAreEqual(ABI47_0_0RCTCornerRadii cornerRadii)
{
  return ABS(cornerRadii.topLeft - cornerRadii.topRight) < ABI47_0_0RCTViewBorderThreshold &&
      ABS(cornerRadii.topLeft - cornerRadii.bottomLeft) < ABI47_0_0RCTViewBorderThreshold &&
      ABS(cornerRadii.topLeft - cornerRadii.bottomRight) < ABI47_0_0RCTViewBorderThreshold;
}

BOOL ABI47_0_0RCTBorderColorsAreEqual(ABI47_0_0RCTBorderColors borderColors)
{
...
ABI47_0_0RCTCornerInsets ABI47_0_0RCTGetCornerInsets(ABI47_0_0RCTCornerRadii cornerRadii, UIEdgeInsets edgeInsets)
{
  return (ABI47_0_0RCTCornerInsets){
      {
...
static UIEdgeInsets ABI47_0_0RCTRoundInsetsToPixel(UIEdgeInsets edgeInsets)
{
  edgeInsets.top = ABI47_0_0RCTRoundPixelValue(edgeInsets.top);
  edgeInsets.bottom = ABI47_0_0RCTRoundPixelValue(edgeInsets.bottom);
  edgeInsets.left = ABI47_0_0RCTRoundPixelValue(edgeInsets.left);
  edgeInsets.right = ABI47_0_0RCTRoundPixelValue(edgeInsets.right);

  return edgeInsets;
}

static void ABI47_0_0RCTPathAddEllipticArc(
...
CGPathRef ABI47_0_0RCTPathCreateWithRoundedRect(CGRect bounds, ABI47_0_0RCTCornerInsets cornerInsets, const CGAffineTransform *transform)
{
  ...
  ABI47_0_0RCTPathAddEllipticArc(
...
static void
ABI47_0_0RCTEllipseGetIntersectionsWithLine(CGRect ellipseBounds, CGPoint lineStart, CGPoint lineEnd, CGPoint intersections[2])
{
...
NS_INLINE BOOL ABI47_0_0RCTCornerRadiiAreAboveThreshold(ABI47_0_0RCTCornerRadii cornerRadii)
{
  return (
      cornerRadii.topLeft > ABI47_0_0RCTViewBorderThreshold || cornerRadii.topRight > ABI47_0_0RCTViewBorderThreshold ||
      cornerRadii.bottomLeft > ABI47_0_0RCTViewBorderThreshold || cornerRadii.bottomRight > ABI47_0_0RCTViewBorderThreshold);
}

static CGPathRef ABI47_0_0RCTPathCreateOuterOutline(BOOL drawToEdge, CGRect rect, ABI47_0_0RCTCornerRadii cornerRadii)
{
  if (drawToEdge) {
    return CGPathCreateWithRect(rect, NULL);
  }

  return ABI47_0_0RCTPathCreateWithRoundedRect(rect, ABI47_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
}

static UIGraphicsImageRenderer *
ABI47_0_0RCTUIGraphicsImageRenderer(CGSize size, CGColorRef backgroundColor, BOOL hasCornerRadii, BOOL drawToEdge)
{
...
static UIImage *ABI47_0_0RCTGetSolidBorderImage(
    ...
    BOOL drawToEdge)
{
  const BOOL hasCornerRadii = ABI47_0_0RCTCornerRadiiAreAboveThreshold(cornerRadii);
  const ABI47_0_0RCTCornerInsets cornerInsets = ABI47_0_0RCTGetCornerInsets(cornerRadii, borderInsets);
  ...
  borderInsets = ABI47_0_0RCTRoundInsetsToPixel(borderInsets);
  ...
  UIGraphicsImageRenderer *const imageRenderer =
      ABI47_0_0RCTUIGraphicsImageRenderer(size, backgroundColor, hasCornerRadii, drawToEdge);
  UIImage *image = [imageRenderer imageWithActions:^(UIGraphicsImageRendererContext *_Nonnull rendererContext) {
    const CGContextRef context = rendererContext.CGContext;
    const CGRect rect = {.size = size};
    CGPathRef path = ABI47_0_0RCTPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
    ...
    CGPathRef insetPath = ABI47_0_0RCTPathCreateWithRoundedRect(UIEdgeInsetsInsetRect(rect, borderInsets), cornerInsets, NULL);

    CGContextAddPath(context, insetPath);
    CGContextEOClip(context);

    BOOL hasEqualColors = ABI47_0_0RCTBorderColorsAreEqual(borderColors);
    if ((drawToEdge || !hasCornerRadii) && hasEqualColors) {
      ...
      if (cornerInsets.topLeft.width > 0 && cornerInsets.topLeft.height > 0) {
        CGPoint points[2];
        ABI47_0_0RCTEllipseGetIntersectionsWithLine(
...
static UIImage *ABI47_0_0RCTGetDashedOrDottedBorderImage(
    ...
    BOOL drawToEdge)
{
  NSCParameterAssert(borderStyle == ABI47_0_0RCTBorderStyleDashed || borderStyle == ABI47_0_0RCTBorderStyleDotted);

  if (!ABI47_0_0RCTBorderColorsAreEqual(borderColors) || !ABI47_0_0RCTBorderInsetsAreEqual(borderInsets)) {
    ABI47_0_0RCTLogWarn(@"Unsupported dashed / dotted border style");
    return nil;
  }
  ...
  const BOOL hasCornerRadii = ABI47_0_0RCTCornerRadiiAreAboveThreshold(cornerRadii);
  UIGraphicsImageRenderer *const imageRenderer =
      ABI47_0_0RCTUIGraphicsImageRenderer(viewSize, backgroundColor, hasCornerRadii, drawToEdge);
  return [imageRenderer imageWithActions:^(UIGraphicsImageRendererContext *_Nonnull rendererContext) {
    ...
    if (backgroundColor) {
      CGPathRef outerPath = ABI47_0_0RCTPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
    ...
    CGPathRef path = ABI47_0_0RCTPathCreateWithRoundedRect(pathRect, ABI47_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);

    CGFloat dashLengths[2];
    dashLengths[0] = dashLengths[1] = (borderStyle == ABI47_0_0RCTBorderStyleDashed ? 3 : 1) * lineWidth;
...
UIImage *ABI47_0_0RCTGetBorderImage(
    ABI47_0_0RCTBorderStyle borderStyle,
    CGSize viewSize,
    ABI47_0_0RCTCornerRadii cornerRadii,
    UIEdgeInsets borderInsets,
...
{
  switch (borderStyle) {
    case ABI47_0_0RCTBorderStyleSolid:
      return ABI47_0_0RCTGetSolidBorderImage(cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
    case ABI47_0_0RCTBorderStyleDashed:
    case ABI47_0_0RCTBorderStyleDotted:
      return ABI47_0_0RCTGetDashedOrDottedBorderImage(
          borderStyle, cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
    case ABI47_0_0RCTBorderStyleUnset:
      break;
  }
