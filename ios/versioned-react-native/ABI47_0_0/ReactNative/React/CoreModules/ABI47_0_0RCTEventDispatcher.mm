...
static NSNumber *ABI47_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI47_0_0RCTUniqueCoalescingKeyGenerator = 0;

@interface ABI47_0_0RCTEventDispatcher () <ABI47_0_0RCTTurboModule>
@end

@implementation ABI47_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI47_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI47_0_0RCTEventDispatcherObserver>> *_observers;
...
- (void)sendViewEventWithName:(NSString *)name ABI47_0_0ReactTag:(NSNumber *)ABI47_0_0ReactTag
{
  [_callableJSModules invokeModule:@"ABI47_0_0RCTViewEventEmitter" method:@"emit" withArgs:@[ name, ABI47_0_0RCTNullIfNil(ABI47_0_0ReactTag) ]];
}

- (void)sendAppEventWithName:(NSString *)name body:(id)body
{
  [_callableJSModules invokeModule:@"ABI47_0_0RCTNativeAppEventEmitter"
                            method:@"emit"
...
{
  [_callableJSModules invokeModule:@"ABI47_0_0RCTDeviceEventEmitter" method:@"emit" withArgs:body ? @[ name, body ] : @[ name ]];
}

- (void)sendTextEventWithType:(ABI47_0_0RCTTextEventType)type
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI47_0_0RCTComponentEvent *event = [[ABI47_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI47_0_0ReactTag body:body];
  [self sendEvent:event];
}

- (void)notifyObserversOfEvent:(id<ABI47_0_0RCTEvent>)event
{
  [_observersLock lock];

  for (id<ABI47_0_0RCTEventDispatcherObserver> observer in _observers) {
...
- (void)sendEvent:(id<ABI47_0_0RCTEvent>)event
{
  ...
  if (event.canCoalesce) {
    eventID = ABI47_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI47_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI47_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI47_0_0RCTGetEventID(event.viewTag, event.eventName, ABI47_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI47_0_0RCTAssert(
        previousEvent == nil,
...
- (void)addDispatchObserver:(id<ABI47_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
- (void)removeDispatchObserver:(id<ABI47_0_0RCTEventDispatcherObserver>)observer
{
  [_observersLock lock];
...
Class ABI47_0_0RCTEventDispatcherCls(void)
{
  return ABI47_0_0RCTEventDispatcher.class;
}
