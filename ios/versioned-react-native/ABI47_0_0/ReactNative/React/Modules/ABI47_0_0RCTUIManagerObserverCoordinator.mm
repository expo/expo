...
@implementation ABI47_0_0RCTUIManagerObserverCoordinator {
  NSHashTable<id<ABI47_0_0RCTUIManagerObserver>> *_observers;
  std::mutex _mutex;
}
...
- (void)addObserver:(id<ABI47_0_0RCTUIManagerObserver>)observer
{
  std::lock_guard<std::mutex> lock(_mutex);
  [self->_observers addObject:observer];
}

- (void)removeObserver:(id<ABI47_0_0RCTUIManagerObserver>)observer
{
  std::lock_guard<std::mutex> lock(_mutex);
...
- (void)uiManagerWillPerformLayout:(ABI47_0_0RCTUIManager *)manager
{
  std::lock_guard<std::mutex> lock(_mutex);

  for (id<ABI47_0_0RCTUIManagerObserver> observer in _observers) {
...
- (void)uiManagerDidPerformLayout:(ABI47_0_0RCTUIManager *)manager
{
  std::lock_guard<std::mutex> lock(_mutex);
...
- (void)uiManagerDidPerformMounting:(ABI47_0_0RCTUIManager *)manager
{
  std::lock_guard<std::mutex> lock(_mutex);
