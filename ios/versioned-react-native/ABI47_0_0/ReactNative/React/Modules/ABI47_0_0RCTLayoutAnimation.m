...
@implementation ABI47_0_0RCTLayoutAnimation

static UIViewAnimationCurve _currentKeyboardAnimationCurve;

static UIViewAnimationOptions UIViewAnimationOptionsFromABI47_0_0RCTAnimationType(ABI47_0_0RCTAnimationType type)
{
  switch (type) {
    case ABI47_0_0RCTAnimationTypeLinear:
      return UIViewAnimationOptionCurveLinear;
    case ABI47_0_0RCTAnimationTypeEaseIn:
      return UIViewAnimationOptionCurveEaseIn;
    case ABI47_0_0RCTAnimationTypeEaseOut:
      return UIViewAnimationOptionCurveEaseOut;
    case ABI47_0_0RCTAnimationTypeEaseInEaseOut:
      return UIViewAnimationOptionCurveEaseInOut;
    case ABI47_0_0RCTAnimationTypeKeyboard:
      // http://stackoverflow.com/questions/18870447/how-to-use-the-default-ios7-uianimation-curve
      return (UIViewAnimationOptions)(_currentKeyboardAnimationCurve << 16);
    default:
      ABI47_0_0RCTLogError(@"Unsupported animation type %lld", (long long)type);
      return UIViewAnimationOptionCurveEaseInOut;
  }
}
...
- (instancetype)initWithDuration:(NSTimeInterval)duration config:(NSDictionary *)config
{
  ...
  if (self = [super init]) {
    _property = [ABI47_0_0RCTConvert NSString:config[@"property"]];

    _duration = [ABI47_0_0RCTConvert NSTimeInterval:config[@"duration"]] ?: duration;
    ...
    _delay = [ABI47_0_0RCTConvert NSTimeInterval:config[@"delay"]];
    if (_delay > 0.0 && _delay < 0.01) {
      ABI47_0_0RCTLogError(@"ABI47_0_0RCTLayoutAnimationGroup expects timings to be in ms, not seconds.");
      _delay = _delay * 1000.0;
    }

    _animationType = [ABI47_0_0RCTConvert ABI47_0_0RCTAnimationType:config[@"type"]];
    if (_animationType == ABI47_0_0RCTAnimationTypeSpring) {
      _springDamping = [ABI47_0_0RCTConvert CGFloat:config[@"springDamping"]];
      _initialVelocity = [ABI47_0_0RCTConvert CGFloat:config[@"initialVelocity"]];
    }
  }
...
- (void)performAnimations:(void (^)(void))animations withCompletionBlock:(void (^)(BOOL completed))completionBlock
{
  if (_animationType == ABI47_0_0RCTAnimationTypeSpring) {
    ...
    UIViewAnimationOptions options =
        UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionsFromABI47_0_0RCTAnimationType(_animationType);
...
- (BOOL)isEqual:(ABI47_0_0RCTLayoutAnimation *)animation
{
