...
char *const ABI47_0_0RCTUIManagerQueueName = "com.facebook.ABI47_0_0React.ShadowQueue";

static BOOL pseudoUIManagerQueueFlag = NO;

dispatch_queue_t ABI47_0_0RCTGetUIManagerQueue(void)
{
  ...
  dispatch_once(&onceToken, ^{
    if ([NSOperation instancesRespondToSelector:@selector(qualityOfService)]) {
      ...
      shadowQueue = dispatch_queue_create(ABI47_0_0RCTUIManagerQueueName, attr);
    } else {
      shadowQueue = dispatch_queue_create(ABI47_0_0RCTUIManagerQueueName, DISPATCH_QUEUE_SERIAL);
...
BOOL ABI47_0_0RCTIsUIManagerQueue()
{
  static void *queueKey = &queueKey;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    dispatch_queue_set_specific(ABI47_0_0RCTGetUIManagerQueue(), queueKey, queueKey, NULL);
  });
  return dispatch_get_specific(queueKey) == queueKey;
}

BOOL ABI47_0_0RCTIsPseudoUIManagerQueue()
{
...
void ABI47_0_0RCTExecuteOnUIManagerQueue(dispatch_block_t block)
{
  if (ABI47_0_0RCTIsUIManagerQueue() || ABI47_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    dispatch_async(ABI47_0_0RCTGetUIManagerQueue(), ^{
      block();
    });
  }
}

void ABI47_0_0RCTUnsafeExecuteOnUIManagerQueueSync(dispatch_block_t block)
{
  if (ABI47_0_0RCTIsUIManagerQueue() || ABI47_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    if (ABI47_0_0RCTIsMainQueue()) {
      ...
      dispatch_sync(ABI47_0_0RCTGetUIManagerQueue(), ^{
        block();
      });
    }
  }
...
NSNumber *ABI47_0_0RCTAllocateRootViewTag()
{
