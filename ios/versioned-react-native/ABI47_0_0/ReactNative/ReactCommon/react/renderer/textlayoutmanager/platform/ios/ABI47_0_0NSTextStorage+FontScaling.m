...
typedef NS_OPTIONS(NSInteger, ABI47_0_0RCTTextSizeComparisonOptions) {
  ABI47_0_0RCTTextSizeComparisonSmaller = 1 << 0,
  ABI47_0_0RCTTextSizeComparisonLarger = 1 << 1,
  ABI47_0_0RCTTextSizeComparisonWithinRange = 1 << 2,
};

@implementation NSTextStorage (ABI47_0_0FontScaling)

- (void)abi47_0_0_rct_scaleFontSizeToFitSize:(CGSize)size
               ...
               maximumFontSize:(CGFloat)maximumFontSize
{
  ...
  while (true) {
    [self abi47_0_0_rct_scaleFontSizeWithRatio:ratio minimumFontSize:minimumFontSize maximumFontSize:maximumFontSize];

    ABI47_0_0RCTTextSizeComparisonOptions comparsion = [self abi47_0_0_rct_compareToSize:size thresholdRatio:0.01];

    if ((comparsion & ABI47_0_0RCTTextSizeComparisonWithinRange) && (comparsion & ABI47_0_0RCTTextSizeComparisonSmaller)) {
      return;
    } else if (comparsion & ABI47_0_0RCTTextSizeComparisonSmaller) {
      bottomRatio = ratio;
    ...
    if (ABS(topRatio - bottomRatio) < kRatioThreshold || ABS(topRatio - ratio) < kRatioThreshold ||
      ...
      [self abi47_0_0_rct_scaleFontSizeWithRatio:lastRatioWhichFits minimumFontSize:minimumFontSize maximumFontSize:maximumFontSize];
      return;
    }
...
- (ABI47_0_0RCTTextSizeComparisonOptions)abi47_0_0_rct_compareToSize:(CGSize)size thresholdRatio:(CGFloat)thresholdRatio
{
  ...
  if (truncatedGlyphRange.location != NSNotFound) {
    return ABI47_0_0RCTTextSizeComparisonLarger;
  }
  ...
  ABI47_0_0RCTTextSizeComparisonOptions result = 0;

  result |= (fitsSize) ? ABI47_0_0RCTTextSizeComparisonSmaller : ABI47_0_0RCTTextSizeComparisonLarger;

  if (ABS(measuredSize.width - size.width) < thresholdSize.width) {
    result = result | ABI47_0_0RCTTextSizeComparisonWithinRange;
  }

  return result;
}

- (void)abi47_0_0_rct_scaleFontSizeWithRatio:(CGFloat)ratio
               minimumFontSize:(CGFloat)minimumFontSize
               maximumFontSize:(CGFloat)maximumFontSize
{
  [self beginEditing];
