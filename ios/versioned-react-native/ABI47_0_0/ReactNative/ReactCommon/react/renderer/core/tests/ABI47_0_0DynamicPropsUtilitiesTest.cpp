...
TEST(DynamicPropsUtilitiesTest, handleNestedObjects) {
  ...
  ABI47_0_0EXPECT_TRUE(result["style"].isObject());
  ABI47_0_0EXPECT_TRUE(result["style"]["backgroundColor"].isString());
  ABI47_0_0EXPECT_TRUE(result["style"]["color"].isString());
  ABI47_0_0EXPECT_TRUE(result["height"].isInt());

  ABI47_0_0EXPECT_EQ(result["style"]["backgroundColor"].asString(), "blue");
  ABI47_0_0EXPECT_EQ(result["style"]["color"], "black");
  ABI47_0_0EXPECT_EQ(result["height"], 100);
}
...
TEST(DynamicPropsUtilitiesTest, handleNull) {
  ...
  ABI47_0_0EXPECT_TRUE(result["height"].isNull());
}
