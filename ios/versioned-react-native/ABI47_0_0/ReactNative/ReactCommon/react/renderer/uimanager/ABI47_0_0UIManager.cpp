...
#include <ABI47_0_0React/ABI47_0_0renderer/uimanager/SurfaceRegistryBinding.h>
...
ShadowTree::Unique UIManager::stopSurface(SurfaceId surfaceId) const {
  ...
  // We execute JavaScript/ABI47_0_0React part of the process at the very end to minimize
...
void UIManager::unregisterCommitHook(
  ...
  auto iterator =
      std::find(commitHooks_.begin(), commitHooks_.end(), &commitHook);
  ABI47_0_0React_native_assert(iterator != commitHooks_.end());
