...
#include "ABI47_0_0UIManagerBinding.h"
...
#include <ABI47_0_0React/ABI47_0_0renderer/runtimescheduler/RuntimeSchedulerBinding.h>
...
void UIManagerBinding::dispatchEvent(
  ...
  auto instanceHandle = eventTarget
    ? [&]() {
      ...
      ABI47_0_0React_native_assert(payload.isObject());
  ...
  eventHandlerWrapper.callback.call(
      ...
      {std::move(instanceHandle),
       ...
       std::move(payload)});
  currentEventPriority_ = ABI47_0_0ReactEventPriority::Default;
}
...
jsi::Value UIManagerBinding::get(
  ...
  if (methodName == "unstable_DefaultEventPriority") {
    return jsi::Value(serialize(ABI47_0_0ReactEventPriority::Default));
  }

  if (methodName == "unstable_DiscreteEventPriority") {
    return jsi::Value(serialize(ABI47_0_0ReactEventPriority::Discrete));
  }
