...
#include "ABI47_0_0SurfaceRegistryBinding.h"
#include <ABI47_0_0React/ABI47_0_0renderer/debug/SystraceSection.h>
#include <ABI47_0_0React/ABI47_0_0renderer/uimanager/bindingUtils.h>
#include <ABI47_0_0React/ABI47_0_0renderer/uimanager/primitives.h>
#include "ABI47_0_0bindingUtils.h"
...
void SurfaceRegistryBinding::startSurface(
  ...
  auto isBridgeless = global.hasProperty(runtime, "ABI47_0_0RN$Bridgeless") &&
      global.getProperty(runtime, "ABI47_0_0RN$Bridgeless").asBool();

  if (isBridgeless) {
    if (!global.hasProperty(runtime, "ABI47_0_0RN$SurfaceRegistry")) {
      throw std::runtime_error(
          "SurfaceRegistryBinding::startSurface: Failed to start Surface \"" +
          moduleName + "\". global.ABI47_0_0RN$SurfaceRegistry was not installed.");
    }

    auto registry = global.getPropertyAsObject(runtime, "ABI47_0_0RN$SurfaceRegistry");
    ...
    if (moduleName != "LogBox" &&
        global.hasProperty(runtime, "ABI47_0_0RN$SurfaceRegistry")) {
      auto registry = global.getPropertyAsObject(runtime, "ABI47_0_0RN$SurfaceRegistry");
      auto method = registry.getPropertyAsFunction(runtime, "renderSurface");
...
void SurfaceRegistryBinding::stopSurface(
  ...
  if (isBridgeless) {
    if (!global.hasProperty(runtime, "ABI47_0_0RN$stopSurface")) {
      // ABI47_0_0ReactFabric module has not been loaded yet; there's no surface to stop.
      return;
    }
    // Bridgeless mode uses a custom JSI binding instead of callable module.
    global.getPropertyAsFunction(runtime, "ABI47_0_0RN$stopSurface")
        .call(runtime, {jsi::Value{surfaceId}});
  } else {
    callMethodOfModule(
        runtime,
        "ABI47_0_0ReactFabric",
