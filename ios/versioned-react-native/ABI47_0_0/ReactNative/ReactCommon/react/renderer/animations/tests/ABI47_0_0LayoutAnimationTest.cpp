...
#include <ABI47_0_0React/ABI47_0_0renderer/animations/LayoutAnimationDriver.h>
    ...
    for (int j = 0; j < stages; j++) {
      ...
      for (int k = 0; k < animation_frames + 2; k++) {
        ...
        if (k != (animation_frames + 1)) {
          ABI47_0_0EXPECT_TRUE(animationDriver->shouldOverridePullTransaction());
        } else if (!commits_conflicting_mutations) {
          ABI47_0_0EXPECT_FALSE(animationDriver->shouldOverridePullTransaction());
        }
        ...
        ABI47_0_0EXPECT_TRUE(transaction.has_value() || k == animation_frames);
