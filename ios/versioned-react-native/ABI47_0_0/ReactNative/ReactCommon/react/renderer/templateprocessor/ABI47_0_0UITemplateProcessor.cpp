...
#include "ABI47_0_0UITemplateProcessor.h"
...
ShadowNode::Shared UITemplateProcessor::runCommand(
    ...
    const ComponentDescriptorRegistry &componentDescriptorRegistry,
    const NativeModuleRegistry &nativeModuleRegistry,
    std::shared_ptr<const ABI47_0_0ReactNativeConfig> const &ABI47_0_0ReactNativeConfig) {
  ...
  if (opcode == "createNode") {
    ...
    registers[registerNumber] = ABI47_0_0ReactNativeConfig->getBool(param);
...
ShadowNode::Shared UITemplateProcessor::buildShadowTree(
  ...
  LOG(ERROR) << "ABI47_0_0React ui template missing returnRoot command :(";
