...
TEST(AttributedStringTest, testToDynamic) {
  ...
  ABI47_0_0EXPECT_EQ(result["string"], fragment.string);
  auto textAttribute = result["fragments"][0]["textAttributes"];
  ABI47_0_0EXPECT_EQ(textAttribute["foregroundColor"], toDynamic(text.foregroundColor));
  ABI47_0_0EXPECT_EQ(textAttribute["opacity"], text.opacity);
  ABI47_0_0EXPECT_EQ(textAttribute["fontStyle"], toString(text.fontStyle.value()));
  ABI47_0_0EXPECT_EQ(textAttribute["fontWeight"], toString(text.fontWeight.value()));
}
