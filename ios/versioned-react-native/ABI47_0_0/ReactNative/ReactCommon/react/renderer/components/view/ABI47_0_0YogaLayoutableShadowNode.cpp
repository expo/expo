...
#include "ABI47_0_0YogaLayoutableShadowNode.h"
...
static int FabricDefaultYogaLog(
    const ABI47_0_0YGConfigRef,
    const ABI47_0_0YGNodeRef,
  ...
  switch (level) {
    case ABI47_0_0YGLogLevelError:
      ABI47_0_0React_native_log_error(buffer.data());
      break;
    case ABI47_0_0YGLogLevelFatal:
      ABI47_0_0React_native_log_fatal(buffer.data());
      break;
    case ABI47_0_0YGLogLevelWarn:
      ABI47_0_0React_native_log_warn(buffer.data());
      break;
    case ABI47_0_0YGLogLevelInfo:
    case ABI47_0_0YGLogLevelDebug:
    case ABI47_0_0YGLogLevelVerbose:
    default:
      ABI47_0_0React_native_log_info(buffer.data());
  }
...
YogaLayoutableShadowNode::YogaLayoutableShadowNode(
  ...
  // This is not a default for `ABI47_0_0YGNode`.
...
YogaLayoutableShadowNode::YogaLayoutableShadowNode(
  ...
  // Note, cloned `ABI47_0_0YGNode` instance (copied using copy-constructor) inherits
...
void YogaLayoutableShadowNode::adoptYogaChild(size_t index) {
  ...
  // Overflow checks.
  ABI47_0_0React_native_assert(children.size() > index);
  ABI47_0_0React_native_assert(children.size() >= yogaNode_.getChildren().size());
  ...
  // address). ABI47_0_0React_native_assert(layoutableChildNode.yogaNode_.getOwner() !=
  // &yogaNode_);
  ...
  if (layoutableChildNode.yogaNode_.getOwner() == nullptr) {
    ...
    // ABI47_0_0React_native_assert(layoutableChildNode.yogaNode_.isDirty());
  } else {
...
void YogaLayoutableShadowNode::appendChild(
  ...
  // (e.g. ABI47_0_0RCTRawText). This used to throw an error, but we are ignoring it
  ...
  if (traitCast<YogaLayoutableShadowNode const *>(childNode.get())) {
    ...
    ABI47_0_0React_native_log_error(
...
void YogaLayoutableShadowNode::updateYogaChildrenOwnersIfNeeded() {
  for (auto &childYogaNode : yogaNode_.getChildren()) {
    if (childYogaNode->getOwner() == &yogaNode_) {
      childYogaNode->setOwner(reinterpret_cast<ABI47_0_0YGNodeRef>(0xBADC0FFEE0DDF00D));
    }
  }
}
...
void YogaLayoutableShadowNode::updateYogaChildren() {
  ...
  auto oldYogaChildren = isClean ? yogaNode_.getChildren() : ABI47_0_0YGVector{};
  ...
  ABI47_0_0React_native_assert(getChildren().size() == yogaNode_.getChildren().size());
...
void YogaLayoutableShadowNode::setSize(Size size) const {
  ...
  style.dimensions()[ABI47_0_0YGDimensionWidth] = yogaStyleValueFromFloat(size.width);
  style.dimensions()[ABI47_0_0YGDimensionHeight] = yogaStyleValueFromFloat(size.height);
...
void YogaLayoutableShadowNode::setPadding(RectangleEdges<Float> padding) const {
  ...
  if (leftPadding != style.padding()[ABI47_0_0YGEdgeLeft] ||
      topPadding != style.padding()[ABI47_0_0YGEdgeTop] ||
      rightPadding != style.padding()[ABI47_0_0YGEdgeRight] ||
      bottomPadding != style.padding()[ABI47_0_0YGEdgeBottom]) {
    style.padding()[ABI47_0_0YGEdgeTop] = yogaStyleValueFromFloat(padding.top);
    style.padding()[ABI47_0_0YGEdgeLeft] = yogaStyleValueFromFloat(padding.left);
    style.padding()[ABI47_0_0YGEdgeRight] = yogaStyleValueFromFloat(padding.right);
    style.padding()[ABI47_0_0YGEdgeBottom] = yogaStyleValueFromFloat(padding.bottom);
    yogaNode_.setStyle(style);
    yogaNode_.setDirty(true);
  }
}
...
void YogaLayoutableShadowNode::layoutTree(
  ...
  ABI47_0_0React_native_assert(!std::isnan(minimumSize.width));
  ABI47_0_0React_native_assert(!std::isnan(minimumSize.height));
  ABI47_0_0React_native_assert(!std::isnan(maximumSize.width));
  ABI47_0_0React_native_assert(!std::isnan(maximumSize.height));
  // Values cannot be negative.
  ABI47_0_0React_native_assert(minimumSize.width >= 0);
  ABI47_0_0React_native_assert(minimumSize.height >= 0);
  ABI47_0_0React_native_assert(maximumSize.width >= 0);
  ABI47_0_0React_native_assert(maximumSize.height >= 0);
  // Mimimum size cannot be infinity.
  ABI47_0_0React_native_assert(!std::isinf(minimumSize.width));
  ABI47_0_0React_native_assert(!std::isinf(minimumSize.height));
  ...
  // Module`. Yoga C++ API (and `ABI47_0_0YGNodeCalculateLayout` function particularly)
  ...
  yogaStyle.maxDimensions()[ABI47_0_0YGDimensionWidth] = std::isfinite(maximumSize.width)
      ? yogaStyleValueFromFloat(maximumSize.width)
      : ABI47_0_0YGValueUndefined;

  yogaStyle.maxDimensions()[ABI47_0_0YGDimensionHeight] =
      std::isfinite(maximumSize.height)
      ? yogaStyleValueFromFloat(maximumSize.height)
      : ABI47_0_0YGValueUndefined;

  yogaStyle.minDimensions()[ABI47_0_0YGDimensionWidth] = minimumSize.width > 0
      ? yogaStyleValueFromFloat(minimumSize.width)
      : ABI47_0_0YGValueUndefined;

  yogaStyle.minDimensions()[ABI47_0_0YGDimensionHeight] = minimumSize.height > 0
  ...
  {
    SystraceSection s("YogaLayoutableShadowNode::ABI47_0_0YGNodeCalculateLayout");
    ABI47_0_0YGNodeCalculateLayout(&yogaNode_, ownerWidth, ownerHeight, direction);
  }
...
void YogaLayoutableShadowNode::layout(LayoutContext layoutContext) {
  ...
  ABI47_0_0React_native_assert(!yogaNode_.isDirty());
  ...
  for (auto childYogaNode : yogaNode_.getChildren()) {
    ...
    ABI47_0_0React_native_assert(&childNode.yogaNode_ == childYogaNode);

    if (childYogaNode->getHasNewLayout()) {
      childYogaNode->setHasNewLayout(false);

      // Reading data from a dirtied node does not make sense.
      ABI47_0_0React_native_assert(!childYogaNode->isDirty());

      // We must copy layout metrics from Yoga node only once (when the parent
      // node exclusively ownes the child node).
      ABI47_0_0React_native_assert(childYogaNode->getOwner() == &yogaNode_);
  ...
  if (yogaNode_.getStyle().overflow() == ABI47_0_0YGOverflowVisible) {
...
ABI47_0_0YGNode *YogaLayoutableShadowNode::yogaNodeCloneCallbackConnector(
...
ABI47_0_0YGSize YogaLayoutableShadowNode::yogaNodeMeasureCallbackConnector(
...
ABI47_0_0YGConfig &YogaLayoutableShadowNode::initializeYogaConfig(ABI47_0_0YGConfig &config) {
  config.setCloneNodeCallback(
      YogaLayoutableShadowNode::yogaNodeCloneCallbackConnector);
  config.useLegacyStretchBehaviour = true;
#ifdef ABI47_0_0RN_DEBUG_YOGA_LOGGER
...
void YogaLayoutableShadowNode::swapLeftAndRightInYogaStyleProps(
  ...
  ABI47_0_0YGStyle::Edges const &position = yogaStyle.position();
  ABI47_0_0YGStyle::Edges const &padding = yogaStyle.padding();
  ABI47_0_0YGStyle::Edges const &margin = yogaStyle.margin();

  // Swap Yoga node values, position, padding and margin.

  if (yogaStyle.position()[ABI47_0_0YGEdgeLeft] != ABI47_0_0YGValueUndefined) {
    yogaStyle.position()[ABI47_0_0YGEdgeStart] = position[ABI47_0_0YGEdgeLeft];
    yogaStyle.position()[ABI47_0_0YGEdgeLeft] = ABI47_0_0YGValueUndefined;
  }

  if (yogaStyle.position()[ABI47_0_0YGEdgeRight] != ABI47_0_0YGValueUndefined) {
    yogaStyle.position()[ABI47_0_0YGEdgeEnd] = position[ABI47_0_0YGEdgeRight];
    yogaStyle.position()[ABI47_0_0YGEdgeRight] = ABI47_0_0YGValueUndefined;
  }

  if (yogaStyle.padding()[ABI47_0_0YGEdgeLeft] != ABI47_0_0YGValueUndefined) {
    yogaStyle.padding()[ABI47_0_0YGEdgeStart] = padding[ABI47_0_0YGEdgeLeft];
    yogaStyle.padding()[ABI47_0_0YGEdgeLeft] = ABI47_0_0YGValueUndefined;
  }

  if (yogaStyle.padding()[ABI47_0_0YGEdgeRight] != ABI47_0_0YGValueUndefined) {
    yogaStyle.padding()[ABI47_0_0YGEdgeEnd] = padding[ABI47_0_0YGEdgeRight];
    yogaStyle.padding()[ABI47_0_0YGEdgeRight] = ABI47_0_0YGValueUndefined;
  }

  if (yogaStyle.margin()[ABI47_0_0YGEdgeLeft] != ABI47_0_0YGValueUndefined) {
    yogaStyle.margin()[ABI47_0_0YGEdgeStart] = margin[ABI47_0_0YGEdgeLeft];
    yogaStyle.margin()[ABI47_0_0YGEdgeLeft] = ABI47_0_0YGValueUndefined;
  }

  if (yogaStyle.margin()[ABI47_0_0YGEdgeRight] != ABI47_0_0YGValueUndefined) {
    yogaStyle.margin()[ABI47_0_0YGEdgeEnd] = margin[ABI47_0_0YGEdgeRight];
    yogaStyle.margin()[ABI47_0_0YGEdgeRight] = ABI47_0_0YGValueUndefined;
  }
...
void YogaLayoutableShadowNode::swapLeftAndRightInViewProps(
  ...
  ABI47_0_0YGStyle::Edges const &border = props.yogaStyle.border();

  if (props.yogaStyle.border()[ABI47_0_0YGEdgeLeft] != ABI47_0_0YGValueUndefined) {
    props.yogaStyle.border()[ABI47_0_0YGEdgeStart] = border[ABI47_0_0YGEdgeLeft];
    props.yogaStyle.border()[ABI47_0_0YGEdgeLeft] = ABI47_0_0YGValueUndefined;
  }

  if (props.yogaStyle.border()[ABI47_0_0YGEdgeRight] != ABI47_0_0YGValueUndefined) {
    props.yogaStyle.border()[ABI47_0_0YGEdgeEnd] = border[ABI47_0_0YGEdgeRight];
    props.yogaStyle.border()[ABI47_0_0YGEdgeRight] = ABI47_0_0YGValueUndefined;
  }
}
...
void YogaLayoutableShadowNode::ensureYogaChildrenOwnersConsistency() const {
  ...
  if (!yogaChildren.empty()) {
    ...
    for (auto const &child : yogaChildren) {
      ABI47_0_0React_native_assert(child->getOwner() == owner);
    }
  }
#endif
}
...
void YogaLayoutableShadowNode::ensureYogaChildrenLookFine() const {
  ...
  for (auto const &yogaChild : yogaChildren) {
    ABI47_0_0React_native_assert(yogaChild->getContext());
    ABI47_0_0React_native_assert(yogaChild->getChildren().size() < 16384);
    if (!yogaChild->getChildren().empty()) {
      ABI47_0_0React_native_assert(!yogaChild->hasMeasureFunc());
    }
  }
#endif
}
...
void YogaLayoutableShadowNode::ensureYogaChildrenAlighment() const {
  ...
  if (getTraits().check(ShadowNodeTraits::Trait::LeafYogaNode)) {
    ABI47_0_0React_native_assert(yogaChildren.empty());
    return;
  }

  ABI47_0_0React_native_assert(yogaChildren.size() == children.size());
