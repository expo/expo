...
#import "ABI47_0_0EXScopedNotificationCategoriesModule.h"
#import "ABI47_0_0EXScopedNotificationCategoryMigrator.h"
#import "ABI47_0_0EXScopedNotificationsUtils.h"

@interface ABI47_0_0EXScopedNotificationCategoriesModule ()

@property (nonatomic, strong) NSString *scopeKey;

@end

@implementation ABI47_0_0EXScopedNotificationCategoriesModule
...
- (void)getNotificationCategoriesAsyncWithResolver:(ABI47_0_0EXPromiseResolveBlock)resolve reject:(ABI47_0_0EXPromiseRejectBlock)reject
{
  [[UNUserNotificationCenter currentNotificationCenter] getNotificationCategoriesWithCompletionHandler:^(NSSet<UNNotificationCategory *> *categories) {
    ...
    for (UNNotificationCategory *category in categories) {
      if ([ABI47_0_0EXScopedNotificationsUtils isId:category.identifier scopedByExperience:self->_scopeKey]) {
...
{
  NSString *scopedCategoryIdentifier = [ABI47_0_0EXScopedNotificationsUtils scopedIdentifierFromId:categoryId
...
- (NSMutableDictionary *)serializeCategory:(UNNotificationCategory *)category
{
  ...
  serializedCategory[@"identifier"] = [ABI47_0_0EXScopedNotificationsUtils getScopeAndIdentifierFromScopedIdentifier:serializedCategory[@"identifier"]].identifier;
                                                                         ...
                                                                         isInExpoGo:(BOOL)isInExpoGo __deprecated_msg("To be removed once SDK 43 is phased out")
{
  if (isInExpoGo) {
    ...
    [ABI47_0_0EXScopedNotificationCategoryMigrator migrateLegacyScopedCategoryIdentifiersForProjectWithScopeKey:scopeKey];
  } else {
    // Used to prefix with "experienceId-" even in standalone apps in SDKs <= 40, so we need to unscope those
    [ABI47_0_0EXScopedNotificationCategoryMigrator unscopeLegacyCategoryIdentifiersForProjectWithScopeKey:scopeKey];
  }
}
