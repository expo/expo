...
#import "ABI47_0_0EXScopedFirebaseCore.h"
...
@implementation ABI47_0_0EXScopedFirebaseCore {
  ...
  for (NSString* name in names) {
    if (!protectedAppNames[name] && (!options || ![name isEqualToString:appName])) {
      [[FIRApp appNamed:name] deleteApp:^(BOOL success) {
        if (!success) {
          ABI47_0_0EXLogWarn(@"Failed to delete Firebase app: %@", name);
        }
      }];
    }
  }
...
+ (nullable NSDictionary *)googleServicesFileFromManifest:(ABI47_0_0EXManifestsManifest *)manifest
{
  ...
  @try {
    ...
    if (error) ABI47_0_0EXLogWarn(@"Invalid googleServicesFile: %@", error);
    return plist;
  }
  @catch(NSException* exception) {
    ABI47_0_0EXLogWarn(@"Invalid googleServicesFile: %@", exception);
    return nil;
  }
}
