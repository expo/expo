...
#import "ABI47_0_0AIRGMSMarker.h"
...
@interface ABI47_0_0AIRGoogleMapMarker ()
- (id)eventFromMarker:(ABI47_0_0AIRGMSMarker*)marker;
@end

@implementation ABI47_0_0AIRGoogleMapMarker {
...
{
  if ((self = [super init])) {
    _realMarker = [[ABI47_0_0AIRGMSMarker alloc] init];
...
- (void)insertABI47_0_0ReactSubview:(id<ABI47_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  if ([subview isKindOfClass:[ABI47_0_0AIRGoogleMapCallout class]]) {
    self.calloutView = (ABI47_0_0AIRGoogleMapCallout *)subview;
  } else { // a child view of the marker
    [self iconViewInsertSubview:(UIView*)subview atIndex:atIndex+1];
  }
  ABI47_0_0AIRDummyView *dummySubview = [[ABI47_0_0AIRDummyView alloc] initWithView:(UIView *)subview];
  [super insertABI47_0_0ReactSubview:(UIView*)dummySubview atIndex:atIndex];
}

- (void)removeABI47_0_0ReactSubview:(id<ABI47_0_0RCTComponent>)dummySubview {
  UIView* subview = ((ABI47_0_0AIRDummyView*)dummySubview).view;
  ...
  [super removeABI47_0_0ReactSubview:(UIView*)dummySubview];
}
...
- (void)didTapInfoWindowOfMarker:(ABI47_0_0AIRGMSMarker *)marker point:(CGPoint)point frame:(CGRect)frame {
...
- (void)didTapInfoWindowOfMarker:(ABI47_0_0AIRGMSMarker *)marker {
    [self didTapInfoWindowOfMarker:marker point:CGPointMake(-1, -1) frame:CGRectZero];
}

- (void)didTapInfoWindowOfMarker:(ABI47_0_0AIRGMSMarker *)marker subview:(ABI47_0_0AIRGoogleMapCalloutSubview*)subview point:(CGPoint)point frame:(CGRect)frame {
...
- (void)didBeginDraggingMarker:(ABI47_0_0AIRGMSMarker *)marker {
  if (!self.onDragStart) return;
  self.onDragStart([self eventFromMarker:marker]);
}

- (void)didEndDraggingMarker:(ABI47_0_0AIRGMSMarker *)marker {
  if (!self.onDragEnd) return;
  self.onDragEnd([self eventFromMarker:marker]);
}

- (void)didDragMarker:(ABI47_0_0AIRGMSMarker *)marker {
...
- (ABI47_0_0RCTBubblingEventBlock)onPress {
...
- (void)setImageSrc:(NSString *)imageSrc
{
  ...
  if (!_iconImageView) {
    // prevent glitch with marker (cf. https://github.com/ABI47_0_0React-native-maps/ABI47_0_0React-native-maps/issues/738)
...
//                                                                   }
                                                                   ...
                                                                   //       Shouldn't required images have size info automatically via ABI47_0_0RN?
...
- (void)setIconSrc:(NSString *)iconSrc
{
  ...
  if (!_realMarker.icon) {
    // prevent glitch with marker (cf. https://github.com/ABI47_0_0React-native-maps/ABI47_0_0React-native-maps/issues/3657)
    UIImage *emptyImage = [[UIImage alloc] init];
    _realMarker.icon = emptyImage;
  }

  _reloadImageCancellationBlock =
  [[_bridge moduleForName:@"ImageLoader"] loadImageWithURLRequest:[ABI47_0_0RCTConvert NSURLRequest:_iconSrc]
                                          size:self.bounds.size
                                         scale:ABI47_0_0RCTScreenScale()
                                       clipped:YES
                                    resizeMode:ABI47_0_0RCTResizeModeCenter
