...
const CLLocationDegrees ABI47_0_0AIRMapDefaultSpan = 0.005;
const NSTimeInterval ABI47_0_0AIRMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI47_0_0AIRMapZoomBoundBuffer = 0.01;
const NSInteger ABI47_0_0AIRMapMaxZoomLevel = 20;
...
@implementation ABI47_0_0AIRMap
{
    UIView *_legalLabel;
    BOOL _initialRegionSet;
    BOOL _initialCameraSet;

    // Array to manually track ABI47_0_0RN subviews
    //
    // ABI47_0_0AIRMap implicitly creates subviews that aren't regular ABI47_0_0RN children
    // (ABI47_0_0SMCalloutView injects an overlay subview), which otherwise confuses ABI47_0_0RN
    // during component re-renders:
    // https://github.com/facebook/ABI47_0_0React-native/blob/v0.16.0/ABI47_0_0React/Modules/ABI47_0_0RCTUIManager.m#L657
    //
    // Implementation based on ABI47_0_0RCTTextField, another component with indirect children
    // https://github.com/facebook/ABI47_0_0React-native/blob/v0.16.0/Libraries/Text/ABI47_0_0RCTTextField.m#L20
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        self.calloutView = [ABI47_0_0SMCalloutView platformCalloutView];
        self.calloutView.delegate = self;

        self.minZoomLevel = 0;
        self.maxZoomLevel = ABI47_0_0AIRMapMaxZoomLevel;
...
- (void)insertABI47_0_0ReactSubview:(id<ABI47_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    ...
    if ([subview isKindOfClass:[ABI47_0_0AIRMapMarker class]]) {
        [self addAnnotation:(id <MKAnnotation>) subview];
    } else if ([subview isKindOfClass:[ABI47_0_0AIRMapPolyline class]]) {
        ((ABI47_0_0AIRMapPolyline *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI47_0_0AIRMapPolygon class]]) {
        ((ABI47_0_0AIRMapPolygon *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI47_0_0AIRMapCircle class]]) {
        ((ABI47_0_0AIRMapCircle *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI47_0_0AIRMapUrlTile class]]) {
        ((ABI47_0_0AIRMapUrlTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    }else if ([subview isKindOfClass:[ABI47_0_0AIRMapWMSTile class]]) {
        ((ABI47_0_0AIRMapWMSTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI47_0_0AIRMapLocalTile class]]) {
        ((ABI47_0_0AIRMapLocalTile *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else if ([subview isKindOfClass:[ABI47_0_0AIRMapOverlay class]]) {
        ((ABI47_0_0AIRMapOverlay *)subview).map = self;
        [self addOverlay:(id<MKOverlay>)subview];
    } else {
        NSArray<id<ABI47_0_0RCTComponent>> *childSubviews = [subview ABI47_0_0ReactSubviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self insertABI47_0_0ReactSubview:(UIView *)childSubviews[i] atIndex:atIndex];
        }
    }
    [_ABI47_0_0ReactSubviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeABI47_0_0ReactSubview:(id<ABI47_0_0RCTComponent>)subview {
    ...
    if ([subview isKindOfClass:[ABI47_0_0AIRMapMarker class]]) {
        ...
        NSArray<id<ABI47_0_0RCTComponent>> *childSubviews = [subview ABI47_0_0ReactSubviews];
        for (int i = 0; i < childSubviews.count; i++) {
          [self removeABI47_0_0ReactSubview:(UIView *)childSubviews[i]];
        }
    }
    [_ABI47_0_0ReactSubviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (NSArray*) markers {
    NSPredicate *filterMarkers = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        ...
        return [marker isKindOfClass:[ABI47_0_0AIRMapMarker class]];
    }];
    NSArray *filteredMarkers = [self.annotations filteredArrayUsingPredicate:filterMarkers];
    return filteredMarkers;
}

- (ABI47_0_0AIRMapMarker*) markerForCallout:(ABI47_0_0AIRMapCallout*)callout {
    ABI47_0_0AIRMapMarker* marker = nil;
    NSArray* markers = [self markers];
    for (ABI47_0_0AIRMapMarker* mrk in markers) {
...
- (CGRect) frameForMarker:(ABI47_0_0AIRMapMarker*) mrkAnn {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI47_0_0AIRMapMarker* mrkAnn in self.markers) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (touchedView) {
        ...
        ABI47_0_0AIRMapCallout* callout = nil;
        ABI47_0_0AIRMapMarker* marker = nil;
        ...
        while (tmp && tmp != win && tmp != self.calloutView) {
            ...
            if ([tmp isKindOfClass:[ABI47_0_0AIRMapCallout class]]) {
                callout = (ABI47_0_0AIRMapCallout*) tmp;
                break;
            }
            tmp = tmp.superview;
        }
...
#pragma mark ABI47_0_0SMCalloutViewDelegate
...
- (void)cacheViewIfNeeded {
    // https://github.com/ABI47_0_0React-native-maps/ABI47_0_0React-native-maps/issues/3100
