...
static NSString *const ABI16_0_0RCTImageStoreURLScheme = @"rct-image-store";

@implementation ABI16_0_0RCTImageStoreManager
{
...
{
  ABI16_0_0RCTAssertThread(_methodQueue, @"Must be called on ABI16_0_0RCTImageStoreManager thread");
  ...
  NSString *imageTag = [NSString stringWithFormat:@"%@://%tu", ABI16_0_0RCTImageStoreURLScheme, _id++];
...
- (void)storeImage:(UIImage *)image withBlock:(void (^)(NSString *imageTag))block
{
  ABI16_0_0RCTAssertParam(block);
  dispatch_async(_methodQueue, ^{
    NSString *imageTag = [self _storeImageData:ABI16_0_0RCTGetImageData(image.CGImage, 0.75)];
...
ABI16_0_0RCT_EXPORT_METHOD(removeImageForTag:(NSString *)imageTag)
{
  [_store removeObjectForKey:imageTag];
}

ABI16_0_0RCT_EXPORT_METHOD(hasImageForTag:(NSString *)imageTag
...
ABI16_0_0RCT_EXPORT_METHOD(getBase64ForTag:(NSString *)imageTag
                  ...
                  errorCallback:(ABI16_0_0RCTResponseErrorBlock)errorCallback)
{
  ...
  if (!imageData) {
    errorCallback(ABI16_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]));
    return;
  }
...
ABI16_0_0RCT_EXPORT_METHOD(addImageFromBase64:(NSString *)base64String
                  successCallback:(ABI16_0_0RCTResponseSenderBlock)successCallback
                  errorCallback:(ABI16_0_0RCTResponseErrorBlock)errorCallback)

{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (imageData) {
      ...
      errorCallback(ABI16_0_0RCTErrorWithMessage(@"Failed to add image from base64String"));
    }
  });
}

#pragma mark - ABI16_0_0RCTURLRequestHandler

- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  return [request.URL.scheme caseInsensitiveCompare:ABI16_0_0RCTImageStoreURLScheme] == NSOrderedSame;
}

- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI16_0_0RCTURLRequestDelegate>)delegate
{
  ...
  dispatch_async(_methodQueue, ^{
    ...
    if (!imageData) {
      NSError *error = ABI16_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Invalid imageTag: %@", imageTag]);
    ...
    if (!sourceRef) {
      NSError *error = ABI16_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to decode data for imageTag: %@", imageTag]);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
      return;
    }
...
@implementation ABI16_0_0RCTImageStoreManager (Deprecated)

- (NSString *)storeImage:(UIImage *)image
{
  ABI16_0_0RCTAssertMainQueue();
  ABI16_0_0RCTLogWarn(@"ABI16_0_0RCTImageStoreManager.storeImage() is deprecated and has poor performance. Use an alternative method instead.");
  __block NSString *imageTag;
  dispatch_sync(_methodQueue, ^{
    imageTag = [self _storeImageData:ABI16_0_0RCTGetImageData(image.CGImage, 0.75)];
  });
  return imageTag;
}
...
{
  ABI16_0_0RCTAssertMainQueue();
  ABI16_0_0RCTLogWarn(@"ABI16_0_0RCTImageStoreManager.imageForTag() is deprecated and has poor performance. Use an alternative method instead.");
...
@implementation ABI16_0_0RCTBridge (ABI16_0_0RCTImageStoreManager)

- (ABI16_0_0RCTImageStoreManager *)imageStoreManager
{
  return [self moduleForClass:[ABI16_0_0RCTImageStoreManager class]];
}
