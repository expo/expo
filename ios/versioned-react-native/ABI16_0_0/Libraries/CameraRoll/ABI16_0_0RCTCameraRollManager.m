...
#import "ABI16_0_0RCTCameraRollManager.h"
...
#import <ReactABI16_0_0/ABI16_0_0RCTImageLoader.h>
#import <ReactABI16_0_0/ABI16_0_0RCTLog.h>
#import <ReactABI16_0_0/ABI16_0_0RCTUtils.h>

#import "ABI16_0_0RCTAssetsLibraryRequestHandler.h"

@implementation ABI16_0_0RCTConvert (ALAssetGroup)

ABI16_0_0RCT_ENUM_CONVERTER(ALAssetsGroupType, (@{
...
+ (ALAssetsFilter *)ALAssetsFilter:(id)json
{
  ...
  if (!filter) {
    ABI16_0_0RCTLogError(@"Invalid filter option: '%@'. Expected one of 'photos',"
                "'videos' or 'all'.", json);
  }
...
@implementation ABI16_0_0RCTCameraRollManager

ABI16_0_0RCT_EXPORT_MODULE()

@synthesize bridge = _bridge;

NSString *const ABI16_0_0RCTErrorUnableToLoad = @"E_UNABLE_TO_LOAD";
NSString *const ABI16_0_0RCTErrorUnableToSave = @"E_UNABLE_TO_SAVE";

ABI16_0_0RCT_EXPORT_METHOD(saveToCameraRoll:(NSURLRequest *)request
                  type:(NSString *)type
  ...
  if ([type isEqualToString:@"video"]) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [self->_bridge.assetsLibrary writeVideoAtPathToSavedPhotosAlbum:request.URL completionBlock:^(NSURL *assetURL, NSError *saveError) {
        if (saveError) {
          reject(ABI16_0_0RCTErrorUnableToSave, nil, saveError);
        } else {
    ...
    [_bridge.imageLoader loadImageWithURLRequest:request
      ...
      if (loadError) {
        reject(ABI16_0_0RCTErrorUnableToLoad, nil, loadError);
        return;
      }
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        [self->_bridge.assetsLibrary writeImageToSavedPhotosAlbum:loadedImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *saveError) {
          if (saveError) {
            ABI16_0_0RCTLogWarn(@"Error saving cropped image: %@", saveError);
            reject(ABI16_0_0RCTErrorUnableToSave, nil, saveError);
          } else {
            resolve(assetURL.absoluteString);
          }
...
static void ABI16_0_0RCTResolvePromise(ABI16_0_0RCTPromiseResolveBlock resolve,
...
ABI16_0_0RCT_EXPORT_METHOD(getPhotos:(NSDictionary *)params
                  resolve:(ABI16_0_0RCTPromiseResolveBlock)resolve
                  reject:(ABI16_0_0RCTPromiseRejectBlock)reject)
{
  checkPhotoLibraryConfig();

  NSUInteger first = [ABI16_0_0RCTConvert NSInteger:params[@"first"]];
  NSString *afterCursor = [ABI16_0_0RCTConvert NSString:params[@"after"]];
  NSString *groupName = [ABI16_0_0RCTConvert NSString:params[@"groupName"]];
  ALAssetsFilter *assetType = [ABI16_0_0RCTConvert ALAssetsFilter:params[@"assetType"]];
  ALAssetsGroupType groupTypes = [ABI16_0_0RCTConvert ALAssetsGroupType:params[@"groupTypes"]];
  ...
  [_bridge.assetsLibrary enumerateGroupsWithTypes:groupTypes usingBlock:^(ALAssetsGroup *group, BOOL *stopGroups) {
      ...
      [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stopAssets) {
        if (result) {
          ...
          if (first == assets.count) {
            ...
            *stopGroups = YES;
            hasNextPage = YES;
            ABI16_0_0RCTAssert(resolvedPromise == NO, @"Resolved the promise before we finished processing the results.");
            ABI16_0_0RCTResolvePromise(resolve, assets, hasNextPage);
            resolvedPromise = YES;
            return;
          }
    ...
    if (error.code != ALAssetsLibraryAccessUserDeniedError) {
      ABI16_0_0RCTLogError(@"Failure while iterating through asset groups %@", error);
    }
    reject(ABI16_0_0RCTErrorUnableToLoad, nil, error);
  }];
}
...
static void checkPhotoLibraryConfig()
{
  ...
  if (![[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSPhotoLibraryUsageDescription"]) {
    ABI16_0_0RCTLogError(@"NSPhotoLibraryUsageDescription key must be present in Info.plist to use camera roll.");
  }
#endif
}
