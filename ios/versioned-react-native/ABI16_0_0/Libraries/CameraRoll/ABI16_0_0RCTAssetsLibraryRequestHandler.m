...
@implementation ABI16_0_0RCTAssetsLibraryRequestHandler
{
     ...
     withDelegate:(id<ABI16_0_0RCTURLRequestDelegate>)delegate
{
  ...
  [[self assetsLibrary] assetForURL:request.URL resultBlock:^(ALAsset *asset) {
    ...
    if (asset) {
      ...
      NSError *error = ABI16_0_0RCTErrorWithMessage(errorMessage);
      [delegate URLRequest:cancellationBlock didCompleteWithError:error];
    }
...
@implementation ABI16_0_0RCTBridge (ABI16_0_0RCTAssetsLibraryImageLoader)

- (ALAssetsLibrary *)assetsLibrary
{
  return [[self moduleForClass:[ABI16_0_0RCTAssetsLibraryRequestHandler class]] assetsLibrary];
}
