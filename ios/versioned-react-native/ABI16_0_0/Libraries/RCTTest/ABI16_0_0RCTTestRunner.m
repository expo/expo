...
#import "ABI16_0_0RCTTestRunner.h"
...
#import "ABI16_0_0FBSnapshotTestController.h"
#import "ABI16_0_0RCTTestModule.h"

static const NSTimeInterval kTestTimeoutSeconds = 120;
static const NSTimeInterval kTestTeardownTimeoutSeconds = 30;

@implementation ABI16_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI16_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI16_0_0RCTAssertParam(app);
  ABI16_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI16_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI16_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
configurationBlock:(void(^)(ABI16_0_0RCTRootView *rootView))configurationBlock
{
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI16_0_0RCTSetLogFunction(^(ABI16_0_0RCTLogLevel level, ABI16_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI16_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI16_0_0RCTBridge *bridge = [[ABI16_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI16_0_0RCTRootView *rootView = [[ABI16_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
#if TARGET_OS_TV
    ...
    ABI16_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI16_0_0RCTTestModule class]];
    ABI16_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI16_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI16_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI16_0_0RCTSetLogFunction(ABI16_0_0RCTDefaultLogFunction);
    ...
    ABI16_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI16_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI16_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI16_0_0RCTAssert(testModule.status != ABI16_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI16_0_0RCTAssert(testModule.status == ABI16_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI16_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
