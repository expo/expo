...
#import "ABI16_0_0RCTRootViewDelegate.h"
...
#if TARGET_OS_TV
#import "ABI16_0_0RCTTVRemoteHandler.h"
#import "ABI16_0_0RCTTVNavigationEventEmitter.h"
#endif

NSString *const ABI16_0_0RCTContentDidAppearNotification = @"ABI16_0_0RCTContentDidAppearNotification";

@interface ABI16_0_0RCTUIManager (ABI16_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI16_0_0RCTRootView
{
  ABI16_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  NSDictionary *_launchOptions;
  ABI16_0_0RCTRootContentView *_contentView;
             ...
             initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI16_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI16_0_0RCTRootView");
  ABI16_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI16_0_0RCTRootView");

  ABI16_0_0RCT_PROFILE_BEGIN_EVENT(ABI16_0_0RCTProfileTagAlways, @"-[ABI16_0_0RCTRootView init]", nil);
  ...
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    _sizeFlexibility = ABI16_0_0RCTRootViewSizeFlexibilityNone;
    self.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidReload)
                                                 name:ABI16_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI16_0_0RCTContentDidAppearNotification
                                               object:self];

#if TARGET_OS_TV
    self.tvRemoteHandler = [ABI16_0_0RCTTVRemoteHandler new];
...
{
  ABI16_0_0RCTBridge *bridge = [[ABI16_0_0RCTBridge alloc] initWithBundleURL:bundleURL
                                            moduleProvider:nil
...
{
  if (self.ReactABI16_0_0PreferredFocusedView) {
    return self.ReactABI16_0_0PreferredFocusedView;
  }
...
{
  return CGSizeMake(
    _sizeFlexibility & ABI16_0_0RCTRootViewSizeFlexibilityWidth ? MIN(_intrinsicContentSize.width, size.width) : size.width,
    _sizeFlexibility & ABI16_0_0RCTRootViewSizeFlexibilityHeight ? MIN(_intrinsicContentSize.height, size.height) : size.height
  );
}
...
{
  return _ReactABI16_0_0ViewController ?: [super ReactABI16_0_0ViewController];
}
...
{
  ABI16_0_0RCTAssertMainQueue();
  if (!super.ReactABI16_0_0Tag) {
    /**
     * Every root view that is created must have a unique ReactABI16_0_0 tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ReactABI16_0_0 tag must be re-assigned every time a new UIManager is created.
     */
    self.ReactABI16_0_0Tag = [_bridge.uiManager allocateRootTag];
  }
  return super.ReactABI16_0_0Tag;
}

- (void)bridgeDidReload
{
  ABI16_0_0RCTAssertMainQueue();
  // Clear the ReactABI16_0_0Tag so it can be re-assigned
  self.ReactABI16_0_0Tag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI16_0_0RCTRootContentView is scoped to the right bridge
  ABI16_0_0RCTBridge *bridge = notification.userInfo[@"bridge"];
  [self bundleFinishedLoading:bridge];
}

- (void)bundleFinishedLoading:(ABI16_0_0RCTBridge *)bridge
{
  if (!bridge.valid) {
    return;
  }
  ...
  _contentView = [[ABI16_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
  ...
  if (_sizeFlexibility == ABI16_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI16_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag": _contentView.ReactABI16_0_0Tag,
    @"initialProps": _appProperties ?: @{},
  };

  ABI16_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
@implementation ABI16_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI16_0_0RCTLogWarn(@"Calling deprecated `[-ABI16_0_0RCTRootView intrinsicSize]`.");
  return self.intrinsicContentSize;
}

@end

@implementation ABI16_0_0RCTUIManager (ABI16_0_0RCTRootView)
