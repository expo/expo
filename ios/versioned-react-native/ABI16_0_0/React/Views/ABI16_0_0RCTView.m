...
@implementation UIView (ABI16_0_0RCTViewUnmounting)
...
- (void)ReactABI16_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI16_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI16_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI16_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ReactABI16_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI16_0_0RootView]) {
      break;
    }
...
static NSString *ABI16_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI16_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI16_0_0RCTView
{
  UIColor *_backgroundColor;
}

@synthesize ReactABI16_0_0ZIndex = _ReactABI16_0_0ZIndex;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI16_0_0RCTBorderStyleSolid;
...
{
  _ReactABI16_0_0LayoutDirection = layoutDirection;
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI16_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI16_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI16_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI16_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI16_0_0RCTPointerEventsNone || _pointerEvents == ABI16_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI16_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI16_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI16_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI16_0_0Tag: %@;", self.ReactABI16_0_0Tag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI16_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ReactABI16_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI16_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.sortedReactABI16_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI16_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI16_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI16_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI16_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ReactABI16_0_0Subviews.count == 0) {
  ...
  for (UIView *view in self.sortedReactABI16_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI16_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI16_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI16_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI16_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI16_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI16_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI16_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI16_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI16_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI16_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI16_0_0RCTCornerRadii){
...
{
  return (ABI16_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI16_0_0RCTUpdateShadowPathForView(self);

  const ABI16_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI16_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI16_0_0RCTRunningInTestEnvironment() &&
  ABI16_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI16_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI16_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI16_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI16_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI16_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI16_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI16_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI16_0_0RCTUpdateShadowPathForView(ABI16_0_0RCTView *view)
{
  if (ABI16_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI16_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI16_0_0RCTCornerInsets cornerInsets = ABI16_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI16_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI16_0_0RCTLogAdvice(@"View #%@ of type %@ has a shadow set but cannot calculate "
        "shadow efficiently. Consider setting a background color to "
        "fix this, or apply the shadow to a more specific component.",
        view.ReactABI16_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI16_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI16_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI16_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI16_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI16_0_0RCTBorderStyle)style \
  {                                                    \
