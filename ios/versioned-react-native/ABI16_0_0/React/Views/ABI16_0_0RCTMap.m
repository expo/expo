...
const CLLocationDegrees ABI16_0_0RCTMapDefaultSpan = 0.005;
const NSTimeInterval ABI16_0_0RCTMapRegionChangeObserveInterval = 0.1;
const CGFloat ABI16_0_0RCTMapZoomBoundBuffer = 0.01;

@implementation ABI16_0_0RCTMap
{
  UIView *_legalLabel;
...
- (void)setAnnotations:(NSArray<ABI16_0_0RCTMapAnnotation *> *)annotations
{
  ...
  NSMutableArray<ABI16_0_0RCTMapAnnotation *> *annotationsToDelete = [NSMutableArray new];
  NSMutableArray<ABI16_0_0RCTMapAnnotation *> *annotationsToAdd = [NSMutableArray new];

  for (ABI16_0_0RCTMapAnnotation *annotation in annotations) {
  ...
  for (ABI16_0_0RCTMapAnnotation *annotation in self.annotations) {
    if (![annotation isKindOfClass:[ABI16_0_0RCTMapAnnotation class]]) {
      continue;
    }
...
- (void)setOverlays:(NSArray<ABI16_0_0RCTMapOverlay *> *)overlays
{
  ...
  for (ABI16_0_0RCTMapOverlay *overlay in overlays) {
    if (![overlay isKindOfClass:[ABI16_0_0RCTMapOverlay class]]) {
      continue;
    }
  ...
  for (ABI16_0_0RCTMapOverlay *overlay in self.overlays) {
    if (![overlay isKindOfClass:[ABI16_0_0RCTMapOverlay class]]) {
      continue;
    }
