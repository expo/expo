...
#import "ABI16_0_0RCTJSCErrorHandling.h"
...
#if (ABI16_0_0RCT_PROFILE || ABI16_0_0RCT_DEV) && __has_include("ABI16_0_0RCTDevMenu.h")
#import "ABI16_0_0RCTDevMenu.h"
#endif

NSString *const ABI16_0_0RCTJSCThreadName = @"com.facebook.ReactABI16_0_0.JavaScript";
NSString *const ABI16_0_0RCTJavaScriptContextCreatedNotification = @"ABI16_0_0RCTJavaScriptContextCreatedNotification";
ABI16_0_0RCT_EXTERN NSString *const ABI16_0_0RCTFBJSContextClassKey = @"_ABI16_0_0RCTFBJSContextClassKey";
ABI16_0_0RCT_EXTERN NSString *const ABI16_0_0RCTFBJSValueClassKey = @"_ABI16_0_0RCTFBJSValueClassKey";
...
struct TaggedScript {
  const facebook::ReactABI16_0_0::ScriptTag tag;
  const NSData *script;
};

#if ABI16_0_0RCT_PROFILE
@interface ABI16_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI16_0_0RCTCookieMap @end
#endif

struct ABI16_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
};

@interface ABI16_0_0RCTJavaScriptContext : NSObject <ABI16_0_0RCTInvalidating>
...
@implementation ABI16_0_0RCTJavaScriptContext
{
  ABI16_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    _context = context;
    _context.name = @"ABI16_0_0RCTJSContext";
    _javaScriptThread = javaScriptThread;

    /**
     * Explicitly introduce a retain cycle here - The ABI16_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI16_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI16_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI16_0_0RCTPerformanceLogger *_performanceLogger;
  ABI16_0_0RCTJavaScriptContext *_context;
...
#if ABI16_0_0RCT_DEV
static void ABI16_0_0RCTInstallJSCProfiler(ABI16_0_0RCTBridge *bridge, JSContextRef context)
{
#if __has_include("ABI16_0_0RCTDevMenu.h")
  __weak ABI16_0_0RCTBridge *weakBridge = bridge;
  __weak ABI16_0_0RCTDevSettings *devSettings = bridge.devSettings;
  if (ABI16_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI16_0_0RCTDevMenuItem buttonItemWithTitleBlock:^NSString *{
      ...
      if (shouldStart != ABI16_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI16_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI16_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI16_0_0RCTProfileSendResult(weakBridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI16_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI16_0_0RCTJSCThreadName;
...
{
  ABI16_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI16_0_0RCTJSCExecutor init]", nil);
...
- (instancetype)initWithJSContextData:(const ABI16_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    _useCustomJSCLibrary = data.useCustomJSCLibrary;
    _valid = YES;
    _javaScriptThread = data.javaScriptThread;
    _context = [[ABI16_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
                                         ...
                                         sourceURL:(NSURL *)sourceURL
{
  ...
  if (taggedScript.tag == facebook::ReactABI16_0_0::ScriptTag::RAMBundle) {
...
- (ABI16_0_0RCTJavaScriptContext *)context
{
  ABI16_0_0RCTAssertThread(_javaScriptThread, @"Must be called on JS thread.");
  if (!self.isValid) {
    return nil;
  }
  ABI16_0_0RCTAssert(_context != nil, @"Fetching context while valid, but before it is created");
  return _context;
}
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI16_0_0RCTProfileDidStartProfiling, ABI16_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_context) {
      ...
      if (self->_useCustomJSCLibrary) {
        JSC_configureJSCForIOS(true, ABI16_0_0RCTJSONStringify(@{
      ...
      self->_context = [[ABI16_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI16_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];
    ...
    if (!threadDictionary[ABI16_0_0RCTFBJSContextClassKey] || !threadDictionary[ABI16_0_0RCTFBJSValueClassKey]) {
      threadDictionary[ABI16_0_0RCTFBJSContextClassKey] = JSC_JSContext(contextRef);
      threadDictionary[ABI16_0_0RCTFBJSValueClassKey] = JSC_JSValue(contextRef);
    }

    ABI16_0_0RCTFBQuickPerformanceLoggerConfigureHooks(context.JSGlobalContextRef);

    __weak ABI16_0_0RCTJSCExecutor *weakSelf = self;
    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI16_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI16_0_0RCT_PROFILE_BEGIN_EVENT(ABI16_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI16_0_0RCT_PROFILE_END_EVENT(ABI16_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI16_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI16_0_0RCT_PROFILE_BEGIN_EVENT(ABI16_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI16_0_0RCT_PROFILE_BEGIN_EVENT(ABI16_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI16_0_0RCT_PROFILE
    __weak ABI16_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI16_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI16_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI16_0_0RCTProfileIsProfiling()) {
        ...
        _ABI16_0_0RCTProfileEndFlowEvent(newCookie);
    ...
    if (JSC_JSSamplingProfilerEnabled(context.JSGlobalContextRef)) {
      ...
      [self->_bridge.devMenu addItem:[ABI16_0_0RCTDevMenuItem buttonItemWithTitle:@"Start / Stop JS Sampling Profiler" handler:^{
        ABI16_0_0RCTJSCExecutor *strongSelf = weakSelf;
      ...
      // (see SamplingProfiler.js for an example of how it could be used with the ABI16_0_0JSCSamplingProfiler module).
...
#if ABI16_0_0RCT_DEV
    ABI16_0_0RCTInstallJSCProfiler(self->_bridge, context.JSGlobalContextRef);
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI16_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ...
    if (logLevel) {
      level = MAX(level, (ABI16_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI16_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI16_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI16_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI16_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI16_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI16_0_0RCTCookieMap *cookieMap = [ABI16_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI16_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI16_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI16_0_0RCTProfileDidStartProfiling];
...
- (int32_t)bytecodeFileFormatVersion
{
  return _useCustomJSCLibrary
    ? facebook::ReactABI16_0_0::customJSCWrapper()->JSBytecodeFileFormatVersion
...
ABI16_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)contextName)
{
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI16_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI16_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI16_0_0RCTJavaScriptCallback)onComplete
{
  ABI16_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI16_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI16_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI16_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, batchedBridgeRef) != kJSTypeUndefined) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && JSC_JSValueGetType(ctx, methodJSRef) != kJSTypeUndefined) {
        ...
        if (!errorJSRef && JSC_JSValueGetType(ctx, methodJSRef) == kJSTypeUndefined) {
          error = ABI16_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && JSC_JSValueGetType(ctx, batchedBridgeRef) == kJSTypeUndefined) {
        error = ABI16_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI16_0_0RCTNSErrorFromJSError([JSC_JSValue(ctx) valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI16_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
{
  ABI16_0_0RCTAssertParam(script);
  ABI16_0_0RCTAssertParam(sourceURL);
...
static TaggedScript loadTaggedScript(NSData *script,
                                     NSURL *sourceURL,
                                     ABI16_0_0RCTPerformanceLogger *performanceLogger,
                                     RandomAccessBundleData &randomAccessBundle,
                                     NSError **error)
{
  ABI16_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);
  ...
  switch (tag) {
    case facebook::ReactABI16_0_0::ScriptTag::RAMBundle:
      [performanceLogger markStartForTag:ABI16_0_0RCTPLRAMBundleLoad];

      loadedScript = loadRAMBundle(sourceURL, error, randomAccessBundle);

      [performanceLogger markStopForTag:ABI16_0_0RCTPLRAMBundleLoad];
      [performanceLogger setValue:loadedScript.length forTag:ABI16_0_0RCTPLRAMStartupCodeSize];
      break;
...
static void registerNativeRequire(JSContext *context, ABI16_0_0RCTJSCExecutor *executor)
{
  __weak ABI16_0_0RCTJSCExecutor *weakExecutor = executor;
...
{
  ABI16_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(taggedScript.script.length)
  }));

  [performanceLogger markStartForTag:ABI16_0_0RCTPLScriptExecution];
  ...
  [performanceLogger markStopForTag:ABI16_0_0RCTPLScriptExecution];

  NSError *error = jsError
    ? ABI16_0_0RCTNSErrorFromJSErrorRef(jsError, ctx)
    : nil;
              ...
              callback:(ABI16_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI16_0_0RCT_DEBUG) {
    ABI16_0_0RCTAssert(ABI16_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI16_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    JSGlobalContextRef ctx = strongSelf->_context.context.JSGlobalContextRef;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI16_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI16_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI16_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
      }
    }
    ABI16_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI16_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI16_0_0RCTFatal(ABI16_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  if (!result) {
    NSError *error = ABI16_0_0RCTNSErrorFromJSErrorRef(jsError, ctx);
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI16_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI16_0_0RCTPLRAMNativeRequires];
  ABI16_0_0RCT_PROFILE_BEGIN_EVENT(ABI16_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI16_0_0RCT_PROFILE_END_EVENT(ABI16_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI16_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI16_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI16_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
@implementation ABI16_0_0RCTJSContextProvider
{
...
- (ABI16_0_0RCTJSContextData)data
{
...
{
  const ABI16_0_0RCTJSContextData data = self.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  return [[ABI16_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
}
