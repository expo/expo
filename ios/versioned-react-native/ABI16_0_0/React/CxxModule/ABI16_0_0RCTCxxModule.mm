...
#import "ABI16_0_0RCTCxxModule.h"
...
@implementation ABI16_0_0RCTCxxModule
{
...
{
  ABI16_0_0RCTAssert([ABI16_0_0RCTBridgeModuleNameForClass([self class]) isEqualToString:@(module->getName().c_str())],
            @"CxxModule class name %@ does not match runtime name %s",
            ABI16_0_0RCTBridgeModuleNameForClass([self class]), module->getName().c_str());
...
- (NSArray *)methodsToExport
{
  ...
  for (const auto &method : _module->getMethods()) {
    [moduleMethods addObject:[[ABI16_0_0RCTCxxMethod alloc] initWithCxxMethod:method]];
  }
