...
#include <cxxReactABI16_0_0/ABI16_0_0CxxMessageQueue.h>
...
struct QueueWithThread {
  ...
  ~QueueWithThread() {
    ...
    queue.reset();
    ABI16_0_0EXPECT_TRUE(done.wait_until(now() + milliseconds(300))) << "Queue did not exit";
  }
...
TEST(CxxMessageQueue, TestQuit) {
  ...
  q->quitSynchronous();
  ABI16_0_0EXPECT_TRUE(done.wait_until(now() + milliseconds(300)))
...
TEST(CxxMessageQueue, TestPostUnregistered) {
  ...
  q->quitSynchronous();
  q.reset();
  ABI16_0_0EXPECT_TRUE(qdone.wait_until(now() + milliseconds(300))) << "Queue did not exit";
}
