...
TEST(JSCExecutor, CallFunction) {
  ...
  ABI16_0_0EXPECT_EQ(11, returnedCall.moduleId);
  ABI16_0_0EXPECT_EQ(10, returnedCall.methodId);
  ASSERT_EQ(4, returnedCall.arguments.size());
  ABI16_0_0EXPECT_EQ(args[0], returnedCall.arguments[0]);
  ABI16_0_0EXPECT_EQ(args[1], returnedCall.arguments[1]);
  ABI16_0_0EXPECT_EQ(args[2], returnedCall.arguments[2]);
  ABI16_0_0EXPECT_EQ(folly::dynamic(4.0), returnedCall.arguments[3]);
}
...
TEST(JSCExecutor, CallFunctionWithMap) {
  ...
  ABI16_0_0EXPECT_EQ("hello4true", returnedCall.arguments[0].getString());
}
...
TEST(JSCExecutor, CallFunctionReturningMap) {
  ...
  ABI16_0_0EXPECT_EQ(folly::dynamic(4.0), foo);
  auto bar = returnedMap.at("bar");
  ABI16_0_0EXPECT_EQ(folly::dynamic(true), bar);
}
...
TEST(JSCExecutor, CallFunctionWithArray) {
  ...
  auto returnedCall = returnedCalls[0];
  ASSERT_EQ(1, returnedCall.arguments.size());
  ABI16_0_0EXPECT_EQ("hello4true3", returnedCall.arguments[0].getString());
}
...
TEST(JSCExecutor, CallFunctionReturningNumberArray) {
  ...
  ABI16_0_0EXPECT_EQ(3, array.size());
  ABI16_0_0EXPECT_EQ(folly::dynamic(3.0), array[0]);
  ABI16_0_0EXPECT_EQ(folly::dynamic(4.0), array[2]);
}
...
TEST(JSCExecutor, SetObjectGlobalVariable) {
  ...
  auto& returnedMap = returnedCall.arguments[0];
  auto foo = returnedMap.at("foo");
  ABI16_0_0EXPECT_EQ(folly::dynamic("hello"), foo);
  auto bar = returnedMap.at("bar");
  ABI16_0_0EXPECT_EQ(folly::dynamic(4.0), bar);
  auto baz = returnedMap.at("baz");
  ABI16_0_0EXPECT_EQ(folly::dynamic(true), baz);
}
