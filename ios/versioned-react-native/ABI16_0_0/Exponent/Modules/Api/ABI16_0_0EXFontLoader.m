...
#import "ABI16_0_0EXFontLoader.h"
...
static NSMutableDictionary *ABI16_0_0EXFonts = nil;

static const char *ABI16_0_0EXFontAssocKey = "ABI16_0_0EXFont";

@interface ABI16_0_0EXFont : NSObject
...
@implementation ABI16_0_0EXFont
...
- (UIFont *) UIFontWithSize:(CGFloat) fsize
{
  ...
  _sizes[size] = uiFont;
  objc_setAssociatedObject(uiFont, ABI16_0_0EXFontAssocKey, self, OBJC_ASSOCIATION_ASSIGN);
  return uiFont;
}
...
@implementation ABI16_0_0RCTFont (ABI16_0_0EXFontLoader)

// Will swap this with +[ABI16_0_0RCTFont updateFont: ...]
          ...
          scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  ABI16_0_0EXFont *exFont = nil;

  // Did we get a new family, and if so, is it associated with an ABI16_0_0EXFont?
  if ([family hasPrefix:exponentPrefix] && ABI16_0_0EXFonts) {
    NSString *suffix = [family substringFromIndex:exponentPrefix.length];
    exFont = ABI16_0_0EXFonts[suffix];
  }

  // Did the passed-in UIFont come from an ABI16_0_0EXFont?
  if (!exFont && uiFont) {
    exFont = objc_getAssociatedObject(uiFont, ABI16_0_0EXFontAssocKey);
  }

  // If it's an ABI16_0_0EXFont, generate the corresponding UIFont, else fallback to ReactABI16_0_0 Native's built-in method
  if (exFont) {
    return [exFont UIFontWithSize:[ABI16_0_0RCTConvert CGFloat:size] ?: uiFont.pointSize ?: defaultFontSize];
  } else {
...
@implementation ABI16_0_0EXFontLoader

ABI16_0_0RCT_EXPORT_MODULE(ExponentFontLoader);
...
+ (void)initialize {
  ...
  method_exchangeImplementations(class_getClassMethod([ABI16_0_0RCTFont class], a),
                                 class_getClassMethod([ABI16_0_0RCTFont class], b));
}

ABI16_0_0RCT_REMAP_METHOD(loadAsync,
...
{
  if (ABI16_0_0EXFonts && ABI16_0_0EXFonts[fontFamilyName]) {
  ...
  if (!ABI16_0_0EXFonts) {
    ABI16_0_0EXFonts = [NSMutableDictionary dictionary];
  }
  ABI16_0_0EXFonts[fontFamilyName] = [[ABI16_0_0EXFont alloc] initWithCGFont:font];
  resolve(nil);
}
