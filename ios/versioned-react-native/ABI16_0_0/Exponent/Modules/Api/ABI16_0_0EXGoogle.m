...
#import "ABI16_0_0EXGoogle.h"

#import <GoogleSignIn/GoogleSignIn.h>
#import <AppAuth.h>

#import <ReactABI16_0_0/ABI16_0_0RCTUtils.h>
#import "ABI16_0_0EXOAuthViewController.h"
#import "ABI16_0_0EXUnversioned.h"

NSString *ABI16_0_0EXGoogleErrorCode = @"GOOGLE_ERROR";

@interface ABI16_0_0EXGoogle () <GIDSignInDelegate, GIDSignInUIDelegate>

@end

@implementation ABI16_0_0EXGoogle
{
  ABI16_0_0RCTPromiseResolveBlock _logInResolve;
  ABI16_0_0RCTPromiseRejectBlock _logInReject;
}

ABI16_0_0RCT_EXPORT_MODULE(ExponentGoogle)
                 ...
                 rejecter:(ABI16_0_0RCTPromiseRejectBlock)reject)
{
  ...
  if ([behavior isEqualToString:@"system"]) {
    ...
    reject(ABI16_0_0EXGoogleErrorCode, [NSString stringWithFormat:@"Invalid behavior %@", behavior], nil);
  }
}
...
-(void)webLogInWithClientId:(NSString *)clientId scopes: (NSArray<NSString *> *)scopes
{
  ...
  if (!urlSchemes || !urlSchemes.count) {
    _logInReject(ABI16_0_0EXGoogleErrorCode,
                 @"Google sign in error",
                 ABI16_0_0RCTErrorWithMessage(@"No OAuthRedirect bundle url type was configured in this app's Info.plist."));
    _logInResolve = nil;
    _logInReject = nil;
    return;
  }
  ...
  OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
    if (authState) {
      ...
      _logInReject(ABI16_0_0EXGoogleErrorCode, @"Google sign in error", error);
    }
  ...
  id<OIDAuthorizationFlowSession> currentAuthorizationFlow =
    [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                   presentingViewController:ABI16_0_0RCTPresentedViewController()
                                                   callback:callback];
...
{
  [ABI16_0_0RCTPresentedViewController() presentViewController:viewController animated:YES completion:nil];
}

-(void)signIn:(GIDSignIn *)signIn dismissViewController:(UIViewController *)viewController
{
  [ABI16_0_0RCTPresentedViewController() dismissViewControllerAnimated:YES completion:nil];
}
