...
#define ABI16_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI16_0_0EXFrameReactABI16_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI16_0_0EXFrameReactABI16_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI16_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI16_0_0Bridge;

@end

@interface ABI16_0_0EXFrame () <ABI16_0_0RCTInvalidating, ABI16_0_0RCTExceptionsManagerDelegate>
...
@implementation ABI16_0_0EXFrame

ABI16_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI16_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI16_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:self selector:@selector(load) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI16_0_0RootView]) {
    [_appManager ReactABI16_0_0RootView].frame = self.bounds;
  }
}
...
#pragma mark - ABI16_0_0EXReactABI16_0_0AppManagerDelegate

- (void)ReactABI16_0_0AppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI16_0_0RootView = [appManager ReactABI16_0_0RootView];
  ReactABI16_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI16_0_0RootView];
}

- (void)ReactABI16_0_0AppManagerDidDestroyApp:(id)appManager
{
  if (_tmrReload) {
    [_tmrReload invalidate];
    _tmrReload = nil;
...
- (void)ReactABI16_0_0AppManagerStartedLoadingJavaScript:(id)appManager
{
  if (_onLoadingStart) {
    _onLoadingStart(nil);
  }
...
- (void)ReactABI16_0_0AppManagerFinishedLoadingJavaScript:(id)appManager
{
  if (_viewTestTimer) {
    [_viewTestTimer invalidate];
  }
...
- (void)ReactABI16_0_0AppManager:(id)appManager failedToLoadJavaScriptWithError:(NSError *)error
{
  [self _sendLoadingError:error];
}

- (void)ReactABI16_0_0AppManager:(id)appManager failedToDownloadBundleWithError:(NSError *)error
{
  
}

- (void)ReactABI16_0_0AppManagerDidForeground:(id)appManager
{
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI16_0_0RootView] &&
      [_appManager ReactABI16_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI16_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI16_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI16_0_0RCTExceptionsManagerDelegate
...
- (void)handleFatalJSExceptionWithMessage:(NSString *)message stack:(NSArray *)stack exceptionId:(NSNumber *)exceptionId
{
  ...
  [_appManager registerErrorForBridge:ABI16_0_0RCTErrorWithMessage(message)];
