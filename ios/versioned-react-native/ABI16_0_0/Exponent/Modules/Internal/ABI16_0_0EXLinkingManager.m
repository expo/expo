...
NSString * const ABI16_0_0EXLinkingEventOpenUrl = @"url";

@interface ABI16_0_0EXLinkingManager ()
...
@implementation ABI16_0_0EXLinkingManager

+ (NSString *)moduleName { return @"ABI16_0_0RCTLinkingManager"; }
...
#pragma mark - ABI16_0_0RCTEventEmitter methods

- (NSArray<NSString *> *)supportedEvents
{
  return @[ABI16_0_0EXLinkingEventOpenUrl];
}
...
- (void)dispatchOpenUrlEvent:(NSURL *)url
{
  if (!url || !url.absoluteString) {
    ABI16_0_0RCTFatal(ABI16_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Tried to open a deep link to an invalid url: %@", url]));
    return;
  }
  if (_hasListeners) {
    [self sendEventWithName:ABI16_0_0EXLinkingEventOpenUrl body:@{@"url": url.absoluteString}];
  }
}

ABI16_0_0RCT_EXPORT_METHOD(openURL:(NSURL *)URL
                  ...
                  reject:(ABI16_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _isExponentUrl:URL]) {
    ...
    BOOL opened = [ABI16_0_0RCTSharedApplication() openURL:URL];
    if (opened) {
      resolve(nil);
    } else {
      reject(ABI16_0_0RCTErrorUnspecified, [NSString stringWithFormat:@"Unable to open URL: %@", URL], nil);
    }
  }
}

ABI16_0_0RCT_EXPORT_METHOD(canOpenURL:(NSURL *)URL
                  resolve:(ABI16_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI16_0_0RCTPromiseRejectBlock)reject)
{
  BOOL canOpen = [self _isExponentUrl:URL];
  if (!canOpen) {
    canOpen = [ABI16_0_0RCTSharedApplication() canOpenURL:URL];
  }
  resolve(@(canOpen));
}

ABI16_0_0RCT_EXPORT_METHOD(getInitialURL:(ABI16_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI16_0_0RCTPromiseRejectBlock)reject)
{
  resolve(ABI16_0_0RCTNullIfNil(_initialUrl.absoluteString));
}
...
- (BOOL)_isExponentUrl: (NSURL *)url
{
  ...
  NSString *versionsPath = [[NSBundle mainBundle] pathForResource:@"ABI16_0_0EXSDKVersions" ofType:@"plist"];
