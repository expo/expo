...
void __attribute__((visibility("hidden"),weak)) ABI11_0_0RCTCustomJSCInit(__unused void *handle) {
  return;
}

static void *ABI11_0_0RCTCustomLibraryHandler(void)
{
  ...
  dispatch_once(&token, ^{
    ...
    if (!handler) {
      ...
      if (err != nullptr && strstr(err, "image not found") == nullptr) {
        ABI11_0_0RCTLogWarn(@"Can't load custom JSC library: %s", err);
      }
    }
  });
...
static void ABI11_0_0RCTSetUpSystemLibraryPointers(ABI11_0_0RCTJSCWrapper *wrapper)
{
...
static void ABI11_0_0RCTSetUpCustomLibraryPointers(ABI11_0_0RCTJSCWrapper *wrapper)
{
  void *libraryHandle = ABI11_0_0RCTCustomLibraryHandler();
  if (!libraryHandle) {
    ABI11_0_0RCTSetUpSystemLibraryPointers(wrapper);
    return;
  }
  ...
  dispatch_once(&once, ^{
    ABI11_0_0RCTCustomJSCInit(libraryHandle);
  });
}

ABI11_0_0RCTJSCWrapper *ABI11_0_0RCTJSCWrapperCreate(BOOL useCustomJSC)
{
  ABI11_0_0RCTJSCWrapper *wrapper = (ABI11_0_0RCTJSCWrapper *)malloc(sizeof(ABI11_0_0RCTJSCWrapper));
  if (useCustomJSC) {
    ABI11_0_0RCTSetUpCustomLibraryPointers(wrapper);
  } else {
    ABI11_0_0RCTSetUpSystemLibraryPointers(wrapper);
  }
  return wrapper;
...
void ABI11_0_0RCTJSCWrapperRelease(ABI11_0_0RCTJSCWrapper *wrapper)
{
  if (wrapper) {
    free(wrapper);
  }
