...
#import "ABI11_0_0RCTJSCErrorHandling.h"

NSString *const ABI11_0_0RCTJSCThreadName = @"com.facebook.ReactABI11_0_0.JavaScript";
NSString *const ABI11_0_0RCTJavaScriptContextCreatedNotification = @"ABI11_0_0RCTJavaScriptContextCreatedNotification";
ABI11_0_0RCT_EXTERN NSString *const ABI11_0_0RCTFBJSContextClassKey = @"_ABI11_0_0RCTFBJSContextClassKey";
ABI11_0_0RCT_EXTERN NSString *const ABI11_0_0RCTFBJSValueClassKey = @"_ABI11_0_0RCTFBJSValueClassKey";

static NSString *const ABI11_0_0RCTJSCProfilerEnabledDefaultsKey = @"ABI11_0_0RCTJSCProfilerEnabled";
...
@interface ABI11_0_0RCTCookieMap : NSObject
{
  @package
  std::unordered_map<NSUInteger, NSUInteger> _cookieMap;
}
@end
@implementation ABI11_0_0RCTCookieMap @end
#endif

struct ABI11_0_0RCTJSContextData {
  BOOL useCustomJSCLibrary;
  NSThread *javaScriptThread;
  JSContext *context;
  ABI11_0_0RCTJSCWrapper *jscWrapper;
};

@interface ABI11_0_0RCTJSContextProvider ()
/** May only be called once, or deadlock will result. */
- (ABI11_0_0RCTJSContextData)data;
@end

@interface ABI11_0_0RCTJavaScriptContext : NSObject <ABI11_0_0RCTInvalidating>
...
@implementation ABI11_0_0RCTJavaScriptContext
{
  ABI11_0_0RCTJavaScriptContext *_selfReference;
                         ...
                         onThread:(NSThread *)javaScriptThread
{
  if ((self = [super init])) {
    ...
    /**
     * Explicitly introduce a retain cycle here - The ABI11_0_0RCTJSCExecutor might
...
{
  if (self.isValid) {
    ABI11_0_0RCTAssertThread(_javaScriptThread, @"Must be invalidated on JS thread.");
...
@implementation ABI11_0_0RCTJSCExecutor
{
  // Set at init time:
  ...
  // Set at setUp time:
  ABI11_0_0RCTPerformanceLogger *_performanceLogger;
  ABI11_0_0RCTJSCWrapper *_jscWrapper;
  ABI11_0_0RCTJavaScriptContext *_context;
...
static void ABI11_0_0RCTInstallJSCProfiler(ABI11_0_0RCTBridge *bridge, JSContextRef context)
{
  if (ABI11_0_0RCTJSCProfilerIsSupported()) {
    [bridge.devMenu addItem:[ABI11_0_0RCTDevMenuItem toggleItemWithKey:ABI11_0_0RCTJSCProfilerEnabledDefaultsKey title:@"Start Profiling" selectedTitle:@"Stop Profiling" handler:^(BOOL shouldStart) {
      if (shouldStart != ABI11_0_0RCTJSCProfilerIsProfiling(context)) {
        if (shouldStart) {
          ABI11_0_0RCTJSCProfilerStart(context);
        } else {
          NSString *outputFile = ABI11_0_0RCTJSCProfilerStop(context);
          NSData *profileData = [NSData dataWithContentsOfFile:outputFile options:NSDataReadingMappedIfSafe error:NULL];
          ABI11_0_0RCTProfileSendResult(bridge, @"cpu-profile", profileData);
        }
      }
    }]];
  }
...
+ (void)runRunLoopThread
{
  @autoreleasepool {
    ...
    while (kCFRunLoopRunStopped != CFRunLoopRunInMode(kCFRunLoopDefaultMode, ((NSDate *)[NSDate distantFuture]).timeIntervalSinceReferenceDate, NO)) {
      ABI11_0_0RCTAssert(NO, @"not reached assertion"); // runloop spun. that's bad.
    }
  }
}
...
{
  NSThread *javaScriptThread = [[NSThread alloc] initWithTarget:[ABI11_0_0RCTJSCExecutor class]
                                                       selector:@selector(runRunLoopThread)
                                                         object:nil];
  javaScriptThread.name = ABI11_0_0RCTJSCThreadName;
...
{
  ABI11_0_0RCT_PROFILE_BEGIN_EVENT(0, @"-[ABI11_0_0RCTJSCExecutor init]", nil);
...
+ (instancetype)initializedExecutorWithContextProvider:(ABI11_0_0RCTJSContextProvider *)JSContextProvider
                                     applicationScript:(NSData *)applicationScript
                                             ...
                                             JSContext:(JSContext **)JSContext
                                                 error:(NSError **)error
{
  const ABI11_0_0RCTJSContextData data = JSContextProvider.data;
  if (JSContext) {
    *JSContext = data.context;
  }
  ABI11_0_0RCTJSCExecutor *executor = [[ABI11_0_0RCTJSCExecutor alloc] initWithJSContextData:data];
...
- (instancetype)initWithJSContextData:(const ABI11_0_0RCTJSContextData &)data
{
  if (self = [super init]) {
    ...
    _context = [[ABI11_0_0RCTJavaScriptContext alloc] initWithJSContext:data.context onThread:_javaScriptThread];
  }
  return self;
}
...
- (ABI11_0_0RCTJavaScriptContext *)context
{
  ABI11_0_0RCTAssertThread(_javaScriptThread, @"Must be called on JS thread.");
  if (!self.isValid) {
    return nil;
  }
  ABI11_0_0RCTAssert(_context != nil, @"Fetching context while valid, but before it is created");
  return _context;
}
...
- (void)setUp
{
  ...
  for (NSString *event in @[ABI11_0_0RCTProfileDidStartProfiling, ABI11_0_0RCTProfileDidEndProfiling]) {
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    if (self->_jscWrapper) {
      ABI11_0_0RCTAssert(self->_context != nil, @"If wrapper was pre-initialized, context should be too");
      context = self->_context.context;
    } else {
      [self->_performanceLogger markStartForTag:ABI11_0_0RCTPLJSCWrapperOpenLibrary];
      self->_jscWrapper = ABI11_0_0RCTJSCWrapperCreate(self->_useCustomJSCLibrary);
      [self->_performanceLogger markStopForTag:ABI11_0_0RCTPLJSCWrapperOpenLibrary];

      ABI11_0_0RCTAssert(self->_context == nil, @"Didn't expect to set up twice");
      context = [self->_jscWrapper->JSContext new];
      self->_context = [[ABI11_0_0RCTJavaScriptContext alloc] initWithJSContext:context onThread:self->_javaScriptThread];
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI11_0_0RCTJavaScriptContextCreatedNotification
                                                          object:context];
    ...
    if (!threadDictionary[ABI11_0_0RCTFBJSContextClassKey] || !threadDictionary[ABI11_0_0RCTFBJSValueClassKey]) {
      threadDictionary[ABI11_0_0RCTFBJSContextClassKey] = self->_jscWrapper->JSContext;
      threadDictionary[ABI11_0_0RCTFBJSValueClassKey] = self->_jscWrapper->JSValue;
    }

    __weak ABI11_0_0RCTJSCExecutor *weakSelf = self;

    context[@"nativeRequireModuleConfig"] = ^NSArray *(NSString *moduleName) {
      ABI11_0_0RCTJSCExecutor *strongSelf = weakSelf;
      if (!strongSelf.valid) {
        return nil;
      }

      ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"nativeRequireModuleConfig", @{ @"moduleName": moduleName });
      NSArray *result = [strongSelf->_bridge configForModuleName:moduleName];
      ABI11_0_0RCT_PROFILE_END_EVENT(ABI11_0_0RCTProfileTagAlways, @"js_call,config");
      return ABI11_0_0RCTNullIfNil(result);
    };
    ...
    context[@"nativeFlushQueueImmediate"] = ^(NSArray<NSArray *> *calls){
      ...
      ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"nativeFlushQueueImmediate", nil);
      [strongSelf->_bridge handleBuffer:calls batchEnded:NO];
      ABI11_0_0RCT_PROFILE_END_EVENT(ABI11_0_0RCTProfileTagAlways, @"js_call");
    };
    ...
    context[@"nativeCallSyncHook"] = ^id(NSUInteger module, NSUInteger method, NSArray *args) {
      ...
      ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"nativeCallSyncHook", nil);
...
#if ABI11_0_0RCT_PROFILE
    __weak ABI11_0_0RCTBridge *weakBridge = self->_bridge;
    context[@"nativeTraceBeginAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI11_0_0RCTProfileIsProfiling()) {
        ...
        NSUInteger newCookie = _ABI11_0_0RCTProfileBeginFlowEvent();
    ...
    context[@"nativeTraceEndAsyncFlow"] = ^(__unused uint64_t tag, __unused NSString *name, int64_t cookie) {
      if (ABI11_0_0RCTProfileIsProfiling()) {
        ...
        _ABI11_0_0RCTProfileEndFlowEvent(newCookie);
...
#if ABI11_0_0RCT_DEV
    ABI11_0_0RCTInstallJSCProfiler(self->_bridge, context.JSGlobalContextRef);
    ...
    context[@"nativeInjectHMRUpdate"] = ^(NSString *sourceCode, NSString *sourceCodeURL) {
      ...
      ABI11_0_0RCTJSCWrapper *jscWrapper = strongSelf->_jscWrapper;
...
static void configureCacheOnContext(JSContext *context, ABI11_0_0RCTJSCWrapper *jscWrapper)
{
  if (jscWrapper->configureJSContextForIOS != NULL) {
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
    ABI11_0_0RCTAssert(cachesPath != nil, @"cachesPath should not be nil");
    if (cachesPath) {
...
/** Installs synchronous hooks that don't require a weak reference back to the ABI11_0_0RCTJSCExecutor. */
static void installBasicSynchronousHooksOnContext(JSContext *context)
{
  ...
  context[@"nativeLoggingHook"] = ^(NSString *message, NSNumber *logLevel) {
    ABI11_0_0RCTLogLevel level = ABI11_0_0RCTLogLevelInfo;
    if (logLevel) {
      level = MAX(level, (ABI11_0_0RCTLogLevel)logLevel.integerValue);
    }

    _ABI11_0_0RCTLogJavaScriptInternal(level, message);
  };
  ...
  if (ABI11_0_0RCTProfileIsProfiling()) {
    ...
    context[@"__ABI11_0_0RCTProfileIsProfiling"] = @YES;
  }
  context[@"nativeTraceBeginSection"] = ^(NSNumber *tag, NSString *profileName, NSDictionary *args) {
    ...
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(tag.longLongValue, profileName, args);
  };
  context[@"nativeTraceEndSection"] = ^(NSNumber *tag) {
    ABI11_0_0RCT_PROFILE_END_EVENT(tag.longLongValue, @"console");
  };
  ABI11_0_0RCTCookieMap *cookieMap = [ABI11_0_0RCTCookieMap new];
  context[@"nativeTraceBeginAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    NSUInteger newCookie = ABI11_0_0RCTProfileBeginAsyncEvent(tag, name, nil);
  ...
  context[@"nativeTraceEndAsyncSection"] = ^(uint64_t tag, NSString *name, NSUInteger cookie) {
    ...
    ABI11_0_0RCTProfileEndAsyncEvent(tag, @"js,async", newCookie, name, @"JS async");
  };
#endif
}
...
{
  [self executeBlockOnJavaScriptQueue:^{
    BOOL enabled = [notification.name isEqualToString:ABI11_0_0RCTProfileDidStartProfiling];
...
- (void)dealloc
{
  ...
  if (_jscWrapper) {
    ABI11_0_0RCTJSCWrapperRelease(_jscWrapper);
    _jscWrapper = NULL;
  }
}

- (void)flushedQueue:(ABI11_0_0RCTJavaScriptCallback)onComplete
{
...
- (void)_callFunctionOnModule:(NSString *)module
                 ...
                 unwrapResult:(BOOL)unwrapResult
                     callback:(ABI11_0_0RCTJavaScriptCallback)onComplete
{
  // TODO: Make this function handle first class instead of dynamically dispatching it. #9317773
...
- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args callback:(ABI11_0_0RCTJavaScriptCallback)onComplete
{
  [self _callFunctionOnModule:module method:method arguments:args returnValue:YES unwrapResult:YES callback:onComplete];
}

- (void)callFunctionOnModule:(NSString *)module method:(NSString *)method arguments:(NSArray *)args jsValueCallback:(ABI11_0_0RCTJavaScriptValueCallback)onComplete
{
...
- (void)_executeJSCall:(NSString *)method
          ...
          unwrapResult:(BOOL)unwrapResult
              callback:(ABI11_0_0RCTJavaScriptCallback)onComplete
{
  ABI11_0_0RCTAssert(onComplete != nil, @"onComplete block should not be nil");
  __weak ABI11_0_0RCTJSCExecutor *weakSelf = self;
  [self executeBlockOnJavaScriptQueue:^{
    ABI11_0_0RCTJSCExecutor *strongSelf = weakSelf;
    ...
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeJSCall", (@{@"method": method, @"args": arguments}));
    ...
    if (batchedBridgeRef != NULL && errorJSRef == NULL && !jscWrapper->JSValueIsUndefined(contextJSRef, batchedBridgeRef)) {
      ...
      if (methodJSRef != NULL && errorJSRef == NULL && !jscWrapper->JSValueIsUndefined(contextJSRef, methodJSRef)) {
        ...
        if (!errorJSRef && jscWrapper->JSValueIsUndefined(contextJSRef, methodJSRef)) {
          error = ABI11_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Unable to execute JS call: method %@ is undefined", method]);
        }
      }
    } else {
      if (!errorJSRef && jscWrapper->JSValueIsUndefined(contextJSRef, batchedBridgeRef)) {
        error = ABI11_0_0RCTErrorWithMessage(@"Unable to execute JS call: __fbBatchedBridge is undefined");
      }
    }
    ...
    if (errorJSRef || error) {
      if (!error) {
        error = ABI11_0_0RCTNSErrorFromJSError([jscWrapper->JSValue valueWithJSValueRef:errorJSRef inContext:context]);
      }
    } else {
    ...
    ABI11_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
...
- (void)executeApplicationScript:(NSData *)script
                       sourceURL:(NSURL *)sourceURL
                      onComplete:(ABI11_0_0RCTJavaScriptCompleteBlock)onComplete
{
  ABI11_0_0RCTAssertParam(script);
  ABI11_0_0RCTAssertParam(sourceURL);
...
static NSData *loadPossiblyBundledApplicationScript(NSData *script, NSURL *sourceURL,
                                                    ABI11_0_0RCTPerformanceLogger *performanceLogger,
                                                    BOOL &isRAMBundle, RandomAccessBundleData &randomAccessBundle,
                                                    NSError **error)
{
  ABI11_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / prepare bundle", nil);

  // The RAM bundle has a magic number in the 4 first bytes `(0xFB0BD1E5)`.
  uint32_t magicNumber = 0;
  [script getBytes:&magicNumber length:sizeof(magicNumber)];
  isRAMBundle = NSSwapLittleIntToHost(magicNumber) == ABI11_0_0RCTRAMBundleMagicNumber;
  if (isRAMBundle) {
    [performanceLogger markStartForTag:ABI11_0_0RCTPLRAMBundleLoad];
    script = loadRAMBundle(sourceURL, error, randomAccessBundle);
    [performanceLogger markStopForTag:ABI11_0_0RCTPLRAMBundleLoad];
    [performanceLogger setValue:script.length forTag:ABI11_0_0RCTPLRAMStartupCodeSize];
  } else {
...
static void registerNativeRequire(JSContext *context, ABI11_0_0RCTJSCExecutor *executor)
{
  __weak ABI11_0_0RCTJSCExecutor *weakExecutor = executor;
  context[@"nativeRequire"] = ^(NSNumber *moduleID) { [weakExecutor _nativeRequire:moduleID]; };
}

static NSError *executeApplicationScript(NSData *script, NSURL *sourceURL, ABI11_0_0RCTJSCWrapper *jscWrapper,
                                         ABI11_0_0RCTPerformanceLogger *performanceLogger, JSGlobalContextRef ctx)
{
  ABI11_0_0RCT_PROFILE_BEGIN_EVENT(0, @"executeApplicationScript / execute script", (@{
    @"url": sourceURL.absoluteString, @"size": @(script.length)
  }));
  [performanceLogger markStartForTag:ABI11_0_0RCTPLScriptExecution];
  ...
  jscWrapper->JSStringRelease(execJSString);
  [performanceLogger markStopForTag:ABI11_0_0RCTPLScriptExecution];

  NSError *error = jsError ? ABI11_0_0RCTNSErrorFromJSErrorRef(jsError, ctx, jscWrapper) : nil;
  ABI11_0_0RCT_PROFILE_END_EVENT(0, @"js_call");
  return error;
}
...
- (void)injectJSONText:(NSString *)script
   asGlobalObjectNamed:(NSString *)objectName
              callback:(ABI11_0_0RCTJavaScriptCompleteBlock)onComplete
{
  if (ABI11_0_0RCT_DEBUG) {
    ABI11_0_0RCTAssert(ABI11_0_0RCTJSONParse(script, NULL) != nil, @"%@ wasn't valid JSON!", script);
  }
  ...
  [self executeBlockOnJavaScriptQueue:^{
    ...
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(0, @"injectJSONText", @{@"objectName": objectName});
    ABI11_0_0RCTJSCWrapper *jscWrapper = strongSelf->_jscWrapper;
    ...
    if (!valueToInject) {
      ...
      error = [NSError errorWithDomain:ABI11_0_0RCTErrorDomain code:2 userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
      ABI11_0_0RCTLogError(@"%@", errorMessage);
    } else {
      ...
      if (jsError) {
        error = ABI11_0_0RCTNSErrorFromJSErrorRef(jsError, ctx, jscWrapper);
      }
    }
    ABI11_0_0RCT_PROFILE_END_EVENT(0, @"js_call,json_call");
...
static void executeRandomAccessModule(ABI11_0_0RCTJSCExecutor *executor, uint32_t moduleID, size_t offset, size_t size)
{
  auto data = std::make_unique<char[]>(size);
  if (!readRandomAccessModule(executor->_randomAccessBundle, offset, size, data.get())) {
    ABI11_0_0RCTFatal(ABI11_0_0RCTErrorWithMessage(@"Error loading RAM module"));
    return;
  }
  ...
  ABI11_0_0RCTJSCWrapper *jscWrapper = executor->_jscWrapper;
  ...
  if (!result) {
    NSError *error = ABI11_0_0RCTNSErrorFromJSErrorRef(jsError, ctx, jscWrapper);
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI11_0_0RCTFatal(error);
      [executor invalidate];
    });
  }
}
...
- (void)_nativeRequire:(NSNumber *)moduleID
{
  ...
  [_performanceLogger addValue:1 forTag:ABI11_0_0RCTPLRAMNativeRequiresCount];
  [_performanceLogger appendStartForTag:ABI11_0_0RCTPLRAMNativeRequires];
  ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", moduleID]), nil);
  ...
  ABI11_0_0RCT_PROFILE_END_EVENT(ABI11_0_0RCTProfileTagAlways, @"js_call");
  [_performanceLogger appendStopForTag:ABI11_0_0RCTPLRAMNativeRequires];
}
...
static NSData *loadRAMBundle(NSURL *sourceURL, NSError **error, RandomAccessBundleData &randomAccessBundle)
{
  ...
  if (!bundle) {
    if (error) {
      *error = ABI11_0_0RCTErrorWithMessage([NSString stringWithFormat:@"Bundle %@ cannot be opened: %d", sourceURL.path, errno]);
    }
    return nil;
  }
  ...
  if (startupCode.isEmpty()) {
    if (error) {
      *error = ABI11_0_0RCTErrorWithMessage(@"Error loading RAM Bundle");
    }
    return nil;
  }
...
ABI11_0_0RCT_EXPORT_METHOD(setContextName:(nonnull NSString *)name)
{
...
@implementation ABI11_0_0RCTJSContextProvider
{
...
- (void)_createContext
{
  _jscWrapper = ABI11_0_0RCTJSCWrapperCreate(_useCustomJSCLibrary);
