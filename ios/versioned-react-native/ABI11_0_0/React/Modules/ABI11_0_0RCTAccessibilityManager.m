...
NSString *const ABI11_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification = @"ABI11_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification";

@interface ABI11_0_0RCTAccessibilityManager ()
...
@implementation ABI11_0_0RCTAccessibilityManager
...
- (void)invalidateMultiplier
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:ABI11_0_0RCTAccessibilityManagerDidUpdateMultiplierNotification object:self];
}
...
- (CGFloat)multiplierForContentSizeCategory:(NSString *)category
{
  ...
  if (m.doubleValue <= 0.0) {
    ABI11_0_0RCTLogError(@"Can't determinte multiplier for category %@. Using 1.0.", category);
    m = @1.0;
  }
...
ABI11_0_0RCT_EXPORT_METHOD(setAccessibilityContentSizeMultipliers:(NSDictionary *)JSMultipliers)
{
  ...
  for (NSString *__nonnull JSCategory in JSMultipliers) {
    NSNumber *m = [ABI11_0_0RCTConvert NSNumber:JSMultipliers[JSCategory]];
...
ABI11_0_0RCT_EXPORT_METHOD(getMultiplier:(ABI11_0_0RCTResponseSenderBlock)callback)
{
  if (callback) {
...
ABI11_0_0RCT_EXPORT_METHOD(getCurrentVoiceOverState:(ABI11_0_0RCTResponseSenderBlock)callback
                  error:(__unused ABI11_0_0RCTResponseSenderBlock)error)
{
...
@implementation ABI11_0_0RCTBridge (ABI11_0_0RCTAccessibilityManager)

- (ABI11_0_0RCTAccessibilityManager *)accessibilityManager
{
  return [self moduleForClass:[ABI11_0_0RCTAccessibilityManager class]];
}
