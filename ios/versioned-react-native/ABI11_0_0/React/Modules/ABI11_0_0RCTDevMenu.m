...
#import "ABI11_0_0RCTWebSocketProxy.h"

#if ABI11_0_0RCT_DEV

static NSString *const ABI11_0_0RCTShowDevMenuNotification = @"ABI11_0_0RCTShowDevMenuNotification";
static NSString *const ABI11_0_0RCTDevMenuSettingsKey = @"ABI11_0_0RCTDevMenu";

@implementation UIWindow (ABI11_0_0RCTDevMenu)

- (void)ABI11_0_0RCT_motionEnded:(__unused UIEventSubtype)motion withEvent:(UIEvent *)event
{
  if (event.subtype == UIEventSubtypeMotionShake) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI11_0_0RCTShowDevMenuNotification object:nil];
  }
}

@end

typedef NS_ENUM(NSInteger, ABI11_0_0RCTDevMenuType) {
  ABI11_0_0RCTDevMenuTypeButton,
  ABI11_0_0RCTDevMenuTypeToggle
};

@interface ABI11_0_0RCTDevMenuItem ()

@property (nonatomic, assign, readonly) ABI11_0_0RCTDevMenuType type;
...
@implementation ABI11_0_0RCTDevMenuItem
{
  id _handler; // block
}

- (instancetype)initWithType:(ABI11_0_0RCTDevMenuType)type
                         key:(NSString *)key
...
{
  return [[self alloc] initWithType:ABI11_0_0RCTDevMenuTypeButton
                                key:nil
                              title:title
                      selectedTitle:nil
                            handler:handler];
...
{
  return [[self alloc] initWithType:ABI11_0_0RCTDevMenuTypeToggle
                                key:key
                              title:title
...
{
  switch (_type) {
    case ABI11_0_0RCTDevMenuTypeButton: {
      if (_handler) {
        ((void(^)())_handler)();
      }
      break;
    }
    case ABI11_0_0RCTDevMenuTypeToggle: {
      if (_handler) {
...
@interface ABI11_0_0RCTDevMenu () <ABI11_0_0RCTBridgeModule, ABI11_0_0RCTInvalidating, ABI11_0_0RCTWebSocketProxyDelegate>

@property (nonatomic, strong) Class executorClass;

@end

@implementation ABI11_0_0RCTDevMenu
{
  ...
  BOOL _jsLoaded;
  NSArray<ABI11_0_0RCTDevMenuItem *> *_presentedItems;
  NSMutableArray<ABI11_0_0RCTDevMenuItem *> *_extraMenuItems;
...
+ (void)initialize
{
  ...
  ABI11_0_0RCTSwapInstanceMethods([UIWindow class], @selector(motionEnded:withEvent:), @selector(ABI11_0_0RCT_motionEnded:withEvent:));
}
  ...
  if ((self = [super init])) {
    ...
    [notificationCenter addObserver:self
                           selector:@selector(showOnShake)
                               name:ABI11_0_0RCTShowDevMenuNotification
                             object:nil];
    ...
    _settings = [[NSMutableDictionary alloc] initWithDictionary:[_defaults objectForKey:ABI11_0_0RCTDevMenuSettingsKey]];
    _extraMenuItems = [NSMutableArray new];

    __weak ABI11_0_0RCTDevMenu *weakSelf = self;

    [_extraMenuItems addObject:[ABI11_0_0RCTDevMenuItem toggleItemWithKey:@"showInspector"
...
// TODO: Move non-UI logic into separate ABI11_0_0RCTDevSettings module
- (void)connectPackager
{
  Class webSocketManagerClass = objc_lookUpClass("ABI11_0_0RCTWebSocketManager");
  id<ABI11_0_0RCTWebSocketProxy> webSocketManager = (id <ABI11_0_0RCTWebSocketProxy>)[webSocketManagerClass sharedInstance];
...
- (void)socketProxy:(__unused id<ABI11_0_0RCTWebSocketProxy>)sender didReceiveMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([target isEqualToString:@"bridge"]) {
    if ([action isEqualToString:@"reload"]) {
      if ([options[@"debug"] boolValue]) {
        _bridge.executorClass = objc_lookUpClass("ABI11_0_0RCTWebSocketExecutor");
      }
      [_bridge reload];
    }
  }
...
- (void)settingsDidChange
{
  ...
  __weak ABI11_0_0RCTDevMenu *weakSelf = self;
  NSDictionary *settings = [_defaults objectForKey:ABI11_0_0RCTDevMenuSettingsKey];
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateSettings:(NSDictionary *)settings
{
  ...
  for (ABI11_0_0RCTDevMenuItem *item in _extraMenuItems) {
    if (item.key) {
...
- (void)updateSetting:(NSString *)name value:(id)value
{
  ...
  [_defaults setObject:_settings forKey:ABI11_0_0RCTDevMenuSettingsKey];
...
- (void)jsLoaded:(NSNotification *)notification
{
  ...
  _liveReloadURL = nil;
  ABI11_0_0RCTSourceCode *sourceCodeModule = [_bridge moduleForClass:[ABI11_0_0RCTSourceCode class]];
  if (!sourceCodeModule.scriptURL) {
    if (!sourceCodeModule) {
      ABI11_0_0RCTLogWarn(@"ABI11_0_0RCTSourceCode module not found");
    } else if (!ABI11_0_0RCTRunningInTestEnvironment()) {
      ABI11_0_0RCTLogWarn(@"ABI11_0_0RCTSourceCode module scriptURL has not been set");
    }
...
{
  [self addItem:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:title handler:handler]];
}
...
- (NSArray<ABI11_0_0RCTDevMenuItem *> *)menuItems
{
  NSMutableArray<ABI11_0_0RCTDevMenuItem *> *items = [NSMutableArray new];

  // Add built-in items

  __weak ABI11_0_0RCTDevMenu *weakSelf = self;

  [items addObject:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:@"Reload" handler:^{
    [weakSelf reload];
  }]];

  Class jsDebuggingExecutorClass = objc_lookUpClass("ABI11_0_0RCTWebSocketExecutor");
  if (!jsDebuggingExecutorClass) {
    [items addObject:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", _webSocketExecutorName] handler:^{
      UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"%@ Debugger Unavailable", self->_webSocketExecutorName]
                                                                               message:[NSString stringWithFormat:@"You need to include the ABI11_0_0RCTWebSocket library to enable %@ debugging", self->_webSocketExecutorName]
                                                                        preferredStyle:UIAlertControllerStyleAlert];

      [ABI11_0_0RCTPresentedViewController() presentViewController:alertController animated:YES completion:NULL];
    }]];
  } else {
    ...
    [items addObject:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:debugTitleJS handler:^{
  ...
  if (_liveReloadURL) {
    ...
    [items addObject:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:liveReloadTitle handler:^{
    ...
    NSString *profilingTitle  = ABI11_0_0RCTProfileIsProfiling() ? @"Stop Systrace" : @"Start Systrace";
    [items addObject:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:profilingTitle handler:^{
      __typeof(self) strongSelf = weakSelf;
      if (strongSelf) {
  ...
  if ([self hotLoadingAvailable]) {
    ...
    [items addObject:[ABI11_0_0RCTDevMenuItem buttonItemWithTitle:hotLoadingTitle handler:^{
      __typeof(self) strongSelf = weakSelf;
      if (strongSelf) {
...
ABI11_0_0RCT_EXPORT_METHOD(show)
{
  if (_actionSheet || !_bridge || ABI11_0_0RCTRunningInAppExtension()) {
    return;
  }

  NSString *title = [NSString stringWithFormat:@"ReactABI11_0_0 Native: Development (%@)", [_bridge class]];
  _actionSheet = [UIAlertController alertControllerWithTitle:title
                                                     message:@""
                                              preferredStyle:UIAlertControllerStyleActionSheet];
  NSArray<ABI11_0_0RCTDevMenuItem *> *items = [self menuItems];
  for (ABI11_0_0RCTDevMenuItem *item in items) {
    switch (item.type) {
      case ABI11_0_0RCTDevMenuTypeButton: {
  ...
  [ABI11_0_0RCTPresentedViewController() presentViewController:_actionSheet animated:YES completion:^(void){}];
}
...
ABI11_0_0RCT_EXPORT_METHOD(debugRemotely:(BOOL)enableDebug)
{
  Class jsDebuggingExecutorClass = NSClassFromString(@"ABI11_0_0RCTWebSocketExecutor");
...
ABI11_0_0RCT_EXPORT_METHOD(setProfilingEnabled:(BOOL)enabled)
{
  _profilingEnabled = enabled;
  [self updateSetting:@"profilingEnabled" value:@(_profilingEnabled)];

  if (_liveReloadURL && enabled != ABI11_0_0RCTProfileIsProfiling()) {
    if (enabled) {
      [_bridge startProfiling];
    } else {
      [_bridge stopProfiling:^(NSData *logData) {
        ABI11_0_0RCTProfileSendResult(self->_bridge, @"systrace", logData);
      }];
    }
  }
}

ABI11_0_0RCT_EXPORT_METHOD(setLiveReloadEnabled:(BOOL)enabled)
{
...
ABI11_0_0RCT_EXPORT_METHOD(setHotLoadingEnabled:(BOOL)enabled)
{
  ...
  if (ABI11_0_0RCTGetURLQueryParam(_bridge.bundleURL, @"hot").boolValue != actuallyEnabled) {
    _bridge.bundleURL = ABI11_0_0RCTURLByReplacingQueryParam(_bridge.bundleURL, @"hot",
...
- (void)checkForUpdates
{
  ...
  _updateTask = [[NSURLSession sharedSession] dataTaskWithURL:_liveReloadURL completionHandler:
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ABI11_0_0RCTDevMenu *strongSelf = weakSelf;
...
- (void)addItem:(ABI11_0_0RCTDevMenu *)item {}

@end

#endif

@implementation  ABI11_0_0RCTBridge (ABI11_0_0RCTDevMenu)

- (ABI11_0_0RCTDevMenu *)devMenu
{
#if ABI11_0_0RCT_DEV
  return [self moduleForClass:[ABI11_0_0RCTDevMenu class]];
#else
  return nil;
#endif
}
