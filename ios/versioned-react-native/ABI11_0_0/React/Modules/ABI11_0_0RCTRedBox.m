...
#import "ABI11_0_0RCTErrorInfo.h"
#import "ABI11_0_0RCTUtils.h"
#import "ABI11_0_0RCTJSStackFrame.h"

#if ABI11_0_0RCT_DEV

@class ABI11_0_0RCTRedBoxWindow;

@protocol ABI11_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI11_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI11_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI11_0_0RCTRedBoxWindow *)redBoxWindow;

@end

@interface ABI11_0_0RCTRedBoxWindow : UIWindow <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, weak) id<ABI11_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI11_0_0RCTRedBoxWindow
{
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI11_0_0RCTJSStackFrame *> *_lastStackTrace;
}
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<ABI11_0_0RCTJSStackFrame *> *)stack isUpdate:(BOOL)isUpdate
{
...
- (void)dismiss
{
  ...
  [ABI11_0_0RCTSharedApplication().delegate.window makeKeyWindow];
}
...
- (void)copyStack
{
  ...
  for (ABI11_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI11_0_0RCTJSStackFrame *)stackFrame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI11_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI11_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI11_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI11_0_0RCTKeyCommands for this, but since
...
@interface ABI11_0_0RCTRedBox () <ABI11_0_0RCTInvalidating, ABI11_0_0RCTRedBoxWindowActionDelegate>
@end

@implementation ABI11_0_0RCTRedBox
{
  ABI11_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI11_0_0RCTErrorCustomizer>> *_errorCustomizers;
}
...
- (ABI11_0_0RCTErrorInfo *)_customizeError:(ABI11_0_0RCTErrorInfo *)error
{
  ...
  for (id<ABI11_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI11_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
{
  NSArray<ABI11_0_0RCTJSStackFrame *> *stack = [ABI11_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
{
  if (![[stack firstObject] isKindOfClass:[ABI11_0_0RCTJSStackFrame class]]) {
    stack = [ABI11_0_0RCTJSStackFrame stackFramesWithDictionaries:stack];
  }

  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_window) {
      self->_window = [[ABI11_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
      self->_window.actionDelegate = self;
    }
    ABI11_0_0RCTErrorInfo *errorInfo = [[ABI11_0_0RCTErrorInfo alloc] initWithErrorMessage:message
                                                                   stack:stack];
...
ABI11_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI11_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI11_0_0RCTJSStackFrame *)stackFrame
{
  if (![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
    ABI11_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI11_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI11_0_0RCTRedBoxWindow *)redBoxWindow {
  [_bridge requestReload];
}

@end

@implementation ABI11_0_0RCTBridge (ABI11_0_0RCTRedBox)

- (ABI11_0_0RCTRedBox *)redBox
{
  return [self moduleForClass:[ABI11_0_0RCTRedBox class]];
}
...
- (ABI11_0_0RCTRedBox *)redBox { return nil; }
