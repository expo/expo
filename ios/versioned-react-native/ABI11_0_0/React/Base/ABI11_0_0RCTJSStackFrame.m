...
static NSRegularExpression *ABI11_0_0RCTJSStackFrameRegex()
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    if (regexError) {
      ABI11_0_0RCTLogError(@"Failed to build regex: %@", [regexError localizedDescription]);
    }
  });
  return _regex;
}

@implementation ABI11_0_0RCTJSStackFrame
...
{
  return @{
     @"methodName": ABI11_0_0RCTNullIfNil(self.methodName),
     @"file": ABI11_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI11_0_0RCTJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
  if (!match) {
    return nil;
  }
...
+ (NSArray<ABI11_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI11_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI11_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI11_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
