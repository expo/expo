...
@implementation ABI11_0_0RCTModuleData
{
...
- (void)setUp
{
  ...
  _hasConstantsToExport = ABI11_0_0RCTClassOverridesInstanceMethod(_moduleClass, @selector(constantsToExport));
}
...
{
  ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"[ABI11_0_0RCTModuleData setUpInstanceAndBridge] [_instanceLock lock]", @{ @"moduleClass": NSStringFromClass(_moduleClass) });
  {
    std::unique_lock<std::mutex> lock(_instanceLock);

    if (!_setupComplete && _bridge.valid) {
      if (!_instance) {
        if (ABI11_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI11_0_0RCTAssertMainQueue();
        }
        ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"[ABI11_0_0RCTModuleData setUpInstanceAndBridge] [_moduleClass new]",  @{ @"moduleClass": NSStringFromClass(_moduleClass) });
        ...
        if (!_instance) {
          ...
          _setupComplete = YES;
          ABI11_0_0RCTLogWarn(@"The module %@ is returning nil from its constructor. You "
                     "may need to instantiate it yourself and pass it into the "
                     "bridge.", _moduleClass);
        }
      }

      if (_instance && ABI11_0_0RCTProfileIsProfiling()) {
        ABI11_0_0RCTProfileHookInstance(_instance);
      }
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"[ABI11_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
      [(id)_instance setValue:_bridge forKey:@"bridge"];
    }
    @catch (NSException *exception) {
      ABI11_0_0RCTLogError(@"%@ has no setter or ivar for its bridge, which is not "
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"[ABI11_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI11_0_0RCTDidInitializeModuleNotification
                                                        object:_bridge
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"[ABI11_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ReactABI11_0_0.%@Queue", self.name];
      ...
      if (implementsMethodQueue) {
        ...
        @catch (NSException *exception) {
          ABI11_0_0RCTLogError(@"%@ is returning nil for its methodQueue, which is not "
...
- (id<ABI11_0_0RCTBridgeModule>)instance
{
  if (!_setupComplete) {
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI11_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, @"[ABI11_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI11_0_0RCTIsMainQueue()) {
        ABI11_0_0RCTLogWarn(@"ABI11_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI11_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI11_0_0RCTBridgeMethod>> *)methods
{
  if (!_methods) {
    NSMutableArray<id<ABI11_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
    ...
    while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
      ...
      for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
          ...
          id<ABI11_0_0RCTBridgeMethod> moduleMethod =
            [[ABI11_0_0RCTModuleMethod alloc] initWithMethodSignature:entries[1]
...
- (void)gatherConstants
{
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI11_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    if (!ABI11_0_0RCTIsMainQueue()) {
      ABI11_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
    }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ABI11_0_0RCTExecuteOnMainThread(^{
...
- (NSArray *)config
{
  ...
  ABI11_0_0RCT_PROFILE_BEGIN_EVENT(ABI11_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI11_0_0RCTModuleData config] %@", _moduleClass]), nil);
  ...
  for (id<ABI11_0_0RCTBridgeMethod> method in self.methods) {
    if (method.functionType == ABI11_0_0RCTFunctionTypePromise) {
      if (!promiseMethods) {
        promiseMethods = [NSMutableArray new];
      }
      [promiseMethods addObject:@(methods.count)];
    }
    else if (method.functionType == ABI11_0_0RCTFunctionTypeSync) {
      if (!syncMethods) {
  ...
  NSArray *config = @[
    self.name,
    ABI11_0_0RCTNullIfNil(constants),
    ABI11_0_0RCTNullIfNil(methods),
    ABI11_0_0RCTNullIfNil(promiseMethods),
    ABI11_0_0RCTNullIfNil(syncMethods)
  ];
  ABI11_0_0RCT_PROFILE_END_EVENT(ABI11_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI11_0_0RCTModuleData config] %@", _moduleClass]));
  return config;
}
