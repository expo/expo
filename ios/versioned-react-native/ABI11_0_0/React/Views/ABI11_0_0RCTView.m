...
@implementation UIView (ABI11_0_0RCTViewUnmounting)

- (void)ReactABI11_0_0_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI11_0_0_remountAllSubviews];
  }
}

- (void)ReactABI11_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ReactABI11_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}

- (UIView *)ReactABI11_0_0_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isReactABI11_0_0RootView]) {
      break;
    }
...
static NSString *ABI11_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  ...
  for (UIView *subview in view.subviews) {
    ...
    if (label) {
      ...
      [str appendString:ABI11_0_0RCTRecursiveAccessibilityLabel(subview)];
    }
  }
  return str;
}

@implementation ABI11_0_0RCTView
{
  UIColor *_backgroundColor;
}

@synthesize ReactABI11_0_0ZIndex = _ReactABI11_0_0ZIndex;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderStyle = ABI11_0_0RCTBorderStyleSolid;
...
ABI11_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:unused)
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI11_0_0RCTRecursiveAccessibilityLabel(self);
}

- (void)setPointerEvents:(ABI11_0_0RCTPointerEvents)pointerEvents
{
  _pointerEvents = pointerEvents;
  self.userInteractionEnabled = (pointerEvents != ABI11_0_0RCTPointerEventsNone);
  if (pointerEvents == ABI11_0_0RCTPointerEventsBoxNone) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI11_0_0RCTPointerEventsNone || _pointerEvents == ABI11_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI11_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI11_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI11_0_0RCTLogError(@"Invalid pointer-events specified %zd on %@", _pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)description
{
  ...
  NSString *replacement = [NSString stringWithFormat:@"; ReactABI11_0_0Tag: %@;", self.ReactABI11_0_0Tag];
  return [superDescription stringByReplacingCharactersInRange:semicolonRange withString:replacement];
}

#pragma mark - Statics for dealing with layoutGuides

+ (void)autoAdjustInsetsForView:(UIView<ABI11_0_0RCTAutoInsetsProtocol> *)parentView
...
{
  while (view) {
    UIViewController *controller = view.ReactABI11_0_0ViewController;
    if (controller) {
      return (UIEdgeInsets){
...
- (void)ReactABI11_0_0_remountAllSubviews
{
  if (_removeClippedSubviews) {
    for (UIView *view in self.sortedReactABI11_0_0Subviews) {
      if (view.superview != self) {
        [self addSubview:view];
        [view ReactABI11_0_0_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ReactABI11_0_0_remountAllSubviews];
  }
}
...
- (void)ReactABI11_0_0_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ReactABI11_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
  ...
  for (UIView *view in self.sortedReactABI11_0_0Subviews) {
    if (!CGRectIsEmpty(CGRectIntersection(clipRect, view.frame))) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ReactABI11_0_0_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ReactABI11_0_0_remountAllSubviews];
  }
...
- (void)didUpdateReactABI11_0_0Subviews
{
  if (_removeClippedSubviews) {
    ...
    [super didUpdateReactABI11_0_0Subviews];
  }
}
...
- (void)updateClippedSubviews
{
  ...
  if (clipView) {
    [self ReactABI11_0_0_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
- (ABI11_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat topScaleFactor = ABI11_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI11_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI11_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI11_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI11_0_0RCTCornerRadii){
...
{
  return (ABI11_0_0RCTBorderColors){
...
- (void)ReactABI11_0_0SetFrame:(CGRect)frame
{
  ...
  [super ReactABI11_0_0SetFrame:frame];
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI11_0_0RCTUpdateShadowPathForView(self);

  const ABI11_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI11_0_0RCTBorderColors borderColors = [self borderColors];

  BOOL useIOSBorderRendering =
  !ABI11_0_0RCTRunningInTestEnvironment() &&
  ABI11_0_0RCTCornerRadiiAreEqual(cornerRadii) &&
  ABI11_0_0RCTBorderInsetsAreEqual(borderInsets) &&
  ABI11_0_0RCTBorderColorsAreEqual(borderColors) &&
  _borderStyle == ABI11_0_0RCTBorderStyleSolid &&

  // iOS draws borders in front of the content whereas ABI11_0_0CSS draws them behind
  // the content. For this reason, only use iOS border drawing when clipping
  // or when the border is hidden.

  (borderInsets.top == 0 || (borderColors.top && CGColorGetAlpha(borderColors.top) == 0) || self.clipsToBounds);

  // iOS clips to the outside of the border, but ABI11_0_0CSS clips to the inside. To
  ...
  UIImage *image = ABI11_0_0RCTGetBorderImage(_borderStyle,
                                     layer.bounds.size,
                                     cornerRadii,
                                     borderInsets,
                                     borderColors,
...
static BOOL ABI11_0_0RCTLayerHasShadow(CALayer *layer)
{
...
- (void)ReactABI11_0_0SetInheritedBackgroundColor:(UIColor *)inheritedBackgroundColor
{
  ...
  if (ABI11_0_0RCTLayerHasShadow(self.layer)) {
    self.backgroundColor = inheritedBackgroundColor;
  }
}

static void ABI11_0_0RCTUpdateShadowPathForView(ABI11_0_0RCTView *view)
{
  if (ABI11_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {

      // If view has a solid background color, calculate shadow path from border
      const ABI11_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI11_0_0RCTCornerInsets cornerInsets = ABI11_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI11_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
      ...
      ABI11_0_0RCTLogWarn(@"View #%@ of type %@ has a shadow set but cannot calculate "
                 "shadow efficiently. Consider setting a background color to "
                 "fix this, or apply the shadow to a more specific component.",
                 view.ReactABI11_0_0Tag, [view class]);
    }
  }
}
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    ...
    const ABI11_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI11_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      ...
      CGPathRef path = ABI11_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI11_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderStyle(side)                           \
  - (void)setBorder##side##Style:(ABI11_0_0RCTBorderStyle)style \
  {                                                    \
