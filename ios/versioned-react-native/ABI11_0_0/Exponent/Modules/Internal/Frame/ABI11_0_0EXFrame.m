...
#define ABI11_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC 0.05

NS_ASSUME_NONNULL_BEGIN

// we don't import ABI11_0_0EXFrameReactABI11_0_0AppManager.h because it's unversioned,
// so we need to let the compiler know that somebody, somewhere, supports this interface.
@interface ABI11_0_0EXFrameReactABI11_0_0AppManagerWithNoWarningsHack
...
@property (nonatomic, strong) UIView * __nullable ReactABI11_0_0RootView;
@property (nonatomic, strong) id __nullable ReactABI11_0_0Bridge;

@end

@interface ABI11_0_0EXFrame () <ABI11_0_0RCTInvalidating, ABI11_0_0RCTExceptionsManagerDelegate>
...
@implementation ABI11_0_0EXFrame

ABI11_0_0RCT_NOT_IMPLEMENTED(- (instancetype)initWithCoder:(NSCoder *)coder)

- (instancetype)init
{
  if (self = [super init]) {
    Class unversionedAppManagerClass = NSClassFromString(@"EXFrameReactAppManager");
    ABI11_0_0RCTAssert(unversionedAppManagerClass, @"Cannot init a Frame with no %@ class", @"EXFrameReactAppManager");
...
- (void)_checkForReload
{
  ...
  if (_needsReload) {
    ...
    _tmrReload = [NSTimer scheduledTimerWithTimeInterval:ABI11_0_0EX_FRAME_RELOAD_DEBOUNCE_SEC target:_appManager selector:@selector(reload) userInfo:nil repeats:NO];
  }
}
...
- (void)layoutSubviews
{
  ...
  if ([_appManager ReactABI11_0_0RootView]) {
    [_appManager ReactABI11_0_0RootView].frame = self.bounds;
  }
}
...
#pragma mark - ABI11_0_0EXReactABI11_0_0AppManagerDelegate

- (void)ReactABI11_0_0AppManagerDidInitApp:(id)appManager
{
  UIView *ReactABI11_0_0RootView = [appManager ReactABI11_0_0RootView];
  ReactABI11_0_0RootView.frame = self.bounds;
  [self addSubview:ReactABI11_0_0RootView];
}

- (void)ReactABI11_0_0AppManagerDidDestroyApp:(id)appManager
{
  if (_tmrReload) {
    [_tmrReload invalidate];
    _tmrReload = nil;
...
- (void)ReactABI11_0_0AppManagerStartedLoadingJavaScript:(id)appManager
{
  if (_onLoadingStart) {
    _onLoadingStart(nil);
  }
...
- (void)ReactABI11_0_0AppManagerFinishedLoadingJavaScript:(id)appManager
{
  if (_viewTestTimer) {
    [_viewTestTimer invalidate];
  }
...
- (void)ReactABI11_0_0AppManager:(id)appManager failedToLoadJavaScriptWithError:(NSError *)error
{
  [self _sendLoadingError:error];
}

- (void)ReactABI11_0_0AppManager:(id)appManager failedToDownloadBundleWithError:(NSError *)error
{
  
}

- (void)ReactABI11_0_0AppManagerDidForeground:(id)appManager
{
...
- (void)_checkAppFinishedLoading:(NSTimer *)timer
{
  ...
  if ([_appManager ReactABI11_0_0RootView] &&
      [_appManager ReactABI11_0_0RootView].subviews.count > 0 &&
      [_appManager ReactABI11_0_0RootView].subviews.firstObject.subviews.count > 0) {
    ABI11_0_0EXAppLoadingManager *appLoading = [_appManager appLoadingManagerInstance];
...
# pragma mark - ABI11_0_0RCTExceptionsManagerDelegate
