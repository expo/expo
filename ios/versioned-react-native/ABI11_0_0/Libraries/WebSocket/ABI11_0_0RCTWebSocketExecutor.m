...
#import "ABI11_0_0RCTWebSocketExecutor.h"

#import "ABI11_0_0RCTBridge.h"
#import "ABI11_0_0RCTConvert.h"
#import "ABI11_0_0RCTLog.h"
#import "ABI11_0_0RCTUtils.h"
#import "ABI11_0_0RCTSRWebSocket.h"

typedef void (^ABI11_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI11_0_0RCTWebSocketExecutor () <ABI11_0_0RCTSRWebSocketDelegate>

@end

@implementation ABI11_0_0RCTWebSocketExecutor
{
  ABI11_0_0RCTSRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI11_0_0RCTWSMessageCallback> *_callbacks;
...
- (void)setUp
{
  if (!_url) {
    ...
    _url = [ABI11_0_0RCTConvert NSURL:URLString];
  }

  _jsQueue = dispatch_queue_create("com.facebook.ReactABI11_0_0.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  _socket = [[ABI11_0_0RCTSRWebSocket alloc] initWithURL:_url];
  ...
  if (![self connectToProxy]) {
    ABI11_0_0RCTLogError(@"Connection to %@ timed out. Are you running node proxy? If "
                 "you are running on the device, check if you have the right IP "
                 "address in `ABI11_0_0RCTWebSocketExecutor.m`.", _url);
    [self invalidate];
    return;
  }
  ...
  if (!runtimeIsReady) {
    ABI11_0_0RCTLogError(@"Runtime is not ready for debugging.\n "
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI11_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI11_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
- (void)webSocket:(ABI11_0_0RCTSRWebSocket *)webSocket didFailWithError:(NSError *)error
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    ABI11_0_0RCTLogError(@"WebSocket connection failed with error %@", error);
  });
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message waitForReply:(ABI11_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    ...
    [self->_socket send:ABI11_0_0RCTJSONStringify(messageWithID, NULL)];
  });
}

- (void)executeApplicationScript:(NSData *)script sourceURL:(NSURL *)URL onComplete:(ABI11_0_0RCTJavaScriptCompleteBlock)onComplete
{
  NSDictionary<NSString *, id> *message = @{
    @"method": @"executeApplicationScript",
    @"url": ABI11_0_0RCTNullIfNil(URL.absoluteString),
...
- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI11_0_0RCTJavaScriptCallback)onComplete
{
  ABI11_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  NSDictionary<NSString *, id> *message = @{
    @"method": method,
    @"arguments": arguments
  };
  [self sendMessage:message waitForReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
    ...
    id objcValue = ABI11_0_0RCTJSONParse(result, NULL);
    onComplete(objcValue, nil);
  }];
}

- (void)injectJSONText:(NSString *)script asGlobalObjectNamed:(NSString *)objectName callback:(ABI11_0_0RCTJavaScriptCompleteBlock)onComplete
{
  dispatch_async(_jsQueue, ^{
...
{
  ABI11_0_0RCTExecuteOnMainQueue(block);
}
...
- (BOOL)isValid
{
  return _socket != nil && _socket.readyState == ABI11_0_0RCTSR_OPEN;
}

- (void)dealloc
{
  ABI11_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
