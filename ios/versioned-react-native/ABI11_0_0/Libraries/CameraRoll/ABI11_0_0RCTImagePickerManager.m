...
#import "ABI11_0_0RCTImagePickerManager.h"
...
@interface ABI11_0_0RCTImagePickerManager ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>

@end

@implementation ABI11_0_0RCTImagePickerManager
{
  NSMutableArray<UIImagePickerController *> *_pickers;
  NSMutableArray<ABI11_0_0RCTResponseSenderBlock> *_pickerCallbacks;
  NSMutableArray<ABI11_0_0RCTResponseSenderBlock> *_pickerCancelCallbacks;
}

ABI11_0_0RCT_EXPORT_MODULE(ImagePickerIOS);
...
ABI11_0_0RCT_EXPORT_METHOD(canRecordVideos:(ABI11_0_0RCTResponseSenderBlock)callback)
{
...
ABI11_0_0RCT_EXPORT_METHOD(canUseCamera:(ABI11_0_0RCTResponseSenderBlock)callback)
{
  callback(@[@([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])]);
}

ABI11_0_0RCT_EXPORT_METHOD(openCameraDialog:(NSDictionary *)config
                  successCallback:(ABI11_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI11_0_0RCTResponseSenderBlock)cancelCallback)
{
  ...
  if ([ABI11_0_0RCTConvert BOOL:config[@"videoMode"]]) {
...
ABI11_0_0RCT_EXPORT_METHOD(openSelectDialog:(NSDictionary *)config
                  successCallback:(ABI11_0_0RCTResponseSenderBlock)callback
                  cancelCallback:(ABI11_0_0RCTResponseSenderBlock)cancelCallback)
{
  if (ABI11_0_0RCTRunningInAppExtension()) {
  ...
  if ([ABI11_0_0RCTConvert BOOL:config[@"showImages"]]) {
    [allowedTypes addObject:(NSString *)kUTTypeImage];
  }
  if ([ABI11_0_0RCTConvert BOOL:config[@"showVideos"]]) {
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
  ...
  if (imageURL) {
    [self _dismissPicker:picker args:@[imageURL.absoluteString, ABI11_0_0RCTNullIfNil(height), ABI11_0_0RCTNullIfNil(width)]];
    return;
  }
        ...
        cancelCallback:(ABI11_0_0RCTResponseSenderBlock)cancelCallback
{
  ...
  UIViewController *rootViewController = ABI11_0_0RCTPresentedViewController();
...
- (void)_dismissPicker:(UIImagePickerController *)picker args:(NSArray *)args
{
  ...
  ABI11_0_0RCTResponseSenderBlock successCallback = _pickerCallbacks[index];
  ABI11_0_0RCTResponseSenderBlock cancelCallback = _pickerCancelCallbacks[index];
