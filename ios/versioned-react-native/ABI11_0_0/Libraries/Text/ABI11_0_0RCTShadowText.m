...
NSString *const ABI11_0_0RCTShadowViewAttributeName = @"ABI11_0_0RCTShadowViewAttributeName";
NSString *const ABI11_0_0RCTIsHighlightedAttributeName = @"IsHighlightedAttributeName";
NSString *const ABI11_0_0RCTReactABI11_0_0TagAttributeName = @"ReactABI11_0_0TagAttributeName";

CGFloat const ABI11_0_0RCTTextAutoSizeDefaultMinimumFontScale       = 0.5f;
CGFloat const ABI11_0_0RCTTextAutoSizeWidthErrorMargin              = 0.05f;
CGFloat const ABI11_0_0RCTTextAutoSizeHeightErrorMargin             = 0.025f;
CGFloat const ABI11_0_0RCTTextAutoSizeGranularity                   = 0.001f;

@implementation ABI11_0_0RCTShadowText
{
...
static ABI11_0_0CSSSize ABI11_0_0RCTMeasure(void *context, float width, ABI11_0_0CSSMeasureMode widthMode, float height, ABI11_0_0CSSMeasureMode heightMode)
{
  ABI11_0_0RCTShadowText *shadowText = (__bridge ABI11_0_0RCTShadowText *)context;
  ...
  ABI11_0_0CSSSize result;
  result.width = ABI11_0_0RCTCeilPixelValue(computedSize.width);
  if (shadowText->_effectiveLetterSpacing < 0) {
    result.width -= shadowText->_effectiveLetterSpacing;
  }
  result.height = ABI11_0_0RCTCeilPixelValue(computedSize.height);
  return result;
}
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    ABI11_0_0CSSNodeSetMeasureFunc(self.cssNode, ABI11_0_0RCTMeasure);
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(contentSizeMultiplierDidChange:)
                                                 name:ABI11_0_0RCTUIManagerWillUpdateViewsDueToContentSizeMultiplierChangeNotification
                                               object:nil];
  }
  return self;
}
...
{
  ABI11_0_0CSSNodeMarkDirty(self.cssNode);
  [self dirtyText];
}
...
{
  if ([[self ReactABI11_0_0Superview] isKindOfClass:[ABI11_0_0RCTShadowText class]]) {
  ...
  NSNumber *parentTag = [[self ReactABI11_0_0Superview] ReactABI11_0_0Tag];
  NSTextStorage *textStorage = [self buildTextStorageForWidth:width widthMode:ABI11_0_0CSSMeasureModeExactly];
  CGRect textFrame = [self calculateTextFrame:textStorage];
  [applierBlocks addObject:^(NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI11_0_0RCTText *view = (ABI11_0_0RCTText *)viewRegistry[self.ReactABI11_0_0Tag];
     ...
     * textStorage (aka attributed string) is copied over from the ABI11_0_0RCTShadowText
     * to the ABI11_0_0RCTText view in time to be used to update the editable text content.
     * TODO: we should establish a delegate relationship betweeen ABI11_0_0RCTTextView
     * and its contaned ABI11_0_0RCTText element when they get inserted and get rid of this
     */
    UIView *parentView = viewRegistry[parentTag];
    if ([parentView respondsToSelector:@selector(performTextUpdate)]) {
      [(ABI11_0_0RCTTextView *)parentView performTextUpdate];
    }
  }];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  ...
  NSTextStorage *textStorage = [self buildTextStorageForWidth:self.frame.size.width widthMode:ABI11_0_0CSSMeasureModeExactly];
  NSLayoutManager *layoutManager = textStorage.layoutManagers.firstObject;
  NSTextContainer *textContainer = layoutManager.textContainers.firstObject;
  NSRange glyphRange = [layoutManager glyphRangeForTextContainer:textContainer];
  NSRange characterRange = [layoutManager characterRangeForGlyphRange:glyphRange actualGlyphRange:NULL];
  [layoutManager.textStorage enumerateAttribute:ABI11_0_0RCTShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(ABI11_0_0RCTShadowView *child, NSRange range, BOOL *_) {
    if (child) {
      ABI11_0_0CSSNodeRef childNode = child.cssNode;
      float width = ABI11_0_0CSSNodeStyleGetWidth(childNode);
      float height = ABI11_0_0CSSNodeStyleGetHeight(childNode);
      if (ABI11_0_0CSSValueIsUndefined(width) || ABI11_0_0CSSValueIsUndefined(height)) {
        ABI11_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      UIFont *font = [textStorage attribute:NSFontAttributeName atIndex:range.location effectiveRange:nil];
      CGRect glyphRect = [layoutManager boundingRectForGlyphRange:range inTextContainer:textContainer];
      CGRect childFrame = {{
        ABI11_0_0RCTRoundPixelValue(glyphRect.origin.x),
        ABI11_0_0RCTRoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
      }, {
        ABI11_0_0RCTRoundPixelValue(width),
        ABI11_0_0RCTRoundPixelValue(height)
      }};
...
- (NSTextStorage *)buildTextStorageForWidth:(CGFloat)width widthMode:(ABI11_0_0CSSMeasureMode)widthMode
{
  ...
  textContainer.size = (CGSize){widthMode == ABI11_0_0CSSMeasureModeUndefined ? CGFLOAT_MAX : width, CGFLOAT_MAX};
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [ABI11_0_0RCTFont updateFont:nil
  ...
  for (ABI11_0_0RCTShadowView *child in [self ReactABI11_0_0Subviews]) {
    if ([child isKindOfClass:[ABI11_0_0RCTShadowText class]]) {
      ABI11_0_0RCTShadowText *shadowText = (ABI11_0_0RCTShadowText *)child;
      ...
      ABI11_0_0RCTShadowRawText *shadowRawText = (ABI11_0_0RCTShadowRawText *)child;
      [attributedString appendAttributedString:[[NSAttributedString alloc] initWithString:shadowRawText.text ?: @""]];
      [child setTextComputed];
    } else {
      float width = ABI11_0_0CSSNodeStyleGetWidth(child.cssNode);
      float height = ABI11_0_0CSSNodeStyleGetHeight(child.cssNode);
      if (ABI11_0_0CSSValueIsUndefined(width) || ABI11_0_0CSSValueIsUndefined(height)) {
        ABI11_0_0RCTLogError(@"Views nested within a <Text> must have a width and height");
      }
      ...
      [attachmentString addAttribute:ABI11_0_0RCTShadowViewAttributeName value:child range:(NSRange){0, attachmentString.length}];
      [attributedString appendAttributedString:attachmentString];
      if (height > heightOfTallestSubview) {
        heightOfTallestSubview = height;
      }
      // Don't call setTextComputed on this child. ABI11_0_0RCTTextManager takes care of
  ...
  if (_isHighlighted) {
    [self _addAttribute:ABI11_0_0RCTIsHighlightedAttributeName withValue:@YES toAttributedString:attributedString];
  }
  ...
  [self _addAttribute:ABI11_0_0RCTReactABI11_0_0TagAttributeName withValue:self.ReactABI11_0_0Tag toAttributedString:attributedString];
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  if (newTextAlign == NSTextAlignmentRight || newTextAlign == NSTextAlignmentLeft) {
    ABI11_0_0RCTShadowView *view = self;
    while (view != nil && ABI11_0_0CSSNodeStyleGetDirection(view.cssNode) == ABI11_0_0CSSDirectionInherit) {
      view = [view ReactABI11_0_0Superview];
    }
    if (view != nil && ABI11_0_0CSSNodeStyleGetDirection(view.cssNode) == ABI11_0_0CSSDirectionRTL) {
  ...
  // Text decoration
  if (_textDecorationLine == ABI11_0_0RCTTextDecorationLineTypeUnderline ||
      _textDecorationLine == ABI11_0_0RCTTextDecorationLineTypeUnderlineStrikethrough) {
    [self _addAttribute:NSUnderlineStyleAttributeName withValue:@(_textDecorationStyle)
     toAttributedString:attributedString];
  }
  if (_textDecorationLine == ABI11_0_0RCTTextDecorationLineTypeStrikethrough ||
      _textDecorationLine == ABI11_0_0RCTTextDecorationLineTypeUnderlineStrikethrough){
...
- (CGRect)updateStorage:(NSTextStorage *)textStorage toFitFrame:(CGRect)frame
{
  ...
  frame.origin.y = self.paddingAsInsets.top + ABI11_0_0RCTRoundPixelValue((CGRectGetHeight(frame) - requiredSize.height) / 2.0f);
  return frame;
}
                               ...
                               prevMid:(CGFloat)prevMid
{
  ...
  if (round((prevMid / ABI11_0_0RCTTextAutoSizeGranularity)) == round((midScale / ABI11_0_0RCTTextAutoSizeGranularity))) {
    //Bail because we can't meet error margin.
    return [self calculateSize:textStorage];
  } else {
    ABI11_0_0RCTSizeComparison comparison = [self attemptScale:midScale
                                            inStorage:textStorage
                                             forFrame:frame];
    if (comparison == ABI11_0_0RCTSizeWithinRange) {
      return [self calculateSize:textStorage];
    } else if (comparison == ABI11_0_0RCTSizeTooLarge) {
      return [self calculateOptimumScaleInFrame:frame
                                     forStorage:textStorage
                                       minScale:minScale
                                       maxScale:midScale - ABI11_0_0RCTTextAutoSizeGranularity
                                        prevMid:midScale];
    } else {
      return [self calculateOptimumScaleInFrame:frame
                                     forStorage:textStorage
                                       minScale:midScale + ABI11_0_0RCTTextAutoSizeGranularity
                                       maxScale:maxScale
                                        prevMid:midScale];
    }
  }
...
- (ABI11_0_0RCTSizeComparison)attemptScale:(CGFloat)scale
                         ...
                         forFrame:(CGRect)frame
{
  ...
  if (fitLines && fitSize) {
    if ((requiredSize.width + (CGRectGetWidth(frame) * ABI11_0_0RCTTextAutoSizeWidthErrorMargin)) > CGRectGetWidth(frame) &&
        (requiredSize.height + (CGRectGetHeight(frame) * ABI11_0_0RCTTextAutoSizeHeightErrorMargin)) > CGRectGetHeight(frame))
    {
      return ABI11_0_0RCTSizeWithinRange;
    } else {
      return ABI11_0_0RCTSizeTooSmall;
    }
  } else {
    return ABI11_0_0RCTSizeTooLarge;
  }
}
...
#define ABI11_0_0RCT_TEXT_PROPERTY(setProp, ivar, type) \
...
ABI11_0_0RCT_TEXT_PROPERTY(AdjustsFontSizeToFit, _adjustsFontSizeToFit, BOOL)
ABI11_0_0RCT_TEXT_PROPERTY(Color, _color, UIColor *)
ABI11_0_0RCT_TEXT_PROPERTY(FontFamily, _fontFamily, NSString *)
ABI11_0_0RCT_TEXT_PROPERTY(FontSize, _fontSize, CGFloat)
ABI11_0_0RCT_TEXT_PROPERTY(FontWeight, _fontWeight, NSString *)
ABI11_0_0RCT_TEXT_PROPERTY(FontStyle, _fontStyle, NSString *)
ABI11_0_0RCT_TEXT_PROPERTY(FontVariant, _fontVariant, NSArray *)
ABI11_0_0RCT_TEXT_PROPERTY(IsHighlighted, _isHighlighted, BOOL)
ABI11_0_0RCT_TEXT_PROPERTY(LetterSpacing, _letterSpacing, CGFloat)
ABI11_0_0RCT_TEXT_PROPERTY(LineHeight, _lineHeight, CGFloat)
ABI11_0_0RCT_TEXT_PROPERTY(NumberOfLines, _numberOfLines, NSUInteger)
ABI11_0_0RCT_TEXT_PROPERTY(EllipsizeMode, _ellipsizeMode, NSLineBreakMode)
ABI11_0_0RCT_TEXT_PROPERTY(TextAlign, _textAlign, NSTextAlignment)
ABI11_0_0RCT_TEXT_PROPERTY(TextDecorationColor, _textDecorationColor, UIColor *);
ABI11_0_0RCT_TEXT_PROPERTY(TextDecorationLine, _textDecorationLine, ABI11_0_0RCTTextDecorationLineType);
ABI11_0_0RCT_TEXT_PROPERTY(TextDecorationStyle, _textDecorationStyle, NSUnderlineStyle);
ABI11_0_0RCT_TEXT_PROPERTY(WritingDirection, _writingDirection, NSWritingDirection)
ABI11_0_0RCT_TEXT_PROPERTY(Opacity, _opacity, CGFloat)
ABI11_0_0RCT_TEXT_PROPERTY(TextShadowOffset, _textShadowOffset, CGSize);
ABI11_0_0RCT_TEXT_PROPERTY(TextShadowRadius, _textShadowRadius, CGFloat);
ABI11_0_0RCT_TEXT_PROPERTY(TextShadowColor, _textShadowColor, UIColor *);
...
- (void)setAllowFontScaling:(BOOL)allowFontScaling
{
  ...
  for (ABI11_0_0RCTShadowView *child in [self ReactABI11_0_0Subviews]) {
    if ([child isKindOfClass:[ABI11_0_0RCTShadowText class]]) {
      ((ABI11_0_0RCTShadowText *)child).allowFontScaling = allowFontScaling;
    }
  }
  [self dirtyText];
}
...
- (void)setFontSizeMultiplier:(CGFloat)fontSizeMultiplier
{
  ...
  if (_fontSizeMultiplier == 0) {
    ABI11_0_0RCTLogError(@"fontSizeMultiplier value must be > zero.");
    _fontSizeMultiplier = 1.0;
  }
  for (ABI11_0_0RCTShadowView *child in [self ReactABI11_0_0Subviews]) {
    if ([child isKindOfClass:[ABI11_0_0RCTShadowText class]]) {
      ((ABI11_0_0RCTShadowText *)child).fontSizeMultiplier = fontSizeMultiplier;
    }
  }
  [self dirtyText];
}
