...
#import "ABI11_0_0RCTTestRunner.h"

#import "ABI11_0_0FBSnapshotTestController.h"
#import "ABI11_0_0RCTAssert.h"
#import "ABI11_0_0RCTLog.h"
#import "ABI11_0_0RCTRootView.h"
#import "ABI11_0_0RCTTestModule.h"
...
@implementation ABI11_0_0RCTTestRunner
{
  FBSnapshotTestController *_testController;
  ABI11_0_0RCTBridgeModuleProviderBlock _moduleProvider;
}
...
{
  ABI11_0_0RCTAssertParam(app);
  ABI11_0_0RCTAssertParam(referenceDirectory);
  ...
  if ((self = [super init])) {
    ...
    if (getenv("CI_USE_PACKAGER")) {
      ...
      _scriptURL = [[NSBundle bundleForClass:[ABI11_0_0RCTBridge class]] URLForResource:@"main" withExtension:@"jsbundle"];
    }
    ABI11_0_0RCTAssert(_scriptURL != nil, @"No scriptURL set");
  }
  return self;
}
...
configurationBlock:(void(^)(ABI11_0_0RCTRootView *rootView))configurationBlock
{
...
expectErrorBlock:(BOOL(^)(NSString *error))expectErrorBlock
{
  ...
  @autoreleasepool {
    ...
    ABI11_0_0RCTSetLogFunction(^(ABI11_0_0RCTLogLevel level, ABI11_0_0RCTLogSource source, NSString *fileName, NSNumber *lineNumber, NSString *message) {
      if (level >= ABI11_0_0RCTLogLevelError) {
        error = message;
      }
    });

    ABI11_0_0RCTBridge *bridge = [[ABI11_0_0RCTBridge alloc] initWithBundleURL:_scriptURL
                                              moduleProvider:_moduleProvider
                                               launchOptions:nil];

    ABI11_0_0RCTRootView *rootView = [[ABI11_0_0RCTRootView alloc] initWithBridge:bridge moduleName:moduleName initialProperties:initialProps];
    rootView.frame = CGRectMake(0, 0, 320, 2000); // Constant size for testing on multiple devices

    ABI11_0_0RCTTestModule *testModule = [rootView.bridge moduleForClass:[ABI11_0_0RCTTestModule class]];
    ABI11_0_0RCTAssert(_testController != nil, @"_testController should not be nil");
    ...
    while (date.timeIntervalSinceNow > 0 && testModule.status == ABI11_0_0RCTTestStatusPending && error == nil) {
    ...
    if ([jsExecutor isKindOfClass:[ABI11_0_0RCTJSCExecutor class]]) {
      weakJSContext = [jsExecutor valueForKey:@"_context"];
    }
    [rootView removeFromSuperview];

    ABI11_0_0RCTSetLogFunction(ABI11_0_0RCTDefaultLogFunction);
    ...
    ABI11_0_0RCTAssert(nonLayoutSubviews.count == 0, @"There shouldn't be any other views: %@", nonLayoutSubviews);

    if (expectErrorBlock) {
      ABI11_0_0RCTAssert(expectErrorBlock(error), @"Expected an error but nothing matched.");
    } else {
      ABI11_0_0RCTAssert(error == nil, @"RedBox error: %@", error);
      ABI11_0_0RCTAssert(testModule.status != ABI11_0_0RCTTestStatusPending, @"Test didn't finish within %0.f seconds", kTestTimeoutSeconds);
      ABI11_0_0RCTAssert(testModule.status == ABI11_0_0RCTTestStatusPassed, @"Test failed");
    }
    [bridge invalidate];
  }
  ...
  ABI11_0_0RCTAssert(!weakJSContext, @"JS context was not deallocated after being invalidated");
}
