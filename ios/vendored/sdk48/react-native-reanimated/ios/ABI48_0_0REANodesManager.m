...
#import <ABI48_0_0RNReanimated/ABI48_0_0REABezierNode.h>
#import <ABI48_0_0RNReanimated/ABI48_0_0REABlockNode.h>
#import <ABI48_0_0RNReanimated/ABI48_0_0REACallFuncNode.h>
...
#import <ABI48_0_0RNReanimated/ABI48_0_0REAEventNode.h>
#import <ABI48_0_0RNReanimated/ABI48_0_0REAFunctionNode.h>
#import <ABI48_0_0RNReanimated/ABI48_0_0REAJSCallNode.h>
...
#import <ABI48_0_0RNReanimated/ABI48_0_0REAPropsNode.h>
...
// Interface below has been added in order to use private methods of ABI48_0_0RCTUIManager,
// ABI48_0_0RCTUIManager#UpdateView is a ABI48_0_0React Method which is exported to JS but in
// Objective-C it stays private
// ABI48_0_0RCTUIManager#setNeedsLayout is a method which updated layout only which
// in its turn will trigger relayout if no batch has been activated

@interface ABI48_0_0RCTUIManager ()

- (void)updateView:(nonnull NSNumber *)ABI48_0_0ReactTag viewName:(NSString *)viewName props:(NSDictionary *)props;

- (void)setNeedsLayout;

@end

@interface ABI48_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands;

- (void)runSyncUIUpdatesWithObserver:(id<ABI48_0_0RCTUIManagerObserver>)observer;

@end

@interface ABI48_0_0ComponentUpdate : NSObject
...
@implementation ABI48_0_0ComponentUpdate
@end

@implementation ABI48_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands
{
  // Accessing some private bits of ABI48_0_0RCTUIManager to provide missing functionality
...
- (void)runSyncUIUpdatesWithObserver:(id<ABI48_0_0RCTUIManagerObserver>)observer
{
  ...
  NSHashTable<id<ABI48_0_0RCTUIManagerObserver>> *soleObserver = [NSHashTable new];
...
@interface ABI48_0_0REANodesManager () <ABI48_0_0RCTUIManagerObserver>

@end

@implementation ABI48_0_0REANodesManager {
  NSMutableDictionary<ABI48_0_0REANodeID, ABI48_0_0REANode *> *_nodes;
  NSMapTable<NSString *, ABI48_0_0REANode *> *_eventMapping;
  NSMutableArray<id<ABI48_0_0RCTEvent>> *_eventQueue;
  CADisplayLink *_displayLink;
  ABI48_0_0REAUpdateContext *_updateContext;
  BOOL _wantRunUpdates;
  BOOL _processingDirectEvent;
  NSMutableArray<ABI48_0_0REAOnAnimationCallback> *_onAnimationCallbacks;
  NSMutableArray<ABI48_0_0REANativeAnimationOp> *_operationsInBatch;
  BOOL _tryRunBatchUpdatesSynchronously;
  ABI48_0_0REAEventHandler _eventHandler;
  volatile void (^_mounting)(void);
  NSMutableDictionary<NSNumber *, ABI48_0_0ComponentUpdate *> *_componentUpdateBuffer;
  volatile atomic_bool _shouldFlushUpdateBuffer;
  NSMutableDictionary<NSNumber *, UIView *> *_viewRegistry;
}

- (instancetype)initWithModule:(ABI48_0_0REAModule *)reanimatedModule uiManager:(ABI48_0_0RCTUIManager *)uiManager
{
  if ((self = [super init])) {
    ...
    _updateContext = [ABI48_0_0REAUpdateContext new];
    _wantRunUpdates = NO;
...
- (ABI48_0_0REANode *)findNodeByID:(ABI48_0_0REANodeID)nodeID
{
...
- (void)onAnimationFrame:(CADisplayLink *)displayLink
{
  ...
  for (NSUInteger i = 0; i < _eventQueue.count; i++) {
    id<ABI48_0_0RCTEvent> event = _eventQueue[i];
    [self processEvent:event];
  }
  [_eventQueue removeAllObjects];

  NSArray<ABI48_0_0REAOnAnimationCallback> *callbacks = _onAnimationCallbacks;
  _onAnimationCallbacks = [NSMutableArray new];
  ...
  // we iterate over it
  for (ABI48_0_0REAOnAnimationCallback block in callbacks) {
    block(displayLink);
  }
...
{
  ABI48_0_0RCTAssert(_mounting == nil, @"Mouting block is expected to not be set");
  _mounting = block;
  return YES;
}
...
{
  if (_wantRunUpdates) {
    [ABI48_0_0REANode runPropUpdates:_updateContext];
  }
  if (_operationsInBatch.count != 0) {
    NSMutableArray<ABI48_0_0REANativeAnimationOp> *copiedOperationsQueue = _operationsInBatch;
    _operationsInBatch = [NSMutableArray new];
                       ...
                       trySynchronously:(BOOL)trySync
{
  ...
  [_operationsInBatch addObject:^(ABI48_0_0RCTUIManager *uiManager) {
    [uiManager updateView:ABI48_0_0ReactTag viewName:viewName props:nativeProps];
  }];
}
...
- (void)createNode:(ABI48_0_0REANodeID)nodeID config:(NSDictionary<NSString *, id> *)config
{
  static NSDictionary *map;
  static dispatch_once_t mapToken;
  dispatch_once(&mapToken, ^{
    map = @{
      @"props" : [ABI48_0_0REAPropsNode class],
      @"style" : [ABI48_0_0REAStyleNode class],
      @"transform" : [ABI48_0_0REATransformNode class],
      @"value" : [ABI48_0_0REAValueNode class],
      @"block" : [ABI48_0_0REABlockNode class],
      @"cond" : [ABI48_0_0REACondNode class],
      @"op" : [ABI48_0_0REAOperatorNode class],
      @"set" : [ABI48_0_0REASetNode class],
      @"debug" : [ABI48_0_0READebugNode class],
      @"clock" : [ABI48_0_0REAClockNode class],
      @"clockStart" : [ABI48_0_0REAClockStartNode class],
      @"clockStop" : [ABI48_0_0REAClockStopNode class],
      @"clockTest" : [ABI48_0_0REAClockTestNode class],
      @"call" : [ABI48_0_0REAJSCallNode class],
      @"bezier" : [ABI48_0_0REABezierNode class],
      @"event" : [ABI48_0_0REAEventNode class],
      @"always" : [ABI48_0_0REAAlwaysNode class],
      @"concat" : [ABI48_0_0REAConcatNode class],
      @"param" : [ABI48_0_0REAParamNode class],
      @"func" : [ABI48_0_0REAFunctionNode class],
      @"callfunc" : [ABI48_0_0REACallFuncNode class]
      //            @"listener": nil,
    };
  });
  ...
  ABI48_0_0REANode *node = [[nodeClass alloc] initWithID:nodeID config:config];
  node.nodesManager = self;
  node.updateContext = _updateContext;
  _nodes[nodeID] = node;
}

- (void)dropNode:(ABI48_0_0REANodeID)nodeID
{
  ABI48_0_0REANode *node = _nodes[nodeID];
  if (node) {
    [node onDrop];
...
- (void)connectNodes:(nonnull NSNumber *)parentID childID:(nonnull ABI48_0_0REANodeID)childID
{
  ABI48_0_0RCTAssertParam(parentID);
  ABI48_0_0RCTAssertParam(childID);

  ABI48_0_0REANode *parentNode = _nodes[parentID];
  ABI48_0_0REANode *childNode = _nodes[childID];
...
- (void)disconnectNodes:(ABI48_0_0REANodeID)parentID childID:(ABI48_0_0REANodeID)childID
{
  ABI48_0_0RCTAssertParam(parentID);
  ABI48_0_0RCTAssertParam(childID);
...
- (void)connectNodeToView:(ABI48_0_0REANodeID)nodeID viewTag:(NSNumber *)viewTag viewName:(NSString *)viewName
{
  ABI48_0_0RCTAssertParam(nodeID);
  ABI48_0_0REANode *node = _nodes[nodeID];
  ABI48_0_0RCTAssertParam(node);

  if ([node isKindOfClass:[ABI48_0_0REAPropsNode class]]) {
    [(ABI48_0_0REAPropsNode *)node connectToView:viewTag viewName:viewName];
  }
}

- (void)disconnectNodeFromView:(ABI48_0_0REANodeID)nodeID viewTag:(NSNumber *)viewTag
{
  ABI48_0_0RCTAssertParam(nodeID);
  ABI48_0_0REANode *node = _nodes[nodeID];
  ABI48_0_0RCTAssertParam(node);
  ...
  if ([node isKindOfClass:[ABI48_0_0REAPropsNode class]]) {
    [(ABI48_0_0REAPropsNode *)node disconnectFromView:viewTag];
  }
}

- (void)attachEvent:(NSNumber *)viewTag eventName:(NSString *)eventName eventNodeID:(ABI48_0_0REANodeID)eventNodeID
{
  ABI48_0_0RCTAssertParam(eventNodeID);
  ABI48_0_0REANode *eventNode = _nodes[eventNodeID];
  ABI48_0_0RCTAssert([eventNode isKindOfClass:[ABI48_0_0REAEventNode class]], @"Event node is of an invalid type");

  NSString *key = [NSString stringWithFormat:@"%@%@", viewTag, ABI48_0_0RCTNormalizeInputEventName(eventName)];
  ABI48_0_0RCTAssert([_eventMapping objectForKey:key] == nil, @"Event handler already set for the given view and event type");
  [_eventMapping setObject:eventNode forKey:key];
}

- (void)detachEvent:(NSNumber *)viewTag eventName:(NSString *)eventName eventNodeID:(ABI48_0_0REANodeID)eventNodeID
{
  NSString *key = [NSString stringWithFormat:@"%@%@", viewTag, ABI48_0_0RCTNormalizeInputEventName(eventName)];
  [_eventMapping removeObjectForKey:key];
}

- (void)processEvent:(id<ABI48_0_0RCTEvent>)event
{
  NSString *key = [NSString stringWithFormat:@"%@%@", event.viewTag, ABI48_0_0RCTNormalizeInputEventName(event.eventName)];
  ABI48_0_0REAEventNode *eventNode = [_eventMapping objectForKey:key];
  [eventNode processEvent:event];
}

- (void)processDirectEvent:(id<ABI48_0_0RCTEvent>)event
{
...
- (BOOL)isDirectEvent:(id<ABI48_0_0RCTEvent>)event
{
  ...
  return [directEventNames containsObject:ABI48_0_0RCTNormalizeInputEventName(event.eventName)];
}
...
- (void)dispatchEvent:(id<ABI48_0_0RCTEvent>)event
{
  ...
  if (_eventHandler != nil) {
    __weak ABI48_0_0REAEventHandler eventHandler = _eventHandler;
    __weak typeof(self) weakSelf = self;
    ABI48_0_0RCTExecuteOnMainQueue(^void() {
      __typeof__(self) strongSelf = weakSelf;
      if (strongSelf == nil) {
        return;
      }
  ...
  ABI48_0_0REANode *eventNode = [_eventMapping objectForKey:key];
...
- (void)setValueForNodeID:(nonnull NSNumber *)nodeID value:(nonnull NSNumber *)newValue
{
  ...
  ABI48_0_0REAValueNode *valueNode = (ABI48_0_0REAValueNode *)node;
...
{
  ABI48_0_0ComponentUpdate *lastSnapshot = _componentUpdateBuffer[viewTag];
  ...
  if (!isNativeViewMounted) {
    if (lastSnapshot == nil) {
      ABI48_0_0ComponentUpdate *propsSnapshot = [ABI48_0_0ComponentUpdate new];
...
{
  UIView *view = [self.uiManager viewForABI48_0_0ReactTag:viewTag];
  ...
  if ([propName isEqualToString:@"opacity"]) {
    ...
    NSInteger zIndex = view.ABI48_0_0ReactZIndex;
...
- (void)maybeFlushUpdateBuffer
{
  ...
  __weak typeof(self) weakSelf = self;
  [_uiManager addUIBlock:^(__unused ABI48_0_0RCTUIManager *manager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    __typeof__(self) strongSelf = weakSelf;
    if (strongSelf == nil) {
      return;
    }
    ...
    for (NSNumber *tag in componentUpdateBuffer) {
      ABI48_0_0ComponentUpdate *componentUpdate = componentUpdateBuffer[tag];
