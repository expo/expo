...
@implementation ABI48_0_0RNSScreenWindowTraits
...
+ (void)assertViewControllerBasedStatusBarAppearenceSet
{
  ...
  if (!viewControllerBasedAppearence) {
    ABI48_0_0RCTLogError(@"If you want to change the appearance of status bar, you have to change \
...
+ (UIStatusBarStyle)statusBarStyleForRNSStatusBarStyle:(ABI48_0_0RNSStatusBarStyle)statusBarStyle
{
  ...
  if (@available(iOS 13.0, *)) {
    switch (statusBarStyle) {
      case ABI48_0_0RNSStatusBarStyleAuto:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleLightContent
            : UIStatusBarStyleDarkContent;
      case ABI48_0_0RNSStatusBarStyleInverted:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleDarkContent
            : UIStatusBarStyleLightContent;
      case ABI48_0_0RNSStatusBarStyleLight:
        return UIStatusBarStyleLightContent;
      case ABI48_0_0RNSStatusBarStyleDark:
  ...
  if (statusBarStyle == ABI48_0_0RNSStatusBarStyleLight) {
    return UIStatusBarStyleLightContent;
  }
...
+ (void)enforceDesiredDeviceOrientation
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIInterfaceOrientation currentDeviceOrientation =
        [ABI48_0_0RNSScreenWindowTraits interfaceOrientationFromDeviceOrientation:[[UIDevice currentDevice] orientation]];
    UIInterfaceOrientation currentInterfaceOrientation = [ABI48_0_0RNSScreenWindowTraits interfaceOrientation];
    UIInterfaceOrientation newOrientation = UIInterfaceOrientationUnknown;
    if ([ABI48_0_0RNSScreenWindowTraits maskFromOrientation:currentDeviceOrientation] & orientationMask) {
      if (!([ABI48_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
      ...
      if (!([ABI48_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
        ...
        // rotation from mask
        newOrientation = [ABI48_0_0RNSScreenWindowTraits defaultOrientationForOrientationMask:orientationMask];
      } else {
...
// method to be used in Expo for checking if ABI48_0_0RNScreens have trait set
                ...
                inViewController:(UIViewController *)vc
{
  ...
  if ([lastViewController conformsToProtocol:@protocol(ABI48_0_0RNScreensViewControllerDelegate)]) {
    ...
    if ([lastViewController isKindOfClass:[ABI48_0_0RNScreensViewController class]]) {
      vc = [(ABI48_0_0RNScreensViewController *)lastViewController findActiveChildVC];
    } else if ([lastViewController isKindOfClass:[ABI48_0_0RNScreensNavigationController class]]) {
      vc = [(ABI48_0_0RNScreensNavigationController *)lastViewController topViewController];
    }
    return [vc isKindOfClass:[ABI48_0_0RNSScreen class]] &&
        [(ABI48_0_0RNSScreen *)vc findChildVCForConfigAndTrait:trait includingModals:includingModals] != nil;
  }
  return NO;
}
...
{
  return [ABI48_0_0RNSScreenWindowTraits shouldAskScreensForTrait:ABI48_0_0RNSWindowTraitOrientation
                                         includingModals:YES
                                        inViewController:vc];
}
