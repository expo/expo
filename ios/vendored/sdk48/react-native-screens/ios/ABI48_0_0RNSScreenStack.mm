...
#import "ABI48_0_0RNSScreenStackAnimator.h"
...
#ifdef RN_FABRIC_ENABLED
    ,
    ABI48_0_0RCTMountingTransactionObserving
#endif
    >
...
@implementation ABI48_0_0RNScreensNavigationController
...
#if !TARGET_OS_TV
@interface ABI48_0_0RNSScreenEdgeGestureRecognizer : UIScreenEdgePanGestureRecognizer
@end

@implementation ABI48_0_0RNSScreenEdgeGestureRecognizer
@end

@interface ABI48_0_0RNSPanGestureRecognizer : UIPanGestureRecognizer
@end

@implementation ABI48_0_0RNSPanGestureRecognizer
@end
#endif

@implementation ABI48_0_0RNSScreenStackView {
  UINavigationController *_controller;
  NSMutableArray<ABI48_0_0RNSScreenView *> *_ABI48_0_0ReactSubviews;
  BOOL _invalidated;
  BOOL _isFullWidthSwiping;
  UIPercentDrivenInteractiveTransition *_interactionController;
  BOOL _hasLayout;
  __weak ABI48_0_0RNSScreenStackManager *_manager;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI48_0_0facebook::ABI48_0_0React::ABI48_0_0RNSScreenStackProps>();
    _props = defaultProps;
...
- (instancetype)initWithManager:(ABI48_0_0RNSScreenStackManager *)manager
{
  if (self = [super init]) {
    _hasLayout = NO;
    _invalidated = NO;
...
- (void)initCommonProps
{
  ...
  _controller = [ABI48_0_0RNScreensNavigationController new];
...
- (void)emitOnFinishTransitioningEvent
{
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const ABI48_0_0facebook::ABI48_0_0React::ABI48_0_0RNSScreenStackEventEmitter>(_eventEmitter)
        ->onFinishTransitioning(ABI48_0_0facebook::ABI48_0_0React::ABI48_0_0RNSScreenStackEventEmitter::OnFinishTransitioning{});
  }
#else
                    ...
                    animated:(BOOL)animated
{
  ...
  [ABI48_0_0RNSScreenStackHeaderConfig willShowViewController:viewController animated:animated withConfig:config];
}
...
- (void)presentationControllerDidDismiss:(UIPresentationController *)presentationController
{
  ...
  if ([presentationController.presentedViewController isKindOfClass:[ABI48_0_0RNSScreen class]]) {
    ...
    if (self.onFinishTransitioning) {
      ...
      // finish event to arrive later than onDismiss (see ABI48_0_0RNSScreen#notifyDismiss)
...
- (void)setModalViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  for (NSUInteger i = changeRootIndex; i < controllers.count; i++) {
    if ([_presentedModals containsObject:controllers[i]]) {
      ABI48_0_0RCTAssert(false, @"Modally presented controllers are being reshuffled, this is not allowed");
    }
  }

  __weak ABI48_0_0RNSScreenStackView *weakSelf = self;
  ...
  void (^finish)(void) = ^{
    ...
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
        ...
        BOOL shouldAnimate = lastModal && [next isKindOfClass:[ABI48_0_0RNSScreen class]] &&
            ((ABI48_0_0RNSScreen *)next).screenView.stackAnimation != ABI48_0_0RNSScreenStackAnimationNone;
  ...
  if (changeRootController.presentedViewController != nil &&
    ...
    BOOL shouldAnimate = changeRootIndex == controllers.count &&
        [changeRootController.presentedViewController isKindOfClass:[ABI48_0_0RNSScreen class]] &&
        ((ABI48_0_0RNSScreen *)changeRootController.presentedViewController).screenView.stackAnimation !=
...
- (void)setPushViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  BOOL firstTimePush = ![previousTop isKindOfClass:[ABI48_0_0RNSScreen class]];
  ...
  if (firstTimePush) {
    ...
    if (![controllers containsObject:previousTop]) {
      ...
      if (![_controller.viewControllers containsObject:top] &&
          ((ABI48_0_0RNSScreenView *)top.view).replaceAnimation == ABI48_0_0RNSScreenReplaceAnimationPush) {
        // setting new controllers with animation does `push` animation by default
#ifdef RN_FABRIC_ENABLED
        auto screenController = (ABI48_0_0RNSScreen *)top;
  ...
  for (ABI48_0_0RNSScreenView *screen in _ABI48_0_0ReactSubviews) {
    if (!screen.dismissed && screen.controller != nil) {
      if (pushControllers.count == 0) {
        ...
        if (screen.stackPresentation == ABI48_0_0RNSScreenStackPresentationPush) {
          [pushControllers addObject:screen.controller];
        } else {
...
{
#ifdef RN_FABRIC_ENABLED
  if ([_controller.visibleViewController isKindOfClass:[ABI48_0_0RNSScreen class]]) {
    [(ABI48_0_0RNSScreen *)_controller.visibleViewController resetViewToScreen];
  }
#else
  dispatch_async(dispatch_get_main_queue(), ^{
    [self invalidate];
...
{
  ABI48_0_0RNSScreenView *screen;
  if (operation == UINavigationControllerOperationPush) {
    screen = ((ABI48_0_0RNSScreen *)toVC).screenView;
  } else if (operation == UINavigationControllerOperationPop) {
    screen = ((ABI48_0_0RNSScreen *)fromVC).screenView;
  }
  if (screen != nil &&
      ...
      (_isFullWidthSwiping || [ABI48_0_0RNSScreenStackAnimator isCustomAnimation:screen.stackAnimation])) {
    return [[ABI48_0_0RNSScreenStackAnimator alloc] initWithOperation:operation];
  }
  return nil;
}
...
- (void)cancelTouchesInParent
{
  ...
  if (parent != nil) {
#ifdef RN_FABRIC_ENABLED
    ABI48_0_0RCTSurfaceTouchHandler *touchHandler = [parent performSelector:@selector(touchHandler)];
#else
    ABI48_0_0RCTTouchHandler *touchHandler = [parent performSelector:@selector(touchHandler)];
#endif
...
{
  ABI48_0_0RNSScreenView *topScreen = _ABI48_0_0ReactSubviews.lastObject;

#if TARGET_OS_TV
  [self cancelTouchesInParent];
  return YES;
#else
  // ABI48_0_0RNSPanGestureRecognizer will receive events iff topScreen.fullScreenSwipeEnabled == YES;
  // Events are filtered in gestureRecognizer:shouldReceivePressOrTouchEvent: method
  if ([gestureRecognizer isKindOfClass:[ABI48_0_0RNSPanGestureRecognizer class]]) {
  ...
  // Now we're dealing with ABI48_0_0RNSScreenEdgeGestureRecognizer (or _UIParallaxTransitionPanGestureRecognizer)
  if (topScreen.customAnimationOnSwipe && [ABI48_0_0RNSScreenStackAnimator isCustomAnimation:topScreen.stackAnimation]) {
    if ([gestureRecognizer isKindOfClass:[ABI48_0_0RNSScreenEdgeGestureRecognizer class]]) {
      ...
      BOOL isCorrectEdge = (_controller.view.semanticContentAttribute == UISemanticContentAttributeForceRightToLeft &&
                            ((ABI48_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeRight) ||
          (_controller.view.semanticContentAttribute != UISemanticContentAttributeForceRightToLeft &&
           ((ABI48_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeLeft);
      if (isCorrectEdge) {
        [self cancelTouchesInParent];
        return YES;
      }
...
- (void)setupGestureHandlers
{
  ...
  ABI48_0_0RNSScreenEdgeGestureRecognizer *leftEdgeSwipeGestureRecognizer =
      [[ABI48_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI48_0_0RNSScreenEdgeGestureRecognizer *rightEdgeSwipeGestureRecognizer =
      [[ABI48_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI48_0_0RNSPanGestureRecognizer *panRecognizer = [[ABI48_0_0RNSPanGestureRecognizer alloc] initWithTarget:self
...
- (void)handleSwipe:(UIPanGestureRecognizer *)gestureRecognizer
{
  ...
  float velocity;
  float distance;
  if (topScreen.swipeDirection == ABI48_0_0RNSScreenSwipeDirectionVertical) {
...
- (BOOL)isInGestureResponseDistance:(UIGestureRecognizer *)gestureRecognizer topScreen:(ABI48_0_0RNSScreenView *)topScreen
{
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  if (CGRectContainsPoint(_controller.navigationBar.frame, point)) {
    ...
    UIView *headerConfig = [[_ABI48_0_0ReactSubviews.lastObject ABI48_0_0ReactSubviews] firstObject];
    if ([headerConfig isKindOfClass:[ABI48_0_0RNSScreenStackHeaderConfig class]]) {
...
// ABI48_0_0RNSScreenStackView is a UIGestureRecognizerDelegate for three types of gesture recognizers:
// ABI48_0_0RNSPanGestureRecognizer, ABI48_0_0RNSScreenEdgeGestureRecognizer, _UIParallaxTransitionPanGestureRecognizer
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceivePressOrTouchEvent:(NSObject *)event
{
  ...
  if (![topScreen isKindOfClass:[ABI48_0_0RNSScreenView class]] || !topScreen.gestureEnabled ||
      _controller.viewControllers.count < 2 || [topScreen isModal]) {
    return NO;
  }

  // We want to pass events to ABI48_0_0RNSPanGestureRecognizer iff full screen swipe is enabled.
  if ([gestureRecognizer isKindOfClass:[ABI48_0_0RNSPanGestureRecognizer class]]) {
    return topScreen.fullScreenSwipeEnabled;
  }

  // ABI48_0_0RNSScreenEdgeGestureRecognizer || _UIParallaxTransitionPanGestureRecognizer
  return YES;
}
    ...
    shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
  if ([gestureRecognizer isKindOfClass:[ABI48_0_0RNSPanGestureRecognizer class]] &&
    ...
    ABI48_0_0RNSPanGestureRecognizer *panGestureRecognizer = (ABI48_0_0RNSPanGestureRecognizer *)gestureRecognizer;
...
{
  if (![subview isKindOfClass:[ABI48_0_0RNSScreenView class]]) {
    ABI48_0_0RCTLogError(@"ScreenStack only accepts children of type Screen");
    return;
  }
...
- (void)mountChildComponentView:(UIView<ABI48_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  ...
  [_ABI48_0_0ReactSubviews insertObject:(ABI48_0_0RNSScreenView *)childComponentView atIndex:index];
  ((ABI48_0_0RNSScreenView *)childComponentView).ABI48_0_0ReactSuperview = self;
  dispatch_async(dispatch_get_main_queue(), ^{
    [self maybeAddToParentAndUpdateContainer];
  });
}
...
{
  ABI48_0_0RNSScreenView *screenChildComponent = (ABI48_0_0RNSScreenView *)childComponentView;
  ...
  ABI48_0_0RCTAssert(
      screenChildComponent.ABI48_0_0ReactSuperview == self,
  ...
  ABI48_0_0RCTAssert(
      ...
      screenChildComponent,
      @(index),
      @([_ABI48_0_0ReactSubviews indexOfObject:screenChildComponent]),
      @([[_ABI48_0_0ReactSubviews objectAtIndex:index] tag]));
  screenChildComponent.ABI48_0_0ReactSuperview = nil;
  [_ABI48_0_0ReactSubviews removeObject:screenChildComponent];
...
- (void)mountingTransactionWillMount:(ABI48_0_0facebook::ABI48_0_0React::MountingTransaction const &)transaction
                withSurfaceTelemetry:(ABI48_0_0facebook::ABI48_0_0React::SurfaceTelemetry const &)surfaceTelemetry
{
  for (auto &mutation : transaction.getMutations()) {
    if (mutation.type == ABI48_0_0facebook::ABI48_0_0React::ShadowViewMutation::Type::Remove &&
        mutation.parentShadowView.componentName != nil &&
        strcmp(mutation.parentShadowView.componentName, "ABI48_0_0RNSScreenStack") == 0) {
      [self takeSnapshot];
      return;
    }
  }
...
{
  return ABI48_0_0facebook::ABI48_0_0React::concreteComponentDescriptorProvider<ABI48_0_0facebook::ABI48_0_0React::ABI48_0_0RNSScreenStackComponentDescriptor>();
}
#else
...
#ifdef RN_FABRIC_ENABLED
Class<ABI48_0_0RCTComponentViewProtocol> ABI48_0_0RNSScreenStackCls(void)
{
  return ABI48_0_0RNSScreenStackView.class;
}
#endif

@implementation ABI48_0_0RNSScreenStackManager {
  NSPointerArray *_stacks;
}

ABI48_0_0RCT_EXPORT_MODULE()

ABI48_0_0RCT_EXPORT_VIEW_PROPERTY(onFinishTransitioning, ABI48_0_0RCTDirectEventBlock);

#ifdef RN_FABRIC_ENABLED
#else
- (UIView *)view
{
  ABI48_0_0RNSScreenStackView *view = [[ABI48_0_0RNSScreenStackView alloc] initWithManager:self];
  if (!_stacks) {
...
- (void)invalidate
{
  for (ABI48_0_0RNSScreenStackView *stack in _stacks) {
