#import "ABI48_0_0RNSVGContextBrush.h"
...
#import "ABI48_0_0RNSVGSolidColorBrush.h"
...
template <typename T>
ABI48_0_0RNSVGBrush *brushFromColorStruct(T fillObject)
{
  ...
  switch (type) {
    ...
    case 0: // solid color
    {
      ...
      ABI48_0_0RNSVGColor *color = ABI48_0_0RCTUIColorFromSharedColor(fillObject.payload) ?: [ABI48_0_0RNSVGColor clearColor];
      return [[ABI48_0_0RNSVGSolidColorBrush alloc] initWithColor:color];
    }
    case 1: // brush
    {
      NSArray *arr = @[ @(type), ABI48_0_0RCTNSStringFromString(fillObject.brushRef) ];
      return [[ABI48_0_0RNSVGPainterBrush alloc] initWithArray:arr];
    }
    case 2: // currentColor
      return [[ABI48_0_0RNSVGBrush alloc] initWithArray:nil];
    case 3: // context-fill
      return [[ABI48_0_0RNSVGContextBrush alloc] initFill];
    case 4: // context-stroke
      return [[ABI48_0_0RNSVGContextBrush alloc] initStroke];
    default:
      ABI48_0_0RCTLogError(@"Unknown brush type: %d", type);
      return nil;
  }
}

template <typename T>
void setCommonNodeProps(T nodeProps, ABI48_0_0RNSVGNode *node)
{
  node.name = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.name);
  ...
  CATransform3D transform3d = ABI48_0_0RCTCATransform3DFromTransformMatrix(nodeProps.transform);
  ...
  node.mask = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.mask);
  node.markerStart = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.markerStart);
  node.markerMid = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.markerMid);
  node.markerEnd = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.markerEnd);
  node.clipPath = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.clipPath);
  node.clipRule = nodeProps.clipRule == 0 ? kRNSVGCGFCRuleEvenodd : kRNSVGCGFCRuleNonzero;
  node.responsible = nodeProps.responsible;
  // onLayout
  node.display = ABI48_0_0RCTNSStringFromStringNilIfEmpty(nodeProps.display);
  std::string pointerEvents = nodeProps.pointerEvents;
  NSString *pointerEventsString = ABI48_0_0RCTNSStringFromStringNilIfEmpty(pointerEvents);
  if ([pointerEventsString isEqualToString:@"auto"]) {
    node.pointerEvents = ABI48_0_0RCTPointerEventsUnspecified;
  } else if ([pointerEventsString isEqualToString:@"none"]) {
    node.pointerEvents = ABI48_0_0RCTPointerEventsNone;
...
static NSMutableArray<ABI48_0_0RNSVGLength *> *createLengthArrayFromStrings(std::vector<std::string> stringArray)
{
  if (stringArray.empty()) {
    return nil;
  }
  NSMutableArray<ABI48_0_0RNSVGLength *> *lengthArray = [NSMutableArray new];
  for (auto str : stringArray) {
    ABI48_0_0RNSVGLength *lengthFromString = [ABI48_0_0RNSVGLength lengthWithString:ABI48_0_0RCTNSStringFromString(str)];
...
template <typename T>
void setCommonRenderableProps(T renderableProps, ABI48_0_0RNSVGRenderable *renderableNode)
{
  ...
  renderableNode.strokeWidth = [ABI48_0_0RNSVGLength lengthWithString:ABI48_0_0RCTNSStringFromString(renderableProps.strokeWidth)];
  ...
  if (renderableProps.propList.size() > 0) {
    ...
    for (auto str : renderableProps.propList) {
      [propArray addObject:ABI48_0_0RCTNSStringFromString(str)];
    }
...
{
  NSString *valueOrNil = ABI48_0_0RCTNSStringFromStringNilIfEmpty(value);
  if (valueOrNil) {
    dict[key] = valueOrNil;
  }
}
...
template <typename T>
void setCommonGroupProps(T groupProps, ABI48_0_0RNSVGGroup *groupNode)
{
  setCommonRenderableProps(groupProps, groupNode);

  if (ABI48_0_0RCTNSStringFromStringNilIfEmpty(groupProps.fontSize)) {
    groupNode.font = @{@"fontSize" : ABI48_0_0RCTNSStringFromString(groupProps.fontSize)};
  }
  if (ABI48_0_0RCTNSStringFromStringNilIfEmpty(groupProps.fontWeight)) {
    groupNode.font = @{@"fontWeight" : ABI48_0_0RCTNSStringFromString(groupProps.fontWeight)};
  }
...
template <typename T>
void setCommonTextProps(T textProps, ABI48_0_0RNSVGText *textNode)
{
  ...
  textNode.inlineSize = [ABI48_0_0RNSVGLength lengthWithString:ABI48_0_0RCTNSStringFromString(textProps.inlineSize)];
  textNode.textLength = [ABI48_0_0RNSVGLength lengthWithString:ABI48_0_0RCTNSStringFromString(textProps.textLength)];
  textNode.baselineShift = ABI48_0_0RCTNSStringFromStringNilIfEmpty(textProps.baselineShift);
  textNode.lengthAdjust = ABI48_0_0RCTNSStringFromStringNilIfEmpty(textProps.lengthAdjust);
  textNode.alignmentBaseline = ABI48_0_0RCTNSStringFromStringNilIfEmpty(textProps.alignmentBaseline);
}

static ABI48_0_0RNSVGVBMOS intToRNSVGVBMOS(int value)
{
  switch (value) {
    case 0:
