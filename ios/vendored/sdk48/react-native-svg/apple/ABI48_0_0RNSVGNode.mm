...
@interface ABI48_0_0RNSVGNode ()
@property (nonatomic, readwrite, weak) ABI48_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI48_0_0RNSVGGroup *textRoot;
@end

@implementation ABI48_0_0RNSVGNode {
  ABI48_0_0RNSVGGlyphContext *glyphContext;
  BOOL _transparent;
  ABI48_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI48_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI48_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
- (void)insertABI48_0_0ReactSubview:(ABI48_0_0RNSVGView *)subview atIndex:(NSInteger)atIndex
{
...
- (void)removeABI48_0_0ReactSubview:(ABI48_0_0RNSVGView *)subview
{
...
{
  // Do nothing, as subviews are inserted by insertABI48_0_0ReactSubview:
}
...
{
  [self clearPath];
  for (__kindof ABI48_0_0RNSVGNode *node in self.subviews) {
...
{
  ABI48_0_0RNSVGNode *node = self;
  while (node != nil) {
    ABI48_0_0RNSVGPlatformView *parent = [node superview];

    if (![parent isKindOfClass:[ABI48_0_0RNSVGNode class]]) {
      return;
    }
    node = (ABI48_0_0RNSVGNode *)parent;
    if (!node.path) {
      return;
    }
    [node clearPath];
...
- (ABI48_0_0RNSVGGroup *)textRoot
{
  ...
  ABI48_0_0RNSVGNode *node = self;
  while (node != nil) {
    if ([node isKindOfClass:[ABI48_0_0RNSVGGroup class]] && [((ABI48_0_0RNSVGGroup *)node) getGlyphContext] != nil) {
      _textRoot = (ABI48_0_0RNSVGGroup *)node;
      break;
    }
...
- (ABI48_0_0RNSVGGroup *)getParentTextRoot
{
  ABI48_0_0RNSVGNode *parent = (ABI48_0_0RNSVGGroup *)[self superview];
  if (![parent isKindOfClass:[ABI48_0_0RNSVGGroup class]]) {
    return nil;
  } else {
...
{
  ABI48_0_0RNSVGGroup *root = self.textRoot;
  if (root == nil) {
    return ABI48_0_0RNSVG_DEFAULT_FONT_SIZE;
  }
...
- (void)reactSetInheritedBackgroundColor:(ABI48_0_0RNSVGColor *)inheritedBackgroundColor
{
...
- (void)setMatrix:(CGAffineTransform)matrix
{
  ...
  if ([container conformsToProtocol:@protocol(ABI48_0_0RNSVGContainer)]) {
    [(id<ABI48_0_0RNSVGContainer>)container invalidate];
  }
}
...
- (void)setClipRule:(ABI48_0_0RNSVGCGFCRule)clipRule
{
...
{
  if (self.clipPath) {
    _clipNode = (ABI48_0_0RNSVGClipPath *)[self.svgView getDefinedClipPath:self.clipPath];
    if (_cachedClipPath) {
      CGPathRelease(_cachedClipPath);
    }
...
- (ABI48_0_0RNSVGSvgView *)svgView
{
  if (_svgView) {
    return _svgView;
  }

  __kindof ABI48_0_0RNSVGPlatformView *parent = self.superview;

  if ([parent class] == [ABI48_0_0RNSVGSvgView class]) {
    _svgView = parent;
  } else if ([parent isKindOfClass:[ABI48_0_0RNSVGNode class]]) {
    _svgView = ((ABI48_0_0RNSVGNode *)parent).svgView;
  } else {
    ABI48_0_0RCTLogError(@"ABI48_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
  }
...
{
  return [ABI48_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
- (CGFloat)relativeOn:(ABI48_0_0RNSVGLength *)length relative:(CGFloat)relative
{
  ABI48_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)relativeOnWidth:(ABI48_0_0RNSVGLength *)length
{
  ABI48_0_0RNSVGLengthUnitType unit = length.unit;
  if (unit == SVG_LENGTHTYPE_NUMBER) {
    return length.value;
...
- (CGFloat)relativeOnHeight:(ABI48_0_0RNSVGLength *)length
{
  ABI48_0_0RNSVGLengthUnitType unit = length.unit;
  if (unit == SVG_LENGTHTYPE_NUMBER) {
    return length.value;
...
- (CGFloat)relativeOnOther:(ABI48_0_0RNSVGLength *)length
{
  ABI48_0_0RNSVGLengthUnitType unit = length.unit;
  if (unit == SVG_LENGTHTYPE_NUMBER) {
    return length.value;
...
- (CGFloat)fromRelative:(ABI48_0_0RNSVGLength *)length
{
  CGFloat unit;
  switch (length.unit) {
...
- (CGFloat)getContextDiagonal
{
  ...
  CGFloat r = sqrt(powX + powY) * ABI48_0_0RNSVG_M_SQRT1_2l;
  return r;
}
...
- (CGFloat)getCanvasDiagonal
{
  ...
  CGFloat powX = width * width;
  CGFloat powY = height * height;
  canvasDiagonal = sqrt(powX + powY) * ABI48_0_0RNSVG_M_SQRT1_2l;
  return canvasDiagonal;
}
...
- (void)traverseSubviews:(BOOL (^)(__kindof ABI48_0_0RNSVGView *node))block
{
  for (ABI48_0_0RNSVGView *node in self.subviews) {
    if (!block(node)) {
      break;
    }
  }
...
- (void)prepareForRecycle
{
  ...
  _pointerEvents = ABI48_0_0RCTPointerEventsUnspecified;
  _responsible = NO;
