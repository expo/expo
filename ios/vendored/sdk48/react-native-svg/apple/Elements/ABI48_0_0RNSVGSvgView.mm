...
@implementation ABI48_0_0RNSVGSvgView {
  NSMutableDictionary<NSString *, ABI48_0_0RNSVGNode *> *_clipPaths;
  NSMutableDictionary<NSString *, ABI48_0_0RNSVGNode *> *_templates;
  NSMutableDictionary<NSString *, ABI48_0_0RNSVGPainter *> *_painters;
  NSMutableDictionary<NSString *, ABI48_0_0RNSVGNode *> *_markers;
  NSMutableDictionary<NSString *, ABI48_0_0RNSVGNode *> *_masks;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    rendered = false;
#ifdef RN_FABRIC_ENABLED
    static const auto defaultProps = std::make_shared<const ABI48_0_0RNSVGSvgViewProps>();
    _props = defaultProps;
...
{
  return concreteComponentDescriptorProvider<ABI48_0_0RNSVGSvgViewComponentDescriptor>();
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  const auto &newProps = *std::static_pointer_cast<const ABI48_0_0RNSVGSvgViewProps>(props);

  self.minX = newProps.minX;
  self.minY = newProps.minY;
  self.vbWidth = newProps.vbWidth;
  self.vbHeight = newProps.vbHeight;
  self.bbWidth = [ABI48_0_0RNSVGLength lengthWithString:ABI48_0_0RCTNSStringFromString(newProps.bbWidth)];
  self.bbHeight = [ABI48_0_0RNSVGLength lengthWithString:ABI48_0_0RCTNSStringFromString(newProps.bbHeight)];
  self.align = ABI48_0_0RCTNSStringFromStringNilIfEmpty(newProps.align);
  self.meetOrSlice = intToRNSVGVBMOS(newProps.meetOrSlice);
  if (ABI48_0_0RCTUIColorFromSharedColor(newProps.tintColor)) {
    self.tintColor = ABI48_0_0RCTUIColorFromSharedColor(newProps.tintColor);
  }
  if (ABI48_0_0RCTUIColorFromSharedColor(newProps.color)) {
    self.tintColor = ABI48_0_0RCTUIColorFromSharedColor(newProps.color);
  }
  _props = std::static_pointer_cast<ABI48_0_0RNSVGSvgViewProps const>(props);
}
...
- (void)invalidate
{
  ...
  if ([parent isKindOfClass:[ABI48_0_0RNSVGNode class]]) {
    ...
    ABI48_0_0RNSVGNode *svgNode = (ABI48_0_0RNSVGNode *)parent;
    [svgNode invalidate];
    rendered = false;
    return;
  }
...
- (void)setBbWidth:(ABI48_0_0RNSVGLength *)bbWidth
{
...
- (void)setBbHeight:(ABI48_0_0RNSVGLength *)bbHeight
{
...
- (void)drawToContext:(CGContextRef)context withRect:(CGRect)rect
{
  ...
  if (self.align) {
    ...
    _viewBoxTransform = [ABI48_0_0RNSVGViewBox getTransform:tRect eRect:rect align:self.align meetOrSlice:self.meetOrSlice];
  ...
  for (ABI48_0_0RNSVGView *node in self.subviews) {
    if ([node isKindOfClass:[ABI48_0_0RNSVGNode class]]) {
      ABI48_0_0RNSVGNode *svg = (ABI48_0_0RNSVGNode *)node;
...
- (void)drawRect:(CGRect)rect
{
  ...
  for (ABI48_0_0RNSVGPlatformView *node in self.subviews) {
    if ([node isKindOfClass:[ABI48_0_0RNSVGNode class]]) {
      ABI48_0_0RNSVGNode *svg = (ABI48_0_0RNSVGNode *)node;
...
- (ABI48_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  for (ABI48_0_0RNSVGNode *node in [self.subviews reverseObjectEnumerator]) {
    if (![node isKindOfClass:[ABI48_0_0RNSVGNode class]]) {
      continue;
    }
    ...
    ABI48_0_0RNSVGPlatformView *hitChild = [node hitTest:transformed withEvent:event];
...
- (void)defineTemplate:(ABI48_0_0RNSVGNode *)definedTemplate templateName:(NSString *)templateName
{
  if (!_templates) {
...
#ifdef RN_FABRIC_ENABLED
Class<ABI48_0_0RCTComponentViewProtocol> ABI48_0_0RNSVGSvgViewCls(void)
{
  return ABI48_0_0RNSVGSvgView.class;
}
#endif // RN_FABRIC_ENABLED
