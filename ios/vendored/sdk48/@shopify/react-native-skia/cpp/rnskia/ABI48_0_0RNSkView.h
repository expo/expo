...
class ABI48_0_0RNSkCanvasProvider {
public:
  explicit ABI48_0_0RNSkCanvasProvider(std::function<void()> requestRedraw)
...
class ABI48_0_0RNSkRenderer {
public:
  explicit ABI48_0_0RNSkRenderer(std::function<void()> requestRedraw)
      : _requestRedraw(requestRedraw) {}

  /**
   Tries to render the current set of drawing operations. If we're busy we'll
   return false so that the calling ABI48_0_0RNSkBaseDrawView can request a new render
   ...
   */
  virtual bool
  tryRender(std::shared_ptr<ABI48_0_0RNSkCanvasProvider> canvasProvider) = 0;
   ...
   */
  virtual void
  renderImmediate(std::shared_ptr<ABI48_0_0RNSkCanvasProvider> canvasProvider) = 0;
...
class ABI48_0_0RNSkImageCanvasProvider : public ABI48_0_0RNSkCanvasProvider {
public:
  ABI48_0_0RNSkImageCanvasProvider(std::function<void()> requestRedraw, float width,
                          float height)
      : ABI48_0_0RNSkCanvasProvider(requestRedraw), _width(width), _height(height) {
...
enum ABI48_0_0RNSkDrawingMode { Default, Continuous };

using ABI48_0_0RNSkTouchInfo = struct {
...
class ABI48_0_0RNSkView : public std::enable_shared_from_this<ABI48_0_0RNSkView> {
public:
  /**
   * Constructor
   */
  ABI48_0_0RNSkView(std::shared_ptr<ABI48_0_0RNSkPlatformContext> context,
           std::shared_ptr<ABI48_0_0RNSkCanvasProvider> canvasProvider,
           std::shared_ptr<ABI48_0_0RNSkRenderer> renderer)
  ...
  virtual void setJsiProperties(
      std::unordered_map<std::string, ABI48_0_0RNJsi::JsiValueWrapper> &props) {
    ...
    for (auto &prop : props) {
      if (prop.first == "onSize") {
        ...
        // Save onSize
        _onSize =
            std::dynamic_pointer_cast<ABI48_0_0RNSkValue>(prop.second.getAsHostObject());
   ...
   */
  void setDrawingMode(ABI48_0_0RNSkDrawingMode mode) {
    _drawingMode = mode;
    requestRedraw();
  }
   ...
   */
  virtual void updateTouchState(std::vector<ABI48_0_0RNSkTouchInfo> &) {
    requestRedraw();
  }
  ...
  sk_sp<SkImage> makeImageSnapshot(std::shared_ptr<SkRect> bounds) {
    auto provider = std::make_shared<ABI48_0_0RNSkImageCanvasProvider>(
        std::bind(&ABI48_0_0RNSkView::requestRedraw, this),
...
protected:
  ...
  std::shared_ptr<ABI48_0_0RNSkCanvasProvider> getCanvasProvider() {
    return _canvasProvider;
  }
  std::shared_ptr<ABI48_0_0RNSkRenderer> getRenderer() { return _renderer; }
...
private:
  ...
  void drawLoopCallback(bool invalidated) {
    if (_redrawRequestCounter > 0 ||
        _drawingMode == ABI48_0_0RNSkDrawingMode::Continuous) {
  ...
  std::shared_ptr<ABI48_0_0RNSkCanvasProvider> _canvasProvider;
  std::shared_ptr<ABI48_0_0RNSkRenderer> _renderer;

  std::shared_ptr<ABI48_0_0RNSkValue> _onSize;
  std::function<void()> _onSizeUnsubscribe;
  ABI48_0_0RNSkDrawingMode _drawingMode;
  size_t _nativeId;
