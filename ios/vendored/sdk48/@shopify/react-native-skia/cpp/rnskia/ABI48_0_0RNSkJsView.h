...
class ABI48_0_0RNSkJsRenderer : public ABI48_0_0RNSkRenderer,
                       public std::enable_shared_from_this<ABI48_0_0RNSkJsRenderer> {
public:
  ABI48_0_0RNSkJsRenderer(std::function<void()> requestRedraw,
  ...
  std::shared_ptr<ABI48_0_0RNSkInfoObject> getInfoObject();
...
private:
  ...
  std::shared_ptr<ABI48_0_0RNSkInfoObject> _infoObject;
  ABI48_0_0RNSkTimingInfo _jsTimingInfo;
  ABI48_0_0RNSkTimingInfo _gpuTimingInfo;
};

class ABI48_0_0RNSkJsView : public ABI48_0_0RNSkView {
public:
  /**
   * Constructor
   */
  ABI48_0_0RNSkJsView(std::shared_ptr<ABI48_0_0RNSkPlatformContext> context,
             std::shared_ptr<ABI48_0_0RNSkCanvasProvider> canvasProvider)
      : ABI48_0_0RNSkView(context, canvasProvider,
                 std::make_shared<ABI48_0_0RNSkJsRenderer>(
                     std::bind(&ABI48_0_0RNSkJsView::requestRedraw, this), context)) {}

  void updateTouchState(std::vector<ABI48_0_0RNSkTouchInfo> &touches) override {
    std::static_pointer_cast<ABI48_0_0RNSkJsRenderer>(getRenderer())
        ->getInfoObject()
      ...
      if (prop.first == "drawCallback") {
        if (prop.second.isUndefinedOrNull()) {
          ...
          continue;
                   ...
                   ABI48_0_0RNJsi::JsiWrapperValueType::Function) {
