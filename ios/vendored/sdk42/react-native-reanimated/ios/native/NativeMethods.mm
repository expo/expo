...
std::vector<std::pair<std::string,double>> measure(int viewTag, ABI42_0_0RCTUIManager *uiManager) {
  UIView *view = [uiManager viewForABI42_0_0ReactTag:@(viewTag)];
  ...
  if (rootView == nil || (![rootView isABI42_0_0ReactRootView])) {
    return std::vector<std::pair<std::string, double>>(1, std::make_pair("x", -1234567.0));
  }
...
void scrollTo(int scrollViewTag, ABI42_0_0RCTUIManager *uiManager, double x, double y, bool animated) {
  UIView *view = [uiManager viewForABI42_0_0ReactTag:@(scrollViewTag)];
  ABI42_0_0RCTScrollView *scrollView = (ABI42_0_0RCTScrollView *) view;
