...
#import "Nodes/ABI42_0_0REANode.h"
#import "Nodes/ABI42_0_0REAPropsNode.h"
#import "Nodes/ABI42_0_0REAStyleNode.h"
#import "Nodes/ABI42_0_0REATransformNode.h"
#import "Nodes/ABI42_0_0REAValueNode.h"
#import "Nodes/ABI42_0_0REABlockNode.h"
#import "Nodes/ABI42_0_0REACondNode.h"
#import "Nodes/ABI42_0_0REAOperatorNode.h"
#import "Nodes/ABI42_0_0REASetNode.h"
#import "Nodes/ABI42_0_0READebugNode.h"
#import "Nodes/ABI42_0_0REAClockNodes.h"
#import "Nodes/ABI42_0_0REAJSCallNode.h"
#import "Nodes/ABI42_0_0REABezierNode.h"
#import "Nodes/ABI42_0_0REAEventNode.h"
#import "ABI42_0_0REAModule.h"
#import "Nodes/ABI42_0_0REAAlwaysNode.h"
#import "Nodes/ABI42_0_0REAConcatNode.h"
#import "Nodes/ABI42_0_0REAParamNode.h"
#import "Nodes/ABI42_0_0REAFunctionNode.h"
#import "Nodes/ABI42_0_0REACallFuncNode.h"
#import <ABI42_0_0React/ABI42_0_0RCTShadowView.h>

// Interface below has been added in order to use private methods of ABI42_0_0RCTUIManager,
// ABI42_0_0RCTUIManager#UpdateView is a ABI42_0_0React Method which is exported to JS but in
// Objective-C it stays private
// ABI42_0_0RCTUIManager#setNeedsLayout is a method which updated layout only which
// in its turn will trigger relayout if no batch has been activated

@interface ABI42_0_0RCTUIManager ()

- (void)updateView:(nonnull NSNumber *)ABI42_0_0ReactTag
...
@interface ABI42_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands;

- (void)runSyncUIUpdatesWithObserver:(id<ABI42_0_0RCTUIManagerObserver>)observer;

@end

@implementation ABI42_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands
{
  // Accessing some private bits of ABI42_0_0RCTUIManager to provide missing functionality
...
- (void)runSyncUIUpdatesWithObserver:(id<ABI42_0_0RCTUIManagerObserver>)observer
{
  ...
  NSHashTable<id<ABI42_0_0RCTUIManagerObserver>> *soleObserver = [NSHashTable new];
...
@interface ABI42_0_0REANodesManager() <ABI42_0_0RCTUIManagerObserver>

@end


@implementation ABI42_0_0REANodesManager
{
  NSMutableDictionary<ABI42_0_0REANodeID, ABI42_0_0REANode *> *_nodes;
  NSMapTable<NSString *, ABI42_0_0REANode *> *_eventMapping;
  NSMutableArray<id<ABI42_0_0RCTEvent>> *_eventQueue;
  CADisplayLink *_displayLink;
  ABI42_0_0REAUpdateContext *_updateContext;
  BOOL _wantRunUpdates;
  BOOL _processingDirectEvent;
  NSMutableArray<ABI42_0_0REAOnAnimationCallback> *_onAnimationCallbacks;
  NSMutableArray<ABI42_0_0REANativeAnimationOp> *_operationsInBatch;
  BOOL _tryRunBatchUpdatesSynchronously;
  ABI42_0_0REAEventHandler _eventHandler;
  volatile void (^_mounting)(void);
}

- (instancetype)initWithModule:(ABI42_0_0REAModule *)reanimatedModule
                     uiManager:(ABI42_0_0RCTUIManager *)uiManager
{
  if ((self = [super init])) {
    ...
    _updateContext = [ABI42_0_0REAUpdateContext new];
    _wantRunUpdates = NO;
...
- (ABI42_0_0REANode *)findNodeByID:(ABI42_0_0REANodeID)nodeID
{
...
- (void)onAnimationFrame:(CADisplayLink *)displayLink
{
  ...
  for (NSUInteger i = 0; i < _eventQueue.count; i++) {
    id<ABI42_0_0RCTEvent> event = _eventQueue[i];
    [self processEvent:event];
  }
  [_eventQueue removeAllObjects];

  NSArray<ABI42_0_0REAOnAnimationCallback> *callbacks = _onAnimationCallbacks;
  _onAnimationCallbacks = [NSMutableArray new];
  ...
  // we iterate over it
  for (ABI42_0_0REAOnAnimationCallback block in callbacks) {
    block(displayLink);
  }
...
- (BOOL)uiManager:(ABI42_0_0RCTUIManager *)manager performMountingWithBlock:(ABI42_0_0RCTUIManagerMountingBlock)block {
  ABI42_0_0RCTAssert(_mounting == nil, @"Mouting block is expected to not be set");
  _mounting = block;
  return YES;
}
...
{
  if (_wantRunUpdates) {
    [ABI42_0_0REANode runPropUpdates:_updateContext];
  }
  if (_operationsInBatch.count != 0) {
    NSMutableArray<ABI42_0_0REANativeAnimationOp> *copiedOperationsQueue = _operationsInBatch;
    _operationsInBatch = [NSMutableArray new];
...
- (void)enqueueUpdateViewOnNativeThread:(nonnull NSNumber *)ABI42_0_0ReactTag
  ...
  [_operationsInBatch addObject:^(ABI42_0_0RCTUIManager *uiManager) {
    [uiManager updateView:ABI42_0_0ReactTag viewName:viewName props:nativeProps];
  }];
}
...
- (void)createNode:(ABI42_0_0REANodeID)nodeID
            config:(NSDictionary<NSString *, id> *)config
{
  static NSDictionary *map;
  static dispatch_once_t mapToken;
  dispatch_once(&mapToken, ^{
    map = @{@"props": [ABI42_0_0REAPropsNode class],
            @"style": [ABI42_0_0REAStyleNode class],
            @"transform": [ABI42_0_0REATransformNode class],
            @"value": [ABI42_0_0REAValueNode class],
            @"block": [ABI42_0_0REABlockNode class],
            @"cond": [ABI42_0_0REACondNode class],
            @"op": [ABI42_0_0REAOperatorNode class],
            @"set": [ABI42_0_0REASetNode class],
            @"debug": [ABI42_0_0READebugNode class],
            @"clock": [ABI42_0_0REAClockNode class],
            @"clockStart": [ABI42_0_0REAClockStartNode class],
            @"clockStop": [ABI42_0_0REAClockStopNode class],
            @"clockTest": [ABI42_0_0REAClockTestNode class],
            @"call": [ABI42_0_0REAJSCallNode class],
            @"bezier": [ABI42_0_0REABezierNode class],
            @"event": [ABI42_0_0REAEventNode class],
            @"always": [ABI42_0_0REAAlwaysNode class],
            @"concat": [ABI42_0_0REAConcatNode class],
            @"param": [ABI42_0_0REAParamNode class],
            @"func": [ABI42_0_0REAFunctionNode class],
            @"callfunc": [ABI42_0_0REACallFuncNode class]
//            @"listener": nil,
            };
  });

  NSString *nodeType = [ABI42_0_0RCTConvert NSString:config[@"type"]];

  Class nodeClass = map[nodeType];
  if (!nodeClass) {
    ABI42_0_0RCTLogError(@"Animated node type %@ not supported natively", nodeType);
    return;
  }

  ABI42_0_0REANode *node = [[nodeClass alloc] initWithID:nodeID config:config];
  node.nodesManager = self;
  node.updateContext = _updateContext;
  _nodes[nodeID] = node;
}

- (void)dropNode:(ABI42_0_0REANodeID)nodeID
{
  ABI42_0_0REANode *node = _nodes[nodeID];
  if (node) {
    [node onDrop];
...
- (void)connectNodes:(nonnull NSNumber *)parentID childID:(nonnull ABI42_0_0REANodeID)childID
{
  ABI42_0_0RCTAssertParam(parentID);
  ABI42_0_0RCTAssertParam(childID);

  ABI42_0_0REANode *parentNode = _nodes[parentID];
  ABI42_0_0REANode *childNode = _nodes[childID];

  ABI42_0_0RCTAssertParam(childNode);

  [parentNode addChild:childNode];
}

- (void)disconnectNodes:(ABI42_0_0REANodeID)parentID childID:(ABI42_0_0REANodeID)childID
{
  ABI42_0_0RCTAssertParam(parentID);
  ABI42_0_0RCTAssertParam(childID);
...
- (void)connectNodeToView:(ABI42_0_0REANodeID)nodeID
                  viewTag:(NSNumber *)viewTag
                 viewName:(NSString *)viewName
{
  ABI42_0_0RCTAssertParam(nodeID);
  ABI42_0_0REANode *node = _nodes[nodeID];
  ABI42_0_0RCTAssertParam(node);

  if ([node isKindOfClass:[ABI42_0_0REAPropsNode class]]) {
    [(ABI42_0_0REAPropsNode *)node connectToView:viewTag viewName:viewName];
  }
}

- (void)disconnectNodeFromView:(ABI42_0_0REANodeID)nodeID
                       viewTag:(NSNumber *)viewTag
{
  ABI42_0_0RCTAssertParam(nodeID);
  ABI42_0_0REANode *node = _nodes[nodeID];
  ...
  if ([node isKindOfClass:[ABI42_0_0REAPropsNode class]]) {
    [(ABI42_0_0REAPropsNode *)node disconnectFromView:viewTag];
  }
}

- (void)attachEvent:(NSNumber *)viewTag
          eventName:(NSString *)eventName
        eventNodeID:(ABI42_0_0REANodeID)eventNodeID
{
  ABI42_0_0RCTAssertParam(eventNodeID);
  ABI42_0_0REANode *eventNode = _nodes[eventNodeID];
  ABI42_0_0RCTAssert([eventNode isKindOfClass:[ABI42_0_0REAEventNode class]], @"Event node is of an invalid type");

  NSString *key = [NSString stringWithFormat:@"%@%@",
                   viewTag,
                   ABI42_0_0RCTNormalizeInputEventName(eventName)];
  ABI42_0_0RCTAssert([_eventMapping objectForKey:key] == nil, @"Event handler already set for the given view and event type");
...
- (void)processEvent:(id<ABI42_0_0RCTEvent>)event
{
  NSString *key = [NSString stringWithFormat:@"%@%@",
                   event.viewTag,
                   ABI42_0_0RCTNormalizeInputEventName(event.eventName)];
  ABI42_0_0REAEventNode *eventNode = [_eventMapping objectForKey:key];
  [eventNode processEvent:event];
}

- (void)processDirectEvent:(id<ABI42_0_0RCTEvent>)event
{
...
- (BOOL)isDirectEvent:(id<ABI42_0_0RCTEvent>)event
{
  ...
  return [directEventNames containsObject:ABI42_0_0RCTNormalizeInputEventName(event.eventName)];
}
...
- (void)dispatchEvent:(id<ABI42_0_0RCTEvent>)event
{
  ...
  if (_eventHandler != nil) {
    __weak ABI42_0_0REAEventHandler eventHandler = _eventHandler;
    __weak typeof(self) weakSelf = self;
    ABI42_0_0RCTExecuteOnMainQueue(^void(){
      __typeof__(self) strongSelf = weakSelf;
      if (strongSelf == nil) {
        return;
      }
  ...
  ABI42_0_0REANode *eventNode = [_eventMapping objectForKey:key];
...
- (void)setValueForNodeID:(nonnull NSNumber *)nodeID value:(nonnull NSNumber *)newValue
{
  ...
  ABI42_0_0REAValueNode *valueNode = (ABI42_0_0REAValueNode *)node;
...
{
    UIView* view = [self.uiManager viewForABI42_0_0ReactTag:viewTag];
    ...
    if ([propName isEqualToString:@"opacity"]) {
        ...
        NSInteger zIndex = view.ABI42_0_0ReactZIndex;
