...
@implementation ABI44_0_0REAUpdateContext
...
- (void)addChild:(ABI44_0_0REANode *)child
{
  if (!_childNodes) {
...
- (void)removeChild:(ABI44_0_0REANode *)child
{
  if (child) {
...
+ (NSMutableArray<ABI44_0_0REANode *> *)updatedNodes
{
  static NSMutableArray<ABI44_0_0REANode *> *updatedNodes;
...
+ (void)findAndUpdateNodes:(nonnull ABI44_0_0REANode *)node
            withVisitedSet:(NSMutableSet<ABI44_0_0REANode *> *)visitedNodes
            withFinalNodes:(NSMutableArray<id<ABI44_0_0REAFinalNode>> *)finalNodes
{
  ...
  for (ABI44_0_0REANode *child in node.childNodes) {
    [self findAndUpdateNodes:child withVisitedSet:visitedNodes withFinalNodes:finalNodes];
  }
  if ([node conformsToProtocol:@protocol(ABI44_0_0REAFinalNode)]) {
    [finalNodes addObject:(id<ABI44_0_0REAFinalNode>)node];
  }
}

+ (void)runPropUpdates:(ABI44_0_0REAUpdateContext *)context
{
