...
@implementation ABI44_0_0RCTConvert (Lottie)
...
+ (NSArray<NSDictionary *> *)LRNColorFilters:(id)json
{
  ...
  [rawFilters enumerateObjectsUsingBlock:^(NSDictionary *rawFilter, NSUInteger idx, BOOL *stop) {
    ...
    #if TARGET_OS_OSX
      NSColor *color = [ABI44_0_0RCTConvert NSColor:rawFilter[@"color"]];
    #else
      UIColor *color = [ABI44_0_0RCTConvert UIColor:rawFilter[@"color"]];
    #endif
      [filters addObject:@{
        @"color": color,
        @"keypath": keypath,
