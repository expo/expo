...
#import "ABI44_0_0RNCNetInfo.h"
#import "ABI44_0_0RNCConnectionStateWatcher.h"
...
@interface ABI44_0_0RNCNetInfo () <ABI44_0_0RNCConnectionStateWatcherDelegate>

@property (nonatomic, strong) ABI44_0_0RNCConnectionStateWatcher *connectionStateWatcher;
@property (nonatomic) BOOL isObserving;

@end

@implementation ABI44_0_0RNCNetInfo

#pragma mark - Module setup

ABI44_0_0RCT_EXPORT_MODULE()

// We need ABI44_0_0RNCReachabilityCallback's and module methods to be called on the same thread so that we can have
...
- (instancetype)init
{
  self = [super init];
  if (self) {
    _connectionStateWatcher = [[ABI44_0_0RNCConnectionStateWatcher alloc] initWithDelegate:self];
  }
  return self;
}
...
#pragma mark - ABI44_0_0RNCConnectionStateWatcherDelegate
...
ABI44_0_0RCT_EXPORT_METHOD(getCurrentState:(nullable NSString *)requestedInterface resolve:(ABI44_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI44_0_0RCTPromiseRejectBlock)reject)
{
  ABI44_0_0RNCConnectionState *state = [self.connectionStateWatcher currentState];
...
- (NSDictionary *)currentDictionaryFromUpdateState:(ABI44_0_0RNCConnectionState *)state withInterface:(nullable NSString *)requestedInterface
{
...
- (NSMutableDictionary *)detailsFromInterface:(nonnull NSString *)requestedInterface withState:(ABI44_0_0RNCConnectionState *)state
{
  NSMutableDictionary *details = [NSMutableDictionary new];
  if ([requestedInterface isEqualToString: ABI44_0_0RNCConnectionTypeCellular]) {
    details[@"cellularGeneration"] = state.cellularGeneration ?: NSNull.null;
    details[@"carrier"] = [self carrier] ?: NSNull.null;
  } else if ([requestedInterface isEqualToString: ABI44_0_0RNCConnectionTypeWifi] || [requestedInterface isEqualToString: ABI44_0_0RNCConnectionTypeEthernet]) {
