...
@implementation ABI44_0_0RNSScreenWindowTraits
...
+ (void)assertViewControllerBasedStatusBarAppearenceSet
{
  ...
  if (!viewControllerBasedAppearence) {
    ABI44_0_0RCTLogError(@"If you want to change the appearance of status bar, you have to change \
...
#if !TARGET_OS_TV
+ (UIStatusBarStyle)statusBarStyleForRNSStatusBarStyle:(ABI44_0_0RNSStatusBarStyle)statusBarStyle
{
#if defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && defined(__IPHONE_13_0) && \
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
  if (@available(iOS 13.0, *)) {
    switch (statusBarStyle) {
      case ABI44_0_0RNSStatusBarStyleAuto:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleLightContent
            : UIStatusBarStyleDarkContent;
      case ABI44_0_0RNSStatusBarStyleInverted:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleDarkContent
            : UIStatusBarStyleLightContent;
      case ABI44_0_0RNSStatusBarStyleLight:
        return UIStatusBarStyleLightContent;
      case ABI44_0_0RNSStatusBarStyleDark:
  ...
  if (statusBarStyle == ABI44_0_0RNSStatusBarStyleLight) {
    return UIStatusBarStyleLightContent;
  }
...
+ (void)enforceDesiredDeviceOrientation
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIInterfaceOrientation currentDeviceOrientation =
        [ABI44_0_0RNSScreenWindowTraits interfaceOrientationFromDeviceOrientation:[[UIDevice currentDevice] orientation]];
    UIInterfaceOrientation currentInterfaceOrientation = [ABI44_0_0RNSScreenWindowTraits interfaceOrientation];
    UIInterfaceOrientation newOrientation = UIInterfaceOrientationUnknown;
    if ([ABI44_0_0RNSScreenWindowTraits maskFromOrientation:currentDeviceOrientation] & orientationMask) {
      if (!([ABI44_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
      ...
      if (!([ABI44_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
        ...
        // rotation from mask
        newOrientation = [ABI44_0_0RNSScreenWindowTraits defaultOrientationForOrientationMask:orientationMask];
      } else {
...
// method to be used in Expo for checking if ABI44_0_0RNScreens have trait set
+ (BOOL)shouldAskScreensForTrait:(ABI44_0_0RNSWindowTrait)trait
                ...
                inViewController:(UIViewController *)vc
{
  ...
  if ([lastViewController conformsToProtocol:@protocol(ABI44_0_0RNScreensViewControllerDelegate)]) {
    ...
    if ([lastViewController isKindOfClass:[ABI44_0_0RNScreensViewController class]]) {
      vc = [(ABI44_0_0RNScreensViewController *)lastViewController findActiveChildVC];
    } else if ([lastViewController isKindOfClass:[ABI44_0_0RNScreensNavigationController class]]) {
      vc = [(ABI44_0_0RNScreensNavigationController *)lastViewController topViewController];
    }
    return [vc isKindOfClass:[ABI44_0_0RNSScreen class]] &&
        [(ABI44_0_0RNSScreen *)vc findChildVCForConfigAndTrait:trait includingModals:includingModals] != nil;
  }
  return NO;
}
...
{
  return [ABI44_0_0RNSScreenWindowTraits shouldAskScreensForTrait:ABI44_0_0RNSWindowTraitOrientation
                                         includingModals:YES
                                        inViewController:vc];
}
