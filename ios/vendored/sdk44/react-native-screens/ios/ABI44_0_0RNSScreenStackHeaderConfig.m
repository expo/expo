...
// of a given ABI44_0_0RCTImageView. See more comments in the code section processing SubviewTypeBackButton
@interface ABI44_0_0RCTImageView (Private)
- (UIImage *)image;
@end

@interface ABI44_0_0RCTImageLoader (Private)
- (id<ABI44_0_0RCTImageCache>)imageCache;
@end

@interface ABI44_0_0RNSScreenStackHeaderSubview : UIView

@property (nonatomic, weak) ABI44_0_0RCTBridge *bridge;
@property (nonatomic, weak) UIView *ABI44_0_0ReactSuperview;
...
@implementation ABI44_0_0RNSScreenStackHeaderSubview
...
{
  // Block any attempt to set coordinates on ABI44_0_0RNSScreenStackHeaderSubview. This
  // makes UINavigationBar the only one to control the position of header content.
  [super ABI44_0_0ReactSetFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
}

@end

@interface ABI44_0_0RNSUIBarButtonItem : UIBarButtonItem

@property (nonatomic) BOOL menuHidden;

@end

@implementation ABI44_0_0RNSUIBarButtonItem
...
@implementation ABI44_0_0RNSScreenStackHeaderConfig {
  NSMutableArray<ABI44_0_0RNSScreenStackHeaderSubview *> *_ABI44_0_0ReactSubviews;
}
...
- (instancetype)init
{
  if (self = [super init]) {
    self.hidden = YES;
    _translucent = YES;
    _ABI44_0_0ReactSubviews = [NSMutableArray new];
  }
  return self;
}

- (void)insertABI44_0_0ReactSubview:(ABI44_0_0RNSScreenStackHeaderSubview *)subview atIndex:(NSInteger)atIndex
{
  [_ABI44_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  subview.ABI44_0_0ReactSuperview = self;
}

- (void)removeABI44_0_0ReactSubview:(ABI44_0_0RNSScreenStackHeaderSubview *)subview
{
  [_ABI44_0_0ReactSubviews removeObject:subview];
}

- (NSArray<UIView *> *)ABI44_0_0ReactSubviews
{
...
- (void)updateViewControllerIfNeeded
{
  ...
  BOOL isInFullScreenModal = nav == nil && _screenView.stackPresentation == ABI44_0_0RNSScreenStackPresentationFullScreenModal;
  ...
  if (vc != nil && (nextVC == vc || isInFullScreenModal)) {
    [ABI44_0_0RNSScreenStackHeaderConfig updateViewController:self.screenView.controller withConfig:self animated:YES];
  }
}
...
+ (void)setAnimatedConfig:(UIViewController *)vc withConfig:(ABI44_0_0RNSScreenStackHeaderConfig *)config
{
...
#endif
  {
    ...
    if (config.titleFontFamily || config.titleFontSize || config.titleFontWeight || config.titleColor) {
      ...
      if (family || weight) {
        attrs[NSFontAttributeName] = [ABI44_0_0RCTFont updateFont:nil
                                              withFamily:family
                                                    size:size
                                                  weight:weight
                                                   style:nil
    ...
    if (@available(iOS 11.0, *)) {
      if (config.largeTitle &&
        ...
        if (largeFamily || largeWeight) {
          largeAttrs[NSFontAttributeName] = [ABI44_0_0RCTFont updateFont:nil
...
+ (UIImage *)loadBackButtonImageInViewController:(UIViewController *)vc withConfig:(ABI44_0_0RNSScreenStackHeaderConfig *)config
{
  BOOL hasBackButtonImage = NO;
  for (ABI44_0_0RNSScreenStackHeaderSubview *subview in config.ABI44_0_0ReactSubviews) {
    if (subview.type == ABI44_0_0RNSScreenStackHeaderSubviewTypeBackButton && subview.subviews.count > 0) {
      hasBackButtonImage = YES;
      ABI44_0_0RCTImageView *imageView = subview.subviews[0];
      if (imageView.image == nil) {
        ...
        ABI44_0_0RCTImageSource *source = imageView.imageSources[0];
        [imageView ABI44_0_0ReactSetFrame:CGRectMake(
      ...
      if (image == nil) {
        ...
        ABI44_0_0RCTImageSource *source = imageView.imageSources[0];
        ABI44_0_0RCTImageLoader *imageloader = [subview.bridge moduleForClass:[ABI44_0_0RCTImageLoader class]];
...
+ (UINavigationBarAppearance *)buildAppearance:(UIViewController *)vc
                                    withConfig:(ABI44_0_0RNSScreenStackHeaderConfig *)config API_AVAILABLE(ios(13.0))
{
                    ...
                    animated:(BOOL)animated
{
  ...
  if (config.backTitle != nil || config.backTitleFontFamily || config.backTitleFontSize ||
    ...
    ABI44_0_0RNSUIBarButtonItem *backBarButtonItem = [[ABI44_0_0RNSUIBarButtonItem alloc] initWithTitle:config.backTitle ?: prevItem.title
  ...
  for (ABI44_0_0RNSScreenStackHeaderSubview *subview in config.ABI44_0_0ReactSubviews) {
    switch (subview.type) {
      case ABI44_0_0RNSScreenStackHeaderSubviewTypeLeft: {
#if !TARGET_OS_TV
      ...
      case ABI44_0_0RNSScreenStackHeaderSubviewTypeRight: {
        UIBarButtonItem *buttonItem = [[UIBarButtonItem alloc] initWithCustomView:subview];
        navitem.rightBarButtonItem = buttonItem;
        break;
      }
      case ABI44_0_0RNSScreenStackHeaderSubviewTypeCenter:
      case ABI44_0_0RNSScreenStackHeaderSubviewTypeTitle: {
...
#if !TARGET_OS_TV
          if (@available(iOS 11.0, *)) {
            ABI44_0_0RNSSearchBar *searchBar = subview.subviews[0];
      ...
      case ABI44_0_0RNSScreenStackHeaderSubviewTypeBackButton: {
        break;
      }
    }
  }
    ...
    [vc.transitionCoordinator
        ...
        completion:^(id<UIViewControllerTransitionCoordinatorContext> _Nonnull context) {
          if ([context isCancelled]) {
            ...
            ABI44_0_0RNSScreenStackHeaderConfig *config = nil;
            for (UIView *subview in fromVC.view.ABI44_0_0ReactSubviews) {
              if ([subview isKindOfClass:[ABI44_0_0RNSScreenStackHeaderConfig class]]) {
                config = (ABI44_0_0RNSScreenStackHeaderConfig *)subview;
                break;
              }
            }
...
@implementation ABI44_0_0RNSScreenStackHeaderConfigManager

ABI44_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  return [ABI44_0_0RNSScreenStackHeaderConfig new];
}

ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(title, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontFamily, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontSize, NSNumber)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontWeight, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(titleColor, UIColor)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(backTitle, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontFamily, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontSize, NSNumber)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(backgroundColor, UIColor)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(blurEffect, UIBlurEffectStyle)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(color, UIColor)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(direction, UISemanticContentAttribute)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitle, BOOL)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontFamily, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontSize, NSNumber)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontWeight, NSString)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleColor, UIColor)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleBackgroundColor, UIColor)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleHideShadow, BOOL)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(hideBackButton, BOOL)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(hideShadow, BOOL)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(backButtonInCustomView, BOOL)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(disableBackButtonMenu, BOOL)
// `hidden` is an UIView property, we need to use different name internally
ABI44_0_0RCT_REMAP_VIEW_PROPERTY(hidden, hide, BOOL)
ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(translucent, BOOL)

@end

@implementation ABI44_0_0RCTConvert (ABI44_0_0RNSScreenStackHeader)
...
ABI44_0_0RCT_ENUM_CONVERTER(
    ABI44_0_0RNSScreenStackHeaderSubviewType,
    (@{
      @"back" : @(ABI44_0_0RNSScreenStackHeaderSubviewTypeBackButton),
      @"left" : @(ABI44_0_0RNSScreenStackHeaderSubviewTypeLeft),
      @"right" : @(ABI44_0_0RNSScreenStackHeaderSubviewTypeRight),
      @"title" : @(ABI44_0_0RNSScreenStackHeaderSubviewTypeTitle),
      @"center" : @(ABI44_0_0RNSScreenStackHeaderSubviewTypeCenter),
      @"searchBar" : @(ABI44_0_0RNSScreenStackHeaderSubviewTypeSearchBar),
    }),
...
ABI44_0_0RCT_ENUM_CONVERTER(UIBlurEffectStyle, ([self blurEffectsForIOSVersion]), UIBlurEffectStyleExtraLight, integerValue)

@end

@implementation ABI44_0_0RNSScreenStackHeaderSubviewManager

ABI44_0_0RCT_EXPORT_MODULE()

ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(type, ABI44_0_0RNSScreenStackHeaderSubviewType)

- (UIView *)view
{
  return [[ABI44_0_0RNSScreenStackHeaderSubview alloc] initWithBridge:self.bridge];
}
