...
#import "ABI44_0_0RNSScreenStackAnimator.h"
...
@interface ABI44_0_0RNSScreenStackView () <
...
@implementation ABI44_0_0RNScreensNavigationController
...
#if !TARGET_OS_TV
@interface ABI44_0_0RNSScreenEdgeGestureRecognizer : UIScreenEdgePanGestureRecognizer
@end

@implementation ABI44_0_0RNSScreenEdgeGestureRecognizer
@end

@interface ABI44_0_0RNSPanGestureRecognizer : UIPanGestureRecognizer
@end

@implementation ABI44_0_0RNSPanGestureRecognizer
@end
#endif

@implementation ABI44_0_0RNSScreenStackView {
  UINavigationController *_controller;
  NSMutableArray<ABI44_0_0RNSScreenView *> *_ABI44_0_0ReactSubviews;
  __weak ABI44_0_0RNSScreenStackManager *_manager;
  BOOL _hasLayout;
  BOOL _invalidated;
...
- (instancetype)initWithManager:(ABI44_0_0RNSScreenStackManager *)manager
{
  if (self = [super init]) {
    _hasLayout = NO;
    _invalidated = NO;
    ...
    _controller = [[ABI44_0_0RNScreensNavigationController alloc] init];
                    ...
                    animated:(BOOL)animated
{
  ...
  [ABI44_0_0RNSScreenStackHeaderConfig willShowViewController:viewController animated:animated withConfig:config];
}
...
- (void)presentationControllerDidDismiss:(UIPresentationController *)presentationController
{
  ...
  if ([screenView isKindOfClass:[ABI44_0_0RNSScreenView class]]) {
    ...
    if (self.onFinishTransitioning) {
      ...
      // finish event to arrive later than onDismiss (see ABI44_0_0RNSScreen#notifyDismiss)
...
{
  if (![subview isKindOfClass:[ABI44_0_0RNSScreenView class]]) {
    ABI44_0_0RCTLogError(@"ScreenStack only accepts children of type Screen");
    return;
  }
...
- (void)ABI44_0_0ReactAddControllerToClosestParent:(UIViewController *)controller
{
  if (!controller.parentViewController) {
    UIView *parentView = (UIView *)self.ABI44_0_0ReactSuperview;
    while (parentView) {
      if (parentView.ABI44_0_0ReactViewController) {
        [parentView.ABI44_0_0ReactViewController addChildViewController:controller];
        [self addSubview:controller.view];
#if !TARGET_OS_TV
        _controller.interactivePopGestureRecognizer.delegate = self;
#endif
        [controller didMoveToParentViewController:parentView.ABI44_0_0ReactViewController];
      ...
      parentView = (UIView *)parentView.ABI44_0_0ReactSuperview;
    }
    return;
  }
}
...
- (void)setModalViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  for (NSUInteger i = changeRootIndex; i < controllers.count; i++) {
    if ([_presentedModals containsObject:controllers[i]]) {
      ABI44_0_0RCTAssert(false, @"Modally presented controllers are being reshuffled, this is not allowed");
    }
  }

  __weak ABI44_0_0RNSScreenStackView *weakSelf = self;
  ...
  void (^finish)(void) = ^{
    ...
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
        ...
        BOOL shouldAnimate = lastModal && [next isKindOfClass:[ABI44_0_0RNSScreen class]] &&
            ((ABI44_0_0RNSScreenView *)next.view).stackAnimation != ABI44_0_0RNSScreenStackAnimationNone;
  ...
  if (changeRootController.presentedViewController != nil &&
    ...
    BOOL shouldAnimate = changeRootIndex == controllers.count &&
        [changeRootController.presentedViewController isKindOfClass:[ABI44_0_0RNSScreen class]] &&
        ((ABI44_0_0RNSScreenView *)changeRootController.presentedViewController.view).stackAnimation !=
  ...
  if (firstTimePush) {
    ...
    if (![controllers containsObject:lastTop]) {
      ...
      if (![_controller.viewControllers containsObject:top] &&
          ((ABI44_0_0RNSScreenView *)top.view).replaceAnimation == ABI44_0_0RNSScreenReplaceAnimationPush) {
  ...
  for (ABI44_0_0RNSScreenView *screen in _ABI44_0_0ReactSubviews) {
    if (!screen.dismissed && screen.controller != nil) {
      if (pushControllers.count == 0) {
        ...
        if (screen.stackPresentation == ABI44_0_0RNSScreenStackPresentationPush) {
          [pushControllers addObject:screen.controller];
        } else {
...
{
  ABI44_0_0RNSScreenView *screen;
  if (operation == UINavigationControllerOperationPush) {
    screen = (ABI44_0_0RNSScreenView *)toVC.view;
  } else if (operation == UINavigationControllerOperationPop) {
    screen = (ABI44_0_0RNSScreenView *)fromVC.view;
  }
  if (screen != nil &&
      ...
      (_isFullWidthSwiping || [ABI44_0_0RNSScreenStackAnimator isCustomAnimation:screen.stackAnimation])) {
    return [[ABI44_0_0RNSScreenStackAnimator alloc] initWithOperation:operation];
  }
  return nil;
}
...
- (void)cancelTouchesInParent
{
  ...
  if (parent != nil) {
    ABI44_0_0RCTTouchHandler *touchHandler = [parent performSelector:@selector(touchHandler)];
    [touchHandler cancel];
    [touchHandler reset];
  }
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
  ...
  if (topScreen.fullScreenSwipeEnabled) {
    // we want only `ABI44_0_0RNSPanGestureRecognizer` to be able to recognize when
    // `fullScreenSwipeEnabled` is set
    if ([gestureRecognizer isKindOfClass:[ABI44_0_0RNSPanGestureRecognizer class]]) {
  ...
  if (topScreen.customAnimationOnSwipe && [ABI44_0_0RNSScreenStackAnimator isCustomAnimation:topScreen.stackAnimation]) {
    if ([gestureRecognizer isKindOfClass:[ABI44_0_0RNSScreenEdgeGestureRecognizer class]]) {
      ...
      BOOL isCorrectEdge = (_controller.view.semanticContentAttribute == UISemanticContentAttributeForceRightToLeft &&
                            ((ABI44_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeRight) ||
          (_controller.view.semanticContentAttribute != UISemanticContentAttributeForceRightToLeft &&
           ((ABI44_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeLeft);
      if (isCorrectEdge) {
        [self cancelTouchesInParent];
        return YES;
      }
    ...
    if ([gestureRecognizer isKindOfClass:[ABI44_0_0RNSScreenEdgeGestureRecognizer class]]) {
      ...
      return NO;
    } else if ([gestureRecognizer isKindOfClass:[ABI44_0_0RNSPanGestureRecognizer class]]) {
...
- (void)setupGestureHandlers
{
  ...
  ABI44_0_0RNSScreenEdgeGestureRecognizer *leftEdgeSwipeGestureRecognizer =
      [[ABI44_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI44_0_0RNSScreenEdgeGestureRecognizer *rightEdgeSwipeGestureRecognizer =
      [[ABI44_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI44_0_0RNSPanGestureRecognizer *panRecognizer = [[ABI44_0_0RNSPanGestureRecognizer alloc] initWithTarget:self
...
@implementation ABI44_0_0RNSScreenStackManager {
  NSPointerArray *_stacks;
}

ABI44_0_0RCT_EXPORT_MODULE()

ABI44_0_0RCT_EXPORT_VIEW_PROPERTY(onFinishTransitioning, ABI44_0_0RCTDirectEventBlock);

- (UIView *)view
{
  ABI44_0_0RNSScreenStackView *view = [[ABI44_0_0RNSScreenStackView alloc] initWithManager:self];
  if (!_stacks) {
...
- (void)invalidate
{
  for (ABI44_0_0RNSScreenStackView *stack in _stacks) {
