#include "ABI49_0_0RNSkManager.h"
...
#include <JsiSkApi.h>
#include <ABI49_0_0RNSkJsiViewApi.h>
#include <ABI49_0_0RNSkValueApi.h>
...
ABI49_0_0RNSkManager::ABI49_0_0RNSkManager(
    ...
    : _jsRuntime(jsRuntime), _jsCallInvoker(jsCallInvoker),
      ...
      _viewApi(std::make_shared<ABI49_0_0RNSkJsiViewApi>(platformContext)) {
...
void ABI49_0_0RNSkManager::invalidate() {
  if (_isInvalidated) {
    return;
  }
  _isInvalidated = true;
...
void ABI49_0_0RNSkManager::registerSkiaView(size_t nativeId,
                                   std::shared_ptr<ABI49_0_0RNSkView> view) {
  if (!_isInvalidated && _viewApi != nullptr)
    _viewApi->registerSkiaView(nativeId, view);
}

void ABI49_0_0RNSkManager::unregisterSkiaView(size_t nativeId) {
  if (!_isInvalidated && _viewApi != nullptr)
    _viewApi->unregisterSkiaView(nativeId);
}

void ABI49_0_0RNSkManager::setSkiaView(size_t nativeId, std::shared_ptr<ABI49_0_0RNSkView> view) {
  if (!_isInvalidated && _viewApi != nullptr)
    _viewApi->setSkiaView(nativeId, view);
}

void ABI49_0_0RNSkManager::installBindings() {
  ...
  auto skiaValueApi = std::make_shared<ABI49_0_0RNSkValueApi>(_platformContext);
  _jsRuntime->global().setProperty(
