#import "ABI45_0_0RNSScreenWindowTraits.h"
#import "ABI45_0_0RNSScreen.h"
#import "ABI45_0_0RNSScreenContainer.h"
#import "ABI45_0_0RNSScreenStack.h"

@implementation ABI45_0_0RNSScreenWindowTraits
...
+ (void)assertViewControllerBasedStatusBarAppearenceSet
{
  ...
  if (!viewControllerBasedAppearence) {
    ABI45_0_0RCTLogError(@"If you want to change the appearance of status bar, you have to change \
...
#if !TARGET_OS_TV
+ (UIStatusBarStyle)statusBarStyleForRNSStatusBarStyle:(ABI45_0_0RNSStatusBarStyle)statusBarStyle
{
#if defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && defined(__IPHONE_13_0) && \
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
  if (@available(iOS 13.0, *)) {
    switch (statusBarStyle) {
      case ABI45_0_0RNSStatusBarStyleAuto:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleLightContent
            : UIStatusBarStyleDarkContent;
      case ABI45_0_0RNSStatusBarStyleInverted:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleDarkContent
            : UIStatusBarStyleLightContent;
      case ABI45_0_0RNSStatusBarStyleLight:
        return UIStatusBarStyleLightContent;
      case ABI45_0_0RNSStatusBarStyleDark:
  ...
  if (statusBarStyle == ABI45_0_0RNSStatusBarStyleLight) {
    return UIStatusBarStyleLightContent;
  }
...
+ (void)enforceDesiredDeviceOrientation
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIInterfaceOrientation currentDeviceOrientation =
        [ABI45_0_0RNSScreenWindowTraits interfaceOrientationFromDeviceOrientation:[[UIDevice currentDevice] orientation]];
    UIInterfaceOrientation currentInterfaceOrientation = [ABI45_0_0RNSScreenWindowTraits interfaceOrientation];
    UIInterfaceOrientation newOrientation = UIInterfaceOrientationUnknown;
    if ([ABI45_0_0RNSScreenWindowTraits maskFromOrientation:currentDeviceOrientation] & orientationMask) {
      if (!([ABI45_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
      ...
      if (!([ABI45_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
        ...
        // rotation from mask
        newOrientation = [ABI45_0_0RNSScreenWindowTraits defaultOrientationForOrientationMask:orientationMask];
      } else {
...
{
  [ABI45_0_0RNSScreenWindowTraits updateStatusBarAppearance];
  [ABI45_0_0RNSScreenWindowTraits enforceDesiredDeviceOrientation];
  [ABI45_0_0RNSScreenWindowTraits updateHomeIndicatorAutoHidden];
}
...
// method to be used in Expo for checking if ABI45_0_0RNScreens have trait set
+ (BOOL)shouldAskScreensForTrait:(ABI45_0_0RNSWindowTrait)trait
                 includingModals:(BOOL)includingModals
                inViewController:(UIViewController *)vc
{
  UIViewController *lastViewController = [[vc childViewControllers] lastObject];
  if ([lastViewController conformsToProtocol:@protocol(ABI45_0_0RNScreensViewControllerDelegate)]) {
    UIViewController *vc = nil;
    if ([lastViewController isKindOfClass:[ABI45_0_0RNScreensViewController class]]) {
      vc = [(ABI45_0_0RNScreensViewController *)lastViewController findActiveChildVC];
    } else if ([lastViewController isKindOfClass:[ABI45_0_0RNScreensNavigationController class]]) {
      vc = [(ABI45_0_0RNScreensNavigationController *)lastViewController topViewController];
    }
    return [vc isKindOfClass:[ABI45_0_0RNSScreen class]] &&
        [(ABI45_0_0RNSScreen *)vc findChildVCForConfigAndTrait:trait includingModals:includingModals] != nil;
  }
  return NO;
}
...
{
  return [ABI45_0_0RNSScreenWindowTraits shouldAskScreensForTrait:ABI45_0_0RNSWindowTraitOrientation
                                         includingModals:YES
                                        inViewController:vc];
}
