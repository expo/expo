...
@implementation ABI45_0_0RNScreensViewController
...
- (UIViewController *)findActiveChildVC
{
  for (UIViewController *childVC in self.childViewControllers) {
    if ([childVC isKindOfClass:[ABI45_0_0RNSScreen class]] &&
        ((ABI45_0_0RNSScreenView *)((ABI45_0_0RNSScreen *)childVC.view)).activityState == ABI45_0_0RNSActivityStateOnTop) {
      return childVC;
    }
  }
...
@implementation ABI45_0_0RNSScreenContainerView {
  BOOL _invalidated;
...
{
  _controller = [[ABI45_0_0RNScreensViewController alloc] init];
...
- (void)insertABI45_0_0ReactSubview:(ABI45_0_0RNSScreenView *)subview atIndex:(NSInteger)atIndex
{
  subview.ABI45_0_0ReactSuperview = self;
  [_ABI45_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  [subview ABI45_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
}

- (void)removeABI45_0_0ReactSubview:(ABI45_0_0RNSScreenView *)subview
{
  subview.ABI45_0_0ReactSuperview = nil;
...
- (UIViewController *)findChildControllerForScreen:(ABI45_0_0RNSScreenView *)screen
{
...
- (void)prepareDetach:(ABI45_0_0RNSScreenView *)screen
{
  [[self findChildControllerForScreen:screen] willMoveToParentViewController:nil];
}

- (void)detachScreen:(ABI45_0_0RNSScreenView *)screen
{
...
- (void)attachScreen:(ABI45_0_0RNSScreenView *)screen atIndex:(NSInteger)index
{
  [_controller addChildViewController:screen.controller];
  // the frame is already set at this moment because we adjust it in insertABI45_0_0ReactSubview. We don't
...
- (void)updateContainer
{
  ...
  for (ABI45_0_0RNSScreenView *screen in _ABI45_0_0ReactSubviews) {
    if (screen.activityState == ABI45_0_0RNSActivityStateInactive && [_activeScreens containsObject:screen]) {
      screenRemoved = YES;
      [self detachScreen:screen];
    }
    [orphaned removeObject:screen];
  }
  for (ABI45_0_0RNSScreenView *screen in orphaned) {
    screenRemoved = YES;
    [self detachScreen:screen];
  }
  ...
  if (screenAdded) {
    ...
    NSInteger index = 0;
    for (ABI45_0_0RNSScreenView *screen in _ABI45_0_0ReactSubviews) {
      if (screen.activityState != ABI45_0_0RNSActivityStateInactive) {
        if ([_activeScreens containsObject:screen] && screen.activityState == ABI45_0_0RNSActivityStateTransitioningOrBelowTop) {
...
- (void)didMoveToWindow
{
  if (self.window && !_invalidated) {
    ...
    // to a window despite the fact it has been removed from the ABI45_0_0React Native view hierarchy.
    [self ABI45_0_0ReactAddControllerToClosestParent:_controller];
  }
}
...
- (void)layoutSubviews
{
  ...
  for (ABI45_0_0RNSScreenView *subview in _ABI45_0_0ReactSubviews) {
    [subview ABI45_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
@implementation ABI45_0_0RNSScreenContainerManager

ABI45_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  return [[ABI45_0_0RNSScreenContainerView alloc] init];
}
