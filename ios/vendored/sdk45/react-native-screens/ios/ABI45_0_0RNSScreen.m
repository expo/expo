...
#import "ABI45_0_0RNSScreenStackHeaderConfig.h"
...
@interface ABI45_0_0RNSScreenView () <UIAdaptivePresentationControllerDelegate, ABI45_0_0RCTInvalidating>
@end

@implementation ABI45_0_0RNSScreenView {
  __weak ABI45_0_0RCTBridge *_bridge;
  ABI45_0_0RNSScreen *_controller;
...
- (instancetype)initWithBridge:(ABI45_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
    _controller = [[ABI45_0_0RNSScreen alloc] initWithView:self];
    _stackPresentation = ABI45_0_0RNSScreenStackPresentationPush;
    _stackAnimation = ABI45_0_0RNSScreenStackAnimationDefault;
    _gestureEnabled = YES;
    _replaceAnimation = ABI45_0_0RNSScreenReplaceAnimationPop;
    _dismissed = NO;
...
{
  _reactFrame = frame;
  UIViewController *parentVC = self.ABI45_0_0ReactViewController.parentViewController;
  if (parentVC != nil && ![parentVC isKindOfClass:[ABI45_0_0RNScreensNavigationController class]]) {
    [super ABI45_0_0ReactSetFrame:frame];
  }
  // when screen is mounted under ABI45_0_0RNScreensNavigationController it's size is controller
...
- (void)setActivityStateOrNil:(NSNumber *)activityStateOrNil
{
  ...
  if (activityStateOrNil != nil && activityState != _activityState) {
    ...
    [_ABI45_0_0ReactSuperview markChildUpdated];
  }
}
...
- (void)setPointerEvents:(ABI45_0_0RCTPointerEvents)pointerEvents
{
  ...
  // any attempt of setting that via ABI45_0_0React props
}

- (void)setStackPresentation:(ABI45_0_0RNSScreenStackPresentation)stackPresentation
{
  switch (stackPresentation) {
    case ABI45_0_0RNSScreenStackPresentationModal:
...
#endif
      break;
    case ABI45_0_0RNSScreenStackPresentationFullScreenModal:
      _controller.modalPresentationStyle = UIModalPresentationFullScreen;
      break;
#if !TARGET_OS_TV
    case ABI45_0_0RNSScreenStackPresentationFormSheet:
      _controller.modalPresentationStyle = UIModalPresentationFormSheet;
      break;
#endif
    case ABI45_0_0RNSScreenStackPresentationTransparentModal:
      _controller.modalPresentationStyle = UIModalPresentationOverFullScreen;
      break;
    case ABI45_0_0RNSScreenStackPresentationContainedModal:
      _controller.modalPresentationStyle = UIModalPresentationCurrentContext;
      break;
    case ABI45_0_0RNSScreenStackPresentationContainedTransparentModal:
      _controller.modalPresentationStyle = UIModalPresentationOverCurrentContext;
      break;
    case ABI45_0_0RNSScreenStackPresentationPush:
  ...
  if (stackPresentation != ABI45_0_0RNSScreenStackPresentationPush) {
  ...
  } else if (_stackPresentation != ABI45_0_0RNSScreenStackPresentationPush) {
...
- (void)setStackAnimation:(ABI45_0_0RNSScreenStackAnimation)stackAnimation
{
  _stackAnimation = stackAnimation;

  switch (stackAnimation) {
    case ABI45_0_0RNSScreenStackAnimationFade:
      _controller.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
      break;
#if !TARGET_OS_TV
    case ABI45_0_0RNSScreenStackAnimationFlip:
      _controller.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
      break;
#endif
    case ABI45_0_0RNSScreenStackAnimationNone:
    case ABI45_0_0RNSScreenStackAnimationDefault:
    case ABI45_0_0RNSScreenStackAnimationSimplePush:
    case ABI45_0_0RNSScreenStackAnimationSlideFromBottom:
    case ABI45_0_0RNSScreenStackAnimationFadeFromBottom:
      // Default
      break;
  }
}
...
- (void)setReplaceAnimation:(ABI45_0_0RNSScreenReplaceAnimation)replaceAnimation
{
  _replaceAnimation = replaceAnimation;
}

#if !TARGET_OS_TV
- (void)setStatusBarStyle:(ABI45_0_0RNSStatusBarStyle)statusBarStyle
{
  _hasStatusBarStyleSet = YES;
  _statusBarStyle = statusBarStyle;
  [ABI45_0_0RNSScreenWindowTraits assertViewControllerBasedStatusBarAppearenceSet];
...
{
  return _ABI45_0_0ReactSuperview;
}

- (void)addSubview:(UIView *)view
{
  if (![view isKindOfClass:[ABI45_0_0RNSScreenStackHeaderConfig class]]) {
    [super addSubview:view];
  } else {
    ((ABI45_0_0RNSScreenStackHeaderConfig *)view).screenView = self;
  }
}
...
- (void)notifyWillAppear
{
  ...
  [self ABI45_0_0ReactSetFrame:_reactFrame];
}
...
- (BOOL)isMountedUnderScreenOrReactRoot
{
  for (UIView *parent = self.superview; parent != nil; parent = parent.superview) {
    if ([parent isKindOfClass:[ABI45_0_0RCTRootView class]] || [parent isKindOfClass:[ABI45_0_0RNSScreenView class]]) {
      return YES;
    }
  }
  return NO;
...
{
  // For RN touches to work we need to instantiate and connect ABI45_0_0RCTTouchHandler. This only applies
  // for screens that aren't mounted under ABI45_0_0RCTRootView e.g., modals that are mounted directly to
...
- (void)presentationControllerWillDismiss:(UIPresentationController *)presentationController
{
  ...
  // Because of that, at the moment when this method gets called the ABI45_0_0React's
  // gesture recognizer is already in FAILED state but cancel events never gets
  // send to JS. Calling "reset" forces ABI45_0_0RCTTouchHanler to dispatch cancel event.
...
- (ABI45_0_0RCTTouchHandler *)touchHandler
{
  if (_touchHandler != nil) {
...
{
  if ([_ABI45_0_0ReactSuperview respondsToSelector:@selector(presentationControllerDidDismiss:)]) {
    [_ABI45_0_0ReactSuperview performSelector:@selector(presentationControllerDidDismiss:) withObject:presentationController];
  }
}
...
@implementation ABI45_0_0RNSScreen {
...
{
  UIViewController *vc = [self findChildVCForConfigAndTrait:ABI45_0_0RNSWindowTraitStyle includingModals:NO];
  return vc == self ? nil : vc;
}

- (UIStatusBarStyle)preferredStatusBarStyle
{
  return [ABI45_0_0RNSScreenWindowTraits statusBarStyleForRNSStatusBarStyle:((ABI45_0_0RNSScreenView *)self.view).statusBarStyle];
}

- (UIViewController *)childViewControllerForStatusBarHidden
{
  UIViewController *vc = [self findChildVCForConfigAndTrait:ABI45_0_0RNSWindowTraitHidden includingModals:NO];
  return vc == self ? nil : vc;
}

- (BOOL)prefersStatusBarHidden
{
  return ((ABI45_0_0RNSScreenView *)self.view).statusBarHidden;
}

- (UIStatusBarAnimation)preferredStatusBarUpdateAnimation
{
  UIViewController *vc = [self findChildVCForConfigAndTrait:ABI45_0_0RNSWindowTraitAnimation includingModals:NO];

  if ([vc isKindOfClass:[ABI45_0_0RNSScreen class]]) {
    return ((ABI45_0_0RNSScreenView *)vc.view).statusBarAnimation;
  }
...
{
  UIViewController *vc = [self findChildVCForConfigAndTrait:ABI45_0_0RNSWindowTraitOrientation includingModals:YES];

  if ([vc isKindOfClass:[ABI45_0_0RNSScreen class]]) {
    return ((ABI45_0_0RNSScreenView *)vc.view).screenOrientation;
  }
...
{
  UIViewController *vc = [self findChildVCForConfigAndTrait:ABI45_0_0RNSWindowTraitHomeIndicatorHidden includingModals:YES];
  return vc == self ? nil : vc;
}

- (BOOL)prefersHomeIndicatorAutoHidden
{
  return ((ABI45_0_0RNSScreenView *)self.view).homeIndicatorHidden;
}
...
- (UIViewController *)findChildVCForConfigAndTrait:(ABI45_0_0RNSWindowTrait)trait includingModals:(BOOL)includingModals
{
  ...
  if ([self.presentedViewController isKindOfClass:[ABI45_0_0RNSScreen class]]) {
    ...
    return !includingModals
        ? nil
        : [(ABI45_0_0RNSScreen *)lastViewController findChildVCForConfigAndTrait:trait includingModals:includingModals]
            ?: lastViewController;
  }
  ...
  if (lastViewController == nil) {
    ...
    if ([lastViewController conformsToProtocol:@protocol(ABI45_0_0RNScreensViewControllerDelegate)]) {
      ...
      if ([childScreen isKindOfClass:[ABI45_0_0RNSScreen class]]) {
        return [(ABI45_0_0RNSScreen *)childScreen findChildVCForConfigAndTrait:trait includingModals:includingModals]
            ?: selfOrNil;
      } else {
        return selfOrNil;
      }
...
- (BOOL)hasTraitSet:(ABI45_0_0RNSWindowTrait)trait
{
  switch (trait) {
    case ABI45_0_0RNSWindowTraitStyle: {
      return ((ABI45_0_0RNSScreenView *)self.view).hasStatusBarStyleSet;
    }
    case ABI45_0_0RNSWindowTraitAnimation: {
      return ((ABI45_0_0RNSScreenView *)self.view).hasStatusBarAnimationSet;
    }
    case ABI45_0_0RNSWindowTraitHidden: {
      return ((ABI45_0_0RNSScreenView *)self.view).hasStatusBarHiddenSet;
    }
    case ABI45_0_0RNSWindowTraitOrientation: {
      return ((ABI45_0_0RNSScreenView *)self.view).hasOrientationSet;
    }
    case ABI45_0_0RNSWindowTraitHomeIndicatorHidden: {
      return ((ABI45_0_0RNSScreenView *)self.view).hasHomeIndicatorHiddenSet;
    }
    default: {
      ABI45_0_0RCTLogError(@"Unknown trait passed: %d", (int)trait);
    }
  }
  return NO;
}
...
- (void)viewDidLayoutSubviews
{
  ...
  // into account only when the view is mounted under ABI45_0_0RNScreensNavigationController in which case system
  ...
  BOOL isDisplayedWithinUINavController =
      [self.parentViewController isKindOfClass:[ABI45_0_0RNScreensNavigationController class]];
  ...
  if ((isDisplayedWithinUINavController || isPresentedAsNativeModal) &&
    ...
    [((ABI45_0_0RNSScreenView *)self.viewIfLoaded) updateBounds];
  }
}
...
- (void)viewWillAppear:(BOOL)animated
{
  ...
  if (!_isSwiping) {
    [((ABI45_0_0RNSScreenView *)self.view) notifyWillAppear];
  ...
  [ABI45_0_0RNSScreenWindowTraits updateWindowTraits];
  if (_shouldNotify) {
    _closing = NO;
...
- (void)hideHeaderIfNecessary
{
  ...
  if (@available(iOS 13.0, *)) {
    ...
    if (currentIndex > 0 && [self.view.ABI45_0_0ReactSubviews[0] isKindOfClass:[ABI45_0_0RNSScreenStackHeaderConfig class]]) {
      UINavigationItem *prevNavigationItem =
          [self.navigationController.viewControllers objectAtIndex:currentIndex - 1].navigationItem;
      ABI45_0_0RNSScreenStackHeaderConfig *config = ((ABI45_0_0RNSScreenStackHeaderConfig *)self.view.ABI45_0_0ReactSubviews[0]);
...
- (void)viewWillDisappear:(BOOL)animated
{
  ...
  if (!_isSwiping) {
    [((ABI45_0_0RNSScreenView *)self.view) notifyWillDisappear];
    if (self.transitionCoordinator.isInteractive) {
      _isSwiping = YES;
    }
  } else {
...
- (void)viewDidAppear:(BOOL)animated
{
  ...
  if (!_isSwiping || _shouldNotify) {
    ...
    [((ABI45_0_0RNSScreenView *)self.view) notifyAppear];
...
- (void)viewDidDisappear:(BOOL)animated
{
  ...
  if (self.parentViewController == nil && self.presentingViewController == nil) {
    if (((ABI45_0_0RNSScreenView *)self.view).preventNativeDismiss) {
      ...
      [((ABI45_0_0RNSScreenView *)self.view).ABI45_0_0ReactSuperview updateContainer];
      [((ABI45_0_0RNSScreenView *)self.view) notifyDismissCancelledWithDismissCount:_dismissCount];
    } else {
      // screen dismissed, send event
      [((ABI45_0_0RNSScreenView *)self.view) notifyDismissedWithCount:_dismissCount];
    }
  }

  // same flow as in viewDidAppear
  if (!_isSwiping || _shouldNotify) {
    [((ABI45_0_0RNSScreenView *)self.view) notifyDisappear];
...
{
  return (int)[[self.view.ABI45_0_0ReactSuperview ABI45_0_0ReactSubviews] indexOfObject:view];
}

- (int)getParentChildrenCount
{
  return (int)[[self.view.ABI45_0_0ReactSuperview ABI45_0_0ReactSubviews] count];
}
...
- (void)notifyTransitionProgress:(double)progress closing:(BOOL)closing goingForward:(BOOL)goingForward
{
  [((ABI45_0_0RNSScreenView *)self.view) notifyTransitionProgress:progress closing:closing goingForward:goingForward];
}

@end

@implementation ABI45_0_0RNSScreenManager

ABI45_0_0RCT_EXPORT_MODULE()

// we want to handle the case when activityState is nil
ABI45_0_0RCT_REMAP_VIEW_PROPERTY(activityState, activityStateOrNil, NSNumber)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(customAnimationOnSwipe, BOOL);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(fullScreenSwipeEnabled, BOOL);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(gestureEnabled, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(preventNativeDismiss, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(replaceAnimation, ABI45_0_0RNSScreenReplaceAnimation)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(stackPresentation, ABI45_0_0RNSScreenStackPresentation)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(stackAnimation, ABI45_0_0RNSScreenStackAnimation)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(swipeDirection, ABI45_0_0RNSScreenSwipeDirection)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(transitionDuration, NSNumber)

ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onAppear, ABI45_0_0RCTDirectEventBlock);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onDisappear, ABI45_0_0RCTDirectEventBlock);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onDismissed, ABI45_0_0RCTDirectEventBlock);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onNativeDismissCancelled, ABI45_0_0RCTDirectEventBlock);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onTransitionProgress, ABI45_0_0RCTDirectEventBlock);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onWillAppear, ABI45_0_0RCTDirectEventBlock);
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onWillDisappear, ABI45_0_0RCTDirectEventBlock);

#if !TARGET_OS_TV
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(screenOrientation, UIInterfaceOrientationMask)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(statusBarAnimation, UIStatusBarAnimation)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(statusBarHidden, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(statusBarStyle, ABI45_0_0RNSStatusBarStyle)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(homeIndicatorHidden, BOOL)
#endif

- (UIView *)view
{
  return [[ABI45_0_0RNSScreenView alloc] initWithBridge:self.bridge];
}

@end

@implementation ABI45_0_0RCTConvert (ABI45_0_0RNSScreen)

ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RNSScreenStackPresentation,
    (@{
      @"push" : @(ABI45_0_0RNSScreenStackPresentationPush),
      @"modal" : @(ABI45_0_0RNSScreenStackPresentationModal),
      @"fullScreenModal" : @(ABI45_0_0RNSScreenStackPresentationFullScreenModal),
      @"formSheet" : @(ABI45_0_0RNSScreenStackPresentationFormSheet),
      @"containedModal" : @(ABI45_0_0RNSScreenStackPresentationContainedModal),
      @"transparentModal" : @(ABI45_0_0RNSScreenStackPresentationTransparentModal),
      @"containedTransparentModal" : @(ABI45_0_0RNSScreenStackPresentationContainedTransparentModal)
    }),
    ABI45_0_0RNSScreenStackPresentationPush,
    integerValue)

ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RNSScreenStackAnimation,
    (@{
      @"default" : @(ABI45_0_0RNSScreenStackAnimationDefault),
      @"none" : @(ABI45_0_0RNSScreenStackAnimationNone),
      @"fade" : @(ABI45_0_0RNSScreenStackAnimationFade),
      @"fade_from_bottom" : @(ABI45_0_0RNSScreenStackAnimationFadeFromBottom),
      @"flip" : @(ABI45_0_0RNSScreenStackAnimationFlip),
      @"simple_push" : @(ABI45_0_0RNSScreenStackAnimationSimplePush),
      @"slide_from_bottom" : @(ABI45_0_0RNSScreenStackAnimationSlideFromBottom),
      @"slide_from_right" : @(ABI45_0_0RNSScreenStackAnimationDefault),
      @"slide_from_left" : @(ABI45_0_0RNSScreenStackAnimationDefault),
    }),
    ABI45_0_0RNSScreenStackAnimationDefault,
    integerValue)

ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RNSScreenReplaceAnimation,
    (@{
      @"push" : @(ABI45_0_0RNSScreenReplaceAnimationPush),
      @"pop" : @(ABI45_0_0RNSScreenReplaceAnimationPop),
    }),
    ABI45_0_0RNSScreenReplaceAnimationPop,
    integerValue)

ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RNSScreenSwipeDirection,
    (@{
      @"vertical" : @(ABI45_0_0RNSScreenSwipeDirectionVertical),
      @"horizontal" : @(ABI45_0_0RNSScreenSwipeDirectionHorizontal),
    }),
...
#if !TARGET_OS_TV
ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RNSStatusBarStyle,
    (@{
      @"auto" : @(ABI45_0_0RNSStatusBarStyleAuto),
      @"inverted" : @(ABI45_0_0RNSStatusBarStyleInverted),
      @"light" : @(ABI45_0_0RNSStatusBarStyleLight),
      @"dark" : @(ABI45_0_0RNSStatusBarStyleDark),
    }),
