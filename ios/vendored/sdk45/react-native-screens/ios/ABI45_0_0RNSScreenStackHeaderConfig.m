...
// of a given ABI45_0_0RCTImageView. See more comments in the code section processing SubviewTypeBackButton
@interface ABI45_0_0RCTImageView (Private)
- (UIImage *)image;
@end

@interface ABI45_0_0RCTImageLoader (Private)
- (id<ABI45_0_0RCTImageCache>)imageCache;
@end

@interface ABI45_0_0RNSScreenStackHeaderSubview : UIView

@property (nonatomic, weak) ABI45_0_0RCTBridge *bridge;
@property (nonatomic, weak) UIView *ABI45_0_0ReactSuperview;
...
@implementation ABI45_0_0RNSScreenStackHeaderSubview
...
{
  // Block any attempt to set coordinates on ABI45_0_0RNSScreenStackHeaderSubview. This
  // makes UINavigationBar the only one to control the position of header content.
  [super ABI45_0_0ReactSetFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
}

@end

@interface ABI45_0_0RNSUIBarButtonItem : UIBarButtonItem

@property (nonatomic) BOOL menuHidden;

@end

@implementation ABI45_0_0RNSUIBarButtonItem
...
@implementation ABI45_0_0RNSScreenStackHeaderConfig {
  NSMutableArray<ABI45_0_0RNSScreenStackHeaderSubview *> *_ABI45_0_0ReactSubviews;
}
...
- (void)insertABI45_0_0ReactSubview:(ABI45_0_0RNSScreenStackHeaderSubview *)subview atIndex:(NSInteger)atIndex
{
  [_ABI45_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  subview.ABI45_0_0ReactSuperview = self;
}

- (void)removeABI45_0_0ReactSubview:(ABI45_0_0RNSScreenStackHeaderSubview *)subview
{
...
{
  for (ABI45_0_0RNSScreenStackHeaderSubview *subview in _ABI45_0_0ReactSubviews) {
    if (subview.type == ABI45_0_0RNSScreenStackHeaderSubviewTypeLeft || subview.type == ABI45_0_0RNSScreenStackHeaderSubviewTypeRight) {
      ...
      // we convert the point to ABI45_0_0RNSScreenStackView since it always contains the header inside it
      CGPoint convertedPoint = [_screenView.ABI45_0_0ReactSuperview convertPoint:point toView:headerComponent];
...
- (void)updateViewControllerIfNeeded
{
  ...
  BOOL isInFullScreenModal = nav == nil && _screenView.stackPresentation == ABI45_0_0RNSScreenStackPresentationFullScreenModal;
  ...
  if (vc != nil && (nextVC == vc || isInFullScreenModal || isPresentingVC)) {
    [ABI45_0_0RNSScreenStackHeaderConfig updateViewController:self.screenView.controller withConfig:self animated:YES];
  }
}
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  // frame of ABI45_0_0RNSScreen will not be changed and screen content will remain the same size.
...
+ (void)setAnimatedConfig:(UIViewController *)vc withConfig:(ABI45_0_0RNSScreenStackHeaderConfig *)config
{
...
#endif
  {
    ...
    if (config.titleFontFamily || config.titleFontSize || config.titleFontWeight || config.titleColor) {
      ...
      if (family || weight) {
        attrs[NSFontAttributeName] = [ABI45_0_0RCTFont updateFont:nil
                                              withFamily:family
                                                    size:size
                                                  weight:weight
                                                   style:nil
    ...
    if (@available(iOS 11.0, *)) {
      if (config.largeTitle &&
        ...
        if (largeFamily || largeWeight) {
          largeAttrs[NSFontAttributeName] = [ABI45_0_0RCTFont updateFont:nil
...
+ (UIImage *)loadBackButtonImageInViewController:(UIViewController *)vc withConfig:(ABI45_0_0RNSScreenStackHeaderConfig *)config
{
  BOOL hasBackButtonImage = NO;
  for (ABI45_0_0RNSScreenStackHeaderSubview *subview in config.ABI45_0_0ReactSubviews) {
    if (subview.type == ABI45_0_0RNSScreenStackHeaderSubviewTypeBackButton && subview.subviews.count > 0) {
      hasBackButtonImage = YES;
      ABI45_0_0RCTImageView *imageView = subview.subviews[0];
      if (imageView.image == nil) {
        ...
        ABI45_0_0RCTImageSource *source = imageView.imageSources[0];
        [imageView ABI45_0_0ReactSetFrame:CGRectMake(
      ...
      if (image == nil) {
        ...
        ABI45_0_0RCTImageSource *source = imageView.imageSources[0];
        ABI45_0_0RCTImageLoader *imageloader = [subview.bridge moduleForClass:[ABI45_0_0RCTImageLoader class]];
...
+ (UINavigationBarAppearance *)buildAppearance:(UIViewController *)vc
                                    withConfig:(ABI45_0_0RNSScreenStackHeaderConfig *)config API_AVAILABLE(ios(13.0))
{
                    ...
                    animated:(BOOL)animated
{
  ...
  if (config.backTitle != nil || config.backTitleFontFamily || config.backTitleFontSize ||
    ...
    ABI45_0_0RNSUIBarButtonItem *backBarButtonItem = [[ABI45_0_0RNSUIBarButtonItem alloc] initWithTitle:config.backTitle ?: prevItem.title
  ...
  for (ABI45_0_0RNSScreenStackHeaderSubview *subview in config.ABI45_0_0ReactSubviews) {
    switch (subview.type) {
      case ABI45_0_0RNSScreenStackHeaderSubviewTypeLeft: {
#if !TARGET_OS_TV
      ...
      case ABI45_0_0RNSScreenStackHeaderSubviewTypeRight: {
        UIBarButtonItem *buttonItem = [[UIBarButtonItem alloc] initWithCustomView:subview];
        navitem.rightBarButtonItem = buttonItem;
        break;
      }
      case ABI45_0_0RNSScreenStackHeaderSubviewTypeCenter:
      case ABI45_0_0RNSScreenStackHeaderSubviewTypeTitle: {
        navitem.titleView = subview;
        break;
      }
      case ABI45_0_0RNSScreenStackHeaderSubviewTypeSearchBar: {
        ...
        if ([subview.subviews[0] isKindOfClass:[ABI45_0_0RNSSearchBar class]]) {
#if !TARGET_OS_TV
          if (@available(iOS 11.0, *)) {
            ABI45_0_0RNSSearchBar *searchBar = subview.subviews[0];
      ...
      case ABI45_0_0RNSScreenStackHeaderSubviewTypeBackButton: {
        break;
      }
    }
  }
    ...
    [vc.transitionCoordinator
        ...
        completion:^(id<UIViewControllerTransitionCoordinatorContext> _Nonnull context) {
          if ([context isCancelled]) {
            ...
            ABI45_0_0RNSScreenStackHeaderConfig *config = nil;
            for (UIView *subview in fromVC.view.ABI45_0_0ReactSubviews) {
              if ([subview isKindOfClass:[ABI45_0_0RNSScreenStackHeaderConfig class]]) {
                config = (ABI45_0_0RNSScreenStackHeaderConfig *)subview;
                break;
              }
            }
...
@implementation ABI45_0_0RNSScreenStackHeaderConfigManager

ABI45_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  return [ABI45_0_0RNSScreenStackHeaderConfig new];
}

ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(title, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontFamily, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontSize, NSNumber)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontWeight, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(titleColor, UIColor)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(backTitle, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontFamily, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontSize, NSNumber)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(backgroundColor, UIColor)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(blurEffect, UIBlurEffectStyle)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(color, UIColor)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(direction, UISemanticContentAttribute)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitle, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontFamily, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontSize, NSNumber)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontWeight, NSString)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleColor, UIColor)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleBackgroundColor, UIColor)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleHideShadow, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(hideBackButton, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(hideShadow, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(backButtonInCustomView, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(disableBackButtonMenu, BOOL)
// `hidden` is an UIView property, we need to use different name internally
ABI45_0_0RCT_REMAP_VIEW_PROPERTY(hidden, hide, BOOL)
ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(translucent, BOOL)

@end

@implementation ABI45_0_0RCTConvert (ABI45_0_0RNSScreenStackHeader)
...
ABI45_0_0RCT_ENUM_CONVERTER(
    ABI45_0_0RNSScreenStackHeaderSubviewType,
    (@{
      @"back" : @(ABI45_0_0RNSScreenStackHeaderSubviewTypeBackButton),
      @"left" : @(ABI45_0_0RNSScreenStackHeaderSubviewTypeLeft),
      @"right" : @(ABI45_0_0RNSScreenStackHeaderSubviewTypeRight),
      @"title" : @(ABI45_0_0RNSScreenStackHeaderSubviewTypeTitle),
      @"center" : @(ABI45_0_0RNSScreenStackHeaderSubviewTypeCenter),
      @"searchBar" : @(ABI45_0_0RNSScreenStackHeaderSubviewTypeSearchBar),
    }),
...
ABI45_0_0RCT_ENUM_CONVERTER(UIBlurEffectStyle, ([self blurEffectsForIOSVersion]), UIBlurEffectStyleExtraLight, integerValue)

@end

@implementation ABI45_0_0RNSScreenStackHeaderSubviewManager

ABI45_0_0RCT_EXPORT_MODULE()

ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(type, ABI45_0_0RNSScreenStackHeaderSubviewType)

- (UIView *)view
{
  return [[ABI45_0_0RNSScreenStackHeaderSubview alloc] initWithBridge:self.bridge];
}
