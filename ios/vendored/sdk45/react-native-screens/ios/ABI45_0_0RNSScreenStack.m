...
@interface ABI45_0_0RNSScreenStackView () <
...
@implementation ABI45_0_0RNScreensNavigationController
...
#if !TARGET_OS_TV
@interface ABI45_0_0RNSScreenEdgeGestureRecognizer : UIScreenEdgePanGestureRecognizer
@end

@implementation ABI45_0_0RNSScreenEdgeGestureRecognizer
@end

@interface ABI45_0_0RNSPanGestureRecognizer : UIPanGestureRecognizer
@end

@implementation ABI45_0_0RNSPanGestureRecognizer
@end
#endif

@implementation ABI45_0_0RNSScreenStackView {
  UINavigationController *_controller;
  NSMutableArray<ABI45_0_0RNSScreenView *> *_ABI45_0_0ReactSubviews;
  __weak ABI45_0_0RNSScreenStackManager *_manager;
  BOOL _hasLayout;
  BOOL _invalidated;
...
- (instancetype)initWithManager:(ABI45_0_0RNSScreenStackManager *)manager
{
  if (self = [super init]) {
    _hasLayout = NO;
    _invalidated = NO;
    ...
    _controller = [[ABI45_0_0RNScreensNavigationController alloc] init];
                    ...
                    animated:(BOOL)animated
{
  ...
  [ABI45_0_0RNSScreenStackHeaderConfig willShowViewController:viewController animated:animated withConfig:config];
}
...
- (void)presentationControllerDidDismiss:(UIPresentationController *)presentationController
{
  ...
  if ([screenView isKindOfClass:[ABI45_0_0RNSScreenView class]]) {
    ...
    if (self.onFinishTransitioning) {
      ...
      // finish event to arrive later than onDismiss (see ABI45_0_0RNSScreen#notifyDismiss)
...
{
  if (![subview isKindOfClass:[ABI45_0_0RNSScreenView class]]) {
    ABI45_0_0RCTLogError(@"ScreenStack only accepts children of type Screen");
    return;
  }
...
- (void)setModalViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  for (NSUInteger i = changeRootIndex; i < controllers.count; i++) {
    if ([_presentedModals containsObject:controllers[i]]) {
      ABI45_0_0RCTAssert(false, @"Modally presented controllers are being reshuffled, this is not allowed");
    }
  }

  __weak ABI45_0_0RNSScreenStackView *weakSelf = self;
  ...
  void (^finish)(void) = ^{
    ...
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
        ...
        BOOL shouldAnimate = lastModal && [next isKindOfClass:[ABI45_0_0RNSScreen class]] &&
            ((ABI45_0_0RNSScreenView *)next.view).stackAnimation != ABI45_0_0RNSScreenStackAnimationNone;
  ...
  if (changeRootController.presentedViewController != nil &&
    ...
    BOOL shouldAnimate = changeRootIndex == controllers.count &&
        [changeRootController.presentedViewController isKindOfClass:[ABI45_0_0RNSScreen class]] &&
        ((ABI45_0_0RNSScreenView *)changeRootController.presentedViewController.view).stackAnimation !=
...
- (void)setPushViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  BOOL firstTimePush = ![lastTop isKindOfClass:[ABI45_0_0RNSScreen class]];
  ...
  if (firstTimePush) {
    ...
    if (![controllers containsObject:lastTop]) {
      ...
      if (![_controller.viewControllers containsObject:top] &&
          ((ABI45_0_0RNSScreenView *)top.view).replaceAnimation == ABI45_0_0RNSScreenReplaceAnimationPush) {
  ...
  for (ABI45_0_0RNSScreenView *screen in _ABI45_0_0ReactSubviews) {
    if (!screen.dismissed && screen.controller != nil) {
      if (pushControllers.count == 0) {
        ...
        if (screen.stackPresentation == ABI45_0_0RNSScreenStackPresentationPush) {
          [pushControllers addObject:screen.controller];
        } else {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  if (CGRectContainsPoint(_controller.navigationBar.frame, point)) {
    ...
    UIView *headerConfig = [[_ABI45_0_0ReactSubviews.lastObject ABI45_0_0ReactSubviews] firstObject];
    if ([headerConfig isKindOfClass:[ABI45_0_0RNSScreenStackHeaderConfig class]]) {
                                                 ...
                                                 toViewController:(UIViewController *)toVC
{
  ...
  if (operation == UINavigationControllerOperationPush) {
    screen = (ABI45_0_0RNSScreenView *)toVC.view;
  } else if (operation == UINavigationControllerOperationPop) {
    screen = (ABI45_0_0RNSScreenView *)fromVC.view;
  }
  if (screen != nil &&
      ...
      (_isFullWidthSwiping || [ABI45_0_0RNSScreenStackAnimator isCustomAnimation:screen.stackAnimation])) {
    return [[ABI45_0_0RNSScreenStackAnimator alloc] initWithOperation:operation];
  }
  return nil;
}
...
- (void)cancelTouchesInParent
{
  ...
  if (parent != nil) {
    ABI45_0_0RCTTouchHandler *touchHandler = [parent performSelector:@selector(touchHandler)];
    [touchHandler cancel];
    [touchHandler reset];
  }
}
...
{
  ABI45_0_0RNSScreenView *topScreen = (ABI45_0_0RNSScreenView *)_controller.viewControllers.lastObject.view;

  if (![topScreen isKindOfClass:[ABI45_0_0RNSScreenView class]] || !topScreen.gestureEnabled ||
  ...
  if (topScreen.fullScreenSwipeEnabled) {
    // we want only `ABI45_0_0RNSPanGestureRecognizer` to be able to recognize when
    // `fullScreenSwipeEnabled` is set
    if ([gestureRecognizer isKindOfClass:[ABI45_0_0RNSPanGestureRecognizer class]]) {
  ...
  if (topScreen.customAnimationOnSwipe && [ABI45_0_0RNSScreenStackAnimator isCustomAnimation:topScreen.stackAnimation]) {
    if ([gestureRecognizer isKindOfClass:[ABI45_0_0RNSScreenEdgeGestureRecognizer class]]) {
      ...
      BOOL isCorrectEdge = (_controller.view.semanticContentAttribute == UISemanticContentAttributeForceRightToLeft &&
                            ((ABI45_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeRight) ||
          (_controller.view.semanticContentAttribute != UISemanticContentAttributeForceRightToLeft &&
           ((ABI45_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeLeft);
      if (isCorrectEdge) {
        [self cancelTouchesInParent];
        return YES;
      }
    ...
    if ([gestureRecognizer isKindOfClass:[ABI45_0_0RNSScreenEdgeGestureRecognizer class]]) {
      ...
      return NO;
    } else if ([gestureRecognizer isKindOfClass:[ABI45_0_0RNSPanGestureRecognizer class]]) {
...
- (void)setupGestureHandlers
{
  ...
  ABI45_0_0RNSScreenEdgeGestureRecognizer *leftEdgeSwipeGestureRecognizer =
      [[ABI45_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI45_0_0RNSScreenEdgeGestureRecognizer *rightEdgeSwipeGestureRecognizer =
      [[ABI45_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI45_0_0RNSPanGestureRecognizer *panRecognizer = [[ABI45_0_0RNSPanGestureRecognizer alloc] initWithTarget:self
...
- (void)handleSwipe:(UIPanGestureRecognizer *)gestureRecognizer
{
  ...
  float velocity;
  float distance;
  if (topScreen.swipeDirection == ABI45_0_0RNSScreenSwipeDirectionVertical) {
...
@implementation ABI45_0_0RNSScreenStackManager {
  NSPointerArray *_stacks;
}

ABI45_0_0RCT_EXPORT_MODULE()

ABI45_0_0RCT_EXPORT_VIEW_PROPERTY(onFinishTransitioning, ABI45_0_0RCTDirectEventBlock);

- (UIView *)view
{
  ABI45_0_0RNSScreenStackView *view = [[ABI45_0_0RNSScreenStackView alloc] initWithManager:self];
  if (!_stacks) {
...
- (void)invalidate
{
  for (ABI45_0_0RNSScreenStackView *stack in _stacks) {
