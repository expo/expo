...
#import "ABI45_0_0RNCWKProcessPoolManager.h"
#if !TARGET_OS_OSX
...
static NSString *const HistoryShimName = @"ABI45_0_0ReactNativeHistoryShim";
...
@interface ABI45_0_0_SwizzleHelperWK : UIView
@property (nonatomic, copy) WKWebView *webView;
@end
@implementation ABI45_0_0_SwizzleHelperWK
...
#if TARGET_OS_OSX
@interface ABI45_0_0RNCWKWebView : WKWebView
@end
@implementation ABI45_0_0RNCWKWebView
- (void)scrollWheel:(NSEvent *)theEvent {
  ABI45_0_0RNCWebView *rncWebView = (ABI45_0_0RNCWebView *)[self superview];
  ABI45_0_0RCTAssert([rncWebView isKindOfClass:[rncWebView class]], @"superview must be an ABI45_0_0RNCWebView");
...
@interface ABI45_0_0RNCWebView () <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler,
#if !TARGET_OS_OSX
    UIScrollViewDelegate,
#endif // !TARGET_OS_OSX
    ABI45_0_0RCTAutoInsetsProtocol>

@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onFileDownload;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onLoadingFinish;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onLoadingError;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onLoadingProgress;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onShouldStartLoadWithRequest;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onHttpError;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onMessage;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onScroll;
@property (nonatomic, copy) ABI45_0_0RCTDirectEventBlock onContentProcessDidTerminate;
#if !TARGET_OS_OSX
@property (nonatomic, copy) WKWebView *webView;
#else
@property (nonatomic, copy) ABI45_0_0RNCWKWebView *webView;
#endif // !TARGET_OS_OSX
...
@implementation ABI45_0_0RNCWebView
{
#if !TARGET_OS_OSX
  UIColor * _savedBackgroundColor;
#else
  ABI45_0_0RCTUIColor * _savedBackgroundColor;
#endif // !TARGET_OS_OSX
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    #else
    super.backgroundColor = [ABI45_0_0RCTUIColor clearColor];
    #endif // !TARGET_OS_OSX
    _bounces = YES;
    _scrollEnabled = YES;
    ...
    #if !TARGET_OS_OSX
    _savedStatusBarStyle = ABI45_0_0RCTSharedApplication().statusBarStyle;
    _savedStatusBarHidden = ABI45_0_0RCTSharedApplication().statusBarHidden;
    #endif // !TARGET_OS_OSX
    ...
    _mediaCapturePermissionGrantType = ABI45_0_0RNCWebViewPermissionGrantType_Prompt;
#endif
  }
    ...
    if(pressSender.state == UIGestureRecognizerStateEnded) {
      if (!self.menuItems || self.menuItems.count == 0) {
        ...
        for(NSDictionary *menuItem in self.menuItems) {
            NSString *menuItemLabel = [ABI45_0_0RCTConvert NSString:menuItem[@"label"]];
            NSString *menuItemKey = [ABI45_0_0RCTConvert NSString:menuItem[@"key"]];
...
- (void)tappedMenuItem:(NSString *)eventType
{
    ...
    if (error != nil) {
      ABI45_0_0RCTLogWarn(@"%@", [NSString stringWithFormat:@"Error evaluating injectedJavaScript: This is possibly due to an unsupported return type. Try adding true to the end of your injectedJavaScript string. %@", error]);
    } else {
      if (self.onCustomMenuSelection) {
        ...
        NSString *label = [ABI45_0_0RCTConvert NSString:selectedMenuItem[@"label"]];
        ...
        ABI45_0_0RCTLogWarn(@"Error evaluating onCustomMenuSelection: You must implement an `onCustomMenuSelection` callback when using custom menu items");
      }
    }
  }];
}
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  if(self.useSharedProcessPool) {
    wkWebViewConfig.processPool = [[ABI45_0_0RNCWKProcessPoolManager sharedManager] sharedProcessPoolForScopeKey:self.scopeKey];
  }
  ...
  [wkWebViewConfig.userContentController addScriptMessageHandler:[[ABI45_0_0RNCWeakScriptMessageDelegate alloc] initWithDelegate:self]
...
#else
    _webView = [[ABI45_0_0RNCWKWebView alloc] initWithFrame:self.bounds configuration: wkWebViewConfig];
#endif // !TARGET_OS_OSX
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    ABI45_0_0RCTUnsafeExecuteOnMainQueueSync(^{
      [ABI45_0_0RCTSharedApplication() setStatusBarStyle:self->_savedStatusBarStyle animated:YES];
    });
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    ABI45_0_0RCTUnsafeExecuteOnMainQueueSync(^{
      [ABI45_0_0RCTSharedApplication() setStatusBarHidden:self->_savedStatusBarHidden animated:YES];
      [ABI45_0_0RCTSharedApplication() setStatusBarStyle:self->_savedStatusBarStyle animated:YES];
    });
#pragma clang diagnostic pop
}
...
#else
- (void)setBackgroundColor:(ABI45_0_0RCTUIColor *)backgroundColor
#endif // !TARGET_OS_OSX
{
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI45_0_0RCTView autoAdjustInsetsForView:self
...
- (void)visitSource
{
    ...
    NSString *html = [ABI45_0_0RCTConvert NSString:_source[@"html"]];
    if (html) {
        NSURL *baseURL = [ABI45_0_0RCTConvert NSURL:_source[@"baseUrl"]];
        if (!baseURL) {
    ...
    NSString *headerCookie = [ABI45_0_0RCTConvert NSString:_source[@"headers"][@"cookie"]];
    if(headerCookie) {
    ...
    [self syncCookiesToWebView:^{
      // Because of the way ABI45_0_0React works, as pages redirect, we actually end up
      ...
      if (request.URL.host) {
          ...
          NSURL* readAccessUrl = _allowingReadAccessToURL ? [ABI45_0_0RCTConvert NSURL:_allowingReadAccessToURL] : request.URL;
...
-(void)setHideKeyboardAccessoryView:(BOOL)hideKeyboardAccessoryView
{
    ...
    NSString* name = [NSString stringWithFormat:@"%@ABI45_0_0_SwizzleHelperWK", subview.class.superclass];
    ...
    if(newClass == nil)
    {
        ...
        Method method = class_getInstanceMethod([ABI45_0_0_SwizzleHelperWK class], @selector(inputAccessoryView));
...
- (void)postMessage:(NSString *)message
{
  ...
  NSString *source = [NSString
    ...
    ABI45_0_0RCTJSONStringify(eventInitDict, NULL)
  ];
...
- (void)layoutSubviews
{
  ...
  // Ensure webview takes the position and dimensions of ABI45_0_0RNCWebView
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString *))completionHandler{
  ...
  const NSRect ABI45_0_0RCTSingleTextFieldFrame = NSMakeRect(0.0, 0.0, 275.0, 22.0);
  NSTextField *textField = [[NSTextField alloc] initWithFrame:ABI45_0_0RCTSingleTextFieldFrame];
...
- (void)                         webView:(WKWebView *)webView
  requestMediaCapturePermissionForOrigin:(WKSecurityOrigin *)origin
    ...
    if (_mediaCapturePermissionGrantType == ABI45_0_0RNCWebViewPermissionGrantType_GrantIfSameHost_ElsePrompt || _mediaCapturePermissionGrantType == ABI45_0_0RNCWebViewPermissionGrantType_GrantIfSameHost_ElseDeny) {
        if ([origin.host isEqualToString:webView.URL.host]) {
            decisionHandler(WKPermissionDecisionGrant);
        } else {
            WKPermissionDecision decision = _mediaCapturePermissionGrantType == ABI45_0_0RNCWebViewPermissionGrantType_GrantIfSameHost_ElsePrompt ? WKPermissionDecisionPrompt : WKPermissionDecisionDeny;
            decisionHandler(decision);
        }
    } else if (_mediaCapturePermissionGrantType == ABI45_0_0RNCWebViewPermissionGrantType_Deny) {
        decisionHandler(WKPermissionDecisionDeny);
    } else if (_mediaCapturePermissionGrantType == ABI45_0_0RNCWebViewPermissionGrantType_Grant) {
        decisionHandler(WKPermissionDecisionGrant);
    } else {
...
-(UIViewController *)topViewController{
    return ABI45_0_0RCTPresentedViewController();
}
...
{
  ABI45_0_0RCTLogWarn(@"Webview Process Terminated");
  if (_onContentProcessDidTerminate) {
    NSMutableDictionary<NSString *, id> *event = [self baseEvent];
    _onContentProcessDidTerminate(event);
  }
          ...
          thenCall: (void (^)(NSString*)) callback
{
  if (self.enableApplePay) {
    ABI45_0_0RCTLogWarn(@"Cannot run javascript when apple pay is enabled");
    return;
  }
      ...
      */
      ...
      for (NSHTTPCookie *cookie in [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookies]) {
        [script appendFormat:@"document.cookie = %@ + '=' + %@",
          ABI45_0_0RCTJSONStringify(cookie.name, NULL),
          ABI45_0_0RCTJSONStringify(cookie.value, NULL)];
        if (cookie.path) {
          [script appendFormat:@" + '; Path=' + %@", ABI45_0_0RCTJSONStringify(cookie.path, NULL)];
        }
...
- (NSURLRequest *)requestForSource:(id)json {
  NSURLRequest *request = [ABI45_0_0RCTConvert NSURLRequest:self.source];
...
@implementation ABI45_0_0RNCWeakScriptMessageDelegate
