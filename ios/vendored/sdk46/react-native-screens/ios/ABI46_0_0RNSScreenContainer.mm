...
@implementation ABI46_0_0RNScreensViewController
...
- (UIViewController *)findActiveChildVC
{
  for (UIViewController *childVC in self.childViewControllers) {
    if ([childVC isKindOfClass:[ABI46_0_0RNSScreen class]] &&
        ((ABI46_0_0RNSScreen *)childVC).screenView.activityState == ABI46_0_0RNSActivityStateOnTop) {
      return childVC;
    }
  }
...
@implementation ABI46_0_0RNSScreenContainerView {
  BOOL _invalidated;
...
- (instancetype)init
{
  if (self = [super init]) {
#ifdef RN_FABRIC_ENABLED
    static const auto defaultProps = std::make_shared<const ABI46_0_0facebook::ABI46_0_0React::ABI46_0_0RNSScreenContainerProps>();
    _props = defaultProps;
#endif
...
{
  _controller = [[ABI46_0_0RNScreensViewController alloc] init];
...
- (void)insertABI46_0_0ReactSubview:(ABI46_0_0RNSScreenView *)subview atIndex:(NSInteger)atIndex
{
  subview.ABI46_0_0ReactSuperview = self;
  [_ABI46_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  [subview ABI46_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
}

- (void)removeABI46_0_0ReactSubview:(ABI46_0_0RNSScreenView *)subview
{
  subview.ABI46_0_0ReactSuperview = nil;
...
- (UIViewController *)findChildControllerForScreen:(ABI46_0_0RNSScreenView *)screen
{
...
- (void)prepareDetach:(ABI46_0_0RNSScreenView *)screen
{
  [[self findChildControllerForScreen:screen] willMoveToParentViewController:nil];
}

- (void)detachScreen:(ABI46_0_0RNSScreenView *)screen
{
...
- (void)attachScreen:(ABI46_0_0RNSScreenView *)screen atIndex:(NSInteger)index
{
  [_controller addChildViewController:screen.controller];
  // the frame is already set at this moment because we adjust it in insertABI46_0_0ReactSubview. We don't
...
- (void)updateContainer
{
  ...
  for (ABI46_0_0RNSScreenView *screen in _ABI46_0_0ReactSubviews) {
    if (screen.activityState == ABI46_0_0RNSActivityStateInactive && [_activeScreens containsObject:screen]) {
      screenRemoved = YES;
      [self detachScreen:screen];
    }
    [orphaned removeObject:screen];
  }
  for (ABI46_0_0RNSScreenView *screen in orphaned) {
    screenRemoved = YES;
    [self detachScreen:screen];
  }
  ...
  if (screenAdded) {
    ...
    NSInteger index = 0;
    for (ABI46_0_0RNSScreenView *screen in _ABI46_0_0ReactSubviews) {
      if (screen.activityState != ABI46_0_0RNSActivityStateInactive) {
        if ([_activeScreens containsObject:screen] && screen.activityState == ABI46_0_0RNSActivityStateTransitioningOrBelowTop) {
...
- (void)didMoveToWindow
{
  if (self.window && !_invalidated) {
    ...
    // to a window despite the fact it has been removed from the ABI46_0_0React Native view hierarchy.
    [self ABI46_0_0ReactAddControllerToClosestParent:_controller];
  }
}
...
- (void)layoutSubviews
{
  ...
  for (ABI46_0_0RNSScreenView *subview in _ABI46_0_0ReactSubviews) {
#ifdef RN_FABRIC_ENABLED
    ABI46_0_0facebook::ABI46_0_0React::LayoutMetrics screenLayoutMetrics = subview.newLayoutMetrics;
    screenLayoutMetrics.frame = ABI46_0_0RCTRectFromCGRect(CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height));
...
{
  if (![childComponentView isKindOfClass:[ABI46_0_0RNSScreenView class]]) {
    ABI46_0_0RCTLogError(@"ScreenContainer only accepts children of type Screen");
    return;
  }

  ABI46_0_0RNSScreenView *screenView = (ABI46_0_0RNSScreenView *)childComponentView;

  ABI46_0_0RCTAssert(
      childComponentView.ABI46_0_0ReactSuperview == nil,
  ...
  [_ABI46_0_0ReactSubviews insertObject:screenView atIndex:index];
  screenView.ABI46_0_0ReactSuperview = self;
  ABI46_0_0facebook::ABI46_0_0React::LayoutMetrics screenLayoutMetrics = screenView.newLayoutMetrics;
  screenLayoutMetrics.frame = ABI46_0_0RCTRectFromCGRect(CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height));
...
{
  ABI46_0_0RCTAssert(
      childComponentView.ABI46_0_0ReactSuperview == self,
      @"Attempt to unmount a view which is mounted inside different view. (parent: %@, child: %@, index: %@)",
      self,
      childComponentView,
      @(index));
  ABI46_0_0RCTAssert(
      (_ABI46_0_0ReactSubviews.count > index) && [_ABI46_0_0ReactSubviews objectAtIndex:index] == childComponentView,
      @"Attempt to unmount a view which has a different index. (parent: %@, child: %@, index: %@, actual index: %@, tag at index: %@)",
      self,
      childComponentView,
      @(index),
      @([_ABI46_0_0ReactSubviews indexOfObject:childComponentView]),
      @([[_ABI46_0_0ReactSubviews objectAtIndex:index] tag]));
  ((ABI46_0_0RNSScreenView *)childComponentView).ABI46_0_0ReactSuperview = nil;
  [_ABI46_0_0ReactSubviews removeObject:childComponentView];
...
{
  return ABI46_0_0facebook::ABI46_0_0React::concreteComponentDescriptorProvider<ABI46_0_0facebook::ABI46_0_0React::ABI46_0_0RNSScreenContainerComponentDescriptor>();
}
...
#ifdef RN_FABRIC_ENABLED
Class<ABI46_0_0RCTComponentViewProtocol> ABI46_0_0RNSScreenContainerCls(void)
{
  return ABI46_0_0RNSScreenContainerView.class;
}
#endif

@implementation ABI46_0_0RNSScreenContainerManager

ABI46_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  return [[ABI46_0_0RNSScreenContainerView alloc] init];
}
