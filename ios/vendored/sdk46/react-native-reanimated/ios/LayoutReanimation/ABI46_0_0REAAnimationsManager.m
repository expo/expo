...
#import <ABI46_0_0React/ABI46_0_0UIView+Private.h>
#import <ABI46_0_0React/ABI46_0_0UIView+React.h>

@interface ABI46_0_0REAAnimationsManager ()
...
@implementation ABI46_0_0REAAnimationsManager {
  ABI46_0_0RCTUIManager *_uiManager;
  ABI46_0_0REAUIManager *_reaUiManager;
...
- (instancetype)initWithUIManager:(ABI46_0_0RCTUIManager *)uiManager
{
  if (self = [super init]) {
    ...
    _reaUiManager = (ABI46_0_0REAUIManager *)uiManager;
...
- (void)scheduleCleaning
{
  ...
  __weak ABI46_0_0REAAnimationsManager *weakSelf = self;
...
- (void)findRoot:(UIView *)view roots:(NSMutableSet<NSNumber *> *)roots
{
  ...
  while (currentView != nil) {
    ViewState state = [_states[currentView.ABI46_0_0ReactTag] intValue];
    if (state == Disappearing) {
      return;
    }
    if (state == ToRemove) {
      lastToRemoveTag = currentView.ABI46_0_0ReactTag;
    }
...
- (BOOL)dfs:(UIView *)root view:(UIView *)view cands:(NSMutableSet<NSNumber *> *)cands
{
  ...
  NSArray<UIView *> *toRemoveCopy = [view.ABI46_0_0ReactSubviews copy];
  for (UIView *child in toRemoveCopy) {
    if (![view isKindOfClass:[ABI46_0_0RCTTextView class]]) {
      cannotStripe |= [self dfs:root view:child cands:cands];
    }
  }
  if (!cannotStripe) {
    if (view.ABI46_0_0ReactSuperview != nil) {
...
- (void)removeLeftovers
{
  ...
  for (NSNumber *viewTag in _toRemove) {
    ...
    if (view == nil) {
      view = [_reaUiManager viewForABI46_0_0ReactTag:viewTag];
...
- (void)notifyAboutProgress:(NSDictionary *)newStyle tag:(NSNumber *)tag
{
  ...
  ABI46_0_0RCTComponentData *componentData = dataComponenetsByName[@"RCTView"];
...
- (void)setNewProps:(NSMutableDictionary *)newProps
              forView:(UIView *)view
    withComponentData:(ABI46_0_0RCTComponentData *)componentData
{
  if (newProps[@"height"]) {
...
- (void)onViewUpdate:(UIView *)view before:(ABI46_0_0REASnapshot *)before after:(ABI46_0_0REASnapshot *)after
{
  ...
  _states[view.ABI46_0_0ReactTag] = [NSNumber numberWithInt:Layout];
  NSDictionary *preparedValues = [self prepareDataForLayoutAnimatingWorklet:currentValues targetValues:targetValues];
  _startAnimationForTag(view.ABI46_0_0ReactTag, @"layout", preparedValues, @(0));
}
