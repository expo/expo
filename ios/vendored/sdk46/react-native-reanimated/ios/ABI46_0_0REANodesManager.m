#import <ABI46_0_0RNReanimated/ABI46_0_0REAAlwaysNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REABezierNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REABlockNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REACallFuncNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REAClockNodes.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REAConcatNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REACondNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0READebugNode.h>
#import <ABI46_0_0RNReanimated/ABI46_0_0REAEventNode.h>
...
#import <ABI46_0_0RNReanimated/ABI46_0_0REAPropsNode.h>
...
// Interface below has been added in order to use private methods of ABI46_0_0RCTUIManager,
// ABI46_0_0RCTUIManager#UpdateView is a ABI46_0_0React Method which is exported to JS but in
// Objective-C it stays private
// ABI46_0_0RCTUIManager#setNeedsLayout is a method which updated layout only which
// in its turn will trigger relayout if no batch has been activated

@interface ABI46_0_0RCTUIManager ()

- (void)updateView:(nonnull NSNumber *)ABI46_0_0ReactTag viewName:(NSString *)viewName props:(NSDictionary *)props;

- (void)setNeedsLayout;

@end

@interface ABI46_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands;

- (void)runSyncUIUpdatesWithObserver:(id<ABI46_0_0RCTUIManagerObserver>)observer;

@end

@interface ABI46_0_0ComponentUpdate : NSObject
...
@implementation ABI46_0_0ComponentUpdate
@end

@implementation ABI46_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands
{
  // Accessing some private bits of ABI46_0_0RCTUIManager to provide missing functionality
...
- (void)runSyncUIUpdatesWithObserver:(id<ABI46_0_0RCTUIManagerObserver>)observer
{
  ...
  NSHashTable<id<ABI46_0_0RCTUIManagerObserver>> *soleObserver = [NSHashTable new];
...
@interface ABI46_0_0REANodesManager () <ABI46_0_0RCTUIManagerObserver>

@end

@implementation ABI46_0_0REANodesManager {
  NSMutableDictionary<ABI46_0_0REANodeID, ABI46_0_0REANode *> *_nodes;
  NSMapTable<NSString *, ABI46_0_0REANode *> *_eventMapping;
  NSMutableArray<id<ABI46_0_0RCTEvent>> *_eventQueue;
  CADisplayLink *_displayLink;
  ABI46_0_0REAUpdateContext *_updateContext;
  BOOL _wantRunUpdates;
  BOOL _processingDirectEvent;
  NSMutableArray<ABI46_0_0REAOnAnimationCallback> *_onAnimationCallbacks;
  NSMutableArray<ABI46_0_0REANativeAnimationOp> *_operationsInBatch;
  BOOL _tryRunBatchUpdatesSynchronously;
  ABI46_0_0REAEventHandler _eventHandler;
  volatile void (^_mounting)(void);
  NSMutableDictionary<NSNumber *, ABI46_0_0ComponentUpdate *> *_componentUpdateBuffer;
  volatile atomic_bool _shouldFlushUpdateBuffer;
  NSMutableDictionary<NSNumber *, UIView *> *_viewRegistry;
}

- (instancetype)initWithModule:(ABI46_0_0REAModule *)reanimatedModule uiManager:(ABI46_0_0RCTUIManager *)uiManager
{
  if ((self = [super init])) {
    ...
    _updateContext = [ABI46_0_0REAUpdateContext new];
    _wantRunUpdates = NO;
...
- (ABI46_0_0REANode *)findNodeByID:(ABI46_0_0REANodeID)nodeID
{
...
- (void)onAnimationFrame:(CADisplayLink *)displayLink
{
  ...
  for (NSUInteger i = 0; i < _eventQueue.count; i++) {
    id<ABI46_0_0RCTEvent> event = _eventQueue[i];
    [self processEvent:event];
  }
  [_eventQueue removeAllObjects];

  NSArray<ABI46_0_0REAOnAnimationCallback> *callbacks = _onAnimationCallbacks;
  _onAnimationCallbacks = [NSMutableArray new];
  ...
  // we iterate over it
  for (ABI46_0_0REAOnAnimationCallback block in callbacks) {
    block(displayLink);
  }
...
{
  ABI46_0_0RCTAssert(_mounting == nil, @"Mouting block is expected to not be set");
  _mounting = block;
  return YES;
}
...
{
  if (_wantRunUpdates) {
    [ABI46_0_0REANode runPropUpdates:_updateContext];
  }
  if (_operationsInBatch.count != 0) {
    NSMutableArray<ABI46_0_0REANativeAnimationOp> *copiedOperationsQueue = _operationsInBatch;
    _operationsInBatch = [NSMutableArray new];
                       ...
                       trySynchronously:(BOOL)trySync
{
  ...
  [_operationsInBatch addObject:^(ABI46_0_0RCTUIManager *uiManager) {
    [uiManager updateView:ABI46_0_0ReactTag viewName:viewName props:nativeProps];
  }];
}
...
- (void)createNode:(ABI46_0_0REANodeID)nodeID config:(NSDictionary<NSString *, id> *)config
{
  static NSDictionary *map;
  static dispatch_once_t mapToken;
  dispatch_once(&mapToken, ^{
    map = @{
      @"props" : [ABI46_0_0REAPropsNode class],
      @"style" : [ABI46_0_0REAStyleNode class],
      @"transform" : [ABI46_0_0REATransformNode class],
      @"value" : [ABI46_0_0REAValueNode class],
      @"block" : [ABI46_0_0REABlockNode class],
      @"cond" : [ABI46_0_0REACondNode class],
      @"op" : [ABI46_0_0REAOperatorNode class],
      @"set" : [ABI46_0_0REASetNode class],
      @"debug" : [ABI46_0_0READebugNode class],
      @"clock" : [ABI46_0_0REAClockNode class],
      @"clockStart" : [ABI46_0_0REAClockStartNode class],
      @"clockStop" : [ABI46_0_0REAClockStopNode class],
      @"clockTest" : [ABI46_0_0REAClockTestNode class],
      @"call" : [ABI46_0_0REAJSCallNode class],
      @"bezier" : [ABI46_0_0REABezierNode class],
      @"event" : [ABI46_0_0REAEventNode class],
      @"always" : [ABI46_0_0REAAlwaysNode class],
      @"concat" : [ABI46_0_0REAConcatNode class],
      @"param" : [ABI46_0_0REAParamNode class],
      @"func" : [ABI46_0_0REAFunctionNode class],
      @"callfunc" : [ABI46_0_0REACallFuncNode class]
      //            @"listener": nil,
    };
  });
  ...
  ABI46_0_0REANode *node = [[nodeClass alloc] initWithID:nodeID config:config];
  node.nodesManager = self;
  node.updateContext = _updateContext;
  _nodes[nodeID] = node;
}

- (void)dropNode:(ABI46_0_0REANodeID)nodeID
{
  ABI46_0_0REANode *node = _nodes[nodeID];
  if (node) {
    [node onDrop];
...
- (void)connectNodes:(nonnull NSNumber *)parentID childID:(nonnull ABI46_0_0REANodeID)childID
{
  ABI46_0_0RCTAssertParam(parentID);
  ABI46_0_0RCTAssertParam(childID);

  ABI46_0_0REANode *parentNode = _nodes[parentID];
  ABI46_0_0REANode *childNode = _nodes[childID];
...
- (void)disconnectNodes:(ABI46_0_0REANodeID)parentID childID:(ABI46_0_0REANodeID)childID
{
  ABI46_0_0RCTAssertParam(parentID);
  ABI46_0_0RCTAssertParam(childID);
...
{
  ABI46_0_0RCTAssertParam(nodeID);
  ABI46_0_0REANode *node = _nodes[nodeID];
  ABI46_0_0RCTAssertParam(node);

  if ([node isKindOfClass:[ABI46_0_0REAPropsNode class]]) {
    [(ABI46_0_0REAPropsNode *)node connectToView:viewTag viewName:viewName];
  }
}

- (void)disconnectNodeFromView:(ABI46_0_0REANodeID)nodeID viewTag:(NSNumber *)viewTag
{
  ABI46_0_0RCTAssertParam(nodeID);
  ABI46_0_0REANode *node = _nodes[nodeID];
  ABI46_0_0RCTAssertParam(node);
  ...
  if ([node isKindOfClass:[ABI46_0_0REAPropsNode class]]) {
    [(ABI46_0_0REAPropsNode *)node disconnectFromView:viewTag];
  }
}

- (void)attachEvent:(NSNumber *)viewTag eventName:(NSString *)eventName eventNodeID:(ABI46_0_0REANodeID)eventNodeID
{
  ABI46_0_0RCTAssertParam(eventNodeID);
  ABI46_0_0REANode *eventNode = _nodes[eventNodeID];
  ABI46_0_0RCTAssert([eventNode isKindOfClass:[ABI46_0_0REAEventNode class]], @"Event node is of an invalid type");

  NSString *key = [NSString stringWithFormat:@"%@%@", viewTag, ABI46_0_0RCTNormalizeInputEventName(eventName)];
  ABI46_0_0RCTAssert([_eventMapping objectForKey:key] == nil, @"Event handler already set for the given view and event type");
  [_eventMapping setObject:eventNode forKey:key];
}

- (void)detachEvent:(NSNumber *)viewTag eventName:(NSString *)eventName eventNodeID:(ABI46_0_0REANodeID)eventNodeID
{
  NSString *key = [NSString stringWithFormat:@"%@%@", viewTag, ABI46_0_0RCTNormalizeInputEventName(eventName)];
  [_eventMapping removeObjectForKey:key];
}

- (void)processEvent:(id<ABI46_0_0RCTEvent>)event
{
  NSString *key = [NSString stringWithFormat:@"%@%@", event.viewTag, ABI46_0_0RCTNormalizeInputEventName(event.eventName)];
  ABI46_0_0REAEventNode *eventNode = [_eventMapping objectForKey:key];
  [eventNode processEvent:event];
}

- (void)processDirectEvent:(id<ABI46_0_0RCTEvent>)event
{
...
- (BOOL)isDirectEvent:(id<ABI46_0_0RCTEvent>)event
{
  ...
  return [directEventNames containsObject:ABI46_0_0RCTNormalizeInputEventName(event.eventName)];
}
...
- (void)dispatchEvent:(id<ABI46_0_0RCTEvent>)event
{
  ...
  if (_eventHandler != nil) {
    __weak ABI46_0_0REAEventHandler eventHandler = _eventHandler;
    __weak typeof(self) weakSelf = self;
    ABI46_0_0RCTExecuteOnMainQueue(^void() {
      __typeof__(self) strongSelf = weakSelf;
      if (strongSelf == nil) {
        return;
      }
  ...
  ABI46_0_0REANode *eventNode = [_eventMapping objectForKey:key];
...
- (void)setValueForNodeID:(nonnull NSNumber *)nodeID value:(nonnull NSNumber *)newValue
{
  ...
  ABI46_0_0REAValueNode *valueNode = (ABI46_0_0REAValueNode *)node;
...
{
  ABI46_0_0ComponentUpdate *lastSnapshot = _componentUpdateBuffer[viewTag];
  if ([self isNotNativeViewFullyMounted:viewTag] || lastSnapshot != nil) {
    if (lastSnapshot == nil) {
      ABI46_0_0ComponentUpdate *propsSnapshot = [ABI46_0_0ComponentUpdate new];
...
{
  UIView *view = [self.uiManager viewForABI46_0_0ReactTag:viewTag];
  ...
  if ([propName isEqualToString:@"opacity"]) {
    ...
    NSInteger zIndex = view.ABI46_0_0ReactZIndex;
...
- (void)maybeFlushUpdateBuffer
{
  ...
  __weak typeof(self) weakSelf = self;
  [_uiManager addUIBlock:^(__unused ABI46_0_0RCTUIManager *manager, __unused NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    __typeof__(self) strongSelf = weakSelf;
    if (strongSelf == nil) {
      return;
    }
    ...
    for (NSNumber *tag in componentUpdateBuffer) {
      ABI46_0_0ComponentUpdate *componentUpdate = componentUpdateBuffer[tag];
