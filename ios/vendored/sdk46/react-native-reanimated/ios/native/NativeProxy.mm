#import <ABI46_0_0RNReanimated/LayoutAnimationsProxy.h>
...
// COPIED FROM ABI46_0_0RCTTurboModule.mm
...
{
  ABI46_0_0REAModule *reanimatedModule = [bridge moduleForClass:[ABI46_0_0REAModule class]];
  ...
  ABI46_0_0RCTUIManager *uiManager = reanimatedModule.nodesManager.uiManager;
  ...
  id<ABI46_0_0RNGestureHandlerStateManager> gestureHandlerStateManager = nil;
  auto setGestureStateFunction = [gestureHandlerStateManager, bridge](int handlerTag, int newState) mutable {
    if (gestureHandlerStateManager == nil) {
      gestureHandlerStateManager = [bridge moduleForName:@"ABI46_0_0RNGestureHandlerModule"];
    }
  ...
  std::shared_ptr<Scheduler> scheduler = std::make_shared<ABI46_0_0REAIOSScheduler>(jsInvoker);
  std::shared_ptr<ErrorHandler> errorHandler = std::make_shared<ABI46_0_0REAIOSErrorHandler>(scheduler);
  std::shared_ptr<NativeReanimatedModule> module;

  __block std::weak_ptr<Scheduler> weakScheduler = scheduler;
  ((ABI46_0_0REAUIManager *)uiManager).flushUiOperations = ^void() {
  ...
  ABI46_0_0REAUIManager *reaUiManagerNoCast = [bridge moduleForClass:[ABI46_0_0REAUIManager class]];
  ABI46_0_0RCTUIManager *reaUiManager = reaUiManagerNoCast;
  ABI46_0_0REAAnimationsManager *animationsManager = [[ABI46_0_0REAAnimationsManager alloc] initWithUIManager:reaUiManager];
  ...
  // sensors
  ABI46_0_0ReanimatedSensorContainer *reanimatedSensorContainer = [[ABI46_0_0ReanimatedSensorContainer alloc] init];
  auto registerSensorFunction = [=](int sensorType, int interval, std::function<void(double[])> setter) -> int {
    return [reanimatedSensorContainer registerSensor:(ABI46_0_0ReanimatedSensorType)sensorType
                                            interval:interval
                                              setter:^(double *data) {
                                                setter(data);
                                              }];
  ...
  [reanimatedModule.nodesManager registerEventHandler:^(NSString *eventName, id<ABI46_0_0RCTEvent> event) {
