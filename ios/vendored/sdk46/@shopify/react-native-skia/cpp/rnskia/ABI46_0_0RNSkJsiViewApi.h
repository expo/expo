...
using CallbackInfo = struct CallbackInfo {
  ...
  std::shared_ptr<ABI46_0_0RNSkDrawView> view;
};

class ABI46_0_0RNSkJsiViewApi : public JsiHostObject {
public:
  ...
  JSI_HOST_FUNCTION(setDrawMode) {
    ...
    if (info->view != nullptr) {
      ...
      if(nextMode.compare("continuous") == 0) {
        info->view->setDrawingMode(ABI46_0_0RNSkDrawingMode::Continuous);
      } else {
        info->view->setDrawingMode(ABI46_0_0RNSkDrawingMode::Default);
      }
    }
    return jsi::Value::undefined();
  }
  ...
  JSI_HOST_FUNCTION(registerValuesInView) {
    ...
    for(size_t i=0; i<size; ++i) {
      auto value = values.getValueAtIndex(runtime, i).asObject(runtime).asHostObject<ABI46_0_0RNSkReadonlyValue>(runtime);
  ...
  JSI_EXPORT_FUNCTIONS(JSI_EXPORT_FUNC(ABI46_0_0RNSkJsiViewApi, setDrawCallback),
                       JSI_EXPORT_FUNC(ABI46_0_0RNSkJsiViewApi, invalidateSkiaView),
                       JSI_EXPORT_FUNC(ABI46_0_0RNSkJsiViewApi, makeImageSnapshot),
                       JSI_EXPORT_FUNC(ABI46_0_0RNSkJsiViewApi, setDrawMode),
                       JSI_EXPORT_FUNC(ABI46_0_0RNSkJsiViewApi, registerValuesInView))

  /**
   * Constructor
   * @param platformContext Platform context
   */
  ABI46_0_0RNSkJsiViewApi(std::shared_ptr<ABI46_0_0RNSkPlatformContext> platformContext)
