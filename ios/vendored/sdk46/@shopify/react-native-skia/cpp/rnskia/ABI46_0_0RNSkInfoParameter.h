...
enum ABI46_0_0RNSkTouchType { Start, Active, End, Cancelled };

using ABI46_0_0RNSkTouchPoint = struct {
  double x;
  double y;
  double force;
  ABI46_0_0RNSkTouchType type;
  size_t id;
  long timestamp;
};

class ABI46_0_0RNSkInfoObject : public JsiHostObject {
public:
  ...
  JSI_EXPORT_PROPERTY_GETTERS(JSI_EXPORT_PROP_GET(ABI46_0_0RNSkInfoObject, width),
                              JSI_EXPORT_PROP_GET(ABI46_0_0RNSkInfoObject, height),
                              JSI_EXPORT_PROP_GET(ABI46_0_0RNSkInfoObject, timestamp),
                              JSI_EXPORT_PROP_GET(ABI46_0_0RNSkInfoObject, touches))
  ...
  void updateTouches(std::vector<ABI46_0_0RNSkTouchPoint>&& touches) {
    std::lock_guard<std::mutex> lock(_mutex);
    // Add timestamp
  ...
  ABI46_0_0RNSkInfoObject() : JsiHostObject() {}

private:
  int _width;
  int _height;
  double _timestamp;
  std::vector<std::vector<ABI46_0_0RNSkTouchPoint>> _currentTouches;
  std::vector<std::vector<ABI46_0_0RNSkTouchPoint>> _touchesCache;
  std::mutex _mutex;
};
} // namespace ABI46_0_0RNSkia
