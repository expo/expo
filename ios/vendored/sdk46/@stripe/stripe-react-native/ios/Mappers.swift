...
//                ]
    ...
    class func mapToBillingDetails(billingDetails: NSDictionary?) -> STPPaymentMethodBillingDetails? {
        ...
        billing.email = ABI46_0_0RCTConvert.nsString(billingDetails["email"])
        billing.phone = ABI46_0_0RCTConvert.nsString(billingDetails["phone"])
        billing.name = ABI46_0_0RCTConvert.nsString(billingDetails["name"])

        let address = STPPaymentMethodAddress()

        if let addressMap = billingDetails["address"] as? NSDictionary {
            address.city = ABI46_0_0RCTConvert.nsString(addressMap["city"])
            address.postalCode = ABI46_0_0RCTConvert.nsString(addressMap["postalCode"])
            address.country = ABI46_0_0RCTConvert.nsString(addressMap["country"])
            address.line1 = ABI46_0_0RCTConvert.nsString(addressMap["line1"])
            address.line2 = ABI46_0_0RCTConvert.nsString(addressMap["line2"])
            address.state = ABI46_0_0RCTConvert.nsString(addressMap["state"])
        }
