...
class ABI47_0_0RNSkTimingInfo {
public:
  ABI47_0_0RNSkTimingInfo(const std::string &name): _name(std::move(name)) {
    reset();
  }

  ~ABI47_0_0RNSkTimingInfo() {}
  ...
  void stopTiming() {
    ...
    if(_didSkip) {
      _didSkip = false;
      ABI47_0_0RNSkLogger::logToConsole("%s: Skipped frame. Previous frame time: %lldms", _name.c_str(), _lastDuration);
    }
  }
