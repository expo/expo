 ...
 */
class ABI47_0_0RNSkReadonlyValue : public JsiSkHostObject,
                          public std::enable_shared_from_this<ABI47_0_0RNSkReadonlyValue>
{
public:
  ABI47_0_0RNSkReadonlyValue(std::shared_ptr<ABI47_0_0RNSkPlatformContext> platformContext)
      : JsiSkHostObject(platformContext),
      _valueHolder(std::make_unique<JsiSimpleValueWrapper>(*platformContext->getJsRuntime()))
      { }

  virtual ~ABI47_0_0RNSkReadonlyValue() {
    invalidate();
  }
  ...
  JSI_EXPORT_PROPERTY_GETTERS(JSI_EXPORT_PROP_GET(ABI47_0_0RNSkReadonlyValue, __typename__),
                              JSI_EXPORT_PROP_GET(ABI47_0_0RNSkReadonlyValue, current))
  ...
  JSI_HOST_FUNCTION(addListener) {
    ...
    auto unsubscribe = addListener([weakSelf = weak_from_this(),
      ...
      if(self) {
        auto selfReadonlyValue = std::dynamic_pointer_cast<ABI47_0_0RNSkReadonlyValue>(self);
