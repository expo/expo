 ...
 */
class ABI47_0_0RNSkComputedValue : public ABI47_0_0RNSkReadonlyValue
{
public:
  ABI47_0_0RNSkComputedValue(std::shared_ptr<ABI47_0_0RNSkPlatformContext> platformContext,
                    jsi::Runtime &runtime,
  ...
  void invalidate() override {
    ABI47_0_0RNSkReadonlyValue::invalidate();
  ...
  void initializeDependencies(jsi::Runtime &runtime, const jsi::Value *arguments, size_t count) {
    // Save dependencies
    std::vector<std::shared_ptr<ABI47_0_0RNSkReadonlyValue>> dependencies;
    ...
    for(size_t i=0; i<size; ++i) {
      ...
      auto value = dep.asObject(runtime).asHostObject<ABI47_0_0RNSkReadonlyValue>(runtime);
      if(value == nullptr) {
        continue;
      }
    ...
    for(const auto &dep: dependencies) {
      _unsubscribers.push_back(dep->addListener([weakSelf = weak_from_this()](jsi::Runtime& runtime) {
        ...
        if(self) {
          auto selfAsThis = std::dynamic_pointer_cast<ABI47_0_0RNSkComputedValue>(self);
