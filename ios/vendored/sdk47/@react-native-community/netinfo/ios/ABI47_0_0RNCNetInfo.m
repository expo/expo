...
#import "ABI47_0_0RNCNetInfo.h"
#import "ABI47_0_0RNCConnectionStateWatcher.h"
...
@interface ABI47_0_0RNCNetInfo () <ABI47_0_0RNCConnectionStateWatcherDelegate>

@property (nonatomic, strong) ABI47_0_0RNCConnectionStateWatcher *connectionStateWatcher;
@property (nonatomic) BOOL isObserving;
@property (nonatomic) NSDictionary *config;

@end

@implementation ABI47_0_0RNCNetInfo

#pragma mark - Module setup

ABI47_0_0RCT_EXPORT_MODULE()

// We need ABI47_0_0RNCReachabilityCallback's and module methods to be called on the same thread so that we can have
...
- (instancetype)init
{
  self = [super init];
  if (self) {
    _connectionStateWatcher = [[ABI47_0_0RNCConnectionStateWatcher alloc] initWithDelegate:self];
  }
  return self;
}
...
#pragma mark - ABI47_0_0RNCConnectionStateWatcherDelegate
...
ABI47_0_0RCT_EXPORT_METHOD(getCurrentState:(nullable NSString *)requestedInterface resolve:(ABI47_0_0RCTPromiseResolveBlock)resolve
                  reject:(__unused ABI47_0_0RCTPromiseRejectBlock)reject)
{
  ABI47_0_0RNCConnectionState *state = [self.connectionStateWatcher currentState];
  resolve([self currentDictionaryFromUpdateState:state withInterface:requestedInterface]);
}

ABI47_0_0RCT_EXPORT_METHOD(configure:(NSDictionary *)config)
{
    self.config = config;
}
...
- (NSDictionary *)currentDictionaryFromUpdateState:(ABI47_0_0RNCConnectionState *)state withInterface:(nullable NSString *)requestedInterface
{
...
- (NSMutableDictionary *)detailsFromInterface:(nonnull NSString *)requestedInterface withState:(ABI47_0_0RNCConnectionState *)state
{
  NSMutableDictionary *details = [NSMutableDictionary new];
  if ([requestedInterface isEqualToString: ABI47_0_0RNCConnectionTypeCellular]) {
    details[@"cellularGeneration"] = state.cellularGeneration ?: NSNull.null;
    details[@"carrier"] = [self carrier] ?: NSNull.null;
  } else if ([requestedInterface isEqualToString: ABI47_0_0RNCConnectionTypeWifi] || [requestedInterface isEqualToString: ABI47_0_0RNCConnectionTypeEthernet]) {
