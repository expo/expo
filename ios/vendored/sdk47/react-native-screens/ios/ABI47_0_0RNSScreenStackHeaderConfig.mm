...
#import "ABI47_0_0RNSUIBarButtonItem.h"

#ifdef RN_FABRIC_ENABLED
#else
// Some RN private method hacking below. Couldn't figure out better way to access image data
// of a given ABI47_0_0RCTImageView. See more comments in the code section processing SubviewTypeBackButton
@interface ABI47_0_0RCTImageView (Private)
- (UIImage *)image;
@end

@interface ABI47_0_0RCTImageLoader (Private)
- (id<ABI47_0_0RCTImageCache>)imageCache;
@end
#endif

@implementation ABI47_0_0RNSScreenStackHeaderConfig {
  NSMutableArray<ABI47_0_0RNSScreenStackHeaderSubview *> *_ABI47_0_0ReactSubviews;
#ifdef RN_FABRIC_ENABLED
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigProps>();
    _props = defaultProps;
    self.hidden = YES;
    _show = YES;
    _translucent = NO;
...
{
  for (ABI47_0_0RNSScreenStackHeaderSubview *subview in _ABI47_0_0ReactSubviews) {
    if (subview.type == ABI47_0_0RNSScreenStackHeaderSubviewTypeLeft || subview.type == ABI47_0_0RNSScreenStackHeaderSubviewTypeRight) {
      ...
      // we convert the point to ABI47_0_0RNSScreenStackView since it always contains the header inside it
      CGPoint convertedPoint = [_screenView.ABI47_0_0ReactSuperview convertPoint:point toView:headerComponent];
...
- (void)updateViewControllerIfNeeded
{
  ...
  BOOL isInFullScreenModal = nav == nil && _screenView.stackPresentation == ABI47_0_0RNSScreenStackPresentationFullScreenModal;
  ...
  if (vc != nil && (nextVC == vc || isInFullScreenModal || isPresentingVC)) {
    [ABI47_0_0RNSScreenStackHeaderConfig updateViewController:self.screenView.controller withConfig:self animated:YES];
  }
}
...
- (void)layoutNavigationControllerView
{
  ...
  // frame of ABI47_0_0RNSScreen will not be changed and screen content will remain the same size.
...
+ (void)setAnimatedConfig:(UIViewController *)vc withConfig:(ABI47_0_0RNSScreenStackHeaderConfig *)config
{
...
#endif
  {
    ...
    if (config.titleFontFamily || config.titleFontSize || config.titleFontWeight || config.titleColor) {
      ...
      if (family || weight) {
        attrs[NSFontAttributeName] = [ABI47_0_0RCTFont updateFont:nil
                                              withFamily:family
                                                    size:size
                                                  weight:weight
                                                   style:nil
    ...
    if (@available(iOS 11.0, *)) {
      if (config.largeTitle &&
        ...
        if (largeFamily || largeWeight) {
          largeAttrs[NSFontAttributeName] = [ABI47_0_0RCTFont updateFont:nil
...
+ (UIImage *)loadBackButtonImageInViewController:(UIViewController *)vc withConfig:(ABI47_0_0RNSScreenStackHeaderConfig *)config
{
#ifdef RN_FABRIC_ENABLED
  @throw([NSException exceptionWithName:@"UNIMPLEMENTED" reason:@"Implement" userInfo:nil]);
#else
  BOOL hasBackButtonImage = NO;
  for (ABI47_0_0RNSScreenStackHeaderSubview *subview in config.ABI47_0_0ReactSubviews) {
    if (subview.type == ABI47_0_0RNSScreenStackHeaderSubviewTypeBackButton && subview.subviews.count > 0) {
      hasBackButtonImage = YES;
      ABI47_0_0RCTImageView *imageView = subview.subviews[0];
      if (imageView.image == nil) {
        ...
        ABI47_0_0RCTImageSource *source = imageView.imageSources[0];
        [imageView ABI47_0_0ReactSetFrame:CGRectMake(
      ...
      if (image == nil) {
        ...
        ABI47_0_0RCTImageSource *source = imageView.imageSources[0];
        ABI47_0_0RCTImageLoader *imageloader = [subview.bridge moduleForClass:[ABI47_0_0RCTImageLoader class]];
...
+ (void)willShowViewController:(UIViewController *)vc
                    ...
                    withConfig:(ABI47_0_0RNSScreenStackHeaderConfig *)config
{
...
+ (UINavigationBarAppearance *)buildAppearance:(UIViewController *)vc
                                    withConfig:(ABI47_0_0RNSScreenStackHeaderConfig *)config API_AVAILABLE(ios(13.0))
{
                    ...
                    animated:(BOOL)animated
{
  ...
  if (config.backTitle != nil || config.backTitleFontFamily || config.backTitleFontSize ||
    ...
    ABI47_0_0RNSUIBarButtonItem *backBarButtonItem = [[ABI47_0_0RNSUIBarButtonItem alloc] initWithTitle:config.backTitle ?: prevItem.title
  ...
  for (ABI47_0_0RNSScreenStackHeaderSubview *subview in config.ABI47_0_0ReactSubviews) {
    switch (subview.type) {
      case ABI47_0_0RNSScreenStackHeaderSubviewTypeLeft: {
#if !TARGET_OS_TV
      ...
      case ABI47_0_0RNSScreenStackHeaderSubviewTypeRight: {
        UIBarButtonItem *buttonItem = [[UIBarButtonItem alloc] initWithCustomView:subview];
        navitem.rightBarButtonItem = buttonItem;
        break;
      }
      case ABI47_0_0RNSScreenStackHeaderSubviewTypeCenter:
      case ABI47_0_0RNSScreenStackHeaderSubviewTypeTitle: {
        navitem.titleView = subview;
        break;
      }
      case ABI47_0_0RNSScreenStackHeaderSubviewTypeSearchBar: {
        ...
        if ([subview.subviews[0] isKindOfClass:[ABI47_0_0RNSSearchBar class]]) {
#if !TARGET_OS_TV
          if (@available(iOS 11.0, *)) {
            ABI47_0_0RNSSearchBar *searchBar = subview.subviews[0];
      ...
      case ABI47_0_0RNSScreenStackHeaderSubviewTypeBackButton: {
#ifdef RN_FABRIC_ENABLED
        ABI47_0_0RCTLogWarn(@"Back button subview is not yet Fabric compatible in react-native-screens");
#endif
        break;
      }
    }
    ...
    [vc.transitionCoordinator
        ...
        completion:^(id<UIViewControllerTransitionCoordinatorContext> _Nonnull context) {
          if ([context isCancelled]) {
            ...
            for (UIView *subview in fromVC.view.ABI47_0_0ReactSubviews) {
...
- (void)insertABI47_0_0ReactSubview:(ABI47_0_0RNSScreenStackHeaderSubview *)subview atIndex:(NSInteger)atIndex
{
  [_ABI47_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  subview.ABI47_0_0ReactSuperview = self;
}

- (void)removeABI47_0_0ReactSubview:(ABI47_0_0RNSScreenStackHeaderSubview *)subview
{
...
{
  if (![childComponentView isKindOfClass:[ABI47_0_0RNSScreenStackHeaderSubview class]]) {
    ABI47_0_0RCTLogError(@"ScreenStackHeader only accepts children of type ScreenStackHeaderSubview");
    return;
  }
  ...
  //  [_ABI47_0_0ReactSubviews insertObject:(ABI47_0_0RNSScreenStackHeaderSubview *)childComponentView atIndex:index];
  [self insertABI47_0_0ReactSubview:(ABI47_0_0RNSScreenStackHeaderSubview *)childComponentView atIndex:index];
  [self updateViewControllerIfNeeded];
}

- (void)unmountChildComponentView:(UIView<ABI47_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  [_ABI47_0_0ReactSubviews removeObject:(ABI47_0_0RNSScreenStackHeaderSubview *)childComponentView];
...
+ (ABI47_0_0facebook::ABI47_0_0React::ComponentDescriptorProvider)componentDescriptorProvider
{
  return ABI47_0_0facebook::ABI47_0_0React::concreteComponentDescriptorProvider<
      ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigComponentDescriptor>();
}
...
- (UISemanticContentAttribute)getDirectionPropValue:(ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigDirection)direction
{
  switch (direction) {
    case ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigDirection::Rtl:
      return UISemanticContentAttributeForceRightToLeft;
    case ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigDirection::Ltr:
           ...
           oldProps:(ABI47_0_0facebook::ABI47_0_0React::Props::Shared const &)oldProps
{
  ...
  const auto &oldScreenProps =
      *std::static_pointer_cast<const ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigProps>(_props);
  const auto &newScreenProps = *std::static_pointer_cast<const ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigProps>(props);
  ...
  _title = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.title);
  if (newScreenProps.titleFontFamily != oldScreenProps.titleFontFamily) {
    _titleFontFamily = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.titleFontFamily);
  }
  _titleFontWeight = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.titleFontWeight);
  ...
  if (newScreenProps.largeTitleFontFamily != oldScreenProps.largeTitleFontFamily) {
    _largeTitleFontFamily = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.largeTitleFontFamily);
  }
  _largeTitleFontWeight = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.largeTitleFontWeight);
  ...
  _backTitle = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.backTitle);
  if (newScreenProps.backTitleFontFamily != oldScreenProps.backTitleFontFamily) {
    _backTitleFontFamily = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.backTitleFontFamily);
  }
  ...
  _titleColor = ABI47_0_0RCTUIColorFromSharedColor(newScreenProps.titleColor);
  _largeTitleColor = ABI47_0_0RCTUIColorFromSharedColor(newScreenProps.largeTitleColor);
  _color = ABI47_0_0RCTUIColorFromSharedColor(newScreenProps.color);
  _backgroundColor = ABI47_0_0RCTUIColorFromSharedColor(newScreenProps.backgroundColor);
  ...
  _props = std::static_pointer_cast<ABI47_0_0facebook::ABI47_0_0React::ABI47_0_0RNSScreenStackHeaderConfigProps const>(props);
}
...
#ifdef RN_FABRIC_ENABLED
Class<ABI47_0_0RCTComponentViewProtocol> ABI47_0_0RNSScreenStackHeaderConfigCls(void)
{
  return ABI47_0_0RNSScreenStackHeaderConfig.class;
}
#endif

@implementation ABI47_0_0RNSScreenStackHeaderConfigManager

ABI47_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  return [ABI47_0_0RNSScreenStackHeaderConfig new];
}

ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(title, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontFamily, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontSize, NSNumber)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontWeight, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(titleColor, UIColor)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(backTitle, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontFamily, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontSize, NSNumber)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(backgroundColor, UIColor)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(blurEffect, UIBlurEffectStyle)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(color, UIColor)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(direction, UISemanticContentAttribute)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitle, BOOL)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontFamily, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontSize, NSNumber)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontWeight, NSString)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleColor, UIColor)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleBackgroundColor, UIColor)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleHideShadow, BOOL)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(hideBackButton, BOOL)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(hideShadow, BOOL)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(backButtonInCustomView, BOOL)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(disableBackButtonMenu, BOOL)
// `hidden` is an UIView property, we need to use different name internally
ABI47_0_0RCT_REMAP_VIEW_PROPERTY(hidden, hide, BOOL)
ABI47_0_0RCT_EXPORT_VIEW_PROPERTY(translucent, BOOL)

@end

@implementation ABI47_0_0RCTConvert (ABI47_0_0RNSScreenStackHeader)
...
ABI47_0_0RCT_ENUM_CONVERTER(UIBlurEffectStyle, ([self blurEffectsForIOSVersion]), UIBlurEffectStyleExtraLight, integerValue)
