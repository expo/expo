...
std::vector<std::pair<std::string, double>> measure(int viewTag, ABI47_0_0RCTUIManager *uiManager)
{
  UIView *view = [uiManager viewForABI47_0_0ReactTag:@(viewTag)];
...
void scrollTo(int scrollViewTag, ABI47_0_0RCTUIManager *uiManager, double x, double y, bool animated)
{
  UIView *view = [uiManager viewForABI47_0_0ReactTag:@(scrollViewTag)];
  ABI47_0_0RCTScrollView *scrollView = (ABI47_0_0RCTScrollView *)view;
  [scrollView scrollToOffset:(CGPoint){(CGFloat)x, (CGFloat)y} animated:animated];
}

void setGestureState(id<ABI47_0_0RNGestureHandlerStateManager> gestureHandlerStateManager, int handlerTag, int newState)
{
