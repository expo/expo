...
static NSCharacterSet *ABI47_0_0RNSVGTSpan_separators = nil;
static CGFloat ABI47_0_0RNSVGTSpan_radToDeg = 180 / (CGFloat)M_PI;
...
@implementation ABI47_0_0RNSVGTSpan {
  CGFloat startOffset;
  ABI47_0_0RNSVGTextPath *textPath;
  ...
  CGFloat firstX;
  CGFloat firstY;
  ABI47_0_0RNSVGPathMeasure *measure;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI47_0_0RNSVGTSpanProps>();
    _props = defaultProps;
  }
  return self;
}
...
{
  return concreteComponentDescriptorProvider<ABI47_0_0RNSVGTSpanComponentDescriptor>();
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  const auto &newProps = *std::static_pointer_cast<const ABI47_0_0RNSVGTSpanProps>(props);

  self.content = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newProps.content);

  setCommonTextProps(newProps, self);
  _props = std::static_pointer_cast<ABI47_0_0RNSVGTSpanProps const>(props);
}
...
- (void)prepareForRecycle
{
  ...
  measure = [[ABI47_0_0RNSVGPathMeasure alloc] init];
  ABI47_0_0RNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
}
#endif // RN_FABRIC_ENABLED
...
- (id)init
{
  ...
  if (ABI47_0_0RNSVGTSpan_separators == nil) {
    ABI47_0_0RNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
  }
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
  if (self.content) {
    ...
    if (self.inlineSize != nil && self.inlineSize.value != 0) {
      if (self.fill) {
        if (self.fill.class == ABI47_0_0RNSVGBrush.class) {
      ...
      if (self.stroke) {
        if (self.stroke.class == ABI47_0_0RNSVGBrush.class) {
          CGColorRef color = [self.tintColor CGColor];
          [self drawWrappedText:context gc:gc rect:rect color:color];
        } else {
      ...
      if (self.path) {
        ...
        for (NSUInteger i = 0; i < count; i++) {
          ABI47_0_0RNSVGPlatformView *label = [emoji objectAtIndex:i];
...
- (NSMutableDictionary *)getAttributes:(ABI47_0_0RNSVGFontData *)fontdata
{
  ...
  bool allowOptionalLigatures = letterSpacing == 0 && fontdata->fontVariantLigatures == ABI47_0_0RNSVGFontVariantLigaturesNormal;
...
static ABI47_0_0RNSVGTopAlignedLabel *label;
- (void)drawWrappedText:(CGContextRef)context gc:(ABI47_0_0RNSVGGlyphContext *)gc rect:(CGRect)rect color:(CGColorRef)color
{
  ...
  ABI47_0_0RNSVGFontData *fontdata = [gc getFont];
  ...
  enum ABI47_0_0RNSVGTextAnchor textAnchor = fontdata->textAnchor;
  NSTextAlignment align;
  switch (textAnchor) {
    case ABI47_0_0RNSVGTextAnchorStart:
      align = NSTextAlignmentLeft;
      break;

    case ABI47_0_0RNSVGTextAnchorMiddle:
      align = NSTextAlignmentCenter;
      break;

    case ABI47_0_0RNSVGTextAnchorEnd:
  ...
  if (!label) {
    label = [[ABI47_0_0RNSVGTopAlignedLabel alloc] init];
  }
  label.attributedText = (__bridge NSAttributedString *_Nullable)(attrString);
  label.lineBreakMode = NSLineBreakByWordWrapping;
  label.backgroundColor = ABI47_0_0RNSVGColor.clearColor;
...
#endif // TARGET_OS_OSX
  label.font = font;
  label.textColor = [ABI47_0_0RNSVGColor colorWithCGColor:color];

  CGFloat fontSize = [gc getFontSize];
  CGFloat height = CGRectGetHeight(rect);
  CGFloat width = [ABI47_0_0RNSVGPropHelper fromRelative:self.inlineSize relative:[gc getWidth] fontSize:fontSize];
...
- (CGPathRef)getLinePath:(NSString *)str context:(CGContextRef)context
{
  ...
  ABI47_0_0RNSVGGlyphContext *gc = [self.textRoot getGlyphContext];
  ABI47_0_0RNSVGFontData *font = [gc getFont];
   ...
   */
  bool allowOptionalLigatures = letterSpacing == 0 && font->fontVariantLigatures == ABI47_0_0RNSVGFontVariantLigaturesNormal;
   ...
   */
  enum ABI47_0_0RNSVGTextAnchor textAnchor = font->textAnchor;
  ABI47_0_0RNSVGText *anchorRoot = [self getTextAnchorRoot];
  CGFloat textMeasure = [anchorRoot getSubtreeTextChunksTotalAdvance];
  CGFloat offset = [ABI47_0_0RNSVGTSpan getTextAnchorOffset:textAnchor width:textMeasure];
  ...
  if (hasTextPath) {
    // sharpMidLine = ABI47_0_0RNSVGTextPathMidLineFromString([textPath midLine]) == ABI47_0_0RNSVGTextPathMidLineSharp;
    /*
     Name
     side
     Value
     ...
     */
    side = ABI47_0_0RNSVGTextPathSideFromString([textPath side]) == ABI47_0_0RNSVGTextPathSideRight ? -1 : 1;
    /*
     Name
     startOffset
     Value
     ...
     the path is reached.
     */
    CGFloat absoluteStartOffset = [ABI47_0_0RNSVGPropHelper fromRelative:textPath.startOffset
                                                       relative:measure.pathLength
                                                       fontSize:fontSize];
    offset += absoluteStartOffset;
    if (measure.isClosed) {
      CGFloat halfPathDistance = measure.pathLength / 2;
      startOfRendering = absoluteStartOffset + (textAnchor == ABI47_0_0RNSVGTextAnchorMiddle ? -halfPathDistance : 0);
      endOfRendering = startOfRendering + measure.pathLength;
    }
    /*
     ABI47_0_0RNSVGTextPathSpacing spacing = textPath.getSpacing();
     if (spacing == ABI47_0_0RNSVGTextPathSpacing.auto) {
     // Hmm, what to do here?
  ...
  ABI47_0_0RNSVGLength *mTextLength = [self textLength];
  enum ABI47_0_0RNSVGTextLengthAdjust mLengthAdjust = ABI47_0_0RNSVGTextLengthAdjustFromString([self lengthAdjust]);
  if (mTextLength != nil) {
    CGFloat author = [ABI47_0_0RNSVGPropHelper fromRelative:mTextLength relative:[gc getWidth] fontSize:fontSize];
    if (author < 0) {
    ...
    switch (mLengthAdjust) {
      default:
      case ABI47_0_0RNSVGTextLengthAdjustSpacing:
        // TODO account for ligatures
        letterSpacing += (author - textMeasure) / (n - 1);
        break;
      case ABI47_0_0RNSVGTextLengthAdjustSpacingAndGlyphs:
  ...
  */
  ...
  enum ABI47_0_0RNSVGAlignmentBaseline baseline = ABI47_0_0RNSVGAlignmentBaselineFromString(self.alignmentBaseline);
  if (baseline != ABI47_0_0RNSVGAlignmentBaselineBaseline) {
    ...
    switch (baseline) {
      ...
      default:
      case ABI47_0_0RNSVGAlignmentBaselineBaseline:
        // Use the dominant baseline choice of the parent.
        // Match the box’s corresponding baseline to that of its parent.
        baselineShift = 0;
        break;

      case ABI47_0_0RNSVGAlignmentBaselineTextBottom:
      case ABI47_0_0RNSVGAlignmentBaselineAfterEdge:
      case ABI47_0_0RNSVGAlignmentBaselineTextAfterEdge:
      ...
      case ABI47_0_0RNSVGAlignmentBaselineCentral:
      ...
      case ABI47_0_0RNSVGAlignmentBaselineMathematical:
      ...
      case ABI47_0_0RNSVGAlignmentBaselineHanging:
        baselineShift = (CGFloat)0.8 * ascenderHeight;
        break;

      case ABI47_0_0RNSVGAlignmentBaselineTextTop:
      case ABI47_0_0RNSVGAlignmentBaselineBeforeEdge:
      case ABI47_0_0RNSVGAlignmentBaselineTextBeforeEdge:
      ...
      case ABI47_0_0RNSVGAlignmentBaselineBottom:
        // Align the top of the aligned subtree with the top of the line box.
        baselineShift = bottom;
        break;

      case ABI47_0_0RNSVGAlignmentBaselineCenter:
        // Align the center of the aligned subtree with the center of the line box.
        baselineShift = totalHeight / 2;
        break;

      case ABI47_0_0RNSVGAlignmentBaselineTop:
    ...
    switch (baseline) {
      ...
      default:
        if (fontData != nil && [baselineShiftString isEqualToString:@"sub"]) {
          ...
          baselineShift -= [ABI47_0_0RNSVGPropHelper fromRelativeWithNSString:baselineShiftString
                                                            relative:fontSize
                                                            fontSize:fontSize];
        }
        break;
  ...
  for (CFIndex ri = 0; ri < runEnd; ri++) {
    ...
    for (CFIndex g = 0; g < runGlyphCount; g++) {
      ...
      bool isWordSeparator = [ABI47_0_0RNSVGTSpan_separators characterIsMember:currentChar];
      ...
      CGFloat r = [gc nextRotation] / ABI47_0_0RNSVGTSpan_radToDeg;
      ...
      if (width == 0) { // Render unicode emoji
        ABI47_0_0RNSVGTextView *label = [[ABI47_0_0RNSVGTextView alloc] init];
...
+ (CGFloat)getTextAnchorOffset:(ABI47_0_0RNSVGTextAnchor)textAnchor width:(CGFloat)width
{
  switch (textAnchor) {
    case ABI47_0_0RNSVGTextAnchorStart:
      return 0;
...
{
  textPath = nil;
  ABI47_0_0RNSVGText *parent = (ABI47_0_0RNSVGText *)[self superview];
  CGPathRef path = nil;
  while (parent) {
    if ([parent class] == [ABI47_0_0RNSVGTextPath class]) {
      textPath = (ABI47_0_0RNSVGTextPath *)parent;
      ABI47_0_0RNSVGNode *definedTemplate = [self.svgView getDefinedTemplate:textPath.href];
      path = [definedTemplate getPath:context];
      [measure extractPathData:path];
      break;
    } else if (![parent isKindOfClass:[ABI47_0_0RNSVGText class]]) {
      break;
    }
    parent = (ABI47_0_0RNSVGText *)[parent superview];
  }
  if (!path) {
    [measure reset];
  }
...
#ifdef RN_FABRIC_ENABLED
Class<ABI47_0_0RCTComponentViewProtocol> ABI47_0_0RNSVGTSpanCls(void)
{
  return ABI47_0_0RNSVGTSpan.class;
}
#endif // RN_FABRIC_ENABLED
