...
@interface ABI47_0_0RNSVGGlyphContext () {
 @public
  ...
  NSMutableArray<NSArray<ABI47_0_0RNSVGLength *> *> *mXsContext_;
  NSMutableArray<NSArray<ABI47_0_0RNSVGLength *> *> *mYsContext_;
  NSMutableArray<NSArray<ABI47_0_0RNSVGLength *> *> *mDXsContext_;
  NSMutableArray<NSArray<ABI47_0_0RNSVGLength *> *> *mDYsContext_;
  NSMutableArray<NSArray<ABI47_0_0RNSVGLength *> *> *mRsContext_;
  ...
  CGFloat mFontSize_;
  ABI47_0_0RNSVGFontData *topFont_;
  ...
  NSArray<ABI47_0_0RNSVGLength *> *mXs_;

  // https://www.w3.org/TR/SVG/text.html#TSpanElementYAttribute
  NSArray<ABI47_0_0RNSVGLength *> *mYs_;
  ...
  NSArray<ABI47_0_0RNSVGLength *> *mDXs_;

  // https://www.w3.org/TR/SVG/text.html#TSpanElementDYAttribute
  NSArray<ABI47_0_0RNSVGLength *> *mDYs_;
  ...
  NSArray<ABI47_0_0RNSVGLength *> *mRs_;
...
@implementation ABI47_0_0RNSVGGlyphContext
...
- (CTFontRef)getGlyphFont
{
  ...
  NSString *fontStyle = ABI47_0_0RNSVGFontStyleStrings[topFont_->fontStyle];
  NSString *fontWeight = ABI47_0_0RNSVGFontWeightStrings[topFont_->fontWeight];
  UIFont *font = [ABI47_0_0RCTFont updateFont:nil
...
- (instancetype)initWithWidth:(CGFloat)width height:(CGFloat)height
{
  ...
  self->mFontSize_ = ABI47_0_0RNSVGFontData_DEFAULT_FONT_SIZE;
  self->topFont_ = [ABI47_0_0RNSVGFontData Defaults];

  self->mXs_ = [[NSArray alloc] init];
  self->mYs_ = [[NSArray alloc] init];
  self->mDXs_ = [[NSArray alloc] init];
  self->mDYs_ = [[NSArray alloc] init];
  self->mRs_ = [[NSArray alloc] initWithObjects:[ABI47_0_0RNSVGLength lengthWithNumber:0], nil];
...
- (ABI47_0_0RNSVGFontData *)getTopOrParentFont:(ABI47_0_0RNSVGGroup *)child
{
  if (self->mTop_ > 0) {
    return self->topFont_;
  } else {
    ABI47_0_0RNSVGGroup *parentRoot = [child getParentTextRoot];
    ABI47_0_0RNSVGFontData *Defaults = [ABI47_0_0RNSVGFontData Defaults];
    while (parentRoot != nil) {
      ABI47_0_0RNSVGFontData *map = [[parentRoot getGlyphContext] getFont];
      if (map != Defaults) {
        return map;
      }
...
- (void)pushNode:(ABI47_0_0RNSVGGroup *)node andFont:(NSDictionary *)font
{
  ABI47_0_0RNSVGFontData *parent = [self getTopOrParentFont:node];
  self->mTop_++;
  if (font == nil) {
    [self->mFontContext_ addObject:parent];
    return;
  }
  ABI47_0_0RNSVGFontData *data = [ABI47_0_0RNSVGFontData initWithNSDictionary:font parent:parent];
...
- (void)pushContext:(ABI47_0_0RNSVGText *)node
             ...
             rotate:(NSArray<ABI47_0_0RNSVGLength *> *)rotate
{
  [self pushNode:(ABI47_0_0RNSVGGroup *)node andFont:font];
  if (x != nil && [x count] != 0) {
    mXsIndex_++;
    mXIndex_ = -1;
...
{
  [ABI47_0_0RNSVGGlyphContext incrementIndices:mXIndices_ topIndex:mXsIndex_];
  long nextIndex = mXIndex_ + 1;
  if (nextIndex < [mXs_ count]) {
    mDX_ = 0;
    mXIndex_ = nextIndex;
    ABI47_0_0RNSVGLength *length = [mXs_ objectAtIndex:nextIndex];
    mX_ = [ABI47_0_0RNSVGPropHelper fromRelative:length relative:mWidth_ fontSize:mFontSize_];
  }
  mX_ += advance;
  return mX_;
}
...
- (CGFloat)nextY
{
  [ABI47_0_0RNSVGGlyphContext incrementIndices:mYIndices_ topIndex:mYsIndex_];
  long nextIndex = mYIndex_ + 1;
  if (nextIndex < [mYs_ count]) {
    mDY_ = 0;
    mYIndex_ = nextIndex;
    ABI47_0_0RNSVGLength *length = [mYs_ objectAtIndex:nextIndex];
    mY_ = [ABI47_0_0RNSVGPropHelper fromRelative:length relative:mHeight_ fontSize:mFontSize_];
  }
  return mY_;
}
...
- (CGFloat)nextDeltaX
{
  [ABI47_0_0RNSVGGlyphContext incrementIndices:mDXIndices_ topIndex:mDXsIndex_];
  long nextIndex = mDXIndex_ + 1;
  if (nextIndex < [mDXs_ count]) {
    mDXIndex_ = nextIndex;
    ABI47_0_0RNSVGLength *length = [mDXs_ objectAtIndex:nextIndex];
    CGFloat val = [ABI47_0_0RNSVGPropHelper fromRelative:length relative:mWidth_ fontSize:mFontSize_];
    mDX_ += val;
  }
  return mDX_;
}
...
- (CGFloat)nextDeltaY
{
  [ABI47_0_0RNSVGGlyphContext incrementIndices:mDYIndices_ topIndex:mDYsIndex_];
  long nextIndex = mDYIndex_ + 1;
  if (nextIndex < [mDYs_ count]) {
    mDYIndex_ = nextIndex;
    ABI47_0_0RNSVGLength *length = [mDYs_ objectAtIndex:nextIndex];
    CGFloat val = [ABI47_0_0RNSVGPropHelper fromRelative:length relative:mHeight_ fontSize:mFontSize_];
    mDY_ += val;
  }
  return mDY_;
}
...
{
  [ABI47_0_0RNSVGGlyphContext incrementIndices:mRIndices_ topIndex:mRsIndex_];
