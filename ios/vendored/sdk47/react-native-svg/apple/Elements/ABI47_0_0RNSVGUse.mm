...
@implementation ABI47_0_0RNSVGUse
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI47_0_0RNSVGUseProps>();
    _props = defaultProps;
  }
  return self;
}
...
{
  return concreteComponentDescriptorProvider<ABI47_0_0RNSVGUseComponentDescriptor>();
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  const auto &newProps = *std::static_pointer_cast<const ABI47_0_0RNSVGUseProps>(props);
  ...
  if (ABI47_0_0RCTNSStringFromStringNilIfEmpty(newProps.height)) {
    self.useheight = [ABI47_0_0RNSVGLength lengthWithString:ABI47_0_0RCTNSStringFromString(newProps.height)];
  }
  if (ABI47_0_0RCTNSStringFromStringNilIfEmpty(newProps.width)) {
    self.usewidth = [ABI47_0_0RNSVGLength lengthWithString:ABI47_0_0RCTNSStringFromString(newProps.width)];
  }
  self.href = ABI47_0_0RCTNSStringFromStringNilIfEmpty(newProps.href);

  setCommonRenderableProps(newProps, self);
  _props = std::static_pointer_cast<ABI47_0_0RNSVGUseProps const>(props);
}
...
- (void)setUsewidth:(ABI47_0_0RNSVGLength *)usewidth
{
...
- (void)setUseheight:(ABI47_0_0RNSVGLength *)useheight
{
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
  ...
  ABI47_0_0RNSVGNode *definedTemplate = [self.svgView getDefinedTemplate:self.href];
  if (definedTemplate) {
    [self beginTransparencyLayer:context];
    [self clip:context];

    if ([definedTemplate isKindOfClass:[ABI47_0_0RNSVGRenderable class]]) {
      [(ABI47_0_0RNSVGRenderable *)definedTemplate mergeProperties:self];
    }

    if ([definedTemplate class] == [ABI47_0_0RNSVGSymbol class]) {
      ABI47_0_0RNSVGSymbol *symbol = (ABI47_0_0RNSVGSymbol *)definedTemplate;
    ...
    if ([definedTemplate isKindOfClass:[ABI47_0_0RNSVGRenderable class]]) {
      [(ABI47_0_0RNSVGRenderable *)definedTemplate resetProperties];
    }
...
- (ABI47_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  ABI47_0_0RNSVGNode const *definedTemplate = [self.svgView getDefinedTemplate:self.href];
  if (event) {
    self.active = NO;
  } else if (self.active) {
    return self;
  }
  ABI47_0_0RNSVGPlatformView const *hitChild = [definedTemplate hitTest:transformed withEvent:event];
  if (hitChild) {
    self.active = YES;
    return self;
  }
...
#ifdef RN_FABRIC_ENABLED
Class<ABI47_0_0RCTComponentViewProtocol> ABI47_0_0RNSVGUseCls(void)
{
  return ABI47_0_0RNSVGUse.class;
}
#endif // RN_FABRIC_ENABLED
