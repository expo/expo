...
@implementation ABI47_0_0RNSVGRadialGradient
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI47_0_0RNSVGRadialGradientProps>();
    _props = defaultProps;
  }
  return self;
}
...
{
  return concreteComponentDescriptorProvider<ABI47_0_0RNSVGRadialGradientComponentDescriptor>();
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  const auto &newProps = *std::static_pointer_cast<const ABI47_0_0RNSVGRadialGradientProps>(props);

  self.fx = [ABI47_0_0RNSVGLength lengthWithString:ABI47_0_0RCTNSStringFromString(newProps.fx)];
  self.fy = [ABI47_0_0RNSVGLength lengthWithString:ABI47_0_0RCTNSStringFromString(newProps.fy)];
  ...
  _props = std::static_pointer_cast<ABI47_0_0RNSVGRadialGradientProps const>(props);
}
...
- (void)setFx:(ABI47_0_0RNSVGLength *)fx
{
  if ([fx isEqualTo:_fx]) {
    return;
  }
...
- (void)setFy:(ABI47_0_0RNSVGLength *)fy
{
  if ([fy isEqualTo:_fy]) {
    return;
  }
...
{
  self.dirty = false;
  NSArray<ABI47_0_0RNSVGLength *> *points = @[ self.fx, self.fy, self.rx, self.ry, self.cx, self.cy ];
...
#ifdef RN_FABRIC_ENABLED
Class<ABI47_0_0RCTComponentViewProtocol> ABI47_0_0RNSVGRadialGradientCls(void)
{
  return ABI47_0_0RNSVGRadialGradient.class;
}
#endif // RN_FABRIC_ENABLED
