...
#import "ABI47_0_0RNSVGBezierElement.h"
...
@implementation ABI47_0_0RNSVGRenderable {
  NSMutableDictionary *_originProperties;
  NSArray<NSString *> *_lastMergedList;
  NSArray<NSString *> *_attributeList;
  NSArray<ABI47_0_0RNSVGLength *> *_sourceStrokeDashArray;
  CGFloat *_strokeDashArrayData;
  CGPathRef _srcHitPath;
}

static ABI47_0_0RNSVGRenderable *_contextElement;
+ (ABI47_0_0RNSVGRenderable *)contextElement
{
  return _contextElement;
}
+ (void)setContextElement:(ABI47_0_0RNSVGRenderable *)contextElement
{
...
- (id)init
{
  if (self = [super init]) {
    _fillOpacity = 1;
    _strokeOpacity = 1;
    _strokeWidth = [ABI47_0_0RNSVGLength lengthWithNumber:1];
...
- (void)setFill:(ABI47_0_0RNSVGBrush *)fill
{
  if (fill == _fill) {
    return;
  }
...
- (void)setFillRule:(ABI47_0_0RNSVGCGFCRule)fillRule
{
...
- (void)setStroke:(ABI47_0_0RNSVGBrush *)stroke
{
  if (stroke == _stroke) {
    return;
  }
...
- (void)setStrokeWidth:(ABI47_0_0RNSVGLength *)strokeWidth
{
...
- (void)setStrokeDasharray:(NSArray<ABI47_0_0RNSVGLength *> *)strokeDasharray
{
...
- (void)setVectorEffect:(ABI47_0_0RNSVGVectorEffect)vectorEffect
{
...
UInt32 ABI47_0_0saturate(CGFloat value)
{
...
- (void)renderTo:(CGContextRef)context rect:(CGRect)rect
{
  ...
  if (self.mask) {
    ...
    ABI47_0_0RNSVGMask *_maskNode = (ABI47_0_0RNSVGMask *)[self.svgView getDefinedMask:self.mask];
    ...
    for (NSUInteger i = 0; i < npixels; i++) {
      ...
      *currentPixel = ABI47_0_0saturate(luma) << 24;
      currentPixel++;
    }
...
- (void)prepareStrokeDash:(NSUInteger)count strokeDasharray:(NSArray<ABI47_0_0RNSVGLength *> *)strokeDasharray
{
...
{
  ABI47_0_0RNSVGMarker *markerStart = (ABI47_0_0RNSVGMarker *)[self.svgView getDefinedMarker:self.markerStart];
  ABI47_0_0RNSVGMarker *markerMid = (ABI47_0_0RNSVGMarker *)[self.svgView getDefinedMarker:self.markerMid];
  ABI47_0_0RNSVGMarker *markerEnd = (ABI47_0_0RNSVGMarker *)[self.svgView getDefinedMarker:self.markerEnd];
  if (markerStart || markerMid || markerEnd) {
    _contextElement = self;
    NSArray<ABI47_0_0RNSVGMarkerPosition *> *positions = [ABI47_0_0RNSVGMarkerPosition fromCGPath:path];
    ...
    for (ABI47_0_0RNSVGMarkerPosition *position in positions) {
      ABI47_0_0RNSVGMarkerType type = [position type];
      ABI47_0_0RNSVGMarker *marker;
      switch (type) {
        case kStartMarker:
          marker = markerStart;
          break;
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
  ...
  if (self.stroke) {
    ...
    NSArray<ABI47_0_0RNSVGLength *> *strokeDasharray = self.strokeDasharray;
...
- (ABI47_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL canReceiveTouchEvents = (self.pointerEvents != ABI47_0_0RCTPointerEventsNone && ![self isHidden]);
