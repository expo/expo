...
@property (nonatomic, readwrite, weak) ABI47_0_0RNSVGSvgView *svgView;
@property (nonatomic, readwrite, weak) ABI47_0_0RNSVGGroup *textRoot;
@end

@implementation ABI47_0_0RNSVGNode {
  ABI47_0_0RNSVGGlyphContext *glyphContext;
  BOOL _transparent;
  ABI47_0_0RNSVGClipPath *_clipNode;
...
CGFloat const ABI47_0_0RNSVG_M_SQRT1_2l = (CGFloat)0.707106781186547524400844362104849039;
CGFloat const ABI47_0_0RNSVG_DEFAULT_FONT_SIZE = 12;
...
{
  ABI47_0_0RNSVGNode *node = self;
  while (node != nil) {
    ABI47_0_0RNSVGPlatformView *parent = [node superview];

    if (![parent isKindOfClass:[ABI47_0_0RNSVGNode class]]) {
      return;
    }
    node = (ABI47_0_0RNSVGNode *)parent;
    if (!node.path) {
      return;
    }
    [node clearPath];
...
- (ABI47_0_0RNSVGGroup *)textRoot
{
  ...
  ABI47_0_0RNSVGNode *node = self;
  while (node != nil) {
    if ([node isKindOfClass:[ABI47_0_0RNSVGGroup class]] && [((ABI47_0_0RNSVGGroup *)node) getGlyphContext] != nil) {
      _textRoot = (ABI47_0_0RNSVGGroup *)node;
      break;
    }
...
{
  ABI47_0_0RNSVGNode *parent = (ABI47_0_0RNSVGGroup *)[self superview];
  if (![parent isKindOfClass:[ABI47_0_0RNSVGGroup class]]) {
    return nil;
  } else {
...
{
  ABI47_0_0RNSVGGroup *root = self.textRoot;
  if (root == nil) {
    return ABI47_0_0RNSVG_DEFAULT_FONT_SIZE;
  }
...
- (void)setMatrix:(CGAffineTransform)matrix
{
  ...
  if ([container conformsToProtocol:@protocol(ABI47_0_0RNSVGContainer)]) {
    [(id<ABI47_0_0RNSVGContainer>)container invalidate];
  }
}
...
- (void)setClipRule:(ABI47_0_0RNSVGCGFCRule)clipRule
{
...
{
  if (self.clipPath) {
    _clipNode = (ABI47_0_0RNSVGClipPath *)[self.svgView getDefinedClipPath:self.clipPath];
    if (_cachedClipPath) {
      CGPathRelease(_cachedClipPath);
    }
...
- (ABI47_0_0RNSVGSvgView *)svgView
{
  if (_svgView) {
    return _svgView;
  }

  __kindof ABI47_0_0RNSVGPlatformView *parent = self.superview;

  if ([parent class] == [ABI47_0_0RNSVGSvgView class]) {
    _svgView = parent;
  } else if ([parent isKindOfClass:[ABI47_0_0RNSVGNode class]]) {
    _svgView = ((ABI47_0_0RNSVGNode *)parent).svgView;
  } else {
    ABI47_0_0RCTLogError(@"ABI47_0_0RNSVG: %@ should be descendant of a SvgViewShadow.", NSStringFromClass(self.class));
  }
...
{
  return [ABI47_0_0RNSVGPropHelper fromRelativeWithNSString:length
...
{
  ABI47_0_0RNSVGLengthUnitType unit = length.unit;
...
- (CGFloat)fromRelative:(ABI47_0_0RNSVGLength *)length
{
  CGFloat unit;
  switch (length.unit) {
...
- (CGFloat)getContextDiagonal
{
  ...
  CGFloat r = sqrt(powX + powY) * ABI47_0_0RNSVG_M_SQRT1_2l;
  return r;
}
...
- (CGFloat)getCanvasDiagonal
{
  ...
  CGFloat powX = width * width;
  CGFloat powY = height * height;
  canvasDiagonal = sqrt(powX + powY) * ABI47_0_0RNSVG_M_SQRT1_2l;
  return canvasDiagonal;
}
...
- (void)prepareForRecycle
{
  ...
  _pointerEvents = ABI47_0_0RCTPointerEventsUnspecified;
  _responsible = NO;
