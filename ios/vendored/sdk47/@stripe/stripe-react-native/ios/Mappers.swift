...
//                ]
    ...
    class func mapToBillingDetails(billingDetails: NSDictionary?) -> STPPaymentMethodBillingDetails? {
        ...
        billing.email = ABI47_0_0RCTConvert.nsString(billingDetails["email"])
        billing.phone = ABI47_0_0RCTConvert.nsString(billingDetails["phone"])
        billing.name = ABI47_0_0RCTConvert.nsString(billingDetails["name"])

        let address = STPPaymentMethodAddress()

        if let addressMap = billingDetails["address"] as? NSDictionary {
            address.city = ABI47_0_0RCTConvert.nsString(addressMap["city"])
            address.postalCode = ABI47_0_0RCTConvert.nsString(addressMap["postalCode"])
            address.country = ABI47_0_0RCTConvert.nsString(addressMap["country"])
            address.line1 = ABI47_0_0RCTConvert.nsString(addressMap["line1"])
            address.line2 = ABI47_0_0RCTConvert.nsString(addressMap["line2"])
            address.state = ABI47_0_0RCTConvert.nsString(addressMap["state"])
        }
