...
#import "ABI47_0_0RNPanHandler.h"

#import <UIKit/UIGestureRecognizerSubclass.h>

@interface ABI47_0_0RNBetterPanGestureRecognizer : UIPanGestureRecognizer
...
@implementation ABI47_0_0RNBetterPanGestureRecognizer {
...
- (void)activateAfterLongPress
{
  ...
  // Send event in ACTIVE state because UIGestureRecognizerStateBegan is mapped to ABI47_0_0RNGestureHandlerStateBegan
  [_gestureHandler handleGesture:self inState:ABI47_0_0RNGestureHandlerStateActive];
}
...
@implementation ABI47_0_0RNPanGestureHandler

- (instancetype)initWithTag:(NSNumber *)tag
{
  if ((self = [super initWithTag:tag])) {
    _recognizer = [[ABI47_0_0RNBetterPanGestureRecognizer alloc] initWithGestureHandler:self];
  }
  return self;
}
...
{
  [super resetConfig];
  ABI47_0_0RNBetterPanGestureRecognizer *recognizer = (ABI47_0_0RNBetterPanGestureRecognizer *)_recognizer;
...
- (void)configure:(NSDictionary *)config
{
...
#if !TARGET_OS_TV && __IPHONE_OS_VERSION_MAX_ALLOWED >= 130400
  if (@available(iOS 13.4, *)) {
    bool enableTrackpadTwoFingerGesture = [ABI47_0_0RCTConvert BOOL:config[@"enableTrackpadTwoFingerGesture"]];
  ...
  if (prop != nil) {
    CGFloat dist = [ABI47_0_0RCTConvert CGFloat:prop];
    recognizer.minDistSq = dist * dist;
  }
  
  prop = config[@"minVelocity"];
  if (prop != nil) {
    CGFloat velocity = [ABI47_0_0RCTConvert CGFloat:prop];
    recognizer.minVelocitySq = velocity * velocity;
  }
    
  prop = config[@"activateAfterLongPress"];
  if (prop != nil) {
    recognizer.activateAfterLongPress = [ABI47_0_0RCTConvert CGFloat:prop] / 1000.0;
...
{
  ABI47_0_0RNGestureHandlerState savedState = _lastState;
...
- (ABI47_0_0RNGestureHandlerEventExtraData *)eventExtraData:(UIPanGestureRecognizer *)recognizer
{
