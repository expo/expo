...
// ABI43_0_0EXStructuredHeadersTestFixtures.h
const generatedConstantHeaders = `// GENERATED CONSTANTS BEGIN

${Object.keys(tests)
  .map(name => `FOUNDATION_EXPORT NSString * const ABI43_0_0EXStructuredHeaders${name}Tests;`)
  .join('\n')}
...
regexFile(
  ...
  path.join(__dirname, '../ABI43_0_0EXStructuredHeadersTestFixtures.h')
);

// ABI43_0_0EXStructuredHeadersTestFixtures.m
const generatedConstantsImpl = `// GENERATED CONSTANTS BEGIN

${Object.entries(tests)
  .map(
    ([name, json]) =>
      `NSString * const ABI43_0_0EXStructuredHeaders${name}Tests = @${JSON.stringify(JSON.stringify(json))};`
  )
  .join('\n\n')}
...
regexFile(
  ...
  path.join(__dirname, '../ABI43_0_0EXStructuredHeadersTestFixtures.m')
);

// ABI43_0_0EXStructuredHeadersParserTests.m'
...
${Object.keys(tests)
  .map(
    name => `- (void)test${name}
{
  [self runTests:ABI43_0_0EXStructuredHeaders${name}Tests];
}`
  )
  .join('\n\n')}
...
regexFile(
  ...
  generatedXCTests,
  path.join(__dirname, '../ABI43_0_0EXStructuredHeadersParserTests.m')
);
