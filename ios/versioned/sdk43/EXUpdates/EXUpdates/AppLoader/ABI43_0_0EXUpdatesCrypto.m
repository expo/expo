...
static NSString * const ABI43_0_0EXUpdatesCryptoPublicKeyUrl = @"https://exp.host/--/manifest-public-key";
static NSString * const ABI43_0_0EXUpdatesCryptoPublicKeyTag = @"exp.host.publickey";
static NSString * const ABI43_0_0EXUpdatesCryptoPublicKeyFilename = @"manifestPublicKey.pem";

@implementation ABI43_0_0EXUpdatesCrypto
...
{
  if (!data || !signature) {
    errorBlock([NSError errorWithDomain:@"ABI43_0_0EXUpdatesCrypto" code:1001 userInfo:@{ NSLocalizedDescriptionKey: @"Cannot verify the manifest because it is empty or has no signature." }]);
    return;
  }
  ...
  ABI43_0_0EXUpdatesFileDownloader *fileDownloader = [[ABI43_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:config URLSessionConfiguration:configuration];
  [fileDownloader downloadDataFromURL:[NSURL URLWithString:ABI43_0_0EXUpdatesCryptoPublicKeyUrl]
                         extraHeaders:@{}
...
+ (void)verifyWithPublicKey:(NSData *)publicKeyData
               ...
               signedString:(NSString *)signedString
                   callback:(ABI43_0_0EXUpdatesVerifySignatureSuccessBlock)callback
{
  if (!publicKeyData) {
    callback(NO);
  } else {
...
+ (nullable SecKeyRef)keyRefFromPEMData:(NSData *)pemData
{
  ...
  NSData *tag = [NSData dataWithBytes:[ABI43_0_0EXUpdatesCryptoPublicKeyTag UTF8String] length:[ABI43_0_0EXUpdatesCryptoPublicKeyTag length]];
