...
static NSString *const ABI43_0_0EXAppAuthError = @"ERR_APP_AUTH";

@interface ABI43_0_0EXAppAuth ()
...
@property (nonatomic, weak) id<ABI43_0_0EXAppAuthSessionsManagerInterface> sessionsManager;

@end

@implementation ABI43_0_0EXAppAuth

#pragma mark - Expo

ABI43_0_0EX_EXPORT_MODULE(ExpoAppAuth);
...
{
  return @{
           @"OAuthRedirect": ABI43_0_0EXNullIfNil([self _getOAuthRedirect]),
           @"URLSchemes": ABI43_0_0EXNullIfNil([[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleURLTypes"]),
           };
}

ABI43_0_0EX_EXPORT_METHOD_AS(executeAsync,
                             ...
                             reject:(ABI43_0_0EXPromiseRejectBlock)reject
{
  ...
  [ABI43_0_0EXUtilities performSynchronouslyOnMainThread:^{
    ...
    __weak id<ABI43_0_0EXAppAuthSessionsManagerInterface> sessionsManager = self->_sessionsManager;
    OIDAuthStateAuthorizationCallback callback = ^(OIDAuthState *_Nullable authState, NSError *_Nullable error) {
      ...
      if (authState) {
        NSDictionary *tokenResponse = [ABI43_0_0EXAppAuth _tokenResponseNativeToJSON:authState.lastTokenResponse request:options];
        resolve(tokenResponse);
      } else {
        ABI43_0_0EXrejectWithError(reject, error);
      }
    };
...
{
  NSDictionary *tokenResponse = [ABI43_0_0EXAppAuth tokenResponseNativeToJSON:input];
  ...
  [output setValue:ABI43_0_0EXNullIfNil(refreshToken) forKey:@"refreshToken"];
...
- (void)_refreshWithConfiguration:(OIDServiceConfiguration *)configuration
  ...
  OIDTokenCallback callback = ^(OIDTokenResponse *_Nullable response, NSError *_Nullable error) {
    if (response) {
      NSDictionary *tokenResponse = [ABI43_0_0EXAppAuth _tokenResponseNativeToJSON:response request:options];
      resolve(tokenResponse);
    } else {
      ABI43_0_0EXrejectWithError(reject, error);
    }
...
// ABI43_0_0EX prefix for versioning to work smoothly
void ABI43_0_0EXrejectWithError(ABI43_0_0EXPromiseRejectBlock reject, NSError *error) {
  NSString *errorMessage = [NSString stringWithFormat:@"%@: %@", ABI43_0_0EXAppAuthError, error.localizedDescription];
