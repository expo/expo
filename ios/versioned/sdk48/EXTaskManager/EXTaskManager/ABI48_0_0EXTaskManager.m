...
NSString * const ABI48_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI48_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI48_0_0EXConstantsInterface> constantsService;
@property (nonatomic, weak) id<ABI48_0_0EXTaskServiceInterface> taskService;

@end

@implementation ABI48_0_0EXTaskManager

ABI48_0_0EX_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI48_0_0EXTaskManagerInterface)];
}
...
// TODO: Remove when adding bare ABI48_0_0React Native support
...
- (void)setModuleRegistry:(ABI48_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  _constantsService = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXConstantsInterface)];
...
{
  return @{
           @"EVENT_NAME": ABI48_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI48_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI48_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI48_0_0EX_EXPORT_METHOD_AS(isAvailableAsync,
                    isAvailable:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  resolve(@(_taskService != nil));
}

ABI48_0_0EX_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI48_0_0EX_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI48_0_0EX_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI48_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI48_0_0EX_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI48_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  resolve(ABI48_0_0EXNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI48_0_0EX_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI48_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI48_0_0EXPromiseRejectBlock)reject)
{
...
# pragma mark - ABI48_0_0EXTaskManagerInterface
...
{
  return [ABI48_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI48_0_0EXTaskManagerEventName body:body];
  } else {
