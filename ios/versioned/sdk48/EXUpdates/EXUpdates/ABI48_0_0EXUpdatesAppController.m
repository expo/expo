...
static NSString * const ABI48_0_0EXUpdatesAppControllerErrorDomain = @"ABI48_0_0EXUpdatesAppController";

static NSString * const ABI48_0_0EXUpdatesUpdateAvailableEventName = @"updateAvailable";
static NSString * const ABI48_0_0EXUpdatesNoUpdateAvailableEventName = @"noUpdateAvailable";
static NSString * const ABI48_0_0EXUpdatesErrorEventName = @"error";

@interface ABI48_0_0EXUpdatesAppController () <ABI48_0_0EXUpdatesErrorRecoveryDelegate>

@property (nonatomic, readwrite, strong) ABI48_0_0EXUpdatesConfig *config;
@property (nonatomic, readwrite, strong, nullable) id<ABI48_0_0EXUpdatesAppLauncher> launcher;
@property (nonatomic, readwrite, strong) ABI48_0_0EXUpdatesDatabase *database;
@property (nonatomic, readwrite, strong) ABI48_0_0EXUpdatesSelectionPolicy *selectionPolicy;
@property (nonatomic, readwrite, strong) ABI48_0_0EXUpdatesSelectionPolicy *defaultSelectionPolicy;
@property (nonatomic, readwrite, strong) ABI48_0_0EXUpdatesErrorRecovery *errorRecovery;
...
@property (nonatomic, strong) ABI48_0_0EXUpdatesAppLoaderTask *loaderTask;
...
@property (nonatomic, readwrite, assign) ABI48_0_0EXUpdatesRemoteLoadStatus remoteLoadStatus;
 ...
 * ExpoUpdatesReactDelegateHandler. It delegates to an instance of ABI48_0_0EXUpdatesAppLoaderTask to start
 ...
 * used by the exported ABI48_0_0EXUpdatesModule through ABI48_0_0EXUpdatesService. Such information includes
 * references to: the database, the ABI48_0_0EXUpdatesConfig object, the path on disk to the updates
 * directory, any currently active ABI48_0_0EXUpdatesAppLoaderTask, the current ABI48_0_0EXUpdatesSelectionPolicy, the
 ...
 */
@implementation ABI48_0_0EXUpdatesAppController

+ (instancetype)sharedInstance
{
  static ABI48_0_0EXUpdatesAppController *theController;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!theController) {
      theController = [[ABI48_0_0EXUpdatesAppController alloc] init];
    }
  });
...
{
  if (self = [super init]) {
    _config = [ABI48_0_0EXUpdatesConfig configWithExpoPlist];
    _database = [[ABI48_0_0EXUpdatesDatabase alloc] init];
    _defaultSelectionPolicy = [ABI48_0_0EXUpdatesSelectionPolicyFactory filterAwarePolicyWithRuntimeVersion:[ABI48_0_0EXUpdatesUtils getRuntimeVersionWithConfig:_config]];
    _errorRecovery = [ABI48_0_0EXUpdatesErrorRecovery new];
    ...
    _isStarted = NO;
    _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusIdle;
    _logger = [ABI48_0_0EXUpdatesLogger new];
    [_logger info:@"ABI48_0_0EXUpdatesAppController sharedInstance created"];
  }
  return self;
}
...
- (void)setConfiguration:(NSDictionary *)configuration
{
  if (_isStarted) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"ABI48_0_0EXUpdatesAppController:setConfiguration should not be called after start"
                                 userInfo:@{}];
  }
...
- (void)start
{
  NSAssert(!_isStarted, @"ABI48_0_0EXUpdatesAppController:start should only be called once per instance");

  if (!_config.isEnabled) {
    ABI48_0_0EXUpdatesAppLauncherNoDatabase *launcher = [[ABI48_0_0EXUpdatesAppLauncherNoDatabase alloc] init];
    _launcher = launcher;
  ...
  if (!_config.updateUrl) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"expo-updates is enabled, but no valid URL is configured under ABI48_0_0EXUpdatesURL. If you are making a release build for the first time, make sure you have run `expo publish` at least once."
                                 userInfo:@{}];
  }
  if (!_config.scopeKey) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
  ...
  [ABI48_0_0EXUpdatesBuildData ensureBuildDataIsConsistentAsync:_database config:_config];

  [_errorRecovery startMonitoring];

  _loaderTask = [[ABI48_0_0EXUpdatesAppLoaderTask alloc] initWithConfig:_config
                                                      database:_database
...
{
  ABI48_0_0EXUpdatesAppLauncherWithDatabase *launcher = [[ABI48_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:_config database:_database directory:_updatesDirectory completionQueue:_controllerQueue];
  ...
  [launcher launchUpdateWithSelectionPolicy:self.selectionPolicy completion:^(NSError * _Nullable error, BOOL success) {
    if (success) {
      ...
      ABI48_0_0RCTReloadCommandSetBundleURL(launcher.launchAssetUrl);
      ABI48_0_0RCTTriggerReloadCommandListeners(@"Requested by JavaScript - Updates.reloadAsync()");
      [self runReaper];
    } else {
...
# pragma mark - ABI48_0_0EXUpdatesAppLoaderTaskDelegate

- (BOOL)appLoaderTask:(ABI48_0_0EXUpdatesAppLoaderTask *)appLoaderTask didLoadCachedUpdate:(nonnull ABI48_0_0EXUpdatesUpdate *)update
{
  return YES;
}
...
{
  [_logger info:@"ABI48_0_0EXUpdatesAppController appLoaderTask didStartLoadingUpdate"
           code:ABI48_0_0EXUpdatesErrorCodeNone
       updateId:update.loggingId
        assetId:nil];
  _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusLoading;
}
...
{
  NSString *logMessage = [NSString stringWithFormat:@"ABI48_0_0EXUpdatesAppController appLoaderTask didFinishWithLauncher, isUpToDate=%d, remoteLoadStatus=%ld", isUpToDate, _remoteLoadStatus];
  ...
  if (_remoteLoadStatus == ABI48_0_0EXUpdatesRemoteLoadStatusLoading && isUpToDate) {
    _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusIdle;
  }
  _launcher = launcher;
  if (self->_delegate) {
    [ABI48_0_0EXUpdatesUtils runBlockOnMainThread:^{
...
{
  NSString *logMessage = [NSString stringWithFormat:@"ABI48_0_0EXUpdatesAppController appLoaderTask didFinishWithError: %@", error.localizedDescription];
...
{
  if (status == ABI48_0_0EXUpdatesBackgroundUpdateStatusError) {
    _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusIdle;
    NSAssert(error != nil, @"Background update with error status must have a nonnull error object");
    [_logger error:@"ABI48_0_0EXUpdatesAppController appLoaderTask didFinishBackgroundUpdateWithStatus=Error"
              code:ABI48_0_0EXUpdatesErrorCodeNone
          updateId:update.loggingId
           assetId:nil];
    [ABI48_0_0EXUpdatesUtils sendEventToBridge:_bridge withType:ABI48_0_0EXUpdatesErrorEventName body:@{@"message": error.localizedDescription}];
  } else if (status == ABI48_0_0EXUpdatesBackgroundUpdateStatusUpdateAvailable) {
    _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded;
    NSAssert(update != nil, @"Background update with error status must have a nonnull update object");
    [_logger info:@"ABI48_0_0EXUpdatesAppController appLoaderTask didFinishBackgroundUpdateWithStatus=NewUpdateLoaded"
             code:ABI48_0_0EXUpdatesErrorCodeNone
         updateId:update.loggingId
          assetId:nil];
    [ABI48_0_0EXUpdatesUtils sendEventToBridge:_bridge withType:ABI48_0_0EXUpdatesUpdateAvailableEventName body:@{@"manifest": update.manifest.rawManifestJSON}];
  } else if (status == ABI48_0_0EXUpdatesBackgroundUpdateStatusNoUpdateAvailable) {
    _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusIdle;
    [_logger error:@"ABI48_0_0EXUpdatesAppController appLoaderTask didFinishBackgroundUpdateWithStatus=NoUpdateAvailable"
              code:ABI48_0_0EXUpdatesErrorCodeNoUpdatesAvailable
          updateId:update.loggingId
           assetId:nil];
    [ABI48_0_0EXUpdatesUtils sendEventToBridge:_bridge withType:ABI48_0_0EXUpdatesNoUpdateAvailableEventName body:@{}];
  }
  [_errorRecovery notifyNewRemoteLoadStatus:_remoteLoadStatus];
}

# pragma mark - ABI48_0_0EXUpdatesAppController+Internal

- (BOOL)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  _updatesDirectory = [ABI48_0_0EXUpdatesUtils initializeUpdatesDirectoryWithError:error];
...
{
  [ABI48_0_0EXUpdatesUtils purgeUpdatesLogsOlderThanOneDay];
}
...
# pragma mark - ABI48_0_0EXUpdatesErrorRecoveryDelegate
...
- (void)relaunchWithCompletion:(ABI48_0_0EXUpdatesAppLauncherCompletionBlock)completion
{
  ...
  [launcher launchUpdateWithSelectionPolicy:self.selectionPolicy completion:^(NSError * _Nullable error, BOOL success) {
    if (success) {
      ...
      [self->_errorRecovery startMonitoring];
      ABI48_0_0RCTReloadCommandSetBundleURL(launcher.launchAssetUrl);
      ABI48_0_0RCTTriggerReloadCommandListeners(@"Relaunch after fatal error");
      completion(nil, YES);
    } else {
      completion(error, NO);
    }
...
- (void)loadRemoteUpdate
{
  ...
  _remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusLoading;
  ABI48_0_0EXUpdatesAppLoader *remoteAppLoader = [[ABI48_0_0EXUpdatesRemoteAppLoader alloc] initWithConfig:_config database:_database directory:_updatesDirectory launchedUpdate:self.launchedUpdate completionQueue:_controllerQueue];
  [remoteAppLoader loadUpdateFromUrl:_config.updateUrl onManifest:^BOOL(ABI48_0_0EXUpdatesUpdate *update) {
    ...
    self->_remoteLoadStatus = update ? ABI48_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded : ABI48_0_0EXUpdatesRemoteLoadStatusIdle;
    [self->_errorRecovery notifyNewRemoteLoadStatus:self->_remoteLoadStatus];
  } error:^(NSError *error) {
    [self->_logger error:[NSString stringWithFormat:@"ABI48_0_0EXUpdatesAppController loadRemoteUpdate error: %@", error.localizedDescription]
              code:ABI48_0_0EXUpdatesErrorCodeUpdateFailedToLoad];
    self->_remoteLoadStatus = ABI48_0_0EXUpdatesRemoteLoadStatusIdle;
    [self->_errorRecovery notifyNewRemoteLoadStatus:self->_remoteLoadStatus];
  }];
}
...
- (void)markFailedLaunchForLaunchedUpdate
{
  ...
  dispatch_async(_database.databaseQueue, ^{
    ABI48_0_0EXUpdatesUpdate *launchedUpdate = self.launchedUpdate;
    if (!launchedUpdate) {
      return;
    }
    [self->_logger error:@"ABI48_0_0EXUpdatesAppController markFailedLaunchForUpdate"
                    code:ABI48_0_0EXUpdatesErrorCodeUnknown
