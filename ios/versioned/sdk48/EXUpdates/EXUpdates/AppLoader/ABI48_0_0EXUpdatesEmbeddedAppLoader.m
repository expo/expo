...
NSString * const ABI48_0_0EXUpdatesEmbeddedManifestName = @"app";
NSString * const ABI48_0_0EXUpdatesEmbeddedManifestType = @"manifest";
NSString * const ABI48_0_0EXUpdatesEmbeddedBundleFilename = @"app";
NSString * const ABI48_0_0EXUpdatesEmbeddedBundleFileType = @"bundle";
NSString * const ABI48_0_0EXUpdatesBareEmbeddedBundleFilename = @"main";
NSString * const ABI48_0_0EXUpdatesBareEmbeddedBundleFileType = @"jsbundle";

static NSString * const ABI48_0_0EXUpdatesEmbeddedAppLoaderErrorDomain = @"ABI48_0_0EXUpdatesEmbeddedAppLoader";

/**
 * Subclass of ABI48_0_0EXUpdatesAppLoader which handles copying the embedded update's assets into the
 ...
 */
@implementation ABI48_0_0EXUpdatesEmbeddedAppLoader
...
{
  static ABI48_0_0EXUpdatesUpdate *embeddedManifest;
  ...
  dispatch_once(&once, ^{
    if (!config.hasEmbeddedUpdate) {
      ...
      NSURL *bundleUrl = [frameworkBundle.resourceURL URLByAppendingPathComponent:@"ABI48_0_0EXUpdates.bundle"];
      NSBundle *bundle = [NSBundle bundleWithURL:bundleUrl];
      NSString *path = [bundle pathForResource:ABI48_0_0EXUpdatesEmbeddedManifestName ofType:ABI48_0_0EXUpdatesEmbeddedManifestType];
      NSData *manifestData = [NSData dataWithContentsOfFile:path];

      // Fallback to main bundle if the embedded manifest is not found in ABI48_0_0EXUpdates.bundle. This is a special case
      // to support the existing structure of Expo "shell apps"
      if (!manifestData) {
        path = [[NSBundle mainBundle] pathForResource:ABI48_0_0EXUpdatesEmbeddedManifestName ofType:ABI48_0_0EXUpdatesEmbeddedManifestType];
        manifestData = [NSData dataWithContentsOfFile:path];
      }

      // Not found in ABI48_0_0EXUpdates.bundle or main bundle
      if (!manifestData) {
      ...
      if (!manifest) {
        ...
        embeddedManifest = [ABI48_0_0EXUpdatesUpdate updateWithEmbeddedManifest:[mutableManifest copy]
                                                                config:config
                                                              database:database];
...
{
  ABI48_0_0EXUpdatesUpdate *embeddedManifest = [[self class] embeddedManifestWithConfig:self.config
  ...
  if (embeddedManifest) {
    ...
    error([NSError errorWithDomain:ABI48_0_0EXUpdatesEmbeddedAppLoaderErrorDomain
                              code:1008
...
- (void)downloadAsset:(ABI48_0_0EXUpdatesAsset *)asset
{
  ...
  dispatch_async([ABI48_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
    if ([[NSFileManager defaultManager] fileExistsAtPath:[destinationUrl path]]) {
      ...
      NSString *bundlePath = [ABI48_0_0EXUpdatesUtils pathForBundledAsset:asset];
...
- (void)loadUpdateFromUrl:(NSURL *)url
               onManifest:(ABI48_0_0EXUpdatesAppLoaderManifestBlock)manifestBlock
                  ...
                  success:(ABI48_0_0EXUpdatesAppLoaderSuccessBlock)success
                    error:(ABI48_0_0EXUpdatesAppLoaderErrorBlock)error
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call ABI48_0_0EXUpdatesEmbeddedAppLoader#loadUpdateFromUrl" userInfo:nil];
}
