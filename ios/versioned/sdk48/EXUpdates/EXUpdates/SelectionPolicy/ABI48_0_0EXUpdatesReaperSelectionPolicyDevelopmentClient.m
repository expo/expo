...
static NSUInteger const ABI48_0_0EXUpdatesReaperSelectionPolicyDefaultMaxUpdates = 10;

@interface ABI48_0_0EXUpdatesReaperSelectionPolicyDevelopmentClient ()

@property (nonatomic, assign) NSUInteger maxUpdatesToKeep;

@end

@implementation ABI48_0_0EXUpdatesReaperSelectionPolicyDevelopmentClient

- (instancetype)init
{
  return [self initWithMaxUpdatesToKeep:ABI48_0_0EXUpdatesReaperSelectionPolicyDefaultMaxUpdates];
}
  ...
  if (self = [super init]) {
    if (maxUpdatesToKeep <= 0) {
      @throw [NSException exceptionWithName:NSInvalidArgumentException
                                     reason:@"Cannot initiailize ABI48_0_0EXUpdatesReaperSelectionPolicy with maxUpdatesToKeep <= 0"
                                   userInfo:nil];
    }
...
- (NSArray<ABI48_0_0EXUpdatesUpdate *> *)updatesToDeleteWithLaunchedUpdate:(ABI48_0_0EXUpdatesUpdate *)launchedUpdate updates:(NSArray<ABI48_0_0EXUpdatesUpdate *> *)updates filters:(nullable NSDictionary *)filters
{
  ...
  NSMutableArray<ABI48_0_0EXUpdatesUpdate *> *updatesMutable = [updates sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
    ABI48_0_0EXUpdatesUpdate *update1 = (ABI48_0_0EXUpdatesUpdate *)obj1;
    ABI48_0_0EXUpdatesUpdate *update2 = (ABI48_0_0EXUpdatesUpdate *)obj2;
  ...
  while (updatesMutable.count > _maxUpdatesToKeep) {
    ABI48_0_0EXUpdatesUpdate *oldest = updatesMutable[0];
    [updatesMutable removeObjectAtIndex:0];
    if ([launchedUpdate.updateId isEqual:oldest.updateId]) {
      if (hasFoundLaunchedUpdate) {
        // avoid infinite loop
        @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Multiple updates with the same ID were passed into ABI48_0_0EXUpdatesReaperSelectionPolicyDevelopmentClient" userInfo:nil];
      }
