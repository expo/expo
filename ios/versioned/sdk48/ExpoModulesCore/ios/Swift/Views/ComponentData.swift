...
/**
 Custom component data extending `ABI48_0_0RCTComponentData`. Its main purpose is to handle event-based props (callbacks),
 but it also simplifies capturing the view config so we can omit some reflections that ABI48_0_0React Native executes.
 */
@objc(ABI48_0_0EXComponentData)
public final class ComponentData: ABI48_0_0RCTComponentData {
  /**
   ...
   */
  @objc
  public init(viewModule: ViewModuleWrapper, managerClass: ViewModuleWrapper.Type, bridge: ABI48_0_0RCTBridge) {
  ...
  // MARK: ABI48_0_0RCTComponentData

  /**
   Creates a setter for the specific prop. For non-event props we just let ABI48_0_0React Native do its job.
   Events are handled differently to conveniently use them in Swift.
   */
  public override func createPropBlock(_ propName: String, isShadowView: Bool) -> ABI48_0_0RCTPropBlockAlias {
  ...
  /**
   The base `ABI48_0_0RCTComponentData` class does some Objective-C dynamic calls in this function, but we don't
  ...
  public override func viewConfig() -> [String: Any] {
    ...
    if let eventNames = moduleHolder?.viewManager?.eventNames {
      for eventName in eventNames {
        directEvents.append(ABI48_0_0RCTNormalizeInputEventName(eventName))
 ...
 */
private func createEventSetter(eventName: String, bridge: ABI48_0_0RCTBridge?) -> ABI48_0_0RCTPropBlockAlias {
  return { [weak bridge] (target: ABI48_0_0RCTComponent, value: Any) in
