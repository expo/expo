...
@interface ABI48_0_0EXModuleRegistry ()

@property (nonatomic, weak) id<ABI48_0_0EXModuleRegistryDelegate> delegate;

@property NSMutableSet<id<ABI48_0_0EXInternalModule>> *internalModulesSet;
@property NSMapTable<Protocol *, id<ABI48_0_0EXInternalModule>> *internalModules;
@property NSMapTable<Protocol *, NSMutableArray<id<ABI48_0_0EXInternalModule>> *> *internalModulesPreResolution;
@property NSMapTable<Class, ABI48_0_0EXExportedModule *> *exportedModulesByClass;
@property NSMutableDictionary<const NSString *, ABI48_0_0EXExportedModule *> *exportedModules;
@property NSMutableDictionary<const NSString *, ABI48_0_0EXViewManager *> *viewManagerModules;
...
@implementation ABI48_0_0EXModuleRegistry
...
- (instancetype)initWithInternalModules:(NSSet<id<ABI48_0_0EXInternalModule>> *)internalModules
                        exportedModules:(NSSet<ABI48_0_0EXExportedModule *> *)exportedModules
                           viewManagers:(NSSet<ABI48_0_0EXViewManager *> *)viewManagers
                       singletonModules:(NSSet *)singletonModules
{
  if (self = [self init]) {
    for (id<ABI48_0_0EXInternalModule> internalModule in internalModules) {
      [self registerInternalModule:internalModule];
    }
    
    for (ABI48_0_0EXExportedModule *exportedModule in exportedModules) {
      [self registerExportedModule:exportedModule];
    }

    for (ABI48_0_0EXViewManager *viewManager in viewManagers) {
...
- (void)setDelegate:(id<ABI48_0_0EXModuleRegistryDelegate>)delegate
{
  _delegate = delegate;
}
...
- (void)resolveInternalModulesConflicts
{
  ...
  for (Protocol *protocol in _internalModulesPreResolution) {
    NSArray<id<ABI48_0_0EXInternalModule>> *conflictingModules = [_internalModulesPreResolution objectForKey:protocol];

    id<ABI48_0_0EXInternalModule> resolvedModule;
...
- (void)initialize
{
  ...
  for (id<ABI48_0_0EXModuleRegistryConsumer> registryConsumer in _registryConsumers) {
...
- (void)registerInternalModule:(id<ABI48_0_0EXInternalModule>)internalModule
{
  for (Protocol *exportedInterface in [[internalModule class] exportedInterfaces]) {
    if (_internalModules) {
      id<ABI48_0_0EXInternalModule> resolvedModule = internalModule;
      if (_delegate && [_internalModules objectForKey:exportedInterface]) {
        id<ABI48_0_0EXInternalModule> oldModule = [_internalModules objectForKey:exportedInterface];
...
- (void)registerExportedModule:(ABI48_0_0EXExportedModule *)exportedModule
{
  const NSString *exportedModuleName = [[exportedModule class] exportedModuleName];
  if (_exportedModules[exportedModuleName]) {
    ABI48_0_0EXLogInfo(@"Module %@ overrides %@ as the module exported as %@.", exportedModule, _exportedModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerViewManager:(ABI48_0_0EXViewManager *)viewManager
{
  const NSString *exportedModuleName = [[viewManager class] exportedModuleName];
  if (_viewManagerModules[exportedModuleName]) {
    ABI48_0_0EXLogInfo(@"View manager %@ overrides %@ as the module exported as %@.", viewManager, _viewManagerModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerSingletonModule:(id)singletonModule
{
  if ([[singletonModule class] respondsToSelector:@selector(name)]) {
    ...
    ABI48_0_0EXLogWarn(@"One of the singleton modules does not respond to +(NSString *)name selector. This probably means you're either try to pass a strange object as a singleton module (it won't get registered in the module registry, sorry) or the ABI48_0_0EXSingletonModule interface and the ABI48_0_0EXModuleRegistry implementations versions are out of sync, which means things will probably not work as expected.");
  }
}

- (void)maybeAddRegistryConsumer:(id)maybeConsumer
{
  if ([maybeConsumer conformsToProtocol:@protocol(ABI48_0_0EXModuleRegistryConsumer)]) {
    [self addRegistryConsumer:(id<ABI48_0_0EXModuleRegistryConsumer>)maybeConsumer];
  }
}

- (void)addRegistryConsumer:(id<ABI48_0_0EXModuleRegistryConsumer>)registryConsumer
{
...
- (ABI48_0_0EXExportedModule *)getExportedModuleForName:(NSString *)name
{
  return _exportedModules[name];
}

- (ABI48_0_0EXExportedModule *)getExportedModuleOfClass:(Class)moduleClass
{
...
- (NSArray<id<ABI48_0_0EXInternalModule>> *)getAllInternalModules
{
  [self resolveInternalModulesConflicts];
  return [_internalModulesSet allObjects];
}

- (NSArray<ABI48_0_0EXExportedModule *> *)getAllExportedModules
{
  return [_exportedModules allValues];
}

- (NSArray<ABI48_0_0EXViewManager *> *)getAllViewManagers
{
