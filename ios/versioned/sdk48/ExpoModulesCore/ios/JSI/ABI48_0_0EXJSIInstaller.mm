...
@interface ABI48_0_0RCTBridge (ExpoBridgeWithRuntime)
...
@implementation ABI48_0_0EXJavaScriptRuntimeManager

+ (nullable ABI48_0_0EXJavaScriptRuntime *)runtimeFromBridge:(nonnull ABI48_0_0RCTBridge *)bridge
{
  jsi::Runtime *jsiRuntime = [bridge respondsToSelector:@selector(runtime)] ? reinterpret_cast<jsi::Runtime *>(bridge.runtime) : nullptr;
  return jsiRuntime ? [[ABI48_0_0EXJavaScriptRuntime alloc] initWithRuntime:jsiRuntime callInvoker:bridge.jsCallInvoker] : nil;
}

+ (BOOL)installExpoModulesHostObject:(nonnull ABI48_0_0EXAppContext *)appContext
{
  ABI48_0_0EXJavaScriptRuntime *runtime = [appContext runtime];
  ...
  ABI48_0_0EXJavaScriptObject *global = [runtime global];
  ABI48_0_0EXJavaScriptObject *mainObject = [runtime mainObject];

  if ([mainObject hasProperty:modulesHostObjectPropertyName]) {
    return false;
  }

  std::shared_ptr<ABI48_0_0expo::ExpoModulesHostObject> modulesHostObjectPtr = std::make_shared<ABI48_0_0expo::ExpoModulesHostObject>(appContext);
  ABI48_0_0EXJavaScriptObject *modulesHostObject = [runtime createHostObject:modulesHostObjectPtr];
  ...
  [mainObject defineProperty:modulesHostObjectPropertyName
                     ...
                     options:ABI48_0_0EXJavaScriptObjectPropertyDescriptorEnumerable];
