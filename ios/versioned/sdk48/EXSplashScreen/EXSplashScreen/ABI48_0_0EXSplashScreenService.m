...
#import <ABI48_0_0EXSplashScreen/ABI48_0_0EXSplashScreenViewNativeProvider.h>
...
@interface ABI48_0_0EXSplashScreenService ()

@property (nonatomic, strong) NSMapTable<UIViewController *, ABI48_0_0EXSplashScreenViewController *> *splashScreenControllers;
/**
...
@implementation ABI48_0_0EXSplashScreenService

ABI48_0_0EX_REGISTER_SINGLETON_MODULE(SplashScreen);
...
{
  id<ABI48_0_0EXSplashScreenViewProvider> splashScreenViewProvider = [ABI48_0_0EXSplashScreenViewNativeProvider new];
  return [self showSplashScreenFor:viewController
                           options:options
          splashScreenViewProvider:splashScreenViewProvider
                   successCallback:^{}
                   failureCallback:^(NSString *message){ ABI48_0_0EXLogWarn(@"%@", message); }];
}
...
{
  if ((options & ABI48_0_0EXSplashScreenForceShow) == 0 && [self.splashScreenControllers objectForKey:viewController]) {
  ...
  ABI48_0_0EXSplashScreenViewController *splashScreenController = [[ABI48_0_0EXSplashScreenViewController alloc] initWithRootView:rootView
...
- (void)onAppContentDidAppear:(UIViewController *)viewController
{
  if ([self isAppActive] && ![self.splashScreenControllers objectForKey:viewController]) {
    ABI48_0_0EXLogWarn(@"No native splash screen registered for given view controller. Call 'SplashScreen.show' for given view controller first.");
  }
...
- (void)onAppContentWillReload:(UIViewController *)viewController
{
  ...
  if (needsShow) {
    // For reloading apps, specify `ABI48_0_0EXSplashScreenForceShow` to show splash screen again
    [self showSplashScreenFor:viewController
                      options:ABI48_0_0EXSplashScreenForceShow
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
  ...
  if (rootViewController) {
    [self showSplashScreenFor:rootViewController options:ABI48_0_0EXSplashScreenDefault];
  }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
  if (object == UIApplication.sharedApplication.keyWindow && [keyPath isEqualToString:kRootViewController]) {
    ...
    if (newRootViewController != nil && newRootViewController != self.observingRootViewController) {
      [self removeRootViewControllerListener];
      [self showSplashScreenFor:newRootViewController options:ABI48_0_0EXSplashScreenDefault];
      [self addRootViewControllerListener];
    }
  }
  if (object == UIApplication.sharedApplication.keyWindow.rootViewController && [keyPath isEqualToString:kView]) {
    ...
    if (newView != nil && [newView.nextResponder isKindOfClass:[UIViewController class]]) {
      ...
      [self hideSplashScreenFor:viewController options:ABI48_0_0EXSplashScreenDefault successCallback:^(BOOL hasEffect){} failureCallback:^(NSString *message){}];
      [self.splashScreenControllers removeObjectForKey:viewController];
      [self showSplashScreenFor:viewController options:ABI48_0_0EXSplashScreenDefault];
    }
  }
}
