...
#import <ABI48_0_0EXSplashScreen/ABI48_0_0EXSplashScreenModule.h>
#import <ABI48_0_0EXSplashScreen/ABI48_0_0EXSplashScreenService.h>
...
@protocol ABI48_0_0EXSplashScreenUtilService

- (UIViewController *)currentViewController;

@end

@interface ABI48_0_0EXSplashScreenModule ()
...
@implementation ABI48_0_0EXSplashScreenModule

ABI48_0_0EX_EXPORT_MODULE(ExpoSplashScreen);
...
ABI48_0_0EX_EXPORT_METHOD_AS(hideAsync,
                    hideWithResolve:(ABI48_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [[self splashScreenService] hideSplashScreenFor:currentViewController
                                            options:ABI48_0_0EXSplashScreenDefault
...
ABI48_0_0EX_EXPORT_METHOD_AS(preventAutoHideAsync,
                    preventAutoHideWithResolve:(ABI48_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  ABI48_0_0EX_WEAKIFY(self);
  dispatch_async(dispatch_get_main_queue(), ^{
 ...
 * Silent agreement is that registered module conforms to "ABI48_0_0EXSplashScreenService" protocol.
 */
- (id<ABI48_0_0EXSplashScreenService>)splashScreenService
{
  return [self.moduleRegistry getSingletonModuleForName:@"SplashScreen"];
}

/**
 * Tries to obtain a reference to the UIViewController for the main ABI48_0_0RCTRootView
 * by iterating through all of the application's windows and their viewControllers
 * until it finds one with a ABI48_0_0RCTRootView.
 */
- (UIViewController *)reactViewController
{
  ...
  id<ABI48_0_0EXSplashScreenUtilService> utilService = [_moduleRegistry getSingletonModuleForName:@"Util"];
  if (utilService != nil) {
  ...
  if (!controller) {
    // no ABI48_0_0RCTRootView was found, so just fall back to the key window's root view controller
...
- (nullable UIViewController *)viewControllerContainingRCTRootView
{
  ...
  for (UIWindow *window in allWindows) {
    ...
    if ([controller.view isKindOfClass:[ABI48_0_0RCTRootView class]]) {
      return controller;
    }
    UIViewController *presentedController = controller.presentedViewController;
    while (presentedController && ![presentedController isBeingDismissed]) {
      if ([presentedController.view isKindOfClass:[ABI48_0_0RCTRootView class]]) {
  ...
  // no ABI48_0_0RCTRootView was found
  return nil;
}
