...
#import <ABI48_0_0EXAV/ABI48_0_0EXVideoView.h>
#import <ABI48_0_0EXAV/ABI48_0_0EXAudioRecordingPermissionRequester.h>
#import <ABI48_0_0EXAV/ABI48_0_0EXAV+AudioSampleCallback.h>

NSString *const ABI48_0_0EXAudioRecordingOptionsIsMeteringEnabledKey = @"isMeteringEnabled";
NSString *const ABI48_0_0EXAudioRecordingOptionsKeepAudioActiveHintKey = @"keepAudioActiveHint";
NSString *const ABI48_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI48_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI48_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI48_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI48_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI48_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI48_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI48_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI48_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI48_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI48_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI48_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

NSString *const ABI48_0_0EXDidUpdatePlaybackStatusEventName = @"didUpdatePlaybackStatus";

NSString *const ABI48_0_0EXDidUpdateMetadataEventName = @"didUpdateMetadata";

@interface ABI48_0_0EXAV ()
...
@property (nonatomic, assign) ABI48_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI48_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI48_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, assign) BOOL isBeingObserved;
@property (nonatomic, strong) NSHashTable <NSObject<ABI48_0_0EXAVObject> *> *videoSet;
...
@property (nonatomic, weak) ABI48_0_0EXModuleRegistry *expoModuleRegistry;
@property (nonatomic, weak) id<ABI48_0_0EXPermissionsInterface> permissionsManager;

@end

@implementation ABI48_0_0EXAV

ABI48_0_0EX_EXPORT_MODULE(ExponentAV);

- (instancetype)init
{
  if (self = [super init]) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI48_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI48_0_0EXAudioInterruptionModeMixWithOthers;
...
{
  return @[@protocol(ABI48_0_0EXAVInterface)];
}

- (void)installJsiBindings
{
  id<ABI48_0_0EXJavaScriptContextProvider> jsContextProvider = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXJavaScriptContextProvider)];
...
- (void)setModuleRegistry:(ABI48_0_0EXModuleRegistry *)expoModuleRegistry
{
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXAppLifecycleService)] unregisterAppLifecycleListener:self];
  ...
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXAppLifecycleService)] registerAppLifecycleListener:self];
  _permissionsManager = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXPermissionsInterface)];
  [ABI48_0_0EXPermissionsMethodsDelegate registerRequesters:@[[ABI48_0_0EXAudioRecordingPermissionRequester new]] withPermissionsManager:_permissionsManager];
}
...
- (void)onAppForegrounded
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI48_0_0EXAVObject> *exAVObject) {
...
- (void)onAppContentWillReload
{
  ...
  // using pointer to deallocated ABI48_0_0EXAVPlayerData in MTAudioTap process callback
  for (NSNumber *key in [_soundDictionary allKeys]) {
    [self _removeAudioCallbackForKey:key];
  }
}

#pragma mark - ABI48_0_0RCTBridgeModule
...
// Required in Expo Go only - ABI48_0_0EXAV conforms to ABI48_0_0RCTBridgeModule protocol
// and in Expo Go, kernel calls [ABI48_0_0EXReactAppManager rebuildBridge]
// which requires this to be implemented. Normal "bare" RN modules
// use ABI48_0_0RCT_EXPORT_MODULE macro which implement this automatically.
+(NSString *)moduleName
{
  return @"ExponentAV";
}

// Both ABI48_0_0RCTBridgeModule and ABI48_0_0EXExportedModule define `constantsToExport`. We implement
// that method for the latter, but ABI48_0_0React Bridge displays a yellow LogBox warning:
// "Module ABI48_0_0EXAV requires main queue setup since it overrides `constantsToExport` but doesn't implement `requiresMainQueueSetup`."
// Since we don't care about that (ABI48_0_0RCTBridgeModule is used here for another reason),
// we just need this to dismiss that warning.
+ (BOOL)requiresMainQueueSetup
{
  // We are now using main thread to avoid thread safety issues with `ABI48_0_0EXAVPlayerData` and `ABI48_0_0EXVideoView`
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI48_0_0EXAVObject> *exAVObject))block
{
  for (ABI48_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI48_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI48_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  ...
  if (!playsInSilentMode && interruptionMode == ABI48_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI48_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and duckOthers == true cannot be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI48_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and allowsRecording == true cannot be set on iOS.");
  } else if (!playsInSilentMode && shouldPlayInBackground) {
    return ABI48_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and staysActiveInBackground == true cannot be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI48_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI48_0_0EXAVAudioSessionMode)audioSessionMode
{
  ...
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI48_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI48_0_0EXAudioInterruptionModeDoNotMix) {
      requiredAudioCategory = AVAudioSessionCategorySoloAmbient;
    } else {
      requiredAudioCategory = AVAudioSessionCategoryAmbient;
    }
  } else {
    ABI48_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI48_0_0EXAVAudioSessionModeActiveMuted ? ABI48_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
    ...
    switch (activeInterruptionMode) {
      case ABI48_0_0EXAudioInterruptionModeDoNotMix:
        break;
      case ABI48_0_0EXAudioInterruptionModeDuckOthers:
        requiredAudioCategoryOptions = AVAudioSessionCategoryOptionDuckOthers;
        break;
      case ABI48_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI48_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI48_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI48_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI48_0_0EXAVObject> *exAVObject) {
    ABI48_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI48_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI48_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI48_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI48_0_0EXErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded && !_staysActiveInBackground && ![_kernelAudioSessionManagerDelegate isActiveForModule:self]) {
    return ABI48_0_0EXErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI48_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI48_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI48_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI48_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI48_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI48_0_0EXPromiseRejectBlock)reject
{
  ABI48_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
...
- (void)_runBlock:(void (^)(ABI48_0_0EXVideoView *view))block
     ...
     withRejecter:(ABI48_0_0EXPromiseRejectBlock)reject
{
  ...
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXUIManager)] executeUIBlock:^(id view) {
    if ([view isKindOfClass:[ABI48_0_0EXVideoView class]]) {
      block(view);
    } else {
      reject(@"E_VIDEO_TAGINCORRECT", [NSString stringWithFormat:@"Invalid view returned from registry, expecting ABI48_0_0EXVideo, got: %@", view], nil);
    }
  } forView:reactTag ofClass:[ABI48_0_0EXVideoView class]];
}
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI48_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI48_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI48_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI48_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI48_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI48_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI48_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI48_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI48_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI48_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI48_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI48_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI48_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI48_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI48_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI48_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI48_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI48_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
    @([self _getFormatIDFromString:iosOptionsFromJS[ABI48_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI48_0_0EXErrorWithMessage(@"Recorder is already prepared.");
  }
  
  id<ABI48_0_0EXFileSystemInterface> fileSystem = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXFileSystemInterface)];
  
  if (!fileSystem) {
    return ABI48_0_0EXErrorWithMessage(@"No FileSystem module.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI48_0_0EXPromiseRejectBlock)reject
{
...
{
  return @[ABI48_0_0EXDidUpdatePlaybackStatusEventName, ABI48_0_0EXDidUpdateMetadataEventName, @"ExponentAV.onError"];
}

#pragma mark - Audio API: Global settings

ABI48_0_0EX_EXPORT_METHOD_AS(setAudioIsEnabled,
...
ABI48_0_0EX_EXPORT_METHOD_AS(setAudioMode,
...
ABI48_0_0EX_EXPORT_METHOD_AS(loadForSound,
                    loadForSound:(NSDictionary *)source
                    withStatus:(NSDictionary *)status
                    resolver:(ABI48_0_0EXPromiseResolveBlock)loadSuccess
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);
  
  ABI48_0_0EX_WEAKIFY(self);
  ABI48_0_0EXAVPlayerData *data = [[ABI48_0_0EXAVPlayerData alloc] initWithEXAV:self
                                                   withSource:source
                                                   withStatus:status
    ...
    if (success) {
      ...
      loadError(@"ABI48_0_0EXAV", error, nil);
    }
  }];
  ...
  data.statusUpdateCallback = ^(NSDictionary *status) {
    ...
    if (self.isBeingObserved) {
      ...
      [self sendEventWithName:ABI48_0_0EXDidUpdatePlaybackStatusEventName body:response];
    }
  };
  ...
  data.metadataUpdateCallback = ^(NSDictionary *metadata) {
    ABI48_0_0EX_ENSURE_STRONGIFY(self);
      if (self.isBeingObserved) {
        ...
        [self sendEventWithName:ABI48_0_0EXDidUpdateMetadataEventName body:response];
      }
  };
...
{
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI48_0_0EXEventEmitterService)] sendEventWithName:eventName body:body];
}

ABI48_0_0EX_EXPORT_METHOD_AS(unloadForSound,
                    unloadForSound:(NSNumber *)key
                    resolver:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI48_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI48_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI48_0_0EX_EXPORT_METHOD_AS(setStatusForSound,
...
ABI48_0_0EX_EXPORT_METHOD_AS(getStatusForSound,
...
ABI48_0_0EX_EXPORT_METHOD_AS(replaySound,
...
ABI48_0_0EX_EXPORT_METHOD_AS(loadForVideo,
                    ...
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI48_0_0EXVideoView *view) {
...
ABI48_0_0EX_EXPORT_METHOD_AS(unloadForVideo,
...
ABI48_0_0EX_EXPORT_METHOD_AS(setStatusForVideo,
...
ABI48_0_0EX_EXPORT_METHOD_AS(replayVideo,
...
ABI48_0_0EX_EXPORT_METHOD_AS(getStatusForVideo,
                    ...
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  [ABI48_0_0EXPermissionsMethodsDelegate getPermissionWithPermissionsManager:_permissionsManager
                                                      withRequester:[ABI48_0_0EXAudioRecordingPermissionRequester class]
                                                            resolve:resolve
                                                             reject:reject];
}
...
ABI48_0_0EX_EXPORT_METHOD_AS(prepareAudioRecorder,
                    prepareAudioRecorder:(NSDictionary *)options
                    resolver:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  _mediaServicesDidReset = false;
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI48_0_0EXAudioRecordingPermissionRequester class]]) {
  ...
  if (_audioRecorder && !error) {
    ...
    if (options[ABI48_0_0EXAudioRecordingOptionsIsMeteringEnabledKey]) {
    ...
    _audioRecorderIsPreparing = false;
    if (!options[ABI48_0_0EXAudioRecordingOptionsKeepAudioActiveHintKey]) {
      [self demoteAudioSessionIfPossible];
    }
  } else {
    reject(@"E_AUDIO_RECORDERNOTCREATED", [NSString stringWithFormat:@"Prepare encountered an error: %@", error.description], error);
...
ABI48_0_0EX_EXPORT_METHOD_AS(startAudioRecording,
                    startAudioRecording:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI48_0_0EXAudioRecordingPermissionRequester class]]) {
    reject(@"E_MISSING_PERMISSION", @"Missing audio recording permission.", nil);
...
ABI48_0_0EX_EXPORT_METHOD_AS(pauseAudioRecording,
                    pauseAudioRecording:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI48_0_0EX_EXPORT_METHOD_AS(stopAudioRecording,
                    stopAudioRecording:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI48_0_0EX_EXPORT_METHOD_AS(getAudioRecordingStatus,
                    getAudioRecordingStatus:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI48_0_0EX_EXPORT_METHOD_AS(unloadAudioRecorder,
                    unloadAudioRecorder:(ABI48_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    [self _removeAudioRecorder:YES];
...
ABI48_0_0EX_EXPORT_METHOD_AS(getAvailableInputs,
                    resolver:(ABI48_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0UMPromiseRejectBlock)reject)
{
...
ABI48_0_0EX_EXPORT_METHOD_AS(getCurrentInput,
                    getCurrentInput:(ABI48_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI48_0_0UMPromiseRejectBlock)reject)
{
...
ABI48_0_0EX_EXPORT_METHOD_AS(setInput,
