...
#import <ABI49_0_0EXAV/ABI49_0_0EXVideoView.h>
#import <ABI49_0_0EXAV/ABI49_0_0EXAudioRecordingPermissionRequester.h>
#import <ABI49_0_0EXAV/ABI49_0_0EXAV+AudioSampleCallback.h>

NSString *const ABI49_0_0EXAudioRecordingOptionsIsMeteringEnabledKey = @"isMeteringEnabled";
NSString *const ABI49_0_0EXAudioRecordingOptionsKeepAudioActiveHintKey = @"keepAudioActiveHint";
NSString *const ABI49_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI49_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI49_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI49_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI49_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI49_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI49_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI49_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI49_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI49_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI49_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI49_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

NSString *const ABI49_0_0EXDidUpdatePlaybackStatusEventName = @"didUpdatePlaybackStatus";

NSString *const ABI49_0_0EXDidUpdateMetadataEventName = @"didUpdateMetadata";

@interface ABI49_0_0EXAV ()
...
@property (nonatomic, assign) ABI49_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI49_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI49_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, assign) BOOL isBeingObserved;
@property (nonatomic, strong) NSHashTable <NSObject<ABI49_0_0EXAVObject> *> *videoSet;
...
@property (nonatomic, weak) ABI49_0_0EXModuleRegistry *expoModuleRegistry;
@property (nonatomic, weak) id<ABI49_0_0EXPermissionsInterface> permissionsManager;

@end

@implementation ABI49_0_0EXAV

ABI49_0_0EX_EXPORT_MODULE(ExponentAV);

- (instancetype)init
{
  if (self = [super init]) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI49_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI49_0_0EXAudioInterruptionModeMixWithOthers;
...
{
  return @[@protocol(ABI49_0_0EXAVInterface)];
}

- (void)installJsiBindings
{
  id<ABI49_0_0EXJavaScriptContextProvider> jsContextProvider = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXJavaScriptContextProvider)];
...
- (void)setModuleRegistry:(ABI49_0_0EXModuleRegistry *)expoModuleRegistry
{
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXAppLifecycleService)] unregisterAppLifecycleListener:self];
  ...
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXAppLifecycleService)] registerAppLifecycleListener:self];
  _permissionsManager = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXPermissionsInterface)];
  [ABI49_0_0EXPermissionsMethodsDelegate registerRequesters:@[[ABI49_0_0EXAudioRecordingPermissionRequester new]] withPermissionsManager:_permissionsManager];
}
...
- (void)onAppForegrounded
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI49_0_0EXAVObject> *exAVObject) {
...
- (void)onAppContentWillReload
{
  ...
  // using pointer to deallocated ABI49_0_0EXAVPlayerData in MTAudioTap process callback
  for (NSNumber *key in [_soundDictionary allKeys]) {
    [self _removeAudioCallbackForKey:key];
  }
}

#pragma mark - ABI49_0_0RCTBridgeModule
...
// Required in Expo Go only - ABI49_0_0EXAV conforms to ABI49_0_0RCTBridgeModule protocol
// and in Expo Go, kernel calls [ABI49_0_0EXReactAppManager rebuildBridge]
// which requires this to be implemented. Normal "bare" RN modules
// use ABI49_0_0RCT_EXPORT_MODULE macro which implement this automatically.
+(NSString *)moduleName
{
  return @"ExponentAV";
}

// Both ABI49_0_0RCTBridgeModule and ABI49_0_0EXExportedModule define `constantsToExport`. We implement
// that method for the latter, but ABI49_0_0React Bridge displays a yellow LogBox warning:
// "Module ABI49_0_0EXAV requires main queue setup since it overrides `constantsToExport` but doesn't implement `requiresMainQueueSetup`."
// Since we don't care about that (ABI49_0_0RCTBridgeModule is used here for another reason),
// we just need this to dismiss that warning.
+ (BOOL)requiresMainQueueSetup
{
  // We are now using main thread to avoid thread safety issues with `ABI49_0_0EXAVPlayerData` and `ABI49_0_0EXVideoView`
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI49_0_0EXAVObject> *exAVObject))block
{
  for (ABI49_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI49_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI49_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  ...
  if (!playsInSilentMode && interruptionMode == ABI49_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI49_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and duckOthers == true cannot be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI49_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and allowsRecording == true cannot be set on iOS.");
  } else if (!playsInSilentMode && shouldPlayInBackground) {
    return ABI49_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and staysActiveInBackground == true cannot be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI49_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI49_0_0EXAVAudioSessionMode)audioSessionMode
{
  ...
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI49_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI49_0_0EXAudioInterruptionModeDoNotMix) {
      requiredAudioCategory = AVAudioSessionCategorySoloAmbient;
    } else {
      requiredAudioCategory = AVAudioSessionCategoryAmbient;
    }
  } else {
    ABI49_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI49_0_0EXAVAudioSessionModeActiveMuted ? ABI49_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
    ...
    switch (activeInterruptionMode) {
      case ABI49_0_0EXAudioInterruptionModeDoNotMix:
        break;
      case ABI49_0_0EXAudioInterruptionModeDuckOthers:
        requiredAudioCategoryOptions = AVAudioSessionCategoryOptionDuckOthers;
        break;
      case ABI49_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI49_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI49_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI49_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI49_0_0EXAVObject> *exAVObject) {
    ABI49_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI49_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI49_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI49_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI49_0_0EXErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded && !_staysActiveInBackground && ![_kernelAudioSessionManagerDelegate isActiveForModule:self]) {
    return ABI49_0_0EXErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI49_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI49_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI49_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI49_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI49_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI49_0_0EXPromiseRejectBlock)reject
{
  ABI49_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
...
- (void)_runBlock:(void (^)(ABI49_0_0EXVideoView *view))block
     ...
     withRejecter:(ABI49_0_0EXPromiseRejectBlock)reject
{
  ...
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXUIManager)] executeUIBlock:^(id view) {
    if ([view isKindOfClass:[ABI49_0_0EXVideoView class]]) {
      block(view);
    } else {
      reject(@"E_VIDEO_TAGINCORRECT", [NSString stringWithFormat:@"Invalid view returned from registry, expecting ABI49_0_0EXVideo, got: %@", view], nil);
    }
  } forView:reactTag ofClass:[ABI49_0_0EXVideoView class]];
}
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI49_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI49_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI49_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI49_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI49_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI49_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI49_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI49_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI49_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI49_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI49_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI49_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI49_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI49_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI49_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI49_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI49_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI49_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
    @([self _getFormatIDFromString:iosOptionsFromJS[ABI49_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI49_0_0EXErrorWithMessage(@"Recorder is already prepared.");
  }
  
  id<ABI49_0_0EXFileSystemInterface> fileSystem = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXFileSystemInterface)];
  
  if (!fileSystem) {
    return ABI49_0_0EXErrorWithMessage(@"No FileSystem module.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI49_0_0EXPromiseRejectBlock)reject
{
...
{
  return @[ABI49_0_0EXDidUpdatePlaybackStatusEventName, ABI49_0_0EXDidUpdateMetadataEventName, @"ExponentAV.onError"];
}

#pragma mark - Audio API: Global settings

ABI49_0_0EX_EXPORT_METHOD_AS(setAudioIsEnabled,
...
ABI49_0_0EX_EXPORT_METHOD_AS(setAudioMode,
...
ABI49_0_0EX_EXPORT_METHOD_AS(loadForSound,
                    loadForSound:(NSDictionary *)source
                    withStatus:(NSDictionary *)status
                    resolver:(ABI49_0_0EXPromiseResolveBlock)loadSuccess
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);
  
  ABI49_0_0EX_WEAKIFY(self);
  ABI49_0_0EXAVPlayerData *data = [[ABI49_0_0EXAVPlayerData alloc] initWithEXAV:self
                                                   withSource:source
                                                   withStatus:status
    ...
    if (success) {
      ...
      loadError(@"ABI49_0_0EXAV", error, nil);
    }
  }];
  ...
  data.statusUpdateCallback = ^(NSDictionary *status) {
    ...
    if (self.isBeingObserved) {
      ...
      [self sendEventWithName:ABI49_0_0EXDidUpdatePlaybackStatusEventName body:response];
    }
  };
  ...
  data.metadataUpdateCallback = ^(NSDictionary *metadata) {
    ABI49_0_0EX_ENSURE_STRONGIFY(self);
      if (self.isBeingObserved) {
        ...
        [self sendEventWithName:ABI49_0_0EXDidUpdateMetadataEventName body:response];
      }
  };
...
{
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXEventEmitterService)] sendEventWithName:eventName body:body];
}

ABI49_0_0EX_EXPORT_METHOD_AS(unloadForSound,
                    unloadForSound:(NSNumber *)key
                    resolver:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI49_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI49_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI49_0_0EX_EXPORT_METHOD_AS(setStatusForSound,
...
ABI49_0_0EX_EXPORT_METHOD_AS(getStatusForSound,
...
ABI49_0_0EX_EXPORT_METHOD_AS(replaySound,
...
ABI49_0_0EX_EXPORT_METHOD_AS(loadForVideo,
                    ...
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI49_0_0EXVideoView *view) {
...
ABI49_0_0EX_EXPORT_METHOD_AS(unloadForVideo,
...
ABI49_0_0EX_EXPORT_METHOD_AS(setStatusForVideo,
...
ABI49_0_0EX_EXPORT_METHOD_AS(replayVideo,
...
ABI49_0_0EX_EXPORT_METHOD_AS(getStatusForVideo,
                    ...
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  [ABI49_0_0EXPermissionsMethodsDelegate getPermissionWithPermissionsManager:_permissionsManager
                                                      withRequester:[ABI49_0_0EXAudioRecordingPermissionRequester class]
                                                            resolve:resolve
                                                             reject:reject];
}
...
ABI49_0_0EX_EXPORT_METHOD_AS(prepareAudioRecorder,
                    prepareAudioRecorder:(NSDictionary *)options
                    resolver:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  _mediaServicesDidReset = false;
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI49_0_0EXAudioRecordingPermissionRequester class]]) {
  ...
  if (_audioRecorder && !error) {
    ...
    if (options[ABI49_0_0EXAudioRecordingOptionsIsMeteringEnabledKey]) {
    ...
    _audioRecorderIsPreparing = false;
    if (!options[ABI49_0_0EXAudioRecordingOptionsKeepAudioActiveHintKey]) {
      [self demoteAudioSessionIfPossible];
    }
  } else {
    reject(@"E_AUDIO_RECORDERNOTCREATED", [NSString stringWithFormat:@"Prepare encountered an error: %@", error.description], error);
...
ABI49_0_0EX_EXPORT_METHOD_AS(startAudioRecording,
                    startAudioRecording:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI49_0_0EXAudioRecordingPermissionRequester class]]) {
    reject(@"E_MISSING_PERMISSION", @"Missing audio recording permission.", nil);
...
ABI49_0_0EX_EXPORT_METHOD_AS(pauseAudioRecording,
                    pauseAudioRecording:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI49_0_0EX_EXPORT_METHOD_AS(stopAudioRecording,
                    stopAudioRecording:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI49_0_0EX_EXPORT_METHOD_AS(getAudioRecordingStatus,
                    getAudioRecordingStatus:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI49_0_0EX_EXPORT_METHOD_AS(unloadAudioRecorder,
                    unloadAudioRecorder:(ABI49_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    [self _removeAudioRecorder:YES];
...
ABI49_0_0EX_EXPORT_METHOD_AS(getAvailableInputs,
                    resolver:(ABI49_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0UMPromiseRejectBlock)reject)
{
...
ABI49_0_0EX_EXPORT_METHOD_AS(getCurrentInput,
                    getCurrentInput:(ABI49_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI49_0_0UMPromiseRejectBlock)reject)
{
...
ABI49_0_0EX_EXPORT_METHOD_AS(setInput,
