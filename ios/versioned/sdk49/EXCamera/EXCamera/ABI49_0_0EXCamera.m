...
#import <ABI49_0_0ExpoModulesCore/ABI49_0_0EXBarcodeScannerProviderInterface.h>
#import <ABI49_0_0EXCamera/ABI49_0_0EXCamera.h>
#import <ABI49_0_0EXCamera/ABI49_0_0EXCameraUtils.h>
#import <ABI49_0_0EXCamera/ABI49_0_0EXCameraCameraPermissionRequester.h>
#import <ABI49_0_0ExpoModulesCore/ABI49_0_0EXAppLifecycleService.h>
#import <ABI49_0_0ExpoModulesCore/ABI49_0_0EXUtilities.h>
#import <ABI49_0_0ExpoModulesCore/ABI49_0_0EXFaceDetectorManagerInterface.h>
#import <ABI49_0_0ExpoModulesCore/ABI49_0_0EXFaceDetectorManagerProviderInterface.h>
...
@interface ABI49_0_0EXCamera ()

@property (nonatomic, weak) id<ABI49_0_0EXFileSystemInterface> fileSystem;
@property (nonatomic, weak) ABI49_0_0EXModuleRegistry *moduleRegistry;
@property (nonatomic, strong) id<ABI49_0_0EXFaceDetectorManagerInterface> faceDetectorManager;
@property (nonatomic, strong) id<ABI49_0_0EXBarCodeScannerInterface> barCodeScanner;
...
@property (nonatomic, strong) ABI49_0_0EXPromiseResolveBlock photoCapturedResolve;
@property (nonatomic, strong) ABI49_0_0EXPromiseRejectBlock photoCapturedReject;

@property (nonatomic, strong) ABI49_0_0EXPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI49_0_0EXPromiseRejectBlock videoRecordedReject;

@property (nonatomic, copy) ABI49_0_0EXDirectEventBlock onCameraReady;
@property (nonatomic, copy) ABI49_0_0EXDirectEventBlock onMountError;
@property (nonatomic, copy) ABI49_0_0EXDirectEventBlock onPictureSaved;
@property (nonatomic, copy) ABI49_0_0EXDirectEventBlock onResponsiveOrientationChanged;

@property (nonatomic, copy) ABI49_0_0EXDirectEventBlock onBarCodeScanned;
@property (nonatomic, copy) ABI49_0_0EXDirectEventBlock onFacesDetected;

@end

@implementation ABI49_0_0EXCamera
...
- (void)dealloc
{
  // In very rare case ABI49_0_0EXCamera might be unmounted (and thus deallocated) after starting taking a photo,
...
- (void)updateFlashMode
{
  ...
  if (_flashMode == ABI49_0_0EXCameraFlashModeTorch) {
    if (![device hasTorch]) {
      return;
    }
    ...
    if (![device lockForConfiguration:&error]) {
      if (error) {
        ABI49_0_0EXLogInfo(@"%s: %@", __func__, error);
      }
      return;
    }
...
- (void)updateFocusDepth
{
  ...
  if (device == nil || device.focusMode != ABI49_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if ([device isLockingFocusWithCustomLensPositionSupported]) {
    ...
    ABI49_0_0EX_WEAKIFY(device);
    [device setFocusModeLockedWithLensPosition:_focusDepth completionHandler:^(CMTime syncTime) {
      ABI49_0_0EX_ENSURE_STRONGIFY(device);
      [device unlockForConfiguration];
    }];
    return;
  }

  ABI49_0_0EXLogInfo(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
  return;
}
...
- (void)updateWhiteBalance
{
  ...
  if (_whiteBalance == ABI49_0_0EXCameraWhiteBalanceAuto) {
    [device setWhiteBalanceMode:AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance];
    [device unlockForConfiguration];
  } else {
    AVCaptureWhiteBalanceTemperatureAndTintValues temperatureAndTint = {
      .temperature = [ABI49_0_0EXCameraUtils temperatureForWhiteBalance:_whiteBalance],
      .tint = 0,
    };
  ...
  if (self.responsiveOrientationWhenOrientationLocked) {
    [self.motionManager startAccelerometerUpdatesToQueue:[NSOperationQueue new]
      withHandler:^(CMAccelerometerData  *accelerometerData, NSError *error) {
        if (!error) {
          UIDeviceOrientation deviceOrientation = [ABI49_0_0EXCameraUtils deviceOrientationForAccelerometerData:self.motionManager.accelerometerData defaultOrientation:self.physicalOrientation];
...
- (void)setIsScanningBarCodes:(BOOL)barCodeScanning
{
  if (_barCodeScanner) {
    ...
    ABI49_0_0EXLogError(@"BarCodeScanner module not found. Make sure `expo-barcode-scanner` is installed and linked correctly.");
  }
}
...
- (void)setIsDetectingFaces:(BOOL)faceDetecting
{
  if (_faceDetectorManager) {
    ...
    ABI49_0_0EXLogError(@"FaceDetector module not found. Make sure `expo-face-detector` is installed and linked correctly.");
  }
}
...
- (void)takePicture:(NSDictionary *)options resolve:(ABI49_0_0EXPromiseResolveBlock)resolve reject:(ABI49_0_0EXPromiseRejectBlock)reject
{
  ...
  [connection setVideoOrientation:[ABI49_0_0EXCameraUtils videoOrientationForDeviceOrientation:deviceOrientation]];
  ...
  switch (_flashMode) {
    case ABI49_0_0EXCameraFlashModeOff:
      requestedFlashMode = AVCaptureFlashModeOff;
      break;
    case ABI49_0_0EXCameraFlashModeAuto:
      requestedFlashMode = AVCaptureFlashModeAuto;
      break;
    case ABI49_0_0EXCameraFlashModeOn:
    case ABI49_0_0EXCameraFlashModeTorch:
                ...
                error:(NSError *)error
{
  ...
  ABI49_0_0EXPromiseRejectBlock reject = _photoCapturedReject;
  ABI49_0_0EXPromiseResolveBlock resolve = _photoCapturedResolve;
...
- (void)handleCapturedImageData:(NSData *)imageData metadata:(NSDictionary *)metadata options:(NSDictionary *)options resolver:(ABI49_0_0EXPromiseResolveBlock)resolve reject:(ABI49_0_0EXPromiseRejectBlock)reject
{
  ...
  takenImage = [ABI49_0_0EXCameraUtils cropImage:takenImage toRect:croppedSize];
  ...
  if ([options[@"exif"] boolValue]) {
    NSMutableDictionary *updatedExif = [ABI49_0_0EXCameraUtils updateExifMetadata:metadata[(NSString *)kCGImagePropertyExifDictionary] withAdditionalData:@{ @"Orientation": @([ABI49_0_0EXCameraUtils exportImageOrientation:takenImage.imageOrientation]) }];
    ...
    processedImageData = [ABI49_0_0EXCameraUtils dataFromImage:takenImage withMetadata:updatedMetadata imageQuality:quality];
  } else {
  ...
  response[@"uri"] = [ABI49_0_0EXCameraUtils writeImage:processedImageData toPath:path];
...
- (void)record:(NSDictionary *)options resolve:(ABI49_0_0EXPromiseResolveBlock)resolve reject:(ABI49_0_0EXPromiseRejectBlock)reject
{
  ...
  if (_movieFileOutput != nil && !_movieFileOutput.isRecording && _videoRecordedResolve == nil && _videoRecordedReject == nil) {
    ...
    if (connection.isVideoStabilizationSupported == NO) {
      ABI49_0_0EXLogWarn(@"%s: Video Stabilization is not supported on this device.", __func__);
    } else {
    ...
    if (options[@"quality"]) {
      ABI49_0_0EXCameraVideoResolution resolution = [options[@"quality"] integerValue];
      preset = [ABI49_0_0EXCameraUtils captureSessionPresetForVideoResolution:resolution];
...
- (void)setVideoOptions:(NSDictionary *)options forConnection:(AVCaptureConnection *)connection onReject:(ABI49_0_0EXPromiseRejectBlock)reject
{
  ABI49_0_0EX_WEAKIFY(self);
  dispatch_async(_sessionQueue, ^{
    ABI49_0_0EX_STRONGIFY(self);
    ...
    if (options[@"codec"]) {
      AVVideoCodecType videoCodecType = [ABI49_0_0EXCameraUtils videoCodecForType: [options[@"codec"] integerValue]];
...
- (void)maybeStartFaceDetection:(BOOL)mirrored {
  if (self.faceDetectorManager) {
    ...
    [connection setVideoOrientation:[ABI49_0_0EXCameraUtils videoOrientationForDeviceOrientation:[[UIDevice currentDevice] orientation]]];
...
#pragma clang diagnostic ignored "-Wunreachable-code"
  ...
  return;
#endif
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI49_0_0EXCameraCameraPermissionRequester class]]) {
...
- (void)initializeCaptureSessionInput
{
  ...
  AVCaptureVideoOrientation orientation = [ABI49_0_0EXCameraUtils videoOrientationForInterfaceOrientation:interfaceOrientation];
  ...
  dispatch_async(_sessionQueue, ^{
    ...
    NSError *error = nil;
    AVCaptureDevice *captureDevice = [ABI49_0_0EXCameraUtils deviceWithMediaType:AVMediaTypeVideo preferringPosition:self.presetCamera];
...
{
  ABI49_0_0EX_WEAKIFY(self);
  AVCaptureVideoOrientation videoOrientation = [ABI49_0_0EXCameraUtils videoOrientationForInterfaceOrientation:orientation];
  [ABI49_0_0EXUtilities performSynchronouslyOnMainThread:^{
    ABI49_0_0EX_ENSURE_STRONGIFY(self);
...
{
  id<ABI49_0_0EXFaceDetectorManagerProviderInterface> faceDetectorProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXFaceDetectorManagerProviderInterface)];

  if (faceDetectorProvider) {
    id<ABI49_0_0EXFaceDetectorManagerInterface> faceDetector = [faceDetectorProvider createFaceDetectorManager];
    if (faceDetector) {
      ABI49_0_0EX_WEAKIFY(self);
...
{
  id<ABI49_0_0EXBarCodeScannerProviderInterface> barCodeScannerProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI49_0_0EXBarCodeScannerProviderInterface)];
  if (barCodeScannerProvider) {
    id<ABI49_0_0EXBarCodeScannerInterface> barCodeScanner = [barCodeScannerProvider createBarCodeScanner];
    if (barCodeScanner) {
      ABI49_0_0EX_WEAKIFY(self);
