...
@objc(ABI49_0_0EXUpdatesAppControllerDelegate)
 ...
 * used by the exported UpdatesModule through ABI49_0_0EXUpdatesService. Such information includes
 ...
 */
@objc(ABI49_0_0EXUpdatesAppController)
@objcMembers
public class AppController: NSObject, AppLoaderTaskDelegate, ErrorRecoveryDelegate, UpdatesStateChangeDelegate {
  private static let ErrorDomain = "ABI49_0_0EXUpdatesAppController"
  private static let ABI49_0_0EXUpdatesEventName = "Expo.nativeUpdatesEvent"
  private static let ABI49_0_0EXUpdatesStateChangeEventName = "Expo.nativeUpdatesStateChangeEvent"
  ...
  /**
   Delegate which will be notified when ABI49_0_0EXUpdates has an update ready to launch and
   `launchAssetUrl` is nonnull.
   */
  public weak var delegate: AppControllerDelegate?

  /**
   The ABI49_0_0RCTBridge for which ABI49_0_0EXUpdates is providing the JS bundle and assets.
   This is optional, but required in order for `Updates.reload()` and Updates module events to work.
   */
  public weak var bridge: ABI49_0_0RCTBridge?

  /**
   The URL on disk to source asset for the ABI49_0_0RCTBridge.
   Will be null until the AppController delegate method is called.
   This should be provided in the `sourceURLForBridge:` method of ABI49_0_0RCTBridgeDelegate.
   */
  ...
  /**
   ...
   can be used by `expo-asset` or a similar module to override ABI49_0_0React Native's asset resolution and
  ...
  /**
   for internal use in ABI49_0_0EXUpdates
   */
  ...
  public func start() {
    ...
    if config.updateUrl == nil {
      NSException(
        name: .internalInconsistencyException,
        reason: "expo-updates is enabled, but no valid URL is configured under ABI49_0_0EXUpdatesURL. If you are making a release build for the first time, make sure you have run `expo publish` at least once."
      )
      .raise()
    }
    ...
    if config.scopeKey == nil {
      NSException(
        name: .internalInconsistencyException,
        reason: "expo-updates was configured with no scope key. Make sure a valid URL is configured under ABI49_0_0EXUpdatesURL."
      )
      .raise()
    }
  ...
  public func requestRelaunch(completion: @escaping ABI49_0_0EXUpdatesAppRelaunchCompletionBlock) {
    ...
    launcherWithDatabase.launchUpdate(withSelectionPolicy: selectionPolicy()) { error, success in
      if success {
        ...
        ABI49_0_0RCTReloadCommandSetBundleURL(self.launcher!.launchAssetUrl)
        ABI49_0_0RCTTriggerReloadCommandListeners("Requested by JavaScript - Updates.reloadAsync()")
        self.runReaper()
  ...
  internal func sendLegacyUpdateEventToBridge(_ eventType: String, body: [String: Any] ) {
    ...
    sendEventToBridge(AppController.ABI49_0_0EXUpdatesEventName, eventType, body: body)
  }
  ...
  internal func sendUpdateStateChangeEventToBridge(_ eventType: UpdatesStateEventType, body: [String: Any?]) {
    ...
    sendEventToBridge(AppController.ABI49_0_0EXUpdatesStateChangeEventName, "\(eventType)", body: body)
  }
  ...
  private func sendEventToBridge(_ eventName: String, _ eventType: String, body: [String: Any?]) {
    ...
    guard let bridge = bridge else {
      ...
      logger.warn(message: "ABI49_0_0EXUpdates: Could not emit event: name = \(eventName), type = \(eventType). Event will be emitted when the bridge is available", code: .jsRuntimeError)
      return
    }
    bridge.enqueueJSCall("ABI49_0_0RCTDeviceEventEmitter.emit", args: [eventName, mutableBody])
  }
  ...
  internal func sendQueuedEventsToBridge() {
    ...
    eventsToSendToJS.forEach { mutableBody in
      bridge.enqueueJSCall("ABI49_0_0RCTDeviceEventEmitter.emit", args: [AppController.ABI49_0_0EXUpdatesEventName, mutableBody])
    }
    eventsToSendToJS = []
  }
  ...
  public func relaunch(completion: @escaping (Error?, Bool) -> Void) {
    ...
    launcher.launchUpdate(withSelectionPolicy: selectionPolicy()) { error, success in
      if success {
        self.launcher = self.candidateLauncher
        self.errorRecovery.startMonitoring()
        ABI49_0_0RCTReloadCommandSetBundleURL(launcher.launchAssetUrl)
        ABI49_0_0RCTTriggerReloadCommandListeners("Relaunch after fatal error")
        completion(nil, true)
      } else {
