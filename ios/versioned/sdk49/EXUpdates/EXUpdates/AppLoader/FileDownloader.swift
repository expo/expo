...
private let ErrorDomain = "ABI49_0_0EXUpdatesFileDownloader"
...
internal final class FileDownloader: NSObject, URLSessionDataDelegate {
  ...
  private func parseMultipartManifestResponse(
    ...
    let reader = ABI49_0_0EXUpdatesMultipartStreamReader(inputStream: InputStream(data: data), boundary: boundary)
    ...
    let completed = data.isEmpty || reader.readAllParts { headers, content, _ in
      if let contentDisposition = (headers as! [String: Any]).stringValueForCaseInsensitiveKey("content-disposition") {
        if let contentDispositionParameters = ABI49_0_0EXUpdatesParameterParser().parseParameterString(
          contentDisposition,
          withDelimiter: FileDownloader.ParameterParserSemicolonDelimiter
        ) as? [String: Any],
