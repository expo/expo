...
#import <ABI49_0_0React/ABI49_0_0RCTAppState.h>
#import <ABI49_0_0React/ABI49_0_0RCTImageLoader.h>

#import <ABI49_0_0ExpoModulesCore/ABI49_0_0EXReactNativeAdapter.h>
...
#import <ABI49_0_0ExpoModulesCore/ABI49_0_0ExpoFabricViewObjC.h>
#endif

@interface ABI49_0_0EXReactNativeAdapter ()
...
@implementation ABI49_0_0EXReactNativeAdapter

ABI49_0_0EX_REGISTER_MODULE();

+ (NSString *)moduleName
{
  return @"ABI49_0_0EXReactNativeAdapter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI49_0_0EXAppLifecycleService), @protocol(ABI49_0_0EXUIManager), @protocol(ABI49_0_0EXJavaScriptContextProvider)];
}
...
{
  __weak ABI49_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI49_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI49_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI49_0_0RCTJSThread];
}
...
# pragma mark - ABI49_0_0EXJavaScriptContextProvider
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI49_0_0RCTContentDidAppearNotification,
                           ABI49_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI49_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI49_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI49_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI49_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
...
- (void)executeUIBlock:(void (^)(UIView *view))block forView:(id)viewId
{
  ...
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    ...
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI49_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
#if RN_FABRIC_ENABLED
        ABI49_0_0RCTSurfacePresenter *surfacePresenter = strongSelf.bridge.surfacePresenter;
        UIView<ABI49_0_0RCTComponentViewProtocol> *componentView = [surfacePresenter.mountingManager.componentViewRegistry findComponentViewWithTag:[viewId integerValue]];
