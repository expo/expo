...
@interface ABI49_0_0EXModulesProxyConfig ()
...
@implementation ABI49_0_0EXModulesProxyConfig
...
@interface ABI49_0_0RCTBridge (RegisterAdditionalModuleClasses)
...
@interface ABI49_0_0EXNativeModulesProxy ()

@property (nonatomic, strong) NSRegularExpression *regexp;
@property (nonatomic, strong) ABI49_0_0EXModuleRegistry *exModuleRegistry;
...
@implementation ABI49_0_0EXNativeModulesProxy {
  __weak ABI49_0_0EXAppContext * _Nullable _appContext;
}

@synthesize bridge = _bridge;
@synthesize nativeModulesConfig = _nativeModulesConfig;

ABI49_0_0RCT_EXPORT_MODULE(NativeUnimoduleProxy)
...
{
  if (self = [super init]) {
    _exModuleRegistry = moduleRegistry != nil ? moduleRegistry : [[ABI49_0_0EXModuleRegistryProvider new] moduleRegistry];
...
/**
 The initializer for Expo Go to pass a custom `ABI49_0_0EXModuleRegistry`
 other than the default one from `ABI49_0_0EXModuleRegistryProvider`.
 The `ABI49_0_0EXModuleRegistry` is still owned by this class.
 */
- (instancetype)initWithCustomModuleRegistry:(nonnull ABI49_0_0EXModuleRegistry *)moduleRegistry
{
...
/**
 Convenience initializer used by ABI49_0_0React Native in the new setup, where the modules are registered automatically.
 */
- (instancetype)init
{
...
- (nonnull ABI49_0_0EXModulesProxyConfig *)nativeModulesConfig
{
  ...
  for (ABI49_0_0EXExportedModule *exportedModule in [_exModuleRegistry getAllExportedModules]) {
    @try {
  ...
  ABI49_0_0EXModulesProxyConfig *config = [[ABI49_0_0EXModulesProxyConfig alloc] initWithConstants:exportedModulesConstants
...
{
  ABI49_0_0ExpoBridgeModule* expoBridgeModule = [bridge moduleForClass:ABI49_0_0ExpoBridgeModule.class];
...
ABI49_0_0RCT_EXPORT_METHOD(callMethod:(NSString *)moduleName methodNameOrKey:(id)methodNameOrKey arguments:(NSArray *)arguments resolver:(ABI49_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI49_0_0RCTPromiseRejectBlock)reject)
{
  ...
  ABI49_0_0EXExportedModule *module = [_exModuleRegistry getExportedModuleForName:moduleName];
  if (module == nil) {
...
- (void)registerExpoModulesInBridge:(ABI49_0_0RCTBridge *)bridge
{
  // Registering expo modules (excluding Swifty view managers!) in bridge is needed only when the proxy module owns
  // the registry (was autoinitialized by ABI49_0_0React Native). Otherwise they're registered by the registry adapter.
  BOOL ownsModuleRegistry = _ownsModuleRegistry && ![bridge moduleIsInitialized:[ABI49_0_0EXReactNativeEventEmitter class]];

  // An array of `ABI49_0_0RCTBridgeModule` classes to register.
  NSMutableArray<Class<ABI49_0_0RCTBridgeModule>> *additionalModuleClasses = [NSMutableArray new];
  ...
  // `registerAdditionalModuleClasses:` call below is not thread-safe if ABI49_0_0RCTUIManager is not initialized.
  // The case happens especially with reanimated which accesses `bridge.uiManager` and initialize bridge in js thread.
  // Accessing uiManager here, we try to make sure ABI49_0_0RCTUIManager is initialized.
  [bridge uiManager];
  ...
  // Get the instance of `ABI49_0_0EXReactEventEmitter` bridge module and give it access to the interop bridge.
  ABI49_0_0EXReactNativeEventEmitter *eventEmitter = [bridge moduleForClass:[ABI49_0_0EXReactNativeEventEmitter class]];
  ...
  if (ownsModuleRegistry) {
    ...
    // It calls `setModuleRegistry:` on all `ABI49_0_0EXModuleRegistryConsumer`s.
    [_exModuleRegistry initialize];
  }
}

- (void)registerAdditionalModuleClasses:(NSArray<Class> *)moduleClasses inBridge:(ABI49_0_0RCTBridge *)bridge
{
  // In remote debugging mode, i.e. executorClass is `ABI49_0_0RCTWebSocketExecutor`,
  // there is a deadlock issue in `registerAdditionalModuleClasses:` and causes app freezed.
  //   - The JS thread acquired the `ABI49_0_0RCTCxxBridge._moduleRegistryLock` lock in `ABI49_0_0RCTCxxBridge._initializeBridgeLocked`
  //      = it further goes into ABI49_0_0RCTObjcExecutor and tries to get module config from main thread
  //   - The main thread is pending in `ABI49_0_0RCTCxxBridge.registerAdditionalModuleClasses` where trying to acquire the same lock.
  ...
  // As we just workaround in `ABI49_0_0RCTWebSocketExecutor` case, the risk of `_moduleRegistryCreated` race condition should be lower.
  //
  // Learn more about the non-locked initialization:
  // https://github.com/facebook/react-native/blob/757bb75fbf837714725d7b2af62149e8e2a7ee51/ABI49_0_0React/CxxBridge/ABI49_0_0RCTCxxBridge.mm#L922-L935
  // See the `_moduleRegistryCreated` false case
  if ([NSStringFromClass([bridge executorClass]) isEqualToString:@"ABI49_0_0RCTWebSocketExecutor"]) {
...
- (Class)registerComponentData:(ViewModuleWrapper *)viewModule inBridge:(ABI49_0_0RCTBridge *)bridge
{
  // Hacky way to get a dictionary with `ABI49_0_0RCTComponentData` from UIManager.
  NSMutableDictionary<NSString *, ABI49_0_0RCTComponentData *> *componentDataByName = [bridge.uiManager valueForKey:@"_componentDataByName"];
  ...
  ABI49_0_0EXComponentData *componentData = [[ABI49_0_0EXComponentData alloc] initWithViewModule:viewModule
  ...
  [[ABI49_0_0RCTComponentViewFactory currentComponentViewFactory] registerComponentViewClass:viewClass];
#endif
 ...
 */
- (void)registerLegacyComponentData:(Class)moduleClass inBridge:(ABI49_0_0RCTBridge *)bridge
{
  // Hacky way to get a dictionary with `ABI49_0_0RCTComponentData` from UIManager.
  NSMutableDictionary<NSString *, ABI49_0_0RCTComponentData *> *componentDataByName = [bridge.uiManager valueForKey:@"_componentDataByName"];
  NSString *className = [moduleClass moduleName] ?: NSStringFromClass(moduleClass);

  if ([moduleClass isSubclassOfClass:[ABI49_0_0RCTViewManager class]] && !componentDataByName[className]) {
    ABI49_0_0RCTComponentData *componentData = [[ABI49_0_0RCTComponentData alloc] initWithManagerClass:moduleClass bridge:bridge eventDispatcher:bridge.eventDispatcher];
    componentDataByName[className] = componentData;
  }
}
