...
/**
 Each module that has a view manager definition needs to be wrapped by `ABI49_0_0RCTViewManager`.
 Unfortunately, we can't use just one class because ABI49_0_0React Native checks for duplicated classes.
 We're generating its subclasses in runtime as a workaround.
 */
@objc
public final class ViewModuleWrapper: ABI49_0_0RCTViewManager, DynamicModuleWrapperProtocol {
  /**
  ...
  /**
   The designated initializer that is used by ABI49_0_0React Native to create module instances.
  ...
  /**
   Dummy initializer, for use only in `ABI49_0_0EXModuleRegistryAdapter.extraModulesForModuleRegistry:`.
   */
  @objc
  ...
  public static func createViewModuleWrapperClass(module: ViewModuleWrapper) -> ViewModuleWrapper.Type? {
    ...
    return prefixedViewName.withCString { viewNamePtr in
      ...
      // ABI49_0_0React Native initializes modules with `init` without params,
...
let directEventBlockImplementation = imp_implementationWithBlock({ ["ABI49_0_0RCTDirectEventBlock"] } as @convention(block) () -> [String])
