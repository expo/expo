...
#import <ABI45_0_0EXSplashScreen/ABI45_0_0EXSplashScreenModule.h>
...
@protocol ABI45_0_0EXSplashScreenUtilService

- (UIViewController *)currentViewController;

@end

@interface ABI45_0_0EXSplashScreenModule ()
...
@implementation ABI45_0_0EXSplashScreenModule

ABI45_0_0EX_EXPORT_MODULE(ExpoSplashScreen);
...
ABI45_0_0EX_EXPORT_METHOD_AS(hideAsync,
                    hideWithResolve:(ABI45_0_0EXPromiseResolveBlock)resolve
...
ABI45_0_0EX_EXPORT_METHOD_AS(preventAutoHideAsync,
                    preventAutoHideWithResolve:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  ABI45_0_0EX_WEAKIFY(self);
  dispatch_async(dispatch_get_main_queue(), ^{
 ...
 * Silent agreement is that registered module conforms to "ABI45_0_0EXSplashScreenService" protocol.
 */
- (id<ABI45_0_0EXSplashScreenService>)splashScreenService
{
  return [self.moduleRegistry getSingletonModuleForName:@"SplashScreen"];
}

/**
 * Tries to obtain a reference to the UIViewController for the main ABI45_0_0RCTRootView
 * by iterating through all of the application's windows and their viewControllers
 * until it finds one with a ABI45_0_0RCTRootView.
 */
- (UIViewController *)reactViewController
{
  ...
  id<ABI45_0_0EXSplashScreenUtilService> utilService = [_moduleRegistry getSingletonModuleForName:@"Util"];
  if (utilService != nil) {
  ...
  if (!controller) {
    // no ABI45_0_0RCTRootView was found, so just fall back to the key window's root view controller
...
- (nullable UIViewController *)viewControllerContainingRCTRootView
{
  ...
  for (UIWindow *window in allWindows) {
    ...
    if ([controller.view isKindOfClass:[ABI45_0_0RCTRootView class]]) {
      return controller;
    }
    UIViewController *presentedController = controller.presentedViewController;
    while (presentedController && ![presentedController isBeingDismissed]) {
      if ([presentedController.view isKindOfClass:[ABI45_0_0RCTRootView class]]) {
  ...
  // no ABI45_0_0RCTRootView was found
  return nil;
}
