...
typedef NS_ENUM(NSInteger, ABI45_0_0EXUpdatesErrorRecoveryTask) {
  ABI45_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate,
  ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchNew,
  ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchCached,
  ABI45_0_0EXUpdatesErrorRecoveryTaskCrash
};

static NSString * const ABI45_0_0EXUpdatesErrorLogFile = @"expo-error.log";
static NSInteger const ABI45_0_0EXUpdatesErrorRecoveryRemoteLoadTimeoutMs = 5000;

@interface ABI45_0_0EXUpdatesErrorRecovery ()
...
@property (nonatomic, copy) ABI45_0_0RCTFatalHandler previousFatalErrorHandler;
@property (nonatomic, copy) ABI45_0_0RCTFatalExceptionHandler previousFatalExceptionHandler;

@end

@implementation ABI45_0_0EXUpdatesErrorRecovery

- (instancetype)init
{
  return [self initWithErrorRecoveryQueue:dispatch_queue_create("expo.controller.errorRecoveryQueue", DISPATCH_QUEUE_SERIAL)
                           diskWriteQueue:nil
                        remoteLoadTimeout:ABI45_0_0EXUpdatesErrorRecoveryRemoteLoadTimeoutMs];
}
...
- (void)notifyNewRemoteLoadStatus:(ABI45_0_0EXUpdatesRemoteLoadStatus)newStatus
{
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    if (newStatus != ABI45_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded) {
      [self->_pipeline removeObject:@(ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchNew)];
    }
    [self _runNextTask];
  });
}
...
- (void)_startPipelineWithEncounteredError:(id)encounteredError
{
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    if (self->_delegate.launchedUpdate.successfulLaunchCount > 0) {
      [self->_pipeline removeObject:@(ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchCached)];
...
- (void)_runNextTask
{
  ...
  switch ((ABI45_0_0EXUpdatesErrorRecoveryTask)nextTask.integerValue) {
    case ABI45_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate:
      [self _waitForRemoteLoaderToFinish];
      break;
    // ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchNew is called only after a new update is downloaded
    // and added to the cache, so it is equivalent to ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchCached
    case ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchNew:
    case ABI45_0_0EXUpdatesErrorRecoveryTaskLaunchCached:
      [self _tryRelaunchFromCache];
      break;
    case ABI45_0_0EXUpdatesErrorRecoveryTaskCrash:
      [self _crash];
      break;
    default:
      @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"ABI45_0_0EXUpdatesErrorRecovery cannot run the next task" userInfo:@{@"nextTaskValue": nextTask}];
  }
}
...
{
  dispatch_assert_queue(_errorRecoveryQueue);
  if (_delegate.remoteLoadStatus == ABI45_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded) {
    [self _runNextTask];
  } else if (_delegate.config.checkOnLaunch != ABI45_0_0EXUpdatesCheckAutomaticallyConfigNever ||
             _delegate.remoteLoadStatus == ABI45_0_0EXUpdatesRemoteLoadStatusLoading) {
    _isWaitingForRemoteUpdate = YES;
    if (_delegate.remoteLoadStatus != ABI45_0_0EXUpdatesRemoteLoadStatusLoading) {
...
- (void)_crash
{
  ...
  if ([initialError isKindOfClass:[NSError class]]) {
    // format these keys similar to RN -- ABI45_0_0RCTFatal in ABI45_0_0RCTAssert.m
    name = [NSString stringWithFormat:@"%@: %@", ABI45_0_0RCTFatalExceptionName, ((NSError *)initialError).localizedDescription];
    reason = ABI45_0_0RCTFormatError(((NSError *)initialError).localizedDescription, ((NSError *)initialError).userInfo[ABI45_0_0RCTJSStackTraceKey], 175);
    userInfo = [((NSError *)initialError).userInfo mutableCopy];
    userInfo[@"ABI45_0_0RCTUntruncatedMessageKey"] = ABI45_0_0RCTFormatError(((NSError *)initialError).localizedDescription, ((NSError *)initialError).userInfo[ABI45_0_0RCTJSStackTraceKey], -1);
  ...
  userInfo[@"ABI45_0_0EXUpdatesLaterEncounteredErrors"] = [_encounteredErrors copy];
...
{
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleJavaScriptDidFailToLoad) name:ABI45_0_0RCTJavaScriptDidFailToLoadNotification object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleContentDidAppear) name:ABI45_0_0RCTContentDidAppearNotification object:nil];
}

- (void)_unregisterObservers
{
  [[NSNotificationCenter defaultCenter] removeObserver:self name:ABI45_0_0RCTJavaScriptDidFailToLoadNotification object:nil];
  [[NSNotificationCenter defaultCenter] removeObserver:self name:ABI45_0_0RCTContentDidAppearNotification object:nil];
}
...
- (void)_handleContentDidAppear
{
  ...
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    self->_pipeline = [self->_pipeline objectsAtIndexes:[self->_pipeline indexesOfObjectsPassingTest:^BOOL(NSNumber *obj, NSUInteger idx, BOOL *stop) {
      return obj.integerValue == ABI45_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate || obj.integerValue == ABI45_0_0EXUpdatesErrorRecoveryTaskCrash;
    }]].mutableCopy;
  });
  ...
  // ABI45_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate task and check for a new update is there is one
...
- (void)_setRCTErrorHandlers
{
  ...
  _previousFatalErrorHandler = ABI45_0_0RCTGetFatalHandler();
  _previousFatalExceptionHandler = ABI45_0_0RCTGetFatalExceptionHandler();

  ABI45_0_0RCTFatalHandler fatalErrorHandler = ^(NSError *error) {
    [self handleError:error];
  };
  ABI45_0_0RCTFatalExceptionHandler fatalExceptionHandler = ^(NSException *exception) {
    [self handleException:exception];
  };
  ABI45_0_0RCTSetFatalHandler(fatalErrorHandler);
  ABI45_0_0RCTSetFatalExceptionHandler(fatalExceptionHandler);
}

- (void)_unsetRCTErrorHandlers
{
  ABI45_0_0RCTSetFatalHandler(_previousFatalErrorHandler);
  ABI45_0_0RCTSetFatalExceptionHandler(_previousFatalExceptionHandler);
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:ABI45_0_0EXUpdatesErrorLogFile] path];
}
