 ...
 *
 *   ABI45_0_0EXUpdatesReleaseChannel
 *   ABI45_0_0EXUpdatesURL
 *
 * and all of the values in json
 *
 *   ABI45_0_0EXUpdatesRequestHeaders
 */
@implementation ABI45_0_0EXUpdatesBuildData

+ (void)ensureBuildDataIsConsistentAsync:(ABI45_0_0EXUpdatesDatabase *)database
...
{
  return @{
    @"ABI45_0_0EXUpdatesURL": config.updateUrl.absoluteString,
    @"ABI45_0_0EXUpdatesReleaseChannel": config.releaseChannel,
    @"ABI45_0_0EXUpdatesRequestHeaders": config.requestHeaders,
  };
}

+ (void)clearAllUpdatesAndSetStaticBuildData:(ABI45_0_0EXUpdatesDatabase *)database
                             config:(ABI45_0_0EXUpdatesConfig *)config
{
  NSError *queryError;
  NSArray<ABI45_0_0EXUpdatesUpdate *> *allUpdates = [database allUpdatesWithConfig:config error:&queryError];
  if (queryError) {
