...
NSString * const ABI45_0_0EXUpdatesEmbeddedManifestName = @"app";
NSString * const ABI45_0_0EXUpdatesEmbeddedManifestType = @"manifest";
NSString * const ABI45_0_0EXUpdatesEmbeddedBundleFilename = @"app";
NSString * const ABI45_0_0EXUpdatesEmbeddedBundleFileType = @"bundle";
NSString * const ABI45_0_0EXUpdatesBareEmbeddedBundleFilename = @"main";
NSString * const ABI45_0_0EXUpdatesBareEmbeddedBundleFileType = @"jsbundle";

static NSString * const ABI45_0_0EXUpdatesEmbeddedAppLoaderErrorDomain = @"ABI45_0_0EXUpdatesEmbeddedAppLoader";

@implementation ABI45_0_0EXUpdatesEmbeddedAppLoader
...
{
  static ABI45_0_0EXUpdatesUpdate *embeddedManifest;
  ...
  dispatch_once(&once, ^{
    if (!config.hasEmbeddedUpdate) {
      ...
      NSURL *bundleUrl = [frameworkBundle.resourceURL URLByAppendingPathComponent:@"ABI45_0_0EXUpdates.bundle"];
      NSBundle *bundle = [NSBundle bundleWithURL:bundleUrl];
      NSString *path = [bundle pathForResource:ABI45_0_0EXUpdatesEmbeddedManifestName ofType:ABI45_0_0EXUpdatesEmbeddedManifestType];
      NSData *manifestData = [NSData dataWithContentsOfFile:path];

      // Fallback to main bundle if the embedded manifest is not found in ABI45_0_0EXUpdates.bundle. This is a special case
      // to support the existing structure of Expo "shell apps"
      if (!manifestData) {
        path = [[NSBundle mainBundle] pathForResource:ABI45_0_0EXUpdatesEmbeddedManifestName ofType:ABI45_0_0EXUpdatesEmbeddedManifestType];
        manifestData = [NSData dataWithContentsOfFile:path];
      }

      // Not found in ABI45_0_0EXUpdates.bundle or main bundle
      if (!manifestData) {
      ...
      if (!manifest) {
        ...
        embeddedManifest = [ABI45_0_0EXUpdatesUpdate updateWithEmbeddedManifest:[mutableManifest copy]
                                                                config:config
                                                              database:database];
...
{
  ABI45_0_0EXUpdatesUpdate *embeddedManifest = [[self class] embeddedManifestWithConfig:self.config
  ...
  if (embeddedManifest) {
    ...
    error([NSError errorWithDomain:ABI45_0_0EXUpdatesEmbeddedAppLoaderErrorDomain
                              code:1008
...
- (void)downloadAsset:(ABI45_0_0EXUpdatesAsset *)asset
{
  ...
  dispatch_async([ABI45_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
    if ([[NSFileManager defaultManager] fileExistsAtPath:[destinationUrl path]]) {
      ...
      NSString *bundlePath = [ABI45_0_0EXUpdatesUtils pathForBundledAsset:asset];
...
- (void)loadUpdateFromUrl:(NSURL *)url
               onManifest:(ABI45_0_0EXUpdatesAppLoaderManifestBlock)manifestBlock
                  ...
                  success:(ABI45_0_0EXUpdatesAppLoaderSuccessBlock)success
                    error:(ABI45_0_0EXUpdatesAppLoaderErrorBlock)error
{
  @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Should not call ABI45_0_0EXUpdatesEmbeddedAppLoader#loadUpdateFromUrl" userInfo:nil];
}
