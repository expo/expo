...
static NSString * const ABI45_0_0EXUpdatesAppLoaderTaskErrorDomain = @"ABI45_0_0EXUpdatesAppLoaderTask";

@interface ABI45_0_0EXUpdatesAppLoaderTask ()
...
@property (nonatomic, strong) id<ABI45_0_0EXUpdatesAppLauncher> finalizedLauncher;
@property (nonatomic, strong) ABI45_0_0EXUpdatesEmbeddedAppLoader *embeddedAppLoader;
@property (nonatomic, strong) ABI45_0_0EXUpdatesRemoteAppLoader *remoteAppLoader;
...
@implementation ABI45_0_0EXUpdatesAppLoaderTask
...
- (void)start
{
  if (!_config.isEnabled) {
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:ABI45_0_0EXUpdatesAppLoaderTaskErrorDomain code:1030 userInfo:@{
                    NSLocalizedDescriptionKey: @"ABI45_0_0EXUpdatesAppLoaderTask was passed a configuration object with updates disabled. You should load updates from an embedded source rather than calling ABI45_0_0EXUpdatesAppLoaderTask, or enable updates in the configuration."
                  }]];
    });
    return;
  }
  ...
  if (!_config.updateUrl) {
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:ABI45_0_0EXUpdatesAppLoaderTaskErrorDomain code:1030 userInfo:@{
                    NSLocalizedDescriptionKey: @"ABI45_0_0EXUpdatesAppLoaderTask was passed a configuration object with a null URL. You must pass a nonnull URL in order to use ABI45_0_0EXUpdatesAppLoaderTask to load updates."
                  }]];
    });
    return;
  }
  ...
  if (!_directory) {
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:ABI45_0_0EXUpdatesAppLoaderTaskErrorDomain code:1030 userInfo:@{
                    NSLocalizedDescriptionKey: @"ABI45_0_0EXUpdatesAppLoaderTask directory must be nonnull."
                  }]];
    });
    return;
  }
  ...
  __block BOOL shouldCheckForUpdate = [ABI45_0_0EXUpdatesUtils shouldCheckForUpdateWithConfig:_config];
  ...
  [self _loadEmbeddedUpdateWithCompletion:^{
    [self _launchWithCompletion:^(NSError * _Nullable error, BOOL success) {
      ...
      if (shouldCheckForUpdate) {
        [self _loadRemoteUpdateWithCompletion:^(NSError * _Nullable error, ABI45_0_0EXUpdatesUpdate * _Nullable update) {
...
- (void)_finishWithError:(nullable NSError *)error
{
  ...
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
      if (self->_isReadyToLaunch && (self->_finalizedLauncher.launchAssetUrl || self->_finalizedLauncher.launchedUpdate.status == ABI45_0_0EXUpdatesUpdateStatusDevelopment)) {
        [self->_delegate appLoaderTask:self didFinishWithLauncher:self->_finalizedLauncher isUpToDate:self->_isUpToDate];
      } else {
        [self->_delegate appLoaderTask:self didFinishWithError:error ?: [NSError errorWithDomain:ABI45_0_0EXUpdatesAppLoaderTaskErrorDomain code:1031 userInfo:@{
          NSLocalizedDescriptionKey: @"ABI45_0_0EXUpdatesAppLoaderTask encountered an unexpected error and could not launch an update."
        }]];
      }
    });
  }
...
{
  [ABI45_0_0EXUpdatesAppLauncherWithDatabase launchableUpdateWithConfig:_config database:_database selectionPolicy:_selectionPolicy completion:^(NSError * _Nullable error, ABI45_0_0EXUpdatesUpdate * _Nullable launchableUpdate) {
    dispatch_async(self->_database.databaseQueue, ^{
      ...
      dispatch_async(self->_loaderTaskQueue, ^{
        ...
        if (self->_config.hasEmbeddedUpdate &&
            [self->_selectionPolicy shouldLoadNewUpdate:[ABI45_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:self->_config database:self->_database]
          ...
          // be sending an HTTP request from ABI45_0_0EXUpdatesEmbeddedAppLoader
          self->_embeddedAppLoader = [[ABI45_0_0EXUpdatesEmbeddedAppLoader alloc] initWithConfig:self->_config database:self->_database directory:self->_directory launchedUpdate:nil completionQueue:self->_loaderTaskQueue];
          [self->_embeddedAppLoader loadUpdateFromEmbeddedManifestWithCallback:^BOOL(ABI45_0_0EXUpdatesUpdate * _Nonnull update) {
            // we already checked using selection policy, so we don't need to check again
            return YES;
          } onAsset:^(ABI45_0_0EXUpdatesAsset *asset, NSUInteger successfulAssetCount, NSUInteger failedAssetCount, NSUInteger totalAssetCount) {
            // do nothing for now
...
{
  ABI45_0_0EXUpdatesAppLauncherWithDatabase *launcher = [[ABI45_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:_config database:_database directory:_directory completionQueue:_loaderTaskQueue];
  _candidateLauncher = launcher;
  [launcher launchUpdateWithSelectionPolicy:_selectionPolicy completion:completion];
}

- (void)_loadRemoteUpdateWithCompletion:(void (^)(NSError * _Nullable error, ABI45_0_0EXUpdatesUpdate * _Nullable update))completion
{
  _remoteAppLoader = [[ABI45_0_0EXUpdatesRemoteAppLoader alloc] initWithConfig:_config database:_database directory:_directory launchedUpdate:_candidateLauncher.launchedUpdate completionQueue:_loaderTaskQueue];
  [_remoteAppLoader loadUpdateFromUrl:_config.updateUrl onManifest:^BOOL(ABI45_0_0EXUpdatesUpdate * _Nonnull update) {
...
- (void)_handleRemoteUpdateLoaded:(nullable ABI45_0_0EXUpdatesUpdate *)update error:(nullable NSError *)error
{
  ...
  dispatch_async(_loaderTaskQueue, ^{
    ...
    if (update) {
      if (!self->_hasLaunched) {
        ABI45_0_0EXUpdatesAppLauncherWithDatabase *newLauncher = [[ABI45_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:self->_config database:self->_database directory:self->_directory completionQueue:self->_loaderTaskQueue];
        ...
        [self _didFinishBackgroundUpdateWithStatus:ABI45_0_0EXUpdatesBackgroundUpdateStatusUpdateAvailable manifest:update error:nil];
        self->_isRunning = NO;
        [self _runReaper];
      }
    } else {
      ...
      [self _finishWithError:error];
      if (error) {
        [self _didFinishBackgroundUpdateWithStatus:ABI45_0_0EXUpdatesBackgroundUpdateStatusError manifest:nil error:error];
      } else {
        [self _didFinishBackgroundUpdateWithStatus:ABI45_0_0EXUpdatesBackgroundUpdateStatusNoUpdateAvailable manifest:nil error:nil];
      }
      self->_isRunning = NO;
      [self _runReaper];
    }
...
- (void)_didFinishBackgroundUpdateWithStatus:(ABI45_0_0EXUpdatesBackgroundUpdateStatus)status manifest:(nullable ABI45_0_0EXUpdatesUpdate *)manifest error:(nullable NSError *)error
{
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
