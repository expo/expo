...
static NSString * const ABI45_0_0EXUpdatesExpoAssetBaseUrl = @"https://classic-assets.eascdn.net/~assets/";
static NSString * const ABI45_0_0EXUpdatesExpoIoDomain = @"expo.io";
static NSString * const ABI45_0_0EXUpdatesExpHostDomain = @"exp.host";
static NSString * const ABI45_0_0EXUpdatesExpoTestDomain = @"expo.test";

@implementation ABI45_0_0EXUpdatesLegacyUpdate
                                     ...
                                     database:(ABI45_0_0EXUpdatesDatabase *)database
{
  ...
  if (manifest.isUsingDeveloperTool) {
    ...
    update.commitTime = [ABI45_0_0RCTConvert NSDate:commitTimeString];
  }

  if (manifest.isDevelopmentMode) {
    update.isDevelopmentMode = YES;
    update.status = ABI45_0_0EXUpdatesUpdateStatusDevelopment;
  } else {
    update.status = ABI45_0_0EXUpdatesUpdateStatusPending;
  }
  ...
  ABI45_0_0EXUpdatesAsset *jsBundleAsset = [[ABI45_0_0EXUpdatesAsset alloc] initWithKey:bundleKey type:ABI45_0_0EXUpdatesEmbeddedBundleFileType];
  jsBundleAsset.url = bundleUrl;
  jsBundleAsset.isLaunchAsset = YES;
  jsBundleAsset.mainBundleFilename = ABI45_0_0EXUpdatesEmbeddedBundleFilename;
...
+ (NSURL *)bundledAssetBaseUrlWithManifest:(ABI45_0_0EXManifestsLegacyManifest *)manifest config:(ABI45_0_0EXUpdatesConfig *)config
{
  ...
  if (!host ||
      [host containsString:ABI45_0_0EXUpdatesExpoIoDomain] ||
      [host containsString:ABI45_0_0EXUpdatesExpHostDomain] ||
      [host containsString:ABI45_0_0EXUpdatesExpoTestDomain]) {
    return [NSURL URLWithString:ABI45_0_0EXUpdatesExpoAssetBaseUrl];
  } else {
