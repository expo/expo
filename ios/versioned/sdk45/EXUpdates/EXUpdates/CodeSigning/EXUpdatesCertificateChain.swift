...
@objc
public class ABI45_0_0EXUpdatesExpoProjectInformation : NSObject {
  ...
  public static func ==(lhs: ABI45_0_0EXUpdatesExpoProjectInformation, rhs: ABI45_0_0EXUpdatesExpoProjectInformation) -> Bool {
 ...
 */
class ABI45_0_0EXUpdatesCertificateChain {
  // ASN.1 path to the extended key usage info within a CERT
  static let ABI45_0_0EXUpdatesCodeSigningCertificateExtendedUsageCodeSigningOID = "1.3.6.1.5.5.7.3.3"
  // OID of expo project info, stored as `<projectId>,<scopeKey>`
  static let ABI45_0_0EXUpdatesCodeSigningCertificateExpoProjectInformationOID = "1.2.840.113556.1.8000.2554.43437.254.128.102.157.7894389.20439.2.1"
  ...
  public func codeSigningCertificate() throws -> Certificate {
    if (certificateStrings.isEmpty) {
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateEmptyError
    }
    
    let certificateChain = try certificateStrings.map { certificateString throws in
      try ABI45_0_0EXUpdatesCertificateChain.constructCertificate(certificateString: certificateString)
    }
    ...
    if (!x509LeafCertificate.isCodeSigningCertificate()) {
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateMissingCodeSigningError
    }
  ...
  private static func constructCertificate(certificateString: String) throws -> Certificate {
    guard let certificateData = certificateString.data(using: .utf8) else {
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateEncodingError
    }
    
    guard let certificateDataDer = decodeToDER(pem: certificateData) else {
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateDERDecodeError
    }
    
    let x509Certificate = try X509Certificate(der: certificateDataDer)
    
    guard x509Certificate.checkValidity() else {
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateValidityError
    }
...
extension X509Certificate {
  ...
  func isCodeSigningCertificate() -> Bool {
    ...
    if (!extendedKeyUsage.contains(ABI45_0_0EXUpdatesCertificateChain.ABI45_0_0EXUpdatesCodeSigningCertificateExtendedUsageCodeSigningOID)) {
      return false
    }
  ...
  func expoProjectInformation() throws -> ABI45_0_0EXUpdatesExpoProjectInformation? {
    guard let projectInformationExtensionValue = extensionObject(oid: ABI45_0_0EXUpdatesCertificateChain.ABI45_0_0EXUpdatesCodeSigningCertificateExpoProjectInformationOID)?.value else {
      return nil
    }
    ...
    if (components.count != 2) {
      throw ABI45_0_0EXUpdatesCodeSigningError.InvalidExpoProjectInformationExtensionValue
    }
    return ABI45_0_0EXUpdatesExpoProjectInformation(projectId: components[0], scopeKey: components[1])
  }
}
...
extension Array where Element == Certificate {
  func validateChain() throws {
    ...
    if (anchorX509Cert.subjectDistinguishedName != anchorX509Cert.issuerDistinguishedName) {
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateRootNotSelfSigned
    }
    ...
    if (count > 1) {
      ...
      if (!rootX509Cert.isCACertificate()) {
        throw ABI45_0_0EXUpdatesCodeSigningError.CertificateRootNotCA
      }
      ...
      for i in (0...(count - 2)).reversed() {
        ...
        if lastExpoProjectInformation != nil && lastExpoProjectInformation != currProjectInformation {
          throw ABI45_0_0EXUpdatesCodeSigningError.CertificateProjectInformationChainError
        }
...
extension SecTrust {
  static func create(certificates: [SecCertificate], policy: SecPolicy) throws -> SecTrust {
    ...
    guard let trust = optionalTrust, status.isSuccess else {
      ...
      throw ABI45_0_0EXUpdatesCodeSigningError.CertificateChainError
    }
    return trust
  }
