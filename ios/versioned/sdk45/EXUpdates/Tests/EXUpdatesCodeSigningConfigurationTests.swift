...
class ABI45_0_0EXUpdatesCodeSigningConfigurationTests : XCTestCase {
  func test_separateCertificateChain() throws {
    ...
    let chain1 = ABI45_0_0EXUpdatesCodeSigningConfiguration.separateCertificateChain(certificateChainInManifestResponse: leafCert)
    XCTAssertEqual(1, chain1.count)
    
    let chain2 = ABI45_0_0EXUpdatesCodeSigningConfiguration.separateCertificateChain(certificateChainInManifestResponse: leafCert + intermediateCert)
    XCTAssertEqual(2, chain2.count)
    
    let chain3 = ABI45_0_0EXUpdatesCodeSigningConfiguration.separateCertificateChain(certificateChainInManifestResponse: leafCert + intermediateCert + rootCert)
    XCTAssertEqual(3, chain3.count)
    
    let chainWithABunchOfNewlinesAndStuff  = ABI45_0_0EXUpdatesCodeSigningConfiguration.separateCertificateChain(certificateChainInManifestResponse: testCert + "\n\n\n\n" + testCert)
  ...
  func test_createAcceptSignatureHeader_CreatesSignatureHeaderDefaultValues() throws {
    ...
    let configuration = try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: cert,
                                                              metadata: [:],
  ...
  func test_createAcceptSignatureHeader_CreatesSignatureHeaderValuesFromConfig() throws {
    let cert = try TestHelper.getTestCertificate(TestCertificate.test)
    let configuration = try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: cert,
                                                              metadata: [ABI45_0_0EXUpdatesCodeSigningMetadataFields.AlgorithmFieldKey: ABI45_0_0EXUpdatesCodeSigningAlgorithm.RSA_SHA256.rawValue,
                                                                         ABI45_0_0EXUpdatesCodeSigningMetadataFields.KeyIdFieldKey: "test"],
                                                              includeManifestResponseCertificateChain: false,
                                                              allowUnsignedManifests: false)
    let signatureHeader = configuration.createAcceptSignatureHeader()
  ...
  func test_createAcceptSignatureHeader_CreatesSignatureHeaderEscapedValues() throws {
    ...
    let configuration = try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: cert,
                                                              metadata: [ABI45_0_0EXUpdatesCodeSigningMetadataFields.AlgorithmFieldKey: ABI45_0_0EXUpdatesCodeSigningAlgorithm.RSA_SHA256.rawValue,
                                                                         ABI45_0_0EXUpdatesCodeSigningMetadataFields.KeyIdFieldKey: #"test"hello\"#],
                                                              includeManifestResponseCertificateChain: false,
                                                              allowUnsignedManifests: false)
    let signatureHeader = configuration.createAcceptSignatureHeader()
  ...
  func test_createAcceptSignatureHeader_ThrowsInvalidAlg() throws {
    let cert = try TestHelper.getTestCertificate(TestCertificate.test)
    XCTAssertThrowsError(try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: cert,
                                                               metadata: [ABI45_0_0EXUpdatesCodeSigningMetadataFields.AlgorithmFieldKey: "fake",
                                                                          ABI45_0_0EXUpdatesCodeSigningMetadataFields.KeyIdFieldKey: "test"],
                                                               includeManifestResponseCertificateChain: false,
  ...
  func test_validateSignature_Valid() throws {
    ...
    XCTAssertEqual(signatureValidationResult.validationResult, ABI45_0_0EXUpdatesValidationResult.Valid)
  ...
  func test_validateSignature_ReturnsFalseWhenSignatureIsInvalid() throws {
    ...
    XCTAssertEqual(signatureValidationResult.validationResult, ABI45_0_0EXUpdatesValidationResult.Invalid)
    XCTAssertNil(signatureValidationResult.expoProjectInformation)
  }
  ...
  func test_validateSignature_ThrowsWhenKeyDoesNotMatch() throws {
    let cert = try TestHelper.getTestCertificate(TestCertificate.test)
    let configuration = try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: cert,
                                                              metadata: [ABI45_0_0EXUpdatesCodeSigningMetadataFields.KeyIdFieldKey: "test"],
                                                              includeManifestResponseCertificateChain: false,
                                                              allowUnsignedManifests: false)
    XCTAssertThrowsError(try configuration.validateSignature(signature: "sig=\"aGVsbG8=\", keyid=\"other\"", signedData: TestHelper.testBody.data(using: .utf8)!, manifestResponseCertificateChain: nil)) { error in
      XCTAssertEqual(error as? ABI45_0_0EXUpdatesCodeSigningError, ABI45_0_0EXUpdatesCodeSigningError.KeyIdMismatchError)
    }
  }
  ...
  func test_validateSignature_DoesNotUseChainInManifestResponseIfFlagIsFalse() throws {
    ...
    let leafCert = try TestHelper.getTestCertificate(TestCertificate.chainLeaf)
    let intermediateCert = try TestHelper.getTestCertificate(TestCertificate.chainIntermediate)
    let configuration = try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: testCert,
                                                              metadata: [:],
                                                              includeManifestResponseCertificateChain: false,
                                                              allowUnsignedManifests: false)
  ...
  func test_validateSignature_DoesUseChainInManifestResponseIfFlagIsTrue() throws {
    ...
    let intermediateCert = try TestHelper.getTestCertificate(TestCertificate.chainIntermediate)
    let rootCert = try TestHelper.getTestCertificate(TestCertificate.chainRoot)
    let configuration = try ABI45_0_0EXUpdatesCodeSigningConfiguration(embeddedCertificateString: rootCert,
                                                              metadata: [ABI45_0_0EXUpdatesCodeSigningMetadataFields.KeyIdFieldKey: "ca-root"],
  ...
  func test_validateSignature_AllowsUnsignedManifestIfAllowUnsignedFlagIsTrue() throws {
    ...
    XCTAssertEqual(signatureValidationResult.validationResult, ABI45_0_0EXUpdatesValidationResult.Skipped)
    XCTAssertNil(signatureValidationResult.expoProjectInformation)
  }
