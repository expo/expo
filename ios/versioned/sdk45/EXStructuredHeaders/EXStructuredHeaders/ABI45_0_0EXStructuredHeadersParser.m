...
#import <ABI45_0_0EXStructuredHeaders/ABI45_0_0EXStructuredHeadersParser.h>

NS_ASSUME_NONNULL_BEGIN

typedef NS_ENUM(NSInteger, ABI45_0_0EXStructuredHeadersParserNumberType) {
  ABI45_0_0EXStructuredHeadersParserNumberTypeInteger,
  ABI45_0_0EXStructuredHeadersParserNumberTypeDecimal
};

static NSString * const ABI45_0_0EXStructuredHeadersParserErrorDomain = @"ABI45_0_0EXStructuredHeadersParser";

@interface ABI45_0_0EXStructuredHeadersParser ()

@property (nonatomic, strong) NSString *raw;
@property (nonatomic, assign) NSUInteger position;

@property (nonatomic, assign) ABI45_0_0EXStructuredHeadersParserFieldType fieldType;
@property (nonatomic, assign) BOOL shouldIgnoreParameters;

@end

@implementation ABI45_0_0EXStructuredHeadersParser

- (instancetype)initWithRawInput:(NSString *)raw fieldType:(ABI45_0_0EXStructuredHeadersParserFieldType)fieldType
{
  return [self initWithRawInput:raw fieldType:fieldType ignoringParameters:NO];
}

- (instancetype)initWithRawInput:(NSString *)raw fieldType:(ABI45_0_0EXStructuredHeadersParserFieldType)fieldType ignoringParameters:(BOOL)shouldIgnoreParameters
{
  if (self = [super init]) {
    _raw = raw;
    _position = 0;
...
- (nullable id)parseStructuredFieldsWithError:(NSError ** _Nullable)error
{
  ...
  id output;
  switch (_fieldType) {
    case ABI45_0_0EXStructuredHeadersParserFieldTypeList:
      output = [self _parseAListWithError:error];
      break;
    case ABI45_0_0EXStructuredHeadersParserFieldTypeDictionary:
      output = [self _parseADictionaryWithError:error];
      break;
    case ABI45_0_0EXStructuredHeadersParserFieldTypeItem:
...
{
  // 4.2.4
  ABI45_0_0EXStructuredHeadersParserNumberType type = ABI45_0_0EXStructuredHeadersParserNumberTypeInteger;
  NSInteger sign = 1;
  ...
  while ([self hasRemaining]) {
    ...
    } else if (type == ABI45_0_0EXStructuredHeadersParserNumberTypeInteger && nextChar == '.') {
      if (inputNumber.length > 12) {
        if (error) *error = [self errorWithMessage:@"Decimal cannot have more than 12 digits before the decimal point"];
        return nil;
      }
      [inputNumber appendFormat:@"%c", nextChar];
      type = ABI45_0_0EXStructuredHeadersParserNumberTypeDecimal;
    } else {
      [self backout];
      break;
    }

    if (type == ABI45_0_0EXStructuredHeadersParserNumberTypeInteger && inputNumber.length > 15) {
      if (error) *error = [self errorWithMessage:@"Integer cannot have more than 15 digits"];
      return nil;
    } else if (type == ABI45_0_0EXStructuredHeadersParserNumberTypeDecimal && inputNumber.length > 16) {
      if (error) *error = [self errorWithMessage:@"Decimal cannot have more than 16 characters"];
      return nil;
    }
  }

  if (type == ABI45_0_0EXStructuredHeadersParserNumberTypeInteger) {
...
{
  return [NSError errorWithDomain:ABI45_0_0EXStructuredHeadersParserErrorDomain code:1 userInfo:@{
