...
@interface ABI45_0_0EXLegacyRemoteNotificationPermissionRequester ()
...
@property (nonatomic, weak) id<ABI45_0_0EXPermissionsRequester> userNotificationPermissionRequester;
@property (nonatomic, weak) dispatch_queue_t methodQueue;
@property (nonatomic, weak) id<ABI45_0_0EXRemoteNotificationPermissionProgressPublisher> permissionProgressPublisher;

@end

@implementation ABI45_0_0EXLegacyRemoteNotificationPermissionRequester
...
{
  __block ABI45_0_0EXPermissionStatus status;
  [ABI45_0_0EXUtilities performSynchronouslyOnMainThread:^{
    status = (ABI45_0_0UMSharedApplication().isRegisteredForRemoteNotifications) ?
    ABI45_0_0EXPermissionStatusGranted :
    ABI45_0_0EXPermissionStatusUndetermined;
  }];
...
- (void)requestPermissionsWithResolver:(ABI45_0_0EXPromiseResolveBlock)resolve rejecter:(ABI45_0_0EXPromiseRejectBlock)reject
{
  ...
  [ABI45_0_0EXUtilities performSynchronouslyOnMainThread:^{
    isRegisteredForRemoteNotifications = ABI45_0_0UMSharedApplication().isRegisteredForRemoteNotifications;
  }];
  ...
  if (isRegisteredForRemoteNotifications) {
    ...
    [_userNotificationPermissionRequester requestPermissionsWithResolver:^(NSDictionary *permission){
      ...
      ABI45_0_0EXPermissionStatus localNotificationsStatus = [[permission objectForKey:@"status"] intValue];
      // We may assume that `ABI45_0_0EXLocalNotificationRequester`'s permission request will always finish
      ...
      if (localNotificationsStatus == ABI45_0_0EXPermissionStatusDenied) {
        [self _clearObserver];
        [self _maybeConsumeResolverWithCurrentPermissions];
      } else {
        self.remoteNotificationsRegistrationIsPending = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
          [ABI45_0_0UMSharedApplication() registerForRemoteNotifications];
        });
      }
