...
#import <ABI45_0_0EXLocalAuthentication/ABI45_0_0EXLocalAuthentication.h>

typedef NS_ENUM(NSInteger, ABI45_0_0EXAuthenticationType) {
  ABI45_0_0EXAuthenticationTypeFingerprint = 1,
  ABI45_0_0EXAuthenticationTypeFacialRecognition = 2,
};

typedef NS_ENUM(NSInteger, ABI45_0_0EXSecurityLevel) {
  ABI45_0_0EXSecurityLevelNone = 0,
  ABI45_0_0EXSecurityLevelSecret = 1,
  ABI45_0_0EXSecurityLevelBiometric = 2,
};

@implementation ABI45_0_0EXLocalAuthentication

ABI45_0_0EX_EXPORT_MODULE(ExpoLocalAuthentication)

ABI45_0_0EX_EXPORT_METHOD_AS(supportedAuthenticationTypesAsync,
                    supportedAuthenticationTypesAsync:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  NSMutableArray *results = [NSMutableArray array];
  if ([[self class] isTouchIdDevice]) {
    [results addObject:@(ABI45_0_0EXAuthenticationTypeFingerprint)];
  }
  if ([[self class] isFaceIdDevice]) {
    [results addObject:@(ABI45_0_0EXAuthenticationTypeFacialRecognition)];
  }
  resolve(results);
}

ABI45_0_0EX_EXPORT_METHOD_AS(hasHardwareAsync,
                    hasHardwareAsync:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
...
ABI45_0_0EX_EXPORT_METHOD_AS(isEnrolledAsync,
                    isEnrolledAsync:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;
...
ABI45_0_0EX_EXPORT_METHOD_AS(getEnrolledLevelAsync,
                    getEnrolledLevelAsync:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;

  int level = ABI45_0_0EXSecurityLevelNone;
  ...
  if (isAuthenticationSupported && error == nil) {
    level = ABI45_0_0EXSecurityLevelSecret;
  }
  ...
  if (isBiometricsSupported && error == nil) {
    level = ABI45_0_0EXSecurityLevelBiometric;
  }
          ...
          localizedReason:reason
                    reply:^(BOOL success, NSError *error) {
                      resolve(@{
                                ...
                                @"warning": ABI45_0_0EXNullIfNil(warningMessage),
                                });
                    }];
  } else {
