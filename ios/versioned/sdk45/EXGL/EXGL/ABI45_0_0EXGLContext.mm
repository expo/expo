...
@interface ABI45_0_0EXGLContext ()

@property (nonatomic, strong) dispatch_queue_t glQueue;
@property (nonatomic, weak) ABI45_0_0EXModuleRegistry *moduleRegistry;
@property (nonatomic, weak) ABI45_0_0EXGLObjectManager *objectManager;
@property (nonatomic, assign) BOOL isContextReady;
@property (nonatomic, assign) BOOL wasPrepareCalled;

@end

@implementation ABI45_0_0EXGLContext

- (instancetype)initWithDelegate:(id<ABI45_0_0EXGLContextDelegate>)delegate andModuleRegistry:(nonnull ABI45_0_0EXModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    self.delegate = delegate;

    _moduleRegistry = moduleRegistry;
    _objectManager = (ABI45_0_0EXGLObjectManager *)[_moduleRegistry getExportedModuleOfClass:[ABI45_0_0EXGLObjectManager class]];
...
- (void)prepare:(void(^)(BOOL))callback
{
  ...
  id<ABI45_0_0EXJavaScriptContextProvider> jsContextProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI45_0_0EXJavaScriptContextProvider)];
  ...
  if (jsRuntimePtr) {
    ...
    [uiManager dispatchOnClientThread:^{
      ABI45_0_0EXGLContext *self = weakSelf;
      id<ABI45_0_0EXUIManager> uiManager = weakUIManager;
    ...
    BLOCK_SAFE_RUN(callback, NO);
    ABI45_0_0EXLogWarn(@"ABI45_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://reactnative.dev/docs/debugging)? ABI45_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI45_0_0EXGL.");
  }
}
  ...
  [self runAsync:^{
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI45_0_0EXErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI45_0_0EXErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = [ABI45_0_0EXUtilities screenScale];
    ...
    if ([format isEqualToString:@"webp"]) {
      ABI45_0_0EXLogWarn(@"iOS doesn't support 'webp' representation, so 'takeSnapshot' won't work with that format. The image is going to be exported as 'png', but consider using a different code for iOS. Check this docs to learn how to do platform specific code (https://reactnative.dev/docs/platform-specific-code)");
