...
void ABI45_0_0EXGLContext::prepareContext(jsi::Runtime &runtime, std::function<void(void)> flushMethod) {
  ...
  try {
    ...
    ABI45_0_0EXGLSysLog("Failed to setup ABI45_0_0EXGLContext [%s]", err.what());
  }
}
...
ABI45_0_0EXGLObjectId ABI45_0_0EXGLContext::createObject(void) noexcept {
  return nextObjectId++;
}

void ABI45_0_0EXGLContext::destroyObject(ABI45_0_0EXGLObjectId exglObjId) noexcept {
  objects.erase(exglObjId);
}

void ABI45_0_0EXGLContext::mapObject(ABI45_0_0EXGLObjectId exglObjId, GLuint glObj) noexcept {
  objects[exglObjId] = glObj;
}

GLuint ABI45_0_0EXGLContext::lookupObject(ABI45_0_0EXGLObjectId exglObjId) noexcept {
