...
ContextWithLock getContext(jsi::Runtime &runtime, const jsi::Value &jsThis) {
  ...
  return ContextGet(static_cast<ABI45_0_0EXGLContextId>(exglCtxId));
}
...
NATIVE_METHOD(getAttachedShaders) {
  ...
  for (auto i = 0; i < count; ++i) {
    ABI45_0_0EXGLObjectId exglObjId = 0;
    for (const auto &pair : ctx->objects) {
    ...
    if (exglObjId == 0) {
      throw std::runtime_error(
          "ABI45_0_0EXGL: Internal error: couldn't find ABI45_0_0EXGLObjectId "
