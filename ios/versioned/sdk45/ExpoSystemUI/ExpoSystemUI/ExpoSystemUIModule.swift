...
public class ExpoSystemUIModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    onCreate {
      ...
      let color = Bundle.main.object(forInfoDictionaryKey: "ABI45_0_0RCTRootViewBackgroundColor") as? Int
  ...
  static func getBackgroundColor() -> String? {
    var color: String?
    ABI45_0_0EXUtilities.performSynchronously {
      ...
      if let window = UIApplication.shared.delegate?.window, let backgroundColor = window?.rootViewController?.view.backgroundColor?.cgColor {
        color = ABI45_0_0EXUtilities.hexString(with: backgroundColor)
      }
    }
    return color
  }
  ...
  static func setBackgroundColorAsync(color: Int?) {
    ABI45_0_0EXUtilities.performSynchronously {
      ...
      let backgroundColor = ABI45_0_0EXUtilities.uiColor(color)
      // Set the app-wide window, this could have future issues when running multiple ABI45_0_0React apps,
