...
NSString * const ABI45_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI45_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI45_0_0EXTaskServiceInterface> taskService;

@end

@implementation ABI45_0_0EXTaskManager

ABI45_0_0EX_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI45_0_0EXTaskManagerInterface)];
}
...
// TODO: Remove when adding bare ABI45_0_0React Native support
...
{
  return @{
           @"EVENT_NAME": ABI45_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI45_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI45_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI45_0_0EX_EXPORT_METHOD_AS(isAvailableAsync,
                    isAvailable:(ABI45_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  resolve(@(_taskService != nil));
}

ABI45_0_0EX_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI45_0_0EX_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI45_0_0EX_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI45_0_0EX_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
  resolve(ABI45_0_0EXNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI45_0_0EX_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI45_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI45_0_0EXPromiseRejectBlock)reject)
{
...
# pragma mark - ABI45_0_0EXTaskManagerInterface
...
{
  return [ABI45_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI45_0_0EXTaskManagerEventName body:body];
  } else {
