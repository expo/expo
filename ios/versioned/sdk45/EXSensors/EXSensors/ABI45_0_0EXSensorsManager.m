...
#import <ABI45_0_0EXSensors/ABI45_0_0EXSensorsManager.h>

@interface ABI45_0_0EXSensorsManager ()
...
@implementation ABI45_0_0EXSensorsManager
                                            ...
                                            withHandler:(void (^)(NSDictionary *event))handlerBlock
{
  ...
  if (![[self manager] isAccelerometerActive]) {
    ...
    __weak ABI45_0_0EXSensorsManager *weakSelf = self;
    [[self manager] startAccelerometerUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMAccelerometerData *data, NSError *error) {
      __strong ABI45_0_0EXSensorsManager *strongSelf = weakSelf;
      if (strongSelf) {
  ...
  [[self manager] startDeviceMotionUpdatesUsingReferenceFrame:CMAttitudeReferenceFrameXArbitraryCorrectedZVertical toQueue:[NSOperationQueue mainQueue] withHandler:^(CMDeviceMotion *data, NSError *error) {
    ...
    NSMutableDictionary *result = [NSMutableDictionary dictionaryWithDictionary:@{
                             ...
                             @"accelerationIncludingGravity": @{
                                 @"x": @((data.userAcceleration.x + data.gravity.x) * ABI45_0_0EXGravity),
                                 @"y": @((data.userAcceleration.y + data.gravity.y) * ABI45_0_0EXGravity),
                                 @"z": @((data.userAcceleration.z + data.gravity.z) * ABI45_0_0EXGravity)
                                 },
                             @"rotation": @{
    ...
    if ([[strongSelf manager] isGyroAvailable]) {
      result[@"acceleration"] = @{
        @"x": @(data.userAcceleration.x * ABI45_0_0EXGravity),
        @"y": @(data.userAcceleration.y * ABI45_0_0EXGravity),
        @"z": @(data.userAcceleration.z * ABI45_0_0EXGravity)
      };
