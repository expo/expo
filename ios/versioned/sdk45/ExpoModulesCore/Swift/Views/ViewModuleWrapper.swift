...
/**
 Each module that has a view manager definition needs to be wrapped by `ABI45_0_0RCTViewManager`.
 Unfortunately, we can't use just one class because ABI45_0_0React Native checks for duplicated classes.
 We're generating its subclasses in runtime as a workaround.
 */
@objc
public final class ViewModuleWrapper: ABI45_0_0RCTViewManager, DynamicModuleWrapperProtocol {
  /**
  ...
  /**
   The designated initializer that is used by ABI45_0_0React Native to create module instances.
  ...
  /**
   Dummy initializer, for use only in `ABI45_0_0EXModuleRegistryAdapter.extraModulesForModuleRegistry:`.
   */
  @objc
  ...
  /**
   The config for `proxiedProperties` prop. In Objective-C style, this function is generated by `ABI45_0_0RCT_CUSTOM_VIEW_PROPERTY` macro.
   */
  @objc
  ...
  /**
   The setter for `proxiedProperties` prop. In Objective-C style, this function is generated by `ABI45_0_0RCT_CUSTOM_VIEW_PROPERTY` macro.
   */
  @objc
  public func set_proxiedProperties(_ json: Any, forView view: UIView, withDefaultView defaultView: UIView) {
    ...
    for (key, value) in json {
      if let prop = props[key] {
        ...
        // method signature in Objective-C. Maybe just call `ABI45_0_0RCTLogError`?
  ...
  public static func createViewModuleWrapperClass(module: ViewModuleWrapper) -> ViewModuleWrapper.Type? {
    ...
    return prefixedViewName.withCString { viewNamePtr in
      ...
      // ABI45_0_0React Native initializes modules with `init` without params,
...
let directEventBlockImplementation = imp_implementationWithBlock({ ["ABI45_0_0RCTDirectEventBlock"] } as @convention(block) () -> [String])
