...
@implementation ABI45_0_0EXReactNativeAdapter

ABI45_0_0EX_REGISTER_MODULE();

+ (NSString *)moduleName
{
  return @"ABI45_0_0EXReactNativeAdapter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI45_0_0EXAppLifecycleService), @protocol(ABI45_0_0EXUIManager), @protocol(ABI45_0_0EXJavaScriptContextProvider)];
}
...
{
  __weak ABI45_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI45_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI45_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI45_0_0RCTJSThread];
}
...
# pragma mark - ABI45_0_0EXJavaScriptContextProvider
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI45_0_0RCTContentDidAppearNotification,
                           ABI45_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI45_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI45_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI45_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI45_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
