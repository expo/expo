...
{
  if ([value isKindOfClass:[ABI45_0_0EXJavaScriptValue class]]) {
    return jsi::Value(runtime, *[(ABI45_0_0EXJavaScriptValue *)value get]);
  }
  if ([value isKindOfClass:[ABI45_0_0EXJavaScriptObject class]]) {
    return jsi::Value(runtime, *[(ABI45_0_0EXJavaScriptObject *)value get]);
  }
...
{
  NSMutableArray<ABI45_0_0EXJavaScriptValue *> *array = [NSMutableArray arrayWithCapacity:count];
  jsi::Runtime *jsiRuntime = [runtime get];

  for (int i = 0; i < count; i++) {
    std::shared_ptr<jsi::Value> value = std::make_shared<jsi::Value>(*jsiRuntime, values[i]);
    array[i] = [[ABI45_0_0EXJavaScriptValue alloc] initWithRuntime:runtime value:value];
  }
  return array;
}
