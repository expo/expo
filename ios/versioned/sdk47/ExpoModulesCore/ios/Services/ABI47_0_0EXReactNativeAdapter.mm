...
#import <ABI47_0_0React/ABI47_0_0RCTAppState.h>
#import <ABI47_0_0React/ABI47_0_0RCTImageLoader.h>

#import <ABI47_0_0ExpoModulesCore/ABI47_0_0EXReactNativeAdapter.h>
...
#import <ABI47_0_0ExpoModulesCore/ABI47_0_0ExpoFabricViewObjC.h>
#endif

@interface ABI47_0_0EXReactNativeAdapter ()
...
@implementation ABI47_0_0EXReactNativeAdapter

ABI47_0_0EX_REGISTER_MODULE();

+ (NSString *)moduleName
{
  return @"ABI47_0_0EXReactNativeAdapter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI47_0_0EXAppLifecycleService), @protocol(ABI47_0_0EXUIManager), @protocol(ABI47_0_0EXJavaScriptContextProvider)];
}
...
{
  __weak ABI47_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI47_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI47_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI47_0_0RCTJSThread];
}
...
# pragma mark - ABI47_0_0EXJavaScriptContextProvider
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI47_0_0RCTContentDidAppearNotification,
                           ABI47_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI47_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI47_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI47_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI47_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
...
- (void)executeUIBlock:(void (^)(UIView *view))block forView:(id)viewId
{
  ...
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    ...
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI47_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
#if RN_FABRIC_ENABLED
        ABI47_0_0RCTSurfacePresenter *surfacePresenter = strongSelf.bridge.surfacePresenter;
        UIView<ABI47_0_0RCTComponentViewProtocol> *componentView = [surfacePresenter.mountingManager.componentViewRegistry findComponentViewWithTag:[viewId integerValue]];
