...
@interface ABI47_0_0EXReactNativeEventEmitter ()
...
@implementation ABI47_0_0EXReactNativeEventEmitter

ABI47_0_0RCT_EXPORT_MODULE(ABI47_0_0EXReactNativeEventEmitter)
...
{
  return @[@protocol(ABI47_0_0EXEventEmitterService)];
}
...
- (NSArray<NSString *> *)supportedEvents
{
  ...
  for (ABI47_0_0EXExportedModule *exportedModule in [_exModuleRegistry getAllExportedModules]) {
    if ([exportedModule conformsToProtocol:@protocol(ABI47_0_0EXEventEmitter)]) {
      id<ABI47_0_0EXEventEmitter> eventEmitter = (id<ABI47_0_0EXEventEmitter>)exportedModule;
...
ABI47_0_0RCT_EXPORT_METHOD(addProxiedListener:(NSString *)moduleName eventName:(NSString *)eventName)
{
  ...
  if (ABI47_0_0RCT_DEBUG && module == nil) {
    ABI47_0_0EXLogError(@"Module for name `%@` has not been found.", moduleName);
    return;
  } else if (ABI47_0_0RCT_DEBUG && ![module conformsToProtocol:@protocol(ABI47_0_0EXEventEmitter)]) {
    ABI47_0_0EXLogError(@"Module `%@` is not an ABI47_0_0EXEventEmitter, thus it cannot be subscribed to.", moduleName);
    return;
  }

  // Validate eventEmitter
  id<ABI47_0_0EXEventEmitter> eventEmitter = (id<ABI47_0_0EXEventEmitter>)module;

  if (ABI47_0_0RCT_DEBUG && ![[eventEmitter supportedEvents] containsObject:eventName]) {
    ABI47_0_0EXLogError(@"`%@` is not a supported event type for %@. Supported events are: `%@`",
...
ABI47_0_0RCT_EXPORT_METHOD(removeProxiedListeners:(NSString *)moduleName count:(double)count)
{
  ...
  if (newModuleListenersCount == 0) {
    ...
    ABI47_0_0EXLogError(@"Attempted to remove more `%@` listeners than added", moduleName);
  ...
  if (_listenersCount - count < 0) {
    ABI47_0_0EXLogError(@"Attempted to remove more proxied event emitter listeners than added");
    _listenersCount = 0;
  } else {
...
- (void)setModuleRegistry:(ABI47_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  // See https://github.com/facebook/react-native/blob/2c2b83171603b47e5eec61eea55139f760dba090/ABI47_0_0React/Base/ABI47_0_0RCTModuleData.mm#L274-L289.
  if ([moduleRegistry isKindOfClass:[ABI47_0_0EXModuleRegistry class]]) {
