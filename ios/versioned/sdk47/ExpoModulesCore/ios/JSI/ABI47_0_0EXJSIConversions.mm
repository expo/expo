...
{
  if ([value isKindOfClass:[ABI47_0_0EXJavaScriptValue class]]) {
    return jsi::Value(runtime, *[(ABI47_0_0EXJavaScriptValue *)value get]);
  }
  if ([value isKindOfClass:[ABI47_0_0EXJavaScriptObject class]]) {
    return jsi::Value(runtime, *[(ABI47_0_0EXJavaScriptObject *)value get]);
  }
  if ([value isKindOfClass:[ABI47_0_0EXJavaScriptWeakObject class]]) {
    return jsi::Value(runtime, *[[(ABI47_0_0EXJavaScriptWeakObject *)value lock] get]);
  }
...
{
  NSMutableArray<ABI47_0_0EXJavaScriptValue *> *array = [NSMutableArray arrayWithCapacity:count];
  jsi::Runtime *jsiRuntime = [runtime get];

  for (int i = 0; i < count; i++) {
    std::shared_ptr<jsi::Value> value = std::make_shared<jsi::Value>(*jsiRuntime, values[i]);
    array[i] = [[ABI47_0_0EXJavaScriptValue alloc] initWithRuntime:runtime value:value];
  }
  return array;
}
