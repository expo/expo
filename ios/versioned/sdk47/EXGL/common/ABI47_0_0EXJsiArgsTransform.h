...
// if T = ABI47_0_0EXWebGLClass then return_type = ABI47_0_0EXGLObjectId else return_type = T
template <typename T>
using type_map = typename std::conditional<std::is_same_v<ABI47_0_0EXWebGLClass, T>, ABI47_0_0EXGLObjectId, T>::type;
...
template <>
inline ABI47_0_0EXGLObjectId unpackArg<ABI47_0_0EXWebGLClass>(jsi::Runtime &runtime, const jsi::Value *jsArgv) {
  if (!jsArgv->isObject() || !jsArgv->asObject(runtime).hasProperty(runtime, "id")) {
    return 0;
  }
  return static_cast<ABI47_0_0EXGLObjectId>(
