...
static NSString * const ABI47_0_0EXUpdatesAppLoaderTaskErrorDomain = @"ABI47_0_0EXUpdatesAppLoaderTask";

@interface ABI47_0_0EXUpdatesAppLoaderTask ()
...
@property (nonatomic, strong) ABI47_0_0EXUpdatesSelectionPolicy * selectionPolicy;
@property (nonatomic, strong) dispatch_queue_t delegateQueue;

@property (nonatomic, strong) id<ABI47_0_0EXUpdatesAppLauncher> candidateLauncher;
@property (nonatomic, strong) id<ABI47_0_0EXUpdatesAppLauncher> finalizedLauncher;
@property (nonatomic, strong) ABI47_0_0EXUpdatesEmbeddedAppLoader *embeddedAppLoader;
@property (nonatomic, strong) ABI47_0_0EXUpdatesRemoteAppLoader *remoteAppLoader;
 ...
 *
 * - Immediately starts an instance of ABI47_0_0EXUpdatesEmbeddedAppLoader to load the embedded update into
 ...
 *   the `launchWaitMs` value in ABI47_0_0EXUpdatesConfig.
 * - Again if the app is configured for automatic update downloads, starts an instance of
 *   ABI47_0_0EXUpdatesRemoteAppLoader to check for and download a new update if there is one.
 * - Once the download succeeds, fails, or the timer runs out (whichever happens first), creates an
 *   instance of ABI47_0_0EXUpdatesAppLauncherWithDatabase and signals that the app is ready to be launched
 ...
 *   be sent to JS.
 */
@implementation ABI47_0_0EXUpdatesAppLoaderTask
...
- (void)start
{
  if (!_config.isEnabled) {
    NSString *errorMessage = @"ABI47_0_0EXUpdatesAppLoaderTask was passed a configuration object with updates disabled. You should load updates from an embedded source rather than calling ABI47_0_0EXUpdatesAppLoaderTask, or enable updates in the configuration.";
    [self->_logger error:errorMessage code:ABI47_0_0EXUpdatesErrorCodeUpdateFailedToLoad];
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:ABI47_0_0EXUpdatesAppLoaderTaskErrorDomain code:1030 userInfo:@{
  ...
  if (!_config.updateUrl) {
    NSString *errorMessage = @"ABI47_0_0EXUpdatesAppLoaderTask was passed a configuration object with a null URL. You must pass a nonnull URL in order to use ABI47_0_0EXUpdatesAppLoaderTask to load updates.";
    [self->_logger error:errorMessage code:ABI47_0_0EXUpdatesErrorCodeUpdateFailedToLoad];
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:ABI47_0_0EXUpdatesAppLoaderTaskErrorDomain code:1030 userInfo:@{
  ...
  if (!_directory) {
    NSString *errorMessage = @"ABI47_0_0EXUpdatesAppLoaderTask directory must be nonnull.";
    [self->_logger error:errorMessage code:ABI47_0_0EXUpdatesErrorCodeUpdateFailedToLoad];
    dispatch_async(_delegateQueue, ^{
      [self->_delegate appLoaderTask:self
                  didFinishWithError:[NSError errorWithDomain:ABI47_0_0EXUpdatesAppLoaderTaskErrorDomain code:1030 userInfo:@{
  ...
  __block BOOL shouldCheckForUpdate = [ABI47_0_0EXUpdatesUtils shouldCheckForUpdateWithConfig:_config];
  ...
  [self _loadEmbeddedUpdateWithCompletion:^{
    [self _launchWithCompletion:^(NSError * _Nullable error, BOOL success) {
      ...
      if (shouldCheckForUpdate) {
        [self _loadRemoteUpdateWithCompletion:^(NSError * _Nullable error, ABI47_0_0EXUpdatesUpdate * _Nullable update) {
...
- (void)_finishWithError:(nullable NSError *)error
{
  ...
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
      if (self->_isReadyToLaunch && (self->_finalizedLauncher.launchAssetUrl || self->_finalizedLauncher.launchedUpdate.status == ABI47_0_0EXUpdatesUpdateStatusDevelopment)) {
        [self->_delegate appLoaderTask:self didFinishWithLauncher:self->_finalizedLauncher isUpToDate:self->_isUpToDate];
      } else {
        [self->_delegate appLoaderTask:self didFinishWithError:error ?: [NSError errorWithDomain:ABI47_0_0EXUpdatesAppLoaderTaskErrorDomain code:1031 userInfo:@{
          NSLocalizedDescriptionKey: @"ABI47_0_0EXUpdatesAppLoaderTask encountered an unexpected error and could not launch an update."
        }]];
      }
    });
  }
...
{
  [ABI47_0_0EXUpdatesAppLauncherWithDatabase launchableUpdateWithConfig:_config database:_database selectionPolicy:_selectionPolicy completion:^(NSError * _Nullable error, ABI47_0_0EXUpdatesUpdate * _Nullable launchableUpdate) {
    dispatch_async(self->_database.databaseQueue, ^{
      ...
      dispatch_async(self->_loaderTaskQueue, ^{
        ...
        if (self->_config.hasEmbeddedUpdate &&
            [self->_selectionPolicy shouldLoadNewUpdate:[ABI47_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:self->_config database:self->_database]
          ...
          // be sending an HTTP request from ABI47_0_0EXUpdatesEmbeddedAppLoader
          self->_embeddedAppLoader = [[ABI47_0_0EXUpdatesEmbeddedAppLoader alloc] initWithConfig:self->_config database:self->_database directory:self->_directory launchedUpdate:nil completionQueue:self->_loaderTaskQueue];
          [self->_embeddedAppLoader loadUpdateFromEmbeddedManifestWithCallback:^BOOL(ABI47_0_0EXUpdatesUpdate * _Nonnull update) {
            // we already checked using selection policy, so we don't need to check again
            return YES;
          } onAsset:^(ABI47_0_0EXUpdatesAsset *asset, NSUInteger successfulAssetCount, NSUInteger failedAssetCount, NSUInteger totalAssetCount) {
            // do nothing for now
...
{
  ABI47_0_0EXUpdatesAppLauncherWithDatabase *launcher = [[ABI47_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:_config database:_database directory:_directory completionQueue:_loaderTaskQueue];
  _candidateLauncher = launcher;
  [launcher launchUpdateWithSelectionPolicy:_selectionPolicy completion:completion];
}

- (void)_loadRemoteUpdateWithCompletion:(void (^)(NSError * _Nullable error, ABI47_0_0EXUpdatesUpdate * _Nullable update))completion
{
  _remoteAppLoader = [[ABI47_0_0EXUpdatesRemoteAppLoader alloc] initWithConfig:_config database:_database directory:_directory launchedUpdate:_candidateLauncher.launchedUpdate completionQueue:_loaderTaskQueue];
  [_remoteAppLoader loadUpdateFromUrl:_config.updateUrl onManifest:^BOOL(ABI47_0_0EXUpdatesUpdate * _Nonnull update) {
...
- (void)_handleRemoteUpdateLoaded:(nullable ABI47_0_0EXUpdatesUpdate *)update error:(nullable NSError *)error
{
  ...
  dispatch_async(_loaderTaskQueue, ^{
    ...
    if (update) {
      if (!self->_hasLaunched) {
        ABI47_0_0EXUpdatesAppLauncherWithDatabase *newLauncher = [[ABI47_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:self->_config database:self->_database directory:self->_directory completionQueue:self->_loaderTaskQueue];
        ...
        [self _didFinishBackgroundUpdateWithStatus:ABI47_0_0EXUpdatesBackgroundUpdateStatusUpdateAvailable manifest:update error:nil];
        self->_isRunning = NO;
        [self _runReaper];
      }
    } else {
      ...
      [self _finishWithError:error];
      if (error) {
        [self _didFinishBackgroundUpdateWithStatus:ABI47_0_0EXUpdatesBackgroundUpdateStatusError manifest:nil error:error];
      } else {
        [self _didFinishBackgroundUpdateWithStatus:ABI47_0_0EXUpdatesBackgroundUpdateStatusNoUpdateAvailable manifest:nil error:nil];
      }
      self->_isRunning = NO;
      [self _runReaper];
    }
...
- (void)_didFinishBackgroundUpdateWithStatus:(ABI47_0_0EXUpdatesBackgroundUpdateStatus)status manifest:(nullable ABI47_0_0EXUpdatesUpdate *)manifest error:(nullable NSError *)error
{
  if (_delegate) {
    dispatch_async(_delegateQueue, ^{
