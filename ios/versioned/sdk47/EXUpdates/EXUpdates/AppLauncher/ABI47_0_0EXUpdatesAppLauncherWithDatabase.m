...
#import <ABI47_0_0EXUpdates/ABI47_0_0EXUpdatesAppLauncherWithDatabase+Tests.h>
...
@interface ABI47_0_0EXUpdatesAppLauncherWithDatabase ()

@property (nullable, nonatomic, strong, readwrite) ABI47_0_0EXUpdatesUpdate *launchedUpdate;
...
@property (nonatomic, copy) ABI47_0_0EXUpdatesAppLauncherCompletionBlock completion;
...
static NSString * const ABI47_0_0EXUpdatesAppLauncherErrorDomain = @"AppLauncher";

/**
 * Implementation of ABI47_0_0EXUpdatesAppLauncher that uses the SQLite database and expo-updates file store
 * as the source of updates.
 *
 * Uses the ABI47_0_0EXUpdatesSelectionPolicy to choose an update from SQLite to launch, then ensures that
 ...
 * Distinct from the ABI47_0_0EXUpdatesAppLoader classes, though, this class does *not* make any major
 ...
 */
@implementation ABI47_0_0EXUpdatesAppLauncherWithDatabase
                   ...
                   completionQueue:(dispatch_queue_t)completionQueue
{
  dispatch_async(database.databaseQueue, ^{
    NSError *error;
    NSArray<ABI47_0_0EXUpdatesUpdate *> *launchableUpdates = [database launchableUpdatesWithConfig:config error:&error];
    ...
    dispatch_async(completionQueue, ^{
      ...
      ABI47_0_0EXUpdatesUpdate *embeddedManifest = [ABI47_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:config database:database];
      NSMutableArray<ABI47_0_0EXUpdatesUpdate *>*filteredLaunchableUpdates = [NSMutableArray new];
      for (ABI47_0_0EXUpdatesUpdate *update in launchableUpdates) {
        if (update.status == ABI47_0_0EXUpdatesUpdateStatusEmbedded) {
...
{
  NSAssert(!_completion, @"ABI47_0_0EXUpdatesAppLauncher:launchUpdateWithSelectionPolicy:successBlock should not be called twice on the same instance");
  _completion = completion;

  if (!_launchedUpdate) {
    [[self class] launchableUpdateWithConfig:_config database:_database selectionPolicy:selectionPolicy completion:^(NSError * _Nullable error, ABI47_0_0EXUpdatesUpdate * _Nullable launchableUpdate) {
      if (error || !launchableUpdate) {
        if (self->_completion) {
          dispatch_async(self->_completionQueue, ^{
            ...
            self->_completion([NSError errorWithDomain:ABI47_0_0EXUpdatesAppLauncherErrorDomain code:1011 userInfo:userInfo], NO);
          });
        }
      } else {
                       ...
                       completion:(ABI47_0_0EXUpdatesAppLauncherQueryCompletionBlock)completionBlock
{
  dispatch_async(database.databaseQueue,^{
    ...
    NSError *dbError = nil;
    readyUpdateIds = [database allUpdateIdsWithStatus:ABI47_0_0EXUpdatesUpdateStatusReady error:&dbError];
    if (dbError != nil) {
...
{
  if (_launchedUpdate.status == ABI47_0_0EXUpdatesUpdateStatusEmbedded) {
    NSAssert(_assetFilesMap == nil, @"assetFilesMap should be null for embedded updates");
    _launchAssetUrl = [[NSBundle mainBundle] URLForResource:ABI47_0_0EXUpdatesBareEmbeddedBundleFilename withExtension:ABI47_0_0EXUpdatesBareEmbeddedBundleFileType];
    ...
    return;
  } else if (_launchedUpdate.status == ABI47_0_0EXUpdatesUpdateStatusDevelopment) {
    dispatch_async(self->_completionQueue, ^{
  ...
  if (_launchedUpdate) {
    ...
    for (ABI47_0_0EXUpdatesAsset *asset in _launchedUpdate.assets) {
...
- (void)_ensureAssetExists:(ABI47_0_0EXUpdatesAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
  [self _checkExistenceOfAsset:asset withLocalUrl:assetLocalUrl completion:^(BOOL exists) {
    ...
    [self _maybeCopyAssetFromMainBundle:asset withLocalUrl:assetLocalUrl completion:^(BOOL success, NSError * _Nullable error) {
      ...
      [self _downloadAsset:asset withLocalUrl:assetLocalUrl completion:^(NSError * _Nullable error, ABI47_0_0EXUpdatesAsset *asset, NSURL *assetLocalUrl) {
        if (error) {
          if (asset.isLaunchAsset) {
...
- (void)_checkExistenceOfAsset:(ABI47_0_0EXUpdatesAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
...
- (void)_maybeCopyAssetFromMainBundle:(ABI47_0_0EXUpdatesAsset *)asset
                         withLocalUrl:(NSURL *)assetLocalUrl
                           completion:(void (^)(BOOL success, NSError * _Nullable error))completion
{
  ABI47_0_0EXUpdatesUpdate *embeddedManifest = [ABI47_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:_config database:_database];
  if (embeddedManifest) {
    ABI47_0_0EXUpdatesAsset *matchingAsset;
    for (ABI47_0_0EXUpdatesAsset *embeddedAsset in embeddedManifest.assets) {
    ...
    if (matchingAsset && matchingAsset.mainBundleFilename) {
      dispatch_async([ABI47_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
        NSString *bundlePath = [ABI47_0_0EXUpdatesUtils pathForBundledAsset:matchingAsset];
        if (bundlePath == nil) {
          dispatch_async(self->_launcherQueue, ^{
            completion(NO, [NSError errorWithDomain:ABI47_0_0EXUpdatesAppLauncherErrorDomain code:1013 userInfo:@{NSLocalizedDescriptionKey: @"Asset bundlePath was unexpectedly nil"}]);
          });
          return;
        }
        NSError *error;
...
- (void)_downloadAsset:(ABI47_0_0EXUpdatesAsset *)asset
          withLocalUrl:(NSURL *)assetLocalUrl
            completion:(void (^)(NSError * _Nullable error, ABI47_0_0EXUpdatesAsset *asset, NSURL *assetLocalUrl))completion
{
  if (!asset.url) {
    completion([NSError errorWithDomain:ABI47_0_0EXUpdatesAppLauncherErrorDomain code:1007 userInfo:@{NSLocalizedDescriptionKey: @"Failed to download asset with no URL provided"}], asset, assetLocalUrl);
  }
  dispatch_async([ABI47_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
...
- (ABI47_0_0EXUpdatesFileDownloader *)downloader
{
  if (!_downloader) {
    _downloader = [[ABI47_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:_config];
  }
  return _downloader;
}
