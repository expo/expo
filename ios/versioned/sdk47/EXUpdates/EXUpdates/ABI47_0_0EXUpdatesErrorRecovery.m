...
typedef NS_ENUM(NSInteger, ABI47_0_0EXUpdatesErrorRecoveryTask) {
  ABI47_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate,
  ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchNew,
  ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchCached,
  ABI47_0_0EXUpdatesErrorRecoveryTaskCrash
};

static NSString * const ABI47_0_0EXUpdatesErrorLogFile = @"expo-error.log";
static NSInteger const ABI47_0_0EXUpdatesErrorRecoveryRemoteLoadTimeoutMs = 5000;

@interface ABI47_0_0EXUpdatesErrorRecovery ()
...
@property (nonatomic, copy) ABI47_0_0RCTFatalHandler previousFatalErrorHandler;
@property (nonatomic, copy) ABI47_0_0RCTFatalExceptionHandler previousFatalExceptionHandler;
...
- (void)notifyNewRemoteLoadStatus:(ABI47_0_0EXUpdatesRemoteLoadStatus)newStatus
{
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    if (newStatus != ABI47_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded) {
      [self->_pipeline removeObject:@(ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchNew)];
    }
    [self _runNextTask];
  });
}
...
- (void)_startPipelineWithEncounteredError:(id)encounteredError
{
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    if (self->_delegate.launchedUpdate.successfulLaunchCount > 0) {
      [self->_pipeline removeObject:@(ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchCached)];
...
- (void)_runNextTask
{
  ...
  switch ((ABI47_0_0EXUpdatesErrorRecoveryTask)nextTask.integerValue) {
    case ABI47_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate:
      [self->_logger info:@"ABI47_0_0EXUpdatesErrorRecovery: attempting to fetch a new update, waiting"];
      [self _waitForRemoteLoaderToFinish];
      break;
    // ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchNew is called only after a new update is downloaded
    // and added to the cache, so it is equivalent to ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchCached
    case ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchNew:
    case ABI47_0_0EXUpdatesErrorRecoveryTaskLaunchCached:
      [self->_logger info:@"ABI47_0_0EXUpdatesErrorRecovery: launching a new or cached update"];
      [self _tryRelaunchFromCache];
      break;
    case ABI47_0_0EXUpdatesErrorRecoveryTaskCrash:
      [self->_logger error:@"ABI47_0_0EXUpdatesErrorRecovery: could not recover from error, crashing" code:ABI47_0_0EXUpdatesErrorCodeUpdateFailedToLoad];
      [self _crash];
      break;
    default:
      @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"ABI47_0_0EXUpdatesErrorRecovery cannot run the next task" userInfo:@{@"nextTaskValue": nextTask}];
  }
}
...
{
  dispatch_assert_queue(_errorRecoveryQueue);
  if (_delegate.remoteLoadStatus == ABI47_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded) {
    [self _runNextTask];
  } else if (_delegate.config.checkOnLaunch != ABI47_0_0EXUpdatesCheckAutomaticallyConfigNever ||
             _delegate.remoteLoadStatus == ABI47_0_0EXUpdatesRemoteLoadStatusLoading) {
    _isWaitingForRemoteUpdate = YES;
    if (_delegate.remoteLoadStatus != ABI47_0_0EXUpdatesRemoteLoadStatusLoading) {
...
- (void)_crash
{
  ...
  if ([initialError isKindOfClass:[NSError class]]) {
    // format these keys similar to RN -- ABI47_0_0RCTFatal in ABI47_0_0RCTAssert.m
    name = [NSString stringWithFormat:@"%@: %@", ABI47_0_0RCTFatalExceptionName, ((NSError *)initialError).localizedDescription];
    reason = ABI47_0_0RCTFormatError(((NSError *)initialError).localizedDescription, ((NSError *)initialError).userInfo[ABI47_0_0RCTJSStackTraceKey], 175);
    userInfo = [((NSError *)initialError).userInfo mutableCopy];
    userInfo[@"ABI47_0_0RCTUntruncatedMessageKey"] = ABI47_0_0RCTFormatError(((NSError *)initialError).localizedDescription, ((NSError *)initialError).userInfo[ABI47_0_0RCTJSStackTraceKey], -1);
  ...
  userInfo[@"ABI47_0_0EXUpdatesLaterEncounteredErrors"] = [_encounteredErrors copy];
...
{
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleJavaScriptDidFailToLoad) name:ABI47_0_0RCTJavaScriptDidFailToLoadNotification object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleContentDidAppear) name:ABI47_0_0RCTContentDidAppearNotification object:nil];
}

- (void)_unregisterObservers
{
  [[NSNotificationCenter defaultCenter] removeObserver:self name:ABI47_0_0RCTJavaScriptDidFailToLoadNotification object:nil];
  [[NSNotificationCenter defaultCenter] removeObserver:self name:ABI47_0_0RCTContentDidAppearNotification object:nil];
}
...
- (void)_handleContentDidAppear
{
  ...
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    self->_pipeline = [self->_pipeline objectsAtIndexes:[self->_pipeline indexesOfObjectsPassingTest:^BOOL(NSNumber *obj, NSUInteger idx, BOOL *stop) {
      return obj.integerValue == ABI47_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate || obj.integerValue == ABI47_0_0EXUpdatesErrorRecoveryTaskCrash;
    }]].mutableCopy;
  });
  ...
  // ABI47_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate task and check for a new update is there is one
...
- (void)_setRCTErrorHandlers
{
  ...
  _previousFatalErrorHandler = ABI47_0_0RCTGetFatalHandler();
  _previousFatalExceptionHandler = ABI47_0_0RCTGetFatalExceptionHandler();

  ABI47_0_0RCTFatalHandler fatalErrorHandler = ^(NSError *error) {
    [self handleError:error];
  };
  ABI47_0_0RCTFatalExceptionHandler fatalExceptionHandler = ^(NSException *exception) {
    [self handleException:exception];
  };
  ABI47_0_0RCTSetFatalHandler(fatalErrorHandler);
  ABI47_0_0RCTSetFatalExceptionHandler(fatalExceptionHandler);
}

- (void)_unsetRCTErrorHandlers
{
  ABI47_0_0RCTSetFatalHandler(_previousFatalErrorHandler);
  ABI47_0_0RCTSetFatalExceptionHandler(_previousFatalExceptionHandler);
...
- (void)writeErrorOrExceptionToLog:(id)errorOrException
{
  dispatch_async(_diskWriteQueue ?: dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    [self->_logger error:[NSString stringWithFormat:@"ABI47_0_0EXUpdatesErrorRecovery fatal exception: %@", serializedError]
                    code:ABI47_0_0EXUpdatesErrorCodeJsRuntimeError];
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:ABI47_0_0EXUpdatesErrorLogFile] path];
}
