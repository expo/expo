...
static NSString * const ABI47_0_0EXUpdatesEventName = @"Expo.nativeUpdatesEvent";
static NSString * const ABI47_0_0EXUpdatesUtilsErrorDomain = @"ABI47_0_0EXUpdatesUtils";

/**
 * Miscellaneous helper functions that are used by multiple classes in the library.
 */
@implementation ABI47_0_0EXUpdatesUtils
...
+ (nullable NSURL *)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  ...
  if (exists) {
    if (!isDir) {
      *error = [NSError errorWithDomain:ABI47_0_0EXUpdatesUtilsErrorDomain code:1005 userInfo:@{NSLocalizedDescriptionKey: @"Failed to create the Updates Directory; a file already exists with the required directory name"}];
      return nil;
    }
  } else {
    NSError *err;
...
+ (void)sendEventToBridge:(nullable ABI47_0_0RCTBridge *)bridge withType:(NSString *)eventType body:(NSDictionary *)body
{
  if (bridge) {
    ...
    [bridge enqueueJSCall:@"ABI47_0_0RCTDeviceEventEmitter.emit" args:@[ABI47_0_0EXUpdatesEventName, mutableBody]];
  } else {
    NSLog(@"ABI47_0_0EXUpdates: Could not emit %@ event. Did you set the bridge property on the controller singleton?", eventType);
  }
}
...
+ (BOOL)shouldCheckForUpdateWithConfig:(ABI47_0_0EXUpdatesConfig *)config
{
  switch (config.checkOnLaunch) {
    case ABI47_0_0EXUpdatesCheckAutomaticallyConfigNever:
      return NO;
    case ABI47_0_0EXUpdatesCheckAutomaticallyConfigErrorRecoveryOnly:
      // check will happen later on if there's an error
      return NO;
    case ABI47_0_0EXUpdatesCheckAutomaticallyConfigWifiOnly: {
    ...
    case ABI47_0_0EXUpdatesCheckAutomaticallyConfigAlways:
    default:
      return YES;
  }
}
...
{
  ABI47_0_0EXUpdatesLogReader *logReader = [ABI47_0_0EXUpdatesLogReader new];
  [logReader purgeLogEntries:^(NSError *error) {
    if (error) {
      NSLog(@"ABI47_0_0EXUpdatesUtils: error in purgeOldUpdatesLogs: %@", [error localizedDescription]);
    }
  }];
}
...
{
#if ABI47_0_0EX_UPDATES_NATIVE_DEBUG
  return YES;
#else
  return NO;
#endif
