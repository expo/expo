...
typedef void (^ABI46_0_0EXUpdatesErrorBlock) (NSError *error);
typedef void (^ABI46_0_0EXUpdatesUpdateSuccessBlock) (NSDictionary * _Nullable manifest);
typedef void (^ABI46_0_0EXUpdatesQuerySuccessBlock) (NSArray<NSUUID *> * _Nonnull updateIds);
typedef void (^ABI46_0_0EXUpdatesProgressBlock) (NSUInteger successfulAssetCount, NSUInteger failedAssetCount, NSUInteger totalAssetCount);
/**
 ...
 */
typedef BOOL (^ABI46_0_0EXUpdatesManifestBlock) (NSDictionary *manifest);

/**
 * Protocol for modules that depend on expo-updates for loading production updates but do not want
 * to depend on expo-updates or delegate control to the singleton ABI46_0_0EXUpdatesAppController.
 */
@protocol ABI46_0_0EXUpdatesExternalInterface
