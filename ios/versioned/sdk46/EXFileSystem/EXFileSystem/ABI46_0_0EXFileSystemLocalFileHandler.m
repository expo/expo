...
@implementation ABI46_0_0EXFileSystemLocalFileHandler
              ...
              rejecter:(ABI46_0_0EXPromiseRejectBlock)reject
{
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:path isDirectory:&isDirectory]) {
    ...
    result[@"size"] = @([ABI46_0_0EXFileSystemLocalFileHandler getFileSize:path attributes:attributes]);
...
+ (unsigned long long)getFileSize:(NSString *)path attributes:(NSDictionary<NSFileAttributeKey, id> *)attributes
{
  ...
  while (file = [contentsEnumurator nextObject]) {
    ...
    folderSize += [ABI46_0_0EXFileSystemLocalFileHandler getFileSize:filePath attributes:fileAttributes];
  }
