...
#import <ABI46_0_0EXAV/ABI46_0_0EXAV+AudioSampleCallback.h>

NSString *const ABI46_0_0EXAudioRecordingOptionsIsMeteringEnabledKey = @"isMeteringEnabled";
NSString *const ABI46_0_0EXAudioRecordingOptionsKeepAudioActiveHintKey = @"keepAudioActiveHint";
NSString *const ABI46_0_0EXAudioRecordingOptionsKey = @"ios";
NSString *const ABI46_0_0EXAudioRecordingOptionExtensionKey = @"extension";
NSString *const ABI46_0_0EXAudioRecordingOptionOutputFormatKey = @"outputFormat";
NSString *const ABI46_0_0EXAudioRecordingOptionAudioQualityKey = @"audioQuality";
NSString *const ABI46_0_0EXAudioRecordingOptionSampleRateKey = @"sampleRate";
NSString *const ABI46_0_0EXAudioRecordingOptionNumberOfChannelsKey = @"numberOfChannels";
NSString *const ABI46_0_0EXAudioRecordingOptionBitRateKey = @"bitRate";
NSString *const ABI46_0_0EXAudioRecordingOptionBitRateStrategyKey = @"bitRateStrategy";
NSString *const ABI46_0_0EXAudioRecordingOptionBitDepthHintKey = @"bitDepthHint";
NSString *const ABI46_0_0EXAudioRecordingOptionLinearPCMBitDepthKey = @"linearPCMBitDepth";
NSString *const ABI46_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey = @"linearPCMIsBigEndian";
NSString *const ABI46_0_0EXAudioRecordingOptionLinearPCMIsFloatKey = @"linearPCMIsFloat";

NSString *const ABI46_0_0EXDidUpdatePlaybackStatusEventName = @"didUpdatePlaybackStatus";

NSString *const ABI46_0_0EXDidUpdateMetadataEventName = @"didUpdateMetadata";

@interface ABI46_0_0EXAV ()
...
@property (nonatomic, assign) ABI46_0_0EXAVAudioSessionMode currentAudioSessionMode;
@property (nonatomic, assign) BOOL isBackgrounded;

@property (nonatomic, assign) ABI46_0_0EXAudioInterruptionMode audioInterruptionMode;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, ABI46_0_0EXAVPlayerData *> *soundDictionary;
@property (nonatomic, assign) BOOL isBeingObserved;
@property (nonatomic, strong) NSHashTable <NSObject<ABI46_0_0EXAVObject> *> *videoSet;
...
@property (nonatomic, weak) ABI46_0_0EXModuleRegistry *expoModuleRegistry;
@property (nonatomic, weak) id<ABI46_0_0EXPermissionsInterface> permissionsManager;

@end

@implementation ABI46_0_0EXAV

ABI46_0_0EX_EXPORT_MODULE(ExponentAV);

- (instancetype)init
{
  if (self = [super init]) {
    _audioIsEnabled = YES;
    _currentAudioSessionMode = ABI46_0_0EXAVAudioSessionModeInactive;
    _isBackgrounded = NO;
    
    _audioInterruptionMode = ABI46_0_0EXAudioInterruptionModeMixWithOthers;
...
{
  return @[@protocol(ABI46_0_0EXAVInterface)];
}

- (void)installJsiBindings
{
  id<ABI46_0_0EXJavaScriptContextProvider> jsContextProvider = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXJavaScriptContextProvider)];
...
- (void)setModuleRegistry:(ABI46_0_0EXModuleRegistry *)expoModuleRegistry
{
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXAppLifecycleService)] unregisterAppLifecycleListener:self];
  ...
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXAppLifecycleService)] registerAppLifecycleListener:self];
  _permissionsManager = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXPermissionsInterface)];
  [ABI46_0_0EXPermissionsMethodsDelegate registerRequesters:@[[ABI46_0_0EXAudioRecordingPermissionRequester new]] withPermissionsManager:_permissionsManager];
}
...
- (void)onAppForegrounded
{
  ...
  [self _runBlockForAllAVObjects:^(NSObject<ABI46_0_0EXAVObject> *exAVObject) {
...
- (void)onAppContentWillReload
{
  ...
  // using pointer to deallocated ABI46_0_0EXAVPlayerData in MTAudioTap process callback
  for (NSNumber *key in [_soundDictionary allKeys]) {
    [self _removeAudioCallbackForKey:key];
  }
}

#pragma mark - ABI46_0_0RCTBridgeModule
...
// Required in Expo Go only - ABI46_0_0EXAV conforms to ABI46_0_0RCTBridgeModule protocol
// and in Expo Go, kernel calls [ABI46_0_0EXReactAppManager rebuildBridge]
// which requires this to be implemented. Normal "bare" RN modules
// use ABI46_0_0RCT_EXPORT_MODULE macro which implement this automatically.
+(NSString *)moduleName
{
  return @"ExponentAV";
}

// Both ABI46_0_0RCTBridgeModule and ABI46_0_0EXExportedModule define `constantsToExport`. We implement
// that method for the latter, but ABI46_0_0React Bridge displays a yellow LogBox warning:
// "Module ABI46_0_0EXAV requires main queue setup since it overrides `constantsToExport` but doesn't implement `requiresMainQueueSetup`."
// Since we don't care about that (ABI46_0_0RCTBridgeModule is used here for another reason),
// we just need this to dismiss that warning.
+ (BOOL)requiresMainQueueSetup
{
  // We are now using main thread to avoid thread safety issues with `ABI46_0_0EXAVPlayerData` and `ABI46_0_0EXVideoView`
  // return `YES` to avoid deadlock warnings.
  return YES;
}

#pragma mark - ABI46_0_0RCTEventEmitter
...
- (void)_runBlockForAllAVObjects:(void (^)(NSObject<ABI46_0_0EXAVObject> *exAVObject))block
{
  for (ABI46_0_0EXAVPlayerData *data in [_soundDictionary allValues]) {
    block(data);
  }
  for (NSObject<ABI46_0_0EXAVObject> *video in [_videoSet allObjects]) {
    block(video);
  }
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode
{
  ...
  ABI46_0_0EXAudioInterruptionMode interruptionMode = ((NSNumber *)mode[@"interruptionModeIOS"]).intValue;
  ...
  if (!playsInSilentMode && interruptionMode == ABI46_0_0EXAudioInterruptionModeDuckOthers) {
    return ABI46_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and duckOthers == true cannot be set on iOS.");
  } else if (!playsInSilentMode && allowsRecording) {
    return ABI46_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and allowsRecording == true cannot be set on iOS.");
  } else if (!playsInSilentMode && shouldPlayInBackground) {
    return ABI46_0_0EXErrorWithMessage(@"Impossible audio mode: playsInSilentMode == false and staysActiveInBackground == true cannot be set on iOS.");
  } else {
    if (!allowsRecording) {
    ...
    if (_currentAudioSessionMode != ABI46_0_0EXAVAudioSessionModeInactive) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(ABI46_0_0EXAVAudioSessionMode)audioSessionMode
{
  ...
  if (!_playsInSilentMode) {
    // _allowsRecording is guaranteed to be false, and _interruptionMode is guaranteed to not be ABI46_0_0EXAudioInterruptionModeDuckOthers (see above)
    if (_audioInterruptionMode == ABI46_0_0EXAudioInterruptionModeDoNotMix) {
      requiredAudioCategory = AVAudioSessionCategorySoloAmbient;
    } else {
      requiredAudioCategory = AVAudioSessionCategoryAmbient;
    }
  } else {
    ABI46_0_0EXAudioInterruptionMode activeInterruptionMode = audioSessionMode == ABI46_0_0EXAVAudioSessionModeActiveMuted ? ABI46_0_0EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
    ...
    switch (activeInterruptionMode) {
      case ABI46_0_0EXAudioInterruptionModeDoNotMix:
        break;
      case ABI46_0_0EXAudioInterruptionModeDuckOthers:
        requiredAudioCategoryOptions = AVAudioSessionCategoryOptionDuckOthers;
        break;
      case ABI46_0_0EXAudioInterruptionModeMixWithOthers:
      default:
...
- (ABI46_0_0EXAVAudioSessionMode)_getAudioSessionModeRequired
{
  __block ABI46_0_0EXAVAudioSessionMode audioSessionModeRequired = ABI46_0_0EXAVAudioSessionModeInactive;
  
  [self _runBlockForAllAVObjects:^(NSObject<ABI46_0_0EXAVObject> *exAVObject) {
    ABI46_0_0EXAVAudioSessionMode audioSessionModeRequiredByThisObject = [exAVObject getAudioSessionModeRequired];
  ...
  if (_audioRecorder) {
    if (_audioRecorderShouldBeginRecording || [_audioRecorder isRecording]) {
      audioSessionModeRequired = ABI46_0_0EXAVAudioSessionModeActive;
    } else if (_audioRecorderIsPreparing && audioSessionModeRequired == ABI46_0_0EXAVAudioSessionModeInactive) {
      audioSessionModeRequired = ABI46_0_0EXAVAudioSessionModeActiveMuted;
    }
  }
...
{
  if (!_audioIsEnabled) {
    return ABI46_0_0EXErrorWithMessage(@"Expo Audio is disabled, so the audio session could not be activated.");
  }
  if (_isBackgrounded && !_staysActiveInBackground && ![_kernelAudioSessionManagerDelegate isActiveForModule:self]) {
    return ABI46_0_0EXErrorWithMessage(@"This experience is currently in the background, so the audio session could not be activated.");
  }
  
  ABI46_0_0EXAVAudioSessionMode audioSessionModeRequired = [self _getAudioSessionModeRequired];
  
  if (audioSessionModeRequired == ABI46_0_0EXAVAudioSessionModeInactive) {
    return nil;
  }
...
- (NSError *)demoteAudioSessionIfPossible
{
  ...
  if (audioSessionModeRequired == ABI46_0_0EXAVAudioSessionModeActiveMuted) {
    NSError *error = [self _updateAudioSessionCategoryForAudioSessionMode:audioSessionModeRequired];
    if (!error) {
      _currentAudioSessionMode = ABI46_0_0EXAVAudioSessionModeActiveMuted;
    }
    return error;
  }
...
- (void)_runBlock:(void (^)(ABI46_0_0EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber *)key
     withRejecter:(ABI46_0_0EXPromiseRejectBlock)reject
{
  ABI46_0_0EXAVPlayerData *data = _soundDictionary[key];
  if (data) {
    block(data);
  } else {
     ...
     withRejecter:(ABI46_0_0EXPromiseRejectBlock)reject
{
  ...
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXUIManager)] executeUIBlock:^(id view) {
    if ([view isKindOfClass:[ABI46_0_0EXVideoView class]]) {
      block(view);
    } else {
      reject(@"E_VIDEO_TAGINCORRECT", [NSString stringWithFormat:@"Invalid view returned from registry, expecting ABI46_0_0EXVideo, got: %@", view], nil);
    }
  } forView:reactTag ofClass:[ABI46_0_0EXVideoView class]];
}
...
- (NSString *)_getBitRateStrategyFromEnum:(NSNumber *)bitRateEnumSelected
{
  if (bitRateEnumSelected) {
    switch ([bitRateEnumSelected integerValue]) {
      case ABI46_0_0EXAudioRecordingOptionBitRateStrategyConstant:
        return AVAudioBitRateStrategy_Constant;
      case ABI46_0_0EXAudioRecordingOptionBitRateStrategyLongTermAverage:
        return AVAudioBitRateStrategy_LongTermAverage;
      case ABI46_0_0EXAudioRecordingOptionBitRateStrategyVariableConstrained:
        return AVAudioBitRateStrategy_VariableConstrained;
        break;
      case ABI46_0_0EXAudioRecordingOptionBitRateStrategyVariable:
...
{
  return @{ABI46_0_0EXAudioRecordingOptionOutputFormatKey: AVFormatIDKey,
           ABI46_0_0EXAudioRecordingOptionAudioQualityKey:
             bitRateStrategy == AVAudioBitRateStrategy_Variable
           ? AVEncoderAudioQualityForVBRKey : AVEncoderAudioQualityKey,
           ABI46_0_0EXAudioRecordingOptionSampleRateKey: AVSampleRateKey,
           ABI46_0_0EXAudioRecordingOptionNumberOfChannelsKey: AVNumberOfChannelsKey,
           ABI46_0_0EXAudioRecordingOptionBitRateKey: AVEncoderBitRateKey,
           ABI46_0_0EXAudioRecordingOptionBitDepthHintKey: AVEncoderBitDepthHintKey,
           ABI46_0_0EXAudioRecordingOptionLinearPCMBitDepthKey: AVLinearPCMBitDepthKey,
           ABI46_0_0EXAudioRecordingOptionLinearPCMIsBigEndianKey: AVLinearPCMIsBigEndianKey,
           ABI46_0_0EXAudioRecordingOptionLinearPCMIsFloatKey: AVLinearPCMIsFloatKey};
}
...
{
  NSDictionary *iosOptionsFromJS = optionsFromJS[ABI46_0_0EXAudioRecordingOptionsKey];
  
  NSString *extension = iosOptionsFromJS[ABI46_0_0EXAudioRecordingOptionExtensionKey];
  _audioRecorderFilename = [NSString stringWithFormat:@"recording-%@%@", [[NSUUID UUID] UUIDString], extension];
  
  NSString *bitRateStrategy = [self _getBitRateStrategyFromEnum:iosOptionsFromJS[ABI46_0_0EXAudioRecordingOptionBitRateStrategyKey]];
  ...
  if (
      iosOptionsFromJS[ABI46_0_0EXAudioRecordingOptionOutputFormatKey] &&
      [iosOptionsFromJS[ABI46_0_0EXAudioRecordingOptionOutputFormatKey] isKindOfClass:[NSString class]]
      ) {
    recorderSettings[AVFormatIDKey] =
    @([self _getFormatIDFromString:iosOptionsFromJS[ABI46_0_0EXAudioRecordingOptionOutputFormatKey]]);
  }
...
{
  if (_audioRecorder) {
    return ABI46_0_0EXErrorWithMessage(@"Recorder is already prepared.");
  }
  
  id<ABI46_0_0EXFileSystemInterface> fileSystem = [_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXFileSystemInterface)];
  
  if (!fileSystem) {
    return ABI46_0_0EXErrorWithMessage(@"No FileSystem module.");
  }
...
- (BOOL)_checkAudioRecorderExistsOrReject:(ABI46_0_0EXPromiseRejectBlock)reject
{
...
{
  return @[ABI46_0_0EXDidUpdatePlaybackStatusEventName, ABI46_0_0EXDidUpdateMetadataEventName, @"ExponentAV.onError"];
}

#pragma mark - Audio API: Global settings

ABI46_0_0EX_EXPORT_METHOD_AS(setAudioIsEnabled,
...
ABI46_0_0EX_EXPORT_METHOD_AS(setAudioMode,
...
ABI46_0_0EX_EXPORT_METHOD_AS(loadForSound,
                    loadForSound:(NSDictionary *)source
                    withStatus:(NSDictionary *)status
                    resolver:(ABI46_0_0EXPromiseResolveBlock)loadSuccess
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)loadError)
{
  NSNumber *key = @(_soundDictionaryKeyCount++);
  
  ABI46_0_0EX_WEAKIFY(self);
  ABI46_0_0EXAVPlayerData *data = [[ABI46_0_0EXAVPlayerData alloc] initWithEXAV:self
                                                   withSource:source
                                                   withStatus:status
    ...
    if (success) {
      ...
      loadError(@"ABI46_0_0EXAV", error, nil);
    }
  }];
  ...
  data.statusUpdateCallback = ^(NSDictionary *status) {
    ...
    if (self.isBeingObserved) {
      ...
      [self sendEventWithName:ABI46_0_0EXDidUpdatePlaybackStatusEventName body:response];
    }
  };
  ...
  data.metadataUpdateCallback = ^(NSDictionary *metadata) {
    ABI46_0_0EX_ENSURE_STRONGIFY(self);
      if (self.isBeingObserved) {
        ...
        [self sendEventWithName:ABI46_0_0EXDidUpdateMetadataEventName body:response];
      }
  };
...
{
  [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(ABI46_0_0EXEventEmitterService)] sendEventWithName:eventName body:body];
}

ABI46_0_0EX_EXPORT_METHOD_AS(unloadForSound,
                    unloadForSound:(NSNumber *)key
                    resolver:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  [self _runBlock:^(ABI46_0_0EXAVPlayerData *data) {
    [self _removeSoundForKey:key];
    resolve([ABI46_0_0EXAVPlayerData getUnloadedStatus]);
  } withSoundForKey:key withRejecter:reject];
}

ABI46_0_0EX_EXPORT_METHOD_AS(setStatusForSound,
...
ABI46_0_0EX_EXPORT_METHOD_AS(getStatusForSound,
...
ABI46_0_0EX_EXPORT_METHOD_AS(replaySound,
...
ABI46_0_0EX_EXPORT_METHOD_AS(loadForVideo,
...
ABI46_0_0EX_EXPORT_METHOD_AS(unloadForVideo,
...
ABI46_0_0EX_EXPORT_METHOD_AS(setStatusForVideo,
...
ABI46_0_0EX_EXPORT_METHOD_AS(replayVideo,
...
ABI46_0_0EX_EXPORT_METHOD_AS(getStatusForVideo,
                    ...
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  [ABI46_0_0EXPermissionsMethodsDelegate getPermissionWithPermissionsManager:_permissionsManager
                                                      withRequester:[ABI46_0_0EXAudioRecordingPermissionRequester class]
                                                            resolve:resolve
                                                             reject:reject];
}
...
ABI46_0_0EX_EXPORT_METHOD_AS(prepareAudioRecorder,
                    prepareAudioRecorder:(NSDictionary *)options
                    resolver:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  _mediaServicesDidReset = false;
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI46_0_0EXAudioRecordingPermissionRequester class]]) {
  ...
  if (_audioRecorder && !error) {
    ...
    if (options[ABI46_0_0EXAudioRecordingOptionsIsMeteringEnabledKey]) {
    ...
    _audioRecorderIsPreparing = false;
    if (!options[ABI46_0_0EXAudioRecordingOptionsKeepAudioActiveHintKey]) {
      [self demoteAudioSessionIfPossible];
    }
  } else {
    reject(@"E_AUDIO_RECORDERNOTCREATED", [NSString stringWithFormat:@"Prepare encountered an error: %@", error.description], error);
...
ABI46_0_0EX_EXPORT_METHOD_AS(startAudioRecording,
                    startAudioRecording:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI46_0_0EXAudioRecordingPermissionRequester class]]) {
    reject(@"E_MISSING_PERMISSION", @"Missing audio recording permission.", nil);
...
ABI46_0_0EX_EXPORT_METHOD_AS(pauseAudioRecording,
                    pauseAudioRecording:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
...
ABI46_0_0EX_EXPORT_METHOD_AS(stopAudioRecording,
                    stopAudioRecording:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    if (_audioRecorder.recording) {
...
ABI46_0_0EX_EXPORT_METHOD_AS(getAudioRecordingStatus,
                    getAudioRecordingStatus:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    resolve([self _getAudioRecorderStatus]);
...
ABI46_0_0EX_EXPORT_METHOD_AS(unloadAudioRecorder,
                    unloadAudioRecorder:(ABI46_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0EXPromiseRejectBlock)reject)
{
  if ([self _checkAudioRecorderExistsOrReject:reject]) {
    [self _removeAudioRecorder:YES];
...
ABI46_0_0EX_EXPORT_METHOD_AS(getAvailableInputs,
                    resolver:(ABI46_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0UMPromiseRejectBlock)reject)
{
...
ABI46_0_0EX_EXPORT_METHOD_AS(getCurrentInput,
                    getCurrentInput:(ABI46_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI46_0_0UMPromiseRejectBlock)reject)
{
...
ABI46_0_0EX_EXPORT_METHOD_AS(setInput,
