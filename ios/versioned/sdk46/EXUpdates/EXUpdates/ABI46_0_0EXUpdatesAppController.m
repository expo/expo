...
static NSString * const ABI46_0_0EXUpdatesAppControllerErrorDomain = @"ABI46_0_0EXUpdatesAppController";

static NSString * const ABI46_0_0EXUpdatesUpdateAvailableEventName = @"updateAvailable";
static NSString * const ABI46_0_0EXUpdatesNoUpdateAvailableEventName = @"noUpdateAvailable";
static NSString * const ABI46_0_0EXUpdatesErrorEventName = @"error";

@interface ABI46_0_0EXUpdatesAppController () <ABI46_0_0EXUpdatesErrorRecoveryDelegate>

@property (nonatomic, readwrite, strong) ABI46_0_0EXUpdatesConfig *config;
@property (nonatomic, readwrite, strong, nullable) id<ABI46_0_0EXUpdatesAppLauncher> launcher;
@property (nonatomic, readwrite, strong) ABI46_0_0EXUpdatesDatabase *database;
@property (nonatomic, readwrite, strong) ABI46_0_0EXUpdatesSelectionPolicy *selectionPolicy;
@property (nonatomic, readwrite, strong) ABI46_0_0EXUpdatesSelectionPolicy *defaultSelectionPolicy;
@property (nonatomic, readwrite, strong) ABI46_0_0EXUpdatesErrorRecovery *errorRecovery;
...
@property (nonatomic, strong) ABI46_0_0EXUpdatesAppLoaderTask *loaderTask;
@property (nonatomic, strong) id<ABI46_0_0EXUpdatesAppLauncher> candidateLauncher;
@property (nonatomic, assign) BOOL hasLaunched;

@property (nonatomic, assign) BOOL isStarted;
@property (nonatomic, assign) BOOL isEmergencyLaunch;

@property (nonatomic, readwrite, assign) ABI46_0_0EXUpdatesRemoteLoadStatus remoteLoadStatus;

@end

@implementation ABI46_0_0EXUpdatesAppController

+ (instancetype)sharedInstance
{
  static ABI46_0_0EXUpdatesAppController *theController;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!theController) {
      theController = [[ABI46_0_0EXUpdatesAppController alloc] init];
    }
  });
...
{
  if (self = [super init]) {
    _config = [ABI46_0_0EXUpdatesConfig configWithExpoPlist];
    _database = [[ABI46_0_0EXUpdatesDatabase alloc] init];
    _defaultSelectionPolicy = [ABI46_0_0EXUpdatesSelectionPolicyFactory filterAwarePolicyWithRuntimeVersion:[ABI46_0_0EXUpdatesUtils getRuntimeVersionWithConfig:_config]];
    _errorRecovery = [ABI46_0_0EXUpdatesErrorRecovery new];
    ...
    _isStarted = NO;
    _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusIdle;
  }
  return self;
}
...
- (void)setConfiguration:(NSDictionary *)configuration
{
  if (_isStarted) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"ABI46_0_0EXUpdatesAppController:setConfiguration should not be called after start"
                                 userInfo:@{}];
  }
...
- (void)setNextSelectionPolicy:(ABI46_0_0EXUpdatesSelectionPolicy *)nextSelectionPolicy
{
...
- (void)start
{
  NSAssert(!_isStarted, @"ABI46_0_0EXUpdatesAppController:start should only be called once per instance");

  if (!_config.isEnabled) {
    ABI46_0_0EXUpdatesAppLauncherNoDatabase *launcher = [[ABI46_0_0EXUpdatesAppLauncherNoDatabase alloc] init];
    _launcher = launcher;
  ...
  if (!_config.updateUrl) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"expo-updates is enabled, but no valid URL is configured under ABI46_0_0EXUpdatesURL. If you are making a release build for the first time, make sure you have run `expo publish` at least once."
                                 userInfo:@{}];
  }
  if (!_config.scopeKey) {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"expo-updates was configured with no scope key. Make sure a valid URL is configured under ABI46_0_0EXUpdatesURL."
                                 userInfo:@{}];
  }
  ...
  [ABI46_0_0EXUpdatesBuildData ensureBuildDataIsConsistentAsync:_database config:_config];

  [_errorRecovery startMonitoring];

  _loaderTask = [[ABI46_0_0EXUpdatesAppLoaderTask alloc] initWithConfig:_config
                                                      database:_database
...
{
  ABI46_0_0EXUpdatesAppLauncherWithDatabase *launcher = [[ABI46_0_0EXUpdatesAppLauncherWithDatabase alloc] initWithConfig:_config database:_database directory:_updatesDirectory completionQueue:_controllerQueue];
  ...
  [launcher launchUpdateWithSelectionPolicy:self.selectionPolicy completion:^(NSError * _Nullable error, BOOL success) {
    if (success) {
      ...
      ABI46_0_0RCTReloadCommandSetBundleURL(launcher.launchAssetUrl);
      ABI46_0_0RCTTriggerReloadCommandListeners(@"Requested by JavaScript - Updates.reloadAsync()");
      [self runReaper];
    } else {
...
# pragma mark - ABI46_0_0EXUpdatesAppLoaderTaskDelegate

- (BOOL)appLoaderTask:(ABI46_0_0EXUpdatesAppLoaderTask *)appLoaderTask didLoadCachedUpdate:(nonnull ABI46_0_0EXUpdatesUpdate *)update
{
  return YES;
}
...
{
  _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusLoading;
}
...
- (void)appLoaderTask:(ABI46_0_0EXUpdatesAppLoaderTask *)appLoaderTask didFinishWithLauncher:(id<ABI46_0_0EXUpdatesAppLauncher>)launcher isUpToDate:(BOOL)isUpToDate
{
  ...
  if (_remoteLoadStatus == ABI46_0_0EXUpdatesRemoteLoadStatusLoading && isUpToDate) {
    _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusIdle;
  }
  _launcher = launcher;
  if (self->_delegate) {
    [ABI46_0_0EXUpdatesUtils runBlockOnMainThread:^{
...
{
  if (status == ABI46_0_0EXUpdatesBackgroundUpdateStatusError) {
    _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusIdle;
    NSAssert(error != nil, @"Background update with error status must have a nonnull error object");
    [ABI46_0_0EXUpdatesUtils sendEventToBridge:_bridge withType:ABI46_0_0EXUpdatesErrorEventName body:@{@"message": error.localizedDescription}];
  } else if (status == ABI46_0_0EXUpdatesBackgroundUpdateStatusUpdateAvailable) {
    _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded;
    NSAssert(update != nil, @"Background update with error status must have a nonnull update object");
    [ABI46_0_0EXUpdatesUtils sendEventToBridge:_bridge withType:ABI46_0_0EXUpdatesUpdateAvailableEventName body:@{@"manifest": update.manifest.rawManifestJSON}];
  } else if (status == ABI46_0_0EXUpdatesBackgroundUpdateStatusNoUpdateAvailable) {
    _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusIdle;
    [ABI46_0_0EXUpdatesUtils sendEventToBridge:_bridge withType:ABI46_0_0EXUpdatesNoUpdateAvailableEventName body:@{}];
  }
  [_errorRecovery notifyNewRemoteLoadStatus:_remoteLoadStatus];
}

# pragma mark - ABI46_0_0EXUpdatesAppController+Internal

- (BOOL)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  _updatesDirectory = [ABI46_0_0EXUpdatesUtils initializeUpdatesDirectoryWithError:error];
...
- (void)runReaper
{
  if (_launcher.launchedUpdate) {
    [ABI46_0_0EXUpdatesReaper reapUnusedUpdatesWithConfig:_config
                                        database:_database
                                       directory:_updatesDirectory
                                 selectionPolicy:self.selectionPolicy
...
# pragma mark - ABI46_0_0EXUpdatesErrorRecoveryDelegate
...
- (void)relaunchWithCompletion:(ABI46_0_0EXUpdatesAppLauncherCompletionBlock)completion
{
  ...
  [launcher launchUpdateWithSelectionPolicy:self.selectionPolicy completion:^(NSError * _Nullable error, BOOL success) {
    if (success) {
      ...
      [self->_errorRecovery startMonitoring];
      ABI46_0_0RCTReloadCommandSetBundleURL(launcher.launchAssetUrl);
      ABI46_0_0RCTTriggerReloadCommandListeners(@"Relaunch after fatal error");
      completion(nil, YES);
    } else {
      completion(error, NO);
    }
...
- (void)loadRemoteUpdate
{
  ...
  _remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusLoading;
  ABI46_0_0EXUpdatesAppLoader *remoteAppLoader = [[ABI46_0_0EXUpdatesRemoteAppLoader alloc] initWithConfig:_config database:_database directory:_updatesDirectory launchedUpdate:self.launchedUpdate completionQueue:_controllerQueue];
  [remoteAppLoader loadUpdateFromUrl:_config.updateUrl onManifest:^BOOL(ABI46_0_0EXUpdatesUpdate *update) {
    ...
    self->_remoteLoadStatus = update ? ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded : ABI46_0_0EXUpdatesRemoteLoadStatusIdle;
    [self->_errorRecovery notifyNewRemoteLoadStatus:self->_remoteLoadStatus];
  } error:^(NSError *error) {
    self->_remoteLoadStatus = ABI46_0_0EXUpdatesRemoteLoadStatusIdle;
    [self->_errorRecovery notifyNewRemoteLoadStatus:self->_remoteLoadStatus];
  }];
}
...
- (void)markFailedLaunchForLaunchedUpdate
{
  ...
  dispatch_async(_database.databaseQueue, ^{
    ABI46_0_0EXUpdatesUpdate *launchedUpdate = self.launchedUpdate;
    if (!launchedUpdate) {
      return;
    }
    NSError *error;
