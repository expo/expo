...
typedef NS_ENUM(NSInteger, ABI46_0_0EXUpdatesErrorRecoveryTask) {
  ABI46_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate,
  ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchNew,
  ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchCached,
  ABI46_0_0EXUpdatesErrorRecoveryTaskCrash
};

static NSString * const ABI46_0_0EXUpdatesErrorLogFile = @"expo-error.log";
static NSInteger const ABI46_0_0EXUpdatesErrorRecoveryRemoteLoadTimeoutMs = 5000;

@interface ABI46_0_0EXUpdatesErrorRecovery ()
...
@property (nonatomic, copy) ABI46_0_0RCTFatalHandler previousFatalErrorHandler;
@property (nonatomic, copy) ABI46_0_0RCTFatalExceptionHandler previousFatalExceptionHandler;

@end

@implementation ABI46_0_0EXUpdatesErrorRecovery

- (instancetype)init
{
  return [self initWithErrorRecoveryQueue:dispatch_queue_create("expo.controller.errorRecoveryQueue", DISPATCH_QUEUE_SERIAL)
                           diskWriteQueue:nil
                        remoteLoadTimeout:ABI46_0_0EXUpdatesErrorRecoveryRemoteLoadTimeoutMs];
}
...
- (void)notifyNewRemoteLoadStatus:(ABI46_0_0EXUpdatesRemoteLoadStatus)newStatus
{
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    if (newStatus != ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded) {
      [self->_pipeline removeObject:@(ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchNew)];
    }
    [self _runNextTask];
  });
}
...
- (void)_startPipelineWithEncounteredError:(id)encounteredError
{
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    if (self->_delegate.launchedUpdate.successfulLaunchCount > 0) {
      [self->_pipeline removeObject:@(ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchCached)];
...
- (void)_runNextTask
{
  ...
  switch ((ABI46_0_0EXUpdatesErrorRecoveryTask)nextTask.integerValue) {
    case ABI46_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate:
      [self _waitForRemoteLoaderToFinish];
      break;
    // ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchNew is called only after a new update is downloaded
    // and added to the cache, so it is equivalent to ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchCached
    case ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchNew:
    case ABI46_0_0EXUpdatesErrorRecoveryTaskLaunchCached:
      [self _tryRelaunchFromCache];
      break;
    case ABI46_0_0EXUpdatesErrorRecoveryTaskCrash:
      [self _crash];
      break;
    default:
      @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"ABI46_0_0EXUpdatesErrorRecovery cannot run the next task" userInfo:@{@"nextTaskValue": nextTask}];
  }
}
...
{
  dispatch_assert_queue(_errorRecoveryQueue);
  if (_delegate.remoteLoadStatus == ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded) {
    [self _runNextTask];
  } else if (_delegate.config.checkOnLaunch != ABI46_0_0EXUpdatesCheckAutomaticallyConfigNever ||
             _delegate.remoteLoadStatus == ABI46_0_0EXUpdatesRemoteLoadStatusLoading) {
    _isWaitingForRemoteUpdate = YES;
    if (_delegate.remoteLoadStatus != ABI46_0_0EXUpdatesRemoteLoadStatusLoading) {
...
- (void)_crash
{
  ...
  if ([initialError isKindOfClass:[NSError class]]) {
    // format these keys similar to RN -- ABI46_0_0RCTFatal in ABI46_0_0RCTAssert.m
    name = [NSString stringWithFormat:@"%@: %@", ABI46_0_0RCTFatalExceptionName, ((NSError *)initialError).localizedDescription];
    reason = ABI46_0_0RCTFormatError(((NSError *)initialError).localizedDescription, ((NSError *)initialError).userInfo[ABI46_0_0RCTJSStackTraceKey], 175);
    userInfo = [((NSError *)initialError).userInfo mutableCopy];
    userInfo[@"ABI46_0_0RCTUntruncatedMessageKey"] = ABI46_0_0RCTFormatError(((NSError *)initialError).localizedDescription, ((NSError *)initialError).userInfo[ABI46_0_0RCTJSStackTraceKey], -1);
  ...
  userInfo[@"ABI46_0_0EXUpdatesLaterEncounteredErrors"] = [_encounteredErrors copy];
...
{
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleJavaScriptDidFailToLoad) name:ABI46_0_0RCTJavaScriptDidFailToLoadNotification object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleContentDidAppear) name:ABI46_0_0RCTContentDidAppearNotification object:nil];
}

- (void)_unregisterObservers
{
  [[NSNotificationCenter defaultCenter] removeObserver:self name:ABI46_0_0RCTJavaScriptDidFailToLoadNotification object:nil];
  [[NSNotificationCenter defaultCenter] removeObserver:self name:ABI46_0_0RCTContentDidAppearNotification object:nil];
}
...
- (void)_handleContentDidAppear
{
  ...
  dispatch_async(_errorRecoveryQueue, ^{
    ...
    self->_pipeline = [self->_pipeline objectsAtIndexes:[self->_pipeline indexesOfObjectsPassingTest:^BOOL(NSNumber *obj, NSUInteger idx, BOOL *stop) {
      return obj.integerValue == ABI46_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate || obj.integerValue == ABI46_0_0EXUpdatesErrorRecoveryTaskCrash;
    }]].mutableCopy;
  });
  ...
  // ABI46_0_0EXUpdatesErrorRecoveryTaskWaitForRemoteUpdate task and check for a new update is there is one
...
- (void)_setRCTErrorHandlers
{
  ...
  _previousFatalErrorHandler = ABI46_0_0RCTGetFatalHandler();
  _previousFatalExceptionHandler = ABI46_0_0RCTGetFatalExceptionHandler();

  ABI46_0_0RCTFatalHandler fatalErrorHandler = ^(NSError *error) {
    [self handleError:error];
  };
  ABI46_0_0RCTFatalExceptionHandler fatalExceptionHandler = ^(NSException *exception) {
    [self handleException:exception];
  };
  ABI46_0_0RCTSetFatalHandler(fatalErrorHandler);
  ABI46_0_0RCTSetFatalExceptionHandler(fatalExceptionHandler);
}

- (void)_unsetRCTErrorHandlers
{
  ABI46_0_0RCTSetFatalHandler(_previousFatalErrorHandler);
  ABI46_0_0RCTSetFatalExceptionHandler(_previousFatalExceptionHandler);
...
+ (NSString *)_errorLogFilePath
{
  ...
  return [[applicationDocumentsDirectory URLByAppendingPathComponent:ABI46_0_0EXUpdatesErrorLogFile] path];
}
