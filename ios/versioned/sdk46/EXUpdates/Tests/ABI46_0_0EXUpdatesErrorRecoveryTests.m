...
@interface ABI46_0_0EXUpdatesErrorRecoveryTests : XCTestCase

@property (nonatomic, strong) ABI46_0_0EXUpdatesErrorRecovery *errorRecovery;
@property (nonatomic, strong) dispatch_queue_t testQueue;

@end

@implementation ABI46_0_0EXUpdatesErrorRecoveryTests

- (void)setUp
{
  _testQueue = dispatch_queue_create("expo.errorRecoveryTestQueue", DISPATCH_QUEUE_SERIAL);
  _errorRecovery = [[ABI46_0_0EXUpdatesErrorRecovery alloc] initWithErrorRecoveryQueue:_testQueue diskWriteQueue:_testQueue remoteLoadTimeout:500];
}

- (void)testHandleError_NewWorkingUpdateAlreadyLoaded
{
  id<ABI46_0_0EXUpdatesErrorRecoveryDelegate> mockDelegate = mockProtocol(@protocol(ABI46_0_0EXUpdatesErrorRecoveryDelegate));
  _errorRecovery.delegate = mockDelegate;

  [given(mockDelegate.remoteLoadStatus) willReturnInteger:ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded];
...
- (void)testHandleError_NewWorkingUpdateAlreadyLoaded_RCTContentDidAppear
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:@"ABI46_0_0RCTContentDidAppearNotification" object:nil];
...
{
  id<ABI46_0_0EXUpdatesErrorRecoveryDelegate> mockDelegate = mockProtocol(^protocol(ABI46_0_0EXUpdatesErrorRecoveryDelegate)); // replace ^ with @ when uncommenting
  _errorRecovery.delegate = mockDelegate;

  [given(mockDelegate.remoteLoadStatus) willReturnInteger:ABI46_0_0EXUpdatesRemoteLoadStatusLoading];

  NSError *mockError = mock([NSError class]);
  [_errorRecovery handleError:mockError];

  [given(mockDelegate.remoteLoadStatus) willReturnInteger:ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded];
  [_errorRecovery notifyNewRemoteLoadStatus:ABI46_0_0EXUpdatesRemoteLoadStatusNewUpdateLoaded];
  dispatch_sync(_testQueue, ^{}); // flush queue
...
- (void)testHandleError_NewBrokenUpdateLoaded_UpdateAlreadyLaunchedSuccessfully
{
  ...
  ABI46_0_0EXUpdatesUpdate *mockUpdate = mock([ABI46_0_0EXUpdatesUpdate class]);
...
- (void)testHandleError_RemoteLoadTimesOut_RCTContentDidAppear
{
  ...
  // if ABI46_0_0RCTContentDidAppear has already fired, we don't want to roll back to an older update
  NSError *mockError = mock([NSError class]);
  [_errorRecovery handleError:mockError];
...
- (void)testHandleError_NoRemoteUpdate
{
  ...
  [given(mockDelegate.remoteLoadStatus) willReturnInteger:ABI46_0_0EXUpdatesRemoteLoadStatusIdle];
  ...
  [_errorRecovery notifyNewRemoteLoadStatus:ABI46_0_0EXUpdatesRemoteLoadStatusIdle];
  dispatch_sync(_testQueue, ^{}); // flush queue
  [self verifySuccessfulRelaunchWithCompletion_WithMockDelegate:mockDelegate];
}
...
- (void)testHandleError_CheckAutomaticallyNever
{
  ...
  ABI46_0_0EXUpdatesConfig *mockConfig = mock([ABI46_0_0EXUpdatesConfig class]);
  [given(mockConfig.checkOnLaunch) willReturnInteger:ABI46_0_0EXUpdatesCheckAutomaticallyConfigNever];
...
- (void)testConsumeErrorLog
{
  ...
  [ABI46_0_0EXUpdatesErrorRecovery consumeErrorLog];
  ...
  NSString *errorLog = [ABI46_0_0EXUpdatesErrorRecovery consumeErrorLog];
...
- (void)verifySuccessfulRelaunchWithCompletion_WithMockDelegate:(id<ABI46_0_0EXUpdatesErrorRecoveryDelegate>)mockDelegate
{
  HCArgumentCaptor *argument = [[HCArgumentCaptor alloc] init];
  [verify(mockDelegate) relaunchWithCompletion:(id)argument];
  ABI46_0_0EXUpdatesAppLauncherCompletionBlock completion = argument.value;
  completion(nil, YES);
  dispatch_sync(_testQueue, ^{}); // flush queue
}

- (void)verifyFailedRelaunchWithCompletion_WithMockDelegate:(id<ABI46_0_0EXUpdatesErrorRecoveryDelegate>)mockDelegate
{
  HCArgumentCaptor *argument = [[HCArgumentCaptor alloc] init];
  [verify(mockDelegate) relaunchWithCompletion:(id)argument];
  ABI46_0_0EXUpdatesAppLauncherCompletionBlock completion = argument.value;
