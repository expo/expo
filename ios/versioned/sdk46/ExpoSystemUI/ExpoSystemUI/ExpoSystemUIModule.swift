...
public class ExpoSystemUIModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    OnCreate {
      ...
      let color = Bundle.main.object(forInfoDictionaryKey: "ABI46_0_0RCTRootViewBackgroundColor") as? Int
  ...
  static func getBackgroundColor() -> String? {
    var color: String?
    ABI46_0_0EXUtilities.performSynchronously {
      ...
      if let window = UIApplication.shared.delegate?.window, let backgroundColor = window?.rootViewController?.view.backgroundColor?.cgColor {
        color = ABI46_0_0EXUtilities.hexString(with: backgroundColor)
      }
    }
    return color
  }
  ...
  static func setBackgroundColorAsync(color: Int?) {
    ABI46_0_0EXUtilities.performSynchronously {
      ...
      let backgroundColor = ABI46_0_0EXUtilities.uiColor(color)
      // Set the app-wide window, this could have future issues when running multiple ABI46_0_0React apps,
