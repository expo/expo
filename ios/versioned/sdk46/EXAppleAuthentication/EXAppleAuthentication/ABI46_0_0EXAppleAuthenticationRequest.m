...
@interface ABI46_0_0EXAppleAuthenticationRequest ()
...
@implementation ABI46_0_0EXAppleAuthenticationRequest
...
- (void)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
{
  ...
  request.requestedScopes = [ABI46_0_0EXAppleAuthenticationMappings importScopes:requestedScopes];
...
+ (ABI46_0_0EXAppleAuthenticationRequest *)performOperation:(ASAuthorizationProviderAuthorizationOperation)operation
                                       withOptions:(NSDictionary *)options
                                      withCallback:(void(^)(NSDictionary *, NSError *))callback
{
  ABI46_0_0EXAppleAuthenticationRequest *authRequest = [[ABI46_0_0EXAppleAuthenticationRequest alloc] initWithOptions:options andCallback:callback];
   ...
   didCompleteWithAuthorization:(ASAuthorization *)authorization
{
  ...
  if (credential.fullName) {
    fullName = [ABI46_0_0EXAppleAuthenticationMappings exportFullName:credential.fullName];
  }
  NSString *authorizationCode;
  if (credential.authorizationCode) {
    authorizationCode = [ABI46_0_0EXAppleAuthenticationMappings exportData:credential.authorizationCode];
  }
  NSString *identityToken;
  if (credential.identityToken) {
    identityToken = [ABI46_0_0EXAppleAuthenticationMappings exportData:credential.identityToken];
  }

  NSDictionary *response = @{
                         @"fullName": ABI46_0_0EXNullIfNil(fullName),
                         @"email": ABI46_0_0EXNullIfNil(credential.email),
                         @"user": credential.user,
                         @"realUserStatus": [ABI46_0_0EXAppleAuthenticationMappings exportRealUserStatus:credential.realUserStatus],
                         @"state": ABI46_0_0EXNullIfNil(credential.state),
                         @"authorizationCode": ABI46_0_0EXNullIfNil(authorizationCode),
                         @"identityToken": ABI46_0_0EXNullIfNil(identityToken),
                         };
