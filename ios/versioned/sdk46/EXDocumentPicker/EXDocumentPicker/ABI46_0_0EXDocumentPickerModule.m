...
#import <ABI46_0_0EXDocumentPicker/ABI46_0_0EXDocumentPickerModule.h>
...
#import <MobileCoreServices/MobileCoreServices.h>
  ...
  static UTType* ABI46_0_0EXConvertMimeTypeToUTType(NSString *mimeType)
  {
...
static NSString * ABI46_0_0EXConvertMimeTypeToUTI(NSString *mimeType)
{
  CFStringRef uti;
...
@interface ABI46_0_0EXDocumentPickerModule () <UIDocumentPickerDelegate, UIAdaptivePresentationControllerDelegate>
...
@implementation ABI46_0_0EXDocumentPickerModule

ABI46_0_0EX_EXPORT_MODULE(ExpoDocumentPicker);
...
ABI46_0_0EX_EXPORT_METHOD_AS(getDocumentAsync,
    ...
    @try {
      ...
      if (@available(iOS 14, *)) {
        ...
        for (int i = 0; i < [mimeTypes count]; i++) {
          utTypes[i] = (NSString *)ABI46_0_0EXConvertMimeTypeToUTType(mimeTypes[i]);
        }
        ...
        for (int i = 0; i < [mimeTypes count]; i++) {
          utiTypes[i] = (NSString *)ABI46_0_0EXConvertMimeTypeToUTI(mimeTypes[i]);
        }
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url
{
  ...
  unsigned long long fileSize = [ABI46_0_0EXDocumentPickerModule getFileSize:[url path] error:&fileSizeError];
  if (fileSizeError) {
  ...
  NSString *extension = [url pathExtension];
  NSString *mimeType = [ABI46_0_0EXDocumentPickerModule getMimeType:extension];
...
+ (unsigned long long)getFileSize:(NSString *)path error:(NSError **)error
{
  ...
  while (file = [contentsEnumurator nextObject]) {
    folderSize += [ABI46_0_0EXDocumentPickerModule getFileSize:[path stringByAppendingPathComponent:file] error:error];
    if (*error) {
      return 0;
    }
  }
