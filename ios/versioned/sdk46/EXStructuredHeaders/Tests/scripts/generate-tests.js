...
// ABI46_0_0EXStructuredHeadersTestFixtures.h
const generatedConstantHeaders = `// GENERATED CONSTANTS BEGIN

${Object.keys(tests)
  .map(name => `FOUNDATION_EXPORT NSString * const ABI46_0_0EXStructuredHeaders${name}Tests;`)
  .join('\n')}
...
regexFile(
  ...
  path.join(__dirname, '../ABI46_0_0EXStructuredHeadersTestFixtures.h')
);

// ABI46_0_0EXStructuredHeadersTestFixtures.m
const generatedConstantsImpl = `// GENERATED CONSTANTS BEGIN

${Object.entries(tests)
  .map(
    ([name, json]) =>
      `NSString * const ABI46_0_0EXStructuredHeaders${name}Tests = @${JSON.stringify(JSON.stringify(json))};`
  )
  .join('\n\n')}
...
regexFile(
  ...
  path.join(__dirname, '../ABI46_0_0EXStructuredHeadersTestFixtures.m')
);

// ABI46_0_0EXStructuredHeadersParserTests.m'
...
${Object.keys(tests)
  .map(
    name => `- (void)test${name}
{
  [self runTests:ABI46_0_0EXStructuredHeaders${name}Tests];
}`
  )
  .join('\n\n')}
...
regexFile(
  ...
  generatedXCTests,
  path.join(__dirname, '../ABI46_0_0EXStructuredHeadersParserTests.m')
);
