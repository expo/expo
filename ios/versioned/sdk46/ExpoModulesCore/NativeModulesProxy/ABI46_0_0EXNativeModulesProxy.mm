...
@interface ABI46_0_0EXModulesProxyConfig ()
...
@implementation ABI46_0_0EXModulesProxyConfig
...
@interface ABI46_0_0RCTBridge (RegisterAdditionalModuleClasses)

- (NSArray<ABI46_0_0RCTModuleData *> *)registerModulesForClasses:(NSArray<Class> *)moduleClasses;
- (void)registerAdditionalModuleClasses:(NSArray<Class> *)modules;

@end

@interface ABI46_0_0RCTBridge (JSIRuntime)

- (void *)runtime;

@end

@interface ABI46_0_0EXNativeModulesProxy ()

@property (nonatomic, strong) NSRegularExpression *regexp;
@property (nonatomic, strong) ABI46_0_0EXModuleRegistry *exModuleRegistry;
...
@implementation ABI46_0_0EXNativeModulesProxy {
  __weak ABI46_0_0EXAppContext * _Nullable _appContext;
}

@synthesize bridge = _bridge;
@synthesize nativeModulesConfig = _nativeModulesConfig;

ABI46_0_0RCT_EXPORT_MODULE(NativeUnimoduleProxy)
 ...
 */
- (instancetype)initWithModuleRegistry:(nullable ABI46_0_0EXModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    _exModuleRegistry = moduleRegistry != nil ? moduleRegistry : [[ABI46_0_0EXModuleRegistryProvider new] moduleRegistry];
...
/**
 The initializer for Expo Go to pass a custom `ABI46_0_0EXModuleRegistry`
 other than the default one from `ABI46_0_0EXModuleRegistryProvider`.
 The `ABI46_0_0EXModuleRegistry` is still owned by this class.
 */
- (instancetype)initWithCustomModuleRegistry:(nonnull ABI46_0_0EXModuleRegistry *)moduleRegistry
{
...
/**
 Convenience initializer used by ABI46_0_0React Native in the new setup, where the modules are registered automatically.
 */
- (instancetype)init
{
...
# pragma mark - ABI46_0_0React API
...
- (nonnull ABI46_0_0EXModulesProxyConfig *)nativeModulesConfig
{
  ...
  NSArray<ABI46_0_0EXViewManager *> *viewManagers = [_exModuleRegistry getAllViewManagers];
  ...
  for (ABI46_0_0EXViewManager *viewManager in viewManagers) {
    viewManagersMetadata[viewManager.viewName] = @{
      @"propsNames": [[viewManager getPropsNames] allKeys]
    };
  }
  
  ABI46_0_0EXModulesProxyConfig *config = [[ABI46_0_0EXModulesProxyConfig alloc] initWithConstants:exportedModulesConstants
...
{
  ABI46_0_0ExpoBridgeModule* expoBridgeModule = [bridge moduleForClass:ABI46_0_0ExpoBridgeModule.class];
...
ABI46_0_0RCT_EXPORT_METHOD(callMethod:(NSString *)moduleName methodNameOrKey:(id)methodNameOrKey arguments:(NSArray *)arguments resolver:(ABI46_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI46_0_0RCTPromiseRejectBlock)reject)
{
...
- (void)registerExpoModulesInBridge:(ABI46_0_0RCTBridge *)bridge
{
  // Registering expo modules (excluding Swifty view managers!) in bridge is needed only when the proxy module owns
  // the registry (was autoinitialized by ABI46_0_0React Native). Otherwise they're registered by the registry adapter.
  BOOL ownsModuleRegistry = _ownsModuleRegistry && ![bridge moduleIsInitialized:[ABI46_0_0EXReactNativeEventEmitter class]];

  // An array of `ABI46_0_0RCTBridgeModule` classes to register.
  NSMutableArray<Class<ABI46_0_0RCTBridgeModule>> *additionalModuleClasses = [NSMutableArray new];
  ...
  if (ownsModuleRegistry) {
    ...
    // so later we'll register an instance created by ABI46_0_0React Native as expo module.
    [additionalModuleClasses addObject:[ABI46_0_0EXReactNativeEventEmitter class]];

    // Add dynamic wrappers for the classic view managers.
    for (ABI46_0_0EXViewManager *viewManager in [_exModuleRegistry getAllViewManagers]) {
      if (![visitedSweetModules containsObject:viewManager.viewName]) {
        Class viewManagerWrapperClass = [ABI46_0_0EXViewManagerAdapterClassesRegistry createViewManagerAdapterClassForViewManager:viewManager];
    ...
    // Otherwise, ABI46_0_0React Native may treat these props as invalid in subclassing views.
    [additionalModuleClasses addObject:[ABI46_0_0EXViewManagerAdapter class]];
    // Also, we have to register component data for the View Adapter.
    // Otherwise, it won't be recognized by the UIManager.
    [self registerLegacyComponentData:[ABI46_0_0EXViewManagerAdapter class] inBridge:bridge];
  ...
  // `registerAdditionalModuleClasses:` call below is not thread-safe if ABI46_0_0RCTUIManager is not initialized.
  // The case happens especially with reanimated which accesses `bridge.uiManager` and initialize bridge in js thread.
  // Accessing uiManager here, we try to make sure ABI46_0_0RCTUIManager is initialized.
  [bridge uiManager];
  ...
  // Get the instance of `ABI46_0_0EXReactEventEmitter` bridge module and give it access to the interop bridge.
  ABI46_0_0EXReactNativeEventEmitter *eventEmitter = [bridge moduleForClass:[ABI46_0_0EXReactNativeEventEmitter class]];
  ...
  if (ownsModuleRegistry) {
    ...
    // It calls `setModuleRegistry:` on all `ABI46_0_0EXModuleRegistryConsumer`s.
    [_exModuleRegistry initialize];
  }
}

- (void)registerAdditionalModuleClasses:(NSArray<Class> *)moduleClasses inBridge:(ABI46_0_0RCTBridge *)bridge
{
  // In remote debugging mode, i.e. executorClass is `ABI46_0_0RCTWebSocketExecutor`,
  // there is a deadlock issue in `registerAdditionalModuleClasses:` and causes app freezed.
  //   - The JS thread acquired the `ABI46_0_0RCTCxxBridge._moduleRegistryLock` lock in `ABI46_0_0RCTCxxBridge._initializeBridgeLocked`
  //      = it further goes into ABI46_0_0RCTObjcExecutor and tries to get module config from main thread
  //   - The main thread is pending in `ABI46_0_0RCTCxxBridge.registerAdditionalModuleClasses` where trying to acquire the same lock.
  ...
  // As we just workaround in `ABI46_0_0RCTWebSocketExecutor` case, the risk of `_moduleRegistryCreated` race condition should be lower.
  //
  // Learn more about the non-locked initialization:
  // https://github.com/facebook/react-native/blob/757bb75fbf837714725d7b2af62149e8e2a7ee51/ABI46_0_0React/CxxBridge/ABI46_0_0RCTCxxBridge.mm#L922-L935
  // See the `_moduleRegistryCreated` false case
  if ([NSStringFromClass([bridge executorClass]) isEqualToString:@"ABI46_0_0RCTWebSocketExecutor"]) {
...
- (Class)registerComponentData:(ViewModuleWrapper *)viewModule inBridge:(ABI46_0_0RCTBridge *)bridge
{
  ...
  ABI46_0_0EXComponentData *componentData = [[ABI46_0_0EXComponentData alloc] initWithViewModule:viewModule
 ...
 */
- (void)registerLegacyComponentData:(Class)moduleClass inBridge:(ABI46_0_0RCTBridge *)bridge
{
  // Hacky way to get a dictionary with `ABI46_0_0RCTComponentData` from UIManager.
  NSMutableDictionary<NSString *, ABI46_0_0RCTComponentData *> *componentDataByName = [bridge.uiManager valueForKey:@"_componentDataByName"];
