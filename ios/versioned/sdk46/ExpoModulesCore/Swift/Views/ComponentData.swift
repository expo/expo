...
/**
 Custom component data extending `ABI46_0_0RCTComponentData`. Its main purpose is to handle event-based props (callbacks),
 but it also simplifies capturing the view config so we can omit some reflections that ABI46_0_0React Native executes.
 */
@objc(ABI46_0_0EXComponentData)
public final class ComponentData: ABI46_0_0RCTComponentData {
  /**
   ...
   */
  @objc
  public init(viewModule: ViewModuleWrapper, managerClass: ViewModuleWrapper.Type, bridge: ABI46_0_0RCTBridge) {
  ...
  // MARK: ABI46_0_0RCTComponentData

  /**
   Creates a setter for the specific prop. For non-event props we just let ABI46_0_0React Native do its job.
   Events are handled differently to conveniently use them in Swift.
   */
  public override func createPropBlock(_ propName: String, isShadowView: Bool) -> ABI46_0_0RCTPropBlockAlias {
  ...
  /**
   The base `ABI46_0_0RCTComponentData` class does some Objective-C dynamic calls in this function, but we don't
  ...
  public override func viewConfig() -> [String: Any] {
    ...
    if let eventNames = moduleHolder?.viewManager?.eventNames {
      for eventName in eventNames {
        directEvents.append(ABI46_0_0RCTNormalizeInputEventName(eventName))
 ...
 */
private func createEventSetter(eventName: String, bridge: ABI46_0_0RCTBridge?) -> ABI46_0_0RCTPropBlockAlias {
  return { [weak bridge] (target: ABI46_0_0RCTComponent, value: Any) in
    ...
    // For callbacks ABI46_0_0React Native passes a bool value whether the prop is specified or not.
    if value as? Bool == true {
      event.settle { [weak target] (body: Any) in
        if let target = target {
          let componentEvent = ABI46_0_0RCTComponentEvent(name: eventName, viewTag: target.abi46_0_0ReactTag, body: ["payload": body])
