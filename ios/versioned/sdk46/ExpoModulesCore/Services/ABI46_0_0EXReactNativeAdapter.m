...
#import <ABI46_0_0React/ABI46_0_0RCTAppState.h>
#import <ABI46_0_0React/ABI46_0_0RCTImageLoader.h>

#import <ABI46_0_0ExpoModulesCore/ABI46_0_0EXReactNativeAdapter.h>

@interface ABI46_0_0EXReactNativeAdapter ()
...
@implementation ABI46_0_0EXReactNativeAdapter

ABI46_0_0EX_REGISTER_MODULE();

+ (NSString *)moduleName
{
  return @"ABI46_0_0EXReactNativeAdapter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI46_0_0EXAppLifecycleService), @protocol(ABI46_0_0EXUIManager), @protocol(ABI46_0_0EXJavaScriptContextProvider)];
}
...
{
  __weak ABI46_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI46_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI46_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI46_0_0RCTJSThread];
}
...
# pragma mark - ABI46_0_0EXJavaScriptContextProvider
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI46_0_0RCTContentDidAppearNotification,
                           ABI46_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI46_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI46_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI46_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI46_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
