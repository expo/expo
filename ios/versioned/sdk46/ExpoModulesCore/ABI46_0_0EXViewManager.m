...
#define ABI46_0_0EX_IS_METHOD_PROPSETTER(methodName) \
  [methodName hasPrefix:@ABI46_0_0EXPAND_AND_QUOTE(ABI46_0_0EX_PROPSETTERS_PREFIX)]

#define ABI46_0_0EX_PROPSETTER_FOR_PROP(propName)\
  QUOTE(ABI46_0_0EX_PROPSETTERS_PREFIX)propName

static const NSString *noViewExceptionName = @"No custom -(UIView *)view implementation.";
static const NSString *noViewExceptionReason = @"You've subclassed an ABI46_0_0EXViewManager, but didn't override the -(UIView *)view method. Override this method and return a new view instance.";

static const NSString *noViewNameExceptionName = @"No custom -(NSString *)viewName implementation.";
static const NSString *noViewNameExceptionReasonFormat = @"You've subclassed an ABI46_0_0EXViewManager in %@, but didn't override the -(NSString *)viewName method. Override this method and return a name of the view component.";

@interface ABI46_0_0EXViewManager ()

@property NSDictionary<NSString *, NSString *> *propsNamesSelectors;

@end

@implementation ABI46_0_0EXViewManager
...
// Scans the class methods for methods with a certain prefix (see macro ABI46_0_0EX_PROPSETTERS_PREFIX),
...
- (NSDictionary<NSString *, NSString *> *)getPropsNames
{
  ...
  @try {
    for(int i = 0; i < methodsCount; i++) {
      ...
      if (ABI46_0_0EX_IS_METHOD_PROPSETTER(methodName)) {
        NSString *propNameWithArguments = [methodName substringFromIndex:[@ABI46_0_0EXPAND_AND_QUOTE(ABI46_0_0EX_PROPSETTERS_PREFIX) length]];
...
- (void)updateProp:(NSString *)propName withValue:(id)value onView:(UIView *)view
{
  if (_propsNamesSelectors[propName]) {
    ...
    if (methodSignature == nil) {
      ...
      ABI46_0_0EXLogError(@"View manager of view '%@' does not implement method for selector '%@'.", [self viewName], NSStringFromSelector(selector));
      return;
    }
    ...
    ABI46_0_0EXLogWarn(@"Tried to set property `%@` on view manager of view `%@` when the view manager does not export such prop.", propName, [self viewName]);
  }
}
