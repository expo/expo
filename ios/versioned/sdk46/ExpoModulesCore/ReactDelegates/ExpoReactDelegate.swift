...
public class ExpoReactDelegate: NSObject {
  ...
  @objc
  public func createBridge(delegate: ABI46_0_0RCTBridgeDelegate, launchOptions: [AnyHashable: Any]?) -> ABI46_0_0RCTBridge {
    ...
    let result = self.handlers.lazy
      ...
      .first(where: { _ in true }) ?? ABI46_0_0RCTBridge(delegate: delegate, launchOptions: launchOptions)!
    self.handlers.forEach { $0.bridgeDidCreate(bridge: result) }
    return result
  }

  @objc
  public func createRootView(bridge: ABI46_0_0RCTBridge, moduleName: String, initialProperties: [AnyHashable: Any]?) -> UIView {
    return self.handlers.lazy
      ...
      .first(where: { _ in true }) ?? ABI46_0_0EXAppSetupDefaultRootView(bridge, moduleName, initialProperties)
  }
