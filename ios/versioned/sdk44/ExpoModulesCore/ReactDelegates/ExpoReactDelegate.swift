...
public class ExpoReactDelegate: NSObject {
  ...
  @objc
  public func createBridge(delegate: ABI44_0_0RCTBridgeDelegate, launchOptions: [AnyHashable : Any]?) -> ABI44_0_0RCTBridge {
    ...
    let result = self.handlers.lazy
      ...
      .first(where: { _ in true }) ?? ABI44_0_0RCTBridge(delegate: delegate, launchOptions: launchOptions)!
    self.handlers.forEach { $0.bridgeDidCreate(bridge: result) }
    return result
  }

  @objc
  public func createRootView(bridge: ABI44_0_0RCTBridge, moduleName: String, initialProperties: [AnyHashable : Any]?) -> ABI44_0_0RCTRootView {
    return self.handlers.lazy
      ...
      .first(where: { _ in true }) ?? ABI44_0_0RCTRootView(bridge: bridge, moduleName: moduleName, initialProperties: initialProperties)
  }
