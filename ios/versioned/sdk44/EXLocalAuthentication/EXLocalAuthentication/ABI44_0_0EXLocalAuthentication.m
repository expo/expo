...
#import <ABI44_0_0EXLocalAuthentication/ABI44_0_0EXLocalAuthentication.h>

typedef NS_ENUM(NSInteger, ABI44_0_0EXAuthenticationType) {
  ABI44_0_0EXAuthenticationTypeFingerprint = 1,
  ABI44_0_0EXAuthenticationTypeFacialRecognition = 2,
};

typedef NS_ENUM(NSInteger, ABI44_0_0EXSecurityLevel) {
  ABI44_0_0EXSecurityLevelNone = 0,
  ABI44_0_0EXSecurityLevelSecret = 1,
  ABI44_0_0EXSecurityLevelBiometric = 2,
};

@implementation ABI44_0_0EXLocalAuthentication

ABI44_0_0EX_EXPORT_MODULE(ExpoLocalAuthentication)

ABI44_0_0EX_EXPORT_METHOD_AS(supportedAuthenticationTypesAsync,
                    supportedAuthenticationTypesAsync:(ABI44_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI44_0_0EXPromiseRejectBlock)reject)
{
  NSMutableArray *results = [NSMutableArray array];
  if ([[self class] isTouchIdDevice]) {
    [results addObject:@(ABI44_0_0EXAuthenticationTypeFingerprint)];
  }
  if ([[self class] isFaceIdDevice]) {
    [results addObject:@(ABI44_0_0EXAuthenticationTypeFacialRecognition)];
  }
  resolve(results);
}

ABI44_0_0EX_EXPORT_METHOD_AS(hasHardwareAsync,
                    hasHardwareAsync:(ABI44_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI44_0_0EXPromiseRejectBlock)reject)
{
...
ABI44_0_0EX_EXPORT_METHOD_AS(isEnrolledAsync,
                    isEnrolledAsync:(ABI44_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI44_0_0EXPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;
...
ABI44_0_0EX_EXPORT_METHOD_AS(getEnrolledLevelAsync,
                    getEnrolledLevelAsync:(ABI44_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI44_0_0EXPromiseRejectBlock)reject)
{
  LAContext *context = [LAContext new];
  NSError *error = nil;

  int level = ABI44_0_0EXSecurityLevelNone;
  ...
  if (isAuthenticationSupported && error == nil) {
    level = ABI44_0_0EXSecurityLevelSecret;
  }
  ...
  if (isBiometricsSupported && error == nil) {
    level = ABI44_0_0EXSecurityLevelBiometric;
  }
          ...
          localizedReason:reason
                    reply:^(BOOL success, NSError *error) {
                      resolve(@{
                                ...
                                @"warning": ABI44_0_0EXNullIfNil(warningMessage),
                                });
                    }];
  } else {
