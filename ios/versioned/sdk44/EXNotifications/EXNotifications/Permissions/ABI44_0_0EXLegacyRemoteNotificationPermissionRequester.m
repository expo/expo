...
@interface ABI44_0_0EXLegacyRemoteNotificationPermissionRequester ()
...
@property (nonatomic, weak) id<ABI44_0_0EXPermissionsRequester> userNotificationPermissionRequester;
@property (nonatomic, weak) dispatch_queue_t methodQueue;
@property (nonatomic, weak) id<ABI44_0_0EXRemoteNotificationPermissionProgressPublisher> permissionProgressPublisher;

@end

@implementation ABI44_0_0EXLegacyRemoteNotificationPermissionRequester
...
{
  __block ABI44_0_0EXPermissionStatus status;
  [ABI44_0_0EXUtilities performSynchronouslyOnMainThread:^{
    status = (ABI44_0_0UMSharedApplication().isRegisteredForRemoteNotifications) ?
...
- (void)requestPermissionsWithResolver:(ABI44_0_0EXPromiseResolveBlock)resolve rejecter:(ABI44_0_0EXPromiseRejectBlock)reject
{
  ...
  [ABI44_0_0EXUtilities performSynchronouslyOnMainThread:^{
    isRegisteredForRemoteNotifications = ABI44_0_0UMSharedApplication().isRegisteredForRemoteNotifications;
  }];
  ...
  if (isRegisteredForRemoteNotifications) {
    ...
    [_userNotificationPermissionRequester requestPermissionsWithResolver:^(NSDictionary *permission){
      ...
      ABI44_0_0EXPermissionStatus localNotificationsStatus = [[permission objectForKey:@"status"] intValue];
      // We may assume that `ABI44_0_0EXLocalNotificationRequester`'s permission request will always finish
      ...
      if (localNotificationsStatus == ABI44_0_0EXPermissionStatusDenied) {
        [self _clearObserver];
        [self _maybeConsumeResolverWithCurrentPermissions];
      } else {
        self.remoteNotificationsRegistrationIsPending = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
          [ABI44_0_0UMSharedApplication() registerForRemoteNotifications];
        });
      }
