...
@interface ABI44_0_0EXNotificationCenterDelegate ()
...
@implementation ABI44_0_0EXNotificationCenterDelegate

ABI44_0_0EX_REGISTER_SINGLETON_MODULE(NotificationCenterDelegate);
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary<UIApplicationLaunchOptionsKey,id> *)launchOptions
{
  if ([UNUserNotificationCenter currentNotificationCenter].delegate) {
    ABI44_0_0EXLogWarn(@"[expo-notifications] ABI44_0_0EXNotificationCenterDelegate encountered already present delegate of UNUserNotificationCenter: %@. "
              "ABI44_0_0EXNotificationCenterDelegate will not overwrite the value not to break other features of your app. "
              "In return, expo-notifications may not work properly. "
              "To fix this problem either remove setting of the second delegate "
              "or set the delegate to an instance of ABI44_0_0EXNotificationCenterDelegate manually afterwards.",
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler
{
  ...
  for (int i = 0; i < _delegates.count; i++) {
    ...
    if ([pointer respondsToSelector:@selector(userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:)] 
      && ![NSStringFromClass([pointer class]) isEqual: @"ABI44_0_0EXUserNotificationManager"]) {
      // Remove ABI44_0_0EXUserNotificationManager check when LegacyNotifications are no longer supported
...
# pragma mark - ABI44_0_0EXNotificationCenterDelegate
...
- (void)addDelegate:(id<ABI44_0_0EXNotificationsDelegate>)delegate
{
  ...
  if ([delegate respondsToSelector:@selector(userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:)]) {
    ...
    // Remove ABI44_0_0EXUserNotificationManager check when LegacyNotifications are no longer supported
    if (![NSStringFromClass([delegate class]) isEqual:@"ABI44_0_0EXUserNotificationManager"]) {
