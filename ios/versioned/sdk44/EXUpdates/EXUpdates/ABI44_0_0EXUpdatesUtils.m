...
static NSString * const ABI44_0_0EXUpdatesEventName = @"Expo.nativeUpdatesEvent";
static NSString * const ABI44_0_0EXUpdatesUtilsErrorDomain = @"ABI44_0_0EXUpdatesUtils";

@implementation ABI44_0_0EXUpdatesUtils
...
+ (nullable NSURL *)initializeUpdatesDirectoryWithError:(NSError ** _Nullable)error
{
  ...
  if (exists) {
    if (!isDir) {
      *error = [NSError errorWithDomain:ABI44_0_0EXUpdatesUtilsErrorDomain code:1005 userInfo:@{NSLocalizedDescriptionKey: @"Failed to create the Updates Directory; a file already exists with the required directory name"}];
      return nil;
    }
  } else {
    NSError *err;
...
+ (void)sendEventToBridge:(nullable ABI44_0_0RCTBridge *)bridge withType:(NSString *)eventType body:(NSDictionary *)body
{
  if (bridge) {
    ...
    [bridge enqueueJSCall:@"ABI44_0_0RCTDeviceEventEmitter.emit" args:@[ABI44_0_0EXUpdatesEventName, mutableBody]];
  } else {
    NSLog(@"ABI44_0_0EXUpdates: Could not emit %@ event. Did you set the bridge property on the controller singleton?", eventType);
  }
}
...
+ (BOOL)shouldCheckForUpdateWithConfig:(ABI44_0_0EXUpdatesConfig *)config
{
  switch (config.checkOnLaunch) {
    case ABI44_0_0EXUpdatesCheckAutomaticallyConfigNever:
      return NO;
    case ABI44_0_0EXUpdatesCheckAutomaticallyConfigErrorRecoveryOnly:
      // check will happen later on if there's an error
      return NO;
    case ABI44_0_0EXUpdatesCheckAutomaticallyConfigWifiOnly: {
    ...
    case ABI44_0_0EXUpdatesCheckAutomaticallyConfigAlways:
    default:
      return YES;
  }
}
