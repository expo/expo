...
struct ABI44_0_0EXUpdatesCodeSigningMetadataFields {
  static let KeyIdFieldKey = "keyid"
  static let AlgorithmFieldKey = "alg"
}

@objc public enum ABI44_0_0EXUpdatesValidationResult : Int {
  case Valid
  case Invalid
  case Skipped
}
...
@objc
public class ABI44_0_0EXUpdatesSignatureValidationResult : NSObject {
  @objc private(set) public var validationResult: ABI44_0_0EXUpdatesValidationResult
  @objc private(set) public var expoProjectInformation: ABI44_0_0EXUpdatesExpoProjectInformation?
  
  required init(validationResult: ABI44_0_0EXUpdatesValidationResult, expoProjectInformation: ABI44_0_0EXUpdatesExpoProjectInformation?) {
...
@objc
public class ABI44_0_0EXUpdatesCodeSigningConfiguration : NSObject {
  private var embeddedCertificateString: String
  private var keyIdFromMetadata: String
  private var algorithmFromMetadata: ABI44_0_0EXUpdatesCodeSigningAlgorithm
  ...
  public required init(embeddedCertificateString: String,
    ...
    self.keyIdFromMetadata = metadata[ABI44_0_0EXUpdatesCodeSigningMetadataFields.KeyIdFieldKey] ?? ABI44_0_0EXUpdatesSignatureHeaderInfo.DefaultKeyId
    self.algorithmFromMetadata = try ABI44_0_0EXUpdatesCodeSigningAlgorithm.parseFromString(metadata[ABI44_0_0EXUpdatesCodeSigningMetadataFields.AlgorithmFieldKey])
  ...
  public func createAcceptSignatureHeader() -> String {
    return "sig, keyid=\"\(ABI44_0_0EXUpdatesCodeSigningConfiguration.escapeStructuredHeaderStringItem(keyIdFromMetadata))\", alg=\"\(ABI44_0_0EXUpdatesCodeSigningConfiguration.escapeStructuredHeaderStringItem(algorithmFromMetadata.rawValue))\""
  }
  
  @objc
  public func validateSignature(signature: String?,
                                signedData: Data,
                                manifestResponseCertificateChain: String?) throws -> ABI44_0_0EXUpdatesSignatureValidationResult {
    guard let signature = signature else {
      if !self.allowUnsignedManifests {
        throw ABI44_0_0EXUpdatesCodeSigningError.SignatureHeaderMissing
      } else {
        // no-op
        return ABI44_0_0EXUpdatesSignatureValidationResult(validationResult: ABI44_0_0EXUpdatesValidationResult.Skipped, expoProjectInformation: nil)
      }
    }
    
    return try validateSignatureInternal(
      signatureHeaderInfo: try ABI44_0_0EXUpdatesSignatureHeaderInfo.parseSignatureHeader(signatureHeader: signature),
      signedData: signedData,
      manifestResponseCertificateChain: manifestResponseCertificateChain
    )
  }
  
  private func validateSignatureInternal(signatureHeaderInfo: ABI44_0_0EXUpdatesSignatureHeaderInfo,
                                         signedData: Data,
                                         manifestResponseCertificateChain: String?) throws -> ABI44_0_0EXUpdatesSignatureValidationResult {
    let certificateChain: ABI44_0_0EXUpdatesCertificateChain
    if (self.includeManifestResponseCertificateChain) {
      certificateChain = try ABI44_0_0EXUpdatesCertificateChain(
        certificateStrings: ABI44_0_0EXUpdatesCodeSigningConfiguration.separateCertificateChain(certificateChainInManifestResponse: manifestResponseCertificateChain ?? "") + [self.embeddedCertificateString]
      )
    } else {
      ...
      if (signatureHeaderInfo.keyId != self.keyIdFromMetadata) {
        throw ABI44_0_0EXUpdatesCodeSigningError.KeyIdMismatchError
      }
      ...
      certificateChain = try ABI44_0_0EXUpdatesCertificateChain(certificateStrings: [embeddedCertificateString])
    }
    ...
    guard let publicKey = secCertificate.publicKey else {
      throw ABI44_0_0EXUpdatesCodeSigningError.CertificateMissingPublicKeyError
    }
    
    guard let signatureData = Data(base64Encoded: signatureHeaderInfo.signature) else {
      throw ABI44_0_0EXUpdatesCodeSigningError.SignatureEncodingError
    }
    
    let isValid = try self.verifyRSASHA256SignedData(signedData: signedData, signatureData: signatureData, publicKey: publicKey)
    return ABI44_0_0EXUpdatesSignatureValidationResult(validationResult: isValid ? ABI44_0_0EXUpdatesValidationResult.Valid : ABI44_0_0EXUpdatesValidationResult.Invalid,
  ...
  private func verifyRSASHA256SignedData(signedData: Data, signatureData: Data, publicKey: SecKey) throws -> Bool {
    ...
    if SecKeyVerifySignature(publicKey, .rsaSignatureDigestPKCS1v15SHA256, hashBytes as CFData, signatureData as CFData, &error) {
      ...
      if let error = error, (error.takeRetainedValue() as Error as NSError).code != errSecVerifyFailed {
        ...
        throw ABI44_0_0EXUpdatesCodeSigningError.SecurityFrameworkError
      }
      return false
    }
  }
