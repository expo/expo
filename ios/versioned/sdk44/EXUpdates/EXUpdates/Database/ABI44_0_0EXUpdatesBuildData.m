 ...
 *
 *   ABI44_0_0EXUpdatesReleaseChannel
 *   ABI44_0_0EXUpdatesURL
 *
 * and all of the values in json
 *
 *   ABI44_0_0EXUpdatesRequestHeaders
 */
@implementation ABI44_0_0EXUpdatesBuildData

+ (void)ensureBuildDataIsConsistentAsync:(ABI44_0_0EXUpdatesDatabase *)database
...
{
  return @{
    @"ABI44_0_0EXUpdatesURL": config.updateUrl.absoluteString,
    @"ABI44_0_0EXUpdatesReleaseChannel": config.releaseChannel,
    @"ABI44_0_0EXUpdatesRequestHeaders": config.requestHeaders,
  };
}

+ (void)clearAllUpdatesAndSetStaticBuildData:(ABI44_0_0EXUpdatesDatabase *)database
                             config:(ABI44_0_0EXUpdatesConfig *)config
{
  NSError *queryError;
  NSArray<ABI44_0_0EXUpdatesUpdate *> *allUpdates = [database allUpdatesWithConfig:config error:&queryError];
  if (queryError) {
