...
@interface ABI44_0_0EXUpdatesDatabase ()

@property (nonatomic, assign) sqlite3 *db;

@end

static NSString * const ABI44_0_0EXUpdatesDatabaseManifestFiltersKey = @"manifestFilters";
static NSString * const ABI44_0_0EXUpdatesDatabaseServerDefinedHeadersKey = @"serverDefinedHeaders";
static NSString * const ABI44_0_0EXUpdatesDatabaseStaticBuildDataKey = @"staticBuildData";

@implementation ABI44_0_0EXUpdatesDatabase
...
- (BOOL)openDatabaseInDirectory:(NSURL *)directory withError:(NSError ** _Nullable)error
{
  ...
  sqlite3 *db;
  if (![ABI44_0_0EXUpdatesDatabaseInitialization initializeDatabaseWithLatestSchemaInDirectory:directory database:&db error:error]) {
    return NO;
  }
...
- (void)addNewAssets:(NSArray<ABI44_0_0EXUpdatesAsset *> *)assets toUpdateWithId:(NSUUID *)updateId error:(NSError ** _Nullable)error
{
  ...
  for (ABI44_0_0EXUpdatesAsset *asset in assets) {
...
{
  if (update.status != ABI44_0_0EXUpdatesUpdateStatusDevelopment) {
    update.status = ABI44_0_0EXUpdatesUpdateStatusReady;
  }
...
- (void)markMissingAssets:(NSArray<ABI44_0_0EXUpdatesAsset *> *)assets error:(NSError ** _Nullable)error
{
  ...
  for (ABI44_0_0EXUpdatesAsset *asset in assets) {
    if ([self _executeSql:updatesSql withArgs:@[@(ABI44_0_0EXUpdatesUpdateStatusPending), @(asset.assetId)] error:error] == nil) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
      return;
    }
  }
...
- (nullable NSArray<ABI44_0_0EXUpdatesUpdate *> *)allUpdatesWithConfig:(ABI44_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI44_0_0EXUpdatesUpdate *> *launchableUpdates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI44_0_0EXUpdatesUpdate *> *)allUpdatesWithStatus:(ABI44_0_0EXUpdatesUpdateStatus)status config:(ABI44_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI44_0_0EXUpdatesUpdate *> *updates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI44_0_0EXUpdatesUpdate *> *)launchableUpdatesWithConfig:(ABI44_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  AND status IN (%li, %li, %li);", (long)ABI44_0_0EXUpdatesUpdateStatusReady, (long)ABI44_0_0EXUpdatesUpdateStatusEmbedded, (long)ABI44_0_0EXUpdatesUpdateStatusDevelopment];
...
- (nullable NSArray<ABI44_0_0EXUpdatesAsset *> *)allAssetsWithError:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI44_0_0EXUpdatesAsset *> *assets = [NSMutableArray arrayWithCapacity:rows.count];
...
{
  return [self _jsonDataWithKey:ABI44_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey error:error];
}

- (nullable NSDictionary *)manifestFiltersWithScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  return [self _jsonDataWithKey:ABI44_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey error:error];
}

- (nullable NSDictionary *)staticBuildDataWithScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  return [self _jsonDataWithKey:ABI44_0_0EXUpdatesDatabaseStaticBuildDataKey scopeKey:scopeKey error:error];
}
...
{
  [self _setJsonData:serverDefinedHeaders withKey:ABI44_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey isInTransaction:NO error:error];
}

- (void)setManifestFilters:(NSDictionary *)manifestFilters withScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  [self _setJsonData:manifestFilters withKey:ABI44_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey isInTransaction:NO error:error];
}
...
- (void)setMetadataWithManifest:(ABI44_0_0EXUpdatesUpdate *)updateManifest error:(NSError ** _Nullable)error
{
  ...
  if (updateManifest.serverDefinedHeaders) {
    if (![self _setJsonData:updateManifest.serverDefinedHeaders
                   withKey:ABI44_0_0EXUpdatesDatabaseServerDefinedHeadersKey
  ...
  if (updateManifest.manifestFilters) {
    if (![self _setJsonData:updateManifest.manifestFilters
                   withKey:ABI44_0_0EXUpdatesDatabaseManifestFiltersKey
                  scopeKey:updateManifest.scopeKey
           isInTransaction:YES
                     error:error]) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
...
{
  [self _setJsonData:staticBuildData withKey:ABI44_0_0EXUpdatesDatabaseStaticBuildDataKey scopeKey:scopeKey isInTransaction:NO error:error];
}
...
- (nullable NSArray<NSDictionary *> *)_executeSql:(NSString *)sql withArgs:(nullable NSArray *)args error:(NSError ** _Nullable)error
{
  ...
  dispatch_assert_queue(_databaseQueue);
  return [ABI44_0_0EXUpdatesDatabaseUtils executeSql:sql withArgs:args onDatabase:_db error:error];
}

- (NSError *)_errorFromSqlite:(struct sqlite3 *)db
{
  return [ABI44_0_0EXUpdatesDatabaseUtils errorFromSqlite:db];
}

- (ABI44_0_0EXUpdatesUpdate *)_updateWithRow:(NSDictionary *)row config:(ABI44_0_0EXUpdatesConfig *)config
{
  NSError *error;
  id manifest = nil;
  ...
  ABI44_0_0EXUpdatesUpdate *update = [ABI44_0_0EXUpdatesUpdate updateWithId:row[@"id"]
                                                 scopeKey:row[@"scope_key"]
                                               commitTime:[ABI44_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"commit_time"]]
                                           runtimeVersion:row[@"runtime_version"]
                                                 manifest:manifest
                                                   status:(ABI44_0_0EXUpdatesUpdateStatus)[(NSNumber *)row[@"status"] integerValue]
                                                     keep:[(NSNumber *)row[@"keep"] boolValue]
                                                   config:config
                                                 database:self];
  update.lastAccessed = [ABI44_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"last_accessed"]];
