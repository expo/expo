...
static NSString * const ABI44_0_0EXUpdatesExpoAssetBaseUrl = @"https://d1wp6m56sqw74a.cloudfront.net/~assets/";
static NSString * const ABI44_0_0EXUpdatesExpoIoDomain = @"expo.io";
static NSString * const ABI44_0_0EXUpdatesExpHostDomain = @"exp.host";
static NSString * const ABI44_0_0EXUpdatesExpoTestDomain = @"expo.test";

@implementation ABI44_0_0EXUpdatesLegacyUpdate
                                     ...
                                     database:(ABI44_0_0EXUpdatesDatabase *)database
{
  ...
  if (manifest.isUsingDeveloperTool) {
    ...
    update.commitTime = [ABI44_0_0RCTConvert NSDate:commitTimeString];
  }

  if (manifest.isDevelopmentMode) {
    update.isDevelopmentMode = YES;
    update.status = ABI44_0_0EXUpdatesUpdateStatusDevelopment;
  } else {
    update.status = ABI44_0_0EXUpdatesUpdateStatusPending;
  }
  ...
  NSMutableArray<ABI44_0_0EXUpdatesAsset *> *processedAssets = [NSMutableArray new];

  NSString *bundleKey = manifest.bundleKey ?: nil;
  ABI44_0_0EXUpdatesAsset *jsBundleAsset = [[ABI44_0_0EXUpdatesAsset alloc] initWithKey:bundleKey type:ABI44_0_0EXUpdatesEmbeddedBundleFileType];
  ...
  for (NSString *bundledAsset in assets) {
    ...
    NSString *key = hash;
    ABI44_0_0EXUpdatesAsset *asset = [[ABI44_0_0EXUpdatesAsset alloc] initWithKey:key type:(NSString *)type];
    asset.url = url;
...
+ (NSURL *)bundledAssetBaseUrlWithManifest:(ABI44_0_0EXManifestsLegacyManifest *)manifest config:(ABI44_0_0EXUpdatesConfig *)config
{
  ...
  if (!host ||
      [host containsString:ABI44_0_0EXUpdatesExpoIoDomain] ||
      [host containsString:ABI44_0_0EXUpdatesExpHostDomain] ||
      [host containsString:ABI44_0_0EXUpdatesExpoTestDomain]) {
    return [NSURL URLWithString:ABI44_0_0EXUpdatesExpoAssetBaseUrl];
  } else {
