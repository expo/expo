      ...
      [ABI44_0_0EXUpdatesCrypto verifySignatureWithData:(NSString *)manifestString
                                  ...
                                  successBlock:^(BOOL isValid) {
                                                  if (isValid) {
                                                    ...
                                                    NSError *error = [NSError errorWithDomain:ABI44_0_0EXUpdatesFileDownloaderErrorDomain code:1003 userInfo:@{NSLocalizedDescriptionKey: @"Manifest verification failed"}];
                       ...
                       errorBlock:(ABI44_0_0EXUpdatesFileDownloaderErrorBlock)errorBlock
{
  ...
  @try {
    update = [ABI44_0_0EXUpdatesUpdate updateWithManifest:mutableManifest.copy
                                                         response:response
                                                           config:_config
                                                         database:database
                                                            error:&error];
...
- (nullable NSDictionary *)_extractUpdateResponseDictionary:(id)parsedJson error:(NSError **)error
{
  ...
  if (error) {
    *error = [NSError errorWithDomain:ABI44_0_0EXUpdatesFileDownloaderErrorDomain code:1009 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"No compatible update found at %@. Only %@ are supported.", _config.updateUrl.absoluteString, _config.sdkVersion]}];
  }
  return nil;
}
