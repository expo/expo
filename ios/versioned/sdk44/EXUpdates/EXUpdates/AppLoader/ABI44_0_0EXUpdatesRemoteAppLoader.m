...
@interface ABI44_0_0EXUpdatesRemoteAppLoader ()

@property (nonatomic, strong) ABI44_0_0EXUpdatesFileDownloader *downloader;
@property (nonatomic, strong) ABI44_0_0EXUpdatesUpdate *remoteUpdate;

@property (nonatomic, strong) dispatch_queue_t completionQueue;

@end
static NSString * const ABI44_0_0EXUpdatesRemoteAppLoaderErrorDomain = @"ABI44_0_0EXUpdatesRemoteAppLoader";

@implementation ABI44_0_0EXUpdatesRemoteAppLoader
               ...
               completionQueue:(dispatch_queue_t)completionQueue
{
  if (self = [super initWithConfig:config database:database directory:directory launchedUpdate:launchedUpdate completionQueue:completionQueue]) {
    _downloader = [[ABI44_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:self.config];
                    ...
                    error:(ABI44_0_0EXUpdatesAppLoaderErrorBlock)error
{
  ...
  self.successBlock = ^(ABI44_0_0EXUpdatesUpdate * _Nullable update) {
  ...
  dispatch_async(self.database.databaseQueue, ^{
    ABI44_0_0EXUpdatesUpdate *embeddedUpdate = [ABI44_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:self.config database:self.database];
    NSDictionary *extraHeaders = [ABI44_0_0EXUpdatesFileDownloader extraHeadersWithDatabase:self.database
                                                                            config:self.config
                                                                    launchedUpdate:self.launchedUpdate
                                                                    embeddedUpdate:embeddedUpdate];
    [self->_downloader downloadManifestFromURL:url withDatabase:self.database extraHeaders:extraHeaders successBlock:^(ABI44_0_0EXUpdatesUpdate *update) {
...
- (void)downloadAsset:(ABI44_0_0EXUpdatesAsset *)asset
{
  ...
  dispatch_async([ABI44_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
    if ([[NSFileManager defaultManager] fileExistsAtPath:[urlOnDisk path]]) {
      ...
      if (!asset.url) {
        [self handleAssetDownloadWithError:[NSError errorWithDomain:ABI44_0_0EXUpdatesRemoteAppLoaderErrorDomain code:1006 userInfo:@{NSLocalizedDescriptionKey: @"Failed to download asset with no URL provided"}] asset:asset];
        return;
      }
      ...
      [self->_downloader downloadFileFromURL:asset.url
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          if (asset.expectedHash && ![asset.expectedHash isEqualToString:hashBase64String]) {
            NSError* error = [NSError errorWithDomain:ABI44_0_0EXUpdatesRemoteAppLoaderErrorDomain
                                                 code:1016
