...
#import <ABI44_0_0EXUpdates/ABI44_0_0EXUpdatesAppLauncherWithDatabase+Tests.h>
...
@interface ABI44_0_0EXUpdatesAppLauncherWithDatabase ()
...
@property (nonatomic, strong) ABI44_0_0EXUpdatesFileDownloader *downloader;
@property (nonatomic, copy) ABI44_0_0EXUpdatesAppLauncherCompletionBlock completion;
...
static NSString * const ABI44_0_0EXUpdatesAppLauncherErrorDomain = @"AppLauncher";

@implementation ABI44_0_0EXUpdatesAppLauncherWithDatabase
                   ...
                   completionQueue:(dispatch_queue_t)completionQueue
{
  dispatch_async(database.databaseQueue, ^{
    NSError *error;
    NSArray<ABI44_0_0EXUpdatesUpdate *> *launchableUpdates = [database launchableUpdatesWithConfig:config error:&error];
    ...
    dispatch_async(completionQueue, ^{
      ...
      ABI44_0_0EXUpdatesUpdate *embeddedManifest = [ABI44_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:config database:database];
      NSMutableArray<ABI44_0_0EXUpdatesUpdate *>*filteredLaunchableUpdates = [NSMutableArray new];
      for (ABI44_0_0EXUpdatesUpdate *update in launchableUpdates) {
        if (update.status == ABI44_0_0EXUpdatesUpdateStatusEmbedded) {
...
{
  NSAssert(!_completion, @"ABI44_0_0EXUpdatesAppLauncher:launchUpdateWithSelectionPolicy:successBlock should not be called twice on the same instance");
  _completion = completion;

  if (!_launchedUpdate) {
    [[self class] launchableUpdateWithConfig:_config database:_database selectionPolicy:selectionPolicy completion:^(NSError * _Nullable error, ABI44_0_0EXUpdatesUpdate * _Nullable launchableUpdate) {
      if (error || !launchableUpdate) {
        if (self->_completion) {
          dispatch_async(self->_completionQueue, ^{
            ...
            self->_completion([NSError errorWithDomain:ABI44_0_0EXUpdatesAppLauncherErrorDomain code:1011 userInfo:userInfo], NO);
          });
        }
      } else {
...
- (void)_ensureAllAssetsExist
{
  if (_launchedUpdate.status == ABI44_0_0EXUpdatesUpdateStatusEmbedded) {
    ...
    return;
  } else if (_launchedUpdate.status == ABI44_0_0EXUpdatesUpdateStatusDevelopment) {
    dispatch_async(self->_completionQueue, ^{
...
- (void)_ensureAssetExists:(ABI44_0_0EXUpdatesAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
  [self _checkExistenceOfAsset:asset withLocalUrl:assetLocalUrl completion:^(BOOL exists) {
    ...
    [self _maybeCopyAssetFromMainBundle:asset withLocalUrl:assetLocalUrl completion:^(BOOL success, NSError * _Nullable error) {
      ...
      [self _downloadAsset:asset withLocalUrl:assetLocalUrl completion:^(NSError * _Nullable error, ABI44_0_0EXUpdatesAsset *asset, NSURL *assetLocalUrl) {
        if (error) {
          if (asset.isLaunchAsset) {
...
- (void)_checkExistenceOfAsset:(ABI44_0_0EXUpdatesAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
...
- (void)_maybeCopyAssetFromMainBundle:(ABI44_0_0EXUpdatesAsset *)asset
                         withLocalUrl:(NSURL *)assetLocalUrl
                           completion:(void (^)(BOOL success, NSError * _Nullable error))completion
{
  ABI44_0_0EXUpdatesUpdate *embeddedManifest = [ABI44_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:_config database:_database];
  if (embeddedManifest) {
    ABI44_0_0EXUpdatesAsset *matchingAsset;
    for (ABI44_0_0EXUpdatesAsset *embeddedAsset in embeddedManifest.assets) {
    ...
    if (matchingAsset && matchingAsset.mainBundleFilename) {
      dispatch_async([ABI44_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
        NSString *bundlePath = [ABI44_0_0EXUpdatesUtils pathForBundledAsset:matchingAsset];
        if (bundlePath == nil) {
          dispatch_async(self->_launcherQueue, ^{
            completion(NO, [NSError errorWithDomain:ABI44_0_0EXUpdatesAppLauncherErrorDomain code:1013 userInfo:@{NSLocalizedDescriptionKey: @"Asset bundlePath was unexpectedly nil"}]);
          });
          return;
        }
        NSError *error;
...
- (void)_downloadAsset:(ABI44_0_0EXUpdatesAsset *)asset
          withLocalUrl:(NSURL *)assetLocalUrl
            completion:(void (^)(NSError * _Nullable error, ABI44_0_0EXUpdatesAsset *asset, NSURL *assetLocalUrl))completion
{
  if (!asset.url) {
...
- (ABI44_0_0EXUpdatesFileDownloader *)downloader
{
  if (!_downloader) {
    _downloader = [[ABI44_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:_config];
  }
  return _downloader;
}
