...
#import <ABI44_0_0EXUpdates/ABI44_0_0EXUpdatesDatabaseIntegrityCheck.h>
 ...
 */
@interface ABI44_0_0EXUpdatesDatabaseIntegrityCheckMockingAssetExists : ABI44_0_0EXUpdatesDatabaseIntegrityCheck

@end

@implementation ABI44_0_0EXUpdatesDatabaseIntegrityCheckMockingAssetExists
...
@interface ABI44_0_0EXUpdatesDatabaseIntegrityCheckTests : XCTestCase
...
@implementation ABI44_0_0EXUpdatesDatabaseIntegrityCheckTests
...
- (void)testFilterEmbeddedUpdates
{
  ...
  update1.status = ABI44_0_0EXUpdatesUpdateStatusEmbedded;
  update2.status = ABI44_0_0EXUpdatesUpdateStatusEmbedded;
  ...
  dispatch_sync(_db.databaseQueue, ^{
    ...
    NSError *error3;
    [ABI44_0_0EXUpdatesDatabaseIntegrityCheck runWithDatabase:_db directory:_testDatabaseDir config:config embeddedUpdate:update2 error:&error3];
    XCTAssertNil(error3);

    NSArray<ABI44_0_0EXUpdatesUpdate *> *allUpdates = [_db allUpdatesWithConfig:config error:nil];
...
{
  ABI44_0_0EXUpdatesAsset *asset1 = [[ABI44_0_0EXUpdatesAsset alloc] initWithKey:@"asset1" type:@"png"];
  asset1.downloadTime = [NSDate date];
  asset1.contentHash = @"hash1";
  ABI44_0_0EXUpdatesAsset *asset2 = [[ABI44_0_0EXUpdatesAsset alloc] initWithKey:@"asset2" type:@"png"];
  ...
  update1.status = ABI44_0_0EXUpdatesUpdateStatusReady;
  update2.status = ABI44_0_0EXUpdatesUpdateStatusReady;
  ...
  dispatch_sync(_db.databaseQueue, ^{
    ...
    NSError *error5;
    [ABI44_0_0EXUpdatesDatabaseIntegrityCheckMockingAssetExists runWithDatabase:_db directory:_testDatabaseDir config:config embeddedUpdate:nil error:&error5];
    XCTAssertNil(error5);

    NSArray<ABI44_0_0EXUpdatesUpdate *> *allUpdates = [_db allUpdatesWithConfig:config error:nil];
    NSArray<ABI44_0_0EXUpdatesAsset *> *allAssets = [_db allAssetsWithError:nil];
    XCTAssertEqual(2, allUpdates.count);
    XCTAssertEqual(2, allAssets.count);

    NSArray<ABI44_0_0EXUpdatesUpdate *> *sortedUpdates = [allUpdates sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"commitTime" ascending:YES]]];
    XCTAssertEqual(ABI44_0_0EXUpdatesUpdateStatusReady, sortedUpdates[0].status);
    XCTAssertEqual(ABI44_0_0EXUpdatesUpdateStatusPending, sortedUpdates[1].status);
  });
}
