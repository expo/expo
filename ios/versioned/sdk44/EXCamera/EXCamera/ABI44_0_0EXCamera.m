...
#import <ABI44_0_0ExpoModulesCore/ABI44_0_0EXBarCodeScannerProviderInterface.h>
...
#import <ABI44_0_0ExpoModulesCore/ABI44_0_0EXFaceDetectorManagerInterface.h>
...
@property (nonatomic, strong) id<ABI44_0_0EXFaceDetectorManagerInterface> faceDetectorManager;
@property (nonatomic, strong) id<ABI44_0_0EXBarCodeScannerInterface> barCodeScanner;
...
@property (nonatomic, strong) ABI44_0_0EXPromiseResolveBlock photoCapturedResolve;
@property (nonatomic, strong) ABI44_0_0EXPromiseRejectBlock photoCapturedReject;

@property (nonatomic, strong) ABI44_0_0EXPromiseResolveBlock videoRecordedResolve;
@property (nonatomic, strong) ABI44_0_0EXPromiseRejectBlock videoRecordedReject;

@property (nonatomic, copy) ABI44_0_0EXDirectEventBlock onCameraReady;
@property (nonatomic, copy) ABI44_0_0EXDirectEventBlock onMountError;
@property (nonatomic, copy) ABI44_0_0EXDirectEventBlock onPictureSaved;

@property (nonatomic, copy) ABI44_0_0EXDirectEventBlock onBarCodeScanned;
@property (nonatomic, copy) ABI44_0_0EXDirectEventBlock onFacesDetected;

@end

@implementation ABI44_0_0EXCamera
...
- (void)dealloc
{
  // In very rare case ABI44_0_0EXCamera might be unmounted (and thus deallocated) after starting taking a photo,
...
- (void)updateFlashMode
{
  ...
  if (_flashMode == ABI44_0_0EXCameraFlashModeTorch) {
    if (![device hasTorch]) {
      return;
    }
    ...
    if (![device lockForConfiguration:&error]) {
      if (error) {
        ABI44_0_0EXLogInfo(@"%s: %@", __func__, error);
      }
      return;
    }
...
- (void)updateFocusDepth
{
  ...
  if (device == nil || device.focusMode != ABI44_0_0EXCameraAutoFocusOff) {
    return;
  }
  ...
  if ([device isLockingFocusWithCustomLensPositionSupported]) {
    ...
    ABI44_0_0EX_WEAKIFY(device);
    [device setFocusModeLockedWithLensPosition:_focusDepth completionHandler:^(CMTime syncTime) {
      ABI44_0_0EX_ENSURE_STRONGIFY(device);
      [device unlockForConfiguration];
    }];
    return;
  }

  ABI44_0_0EXLogInfo(@"%s: Setting focusDepth isn't supported for this camera device", __func__);
  return;
}
...
- (void)updateWhiteBalance
{
  ...
  if (_whiteBalance == ABI44_0_0EXCameraWhiteBalanceAuto) {
    [device setWhiteBalanceMode:AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance];
    [device unlockForConfiguration];
  } else {
    AVCaptureWhiteBalanceTemperatureAndTintValues temperatureAndTint = {
      .temperature = [ABI44_0_0EXCameraUtils temperatureForWhiteBalance:_whiteBalance],
      .tint = 0,
    };
...
- (void)setIsScanningBarCodes:(BOOL)barCodeScanning
{
  if (_barCodeScanner) {
    ...
    ABI44_0_0EXLogError(@"BarCodeScanner module not found. Make sure `expo-barcode-scanner` is installed and linked correctly.");
  }
}
...
- (void)setIsDetectingFaces:(BOOL)faceDetecting
{
  if (_faceDetectorManager) {
    ...
    ABI44_0_0EXLogError(@"FaceDetector module not found. Make sure `expo-face-detector` is installed and linked correctly.");
  }
}
...
- (void)takePicture:(NSDictionary *)options resolve:(ABI44_0_0EXPromiseResolveBlock)resolve reject:(ABI44_0_0EXPromiseRejectBlock)reject
{
  ...
  [connection setVideoOrientation:[ABI44_0_0EXCameraUtils videoOrientationForDeviceOrientation:[[UIDevice currentDevice] orientation]]];
  ...
  switch (_flashMode) {
    case ABI44_0_0EXCameraFlashModeOff:
      requestedFlashMode = AVCaptureFlashModeOff;
      break;
    case ABI44_0_0EXCameraFlashModeAuto:
      requestedFlashMode = AVCaptureFlashModeAuto;
      break;
    case ABI44_0_0EXCameraFlashModeOn:
    case ABI44_0_0EXCameraFlashModeTorch:
                ...
                error:(NSError *)error
{
  ...
  ABI44_0_0EXPromiseRejectBlock reject = _photoCapturedReject;
  ABI44_0_0EXPromiseResolveBlock resolve = _photoCapturedResolve;
...
- (void)handleCapturedImageData:(NSData *)imageData metadata:(NSDictionary *)metadata options:(NSDictionary *)options resolver:(ABI44_0_0EXPromiseResolveBlock)resolve reject:(ABI44_0_0EXPromiseRejectBlock)reject
{
  ...
  takenImage = [ABI44_0_0EXCameraUtils cropImage:takenImage toRect:croppedSize];
  ...
  if ([options[@"exif"] boolValue]) {
    NSMutableDictionary *updatedExif = [ABI44_0_0EXCameraUtils updateExifMetadata:metadata[(NSString *)kCGImagePropertyExifDictionary] withAdditionalData:@{ @"Orientation": @([ABI44_0_0EXCameraUtils exportImageOrientation:takenImage.imageOrientation]) }];
    ...
    processedImageData = [ABI44_0_0EXCameraUtils dataFromImage:takenImage withMetadata:updatedMetadata imageQuality:quality];
  } else {
  ...
  response[@"uri"] = [ABI44_0_0EXCameraUtils writeImage:processedImageData toPath:path];
...
- (void)record:(NSDictionary *)options resolve:(ABI44_0_0EXPromiseResolveBlock)resolve reject:(ABI44_0_0EXPromiseRejectBlock)reject
{
  ...
  if (_movieFileOutput != nil && !_movieFileOutput.isRecording && _videoRecordedResolve == nil && _videoRecordedReject == nil) {
    ...
    if (connection.isVideoStabilizationSupported == NO) {
      ABI44_0_0EXLogWarn(@"%s: Video Stabilization is not supported on this device.", __func__);
    } else {
    ...
    if (options[@"quality"]) {
      ABI44_0_0EXCameraVideoResolution resolution = [options[@"quality"] integerValue];
      preset = [ABI44_0_0EXCameraUtils captureSessionPresetForVideoResolution:resolution];
...
- (void)setVideoOptions:(NSDictionary *)options forConnection:(AVCaptureConnection *)connection onReject:(ABI44_0_0EXPromiseRejectBlock)reject
{
  ABI44_0_0EX_WEAKIFY(self);
  dispatch_async(_sessionQueue, ^{
    ABI44_0_0EX_STRONGIFY(self);
    ...
    if (options[@"codec"]) {
      AVVideoCodecType videoCodecType = [ABI44_0_0EXCameraUtils videoCodecForType: [options[@"codec"] integerValue]];
...
#pragma clang diagnostic ignored "-Wunreachable-code"
  ...
  return;
#endif
  if (![_permissionsManager hasGrantedPermissionUsingRequesterClass:[ABI44_0_0EXCameraCameraPermissionRequester class]]) {
...
- (void)initializeCaptureSessionInput
{
  ...
  AVCaptureVideoOrientation orientation = [ABI44_0_0EXCameraUtils videoOrientationForInterfaceOrientation:interfaceOrientation];
  ...
  dispatch_async(_sessionQueue, ^{
    ...
    NSError *error = nil;
    AVCaptureDevice *captureDevice = [ABI44_0_0EXCameraUtils deviceWithMediaType:AVMediaTypeVideo preferringPosition:self.presetCamera];
...
{
  ABI44_0_0EX_WEAKIFY(self);
  AVCaptureVideoOrientation videoOrientation = [ABI44_0_0EXCameraUtils videoOrientationForInterfaceOrientation:orientation];
  [ABI44_0_0EXUtilities performSynchronouslyOnMainThread:^{
    ABI44_0_0EX_ENSURE_STRONGIFY(self);
...
{
  id<ABI44_0_0EXFaceDetectorManagerProviderInterface> faceDetectorProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI44_0_0EXFaceDetectorManagerProviderInterface)];

  if (faceDetectorProvider) {
    id<ABI44_0_0EXFaceDetectorManagerInterface> faceDetector = [faceDetectorProvider createFaceDetectorManager];
    if (faceDetector) {
      ABI44_0_0EX_WEAKIFY(self);
...
{
  id<ABI44_0_0EXBarCodeScannerProviderInterface> barCodeScannerProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI44_0_0EXBarCodeScannerProviderInterface)];
  if (barCodeScannerProvider) {
    id<ABI44_0_0EXBarCodeScannerInterface> barCodeScanner = [barCodeScannerProvider createBarCodeScanner];
    if (barCodeScanner) {
      ABI44_0_0EX_WEAKIFY(self);
