...
#import <ABI44_0_0EXPrint/ABI44_0_0EXPrint.h>
#import <ABI44_0_0EXPrint/ABI44_0_0EXWKPDFRenderer.h>
...
NSString *const ABI44_0_0EXPrintOrientationPortrait = @"portrait";
NSString *const ABI44_0_0EXPrintOrientationLandscape = @"landscape";

@interface ABI44_0_0EXPrint () <UIPrintInteractionControllerDelegate, UIPrinterPickerControllerDelegate>
...
@implementation ABI44_0_0EXPrint

ABI44_0_0EX_EXPORT_MODULE(ExponentPrint);
...
- (NSDictionary *)constantsToExport
{
  return @{
           @"Orientation": @{
               @"portrait": ABI44_0_0EXPrintOrientationPortrait,
               @"landscape": ABI44_0_0EXPrintOrientationLandscape,
               },
           };
}

ABI44_0_0EX_EXPORT_METHOD_AS(print,
                    ...
                    rejecter:(ABI44_0_0EXPromiseRejectBlock)reject)
{
  [self _getPrintingDataForOptions:options callback:^(NSData *printingData, NSDictionary *errorDetails) {
    if (errorDetails != nil) {
      reject(errorDetails[@"code"], errorDetails[@"message"], ABI44_0_0EXErrorWithMessage(errorDetails[@"message"]));
      return;
    }
    ...
    if (printingData == nil) {
      ...
      if (htmlString != nil) {
        ...
        reject(@"E_HTML_INVALID", message, ABI44_0_0EXErrorWithMessage(message));
        return;
      }
    }
...
ABI44_0_0EX_EXPORT_METHOD_AS(selectPrinter,selectPrinter:(ABI44_0_0EXPromiseResolveBlock)resolve
                  rejecter:(ABI44_0_0EXPromiseRejectBlock)reject)
{
...
ABI44_0_0EX_EXPORT_METHOD_AS(printToFileAsync,
...
{
  __block ABI44_0_0EXWKPDFRenderer *renderTask;
  ...
  if ([options[@"useMarkupFormatter"] boolValue]) {
    ...
    renderTask = [ABI44_0_0EXWKPDFRenderer new];
...
{
  id<ABI44_0_0EXUtilitiesInterface> utils = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI44_0_0EXUtilitiesInterface)];
...
- (void)_getPrintingDataForOptions:(nonnull NSDictionary *)options callback:(void(^)(NSData *, NSDictionary *))callback
{
  ...
  if (options[@"html"] && ![options[@"useMarkupFormatter"] boolValue]) {
    __block ABI44_0_0EXWKPDFRenderer *renderTask = [ABI44_0_0EXWKPDFRenderer new];
...
{
  if ([orientation isEqualToString:ABI44_0_0EXPrintOrientationLandscape]) {
