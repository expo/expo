...
static NSString *const EXVideoPlaybackBufferEmptyKeyPath = @"playbackBufferEmpty";
static NSString *const EXVideoReadyForDisplayKeyPath = @"readyForDisplay";
static NSString *const EXVideoPlaybackRateKeyPath = @"rate";
...
{
  EXVideoPlayerViewController* playerLayer = [[EXVideoPlayerViewController alloc] init];
...
{
  [_playerItem addObserver:self forKeyPath:EXVideoStatusKeyPath options:0 context:nil];
  [_playerItem addObserver:self forKeyPath:EXVideoPlaybackBufferEmptyKeyPath options:0 context:nil];
  [_playerItem addObserver:self forKeyPath:EXVideoPlaybackLikelyToKeepUpKeyPath options:0 context:nil];
...
- (void)removePlayerItemObservers
{
  if (_playerItemObserversSet) {
    [_playerItem removeObserver:self forKeyPath:EXVideoStatusKeyPath];
    [_playerItem removeObserver:self forKeyPath:EXVideoPlaybackBufferEmptyKeyPath];
    [_playerItem removeObserver:self forKeyPath:EXVideoPlaybackLikelyToKeepUpKeyPath];
...
- (void)setSrc:(NSDictionary *)source
{
  ...
  if (_playbackRateObserverRegistered) {
    [_player removeObserver:self forKeyPath:EXVideoPlaybackRateKeyPath context:nil];
    _playbackRateObserverRegistered = NO;
  }
  ...
  [_player addObserver:self forKeyPath:EXVideoPlaybackRateKeyPath options:0 context:nil];
  _playbackRateObserverRegistered = YES;

  const Float64 progressUpdateIntervalMS = _progressUpdateInterval / 1000;
  // @see endScrubbing in AVPlayerDemoPlaybackViewController.m of https://developer.apple.com/library/ios/samplecode/AVPlayerDemo/Introduction/Intro.html
  __weak EXVideo *weakSelf = self;
  ...
  if ([changedProps containsObject:@"src"]) {
    if (_onVideoLoadStart) {
      ...
      if (uri) {
        ...
        if (_onVideoError) {
          NSError *err = RCTErrorWithMessage(@"`src` prop must contain a `uri` property");
          _onVideoError(@{
                          @"error": @{
                              @"code": @(err.code),
                              @"domain": err.domain,
...
- (AVPlayerItem*)playerItemForSource:(NSDictionary *)source
{
  ...
  if (isNetwork || isAsset)  {
    ...
    RCTFatal(RCTErrorWithMessage(@"Source must specify isNetwork or isAsset"));
    return nil;
  }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
  if (object == _playerItem) {
    if ([keyPath isEqualToString:EXVideoStatusKeyPath]) {
    ...
    } else if ([keyPath isEqualToString:EXVideoPlaybackBufferEmptyKeyPath]) {
      _playerBufferEmpty = YES;
    } else if ([keyPath isEqualToString:EXVideoPlaybackLikelyToKeepUpKeyPath]) {
    ...
    if ([keyPath isEqualToString:EXVideoReadyForDisplayKeyPath] && [change objectForKey:NSKeyValueChangeNewKey]) {
