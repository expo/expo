...
@implementation EXAppSplashScreenConfigurationBuilder

+ (EXSplashScreenConfiguration *)parseManifest:(NSDictionary *)manifest
{
  UIColor *backgroundColor = [EXAppSplashScreenConfigurationBuilder parseBackgroundColor:manifest];
  NSString *imageUrl = [EXAppSplashScreenConfigurationBuilder parseImageUrl:manifest];
  SplashScreenImageResizeMode imageResizeMode = [EXAppSplashScreenConfigurationBuilder parseImageResizeMode:manifest];
...
+ (UIColor * _Nonnull)parseBackgroundColor:(NSDictionary *)manifest
{
  ...
  NSString *hexString = [EXAppSplashScreenConfigurationBuilder getStringFromManifest:manifest
                                                                              paths:@[
...
{
  return [EXAppSplashScreenConfigurationBuilder getStringFromManifest:manifest
                                                                paths:@[
...
{
  NSString *resizeMode = [EXAppSplashScreenConfigurationBuilder getStringFromManifest:manifest
                                                                                      paths:@[
                                        ...
                                        paths:(NSArray<NSArray<const NSString *> *> *)paths
{
  for (NSArray<const NSString *> *path in paths) {
    NSString *result = [EXAppSplashScreenConfigurationBuilder getStringFromManifest:manifest path:path];
    if (result) {
      return result;
    }
  }
