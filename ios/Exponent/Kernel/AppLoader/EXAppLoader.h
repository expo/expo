...
- (void)appLoader:(EXAppLoader *)appLoader didLoadOptimisticManifest:(EXRawManifestsRawManifest *)manifest;
- (void)appLoader:(EXAppLoader *)appLoader didLoadBundleWithProgress:(EXLoadingProgress *)progress;
- (void)appLoader:(EXAppLoader *)appLoader didFinishLoadingManifest:(EXRawManifestsRawManifest *)manifest bundle:(NSData *)data;
- (void)appLoader:(EXAppLoader *)appLoader didFailWithError:(NSError *)error;
- (void)appLoader:(EXAppLoader *)appLoader didResolveUpdatedBundleWithManifest:(EXRawManifestsRawManifest * _Nullable)manifest isFromCache:(BOOL)isFromCache error:(NSError * _Nullable)error;
...
@property (nonatomic, readonly) EXRawManifestsRawManifest * _Nullable manifest; // possibly optimistic
@property (nonatomic, readonly) EXRawManifestsRawManifest * _Nullable cachedManifest; // we definitely have this manifest and its bundle on the device
...
- (instancetype)initWithLocalManifest:(EXRawManifestsRawManifest * _Nonnull)manifest;
 ...
 */
- (void)fetchManifestWithCacheBehavior:(EXManifestCacheBehavior)cacheBehavior success:(void (^)(EXRawManifestsRawManifest *))success failure:(void (^)(NSError *))failure;
