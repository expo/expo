...
@property (nonatomic, nullable) EXSyncConfig *config;
@property (nonatomic, nullable) id<EXSyncSelectionPolicy> selectionPolicy;
@property (nonatomic, nullable) id<EXSyncLauncher> appLauncher;
@property (nonatomic, assign) BOOL isEmergencyLaunch;

@end

@implementation EXAppLoaderNew
...
- (instancetype)initWithManifestUrl:(NSURL *)url
{
  if (self = [super init]) {
    _manifestUrl = url;
    _httpManifestUrl = [EXAppLoaderNew _httpUrlFromManifestUrl:_manifestUrl];
...
{
  if (self.delegate) {
    [self.delegate appLoader:self didResolveUpdatedBundleWithManifest:update.rawManifest isFromCache:(status == EXSyncBackgroundUpdateStatusNoManifestAvailable) error:error];
  }
}
...
{
  EXSyncDatabaseManager *updatesDatabaseManager = [EXKernel sharedInstance].serviceRegistry.updatesDatabaseManager;
...
- (void)_startLoaderTask
{
  ...
  _config = [EXSyncConfig configWithDictionary:@{
    @"EXSyncURL": httpManifestUrl.absoluteString,
    @"EXSyncSDKVersion": [self _sdkVersions],
    @"EXSyncScopeKey": httpManifestUrl.absoluteString,
    @"EXSyncReleaseChannel": [EXEnvironment sharedEnvironment].releaseChannel,
    @"EXSyncHasEmbeddedManifest": @([EXEnvironment sharedEnvironment].isDetached),
    @"EXSyncEnabled": @([EXEnvironment sharedEnvironment].areRemoteUpdatesEnabled),
    @"EXSyncLaunchWaitMs": launchWaitMs,
    @"EXSyncCheckOnLaunch": shouldCheckOnLaunch ? @"ALWAYS" : @"NEVER",
    @"EXSyncExpectsSignedManifest": @YES,
    @"EXSyncRequestHeaders": [self _requestHeaders]
  }];
  ...
  _selectionPolicy = [[EXSyncSelectionPolicyFilterAware alloc] initWithRuntimeVersions:sdkVersions];
...
{
  EXSyncLauncherNoDatabase *appLauncher = [[EXSyncLauncherNoDatabase alloc] init];
